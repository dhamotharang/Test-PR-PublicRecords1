...
curr := distribute(file_compid_best,hash(did));
// prev := distribute(dataset(ut.foreign_prod+'~thor_data400::base::best_compidw20090506-194733',Watchdog.Layout_Best,flat),hash(did));
prev := dataset('~thor_data400::base::best_compid_father',Watchdog.Layout_Best,flat);
...
self.Bkrupt_CrtCode_CaseNo	:=	if(l.Bkrupt_CrtCode_CaseNo[1..12]=r.Bkrupt_CrtCode_CaseNo[1..12]
								,'',if(l.Bkrupt_CrtCode_CaseNo='' and r.Bkrupt_CrtCode_CaseNo<>'','*NULL',l.Bkrupt_CrtCode_CaseNo));
self.main_count				:=	if(l.main_count=r.main_count or l.main_count< r.main_count and l.main_count>0
								,0,if(l.main_count=0 and r.main_count>0,-1,l.main_count));
self.search_count			:=	if(l.search_count=r.search_count or l.search_count< r.search_count and l.search_count>0
								,0,if(l.search_count=0 and r.search_count>0,-1,l.search_count));

...
export compID_best(
	...
	out0 := join(distribute(with_wdog(did>0),hash(did)),File_Dl
					...
					,left outer
					,local)
	:persist('~thor_data400::persist::compid::out0');

	compID_out:=project(out0(did>0,trim(compid_in.state)<>''),transform({Layout_compID-[cr]
																		,string16 NEW_CID_ADL
																		,string1 ADL_STABILITY_FLAG:=''
																		,string1 cr:='\n'}

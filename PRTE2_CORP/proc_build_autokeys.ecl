IMPORT RoxieKeyBuild,AutoKeyB2,PRTE,_control, autokeyb,Business_Header_SS,business_header,ut,corp2,doxie,address,corp2_services;
export proc_build_autokeys(string filedate) := 
function
d1 :=distribute(files.DS_corp_Direct,hash(corp_key));
d1_dedup :=dedup(sort(d1,corp_key,local),corp_key,local);
pre_d2:=distribute(files.corp2_cont_Base,hash(corp_key));
...
Layouts.layout_common_with_corp all_business_addresses(corp2.layout_corporate_direct_corp_base le,integer C):=transform
...
Layouts.layout_common_with_corp both_ra_cities(Layouts.layout_common_with_corp le,integer C):=transform
...
Layouts.layout_common_with_corp get_address_for_blank(Layouts.layout_common_with_corp le):=transform
...
Layouts.layout_common_with_corp all_companies(Layouts.layout_common_with_corp le,integer C):=transform
...
Layouts.layout_common_with_corp all_feins(Layouts.layout_common_with_corp le,integer C):=transform
...
corp2_services.assorted_layouts.layout_common all_names(Layouts.layout_common_with_corp le,integer C) :=transform
...
pass_to_build := withaddcontactaddr_corp;

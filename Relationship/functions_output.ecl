EXPORT functions_output := module
	...
	shared getRelCnt(ROW(layout_output.titled) le, string curRel) :=
		function
			outCnt := case(curRel,
	...
	shared dataset(layout_output.normedRelTypeRec) getChildRecs(dataset(layout_output.titled) rel_in) :=
		function
	...
	export convertTitledToKey(dataset(layout_output.titled) rel_in) :=
		function
			childRecs := getChildRecs(rel_in);
			layout_output.key deNorm(layout_output.titled L, dataset(layout_output.normedRelTypeRec) R) :=
				transform
	 ...
	 export convertKeyToTitled(dataset(layout_output.key) rel_in) :=
		function
			relOut := project(rel_in,
												transform(layout_output.titled,

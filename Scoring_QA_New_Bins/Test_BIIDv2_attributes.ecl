...
ip:='~';
   ...
   // jn;
	 dedup_jn:=dedup(jn,all);
	 	...
	 	out_file := output(choosen(dedup_jn,all), ,'~wema::nonfcra::attributes::biidv2' + a1, CSV(heading(single), quote('"')), overwrite,EXPIRE(10));
			
				string out_file_layout := '';
      outfile := dataset('~wema::nonfcra::attributes::biidv2' + a1, typeof(out_file_layout));
	...
	new_bins_remove_superfile:= if(  STD.File.FileExists('~scoringqa::bss::stats1::'+ a1) and 
        ...
        new_bins:= if( not NOTHOR(STD.File.FileExists('~scoringqa::bss::stats1::'+ a1) and STD.File.FileExists('~scoringqa::bss::averages1::'+ a1) and 
   					...
   					FileServices.CreateSuperFile('~scoringqa::bss::dids1::' + a1),rpt30) );
         ...
         old_bins:= if( not NOTHOR(STD.File.FileExists('~scoringqa::bss::stats1::'+ b1) and STD.File.FileExists('~scoringqa::bss::averages1::'+ b1)  and
   					...
   					FileServices.CreateSuperFile('~scoringqa::bss::dids1::' + b1),rpt30_1) );

	 send_file:=	
fileservices.SendEmailAttachText('isabel.ma@lexisnexisrisk.com',					
					'BIIDv2 Attribute Distribution Report',
																				...
																				'BIIDv2_AttributeDistributionShifts_' + a1 + '_vs_' + b1 + '.csv',
																				 ,
																				 ,
...
EXPORT Test_BIIDv2_attributes := 'todo';
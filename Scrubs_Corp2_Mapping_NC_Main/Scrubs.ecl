...
EXPORT FromNone(DATASET(Layout_in_file) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_fed_tax_id_Invalid := Fields.InValid_corp_fed_tax_id((SALT35.StrType)le.corp_fed_tax_id);
    SELF.corp_term_exist_cd_Invalid := Fields.InValid_corp_term_exist_cd((SALT35.StrType)le.corp_term_exist_cd);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_address1_effective_date_Invalid << 22 ) + ( le.corp_filing_date_Invalid << 24 ) + ( le.corp_status_cd_Invalid << 26 ) + ( le.corp_inc_state_Invalid << 27 ) + ( le.corp_inc_date_Invalid << 28 ) + ( le.corp_fed_tax_id_Invalid << 30 ) + ( le.corp_term_exist_cd_Invalid << 32 ) + ( le.corp_foreign_domestic_ind_Invalid << 33 ) + ( le.corp_forgn_state_cd_Invalid << 34 ) + ( le.corp_forgn_state_desc_Invalid << 35 ) + ( le.corp_forgn_date_Invalid << 36 ) + ( le.corp_orig_org_structure_cd_Invalid << 38 ) + ( le.corp_for_profit_ind_Invalid << 39 ) + ( le.corp_agent_assign_date_Invalid << 40 ) + ( le.recordorigin_Invalid << 42 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_fed_tax_id_Invalid := (le.ScrubsBits1 >> 30) & 3;
    ...
    SELF.corp_orig_org_structure_cd_Invalid := (le.ScrubsBits1 >> 38) & 1;
    SELF.corp_for_profit_ind_Invalid := (le.ScrubsBits1 >> 39) & 1;
    SELF.corp_agent_assign_date_Invalid := (le.ScrubsBits1 >> 40) & 3;
    SELF.recordorigin_Invalid := (le.ScrubsBits1 >> 42) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.dt_vendor_first_reported_Invalid,le.dt_vendor_last_reported_Invalid,le.dt_first_seen_Invalid,le.dt_last_seen_Invalid,le.corp_ra_dt_first_seen_Invalid,le.corp_ra_dt_last_seen_Invalid,le.corp_key_Invalid,le.corp_vendor_Invalid,le.corp_state_origin_Invalid,le.corp_process_date_Invalid,le.corp_orig_sos_charter_nbr_Invalid,le.corp_legal_name_Invalid,le.corp_ln_name_type_cd_Invalid,le.corp_address1_effective_date_Invalid,le.corp_filing_date_Invalid,le.corp_status_cd_Invalid,le.corp_inc_state_Invalid,le.corp_inc_date_Invalid,le.corp_fed_tax_id_Invalid,le.corp_term_exist_cd_Invalid,le.corp_foreign_domestic_ind_Invalid,le.corp_forgn_state_cd_Invalid,le.corp_forgn_state_desc_Invalid,le.corp_forgn_date_Invalid,le.corp_orig_org_structure_cd_Invalid,le.corp_for_profit_ind_Invalid,le.corp_agent_assign_date_Invalid,le.recordorigin_Invalid,100);
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT35.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_forgn_state_desc:invalid_fstate_desc:ALLOW'

Import _Control,iesp,ConsumerDisclosure,LN_PropertyV2,ut;
...
//Property Assessment
Mac_Norm(PropertyAssessment,OutPropertyAssessmentSearch,t_FcraDataServicePropertyAssessment);

dNorm :=  OutPropertyAssessmentSearch;
...
//sanity check for did=78108113
OUTPUT(SORT(orphans_ds(did='78108113'),did),NAMED('orphans_List_did_78108113'));
Output(SORT(rolled(version='prop_overrides_keys',did=78108113),did), named('prop_overrides_did_78108113'));

//compare PII
Output(SORT(rolled(version='prop_payload_keys',ln_fares_id='DA0076475186'),did), named('prop_payload_DA0076475186'));


oldPersistenLayout := RECORD(recordof(rolled))
	INTEGER oldPersistentRecId;
END;

oldPersistenLayout recxform(rolled L):= TRANSFORM
	self.oldPersistentRecId := L.persistent_record_id;
	Self :=L;
END;

//recreate persistent rec id for payload
payloadRecByLN_Faers := project(rolled(version='prop_payload_keys',ln_fares_id='DA0076475186'),recxform(left));
payloadPersistentVal :=LN_PropertyV2.fn_append_puid(payloadRecByLN_Faers);
output(payloadPersistentVal, named('prop_payload_DA0076475186_persistent_hash'));

Output(SORT(rolled(version='prop_overrides_keys',ln_fares_id='DA0076475186'),did), named('prop_overrides_DA0076475186'));
//recreate persistent rec id for overrides
overrideRecByLN_Faers :=project(rolled(version='prop_overrides_keys',ln_fares_id='DA0076475186'),recxform(left));
overridePersistentVal :=LN_PropertyV2.fn_append_puid(overrideRecByLN_Faers);
output(overridePersistentVal, named('prop_Overrides_DA0076475186_persistent_hash'));

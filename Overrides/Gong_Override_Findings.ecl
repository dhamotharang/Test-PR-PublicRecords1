...
EXPORT Gong_Override_Findings(DATASET(Override_Layouts.Layout_Get_Orphans) orphansIn, STRING filedate) :=  FUNCTION
	...
	payload_pii_ds := ROLLUP(payload_name_ds,  xform_payload(LEFT), listed_name, prim_name, prim_range, z5, LOCAL);
  	...
  	orphan_layout  PayloadCheck(total_orphans l) := TRANSFORM		
		payload_name_count_ds 	:=  payload_pii_ds(listed_name = l.listed_name,  
													prim_range = l.prim_range,
													prim_name = l.prim_name,  
													z5 = l.z5);		
		...
		SELF.found_in_payload :=  IF (COUNT(payload_name_count_ds) > 0 OR COUNT(payload_did_count_ds) > 0 OR COUNT(payload_recid_count_ds) > 0, 'Y', 'N');  
		SELF := l;
	END;

	total_orphans_checked := PROJECT(total_orphans, PayloadCheck(LEFT));
   	 
	Gong_Orphans := PROJECT(total_orphans_checked(found_in_payload = 'N'), 
								TRANSFORM(File_Override_Orphans.orphan_rec,
									SELF.datagroup := Constants.GONG, 
									SELF.did := (STRING) LEFT.did, 
									SELF.recid := (STRING) LEFT.recid,
									SELF.flag_file_ID := LEFT.flag_file_ID;
									SELF := LEFT));
	OUTPUT(Gong_Orphans, NAMED('Gong_Orphans'));

	#IF(overrides.Constants.GROWTH_CHECK_CALL)			
		build_stats := IF(COUNT(Gong_Orphans) > 0, GrowthCheck(Constants.GONG).BuildStats(filedate, Gong_Orphans));
		build_stats; // this forces the call

		stats_alerts :=  GrowthCheck(Constants.GONG).StatsAlerts;
		...
		result_orphans := IF(~stats_alerts, Gong_Orphans);

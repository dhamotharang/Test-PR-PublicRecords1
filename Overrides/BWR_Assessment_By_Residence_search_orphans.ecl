Import _Control,iesp,ln_propertyv2,ConsumerDisclosure;
...
//assessment by residence
Mac_Norm(AssessmentByResidence,OutAssessmentByResidenceSearch,t_FcraDataServicePropertyAssessment);

dNorm :=   OutAssessmentByResidenceSearch;				 
...
//params:payload keys from prop search only,,prop search,outrecord,assessment by residence.
...
// cmbprop_dist := distribute(cmbprop, HASH32(ln_fares_id,did));

// combined_sorted := sort(cmbprop_dist,ln_fares_id,did,local);

// rolled := Rollup(combined_sorted, xform(left), ln_fares_id,did,local);
...
//did 743206905 sanity check
Output(SORT(rolled(did=743206905),version), named('Both_Overrides_Payload_by_did_743206905'));
OUTPUT(overrided_by_did(did=743206905),NAMED('overrided_did_743206905'));
Output(SORT(rolled(version='prop_payload_keys',did=743206905),did), named('prop_payload_keys_Only_by_did_743206905'));

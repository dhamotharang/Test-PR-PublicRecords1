  ...
  EXPORT HTA := MODULE
	   EXPORT Default := MODULE,VIRTUAL
		 ...
		 EXPORT Atkinson := MODULE(Default)
				...
				EXPORT DATASET(Types.Element) hReflection := ML_Mat.Sub(IdentityM, ML_Mat.Scale(ML_Mat.Mul(houseVec,ML_Mat.Trans(houseVec)),2));
		 END;
		 ...
		 EXPORT Golub := MODULE(Default)
				...
				EXPORT DATASET(Types.Element) hReflection := ML_Mat.Sub(IdentityM, ML_Mat.Scale(ML_Mat.Mul(hV,ML_Mat.Trans(hV)),Beta));
		 END;

IMPORT ADDRESS_ATTRIBUTES, LN_PROPERTYV2, UT;

//Business Header SEC RANGE Counts
Address_Attributes.Layouts.BIP_RISK getBIPCounts(Address_Attributes.file_BIP_base l, Address_Attributes.file_BIP_count r) := TRANSFORM
	SELF.zip 					:= l.zip;
	...
	SELF := [];
END;
BIP_Count := JOIN(Address_Attributes.file_BIP_base, Address_Attributes.file_BIP_count,
...
d_sec_count := DISTRIBUTE(Address_Attributes.file_BIP_count, HASH32(zip,prim_range,prim_name,addr_suffix,predir,postdir));
...
Address_Attributes.Layouts.BIP_RISK joinDNBFlags(with_Prop l, Address_Attributes.file_BIP_dnb r) := TRANSFORM
	SELF.zip 					:= l.zip;
	SELF.prim_range 	:= l.prim_range;
	SELF.prim_name 		:= l.prim_name;
	SELF.addr_suffix 	:= l.addr_suffix;
...
with_DNB := JOIN(with_Prop, Address_Attributes.file_BIP_dnb,		
		LEFT.zip 				 = RIGHT.zip AND
		LEFT.prim_range  = RIGHT.prim_range AND 
		LEFT.prim_name 	 = RIGHT.prim_name AND 
		LEFT.addr_suffix = RIGHT.addr_suffix AND 
...
Address_Attributes.Layouts.BIP_RISK addVelocity(with_DNB l, Address_Attributes.file_address_velocity r) := TRANSFORM
	SELF.zip 					:= l.zip;
	SELF.prim_range 	:= l.prim_range;
	SELF.prim_name 		:= l.prim_name;
	SELF.addr_suffix 	:= l.addr_suffix;

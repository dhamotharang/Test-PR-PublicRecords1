EXPORT RV_Scores_Attributes_V5_XML_Prescreen_Macro(fcraroxie_IP, neutralroxie_IP, Thread, Timeout, Retry, Input_file_name, Output_file_name, records_ToRun):= FUNCTIONMACRO
...
IMPORT Models, iESP, Risk_Indicators, RiskWise, RiskProcessing, UT, Gateway;
    ...
    // models		
		model1 := 'RVP1503_1_0'; //  Prescreen model not implemented yet in RV5.
		model2 := ''; 
		model3 := ''; 
		model4 := ''; 
	  ...
	  //************** INPUT FILE GENERATION ****************	
		...
		soapLayout prescreen_intoSOAP(ds_raw_input le, UNSIGNED4 c) := TRANSFORM
	  ...
	  //ds_soap_in
		prescreen_soap_in := DISTRIBUTE(PROJECT(ds_raw_input, prescreen_intoSOAP(LEFT, COUNTER)), RANDOM());
		...
		layout_Soap_output myFail(prescreen_soap_in le) := TRANSFORM
	  ...
	  //*********** PERFORMING SOAPCALL TO ROXIE ************ 
		...
		prescreen_Soap_output := SOAPCALL(prescreen_soap_in, 
						FCRARoxieIP,
    ...
    //Mapping Version5 attributes 	
	  ...
	  //Appeding internal extras to Soap output file 
		Global_output_lay normit(flattened_result L, prescreen_soap_in R) := TRANSFORM
		...
		ds_with_extras := JOIN(flattened_result,prescreen_soap_in,LEFT.accountnumber=(STRING)RIGHT.RiskView2Request[1].user.accountnumber,normit(LEFT,RIGHT));
		...
		// output(prescreen_soap_in,named('prescreen_soap_in'));

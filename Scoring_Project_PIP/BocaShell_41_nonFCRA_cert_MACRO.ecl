EXPORT BocaShell_41_nonFCRA_cert_MACRO( bs_version, nonfcraroxie_IP,neutralroxie_IP, Thread, Timeout, Retry, Input_file_name,Output_file_name, records_ToRun, retro_date = 999999):= functionmacro
			...
			RemoveFares := scoring_project_pip.User_Settings_Module.BocaShell_41_NONFCRA_settings.RemoveFares;	// change this to TRUE for FARES filtering
			LeadIntegrityMode := scoring_project_pip.User_Settings_Module.BocaShell_41_NONFCRA_settings.LeadIntegrityMode;  // change this to TRUE for LeadIntegrity modeling

			DRM := scoring_project_pip.User_Settings_Module.BocaShell_41_NONFCRA_settings.DRM;  // byte 6, if 1, restricts experian, byte 8, if 1, restricts equifax, 
			                                                                                    // byte 10 restricts Transunion, 12 restricts ADVO, 13 restricts bureau deceased data
			DPPA := scoring_project_pip.User_Settings_Module.BocaShell_41_NONFCRA_settings.DPPA;
			GLB := scoring_project_pip.User_Settings_Module.BocaShell_41_NONFCRA_settings.GLB;
...
// output(ds_soap_in);
// output(ds_soap_output);
// output(ds_soap_output_pjt);
			return final_output;

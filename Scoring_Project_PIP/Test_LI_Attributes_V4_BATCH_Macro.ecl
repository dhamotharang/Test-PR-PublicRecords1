EXPORT Test_LI_Attributes_V4_BATCH_Macro(roxie_ip, Gateway_dummy, Thread, Timeout, Retry, Input_file_name, Output_file_name, records_ToRun):= FUNCTIONMACRO
...
IMPORT Models, iESP, Risk_Indicators, RiskWise, UT, RiskProcessing ;
    ...
    //*********** SETTINGS ********************************
	  ...
	  GLB:=Scoring_Project_PIP.User_Settings_Module.LI_Attributes_V4_BATCH_Generic_msn1106_0_settings.GLB;		
		DPPA:=Scoring_Project_PIP.User_Settings_Module.LI_Attributes_V4_BATCH_Generic_msn1106_0_settings.DPPA;
		HistoryDate := 201207; //archive mode
	  ...
	  ds_raw_input := IF(no_of_records = 0, 
	    ...
	    //*********** LI Attributes V4 BATCH SETUP AND SOAPCALL ******************
    ...
    //**************** ADDING DID'S & INTERNAL EXTRAS TO RESULTS *************************** 
		...
		res:=JOIN(ds_slim, did_results, LEFT.accountnumber = (STRING)RIGHT.AcctNo,
																TRANSFORM(Global_output_lay,
																						SELF.DID := RIGHT.DID;   									
																						SELF := LEFT;   						
																					), left outer);

		 ds_with_extras := JOIN(res,soap_in, LEFT.accountnumber =(STRING)RIGHT.batch_in[1].acctno,
																TRANSFORM(Global_output_lay,

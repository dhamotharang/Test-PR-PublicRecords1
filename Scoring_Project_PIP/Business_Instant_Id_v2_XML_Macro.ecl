EXPORT Business_Instant_Id_v2_XML_Macro(roxie_ip,Gateway_dummy, Thread, Timeout, Retry, Input_file_name,Output_file_name, records_ToRun):= FUNCTIONMACRO
...
_OverRideExperianRestriction := FALSE;
		...
		recordsToRun := records_ToRun; // Set to 0 (zero) to run all records.
		eyeball      := 10;
		// threads      := Thread;
		// retry := retry;
		// timeout := timeout;

// RoxieIP := roxie_ip; // Staging/Cert
		integer retry := retry;
...
// File names.
// inputFile :='~dvstemp::in::hughle01_4619_biid2_input';
...
// Round #1:
BIID20_results_raw := 
				SOAPCALL(BIID20_input, RoxieIP,
				'BusinessInstantID20_Services.InstantID20_Batch_Service', {BIID20_input}, 
...
// OUTPUT(CHOOSEN(BIID20Final         , eyeball), NAMED(outputFile));
// OUTPUT(CHOOSEN(BIID20Failed_Inputs, eyeball), NAMED(outputFile + '_Failed_Inputs'));

final_output:= OUTPUT(BIID20Final        , , outputFile , thor,compressed, OVERWRITE);
// OUTPUT(BIID20Failed_Inputs, , outputFile + '_Failed_Inputs', thor,compressed, OVERWRITE);

// test_lay:=record
// string acctno;
// string bvi;
// string rep1_cvi;
// string bus2exec_index_rep1;
// end;

// pjt:=project(BIID20Final,transform(test_lay,self:=left;));
// OUTPUT(pjt        , , outputFile, thor,compressed, OVERWRITE);

RETURN final_output;
// EXPORT biid_test_macro := 'todo';
ENDMACRO;
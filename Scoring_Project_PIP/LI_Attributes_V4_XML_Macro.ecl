EXPORT LI_Attributes_V4_XML_Macro(roxie_ip, Gateway_dummy, Thread, Timeout, Retry, Input_file_name, Output_file_name, records_ToRun):= FUNCTIONMACRO
...
IMPORT Models, iESP, Risk_Indicators, RiskWise, UT, RiskProcessing, scoring ;
		...
		GLB:=Scoring_Project_PIP.User_Settings_Module.LI_Attributes_V4_XML_Generic_msn1106_0_settings.GLB;
		DRM:=Scoring_Project_PIP.User_Settings_Module.LI_Attributes_V4_XML_Generic_msn1106_0_settings.DRM;
		IncludeVersion4:=if(Scoring_Project_PIP.User_Settings_Module.LI_Attributes_V4_XML_Generic_msn1106_0_settings.IncludeVersion4,4,3);
    DPPA:=Scoring_Project_PIP.User_Settings_Module.LI_Attributes_V4_XML_Generic_msn1106_0_settings.DPPA;
		HistoryDate := 999999;
    ...
    //*********** LI Attributes V4 XML SETUP AND SOAPCALL ******************
		...
		layout_soap_input append_settings(ds_raw_input le) := TRANSFORM
			...
			o := PROJECT(ut.ds_oneRecord, TRANSFORM(iesp.leadintegrity.t_LeadIntegrityOptions, SELF.AttributesVersionRequest := (STRING)IncludeVersion4; SELF := []));
    ...
    //Soap output layout
		...
		layout_Soap_output myFail2(soap_in le) := TRANSFORM
			SELF.errorcode := FAILCODE + FAILMESSAGE;
			SELF.Result.InputEcho.Seq:=le.AccountNumber;
			SELF := le;
			SELF := [];
		END;
    ...
    END;
		...
		ds_with_extras := JOIN(soap_in, ds_slim,LEFT.LeadIntegrityRequest[1].SearchBy.Seq=RIGHT.seq,

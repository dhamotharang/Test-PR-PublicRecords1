EXPORT Generic_Tracking_Report_Boca_Shells(ds, layout , type_, join_by) := Functionmacro
import ut;
...
nonfcra_ds_prev := distribute(dataset('~'+ nonfcra_p_file_name, layout, thor),(integer)join_by);
...
ds_results2 := Scoring_Project_PIP.Compare_dsets_macro_email(clean_ds_1_flat, clean_ds_2_flat, [join_by], thresh);

ds_results := ds_results2(field not in ['aircraft_build_date', 'historydatetimestamp', 'property_build_date', 'time_ms']);

EXPORT macCleanProviderDataV2 (Infile, Input_ProviderKey = '',Input_NPINumber = '',Input_DEANumber = '',Input_Taxonomy = '',Input_ProviderTaxID = '',
															 ...
															 Input_UserDefinedField08 = '',Input_UserDefinedField09 = '',Input_UserDefinedField10 = '',Input_ProviderDOB = '',Input_ProviderSSN = '',appendPrefix = '\'\'') := FUNCTIONMACRO
	 ...
	 Provider_Clean_REC := RECORD
		...
		STRING8  #EXPAND(appendPrefix + 'ProviderDOB');		
		STRING9  #EXPAND(appendPrefix + 'ProviderSSN');		
	END;
	...
	Provider_Clean_REC clean (Infile L) := transform
		...
		STRING9  IDOB						:= #IF ( #TEXT(Input_ProviderDOB) <> '' ) 	(STRING) L.Input_ProviderDOB	#ELSE '' #END;
		STRING9  ISSN						:= #IF ( #TEXT(Input_ProviderSSN) <> '' ) 	(STRING) L.Input_ProviderSSN	#ELSE '' #END;
		...
		ProviderAddressLinea := REGEXREPLACE('-|\\¦|@|#|\\$|%|¬|\\&|\\*|\\(|\\)|_|\\+|=|:|;|!|¢|\\}|\\{|,|\\.|/|\\?|\\||\\\\|<|>|"|`|\'|~',HealthCareProvider.CleanData.fUpperCleanSpaces(HealthCareProvider.CleanData.fReplaceUnprintable(IAddressLine1)),' ');
		ProviderAddressLineb := REGEXREPLACE('-|\\¦|@|#|\\$|%|¬|\\&|\\*|\\(|\\)|_|\\+|=|:|;|!|¢|\\}|\\{|,|\\.|/|\\?|\\||\\\\|<|>|"|`|\'|~',HealthCareProvider.CleanData.fUpperCleanSpaces(HealthCareProvider.CleanData.fReplaceUnprintable(IAddressLine2)),' ');	
		...
		CleanedAddress 	:= Address.CleanAddressFieldsFips(Address.CleanAddress182(AddressLine1, AddressLine2)).addressrecord;																					 
					...
					SELF.#EXPAND(appendPrefix + 'ProviderDOB')	:=	IF(HealthCareProvider.isValidDOB (IDOB),IDOB,'');
					SELF.#EXPAND(appendPrefix + 'ProviderSSN')	:=	IF(HealthCareProvider.isValidSSN (ISSN),ISSN,'');
					SELF := L;
					SELF := [];
		END;

...
f:= IF(no_of_records > 0, CHOOSEN(DATASET(ut.foreign_prod + infile_name, BTSTCustom, thor), no_of_records),
                            DATASET(ut.foreign_prod + infile_name, BTSTCustom, thor ));



rawInput := DISTRIBUTE(f, RANDOM());
...
soap_in := DISTRIBUTE(PROJECT(rawInput, to_soap(LEFT)), Random());

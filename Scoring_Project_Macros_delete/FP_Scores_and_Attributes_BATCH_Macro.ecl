EXPORT FP_Scores_and_Attributes_BATCH_Macro (roxie_ip,Gateway, Thread, Timeout, Retry, Input_file_name,Output_file_name, records_ToRun):= functionmacro
...
f1 := IF(no_of_records = 0, dataset(ut.foreign_prod + Infile_name, prii_layout, csv(quote('"'))), choosen(dataset (ut.foreign_prod + Infile_name, prii_layout, csv(quote('"'))), no_of_records));

                           
f := DISTRIBUTE(f1, RANDOM());  //Turn on if not testing														
...
batch_response_layout myFail(dist_dataset le) := TRANSFORM
...
transform(d_attributes_norm,
  self.accountnumber := right.orig_account;
...
fdInput_errors := join(fdInput_ALL, fdInput, left.batch_in[1].acctno=right.batch_in[1].acctno, transform(d_attributes_norm,
self.errorcode := doxie.ErrorCodes(301);
self.accountnumber := left.orig_account;
...
final := sort(ungroup(p_final + fdInput_errors), accountnumber);


op_final := output(final,, outfile_name, CSV(HEADING(single), QUOTE('"')), OVERWRITE);

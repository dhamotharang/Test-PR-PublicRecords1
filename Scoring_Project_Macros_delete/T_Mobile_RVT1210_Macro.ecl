...
f1 := IF(no_of_records <= 0, dataset(ut.foreign_prod + infile_name, input_layout, thor ),
                            CHOOSEN(DATASET(ut.foreign_prod + infile_name, input_layout, thor), no_of_records));
...
l t_f(ds le, INTEGER c) := TRANSFORM
	...
	self.DataRestrictionMask := '00000000000101';  // Tmobile specific DRM, pulled from the Roxie logs

...
EXPORT Instant_ID_BATCH_Macro (roxie_ip,Gateway, Thread, Timeout, Retry, Input_file_name,Output_file_name, records_ToRun):= functionmacro
...
f1 := IF(no_of_records = 0, dataset(ut.foreign_prod + Infile_name, layout_input, csv(quote('"'))), choosen(dataset (ut.foreign_prod + Infile_name, layout_input, csv(quote('"'))), no_of_records));
...
soap_in := DISTRIBUTE(PROJECT(p_f, make_rv_in(LEFT, counter)), Random());
...
op_final := output(j_f,, outfile_name, CSV(HEADING(2), QUOTE('"')), OVERWRITE);

EXPORT RV_Attributes_V3_XML_Macro (fcraroxie_IP,neutralroxie_IP, Thread, Timeout, Retry, Input_file_name,Output_file_name, records_ToRun):= functionmacro
...
//	self.HistoryDateYYYYMM := le.historydateyyyymm; //999999;
  ...
  // self.gateways := dataset([{'neutralroxie', riskwise.shortcuts.prod_batch_neutral}], risk_indicators.Layout_Gateways_In);
	...
	// self.gateways := dataset([{'neutralroxie', riskwise.shortcuts.staging_neutral_roxieIP}], risk_indicators.Layout_Gateways_In);	
		self.gateways := dataset([{'neutralroxie', neutralroxieIP}], risk_indicators.Layout_Gateways_In); //pointing to QA Roxie as per Nathan's request
...
op := output(count(j_f(errorcode<>'')),named('rva30_error_count'));

...
f1 := IF(no_of_records = 0, 
                DATASET(ut.foreign_prod +Infile_name, rec_input, CSV(HEADING(single), QUOTE('"')) ),
...
indata := Distribute(project(f1, TRANSFORM(layout, self.accountnumber := left.account_number;
																							self := left;)), Random());

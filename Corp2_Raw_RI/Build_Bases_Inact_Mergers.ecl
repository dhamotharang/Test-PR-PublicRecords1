...
EXPORT Build_Bases_Inact_Mergers(
	STRING																			pfiledate,
	STRING																			pversion,
	BOOLEAN                                     pUseOtherEnvironment,
	DATASET(Corp2_Raw_RI.Layouts.MergersIn)			pInMergers   	= Corp2_Raw_RI.Files(pfiledate,pUseOtherEnvironment).Input.Inactive_Mergers.logical,
	DATASET(Corp2_Raw_RI.Layouts.MergersBase)		pBaseMergers 	= IF(Corp2_Raw_RI._Flags.Base.Inactive_Mergers, Corp2_Raw_RI.Files(,pUseOtherEnvironment := FALSE).Base.Inactive_Mergers.qa, 	DATASET([], Corp2_Raw_RI.Layouts.MergersBase))
) := MODULE
	...
	tools.mac_WriteFile(Corp2_Raw_RI.Filenames(pversion).Base.Inactive_Mergers.New, baseactions, Build_Mergers_Base);

	EXPORT All := IF(tools.fun_IsValidVersion(pversion),
									 Build_Mergers_Base,
									 OUTPUT('No Valid version parameter passed, skipping Corp2_Raw_RI.Build_Bases_Inact_Mergers attribute')
									 );

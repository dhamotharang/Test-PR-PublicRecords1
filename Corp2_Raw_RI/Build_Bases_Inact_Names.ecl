...
EXPORT Build_Bases_Inact_Names(
	STRING																		pfiledate,
	STRING																		pversion,
	BOOLEAN                                   pUseOtherEnvironment,
	DATASET(Corp2_Raw_RI.Layouts.NamesIn)			pInNames   	= Corp2_Raw_RI.Files(pfiledate,pUseOtherEnvironment).Input.Inactive_Names.logical,
	DATASET(Corp2_Raw_RI.Layouts.NamesBase)		pBaseNames 	= IF(Corp2_Raw_RI._Flags.Base.Inactive_Names, Corp2_Raw_RI.Files(,pUseOtherEnvironment := FALSE).Base.Inactive_Names.qa, 	DATASET([], Corp2_Raw_RI.Layouts.NamesBase))
) := MODULE
	...
	tools.mac_WriteFile(Corp2_Raw_RI.Filenames(pversion).Base.Inactive_Names.New, baseactions, Build_Names_Base);

	EXPORT All := IF(tools.fun_IsValidVersion(pversion),
									 Build_Names_Base,
									 OUTPUT('No Valid version parameter passed, skipping Corp2_Raw_RI.Build_Bases_Inact_Names attribute')
									 );

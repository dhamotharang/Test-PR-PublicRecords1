...
EXPORT Build_Bases_Inact_Amendments(
	STRING																				 pfiledate,
	STRING																				 pversion,
	BOOLEAN                                        pUseOtherEnvironment,
	DATASET(Corp2_Raw_RI.Layouts.AmendmentsIn)		 pInAmendments   	= Corp2_Raw_RI.Files(pfiledate,pUseOtherEnvironment).Input.Inactive_Amendments.logical,
	DATASET(Corp2_Raw_RI.Layouts.AmendmentsBase)	 pBaseAmendments 	= IF(Corp2_Raw_RI._Flags.Base.Inactive_Amendments, Corp2_Raw_RI.Files(,pUseOtherEnvironment := FALSE).Base.Inactive_Amendments.qa, 	DATASET([], Corp2_Raw_RI.Layouts.AmendmentsBase))
) := MODULE
	...
	tools.mac_WriteFile(Corp2_Raw_RI.Filenames(pversion).Base.Inactive_Amendments.New, baseactions, Build_Amendments_Base);

	EXPORT All := IF(tools.fun_IsValidVersion(pversion),
									 Build_Amendments_Base,
									 OUTPUT('No Valid version parameter passed, skipping Corp2_Raw_RI.Build_Bases_Inact_Amendments attribute')
									 );

...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Corp2_Mapping.LayoutsCommon.Stock)
    ...
    UNSIGNED1 stock_addl_info_Invalid;
    UNSIGNED1 stock_stock_description_Invalid;
  END;
...
EXPORT FromNone(DATASET(Corp2_Mapping.LayoutsCommon.Stock) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.stock_addl_info_Invalid := Fields.InValid_stock_addl_info((SALT34.StrType)le.stock_addl_info);
    SELF.stock_stock_description_Invalid := Fields.InValid_stock_stock_description((SALT34.StrType)le.stock_stock_description);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Corp2_Mapping.LayoutsCommon.Stock);
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.corp_key_Invalid << 0 ) + ( le.corp_vendor_Invalid << 2 ) + ( le.corp_state_origin_Invalid << 3 ) + ( le.corp_process_date_Invalid << 4 ) + ( le.corp_sos_charter_nbr_Invalid << 6 ) + ( le.stock_shares_issued_Invalid << 8 ) + ( le.stock_change_date_Invalid << 9 ) + ( le.stock_addl_info_Invalid << 11 ) + ( le.stock_stock_description_Invalid << 12 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.stock_shares_issued_Invalid := (le.ScrubsBits1 >> 8) & 1;
    SELF.stock_change_date_Invalid := (le.ScrubsBits1 >> 9) & 3;
    SELF.stock_addl_info_Invalid := (le.ScrubsBits1 >> 11) & 1;
    SELF.stock_stock_description_Invalid := (le.ScrubsBits1 >> 12) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    stock_addl_info_ALLOW_ErrorCount := COUNT(GROUP,h.stock_addl_info_Invalid=1);
    stock_stock_description_ENUM_ErrorCount := COUNT(GROUP,h.stock_stock_description_Invalid=1);
  END;
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.corp_key_Invalid,le.corp_vendor_Invalid,le.corp_state_origin_Invalid,le.corp_process_date_Invalid,le.corp_sos_charter_nbr_Invalid,le.stock_shares_issued_Invalid,le.stock_change_date_Invalid,le.stock_addl_info_Invalid,le.stock_stock_description_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_corp_key(le.corp_key_Invalid),Fields.InvalidMessage_corp_vendor(le.corp_vendor_Invalid),Fields.InvalidMessage_corp_state_origin(le.corp_state_origin_Invalid),Fields.InvalidMessage_corp_process_date(le.corp_process_date_Invalid),Fields.InvalidMessage_corp_sos_charter_nbr(le.corp_sos_charter_nbr_Invalid),Fields.InvalidMessage_stock_shares_issued(le.stock_shares_issued_Invalid),Fields.InvalidMessage_stock_change_date(le.stock_change_date_Invalid),Fields.InvalidMessage_stock_addl_info(le.stock_addl_info_Invalid),Fields.InvalidMessage_stock_stock_description(le.stock_stock_description_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.stock_addl_info_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.stock_stock_description_Invalid,'ENUM','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_sos_charter_nbr','stock_shares_issued','stock_change_date','stock_addl_info','stock_stock_description','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_shares','invalid_date','invalid_stock_addl_info','invalid_stock_desc','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT34.StrType)le.corp_key,(SALT34.StrType)le.corp_vendor,(SALT34.StrType)le.corp_state_origin,(SALT34.StrType)le.corp_process_date,(SALT34.StrType)le.corp_sos_charter_nbr,(SALT34.StrType)le.stock_shares_issued,(SALT34.StrType)le.stock_change_date,(SALT34.StrType)le.stock_addl_info,(SALT34.StrType)le.stock_stock_description,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'stock_addl_info:invalid_stock_addl_info:ALLOW'
          ,'stock_stock_description:invalid_stock_desc:ENUM','UNKNOWN');
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_stock_addl_info(1)
          ,Fields.InvalidMessage_stock_stock_description(1),'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.stock_addl_info_ALLOW_ErrorCount
          ,le.stock_stock_description_ENUM_ErrorCount,0);
      SELF.rulepcnt := 100 * CHOOSE(c
          ...
          ,le.stock_change_date_ALLOW_ErrorCount,le.stock_change_date_CUSTOM_ErrorCount
          ,le.stock_addl_info_ALLOW_ErrorCount
          ,le.stock_stock_description_ENUM_ErrorCount,0) / le.TotalCnt + 0.5;
    END;

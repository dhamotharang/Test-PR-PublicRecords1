...
//f := dataset('~dvstemp::in::GE_Samp_BIID', sd2i, csv(quote('"')));
f := enth(dataset('~dvstemp::in::GE_Samp_BIID', sd2i, csv(quote('"'))),5);
output(f);
...
Layout_Business_Shell_Input into_bus_input(f le, integer C) := transform
	...
	self.companyname:= le.employername2;
	self.addr := le.streetaddress2;
	self.city := le.city2;
	self.state := le.state2;
	self.zip := le.zip2;
	self.taxidnumber := le.ssn2;
	self.businessphone := le.homephone2;
	self.representativefirstname := le.firstname;
	self.representativelastname  := le.lastname;
	self.representativeaddr := le.streetaddress;
	self.representativecity := le.city;
	self.representativestate := le.state;
	self.representativezip := le.zip;
	self.representativessn := le.ssn;
	self.representativedob := le.dateofbirth;
...
output(final);
output(final,, '~dvstemp::out::business_shell', overwrite);

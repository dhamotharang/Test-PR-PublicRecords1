...
EXPORT EmpIDCompareService := MACRO
  IMPORT SALT32,BIPV2_EmpID_Down,ut;
...
odl := PROJECT(CHOOSEN(BIPV2_EmpID_Down.Keys(BFile).Candidates(EmpID=EmpIDone),100000),BIPV2_EmpID_Down.match_candidates(BFile).layout_candidates);
odr := PROJECT(CHOOSEN(BIPV2_EmpID_Down.Keys(BFile).Candidates(EmpID=EmpIDTwo),100000),BIPV2_EmpID_Down.match_candidates(BFile).layout_candidates);
...
mtch := BIPV2_EmpID_Down.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,EmpIDone,EmpIDtwo,0,0}],BIPV2_EmpID_Down.match_candidates(BFile).layout_matches));

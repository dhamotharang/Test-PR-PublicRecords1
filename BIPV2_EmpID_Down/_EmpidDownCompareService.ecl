import BIPV2_EmpID_Down,tools;

EXPORT _EmpidDownCompareService(
...
function
  ...
  kFile := BIPV2_EmpID_Down.Keys(BFile/*,version*/);

  odl   := PROJECT(CHOOSEN(KFile.Candidates(Empid=Empidone),100000),BIPV2_EmpID_Down.match_candidates(BFile).layout_candidates);
  odr   := PROJECT(CHOOSEN(KFile.Candidates(Empid=EmpidTwo),100000),BIPV2_EmpID_Down.match_candidates(BFile).layout_candidates);
  k     := KFile.Specificities_Key;
  s     := GLOBAL(PROJECT(k,BIPV2_EmpID_Down.Layout_Specificities.R)[1]);
  odlv  := BIPV2_EmpID_Down.Debug(BFile,s).RolledEntities(odl);
  odrv  := BIPV2_EmpID_Down.Debug(BFile,s).RolledEntities(odr);

  // BIPV2_EmpID_Down.match_candidates(BFile).layout_attribute_matches ainto(KFile.Attribute_Matches le) := TRANSFORM
    // SELF := le;
  // END;
  ...
  mtch  := BIPV2_EmpID_Down.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,Empidone,Empidtwo,0,0}],BIPV2_EmpID_Down.match_candidates(BFile).layout_matches)/*,am*/);

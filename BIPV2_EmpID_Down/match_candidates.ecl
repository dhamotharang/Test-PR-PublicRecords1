...
pj5 do_computes(pj5 le) := TRANSFORM
  SELF := le;
END;
SHARED propogated := PROJECT(pj5,do_computes(left)) : PERSIST('~temp::EmpID::BIPV2_EmpID_Down::mc_props::EmpID',EXPIRE(Config.PersistExpire)); // to allow to 'jump' over an exported value
...
SHARED Annotated := DISTRIBUTE(j0,hash(EmpID)) : PERSIST('~temp::EmpID::BIPV2_EmpID_Down::mc',EXPIRE(Config.PersistExpire)); // Distributed for keybuild case

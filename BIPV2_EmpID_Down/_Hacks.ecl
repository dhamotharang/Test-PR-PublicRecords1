// -- BIPV2_EmpID_Down._Hacks('BIPV2_EmpID_Down',,true).do_all_hacks;
import SALTTOOLS22,STD,wk_ut,tools;
EXPORT _Hacks(
   string   pModule               = 'BIPV2_EmpID_Down'
...
// - added keysuffix to changes filename.  so it will be unique per build and prevent superfile error too.
  // import bipv2;
  // EXPORT OutputChanges := OUTPUT(MM.IdChanges,,'~temp::EmpID::BIPV2_EmpID_Down::changes_it'+iter+'_'+ bipv2.KeySuffix,OVERWRITE,COMPRESSED);// Changes made
  
    dme := dataset([{
          'SHARED ChangeName := \'[~]temp::EmpID::BIPV2_EmpID_Down::changes_it\'[+]iter;'
      ,''
      ,   'import bipv2;\n'

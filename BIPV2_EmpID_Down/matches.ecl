...
mjs0 := o : PERSIST('~temp::EmpID::BIPV2_EmpID_Down::mj::0',EXPIRE(Config.PersistExpire));
 
RETURN  mjs0;
ENDMACRO;
SHARED all_mjs := MAC_DoJoins(h,match_join);
EXPORT All_Matches := all_mjs : PERSIST('~temp::EmpID::BIPV2_EmpID_Down::all_m',EXPIRE(Config.PersistExpire)); // To by used by rcid and EmpID
SALT32.mac_avoid_transitives(All_Matches,EmpID1,EmpID2,Conf,DateOverlap,Rule,o);
EXPORT PossibleMatches := o : PERSIST('~temp::EmpID::BIPV2_EmpID_Down::mt',EXPIRE(Config.PersistExpire));
EXPORT Matches := PossibleMatches(Conf>=MatchThreshold) : PERSIST('~temp::EmpID::BIPV2_EmpID_Down::Matches::Matches',EXPIRE(Config.PersistExpire));
...
EXPORT PreIDs := BIPV2_EmpID_Down.Fields.UIDConsistency(ih_thin); // Export whole consistency module
EXPORT PostIDs := BIPV2_EmpID_Down.Fields.UIDConsistency(Patched_Infile_thin); // Export whole consistency module

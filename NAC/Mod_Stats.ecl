...
EXPORT BenefitMonthCnt():=function
	...
	RETURN sort(table(nac.Files().Base,{
																Case_Benefit_Month
...
SHARED infile:=dataset('~nac::in::'+fname,{string75 fn { virtual(logicalfilename)},NAC.Layouts.load},flat);
...
r tr1(withRC l, integer c):=transform
	...
	self.err:=choose(c
				// case_state_abbreviation
				,if(l.case_state_abbreviation in NAC.Mod_Sets.Consortium,'','E006')
				// Case_Benefit_Type
				,if(l.Case_Benefit_Type in NAC.Mod_Sets.Benefit_Type,'','E015')
				// Case_Benefit_Month
				...
				,map(l.Case_Physical_Address_Street_1=''
							or trim(regexreplace(NAC.Mod_Sets.RegexBadAddress,StringLib.StringCleanSpaces(l.Case_Physical_Address_Street_1),'$2',nocase)) in NAC.Mod_Sets.SetBadAddress
									=> 'W003'
				...
				// Case_Physical_Address_State
				,if(l.Case_Physical_Address_State in NAC.Mod_Sets.States,'','W003')
				// Case_Physical_Address_Zip
				...
				,map(l.Case_Mailing_Address_Street_1=''
							or trim(regexreplace(NAC.Mod_Sets.RegexBadAddress,StringLib.StringCleanSpaces(l.Case_Mailing_Address_Street_1),'$2',nocase)) in NAC.Mod_Sets.SetBadAddress
									=> 'W003'
				...
				// Case_Mailing_Address_State
				,if(l.Case_Mailing_Address_State in NAC.Mod_Sets.States,'','W003')
				// Case_Mailing_Address_Zip
				...
				,''
				// Client_Gender
				,if(l.Client_Gender in NAC.Mod_Sets.Gender,'','W003')
				// Client_Race
				,if(l.Client_Race in NAC.Mod_Sets.Race,'','W003')
				// Client_Ethnicity
				,if(l.Client_Ethnicity in NAC.Mod_Sets.Ethnicity,'','W003')
				// Client_SSN
				,if(l.Client_SSN_Type_Indicator=NAC.Mod_Sets.Actual_Type and length(regexreplace('[^0-9]',l.Client_SSN,''))<>9,'W003','')
				// Client_SSN_Type_Indicator
				,if(l.Client_SSN_Type_Indicator in NAC.Mod_Sets.SSN_Type,'','W003')
				// Client_DOB
				,if(length(trim(l.Client_DOB,left,right))=8,'','W003')
				// Client_DOB_Type_Indicator
				,if(l.Client_DOB_Type_Indicator in NAC.Mod_Sets.DOB_Type,'','W003')
				// Client_Eligible_Status_Indicator
				,if(l.Client_Eligible_Status_Indicator in NAC.Mod_Sets.Eligible_Status,'','W003')
				// Client_Eligible_Status_Date
...
r tr2(r l,integer c):=transform
	...
	self.err:=choose(c
				...
				,map(//an address may be usable if:
					// a) street1 is not blank or poor
					 ...
					 // b.1) we either have a city and state
						or	(  (l.Case_Physical_Address_City='' or l.Case_Physical_Address_State not in NAC.Mod_Sets.States)
					 // b.2) or zip
					...
					// a) street1 is not blank or poor
						...
						or trim(regexreplace(NAC.Mod_Sets.RegexBadAddress,StringLib.StringCleanSpaces(l.Case_Mailing_Address_Street_1),'$2',nocase)) in NAC.Mod_Sets.SetBadAddress
					 // b.1) we either have a city and state
						or	(  (l.Case_Mailing_Address_City='' or l.Case_Mailing_Address_State not in NAC.Mod_Sets.States)
					 // b.2) or zip
				...
				,map(l.Client_Eligible_Status_Indicator in NAC.Mod_Sets.Eligible_Status
							and ((unsigned)l.Client_Eligible_Status_Date=0 or length(trim(l.Client_Eligible_Status_Date,left,right))<>8)
								=> 'W008'
						,l.Client_Eligible_Status_Indicator not in NAC.Mod_Sets.Eligible_Status

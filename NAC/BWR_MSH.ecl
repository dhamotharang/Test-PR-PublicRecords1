import	NAC, lib_FileServices, lib_StringLib, lib_ThorLib, Ut, Std;
...
string8		gCurrentDate					:=	intformat(Std.Date.CurrentDate(true), 8, 1) : global, independent;//  gCurrentDateTime[1..4] + gCurrentDateTime[6..7] + gCurrentDateTime[9..10];
string6		gCurrentTime					:=	intformat(Std.Date.CurrentTime(true), 6, 1) : global, independent;//	gCurrentDateTime[11..16];
...
recordof(NAC.MSH_Bucket_5_Data)	tFlipIntrastateBucket5(NAC.MSH_Bucket_5_Data pInput)	:=
transform
	self.SourceClientID	:=	pInput.TargetClientID;
	self.TargetClientID	:=	pInput.SourceClientID;
	self								:=	pInput;
end;
dFlipIntrastateBucket5	:=	project(NAC.MSH_Bucket_5_Data(SourceState <> '' and SourceState = TargetState), tFlipIntrastateBucket5(left)); 
dDedupBucket5						:=	dedup(NAC.MSH_Bucket_5_Data + dFlipIntrastateBucket5, SourceState, SourceClientID, TargetState, TargetClientID, all);

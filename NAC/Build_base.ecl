...
EXPORT Build_base(string Version) := FUNCTION

f0:=NAC.Files().Load_in;

NAC.Layouts.base tr(f0 l) := transform
	self.FileName := l.fn;
...
seed:=if(IsInitialBuild,1,max(NAC.Files().Base,PrepRecSeq)+1);
MAC_Sequence_Records(f1_dedup,PrepRecSeq,f2,seed);

NAC.Layouts.base tr3 (NAC.Layouts.base L, INTEGER C):= TRANSFORM
	...
	Client_SSN		                := if(l.Client_SSN_Type_Indicator=NAC.Mod_Sets.Actual_Type
...
////////////////////////////////////////////////////
	self.title       					:=	map(
																		...
																		,'');
	self.prefname		  				:=	NID.PreferredFirst(self.fname);
	self											:=	l;
end;
...
NAC.Layouts.base tr4(UniqueAddresses l) := transform
...
did_add.MAC_Match_Flex
	(ToADL , matchset,
	 ...
	 DID, NAC.Layouts.base, true, did_score,
	 0, ds_did);

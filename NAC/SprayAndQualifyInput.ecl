...
dRaw:=dataset('~nac::in::'+fname+'_temp',{string FullRecord},csv(terminator(['\r\n', '\n']), separator([]), quote([]), notrim));
...
SprayIt:=sequential(
						output('Spraying: '+ ip + spraying + fname + ' -> ' + '~nac::in::'+fname)
						,nothor(FileServices.SprayVariable(
							 IP
							,spraying + fname
							,,,,
							,if(_Control.ThisEnvironment.Name='Dataland','thor40_241','thor400_44')
							,'~nac::in::'+fname+'_temp'
							,
							,
							,
...
d:=dataset('~nac::in::'+fname+'_temp',{string FullRecord},csv(terminator(['\r\n', '\n']), separator([]), quote([]), notrim));
...
out_NCF_reports:=	sequential(
											...
											,NAC.Mod_NCD(fname).ProcOutputNCMFile
											...
											,NAC.Mod_NCD(fname).ProcOutputNCDFile
											...
											,output(NAC.InputFileValidationReport(fname).NCR,,'~nac::out::'+fNCR,compressed)
						...
						,sequential(
											...
											,if(FileFound
														,sequential(
																			...
																			,DeleteTemp
																			,out_NCF_reports
																			,despray_NCF_reports('NCM')
																			,despray_NCF_reports('NCD')
																			,despray_NCF_reports('NCR')

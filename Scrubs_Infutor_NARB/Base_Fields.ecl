...
EXPORT SALT311.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_mandatory','invalid_pastdate8','invalid_record_type','invalid_numeric','invalid_st','invalid_zip5','invalid_alphanum_specials','invalid_phone','invalid_rcid','invalid_sic','invalid_url','invalid_address_type_code','invalid_norm_type','invalid_email');
EXPORT FieldTypeNum(SALT311.StrType fn) := CASE(fn,'invalid_mandatory' => 1,'invalid_pastdate8' => 2,'invalid_record_type' => 3,'invalid_numeric' => 4,'invalid_st' => 5,'invalid_zip5' => 6,'invalid_alphanum_specials' => 7,'invalid_phone' => 8,'invalid_rcid' => 9,'invalid_sic' => 10,'invalid_url' => 11,'invalid_address_type_code' => 12,'invalid_norm_type' => 13,'invalid_email' => 14,0);
...
EXPORT MakeFT_invalid_zip5(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_zip5(SALT311.StrType s) := WHICH(~Scrubs_Infutor_NARB.Functions.fn_numeric(s,5)>0);
EXPORT InValidMessageFT_invalid_zip5(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.CustomFail('Scrubs_Infutor_NARB.Functions.fn_numeric'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_alphanum_specials(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ @#-:./,`&\'Ã£Ã±Ã¨Ã³Ã©Ã Ã‚Ã¡Ã”Ã­Ã¶Ã‹Ãº'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_alphanum_specials(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ @#-:./,`&\'Ã£Ã±Ã¨Ã³Ã©Ã Ã‚Ã¡Ã”Ã­Ã¶Ã‹Ãº'))));
EXPORT InValidMessageFT_invalid_alphanum_specials(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ @#-:./,`&\'Ã£Ã±Ã¨Ã³Ã©Ã Ã‚Ã¡Ã”Ã­Ã¶Ã‹Ãº'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_phone(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_phone(SALT311.StrType s) := WHICH(~Scrubs_Infutor_NARB.Functions.fn_verify_optional_phone(s)>0);
EXPORT InValidMessageFT_invalid_phone(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.CustomFail('Scrubs_Infutor_NARB.Functions.fn_verify_optional_phone'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_rcid(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_rcid(SALT311.StrType s) := WHICH(~Scrubs_Infutor_NARB.Functions.fn_rcid(s)>0);
EXPORT InValidMessageFT_invalid_rcid(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.CustomFail('Scrubs_Infutor_NARB.Functions.fn_rcid'),SALT311.HygieneErrors.Good);
...
EXPORT SALT311.StrType FlatName(UNSIGNED2 i) := CHOOSE(i,'rcid','powid','proxid','seleid','orgid','ultid','did','dt_first_seen','dt_last_seen','dt_vendor_first_reported','dt_vendor_last_reported','process_date','record_type','pid','address_type_code','url','sic1','sic2','sic3','sic4','sic5','incorporation_state','email','contact_title','normcompany_type','clean_company_name','clean_phone','fname','lname','prim_range','p_city_name','v_city_name','st','zip','prep_address_line1','prep_address_line_last');
EXPORT FieldNum(SALT311.StrType fn) := CASE(fn,'rcid' => 0,'powid' => 1,'proxid' => 2,'seleid' => 3,'orgid' => 4,'ultid' => 5,'did' => 6,'dt_first_seen' => 7,'dt_last_seen' => 8,'dt_vendor_first_reported' => 9,'dt_vendor_last_reported' => 10,'process_date' => 11,'record_type' => 12,'pid' => 13,'address_type_code' => 14,'url' => 15,'sic1' => 16,'sic2' => 17,'sic3' => 18,'sic4' => 19,'sic5' => 20,'incorporation_state' => 21,'email' => 22,'contact_title' => 23,'normcompany_type' => 24,'clean_company_name' => 25,'clean_phone' => 26,'fname' => 27,'lname' => 28,'prim_range' => 29,'p_city_name' => 30,'v_city_name' => 31,'st' => 32,'zip' => 33,'prep_address_line1' => 34,'prep_address_line_last' => 35,0);
EXPORT SET OF SALT311.StrType FieldRules(UNSIGNED2 i) := CHOOSE(i,['CUSTOM'],['LENGTHS'],['LENGTHS'],['LENGTHS'],['LENGTHS'],['LENGTHS'],['LENGTHS'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['ENUM'],['CUSTOM'],['ENUM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['ALLOW'],['LENGTHS'],['ENUM'],['LENGTHS'],['CUSTOM'],['ALLOW'],['ALLOW'],['LENGTHS'],['LENGTHS'],['LENGTHS'],['CUSTOM'],['CUSTOM'],['LENGTHS'],['LENGTHS'],[]);
...
EXPORT Make_rcid(SALT311.StrType s0) := MakeFT_invalid_rcid(s0);
EXPORT InValid_rcid(SALT311.StrType s) := InValidFT_invalid_rcid(s);
...
EXPORT Make_fname(SALT311.StrType s0) := MakeFT_invalid_alphanum_specials(s0);
EXPORT InValid_fname(SALT311.StrType s) := InValidFT_invalid_alphanum_specials(s);
EXPORT InValidMessage_fname(UNSIGNED1 wh) := InValidMessageFT_invalid_alphanum_specials(wh);
 
EXPORT Make_lname(SALT311.StrType s0) := MakeFT_invalid_alphanum_specials(s0);
EXPORT InValid_lname(SALT311.StrType s) := InValidFT_invalid_alphanum_specials(s);
EXPORT InValidMessage_lname(UNSIGNED1 wh) := InValidMessageFT_invalid_alphanum_specials(wh);
 
EXPORT Make_prim_range(SALT311.StrType s0) := MakeFT_invalid_mandatory(s0);
EXPORT InValid_prim_range(SALT311.StrType s) := InValidFT_invalid_mandatory(s);
...
EXPORT Make_prep_address_line1(SALT311.StrType s0) := MakeFT_invalid_mandatory(s0);
EXPORT InValid_prep_address_line1(SALT311.StrType s) := InValidFT_invalid_mandatory(s);
EXPORT InValidMessage_prep_address_line1(UNSIGNED1 wh) := InValidMessageFT_invalid_mandatory(wh);
 
EXPORT Make_prep_address_line_last(SALT311.StrType s0) := MakeFT_invalid_mandatory(s0);
EXPORT InValid_prep_address_line_last(SALT311.StrType s) := InValidFT_invalid_mandatory(s);

...
EXPORT FUNCTIONS := MODULE
  ...
  EXPORT PerformDeltabaseCall(DATASET(PCL.Layout_PCRequestStatus) dsPCValidSearchRecs,
    ...
    PCL.Layout_Soapcall_Response failx() := TRANSFORM
      SELF.response._Header.Exceptions := IF(FAILMESSAGE <> '', DATASET([{'Roxie', FAILCODE,CM.ESP_Method,FAILMESSAGE}],iesp.share.t_WsException));
      SELF.response._Header.Status  := (INTEGER) CS.SoapError;
      SELF.response._Header.Message := CM.SoapErrorMessage;
      SELF := [];
    END;

    dsSoapRes := SOAPCALL(dsPCReq,
                          URL,
                          CM.ESP_Method, 
    ...
    dsSoapResCheck := PROJECT(dsSoapRes, TRANSFORM(PCL.Layout_Soapcall_Response,
    ...
    dsSoapOut := CATCH(dsSoapResCheck, ONFAIL(failx()));

    dsPCDB := IF(URL<>'', dsSoapOut[1]);
  ...
  EXPORT PerformGetPCR( DATASET(PCL.Layout_PCRequestStatus) dsPCValidSearchRecs) := FUNCTION
    ...
    PCL.Layout_PCResponseRec map_pcr_fields(FCRA.Key_Override_PCR_DID rt, string recordType) := transform
      ...
      self.Content := if(recordtype=PersonContext.Constants.RecordTypes.SF, PersonContext.Constants.security_freeze_default_purposes, '');  

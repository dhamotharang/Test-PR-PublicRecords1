import iesp,ABMS,Autokey_batch;
export ABMS_Records(unsigned1 glb=8, unsigned1 dppa=0, unsigned1 PenaltThreshold=10) := module
	...
	Export ids_by_ak (dataset(myLayouts.autokeyInput) input):= function
	...
	Export getBiogNumbers(dataset(ABMS_Layouts.autokeyInput) inputData) := function
	...
	Export getRecords(dataset(ABMS_Layouts.autokeyInput) inputData) := function
	...
	Export getRecordsBatch(dataset(ABMS_Layouts.autokeyInput) inputData) := function

Import ams, address, AutoStandardI, DEA, Doxie, ingenix_natlprof, DOXIE_CRS,watchdog,doxie_files,suppress,deathv2_services,NPPES,iesp,codes,ut;
EXPORT Provider_Records_Functions := MODULE
	...
	Export getSSN(dataset(mylayouts.layout_slim_sanction) input) := Function
		...
		mylayouts.layout_ssns_freq get_provider_ssns(mylayouts.layout_sanc_DID l, recordof(watchdog.Key_watchdog_glb) r) := transform
			self.ssn := r.ssn;
			self := l;
		end;

		f_ssns := join(byDids, watchdog.Key_watchdog_glb, 
										keyed(left.did=right.did) AND (RIGHT.Valid_SSN = 'G' or RIGHT.Valid_SSN = ' ' or RIGHT.Valid_SSN = ''),get_provider_ssns(left, right),keep(myConst.MAX_RECS_ON_JOIN),limit(0),left outer)(ssn<>'');

...
EXPORT IParam := MODULE
	...
	EXPORT getSearchParams(iesp.bair_offender.t_BAIROffenderSearchBy searchBy, boolean IsRAIDS = FALSE) := FUNCTION
		
		base_params := BairRx_Common.IParam.getSearchParams(searchBy, BairRx_Common.Constants.SearchType.Offender, , IsRAIDS);
		
		BairRx_Common.Layouts.ModeContext xtModeContext() := TRANSFORM						
			SELF.mID := BairRx_Common.Constants.Mode.OFF,
			SELF.codes := BairRx_Common.Functions.SplitClassCodes(BairRx_Common.Constants.Mode.OFF, searchBy.Classification),
			SELF.clean_codes := BairRx_Common.Functions.SplitClassCodes(BairRx_Common.Constants.Mode.OFF, searchBy.Classification, TRUE),
		...
		BairRx_Common.Layouts.ActiveModes xtActiveMode() := TRANSFORM
			SELF.OFF := exists(inModeContext[1].codes);
		END;

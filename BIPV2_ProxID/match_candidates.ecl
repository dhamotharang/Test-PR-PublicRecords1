...
layout_withpropvars take_cnp_number(with_props le,cnp_number_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj15 := JOIN(pj9,cnp_number_props,left.Proxid=right.Proxid,take_cnp_number(left,right),LEFT OUTER,HASH/*,HINT(parallel_match)*//*HACK to prevent memory limit exceeded error*/);
SHARED SrcRidVlid_prop6 := JOIN(SrcRidVlid_prop5,cnp_number_props,left.Proxid=right.Proxid,LOCAL);
SHARED ForeignCorpkey_prop6 := JOIN(ForeignCorpkey_prop5,cnp_number_props,left.Proxid=right.Proxid,LOCAL);
SHARED RAAddresses_prop6 := JOIN(RAAddresses_prop5,cnp_number_props,left.Proxid=right.Proxid,LOCAL);
SHARED FilterPrimNames_prop6 := JOIN(FilterPrimNames_prop5,cnp_number_props,left.Proxid=right.Proxid,LOCAL);
...
SHARED SrcRidVlid_prop9 := JOIN(SrcRidVlid_prop8,prim_range_derived_props,left.Proxid=right.Proxid,LOCAL);
SHARED ForeignCorpkey_prop9 := JOIN(ForeignCorpkey_prop8,prim_range_derived_props,left.Proxid=right.Proxid,LOCAL);
SHARED RAAddresses_prop9 := JOIN(RAAddresses_prop8,prim_range_derived_props,left.Proxid=right.Proxid,LOCAL);
SHARED FilterPrimNames_prop9 := JOIN(FilterPrimNames_prop8,prim_range_derived_props,left.Proxid=right.Proxid,LOCAL);
...
pj30 do_computes(pj30 le) := TRANSFORM
  ...
  SELF := le;
END;
SHARED propogated := PROJECT(pj30,do_computes(left)) : PERSIST('~temp::Proxid::BIPV2_ProxID::mc_props::DOT_Base',EXPIRE(Config.PersistExpire)); // to allow to 'jump' over an exported value
...
EXPORT Layout_SrcRidVlid_Candidates := RECORD
  ...
  BOOLEAN company_fein_isnull := SrcRidVlid_prp.company_fein  IN SET(s.nulls_company_fein,company_fein); // Simplify later processing 
...
END;
SHARED SrcRidVlid_pp := TABLE(SrcRidVlid_prp,Layout_SrcRidVlid_Candidates)((~cnp_name_isnull OR ~active_duns_number_isnull OR ~company_fein_isnull),~prim_name_derived_isnull);
EXPORT Layout_ForeignCorpkey_Candidates := RECORD
  ...
  INTEGER2 company_fein_weight100 := 0; // Contains 100x the specificity
  BOOLEAN company_fein_isnull := ForeignCorpkey_prp.company_fein  IN SET(s.nulls_company_fein,company_fein); // Simplify later processing 
  UNSIGNED company_fein_cnt := 0; // Number of instances with this particular field value
  UNSIGNED company_fein_e1_cnt := 0; // Number of names instances matching this one by edit distance
...
END;
SHARED ForeignCorpkey_pp := TABLE(ForeignCorpkey_prp,Layout_ForeignCorpkey_Candidates)((~cnp_name_isnull OR ~active_duns_number_isnull OR ~company_fein_isnull),~prim_name_derived_isnull);
EXPORT Layout_RAAddresses_Candidates := RECORD
  ...
  INTEGER2 company_fein_weight100 := 0; // Contains 100x the specificity
  BOOLEAN company_fein_isnull := RAAddresses_prp.company_fein  IN SET(s.nulls_company_fein,company_fein); // Simplify later processing 
  UNSIGNED company_fein_cnt := 0; // Number of instances with this particular field value
  UNSIGNED company_fein_e1_cnt := 0; // Number of names instances matching this one by edit distance
...
END;
SHARED RAAddresses_pp := TABLE(RAAddresses_prp,Layout_RAAddresses_Candidates)((~cnp_name_isnull OR ~active_duns_number_isnull OR ~company_fein_isnull),~prim_name_derived_isnull);
EXPORT Layout_FilterPrimNames_Candidates := RECORD
  ...
  INTEGER2 company_fein_weight100 := 0; // Contains 100x the specificity
  BOOLEAN company_fein_isnull := FilterPrimNames_prp.company_fein  IN SET(s.nulls_company_fein,company_fein); // Simplify later processing 
  UNSIGNED company_fein_cnt := 0; // Number of instances with this particular field value
  UNSIGNED company_fein_e1_cnt := 0; // Number of names instances matching this one by edit distance
...
END;
SHARED FilterPrimNames_pp := TABLE(FilterPrimNames_prp,Layout_FilterPrimNames_Candidates)((~cnp_name_isnull OR ~active_duns_number_isnull OR ~company_fein_isnull),~prim_name_derived_isnull);
...
END;
h1 := TABLE(h0,layout_candidates)((~cnp_name_isnull OR ~active_duns_number_isnull OR ~company_fein_isnull),~prim_name_derived_isnull);
...
layout_candidates add_unk_corp_key(layout_candidates le,Specificities(ih).unk_corp_key_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT30.MAC_Choose_JoinType(j9,s.nulls_unk_corp_key,Specificities(ih).unk_corp_key_values_persisted,unk_corp_key,unk_corp_key_weight100,add_unk_corp_key,j8);
layout_candidates add_foreign_corp_key(layout_candidates le,Specificities(ih).foreign_corp_key_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT30.MAC_Choose_JoinType(j8,s.nulls_foreign_corp_key,Specificities(ih).foreign_corp_key_values_persisted,foreign_corp_key,foreign_corp_key_weight100,add_foreign_corp_key,j7);
...
SHARED Annotated := DISTRIBUTE(j0,hash(Proxid)) : PERSIST('~temp::Proxid::BIPV2_ProxID::mc',EXPIRE(Config.PersistExpire)); // Distributed for keybuild case
...
layout_SrcRidVlid_candidates add_SrcRidVlid_cnp_name(layout_SrcRidVlid_candidates le,Specificities(ih).cnp_name_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT30.MAC_Choose_JoinType(jSrcRidVlid_9,s.nulls_cnp_name,Specificities(ih).cnp_name_values_persisted,cnp_name,cnp_name_weight100,add_SrcRidVlid_cnp_name,jSrcRidVlid_13);
...
EXPORT SrcRidVlid_candidates := jSrcRidVlid_21 : PERSIST('~temp::Proxid::BIPV2_ProxID::mc::SrcRidVlid',EXPIRE(Config.PersistExpire));
...
layout_ForeignCorpkey_candidates add_ForeignCorpkey_cnp_name(layout_ForeignCorpkey_candidates le,Specificities(ih).cnp_name_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT30.MAC_Choose_JoinType(jForeignCorpkey_9,s.nulls_cnp_name,Specificities(ih).cnp_name_values_persisted,cnp_name,cnp_name_weight100,add_ForeignCorpkey_cnp_name,jForeignCorpkey_13);
...
EXPORT ForeignCorpkey_candidates := jForeignCorpkey_21 : PERSIST('~temp::Proxid::BIPV2_ProxID::mc::ForeignCorpkey',EXPIRE(Config.PersistExpire));
...
layout_RAAddresses_candidates add_RAAddresses_cnp_name(layout_RAAddresses_candidates le,Specificities(ih).cnp_name_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT30.MAC_Choose_JoinType(jRAAddresses_9,s.nulls_cnp_name,Specificities(ih).cnp_name_values_persisted,cnp_name,cnp_name_weight100,add_RAAddresses_cnp_name,jRAAddresses_13);
...
EXPORT RAAddresses_candidates := jRAAddresses_21 : PERSIST('~temp::Proxid::BIPV2_ProxID::mc::RAAddresses',EXPIRE(Config.PersistExpire));
...
layout_FilterPrimNames_candidates add_FilterPrimNames_cnp_name(layout_FilterPrimNames_candidates le,Specificities(ih).cnp_name_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT30.MAC_Choose_JoinType(jFilterPrimNames_9,s.nulls_cnp_name,Specificities(ih).cnp_name_values_persisted,cnp_name,cnp_name_weight100,add_FilterPrimNames_cnp_name,jFilterPrimNames_13);

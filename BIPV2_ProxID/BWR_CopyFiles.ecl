...
filedescription := workunit + '\n Copied BIPV2_ProxID_dev iteration 35 files to BIPV2_ProxID.  First time we are using DBA in prod. BIPV2_ProxID_dev workunit: W20130605-103201';
Filterregex     := 'key';
destgroup       := tools.fun_Groupname('20');
//copy prod to prod
tools.fun_CopyRename(keynames,true ,['BIPV2_ProxID','BIPV2_ProxID_dev'] ,[version,BIPV2.keysuffix] ,filedescription,Filterregex ,pdestinationgroup := destgroup ,pSkipSuperStuff := SkipSuperStuff,pIsTesting := istesting );   //package file keys         

...
Command line options: -MBIPV2_ProxID -eC:\Users\bentlela\AppData\Local\Temp\TFRE017.tmp 
...
Total available specificity:360
...
company_inc_state Scoring: Two company_inc_state fields will be considered to match if:
  - they are identical
...
(This subtraction estimate is based upon 11% of clusters with 2 or more records have 2 or more values for company_charter_number.)
 
active_corp_key Scoring: This field is not used for scoring; rather it is carried along for context and debugging
...
hist_corp_key Scoring: If a field is null and another record in the same cluster has a value for that field then the other records value will be used.
Two hist_corp_key fields will be considered to match if:
  - they are identical
The exact number of points allocated to a match will depend upon the global scarcity of the hist_corp_key
but should average 19 points with a failed match substracting 16 points.
(This subtraction estimate is based upon 15% of clusters with 2 or more records have 2 or more values for hist_corp_key.)
...
cnp_name_phonetic Scoring: Two cnp_name_phonetic fields will be considered to match if:
  - they are identical
  - if the two fields have the same metaphone handle
The exact number of points allocated to a match will depend upon the global scarcity of the cnp_name_phonetic
and the degree of fuzziness required but should average 12 points with a failed match substracting 7 points.
(This subtraction estimate is based upon 33% of clusters with 2 or more records have 2 or more values for cnp_name_phonetic.)
...
Additional Scoring for fields charter_number,inc_state from file ActiveCorpKeys:
 A score will be allocated for ActiveCorpKeys only in the event of an exact match, no penalty attaches to a no-match
The exact score will depend upon the global scarcity of ActiveCorpKeys but should average 19.
If multiple ActiveCorpKeys match for the same cluster pairing only the highest scoring 1 will apply.

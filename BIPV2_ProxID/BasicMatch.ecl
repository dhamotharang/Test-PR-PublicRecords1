...
SHARED  h00 := LinkBlockers(ih).input_file;
  SHARED s := Specificities(ih).specificities[1];
  SHARED h00_match := h00((cnp_name NOT IN SET(s.nulls_cnp_name,cnp_name) OR active_duns_number NOT IN SET(s.nulls_active_duns_number,active_duns_number) OR company_fein NOT IN SET(s.nulls_company_fein,company_fein)),prim_name_derived NOT IN SET(s.nulls_prim_name_derived,prim_name_derived), 
      0 + IF( active_duns_number  IN SET(s.nulls_active_duns_number,active_duns_number), 0, 28 ) + IF( active_enterprise_number  IN SET(s.nulls_active_enterprise_number,active_enterprise_number), 0, 27 ) + IF( active_domestic_corp_key  IN SET(s.nulls_active_domestic_corp_key,active_domestic_corp_key), 0, 27 ) + IF( hist_enterprise_number  IN SET(s.nulls_hist_enterprise_number,hist_enterprise_number), 0, 27 ) + IF( hist_duns_number  IN SET(s.nulls_hist_duns_number,hist_duns_number), 0, 28 ) + IF( hist_domestic_corp_key  IN SET(s.nulls_hist_domestic_corp_key,hist_domestic_corp_key), 0, 27 ) + IF( foreign_corp_key  IN SET(s.nulls_foreign_corp_key,foreign_corp_key), 0, 27 ) + IF( unk_corp_key  IN SET(s.nulls_unk_corp_key,unk_corp_key), 0, 27 ) + IF( ebr_file_number  IN SET(s.nulls_ebr_file_number,ebr_file_number), 0, 28 ) + IF( company_fein  IN SET(s.nulls_company_fein,company_fein), 0, 27 ) + IF( cnp_name  IN SET(s.nulls_cnp_name,cnp_name), 0, 15 ) + IF( cnp_number  IN SET(s.nulls_cnp_number,cnp_number), 0, 15 ) + IF( cnp_btype  IN SET(s.nulls_cnp_btype,cnp_btype), 0, 3 ) + IF( company_phone  IN SET(s.nulls_company_phone,company_phone), 0, 27 ) + IF( prim_name_derived  IN SET(s.nulls_prim_name_derived,prim_name_derived), 0, 13 ) + IF( sec_range  IN SET(s.nulls_sec_range,sec_range), 0, 12 ) + IF( v_city_name  IN SET(s.nulls_v_city_name,v_city_name), 0, 11 ) + IF( st  IN SET(s.nulls_st,st), 0, 5 ) + IF( zip  IN SET(s.nulls_zip,zip), 0, 14 ) + IF( prim_range_derived  IN SET(s.nulls_prim_range_derived,prim_range_derived), 0, 13 ) >= Config.BasicMatchThreshold); // Potentially remove anything that would violate FORCE(+) constraints
...
SHARED NotBlocked := JOIN(PickOne,Block,LEFT.Proxid1=RIGHT.Proxid1 AND LEFT.Proxid2=RIGHT.Proxid2,TRANSFORM(LEFT),LEFT ONLY, LOOKUP) : PERSIST('~temp::Proxid::BIPV2_ProxID::BasicMatch',EXPIRE(Config.PersistExpire));
EXPORT patch_file := NotBlocked;
  ut.MAC_Patch_Id(h00,Proxid,NotBlocked,Proxid1,Proxid2,o1); // Patch the input file

...
IMPORT SALT37,std;
EXPORT MOD_Attr_SrcRidVlid(DATASET(layout_DOT_Base) ih,UNSIGNED MatchThreshold = Config.MatchThreshold) := MODULE
...
match_candidates(ih).Layout_Attribute_Matches Score(Cands le,Cands ri,UNSIGNED cnp_name_support0 = 0) := TRANSFORM
  ...
  SELF.Conf_Prop := (cnp_number_score + active_duns_number_score + company_fein_score + active_enterprise_number_score + active_domestic_corp_key_score) / 100; // Score based on forced fields

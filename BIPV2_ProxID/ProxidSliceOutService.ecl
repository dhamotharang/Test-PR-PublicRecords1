...
BIPV2_ProxID.match_candidates(BFile).layout_candidates into(BIPV2_ProxID.Keys(BFile).Candidates le) := TRANSFORM
  SELF := le;
END;
odl := PROJECT(CHOOSEN(BIPV2_ProxID.Keys(BFile).Candidates(Proxid=(UNSIGNED)Proxidstr),10000),into(LEFT));
odr := PROJECT(CHOOSEN(BIPV2_ProxID.Keys(BFile).Candidates(Proxid=(UNSIGNED)Pref_Proxidstr),10000),into(LEFT));

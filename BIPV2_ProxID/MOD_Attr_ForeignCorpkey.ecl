...
IMPORT SALT311,std;
EXPORT MOD_Attr_ForeignCorpkey(DATASET(layout_DOT_Base) ih,UNSIGNED MatchThreshold = Config.MatchThreshold) := MODULE
...
// We are going to create a record for each candidate pair; this record will have a child dataset for the attribute values of each side
  ...
  Cands := DEDUP( SORT( DISTRIBUTE(Cands1,HASH(Proxid)), WHOLE RECORD, LOCAL), WHOLE RECORD, LOCAL);
  ...
  Jnd := JOIN(Mtch,Cands,LEFT.id1=RIGHT.Proxid,LOCAL);
  Bads := JOIN(DISTRIBUTE(Jnd,HASH(id2)),Cands,LEFT.id2=RIGHT.Proxid AND LEFT.Context=RIGHT.Context AND LEFT.Basis<>RIGHT.Basis,TRANSFORM(LEFT),LOCAL);

...
pnumiters    := 1                                         ;
pversion     := '20140129'                           ;
pDOTFile     := 'BIPV2_ProxID.In_DOT_Base'                  ;
pdopatch     := false                                      ;
...
infile       := BIPV2_ProxID.files('20140129_66',true).base.logical;//(st in ['FL','OH','GA','PA'],v_city_name in ['DAYTON','MIAMISBURG','BOCA RATON','ATLANTA','BETHLEHEM']);
writeoutfile := tools.macf_writefile(BIPV2_ProxID.filenames(pversion).base.logical,infile,pOverwrite := TRUE);

...
SALT37.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.id := le.Proxid;
  SELF.Src := le.source;
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT37.StrType)le.cnp_number),TRIM((SALT37.StrType)le.st),TRIM((SALT37.StrType)le.prim_range_derived),TRIM((SALT37.StrType)le.hist_duns_number),TRIM((SALT37.StrType)le.ebr_file_number),TRIM((SALT37.StrType)le.active_duns_number),TRIM((SALT37.StrType)le.hist_enterprise_number),TRIM((SALT37.StrType)le.hist_domestic_corp_key),TRIM((SALT37.StrType)le.foreign_corp_key),TRIM((SALT37.StrType)le.unk_corp_key),TRIM((SALT37.StrType)le.company_fein),TRIM((SALT37.StrType)le.company_phone),TRIM((SALT37.StrType)le.active_enterprise_number),TRIM((SALT37.StrType)le.active_domestic_corp_key),TRIM((SALT37.StrType)le.cnp_name),TRIM((SALT37.StrType)le.zip),TRIM((SALT37.StrType)le.prim_name_derived),TRIM((SALT37.StrType)le.sec_range),TRIM((SALT37.StrType)le.v_city_name),TRIM((SALT37.StrType)le.cnp_btype),TRIM((SALT37.StrType)le.company_name),TRIM((SALT37.StrType)le.company_name_type_raw),TRIM((SALT37.StrType)le.company_name_type_derived),TRIM((SALT37.StrType)le.cnp_hasnumber),TRIM((SALT37.StrType)le.cnp_lowv),TRIM((SALT37.StrType)le.cnp_translated),TRIM((SALT37.StrType)le.cnp_classid),TRIM((SALT37.StrType)le.company_foreign_domestic),TRIM((SALT37.StrType)le.company_bdid),TRIM((SALT37.StrType)le.prim_name),TRIM((SALT37.StrType)le.prim_range),TRIM((SALT37.StrType)le.dt_first_seen),TRIM((SALT37.StrType)le.dt_last_seen)));
  SELF.FldNo := C;
END;
...
SHARED FldIds := DATASET([{1,'cnp_number'}
      ...
      ,{20,'cnp_btype'}
      ,{21,'company_name'}
      ,{22,'company_name_type_raw'}
      ,{23,'company_name_type_derived'}
      ,{24,'cnp_hasnumber'}
      ,{25,'cnp_lowv'}
      ,{26,'cnp_translated'}
      ,{27,'cnp_classid'}

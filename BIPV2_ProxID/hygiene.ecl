...
EXPORT Summary(SALT311.Str30Type  txt,BOOLEAN Glob=TRUE) := FUNCTION
  SummaryLayout := RECORD
    ...
    populated_active_corp_key_cnt := COUNT(GROUP,h.active_corp_key <> (TYPEOF(h.active_corp_key))'');
    populated_active_corp_key_pcnt := AVE(GROUP,IF(h.active_corp_key = (TYPEOF(h.active_corp_key))'',0,100));
    maxlength_active_corp_key := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.active_corp_key)));
    avelength_active_corp_key := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.active_corp_key)),h.active_corp_key<>(typeof(h.active_corp_key))'');
    ...
    populated_hist_corp_key_cnt := COUNT(GROUP,h.hist_corp_key <> (TYPEOF(h.hist_corp_key))'');
    populated_hist_corp_key_pcnt := AVE(GROUP,IF(h.hist_corp_key = (TYPEOF(h.hist_corp_key))'',0,100));
    maxlength_hist_corp_key := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.hist_corp_key)));
    avelength_hist_corp_key := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.hist_corp_key)),h.hist_corp_key<>(typeof(h.hist_corp_key))'');
    ...
    populated_cnp_name_phonetic_cnt := COUNT(GROUP,h.cnp_name_phonetic <> (TYPEOF(h.cnp_name_phonetic))'');
    populated_cnp_name_phonetic_pcnt := AVE(GROUP,IF(h.cnp_name_phonetic = (TYPEOF(h.cnp_name_phonetic))'',0,100));
    maxlength_cnp_name_phonetic := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.cnp_name_phonetic)));
    avelength_cnp_name_phonetic := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.cnp_name_phonetic)),h.cnp_name_phonetic<>(typeof(h.cnp_name_phonetic))'');
...
SHARED FldIds := DATASET([{1,'active_duns_number'}
      ...
      ,{11,'company_fein'}
      ,{12,'company_name'}
      ,{13,'cnp_name_phonetic'}
      ,{14,'cnp_name'}
      ,{15,'company_name_type_raw'}
      ,{16,'company_name_type_derived'}
      ,{17,'cnp_hasnumber'}
      ,{18,'cnp_number'}
      ,{19,'cnp_btype'}
      ,{20,'cnp_lowv'}
      ,{21,'cnp_translated'}
      ,{22,'cnp_classid'}
      ,{23,'company_foreign_domestic'}
      ,{24,'company_bdid'}
      ,{25,'company_phone'}
      ,{26,'prim_name'}
      ,{27,'prim_name_derived'}
      ,{28,'sec_range'}
      ,{29,'v_city_name'}
      ,{30,'st'}
      ,{31,'zip'}
      ,{32,'prim_range'}
      ,{33,'prim_range_derived'}
      ,{34,'dt_first_seen'}
      ,{35,'dt_last_seen'}],SALT311.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_active_corp_key((SALT311.StrType)le.active_corp_key),
    ...
    Fields.InValid_hist_corp_key((SALT311.StrType)le.hist_corp_key),
    ...
    Fields.InValid_cnp_name_phonetic((SALT311.StrType)le.cnp_name_phonetic),

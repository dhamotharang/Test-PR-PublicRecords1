...
def dCompanyNameScore(): 
	return [
		('debug','(SELF\.cnp_name_score := )([^;]*?)(;)'			,'HACKCompanyNameScore', 'SELF.cnp_name_score := MAP ( le.cnp_name = ri.cnp_name\n'
			...
			+'    => 0\n'
			+'    , -9999 );/*HACKCompanyNameScore*/','company name replacement'),
		('matches','(INTEGER2 cnp_name_score := )([^;]*?)(;)','HACKCompanyNameScore', 'INTEGER2 cnp_name_score := MAP ( le.cnp_name = ri.cnp_name \n'
			+'    or (cnp_name_score_supp >= Config.cnp_name_Force * 100 and cnp_name_support0 = 0)\n'
			+'    or (cnp_name_score_supp >= Config.cnp_name_Force * 100 and cnp_name_score_temp < Config.cnp_name_Force * 100 and cnp_name_support0 > 0 /*and regexfind(\'fbn|dba|fictitious|assumed|trade\',le.company_name_type_raw + le.company_name_type_derived + ri.company_name_type_raw + ri.company_name_type_derived,nocase)*/)  \n'
			+'    => cnp_name_score_supp\n'
	...
	return [
		...
		('debug', 'END;[ \n\t]*SHARED AppendAttribs.*END;[ \n\t]*EXPORT Layout_RolledEntity', 'HACKDebug03','END;\n'
     		...
     		+ '    SELF := le;\n'
      		...
      		+ 'END;\n'
      		+ '\n'
      		+ 'EXPORT Layout_RolledEntity','hack attribute file scores'),
...
def dBasicMatch():
	return [
		('BasicMatch','(h01.*?;)\n  (h02.*?; \/\/ ,LOCAL ok - we don\'t need a perfect dedup - this is an optimization)\n  (Match := JOIN\(h02,)','HACKBasicMatch01','  h01 := table     (h00_match ,{SALT_Partition,active_duns_number,active_enterprise_number,active_domestic_corp_key,hist_enterprise_number,hist_duns_number,hist_domestic_corp_key,foreign_corp_key,unk_corp_key,ebr_file_number,company_fein,cnp_name,company_name_type_derived,cnp_number,cnp_btype,company_phone,prim_name_derived,sec_range,v_city_name,st,zip,prim_range_derived,unsigned6 Proxid := min(group,Proxid)}\n'
			+'                             ,SALT_Partition,active_duns_number,active_enterprise_number,active_domestic_corp_key,hist_enterprise_number,hist_duns_number,hist_domestic_corp_key,foreign_corp_key,unk_corp_key,ebr_file_number,company_fein,cnp_name,company_name_type_derived,cnp_number,cnp_btype,company_phone,prim_name_derived,sec_range,v_city_name,st,zip,prim_range_derived,merge);/*HACKBasicMatch01 TO SPEED UP*/\n'
			+'Match := JOIN(h01,','fix dedup skew in basic match'),
...
def dConfig():
	return [
		('Config','DoSliceouts := TRUE'  ,'HACKConfig'  ,'DoSliceouts := false/*HACKConfig*/' ,'disable Sliceouts')
	]

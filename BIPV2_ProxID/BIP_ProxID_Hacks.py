...
def dMatches():
	return [
		...
		('matches','AllAttrMatches := SORT\(MOD_Attr_SrcRidVlid\(ih\)\.Match\+MOD_Attr_ActiveCorpKeys\(ih\)\.Match\+MOD_Attr_RAAddresses\(ih\)\.Match\+MOD_Attr_FilterPrimNames\(ih\)\.Match\+MOD_Attr_UnderLinks\(ih\)\.Match,Proxid1','HACKMatches04','AllAttrMatches := SORT(Mod_Attr_SrcRidVlid(ih).Match+MOD_Attr_UnderLinks(ih).Match/*HACKMatches04*/ /* +MOD_Attr_ActiveCorpKeys(ih).Match+Mod_Attr_RAAddresses(ih).Match+Mod_Attr_FilterPrimNames(ih).Match*/,Proxid1','only use srcridvlid att matches for speedup'),
...
##---------------------------------------------------------------------------
##dModAttrActiveCorpKey
##ModAttrActiveCorpKey hacks
##---------------------------------------------------------------------------
def dModAttrActiveCorpKey():
	return [
		('MOD_Attr_ActiveCorpKeys','^.*$'  ,'HACK'  ,'// HACK: Logic to handle the matching around Active Corpkeys\n'
    		+ ' \n'
      		...
      		+ 'END;\n','Hack Attribute file Active Corpkeys.')
...
def dMODAttrFilterPrimNames():
	return [
		('MOD_Attr_FilterPrimNames','^.*$' ,'HACK','// Logic to handle the matching around FilterPrimNames\n'
      		...
      		+ '  SELF.Conf_Prop := (cnp_number_score + active_duns_number_score + active_enterprise_number_score) / 100; // Score based on forced fields\n'
      		+ '  SELF.Conf := ri.Basis_weight100 *  1.00/100;\n'
      		+ 'end;\n'
      		+ 'Matches0 := DISTRIBUTE(JOIN(Cands,Cands,LEFT.Basis = RIGHT.Basis AND LEFT.Proxid > RIGHT.Proxid,Score(LEFT,RIGHT)),Proxid1+Proxid2);\n'
      		+ ' \n'
...
##---------------------------------------------------------------------------
##dMOD_Attr_SrcRidVlid
##MOD_Attr_SrcRidVlid hacks
##---------------------------------------------------------------------------
def dMOD_Attr_SrcRidVlid():
	return [
      ('MOD_Attr_SrcRidVlid','le\.company_inc_state_isnull OR ri\.company_inc_state_isnull OR '  ,'HACK remove inc state isnull check'  ,'/*le.company_inc_state_isnull OR ri.company_inc_state_isnull OR*//*HACK remove inc state isnull check */' ,'HACK remove inc state isnull check')
...
def dMOD_Attr_UnderLinks():
	return [
      ('MOD_Attr_UnderLinks','le\.company_inc_state_isnull OR ri\.company_inc_state_isnull OR '  ,'HACK remove inc state isnull check'  ,'/*le.company_inc_state_isnull OR ri.company_inc_state_isnull OR*//*HACK remove inc state isnull check */' ,'HACK remove inc state isnull check')
...
def dProcIterate():
	return [
    	...
    	('Proc_Iterate','\- Config\.SliceDistance'          ,'HACKProcIterate05' ,'/*- Config.SliceDistance*//*HACKProcIterate05*/'  ,'remove slicedistance' ),
...
def dLinkBlockers():
	return [
      ...
      ('LinkBlockers','AND \(LEFT\.hist_corp_key = RIGHT\.hist_corp_key OR RIGHT\.hist_corp_key = \(TYPEOF\(RIGHT\.hist_corp_key\)\)\'\'\)'  ,'HACK hist_corp_key not needed'  ,'// AND (LEFT.hist_corp_key = RIGHT.hist_corp_key OR RIGHT.hist_corp_key = (TYPEOF(RIGHT.hist_corp_key))\'\')/*HACK hist_corp_key not needed*/' ,'HACK hist_corp_key not needed')

...
SHARED match_candidates(ih).layout_matches match_join(match_candidates(ih).layout_candidates le,match_candidates(ih).layout_candidates ri,UNSIGNED c=0,UNSIGNED outside=0,UNSIGNED cnp_name_support0 = 0) := TRANSFORM
  ...
  INTEGER2 cnp_name_score := MAP ( le.cnp_name = ri.cnp_name 
    ...
    ,company_fein_score > Config.company_fein_Force            *100 and regexfind('fbn|dba|fictitious|assumed|trade',le.company_name_type_raw + le.company_name_type_derived + ri.company_name_type_raw + ri.company_name_type_derived,nocase)
    => company_fein_score
...
// Compute the score for the concept company_addr1
  INTEGER2 company_addr1_score_ext := SALT37.ClipScore(MAX(company_addr1_score_pre,0) + prim_range_derived_score + prim_name_derived_score + sec_range_score + MAX(company_address_score_pre,0));// Score in surrounding context
...
// Compute the score for the concept company_csz
  INTEGER2 company_csz_score_ext := SALT37.ClipScore(MAX(company_csz_score_pre,0) + v_city_name_score + st_score + zip_score + MAX(company_address_score_pre,0));// Score in surrounding context
...
// Compute the score for the concept company_address
  INTEGER2 company_address_score_ext := SALT37.ClipScore(MAX(company_address_score_pre,0)+ company_addr1_score + prim_range_derived_score + prim_name_derived_score + sec_range_score+ company_csz_score + v_city_name_score + st_score + zip_score);// Score in surrounding context
  ...
  SELF.Conf_Prop := (0 + cnp_number_score_prop + prim_range_derived_score_prop + hist_duns_number_score_prop + ebr_file_number_score_prop + active_duns_number_score_prop + hist_enterprise_number_score_prop + hist_domestic_corp_key_score_prop + foreign_corp_key_score_prop + unk_corp_key_score_prop + company_fein_score_prop + company_phone_score_prop + active_enterprise_number_score_prop + active_domestic_corp_key_score_prop + company_addr1_score_prop + sec_range_score_prop + company_address_score_prop) / 100; // Score based on propogated fields
  import ut;
...
mj0 := JOIN( dn0_deduped, dn0_deduped, LEFT.Proxid > RIGHT.Proxid AND ( LEFT.SALT_Partition = RIGHT.SALT_Partition OR LEFT.SALT_Partition='' OR RIGHT.SALT_Partition = '' )
    AND LEFT.cnp_number = RIGHT.cnp_number AND LEFT.prim_name_derived = RIGHT.prim_name_derived
    ...
    AND ~(( left.cnp_name = right.cnp_name ) AND ( left.company_address = right.company_address )),trans(LEFT,RIGHT,0),UNORDERED,
...
SALT37.mac_select_best_matches(last_mjs_t,rcid1,rcid2,o);
...
// SALT37.mac_avoid_transitives(All_Matches,Proxid1,Proxid2,Conf,DateOverlap,Rule,o); /* HACK - disable default salt mac_avoid_transitives*/
import BIPV2_Tools;
...
SALT37.mac_get_BestPerRecord( All_Matches,rcid1,Proxid1,rcid2,Proxid2,o );
...
SALT37.mac_cluster_breadth(in_matches,rcid1,rcid2,Proxid1,o);
SHARED in_matches1 := o;
missed_linkages := JOIN(in_matches1,Specificities(ih).ClusterSizes(InCluster>1),LEFT.Proxid1=RIGHT.Proxid,RIGHT ONLY,LOCAL);
missed_linkages1 := JOIN(h,missed_linkages,LEFT.Proxid=RIGHT.Proxid,TRANSFORM(RECORDOF(missed_linkages),SELF.Proxid1:=RIGHT.Proxid,SELF.rcid1:=LEFT.rcid,SELF:=RIGHT),LOCAL);
o1 := JOIN(in_matches1,Specificities(ih).ClusterSizes,LEFT.Proxid1=RIGHT.Proxid,LOCAL);
EXPORT ClusterLinkages := o1 + missed_linkages1 : PERSIST('~temp::Proxid::BIPV2_ProxID::clu',EXPIRE(BIPV2_ProxID.Config.PersistExpire));
...
Pref_Layout := RECORD
  SALT37.UIDType rcid;  //Outcast
  SALT37.UIDType Proxid;  // Outcase within
  INTEGER2 Closest; // Best present link
  SALT37.UIDType Pref_rcid; // Prefers this record
  SALT37.UIDType Pref_Proxid; // Prefers this cluster
...
EXPORT ToSlice := IF ( Config.DoSliceouts, DEDUP(SORT(DISTRIBUTE(BetterElsewhere(Pref_Margin>10),HASH(Proxid)),Proxid,-Pref_Margin,LOCAL),Proxid,LOCAL)) : PERSIST('~temp::Proxid::BIPV2_ProxID::Matches::ToSlice',EXPIRE(BIPV2_ProxID.Config.PersistExpire));
// 1024x better in new place
  SALT37.MAC_Avoid_SliceOuts(PossibleMatches,Proxid1,Proxid2,Proxid,Pref_Proxid,ToSlice,m); // If Proxid is slice target - don't use in match
...
SHARED ih_thin := TABLE(ih_init,{rcid,dotid,Proxid,lgid3,orgid,ultid});
  SALT37.MAC_Reassign_UID(ih_thin,split_patch,Proxid,rcid,ih0); // Perform splits
  SALT37.utMAC_Patch_Id(ih0,Proxid,BasicMatch(ih).patch_file,Proxid1,Proxid2,ihbp); // Perform basic matches
  SALT37.MAC_Reassign_UID(ihbp,Cleave(ih).patch_file,Proxid,rcid,ih1); // Perform cleaves
  SALT37.MAC_SliceOut_ByRID(ih1,rcid,Proxid,ToSlice,rcid,sliced0); // Execute Sliceouts
  sliced := IF( Config.DoSliceouts, sliced0, ih1); // Config-based ability to remove sliceout
  SALT37.utMAC_Patch_Id(sliced,Proxid,Matches,Proxid1,Proxid2,o); // Join Clusters
  Patchlgid3 := BIPV2_Tools.MAC_ParentId_Patch(o,lgid3,Proxid);  // Collapse any lgid3 now joined by Proxid
...
SALT37.utMAC_Patch_Id(h,Proxid,Matches,Proxid1,Proxid2,o1);

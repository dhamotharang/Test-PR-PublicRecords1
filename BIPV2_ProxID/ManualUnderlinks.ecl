...
export ManualUnderLinks := module
	...
	export createLogicalFile(dataset(recLayout) newData) := function	
		...
		linksProxIDs := dedup(sort(newData, underLinkId), underLinkId);
		...
		rec addId(linksProxIDs L, integer cnt) := transform
			self.new_underLinkId := maxId + cnt;
			self 							:= L;
		end;
		linksWithIds := project(linksProxIDs, AddId(left, counter));

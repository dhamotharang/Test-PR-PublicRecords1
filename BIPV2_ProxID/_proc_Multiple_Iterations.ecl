...
functionmacro
  ...
  import wk_ut,tools,bipv2,mdr,BIPV2_ProxID,SALTTOOLS22,BIPV2_Files,BIPV2_ProxID_mj6;
...
//  #workunit('name','BIPV2_ProxID Specificites + kick off iterations ' + (string)startiter + '-' + (string)(startiter + numiters - 1));
  ...
  kickPreprocess := wk_ut.mac_ChainWuids(ecltextPreprocess ,1         ,1        ,prevcombo,[]           ,cluster,pOutputEcl := false,pUniqueOutput := pUniqueOut + 'Patch',pNotifyEmails := BIPV2_Build.mod_email.emailList

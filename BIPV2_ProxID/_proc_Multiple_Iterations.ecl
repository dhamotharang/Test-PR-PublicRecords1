...
functionmacro
  ...
  import wk_ut,tools,bipv2,mdr,BIPV2_ProxID,SALT25,SALTTOOLS22,BIPV2_Files;
...
//  #workunit('name','BIPV2_ProxID Specificites + kick off iterations ' + (string)startiter + '-' + (string)(startiter + numiters - 1));
  ...
  ecltextPatch   := '#workunit(\'name\',BIPV2_ProxID._Constants().name + \' @version@ ID Patch\'     );\n\n#workunit(\'priority\',\'high\');\nBIPV2_ProxID._fPatch_Foreign_Corpkey_Overlinking(@INFILE@,\'@version@\');';  
  ecltextPatch2  := regexreplace('@INFILE@',ecltextPatch,pDOTFile,nocase);
   
  ecltextPromote := '#workunit(\'name\',BIPV2_ProxID._Constants().name + \' @version@ Pre Promote\'     );\n\n#workunit(\'priority\',\'high\');\n' + 'sequential(\n'
   + '   BIPV2_ProxID.Promote(,\'base\',pOddFilename := \'' + pDotFilename + '\').odd2built\n' 
   + '  ,BIPV2_ProxID._PreProcess_Strata(,bipv2_proxid.files().base.built,\'@version@\',\'PrePost\')\n'
  + ');';  

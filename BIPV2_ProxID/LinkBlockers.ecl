...
// First step is to convert the 'match criteria' in the block file into a Proxid stream
  SHARED IdRec := RECORD
    SALT311.UIDType Proxid; // The cluster id
...
// If and when that becomes an issue we will need to give the option of pulling out certain high specificity fields
  AllIds0 := JOIN(h, bp,
    ...
    // AND (LEFT.hist_corp_key = RIGHT.hist_corp_key OR RIGHT.hist_corp_key = (TYPEOF(RIGHT.hist_corp_key))'')/*HACK hist_corp_key not needed*/
    ...
    AND (LEFT.prim_range_derived = RIGHT.prim_range_derived OR RIGHT.prim_range_derived = (TYPEOF(RIGHT.prim_range_derived))'')
...
EXPORT Patches_dirty_rids0 :=JOIN(PatchesAll, PatchesAll, LEFT.rcid = RIGHT.rcid AND LEFT.Proxid <> RIGHT.Proxid, TRANSFORM(LEFT),HASH) : PERSIST('~temp::Proxid::BIPV2_ProxID::BlockLink::patches_dirty_rids0',EXPIRE(BIPV2_ProxID.Config.PersistExpire));
EXPORT IgnoreRules0 := DEDUP(SORT(DISTRIBUTE(Patches_dirty_rids0,HASH(RuleNum)), RuleNum, LOCAL),RuleNum, LOCAL);
EXPORT Patches_dirty0 := JOIN(PatchesAll,  IgnoreRules0, LEFT.RuleNum = RIGHT.RuleNum, TRANSFORM(LEFT)) : PERSIST('~temp::Proxid::BIPV2_ProxID::BlockLink::patches_dirty0',EXPIRE(BIPV2_ProxID.Config.PersistExpire));
EXPORT Patches := JOIN(PatchesAll,  IgnoreRules0, LEFT.RuleNum = RIGHT.RuleNum, TRANSFORM(LEFT), LEFT ONLY) : PERSIST('~temp::Proxid::BIPV2_ProxID::BlockLink::patches',EXPIRE(BIPV2_ProxID.Config.PersistExpire));
...
SALT311.MAC_Reassign_UID(h,Patches,Proxid,rcid,h1);

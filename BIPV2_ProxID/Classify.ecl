...
SHARED TotalClusters := specificities(h).TotalClusters;
  ...
  company_fein_tokens := PROJECT(specificities(h).company_fein_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.company_fein; SELF.TokenType := 10; SELF.Spc := LEFT.field_Specificity ));
  ...
  cnp_number_tokens := PROJECT(specificities(h).cnp_number_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.cnp_number; SELF.TokenType := 16; SELF.Spc := LEFT.field_Specificity ));
...
EXPORT ConceptTemplatesKey := '~'+'key::BIPV2_ProxID::Proxid::Token::ConceptKey';

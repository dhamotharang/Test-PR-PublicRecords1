...
EXPORT various_appends(unsigned1 mode, STRING sVersion) := MODULE
...
//came_from = 4 are the additional core records
dWithInfutor2 :=JOIN(distribute(coreInfutor, hash(did)),
					 distribute(dWithInfutor1, hash(did)),					
					LEFT.did=RIGHT.did,
					TRANSFORM(reunion.layouts.lMainRaw,SELF.title:='';SELF.dob:=(STRING)RIGHT.dob;SELF.came_from := '4'; SELF:=LEFT;SELF:=RIGHT;),
					LEFT ONLY,
					LOCAL);

dWithInfutor := dWithInfutor1 + dWithInfutor2;
...
EXPORT all := cleaned_did : PERSIST('~thor::persist::mylife::appends::' + reunion.Constants.sMode(mode), EXPIRE(10), REFRESH(FALSE));

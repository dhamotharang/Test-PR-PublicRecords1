...
function
  ...
  // wk_ut.UnProtect_Wuid('');

  // mywuids := wk_ut.get_Running_Workunits('W20080111-111018','W20141231-111018','')(protected = true,owner = 'lbentley_prod',not regexfind('stat|dashboard|sample|segment',job,nocase));
  mywuids := wk_ut.get_Running_Workunits(pLowWuid,pHighWuid,'')(
  ...
  // mywuids := wk_ut.get_Running_Workunits('W20150313-090118','W20151231-111018','')(owner = 'lbentley_prod',regexfind('running|wait|compiling',state,nocase),wuid != workunit);

  return sequential(
    output(mywuids,all)
   ,iff(pIsTesting = false
      ,apply(mywuids ,output(wk_ut.Abort_Workunit(wuid),named('AbortWuids'),extend))
   )
  );

...
EXPORT Rewind_Build(
  ...
  ,dataset(wk_ut.Layouts.wks_slim_filename)   pWorkman_Superfile            // the workman superfile for your build.  it should contain all of the workman build files.
...
function
  ...
  ds_workman_extra_fnames             := 
    ...
    + project(ds_workman(esp != wk_ut._Constants.LocalEsp)  ,transform({string srcname,string dstname,string esp},self.srcname := '~' + trim(left.__filename),self.dstname := if(STD.Str.Find(self.srcname ,left.wuid, 1) = 0,trim(self.srcname) + '.' + trim(left.wuid) ,self.srcname),self.esp  := wk_ut._Constants.LocalEsp))
  ;
  ...
  rename_workman_files := apply(global(ds_workman_extra_fnames_filtered,few)  ,sequential(wk_ut.Rename_File(srcname,dstname,true,esp)));
  
  return sequential(
     apply(global(ds_workman,few) ,sequential(
         STD.System.Log.addWorkunitInformation('Restoring '       + wuid + ' on ' + ut.GetTimeDate())
        ,output(wk_ut.Restore_Workunit(wuid,esp),named('restoring_wuids'),extend) 
    ...
    ,iff(pDeleteFiles  = true 
      ,apply(global(files2delete(isLocal = false),few)
        ,sequential(

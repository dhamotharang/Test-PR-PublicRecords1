...
dwks20130521 := wk_ut.mac_CreateSummaryReport ( 'W20130525-100331' ,'W20130614-232519','lbentley_prod','',['BIPV2*',jobregex],['MatchesPerformed','PreClusterCount','PostClusterCount']
...
//  { wks_filt[1].wuid 	,wks_filt[1].owner ,wks_filt[1].cluster ,wks_filt[1].job 	,wks_filt[1].state 	,wks_filt[1].priority ,wks_filt[1].online ,wks_filt[1].protected 
  // get workunits
  #SET(ECL  ,'dgetwuidlist := global(nothor(wk_ut.get_WorkunitList ( \n')
  ...
  #APPEND(ECL  ,'jobname_filter					:= if(' + #TEXT(pJobname[2]) + '			!= \'\', regexfind(' + #TEXT(pJobname[2]) + '	  , dgetwuidlist.Job	, nocase), true);\n')
	...
	#APPEND(ECL  ,'restorewuids := apply(wks_filt(online = false),output(wk_ut.Restore_Workunit(wuid,' + #TEXT(pEsp) + '),named(\'RestoreWuids\'),EXTEND));\n')
...
///////////////
  ...
  #LOOP
    ...
    #SET(SCALARTEMP   ,wk_ut.mac_parsefieldname(pSetScalarResults[%CNTR%]))
    ...
    #LOOP
      ...
      #END
      #IF(trim(%'SCALARINDEXFIELD'%) = '')
        #APPEND(SCALARTRANSFORM  ,%'SCALARFIELD'% + %'NESTEDCNTR'% + ' := global(wk_ut.get_Scalar_Result(wks_filt[' + %'NESTEDCNTR'% + ' ].wuid,\'' + %'SCALARRESULT'% + '\',' + #TEXT(pEsp) + '),few);\n')
      #ELSE
        #APPEND(SCALARTRANSFORM  ,%'SCALARFIELD'% + %'NESTEDCNTR'% + ' := global(if((unsigned)wk_ut.get_DS_Count(wks_filt[' + %'NESTEDCNTR'% + ' ].wuid,\'' + %'SCALARRESULT'% + '\',' + #TEXT(pEsp) + ') != 0,wk_ut.get_DS_Result    (wks_filt[' + %'NESTEDCNTR'% + '].wuid,\'' + %'SCALARRESULT'% + '\'  ,' + %'SCALARFIELD'% + '_lay' + ' ,' + #TEXT(pEsp) + ')  ,dataset([],' + %'SCALARFIELD'% + '_lay' + '))' + %'SCALARINDEXFIELD'% + ',few);\n')
      #END
...
///////////
  ...
  #LOOP
    ...
    #LOOP
      ...
      #APPEND(DSCOUNTSTRANSFORM  ,pSetDatasetCounts[%CNTR%] + %'NESTEDCNTR'% + ' := global(wk_ut.get_DS_Count(wks_filt[' + %'NESTEDCNTR'% + ' ].wuid,\'' + pSetDatasetCounts[%CNTR%] + '\',' + #TEXT(pEsp) + '),few);\n')
  ...
  #LOOP
    #IF(%CNTR% > pNumRecords)
      #BREAK
    #END
    #APPEND(TOTALTHORTIMEVALUES  ,'totalthortime'+ %'CNTR'% + ' := global(wk_ut.get_TotalTime    (wks_filt[' + %'CNTR'% + ' ].wuid,' + #TEXT(pEsp) + '),few);\n')
    #APPEND(DESCRIPTIONVALUES    ,'description'  + %'CNTR'% + ' := global(wk_ut.get_Description  (wks_filt[' + %'CNTR'% + ' ].wuid,' + #TEXT(pEsp) + '),few);\n')

    #APPEND(WUIDFILES  ,'filesreadraw'+ %'CNTR'% + '     := global(nothor(project((wk_ut.get_FilesRead   (trim(wks_filt[' + %'CNTR'% + ' ].wuid,all),' + #TEXT(pEsp) + ')(if(' + %'FILESREADREGEXFILTER'%    + ' != \'\' ,regexfind(' + %'FILESREADREGEXFILTER'%    + ',name,nocase)  ,true))),{string name})),few);\n')
    #APPEND(WUIDFILES  ,'fileswritten'+ %'CNTR'% + '     := global(nothor(project((wk_ut.get_FilesWritten(trim(wks_filt[' + %'CNTR'% + ' ].wuid,all),' + #TEXT(pEsp) + ')(if(' + %'FILESWRITTENREGEXFILTER'% + ' != \'\' ,regexfind(' + %'FILESWRITTENREGEXFILTER'% + ',name,nocase)  ,true))),{string name})),few);\n')
  ...
  #LOOP
    ...
    #LOOP
      ...
      #APPEND(GETDSRESULT  , pSetDatasetResults[%CNTR%] + %'NESTEDCNTR'% + '   := global(if((unsigned)wk_ut.get_DS_Count(wks_filt[' + %'NESTEDCNTR'% + ' ].wuid,\'' + pSetDatasetResults[%CNTR%] + '\',' + #TEXT(pEsp) + ') != 0,wk_ut.get_DS_Result    (wks_filt[' + %'NESTEDCNTR'% + '].wuid,\'' + pSetDatasetResults[%CNTR%] + '\'  ,' + pSetDatasetResults[%CNTR%] + '_lay' + ' ,' + #TEXT(pEsp) + ')  ,dataset([],' + pSetDatasetResults[%CNTR%] + '_lay' + ')),few);\n')

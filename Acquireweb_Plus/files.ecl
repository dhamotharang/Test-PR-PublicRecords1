...
//Business files - DF-21679
  EXPORT Email_Business     :=  DATASET('~thor::in::acquireweb::business', layouts.Business_raw, CSV(HEADING(1),TERMINATOR('\n'),SEPARATOR('|'),QUOTE('')));
  EXPORT Email_Busi_dedup   :=  DISTRIBUTE(DEDUP(SORT(Email_Business,RECORD)),HASH32(awid_business));
	EXPORT Email_IPAddress    :=  DATASET('~thor::in::acquireweb::ipaddress', layouts.Acquireweb_IPAddress, CSV(HEADING(1),TERMINATOR('\n'),SEPARATOR('|'),QUOTE('')));	

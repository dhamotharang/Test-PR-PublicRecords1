...
EXPORT FromNone(DATASET(Layout_Property_Assessor) h) := MODULE
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    ...
    SELF.ScrubsBits2 := ( le.tax_amount_Invalid << 0 ) + ( le.tax_year_Invalid << 1 ) + ( le.tax_delinquent_year_Invalid << 2 ) + ( le.building_area_Invalid << 3 ) + ( le.building_area_indicator_Invalid << 4 ) + ( le.building_area1_indicator_Invalid << 5 ) + ( le.building_area2_indicator_Invalid << 6 ) + ( le.building_area3_indicator_Invalid << 7 ) + ( le.building_area4_indicator_Invalid << 8 ) + ( le.building_area5_indicator_Invalid << 9 ) + ( le.building_area6_indicator_Invalid << 10 ) + ( le.building_area7_indicator_Invalid << 11 ) + ( le.year_built_Invalid << 12 ) + ( le.effective_year_built_Invalid << 13 ) + ( le.no_of_stories_Invalid << 14 ) + ( le.garage_type_code_Invalid << 16 ) + ( le.pool_code_Invalid << 17 ) + ( le.type_construction_code_Invalid << 18 ) + ( le.building_quality_code_Invalid << 19 ) + ( le.exterior_walls_code_Invalid << 20 ) + ( le.roof_type_code_Invalid << 21 ) + ( le.floor_cover_code_Invalid << 22 ) + ( le.heating_code_Invalid << 23 ) + ( le.heating_fuel_type_code_Invalid << 24 ) + ( le.air_conditioning_type_code_Invalid << 25 ) + ( le.fireplace_indicator_Invalid << 26 ) + ( le.basement_code_Invalid << 27 ) + ( le.building_class_code_Invalid << 28 ) + ( le.site_influence1_code_Invalid << 29 ) + ( le.amenities1_code_Invalid << 30 ) + ( le.amenities2_code_Invalid << 31 ) + ( le.amenities3_code_Invalid << 32 ) + ( le.amenities4_code_Invalid << 33 ) + ( le.other_buildings1_code_Invalid << 34 ) + ( le.tape_cut_date_Invalid << 35 ) + ( le.certification_date_Invalid << 37 ) + ( le.fips_Invalid << 39 ) + ( le.tax_less_than_total_assessed_Invalid << 0 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.building_area1_indicator_Invalid := (le.ScrubsBits2 >> 5) & 1;
    SELF.building_area2_indicator_Invalid := (le.ScrubsBits2 >> 6) & 1;
    SELF.building_area3_indicator_Invalid := (le.ScrubsBits2 >> 7) & 1;
    SELF.building_area4_indicator_Invalid := (le.ScrubsBits2 >> 8) & 1;
    SELF.building_area5_indicator_Invalid := (le.ScrubsBits2 >> 9) & 1;
    SELF.building_area6_indicator_Invalid := (le.ScrubsBits2 >> 10) & 1;
    SELF.building_area7_indicator_Invalid := (le.ScrubsBits2 >> 11) & 1;
    SELF.year_built_Invalid := (le.ScrubsBits2 >> 12) & 1;
    SELF.effective_year_built_Invalid := (le.ScrubsBits2 >> 13) & 1;
    SELF.no_of_stories_Invalid := (le.ScrubsBits2 >> 14) & 3;
    SELF.garage_type_code_Invalid := (le.ScrubsBits2 >> 16) & 1;
    SELF.pool_code_Invalid := (le.ScrubsBits2 >> 17) & 1;
    SELF.type_construction_code_Invalid := (le.ScrubsBits2 >> 18) & 1;
    SELF.building_quality_code_Invalid := (le.ScrubsBits2 >> 19) & 1;
    SELF.exterior_walls_code_Invalid := (le.ScrubsBits2 >> 20) & 1;
    SELF.roof_type_code_Invalid := (le.ScrubsBits2 >> 21) & 1;
    SELF.floor_cover_code_Invalid := (le.ScrubsBits2 >> 22) & 1;
    SELF.heating_code_Invalid := (le.ScrubsBits2 >> 23) & 1;
    SELF.heating_fuel_type_code_Invalid := (le.ScrubsBits2 >> 24) & 1;
    SELF.air_conditioning_type_code_Invalid := (le.ScrubsBits2 >> 25) & 1;
    SELF.fireplace_indicator_Invalid := (le.ScrubsBits2 >> 26) & 1;
    SELF.basement_code_Invalid := (le.ScrubsBits2 >> 27) & 1;
    SELF.building_class_code_Invalid := (le.ScrubsBits2 >> 28) & 1;
    SELF.site_influence1_code_Invalid := (le.ScrubsBits2 >> 29) & 1;
    SELF.amenities1_code_Invalid := (le.ScrubsBits2 >> 30) & 1;
    SELF.amenities2_code_Invalid := (le.ScrubsBits2 >> 31) & 1;
    SELF.amenities3_code_Invalid := (le.ScrubsBits2 >> 32) & 1;
    SELF.amenities4_code_Invalid := (le.ScrubsBits2 >> 33) & 1;
    SELF.other_buildings1_code_Invalid := (le.ScrubsBits2 >> 34) & 1;
    SELF.tape_cut_date_Invalid := (le.ScrubsBits2 >> 35) & 3;
    SELF.certification_date_Invalid := (le.ScrubsBits2 >> 37) & 3;
    SELF.fips_Invalid := (le.ScrubsBits2 >> 39) & 3;
    SELF.trans_before_proc_Invalid := (le.ScrubsBits1 >> 41) & 3;
    SELF.rec_before_proc_Invalid := (le.ScrubsBits1 >> 43) & 3;
    SELF.sale_before_proc_Invalid := (le.ScrubsBits1 >> 45) & 3;
    SELF.ptrans_before_proc_Invalid := (le.ScrubsBits1 >> 47) & 3;
    SELF.prior_rec_before_proc_Invalid := (le.ScrubsBits1 >> 49) & 3;
    SELF.cert_before_proc_Invalid := (le.ScrubsBits1 >> 51) & 3;
    SELF.cut_before_proc_Invalid := (le.ScrubsBits1 >> 53) & 3;
    SELF.AssImp_Eq_AssTot_Invalid := (le.ScrubsBits1 >> 55) & 3;
    SELF.MarImp_Eq_MarTot_Invalid := (le.ScrubsBits1 >> 57) & 3;
    SELF.AssLnd_Eq_AssTot_Invalid := (le.ScrubsBits1 >> 59) & 3;
    SELF.MarLnd_Eq_MarTot_Invalid := (le.ScrubsBits1 >> 61) & 3;
    SELF.tax_less_than_total_assessed_Invalid := (le.ScrubsBits2 >> 0) & 3;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h, BOOLEAN Glob = FALSE) := MODULE
  ...
  END;
  EXPORT AllErrors := NORMALIZE(h,101,Into(LEFT,COUNTER));

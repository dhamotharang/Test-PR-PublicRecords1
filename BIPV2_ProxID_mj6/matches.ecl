...
match_candidates(ih).Layout_Attribute_Matches CombineResults(match_candidates(ih).Layout_Attribute_Matches le,match_candidates(ih).Layout_Attribute_Matches ri) := TRANSFORM
  ...
  SELF := le;
END;
EXPORT All_Attribute_Matches := ROLLUP(AllAttrMatches,LEFT.Proxid1=RIGHT.Proxid1 AND LEFT.Proxid2=RIGHT.Proxid2,CombineResults(LEFT,RIGHT),LOCAL) : PERSIST('~temp::Proxid::BIPV2_ProxID_mj6::attribute_matches',EXPIRE(Config.PersistExpire));
...
EXPORT All_Matches := all_matches0 : PERSIST('~temp::Proxid::BIPV2_ProxID_mj6::all_m',EXPIRE(Config.PersistExpire)); // To by used by rcid and Proxid
...
o := BIPV2_ProxID_mj6.mac_avoid_transitives_scalene(All_Matches,Proxid1,Proxid2,Conf,DateOverlap,Rule,MatchThreshold,10); // HACK - Use new transitives macro, bucket size 10*/

EXPORT PossibleMatches := o : PERSIST('~temp::Proxid::BIPV2_ProxID_mj6::mt',EXPIRE(Config.PersistExpire));
SALT30.mac_get_BestPerRecord( All_Matches,rcid1,Proxid1,rcid2,Proxid2,o );
EXPORT BestPerRecord := o : PERSIST('~temp::Proxid::BIPV2_ProxID_mj6::mr',EXPIRE(Config.PersistExpire));
...
EXPORT ClusterLinkages := o1 : PERSIST('~temp::Proxid::BIPV2_ProxID_mj6::clu',EXPIRE(Config.PersistExpire));
...
EXPORT PreIDs := BIPV2_ProxID_mj6.Fields.UIDConsistency(ih); // Export whole consistency module
EXPORT PostIDs := BIPV2_ProxID_mj6.Fields.UIDConsistency(Patched_Infile); // Export whole consistency module

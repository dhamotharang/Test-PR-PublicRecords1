...
function
  ...
  kFile := BIPV2_ProxID_mj6.Keys(BFile,version);

  odl   := PROJECT(CHOOSEN(KFile.Candidates(Proxid=Proxidone),100000),BIPV2_ProxID_mj6.match_candidates(BFile).layout_candidates);
  odr   := PROJECT(CHOOSEN(KFile.Candidates(Proxid=ProxidTwo),100000),BIPV2_ProxID_mj6.match_candidates(BFile).layout_candidates);
  ...
  BIPV2_ProxID_mj6.match_candidates(BFile).layout_attribute_matches ainto(KFile.Attribute_Matches le) := TRANSFORM
    SELF := le;
  END;

...
odl := PROJECT(CHOOSEN(BIPV2_ProxID_mj6.Keys(BFile,combo).Candidates(Proxid=Proxidone),100000),BIPV2_ProxID_mj6.match_candidates(BFile).layout_candidates);
odr := PROJECT(CHOOSEN(BIPV2_ProxID_mj6.Keys(BFile,combo).Candidates(Proxid=ProxidTwo),100000),BIPV2_ProxID_mj6.match_candidates(BFile).layout_candidates);
k := BIPV2_ProxID_mj6.Keys(BFile,combo).Specificities_Key;
...
BIPV2_ProxID_mj6.match_candidates(BFile).layout_attribute_matches ainto(BIPV2_ProxID_mj6.Keys(BFile,combo).Attribute_Matches le) := TRANSFORM
  SELF := le;
END;
am := PROJECT(BIPV2_ProxID_mj6.Keys(BFile,combo).Attribute_Matches(Proxid1=Proxidone,Proxid2=Proxidtwo)+BIPV2_ProxID_mj6.Keys(BFile,combo).Attribute_Matches(Proxid1=Proxidtwo,Proxid2=Proxidone),ainto(LEFT));

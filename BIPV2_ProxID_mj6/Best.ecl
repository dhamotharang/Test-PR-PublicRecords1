...
EXPORT SINGLEPRIMRANGE_tab_ := DISTRIBUTE(TABLE(h(cnp_name NOT IN SET(s.nulls_cnp_name,cnp_name),prim_name_derived NOT IN SET(s.nulls_prim_name_derived,prim_name_derived),v_city_name NOT IN SET(s.nulls_v_city_name,v_city_name),st NOT IN SET(s.nulls_st,st),zip NOT IN SET(s.nulls_zip,zip)),{cnp_name,prim_name_derived,v_city_name,st,zip,prim_range,UNSIGNED Row_Cnt := COUNT(GROUP)},cnp_name,prim_name_derived,v_city_name,st,zip,prim_range,MERGE),HASH(cnp_name,prim_name_derived,v_city_name,st,zip)); // Slim and reduce row-count
  Slim := TABLE(SINGLEPRIMRANGE_tab_(prim_range NOT IN SET(s.nulls_prim_range,prim_range)),{cnp_name,prim_name_derived,v_city_name,st,zip,prim_range,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
  END;
EXPORT SINGLEPRIMRANGE_tab_prim_range := ROLLUP( SORT(Slim,EXCEPT Row_Cnt,LOCAL),TR(LEFT,RIGHT), EXCEPT Row_Cnt,LOCAL);
//Now actually find the best value
  grp := GROUP( SINGLEPRIMRANGE_tab_prim_range,cnp_name,prim_name_derived,v_city_name,st,zip,ALL,LOCAL);
...
// A best solution is unique if its Row_Cnt is the same as the sum of the row_cnts.
  Totals := TABLE(grp,{cnp_name,prim_name_derived,v_city_name,st,zip,Tot := SUM(GROUP,Row_Cnt)},cnp_name,prim_name_derived,v_city_name,st,zip);
export SINGLEPRIMRANGE_method_prim_range := JOIN( cmn,Totals,LEFT.cnp_name = RIGHT.cnp_name AND LEFT.prim_name_derived = RIGHT.prim_name_derived AND LEFT.v_city_name = RIGHT.v_city_name AND LEFT.st = RIGHT.st AND LEFT.st = RIGHT.st AND LEFT.zip = RIGHT.zip AND LEFT.Row_Cnt = RIGHT.Tot,TRANSFORM(LEFT),LOCAL);
...
// 0 - Gathering type:SINGLEPRIMRANGE Entries:1
  R0 := RECORD
    TYPEOF(SINGLEPRIMRANGE_method_prim_range.cnp_name) cnp_name;
    TYPEOF(SINGLEPRIMRANGE_method_prim_range.prim_name_derived) prim_name_derived;
    TYPEOF(SINGLEPRIMRANGE_method_prim_range.v_city_name) v_city_name;
    TYPEOF(SINGLEPRIMRANGE_method_prim_range.st) st;
    TYPEOF(SINGLEPRIMRANGE_method_prim_range.zip) zip;
    TYPEOF(SINGLEPRIMRANGE_method_prim_range.prim_range) SINGLEPRIMRANGE_prim_range;
    UNSIGNED prim_range_SINGLEPRIMRANGE_Row_Cnt;
  END;
  R0 T0(SINGLEPRIMRANGE_method_prim_range ri) := TRANSFORM
    SELF.SINGLEPRIMRANGE_prim_range := ri.prim_range;
    SELF.prim_range_SINGLEPRIMRANGE_Row_Cnt := ri.Row_Cnt;
    SELF := ri;
  END;
  J0 := PROJECT(SINGLEPRIMRANGE_method_prim_range,T0(left));
EXPORT BestBy_cnp_name_prim_name_derived_v_city_name_st_zip_np := J0;
EXPORT BestBy_cnp_name_prim_name_derived_v_city_name_st_zip := BestBy_cnp_name_prim_name_derived_v_city_name_st_zip_np  : PERSIST('~temp::Proxid::BIPV2_ProxID_mj6::BestBy_cnp_name_prim_name_derived_v_city_name_st_zip::best');
// Now gather some statistics to see how we did
  R := RECORD
    NumberOfBasis := COUNT(GROUP);
    REAL8 SINGLEPRIMRANGE_prim_range_pcnt := AVE(GROUP,IF(BestBy_cnp_name_prim_name_derived_v_city_name_st_zip.SINGLEPRIMRANGE_prim_range=(typeof(BestBy_cnp_name_prim_name_derived_v_city_name_st_zip.SINGLEPRIMRANGE_prim_range))'',0,100));
  END;
...
SHARED F_BestBy_cnp_name_prim_name_derived_v_city_name_st_zip(DATASET({BestBy_cnp_name_prim_name_derived_v_city_name_st_zip}) d) := FUNCTION
  ...
  R T(BestBy_cnp_name_prim_name_derived_v_city_name_st_zip le) := TRANSFORM
    SELF.prim_range := le.SINGLEPRIMRANGE_prim_range;
    SELF := le; // Copy BASIS
  END;
  P1 := PROJECT(d,T(LEFT));
  RETURN P1;

...
EXPORT IParam := MODULE
	...
	EXPORT getSearchParams(iesp.bair_cfs.t_BAIRCFSSearchBy searchBy) := FUNCTION
		
		base_params := BairRx_Common.IParam.getSearchParams(searchBy, BairRx_Common.Constants.SearchType.CFS);
		
		BairRx_Common.Layouts.ModeContext xtModeContext() := TRANSFORM						
			SELF.mID := BairRx_Common.Constants.Mode.CFS,
			SELF.codes := BairRx_Common.Functions.SplitClassCodes(BairRx_Common.Constants.Mode.CFS, searchBy.Classification),
			SELF.clean_codes := BairRx_Common.Functions.SplitClassCodes(BairRx_Common.Constants.Mode.CFS, searchBy.Classification, TRUE),
		...
		BairRx_Common.Layouts.ActiveModes xtActiveMode() := TRANSFORM
			SELF.CFS := exists(inModeContext[1].codes);
		END;

IMPORT BairRx_Common,BairRx_CFS, iesp;

EXPORT GetIDs(BairRx_Common.IParam.SearchParam inMod) := FUNCTION
	
		MC := inMod.ModeContext(mID=BairRx_Common.Constants.Mode.CFS)[1];
		DR := MC.data_relation;
		...
		BoolEIDs := IF ((booleanSearch AND COUNT(GIDs) < BairRx_Common.Constants.Filter.MaxFocusDocs)
								,PROJECT(GIDs,TRANSFORM(iesp.share.t_StringArrayItem,SELF.value := LEFT.EID)));

		// boolean search only if filters are enabled.
		dBoolSrchRecs := IF(booleanSearch, BairRx_Common.BooleanFilterSearch(inMod, BairRx_Common.Constants.SearchType.CFS, expFilter, GH.SearchGrid,BoolEIDs,booleanSearch));		

/*2017-08-11T00:20:52Z (Wendy Ma)
bug# DF-19884
*/
import UtilFile,ut,address,AID,header, NID;

export map_util_daily_new(dataset(UtilFile.layout_util.daily_in_new) util_daily_in)	:= module
util_daily_in_dedup := dedup(util_daily_in, all);

//adding check point for SSN suppression

util_daily_SSN_correction := UtilFile.fn_SSN_correction(util_daily_in_dedup);
...
file_daily_preID := project(util_daily_SSN_correction, temp_rec);
...
self.addr_dual      := if(S_address = '' and B_address = '', 'F',if(S_address <> '' and B_address <> '' and S_address <> B_address, 'Y' ,
if(S_address <> '' and B_address <> '' and S_address = B_address, 'M', 'N')));
...
if(regexfind(searchpattern2, TRIM(le.last_name,LEFT,RIGHT)), remove_double_prefix(name_func(trim(le.last_name,left,right))),if(regexfind(searchpattern1, TRIM(le.last_name,LEFT,RIGHT)),
remove_prefix(name_func(trim(le.last_name,left,right))), name_func(trim(le.last_name,left,right)))));
...
//blank out in
UtilFile.mac_cleandates(map_combine, map_combine_clean_dates)

//blank out invalid dates in DID file
export util_daily_clean:= map_combine_clean_dates;

...
export map_util_daily(dataset(UtilFile.layout_util.daily_in) util_daily_in)	:=
module

util_daily_in_dedup := dedup(util_daily_in, all);

temp_rec := record
integer id := 0;
UtilFile.layout_util.daily_in;
end;
 
file_daily_preID := project(util_daily_in_dedup, temp_rec);

Ut.MAC_Sequence_Records(file_daily_preID, id, daily_id)

//normalize on dual address

utilfile.Layout_Util.temp tnormAddr(daily_id le, integer cnt) := transform

self.id := (string15)(ut.DaysSince1900(ut.GetDate[1..4], ut.GetDate[5..6], ut.GetDate[7..8]) + intformat(le.id, 10, 1));
self.addr_type      := choose(cnt, if(le.S_address <> '', 'S', ''), if(le.B_address <> '' and le.S_address <> le.B_address, 'B', ''));
self.address        := choose(cnt, le.S_address, le.B_address);
self.addr_dual      := if(Le.S_address = '' and Le.B_address = '', 'F',if(Le.S_address <> '' and Le.B_address <> '' and Le.S_address <> Le.B_address, 'Y' ,'N'));
...
Norm_dual_addr_slim := dedup(Norm_dual_addr(addr_dual = 'F' or addr_type <> ''), all);
...
self.address_street := le.address[1..10];
self.address_street_name := le.address[11..110];
self.address_street_type := le.address[111..115];
self.Address_Street_direction := le.address[116..117];
self.Address_Apartment := le.address[118..127];
self.Address_City := le.address[128..147]; 
self.Address_State := le.address[148..149]; 
self.Address_Zip := le.address[150..159]; 
self.connect_date := le.Service_Connect_date;
self.util_type := le.Service_Type;
self.phone := le.service_Phone;
...
append_nameid := name_out : persist('~thor_data400::persist::utility_daily_name_flag');
...
shared map_combine := (map_addr_clean + map_invalid_addr)(UtilFile.Blocked_data()) : persist('~thor_data400::persist::utility_daily_clean');

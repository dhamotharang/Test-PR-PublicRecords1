...
export map_util_nctue_hist(dataset(UtilFile.layout_utility_nctue_in) util_daily_in)	:= module
...
file_daily_preID := project(util_daily_in, temp_rec);
...
self.Exchange_Serial_Number := le.exchange_reference_number;
self.Date_added_to_exchange := le.most_recent_update_date;
self.Connect_date           := le.last_update_date;
...
append_nameid := name_out : persist('~thor_data400::persist::utility_nctue_hist_name_20130531');
...
shared map_combine := (map_addr_clean + map_invalid_addr)(UtilFile.Blocked_data()) : persist('~thor_data400::persist::utility_nctue_hist_only_clean_20130531');

export util_NCTUE_clean:= map_combine;
...
export util_nctue_canadian := map_combine((unsigned)append_rawaid = 0 and is_canadian_phone(work_phone, phone));

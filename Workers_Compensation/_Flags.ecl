...
export _Flags := module
 ...
 export ExistCurrentSprayed		 := nothor(fileservices.superfileexists(filenames().input.using)) and 
												    count(nothor(fileservices.superfilecontents(filenames().input.using))) > 0;
 export ExistBaseFile					 := count(nothor(fileservices.superfilecontents(filenames().base_full.qa))) > 0;	

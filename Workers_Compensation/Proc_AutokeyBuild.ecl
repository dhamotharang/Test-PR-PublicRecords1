...
EXPORT proc_autokeybuild(STRING filedate) := FUNCTION
		...
		c := Workers_Compensation.Constants(filedate);
	...
	mod_AKB := module(AutokeyB2.Fn_Build.params) 
		...
		EXPORT STRING L_inkeyname := 	Workers_Compensation.constants().ak_keyname; 
		EXPORT STRING L_inlogical := 	Workers_Compensation.constants(filedate).ak_logical; 

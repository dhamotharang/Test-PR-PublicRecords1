...
phplus_in_s := sort(distribute(phplus_in, hash(CellPhoneIDKey)), CellPhoneIDKey, local);
ut.MAC_Sequence_Records(phplus_in_s, pdid, phplus_prep_id);

phplus_prep_s := sort(group(phplus_prep_id, CellPhoneIDKey), pdid);
...
recordof(phplus_in) t_assign_did(phplus_prep_s le, phplus_prep_s ri) := Transform
		...
		self.phone7_did_key := if(ri.did > 0, ri.phone7_did_key, hashmd5((data) le.phone7 + (data)self.did));
		self := ri;
end;

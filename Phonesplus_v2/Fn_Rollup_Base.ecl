...
export Fn_Rollup_Base(dataset(recordof(Phonesplus_v2.Layout_PhonesPlus_Base/*DF-25784*/)) phplus_in, string file_type = '', string pversion) := function
...
recordof(phplus_in) tRollup(current_and_prev_s le, current_and_prev_s ri) := TRANSFORM
  ...
  SELF.last_build_date			    :=	MAX(le.last_build_date, ri.last_build_date);
	SELF := ri;
end;

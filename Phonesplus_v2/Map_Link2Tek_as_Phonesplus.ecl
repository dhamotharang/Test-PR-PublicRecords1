//****************Link2Tek to a common layout********************
import ut, _validate, VersionControl, mdr, Link2Tek;
phone_file:= Link2Tek.Files().base.qa;	
...
Layout_In_Phonesplus.Layout_In_Common t_map_common_layout(phone_f input) := Transform
  ...
  self.orig_phone_type      := 'W';	
	self.DateFirstSeen 				:=  (unsigned3)(_validate.date.fCorrectedDateString((string)input.dt_last_seen)[1..6])[1..6];
	self.DateLastSeen 				:=  (unsigned3)(_validate.date.fCorrectedDateString((string)input.dt_first_seen)[1..6]);
	...
	self.src_all									:= if(input.isupdating, translation_codes.source_bitmap_code(mdr.sourceTools.src_Link2Tek), 10);
  self.append_avg_source_conf 	:= phonesplus_v2.Translation_Codes.source_conf(mdr.sourceTools.src_Link2Tek);
	...
	self.OrigName					:= StringLib.StringCleanSpaces(input.orig_first_name + ' ' + input.orig_middle_name + ' ' + input.orig_last_name);
	self.Address1				:= StringLib.StringCleanSpaces(input.orig_house_number + ' ' +
																										 input.orig_predirection + ' ' + 
																										 input.orig_street_name + ' ' + 
																										 input.orig_suffix + ' ' + 
																										 input.orig_postdirection );

	self.Address2				:=StringLib.StringCleanSpaces(input.orig_unit_name +  ' ' + 
																										 input.orig_unit_number);
	
	self.OrigCity				:= input.orig_city;
	self.OrigState			:= input.orig_state;
	self.OrigZip				:= input.orig_zip;
	self.state					:= input.st;
	self.zip5						:= input.zip;
	self.orig_phone			:= input.orig_Phone;
	self.npa						:= input.clean_phone[..3];
	self.phone7					:= input.clean_phone[4..];
...
export Map_Link2Tek_as_Phonesplus := project(phone_f,t_map_common_layout(left)) 
									  :persist('~thor_data400::persist::Phonesplus::Map_Link2Tek_as_Phonesplus');
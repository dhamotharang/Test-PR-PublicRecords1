...
odl := PROJECT(CHOOSEN(BIPV2_POWID_Platform.Keys(BFile).Candidates(POWID=POWIDone),100000),BIPV2_POWID_Platform.match_candidates(BFile).layout_candidates);
odr := PROJECT(CHOOSEN(BIPV2_POWID_Platform.Keys(BFile).Candidates(POWID=POWIDTwo),100000),BIPV2_POWID_Platform.match_candidates(BFile).layout_candidates);
k := BIPV2_POWID_Platform.Keys(BFile).Specificities_Key;
s := GLOBAL(PROJECT(k,BIPV2_POWID_Platform.Layout_Specificities.R)[1]);
odlv := BIPV2_POWID_Platform.Debug(BFile,s).RolledEntities(odl);
odrv := BIPV2_POWID_Platform.Debug(BFile,s).RolledEntities(odr);
BIPV2_POWID_Platform.match_candidates(BFile).layout_attribute_matches ainto(BIPV2_POWID_Platform.Keys(BFile).Attribute_Matches le) := TRANSFORM
  SELF := le;
END;
am := PROJECT(BIPV2_POWID_Platform.Keys(BFile).Attribute_Matches(POWID1=POWIDone,POWID2=POWIDtwo)+BIPV2_POWID_Platform.Keys(BFile).Attribute_Matches(POWID1=POWIDtwo,POWID2=POWIDone),ainto(LEFT));
mtch := BIPV2_POWID_Platform.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,POWIDone,POWIDtwo,0,0}],BIPV2_POWID_Platform.match_candidates(BFile).layout_matches),am);

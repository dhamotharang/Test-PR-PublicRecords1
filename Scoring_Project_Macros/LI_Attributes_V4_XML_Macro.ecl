...
outputshell := '~sghatti::out::nonFcra_LIAttributes_v4_shell_'  + ut.GetDate  ;
...
// did_results:=Scoring_Project_Macros.IID_macro(f,threads,roxieIP,dppapurpose,GLB,DRM,HistoryDate);  
...
layout_soap transform_input_request( LeadIntegrity_input_temp le) := TRANSFORM
	u := PROJECT(ut.ds_oneRecord, TRANSFORM(iesp.share.t_User, SELF.AccountNumber := le.accountnumber; SELF.DLPurpose := '0'; SELF.GLBPurpose := (string)GLB; SELF.DataRestrictionMask := DRM; SELF := []));
...
LeadIntegrityoutput myFail2(LeadIntegrity_input le) := TRANSFORM
	...
	self.Result.InputEcho.Seq:=le.old_account_number;
	SELF := le;
	SELF := [];
END;
...
op :=  OUTPUT(CHOOSEN(LeadIntegrity_attributes(errorcode<>''), eyeball), NAMED('LeadIntegrity_errors'));

...
// isFCRA:=if(Scoring_Project_PIP.User_Settings_Module.RV_Attributes_V2_BATCH_CreditAcceptance_settings.isFCRA=true,'FCRA','NONFCRA');
...
l t_f(f1 le, INTEGER c) := TRANSFORM
...
p_f := project(f1, t_f(left, counter));
// output(choosen(p_f, eyeball), named('RV_Input'));
// output(count(p_f));



soap_in :=DISTRIBUTE(PROJECT(p_f,TRANSFORM(layout_soap,SELF := LEFT)), RANDOM());
...
get_attribute(dataset(Layout_AttributeGroup) groupi, integer i) := function
	...
	// end of temporary conversion for booleans
   			...
   			// ox := RECORD
   	// recordof(just_rv_attributes_v2);
   ...
   END;
    ...
    Global_output_lay normit(soap_results L, soap_in R) := transform
...
self.idTheftFlag:=v2215.attribute[1].value;
      ...
      j_f := JOIN(soap_results,soap_in,(integer)LEFT.accountnumber=(integer)RIGHT.accountnumber,normit(LEFT,RIGHT));

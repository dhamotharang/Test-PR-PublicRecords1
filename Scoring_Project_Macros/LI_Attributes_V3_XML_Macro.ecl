EXPORT LI_Attributes_V3_XML_Macro (roxie_ip,Gateway_dummy, Thread, Timeout, Retry, Input_file_name,Output_file_name, records_ToRun):= functionmacro
...
shell_input := DISTRIBUTE(p_f(ssn = ''), RANDOM()); //modified to only pass in the records without SSN to avoid duplicate records

LeadIntegrity_input_temp := shell_input;

op1 := output(LeadIntegrity_input_temp, named('LeadIntegrity_input_temp'));
...
op2 := output(LeadIntegrity_input, named('LeadIntegrity_input'));
// OUTPUT(CHOOSEN(LeadIntegrity_input, 200), NAMED('LeadIntegrity_input'));  
...
op3 := output(LeadIntegrity_attributes, named('LeadIntegrity_attributes'));				
// OUTPUT(CHOOSEN(LeadIntegrity_attributes(errorcode=''), eyeball), NAMED('LeadIntegrity_results'));
op := OUTPUT(CHOOSEN(LeadIntegrity_attributes(errorcode<>''), eyeball), NAMED('LeadIntegrity_errors_30'));
...
END;
	Global_output_lay slim_v3( LeadIntegrity_input le, LeadIntegrity_attributes ri ) := TRANSFORM

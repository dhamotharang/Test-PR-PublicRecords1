EXPORT BusinessShell_NonSBFE_Report := function

// #workunit('name', 'BusinessShell NonSBFE Report');
...
dt2_1 := 	p_file_name1[length(p_file_name1)-9.. length(p_file_name1)-2];
  ...
  new_bins_remove_file:= if(  STD.File.FileExists('~scoring_project::businessshell_NONSBFE_stats_'+ dt1_1) ,
	
   																				 
         	           sequential(FileServices.DeletelogicalFile('~scoring_project::businessshell_NONSBFE_stats_' + dt1_1)));
   															
   					
  old_bins_remove_file:= if(  STD.File.FileExists('~scoring_project::businessshell_NONSBFE_stats_'+ dt2_1) ,
   																				 
         	           sequential(FileServices.DeletelogicalFile('~scoring_project::businessshell_NONSBFE_stats_' + dt2_1)) 
   															);	
      			 
  new_bins:=  Scoring_Project.BusinessShell_NONSBFE_MACRO(dt1_1,dt2_1);	
                   																		 
  old_bins:=  Scoring_Project.BusinessShell_NONSBFE_MACRO(dt2_1,dt1_1);	
...
END;	
 ...
 jn := JOIN(final_results, tb, left.field_name = right.field_name and
															 ...
															 TRANSFORM(compare_layout_3,
															 					self.field_name := StringLib.StringGetNthWord(StringLib.StringFindReplace(left.field_name, '.', ' '), StringLib.StringFindCount(left.field_name, '.') + 1);
																				self.sumofPSI := right.sumofPSI;
																				self.warning_flag := If(right.sumofPSI > 0.1 , 1, 0);
																																						self := left;));
																																

			out_file := output(choosen(jn,all), ,'~scoring_project::businessShell_NONSBFE_results_new_' + dt1_1, CSV(heading(single), quote('"')), overwrite );
			
			
				string out_file_layout := '';
      outfile := dataset('~scoring_project::businessShell_NONSBFE_results_new_' + dt1_1, typeof(out_file_layout));
...
// seq: WHEN(CRON('00 10 * * *')), 
 FAILURE(FileServices.SendEmail(Scoring_Project_DailyTracking.email_distribution.Bocashell_collections_fail_list,'BusinessShell NONSBFE Attributes Macro Call and Distribution Report failed','The failed workunit is:' + workunit + FailMessage));
 // FAILURE(FileServices.SendEmail('bridgett.braaten@lexisnexis.com','BusinessShell NONSBFE Attributes Macro Call and Distribution Report failed','The failed workunit is:' + workunit + FailMessage));

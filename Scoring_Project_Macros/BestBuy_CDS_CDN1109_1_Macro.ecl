...
dset:= IF(no_of_records > 0, CHOOSEN(DATASET( infile_name, layout_input,thor), no_of_records),
...
f:=		 PROJECT(dset,TRANSFORM(layout_input_pjt,SELF :=
 LEFT));
...
layout_soap_boca transform_input_boca(f le) := TRANSFORM
	SELF.old_account_number := (string)le.AccountNumber;
	SELF.Accountnumber := (STRING)le.Accountnumber;	
	SELF.DPPAPurpose := DPPA;
	SELF.GLBPurpose := GLB;
	SELF.adl_based_shell := ADL_shell;
	SELF.DataRestrictionMask := DRM;
	SELF.HistoryDateYYYYMM := HistoryDate;
	SELF.Version := Version;
	SELF.wuid := thorlib.wuid();
	self.jobowner := thorlib.jobowner();
	self.FirstName:= le.pii1.FirstName;
	self.MiddleName:= le.pii1.MiddleName;
	self.LastName:= le.pii1.LastName;
	self.StreetAddress:= le.pii1.StreetAddress;
	self.City:= le.pii1.City;
	self.State:= le.pii1.State;
	self.Zip:= le.pii1.Zip;
	self.HomePhone:= le.pii1.HomePhone;
	SELF := le;
	SELF := [];
END;

p_f_1 := PROJECT(f, transform_input_boca(LEFT));
shell_input := DISTRIBUTE(p_f_1, RANDOM());
...
layout_soap_in to_soap(rawInput le) := TRANSFORM
	...
	self.DataRestrictionMask := DRM;  // to allow use everything
...
soap_in1 := PROJECT(rawInput, to_soap(LEFT));
soap_in := distribute(soap_in1, random());
...
// OUTPUT(CHOOSEN(soap_in, eyeball), NAMED('Sample_SOAP_In'));
   ...
   layout_with_errcode := RECORD
      ...
      layout_with_errcode myFail1( soap_in le ) := TRANSFORM
...
END;
            ...
            Global_output_lay flatten(soapresults le) := transform
            ...
            soap_out  := project(soapresults,flatten(left));
         	...
         	SELF.fname := le.pii1.firstname;
         	SELF.mname := le.pii1.middlename;
         	SELF.lname := le.pii1.lastname;
         	SELF.in_streetAddress := le.pii1.streetaddress;
         	SELF.in_city := le.pii1.city;
         	SELF.in_state := le.pii1.state;
         	SELF.in_zipCode := le.pii1.zip;
         	SELF.phone10 := le.pii1.homephone;
         	SELF.ssn := le.pii1.ssn;
         	SELF.dob := le.pii1.dateofbirth;
         	SELF.wphone10 := le.pii1.workphone;
         	// SELF. := le.Income;
         	SELF.dl_number := le.pii1.dlnumber;
         	SELF.dl_state := le.pii1.dlstate;
         	// SELF. := le.Balance;
         	// SELF. := le.ChargeOffDate;
         	// SELF. := le.FomerLast;
         	SELF.email_address := le.pii1.email;
         	SELF.employer_name := le.pii1.company;
...
did_layout_input func(dset l):=transform
...
did_infile := project(dset,func(left));
           

did_results:=Scoring_Project_Macros.IID_macro(did_infile,threads,roxieIP,DPPA,GLB,DRM,HistoryDate);            
               

Global_output_lay xform(did_results l, soap_out r) := TRANSFORM
               self.did := l.did;
               self := r;
               end;
               
res := JOIN(did_results, soap_out, left.acctno = right.acctno, xform(left, right),right outer);
...
Global_output_lay xform1(res l, soap_in r) := TRANSFORM
                  		            ...
                  		            self.DID := l.did; 
                  				        SELF.historydate:=(string)r.historydateyyyymm;
   																...
   																// SELF.DOBPop:= r.pii1.dateofbirth<>'';
...
final_output:=join(res,soap_in,left.acctno=(string)right.account ,xform1(left, right));	

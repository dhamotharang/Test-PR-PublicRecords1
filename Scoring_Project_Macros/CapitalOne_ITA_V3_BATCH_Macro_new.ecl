...
/* // UPDATED ON 9/17 PER EMAIL FROM R. PETROSINO LABLED RE: CAPONE ITA - Nathan
   DPPA := 0;
   GLB := 5;
   // Set to TRUE to run the ADL Based Boca Shell, FALSE to skip it
   ADL_shell := true;
   DRM := '000000000000010000000000000000';  // to allow use of both Equifax and Experian, this is the default value for all legacy scoring products
   DPM := '110001000100000000000000000000';
   HistoryDate := 999999;
   // Boca Shell Version and LeadIntegrity version
   Version := 3;
...
inputData1 := dataset( infile_name, newlayout,thor);
...
layout_soap_input make_rv_in(f le, integer c) := TRANSFORM
	...
	SELF.batch_in := batch;
	// SELF.gateways := DATASET([{'FCRA', roxieIP}], risk_indicators.layout_gateways_in); // No need for gateways
	SELF.DPPAPurpose := DPPA;  // DPPA defaults to 0 if not included in input layout
	SELF.GLBPurpose := GLB;  // GLB defaults to 5 if not included in input layout
  SELF.DataRestrictionMask := DRM;
  SELF.DataPermissionMask := DPM;
	// SELF.ModelName;  //Not inputting any model names
	SELF.Version := Version;
	SELF.HistoryDateYYYYMM := HistoryDate;
	SELF := [];
END;

soap_in := DISTRIBUTE(PROJECT(f, make_rv_in(LEFT, counter)), RANDOM());
...
Global_output_lay v3_trans(LeadIntegrityoutput le, unsigned6 c) := TRANSFORM
...
Global_output_lay xform(did_results l, ds_slim r) := TRANSFORM
                                                       self.did := l.did;
                                                       self := r;
			                                                 self:=[];
                                                   end;
      
res := JOIN(did_results,ds_slim, left.acctno = right.acctno, xform(left, right),right outer);



 ds_with_extras := JOIN(res, soap_in, LEFT.acctno =RIGHT.batch_in[1].acctno,
...
// output(count(inputData1), named('rawdata_count'));
// output(choosen(inputData1, 25), named('rawdata'));
// output(count(inputData1(ssn = '')), named('rawdata_count_blank_ssn'));
// output(count(inputData1(ssn <> '')), named('rawdata_count_pop_ssn'));

// output(count(soap_in), named('soap_in_input_count'));
// output(choosen(soap_in, 25), named('soap_in'));
// output(count(soap_in(batch_in[1].ssn = '')), named('soap_in_inputcount_blank_ssn'));
// output(count(soap_in(batch_in[1].ssn <> '')), named('soap_in_input_count_pop_ssn'));

// output(count(appended_ssns), named('appendedssn_count'));
// output(choosen(appended_ssns, 25), named('appendedssn'));
// output(count(appended_ssns(ssn = '')), named('appendedssn_count_blank_ssn'));
// output(count(appended_ssns(ssn <> '')), named('appendedssn_count_pop_ssn'));

// output(count(ds_with_extras), named('VerifiedSSN_count'));
// output(count(ds_with_extras(Version3__VerifiedSSN = '')), named('VerifiedSSN_missing'));
// output(count(ds_with_extras(Version3__VerifiedSSN = '-1')), named('VerifiedSSN_negone'));
// output(count(ds_with_extras(Version3__VerifiedSSN = '0')), named('VerifiedSSN_zero'));
// output(count(ds_with_extras(Version3__VerifiedSSN not in ['', '-1', '0'])), named('VerifiedSSN_hit'));

...
layout_soap transform_input_request(ita_input_temp le) := TRANSFORM
	u := PROJECT(ut.ds_oneRecord, TRANSFORM(iesp.share.t_User, SELF.AccountNumber := le.old_account_number; SELF.DLPurpose := '0'; SELF.GLBPurpose := (string)GLB; SELF.DataRestrictionMask := DRM; SELF := []));
...
itaoutput myFail2(ita_input le) := TRANSFORM
	...
	self.Result.InputEcho.Seq:=le.FullAccountNumber;
	SELF := le;
	SELF := [];
END;
...
final := JOIN(ita_input, ita_attributes, LEFT.LeadIntegrityRequest[1].SearchBy.Seq=RIGHT.Result.InputEcho.Seq,
	TRANSFORM(Global_output_lay,
		SELF.acctno := LEFT.FullAccountNumber;
...
op_final   := output(final,, outfile_name,thor,compressed, OVERWRITE);

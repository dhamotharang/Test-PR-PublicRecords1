 EXPORT FP_Scores_and_Attributes_XML_Macro (roxie_ip,Gateway_dummy, Thread, Timeout, Retry, Input_file_name,Output_file_name, records_ToRun):= functionmacro
...
DPPA:=Scoring_Project_PIP.User_Settings_Module.FP_V2_XML_Generic_FP1109_0_settings.DPPA;
GLB:=Scoring_Project_PIP.User_Settings_Module.FP_V2_XML_Generic_FP1109_0_settings.GLB;
DRM:=Scoring_Project_PIP.User_Settings_Module.FP_V2_XML_Generic_FP1109_0_settings.DRM;
...
get_attribute(dataset(Layout_AttributeGroup) groupi, integer i) := function
   ...
   normed := JOIN(resu, dist_dataset,LEFT.accountnumber=RIGHT.accountnumber, normit(LEFT,RIGHT));
   ...
   op_final :=   output(NewRecs,,Output_file_name,thor,compressed, overwrite );

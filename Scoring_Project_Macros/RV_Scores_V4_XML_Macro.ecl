...
DPPA:=Scoring_Project_PIP.User_Settings_Module.RV_Scores_V4_XML_Generic_settings.DPPA;
GLB:=Scoring_Project_PIP.User_Settings_Module.RV_Scores_V4_XML_Generic_settings.GLB;
...
// adding internal extras
			
j_final_internal_extras := join(j_final, dist_dataset, left.acctno=right.accountnumber, transform(Global_output_lay,   					
		...
		self.SSNLength := (string)(length(trim(right.ssn,left,right))) ;
		self.DOBPop := right.dateofbirth<>'';
		self.EmailPop := right.email<>'';
		self.IPAddrPop := right.ipaddress<>'';
  ...
  op_final :=  output(j_final_internal_extras,,outfile_name,thor,compressed, overwrite );

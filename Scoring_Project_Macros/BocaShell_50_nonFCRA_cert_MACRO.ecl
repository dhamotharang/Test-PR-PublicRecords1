EXPORT BocaShell_50_nonFCRA_cert_MACRO( bs_version, nonfcraroxie_IP,neutralroxie_IP, Thread, Timeout, Retry, Input_file_name,Output_file_name, records_ToRun, retro_date = 999999):= functionmacro
			...
			RemoveFares := Scoring_Project_Macros.User_Settings_Module.BocaShell_50_NONFCRA_settings.RemoveFares;	// change this to TRUE for FARES filtering
			LeadIntegrityMode := Scoring_Project_Macros.User_Settings_Module.BocaShell_50_NONFCRA_settings.LeadIntegrityMode;  // change this to TRUE for LeadIntegrity modeling

			DRM := Scoring_Project_Macros.User_Settings_Module.BocaShell_50_NONFCRA_settings.DRM;  // byte 6, if 1, restricts experian, byte 8, if 1, restricts equifax, 
			                                                                                   // byte 10 restricts Transunion, 12 restricts ADVO, 13 restricts bureau deceased data
			DPPA := Scoring_Project_Macros.User_Settings_Module.BocaShell_50_NONFCRA_settings.DPPA;
			GLB := Scoring_Project_Macros.User_Settings_Module.BocaShell_50_NONFCRA_settings.GLB;
			...
			final_output := OUTPUT (ds_soap_output_pjt, , out_name_head , thor,cluster('thor400_31_store'),compressed, overwrite,expire(30));

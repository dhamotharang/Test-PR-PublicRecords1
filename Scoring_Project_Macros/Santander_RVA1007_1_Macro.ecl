EXPORT Santander_RVA1007_1_Macro (fcraroxie_IP,neutralroxie_IP, Thread, Timeout, Retry, Input_file_name,Output_file_name, records_ToRun):= functionmacro
...
f1 := IF(no_of_records <= 0, dataset( infile_name, prii_layout, thor ),
                            CHOOSEN(DATASET( infile_name, prii_layout, thor), no_of_records));
...
paramsA := dataset([{'Custom', 'rva1007_1'}], models.Layout_Parameters);	// use this for non-version1 models, also possibly change the scores that you call below

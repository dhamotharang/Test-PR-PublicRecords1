...
GLB:=Scoring_Project_PIP.User_Settings_Module.RV_Attributes_V3_XML_Generic_settings.GLB;
DPPA:=Scoring_Project_PIP.User_Settings_Module.RV_Attributes_V3_XML_Generic_settings.DPPA;
DRM:=Scoring_Project_PIP.User_Settings_Module.RV_Attributes_V3_XML_Generic_settings.DRM;
...
l t_f(f le, INTEGER c) := TRANSFORM
	...
	SELF.RequestedAttributeGroups := dataset([{IncludeVersion3}], layout_attributes_in); // to get version1 attributes, you need to modify this
...
// self.gateways := dataset([{'neutralroxie', 'http://roxiebatch.br.seisint.com:9856'}], risk_indicators.Layout_Gateways_In);
	...
	self.DataRestrictionMask := DRM; // to restrict fares, experian and transunion 
...
just_groups := project(l, transform(layout_RVAttributesOut, self := l.attributegroups));
   ...
   j_f := JOIN(resu,dist_dataset,LEFT.accountnumber=(string)RIGHT.accountnumber,normit(LEFT,RIGHT));

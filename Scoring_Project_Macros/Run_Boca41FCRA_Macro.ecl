EXPORT Run_Boca41FCRA_Macro := function


// #workunit('name','Boca41FCRA_Macro_scheduling');
...
dt := ut.getdate;
 ...
 filenames_details1 :=  OUTPUT(nothor(STD.File.LogicalFileList('scoringqa::out::fcra::bocashell_41_historydate_999999_cert_*_1')),,'~scoringqa::out::fcra::bocashell_41_historydate_999999_cert_file_' + dt, thor, overwrite );
...
ds1 := dataset('~scoringqa::out::fcra::bocashell_41_historydate_999999_cert_file_'+ dt , lay1, thor);
...
c_file_name1 := filelist1[1].name[1..59] + curr_date ;
...
dt2_1 := 	p_file_name1[length(p_file_name1)-9.. length(p_file_name1)-2];
  ...
  new_bins_remove_file:= if(  STD.File.FileExists('~scoring_project::bocashell_41_cert_FCRA_stats_'+ dt1_1) ,
   																				 
         	           sequential(FileServices.DeletelogicalFile('~scoring_project::bocashell_41_cert_FCRA_stats_' + dt1_1)) 
   															);
   					
  old_bins_remove_file:= if(  STD.File.FileExists('~scoring_project::bocashell_41_cert_FCRA_stats_'+ dt2_1) ,
   																				 
         	           sequential(FileServices.DeletelogicalFile('~scoring_project::bocashell_41_cert_FCRA_stats_' + dt2_1)) 
   															);	
      			 
  new_bins:=  Scoring_Project.BocaShell_41_Cert_FCRA_MACRO(dt1_1,dt2_1);	
                   																		 
  old_bins:=  Scoring_Project.BocaShell_41_Cert_FCRA_MACRO(dt2_1,dt1_1);	
...
dset_curr1 := dataset('~scoring_project::bocashell_41_cert_FCRA_stats_' + dt1_1, layout, thor);
dset_prev1 := dataset('~scoring_project::bocashell_41_cert_FCRA_stats_' + dt2_1, layout, thor);
...
END;	
 ...
 jn := JOIN(final_results, tb, left.field_name = right.field_name and
			...
			out_file := output(choosen(jn,all), ,'~scoring_project::bocashell_41_cert_FCRA_results_new_' + dt1_1, CSV(heading(single), quote('"')), overwrite );
			
			
				string out_file_layout := '';
      outfile := dataset('~scoring_project::bocashell_41_cert_FCRA_results_new_' + dt1_1, typeof(out_file_layout));
 ...
 // distro :=  SEQUENTIAL(out_file,	fileservices.SendEmailAttachText('Benjamin.Karnatz@lexisnexis.com',		
 distro :=  SEQUENTIAL(out_file,	fileservices.SendEmailAttachText(Scoring_Project_DailyTracking.email_distribution.Bocashell_collections_success_list,					
					   'BOCASHELL 4.1 FCRA Attribute Distribution Report',
																				'BOCASHELL 4.1 FCRA Attribute Distribution Report '+ dt1_1 + ' vs ' + dt2_1 + '\n Please view attachment.',
																				 XtabOut[no_of_records].line ,
																				 'text/plain; charset=ISO-8859-3', 
																				'BocaShell 4.1 FCRA Attributes Comparing' + dt1_1 + '_vs_' + dt2_1 + '.csv',
																				 ,
																				 ,
...
final := sequential( filenames_details1, new_bins_remove_file,old_bins_remove_file,new_bins,old_bins,distro):
 ...
 FAILURE(FileServices.SendEmail(Scoring_Project_DailyTracking.email_distribution.Bocashell_collections_fail_list,'BOCASHELL 4.1 FCRA Attributes Macro Call and Distribution Report failed','The failed workunit is:' + workunit + FailMessage));

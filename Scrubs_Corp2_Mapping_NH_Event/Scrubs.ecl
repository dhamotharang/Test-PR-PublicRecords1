...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.corp_key_Total_ErrorCount > 0, 1, 0) + IF(le.corp_vendor_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_state_origin_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_sos_charter_nbr_Total_ErrorCount > 0, 1, 0) + IF(le.event_filing_reference_nbr_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.event_filing_date_Total_ErrorCount > 0, 1, 0) + IF(le.event_date_type_cd_ALLOW_ErrorCount > 0, 1, 0) + IF(le.event_date_type_desc_ALLOW_ErrorCount > 0, 1, 0) + IF(le.event_desc_CUSTOM_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.corp_key_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_key_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_vendor_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_state_origin_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_sos_charter_nbr_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_sos_charter_nbr_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.event_filing_reference_nbr_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.event_filing_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.event_filing_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.event_date_type_cd_ALLOW_ErrorCount > 0, 1, 0) + IF(le.event_date_type_desc_ALLOW_ErrorCount > 0, 1, 0) + IF(le.event_desc_CUSTOM_ErrorCount > 0, 1, 0);
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_event_desc(1)
          ,'Fields with errors'
...
EXPORT StandardStats(DATASET(Corp2_mapping.LayoutsCommon.Events) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Corp2_Mapping_NH_Event, Fields, 'RECORDOF(scrubsSummaryOverall)', '');

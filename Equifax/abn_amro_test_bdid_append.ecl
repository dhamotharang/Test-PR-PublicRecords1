...
// Extract list of BDIDs from ABN AMRO Businesses
busprep := abn_amro_test_prep(bdid <> 0);

layout_bdid_list := record
unsigned6 bdid;
end;

layout_bdid_list GetBDIDs(Layout_ABN_AMRO_Test_Base l) := transform
self := l;
end;

bdid_list_init := project(busprep, GetBDIDs(left));
...
// Select only BDIDs for ABN AMRO
...
Layout_ABN_AMRO_Test_Append InitBusinessAppend(Layout_ABN_AMRO_Test_Base l) := transform
self := l;
end;
...
Layout_ABN_AMRO_Test_Append AppendRecordCounts(Layout_ABN_AMRO_Test_Append l, layout_record_counts r) := transform
...
Layout_ABN_AMRO_Test_Append FormatAppend(Layout_ABN_AMRO_Test_Base l) := transform
self := l;
end;

busappnd_nobdid := project(abn_amro_test_prep(bdid = 0), FormatAppend(left));
						
export abn_amro_test_bdid_append := busappnd_nobdid + busappnd_counts : persist('TMTEST::abn_amro_nohit_bdid_append');

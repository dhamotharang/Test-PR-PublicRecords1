...
abn_amro_init := abn_amro_c2btest_bdid_append;

layout_gid_list := record
abn_amro_init.group_id;
end;

abn_amro_gid_list := table(abn_amro_init(group_id <> 0), layout_gid_list);
abn_amro_gid_list_dedup := dedup(abn_amro_gid_list, all);

// Get corresponding BDIDs for associated groups
...
self.bdid := r.bdid;
self.confidence_level := if(r.bdid <> 0, 2, l.confidence_level);
...
abn_amro_combine_gid_sort := sort(abn_amro_combined_dedup, seq, confidence_level, group_id, bdid)  : persist('TMTEST::abn_amro_c2btest_gid_append');

export abn_amro_c2btest_gid_append := abn_amro_combine_gid_sort;
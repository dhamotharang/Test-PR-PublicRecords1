...
nh_did := table(nh(did <> 0, bdid = 0 or (bdid <> 0 and bdid_score < 75)), layout_nohit_did);

layout_nohit_did AssignBDIDbyDID(bc_slim l, layout_nohit_did r) := transform
self.bdid := l.bdid;
self.bdid_score := 100 - ut.CompanySimilar100(l.company_name, r.INQR_BUS);
self := r;
end;
...
nh_did_bdid := join(bc_slim,
					...
					left.did = right.did and
					  ut.CompanySimilar100(left.company_name, right.INQR_BUS) <= 35,
...
Layout_NoHit_Test_Base AppendNH_DID_BDID(Layout_NoHit_Test_Base l, layout_nohit_did r) := transform
self.bdid := r.bdid;
...
// Use name if no DID
nh_name := table(nh(did = 0, lname <> '', INQR_STATE <> '', bdid = 0 or (bdid <> 0 and bdid_score < 75)), layout_nohit_name);

layout_nohit_name AssignBDIDbyName(bc_slim l, layout_nohit_name r) := transform
self.bdid := l.bdid;
self.bdid_score := 100 - ut.CompanySimilar100(l.company_name, r.INQR_BUS);
self := r;
end;
...
nh_name_bdid := join(bc_slim(company_state <> ''),
                    nh_name,
					left.company_state = right.INQR_STATE and
					  ut.CompanySimilar100(left.company_name, right.INQR_BUS) <= 25 and
...
Layout_NoHit_Test_Base AppendNH_Name_BDID(Layout_NoHit_Test_Base l, layout_nohit_name r) := transform
self.bdid := r.bdid;
self.bdid_score := r.bdid_score;
self := l;
end;
...
Layout_NoHit_Test_Base ReplaceNH(Layout_NoHit_Test_Base l, Business_Header_SS.Layout_BDID_OutBatch r) := transform
...
nh_bdid_best_full := join(nh_bdid_full,
						  ...
						  lookup) : persist('TMTEST::equifax_nohit_bdid_to_replace');
...
Layout_NoHit_Test_Base RemoveContactRecs(Layout_NoHit_Test_Base l, layout_seq r) := transform
self := l;
end;
...
Layout_NoHit_Test_Base ClearBDID(Layout_NoHit_Test_Base l ) := transform
self.bdid := 0;
self.bdid_score := 0;
self.best_phone := '';
self.best_fein := '';
...
export nohit_test_bdid_by_contact := nh_out : persist('TMTEST::equifax_nohit_bdid_by_contact');
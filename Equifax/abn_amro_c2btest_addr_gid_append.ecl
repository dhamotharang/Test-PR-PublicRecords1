...
abn_amro_init := abn_amro_c2btest_phone_append;
...
abn_amro_gid_list := table(abn_amro_init(group_id <> 0, confidence_level = 3), layout_gid_list);
...
self.bdid := r.bdid;
self.confidence_level := if(r.bdid <> 0, 5, l.confidence_level);
...
abn_amro_combine_gid_sort := sort(abn_amro_combined_dedup, seq, confidence_level, group_id, bdid)  : persist('TMTEST::abn_amro_c2btest_addr_gid_append');

export abn_amro_c2btest_addr_gid_append := abn_amro_combine_gid_sort;

/**
	creates the base file from the Cortera tradeline file
**/
import Std, mdr;
...
EXPORT proc_create_base(DATASET($.Layout_Tradeline) tradelines = $.In_Tradeline, 
												DATASET($.Layout_Tradeline_Base) base = $.Files.Base,
												integer4 dt = Std.Date.Today()) := FUNCTION
	
	unsigned8 seed := MAX(base, rid) + 1;
	adds := PROJECT(tradelines, xBase(LEFT, dt, seed + COUNTER));

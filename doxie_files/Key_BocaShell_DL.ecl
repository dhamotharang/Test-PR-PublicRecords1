...
dlrec get_dl_num(doxie_files.File_dl ri) :=
TRANSFORM
...
dl_id := group(SORT(distribute(project(doxie_files.File_dl(did != 0), get_dl_num(LEFT)), hash(did)),did, dl_number, source_code, orig_state, local),did, dl_number,  source_code, orig_state, local);

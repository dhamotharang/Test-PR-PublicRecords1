...
iids_wseq := project(doxie_files.File_VehicleContacts(did != 0), add_seqno(LEFT));

vehrec add_Vehicles(vehrec le, doxie_files.File_VehicleVehicles ri) :=
TRANSFORM
...
vehicles_added := group(sort(distribute(JOIN(distribute(iids_wseq, hash(seq_no)),
				   distribute(doxie_files.File_VehicleVehicles, hash(seq_no)),					
					left.seq_no = right.seq_no and  
					RIGHT.orig_vin<>'',
				   add_Vehicles(LEFT,RIGHT), local), hash(did)), did, vehicle1.vin, local), did, local);

vehrec roll_vehicles(vehrec le, vehrec ri) :=
TRANSFORM

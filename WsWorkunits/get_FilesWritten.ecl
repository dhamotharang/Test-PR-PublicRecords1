...
function
  ...
  ds_filesread    := WsWorkunits.get_FilesRead(pWorkunitID ,pesp,pUseGlobal);
  ...
  FilesWritten    := if(pUseGlobal = true
                        ,project(global(FilesWritten2,few),layouts.WsFileWritten)
                        ,project(FilesWritten2,layouts.WsFileWritten)
                     );

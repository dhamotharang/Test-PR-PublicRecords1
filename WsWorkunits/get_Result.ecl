...
function
  ...
  parsed_result     := WsWorkunits.parsefieldname(pResult);
  ...
  ds_results := WsWorkunits.get_All_Results(pWuid,pesp,pCount);
...
// Proxid_Cnt
  ...
  */
  ...
  ds_result := map(
     parsed_resultname != '' and parsed_fieldname != '' and exists(ds_result_field_match      )                                       => /*'1-' + */ds_result_field_match[1].field_value
    ,parsed_resultname != '' and parsed_fieldname != '' and exists(ds_result_fieldvalue_match ) and count_fields                  = 2 => /*'2-' + */ds_result_name(record_number = ds_result_fieldvalue_match[1].record_number,trim(field_value) != trim(parsed_fieldname ))[1].field_value
    ,parsed_resultname != ''                            and exists(ds_fieldvalue_match        ) and count_fields_fieldvalue_match = 2 => /*'3-' + */ds_result_name_fieldvalue_match    (record_number = ds_fieldvalue_match       [1].record_number,trim(field_value) != trim(parsed_resultname))[1].field_value
    ,parsed_resultname != ''                            and exists(ds_fieldname_match         )                                       => /*'4-' + */ds_fieldname_match[1].field_value
    ,parsed_resultname != ''                            and exists(ds_result_match            )                                       => /*'5-' + */ds_result_match   [1].field_value
    ,''
  );

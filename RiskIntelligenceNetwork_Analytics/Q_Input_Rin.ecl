...
EXPORT Q_Input_Rin(DATASET(RECORDOF(RiskIntelligenceNetwork_Analytics.Layouts.KelInputLayout)) __PInputPIIDataset) := MODULE
  ...
  SHARED E_Customer_Params := MODULE(E_Customer)
    SHARED __Mapping0 := 'gc_id(DEFAULT:UID|OVERRIDE:Customer_Id_:0),ind_type(OVERRIDE:Industry_Type_:0),agency_state(OVERRIDE:Jurisdiction_State_:\'\')';
  ...
  SHARED E_Drivers_License_Params := MODULE(E_Drivers_License)
    SHARED __Mapping0 := 'ottodriverslicenseid(DEFAULT:UID|OVERRIDE:Otto_Drivers_License_Id_:\'\'),associatedcustomerfileinfo(DEFAULT:_r_Customer_:0),sourcecustomerfileinfo(DEFAULT:_r_Source_Customer_:0),dl_number(OVERRIDE:License_Number_:\'\'),dl_state(OVERRIDE:State_:\'\')';
  ...
  SHARED E_Email_Params := MODULE(E_Email)
    SHARED __Mapping0 := 'ottoemailid(DEFAULT:UID),batchin_rec.email_address(OVERRIDE:Email_Address_:\'\'),isdisposableemail(OVERRIDE:_isdisposableemail_:0)';
    SHARED InLayout __Mapping0_Transform(InLayout __r) := TRANSFORM
      SELF.Host_ := __CN('');
  ...
  SHARED __EE75556 := __ENH_Event;
  SHARED __ST9562_Layout := RECORD

...
, DATASET(Layout_Acquireweb) Delta = DATASET([],Layout_Acquireweb)
, DATASET(Layout_Acquireweb) dsBase = In_Acquireweb // Change IN_Acquireweb to change input to ingest process
, DATASET(RECORDOF(Acquireweb_Email.prep_ingest))  infile = Acquireweb_Email.prep_ingest
) := MODULE
  SHARED NullFile := DATASET([],Layout_Acquireweb); // Use to replace files you wish to remove
  ...
  SHARED AllRecs := ORe+NR1+NR(RCID<>0) : PERSIST('~temp::Acquireweb_Email::Ingest_Cache',EXPIRE(Acquireweb_Email.Config.PersistExpire));
  ...
  EXPORT NewRecords_NoTag := PROJECT(NewRecords,Layout_Acquireweb);
  EXPORT OldRecords :=PROJECT( AllRecs(__Tpe=RecordType.Old), NoFlagsRec);
  EXPORT OldRecords_NoTag := PROJECT(OldRecords,Layout_Acquireweb);
  EXPORT UpdatedRecords := PROJECT(AllRecs(__Tpe=RecordType.Updated), NoFlagsRec);
  EXPORT UpdatedRecords_NoTag := PROJECT(UpdatedRecords,Layout_Acquireweb);
  EXPORT AllRecords := IF(incremental, NewRecords, PROJECT(AllRecs, NoFlagsRec));
  EXPORT AllRecords_NoTag := PROJECT(AllRecords,Layout_Acquireweb); // Records in 'pure' format

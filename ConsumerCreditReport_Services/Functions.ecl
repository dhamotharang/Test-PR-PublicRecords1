IMPORT Address, Didville, Gateway, FFD, iesp, PersonContext, Risk_Indicators, RiskView, Suppress;
...
EXPORT Functions := MODULE
	...
	SHARED do_Didville (DATASET(DidVille.Layout_Did_OutBatch) ds_didville_in, UNSIGNED3 ScoreThreshold) := FUNCTION
		...
		ds_didville_sort:=PROJECT(SORT(ds_didville_out(score>=ScoreThreshold),seq,-score),TRANSFORM(didRollRec,SELF:=LEFT));
	...
	EXPORT append_Dids (DATASET(ConsumerCreditReport_Services.Layouts.workRec) ds_work_in,
		...
		ds_didville_out:=do_Didville(ds_didville_in,in_mod.ScoreThreshold);
		Suppress.MAC_Suppress(ds_didville_out,ds_dids,in_mod.applicationtype,Suppress.Constants.LinkTypes.DID,did);

		ConsumerCreditReport_Services.Layouts.workRec appendDids(ds_work_in L, ds_didville_out R) := TRANSFORM
			hasTooManySubjects:=R.cnt>1;
			SELF.did:=IF(NOT hasTooManySubjects,R.did,L.did);
			SELF.score:=IF(NOT hasTooManySubjects,R.score,L.score);
	...
	EXPORT append_UniqueId (DATASET(ConsumerCreditReport_Services.Layouts.ccrResp) ds_ccr_resp,
		...
		DidVille.Layout_Did_OutBatch didvilleReq(ds_ccr_resp L):= TRANSFORM
			SELF.seq := (UNSIGNED)L.AccountNumber,
			Subj := L.ConsumerCreditReports[1].ReportHeader.Subject;
			SELF.title  := '';
			SELF.fname  := Subj.Name.First;
			SELF.mname  := Subj.Name.Middle;
			SELF.lname  := Subj.Name.Last;
			SELF.suffix := Subj.Name.Suffix;
			SELF.ssn := Subj.SSN;
			DOB := iesp.ECL2ESP.DateToString(Subj.DOB);
			SELF.dob := IF((UNSIGNED)DOB>0,DOB,'');
			Addr := L.ConsumerCreditReports[1].Addresses[1];
			SELF.phone10     := Addr.PhoneNumber;
		...
		ConsumerCreditReport_Services.Layouts.ccrResp appendUniqueid(ds_ccr_resp L, ds_didville_out R) := TRANSFORM
			...
			SELF.Score2:=IF(NOT hasTooManySubjects,R.score,0);
		...
		RETURN JOIN(ds_ccr_resp,ds_didville_out(did!=0),
	...
	EXPORT format_Rejects (DATASET(ConsumerCreditReport_Services.Layouts.workRec) ds_work_in) := FUNCTION
		...
		ConsumerCreditReport_Services.Layouts.ccrResp xformRejects(ds_work_in L) := TRANSFORM
			...
			SELF.Score1:=IF(L.did!=0,L.score,0);

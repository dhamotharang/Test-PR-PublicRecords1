...
EXPORT FromNone(DATASET(Layout_In_File) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_acts_Invalid := Fields.InValid_corp_acts((SALT34.StrType)le.corp_acts);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_status_cd_Invalid << 22 ) + ( le.corp_status_desc_Invalid << 23 ) + ( le.corp_inc_state_Invalid << 24 ) + ( le.corp_inc_date_Invalid << 25 ) + ( le.corp_foreign_domestic_ind_Invalid << 27 ) + ( le.corp_forgn_state_desc_Invalid << 28 ) + ( le.corp_forgn_date_Invalid << 29 ) + ( le.corp_orig_org_structure_cd_Invalid << 31 ) + ( le.corp_for_profit_ind_Invalid << 32 ) + ( le.corp_acts_Invalid << 33 ) + ( le.corp_tax_program_cd_Invalid << 34 ) + ( le.corp_agent_country_Invalid << 35 ) + ( le.corp_country_of_formation_Invalid << 36 ) + ( le.recordorigin_Invalid << 37 ) + ( le.InternalField1_Invalid << 38 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_acts_Invalid := (le.ScrubsBits1 >> 33) & 1;
    SELF.corp_tax_program_cd_Invalid := (le.ScrubsBits1 >> 34) & 1;
    SELF.corp_agent_country_Invalid := (le.ScrubsBits1 >> 35) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_acts_ALLOW_ErrorCount := COUNT(GROUP,h.corp_acts_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_acts_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.corp_tax_program_cd_Invalid,'ENUM','UNKNOWN')
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_status_desc:invalid_alphablankBar:ALLOW'
          ...
          ,'corp_acts:invalid_corp_acts:ALLOW'
          ,'corp_tax_program_cd:invalid_tax_program_cd:ENUM'
          ,'corp_agent_country:invalid_country_of_formation:ALLOW'
          ,'corp_country_of_formation:invalid_country_of_formation:ALLOW'
          ,'recordorigin:invalid_recordorigin:ENUM'
          ,'InternalField1:invalid_code:ALLOW','UNKNOWN');
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_corp_acts(1)
          ,Fields.InvalidMessage_corp_tax_program_cd(1)
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_acts_ALLOW_ErrorCount
          ,le.corp_tax_program_cd_ENUM_ErrorCount

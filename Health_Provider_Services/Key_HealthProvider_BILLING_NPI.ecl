...
//BILLING_NPI_NUMBER:+:MAINNAME:SNAME:GENDER:DOB:C_LIC_NBR:LIC_STATE
...
DataForKey0 := DEDUP(SORT(TABLE(h((BILLING_NPI_NUMBER NOT IN SET(s.nulls_BILLING_NPI_NUMBER,BILLING_NPI_NUMBER) AND BILLING_NPI_NUMBER <> (TYPEOF(BILLING_NPI_NUMBER))'')),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,LNPID,BILLING_NPI_NUMBER,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'BILLING_NPI','MAINNAME',CntRed_MAINNAME*100,CntRed_MAINNAME*TSize},{'BILLING_NPI','SNAME',CntRed_SNAME*100,CntRed_SNAME*TSize},{'BILLING_NPI','GENDER',CntRed_GENDER*100,CntRed_GENDER*TSize},{'BILLING_NPI','DOB',CntRed_DOB*100,CntRed_DOB*TSize},{'BILLING_NPI','C_LIC_NBR',CntRed_C_LIC_NBR*100,CntRed_C_LIC_NBR*TSize},{'BILLING_NPI','LIC_STATE',CntRed_LIC_STATE*100,CntRed_LIC_STATE*TSize}],SALT311.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.BILLING_NPI_NUMBER <> (TYPEOF(le.BILLING_NPI_NUMBER))'' AND Fields.InValid_BILLING_NPI_NUMBER((SALT311.StrType)le.BILLING_NPI_NUMBER)=0;
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch(TYPEOF(h.BILLING_NPI_NUMBER) param_BILLING_NPI_NUMBER = (TYPEOF(h.BILLING_NPI_NUMBER))'') := 
    STEPPED( LIMIT( Key(
          KEYED((BILLING_NPI_NUMBER = param_BILLING_NPI_NUMBER))),Config.BILLING_NPI_MAXBLOCKLIMIT,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
 
 
EXPORT ScoredLNPIDFetch(TYPEOF(h.BILLING_NPI_NUMBER) param_BILLING_NPI_NUMBER = (TYPEOF(h.BILLING_NPI_NUMBER))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.MNAME) param_MNAME = (TYPEOF(h.MNAME))'',TYPEOF(h.LNAME) param_LNAME = (TYPEOF(h.LNAME))'',TYPEOF(h.FNAME_len) param_FNAME_len = (TYPEOF(h.FNAME_len))'',TYPEOF(h.MNAME_len) param_MNAME_len = (TYPEOF(h.MNAME_len))'',TYPEOF(h.LNAME_len) param_LNAME_len = (TYPEOF(h.LNAME_len))'',TYPEOF(h.SNAME) param_SNAME = (TYPEOF(h.SNAME))'',TYPEOF(h.GENDER) param_GENDER = (TYPEOF(h.GENDER))'',UNSIGNED4 param_DOB,TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR = (TYPEOF(h.C_LIC_NBR))'',TYPEOF(h.C_LIC_NBR_len) param_C_LIC_NBR_len = (TYPEOF(h.C_LIC_NBR_len))'',TYPEOF(h.LIC_STATE) param_LIC_STATE = (TYPEOF(h.LIC_STATE))'') := FUNCTION
  RawData := RawFetch(param_BILLING_NPI_NUMBER);
 
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    SELF.keys_used := 1 << 19; // Set bitmap for keys used
    SELF.keys_failed := IF(le.LNPID = 0, 1 << 19, 0); // Set bitmap for key failed
    SELF.BILLING_NPI_NUMBER_match_code := match_methods(File_HealthProvider).match_BILLING_NPI_NUMBER(le.BILLING_NPI_NUMBER,param_BILLING_NPI_NUMBER,TRUE);
    SELF.BILLING_NPI_NUMBERWeight := (50+MAP (
           SELF.BILLING_NPI_NUMBER_match_code = SALT311.MatchCode.ExactMatch =>le.BILLING_NPI_NUMBER_weight100,
    ...
    SELF.Weight := IF(le.LNPID = 0, 100, MAX(0, SELF.BILLING_NPI_NUMBERWeight) + MAX(SELF.MAINNAMEWeight, (MAX(0, SELF.FNAMEWeight) + MAX(0, SELF.MNAMEWeight) + MAX(0, SELF.LNAMEWeight))) + MAX(0, SELF.SNAMEWeight) + MAX(0, SELF.GENDERWeight) + MAX(0, SELF.DOBWeight) + MAX(0, SELF.C_LIC_NBRWeight) + MAX(0, SELF.LIC_STATEWeight));
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.BILLING_NPI_NUMBER_match_code := match_methods(File_HealthProvider).match_BILLING_NPI_NUMBER(le.BILLING_NPI_NUMBER,ri.BILLING_NPI_NUMBER,TRUE);
    SELF.BILLING_NPI_NUMBERWeight := (50+MAP (
           SELF.BILLING_NPI_NUMBER_match_code = SALT311.MatchCode.ExactMatch =>le.BILLING_NPI_NUMBER_weight100,
           -0.332*le.BILLING_NPI_NUMBER_weight100))/100*0.50; 
    SELF.FNAME_match_code := MAP(
  ...
  J0 := JOIN(Recs1,Key,((RIGHT.BILLING_NPI_NUMBER = LEFT.BILLING_NPI_NUMBER)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.BILLING_NPI_NUMBER = LEFT.BILLING_NPI_NUMBER)),Config.BILLING_NPI_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((RIGHT.BILLING_NPI_NUMBER = LEFT.BILLING_NPI_NUMBER)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.BILLING_NPI_NUMBER = LEFT.BILLING_NPI_NUMBER)),Config.BILLING_NPI_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_BILLING_NPI_NUMBER='',Input_FNAME='',Input_MNAME='',Input_LNAME='',Input_SNAME='',Input_GENDER='',Input_DOB='',Input_C_LIC_NBR='',Input_LIC_STATE='',output_file,AsIndex='true') := MACRO

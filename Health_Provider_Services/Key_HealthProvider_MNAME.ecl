IMPORT SALT311,std;
EXPORT Key_HealthProvider_MNAME := MODULE
...
EXPORT KeyName := Health_Provider_Services.Files.FILE_MName_SF; //'~'+'key::Health_Provider_Services::LNPID::Refs::MNAME';
...
DataForKey0 := DEDUP(SORT(TABLE(h((LNAME NOT IN SET(s.nulls_LNAME,LNAME) AND LNAME <> (TYPEOF(LNAME))''),(MNAME NOT IN SET(s.nulls_MNAME,MNAME) AND MNAME <> (TYPEOF(MNAME))''),(ST NOT IN SET(s.nulls_ST,ST) AND ST <> (TYPEOF(ST))'')),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,LNPID,LNAME,MNAME,ST,GENDER,SNAME,FNAME,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'MNAME','PRIM_RANGE',CntRed_PRIM_RANGE*100,CntRed_PRIM_RANGE*TSize},{'MNAME','PRIM_NAME',CntRed_PRIM_NAME*100,CntRed_PRIM_NAME*TSize},{'MNAME','SEC_RANGE',CntRed_SEC_RANGE*100,CntRed_SEC_RANGE*TSize},{'MNAME','V_CITY_NAME',CntRed_V_CITY_NAME*100,CntRed_V_CITY_NAME*TSize},{'MNAME','ZIP',CntRed_ZIP*100,CntRed_ZIP*TSize},{'MNAME','DOB',CntRed_DOB*100,CntRed_DOB*TSize},{'MNAME','C_LIC_NBR',CntRed_C_LIC_NBR*100,CntRed_C_LIC_NBR*TSize},{'MNAME','LIC_STATE',CntRed_LIC_STATE*100,CntRed_LIC_STATE*TSize},{'MNAME','NPI_NUMBER',CntRed_NPI_NUMBER*100,CntRed_NPI_NUMBER*TSize},{'MNAME','BILLING_TAX_ID',CntRed_BILLING_TAX_ID*100,CntRed_BILLING_TAX_ID*TSize}],SALT311.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.LNAME <> (TYPEOF(le.LNAME))'' AND Fields.InValid_LNAME((SALT311.StrType)le.LNAME)=0 AND le.MNAME <> (TYPEOF(le.MNAME))'' AND Fields.InValid_MNAME((SALT311.StrType)le.MNAME)=0 AND le.ST <> (TYPEOF(le.ST))'' AND Fields.InValid_ST((SALT311.StrType)le.ST)=0;
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch(TYPEOF(h.LNAME) param_LNAME = (TYPEOF(h.LNAME))'',TYPEOF(h.LNAME_len) param_LNAME_len = (TYPEOF(h.LNAME_len))'',TYPEOF(h.MNAME) param_MNAME = (TYPEOF(h.MNAME))'',TYPEOF(h.MNAME_len) param_MNAME_len = (TYPEOF(h.MNAME_len))'',TYPEOF(h.ST) param_ST = (TYPEOF(h.ST))'',TYPEOF(h.GENDER) param_GENDER = (TYPEOF(h.GENDER))'',TYPEOF(h.SNAME) param_SNAME = (TYPEOF(h.SNAME))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.FNAME_len) param_FNAME_len = (TYPEOF(h.FNAME_len))'') := 
    STEPPED( LIMIT( Key(
          KEYED((LNAME = param_LNAME))
      AND KEYED((MNAME = param_MNAME))
      ...
      AND ((param_FNAME = (TYPEOF(FNAME))'' OR FNAME = (TYPEOF(FNAME))'') OR (FNAME = param_FNAME) OR ((FNAME[1..LENGTH(TRIM(param_FNAME))] = param_FNAME OR param_FNAME[1..LENGTH(TRIM(FNAME))] = FNAME)  OR  (FNAME_PreferredName = fn_PreferredName(param_FNAME))  OR  (metaphonelib.DMetaPhone1(FNAME)=metaphonelib.DMetaPhone1(param_FNAME))  OR (Config.WithinEditN(FNAME,FNAME_len,param_FNAME,param_FNAME_len,2, 5)) ))),Config.MNAME_MAXBLOCKLIMIT,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
 
 
EXPORT ScoredLNPIDFetch(TYPEOF(h.LNAME) param_LNAME = (TYPEOF(h.LNAME))'',TYPEOF(h.LNAME_len) param_LNAME_len = (TYPEOF(h.LNAME_len))'',TYPEOF(h.MNAME) param_MNAME = (TYPEOF(h.MNAME))'',TYPEOF(h.MNAME_len) param_MNAME_len = (TYPEOF(h.MNAME_len))'',TYPEOF(h.ST) param_ST = (TYPEOF(h.ST))'',TYPEOF(h.GENDER) param_GENDER = (TYPEOF(h.GENDER))'',TYPEOF(h.SNAME) param_SNAME = (TYPEOF(h.SNAME))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.FNAME_len) param_FNAME_len = (TYPEOF(h.FNAME_len))'',TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE = (TYPEOF(h.PRIM_RANGE))'',TYPEOF(h.PRIM_NAME) param_PRIM_NAME = (TYPEOF(h.PRIM_NAME))'',TYPEOF(h.SEC_RANGE) param_SEC_RANGE = (TYPEOF(h.SEC_RANGE))'',TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME = (TYPEOF(h.V_CITY_NAME))'',TYPEOF(h.ZIP) param_ZIP = (TYPEOF(h.ZIP))'',UNSIGNED4 param_DOB,TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR = (TYPEOF(h.C_LIC_NBR))'',TYPEOF(h.C_LIC_NBR_len) param_C_LIC_NBR_len = (TYPEOF(h.C_LIC_NBR_len))'',TYPEOF(h.LIC_STATE) param_LIC_STATE = (TYPEOF(h.LIC_STATE))'',TYPEOF(h.NPI_NUMBER) param_NPI_NUMBER = (TYPEOF(h.NPI_NUMBER))'',TYPEOF(h.BILLING_TAX_ID) param_BILLING_TAX_ID = (TYPEOF(h.BILLING_TAX_ID))'') := FUNCTION
  RawData := RawFetch(param_LNAME,param_LNAME_len,param_MNAME,param_MNAME_len,param_ST,param_GENDER,param_SNAME,param_FNAME,param_FNAME_len);
 
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    SELF.keys_used := 1 << 5; // Set bitmap for keys used
    SELF.keys_failed := IF(le.LNPID = 0, 1 << 5, 0); // Set bitmap for key failed
    ...
    SELF.MNAME_match_code := match_methods(File_HealthProvider).match_MNAME(le.MNAME,param_MNAME,le.MNAME_len,param_MNAME_len,TRUE);
    SELF.MNAMEWeight := (50+MAP (
           SELF.MNAME_match_code = SALT311.MatchCode.ExactMatch =>le.MNAME_weight100,
           -0.932*le.MNAME_weight100))/100; 
    SELF.ST_match_code := match_methods(File_HealthProvider).match_ST(le.ST,param_ST,TRUE);
    ...
    SELF.ZIP_match_code := MAP(
           ...
           match_methods(File_HealthProvider).match_ZIP(le.ZIP,param_ZIP,FALSE));
    ...
    SELF.Weight := IF(le.LNPID = 0, 100, MAX(0, SELF.LNAMEWeight) + MAX(0, SELF.MNAMEWeight) + MAX(0, SELF.STWeight) + MAX(0, SELF.GENDERWeight) + MAX(0, SELF.SNAMEWeight) + MAX(0, SELF.FNAMEWeight) + MAX(0, SELF.PRIM_RANGEWeight) + MAX(0, SELF.PRIM_NAMEWeight) + MAX(0, SELF.SEC_RANGEWeight) + MAX(0, SELF.V_CITY_NAMEWeight) + MAX(0, SELF.ZIPWeight) + MAX(0, SELF.DOBWeight) + MAX(0, SELF.C_LIC_NBRWeight) + MAX(0, SELF.LIC_STATEWeight) + MAX(0, SELF.NPI_NUMBERWeight) + MAX(0, SELF.BILLING_TAX_IDWeight));
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.LNAMEWeight := (50+MAP (
           SELF.LNAME_match_code = SALT311.MatchCode.ExactMatch =>le.LNAME_weight100,
           -0.874*le.LNAME_weight100))/100; 
    SELF.MNAME_match_code := match_methods(File_HealthProvider).match_MNAME(le.MNAME,ri.MNAME,le.MNAME_len,ri.MNAME_len,TRUE);
    SELF.MNAMEWeight := (50+MAP (
           SELF.MNAME_match_code = SALT311.MatchCode.ExactMatch =>le.MNAME_weight100,
           -0.932*le.MNAME_weight100))/100; 
    SELF.ST_match_code := match_methods(File_HealthProvider).match_ST(le.ST,ri.ST,TRUE);
    SELF.STWeight := (50+MAP (
           SELF.ST_match_code = SALT311.MatchCode.ExactMatch =>le.ST_weight100,
           -0.823*le.ST_weight100))/100; 
    SELF.GENDER_match_code := MAP(
    ...
    SELF.ZIP_match_code := MAP(
           ...
           match_methods(File_HealthProvider).match_ZIP(le.ZIP,ri.ZIP,FALSE));
    SELF.ZIPWeight := (50+MAP (
           SELF.ZIP_match_code = SALT311.MatchCode.OneSideNull => 0,
           SELF.ZIP_match_code = SALT311.MatchCode.ExactMatch =>le.ZIP_weight100,
           -0.495*le.ZIP_weight100))/100; 
    ...
    SELF := le;
  END;
  Recs0 := Recs(LNAME <> (TYPEOF(LNAME))'',MNAME <> (TYPEOF(MNAME))'',ST <> (TYPEOF(ST))'');
  ...
  J0 := JOIN(Recs1,Key,((RIGHT.LNAME = LEFT.LNAME))
    ...
    ATMOST(((RIGHT.LNAME = LEFT.LNAME))
     AND ((RIGHT.MNAME = LEFT.MNAME))
     AND ((RIGHT.ST = LEFT.ST)),Config.MNAME_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((RIGHT.LNAME = LEFT.LNAME))
    ...
    ATMOST(((RIGHT.LNAME = LEFT.LNAME))
     AND ((RIGHT.MNAME = LEFT.MNAME))
     AND ((RIGHT.ST = LEFT.ST)),Config.MNAME_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_LNAME='',Input_MNAME='',Input_ST='',Input_GENDER='',Input_SNAME='',Input_FNAME='',Input_PRIM_RANGE='',Input_PRIM_NAME='',Input_SEC_RANGE='',Input_V_CITY_NAME='',Input_ZIP='',Input_DOB='',Input_C_LIC_NBR='',Input_LIC_STATE='',Input_NPI_NUMBER='',Input_BILLING_TAX_ID='',output_file,AsIndex='true') := MACRO
  ...
  #IF(#TEXT(Input_LNAME)<>'' AND #TEXT(Input_MNAME)<>'' AND #TEXT(Input_ST)<>'')
    ...
    output_file := Health_Provider_Services.Key_HealthProvider_MNAME.ScoredFetch_Batch(%p%,AsIndex);
  #ELSE

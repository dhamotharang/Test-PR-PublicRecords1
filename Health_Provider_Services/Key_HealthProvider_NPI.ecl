IMPORT SALT311,std;
EXPORT Key_HealthProvider_NPI := MODULE
 
//NPI_NUMBER:+:MAINNAME:SNAME:GENDER:DOB:C_LIC_NBR:LIC_STATE
EXPORT KeyName := Health_Provider_Services.Files.FILE_NPI_SF;  //'~'+'key::Health_Provider_Services::LNPID::Refs::NPI';
...
DataForKey0 := DEDUP(SORT(TABLE(h((NPI_NUMBER NOT IN SET(s.nulls_NPI_NUMBER,NPI_NUMBER) AND NPI_NUMBER <> (TYPEOF(NPI_NUMBER))'')),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,LNPID,NPI_NUMBER,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'NPI','MAINNAME',CntRed_MAINNAME*100,CntRed_MAINNAME*TSize},{'NPI','SNAME',CntRed_SNAME*100,CntRed_SNAME*TSize},{'NPI','GENDER',CntRed_GENDER*100,CntRed_GENDER*TSize},{'NPI','DOB',CntRed_DOB*100,CntRed_DOB*TSize},{'NPI','C_LIC_NBR',CntRed_C_LIC_NBR*100,CntRed_C_LIC_NBR*TSize},{'NPI','LIC_STATE',CntRed_LIC_STATE*100,CntRed_LIC_STATE*TSize}],SALT311.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.NPI_NUMBER <> (TYPEOF(le.NPI_NUMBER))'' AND Fields.InValid_NPI_NUMBER((SALT311.StrType)le.NPI_NUMBER)=0;
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch(TYPEOF(h.NPI_NUMBER) param_NPI_NUMBER = (TYPEOF(h.NPI_NUMBER))'') := 
    STEPPED( LIMIT( Key(
          KEYED((NPI_NUMBER = param_NPI_NUMBER))),Config.NPI_MAXBLOCKLIMIT,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
...
EXPORT ScoredLNPIDFetch(TYPEOF(h.NPI_NUMBER) param_NPI_NUMBER = (TYPEOF(h.NPI_NUMBER))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.MNAME) param_MNAME = (TYPEOF(h.MNAME))'',TYPEOF(h.LNAME) param_LNAME = (TYPEOF(h.LNAME))'',TYPEOF(h.FNAME_len) param_FNAME_len = (TYPEOF(h.FNAME_len))'',TYPEOF(h.MNAME_len) param_MNAME_len = (TYPEOF(h.MNAME_len))'',TYPEOF(h.LNAME_len) param_LNAME_len = (TYPEOF(h.LNAME_len))'',TYPEOF(h.SNAME) param_SNAME = (TYPEOF(h.SNAME))'',TYPEOF(h.GENDER) param_GENDER = (TYPEOF(h.GENDER))'',UNSIGNED4 param_DOB,TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR = (TYPEOF(h.C_LIC_NBR))'',TYPEOF(h.C_LIC_NBR_len) param_C_LIC_NBR_len = (TYPEOF(h.C_LIC_NBR_len))'',TYPEOF(h.LIC_STATE) param_LIC_STATE = (TYPEOF(h.LIC_STATE))'') := FUNCTION
  RawData := RawFetch(param_NPI_NUMBER);
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.NPI_NUMBER_match_code := match_methods(File_HealthProvider).match_NPI_NUMBER(le.NPI_NUMBER,param_NPI_NUMBER,TRUE);
    ...
    SELF.Weight := IF(le.LNPID = 0, 100, MAX(0, SELF.NPI_NUMBERWeight) + MAX(SELF.MAINNAMEWeight, (MAX(0, SELF.FNAMEWeight) + MAX(0, SELF.MNAMEWeight) + MAX(0, SELF.LNAMEWeight))) + MAX(0, SELF.SNAMEWeight) + MAX(0, SELF.GENDERWeight) + MAX(0, SELF.DOBWeight) + MAX(0, SELF.C_LIC_NBRWeight) + MAX(0, SELF.LIC_STATEWeight));
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.NPI_NUMBER_match_code := match_methods(File_HealthProvider).match_NPI_NUMBER(le.NPI_NUMBER,ri.NPI_NUMBER,TRUE);
    SELF.NPI_NUMBERWeight := (50+MAP (
           SELF.NPI_NUMBER_match_code = SALT311.MatchCode.ExactMatch =>le.NPI_NUMBER_weight100,
           -1.000*le.NPI_NUMBER_weight100))/100; 
    SELF.FNAME_match_code := MAP(
  ...
  J0 := JOIN(Recs1,Key,((RIGHT.NPI_NUMBER = LEFT.NPI_NUMBER)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.NPI_NUMBER = LEFT.NPI_NUMBER)),Config.NPI_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((RIGHT.NPI_NUMBER = LEFT.NPI_NUMBER)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.NPI_NUMBER = LEFT.NPI_NUMBER)),Config.NPI_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_NPI_NUMBER='',Input_FNAME='',Input_MNAME='',Input_LNAME='',Input_SNAME='',Input_GENDER='',Input_DOB='',Input_C_LIC_NBR='',Input_LIC_STATE='',output_file,AsIndex='true') := MACRO
  IMPORT SALT311,Health_Provider_Services;
  #IF(#TEXT(Input_NPI_NUMBER)<>'')
    #UNIQUENAME(trans)
    Health_Provider_Services.Key_HealthProvider_NPI.InputLayout_Batch %trans%(InFile le) := TRANSFORM
    ...
    output_file := Health_Provider_Services.Key_HealthProvider_NPI.ScoredFetch_Batch(%p%,AsIndex);
  #ELSE

export mac_xlinking_on_roxie_withindistance (infile,Input_IDL = '',Input_FNAME = '',Input_MNAME = '',Input_LNAME = '',Input_SNAME = '',Input_GENDER = '',
																							...
																							Input_SOURCE_RID = '',OutFile, score = 45, distance = 0) := MACRO
...
%layout_seq% %AssignSeq%(infile l, unsigned4 cnt) := transform
  #IF ( #TEXT(Input_IDL) <> '' )
		self.uniqueId := (typeof(SELF.uniqueId))l.Input_IDL;
  #ELSE
    self.uniqueId := (typeof(SELF.uniqueId))cnt;
  #END
	self := l;
end;
...
#uniquename(result)
%result% := Health_Provider_Services.MEOW_xLNPID(%pr%).Raw_Results;
// output (%result%);
// output (Health_Provider_Services.MEOW_xLNPID(%pr%).Raw_Results);
// Filter results using score and distance
maxScore := if(count(%result%.results) > 1, %result%.results[1].weight, 0);

#uniquename(x_score)
%x_score% := score;
#uniquename(x_distance)
%x_distance% := distance;

#uniquename(xform)
recordof(%result%) %xform%(%result% l) := transform
	maxScore 			:= if(count(l.results) > 1, l.results[1].weight, 0);
	self.results  := l.results(weight >= %x_score% and weight >= (maxscore - %x_distance%));
	self := l;
end;

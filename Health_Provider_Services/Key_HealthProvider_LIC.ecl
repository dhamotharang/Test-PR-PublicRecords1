...
EXPORT KeyName := Health_Provider_Services.Files.FILE_LIC_SF; //'~'+'key::Health_Provider_Services::LNPID::Refs::LIC';
...
DataForKey0 := DEDUP(SORT(TABLE(h((C_LIC_NBR NOT IN SET(s.nulls_C_LIC_NBR,C_LIC_NBR) AND C_LIC_NBR <> (TYPEOF(C_LIC_NBR))''),(LIC_STATE NOT IN SET(s.nulls_LIC_STATE,LIC_STATE) AND LIC_STATE <> (TYPEOF(LIC_STATE))'')),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
EXPORT Shrinkage := DATASET([{'LIC','MAINNAME',CntRed_MAINNAME*100,CntRed_MAINNAME*TSize},{'LIC','SSN',CntRed_SSN*100,CntRed_SSN*TSize},{'LIC','GENDER',CntRed_GENDER*100,CntRed_GENDER*TSize},{'LIC','SNAME',CntRed_SNAME*100,CntRed_SNAME*TSize},{'LIC','DOB',CntRed_DOB*100,CntRed_DOB*TSize}],SALT311.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.C_LIC_NBR <> (TYPEOF(le.C_LIC_NBR))'' AND Fields.InValid_C_LIC_NBR((SALT311.StrType)le.C_LIC_NBR)=0 AND le.LIC_STATE <> (TYPEOF(le.LIC_STATE))'' AND Fields.InValid_LIC_STATE((SALT311.StrType)le.LIC_STATE)=0;
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch(TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR = (TYPEOF(h.C_LIC_NBR))'',TYPEOF(h.C_LIC_NBR_len) param_C_LIC_NBR_len = (TYPEOF(h.C_LIC_NBR_len))'',TYPEOF(h.LIC_STATE) param_LIC_STATE = (TYPEOF(h.LIC_STATE))'') := 
    STEPPED( LIMIT( Key(
          KEYED((C_LIC_NBR = param_C_LIC_NBR))
      AND KEYED((LIC_STATE = param_LIC_STATE))),Config.LIC_MAXBLOCKLIMIT,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
 
 
EXPORT ScoredLNPIDFetch(TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR = (TYPEOF(h.C_LIC_NBR))'',TYPEOF(h.C_LIC_NBR_len) param_C_LIC_NBR_len = (TYPEOF(h.C_LIC_NBR_len))'',TYPEOF(h.LIC_STATE) param_LIC_STATE = (TYPEOF(h.LIC_STATE))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.MNAME) param_MNAME = (TYPEOF(h.MNAME))'',TYPEOF(h.LNAME) param_LNAME = (TYPEOF(h.LNAME))'',TYPEOF(h.FNAME_len) param_FNAME_len = (TYPEOF(h.FNAME_len))'',TYPEOF(h.MNAME_len) param_MNAME_len = (TYPEOF(h.MNAME_len))'',TYPEOF(h.LNAME_len) param_LNAME_len = (TYPEOF(h.LNAME_len))'',TYPEOF(h.SSN) param_SSN = (TYPEOF(h.SSN))'',TYPEOF(h.SSN_len) param_SSN_len = (TYPEOF(h.SSN_len))'',TYPEOF(h.GENDER) param_GENDER = (TYPEOF(h.GENDER))'',TYPEOF(h.SNAME) param_SNAME = (TYPEOF(h.SNAME))'',UNSIGNED4 param_DOB) := FUNCTION
  RawData := RawFetch(param_C_LIC_NBR,param_C_LIC_NBR_len,param_LIC_STATE);
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.LIC_STATE_match_code := match_methods(File_HealthProvider).match_LIC_STATE(le.LIC_STATE,param_LIC_STATE,TRUE);
    ...
    SELF.DOBWeight :=  MAP(
           ...
           DOB_AggWeight); 
    SELF.Weight := IF(le.LNPID = 0, 100, MAX(0, SELF.C_LIC_NBRWeight) + MAX(0, SELF.LIC_STATEWeight) + MAX(SELF.MAINNAMEWeight, (MAX(0, SELF.FNAMEWeight) + MAX(0, SELF.MNAMEWeight) + MAX(0, SELF.LNAMEWeight))) + MAX(0, SELF.SSNWeight) + MAX(0, SELF.GENDERWeight) + MAX(0, SELF.SNAMEWeight) + MAX(0, SELF.DOBWeight));
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.C_LIC_NBRWeight := (50+MAP (
           ...
           SELF.C_LIC_NBR_match_code = SALT311.MatchCode.ExactMatch =>le.C_LIC_NBR_weight100,
           -0.671*le.C_LIC_NBR_weight100))/100; 
    SELF.LIC_STATE_match_code := match_methods(File_HealthProvider).match_LIC_STATE(le.LIC_STATE,ri.LIC_STATE,TRUE);
    SELF.LIC_STATEWeight := (50+MAP (
           SELF.LIC_STATE_match_code = SALT311.MatchCode.ExactMatch =>le.LIC_STATE_weight100,
           -0.810*le.LIC_STATE_weight100))/100; 
    SELF.FNAME_match_code := MAP(
  ...
  J0 := JOIN(Recs1,Key,((RIGHT.C_LIC_NBR = LEFT.C_LIC_NBR))
     AND ((RIGHT.LIC_STATE = LEFT.LIC_STATE)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.C_LIC_NBR = LEFT.C_LIC_NBR))
     AND ((RIGHT.LIC_STATE = LEFT.LIC_STATE)),Config.LIC_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((RIGHT.C_LIC_NBR = LEFT.C_LIC_NBR))
     AND ((RIGHT.LIC_STATE = LEFT.LIC_STATE)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.C_LIC_NBR = LEFT.C_LIC_NBR))
     AND ((RIGHT.LIC_STATE = LEFT.LIC_STATE)),Config.LIC_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_C_LIC_NBR='',Input_LIC_STATE='',Input_FNAME='',Input_MNAME='',Input_LNAME='',Input_SSN='',Input_GENDER='',Input_SNAME='',Input_DOB='',output_file,AsIndex='true') := MACRO
  ...
  #IF(#TEXT(Input_C_LIC_NBR)<>'' AND #TEXT(Input_LIC_STATE)<>'')
    #UNIQUENAME(trans)
    Health_Provider_Services.Key_HealthProvider_LIC.InputLayout_Batch %trans%(InFile le) := TRANSFORM
    ...
    output_file := Health_Provider_Services.Key_HealthProvider_LIC.ScoredFetch_Batch(%p%,AsIndex);
  #ELSE

IMPORT SALT311,std;
EXPORT Key_HealthProvider_NAMEL := MODULE
...
EXPORT KeyName := Health_Provider_Services.Files.FILE_Name_ST_Lic_SF; //'~'+'key::Health_Provider_Services::LNPID::Refs::NAMEL';
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,LNPID,LNAME,ST,C_LIC_NBR,FNAME,GENDER,SNAME,MNAME,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'NAMEL','PRIM_RANGE',CntRed_PRIM_RANGE*100,CntRed_PRIM_RANGE*TSize},{'NAMEL','PRIM_NAME',CntRed_PRIM_NAME*100,CntRed_PRIM_NAME*TSize},{'NAMEL','SEC_RANGE',CntRed_SEC_RANGE*100,CntRed_SEC_RANGE*TSize},{'NAMEL','V_CITY_NAME',CntRed_V_CITY_NAME*100,CntRed_V_CITY_NAME*TSize},{'NAMEL','ZIP',CntRed_ZIP*100,CntRed_ZIP*TSize},{'NAMEL','DOB',CntRed_DOB*100,CntRed_DOB*TSize},{'NAMEL','LIC_STATE',CntRed_LIC_STATE*100,CntRed_LIC_STATE*TSize},{'NAMEL','NPI_NUMBER',CntRed_NPI_NUMBER*100,CntRed_NPI_NUMBER*TSize},{'NAMEL','BILLING_TAX_ID',CntRed_BILLING_TAX_ID*100,CntRed_BILLING_TAX_ID*TSize}],SALT311.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.LNAME <> (TYPEOF(le.LNAME))'' AND Fields.InValid_LNAME((SALT311.StrType)le.LNAME)=0 AND le.ST <> (TYPEOF(le.ST))'' AND Fields.InValid_ST((SALT311.StrType)le.ST)=0 AND le.C_LIC_NBR <> (TYPEOF(le.C_LIC_NBR))'' AND Fields.InValid_C_LIC_NBR((SALT311.StrType)le.C_LIC_NBR)=0;
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch(TYPEOF(h.LNAME) param_LNAME = (TYPEOF(h.LNAME))'',TYPEOF(h.LNAME_len) param_LNAME_len = (TYPEOF(h.LNAME_len))'',TYPEOF(h.ST) param_ST = (TYPEOF(h.ST))'',TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR = (TYPEOF(h.C_LIC_NBR))'',TYPEOF(h.C_LIC_NBR_len) param_C_LIC_NBR_len = (TYPEOF(h.C_LIC_NBR_len))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.FNAME_len) param_FNAME_len = (TYPEOF(h.FNAME_len))'',TYPEOF(h.GENDER) param_GENDER = (TYPEOF(h.GENDER))'',TYPEOF(h.SNAME) param_SNAME = (TYPEOF(h.SNAME))'',TYPEOF(h.MNAME) param_MNAME = (TYPEOF(h.MNAME))'',TYPEOF(h.MNAME_len) param_MNAME_len = (TYPEOF(h.MNAME_len))'') := 
    STEPPED( LIMIT( Key(
      ...
      AND ((param_MNAME = (TYPEOF(MNAME))'' OR MNAME = (TYPEOF(MNAME))'') OR (MNAME = param_MNAME) OR ((MNAME[1..LENGTH(TRIM(param_MNAME))] = param_MNAME OR param_MNAME[1..LENGTH(TRIM(MNAME))] = MNAME)  OR  (metaphonelib.DMetaPhone1(MNAME)=metaphonelib.DMetaPhone1(param_MNAME))  OR (Config.WithinEditN(MNAME,MNAME_len,param_MNAME,param_MNAME_len,2, 5)) ))),Config.NAMEL_MAXBLOCKLIMIT,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
 
 
EXPORT ScoredLNPIDFetch(TYPEOF(h.LNAME) param_LNAME = (TYPEOF(h.LNAME))'',TYPEOF(h.LNAME_len) param_LNAME_len = (TYPEOF(h.LNAME_len))'',TYPEOF(h.ST) param_ST = (TYPEOF(h.ST))'',TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR = (TYPEOF(h.C_LIC_NBR))'',TYPEOF(h.C_LIC_NBR_len) param_C_LIC_NBR_len = (TYPEOF(h.C_LIC_NBR_len))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.FNAME_len) param_FNAME_len = (TYPEOF(h.FNAME_len))'',TYPEOF(h.GENDER) param_GENDER = (TYPEOF(h.GENDER))'',TYPEOF(h.SNAME) param_SNAME = (TYPEOF(h.SNAME))'',TYPEOF(h.MNAME) param_MNAME = (TYPEOF(h.MNAME))'',TYPEOF(h.MNAME_len) param_MNAME_len = (TYPEOF(h.MNAME_len))'',TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE = (TYPEOF(h.PRIM_RANGE))'',TYPEOF(h.PRIM_NAME) param_PRIM_NAME = (TYPEOF(h.PRIM_NAME))'',TYPEOF(h.SEC_RANGE) param_SEC_RANGE = (TYPEOF(h.SEC_RANGE))'',TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME = (TYPEOF(h.V_CITY_NAME))'',TYPEOF(h.ZIP) param_ZIP = (TYPEOF(h.ZIP))'',UNSIGNED4 param_DOB,TYPEOF(h.LIC_STATE) param_LIC_STATE = (TYPEOF(h.LIC_STATE))'',TYPEOF(h.NPI_NUMBER) param_NPI_NUMBER = (TYPEOF(h.NPI_NUMBER))'',TYPEOF(h.BILLING_TAX_ID) param_BILLING_TAX_ID = (TYPEOF(h.BILLING_TAX_ID))'') := FUNCTION
  RawData := RawFetch(param_LNAME,param_LNAME_len,param_ST,param_C_LIC_NBR,param_C_LIC_NBR_len,param_FNAME,param_FNAME_len,param_GENDER,param_SNAME,param_MNAME,param_MNAME_len);
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.Weight := IF(le.LNPID = 0, 100, MAX(0, SELF.LNAMEWeight) + MAX(0, SELF.STWeight) + MAX(0, SELF.C_LIC_NBRWeight) + MAX(0, SELF.FNAMEWeight) + MAX(0, SELF.GENDERWeight) + MAX(0, SELF.SNAMEWeight) + MAX(0, SELF.MNAMEWeight) + MAX(0, SELF.PRIM_RANGEWeight) + MAX(0, SELF.PRIM_NAMEWeight) + MAX(0, SELF.SEC_RANGEWeight) + MAX(0, SELF.V_CITY_NAMEWeight) + MAX(0, SELF.ZIPWeight) + MAX(0, SELF.DOBWeight) + MAX(0, SELF.LIC_STATEWeight) + MAX(0, SELF.NPI_NUMBERWeight) + MAX(0, SELF.BILLING_TAX_IDWeight));
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF := le;
  END;
  Recs0 := Recs(LNAME <> (TYPEOF(LNAME))'',ST <> (TYPEOF(ST))'',C_LIC_NBR <> (TYPEOF(C_LIC_NBR))'');
  ...
  J0 := JOIN(Recs1,Key,((RIGHT.LNAME = LEFT.LNAME))
    ...
    ATMOST(((RIGHT.LNAME = LEFT.LNAME))
     AND ((RIGHT.ST = LEFT.ST))
     AND ((RIGHT.C_LIC_NBR = LEFT.C_LIC_NBR)),Config.NAMEL_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((RIGHT.LNAME = LEFT.LNAME))
    ...
    ATMOST(((RIGHT.LNAME = LEFT.LNAME))
     AND ((RIGHT.ST = LEFT.ST))
     AND ((RIGHT.C_LIC_NBR = LEFT.C_LIC_NBR)),Config.NAMEL_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_LNAME='',Input_ST='',Input_C_LIC_NBR='',Input_FNAME='',Input_GENDER='',Input_SNAME='',Input_MNAME='',Input_PRIM_RANGE='',Input_PRIM_NAME='',Input_SEC_RANGE='',Input_V_CITY_NAME='',Input_ZIP='',Input_DOB='',Input_LIC_STATE='',Input_NPI_NUMBER='',Input_BILLING_TAX_ID='',output_file,AsIndex='true') := MACRO
  ...
  #IF(#TEXT(Input_LNAME)<>'' AND #TEXT(Input_ST)<>'' AND #TEXT(Input_C_LIC_NBR)<>'')
    ...
    output_file := Health_Provider_Services.Key_HealthProvider_NAMEL.ScoredFetch_Batch(%p%,AsIndex);
  #ELSE

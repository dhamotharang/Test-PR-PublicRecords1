...
DataForKey0 := DEDUP(SORT(TABLE(h((DEA_NUMBER NOT IN SET(s.nulls_DEA_NUMBER,DEA_NUMBER) AND DEA_NUMBER <> (TYPEOF(DEA_NUMBER))'')),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,LNPID,DEA_NUMBER,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'DEA','MAINNAME',CntRed_MAINNAME*100,CntRed_MAINNAME*TSize},{'DEA','SNAME',CntRed_SNAME*100,CntRed_SNAME*TSize},{'DEA','GENDER',CntRed_GENDER*100,CntRed_GENDER*TSize},{'DEA','DOB',CntRed_DOB*100,CntRed_DOB*TSize},{'DEA','C_LIC_NBR',CntRed_C_LIC_NBR*100,CntRed_C_LIC_NBR*TSize},{'DEA','LIC_STATE',CntRed_LIC_STATE*100,CntRed_LIC_STATE*TSize}],SALT311.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.DEA_NUMBER <> (TYPEOF(le.DEA_NUMBER))'' AND Fields.InValid_DEA_NUMBER((SALT311.StrType)le.DEA_NUMBER)=0;
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch(TYPEOF(h.DEA_NUMBER) param_DEA_NUMBER = (TYPEOF(h.DEA_NUMBER))'') := 
    STEPPED( LIMIT( Key(
          KEYED((DEA_NUMBER = param_DEA_NUMBER))),Config.DEA_MAXBLOCKLIMIT,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
 
 
EXPORT ScoredLNPIDFetch(TYPEOF(h.DEA_NUMBER) param_DEA_NUMBER = (TYPEOF(h.DEA_NUMBER))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.MNAME) param_MNAME = (TYPEOF(h.MNAME))'',TYPEOF(h.LNAME) param_LNAME = (TYPEOF(h.LNAME))'',TYPEOF(h.FNAME_len) param_FNAME_len = (TYPEOF(h.FNAME_len))'',TYPEOF(h.MNAME_len) param_MNAME_len = (TYPEOF(h.MNAME_len))'',TYPEOF(h.LNAME_len) param_LNAME_len = (TYPEOF(h.LNAME_len))'',TYPEOF(h.SNAME) param_SNAME = (TYPEOF(h.SNAME))'',TYPEOF(h.GENDER) param_GENDER = (TYPEOF(h.GENDER))'',UNSIGNED4 param_DOB,TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR = (TYPEOF(h.C_LIC_NBR))'',TYPEOF(h.C_LIC_NBR_len) param_C_LIC_NBR_len = (TYPEOF(h.C_LIC_NBR_len))'',TYPEOF(h.LIC_STATE) param_LIC_STATE = (TYPEOF(h.LIC_STATE))'') := FUNCTION
  RawData := RawFetch(param_DEA_NUMBER);
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.DEA_NUMBER_match_code := match_methods(File_HealthProvider).match_DEA_NUMBER(le.DEA_NUMBER,param_DEA_NUMBER,TRUE);
    SELF.DEA_NUMBERWeight := (50+MAP (
           SELF.DEA_NUMBER_match_code = SALT311.MatchCode.ExactMatch =>le.DEA_NUMBER_weight100,
    ...
    SELF.Weight := IF(le.LNPID = 0, 100, MAX(0, SELF.DEA_NUMBERWeight) + MAX(SELF.MAINNAMEWeight, (MAX(0, SELF.FNAMEWeight) + MAX(0, SELF.MNAMEWeight) + MAX(0, SELF.LNAMEWeight))) + MAX(0, SELF.SNAMEWeight) + MAX(0, SELF.GENDERWeight) + MAX(0, SELF.DOBWeight) + MAX(0, SELF.C_LIC_NBRWeight) + MAX(0, SELF.LIC_STATEWeight));
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.DEA_NUMBER_match_code := match_methods(File_HealthProvider).match_DEA_NUMBER(le.DEA_NUMBER,ri.DEA_NUMBER,TRUE);
    SELF.DEA_NUMBERWeight := (50+MAP (
           SELF.DEA_NUMBER_match_code = SALT311.MatchCode.ExactMatch =>le.DEA_NUMBER_weight100,
           -0.853*le.DEA_NUMBER_weight100))/100; 
    SELF.FNAME_match_code := MAP(
  ...
  J0 := JOIN(Recs1,Key,((RIGHT.DEA_NUMBER = LEFT.DEA_NUMBER)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.DEA_NUMBER = LEFT.DEA_NUMBER)),Config.DEA_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((RIGHT.DEA_NUMBER = LEFT.DEA_NUMBER)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.DEA_NUMBER = LEFT.DEA_NUMBER)),Config.DEA_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);

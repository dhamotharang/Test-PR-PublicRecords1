...
EXPORT KeyName := Health_Provider_Services.Files.FILE_Address_SF; //'~'+'key::Health_Provider_Services::LNPID::Refs::ADDRESS';
...
EXPORT Shrinkage := DATASET([{'ADDRESS','V_CITY_NAME',CntRed_V_CITY_NAME*100,CntRed_V_CITY_NAME*TSize},{'ADDRESS','ST',CntRed_ST*100,CntRed_ST*TSize},{'ADDRESS','GENDER',CntRed_GENDER*100,CntRed_GENDER*TSize},{'ADDRESS','LNAME',CntRed_LNAME*100,CntRed_LNAME*TSize},{'ADDRESS','MNAME',CntRed_MNAME*100,CntRed_MNAME*TSize},{'ADDRESS','SNAME',CntRed_SNAME*100,CntRed_SNAME*TSize},{'ADDRESS','DOB',CntRed_DOB*100,CntRed_DOB*TSize},{'ADDRESS','C_LIC_NBR',CntRed_C_LIC_NBR*100,CntRed_C_LIC_NBR*TSize},{'ADDRESS','LIC_STATE',CntRed_LIC_STATE*100,CntRed_LIC_STATE*TSize},{'ADDRESS','BILLING_TAX_ID',CntRed_BILLING_TAX_ID*100,CntRed_BILLING_TAX_ID*TSize}],SALT311.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.PRIM_RANGE <> (TYPEOF(le.PRIM_RANGE))'' AND Fields.InValid_PRIM_RANGE((SALT311.StrType)le.PRIM_RANGE)=0 AND le.PRIM_NAME <> (TYPEOF(le.PRIM_NAME))'' AND Fields.InValid_PRIM_NAME((SALT311.StrType)le.PRIM_NAME)=0 AND le.ZIP <> (TYPEOF(le.ZIP))'' AND Fields.InValid_ZIP((SALT311.StrType)le.ZIP)=0;
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch(TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE = (TYPEOF(h.PRIM_RANGE))'',TYPEOF(h.PRIM_NAME) param_PRIM_NAME = (TYPEOF(h.PRIM_NAME))'',TYPEOF(h.ZIP) param_ZIP = (TYPEOF(h.ZIP))'',TYPEOF(h.SEC_RANGE) param_SEC_RANGE = (TYPEOF(h.SEC_RANGE))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.FNAME_len) param_FNAME_len = (TYPEOF(h.FNAME_len))'') := 
    STEPPED( LIMIT( Key(
      ...
      AND ((param_FNAME = (TYPEOF(FNAME))'' OR FNAME = (TYPEOF(FNAME))'') OR (FNAME = param_FNAME) OR ((FNAME[1..LENGTH(TRIM(param_FNAME))] = param_FNAME OR param_FNAME[1..LENGTH(TRIM(FNAME))] = FNAME)  OR  (FNAME_PreferredName = fn_PreferredName(param_FNAME))  OR  (metaphonelib.DMetaPhone1(FNAME)=metaphonelib.DMetaPhone1(param_FNAME))  OR (Config.WithinEditN(FNAME,FNAME_len,param_FNAME,param_FNAME_len,2, 5)) ))),Config.ADDRESS_MAXBLOCKLIMIT,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
 
 
EXPORT ScoredLNPIDFetch(TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE = (TYPEOF(h.PRIM_RANGE))'',TYPEOF(h.PRIM_NAME) param_PRIM_NAME = (TYPEOF(h.PRIM_NAME))'',TYPEOF(h.ZIP) param_ZIP = (TYPEOF(h.ZIP))'',TYPEOF(h.SEC_RANGE) param_SEC_RANGE = (TYPEOF(h.SEC_RANGE))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.FNAME_len) param_FNAME_len = (TYPEOF(h.FNAME_len))'',TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME = (TYPEOF(h.V_CITY_NAME))'',TYPEOF(h.ST) param_ST = (TYPEOF(h.ST))'',TYPEOF(h.GENDER) param_GENDER = (TYPEOF(h.GENDER))'',TYPEOF(h.LNAME) param_LNAME = (TYPEOF(h.LNAME))'',TYPEOF(h.LNAME_len) param_LNAME_len = (TYPEOF(h.LNAME_len))'',TYPEOF(h.MNAME) param_MNAME = (TYPEOF(h.MNAME))'',TYPEOF(h.MNAME_len) param_MNAME_len = (TYPEOF(h.MNAME_len))'',TYPEOF(h.SNAME) param_SNAME = (TYPEOF(h.SNAME))'',UNSIGNED4 param_DOB,TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR = (TYPEOF(h.C_LIC_NBR))'',TYPEOF(h.C_LIC_NBR_len) param_C_LIC_NBR_len = (TYPEOF(h.C_LIC_NBR_len))'',TYPEOF(h.LIC_STATE) param_LIC_STATE = (TYPEOF(h.LIC_STATE))'',TYPEOF(h.BILLING_TAX_ID) param_BILLING_TAX_ID = (TYPEOF(h.BILLING_TAX_ID))'') := FUNCTION
  RawData := RawFetch(param_PRIM_RANGE,param_PRIM_NAME,param_ZIP,param_SEC_RANGE,param_FNAME,param_FNAME_len);
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.PRIM_RANGE_match_code := match_methods(File_HealthProvider).match_PRIM_RANGE(le.PRIM_RANGE,param_PRIM_RANGE,TRUE);
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.PRIM_RANGE_match_code := match_methods(File_HealthProvider).match_PRIM_RANGE(le.PRIM_RANGE,ri.PRIM_RANGE,TRUE);
    SELF.PRIM_RANGEWeight := (50+MAP (
           SELF.PRIM_RANGE_match_code = SALT311.MatchCode.ExactMatch =>le.PRIM_RANGE_weight100,
           -0.447*le.PRIM_RANGE_weight100))/100; 
  ...
  J0 := JOIN(Recs1,Key,((RIGHT.PRIM_RANGE = LEFT.PRIM_RANGE))
    ...
    ATMOST(((RIGHT.PRIM_RANGE = LEFT.PRIM_RANGE))
     AND ((RIGHT.PRIM_NAME = LEFT.PRIM_NAME))
     AND ((RIGHT.ZIP = LEFT.ZIP)),Config.ADDRESS_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((RIGHT.PRIM_RANGE = LEFT.PRIM_RANGE))
     AND ((RIGHT.PRIM_NAME = LEFT.PRIM_NAME))
     AND ((RIGHT.ZIP = LEFT.ZIP))
     AND ((LEFT.SEC_RANGE = (TYPEOF(RIGHT.SEC_RANGE))'' OR RIGHT.SEC_RANGE = (TYPEOF(RIGHT.SEC_RANGE))'') OR (RIGHT.SEC_RANGE = LEFT.SEC_RANGE) OR ((SALT311.HyphenMatch(RIGHT.SEC_RANGE,LEFT.SEC_RANGE,1)<=2)))
     AND ((LEFT.FNAME = (TYPEOF(RIGHT.FNAME))'' OR RIGHT.FNAME = (TYPEOF(RIGHT.FNAME))'') OR (RIGHT.FNAME = LEFT.FNAME) OR ((RIGHT.FNAME[1..LENGTH(TRIM(LEFT.FNAME))] = LEFT.FNAME OR LEFT.FNAME[1..LENGTH(TRIM(RIGHT.FNAME))] = RIGHT.FNAME)  OR  (RIGHT.FNAME_PreferredName = fn_PreferredName(LEFT.FNAME))  OR  (metaphonelib.DMetaPhone1(RIGHT.FNAME)=metaphonelib.DMetaPhone1(LEFT.FNAME))  OR (Config.WithinEditN(RIGHT.FNAME,RIGHT.FNAME_len,LEFT.FNAME,LEFT.FNAME_len,2, 5)) )),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.PRIM_RANGE = LEFT.PRIM_RANGE))
     AND ((RIGHT.PRIM_NAME = LEFT.PRIM_NAME))
     AND ((RIGHT.ZIP = LEFT.ZIP)),Config.ADDRESS_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_PRIM_RANGE='',Input_PRIM_NAME='',Input_ZIP='',Input_SEC_RANGE='',Input_FNAME='',Input_V_CITY_NAME='',Input_ST='',Input_GENDER='',Input_LNAME='',Input_MNAME='',Input_SNAME='',Input_DOB='',Input_C_LIC_NBR='',Input_LIC_STATE='',Input_BILLING_TAX_ID='',output_file,AsIndex='true') := MACRO
  ...
  #IF(#TEXT(Input_PRIM_RANGE)<>'' AND #TEXT(Input_PRIM_NAME)<>'' AND #TEXT(Input_ZIP)<>'')
    #UNIQUENAME(trans)
    Health_Provider_Services.Key_HealthProvider_ADDRESS.InputLayout_Batch %trans%(InFile le) := TRANSFORM
    ...
    output_file := Health_Provider_Services.Key_HealthProvider_ADDRESS.ScoredFetch_Batch(%p%,AsIndex);
  #ELSE

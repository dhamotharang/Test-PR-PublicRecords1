...
#uniquename(getLNPID)
%layout_ref% %getLNPID% (Health_Provider_Services.Process_xLNPID_Layouts.OutputLayout_Batch l, INTEGER C) := transform,skip (l.Results[c].weight < 25)
...
// output (%lnpid_file%);
%best_lnpid% := dedup(sort(%lnpid_file%,uniqueID,-best_criteria, -weight,local),uniqueID,local);

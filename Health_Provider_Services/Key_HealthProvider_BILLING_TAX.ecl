...
DataForKey0 := DEDUP(SORT(TABLE(h((BILLING_TAX_ID NOT IN SET(s.nulls_BILLING_TAX_ID,BILLING_TAX_ID) AND BILLING_TAX_ID <> (TYPEOF(BILLING_TAX_ID))'')),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,LNPID,BILLING_TAX_ID,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT ScoredLNPIDFetch(TYPEOF(h.BILLING_TAX_ID) param_BILLING_TAX_ID = (TYPEOF(h.BILLING_TAX_ID))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.MNAME) param_MNAME = (TYPEOF(h.MNAME))'',TYPEOF(h.LNAME) param_LNAME = (TYPEOF(h.LNAME))'',TYPEOF(h.FNAME_len) param_FNAME_len = (TYPEOF(h.FNAME_len))'',TYPEOF(h.MNAME_len) param_MNAME_len = (TYPEOF(h.MNAME_len))'',TYPEOF(h.LNAME_len) param_LNAME_len = (TYPEOF(h.LNAME_len))'',TYPEOF(h.SNAME) param_SNAME = (TYPEOF(h.SNAME))'',TYPEOF(h.GENDER) param_GENDER = (TYPEOF(h.GENDER))'',UNSIGNED4 param_DOB,TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR = (TYPEOF(h.C_LIC_NBR))'',TYPEOF(h.C_LIC_NBR_len) param_C_LIC_NBR_len = (TYPEOF(h.C_LIC_NBR_len))'',TYPEOF(h.LIC_STATE) param_LIC_STATE = (TYPEOF(h.LIC_STATE))'') := FUNCTION
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.BILLING_TAX_ID_match_code := match_methods(File_HealthProvider).match_BILLING_TAX_ID(le.BILLING_TAX_ID,param_BILLING_TAX_ID,TRUE);
    ...
    SELF.Weight := IF(le.LNPID = 0, 100, MAX(0, SELF.BILLING_TAX_IDWeight) + MAX(SELF.MAINNAMEWeight, (MAX(0, SELF.FNAMEWeight) + MAX(0, SELF.MNAMEWeight) + MAX(0, SELF.LNAMEWeight))) + MAX(0, SELF.SNAMEWeight) + MAX(0, SELF.GENDERWeight) + MAX(0, SELF.DOBWeight) + MAX(0, SELF.C_LIC_NBRWeight) + MAX(0, SELF.LIC_STATEWeight));
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.BILLING_TAX_ID_match_code := match_methods(File_HealthProvider).match_BILLING_TAX_ID(le.BILLING_TAX_ID,ri.BILLING_TAX_ID,TRUE);
    SELF.BILLING_TAX_IDWeight := (50+MAP (
           SELF.BILLING_TAX_ID_match_code = SALT311.MatchCode.ExactMatch =>le.BILLING_TAX_ID_weight100,
           -0.392*le.BILLING_TAX_ID_weight100))/100*0.50; 
    SELF.FNAME_match_code := MAP(
  ...
  J0 := JOIN(Recs1,Key,((RIGHT.BILLING_TAX_ID = LEFT.BILLING_TAX_ID)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.BILLING_TAX_ID = LEFT.BILLING_TAX_ID)),Config.BILLING_TAX_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((RIGHT.BILLING_TAX_ID = LEFT.BILLING_TAX_ID)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.BILLING_TAX_ID = LEFT.BILLING_TAX_ID)),Config.BILLING_TAX_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);

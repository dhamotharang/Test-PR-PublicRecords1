...
EXPORT Summary(SALT36.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_event_book_nbr := MAX(GROUP,LENGTH(TRIM((SALT36.StrType)h.event_book_nbr)));
    avelength_event_book_nbr := AVE(GROUP,LENGTH(TRIM((SALT36.StrType)h.event_book_nbr)),h.event_book_nbr<>(typeof(h.event_book_nbr))'');
    populated_event_page_nbr_pcnt := AVE(GROUP,IF(h.event_page_nbr = (TYPEOF(h.event_page_nbr))'',0,100));
    maxlength_event_page_nbr := MAX(GROUP,LENGTH(TRIM((SALT36.StrType)h.event_page_nbr)));
    avelength_event_page_nbr := AVE(GROUP,LENGTH(TRIM((SALT36.StrType)h.event_page_nbr)),h.event_page_nbr<>(typeof(h.event_page_nbr))'');
    populated_event_certification_nbr_pcnt := AVE(GROUP,IF(h.event_certification_nbr = (TYPEOF(h.event_certification_nbr))'',0,100));
    maxlength_event_certification_nbr := MAX(GROUP,LENGTH(TRIM((SALT36.StrType)h.event_certification_nbr)));
    avelength_event_certification_nbr := AVE(GROUP,LENGTH(TRIM((SALT36.StrType)h.event_certification_nbr)),h.event_certification_nbr<>(typeof(h.event_certification_nbr))'');
  END;
...
SHARED FldIds := DATASET([{1,'corp_key'}
      ...
      ,{27,'event_book_nbr'}
      ,{28,'event_page_nbr'}
      ,{29,'event_certification_nbr'}],SALT36.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_event_book_nbr((SALT36.StrType)le.event_book_nbr),
    Fields.InValid_event_page_nbr((SALT36.StrType)le.event_page_nbr),
    Fields.InValid_event_certification_nbr((SALT36.StrType)le.event_certification_nbr),
    0);
...
PrettyErrorTotals := RECORD
  ...
  FieldType := CHOOSE(TotalErrors.FieldNum,'invalid_corp_key','Unknown','invalid_corp_vendor','Unknown','Unknown','invalid_state_origin','invalid_date','invalid_charter','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','invalid_filing_desc','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown');

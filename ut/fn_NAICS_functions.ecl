...
//null is valid	
  string trim_code := TRIM(code, LEFT, RIGHT);
	...
	string temp_code :=
					...
					trim_code);
	setNAICS := if(temp_code != '' AND length_of_code >= 3 AND length_of_code <= 6 AND filtered_code = '',set(pNAICSLookup, naics_code),[]);
...
EXPORT dataset fn_validate_NAICSCode_Dataset(dataset(NAICSCodes.Layouts.naicsLookup) naicsDS) := function
	...
	NAICSCodes.Layouts.naicsLookup codeXrf(NAICSCodes.Layouts.naicsLookup	L) := transform
	  ...
	  setNAICS := set(pNAICSLookup, naics_code);
		SELF.naics_description := if(temp_code in setNAICS AND (length_of_code >= 3 AND length_of_code <= 6), L.naics_description, '');

export MAC_SF_BuildProcess(thedataset, basename, seq_name, numgenerations = '3', csvout = 'false') := macro
/*
	...
	if(%ng% not in [2,3], fail('ut.MAC_SF_BuildProcess failure, numgenerations not in [2,3]'),
  	 if(%workalreadydone%(FileServices.GetSuperFileSubName(basename,1)),
		...
		sequential(
			...
			#else
				output(thedataset,,basename+thorlib.wuid(),overwrite,csv(quote('"'))),
			#end

export MAC_Append_Id_Local(infile,link_Field,append_field,patchfile,left_field,right_field,outfile) := macro
...
#uniquename(dinfile)
%dinfile% := distribute(infile,hash(link_field));

//****** Distribute patchfile by left_field to prepare for local join.
#uniquename(dpatchfile)
%dpatchfile% := distribute(patchfile,hash(left_field));
...
//outfile := if ( count(patchfile) < 30000000, %j2%, %j1% );
outfile := %j1%;

...
export proc_Orbit4_CreateBuild_AddItem_npf_sp(string buildname,string Buildvs, string email_list = '',boolean skipcreatebuild = false,boolean skipupdatebuild = false,   boolean skipaddcomponents = false, boolean runcreatebuild = true, boolean runaddcomponentsonly = false,string wuid) := function
...
get_build_candidates := 	Orbit4.GetBuildCandidates(buildname,
		 ...
		 get_new_build_candidates := project(get_build_candidates,transform( Orbit4.Layouts.OrbitBuildInstancenewLayout , self := left));

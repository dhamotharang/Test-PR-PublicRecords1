...
EXPORT raw_NonMember_MAC_PopulationStatistics(infile,Ref='',Input_bbb_id = '',Input_company_name = '',Input_address = '',Input_country = '',Input_phone = '',Input_phone_type = '',Input_listing_month = '',Input_listing_day = '',Input_listing_year = '',Input_http_link = '',Input_non_member_title = '',Input_non_member_category = '',OutFile) := MACRO
...
+    #IF( #TEXT(Input_non_member_title)='' )
      '' 
    #ELSE
        IF( le.Input_non_member_title = (TYPEOF(le.Input_non_member_title))'','',':non_member_title')
    #END
 
+    #IF( #TEXT(Input_non_member_category)='' )
      '' 
    #ELSE
        IF( le.Input_non_member_category = (TYPEOF(le.Input_non_member_category))'','',':non_member_category')
    #END
;
  END;
  #uniquename(op)

...
#workunit('name', 'NonFCRA Inquiry Cnts');

BuildVersion := ADV.InquiryNWeekly.BuildVersion;
...
nonfcra_inquiries_all := project(Key((unsigned)search_info.datetime[1..4]>=2008), transform_raw(left));

deduped_inquiries := dedup(sort(nonfcra_inquiries_all, person_q.Appended_ADL, transaction_id, sequence_number, logdate), 
...
Despray              := std.file.despray(DesprayLogicalFileName, '10.48.72.34', 'C:\\adv\\public_records\\InquiryNonFCRAWeekly\\results\\NonFCRAInquiryCounts.csv',,,,true);

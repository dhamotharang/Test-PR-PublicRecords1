...
File := dataset('~thor_data400::persist::ecrash_ssv2', Layout, thor);

BuildVersion := sort(ADV.CurrentBuildVersions.File(datasetname='EcrashV2Keys' and location = 'B' and cluster = 'N'), -buildversion)[1].buildversion;

agencyData := File(report_code = 'EA');
cruArchive := File(report_code not in ['EA','TM','TF', 'A']);
iyetekData := File(report_code = 'TM' or report_code = 'TF' );
ntlData    := File( report_code = 'A');
...
agencyDataSampleSize := (agencyDataCount/totalCount) * SampleSize;
cruArchiveSampleSize := (cruArchiveCount/totalCount) * SampleSize;
iyetekDataSampleSize := (iyetekDataCount/totalCount) * SampleSize;
ntlDataSampleSize    := (ntlDataCount/totalCount) * SampleSize;

agencyDataSample := topn(agencyData, map(agencyDataCount<minSampleSize=>agencyDataCount, agencyDataSampleSize),-accident_date);														 
cruArchiveSample := topn(cruArchive, map(cruArchiveCount<minSampleSize=>cruArchiveCount, cruArchiveSampleSize),-accident_date);														 
iyetekDataSample := topn(iyetekData, map(iyetekDataCount<minSampleSize=>iyetekDataCount, iyetekDataSampleSize),-accident_date);														 
ntlDataSample    := topn(ntlData   , map(ntlDataCount<minSampleSize=>ntlDataCount, ntlDataSampleSize),-accident_date);								

sampleFile := agencyDataSample+ cruArchiveSample + iyetekDataSample + ntlDataSample;

output(distribute(sampleFile), ,'~adv::sample::ecrashv2_ssv2::' + BuildVersion , thor , overwrite, expire(1), compressed);
...
#workunit('name', 'NonFCRA Inquiry Daily Cnts');

BuildVersion := ADV.CurrentBuildVersions.File(datasetname='InquiryTableUpdateKeys' and envment='Q' and location = 'B' and cluster = 'N')[1].buildversion;
...
nonfcra_inquiries := project(Key((unsigned)search_info.datetime[1..4]>=2008), transform_raw(left));
...
OutRec NormAdls(recordof(InqByUniqueADL) L, integer C) := TRANSFORM
	...
	self.source_file := 'InquirytableUpdateKeys';
END;
...
DesprayLogicalFileName   := '~qc::bsw::adv::inquirynweekly::counts::vertical::despray';

NonFCRAInquiryCounts :=  InqTotals_Norm + InqByUniqueADL_Norm;
CreateDesprayFile    := output(choosen(NonFCRAInquiryCounts,all), ,DesprayLogicalFileName, CSV(heading(single), quote('"')), overwrite, expire(30) );
Despray              := std.file.despray(DesprayLogicalFileName, '10.48.72.34', 'C:\\adv\\public_records\\InquiryDaily\\results\\NonFCRAInquiryDailyCounts.csv',,,,true);

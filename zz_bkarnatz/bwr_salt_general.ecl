...
export mac_profile( dIn ) := functionmacro
  ...
  %d_norm%:=
		normalize(dIn,
		          %field_cnt%,
	            transform(SALT23.MAC_Character_Counts.Data_Layout,
									      self.fldno:=counter,
												self.fld:=choose(#expand(%'field_values'%))
												)
							);
						
return SALT23.MAC_Character_Counts.FN_Profile(%d_norm%,
		dataset([#expand(%'field_set'%)]
		        , SALT23.MAC_Character_Counts.Field_Identification ) );
	#end
endmacro;

// import compare_test;

																			//Put in Layout of desired dataset
lay:=RECORD
 ...
 END;
	...
	// ip:=ut.foreign_dataland_boca;
	 ...
	 file1:= dataset('~scoringqa::out::nonfcra::fraudpoint_batch_generic_fp1109_0_v2_20140714_1',lay, thor);  //rename dataset location with desired date
	

	// file1;
	
	
	New_Report:=mac_profile(file1);

  OUTPUT(CHOOSEN(New_Report, all), named('all_output'));
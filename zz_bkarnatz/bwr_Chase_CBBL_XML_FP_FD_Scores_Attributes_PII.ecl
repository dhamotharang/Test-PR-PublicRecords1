...
basefilename := '~scoringqa::out::nonfcra::cbbl_xml_chase_20150306_1';
testfilename := '~scoringqa::out::nonfcra::cbbl_xml_chase_20150307_1';  
pii_name := scoring_project_pip.Input_Sample_Names.Chase_CBBL_inFile_name;
...
j2 := join(ds_join_baseline, ds_join_second, left.results.acctno = right.results.acctno
					AND (integer)LEFT.results.cmpyscore < (integer)Right.results.cmpyscore,
					// AND (integer)RIGHT.results.cmpyscore < '300',
					TRANSFORM(cmpr, SELF.res := LEFT + RIGHT + blank));

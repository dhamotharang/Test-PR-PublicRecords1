...
basefilename := '~scoringqa::out::nonfcra::pi02_xml_chase_fp3710_0_20150204_1';    // Can change xml to batch
testfilename := '~scoringqa::out::nonfcra::pi02_xml_chase_fp3710_0_20150205_1';  
pii_name := scoring_project_pip.Input_Sample_Names.Chase_PIO2_xml_inFile_name;  //XML or Batch
...
j1 := join(ds_join_baseline, ds_join_second, left.results.acctno = right.results.acctno
					AND LEFT.results.hriskaddrflag <> RIGHT.results.hriskaddrflag,
...
OUTPUT(count(j1), NAMED('Diff_hriskaddrflag_Count'));
OUTPUT(count(j2), NAMED('Diff_errorcode_count'));
// OUTPUT(count(j3), NAMED('death_count'));
OUTPUT(CHOOSEN(j1, 25), named('Diff_hriskaddrflag'));
 OUTPUT(CHOOSEN(j2, 25), named('Diff_errorcode'));

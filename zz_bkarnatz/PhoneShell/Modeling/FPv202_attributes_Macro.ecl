EXPORT FPv202_attributes_Macro (string IP, unsigned1 Threads, unsigned records, string filename) := Function;
...
Inputfilename := '~scoring_project::in::ndobmeier::fp30dev::fp3_modelval_pii';
outputFile := '~ScoringQA::out::FP202_Attributes_' + filename + '_' + thorlib.wuid();
qa_outputfile := outputfile+'_QA';


eyeball 								:= 25;	//number of records to display in outputs
unsigned 	record_limit 	:= records;   //number of records to process from input file; 0 means ALL
...
f := if(record_limit = 0, //depending on the record_limit, either process the entire input file or the number of records specified
				dataset(Inputfilename,prii_layout, thor),
				choosen(dataset(Inputfilename,prii_layout, thor), record_limit));
...
Return if(SuppressCompromisedDLs,

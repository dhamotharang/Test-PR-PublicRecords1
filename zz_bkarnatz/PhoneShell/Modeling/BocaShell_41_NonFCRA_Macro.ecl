EXPORT BocaShell_41_NonFCRA_Macro(string IP, unsigned1 Threads, unsigned records, string filename) := Function
...
IMPORT Risk_Indicators, RiskWise, riskprocessing;

unsigned record_limit := records;    //number of records to read from input file; 0 means ALL
...
outfile_name := '~ScoringQA::out::BocaShell_41_NonFCRA_' + filename + '_' + thorlib.wuid();
//==================  input file layout  ========================
// layout_input := RECORD
    // STRING Account;
    // STRING FirstName;
...
// this is left for convenience: history date from the input file may be overwritten here
	...
	SELF.HistoryDateYYYYMM := (Integer) le.historydateyyyymm;
...
f := dataset(outfile_name, riskprocessing.layouts.layout_internal_shell_noDatasets, thor);
...
Return output(edina,, outfile_name + '_edina_v41', CSV(heading(single), quote('"')), overwrite);

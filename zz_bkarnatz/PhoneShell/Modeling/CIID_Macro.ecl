EXPORT CIID_Macro(string IP, unsigned1 Threads, unsigned records, string filename) := Function;
...
prii_layout := RECORD
  ...
  string unparsedfullname{xpath('unparsedfullname')};
...
f := IF(records <= 0, DATASET('~njj::ciid::inputsample_csv_1',prii_layout, csv(quote('"'))),
																	 CHOOSEN(DATASET('~njj::ciid::inputsample_csv_1',prii_layout, csv(quote('"'))), records));
...
// output(j_f);
Return output(j_f,,'~ScoringQA::out::CIID_' + filename + '_' + thorlib.wuid(),CSV(HEADING(single), QUOTE('"')), overwrite);
// output(j_f(errorcode<>''),,'~ScoringQA::out::CIID_GongNeustar_PhonesPlusV2_20181217b_Base_error',CSV(QUOTE('"')), overwrite);

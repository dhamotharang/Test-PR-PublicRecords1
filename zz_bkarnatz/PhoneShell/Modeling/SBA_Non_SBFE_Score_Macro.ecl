EXPORT SBA_Non_SBFE_Score_Macro (string IP, unsigned1 Threads, unsigned records, string filename) := Function;
...
// outputFile := '~atomovic::new::amex_8055_gcp_output1_sba_nonsbfe_new_model';
outputFile := '~ScoringQA::out::SBA_nonSBFE_Score_' + filename + '_' + thorlib.wuid();
...
f1 := IF(recordsToRun <= 0, DATASET(inputFile, Layout, CSV(heading(single), quote('"'))), 
                          CHOOSEN(DATASET(inputFile, Layout, CSV(heading(single), quote('"'))), recordsToRun));
										
// output(f1, NAMED('Total_Input')); 
// output(count(f1), NAMED('Total_Input_Cnt')); 
...
New_Lay intoSOAP(f1 le) := TRANSFORM

	self.AccountNumber := le.field1;
	self.CompanyName := le.field2;
	self.AlternateCompanyName := le.field3;
	self.Addr := le.field4;
	self.City := le.field5;
	self.State := le.field6;
	self.Zip := le.field7;
	self.BusinessPhone := le.field8;
	self.TaxIdNumber := le.field9;
	self.BusinessIPAddress := le.field10;
	self.Representativefirstname := le.field13;
	self.RepresentativeMiddleName := le.field14;
	self.Representativelastname := le.field15;
	self.RepresentativeNameSuffix := le.field16;
	self.RepresentativeAddr := le.field17;
	self.RepresentativeCity := le.field18;
	self.RepresentativeState := le.field19;
	self.RepresentativeZip := le.field20;
	self.RepresentativeSSN := le.field21;
	self.RepresentativeDOB := le.field22;
	self.RepresentativeAge := le.field23;
	self.RepresentativeDLNumber := le.field24;
	self.RepresentativeDLState := le.field25;
	self.RepresentativeHomePhone := le.field26;
	self.RepresentativeEmailAddress := le.field27;
	self.RepresentativeFormerLastName := le.field28;
	self.historydate := (integer)le.field30;
	self.SICCode := le.field36;
	self.NAICCode := le.field37;
	SELF := [];
END;

f := DISTRIBUTE(PROJECT(f1, intoSOAP(LEFT)), RANDOM());

// output(f, NAMED('Total_trans_Input')); 
// output(count(f), NAMED('Total_trans_Input_Cnt')); 
...
Return OUTPUT(flatResults,, outputFile, CSV(HEADING(single), QUOTE('"')), OVERWRITE);

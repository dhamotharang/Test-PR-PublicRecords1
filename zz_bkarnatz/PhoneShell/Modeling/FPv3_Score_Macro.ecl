EXPORT FPv3_Score_Macro(string IP, unsigned1 Threads, unsigned records, string filename) := Function
...
// infile_name := ut.foreign_prod + 'nmontpetit::out::chase_1139_apr_dev_sample_pii';
...
outputfile := '~ScoringQA::out::FP3Score_' + filename + '_' + thorlib.wuid();
qa_outputfile := outputfile+'_QA';

eyeball := 25;	//number of records to display in outputs
record_limit := records;   //number of records to process from input file; 0 means ALL
...
f := if(record_limit = 0, //depending on the record_limit, either process the entire input file or the number of records specified
				// dataset(infile_name,prii_layout, csv(quote('"'))),
				// choosen(dataset(infile_name,prii_layout, csv(quote('"'))), record_limit));
				dataset(infile_name,prii_layout, thor),
				choosen(dataset(infile_name,prii_layout, thor), record_limit));
 ...
 //**************************************************************************************** 
  // When hard-coding archive dates, uncomment and modify one of the following sets of code 
	...
	//****************************************************************************************
		...
		self.historyDateTimeStamp := (string)le.historydateyyyymm; 
...
Return output(rolled_to_1_record,,outputfile,CSV(heading(single), quote('"')), overwrite);

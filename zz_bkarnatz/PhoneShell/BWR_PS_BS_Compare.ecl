...
ds_PhoneShellResult := dataset(PSResultFile, PSResultLay, thor);
...
config := RECORD
   ...
   DATASET(configproperties) properties;
  END;

ds_BocaShellInputLay := RECORD
  ...
  boolean fraudpointmode;
  string20 historydatetimestamp;
  boolean include_nonfcra_collections_inquiries;
  boolean retaininputdid;
  string50 datapermissionmask;
  boolean isprescreen;
  boolean includelnj;
  DATASET(config) gateways;
...
// BSInputFile := '~scoringqa::out::phoneshell_project_bocashell_54_nonfcra_may_july_psmatch_w20190416-110232_bsinputsample';
BSInputFile := '~scoringqa::out::phoneshell_project_bocashell_54_nonfcra_may_july_psmatch_w20190424-091550_bsinputsample';

ds_BocaShellInput := dataset(BSInputFile, ds_BocaShellInputLay, thor);
...
// BSResultFile := '~scoringqa::out::phoneshell_project_bocashell_54_nonfcra_may_july_psmatch_w20190416-110232';
BSResultFile := '~scoringqa::out::phoneshell_project_bocashell_54_nonfcra_may_july_psmatch_w20190424-091550';

ds_BocaShellResult := dataset(BSResultFile, BSResultLay, thor);

output(choosen(ds_PhoneShellInput,5), named('ds_PhoneShellInput'));
output(choosen(ds_PhoneShellResult,5), named('ds_PhoneShellResult'));
output(choosen(ds_BocaShellInput,5), named('ds_BocaShellInput'));
output(choosen(ds_BocaShellResult,5), named('ds_BocaShellResult'));
...
NewLay New_transPS(ds_PhoneShellResult le) := TRANSFORM
...
ds_trans_PS_Result := project(ds_PhoneShellResult, New_transPS(left));
...
output(count(ds_trans_PS_Result_deduped), named('count_ds_trans_PS_Result_deduped'));
output(count(ds_trans_BS_Result), named('count_ds_trans_BS_Result'));
...
ashirey.flatten(ds_join_PhoneShell, flatten_PhoneShell);
ashirey.flatten(ds_join_BocaShell, flatten_BocaShell);

scoring_project_pip.COMPARE_DSETS_MACRO(flatten_PhoneShell, flatten_BocaShell, ['results__accountnumber'], 0 );

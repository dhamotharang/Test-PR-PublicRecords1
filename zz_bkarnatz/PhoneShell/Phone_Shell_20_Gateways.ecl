// EXPORT Phone_Shell_20_Gateways := 'todo';
...
#WORKUNIT('name', 'Phone_Shell_20_Gateways');
...
// OutputFile := '~ScoringQA::out::phone_shell_version_20_May_July_gateways_NoRestrictions' + '_PhonesPlusv2_Gong_Base_' + thorlib.wuid();						//Run with No Restrictions
OutputFile := '~ScoringQA::out::phone_shell_version_20_May_July_gateways_Restrictions' + '_PhonesPlusv2_Gong_Base_' + thorlib.wuid();         //Run with Restrictions
...
layoutSOAPIn intoSOAP(Input le) := TRANSFORM
  ...
  SELF.Zip_In := le.zip[1..5] ;	   // Looks like bocashell only takes zip5, so dropping the last 4 to make things match
	SELF.SSN_In := le.SSN;
 ...
 SELF.IncludeLastResort := IncludeLastResort;
	...
	SELF.BocaShell_IncludeRelatives := TRUE;       //Defaulted Value
	// SELF.BocaShell_IncludeDL := FALSE;       //Defaulted Value
	SELF.BocaShell_IncludeDL := TRUE;          //Turning on to match BocaShell
	// SELF.BocaShell_IncludeVehicle := FALSE;       //Defaulted Value
	SELF.BocaShell_IncludeVehicle := TRUE;    //Turning on to match BocaShell
	SELF.BocaShell_IncludeDerog := TRUE;       //Defaulted Value
	SELF.BocaShell_Include_OFAC := FALSE;       //Defaulted Value
	SELF.BocaShell_OFAC_Only := TRUE;       //Defaulted Value
	SELF.BocaShell_ExcludeWatchlists := FALSE;       //Defaulted Value
	SELF.BocaShell_Include_AdditionalWatchlists := FALSE;       //Defaulted Value
	// SELF.BocaShell_DoScore := FALSE;       //Defaulted Value
	SELF.BocaShell_DoScore := TRUE;         //Turning on to match BocaShell
	SELF.BocaShell_SuppressNearDups := FALSE;       //Defaulted Value
	SELF.BocaShell_Require2Elements := FALSE;       //Defaulted Value
	// SELF.BocaShell_AppendBest := 0; //orig phone shell settings
	SELF.BocaShell_AppendBest := 1;       //Defaulted Value
	SELF.BocaShell_OFAC_Version := 1;       //Defaulted Value
	SELF.BocaShell_DOB_Radius := -1;       //Defaulted Value
 // SELF.BocaShell_Watchlist_Threshold := '0.84';    //Defaulted Value
...
// now using new modeling shell for PhoneShell v2
modelingShell := SORT(Phone_Shell.To_Modeling_Shell_v2(goodResults, DataRestrictionMask), AcctNo, -LENGTH(TRIM(Source_List)), Gathered_Ph);

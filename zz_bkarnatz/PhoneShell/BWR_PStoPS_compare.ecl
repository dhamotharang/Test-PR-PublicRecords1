// EXPORT BWR_PStoPS_compare := 'todo';

Import scoring_project_pip, ashirey;


layout_dedup_hist_phone := RECORD
    string20 acctno;
    string10 phone10;
   END;
...
input := RECORD
   ...
   string10 homephone;
   string10 workphone;
   boolean experiangatewayenabled;
   boolean targusgatewayenabled;
   boolean transuniongatewayenabled;
   boolean insurancegatewayenabled;
   DATASET(layout_dedup_hist_phone) inputphonelist;
  END;

layout_gateways_in := RECORD
   string servicename;
   string url;
  END;
...
PSInputLay := RECORD
  ...
  string homephone_in;
  string workphone_in;
  boolean enabletransuniongateway;
  boolean enableinsurancegateway;
  boolean enabletargusgateway;
  DATASET(input) batch_input;
  DATASET(layout_gateways_in) gateways;
  unsigned8 glbpurpose;
  unsigned8 dppapurpose;
  string datarestrictionmask;
  string datapermissionmask;
  integer8 phone_restriction_mask;
  unsigned8 maxnumberofphones;
  unsigned8 insuranceverificationagelimit;
  string spiiaccesslevel;
  unsigned8 relocationsmaxdaysbefore;
  unsigned8 relocationsmaxdaysafter;
  unsigned8 relocationstargetradius;
  string verticalmarket;
  boolean includelastresort;
  unsigned8 phoneshell_version;
  boolean bocashell_includerelatives;
  boolean bocashell_includedl;
  boolean bocashell_includevehicle;
  boolean bocashell_includederog;
  boolean bocashell_include_ofac;
  boolean bocashell_ofac_only;
  boolean bocashell_excludewatchlists;
  boolean bocashell_include_additionalwatchlists;
  boolean bocashell_doscore;
  boolean bocashell_suppressneardups;
  boolean bocashell_require2elements;
  unsigned8 bocashell_appendbest;
  unsigned8 bocashell_ofac_version;
  integer8 bocashell_dob_radius;
  string bocashell_watchlist_threshold;
  string phone_score_model;
  unsigned2 score_threshold;
  boolean usepremiumsource_a;
  integer8 premiumsource_a_limit;
  boolean includephonesfeedback;
  unsigned8 did;
 END;

// PSInputFile := '~scoringqa::out::phone_shell_version_20_phonesplusv2__nogateway_restrictions_bsmatch_w20190416-110313_psinputsample';
// PSInputFile := '~scoringqa::out::phone_shell_version_20_phonesplusv2__nogateway_restrictions_bsmatch_w20190424-091542_psinputsample';

// ds_PhoneShellInput := dataset(PSINputFile, PSInputLay, thor);
...
layout_boca_shell := RECORD
  ...
  lj_datasets lnj_datasets;
  DATASET(tmp_consumer_statements) consumerstatements{xpath('ConsumerStatements/ConsumerStatement'), maxcount(100)};
...
// PSResultFile := '~scoringqa::out::phone_shell_version_20_phonesplusv2__nogateway_restrictions_bsmatch_w20190416-110313';
// PSResultFile := '~scoringqa::out::phone_shell_version_20_phonesplusv2__nogateway_restrictions_bsmatch_w20190424-091542';

// PSResultFilebase := '~scoringqa::out::phone_shell_version_10_phonesplusv2__nogateway_restrictions_nov18_jan19_input_cert_base_rr_w20190513-175745';            //PhoneShell using BS 41 from toggle run 2   
// PSResultFiletest := '~scoringqa::out::phone_shell_version_10_phonesplusv2__nogateway_restrictions_nov18_jan19_input_cert_second_rr_w20190513-180958';           //PhoneShell using BS 41 from toggle run 3   

PSResultFilebase := '~scoringqa::out::phone_shell_version_20_phonesplusv2__nogateway_restrictions_nov18_jan19_input_cert_base_rr_w20190513-175636';            //PhoneShell using BS 41 from toggle run 2   
PSResultFiletest := '~scoringqa::out::phone_shell_version_20_phonesplusv2__nogateway_restrictions_nov18_jan19_input_cert_second_rr_w20190513-181025';           //PhoneShell using BS 41 from toggle run 3   



ds_PhoneShellResult_base := dataset(PSResultFilebase, PSResultLay, thor);
ds_PhoneShellResult_test := dataset(PSResultFiletest, PSResultLay, thor);


output(choosen(ds_PhoneShellResult_base,5), named('ds_PhoneShellResult_Base'));
output(choosen(ds_PhoneShellResult_test,5), named('ds_PhoneShellResult_test'));


/* transforming PS and BS inputs in common input layout */
// NewInputLay := Record
  // string acctno;
  // string firstname_in;
  // string middlename_in;
...
// NewInputLay Input_transPS(ds_PhoneShellInput le) := TRANSFORM
	// self := le;
	// self := [];
// END; 

// NewInputLay Input_transBS(ds_BocaShellInput le) := TRANSFORM
...
// ds_trans_PS_input := project(ds_PhoneShellInput, Input_transPS(left));
// ds_trans_BS_input := project(ds_BocaShellInput, Input_transBS(left));

// output(count(ds_trans_PS_input), named('count_ds_trans_PS_input'));
// output(choosen(ds_trans_PS_input,5), named('ds_trans_PS_input'));
// output(count(ds_trans_BS_input), named('count_ds_trans_BS_input'));
// output(choosen(ds_trans_BS_input,5), named('ds_trans_BS_input'));

/* transforming PS and BS into common Layouts*/
NewLay := Record
  string accountnumber;
  layout_instantid_results iid;
End;

NewLay New_transPS(ds_PhoneShellResult_base le) := TRANSFORM
	self.accountnumber := le.phone_shell.input_echo.acctno;
	self.iid := le.boca_shell.iid;
	self := [];
END; 

NewLay New_transPS1(ds_PhoneShellResult_test le) := TRANSFORM
	self.accountnumber := le.phone_shell.input_echo.acctno;
	self.iid := le.boca_shell.iid;
	self := [];
END; 
...
// NewLay New_transBS(ds_BocaShellResult le) := TRANSFORM
	// self.accountnumber := le.accountnumber;
	// self.iid := le.iid;
	// self := [];
// END; 

ds_trans_PS_Result := project(ds_PhoneShellResult_base, New_transPS(left));
ds_trans_PS_Result1 := project(ds_PhoneShellResult_test, New_transPS1(left));
// ds_trans_BS_Result := project(ds_BocaShellResult, New_transBS(left));
ds_trans_PS_Result_deduped := dedup(ds_trans_PS_Result);
ds_trans_PS_Result1_deduped := dedup(ds_trans_PS_Result1);

// output(choosen(ds_trans_PS_Result,5), named('ds_trans_PS_Result'));
// output(choosen(ds_trans_PS_Result_deduped,5), named('ds_trans_PS_Result_deduped'));
// output(choosen(ds_trans_BS_Result,5), named('ds_trans_BS_Result'));

output(count(ds_trans_PS_Result_deduped), named('count_ds_trans_PS_Result_deduped_base'));
output(choosen(ds_trans_PS_Result_deduped,5), named('ds_trans_PS_Result_deduped_base'));
output(count(ds_trans_PS_Result1_deduped), named('count_ds_trans_PS_Result1_deduped_test'));
output(choosen(ds_trans_PS_Result1_deduped,5), named('ds_ds_trans_PS_Result1_deduped_test'));


/* Joining the PII and Results*/
// join_lay := RECORD
	// NewLay results;
	// NewInputLay pii;
// END;
...
// ds_join_PhoneShell := JOIN(ds_trans_PS_Result_deduped, ds_trans_PS_input, LEFT.accountnumber = RIGHT.acctno, TRANSFORM(join_lay, SELF.results := LEFT; SELF.pii := RIGHT));
// ds_join_BocaShell := JOIN(ds_trans_BS_Result, ds_trans_BS_input, LEFT.accountnumber = RIGHT.acctno, TRANSFORM(join_lay, SELF.results := LEFT; SELF.pii := RIGHT));

// output(count(ds_join_PhoneShell), named('count_ds_join_PhoneShell'));
// output(choosen(ds_join_PhoneShell,5), named('ds_join_PhoneShell'));
// output(count(ds_join_BocaShell), named('count_ds_join_BocaShell'));
// output(choosen(ds_join_BocaShell,5), named('ds_join_BocaShell'));

ashirey.flatten(ds_trans_PS_Result_deduped, flatten_PhoneShell_base);
ashirey.flatten(ds_trans_PS_Result1_deduped, flatten_Phoneshell_test);

scoring_project_pip.COMPARE_DSETS_MACRO(flatten_PhoneShell_base, flatten_Phoneshell_test, ['accountnumber'], 0 );
// scoring_project_pip.CROSSTAB_MACRO(flatten_PhoneShell, flatten_BocaShell, ['results__accountnumber'], 'iid_flags');


/* Comparing individual Attributes */
// cmpr := record, maxlength(12000)
...
// j1 := join(ds_join_PhoneShell, ds_join_BocaShell, left.results.accountnumber = right.results.accountnumber
					// AND LEFT.results.iid.verified_dl_sources <> RIGHT.results.iid.verified_dl_sources,
					// TRANSFORM(cmpr, SELF.res := LEFT + RIGHT + blank));

// j2 := join(ds_join_PhoneShell, ds_join_BocaShell, left.results.accountnumber = right.results.accountnumber
					// AND LEFT.results.iid.iid_flags <> RIGHT.results.iid.iid_flags,
					// TRANSFORM(cmpr, SELF.res := LEFT + RIGHT + blank));

// j3 := join(ds_join_PhoneShell, ds_join_BocaShell, left.results.accountnumber = right.results.accountnumber
					// AND LEFT.results.iid.did2 <> RIGHT.results.iid.did2,
					// TRANSFORM(cmpr, SELF.res := LEFT + RIGHT + blank));


// OUTPUT(count(j1), NAMED('Diff_verified_dl_sources_Count'));
// OUTPUT(CHOOSEN(j1, 25), named('Diff_verified_dl_sources'));
// OUTPUT(count(j2), NAMED('Diff_iid_flags_count'));
// OUTPUT(CHOOSEN(j2, 25), named('Diff_iid_flags'));
// OUTPUT(count(j3), NAMED('Diff_iid_did2_count'));
// OUTPUT(CHOOSEN(j3, 25), named('Diff_iid_did2'));

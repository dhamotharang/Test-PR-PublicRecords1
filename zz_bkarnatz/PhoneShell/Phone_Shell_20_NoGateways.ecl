// EXPORT Phone_Shell_20_NoGateways := 'todo';
...
#WORKUNIT('name', 'Phone_Shell_20_NoGateways');
...
// OutputFile := '~ScoringQA::out::phone_shell_version_20_May_July_No_gateways_NoRestrictions' + '_PhonesPlusv2_Gong_Base_' + thorlib.wuid();						//Run with No Restrictions
OutputFile := '~ScoringQA::out::phone_shell_version_20_May_July_No_gateways_Restrictions' + '_PhonesPlusv2_Gong_Base_' + thorlib.wuid();         //Run with Restrictions
...
layoutSOAPIn intoSOAP(Input le) := TRANSFORM
	...
	SELF.Zip_In := le.zip[1..5];	   // doing this to match bocashell	
	SELF.SSN_In := le.SSN;

...
basefilename := '~scoringqa::out::nonfcra::leadintegrity_batch_generic_attributes_v4_20150929_1';    // v3 or v4   XML or Batch
testfilename := '~scoringqa::out::nonfcra::leadintegrity_batch_generic_attributes_v4_20150930_1';
...
j1 := join(ds_join_baseline, ds_join_second, left.results.AccountNumber = right.results.AccountNumber
			  ...
			  AND LEFT.results.divaddridentitycountnew <> RIGHT.results.divaddridentitycountnew,
...
OUTPUT(count(j1), NAMED('Diff_divaddridentitycountnew_count'));
// OUTPUT(count(j2), NAMED('To_default_count'));
// OUTPUT(count(j3), NAMED('Increase_Count'));
// OUTPUT(count(j4), NAMED('Decrease_count'));
OUTPUT(CHOOSEN(j1, 25), named('Diff_divaddridentitycountnew'));
...
// scoring_qa.CROSSTAB_MACRO(ds_baseline, ds_new, ['AccountNumber'], 'divaddridentitycountnew');
EXPORT SBFE_Credit_Score_stats_macro(ds, at_name) := FUNCTIONMACRO
	...
	ds_slim := ds( (STRING) #expand(at_name) NOT IN ['', '222', '0']);
	...
	stats_layout := RECORD
		...
		REAL miss_rate;
		REAL zero_rate;
		REAL Two_Hundred_Twenty_Two_Rate;
		REAL hit_rate;
		REAL outlier_count;
	END;
	...
	thresh := MAX( 2 * temp_std_dev, 2);
		...
		SELF.Two_Hundred_Twenty_Two_Rate := COUNT( ds( (STRING) #expand(at_name) = '222')) / full_count;  // #222/Full Count 
		SELF.hit_rate := 	(full_count -																														// Full Count-#(blank, 0, 222)/Full Count

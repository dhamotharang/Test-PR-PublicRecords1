EXPORT RV_Attributes_V4_XML_Macro_HistoryDate(fcraroxie_IP, neutralroxie_IP, Thread, Timeout, Retry, Input_dataset, Output_file_name, historydate, DPPA, GLB, DRM):=FUNCTIONMACRO
...
IMPORT Models, iESP, Risk_Indicators, RiskWise, RiskProcessing, UT;
		...
		layout_soap append_settings(Input_dataset le, INTEGER c) := TRANSFORM
    ...
    //ds_soap_in
		soap_in := DISTRIBUTE(PROJECT(Input_dataset, append_settings(LEFT, counter)), Random());

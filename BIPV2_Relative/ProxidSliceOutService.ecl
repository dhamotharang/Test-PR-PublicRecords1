...
odl := project(choosen(BIPV2_Relative.Keys(BFile).Candidates(Proxid=(unsigned)Proxidstr),10000),into(left));
odr := project(choosen(BIPV2_Relative.Keys(BFile).Candidates(Proxid=(unsigned)Pref_Proxidstr),10000),into(left));
...
odl_noprop := BIPV2_Relative.Debug(BFile,s).RemoveProps(odl); // Remove propogated values
odr_noprop := BIPV2_Relative.Debug(BFile,s).RemoveProps(odr); // Remove propogated values
mtchnp := BIPV2_Relative.Debug(BFile,s).AnnotateClusterMatches(odl_noprop,(unsigned)rcidstr);
mtchrnp := BIPV2_Relative.Debug(BFile,s).AnnotateClusterMatches(odr_noprop+odl_noprop(rcid=(unsigned)rcidstr),(unsigned)rcidstr);
output( choosen(sort(mtchnp,-Conf),3),named('RecordMatches'));
output( choosen(sort(mtchrnp,-Conf),3),named('PreferredRecordMatches'));
mtch := BIPV2_Relative.Debug(BFile,s).AnnotateClusterMatches(odl,(unsigned)rcidstr);
mtchr := BIPV2_Relative.Debug(BFile,s).AnnotateClusterMatches(odr+odl(rcid=(unsigned)rcidstr),(unsigned)rcidstr);

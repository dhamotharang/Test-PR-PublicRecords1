...
*/
IMPORT Address, AutoStandardI, BIPV2, Census_Data, Doxie, dx_Gong, iesp, MDR, Codes, Suppress;
...
EXPORT OperationsSitesSection := MODULE;
 ...
 export fn_FullView(
  ...
  // Filter to only keep recs that did not have an address cleaning error and
	...
	ds_bh_keyrecs_slimmed := project (ds_bh_keyrecs(err_stat[1] !='E' and
	                                                ...
	                                                (prim_range !=''  or prim_name !=''   or
	                                                 p_city_name !='' or v_city_name !='' or st !=''  or zip !=''   or company_phone !='')),  
                    ...
                    // Similar to coding in the BestSection.  So probably need to put this into a function
                     ...
                     self.idtype := if  (MDR.sourcetools.SourceIsBusiness_Registration(left.source) OR
                                                   MDR.sourceTools.SourceIsExperian_FEIN(left.source) ,'',temp_idType);
                     ...
                     temp_srcdocid := if (MDR.SourceTools.SourceIsBankruptcy(left.source), //for bug 149258
                         ...
                         self.bh_phone_type := if (left.company_phone !='',  left.phone_type,''); //save for use later
        ...
        // added Address.FipsToMSA attribute) was created to handle this situation.
                       ...
                       Address.FipsToMSA2_NEStateCity(left.fips_state + left.fips_county + left.st +
                           if(left.v_city_name !='', left.v_city_name, left.p_city_name)),
                       '0000');
  ...
  if(le.dt_first_seen < ri.dt_first_seen,  le.dt_first_seen, ri.dt_first_seen));
     ...
     ds_unique_phones_gong_rolled := rollup(
          ...
          group,
            transform(rec_layout_addrphone_wphonemeta,
                ...
                self.from_date := if ( (string8) min(rows(left)(current_flag),gong_dt_first_seen) <> '', (string8) min(rows(left)(current_flag),gong_dt_first_seen),
                                         (string8) min(rows(left),gong_dt_first_seen));
                self.to_date :=  if ( (string8) max(rows(left)(current_flag),gong_dt_last_seen) <> '', (string8) max(rows(left)(current_flag),gong_dt_last_seen),
                                            (string8) max(rows(left),gong_dt_last_seen));
...
// First sort/group the recs.
  ds_addrphone_recs_srtd_grpd := group(//re-sort all recs/phone#s for each address
    //in descending date order???
      ...
      TopBusiness_Services.OperationsSitesSection_Layouts.rec_OpsSitesChild_Address tf_rollup_phones(
        ...
        iesp.constants.TOPBUSINESS.MAX_COUNT_BIZRPT_PHONES_PER_ADDRESS);
         ...
         self.city_name   := if(l.p_city_name = l.v_city_name,l.p_city_name,l.v_city_name);
	...
	ds_addrphone_recs_rolled_phones := rollup(ds_addrphone_recs_srtd_grpd, group,
 ...
 // Now count the number of addresses for each state for a set on linkids.
    rec_layout_address_st_count := record
     ...
     ds_ids_states_tabled := table(ds_addrphone_recs_rolled_phones,rec_layout_address_st_count,
            #expand(BIPV2.IDmacros.mac_ListTop3Linkids()),st,few);  //few???

   // Join number of addresses per state count for the linkids back to the address recs
   //  with rolled phones.
     ds_addrphone_recs_plus_stcnt := join(ds_addrphone_recs_rolled_phones,ds_ids_states_tabled,
         BIPV2.IDmacros.mac_JoinTop3Linkids() and left.st = right.st,
...
// property ln_fares_id when applicable.
  ds_addrphone_recs_top50_wproplink := join(ds_addrphone_recs_top50_perst, ds_in_prop_recs,
...
// here or somewhere above???
  // Join to a Census_Data key to fill in the county_name.
	ds_addrphone_recs_wcounty_name :=  join(ds_addrphone_recs_top50_wproplink,   Census_Data.Key_Fips2County,
...
tf_rollup_staterecs(left,rows(left)));
  ...
  ds_recs_rptdetail := project(ds_parentrecs,   tf_rpt_detail(left));

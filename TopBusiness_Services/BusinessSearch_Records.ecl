IMPORT autostandardI, BIPV2, BIPV2_Best,
  BIPV2_WAF, Census_Data, Doxie, dx_BestRecords,
...
// used by ssn masking function
 ...
 string6 ssn_mask_value := mod_access.ssn_mask;
   ...
   unique_phones_wgongdata_0_SELEID := Suppress.MAC_SuppressSource(unique_phones_wgongdata_0_SELEID_raw, mod_access);
...
//------------------------------------------------- proxid phone metadata	-------------
  ...
  unique_phones_wgongdata_0_a := Suppress.MAC_SuppressSource(unique_phones_wgongdata_0_a_raw, mod_access);
  ...
  unique_phones_wgongdata_1 := dedup(unique_phones_wgongdata_0, all);
    ...
    unique_phones_gong_rolled := rollup(
       ...
       //added filter seleid <> 0 because some proxid's that are a non source = D then have a seleid
          ...
          best_perms := dx_BestRecords.Functions.get_perm_type_idata(mod_access);
          ...
          tmp_return_results_NEW := join(ds_UltInfo(seleid <> 0), TopResultsScored_wirelessIndicator,
            ...
            transform(TopBusiness_Services.BusinessSearch_Layouts.tmp_output_layout_full,
              ...
              // If SALT contact did is present on opt out, replace with best contact info.
              ds_dids := DATASET([{RIGHT.contact_did}, {LEFT.contact_did}], {unsigned did})(did <> 0);
              allowed_dids := Suppress.MAC_SuppressSource(ds_dids, mod_access, gsid_field:=NULL);
              is_salt_contact_suppressed :=  RIGHT.contact_did <> 0 AND NOT EXISTS(allowed_dids(did = RIGHT.contact_did));
              is_bip_contact_suppressed :=  LEFT.contact_did <> 0 AND NOT EXISTS(allowed_dids(did = LEFT.contact_did));
              best_data := dx_BestRecords.get(ds_dids, did, best_perms);
              best_salt_contact := best_data(did = RIGHT.contact_did)[1];
              best_bip_contact := best_data(did = LEFT.contact_did)[1];
              ...
              self.Best.ContactInfo.Title := IF(is_bip_contact_suppressed, '', left.contact_job_title_derived);
              self.Best.ContactInfo.UniqueID :=  (string12) left.contact_did;
              self.Best.ContactInfo.Name.Prefix := IF(is_bip_contact_suppressed, best_bip_contact.title, left.title);
              self.Best.ContactInfo.Name.First  := IF(is_bip_contact_suppressed, best_bip_contact.fname, left.fname);
              self.Best.ContactInfo.Name.Last   := IF(is_bip_contact_suppressed, best_bip_contact.lname, left.lname);
              self.Best.ContactInfo.Name.Middle := IF(is_bip_contact_suppressed, best_bip_contact.mname, left.mname);
              self.Best.ContactInfo.Name.Suffix := IF(is_bip_contact_suppressed, best_bip_contact.name_suffix, left.name_suffix);
              ...
              // Use best data in place of salt contact when suppressed, only for fields with values.
              optout_fname := MAP(
                is_salt_contact_suppressed AND right.fname <> '' => best_salt_contact.fname,
                is_salt_contact_suppressed => '',
                right.fname);

              optout_mname := MAP(
                is_salt_contact_suppressed AND right.mname <> '' => best_salt_contact.mname,
                is_salt_contact_suppressed => '',
                right.mname);

              optout_lname := MAP(
                is_salt_contact_suppressed AND right.lname <> '' => best_salt_contact.lname,
                is_salt_contact_suppressed => '',
                right.lname);

              optout_ssn := MAP(
                is_salt_contact_suppressed AND right.contact_ssn <> '' => best_salt_contact.ssn,
                is_salt_contact_suppressed => '',
                right.contact_ssn);

              tmpFname := if (not isDNBDMIrow, optout_fname, '');
              self.match.ContactInfo.Name.First :=  tmpFname;

              tmpLname := if (not isDNBDMIrow, optout_lname, '');
              self.match.ContactInfo.Name.Last :=  tmpLname;

              tmpMname := if (not isDNBDMIrow, optout_mname, '');
              self.match.ContactInfo.Name.Middle := tmpMname;
              tmpSSN := if (not isDNBDMIRow, optout_ssn, '');

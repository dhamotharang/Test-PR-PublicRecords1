...
EXPORT AssociateSection := MODULE;
 ...
 export fn_FullView(
  ...
  ds_bankr_linkids_keyrecs_plusparty := 
	                         join(ds_bankr_linkids_keyrecs_deduped,
  ...
  ds_bankr_common_child := project(ds_bankr_linkids_keyrecs_plusparty_dd,
        ...
        self.associate_type             := if(left.cname !='',Constants.Business,
				                                                      Constants.Person);
  ...
  ds_mvr_common_child := project(ds_mvr_linkids_keyrecs_plusparty_dd,
        ...
        self.associate_type             := if(left.append_clean_cname !='',Constants.Business,
			  ...
			  self := left, // to preserve rptd on ids & other fields being kept with same name
				self := [], // to null out any unassigned fields, 2 total_* fields
			));
  ...
  ds_ucc_common_child := project(ds_ucc_linkids_keyrecs_plusparty_dd,
        ...
        self.associate_type             := if(left.company_name !='',Constants.Business,
				                                                             Constants.Person),
  ...
  ds_wc_linkids_keyrecs_plusparty :=
	                   join(ds_wc_linkids_keyrecs_deduped,
...
// !!! Do roll-up instead of sort/dedup above???  See comments below in the NOD section.???
  ...
  // Then "Normalize" to put each "name" field (up to 7) on a separate record.   ???
    ...
    recordof(ds_fc_linkids_keyrecs_pp_dd) L, integer C) := transform
        ...
        self.associate_type             := 
				     choose(C, if(L.name1_company !='',Constants.Business,Constants.Person),
  ...
  ds_fc_common_child_normed := normalize(ds_fc_linkids_keyrecs_pp_dd,
	...
	ds_fc_common_child_normed_dd := dedup(sort(ds_fc_common_child_normed(
	                                               (company_name != '' and  
																								  (company_name != forec_name_company
																									 or
																									 (associated_business_linkids.seleid != 0 and
																									  seleid != associated_business_linkids.seleid) 
																									)
																								 ) 
																								 or 
	                                               person_name.lname !=''
  ...
  ds_all_common_child_sorted := dedup(
	                                    sort(ds_all_common_child,
 				                              #expand(BIPV2.IDmacros.mac_ListTop3Linkids()),
																		  associate_type,
  ...
  ds_all_common_child_rolled := rollup(ds_all_common_child_sorted,
																	     tf_rollup_common_child(left,right),
                                       #expand(BIPV2.IDmacros.mac_ListTop3Linkids()),
															         associate_type,
															         if(associated_business_linkids.ultid !=0, associated_business_linkids.ultid,0), //???
                                       if(associated_business_linkids.orgid !=0, associated_business_linkids.orgid,0),  //???
  ...
  // Split into separate datasets for business & person associates
	ds_all_common_child_rolled_bus := ds_all_common_child_rolled(
	                                     associate_type != Constants.PERSON);
	ds_all_common_child_rolled_per := ds_all_common_child_rolled(
	                                     associate_type = Constants.PERSON);
  ...
  ds_all_common_child_top100_bp := dedup(sort(ds_all_common_child_wcounts,
	                                            #expand(BIPV2.IDmacros.mac_ListTop3Linkids()),
																		          associate_type,
																				      -date_vendor_last_reported), //most recent???
	                                       #expand(BIPV2.IDmacros.mac_ListTop3Linkids()),
																	       associate_type,
   ...
   ds_all_common_child_top100_bus := 
	                       ds_all_common_child_top100_bp(associate_type = Constants.BUSINESS);
   ds_all_common_child_top100_per := 
  ...
  ds_all_common_child_top100_per_derogb := join(ds_all_common_child_top100_per_masked,
	...
	ds_all_common_child_top100_per_derogl:= join(ds_all_common_child_top100_per_derogb,
	                                             LiensV2.key_liens_DID,
			                                           keyed((unsigned6)left.did = right.did),
																							 transform(AssociateSection_Layouts.rec_child_party,
															                    self.has_derog := if(left.has_derog,left.has_derog, 
																																			 if(right.did != 0,true,false)),
													                        self := left),
													                     left outer, keep(1)); //???
  ...
  AssociateSection_Layouts.rec_parent_section tf_denorm_children(
      ...
      // Determine total values
		  ...
		  self.total_person   := max(R,total_person);
			...
			self.business_associates := choosen(project(sort(R(associate_type=Constants.BUSINESS),
			                                                 company_name

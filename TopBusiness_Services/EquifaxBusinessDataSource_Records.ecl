...
// ================================================================================
IMPORT BIPV2, iesp,Address ,TopBusiness_Services, STD;
 ...
 := MODULE
  ...
  SHARED in_docs_linkonly := PROJECT(in_docids,TRANSFORM(BIPV2.IDlayouts.l_xlink_ids,
   ...
   SHARED EquifaxBusinessDataRecs_idValue := JOIN(ds_EquifaxBusinessDataRecsSlim,in_docids,
										...
										TRANSFORM(LEFT)); // do I need a keep(1) here?
  															
	UNSIGNED1 region := address.Components.Country.US; // default
	
	iesp.topbusinessOtherSources.t_otherSICCode xform_sic(RECORDOF(EquifaxBusinessDataRecs_idValue) L, INTEGER C) := TRANSFORM
	...
	iesp.topbusinessOtherSources.t_otherNAICSCodes xform_naics(RECORDOF(EquifaxBusinessDataRecs_idValue) L, INTEGER C) := TRANSFORM
	 ...
	 // output(ds_EquifaxBusinessDataRecs, named('ds_EquifaxBusinessDataRecs'));
	// output(ds_EquifaxBusinessDataRecsSlim, named('ds_EquifaxBusinessDataRecsSlim'));

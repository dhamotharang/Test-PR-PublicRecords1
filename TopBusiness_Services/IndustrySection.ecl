/* TBD: 
     ...
     2. Check/review individual source industry key mappers (i.e. Corp2.As_Industry, etc.),
        ...
        If any discrepencies vs BIP1 TopBusiness.***_AsMasters.As_Industry_Master, 
        discuss with Tom Reed & Giri Rajulapalli???
*/
...
EXPORT GetIndustryBipLinkids(dataset(BIPV2.IDlayouts.l_xlink_ids)  bipLinkids, 
 ...
 ds_industry_keyrecs  := GetIndustryBipLinkids(ds_in_ids_only
  ...
  ds_industry_keyrecs_srcs := dedup(sort(project(ds_industry_keyrecs_touse, 
     transform(IndustrySection_Layouts.rec_IndustryChild_Source,
  ...
  // First filter to process recs with non-blank, non-zero SIC codes.
	ds_indusrecs_sic_filtered := ds_industry_keyrecs_touse(siccode !='' and 
																									       siccode !='0000');
	...
	ds_indusrecs_sic_wdescs := join(dedup(sort(ds_indusrecs_sic_filtered, 
                                            #expand(BIPV2.IDmacros.mac_ListTop3Linkids()),
																						siccode,record),
														            #expand(BIPV2.IDmacros.mac_ListTop3Linkids()),
																				siccode), 
                                    ...
                                    left.siccode = right.sic4_code,	
																  transform(IndustrySection_Layouts.rec_IndustryChild_SIC,
																    ...
																    self.sicdescription := 
																		  StringLib.StringToProperCase(
																			  StringLib.StringToLowerCase(right.SIC4_Description)),
  ...
  ds_indusrecs_naics_filtered := ds_industry_keyrecs_touse (naics !=''and 
	                                                          naics !='000000'); 
  ...
  ds_indusrecs_naics_wdescs := join(dedup(sort(ds_indusrecs_naics_filtered, 
	                                             #expand(BIPV2.IDmacros.mac_ListTop3Linkids()), 
																							 naics, record), 
	                                        #expand(BIPV2.IDmacros.mac_ListTop3Linkids()),
																					naics),
                                      ...
                                      left.naics = right.NAICS_code, 
																	  transform(IndustrySection_Layouts.rec_IndustryChild_NAICS,
  ...
  ds_indusrecs_windesc_deduped := 
	      project(dedup(sort(ds_indusrecs_windesc, 
	              ...
	              transform(IndustrySection_Layouts.rec_ids_withdata_slimmed,
								  self := left));	
  ...
  // put into the needed parent layout.
	ds_parents_windesc := join(ds_in_ids_wacctno,ds_indusrecs_windesc_deduped,
	                           ...
	                           transform(IndustrySection_Layouts.rec_IndustryParent,
  ...
  IndustrySection_Layouts.rec_IndustryParent tf_denorm_SIC(
	  ...
	  dataset(IndustrySection_Layouts.rec_IndustryChild_SIC) R)	:= transform
		  self.SICs    := choosen(project(R,IndustrySection_Layouts.rec_IndustryChild_SIC),
  ...
  IndustrySection_Layouts.rec_IndustryParent tf_denorm_NAICS(
	  IndustrySection_Layouts.rec_IndustryParent L,   
	  dataset(IndustrySection_Layouts.rec_IndustryChild_NAICS) R)	:= transform
		  self.NAICSs := choosen(project(R,IndustrySection_Layouts.rec_IndustryChild_NAICS),
  ...
  IndustrySection_Layouts.rec_IndustryParent tf_denorm_sourceinfo(
	  IndustrySection_Layouts.rec_IndustryParent L,   
	  dataset(IndustrySection_Layouts.rec_IndustryChild_Source) R)	:= transform
  ...
  // transform to handle "SourceDocs" child dataset
	iesp.topbusiness_share.t_TopBusinessSourceDocInfo 
	  tf_rpt_Source(IndustrySection_Layouts.rec_IndustryChild_Source l) := transform
  ...
  // transform to handle "SIC" child dataset
	iesp.TopbusinessReport.t_TopbusinessIndustrySIC 
	  tf_rpt_SIC(IndustrySection_Layouts.rec_IndustryChild_SIC l) := transform
  ...
  // transform to handle "NAICS" child dataset
	iesp.TopbusinessReport.t_TopbusinessIndustryNAICS 
	  tf_rpt_NAICS(IndustrySection_Layouts.rec_IndustryChild_NAICS l) := transform
      self.NAICS            := l.naics,
      self.NAICSDescription := l.NAICSDescription;
	end;

  // transform to store data in the iesp report detail parent dataset fields
	IndustrySection_Layouts.rec_ids_plus_IndustryDetail
	  tf_rpt_detail(IndustrySection_Layouts.rec_IndustryParent l) := transform
  ...
  IndustrySection_Layouts.rec_ids_plus_IndustrySection tf_rollup_rptdetail(
	  IndustrySection_Layouts.rec_ids_plus_IndustryDetail l,   
	  dataset(IndustrySection_Layouts.rec_ids_plus_IndustryDetail) allrows) := transform
      self.acctno          := '';
  ...
  ds_all_wacctno_joined := join(ds_in_ids_wacctno,ds_all_rptdetail_rolled,
																	BIPV2.IDmacros.mac_JoinTop3Linkids(),
														    transform(IndustrySection_Layouts.rec_ids_plus_IndustrySection,
	...
	ds_final_results := rollup(group(sort(ds_all_wacctno_joined,acctno),
	                                 acctno),
														 group,
		                         transform(IndustrySection_Layouts.rec_Final,
                               self := left));

// ================================================================================
// ===== RETURNS BBB Non member Source Doc and Source Count Info
// ================================================================================
IMPORT BIPV2, BBB2, ut, iesp, MDR;

EXPORT BBBNonMemSource_Records (
  dataset(Layouts.rec_input_ids_wSrc) in_docids,
  SourceService_Layouts.OptionsLayout inoptions, 
	boolean IsFCRA = false) 
 := MODULE

	// There isn't any unique id key file for bbb non member to build the source doc from, for this reason
  ...
  // *** Key fetch to get bbb data from BBB Non Member linkid
  bbb_recs := BBB2.Key_BBB_Non_Member_LinkIds.kfetch(DEDUP(in_docs_linkonly,ALL),inoptions.fetch_level,,
	...
	SHARED iesp.topbusinessOtherSources.t_OtherSourceRecord toOut(recordof(bbb_idValue) L) := TRANSFORM
		...
		SELF.BusinessDescription		:= L.non_member_category;
	...
	SHARED SourceCount_Layouts.SourceDetailsLayout xform_Details(recordof(bbb_idValue) L) := TRANSFORM
			self.src			:= 'BBBNM';
			self.src_desc := 'Better Business Bureau Non-Member';

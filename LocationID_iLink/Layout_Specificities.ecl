...
EXPORT err_stat_ChildRec := RECORD
  ...
  UNSIGNED4 id;
END;
EXPORT cntprimname_ChildRec := RECORD
  TYPEOF(l.cntprimname) cntprimname;
  UNSIGNED8 cnt;
  UNSIGNED4 id;
END;
EXPORT R := RECORD,MAXLENGTH(32000)
 UNSIGNED1 dummy;
  ...
  REAL4 cntprimname_specificity;
  REAL4 cntprimname_switch;
  REAL4 cntprimname_maximum;
  DATASET(cntprimname_ChildRec) nulls_cntprimname {MAXCOUNT(100)};
END;
END;

...
EXPORT SALT37.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'alpha','number','upper','zip5','hasZip4','alpha_st');
EXPORT FieldTypeNum(SALT37.StrType fn) := CASE(fn,'alpha' => 1,'number' => 2,'upper' => 3,'zip5' => 4,'hasZip4' => 5,'alpha_st' => 6,0);
...
EXPORT SALT37.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'prim_range','v_city_name','st','zip5','cntprimname','sec_range','postdir','unit_desig','aid','predir','prim_name','rec_type','err_stat','addr_suffix');
EXPORT FieldNum(SALT37.StrType fn) := CASE(fn,'prim_range' => 0,'v_city_name' => 1,'st' => 2,'zip5' => 3,'cntprimname' => 4,'sec_range' => 5,'postdir' => 6,'unit_desig' => 7,'aid' => 8,'predir' => 9,'prim_name' => 10,'rec_type' => 11,'err_stat' => 12,'addr_suffix' => 13,0);
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_prim_range,1,0)+ IF( SELF.Diff_v_city_name,1,0)+ IF( SELF.Diff_st,1,0)+ IF( SELF.Diff_zip5,1,0)+ IF( SELF.Diff_cntprimname,1,0)+ IF( SELF.Diff_sec_range,1,0)+ IF( SELF.Diff_postdir,1,0)+ IF( SELF.Diff_unit_desig,1,0)+ IF( SELF.Diff_aid,1,0)+ IF( SELF.Diff_predir,1,0)+ IF( SELF.Diff_prim_name,1,0)+ IF( SELF.Diff_rec_type,1,0)+ IF( SELF.Diff_err_stat,1,0)+ IF( SELF.Diff_addr_suffix,1,0);
  END;

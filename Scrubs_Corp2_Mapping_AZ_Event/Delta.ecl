...
EXPORT Delta(DATASET(Layout_in_file)old_s, DATASET(Layout_in_file) new_s) := MODULE//Routines to compute the differences between two instances of a file
  SHARED inFieldList := ['corp_key','corp_sos_charter_nbr','event_date_type_cd'];
  ...
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE) := FUNCTION
    ...
    SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_Corp2_Mapping_AZ_Event, Fields, 'RECORDOF(hygieneDiffOverall)', FALSE);

...
EXPORT MakeFT_invalid_cd(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_cd(SALT32.StrType s) := WHICH(((SALT32.StrType) s) NOT IN ['C','D','M','N','O','V','X','FIL',' ']);
EXPORT InValidMessageFT_invalid_cd(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInEnum('C|D|M|N|O|V|X|FIL| '),SALT32.HygieneErrors.Good);
 
EXPORT SALT32.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'corp_key','corp_sos_charter_nbr','event_date_type_cd');
EXPORT FieldNum(SALT32.StrType fn) := CASE(fn,'corp_key' => 0,'corp_sos_charter_nbr' => 1,'event_date_type_cd' => 2,0);
...
EXPORT Make_event_date_type_cd(SALT32.StrType s0) := MakeFT_invalid_cd(s0);
EXPORT InValid_event_date_type_cd(SALT32.StrType s) := InValidFT_invalid_cd(s);

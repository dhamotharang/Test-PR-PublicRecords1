EXPORT Monitor := MODULE
	...
	EXPORT FindTimeIssues	() 		:= MODULE
			...
			export EmailNotification := if (WorkunitTimings[1].duration > _Constants.max_execution_time, fileservices.sendemail(vRecipients,	vSubject, vBody));
	...
	EXPORT CheckGeoMaxCalls	() 		:= MODULE
			...
			export EmailNotification := if (xcount >= _Constants.notify_after_calls, fileservices.sendemail(vRecipients,	vSubject, vBody	));
	END;

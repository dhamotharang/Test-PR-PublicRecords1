...
EXPORT SALT311.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_charter_nbr','invalid_stock_class','invalid_stock_shares_issued','invalid_stock_authorized_nbr','invalid_stock_par_value');
EXPORT FieldTypeNum(SALT311.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_corp_vendor' => 2,'invalid_state_origin' => 3,'invalid_mandatory' => 4,'invalid_date' => 5,'invalid_charter_nbr' => 6,'invalid_stock_class' => 7,'invalid_stock_shares_issued' => 8,'invalid_stock_authorized_nbr' => 9,'invalid_stock_par_value' => 10,0);
...
EXPORT MakeFT_invalid_stock_class(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_stock_class(SALT311.StrType s) := WHICH(~Scrubs_Corp2_Mapping_NH_Stock.Functions.valid_stock_class_type_desc(s)>0);
EXPORT InValidMessageFT_invalid_stock_class(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.CustomFail('Scrubs_Corp2_Mapping_NH_Stock.Functions.valid_stock_class_type_desc'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_stock_shares_issued(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,' 0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_stock_shares_issued(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,' 0123456789'))));
EXPORT InValidMessageFT_invalid_stock_shares_issued(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars(' 0123456789'),SALT311.HygieneErrors.Good);
...
EXPORT SET OF SALT311.StrType FieldRules(UNSIGNED2 i) := CHOOSE(i,['ALLOW','LENGTHS'],['ENUM'],[],[],['ENUM'],['ALLOW','CUSTOM'],['ALLOW','LENGTHS'],[],[],[],['CUSTOM'],['ALLOW'],['ALLOW'],['ALLOW'],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[]);
...
EXPORT Make_stock_class(SALT311.StrType s0) := MakeFT_invalid_stock_class(s0);
EXPORT InValid_stock_class(SALT311.StrType s) := InValidFT_invalid_stock_class(s);
EXPORT InValidMessage_stock_class(UNSIGNED1 wh) := InValidMessageFT_invalid_stock_class(wh);
 
EXPORT Make_stock_shares_issued(SALT311.StrType s0) := MakeFT_invalid_stock_shares_issued(s0);
EXPORT InValid_stock_shares_issued(SALT311.StrType s) := InValidFT_invalid_stock_shares_issued(s);
EXPORT InValidMessage_stock_shares_issued(UNSIGNED1 wh) := InValidMessageFT_invalid_stock_shares_issued(wh);
...
EXPORT Make_stock_nbr_par_shares(SALT311.StrType s0) := s0;
EXPORT InValid_stock_nbr_par_shares(SALT311.StrType s) := 0;
...
EXPORT Make_stock_total_capital(SALT311.StrType s0) := s0;
EXPORT InValid_stock_total_capital(SALT311.StrType s) := 0;
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT311,Scrubs_Corp2_Mapping_NH_Stock;

IMPORT UT,STD,LUCI;
IMPORT HCSE_SETC_V3_LOB_EXT_MODEL AS LUCI_Model;
SHARED Constants := LUCI.Constants;
SHARED Layouts := HCSE_SETC_V3_LOB_EXT_MODEL.z_layouts;
...
SHARED SprayCSVName      := '~HCSE_SETC_V3_LOB_EXT_MODEL::' + (STRING)STD.Date.Today() + '::' + ModelName +'_from_csv'; // Will set up by user if test file is a logical file.
...
SHARED isCSVFile := TRUE; // Will set up by user.
  LUCI.FNSpray(LandingZoneIP, lzFilePath, SprayCSVName, CSVSpraySeparator, CSVSprayQuote);
...
\******************************************************************************/
  ...
  Layouts.StandLayouts xGetLUCIPointsAssignments(LUCI_Points_Assignments_Pre L) := TRANSFORM
    ...
    SELF.SOURCE := 'HPCC';
    SELF.stdrawscore  := (string)L.Model1_Score1_Score0;
    SELF.std_scr := (string)L.Model1_Score;
    SELF := L;
    SELF := [];
  END;

...
rIssueDateYearMonthTable
 :=
  record
	...
	unsigned3	YearMonthCount	:=	count(group,dStateIssueDate.issue_year_month);
  end
 ;
...
Drivers.Layout_DL	tExperianFixedToCommon(Drivers.Mapping_Experian_XML_Fixed pInput, dLatestYearMonth pLatestDate)
 :=
  transform
	...
	self.orig_expiration_date		:=	(unsigned4)fDateStringFromSlashedMDY(pInput.expiration_date);
	...
	self.dl_number 					:=	if(pInput.dl_number<>'',
										   pInput.dl_number,
										   'LNID:' + (string)hash(self.orig_state,self.name,self.dob,self.sex_flag,self.height,self.weight,self.zip)
										  );
	...
	self.restrictions_delimited		:=	lRestriction1
									...
									;
	self.license_type				:=	fLicenseTypeFromLicenseClassString(pInput.license_class1);
	self.lic_endorsement			:=	trim(fEndorsementCodeFromLicenseClassString(pInput.license_class1)
										   + fEndorsementCodeFromLicenseClassString(pInput.license_class2)
										   + fEndorsementCodeFromLicenseClassString(pInput.license_class3)
										   ,all);
	...
	self.history					:=	if(self.orig_expiration_date = 0,		// if no expiration date, set history to 'U'
										   'U',
										   ''
										  );

...
Drivers.Layout_DL	tFixDummyDates(dInlineDummyData_Input pInput)
 :=
  transform
	self.dt_first_seen				:=	if(pInput.dt_first_seen = 0,			(unsigned3)((string6)Drivers.Version_Development[1..6]),pInput.dt_first_seen);
	self.dt_last_seen				:=	if(pInput.dt_last_seen = 0,				(unsigned3)((string6)Drivers.Version_Development[1..6]),pInput.dt_last_seen);
	self.dt_vendor_first_reported	:=	if(pInput.dt_vendor_first_reported = 0,	(unsigned3)((string6)Drivers.Version_Development[1..6]),pInput.dt_vendor_first_reported);
	self.dt_vendor_last_reported	:=	if(pInput.dt_vendor_last_reported = 0,	(unsigned3)((string6)Drivers.Version_Development[1..6]),pInput.dt_vendor_last_reported);
	self							:=	pInput;
  end
 ;

...
export Candidates := index(prop_file,{},{prop_file},'~temp::::bell_thrive::match_candidates_debug');
ms_temp := sort(mtch,Conf,1,2,skew(1.0)); // Some headers have very skewed IDs
export MatchSample := index(ms_temp,{Conf,1,2},{mtch},'~temp::::bell_thrive::match_sample_debug',sort keyed);
export Specificities_Key := index(s,{1},{s},'~temp::::bell_thrive::specificities_debug');
prop_file := matches(ih).Patched_Candidates; // Available for External ADL2
export PatchedCandidates := index(prop_file,{},{prop_file},'~temp::::bell_thrive::patched_candidates');
end;

...
function
	...
	dStandardizedName := project(pSprayedFile	,transform(
		layouts.base,
			...
			self.rawfields									:= left;
			self.clean_phones.Phone_Home		:= ut.CleanPhone(left.Phone_Home					);			
			self.clean_phones.Phone_Cell		:= ut.CleanPhone(left.Phone_Cell					);			
			self.clean_phones.Phone_Work		:= ut.CleanPhone(left.Phone_Work					);			

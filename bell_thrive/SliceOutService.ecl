...
odl := project(choosen(bell_thrive.Keys(BFile).Candidates(=(unsigned)str),10000),into(left));
odr := project(choosen(bell_thrive.Keys(BFile).Candidates(=(unsigned)Pref_str),10000),into(left));
...
odl_noprop := bell_thrive.Debug(BFile,s).RemoveProps(odl); // Remove propogated values
odr_noprop := bell_thrive.Debug(BFile,s).RemoveProps(odr); // Remove propogated values
mtchnp := bell_thrive.Debug(BFile,s).AnnotateClusterMatches(odl_noprop,(unsigned)str);
mtchrnp := bell_thrive.Debug(BFile,s).AnnotateClusterMatches(odr_noprop+odl_noprop(=(unsigned)str),(unsigned)str);
output( choosen(sort(mtchnp,-Conf),3),named('RecordMatches'));
output( choosen(sort(mtchrnp,-Conf),3),named('PreferredRecordMatches'));
mtch := bell_thrive.Debug(BFile,s).AnnotateClusterMatches(odl,(unsigned)str);
mtchr := bell_thrive.Debug(BFile,s).AnnotateClusterMatches(odr+odl(=(unsigned)str),(unsigned)str);

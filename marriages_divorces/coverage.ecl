...
in_md := marriages_divorces.File_Marriage_Divorce_Base(filing_type in ['3','7']);
...
r1 t_norm_mar(in_md_mar le) := transform
 self.event_date      := if(le.marriage_dt<>'',le.marriage_dt,le.filing_dt);
 self.event_date_yyyy := if(le.marriage_dt<>'',le.marriage_dt[1..4],le.filing_dt[1..4]);
 self.date_type       := 'MARRIAGE';
 self                 := le;
end;

r1 t_norm_div(in_md_div le) := transform
 self.event_date      := if(le.divorce_dt<>'',le.divorce_dt,le.filing_dt);
 self.event_date_yyyy := if(le.divorce_dt<>'',le.divorce_dt[1..4],le.filing_dt[1..4]);
...
string fgooddate(string8 pDate) := if(length(trim(pDate)) in [4,6,8] 
									 ...
									 and (string)((integer)ut.GetDate[1..4]),
									pDate,'');
...
output(coverage_candidates0,all);
output(coverage_candidates,all);
...
d1 := sort(table(j1_filt_a,final_stat, state_origin,vendor,date_type),state_origin,vendor,date_type);
d2 := sort(table(j1_filt_b,final_stat2,state_origin,vendor,date_type),state_origin,vendor,date_type);
...
export coverage := output(j_final,named('marriage_divorce_coverage'));
import std,address,bair,ut,DID_Add;

EXPORT Update_Base (string filedate, boolean pUseProd = false, boolean pUseDelta = false) := function

dp:=Bair.files(,true).DataProviderLoc_base.built;
r:=layouts.Base;
dd:=
	Standardize_input(filedate,pUseProd,pUseDelta).cfs
	+ Standardize_input(filedate,pUseProd,pUseDelta).mo
	+ Standardize_input(filedate,pUseProd,pUseDelta).pers
	+ Standardize_input(filedate,pUseProd,pUseDelta).veh
	+ Standardize_input(filedate,pUseProd,pUseDelta).offen
	+ Standardize_input(filedate,pUseProd,pUseDelta).crash
	+ Standardize_input(filedate,pUseProd,pUseDelta).lpr
	;

ddd:=dedup(sort(dd,record,except Prepped_rec_type),record,except Prepped_rec_type)
			;

	SHARED fn_scale_eid(string23 eid) := function
...
d:=join(ddd, dp
					,left.data_provider_id = right.dataProviderID
						,transform(r
							,self.eid_hash:=fn_scale_eid(trim(left.eid))
...
names0:=table(j(Prepped_name<>''),{Prepped_name,name_hint,title,fname,mname,lname,name_suffix,clean_company_name},Prepped_name,few);

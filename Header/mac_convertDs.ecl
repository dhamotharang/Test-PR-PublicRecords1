EXPORT mac_convertDs := MODULE
  ...
  EXPORT IngestStattoHTML(raw_data, col1='td1',col2='td2',col3='td3',col4='td4',col5='td5',col6='td6',col7='td7',col8='td8',col9='td9',col10='td10',col11='td11',col12='td12',skipParentTages = false) := FUNCTIONMACRO
    IMPORT std;
    pre_p := record
        string html := '';
        string td1 := '';
    ...
    pre toHtmlColumns(recordof(p_raw_data) L) := TRANSFORM
            ...
            SELF.td7 := (string)L.col7;
            SELF.td8 := (string)L.col8;
            SELF.td9 := (string)L.col9;
            SELF.td10 := (string)L.col10;
            SELF.td11 := (string)L.col11;
            SELF.td12 := (string)L.col12;
            SELF := L;
    ...
    pre toHtmlHeading(recordof(p_raw_data) L) := TRANSFORM
            ...
            SELF.td7 := regexreplace('[<>]',regexfind('[^/][^>]*>',trim(ps[7]),0),'');
            SELF.td8 := regexreplace('[<>]',regexfind('[^/][^>]*>',trim(ps[8]),0),'');
            SELF.td9 := regexreplace('[<>]',regexfind('[^/][^>]*>',trim(ps[9]),0),'');
            SELF.td10 := regexreplace('[<>]',regexfind('[^/][^>]*>',trim(ps[10]),0),'');
            SELF.td11 := regexreplace('[<>]',regexfind('[^/][^>]*>',trim(ps[11]),0),'');
            SELF.td12 := regexreplace('[<>]',regexfind('[^/][^>]*>',trim(ps[12]),0),'');
            SELF := L;
    
    END;
    
    pre_r := 	project(p_raw_data[1],toHtmlHeading(LEFT))+ project(p_raw_data,toHtmlColumns(LEFT));    

...
*/
import header, ut, mdr, watchdog,std;

export fn_bestlname(dataset(recordof(header.layout_header)) in_hdr) := function
integer8 monthsever(string6 strdate) :=
	((integer)(strdate[1..4])) * 12 + 
	((integer)(strdate[5..6]));

f := in_hdr(~(lname='INVEST' and src in mdr.sourceTools.set_Utility_sources));

watchdog.Mac_Bucket_Dates (f, dt_last_seen, ,dt_vendor_last_reported, ,3, date_bucket, date_bucket_f)
...
BestLastName_sub1 := dup_lname;// : persist('~thor_data400::persist::Watchdog_BestLastName_sub1');

return BestLastName_sub1;

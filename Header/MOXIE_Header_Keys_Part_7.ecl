...
#workunit ('name', 'Build header key 7');
...
Key7a	:= BUILDINDEX(City_dedup,{st,city_from_zip,lfmname,dob,(big_endian unsigned8 )__filepos},header.base_key_name + 'st.city.lfmname.dob.key',moxie,overwrite);
Key7b	:= BUILDINDEX(City_dedup,{st,city_from_zip,dob,fname,(big_endian unsigned8 )__filepos},header.base_key_name + 'st.city.dob.fname.key',moxie,overwrite);
Key7c	:= BUILDINDEX(n_dedup,{st,city_from_zip,dph_lname,name_first,name_middle,lname,dob,(big_endian unsigned8 )__filepos},
			header.base_key_name + 'st.city.dph_lname.name_first.name_middle.name_last.dob.key',moxie,overwrite);

export MOXIE_Header_Keys_Part_7
 :=
  parallel(
			   Key7a
			  ,Key7b

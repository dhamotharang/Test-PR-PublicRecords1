import header, address, ut, doxie, header_quick, AID, nid;

dds := header.file_header_in(true).eq_uid_weekly;
weekly_data := when(dataset('~thor_data400::headerbuild_eq_src_weekly',{dds},flat),output(choosen(dds,1)),BEFORE);
...
dHeadersInNameCleaned := project(weekly_data,tCleanNames(left));
...
dNormalizedAddresses := dNormalizedAddresses2(nametype='P');// : persist('~thor_data400::persist::header_preprocess_weekly_name_clean');

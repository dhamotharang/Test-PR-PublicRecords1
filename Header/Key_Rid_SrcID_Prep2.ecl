...
export Key_Rid_SrcID_prep2(boolean pFastHeader=false) := function
	...
	get_recs := get_recs0 + if(~pFastHeader,header.file_transunion_rid_srcid + header.file_tn_rid_srcid);

	return index(get_recs, {rid}, {get_recs},data_services.Data_location.prefix('Source')+'thor_data400::key::header_rid_srid'+SF_suffix(pFastHeader)+'_' + doxie.Version_SuperKey,opt);

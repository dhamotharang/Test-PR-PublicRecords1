EXPORT MAC_Append_addr_ind_Thor(ds_in, addr_ind, src = '', did, prim_range, prim_name, sec_range = '', 
...
#UNIQUENAME(qh);
#SET(finalOutput,%'qh'% + ' := DISTRIBUTE(PULL(');

#IF(isFCRA = TRUE) 
	#APPEND(finalOutput,'header_quick.Key_Did_FCRA),did);\n');
#ELSE 
	#APPEND(finalOutput,'header_quick.key_did),did);\n');
#END
...
#APPEND(finalOutput,%'bh_matches'% + ' := join(' + %'roll_bh'% + ',' + %'get_ua_thor'% + ',\n\tleft.' + #TEXT(did) + ' = right.did AND\n\tleft.' + #TEXT(prim_range) + ' = right.prim_range AND\n\tleft.' + #TEXT(prim_name) + ' = right.prim_name AND\n\t' + %'cleanApt'% + '(left.' + %'sec_rangeName'% + ') = ' + %'cleanApt'% + '(right.sec_range) AND\n\tleft.' + #TEXT(zip) + ' = right.zip,\n\ttransform(' + %'newrec'% + ',\n\t\tself.' + #TEXT(addr_ind) + ' := right.addr_ind,\n\t\tself.apt_cnt := right.apt_cnt,\n\t\tself.best_addr_ind := [],\n\t\tself := left),local);\n');

#IF(Debug = TRUE)
	#APPEND(finalOutput,'OUTPUT(SORT(' + %'bh_matches'% + ',' + #TEXT(did) + ',(unsigned)' + #TEXT(addr_ind) + ',(unsigned)best_addr_rank,local),named(\'bh_matches\'));\n');
#END
												
//QH has a blank sec range while unique Address Key does not
#UNIQUENAME(bh_blanksec);
#APPEND(finalOutput,%'bh_blanksec'% + ' := join(' + %'roll_bh'% + ',' + %'get_ua_thor'% + ',\n\tleft.' + #TEXT(did) + ' = right.did AND\n\tleft.' + #TEXT(prim_range) + ' = right.prim_range AND\n\tleft.' + #TEXT(prim_name) + ' = right.prim_name AND\n\tright.sec_range <> \'\' AND\n\tleft.' + %'sec_rangeName'% + ' = \'\' AND\n\tleft.' + #TEXT(zip) + ' = right.zip,\n\ttransform(' + %'newrec'% + ',\n\t\tself.' + #TEXT(addr_ind) + ' := right.addr_ind,\n\t\tself.apt_cnt := right.apt_cnt,\n\t\tself.best_addr_ind := [],\n\t\tself := left),local);\n');

#IF(Debug = TRUE)
	#APPEND(finalOutput,'OUTPUT(SORT(' + %'bh_blanksec'% + ',' + #TEXT(did) + ',(unsigned)' + #TEXT(addr_ind) + ',(unsigned)best_addr_rank,local),named(\'bh_blanksec\'));\n');
#END

//QH has a sec range, but the sec range is blank for the Unique Address Key
#UNIQUENAME(ua_blankrec);
#APPEND(finalOutput,%'ua_blankrec'% + ' := dedup(sort(' + %'get_ua_thor'% + ',did,(integer)addr_ind,-sec_range,local),did,addr_ind,local);\n');

#UNIQUENAME(bh_nonblanksec);
#APPEND(finalOutput,%'bh_nonblanksec'% + ' := join(' + %'roll_bh'% + ',' + %'ua_blankrec'% + ',\n\tleft.' + #TEXT(did) + ' = right.did AND\n\tleft.' + #TEXT(prim_range) + ' = right.prim_range AND\n\tleft.' + #TEXT(prim_name) + ' = right.prim_name AND\n\tright.sec_range = \'\' AND\n\tleft.' + %'sec_rangeName'% + ' <> \'\' AND\n\tleft.' + #TEXT(zip) + ' = right.zip,\n\ttransform(' + %'newrec'% + ',\n\t\tself.' + #TEXT(addr_ind) + ' := right.addr_ind,\n\t\tself.apt_cnt := right.apt_cnt,\n\t\tself.best_addr_ind  := [],\n\t\tself := left),local);\n');

#IF(Debug = TRUE)
	#APPEND(finalOutput,'OUTPUT(SORT(' + %'bh_nonblanksec'% + ',' + #TEXT(did) + ',(unsigned)' + #TEXT(addr_ind) + ',(unsigned)best_addr_rank,local),named(\'bh_nonblanksec\'));\n');
#END

#UNIQUENAME(bh_adddates);
#APPEND(finalOutput,%'bh_adddates'% + ' := join(' + %'bh_matches'% + ' + ' + %'bh_blanksec'% + ' + ' + %'bh_nonblanksec'% + ',' + %'get_ua_thor'% + ',\n\tleft.' + #TEXT(did) + ' = right.did AND left.' + #TEXT(addr_ind) + ' = right.addr_ind,\n\ttransform(' + %'newrec'% + ',\n\t\tself.dt_first_seen_addr := ' + %'mindate'% + '(left.dt_first_seen_addr,right.dt_first_seen),\n\t\tself.dt_last_seen_addr := max(left.dt_last_seen_addr,right.dt_last_seen),\n\t\tself.dt_vendor_first_reported_addr := ' + %'mindate'% + '(left.dt_vendor_first_reported_addr,right.dt_vendor_first_reported),\n\t\tself.dt_vendor_last_reported_addr := max(left.dt_vendor_last_reported_addr,right.dt_vendor_last_reported),\n\t\tself := left),local);\n');

#IF(Debug = TRUE)
	#APPEND(finalOutput,'OUTPUT(SORT(' + %'bh_adddates'% + ',' + #TEXT(did) +',(unsigned)' + #TEXT(addr_ind) + ',(unsigned)best_addr_rank,local),named(\'bh_adddates\'));\n');
#END

#UNIQUENAME(bh_matches_all);
#APPEND(finalOutput,%'bh_matches_all'% + ' := dedup(sort(' + %'bh_adddates'% + ',' + #TEXT(did) + ',' + #TEXT(prim_range) + ',' + #TEXT(prim_name) + ',' + %'sec_rangeName'% + ',' + %'stName'% + ',(integer)' + #TEXT(addr_ind) + ',local),' + #TEXT(did) + ',' + #TEXT(prim_range) + ',' + #TEXT(prim_name) + ',' + %'sec_rangeName'% + ',' + %'stName'% + ',local);\n');

#IF(Debug = TRUE)
	#APPEND(finalOutput,'OUTPUT(SORT(' + %'bh_matches_all'% + ',' + #TEXT(did) +',(unsigned)' + #TEXT(addr_ind) + ',(unsigned)best_addr_rank,local),named(\'bh_matches_all\'));\n');
#END
...
#APPEND(finalOutput,%'bh_mismatches_dated'% + ' := join(' + %'bh_mismatches_nbrd'% + ',' + %'bh_mismatches_rolled'% + ',\n\tleft.' + #TEXT(did) + ' = right.' + #TEXT(did) + ' AND left.' + #TEXT(addr_ind) + ' = right.' + #TEXT(addr_ind) + ',\n\ttransform(' + %'newrec'% + ',\n\t\tself.dt_first_seen_addr := right.dt_first_seen_addr,\n\t\tself.dt_last_seen_addr  := right.dt_last_seen_addr,\n\t\tself.dt_vendor_first_reported_addr := right.dt_vendor_first_reported_addr,\n\t\tself.dt_vendor_last_reported_addr  := right.dt_vendor_last_reported_addr,\n\t\tself.' + #TEXT(addr_ind) + ' := left.addr_ind_new; self := left),local);\n');

#IF(Debug = TRUE)
	#APPEND(finalOutput,'OUTPUT(SORT(' + %'bh_mismatches_dated'% + ',' + #TEXT(did) + ',(unsigned)' + #TEXT(addr_ind) + ',(unsigned)best_addr_rank,local),named(\'bh_mismatches_dated\'));\n');
#END

#UNIQUENAME(bh_mismatches_joined)
#APPEND(finalOutput,%'bh_mismatches_joined'% + ' := join(' + %'get_ua_thor'% + '((unsigned)addr_ind < 91),' + %'bh_mismatches_dated'% + '(dt_last_seen_addr > 0),\n\tLEFT.' + #TEXT(did) + ' = RIGHT.' + #TEXT(did) + ' AND RIGHT.dt_last_seen_addr >= LEFT.dt_last_seen,\n\tTRANSFORM(' + %'bh_join_rec'% + ',\n\t\tself.addr_ind_new := left.' + #TEXT(addr_ind) + ';\n\t\tself.dt_first_seen_ua := left.dt_first_seen;\n\t\tself.dt_last_seen_ua := left.dt_last_seen;\n\t\tself:= right),local);\n');

#IF(Debug = TRUE)
	#APPEND(finalOutput,'OUTPUT(SORT(' + %'bh_mismatches_joined'% + ',' + #TEXT(did) + ',(unsigned)' + #TEXT(addr_ind) + ',(unsigned)best_addr_rank,local),named(\'bh_mismatches_joined\'));\n');
#END

#UNIQUENAME(bh_mismatches_ordered)
#APPEND(finalOutput,%'bh_mismatches_ordered'% + ' := dedup(sort(' + %'bh_mismatches_joined'% + '(dt_last_seen_ua > 0),' + #TEXT(did) + ',(integer)' + #TEXT(addr_ind) + ',(integer)addr_ind_new,-dt_last_seen_addr,-dt_first_seen_addr,local),' + #TEXT(did) + ',' + #TEXT(addr_ind) + ',local);\n');
...
#APPEND(finalOutput,%'bh_mismatches_unordered'% + ' := dedup(sort(join(' + %'bh_mismatches_dated'% + ',' + %'bh_mismatches_ordered'% + ',\n\tleft.' + #TEXT(did) + '=right.' + #TEXT(did) + ' and left.' + #TEXT(addr_ind) + '=right.' + #TEXT(addr_ind) + ',\n\ttransform(' + %'bh_join_rec'% + ',\n\t\tself.addr_ind_new := (string)(90 + (unsigned)left.' + #TEXT(addr_ind) + ');\n\t\tself:=left;self:=[]),left only,local),\n\t\t' + #TEXT(did) + ',(integer)' + #TEXT(addr_ind) + ',(integer)addr_ind_new,local),\n\t\t' + #TEXT(did) + ',' + #TEXT(addr_ind) + ',addr_ind_new,local);\n');

#IF(Debug = TRUE)
	#APPEND(finalOutput,'OUTPUT(' + %'bh_mismatches_unordered'% + ',named(\'bh_mismatches_unordered\'));\n');
#END

#UNIQUENAME(bh_reordered)
#APPEND(finalOutput,%'bh_reordered'% + ' := iterate(sort(project(dedup(sort(' + %'get_ua_thor'% + '((unsigned)addr_ind < 91),did,addr_ind,(unsigned)best_addr_rank,local),did,addr_ind,local),\n\tTRANSFORM(' + %'bh_join_rec'% + ',\n\t\tself.' + #TEXT(did) + ' := left.did;\n\t\tself.' + #TEXT(addr_ind) + ' := left.addr_ind;\n\t\tself.' + #TEXT(prim_name) + ' := left.prim_name;\n\t\tself.' + #TEXT(prim_range) + ' := left.prim_range;\n\t\tself.' + %'cityName'% + ' := left.city;\n\t\tself.' + %'stName'% + ' := left.st;\n\t\tself.' + #TEXT(zip) + ' := left.zip;\n\t\tself.addr_ind_new := left.' + #TEXT(addr_ind) + ';\n\t\tself.dt_first_seen_addr := left.dt_first_seen;\n\t\tself.dt_last_seen_addr := left.dt_last_seen;\n\t\tself:=left;self:=[]),local) + ' + %'bh_mismatches_ordered'% + ' + ' + %'bh_mismatches_unordered'% + ',' + #TEXT(did) + ',(integer)addr_ind_new,-isQH,-dt_last_seen_addr,-dt_first_seen_addr,-' + #TEXT(prim_name) + ',-' + #TEXT(prim_range) + ',-' + %'cityName'% + ',-' + %'stName'% + ',-' + #TEXT(zip) + ',local),\n\t\ttransform(' + %'bh_join_rec'% + ',\n\t\t\tself.addr_ind_new := IF(left.' + #TEXT(did) + ' <> right.' + #TEXT(did) + ',\'1\',\n\t\t\t(string)((unsigned)left.addr_ind_new+1));\n\t\t\tself:=right),local);\n');

#IF(Debug = TRUE)
	#APPEND(finalOutput,'OUTPUT(' + %'bh_reordered'% + ',named(\'bh_reordered\'));\n');
#END
...
#IF(Debug = TRUE)
	#APPEND(finalOutput,'OUTPUT(SORT(' + %'bh_mismatches_updated'% + ',' + #TEXT(did) + ',(unsigned)' + #TEXT(addr_ind) + ',(unsigned)best_addr_rank,local),named(\'bh_mismatches_updated\'));\n');
#END
...
#IF(Debug = TRUE)
	#APPEND(finalOutput,'OUTPUT(SORT(' + %'bh_mismatches_full'% + ',' + #TEXT(did) + ',(unsigned)' + #TEXT(addr_ind) + ',(unsigned)best_addr_rank,local),named(\'bh_mismatches_full\'));\n'); 
#END

#UNIQUENAME(bh_matches_updated)
#APPEND(finalOutput,%'bh_matches_updated'% + ' := join(' + %'bh_reordered'% + '(isQH = \'\'),' + %'get_ua'% + '((unsigned)' + #TEXT(addr_ind) + ' < 91) + ' + %'bh_matches_full'% + ',\n\tleft.' + #TEXT(did) + ' = right.' + #TEXT(did) + ' and left.' + #TEXT(addr_ind) + ' = right.' + #TEXT(addr_ind) + ',\n\ttransform(' + %'newrec'% + ',\n\t\tself.' + #TEXT(addr_ind) + ' := left.addr_ind_new,self.best_addr_ind := IF((unsigned)left.addr_ind_new < 91 AND right.best_addr_rank = \'1\',\'B\',\'\'),\n\t\tself:=right),local) + ' + %'bh_matches_full'% + '((unsigned)' + #TEXT(addr_ind) + ' > 90);\n');

#IF(Debug = TRUE)
	#APPEND(finalOutput,'OUTPUT(SORT(' + %'bh_matches_updated'% + ',' + #TEXT(did) + ',(unsigned)' + #TEXT(addr_ind) + ',(unsigned)best_addr_rank,local),named(\'bh_matches_updated\'));\n');   
#END

#UNIQUENAME(bh_all)
#APPEND(finalOutput,%'bh_all'% + ' := ' + %'bh_mismatches_full'% + ' + ' + %'bh_matches_updated'% + ' + ' + %'get_ua'% + '(' + #TEXT(addr_ind) + ' BETWEEN \'91\' AND \'99\');\n'); 

#APPEND(finalOutput,'return SORT(' + %'bh_all'% + ',' + #TEXT(did) + ',(unsigned)' + #TEXT(addr_ind) + ',(unsigned)best_addr_rank,local);');

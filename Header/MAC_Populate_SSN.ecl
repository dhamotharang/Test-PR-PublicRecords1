export MAC_Populate_SSN(inds,outds) :=
MACRO
import watchdog;
...
TRANSFORM
	...
	SELF := le;
END;
%j% := JOIN(inds, watchdog.File_Best, LEFT.did=RIGHT.did, %add_best_ssn%(LEFT,RIGHT), LEFT OUTER, LOCAL);

// For DIDs that have a best ssn
%with_best_ssn% := %j%(best_ssn<>'');
// Propogate best into blanks
%with_more_best_ssn% := PROJECT(%with_best_ssn%, TRANSFORM(%sm_rec%, SELF.ssn := IF(LEFT.ssn='',LEFT.best_ssn,LEFT.ssn), 
																																		 SELF.valid_ssn := IF(LEFT.ssn='','G',LEFT.valid_ssn), SELF := LEFT));
// Dupe bad SSN records with the best SSN
%bad_with_best% := PROJECT(%with_best_ssn%(valid_ssn IN ['F','R','O','U','B']), TRANSFORM(%sm_rec%, SELF.ssn := LEFT.best_ssn, SELF.valid_ssn := 'G', SELF := LEFT));
...
outds := %all_with_best% + %without_best_ssn%;

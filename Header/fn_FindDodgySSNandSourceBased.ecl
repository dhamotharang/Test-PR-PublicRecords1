...
export fn_FindDodgySSNandSourceBased(dataset(recordof(header.layout_header)) in_hdr) := function

header_recs            := watchdog.fn_best_ssn(in_hdr).hdr1;
check_for_ssn_flipping := watchdog.fn_best_ssn(header_recs).check_for_ff;

f_potential_dodgy_1 := dedup(check_for_ssn_flipping(ssn_switch='N' and ssn_belongs_to_relative=false and other_ssn_belongs_to_relative=false and total_times_seen>1 and other_total_times_seen>1),did,all);
f_potential_dodgy_2 := dedup(check_for_ssn_flipping(ssn_switch='Y' and ssn_belongs_to_relative=false and other_ssn_belongs_to_relative=false and total_times_seen>25 and other_total_times_seen>25),did,all);

f_potential_dodgy   := f_potential_dodgy_1+f_potential_dodgy_2;

...
export 
header.Layout_header Tra_Merge_Headers(header.Layout_header l, header.Layout_header r) := transform
  ...
  self.dt_last_seen := 
	Max(Max(if(l.rec_type = '1', l.dt_first_seen, 0),
  ...
  self.name_suffix := if (l.name_suffix = '' or ut.fGetSuffix(l.name_suffix)=r.name_suffix, r.name_suffix, l.name_suffix );
  ...
  self.unit_desig := map(
												l.rawaid=0 and r.rawaid>0 => r.unit_desig
												,l.rawaid>0 and r.rawaid=0 => l.unit_desig
  ...
  self.sec_range := map(
												l.rawaid=0 and r.rawaid>0 => r.sec_range
												,l.rawaid>0 and r.rawaid=0 => l.sec_range
  ...
  self.pflag1 := map(l.pflag1 = '+'  => l.pflag1,
					 l.pflag1 = 'A' and r.pflag1 = '+' => r.pflag1,
					 l.pflag1 = '' and r.pflag1 != 'P' => r.pflag1,
					 l.pflag1 = 'P' => r.pflag1,
					 l.pflag1);
  self.pflag2 := map(l.pflag2 != 'A' and r.pflag2 = 'A' => l.pflag2,
					 ...
					 l.pflag2 = '' => r.pflag2,
					 l.pflag2);
  self.pflag3 := if(mdr.sourcetools.sourceisglb(l.src)=true
										,if((header.isPreGLB(l) and l.ssn = '' and //if adding GLB SSN to preGLB rec, then 'G'
												 ~header.isPreGLB(r) and r.ssn <> '') or
												(header.isPreGLB(r) and r.ssn = '' and 
												 ~header.isPreGLB(l) and l.ssn <> '')
												 ,'G'
												 ,if((header.isPreGLB(l) and l.ssn <> '' and l.pflag3 = '') or 	//if adding preGLB SSN to 'G' rec, blank it out
										...
										,if (l.pflag3 = ''
												,r.pflag3
												,l.pflag3)
										); //else, just bring a non-blank flag accross
  self.jflag2 := if ( l.jflag2 = '', r.jflag2, l.jflag2 );
	self.jflag3 := if ( l.jflag3 = '', r.jflag3, l.jflag3 );

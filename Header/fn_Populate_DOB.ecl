import watchdog;
export fn_Populate_DOB(dataset(layout_matchcandidates) inds,dataset(watchdog.Layout_Best) bestfile) :=
FUNCTION
...
TRANSFORM
	...
	SELF := le;
END;
%j2% := JOIN(inds, bestfile, LEFT.did=RIGHT.did, %add_best_dob%(LEFT,RIGHT), LEFT OUTER, LOCAL);
...
return %all_with_best_dob% + %without_best_dob_prop%;

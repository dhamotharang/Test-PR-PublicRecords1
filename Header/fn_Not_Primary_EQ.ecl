...
export fn_Not_Primary_EQ	(
							dataset(recordof(header.Layout_New_Records)) infile
							)
 :=
 FUNCTION
	...
	eq_type1:=sort(distribute(infile1(did<>0,rec_type='1',IsValidVendor(vendor_id))
	...
	preprec tr3(infile1 l,didAfect r) :=transform
		...
		self.rec_type		:=if(r.hd_rid<>1, l.rec_type, '2');
	...
	return project(outfile,header.Layout_New_Records) + infile(did=0);

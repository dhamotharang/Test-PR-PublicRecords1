...
ids_a app_hh_inf(f le, header.fcra_hhid_table_i ri) := transform
...
jn := join(f,header.fcra_hhid_table_i,left.person1=right.did,app_hh_inf(left,right),hash);
...
lname_equiv take_both(jn le, header.fcra_hhid_table_i ri) := transform
  self.le_hhid := le.hhid;
  self.ri_hhid := ri.hhid_indiv;
  end;

l_e := join(jn,header.fcra_hhid_table_i,left.address_id=right.addr_id and

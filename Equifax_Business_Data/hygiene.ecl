...
EXPORT Summary(SALT311.Str30Type  txt,BOOLEAN Glob=TRUE) := FUNCTION
  SummaryLayout := RECORD
    ...
    populated_EFX_STATE_cnt := COUNT(GROUP,h.EFX_STATE <> (TYPEOF(h.EFX_STATE))'');
    populated_EFX_STATE_pcnt := AVE(GROUP,IF(h.EFX_STATE = (TYPEOF(h.EFX_STATE))'',0,100));
    maxlength_EFX_STATE := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_STATE)));
    avelength_EFX_STATE := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_STATE)),h.EFX_STATE<>(typeof(h.EFX_STATE))'');
    ...
    populated_EFX_ZIPCODE_cnt := COUNT(GROUP,h.EFX_ZIPCODE <> (TYPEOF(h.EFX_ZIPCODE))'');
    populated_EFX_ZIPCODE_pcnt := AVE(GROUP,IF(h.EFX_ZIPCODE = (TYPEOF(h.EFX_ZIPCODE))'',0,100));
    maxlength_EFX_ZIPCODE := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_ZIPCODE)));
    avelength_EFX_ZIPCODE := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_ZIPCODE)),h.EFX_ZIPCODE<>(typeof(h.EFX_ZIPCODE))'');
    populated_EFX_ZIP4_cnt := COUNT(GROUP,h.EFX_ZIP4 <> (TYPEOF(h.EFX_ZIP4))'');
    populated_EFX_ZIP4_pcnt := AVE(GROUP,IF(h.EFX_ZIP4 = (TYPEOF(h.EFX_ZIP4))'',0,100));
    maxlength_EFX_ZIP4 := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_ZIP4)));
    avelength_EFX_ZIP4 := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_ZIP4)),h.EFX_ZIP4<>(typeof(h.EFX_ZIP4))'');
    populated_EFX_LAT_cnt := COUNT(GROUP,h.EFX_LAT <> (TYPEOF(h.EFX_LAT))'');
    populated_EFX_LAT_pcnt := AVE(GROUP,IF(h.EFX_LAT = (TYPEOF(h.EFX_LAT))'',0,100));
    maxlength_EFX_LAT := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_LAT)));
    avelength_EFX_LAT := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_LAT)),h.EFX_LAT<>(typeof(h.EFX_LAT))'');
    populated_EFX_LON_cnt := COUNT(GROUP,h.EFX_LON <> (TYPEOF(h.EFX_LON))'');
    populated_EFX_LON_pcnt := AVE(GROUP,IF(h.EFX_LON = (TYPEOF(h.EFX_LON))'',0,100));
    maxlength_EFX_LON := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_LON)));
    avelength_EFX_LON := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_LON)),h.EFX_LON<>(typeof(h.EFX_LON))'');
    ...
    populated_EFX_CONGRESS_cnt := COUNT(GROUP,h.EFX_CONGRESS <> (TYPEOF(h.EFX_CONGRESS))'');
    populated_EFX_CONGRESS_pcnt := AVE(GROUP,IF(h.EFX_CONGRESS = (TYPEOF(h.EFX_CONGRESS))'',0,100));
    maxlength_EFX_CONGRESS := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_CONGRESS)));
    avelength_EFX_CONGRESS := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_CONGRESS)),h.EFX_CONGRESS<>(typeof(h.EFX_CONGRESS))'');
    ...
    populated_EFX_SECSTAT_cnt := COUNT(GROUP,h.EFX_SECSTAT <> (TYPEOF(h.EFX_SECSTAT))'');
    populated_EFX_SECSTAT_pcnt := AVE(GROUP,IF(h.EFX_SECSTAT = (TYPEOF(h.EFX_SECSTAT))'',0,100));
    maxlength_EFX_SECSTAT := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_SECSTAT)));
    avelength_EFX_SECSTAT := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_SECSTAT)),h.EFX_SECSTAT<>(typeof(h.EFX_SECSTAT))'');
    populated_EFX_STATEC2_cnt := COUNT(GROUP,h.EFX_STATEC2 <> (TYPEOF(h.EFX_STATEC2))'');
    populated_EFX_STATEC2_pcnt := AVE(GROUP,IF(h.EFX_STATEC2 = (TYPEOF(h.EFX_STATEC2))'',0,100));
    maxlength_EFX_STATEC2 := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_STATEC2)));
    avelength_EFX_STATEC2 := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_STATEC2)),h.EFX_STATEC2<>(typeof(h.EFX_STATEC2))'');
    populated_EFX_SECZIP_cnt := COUNT(GROUP,h.EFX_SECZIP <> (TYPEOF(h.EFX_SECZIP))'');
    populated_EFX_SECZIP_pcnt := AVE(GROUP,IF(h.EFX_SECZIP = (TYPEOF(h.EFX_SECZIP))'',0,100));
    maxlength_EFX_SECZIP := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_SECZIP)));
    avelength_EFX_SECZIP := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_SECZIP)),h.EFX_SECZIP<>(typeof(h.EFX_SECZIP))'');
    populated_EFX_SECZIP4_cnt := COUNT(GROUP,h.EFX_SECZIP4 <> (TYPEOF(h.EFX_SECZIP4))'');
    populated_EFX_SECZIP4_pcnt := AVE(GROUP,IF(h.EFX_SECZIP4 = (TYPEOF(h.EFX_SECZIP4))'',0,100));
    maxlength_EFX_SECZIP4 := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_SECZIP4)));
    avelength_EFX_SECZIP4 := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_SECZIP4)),h.EFX_SECZIP4<>(typeof(h.EFX_SECZIP4))'');
    populated_EFX_SECLAT_cnt := COUNT(GROUP,h.EFX_SECLAT <> (TYPEOF(h.EFX_SECLAT))'');
    populated_EFX_SECLAT_pcnt := AVE(GROUP,IF(h.EFX_SECLAT = (TYPEOF(h.EFX_SECLAT))'',0,100));
    maxlength_EFX_SECLAT := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_SECLAT)));
    avelength_EFX_SECLAT := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_SECLAT)),h.EFX_SECLAT<>(typeof(h.EFX_SECLAT))'');
    populated_EFX_SECLON_cnt := COUNT(GROUP,h.EFX_SECLON <> (TYPEOF(h.EFX_SECLON))'');
    populated_EFX_SECLON_pcnt := AVE(GROUP,IF(h.EFX_SECLON = (TYPEOF(h.EFX_SECLON))'',0,100));
    maxlength_EFX_SECLON := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_SECLON)));
    avelength_EFX_SECLON := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_SECLON)),h.EFX_SECLON<>(typeof(h.EFX_SECLON))'');
    ...
    populated_EFX_GOV1057_cnt := COUNT(GROUP,h.EFX_GOV1057 <> (TYPEOF(h.EFX_GOV1057))'');
    populated_EFX_GOV1057_pcnt := AVE(GROUP,IF(h.EFX_GOV1057 = (TYPEOF(h.EFX_GOV1057))'',0,100));
    maxlength_EFX_GOV1057 := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_GOV1057)));
    avelength_EFX_GOV1057 := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_GOV1057)),h.EFX_GOV1057<>(typeof(h.EFX_GOV1057))'');
    ...
    populated_EFX_TX_HUBCERTNUM_cnt := COUNT(GROUP,h.EFX_TX_HUBCERTNUM <> (TYPEOF(h.EFX_TX_HUBCERTNUM))'');
    populated_EFX_TX_HUBCERTNUM_pcnt := AVE(GROUP,IF(h.EFX_TX_HUBCERTNUM = (TYPEOF(h.EFX_TX_HUBCERTNUM))'',0,100));
    maxlength_EFX_TX_HUBCERTNUM := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_TX_HUBCERTNUM)));
    avelength_EFX_TX_HUBCERTNUM := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.EFX_TX_HUBCERTNUM)),h.EFX_TX_HUBCERTNUM<>(typeof(h.EFX_TX_HUBCERTNUM))'');
    ...
    populated_normCompany_Name_cnt := COUNT(GROUP,h.normCompany_Name <> (TYPEOF(h.normCompany_Name))'');
    populated_normCompany_Name_pcnt := AVE(GROUP,IF(h.normCompany_Name = (TYPEOF(h.normCompany_Name))'',0,100));
    maxlength_normCompany_Name := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.normCompany_Name)));
    avelength_normCompany_Name := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.normCompany_Name)),h.normCompany_Name<>(typeof(h.normCompany_Name))'');
    ...
    populated_Norm_Geo_Precision_cnt := COUNT(GROUP,h.Norm_Geo_Precision <> (TYPEOF(h.Norm_Geo_Precision))'');
    populated_Norm_Geo_Precision_pcnt := AVE(GROUP,IF(h.Norm_Geo_Precision = (TYPEOF(h.Norm_Geo_Precision))'',0,100));
    maxlength_Norm_Geo_Precision := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Geo_Precision)));
    avelength_Norm_Geo_Precision := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Geo_Precision)),h.Norm_Geo_Precision<>(typeof(h.Norm_Geo_Precision))'');
    populated_Exploded_Desc_Corporate_Amount_Precision_cnt := COUNT(GROUP,h.Exploded_Desc_Corporate_Amount_Precision <> (TYPEOF(h.Exploded_Desc_Corporate_Amount_Precision))'');
    ...
    maxlength_Exploded_Desc_Corporate_Amount_Precision := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Corporate_Amount_Precision)));
    avelength_Exploded_Desc_Corporate_Amount_Precision := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Corporate_Amount_Precision)),h.Exploded_Desc_Corporate_Amount_Precision<>(typeof(h.Exploded_Desc_Corporate_Amount_Precision))'');
    populated_Exploded_Desc_Location_Amount_Precision_cnt := COUNT(GROUP,h.Exploded_Desc_Location_Amount_Precision <> (TYPEOF(h.Exploded_Desc_Location_Amount_Precision))'');
    ...
    maxlength_Exploded_Desc_Location_Amount_Precision := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Location_Amount_Precision)));
    avelength_Exploded_Desc_Location_Amount_Precision := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Location_Amount_Precision)),h.Exploded_Desc_Location_Amount_Precision<>(typeof(h.Exploded_Desc_Location_Amount_Precision))'');
    populated_Exploded_Desc_Public_Co_Indicator_cnt := COUNT(GROUP,h.Exploded_Desc_Public_Co_Indicator <> (TYPEOF(h.Exploded_Desc_Public_Co_Indicator))'');
    ...
    maxlength_Exploded_Desc_Public_Co_Indicator := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Public_Co_Indicator)));
    avelength_Exploded_Desc_Public_Co_Indicator := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Public_Co_Indicator)),h.Exploded_Desc_Public_Co_Indicator<>(typeof(h.Exploded_Desc_Public_Co_Indicator))'');
    populated_Exploded_Desc_Stock_Exchange_cnt := COUNT(GROUP,h.Exploded_Desc_Stock_Exchange <> (TYPEOF(h.Exploded_Desc_Stock_Exchange))'');
    ...
    maxlength_Exploded_Desc_Stock_Exchange := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Stock_Exchange)));
    avelength_Exploded_Desc_Stock_Exchange := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Stock_Exchange)),h.Exploded_Desc_Stock_Exchange<>(typeof(h.Exploded_Desc_Stock_Exchange))'');
    populated_Exploded_Desc_Telemarketablity_Score_cnt := COUNT(GROUP,h.Exploded_Desc_Telemarketablity_Score <> (TYPEOF(h.Exploded_Desc_Telemarketablity_Score))'');
    ...
    maxlength_Exploded_Desc_Telemarketablity_Score := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Telemarketablity_Score)));
    avelength_Exploded_Desc_Telemarketablity_Score := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Telemarketablity_Score)),h.Exploded_Desc_Telemarketablity_Score<>(typeof(h.Exploded_Desc_Telemarketablity_Score))'');
    populated_Exploded_Desc_Telemarketablity_Total_Indicator_cnt := COUNT(GROUP,h.Exploded_Desc_Telemarketablity_Total_Indicator <> (TYPEOF(h.Exploded_Desc_Telemarketablity_Total_Indicator))'');
    ...
    maxlength_Exploded_Desc_Telemarketablity_Total_Indicator := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Telemarketablity_Total_Indicator)));
    avelength_Exploded_Desc_Telemarketablity_Total_Indicator := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Telemarketablity_Total_Indicator)),h.Exploded_Desc_Telemarketablity_Total_Indicator<>(typeof(h.Exploded_Desc_Telemarketablity_Total_Indicator))'');
    populated_Exploded_Desc_Telemarketablity_Total_Score_cnt := COUNT(GROUP,h.Exploded_Desc_Telemarketablity_Total_Score <> (TYPEOF(h.Exploded_Desc_Telemarketablity_Total_Score))'');
    ...
    maxlength_Exploded_Desc_Telemarketablity_Total_Score := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Telemarketablity_Total_Score)));
    avelength_Exploded_Desc_Telemarketablity_Total_Score := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Telemarketablity_Total_Score)),h.Exploded_Desc_Telemarketablity_Total_Score<>(typeof(h.Exploded_Desc_Telemarketablity_Total_Score))'');
    populated_Exploded_Desc_Government1057_Entity_cnt := COUNT(GROUP,h.Exploded_Desc_Government1057_Entity <> (TYPEOF(h.Exploded_Desc_Government1057_Entity))'');
    ...
    maxlength_Exploded_Desc_Government1057_Entity := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Government1057_Entity)));
    avelength_Exploded_Desc_Government1057_Entity := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Government1057_Entity)),h.Exploded_Desc_Government1057_Entity<>(typeof(h.Exploded_Desc_Government1057_Entity))'');
    populated_Exploded_Desc_Merchant_Type_cnt := COUNT(GROUP,h.Exploded_Desc_Merchant_Type <> (TYPEOF(h.Exploded_Desc_Merchant_Type))'');
    ...
    maxlength_Exploded_Desc_Merchant_Type := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Merchant_Type)));
    avelength_Exploded_Desc_Merchant_Type := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Merchant_Type)),h.Exploded_Desc_Merchant_Type<>(typeof(h.Exploded_Desc_Merchant_Type))'');
    populated_Exploded_Desc_Busstatcd_cnt := COUNT(GROUP,h.Exploded_Desc_Busstatcd <> (TYPEOF(h.Exploded_Desc_Busstatcd))'');
    populated_Exploded_Desc_Busstatcd_pcnt := AVE(GROUP,IF(h.Exploded_Desc_Busstatcd = (TYPEOF(h.Exploded_Desc_Busstatcd))'',0,100));
    maxlength_Exploded_Desc_Busstatcd := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Busstatcd)));
    avelength_Exploded_Desc_Busstatcd := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Busstatcd)),h.Exploded_Desc_Busstatcd<>(typeof(h.Exploded_Desc_Busstatcd))'');
    populated_Exploded_Desc_CMSA_cnt := COUNT(GROUP,h.Exploded_Desc_CMSA <> (TYPEOF(h.Exploded_Desc_CMSA))'');
    populated_Exploded_Desc_CMSA_pcnt := AVE(GROUP,IF(h.Exploded_Desc_CMSA = (TYPEOF(h.Exploded_Desc_CMSA))'',0,100));
    maxlength_Exploded_Desc_CMSA := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_CMSA)));
    avelength_Exploded_Desc_CMSA := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_CMSA)),h.Exploded_Desc_CMSA<>(typeof(h.Exploded_Desc_CMSA))'');
    populated_Exploded_Desc_Corpamountcd_cnt := COUNT(GROUP,h.Exploded_Desc_Corpamountcd <> (TYPEOF(h.Exploded_Desc_Corpamountcd))'');
    ...
    maxlength_Exploded_Desc_Corpamountcd := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Corpamountcd)));
    avelength_Exploded_Desc_Corpamountcd := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Corpamountcd)),h.Exploded_Desc_Corpamountcd<>(typeof(h.Exploded_Desc_Corpamountcd))'');
    populated_Exploded_Desc_Corpamountprec_cnt := COUNT(GROUP,h.Exploded_Desc_Corpamountprec <> (TYPEOF(h.Exploded_Desc_Corpamountprec))'');
    ...
    maxlength_Exploded_Desc_Corpamountprec := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Corpamountprec)));
    avelength_Exploded_Desc_Corpamountprec := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Corpamountprec)),h.Exploded_Desc_Corpamountprec<>(typeof(h.Exploded_Desc_Corpamountprec))'');
    populated_Exploded_Desc_Corpamounttp_cnt := COUNT(GROUP,h.Exploded_Desc_Corpamounttp <> (TYPEOF(h.Exploded_Desc_Corpamounttp))'');
    ...
    maxlength_Exploded_Desc_Corpamounttp := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Corpamounttp)));
    avelength_Exploded_Desc_Corpamounttp := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Corpamounttp)),h.Exploded_Desc_Corpamounttp<>(typeof(h.Exploded_Desc_Corpamounttp))'');
    populated_Exploded_Desc_Corpempcd_cnt := COUNT(GROUP,h.Exploded_Desc_Corpempcd <> (TYPEOF(h.Exploded_Desc_Corpempcd))'');
    populated_Exploded_Desc_Corpempcd_pcnt := AVE(GROUP,IF(h.Exploded_Desc_Corpempcd = (TYPEOF(h.Exploded_Desc_Corpempcd))'',0,100));
    maxlength_Exploded_Desc_Corpempcd := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Corpempcd)));
    avelength_Exploded_Desc_Corpempcd := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Corpempcd)),h.Exploded_Desc_Corpempcd<>(typeof(h.Exploded_Desc_Corpempcd))'');
    populated_Exploded_Desc_Ctrytelcd_cnt := COUNT(GROUP,h.Exploded_Desc_Ctrytelcd <> (TYPEOF(h.Exploded_Desc_Ctrytelcd))'');
    populated_Exploded_Desc_Ctrytelcd_pcnt := AVE(GROUP,IF(h.Exploded_Desc_Ctrytelcd = (TYPEOF(h.Exploded_Desc_Ctrytelcd))'',0,100));
    maxlength_Exploded_Desc_Ctrytelcd := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Ctrytelcd)));
    avelength_Exploded_Desc_Ctrytelcd := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Exploded_Desc_Ctrytelcd)),h.Exploded_Desc_Ctrytelcd<>(typeof(h.Exploded_Desc_Ctrytelcd))'');
    ...
    populated_Norm_Address_cnt := COUNT(GROUP,h.Norm_Address <> (TYPEOF(h.Norm_Address))'');
    populated_Norm_Address_pcnt := AVE(GROUP,IF(h.Norm_Address = (TYPEOF(h.Norm_Address))'',0,100));
    maxlength_Norm_Address := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Address)));
    avelength_Norm_Address := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Address)),h.Norm_Address<>(typeof(h.Norm_Address))'');
    ...
    populated_Norm_StateC2_cnt := COUNT(GROUP,h.Norm_StateC2 <> (TYPEOF(h.Norm_StateC2))'');
    populated_Norm_StateC2_pcnt := AVE(GROUP,IF(h.Norm_StateC2 = (TYPEOF(h.Norm_StateC2))'',0,100));
    maxlength_Norm_StateC2 := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_StateC2)));
    avelength_Norm_StateC2 := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_StateC2)),h.Norm_StateC2<>(typeof(h.Norm_StateC2))'');
    ...
    populated_Norm_Lat_cnt := COUNT(GROUP,h.Norm_Lat <> (TYPEOF(h.Norm_Lat))'');
    populated_Norm_Lat_pcnt := AVE(GROUP,IF(h.Norm_Lat = (TYPEOF(h.Norm_Lat))'',0,100));
    maxlength_Norm_Lat := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Lat)));
    avelength_Norm_Lat := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Lat)),h.Norm_Lat<>(typeof(h.Norm_Lat))'');
    populated_Norm_Lon_cnt := COUNT(GROUP,h.Norm_Lon <> (TYPEOF(h.Norm_Lon))'');
    populated_Norm_Lon_pcnt := AVE(GROUP,IF(h.Norm_Lon = (TYPEOF(h.Norm_Lon))'',0,100));
    maxlength_Norm_Lon := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Lon)));
    avelength_Norm_Lon := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Lon)),h.Norm_Lon<>(typeof(h.Norm_Lon))'');
    populated_Norm_Geoprec_cnt := COUNT(GROUP,h.Norm_Geoprec <> (TYPEOF(h.Norm_Geoprec))'');
    populated_Norm_Geoprec_pcnt := AVE(GROUP,IF(h.Norm_Geoprec = (TYPEOF(h.Norm_Geoprec))'',0,100));
    maxlength_Norm_Geoprec := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Geoprec)));
    avelength_Norm_Geoprec := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Geoprec)),h.Norm_Geoprec<>(typeof(h.Norm_Geoprec))'');
    populated_Norm_Region_cnt := COUNT(GROUP,h.Norm_Region <> (TYPEOF(h.Norm_Region))'');
    populated_Norm_Region_pcnt := AVE(GROUP,IF(h.Norm_Region = (TYPEOF(h.Norm_Region))'',0,100));
    maxlength_Norm_Region := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Region)));
    avelength_Norm_Region := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Region)),h.Norm_Region<>(typeof(h.Norm_Region))'');
    populated_Norm_Ctryisocd_cnt := COUNT(GROUP,h.Norm_Ctryisocd <> (TYPEOF(h.Norm_Ctryisocd))'');
    populated_Norm_Ctryisocd_pcnt := AVE(GROUP,IF(h.Norm_Ctryisocd = (TYPEOF(h.Norm_Ctryisocd))'',0,100));
    maxlength_Norm_Ctryisocd := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Ctryisocd)));
    avelength_Norm_Ctryisocd := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Ctryisocd)),h.Norm_Ctryisocd<>(typeof(h.Norm_Ctryisocd))'');
    populated_Norm_Ctrynum_cnt := COUNT(GROUP,h.Norm_Ctrynum <> (TYPEOF(h.Norm_Ctrynum))'');
    populated_Norm_Ctrynum_pcnt := AVE(GROUP,IF(h.Norm_Ctrynum = (TYPEOF(h.Norm_Ctrynum))'',0,100));
    maxlength_Norm_Ctrynum := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Ctrynum)));
    avelength_Norm_Ctrynum := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Ctrynum)),h.Norm_Ctrynum<>(typeof(h.Norm_Ctrynum))'');
    populated_Norm_Ctryname_cnt := COUNT(GROUP,h.Norm_Ctryname <> (TYPEOF(h.Norm_Ctryname))'');
    populated_Norm_Ctryname_pcnt := AVE(GROUP,IF(h.Norm_Ctryname = (TYPEOF(h.Norm_Ctryname))'',0,100));
    maxlength_Norm_Ctryname := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Ctryname)));
    avelength_Norm_Ctryname := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.Norm_Ctryname)),h.Norm_Ctryname<>(typeof(h.Norm_Ctryname))'');
...
SALT311.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  ...
  END;
SHARED Pairs0 := NORMALIZE(ENTH(h,Config.CorrelateSampleSize),260*260,IntoP(LEFT,COUNTER))(FldNo1<FldNo2);
SHARED FldIds := DATASET([{1,'dt_first_seen'}
      ...
      ,{211,'AT_CERTEXP10'}
      ,{212,'EFX_FOREIGN'}
      ,{213,'normCompany_Name'}
      ,{214,'normCompany_Type'}
      ,{215,'Norm_Geo_Precision'}
      ,{216,'Exploded_Desc_Corporate_Amount_Precision'}
      ,{217,'Exploded_Desc_Location_Amount_Precision'}
      ,{218,'Exploded_Desc_Public_Co_Indicator'}
      ,{219,'Exploded_Desc_Stock_Exchange'}
      ,{220,'Exploded_Desc_Telemarketablity_Score'}
      ,{221,'Exploded_Desc_Telemarketablity_Total_Indicator'}
      ,{222,'Exploded_Desc_Telemarketablity_Total_Score'}
      ,{223,'Exploded_Desc_Government1057_Entity'}
      ,{224,'Exploded_Desc_Merchant_Type'}
      ,{225,'Exploded_Desc_Busstatcd'}
      ,{226,'Exploded_Desc_CMSA'}
      ,{227,'Exploded_Desc_Corpamountcd'}
      ,{228,'Exploded_Desc_Corpamountprec'}
      ,{229,'Exploded_Desc_Corpamounttp'}
      ,{230,'Exploded_Desc_Corpempcd'}
      ,{231,'Exploded_Desc_Ctrytelcd'}
      ,{232,'NormAddress_Type'}
      ,{233,'Norm_Address'}
      ,{234,'Norm_City'}
      ,{235,'Norm_State'}
      ,{236,'Norm_StateC2'}
      ...
      ,{244,'Norm_Ctrynum'}
      ,{245,'Norm_Ctryname'}
      ,{246,'clean_company_name'}
      ,{247,'clean_phone'}
      ,{248,'clean_secondary_phone'}
      ,{249,'prim_range'}
      ,{250,'predir'}
      ,{251,'prim_name'}
      ,{252,'addr_suffix'}
      ...
      ,{258,'st'}
      ,{259,'cart'}
      ,{260,'cr_sort_sz'}],SALT311.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED2 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_EFX_ID((SALT311.StrType)le.EFX_ID),
    Fields.InValid_EFX_NAME((SALT311.StrType)le.EFX_NAME),
    Fields.InValid_EFX_LEGAL_NAME((SALT311.StrType)le.EFX_LEGAL_NAME),
    Fields.InValid_EFX_ADDRESS((SALT311.StrType)le.EFX_ADDRESS),
    Fields.InValid_EFX_CITY((SALT311.StrType)le.EFX_CITY),
    Fields.InValid_EFX_STATE((SALT311.StrType)le.EFX_STATE),
    Fields.InValid_EFX_STATEC((SALT311.StrType)le.EFX_STATEC),
    Fields.InValid_EFX_ZIPCODE((SALT311.StrType)le.EFX_ZIPCODE),
    Fields.InValid_EFX_ZIP4((SALT311.StrType)le.EFX_ZIP4),
    Fields.InValid_EFX_LAT((SALT311.StrType)le.EFX_LAT),
    Fields.InValid_EFX_LON((SALT311.StrType)le.EFX_LON),
    Fields.InValid_EFX_GEOPREC((SALT311.StrType)le.EFX_GEOPREC),
    Fields.InValid_EFX_REGION((SALT311.StrType)le.EFX_REGION),
    Fields.InValid_EFX_CTRYISOCD((SALT311.StrType)le.EFX_CTRYISOCD),
    Fields.InValid_EFX_CTRYNUM((SALT311.StrType)le.EFX_CTRYNUM),
    Fields.InValid_EFX_CTRYNAME((SALT311.StrType)le.EFX_CTRYNAME),
    Fields.InValid_EFX_COUNTYNM((SALT311.StrType)le.EFX_COUNTYNM),
    Fields.InValid_EFX_COUNTY((SALT311.StrType)le.EFX_COUNTY),
    Fields.InValid_EFX_CMSA((SALT311.StrType)le.EFX_CMSA),
    Fields.InValid_EFX_CMSADESC((SALT311.StrType)le.EFX_CMSADESC),
    Fields.InValid_EFX_SOHO((SALT311.StrType)le.EFX_SOHO),
    Fields.InValid_EFX_BIZ((SALT311.StrType)le.EFX_BIZ),
    Fields.InValid_EFX_RES((SALT311.StrType)le.EFX_RES),
    Fields.InValid_EFX_CMRA((SALT311.StrType)le.EFX_CMRA),
    Fields.InValid_EFX_CONGRESS((SALT311.StrType)le.EFX_CONGRESS),
    Fields.InValid_EFX_SECADR((SALT311.StrType)le.EFX_SECADR),
    Fields.InValid_EFX_SECCTY((SALT311.StrType)le.EFX_SECCTY),
    Fields.InValid_EFX_SECSTAT((SALT311.StrType)le.EFX_SECSTAT),
    Fields.InValid_EFX_STATEC2((SALT311.StrType)le.EFX_STATEC2),
    Fields.InValid_EFX_SECZIP((SALT311.StrType)le.EFX_SECZIP),
    Fields.InValid_EFX_SECZIP4((SALT311.StrType)le.EFX_SECZIP4),
    Fields.InValid_EFX_SECLAT((SALT311.StrType)le.EFX_SECLAT),
    Fields.InValid_EFX_SECLON((SALT311.StrType)le.EFX_SECLON),
    Fields.InValid_EFX_SECGEOPREC((SALT311.StrType)le.EFX_SECGEOPREC),
    Fields.InValid_EFX_SECREGION((SALT311.StrType)le.EFX_SECREGION),
    Fields.InValid_EFX_SECCTRYISOCD((SALT311.StrType)le.EFX_SECCTRYISOCD),
    Fields.InValid_EFX_SECCTRYNUM((SALT311.StrType)le.EFX_SECCTRYNUM),
    Fields.InValid_EFX_SECCTRYNAME((SALT311.StrType)le.EFX_SECCTRYNAME),
    Fields.InValid_EFX_CTRYTELCD((SALT311.StrType)le.EFX_CTRYTELCD),
    Fields.InValid_EFX_PHONE((SALT311.StrType)le.EFX_PHONE),
    Fields.InValid_EFX_FAXPHONE((SALT311.StrType)le.EFX_FAXPHONE),
    Fields.InValid_EFX_BUSSTAT((SALT311.StrType)le.EFX_BUSSTAT),
    Fields.InValid_EFX_BUSSTATCD((SALT311.StrType)le.EFX_BUSSTATCD),
    Fields.InValid_EFX_WEB((SALT311.StrType)le.EFX_WEB),
    Fields.InValid_EFX_YREST((SALT311.StrType)le.EFX_YREST),
    Fields.InValid_EFX_CORPEMPCNT((SALT311.StrType)le.EFX_CORPEMPCNT),
    Fields.InValid_EFX_LOCEMPCNT((SALT311.StrType)le.EFX_LOCEMPCNT),
    Fields.InValid_EFX_CORPEMPCD((SALT311.StrType)le.EFX_CORPEMPCD),
    Fields.InValid_EFX_LOCEMPCD((SALT311.StrType)le.EFX_LOCEMPCD),
    Fields.InValid_EFX_CORPAMOUNT((SALT311.StrType)le.EFX_CORPAMOUNT),
    Fields.InValid_EFX_CORPAMOUNTCD((SALT311.StrType)le.EFX_CORPAMOUNTCD),
    Fields.InValid_EFX_CORPAMOUNTTP((SALT311.StrType)le.EFX_CORPAMOUNTTP),
    Fields.InValid_EFX_CORPAMOUNTPREC((SALT311.StrType)le.EFX_CORPAMOUNTPREC),
    Fields.InValid_EFX_LOCAMOUNT((SALT311.StrType)le.EFX_LOCAMOUNT),
    Fields.InValid_EFX_LOCAMOUNTCD((SALT311.StrType)le.EFX_LOCAMOUNTCD),
    Fields.InValid_EFX_LOCAMOUNTTP((SALT311.StrType)le.EFX_LOCAMOUNTTP),
    Fields.InValid_EFX_LOCAMOUNTPREC((SALT311.StrType)le.EFX_LOCAMOUNTPREC),
    Fields.InValid_EFX_PUBLIC((SALT311.StrType)le.EFX_PUBLIC),
    Fields.InValid_EFX_STKEXC((SALT311.StrType)le.EFX_STKEXC),
    Fields.InValid_EFX_TCKSYM((SALT311.StrType)le.EFX_TCKSYM),
    Fields.InValid_EFX_PRIMSIC((SALT311.StrType)le.EFX_PRIMSIC),
    Fields.InValid_EFX_SECSIC1((SALT311.StrType)le.EFX_SECSIC1),
    Fields.InValid_EFX_SECSIC2((SALT311.StrType)le.EFX_SECSIC2),
    Fields.InValid_EFX_SECSIC3((SALT311.StrType)le.EFX_SECSIC3),
    Fields.InValid_EFX_SECSIC4((SALT311.StrType)le.EFX_SECSIC4),
    Fields.InValid_EFX_PRIMSICDESC((SALT311.StrType)le.EFX_PRIMSICDESC),
    Fields.InValid_EFX_SECSICDESC1((SALT311.StrType)le.EFX_SECSICDESC1),
    Fields.InValid_EFX_SECSICDESC2((SALT311.StrType)le.EFX_SECSICDESC2),
    Fields.InValid_EFX_SECSICDESC3((SALT311.StrType)le.EFX_SECSICDESC3),
    Fields.InValid_EFX_SECSICDESC4((SALT311.StrType)le.EFX_SECSICDESC4),
    Fields.InValid_EFX_PRIMNAICSCODE((SALT311.StrType)le.EFX_PRIMNAICSCODE),
    Fields.InValid_EFX_SECNAICS1((SALT311.StrType)le.EFX_SECNAICS1),
    Fields.InValid_EFX_SECNAICS2((SALT311.StrType)le.EFX_SECNAICS2),
    Fields.InValid_EFX_SECNAICS3((SALT311.StrType)le.EFX_SECNAICS3),
    Fields.InValid_EFX_SECNAICS4((SALT311.StrType)le.EFX_SECNAICS4),
    Fields.InValid_EFX_PRIMNAICSDESC((SALT311.StrType)le.EFX_PRIMNAICSDESC),
    Fields.InValid_EFX_SECNAICSDESC1((SALT311.StrType)le.EFX_SECNAICSDESC1),
    Fields.InValid_EFX_SECNAICSDESC2((SALT311.StrType)le.EFX_SECNAICSDESC2),
    Fields.InValid_EFX_SECNAICSDESC3((SALT311.StrType)le.EFX_SECNAICSDESC3),
    Fields.InValid_EFX_SECNAICSDESC4((SALT311.StrType)le.EFX_SECNAICSDESC4),
    Fields.InValid_EFX_DEAD((SALT311.StrType)le.EFX_DEAD),
    Fields.InValid_EFX_DEADDT((SALT311.StrType)le.EFX_DEADDT),
    Fields.InValid_EFX_MRKT_TELEVER((SALT311.StrType)le.EFX_MRKT_TELEVER),
    Fields.InValid_EFX_MRKT_TELESCORE((SALT311.StrType)le.EFX_MRKT_TELESCORE),
    Fields.InValid_EFX_MRKT_TOTALSCORE((SALT311.StrType)le.EFX_MRKT_TOTALSCORE),
    Fields.InValid_EFX_MRKT_TOTALIND((SALT311.StrType)le.EFX_MRKT_TOTALIND),
    Fields.InValid_EFX_MRKT_VACANT((SALT311.StrType)le.EFX_MRKT_VACANT),
    Fields.InValid_EFX_MRKT_SEASONAL((SALT311.StrType)le.EFX_MRKT_SEASONAL),
    Fields.InValid_EFX_MBE((SALT311.StrType)le.EFX_MBE),
    Fields.InValid_EFX_WBE((SALT311.StrType)le.EFX_WBE),
    Fields.InValid_EFX_MWBE((SALT311.StrType)le.EFX_MWBE),
    Fields.InValid_EFX_SDB((SALT311.StrType)le.EFX_SDB),
    Fields.InValid_EFX_HUBZONE((SALT311.StrType)le.EFX_HUBZONE),
    Fields.InValid_EFX_DBE((SALT311.StrType)le.EFX_DBE),
    Fields.InValid_EFX_VET((SALT311.StrType)le.EFX_VET),
    Fields.InValid_EFX_DVET((SALT311.StrType)le.EFX_DVET),
    Fields.InValid_EFX_8a((SALT311.StrType)le.EFX_8a),
    Fields.InValid_EFX_8aEXPDT((SALT311.StrType)le.EFX_8aEXPDT),
    Fields.InValid_EFX_DIS((SALT311.StrType)le.EFX_DIS),
    Fields.InValid_EFX_SBE((SALT311.StrType)le.EFX_SBE),
    Fields.InValid_EFX_BUSSIZE((SALT311.StrType)le.EFX_BUSSIZE),
    Fields.InValid_EFX_LBE((SALT311.StrType)le.EFX_LBE),
    Fields.InValid_EFX_GOV((SALT311.StrType)le.EFX_GOV),
    Fields.InValid_EFX_FGOV((SALT311.StrType)le.EFX_FGOV),
    Fields.InValid_EFX_GOV1057((SALT311.StrType)le.EFX_GOV1057),
    Fields.InValid_EFX_NONPROFIT((SALT311.StrType)le.EFX_NONPROFIT),
    Fields.InValid_EFX_MERCTYPE((SALT311.StrType)le.EFX_MERCTYPE),
    Fields.InValid_EFX_HBCU((SALT311.StrType)le.EFX_HBCU),
    Fields.InValid_EFX_GAYLESBIAN((SALT311.StrType)le.EFX_GAYLESBIAN),
    Fields.InValid_EFX_WSBE((SALT311.StrType)le.EFX_WSBE),
    Fields.InValid_EFX_VSBE((SALT311.StrType)le.EFX_VSBE),
    Fields.InValid_EFX_DVSBE((SALT311.StrType)le.EFX_DVSBE),
    Fields.InValid_EFX_MWBESTATUS((SALT311.StrType)le.EFX_MWBESTATUS),
    Fields.InValid_EFX_NMSDC((SALT311.StrType)le.EFX_NMSDC),
    Fields.InValid_EFX_WBENC((SALT311.StrType)le.EFX_WBENC),
    Fields.InValid_EFX_CA_PUC((SALT311.StrType)le.EFX_CA_PUC),
    Fields.InValid_EFX_TX_HUB((SALT311.StrType)le.EFX_TX_HUB),
    Fields.InValid_EFX_TX_HUBCERTNUM((SALT311.StrType)le.EFX_TX_HUBCERTNUM),
    Fields.InValid_EFX_GSAX((SALT311.StrType)le.EFX_GSAX),
    Fields.InValid_EFX_CALTRANS((SALT311.StrType)le.EFX_CALTRANS),
    Fields.InValid_EFX_EDU((SALT311.StrType)le.EFX_EDU),
    Fields.InValid_EFX_MI((SALT311.StrType)le.EFX_MI),
    Fields.InValid_EFX_ANC((SALT311.StrType)le.EFX_ANC),
    Fields.InValid_AT_CERT1((SALT311.StrType)le.AT_CERT1),
    Fields.InValid_AT_CERT2((SALT311.StrType)le.AT_CERT2),
    Fields.InValid_AT_CERT3((SALT311.StrType)le.AT_CERT3),
    Fields.InValid_AT_CERT4((SALT311.StrType)le.AT_CERT4),
    Fields.InValid_AT_CERT5((SALT311.StrType)le.AT_CERT5),
    Fields.InValid_AT_CERT6((SALT311.StrType)le.AT_CERT6),
    Fields.InValid_AT_CERT7((SALT311.StrType)le.AT_CERT7),
    Fields.InValid_AT_CERT8((SALT311.StrType)le.AT_CERT8),
    Fields.InValid_AT_CERT9((SALT311.StrType)le.AT_CERT9),
    Fields.InValid_AT_CERT10((SALT311.StrType)le.AT_CERT10),
    Fields.InValid_AT_CERTDESC1((SALT311.StrType)le.AT_CERTDESC1),
    Fields.InValid_AT_CERTDESC2((SALT311.StrType)le.AT_CERTDESC2),
    Fields.InValid_AT_CERTDESC3((SALT311.StrType)le.AT_CERTDESC3),
    Fields.InValid_AT_CERTDESC4((SALT311.StrType)le.AT_CERTDESC4),
    Fields.InValid_AT_CERTDESC5((SALT311.StrType)le.AT_CERTDESC5),
    Fields.InValid_AT_CERTDESC6((SALT311.StrType)le.AT_CERTDESC6),
    Fields.InValid_AT_CERTDESC7((SALT311.StrType)le.AT_CERTDESC7),
    Fields.InValid_AT_CERTDESC8((SALT311.StrType)le.AT_CERTDESC8),
    Fields.InValid_AT_CERTDESC9((SALT311.StrType)le.AT_CERTDESC9),
    Fields.InValid_AT_CERTDESC10((SALT311.StrType)le.AT_CERTDESC10),
    Fields.InValid_AT_CERTSRC1((SALT311.StrType)le.AT_CERTSRC1),
    Fields.InValid_AT_CERTSRC2((SALT311.StrType)le.AT_CERTSRC2),
    Fields.InValid_AT_CERTSRC3((SALT311.StrType)le.AT_CERTSRC3),
    Fields.InValid_AT_CERTSRC4((SALT311.StrType)le.AT_CERTSRC4),
    Fields.InValid_AT_CERTSRC5((SALT311.StrType)le.AT_CERTSRC5),
    Fields.InValid_AT_CERTSRC6((SALT311.StrType)le.AT_CERTSRC6),
    Fields.InValid_AT_CERTSRC7((SALT311.StrType)le.AT_CERTSRC7),
    Fields.InValid_AT_CERTSRC8((SALT311.StrType)le.AT_CERTSRC8),
    Fields.InValid_AT_CERTSRC9((SALT311.StrType)le.AT_CERTSRC9),
    Fields.InValid_AT_CERTSRC10((SALT311.StrType)le.AT_CERTSRC10),
    Fields.InValid_AT_CERTLEV1((SALT311.StrType)le.AT_CERTLEV1),
    Fields.InValid_AT_CERTLEV2((SALT311.StrType)le.AT_CERTLEV2),
    Fields.InValid_AT_CERTLEV3((SALT311.StrType)le.AT_CERTLEV3),
    Fields.InValid_AT_CERTLEV4((SALT311.StrType)le.AT_CERTLEV4),
    Fields.InValid_AT_CERTLEV5((SALT311.StrType)le.AT_CERTLEV5),
    Fields.InValid_AT_CERTLEV6((SALT311.StrType)le.AT_CERTLEV6),
    Fields.InValid_AT_CERTLEV7((SALT311.StrType)le.AT_CERTLEV7),
    Fields.InValid_AT_CERTLEV8((SALT311.StrType)le.AT_CERTLEV8),
    Fields.InValid_AT_CERTLEV9((SALT311.StrType)le.AT_CERTLEV9),
    Fields.InValid_AT_CERTLEV10((SALT311.StrType)le.AT_CERTLEV10),
    Fields.InValid_AT_CERTNUM1((SALT311.StrType)le.AT_CERTNUM1),
    Fields.InValid_AT_CERTNUM2((SALT311.StrType)le.AT_CERTNUM2),
    Fields.InValid_AT_CERTNUM3((SALT311.StrType)le.AT_CERTNUM3),
    Fields.InValid_AT_CERTNUM4((SALT311.StrType)le.AT_CERTNUM4),
    Fields.InValid_AT_CERTNUM5((SALT311.StrType)le.AT_CERTNUM5),
    Fields.InValid_AT_CERTNUM6((SALT311.StrType)le.AT_CERTNUM6),
    Fields.InValid_AT_CERTNUM7((SALT311.StrType)le.AT_CERTNUM7),
    Fields.InValid_AT_CERTNUM8((SALT311.StrType)le.AT_CERTNUM8),
    Fields.InValid_AT_CERTNUM9((SALT311.StrType)le.AT_CERTNUM9),
    Fields.InValid_AT_CERTNUM10((SALT311.StrType)le.AT_CERTNUM10),
    Fields.InValid_AT_CERTEXP1((SALT311.StrType)le.AT_CERTEXP1),
    Fields.InValid_AT_CERTEXP2((SALT311.StrType)le.AT_CERTEXP2),
    Fields.InValid_AT_CERTEXP3((SALT311.StrType)le.AT_CERTEXP3),
    Fields.InValid_AT_CERTEXP4((SALT311.StrType)le.AT_CERTEXP4),
    Fields.InValid_AT_CERTEXP5((SALT311.StrType)le.AT_CERTEXP5),
    Fields.InValid_AT_CERTEXP6((SALT311.StrType)le.AT_CERTEXP6),
    Fields.InValid_AT_CERTEXP7((SALT311.StrType)le.AT_CERTEXP7),
    Fields.InValid_AT_CERTEXP8((SALT311.StrType)le.AT_CERTEXP8),
    Fields.InValid_AT_CERTEXP9((SALT311.StrType)le.AT_CERTEXP9),
    Fields.InValid_AT_CERTEXP10((SALT311.StrType)le.AT_CERTEXP10),
    Fields.InValid_EFX_FOREIGN((SALT311.StrType)le.EFX_FOREIGN),
    Fields.InValid_normCompany_Name((SALT311.StrType)le.normCompany_Name),
    Fields.InValid_normCompany_Type((SALT311.StrType)le.normCompany_Type),
    Fields.InValid_Norm_Geo_Precision((SALT311.StrType)le.Norm_Geo_Precision),
    Fields.InValid_Exploded_Desc_Corporate_Amount_Precision((SALT311.StrType)le.Exploded_Desc_Corporate_Amount_Precision),
    Fields.InValid_Exploded_Desc_Location_Amount_Precision((SALT311.StrType)le.Exploded_Desc_Location_Amount_Precision),
    Fields.InValid_Exploded_Desc_Public_Co_Indicator((SALT311.StrType)le.Exploded_Desc_Public_Co_Indicator),
    Fields.InValid_Exploded_Desc_Stock_Exchange((SALT311.StrType)le.Exploded_Desc_Stock_Exchange),
    Fields.InValid_Exploded_Desc_Telemarketablity_Score((SALT311.StrType)le.Exploded_Desc_Telemarketablity_Score),
    Fields.InValid_Exploded_Desc_Telemarketablity_Total_Indicator((SALT311.StrType)le.Exploded_Desc_Telemarketablity_Total_Indicator),
    Fields.InValid_Exploded_Desc_Telemarketablity_Total_Score((SALT311.StrType)le.Exploded_Desc_Telemarketablity_Total_Score),
    Fields.InValid_Exploded_Desc_Government1057_Entity((SALT311.StrType)le.Exploded_Desc_Government1057_Entity),
    Fields.InValid_Exploded_Desc_Merchant_Type((SALT311.StrType)le.Exploded_Desc_Merchant_Type),
    Fields.InValid_Exploded_Desc_Busstatcd((SALT311.StrType)le.Exploded_Desc_Busstatcd),
    Fields.InValid_Exploded_Desc_CMSA((SALT311.StrType)le.Exploded_Desc_CMSA),
    Fields.InValid_Exploded_Desc_Corpamountcd((SALT311.StrType)le.Exploded_Desc_Corpamountcd),
    Fields.InValid_Exploded_Desc_Corpamountprec((SALT311.StrType)le.Exploded_Desc_Corpamountprec),
    Fields.InValid_Exploded_Desc_Corpamounttp((SALT311.StrType)le.Exploded_Desc_Corpamounttp),
    Fields.InValid_Exploded_Desc_Corpempcd((SALT311.StrType)le.Exploded_Desc_Corpempcd),
    Fields.InValid_Exploded_Desc_Ctrytelcd((SALT311.StrType)le.Exploded_Desc_Ctrytelcd),
    Fields.InValid_NormAddress_Type((SALT311.StrType)le.NormAddress_Type),
    Fields.InValid_Norm_Address((SALT311.StrType)le.Norm_Address),
    Fields.InValid_Norm_City((SALT311.StrType)le.Norm_City),
    Fields.InValid_Norm_State((SALT311.StrType)le.Norm_State),
    Fields.InValid_Norm_StateC2((SALT311.StrType)le.Norm_StateC2),
    Fields.InValid_Norm_Zip((SALT311.StrType)le.Norm_Zip),
    Fields.InValid_Norm_Zip4((SALT311.StrType)le.Norm_Zip4),
    Fields.InValid_Norm_Lat((SALT311.StrType)le.Norm_Lat),
    Fields.InValid_Norm_Lon((SALT311.StrType)le.Norm_Lon),
    Fields.InValid_Norm_Geoprec((SALT311.StrType)le.Norm_Geoprec),
    Fields.InValid_Norm_Region((SALT311.StrType)le.Norm_Region),
    Fields.InValid_Norm_Ctryisocd((SALT311.StrType)le.Norm_Ctryisocd),
    Fields.InValid_Norm_Ctrynum((SALT311.StrType)le.Norm_Ctrynum),
    Fields.InValid_Norm_Ctryname((SALT311.StrType)le.Norm_Ctryname),
    Fields.InValid_clean_company_name((SALT311.StrType)le.clean_company_name),
    Fields.InValid_clean_phone((SALT311.StrType)le.clean_phone),
    Fields.InValid_clean_secondary_phone((SALT311.StrType)le.clean_secondary_phone),
...
END;
Errors := NORMALIZE(h,260,NoteErrors(LEFT,COUNTER));
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doSummaryPerSrc = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Equifax_Business_Data, Fields, 'RECORDOF(fieldPopulationOverall)', TRUE);

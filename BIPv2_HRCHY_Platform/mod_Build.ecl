...
lj := BIPv2_HRCHY_Platform.PrepSources.LNCA(head, lncad);
fj := BIPv2_HRCHY_Platform.PrepSources.FRAN(head, frand);
dj := BIPv2_HRCHY_Platform.PrepSources.DUNS(head, dunsd);
aj := dedup(lj + dj + fj, all) : persist('~thor_data400::BIPv2_HRCHY_Platform::mod_Build.aj');


//***** IDENTIFY COMMON HRCHY TYPES THAT CAN BE PATCHED
d2 := BIPv2_HRCHY_Platform.PrepSources.Patch(aj);

//***** HANDLE OVERLAPPING SOURCES
d3 := BIPv2_HRCHY_Platform.PrepSources.HandleDups(d2) : persist('~thor_data400::BIPv2_HRCHY_Platform::mod_Build.d3' );


//***** CALL THE FUNCTIONS THAT ASSIGN ORGID/ULTID AND CREATE LGID TABLE
shared rc := Functions.RemoveConflict(project(d3, BIPv2_HRCHY_Platform.Layouts.inrec)) : persist('~thor_data400::cemtemp::rc' );

shared wpc := Functions.WithParentsChildren(rc) : persist('~thor_data400::BIPv2_HRCHY_Platform::mod_Build.wpc' );

export LGIDTable := Functions.LgidTable(wpc) 
	#IF(Debug)
	: persist('~thor_data400::BIPv2_HRCHY_Platform::mod_Build.lt' )
	#END	
	;

export wi :=  Functions.WithIds(wpc,LGIDTable) 
	#IF(Debug)
		: persist('~thor_data400::BIPv2_HRCHY_Platform::mod_Build.wi' )
	#END	
	;	
...
f := dataset('~thor_data400::BIPv2_HRCHY_Platform::mod_Build.f', fr, thor);
aj := dataset('~thor_data400::BIPv2_HRCHY_Platform::mod_Build.aj', ajr, thor);
wpc := dataset('~thor_data400::BIPv2_HRCHY_Platform::mod_Build.wpc', wpcr, thor);
*/
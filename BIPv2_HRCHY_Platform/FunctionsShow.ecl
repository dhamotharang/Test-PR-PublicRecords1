...
// output(BIPv2_HRCHY_Platform.FunctionsShow.ShowDirectParentsChildren(mysele), named('results'));//W20151218-091718
...
shared GetLGID(
	unsigned6 myproxid
	,string1 SourcePicked = ''/*default is to pick based on availability of preferred hrchy sources, but you may specify one of BIPv2_HRCHY_Platform_Dev.Constants.Sources*/) :=
FUNCTION
...
export ShowParents(
	unsigned6 myproxid
	,string1 SourcePicked = ''/*default it to pick based on availability of preferred hrchy sources, but you may specify one of BIPv2_HRCHY_Platform_Dev.Constants.Sources*/) :=
FUNCTION
...
lp := 
  LOOP(
    lb, 
    ut.min2(BIPv2_HRCHY_Platform.Constants.max_depth_supported, max(lb(proxid = myproxid), proxid_level_within_lgid)),//might need + 1 on the second item here, test to see
    join(
      rows(left), 
      lb, 
...
import BIPV2;
ref :=
	BIPv2_HRCHY_Platform_dev;
	// BIPv2_HRCHY_Platform;
ipl 	:= INDEX(
		...
		// '~thor_data400::BIPv2_HRCHY_Platform::key::'+BIPV2.KeySuffix+'::proxid'
...
ilgid := INDEX(
		...
		{lgid},
		{dataset([], ref.Layouts.lgidr)},		
		// '~thor_data400::BIPv2_HRCHY_Platform::key::'+BIPV2.KeySuffix+'::lgid'

...
FUNCTION
	...
	lncar := BIPv2_HRCHY_Platform.Layouts.lncar;
	
	// ACTIVE
	active_lnca := lncad(record_type in [dcav2.Utilities.RecordType.Updated,dcav2.Utilities.RecordType.New]);

	// SLIM
	lnca := project(active_lnca, transform(lncar, self := left.rawfields, self.date_last_seen := left.date_last_seen)) : persist('~thor_data400::BIPv2_HRCHY_Platform::mod_Build.lnca');
	...
	join(
		...
		transform(
			BIPv2_HRCHY_Platform.Layouts.prep_r2,
...
FUNCTION
	...
	dedup(
		project(
			active_duns, 
			transform(
				BIPv2_HRCHY_Platform.Layouts.dunsr, 
				self := left.rawfields, 
				self.date_last_seen := left.date_last_seen, 
				self:= left.clean_address
			))
		, all) : persist('~thor_data400::BIPv2_HRCHY_Platform::mod_Build.duns');//lots of dups here
		
	// JOIN TO HEADER
	BIPv2_HRCHY_Platform.Layouts.prep_r2 dunxf(duns le, head ri) := transform
...
FUNCTION
	...
	// SLIM
	fran := project(active_fran, BIPv2_HRCHY_Platform.Layouts.franr);
	
	// JOIN TO HEADER
	fj0 :=
	join(
		fran((unsigned6)franchisee_id > 0, BIPv2_HRCHY_Platform.Constants.FRAN.ctype_decode(unit_flag) <> ''),
...
export Patch(
	dataset(BIPv2_HRCHY_Platform.Layouts.prep_r2) aj
) :=
FUNCTION
...
export HandleDups(
	dataset(BIPv2_HRCHY_Platform.Layouts.prep_r2) d2
) :=
FUNCTION

	//***** FIND SEMI DUPS (MORE THAN ONE ENTRY FOR AN ID) FOR INVESTIGATION
	duprec := record
		d2.id;
		cnt := count(group)
	end;

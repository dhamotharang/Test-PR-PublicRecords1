...
EXPORT Proc_BID_AutokeyBuild(STRING filedate) := FUNCTION

		c := NaturalDisaster_Readiness.Constants(filedate, 'BID');
		ak_dataset	:= c.ak_dataset;  		/* Base file projected as new layout with did & bdid */
		ak_keyname 	:= c.ak_keynamebid;  	/* AutoKey Superfile container name */
		ak_logical	:= c.ak_logicalbid;  	/* AutoKey Logical file name */     
	...
	mod_AKB := module(AutokeyB2.Fn_Build.params) 
		...
		EXPORT STRING L_inkeyname := 	NaturalDisaster_Readiness.constants().ak_keynamebid; 
		EXPORT STRING L_inlogical := 	NaturalDisaster_Readiness.constants(filedate).ak_logicalbid; 

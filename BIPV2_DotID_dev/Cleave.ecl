...
SHARED Candidates_cnp_number := Candidates_cnp_number_ni : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::Cleave::Candidates_cnp_number',EXPIRE(Config.PersistExpire));
 
EXPORT Candidates_cnp_numberKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Candidates_cnp_number';
 
EXPORT Candidates_cnp_number_Idx := INDEX(Candidates_cnp_number,{DOTid},{Candidates_cnp_number},Candidates_cnp_numberKeyName);
SHARED Possibles_cnp_number := Possibles_cnp_number_ni : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::Cleave::Possibles_cnp_number',EXPIRE(Config.PersistExpire));
 
EXPORT Possibles_cnp_numberKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Possibles_cnp_number';
...
EXPORT CleaveBy_cnp_number(DATASET(RECORDOF(h)) inp) := FUNCTION
  ...
  R TakeCleave(inp le,k ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J0 := JOIN(inp,k,LEFT.DOTid=RIGHT.DOTid,TakeCleave(LEFT,RIGHT),LOOKUP,LEFT OUTER);
  J1 := JOIN(inp,k,LEFT.DOTid=RIGHT.DOTid,TakeCleave(LEFT,RIGHT),LEFT OUTER); // Debug/roxie version
...
EXPORT FullTallied_active_enterprise_number := Tallied_active_enterprise_number(active_enterprise_number NOT IN SET(s.nulls_active_enterprise_number,active_enterprise_number));// Only 'full' values for cleave-determination
...
SHARED Candidates_active_enterprise_number := Candidates_active_enterprise_number_ni : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::Cleave::Candidates_active_enterprise_number',EXPIRE(Config.PersistExpire));
 
EXPORT Candidates_active_enterprise_numberKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Candidates_active_enterprise_number';
...
SHARED Possibles_active_enterprise_number := Possibles_active_enterprise_number_ni : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::Cleave::Possibles_active_enterprise_number',EXPIRE(Config.PersistExpire));
 
EXPORT Possibles_active_enterprise_numberKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Possibles_active_enterprise_number';
...
EXPORT CleaveBy_active_enterprise_number(DATASET(RECORDOF(h)) inp) := FUNCTION
  ...
  END;
  R TakeCleave(inp le,k ri) := TRANSFORM
    BOOLEAN left_match := ri.left_active_enterprise_number = le.active_enterprise_number AND le.active_enterprise_number NOT IN SET(s.nulls_active_enterprise_number,active_enterprise_number);
    BOOLEAN right_match := ri.right_active_enterprise_number = le.active_enterprise_number AND le.active_enterprise_number NOT IN SET(s.nulls_active_enterprise_number,active_enterprise_number);
    SELF.__Cluster := MAP ( ri.DOTid = 0 => -1, left_match => 1, right_match => 2, 0 );
    SELF.__Shadow := le.DOTid;
    SELF := le;
  END;
...
SHARED Candidates_active_domestic_corp_key := Candidates_active_domestic_corp_key_ni : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::Cleave::Candidates_active_domestic_corp_key',EXPIRE(Config.PersistExpire));
 
EXPORT Candidates_active_domestic_corp_keyKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Candidates_active_domestic_corp_key';
...
SHARED Possibles_active_domestic_corp_key := Possibles_active_domestic_corp_key_ni : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::Cleave::Possibles_active_domestic_corp_key',EXPIRE(Config.PersistExpire));
 
EXPORT Possibles_active_domestic_corp_keyKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Possibles_active_domestic_corp_key';
...
EXPORT CandidatesCounts := DATASET([{COUNT(Candidates_cnp_number),COUNT(Candidates_active_enterprise_number),COUNT(Candidates_active_domestic_corp_key)}],{UNSIGNED count_Candidates_cnp_number,UNSIGNED count_Candidates_active_enterprise_number,UNSIGNED count_Candidates_active_domestic_corp_key});
  converted0 := CleaveBy_cnp_number(h);
  converted1 := CleaveBy_active_enterprise_number(converted0);
  converted2 := CleaveBy_active_domestic_corp_key(converted1);
  converted3 := converted2;
SHARED input_file_np := IF(Config.ByPassCleave,h,converted3);
EXPORT input_file := input_file_np : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::Cleave::input_file',EXPIRE(Config.PersistExpire));
...
EXPORT patch_file := patch_file_np : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::Cleave::patch_file',EXPIRE(Config.PersistExpire));

...
SHARED  h00 := Specificities(ih).input_file;
  ...
  SHARED h00_match := h00( 
      0 + IF( cnp_name  IN SET(s.nulls_cnp_name,cnp_name), 0, 25 ) + IF( corp_legal_name  IN SET(s.nulls_corp_legal_name,corp_legal_name), 0, 25 ) + IF( cnp_number  IN SET(s.nulls_cnp_number,cnp_number), 0, 14 ) + IF( cnp_btype  IN SET(s.nulls_cnp_btype,cnp_btype), 0, 3 ) + IF( company_fein  IN SET(s.nulls_company_fein,company_fein), 0, 27 ) + IF( active_duns_number  IN SET(s.nulls_active_duns_number,active_duns_number), 0, 24 ) + IF( active_enterprise_number  IN SET(s.nulls_active_enterprise_number,active_enterprise_number), 0, 27 ) + IF( active_domestic_corp_key  IN SET(s.nulls_active_domestic_corp_key,active_domestic_corp_key), 0, 27 ) + IF( prim_range  IN SET(s.nulls_prim_range,prim_range), 0, 13 ) + IF( prim_name  IN SET(s.nulls_prim_name,prim_name), 0, 15 ) + IF( sec_range  IN SET(s.nulls_sec_range,sec_range), 0, 12 ) + IF( st  IN SET(s.nulls_st,st), 0, 5 ) + IF( v_city_name  IN SET(s.nulls_v_city_name,v_city_name), 0, 11 ) + IF( zip  IN SET(s.nulls_zip,zip), 0, 14 ) + IF( isContact  IN SET(s.nulls_isContact,isContact), 0, 1 ) + IF( fname  IN SET(s.nulls_fname,fname), 0, 9 ) + IF( mname  IN SET(s.nulls_mname,mname), 0, 10 ) + IF( lname  IN SET(s.nulls_lname,lname), 0, 11 ) + IF( name_suffix  IN SET(s.nulls_name_suffix,name_suffix), 0, 9 ) + IF( contact_ssn  IN SET(s.nulls_contact_ssn,contact_ssn), 0, 28 ) >= Config.BasicMatchThreshold); // Potentially remove anything that would violate FORCE(+) constraints

// -- BIPV2_DotID_dev._ApplyHacks('BIPV2_DotID_dev',,true).do_all_hacks;
import SALTTOOLS22,STD,wk_ut,tools; 
EXPORT _ApplyHacks(
   string   pModule               = 'BIPV2_DotID_dev'
...
module
  ...
  function
    ...
    { 
      ...
      + '  Cands0 := BIPV2_DotID_dev[.]match_candidates[(]inhead[)][.]ForeignCorpkey_candidates;[[:space:]]*'
      ...
      + '    Cands0[.]Basis;[[:space:]]*'
      + '    SALT33[.]StrType Context := SALT33[.]GetNthWord[(]Cands0[.]Basis,2,\'[|]\'[)]; // Context for the basis [(]\'<\'[)][[:space:]]*'
      + '  END;[[:space:]]*'
      ...
      + '  Cands0 := BIPV2_DotID_dev._attr_ck_charters;\n'
  ...
  function
    ...
    dme := dataset([{
          'SHARED ChangeName := \'[~]temp::DOTid::BIPV2_DotID_dev::changes_it\'[+]iter;'
      ,''
      ,   'import bipv2;\n'
        + 'SHARED ChangeName := \'~temp::DOTid::BIPV2_DotID_dev::changes_it\'+iter+\'_\'+ pversion/*HACK*/;'

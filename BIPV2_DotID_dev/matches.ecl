...
// Compute the score for the concept csz
  INTEGER2 csz_score_ext := SALT30.ClipScore(MAX(csz_score_pre,0) + v_city_name_score + st_score + zip_score + MAX(address_score_pre,0));// Score in surrounding context
...
// Compute the score for the concept address
  INTEGER2 address_score_ext := SALT30.ClipScore(MAX(address_score_pre,0)+ addr1_score + prim_range_score + prim_name_score + sec_range_score+ csz_score + v_city_name_score + st_score + zip_score);// Score in surrounding context
...
EXPORT All_Matches := all_matches0 : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::all_m',EXPIRE(Config.PersistExpire)); // To by used by rcid and DOTid
SALT30.mac_avoid_transitives(All_Matches,DOTid1,DOTid2,Conf,DateOverlap,Rule,o);
EXPORT PossibleMatches := o : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::mt',EXPIRE(Config.PersistExpire));
SALT30.mac_get_BestPerRecord( All_Matches,rcid1,DOTid1,rcid2,DOTid2,o );
EXPORT BestPerRecord := o : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::mr',EXPIRE(Config.PersistExpire));
//Now lets see if any slice-outs are needed
too_big := TABLE(h,{DOTid, InCluster := COUNT(GROUP)},DOTid,MERGE)(InCluster>1000); // DOTid that are too big for sliceout
h_ok := JOIN(h,too_big,LEFT.DOTid=RIGHT.DOTid,TRANSFORM(LEFT),LOOKUP,LEFT ONLY);
SHARED in_matches := JOIN(h_ok,h_ok,LEFT.DOTid=RIGHT.DOTid AND (>STRING6<)LEFT.rcid<>(>STRING6<)RIGHT.rcid,match_join(LEFT,RIGHT,9999),LOCAL,HINT(unsorted_output));
SALT30.mac_cluster_breadth(in_matches,rcid1,rcid2,DOTid1,o);
o1 := JOIN(o,Specificities(ih).ClusterSizes,LEFT.DOTid1=RIGHT.DOTid,LOCAL);
EXPORT ClusterLinkages := o1 : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::clu',EXPIRE(Config.PersistExpire));
...
Pref_Layout := RECORD
  ...
  SALT30.UIDType DOTid;  // Outcase within
  INTEGER2 Closest; // Best present link
  SALT30.UIDType Pref_rcid; // Prefers this record
  SALT30.UIDType Pref_DOTid; // Prefers this cluster
...
WillJoin1 := JOIN(CurrentJumpers,All_Matches(Conf>=MatchThreshold),LEFT.DOTid=RIGHT.DOTid1 AND LEFT.Pref_DOTid=RIGHT.DOTid2,TRANSFORM(LEFT),LEFT ONLY,HASH);
WillJoin2 := JOIN(WillJoin1,All_Matches(Conf>=MatchThreshold),LEFT.DOTid=RIGHT.DOTid2 AND LEFT.Pref_DOTid=RIGHT.DOTid1,TRANSFORM(LEFT),LEFT ONLY,HASH);
EXPORT BetterElsewhere := WillJoin2;
EXPORT ToSlice := IF ( Config.DoSliceouts, DEDUP(SORT(DISTRIBUTE(BetterElsewhere(Pref_Margin>10),HASH(DOTid)),DOTid,-Pref_Margin,LOCAL),DOTid,LOCAL)) : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::Matches::ToSlice',EXPIRE(Config.PersistExpire));
// 1024x better in new place
  SALT30.MAC_Avoid_SliceOuts(PossibleMatches,DOTid1,DOTid2,DOTid,Pref_DOTid,ToSlice,m); // If DOTid is slice target - don't use in match
...
ut.MAC_Patch_Id(ih,DOTid,BasicMatch(ih).patch_file,DOTid1,DOTid2,ihbp); // Perform basic matches
SALT30.MAC_Reassign_UID(ihbp,Cleave(ih).patch_file,DOTid,rcid,ih1); // Perform cleaves
SALT30.MAC_SliceOut_ByRID(ih1,rcid,DOTid,ToSlice,rcid,sliced0); // Execute Sliceouts
  sliced := IF( Config.DoSliceouts, sliced0, ih1); // Compile time ability to remove sliceout cost
ut.MAC_Patch_Id(sliced,DOTid,Matches,DOTid1,DOTid2,o); // Join Clusters
  Patchproxid := SALT30.MAC_ParentId_Patch(o,proxid,DOTid);  // Collapse any proxid now joined by DOTid
  Patchlgid3 := SALT30.MAC_ParentId_Patch(Patchproxid,lgid3,proxid);  // Collapse any lgid3 now joined by proxid
...
EXPORT id_shift_r := RECORD
    SALT30.UIDType rcid;
    SALT30.UIDType DOTid_before;
    SALT30.UIDType DOTid_after;
  ...
  END;
EXPORT IdChanges := JOIN(ih,patched_infile,LEFT.rcid = RIGHT.rcid AND (LEFT.DOTid<>RIGHT.DOTid OR LEFT.proxid<>RIGHT.proxid OR LEFT.lgid3<>RIGHT.lgid3 OR LEFT.orgid<>RIGHT.orgid OR LEFT.ultid<>RIGHT.ultid),note_change(LEFT,RIGHT));
...
EXPORT PreIDs := BIPV2_DOTID_dev.Fields.UIDConsistency(ih); // Export whole consistency module
EXPORT PostIDs := BIPV2_DOTID_dev.Fields.UIDConsistency(Patched_Infile); // Export whole consistency module
EXPORT PatchingError0 := PreIDs.IdCounts[1].DOTid_count - PostIDs.IdCounts[1].DOTid_count - MatchesPerformed - COUNT(BasicMatch(ih).patch_file) + SlicesPerformed + Cleave(ih).patch_count; // Should be zero

//Executable code
EXPORT Proc_Iterate(STRING iter,DATASET(Layout_DOT) InFile0 = BIPV2_DOTID_dev.In_DOT,STRING OutFileNameP = '~temp::DOTid::BIPV2_DOTID_dev::it',UNSIGNED MatchThreshold = Config.MatchThreshold,BOOLEAN Debugging = true) := MODULE
SHARED InFile := InFile0;
SHARED MM := BIPV2_DOTID_dev.matches(InFile,MatchThreshold); // Get the matching module
...
SHARED BM := BIPV2_DOTID_dev.BasicMatch(InFile);
...
MP := PARALLEL( OUTPUT(MM.MatchesPerformed,NAMED('MatchesPerformed')),OUTPUT(BIPV2_DOTID_dev.BasicMatch(InFile).basic_match_count,NAMED('BasicMatchesPerformed')));
...
d := DATASET([{MM.PatchingError0,MM.DuplicateRids0,COUNT(BIPV2_DOTID_dev.match_candidates(InFile).Unlinkables),COUNT(BIPV2_DOTID_dev.specificities(InFile).Rejected_file)}],{INTEGER PatchingError0, INTEGER DuplicateRids0, UNSIGNED UnlinkableRecords0,UNSIGNED RecordsRejected0});
...
ChangeName := '~temp::DOTid::BIPV2_DOTID_dev::changes_it'+iter;
...
//EXPORT LoopN(UNSIGNED N,UNSIGNED mt=Config.MatchThreshold) := LOOP(InFile,N,BIPV2_DOTID_dev.matches(ROWS(LEFT),mt).patched_infile); // Loop N times
//EXPORT LoopThisN(DATASET(BIPV2_DOTID_dev.Layout_DOT)d,UNSIGNED N,UNSIGNED mt=Config.MatchThreshold) := LOOP(d,N,BIPV2_DOTID_dev.matches(ROWS(LEFT),mt).patched_infile);

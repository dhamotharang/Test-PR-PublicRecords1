...
function
  ...
  kFile := BIPV2_DotID_dev.Keys(BFile/*,version*/);
  odl   := PROJECT(CHOOSEN(KFile.Candidates(DotID=DotIDone),100000),BIPV2_DotID_dev.match_candidates(BFile).layout_candidates);
  odr   := PROJECT(CHOOSEN(KFile.Candidates(DotID=DotIDTwo),100000),BIPV2_DotID_dev.match_candidates(BFile).layout_candidates);
  ...
  BIPV2_DotID_dev.match_candidates(BFile).layout_attribute_matches ainto(KFile.Attribute_Matches le) := TRANSFORM
    SELF := le;
  END;

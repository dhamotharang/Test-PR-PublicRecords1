import BIPV2_Files, BIPV2, BIPV2_DotID_dev,wk_ut,tools,std;
...
l_base    := BIPV2_Files.files_dotid('BIPV2_DotID_dev').l_DOTID;
export _proc_dotid(
  ...
  shared _files_dotid := BIPV2_Files.files_dotid('BIPV2_DotID_dev');
  
	/* ---------------------- Files -------------------------------------- */
	export f_out(string istr)		:= BIPV2_Files.files_dotid('BIPV2_DotID_dev').FILE_DOT_SALT_OP              + '::' + Build_Date + '::it' + istr;
	shared f_hist(string istr)	:= BIPv2_Files.files_dotid('BIPV2_DotID_dev').FILE_DOT_SALT_POSSIBLE_MATCH  + '::' + Build_Date + '::it' + istr;
	shared f_init								:= BIPv2_Files.files_dotid('BIPV2_DotID_dev').FILE_INIT                     + '::' + Build_Date;
  ...
  export preProcessStrata(
	...
	/* ---------------------- SALT Specificities ------------------------- */
  import SALTTOOLS22;
	shared specMod := BIPV2_DotID_dev.specificities(input);
	shared specBuild := specMod.Build;
	shared specDebug := sequential(OUTPUT(specMod.Specificities,NAMED('Specificities')),OUTPUT(specMod.SpcShift,NAMED('SpcShift'))  ,SALTTOOLS22.mac_Patch_SPC(SpecMod.Specificities,'BIPV2_DotID_dev._SPC',input,,false,true)); //add patching of specs to spc file attribute
	
	/* ---------------------- SALT Iteration ----------------------------- */
	// shared possibleMatches := output(BIPV2_DotID_dev.matches(input).PossibleMatches,,f_hist(iter),overwrite,compressed);
	shared saltMod 	:= BIPV2_DotID_dev.Proc_Iterate(Build_Date,iter, input, f_out(''));
	...
	/* ---------------------- Review Samples ----------------------------- */
	export outputReviewSamples := BIPV2_DotID_dev._Samples(input).out;
	...
	export DATASET(l_common) inline_iteration(DATASET(l_common) ds) := FUNCTION
		...
		ds2 := BIPV2_DotID_dev.matches(ds1,BIPV2_DotID_dev.Config.MatchThreshold).patched_infile;
  ...
  functionmacro
		eclSpec		:= 'import BIPV2_Files,BIPV2_build;\npversion  := \'@version@\';\nlih := project(' + #TEXT(pInputfile) + ',BIPV2_Files.files_dotid(\'BIPV2_DotID_dev\').l_DOTID);\n#OPTION(\'multiplePersistInstances\',FALSE);\n#workunit(\'name\',\'BIPV2 DotID \' + pversion + \' Specificities \');\n#workunit(\'priority\',\'high\');\nBIPV2_DotID_dev._proc_dotid(lih).runSpecBuild;';
 ...
 endmacro;
	...
	export MultIter_wuName(unsigned startIter, unsigned numIters) := 
		'BIPV2_DotID_dev Controller ' + BIPV2.KeySuffix + ' ' + (string)startiter + '-' + (string)(startiter + numiters - 1);
    ...
    fbool(boolean pinput) := if(pinput = true,'true','false');
		...
		eclInit		:= 'import BIPV2_Files,BIPV2_build;\niteration := \'@iteration@\';\npversion  := \'@version@\';\n#workunit(\'name\',\'BIPV2_DotID_dev \' + pversion + \' Init \');\n#workunit(\'priority\',\'high\');\n#OPTION(\'multiplePersistInstances\',FALSE);\n'                                                                                + 'BIPV2_DotID_dev._proc_dotid(   ,         ,\''         + pversion + '\',' + fbool(pFixDotidOverlinks) + ').init('               + #TEXT(p_Init_File)+ ');';
		eclSpec		:= 'import BIPV2_Files,BIPV2_build;\niteration := \'@iteration@\';\npversion  := \'@version@\';\nlih := BIPV2_Files.files_dotid(\'BIPV2_DotID_dev\').DS_BUILDING;\n#OPTION(\'multiplePersistInstances\',FALSE);\n#workunit(\'name\',\'BIPV2_DotID_dev \' + pversion + \' Specificities \');\n#workunit(\'priority\',\'high\');\n'     + 'BIPV2_DotID_dev._proc_dotid(lih,iteration,\''         + pversion + '\',' + fbool(pFixDotidOverlinks) + ').runSpecBuild;';
		eclIter		:= 'import BIPV2_Files,BIPV2_build;\niteration := \'@iteration@\';\npversion  := \'@version@\';\nlih := BIPV2_Files.files_dotid(\'BIPV2_DotID_dev\').DS_BUILDING;\n#OPTION(\'multiplePersistInstances\',FALSE);\n#workunit(\'name\',\'BIPV2_DotID_dev \' + pversion + \' iter \' + iteration);\n#workunit(\'priority\',\'high\');\n'  + 'BIPV2_DotID_dev._proc_dotid(lih,iteration,\''         + pversion + '\',' + fbool(pFixDotidOverlinks) + ').runIter;';
		eclPost		:= 'import BIPV2_build;\n#workunit(\'name\',\'BIPV2_DotID_dev @version@ PostProcess\');\n'                                                                                                                                                                                                                                            + 'BIPV2_DotID_dev._proc_dotid(,\'' + lastIter + '\',\'' + pversion + '\',' + fbool(pFixDotidOverlinks) + ').updateSuperFiles(,'  + #TEXT(p_Init_File)+ ')';
	...
	//		#workunit('name',BIPV2_DotID_dev._proc_dotid().MultIter_wuName(startIter,numIters));
	//		BIPV2_DotID_dev._proc_dotid().MultIter_run(startIter,numIters,doSpec);

IMPORT ut,SALT30;
IMPORT BIPV2;
EXPORT specificities(DATASET(layout_DOT) h) := MODULE
 
IMPORT BIPV2;
EXPORT input_layout := RECORD // project out required fields
  SALT30.UIDType DOTid := h.DOTid; // using existing id field
  SALT30.UIDType proxid := h.proxid; // Copy ID from hierarchy
  ...
  h.contact_ssn;
  h.contact_phone;
  TYPEOF(h.contact_email) contact_email := (TYPEOF(h.contact_email))Fields.Make_contact_email((SALT30.StrType)h.contact_email ); // Cleans before using
...
input_layout do_computes(h01 le) := TRANSFORM
  SELF.csz := IF (Fields.InValid_csz((SALT30.StrType)le.v_city_name,(SALT30.StrType)le.st,(SALT30.StrType)le.zip),0,HASH32((SALT30.StrType)le.v_city_name,(SALT30.StrType)le.st,(SALT30.StrType)le.zip)); // Combine child fields into 1 for specificity counting
  ...
  SELF.address := IF (Fields.InValid_address((SALT30.StrType)SELF.addr1,(SALT30.StrType)SELF.csz),0,HASH32((SALT30.StrType)SELF.addr1,(SALT30.StrType)SELF.csz)); // Combine child fields into 1 for specificity counting
  SELF := le;
END;
h02 := PROJECT(h01,do_computes(LEFT));
SHARED h0 := SALT30.MAC_Partition_Stars(h02,SALT_Partition,DOTid); // Note b-list inside a-list clusters
EXPORT PartitionCounts := TABLE(h0,{SALT_Partition,Cnt := COUNT(GROUP)},SALT_Partition,FEW);
SHARED reject := Fields.Invalid_company_name((SALT30.StrType)h0.company_name)>0;
EXPORT rejected_file := h0(reject);
 
EXPORT input_file_np := h0; // No-persist version for remote_linking
 
EXPORT input_file := h0(~Reject) : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::Specificities_Cache',EXPIRE(Config.PersistExpire));
//We have DOTid specified - so we can compute statistics on the cluster counts
  r0 := RECORD
    input_file.DOTid;
    UNSIGNED6 InCluster := COUNT(GROUP);
  end;
EXPORT ClusterSizes := TABLE(input_file,r0,DOTid,LOCAL) : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::Cluster_Sizes',EXPIRE(Config.PersistExpire));
EXPORT TotalClusters := COUNT(ClusterSizes);
 
EXPORT  cnp_name_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,cnp_name); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(cnp_name_deduped,cnp_name,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
  ...
  SALT30.mac_edit_distance_pairs(specs_added,cnp_name,cnt,1,false,distance_computed);//Computes specificities of fuzzy matches
EXPORT cnp_name_values_persisted_temp := distance_computed;
 
EXPORT  corp_legal_name_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,corp_legal_name); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(corp_legal_name_deduped,corp_legal_name,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT30.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
SHARED corp_legal_name_sa := specs_added; // Hope over shared/export below
// This field is a word bag; so create specifcities for the words too
  SALT30.MAC_Field_Variants_WordBag(corp_legal_name_deduped,DOTid,corp_legal_name,expanded)// expand out all variants of wordbag
  SALT30.Mac_Field_Count_UID(expanded,corp_legal_name,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
...
EXPORT corp_legal_nameValuesKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::corp_legal_name';
...
EXPORT corp_legal_name_values_key := INDEX(corp_legal_name_ad,{corp_legal_name},{corp_legal_name_ad},corp_legal_nameValuesKeyName);
  SALT30.mac_wordbag_addweights(corp_legal_name_sa,corp_legal_name,corp_legal_name_ad,p);
EXPORT corp_legal_name_values_persisted_temp := p;
 
EXPORT  cnp_number_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,cnp_number); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(cnp_number_deduped,cnp_number,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  cnp_btype_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,cnp_btype); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(cnp_btype_deduped,cnp_btype,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  company_fein_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,company_fein); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(company_fein_deduped,company_fein,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  active_duns_number_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,active_duns_number); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(active_duns_number_deduped,active_duns_number,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  active_enterprise_number_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,active_enterprise_number); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(active_enterprise_number_deduped,active_enterprise_number,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  active_domestic_corp_key_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,active_domestic_corp_key); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(active_domestic_corp_key_deduped,active_domestic_corp_key,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  prim_range_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,prim_range); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(prim_range_deduped,prim_range,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  prim_name_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,prim_name); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(prim_name_deduped,prim_name,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  sec_range_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,sec_range); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(sec_range_deduped,sec_range,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  st_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,st); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(st_deduped,st,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  v_city_name_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,v_city_name); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(v_city_name_deduped,v_city_name,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  zip_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,zip); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(zip_deduped,zip,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  csz_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,csz); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(csz_deduped,csz,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT30.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
EXPORT csz_values_persisted_temp := specs_added;
 
EXPORT  addr1_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,addr1); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(addr1_deduped,addr1,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT30.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
EXPORT addr1_values_persisted_temp := specs_added;
 
EXPORT  address_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,address); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(address_deduped,address,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT30.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
EXPORT address_values_persisted_temp := specs_added;
 
EXPORT  isContact_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,isContact); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(isContact_deduped,isContact,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT30.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
EXPORT isContact_values_persisted_temp := specs_added;
 
EXPORT  fname_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,fname); // Reduce to field values by UID
  SALT30.MAC_Field_Variants_Initials(fname_deduped,DOTid,fname,expanded) // expand out all variants of initial
  SALT30.Mac_Field_Count_UID(expanded,fname,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  ...
  SALT30.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
  SALT30.mac_edit_distance_pairs(specs_added,fname,cnt,1,false,distance_computed);//Computes specificities of fuzzy matches
EXPORT fname_values_persisted_temp := distance_computed;
 
EXPORT  mname_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,mname); // Reduce to field values by UID
  SALT30.MAC_Field_Variants_Initials(mname_deduped,DOTid,mname,expanded) // expand out all variants of initial
  SALT30.Mac_Field_Count_UID(expanded,mname,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT30.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
  SALT30.mac_edit_distance_pairs(specs_added,mname,cnt,1,false,distance_computed);//Computes specificities of fuzzy matches
EXPORT mname_values_persisted_temp := distance_computed;
 
EXPORT  lname_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,lname); // Reduce to field values by UID
  expanded := SALT30.MAC_Field_Variants_Hyphen(lname_deduped,DOTid,lname,2); // expand out all variants when hyphenated
  SALT30.Mac_Field_Count_UID(expanded,lname,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT30.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
  SALT30.mac_edit_distance_pairs(specs_added,lname,cnt,1,false,distance_computed);//Computes specificities of fuzzy matches
EXPORT lname_values_persisted_temp := distance_computed;
 
EXPORT  name_suffix_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,name_suffix); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(name_suffix_deduped,name_suffix,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  ...
  with_id := table(counted,r1);
  SALT30.MAC_Field_Accumulate_Counts(with_id,name_suffix_NormSuffix,NormSuffix_cnt,fuzzies_counted0)
  ut.MAC_Sequence_Records(fuzzies_counted0,id,sequenced)
  SALT30.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
EXPORT name_suffix_values_persisted_temp := specs_added;
 
EXPORT  contact_ssn_deduped := SALT30.MAC_Field_By_UID(input_file,DOTid,contact_ssn); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(contact_ssn_deduped,contact_ssn,DOTid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  ...
  with_id := table(counted,r1);
  SALT30.MAC_Field_Accumulate_Counts(with_id,contact_ssn_Right4,Right4_cnt,fuzzies_counted0)
  ut.MAC_Sequence_Records(fuzzies_counted0,id,sequenced)
  SALT30.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
  SALT30.mac_edit_distance_pairs(specs_added,contact_ssn,cnt,1,false,distance_computed);//Computes specificities of fuzzy matches
EXPORT contact_ssn_values_persisted_temp := distance_computed;
 
EXPORT cnp_nameValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::cnp_name';
...
SALT30.MAC_Field_Bfoul(cnp_name_deduped,cnp_name,DOTid,cnp_name_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT corp_legal_nameValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::corp_legal_name';
...
SALT30.MAC_Field_Bfoul(corp_legal_name_deduped,corp_legal_name,DOTid,corp_legal_name_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
EXPORT corp_legal_name_switch := bf;
EXPORT corp_legal_name_max := MAX(corp_legal_name_values_persisted,field_specificity);
SALT30.MAC_Field_Specificity(corp_legal_name_values_persisted,corp_legal_name,corp_legal_name_nulls,ol) // Compute column level specificity
EXPORT corp_legal_name_specificity := ol;
 
EXPORT cnp_numberValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::cnp_number';
...
SALT30.MAC_Field_Bfoul(cnp_number_deduped,cnp_number,DOTid,cnp_number_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT cnp_btypeValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::cnp_btype';
...
SALT30.MAC_Field_Bfoul(cnp_btype_deduped,cnp_btype,DOTid,cnp_btype_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT company_feinValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::company_fein';
...
SALT30.MAC_Field_Bfoul(company_fein_deduped,company_fein,DOTid,company_fein_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT active_duns_numberValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::active_duns_number';
...
SALT30.MAC_Field_Bfoul(active_duns_number_deduped,active_duns_number,DOTid,active_duns_number_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT active_enterprise_numberValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::active_enterprise_number';
...
SALT30.MAC_Field_Bfoul(active_enterprise_number_deduped,active_enterprise_number,DOTid,active_enterprise_number_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT active_domestic_corp_keyValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::active_domestic_corp_key';
...
SALT30.MAC_Field_Bfoul(active_domestic_corp_key_deduped,active_domestic_corp_key,DOTid,active_domestic_corp_key_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT prim_rangeValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::prim_range';
...
SALT30.MAC_Field_Bfoul(prim_range_deduped,prim_range,DOTid,prim_range_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT prim_nameValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::prim_name';
...
SALT30.MAC_Field_Bfoul(prim_name_deduped,prim_name,DOTid,prim_name_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT sec_rangeValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::sec_range';
...
SALT30.MAC_Field_Bfoul(sec_range_deduped,sec_range,DOTid,sec_range_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT stValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::st';
...
EXPORT st_nulls := nv;
SALT30.MAC_Field_Bfoul(st_deduped,st,DOTid,st_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
EXPORT st_switch := bf;
...
EXPORT v_city_nameValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::v_city_name';
...
SALT30.MAC_Field_Bfoul(v_city_name_deduped,v_city_name,DOTid,v_city_name_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT zipValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::zip';
...
EXPORT zip_nulls := nv;
SALT30.MAC_Field_Bfoul(zip_deduped,zip,DOTid,zip_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT cszValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::csz';
...
SALT30.MAC_Field_Nulls(csz_values_persisted,Layout_Specificities.csz_ChildRec,nv) // Use automated NULL spotting
EXPORT csz_nulls := nv;
SALT30.MAC_Field_Bfoul(csz_deduped,csz,DOTid,csz_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
EXPORT csz_switch := bf;
EXPORT csz_max := MAX(csz_values_persisted,field_specificity);
SALT30.MAC_Field_Specificity(csz_values_persisted,csz,csz_nulls,ol) // Compute column level specificity
EXPORT csz_specificity := ol;
 
EXPORT addr1ValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::addr1';
...
SALT30.MAC_Field_Bfoul(addr1_deduped,addr1,DOTid,addr1_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT addressValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::address';
...
SALT30.MAC_Field_Bfoul(address_deduped,address,DOTid,address_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT isContactValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::isContact';
...
SALT30.MAC_Field_Bfoul(isContact_deduped,isContact,DOTid,isContact_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
EXPORT isContact_switch := bf;
EXPORT isContact_max := MAX(isContact_values_persisted,field_specificity);
SALT30.MAC_Field_Specificity(isContact_values_persisted,isContact,isContact_nulls,ol) // Compute column level specificity
EXPORT isContact_specificity := ol;
 
EXPORT fnameValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::fname';
...
SALT30.MAC_Field_Bfoul(fname_deduped,fname,DOTid,fname_nulls,ClusterSizes,true,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT mnameValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::mname';
...
SALT30.MAC_Field_Bfoul(mname_deduped,mname,DOTid,mname_nulls,ClusterSizes,true,false,bf) // Compute the chances of a field having 2 values for one entity
EXPORT mname_switch := bf;
EXPORT mname_max := MAX(mname_values_persisted,field_specificity);
SALT30.MAC_Field_Specificity(mname_values_persisted,mname,mname_nulls,ol) // Compute column level specificity
EXPORT mname_specificity := ol;
 
EXPORT lnameValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::lname';
...
SALT30.MAC_Field_Bfoul(lname_deduped,lname,DOTid,lname_nulls,ClusterSizes,false,true,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT name_suffixValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::name_suffix';
...
SALT30.MAC_Field_Nulls(name_suffix_values_persisted,Layout_Specificities.name_suffix_ChildRec,nv) // Use automated NULL spotting
EXPORT name_suffix_nulls := nv;
SALT30.MAC_Field_Bfoul(name_suffix_deduped,name_suffix,DOTid,name_suffix_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
EXPORT name_suffix_switch := bf;
EXPORT name_suffix_max := MAX(name_suffix_values_persisted,field_specificity);
SALT30.MAC_Field_Specificity(name_suffix_values_persisted,name_suffix,name_suffix_nulls,ol) // Compute column level specificity
EXPORT name_suffix_specificity := ol;
 
EXPORT contact_ssnValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::contact_ssn';
...
SALT30.MAC_Field_Bfoul(contact_ssn_deduped,contact_ssn,DOTid,contact_ssn_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT contact_ssn_specificity := ol;
  ...
  r := RECORD
    ...
    SALT30.UIDType DOTid := infile.dotid;
    UNSIGNED Basis_cnt := 0;
    INTEGER2 Basis_weight100 := 0;
  END;
  t := TABLE(infile,r);
SHARED ForeignCorpkey_attributes := DEDUP( SORT( DISTRIBUTE( t, DOTid ), DOTid, Basis, LOCAL), DOTid, Basis, LOCAL) : PERSIST('~temp::DOTid::BIPV2_DOTID_dev::values::ForeignCorpkey',EXPIRE(Config.PersistExpire));
  SALT30.Mac_Specificity_Local(ForeignCorpkey_attributes,Basis,DOTid,ForeignCorpkey_nulls,Layout_Specificities.ForeignCorpkey_ChildRec,ForeignCorpkey_specificity,ForeignCorpkey_switch,ForeignCorpkey_values);
EXPORT ForeignCorpkey_max := MAX(ForeignCorpkey_values,field_specificity);
 
EXPORT ForeignCorpkeyValuesIndexKeyName := '~'+'key::BIPV2_DOTID_dev::DOTid::Word::ForeignCorpkey';
...
// Service functions for specificity profiling
  ...
  SALT30.MAC_Specificity_Values(corp_legal_name_values_persisted,corp_legal_name,'corp_legal_name',corp_legal_name_specificity,corp_legal_name_specificity_profile);
  ...
  SALT30.MAC_Specificity_Values(isContact_values_persisted,isContact,'isContact',isContact_specificity,isContact_specificity_profile);
  SALT30.MAC_Specificity_Values(fname_values_persisted,fname,'fname',fname_specificity,fname_specificity_profile);
  SALT30.MAC_Specificity_Values(mname_values_persisted,mname,'mname',mname_specificity,mname_specificity_profile);
  SALT30.MAC_Specificity_Values(lname_values_persisted,lname,'lname',lname_specificity,lname_specificity_profile);
  SALT30.MAC_Specificity_Values(name_suffix_values_persisted,name_suffix,'name_suffix',name_suffix_specificity,name_suffix_specificity_profile);

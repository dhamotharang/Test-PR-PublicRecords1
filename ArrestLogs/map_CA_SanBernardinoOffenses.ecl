...
Crim_Common.Layout_In_Court_Offenses tSanBernardino(fSanBernardino input) := Transform
...
self.source_file 				:= '(CV)CA-SanBernardinoArrest';
self.off_comp 					:= '';
...
self.num_of_counts 				:= '';
self.le_agency_cd 				:= '';
self.le_agency_desc 			:= if(trim(input.Arrest_Agency,left,right)<>'XXXXXXXXXX',
									trim(input.Arrest_Agency,left,right),
									'');
...
self.arr_off_lev 				:= if(trim(input.charge_type,left,right) = 'FEL', 'FEL',
									if(trim(input.charge_type,left,right) = 'MISD', 'MISD',
									''));
self.arr_statute 				:= '';//if(input.charge_code<>'""',input.charge_code,'');
...
self.arr_disp_date 				:= if(fSlashedMDYtoCYMD(input.arrest_dt) < Crim_Common.Version_In_Arrest_Offenses and
									  ...
									  '');
self.arr_disp_code 				:= '';
self.arr_disp_desc_1 			:= trim(StringLib.StringToUpperCase(input.disposition),left,right);
self.arr_disp_desc_2 			:= if(input.bail_2 ~in ['0','','NO BAIL'],
									 ('BAIL AMT '+'$'+Arrestlogs.FormatMoney((integer)input.bail_2)),
								     '');
self.pros_refer_cd 				:= '';
self.pros_refer 				:= '';
self.pros_assgn_cd 				:= '';
...
arrOut:= pSanBernardino + ArrestLogs.FileAbinitioOffenses(vendor='A2');
...
dd_arrOut:= dedup(sort(distribute(arrOut,hash(offender_key)),
									...
									PERSIST('~thor_dell400::persist::ArrestLogs_CA_SanBernardino_Offenses');

export map_CA_SanBernardinoOffenses	:= dd_arrOut;
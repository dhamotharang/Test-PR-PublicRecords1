...
d	:= ArrestLogs.file_CA_Tehama;

fTehama := d(trim(d.Name,left,right)<>'Name');

Crim_Common.Layout_In_Court_Offenses tTehama(fTehama input) := Transform
...
self.traffic_dl_st 				:= '';
self.arr_off_code 				:= '';
self.arr_off_desc_1 			:= StringLib.StringToUpperCase(input.offense2);
self.arr_off_desc_2 			:= '';
...
//arrOut:= pTehama + ArrestLogs.FileAbinitioOffenses(vendor='');

dd_arrOut:= dedup(sort(distribute(pTehama,hash(offender_key)),
									...
									PERSIST('~thor_dell400::persist::ArrestLogs_CA_Tehama_Offenses');

export map_CA_TehamaOffenses := dd_arrOut;
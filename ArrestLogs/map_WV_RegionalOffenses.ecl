...
self.arr_off_lev 				:= if(regexfind('Misdemeanor',input.imprison_status,0)<>'','M',
									if(regexfind('Felon',input.imprison_status,0)<>'','F',
									''));
self.arr_statute 				:= '';
...
dd_arrOut:= dedup(sort(distribute(pRegional,hash(offender_key)),
									...
									PERSIST('~thor_dell400::persist::ArrestLogs_WV_Regional_Offenses');

export Map_WV_RegionalOffenses := dd_arrOut;
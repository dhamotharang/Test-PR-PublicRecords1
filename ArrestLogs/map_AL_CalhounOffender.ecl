...
p	:= ArrestLogs.file_AL_Calhoun.cmbnd(ID <>'ID');

Crim_Common.Layout_In_Court_Offender tCalhoun(p input) := Transform
...
self.case_type			:= '';
self.case_type_desc		:= '';
self.case_filing_dt		:= if(fSlashedMDYtoCYMD(input.Booking_Dt) < Crim_Common.Version_In_Arrest_Offender
...
pCalhoun := project(p, tCalhoun(left));

ArrestLogs.ArrestLogs_clean(pCalhoun,cleanCalhoun);

arrOut:= cleanCalhoun;
// + ArrestLogs.FileAbinitioOffender(vendor='AD');
...
dd_arrOut:= dedup(sort(distribute(arrOut,hash(offender_key)),
										...
										PERSIST('~thor_dell400::persist::Arrestlogs_AL_Calhoun_Offender');

export map_AL_CalhounOffender := dd_arrOut;
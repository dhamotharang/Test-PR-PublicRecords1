...
d	:= ArrestLogs.file_OR_Yamhill;

fYamhill := d(trim(d.Name,left,right)<>'Name');

Crim_Common.Layout_In_Court_Offenses tYamhill(fYamhill input) := Transform
...
self.traffic_dl_st 				:= '';
self.arr_off_code 				:= '';
self.arr_off_desc_1 			:= regexreplace('[0-9]+|'+' CT |'+'CT.|'+'MEASURE|'+'@|'+'OSB|'+'#|'+' I |'+' II|'+ ' III|'+'IV|'+'$|'+':|'+'\\*',input.offense2,'');
self.arr_off_desc_2 			:= '';
...
self.arr_disp_desc_2 			:= if(trim(input.total_bail,all) ~in ['0','$0.00',''],
...
//arrOut:= pYamhill + ArrestLogs.FileAbinitioOffenses(vendor='');

dd_arrOut:= dedup(sort(distribute(pYamhill,hash(offender_key)),
									...
									PERSIST('~thor_dell400::persist::ArrestLogs_OR_Yamhill_Offenses');

export Map_OR_YamhillOffenses := dd_arrOut;
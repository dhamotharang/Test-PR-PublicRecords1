...
EXPORT MAC_PopulationStatistics(infile,Ref='',Input_corp_key = '',Input_corp_orig_sos_charter_nbr = '',Input_corp_legal_name = '',Input_dt_first_seen = '',Input_dt_last_seen = '',Input_corp_process_date = '',Input_corp_ra_dt_first_seen = '',Input_corp_ra_dt_last_seen = '',Input_dt_vendor_first_reported = '',Input_dt_vendor_last_reported = '',Input_corp_trademark_first_use_date = '',Input_corp_trademark_first_use_date_in_state = '',Input_corp_trademark_expiration_date = '',Input_corp_trademark_filing_date = '',Input_corp_vendor = '',Input_corp_state_origin = '',Input_corp_inc_state = '',Input_corp_trademark_status = '',OutFile) := MACRO
...
+    #IF( #TEXT(Input_corp_trademark_first_use_date)='' )
      '' 
    #ELSE
        IF( (unsigned)le.Input_corp_trademark_first_use_date = 0,'', ':corp_trademark_first_use_date(' + SALT32.fn_date_valid_as_text((unsigned)le.Input_corp_trademark_first_use_date) + ')' )
    #END
 
+    #IF( #TEXT(Input_corp_trademark_first_use_date_in_state)='' )
      '' 
    #ELSE
        IF( (unsigned)le.Input_corp_trademark_first_use_date_in_state = 0,'', ':corp_trademark_first_use_date_in_state(' + SALT32.fn_date_valid_as_text((unsigned)le.Input_corp_trademark_first_use_date_in_state) + ')' )
    #END
 
+    #IF( #TEXT(Input_corp_trademark_expiration_date)='' )
      '' 
    #ELSE
        IF( (unsigned)le.Input_corp_trademark_expiration_date = 0,'', ':corp_trademark_expiration_date(' + SALT32.fn_date_valid_as_text((unsigned)le.Input_corp_trademark_expiration_date) + ')' )
    #END
 
+    #IF( #TEXT(Input_corp_trademark_filing_date)='' )
      '' 
    #ELSE
        IF( (unsigned)le.Input_corp_trademark_filing_date = 0,'', ':corp_trademark_filing_date(' + SALT32.fn_date_valid_as_text((unsigned)le.Input_corp_trademark_filing_date) + ')' )
    #END

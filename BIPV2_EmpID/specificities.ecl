...
EXPORT input_layout := RECORD // project out required fields
  ...
  TYPEOF(h.cname_devanitize) cname_devanitize := (TYPEOF(h.cname_devanitize))Fields.Make_cname_devanitize((SALT32.StrType)h.cname_devanitize ); // Cleans before using
  UNSIGNED1 cname_devanitize_len := LENGTH(TRIM((SALT32.StrType)h.cname_devanitize));
  h.prim_range;
  h.prim_name;
  h.zip;
  h.zip4;
...
SHARED reject := Fields.Invalid_isCorpEnhanced((SALT32.StrType)h0.isCorpEnhanced)>0 OR Fields.Invalid_contact_job_title_derived((SALT32.StrType)h0.contact_job_title_derived)>0 OR Fields.Invalid_zip4((SALT32.StrType)h0.zip4)>0;
EXPORT rejected_file := h0(reject);
 
EXPORT input_file_np := h0; // No-persist version for remote_linking
 
EXPORT input_file := h0(~Reject) : PERSIST('~temp::EmpID::BIPV2_EmpID::Specificities_Cache',EXPIRE(Config.PersistExpire));
//We have EmpID specified - so we can compute statistics on the cluster counts
  r0 := RECORD
    input_file.EmpID;
    UNSIGNED6 InCluster := COUNT(GROUP);
  end;
EXPORT ClusterSizes := TABLE(input_file,r0,EmpID,LOCAL) : PERSIST('~temp::EmpID::BIPV2_EmpID::Cluster_Sizes',EXPIRE(Config.PersistExpire));
EXPORT TotalClusters := COUNT(ClusterSizes);
 
EXPORT  cname_devanitize_deduped := SALT32.MAC_Field_By_UID(input_file,EmpID,cname_devanitize); // Reduce to field values by UID
  SALT32.Mac_Field_Count_UID(cname_devanitize_deduped,cname_devanitize,EmpID,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
...
SHARED cname_devanitize_sa := specs_added; // Hop over shared/export below
// This field is a word bag; so create specificities for the words too
  SALT32.MAC_Field_Variants_WordBag(cname_devanitize_deduped,EmpID,cname_devanitize,expanded)// expand out all variants of wordbag
  SALT32.Mac_Field_Count_UID(expanded,cname_devanitize,EmpID,counted,counted_clusters) // count the number of UIDs with each field value
...
EXPORT cname_devanitizeValuesKeyName := '~'+'key::BIPV2_EmpID::EmpID::WordBag::cname_devanitize';
...
EXPORT cname_devanitize_values_key := INDEX(cname_devanitize_ad,{cname_devanitize},{cname_devanitize_ad},cname_devanitizeValuesKeyName);
  SALT32.mac_wordbag_addweights(cname_devanitize_sa,cname_devanitize,cname_devanitize_ad,p);
EXPORT cname_devanitize_values_persisted_temp := p;
 
EXPORT  prim_range_deduped := SALT32.MAC_Field_By_UID(input_file,EmpID,prim_range); // Reduce to field values by UID
  SALT32.Mac_Field_Count_UID(prim_range_deduped,prim_range,EmpID,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT32.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
EXPORT prim_range_values_persisted_temp := specs_added : PERSIST('~temp::EmpID::BIPV2_EmpID::values::prim_range',EXPIRE(Config.PersistExpire));
 
EXPORT  prim_name_deduped := SALT32.MAC_Field_By_UID(input_file,EmpID,prim_name); // Reduce to field values by UID
  SALT32.Mac_Field_Count_UID(prim_name_deduped,prim_name,EmpID,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT32.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
EXPORT prim_name_values_persisted_temp := specs_added : PERSIST('~temp::EmpID::BIPV2_EmpID::values::prim_name',EXPIRE(Config.PersistExpire));
 
EXPORT  zip_deduped := SALT32.MAC_Field_By_UID(input_file,EmpID,zip); // Reduce to field values by UID
  SALT32.Mac_Field_Count_UID(zip_deduped,zip,EmpID,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT32.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
EXPORT zip_values_persisted_temp := specs_added : PERSIST('~temp::EmpID::BIPV2_EmpID::values::zip',EXPIRE(Config.PersistExpire));
...
EXPORT  fname_deduped := SALT32.MAC_Field_By_UID(input_file,EmpID,fname); // Reduce to field values by UID
  ...
  SALT32.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
  SALT32.MAC_Field_Initial_Specificities(specs_added,fname,initial_specs_added) // add initial char specificities
EXPORT fname_values_persisted_temp := initial_specs_added : PERSIST('~temp::EmpID::BIPV2_EmpID::values::fname',EXPIRE(Config.PersistExpire));
 
EXPORT  lname_deduped := SALT32.MAC_Field_By_UID(input_file,EmpID,lname); // Reduce to field values by UID
  SALT32.Mac_Field_Count_UID(lname_deduped,lname,EmpID,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT32.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
EXPORT lname_values_persisted_temp := specs_added : PERSIST('~temp::EmpID::BIPV2_EmpID::values::lname',EXPIRE(Config.PersistExpire));
 
EXPORT  contact_phone_deduped := SALT32.MAC_Field_By_UID(input_file,EmpID,contact_phone); // Reduce to field values by UID
  SALT32.Mac_Field_Count_UID(contact_phone_deduped,contact_phone,EmpID,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT32.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
EXPORT contact_phone_values_persisted_temp := specs_added : PERSIST('~temp::EmpID::BIPV2_EmpID::values::contact_phone',EXPIRE(Config.PersistExpire));
...
EXPORT  contact_did_deduped := SALT32.MAC_Field_By_UID(input_file,EmpID,contact_did); // Reduce to field values by UID
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT32.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
EXPORT contact_did_values_persisted_temp := specs_added : PERSIST('~temp::EmpID::BIPV2_EmpID::values::contact_did',EXPIRE(Config.PersistExpire));
...
EXPORT  contact_ssn_deduped := SALT32.MAC_Field_By_UID(input_file,EmpID,contact_ssn); // Reduce to field values by UID
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT32.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
EXPORT contact_ssn_values_persisted_temp := specs_added : PERSIST('~temp::EmpID::BIPV2_EmpID::values::contact_ssn',EXPIRE(Config.PersistExpire));
...
EXPORT  dt_first_seen_deduped := SALT32.MAC_Field_By_UID(input_file,EmpID,dt_first_seen); // Reduce to field values by UID
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT32.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
EXPORT dt_first_seen_values_persisted_temp := specs_added : PERSIST('~temp::EmpID::BIPV2_EmpID::values::dt_first_seen',EXPIRE(Config.PersistExpire));
...
EXPORT  dt_last_seen_deduped := SALT32.MAC_Field_By_UID(input_file,EmpID,dt_last_seen); // Reduce to field values by UID
  ...
  ut.MAC_Sequence_Records(with_id,id,sequenced)
  SALT32.MAC_Field_Specificities(sequenced,specs_added) // Compute specificity for each value
EXPORT dt_last_seen_values_persisted_temp := specs_added : PERSIST('~temp::EmpID::BIPV2_EmpID::values::dt_last_seen',EXPIRE(Config.PersistExpire));
 
EXPORT BuildBOWFields := BUILDINDEX(cname_devanitize_values_key, OVERWRITE);
 
EXPORT cname_devanitizeValuesIndexKeyName := '~'+'key::BIPV2_EmpID::EmpID::Word::cname_devanitize';
...
SALT32.MAC_Field_Bfoul(cname_devanitize_deduped,cname_devanitize,EmpID,cname_devanitize_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
EXPORT cname_devanitize_switch := bf;
EXPORT cname_devanitize_max := MAX(cname_devanitize_values_persisted,field_specificity);
SALT32.MAC_Field_Specificity(cname_devanitize_values_persisted,cname_devanitize,cname_devanitize_nulls,ol) // Compute column level specificity
EXPORT cname_devanitize_specificity := ol;
 
EXPORT prim_rangeValuesIndexKeyName := '~'+'key::BIPV2_EmpID::EmpID::Word::prim_range';
...
SALT32.MAC_Field_Bfoul(prim_range_deduped,prim_range,EmpID,prim_range_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT prim_nameValuesIndexKeyName := '~'+'key::BIPV2_EmpID::EmpID::Word::prim_name';
...
SALT32.MAC_Field_Bfoul(prim_name_deduped,prim_name,EmpID,prim_name_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT zipValuesIndexKeyName := '~'+'key::BIPV2_EmpID::EmpID::Word::zip';
...
EXPORT zip_nulls := nv;
SALT32.MAC_Field_Bfoul(zip_deduped,zip,EmpID,zip_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT fnameValuesIndexKeyName := '~'+'key::BIPV2_EmpID::EmpID::Word::fname';
...
EXPORT lnameValuesIndexKeyName := '~'+'key::BIPV2_EmpID::EmpID::Word::lname';
...
SALT32.MAC_Field_Bfoul(lname_deduped,lname,EmpID,lname_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT contact_phoneValuesIndexKeyName := '~'+'key::BIPV2_EmpID::EmpID::Word::contact_phone';
...
SALT32.MAC_Field_Nulls(contact_phone_values_persisted,Layout_Specificities.contact_phone_ChildRec,nv) // Use automated NULL spotting
EXPORT contact_phone_nulls := nv;
SALT32.MAC_Field_Bfoul(contact_phone_deduped,contact_phone,EmpID,contact_phone_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
EXPORT contact_phone_switch := bf;
EXPORT contact_phone_max := MAX(contact_phone_values_persisted,field_specificity);
SALT32.MAC_Field_Specificity(contact_phone_values_persisted,contact_phone,contact_phone_nulls,ol) // Compute column level specificity
EXPORT contact_phone_specificity := ol;
 
EXPORT contact_didValuesIndexKeyName := '~'+'key::BIPV2_EmpID::EmpID::Word::contact_did';
...
EXPORT contact_ssnValuesIndexKeyName := '~'+'key::BIPV2_EmpID::EmpID::Word::contact_ssn';
...
EXPORT dt_first_seenValuesIndexKeyName := '~'+'key::BIPV2_EmpID::EmpID::Word::dt_first_seen';
...
EXPORT dt_last_seenValuesIndexKeyName := '~'+'key::BIPV2_EmpID::EmpID::Word::dt_last_seen';
...
// Service functions for specificity profiling
  SALT32.MAC_Specificity_Values(cname_devanitize_values_persisted,cname_devanitize,'cname_devanitize',cname_devanitize_specificity,cname_devanitize_specificity_profile);
  ...
  SALT32.MAC_Specificity_Values(contact_phone_values_persisted,contact_phone,'contact_phone',contact_phone_specificity,contact_phone_specificity_profile);

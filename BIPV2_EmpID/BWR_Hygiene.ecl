...
#workunit('name','BIPV2_EmpID.BWR_Hygiene - Hygiene & Stats - SALT V3.2.0');
IMPORT BIPV2_EmpID,SALT32;
// First create an instantiated hygiene module
  infile := BIPV2_EmpID.In_EmpID;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := BIPV2_EmpID.Hygiene(ip);
  ...
  CM := BIPV2_EmpID.Fields.UIDConsistency(BIPV2_EmpID.In_EmpID);
...
//  OUTPUT(CM.OrgID_TwoParents,NAMED('TwoparentsOrgID'));
  ...
  //  OUTPUT(SALT32.MAC_CrossTab(infile,source,cname_devanitize,Examples),NAMED('cname_devanitizeBysource'));
  ...
  //  OUTPUT(SALT32.MAC_CrossTab(infile,source,isCorpEnhanced,Examples),NAMED('isCorpEnhancedBysource'));
  //  OUTPUT(SALT32.MAC_CrossTab(infile,source,contact_job_title_derived,Examples),NAMED('contact_job_title_derivedBysource'));

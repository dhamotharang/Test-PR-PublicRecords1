...
EXPORT layout_sample_matches sample_match_join(match_candidates(ih).layout_candidates le,match_candidates(ih).layout_candidates ri,UNSIGNED c=0,UNSIGNED outside=0) := TRANSFORM
  ...
  SELF.contact_phone_match_code := MAP(
		le.contact_phone_isnull OR ri.contact_phone_isnull => SALT32.MatchCode.OneSideNull,
  ...
  SELF.cname_devanitize_match_code := MAP(
		le.cname_devanitize_isnull OR ri.cname_devanitize_isnull => SALT32.MatchCode.OneSideNull,
		match_methods(ih).match_cname_devanitize(le.cname_devanitize,ri.cname_devanitize, le.cname_devanitize_len, ri.cname_devanitize_len));
...
EXPORT Layout_RolledEntity := RECORD,MAXLENGTH(63000)
  ...
  DATASET(SALT32.Layout_FieldValueList) cname_devanitize_Values := DATASET([],SALT32.Layout_FieldValueList);
  ...
  DATASET(SALT32.Layout_FieldValueList) isCorpEnhanced_Values := DATASET([],SALT32.Layout_FieldValueList);
  DATASET(SALT32.Layout_FieldValueList) contact_job_title_derived_Values := DATASET([],SALT32.Layout_FieldValueList);
...
SHARED RollEntities(dataset(Layout_RolledEntity) infile) := FUNCTION
  ...
  SELF.EmpID := le.EmpID;
    ...
    SELF.cname_devanitize_values := SALT32.fn_combine_fieldvaluelist(le.cname_devanitize_values,ri.cname_devanitize_values);
    ...
    SELF.isCorpEnhanced_values := SALT32.fn_combine_fieldvaluelist(le.isCorpEnhanced_values,ri.isCorpEnhanced_values);
    SELF.contact_job_title_derived_values := SALT32.fn_combine_fieldvaluelist(le.contact_job_title_derived_values,ri.contact_job_title_derived_values);
  ...
  Layout_RolledEntity SortValues(Layout_RolledEntity le) := TRANSFORM
    ...
    SELF.cname_devanitize_values := SORT(le.cname_devanitize_values, -cnt, val, LOCAL);
    ...
    SELF.isCorpEnhanced_values := SORT(le.isCorpEnhanced_values, -cnt, val, LOCAL);
    SELF.contact_job_title_derived_values := SORT(le.contact_job_title_derived_values, -cnt, val, LOCAL);
...
Layout_RolledEntity into(in_data le) := TRANSFORM
  ...
  SELF.contact_phone_Values := IF ( le.contact_phone  IN SET(s.nulls_contact_phone,contact_phone) OR le.contact_phone = (TYPEOF(le.contact_phone))'',DATASET([],SALT32.Layout_FieldValueList),DATASET([{TRIM((SALT32.StrType)le.contact_phone)}],SALT32.Layout_FieldValueList));
  ...
  SELF.cname_devanitize_Values := IF ( le.cname_devanitize  IN SET(s.nulls_cname_devanitize,cname_devanitize) OR le.cname_devanitize = (TYPEOF(le.cname_devanitize))'',DATASET([],SALT32.Layout_FieldValueList),DATASET([{TRIM((SALT32.StrType)le.cname_devanitize)}],SALT32.Layout_FieldValueList));
  ...
  SELF.isCorpEnhanced_Values := DATASET([{TRIM((SALT32.StrType)le.isCorpEnhanced)}],SALT32.Layout_FieldValueList);
  SELF.contact_job_title_derived_Values := DATASET([{TRIM((SALT32.StrType)le.contact_job_title_derived)}],SALT32.Layout_FieldValueList);
  ...
  SELF.cnp_name_Values := DATASET([{TRIM((SALT32.StrType)le.cnp_name)}],SALT32.Layout_FieldValueList);
...
Layout_Chubbies0 NoteSize(Layout_Chubbies0 le) := TRANSFORM
  ...
  SELF.contact_phone_size := SALT32.Fn_SwitchSpec(s.contact_phone_switch,count(le.contact_phone_values));
  SELF.contact_did_size := SALT32.Fn_SwitchSpec(s.contact_did_switch,count(le.contact_did_values));
  SELF.cname_devanitize_size := SALT32.Fn_SwitchSpec(s.cname_devanitize_switch,count(le.cname_devanitize_values));

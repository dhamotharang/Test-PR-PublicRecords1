//Executable code
EXPORT Proc_Iterate(string pversion/*HACK*/,string iter,DATASET(Layout_EmpID) InFile0 = BIPV2_EmpID.In_EmpID,STRING OutFileNameP = '~temp::EmpID::BIPV2_EmpID::it',UNSIGNED MatchThreshold = Config.MatchThreshold,BOOLEAN Debugging = true) := MODULE
SHARED InFile := InFile0;
SHARED MM := BIPV2_EmpID.matches(InFile,MatchThreshold); // Get the matching module
OSR := OUTPUT(CHOOSEN(MM.MatchSampleRecords,10000),NAMED('MatchSampleRecords'));
EXPORT OutputSamples := OSR;// Provide the records!
SHARED BM := BIPV2_EmpID.BasicMatch(InFile);
...
OS := OUTPUT(BIPV2_EmpID.specificities(InFile).specificities,NAMED('Specificities'));
OPRP := OUTPUT(BIPV2_EmpID.match_candidates(InFile).prepropogationstats,NAMED('PrePopStats'));
OPP := OUTPUT(BIPV2_EmpID.match_candidates(InFile).postpropogationstats,NAMED('PostPopStats'));
...
MP := PARALLEL( OUTPUT(MM.MatchesPerformed,NAMED('MatchesPerformed')),OUTPUT(BIPV2_EmpID.BasicMatch(InFile).basic_match_count,NAMED('BasicMatchesPerformed')));
...
SpcS := OUTPUT(BIPV2_EmpID.specificities(InFile).SpcShift,NAMED('SPCShift'));
EXPORT ExecutionStats := PARALLEL(OS,SpcS,PRPP,PPP,PRPPS,PPPS,MP,RE,CB,MPPA,MPPR,OPRP,OPP);
d := DATASET([{MM.PatchingError0,MM.DuplicateRids0,COUNT(BIPV2_EmpID.match_candidates(InFile).Unlinkables),COUNT(BIPV2_EmpID.specificities(InFile).Rejected_file)}],{INTEGER PatchingError0, INTEGER DuplicateRids0, UNSIGNED UnlinkableRecords0,UNSIGNED RecordsRejected0});
...
EXPORT OutputChangesA := SEQUENTIAL( IF(FileServices.SuperFileExists(BIPV2_EmpID.Keys(InFile).MatchHistoryName),FileServices.RemoveSuperFile(BIPV2_EmpID.Keys(InFile).MatchHistoryName,ChangeName)),OUTPUT(MM.IdChanges,,ChangeName,OVERWRITE,COMPRESSED),FileServices.AddSuperFile(BIPV2_EmpID.Keys(InFile).MatchHistoryName,ChangeName));
//EXPORT LoopN(UNSIGNED N,UNSIGNED mt=Config.MatchThreshold) := LOOP(InFile,N,BIPV2_EmpID.matches(ROWS(LEFT),mt).patched_infile); // Loop N times
//EXPORT LoopThisN(d,N,mt=Config.MatchThreshold,ThinLoop=FALSE) := FUNCTIONMACRO
//	IMPORT BIPV2_EmpID;
//	#IF (ThinLoop=FALSE)
//		RETURN LOOP(PROJECT(d,BIPV2_EmpID.Layout_EmpID),N,BIPV2_EmpID.matches(ROWS(LEFT),mt).patched_infile);
//	#ELSE
//		RETURN JOIN(d, LOOP(PROJECT(d,BIPV2_EmpID.Layout_EmpID),N,BIPV2_EmpID.matches(ROWS(LEFT),mt).patched_infile), LEFT.rcid=RIGHT.rcid, TRANSFORM(RECORDOF(LEFT),SELF:=RIGHT,SELF:=LEFT), KEEP(1), SMART);
//	#END
//ENDMACRO;
//EXPORT LoopThisK(d,K,NMax=100,ThinLoop=FALSE) := FUNCTIONMACRO
//	IMPORT BIPV2_EmpID;
//	#IF (ThinLoop=FALSE)
//		RETURN LOOP(PROJECT(d,BIPV2_EmpID.Layout_EmpID),COUNTER<=NMax AND COUNT(DEDUP(ROWS(LEFT),EmpID,ALL))>K,BIPV2_EmpID.matches(ROWS(LEFT),0).patched_infile);
//	#ELSE
//		RETURN JOIN(d, LOOP(PROJECT(d,BIPV2_EmpID.Layout_EmpID),COUNTER<=NMax AND COUNT(DEDUP(ROWS(LEFT),EmpID,ALL))>K,BIPV2_EmpID.matches(ROWS(LEFT),0).patched_infile), LEFT.rcid=RIGHT.rcid, TRANSFORM(RECORDOF(LEFT),SELF:=RIGHT,SELF:=LEFT), KEEP(1), SMART);
//	#END
//ENDMACRO;

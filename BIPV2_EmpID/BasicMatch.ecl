...
SHARED  h00 := Specificities(ih).input_file;
  ...
  SHARED h00_match := h00( 
      0 + IF( cname_devanitize  IN SET(s.nulls_cname_devanitize,cname_devanitize) OR cname_devanitize = (TYPEOF(cname_devanitize))'', 0, 20 ) + IF( prim_range  IN SET(s.nulls_prim_range,prim_range) OR prim_range = (TYPEOF(prim_range))'', 0, 13 ) + IF( prim_name  IN SET(s.nulls_prim_name,prim_name) OR prim_name = (TYPEOF(prim_name))'', 0, 15 ) + IF( zip  IN SET(s.nulls_zip,zip) OR zip = (TYPEOF(zip))'', 0, 14 ) + IF( fname  IN SET(s.nulls_fname,fname) OR fname = (TYPEOF(fname))'', 0, 8 ) + IF( lname  IN SET(s.nulls_lname,lname) OR lname = (TYPEOF(lname))'', 0, 15 ) + IF( contact_phone  IN SET(s.nulls_contact_phone,contact_phone) OR contact_phone = (TYPEOF(contact_phone))'', 0, 24 ) + IF( contact_did  IN SET(s.nulls_contact_did,contact_did) OR contact_did = (TYPEOF(contact_did))'', 0, 24 ) + IF( contact_ssn  IN SET(s.nulls_contact_ssn,contact_ssn) OR contact_ssn = (TYPEOF(contact_ssn))'', 0, 4 ) >= Config.BasicMatchThreshold); // Potentially remove anything that would violate FORCE(+) constraints

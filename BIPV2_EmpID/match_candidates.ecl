...
SHARED thin_table := DISTRIBUTE(TABLE(h00(SALT_Partition<>'*'),{rcid,isCorpEnhanced,contact_job_title_derived,cname_devanitize,cname_devanitize_len,prim_range,prim_name,zip,zip4,fname,lname,contact_phone,contact_did,contact_ssn,company_name,sec_range,v_city_name,st,company_inc_state,company_charter_number,active_duns_number,hist_duns_number,active_domestic_corp_key,hist_domestic_corp_key,foreign_corp_key,unk_corp_key,company_fein,cnp_btype,cnp_name,company_name_type_derived,company_bdid,nodes_total,dt_first_seen,dt_last_seen,SALT_Partition,UltID,OrgID,EmpID}),HASH(EmpID));
 
//Prepare for field propagations ...
PrePropCounts := RECORD
  REAL8 cname_devanitize_pop := AVE(GROUP,IF(thin_table.cname_devanitize  IN SET(s.nulls_cname_devanitize,cname_devanitize) OR thin_table.cname_devanitize = (TYPEOF(thin_table.cname_devanitize))'',0,100));
  ...
  REAL8 contact_phone_pop := AVE(GROUP,IF(thin_table.contact_phone  IN SET(s.nulls_contact_phone,contact_phone) OR thin_table.contact_phone = (TYPEOF(thin_table.contact_phone))'',0,100));
...
SALT32.mac_prop_field(with_props(cname_devanitize NOT IN SET(s.nulls_cname_devanitize,cname_devanitize)),cname_devanitize,EmpID,cname_devanitize_props); // For every DID find the best FULL cname_devanitize
layout_withpropvars take_cname_devanitize(with_props le,cname_devanitize_props ri) := TRANSFORM
  ...
  SELF.cname_devanitize_len := IF ( le.cname_devanitize IN SET(s.nulls_cname_devanitize,cname_devanitize) and ri.EmpID<>(TYPEOF(ri.EmpID))'', LENGTH(TRIM(ri.cname_devanitize)), le.cname_devanitize_len );
  SELF := le;
  END;
...
SALT32.mac_prop_field(with_props(prim_name NOT IN SET(s.nulls_prim_name,prim_name)),prim_name,EmpID,prim_name_props); // For every DID find the best FULL prim_name
...
pj4 do_computes(pj4 le) := TRANSFORM
  SELF := le;
END;
SHARED propogated := PROJECT(pj4,do_computes(left)) : PERSIST('~temp::EmpID::BIPV2_EmpID::mc_props::EmpID',EXPIRE(Config.PersistExpire)); // to allow to 'jump' over an exported value
PostPropCounts := RECORD
  REAL8 cname_devanitize_pop := AVE(GROUP,IF(propogated.cname_devanitize  IN SET(s.nulls_cname_devanitize,cname_devanitize) OR propogated.cname_devanitize = (TYPEOF(propogated.cname_devanitize))'',0,100));
  ...
  REAL8 contact_phone_pop := AVE(GROUP,IF(propogated.contact_phone  IN SET(s.nulls_contact_phone,contact_phone) OR propogated.contact_phone = (TYPEOF(propogated.contact_phone))'',0,100));
...
EXPORT PostPropogationStats := TABLE(propogated,PostPropCounts);
  Grpd := GROUP( DISTRIBUTE( TABLE( propogated, { propogated, UNSIGNED2 Buddies := 0 }),HASH(EmpID)),cname_devanitize,prim_range,prim_name,zip,fname,lname,contact_phone,contact_did,contact_ssn,dt_first_seen,dt_last_seen);
...
EXPORT Layout_Candidates := RECORD // A record to hold weights of each field value
  ...
  BOOLEAN cname_devanitize_isnull := h0.cname_devanitize  IN SET(s.nulls_cname_devanitize,cname_devanitize) OR h0.cname_devanitize = (TYPEOF(h0.cname_devanitize))''; // Simplify later processing 
  ...
  BOOLEAN contact_phone_isnull := h0.contact_phone  IN SET(s.nulls_contact_phone,contact_phone) OR h0.contact_phone = (TYPEOF(h0.contact_phone))''; // Simplify later processing 
...
layout_candidates add_fname(layout_candidates le,Specificities(ih).fname_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT32.MAC_Choose_JoinType(j8,s.nulls_fname,Specificities(ih).fname_values_persisted,fname,fname_weight100,add_fname,j7);
layout_candidates add_prim_range(layout_candidates le,Specificities(ih).prim_range_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT32.MAC_Choose_JoinType(j7,s.nulls_prim_range,Specificities(ih).prim_range_values_persisted,prim_range,prim_range_weight100,add_prim_range,j6);
...
layout_candidates add_lname(layout_candidates le,Specificities(ih).lname_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT32.MAC_Choose_JoinType(j5,s.nulls_lname,Specificities(ih).lname_values_persisted,lname,lname_weight100,add_lname,j4);
...
layout_candidates add_cname_devanitize(layout_candidates le,Specificities(ih).cname_devanitize_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  SELF.cname_devanitize_weight100 := MAP (le.cname_devanitize_isnull => 0, patch_default and ri.field_specificity=0 => s.cname_devanitize_maximum, ri.field_specificity) * 100; // If never seen before - must be rare
  SELF.cname_devanitize := IF( ri.field_specificity<>0 or ri.word<>'',SELF.cname_devanitize_weight100+' '+ri.word,SALT32.Fn_WordBag_AppendSpecs_Fake(le.cname_devanitize, s.cname_devanitize_specificity) );// Copy in annotated wordstring
  SELF := le;
END;
SALT32.MAC_Choose_JoinType(j3,s.nulls_cname_devanitize,Specificities(ih).cname_devanitize_values_persisted,cname_devanitize,cname_devanitize_weight100,add_cname_devanitize,j2);
layout_candidates add_contact_did(layout_candidates le,Specificities(ih).contact_did_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT32.MAC_Choose_JoinType(j2,s.nulls_contact_did,Specificities(ih).contact_did_values_persisted,contact_did,contact_did_weight100,add_contact_did,j1);
layout_candidates add_contact_phone(layout_candidates le,Specificities(ih).contact_phone_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT32.MAC_Choose_JoinType(j1,s.nulls_contact_phone,Specificities(ih).contact_phone_values_persisted,contact_phone,contact_phone_weight100,add_contact_phone,j0);
//Using HASH(did) to get smoother distribution
SHARED Annotated := DISTRIBUTE(j0,hash(EmpID)) : PERSIST('~temp::EmpID::BIPV2_EmpID::mc',EXPIRE(Config.PersistExpire)); // Distributed for keybuild case

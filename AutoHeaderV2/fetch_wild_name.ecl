...
export fetch_wild_name (dataset (AutoheaderV2.layouts.search) ds_search, integer search_code=0) := function
  ...
  // DOB-related values
	...
	index_Lev1 := project(i(lname_fil,
												fname_fil, 
												nofold(mname_fil),
												nofold(yob_fil),
												nofold(ssn4_fil or temp_ssn_value=''),
												nofold(gen_fil)), 
												transform (AutoheaderV2.layouts.search_out, self.did := left.did, Self.fetch_hit := AutoheaderV2.Constants.FetchHit.W_NAME));

	Fetch_Lev1 := AutoheaderV2.functions.FetchConstraints(index_Lev1,ut.limits.FETCH_KEYED,ut.limits.FETCH_UNKEYED,no_fail,203,,AutoheaderV2.Constants.FetchHit.W_NAME);
	
	final_res := if(first_fil,
									if(exists(Fetch_Lev1), 
											Fetch_Lev1,
											AutoheaderV2.functions.MakeDiagnosticDataset(AutoheaderV2.Constants.FetchHit.W_NAME, AutoheaderV2.Constants.STATUS._NOT_FOUND)),
									AutoheaderV2.functions.MakeDiagnosticDataset(AutoheaderV2.Constants.FetchHit.W_NAME, AutoheaderV2.Constants.Status._DATA));

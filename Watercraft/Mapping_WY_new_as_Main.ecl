...
Layout_WY_clean_temp main_mapping_temp(hull_clean_in L, watercraft.file_MIC R) := transform
  self := L;
  self := R;
  self.is_hull_id_in_MIC := if(trim(L.hull_id, left, right)[1..3] = R.MIC, true, false);
end;

Mapping_WY_as_Main_temp := join(hull_clean_in, 
                                watercraft.file_MIC, 
...
watercraft.Layout_Watercraft_Main_Base main_mapping_format(Mapping_WY_as_Main_temp L) := transform
	...
	self.propulsion_description				:=	TRIM(StringLib.StringToUpperCase(L.PROP),LEFT,RIGHT);
	
	self.vehicle_type_Code					:=	'';
	self.vehicle_type_Description			:=	TRIM(StringLib.StringToUpperCase(L.VEH_TYPE),LEFT,RIGHT);
	
	self.fuel_code							:=	'';
	self.fuel_description					:=	L.FUEL;
	
	self.hull_type_code						:=	'';
	self.hull_type_description				:=	TRIM(StringLib.StringToUpperCase(L.HULL),LEFT,RIGHT);
	
	self.use_code							:=	'';
	self.use_description					:=	TRIM(StringLib.StringToUpperCase(L.USE),LEFT,RIGHT);
	...
	STRING _clr := TRIM(L.COLORID,LEFT,RIGHT);
	self.watercraft_color_1_code			:=	_clr;
	self.watercraft_color_1_description		:=	MAP(_clr = '1' => 'SOLID',	
                                                    _clr = '2' => 'TWO-TONE',	
                                                    _clr = '3' => 'OTHER','');	
	...
	self.registration_renewal_date			:=	L.REG_DATE;
	self.decal_number						:=	'';
	...
	self.title_number						:=	'';
	self.title_issue_date					:=	L.PURCHASEDATE;
...
export Mapping_WY_new_as_Main := project(Mapping_WY_as_Main_temp, main_mapping_format(left));

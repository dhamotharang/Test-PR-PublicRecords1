...
Watercraft.Layout_Watercraft_Search_Group search_mapping_format(Watercraft.Layout_WV_clean_in L, integer1 C)
 :=
  transform
	self.date_first_seen			:=	if(L.reg_date = '19950318', L.title_date, if(L.reg_date > L.title_date,
	                                    L.title_date, L.reg_date));
	self.date_last_seen				:=	if(L.reg_date = '19950318', L.title_date, if(L.reg_date > L.title_date,
	                                    L.reg_date, L.title_date));
...
Mapping_WV_as_Search_norm			:= normalize(Watercraft.file_WV_clean_in,3,search_mapping_format(left,counter));

export Mapping_WV_as_Search         := Mapping_WV_as_Search_norm(clean_pname <> '' or company_name <> '');

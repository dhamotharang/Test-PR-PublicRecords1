EXPORT _Constants(BOOLEAN	pUseOtherEnvironment = FALSE) := MODULE
	...
	EXPORT autokey_buildskipset := ['S', 'F', 'C'];
	/*
	...
	*/
	...
	EXPORT ak_qa_keyname   := _Dataset(pUseOtherEnvironment).thor_cluster_files + 'key::clia::qa::autokey::';
	EXPORT statsTemplate   := lTemplate('stats');


  // CLIN Autokey Search variables
	EXPORT         TYPE_STR	       := 'AK';
	EXPORT BOOLEAN WORK_HARD       := TRUE; 
	EXPORT BOOLEAN NO_FAIL         := TRUE;
	EXPORT BOOLEAN USE_ALL_LOOKUPS := TRUE;
	EXPORT         AUTOKEY_NAME    := CLIA._Dataset().thor_cluster_files + 'key::' + CLIA._Dataset().name +
	                                     '::qa::autokey::';

...
OS := OUTPUT(HealthcareNoMatchHeader_InternalLinking./*HACK-O-MATIC*/Specificities(pSrc,pVersion,InFile).specificities,NAMED('Specificities'));
OPRP := OUTPUT(HealthcareNoMatchHeader_InternalLinking./*HACK-O-MATIC*/match_candidates(pSrc,pVersion,InFile).prepropogationstats,NAMED('PrePopStats'));
OPP := OUTPUT(HealthcareNoMatchHeader_InternalLinking./*HACK-O-MATIC*/match_candidates(pSrc,pVersion,InFile).postpropogationstats,NAMED('PostPopStats'));
PRPPS := OUTPUT(MM.PreIds.IdCounts,NAMED('PreClusterCount'));
PRPP := OUTPUT(MM.PreIds.nomatch_id_Clusters,NAMED('PreClusters'),ALL);
PPPS := OUTPUT(MM.PostIds.IdCounts,NAMED('PostClusterCount'));
PPP := OUTPUT(MM.PostIds.nomatch_id_Clusters,NAMED('PostClusters'),ALL);
SpcS := OUTPUT(HealthcareNoMatchHeader_InternalLinking./*HACK-O-MATIC*/Specificities(pSrc,pVersion,InFile).SpcShift,NAMED('SPCShift'));
...
EXPORT ValidityStats := PARALLEL( OUTPUT(dsValidityStats,NAMED('ValidityStatisticsIterate')), OUTPUT(MM.PostIds.Advanced0,NAMED('IdConsistency0')));

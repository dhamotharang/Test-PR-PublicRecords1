...
// 	LOADXML('<xml/>');
  ...
  #SET(Source         ,#TEXT(pSrc    ))
  #SET(Version        ,#TEXT(pVersion))
  #SET(lID            ,#TEXT(pID     ))
...
////
  ...
  #APPEND(ECL    ,'allproxidcands    := project(' + %'kcand'% + '(' + %'lID'% + ' in setsampleproxids)  ,' + %'lModule'% + '.match_candidates(' + %'Source'% + ',' + %'Version'% + ',' + %'lInfile'% + ').layout_candidates);\n')
  #APPEND(ECL    ,'s := dataset([],' + %'lModule'% + '.Layout_Specificities.R);\n')
  #APPEND(ECL    ,'allproxidsrolled := ' + %'lModule'% + '.Debug(' + %'Source'% + ',' + %'Version'% + ',' + %'lInfile'% + ',s).RolledEntities(allproxidcands);\n')

...
EXPORT GetLayerGroup($.IParam.SearchParam inMod) := FUNCTION
	...
	dLayerInfo := JOIN(dIDs,GroupLayerkey, 
												...
												TRANSFORM(RIGHT), 
												LIMIT(0), KEEP($.Constants.MAX_SRCH_RESULT));
	...
	dLayerInfoSorted := SORT(PROJECT(dLayerInfo,Layerrec),data_provider_id);
	...
	dLayerinfoGrouped := ROLLUP(GROUP(dLayerInfoSorted,data_provider_id,LayerGroupId,LayerGroupName),GROUP,GroupLayerInfo(LEFT,ROWS(LEFT)));	

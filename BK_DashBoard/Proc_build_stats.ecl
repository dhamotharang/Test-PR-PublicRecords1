...
despray_files := parallel(
	         Bk_dashboard.fn_despray(Bk_dashboard.filenames.linking),  
					 Bk_dashboard.fn_despray(Bk_dashboard.filenames.overall), 
					 Bk_dashboard.fn_despray(Bk_dashboard.filenames.segmentation),
					 Bk_dashboard.fn_despray(Bk_dashboard.filenames.linking_in_days),  
					 Bk_dashboard.fn_despray(Bk_dashboard.filenames.overall_in_days), 
	        );

Emailnotification := 'wenhong.ma@lexisnexisrisk.com; Alan.Schwartz@lexisnexisrisk.com';  
									
email_alert := FileServices.SendEmail(Emailnotification,'BK DashBoard Stats'+ filedate ,'BK DashBoard Stats ' +filedate+ ' despray on edata12' + failmessage);

export proc_build_stats := if(filedate = BK_DashBoard.buildversion,
sequential(build_stats, output_samples,despray_files,email_alert), output('Bankruptcy Keys not up to date'));

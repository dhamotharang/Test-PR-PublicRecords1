...
MA_norm := normalize(file_in, 3, tnormalize(left, counter));

MA_dist := distribute(MA_norm(lname <> '' or fname <> '' or mname <> '' or cname <> ''), hash(tmsid,rmsid));

MA_sort  := sort(MA_dist,record,EXCEPT Date_First_Seen, Date_Last_Seen,
...
export mapping_MA_party := rollup(MA_sort,rollupXform(LEFT,RIGHT),RECORD,

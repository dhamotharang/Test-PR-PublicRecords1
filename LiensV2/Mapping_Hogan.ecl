...
self.lkp_county        := IF(L.courtid = 'MSHINC3','',R.county);
self.lkp_court_name    := IF(L.courtid = 'MSHINC3','MISSISSIPPI DEPT OF REVENUE',R.courtName);
self.lkp_city          := R.city;
self.lkp_state         := R.state;
self := L ;
end;

filein_join := join(liensv2.file_Hogan_in, liensv2.file_lookup_in, left.COURTID = right.LNCourtCode, tjoin(left,right), left outer,many LOOKUP);
...
self.agency            := L.lkp_court_name;
self.agency_city       := L.lkp_city;
self.agency_state      := L.lkp_state;
self.agency_county     := L.lkp_county ;
...
self.AgencyID     := L.CourtID; //DF-24061 VC
self.Global_sid   := 0;
self.Record_sid   := 0;
self.Agencyid_src := 'R';
end;

...
//				  did_add.MAC_Add_SSN_By_DIDliensV2.Layout_liens_party_SSN_bid,
//					TopBusiness_External

// PURPOSE	 	: BID, DID, BDID and append_ssn daily NYFederal file 
...
file_in := liensV2.Mapping_NYFDLN_party;
...
post_DID_BDID := postbdid +  postDID;
...
Full_NYFDLN_Party_nondist := dataset('~thor_data400::base::Liens::party::NYFDLN',liensv2.Layout_liens_party_SSN_BIPV2_with_LinkFlags,thor);

// Add Full File and Daily Party File (distributed)

daily_plus_full := distribute((post_append_ssn + Full_NYFDLN_Party_nondist),hash(tmsid)); 
...
export NYFederal_DID := full_dedup :persist('~thor_data400::persist::Liens::NYFederal_DID');
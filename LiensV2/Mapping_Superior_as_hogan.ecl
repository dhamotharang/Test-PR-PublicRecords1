...
self.filing_jurisdiction := trim(L.header_filing_state);
self.filing_state      := '';
self.filing_number     :=  L.case_number;
self.orig_filing_type   := '';
self.orig_filing_date   := l.docket_date;
self.orig_filing_time   := '';
self.filing_status     	:= if(L.disp_code_desc <> '' , L.disposition_code , L.status) ;
self.filing_status_desc := if(L.disp_code_desc <> '' , L.disp_code_desc , L.status_code_desc) ;
...
self.filing_book  := '';
self.filing_page  := '';
self.release_date := l.disposition_dt;
self.amount       := '';
self.eviction     := if(regexfind('(EJECT)|(EVICT)', L.filing_type_desc + L.case_type_desc), 'Y', '');
self.judg_satisfied_date := '';
self.judg_vacated_date := '';
self.tax_code          := if(L.tax_type_code_desc = 'NO TAX TYPE', '' , L.tax_type_code_desc); 
...
self.filing_book  := '';
self.filing_page  := '';
self.release_date := l.DISPOSITION_DATE;
self.debtor_name := L.DEBTOR_NAME; 
self.debtor_lname := '';
self.debtor_fname := '';
self.debtor_mname := '';
self.debtor_suffix := '';
self.debtor_tax_id :=  if(L.DEBTOR_TYPE = 'C', if(regexfind('[A-Z]',stringlib.stringfilterout(L.ssn,'-X')) ,'',stringlib.stringfilterout(L.ssn,'-X')), '');
self.debtor_ssn :=     if(L.DEBTOR_TYPE = 'I', if(regexfind('[A-Z]',stringlib.stringfilterout(L.ssn,'-X')) ,'',stringlib.stringfilterout(L.ssn,'-X')), '');
...
// output(choosen(mapping_superior_judgment_liens, 200) , named('SUPERIOR__mapping_superior_judgment_liens'));
// output(choosen(mapping_superior_sub_judgment_liens, 200) , named('SUPERIOR__mapping_superior_sub_judgment_liens'));
// output(choosen(mapping_superior_debtor_liens, 200) , named('SUPERIOR__mapping_superior_debtor_liens'));
// output(choosen(mapping_superior_creditor, 200) , named('SUPERIOR__mapping_superior_creditor'));
// output(choosen(mapping_superior_debtor_atty_liens, 200) , named('SUPERIOR__mapping_superior_debtor_atty_liens'));
// output(choosen(mapping_superior_creditor_atty, 200) , named('SUPERIOR__mapping_superior_creditor_atty'));

export Mapping_Superior_as_hogan := mapping_superior_judgment_liens
									+ mapping_superior_sub_judgment_liens
									+ mapping_superior_debtor_liens
                                    + mapping_superior_creditor
									+ mapping_superior_debtor_atty_liens
									+ mapping_superior_creditor_atty
									;

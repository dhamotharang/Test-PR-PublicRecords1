...
export Proc_Copy_FCRA_Keys(string filedate,string datasetname,string keysuffix, boolean doreplicate=false) := function
	...
	email_me := fileservices.sendemail(
													...
													'Liensv2 NonFCRA-FCRA Copy Failed ' + filedate,
													sourcekeyname + ' does not exist, check' + WORKUNIT
													);
	
	copyfile := nothor( if(~doreplicate ,if (
					...
					email_me
					),fileservices.replicate('~thor_data400::key::'+datasetname+'::fcra::'+filedate+'::'+keysuffix)));

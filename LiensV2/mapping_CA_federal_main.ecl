...
CA_federal_main_temp := project(liensV2.mapping_CA_federal, tmakemain(left));
CA_federal_main_dist := distribute(CA_federal_main_temp, hash(tmsid));
CA_federal_main_temp_sort  := sort(CA_federal_main_dist, tmsid, rmsid,record_code,date_vendor_removed, filing_jurisdiction,filing_state,orig_filing_number,orig_filing_type, 
...
file_flat := project(CA_federal_main_temp_sort, tmakefatrecord(left));

// Full File - distributed (tmsid)

Full_CA_Main_nondist := dataset('~thor_data400::base::Liens::Main::CA_federal',liensv2.layout_liens_main_module.layout_liens_main,thor);

Full_CA_Main := distribute(Full_CA_Main_nondist,hash(tmsid));

// Add Full File and Daily file (distributed)

daily_plus_full := Full_CA_Main + file_flat;

// Sort and rollup full file (local)

full_sort := project(sort(daily_plus_full,record,except process_date,orig_filing_time,local), 
							transform(liensv2.layout_liens_main_module.layout_liens_main, self.eviction := regexreplace('Nn', left.eviction, ''), self := left));
...
CA_federal_main_out := rollup(full_sort, tmsid +rmsid+record_code+date_vendor_removed+ filing_jurisdiction+filing_state+orig_filing_number+orig_filing_type+ 
                              orig_filing_date+orig_filing_time + case_number+filing_number+filing_type_desc+ filing_date + filing_time+ vendor_entry_date+judge+case_title+ 
							  filing_book+filing_page+release_date+amount+eviction+satisifaction_type+judg_satisfied_date+judg_vacated_date+tax_code+irs_serial_number+effective_date+

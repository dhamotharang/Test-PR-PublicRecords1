...
rec_temp := record
	Layout_liens_party_SSN_BIPV2_with_LinkFlags;
	integer8  temp_DID;
	integer8  temp_bdid;
end;
...
post_append_ssn_dist := post_append_ssn; // Distribute full daily file by tmsid

// Full File - Distributed

Full_ILFDLN_Party_nondist := dataset('~thor_data400::base::Liens::party::ILFDLN',liensv2.Layout_liens_party_SSN_BIPV2_with_LinkFlags,thor);

Full_ILFDLN_Party := Full_ILFDLN_Party_nondist;

// Add Full File and Daily Party File (distributed)

daily_plus_full := distribute((post_append_ssn_dist + Full_ILFDLN_Party),hash(tmsid));

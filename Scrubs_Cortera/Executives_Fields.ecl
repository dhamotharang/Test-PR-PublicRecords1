...
EXPORT Make_exec_title(SALT311.StrType s0) := MakeFT_Alpha(s0);
EXPORT InValid_exec_title(SALT311.StrType s) := InValidFT_Alpha(s);
EXPORT InValidMessage_exec_title(UNSIGNED1 wh) := InValidMessageFT_Alpha(wh);
...
#uniquename(AggRec)
  %AggRec% := RECORD
    ...
    Count_Diff_exec_title := COUNT(GROUP,%Closest%.Diff_exec_title);
  END;

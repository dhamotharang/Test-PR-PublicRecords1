IMPORT SALT311,STD;
EXPORT Header_Delta(DATASET(Header_Layout_Cortera)old_s, DATASET(Header_Layout_Cortera) new_s) := MODULE//Routines to compute the differences between two instances of a file
  ...
  EXPORT DifferenceSummary := Header_hygiene(old_s).Summary('Old') + Header_hygiene(new_s).Summary('New') + Header_hygiene(PROJECT(Differences(deleted), TRANSFORM(Header_Layout_Cortera, SELF := LEFT.old_rec))).Summary('Deletions') + Header_hygiene(PROJECT(Differences(added), TRANSFORM(Header_Layout_Cortera, SELF := LEFT.new_rec))).Summary('Additions');
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE) := FUNCTION
    myTimeStamp := (UNSIGNED6)SALT311.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
    hygieneDiffOverall := DifferenceSummary;
    SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_Cortera, Header_Fields, 'RECORDOF(hygieneDiffOverall)', FALSE);

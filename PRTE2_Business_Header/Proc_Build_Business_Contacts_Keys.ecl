import business_header, ut, doxie_cbrs, RoxieKeyBuild, versioncontrol, aid;

export Proc_Build_Business_Contacts_Keys(string pversion) := 
module

	/////////////////////////////////////////////////////////////////////////
	in_hdr := files().base.business_contacts.keybuild;
	...
	Business_Header.macGetCleanAddr(in_hdr_norm_Addr, prim_range, predir, prim_name, addr_suffix, postdir, 
	...
	VersionControl.macBuildNewLogicalFile(PRTE2_business_header._Dataset().thor_cluster_Files +'base::business_header::BC_bestAddr::keybuild',in_hdr_With_Best_Addr,abuildBaseForKeys,,,true);
	//********************************* End of AID process *********************************
	
	shared keyName			:= keynames(pversion);
	
	VersionControl.macBuildNewLogicalKeyWithName(business_header.Key_Business_Contacts_BDID									,keyName.Contacts.Bdid.New						,Build_Key1				);
	VersionControl.macBuildNewLogicalKeyWithName(business_header.Key_Company_Title													,keyName.Contacts.Companytitle.New		,Build_Key2				);  // removed one node key flag set to true
	VersionControl.macBuildNewLogicalKeyWithName(business_header.Key_Business_Contacts_State_LFName					,keyName.Contacts.StateLfmname.New		,Build_Key3				);
	VersionControl.macBuildNewLogicalKeyWithName(business_header.Key_Prep_Business_Contacts_State_City_Name	,keyName.Contacts.StateCityName.New		,Build_Key4				);
	VersionControl.macBuildNewLogicalKeyWithName(business_header.Key_Business_Contacts_FP										,keyName.Contacts.filepos.New					,Build_Key5				);
	VersionControl.macBuildNewLogicalKeyWithName(business_header.Key_Business_Contacts_DID									,keyName.Contacts.Did.New							,Build_Key6				);
	VersionControl.macBuildNewLogicalKeyWithName(business_header.Key_Prep_Business_Contacts_Stats						,keyName.ContactsStat.FileposData.New	,Build_Key7				);
	VersionControl.macBuildNewLogicalKeyWithName(business_header.Key_Business_Contacts_SSN									,keyName.Contacts.Ssn.New							,Build_Key8				);

...
// Get All FEIN's
bh := distribute(Prte2_Business_Header.Files().Base.Business_Headers.Built(fein <> 0), hash(fein));
...
bhb := distribute(Prte2_Business_Header.Files().Base.Business_Header_Best.Built(fein <> 0), hash(fein));
...
rec getBest(rec l, Prte2_Business_Header.Files().Base.Business_Header_Best.Built r) := transform
...
export FEIN_Table := ddp 
	: PERSIST( '~prte::persist::Business_Risk::FEIN_Table');

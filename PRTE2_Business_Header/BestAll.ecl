...
export BestAll(
	 ...
	 dataset(Business_Header.Layout_Business_Header_Base	)	pInput							  = prte2_business_header.Files().Base.Business_Headers.Built
...
function
	...
	////////////////////////////////////////////////////////////////////////
	// removing the filter code as it drops the EBR and Experian Vehical data.
	//bh_filtered 		:= Business_Header.filters.bases.business_header_best(pInput);
	
	bh_with_dppa 		:= pInput(dppa = true);
	bh_without_dppa := pInput(dppa = false);
	...
	Business_Header.Layout_Business_Header_Base joindppa2nondppa(bh_without_dppa_sort l, bh_with_dppa_sort r) := transform
	self := r;
	end;
	...
	Business_Header.Layout_Business_Header_Base joindppa2state(bh_dppa_only_bdids_dedup l, bh_dppa_only_bdids_sort r) := transform
	self := r;
	end;
	...
	lbestjoined := Business_Header.BestJoined(bh_all_in, nametopass, best_poss_non_DandB) : persist(persistname);

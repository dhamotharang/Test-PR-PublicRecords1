...
event_update_init := project(dedup(Files.Input.Events.Using, all), InitEventUpdate(left));
					   
// Fix any dates necessary
event_update_init_fix := fFixEventDates(event_update_init(corp_state_origin in setStatesNeedDatesFixed));
event_update_init_combined := event_update_init(corp_state_origin not in setStatesNeedDatesFixed) + event_update_init_fix;
...
event_current_init := project(Files.Base.Events.QA(Filters.Base.Events), InitCurrentEventBase(left));
...
event_update_combined := if(Flags.Update.Events,
...
export Update_Event := event_update : persist(persistnames.UpdateEvent);
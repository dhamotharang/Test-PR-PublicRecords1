import ut, Business_Header, Business_Header_SS, address, Header, Header_Slimsort, didville, ut, DID_Add;

// Project Update to Temp Format
Layout_Corporate_Direct_Cont_Base InitUpdate(Layout_Corporate_Direct_Cont_In l, unsigned1 cnt) := transform
// Uppercase fields
...
self.dt_last_seen := if(
  ut.LatestDate((unsigned4)fCheckdate(l.cont_filing_date), 
  ut.LatestDate((unsigned4)fCheckdate(l.cont_effective_date), (unsigned4)fCheckdate(l.cont_address_effective_date))) <> 0,
  ut.LatestDate((unsigned4)fCheckdate(l.cont_filing_date), 
  ut.LatestDate((unsigned4)fCheckdate(l.cont_effective_date), (unsigned4)fCheckdate(l.cont_address_effective_date))),
...
corp_cont_file := Files.Input.Cont.Using;
...
cont_update_init_fix := fFixContDates(cont_update_init(corp_state_origin in setStatesNeedDatesFixed));
cont_update_init_combined := cont_update_init(corp_state_origin not in setStatesNeedDatesFixed) + cont_update_init_fix;
...
Layout_Corporate_Direct_Cont_Base RollupUpdate(Layout_Corporate_Direct_Cont_Base l, Layout_Corporate_Direct_Cont_Base r) := transform
SELF.dt_first_seen := 
...
Layout_Corporate_Direct_Cont_Base InitCurrentBase(Layout_Corporate_Direct_Cont_Base l) := transform
self.bdid := 0;
self.record_type := 'H';
self := l;
end;

cont_current_init := project(Files.Base.Cont.QA(corp_vendor <> 'EX', Filters.Base.Cont), InitCurrentBase(left));
...
cont_update_combined := if(Flags.Update.Cont,
...
cont_update_combined_history := Corp4AsCorp2Contacts(corp_state_origin in setCorp4AddStates) + fCombineContHistory(cont_update_combined);
...
Layout_Corporate_Direct_Cont_Base NormRegAgents(Layout_Corporate_Direct_Corp_Base l, unsigned1 cnt) := transform
...
corp_cont_norm_ra := normalize(Update_Corp, 2, NormRegAgents(left, counter));
...
cont_update_combined_propagate := group(iterate(cont_update_combined_grpd_sort, tPropagateContFields(left, right)));
...
Layout_Corporate_Direct_Cont_Base UpdateDates(Layout_Corporate_Direct_Cont_Base l, layout_event_slim r) := transform
self.dt_vendor_last_reported := ut.LatestDate(l.dt_vendor_last_reported, r.dt_vendor_last_reported);
...
Business_Header_SS.MAC_Add_BDID_Flex(cont_bdid_nomatch,
                                  ...
                                  bdid, Layout_Corporate_Direct_Cont_Base,
                                  FALSE, BDID_score_field,
                                  cont_bdid_rematch)

cont_bdid_all := cont_bdid_match + cont_bdid_rematch : persist(persistnames.UpdateContBDID);

Layout_Corporate_Direct_Cont_Base_DID := record
unsigned6 uid := 0;
Layout_Corporate_Direct_Cont_Base;
unsigned6 adl := 0;
...
Layout_Corporate_Direct_Cont_Base AssignDIDs(Layout_Corporate_Direct_Cont_Base_DID l, layout_corp_cont_slim r) := transform
...
export Update_Cont := cont_did_append : persist(persistnames.UpdateCont);
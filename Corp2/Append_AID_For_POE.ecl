import poe,address,aid;

export Append_AID_For_POE :=
module

	export fPreProcess(

		dataset(POE.Layouts.Base) pPOE
	...
	function
		...
		dPrep_for_aid := project(
			 pPOE
			,transform(
				 poe.layouts.temporary.aid_prep
				,
	...
	export fStandardizeAddresses(
		...
		dataset(poe.layouts.temporary.aid_prep) pAddrPrep
	...
	function
		...
		addresslayout tNormalizeAddress(poe.layouts.temporary.aid_prep l, unsigned1 cnt) :=
		transform
		...
		poe.layouts.temporary.aid_prep tGetStandardizedAddress(poe.layouts.temporary.aid_prep l	,dAddressStandardized_dist r) :=
		transform
	...
	function
		...
		dStandardizeAddress	:= fStandardizeAddresses(dPreprocess) 
			: persist(persistnames.fAppend_AID_For_POE);
	                                                                                                                             
		return PROJECT(dStandardizeAddress,TRANSFORM(poe.Layouts.Base,					                                 
			                                         self 						:= LEFT;));

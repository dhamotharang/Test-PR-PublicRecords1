...
export fProf_License_As_Business_Header(

	dataset(layout_prolic_out_with_AID) pProfLic = Prof_License.File_prof_license_base_AID,boolean isPRCT=false

) :=
  function
  
	Proflic_In := pProfLic(business_flag = 'Y', company_name <> '', prolic_key[1..1]<>'C');

	Business_Header.Layout_Business_Header_New Translate_Proflic_To_BHF(layout_prolic_out_with_AID l) := transform
	...
	self.phone := if(l.orig_name = ''	,(unsigned6)l.phone, 0);

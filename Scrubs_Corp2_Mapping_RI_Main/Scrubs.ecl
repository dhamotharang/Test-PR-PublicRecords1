...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Layout_in_file)
    ...
    UNSIGNED1 InternalField4_Invalid;
...
EXPORT FromNone(DATASET(Layout_in_file) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_filing_date_Invalid := Fields.InValid_corp_filing_date((SALT36.StrType)le.corp_filing_date);
    SELF.corp_status_date_Invalid := Fields.InValid_corp_status_date((SALT36.StrType)le.corp_status_date);
    SELF.corp_inc_state_Invalid := Fields.InValid_corp_inc_state((SALT36.StrType)le.corp_inc_state);
    SELF.corp_inc_date_Invalid := Fields.InValid_corp_inc_date((SALT36.StrType)le.corp_inc_date);
    SELF.corp_term_exist_cd_Invalid := Fields.InValid_corp_term_exist_cd((SALT36.StrType)le.corp_term_exist_cd);
    ...
    SELF.InternalField1_Invalid := Fields.InValid_InternalField1((SALT36.StrType)le.InternalField1);
    SELF.InternalField2_Invalid := Fields.InValid_InternalField2((SALT36.StrType)le.InternalField2);
    SELF.InternalField3_Invalid := Fields.InValid_InternalField3((SALT36.StrType)le.InternalField3);
    SELF.InternalField4_Invalid := Fields.InValid_InternalField4((SALT36.StrType)le.InternalField4);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_filing_date_Invalid << 22 ) + ( le.corp_status_date_Invalid << 24 ) + ( le.corp_inc_state_Invalid << 26 ) + ( le.corp_inc_date_Invalid << 27 ) + ( le.corp_term_exist_cd_Invalid << 29 ) + ( le.corp_foreign_domestic_ind_Invalid << 30 ) + ( le.corp_forgn_state_cd_Invalid << 31 ) + ( le.corp_forgn_state_desc_Invalid << 32 ) + ( le.corp_forgn_date_Invalid << 33 ) + ( le.corp_for_profit_ind_Invalid << 35 ) + ( le.InternalField1_Invalid << 36 ) + ( le.InternalField2_Invalid << 37 ) + ( le.InternalField3_Invalid << 38 ) + ( le.InternalField4_Invalid << 39 ) + ( le.recordorigin_Invalid << 40 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.InternalField1_Invalid := (le.ScrubsBits1 >> 36) & 1;
    SELF.InternalField2_Invalid := (le.ScrubsBits1 >> 37) & 1;
    SELF.InternalField3_Invalid := (le.ScrubsBits1 >> 38) & 1;
    SELF.InternalField4_Invalid := (le.ScrubsBits1 >> 39) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    InternalField4_ALLOW_ErrorCount := COUNT(GROUP,h.InternalField4_Invalid=1);
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT36.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'InternalField2:invalid_country:ALLOW'
          ,'InternalField3:invalid_country:ALLOW'
          ,'InternalField4:invalid_alphablank:ALLOW'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_InternalField4(1)
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.InternalField4_ALLOW_ErrorCount

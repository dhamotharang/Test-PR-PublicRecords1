import BIPV2_Files, BIPV2, MDR, BIPV2_LGID3_PlatForm,ut,bipv2_build,wk_ut,tools,std,BIPV2_Tools,mdr,SALTTOOLS22;
...
//----end of merge-------------------------------
	...
	/* ---------------------- SALT Specificities ------------------------- */
	shared specMod := BIPV2_LGID3_PlatForm.specificities(input);
	...
	shared specDebug := parallel(OUTPUT(specMod.Specificities,NAMED('Specificities')),OUTPUT(specMod.SpcShift,NAMED('SpcShift'))
                      ,SALTTOOLS22.mac_Patch_SPC(SpecMod.Specificities,'BIPV2_LGID3_PlatForm._SPC',input,,false));
	
	/* ---------------------- SALT Iteration ----------------------------- */
	shared possibleMatches := output(BIPV2_LGID3_PlatForm.matches(input).PossibleMatches,,f_hist(iter),overwrite,compressed);
	shared saltMod 	:= BIPV2_LGID3_PlatForm.Proc_Iterate(iter, input, f_out(''));
	...
	/* ---------------------- Review Samples ----------------------------- */
	export outputReviewSamples := BIPV2_LGID3_PlatForm._Samples(input).out;
	/* ---------------------- Inline Iteration Run for Testing ----------- */	
	export DATASET(l_common) inline_iteration(DATASET(l_common) ds) := FUNCTION
		ds1 := preProcess(ds);
		mod := BIPV2_LGID3_PlatForm.matches(ds1,BIPV2_LGID3_PlatForm.Config.MatchThreshold);
	...
	export MultIter(unsigned startIter, unsigned numIters) := 
			'BIPV2_LGID3_PlatForm Controller ' + pversion + ' ' + (string)startiter + '-' + (string)(startiter + numiters - 1);
  export MultIter_run(startIter, numIters,doInit,doSpec,doIter = 'true',doPost = 'true',pversion = 'bipv2.KeySuffix') := functionmacro
    ...
    eclInit		:= 'import BIPV2_Files,BIPV2_LGID3_PlatForm;\niteration := \'@iteration@\';\npversion  := \'@version@\';\n#workunit(\'name\',\'BIPV2_LGID3_PlatForm \' + pversion + \' Init \');\n#workunit(\'priority\',\'high\');\n\n#OPTION(\'multiplePersistInstances\',FALSE);\n' + 'BIPV2_LGID3_PlatForm._proc_lgid3(,,pversion).initFromHrchy;\n';
    eclSpec		:= 'import BIPV2_Files,BIPV2_LGID3_PlatForm;\niteration := \'@iteration@\';\npversion  := \'@version@\';\nlih := BIPV2_LGID3_PlatForm.In_LGID3;\n\n#OPTION(\'multiplePersistInstances\',FALSE);\n#workunit(\'name\',\'BIPV2_LGID3_PlatForm \' + pversion + \' Specificities \');\n#workunit(\'priority\',\'high\');\nBIPV2_LGID3_PlatForm._proc_lgid3(lih,iteration,pversion).runSpecBuild;\n';
    eclIter		:= 'import BIPV2_Files,BIPV2_LGID3_PlatForm;\niteration := \'@iteration@\';\npversion  := \'@version@\';\nlih := BIPV2_LGID3_PlatForm.In_LGID3;\n\n#OPTION(\'multiplePersistInstances\',FALSE);\n#workunit(\'name\',\'BIPV2_LGID3_PlatForm \' + pversion + \' iter \' + iteration);\n#workunit(\'priority\',\'high\');\nBIPV2_LGID3_PlatForm._proc_lgid3(lih,iteration,pversion).runIter;\n';
    eclPost		:= 'import BIPV2_LGID3_PlatForm;\n\n#workunit(\'name\',\'BIPV2_LGID3_PlatForm @version@ PostProcess\');\n\n#workunit(\'priority\',\'high\');\n\n#OPTION(\'multiplePersistInstances\',FALSE);\npversion  := \'@version@\';\n' + 'BIPV2_LGID3_PlatForm._proc_lgid3(,\'' + lastIter + '\',pversion).updateSuperfiles()';

...
EXPORT ForceFails := TABLE(RRF,R);
  ...
  R tosummary(ds le) := TRANSFORM
    SELF := le;
    SELF.Summary := IF(le.sbfe_id_Score = 0,'','|'+IF(le.sbfe_id_Score < 0,'-','')+'sbfe_id')+IF(le.Lgid3IfHrchy_Score = 0,'','|'+IF(le.Lgid3IfHrchy_Score < 0,'-','')+'Lgid3IfHrchy')+IF(le.active_duns_number_Score = 0,'','|'+IF(le.active_duns_number_Score < 0,'-','')+'active_duns_number')+IF(le.company_name_Score = 0,'','|'+IF(le.company_name_Score < 0,'-','')+'company_name')+IF(le.duns_number_Score = 0,'','|'+IF(le.duns_number_Score < 0,'-','')+'duns_number')+IF(le.duns_number_concept_Score = 0,'','|'+IF(le.duns_number_concept_Score < 0,'-','')+'duns_number_concept')+IF(le.company_fein_Score = 0,'','|'+IF(le.company_fein_Score < 0,'-','')+'company_fein')+IF(le.company_charter_number_Score = 0,'','|'+IF(le.company_charter_number_Score < 0,'-','')+'company_charter_number')+IF(le.cnp_number_Score = 0,'','|'+IF(le.cnp_number_Score < 0,'-','')+'cnp_number')+IF(le.company_inc_state_Score = 0,'','|'+IF(le.company_inc_state_Score < 0,'-','')+'company_inc_state')+IF(le.cnp_btype_Score = 0,'','|'+IF(le.cnp_btype_Score < 0,'-','')+'cnp_btype');
  END;

...
EXPORT _AggLgid3s(
	 dataset(layout_LGID3)	pDataset
) :=
function
	dslim := project(pDataset, transform(
	{pDataset.lgid3,pDataset.company_name,pDataset.cnp_name,pdataset.company_name_type_derived,pdataset.nodes_total
  ,pDataset.cnp_number,pDataset.cnp_btype
	,string address/*,string contact*/,pDataset.source,pDataset.vl_id,pDataset.company_charter_number,pDataset.company_inc_state,pDataset.active_duns_number,pDataset.duns_number,pDataset.active_enterprise_number
...
//    self.srcvlid  := trim(left.source,left,right) + '-' + trim((string)left.source_record_id,left,right) + '-' + trim(left.vl_id,left,right);
	...
	dpost := tools.mac_AggregateFieldsPerID(dslim,lgid3,,false);

...
//  version               := pversion;
  ...
  kFile := BIPV2_LGID3_PlatForm.Keys2(BFile,pversion);

  odl   := PROJECT(CHOOSEN(KFile.Candidates(Lgid3=Lgid3one),100000),BIPV2_LGID3_PlatForm.match_candidates(BFile).layout_candidates);
  odr   := PROJECT(CHOOSEN(KFile.Candidates(Lgid3=Lgid3Two),100000),BIPV2_LGID3_PlatForm.match_candidates(BFile).layout_candidates);
  k     := KFile.Specificities_Key;
  s     := GLOBAL(PROJECT(k,BIPV2_LGID3_PlatForm.Layout_Specificities.R)[1]);
  odlv  := BIPV2_LGID3_PlatForm.Debug(BFile,s).RolledEntities(odl);
  odrv  := BIPV2_LGID3_PlatForm.Debug(BFile,s).RolledEntities(odr);

  mtch := BIPV2_LGID3_PlatForm.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,LGID3one,LGID3two,0,0}],BIPV2_LGID3_PlatForm.match_candidates(BFile).layout_matches));

  // BIPV2_LGID3_PlatForm.match_candidates(BFile).layout_attribute_matches ainto(KFile.Attribute_Matches le) := TRANSFORM
    // SELF := le;
  // END;
  ...
  // mtch  := BIPV2_LGID3_PlatForm.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,Lgid3one,Lgid3two,0,0}],BIPV2_LGID3_PlatForm.match_candidates(BFile).layout_matches),am);

  // get layout with the scores only for easier reading
  dnorm_specs_filt2 := BIPV2_LGID3_PlatForm._fn_normscores(mtch);

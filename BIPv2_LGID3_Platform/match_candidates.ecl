...
pj12 do_computes(pj12 le) := TRANSFORM
  ...
  SELF := le;
END;
SHARED propogated := PROJECT(pj12,do_computes(left)) : PERSIST('~temp::LGID3::BIPV2_LGID3_PlatForm::mc_props::LGID3',EXPIRE(Config.PersistExpire)); // to allow to 'jump' over an exported value
...
SHARED Annotated := DISTRIBUTE(j0,hash(LGID3)) : PERSIST('~temp::LGID3::BIPV2_LGID3_PlatForm::mc',EXPIRE(Config.PersistExpire)); // Distributed for keybuild case

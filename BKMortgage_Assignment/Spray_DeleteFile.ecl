...
SHARED Spray_Delete_Release  		:= STD.File.SprayVariable(pServerIP,filepath_Release + fRelease,
  ...
  TransformFile_Assignment:= FUNCTION
	  dsraw := DATASET(dst_Delete_Assignment_raw,
										          BKMortgage_Assignment.layouts.Delete_Rec,CSV(SEPARATOR('\t'),QUOTE(''),TERMINATOR('\n')));
		ds    := PROJECT(dsraw,TRANSFORM(BKMortgage_Assignment.layouts.Delete_Rec,SELF.ln_filedate := (string)version; SELF.Delete_Flag := 'DELETE'; SELF := LEFT; SELF :=[]));
	RETURN ds;
	END;
	
  TransformFile_Release := FUNCTION
	  dsraw := DATASET(dst_Delete_Release_raw,
										          BKMortgage_Release.Layouts.Delete_Rec,CSV(SEPARATOR('\t'),QUOTE(''),TERMINATOR('\r\n')));
		ds    := PROJECT(dsraw,TRANSFORM(BKMortgage_Release.layouts.Delete_Rec,SELF.ln_filedate := (string)version; SELF.Delete_Flag := 'DELETE'; SELF := LEFT; SELF :=[]));
	RETURN ds;
	END;

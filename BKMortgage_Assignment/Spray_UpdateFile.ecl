IMPORT STD,_control, $, BKMortgage_Release;
...
SHARED Spray_Release_Orphan  := STD.File.SprayVariable(pServerIP,filepath_Release + fn_Release_Orph,
  ...
  TransformFile_Assignment := FUNCTION
	  dsraw := dataset(dst_Assignment_raw,
		...
		ds    := PROJECT(dsraw,TRANSFORM(BKMortgage_Assignment.Layouts.Raw_out,SELF.ln_filedate := (string)version; SELF.bk_infile_type := 'ASSIGN_UPDATE'; 
																			SELF := LEFT; SELF :=[]));
	RETURN ds;
	END;
  ...
  TransformFile_Release := FUNCTION
	  dsraw := dataset(dst_Release_raw,
		...
		ds    := PROJECT(dsraw,TRANSFORM(BKMortgage_Release.Layouts.raw_out,SELF.ln_filedate := (string)version; SELF.bk_infile_type := 'RELEASE_UPDATE'; 
																			SELF := LEFT; SELF :=[]));
	RETURN ds;
	END;

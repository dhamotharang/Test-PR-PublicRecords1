IMPORT STD,SALT311;
EXPORT AssignRefresh_Ingest(BOOLEAN incremental=FALSE
, DATASET(AssignRefresh_Layout_BKMortgage) Delta = DATASET([],AssignRefresh_Layout_BKMortgage)
, DATASET(AssignRefresh_Layout_BKMortgage) dsBase = AssignRefresh_In_BKMortgage // Change IN_BKMortgage to change input to ingest process
, DATASET(RECORDOF(BKMortgage_Assignment.AssignRefresh_prep_ingest_file))  infile = BKMortgage_Assignment.AssignRefresh_prep_ingest_file
) := MODULE
  SHARED NullFile := DATASET([],AssignRefresh_Layout_BKMortgage); // Use to replace files you wish to remove
  ...
  SHARED WithRT := RECORD
    AssignRefresh_Layout_BKMortgage;
  ...
  SHARED AllRecs := ORe+NR1+NR(record_id<>0) : PERSIST('~temp::BKMortgage_Assignment::Refresh_Ingest_Cache',EXPIRE(BKMortgage_Assignment.Config.PersistExpire));
  ...
  EXPORT NewRecords_NoTag := PROJECT(NewRecords,AssignRefresh_Layout_BKMortgage);
  EXPORT OldRecords :=PROJECT( AllRecs(__Tpe=RecordType.Old), NoFlagsRec);
  EXPORT OldRecords_NoTag := PROJECT(OldRecords,AssignRefresh_Layout_BKMortgage);
  EXPORT UpdatedRecords := PROJECT(AllRecs(__Tpe=RecordType.Updated), NoFlagsRec);
  EXPORT UpdatedRecords_NoTag := PROJECT(UpdatedRecords,AssignRefresh_Layout_BKMortgage);
  EXPORT AllRecords := IF(incremental, NewRecords, PROJECT(AllRecs, NoFlagsRec));
  EXPORT AllRecords_NoTag := PROJECT(AllRecords,AssignRefresh_Layout_BKMortgage); // Records in 'pure' format

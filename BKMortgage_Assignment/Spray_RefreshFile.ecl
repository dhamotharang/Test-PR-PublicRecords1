IMPORT $, BKMortgage_Release, STD,_control;
 ...
 SHARED Spray_Release_refresh := STD.File.SprayVariable(pServerIP,filepath_Release + fn_Release_Ref,
  ...
  TransformFile_Assignment_Orphan := FUNCTION
	  dsraw := DATASET(dst_Assignment_Orph_raw,
										          BKMortgage_Assignment.Layouts.Raw_in,CSV(SEPARATOR('\t'),QUOTE(''),TERMINATOR(['\n','\r','\r\n'])));
		ds    := PROJECT(dsraw,TRANSFORM(BKMortgage_Assignment.Layouts.Raw_out,SELF.ln_filedate := (string)version; SELF.bk_infile_type := 'ASSIGN_ORPHAN'; 
																		SELF := LEFT; SELF :=[]));
	RETURN ds;
	END;
  ...
  TransformFile_Assignment_refresh := FUNCTION
	  dsraw := DATASET(dst_Assignment_ref_raw,
										          BKMortgage_Assignment.Layouts.Raw_in,CSV(SEPARATOR('\t'),QUOTE(''),TERMINATOR(['\n','\r','\r\n'])));
		ds    := PROJECT(dsraw,TRANSFORM(BKMortgage_Assignment.Layouts.Raw_out,SELF.ln_filedate := (string)version; SELF.bk_infile_type := 'ASSIGN_REFRESH';
																		SELF := LEFT; SELF :=[]));
	RETURN ds;
	END;
  ...
  TransformFile_Release_Orphan := FUNCTION
	  dsraw := DATASET(dst_Release_orph_raw,
										          BKMortgage_Release.Layouts.Raw_in,CSV(SEPARATOR('\t'),QUOTE(''),TERMINATOR(['\n','\r','\r\n'])));
		ds    := PROJECT(dsraw,TRANSFORM(BKMortgage_Release.Layouts.Raw_out,SELF.ln_filedate := (string)version; SELF.bk_infile_type := 'RELEASE_ORPHAN';
																			SELF := LEFT; SELF :=[]));
	RETURN ds;
	END;
  ...
  TransformFile_Release_refresh := FUNCTION
	  dsraw := DATASET(dst_Release_ref_raw,
										          BKMortgage_Release.Layouts.Raw_in,CSV(SEPARATOR('\t'),QUOTE(''),TERMINATOR(['\n','\r','\r\n'])));
		ds    := PROJECT(dsraw,TRANSFORM(BKMortgage_Release.Layouts.Raw_out,SELF.ln_filedate := (string)version; SELF.bk_infile_type := 'RELEASE_REFRESH';
																		SELF := LEFT; SELF :=[]));
	RETURN ds;
	END;	

//this attribute is designed to be scheduled to run every 1 minutes
//using ECLScheduler (WHEN(cron))
//it autoincrements the filename so that we can see each run's completion
#workunit('name', 'Stress Test')
...
output(base_flpszanbm_1,,_Certification.Setup.filename + '_STRESSTEST_'+ INTFORMAT(random(),10,1),overwrite ) : WHEN(CRON('* * * * *'),COUNT(10));

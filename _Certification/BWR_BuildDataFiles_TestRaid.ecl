Base := _Certification.Setup.NodeMult1;		//max = 20
Mult := _Certification.Setup.NodeMult2;		//max = 20
...
BlankSet  := DATASET([{'','',0,'',0,0,'','',1,0}],_Certification.Layout_full_format);

_Certification.Layout_full_format norm1(_Certification.Layout_full_format l, integer c) := transform
  self.fname := fname_i[c];
  self := l;
  end;
base_fn := normalize( BlankSet, Base, norm1(left, counter));

output(base_fn,,'~raid_test::temp::test1',overwrite);
output(base_fn,,'~raid_test::temp::test11',overwrite);
output(base_fn,,'~raid_test::temp::test12',overwrite);

_Certification.Layout_full_format norm2(_Certification.Layout_full_format l, integer c) := transform
  self.lname := lname_i[c];
  self := l;
  end;
base_fln := normalize( base_fn, Mult, norm2(left, counter));

output(base_fln,,'~raid_test::temp::test2',overwrite);

_Certification.Layout_full_format norm3(_Certification.Layout_full_format l, integer c) := transform
  self.PRANGE := PRANGE_i[c];
  self := l;
  end;
base_flpn := normalize( base_fln, 10, norm3(left, counter));

output(base_fln,,'~raid_test::temp::test3',overwrite);

_Certification.Layout_full_format norm4(_Certification.Layout_full_format l, integer c) := transform
  self.street := street_i[c];
  self := l;
  end;
base_flpsn := normalize( base_flpn, 10, norm4(left, counter));

output(base_fln,,'~raid_test::temp::test4',overwrite);

_Certification.Layout_full_format norm5(_Certification.Layout_full_format l, integer c) := transform
  self.zips := zips_i[c];
  self := l;
  end;
base_flpszn := normalize( base_flpsn, 10, norm5(left, counter));

output(base_fln,,'~raid_test::temp::test5',overwrite);

_Certification.Layout_full_format norm6(_Certification.Layout_full_format l, integer c) := transform
  self.age := age_i[c];
  self := l;
  end;
base_flpszan := normalize( base_flpszn, 10, norm6(left, counter));

output(base_fln,,'~raid_test::temp::test6',overwrite);

_Certification.Layout_full_format add_b_normalize(base_flpszan l, integer c) := transform
...
output(base_fln,,'~raid_test::temp::test7',overwrite);

_Certification.Layout_full_format add_m_normalize(base_flpszanb l, integer c) := transform
...
output(base_fln,,'~raid_test::temp::test8',overwrite);

...
#workunit('name','Scrubs_PhonesFeedback.BWR_Hygiene - Hygiene & Stats - SALT V3.11.4');
IMPORT Scrubs_PhonesFeedback,SALT311;
// First create an instantiated hygiene module
  infile := Scrubs_PhonesFeedback.In_PhonesFeedback;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_PhonesFeedback.hygiene(ip);
  ...
  OUTPUT(SALT311.MAC_Character_Counts.EclRecord(p,'Layout_PhonesFeedback'),NAMED('OptimizedLayout'));// File layout suggested by data

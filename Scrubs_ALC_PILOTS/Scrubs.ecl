...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Layout_ALC_PILOTS)
    ...
    UNSIGNED1 cert_lvl_Invalid;
    UNSIGNED1 cert_type_Invalid;
    UNSIGNED1 cert_type_secondary_Invalid;
    UNSIGNED1 county_cd_Invalid;
    UNSIGNED1 msa_Invalid;
    UNSIGNED1 nielsen_county_cd_Invalid;
    UNSIGNED1 rating_Invalid;
    UNSIGNED1 phone_Invalid;
    UNSIGNED1 list_id_Invalid;
    UNSIGNED1 scno_Invalid;
    UNSIGNED1 custno_Invalid;
  END;
  EXPORT  Bitmap_Layout := RECORD(Layout_ALC_PILOTS)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(Layout_ALC_PILOTS) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.cert_lvl_Invalid := Fields.InValid_cert_lvl((SALT32.StrType)le.cert_lvl);
    SELF.cert_type_Invalid := Fields.InValid_cert_type((SALT32.StrType)le.cert_type);
    SELF.cert_type_secondary_Invalid := Fields.InValid_cert_type_secondary((SALT32.StrType)le.cert_type_secondary);
    ...
    SELF.rating_Invalid := Fields.InValid_rating((SALT32.StrType)le.rating);
  ...
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Layout_ALC_PILOTS);
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.fname_Invalid << 0 ) + ( le.lname_Invalid << 1 ) + ( le.title_Invalid << 2 ) + ( le.state_Invalid << 3 ) + ( le.zip_Invalid << 4 ) + ( le.zip4_Invalid << 5 ) + ( le.cart_Invalid << 6 ) + ( le.bar_Invalid << 7 ) + ( le.gender_Invalid << 8 ) + ( le.addr_type_Invalid << 9 ) + ( le.cert_lvl_Invalid << 10 ) + ( le.cert_type_Invalid << 11 ) + ( le.cert_type_secondary_Invalid << 12 ) + ( le.county_cd_Invalid << 13 ) + ( le.msa_Invalid << 14 ) + ( le.nielsen_county_cd_Invalid << 15 ) + ( le.rating_Invalid << 16 ) + ( le.phone_Invalid << 17 ) + ( le.list_id_Invalid << 18 ) + ( le.scno_Invalid << 19 ) + ( le.custno_Invalid << 20 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,Layout_ALC_PILOTS);
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.cert_lvl_Invalid := (le.ScrubsBits1 >> 10) & 1;
    SELF.cert_type_Invalid := (le.ScrubsBits1 >> 11) & 1;
    SELF.cert_type_secondary_Invalid := (le.ScrubsBits1 >> 12) & 1;
    SELF.county_cd_Invalid := (le.ScrubsBits1 >> 13) & 1;
    SELF.msa_Invalid := (le.ScrubsBits1 >> 14) & 1;
    SELF.nielsen_county_cd_Invalid := (le.ScrubsBits1 >> 15) & 1;
    SELF.rating_Invalid := (le.ScrubsBits1 >> 16) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    cert_lvl_ALLOW_ErrorCount := COUNT(GROUP,h.cert_lvl_Invalid=1);
    cert_type_ALLOW_ErrorCount := COUNT(GROUP,h.cert_type_Invalid=1);
    cert_type_secondary_ALLOW_ErrorCount := COUNT(GROUP,h.cert_type_secondary_Invalid=1);
    ...
    rating_ALLOW_ErrorCount := COUNT(GROUP,h.rating_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.fname_Invalid,le.lname_Invalid,le.title_Invalid,le.state_Invalid,le.zip_Invalid,le.zip4_Invalid,le.cart_Invalid,le.bar_Invalid,le.gender_Invalid,le.addr_type_Invalid,le.cert_lvl_Invalid,le.cert_type_Invalid,le.cert_type_secondary_Invalid,le.county_cd_Invalid,le.msa_Invalid,le.nielsen_county_cd_Invalid,le.rating_Invalid,le.phone_Invalid,le.list_id_Invalid,le.scno_Invalid,le.custno_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_fname(le.fname_Invalid),Fields.InvalidMessage_lname(le.lname_Invalid),Fields.InvalidMessage_title(le.title_Invalid),Fields.InvalidMessage_state(le.state_Invalid),Fields.InvalidMessage_zip(le.zip_Invalid),Fields.InvalidMessage_zip4(le.zip4_Invalid),Fields.InvalidMessage_cart(le.cart_Invalid),Fields.InvalidMessage_bar(le.bar_Invalid),Fields.InvalidMessage_gender(le.gender_Invalid),Fields.InvalidMessage_addr_type(le.addr_type_Invalid),Fields.InvalidMessage_cert_lvl(le.cert_lvl_Invalid),Fields.InvalidMessage_cert_type(le.cert_type_Invalid),Fields.InvalidMessage_cert_type_secondary(le.cert_type_secondary_Invalid),Fields.InvalidMessage_county_cd(le.county_cd_Invalid),Fields.InvalidMessage_msa(le.msa_Invalid),Fields.InvalidMessage_nielsen_county_cd(le.nielsen_county_cd_Invalid),Fields.InvalidMessage_rating(le.rating_Invalid),Fields.InvalidMessage_phone(le.phone_Invalid),Fields.InvalidMessage_list_id(le.list_id_Invalid),Fields.InvalidMessage_scno(le.scno_Invalid),Fields.InvalidMessage_custno(le.custno_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.cert_lvl_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.cert_type_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.cert_type_secondary_Invalid,'ALLOW','UNKNOWN')
          ...
          ,CHOOSE(le.rating_Invalid,'ALLOW','UNKNOWN')
    ...
    SELF.FieldName := CHOOSE(c,'fname','lname','title','state','zip','zip4','cart','bar','gender','addr_type','cert_lvl','cert_type','cert_type_secondary','county_cd','msa','nielsen_county_cd','rating','phone','list_id','scno','custno','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_alphaspacequote','invalid_alphaspacequote','invalid_alphaspace','invalid_alpha','invalid_numeric','invalid_numeric','invalid_alphanumeric','invalid_numeric','invalid_gender','invalid_addr_type','invalid_alphapound','invalid_alphapound','invalid_alphanumericpound','invalid_alphanumeric','invalid_numeric','invalid_alphanumeric','invalid_numericpound','invalid_numeric','invalid_numeric','invalid_numeric','invalid_alphanumeric','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT32.StrType)le.fname,(SALT32.StrType)le.lname,(SALT32.StrType)le.title,(SALT32.StrType)le.state,(SALT32.StrType)le.zip,(SALT32.StrType)le.zip4,(SALT32.StrType)le.cart,(SALT32.StrType)le.bar,(SALT32.StrType)le.gender,(SALT32.StrType)le.addr_type,(SALT32.StrType)le.cert_lvl,(SALT32.StrType)le.cert_type,(SALT32.StrType)le.cert_type_secondary,(SALT32.StrType)le.county_cd,(SALT32.StrType)le.msa,(SALT32.StrType)le.nielsen_county_cd,(SALT32.StrType)le.rating,(SALT32.StrType)le.phone,(SALT32.StrType)le.list_id,(SALT32.StrType)le.scno,(SALT32.StrType)le.custno,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT32.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'cert_lvl:invalid_alphapound:ALLOW'
          ,'cert_type:invalid_alphapound:ALLOW'
          ,'cert_type_secondary:invalid_alphanumericpound:ALLOW'
          ,'county_cd:invalid_alphanumeric:ALLOW'
          ,'msa:invalid_numeric:ALLOW'
          ,'nielsen_county_cd:invalid_alphanumeric:ALLOW'
          ,'rating:invalid_numericpound:ALLOW'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_cert_lvl(1)
          ,Fields.InvalidMessage_cert_type(1)
          ,Fields.InvalidMessage_cert_type_secondary(1)
          ,Fields.InvalidMessage_county_cd(1)
          ,Fields.InvalidMessage_msa(1)
          ,Fields.InvalidMessage_nielsen_county_cd(1)
          ,Fields.InvalidMessage_rating(1)
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.cert_lvl_ALLOW_ErrorCount
          ,le.cert_type_ALLOW_ErrorCount
          ,le.cert_type_secondary_ALLOW_ErrorCount
          ,le.county_cd_ALLOW_ErrorCount
          ,le.msa_ALLOW_ErrorCount
          ,le.nielsen_county_cd_ALLOW_ErrorCount
          ,le.rating_ALLOW_ErrorCount

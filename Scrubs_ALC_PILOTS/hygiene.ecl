IMPORT SALT37;
EXPORT hygiene(dataset(layout_ALC_PILOTS) h) := MODULE
...
EXPORT Summary(SALT37.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    populated_cert_lvl_pcnt := AVE(GROUP,IF(h.cert_lvl = (TYPEOF(h.cert_lvl))'',0,100));
    maxlength_cert_lvl := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.cert_lvl)));
    avelength_cert_lvl := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.cert_lvl)),h.cert_lvl<>(typeof(h.cert_lvl))'');
    populated_cert_type_pcnt := AVE(GROUP,IF(h.cert_type = (TYPEOF(h.cert_type))'',0,100));
    maxlength_cert_type := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.cert_type)));
    avelength_cert_type := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.cert_type)),h.cert_type<>(typeof(h.cert_type))'');
    populated_cert_type_secondary_pcnt := AVE(GROUP,IF(h.cert_type_secondary = (TYPEOF(h.cert_type_secondary))'',0,100));
    maxlength_cert_type_secondary := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.cert_type_secondary)));
    avelength_cert_type_secondary := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.cert_type_secondary)),h.cert_type_secondary<>(typeof(h.cert_type_secondary))'');
    ...
    populated_rating_pcnt := AVE(GROUP,IF(h.rating = (TYPEOF(h.rating))'',0,100));
    maxlength_rating := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.rating)));
    avelength_rating := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.rating)),h.rating<>(typeof(h.rating))'');
...
SALT37.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT37.StrType)le.fname),TRIM((SALT37.StrType)le.lname),TRIM((SALT37.StrType)le.title),TRIM((SALT37.StrType)le.company),TRIM((SALT37.StrType)le.address1),TRIM((SALT37.StrType)le.address2),TRIM((SALT37.StrType)le.city),TRIM((SALT37.StrType)le.state),TRIM((SALT37.StrType)le.zip),TRIM((SALT37.StrType)le.zip4),TRIM((SALT37.StrType)le.cart),TRIM((SALT37.StrType)le.bar),TRIM((SALT37.StrType)le.gender),TRIM((SALT37.StrType)le.country),TRIM((SALT37.StrType)le.postal_cd),TRIM((SALT37.StrType)le.dpv),TRIM((SALT37.StrType)le.addr_type),TRIM((SALT37.StrType)le.cert_lvl),TRIM((SALT37.StrType)le.cert_type),TRIM((SALT37.StrType)le.cert_type_secondary),TRIM((SALT37.StrType)le.county_cd),TRIM((SALT37.StrType)le.msa),TRIM((SALT37.StrType)le.nielsen_county_cd),TRIM((SALT37.StrType)le.rating),TRIM((SALT37.StrType)le.phone),TRIM((SALT37.StrType)le.list_id),TRIM((SALT37.StrType)le.scno),TRIM((SALT37.StrType)le.keycode),TRIM((SALT37.StrType)le.custno)));
  SELF.FldNo := C;
END;
...
SHARED FldIds := DATASET([{1,'fname'}
      ...
      ,{27,'scno'}
      ,{28,'keycode'}
      ,{29,'custno'}],SALT37.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_cert_lvl((SALT37.StrType)le.cert_lvl),
    Fields.InValid_cert_type((SALT37.StrType)le.cert_type),
    Fields.InValid_cert_type_secondary((SALT37.StrType)le.cert_type_secondary),
    ...
    Fields.InValid_rating((SALT37.StrType)le.rating),

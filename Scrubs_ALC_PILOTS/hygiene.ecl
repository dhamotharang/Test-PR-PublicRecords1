...
EXPORT Summary(SALT32.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_cert_lvl := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.cert_lvl)));
    avelength_cert_lvl := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.cert_lvl)),h.cert_lvl<>(typeof(h.cert_lvl))'');
    populated_cert_type_pcnt := AVE(GROUP,IF(h.cert_type = (TYPEOF(h.cert_type))'',0,100));
    maxlength_cert_type := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.cert_type)));
    avelength_cert_type := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.cert_type)),h.cert_type<>(typeof(h.cert_type))'');
    populated_cert_type_secondary_pcnt := AVE(GROUP,IF(h.cert_type_secondary = (TYPEOF(h.cert_type_secondary))'',0,100));
    maxlength_cert_type_secondary := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.cert_type_secondary)));
    avelength_cert_type_secondary := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.cert_type_secondary)),h.cert_type_secondary<>(typeof(h.cert_type_secondary))'');
    ...
    maxlength_rating := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.rating)));
    avelength_rating := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.rating)),h.rating<>(typeof(h.rating))'');
...
SALT32.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT32.StrType)le.fname),TRIM((SALT32.StrType)le.lname),TRIM((SALT32.StrType)le.title),TRIM((SALT32.StrType)le.company),TRIM((SALT32.StrType)le.address1),TRIM((SALT32.StrType)le.address2),TRIM((SALT32.StrType)le.city),TRIM((SALT32.StrType)le.state),TRIM((SALT32.StrType)le.zip),TRIM((SALT32.StrType)le.zip4),TRIM((SALT32.StrType)le.cart),TRIM((SALT32.StrType)le.bar),TRIM((SALT32.StrType)le.gender),TRIM((SALT32.StrType)le.country),TRIM((SALT32.StrType)le.postal_cd),TRIM((SALT32.StrType)le.dpv),TRIM((SALT32.StrType)le.addr_type),TRIM((SALT32.StrType)le.cert_lvl),TRIM((SALT32.StrType)le.cert_type),TRIM((SALT32.StrType)le.cert_type_secondary),TRIM((SALT32.StrType)le.county_cd),TRIM((SALT32.StrType)le.msa),TRIM((SALT32.StrType)le.nielsen_county_cd),TRIM((SALT32.StrType)le.rating),TRIM((SALT32.StrType)le.phone),TRIM((SALT32.StrType)le.list_id),TRIM((SALT32.StrType)le.scno),TRIM((SALT32.StrType)le.keycode),TRIM((SALT32.StrType)le.custno)));
  SELF.FldNo := C;
END;
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_cert_lvl((SALT32.StrType)le.cert_lvl),
    Fields.InValid_cert_type((SALT32.StrType)le.cert_type),
    Fields.InValid_cert_type_secondary((SALT32.StrType)le.cert_type_secondary),
    ...
    Fields.InValid_rating((SALT32.StrType)le.rating),

...
export MFind_raw := module
	...
	export get_Vids_from_dids(dataset(doxie.layout_references) in_dids,
		...
		res := join(dedup(sort(in_dids,did),did),key,
		            keyed(left.did = right.did),
								transform(MFindV2_Services.layout_Search_ids,self.vid := trim(right.trim_vid,all)),
								keep(10000));
  ...
  export report_view := module
		...
		export by_Vid(dataset(MFindV2_Services.layout_Search_ids) in_Vids):= function
		  return MFindV2_Services.get_Mfind_byVids(in_Vids).report;
		end;
	...
	export search_view := module
		...
		export by_Vid(dataset(MFindV2_Services.layout_Search_ids) in_Vids):= function
			return MFindV2_Services.get_Mfind_byVids(in_Vids).search;
		end;

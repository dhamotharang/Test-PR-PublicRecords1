...
EXPORT MakeFT_invalid_alphanumericpoundspacequote(SALT32.StrType s0) := FUNCTION
  s1 := SALT32.stringfilter(s0,' \'#ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_alphanumericpoundspacequote(SALT32.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT32.StringFilter(s,' \'#ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'))));
EXPORT InValidMessageFT_invalid_alphanumericpoundspacequote(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInChars(' \'#ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'),SALT32.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_business_ind(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_business_ind(SALT32.StrType s) := WHICH(((SALT32.StrType) s) NOT IN ['1','2',' ']);
EXPORT InValidMessageFT_invalid_business_ind(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInEnum('1|2| '),SALT32.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_number_of_lawyers_range(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_number_of_lawyers_range(SALT32.StrType s) := WHICH(((SALT32.StrType) s) NOT IN ['01','02','03','04','05','06','07','08','09',' ']);
EXPORT InValidMessageFT_invalid_number_of_lawyers_range(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInEnum('01|02|03|04|05|06|07|08|09| '),SALT32.HygieneErrors.Good);
...
EXPORT SALT32.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'fname','lname','title','company','address1','address2','city','state','zip','zip4','cart','bar','gender','country','postal_cd','dpv','addr_type','business_ind','county_cd','msa','nielsen_county_cd','number_of_lawyers_range','practice_area','title_cd','phone','list_id','scno','keycode','custno');
EXPORT FieldNum(SALT32.StrType fn) := CASE(fn,'fname' => 0,'lname' => 1,'title' => 2,'company' => 3,'address1' => 4,'address2' => 5,'city' => 6,'state' => 7,'zip' => 8,'zip4' => 9,'cart' => 10,'bar' => 11,'gender' => 12,'country' => 13,'postal_cd' => 14,'dpv' => 15,'addr_type' => 16,'business_ind' => 17,'county_cd' => 18,'msa' => 19,'nielsen_county_cd' => 20,'number_of_lawyers_range' => 21,'practice_area' => 22,'title_cd' => 23,'phone' => 24,'list_id' => 25,'scno' => 26,'keycode' => 27,'custno' => 28,0);
...
EXPORT Make_title(SALT32.StrType s0) := MakeFT_invalid_alphanumericpoundspacequote(s0);
EXPORT InValid_title(SALT32.StrType s) := InValidFT_invalid_alphanumericpoundspacequote(s);
...
EXPORT Make_business_ind(SALT32.StrType s0) := MakeFT_invalid_business_ind(s0);
EXPORT InValid_business_ind(SALT32.StrType s) := InValidFT_invalid_business_ind(s);
...
EXPORT Make_number_of_lawyers_range(SALT32.StrType s0) := MakeFT_invalid_number_of_lawyers_range(s0);
EXPORT InValid_number_of_lawyers_range(SALT32.StrType s) := InValidFT_invalid_number_of_lawyers_range(s);
EXPORT InValidMessage_number_of_lawyers_range(UNSIGNED1 wh) := InValidMessageFT_invalid_number_of_lawyers_range(wh);
 
EXPORT Make_practice_area(SALT32.StrType s0) := MakeFT_invalid_alphanumericpound(s0);
EXPORT InValid_practice_area(SALT32.StrType s) := InValidFT_invalid_alphanumericpound(s);
EXPORT InValidMessage_practice_area(UNSIGNED1 wh) := InValidMessageFT_invalid_alphanumericpound(wh);
 
EXPORT Make_title_cd(SALT32.StrType s0) := MakeFT_invalid_numericpound(s0);
EXPORT InValid_title_cd(SALT32.StrType s) := InValidFT_invalid_numericpound(s);

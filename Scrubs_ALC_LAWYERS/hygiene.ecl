...
EXPORT Summary(SALT37.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_business_ind := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.business_ind)));
    avelength_business_ind := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.business_ind)),h.business_ind<>(typeof(h.business_ind))'');
    ...
    maxlength_number_of_lawyers_range := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.number_of_lawyers_range)));
    avelength_number_of_lawyers_range := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.number_of_lawyers_range)),h.number_of_lawyers_range<>(typeof(h.number_of_lawyers_range))'');
    populated_practice_area_pcnt := AVE(GROUP,IF(h.practice_area = (TYPEOF(h.practice_area))'',0,100));
    maxlength_practice_area := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.practice_area)));
    avelength_practice_area := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.practice_area)),h.practice_area<>(typeof(h.practice_area))'');
    populated_title_cd_pcnt := AVE(GROUP,IF(h.title_cd = (TYPEOF(h.title_cd))'',0,100));
    maxlength_title_cd := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.title_cd)));
    avelength_title_cd := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.title_cd)),h.title_cd<>(typeof(h.title_cd))'');
...
SALT37.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT37.StrType)le.fname),TRIM((SALT37.StrType)le.lname),TRIM((SALT37.StrType)le.title),TRIM((SALT37.StrType)le.company),TRIM((SALT37.StrType)le.address1),TRIM((SALT37.StrType)le.address2),TRIM((SALT37.StrType)le.city),TRIM((SALT37.StrType)le.state),TRIM((SALT37.StrType)le.zip),TRIM((SALT37.StrType)le.zip4),TRIM((SALT37.StrType)le.cart),TRIM((SALT37.StrType)le.bar),TRIM((SALT37.StrType)le.gender),TRIM((SALT37.StrType)le.country),TRIM((SALT37.StrType)le.postal_cd),TRIM((SALT37.StrType)le.dpv),TRIM((SALT37.StrType)le.addr_type),TRIM((SALT37.StrType)le.business_ind),TRIM((SALT37.StrType)le.county_cd),TRIM((SALT37.StrType)le.msa),TRIM((SALT37.StrType)le.nielsen_county_cd),TRIM((SALT37.StrType)le.number_of_lawyers_range),TRIM((SALT37.StrType)le.practice_area),TRIM((SALT37.StrType)le.title_cd),TRIM((SALT37.StrType)le.phone),TRIM((SALT37.StrType)le.list_id),TRIM((SALT37.StrType)le.scno),TRIM((SALT37.StrType)le.keycode),TRIM((SALT37.StrType)le.custno)));
  SELF.FldNo := C;
END;
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_business_ind((SALT37.StrType)le.business_ind),
    ...
    Fields.InValid_number_of_lawyers_range((SALT37.StrType)le.number_of_lawyers_range),
    Fields.InValid_practice_area((SALT37.StrType)le.practice_area),
    Fields.InValid_title_cd((SALT37.StrType)le.title_cd),

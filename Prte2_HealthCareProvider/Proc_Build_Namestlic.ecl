...
EXPORT PROC_BUILD_namestlic(String pversion) := FUNCTION
namestlic_layout:=RECORD
  string28 lname;
  string2 st;
  string25 c_lic_nbr;
  string1 gender;
...
namestlic_Build :=  DATASET([ ],namestlic_layout);
 namestlic_key := INDEX(namestlic_Build,
 { lname,
  st,
  c_lic_nbr,
  gender,
 ...
 sec_range_weight100,
  v_city_name_weight100,
   ...
   billing_tax_id_weight100}
	 ,{namestlic_Build},
	 prte2.constants.prefix + 'key::healthcareprovider::' + pversion + '::namestlic'); 
  
	BUILDindex(namestlic_Key);

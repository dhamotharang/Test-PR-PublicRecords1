...
EXPORT PROC_BUILD_tax(String pversion) := FUNCTION
tax_layout:=RECORD
 ...
 tax_Build :=  DATASET([ ],tax_layout);
 tax_key := INDEX(tax_Build,
   {billing_tax_id,
   lnpid,
   fname_preferredname,
   fname,
  ...
  dob_day_weight100,
   ...
   lic_state_weight100},{tax_Build},
   prte2.constants.prefix + 'key::healthcareprovider::' + pversion + '::billing::tax'); 

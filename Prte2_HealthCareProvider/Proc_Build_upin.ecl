...
EXPORT PROC_BUILD_upin(String pversion) := FUNCTION
upin_layout:=RECORD
  string6 upin;
  unsigned6 lnpid;
 ...
 END;
 upin_Build :=  DATASET([ ],upin_layout);
 upin_key := INDEX(upin_Build,
 {   upin,
   lnpid,
   fname_preferredname,
   fname,
  ...
  fname_e2p_weight100,
   ...
   lic_state_weight100},{upin_Build},
	 prte2.constants.prefix + 'key::healthcareprovider::' + pversion + '::upin'); 

...
EXPORT PROC_BUILD_dea(String pversion) := FUNCTION
dea_layout:=RECORD
  string10 dea_number;
  unsigned6 lnpid;
 ...
 dea_Build :=  DATASET([ ],dea_layout);
 dea_key := INDEX(dea_Build,
    {dea_number,
   lnpid,
   fname_preferredname,
   fname,
  ...
  lname_initial_char_weight100,
   ...
   lic_state_weight100},{dea_Build},
	
	prte2.constants.prefix + 'key::healthcareprovider::' + pversion + '::dea'); 

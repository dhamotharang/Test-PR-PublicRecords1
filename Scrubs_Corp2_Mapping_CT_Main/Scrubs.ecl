...
EXPORT FromNone(DATASET(corp2_Mapping.LayoutsCommon.Main) h) := MODULE
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_process_date_Invalid << 12 ) + ( le.corp_inc_date_Invalid << 14 ) + ( le.corp_forgn_date_Invalid << 16 ) + ( le.corp_filing_date_Invalid << 18 ) + ( le.corp_status_date_Invalid << 20 ) + ( le.corp_merger_date_Invalid << 22 ) + ( le.corp_ra_resign_date_Invalid << 24 ) + ( le.corp_term_exist_exp_Invalid << 26 ) + ( le.cont_effective_date_Invalid << 28 ) + ( le.corp_dissolved_date_Invalid << 30 ) + ( le.corp_name_status_date_Invalid << 32 ) + ( le.corp_name_reservation_expiration_date_Invalid << 34 ) + ( le.corp_key_Invalid << 36 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 38 ) + ( le.corp_vendor_Invalid << 40 ) + ( le.corp_state_origin_Invalid << 41 ) + ( le.corp_legal_name_Invalid << 42 ) + ( le.corp_inc_state_Invalid << 43 ) + ( le.corp_forgn_state_desc_Invalid << 44 ) + ( le.corp_foreign_domestic_ind_Invalid << 45 ) + ( le.corp_for_profit_ind_Invalid << 46 ) + ( le.corp_ln_name_type_cd_Invalid << 47 ) + ( le.corp_ln_name_type_desc_Invalid << 48 ) + ( le.corp_orig_bus_type_cd_Invalid << 49 ) + ( le.corp_status_cd_Invalid << 50 ) + ( le.corp_orig_org_structure_cd_Invalid << 51 ) + ( le.corp_country_of_formation_Invalid << 52 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_filing_date_Invalid := (le.ScrubsBits1 >> 18) & 3;
    SELF.corp_status_date_Invalid := (le.ScrubsBits1 >> 20) & 3;
    SELF.corp_merger_date_Invalid := (le.ScrubsBits1 >> 22) & 3;
    SELF.corp_ra_resign_date_Invalid := (le.ScrubsBits1 >> 24) & 3;
    SELF.corp_term_exist_exp_Invalid := (le.ScrubsBits1 >> 26) & 3;
    SELF.cont_effective_date_Invalid := (le.ScrubsBits1 >> 28) & 3;
    SELF.corp_dissolved_date_Invalid := (le.ScrubsBits1 >> 30) & 3;
    SELF.corp_name_status_date_Invalid := (le.ScrubsBits1 >> 32) & 3;
    SELF.corp_name_reservation_expiration_date_Invalid := (le.ScrubsBits1 >> 34) & 3;
    SELF.corp_key_Invalid := (le.ScrubsBits1 >> 36) & 3;
    SELF.corp_orig_sos_charter_nbr_Invalid := (le.ScrubsBits1 >> 38) & 3;
    SELF.corp_vendor_Invalid := (le.ScrubsBits1 >> 40) & 1;
    SELF.corp_state_origin_Invalid := (le.ScrubsBits1 >> 41) & 1;
    SELF.corp_legal_name_Invalid := (le.ScrubsBits1 >> 42) & 1;
    SELF.corp_inc_state_Invalid := (le.ScrubsBits1 >> 43) & 1;
    SELF.corp_forgn_state_desc_Invalid := (le.ScrubsBits1 >> 44) & 1;
    SELF.corp_foreign_domestic_ind_Invalid := (le.ScrubsBits1 >> 45) & 1;
    SELF.corp_for_profit_ind_Invalid := (le.ScrubsBits1 >> 46) & 1;
    SELF.corp_ln_name_type_cd_Invalid := (le.ScrubsBits1 >> 47) & 1;
    SELF.corp_ln_name_type_desc_Invalid := (le.ScrubsBits1 >> 48) & 1;
    SELF.corp_orig_bus_type_cd_Invalid := (le.ScrubsBits1 >> 49) & 1;
    SELF.corp_status_cd_Invalid := (le.ScrubsBits1 >> 50) & 1;
    SELF.corp_orig_org_structure_cd_Invalid := (le.ScrubsBits1 >> 51) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.dt_vendor_first_reported_Invalid,le.dt_vendor_last_reported_Invalid,le.dt_first_seen_Invalid,le.dt_last_seen_Invalid,le.corp_ra_dt_first_seen_Invalid,le.corp_ra_dt_last_seen_Invalid,le.corp_process_date_Invalid,le.corp_inc_date_Invalid,le.corp_forgn_date_Invalid,le.corp_filing_date_Invalid,le.corp_status_date_Invalid,le.corp_merger_date_Invalid,le.corp_ra_resign_date_Invalid,le.corp_term_exist_exp_Invalid,le.cont_effective_date_Invalid,le.corp_dissolved_date_Invalid,le.corp_name_status_date_Invalid,le.corp_name_reservation_expiration_date_Invalid,le.corp_key_Invalid,le.corp_orig_sos_charter_nbr_Invalid,le.corp_vendor_Invalid,le.corp_state_origin_Invalid,le.corp_legal_name_Invalid,le.corp_inc_state_Invalid,le.corp_forgn_state_desc_Invalid,le.corp_foreign_domestic_ind_Invalid,le.corp_for_profit_ind_Invalid,le.corp_ln_name_type_cd_Invalid,le.corp_ln_name_type_desc_Invalid,le.corp_orig_bus_type_cd_Invalid,le.corp_status_cd_Invalid,le.corp_orig_org_structure_cd_Invalid,le.corp_country_of_formation_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_dt_vendor_first_reported(le.dt_vendor_first_reported_Invalid),Fields.InvalidMessage_dt_vendor_last_reported(le.dt_vendor_last_reported_Invalid),Fields.InvalidMessage_dt_first_seen(le.dt_first_seen_Invalid),Fields.InvalidMessage_dt_last_seen(le.dt_last_seen_Invalid),Fields.InvalidMessage_corp_ra_dt_first_seen(le.corp_ra_dt_first_seen_Invalid),Fields.InvalidMessage_corp_ra_dt_last_seen(le.corp_ra_dt_last_seen_Invalid),Fields.InvalidMessage_corp_process_date(le.corp_process_date_Invalid),Fields.InvalidMessage_corp_inc_date(le.corp_inc_date_Invalid),Fields.InvalidMessage_corp_forgn_date(le.corp_forgn_date_Invalid),Fields.InvalidMessage_corp_filing_date(le.corp_filing_date_Invalid),Fields.InvalidMessage_corp_status_date(le.corp_status_date_Invalid),Fields.InvalidMessage_corp_merger_date(le.corp_merger_date_Invalid),Fields.InvalidMessage_corp_ra_resign_date(le.corp_ra_resign_date_Invalid),Fields.InvalidMessage_corp_term_exist_exp(le.corp_term_exist_exp_Invalid),Fields.InvalidMessage_cont_effective_date(le.cont_effective_date_Invalid),Fields.InvalidMessage_corp_dissolved_date(le.corp_dissolved_date_Invalid),Fields.InvalidMessage_corp_name_status_date(le.corp_name_status_date_Invalid),Fields.InvalidMessage_corp_name_reservation_expiration_date(le.corp_name_reservation_expiration_date_Invalid),Fields.InvalidMessage_corp_key(le.corp_key_Invalid),Fields.InvalidMessage_corp_orig_sos_charter_nbr(le.corp_orig_sos_charter_nbr_Invalid),Fields.InvalidMessage_corp_vendor(le.corp_vendor_Invalid),Fields.InvalidMessage_corp_state_origin(le.corp_state_origin_Invalid),Fields.InvalidMessage_corp_legal_name(le.corp_legal_name_Invalid),Fields.InvalidMessage_corp_inc_state(le.corp_inc_state_Invalid),Fields.InvalidMessage_corp_forgn_state_desc(le.corp_forgn_state_desc_Invalid),Fields.InvalidMessage_corp_foreign_domestic_ind(le.corp_foreign_domestic_ind_Invalid),Fields.InvalidMessage_corp_for_profit_ind(le.corp_for_profit_ind_Invalid),Fields.InvalidMessage_corp_ln_name_type_cd(le.corp_ln_name_type_cd_Invalid),Fields.InvalidMessage_corp_ln_name_type_desc(le.corp_ln_name_type_desc_Invalid),Fields.InvalidMessage_corp_orig_bus_type_cd(le.corp_orig_bus_type_cd_Invalid),Fields.InvalidMessage_corp_status_cd(le.corp_status_cd_Invalid),Fields.InvalidMessage_corp_orig_org_structure_cd(le.corp_orig_org_structure_cd_Invalid),Fields.InvalidMessage_corp_country_of_formation(le.corp_country_of_formation_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_country_of_formation_Invalid,'ALLOW','UNKNOWN'),'UNKNOWN'));
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_orig_bus_type_cd:invalid_orig_bus_type_cd:ENUM'
          ,'corp_status_cd:invalid_corp_status_cd:ENUM'
          ,'corp_orig_org_structure_cd:invalid_orig_org_structure_cd:ENUM'
          ,'corp_country_of_formation:invalid_alphablank:ALLOW','UNKNOWN');
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_corp_country_of_formation(1),'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_country_of_formation_ALLOW_ErrorCount,0);
      SELF.rulepcnt := 100 * CHOOSE(c
          ...
          ,le.corp_status_cd_ENUM_ErrorCount
          ,le.corp_orig_org_structure_cd_ENUM_ErrorCount
          ,le.corp_country_of_formation_ALLOW_ErrorCount,0) / le.TotalCnt + 0.5;
    END;
    SummaryInfo := NORMALIZE(SummaryStats,71,Into(LEFT,COUNTER));
    orb_r := RECORD
      AllErrors.Src;

EXPORT NONFCRA_CBBL_xml_chase(route,current_dt,previous_dt) := functionmacro



 file1_2:= distribute(dataset(route + scoring_project_pip.Output_Sample_Names.CBBL_Scores_XML_Chase_outfile + previous_dt, Scoring_Project_Macros.Global_Output_Layouts.NONFCRA_Chase_CBBL_Global_Layout,


thor),(integer)acctno);
 
 file2_2:= distribute(dataset(route + scoring_project_pip.Output_Sample_Names.CBBL_Scores_XML_Chase_outfile + current_dt,Scoring_Project_Macros.Global_Output_Layouts.NONFCRA_Chase_CBBL_Global_Layout,
...
ppd:=if(pfc!= 0 and cfc=0,1,(cf/cfc)*100);
		    ...
		    Scoring_QA.Range_function_module.Range_Function_200(DS2,'phonescore',dis21);
		  	...
		  	Scoring_QA.Range_function_module.Range_Function_200(DS2,'cmpyscore2',dis24);
				Scoring_QA.Range_function_module.Range_Function_214(DS2,'cmpyaddrscore',dis25);
		    ...
		    Scoring_QA.Range_function_module.Average_func(DS2,'phonescore',ave11);
		  	...
		  	Scoring_QA.Range_function_module.Average_func(DS2,'cmpyscore2',ave14);
				Scoring_QA.Range_function_module.Average_Function_2(DS2,'cmpyaddrscore',ave15);
...
compare_layout_stats func1(result2_stats l):=transform
	...
	compare_layout := RECORD
				 ...
				 did_results := DATASET([{'nonfcra_cbbl','xml','did','<DID not equal>',count(file1),count(file2),count(file2)-count(file1),pfc,'<DID not equal>',pf,cf,'','','','',pd,abd,ppd,0}
	                    ], compare_layout);
   	
							
   
		
		FileNameNewLogical:='~ScoringQA::bss::stats::'+ scoring_project_pip.Output_Sample_Names.CBBL_Scores_XML_Chase_outfile[2..] + current_dt;
		
		FileNameNewLogical1:='~ScoringQA::bss::averages::'+ scoring_project_pip.Output_Sample_Names.CBBL_Scores_XML_Chase_outfile[2..] + current_dt;
		
		FileNameNewLogical2:='~ScoringQA::bss::dids::'+ scoring_project_pip.Output_Sample_Names.CBBL_Scores_XML_Chase_outfile[2..] + current_dt;

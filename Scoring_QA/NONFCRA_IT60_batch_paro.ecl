EXPORT NONFCRA_IT60_batch_paro(route ,name,current_dt,previous_dt) := functionmacro
...
compare_layout_stats func1(result2_stats l):=transform
	...
	compare_layout := RECORD
		  ...
		  did_results := DATASET([{'IT60_paro','batch','did','<DID not equal>',count(file1),count(file2),count(file2)-count(file1),pfc,'<DID not equal>',pf,cf,'','','','',pd,abd,ppd,0}
	                    ], compare_layout);

EXPORT FCRA_RiskView_batch_capitalone_attributes_v3(route,current_dt,previous_dt):= functionmacro



file1_2:= dataset(route + scoring_project_pip.Output_Sample_Names.RV_Attributes_V3_BATCH_CapOne_outfile + previous_dt,Scoring_Project_Macros.Global_Output_Layouts.FCRA_RiskView_BATCH_Capitalone_Attributes_V3_Global_Layout,

thor);
file2_2:= dataset(route + scoring_project_pip.Output_Sample_Names.RV_Attributes_V3_BATCH_CapOne_outfile + current_dt, Scoring_Project_Macros.Global_Output_Layouts.FCRA_RiskView_BATCH_Capitalone_Attributes_V3_Global_Layout,
...
ppd:=if(pfc!= 0 and cfc=0,1,(cf/cfc)*100);
         	...
         	Scoring_QA.Range_function_module.range_function_50(DS2,'paagenewestrecord',ran11);
   				Scoring_QA.Range_function_module.range_function_57(DS2,'palenofres',ran12);
         	...
         	ran:= ran1  + ran2  + ran3  + ran4  + ran5  + ran6  + ran7  + ran8  + ran9  + ran10 +
         	  ...
         	  Scoring_QA.Range_function_module.range_function_31(DS2,'lienlandlordtenantreleasedcount',ran0_42);
   				  Scoring_QA.Range_function_module.range_function_31(DS2,'lienjudgmentreleasedcount',ran0_43);
   					...
   					Scoring_QA.Range_function_module.range_function_25(DS2,'bankruptcy_count30',ran0_47);
   					Scoring_QA.Range_function_module.range_function_25(DS2,'bankruptcy_count90',ran0_48);
   					Scoring_QA.Range_function_module.range_function_25(DS2,'bankruptcy_count180',ran0_49);
   					Scoring_QA.Range_function_module.range_function_25(DS2,'bankruptcy_count12',ran0_50);
   					Scoring_QA.Range_function_module.range_function_25(DS2,'bankruptcy_count24',ran0_51);
   					Scoring_QA.Range_function_module.range_function_25(DS2,'bankruptcy_count36',ran0_52);
   					Scoring_QA.Range_function_module.range_function_25(DS2,'bankruptcy_count60',ran0_53);
   				  ...
   				  Scoring_QA.Range_function_module.Distinct_function(DS2,'ssndeceased',dis10);
   					...
   					Scoring_QA.Range_function_module.range_function_109(DS2,'lowissuedate',dis14);
   				  ...
   				  Scoring_QA.Range_function_module.range_function_25(DS2,'num_nonderogs36',dis120);
   					...
   					Scoring_QA.Range_function_module.range_function_103(DS2,'expire_date_last_proflic',dis124);
      	...
      	Scoring_QA.Range_function_module.Average_func(DS2,'propagenewestpurchase',ave60);
      	  ...
      	  Scoring_QA.Range_function_module.Average_func(DS2,'lienlandlordtenantreleasedcount',ave112);
				  ...
				  Scoring_QA.Range_function_module.Average_func(DS2,'num_nonderogs60',ave140);
					...
					Scoring_QA.Range_function_module.Average_func(DS2,'total_number_derogs',ave148);
					Scoring_QA.Range_function_module.Average_func(DS2,'felonies',ave149);
					Scoring_QA.Range_function_module.Average_func(DS2,'felonies12',ave150);
					Scoring_QA.Range_function_module.Average_func(DS2,'felonies180',ave151);
				  Scoring_QA.Range_function_module.Average_func(DS2,'felonies24',ave152);
					Scoring_QA.Range_function_module.Average_func(DS2,'felonies30',ave153);
					Scoring_QA.Range_function_module.Average_func(DS2,'felonies36',ave154);
					Scoring_QA.Range_function_module.Average_func(DS2,'felonies60',ave155);
					Scoring_QA.Range_function_module.Average_func(DS2,'felonies90',ave156);
					Scoring_QA.Range_function_module.Average_func(DS2,'educationattendedcollege',ave157);
					Scoring_QA.Range_function_module.Average_func(DS2,'educationinstitutionrating',ave158);
					Scoring_QA.Range_function_module.Average_func(DS2,'educationprogram2yr',ave159);
					Scoring_QA.Range_function_module.Average_func(DS2,'educationprogram4yr',ave160);
					Scoring_QA.Range_function_module.Average_func(DS2,'educationprogramgraduate',ave161);
					Scoring_QA.Range_function_module.Average_func(DS2,'educationinstitutionprivate',ave162);
      	   ...
      	   avearage:= ave1  + ave2  + ave3  + ave5  + ave6  + ave7  + ave8  + ave9  + ave10 +
                ...
                ave81 + ave82 + ave83 + ave84 + ave85 + ave86 + ave87 + ave88 + ave89 + ave90 + 
				       ...
				       ave101 + ave102 + ave103 + ave104 + ave105 + ave106 + ave107 + ave108 + ave109 + ave110+ 
						   ave111 + ave112 + ave113 + ave114 + ave115  +
							 ave124 + ave125 + ave126 + ave127 + ave128 + ave129 + ave130+
						   ave131 + ave132 + ave133 + ave134 + ave135 + ave136 + ave137 + ave138 + ave139 + ave140 + 
							 ave141 + ave142 + ave143 + ave144 + ave145 + ave146 + ave147 +ave148 +ave149 +ave150 +ave151 + 
								ave152 + ave153 + ave154 + ave155 + ave156 + ave157 + ave158 + ave159 + ave160  + ave161 + ave162;
   ...
   result4_stats:=avearage;
    ...
    END;
		...
		compare_layout_stats func(result4_stats l):=transform
                                         self.file_version:='RISKVIEW_v3_capone_prescreen';
																				 self.mode:='batch';
...
compare_layout_stats func1(result2_stats l):=transform
	...
	compare_layout := RECORD
		  ...
		  did_results := DATASET([{'RISKVIEW_v3_capone_prescreen','batch','did','<DID not equal>',count(file1),count(file2),count(file2)-count(file1),pfc,'<DID not equal>',pf,cf,'','','','',pd,abd,ppd,0}
	                    ], compare_layout);
		...
		FileNameNewLogical:='~ScoringQA::bss::stats::'+ scoring_project_pip.Output_Sample_Names.RV_Attributes_V3_BATCH_CapOne_outfile[2..] + current_dt;
		
		FileNameNewLogical1:='~ScoringQA::bss::averages::'+ scoring_project_pip.Output_Sample_Names.RV_Attributes_V3_BATCH_CapOne_outfile[2..] + current_dt;
		
		FileNameNewLogical2:='~ScoringQA::bss::dids::'+ scoring_project_pip.Output_Sample_Names.RV_Attributes_V3_BATCH_CapOne_outfile[2..] + current_dt;

EXPORT NONFCRA_IT61_batch_paro(route ,name,current_dt,previous_dt) := functionmacro



file1:= dataset(route + 'scoringqa::out::nonfcra::'+name +previous_dt,Scoring_Project_Macros.Global_Output_Layouts.NONFCRA_IT61_Paro_Global_Layout	,


 thor);
 
 file2:= dataset(route + 'scoringqa::out::nonfcra::'+ name +current_dt, Scoring_Project_Macros.Global_Output_Layouts.NONFCRA_IT61_Paro_Global_Layout,
...
compare_layout_stats func1(result2_stats l):=transform
	...
	compare_layout := RECORD
		  ...
		  did_results := DATASET([{'IT61_paro','batch','did','<DID not equal>',count(file1),count(file2),count(file2)-count(file1),pfc,'<DID not equal>',pf,cf,'','','','',pd,abd,ppd,0}
	                    ], compare_layout);

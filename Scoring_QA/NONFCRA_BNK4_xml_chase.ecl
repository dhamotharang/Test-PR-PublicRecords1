EXPORT NONFCRA_BNK4_xml_chase(route,current_dt,previous_dt) := functionmacro



 file1_2:= distribute(dataset(route + scoring_project_pip.Output_Sample_Names.BC10_Scores_XML_Chase_BNK4_outfile + previous_dt, Scoring_Project_Macros.Global_Output_Layouts.NONFCRA_Chase_BNK4_Global_Layout,


thor),(integer)acctno);
 
 file2_2:= distribute(dataset(route + scoring_project_pip.Output_Sample_Names.BC10_Scores_XML_Chase_BNK4_outfile + current_dt, Scoring_Project_Macros.Global_Output_Layouts.NONFCRA_Chase_BNK4_Global_Layout,
...
compare_layout_stats func1(result2_stats l):=transform
	...
	compare_layout := RECORD
				  ...
				  did_results := DATASET([{'nonfcra_bnk4_chase','xml','did','<DID not equal>',count(file1),count(file2),count(file2)-count(file1),pfc,'<DID not equal>',pf,cf,'','','','',pd,abd,ppd,0}
	                    ], compare_layout);
   						
				
   
		
		FileNameNewLogical:='~ScoringQA::bss::stats::'+ scoring_project_pip.Output_Sample_Names.BC10_Scores_XML_Chase_BNK4_outfile[2..] + current_dt;
		
		FileNameNewLogical1:='~ScoringQA::bss::averages::'+ scoring_project_pip.Output_Sample_Names.BC10_Scores_XML_Chase_BNK4_outfile[2..] + current_dt;
		
		FileNameNewLogical2:='~ScoringQA::bss::dids::'+ scoring_project_pip.Output_Sample_Names.BC10_Scores_XML_Chase_BNK4_outfile[2..] + current_dt;

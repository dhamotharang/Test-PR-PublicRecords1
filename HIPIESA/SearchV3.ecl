...
<message name="SASearch">
  ...
  <part name="ProviderTaxID"        type="xsd:string"/>
...
EXPORT SearchV3(string GcId, string JobId) := FUNCTION
IMPORT STD;
...
string InputProviderTaxID       := '' : STORED('ProviderTaxID');
...
Layouts := HIPIESA.SearchV3Objects.Layouts;
Indexes := HIPIESA.SearchV3Objects.Indexes(GcId + '::' + JobId);
...
ProviderTaxIdIndex      := Indexes.ProviderTaxIdIndex;
...
Layouts.ProviderSearchLayout tInput() := TRANSFORM
  ...
  SELF.ProviderTaxID        := STD.Str.ToUpperCase(TRIM(InputProviderTaxID));
  ...
  SELF.State                := IF(IsAddressSearch = TRUE, IF(State <> '', State, HIPIESA.SearchV3Objects.CleanState(SELF.InputState)), '');
...
//By ProviderTaxId
dsByProviderTaxId := JOIN(ProviderSearchDs(ProviderTaxID <> ''), ProviderTaxIdIndex,
	KEYED(LEFT.ProviderTaxID = RIGHT.providertaxid),
	TRANSFORM(Layouts.recordIdLayout,
		SELF	:= RIGHT),
	LIMIT(0), KEEP(1000));
...
dsByAddress := MAP(
  ...
  HIPIESA.SearchV3Objects.ConditionalJoin(5, addressTables, addressSwitches)
);
...
boolean isTaxIdSearch         := EXISTS(ProviderSearchDs(ProviderTaxId <> ''));
boolean isKeySearch           := EXISTS(ProviderSearchDs(ProviderKey <> ''));

searchTables   := [dsByAddress, dsByFacilityName, dsByProviderLastName, dsByProviderFirstName, dsByProviderNpi, dsByProviderTaxId, dsByProviderKey];
searchSwitches := [isAddressSearch, isFacilityNameSearch, isLastNameSearch, isFirstNameSearch, isNpiSearch, isTaxIdSearch, isKeySearch];
recordIds := HIPIESA.SearchV3Objects.ConditionalJoin(7, searchTables, searchSwitches);

...
EXPORT ProviderTaxIdLayout := RECORD
  string10 providertaxid;
  unsigned8 recordid;
  string20 preferredfirstname;
  string20 phoneticlastname;
...
EXPORT SASearchInput:= RECORD
  ...
  STRING ProviderTaxID{XPATH('providertaxid')}:='';
...
EXPORT Indexes(string filenameSuffix) := MODULE
  ...
  EXPORT ProviderTaxIdIndexFile := '~sa::key::taxid::' + Suffix;
  SHARED ProviderTaxIdDs := DATASET('', Layouts.ProviderTaxIdLayout, THOR);
  EXPORT ProviderTaxIdIndex := INDEX(ProviderTaxIdDs, {(string10)providertaxid}, {ProviderTaxIdDs}, ProviderTaxIdIndexFile);
...
EXPORT ConditionalJoin(UNSIGNED1 tableCount, SET OF DATASET(Layouts.recordIdLayout) tableSet, SET OF BOOLEAN switchSet) := FUNCTION
  ...
  allRecords := CHOOSE(tableCount,
	  tableSet[1],
		...
		tableSet[1] + tableSet[2] + tableSet[3] + tableSet[4] + tableSet[5] + tableSet[6] + tableSet[7] + tableSet[8],
		tableSet[1] + tableSet[2] + tableSet[3] + tableSet[4] + tableSet[5] + tableSet[6] + tableSet[7] + tableSet[8] + tableSet[9],
		emptyTable
	);	
	...
	out8 := IF(tableCount >= 8,
	  IF(switchSet[8] = TRUE,
		  JOIN(out7, tableSet[8], LEFT.RecordId = RIGHT.RecordId, TRANSFORM(Layouts.recordIdLayout, SELF := LEFT), LIMIT(0), KEEP(10000)),
			out7
		),
		out7
	);
	out9 := IF(tableCount >= 9,
	  IF(switchSet[9] = TRUE,
		  JOIN(out8, tableSet[9], LEFT.RecordId = RIGHT.RecordId, TRANSFORM(Layouts.recordIdLayout, SELF := LEFT), LIMIT(0), KEEP(10000)),
			out8
		),
		out8
	);

  out := DEDUP(SORT(out9, RecordId), RecordId);

...
// Compile and publish this function to roxie as hipiesa.mainsearchv4
EXPORT MainSearchV4 := FUNCTION
  #OPTION('soapTraceLevel', '8');

  STRING GCID                     := 'V3'     : STORED('GCID');
  ...
  // IMPORTANT - this variable RoxieInternalServiceUrl needs to reflect the server URL on which this query is deployed:
	//Dev
  ...
  SASearchOutput SetError (SASearchInput L) := transform
	...
	SASearchInput tInput() := TRANSFORM
		...
		SELF.ProviderTaxID     := InputProviderTaxID;
  ...
  dOut := SOAPCALL(dIn,RoxieInternalServiceUrl,ServiceName,{dIn},DATASET(SASearchOutput),ONFAIL(SetError(LEFT)), TIMEOUT(30), RETRY(1));

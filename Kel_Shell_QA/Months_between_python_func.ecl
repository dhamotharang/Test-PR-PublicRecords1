IMPORT R, ut, Python;
EXPORT string Months_between_python_func(Recordof(Kel_Shell_QA.Layouts.AC_Lay) data_row,string cond, string def_par, set of string lay_set):= embed(Python)
import re
result =''
layout_string_old = lay_set
while ' ' in layout_string_old:
  layout_string_old.remove(' ')
while '\n' in layout_string_old:
  layout_string_old.remove('\n')
layout_string_new = [''] + layout_string_old
...
for word in dts:
    try:
        ...
        return 'NA'
if (dts[0] in def_vals) or (dts[1] in def_vals)  or (dts[2] in def_vals):
		return 'PASS'
if (dts[0] in def_vals) or (dts[1] in def_vals):
    return 'PASS'
else: 

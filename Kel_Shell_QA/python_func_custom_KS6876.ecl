IMPORT R, ut, python3; 
EXPORT string python_func_custom_KS6876(Recordof(Kel_Shell_QA.Layouts.AC_Lay) data_row,string cond, string def_par, set of string lay_set):= embed(python3)
...
if (dts[0] in def_vals) or (dts[1] in def_vals):
    ...
    #print evalu
    #return str(dt)
return str(dts[0] +','+ min(dts[1].split('|')) +','+ min(dts[2],dts[3]) +','+ dts[4])
endembed;
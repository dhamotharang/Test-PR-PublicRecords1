...
#IF(Kel_Shell_QA.Constants.PythonVersion = 2) // Allow for easy swapping between Python 2 and 3
IMPORT Python;
EXPORT string Months_between_python_func3(Recordof(Kel_Shell_QA.Layouts.AC_Lay) data_row,string cond, string def_par, set of string lay_set):= EMBED(python)
import re
result =''
str_list = lay_set
...
#ELSE
IMPORT Python3;
EXPORT string Months_between_python_func3(Recordof(Kel_Shell_QA.Layouts.AC_Lay) data_row,string cond, string def_par, set of string lay_set):= EMBED(Python3)
import re
result =''
str_list = lay_set
str_list = ' '.join(str_list).split() 
...
if (dts[0] in def_vals) or (dts[1] in def_vals)  or (dts[2] in def_vals)  or (dts[3] in def_vals):
    ...
    dt=min(dts[0].split('|')[0],dts[1].split('|')[0])
    dt2=dts[2].split('|')[0]

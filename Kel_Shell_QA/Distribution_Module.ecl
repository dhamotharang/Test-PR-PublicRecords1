...
// DS:=join(fileop(STD.Str.ToUpperCase(trim(fieldname,left,right))='INPUTFIRSTNAMEECHO'),
	...
	DS:=join(infile,RulesFile,STD.Str.ToUpperCase(trim(left.fieldname,left,right))= 
                      STD.Str.ToUpperCase(trim(right.eclfieldname,left,right)),inner);
...
#uniquename(tble)
%tble% := project(DS,transform({%lay%; recordof(DS)- {STRING Category;}},
...
// Creating BINS for valid ranges
			 ...
			 %Ran% :=project(%tble%,transform(%lay%, 
									 ...
									 self.attribute_value :=if(trim(left.OtherOutputRules,left,right) <> 'No BinSize' or trim(left.max,left,right) <> 'No Max',
									                        if(regexfind('[0-9]',TRIM((string)left.field_value,LEFT,RIGHT)) or length(TRIM((string)left.field_value,LEFT,RIGHT))= 0 ,kel_Shell_QA.CreateBins.CreateBins_ranges(left.fieldname,(string)left.field_value,(integer)left.min,(integer)left.max,(integer)left.OtherOutputRules,left.DefaultValues),'UNDEFINED'),
																						 'NA');									
/* 									 MAP(left.fieldname in ['AppendedLexIDScore']=> kel_Shell_QA.CreateBins.CreateBins_ranges(left.fieldname,(string)left.field_value,(integer)left.min,(integer)left.max,(integer)left.OtherOutputRules,left.DefaultValues),
   																							left.Category not in['Input Echo','Input Clean','Business Input Echo'] => kel_Shell_QA.CreateBins.CreateBins_ranges(left.fieldname,(string)left.field_value,(integer)left.min,(integer)left.max,(integer)left.OtherOutputRules,left.DefaultValues),
   																							'NA');
*/
									 self:=left;                                                        
										 ));
   		 ...
   		 %Ran2% :=project(%tble%,transform(%lay%, 
   										 ...
   										 kel_Shell_QA.CreateBins.CreateBins_valid_default(left.fieldname,(string)left.field_value,(integer)left.min,(integer)left.max,left.defaultvalues),'UNDEFINED'),
   																					 'NA');
...
#uniquename(Ran)
%Ran% :=project(%tble%,transform( {STRING Attribute;STRING Category;STRING Distribution_type;STRING Attribute_value; string Attribute_range;string val;} , 
               self.Distribution_type:='RANGE';
   						 self.attribute_value :=if(trim(left.OtherOutputRules,left,right) <> 'No BinSize' or trim(left.max,left,right) <> 'No Max',
							 ...
							 self.Attribute_range:=if(trim(left.OtherOutputRules,left,right) <> 'No BinSize' or trim(left.max,left,right) <> 'No Max',
							       if(regexfind('[0-9]',TRIM((string)left.field_value,LEFT,RIGHT)) or length(TRIM((string)left.field_value,LEFT,RIGHT))= 0 ,
										 kel_Shell_QA.CreateBins.CreateBins_Range(left.fieldname,(string)left.field_value,(integer)left.min,(integer)left.max,left.defaultvalues),'UNDEFINED'),
																					 'NA');												 
...
#UNIQUENAME(stat_group)
	...
	%stat_data%   := table (%Ran%,%stat_group%, %Ran%.Attribute,%Ran%.Category,%Ran%.Distribution_type,%Ran%.Attribute_value,%Ran%.Attribute_range);		

...
EXPORT FromNone(DATASET(Layout_BKForeclosure_Nod) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.nod_apn_Invalid := Fields.InValid_nod_apn((SALT311.StrType)le.nod_apn);
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.nod_apn_Invalid := (le.ScrubsBits1 >> 49) & 1;
    SELF.property_full_addr_Invalid := (le.ScrubsBits1 >> 50) & 1;
    SELF.prop_addr_city_Invalid := (le.ScrubsBits1 >> 51) & 1;
    SELF.prop_addr_state_Invalid := (le.ScrubsBits1 >> 52) & 3;
    SELF.prop_addr_zip5_Invalid := (le.ScrubsBits1 >> 54) & 3;
    SELF.prop_addr_zip4_Invalid := (le.ScrubsBits1 >> 56) & 3;
    SELF.apn_Invalid := (le.ScrubsBits1 >> 58) & 1;
    SELF.nod_source_Invalid := (le.ScrubsBits1 >> 59) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    nod_apn_ALLOW_ErrorCount := COUNT(GROUP,h.nod_apn_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.nod_apn_Invalid,'ALLOW','UNKNOWN')
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_nod_apn(1)

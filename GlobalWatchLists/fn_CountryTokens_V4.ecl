...
export fn_CountryTokens_V4(DATASET(GlobalWatchLists.Layout_GlobalWatchLists_V4.LayoutCountryTokenPrep) inData, 
															 boolean isSearchData = false) := function

rPrepTokens := record GlobalWatchLists.Layout_GlobalWatchLists_V4.LayoutCountryTokenPrep; dataset(TextSearchDataRecord) tokens; end;
rPrepTokens getWordTokens(inData l) := transform
	self.tokens := EntityScoreLib.TextSearchData(trim(l.CountryName), 3);
	self := l;
end;  
...
TRANSFORM
	...
	len := length(tkn);
	isCommon := EntityScoreLib.IsCountryCommonWord(tkn) or EntityScoreLib.IsDirectionCommonWord(tkn);
...
//output(word_tokens);
//output(normed_tokens);
//output(rolled_tokens);
//output(dsTokens);
//output(normedtokens);
//output(rolledtokens);

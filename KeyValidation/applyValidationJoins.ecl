...
//************************************************************************************************************************************//
EXPORT applyValidationJoins(__dsName, __keyName, __keyType, __parentType, __buildVersion,  
  ...
  dedupedParentFileD1 := helperCode.setUpFileForJoin(__parentFile, distribution1Fields, __isDev);
	...
	// if(std.file.fileExists(knownIssuesFileName), output('known Issues exist for '+ __keyName),output(dataset([], recordof(allUnmatchedRecords)), , knownIssuesFileName, thor));
	
	knownIssuesJoinCondition := if(__keyType='AutoKey', KeyValidation.getKnownIssuesJoinCondition(__join1Fields, __join2Fields), join1Condition);
  unmatchedRecords := helperCode.filterKnownIssues(allUnmatchedRecords, knownIssuesFileName, knownIssuesJoinCondition); 
	// unmatchedRecords := 	allUnmatchedRecords;																		
  ...
  knownIssuesCount :=  count(allUnmatchedRecords) - unmatchedRecordCount;
	...
	distribution2Fields := Vikram.mergeArray(__join2Fields);
	
	//If there are more than 500 unmatched records, 
	//the comparison report is shown only for the first 500 unmatched records
  unmatchedRecordsSample := if(unmatchedRecordCount> 500, choosen(unmatchedRecords, 500), unmatchedRecords);
	unmatchedRecordsWithAssociatedParentRecords := KeyValidation.generateDetailedReport(unmatchedRecordsSample, 
																																																							dedupedParentFileD1, distribution2Fields);

	keyLogicalFileName := if(__isDev,  '~qa::keyval::testing::'+__dsName+'::' +__buildVersion+'::' + __keyName,  
																										 '~qa::keyval::'+__dsName+'::' +__buildVersion+'::' + __keyName);
  ...
  //showing the unmatched results as well as the short comparison report on thor
		if(testResult = 'FAIL', output(unmatchedRecordsWithAssociatedParentRecords, , keyLogicalFileName, thor, overwrite, expire(10)));
	...
	//summary record that is returned with stats for the corresponding key
	 summaryRecord := dataset([{__keyName, __keyType, __parentType,parentDSCount, keyDSCount, joinedCount, 
														unmatchedRecordCount, knownIssuesCount, testResult,
														 if(__isDev, keyLogicalFileName, if(testResult = 'FAIL', keyLogicalFileName, 'No Errors found'))}],

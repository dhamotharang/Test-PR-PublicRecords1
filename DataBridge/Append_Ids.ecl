...
EXPORT Append_Ids := MODULE
	...
	// -- Standardizes names
	//////////////////////////////////////////////////////////////////////////////////////
	EXPORT fAppendDid(DATASET(DataBridge.Layouts.Base) pDataset) :=	FUNCTION

		DataBridge.Layouts.Temp.StandardizeInput tAddUniqueId(DataBridge.Layouts.Base L, UNSIGNED8 cnt) :=	TRANSFORM
			SELF.unique_id		:= cnt;
			SELF							:= L;
		END;   
    ...
    DataBridge.Layouts.Temp.DidSlim taddDID(DataBridge.Layouts.Temp.StandardizeInput L) :=  TRANSFORM
	    SELF.fname            := L.fname;
	    SELF.mname            := L.mname;
	    SELF.lname            := L.lname;
	    SELF.name_suffix      := L.name_suffix;
			...
			SELF.zip5             := L.zip;
			SELF.state            := L.st;
      SELF.phone            := L.clean_telephone_num;      
			SELF.did              := 0;
			SELF.did_score		    := 0;
			SELF			            := L;
			SELF							    := [];
    ...
    did_Add.MAC_Match_Flex(
	    ...
	    prim_range,                          //Prim_range  
			...
			phone,                               //Phone
	    did,   			                         //DID
	    DataBridge.Layouts.Temp.DidSlim,   //Output rec layout
    ...
    dGoodDIDs := DISTRIBUTE(postDID(did != 0), unique_id);
		...
		DataBridge.Layouts.Base tAssignDIDs(DataBridge.Layouts.Temp.UniqueId L,DataBridge.Layouts.Temp.DidSlim R) := TRANSFORM
			SELF.did          := R.did;
	...
	// -- Creates the linkIDs
	//////////////////////////////////////////////////////////////////////////////////////
	EXPORT fAppendBIP(DATASET(DataBridge.Layouts.Base) pDataset) :=	FUNCTION
		...
		//////////////////////////////////////////////////////////////////////////////////////
		DataBridge.Layouts.Temp.BIPSlim tSlimForBIPing(DataBridge.Layouts.Temp.StandardizeInput L) := TRANSFORM
    ...
    dSlimInput	:= project(dAddUniqueId(clean_company_name <> '') ,tSlimForBIPing(LEFT));
		...
		Business_Header_SS.MAC_Add_BDID_Flex(
			...
			,fein_not_used                        // fein              
			,bdid_not_used 								        // bdid												
			,DataBridge.Layouts.Temp.BIPSlim    // Output Layout 
    ...
    dBIP_dist           := DISTRIBUTE(dBIPOut(Ultid 	!= 0 OR 
		...
		DataBridge.Layouts.Base tAssignLinkids(DataBridge.Layouts.Temp.StandardizeInput L,DataBridge.Layouts.Temp.BIPSlim R) := TRANSFORM
			SELF.Ultid				:= R.Ultid;
	...
	EXPORT fAll(DATASET(DataBridge.Layouts.Base) pDataset, STRING pversion) := FUNCTION

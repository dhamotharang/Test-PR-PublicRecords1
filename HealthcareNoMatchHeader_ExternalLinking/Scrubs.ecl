...
EXPORT FromNone(DATASET(Layout_HEADER) h) := MODULE
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.SRC_Invalid << 0 ) + ( le.SSN_Invalid << 2 ) + ( le.DOB_Invalid << 4 ) + ( le.LEXID_Invalid << 6 ) + ( le.SUFFIX_Invalid << 8 ) + ( le.FNAME_Invalid << 10 ) + ( le.MNAME_Invalid << 12 ) + ( le.LNAME_Invalid << 14 ) + ( le.GENDER_Invalid << 16 ) + ( le.PRIM_NAME_Invalid << 18 ) + ( le.PRIM_RANGE_Invalid << 20 ) + ( le.SEC_RANGE_Invalid << 22 ) + ( le.CITY_NAME_Invalid << 24 ) + ( le.ST_Invalid << 26 ) + ( le.ZIP_Invalid << 28 ) + ( le.DT_FIRST_SEEN_Invalid << 30 ) + ( le.DT_LAST_SEEN_Invalid << 32 );
    SELF.ScrubsCleanBits1 := ( IF(le.SRC_wouldClean, 1, 0) << 0 ) + ( IF(le.SSN_wouldClean, 1, 0) << 1 ) + ( IF(le.DOB_wouldClean, 1, 0) << 2 ) + ( IF(le.LEXID_wouldClean, 1, 0) << 3 ) + ( IF(le.SUFFIX_wouldClean, 1, 0) << 4 ) + ( IF(le.FNAME_wouldClean, 1, 0) << 5 ) + ( IF(le.MNAME_wouldClean, 1, 0) << 6 ) + ( IF(le.LNAME_wouldClean, 1, 0) << 7 ) + ( IF(le.GENDER_wouldClean, 1, 0) << 8 ) + ( IF(le.PRIM_NAME_wouldClean, 1, 0) << 9 ) + ( IF(le.PRIM_RANGE_wouldClean, 1, 0) << 10 ) + ( IF(le.SEC_RANGE_wouldClean, 1, 0) << 11 ) + ( IF(le.CITY_NAME_wouldClean, 1, 0) << 12 ) + ( IF(le.ST_wouldClean, 1, 0) << 13 ) + ( IF(le.ZIP_wouldClean, 1, 0) << 14 ) + ( IF(le.DT_FIRST_SEEN_wouldClean, 1, 0) << 15 ) + ( IF(le.DT_LAST_SEEN_wouldClean, 1, 0) << 16 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.SSN_Invalid := (le.ScrubsBits1 >> 2) & 3;
    SELF.DOB_Invalid := (le.ScrubsBits1 >> 4) & 3;
    SELF.LEXID_Invalid := (le.ScrubsBits1 >> 6) & 3;
    SELF.SUFFIX_Invalid := (le.ScrubsBits1 >> 8) & 3;
    SELF.FNAME_Invalid := (le.ScrubsBits1 >> 10) & 3;
    SELF.MNAME_Invalid := (le.ScrubsBits1 >> 12) & 3;
    SELF.LNAME_Invalid := (le.ScrubsBits1 >> 14) & 3;
    SELF.GENDER_Invalid := (le.ScrubsBits1 >> 16) & 3;
    SELF.PRIM_NAME_Invalid := (le.ScrubsBits1 >> 18) & 3;
    SELF.PRIM_RANGE_Invalid := (le.ScrubsBits1 >> 20) & 3;
    SELF.SEC_RANGE_Invalid := (le.ScrubsBits1 >> 22) & 3;
    SELF.CITY_NAME_Invalid := (le.ScrubsBits1 >> 24) & 3;
    SELF.ST_Invalid := (le.ScrubsBits1 >> 26) & 3;
    SELF.ZIP_Invalid := (le.ScrubsBits1 >> 28) & 3;
    ...
    SELF.DOB_wouldClean := le.ScrubsCleanBits1 >> 2;
    SELF.LEXID_wouldClean := le.ScrubsCleanBits1 >> 3;
    SELF.SUFFIX_wouldClean := le.ScrubsCleanBits1 >> 4;
    SELF.FNAME_wouldClean := le.ScrubsCleanBits1 >> 5;
    SELF.MNAME_wouldClean := le.ScrubsCleanBits1 >> 6;
    SELF.LNAME_wouldClean := le.ScrubsCleanBits1 >> 7;
    SELF.GENDER_wouldClean := le.ScrubsCleanBits1 >> 8;
    SELF.PRIM_NAME_wouldClean := le.ScrubsCleanBits1 >> 9;
    SELF.PRIM_RANGE_wouldClean := le.ScrubsCleanBits1 >> 10;
    SELF.SEC_RANGE_wouldClean := le.ScrubsCleanBits1 >> 11;
    SELF.CITY_NAME_wouldClean := le.ScrubsCleanBits1 >> 12;
    SELF.ST_wouldClean := le.ScrubsCleanBits1 >> 13;
    SELF.ZIP_wouldClean := le.ScrubsCleanBits1 >> 14;
  ...
  EXPORT Changed := JOIN(ExpandedInfile,FromNew, LEFT.RID=RIGHT.RID AND (LEFT.SRC_Invalid <> RIGHT.SRC_Invalid OR LEFT.SSN_Invalid <> RIGHT.SSN_Invalid OR LEFT.DOB_Invalid <> RIGHT.DOB_Invalid OR LEFT.LEXID_Invalid <> RIGHT.LEXID_Invalid OR LEFT.SUFFIX_Invalid <> RIGHT.SUFFIX_Invalid OR LEFT.FNAME_Invalid <> RIGHT.FNAME_Invalid OR LEFT.MNAME_Invalid <> RIGHT.MNAME_Invalid OR LEFT.LNAME_Invalid <> RIGHT.LNAME_Invalid OR LEFT.GENDER_Invalid <> RIGHT.GENDER_Invalid OR LEFT.PRIM_NAME_Invalid <> RIGHT.PRIM_NAME_Invalid OR LEFT.PRIM_RANGE_Invalid <> RIGHT.PRIM_RANGE_Invalid OR LEFT.SEC_RANGE_Invalid <> RIGHT.SEC_RANGE_Invalid OR LEFT.CITY_NAME_Invalid <> RIGHT.CITY_NAME_Invalid OR LEFT.ST_Invalid <> RIGHT.ST_Invalid OR LEFT.ZIP_Invalid <> RIGHT.ZIP_Invalid OR LEFT.DT_FIRST_SEEN_Invalid <> RIGHT.DT_FIRST_SEEN_Invalid OR LEFT.DT_LAST_SEEN_Invalid <> RIGHT.DT_LAST_SEEN_Invalid), TRANSFORM(RIGHT));
END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h, BOOLEAN Glob = FALSE) := MODULE
  r := RECORD
    ...
    AnyRule_WithErrorsCount := COUNT(GROUP, h.SRC_Invalid > 0 OR h.SSN_Invalid > 0 OR h.DOB_Invalid > 0 OR h.LEXID_Invalid > 0 OR h.SUFFIX_Invalid > 0 OR h.FNAME_Invalid > 0 OR h.MNAME_Invalid > 0 OR h.LNAME_Invalid > 0 OR h.GENDER_Invalid > 0 OR h.PRIM_NAME_Invalid > 0 OR h.PRIM_RANGE_Invalid > 0 OR h.SEC_RANGE_Invalid > 0 OR h.CITY_NAME_Invalid > 0 OR h.ST_Invalid > 0 OR h.ZIP_Invalid > 0 OR h.DT_FIRST_SEEN_Invalid > 0 OR h.DT_LAST_SEEN_Invalid > 0);
    AnyRule_WithEditsCount := COUNT(GROUP, h.SRC_wouldClean OR h.SSN_wouldClean OR h.DOB_wouldClean OR h.LEXID_wouldClean OR h.SUFFIX_wouldClean OR h.FNAME_wouldClean OR h.MNAME_wouldClean OR h.LNAME_wouldClean OR h.GENDER_wouldClean OR h.PRIM_NAME_wouldClean OR h.PRIM_RANGE_wouldClean OR h.SEC_RANGE_wouldClean OR h.CITY_NAME_wouldClean OR h.ST_wouldClean OR h.ZIP_wouldClean OR h.DT_FIRST_SEEN_wouldClean OR h.DT_LAST_SEEN_wouldClean);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.SRC_Total_ErrorCount > 0, 1, 0) + IF(le.SSN_Total_ErrorCount > 0, 1, 0) + IF(le.DOB_Total_ErrorCount > 0, 1, 0) + IF(le.LEXID_Total_ErrorCount > 0, 1, 0) + IF(le.SUFFIX_Total_ErrorCount > 0, 1, 0) + IF(le.FNAME_Total_ErrorCount > 0, 1, 0) + IF(le.MNAME_Total_ErrorCount > 0, 1, 0) + IF(le.LNAME_Total_ErrorCount > 0, 1, 0) + IF(le.GENDER_Total_ErrorCount > 0, 1, 0) + IF(le.PRIM_NAME_Total_ErrorCount > 0, 1, 0) + IF(le.PRIM_RANGE_Total_ErrorCount > 0, 1, 0) + IF(le.SEC_RANGE_Total_ErrorCount > 0, 1, 0) + IF(le.CITY_NAME_Total_ErrorCount > 0, 1, 0) + IF(le.ST_Total_ErrorCount > 0, 1, 0) + IF(le.ZIP_Total_ErrorCount > 0, 1, 0) + IF(le.DT_FIRST_SEEN_Total_ErrorCount > 0, 1, 0) + IF(le.DT_LAST_SEEN_Total_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.SRC_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.SRC_QUOTES_ErrorCount > 0, 1, 0) + IF(le.SSN_QUOTES_ErrorCount > 0, 1, 0) + IF(le.SSN_ALLOW_ErrorCount > 0, 1, 0) + IF(le.DOB_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.DOB_QUOTES_ErrorCount > 0, 1, 0) + IF(le.LEXID_QUOTES_ErrorCount > 0, 1, 0) + IF(le.LEXID_ALLOW_ErrorCount > 0, 1, 0) + IF(le.SUFFIX_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.SUFFIX_QUOTES_ErrorCount > 0, 1, 0) + IF(le.FNAME_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.FNAME_QUOTES_ErrorCount > 0, 1, 0) + IF(le.MNAME_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.MNAME_QUOTES_ErrorCount > 0, 1, 0) + IF(le.LNAME_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.LNAME_QUOTES_ErrorCount > 0, 1, 0) + IF(le.GENDER_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.GENDER_QUOTES_ErrorCount > 0, 1, 0) + IF(le.PRIM_NAME_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.PRIM_NAME_QUOTES_ErrorCount > 0, 1, 0) + IF(le.PRIM_RANGE_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.PRIM_RANGE_QUOTES_ErrorCount > 0, 1, 0) + IF(le.SEC_RANGE_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.SEC_RANGE_QUOTES_ErrorCount > 0, 1, 0) + IF(le.CITY_NAME_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.CITY_NAME_QUOTES_ErrorCount > 0, 1, 0) + IF(le.ST_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.ST_QUOTES_ErrorCount > 0, 1, 0) + IF(le.ZIP_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.ZIP_QUOTES_ErrorCount > 0, 1, 0) + IF(le.DT_FIRST_SEEN_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.DT_FIRST_SEEN_QUOTES_ErrorCount > 0, 1, 0) + IF(le.DT_LAST_SEEN_LEFTTRIM_ErrorCount > 0, 1, 0) + IF(le.DT_LAST_SEEN_QUOTES_ErrorCount > 0, 1, 0);
    SELF.Rules_NoErrors := NumRules - SELF.Rules_WithErrors;
    SELF.Rules_WithEdits := IF(le.SRC_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.SRC_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.SSN_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.SSN_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.DOB_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.DOB_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.LEXID_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.LEXID_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.SUFFIX_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.SUFFIX_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.FNAME_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.FNAME_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.MNAME_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.MNAME_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.LNAME_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.LNAME_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.GENDER_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.GENDER_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.PRIM_NAME_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.PRIM_NAME_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.PRIM_RANGE_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.PRIM_RANGE_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.SEC_RANGE_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.SEC_RANGE_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.CITY_NAME_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.CITY_NAME_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.ST_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.ST_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.ZIP_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.ZIP_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.DT_FIRST_SEEN_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.DT_FIRST_SEEN_QUOTES_WouldModifyCount > 0, 1, 0) + IF(le.DT_LAST_SEEN_LEFTTRIM_WouldModifyCount > 0, 1, 0) + IF(le.DT_LAST_SEEN_QUOTES_WouldModifyCount > 0, 1, 0);
    SELF := le;
  END;
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    SELF.Src :=  le.SRC;
    UNSIGNED1 ErrNum := CHOOSE(c,le.SRC_Invalid,le.SSN_Invalid,le.DOB_Invalid,le.LEXID_Invalid,le.SUFFIX_Invalid,le.FNAME_Invalid,le.MNAME_Invalid,le.LNAME_Invalid,le.GENDER_Invalid,le.PRIM_NAME_Invalid,le.PRIM_RANGE_Invalid,le.SEC_RANGE_Invalid,le.CITY_NAME_Invalid,le.ST_Invalid,le.ZIP_Invalid,le.DT_FIRST_SEEN_Invalid,le.DT_LAST_SEEN_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_SRC(le.SRC_Invalid),Fields.InvalidMessage_SSN(le.SSN_Invalid),Fields.InvalidMessage_DOB(le.DOB_Invalid),Fields.InvalidMessage_LEXID(le.LEXID_Invalid),Fields.InvalidMessage_SUFFIX(le.SUFFIX_Invalid),Fields.InvalidMessage_FNAME(le.FNAME_Invalid),Fields.InvalidMessage_MNAME(le.MNAME_Invalid),Fields.InvalidMessage_LNAME(le.LNAME_Invalid),Fields.InvalidMessage_GENDER(le.GENDER_Invalid),Fields.InvalidMessage_PRIM_NAME(le.PRIM_NAME_Invalid),Fields.InvalidMessage_PRIM_RANGE(le.PRIM_RANGE_Invalid),Fields.InvalidMessage_SEC_RANGE(le.SEC_RANGE_Invalid),Fields.InvalidMessage_CITY_NAME(le.CITY_NAME_Invalid),Fields.InvalidMessage_ST(le.ST_Invalid),Fields.InvalidMessage_ZIP(le.ZIP_Invalid),Fields.InvalidMessage_DT_FIRST_SEEN(le.DT_FIRST_SEEN_Invalid),Fields.InvalidMessage_DT_LAST_SEEN(le.DT_LAST_SEEN_Invalid),'UNKNOWN'));
    ...
    SELF.FieldName := CHOOSE(c,'SRC','SSN','DOB','LEXID','SUFFIX','FNAME','MNAME','LNAME','GENDER','PRIM_NAME','PRIM_RANGE','SEC_RANGE','CITY_NAME','ST','ZIP','DT_FIRST_SEEN','DT_LAST_SEEN','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'DEFAULT','NUMBER','DEFAULT','NUMBER','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','UNKNOWN');
...
EXPORT StandardStats(DATASET(Layout_HEADER) inFile, BOOLEAN doErrorOverall = TRUE, BOOLEAN doErrorPerSrc = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(HealthcareNoMatchHeader_ExternalLinking, Fields, 'RECORDOF(scrubsSummaryOverall)', 'SRC');

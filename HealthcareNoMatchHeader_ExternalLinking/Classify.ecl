...
SHARED TotalClusters := Specificities(h).TotalClusters;
  SSN_tokens := PROJECT(Specificities(h).SSN_values_persisted,TRANSFORM(SALT311.Layout_Classify_Token,SELF.TokenValue := (SALT311.StrType)LEFT.SSN; SELF.TokenType := 2; SELF.Spc := LEFT.field_Specificity ));
  LEXID_tokens := PROJECT(Specificities(h).LEXID_values_persisted,TRANSFORM(SALT311.Layout_Classify_Token,SELF.TokenValue := (SALT311.StrType)LEFT.LEXID; SELF.TokenType := 4; SELF.Spc := LEFT.field_Specificity ));
  SUFFIX_tokens := PROJECT(Specificities(h).SUFFIX_values_persisted,TRANSFORM(SALT311.Layout_Classify_Token,SELF.TokenValue := (SALT311.StrType)LEFT.SUFFIX; SELF.TokenType := 5; SELF.Spc := LEFT.field_Specificity ));
  FNAME_tokens := PROJECT(Specificities(h).FNAME_values_persisted,TRANSFORM(SALT311.Layout_Classify_Token,SELF.TokenValue := (SALT311.StrType)LEFT.FNAME; SELF.TokenType := 6; SELF.Spc := LEFT.field_Specificity ));
  MNAME_tokens := PROJECT(Specificities(h).MNAME_values_persisted,TRANSFORM(SALT311.Layout_Classify_Token,SELF.TokenValue := (SALT311.StrType)LEFT.MNAME; SELF.TokenType := 7; SELF.Spc := LEFT.field_Specificity ));
  LNAME_tokens := PROJECT(Specificities(h).LNAME_values_persisted,TRANSFORM(SALT311.Layout_Classify_Token,SELF.TokenValue := (SALT311.StrType)LEFT.LNAME; SELF.TokenType := 8; SELF.Spc := LEFT.field_Specificity ));
  GENDER_tokens := PROJECT(Specificities(h).GENDER_values_persisted,TRANSFORM(SALT311.Layout_Classify_Token,SELF.TokenValue := (SALT311.StrType)LEFT.GENDER; SELF.TokenType := 9; SELF.Spc := LEFT.field_Specificity ));
  ...
  PRIM_RANGE_tokens := PROJECT(Specificities(h).PRIM_RANGE_values_persisted,TRANSFORM(SALT311.Layout_Classify_Token,SELF.TokenValue := (SALT311.StrType)LEFT.PRIM_RANGE; SELF.TokenType := 11; SELF.Spc := LEFT.field_Specificity ));
  ...
  ST_tokens := PROJECT(Specificities(h).ST_values_persisted,TRANSFORM(SALT311.Layout_Classify_Token,SELF.TokenValue := (SALT311.StrType)LEFT.ST; SELF.TokenType := 14; SELF.Spc := LEFT.field_Specificity ));
...
SHARED all_tokens0 := SSN_tokens + LEXID_tokens + SUFFIX_tokens + FNAME_tokens + MNAME_tokens + LNAME_tokens + GENDER_tokens + PRIM_NAME_tokens + PRIM_RANGE_tokens + SEC_RANGE_tokens + CITY_NAME_tokens + ST_tokens + ZIP_tokens;
SHARED all_tokens := SALT311.fn_process_multitokens(all_tokens0);
 
EXPORT TokenKeyName := '~'+'key::HealthcareNoMatchHeader_ExternalLinking::nomatch_id::Token::TokenKey';
 
EXPORT TokenKey := INDEX(all_tokens,{UNSIGNED4 TokenHash := HASH32(TokenValue),TokenType},{all_tokens},TokenKeyName);
 
EXPORT MultiTokenKeyName := '~'+'key::HealthcareNoMatchHeader_ExternalLinking::nomatch_id::Token::MultiTokenKey';
...
EXPORT MultiTokenKey := INDEX(all_tokens0(SALT311.WordCount(TokenValue)>1),{UNSIGNED4 TokenHash := HASH32(TokenValue),TokenType},{all_tokens0},MultiTokenKeyName);
  MAINNAME_tokens := PROJECT(Specificities(h).MAINNAME_values_persisted,TRANSFORM(SALT311.Layout_Classify_Concept,SELF.ConceptHash := LEFT.MAINNAME; SELF.TokenType := 18; SELF.Spc := LEFT.field_Specificity ));
  ADDR1_tokens := PROJECT(Specificities(h).ADDR1_values_persisted,TRANSFORM(SALT311.Layout_Classify_Concept,SELF.ConceptHash := LEFT.ADDR1; SELF.TokenType := 19; SELF.Spc := LEFT.field_Specificity ));
  LOCALE_tokens := PROJECT(Specificities(h).LOCALE_values_persisted,TRANSFORM(SALT311.Layout_Classify_Concept,SELF.ConceptHash := LEFT.LOCALE; SELF.TokenType := 20; SELF.Spc := LEFT.field_Specificity ));
  ADDRESS_tokens := PROJECT(Specificities(h).ADDRESS_values_persisted,TRANSFORM(SALT311.Layout_Classify_Concept,SELF.ConceptHash := LEFT.ADDRESS; SELF.TokenType := 21; SELF.Spc := LEFT.field_Specificity ));
  FULLNAME_tokens := PROJECT(Specificities(h).FULLNAME_values_persisted,TRANSFORM(SALT311.Layout_Classify_Concept,SELF.ConceptHash := LEFT.FULLNAME; SELF.TokenType := 22; SELF.Spc := LEFT.field_Specificity ));
SHARED all_tokens1 := MAINNAME_tokens + ADDR1_tokens + LOCALE_tokens + ADDRESS_tokens + FULLNAME_tokens;
 
EXPORT ConceptKeyName := '~'+'key::HealthcareNoMatchHeader_ExternalLinking::nomatch_id::Token::ConceptKey';
...
shared MAINNAME_combinations := o_tot;
  Layout_ConceptTemplate Into(MAINNAME_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.FNAME_filled => 6, le.MNAME_filled => 7, le.LNAME_filled => 8,0);
    SELF.FieldNumber2 := MAP ( le.MNAME_filled AND SELF.FieldNumber1 != 7 => 7, le.LNAME_filled AND SELF.FieldNumber1 != 8 => 8,0);
    SELF.FieldNumber3 := MAP ( le.LNAME_filled AND SELF.FieldNumber1 != 8 AND SELF.FieldNumber2 != 8 => 8,0);
    SELF := le;
  END;
...
shared ADDR1_combinations := o_tot;
  Layout_ConceptTemplate Into(ADDR1_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.PRIM_RANGE_filled => 11, le.SEC_RANGE_filled => 12, le.PRIM_NAME_filled => 10,0);
...
shared LOCALE_combinations := o_tot;
  Layout_ConceptTemplate Into(LOCALE_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.CITY_NAME_filled => 13, le.ST_filled => 14, le.ZIP_filled => 15,0);
    SELF.FieldNumber2 := MAP ( le.ST_filled AND SELF.FieldNumber1 != 14 => 14, le.ZIP_filled AND SELF.FieldNumber1 != 15 => 15,0);
...
shared ADDRESS_combinations := o_tot;
  Layout_ConceptTemplate Into(ADDRESS_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.ADDR1_filled => 19, le.LOCALE_filled => 20,0);
    SELF.FieldNumber2 := MAP ( le.LOCALE_filled AND SELF.FieldNumber1 != 20 => 20,0);
    SELF := le;
  END;
...
shared FULLNAME_combinations := o_tot;
  Layout_ConceptTemplate Into(FULLNAME_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.MAINNAME_filled => 18, le.SUFFIX_filled => 5,0);
    SELF.FieldNumber2 := MAP ( le.SUFFIX_filled AND SELF.FieldNumber1 != 5 => 5,0);
    SELF := le;
  END;
shared FULLNAME_templates := project(FULLNAME_combinations,Into(LEFT));
SHARED all_templates := MAINNAME_templates + ADDR1_templates + LOCALE_templates + ADDRESS_templates + FULLNAME_templates;
 
EXPORT ConceptTemplatesKey := '~'+'key::HealthcareNoMatchHeader_ExternalLinking::nomatch_id::Token::ConceptTemplatesKey';

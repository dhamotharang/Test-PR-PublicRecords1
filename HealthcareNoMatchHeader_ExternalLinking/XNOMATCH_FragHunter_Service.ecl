/*--SOAP--
<message name="XNOMATCH_FragHunter_Service">
...
/*--INFO-- Return all available data for nomatch_id with similar fields to the one provided</p>
...
<p>DOB:FNAME</p>
*/
EXPORT XNOMATCH_FragHunter_Service := MACRO
  ...
  Template := DATASET([],HealthcareNoMatchHeader_ExternalLinking.Process_XNOMATCH_Layouts(pSrc,pVersion,pInfile).InputLayout);
  ...
  ,false,false,e_RID,e_nomatch_id}],HealthcareNoMatchHeader_ExternalLinking.Process_XNOMATCH_Layouts(pSrc,pVersion,pInfile).InputLayout);
 
  pm := HealthcareNoMatchHeader_ExternalLinking.MEOW_XNOMATCH(pSrc,pVersion,pInfile,Input_Data); // This module performs regular xnomatch_id functions
  ds := pm.DataToSearch;
  fids := SET(pm.Uid_Results,nomatch_id);
  odm := HealthcareNoMatchHeader_ExternalLinking.MEOW_XNOMATCH(pSrc,pVersion,pInfile,ds, true, fids);

...
output_file			:= '~nkoubsky::out::fcra_rvscores_v4_Dev196_' + thorlib.wuid(); 
...
// output(f, named('rvs_original_input'));
// output(count(f));
...
op_final :=  output(j_f,,output_file,CSV(heading(single), quote('"')));

// op_despray := FileServices.DeSpray('~sghatti::out::fcra_rvscores_v0_' + ut.GetDate  ,'10.48.72.175', 'G:\\Scoring Project\\landing\\'+ 'fcra_rvscores_v0_' + ut.GetDate +  '_' + ut.getTime() + '.csv',,,,TRUE);

// sequential(op_final, op_despray);

sequential(op_final);

export RVScores := 'success';

...
// f := DATASET('~tfuerstenberg::in::acc_750_in',prii_layout,csv(quote('"')));
//f := choosen(DATASET('~tfuerstenberg::in::fcb_2500_personal_in',prii_layout,csv(quote('"'))),1);
...
outfile_name := '~nkoubsky::out::RiskProcessing_Instant_ID_Batch_Dev192_' + thorlib.wuid();
...
neutral_roxie_IP := riskwise.shortcuts.Dev192;
...
Risk_Indicators.Layout_Batch_In make_batch_in(p_f le, integer c) := TRANSFORM
	...
	self.historydateyyyymm := 99999999;
	SELF := le;
	SELF := [];
END;
...
layout_soap_input make_rv_in(p_f le, integer c) := TRANSFORM
	...
	SELF.DataRestrictionMask := le.DataRestrictionMask;
	// self.batch_in.historydateyyyymm := 99999999;
	END;
...
resu := soapcall(soap_in, neutral_roxie_IP,
				'risk_indicators.InstantID_batch.0', {soap_in}, 
				DATASET(xlayout), RETRY(1), TIMEOUT(120),
...
EXPORT Instant_ID_Batch_Dev192 := 'Success';

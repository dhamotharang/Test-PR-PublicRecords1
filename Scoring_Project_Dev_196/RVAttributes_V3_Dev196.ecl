...
outfile_name := '~nkoubsky::out::fcra_rvattributes_v30_Dev196_' + thorlib.wuid();
...
//	self.HistoryDateYYYYMM := le.historydateyyyymm; //999999;
  ...
  // self.gateways := dataset([{'neutralroxie', riskwise.shortcuts.prod_batch_neutral}], risk_indicators.Layout_Gateways_In);
	...
	self.gateways := dataset([{'neutralroxie', neutral_roxie_IP}], risk_indicators.Layout_Gateways_In);	
...
// op_despray := FileServices.DeSpray('~sghatti::out::fcra_rvattributes_v30_' + ut.getdate ,'10.48.72.175', 'G:\\Scoring Project\\landing\\'+ 'fcra_rvattributes_v30_' + ut.getdate  + '_' + ut.getTime()  + '.csv',,,,TRUE);

// sequential(op_final, op_despray);

sequential(op_final);

EXPORT RVAttributes_30 := 'Success';
/*2013-09-19T22:44:56Z (Siva Ghatti)
...
outfile_name := '~nkoubsky::out::fcra_rvattributes_v4_Dev196_' + thorlib.wuid();
...
// f := choosen(DATASET('~thor40_241::riskviewattribute_input',prii_layout,csv(heading(1),separator(','),terminator('\n'),maxlength(4096))), 10);
...
layout_soap t_f(f le, INTEGER c) := TRANSFORM
	...
	self.gateways := dataset([{'neutralroxie', riskwise.shortcuts.dev196}], risk_indicators.Layout_Gateways_In);
...
// output(choosen(p_f, eyeball), named('RVA_Input'));
// output(count(p_f), named('rva_input_count'));
...
// output(choosen(j_f, eyeball), named('rva_results_flattened'));

output(count(j_f(errorcode<>'')),named('rva_error_count'));

op_final := output(j_f,,outfile_name,CSV(heading(single), quote('"')));


// op_despray := FileServices.DeSpray('~sghatti::out::fcra_rvattributes_v4_' + ut.getdate ,'10.48.72.175', 'G:\\Scoring Project\\landing\\'+ 'fcra_rvattributes_v4_' + ut.getdate  + '_' + ut.getTime()  + '.csv',,,,TRUE);

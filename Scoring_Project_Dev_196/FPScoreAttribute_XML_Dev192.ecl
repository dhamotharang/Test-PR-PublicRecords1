...
outputFile := '~nkoubsky::out::nonFcra_FPScoresAttributes_v2_dev192_'  + thorlib.wuid();

roxieIP := RiskWise.shortcuts.Dev192;  //dev192

f := IF(recordsToRun <= 0, DATASET( inputFile, layout_input, csv(quote('"'))),
                            CHOOSEN(DATASET(inputFile, layout_input, CSV(QUOTE('"'))), recordsToRun));


// f := dataset(filename,prii_layout, csv(quote('"')));

// output(choosen(f,eyeball), named('fpsa_original_input'));
...
resu := soapcall(dist_dataset, roxieIP,
				'Models.FraudAdvisor_Service.1', {dist_dataset}, 
				DATASET(layout_FDAttributesOut),
				PARALLEL(threads), TIMEOUT(120),
				retry(3),
				onFail(myFail(LEFT)));
...
EXPORT FPScoreAttribute_XML_Dev192 := output(rolled_to_1_record,,outputfile,CSV(heading(single), quote('"')) );
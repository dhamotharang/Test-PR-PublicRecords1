IMPORT lib_fileservices, _control, lib_STRINGlib, _Validate, did_add, didville, ut, std, UPI_DataBuild__old, 
...
EXPORT Append_LexID (string pVersion, boolean pUseProd, string gcid, unsigned1 pLexidThreshold, string pHistMode, string10 pBatch_jobid,
						dataset(UPI_DataBuild__old.Layouts_V2.input_processing)pBaseFile):= MODULE

	EXPORT LexID_append := FUNCTION
			
		temp_base_layout := record
			UPI_DataBuild__old.Layouts_V2.input_processing;
			string50 		temp_fname := '';
			string50 		temp_mname := '';
			string50 		temp_lname := '';
...
#STORED('did_add_force','thor'); // remove or set to 'roxi' to put recs through roxi
	  ...
	  // denormRecs := DENORMALIZE(sort(distribute(norm_base, hash(rec_number)),rec_number,local), sort_all,
		...
		RETURN project(denormRecs, UPI_DataBuild__old.Layouts_V2.Input_processing); // returning file in input_processing layout not base layout
	END;
END;

...
EXPORT Convert_Base_toHeader (string pVersion, boolean pUseProd, string gcid, string pHistMode) := MODULE
		...
		EXPORT Build_it := FUNCTION
			...
			pBaseFile		:= map(												 
											...
											NOTHOR(FileServices.GetSuperFileSubCount(UPI_DataBuild__old.Filenames_V2(pVersion, pUseProd, gcid, pHistMode).member_lBaseTemplate_built)) = 0 =>
														dataset([],UPI_DataBuild__old.Layouts_V2.base),
											UPI_DataBuild__old.Files_V2(pVersion,pUseProd,gcid,pHistMode).member_base.qa);
												 
			header_layout := record
				UPI_DataBuild__old.Layouts_V2.as_header;
				unsigned1	__tpe;
			end;

			header_layout xformHL(UPI_DataBuild__old.Layouts_V2.base L) := TRANSFORM

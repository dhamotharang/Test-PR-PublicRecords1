...
EXPORT Aggregate_Report(boolean pUseProd, string gcid, string pHistMode, dataset (UPI_DataBuild__old.Layouts_V2.jobID_list) pBatch_jobID_list, string pVersion_start, string pVersion_end):= MODULE
   ...
   d := dataset([{min_lexid_score, max_lexid_score, ave_lexid_score, cnt_lexid_change, pct_lexid_change, 
                  ...
                  UPI_DataBuild__old.Layouts_V2.aggregate_fields);       									

    return d;
	endmacro;
	
	export startFile				:= sort(dataset('~ushc::crk::slim_history::' + gcid + '::qa', UPI_DataBuild__old.Layouts_V2.slim_history, thor), source_rid, batch_jobID);
  ...
  export pBatch_jobID_set	:= set(pBatch_jobID_list,batch_jobID);
	...
	export cnt_new_crk	:= function
		...
		prev_base					:= UPI_DataBuild__old.Files_V2('',pUseProd,gcid,pHistMode).member_base.qa;

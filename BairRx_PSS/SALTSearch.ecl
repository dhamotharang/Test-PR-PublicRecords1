IMPORT Bair_ExternalLinkKeys_V2;
IMPORT BairRx_Common, BairRx_PSS;

EXPORT SALTSearch := MODULE
	
	SHARED GetMod(DATASET(BairRx_PSS.SALTLayout.LayoutIn) inParam) := Bair_ExternalLinkKeys_V2.MEOW_PS(inParam);
	...
	EXPORT GetRawRecs(DATASET(BairRx_PSS.SALTLayout.LayoutIn) inParam) := FUNCTION
		...
		dRawRecs := PROJECT(dRaw,
			...
			IF(BairRx_Common.Debug.IsMax(), output(choosen(dRaw, 25), named('dRaw'),extend));	
			IF(BairRx_Common.Debug.IsMax(), output(choosen(dRawRecs, 25), named('dRawRecs'),extend));	
	...
	IF(BairRx_Common.Debug.IsMax(), output(choosen(dRecsInc, 25), named('dRecsInc'),extend));		
	IF(BairRx_Common.Debug.IsMax(), output(choosen(dRecsClean, 25), named('dRecsClean'),extend));		
	IF(BairRx_Common.Debug.IsMax(), output(choosen(dEIDSequenceRecs, 25), named('dEIDSequenceRecs'),extend));		
	IF(BairRx_Common.Debug.IsMax(), output(choosen(dEIDCleanDate, 25), named('dEIDCleanDate'),extend));		
	IF(BairRx_Common.Debug.IsMax(), output(choosen(dRecsout, 25), named('dRecsout'),extend));		

...
EXPORT Raw := MODULE
	...
	EXPORT GetEvents(DATASET(BairRx_Common.Layouts.EID) dIDs) := FUNCTION
		dMODeltaRecs 	:= JOIN(dIDs, BairRx_Common.Keys.PayloadMOKey(TRUE), KEYED(LEFT.eid = RIGHT.eid) AND RIGHT.MOSTAMP = 0, TRANSFORM(BairRx_PSS.Layouts.LayoutMO,	SELF := RIGHT,SELF := []),LIMIT(0),KEEP(1));		
		dMOMainIDs 		:= JOIN(dIDs,dMODeltaRecs, LEFT.eid = RIGHT.eid, TRANSFORM(LEFT), LEFT ONLY);
		dMOMainRecs 	:= JOIN(dMOMainIDs, BairRx_Common.Keys.PayloadMOKey(FALSE), KEYED(LEFT.eid = RIGHT.eid) AND RIGHT.MOSTAMP = 0, TRANSFORM(BairRx_PSS.Layouts.LayoutMO,	SELF := RIGHT, SELF := []),LIMIT(0),KEEP(1));
		dMORecs := dMOMainRecs + dMODeltaRecs;	

...
EXPORT SearchService() := MACRO
  ...
  ds_in := ds_input[1];
	...
	//Checking that gc_id, industry type, and product code have some values.
  IF( gc_id_in = 0,  
  ...
  // delta_strict : 1=fail if delta query fails, 0=do not fail if delta query fails, default: 0
	...
	FDN_Services.Layouts.batch_search_rec xform2(iesp.frauddefensenetwork.t_FDNSearchRequest L) := TRANSFORM
	...
	//Validate at least one search param is provided
 	validationMod:= FDN_Services.ValidateInput.fn_checkIfValid(ds_seq[1]);
      	IF(~validationMod.isValidSearchInput,
      			FAIL( ut.constants_MessageCodes.FDN_NO_INPUT_PROVIDED, ut.MapMessageCodes(ut.constants_MessageCodes.FDN_NO_INPUT_PROVIDED)));
  ...
  //Common Point for Service Call and Internal Call
	ds_res := FDN_Services.Search_Records(ds_seq, gc_id_in, ind_type_in, 
																				product_code_in, ds_excl_ind, ds_rec_file, DeltaUse, DeltaStrict);
	...
	ds_res_filter := ds_res(doxie.DataPermission.use_FDNContributoryData OR   
									        classification_Permissible_use_access.file_type <> 
													                     FDN_Services.Constants.FileTypeCodes.CONTRIBUTORY);

  results := FDN_Services.GetResults(ds_res_filter);

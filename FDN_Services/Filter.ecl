IMPORT iesp, FDN_Services, BatchShare;
	...
	EXPORT Filter := MODULE
		...
		EXPORT filterRecs(DATASET(FDN_Services.Layouts.raw_rec) ds_raw_recs_in) := FUNCTION
	 		  		
   		fdrReportedDate							:=  ds_raw_recs_in(IF(hasTxt(filterby_StartingReportedDate) AND hasTxt(filterby_EndingReportedDate), reported_date BETWEEN filterby_StartingReportedDate AND filterby_EndingReportedDate,
																										 IF(hasTxt(filterby_StartingReportedDate), reported_date >= filterby_StartingReportedDate, reported_date >= filterby_EndingReportedDate)));
			fdrEventDate								:= 	fdrReportedDate(IF(hasTxt(filterby_StartingEventDate) AND hasTxt(filterby_EndingEventDate), event_date BETWEEN filterby_StartingEventDate AND filterby_EndingEventDate,
																									    IF(hasTxt(filterby_StartingEventDate), event_date >= filterby_StartingEventDate, event_date >= filterby_EndingEventDate)));

...
export  Match1L( StrType L1, unsigned2 lspec1, boolean lnne1, integer lfor1, Boolean linit1, integer edit1, StrType R1, unsigned2 rspec1) := FUNCTION
	...
	result1 := map( SALT31.WithinEditN(L1,R1,edit1)  => if(minWeight > lfor1*100, minWeight, FailedForce), 
	...
	result := MAP ( 
							 not linit1 and not lnne1 and SALT31.WithinEditN(L1,R1,edit1) => if(minWeight > lfor1*100, minWeight, FailedForce),	// boolean linit - initial flag,  boolean lnnel - nulls not allowed flag
							 not linit1 and lnne1 and ( L1 = '' or R1 = '' ) 			=> 0,
							 not linit1 and lnne1 and SALT31.WithinEditN(L1,R1,edit1) => if(minWeight > lfor1*100, minWeight, FailedForce),
...
export Match2L(
    ...
    L1BlankCost := IF ( lfor1 > 0, FailedForce, Failed );
	 ...
	 result := MAP(not lnne1  and not linit1  and lfor1 > 0 	     => r0,
								...
								linit1 or linit2                                			=> r0,
								SALT31.WithinEditN(L1,R1,edit1) and SALT31.WithinEditN(L2,R2,edit2) => if(weight1+weight2 > lfor1*100, min(lspec1, rspec1) + min(lspec2, rspec2), FailedForce), 
								SALT31.WithinEditN(L1,R2,edit1) and SALT31.WithinEditN(L2,R1,edit2) => if(weight1+weight2 > lfor1*100, min(lspec1, rspec2) + min(lspec2, rspec1), FailedForce), 
...
shared stringCompare(StrType L1, boolean init, boolean nne, integer edit, StrType R1) := function
	...
	result := map(not nne and (L1 = '' or R1 = '') => false,
							  ...
							  SALT31.WithinEditN(L1,R1,edit) => true , 
							  false);
...
/*	 
	...
	*/
	 L1R1 := SALT31.Min0(Match1L(L1,lspec1,lnne1,lfor1,linit1,edit1,R1, rspec1)); // take force into consideration
	 L1R2 := SALT31.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor2),linit1,edit1,R2, rspec2));
   L1R3 := SALT31.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor3),linit1,edit1,R3, rspec3));
...
shared Match4L(
   ...
   iL1R1 := getInitMatchWeight(L1, lspec1, R1, rspec1);	 
	 ...
	 */
	 ...
	 L1R2 := SALT31.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor2),linit1,edit1,R2, rspec2));
   L1R3 := SALT31.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor3),linit1,edit1,R3, rspec3));
	 L1R4 := SALT31.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor3),linit1,edit1,R4, rspec4));
...
*/
   ...
   L1R3 := SALT31.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor3),linit1,edit1,R3, rspec3));
	 L1R4 := SALT31.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor3),linit1,edit1,R4, rspec4));
	 L1R5 := SALT31.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor3),linit1,edit1,R5, rspec5));

...
dsLicenseNbr := project(f_sanctn_incidentcode(NOT REGEXFIND('NMLS',CODE_TYPE,NOCASE)),xformCODES(left));	
dsLicenseNbr_dedup := dedup(dsLicenseNbr ,RECORD);
		
export key_license_midex  := index(dsLicenseNbr_dedup
																	, {midex_rpt_nbr}
																	,{dsLicenseNbr_dedup}
																	,Data_Services.Data_Location.Prefix('sanctn')+ Keyname + doxie.Version_SuperKey +'::license_midex');

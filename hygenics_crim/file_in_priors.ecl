...
priors:=  dataset(data_services.foreign_prod+'thor_200::in::crim::hd::aoc_priors',
								layout_in_priors,
								CSV(SEPARATOR('|'), TERMINATOR(['\n', '\r\n']), QUOTE('"'), MAXLENGTH(6000)))  (stringlib.StringToUpperCase(recordid[1..8])<>'RECORDID');// and StateCode in _include_states);
priors_cw:=  dataset(data_services.foreign_prod+'thor_200::in::crim::hd::aoc_priors_cw',
								layout_in_priors,
								CSV(SEPARATOR('|'), TERMINATOR(['\n', '\r\n']), QUOTE('"'), MAXLENGTH(6000)))  (stringlib.StringToUpperCase(recordid[1..8])<>'RECORDID');// and StateCode in _include_states);
...
export file_in_priors := priors+proj_prior_cw;
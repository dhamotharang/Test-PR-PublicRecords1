...
sentence      :=  dataset(data_services.foreign_prod+'thor_200::in::crim::hd::aoc_sentence',
							          	layout_in_sentence,
								          CSV(SEPARATOR('|'), TERMINATOR(['\n', '\r\n']), QUOTE('"'), MAXLENGTH(6000))) (stringlib.StringToUpperCase(recordid[1..8])<>'RECORDID');// and StateCode in _include_states);

sentence_cw   :=  dataset(data_services.foreign_prod+'thor_200::in::crim::hd::aoc_sentence_cw',
								          layout_in_sentence,
								          CSV(SEPARATOR('|'), TERMINATOR(['\n', '\r\n']), QUOTE('"'), MAXLENGTH(6000)))  (stringlib.StringToUpperCase(recordid[1..8])<>'RECORDID');// and StateCode in _include_states);
...
sentence_ie   :=  dataset(data_services.foreign_prod+'thor_200::in::crim::hd::aoc_sentence_ie',
								          layout_in_sentence,
								          CSV(SEPARATOR('|'), TERMINATOR(['\n', '\r\n']), QUOTE('"'), MAXLENGTH(6000)))  (stringlib.StringToUpperCase(recordid[1..8])<>'RECORDID');// and StateCode in _include_states);
...
export file_in_sentence := sentence+proj_sent_cw+proj_sent_ie;
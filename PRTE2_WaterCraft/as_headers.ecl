IMPORT Header, Business_Header, PRTE2_Watercraft, Watercraft, LiensV2, MDR, ut, _Validate, Address;

EXPORT as_headers := MODULE
	//For Person Records
	EXPORT person_header_watercraft := FUNCTION
		RETURN Watercraft.Watercraft_as_Header(PRTE2_Watercraft.Files.Search, PRTE2_Watercraft.Files.Main, PRTE2_Watercraft.Files.CoastGuard,false,false,true);
	END;
																						
	//For Business Records
	SHARED dWatercraftSearch	:=	PRTE2_Watercraft.Files.Search (source_code != 'W1');

	EXPORT business_header_watercraft := FUNCTION
		RETURN Watercraft.fWatercraft_as_Business_Header(dWatercraftSearch);
	END;
	
	//For Business records with contacts
	EXPORT business_contacts_Watercraft := FUNCTION
		RETURN Watercraft.fWatercraft_as_Business_Contact(dWatercraftSearch,true);
	END;

	//New Business Header with Contacts	--kept code here as rollup was not working so there was triple the output								
	rNewBusinessLayout := RECORD
		Business_Header.Layout_Business_Linking.Linking_Interface;
	END;
   
	rNewBusinessLayout tWatercraftAsBusinessLinking(Watercraft.Layout_Watercraft_Search_Base pInput, INTEGER CTR) := TRANSFORM,
    ...
    SELF.contact_did                  := (UNSIGNED)pInput.did;
		// Business Data
    ...
    SELF.company_address_type_raw    := 'CC';
	  ...
	  SELF.company_phone               := CHOOSE(CTR, ut.CleanPhone(pInput.phone_1), ut.CleanPhone(pInput.phone_2));
		...
		SELF.source_docid                :=	pInput.state_origin + '|' + TRIM(pInput.watercraft_key,LEFT,RIGHT) + '|' + trim(pInput.sequence_key);
	...
	dWtrcrft_bus := dWatercraftAsBusLnk(company_bdid <> 0);
	...
	EXPORT new_business_header_watercraft := dWatercraft_rollup;

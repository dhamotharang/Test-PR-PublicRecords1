...
function
	...
	pPeopleHeader_:=join(pPeopleHeader, header.file_ssn_map((unsigned3)ssn5>0)
	...
	Consumer_Layouts.Response.Consumer_Extract_temp tConvert2Response(pPeopleHeader_ l,unsigned8 cnt) :=
	transform
		...
		self.Extract.SSN_issuing_state	:= StringLib.StringToUpperCase(l.SSN_issuing_state);
		self.Extract.SSN_issuing_date	:= if((unsigned4)l.SSN_issuing_date != 0, intformat(ut.Date_MMDDYYYY_i2((string)l.SSN_issuing_date),8,1),'');
...
// ?	SSN
	...
	SSN_match := join(
		...
		,transform(
			 Consumer_Layouts.Response.SSN_Append_temp
...
///////
	...
	VersionControl.macBuildNewLogicalFile(Filenames(pversion).out.SSN_Append.new					,dresponse,BuildResponseFile	,,pCsvout,pOverwrite,pSeparator);
	VersionControl.macBuildNewLogicalFile(Filenames(pversion).out.SSN_Append.new + 'PipeDelimited'	,dresponse,BuildResponseFilePipe,,pCsvout,pOverwrite,pSeparator);

	dresponse_forstats := project(dresponse	,transform({Consumer_Layouts.Response.SSN_append,string stuff}, self := left;self.stuff := 'G'));
    ...
    integer countGroup := count(group);
		...
		SSN_issuing_state_CountNonBlank := sum(group,if(dresponse_forstats.Appended_data.SSN_issuing_state	<>'',1,0));
		SSN_issuing_date_CountNonBlank  := sum(group,if(dresponse_forstats.Appended_data.SSN_issuing_date	<>'',1,0));

...
export fnPublic_Record_Clean := function

inputPubRec := files.input.PubRec;

clean_layout := record
	recordof(files.input.PubRec);
	address.Layout_Clean_Name;
	address.Layout_Clean182;
end;

clean_layout tcleaninput(inputPubRec le) := transform
//// Functions
 	  string73 temp_name := if(le.name <> '', address.CleanPersonFML73(le.name), '');
...
cleaned_inputPubRec := project(inputPubRec, tcleaninput(left)) : persist('persist::fds::pub_rec::cleaned');
...
did_add.MAC_Match_Flex(cleaned_inputPubRec, didmatchset,
...
outfile := fein_outfile : persist('persist::fds::pub_rec::did_bdid_ssn_fein');

...
module
	...
	shared lay_rel_out			:= Layout_consumer.Response.Relatives_Append				;
	shared lay_rel_out_temp	:= Layout_consumer.Temporary.Relatives_Append_Temp	;
...
//			and left.clean_name.mname = right.clean_name.mname
	...
	VersionControl.macBuildNewLogicalFile(Filenames(pversion).out.Relatives_Append.new										,all_recs_out	,BuildResponseFile,,,true);
	VersionControl.macBuildNewLogicalFile(Filenames(pversion).out.Relatives_Append.new + 'PipeDelimited'	,all_recs_out	,BuildResponseFilePipe,,true,true,'|');
...
//	dresponse_forstats := project(dataset(Filenames(pversion).out.Relatives_Append.new,lay_rel_out,flat)	,transform({lay_rel_out,string stuff}, self := left;self.stuff := 'G'));
    ...
    integer countGroup := count(group);
		...
		Alias_Name_CountNonBlank   											:= sum(group,if(dresponse_forstats.Appended_data.Alias_Name															<>'',1,0));
		...
		deceased_indicator_CountNonBlank   							:= sum(group,if(dresponse_forstats.Appended_data.deceased_indicator											=true,1,0));
		count_of_all_relatives_available_CountNonBlank  := sum(group,if(dresponse_forstats.Appended_data.count_of_all_relatives_available				<>'',1,0));

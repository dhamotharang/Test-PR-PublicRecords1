...
function
	...
	Consumer_Layouts.Response.AddrHist_Extract_temp tConvert2Response(pPeopleHeader l,unsigned8 cnt) :=
	transform
		...
		self.Extract.dob				:= if(l.dob != 0, intformat(ut.Date_MMDDYYYY_i2((string)l.dob),8,1),'');
		self.Extract.added_date			:= if(l.dt_first_seen != 0, intformat(ut.Date_MMDDYYYY_i2((string)l.dt_first_seen+'01'),8,1),'');
...
// ?	SSN
	PeopleHeader:=project(pPeopleHeader	,tConvert2Response(left,counter));
	...
	SSN_match := join(
		...
		,transform(
			 Consumer_Layouts.Response.AddrHist_Append_temp
...
///////
					...
					,local)
						...
						and	left.appended_data.state			=right.appended_data.state
								,transform(recordof(dresponse_match)
									...
									,self.appended_data.dob					:=if(left.appended_data.dob='',right.appended_data.dob,left.appended_data.dob)
									,self:=left
								)
			,local);

	dresponse := table(dresponse1,{
								did
								,SearchCriteria
								,Appended_data
								,count_of_total_previous_addresses_available:=count(group)
							}
							,SearchCriteria.Record_ID
							,appended_data.first_name
							,appended_data.last_name
	...
	VersionControl.macBuildNewLogicalFile(Filenames(pversion).out.AddrHist_Append.new					,dresponse,BuildResponseFile	,,pCsvout,pOverwrite,pSeparator);
	VersionControl.macBuildNewLogicalFile(Filenames(pversion).out.AddrHist_Append.new + 'PipeDelimited'	,dresponse,BuildResponseFilePipe,,pCsvout,pOverwrite,pSeparator);

	dresponse_forstats := project(dresponse	,transform({Consumer_Layouts.Response.AddrHist_Append
														,string stuff}
															,self.Appended_data.count_of_total_previous_addresses_available := (string)left.count_of_total_previous_addresses_available
															,self.stuff := 'G'
															,self := left
															));
    ...
    integer countGroup := count(group);
		...
		date_of_birth_CountNonBlank   	:= sum(group,if(dresponse_forstats.Appended_data.dob	<>'',1,0));
		added_date_CountNonBlank 	  	:= sum(group,if(dresponse_forstats.Appended_data.added_date	<>'',1,0));
	...
	tSSN_match:=count(table(SSN_match,{searchcriteria.record_id},searchcriteria.record_id,few));
	tSSN_name_match:=count(table(SSN_name_match,{searchcriteria.record_id},searchcriteria.record_id,few));
	tSSN_nmaddr_match:=count(table(SSN_nmaddr_match,{searchcriteria.record_id},searchcriteria.record_id,few));
	tName_addr_match:=count(table(name_addr_match,{searchcriteria.record_id},searchcriteria.record_id,few));
	tNcsz_match:=count(table(ncsz_match,{searchcriteria.record_id},searchcriteria.record_id,few));
	tNs_match:=count(table(ns_match,{searchcriteria.record_id},searchcriteria.record_id,few));
	tAddr_match:=count(table(addr_match,{searchcriteria.record_id},searchcriteria.record_id,few));
	tPh_match:=count(table(ph_match,{searchcriteria.record_id},searchcriteria.record_id,few));

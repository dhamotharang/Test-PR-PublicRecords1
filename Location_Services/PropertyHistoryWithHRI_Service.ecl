/*--SOAP--
<message name="PropertyHistoryWithHRI_Service">
...
MACRO
  ...
  iesp.ECL2ESP.SetInputReportBy(PROJECT(xmlSearchBy,TRANSFORM(iesp.bpsreport.t_BpsReportBy,SELF := LEFT,SELF := [])));
	...
	reportParams := MODULE(Location_Services.iParam.PropHistHRI)
		...
		EXPORT BOOLEAN   IgnoreFares         := globalMod.ignoreFares : STORED('ignoreFares');
		EXPORT BOOLEAN   IgnoreFidelity      := globalMod.ignoreFidelity : STORED('ignoreFidelity');

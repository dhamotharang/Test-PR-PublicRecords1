...
import codes, address, ln_propertyv2, Location_Services, doxie, ut, risk_indicators, doxie_cbrs, LN_PropertyV2_Services, Suppress,
       STD;

todays_date := (string) STD.Date.Today();

export Property_History_Function(DATASET(Location_Services.Layout_PropHistory_In) inData,
                                 doxie.IDataAccess mod_access,
                                 BOOLEAN doAVM, BOOLEAN includeAssignmentsAndReleases) :=
FUNCTION
  ...
  Location_Services.Layout_PropHistory_Out rfmt(Location_Services.Layout_PropHistory_In l) := TRANSFORM
    SELF.reqData := PROJECT(l, TRANSFORM(Location_Services.Layout_PropHistory_In, SELF:=LEFT));
    SELF := [];
  END;
  ...
  WorkLayout getFaresID1(Location_Services.Layout_PropHistory_In l, key_addr R) := TRANSFORM
    SELF.seq := l.seq;
  ...
  WorkLayout getFaresID2(Location_Services.Layout_PropHistory_In l, LN_PropertyV2.key_assessor_parcelnum R) := TRANSFORM
    SELF.seq := l.seq;
    SELF.fares_id := R.ln_fares_id;
  END;

  WorkLayout getFaresID3(Location_Services.Layout_PropHistory_In l, LN_PropertyV2.key_deed_parcelnum R) := TRANSFORM
    SELF.seq := l.seq;
    SELF.fares_id := R.ln_fares_id;
  END;
  ...
  addrsrc := record
    Location_Services.layout_address address;
    string2 source_code;
    unsigned4 seq;
  ...
  sourceaddrs := join(fares,key_search,
            ...
            keyed(right.source_code_1 IN (if(includeAssignmentsAndReleases, ['O','B'], ['O'])) ),//Added 'B' for Assignments and Releases
  ...
  empty_whriAddr_pre := join(withPropValue, sources_whri,
  ...
  empty_whriAddr := dedup(sort(empty_whriAddr_pre,record),record);
  ...
  layout_deed_mortgage_summary get_deed_summary(fares L, key_deed R) := transform
    ...
    self.buyer1 := R.name1; //need this as sale.buyer1 in cvtLPTD transform below is not getting updated as the field name if different from the key
  ...
  dmrec := record
    dmData0;
    dataset(Location_Services.Layout_PersonData) buyers  {maxcount(consts.max_buyers)};
    dataset(Location_Services.Layout_PersonData) sellers {maxcount(consts.max_sellers)};
    string1 source;
  ...
  Layout_PersonData_wbdid into_person(best_plus L,codes.Key_Codes_V3 r) := transform
    ...
    self.phones := choosen(location_services.get_Phone_Info(L.phone, L.lname, L.prim_range, L.predir, L.prim_name,
      L.sec_range, L.st, L.zip, mod_access), consts.max_phones);
  ...
  location_services.Layout_PersonData person_from_dmData(key_search L):=Transform
  ...
  w_vendor_source_flag :=record
    location_services.Layout_PropertyTransaction;
  END;
  ...
  w_vendor_source_flag cvtLPTD(DmData l, string addr1, dataset(risk_indicators.layout_desc) hri_addr) := TRANSFORM
    ...
    SELF.deed.first_td_due_date := L.first_td_due_date;
  ...
  location_services.Layout_PersonData into_person2(dmrec L, codes.Key_Codes_V3 r) := transform
    self.did := L.did;
    self.current_address := dataset([{L.prim_range, L.predir, L.prim_name, L.suffix, L.postdir, L.unit_desig, L.sec_range, L.city_name, L.st, L.zip, L.zip4, L.addr_dt_last_seen, choosen(L.hri_address,consts.max_hri_addr)}], layout_person_address);
    self.phones := choosen(location_services.get_Phone_Info(L.phone, L.lname, L.prim_range, L.predir, L.prim_name,
      L.sec_range, L.st, L.zip, mod_access), consts.max_phones);
  ...
  // restrict minors
  bestRecords := bestRecords_all(doxie.compliance.minor_ok(age, mod_access.show_minors));

  string os(string s) := if (s ='', '', trim(s) + ' ');

  location_services.layout_resident into_resident(bestRecords L) := transform
    self.did := L.did;
    ...
    self.phone_Info := choosen(location_services.get_Phone_Info(L.phone, L.lname, L.prim_range, L.predir, L.prim_name,
      L.sec_range, L.st, L.zip, mod_access), consts.max_hri_phone);
  ...
  location_services.Layout_PropHistory_Out add_trans_new(location_services.Layout_PropHistory_Out L,last_assess_mapped R) := Transform

...
EXPORT PropertyHistoryWithHRI_Records(DATASET(Autokey_batch.Layouts.rec_inBatchMaster)       dIn,
...
FUNCTION
  ...
  keyAssessmentFID := LN_PropertyV2.key_assessor_fid();
	...
	// Check if it resolved to multiple properties
	IF(COUNT(dPropAddress) > 1,FAIL(203,doxie.ErrorCodes(203)));
	...
	TRANSFORM		
		...
		TRANSFORM
			...
			SELF.Fireplace              := ut.CleanSpacesAndUpper(pInput.fireplace_indicator) = 'Y';
			SELF.Pool                   := ut.CleanSpacesAndUpper(pInput.pool_indicator) = 'Y';
		...
		TRANSFORM
			...
			SELF.Sale.MonthsSinceSaleDate      := IF((INTEGER)pInput.contract_date > todays_date or pInput.contract_date = '',
																								-1,
																								ut.DaysApart((string) todays_date,pInput.contract_date) DIV 30);

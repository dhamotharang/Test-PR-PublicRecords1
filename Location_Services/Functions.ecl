  ...
  EXPORT GetSecRanges (DATASET(Autokey_batch.Layouts.rec_inBatchMaster) dReqCleanAddr) :=
	...
	FUNCTION
      ...
      addr := IF( pInput.person_addr.prim_name != '' or pInput.person_addr.st != '',
		...
		Suppress.MAC_Suppress(dFaresId,dFaresIdSuppress,inMod.ApplicationType,,,Suppress.Constants.DocTypes.FaresID,ln_fares_id);
	...
	FUNCTION
		...
		// Apply suppression
		Suppress.MAC_Suppress(dBuyers,dBuyerDIDSuppress,inMod.ApplicationType,Suppress.Constants.LinkTypes.DID,did);
		Suppress.MAC_Suppress(dBuyerDIDSuppress,dBuyerSuppress,inMod.ApplicationType,Suppress.Constants.LinkTypes.SSN,ssn);
		
		Suppress.MAC_Suppress(dSellers,dSellerDIDSuppress,inMod.ApplicationType,Suppress.Constants.LinkTypes.DID,did);
		Suppress.MAC_Suppress(dSellerDIDSuppress,dSellerSuppress,inMod.ApplicationType,Suppress.Constants.LinkTypes.SSN,ssn);
		...
		dBuyerDIDs  := PROJECT(dBuyerSuppress,Doxie_Raw.Layouts.RelDetailsIn);
		dSellerDIDs := PROJECT(dSellerSuppress,Doxie_Raw.Layouts.RelDetailsIn);
		
		// PROJECT(inMod,Doxie_Raw.iParam.RelDetails,OPT) - had to do a PROJECT due to a roxie platform issue
		relDetailsMod := MODULE(PROJECT(inMod,Doxie_Raw.iParam.RelDetails,OPT))
			EXPORT UNSIGNED1 GLBPurpose        := inMod.GLBPurpose;
			EXPORT UNSIGNED1 DPPAPurpose       := inMod.DPPAPurpose;
			EXPORT BOOLEAN   LnBranded         := inMod.LnBranded;
			EXPORT STRING32  ApplicationType   := inMod.ApplicationType;
			EXPORT STRING5   IndustryClass     := inMod.IndustryClass;
			...
			EXPORT UNSIGNED3 MaxAssociates     := Location_Services.consts.max_assocs;
			EXPORT BOOLEAN   IncludeNeighbors  := FALSE;
		END;
		
		dDIDAssociations := IF(EXISTS(dSellerDIDs(did != 0)),
														Doxie_Raw.Get_DID_Associations(dBuyerDIDs,relDetailsMod,TRUE,dSellerDIDs));
				
		// Tie back the relationship to the transactions
		lPropHistCollusion.Deeds tDeedsWithBSRelationship(lPropHistCollusion.Deeds le,Doxie_Raw.Layouts.RelDetailsOut ri) :=
		TRANSFORM
	...
	FUNCTION
		...
		dAddrResidents := doxie_raw.residents_raw(dAddress,, //dateVal
																							inMod.DPPAPurpose,inMod.GLBPurpose,
																							inMod.SSNMask,
																							inMod.LNBranded);

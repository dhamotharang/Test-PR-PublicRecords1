IMPORT BIPV2, Doxie_Raw, iesp, LN_propertyv2, TopBusiness_Services, Doxie;

EXPORT GetByBusinessIds(DATASET(Doxie_Raw.Layout_address_input) addr_in,
	...
	EXPORT GetLinkIds() := FUNCTION
		...
		BIPV2.IDlayouts.l_xlink_ids2 xForm_LinkIds(linkIdsInfoOut le) := TRANSFORM
			SELF := le.BusinessIds;
		END;
	...
	EXPORT GetPropFids() := FUNCTION
		...
		property_recs_raw := LN_propertyv2.key_Linkids.kfetch2(getLinkIds(),
	...
	EXPORT GetSourceRecs() := FUNCTION
		...
		source_recs := CHOOSEN(BIPV2.Key_BH_Linking_Ids.kFetch2(GetLinkIds(),
																TopBusiness_Services.Constants.sourceLinkIdLevel,,,,,,,,mod_access),

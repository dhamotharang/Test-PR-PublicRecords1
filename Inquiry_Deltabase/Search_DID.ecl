...
EXPORT Inquiry_Deltabase.Layouts.Inquiry_DID Search_DID (DATASET(Inquiry_Deltabase.Layouts.Input_Deltabase_DID) SearchInput,
	...
	Inquiry_Deltabase.Layouts.Deltabase_Input generateSelects(Inquiry_Deltabase.Layouts.Input_Deltabase_DID le) := TRANSFORM
		...
		Valid_Query := DID > 0; // Make sure we are only calling the Deltabase for plausible searches
		...
		GeneratedSelect := 'SELECT \'' + (STRING)le.Seq + '\' AS Seq, ' + // Force Seq into the response for each SELECT statement, this allows for us to use this in batch
									...
									'WHERE i.Response_LexID = \'' + (STRING)DID + '\' ' +
	...
	Inquiry_Deltabase.Layouts.Inquiry_DID intoKeyLayout(Inquiry_Deltabase.Layouts.Deltabase_Record le) := TRANSFORM
		// Index Fields
		...
		SELF := [];
	END;
	DeltabaseResults := PROJECT(DeltabaseResponse.Response, intoKeyLayout(LEFT)) (S_DID <> 0); // Only return valid DIDs (In case Deltabase Errors)

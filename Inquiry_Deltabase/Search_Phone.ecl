...
EXPORT Inquiry_Deltabase.Layouts.Inquiry_Phone Search_Phone (DATASET(Inquiry_Deltabase.Layouts.Input_Deltabase_Phone) SearchInput,
	...
	Inquiry_Deltabase.Layouts.Deltabase_Input generateSelects(Inquiry_Deltabase.Layouts.Input_Deltabase_Phone le) := TRANSFORM
		...
		Valid_Query := Phone10 <> '' AND LENGTH(Phone10) = 10; // Make sure we are only calling the Deltabase for plausible searches
	...
	Inquiry_Deltabase.Layouts.Inquiry_Phone intoKeyLayout(Inquiry_Deltabase.Layouts.Deltabase_Record le) := TRANSFORM
		// Index Fields
		...
		SELF := [];
	END;
	DeltabaseResults := PROJECT(DeltabaseResponse.Response, intoKeyLayout(LEFT)) (Phone10 <> ''); // Only return valid Phones (In case Deltabase Errors)

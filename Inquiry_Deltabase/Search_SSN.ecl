...
EXPORT Inquiry_Deltabase.Layouts.Inquiry_SSN Search_SSN (DATASET(Inquiry_Deltabase.Layouts.Input_Deltabase_SSN) SearchInput,
	...
	Inquiry_Deltabase.Layouts.Deltabase_Input generateSelects(Inquiry_Deltabase.Layouts.Input_Deltabase_SSN le) := TRANSFORM
		...
		Valid_Query := SSN <> '' AND LENGTH(SSN) = 9; // Make sure we are only calling the Deltabase for plausible searches
	...
	Inquiry_Deltabase.Layouts.Inquiry_SSN intoKeyLayout(Inquiry_Deltabase.Layouts.Deltabase_Record le) := TRANSFORM
		// Index Fields
		SELF.SSN := le.SSN;
		// Inquiry Key Payload
		...
		SELF := [];
	END;
	DeltabaseResults := PROJECT(DeltabaseResponse.Response, intoKeyLayout(LEFT)) (SSN <> ''); // Only return valid SSNs (In case Deltabase Errors)

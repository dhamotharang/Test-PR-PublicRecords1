...
EXPORT Inquiry_Deltabase.Layouts.Inquiry_IPAddr Search_IPAddr (DATASET(Inquiry_Deltabase.Layouts.Input_Deltabase_IPAddr) SearchInput,
	...
	Inquiry_Deltabase.Layouts.Deltabase_Input generateSelects(Inquiry_Deltabase.Layouts.Input_Deltabase_IPAddr le) := TRANSFORM
		...
		Valid_Query := IPAddr <> ''; // Make sure we are only calling the Deltabase for plausible searches
	...
	Inquiry_Deltabase.Layouts.Inquiry_IPAddr intoKeyLayout(Inquiry_Deltabase.Layouts.Deltabase_Record le) := TRANSFORM
		// Index Fields
		...
		SELF := [];
	END;
	DeltabaseResults := PROJECT(DeltabaseResponse.Response, intoKeyLayout(LEFT)) (IPAddr <> ''); // Only return valid IPAddrs (In case Deltabase Errors)

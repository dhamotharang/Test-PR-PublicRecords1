...
EXPORT Inquiry_Deltabase.Layouts.Inquiry_Name Search_Name (DATASET(Inquiry_Deltabase.Layouts.Input_Deltabase_Name) SearchInput,
	...
	Inquiry_Deltabase.Layouts.Deltabase_Input generateSelects(Inquiry_Deltabase.Layouts.Input_Deltabase_Name le) := TRANSFORM
		...
		Valid_Query := FirstName <> '' AND (LastName <> '' OR MiddleInitial <> ''); // Make sure we are only calling the Deltabase for plausible searches
		...
		GeneratedSelect := 'SELECT \'' + (STRING)le.Seq + '\' AS Seq, ' + // Force Seq into the response for each SELECT statement, this allows for us to use this in batch
									...
									// Done Generating the Response Layout, now pick out the right Name - First Name and if Last is input must match, otherwise First Name and Middle Initial must match
									'WHERE i.First_Name = \'' + FirstName + '\' AND ' + IF(LastName <> '', 'i.Last_Name = \'' + LastName + '\' ', 'i.Middle_Name LIKE \'' + MiddleInitial + '%\' ') +
	...
	Inquiry_Deltabase.Layouts.Inquiry_Name intoKeyLayout(Inquiry_Deltabase.Layouts.Deltabase_Record le) := TRANSFORM
		// Index Fields
		SELF.FName := le.FName;
		SELF.MName := le.MName;
		SELF.LName := le.LName;
		...
		SELF := [];
	END;
	DeltabaseResults := PROJECT(DeltabaseResponse.Response, intoKeyLayout(LEFT)) (FName <> '' AND (LName <> '' OR MName <> '')); // Only return valid Names (In case Deltabase Errors)

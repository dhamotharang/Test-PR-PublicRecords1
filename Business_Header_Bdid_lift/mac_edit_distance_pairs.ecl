export MAC_Edit_Distance_Pairs(infile,infield,countfield,thresh,phonetics,outfile) := MACRO
  ...
  #uniquename(p_cnt_rt)
	...
	#uniquename(j1)
	%j1% := join(%inf7%,%inf7%,left.infield[1..2]=right.infield[1..2] and  left.infield[3..*]=right.infield[3..*] and ut.WithinEditN(left.infield,right.infield,thresh),%trans%(left,right),atmost(left.infield[1..2]=right.infield[1..2] and  left.infield[3..*]=right.infield[3..*],1000));
	#uniquename(j2)
	%j2% := join(%inf%,%inf%,left.infield[1..3]=right.infield[1..3] and  left.infield[4..*]=right.infield[4..*] and ut.WithinEditN(left.infield,right.infield,thresh),%trans%(left,right),atmost(left.infield[1..3]=right.infield[1..3] and  left.infield[4..*]=right.infield[4..*],1000));
	#uniquename(j3)
	%j3% := join(%inf7%,%inf7%,left.infield[3..*]=right.infield[3..*] and left.infield[length(trim(left.infield))-2..] = right.infield[length(trim(right.infield))-2..] and ut.WithinEditN(left.infield,right.infield,thresh),%trans%(left,right),atmost(left.infield[3..*]=right.infield[3..*] and left.infield[length(trim(left.infield))-2..] = right.infield[length(trim(right.infield))-2..],1000));
	#uniquename(j4)
	%j4% := join(%inf%,%inf%,left.infield[4..*]=right.infield[4..*] and left.infield[length(trim(left.infield))-3..] = right.infield[length(trim(right.infield))-3..] and ut.WithinEditN(left.infield,right.infield,thresh),%trans%(left,right),atmost(left.infield[4..*]=right.infield[4..*] and left.infield[length(trim(left.infield))-3..] = right.infield[length(trim(right.infield))-3..],1000));
	#uniquename(aj)
	%aj% := dedup( sort( %j1%+%j2%+%j3%+%j4%, whole record, skew(1) ), whole record );

import risk_indicators, ut, ashirey,scoring_project_pip, zz_asinha;

EXPORT string lib_fileservices__fslogicalfilename := string{maxlength(255)};
...
// eyeball := 25;
// basefilename := ut.foreign_prod_boca + 'scoringqa::out::nonfcra::bocashell_41_historydate_999999_prod_20171004_1';                
// testfilename := ut.foreign_prod_boca + 'scoringqa::out::nonfcra::bocashell_41_historydate_999999_prod_20171005_1';    
basefilename := '~scoringqa::out::nonfcra::bocashell_41_historydate_999999_cert_20170928_1';                
testfilename :=  '~scoringqa::out::nonfcra::bocashell_41_historydate_999999_cert_20170929_1';             

// basefilename := '~scoringqa::out::bs_41_tracking_edina_nonfcra_no_edina_crim_exp_phase3_fcra_baseline_20170928';                
// testfilename := '~scoringqa::out::bs_41_tracking_edina_nonfcra_no_edina_crim_exp_phase3_fcra_second_2017';    
...
Layout trans_noerror(Layout le) := Transform
...
ds_baseline_new := join(ds_baseline_noerrors, ds_new_noerrors, left.accountnumber = right.accountnumber, trans_noerror(Left) );
ds_test_new := join(ds_baseline_noerrors, ds_new_noerrors, left.accountnumber = right.accountnumber, trans_noerror(Right) );
...
ds_new_proj := project(ds_new,transform(Layout,self:=left;
                                                    self:=[];
                                           ));


// j1 := join(ds_baseline, ds_new, left.AccountNumber = right.AccountNumber
				// AND left.bjl.last_criminal_date - right.bjl.last_criminal_date > 10000,
			// transform(cmpr, self.res := left + right));
			
			
j2 := join(ds_baseline, ds_new, left.AccountNumber = right.AccountNumber
				AND left.employment.source_ct <> right.employment.source_ct,
			transform(cmpr, self.res := left + right));
...
// j4 := join(ds_baseline, ds_new, left.AccountNumber = right.AccountNumber
				// AND left.total_number_derogs <> right.total_number_derogs,
			// transform(cmpr, self.res := left + right));
			
// j5 := join(ds_baseline, ds_new, left.AccountNumber = right.AccountNumber
				// AND left.bjl.felony_count <> right.bjl.felony_count,
			// transform(cmpr, self.res := left + right));	
...
// joinedrec dojoin(cmpr le, cmpr ri) := transform
	// self.diff := (integer)ri.res.bjl.last_criminal_date - (integer)le.res.bjl.last_criminal_date;
	// self := le;
// End;

// j6 := join(ds_baseline, ds_new,left.AccountNumber = right.AccountNumber, dojoin(left,right));

// t_1 := table(j6, {diff; total := count(group)}, diff);

// t_2 := sort(t_1, diff);

// Output(choosen(t_2, 500), named('Table'));
			
// OUTPUT(COUNT(j1), named('last_criminal_date_count'));			
// OUTPUT(CHOOSEN(j1, 10), NAMED('last_criminal_date'))	;

OUTPUT(COUNT(j2), named('Change_employment__source_ct_count'));			
OUTPUT(CHOOSEN(j2, 10), NAMED('employment__source_ct'))	;	

// OUTPUT(COUNT(j3), named('Change_criminal_count_count'));			
// OUTPUT(CHOOSEN(j3, 10), NAMED('criminal_count'))	;	

// OUTPUT(COUNT(j4), named('total_number_derogs_count'));			
// OUTPUT(CHOOSEN(j4, 10), NAMED('total_number_derogs'))	;	

// OUTPUT(COUNT(j5), named('Change_felony_count_count'));			
// OUTPUT(CHOOSEN(j5, 10), NAMED('felony_count'))	;	
...
// scoring_project_pip.CROSSTAB_MACRO( flatten_baseline, flatten_second, ['accountnumber'], 'employment__source_ct');
...
EXPORT MakeFT_invalid_corp_vendor(SALT30.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_vendor(SALT30.StrType s) := WHICH(((SALT30.StrType) s) NOT IN ['08']);
EXPORT InValidMessageFT_invalid_corp_vendor(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInEnum('08'),SALT30.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_state_origin(SALT30.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_state_origin(SALT30.StrType s) := WHICH(((SALT30.StrType) s) NOT IN ['CO']);
EXPORT InValidMessageFT_invalid_state_origin(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInEnum('CO'),SALT30.HygieneErrors.Good);
...
EXPORT Make_ar_report_dt(SALT30.StrType s0) := s0;
EXPORT InValid_ar_report_dt(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_report_dt(UNSIGNED1 wh) := '';
 
EXPORT Make_ar_report_nbr(SALT30.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_ar_report_nbr(SALT30.StrType s) := InValidFT_invalid_numeric(s);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT30,Scrubs_Corp2_Mapping_CO_AR;

...
build_version := if(status <> 3, ver, today); // 3 -> Completed

incremental := if(isMonthly, 'false', 'true');
ingestType := if(isMonthly, 'monthly', 'incremental');

norun := build_version = ver and status = 3;
...
+'#WORKUNIT(\'name\',\'' + build_version + ' Header Ingest ' + ingestType + if(status <> 3, ' RECOVER ', '') + '\');\n\n'

...
pversion := '20160328';
pUseProd := false;
#workunit('name', 'Organization BASE Build ' + pversion);
	 Org_lInputTemplate    :=Org_Mast._Dataset(pUseProd).thor_cluster_files + 'in::' 	 +Org_Mast. _Dataset().name + '::Org_Master_Organization';//::'/*+pversion*/;
  Org_lBuiltTemplate    :=Org_Mast._Dataset(pUseProd).thor_cluster_files + 'base::' 	 +Org_Mast. _Dataset().name + '::Org_Master_Organization::'/*+pversion*/+'Built';
  // pos_lQATemplate    :=Org_Mast._Dataset(pUseProd).thor_cluster_files + 'base::' 	 +Org_Mast. _Dataset().name + '::Org_Master_pos::'/*+pversion*/+'QA';
	
	  fileservices.RemoveOwnedSubFiles(Org_lInputTemplate);
	  fileservices.RemoveOwnedSubFiles(Org_lBuiltTemplate);
	  // fileservices.RemoveOwnedSubFiles(pos_lQATemplate);
   	 ...
   	 // spray_;
   	  ...
   	  // in_Org_file						:= Org_Mast._Dataset(pUseProd).thor_cluster_files + 'in::' 	 + Org_Mast._Dataset().name + '::Org_Master_Organization::'+pversion;//@version@';
   	
   	  // Org									  := DATASET(in_Org_file, Org_Mast.layouts.Organization_in, csv( separator('\t'), heading(1), terminator(['\n', '\r\n']), QUOTE('\"') ));
		  // Org;

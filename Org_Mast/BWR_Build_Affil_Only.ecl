...
pversion := '20160328';
pUseProd := false;
#workunit('name', 'AFFIL BASE Build ' + pversion);
	pos_lInputTemplate    :=Org_Mast._Dataset(pUseProd).thor_cluster_files + 'in::' 	 +Org_Mast. _Dataset().name + '::Org_Master_affiliations';//::'/*+pversion*/;
  pos_lBuiltTemplate    :=Org_Mast._Dataset(pUseProd).thor_cluster_files + 'base::' 	 +Org_Mast. _Dataset().name + '::Org_Master_affiliations::'/*+pversion*/+'Built';
  // pos_lQATemplate    :=Org_Mast._Dataset(pUseProd).thor_cluster_files + 'base::' 	 +Org_Mast. _Dataset().name + '::Org_Master_pos::'/*+pversion*/+'QA';
	  ...
	  // fileservices.RemoveOwnedSubFiles(pos_lQATemplate);
   	...
   	// std_input;
		// sequential(
		 // Org_Mast.Build_Base.build_base_affiliations(pversion,pUseProd).affiliations_all;

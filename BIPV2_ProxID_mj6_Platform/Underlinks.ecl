...
EXPORT RRF := RawResults(Conf>=LowerMatchThreshold);
  R := RECORD
    ...
    company_addr1_skipped := COUNT(GROUP,RRF.company_addr1_skipped);
    company_addr1_skipped_pcnt := AVE(GROUP,IF(RRF.company_addr1_skipped,100,0));
...
EXPORT ForceFails := TABLE(RRF,R);
  ...
  R tosummary(ds le) := TRANSFORM
    SELF := le;
    SELF.Summary := IF(le.cnp_name_Score = 0,'','|'+IF(le.cnp_name_Score < 0,'-','')+'cnp_name')+IF(le.cnp_number_Score = 0,'','|'+IF(le.cnp_number_Score < 0,'-','')+'cnp_number')+IF(le.prim_range_Score = 0,'','|'+IF(le.prim_range_Score < 0,'-','')+'prim_range')+IF(le.prim_name_derived_Score = 0,'','|'+IF(le.prim_name_derived_Score < 0,'-','')+'prim_name_derived')+IF(le.st_Score = 0,'','|'+IF(le.st_Score < 0,'-','')+'st')+IF(le.zip_Score = 0,'','|'+IF(le.zip_Score < 0,'-','')+'zip')+IF(le.ebr_file_number_Score = 0,'','|'+IF(le.ebr_file_number_Score < 0,'-','')+'ebr_file_number')+IF(le.hist_duns_number_Score = 0,'','|'+IF(le.hist_duns_number_Score < 0,'-','')+'hist_duns_number')+IF(le.hist_domestic_corp_key_Score = 0,'','|'+IF(le.hist_domestic_corp_key_Score < 0,'-','')+'hist_domestic_corp_key')+IF(le.foreign_corp_key_Score = 0,'','|'+IF(le.foreign_corp_key_Score < 0,'-','')+'foreign_corp_key')+IF(le.unk_corp_key_Score = 0,'','|'+IF(le.unk_corp_key_Score < 0,'-','')+'unk_corp_key')+IF(le.company_phone_Score = 0,'','|'+IF(le.company_phone_Score < 0,'-','')+'company_phone')+IF(le.active_duns_number_Score = 0,'','|'+IF(le.active_duns_number_Score < 0,'-','')+'active_duns_number')+IF(le.active_domestic_corp_key_Score = 0,'','|'+IF(le.active_domestic_corp_key_Score < 0,'-','')+'active_domestic_corp_key')+IF(le.company_fein_Score = 0,'','|'+IF(le.company_fein_Score < 0,'-','')+'company_fein')+IF(le.active_enterprise_number_Score = 0,'','|'+IF(le.active_enterprise_number_Score < 0,'-','')+'active_enterprise_number')+IF(le.sec_range_Score = 0,'','|'+IF(le.sec_range_Score < 0,'-','')+'sec_range')+IF(le.v_city_name_Score = 0,'','|'+IF(le.v_city_name_Score < 0,'-','')+'v_city_name')+IF(le.hist_enterprise_number_Score = 0,'','|'+IF(le.hist_enterprise_number_Score < 0,'-','')+'hist_enterprise_number')+IF(le.company_csz_Score = 0,'','|'+IF(le.company_csz_Score < 0,'-','')+'company_csz')+IF(le.company_addr1_Score = 0,'','|'+IF(le.company_addr1_Score < 0,'-','')+'company_addr1')+IF(le.company_address_Score = 0,'','|'+IF(le.company_address_Score < 0,'-','')+'company_address');
  END;

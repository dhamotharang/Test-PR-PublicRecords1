...
EXPORT Summary(SALT30.Str30Type txt,BOOLEAN Glob=TRUE) := FUNCTION
  ...
  R1 := RECORD
    UNSIGNED LinkingPotential :=  + T.Populated_cnp_name_pcnt *  25.00 / 100 + T.Populated_cnp_number_pcnt *  14.00 / 100 + T.Populated_active_duns_number_pcnt *  27.00 / 100 + T.Populated_active_enterprise_number_pcnt *  26.00 / 100 + T.Populated_active_domestic_corp_key_pcnt *  27.00 / 100 + T.Populated_hist_enterprise_number_pcnt *  28.00 / 100 + T.Populated_hist_duns_number_pcnt *  27.00 / 100 + T.Populated_hist_domestic_corp_key_pcnt *  27.00 / 100 + T.Populated_foreign_corp_key_pcnt *  27.00 / 100 + T.Populated_unk_corp_key_pcnt *  27.00 / 100 + T.Populated_ebr_file_number_pcnt *  28.00 / 100 + T.Populated_company_fein_pcnt *  26.00 / 100 + T.Populated_company_phone_pcnt *  27.00 / 100 + T.Populated_prim_range_pcnt *  13.00 / 100 + T.Populated_prim_name_derived_pcnt *  15.00 / 100 + T.Populated_sec_range_pcnt *  12.00 / 100 + T.Populated_v_city_name_pcnt *  11.00 / 100 + T.Populated_st_pcnt *   5.00 / 100 + T.Populated_zip_pcnt *  14.00 / 100 + T.Populated_dt_first_seen_pcnt *   0.00 / 100 + T.Populated_dt_last_seen_pcnt *   0.00 / 100;
    T;
  END;
  RETURN TABLE(T,R1);
END;
...
EXPORT SourceCounts := Summary('SummaryBySource',FALSE);  R := RECORD
  ...
  R T(SourceCounts le, SourceCounts ri) := TRANSFORM
    ...
    SELF.LinkingPotential := 0 + le.Populated_cnp_name_pcnt*ri.Populated_cnp_name_pcnt *  25.00 / 10000 + le.Populated_cnp_number_pcnt*ri.Populated_cnp_number_pcnt *  14.00 / 10000 + le.Populated_active_duns_number_pcnt*ri.Populated_active_duns_number_pcnt *  27.00 / 10000 + le.Populated_active_enterprise_number_pcnt*ri.Populated_active_enterprise_number_pcnt *  26.00 / 10000 + le.Populated_active_domestic_corp_key_pcnt*ri.Populated_active_domestic_corp_key_pcnt *  27.00 / 10000 + le.Populated_hist_enterprise_number_pcnt*ri.Populated_hist_enterprise_number_pcnt *  28.00 / 10000 + le.Populated_hist_duns_number_pcnt*ri.Populated_hist_duns_number_pcnt *  27.00 / 10000 + le.Populated_hist_domestic_corp_key_pcnt*ri.Populated_hist_domestic_corp_key_pcnt *  27.00 / 10000 + le.Populated_foreign_corp_key_pcnt*ri.Populated_foreign_corp_key_pcnt *  27.00 / 10000 + le.Populated_unk_corp_key_pcnt*ri.Populated_unk_corp_key_pcnt *  27.00 / 10000 + le.Populated_ebr_file_number_pcnt*ri.Populated_ebr_file_number_pcnt *  28.00 / 10000 + le.Populated_company_fein_pcnt*ri.Populated_company_fein_pcnt *  26.00 / 10000 + le.Populated_company_phone_pcnt*ri.Populated_company_phone_pcnt *  27.00 / 10000 + le.Populated_prim_range_pcnt*ri.Populated_prim_range_pcnt *  13.00 / 10000 + le.Populated_prim_name_derived_pcnt*ri.Populated_prim_name_derived_pcnt *  15.00 / 10000 + le.Populated_sec_range_pcnt*ri.Populated_sec_range_pcnt *  12.00 / 10000 + le.Populated_v_city_name_pcnt*ri.Populated_v_city_name_pcnt *  11.00 / 10000 + le.Populated_st_pcnt*ri.Populated_st_pcnt *   5.00 / 10000 + le.Populated_zip_pcnt*ri.Populated_zip_pcnt *  14.00 / 10000 + le.Populated_dt_first_seen_pcnt*ri.Populated_dt_first_seen_pcnt *   0.00 / 10000 + le.Populated_dt_last_seen_pcnt*ri.Populated_dt_last_seen_pcnt *   0.00 / 10000;
  END;
...
invRec invert(summary0 le, INTEGER C) := TRANSFORM
  ...
  SELF.FieldName := CHOOSE(C,'cnp_name','cnp_number','active_duns_number','active_enterprise_number','active_domestic_corp_key','hist_enterprise_number','hist_duns_number','hist_domestic_corp_key','foreign_corp_key','unk_corp_key','ebr_file_number','company_fein','company_phone','prim_range','prim_name_derived','sec_range','v_city_name','st','zip','dt_first_seen','dt_last_seen');
  SELF.populated_pcnt := CHOOSE(C,le.populated_cnp_name_pcnt,le.populated_cnp_number_pcnt,le.populated_active_duns_number_pcnt,le.populated_active_enterprise_number_pcnt,le.populated_active_domestic_corp_key_pcnt,le.populated_hist_enterprise_number_pcnt,le.populated_hist_duns_number_pcnt,le.populated_hist_domestic_corp_key_pcnt,le.populated_foreign_corp_key_pcnt,le.populated_unk_corp_key_pcnt,le.populated_ebr_file_number_pcnt,le.populated_company_fein_pcnt,le.populated_company_phone_pcnt,le.populated_prim_range_pcnt,le.populated_prim_name_derived_pcnt,le.populated_sec_range_pcnt,le.populated_v_city_name_pcnt,le.populated_st_pcnt,le.populated_zip_pcnt,le.populated_dt_first_seen_pcnt,le.populated_dt_last_seen_pcnt);
  SELF.maxlength := CHOOSE(C,le.maxlength_cnp_name,le.maxlength_cnp_number,le.maxlength_active_duns_number,le.maxlength_active_enterprise_number,le.maxlength_active_domestic_corp_key,le.maxlength_hist_enterprise_number,le.maxlength_hist_duns_number,le.maxlength_hist_domestic_corp_key,le.maxlength_foreign_corp_key,le.maxlength_unk_corp_key,le.maxlength_ebr_file_number,le.maxlength_company_fein,le.maxlength_company_phone,le.maxlength_prim_range,le.maxlength_prim_name_derived,le.maxlength_sec_range,le.maxlength_v_city_name,le.maxlength_st,le.maxlength_zip,le.maxlength_dt_first_seen,le.maxlength_dt_last_seen);
  SELF.avelength := CHOOSE(C,le.avelength_cnp_name,le.avelength_cnp_number,le.avelength_active_duns_number,le.avelength_active_enterprise_number,le.avelength_active_domestic_corp_key,le.avelength_hist_enterprise_number,le.avelength_hist_duns_number,le.avelength_hist_domestic_corp_key,le.avelength_foreign_corp_key,le.avelength_unk_corp_key,le.avelength_ebr_file_number,le.avelength_company_fein,le.avelength_company_phone,le.avelength_prim_range,le.avelength_prim_name_derived,le.avelength_sec_range,le.avelength_v_city_name,le.avelength_st,le.avelength_zip,le.avelength_dt_first_seen,le.avelength_dt_last_seen);
END;
...
SALT30.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.id := le.Proxid;
  SELF.Src := le.source;
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT30.StrType)le.cnp_name),TRIM((SALT30.StrType)le.cnp_number),TRIM((SALT30.StrType)le.active_duns_number),TRIM((SALT30.StrType)le.active_enterprise_number),TRIM((SALT30.StrType)le.active_domestic_corp_key),TRIM((SALT30.StrType)le.hist_enterprise_number),TRIM((SALT30.StrType)le.hist_duns_number),TRIM((SALT30.StrType)le.hist_domestic_corp_key),TRIM((SALT30.StrType)le.foreign_corp_key),TRIM((SALT30.StrType)le.unk_corp_key),TRIM((SALT30.StrType)le.ebr_file_number),TRIM((SALT30.StrType)le.company_fein),TRIM((SALT30.StrType)le.company_phone),TRIM((SALT30.StrType)le.prim_range),TRIM((SALT30.StrType)le.prim_name_derived),TRIM((SALT30.StrType)le.sec_range),TRIM((SALT30.StrType)le.v_city_name),TRIM((SALT30.StrType)le.st),TRIM((SALT30.StrType)le.zip),TRIM((SALT30.StrType)le.dt_first_seen),TRIM((SALT30.StrType)le.dt_last_seen)));
  SELF.FldNo := C;
END;
...
SALT30.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 21);
  SELF.FldNo2 := 1 + (C % 21);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT30.StrType)le.cnp_name),TRIM((SALT30.StrType)le.cnp_number),TRIM((SALT30.StrType)le.active_duns_number),TRIM((SALT30.StrType)le.active_enterprise_number),TRIM((SALT30.StrType)le.active_domestic_corp_key),TRIM((SALT30.StrType)le.hist_enterprise_number),TRIM((SALT30.StrType)le.hist_duns_number),TRIM((SALT30.StrType)le.hist_domestic_corp_key),TRIM((SALT30.StrType)le.foreign_corp_key),TRIM((SALT30.StrType)le.unk_corp_key),TRIM((SALT30.StrType)le.ebr_file_number),TRIM((SALT30.StrType)le.company_fein),TRIM((SALT30.StrType)le.company_phone),TRIM((SALT30.StrType)le.prim_range),TRIM((SALT30.StrType)le.prim_name_derived),TRIM((SALT30.StrType)le.sec_range),TRIM((SALT30.StrType)le.v_city_name),TRIM((SALT30.StrType)le.st),TRIM((SALT30.StrType)le.zip),TRIM((SALT30.StrType)le.dt_first_seen),TRIM((SALT30.StrType)le.dt_last_seen)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT30.StrType)le.cnp_name),TRIM((SALT30.StrType)le.cnp_number),TRIM((SALT30.StrType)le.active_duns_number),TRIM((SALT30.StrType)le.active_enterprise_number),TRIM((SALT30.StrType)le.active_domestic_corp_key),TRIM((SALT30.StrType)le.hist_enterprise_number),TRIM((SALT30.StrType)le.hist_duns_number),TRIM((SALT30.StrType)le.hist_domestic_corp_key),TRIM((SALT30.StrType)le.foreign_corp_key),TRIM((SALT30.StrType)le.unk_corp_key),TRIM((SALT30.StrType)le.ebr_file_number),TRIM((SALT30.StrType)le.company_fein),TRIM((SALT30.StrType)le.company_phone),TRIM((SALT30.StrType)le.prim_range),TRIM((SALT30.StrType)le.prim_name_derived),TRIM((SALT30.StrType)le.sec_range),TRIM((SALT30.StrType)le.v_city_name),TRIM((SALT30.StrType)le.st),TRIM((SALT30.StrType)le.zip),TRIM((SALT30.StrType)le.dt_first_seen),TRIM((SALT30.StrType)le.dt_last_seen)));
  END;
...
SHARED FldIds := DATASET([{1,'cnp_name'}
      ,{2,'cnp_number'}
      ,{3,'active_duns_number'}
      ,{4,'active_enterprise_number'}
      ,{5,'active_domestic_corp_key'}
...
PrettyErrorTotals := RECORD
  ...
  FieldType := CHOOSE(TotalErrors.FieldNum,'Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','alpha','number','Unknown','Unknown','Unknown','Unknown','Unknown');
  ErrorMessage := CHOOSE(TotalErrors.FieldNum,Fields.InValidMessage_cnp_name(TotalErrors.ErrorNum),Fields.InValidMessage_cnp_number(TotalErrors.ErrorNum),Fields.InValidMessage_active_duns_number(TotalErrors.ErrorNum),Fields.InValidMessage_active_enterprise_number(TotalErrors.ErrorNum),Fields.InValidMessage_active_domestic_corp_key(TotalErrors.ErrorNum),Fields.InValidMessage_hist_enterprise_number(TotalErrors.ErrorNum),Fields.InValidMessage_hist_duns_number(TotalErrors.ErrorNum),Fields.InValidMessage_hist_domestic_corp_key(TotalErrors.ErrorNum),Fields.InValidMessage_foreign_corp_key(TotalErrors.ErrorNum),Fields.InValidMessage_unk_corp_key(TotalErrors.ErrorNum),Fields.InValidMessage_ebr_file_number(TotalErrors.ErrorNum),Fields.InValidMessage_company_fein(TotalErrors.ErrorNum),Fields.InValidMessage_company_phone(TotalErrors.ErrorNum),Fields.InValidMessage_prim_range(TotalErrors.ErrorNum),Fields.InValidMessage_prim_name_derived(TotalErrors.ErrorNum),Fields.InValidMessage_sec_range(TotalErrors.ErrorNum),Fields.InValidMessage_v_city_name(TotalErrors.ErrorNum),Fields.InValidMessage_st(TotalErrors.ErrorNum),Fields.InValidMessage_zip(TotalErrors.ErrorNum),'','','',Fields.InValidMessage_dt_first_seen(TotalErrors.ErrorNum),Fields.InValidMessage_dt_last_seen(TotalErrors.ErrorNum));
  TotalErrors.Cnt;
END;

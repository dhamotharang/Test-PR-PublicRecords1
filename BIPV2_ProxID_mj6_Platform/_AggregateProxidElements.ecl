import tools;
EXPORT _AggregateProxidElements(
	 dataset(layout_DOT_Base)	pDataset
) :=
function
	dslim := project(pDataset, transform(
	{pDataset.proxid,pDataset.cnp_name
  ,pDataset.cnp_number
	,pDataset.prim_name_derived,string address/*,string contact*/,pDataset.source,pDataset.active_duns_number,pDataset.hist_duns_number,pDataset.active_enterprise_number
  ,string active_domestic_corp_key,string foreign_corp_key,string fein
},
//		self.corp_key	:= if(left.company_charter_state != '' and left.company_charter_number != ''	,trim(left.company_charter_state,left,right) + '-' + trim(left.company_charter_number,left,right)	,'');
		...
		self.address	:= stringlib.stringcleanspaces(trim(left.prim_range,left,right) + ' ' + trim(left.prim_name_derived,left,right) + ' ' + trim(left.sec_range,left,right) + ' ' + trim(left.v_city_name,left,right) + ' ' + trim(left.st,left,right) + ' ' + trim(left.zip));

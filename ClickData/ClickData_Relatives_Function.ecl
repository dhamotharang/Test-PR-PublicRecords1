...
boolean glb := ut.glb_ok(glbpurpose);
...
gfD := gong.Key_History_did;
gfH := gong.Key_History_HHID;

outf3 get_gong_dates(outf3 L, gfD R) := transform
...
outf3 get_gong_dates2(outf3 L, gfH R) := transform
	self.EDA_Disconnect := if ((integer)L.eda_disconnect > (integer)R.deletion_date, L.eda_disconnect, R.deletion_date);
	self.EDA_Connect := if (((integer)L.eda_connect != 0 and (integer)L.eda_connect < (integer)R.dt_first_seen) or (integer)R.dt_first_seen = 0, L.eda_connect, R.dt_first_seen);
	self := L;
end;

outf4a := join(outf3((integer)adl != 0),
			gfD,
				keyed((integer)left.adl = right.l_did),
			get_gong_dates(LEFT,RIGHT), 
			left outer) + outf3((integer)adl = 0);
			
outf4 := join(outf4a((integer)hhid != 0),
			gfH,
				keyed((integer)left.hhid = right.s_hhid),
			get_gong_dates2(LEFT,RIGHT),
			left outer) + outf4a((integer)hhid = 0);

...
*/
import iesp,AutoStandardI,doxie_ln;
export Clickdata_Interactive_Best_Service := macro
    ...
    tempmod := AutoStandardI.GlobalModule();
		...
		appType := AutoStandardI.InterfaceTranslator.application_type_val.val(project(tempmod,AutoStandardI.InterfaceTranslator.application_type_val.params));
		...
		// 		
		out_recs_raw := ungroup(clickdata.ClickData_Best_Function(f_in_raw, true, true,,,appType));				
		out_recs := ClickData.fn_filter_demographics(out_recs_raw, stringlib.StringToUpperCase(DataPackageEnhancement));	

...
export Proc_Build_NOD_Autokeys_bid(string filedate) :=
function

	c := Foreclosure_Services.Constants(filedate); 

	ak_keyname  := c.ak_nod_keyname_bid;
	ak_logical  := c.ak_nod_logical_bid;
	ak_dataset  := c.ak_nod_dataset_bid;
	ak_skipSet  := c.ak_nod_skipSet_bid;
	ak_typeStr  := c.ak_nod_typeStr_bid;

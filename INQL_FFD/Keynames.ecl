...
module
  ...
  data_env := if(isFCRA,1,0);
	...
	export Group_RID								:= tools.mod_FilenamesBuild(lGroupRIDTemplate ,pVersion);
	...
	export Group_RID_Encrypted							:= tools.mod_FilenamesBuild(lGroupRIDEncryptedTemplate ,pVersion);
	
	// export lGroupRIDEncryptedTemplateFull	:= lPrefix + '::@version@::group_rid_full::encrypted';
	// export GroupRID_Encrypted_full				:= tools.mod_FilenamesBuild(lGroupRIDEncryptedTemplateFull ,pVersion);
	
	// export lGroupRIDEncryptedTemplateDelta	:= lPrefix + '::@version@::group_rid_delta::encrypted';
	...
	export dAll_filenames := 
		...
		Group_RID.dAll_filenames	+
		Group_RID_Encrypted.dAll_filenames	
		// +

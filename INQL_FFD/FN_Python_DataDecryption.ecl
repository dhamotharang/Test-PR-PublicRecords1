...
EXPORT FN_Python_DataDecryption := MODULE
  ...
  EXPORT PythonDecryptRsaPksc(string msg_encrypted, string original_key) :=
    python_decrypt_pksc (msg_encrypted, original_key, inql_ffd._Constants.DECRYPTION_KEY_PRIVATE);
  ...
  EXPORT PythonDecryptRsaPksc_OAEP(string msg_encrypted) :=
    python_decrypt_pksc_OAEP (msg_encrypted, inql_ffd._Constants.DECRYPTION_KEY_PRIVATE);

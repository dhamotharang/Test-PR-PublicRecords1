...
export  file_Facilities_Conversion(string process_date) := function
...
layout_FacilityAddl xformFile(CNLD_Facilities.layout pInput) 
    := 
	   TRANSFORM
			self.first_seen_date := process_date;
			self.last_seen_date	 := process_date;
...
// d_final := output(dsRecOutNorm_dist,,'~thor_data400::in::cnldfacilities::'+process_date+'::base',overwrite);

// add_super := sequential(fileservices.startsuperfiletransaction(),
													// fileservices.addsuperfile('~thor_data400::in::cnldfacilities::base','~thor_data400::in::cnldfacilities::'+process_date+'::base'),

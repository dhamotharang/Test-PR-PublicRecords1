import Business_Header, ut,mdr,CNLD_Facilities;

export fCNLD_Facilities_As_Business_Header(

	dataset(CNLD_Facilities.layout_Facilities_AID_schd) pFacilities = CNLD_Facilities.file_Facilities_AID

) :=
  function
  
	Facilities_In := pFacilities;

	Business_Header.Layout_Business_Header_New Translate_Facilities_To_BHF(CNLD_Facilities.layout_Facilities_AID_schd l) := transform
	self.source := MDR.sourceTools.src_CNLD_Facilities;          // Source file type
	self.source_group := '';
	self.vl_id := if(l.st_lic_in<> '', l.st_lic_in, '') + if(l.st_lic_num <> '', l.st_lic_num, '');
	self.vendor_id := if(l.st_lic_in<> '', l.st_lic_in, '') + if(l.st_lic_num <> '', l.st_lic_num, '');
	self.dt_first_seen := (unsigned4)if(l.first_seen_date <> '',l.first_seen_date, version);
	self.dt_last_seen := (unsigned4)if(l.last_seen_date <> '',l.last_seen_date, version);
	self.dt_vendor_first_reported := (unsigned4)if(l.first_seen_date <> '',l.first_seen_date, version);
	self.dt_vendor_last_reported := (unsigned4)if(l.last_seen_date <> '',l.last_seen_date, version);
	self.company_name := Stringlib.StringToUpperCase(l.org_name);
	self.city := l.v_city_name;
	self.state := l.st;
	self.zip := (unsigned3)l.zip5;
	self.zip4 := (unsigned2)l.zip4;
	self.phone := if(l.addr_phone <> '', (unsigned6)l.addr_phone, 0);
	self.phone_score := if((integer)l.addr_phone = 0, 0, 2);  // Non-zero score if phone is not blank
	self.current := true;
	...
	Facility_Init := project(Facilities_In, Translate_Facilities_To_BHF(left));

	// Rollup
	Business_Header.Layout_Business_Header_New RollupFDIC(Business_Header.Layout_Business_Header_New L, Business_Header.Layout_Business_Header_New R) := TRANSFORM
	SELF.dt_first_seen := 
	...
	Facilities_Init_Dist := DISTRIBUTE(Facility_Init,HASH(zip, TRIM(prim_name), TRIM(prim_range), TRIM(source_group), TRIM(company_name)));
	Facilities_Init_Sort := SORT(Facilities_Init_Dist, zip, prim_range, prim_name, source_group, company_name,
																IF(sec_range <> '', 0, 1), sec_range,
																IF(phone <> 0, 0, 1), phone,
																IF(fein <> 0, 0, 1), fein,
																dt_vendor_last_reported, dt_vendor_first_reported, dt_last_seen, LOCAL);
																
	Facilities_Init_Rollup := ROLLUP(Facilities_Init_Sort,

...
module
	...
	function
		...
		NID.Mac_CleanFullNames(pPreProcessInput, cleaned_name_output, cleaned_carrier_name);
		...
		transform
			...
			// -- Map Fields
			//////////////////////////////////////////////////////////////////////////////////////
			self.cleaned_carrier_name := if(l.nametype IN ['P','D'],'',l.cleaned_carrier_name);
			...
			self.name_suffix	      	:= if(l.nametype IN ['P','D'],l.name_suffix,'');
			self.name_score	        	:= '';
	...
	function
		...
		//////////////////////////////////////////////////////////////////////////////////////
		layouts.base Prepare_Address(Layouts.Base l) := transform
			self.prep_addr_line1		 := stringlib.stringcleanspaces(l.cleaned_carrier_street);
			self.prep_addr_line_last := stringlib.stringcleanspaces(l.cleaned_carrier_city + if (trim(l.cleaned_carrier_city) <> '',', ','') + l.cleaned_carrier_state + ' ' + l.cleaned_carrier_zip_code[1..5]);
			self 										 := l;			
		end;
			
		AddressProj				:= project(pStandardizeNameInput,Prepare_Address(left));
	
		HasAddress				:= trim(AddressProj.prep_addr_line_last, left,right) != '';
										
		dWith_address			:= AddressProj(HasAddress);
		dWithout_address	:= AddressProj(not(HasAddress));

...
EXPORT mac_compare_results(in_testcases, in_recs_a, in_recs_b, out_recs) := MACRO
  ...
  out_recs := join(%out_recs_a%, %rolled_recs_b%,
    ...
    TRANSFORM(dev_regression.layouts.testcase_result,
      ...
      SELF.result := map(
        //if status is the same and corresponds to one of the doxie errors, I want a specific result code
        (LEFT.soap_status = RIGHT.soap_status) AND (LEFT.soap_message != '') AND
            (STD.Str.Find (LEFT.soap_message, TRIM(doxie.ErrorCodes((integer)RIGHT.soap_status), RIGHT), 1) > 0) => -2,

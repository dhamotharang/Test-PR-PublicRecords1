...
EXPORT KeyName := PRTE_Health_Provider_Services.Files.FILE_NPI; //'~'+'key::PRTE_Health_Provider_Services::LNPID::Refs::NPI';
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,LNPID,NPI_NUMBER,FNAME,MNAME,LNAME,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'NPI','SNAME',CntRed_SNAME*100,CntRed_SNAME*TSize},{'NPI','GENDER',CntRed_GENDER*100,CntRed_GENDER*TSize},{'NPI','DOB',CntRed_DOB*100,CntRed_DOB*TSize},{'NPI','C_LIC_NBR',CntRed_C_LIC_NBR*100,CntRed_C_LIC_NBR*TSize},{'NPI','LIC_STATE',CntRed_LIC_STATE*100,CntRed_LIC_STATE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.NPI_NUMBER <> (typeof(le.NPI_NUMBER))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.NPI_NUMBER) param_NPI_NUMBER,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.LNAME) param_LNAME) := 
    STEPPED( LIMIT( Key(
...
EXPORT ScoredLNPIDFetch(TYPEOF(h.NPI_NUMBER) param_NPI_NUMBER,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.LNAME) param_LNAME,TYPEOF(h.SNAME) param_SNAME,TYPEOF(h.GENDER) param_GENDER,UNSIGNED4 param_DOB,TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR,TYPEOF(h.LIC_STATE) param_LIC_STATE) := FUNCTION
  RawData := RawFetch(param_NPI_NUMBER,param_FNAME,param_MNAME,param_LNAME);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.NPI_NUMBER_match_code := MAP(
		le.NPI_NUMBER = (TYPEOF(le.NPI_NUMBER))'' OR le.NPI_NUMBER = (TYPEOF(le.NPI_NUMBER))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthProvider).match_NPI_NUMBER(le.NPI_NUMBER,param_NPI_NUMBER,TRUE));
    ...
    SELF.Weight :=MAX(0,SELF.NPI_NUMBERWeight) + MAX(self.MAINNAMEWeight,MAX(0,SELF.FNAMEWeight) + MAX(0,SELF.MNAMEWeight) + MAX(0,SELF.LNAMEWeight)) + MAX(0,SELF.SNAMEWeight) + MAX(0,SELF.GENDERWeight) + MAX(0,SELF.DOBWeight) + MAX(0,SELF.C_LIC_NBRWeight) + MAX(0,SELF.LIC_STATEWeight);
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.NPI_NUMBER_match_code := MAP(
		le.NPI_NUMBER = (TYPEOF(le.NPI_NUMBER))'' OR le.NPI_NUMBER = (TYPEOF(le.NPI_NUMBER))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthProvider).match_NPI_NUMBER(le.NPI_NUMBER,ri.NPI_NUMBER,TRUE));
...
#IF(#TEXT(Input_NPI_NUMBER)<>'')
  #uniquename(trans)
  PRTE_Health_Provider_Services.Key_HealthProvider_NPI.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Provider_Services.Key_HealthProvider_NPI.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

...
EXPORT KeyName := PRTE_Health_Provider_Services.Files.FILE_CNSMR_SSN; //'~'+'key::PRTE_Health_Provider_Services::LNPID::Refs::CNSMR_SSN_LP';
...
DataForKey0 := DEDUP(SORT(TABLE(h(CNSMR_SSN NOT IN SET(s.nulls_CNSMR_SSN,CNSMR_SSN)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
EXPORT Shrinkage := DATASET([{'CNSMR_SSN_LP','V_CITY_NAME',CntRed_V_CITY_NAME*100,CntRed_V_CITY_NAME*TSize},{'CNSMR_SSN_LP','ST',CntRed_ST*100,CntRed_ST*TSize},{'CNSMR_SSN_LP','GENDER',CntRed_GENDER*100,CntRed_GENDER*TSize},{'CNSMR_SSN_LP','SNAME',CntRed_SNAME*100,CntRed_SNAME*TSize},{'CNSMR_SSN_LP','CNSMR_DOB',CntRed_CNSMR_DOB*100,CntRed_CNSMR_DOB*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.CNSMR_SSN <> (typeof(le.CNSMR_SSN))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.CNSMR_SSN) param_CNSMR_SSN,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.LNAME) param_LNAME) := 
    STEPPED( LIMIT( Key(
          ( CNSMR_SSN = param_CNSMR_SSN AND param_CNSMR_SSN <> (TYPEOF(CNSMR_SSN))'' )
...
EXPORT ScoredLNPIDFetch(TYPEOF(h.CNSMR_SSN) param_CNSMR_SSN,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.LNAME) param_LNAME,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.GENDER) param_GENDER,TYPEOF(h.SNAME) param_SNAME,UNSIGNED4 param_CNSMR_DOB) := FUNCTION
  RawData := RawFetch(param_CNSMR_SSN,param_FNAME,param_MNAME,param_LNAME);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.CNSMR_SSN_match_code := MAP(
		le.CNSMR_SSN = (TYPEOF(le.CNSMR_SSN))'' OR le.CNSMR_SSN = (TYPEOF(le.CNSMR_SSN))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthProvider).match_CNSMR_SSN(le.CNSMR_SSN,param_CNSMR_SSN,TRUE));
    SELF.CNSMR_SSNWeight := (50+MAP ( le.CNSMR_SSN = param_CNSMR_SSN  => le.CNSMR_SSN_weight100,
          le.CNSMR_SSN = (TYPEOF(le.CNSMR_SSN))'' OR param_CNSMR_SSN = (TYPEOF(le.CNSMR_SSN))'' => 0,
    ...
    SELF.Weight :=MAX(0,SELF.CNSMR_SSNWeight) + MAX(self.MAINNAMEWeight,MAX(0,SELF.FNAMEWeight) + MAX(0,SELF.MNAMEWeight) + MAX(0,SELF.LNAMEWeight)) + MAX(0,SELF.V_CITY_NAMEWeight) + MAX(0,SELF.STWeight) + MAX(0,SELF.GENDERWeight) + MAX(0,SELF.SNAMEWeight) + MAX(0,SELF.CNSMR_DOBWeight);
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.CNSMR_SSN_match_code := MAP(
		le.CNSMR_SSN = (TYPEOF(le.CNSMR_SSN))'' OR le.CNSMR_SSN = (TYPEOF(le.CNSMR_SSN))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthProvider).match_CNSMR_SSN(le.CNSMR_SSN,ri.CNSMR_SSN,TRUE));
    SELF.CNSMR_SSNWeight := (50+MAP ( le.CNSMR_SSN = ri.CNSMR_SSN  => le.CNSMR_SSN_weight100,
          le.CNSMR_SSN = (TYPEOF(le.CNSMR_SSN))'' OR ri.CNSMR_SSN = (TYPEOF(le.CNSMR_SSN))'' => 0,
           -0.963*le.CNSMR_SSN_weight100))/100; 
    SELF.FNAME_match_code := MAP(
  ...
  J0 := JOIN(Recs1,Key,LEFT.CNSMR_SSN = RIGHT.CNSMR_SSN
     AND ( ( 
    ...
    ATMOST(LEFT.CNSMR_SSN = RIGHT.CNSMR_SSN,10000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.CNSMR_SSN = RIGHT.CNSMR_SSN
     AND ( ( 
        (LEFT.FNAME[1..LENGTH(TRIM(RIGHT.FNAME))] = RIGHT.FNAME OR RIGHT.FNAME[1..LENGTH(TRIM(LEFT.FNAME))] = LEFT.FNAME OR metaphonelib.DMetaPhone1(LEFT.FNAME)=metaphonelib.DMetaPhone1(RIGHT.FNAME) OR SALT29.WithinEditN(LEFT.FNAME,RIGHT.FNAME,1, 0)  OR RIGHT.FNAME_PreferredName = fn_PreferredName(LEFT.FNAME) ) AND (LEFT.MNAME[1..LENGTH(TRIM(RIGHT.MNAME))] = RIGHT.MNAME OR RIGHT.MNAME[1..LENGTH(TRIM(LEFT.MNAME))] = LEFT.MNAME OR SALT29.WithinEditN(LEFT.MNAME,RIGHT.MNAME,2, 0) ) AND (LEFT.LNAME = (TYPEOF(LEFT.LNAME))'' OR RIGHT.LNAME = (TYPEOF(RIGHT.LNAME))'' OR metaphonelib.DMetaPhone1(LEFT.LNAME)=metaphonelib.DMetaPhone1(RIGHT.LNAME) OR SALT29.WithinEditN(LEFT.LNAME,RIGHT.LNAME,1, 0) OR SALT29.HyphenMatch(LEFT.LNAME,RIGHT.LNAME,1)<=2 )
          OR SALT29.fn_concept_wordbag_EditN.Match3((SALT29.StrType)RIGHT.FNAME,RIGHT.FNAME_MAINNAME_weight100,true,0,true,1,(SALT29.StrType)RIGHT.MNAME,RIGHT.MNAME_MAINNAME_weight100,true,0,true,2,(SALT29.StrType)RIGHT.LNAME,RIGHT.LNAME_MAINNAME_weight100,true,0,false,1,(SALT29.StrType)LEFT.FNAME,0,(SALT29.StrType)LEFT.MNAME,0,(SALT29.StrType)LEFT.LNAME,0)>= 0) ),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.CNSMR_SSN = RIGHT.CNSMR_SSN,10000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_CNSMR_SSN='',Input_FNAME='',Input_MNAME='',Input_LNAME='',Input_V_CITY_NAME='',Input_ST='',Input_GENDER='',Input_SNAME='',Input_CNSMR_DOB='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,PRTE_Health_Provider_Services;
#IF(#TEXT(Input_CNSMR_SSN)<>'')
  #uniquename(trans)
  PRTE_Health_Provider_Services.Key_HealthProvider_CNSMR_SSN_LP.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Provider_Services.Key_HealthProvider_CNSMR_SSN_LP.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

...
EXPORT KeyName := PRTE_Health_Provider_Services.Files.FILE_CNSMR_DOB; //'~'+'key::PRTE_Health_Provider_Services::LNPID::Refs::CNSMR_DOB_LP';
...
#IF(#TEXT(Input_CNSMR_DOB)<>'' AND #TEXT(Input_FNAME)<>'' AND #TEXT(Input_LNAME)<>'')
  #uniquename(trans)
  PRTE_Health_Provider_Services.Key_HealthProvider_CNSMR_DOB_LP.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Provider_Services.Key_HealthProvider_CNSMR_DOB_LP.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

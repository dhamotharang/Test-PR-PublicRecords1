EXPORT Key_HealthProvider_TAX := MODULE
IMPORT SALT29,ut,std;
//TAX_ID:?:MAINNAME:+:SNAME:GENDER:DOB:C_LIC_NBR:LIC_STATE
EXPORT KeyName := PRTE_Health_Provider_Services.Files.FILE_TAX; //'~'+'key::PRTE_Health_Provider_Services::LNPID::Refs::TAX';
...
//Scores for various field components
  h.TAX_ID_weight100 ; // Contains 100x the specificity
...
DataForKey0 := DEDUP(SORT(TABLE(h(TAX_ID NOT IN SET(s.nulls_TAX_ID,TAX_ID)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
EXPORT Key := INDEX(DataForKey0,,KeyName);
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,LNPID,TAX_ID,FNAME,MNAME,LNAME,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'TAX','SNAME',CntRed_SNAME*100,CntRed_SNAME*TSize},{'TAX','GENDER',CntRed_GENDER*100,CntRed_GENDER*TSize},{'TAX','DOB',CntRed_DOB*100,CntRed_DOB*TSize},{'TAX','C_LIC_NBR',CntRed_C_LIC_NBR*100,CntRed_C_LIC_NBR*TSize},{'TAX','LIC_STATE',CntRed_LIC_STATE*100,CntRed_LIC_STATE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.TAX_ID <> (typeof(le.TAX_ID))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.TAX_ID) param_TAX_ID,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.LNAME) param_LNAME) := 
    STEPPED( LIMIT( Key(
          ( TAX_ID = param_TAX_ID AND param_TAX_ID <> (TYPEOF(TAX_ID))'' )
...
EXPORT ScoredLNPIDFetch(TYPEOF(h.TAX_ID) param_TAX_ID,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.LNAME) param_LNAME,TYPEOF(h.SNAME) param_SNAME,TYPEOF(h.GENDER) param_GENDER,UNSIGNED4 param_DOB,TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR,TYPEOF(h.LIC_STATE) param_LIC_STATE) := FUNCTION
  RawData := RawFetch(param_TAX_ID,param_FNAME,param_MNAME,param_LNAME);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.TAX_ID_match_code := MAP(
		le.TAX_ID = (TYPEOF(le.TAX_ID))'' OR le.TAX_ID = (TYPEOF(le.TAX_ID))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthProvider).match_TAX_ID(le.TAX_ID,param_TAX_ID,TRUE));
    SELF.TAX_IDWeight := (50+MAP ( le.TAX_ID = param_TAX_ID  => le.TAX_ID_weight100,
          le.TAX_ID = (TYPEOF(le.TAX_ID))'' OR param_TAX_ID = (TYPEOF(le.TAX_ID))'' => 0,
           -1.000*le.TAX_ID_weight100))/100; 
    ...
    SELF.Weight :=MAX(0,SELF.TAX_IDWeight) + MAX(self.MAINNAMEWeight,MAX(0,SELF.FNAMEWeight) + MAX(0,SELF.MNAMEWeight) + MAX(0,SELF.LNAMEWeight)) + MAX(0,SELF.SNAMEWeight) + MAX(0,SELF.GENDERWeight) + MAX(0,SELF.DOBWeight) + MAX(0,SELF.C_LIC_NBRWeight) + MAX(0,SELF.LIC_STATEWeight);
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.TAX_ID_match_code := MAP(
		le.TAX_ID = (TYPEOF(le.TAX_ID))'' OR le.TAX_ID = (TYPEOF(le.TAX_ID))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthProvider).match_TAX_ID(le.TAX_ID,ri.TAX_ID,TRUE));
    SELF.TAX_IDWeight := (50+MAP ( le.TAX_ID = ri.TAX_ID  => le.TAX_ID_weight100,
          le.TAX_ID = (TYPEOF(le.TAX_ID))'' OR ri.TAX_ID = (TYPEOF(le.TAX_ID))'' => 0,
           -1.000*le.TAX_ID_weight100))/100; 
    SELF.FNAME_match_code := MAP(
    ...
    SELF := le;
  END;
  Recs0 := Recs(TAX_ID <> (typeof(TAX_ID))'');
  ...
  J0 := JOIN(Recs1,Key,LEFT.TAX_ID = RIGHT.TAX_ID
     AND ( ( 
    ...
    ATMOST(LEFT.TAX_ID = RIGHT.TAX_ID,10000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.TAX_ID = RIGHT.TAX_ID
     AND ( ( 
        (LEFT.FNAME[1..LENGTH(TRIM(RIGHT.FNAME))] = RIGHT.FNAME OR RIGHT.FNAME[1..LENGTH(TRIM(LEFT.FNAME))] = LEFT.FNAME OR metaphonelib.DMetaPhone1(LEFT.FNAME)=metaphonelib.DMetaPhone1(RIGHT.FNAME) OR SALT29.WithinEditN(LEFT.FNAME,RIGHT.FNAME,1, 0)  OR RIGHT.FNAME_PreferredName = fn_PreferredName(LEFT.FNAME) ) AND (LEFT.MNAME[1..LENGTH(TRIM(RIGHT.MNAME))] = RIGHT.MNAME OR RIGHT.MNAME[1..LENGTH(TRIM(LEFT.MNAME))] = LEFT.MNAME OR SALT29.WithinEditN(LEFT.MNAME,RIGHT.MNAME,2, 0) ) AND (LEFT.LNAME = (TYPEOF(LEFT.LNAME))'' OR RIGHT.LNAME = (TYPEOF(RIGHT.LNAME))'' OR metaphonelib.DMetaPhone1(LEFT.LNAME)=metaphonelib.DMetaPhone1(RIGHT.LNAME) OR SALT29.WithinEditN(LEFT.LNAME,RIGHT.LNAME,1, 0) OR SALT29.HyphenMatch(LEFT.LNAME,RIGHT.LNAME,1)<=2 )
          OR SALT29.fn_concept_wordbag_EditN.Match3((SALT29.StrType)RIGHT.FNAME,RIGHT.FNAME_MAINNAME_weight100,true,0,true,1,(SALT29.StrType)RIGHT.MNAME,RIGHT.MNAME_MAINNAME_weight100,true,0,true,2,(SALT29.StrType)RIGHT.LNAME,RIGHT.LNAME_MAINNAME_weight100,true,0,false,1,(SALT29.StrType)LEFT.FNAME,0,(SALT29.StrType)LEFT.MNAME,0,(SALT29.StrType)LEFT.LNAME,0)>= 0) ),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.TAX_ID = RIGHT.TAX_ID,10000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_TAX_ID='',Input_FNAME='',Input_MNAME='',Input_LNAME='',Input_SNAME='',Input_GENDER='',Input_DOB='',Input_C_LIC_NBR='',Input_LIC_STATE='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,PRTE_Health_Provider_Services;
#IF(#TEXT(Input_TAX_ID)<>'')
  #uniquename(trans)
  PRTE_Health_Provider_Services.Key_HealthProvider_TAX.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Provider_Services.Key_HealthProvider_TAX.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

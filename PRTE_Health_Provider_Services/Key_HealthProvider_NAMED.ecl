...
EXPORT KeyName := PRTE_Health_Provider_Services.Files.FILE_Name; //'~'+'key::PRTE_Health_Provider_Services::LNPID::Refs::NAMED';
...
#IF(#TEXT(Input_LNAME)<>'' AND #TEXT(Input_ZIP)<>'' AND #TEXT(Input_C_LIC_NBR)<>'')
  #uniquename(trans)
  PRTE_Health_Provider_Services.Key_HealthProvider_NAMED.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Provider_Services.Key_HealthProvider_NAMED.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

...
EXPORT KeyName := PRTE_Health_Provider_Services.Files.FILE_SSN; //'~'+'key::PRTE_Health_Provider_Services::LNPID::Refs::SSN_LP';
...
DataForKey0 := DEDUP(SORT(TABLE(h(SSN NOT IN SET(s.nulls_SSN,SSN)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
EXPORT Key := INDEX(DataForKey0,,KeyName);
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,LNPID,SSN,FNAME,MNAME,LNAME,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'SSN_LP','V_CITY_NAME',CntRed_V_CITY_NAME*100,CntRed_V_CITY_NAME*TSize},{'SSN_LP','ST',CntRed_ST*100,CntRed_ST*TSize},{'SSN_LP','GENDER',CntRed_GENDER*100,CntRed_GENDER*TSize},{'SSN_LP','SNAME',CntRed_SNAME*100,CntRed_SNAME*TSize},{'SSN_LP','DOB',CntRed_DOB*100,CntRed_DOB*TSize},{'SSN_LP','C_LIC_NBR',CntRed_C_LIC_NBR*100,CntRed_C_LIC_NBR*TSize},{'SSN_LP','LIC_STATE',CntRed_LIC_STATE*100,CntRed_LIC_STATE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.SSN <> (typeof(le.SSN))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.SSN) param_SSN,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.LNAME) param_LNAME) := 
    STEPPED( LIMIT( Key(
          ( SSN = param_SSN AND param_SSN <> (TYPEOF(SSN))'' )
...
EXPORT ScoredLNPIDFetch(TYPEOF(h.SSN) param_SSN,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.LNAME) param_LNAME,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.GENDER) param_GENDER,TYPEOF(h.SNAME) param_SNAME,UNSIGNED4 param_DOB,TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR,TYPEOF(h.LIC_STATE) param_LIC_STATE) := FUNCTION
  RawData := RawFetch(param_SSN,param_FNAME,param_MNAME,param_LNAME);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.SSN_match_code := MAP(
		le.SSN = (TYPEOF(le.SSN))'' OR le.SSN = (TYPEOF(le.SSN))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthProvider).match_SSN(le.SSN,param_SSN,TRUE));
    SELF.SSNWeight := (50+MAP ( le.SSN = param_SSN  => le.SSN_weight100,
          le.SSN = (TYPEOF(le.SSN))'' OR param_SSN = (TYPEOF(le.SSN))'' => 0,
    ...
    SELF.Weight :=MAX(0,SELF.SSNWeight) + MAX(self.MAINNAMEWeight,MAX(0,SELF.FNAMEWeight) + MAX(0,SELF.MNAMEWeight) + MAX(0,SELF.LNAMEWeight)) + MAX(0,SELF.V_CITY_NAMEWeight) + MAX(0,SELF.STWeight) + MAX(0,SELF.GENDERWeight) + MAX(0,SELF.SNAMEWeight) + MAX(0,SELF.DOBWeight) + MAX(0,SELF.C_LIC_NBRWeight) + MAX(0,SELF.LIC_STATEWeight);
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.SSN_match_code := MAP(
		le.SSN = (TYPEOF(le.SSN))'' OR le.SSN = (TYPEOF(le.SSN))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthProvider).match_SSN(le.SSN,ri.SSN,TRUE));
    SELF.SSNWeight := (50+MAP ( le.SSN = ri.SSN  => le.SSN_weight100,
          le.SSN = (TYPEOF(le.SSN))'' OR ri.SSN = (TYPEOF(le.SSN))'' => 0,
           -0.965*le.SSN_weight100))/100; 
    SELF.FNAME_match_code := MAP(
  ...
  J0 := JOIN(Recs1,Key,LEFT.SSN = RIGHT.SSN
     AND ( ( 
    ...
    ATMOST(LEFT.SSN = RIGHT.SSN,10000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.SSN = RIGHT.SSN
     AND ( ( 
        (LEFT.FNAME[1..LENGTH(TRIM(RIGHT.FNAME))] = RIGHT.FNAME OR RIGHT.FNAME[1..LENGTH(TRIM(LEFT.FNAME))] = LEFT.FNAME OR metaphonelib.DMetaPhone1(LEFT.FNAME)=metaphonelib.DMetaPhone1(RIGHT.FNAME) OR SALT29.WithinEditN(LEFT.FNAME,RIGHT.FNAME,1, 0)  OR RIGHT.FNAME_PreferredName = fn_PreferredName(LEFT.FNAME) ) AND (LEFT.MNAME[1..LENGTH(TRIM(RIGHT.MNAME))] = RIGHT.MNAME OR RIGHT.MNAME[1..LENGTH(TRIM(LEFT.MNAME))] = LEFT.MNAME OR SALT29.WithinEditN(LEFT.MNAME,RIGHT.MNAME,2, 0) ) AND (LEFT.LNAME = (TYPEOF(LEFT.LNAME))'' OR RIGHT.LNAME = (TYPEOF(RIGHT.LNAME))'' OR metaphonelib.DMetaPhone1(LEFT.LNAME)=metaphonelib.DMetaPhone1(RIGHT.LNAME) OR SALT29.WithinEditN(LEFT.LNAME,RIGHT.LNAME,1, 0) OR SALT29.HyphenMatch(LEFT.LNAME,RIGHT.LNAME,1)<=2 )
          OR SALT29.fn_concept_wordbag_EditN.Match3((SALT29.StrType)RIGHT.FNAME,RIGHT.FNAME_MAINNAME_weight100,true,0,true,1,(SALT29.StrType)RIGHT.MNAME,RIGHT.MNAME_MAINNAME_weight100,true,0,true,2,(SALT29.StrType)RIGHT.LNAME,RIGHT.LNAME_MAINNAME_weight100,true,0,false,1,(SALT29.StrType)LEFT.FNAME,0,(SALT29.StrType)LEFT.MNAME,0,(SALT29.StrType)LEFT.LNAME,0)>= 0) ),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.SSN = RIGHT.SSN,10000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
#IF(#TEXT(Input_SSN)<>'')
  #uniquename(trans)
  PRTE_Health_Provider_Services.Key_HealthProvider_SSN_LP.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Provider_Services.Key_HealthProvider_SSN_LP.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

EXPORT Key_HealthProvider_BILLING_NPI := MODULE
IMPORT SALT29,ut,std;
//BILLING_NPI_NUMBER:?:MAINNAME:+:SNAME:GENDER:DOB
EXPORT KeyName := PRTE_Health_Provider_Services.Files.FILE_BILLING_NPI; //'~'+'key::PRTE_Health_Provider_Services::LNPID::Refs::BILLING_NPI';
...
//Scores for various field components
  h.BILLING_NPI_NUMBER_weight100 ; // Contains 100x the specificity
...
DataForKey0 := DEDUP(SORT(TABLE(h(BILLING_NPI_NUMBER NOT IN SET(s.nulls_BILLING_NPI_NUMBER,BILLING_NPI_NUMBER)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
EXPORT Key := INDEX(DataForKey0,,KeyName);
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,LNPID,BILLING_NPI_NUMBER,FNAME,MNAME,LNAME,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'BILLING_NPI','SNAME',CntRed_SNAME*100,CntRed_SNAME*TSize},{'BILLING_NPI','GENDER',CntRed_GENDER*100,CntRed_GENDER*TSize},{'BILLING_NPI','DOB',CntRed_DOB*100,CntRed_DOB*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.BILLING_NPI_NUMBER <> (typeof(le.BILLING_NPI_NUMBER))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.BILLING_NPI_NUMBER) param_BILLING_NPI_NUMBER,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.LNAME) param_LNAME) := 
    STEPPED( LIMIT( Key(
          ( BILLING_NPI_NUMBER = param_BILLING_NPI_NUMBER AND param_BILLING_NPI_NUMBER <> (TYPEOF(BILLING_NPI_NUMBER))'' )
...
EXPORT ScoredLNPIDFetch(TYPEOF(h.BILLING_NPI_NUMBER) param_BILLING_NPI_NUMBER,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.LNAME) param_LNAME,TYPEOF(h.SNAME) param_SNAME,TYPEOF(h.GENDER) param_GENDER,UNSIGNED4 param_DOB) := FUNCTION
  RawData := RawFetch(param_BILLING_NPI_NUMBER,param_FNAME,param_MNAME,param_LNAME);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    SELF.keys_used := 1 << 19; // Set bitmap for key used
    SELF.keys_failed := IF(le.LNPID = 0, 1 << 19, 0); // Set bitmap for key failed
    SELF.BILLING_NPI_NUMBER_match_code := MAP(
		le.BILLING_NPI_NUMBER = (TYPEOF(le.BILLING_NPI_NUMBER))'' OR le.BILLING_NPI_NUMBER = (TYPEOF(le.BILLING_NPI_NUMBER))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthProvider).match_BILLING_NPI_NUMBER(le.BILLING_NPI_NUMBER,param_BILLING_NPI_NUMBER,TRUE));
    SELF.BILLING_NPI_NUMBERWeight := (50+MAP ( le.BILLING_NPI_NUMBER = param_BILLING_NPI_NUMBER  => le.BILLING_NPI_NUMBER_weight100,
          le.BILLING_NPI_NUMBER = (TYPEOF(le.BILLING_NPI_NUMBER))'' OR param_BILLING_NPI_NUMBER = (TYPEOF(le.BILLING_NPI_NUMBER))'' => 0,
           -0.332*le.BILLING_NPI_NUMBER_weight100))/100*0.50; 
    ...
    SELF.Weight :=MAX(0,SELF.BILLING_NPI_NUMBERWeight) + MAX(self.MAINNAMEWeight,MAX(0,SELF.FNAMEWeight) + MAX(0,SELF.MNAMEWeight) + MAX(0,SELF.LNAMEWeight)) + MAX(0,SELF.SNAMEWeight) + MAX(0,SELF.GENDERWeight) + MAX(0,SELF.DOBWeight);
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.BILLING_NPI_NUMBER_match_code := MAP(
		le.BILLING_NPI_NUMBER = (TYPEOF(le.BILLING_NPI_NUMBER))'' OR le.BILLING_NPI_NUMBER = (TYPEOF(le.BILLING_NPI_NUMBER))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthProvider).match_BILLING_NPI_NUMBER(le.BILLING_NPI_NUMBER,ri.BILLING_NPI_NUMBER,TRUE));
    SELF.BILLING_NPI_NUMBERWeight := (50+MAP ( le.BILLING_NPI_NUMBER = ri.BILLING_NPI_NUMBER  => le.BILLING_NPI_NUMBER_weight100,
          le.BILLING_NPI_NUMBER = (TYPEOF(le.BILLING_NPI_NUMBER))'' OR ri.BILLING_NPI_NUMBER = (TYPEOF(le.BILLING_NPI_NUMBER))'' => 0,
           -0.332*le.BILLING_NPI_NUMBER_weight100))/100*0.50; 
    SELF.FNAME_match_code := MAP(
    ...
    SELF := le;
  END;
  Recs0 := Recs(BILLING_NPI_NUMBER <> (typeof(BILLING_NPI_NUMBER))'');
  ...
  J0 := JOIN(Recs1,Key,LEFT.BILLING_NPI_NUMBER = RIGHT.BILLING_NPI_NUMBER
     AND ( ( 
    ...
    ATMOST(LEFT.BILLING_NPI_NUMBER = RIGHT.BILLING_NPI_NUMBER,10000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.BILLING_NPI_NUMBER = RIGHT.BILLING_NPI_NUMBER
     AND ( ( 
        (LEFT.FNAME[1..LENGTH(TRIM(RIGHT.FNAME))] = RIGHT.FNAME OR RIGHT.FNAME[1..LENGTH(TRIM(LEFT.FNAME))] = LEFT.FNAME OR metaphonelib.DMetaPhone1(LEFT.FNAME)=metaphonelib.DMetaPhone1(RIGHT.FNAME) OR SALT29.WithinEditN(LEFT.FNAME,RIGHT.FNAME,1, 0)  OR RIGHT.FNAME_PreferredName = fn_PreferredName(LEFT.FNAME) ) AND (LEFT.MNAME[1..LENGTH(TRIM(RIGHT.MNAME))] = RIGHT.MNAME OR RIGHT.MNAME[1..LENGTH(TRIM(LEFT.MNAME))] = LEFT.MNAME OR SALT29.WithinEditN(LEFT.MNAME,RIGHT.MNAME,2, 0) ) AND (LEFT.LNAME = (TYPEOF(LEFT.LNAME))'' OR RIGHT.LNAME = (TYPEOF(RIGHT.LNAME))'' OR metaphonelib.DMetaPhone1(LEFT.LNAME)=metaphonelib.DMetaPhone1(RIGHT.LNAME) OR SALT29.WithinEditN(LEFT.LNAME,RIGHT.LNAME,1, 0) OR SALT29.HyphenMatch(LEFT.LNAME,RIGHT.LNAME,1)<=2 )
          OR SALT29.fn_concept_wordbag_EditN.Match3((SALT29.StrType)RIGHT.FNAME,RIGHT.FNAME_MAINNAME_weight100,true,0,true,1,(SALT29.StrType)RIGHT.MNAME,RIGHT.MNAME_MAINNAME_weight100,true,0,true,2,(SALT29.StrType)RIGHT.LNAME,RIGHT.LNAME_MAINNAME_weight100,true,0,false,1,(SALT29.StrType)LEFT.FNAME,0,(SALT29.StrType)LEFT.MNAME,0,(SALT29.StrType)LEFT.LNAME,0)>= 0) ),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.BILLING_NPI_NUMBER = RIGHT.BILLING_NPI_NUMBER,10000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_BILLING_NPI_NUMBER='',Input_FNAME='',Input_MNAME='',Input_LNAME='',Input_SNAME='',Input_GENDER='',Input_DOB='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,PRTE_Health_Provider_Services;
#IF(#TEXT(Input_BILLING_NPI_NUMBER)<>'')
  #uniquename(trans)
  PRTE_Health_Provider_Services.Key_HealthProvider_BILLING_NPI.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Provider_Services.Key_HealthProvider_BILLING_NPI.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

...
pj27 do_computes(pj27 le) := TRANSFORM
  ...
  SELF := le;
END;
SHARED propogated := PROJECT(pj27,do_computes(left)) : PERSIST('~temp::LNPID::PRTE_Health_Provider_Services::mc_props::HealthProvider',EXPIRE(30)); // to allow to 'jump' over an exported value
...
SHARED Annotated := DISTRIBUTE(j0,hash(LNPID)) : PERSIST('~temp::LNPID::PRTE_Health_Provider_Services::mc',EXPIRE(30)); // Distributed for keybuild case

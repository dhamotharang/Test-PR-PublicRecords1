...
EXPORT KeyName := PRTE_Health_Provider_Services.Files.FILE_ZIP; //'~'+'key::PRTE_Health_Provider_Services::LNPID::Refs::ZIP_PR';
...
EXPORT ScoredLNPIDFetch(TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.GENDER) param_GENDER,TYPEOF(h.LNAME) param_LNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.SNAME) param_SNAME,UNSIGNED4 param_DOB,TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR,TYPEOF(h.LIC_STATE) param_LIC_STATE,TYPEOF(h.BILLING_TAX_ID) param_BILLING_TAX_ID) := FUNCTION
  RawData := RawFetch(param_PRIM_NAME,param_ZIP,param_FNAME);
...
#IF(#TEXT(Input_PRIM_NAME)<>'' AND #TEXT(Input_ZIP)<>'')
  #uniquename(trans)
  PRTE_Health_Provider_Services.Key_HealthProvider_ZIP_PR.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Provider_Services.Key_HealthProvider_ZIP_PR.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

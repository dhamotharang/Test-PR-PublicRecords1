...
EXPORT KeyName := PRTE_Health_Provider_Services.Files.FILE_LName; //'~'+'key::PRTE_Health_Provider_Services::LNPID::Refs::LNAME';
...
DataForKey0 := DEDUP(SORT(TABLE(h(LNAME NOT IN SET(s.nulls_LNAME,LNAME),ZIP NOT IN SET(s.nulls_ZIP,ZIP)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
EXPORT ScoredLNPIDFetch(TYPEOF(h.LNAME) param_LNAME,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.GENDER) param_GENDER,TYPEOF(h.SNAME) param_SNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST,UNSIGNED4 param_DOB,TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR,TYPEOF(h.LIC_STATE) param_LIC_STATE,TYPEOF(h.NPI_NUMBER) param_NPI_NUMBER,TYPEOF(h.BILLING_TAX_ID) param_BILLING_TAX_ID) := FUNCTION
  RawData := RawFetch(param_LNAME,param_ZIP,param_FNAME,param_GENDER,param_SNAME,param_MNAME);
...
#IF(#TEXT(Input_LNAME)<>'' AND #TEXT(Input_ZIP)<>'')
  #uniquename(trans)
  PRTE_Health_Provider_Services.Key_HealthProvider_LNAME.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Provider_Services.Key_HealthProvider_LNAME.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

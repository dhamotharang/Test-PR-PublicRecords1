...
EXPORT KeyName := PRTE_Health_Provider_Services.Files.FILE_BDID; //'~'+'key::PRTE_Health_Provider_Services::LNPID::Refs::BID';
...
EXPORT Shrinkage := DATASET([{'BID','SNAME',CntRed_SNAME*100,CntRed_SNAME*TSize},{'BID','GENDER',CntRed_GENDER*100,CntRed_GENDER*TSize},{'BID','DOB',CntRed_DOB*100,CntRed_DOB*TSize},{'BID','C_LIC_NBR',CntRed_C_LIC_NBR*100,CntRed_C_LIC_NBR*TSize},{'BID','LIC_STATE',CntRed_LIC_STATE*100,CntRed_LIC_STATE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.BDID <> (typeof(le.BDID))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.BDID) param_BDID,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.LNAME) param_LNAME) := 
    STEPPED( LIMIT( Key(
          ( BDID = param_BDID AND param_BDID <> (TYPEOF(BDID))'' )
...
EXPORT ScoredLNPIDFetch(TYPEOF(h.BDID) param_BDID,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.LNAME) param_LNAME,TYPEOF(h.SNAME) param_SNAME,TYPEOF(h.GENDER) param_GENDER,UNSIGNED4 param_DOB,TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR,TYPEOF(h.LIC_STATE) param_LIC_STATE) := FUNCTION
  RawData := RawFetch(param_BDID,param_FNAME,param_MNAME,param_LNAME);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    SELF.keys_used := 1 << 22; // Set bitmap for key used
    SELF.keys_failed := IF(le.LNPID = 0, 1 << 22, 0); // Set bitmap for key failed
    SELF.BDID_match_code := MAP(
		le.BDID = (TYPEOF(le.BDID))'' OR le.BDID = (TYPEOF(le.BDID))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthProvider).match_BDID(le.BDID,param_BDID,TRUE));
    ...
    SELF.Weight :=MAX(0,SELF.BDIDWeight) + MAX(self.MAINNAMEWeight,MAX(0,SELF.FNAMEWeight) + MAX(0,SELF.MNAMEWeight) + MAX(0,SELF.LNAMEWeight)) + MAX(0,SELF.SNAMEWeight) + MAX(0,SELF.GENDERWeight) + MAX(0,SELF.DOBWeight) + MAX(0,SELF.C_LIC_NBRWeight) + MAX(0,SELF.LIC_STATEWeight);
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.BDID_match_code := MAP(
		le.BDID = (TYPEOF(le.BDID))'' OR le.BDID = (TYPEOF(le.BDID))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthProvider).match_BDID(le.BDID,ri.BDID,TRUE));
  ...
  J0 := JOIN(Recs1,Key,LEFT.BDID = RIGHT.BDID
     AND ( ( 
  ...
  J1 := JOIN(Recs1,PULL(Key),LEFT.BDID = RIGHT.BDID
     AND ( ( 
        (LEFT.FNAME[1..LENGTH(TRIM(RIGHT.FNAME))] = RIGHT.FNAME OR RIGHT.FNAME[1..LENGTH(TRIM(LEFT.FNAME))] = LEFT.FNAME OR metaphonelib.DMetaPhone1(LEFT.FNAME)=metaphonelib.DMetaPhone1(RIGHT.FNAME) OR SALT29.WithinEditN(LEFT.FNAME,RIGHT.FNAME,1, 0)  OR RIGHT.FNAME_PreferredName = fn_PreferredName(LEFT.FNAME) ) AND (LEFT.MNAME[1..LENGTH(TRIM(RIGHT.MNAME))] = RIGHT.MNAME OR RIGHT.MNAME[1..LENGTH(TRIM(LEFT.MNAME))] = LEFT.MNAME OR SALT29.WithinEditN(LEFT.MNAME,RIGHT.MNAME,2, 0) ) AND (LEFT.LNAME = (TYPEOF(LEFT.LNAME))'' OR RIGHT.LNAME = (TYPEOF(RIGHT.LNAME))'' OR metaphonelib.DMetaPhone1(LEFT.LNAME)=metaphonelib.DMetaPhone1(RIGHT.LNAME) OR SALT29.WithinEditN(LEFT.LNAME,RIGHT.LNAME,1, 0) OR SALT29.HyphenMatch(LEFT.LNAME,RIGHT.LNAME,1)<=2 )
          OR SALT29.fn_concept_wordbag_EditN.Match3((SALT29.StrType)RIGHT.FNAME,RIGHT.FNAME_MAINNAME_weight100,true,0,true,1,(SALT29.StrType)RIGHT.MNAME,RIGHT.MNAME_MAINNAME_weight100,true,0,true,2,(SALT29.StrType)RIGHT.LNAME,RIGHT.LNAME_MAINNAME_weight100,true,0,false,1,(SALT29.StrType)LEFT.FNAME,0,(SALT29.StrType)LEFT.MNAME,0,(SALT29.StrType)LEFT.LNAME,0)>= 0) ),Score_Batch(RIGHT,LEFT),
...
#IF(#TEXT(Input_BDID)<>'')
  #uniquename(trans)
  PRTE_Health_Provider_Services.Key_HealthProvider_BID.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Provider_Services.Key_HealthProvider_BID.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

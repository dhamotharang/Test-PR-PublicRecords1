...
EXPORT KeyName := PRTE_Health_Provider_Services.Files.FILE_MName; //'~'+'key::PRTE_Health_Provider_Services::LNPID::Refs::MNAME';
...
DataForKey0 := DEDUP(SORT(TABLE(h(LNAME NOT IN SET(s.nulls_LNAME,LNAME),MNAME NOT IN SET(s.nulls_MNAME,MNAME),ST NOT IN SET(s.nulls_ST,ST)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
layout note_init5(layout le,Specificities(File_HealthProvider).FNAME_values_persisted ri) := TRANSFORM
  SELF.FNAME_initial_weight100 := ri.field_specificity * 100;
  SELF := le;
END;
DataForKey1 := JOIN(DataForKey0,Specificities(File_HealthProvider).FNAME_values_persisted(LENGTH(TRIM(FNAME))=1),LEFT.FNAME[1]=RIGHT.FNAME[1],note_init5(LEFT,RIGHT),LOOKUP,LEFT OUTER); // Append specificities for initials of FNAME
...
EXPORT ScoredLNPIDFetch(TYPEOF(h.LNAME) param_LNAME,TYPEOF(h.MNAME) param_MNAME,TYPEOF(h.ST) param_ST,TYPEOF(h.GENDER) param_GENDER,TYPEOF(h.SNAME) param_SNAME,TYPEOF(h.FNAME) param_FNAME,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ZIP) param_ZIP,UNSIGNED4 param_DOB,TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR,TYPEOF(h.LIC_STATE) param_LIC_STATE,TYPEOF(h.NPI_NUMBER) param_NPI_NUMBER,TYPEOF(h.BILLING_TAX_ID) param_BILLING_TAX_ID) := FUNCTION
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.MNAME_match_code := MAP(
		le.MNAME = (TYPEOF(le.MNAME))'' OR le.MNAME = (TYPEOF(le.MNAME))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthProvider).match_MNAME(le.MNAME,param_MNAME,TRUE));
    SELF.MNAMEWeight := (50+MAP ( le.MNAME = param_MNAME  => le.MNAME_weight100,
          le.MNAME = (TYPEOF(le.MNAME))'' OR param_MNAME = (TYPEOF(le.MNAME))'' => 0,
  ...
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.LNAMEWeight := (50+MAP ( le.LNAME = ri.LNAME  => le.LNAME_weight100,
          le.LNAME = (TYPEOF(le.LNAME))'' OR ri.LNAME = (TYPEOF(le.LNAME))'' => 0,
           -0.874*le.LNAME_weight100))/100; 
    SELF.MNAME_match_code := MAP(
		le.MNAME = (TYPEOF(le.MNAME))'' OR le.MNAME = (TYPEOF(le.MNAME))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthProvider).match_MNAME(le.MNAME,ri.MNAME,TRUE));
    SELF.MNAMEWeight := (50+MAP ( le.MNAME = ri.MNAME  => le.MNAME_weight100,
          le.MNAME = (TYPEOF(le.MNAME))'' OR ri.MNAME = (TYPEOF(le.MNAME))'' => 0,
           -0.932*le.MNAME_weight100))/100; 
    SELF.ST_match_code := MAP(
...
#IF(#TEXT(Input_LNAME)<>'' AND #TEXT(Input_MNAME)<>'' AND #TEXT(Input_ST)<>'')
  #uniquename(trans)
  PRTE_Health_Provider_Services.Key_HealthProvider_MNAME.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Provider_Services.Key_HealthProvider_MNAME.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

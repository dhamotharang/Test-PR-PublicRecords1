...
patchspc	:= SALTTOOLS22.mac_Patch_SPC(mspecs.Specificities,'test._SPC',test.In_Base,_control.MyInfo.UserID,_control.MyInfo.Password,'10.241.3.242:8145');
...
functionmacro
  ...
  lesp        := pESP + ':8145';
	...
	datts1 := SALTTOOLS22.mod_Soapcalls.fFindAttributes(lmodule,lattribute,lesp,true,pIsSandbox := pSandbox);
	datts2 := SALTTOOLS22.mod_Soapcalls.fFindAttributes(lmodule,lattribute,lesp,true,pIsSandbox := not pSandbox);
	datts := if(count(datts1(trim(text,left,right) != '')) > 0 ,datts1  ,datts2);
	...
	))(not regexfind('^(import|salt|output).*$',line,nocase));
	...
	SALTTOOLS22.mac_LayoutTools(recordof(pInSpecificities),layouttools,false,true,'specificity|switch');
	...
	updateSPC := SALTTOOLS22.mod_Soapcalls.fSaveAttribute(loutmodule,loutattribute,drolluptext[1].line,lesp);

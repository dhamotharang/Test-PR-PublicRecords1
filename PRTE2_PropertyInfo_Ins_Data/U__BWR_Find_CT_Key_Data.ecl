...
************************************************************************
====>>> NOTE The PropertyInfo Build doesn't create SF - need TODO!
...
keyn1 := foreignProd+'prte::key::propertyinfo::'+fileVersion+'::address';
keyn2 := foreignProd+'prte::key::propertyinfo::'+fileVersion+'::rid';
...
PI_Rid_IndexFields := RECORD
  unsigned6 property_rid;
END;

Layout_PI_Addr := RECORD
  unsigned6 property_rid;
END;
PI_Addr_IndexFields := RECORD
  string28 prim_name;
  string10 prim_range;
  string5 zip;
  string2 predir;
...
key1 := INDEX({PI_Addr_IndexFields}, Layout_PI_Addr, keyn1);
key2 := INDEX({PI_Rid_IndexFields}, Layout_PI_Rid, keyn2);

// ----------------------------------------------------------------------------------------------------------
rid_recs := key1(prim_range=q_prange AND prim_name=q_pname AND zip=q_zip);
rid_list := SET(rid_recs,property_rid);
OUTPUT(rid_recs);
// OUTPUT(rid_list);
recs := key2(property_rid IN rid_list);
OUTPUT(recs,all,NAMED('Payload_Data'));

EXPORT bwr_mac_get_score_details_test( product_type,lay, field_name, date_in, retvalfinal ) := macro
...
%table_in2%   := table ( %file_table% , { acctno, lay, field_name,  string80 score:= scoring_project_ks.bwr_get_score( lay,(string) #expand(field_name))  } ) :independent ;
...
#UNIQUENAME(table_in)
%table_in%   := table ( %file_table% , { scoring_project_ks.bwr_get_score_bin( lay,(string) #expand(field_name)) } );


#UNIQUENAME(table_count)
%table_count% := count(%table_in%(bwr_get_score_bin<>'NULL'));
...
%table_grp%   := table ( %table_in% , %rec_group%, bwr_get_score_bin );

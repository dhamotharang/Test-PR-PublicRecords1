IMPORT ut,SALT34;
EXPORT hygiene(dataset(layout_In_CA) h) := MODULE
...
SALT34.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT34.StrType)le.dt_vendor_first_reported),TRIM((SALT34.StrType)le.dt_vendor_last_reported),TRIM((SALT34.StrType)le.dt_first_seen),TRIM((SALT34.StrType)le.dt_last_seen),TRIM((SALT34.StrType)le.corp_ra_dt_first_seen),TRIM((SALT34.StrType)le.corp_ra_dt_last_seen),TRIM((SALT34.StrType)le.corp_key),TRIM((SALT34.StrType)le.corp_vendor),TRIM((SALT34.StrType)le.corp_state_origin),TRIM((SALT34.StrType)le.corp_process_date),TRIM((SALT34.StrType)le.corp_orig_sos_charter_nbr),TRIM((SALT34.StrType)le.corp_legal_name),TRIM((SALT34.StrType)le.corp_status_cd),TRIM((SALT34.StrType)le.corp_status_date),TRIM((SALT34.StrType)le.corp_inc_state),TRIM((SALT34.StrType)le.corp_inc_date),TRIM((SALT34.StrType)le.corp_foreign_domestic_ind),TRIM((SALT34.StrType)le.corp_forgn_date),TRIM((SALT34.StrType)le.corp_orig_org_structure_cd),TRIM((SALT34.StrType)le.corp_for_profit_ind),TRIM((SALT34.StrType)le.corp_ln_name_type_cd),TRIM((SALT34.StrType)le.corp_ln_name_type_desc),TRIM((SALT34.StrType)le.corp_ra_resign_date),TRIM((SALT34.StrType)le.corp_name_status_date),TRIM((SALT34.StrType)le.corp_orig_bus_type_cd),TRIM((SALT34.StrType)le.corp_term_exist_cd),TRIM((SALT34.StrType)le.corp_forgn_state_desc),TRIM((SALT34.StrType)le.corp_tax_program_cd)));
  SELF.FldNo := C;
END;
...
SHARED FldIds := DATASET([{1,'dt_vendor_first_reported'}
      ...
      ,{18,'corp_forgn_date'}
      ,{19,'corp_orig_org_structure_cd'}
      ,{20,'corp_for_profit_ind'}
      ,{21,'corp_ln_name_type_cd'}
      ,{22,'corp_ln_name_type_desc'}
      ,{23,'corp_ra_resign_date'}
      ,{24,'corp_name_status_date'}
      ,{25,'corp_orig_bus_type_cd'}
      ,{26,'corp_term_exist_cd'}
      ,{27,'corp_forgn_state_desc'}
      ,{28,'corp_tax_program_cd'}],SALT34.MAC_Character_Counts.Field_Identification);

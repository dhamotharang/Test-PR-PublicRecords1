...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Corp2_Mapping.LayoutsCommon.Main)
    ...
    UNSIGNED1 corp_tax_program_cd_Invalid;
  END;
...
EXPORT FromNone(DATASET(Corp2_Mapping.LayoutsCommon.Main) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_tax_program_cd_Invalid := Fields.InValid_corp_tax_program_cd((SALT34.StrType)le.corp_tax_program_cd);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Corp2_Mapping.LayoutsCommon.Main);
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_status_cd_Invalid << 21 ) + ( le.corp_status_date_Invalid << 22 ) + ( le.corp_inc_state_Invalid << 24 ) + ( le.corp_inc_date_Invalid << 25 ) + ( le.corp_foreign_domestic_ind_Invalid << 27 ) + ( le.corp_forgn_date_Invalid << 28 ) + ( le.corp_orig_org_structure_cd_Invalid << 30 ) + ( le.corp_for_profit_ind_Invalid << 31 ) + ( le.corp_ln_name_type_cd_Invalid << 32 ) + ( le.corp_ln_name_type_desc_Invalid << 33 ) + ( le.corp_ra_resign_date_Invalid << 34 ) + ( le.corp_name_status_date_Invalid << 36 ) + ( le.corp_orig_bus_type_cd_Invalid << 38 ) + ( le.corp_term_exist_cd_Invalid << 39 ) + ( le.corp_forgn_state_desc_Invalid << 40 ) + ( le.corp_tax_program_cd_Invalid << 41 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_status_cd_Invalid := (le.ScrubsBits1 >> 21) & 1;
    SELF.corp_status_date_Invalid := (le.ScrubsBits1 >> 22) & 3;
    ...
    SELF.corp_orig_org_structure_cd_Invalid := (le.ScrubsBits1 >> 30) & 1;
    SELF.corp_for_profit_ind_Invalid := (le.ScrubsBits1 >> 31) & 1;
    SELF.corp_ln_name_type_cd_Invalid := (le.ScrubsBits1 >> 32) & 1;
    SELF.corp_ln_name_type_desc_Invalid := (le.ScrubsBits1 >> 33) & 1;
    SELF.corp_ra_resign_date_Invalid := (le.ScrubsBits1 >> 34) & 3;
    SELF.corp_name_status_date_Invalid := (le.ScrubsBits1 >> 36) & 3;
    SELF.corp_orig_bus_type_cd_Invalid := (le.ScrubsBits1 >> 38) & 1;
    SELF.corp_term_exist_cd_Invalid := (le.ScrubsBits1 >> 39) & 1;
    SELF.corp_forgn_state_desc_Invalid := (le.ScrubsBits1 >> 40) & 1;
    SELF.corp_tax_program_cd_Invalid := (le.ScrubsBits1 >> 41) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_tax_program_cd_ENUM_ErrorCount := COUNT(GROUP,h.corp_tax_program_cd_Invalid=1);
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_orig_bus_type_cd:invalid_corp_orig_bus_type_cd:ENUM'
          ,'corp_term_exist_cd:invalid_corp_term_exist_cd:ENUM'
          ,'corp_forgn_state_desc:invalid_alphablank:ALLOW'
          ,'corp_tax_program_cd:invalid_tax_program_cd:ENUM','UNKNOWN');
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_corp_tax_program_cd(1),'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_tax_program_cd_ENUM_ErrorCount,0);
      SELF.rulepcnt := 100 * CHOOSE(c
          ...
          ,le.corp_term_exist_cd_ENUM_ErrorCount
          ,le.corp_forgn_state_desc_ALLOW_ErrorCount
          ,le.corp_tax_program_cd_ENUM_ErrorCount,0) / le.TotalCnt + 0.5;
    END;

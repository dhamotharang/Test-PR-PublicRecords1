...
export build_base(string filedate, boolean pUseProd = false)  := function
        base_rec := if(nothor(global(STD.File.FileExists(crim_offense_cat.Filenames(pUseProd).base + filedate))),
        ...
        all_seq := if(nothor(global(STD.File.FileExists(crim_offense_cat.Filenames(pUseProd).base + filedate))),

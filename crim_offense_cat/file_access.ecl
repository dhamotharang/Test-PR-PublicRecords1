...
export file_access(pUseProd = false) := module
    ...
    export Base := dataset(crim_offense_cat.Filenames(pUseProd).processedbase, crim_offense_cat.layouts.base_layout, thor, __compressed__):persist(crim_offense_cat.filenames(pUseProd).processedbase + 'persist');
end;
...
// Output only a subset based on zipcode prefix
set_zipcode_prefix := ['480', '802', '332', '750', '751', '752', '980'];

// Project to output layout
export dm2_out_subset := project(dm2_siccodes(zip[1..3] in set_zipcode_prefix),
                          transform(Layout_DM2_Out, self.Location_Phone := left.bus_best_phone,
					                           self.SIC := left.siccode;
										  self := left)) : persist('TMTEST::dm2_out_subset');

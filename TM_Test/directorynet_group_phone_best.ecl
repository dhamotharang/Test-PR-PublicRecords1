...
dn_group_init := directnet_did_to_business(bdid <> 0 and bus_best_phone = '');
dn_other := directnet_did_to_business(not(bdid <> 0 and bus_best_phone = ''));
...
dn_group_list := project(dn_group_init,
                         transform(layout_group_id, self := left));
					
dn_group_list_dedup := dedup(dn_group_list, all);

dn_group_gid_list := join(bhsg,
                           dn_group_list_dedup,
					  left.group_id = right.group_id,
					  transform(Business_Header.Layout_BH_Super_Group, self := left),
					  hash);
					  
bh_mktapp := Business_Header.File_Business_Header_MktApp;
//bh_mktapp := Business_Header.File_Business_Header;
...
Layout_DirectoryNet_Base AppendPhones(Layout_DirectoryNet_Base l, layout_bh_bestphone r) := transform
self.bus_best_phone := if(r.phone <> 0, intformat(r.phone, 10, 1), '');
self := l;
end;

dn_phone_append := join(dn_group_init,
                        bh_bp,
				    left.group_id = right.group_id,
...
export directorynet_group_phone_best := dn_phone_append + dn_other : persist('TMTEST::directorynet_group_phone_best');
...
// Determine Gong records without a current listing
gbc := Gong.File_History(bdid <> 0, publish_code IN ['P','U'], current_record_flag = 'Y');
gbnc := Gong.File_History(bdid <> 0, publish_code IN ['P','U'], current_record_flag <> 'Y');

gb_not_current := join(gbnc,
                      gbc,
				  left.bdid = right.bdid,
...
// Output sample from Gong not current
bdid_gong_sample := join(gb_not_current,
                         bdid_people_sample,
					left.bdid = right.bdid,
					transform(Gong.Layout_history, self := left),
					hash);

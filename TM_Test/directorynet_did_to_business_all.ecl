...
dn_group_init := directorynet_prep_all(did <> 0 and bdid = 0);
dn_other := directorynet_prep_all(not(did <> 0 and bdid = 0));
...
dn_append_gid := join(dn_best_append,
					    ...
					    left outer,
					    hash) : persist('TMTEST::directorynet_did_bdid_append_all');


export directorynet_did_to_business_all := dn_append_gid + dn_other : persist('TMTEST::directorynet_did_to_business_all');
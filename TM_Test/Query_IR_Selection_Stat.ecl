...
ss_base := dataset('TMTEMP::IR_Sample_Counts', layout_sample_stats, flat);
...
ss_sample_base := join(bhb,
                      ss_base_limited,
				  left.bdid = right.bdid,
				  AppendBest(left, right),
				  hash)  : persist('TMTEMP::IR_Sample_Base');
				  
ir_sample := ss_sample_base;
...
gh := join(Gong.File_Gong_History_Full(bdid <> 0),
           ir_bdids,
		 left.bdid = right.bdid,
...
ir_out_bbb := join(ir_out_bh,
			    ...
			    left outer,
			    hash) : persist('TMTEMP::IR_Sample_Base_Out');

output(count(ir_out_bbb), named('IR_Sample_Base_Count'));
output(ir_out_bbb);

layout_sample_base_selection := record
ir_out_bbb.selection_flag;
end;

ir_sample_base_selection := table(ir_out_bbb, layout_sample_base_selection);

layout_sample_base_selection_stat := record
ir_sample_base_selection.selection_flag;
cnt := count(group);
end;

ir_sample_base_selection_stat := table(ir_sample_base_selection, layout_sample_base_selection_stat, selection_flag, few);

output(ir_sample_base_selection_stat, named('IR_Sample_Base_Selection_Stat'));

ir_sample_out := dataset('TMTEMP::IR_Sample_Out', layout_sample_out, flat);

layout_sample_selection := record
ir_sample_out.selection_flag;
end;

ir_sample_selection := table(ir_sample_out, layout_sample_selection);

layout_sample_selection_stat := record
ir_sample_selection.selection_flag;
cnt := count(group);
end;

ir_sample_selection_stat := table(ir_sample_selection, layout_sample_selection_stat, selection_flag, few);

output(ir_sample_selection_stat, named('IR_Sample_Selection_Stat'));

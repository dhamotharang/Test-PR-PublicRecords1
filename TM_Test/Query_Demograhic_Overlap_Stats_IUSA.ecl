...
bh_list_noniusa := dataset('~thor_data400::TMTEST::Demographic_BDID_List_NonIUSA', layout_bdid_list, flat);				   
...
bh_revenue_table_sort := sort(revenue_table_dist(source <> 'IA'), bdid, -revenues, local);
bh_revenue_table_dedup := dedup(bh_revenue_table_sort, bdid, local);

iusa_revenue_table_sort := sort(revenue_table_dist(source = 'IA'), bdid, -revenues, local);
iusa_revenue_table_dedup := dedup(iusa_revenue_table_sort, bdid, local);
...
bh_number_employees_table_sort := sort(number_employees_table_dist(source <> 'IA'), bdid, -number_employees, local);
bh_number_employees_table_dedup := dedup(bh_number_employees_table_sort, bdid, local);

iusa_number_employees_table_sort := sort(number_employees_table_dist(source = 'IA'), bdid, -number_employees, local);
iusa_number_employees_table_dedup := dedup(iusa_number_employees_table_sort, bdid, local);
...
dmg_iusa_bdid_only_init := project(iusa_bdid_only,
                                 transform(layout_demographic_bdid,
					        self.overlap := 'IO',
					        self := left));

dmg_iusa_bdid_only_dist := distribute(dmg_iusa_bdid_only_init, hash(bdid));

dmg_bh_iusa_bdid_overlap_init := project(bh_iusa_bdid_overlap,
                                       transform(layout_demographic_bdid,
					              self.overlap := 'OV',
					              self := left));

dmg_bh_iusa_bdid_overlap_dist := distribute(dmg_bh_iusa_bdid_overlap_init, hash(bdid));
...
dmg_iusa_bdid_only_revenue_append := join(dmg_iusa_bdid_only_dist,
                                        iusa_revenue_table_dedup,
					               left.bdid = right.bdid,
					               AppendRevenues(left, right),
					               left outer,
					               local);

dmg_bh_iusa_bdid_overlap_revenue_append := join(dmg_bh_iusa_bdid_overlap_dist,
...
dmg_iusa_bdid_only_number_employees_append := join(dmg_iusa_bdid_only_revenue_append,
                                                 iusa_number_employees_table_dedup,
					                        left.bdid = right.bdid,
					                        AppendNumberEmployees(left, right),
					                        left outer,
					                        local);

dmg_bh_iusa_bdid_overlap_number_employees_append := join(dmg_bh_iusa_bdid_overlap_revenue_append,
...
dmg_all := dmg_bh_bdid_only_number_employees_append +
           dmg_iusa_bdid_only_number_employees_append +
		 dmg_bh_iusa_bdid_overlap_number_employees_append : persist('TMTEST::Demographics_Append_All');

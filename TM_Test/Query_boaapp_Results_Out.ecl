//f := TM_Test.boaapp_prep;
f := dataset('TMTEST::boaapp_prep', TM_Test.layout_boaapp_base, flat);
...
boaapp_stats := join(Business_Risk.BDID_Table,
                     bdid_list,
				 left.bdid = right.bdid,
				 transform(layout_boaapp_combined, self.ACCOUNT := right.ACCOUNT, self := left),
				 hash);

brt := Business_Risk.BDID_Risk_Table;
			
layout_boaapp_combined AppendRiskData(layout_boaapp_combined l, brt r) := transform
self := r;
self := l;
end;
...
// Append scores
boaapp_score := join(boaapp_stats,
                     brt,
				 left.bdid = right.bdid,
...
layout_boaapp_out FormatOutput(layout_boaapp_combined l) := transform
...
boaapp_out := project(boaapp_score, FormatOutput(left));

output(sort(boaapp_out, ACCOUNT),,'OUT::boaapp_results',overwrite);

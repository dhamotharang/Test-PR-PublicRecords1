...
layout_gecapital_seq := record
unsigned4 seq;
layout_gecapital_perf_in;
end;

// Add unique record id to input
layout_gecapital_seq AssignSeq(layout_gecapital_perf_in l, unsigned4 cnt) := transform
self.seq := cnt;
self := l;
end;

gecapital_seq := project(file_gecapital_perf_in, AssignSeq(left, counter));

// clean the names and addresses
layout_gecapital_clean := record
layout_gecapital_seq;
string182 clean_bus_address;
string10  phone10;
end;

layout_gecapital_clean CleanInput(layout_gecapital_seq l) := transform
self.clean_bus_address := AddrCleanLib.CleanAddress182(l.ADDRESS, trim(l.CITY) + ' ' + trim(l.STATE) + ' ' + trim(l.ZIP));
self.phone10 := Address.CleanPhone(l.WORKPHONE);
self := l;
end;

gecapital_clean_init := project(gecapital_seq, CleanInput(left));

// Project clean fields to base format
layout_gecapital_base InitBase(layout_gecapital_clean l) := transform
...
gecapital_clean := project(gecapital_clean_init, InitBase(left)) : persist('TMTEST::gecapital_clean');

// Project into BDID in batch format
Business_Header_SS.Layout_BDID_OutBatch InitBatch(layout_gecapital_base l) := transform
...
gecapital_bdid_batch_in := project(gecapital_clean, InitBatch(left));

matchset := ['A','P','N'];

Business_Header_SS.MAC_Match_Flex
(
	gecapital_bdid_batch_in, matchset,
	company_name, 
...
Business_Header_SS.MAC_BestAppend(
	...
	'BEST_ALL',
	'BEST_ALL',
	gecapital_bdid_batch_best,
	false
)

gecapital_bdid_batch := gecapital_bdid_batch_best;

// Join to base file to add bdid information
layout_gecapital_base CombineBest(layout_gecapital_base l, Business_Header_SS.Layout_BDID_OutBatch r) := transform
self.bdid := r.bdid;
...
gecapital_bdid_best := join(gecapital_clean,
                                gecapital_bdid_batch,
						  left.seq = right.seq,
						  CombineBest(left, right),
						  left outer,
						  hash)   : persist('TMTEST::gecapital_bdid_best');

					 						  
// Append the group id
bhg := Business_Header.File_Super_Group;

gecapital_group_id := join(bhg,
                      gecapital_bdid_best(bdid <> 0),
				  left.bdid = right.bdid,
				  transform(layout_gecapital_base, self.group_id := left.group_id, self := right),
				  right outer,
				  hash);
						 
gecapital_group_id_all := gecapital_group_id + gecapital_bdid_best(bdid = 0);
					 
gecapital_out := sort(gecapital_group_id_all, seq);

export gecapital_perf_prep := gecapital_out : persist('TMTEST::gecapital_prep');

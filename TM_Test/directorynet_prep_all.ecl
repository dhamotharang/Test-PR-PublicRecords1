...
directorynet_group_id_all := directorynet_group_id + directorynet_bdid_best(bdid = 0) : persist('TMTEST::directorynet_bdid_best_all');
...
directorynet_exec_did_best_dist := distribute(directorynet_exec_did_best, hash(seq)) : persist('TMTEST::directorynet_did_all');
...
directorynet_exec_did_append := join(directorynet_gid_all_dist,
					            ...
					            left outer,
					            local)    : persist('TMTEST::directorynet_did_best_all');

directorynet_prep_out := dedup(sort(directorynet_exec_did_append, seq, -score), seq);

export directorynet_prep_all := directorynet_prep_out  : persist('TMTEST::directorynet_bdid_did_prep_all');

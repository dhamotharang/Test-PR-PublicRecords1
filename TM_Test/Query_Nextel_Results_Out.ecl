//f := TM_Test.nextel_perf_prep;
f := dataset('TMTEST::nextel_prep', TM_Test.layout_nextel_perf_base, flat);
...
nextel_stats := join(Business_Risk.BDID_Table,
                     bdid_list,
				 left.bdid = right.bdid,
				 transform(layout_nextel_combined, self.ACCOUNT := right.ACCOUNT, self := left),
				 hash);

brt := Business_Risk.BDID_Risk_Table;
			
layout_nextel_combined AppendRiskData(layout_nextel_combined l, brt r) := transform
self := r;
self := l;
end;
...
// Append scores
nextel_score := join(nextel_stats,
                     brt,
				 left.bdid = right.bdid,
...
layout_nextel_out FormatOutput(layout_nextel_combined l) := transform
...
nextel_out := project(nextel_score, FormatOutput(left));

output(sort(nextel_out, ACCOUNT),,'OUT::nextel_results',overwrite);

...
layout_amex_iclic_test_seq := record
unsigned4 seq;
layout_amex_iclic_test_in;
end;

// Add unique sequence number to input
layout_amex_iclic_test_seq SequenceInput(layout_amex_iclic_test_in l, unsigned4 cnt) := transform
self.seq := cnt;
self := l;
end;

amex_iclic_test_seq := project(file_amex_iclic_test_in, SequenceInput(left, counter));

// clean the names and addresses
layout_amex_iclic_test_clean := record
layout_amex_iclic_test_seq;
...
layout_amex_iclic_test_clean CleanInput(layout_amex_iclic_test_seq l) := transform
self.clean_bus_address := addrcleanlib.cleanAddress182(trim(l.Address_1) + ' ' + trim(l.Address_2), trim(l.City) + ', ' + trim(l.State) + ' ' + trim(l.Zip[1..5]));
self.clean_name := addrcleanlib.cleanPerson73(l.CEO_Name);
self.phone10 := Address.CleanPhone(l.Phone);
self.fein := '';
self := l;
end;

amex_iclic_test_clean_init := project(amex_iclic_test_seq, CleanInput(left));

// Project clean fields to base format
layout_amex_iclic_test_base InitBase(layout_amex_iclic_test_clean l) := transform
...
amex_iclic_test_clean := project(amex_iclic_test_clean_init, InitBase(left)) : persist('TMTEST::amex_iclic_test_clean');

// Project into BDID in batch format
Business_Header_SS.Layout_BDID_OutBatch InitBatch(layout_amex_iclic_test_base l) := transform
...
amex_iclic_bdid_batch_in := project(amex_iclic_test_clean, InitBatch(left));

/*
// Append BDID and Best/Verify Information
Business_Header_SS.MAC_BDID_Append(
	amex_iclic_bdid_batch_in,
	amex_iclic_bdid_batch_match,
	1
)
*/
...
(
	amex_iclic_bdid_batch_in, matchset,
	company_name, 
...
Business_Header_SS.MAC_BestAppend(
	...
	'BEST_ALL',
	'BEST_ALL',
	amex_iclic_bdid_batch_best,
	false
)

// Join to base file to add bdid information
layout_amex_iclic_test_base CombineBest(layout_amex_iclic_test_base l, Business_Header_SS.Layout_BDID_OutBatch r) := transform
self.bdid := r.bdid;
...
amex_iclic_test_bdid_best := join(amex_iclic_test_clean,
                                amex_iclic_bdid_batch_best,
						  left.seq = right.seq,
...
didville.Layout_Did_OutBatch InitPrin(layout_amex_iclic_test_base l) := transform
self.dob := '';
self.ssn := '';
...
amex_iclic_prin_init := project(amex_iclic_test_bdid_best(lname <> ''),InitPrin(left));
amex_iclic_prin_to_did := amex_iclic_prin_init((prim_name <> '' and z5 <> '') or ssn <> '');

didville.MAC_DidAppend(amex_iclic_prin_to_did, amex_iclic_prin_did_match, true, 'Z4G')

didville.MAC_BestAppend(amex_iclic_prin_did_match,'BEST_ALL','BEST_ALL',0,true,amex_iclic_prin_did_best)

amex_iclic_prin_did_best_dist := distribute(amex_iclic_prin_did_best(did <> 0), hash(seq));
amex_iclic_prin_did_best_sort := sort(amex_iclic_prin_did_best_dist, seq, -score, local);
amex_iclic_prin_did_best_dedup := dedup(amex_iclic_prin_did_best_sort, seq, local);

amex_iclic_test_bdid_best_dist := distribute(amex_iclic_test_bdid_best, hash(seq));

Layout_amex_iclic_Test_Prin_Base FormatPrin(layout_amex_iclic_test_base l, didville.Layout_Did_OutBatch r) := transform
self := r;
self := l;
end;

amex_iclic_prin_did_base := join(amex_iclic_test_bdid_best_dist,
                                 amex_iclic_prin_did_best_dedup,
					        left.seq = right.seq,
						   FormatPrin(left, right),
						   local) : persist('TMTEST::amex_iclic_test_did_best');

// Add did and did scores
layout_amex_iclic_test_base AppendDID(layout_amex_iclic_test_base l, didville.Layout_Did_OutBatch r) := transform
self.did := r.did;
self.score := r.score;
self := l;
end;

amex_iclic_test_best := join(amex_iclic_test_bdid_best_dist,
                             amex_iclic_prin_did_best_dedup,
					    left.seq = right.seq,
...
amex_iclic_out := sort(amex_iclic_test_best, AXP_Vendor_Key, if(bdid <> 0, 0, 1), -bdid_score);

export amex_iclic_test_prep := amex_iclic_out  : persist('TMTEST::amex_iclic_test_best');

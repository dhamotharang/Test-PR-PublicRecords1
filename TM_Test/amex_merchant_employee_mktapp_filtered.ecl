...
amex_merchant_init := amex_merchant_mktapp_filtered;
...
amex_merchant_bdid_list := table(amex_merchant_init(bdid <> 0, bdid_mkt_flag = 'N'), layout_bdid_list);
...
bc := Business_Header.File_Business_Contacts_MktApp(bdid <> 0, from_hdr = 'N', did <> 0, not glb, not dppa);
...
export amex_merchant_employee_mktapp_filtered := amex_merchant_emp_best_combined_sort : persist('TMTEST::amex_merchant_employee_base_mktapp');
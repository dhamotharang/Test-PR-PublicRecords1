...
// select non D&B Business Header records
bh_list_init := project(bh(source <> 'D'),
                        transform(layout_bdid_list, self := left));
				    
bh_list_dedup := dedup(bh_list_init, bdid, all);

bh_list_nondnb := join(bh_list_dedup,
                       bhb_nba_grp,
				   left.bdid = right.bdid,
				   transform(layout_bdid_list, self := right),
				   hash);

output(bh_list_nondnb,,'TMTEST::Demographic_BDID_List_NonDNB', overwrite);				   
output(count(bh_list_nondnb), named('Business_Header_BDID_Non_DNB'));
				   
bh_list_nondnb_gid := dedup(bh_list_nondnb, group_id, all);

output(count(bh_list_nondnb_gid), named('Business_Header_Group_Non_DNB'));

				   
// Select active D&B Records with non-blank addresses
db := DNB.File_DNB_Base(bdid <> 0, record_type = 'C', active_duns_number = 'Y',
                        (integer)zip <> 0, prim_name <> '', prim_name[1..6] <> 'PO BOX');

				    
db_list_init := project(db,
                        transform(layout_bdid_list, self := left));
				    
db_list_dedup := dedup(db_list_init, bdid, all);
			   
db_list := join(db_list_dedup,
                bhb_nba_grp,
			 left.bdid = right.bdid,
			 transform(layout_bdid_list, self := right),
			 hash);

output(db_list,,'TMTEST::Demographic_BDID_List_DNB', overwrite);				   
output(count(db_list), named('Dun_and_Bradstreet_BDID'));
			 
db_list_gid := dedup(db_list, group_id, all);

output(count(db_list_gid), named('Dun_and_Bradstreet_Group'));
...
output(count(db_bdid_only), named('Dun_and_Bradstreet_BDID_Only'));
...
output(count(bh_db_bdid_overlap), named('Business_Header_Dun_and_Bradstreet_BDID_Overlap'));

// Find Group overlap and non-overlap
bh_gid_only := join(bh_list_nondnb_gid,
                     db_list_gid,
...
db_gid_only := join(bh_list_nondnb_gid,
                     db_list_gid,
				 left.group_id = right.group_id,
				 transform(layout_bdid_list, self := right),
				 right only,
...
output(count(db_gid_only), named('Dun_and_Bradstreet_Group_Only'));

bh_db_gid_overlap := join(bh_list_nondnb_gid,
                      db_list_gid,
				  left.group_id = right.group_id,
				  transform(layout_bdid_list, self := left),
				  hash);

output(count(bh_db_gid_overlap), named('Business_Header_Dun_and_Bradstreet_Group_Overlap'));

// Combined BDID list
bdid_list := dedup(bh_list_nondnb + db_list, bdid, all);

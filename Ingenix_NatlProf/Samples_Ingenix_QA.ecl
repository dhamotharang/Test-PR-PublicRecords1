...
Ingenix_Provider_File 				:= dedup(dataset('~thor_data400::base::ingenix_providers_did',Ingenix_NatlProf.Layout_Provider_Base_Rid,flat), record);
Ingenix_Provider_Father_File  := dedup(dataset('~thor_data400::base::ingenix_providers_did_father',Ingenix_NatlProf.Layout_Provider_Base_Rid,flat), record);

Ingenix_Provider_File_dist 				:= sort(distribute(Ingenix_Provider_File,HASH32(providerid)),providerid, skew(0.5,0.5));
Ingenix_Provider_Father_File_dist := sort(distribute(Ingenix_Provider_Father_File,HASH32(providerid)),providerid, skew(0.5,0.5));
...
Ingenix_Provider_Join := join(Ingenix_Provider_File_dist,Ingenix_Provider_Father_File_dist,LEFT.providerid = RIGHT.providerid,join_ing_prov_trans(LEFT,RIGHT),LEFT ONLY,local);
...
Ingenix_Sanc_Father_File 			:= dedup(dataset('~thor_data400::base::ingenix_sanctions_did_father',Ingenix_NatlProf.layout_sanctions_DID_RecID,flat),record);
Ingenix_Sanc_File_dist 				:= sort(distribute(Ingenix_Sanc_File,HASH32(sanc_id)),sanc_id,skew(0.5,0.5));
Ingenix_Sanc_Father_File_dist := sort(distribute(Ingenix_Sanc_Father_File,HASH32(sanc_id)),sanc_id,skew(0.5,0.5));

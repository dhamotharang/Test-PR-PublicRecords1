...
file_dist := distribute(File_in_ProviderDegree.Allsrc, hash(FILETYP,ProviderID));
file_sort := sort(file_dist,FILETYP, ProviderID, Degree, -ProcessDate,local);
...
export update_degree := rollup(file_sort,rollupXform(LEFT,RIGHT),FILETYP,ProviderID,Degree,local);

...
file_dist := distribute(File_in_ProviderUPIN.Allsrc, hash(FILETYP, ProviderID));
file_sort := sort(file_dist,FILETYP, ProviderID, UPIN, -ProcessDate, local);
...
export update_UPIN := rollup(file_sort,rollupXform(LEFT,RIGHT),FILETYP, ProviderID, UPIN,local);

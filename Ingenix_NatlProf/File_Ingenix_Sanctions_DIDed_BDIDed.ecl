...
a := Ingenix_NatlProf.File_Sanctions_Cleaned_DIDed_dates;
b := Ingenix_NatlProf.File_Sanctions_Cleaned_BDIDed_dates;
...
Layout_final jointhem(a l,b r) := Transform
self.did  := l.did  ;
self.bdid := r.bdid ;
self      := l      ;
end;

export File_Ingenix_Sanctions_DIDed_BDIDed := join(a,b,
																	  left.Sanc_ID = right.Sanc_id,
																		jointhem(left,right)
																	 ):persist('~thor_data400::persist::Ingenix_Sanctions_DIDed_BDIDed');
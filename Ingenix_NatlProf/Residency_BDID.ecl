import Ingenix_natlprof, did_add, Business_Header_SS, ut, business_header;


file_in := Ingenix_NatlProf.update_residency_bdid;

Pre_BDID_Rec
 :=
  record
	Ingenix_NatlProf.Layout_Residency_Base_BIP;
	integer8	temp_BDID				:= 0;
	integer8	temp_BDID_SCORE	:= 0;
  end
 ;

Pre_BDID_Rec taddBDID(file_in L)
 :=
  transform
	self.DotID			:= 0;
	self.DotScore		:= 0;
...
Layout_Residency_Base_BIP tBdid(PostBDIDpersist L)
 :=
  transform
...
export Residency_BDID := project(PostBDIDpersist, tBdid(left)):persist('~thor_data400::persist::Ingenix_provider_residency_bdid');

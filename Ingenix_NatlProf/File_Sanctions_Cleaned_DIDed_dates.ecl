...
raw_sanc_did_file := Ingenix_NatlProf.update_BWR_Sanctions_Did_File;
...
base_srt roll_it(base_srt l, base_srt r):= transform
  self.did 															:= if(l.did='', r.did, l.did);
	...
	self.prov_clean_name_suffix 					:= if(l.prov_clean_name_suffix = '' or ut.Translate_Suffix(l.prov_clean_name_suffix)=r.prov_clean_name_suffix, 
																							r.prov_clean_name_suffix, l.prov_clean_name_suffix);
  ...
  self.SANC_REINDTE_form 								:= if(l.SANC_REINDTE_form <> '' and self.SANC_SANCDTE_form <> '' and l.SANC_REINDTE_form < self.SANC_SANCDTE_form , '', l.SANC_REINDTE_form);
	self.SANC_REINDTE 										:= if(l.SANC_REINDTE_form <> '' and self.SANC_SANCDTE_form <> '' and l.SANC_REINDTE_form < self.SANC_SANCDTE_form , '', l.SANC_REINDTE);

...
in_file := File_in_ProviderAddressTaxID.Allsrc ;

file_dist := distribute(in_file, hash(FILETYP, ProviderID, addressID));
file_sort := sort(file_dist,FILETYP, ProviderID,addressID, TaxID, -ProcessDate, local);

new_rec := record

Ingenix_NatlProf.Layout_in_ProviderAddressTaxID.raw_srctype;
...
ProviderAddressTaxID := project(file_sort, tappending(left));
...
export update_ProviderAddressTaxID := rollup(ProviderAddressTaxID,rollupXform(LEFT,RIGHT),FILETYP, ProviderID, addressID, TaxID,local);  

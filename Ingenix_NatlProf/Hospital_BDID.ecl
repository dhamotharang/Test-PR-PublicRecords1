...
file_in := Ingenix_NatlProf.update_hospital_bdid;

Pre_BDID_Rec := record
	ingenix_natlprof.Layout_hospital_Base_BIP;
...
Layout_hospital_Base_BIP tBdid(PostBDIDpersist L) :=  transform
	self.BDID				:=	intformat(L.temp_BDID,12,1);
	self.BDID_SCORE	:=	(string3)L.temp_BDID_SCORE;
	self            := L;
end;
	
export Hospital_BDID := project(PostBDIDpersist, tBdid(left)):persist('~thor_data400::persist::Ingenix_provider_hospital_bdid');

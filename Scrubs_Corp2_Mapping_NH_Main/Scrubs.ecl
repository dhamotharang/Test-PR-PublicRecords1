...
EXPORT FromNone(DATASET(Corp2_mapping.LayoutsCommon.Main) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_standing_Invalid := Fields.InValid_corp_standing((SALT311.StrType)le.corp_standing);
    ...
    SELF.corp_ra_address_type_cd_Invalid := Fields.InValid_corp_ra_address_type_cd((SALT311.StrType)le.corp_ra_address_type_cd);
    ...
    SELF.cont_address_type_cd_Invalid := Fields.InValid_cont_address_type_cd((SALT311.StrType)le.cont_address_type_cd);
    SELF.cont_address_type_desc_Invalid := Fields.InValid_cont_address_type_desc((SALT311.StrType)le.cont_address_type_desc);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_ln_name_type_desc_Invalid << 22 ) + ( le.corp_address1_type_cd_Invalid << 23 ) + ( le.corp_address1_type_desc_Invalid << 24 ) + ( le.corp_status_desc_Invalid << 25 ) + ( le.corp_standing_Invalid << 26 ) + ( le.corp_inc_state_Invalid << 27 ) + ( le.corp_inc_date_Invalid << 28 ) + ( le.corp_term_exist_cd_Invalid << 30 ) + ( le.corp_term_exist_exp_Invalid << 31 ) + ( le.corp_term_exist_desc_Invalid << 33 ) + ( le.corp_foreign_domestic_ind_Invalid << 34 ) + ( le.corp_forgn_state_cd_Invalid << 35 ) + ( le.corp_forgn_state_desc_Invalid << 36 ) + ( le.corp_forgn_date_Invalid << 37 ) + ( le.corp_orig_org_structure_desc_Invalid << 39 ) + ( le.corp_for_profit_ind_Invalid << 40 ) + ( le.corp_ra_address_type_cd_Invalid << 41 ) + ( le.corp_ra_address_type_desc_Invalid << 42 ) + ( le.cont_address_type_cd_Invalid << 43 ) + ( le.cont_address_type_desc_Invalid << 44 ) + ( le.recordorigin_Invalid << 45 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_status_desc_Invalid := (le.ScrubsBits1 >> 25) & 1;
    ...
    SELF.corp_term_exist_exp_Invalid := (le.ScrubsBits1 >> 31) & 3;
    SELF.corp_term_exist_desc_Invalid := (le.ScrubsBits1 >> 33) & 1;
    ...
    SELF.corp_orig_org_structure_desc_Invalid := (le.ScrubsBits1 >> 39) & 1;
    SELF.corp_for_profit_ind_Invalid := (le.ScrubsBits1 >> 40) & 1;
    SELF.corp_ra_address_type_cd_Invalid := (le.ScrubsBits1 >> 41) & 1;
    SELF.corp_ra_address_type_desc_Invalid := (le.ScrubsBits1 >> 42) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    AnyRule_WithErrorsCount := COUNT(GROUP, h.dt_vendor_first_reported_Invalid > 0 OR h.dt_vendor_last_reported_Invalid > 0 OR h.dt_first_seen_Invalid > 0 OR h.dt_last_seen_Invalid > 0 OR h.corp_ra_dt_first_seen_Invalid > 0 OR h.corp_ra_dt_last_seen_Invalid > 0 OR h.corp_key_Invalid > 0 OR h.corp_vendor_Invalid > 0 OR h.corp_state_origin_Invalid > 0 OR h.corp_process_date_Invalid > 0 OR h.corp_orig_sos_charter_nbr_Invalid > 0 OR h.corp_legal_name_Invalid > 0 OR h.corp_ln_name_type_cd_Invalid > 0 OR h.corp_ln_name_type_desc_Invalid > 0 OR h.corp_address1_type_cd_Invalid > 0 OR h.corp_address1_type_desc_Invalid > 0 OR h.corp_status_desc_Invalid > 0 OR h.corp_standing_Invalid > 0 OR h.corp_inc_state_Invalid > 0 OR h.corp_inc_date_Invalid > 0 OR h.corp_term_exist_cd_Invalid > 0 OR h.corp_term_exist_exp_Invalid > 0 OR h.corp_term_exist_desc_Invalid > 0 OR h.corp_foreign_domestic_ind_Invalid > 0 OR h.corp_forgn_state_cd_Invalid > 0 OR h.corp_forgn_state_desc_Invalid > 0 OR h.corp_forgn_date_Invalid > 0 OR h.corp_orig_org_structure_desc_Invalid > 0 OR h.corp_for_profit_ind_Invalid > 0 OR h.corp_ra_address_type_cd_Invalid > 0 OR h.corp_ra_address_type_desc_Invalid > 0 OR h.cont_address_type_cd_Invalid > 0 OR h.cont_address_type_desc_Invalid > 0 OR h.recordorigin_Invalid > 0);
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ...
          ,le.cont_address_type_cd_CUSTOM_ErrorCount
          ,le.cont_address_type_desc_CUSTOM_ErrorCount
          ,le.recordorigin_ALLOW_ErrorCount,0) / le.TotalCnt, CHOOSE(c - NumRules
    ...
    SALT311.ScrubsOrbitLayout xNormHygieneStats(hygiene_summaryStats le, UNSIGNED c, STRING suffix) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'recordorigin:' + getFieldTypeText(h.recordorigin) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix
          ,'internalfield1:' + getFieldTypeText(h.internalfield1) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix,'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.populated_recordorigin_cnt
          ,le.populated_internalfield1_cnt,0);
      SELF.rulepcnt := CHOOSE(c
          ...
          ,le.populated_internalfield1_pcnt,0);
      SELF.ErrorMessage := '';
    END;
    FieldPopStats := NORMALIZE(hygiene_summaryStats,291,xNormHygieneStats(LEFT,COUNTER,'POP'));
  ...
  // record count stats
    ...
    DeltaFieldPopStats := NORMALIZE(deltaHygieneSummary(txt <> 'New'),291,xNormHygieneStats(LEFT,COUNTER,'DELTA'));
...
EXPORT StandardStats(DATASET(Corp2_mapping.LayoutsCommon.Main) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Corp2_Mapping_NH_Main, Fields, 'RECORDOF(scrubsSummaryOverall)', '');

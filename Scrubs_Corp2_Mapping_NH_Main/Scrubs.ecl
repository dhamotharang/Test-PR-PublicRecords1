...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Corp2_Mapping.LayoutsCommon.Main)
    ...
    UNSIGNED1 corp_merged_corporation_id_Invalid;
...
EXPORT FromNone(DATASET(Corp2_Mapping.LayoutsCommon.Main) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_merged_corporation_id_Invalid := Fields.InValid_corp_merged_corporation_id((SALT34.StrType)le.corp_merged_corporation_id);
    SELF.corp_merger_date_Invalid := Fields.InValid_corp_merger_date((SALT34.StrType)le.corp_merger_date);
    SELF.corp_merger_id_Invalid := Fields.InValid_corp_merger_id((SALT34.StrType)le.corp_merger_id);
    SELF.corp_merger_indicator_Invalid := Fields.InValid_corp_merger_indicator((SALT34.StrType)le.corp_merger_indicator);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_ln_name_type_desc_Invalid << 22 ) + ( le.corp_address1_type_cd_Invalid << 23 ) + ( le.corp_address1_type_desc_Invalid << 24 ) + ( le.corp_status_cd_Invalid << 25 ) + ( le.corp_status_desc_Invalid << 26 ) + ( le.corp_standing_Invalid << 27 ) + ( le.corp_inc_state_Invalid << 28 ) + ( le.corp_inc_date_Invalid << 29 ) + ( le.corp_term_exist_cd_Invalid << 31 ) + ( le.corp_term_exist_exp_Invalid << 32 ) + ( le.corp_term_exist_desc_Invalid << 34 ) + ( le.corp_foreign_domestic_ind_Invalid << 35 ) + ( le.corp_forgn_state_cd_Invalid << 36 ) + ( le.corp_forgn_state_desc_Invalid << 37 ) + ( le.corp_forgn_date_Invalid << 38 ) + ( le.corp_orig_org_structure_cd_Invalid << 40 ) + ( le.corp_orig_org_structure_desc_Invalid << 41 ) + ( le.corp_for_profit_ind_Invalid << 42 ) + ( le.corp_ra_address_type_cd_Invalid << 43 ) + ( le.corp_ra_address_type_desc_Invalid << 44 ) + ( le.cont_title1_desc_Invalid << 45 ) + ( le.cont_title2_desc_Invalid << 46 ) + ( le.cont_title3_desc_Invalid << 47 ) + ( le.cont_title4_desc_Invalid << 48 ) + ( le.cont_title5_desc_Invalid << 49 ) + ( le.cont_address_type_cd_Invalid << 50 ) + ( le.cont_address_type_desc_Invalid << 51 ) + ( le.corp_merged_corporation_id_Invalid << 52 ) + ( le.corp_merger_date_Invalid << 53 ) + ( le.corp_merger_id_Invalid << 55 ) + ( le.corp_merger_indicator_Invalid << 56 ) + ( le.recordorigin_Invalid << 57 ) + ( le.internalfield1_Invalid << 58 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_standing_Invalid := (le.ScrubsBits1 >> 27) & 1;
    ...
    SELF.corp_orig_org_structure_desc_Invalid := (le.ScrubsBits1 >> 41) & 1;
    SELF.corp_for_profit_ind_Invalid := (le.ScrubsBits1 >> 42) & 1;
    SELF.corp_ra_address_type_cd_Invalid := (le.ScrubsBits1 >> 43) & 1;
    SELF.corp_ra_address_type_desc_Invalid := (le.ScrubsBits1 >> 44) & 1;
    SELF.cont_title1_desc_Invalid := (le.ScrubsBits1 >> 45) & 1;
    SELF.cont_title2_desc_Invalid := (le.ScrubsBits1 >> 46) & 1;
    SELF.cont_title3_desc_Invalid := (le.ScrubsBits1 >> 47) & 1;
    SELF.cont_title4_desc_Invalid := (le.ScrubsBits1 >> 48) & 1;
    SELF.cont_title5_desc_Invalid := (le.ScrubsBits1 >> 49) & 1;
    SELF.cont_address_type_cd_Invalid := (le.ScrubsBits1 >> 50) & 1;
    SELF.cont_address_type_desc_Invalid := (le.ScrubsBits1 >> 51) & 1;
    SELF.corp_merged_corporation_id_Invalid := (le.ScrubsBits1 >> 52) & 1;
    SELF.corp_merger_date_Invalid := (le.ScrubsBits1 >> 53) & 3;
    SELF.corp_merger_id_Invalid := (le.ScrubsBits1 >> 55) & 1;
    SELF.corp_merger_indicator_Invalid := (le.ScrubsBits1 >> 56) & 1;
    SELF.recordorigin_Invalid := (le.ScrubsBits1 >> 57) & 1;
    SELF.internalfield1_Invalid := (le.ScrubsBits1 >> 58) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_forgn_state_cd_CUSTOM_ErrorCount := COUNT(GROUP,h.corp_forgn_state_cd_Invalid=1);
    ...
    cont_title2_desc_CUSTOM_ErrorCount := COUNT(GROUP,h.cont_title2_desc_Invalid=1);
    cont_title3_desc_CUSTOM_ErrorCount := COUNT(GROUP,h.cont_title3_desc_Invalid=1);
    cont_title4_desc_CUSTOM_ErrorCount := COUNT(GROUP,h.cont_title4_desc_Invalid=1);
    cont_title5_desc_CUSTOM_ErrorCount := COUNT(GROUP,h.cont_title5_desc_Invalid=1);
    cont_address_type_cd_CUSTOM_ErrorCount := COUNT(GROUP,h.cont_address_type_cd_Invalid=1);
    cont_address_type_desc_CUSTOM_ErrorCount := COUNT(GROUP,h.cont_address_type_desc_Invalid=1);
    corp_merged_corporation_id_ALLOW_ErrorCount := COUNT(GROUP,h.corp_merged_corporation_id_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_forgn_state_cd_Invalid,'CUSTOM','UNKNOWN')
          ...
          ,CHOOSE(le.cont_title2_desc_Invalid,'CUSTOM','UNKNOWN')
          ,CHOOSE(le.cont_title3_desc_Invalid,'CUSTOM','UNKNOWN')
          ,CHOOSE(le.cont_title4_desc_Invalid,'CUSTOM','UNKNOWN')
          ,CHOOSE(le.cont_title5_desc_Invalid,'CUSTOM','UNKNOWN')
          ,CHOOSE(le.cont_address_type_cd_Invalid,'CUSTOM','UNKNOWN')
          ,CHOOSE(le.cont_address_type_desc_Invalid,'CUSTOM','UNKNOWN')
          ,CHOOSE(le.corp_merged_corporation_id_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.corp_merger_date_Invalid,'ALLOW','CUSTOM','UNKNOWN')
          ,CHOOSE(le.corp_merger_id_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.corp_merger_indicator_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.recordorigin_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.internalfield1_Invalid,'CUSTOM','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'dt_vendor_first_reported','dt_vendor_last_reported','dt_first_seen','dt_last_seen','corp_ra_dt_first_seen','corp_ra_dt_last_seen','corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_orig_sos_charter_nbr','corp_legal_name','corp_ln_name_type_cd','corp_ln_name_type_desc','corp_address1_type_cd','corp_address1_type_desc','corp_status_cd','corp_status_desc','corp_standing','corp_inc_state','corp_inc_date','corp_term_exist_cd','corp_term_exist_exp','corp_term_exist_desc','corp_foreign_domestic_ind','corp_forgn_state_cd','corp_forgn_state_desc','corp_forgn_date','corp_orig_org_structure_cd','corp_orig_org_structure_desc','corp_for_profit_ind','corp_ra_address_type_cd','corp_ra_address_type_desc','cont_title1_desc','cont_title2_desc','cont_title3_desc','cont_title4_desc','cont_title5_desc','cont_address_type_cd','cont_address_type_desc','corp_merged_corporation_id','corp_merger_date','corp_merger_id','corp_merger_indicator','recordorigin','internalfield1','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_mandatory','invalid_name_type_code','invalid_name_type_desc','invalid_address1_type_cd','invalid_address1_type_desc','invalid_corp_status_code','invalid_corp_status_desc','invalid_flag_code','invalid_state_origin','invalid_date','invalid_term_cd','invalid_future_date','invalid_term_desc','invalid_forgn_dom_code','invalid_corp_forgn_state_cd','invalid_corp_forgn_state_desc','invalid_date','invalid_type_code','invalid_type_desc','invalid_flag_code','invalid_ra_address_type_cd','invalid_ra_address_type_desc','invalid_title_desc','invalid_title_desc','invalid_title_desc','invalid_title_desc','invalid_title_desc','invalid_contact_address_type_cd','invalid_contact_address_type_desc','invalid_numericblank','invalid_date','invalid_numericblank','invalid_merger_cd','invalid_recordorigin','invalid_addresstypeid','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT34.StrType)le.dt_vendor_first_reported,(SALT34.StrType)le.dt_vendor_last_reported,(SALT34.StrType)le.dt_first_seen,(SALT34.StrType)le.dt_last_seen,(SALT34.StrType)le.corp_ra_dt_first_seen,(SALT34.StrType)le.corp_ra_dt_last_seen,(SALT34.StrType)le.corp_key,(SALT34.StrType)le.corp_vendor,(SALT34.StrType)le.corp_state_origin,(SALT34.StrType)le.corp_process_date,(SALT34.StrType)le.corp_orig_sos_charter_nbr,(SALT34.StrType)le.corp_legal_name,(SALT34.StrType)le.corp_ln_name_type_cd,(SALT34.StrType)le.corp_ln_name_type_desc,(SALT34.StrType)le.corp_address1_type_cd,(SALT34.StrType)le.corp_address1_type_desc,(SALT34.StrType)le.corp_status_cd,(SALT34.StrType)le.corp_status_desc,(SALT34.StrType)le.corp_standing,(SALT34.StrType)le.corp_inc_state,(SALT34.StrType)le.corp_inc_date,(SALT34.StrType)le.corp_term_exist_cd,(SALT34.StrType)le.corp_term_exist_exp,(SALT34.StrType)le.corp_term_exist_desc,(SALT34.StrType)le.corp_foreign_domestic_ind,(SALT34.StrType)le.corp_forgn_state_cd,(SALT34.StrType)le.corp_forgn_state_desc,(SALT34.StrType)le.corp_forgn_date,(SALT34.StrType)le.corp_orig_org_structure_cd,(SALT34.StrType)le.corp_orig_org_structure_desc,(SALT34.StrType)le.corp_for_profit_ind,(SALT34.StrType)le.corp_ra_address_type_cd,(SALT34.StrType)le.corp_ra_address_type_desc,(SALT34.StrType)le.cont_title1_desc,(SALT34.StrType)le.cont_title2_desc,(SALT34.StrType)le.cont_title3_desc,(SALT34.StrType)le.cont_title4_desc,(SALT34.StrType)le.cont_title5_desc,(SALT34.StrType)le.cont_address_type_cd,(SALT34.StrType)le.cont_address_type_desc,(SALT34.StrType)le.corp_merged_corporation_id,(SALT34.StrType)le.corp_merger_date,(SALT34.StrType)le.corp_merger_id,(SALT34.StrType)le.corp_merger_indicator,(SALT34.StrType)le.recordorigin,(SALT34.StrType)le.internalfield1,'***SALTBUG***');
  END;
  EXPORT AllErrors := NORMALIZE(h,46,Into(LEFT,COUNTER));
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_address1_type_cd:invalid_address1_type_cd:CUSTOM'
          ,'corp_address1_type_desc:invalid_address1_type_desc:CUSTOM'
          ,'corp_status_cd:invalid_corp_status_code:CUSTOM'
          ...
          ,'corp_term_exist_desc:invalid_term_desc:ENUM'
          ,'corp_foreign_domestic_ind:invalid_forgn_dom_code:ENUM'
          ,'corp_forgn_state_cd:invalid_corp_forgn_state_cd:CUSTOM'
          ,'corp_forgn_state_desc:invalid_corp_forgn_state_desc:CUSTOM'
          ,'corp_forgn_date:invalid_date:ALLOW','corp_forgn_date:invalid_date:CUSTOM'
          ,'corp_orig_org_structure_cd:invalid_type_code:CUSTOM'
          ,'corp_orig_org_structure_desc:invalid_type_desc:CUSTOM'
          ,'corp_for_profit_ind:invalid_flag_code:ENUM'
          ,'corp_ra_address_type_cd:invalid_ra_address_type_cd:CUSTOM'
          ,'corp_ra_address_type_desc:invalid_ra_address_type_desc:CUSTOM'
          ,'cont_title1_desc:invalid_title_desc:CUSTOM'
          ,'cont_title2_desc:invalid_title_desc:CUSTOM'
          ,'cont_title3_desc:invalid_title_desc:CUSTOM'
          ,'cont_title4_desc:invalid_title_desc:CUSTOM'
          ,'cont_title5_desc:invalid_title_desc:CUSTOM'
          ,'cont_address_type_cd:invalid_contact_address_type_cd:CUSTOM'
          ,'cont_address_type_desc:invalid_contact_address_type_desc:CUSTOM'
          ,'corp_merged_corporation_id:invalid_numericblank:ALLOW'
          ,'corp_merger_date:invalid_date:ALLOW','corp_merger_date:invalid_date:CUSTOM'
          ,'corp_merger_id:invalid_numericblank:ALLOW'
          ,'corp_merger_indicator:invalid_merger_cd:ENUM'
          ,'recordorigin:invalid_recordorigin:ALLOW'
          ,'internalfield1:invalid_addresstypeid:CUSTOM','UNKNOWN');
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_forgn_state_cd_CUSTOM_ErrorCount
          ...
          ,le.cont_title2_desc_CUSTOM_ErrorCount
          ,le.cont_title3_desc_CUSTOM_ErrorCount
          ,le.cont_title4_desc_CUSTOM_ErrorCount
          ,le.cont_title5_desc_CUSTOM_ErrorCount
          ,le.cont_address_type_cd_CUSTOM_ErrorCount
          ,le.cont_address_type_desc_CUSTOM_ErrorCount
          ,le.corp_merged_corporation_id_ALLOW_ErrorCount
          ,le.corp_merger_date_ALLOW_ErrorCount,le.corp_merger_date_CUSTOM_ErrorCount
          ,le.corp_merger_id_ALLOW_ErrorCount
    ...
    END;
    SummaryInfo := NORMALIZE(SummaryStats,59,Into(LEFT,COUNTER));
    orb_r := RECORD
      AllErrors.Src;

 ...
 EXPORT Proc_build_strata_10Fields(STRING version) := FUNCTION
 IMPORT STRATA;

 Basefile:=OutwardMedia.Files.File_OutwardMedia_Base1;
  ...
  firstname_Cnt   :=  sum(group,if(basefile.firstname<>'',1,0));
	...
	address_Cnt   :=  sum(group,if(basefile.address<>'',1,0));
 ...
 strata.createXMLStats(basestrata,'OutwardMedia stats','data',version,'',strataResults);

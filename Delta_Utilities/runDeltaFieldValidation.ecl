...
EXPORT runDeltaFieldValidation(k, sf_kname, DEBUG = false) := FUNCTIONMACRO
  ...
  LOCAL eyeball := CHOOSEN(k, 20);
	...
	//** Duplicate RECORD_SID - Record SIDs must be unique
  LOCAL dupes := CHOOSEN(SORT(TABLE(KS,S,KS.record_sid), -N), 10)(N > max_count_unique_sids);
	
	//** Blank RECORD_SID - Record SIDs must be populated
  LOCAL blank_rids := CHOOSEN(k(record_sid=0), 10);
	
	//** Blank DATE_EFFECTIVE_FIRST - DATE_EFFECTIVE_FIRST must be populated
  LOCAL blank_dt_first := CHOOSEN(k(dt_effective_first=0), 10);
	
	//** Invalid DELTA_IND - DELTA_IND must be populated with values: [0,1,3] 0 - main record, 1 - incremental add, 3 - incremental delete
	//** if no deltas, all delta_ind should be 0
  LOCAL invalid_delta_ind := if(DELTA_Cnt > 0, CHOOSEN(k(delta_ind>3), 10), CHOOSEN(k(delta_ind>0), 10));
	
	//** When DATE_EFFECTIVE_LAST is populated, DATE_EFFECTIVE_FIRST must be <= DATE_EFFECTIVE_LAST 
  //** All records with DELTA_IND = 3 must have DATE_EFFECTIVE_LAST populated
  LOCAL inconsistent_deletes := CHOOSEN(k(dt_effective_last>0, (delta_ind<>3 OR dt_effective_first > dt_effective_last)), 10) +
                                CHOOSEN(k(delta_ind=3 AND dt_effective_last = 0), 10); 

import AutostandardI,iesp,doxie,STD,VehicleV2,ut,Suppress;
export fn_smart_rollup_veh(dataset(iesp.motorvehicle.t_MotorVehicleReport2Record) inRecs,unsigned6 subjectDID) := function
  ...
  dRecs fixCurrent(dRecs L) := transform
	  owners_pre := LIMIT(VehicleV2.Key_Vehicle_Party_Key(keyed(vehicle_key = L.vehicleInfo.vehicleRecordId)),1000,KEYED,SKIP);
    owners := suppress.MAC_SuppressSource(owners_pre,mod_access,append_did);

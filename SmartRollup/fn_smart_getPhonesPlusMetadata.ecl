...
export fn_smart_getPhonesPlusMetadata := MODULE
	...
	EXPORT byPhone(DATASET(fn_smart_getAddrMetadata.outLayout) addresses) := FUNCTION
		...
		iesp.smartlinxreport.t_SLRAddress getPhonesPlusMetadata(fn_smart_getAddrMetadata.outLayout L) := TRANSFORM
			tmpPhone getPhones(iesp.bpsreport.t_BpsReportPhone LE) := TRANSFORM
				...
				END;
				rawphones := PROJECT(doxie.phone_noreconn_records(srchMod).val(dids_deduped),toESDL(LEFT));
	...
	EXPORT byDid(doxie.layout_best B) := FUNCTION
		srchMod := MODULE(PROJECT(AutoStandardI.GlobalModule(),doxie.phone_noreconn_param.searchParams,OPT))
			...
			EXPORT UNSIGNED8 MaxResultsThisTime := iesp.Constants.BR.MaxPhonesPlus;
		END;
		phonesPlusMetadata := doxie.phone_noreconn_records(srchMod).val(dids_deduped);

IMPORT ln_propertyv2, PRTE2;

// standardized_land_use_code needs filling still?
EXPORT Layouts_V2 := MODULE

		// Not needed because Boca build does the transform - but we should keep an eye on new fields
		EXPORT DeedLayout := ln_propertyv2.layout_deed_mortgage_common_model_base;
		EXPORT TaxLayout := ln_propertyv2.layout_property_common_model_base;
		EXPORT SearchLayout := LN_PropertyV2.layout_search_building;
		...
		EXPORT LN_spreadsheet := RECORD
			...
			STRING  dob;
			STRING  address;
			STRING	unitDesc;		// I'll generate this for now, we then might want to fix from header and then merge address+' '+unitDesc+' '+apt
			STRING  apt;
			STRING  city;
			STRING2 state;
			STRING  zip;
			...
			string10 bphone;
			string1 which_orig;
			unsigned3 dt_first_seen;							//TODO - might want to start with this from BHDR/CSV
			unsigned3 dt_last_seen;								//TODO - might want to start with this from BHDR/CSV
			unsigned3 dt_vendor_first_reported;		//TODO - might want to start with this from BHDR/CSV
			unsigned3 dt_vendor_last_reported;		//TODO - might want to start with this from BHDR/CSV
			...
			string20 pfname;
			string1 minit;
			unsigned8 proc_date;							// set to build date
			string8 process_date;							// set to build date

...
out := FROMXML(rec,LEFT.Response.DteRequest.TaskEx.RequestOpaqueContent);
...
SELF.SuppressRecord := IF(LEFT.Response.DTERequest.TaskEx.TaskStatus <> '0', TRUE, FALSE);
SELF := RIGHT;
));
...
EXPORT JuLiProcessStatusRefresh(grouped dataset(risk_indicators.Layout_Boca_Shell) clam, dataset(Gateway.Layouts.Config) gateways, dataset(riskview.layouts.layout_riskview5_search_results) riskview5_final_results, boolean ExcludeStatusRefresh, string10 StatusRefreshWaitPeriod) := FUNCTION
...
riskview5_suppressed := IF(ExcludeStatusRefresh = TRUE, PROJECT(riskview5_final_results, 
TRANSFORM(RECORDOF(LEFT), 
...
SELF.Request_XML := '<RMSID>' + LEFT.RMSID + '</RMSID>' + '<TMSID>' + LEFT.TMSID + '</TMSID>';

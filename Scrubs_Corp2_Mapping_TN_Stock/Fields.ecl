...
EXPORT SALT34.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_charter_nbr','invalid_mandatory','invalid_date','invalid_stock_shares_issued','invalid_stock_type');
EXPORT FieldTypeNum(SALT34.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_corp_vendor' => 2,'invalid_state_origin' => 3,'invalid_charter_nbr' => 4,'invalid_mandatory' => 5,'invalid_date' => 6,'invalid_stock_shares_issued' => 7,'invalid_stock_type' => 8,0);
...
EXPORT MakeFT_invalid_stock_type(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_stock_type(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['COMMON',' ']);
EXPORT InValidMessageFT_invalid_stock_type(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('COMMON| '),SALT34.HygieneErrors.Good);
...
EXPORT Make_stock_type(SALT34.StrType s0) := MakeFT_invalid_stock_type(s0);
EXPORT InValid_stock_type(SALT34.StrType s) := InValidFT_invalid_stock_type(s);
EXPORT InValidMessage_stock_type(UNSIGNED1 wh) := InValidMessageFT_invalid_stock_type(wh);
...
EXPORT Make_stock_par_value(SALT34.StrType s0) := s0;
EXPORT InValid_stock_par_value(SALT34.StrType s) := FALSE;
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT34,Scrubs_Corp2_Mapping_TN_Stock;

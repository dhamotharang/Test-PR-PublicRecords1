export MAC_data_StName (indataset,infname,inmname,inlname,
						inssn,
						indob,
						phone,
...
#uniquename(p)
#uniquename(recs)
Autokey.Layout_StName %proj%(%indata% le) :=
TRANSFORM
	SELF.st := le.inst;
	...
	SELF.did := le.indid;
END;
%p% := distribute(PROJECT(%indata%(inst<>'',inlname<>''), %proj%(LEFT)),hash(did));

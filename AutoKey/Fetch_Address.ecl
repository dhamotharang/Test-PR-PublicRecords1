...
tempmod := module(AutoKeyI.FetchI_Indv_Address.old.params.full)
	...
	export boolean addr_range := ^.addr_range;
	export set of string10 prim_range_set_value := ^.prim_range_set_value;
	...
	export integer FuzzySecRange_value := ^.FuzzySecRange_value;
	export boolean do_primname_word_match := ^.do_primname_word_match;
end;

RETURN AutoKeyI.FetchI_Indv_Address.old.do(tempmod);
END;

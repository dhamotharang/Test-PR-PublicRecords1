IMPORT Address, ut, idl_header, aid, Business_Header_SS, business_header,bipv2;

EXPORT Proc_Build_SDAA_Base(STRING filedate) 
:= FUNCTION 
dBAse  		    :=Mapping_Cleansing_SDAA(filedate) ;

dPrev_SDAA_Base	:= project(File_SDAA_Base, 
...
dBaseFile     := dBase+dPrev_SDAA_Base;
...
dCleanAddrBase_srt:= SORT(DISTRIBUTE(dCleanAddrBase,HASH(COMPANY_NAME)),EXCEPT Dt_First_Seen, Dt_Last_Seen, rawaid, company_rawaid, ace_aid, prep_addr_line1, prep_addr_line_last, LOCAL);
...
ut.MAC_SF_BuildProcess(dBaseout,	SDA_SDAA.cluster.cluster_out+'base::sdAA',	Outbase,	3,,true);

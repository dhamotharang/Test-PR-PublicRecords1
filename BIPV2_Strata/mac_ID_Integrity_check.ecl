...
// ['pRid:pDid','pDid:pParent_ID:empid','empid:orgid','pParent_ID:powid:lgid3','powid:orgid','lgid3:seleid','seleid:orgid','orgid:ultid','ultid']
r := RECORD
...
// dot_prox    := BIPV2_strata.mac_ID_Integrity_check(ds_dataset,pRid,pDid  ,true ,pParent_ID);
///////
...
lOutput := dataset([
  {#TEXT(pDid)   ,#TEXT(pParent_ID) ,countgroup ,pDid_Unique  ,d_nb.pDid_null0  ,d_Ad.pDid_unbased0    ,pDid_pParent_ID_checks[1].DuplicateRids0,pDid_pParent_ID_checks[1].DidsNoRid0,pDid_pParent_ID_checks[1].DidsAboveRid0  ,d_nb.pDid_belowpParent_ID0  ,d_Ad.pDid_Twoparents_pParent_ID0  ,pDid_pParent_ID_checks[1].DidsMultiParent0,pDid_pParent_ID_checks[1].ParentNoDid0,pDid_pParent_ID_checks[1].ParentAboveDid0}
],lay_integrity);                                                                                                                                                                                                                                                                                                                                      
// loutput2 := join(lOutput,id_integrity_prep ,left.id = right.id and left.parent_id = right.parent_id  ,transform(Layout_Uniques ,self := right,self := left),left outer);
// output(loutput2);
    return loutput;
endmacro;

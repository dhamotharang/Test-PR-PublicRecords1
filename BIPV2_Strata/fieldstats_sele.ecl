...
module
  ...
  shared active   := join(pinFile, segmentationFile(inactive = ''),  left.seleid = right.id, transform(recordof(left), self := left), hash);
  shared inactive := join(pinFile, segmentationFile(inactive != ''), left.seleid = right.id, transform(recordof(left), self := left), hash);

...
module
  ...
  shared active   := join(pinfile, segmentationFile(inactive = ''),  left.proxid = right.id, transform(recordof(left), self := left), hash);
  shared inactive := join(pinfile, segmentationFile(inactive != ''), left.proxid = right.id, transform(recordof(left), self := left), hash);

...
import Data_Services, scrubs_sexoffender_main;
...
offender_soff := scrubs_sexoffender_main.in_sexoffender_main;

offense_soff := dataset(Data_Services.foreign_prod  +
...
layout_sexoffender_offense tr_join_offender_and_offense(slim_offender_soff L, offense_soff R) := TRANSFORM
SELF:= L;
SELF := R;
END;
...
EXPORT in_sexoffender_offense := join_offender_and_offense;		 

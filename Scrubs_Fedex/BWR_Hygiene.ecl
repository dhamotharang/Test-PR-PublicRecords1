...
#workunit('name','Scrubs_Fedex.BWR_Hygiene - Hygiene & Stats - SALT V3.11.9');
IMPORT Scrubs_Fedex,SALT311;
// First create an instantiated hygiene module
  infile := Scrubs_Fedex.In_Fedex;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_Fedex.hygiene(ip);
  ...
  OUTPUT(SALT311.MAC_Character_Counts.EclRecord(p,'Layout_Fedex'),NAMED('OptimizedLayout'));// File layout suggested by data

...
//////////////////////////////////////////////////////////////////////////////////////
HasAddress	:=	trim(pre_DB_norm.prep_addr_line1, left,right) != ''
						and trim(pre_DB_norm.prep_addr_line_last, left,right) != '';
...
persistent_recID_join  := join(Update_Base,Previous_Base,
															 trim(left.tmsid,left,right)         		           = trim(right.tmsid,left,right)and
																	  left.raw_aid         	        						   =      right.raw_aid and
															 trim(left.company_name,left,right)       				 = trim(right.company_name,left,right)and
															 trim(left.trade_style,left,right)        				 = trim(right.trade_style,left,right)and
															 trim(left.orig_company_name,left,right)           = trim(right.orig_company_name  ,left,right) and  
															 trim(left.clean_cname ,left,right)  		           = trim(right.clean_cname  ,left,right) and       
															 trim(left.orig_address1,left,right)  		         = trim(right.orig_address1,left,right) and    
															 ...
															 trim(left.Top_Contact_Name,left,right)            = trim(right.Top_Contact_Name  ,left,right) and 
															 trim(left.Top_Contact_Title,left,right)           = trim(right.Top_Contact_Title  ,left,right) and 
															 trim(left.Hdqtr_Parent_Duns_Number,left,right)    = trim(right.Hdqtr_Parent_Duns_Number  ,left,right) and 
...
retval := rollup(DB_sort,rollupXform(LEFT,RIGHT),RECORD,

...
PRTE2_Header_Ins_Data_Work.U_BWR_View_Related_Files
		...
		remote MHDR
************************************************************************************** */
IMPORT PRTE2_Header_Ins,PRTE2_Header_Ins_Data_Work,PRTE2_X_Ins_DataCleanse;
...
IHDR := SORT(PRTE2_X_Ins_DataCleanse.Files_Alpha.InsHeadDLDeath_Base_DS_Prod,uniqueID);
OUTPUT(IHDR,NAMED('IHDR'));
OUTPUT(COUNT(IHDR),NAMED('C_IHDR'));

IHDR2 := CHOOSEN(PRTE2_X_Ins_DataCleanse.Files_Alpha.InsHead_Base_DS_PROD,500)(fb_src='EXPERIAN');
OUTPUT(IHDR2,NAMED('IHDR2'));
OUTPUT(COUNT(IHDR2),NAMED('C_IHDR2'));

MHDR_ORIG := SORT(PRTE2_X_Ins_DataCleanse.Files_Alpha.Merged_Headers_DS,id);
OUTPUT(MHDR_ORIG,NAMED('MHDR_ORIG'));
OUTPUT(COUNT(MHDR_ORIG),NAMED('C_MHDR_ORIG'));

MHDR_NewPS := SORT(PRTE2_X_Ins_DataCleanse.Files_Alpha.PS_Merged_Headers_DS,id);

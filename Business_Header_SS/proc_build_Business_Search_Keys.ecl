IMPORT Business_Header, Text, ut, versioncontrol;
...
module
	...
	shared bh 					:= pBH;
	shared BaseName			:= business_header.filenames(pversion).Base;
	shared KeyName			:= business_header.keynames(pversion).Base;
	...
	VersionControl.macBuildNewLogicalKeyWithName(Key_Prep_BH_Header_Words							,keyName.Conamewords.New					,buildkey1	);
	VersionControl.macBuildNewLogicalKeyWithName(Key_Prep_BH_BDID_City_Zip_Fein_Phone	,keyName.BdidCityZipFeinPhone.New	,buildkey2	);
	VersionControl.macBuildNewLogicalKeyWithName(Key_BH_Header_Words_Metaphone				,keyName.CoNameWordsMetaphone.New	,buildkey3	);

...
export Data_Layout := RECORD
  SALT20.StrType Fld { MAXLENGTH(2048) };
	UNSIGNED2 FldNo;
  END;
...
shared Pattern_Layout := record
  SALT20.StrType data_pattern {maxlength(4092)};
	unsigned cnt;
	end;
	
shared Value_Layout := record
  SALT20.StrType val  {maxlength(4092)};
	unsigned cnt;
	end;
export Field_Identification := RECORD
  unsigned2 FldNo;
  SALT20.StrType FieldName;
  END;
...
glbl_field_cnts := record
  FldInv1;
	unsigned2 xx_l := length(trim((SALT20.StrType)FldInv1.Fld));
	unsigned2 xx_wc := ut.NoWords((SALT20.StrType)FldInv1.Fld);
  end;
...
r tr(FldInv le,unsigned co) := transform
  self.c := ((SALT20.StrType)le.Fld)[co];
	self := le;
  end;
...
dr := record
	  SALT20.StrType Fld  {maxlength(2048)} := SALT20.fn_data_pattern(FldInv.Fld);
		FldInv.Cnt;
		FldInv.FldNo;
  end;
t := table(FldInv,dr);

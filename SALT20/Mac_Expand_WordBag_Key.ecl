...
export mac_expand_wordbag_key(inkey,hash_field,field,outkey,scorefield='') := MACRO
  #uniquename(tr)
	typeof(inkey) %tr%(inkey le,unsigned c) := transform
	  self.hash_field := hash32(ut.Word(le.field,c));
		#IF (#TEXT(scorefield)<>'')
		self.scorefield := (unsigned)ut.Word(le.field,c+1);
		#END
	  self := le;
	end;
	outkey := normalize(inkey,ut.NoWords(left.field)/2,%tr%(left,counter*2));

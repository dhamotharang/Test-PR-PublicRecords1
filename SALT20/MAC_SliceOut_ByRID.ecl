...
%need_reset% := JOIN(%i_recs%,%i_recs%(processf=2),left.did = right.did and left.rid<>right.rid, %note_needs%(left,right), left outer);
...
#uniquename(staterec)
%staterec% := RECORD
  SALT20.UIDType Compare_did := 0;
  SALT20.UIDType Copy_did := 0;
  END;

export mac_wordbag_addweights(infile,infield,value_file,outfile) := MACRO
  ...
  #uniquename(r)
	%r% := record
		...
		unsigned2 node;
		unsigned1 pos;
		SALT22.StrType word {maxlength(8184)} := '' ;
	  end;
	#uniquename(nt)
	%r% %nt%(%p% le,unsigned c) := transform
		self.word := SALT22.GetNthWord(le.infield,c);
		self.pos := c;
		self := le;
	  end;
	#uniquename(n)
	%n% := normalize(%p%,SALT22.WordCount(left.infield),%nt%(left,counter));
	#uniquename(add_s)
	%r% %add_s%(%n% le,value_file ri) := transform
	  self.word := trim(le.word) + ' ' + (SALT22.StrType)((unsigned)ri.field_specificity);
	  self := le;
	  end;
	#uniquename(j)

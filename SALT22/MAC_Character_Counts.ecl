...
export Data_Layout := RECORD
  SALT22.StrType Fld { MAXLENGTH(200000) };
	UNSIGNED2 FldNo;
  END;
...
shared Pattern_Layout := record
  SALT22.StrType data_pattern {maxlength(200000)};
	unsigned cnt;
	end;
	
shared Value_Layout := record
  SALT22.StrType val  {maxlength(200000)};
	unsigned cnt;
	end;
export Field_Identification := RECORD
  unsigned2 FldNo;
  SALT22.StrType FieldName;
  END;
...
glbl_field_cnts := record
  FldInv1;
	unsigned2 xx_l := length(trim((SALT22.StrType)FldInv1.Fld));
	unsigned2 xx_wc := SALT22.WordCount((SALT22.StrType)FldInv1.Fld);
  end;
...
r tr(FldInv le,unsigned co) := transform
  self.c := ((SALT22.StrType)le.Fld)[co];
	self := le;
  end;
...
dr := record
	  SALT22.StrType Fld  {maxlength(200000)} := SALT22.fn_data_pattern(FldInv.Fld);
		FldInv.Cnt;
		FldInv.FldNo;
  end;
t := table(FldInv,dr);

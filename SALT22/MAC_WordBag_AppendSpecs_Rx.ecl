...
#uniquename(d)
%d% := dataset([{SALT22.WordCount(instring),instring}],%r%);
	
#uniquename(tr)
%r% %tr%(%r% s_in, unsigned2 c) := transform
  self.pos := c;
	self.s := SALT22.GetNthWord(s_in.s,c);
  end;
...
#uniquename(bse)
%bse% := dataset([{0,(SALT22.StrType)(100*SUM(%j%,spec))}],%r%);
#uniquename(addw)
%r% %addw%(%r% le, %r2% ri) := transform
  self.s := trim(le.s)+' '+trim(ri.s)+' '+(SALT22.StrType)ri.spec;
  self := le;
  end;
SALT22.StrType o := if ( instring='','',trim(denormalize(%bse%,sort(%j%,pos),true,%addw%(left,right),all)[1].s) );
  ENDMACRO;

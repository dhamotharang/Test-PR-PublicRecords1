...
#workunit('name','Scrubs_Voters.Base_Reg_BWR_Hygiene - Hygiene & Stats - SALT V3.9.0');
IMPORT Scrubs_Voters,SALT39;
// First create an instantiated hygiene module
  infile := Scrubs_Voters.Base_Reg_In_Voters;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_Voters.Base_Reg_hygiene(ip);
  ...
  OUTPUT(SALT39.MAC_Character_Counts.EclRecord(p,'Base_Reg_Layout_Voters'),NAMED('OptimizedLayout'));// File layout suggested by data

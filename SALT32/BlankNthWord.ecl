...
#else
			MAP(n<=1 => sep+SALT32.GetRangeOfWords(s,n+1,wc,sep),
			    n=wc => SALT32.GetRangeOfWords(s,1,wc-1,sep)+sep,
					n>wc => SALT32.GetRangeOfWords(s,1,wc,sep),
					SALT32.GetRangeOfWords(s,1,n-1,sep)+sep+sep+SALT32.GetRangeOfWords(s,n+1,wc,sep));
#end
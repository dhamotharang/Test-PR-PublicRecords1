...
export Failed := -1;
shared getInitMatchWeight(StrType L1, unsigned2 lspec1, unsigned2 ispec1, StrType R1) := function
	iWeight := map((length(L1) > length(R1)) and lspec1 > 0 => ((lspec1 - ispec1)/(length(L1) - 1)) * length(R1) + (ispec1* length(L1) - lspec1)/(length(L1) - 1), //equation of a line between points (1, initial_weight) and (length(d), full_weight)   // previously lspec1 * length(r1)/length(l1), 
...
export  Match1L( StrType L1, unsigned2 lspec1, boolean lnne1, integer lfor1, Boolean linit1, unsigned2 ispec1, integer edit1, StrType R1) := FUNCTION
	// Wait based on StrType match
	init_weight := getInitMatchWeight(L1, lspec1, ispec1, R1);
													
	weight := lspec1;
	minWeight := lspec1;
	
	result1 := map( SALT32.WithinEditN(L1,R1,edit1)  => if(minWeight > lfor1*100, minWeight, FailedForce), 
								 length(R1) > length(L1) and L1 = R1[1..length(TRIM(L1))] => if(minWeight > lfor1*100,init_weight,FailedForce),
								 length(L1) > length(R1) and R1 = L1[1..length(TRIM(R1))] => if(minWeight > lfor1*100,init_weight,FailedForce),
								 Failed);
								 
	result := MAP ( 
							 not linit1 and not lnne1 and SALT32.WithinEditN(L1,R1,edit1) => if(minWeight > lfor1*100, minWeight, FailedForce),	// boolean linit - initial flag,  boolean lnnel - nulls not allowed flag
							 not linit1 and lnne1 and ( L1 = '' or R1 = '' ) 			=> 0,
							 not linit1 and lnne1 and SALT32.WithinEditN(L1,R1,edit1) => if(minWeight > lfor1*100, minWeight, FailedForce),
...
export Match2L(
    ...
    L1BlankCost := IF ( lfor1 > 0, FailedForce, Failed );
	 ...
	 L1R1 := Match1L(L1,lspec1,lnne1,lfor1,linit1,ispec1,edit1,R1);
	 L1R2 := Match1L(L1,lspec1,lnne1,lfor1,linit1,ispec1,edit1,R2);
	 L2R1 := Match1L(L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,R1);
	 L2R2 := Match1L(L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,R2);
	 ...
	 result := MAP(not lnne1  and not linit1  and lfor1 > 0 	     => r0,
								...
								linit1 or linit2                                			=> r0,
								SALT32.WithinEditN(L1,R1,edit1) and SALT32.WithinEditN(L2,R2,edit2) => if(weight1+weight2 > lfor1*100, lspec1 + lspec2, FailedForce), 
								SALT32.WithinEditN(L1,R2,edit1) and SALT32.WithinEditN(L2,R1,edit2) => if(weight1+weight2 > lfor1*100, lspec1 + lspec2, FailedForce), 
...
export Match2(
							...
							SALT32.Min0(
								MAX( 
										 Match2L(L1,lspec1,lnne1,lfor1,linit1,ispec1,edit1,L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,R1,R2)
...
shared stringCompare(StrType L1, boolean init, boolean nne, integer edit, StrType R1) := function
	...
	result := map(not nne and (L1 = '' or R1 = '') => false,
							  ...
							  SALT32.WithinEditN(L1,R1,edit) => true , 
							  false);
...
shared Match3L(
   ...
   L1BlankCost := IF ( lfor1 > 0, FailedForce, Failed );
	L23R12 := Match2L(L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,L3,lspec3,lnne3,lfor3,linit3,ispec3,edit3,R1,R2);
	L23R23 := Match2L(L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,L3,lspec3,lnne3,lfor3,linit3,ispec3,edit3,R2,R3);
	L23R13 := Match2L(L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,L3,lspec3,lnne3,lfor3,linit3,ispec3,edit3,R1,R3);
/*	 
	...
	*/
	 L1R1 := SALT32.Min0(Match1L(L1,lspec1,lnne1,lfor1,linit1,ispec1,edit1,R1)); // take force into consideration
	 L1R2 := SALT32.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor2),linit1,ispec1,edit1,R2));
   L1R3 := SALT32.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor3),linit1,ispec1,edit1,R3));
...
export Match3(
							...
							SALT32.Min0(
								MAX(
									 Match3L(trim(L1),lspec1,lnne1,lfor1,linit1,ispec1,edit1,trim(L2),lspec2,lnne2,lfor2,linit2,ispec2,edit2,trim(L3),lspec3,lnne3,lfor3,linit3,ispec3,edit3,trim(R1),trim(R2),trim(R3))
...
shared Match4L(
   ...
   L1BlankCost := IF ( lfor1 > 0, FailedForce, Failed );
	 ...
	 L234R123 := Match3L(L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,L3,lspec3,lnne3,lfor3,linit3,ispec3,edit3,L4,lspec4,lnne4,lfor4,linit4,ispec4,edit4,R1,R2,R3);
	 L234R124 := Match3L(L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,L3,lspec3,lnne3,lfor3,linit3,ispec3,edit3,L4,lspec4,lnne4,lfor4,linit4,ispec4,edit4,R1,R2,R4);
	 L234R134 := Match3L(L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,L3,lspec3,lnne3,lfor3,linit3,ispec3,edit3,L4,lspec4,lnne4,lfor4,linit4,ispec4,edit4,R1,R3,R4);
	 L234R234 := Match3L(L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,L3,lspec3,lnne3,lfor3,linit3,ispec3,edit3,L4,lspec4,lnne4,lfor4,linit4,ispec4,edit4,R2,R3,R4);
	 /*
   iL1R1 := getInitMatchWeight(L1, lspec1, R1, rspec1);	 
	 iL1R2 := getInitMatchWeight(L1, lspec1, R2, rspec2);	 
	 iL1R3 := getInitMatchWeight(L1, lspec1, R3, rspec3);	 
	 ...
	 */
	 ...
	 L1R2 := SALT32.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor2),linit1,ispec1,edit1,R2));
   L1R3 := SALT32.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor3),linit1,ispec1,edit1,R3));
	 L1R4 := SALT32.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor3),linit1,ispec1,edit1,R4));
...
shared Match5L(
   ...
   L1BlankCost := IF ( lfor1 > 0, FailedForce, Failed );
	 ...
	 L2345R1234 := Match4L(L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,L3,lspec3,lnne3,lfor3,linit3,ispec3,edit3,L4,lspec4,lnne4,lfor4,linit4,ispec4,edit4,L5,lspec5,lnne5,lfor5,linit5,ispec5,edit5,R1,R2,R3,R4);
	 L2345R1235 := Match4L(L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,L3,lspec3,lnne3,lfor3,linit3,ispec3,edit3,L4,lspec4,lnne4,lfor4,linit4,ispec4,edit4,L5,lspec5,lnne5,lfor5,linit5,ispec5,edit5,R1,R2,R3,R5);
	 L2345R1245 := Match4L(L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,L3,lspec3,lnne3,lfor3,linit3,ispec3,edit3,L4,lspec4,lnne4,lfor4,linit4,ispec4,edit4,L5,lspec5,lnne5,lfor5,linit5,ispec5,edit5,R1,R2,R4,R5);
	 L2345R1345 := Match4L(L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,L3,lspec3,lnne3,lfor3,linit3,ispec3,edit3,L4,lspec4,lnne4,lfor4,linit4,ispec4,edit4,L5,lspec5,lnne5,lfor5,linit5,ispec5,edit5,R1,R3,R4,R5);
	 L2345R2345 := Match4L(L2,lspec2,lnne2,lfor2,linit2,ispec2,edit2,L3,lspec3,lnne3,lfor3,linit3,ispec3,edit3,L4,lspec4,lnne4,lfor4,linit4,ispec4,edit4,L5,lspec5,lnne5,lfor5,linit5,ispec5,edit5,R2,R3,R4,R5);
	 /*
    iL1R1 := getInitMatchWeight(L1, lspec1, R1, rspec1);	 
	 iL1R2 := getInitMatchWeight(L1, lspec1, R2, rspec2);	 
	 iL1R3 := getInitMatchWeight(L1, lspec1, R3, rspec3);	 
...
*/
   ...
   L1R3 := SALT32.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor3),linit1,ispec1,edit1,R3));
	 L1R4 := SALT32.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor3),linit1,ispec1,edit1,R4));
	 L1R5 := SALT32.Min0(Match1L(L1,lspec1,lnne1,MAX(lfor1,lfor3),linit1,ispec1,edit1,R5));
							...
							SALT32.Min0(
								MAX(
									 Match5L(trim(L1),lspec1,lnne1,lfor1,linit1,ispec1,edit1,trim(L2),lspec2,lnne2,lfor2,linit2,ispec2,edit2,
													 ...
													 trim(R1),trim(R2),trim(R3),trim(R4),trim(R5))

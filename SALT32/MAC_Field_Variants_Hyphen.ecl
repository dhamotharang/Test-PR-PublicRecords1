...
NB(SALT32.StrType s) := SALT32.StringFilterOut(s,' -');
...
Fix(SALT32.StrType s) := SALT32.StringCleanSpaces(SALT32.StringSubstituteOut(s,'-',' '));
TYPEOF(fieldvals) apfw(fieldvals le,UNSIGNED c) := TRANSFORM
  SELF.infield := SALT32.GetNthWord(Fix(le.infield),c);
  SELF := le;
  END;
sw0 := DISTRIBUTED(NORMALIZE(fieldvals,SALT32.WordCount(Fix(LEFT.infield)),apfw(LEFT,COUNTER)),ufield); // Get each word

...
functionmacro
  ...
  dGetWuidsInfo := project(dnormset,transform({string wuid,string state,string realstate,string realstate2,string realjobname,string jobname,string total_thor_time,string timestamp},
    self.wuid             := left.wuid;
    self.state            := trim(wk_ut_dev.get_WUInfo(left.wuid).State_nofail,left,right);
    self.realstate        := if(self.state[1..6] = 'failed' ,'failed' ,self.state);
    self.realstate2       := if(trim(wk_ut_dev.get_WUInfo(left.wuid).State_nofail,left,right)[1..6] = 'failed' ,'failed' ,trim(wk_ut_dev.get_WUInfo(left.wuid).State_nofail,left,right));//do it twice to make sure
    self.realjobname      := trim(wk_ut_dev.get_WUInfo(left.wuid).JobName_nofail);
    self.jobname          := if(self.realjobname = ''  ,left.wuid  ,self.realjobname + ' (' + left.wuid + ')');
    self.total_thor_time  := wk_ut_dev.get_TotalTime(left.wuid,,true);  
    self.timestamp        := ut.GetTimeDate();
  ));
  
  // getstate      := trim(wk_ut_dev.get_State(watchworkunit),left,right);
  ...
  dothis := sequential(
     output(dGetWuidsInfo)
    ,output('Checked on ' + ut.GetTimeDate())
    ,iff(AreAllWuidsFinished = true
      ,sequential(doNotify,FAIL('Watcher stopped, Notify event has been triggered because wuids have completed/failed/aborted on ' + ut.GetTimeDate() + ' with this errors: ' /*+ getstate*/)))
  );

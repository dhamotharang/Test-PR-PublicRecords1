...
module
  ...
  export MailPort     := 25;
	...
	export IsDev := if(regexfind('Dataland', _Control.ThisEnvironment.Name, nocase)
													,true		
													,false	
											);
  
  export DevEsps      := ['10.241.3.240','10.241.3.241','10.241.3.242','10.241.3.243'];
  export ProdEsps     := ['prod_esp.br.seisint.com','10.173.84.202','10.173.85.202','10.241.20.202','10.241.30.202','10.173.84.205'];
  
  export LocalEsp    := if(IsDev  ,'10.241.3.242'   ,'prod_esp.br.seisint.com' );
  export LocalEsps   := if(IsDev  ,DevEsps          ,ProdEsps     );
  
  export Esp2Hthor(string pEsp) := if(pEsp in DevEsps ,'infinband_hthor'  ,'hthor');

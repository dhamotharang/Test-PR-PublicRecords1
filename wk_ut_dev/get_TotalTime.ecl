...
function
  ...
  total_thor_time := if(pDoNotFail   = false  ,wk_ut_dev.get_WUInfo(wuid,pesp).totalthortime  ,wk_ut_dev.get_WUInfo(wuid,pesp).totalthortime_nofail );
  dtimes          := if(pDoNotFail   = false  ,wk_ut_dev.get_WUInfo(wuid,pesp).dnormTimers    ,wk_ut_dev.get_WUInfo(wuid,pesp).dnormTimers_nofail   );

  thor_time_secs := wk_ut_dev.Convert2Seconds(total_thor_time);
  ...
  dcopytimes := project(dtimes,transform({recordof(left),string total_seconds,string rtotal_secs,string total_pretty,string rtotal_pretty}
    ,self.total_seconds   := (string)wk_ut_dev.Convert2Seconds(left.value)
    ,self.total_pretty    := wk_ut_dev.ConvertSecs2ReadableTime((real8)self.total_seconds)
  ...
  diterate := iterate(dsort,transform(recordof(left),self.rtotal_secs := (string)((real8)left.rtotal_secs + (real8)right.rtotal_secs),self.rtotal_pretty := wk_ut_dev.ConvertSecs2ReadableTime((real8)self.rtotal_secs),self := right));
  ...
  total_time        := wk_ut_dev.ConvertSecs2ThorTime(total_time_secs);
  total_graph_time  := wk_ut_dev.ConvertSecs2ReadableTime(sum(dgraphtimes,(real8)total_seconds));
  total_dfu_time    := wk_ut_dev.ConvertSecs2ReadableTime(sum(ddfutimes,(real8)total_seconds));

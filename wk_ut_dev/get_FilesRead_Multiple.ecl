...
function
  ...
  drunningwuids := global(nothor(wk_ut_dev.get_Running_Workunits(pLowWuid,pHighWuid,pState)));
  ...
  dproj3 := project(dproj2,transform(recordof(left),self.job := wk_ut_dev.get_Jobname(trim(left.wuid,all)),self := left));

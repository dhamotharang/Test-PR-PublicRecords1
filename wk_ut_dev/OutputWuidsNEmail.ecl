...
function
  ...
  getstate1       := wk_ut_dev.get_State(pWuid,pEsp);    
  realstate1      := if(getstate1[1..6] = 'failed' ,'failed' ,getstate1);
  thor_time1      := wk_ut_dev.get_TotalTime(pWuid,pEsp); 
  thor_time_secs  := wk_ut_dev.Convert2Seconds(thor_time1);
  jobname1        := wk_ut_dev.get_Jobname(pWuid,pEsp);   
  ...
  dWUDetails1 := dataset([{jobname1 ,pWuid ,getstate1 ,piteration ,pversion ,thor_time1,thor_time_secs,Run_Total_Thor_Time,Run_Total_Time_secs}] ,layouts.wks_slim);
  ...
  childwuids := if(wk_ut_dev.get_Scalar_Result(pWuid,'Workunits',pEsp) != '' and pESP in _constants.LocalEsps,wk_ut_dev.get_DS_Result(pWuid,'Workunits',wk_ut_dev.layouts.wks_slim),dataset([],wk_ut_dev.layouts.wks_slim));
  ...
  doit :=  iff(pWuid != '',sequential(output(dWUDetails1 + iterwuids,named('Workunits'),EXTEND),if(pShouldEmail ,sendemail1)));

/*NOT USED AT THIS TIME, look at wk_ut_dev.CreateWuidNWait*/
...
EXPORT mac_CreateWuidNWait(
  ...
  ,pcluster      = wk_ut_dev._Constants.Groupname(,true)
...
functionmacro
  ...
  #APPEND(ECL ,'createwatcherworkunit1 := tools.mod_Soapcalls.fSubmitNewWorkunit(\'\'wk_ut_dev.mac_Watcher(\\\'\' + createworkunit1 + \'\\\',\\\'' + %'workunitevent'% + '\\\',\\\'' + pNotifyEmails + '\\\',\\\'5\\\');\'\',watchercluster,watchercluster,pESP,\'8010\') : independent; // add independent so that they don\'t get reevalulated each time they are accessed\n')
  ...
  getstate1   := wk_ut_dev.get_WUInfo(createworkunit1).State;    
  realstate1  := if(getstate1[1..6] = 'failed' ,'failed' ,getstate1);
  thor_time1  := wk_ut_dev.get_TotalTime(createworkunit1);    
  jobname1    := wk_ut_dev.get_WUInfo(createworkunit1).JobName;   
  
  #SET(CNTR ,1)
  #SET(SUMMLAYOUT ,'{layouts.wks_slim')
  #SET(SUMMROW    ,'{jobname1 ,createworkunit1 ,getstate1 ,piteration ,pversion ,thor_time1,ut.GetTimeDate(),createwatcherworkunit1,' + %'workunitevent'%)
  #SET(EXTRAEMAILINFO ,'')
  ...
  #LOOP  
    ...
    #APPEND(SUMMROW ,',wk_ut_dev.get_Scalar_Result(' + createworkunit1 + ',\'' + pSetResults[%CNTR%] + '\') ')
    #SET(CNTR ,%CNTR% + 1)
  #END
  #APPEND(SUMMLAYOUT ,'}')
  #APPEND(SUMMROW ,'}')

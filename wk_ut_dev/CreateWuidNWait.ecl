...
function
  ...
  createworkunit1 := wk_ut_dev.CreateWuid(
     ecl
    ,pcluster
    ,pESP
  ...
  createwatcherworkunit1 := wk_ut_dev.CreateWuid(
     'wk_ut_dev.mac_notify(\'' + createworkunit1 + '\',\'' + %'workunitevent'% + '\',\'' + pNotifyEmails + '\',\'' + pPollingFrequency + '\',\'' + localesp + '\');'
    ,watchercluster
    ,pESP
  ); // add independent so that they don't get reevalulated each time they are accessed
  ...
  getstate1       := wk_ut_dev.get_State(createworkunit1,pESP);    
  ...
  thor_time1      := wk_ut_dev.get_TotalTime(createworkunit1,pESP); 
  thor_time_secs  := wk_ut_dev.Convert2Seconds(thor_time1);
  jobname1        := wk_ut_dev.get_Jobname(createworkunit1,pESP);   
  Errors          := wk_ut_dev.get_Errors(createworkunit1,pESP);
  ...
  dWUDetails1 := dataset([{jobname1 ,createworkunit1 ,getstate1 ,piteration ,pversion ,thor_time1,thor_time_secs,Run_Total_Thor_Time,Run_Total_Time_secs,'',0.0,ut.GetTimeDate()}] ,layouts.wks_slim);
  jobname2 := if(jobname1 != '' ,jobname1 ,createworkunit1);
  sendemail1 := wk_ut_dev.Send_Email(
                             pNotifyEmails
                            ,jobname2 + ' has ' + getstate1 + ' in ' + thor_time1 + ' on ' + _Control.ThisEnvironment.Name + ' on this date: ' + ut.GetTimeDate()
                            , if(jobname1 != ''
                            , 'Jobname        : ' + jobname1 + '\n','')
  ...
  childwuids := if(wk_ut_dev.get_Scalar_Result(createworkunit1,'Workunits',pESP) != '' /*and pESP in _constants.LocalEsps*/,wk_ut_dev.get_DS_Result(createworkunit1 ,'Workunits',wk_ut_dev.layouts.wks_slim,pESP),dataset([],wk_ut_dev.layouts.wks_slim));

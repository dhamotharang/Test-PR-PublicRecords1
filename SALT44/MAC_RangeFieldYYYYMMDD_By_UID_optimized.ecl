...
%infile_parse% := RECORD
  SALT44.UIDType ufield := infile.ufield;
...
%idata_prep_days% := RECORD
 %idata_prepped%;
 UNSIGNED4 dfs_int_low  := SALT44.fn_YYYYMMDD_to_Int(%idata_prepped%.dfs);
 UNSIGNED4 dfs_int_high := SALT44.fn_YYYYMMDD_to_Int((UNSIGNED8)(%idata_prepped%.dfs_year * 10000 + %idata_prepped%.dfs_month * 100 + %idata_prepped%.dfs_day_high));
 UNSIGNED4 dls_int_low  := SALT44.fn_YYYYMMDD_to_Int((UNSIGNED8)(%idata_prepped%.dls_year * 10000 + %idata_prepped%.dls_month * 100 + 1));
 UNSIGNED4 dls_int_high := SALT44.fn_YYYYMMDD_to_Int(%idata_prepped%.dls);
END;
...
%range_ent_rec% := RECORD 
	...
	SALT44.UIDType ufield;
END;
// List entities that span a whole year
#uniquename(tr_YYYY)
%range_ent_rec% %tr_YYYY%(%idata_prepped% le,INTEGER c) := TRANSFORM
	SELF.infield_beg := SALT44.fn_YYYYMMDD_to_Int(c* 10000 + 101); // Jan 1st of this year
	SELF.infield_end := SALT44.fn_YYYYMMDD_to_Int(c* 10000 + 1231); // Dec 31st of this year
...
%range_ent_rec% %tr_YYYYMM_dfs%(%idata_prepped% le,INTEGER c) := TRANSFORM
	SELF.infield_beg := SALT44.fn_YYYYMMDD_to_Int(le.dfs_year * 10000 + c * 100 + 1 ); // 1st of this month
	SELF.infield_end := SALT44.fn_YYYYMMDD_to_Int(le.dfs_year * 10000 + c * 100 + end_of_month_day(c,le.dfs_year)); // Last date of this month
...
%range_ent_rec% %tr_YYYYMM_dls%(%idata_prepped% le,INTEGER c) := TRANSFORM
	SELF.infield_beg := SALT44.fn_YYYYMMDD_to_Int(le.dls_year * 10000 + c * 100 + 1 ); // 1st of this month
	SELF.infield_end := SALT44.fn_YYYYMMDD_to_Int(le.dls_year * 10000 + c * 100+ end_of_month_day(c,le.dls_year)); // Last date of this month
	SELF.ufield := le.ufield;

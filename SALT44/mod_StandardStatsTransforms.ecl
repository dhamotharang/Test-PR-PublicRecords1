...
EXPORT mod_StandardStatsTransforms := MODULE
	EXPORT mac_hygieneSummaryTransform(mymod, fieldsAttr, inLayout, hasSourceField, outputECLText = FALSE) := MACRO
		...
		#SET(currentField, '');
		#SET(currentFlat, '');
		#APPEND(outECL, 'SALT44.Layout_Stats_Standard.Main xSummary(' + inLayout + 'le, INTEGER cnt, UNSIGNED useTimestamp, STRING suffix = \'\', STRING statCategoryMore = \'\') := TRANSFORM\n');
	...
	EXPORT mac_hygieneTotalRecs(dsSummaryIn, useTimestamp, statnameSuffixIn, hasSourceField, statCategoryMoreIn) := FUNCTIONMACRO
		SALT44.Layout_Stats_Standard.Main xTotalRecs(dsSummaryIn le) := TRANSFORM
	...
	EXPORT hygieneAllProfiles(DATASET(SALT44.MAC_Character_Counts.ResultLine_Layout) allProfilesIn, UNSIGNED useTimestamp, UNSIGNED1 numExamples, STRING statCategoryMoreIn = '') := FUNCTION
		SALT44.Layout_Stats_Standard.Main xAllProfiles(allProfilesIn le, INTEGER cnt) := TRANSFORM
			SELF.dateTimeStamp := useTimestamp;
			SELF.wuid := WORKUNIT;
			...
			SELF.dsExamples := IF(cnt = 1, PROJECT(SORT(le.Frequent_Terms, -cnt)[1..numExamples], TRANSFORM(SALT44.Layout_Stats_Standard.Examples, SELF := LEFT)));
			SELF := [];
		END;
	...
	EXPORT mac_scrubsSummaryStatsFieldErrTransform(mymod, fieldsAttr, inLayout, srcField, outputECLText = FALSE) := MACRO
		...
		#APPEND(outECL, 'SALT44.Layout_Stats_Standard.Main xSummaryStats(' + inLayout + 'le, INTEGER cnt, UNSIGNED useTimestamp, STRING suffixIn, STRING statCategoryMoreIn = \'\') := TRANSFORM\n');
		#APPEND(outECL, '  SELF.dateTimeStamp := useTimeStamp;\n');
		#APPEND(outECL, '  SELF.wuid := WORKUNIT;\n');
		#IF(TRIM(srcField) > '')
	...
	EXPORT scrubsSummaryStatsGeneral(scrubsSummaryIn, srcField = '', useTimeStamp, suffixIn, statCategoryMoreIn) := FUNCTIONMACRO
		SALT44.Layout_Stats_Standard.Main xGeneralStats(scrubsSummaryIn le, INTEGER cnt) := TRANSFORM
			SELF.dateTimeStamp := useTimestamp;
			SELF.wuid := WORKUNIT;
	...
	EXPORT mac_deltaDifferencesFieldChanges(inLayout, useFields, useTimeStamp, hasSourceField, outputECLText = FALSE) := MACRO
		...
		#SET(statvalECL, '');
		#SET(nField, 1);
		#APPEND(outECL, 'SALT44.Layout_Stats_Standard.Main xByRIDFieldChanges(' + inLayout + 'le, INTEGER cnt, UNSIGNED useTimestamp, STRING suffixIn = \'\', STRING statCategoryMoreIn = \'\') := TRANSFORM\n');
		#APPEND(outECL, '  SELF.dateTimeStamp := useTimeStamp;\n');
		#APPEND(outECL, '  SELF.wuid := WORKUNIT;\n');
		#IF(hasSourceField)
			#APPEND(outECL, '  SELF.source := le.srcField;\n');
	...
	EXPORT mac_deltaSourceRidStats(inLayout, useFields, useTimeStamp, srcField, srcRIDField, outputECLText = FALSE) := MACRO
		...
		#SET(statvalECL, '');
		#SET(nField, 1);
		#APPEND(outECL, 'SALT44.Layout_Stats_Standard.Main xBySrcRIDFieldChanges(' + inLayout + 'le, INTEGER cnt, UNSIGNED useTimestamp, STRING suffixIn = \'\', STRING statCategoryMoreIn = \'\') := TRANSFORM\n');
		#APPEND(outECL, '  SELF.dateTimeStamp := useTimeStamp;\n');
		#APPEND(outECL, '  SELF.wuid := WORKUNIT;\n');
	...
	EXPORT MAC_ingestInfileOverallCount(cntIn, labelIn, useTimeStamp) := FUNCTIONMACRO
		SALT44.Layout_Stats_Standard.Main xCnt() := TRANSFORM
			SELF.dateTimeStamp := useTimestamp;
			SELF.wuid := WORKUNIT;
	...
	EXPORT MAC_ingestInfileSourceCounts(ingestSrcCntsIn, srcField, useTimeStamp) := FUNCTIONMACRO
		SALT44.Layout_Stats_Standard.Main xSrcCnts(ingestSrcCntsIn le) := TRANSFORM
			SELF.dateTimeStamp := useTimestamp;
			SELF.wuid := WORKUNIT;
			#IF(TRIM(#TEXT(srcField)) > '')
				SELF.source := le.srcField;
	...
	EXPORT MAC_ingestStatus(ingestStatusStatsIn, srcField = '', useTimeStamp) := FUNCTIONMACRO
		SALT44.Layout_Stats_Standard.Main xIngestStatus(ingestStatusStatsIn le) := TRANSFORM
			SELF.dateTimeStamp := useTimestamp;
			SELF.wuid := WORKUNIT;
			#IF(TRIM(#TEXT(srcField)) > '')
				SELF.source := le.srcField;

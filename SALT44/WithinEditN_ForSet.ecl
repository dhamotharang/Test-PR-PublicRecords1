...
namespace ns_SALT44_WithinEditN_ForSet {
...
using namespace ns_SALT44_WithinEditN_ForSet;
#body
#option pure
{	
  ...
  const char * cur = (const char *)l;	
	...
	while (cur != end)
	{
		...
		if (ns_SALT44_WithinEditN_ForSet::utWithinEditN_WithLengths(cur,len,r,lenR,ed,mode))
			return index;
		cur += len;	
		index++;
	}

...
NB(SALT44.StrType s) := SALT44.StringFilterOut(s,' -');
...
Fix(SALT44.StrType s) := SALT44.StringCleanSpaces(SALT44.StringSubstituteOut(s,'-',' '));
TYPEOF(fieldvals) apfw(fieldvals le,UNSIGNED c) := TRANSFORM
  SELF.infield := SALT44.GetNthWord(Fix(le.infield),c);
  SELF := le;
  END;
sw0 := DISTRIBUTED(NORMALIZE(fieldvals,SALT44.WordCount(Fix(LEFT.infield)),apfw(LEFT,COUNTER)),ufield); // Get each word

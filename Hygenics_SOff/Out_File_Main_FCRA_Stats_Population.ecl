EXPORT Out_File_Main_FCRA_Stats_Population(pMain
																					,pOffenses
																					,pVersion
																					,zOut) := MACRO
...
%dPopulationStats_SexOffender_file_Main% := table(pMain(seisint_primary_key[1..4] not in hygenics_search.Sex_Offenders_Not_Updating.SO_By_Key and source_file not in Hygenics_search.Sex_Offenders_Not_Updating.SO_By_Source)
											     ,%rPopulationStats_SexOffender_file_Main%
											     ,orig_state_code
											     ,source_file
											     ,few);
...
%dPopulationStats_File_Offenses% := table(pOffenses(seisint_primary_key[1..4] not in hygenics_search.Sex_Offenders_Not_Updating.SO_By_Key and source_file not in Hygenics_search.Sex_Offenders_Not_Updating.SO_By_Source)
													,%rPopulationStats_File_Offenses%

...
EXPORT IParam := MODULE
	...
	EXPORT getSearchParams(iesp.bair_lpr.t_BAIRLPRSearchBy searchBy) := FUNCTION
		
		base_params := BairRx_Common.IParam.getSearchParams(searchBy, BairRx_Common.Constants.SearchType.LPR);
		
		BairRx_Common.Layouts.ModeContext xtModeContext() := TRANSFORM						
			SELF.mID := BairRx_Common.Constants.Mode.LPR,
			SELF.codes := BairRx_Common.Functions.SplitClassCodes(BairRx_Common.Constants.Mode.LPR, searchBy.Classification),
			SELF.clean_codes := BairRx_Common.Functions.SplitClassCodes(BairRx_Common.Constants.Mode.LPR, searchBy.Classification, TRUE),
		...
		BairRx_Common.Layouts.ActiveModes xtActiveMode() := TRANSFORM
			SELF.LPR := exists(inModeContext[1].codes);
		END;

...
FUNCTION
  ...
  dWaterfallPhones := IF(inMod.UseInhousePhones, dWaterfallPhones_pre, dWaterfallPhones_pre(subj_phone_type_new = MDR.sourceTools.src_EQUIFAX));
    ...
    SELF.lname             := IF(ri.subj_last <> '', ri.subj_last,ri.p_name_last);
		...
		SELF.dt_first_seen     := CASE(LENGTH(TRIM(ri.subj_date_first)),
                                    8 => ri.subj_date_first,
                                    6 => ri.subj_date_first + '01',
                                    '');
		SELF.dt_last_seen      := CASE(LENGTH(TRIM(ri.subj_date_last)),
                                    8 => ri.subj_date_last,
                                    6 => ri.subj_date_last + '01',
                                    '');

IMPORT Autokey_Batch,BatchShare,Gateway,MDR,Phones,PhoneFinder_Services,Royalty,Suppress;
...
MODULE
	...
	SHARED dSearchRecs		:= UNGROUP(dPhoneSearchResults + dWaterfallResults);

	// accudata_ocn gateway porting info
  ...
  SHARED accu_porting := PhoneFinder_Services.GetAccuDataPhones.GetAccuData_Ocn_PortingData(ds_in_accu,
	...
	dSearchResultsUnfiltered := IF((inMod.IncludePhoneMetadata AND EXISTS(dSearchRecs))
																					OR (inMod.TransactionType = PhoneFinder_Services.Constants.TransType.PhoneRiskAssessment AND EXISTS(dInPhone))
																																	...
																																	,dSearchRecs);
	PhoneSearchResults:=UNGROUP(dSearchResultsUnfiltered(acctno IN SET(dInPhone,acctno)));
	DidSearchResults	:=UNGROUP(dSearchResultsUnfiltered(acctno NOT IN SET(dInPhone,acctno)));
	...
	SHARED dResults_accu_ocn:= dResultsDedup + accu_royalty;	
	
	SHARED dResultsDedupQSent := dResults_accu_ocn + dSearchRecs(typeflag = Phones.Constants.TypeFlag.DataSource_PV);
	...
	dRoylatiesAccu_OCN    := dedup(sort(dResults_accu_ocn,acctno, typeflag), acctno, typeflag);
	dRoyaltiesAccudata_ocn:= Royalty.RoyaltyAccudata.GetBatchRoyaltiesByAcctno(dAppendDIDs,dRoylatiesAccu_OCN ,typeflag); // accudata_ocn royalty

...
EXPORT SALT33.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'alpha','number','upper','cname');
EXPORT FieldTypeNum(SALT33.StrType fn) := CASE(fn,'alpha' => 1,'number' => 2,'upper' => 3,'cname' => 4,0);
...
EXPORT MakeFT_alpha(SALT33.StrType s0) := FUNCTION
  ...
  RETURN  s2;
END;
EXPORT InValidFT_alpha(SALT33.StrType s) := WHICH(SALT33.stringtouppercase(s)<>s,LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZ'))));
EXPORT InValidMessageFT_alpha(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotCaps,SALT33.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZ'),SALT33.HygieneErrors.Good);
...
EXPORT MakeFT_upper(SALT33.StrType s0) := FUNCTION
  s1 := SALT33.stringtouppercase(s0); // Force to upper case
  RETURN  s1;
END;
EXPORT InValidFT_upper(SALT33.StrType s) := WHICH(SALT33.stringtouppercase(s)<>s);
EXPORT InValidMessageFT_upper(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotCaps,SALT33.HygieneErrors.Good);
 
EXPORT MakeFT_cname(SALT33.StrType s0) := FUNCTION
  s1 := SALT33.stringtouppercase(s0); // Force to upper case
  ...
  RETURN  s3;
END;
EXPORT InValidFT_cname(SALT33.StrType s) := WHICH(SALT33.stringtouppercase(s)<>s,LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789\' <>{}[]-^=!+&,./"'))),~fn_cname(s));
EXPORT InValidMessageFT_cname(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotCaps,SALT33.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789\' <>{}[]-^=!+&,./"'),SALT33.HygieneErrors.CustomFail('fn_cname'),SALT33.HygieneErrors.Good);
...
EXPORT Make_st(SALT33.StrType s0) := MakeFT_alpha(s0);
EXPORT InValid_st(SALT33.StrType s) := InValidFT_alpha(s);
...
EXPORT Make_contact_ssn(SALT33.StrType s0) := MakeFT_number(s0);
EXPORT InValid_contact_ssn(SALT33.StrType s) := InValidFT_number(s);
EXPORT InValidMessage_contact_ssn(UNSIGNED1 wh) := InValidMessageFT_number(wh);
 
EXPORT Make_company_fein(SALT33.StrType s0) := s0;
EXPORT InValid_company_fein(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_company_fein(UNSIGNED1 wh) := '';
 
EXPORT Make_active_enterprise_number(SALT33.StrType s0) := s0;
EXPORT InValid_active_enterprise_number(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_active_enterprise_number(UNSIGNED1 wh) := '';
 
EXPORT Make_active_domestic_corp_key(SALT33.StrType s0) := s0;
EXPORT InValid_active_domestic_corp_key(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_active_domestic_corp_key(UNSIGNED1 wh) := '';
 
EXPORT Make_cnp_name(SALT33.StrType s0) := s0;
EXPORT InValid_cnp_name(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_cnp_name(UNSIGNED1 wh) := '';
 
EXPORT Make_corp_legal_name(SALT33.StrType s0) := s0;
EXPORT InValid_corp_legal_name(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_corp_legal_name(UNSIGNED1 wh) := '';
 
EXPORT Make_address(SALT33.StrType s0) := s0;
EXPORT InValid_address(SALT33.StrType addr1,SALT33.StrType csz) := FALSE;
...
EXPORT Make_addr1(SALT33.StrType s0) := s0;
EXPORT InValid_addr1(SALT33.StrType prim_range,SALT33.StrType prim_name,SALT33.StrType sec_range) := FALSE;
...
EXPORT Make_csz(SALT33.StrType s0) := s0;
EXPORT InValid_csz(SALT33.StrType v_city_name,SALT33.StrType st,SALT33.StrType zip) := FALSE;
...
EXPORT Make_cnp_translated(SALT33.StrType s0) := s0;
EXPORT InValid_cnp_translated(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_cnp_translated(UNSIGNED1 wh) := '';
 
EXPORT Make_cnp_classid(SALT33.StrType s0) := s0;
EXPORT InValid_cnp_classid(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_cnp_classid(UNSIGNED1 wh) := '';
 
EXPORT Make_company_bdid(SALT33.StrType s0) := s0;
EXPORT InValid_company_bdid(SALT33.StrType s) := FALSE;
...
EXPORT Make_company_name(SALT33.StrType s0) := MakeFT_cname(s0);
EXPORT InValid_company_name(SALT33.StrType s) := InValidFT_cname(s);
...
EXPORT Make_contact_phone(SALT33.StrType s0) := s0;
EXPORT InValid_contact_phone(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_contact_phone(UNSIGNED1 wh) := '';
 
EXPORT Make_contact_email(SALT33.StrType s0) := MakeFT_upper(s0);
EXPORT InValid_contact_email(SALT33.StrType s) := InValidFT_upper(s);
EXPORT InValidMessage_contact_email(UNSIGNED1 wh) := InValidMessageFT_upper(wh);
 
EXPORT Make_contact_job_title_raw(SALT33.StrType s0) := s0;
EXPORT InValid_contact_job_title_raw(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_contact_job_title_raw(UNSIGNED1 wh) := '';
 
EXPORT Make_company_department(SALT33.StrType s0) := s0;
EXPORT InValid_company_department(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_company_department(UNSIGNED1 wh) := '';
 
EXPORT Make_dt_first_seen(SALT33.StrType s0) := s0;
EXPORT InValid_dt_first_seen(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_dt_first_seen(UNSIGNED1 wh) := '';
 
EXPORT Make_dt_last_seen(SALT33.StrType s0) := s0;
EXPORT InValid_dt_last_seen(SALT33.StrType s) := FALSE;
...
EXPORT UIDConsistency(infile) := FUNCTIONMACRO
  ...
  EXPORT DOTid_Clusters := SALT33.MOD_ClusterStats.Counts(f,DOTid);
  EXPORT proxid_Clusters := SALT33.MOD_ClusterStats.Counts(f,proxid);
  EXPORT lgid3_Clusters := SALT33.MOD_ClusterStats.Counts(f,lgid3);
 ...
 // Children with two parents
  ...
  EXPORT rcid_Twoparents := DEDUP(JOIN(f_thin,f_thin,LEFT.rcid=RIGHT.rcid AND LEFT.DOTid>RIGHT.DOTid,TRANSFORM({SALT33.UIDType DOTid1,SALT33.UIDType rcid,SALT33.UIDType DOTid2},SELF.DOTid1:=LEFT.DOTid,SELF.DOTid2:=RIGHT.DOTid,SELF.rcid:=LEFT.rcid),HASH),WHOLE RECORD,ALL);
    f_thin := TABLE(f(DOTid<>0,proxid<>0),{DOTid,proxid},DOTid,proxid,MERGE);
  EXPORT DOTid_Twoparents := DEDUP(JOIN(f_thin,f_thin,LEFT.DOTid=RIGHT.DOTid AND LEFT.proxid>RIGHT.proxid,TRANSFORM({SALT33.UIDType proxid1,SALT33.UIDType DOTid,SALT33.UIDType proxid2},SELF.proxid1:=LEFT.proxid,SELF.proxid2:=RIGHT.proxid,SELF.DOTid:=LEFT.DOTid),HASH),WHOLE RECORD,ALL);
    f_thin := TABLE(f(proxid<>0,lgid3<>0),{proxid,lgid3},proxid,lgid3,MERGE);
  EXPORT proxid_Twoparents := DEDUP(JOIN(f_thin,f_thin,LEFT.proxid=RIGHT.proxid AND LEFT.lgid3>RIGHT.lgid3,TRANSFORM({SALT33.UIDType lgid31,SALT33.UIDType proxid,SALT33.UIDType lgid32},SELF.lgid31:=LEFT.lgid3,SELF.lgid32:=RIGHT.lgid3,SELF.proxid:=LEFT.proxid),HASH),WHOLE RECORD,ALL);
    f_thin := TABLE(f(lgid3<>0,orgid<>0),{lgid3,orgid},lgid3,orgid,MERGE);
  EXPORT lgid3_Twoparents := DEDUP(JOIN(f_thin,f_thin,LEFT.lgid3=RIGHT.lgid3 AND LEFT.orgid>RIGHT.orgid,TRANSFORM({SALT33.UIDType orgid1,SALT33.UIDType lgid3,SALT33.UIDType orgid2},SELF.orgid1:=LEFT.orgid,SELF.orgid2:=RIGHT.orgid,SELF.lgid3:=LEFT.lgid3),HASH),WHOLE RECORD,ALL);
 ...
 // Now compute the more involved consistency checks
  ...
  EXPORT Advanced0 := SORT(SALT33.MAC_Pivot(Advanced00,Advanced0Layout), MAP(label='rcid'=>0,label='DOTid'=>1,label='proxid'=>2,label='lgid3'=>3,label='orgid'=>4,5));
  END;
  RETURN m;
ENDMACRO;
END;

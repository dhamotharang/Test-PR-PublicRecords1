...
OSR := OUTPUT(CHOOSEN(BIPV2_DOTID.matches(InFile).MatchSampleRecords,10000),NAMED('MatchSampleRecords'));
OSL := OUTPUT(CHOOSEN(BIPV2_DOTID.matches(InFile).ToSlice,1000),NAMED('SliceOutCandidates'));
EXPORT OutputSamples := PARALLEL(OSR,OSL);// Provide the records!
OMatchSamples := OUTPUT(CHOOSEN(BIPV2_DOTID.matches(InFile).MatchSample,1000),NAMED('MatchSample'));
OBSamples := OUTPUT(CHOOSEN(BIPV2_DOTID.matches(InFile).BorderlineMatchSample,10000),NAMED('BorderlineMatchSample'));
OAS := OUTPUT(CHOOSEN(BIPV2_DOTID.matches(InFile).AlmostMatchSample,10000),NAMED('AlmostMatchSample'));
...
PRPPS := OUTPUT(BIPV2_DOTID.matches(InFile).PrePatchIdCount,NAMED('PreClusterCount'),all);
PRPP := OUTPUT(BIPV2_DOTID.matches(InFile).PreClusters,NAMED('PreClusters'),all);
PPPS := OUTPUT(BIPV2_DOTID.matches(InFile).PostPatchIDCount,NAMED('PostClusterCount'),all);
PPP := OUTPUT(BIPV2_DOTID.matches(InFile).PostClusters,NAMED('PostClusters'),all);
MP := OUTPUT(BIPV2_DOTID.matches(InFile).MatchesPerformed,NAMED('MatchesPerformed'));
SP := OUTPUT(BIPV2_DOTID.matches(InFile).SlicesPerformed,NAMED('SlicesPerformed'));
MPPA := OUTPUT(BIPV2_DOTID.matches(InFile).MatchesPropAssisted * 100 / BIPV2_DOTID.matches(InFile).MatchesPerformed,NAMED('PropagationAssisted_Pcnt'));
MPPR := OUTPUT(BIPV2_DOTID.matches(InFile).MatchesPropRequired * 100 / BIPV2_DOTID.matches(InFile).MatchesPerformed,NAMED('PropagationRequired_Pcnt'));
...
d := DATASET([{BIPV2_DOTID.matches(InFile).PatchingError0,BIPV2_DOTID.matches(InFile).DidsNoRid0,BIPV2_DOTID.matches(InFile).DidsAboveRid0,BIPV2_DOTID.matches(InFile).DuplicateRids0,COUNT(BIPV2_DOTID.match_candidates(InFile).Unlinkables),COUNT(BIPV2_DOTID.specificities(InFile).Rejected_file)}],{INTEGER PatchingError0, INTEGER DidsNoRid0, INTEGER DidsAboveRid0, INTEGER DuplicateRids0, UNSIGNED UnlinkableRecords0,UNSIGNED RecordsRejected0});
EXPORT ValidityStats := OUTPUT(d,NAMED('ValidityStatistics'));
EXPORT DebugKeys := Keys(InFile).BuildAll;
/*CUSTOM*/EXPORT OutputFile := OUTPUT(BIPV2_DOTID.matches(InFile).patched_infile,,BIPV2_Files.files_dotid.FILE_DOT_SALT_OP+iter+'_'+BIPV2.KeySuffix,COMPRESSED);// Change file for each iteration
/*CUSTOM*/EXPORT OutputFileA := OUTPUT(BIPV2_DOTID.matches(InFile).patched_infile,,'~temp::DotID::BIPV2_DOTID::it'+iter+'_'+BIPV2.KeySuffix,OVERWRITE,COMPRESSED);// Change file for each iteration
/*CUSTOM*/export possibleMatches := output(BIPV2_DOTID.matches(InFile).PossibleMatches,,BIPV2_Files.files_dotid.FILE_DOT_SALT_POSSIBLE_MATCH+iter+'_'+BIPV2.KeySuffix,overwrite);// Change file for each iteration
// EXPORT LoopN(UNSIGNED N,UNSIGNED mt=31) := LOOP(BIPV2_DOTID.In_DOT,N,BIPV2_DOTID.matches(ROWS(LEFT),mt).patched_infile); // Loop N times
// EXPORT LoopThisN(DATASET(BIPV2_DOTID.Layout_DOT)d,UNSIGNED N,UNSIGNED mt=31) := LOOP(d,N,BIPV2_DOTID.matches(ROWS(LEFT),mt).patched_infile);
// EXPORT LoopThisK(DATASET(BIPV2_DOTID.Layout_DOT)d,UNSIGNED K,UNSIGNED NMax=100) := LOOP(d,COUNTER<=NMax AND COUNT(DEDUP(ROWS(LEFT),DOTid,ALL))>K,BIPV2_DOTID.matches(ROWS(LEFT),0).patched_infile); // Loop until K clusters left
...
/* =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= */
shared fprefix := '~temp::DotID::BIPV2_DOTID::';

...
// Compute the score for the concept addr1
  INTEGER2 addr1_score_ext := SALT33.ClipScore(MAX(addr1_score_pre,0) + prim_range_score + prim_name_score + sec_range_score + MAX(address_score_pre,0));// Score in surrounding context
...
// Compute the score for the concept csz
  INTEGER2 csz_score_ext := SALT33.ClipScore(MAX(csz_score_pre,0) + v_city_name_score + st_score + zip_score + MAX(address_score_pre,0));// Score in surrounding context
...
// Compute the score for the concept address
  INTEGER2 address_score_ext := SALT33.ClipScore(MAX(address_score_pre,0)+ addr1_score + prim_range_score + prim_name_score + sec_range_score+ csz_score + v_city_name_score + st_score + zip_score);// Score in surrounding context
...
mj0 := JOIN( dn0_deduped, dn0_deduped, LEFT.DOTid > RIGHT.DOTid AND ( LEFT.SALT_Partition = RIGHT.SALT_Partition OR LEFT.SALT_Partition='' OR RIGHT.SALT_Partition = '' )
    AND LEFT.cnp_number = RIGHT.cnp_number
    AND LEFT.prim_range = RIGHT.prim_range
    AND LEFT.prim_name = RIGHT.prim_name
    AND LEFT.st = RIGHT.st
    AND LEFT.isContact = RIGHT.isContact
    AND ( ~left.prim_name_isnull AND ~right.prim_name_isnull )
    AND ( ~left.st_isnull AND ~right.st_isnull )
    AND ( left.company_fein = right.company_fein OR left.company_fein_isnull OR right.company_fein_isnull )
    ...
    AND ( ~left.cnp_name_isnull AND ~right.cnp_name_isnull ) AND ( ~left.address_isnull AND ~right.address_isnull )
    AND ( left.sec_range = right.sec_range OR left.sec_range_isnull OR right.sec_range_isnull )
    /* AND ( ~left.lname_isnull AND ~right.lname_isnull ) *//*HACK*/
    /* AND ( ~left.fname_isnull AND ~right.fname_isnull ) *//*HACK*/
    AND ( left.name_suffix = right.name_suffix OR left.name_suffix_isnull OR right.name_suffix_isnull )
...
SHARED ih_thin := TABLE(ih_init,{rcid,DOTid,proxid,lgid3,orgid,ultid});
  ...
  SALT33.MAC_Reassign_UID(ihbp,Cleave(ih).patch_file,DOTid,rcid,ih1); // Perform cleaves
  ut.MAC_Patch_Id(ih1,DOTid,Matches,DOTid1,DOTid2,o); // Join Clusters
  Patchproxid := SALT33.MAC_ParentId_Patch(o,proxid,DOTid);  // Collapse any proxid now joined by DOTid
  Patchlgid3 := SALT33.MAC_ParentId_Patch(Patchproxid,lgid3,proxid);  // Collapse any lgid3 now joined by proxid
  Patchorgid := SALT33.MAC_ParentId_Patch(Patchlgid3,orgid,lgid3);  // Collapse any orgid now joined by lgid3
...
EXPORT id_shift_r := RECORD
    SALT33.UIDType rcid;
    SALT33.UIDType DOTid_before;
    SALT33.UIDType DOTid_after;
    SALT33.UIDType proxid_before;
    SALT33.UIDType proxid_after;
    SALT33.UIDType lgid3_before;
    SALT33.UIDType lgid3_after;

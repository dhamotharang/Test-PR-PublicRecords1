...
EXPORT FromNone(DATASET(Layout_DOT) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.contact_ssn_Invalid := Fields.InValid_contact_ssn((SALT33.StrType)le.contact_ssn);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.st_Invalid << 0 ) + ( le.contact_ssn_Invalid << 2 ) + ( le.zip_Invalid << 3 ) + ( le.company_name_Invalid << 4 ) + ( le.contact_email_Invalid << 6 );
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    SELF.Src :=  le.source;
    UNSIGNED1 ErrNum := CHOOSE(c,le.st_Invalid,le.contact_ssn_Invalid,le.zip_Invalid,le.company_name_Invalid,le.contact_email_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_st(le.st_Invalid),Fields.InvalidMessage_contact_ssn(le.contact_ssn_Invalid),Fields.InvalidMessage_zip(le.zip_Invalid),Fields.InvalidMessage_company_name(le.company_name_Invalid),Fields.InvalidMessage_contact_email(le.contact_email_Invalid),'UNKNOWN'));
    ...
    SELF.FieldName := CHOOSE(c,'st','contact_ssn','zip','company_name','contact_email','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'alpha','number','number','cname','upper','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT33.StrType)le.st,(SALT33.StrType)le.contact_ssn,(SALT33.StrType)le.zip,(SALT33.StrType)le.company_name,(SALT33.StrType)le.contact_email,'***SALTBUG***');
  END;

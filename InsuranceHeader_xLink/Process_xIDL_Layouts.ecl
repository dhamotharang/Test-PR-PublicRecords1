...
IMPORT SALT311,STD,IDL_Header,MDR; /*PHACK01*/
...
SHARED PayloadCand0_asof := SALT311.MAC_DatasetAsOf(PayloadCand0,RID,DID,,DT_EFFECTIVE_FIRST,DT_EFFECTIVE_LAST,,'YYYYMMDD',TRUE);
EXPORT PayloadCand := Scaled_Candidates(PayloadCand0_asof, match_candidates(PayloadCand0_asof, TRUE).candidates, TRUE);
SHARED IncCand_asof := SALT311.MAC_DatasetAsOf(incDS & PayloadCand0,RID,DID,,DT_EFFECTIVE_FIRST,DT_EFFECTIVE_LAST,,'YYYYMMDD',TRUE);
EXPORT IncCand := Scaled_Candidates(IncCand_asof, match_candidates(IncCand_asof, TRUE).candidates, TRUE);
EXPORT FullCand := Scaled_Candidates(File_InsuranceHeader, match_candidates(File_InsuranceHeader, FALSE).candidates, FALSE);
 
EXPORT KeyName := KeyNames().header_logical; /*HACK40*/
 
EXPORT KeyName_sf := KeyNames().header_super; /*HACK10*/
 
EXPORT Key := INDEX(h0,{DID},{h0},KeyName_sf); /*PHACK03*/
...
SHARED changedIDs := IF(incremental, TABLE(DISTRIBUTE(h, DID), {DID}, DID, LOCAL));
 
EXPORT KeyChangeIDName := '~'+KeyPrefix+'::'+'key::InsuranceHeader_xLink'+'::'+Config.KeyInfix+'::DID::changeid';
 
EXPORT KeyChangeIDName_sf := '~'+KeyPrefix+'::'+'key::InsuranceHeader_xLink'+'::'+Config.KeySuperfile+'::DID::changeid';
 
EXPORT KeyChangeID := INDEX(changedIDs,{DID},{},KeyChangeIDName_sf);
 
EXPORT AssignCurrentKeyChangeIDToSuperFile := FileServices.AddSuperFile(KeyChangeIDName_sf,KeyChangeIDName);
 
EXPORT ClearKeyChangeIDSuperFile := SEQUENTIAL(FileServices.CreateSuperFile(KeyChangeIDName_sf,,TRUE),FileServices.ClearSuperFile(KeyChangeIDName_sf));
// Create key to get from historic versions of higher order keys
 
EXPORT KeyIDHistoryName := KeyNames().id_history_logical; /*HACK40a*/
 
EXPORT KeyIDHistoryName_sf := KeyNames().id_history_super; /*PHACK07*/
...
EXPORT Key0Name := KeyNames().header0_logical; /*HACK40b*/
 
EXPORT Key0Name_sf := KeyNames().header0_super; /*PHACK08*/
DataForKey := IF(incremental /*PHACK18*/, Relatives);
 
EXPORT Key0 := INDEX(DataForKey,{DID},{DataForKey},Key0Name_sf);
...
EXPORT Key1Name := KeyNames().header1_logical; /*HACK40c*/
 
EXPORT Key1Name_sf := KeyNames().header1_super; /*PHACK08b*/
DataForKey := IF(incremental /*PHACK18*/, VehicleID);
 
EXPORT Key1 := INDEX(DataForKey,{DID},{DataForKey},Key1Name_sf);
 
EXPORT AssignCurrentKey1ToSuperFile := FileServices.AddSuperFile(Key1Name_sf,Key1Name);
 
EXPORT ClearKey1SuperFile := SEQUENTIAL(FileServices.CreateSuperFile(Key1Name_sf,,TRUE),FileServices.ClearSuperFile(Key1Name_sf));
  infile1 := Key1;
 
EXPORT BuildAll := PARALLEL(BUILDINDEX(Key, KeyName, OVERWRITE),BUILDINDEX(KeyChangeID, KeyChangeIDName, OVERWRITE),BUILDINDEX(KeyIDHistory, KeyIDHistoryName, OVERWRITE),BUILDINDEX(Key0, Key0Name, OVERWRITE),BUILDINDEX(Key1, Key1Name, OVERWRITE));
EXPORT MAC_GenerateMergedKey(in_superfileIn, in_outfileName, in_minDate, in_replaceExisting, in_indexFields, in_payloadFields, in_keyDefName) := FUNCTIONMACRO
  ...
  Layout_modifyDate xModified(childFiles L) := TRANSFORM
    //SELF.modifyDate := Config.DateModified(L.name);
SELF.modifyDate   := (UNSIGNED4)L.name[STD.Str.Find(L.name,'_xlink',1) + 8..IF(STD.Str.Find(L.name,'did',1)> 0 , STD.Str.Find(L.name,'did',1), STD.Str.Find(L.name,'idl',1)) -3]; /*PHACK10*/
  ...
  mergeKeys := SALT311.MAC_BuildKeyMerge('childFiles_use', 'name', Config.MaxMergeFiles, in_indexFields + IF(TRIM(in_payloadFields) > '', ',' + in_payloadFields, ''), in_keyDefName);
...
EXPORT MergeKeyFiles(STRING superFileIn, STRING outfileName, UNSIGNED4 minDate = 0, BOOLEAN replaceExisting = FALSE) := FUNCTION
  fieldListIndex := 'DID';
  fieldListPayload := SALT311.MAC_FieldListCSVFromDataset(h,['DID'],',');
...
EXPORT MergeKeyIDHistoryFiles(STRING superFileIn, STRING outfileName, UNSIGNED4 minDate = 0, BOOLEAN replaceExisting = FALSE) := FUNCTION
  fieldListIndex := 'RID';
  fieldListPayload := SALT311.MAC_FieldListCSVFromDataset(sIDHist,['RID'],',');
...
END;
EXPORT MergeKeyChangeIDFiles(STRING superFileIn, STRING outfileName, UNSIGNED4 minDate = 0, BOOLEAN replaceExisting = FALSE) := FUNCTION
  fieldListIndex := 'DID';
  fieldListPayload := SALT311.MAC_FieldListCSVFromDataset(changedIDs,['DID'],',');
  RETURN MAC_GenerateMergedKey(superFileIn, outfileName, minDate, replaceExisting, fieldListIndex, fieldListPayload, 'KeyChangeID');
END;
EXPORT FullBuildDelta(DATASET(RECORDOF(Key)) keyPayloadExisting = Config.PayloadKey, DATASET(RECORDOF(Key)) keyPayloadNewFull = INDEX(Key,KeyName)) := FUNCTION
  snapshotExisting := SALT311.MAC_DatasetAsOf(keyPayloadExisting, RID, DID,, DT_EFFECTIVE_FIRST, DT_EFFECTIVE_LAST,, 'YYYYMMDD', TRUE);
  snapshotNewFull  := SALT311.MAC_DatasetAsOf(keyPayloadNewFull, RID, DID,, DT_EFFECTIVE_FIRST, DT_EFFECTIVE_LAST,, 'YYYYMMDD', TRUE);
  resultRecs       := SALT311.MAC_PoisonRecords(snapshotExisting, snapshotNewFull, 'DID,RID', DT_EFFECTIVE_FIRST, DT_EFFECTIVE_LAST, 'YYYYMMDD');
  RETURN resultRecs;
END;
...
EXPORT id_stream_historic(DATASET(id_stream_layout) id) := FUNCTION
  ...
  J_rec := PROJECT(SALT311.MAC_DatasetAsOf(J_rec0, RID, DID, UniqueId, DT_EFFECTIVE_FIRST, DT_EFFECTIVE_LAST, , 'YYYYMMDD', FALSE), id_stream_layout);
  ...
  J0 := PROJECT(SALT311.MAC_DatasetAsOf(J00, RID, DID, UniqueId, DT_EFFECTIVE_FIRST, DT_EFFECTIVE_LAST, , 'YYYYMMDD', FALSE), id_stream_layout);
  RETURN J_rec+J0;
END;
EXPORT Fetch_Stream(DATASET(id_stream_layout) d) := FUNCTION
    ...
    DLayout tr(id_stream_layout le, k ri) := TRANSFORM
      ...
      SELF := le;
    END;
    J0 := JOIN( d,k,(LEFT.DID = RIGHT.DID),tr(LEFT,RIGHT), LEFT OUTER, KEEP(Config.JoinLimit)); // Ignore excess records without error
    J1 := SALT311.MAC_DatasetAsOf(J0, RID, DID, UniqueId, DT_EFFECTIVE_FIRST, DT_EFFECTIVE_LAST, , 'YYYYMMDD', FALSE);
...
EXPORT MAC_Fetch_Batch(infile, In_DID, In_UniqueID, asIndex = TRUE) := FUNCTIONMACRO
  ...
  JEntKeyed1 := SALT311.MAC_DatasetAsof(JEntKeyed0, RID, DID, In_UniqueID, DT_EFFECTIVE_FIRST, DT_EFFECTIVE_LAST, , 'YYYYMMDD', FALSE);
  ...
  JEntPull1 := SALT311.MAC_DatasetAsof(JEntPull0, RID, DID, In_UniqueID, DT_EFFECTIVE_FIRST, DT_EFFECTIVE_LAST, , 'YYYYMMDD', TRUE);
  ...
  JRecKeyed_21 := SALT311.MAC_DatasetAsof(JRecKeyed_20, RID, DID, In_UniqueID, DT_EFFECTIVE_FIRST, DT_EFFECTIVE_LAST, , 'YYYYMMDD', FALSE);
  ...
  JRecPull_21 := SALT311.MAC_DatasetAsof(JRecPull_20, RID, DID, In_UniqueID, DT_EFFECTIVE_FIRST, DT_EFFECTIVE_LAST, , 'YYYYMMDD', TRUE);
...
// Below only used in header search (data returning) case
  ...
  SALT311.UIDType Entered_DID := 0; // Allow user to enter DID to pull data
END;
...
EXPORT CleanInput(DATASET(InputLayout) inData) := FUNCTION
  InputLayout CleanT(inData le) := TRANSFORM
    ...
    SELF.SNAME := (TYPEOF(SELF.SNAME))InsuranceHeader_xLink.Fields.Make_SNAME((SALT311.StrType)le.SNAME);
    SELF.FNAME := (TYPEOF(SELF.FNAME))InsuranceHeader_xLink.Fields.Make_FNAME((SALT311.StrType)le.FNAME);
    SELF.MNAME := (TYPEOF(SELF.MNAME))InsuranceHeader_xLink.Fields.Make_MNAME((SALT311.StrType)le.MNAME);
    SELF.LNAME := (TYPEOF(SELF.LNAME))InsuranceHeader_xLink.Fields.Make_LNAME((SALT311.StrType)le.LNAME);
    SELF.DERIVED_GENDER := (TYPEOF(SELF.DERIVED_GENDER))InsuranceHeader_xLink.Fields.Make_DERIVED_GENDER((SALT311.StrType)le.DERIVED_GENDER);
    SELF.PRIM_RANGE := (TYPEOF(SELF.PRIM_RANGE))InsuranceHeader_xLink.Fields.Make_PRIM_RANGE((SALT311.StrType)le.PRIM_RANGE);
    SELF.PRIM_NAME := (TYPEOF(SELF.PRIM_NAME))InsuranceHeader_xLink.Fields.Make_PRIM_NAME((SALT311.StrType)le.PRIM_NAME);
    SELF.SEC_RANGE := (TYPEOF(SELF.SEC_RANGE))InsuranceHeader_xLink.Fields.Make_SEC_RANGE((SALT311.StrType)le.SEC_RANGE);
    SELF.CITY := (TYPEOF(SELF.CITY))InsuranceHeader_xLink.Fields.Make_CITY((SALT311.StrType)le.CITY);
    SELF.ST := (TYPEOF(SELF.ST))InsuranceHeader_xLink.Fields.Make_ST((SALT311.StrType)le.ST);
    SELF.ZIP_cases := le.ZIP_cases;
    SELF.SSN5 := (TYPEOF(SELF.SSN5))InsuranceHeader_xLink.Fields.Make_SSN5((SALT311.StrType)le.SSN5);
    SELF.SSN4 := (TYPEOF(SELF.SSN4))InsuranceHeader_xLink.Fields.Make_SSN4((SALT311.StrType)le.SSN4);
    SELF.DOB := (TYPEOF(SELF.DOB))InsuranceHeader_xLink.Fields.Make_DOB((SALT311.StrType)le.DOB);
    SELF.PHONE := (TYPEOF(SELF.PHONE))le.PHONE;
    SELF.DL_STATE := (TYPEOF(SELF.DL_STATE))InsuranceHeader_xLink.Fields.Make_DL_STATE((SALT311.StrType)le.DL_STATE);
    SELF.DL_NBR := (TYPEOF(SELF.DL_NBR))InsuranceHeader_xLink.Fields.Make_DL_NBR((SALT311.StrType)le.DL_NBR);
    SELF.SRC := (TYPEOF(SELF.SRC))InsuranceHeader_xLink.Fields.Make_SRC((SALT311.StrType)le.SRC);
    SELF.SOURCE_RID := (TYPEOF(SELF.SOURCE_RID))InsuranceHeader_xLink.Fields.Make_SOURCE_RID((SALT311.StrType)le.SOURCE_RID);
    SELF.DT_FIRST_SEEN := (TYPEOF(SELF.DT_FIRST_SEEN))InsuranceHeader_xLink.Fields.Make_DT_FIRST_SEEN((SALT311.StrType)le.DT_FIRST_SEEN);
    SELF.DT_LAST_SEEN := (TYPEOF(SELF.DT_LAST_SEEN))InsuranceHeader_xLink.Fields.Make_DT_LAST_SEEN((SALT311.StrType)le.DT_LAST_SEEN);
    SELF.DT_EFFECTIVE_FIRST := (TYPEOF(SELF.DT_EFFECTIVE_FIRST))InsuranceHeader_xLink.Fields.Make_DT_EFFECTIVE_FIRST((SALT311.StrType)le.DT_EFFECTIVE_FIRST);
    SELF.DT_EFFECTIVE_LAST := (TYPEOF(SELF.DT_EFFECTIVE_LAST))InsuranceHeader_xLink.Fields.Make_DT_EFFECTIVE_LAST((SALT311.StrType)le.DT_EFFECTIVE_LAST);
    SELF.MAINNAME := (TYPEOF(SELF.MAINNAME))InsuranceHeader_xLink.Fields.Make_MAINNAME((SALT311.StrType)le.MAINNAME);
    SELF.FULLNAME := (TYPEOF(SELF.FULLNAME))InsuranceHeader_xLink.Fields.Make_FULLNAME((SALT311.StrType)le.FULLNAME);
    SELF.ADDR1 := (TYPEOF(SELF.ADDR1))InsuranceHeader_xLink.Fields.Make_ADDR1((SALT311.StrType)le.ADDR1);
    SELF.LOCALE := (TYPEOF(SELF.LOCALE))InsuranceHeader_xLink.Fields.Make_LOCALE((SALT311.StrType)le.LOCALE);
    SELF.ADDRESS := (TYPEOF(SELF.ADDRESS))InsuranceHeader_xLink.Fields.Make_ADDRESS((SALT311.StrType)le.ADDRESS);
...
EXPORT LayoutScoredFetch := RECORD // Nulls required for linkpaths that do not have field
  ...
  TYPEOF(Key1.VIN) VIN := (TYPEOF(Key1.VIN))'';
  ...
  Config.KeysBitmapType keys_poisoned := 0; // A bitmap of keys that were poisoned
END;
 
EXPORT LayoutScoredFetch update_forcefailed(LayoutScoredFetch le, BOOLEAN disableForce = FALSE) := TRANSFORM
  SELF.ForceFailed := IF(disableForce, FALSE, (((le.DOB_match_code = SALT311.MatchCode.GenerationNoMatch) OR (le.DOBWeight_year<>0 OR le.DOBWeight_month<>0 OR le.DOBWeight_day<>0) AND (le.DOBWeight < Config.DOB_Force)) AND (le.SSN5Weight=0 OR le.SSN5Weight < Config.DOB_OR1_SSN5_Force) AND (le.SSN4Weight=0 OR le.SSN4Weight < Config.DOB_OR2_SSN4_Force)));
  SELF := le;
END;
 
EXPORT LayoutScoredFetch apply_poison(LayoutScoredFetch le) := TRANSFORM
  SELF.keys_poisoned := le.keys_poisoned;
  SELF.keys_used := le.keys_used;
...
EXPORT isWeightForcedDown(INTEGER2 l_weight,INTEGER2 r_weight, INTEGER1 l_mc=SALT311.MatchCode.NoMatch, INTEGER1 r_mc=SALT311.MatchCode.NoMatch) :=
  ...
  SELF.ZIP_cases := le.ZIP_cases + ri.ZIP_cases; /*ZIPCASESHACK01b*/
  ...
  BOOLEAN DirectMAINNAMEWeightForcedDown := IF ( isWeightForcedDown(le.MAINNAMEWeight,ri.MAINNAMEWeight,le.MAINNAME_match_code,ri.MAINNAME_match_code),TRUE,FALSE );
	BOOLEAN MAINNAMEWeightForcedDown := FNAMEWeightForcedDown OR MNAMEWeightForcedDown OR LNAMEWeightForcedDown; /*HACK41a*/ 
  SELF.MAINNAMEWeight := MAP (
      DirectMAINNAMEWeightForcedDown => IF ( isLeftWinner(le.MAINNAMEWeight,ri.MAINNAMEWeight,le.MAINNAME_match_code,ri.MAINNAME_match_code ), le.MAINNAMEWeight, ri.MAINNAMEWeight ),
		MAINNAMEWeightForcedDown AND (le.MAINNAME_match_code = ri.MAINNAME_match_code) => MIN(le.MAINNAMEWeight, ri.MAINNAMEWeight),/*HACK41b*/
  ...
  SELF.MAINNAME_match_code :=  MAP (
      DirectMAINNAMEWeightForcedDown => IF ( isLeftWinner(le.MAINNAMEWeight,ri.MAINNAMEWeight,le.MAINNAME_match_code,ri.MAINNAME_match_code ), le.MAINNAME_match_code, ri.MAINNAME_match_code ),
		...
		MAINNAMEWeightForcedDown AND (le.MAINNAME_match_code = ri.MAINNAME_match_code) AND MIN(le.MAINNAMEWeight, ri.MAINNAMEWeight)=ri.MAINNAMEWeight => ri.MAINNAME_match_code, /*HACK41c*/
  ...
  BOOLEAN DirectFULLNAMEWeightForcedDown := IF ( isWeightForcedDown(le.FULLNAMEWeight,ri.FULLNAMEWeight,le.FULLNAME_match_code,ri.FULLNAME_match_code),TRUE,FALSE ); 
	BOOLEAN FULLNAMEWeightForcedDown := MAINNAMEWeightForcedDown OR SNAMEWeightForcedDown; /*HACK42a*/
  SELF.FULLNAMEWeight := MAP (
      DirectFULLNAMEWeightForcedDown => IF ( isLeftWinner(le.FULLNAMEWeight,ri.FULLNAMEWeight,le.FULLNAME_match_code,ri.FULLNAME_match_code ), le.FULLNAMEWeight, ri.FULLNAMEWeight ),
		FULLNAMEWeightForcedDown AND (le.FULLNAME_match_code = ri.FULLNAME_match_code) => MIN(le.FULLNAMEWeight, ri.FULLNAMEWeight),/*HACK42b*/
  ...
  SELF.FULLNAME_match_code :=  MAP (
      DirectFULLNAMEWeightForcedDown => IF ( isLeftWinner(le.FULLNAMEWeight,ri.FULLNAMEWeight,le.FULLNAME_match_code,ri.FULLNAME_match_code ), le.FULLNAME_match_code, ri.FULLNAME_match_code ),
		...
		FULLNAMEWeightForcedDown AND (le.FULLNAME_match_code = ri.FULLNAME_match_code) AND MIN(le.FULLNAMEWeight, ri.FULLNAMEWeight)=ri.FULLNAMEWeight => ri.FULLNAME_match_code, /*HACK42c*/
  ...
  BOOLEAN DirectADDR1WeightForcedDown := IF ( isWeightForcedDown(le.ADDR1Weight,ri.ADDR1Weight,le.ADDR1_match_code,ri.ADDR1_match_code),TRUE,FALSE ); 
	BOOLEAN ADDR1WeightForcedDown := PRIM_RANGEWeightForcedDown OR SEC_RANGEWeightForcedDown OR PRIM_NAMEWeightForcedDown; /*HACK43a*/
  SELF.ADDR1Weight := MAP (
      DirectADDR1WeightForcedDown => IF ( isLeftWinner(le.ADDR1Weight,ri.ADDR1Weight,le.ADDR1_match_code,ri.ADDR1_match_code ), le.ADDR1Weight, ri.ADDR1Weight ),
		ADDR1WeightForcedDown AND (le.ADDR1_match_code = ri.ADDR1_match_code) => MIN(le.ADDR1Weight, ri.ADDR1Weight),/*HACK43b*/
  ...
  SELF.ADDR1_match_code :=  MAP (
      DirectADDR1WeightForcedDown => IF ( isLeftWinner(le.ADDR1Weight,ri.ADDR1Weight,le.ADDR1_match_code,ri.ADDR1_match_code ), le.ADDR1_match_code, ri.ADDR1_match_code ),
		...
		ADDR1WeightForcedDown AND (le.ADDR1_match_code = ri.ADDR1_match_code) AND MIN(le.ADDR1Weight, ri.ADDR1Weight)=ri.ADDR1Weight => ri.ADDR1_match_code, /*HACK43c*/
  ...
  BOOLEAN DirectLOCALEWeightForcedDown := IF ( isWeightForcedDown(le.LOCALEWeight,ri.LOCALEWeight,le.LOCALE_match_code,ri.LOCALE_match_code),TRUE,FALSE ); 
	BOOLEAN LOCALEWeightForcedDown := CITYWeightForcedDown OR STWeightForcedDown; /*HACK44a*/
  SELF.LOCALEWeight := MAP (
      DirectLOCALEWeightForcedDown => IF ( isLeftWinner(le.LOCALEWeight,ri.LOCALEWeight,le.LOCALE_match_code,ri.LOCALE_match_code ), le.LOCALEWeight, ri.LOCALEWeight ),
		LOCALEWeightForcedDown AND (le.LOCALE_match_code = ri.LOCALE_match_code) => MIN(le.LOCALEWeight, ri.LOCALEWeight),/*HACK44b*/
  ...
  SELF.LOCALE_match_code :=  MAP (
      DirectLOCALEWeightForcedDown => IF ( isLeftWinner(le.LOCALEWeight,ri.LOCALEWeight,le.LOCALE_match_code,ri.LOCALE_match_code ), le.LOCALE_match_code, ri.LOCALE_match_code ),
		...
		LOCALEWeightForcedDown AND (le.LOCALE_match_code = ri.LOCALE_match_code) AND MIN(le.LOCALEWeight, ri.LOCALEWeight)=ri.LOCALEWeight => ri.LOCALE_match_code, /*HACK44c*/
  ...
  BOOLEAN DirectADDRESSWeightForcedDown := IF ( isWeightForcedDown(le.ADDRESSWeight,ri.ADDRESSWeight,le.ADDRESS_match_code,ri.ADDRESS_match_code),TRUE,FALSE ); 
	BOOLEAN ADDRESSWeightForcedDown := ADDR1WeightForcedDown OR LOCALEWeightForcedDown; /*HACK45a*/
  SELF.ADDRESSWeight := MAP (
      DirectADDRESSWeightForcedDown => IF ( isLeftWinner(le.ADDRESSWeight,ri.ADDRESSWeight,le.ADDRESS_match_code,ri.ADDRESS_match_code ), le.ADDRESSWeight, ri.ADDRESSWeight ),
		ADDRESSWeightForcedDown AND (le.ADDRESS_match_code = ri.ADDRESS_match_code) => MIN(le.ADDRESSWeight, ri.ADDRESSWeight),/*HACK45b*/
  ...
  SELF.ADDRESS_match_code :=  MAP (
      DirectADDRESSWeightForcedDown => IF ( isLeftWinner(le.ADDRESSWeight,ri.ADDRESSWeight,le.ADDRESS_match_code,ri.ADDRESS_match_code ), le.ADDRESS_match_code, ri.ADDRESS_match_code ),
		...
		ADDRESSWeightForcedDown AND (le.ADDRESS_match_code = ri.ADDRESS_match_code) AND MIN(le.ADDRESSWeight, ri.ADDRESSWeight)=ri.ADDRESSWeight => ri.ADDRESS_match_code, /*HACK45c*/
...
EXPORT KeysBitmapDecode(Config.KeysBitmapType bitmap,STRING prefix='') := FUNCTION
  MapLP(UNSIGNED1 code,STRING prefix='') := CASE(code,1=>IF(prefix<>'',prefix,'UBER'),2=>IF(prefix<>'',prefix+'NAME','NAME'),3=>IF(prefix<>'',prefix+'ADDRESS','ADDRESS'),4=>IF(prefix<>'',prefix+'SSN','SSN'),5=>IF(prefix<>'',prefix+'SSN4','SSN4'),6=>IF(prefix<>'',prefix+'DOB','DOB'),7=>IF(prefix<>'',prefix+'DOBF','DOBF'),8=>IF(prefix<>'',prefix+'ZIP_PR','ZIP_PR'),9=>IF(prefix<>'',prefix+'SRC_RID','SRC_RID'),10=>IF(prefix<>'',prefix+'DLN','DLN'),11=>IF(prefix<>'',prefix+'PH','PH'),12=>IF(prefix<>'',prefix+'LFZ','LFZ'),13=>IF(prefix<>'',prefix+'RELATIVE','RELATIVE'),14=>IF(prefix<>'',prefix+'VIN','VIN'),'');
  REC := {STRING txt};
  ds := DATASET([{''}],REC);
  REC decode(REC le, UNSIGNED1 c) := TRANSFORM
    SELF.txt := le.txt+IF((bitmap&(1<<c-1))>0,MapLP(c,prefix),'');
  END;
  N0 := NORMALIZE(ds,13,decode(LEFT,COUNTER));
...
EXPORT AdjustKeysUsedAndFailed(DATASET(LayoutScoredFetch) in_data) := FUNCTION
  LayoutScoredFetch AdjustFlags(LayoutScoredFetch le, Config.KeysBitmapType flagFail, Config.KeysBitmapType flagPoison) := TRANSFORM
    SELF.keys_used := le.keys_used | flagFail | flagPoison;
    SELF.keys_failed := le.keys_failed | flagFail;
    SELF.keys_poisoned := le.keys_poisoned | flagPoison;
    SELF := le;
  END;
  outR := { Config.KeysBitmapType keys_failed; Config.KeysBitmapType keys_poisoned; };
  outR AggregateFlags(LayoutScoredFetch le, outR ri) := TRANSFORM
    SELF.keys_failed := ri.keys_failed | le.keys_failed;
    SELF.keys_poisoned := ri.keys_poisoned | le.keys_poisoned;
  END;
  agg := AGGREGATE(in_data(DID=0,RID=0),outR,AggregateFlags(LEFT,RIGHT),FEW);
  flgFail := agg[1].keys_failed;
  flgPoison := agg[1].keys_poisoned;
  RETURN IF(COUNT(in_data(DID = 0,RID = 0)) > 0, IF(COUNT(in_data(DID <> 0 OR RID <> 0)) > 0, PROJECT(in_data(DID <> 0 OR RID <> 0), AdjustFlags(LEFT, flgFail, flgPoison)), PROJECT(in_data(DID = 0,RID = 0)[1..1], AdjustFlags(LEFT, flgFail, flgPoison))), in_data);
END;
...
aggregateRec := RECORD
  ...
  DERIVED_GENDERWeight := MAX(GROUP,IF( in_data.DERIVED_GENDER_match_code=SALT311.MatchCode.ExactMatch, in_data.DERIVED_GENDERWeight,0 ));
  ...
  SSN5Weight := MAX(GROUP,IF( in_data.SSN5_match_code=SALT311.MatchCode.ExactMatch, in_data.SSN5Weight,0 ));
  SSN4Weight := MAX(GROUP,IF( in_data.SSN4_match_code=SALT311.MatchCode.ExactMatch, in_data.SSN4Weight,0 ));
  ...
  DL_STATEWeight := MAX(GROUP,IF( in_data.DL_STATE_match_code=SALT311.MatchCode.ExactMatch, in_data.DL_STATEWeight,0 ));
  DL_NBRWeight := MAX(GROUP,IF( in_data.DL_NBR_match_code=SALT311.MatchCode.ExactMatch, in_data.DL_NBRWeight,0 ));
  ...
  DT_EFFECTIVE_FIRSTWeight := MAX(GROUP,IF( in_data.DT_EFFECTIVE_FIRST_match_code=SALT311.MatchCode.ExactMatch, in_data.DT_EFFECTIVE_FIRSTWeight,0 ));
  DT_EFFECTIVE_LASTWeight := MAX(GROUP,IF( in_data.DT_EFFECTIVE_LAST_match_code=SALT311.MatchCode.ExactMatch, in_data.DT_EFFECTIVE_LASTWeight,0 ));
  ...
  fname2Weight := MAX(GROUP,IF( in_data.fname2_match_code=SALT311.MatchCode.ExactMatch, in_data.fname2Weight,0 ));
  lname2Weight := MAX(GROUP,IF( in_data.lname2_match_code=SALT311.MatchCode.ExactMatch, in_data.lname2Weight,0 ));
  VINWeight := MAX(GROUP,IF( in_data.VIN_match_code=SALT311.MatchCode.ExactMatch, in_data.VINWeight,0 ));
END;
...
LayoutScoredFetch FixScores(LayoutScoredFetch le, aggregateRec ri) := TRANSFORM
  ...
  SELF.DERIVED_GENDERWeight := MAP( ri.DERIVED_GENDERWeight=0 OR le.DERIVED_GENDER_match_code=SALT311.MatchCode.ExactMatch => le.DERIVED_GENDERWeight,MIN(le.DERIVED_GENDERWeight,ri.DERIVED_GENDERWeight-1) );
  ...
  SELF.SSN5Weight := MAP( ri.SSN5Weight=0 OR le.SSN5_match_code=SALT311.MatchCode.ExactMatch => le.SSN5Weight,MIN(le.SSN5Weight,ri.SSN5Weight-1) );
  SELF.SSN4Weight := MAP( ri.SSN4Weight=0 OR le.SSN4_match_code=SALT311.MatchCode.ExactMatch => le.SSN4Weight,MIN(le.SSN4Weight,ri.SSN4Weight-1) );
  ...
  SELF.DL_STATEWeight := MAP( ri.DL_STATEWeight=0 OR le.DL_STATE_match_code=SALT311.MatchCode.ExactMatch => le.DL_STATEWeight,MIN(le.DL_STATEWeight,ri.DL_STATEWeight-1) );
  SELF.DL_NBRWeight := MAP( ri.DL_NBRWeight=0 OR le.DL_NBR_match_code=SALT311.MatchCode.ExactMatch => le.DL_NBRWeight,MIN(le.DL_NBRWeight,ri.DL_NBRWeight-1) );
  ...
  SELF.DT_EFFECTIVE_FIRSTWeight := MAP( ri.DT_EFFECTIVE_FIRSTWeight=0 OR le.DT_EFFECTIVE_FIRST_match_code=SALT311.MatchCode.ExactMatch => le.DT_EFFECTIVE_FIRSTWeight,MIN(le.DT_EFFECTIVE_FIRSTWeight,ri.DT_EFFECTIVE_FIRSTWeight-1) );
  SELF.DT_EFFECTIVE_LASTWeight := MAP( ri.DT_EFFECTIVE_LASTWeight=0 OR le.DT_EFFECTIVE_LAST_match_code=SALT311.MatchCode.ExactMatch => le.DT_EFFECTIVE_LASTWeight,MIN(le.DT_EFFECTIVE_LASTWeight,ri.DT_EFFECTIVE_LASTWeight-1) );
  ...
  SELF.fname2Weight := MAP( ri.fname2Weight=0 OR le.fname2_match_code=SALT311.MatchCode.ExactMatch => le.fname2Weight,MIN(le.fname2Weight,ri.fname2Weight-1) );
  SELF.lname2Weight := MAP( ri.lname2Weight=0 OR le.lname2_match_code=SALT311.MatchCode.ExactMatch => le.lname2Weight,MIN(le.lname2Weight,ri.lname2Weight-1) );
  SELF.VINWeight := MAP( ri.VINWeight=0 OR le.VIN_match_code=SALT311.MatchCode.ExactMatch => le.VINWeight,MIN(le.VINWeight,ri.VINWeight-1) );
  INTEGER2 Weight := MAX(0, SELF.DERIVED_GENDERWeight) + MAX(0, SELF.SSN5Weight) + MAX(0, SELF.SSN4Weight) + MAX(0, SELF.DOBWeight) + MAX(0, SELF.PHONEWeight) + MAX(0, SELF.DL_STATEWeight) + MAX(0, SELF.DL_NBRWeight) + MAX(0, SELF.SRCWeight) + MAX(0, SELF.SOURCE_RIDWeight) + (MAX(SELF.MAINNAMEWeight, (MAX(0, SELF.FNAMEWeight) + MAX(0, SELF.MNAMEWeight) + MAX(0, SELF.LNAMEWeight))) + MAX(0, SELF.SNAMEWeight)) + ((MAX(0, SELF.PRIM_RANGEWeight) + MAX(0, SELF.SEC_RANGEWeight) + MAX(0, SELF.PRIM_NAMEWeight)) + (MAX(0, SELF.CITYWeight) + MAX(0, SELF.STWeight) + MAX(0, SELF.ZIPWeight))) + MAX(0, SELF.fname2Weight) + MAX(0, SELF.lname2Weight) + MAX(0, SELF.VINWeight);
...
EXPORT CombineAllScores(DATASET(LayoutScoredFetch) in_data, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  END;
  //r0 := ROLLUP( SORT( GROUP( SORT ( DISTRIBUTE(In_Data,HASH(Reference)),Reference, LOCAL ), Reference, LOCAL),DID),LEFT.DID=RIGHT.DID,Combine_Scores(LEFT,RIGHT,In_disableForce))(SALT311.DebugMode OR ~ForceFailed);
  r0 := ROLLUP( SORT( GROUP( SORT ( DISTRIBUTE(In_Data,HASH(Reference)),Reference, LOCAL ), Reference, LOCAL),DID,-Weight,RECORD),LEFT.DID=RIGHT.DID,Combine_Scores(LEFT,RIGHT,In_disableForce))(SALT311.DebugMode OR ~ForceFailed);
  r11:= combine_Zip(UNGROUP(r0));
	R1 := AdjustScoresForNonExactMatches(r11);/*ZIPCASESHACK01c*/
  R12 := InsuranceHeader_xlink.patch_specificities(r1); /*PHACK17*/
  // R12 := R1;
	R2 := ROLLUP( TOPN(R12,100,-Weight),GROUP, Create_Output(LEFT,ROWS(LEFT)) );
  SALT311.MAC_External_AddPcnt(R2,LayoutScoredFetch,Results,OutputLayout_Batch,29,R3);/*HACK16*/
  RETURN r3;
END;
...
// Note - results are returned distributed by HASH(reference) - this is part of the specification
  ...
  rolled := ROLLUP ( SORT( DISTRIBUTE( in_data, HASH(reference) ), Reference, DID, -weight, RECORD, LOCAL), Combine_Scores(LEFT,RIGHT, In_disableForce), Reference, DID, LOCAL);
...
EXPORT KeysUsedToText(UNSIGNED4 k) := FUNCTION
  list := IF(k&1 <>0,'UberKey,','') + IF(k&(1<<1)<>0,'NAME,','') + IF(k&(1<<2)<>0,'ADDRESS,','') + IF(k&(1<<3)<>0,'SSN,','') + IF(k&(1<<4)<>0,'SSN4,','') + IF(k&(1<<5)<>0,'DOB,','') + IF(k&(1<<6)<>0,'DOBF,','') + IF(k&(1<<7)<>0,'ZIP_PR,','') + IF(k&(1<<8)<>0,'SRC_RID,','') + IF(k&(1<<9)<>0,'DLN,','') + IF(k&(1<<10)<>0,'PH,','') + IF(k&(1<<11)<>0,'LFZ,','') + IF(k&(1<<12)<>0,'RELATIVE,','') + IF(k&(1<<13)<>0,'VIN,','');
  RETURN list[1..LENGTH(TRIM(list))-1]; // Strim last ,
end;
EXPORT Layout_RolledEntity := RECORD,MAXLENGTH(63000)
  TYPEOF(h.did) DID; /*PHACK05*/
  DATASET(SALT311.Layout_FieldValueList) DERIVED_GENDER_Values := DATASET([],SALT311.Layout_FieldValueList);
  DATASET(SALT311.Layout_FieldValueList) SSN5_Values := DATASET([],SALT311.Layout_FieldValueList);
  DATASET(SALT311.Layout_FieldValueList) SSN4_Values := DATASET([],SALT311.Layout_FieldValueList);
  ...
  DATASET(SALT311.Layout_FieldValueList) DL_STATE_Values := DATASET([],SALT311.Layout_FieldValueList);
  DATASET(SALT311.Layout_FieldValueList) DL_NBR_Values := DATASET([],SALT311.Layout_FieldValueList);
  ...
  DATASET(SALT311.Layout_FieldValueList) DT_EFFECTIVE_FIRST_Values := DATASET([],SALT311.Layout_FieldValueList);
  DATASET(SALT311.Layout_FieldValueList) DT_EFFECTIVE_LAST_Values := DATASET([],SALT311.Layout_FieldValueList);
  ...
  DATASET(SALT311.Layout_FieldValueList) fname2_Values := DATASET([],SALT311.Layout_FieldValueList);
  DATASET(SALT311.Layout_FieldValueList) lname2_Values := DATASET([],SALT311.Layout_FieldValueList);
  DATASET(SALT311.Layout_FieldValueList) VIN_Values := DATASET([],SALT311.Layout_FieldValueList);
END;
...
SHARED RollEntities(dataset(Layout_RolledEntity) infile) := FUNCTION
  ...
  SELF.DID := le.DID;
    SELF.DERIVED_GENDER_values := SALT311.fn_combine_fieldvaluelist(le.DERIVED_GENDER_values,ri.DERIVED_GENDER_values);
    SELF.SSN5_values := SALT311.fn_combine_fieldvaluelist(le.SSN5_values,ri.SSN5_values);
    SELF.SSN4_values := SALT311.fn_combine_fieldvaluelist(le.SSN4_values,ri.SSN4_values);
    ...
    SELF.DL_STATE_values := SALT311.fn_combine_fieldvaluelist(le.DL_STATE_values,ri.DL_STATE_values);
    SELF.DL_NBR_values := SALT311.fn_combine_fieldvaluelist(le.DL_NBR_values,ri.DL_NBR_values);
    ...
    SELF.DT_EFFECTIVE_FIRST_values := SALT311.fn_combine_fieldvaluelist(le.DT_EFFECTIVE_FIRST_values,ri.DT_EFFECTIVE_FIRST_values);
    SELF.DT_EFFECTIVE_LAST_values := SALT311.fn_combine_fieldvaluelist(le.DT_EFFECTIVE_LAST_values,ri.DT_EFFECTIVE_LAST_values);
    ...
    SELF.fname2_values := SALT311.fn_combine_fieldvaluelist(le.fname2_values,ri.fname2_values);
    SELF.lname2_values := SALT311.fn_combine_fieldvaluelist(le.lname2_values,ri.lname2_values);
    SELF.VIN_values := SALT311.fn_combine_fieldvaluelist(le.VIN_values,ri.VIN_values);
  END;
...
EXPORT RolledEntities(DATASET(RECORDOF(Key)) in_data,DATASET(RECORDOF(Key0)) in_data0,DATASET(RECORDOF(Key1)) in_data1) := FUNCTION
 
Layout_RolledEntity into0(in_data0 le) := TRANSFORM
  SELF.DID := le.DID;
  SELF.fname2_Values := IF ( (SALT311.StrType)le.fname2 = '',DATASET([],SALT311.Layout_FieldValueList),DATASET([{TRIM((SALT311.StrType)le.fname2)}],SALT311.Layout_FieldValueList));
  SELF.lname2_Values := IF ( (SALT311.StrType)le.lname2 = '',DATASET([],SALT311.Layout_FieldValueList),DATASET([{TRIM((SALT311.StrType)le.lname2)}],SALT311.Layout_FieldValueList));
END;
AsFieldValues0 := PROJECT(in_data0,into0(LEFT));
 
Layout_RolledEntity into1(in_data1 le) := TRANSFORM
  SELF.DID := le.DID;
  SELF.VIN_Values := IF ( (SALT311.StrType)le.VIN = '',DATASET([],SALT311.Layout_FieldValueList),DATASET([{TRIM((SALT311.StrType)le.VIN)}],SALT311.Layout_FieldValueList));
END;
AsFieldValues1 := PROJECT(in_data1,into1(LEFT));
 
Layout_RolledEntity into(in_data le) := TRANSFORM
  SELF.DID := le.DID;
  SELF.DERIVED_GENDER_Values := IF ( (SALT311.StrType)le.DERIVED_GENDER = '',DATASET([],SALT311.Layout_FieldValueList),DATASET([{TRIM((SALT311.StrType)le.DERIVED_GENDER)}],SALT311.Layout_FieldValueList));
  SELF.SSN5_Values := IF ( (SALT311.StrType)le.SSN5 = '',DATASET([],SALT311.Layout_FieldValueList),DATASET([{TRIM((SALT311.StrType)le.SSN5)}],SALT311.Layout_FieldValueList));
  SELF.SSN4_Values := IF ( (SALT311.StrType)le.SSN4 = '',DATASET([],SALT311.Layout_FieldValueList),DATASET([{TRIM((SALT311.StrType)le.SSN4)}],SALT311.Layout_FieldValueList));
  ...
  SELF.DL_STATE_Values := IF ( (SALT311.StrType)le.DL_STATE = '',DATASET([],SALT311.Layout_FieldValueList),DATASET([{TRIM((SALT311.StrType)le.DL_STATE)}],SALT311.Layout_FieldValueList));
  SELF.DL_NBR_Values := IF ( (SALT311.StrType)le.DL_NBR = '',DATASET([],SALT311.Layout_FieldValueList),DATASET([{TRIM((SALT311.StrType)le.DL_NBR)}],SALT311.Layout_FieldValueList));
  ...
  SELF.DT_EFFECTIVE_FIRST_Values := DATASET([{TRIM((SALT311.StrType)le.DT_EFFECTIVE_FIRST)}],SALT311.Layout_FieldValueList);
  SELF.DT_EFFECTIVE_LAST_Values := DATASET([{TRIM((SALT311.StrType)le.DT_EFFECTIVE_LAST)}],SALT311.Layout_FieldValueList);

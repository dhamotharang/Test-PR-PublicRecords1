...
EXPORT KeyName := KeyNames().DLN_logical; /*HACK40*/
 
EXPORT KeyName_sf := KeyNames().DLN_super; /*HACK10*/
...
EXPORT Shrinkage := DATASET([{'DLN','SSN5',CntRed_SSN5*100,CntRed_SSN5*TSize},{'DLN','SSN4',CntRed_SSN4*100,CntRed_SSN4*TSize},{'DLN','DERIVED_GENDER',CntRed_DERIVED_GENDER*100,CntRed_DERIVED_GENDER*TSize},{'DLN','SNAME',CntRed_SNAME*100,CntRed_SNAME*TSize},{'DLN','DOB',CntRed_DOB*100,CntRed_DOB*TSize}],SALT311.ShrinkLayout);
...
EXPORT CanSearch(Process_xIDL_Layouts().InputLayout le) := le.DL_NBR <> (TYPEOF(le.DL_NBR))'' AND Fields.InValid_DL_NBR((SALT311.StrType)le.DL_NBR)=0 AND le.DL_STATE <> (TYPEOF(le.DL_STATE))'' AND Fields.InValid_DL_STATE((SALT311.StrType)le.DL_STATE)=0;
KeyRec := RECORDOF(Key);
...
EXPORT RawFetch(TYPEOF(h.DL_NBR) param_DL_NBR = (TYPEOF(h.DL_NBR))'',TYPEOF(h.DL_NBR_len) param_DL_NBR_len = (TYPEOF(h.DL_NBR_len))'',TYPEOF(h.DL_STATE) param_DL_STATE = (TYPEOF(h.DL_STATE))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.MNAME) param_MNAME = (TYPEOF(h.MNAME))'',TYPEOF(h.LNAME) param_LNAME = (TYPEOF(h.LNAME))'',TYPEOF(h.FNAME_len) param_FNAME_len = (TYPEOF(h.FNAME_len))'',TYPEOF(h.MNAME_len) param_MNAME_len = (TYPEOF(h.MNAME_len))'',TYPEOF(h.LNAME_len) param_LNAME_len = (TYPEOF(h.LNAME_len))'') := 
    STEPPED( LIMIT( Key(
          KEYED((DL_NBR = param_DL_NBR))
      AND KEYED((DL_STATE = param_DL_STATE))
...
useDate := (UNSIGNED4)SALT311.fn_now('YYYYMMDD');
  ...
  result_rollup0 := DEDUP(SORT(result0,DID,DL_NBR,DL_STATE,FNAME,MNAME,LNAME,SSN5,SSN4,DERIVED_GENDER,SNAME,DOB_year,DOB_month,DOB_day,DT_EFFECTIVE_FIRST,-DT_EFFECTIVE_LAST),DID,DL_NBR,DL_STATE,FNAME,MNAME,LNAME,SSN5,SSN4,DERIVED_GENDER,SNAME,DOB_year,DOB_month,DOB_day,DT_EFFECTIVE_FIRST);
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  J0 := JOIN(Recs1,Key,((RIGHT.DL_NBR = LEFT.DL_NBR))
     AND ((RIGHT.DL_STATE = LEFT.DL_STATE))
     AND (( 
 ...
 OR LEFT.FNAME = RIGHT.FNAME))),/*HACK14d*/Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.DL_NBR = LEFT.DL_NBR))
     AND ((RIGHT.DL_STATE = LEFT.DL_STATE)),Config.DLN_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((RIGHT.DL_NBR = LEFT.DL_NBR))
     AND ((RIGHT.DL_STATE = LEFT.DL_STATE))
     AND (( 
        ((LEFT.FNAME = (TYPEOF(RIGHT.FNAME))'' OR RIGHT.FNAME = (TYPEOF(RIGHT.FNAME))'') OR (RIGHT.FNAME = LEFT.FNAME) OR ((RIGHT.FNAME[1..LENGTH(TRIM(LEFT.FNAME))] = LEFT.FNAME OR LEFT.FNAME[1..LENGTH(TRIM(RIGHT.FNAME))] = RIGHT.FNAME)  OR  (RIGHT.FNAME_PreferredName = fn_PreferredName(LEFT.FNAME))  OR  (metaphonelib.DMetaPhone1(RIGHT.FNAME)=metaphonelib.DMetaPhone1(LEFT.FNAME))  OR (Config.WithinEditN(RIGHT.FNAME,RIGHT.FNAME_len,LEFT.FNAME,LEFT.FNAME_len,1, Config.FNAME_LENGTH_EDIT2) /*HACK05*/) OR (Config.WildMatch(RIGHT.FNAME,LEFT.FNAME,FALSE)) ))
        AND ((LEFT.MNAME = (TYPEOF(RIGHT.MNAME))'' OR RIGHT.MNAME = (TYPEOF(RIGHT.MNAME))'') OR (RIGHT.MNAME = LEFT.MNAME) OR ((RIGHT.MNAME[1..LENGTH(TRIM(LEFT.MNAME))] = LEFT.MNAME OR LEFT.MNAME[1..LENGTH(TRIM(RIGHT.MNAME))] = RIGHT.MNAME)  OR (Config.WithinEditN(RIGHT.MNAME,RIGHT.MNAME_len,LEFT.MNAME,LEFT.MNAME_len,2, 0)) ))
 ...
 OR LEFT.FNAME = RIGHT.FNAME))),/*HACK14d*/Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.DL_NBR = LEFT.DL_NBR))
     AND ((RIGHT.DL_STATE = LEFT.DL_STATE)),Config.DLN_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
  useDate := (UNSIGNED4)SALT311.fn_now('YYYYMMDD');
  ...
  result_rollup0 := DEDUP(SORT(J2_dist,Reference,DID,DL_NBR,DL_STATE,FNAME,MNAME,LNAME,SSN5,SSN4,DERIVED_GENDER,SNAME,DOB_year,DOB_month,DOB_day,DT_EFFECTIVE_FIRST,-DT_EFFECTIVE_LAST, LOCAL),Reference,DID,DL_NBR,DL_STATE,FNAME,MNAME,LNAME,SSN5,SSN4,DERIVED_GENDER,SNAME,DOB_year,DOB_month,DOB_day,DT_EFFECTIVE_FIRST, LOCAL);

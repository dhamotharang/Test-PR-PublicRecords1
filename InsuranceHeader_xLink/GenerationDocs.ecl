Generated by SALT V3.11.10-PV1
Command line options: -MInsuranceHeader_xLink -eC:\Users\limaam\AppData\Local\Temp\TFR3AEC.tmp 
...
LinkPath VIN:
VIN:?:FNAMEIn order for LinkPath VIN to be considered for matching the field Basis
must be present in the query and for a match it must be completely identical.
...
DL_STATE Scoring:
Two DL_STATE fields will be considered to match in optional or extra credit positions if:
  - they are identical
...
Date fields are not presently used for scoring but are carried along for debugging purposes
DT_EFFECTIVE_FIRST Scoring:
Date fields are not presently used for scoring but are carried along for debugging purposes
DT_EFFECTIVE_LAST Scoring:
Date fields are not presently used for scoring but are carried along for debugging purposes

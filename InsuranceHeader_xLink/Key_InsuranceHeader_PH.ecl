...
EXPORT KeyName := KeyNames().PH_logical;/*HACK40*/
 
EXPORT KeyName_sf := KeyNames().PH_super; /*HACK10*/
...
EXPORT Shrinkage := DATASET([{'PH','CITY',CntRed_CITY*100,CntRed_CITY*TSize},{'PH','ST',CntRed_ST*100,CntRed_ST*TSize},{'PH','SSN5',CntRed_SSN5*100,CntRed_SSN5*TSize},{'PH','SSN4',CntRed_SSN4*100,CntRed_SSN4*TSize}],SALT311.ShrinkLayout);
...
EXPORT CanSearch(Process_xIDL_Layouts().InputLayout le) := le.PHONE <> (TYPEOF(le.PHONE))'' AND Fields.InValid_PHONE((SALT311.StrType)le.PHONE)=0;
KeyRec := RECORDOF(Key);
...
EXPORT RawFetch(TYPEOF(h.PHONE) param_PHONE = (TYPEOF(h.PHONE))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.MNAME) param_MNAME = (TYPEOF(h.MNAME))'',TYPEOF(h.LNAME) param_LNAME = (TYPEOF(h.LNAME))'',TYPEOF(h.FNAME_len) param_FNAME_len = (TYPEOF(h.FNAME_len))'',TYPEOF(h.MNAME_len) param_MNAME_len = (TYPEOF(h.MNAME_len))'',TYPEOF(h.LNAME_len) param_LNAME_len = (TYPEOF(h.LNAME_len))'',UNSIGNED4 param_DOB) := 
    STEPPED( LIMIT( Key(
      ...
      AND (SALT311.MOD_DateMatch((param_DOB DIV 10000 ),(param_DOB DIV 100 % 100 ),(param_DOB % 100),DOB_year,DOB_month,DOB_day,true,true,13,true,0,0).NNEQ)),MAX_BLOCKLIMIT/*HACK24b*/,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),DID);
...
useDate := (UNSIGNED4)SALT311.fn_now('YYYYMMDD');
  ...
  result_rollup0 := DEDUP(SORT(result0,DID,PHONE,FNAME,MNAME,LNAME,DOB_year,DOB_month,DOB_day,CITY,ST,SSN5,SSN4,DT_EFFECTIVE_FIRST,-DT_EFFECTIVE_LAST),DID,PHONE,FNAME,MNAME,LNAME,DOB_year,DOB_month,DOB_day,CITY,ST,SSN5,SSN4,DT_EFFECTIVE_FIRST);
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  J0 := JOIN(Recs1,Key,((RIGHT.PHONE = LEFT.PHONE))
     ...
     AND (SALT311.MOD_DateMatch((LEFT.DOB DIV 10000 ),(LEFT.DOB DIV 100 % 100 ),(LEFT.DOB % 100),RIGHT.DOB_year,RIGHT.DOB_month,RIGHT.DOB_day,true,true,13,true,0,0).NNEQ),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.PHONE = LEFT.PHONE)),Config.PH_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((RIGHT.PHONE = LEFT.PHONE))
     AND (( 
        ...
        AND ((LEFT.LNAME = (TYPEOF(RIGHT.LNAME))'' OR RIGHT.LNAME = (TYPEOF(RIGHT.LNAME))'') OR (RIGHT.LNAME = LEFT.LNAME) OR ( (metaphonelib.DMetaPhone1(RIGHT.LNAME)=metaphonelib.DMetaPhone1(LEFT.LNAME))  OR (Config.WithinEditN(RIGHT.LNAME,RIGHT.LNAME_len,LEFT.LNAME,LEFT.LNAME_len,1, Config.LNAME_LENGTH_EDIT2) /*HACK09*/)  OR (SALT311.HyphenMatch(RIGHT.LNAME,LEFT.LNAME,1)<=2)OR (Config.WildMatch(RIGHT.LNAME,LEFT.LNAME,FALSE)) ))
          OR SALT311.fn_concept_wordbag_EditN_EL.Match3((SALT311.StrType)RIGHT.FNAME,RIGHT.FNAME_MAINNAME_weight100,true,0,false/*HACK13*/,RIGHT.FNAME_initial_char_weight100,1,0,RIGHT.FNAME_len,RIGHT.FNAME_MAINNAME_fuzzy_weight100,(SALT311.StrType)RIGHT.MNAME,RIGHT.MNAME_MAINNAME_weight100,true,0,true,RIGHT.MNAME_initial_char_weight100,2,0,RIGHT.MNAME_len,RIGHT.MNAME_MAINNAME_fuzzy_weight100,(SALT311.StrType)RIGHT.LNAME,RIGHT.LNAME_MAINNAME_weight100,true,0,false,0,1,0,RIGHT.LNAME_len,RIGHT.LNAME_MAINNAME_fuzzy_weight100,(SALT311.StrType)LEFT.FNAME,LEFT.FNAME_len,(SALT311.StrType)LEFT.MNAME,LEFT.MNAME_len,(SALT311.StrType)LEFT.LNAME,LEFT.LNAME_len,InsuranceHeader_xLink.Config.WithinEditN) > 0))
     AND (SALT311.MOD_DateMatch((LEFT.DOB DIV 10000 ),(LEFT.DOB DIV 100 % 100 ),(LEFT.DOB % 100),RIGHT.DOB_year,RIGHT.DOB_month,RIGHT.DOB_day,true,true,13,true,0,0).NNEQ),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.PHONE = LEFT.PHONE)),Config.PH_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
  useDate := (UNSIGNED4)SALT311.fn_now('YYYYMMDD');
  ...
  result_rollup0 := DEDUP(SORT(J2_dist,Reference,DID,PHONE,FNAME,MNAME,LNAME,DOB_year,DOB_month,DOB_day,CITY,ST,SSN5,SSN4,DT_EFFECTIVE_FIRST,-DT_EFFECTIVE_LAST, LOCAL),Reference,DID,PHONE,FNAME,MNAME,LNAME,DOB_year,DOB_month,DOB_day,CITY,ST,SSN5,SSN4,DT_EFFECTIVE_FIRST, LOCAL);

...
EXPORT CustTest_Proc_GoExternal(STRING fileVersion) := FUNCTION
	...
	BK6 	:= 	if(~fileservices.fileexists(modCustTestFile.src_rid_logical)  ,BUILDINDEX(Key_InsuranceHeader_SRC_RID().Key,	modCustTestFile.src_rid_logical,	 OVERWRITE));
	...
	BK14    := 	if(~fileservices.fileexists(modCustTestFile.header0_logical),		BUILDINDEX(Process_xIDL_Layouts().Key0, 	modCustTestFile.header0_logical,OVERWRITE));
	BK15    := 	if(~fileservices.fileexists(modCustTestFile.header1_logical),		BUILDINDEX(Process_xIDL_Layouts().Key1, 	modCustTestFile.header1_logical,OVERWRITE));
	BK16    := 	if(~fileservices.fileexists(modCustTestFile.VIN_logical),		BUILDINDEX(Key_InsuranceHeader_VIN().Key, 	modCustTestFile.VIN_logical,OVERWRITE));
	RETURN SEQUENTIAL(buildSpec,
										PARALLEL(BaseK,BK0,BK1,BK2,BK3,BK4,BK5,
										BK6,BK7,BK8,BK9,BK10,BK11,BK12,BK13,BK14,BK15,BK16)

...
EXPORT MEOW_xIDL_Service := MACRO
  #WEBSERVICE(FIELDS(
	...
	DESCRIPTION('SALT V3.11 <p/> Attempt to resolve or find DIDs. <p>The more data input the better' + '</p><p>fields name, address1 and address2 are cleaned before using as input' + '</p><p>Debug option is always false, but if on it will show the outputs that follows the process to append a LexID'));/*HACK19*/

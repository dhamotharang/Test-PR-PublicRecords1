...
virtual_fraud_func_set := inquiry_acclogs.score_constants.virtual_fraud_db_function_set;
...
%temp_layout% := record
	%fp_batch_in%;
	#if(isdaily)
	inquiry_acclogs.Layout_In_Common - layout_exclusions;
	#else
...
inquiries_with_score_appended := project(join(distribute(infile_iter,hash(acctno)),distribute(pipe_results,hash(acctno)),
 ...
 #if(isdaily)
 inquiry_acclogs.Layout_In_Common,
 #else
 	inquiry_acclogs.Layout.Common_ThorAdditions,
#end

...
wuname := 'FCRA Inquiry Tracking Build';

#workunit('name',wuname);

valid_state := ['blocked','compiled','submitted','running'];

d := sort(nothor(WorkunitServices.WorkunitList('',,,'',''))(wuid <> thorlib.wuid() and job = wuname and state in valid_state), -wuid);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		// sc 			:= fileservices.superfilecontents('~thor10_11::out::inquiry_tracking::weekly_historical')[1].name;
		// findex 	:= stringlib.stringfind(sc, '::', 3)+2;
		// lindex 	:= stringlib.stringfind(sc, '::', 4)-1;

HistoryVersion := ut.GetDate: independent; //version of base file

MakeNewCompCounts := nothor(~fileservices.fileexists('~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_companyids')); //do counts exist?
...
inq_fcra :=(inquiry_acclogs.File_FCRA_Banko_Logs_Common +
						...
						inquiry_acclogs.File_FCRA_Accurint_Logs_Common+
						inquiry_acclogs.File_FCRA_Riskwise_Logs_Common)(~inquiry_acclogs.fnTranslations.is_SubMarketFilter(allow_flags.allowflags) and
...
inq_file := project(inq_fcra,
							transform({inquiry_acclogs.Layout.Common, string source}, self.source := 'FCRA', self := left));
...
slim_Entity t_map_slim (inq_file le) := transform
	...
	boolean is_buser := trim(le.BusUser_Q.First_Name + 
		...
		self.fcra_permissions				:= le.permissions.fcra_purpose;
		self.dppa_permissions				:= le.permissions.dppa_purpose;
		self.glb_permissions				:= le.permissions.glb_purpose;
...
// prep_slim_inq := distribute(project(inq_file, t_map_slim(left)), 
												// hash(transaction_id, transaction_type, sequence_number, function_description, datetime, login_history_id, product_code, fname, lname))
												 // : independent;
												 
prep_slim_inq := project(inq_file, t_map_slim(left));
...
prefix_comprehensive_table := table(slim_inq, {global_company_id, company_id, transaction_datey, transaction_datem, transaction_dated, stat_type, industry, vertical, sub_market,use, product, source, product_code, fcra_permissions, glb_permissions, dppa_permissions, has_ssn, has_email, has_ein, has_dob, has_address, has_phone, has_adl, has_bdid, opt_out,
															cnt := count(group)},
															global_company_id, company_id, transaction_datey, transaction_datem, transaction_dated, stat_type, industry, vertical, sub_market,use, product, source, product_code, fcra_permissions, glb_permissions, dppa_permissions, has_ssn, has_email, has_ein, has_dob, has_address, has_phone, has_adl, has_bdid, opt_out, local);
...
comprehensive_table_yyyymmdd				:=	table(pcomprehensive_table_yyyymmdd, 
																		{transaction_datey, transaction_datem, transaction_dated, stat_type, industry, vertical, sub_market,use, product, source, product_code, fcra_permissions, glb_permissions, dppa_permissions, has_ssn, has_email, has_ein, has_dob, has_address, has_phone, has_adl, has_bdid, opt_out,
																		 cntsum := sum(group, cnt)},
																		 transaction_datey, transaction_datem, transaction_dated, stat_type, industry, vertical, sub_market,use, product, source, product_code, fcra_permissions, glb_permissions, dppa_permissions, has_ssn, has_email, has_ein, has_dob, has_address, has_phone, has_adl, has_bdid, opt_out, few)
															;
...
comprehensive_table_nodate 					:= table(pcomprehensive_table_yyyymmdd, 
																		{stat_type, industry, vertical, sub_market,use, product, source, product_code, fcra_permissions, glb_permissions, dppa_permissions, has_ssn, has_email, has_ein, has_dob, has_address, has_phone, has_adl, has_bdid, opt_out, 
																		 cntsum := sum(group, cnt)},
																		 stat_type, industry, vertical, sub_market,use, product, source, product_code, fcra_permissions, glb_permissions, dppa_permissions, has_ssn, has_email, has_ein, has_dob, has_address, has_phone, has_adl, has_bdid, opt_out, few)
															;
...
header_t := dataset([{'SOURCE',0,'NEWEST DATE','GLOBAL CID','COMPANY ID','PRODUCT CODE','PRODUCT','VERTICAL','INDUSTRY','SUB MARKET', 'USE', 'OPT OUT', 'FCRA PERMISSIONS','GLB PERMISSIONS','DPPA PERMISSIONS',
										...
										{STRING8 SOURCE, UNSIGNED8 total, STRING15 newest_date, STRING10 global_company_id, STRING10 company_id , STRING12 product_code , 
										 STRING30 product , STRING30 vertical , STRING30 industry , STRING30 sub_market ,STRING10 use , STRING11 opt_out,
										 STRING20 fcra_permissions, STRING20 glb_permissions, STRING20 dppa_permissions ,
										  UNSIGNED8 MonthBack_0 , UNSIGNED8 MonthBack_1 , UNSIGNED8 MonthBack_2 , UNSIGNED8 MonthBack_3 , 
...
t := table(pcomprehensive_table_yyyymmdd(subtract_months(mxDT, 23) <= transaction_datey+transaction_datem), 
							{STRING8 SOURCE := 'FCRA', UNSIGNED8 TOTAL := sum(group, cnt), STRING15 newest_date := max(group, transaction_datey+transaction_datem+transaction_dated), 
							 ...
							 STRING20 fcra_permissions := fcra_permissions, STRING20 glb_permissions := glb_permissions, STRING20 dppa_permissions := dppa_permissions,
...
nothor(fileservices.Despray(dsnm, 
										 _control.IPAddress.bctlpedata10, 
										 '/data/inquiry_data_01/stats/FCRA/'+unixnm, 
										 , 
										 , 
										 , 
										 true));
...
Distribution_Cnts := sequential(
	...
	output(sort(header_t&t, record),,'~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_companyid_products', overwrite, csv(heading(single), quote('"'), separator('\t')));
	output(sort(pcomprehensive_table_yyyymmdd, record),,'~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_companyids', overwrite, csv(heading(single), quote('"'), separator('\t')));
	output(sort(comprehensive_table_yyyymmdd, record),,'~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_yyyymmdd', overwrite, csv(heading(single), quote('"'), separator('\t')));
	output(sort(comprehensive_table_nodate, record),,'~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_nodate', overwrite, csv(heading(single),quote('"'), separator('\t')));
	output(sort(comprehensive_table_yyyymmdd_only, record),,'~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_yyyymmdd_only', overwrite, csv(heading(single),quote('"'), separator('\t'), heading(single)));
	output(sort(comprehensive_table_blankindustry, record),,'~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_blankindustry', overwrite, csv(heading(single),quote('"'), separator('\t'), heading(single)));

	despray('~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_companyid_products', 'comprehensive_table_fcra_companyid_products_'+HistoryVersion+'.csv'),
	despray('~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_companyids', 'comprehensive_table_fcra_companyids_'+HistoryVersion+'.csv'),
	despray('~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_yyyymmdd', 'comprehensive_table_fcra_yyyymmdd_'+HistoryVersion+'.csv'),
	despray('~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_nodate', 'comprehensive_table_fcra_nodate_'+HistoryVersion+'.csv'),
	despray('~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_blankindustry', 'comprehensive_table_fcra_blankindustry_'+HistoryVersion+'.csv'),
	despray('~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_yyyymmdd_only', 'comprehensive_table_fcra_yyyymmdd_only_'+HistoryVersion+'.csv')

);

Population_Cnts := sequential(

	output(comprehensive_population_stats,,'~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_population', overwrite, csv(heading(single), quote('"'), separator('\t')));
	output(comprehensive_population_stats_slimmed,,'~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_populationslim', overwrite, csv(heading(single), quote('"'), separator('\t')));

	despray('~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_population', 'comprehensive_table_fcra_population_'+HistoryVersion+'.csv'),
	despray('~thor20_11::out::inquiry_acclogs::fcra::'+HistoryVersion+'::comprehensive_table_populationslim', 'comprehensive_table_fcra_populationslim_'+HistoryVersion+'.csv')

);


DoComprehensive := if(MakeNewCompCounts, sequential(Distribution_Cnts));//, Population_Cnts));
...
infiles  := '/data/inquiry_data_01/stats/FCRA/';

eData12FileList := nothor(fileservices.RemoteDirectory(IP,infiles,,FALSE)(regexfind('(fcra)', name)));

MoveEData12Files(string name) := function

	del := nothor(fileservices.DeleteExternalFile(IP, infiles+'old/'+name)); 
	mov := nothor(fileservices.MoveExternalFile(IP, infiles+name, infiles+'old/'+name));

RETURN SEQUENTIAL( mov);

end;

									
EXPORT _SCH_FCRAComprehensiveStats := if(~active_workunit, SEQUENTIAL(docomprehensive), output(d, named('ActiveWorkunit')))
	// : when(cron('30 12 * * 6'))
	;

...
df := DEDUP(TABLE(Inquiry_AccLogs.File_Inquiry_Base.Update(bus_intel.industry <> '' and search_info.transaction_id != '' and 
...
export Key_Inquiry_Transaction_ID_Update := INDEX(df, {string50 transaction_id := transaction_id}, {df},
	  Data_Services.Data_location.Prefix('Inquiry') + 'thor_data400::key::inquiry_table::' + doxie.version_superkey  + '::transaction_id_update');							

...
export fnMapCommon_FCRA_ProdR3 := module
...
NormFiles := normalize(outfileBatchR3, 2,
													transform(recordof(outfileBatchR3),
...
clean_out :=  PROJECT(NormFiles,
								TRANSFORM(Inquiry_AccLogs.layout_in_common, 
																			...
																			self := LEFT;
																			self := []));// : persist('~persist::batchr3::clean');
...
prev_base := inquiry_acclogs.File_FCRA_ProdR3_Logs_Common;

prev_base_trans := project(prev_base, transform({inquiry_acclogs.Layout.Common, string source := 'BATCHR3'}, self := left));
...
newFile := if(ReAppendDay, 
							dedup(sort(distribute(person_project + bus_project + prev_base_ready, 
								hash(search_info.Sequence_Number + person_q.lname + bususer_q.lname + bus_q.cname)), record, local), record, local),
							person_project + bus_project)(mbs.company_id + mbs.global_company_id <> '');

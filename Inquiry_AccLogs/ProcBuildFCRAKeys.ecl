...
logs_version := dataset(ut.foreign_fcra_logs + 'thor35_21::out::inquiry::processedfiles', recordof(workunitservices.WorkunitFilesRead(workunit)), thor, opt)
...
BuildKeys := sequential(
                output(dataset([{ut.GetDate}],{string8 filedate}),,'~thor::inquiry_fcra_key_build',overwrite),
								if(redid, inquiry_acclogs.File_FCRA_Inquiry_BaseSourced.create_appends); //creates the re-dided base file lookup
									 parallel(bk_addr, bk_did, bk_phone, bk_ssn, bk_bgroup); // update ids and build keys
									 parallel(mv_addr, mv_did, mv_phone, mv_ssn, mv_bgroup);
									 parallel(mv2qa_addr, mv2qa_did, mv2qa_phone, mv2qa_ssn, mv2qa_bgroup);

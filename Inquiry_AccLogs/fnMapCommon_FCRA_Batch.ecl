...
export fnMapCommon_FCRA_Batch := module
...
inputfile := distribute(choosen(inquiry_acclogs.File_FCRA_Batch_Logs
...
inquiry_acclogs.File_MBSApp(cleaned_fields, 'BATCH', 'Y', mbs_outfile) 
...
prev_base := inquiry_acclogs.File_FCRA_Batch_Logs_Common;

prev_base_trans := project(prev_base, transform({inquiry_acclogs.Layout.Common, string source := 'BATCH'}, self := left));

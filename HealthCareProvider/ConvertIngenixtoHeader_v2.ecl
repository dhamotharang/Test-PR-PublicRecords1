...
import Health_Provider_Test, CLUEAuto;
EXPORT ConvertIngenixtoHeader_v2(DATASET(Layouts.INGENIX_DID) Infile, boolean useLocal = true, string persistPath = '~thor21::bbounds::provider_header::') := FUNCTION
  ...
  currentInfileRs1      := ingenixDidRawRs(processdate = currentProcessdate);
  currentInfileRs2      := DISTRIBUTE(currentInfileRs1, HASH32(providerid));
  currentInfileRs3      := SORT(currentInfileRs2, providerid, LOCAL);
  currentInfileRs       := currentInfileRs2;   // distributed/sorted lhs ... multiple times

...
IMPORT AppendCounty, AppendCleanAddress, BIPV2, ecl;
  ...
  LOCAL _InData := PROJECT(InData, TRANSFORM(ExpandedBipRec,    
    ...
    SELF := LEFT));
  LOCAL rSearch := {RECORDOF(_InData) OR {INTEGER _searchUlt}};
  LOCAL dSearch := PROJECT(_InData, TRANSFORM(rSearch,
    SELF._searchUlt := (INTEGER)LEFT.InUltId,
    SELF := LEFT));
  LOCAL dDistributed  := DISTRIBUTE(dSearch((UNSIGNED)InUltId <> 0), HASH32(InUltId));
  LOCAL dupData    := DEDUP(SORT(dDistributed, InUltId, InSeleId, InProxId, LOCAL), InUltId, InSeleId, InProxId, LOCAL);
	
  LOCAL ExpandedProxFromUlt := ecl.macJoinKey(dupData, Business.Keys.Key_BestStaticLinkIDs,
    'KEYED((INTEGER)LEFT._searchUlt = (INTEGER)RIGHT.UltId) AND WILD(RIGHT.OrgId) AND KEYED((INTEGER)RIGHT.SELEID != 0 AND (INTEGER)RIGHT.PROXID != 0)', 
    '(INTEGER)LEFT.UltId=(INTEGER)RIGHT._searchUlt AND (INTEGER)LEFT.SELEID != 0 AND (INTEGER)LEFT.PROXID != 0', 
  ...
  LOCAL dExpandedAllUlt := JOIN(PULL(Business.Keys.Key_BestStaticLinkIDs), dupData, 
    (INTEGER)LEFT.UltId=(INTEGER)RIGHT._searchUlt,
    TRANSFORM({RECORDOF(RIGHT), RECORDOF(LEFT)},
      SELF := LEFT,
  ...
  LOCAL _dExpandedFinal  := PROJECT(dExpandedFilter(Company_Address[1].Company_St IN JurisdictionStateSet AND (Company_Address[1].company_prim_range <> '' OR Company_Address[1].company_prim_name <> '')),
    TRANSFORM(ExpandedBipRec, 
      SELF.InUltId := (TYPEOF(LEFT.InUltID))LEFT.UltID,
			...
			SELF.Incounty := AppendCounty.Counties.CountyStCountyNameDCT[LEFT.Company_Address[1].company_st,LEFT.Company_Address[1].county_name].fips,                        

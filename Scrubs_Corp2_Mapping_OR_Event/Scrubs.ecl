...
EXPORT FromNone(DATASET(corp2_mapping.layoutscommon.events) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.event_filing_date_Invalid := Fields.InValid_event_filing_date((SALT32.StrType)le.event_filing_date);
    SELF.event_date_type_cd_Invalid := Fields.InValid_event_date_type_cd((SALT32.StrType)le.event_date_type_cd);
    SELF.event_date_type_desc_Invalid := Fields.InValid_event_date_type_desc((SALT32.StrType)le.event_date_type_desc);
    SELF.event_filing_desc_Invalid := Fields.InValid_event_filing_desc((SALT32.StrType)le.event_filing_desc);
    SELF := le;
  END;
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.corp_key_Invalid << 0 ) + ( le.corp_vendor_Invalid << 2 ) + ( le.corp_state_origin_Invalid << 3 ) + ( le.corp_process_date_Invalid << 4 ) + ( le.corp_sos_charter_nbr_Invalid << 6 ) + ( le.event_filing_date_Invalid << 8 ) + ( le.event_date_type_cd_Invalid << 10 ) + ( le.event_date_type_desc_Invalid << 11 ) + ( le.event_filing_desc_Invalid << 12 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.event_date_type_cd_Invalid := (le.ScrubsBits1 >> 10) & 1;
    SELF.event_date_type_desc_Invalid := (le.ScrubsBits1 >> 11) & 1;
    SELF.event_filing_desc_Invalid := (le.ScrubsBits1 >> 12) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    event_filing_desc_CUSTOM_ErrorCount := COUNT(GROUP,h.event_filing_desc_Invalid=1);
  END;
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.corp_key_Invalid,le.corp_vendor_Invalid,le.corp_state_origin_Invalid,le.corp_process_date_Invalid,le.corp_sos_charter_nbr_Invalid,le.event_filing_date_Invalid,le.event_date_type_cd_Invalid,le.event_date_type_desc_Invalid,le.event_filing_desc_Invalid,100);
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.event_date_type_desc_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.event_filing_desc_Invalid,'CUSTOM','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_sos_charter_nbr','event_filing_date','event_date_type_cd','event_date_type_desc','event_filing_desc','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_date','invalid_date_type_cd','invalid_date_type_desc','invalid_event_filing_desc','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT32.StrType)le.corp_key,(SALT32.StrType)le.corp_vendor,(SALT32.StrType)le.corp_state_origin,(SALT32.StrType)le.corp_process_date,(SALT32.StrType)le.corp_sos_charter_nbr,(SALT32.StrType)le.event_filing_date,(SALT32.StrType)le.event_date_type_cd,(SALT32.StrType)le.event_date_type_desc,(SALT32.StrType)le.event_filing_desc,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT32.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'event_filing_desc:invalid_event_filing_desc:CUSTOM','UNKNOWN');
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.event_filing_desc_CUSTOM_ErrorCount,0);
      SELF.rulepcnt := 100 * CHOOSE(c
          ...
          ,le.event_date_type_cd_ENUM_ErrorCount
          ,le.event_date_type_desc_ENUM_ErrorCount
          ,le.event_filing_desc_CUSTOM_ErrorCount,0) / le.TotalCnt + 0.5;
    END;

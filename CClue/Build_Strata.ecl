...
function
	...
	Strata.mac_CreateXMLStats(dUpdate.dCleanAddressState				,_Constants().Name	,'base_file'	,pversion	,email_notification_lists().buildsuccess	,BuildCleanAddressState_Strata				,'St'						,'Population'	,,pIsTesting,pOverwrite);
	...
	Strata.mac_CreateXMLStats(dUpdate.dUniqueCleanAddressState	,_Constants().Name	,'base_file'	,pversion	,email_notification_lists().buildsuccess	,BuildUniqueCleanAddressState_Strata	,'St'						,'Uniques'		,,pIsTesting,pOverwrite);
	...
	if(tools.fun_IsValidVersion(pversion)
		,full_build		
		,output('No Valid version parameter passed, skipping CClue.Build_Strata attribute')
	);

...
EXPORT KeyName := KeyPrefix+'::'+'key::Bair_ExternalLinkKeys_V2'+'::'+Config.KeyInfix+'::EID_HASH::meow';
 
EXPORT KeyName_sf := KeyPrefix+'::'+'key::Bair_ExternalLinkKeys_V2'+'::'+KeySuperfile+'::EID_HASH::meow';
...
EXPORT Fetch_Stream(DATASET(id_stream_layout) d) := FUNCTION
    ...
    DLayout tr(id_stream_layout le, k ri) := TRANSFORM
      ...
      SELF := le;
    END;
    J := JOIN( d,k,(LEFT.EID_HASH = RIGHT.EID_HASH),tr(LEFT,RIGHT), LEFT OUTER, KEEP(10000), LIMIT(Config.JoinLimit)); // Ignore excess records without erroring
    RETURN J;
END;
...
EXPORT MAC_Fetch_Batch(infile, In_EID_HASH, In_UniqueID, asIndex = TRUE) := FUNCTIONMACRO
  IMPORT SALT37,Bair_ExternalLinkKeys_V2;
  payloadKey := Bair_ExternalLinkKeys_V2.Process_PS_Layouts.Key;
  idHistKey := Bair_ExternalLinkKeys_V2.Process_PS_Layouts.KeyIDHistory;
  outLayout := RECORD
    infile.In_UniqueID;
    SALT37.UIDType inputEID_HASH;
    RECORDOF(payloadKey);
  END;
  outLayout emptyResult(infile le) := TRANSFORM
    SELF.In_UniqueID := le.In_UniqueID;
    SELF.inputEID_HASH := le.In_EID_HASH;
    SELF := [];
  END;
 
  outLayout xJoinHist(outLayout le, idHistKey ri) := TRANSFORM
    SELF.In_UniqueID := le.In_UniqueID;
    SELF.inputEID_HASH := le.inputEID_HASH;
    SELF := ri;
    SELF := [];
  END;
  ...
  JEntKeyed := JOIN(infile_emptyResult, payloadKey, LEFT.inputEID_HASH = RIGHT.EID_HASH, xJoinPayload(LEFT,RIGHT), LEFT OUTER);
  JEntPull := JOIN(infile_emptyResult, PULL(payloadKey), LEFT.inputEID_HASH = RIGHT.EID_HASH, xJoinPayload(LEFT,RIGHT), LEFT OUTER, HASH);
 
  resultsEnt := IF(asIndex, JEntKeyed, JEntPull);
  resultsEnt_found := resultsEnt(EID_HASH > 0);
  resultsEnt_notfound := resultsEnt(EID_HASH = 0);
 
  JRecKeyed_1 := JOIN(resultsEnt_notfound, idHistKey, LEFT.inputEID_HASH = RIGHT., xJoinHist(LEFT,RIGHT), LEFT OUTER);
  JRecPull_1 := JOIN(resultsEnt_notfound, PULL(idHistKey), LEFT.inputEID_HASH = RIGHT., xJoinHist(LEFT,RIGHT), LEFT OUTER, HASH);
 
  resultsIDHist := IF(asIndex, JRecKeyed_1, JRecPull_1);
  resultsIDHist_found := resultsIDHist(EID_HASH > 0);
  resultsIDHist_notfound := resultsIDHist(EID_HASH = 0);
 
  JRecKeyed_2 := JOIN(resultsIDHist_found, payloadKey, LEFT.EID_HASH = RIGHT.EID_HASH, xJoinPayload(LEFT,RIGHT), LEFT OUTER);
  JRecPull_2 := JOIN(resultsIDHist_found, PULL(payloadKey), LEFT.EID_HASH = RIGHT.EID_HASH, xJoinPayload(LEFT,RIGHT), LEFT OUTER, HASH);
 
  resultsIDHistAll := IF(asIndex, JRecKeyed_2, JRecPull_2);
  resultsAll := resultsEnt_found & resultsIDHistAll & PROJECT(resultsIDHist_notfound, TRANSFORM(outLayout, SELF.inputEID_HASH := LEFT.inputEID_HASH, SELF.In_UniqueID := LEFT.In_UniqueID, SELF := []));
...
// Below only used in header search (data returning) case
  ...
  SALT37.UIDType Entered_EID_HASH := 0; // Allow user to enter EID_HASH to pull data
END;
 
EXPORT HardKeyMatch(InputLayout le) := le.FNAME <> (TYPEOF(le.FNAME))'' AND Fields.InValid_FNAME((SALT37.StrType)le.FNAME)=0 AND le.LNAME <> (TYPEOF(le.LNAME))'' AND Fields.InValid_LNAME((SALT37.StrType)le.LNAME)=0 AND le.ST <> (TYPEOF(le.ST))'' AND Fields.InValid_ST((SALT37.StrType)le.ST)=0 OR le.PRIM_RANGE <> (TYPEOF(le.PRIM_RANGE))'' AND Fields.InValid_PRIM_RANGE((SALT37.StrType)le.PRIM_RANGE)=0 AND le.PRIM_NAME <> (TYPEOF(le.PRIM_NAME))'' AND Fields.InValid_PRIM_NAME((SALT37.StrType)le.PRIM_NAME)=0 AND le.ZIP <> (TYPEOF(le.ZIP))'' AND Fields.InValid_ZIP((SALT37.StrType)le.ZIP)=0 OR le.DOB <> (TYPEOF(le.DOB))'' AND Fields.InValid_DOB((SALT37.StrType)le.DOB)=0 AND le.LNAME <> (TYPEOF(le.LNAME))'' AND Fields.InValid_LNAME((SALT37.StrType)le.LNAME)=0 OR le.ZIP <> (TYPEOF(le.ZIP))'' AND Fields.InValid_ZIP((SALT37.StrType)le.ZIP)=0 AND le.PRIM_RANGE <> (TYPEOF(le.PRIM_RANGE))'' AND Fields.InValid_PRIM_RANGE((SALT37.StrType)le.PRIM_RANGE)=0 OR le.DL <> (TYPEOF(le.DL))'' AND Fields.InValid_DL((SALT37.StrType)le.DL)=0 AND le.DL_ST <> (TYPEOF(le.DL_ST))'' AND Fields.InValid_DL_ST((SALT37.StrType)le.DL_ST)=0 OR le.PHONE <> (TYPEOF(le.PHONE))'' AND Fields.InValid_PHONE((SALT37.StrType)le.PHONE)=0 OR le.LNAME <> (TYPEOF(le.LNAME))'' AND Fields.InValid_LNAME((SALT37.StrType)le.LNAME)=0 AND le.FNAME <> (TYPEOF(le.FNAME))'' AND Fields.InValid_FNAME((SALT37.StrType)le.FNAME)=0 OR le.VIN <> (TYPEOF(le.VIN))'' AND Fields.InValid_VIN((SALT37.StrType)le.VIN)=0 OR le.LEXID <> (TYPEOF(le.LEXID))'' AND Fields.InValid_LEXID((SALT37.StrType)le.LEXID)=0 OR le.POSSIBLE_SSN <> (TYPEOF(le.POSSIBLE_SSN))'' AND Fields.InValid_POSSIBLE_SSN((SALT37.StrType)le.POSSIBLE_SSN)=0 OR le.LATITUDE <> (TYPEOF(le.LATITUDE))'' AND Fields.InValid_LATITUDE((SALT37.StrType)le.LATITUDE)=0 AND le.LONGITUDE <> (TYPEOF(le.LONGITUDE))'' AND Fields.InValid_LONGITUDE((SALT37.StrType)le.LONGITUDE)=0 OR le.PLATE <> (TYPEOF(le.PLATE))'' AND Fields.InValid_PLATE((SALT37.StrType)le.PLATE)=0 OR le.CLEAN_COMPANY_NAME <> (TYPEOF(le.CLEAN_COMPANY_NAME))'' AND Fields.InValid_CLEAN_COMPANY_NAME((SALT37.StrType)le.CLEAN_COMPANY_NAME)=0;
EXPORT LayoutScoredFetch := RECORD // Nulls required for linkpaths that do not have field
  ...
  DATASET(SALT37.layout_gss_cases) LNAME_GSS_cases := DATASET([],SALT37.layout_gss_cases);
  INTEGER2 LNAME_GSS_Weight := 0;
...
EXPORT LayoutScoredFetch update_forcefailed(LayoutScoredFetch le, BOOLEAN disableForce = FALSE) := TRANSFORM
  SELF.ForceFailed := IF(disableForce, FALSE, ((le.DOBWeight_year<>0 OR le.DOBWeight_month<>0 OR le.DOBWeight_day<>0) AND (le.DOBWeight_year + le.DOBWeight_month + le.DOBWeight_day < Config.DOB_Force)));
  SELF := le;
END;
...
EXPORT LayoutScoredFetch combine_scores(LayoutScoredFetch le,LayoutScoredFetch ri, BOOLEAN In_disableForce = FALSE) := TRANSFORM
  ...
  SELF.LNAME_gss_Weight := IF ( EXISTS(le.LNAME_gss_cases(ri.LNAME_gss_cases[1].gss_hash=gss_hash)), le.LNAME_gss_Weight, le.LNAME_gss_Weight + ri.LNAME_gss_Weight );
  SELF.LNAME_gss_cases := IF ( EXISTS(le.LNAME_gss_cases(ri.LNAME_gss_cases[1].gss_hash=gss_hash)), le.LNAME_gss_cases, le.LNAME_gss_cases + ri.LNAME_gss_cases );
  SELF.LNAMEWeight := MAP ( SELF.LNAME_GSS_Weight > le.LNAMEWeight AND SELF.LNAME_GSS_Weight > ri.LNAMEWeight => SELF.LNAME_GSS_Weight,
                         le.LNAMEWeight>ri.LNAMEWeight => le.LNAMEWeight, ri.LNAMEWeight );
  SELF.LNAME := IF ( le.LNAMEWeight>ri.LNAMEWeight AND le.LNAME <> (TYPEOF(le.LNAME))'', le.LNAME, ri.LNAME );
...
aggregateRec := RECORD
  ...
  P_CITY_NAMEWeight := MAX(GROUP,IF( in_data.P_CITY_NAME_match_code=SALT37.MatchCode.ExactMatch, in_data.P_CITY_NAMEWeight,0 ));
  ...
  DL_STWeight := MAX(GROUP,IF( in_data.DL_ST_match_code=SALT37.MatchCode.ExactMatch, in_data.DL_STWeight,0 ));
  DLWeight := MAX(GROUP,IF( in_data.DL_match_code=SALT37.MatchCode.ExactMatch, in_data.DLWeight,0 ));
  LEXIDWeight := MAX(GROUP,IF( in_data.LEXID_match_code=SALT37.MatchCode.ExactMatch, in_data.LEXIDWeight,0 ));
  POSSIBLE_SSNWeight := MAX(GROUP,IF( in_data.POSSIBLE_SSN_match_code=SALT37.MatchCode.ExactMatch, in_data.POSSIBLE_SSNWeight,0 ));
  CRIMEWeight := MAX(GROUP,IF( in_data.CRIME_match_code=SALT37.MatchCode.ExactMatch, in_data.CRIMEWeight,0 ));
  NAME_TYPEWeight := MAX(GROUP,IF( in_data.NAME_TYPE_match_code=SALT37.MatchCode.ExactMatch, in_data.NAME_TYPEWeight,0 ));
  CLEAN_GENDERWeight := MAX(GROUP,IF( in_data.CLEAN_GENDER_match_code=SALT37.MatchCode.ExactMatch, in_data.CLEAN_GENDERWeight,0 ));
  CLASS_CODEWeight := MAX(GROUP,IF( in_data.CLASS_CODE_match_code=SALT37.MatchCode.ExactMatch, in_data.CLASS_CODEWeight,0 ));
  ...
  DATA_PROVIDER_ORIWeight := MAX(GROUP,IF( in_data.DATA_PROVIDER_ORI_match_code=SALT37.MatchCode.ExactMatch, in_data.DATA_PROVIDER_ORIWeight,0 ));
  VINWeight := MAX(GROUP,IF( in_data.VIN_match_code=SALT37.MatchCode.ExactMatch, in_data.VINWeight,0 ));
  PLATEWeight := MAX(GROUP,IF( in_data.PLATE_match_code=SALT37.MatchCode.ExactMatch, in_data.PLATEWeight,0 ));
  LATITUDEWeight := MAX(GROUP,IF( in_data.LATITUDE_match_code=SALT37.MatchCode.ExactMatch, in_data.LATITUDEWeight,0 ));
  LONGITUDEWeight := MAX(GROUP,IF( in_data.LONGITUDE_match_code=SALT37.MatchCode.ExactMatch, in_data.LONGITUDEWeight,0 ));
  SEARCH_ADDR1Weight := MAX(GROUP,IF( in_data.SEARCH_ADDR1_match_code=SALT37.MatchCode.ExactMatch, in_data.SEARCH_ADDR1Weight,0 ));
  SEARCH_ADDR2Weight := MAX(GROUP,IF( in_data.SEARCH_ADDR2_match_code=SALT37.MatchCode.ExactMatch, in_data.SEARCH_ADDR2Weight,0 ));
  CLEAN_COMPANY_NAMEWeight := MAX(GROUP,IF( in_data.CLEAN_COMPANY_NAME_match_code=SALT37.MatchCode.ExactMatch, in_data.CLEAN_COMPANY_NAMEWeight,0 ));
...
LayoutScoredFetch FixScores(LayoutScoredFetch le, aggregateRec ri) := TRANSFORM
  ...
  SELF.P_CITY_NAMEWeight := MAP( ri.P_CITY_NAMEWeight=0 OR le.P_CITY_NAME_match_code=SALT37.MatchCode.ExactMatch => le.P_CITY_NAMEWeight,MIN(le.P_CITY_NAMEWeight,ri.P_CITY_NAMEWeight-1) );
  ...
  SELF.DL_STWeight := MAP( ri.DL_STWeight=0 OR le.DL_ST_match_code=SALT37.MatchCode.ExactMatch => le.DL_STWeight,MIN(le.DL_STWeight,ri.DL_STWeight-1) );
  SELF.DLWeight := MAP( ri.DLWeight=0 OR le.DL_match_code=SALT37.MatchCode.ExactMatch => le.DLWeight,MIN(le.DLWeight,ri.DLWeight-1) );
  SELF.LEXIDWeight := MAP( ri.LEXIDWeight=0 OR le.LEXID_match_code=SALT37.MatchCode.ExactMatch => le.LEXIDWeight,MIN(le.LEXIDWeight,ri.LEXIDWeight-1) );
  SELF.POSSIBLE_SSNWeight := MAP( ri.POSSIBLE_SSNWeight=0 OR le.POSSIBLE_SSN_match_code=SALT37.MatchCode.ExactMatch => le.POSSIBLE_SSNWeight,MIN(le.POSSIBLE_SSNWeight,ri.POSSIBLE_SSNWeight-1) );
  SELF.CRIMEWeight := MAP( ri.CRIMEWeight=0 OR le.CRIME_match_code=SALT37.MatchCode.ExactMatch => le.CRIMEWeight,MIN(le.CRIMEWeight,ri.CRIMEWeight-1) );
  SELF.NAME_TYPEWeight := MAP( ri.NAME_TYPEWeight=0 OR le.NAME_TYPE_match_code=SALT37.MatchCode.ExactMatch => le.NAME_TYPEWeight,MIN(le.NAME_TYPEWeight,ri.NAME_TYPEWeight-1) );
  SELF.CLEAN_GENDERWeight := MAP( ri.CLEAN_GENDERWeight=0 OR le.CLEAN_GENDER_match_code=SALT37.MatchCode.ExactMatch => le.CLEAN_GENDERWeight,MIN(le.CLEAN_GENDERWeight,ri.CLEAN_GENDERWeight-1) );
  SELF.CLASS_CODEWeight := MAP( ri.CLASS_CODEWeight=0 OR le.CLASS_CODE_match_code=SALT37.MatchCode.ExactMatch => le.CLASS_CODEWeight,MIN(le.CLASS_CODEWeight,ri.CLASS_CODEWeight-1) );
  ...
  SELF.DATA_PROVIDER_ORIWeight := MAP( ri.DATA_PROVIDER_ORIWeight=0 OR le.DATA_PROVIDER_ORI_match_code=SALT37.MatchCode.ExactMatch => le.DATA_PROVIDER_ORIWeight,MIN(le.DATA_PROVIDER_ORIWeight,ri.DATA_PROVIDER_ORIWeight-1) );
  SELF.VINWeight := MAP( ri.VINWeight=0 OR le.VIN_match_code=SALT37.MatchCode.ExactMatch => le.VINWeight,MIN(le.VINWeight,ri.VINWeight-1) );
  SELF.PLATEWeight := MAP( ri.PLATEWeight=0 OR le.PLATE_match_code=SALT37.MatchCode.ExactMatch => le.PLATEWeight,MIN(le.PLATEWeight,ri.PLATEWeight-1) );
  SELF.LATITUDEWeight := MAP( ri.LATITUDEWeight=0 OR le.LATITUDE_match_code=SALT37.MatchCode.ExactMatch => le.LATITUDEWeight,MIN(le.LATITUDEWeight,ri.LATITUDEWeight-1) );
  SELF.LONGITUDEWeight := MAP( ri.LONGITUDEWeight=0 OR le.LONGITUDE_match_code=SALT37.MatchCode.ExactMatch => le.LONGITUDEWeight,MIN(le.LONGITUDEWeight,ri.LONGITUDEWeight-1) );
  SELF.SEARCH_ADDR1Weight := MAP( ri.SEARCH_ADDR1Weight=0 OR le.SEARCH_ADDR1_match_code=SALT37.MatchCode.ExactMatch => le.SEARCH_ADDR1Weight,MIN(le.SEARCH_ADDR1Weight,ri.SEARCH_ADDR1Weight-1) );
  SELF.SEARCH_ADDR2Weight := MAP( ri.SEARCH_ADDR2Weight=0 OR le.SEARCH_ADDR2_match_code=SALT37.MatchCode.ExactMatch => le.SEARCH_ADDR2Weight,MIN(le.SEARCH_ADDR2Weight,ri.SEARCH_ADDR2Weight-1) );
  SELF.CLEAN_COMPANY_NAMEWeight := MAP( ri.CLEAN_COMPANY_NAMEWeight=0 OR le.CLEAN_COMPANY_NAME_match_code=SALT37.MatchCode.ExactMatch => le.CLEAN_COMPANY_NAMEWeight,MIN(le.CLEAN_COMPANY_NAMEWeight,ri.CLEAN_COMPANY_NAMEWeight-1) );
...
END;
  ...
  RETURN SORT(GROUP(R2,Reference,ALL),-weight,EID_HASH);
END;
EXPORT CombineAllScores(DATASET(LayoutScoredFetch) in_data, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  END;
  r0 := ROLLUP( SORT( GROUP( SORT ( DISTRIBUTE(In_Data,HASH(reference)),Reference, LOCAL ), Reference, LOCAL),EID_HASH),LEFT.EID_HASH=RIGHT.EID_HASH,Combine_Scores(LEFT,RIGHT,In_disableForce))(SALT37.DebugMode OR ~ForceFailed);
  r1 := AdjustScoresForNonExactMatches(UNGROUP(r0));
  R2 := ROLLUP( TOPN(r1,100,-Weight),GROUP, Create_Output(LEFT,ROWS(LEFT)) );
  SALT37.MAC_External_AddPcnt(R2,LayoutScoredFetch,Results,OutputLayout_Batch,28,R3);
  RETURN r3;
END;
...
// Note - results are returned distributed by HASH(reference) - this is part of the specification
  rolled := ROLLUP ( SORT( DISTRIBUTE( in_data, HASH(reference) ), Reference, EID_HASH, LOCAL), Combine_Scores(LEFT,RIGHT, In_disableForce), Reference, EID_HASH, LOCAL);
...
EXPORT Layout_RolledEntity := RECORD,MAXLENGTH(63000)
  ...
  DATASET(SALT37.Layout_FieldValueList) P_CITY_NAME_Values := DATASET([],SALT37.Layout_FieldValueList);
  ...
  DATASET(SALT37.Layout_FieldValueList) DL_ST_Values := DATASET([],SALT37.Layout_FieldValueList);
  DATASET(SALT37.Layout_FieldValueList) DL_Values := DATASET([],SALT37.Layout_FieldValueList);
  DATASET(SALT37.Layout_FieldValueList) LEXID_Values := DATASET([],SALT37.Layout_FieldValueList);
  DATASET(SALT37.Layout_FieldValueList) POSSIBLE_SSN_Values := DATASET([],SALT37.Layout_FieldValueList);
  DATASET(SALT37.Layout_FieldValueList) CRIME_Values := DATASET([],SALT37.Layout_FieldValueList);
  DATASET(SALT37.Layout_FieldValueList) NAME_TYPE_Values := DATASET([],SALT37.Layout_FieldValueList);
  DATASET(SALT37.Layout_FieldValueList) CLEAN_GENDER_Values := DATASET([],SALT37.Layout_FieldValueList);
  DATASET(SALT37.Layout_FieldValueList) CLASS_CODE_Values := DATASET([],SALT37.Layout_FieldValueList);
  ...
  DATASET(SALT37.Layout_FieldValueList) DATA_PROVIDER_ORI_Values := DATASET([],SALT37.Layout_FieldValueList);
  DATASET(SALT37.Layout_FieldValueList) VIN_Values := DATASET([],SALT37.Layout_FieldValueList);
  DATASET(SALT37.Layout_FieldValueList) PLATE_Values := DATASET([],SALT37.Layout_FieldValueList);
  DATASET(SALT37.Layout_FieldValueList) LATITUDE_Values := DATASET([],SALT37.Layout_FieldValueList);
  DATASET(SALT37.Layout_FieldValueList) LONGITUDE_Values := DATASET([],SALT37.Layout_FieldValueList);
  DATASET(SALT37.Layout_FieldValueList) SEARCH_ADDR1_Values := DATASET([],SALT37.Layout_FieldValueList);
  DATASET(SALT37.Layout_FieldValueList) SEARCH_ADDR2_Values := DATASET([],SALT37.Layout_FieldValueList);
  DATASET(SALT37.Layout_FieldValueList) CLEAN_COMPANY_NAME_Values := DATASET([],SALT37.Layout_FieldValueList);
...
SHARED RollEntities(dataset(Layout_RolledEntity) infile) := FUNCTION
  ...
  SELF.EID_HASH := le.EID_HASH;
    ...
    SELF.P_CITY_NAME_values := SALT37.fn_combine_fieldvaluelist(le.P_CITY_NAME_values,ri.P_CITY_NAME_values);
    ...
    SELF.DL_ST_values := SALT37.fn_combine_fieldvaluelist(le.DL_ST_values,ri.DL_ST_values);
    SELF.DL_values := SALT37.fn_combine_fieldvaluelist(le.DL_values,ri.DL_values);
    SELF.LEXID_values := SALT37.fn_combine_fieldvaluelist(le.LEXID_values,ri.LEXID_values);
    SELF.POSSIBLE_SSN_values := SALT37.fn_combine_fieldvaluelist(le.POSSIBLE_SSN_values,ri.POSSIBLE_SSN_values);
    SELF.CRIME_values := SALT37.fn_combine_fieldvaluelist(le.CRIME_values,ri.CRIME_values);
    SELF.NAME_TYPE_values := SALT37.fn_combine_fieldvaluelist(le.NAME_TYPE_values,ri.NAME_TYPE_values);
    SELF.CLEAN_GENDER_values := SALT37.fn_combine_fieldvaluelist(le.CLEAN_GENDER_values,ri.CLEAN_GENDER_values);
    SELF.CLASS_CODE_values := SALT37.fn_combine_fieldvaluelist(le.CLASS_CODE_values,ri.CLASS_CODE_values);
    ...
    SELF.DATA_PROVIDER_ORI_values := SALT37.fn_combine_fieldvaluelist(le.DATA_PROVIDER_ORI_values,ri.DATA_PROVIDER_ORI_values);
    SELF.VIN_values := SALT37.fn_combine_fieldvaluelist(le.VIN_values,ri.VIN_values);
    SELF.PLATE_values := SALT37.fn_combine_fieldvaluelist(le.PLATE_values,ri.PLATE_values);
    SELF.LATITUDE_values := SALT37.fn_combine_fieldvaluelist(le.LATITUDE_values,ri.LATITUDE_values);
    SELF.LONGITUDE_values := SALT37.fn_combine_fieldvaluelist(le.LONGITUDE_values,ri.LONGITUDE_values);
    SELF.SEARCH_ADDR1_values := SALT37.fn_combine_fieldvaluelist(le.SEARCH_ADDR1_values,ri.SEARCH_ADDR1_values);
    SELF.SEARCH_ADDR2_values := SALT37.fn_combine_fieldvaluelist(le.SEARCH_ADDR2_values,ri.SEARCH_ADDR2_values);
    SELF.CLEAN_COMPANY_NAME_values := SALT37.fn_combine_fieldvaluelist(le.CLEAN_COMPANY_NAME_values,ri.CLEAN_COMPANY_NAME_values);
...
Layout_RolledEntity into(in_data le) := TRANSFORM
  ...
  SELF.P_CITY_NAME_Values := IF ( (SALT37.StrType)le.P_CITY_NAME = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.P_CITY_NAME)}],SALT37.Layout_FieldValueList));
  ...
  SELF.DL_ST_Values := IF ( (SALT37.StrType)le.DL_ST = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.DL_ST)}],SALT37.Layout_FieldValueList));
  SELF.DL_Values := IF ( (SALT37.StrType)le.DL = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.DL)}],SALT37.Layout_FieldValueList));
  SELF.LEXID_Values := IF ( (SALT37.StrType)le.LEXID = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.LEXID)}],SALT37.Layout_FieldValueList));
  SELF.POSSIBLE_SSN_Values := IF ( (SALT37.StrType)le.POSSIBLE_SSN = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.POSSIBLE_SSN)}],SALT37.Layout_FieldValueList));
  SELF.CRIME_Values := IF ( (SALT37.StrType)le.CRIME = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.CRIME)}],SALT37.Layout_FieldValueList));
  SELF.NAME_TYPE_Values := IF ( (SALT37.StrType)le.NAME_TYPE = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.NAME_TYPE)}],SALT37.Layout_FieldValueList));
  SELF.CLEAN_GENDER_Values := IF ( (SALT37.StrType)le.CLEAN_GENDER = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.CLEAN_GENDER)}],SALT37.Layout_FieldValueList));
  SELF.CLASS_CODE_Values := IF ( (SALT37.StrType)le.CLASS_CODE = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.CLASS_CODE)}],SALT37.Layout_FieldValueList));
  SELF.DT_FIRST_SEEN_Values := IF ( (SALT37.StrType)le.DT_FIRST_SEEN = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.DT_FIRST_SEEN)}],SALT37.Layout_FieldValueList));
  SELF.DT_LAST_SEEN_Values := IF ( (SALT37.StrType)le.DT_LAST_SEEN = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.DT_LAST_SEEN)}],SALT37.Layout_FieldValueList));
  SELF.DATA_PROVIDER_ORI_Values := IF ( (SALT37.StrType)le.DATA_PROVIDER_ORI = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.DATA_PROVIDER_ORI)}],SALT37.Layout_FieldValueList));
  SELF.VIN_Values := IF ( (SALT37.StrType)le.VIN = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.VIN)}],SALT37.Layout_FieldValueList));
  SELF.PLATE_Values := IF ( (SALT37.StrType)le.PLATE = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.PLATE)}],SALT37.Layout_FieldValueList));
  SELF.LATITUDE_Values := IF ( (SALT37.StrType)le.LATITUDE = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.LATITUDE)}],SALT37.Layout_FieldValueList));
  SELF.LONGITUDE_Values := IF ( (SALT37.StrType)le.LONGITUDE = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.LONGITUDE)}],SALT37.Layout_FieldValueList));
  SELF.SEARCH_ADDR1_Values := IF ( (SALT37.StrType)le.SEARCH_ADDR1 = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.SEARCH_ADDR1)}],SALT37.Layout_FieldValueList));
  SELF.SEARCH_ADDR2_Values := IF ( (SALT37.StrType)le.SEARCH_ADDR2 = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.SEARCH_ADDR2)}],SALT37.Layout_FieldValueList));
  SELF.CLEAN_COMPANY_NAME_Values := IF ( (SALT37.StrType)le.CLEAN_COMPANY_NAME = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.CLEAN_COMPANY_NAME)}],SALT37.Layout_FieldValueList));
  SELF.FULLNAME_Values := IF ( (SALT37.StrType)le.FNAME = '' AND (SALT37.StrType)le.MNAME = '' AND (SALT37.StrType)le.LNAME = '' AND (SALT37.StrType)le.NAME_SUFFIX = '',DATASET([],SALT37.Layout_FieldValueList),DATASET([{TRIM((SALT37.StrType)le.FNAME) + ' ' + TRIM((SALT37.StrType)le.MNAME) + ' ' + TRIM((SALT37.StrType)le.LNAME) + ' ' + TRIM((SALT37.StrType)le.NAME_SUFFIX)}],SALT37.Layout_FieldValueList));
END;

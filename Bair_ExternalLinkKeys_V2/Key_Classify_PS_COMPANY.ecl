...
EXPORT KeyName := KeyPrefix+'::'+'key::Bair_ExternalLinkKeys_V2'+'::'+Config.KeyInfix+'::EID_HASH::Refs::COMPANY';
 
EXPORT KeyName_sf := KeyPrefix+'::'+'key::Bair_ExternalLinkKeys_V2'+'::'+KeySuperfile+'::EID_HASH::Refs::COMPANY';
...
EXPORT Shrinkage := DATASET([{'COMPANY','P_CITY_NAME',CntRed_P_CITY_NAME*100,CntRed_P_CITY_NAME*TSize},{'COMPANY','ST',CntRed_ST*100,CntRed_ST*TSize},{'COMPANY','SEARCH_ADDR2',CntRed_SEARCH_ADDR2*100,CntRed_SEARCH_ADDR2*TSize},{'COMPANY','LEXID',CntRed_LEXID*100,CntRed_LEXID*TSize}],SALT37.ShrinkLayout);
EXPORT CanSearch(Process_PS_Layouts.InputLayout le) := le.CLEAN_COMPANY_NAME <> (TYPEOF(le.CLEAN_COMPANY_NAME))'' AND Fields.InValid_CLEAN_COMPANY_NAME((SALT37.StrType)le.CLEAN_COMPANY_NAME)=0;
KeyRec := RECORDOF(Key);
...
EXPORT RawFetch(TYPEOF(h.CLEAN_COMPANY_NAME) param_CLEAN_COMPANY_NAME = (TYPEOF(h.CLEAN_COMPANY_NAME))'',TYPEOF(h.SEARCH_ADDR1) param_SEARCH_ADDR1 = (TYPEOF(h.SEARCH_ADDR1))'') := 
    STEPPED( LIMIT( Key(
      ...
      AND KEYED(( SEARCH_ADDR1 = (TYPEOF(SEARCH_ADDR1))'' OR param_SEARCH_ADDR1 = (TYPEOF(SEARCH_ADDR1))'' OR SEARCH_ADDR1 = param_SEARCH_ADDR1 ))),Config.COMPANY_MAXBLOCKLIMIT,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),EID_HASH);
...
EXPORT ScoredEID_HASHFetch(TYPEOF(h.CLEAN_COMPANY_NAME) param_CLEAN_COMPANY_NAME = (TYPEOF(h.CLEAN_COMPANY_NAME))'',TYPEOF(h.SEARCH_ADDR1) param_SEARCH_ADDR1 = (TYPEOF(h.SEARCH_ADDR1))'',TYPEOF(h.P_CITY_NAME) param_P_CITY_NAME = (TYPEOF(h.P_CITY_NAME))'',TYPEOF(h.ST) param_ST = (TYPEOF(h.ST))'',TYPEOF(h.SEARCH_ADDR2) param_SEARCH_ADDR2 = (TYPEOF(h.SEARCH_ADDR2))'',TYPEOF(h.LEXID) param_LEXID = (TYPEOF(h.LEXID))'',BOOLEAN param_disableForce = FALSE) := FUNCTION
  ...
  Process_PS_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.CLEAN_COMPANY_NAME_match_code := match_methods(File_Classify_PS).match_CLEAN_COMPANY_NAME(le.CLEAN_COMPANY_NAME,param_CLEAN_COMPANY_NAME,TRUE);
    SELF.CLEAN_COMPANY_NAMEWeight := (50+MAP (
           le.CLEAN_COMPANY_NAME = param_CLEAN_COMPANY_NAME  => le.CLEAN_COMPANY_NAME_weight100,
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  Process_PS_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.CLEAN_COMPANY_NAME_match_code := match_methods(File_Classify_PS).match_CLEAN_COMPANY_NAME(le.CLEAN_COMPANY_NAME,ri.CLEAN_COMPANY_NAME,TRUE);
    SELF.CLEAN_COMPANY_NAMEWeight := (50+MAP (
           le.CLEAN_COMPANY_NAME = ri.CLEAN_COMPANY_NAME  => le.CLEAN_COMPANY_NAME_weight100,
  ...
  J0 := JOIN(Recs1,Key,LEFT.CLEAN_COMPANY_NAME = RIGHT.CLEAN_COMPANY_NAME
    ...
    ATMOST(LEFT.CLEAN_COMPANY_NAME = RIGHT.CLEAN_COMPANY_NAME,Config.COMPANY_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.CLEAN_COMPANY_NAME = RIGHT.CLEAN_COMPANY_NAME
     AND ( LEFT.SEARCH_ADDR1 = (TYPEOF(LEFT.SEARCH_ADDR1))'' OR RIGHT.SEARCH_ADDR1 = (TYPEOF(RIGHT.SEARCH_ADDR1))'' OR LEFT.SEARCH_ADDR1 = RIGHT.SEARCH_ADDR1  ),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.CLEAN_COMPANY_NAME = RIGHT.CLEAN_COMPANY_NAME,Config.COMPANY_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_CLEAN_COMPANY_NAME='',Input_SEARCH_ADDR1='',Input_P_CITY_NAME='',Input_ST='',Input_SEARCH_ADDR2='',Input_LEXID='',output_file,AsIndex='true', In_disableForce = 'false') := MACRO
IMPORT SALT37,Bair_ExternalLinkKeys_V2;
#IF(#TEXT(Input_CLEAN_COMPANY_NAME)<>'')
  #uniquename(trans)
  Bair_ExternalLinkKeys_V2.Key_Classify_PS_COMPANY.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := Bair_ExternalLinkKeys_V2.Key_Classify_PS_COMPANY.ScoredFetch_Batch(%p%,AsIndex, In_disableForce);
#ELSE

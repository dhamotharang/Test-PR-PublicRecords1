...
EXPORT Summary(SALT37.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_DL_ST := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.DL_ST)));
    avelength_DL_ST := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.DL_ST)),h.DL_ST<>(typeof(h.DL_ST))'');
    populated_DL_pcnt := AVE(GROUP,IF(h.DL = (TYPEOF(h.DL))'',0,100));
    maxlength_DL := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.DL)));
    avelength_DL := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.DL)),h.DL<>(typeof(h.DL))'');
    populated_LEXID_pcnt := AVE(GROUP,IF(h.LEXID = (TYPEOF(h.LEXID))'',0,100));
    maxlength_LEXID := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.LEXID)));
    avelength_LEXID := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.LEXID)),h.LEXID<>(typeof(h.LEXID))'');
    populated_POSSIBLE_SSN_pcnt := AVE(GROUP,IF(h.POSSIBLE_SSN = (TYPEOF(h.POSSIBLE_SSN))'',0,100));
    maxlength_POSSIBLE_SSN := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.POSSIBLE_SSN)));
    avelength_POSSIBLE_SSN := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.POSSIBLE_SSN)),h.POSSIBLE_SSN<>(typeof(h.POSSIBLE_SSN))'');
    populated_CRIME_pcnt := AVE(GROUP,IF(h.CRIME = (TYPEOF(h.CRIME))'',0,100));
    maxlength_CRIME := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.CRIME)));
    avelength_CRIME := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.CRIME)),h.CRIME<>(typeof(h.CRIME))'');
    populated_NAME_TYPE_pcnt := AVE(GROUP,IF(h.NAME_TYPE = (TYPEOF(h.NAME_TYPE))'',0,100));
    maxlength_NAME_TYPE := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.NAME_TYPE)));
    avelength_NAME_TYPE := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.NAME_TYPE)),h.NAME_TYPE<>(typeof(h.NAME_TYPE))'');
    populated_CLEAN_GENDER_pcnt := AVE(GROUP,IF(h.CLEAN_GENDER = (TYPEOF(h.CLEAN_GENDER))'',0,100));
    maxlength_CLEAN_GENDER := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.CLEAN_GENDER)));
    avelength_CLEAN_GENDER := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.CLEAN_GENDER)),h.CLEAN_GENDER<>(typeof(h.CLEAN_GENDER))'');
    populated_CLASS_CODE_pcnt := AVE(GROUP,IF(h.CLASS_CODE = (TYPEOF(h.CLASS_CODE))'',0,100));
    maxlength_CLASS_CODE := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.CLASS_CODE)));
    avelength_CLASS_CODE := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.CLASS_CODE)),h.CLASS_CODE<>(typeof(h.CLASS_CODE))'');
    ...
    maxlength_DATA_PROVIDER_ORI := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.DATA_PROVIDER_ORI)));
    avelength_DATA_PROVIDER_ORI := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.DATA_PROVIDER_ORI)),h.DATA_PROVIDER_ORI<>(typeof(h.DATA_PROVIDER_ORI))'');
    populated_VIN_pcnt := AVE(GROUP,IF(h.VIN = (TYPEOF(h.VIN))'',0,100));
    maxlength_VIN := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.VIN)));
    avelength_VIN := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.VIN)),h.VIN<>(typeof(h.VIN))'');
    populated_PLATE_pcnt := AVE(GROUP,IF(h.PLATE = (TYPEOF(h.PLATE))'',0,100));
    maxlength_PLATE := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.PLATE)));
    avelength_PLATE := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.PLATE)),h.PLATE<>(typeof(h.PLATE))'');
    ...
    maxlength_SEARCH_ADDR1 := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.SEARCH_ADDR1)));
    avelength_SEARCH_ADDR1 := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.SEARCH_ADDR1)),h.SEARCH_ADDR1<>(typeof(h.SEARCH_ADDR1))'');
    populated_SEARCH_ADDR2_pcnt := AVE(GROUP,IF(h.SEARCH_ADDR2 = (TYPEOF(h.SEARCH_ADDR2))'',0,100));
    maxlength_SEARCH_ADDR2 := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.SEARCH_ADDR2)));
    avelength_SEARCH_ADDR2 := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.SEARCH_ADDR2)),h.SEARCH_ADDR2<>(typeof(h.SEARCH_ADDR2))'');
...
SALT37.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.id := le.EID_HASH;
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT37.StrType)le.NAME_SUFFIX),TRIM((SALT37.StrType)le.FNAME),TRIM((SALT37.StrType)le.MNAME),TRIM((SALT37.StrType)le.LNAME),TRIM((SALT37.StrType)le.PRIM_RANGE),TRIM((SALT37.StrType)le.PRIM_NAME),TRIM((SALT37.StrType)le.SEC_RANGE),TRIM((SALT37.StrType)le.P_CITY_NAME),TRIM((SALT37.StrType)le.ST),TRIM((SALT37.StrType)le.ZIP),TRIM((SALT37.StrType)le.DOB),TRIM((SALT37.StrType)le.PHONE),TRIM((SALT37.StrType)le.DL_ST),TRIM((SALT37.StrType)le.DL),TRIM((SALT37.StrType)le.LEXID),TRIM((SALT37.StrType)le.POSSIBLE_SSN),TRIM((SALT37.StrType)le.CRIME),TRIM((SALT37.StrType)le.NAME_TYPE),TRIM((SALT37.StrType)le.CLEAN_GENDER),TRIM((SALT37.StrType)le.CLASS_CODE),TRIM((SALT37.StrType)le.DT_FIRST_SEEN),TRIM((SALT37.StrType)le.DT_LAST_SEEN),TRIM((SALT37.StrType)le.DATA_PROVIDER_ORI),TRIM((SALT37.StrType)le.VIN),TRIM((SALT37.StrType)le.PLATE),TRIM((SALT37.StrType)le.LATITUDE),TRIM((SALT37.StrType)le.LONGITUDE),TRIM((SALT37.StrType)le.SEARCH_ADDR1),TRIM((SALT37.StrType)le.SEARCH_ADDR2),TRIM((SALT37.StrType)le.CLEAN_COMPANY_NAME)));
  SELF.FldNo := C;
END;
...
SALT37.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 30);
  SELF.FldNo2 := 1 + (C % 30);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT37.StrType)le.NAME_SUFFIX),TRIM((SALT37.StrType)le.FNAME),TRIM((SALT37.StrType)le.MNAME),TRIM((SALT37.StrType)le.LNAME),TRIM((SALT37.StrType)le.PRIM_RANGE),TRIM((SALT37.StrType)le.PRIM_NAME),TRIM((SALT37.StrType)le.SEC_RANGE),TRIM((SALT37.StrType)le.P_CITY_NAME),TRIM((SALT37.StrType)le.ST),TRIM((SALT37.StrType)le.ZIP),TRIM((SALT37.StrType)le.DOB),TRIM((SALT37.StrType)le.PHONE),TRIM((SALT37.StrType)le.DL_ST),TRIM((SALT37.StrType)le.DL),TRIM((SALT37.StrType)le.LEXID),TRIM((SALT37.StrType)le.POSSIBLE_SSN),TRIM((SALT37.StrType)le.CRIME),TRIM((SALT37.StrType)le.NAME_TYPE),TRIM((SALT37.StrType)le.CLEAN_GENDER),TRIM((SALT37.StrType)le.CLASS_CODE),TRIM((SALT37.StrType)le.DT_FIRST_SEEN),TRIM((SALT37.StrType)le.DT_LAST_SEEN),TRIM((SALT37.StrType)le.DATA_PROVIDER_ORI),TRIM((SALT37.StrType)le.VIN),TRIM((SALT37.StrType)le.PLATE),TRIM((SALT37.StrType)le.LATITUDE),TRIM((SALT37.StrType)le.LONGITUDE),TRIM((SALT37.StrType)le.SEARCH_ADDR1),TRIM((SALT37.StrType)le.SEARCH_ADDR2),TRIM((SALT37.StrType)le.CLEAN_COMPANY_NAME)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT37.StrType)le.NAME_SUFFIX),TRIM((SALT37.StrType)le.FNAME),TRIM((SALT37.StrType)le.MNAME),TRIM((SALT37.StrType)le.LNAME),TRIM((SALT37.StrType)le.PRIM_RANGE),TRIM((SALT37.StrType)le.PRIM_NAME),TRIM((SALT37.StrType)le.SEC_RANGE),TRIM((SALT37.StrType)le.P_CITY_NAME),TRIM((SALT37.StrType)le.ST),TRIM((SALT37.StrType)le.ZIP),TRIM((SALT37.StrType)le.DOB),TRIM((SALT37.StrType)le.PHONE),TRIM((SALT37.StrType)le.DL_ST),TRIM((SALT37.StrType)le.DL),TRIM((SALT37.StrType)le.LEXID),TRIM((SALT37.StrType)le.POSSIBLE_SSN),TRIM((SALT37.StrType)le.CRIME),TRIM((SALT37.StrType)le.NAME_TYPE),TRIM((SALT37.StrType)le.CLEAN_GENDER),TRIM((SALT37.StrType)le.CLASS_CODE),TRIM((SALT37.StrType)le.DT_FIRST_SEEN),TRIM((SALT37.StrType)le.DT_LAST_SEEN),TRIM((SALT37.StrType)le.DATA_PROVIDER_ORI),TRIM((SALT37.StrType)le.VIN),TRIM((SALT37.StrType)le.PLATE),TRIM((SALT37.StrType)le.LATITUDE),TRIM((SALT37.StrType)le.LONGITUDE),TRIM((SALT37.StrType)le.SEARCH_ADDR1),TRIM((SALT37.StrType)le.SEARCH_ADDR2),TRIM((SALT37.StrType)le.CLEAN_COMPANY_NAME)));
  END;
...
SHARED FldIds := DATASET([{1,'NAME_SUFFIX'}
      ...
      ,{28,'SEARCH_ADDR1'}
      ,{29,'SEARCH_ADDR2'}
      ,{30,'CLEAN_COMPANY_NAME'}],SALT37.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_DL_ST((SALT37.StrType)le.DL_ST),
    Fields.InValid_DL((SALT37.StrType)le.DL),
    Fields.InValid_LEXID((SALT37.StrType)le.LEXID),
    Fields.InValid_POSSIBLE_SSN((SALT37.StrType)le.POSSIBLE_SSN),
    Fields.InValid_CRIME((SALT37.StrType)le.CRIME),
    Fields.InValid_NAME_TYPE((SALT37.StrType)le.NAME_TYPE),
    Fields.InValid_CLEAN_GENDER((SALT37.StrType)le.CLEAN_GENDER),
    Fields.InValid_CLASS_CODE((SALT37.StrType)le.CLASS_CODE),
    Fields.InValid_DT_FIRST_SEEN((SALT37.StrType)le.DT_FIRST_SEEN),
    Fields.InValid_DT_LAST_SEEN((SALT37.StrType)le.DT_LAST_SEEN),
    Fields.InValid_DATA_PROVIDER_ORI((SALT37.StrType)le.DATA_PROVIDER_ORI),
    Fields.InValid_VIN((SALT37.StrType)le.VIN),
    Fields.InValid_PLATE((SALT37.StrType)le.PLATE),
    Fields.InValid_LATITUDE((SALT37.StrType)le.LATITUDE),
    Fields.InValid_LONGITUDE((SALT37.StrType)le.LONGITUDE),
    Fields.InValid_SEARCH_ADDR1((SALT37.StrType)le.SEARCH_ADDR1),
    Fields.InValid_SEARCH_ADDR2((SALT37.StrType)le.SEARCH_ADDR2),
...
PrettyErrorTotals := RECORD
  ...
  FieldType := CHOOSE(TotalErrors.FieldNum,'T_ALPHANUM','T_ALPHANUM','T_ALPHANUM','T_ALPHANUM','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','NUMBER','DEFAULT','NUMBER','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','Unknown','Unknown');
...
EXPORT ClusterCounts := SALT37.MOD_ClusterStats.Counts(h,EID_HASH);
END;

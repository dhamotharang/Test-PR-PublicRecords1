EXPORT createUpdateSuperFile(string pversion, boolean pUseDelta = false) := MODULE
	...
	//qa super
			...
			IF( NOT fileservices.superfileexists(KeyNames(pversion,pUseDelta).lname_super),fileservices.createsuperfile(KeyNames(pversion,pUseDelta).lname_super)),
	//father		
			...
			IF( NOT fileservices.superfileexists(KeyNames(pversion,pUseDelta).lname_father),fileservices.createsuperfile(KeyNames(pversion,pUseDelta).lname_father)),
			);												
		RETURN superFiles;
	END;
	...
	EXPORT updatexIDLSuperFile(string baseSuperFile, string fatherSuperFile, string inFile,boolean pUseDelta = false,  string delta_file='' ) := FUNCTION
		 ...
		 action := 	If(pUseDelta,
								...
								,Sequential(
										nothor(fileservices.Clearsuperfile(fatherSuperFile,true))
									 ,nothor(fileservices.Addsuperfile(fatherSuperFile,baseSuperFile,addcontents := true))
									 ,nothor(fileservices.Clearsuperfile(baseSuperFile))
									 ,nothor(fileservices.Addsuperfile(baseSuperFile,inFile))
													)
	...
	KeyIn := Bair_ExternalLinkKeys_V2.KeyNames(pversion,pUseDelta);
	...
	t15	:= updatexIDLSuperFile(KeyIn.lname_super,			KeyIn.lname_father,			KeyIn.lname_logical		,pUseDelta, 	KeyIn.lname_Delta);	

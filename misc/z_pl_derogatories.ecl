import prof_licensev2,doxie,doxie_files,sexoffender;
...
pd := prof_licensev2.File_Proflic_Base_Tiers((integer) did<>0);
pds := sort(distribute(pd,hash(prolic_key)),prolic_key,-prolic_seq_id,local);
pdd := dedup(pds,							prolic_key,     local);
...
o1:=output(providers_with_crim,,'~thor::temp::_pl_derogatories',overwrite);

export _pl_derogatories := o1;

IMPORT Address, BatchDatasets, FraudShared;
...
EXPORT IParam := MODULE
  ...
  EXPORT getBatchParams() := FUNCTION
    ...
    in_mod := MODULE(PROJECT(base_params, BatchParams, OPT))        
      ...
      // EXPORT UNSIGNED8   MaxResultsPerAcct    := FraudGovPlatform_Services.Constants.Defaults.MaxResultsPerAcctno  : stored('MaxResultsPerAcct');
      ...
      EXPORT integer   MaxVelocities      := FraudGovPlatform_Services.Constants.MAXVELOCITIES : STORED('MaxVelocities');
      EXPORT string    FraudPlatform			:= FraudGovPlatform_Services.Constants.FRAUDPLATFORM : STORED('FraudPlatform');
    END;

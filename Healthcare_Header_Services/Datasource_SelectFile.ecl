...
EXPORT Datasource_SelectFile := MODULE
	Export get_providers_base (dataset(Layouts.searchKeyResults_plus_input) input):= function
			...
			rawdataIndividualbyVendorid:= join(noHits, Enclarity.Keys(,true).individual_group_key.qa,
											keyed(left.vendorid = right.group_key) and left.src = Constants.SRC_SELECTFILE and right.record_type = 'C',
											transform(Layouts.selectfile_providers_base_with_input,
			...
			rawdataIndividualwithAddrScores := join(rawdataIndividualbyVendorid, Enclarity.Keys(,true).associate_group_key.qa,
			...
			baseRecs := project(sort(rawdataDeduped,acctno,-addr_conf_score,-clean_last_verify_date),Transforms.build_selectfile_Provider_base(left));
			// rejoinLNPID := join(dedup(sort(input(vendorid<>''),acctno,lnpid,vendorid),acctno,lnpid,vendorid),providers_rolled,left.acctno=right.acctno and left.vendorid = right.vendorid,transform(Layouts.CombinedHeaderResults,self:=left;self:=right),keep(Constants.IDS_PER_DID), limit(0));
			// output(rawdataIndividual,named('rawdataIndividual'));
			// output(noHits,named('noHits'));
			// output(rawdataIndividualbyVendorid);
	...
	Export appendLicense(dataset(Layouts.CombinedHeaderResults) input, dataset(Layouts.GroupKey) searchby):= function
	...
	Export appendSpecialty(dataset(Layouts.CombinedHeaderResults) input, dataset(Layouts.GroupKey) searchby):= function
	...
	Export appendMedSchool(dataset(Layouts.CombinedHeaderResults) input, dataset(Layouts.GroupKey) searchby):= function
			rawdata := join(searchby, Enclarity.Keys(,true).medschool_group_key.qa,
											keyed(left.group_key = right.group_key),
											transform(Layouts.layout_medschool, 
																		self.acctno:=left.acctno;
																		self.group_key:=left.group_key;
																		self.providerid := left.providerid;
																		...
																		self:=[]),
											keep(Constants.MAX_RECS_ON_JOIN), limit(0))(MedSchoolName<>''); 
			Layouts.layout_child_medschool doRollup(Layouts.layout_medschool l, dataset(Layouts.layout_medschool) r) := TRANSFORM
				SELF.acctno := l.acctno;
				self.group_key := l.group_key;
				self.ProviderID := l.ProviderID;
				self.childinfo := r;
	...
	Export appendHospitals(dataset(Layouts.CombinedHeaderResults) input, dataset(Layouts.GroupKey) searchby):= function
	...
	Export appendAssociation(dataset(Layouts.CombinedHeaderResults) input, dataset(Layouts.GroupKey) searchby):= function
			rawdata := join(searchby, Enclarity.Keys(,true).associate_group_key.qa,
	...
	Export appendTaxonomy(dataset(Layouts.CombinedHeaderResults) input, dataset(Layouts.GroupKey) searchby):= function
			rawdata := join(searchby, Enclarity.Keys(,true).taxonomy_group_key.qa,
											keyed(left.group_key = right.group_key),
											transform(Layouts.layout_taxonomy, 
																		self.acctno:=left.acctno;
																		self.group_key:=left.group_key;
																		self.providerid := left.providerid;
																		...
																		self:=[]),
											keep(Constants.MAX_RECS_ON_JOIN), limit(0)); 
			Layouts.layout_child_taxonomy doRollup(Layouts.layout_taxonomy l, dataset(Layouts.layout_taxonomy) r) := TRANSFORM
				SELF.acctno := l.acctno;
				self.group_key := l.group_key;
				self.ProviderID := l.ProviderID;
				self.childinfo := r;
	...
	Shared appendSanctionGroup(dataset(Layouts.layout_sanctions) input):= function
			...
			sanctions_final := join(input,grpDate, left.acctno=right.acctno and left.ProviderID = right.ProviderID and
																	...
																	transform(Layouts.layout_sanctions, self.GroupSancDate:=right.clean_sanc1_date;self:=left;),keep(Constants.MAX_RECS_ON_JOIN), limit(0));
			//Set Sancid
			...
			Layouts.layout_sanctions addSeq(Layouts.layout_sanctions inp,integer C) := transform
	...
	Shared appendSanctionLookupStep1(dataset(Layouts.layout_sanctions) input):= function
			rawdatalookup1 := join(input, Enclarity.Keys(,true).sanc_codes_sanc_codes.qa,
											keyed(left.sanc1_code = right.sanc_code),
											transform(Layouts.layout_sanctions, 
																		self.sanc_desc := right.sanc_desc;
																		self:=left;
																		self:=[]),
											keep(Constants.MAX_RECS_ON_JOIN), limit(0),left outer); 
	...
	Shared appendSanctionLookup(dataset(Layouts.layout_sanctions) input):= function
	...
	Export appendSanction(dataset(Layouts.CombinedHeaderResults) input, dataset(Layouts.GroupKey) searchby):= function
			...
			Layouts.layout_child_sanctions doRollup(Layouts.layout_sanctions l, dataset(Layouts.layout_sanctions) r) := TRANSFORM
				SELF.acctno := l.acctno;
				self.group_key := l.group_key;
				self.ProviderID := l.ProviderID;
				self.childinfo := r;
	...
	Shared get_facilities_base (dataset(Layouts.searchKeyResults_plus_input) input):= function
			// rawdataFacilities:= join(dedup(sort(input(lnpid>0),record),record), Enclarity.Keys(,true).facility_lnpid.qa,
											// keyed(left.lnpid = right.lnpid),
											// transform(Layouts.selectfile_facilities_base_with_input, 
																		// SELF.clean_prim_range:=right.prim_range;
																		// SELF.clean_predir:=right.predir;
																		// SELF.clean_prim_name:=right.prim_name;
																		// SELF.clean_addr_suffix:=right.addr_suffix;
																		...
																		// self:=right),
											// keep(Constants.MAX_RECS_ON_JOIN), limit(0)); 
			// noHits := join(input,rawdataFacilities,left.acctno=right.acctno,transform(Layouts.searchKeyResults_plus_input, self:=left),left only);
			noHits := input;
			...
			getFEIN := dedup(sort(join(input(vendorid<>''),enclarity.Keys(,true).associate_bgk.qa,
													...
													transform(Layouts.layout_acct_fein, self.acctno:=left.acctno;self.providerid:=left.lnpid;self.fein:=right.bill_tin),
													keep(Constants.MAX_RECS_ON_JOIN), limit(0)),record),record); 
			layouts.layout_child_fein doRollup(layouts.layout_acct_fein l, dataset(layouts.layout_acct_fein) r) := TRANSFORM
				SELF.acctno := l.acctno;
				SELF.ProviderID := l.providerid;
				self.childinfo := project(r,layouts.layout_fein);
			END;
			...
			mergewithBase := join(baseRecs,merge_facilities_rolled,left.acctno=right.acctno and left.lnpid = right.lnpid,transform(Layouts.CombinedHeaderResults,
			...
			leftonlyRecs := join(baseRecs,mergewithBase,left.acctno=right.acctno and left.lnpid = right.lnpid,transform(Layouts.CombinedHeaderResults,self:=left),left only);
			rightonlyRecs := join(merge_facilities_rolled,mergewithBase,left.acctno=right.acctno and left.lnpid = right.lnpid,transform(Layouts.CombinedHeaderResults,self:=left),left only);
			...
			updateFEIN := join(mergedRecs,fein_rolled,left.acctno=right.acctno and left.lnpid=right.providerid,transform(Layouts.CombinedHeaderResults, self.feins:=dedup(sort(left.feins+right.childinfo,record),record);self:=left;self:=[];),left outer,keep(Constants.MAX_RECS_ON_JOIN), limit(0));
			updateSanctions := project(updateFEIN,transform(Layouts.CombinedHeaderResults,
																										...
																										self := Left));
			getInput:= project(input,transform(layouts.autokeyInput,self.license_state:=left.UserLicenseState;self.license_number:=left.UserLicenseNumber;
	...
	Export get_selectfile_entity (dataset(Layouts.searchKeyResults_plus_input) input,dataset(Layouts.common_runtime_config) cfg):= function
			providers_base := get_providers_base(input);
			getkeys := dedup(project(providers_base,transform(Layouts.GroupKey, self.acctno := left.acctno;self.ProviderID:=left.srcid;self.group_key :=left.Vendorid;)),all,hash);
			// Append Other data
	...
	Export get_selectfile_byBusReEntry (dataset(Layouts.autokeyInput) input):= function
		...
		akFmt:=dedup(sort(project(ak,transform(Layouts.searchKeyResults, self.vendorid:=(string)left.group_key; self.acctno:=left.acctno;self.src:=Constants.SRC_SELECTFILE;self.prov_id:=left.fakeid;self.isAutokeysResult:=true;self:=[];)),record),record);
		...
		possibleFEIN_input := dedup(sort(join(input,enclarity.Keys(,true).associate_bill_tin.qa,keyed((string)left.providerid=right.bill_tin),transform(Layouts.searchKeyResults_plus_input,
																																						self.isAutokeysResult := true;
																																						self.src:=Constants.SRC_SELECTFILE;
																																						...
																																						self:=right;self:=left),keep(Constants.MAX_RECS_ON_JOIN),limit(0))(vendorid<>''),record),record);
	...
	Export get_selectfile_byBusAutokeys (dataset(Layouts.autokeyInput) input):= function
		...
		akFmt:=dedup(sort(project(akFilter,transform(Layouts.searchKeyResults, self.vendorid:=(string)left.group_key; self.acctno:=left.acctno;self.src:=Constants.SRC_SELECTFILE;self.prov_id:=left.fakeid;self.isAutokeysResult:=true;self:=[];)),record),record);
		searchKeyResults_w_input := join(input,akFmt,left.acctno=right.acctno,transform(Layouts.searchKeyResults_plus_input,
																																						self.isAutokeysResult := true;
																																						self.UserLicenseNumber:=left.license_number;
																																						self.UserLicenseState:=left.license_state;
		...
		searchByFEIN_w_input := dedup(sort(join(searchByFEIN,enclarity.Keys(,true).associate_bill_tin.qa,keyed(left.FEIN=right.bill_tin),transform(Layouts.searchKeyResults_plus_input,
																																						self.src:=Constants.SRC_SELECTFILE;
																																						self.isAutokeysResult := true;
	...
	Export getSanctionsCommon (dataset(Layouts.autokeyInput) input, dataset(Layouts.sanc_lookup_rec) sanc_keyRec, dataset(Layouts.common_runtime_config) cfg):= function
		...
		layouts.layout_LegacySanctions get_sanctions_by_sancid(Layouts.sanc_lookup_rec l, doxie_files.key_sanctions_sancid r) := transform
		...
		sancs_by_sid_filter1:=join(input(cfg[1].excludeLegacySanctions=false),sancs_by_sid,left.acctno=right.acctno and 
														 ...
														 left.providersrc = Constants.SRC_SANC or right.src ='NH' or right.src = 'LN'),
															transform(layouts.layout_LegacySanctions, 
																...
																isIndvSearch := left.comp_name = '' and left.name_last <> '' or left.providersrc = Constants.SRC_SANC;
																self.SANC_ID:=if((isBusSearch and ((bestmatch>=Constants.BUS_NAME_BIPMATCH_THRESHOLD and cityStateMatch) or (right.src ='NH' or right.src = 'LN'))) or 
																									isIndvSearch,(integer)right.SANC_ID,skip);
																self:=right),keep(Constants.MAX_RECS_ON_JOIN), limit(0));
		//Handle Jr/Sr problems
		sancs_by_sid_filtered:=join(input,dedup(sancs_by_sid_filter1,record,all),left.acctno=right.acctno,transform(layouts.layout_LegacySanctions,
																										...
																										self.providerid:=if(skipRec or 
																																				(left.providersrc = Constants.SRC_SANC and (integer)left.providerid <> (integer)right.sanc_id),
		...
		layouts.layout_child_LegacySanctions doRollup(layouts.layout_LegacySanctions l, dataset(layouts.layout_LegacySanctions) r) := TRANSFORM
			SELF.acctno := l.acctno;
			self.ProviderID := l.ProviderID;
			self.childinfo := r;
		END;
		results_rolled := rollup(f_sancs_dep,group,doRollup(left,rows(left)));
		fullRec := project(dedup(sort(f_sancs_dep,acctno,providerid),acctno,providerid), transform(Layouts.CombinedHeaderResults,
																self.acctno := left.acctno;
																self.LNPID := left.providerid;
																self.srcID := left.SANC_ID;
																self.src := Constants.SRC_SANC;
																self.names := dataset([{1,0,'',
																											...
																											left.sanc_busnme}],Layouts.layout_nameinfo);
																self.Addresses := dataset([{1,0,
																											...
																											'','','',''}],Layouts.layout_addressinfo);
																self.dobs := dataset([{left.sanc_dob}],Layouts.layout_dob)(dob<>'');
																self.dids := dataset([{(integer)left.did}],Layouts.layout_did)(did>0);
																self.taxids := dataset([{left.sanc_tin}],Layouts.layout_taxid)(taxid<>'');
																self.upins := dataset([{left.acctno,left.SANC_ID,left.sanc_upin}],Layouts.layout_upin)(upin<>'');
																self.StateLicenses := dataset([{left.acctno,left.SANC_ID,0,left.sanc_sancst,left.sanc_licnbr,'',''}],Layouts.layout_licenseinfo)(LicenseNumber<>'');
																self:=left;self:=[];));
		fullRecFinal := join(fullRec,results_rolled, left.acctno=right.acctno and left.LNPID=right.ProviderID, transform(Layouts.CombinedHeaderResults,self.LegacySanctions:=right.childinfo;self:=left),keep(Constants.MAX_RECS_ON_JOIN), limit(0));
		// output(sanc_keyRec);
		// output(input);
		// output(sancs_by_sid);
	...
	Export getSanctions (dataset(Layouts.autokeyInput) input,dataset(Layouts.common_runtime_config) cfg):= function
		//Hit by License Number or UPIN
		sanc_byUpin := join(input(cfg[1].excludeLegacySanctions=false), doxie_files.key_upin_sancid, keyed(left.upin = right.l_upin),transform(Layouts.sanc_lookup_rec, self.acctno:=left.acctno;self.providerid:=left.providerid;self.SANC_ID:=right.sanc_id;self.src:='U'),keep(Constants.MAX_RECS_ON_JOIN), limit(0));
		sanc_byStLic := join(input(cfg[1].excludeLegacySanctions=false), doxie_files.key_license_sancid, keyed(left.license_state = right.SANC_SANCST) and left.license_number = right.SANC_LICNBR,transform(Layouts.sanc_lookup_rec, self.acctno:=left.acctno;self.providerid:=left.providerid;self.SANC_ID:=right.sanc_id;self.src:='L'),keep(Constants.MAX_RECS_ON_JOIN), limit(0));
		...
		NoHitsforAK := join(input(cfg[1].excludeLegacySanctions=false),sanc_keyRec,left.acctno=right.acctno,transform(Layouts.autokeyInput,self:=left),left only);
		//Hit autokeys
		//Remove bad input records before sending to autokeys
		removeBadRecs := Project(NoHitsforAK, transform(Layouts.autokeyInput,
		...
		removeBadRecsSanc := Project(removeBadRecs, transform(Layouts.autokeyInput,
																		noAddress:=left.prim_name='' and left.p_city_name='' and left.st ='' and left.z5 = '';;
																		self.name_last := if(noAddress,skip,left.name_last);
																		self := left;));

		sanc_ids_byak := project(AutoKey_for_Batch(removeBadRecsSanc).sanc_autokeys,transform(Layouts.sanc_lookup_rec, self.acctno:=left.acctno;self.providerid:=(integer)left.sanc_id;self.SANC_ID:=(integer)left.sanc_id;self.src:='AK'));
	...
	Export getBusSanctions (dataset(Layouts.autokeyInput) input,dataset(Layouts.common_runtime_config) cfg):= function
		//Hit by Busness Name
		bussanc_by_Bus_name := join(dedup(sort(input(cfg[1].excludeLegacySanctions=false and comp_name <>''),acctno,comp_name),acctno,comp_name), doxie_files.key_sanctions_busname,keyed(left.comp_name[1..7] = right.s_sanc_busnme[1..7]),
												transform(Layouts.sanc_lookup_rec, 
																	...
																	self.src:='BUS';
																	self.SANC_ID:=if(bestmatch>=Constants.BUS_NAME_BIPMATCH_THRESHOLD,(integer)right.SANC_ID,skip);
																	self:=left;),keep(Constants.MAX_RECS_ON_JOIN), limit(0));
		//Hit by FEIN and match on Business Name TaxID FEIN
		Bussanc_bytaxid := join(dedup(sort(input(cfg[1].excludeLegacySanctions=false and TaxID <>''),acctno,TaxID),acctno,TaxID), doxie_files.key_sanctions_tin, keyed((string)INTFORMAT((integer)left.TaxID,9,1) = right.s_SANC_tin and left.taxid <>''),transform(Layouts.sanc_lookup_rec,keepit := (integer)right.SANC_ID>0 and (integer)right.SANC_ID<10000000;self.acctno:=if(keepit,left.acctno,skip);self.providerid:=left.providerid;self.SANC_ID:=(integer)right.sanc_id;self.src:='F'),keep(Constants.MAX_RECS_ON_JOIN), limit(0));
		Bussanc_byfein := join(dedup(sort(input(cfg[1].excludeLegacySanctions=false and fein <> ''),acctno,fein),acctno,fein), doxie_files.key_sanctions_tin, keyed((string)INTFORMAT((integer)left.fein,9,1) = right.s_SANC_tin and left.fein <>''),transform(Layouts.sanc_lookup_rec, keepit := (integer)right.SANC_ID>0 and (integer)right.SANC_ID<10000000;self.acctno:=if(keepit,left.acctno,skip);self.providerid:=left.providerid;self.SANC_ID:=(integer)right.sanc_id;self.src:='F'),keep(Constants.MAX_RECS_ON_JOIN), limit(0));
		NoHitsforAK := join(input(cfg[1].excludeLegacySanctions=false),bussanc_by_Bus_name+Bussanc_byfein+Bussanc_bytaxid,left.acctno=right.acctno,transform(Layouts.autokeyInput,self:=left),left only);
		//Hit autokeys (Address search)

		sanc_ids_byak := project(AutoKey_for_Batch(NoHitsforAK).sanc_autokeys,transform(Layouts.sanc_lookup_rec, self.acctno:=left.acctno;self.SANC_ID:=(integer)left.sanc_id;self.src:='AK'));
		//Only return Sanction ID < 1000000 as those above that number are backfill keys Enclarity data not Gap data
...
/* Need good test case to finish up
	Export getBusSanctionsForResults (dataset(Layouts.CombinedHeaderResults) recs,dataset(Layouts.common_runtime_config) cfg):= function
...
*/
	...
	Export getSanctionsReentry (dataset(Layouts.autokeyInput) input,dataset(Layouts.common_runtime_config) cfg):= function
		//Provider ID should be the Sanc ID
		recs := project(input,transform(Layouts.sanc_lookup_rec, self.acctno:=left.acctno;self.SANC_ID:=left.ProviderID;self.src:='S'));
		rawdata := getSanctionsCommon(input,recs,cfg);
		return rawdata;
	end;
	//Support Gap Sanctions via found business records and Input Criteria
	Export getAppendBusSanctions (dataset(Layouts.CombinedHeaderResults) Recs,dataset(Layouts.autokeyInput) input,dataset(Layouts.common_runtime_config) cfg):= function
			...
			prefill := join(recs,getLinkedSanctions,left.acctno=right.acctno and left.lnpid=right.lnpid, 
																								transform(Layouts.CombinedHeaderResults,
																														...
																														keep(Constants.MAX_RECS_ON_JOIN), limit(0));
			//Remaining Sanctions
			remainingRecs := join(recs,checklnpidkey,left.acctno=right.acctno,transform(Layouts.CombinedHeaderResults, self:=left),left only);
			remainingInputs := join(input,checklnpidkey,left.acctno=right.acctno,transform(Layouts.autokeyInput, self:=left),left only);
			...
			appendRecs:=join(remainingRecs,getSanc,left.acctno=right.acctno, transform(Layouts.CombinedHeaderResults,
			...
			unlinkedRecs := join(recs,linkedRecs,left.acctno=right.acctno,transform(Layouts.CombinedHeaderResults, self:=left),left only);

...
EXPORT Records := module
	...
	Export getRecordsIndividual (dataset(Healthcare_Header_Services.Layouts.autokeyInput) input, dataset(Healthcare_Header_Services.Layouts.common_runtime_config) cfg = RunTimeConfig) := function
	...
	Export getRecordsBusiness (dataset(Healthcare_Header_Services.Layouts.autokeyInput) input, dataset(Healthcare_Header_Services.Layouts.common_runtime_config) cfg = RunTimeConfig) := function
		ReEntryRecords := input(ProviderSrc <> '' and ProviderID > 0);//These are records that are coming back into the code from a result list
		SearchInputRecords := join(input,ReEntryRecords,left.acctno = right.acctno,transform(Healthcare_Header_Services.Layouts.autokeyInput,self:=left),left only);//Contains input criteria
		ReEntryHeaderRecords := ReEntryRecords(ProviderSrc = Healthcare_Header_Services.Constants.SRC_HEADER);//These are records that are coming back into the code from a result list that originated from the header
		...
		NoHitsforBusDeepDive := join (BusinessRecords,finalRawBusinesses(issearchfailed=false)+getGSABusSanctionsHits,left.acctno=right.acctno,transform(Healthcare_Header_Services.Layouts.autokeyInput,self:=left),left only);
		//Do Business Deep Dive
		getBusinessDeepDive := Datasource_Boca_Bus_Header.get_boca_bus_header_entity(NoHitsforBusDeepDive(cfg[1].doDeepDive or cfg[1].IncludeAlsoFound),cfg);
	...
	Export getRecordsBusinessLegacy (dataset(Healthcare_Header_Services.Layouts.autokeyInput) input, dataset(Healthcare_Header_Services.Layouts.common_runtime_config) cfg = RunTimeConfig) := function
		...
		//Do Business Deep Dive
		getBusinessDeepDive := Datasource_Boca_Bus_Header.get_boca_bus_header_entity(NoHitsforBusDeepDive,cfg);
		//Final Business Records
		...
		finalRecs := Project(Functions.doPenalty(rolledRecs,dedup(sort(input,acctno),acctno),cfg[1].penalty_threshold), transform(Healthcare_Header_Services.Layouts.CombinedHeaderResults, 		self.Names         := LEFT.Names(nameSeq>0); self := left;));
	...
	Export getRecordsAppend (dataset(Healthcare_Header_Services.Layouts.autokeyInput) input, dataset(Healthcare_Header_Services.Layouts.CombinedHeaderResults) rawRecs, dataset(Healthcare_Header_Services.Layouts.common_runtime_config) cfg = RunTimeConfig) := function
		...
		fmtRec_Final := join(fmtRec_GSASanctions,input(derivedinputrecord=false),left.acctno=right.acctno,Functions_Validation.processVerifications(left,right,cfg),keep(Healthcare_Header_Services.Constants.MAX_RECS_ON_JOIN), limit(0));
    fmtRec_FinalNew := if(cfg[1].includeAffiliations,functions.AppendAffiliations(fmtRec_Final,cfg),fmtRec_Final);
		// fmtRec_Final := join(fmtRec_w_ABMS,input(derivedinputrecord=false),left.acctno=right.acctno,Functions_Validation.processVerifications(left,right),keep(Healthcare_Header_Services.Constants.MAX_RECS_ON_JOIN), limit(0));
	...
	Export convertInputtoDataset(Healthcare_Header_Services.IParams.searchParams inputCriteria,unsigned1 acctno = 1) := function

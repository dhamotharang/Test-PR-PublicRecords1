import Org_Mast,HMS_STLIC,hms_kop_trgt_harv,suppress,DOXIE; 
...
Export get_hms_Indiv (dataset(healthcare_header_services.Layouts.searchKeyResults_plus_input) input,dataset(healthcare_header_services.Layouts.common_runtime_config) cfg):= function
      ...
      suppressmachms:=Suppress.MAC_FlagSuppressedSource(audits+NoAudits,mod_access); 
			...
			hms_providers_rolled := rollup(hms_providers_final_grouped, group, Healthcare_Header_Services.Transforms.doHmsIndivRecordSrcIdRollup(left,rows(left)));
			// output(suppressmachms,named('suppressmachms'),overwrite);
			// output(setOptOuthms,named('setOptOuthms'),overwrite);
			//output(baseRecs,named('baseRecs'),overwrite);
			// output(hms_providers_final_sorted,named('hms_providers_final_sorted'),overwrite);
			// output(hms_providers_final_grouped,named('hms_providers_final_grouped'),overwrite);
			// output(hms_providers_rolled,named('hms_providers_rolled'),overwrite);
	
				return hms_providers_rolled;
end;
Export get_hms_Facility (dataset(Healthcare_Header_Services.Layouts.searchKeyResults_plus_input) input,dataset(Healthcare_Header_Services.Layouts.common_runtime_config) cfg):= function
			 noHits := input;
			 rawdataFacilitybyVendorid := join(noHits(vendorid<>''),  Org_Mast.Keys(,true).Facilities_LNFID.qa,
																					keyed(left.lnpid = right.lnfid),
																								transform(Healthcare_Header_Services.Layouts.hms_base_with_input,
																													self.l_providerid := left.lnpid;
      ...
      rawdataFacilitybyVendoridSorted:=dedup(sort(rawdataFacilitybyVendorid,record),record);
			baseRecs := project(rawdataFacilitybyVendoridSorted,Healthcare_Header_Services.Transforms.build_hms_facility_base(left));
			...
			hms_providers_rolled := rollup(hms_providers_final_grouped, group, Healthcare_Header_Services.Transforms.doHmsRecordSrcIdRollup(left,rows(left)));
     // output(hms_providers_rolled,named('get_hms_Facility'),overwrite);     	
			return hms_providers_rolled;
end;
Export get_hms_entity (dataset(Healthcare_Header_Services.Layouts.searchKeyResults_plus_input) input1,dataset(Healthcare_Header_Services.Layouts.common_runtime_config) cfg):= function

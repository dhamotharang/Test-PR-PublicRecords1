...
LogsRaw := DISTRIBUTE(PULL(LogFile (StringLib.StringToUpperCase(TRIM(Product)) IN ['RISKVIEW', 'RISKVIEWATTRIBUTES'] AND datetime[1..8] BETWEEN BeginDate AND EndDate AND StringLib.StringToLowerCase(TRIM(login_id)) NOT IN Risk_Reporting.Constants.IgnoredLogins AND customer_id NOT IN Risk_Reporting.Constants.IgnoredAccountIDs)));
...
Risk_Reporting.Layouts.Parsed_RiskView_Tracking_Layout parseInput () := TRANSFORM
	...
	SELF := [];
END;
parsedInput_1 := PARSE(Logs_1, inputxml, parseInput(), XML('RiskView'));
OUTPUT(CHOOSEN(parsedInput_1, eyeball), NAMED('Sample_parsedInput_1_Input'));
...
Logs_2 := PROJECT(LogsRaw, TRANSFORM({RECORDOF(LogsRaw), STRING30 TransactionID, STRING10 AccountID, STRING8 TransactionDate}, 
				// Need to mold the input XML into something that can be passed through the same PARSE function - this means making sure the first tag is <RiskView>
				inputXMLTemp1 := StringLib.StringFindReplace(LEFT.inputxml, '<RiskViewRequest>', '<RiskViewRequest><TransactionId>' + LEFT.Transaction_Id + '</TransactionId>');
				inputXMLTemp2 := StringLib.StringFindReplace(inputXMLTemp1, '<RiskViewAttributesRequest>', '<RiskViewRequest><TransactionId>' + LEFT.Transaction_Id + '</TransactionId>');
				SELF.inputxml := StringLib.StringFindReplace(inputXMLTemp2, '</RiskViewAttributesRequest>', '</RiskViewRequest>'); 
				SELF.outputxml := '<RiskView>' + LEFT.outputxml + '</RiskView>';
				SELF.TransactionID := LEFT.Transaction_ID;
				SELF.AccountID := LEFT.customer_id;
				SELF.TransactionDate := LEFT.DateTime[1..8];
				SELF := LEFT));
Risk_Reporting.Layouts.Parsed_RiskView_Tracking_Layout parseInput2 () := TRANSFORM
	SELF.TransactionID	:= TRIM(XMLTEXT('TransactionId')); // Forced into the record so I can join it all together
	SELF.CompanyName		:= TRIM(XMLTEXT('User/EndUser/CompanyName'));
	SELF.FirstName			:= TRIM(XMLTEXT('SearchBy/Name/First'));
	SELF.LastName				:= TRIM(XMLTEXT('SearchBy/Name/Last'));
	...
	SELF := [];
END;
parsedInput_2 := PARSE(Logs_2, inputxml, parseInput2(), XML('RiskViewRequest'));
...
Parsed_Layout_Temp := RECORD
	Risk_Reporting.Layouts.Parsed_RiskView_Tracking_Layout;
...
Parsed_Layout_Temp parseOutput () := TRANSFORM
	...
	SELF := [];
END;
parsedOutputTemp_1 := PARSE(Logs_11, outputxml, parseOutput(), XML('RiskView'));
parsedOutputTemp_2 := PARSE(Logs_22, outputxml, parseOutput(), XML('RiskView'));
//
...
Risk_Reporting.Layouts.Parsed_RiskView_Tracking_Layout normScores(Parsed_Layout_Temp le, UNSIGNED1 t) := TRANSFORM
	SELF.Score := CASE(t,
		1 => le.Score1,
		2 => le.Score2,
		3 => le.Score3,
...
Risk_Reporting.Layouts.Parsed_RiskView_Tracking_Layout combineParsedRecords(Risk_Reporting.Layouts.Parsed_RiskView_Tracking_Layout le, Risk_Reporting.Layouts.Parsed_RiskView_Tracking_Layout ri) := TRANSFORM
	SELF.Score	:= ri.Score;
	SELF.Model	:= ri.Model;
	SELF.RC1		:= ri.RC1;
	SELF.RC2		:= ri.RC2;
...
parsedRecordsTemp := JOIN(parsedRecordsTemp_in, parsedOutput, LEFT.TransactionID = RIGHT.TransactionID, combineParsedRecords(LEFT, RIGHT), KEEP(10), ATMOST(RiskWise.max_atmost)) (TRIM(StringLib.StringToUpperCase(model)[1..6]) NOT IN ['', 'INCOME'] AND TRIM(StringLib.StringToUpperCase(companyname)) NOT IN ['', 'RISKWISE INTERNAL ADMIN']);
...
trackingLayout := RECORD
	...
	DECIMAL6_3	Percent_Scored_101			:= (COUNT(GROUP, (INTEGER)finalRecords.Score = 101) / COUNT(GROUP)) * 100;
	DECIMAL6_3	Percent_Scored_102			:= (COUNT(GROUP, (INTEGER)finalRecords.Score = 102) / COUNT(GROUP)) * 100;
	DECIMAL6_3	Percent_Scored_103			:= (COUNT(GROUP, (INTEGER)finalRecords.Score = 103) / COUNT(GROUP)) * 100;
	DECIMAL6_3	Percent_Scored_104			:= (COUNT(GROUP, (INTEGER)finalRecords.Score = 104) / COUNT(GROUP)) * 100;
...
averageTrackingLayout := RECORD
	...
	DECIMAL6_3	Percent_Scored_101;			
	DECIMAL6_3	Percent_Scored_102;			
	DECIMAL6_3	Percent_Scored_103;			
	DECIMAL6_3	Percent_Scored_104;			
...
averageTrackingLayout getAverages(ut.ds_oneRecord le) := TRANSFORM
	...
	SELF.Percent_Scored_101 := percented(Percent_Scored_101);			
	SELF.Percent_Scored_102 := percented(Percent_Scored_102);			
	SELF.Percent_Scored_103 := percented(Percent_Scored_103);			
	SELF.Percent_Scored_104 := percented(Percent_Scored_104);			
...
averageTrackingLayout getDifferences(ut.ds_oneRecord le) := TRANSFORM
	...
	SELF.Percent_Scored_101 := difference(Percent_Scored_101);
	SELF.Percent_Scored_102 := difference(Percent_Scored_102);
	SELF.Percent_Scored_103 := difference(Percent_Scored_103);	
	SELF.Percent_Scored_104 := difference(Percent_Scored_104);	
...
potentialProblems(Percent_Scored_101, 'Percent_Scored_101', Risk_Reporting.Constants.ThreshPercent_Scored_101) +
potentialProblems(Percent_Scored_102, 'Percent_Scored_102', Risk_Reporting.Constants.ThreshPercent_Scored_102) +
potentialProblems(Percent_Scored_103, 'Percent_Scored_103', Risk_Reporting.Constants.ThreshPercent_Scored_103) +
potentialProblems(Percent_Scored_104, 'Percent_Scored_104', Risk_Reporting.Constants.ThreshPercent_Scored_104) +
...
'% Scored 101\t\t\t\t|' + genRow(Percent_Scored_101) +
'% Scored 102\t\t\t\t|' + genRow(Percent_Scored_102) +
'% Scored 103\t\t\t\t|' + genRow(Percent_Scored_103) +
'% Scored 104\t\t\t\t|' + genRow(Percent_Scored_104) +
...
FileServices.SendEmail(Risk_Reporting.Constants.emailv2RiskViewReportsTo, subject,
		emailBody,
		GETENV('SMTPserver'),

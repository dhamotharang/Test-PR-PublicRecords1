...
Risk_Reporting.Layouts.Parsed_RiskView_Tracking_Layout parseInput () := TRANSFORM
	...
	SELF := [];
END;
parsedInput := PARSE(Logs, inputxml, parseInput(), XML('RiskView'));
...
Parsed_Layout_Temp parseOutput () := TRANSFORM
	...
	SELF := [];
END;
parsedOutputTemp := PARSE(Logs, outputxml, parseOutput(), XML('RiskView'));
...
parsedRecordsTemp := JOIN(parsedInput, parsedOutput, LEFT.TransactionID = RIGHT.TransactionID, combineParsedRecords(LEFT, RIGHT), KEEP(10), ATMOST(RiskWise.max_atmost)) (TRIM(StringLib.StringToUpperCase(model)[1..6]) NOT IN ['', 'INCOME'] AND TRIM(StringLib.StringToUpperCase(companyname)) NOT IN ['', 'RISKWISE INTERNAL ADMIN']);

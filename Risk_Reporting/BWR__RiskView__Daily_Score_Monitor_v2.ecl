...
parsedRecordsTemp := JOIN(parsedRecordsTemp_in, parsedOutput, LEFT.TransactionID = RIGHT.TransactionID, combineParsedRecords(LEFT, RIGHT), KEEP(10), ATMOST(RiskWise.max_atmost)) (TRIM(StringLib.StringToUpperCase(model)[1..6]) NOT IN ['', 'INCOME'] AND TRIM(StringLib.StringToUpperCase(EndUserCompanyName)) NOT IN ['', 'RISKWISE INTERNAL ADMIN']);

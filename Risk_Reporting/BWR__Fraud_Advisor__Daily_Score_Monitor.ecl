...
// currentDate := ut.date_math('20120928', 2);

#workunit('name', 'Fraud_Point_Daily_Score_Monitor__' + currentDate);
...
baseFile := '~thor400::reports::fraudpoint::score_monitoring_';
// baseFile := '~bpahl::fraudpoint::score_monitoring_';
...
yesterdaysLogsRaw := LogFile (StringLib.StringToUpperCase(TRIM(Product)) = 'FRAUDPOINT' AND datetime[1..8] = ut.date_math(currentDate, -1 - dataLoadDelayDays));
...
Parsed_Layout parseInput () := TRANSFORM
	...
	SELF := [];
END;
parsedInput := PARSE(yesterdaysLogs, inputxml, parseInput(), XML('FraudPoint'));
...
Parsed_Layout_Temp parseOutput () := TRANSFORM
	...
	SELF := [];
END;
parsedOutputTemp := PARSE(yesterdaysLogs, outputxml, parseOutput(), XML('FraudPoint'));
...
// Join the parsed input/output and then filter out the results where no model was requested or where this was a model that wasn't 0-999 scores (Such as 00-50 score models)
parsedRecords := JOIN(parsedInput, parsedOutput, LEFT.TransactionID = RIGHT.TransactionID, combineParsedRecords(LEFT, RIGHT), KEEP(10), ATMOST(RiskWise.max_atmost)) (TRIM(StringLib.StringToUpperCase(model)) NOT IN ['', '00 TO 50', '10 TO 50']);
...
Monitor_Layout := RECORD
	...
	DECIMAL6_3	Percent_Email_Populated									:= 0.0;
	DECIMAL6_3	Percent_IP_Address_Populated						:= 0.0;
	
	// Score Buckets
	DECIMAL6_3	Percent_Scored_200											:= 0.0;
	DECIMAL6_3	Percent_Scored_201											:= 0.0;
	DECIMAL6_3	Percent_Scored_202											:= 0.0;
	DECIMAL6_3	Percent_Scored_203											:= 0.0;
	DECIMAL6_3	Percent_Scored_222											:= 0.0;
	DECIMAL6_3	Percent_Scored_Between_300_350					:= 0.0;
	DECIMAL6_3	Percent_Scored_Between_351_400					:= 0.0;
	DECIMAL6_3	Percent_Scored_Between_401_450					:= 0.0;
	DECIMAL6_3	Percent_Scored_Between_451_500					:= 0.0;
	DECIMAL6_3	Percent_Scored_Between_501_550					:= 0.0;
	DECIMAL6_3	Percent_Scored_Between_551_600					:= 0.0;
	DECIMAL6_3	Percent_Scored_Between_601_650					:= 0.0;
	DECIMAL6_3	Percent_Scored_Between_651_700					:= 0.0;
	DECIMAL6_3	Percent_Scored_Between_701_750					:= 0.0;
	DECIMAL6_3	Percent_Scored_Between_751_900					:= 0.0;
...
emailPopulated := COUNT(parsedRecords (Email != ''));
ipAddressPopulated := COUNT(parsedRecords (IPAddress != ''));

scored200 := COUNT(parsedRecords ((UNSIGNED2)Score = 200));
scored201 := COUNT(parsedRecords ((UNSIGNED2)Score = 201));
scored202 := COUNT(parsedRecords ((UNSIGNED2)Score = 202));
scored203 := COUNT(parsedRecords ((UNSIGNED2)Score = 203));
scored222 := COUNT(parsedRecords ((UNSIGNED2)Score = 222));
scored300_350 := COUNT(parsedRecords ((UNSIGNED2)Score BETWEEN 300 AND 350));
scored351_400 := COUNT(parsedRecords ((UNSIGNED2)Score BETWEEN 351 AND 400));
scored401_450 := COUNT(parsedRecords ((UNSIGNED2)Score BETWEEN 401 AND 450));
scored451_500 := COUNT(parsedRecords ((UNSIGNED2)Score BETWEEN 451 AND 500));
scored501_550 := COUNT(parsedRecords ((UNSIGNED2)Score BETWEEN 501 AND 550));
scored551_600 := COUNT(parsedRecords ((UNSIGNED2)Score BETWEEN 551 AND 600));
scored601_650 := COUNT(parsedRecords ((UNSIGNED2)Score BETWEEN 601 AND 650));
scored651_700 := COUNT(parsedRecords ((UNSIGNED2)Score BETWEEN 651 AND 700));
scored701_750 := COUNT(parsedRecords ((UNSIGNED2)Score BETWEEN 701 AND 750));
scored751_900 := COUNT(parsedRecords ((UNSIGNED2)Score BETWEEN 751 AND 900));
scoredAbove900 := COUNT(parsedRecords ((UNSIGNED2)Score BETWEEN 901 AND 1000));
averageScoreRecs := parsedRecords ((UNSIGNED2)Score BETWEEN 300 AND 900);
...
Monitor_Layout getReport(ut.ds_oneRecord le) := TRANSFORM
	...
	SELF.Percent_Email_Populated									:= (emailPopulated / totalModelScores) * 100;
	SELF.Percent_IP_Address_Populated							:= (IPAddressPopulated / totalModelScores) * 100;
	SELF.Percent_Scored_200												:= (scored200 / totalModelScores) * 100;
	SELF.Percent_Scored_201												:= (scored201 / totalModelScores) * 100;
	SELF.Percent_Scored_202												:= (scored202 / totalModelScores) * 100;
	SELF.Percent_Scored_203												:= (scored203 / totalModelScores) * 100;
	SELF.Percent_Scored_222												:= (scored222 / totalModelScores) * 100;
	SELF.Percent_Scored_Between_300_350						:= (scored300_350 / totalModelScores) * 100;
	SELF.Percent_Scored_Between_351_400						:= (scored351_400 / totalModelScores) * 100;
	SELF.Percent_Scored_Between_401_450						:= (scored401_450 / totalModelScores) * 100;
	SELF.Percent_Scored_Between_451_500						:= (scored451_500 / totalModelScores) * 100;
	SELF.Percent_Scored_Between_501_550						:= (scored501_550 / totalModelScores) * 100;
	SELF.Percent_Scored_Between_551_600						:= (scored551_600 / totalModelScores) * 100;
	SELF.Percent_Scored_Between_601_650						:= (scored601_650 / totalModelScores) * 100;
	SELF.Percent_Scored_Between_651_700						:= (scored651_700 / totalModelScores) * 100;
	SELF.Percent_Scored_Between_701_750						:= (scored701_750 / totalModelScores) * 100;
	SELF.Percent_Scored_Between_751_900						:= (scored751_900 / totalModelScores) * 100;
...
OUTPUT(comboFile,, currentDateFile, CSV(HEADING(single), QUOTE('"')), OVERWRITE, EXPIRE(91))) : SUCCESS(SEQUENTIAL(STD.File.SetFileDescription(currentDateFile, 'Score and Attribute Outcome Tracking - Fraud Point Daily Score Monitor CSV File - ' + currentDate), STD.File.ProtectLogicalFile(currentDateFile, TRUE)));
...
'Total_Number_Of_0-999_Model_Scores,' + 
...
'Percent_Email_Populated,' + 
'Percent_IP_Address_Populated,' + 
'Percent_Scored_200,' + 
'Percent_Scored_201,' + 
'Percent_Scored_202,' + 
'Percent_Scored_203,' + 
'Percent_Scored_222,' + 
'Percent_Scored_Between_300_350,' + 
'Percent_Scored_Between_351_400,' + 
'Percent_Scored_Between_401_450,' + 
'Percent_Scored_Between_451_500,' + 
'Percent_Scored_Between_501_550,' + 
'Percent_Scored_Between_551_600,' + 
'Percent_Scored_Between_601_650,' + 
'Percent_Scored_Between_651_700,' + 
'Percent_Scored_Between_701_750,' + 
'Percent_Scored_Between_751_900,' + 
...
thisRowS := (STRING)le.Date_Of_Report + ',' + 
						...
						(STRING)le.Percent_Email_Populated + ',' +
						(STRING)le.Percent_IP_Address_Populated + ',' +
						(STRING)le.Percent_Scored_200 + ',' +
						(STRING)le.Percent_Scored_201 + ',' +
						(STRING)le.Percent_Scored_202 + ',' +
						(STRING)le.Percent_Scored_203 + ',' +
						(STRING)le.Percent_Scored_222 + ',' +
						(STRING)le.Percent_Scored_Between_300_350 + ',' +
						(STRING)le.Percent_Scored_Between_351_400 + ',' +
						(STRING)le.Percent_Scored_Between_401_450 + ',' +
						(STRING)le.Percent_Scored_Between_451_500 + ',' +
						(STRING)le.Percent_Scored_Between_501_550 + ',' +
						(STRING)le.Percent_Scored_Between_551_600 + ',' +
						(STRING)le.Percent_Scored_Between_601_650 + ',' +
						(STRING)le.Percent_Scored_Between_651_700 + ',' +
						(STRING)le.Percent_Scored_Between_701_750 + ',' +
						(STRING)le.Percent_Scored_Between_751_900 + ',' +
...
'Total Number of 0-999 Model Scores\t|' + genRowRound(Total_Number_Of_Model_Scores) +
...
'% Email Populated\t\t\t|' + genRowPercent(Percent_Email_Populated) +
'% IP Address Populated\t\t|' + genRowPercent(Percent_IP_Address_Populated) +
'% Scored 200\t\t\t\t|' + genRowPercent(Percent_Scored_200) +
'% Scored 201\t\t\t\t|' + genRowPercent(Percent_Scored_201) +
'% Scored 202\t\t\t\t|' + genRowPercent(Percent_Scored_202) +
'% Scored 203\t\t\t\t|' + genRowPercent(Percent_Scored_203) +
'% Scored 222\t\t\t\t|' + genRowPercent(Percent_Scored_222) +
'% Scored Between 300 - 350\t\t|' + genRowPercent(Percent_Scored_Between_300_350) +
'% Scored Between 351 - 400\t\t|' + genRowPercent(Percent_Scored_Between_351_400) +
'% Scored Between 401 - 450\t\t|' + genRowPercent(Percent_Scored_Between_401_450) +
'% Scored Between 451 - 500\t\t|' + genRowPercent(Percent_Scored_Between_451_500) +
'% Scored Between 501 - 550\t\t|' + genRowPercent(Percent_Scored_Between_501_550) +
'% Scored Between 551 - 600\t\t|' + genRowPercent(Percent_Scored_Between_551_600) +
'% Scored Between 601 - 650\t\t|' + genRowPercent(Percent_Scored_Between_601_650) +
'% Scored Between 651 - 700\t\t|' + genRowPercent(Percent_Scored_Between_651_700) +
'% Scored Between 701 - 750\t\t|' + genRowPercent(Percent_Scored_Between_701_750) +
'% Scored Between 751 - 900\t\t|' + genRowPercent(Percent_Scored_Between_751_900) +
...
#IF(%firstOfMonth%)
FileServices.SendEmailAttachData(Risk_Reporting.Constants.emailFraudPointReportsTo, subject,
																		emailBody,
																		(DATA)attachment,
																		'text/csv',
																		'Fraud Point - ' + TRIM(Risk_Reporting.Constants.month((UNSIGNED1)((STRING)ut.date_math(currentDate, -1)[5..6]))) + ', ' + ((STRING)ut.date_math(currentDate, -1)[1..4]) + ' Report.csv',
																		GETENV('SMTPserver'),
																		(UNSIGNED4)GETENV('SMTPport', '25'),
																		'ThorReport@lexisnexis.com');
#ELSE
FileServices.SendEmail(Risk_Reporting.Constants.emailFraudPointReportsTo, subject,
																		emailBody,
																		GETENV('SMTPserver'),
																		(UNSIGNED4)GETENV('SMTPport', '25'),
																		'ThorReport@lexisnexis.com');		

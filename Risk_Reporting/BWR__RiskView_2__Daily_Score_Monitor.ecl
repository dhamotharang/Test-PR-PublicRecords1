...
Risk_Reporting.Layouts.Parsed_RiskView2_Tracking_Layout parseInput () := TRANSFORM
	...
	SELF := [];
END;
parsedInput := PARSE(Logs, inputxml, parseInput(), XML('RiskView2'));
...
parsedRecordsTemp := JOIN(parsedInput, parsedOutput, LEFT.TransactionID = RIGHT.TransactionID, combineParsedRecords(LEFT, RIGHT), KEEP(10), ATMOST(RiskWise.max_atmost)) (TRIM(StringLib.StringToUpperCase(companyname)) NOT IN ['', 'RISKWISE INTERNAL ADMIN']);

...
Parsed_Layout parseInput () := TRANSFORM
	...
	SELF := [];
END;
parsedInput_1 := PARSE(Logs_1, inputxml, parseInput(), XML('PremiseAssociation'));
OUTPUT(CHOOSEN(parsedInput_1, eyeball), NAMED('Sample_Parsed_parsedInput_1'));
...
Parsed_Layout parseInput2 () := TRANSFORM
	...
	SELF := [];
END;
parsedInput_2 := PARSE(Logs_2, inputxml, parseInput2(), XML('PremiseAssociationRequest'));
...
Parsed_Layout_Temp parseOutput () := TRANSFORM
	...
	SELF := [];
END;
parsedOutput_1 := PARSE(Logs_11, outputxml, parseOutput(), XML('PremiseAssociation'));
parsedOutput_2 := PARSE(Logs_22, outputxml, parseOutput(), XML('PremiseAssociation'));

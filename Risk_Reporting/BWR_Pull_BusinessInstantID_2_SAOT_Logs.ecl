#workunit('name', 'Business_Instant_ID_2_Pull_SAOT_Logs');
...
// AccountIDs := ['1005199','1006061','1007104','1405920','1497244','1559562','1562626','1566015','1578295','1580841','1633577','1641250','1641680','1642520','1648837','1654337','6713493']; // Set to a blank string dataset [''] to pull all records except for test transaction login ids
AccountIDs := ['1614441']; // Set to a blank string dataset [''] to pull all records except for test transaction login ids

eyeball := 100;

outputFile := '~evanheel::out::Business_InstantID2_SAOT_' + BeginDate + '-' + EndDate + '_' + AccountIDs[1];
...
LogsRaw := IF(AccountIDs[1] != '', DISTRIBUTE(PULL(LogFile (StringLib.StringToUpperCase(TRIM(Product)) IN ['BUSINESSINSTANTID2'] AND datetime[1..8] BETWEEN BeginDate AND EndDate AND customer_id IN AccountIDs AND customer_id NOT IN Risk_Reporting.Constants.IgnoredAccountIDs))),
																	 DISTRIBUTE(PULL(LogFile (StringLib.StringToUpperCase(TRIM(Product)) IN ['BUSINESSINSTANTID2'] AND datetime[1..8] BETWEEN BeginDate AND EndDate AND StringLib.StringToLowerCase(TRIM(login_id)) NOT IN Risk_Reporting.Constants.IgnoredLogins AND customer_id NOT IN Risk_Reporting.Constants.IgnoredAccountIDs))));

linerec := {STRING line};

OUTPUT(CHOOSEN(LogsRaw, eyeball), NAMED('LogsRaw'));
...
OUTPUT(COUNT(Logs), NAMED('cnt_Logs'));
...
OUTPUT(COUNT(parsedInput), NAMED('parsedInput'));
...
OUTPUT(COUNT(parsedOutput), NAMED('parsedOutput'));
...
OUTPUT(COUNT(parsedRecordsTemp), NAMED('parsedRecordsTemp'));
...
OUTPUT(CHOOSEN(parsedRecords, eyeball), NAMED('parsedRecords'));
...
OUTPUT(CHOOSEN(finalRecords, eyeball), NAMED('finalRecords'));

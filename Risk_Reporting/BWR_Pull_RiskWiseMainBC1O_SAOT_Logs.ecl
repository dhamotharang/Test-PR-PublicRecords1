...
LogsRaw := IF(AccountIDs[1] != '', DISTRIBUTE(PULL(LogFile (StringLib.StringToUpperCase(TRIM(Product)) IN ['RISKWISE.RISKWISEMAINBC1O'] AND datetime[1..8] BETWEEN BeginDate AND EndDate AND customer_id IN AccountIDs AND customer_id NOT IN Risk_Reporting.Constants.IgnoredAccountIDs))),
...
Risk_Reporting.Layouts.Parsed_RiskWiseMainBC1O_Layout parseInput () := TRANSFORM
	...
	SELF := [];
END;
parsedInput := DISTRIBUTE(PARSE(Logs, inputxml, parseInput(), XML('RiskWise.RiskWiseMainBC1O')), HASH64(TransactionID));
...
Risk_Reporting.Layouts.Parsed_RiskWiseMainBC1O_Layout parseOutput () := TRANSFORM
	...
	SELF := [];
END;
parsedOutput := PARSE(Logs, outputxml, parseOutput(), XML('RiskWise.RiskWiseMainBC1O'));

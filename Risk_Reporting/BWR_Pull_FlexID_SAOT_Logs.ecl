...
LogsRaw := IF(AccountIDs[1] != '', DISTRIBUTE(PULL(LogFile (StringLib.StringToUpperCase(TRIM(Product)) IN ['FLEXID'] AND datetime[1..8] BETWEEN BeginDate AND EndDate AND customer_id IN AccountIDs AND customer_id NOT IN Risk_Reporting.Constants.IgnoredAccountIDs /*AND Transaction_ID IN transactionIDs*/))),
...
Risk_Reporting.Layouts.Parsed_FlexID_Layout parseInput () := TRANSFORM
	...
	SELF := [];
END;
parsedInput := DISTRIBUTE(PARSE(Logs, inputxml, parseInput(), XML('FlexID')), HASH64(TransactionID));
...
Risk_Reporting.Layouts.Parsed_FlexID_Layout parseOutput () := TRANSFORM
  ...
  SELF.FraudPointRC1	:=TRIM(XMLTEXT('Result/Models[1]/Model[1]/Scores[1]/Score[1]/HighRiskIndicators/HighRiskIndicator[1]/RiskCode'));
	...
	SELF := [];
END;
parsedOutputTemp := PARSE(Logs, outputxml, parseOutput(), XML('FlexID'));

...
LogsRaw_1 := DISTRIBUTE(PULL(LogFile (StringLib.StringToUpperCase(TRIM(Product)) IN ['PREMISEASSOCIATION'] AND datetime[1..8] BETWEEN BeginDate AND EndDate AND customer_id IN AccountIDs)));
...
Parsed_Layout parseInput () := TRANSFORM
	...
	SELF := [];
END;
parsedInput_1 := DISTRIBUTE(PARSE(Logs_1, inputxml, parseInput(), XML('PremiseAssociation')), HASH64(TransactionID));
...
Logs_2 := PROJECT(LogsRaw_1, TRANSFORM({RECORDOF(LogsRaw_1), STRING30 TransactionID, STRING10 AccountID, STRING8 TransactionDate}, 
				SELF.inputxml := StringLib.StringFindReplace(LEFT.inputxml, '<PremiseAssociationRequest>', '<PremiseAssociationRequest><TransactionId>' + LEFT.Transaction_Id + '</TransactionId>'); 
				SELF.outputxml := '<PremiseAssociation>' + LEFT.outputxml + '</PremiseAssociation>';
				SELF.TransactionID := LEFT.Transaction_ID;
				SELF.AccountID := LEFT.customer_id;
				SELF.TransactionDate := LEFT.DateTime[1..8];
...
Parsed_Layout parseInput_2 () := TRANSFORM
	SELF.TransactionID	:= TRIM(XMLTEXT('TransactionId')); // Forced into the record so I can join it all together
	SELF.CompanyName		:= TRIM(XMLTEXT('User/EndUser/CompanyName'));
	SELF.FirstName			:= TRIM(XMLTEXT('SearchBy/Name/First'));
	SELF.LastName				:= TRIM(XMLTEXT('SearchBy/Name/Last'));
	...
	SELF := [];
END;
parsedInput_2 := DISTRIBUTE(PARSE(Logs_2, inputxml, parseInput_2(), XML('PremiseAssociationRequest')), HASH64(TransactionID));
...
parsedOutputTemp_1 := PARSE(Logs_11, outputxml, parseOutput(), XML('PremiseAssociation'));
parsedOutputTemp_2 := PARSE(Logs_22, outputxml, parseOutput(), XML('PremiseAssociationRequest'));
...
// table(finalRecords(AccountID = '118436' and TransactionDate between BeginDate and enddate), {TransactionDate,  cnt := count(group)}, TransactionDate,  few);

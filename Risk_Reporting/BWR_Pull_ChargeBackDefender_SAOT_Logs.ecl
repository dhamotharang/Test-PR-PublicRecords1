...
LogsRaw := IF(AccountIDs[1] != '', DISTRIBUTE(PULL(LogFile (StringLib.StringToUpperCase(TRIM(Product)) IN ['CHARGEBACKDEFENDER'] AND datetime[1..8] BETWEEN BeginDate AND EndDate 
...
Logs := choosen(PROJECT(LogsRaw, TRANSFORM({RECORDOF(LogsRaw), STRING30 TransactionID, STRING10 AccountID, STRING8 TransactionDate}, 
...
//BillTo
	...
	SELF := [];
END;
parsedInput := DISTRIBUTE(PARSE(Logs, inputxml, parseInput(), XML('ChargebackDefender')), HASH64(TransactionID));
...
//BillTo
	...
	SELF := ri;
END;
parsedOutput := PARSE(Logs, outputxml, parseOutput(), XML('ChargebackDefender'));

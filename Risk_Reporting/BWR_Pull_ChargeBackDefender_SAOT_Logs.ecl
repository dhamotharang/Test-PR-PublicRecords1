#workunit('name', 'BWR_Pull_ChargeBackDefender_SAOT_Logs');
...
outputFile := '~akoenen::out::ChargebackDefender_SAOT_' + BeginDate + '-' + EndDate + '_' + AccountIDs[1];
...
LogsRaw_1 := IF(AccountIDs[1] != '', DISTRIBUTE(PULL(LogFile (StringLib.StringToUpperCase(TRIM(Product)) IN ['CHARGEBACKDEFENDER'] AND datetime[1..8] BETWEEN BeginDate AND EndDate 
													AND customer_id IN AccountIDs AND customer_id NOT IN Risk_Reporting.Constants.IgnoredAccountIDs
							))),
																	 DISTRIBUTE(PULL(LogFile (StringLib.StringToUpperCase(TRIM(Product)) IN ['CHARGEBACKDEFENDER'] AND datetime[1..8] BETWEEN BeginDate AND EndDate 
...
Logs_1 := choosen(PROJECT(LogsRaw_1, TRANSFORM({RECORDOF(LogsRaw_1), STRING30 TransactionID, STRING10 AccountID, STRING8 TransactionDate}, 
...
//options
	...
	SELF.RequestedAttributeGroups := TRIM(XMLTEXT('Options/RequestedAttributeGroups[1]/Name[1]')) + ' ' + 
															TRIM(XMLTEXT('Options/RequestedAttributeGroups[1]/Name[2]')) + ' ' +  
															TRIM(XMLTEXT('Options/RequestedAttributeGroups[1]/Name[3]'));
	SELF.Model := TRIM(XMLTEXT('Options/IncludeModels/ChargebackDefender'));
//BillTo
	...
	SELF := [];
END;
parsedInput_1 := DISTRIBUTE(PARSE(Logs_1, inputxml, parseInput(), XML('ChargebackDefender')), HASH64(TransactionID));
...
//
Logs_2 := choosen(PROJECT(LogsRaw_1, TRANSFORM({RECORDOF(LogsRaw_1), STRING30 TransactionID, STRING10 AccountID, STRING8 TransactionDate}, 
...
Risk_Reporting.Layouts.Parsed_ChargeBackDefender_Layout parseInput_2 () := TRANSFORM
	SELF.TransactionID	:= TRIM(XMLTEXT('TransactionId')); // Forced into the record so I can join it all together
	SELF.CompanyName		:= TRIM(XMLTEXT('User/EndUser/CompanyName'));
//options
	SELF.OrderType 			:= TRIM(XMLTEXT('Options/OrderType'));
...
//BillTo
	...
	SELF := [];
END;
parsedInput_2 := DISTRIBUTE(PARSE(Logs_2, inputxml, parseInput(), XML('ChargebackDefenderRequest')), HASH64(TransactionID));
...
Risk_Reporting.Layouts.Parsed_ChargeBackDefender_Layout parseOutput () := TRANSFORM
...
// bill to verified
	...
	self.veraddr:=	TRIM(XMLTEXT('Result/BillTo/VerifiedInput/Address/StreetAddress1')) ;
	...
	self.verZip4:=TRIM(XMLTEXT('Result/BillTo/VerifiedInput/Address/Zip4')) ;	

// bill to corrected
	self.correctlast:=TRIM(XMLTEXT('Result/BillTo/InputCorrected/Name/Last')) ;
	self.correctaddr :=	TRIM(XMLTEXT('Result/BillTo/InputCorrected/Address/StreetAddress1')) ;
	self.correcthphone:=TRIM(XMLTEXT('Result/BillTo/InputCorrected/Phone10')) ;	
	self.correctsocs:=TRIM(XMLTEXT('Result/BillTo/InputCorrected/SSN')) ; 
//extras	

	self.nameaddrphone := TRIM(XMLTEXT('Result/BillTo/PhoneOfNameAddress')) ;	
	self.socsverlevel := TRIM(XMLTEXT('Result/BillTo/NameAddressSSNSummary')) ;	
	self.phoneverlevel :=TRIM(XMLTEXT('Result/BillTo/NameAddressPhoneSummary')) ;	
	self.sic := TRIM(XMLTEXT('Result/BillTo/SIC')) ;	
	self.hphonetypeflag := TRIM(XMLTEXT('Result/BillTo/PhoneType')) ;	
	self.dwelltypeflag := TRIM(XMLTEXT('Result/BillTo/DwellingType')) ;	
	SELF.altareacode := (string) TRIM(XMLTEXT('Result/BillTo/NewAreaCode/AreaCode')) ;	

	SELF.areacodesplitdate	:= TRIM(XMLTEXT('Result/BillTo/NewAreaCode/EffectiveDate')) + 
...
//ship to
	...
	self.veraddr2 := 	TRIM(XMLTEXT('Result/ShipTo/VerifiedInput/Address/StreetAddress1')) ;
	...
	self.verzip42:=TRIM(XMLTEXT('Result/ShipTo/VerifiedInput/Address/Zip4')) ;	
	
	// ship to corrected
	self.correctlast2 :=TRIM(XMLTEXT('Result/ShipTo/InputCorrected/Name/Last')) ;
	self.correctaddr2 :=	TRIM(XMLTEXT('Result/ShipTo/InputCorrected/Address/StreetAddress1')) ;
	self.correcthphone2 :=TRIM(XMLTEXT('Result/ShipTo/InputCorrected/Phone10')) ;	
//extras
	self.nameaddrphone2 := TRIM(XMLTEXT('Result/ShipTo/PhoneOfNameAddress')) ;	
	self.sic2 := TRIM(XMLTEXT('Result/ShipTo/SIC')) ;	
	self.hphonetypeflag2 := TRIM(XMLTEXT('Result/ShipTo/PhoneType')) ;	
	self.dwelltypeflag2 := TRIM(XMLTEXT('Result/ShipTo/DwellingType')) ;	
	SELF.altareacode2 := (string) TRIM(XMLTEXT('Result/ShipTo/NewAreaCode/AreaCode')) ;	
	SELF.areacodesplitdate2	:= TRIM(XMLTEXT('Result/ShipTo/NewAreaCode/EffectiveDate')) + 
  ...
  END;
 Risk_Reporting.Layouts.Parsed_ChargeBackDefender_Layout combineParsedRecords(Risk_Reporting.Layouts.Parsed_ChargeBackDefender_Layout le, Risk_Reporting.Layouts.Parsed_ChargeBackDefender_Layout ri) := TRANSFORM
...
//options
	...
	SELF.RequestedAttributeGroups := le.RequestedAttributeGroups;
	SELF.Model := le.Model;
//BillTo
...
OUTPUT(finalRecords,, outputFile + '_' + ThorLib.Wuid() + '.csv', CSV(HEADING(single), QUOTE('"')), EXPIRE(30), OVERWRITE);
 // table(finalRecords(AccountID = '1595120' and TransactionDate between BeginDate and enddate), {TransactionDate,  cnt := count(group)}, TransactionDate,  few);

...
EXPORT Spray_CaliforniaStateDeathData_1999(
	STRING		pVersionDate	= '',
	...
	STRING		pFilename			= 'california_deathm_raw_dth1999*csv',
	...
	[
		{
			...
			0, 																			// record_size
			Filenames(state+'1999').input.Template, 				// Thor_filename_template
			[ {Filenames(state+'1999').input.sprayed	}	], 	// dSuperfilenames
	...
	RETURN IF(	pDirectory != '',
		IF (	pPromote,
			...
			(
				...
				FileServices.AddSuperFile(Filenames(state+'1999').input.sprayed+'_history',Filenames(state+'1999').input.sprayed,,TRUE),
				FileServices.ClearSuperFile(Death_Master.Filenames(state+'1999').input.sprayed),

IMPORT SALT32,std;
EXPORT match_methods(DATASET(layout_In_LA) ih) := MODULE
...
EXPORT match_corp_number_of_amendments(TYPEOF(h.corp_number_of_amendments) L, TYPEOF(h.corp_number_of_amendments) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT32.MatchCode.ExactMatch,
    SALT32.MatchCode.NoMatch),
  MAP(L = R => SALT32.MatchCode.ExactMatch, SALT32.MatchCode.NoMatch)
);

...
EXPORT Files := MODULE
	...
	// EXPORT DEA_DID_DS 				:= DATASET ('~' + ut.foreign_prod_boca + 'thor_data400::base::dea',healthcarefacility.Layouts.DEA_DID,thor);	
	// EXPORT NPPES_DID_DS				:= DATASET ('~' + ut.foreign_prod_boca + 'thor_data400::base::nppes',healthcarefacility.Layouts.NPPES_DID,thor);
	// EXPORT PROF_LIC_DS				:= DATASET ('~' + ut.foreign_prod_boca + 'thor_data400::base::prolicv2::proflic_base::built',healthcarefacility.Layouts.PROF_LIC,thor);	
	// EXPORT Enclarity_Fac_DS		:= DATASET ('~' + ut.foreign_prod_boca + 'thor_data400::base::enclarity::facility::qa',Layouts.Enclarity_REC,thor);	
	// EXPORT Enclarity_ASSOC_DS	:= DATASET ('~' + ut.foreign_prod_boca + 'thor_data400::base::enclarity::associate::qa',Layouts.Enclarity_ASSOC_REC,thor);		
	// EXPORT CLIA_DS				 		:= DATASET ('~' + ut.foreign_prod_boca + 'thor_data400::base::clia::built::data',Layouts.CLIA_REC,thor);	
	// EXPORT NCPDP_DS				 		:= DATASET ('~' + ut.foreign_prod_boca + 'thor_data400::base::ncpdp::combined_file::qa::data',Layouts.NCPDP_REC,thor);	
	...
	EXPORT Facility_SF									:=	HealthCare_Prefix	+ '::qa::' + Header_Suffix;
	EXPORT Facility_Father_SF						:=	HealthCare_Prefix	+ '::father::' + Header_Suffix;
	EXPORT Facility_GrandFather_SF			:=	HealthCare_Prefix	+ '::grandfather::' + Header_Suffix;
	EXPORT Facility_Delete_SF						:=	HealthCare_Prefix	+ '::delete::' + Header_Suffix;

	EXPORT facility_Salt_Input_DS								:= DATASET (facility_in_SF,HealthCareFacility.Layout_HealthProvider.HealthCareProvider_Header,THOR,OPT);		
	EXPORT facility_Salt_Output_DS							:= DATASET (facility_Out_SF,HealthCareFacility.Layout_HealthProvider.HealthCareProvider_Header,THOR,OPT);		
	EXPORT facility_Salt_Output_Father_DS				:= DATASET (facility_Out_Father_SF,HealthCareFacility.Layout_HealthProvider.HealthCareProvider_Header,THOR,OPT);		
	EXPORT facility_Salt_Output_GrandFather_DS	:= DATASET (facility_Out_GrandFather_SF,HealthCareFacility.Layout_HealthProvider.HealthCareProvider_Header,THOR,OPT);		
	EXPORT Facility_Salt_Input_Header_DS				:= DATASET (Facility_SF,HealthCareFacility.Layout_HealthProvider.HealthCareProvider_Header,THOR,OPT);		
	
	EXPORT Possible_Match_DS										:= DATASET (PossibleMatch_SF,HealthCareProvider.Layouts.Possible_Match_REC,THOR,OPT);		
	EXPORT facility_Deleted_DS									:= DATASET (Facility_Deleted_SF,HealthCareFacility.Layout_HealthProvider.HealthCareProvider_Header,THOR,OPT);			
	
	EXPORT Facility_Header_DS										:= DATASET (Facility_SF,HealthCareFacility.Layout_HealthProvider.HealthCareProvider_Header,THOR,OPT);	
END;

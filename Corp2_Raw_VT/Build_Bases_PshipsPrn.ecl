...
EXPORT Build_Bases_PshipsPrn(
	...
	workingUpdate := PROJECT(pInPshipsPrn, standardize_input(LEFT));

	// Combine Update with Previous Base.
	combined 			:= workingUpdate + pBasePshipsPrn;
	...
	tools.mac_WriteFile(Corp2_Raw_VT.Filenames(pversion).Base.PshipsPrn.New, BaseRollup, Build_Base);

	EXPORT All := IF(tools.fun_IsValidVersion(pversion),
									 Build_Base,
									 OUTPUT('No Valid version parameter passed, skipping Corp2_Raw_VT.Build_Bases_PshipsPrn attribute')
									 );

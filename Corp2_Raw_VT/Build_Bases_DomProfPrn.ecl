...
EXPORT Build_Bases_DomProfPrn(
	STRING																				pfiledate,
	STRING																				pversion,
	BOOLEAN																				pUseOtherEnvironment,
	DATASET(Corp2_Raw_VT.Layouts.PrnLayoutIn)	 		DomProfPrnLayoutIn   = Corp2_Raw_VT.Files(pfiledate,pUseOtherEnvironment).Input.DomProfPrn.logical,
	...
	workingUpdate := PROJECT(DomProfPrnLayoutIn, standardize_input(LEFT));

	// Combine Update with Previous Base.
	combined 			:= workingUpdate + pBaseDomProfPrn;
	...
	tools.mac_WriteFile(Corp2_Raw_VT.Filenames(pversion).Base.DomProfPrn.New, BaseRollup, Build_Base);

	EXPORT All := IF(tools.fun_IsValidVersion(pversion),
											Build_Base,
											OUTPUT('No Valid version parameter passed, skipping Corp2_Raw_VT.Build_Bases_DomProfPrn attribute')
									 );

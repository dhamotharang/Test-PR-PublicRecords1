...
EXPORT Build_Bases_TrdNamesBus(
	STRING																				pfiledate,
	STRING																				pversion,
	...
	workingUpdate := PROJECT(pInTrdNamesBus, standardize_input(LEFT));

	// Combine Update with Previous Base.
	combined 			:= workingUpdate + pBaseTrdNamesBus;
	...
	tools.mac_WriteFile(Corp2_Raw_VT.Filenames(pversion).Base.TrdNamesBus.New, BaseRollup, Build_Base);

	EXPORT All := IF( tools.fun_IsValidVersion(pversion),
										Build_Base,
										OUTPUT('No Valid version parameter passed, skipping Corp2_Raw_VT.Build_Bases_TrdNamesBus attribute')
									 );

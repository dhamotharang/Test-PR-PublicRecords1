...
Layout_Moxie_Offender_temp tSexPredSearchandSexPred2toOffender(Layout_Sex_Pred_Search_DID pSexPredSearch,Layout_Sex_Pred_2 pSexPred2)
 :=
  transform
...
dSexPredJoined		:= join(dSexPredSearchSort,dSexPred2Sort,
							...
							tSexPredSearchandSexPred2toOffender(left,right),
							local
						   );

#if(CrimSrch.Switch_ShouldUsePersists = CrimSrch.Switch_YesValue)
export Sex_Pred_Search_DID_as_CrimSrch_Offender
 :=	dSexPredJoined
 :	persist('persist::Sex_Pred_Search_DID_as_CrimSrch_Offender')
 ;
#else
export Sex_Pred_Search_DID_as_CrimSrch_Offender
 :=	dSexPredJoined

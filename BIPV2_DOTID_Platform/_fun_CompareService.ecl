import BIPV2_DOTID_PLATFORM,tools;
...
function
  ...
  kFile := BIPV2_DOTID_PLATFORM.Keys(BFile/*,version*/);
  odl   := PROJECT(CHOOSEN(KFile.Candidates(DotID=DotIDone),100000),BIPV2_DOTID_PLATFORM.match_candidates(BFile).layout_candidates);
  odr   := PROJECT(CHOOSEN(KFile.Candidates(DotID=DotIDTwo),100000),BIPV2_DOTID_PLATFORM.match_candidates(BFile).layout_candidates);
  ...
  BIPV2_DOTID_PLATFORM.match_candidates(BFile).layout_attribute_matches ainto(KFile.Attribute_Matches le) := TRANSFORM
    SELF := le;
  END;

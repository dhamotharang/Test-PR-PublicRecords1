...
iter := '25';
#workunit('name','BIPV2_DOTID_PLATFORM Iter ' + iter + ' review samples');
psetReviewers           := ['CM','LB','TL','JL','FN','DW'];
pNumSamplesPerReviewer  := 15;

kmtch      := index(BIPV2_DOTID_PLATFORM.Keys(BIPV2_DOTID_PLATFORM.In_DOT).MatchSample ,BIPV2_DOTID_PLATFORM.keynames('20130330' + iter).match_sample_debug.logical);

tools.mac_GetReviewSamples(kmtch,DotID,'21',pNumSamplesPerReviewer,psetReviewers,true);
*/
import tools;
...
EXPORT mac_GetSALTReviewSamples(
  ...
  ,pInFile                            //Infile for matching(BIPV2_DOTID_PLATFORM.In_DOT)
...
////
  #APPEND(ECL    ,'allsamplerecs     := sort(samplerecseq + samplerecsgt,DotID1 + DotID2);\n')
...
////
  #APPEND(ECL    ,'allsampleDotIDs :=   table(table(samplerecseq + samplerecsgt  ,{unsigned6 ' + %'lID'% + ' := ' + %'lID'% + '1})\n')
  #APPEND(ECL    ,'                          + table(samplerecseq + samplerecsgt  ,{unsigned6 ' + %'lID'% + ' := ' + %'lID'% + '2})\n')
  #APPEND(ECL    ,'                      ,{' + %'lID'% + '},' + %'lID'% + ',few);\n')
  #APPEND(ECL    ,'setsampleDotIDs  := set(allsampleDotIDs  ,' + %'lID'% + ');\n')
  #APPEND(ECL    ,'allDotIDcands    := project(kcand(' + %'lID'% + ' in setsampleDotIDs)  ,' + %'lModule'% + '.match_candidates(' + %'lInfile'% + ').layout_candidates);\n')
  #APPEND(ECL    ,'s := dataset([],' + %'lModule'% + '.Layout_Specificities.R);\n')
  #APPEND(ECL    ,'allDotIDsrolled := ' + %'lModule'% + '.Debug(' + %'lInfile'% + ',s).RolledEntities(allDotIDcands);\n')
  #APPEND(ECL    ,'layrolled := recordof(allDotIDsrolled);\n\n')
  ...
  #LOOP
    ...
    #APPEND(NormSamples ,psetReviewers[%CNTR%] + '_norm := project(normalize(' + psetReviewers[%CNTR%] + ',3,transform({integer2 conf,unsigned6 DotID},self.DotID := choose(counter  ,left.DotID1,left.DotID2,0),self.conf := left.conf)) ,transform({unsigned cnt,recordof(left)},self.cnt := counter + ((startrec' + %'CNTR'% + ' - 1) * 3),self := left));\n')
    #APPEND(RollSamples ,psetReviewers[%CNTR%] + '_rolled := sort(join(' + psetReviewers[%CNTR%] + '_norm ,allDotIDsrolled ,left.DotID = right.DotID ,transform({unsigned cnt,integer2 conf,recordof(right)},self.cnt := left.cnt,self.conf := if(right.DotID != 0  ,left.conf,0),self := right),left outer,lookup),cnt);\n')

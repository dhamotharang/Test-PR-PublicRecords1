IMPORT tools, HMS_Medicaid_VT,HMS_Medicaid_IL,HMS_Medicaid_Common;
Medicaid_State := 'IL';      
EXPORT Files(STRING pversion = '', boolean pUseProd = false) := MODULE
	...
	EXPORT Medicaid_File := Dataset(inFile,HMS_Medicaid_Common.Layouts.Medicaid_IL, csv( separator('\t'),heading(2), terminator(['\n', '\r\n']), quote('"')));  //EXPORT SureScripts_File := Dataset(inFile,Layouts.SureScripts_In_raw, flat);//csv( separator('\t'),heading(single), terminator(['\n', '\r\n']), quote('')));  
	EXPORT Medicaid_input := Medicaid_File;
	//EXPORT Medicaid_input_Raw:= Medicaid_File_raw;
	tools.mac_FilesBase(HMS_Medicaid_Common.Filenames(Medicaid_State,pversion,pUseProd).Base, HMS_Medicaid_Common.layouts.Base_IL, base);

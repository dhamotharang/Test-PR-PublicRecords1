...
export _Flags(string pInput_Name='', string pBase_Name='')  := module
	...
	export ExistCurrentSprayed			:= fileservices.superfileexists(HMS_Medicaid_Common.FileNames('IL',pInput_Name,).lInputTemplate) and 
																		 count(fileservices.superfilecontents(HMS_Medicaid_Common.FileNames('IL',pInput_Name,).lInputTemplate  )) > 0  ;
	export ExistBaseFile						:= count(nothor(fileservices.superfilecontents(HMS_Medicaid_Common.FileNames('IL',pBase_name,).base.qa))) > 0;	

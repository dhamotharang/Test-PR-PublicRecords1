...
export Fetch_Name(
	...
	gl_autokey.autokey_interfaces.name in_parms) :=
		function

			i := autokey.key_name(autokey_keyname);
			...
			f := 
				 project(
						 i(in_parms.did_value = '' and in_parms.lname_value != '' AND in_parms.state_value = '' AND in_parms.city_value = '' AND in_parms.zip_value = [],
						keyed(dph_lname=metaphonelib.DMetaPhone1(in_parms.lname_value)[1..6]),
						keyed((lname=(STRING20)in_parms.lname_value OR (in_parms.phonetics and in_parms.workHard))),
						keyed(pfe(pfname,in_parms.fname_value) OR (LENGTH(TRIM(in_parms.fname_value))<2) and in_parms.workHard),
						keyed(fname[1..length(trim(in_parms.fname_value))]=in_parms.fname_value OR (in_parms.nicknames AND LENGTH(TRIM(in_parms.fname_value))>=2)), 

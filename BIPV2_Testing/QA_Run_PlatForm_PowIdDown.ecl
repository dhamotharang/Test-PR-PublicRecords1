 import BIPV2_Files,BIPV2_build, BIPV2_POWID_DOWN_Platform; //
/*   iteration := '1';
   pversion  := '20160112';
   ...
   BIPV2_POWID_DOWN_platform.proc_powid_down_platform().initFromLGID3;
*/

//If meet trouble, run the following specificities first:
/* rec:=RECORD
     unsigned6 rcid;
     string2 source;
    ...
    import BIPV2_Files,BIPV2_build;//
      ...
      //lih := BIPV2_Files.files_powid_down.DS_BUILDING;
   	 lih :=dataset('~	thor_data400::bipv2_powid_down_platform::init::20160112copynew',BIPV2_Testing.Test_Layout,thor);
      #OPTION('multiplePersistInstances',FALSE);
      #workunit('name','BIPV2_powid_down ' + pversion + ' Specificities ');
      #workunit('priority','high');
      BIPV2_POWID_DOWN_Platform.proc_powid_down_platform(lih,iteration).runSpecBuild;
*/

//Run this in a build window before plateform and after platform change and write down the corresponding Workunits, 
//respectively. The workunits need t go to QA_PlatFormComparisonSummary as WU_PowIdDown_Before and WU_PowIdDown_After.
...
rec:=RECORD
 ...
 import BIPV2_Files,BIPV2_build; //
   iteration := '1';
   pversion  := '20160112';
   //lih :=dataset('~thor_data400::bipv2_powid_down_platform::init::20160112copy',rec,thor);
	 lih :=dataset('~thor_data400::bipv2_powid_down_platform::init::20160112copynew',BIPV2_Testing.Test_Layout,thor);
   #OPTION('multiplePersistInstances',FALSE);
   #workunit('name','BIPV2_powid_down_platform ' + pversion + ' iter ' + iteration);
   #workunit('priority','high');
	 BIPV2_POWID_DOWN_Platform.proc_powid_down_platform(lih,iteration).runIter;

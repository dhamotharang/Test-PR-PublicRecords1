...
import aca, codes, watchdog, Business_Header, Business_Header_SS, BankruptcyV2, BankruptcyV3, corp2, 
		yellowpages, Risk_Indicators, did_add, doxie, doxie_files, ut, address, gong, riskwise, suppress, liensv2, PAW, census_data, EBR, DCA, iesp, Business_Risk;

Layout_Input := Business_Risk.Layout_Input;
Layout_output := Business_Risk.Layout_output;
getWatchLists2 := Business_Risk.getWatchLists2;

export InstantID_Function(DATASET(Layout_Input) indata1, dataset(risk_indicators.Layout_Gateways_In) gateways, 
...
wRep_nobest get_rep_best(wRep_nobest L, watchdog.Key_Watchdog_GLB R) := transform
...
wRep_nobest get_rep_best_nonExperian(wRep_nobest L, watchdog.Key_Watchdog_GLB_nonExperian R) := transform
	self.repbestfname := R.fname;
	self.repbestlname := R.lname;
	self.repbestaddr1 := Risk_Indicators.MOD_AddressClean.street_address('',R.prim_Range, R.predir, R.prim_name, R.suffix, R.postdir, R.unit_desig, R.sec_range);
	self.repbestprimrange 	:= R.prim_range;
...
wRep_nobest get_rep_best_nonglb(wRep_nobest L, watchdog.Key_Watchdog_nonglb R) := transform
	self.repbestfname := R.fname;
	self.repbestlname := R.lname;
	self.repbestaddr1  := Risk_Indicators.MOD_AddressClean.street_address('',R.prim_Range, R.predir, R.prim_name, R.suffix, R.postdir, R.unit_desig, R.sec_range);
	self.repbestprimrange 	:= R.prim_range;
...
wRep := if (ut.glb_ok(glb), 
						
						 if(experian_permitted, 
								join(wRep_nobest, watchdog.Key_Watchdog_GLB,
									left.Repdid != 0 and keyed(left.Repdid = right.did),
										get_rep_best(LEFT,RIGHT), left outer),
							 
							 join(wRep_nobest, watchdog.Key_Watchdog_GLB_nonExperian,
										left.Repdid != 0 and keyed(left.Repdid = right.did),
										 get_rep_best_nonExperian(LEFT,RIGHT), left outer) ),
										 
						 // if glb isn't ok, use the nonglb key	 
					   join(wRep_nobest, watchdog.Key_Watchdog_nonglb,
							left.Repdid != 0 and keyed(left.Repdid = right.did),
						   get_rep_best_nonglb(LEFT,RIGHT),left outer));
...
bocaShell := Risk_Indicators.Boca_Shell_Function(ciid_results, gateways, dppa, glb, isUtility, ln_branded,
																									 /* includeRelativeInfo */TRUE,  /* includeDLInfo */FALSE,  /* includeVehInfo */FALSE,
																									 /* includeDerogInfo */TRUE, IF(IncludeRepAttributes, 3, bsversion), DataRestriction := DataRestriction);
...
//------------- OFAC/Watchlist info --------------
normalWatch := if((ExcludeWatchLists and ofac_version=1) or (ofac_version>1 and Include_Ofac=FALSE and Include_Additional_Watchlists=FALSE and count(watchlists_requested)=0),
									grouped_rep,
					getWatchLists2(grouped_rep, tribcode != '' OR ofac_only,,ofac_version,include_ofac,include_additional_watchlists,Global_WatchList_Threshold,watchlists_requested));
...
dirs_by_addr := RiskWise.getDirsByAddr( deduped_input_addresses, isFCRA );

deduped_input_phones := dedup(sort(risk_indic_indata, phone10, seq), phone10);
dirs_by_phone := RiskWise.getDirsByPhone( deduped_input_phones, gateways, dppa, glb, isFCRA);
...
RiskWise.Layout_Dirs_Address Ri

) := transform	
	cmpymatch_score 	:= tmax2(CnameScore(l.company_name, ri.listed_name), CnameScore(L.alt_company_name, ri.listed_name));
	cmpymatch 		:= g(cmpymatch_score);
	//phonematchscore 	:= tscore(ut.max2(0,Risk_Indicators.PhoneScore(l.phone10, ri.phone10)));
	phonematchscore 	:= tscore(Max(0,Risk_Indicators.PhoneScore(l.phone10, ri.phone10)));
	phonematch 		:= gn(phonematchscore);
	
	zip_score := Risk_Indicators.AddrScore.zip_score(l.orig_z5, ri.z5);
	cityst_score := Risk_Indicators.AddrScore.citystate_score(l.p_city_name, l.st, ri.p_city_name, ri.st, l.cityzipflag);
	...
	self.vercmpy 		:= IF (cmpymatch, ri.listed_name, l.vercmpy);	
	...
	self.verphone 		:= if (L.phoneMatchFlag = 'Y', L.verphone, if (self.phoneMatchFlag = 'Y' and cmpymatch, ri.phone10, L.verphone));
	...
	self.verAddr		:= if (self.AddrMatchFlag = 'Y' and cmpymatch, Risk_Indicators.MOD_AddressClean.street_address('',ri.prim_range, ri.predir, ri.prim_name, ri.suffix, ri.postdir, ri.unit_desig, ri.sec_range), L.verAddr);
	self.vercity		:= map(cmpymatch and MIN(ut.stringsimilar(ri.p_city_name,L.p_city_name),ut.stringsimilar(L.p_city_name,ri.p_city_name)) < 3 => ri.p_city_name,
						  cmpymatch and MIN(ut.stringsimilar(ri.v_city_name,L.v_city_name),ut.stringsimilar(L.v_city_name,ri.v_city_name)) < 3 => ri.v_city_name, 
						  L.vercity);
	self.cityMatchFlag	:= map (cmpymatch and l.p_city_name != '' and MIN(ut.stringsimilar(ri.p_city_name,L.p_city_name),ut.stringsimilar(L.p_city_name,ri.p_city_name)) < 3 => 'Y',
						   cmpymatch and l.v_city_name != '' and MIN(ut.stringsimilar(ri.v_city_name,L.v_city_name),ut.stringsimilar(L.v_city_name,ri.v_city_name)) < 3 => 'Y',
						   'N');
	self.verstate 		:= IF (cmpymatch and ri.st = L.st, ri.st, L.verstate);
	self.stateMatchFlag := IF (cmpymatch and L.st != '' and ri.st = L.st, 'Y', 'N');
	self.verzip		:= IF (cmpymatch and (integer)ri.z5  = (integer)L.z5, 
							(if((integer)ri.z5=0,'',(string)ri.z5)), L.verzip);
	self.verzip4	:= IF (cmpymatch and (integer)ri.z5 = (integer)L.z5 and (integer)ri.z4 = (integer)L.zip4, 
							(if((integer)ri.z4=0,'',(string)ri.z4)), L.verzip4);
	self.zipMatchFlag	:= IF (cmpymatch and L.z5 != '' and (integer)ri.z5  = (integer)L.z5, 'Y', 'N');
	self.gong_cmatch	:= cmpymatch;
	self.gong_pmatch	:= cmpymatch and phonematch;
	self.gong_amatch	:= cmpymatch and ga(addrmatchscore);
	//------ From join below
	self.CmpyPhoneFromAddr := if (cmpymatch, ri.phone10, l.cmpyphonefromaddr); 
	...
	self.phonecmpycount := L.phonecmpycount + IF(cmpymatch and phonematch and ri.business_flag,1,0);
	self.phoneaddrcount := L.phoneaddrcount + IF(phonematch and addrmatch and ri.business_flag,1,0);
	self.cmpyaddrcount	:= L.cmpyaddrcount + IF(cmpymatch and addrmatch and ri.business_flag,1,0);
	self.phonecmpyaddrcount := L.phonecmpyaddrcount + IF(phonematch and addrmatch and cmpymatch, 1, 0) ;
	self.wrongphoneflag			 := map(ri.prim_name = '' or L.phone10 = '' => '', // left outer condition - no match
								   ~cmpymatch and ~phonematch => '0',
								   ~phonematch => '1',
								   '2');
	self.resAddrFlag 			 := if (ri.business_flag or ri.prim_name = '', 'N', 'Y');
	self.busphonelat			 := if (L.busphonelat = '' and phonematch, ri.geo_lat, L.busphonelat);
	self.busphonelong			 := if (L.busphonelong = '' and phonematch, ri.geo_long, L.busphonelong);
	self.busPhoneZip			 := if (L.busPhoneZip = ''and phonematch, ri.z5, L.busPhoneZip);
		
	myGetDate := risk_indicators.iid_constants.myGetDate(l.historydate);
	isrecent := recent_check(myGetDate,ri.dt_last_seen[1..6]+'31');
...
working_layout check_phone_vs_bh_2(byAddrRollPhone L, business_header_ss.Key_BH_Phone R) := transform
	cmpymatch 	:= g(tmax2(CnameScore(l.company_name, r.company_name),CnameScore(L.alt_company_name, R.company_name)));
	...
	addrmatch := ga(Risk_Indicators.AddrScore.AddressScore(l.prim_range, l.prim_name, l.sec_range, 
																						r.prim_range, r.prim_name, r.sec_range,
																						zip_score));
...
bans_bdid := BankruptcyV2.key_bankruptcy_bdid();
...
bkrec := record
	recordof(BankruptcyV3.key_bankruptcyv3_search_full());
	unsigned6	seq;
end;

bkrec get_bkrupt_info(bk_keys L, recordof(BankruptcyV3.key_bankruptcyv3_search_full()) R) := transform
	self.seq := L.seq;
	self := R;
end;

bkrecs := join(bk_keys, 
			BankruptcyV3.key_bankruptcyv3_search_full(),
...
// output(ciid_results, named('ciid_results'));
// output(repout, named('repout'));
// output(wRep_nobest, named('wRep_nobest'));
// output(normalWatch, named('normalWatch'));
// output(withWatchlistsData, named('withWatchlistsData'));

// output(corp_sort, named('corp_sort'));
// output(wstanding, named('wstanding'));
// output(wrelatives_pre_roll, named('wrelatives_pre_roll'));
// output(wrel_preroll_sort, named('wrel_preroll_sort'));
// output(wrelatives, named('wrelatives'));
// output(selected_bdid, named('selected_bdid'));
...
// output(withReasons, named('withReasons'));

...
EXPORT MAC_PopulationStatistics(infile,Ref='',Input_dt_vendor_first_reported = '',Input_dt_vendor_last_reported = '',Input_dt_first_seen = '',Input_dt_last_seen = '',Input_corp_ra_dt_first_seen = '',Input_corp_ra_dt_last_seen = '',Input_corp_process_date = '',Input_corp_filing_date = '',Input_corp_status_date = '',Input_corp_inc_date = '',Input_corp_forgn_date = '',Input_corp_ra_resign_date = '',Input_corp_merger_date = '',Input_corp_merger_effective_date = '',Input_corp_name_reservation_date = '',Input_corp_name_reservation_expiration_date = '',Input_corp_name_status_date = '',Input_corp_key = '',Input_corp_orig_sos_charter_nbr = '',Input_corp_vendor = '',Input_corp_state_origin = '',Input_corp_inc_state = '',Input_corp_legal_name = '',Input_corp_forgn_state_desc = '',Input_cont_country = '',Input_corp_agent_country = '',Input_recordorigin = '',Input_corp_ln_name_type_cd = '',Input_corp_status_cd = '',Input_corp_name_status_cd = '',Input_corp_orig_org_structure_cd = '',Input_corp_orig_bus_type_cd = '',OutFile) := MACRO
...
+    #IF( #TEXT(Input_corp_merger_effective_date)='' )
      '' 
    #ELSE
        IF( (unsigned)le.Input_corp_merger_effective_date = 0,'', ':corp_merger_effective_date(' + SALT34.fn_date_valid_as_text((unsigned)le.Input_corp_merger_effective_date) + ')' )
    #END
 
+    #IF( #TEXT(Input_corp_name_reservation_date)='' )
      '' 
    #ELSE
        IF( (unsigned)le.Input_corp_name_reservation_date = 0,'', ':corp_name_reservation_date(' + SALT34.fn_date_valid_as_text((unsigned)le.Input_corp_name_reservation_date) + ')' )
    #END
 
+    #IF( #TEXT(Input_corp_name_reservation_expiration_date)='' )
      '' 
    #ELSE
        IF( (unsigned)le.Input_corp_name_reservation_expiration_date = 0,'', ':corp_name_reservation_expiration_date(' + SALT34.fn_date_valid_as_text((unsigned)le.Input_corp_name_reservation_expiration_date) + ')' )
    #END

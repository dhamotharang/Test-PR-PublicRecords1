  ...
  EXPORT getMatchedEntityTypes(DATASET(FraudShared_Services.Layouts.BatchInExtended_rec) ds_batch_in,
    ...
    FraudShared_Services.Layouts.layout_velocity_matches xform_velocity_matches(FraudShared_Services.Layouts.BatchInExtended_rec l,
      ...
      emailAddress := IF(l.email_address <> '' AND
                          (l.email_address = r.email_address OR
                           (platform = FraudShared_Services.Constants.Platform.FraudGov AND
                            (TRIM(regexfind('(.*)@(.*)$',l.email_address,1)) = TRIM(regexfind('(.*)@(.*)$',r.email_address,1)) OR
                             TRIM(regexfind('(.*)@(.*)$',l.email_address,2)) = TRIM(regexfind('(.*)@(.*)$',r.email_address,2)))
                           )),

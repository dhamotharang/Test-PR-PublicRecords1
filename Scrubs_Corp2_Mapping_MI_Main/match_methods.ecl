IMPORT SALT32,std;
EXPORT match_methods(DATASET(layout_In_MI) ih) := MODULE
...
EXPORT match_corp_llc_managed_cd(TYPEOF(h.corp_llc_managed_cd) L, TYPEOF(h.corp_llc_managed_cd) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT32.MatchCode.ExactMatch,
    SALT32.MatchCode.NoMatch),
  MAP(L = R => SALT32.MatchCode.ExactMatch, SALT32.MatchCode.NoMatch)
);

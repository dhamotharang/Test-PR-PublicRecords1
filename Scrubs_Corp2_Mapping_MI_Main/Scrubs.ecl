...
// The module to handle the case where no scrubs exist
  ...
  EXPORT Expanded_Layout := RECORD(Corp2_Mapping.LayoutsCommon.Main)
    ...
    UNSIGNED1 cont_address_line3_Invalid;
...
EXPORT FromNone(DATASET(Corp2_Mapping.LayoutsCommon.Main) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_address1_line3_Invalid := Fields.InValid_corp_address1_line3((SALT38.StrType)le.corp_address1_line3);
    ...
    SELF.cont_address_line3_Invalid := Fields.InValid_cont_address_line3((SALT38.StrType)le.cont_address_line3);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_ln_name_type_desc_Invalid << 22 ) + ( le.corp_address1_line3_Invalid << 23 ) + ( le.corp_status_comment_Invalid << 24 ) + ( le.corp_inc_state_Invalid << 25 ) + ( le.corp_inc_date_Invalid << 26 ) + ( le.corp_forgn_state_cd_Invalid << 28 ) + ( le.corp_forgn_state_desc_Invalid << 29 ) + ( le.corp_forgn_date_Invalid << 30 ) + ( le.corp_orig_org_structure_desc_Invalid << 32 ) + ( le.cont_address_line3_Invalid << 33 ) + ( le.corp_dissolved_date_Invalid << 34 ) + ( le.recordorigin_Invalid << 36 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_status_comment_Invalid := (le.ScrubsBits1 >> 24) & 1;
    SELF.corp_inc_state_Invalid := (le.ScrubsBits1 >> 25) & 1;
    SELF.corp_inc_date_Invalid := (le.ScrubsBits1 >> 26) & 3;
    SELF.corp_forgn_state_cd_Invalid := (le.ScrubsBits1 >> 28) & 1;
    SELF.corp_forgn_state_desc_Invalid := (le.ScrubsBits1 >> 29) & 1;
    SELF.corp_forgn_date_Invalid := (le.ScrubsBits1 >> 30) & 3;
    SELF.corp_orig_org_structure_desc_Invalid := (le.ScrubsBits1 >> 32) & 1;
    SELF.cont_address_line3_Invalid := (le.ScrubsBits1 >> 33) & 1;
    SELF.corp_dissolved_date_Invalid := (le.ScrubsBits1 >> 34) & 3;
    SELF.recordorigin_Invalid := (le.ScrubsBits1 >> 36) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    cont_address_line3_ALLOW_ErrorCount := COUNT(GROUP,h.cont_address_line3_Invalid=1);
    ...
    AnyRule_WithErrorsCount := COUNT(GROUP, h.dt_vendor_first_reported_Invalid > 0 OR h.dt_vendor_last_reported_Invalid > 0 OR h.dt_first_seen_Invalid > 0 OR h.dt_last_seen_Invalid > 0 OR h.corp_ra_dt_first_seen_Invalid > 0 OR h.corp_ra_dt_last_seen_Invalid > 0 OR h.corp_key_Invalid > 0 OR h.corp_vendor_Invalid > 0 OR h.corp_state_origin_Invalid > 0 OR h.corp_process_date_Invalid > 0 OR h.corp_orig_sos_charter_nbr_Invalid > 0 OR h.corp_legal_name_Invalid > 0 OR h.corp_ln_name_type_cd_Invalid > 0 OR h.corp_ln_name_type_desc_Invalid > 0 OR h.corp_address1_line3_Invalid > 0 OR h.corp_status_comment_Invalid > 0 OR h.corp_inc_state_Invalid > 0 OR h.corp_inc_date_Invalid > 0 OR h.corp_forgn_state_cd_Invalid > 0 OR h.corp_forgn_state_desc_Invalid > 0 OR h.corp_forgn_date_Invalid > 0 OR h.corp_orig_org_structure_desc_Invalid > 0 OR h.cont_address_line3_Invalid > 0 OR h.corp_dissolved_date_Invalid > 0 OR h.recordorigin_Invalid > 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.cont_address_line3_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.corp_dissolved_date_Invalid,'ALLOW','CUSTOM','UNKNOWN')
          ,CHOOSE(le.recordorigin_Invalid,'ENUM','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'dt_vendor_first_reported','dt_vendor_last_reported','dt_first_seen','dt_last_seen','corp_ra_dt_first_seen','corp_ra_dt_last_seen','corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_orig_sos_charter_nbr','corp_legal_name','corp_ln_name_type_cd','corp_ln_name_type_desc','corp_address1_line3','corp_status_comment','corp_inc_state','corp_inc_date','corp_forgn_state_cd','corp_forgn_state_desc','corp_forgn_date','corp_orig_org_structure_desc','cont_address_line3','corp_dissolved_date','recordorigin','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_mandatory','invalid_type_cd','invalid_type_desc','invalid_alpha','invalid_status_comment','invalid_state_origin','invalid_date','invalid_forgn_state','invalid_forgn_state','invalid_date','invalid_org_structure_desc','invalid_alpha','invalid_date','invalid_recordorigin','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT38.StrType)le.dt_vendor_first_reported,(SALT38.StrType)le.dt_vendor_last_reported,(SALT38.StrType)le.dt_first_seen,(SALT38.StrType)le.dt_last_seen,(SALT38.StrType)le.corp_ra_dt_first_seen,(SALT38.StrType)le.corp_ra_dt_last_seen,(SALT38.StrType)le.corp_key,(SALT38.StrType)le.corp_vendor,(SALT38.StrType)le.corp_state_origin,(SALT38.StrType)le.corp_process_date,(SALT38.StrType)le.corp_orig_sos_charter_nbr,(SALT38.StrType)le.corp_legal_name,(SALT38.StrType)le.corp_ln_name_type_cd,(SALT38.StrType)le.corp_ln_name_type_desc,(SALT38.StrType)le.corp_address1_line3,(SALT38.StrType)le.corp_status_comment,(SALT38.StrType)le.corp_inc_state,(SALT38.StrType)le.corp_inc_date,(SALT38.StrType)le.corp_forgn_state_cd,(SALT38.StrType)le.corp_forgn_state_desc,(SALT38.StrType)le.corp_forgn_date,(SALT38.StrType)le.corp_orig_org_structure_desc,(SALT38.StrType)le.cont_address_line3,(SALT38.StrType)le.corp_dissolved_date,(SALT38.StrType)le.recordorigin,'***SALTBUG***');
  END;
  ...
  // field error stats
    SALT38.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_address1_line3:invalid_alpha:ALLOW'
          ,'corp_status_comment:invalid_status_comment:ALLOW'
          ,'corp_inc_state:invalid_state_origin:ENUM'
          ,'corp_inc_date:invalid_date:ALLOW','corp_inc_date:invalid_date:CUSTOM'
          ,'corp_forgn_state_cd:invalid_forgn_state:CUSTOM'
          ,'corp_forgn_state_desc:invalid_forgn_state:CUSTOM'
          ...
          ,'cont_address_line3:invalid_alpha:ALLOW'
          ,'corp_dissolved_date:invalid_date:ALLOW','corp_dissolved_date:invalid_date:CUSTOM'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_cont_address_line3(1)
          ,Fields.InvalidMessage_corp_dissolved_date(1),Fields.InvalidMessage_corp_dissolved_date(2)
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.cont_address_line3_ALLOW_ErrorCount
          ,le.corp_dissolved_date_ALLOW_ErrorCount,le.corp_dissolved_date_CUSTOM_ErrorCount
...
EXPORT StandardStats(DATASET(Corp2_Mapping.LayoutsCommon.Main) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  ...
  SALT38.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Corp2_Mapping_MI_Main, Fields, 'RECORDOF(scrubsSummaryOverall)', '');

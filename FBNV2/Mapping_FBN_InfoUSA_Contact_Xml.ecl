...
layout_common.contact_AID tFiling(dFiling pInput)
   :=TRANSFORM
      //string182 clean_address         := if(pInput.clean_ct_address<>'',pInput.clean_ct_address,pInput.clean_address);
			...
			self.contact_type	    		:= 'C';
			self.CONTACT_NAME_FORMAT        :=  IF(pInput.CtGN<>'','P','');
			self.contact_name			    := trim(pInput.CtGN)+' '+pinput.CTLN;
			self.contact_addr 				:= pInput.CTADDRESS;
			self.contact_CITY               := pInput.CTCITY ;
			self.contact_STATE              := pInput.CTSTATE ;
			self.contact_ZIP 				:= (integer) pInput.CTZIP;			
			self.contact_phone       		:= pInput.CTPHONE; 
...
dOut  	    :=rollup(dSort ,rollupXform(left,right),
					RECORD,except dt_first_seen,dt_last_seen, dt_vendor_first_reported,dt_vendor_last_reported,local) 
				 :PERSIST(Cluster.cluster_out+'persist::FBNV2::infousa::Contact::XML'); 	  

export Mapping_FBN_InfoUSA_Contact_xml := dOut;
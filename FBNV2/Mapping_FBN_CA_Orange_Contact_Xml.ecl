...
layout_common.contact_AID tFiling(dFiling pInput)
   :=TRANSFORM
			...
			self.contact_type	    		:= 'O';
			self.contact_NAME          		:= if(pInput.Ownerflname='',trim(pInput.OwnerGN)+' '
			                                                      +trim(pInput.OwnerMN,left,right)+' '
																  +trim(pInput.OwnerLN,left,right),
			                                                      pInput.Ownerflname);			
			...
			self.contact_ZIP          		:= (integer)pInput.OWNERZIP  ;   
...
dout        :=rollup(dSortFiling,rollupXform(left,right),
					RECORD,except dt_first_seen,dt_last_seen, dt_vendor_first_reported,dt_vendor_last_reported,local)
					:persist(cluster.cluster_in+'persist::FBNv2::CA::ORANGE::CONTACT::xml');

export Mapping_FBN_CA_Orange_Contact_xml      :=	dOut;
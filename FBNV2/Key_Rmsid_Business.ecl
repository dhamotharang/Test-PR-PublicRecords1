...
recordof(dBase) suppress_FilingNbr(dBase pInput):= TRANSFORM
...
sfilingnum:=PROJECT(dBase, suppress_FilingNbr(left));

dSort         := sort(distribute(sfilingnum,hash(tmsid,rmsid)),tmsid,rmsid,local);
export  Key_Rmsid_Business:= INDEX(dSort ,{tmsid,rmsid},{sfilingnum},KeyName+doxie.Version_SuperKey+'::Rmsid_Business');

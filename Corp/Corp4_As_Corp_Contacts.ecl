...
Layout_Corp_Cont_Base TranslateCorp4ToCorpContacts(Layout_Corp4_Cont_Temp l) := transform
self.dt_first_seen := (unsigned4)CheckDate(l.dt_first_seen);
self.dt_last_seen := (unsigned4)CheckDate(l.dt_last_seen);
self.dt_vendor_first_reported := (unsigned4)CheckDate(l.dt_first_seen);
self.dt_vendor_last_reported := (unsigned4)CheckDate(l.dt_last_seen);
self.corp_key := Map_Corp_Key(l.state_origin, l.sos_ter_nbr);
self.corp_vendor := 'EX';
self.corp_state_origin := l.state_origin;
self.corp_process_date := l.extract_date;
self.corp_orig_sos_charter_nbr := l.sos_ter_nbr;
self.corp_sos_charter_nbr := Map_Charter_Number('EX',l.state_origin,l.sos_ter_nbr);
...
Corp_Company_Dist := distribute(Corp4_As_Corp, hash(corp_key));

Layout_Corp_Cont_Base AppendCompanyToContact(Layout_Corp_Cont_Base l, Layout_Corp_Base r) := transform
...
export Corp4_As_Corp_Contacts := Corp_Cont_Dedup : persist('TEMP::Corp4_As_Corp_Contacts');
corp_base := File_Corp_Base(corp_vendor <> 'EX');

corp_all := corp_base + Corp.Corp4_As_Corp_History;
...
Corp.Layout_Corp_Base SetRecordType(Corp.Layout_Corp_Base l, Corp.Layout_Corp_Base r) := transform
...
/*
layout_corpkey_list := record
corp_all.corp_key;
corp_all.corp_vendor;
end;

corp_list := table(corp_all_final(corp_vendor = 'EX'), layout_corpkey_list);
corp_list_dedup := dedup(corp_list, corp_key, all);
corp_list_enth := enth(corp_list_dedup, 400);

Corp.Layout_Corp_Base SelectHistory(Corp.Layout_Corp_Base l, layout_corpkey_list r) := transform
self := l;
end;

// Extract some examples of direct and Experian history
corp_all_ex := join(corp_all_final,
                    corp_list_enth,
					left.corp_key = right.corp_key,
					SelectHistory(left, right),
					lookup);
					
output(choosen(sort(corp_all_ex, corp_key, if(corp_vendor = 'EX', 1, 0), -dt_vendor_last_reported, -dt_last_seen),2000),all);

corp_all_ex_dist := distribute(corp_all_ex, hash(corp_key));
*/
...
Corp.Layout_Corp_Base SelectCorpKeys(Corp.Layout_Corp_Base l, layout_corp_key r) := transform
self := l;
end;
...
Corp.Layout_Corp_Base RollupExperian(Corp.Layout_Corp_Base l, Corp.Layout_Corp_Base r) := transform
SELF.dt_first_seen := 
...
export Corp_Combined_History := corp_all_combined : persist('TEMP::Corp_Combined_History');
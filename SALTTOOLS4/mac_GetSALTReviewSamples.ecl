...
export outputReviewSamples     := SALTTOOLS4.mac_GetSALTReviewSamples(kmtch,kcand,BIPV2_LGID3.In_LGID3,LGID3,ConfThreshold,pNumSamplesPerReviewer,psetReviewers,false);
outputReviewSamples;
*/
import SALTTOOLS4,std;
...
////
  ...
  #SET(layscore  , SALTTOOLS4.mac_FilterLayout(recordof(pMatchSampleKey),true,'^(?!.*?(left|right|skipped).*).*$'    ,true))
  #SET(layfields , SALTTOOLS4.mac_FilterLayout(recordof(pMatchSampleKey),true,'^(?=.*?(left|right|skipped).*).*$'    ,true)) 
  #SET(laynorm   , SALTTOOLS4.mac_FilterLayout(recordof(pMatchSampleKey),true,'^(conf|support|attribute_conf)'      ,true)) 
  #SET(laynorm   , %'laynorm'%[1..(length(%'laynorm'%) - 1)] + ',typeof(' + %'kcand'% + '.' + %'lID'% + ') ' + %'lID'% + '}'  )
  #SET(setfields   ,SALTTOOLS4.mac_SetFields(recordof(pMatchSampleKey),'^(support_|attribute_).*$',true,true) )

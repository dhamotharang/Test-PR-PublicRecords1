...
IMPORT Scrubs_Inql_Nonfcra_Bridger as S;

F := S.In_File: independent;		  // input file
o := 'Scrubs_Inql_Nonfcra_Bridger';		// orbit profile name
m := 'Inql_Nonfcra_Bridger';
...
EXPORT proc_generate_report (boolean submitTheStats = true) := FUNCTION
        ...
        disp_orbit_report := output(orbitStats,all,named('OrbitReportBridger'), overwrite);
        submitStats       := Scrubs.OrbitProfileStats(o,'ScrubsAlerts',orbitStats,ver,m,CustomTag:='').RemoteSubmitStats(false);

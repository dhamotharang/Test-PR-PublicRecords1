...
#workunit('name','BDID Regression Roxie');
#option ('spillMultiCondition', true);

old := bdid_regression.File_Control_Roxie;
new := bdid_regression.Common_BDID_Roxie  : persist('persist::bdid_regression_Roxie');
output(new,,'BASE::BDID_Regression_Common_DID_Roxie' + bdid_regression.version, overwrite);

//****** COMPARE THE MATCHES BY RID -> DID
did_regression.MAC_DID_Diff(old, new, bdid, did_diff, rid, bdid_score, 75)

output(did_diff,,'BASE::BDID_Regression_Full_Diff_Roxie' + bdid_regression.version, overwrite);



//did_diff := dataset('BASE::BDID_Regression_Full_Diff_Roxie' + bdid_regression.version, bdid_regression.Layout_DID_Diff_full, flat);
i := 1000;

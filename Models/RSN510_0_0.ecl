...
export RSN510_0_0(grouped dataset(Risk_Indicators.Layout_Boca_Shell) clam, unsigned3 history_date=999999, boolean OFAC=true) := FUNCTION
...
Layout_ModelOut addReasons(Layout_ModelOut le, iid ri) := transform
	SELF.ri := risk_indicators.reasonCodes(ri, 4);
	SELF := le;
end;

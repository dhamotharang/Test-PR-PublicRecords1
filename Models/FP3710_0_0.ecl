...
export FP3710_0_0(
	...
	results := join(clam, easi.Key_Easi_Census,
				...
				join2recs(left,right), atmost(riskwise.max_atmost),
				left outer);
...
temp := record
	...
	temp doModel(results le) := TRANSFORM
		...
		rc_ssnlowissue  								 := le.bs.iid.socllowissue; // added to this model on 20090909
    ...
    phones_per_adl                   :=  le.bs.velocity_counters.phones_per_adl; // Current # of Unique Phones Found with ADL
		...
		sysyear :=  __common__( if( archive_date=999999, (integer)((STRING8)Std.Date.Today())[1..4], archive_date/100 ));
		phone_zip_mismatch :=  __common__( (integer)((INTEGER)rc_phonezipflag=1));
		disconnected :=  __common__( (integer)((INTEGER)rc_hriskphoneflag=5));
		deceased :=  __common__( (integer)(rc_decsflag));
		
		dob_yr :=  __common__( if((INTEGER)in_dob=0, -1, (INTEGER)in_dob[1..4]));	
		
		cell_mobile_pager :=  __common__( (INTEGER)((INTEGER)rc_hriskphoneflag in [1,2,3] ));

		pnotpot :=  __common__( 1-(INTEGER)(rc_hphonetypeflag in ['0','Z']));
		phninval :=  __common__( (INTEGER)((INTEGER)rc_phonevalflag=0));


		phninval2           :=  __common__( (INTEGER)((INTEGER)rc_hphonevalflag=0));
		busphone_flag       :=  __common__( (INTEGER)((INTEGER)rc_hphonevalflag=1));
		hr_phone_flag       :=  __common__( (INTEGER)((INTEGER)rc_hphonevalflag=3));
		non_land_phone_flag :=  __common__( (INTEGER)((INTEGER)rc_hphonevalflag=4));

		ssnprior_pi :=  __common__( (INTEGER)((INTEGER)rc_ssndobflag=1));
		ssninval_pw :=  __common__( (INTEGER)((INTEGER)rc_pwssnvalflag in [1,2,3]));
		addinval    :=  __common__( (INTEGER)(rc_addrvalflag='N'));
		aptflag     :=  __common__( (INTEGER)(rc_dwelltype='A'));
		bkflag      :=  __common__( (INTEGER)((INTEGER)rc_bansflag in [1,2]));
		hirisk_commercial_flag :=  __common__( (INTEGER)((INTEGER)rc_addrcommflag in [1,2]));
		city_zip_mismatch :=  __common__( (INTEGER)((INTEGER)rc_cityzipflag=1));


		/* Verification                  */

		source_FR_tot :=  __common__( contains_i(rc_sources,'FR,'));
		source_L2_tot :=  __common__( contains_i(rc_sources,'L2,'));
		source_LI_tot :=  __common__( contains_i(rc_sources,'LI,'));

		source_DA_fst :=  __common__( contains_i(fname_sources,'DA,'));
		source_FR_fst :=  __common__( contains_i(fname_sources,'FR,'));
		source_P_fst  :=  __common__( contains_i(fname_sources,'P ,'));

		source_BA_lst :=  __common__( contains_i(lname_sources,'BA,'));
		source_DA_lst :=  __common__( contains_i(lname_sources,'DA,'));
		source_FR_lst :=  __common__( contains_i(lname_sources,'FR,'));
		source_P_lst  :=  __common__( contains_i(lname_sources,'P ,'));

		source_AM_add :=  __common__( contains_i(addr_sources ,'AM,'));
		source_AR_add :=  __common__( contains_i(addr_sources ,'AR,'));
		source_BA_add :=  __common__( contains_i(addr_sources ,'BA,'));
		source_D_add  :=  __common__( contains_i(addr_sources ,'D ,'));
		source_EB_add :=  __common__( contains_i(addr_sources ,'EB,'));
		source_EM_add :=  __common__( contains_i(addr_sources ,'EM,'));
		source_VO_add :=  __common__( contains_i(addr_sources, 'VO,')); // voter is seperate from EM now on new records
		source_EM_VO_add :=  __common__( if(source_EM_add=1 or source_VO_add=1, 1, 0));	// check for either em or vo
		source_FR_add :=  __common__( contains_i(addr_sources ,'FR,'));
		source_P_add  :=  __common__( contains_i(addr_sources ,'P ,'));
		source_PL_add :=  __common__( contains_i(addr_sources ,'PL,'));
		source_TU_add :=  __common__( contains_i(addr_sources ,'TU,'));
		source_V_add  :=  __common__( contains_i(addr_sources ,'V ,'));
		source_W_add  :=  __common__( contains_i(addr_sources ,'W ,'));
		source_WP_add :=  __common__( contains_i(addr_sources ,'WP,'));


		num_pos_sources_NOEQ_add :=  __common__(
			source_AM_add
			+ source_AR_add
			+ source_BA_add
			+ source_D_add 
		...
		source_DA_flag :=  __common__( (integer)(source_DA_fst=1 and source_DA_lst=1));
		source_FR_flag :=  __common__( (source_FR_tot = 1));
		source_L2_flag :=  __common__( (source_L2_tot = 1));
		source_LI_flag :=  __common__( (source_LI_tot = 1));

		source_P_Flag :=  __common__( map(
			source_P_fst=1 and source_P_lst=1 and source_P_add=1 => 1,
			source_P_fst=1 and source_P_lst=1                    => -1,
			0
		));

		source_lien_flag :=  __common__( (integer)source_L2_flag + (integer)source_LI_flag);


		/* Nap Summary */
		contrary_phone :=  __common__( (INTEGER)(nap_summary=1));

		verfst_p :=  __common__( (INTEGER)(nap_summary in [2,3,4,8,9,10,12]));
		verlst_p :=  __common__( (INTEGER)(nap_summary in [2,5,7,8,9,11,12]));
		veradd_p :=  __common__( (INTEGER)(nap_summary in [3,5,6,8,10,11,12]));
		verphn_p :=  __common__( (INTEGER)(nap_summary in [4,6,7,9,10,11,12]));
                  
		best_match_level :=  __common__( map(
			(INTEGER)add1_isbestmatch=1 => 2,
			(INTEGER)add2_isbestmatch=1 => 0,
			1
		));

		ver_phone_tree :=  __common__( map(
			contrary_phone=1                                        => -1,
		...
		combo_dobcount_c :=  __common__( min( combo_dobcount, 3 ));


		rc_lnamessnmatch2_c :=  __common__( (integer)rc_lnamessnmatch2);
		rc_fnamessnmatch_c  :=  __common__( (integer)rc_fnamessnmatch);
		combo_dobscore_c :=  __common__( (integer)(combo_dobscore=100));


		num_pos_sources_NOEQ_add_c :=  __common__( min( num_pos_sources_NOEQ_add, 2 ));

		ver_phone_tree_m :=  __common__( map( 
		...
		rc_lnamessnmatch2_c_m :=  __common__( if( rc_lnamessnmatch2_c=0, 0.0670427425, 0.047043714 ));
		rc_fnamessnmatch_c_m  :=  __common__( if( rc_fnamessnmatch_c=0,  0.0684438802, 0.0461804887 ));


		best_match_level_m :=  __common__( map( 
		...
		combo_dob_flag_m :=  __common__( map( 
		...
		nas_ver :=  __common__( (integer)(nas_summary >= 10));

		ver_pl_p :=  __common__( map( 
		...
		ver_mod_best2 :=  __common__( -2.748940675
		...
		ver_mod_notbest2 :=  __common__( -3.478058771
		...
		ver_mod2 :=  __common__( if(add1_isbestmatch, ver_Mod_best2, ver_mod_notbest2 ));
		ver_mod3 :=  __common__( (exp(ver_mod2 )) / (1+exp(ver_mod2 )));


		/* FP                            */
		cellphone :=  __common__( (integer)( cell_mobile_pager=1 or non_land_phone_flag=1 ));
	

		pnotpot2 :=  __common__( map( 
			pnotpot = 1 and cellphone = 0 => 2,
			pnotpot = 1 or  cellphone = 1 => 1,
			0
		));

		phninval_flag :=  __common__( (integer)(phninval=1 or phninval2=1));

		
		phnprob_tree :=  __common__( map( 
...
/* Address */
		ec1 :=  __common__( trim(error_codes)[1]);
		ec3 :=  __common__( trim(error_codes)[3]);
		ec4 :=  __common__( trim(error_codes)[4]);
		
		addr_changed :=  __common__( (INTEGER)(ec1='S' AND ec3 != '0'));
		unit_changed :=  __common__( (INTEGER)(ec1='S' AND ec4 != '0'));

		casserror3 :=  __common__( map(
		...
		/* SSN */
		ssnprob :=  __common__( (integer)(deceased=1 or ssnprior_pi=1 or ssninval_pw=1));

		phnprob_tree_m :=  __common__( map( 
		...
		ssns_per_adl_c6_c   :=  __common__( min( ssns_per_adl_c6  , 2 ));
		addrs_per_adl_c6_c  :=  __common__( min( addrs_per_adl_c6 , 2 ));
		phones_per_adl_c6_c :=  __common__( min( phones_per_adl_c6, 2 ));

		velo_adl_c6 :=  __common__( map( 
		...
		addrs_per_ssn_c :=  __common__( 1-((integer)(addrs_per_ssn in [2,3])));

		nameperssn_count_c :=  __common__( min( nameperssn_count, 1 ));

		adlperssn_count_c :=  __common__( map( 
			adlperssn_count = 1 => 0,
			adlperssn_count = 2 => 1,
			adlperssn_count = 3 => 2,
			//adlperssn_count_c => 3
		...
		adls_per_addr_c :=  __common__( map( 
		...
		ssns_per_addr_c :=  __common__( map( 
		...
		phones_per_addr_c :=  __common__( map( 
			phones_per_addr = 1 => 0,
		...
		ssns_per_addr_c6_c   :=  __common__( min( ssns_per_addr_c6, 4 ));
		phones_per_addr_c6_c :=  __common__( min( phones_per_addr_c6, 3 ));

		velo_addr_c6 :=  __common__( map( 
		...
		dist_a1a2_flag :=  __common__( map( 
			dist_a1toa2 = 0 => 0,
			dist_a1toa2 >= 20 => 2,
			1
		));

		dist_a1a3_flag :=  __common__( map( 
			dist_a1toa3 = 0 => 0,
			dist_a1toa3 >= 20 => 2,
			1
		));

		dist_flag :=  __common__( map( 
			dist_a1a2_flag = 0 => 0,
			dist_a1a3_flag = 0 => 1,
			dist_a1a3_flag = 1 => 2,
			dist_a1a2_flag = 1 => 3,
		...
		add1_lres_flag :=  __common__( map( 
			add1_lres = 0 => 0,
			add1_lres <= 4 => 1,
			2
		));

		add2_lres_flag :=  __common__( map( 
			add2_lres = 0 => 0,
			add2_lres <= 5 => 1,
			2
		));

		lres_flag :=  __common__( map( 
		...
		addr_stability2 :=  __common__( map( 
		...
		add1_ownership_flag :=  __common__( map( 
		...
		add2_ownership_flag :=  __common__( map( 
			add2_applicant_owned => 2,
			add2_family_owned    => 1,
			0
		));

		dist_flag_m :=  __common__( map( 
		...
		add1_ownership_flag_m :=  __common__( map( 
		...
		add2_ownership_flag_m :=  __common__( map( 
		...
		addr_stability2_m :=  __common__( map( 
		...
		prop_mod_a :=  __common__( -6.417021023
		...
		prop_mod :=  __common__( (exp(prop_mod_a)) / (1+exp(prop_mod_a)));


		/* Derog                         */
		lien_rec_unrel_flag :=  __common__( (integer)(liens_recent_unreleased_count>0));
		lien_hist_unrel_flag :=  __common__( (integer)(liens_historical_unreleased_ct>0));
		lien_rec_rel_flag :=  __common__( (integer)(liens_recent_released_count>0));
		lien_hist_rel_flag :=  __common__( (integer)(liens_historical_released_count>0));

		lien_level :=  __common__( map(
		...
		criminal_flag :=  __common__( (integer)(criminal_count>0));
		crim_drug_flag :=  __common__( (integer)(criminal_flag=1 or source_DA_flag=1));

		bankrupt_flag :=  __common__( max( bkflag, bankrupt, source_BA_lst ));


		derog_level2 :=  __common__( map( 
			lien_level=2 or crim_drug_flag=1 or source_FR_flag => 2,
			lien_level=1 or bankrupt_flag=1 => 1,
			0
		));

		derog_level2_m :=  __common__( map( 
		...
		fp_mod3_a :=  __common__( -5.286940933
		...
		fp_mod3 :=  __common__( (exp(fp_mod3_a)) / (1+exp(fp_mod3_a)));


		/* Age                           */
		dob_age :=  __common__( if(dob_yr=-1, 0, sysyear - dob_yr));

		Risk_Level_m :=  __common__( map( rc_ssnlowissue[1..4]='1936' => 0.1003666,
												 dob_age >= 62 => 0.0430723727 ,
												 dob_age <= 20 => 0.0738688827 ,
												 dob_age <= 27 => 0.0492365223 ,
												 dob_age <= 47 => 0.0430723727 ,
												 dob_age <= 60 => 0.049876109   ,
												 0.0595642614) );


		/* Relatives                     */
		rel_prop_owned_risk :=  __common__( map( 
			rel_prop_owned_count = 1 => 0,
			rel_prop_owned_count in [2,3,4,5] => 1,
			2
		));

		rel_prop_owned_p_amt_risk :=  __common__( map( 
		...
		rel_prop_sold_p_amt_risk :=  __common__( map( 
		...
		rel_within25miles_risk :=  __common__( map( 
		...
		rel_within100miles_risk :=  __common__( map( 
			rel_within100miles_count in [1,2] => 0,
			1
		));

		rel_withinother_risk :=  __common__( min ( rel_withinother_count, 5 ));


		rel_homeunder150_risk :=  __common__( map( 
		...
		rel_educationunder12_risk :=  __common__( map( 
			rel_educationunder12_count = 0 => 0,
			rel_educationunder12_count = 1 => 1,
		   2
		));

		rel_ageunder30_risk :=  __common__( map( 
		...
		rel_ageunder60_risk :=  __common__( map( 
		...
		rel_mod_a :=  __common__( -4.431264259
		...
		rel_mod :=  __common__( (exp(rel_mod_a)) / (1+exp(rel_mod_a)));

		ams_4year :=  __common__( (integer)(ams_college_code[1] = '4'));


		/* Census */

		c_young_x         :=  __common__( (integer)(cenmatch=1 and (REAL)c_young        >= 42  ));
		c_cartheft_x      :=  __common__( (integer)(cenmatch=1 and (REAL)c_cartheft     >= 146 ));
		c_construction_x  :=  __common__( (integer)(cenmatch=1 and (REAL)c_construction <= 0   ));
		c_rental_x        :=  __common__( (integer)(cenmatch=1 and (REAL)c_rental       >= 186 ));
		c_new_homes_x     :=  __common__( (integer)(cenmatch=1 and (REAL)c_new_homes    <= 93  ));


	  // Check to see if any values used in calculation are empty.
		run_calc :=  __common__( Map(length(trim(c_fammar_p)) = 0 => 0,
		...
		cenmod2_a :=  __common__(
		 ...
		 cenmod2 :=  __common__( if( cenmatch=1 and run_calc=1, (exp(cenmod2_a)) / (1+exp(cenmod2_a)), 0.0796178));
				
				
 
		/* FP3710 Model */
		base  :=  __common__( 701);
		odds  :=  __common__( 1 / 201);
		point :=  __common__( -50);



		 fd_mod13b_t :=  __common__( -3.914470584
			+ rel_mod  * 5.6938954562
			+ prop_mod  * 3.5029999684
			+ Risk_Level_m  * 17.561090948
		 ...
		 fd_mod13b_a :=  __common__( fd_mod13b_t -2.662320034);		 
		 
		 fd_mod13b_b :=  __common__( (exp(fd_mod13b_a)) / (1+exp(fd_mod13b_a)));
		 phat :=  __common__( fd_mod13b_b);
		 fd_mod13b :=  __common__( (integer)(point*(log(phat/(1-phat)) - log(odds))/log(2) + base));
		 
// ********************************************************************************************************
/* Insert Code to Handle Missing Phone, SSN and DOB                  10-01-08 PK */


     fd_mod13c :=  __common__( fd_mod13b);

     source_AM_ssn :=  __common__( contains_i(ssn_sources ,'AM,'));
     source_AR_ssn :=  __common__( contains_i(ssn_sources ,'AR,'));
     source_BA_ssn :=  __common__( contains_i(ssn_sources ,'BA,'));
     source_D_ssn  :=  __common__( contains_i(ssn_sources ,'D ,'));
     source_EB_ssn :=  __common__( contains_i(ssn_sources ,'EB,'));
     source_EM_ssn :=  __common__( contains_i(ssn_sources ,'EM,'));
		 source_VO_ssn :=  __common__( contains_i(ssn_sources ,'VO,'));
		 source_EM_VO_ssn :=  __common__( if(source_EM_ssn=1 or source_VO_ssn=1, 1, 0));
     source_P_ssn  :=  __common__( contains_i(ssn_sources ,'P ,'));
     source_PL_ssn :=  __common__( contains_i(ssn_sources ,'PL,'));
     source_TU_ssn :=  __common__( contains_i(ssn_sources ,'TU,'));
     source_V_ssn  :=  __common__( contains_i(ssn_sources ,'V ,'));
     source_W_ssn  :=  __common__( contains_i(ssn_sources ,'W ,'));
     source_WP_ssn :=  __common__( contains_i(ssn_sources ,'WP,'));
                              
		                 
     num_pos_sources_NOEQ_ssn :=  __common__( sum(
                              source_AM_ssn ,
                              source_AR_ssn ,
                              source_BA_ssn ,
                              source_D_ssn  ,
     ...
     num_pos_sources_NOEQ_ssn_c :=  __common__( min ( num_pos_sources_NOEQ_ssn , 1 ));

     num_pos_sources_NOEQ_ssn_c_m :=  __common__( if(num_pos_sources_NOEQ_ssn_c = 0, 0.1022262608, 0.0492867332));

     ssns_per_adl_c :=  __common__( if(ssns_per_adl = 1, 0, 1));

     addrs_per_adl_c :=  __common__( Map(addrs_per_adl = 7 => 0,
														(( addrs_per_adl >= 3 ) and ( addrs_per_adl <= 20 )) => 1,
														addrs_per_adl = 2 => 2,
														3));

     phones_per_adl_c :=  __common__( Map(phones_per_adl = 1 => 0,
														 phones_per_adl = 0 => 1,
														 2));


     velo_adl :=  __common__( Map(ssns_per_adl_c <= 0  and addrs_per_adl_c <= 0  and phones_per_adl_c <= 0  => 0,
     ...
     city_zip_mismatch_m :=  __common__( if(city_zip_mismatch = 0, 0.0524951824, 0.1888111888));

     hirisk_commercial_flag_m :=  __common__( if(hirisk_commercial_flag = 0, 0.0526736898, 0.1102040816));


     addprob_mod_tmp :=  __common__( -4.30549571
                  + city_zip_mismatch_m  * 4.8402114832
                  + casserror3_m  * 6.119055673
                  + addprob_tree_m  * 14.455156819 );
		 
     addprob_mod :=  __common__( (exp(addprob_mod_tmp )) / (1+exp(addprob_mod_tmp )));

     addprob_mod2_tmp :=  __common__( -4.994542159
                  + city_zip_mismatch_m  * 9.6138923018
                  + hirisk_commercial_flag_m  * 12.730319108
                  + casserror3_m  * 16.93160518 );
		 
     addprob_mod2 :=  __common__( (exp(addprob_mod2_tmp )) / (1+exp(addprob_mod2_tmp )));



     rc_fnamecount_c :=  __common__( min( rc_fnamecount, 3 ));

     rc_fnamecount_c_m :=  __common__( Map(rc_fnamecount_c = 0 => 0.2291666667,
     ...
     combo_ssncount_c :=  __common__( min ( combo_ssncount   , 2 ));

     combo_ssncount_c_m :=  __common__( Map(combo_ssncount_c = 0 => 0.2328244275,
															 combo_ssncount_c = 1 => 0.0814006367,
															 combo_ssncount_c = 2 => 0.0492080634,
															 -9999));


     combo_addrscore_flag :=  __common__( if(combo_addrscore = 100, 1, 0));
     
     combo_addrscore_flag_m :=  __common__( if(combo_addrscore_flag = 0, 0.1462184874, 0.042983936));

     rc_ssncount_c :=  __common__( min ( rc_ssncount  , 3 ));

     rc_ssncount_c_m :=  __common__( Map(rc_ssncount_c = 0 => 0.2328244275,
     ...
     source_EQ_fst :=  __common__( contains_i(fname_sources ,'EQ,'));
     source_EQ_lst :=  __common__( contains_i(lname_sources ,'EQ,'));
     source_EQ_add :=  __common__( contains_i(addr_sources ,'EQ,'));
     source_EQ_ssn :=  __common__( contains_i(ssn_sources ,'EQ,'));
 

     num_pos_sources_add :=  __common__( sum(
                              source_AM_add ,
                              source_AR_add ,
                              source_BA_add ,
                              source_D_add  ,
     ...
     num_pos_sources_add_c :=  __common__( min ( num_pos_sources_add, 3 ));


     source_EQ_flag :=  __common__( map(source_EQ_fst = 1 and source_EQ_lst = 1 and source_EQ_add = 1 and source_EQ_ssn = 1 => 1,
													 source_EQ_fst = 1 and source_EQ_lst = 1 and source_EQ_ssn = 1 => -1,
													 0));


     vaddflag :=  __common__( Map(num_pos_sources_add_c >= 3 and source_EQ_flag >= 1 => 4,
     ...
     vaddflag_m :=  __common__( Map(vaddflag = 0 => 0.1878021569,
     ...
     EQ_count_c :=  __common__( Map(EQ_count <= 0 => 0,
											 EQ_count <= 1 => 1,
											 EQ_count <= 2 => 2,
											 EQ_count <= 6 => 3,
											 EQ_count <= 8 => 4,
     ...
     EQ_count_c_m :=  __common__( Map(EQ_count_c = 0 => 0.0681362725,
     ...
     combo_fnamecount_c  :=  __common__( min ( combo_fnamecount , 4 ));

     combo_fnamecount_c_m :=  __common__( Map(combo_fnamecount_c = 0 => 0.2647058824,
     ...
     ssnprob_nodob :=  __common__( if(deceased = 1 or ssninval_pw = 1, 1, 0));       /* Added 6-16 PK */
 
     ver_mod_best_nodob :=  __common__( -2.817321692                                      /* Added 6-11 PK */
                       + rc_fnamessnmatch_c_m  * 14.458214449
                       + num_pos_sources_NOEQ_ssn_c_m  * 10.74621313
                       + nas_ver  * -1.384909133
                       + ver_pl_p  * -0.424054628 );
											 
     ver_mod_nodob_a :=  __common__( (exp( ver_mod_best_nodob )) / (1+exp( ver_mod_best_nodob )));


     ver_mod_notbest_nodob :=  __common__( -2.810354097                                 /* Added 6-11 PK */
     ...
     ver_mod_nodob_b :=  __common__( (exp( ver_mod_notbest_nodob )) / (1+exp( ver_mod_notbest_nodob )));
		 
		 ver_mod_nodob :=  __common__( if(add1_isbestmatch, ver_mod_nodob_a, ver_mod_nodob_b));
		 
		 
		 
     fp_mod_nodob_tmp :=  __common__( -5.303359747                                    /* NoDob FP Mod Added 6-16 PK */
     ...
     fp_mod_nodob :=  __common__( (exp(fp_mod_nodob_tmp )) / (1+exp(fp_mod_nodob_tmp )));


     fd_mod13c_NoDob_tmp :=  __common__( -2.88997184
                  + rel_mod  * 5.38990313
     ...
     phat2 :=  __common__( (exp( fd_mod13c_NoDob_tmp )) / (1+exp( fd_mod13c_NoDob_tmp )));
     fd_mod13c_NoDob :=  __common__( (INTEGER)(point*(log(phat2/(1-phat2)) - log(odds))/log(2) + base));
...
/* Added Code to Handle Missing Phone And/Or SSN                    8-22-2008  PK  */
     ...
     /* NoPhn Specific Code */
        ...
        ver_phone_nophn_tree :=  __common__( Map(veradd_p = 1 and verlst_p = 1 and verfst_p = 1 => 3,
																		veradd_p = 1 and verlst_p = 1                  => 2,
																		veradd_p = 1                                   => 1,
																		0));

        ver_phone_nophn_tree_m :=  __common__( Map(ver_phone_nophn_tree = 0 => 0.0799772756,
        ...
        fp_mod_nophn_tmp :=  __common__( -4.36620104
                     + addprob_mod  * 14.087731403
                     + ssnprob  * 1.7646756233
                     + derog_level2_m  * 12.462135456 );
										 
        fp_mod_nophn :=  __common__( (exp(fp_mod_nophn_tmp )) / (1+exp(fp_mod_nophn_tmp )));


        fd_mod13b_nophn_tmp :=  __common__( -3.272175166
                     + rel_mod  * 6.4468814916
                     + prop_mod  * 3.0245521128
                     + Risk_Level_m  * 16.270260998
        ...
        phat3 :=  __common__( (exp(fd_mod13b_nophn_tmp )) / (1+exp(fd_mod13b_nophn_tmp )));
        fd_mod13b_nophn :=  __common__( (INTEGER)(point*(log(phat3/(1-phat3)) - log(odds))/log(2) + base));



     /* NoPhn-NoDOB Specific Code                          9-18 PK */

        ver_mod_best_nophn_nodob :=  __common__( -5.450978595
        ...
        ver_mod3_nophn_nodob_a :=  __common__( (exp(ver_mod_best_nophn_nodob )) / (1+exp(ver_mod_best_nophn_nodob )));

        ver_mod_notbest_nophn_nodob :=  __common__( -2.133995178
        ...
        ver_mod3_nophn_nodob_b :=  __common__( (exp(ver_mod_notbest_nophn_nodob )) / (1+exp(ver_mod_notbest_nophn_nodob )));

				ver_mod3_nophn_nodob :=  __common__( if(add1_isbestmatch, ver_mod3_nophn_nodob_a, ver_mod3_nophn_nodob_b));
				

        fp_mod_nophn_nodob_tmp :=  __common__( -4.363028812
                     + addprob_mod  * 14.179441454
                     + ssnprob_nodob  * 2.2731785863
                     + derog_level2_m  * 12.320909962);
										 
        fp_mod_nophn_nodob :=  __common__( (exp(fp_mod_nophn_nodob_tmp )) / (1+exp(fp_mod_nophn_nodob_tmp )));


        fd_mod13b_nophn_nodob_tmp :=  __common__( -3.308262932
        ...
        phat4 :=  __common__( (exp(fd_mod13b_nophn_nodob_tmp )) / (1+exp(fd_mod13b_nophn_nodob_tmp )));
        fd_mod13b_nophn_nodob :=  __common__( (INTEGER)(point*(log(phat4/(1-phat4)) - log(odds))/log(2) + base));



   /* NoSSN Specific Code */

     combo_dob_flag_nossn_a :=  __common__( Map(combo_dobscore_c = 0 and  combo_dobcount_c   = 0 => 0,
     ...
     ver_phone_tree_nossn_a :=  __common__( Map(verphn_p = 1 and verlst_p = 1 and verfst_p = 1 => 3,
																		verphn_p = 1 and verlst_p = 1                  => 2,
																		verphn_p = 1                                   => 1,
																		0));

     source_P_flag_nossn_a :=  __common__( Map(source_P_add = 1 and source_P_lst = 1 and source_P_fst = 1 => 2,
																	   source_P_lst = 1 and source_P_fst = 1 => 1,
																	   0));

     num_pos_sources_NOEQ_add_c_nossn_a :=  __common__( 1 * num_pos_sources_NOEQ_add_c);

     source_EQ_flag_nossn_a :=  __common__( Map(source_EQ_add = 1 and source_EQ_lst = 1 and source_EQ_fst = 1 => 2,
																		  source_EQ_lst = 1 and source_EQ_fst = 1 => 1,
                                      0));

     num_pos_sources_add_c_nossn_a :=  __common__( 1 * num_pos_sources_add_c);


     combo_dob_flag_nossn_b :=  __common__( Map(combo_dobscore_c = 0 and  combo_dobcount_c   = 0 => 10,
																		combo_dobscore_c = 0                             => 11,
																		combo_dobscore_c = 1 and  combo_dobcount_c  <= 2 => 12,
																		combo_dobscore_c = 1                             => 13,
																		-9999));

     ver_phone_tree_nossn_b :=  __common__( Map(verphn_p = 1 and verlst_p = 1 and verfst_p = 1    => 13,
																		verphn_p = 1 and ( verlst_p = 1 or verfst_p = 1 ) => 12,
																		verphn_p = 1                                      => 10,
																		contrary_phone = 1                                => 10,
																		11));

     source_P_flag_nossn_b :=  __common__( Map(source_P_add = 1 and source_P_lst = 1 and source_P_fst = 1 => 12,
																	 source_P_lst = 1 and source_P_fst = 1 => 10,
																	 11));

     num_pos_sources_NOEQ_add_c_nossn_b :=  __common__( 1 * num_pos_sources_NOEQ_add_c + 10);

     source_EQ_flag_nossn_b :=  __common__( Map(source_EQ_add = 1 and source_EQ_lst = 1 and source_EQ_fst = 1 => 12,
																		source_EQ_lst = 1 and source_EQ_fst = 1 => 11,
																		10));

     num_pos_sources_add_c_nossn_b :=  __common__( 1 * num_pos_sources_add_c + 10);



		 combo_dob_flag_nossn :=  __common__( if(add1_isbestmatch, combo_dob_flag_nossn_a, combo_dob_flag_nossn_b));
		 ver_phone_tree_nossn :=  __common__( if(add1_isbestmatch, ver_phone_tree_nossn_a, ver_phone_tree_nossn_b));
		 source_P_flag_nossn :=  __common__(  if(add1_isbestmatch, source_P_flag_nossn_a, source_P_flag_nossn_b));
		 num_pos_sources_NOEQ_add_c_nossn :=  __common__( if(add1_isbestmatch, num_pos_sources_NOEQ_add_c_nossn_a, num_pos_sources_NOEQ_add_c_nossn_b));
		 source_EQ_flag_nossn :=  __common__( if(add1_isbestmatch, source_EQ_flag_nossn_a, source_EQ_flag_nossn_b));
		 num_pos_sources_add_c_nossn :=  __common__( if(add1_isbestmatch, num_pos_sources_add_c_nossn_a, num_pos_sources_add_c_nossn_b));



     combo_dob_flag_nossn_m :=  __common__( Map(combo_dob_flag_nossn =  0 => 0.0805084746,
     ...
     ver_phone_tree_nossn_m :=  __common__( Map(ver_phone_tree_nossn =  0 => 0.0543197103,
     ...
     source_P_flag_nossn_m :=  __common__( Map(source_P_flag_nossn =  0 => 0.041014611,
     ...
     num_pos_src_NOEQ_add_c_nossn_m :=  __common__( Map(num_pos_sources_NOEQ_add_c_nossn =  0 => 0.0428911835,
     ...
     source_EQ_flag_nossn_m :=  __common__( Map(source_EQ_flag_nossn =  0 => 0.0507075472,
     ...
     num_pos_sources_add_c_nossn_m :=  __common__( Map(num_pos_sources_add_c_nossn =  0 => 0.0772833724,
     ...
     ver_mod_best_nossn :=  __common__( -5.352053576
     ...
     ver_mod_nossn_a :=  __common__( (exp(ver_mod_best_nossn )) / (1+exp(ver_mod_best_nossn )));


     ver_mod_notbest_nossn :=  __common__( -5.560172854
     ...
     ver_mod_nossn_b :=  __common__( (exp(ver_mod_notbest_nossn )) / (1+exp(ver_mod_notbest_nossn )));


		 ver_mod_nossn :=  __common__( if(add1_isbestmatch, ver_mod_nossn_a, ver_mod_nossn_b)); 
		 
		 

     fd_mod13b_nossn_tmp :=  __common__( -4.584758517
                  + rel_mod  * 6.2893752453
                  + prop_mod  * 3.8288237831
                  + Risk_Level_m  * 17.527390517
     ...
     phat5 :=  __common__( (exp(fd_mod13b_nossn_tmp )) / (1+exp(fd_mod13b_nossn_tmp )));
     fd_mod13b_nossn :=  __common__( (INTEGER)(point*(log(phat5/(1-phat5)) - log(odds))/log(2) + base));


   /* NoSSN-NoDOB Specific Code                               9-18-2008 PK */

     ver_mod_best_nossn_nodob :=  __common__( -6.56208862
     ...
     ver_mod_nossn_nodob_a :=  __common__( (exp(ver_mod_best_nossn_nodob )) / (1+exp(ver_mod_best_nossn_nodob )));



     ver_mod_notbest_nossn_nodob :=  __common__( -4.843506848
     ...
     ver_mod_nossn_nodob_b :=  __common__( (exp(ver_mod_notbest_nossn_nodob )) / (1+exp(ver_mod_notbest_nossn_nodob )));

		 ver_mod_nossn_nodob :=  __common__( if(add1_isbestmatch, ver_mod_nossn_nodob_a, ver_mod_nossn_nodob_b)); 
		 

     fp_mod_nossn_nodob_tmp :=  __common__( -4.978128798
                  + phnprob_tree_m  * 11.565293683
                  + addprob_mod  * 11.715693326
                  + derog_level2_m  * 13.783518279  );
									
     fp_mod_nossn_nodob :=  __common__( (exp(fp_mod_nossn_nodob_tmp )) / (1+exp(fp_mod_nossn_nodob_tmp )));


     fd_mod13b_nossn_nodob_tmp :=  __common__( -3.697873922
                  + rel_mod  * 6.136747898
     ...
     phat6 :=  __common__( (exp(fd_mod13b_nossn_nodob_tmp )) / (1+exp(fd_mod13b_nossn_nodob_tmp )));
     fd_mod13b_nossn_nodob :=  __common__( (INTEGER)(point*(log(phat6/(1-phat6)) - log(odds))/log(2) + base));



   /* NoPhnSSN Specific Code */

     combo_dob_flag_nophnssn_a :=  __common__( Map(combo_dobscore_c = 0 and  combo_dobcount_c = 0 => 0,
																				 combo_dobscore_c = 0 and  combo_dobcount_c   = 1 => 1,
																				 combo_dobscore_c = 0                             => 2,
																				 combo_dobscore_c = 1 and  combo_dobcount_c  <= 1 => 2,
																				 combo_dobscore_c = 1                             => 3,
     ...
     source_P_flag_nophnssn_a :=  __common__( Map(source_P_add = 1 and source_P_lst = 1 and source_P_fst = 1 => 2,
																				source_P_lst = 1 and source_P_fst = 1 => 1,
																				0));

     num_pos_src_NOEQ_add_c_nophnssn_a :=  __common__( 1 * num_pos_sources_NOEQ_add_c);

     source_EQ_flag_nophnssn_a :=  __common__( Map(source_EQ_add = 1 and source_EQ_lst = 1 and source_EQ_fst = 1 => 2,
																			 source_EQ_lst = 1 and source_EQ_fst = 1 => 1,
																			 0));

     num_pos_src_add_c_nophnssn_a :=  __common__( 1 * num_pos_sources_add_c);


     combo_dob_flag_nophnssn_b :=  __common__( Map(combo_dobscore_c = 0 and  combo_dobcount_c   = 0 => 10,
																				 combo_dobscore_c = 0                             => 11,
																				 combo_dobscore_c = 1 and  combo_dobcount_c  <= 2 => 12,
																				 combo_dobscore_c = 1                             => 13,
																				 -9999));

     source_P_flag_nophnssn_b :=  __common__( Map(source_P_add = 1 and source_P_lst = 1 and source_P_fst = 1 => 12,
																				source_P_lst = 1 and source_P_fst = 1 => 10,
																				11));

     num_pos_src_NOEQ_add_c_nophnssn_b :=  __common__( 1 * num_pos_sources_NOEQ_add_c + 10);

     source_EQ_flag_nophnssn_b :=  __common__( Map(source_EQ_add = 1 and source_EQ_lst = 1 and source_EQ_fst = 1 => 12,
																				 source_EQ_lst = 1 and source_EQ_fst = 1 => 11,
																				 10));

     num_pos_src_add_c_nophnssn_b :=  __common__( 1 * num_pos_sources_add_c + 10);

		 
		 combo_dob_flag_nophnssn :=  __common__( if(add1_isbestmatch, combo_dob_flag_nophnssn_a, combo_dob_flag_nophnssn_b));
		 source_P_flag_nophnssn :=  __common__( if(add1_isbestmatch, source_P_flag_nophnssn_a, source_P_flag_nophnssn_b));
		 source_EQ_flag_nophnssn :=  __common__( if(add1_isbestmatch, source_EQ_flag_nophnssn_a, source_EQ_flag_nophnssn_b));
		 num_pos_src_NOEQ_add_c_nophnssn :=  __common__( if(add1_isbestmatch, num_pos_src_NOEQ_add_c_nophnssn_a, num_pos_src_NOEQ_add_c_nophnssn_b));
		 num_pos_src_add_c_nophnssn :=  __common__( if(add1_isbestmatch, num_pos_src_add_c_nophnssn_a, num_pos_src_add_c_nophnssn_b));
		 
		 
     combo_dob_flag_nophnssn_m :=  __common__( Map(combo_dob_flag_nophnssn =  0 => 0.0829655781,
     ...
     source_p_flag_nophnssn_m :=  __common__( Map(source_p_flag_nophnssn =  0 => 0.0413430959,
     ...
     n_pos_src_noeq_add_c_nophnssn_m :=  __common__( Map(num_pos_src_noeq_add_c_nophnssn =  0 => 0.043516561,
     ...
     source_eq_flag_nophnssn_m :=  __common__( Map(source_eq_flag_nophnssn =  0 => 0.0501835985,
     ...
     num_pos_src_add_c_nophnssn_m :=  __common__( Map(num_pos_src_add_c_nophnssn =  0 => 0.0790697674,
     ...
     ver_mod_best_nophnssn :=  __common__( -4.783913239
     ...
     ver_mod_nophnssn_a :=  __common__( (exp(ver_mod_best_nophnssn )) / (1+exp(ver_mod_best_nophnssn )));

     ver_mod_notbest_nophnssn :=  __common__( -4.093440283
     ...
     ver_mod_nophnssn_b :=  __common__( (exp(ver_mod_notbest_nophnssn )) / (1+exp(ver_mod_notbest_nophnssn )));

		 ver_mod_nophnssn :=  __common__( if(add1_isbestmatch, ver_mod_nophnssn_a, ver_mod_nophnssn_b));
		 
		 

     fd_mod13b_NoPhnSSN_tmp :=  __common__( -3.01706852
                  + rel_mod  * 6.883229321
                  + prop_mod  * 3.7861659367
                  + Risk_Level_m  * 16.186071473
     ...
     phat7 :=  __common__( (exp(fd_mod13b_NoPhnSSN_tmp )) / (1+exp(fd_mod13b_NoPhnSSN_tmp )));
     fd_mod13b_NoPhnSSN :=  __common__( (INTEGER)(point*(log(phat7/(1-phat7)) - log(odds))/log(2) + base));



   /* NoPhnSSN_NoDob Specific Code */

      ver_mod_best_nophnssn_nodob :=  __common__( -5.822216599
      ...
      ver_mod3_nophnssn_nodob_a :=  __common__( (exp(ver_mod_best_nophnssn_nodob )) / (1+exp(ver_mod_best_nophnssn_nodob )));


      ver_mod_notbest_nophnssn_nodob :=  __common__( -3.547126855
     ...
     ver_mod3_nophnssn_nodob_b :=  __common__( (exp(ver_mod_notbest_nophnssn_nodob )) / (1+exp(ver_mod_notbest_nophnssn_nodob )));

		 ver_mod3_nophnssn_nodob :=  __common__( if(add1_isbestmatch, ver_mod3_nophnssn_nodob_a, ver_mod3_nophnssn_nodob_b));



     fp_mod_nophnssn_nodob_tmp :=  __common__( -4.480101051
                  + addprob_mod  * 14.200619431
                  + derog_level2_m  * 14.628264547 );
									
     fp_mod_nophnssn_nodob :=  __common__( (exp(fp_mod_nophnssn_nodob_tmp )) / (1+exp(fp_mod_nophnssn_nodob_tmp )));


     fd_mod13b_NoPhnSSN_NoDob_tmp :=  __common__( -2.854544073
     ...
     phat8 :=  __common__( (exp(fd_mod13b_NoPhnSSN_NoDob_tmp )) / (1+exp(fd_mod13b_NoPhnSSN_NoDob_tmp )));
     fd_mod13b_NoPhnSSN_NoDob :=  __common__( (INTEGER)(point*(log(phat8/(1-phat8)) - log(odds))/log(2) + base));


     ssnpop :=  __common__( ((integer)ssnlength>0));

     fdef710_a :=  __common__( Map(hphnpop = 0 and (integer)ssnpop = 0 => fd_mod13b_NoPhnSSN,
											 hphnpop = 0                => fd_mod13b_NoPhn,
											 (integer)ssnpop = 0 								=> fd_mod13b_NoSSN,
											 fd_mod13c));

     fdef710_b :=  __common__( Map(hphnpop = 0 and (integer)ssnpop = 0 => fd_mod13b_NoPhnSSN_NoDob,
												hphnpop = 0                	=> fd_mod13b_NoPhn_NoDob,
												(integer)ssnpop = 0 									=> fd_mod13b_NoSSN_NoDob,
												fd_mod13c_NoDob));

		 fdef710 :=  __common__( if(dobpop = 1, fdef710_a, fdef710_b));


/*   FP3710_a :=  __common__( fd_mod13b);                 out on 10-01-2008  PK   */
     FP3710_a :=  __common__( fdef710);            /*      Added 10-01-2008  PK   */
...
// ********************************************************************************************************		 
		...
		/* Overrides */
		usps_deliverable :=  __common__( (integer)(rc_addrvalflag='V'));
		
		ssnhighissue_yr :=  __common__( if((INTEGER)rc_ssnhighissue=0, -1, (INTEGER)rc_ssnhighissue[1..4]));
		dob_mismatch :=  __common__( map(
			ssnhighissue_yr = -1 or dob_yr = -1 => 0,
			dob_yr > ssnhighissue_yr and dobpop=1 => dob_yr - ssnhighissue_yr,
			0
		));

		verssn_s :=  __common__( (integer)(nas_summary in [4,6,7,9,10,11,12]));
		hr_phone :=  __common__( (integer)((integer)rc_hriskphoneflag=6));
		phn_corrections :=  __common__( (integer)((integer)rc_hrisksicphone=2225));
		not_deliverable :=  __common__( (integer)(usps_deliverable=0));
		add1_hr_address :=  __common__( (integer)(rc_hriskaddrflag=4));
		corrections :=  __common__( (integer)(rc_hrisksic=2225));
		ssn_valid :=  __common__( (integer)((INTEGER)rc_ssnvalflag in [0,3] or ((integer)rc_ssnvalflag=2 and (integer)ssnlength=4) ));
		watchlist_hit :=  __common__( (integer)(rc_watchlist_flag));
		dob_mismatch_flag :=  __common__( (integer)(dob_mismatch > 0));
	

    cap_ssnver2              :=  __common__( if( (integer)ssnpop =1 and (nas_ver = 0  ), 765, 999999));
    cap_ssnver3              :=  __common__( if( (integer)ssnpop =1 and (verssn_s = 0 ), 680, 999999));
    cap_disc                 :=  __common__( if( hphnpop=1 and (verphn_p = 0 and disconnected = 1 ), 680, 999999));
    cap_pnotpot              :=  __common__( if( hphnpop=1 and (verphn_p = 0 and pnotpot      = 1 ), 765, 999999));
    cap_phninval             :=  __common__( if( hphnpop=1 and (verphn_p = 0 and (phninval = 1 or phninval2 = 1)), 680, 999999));
    cap_hr_phn               :=  __common__( if( hphnpop=1 and (hr_phone = 1 or hr_phone_flag = 1 or phn_corrections = 1), 680, 999999));
    cap_busphone             :=  __common__( if( hphnpop=1 and (verphn_p = 0 and busphone_flag = 1 ), 765, 999999));
    cap_pzipmis1             :=  __common__( if( hphnpop=1 and (verphn_p = 0 and phone_zip_mismatch = 1), 680, 999999));
    cap_pzipmis2             :=  __common__( if( hphnpop=1 and (verphn_p = 1 and phone_zip_mismatch = 1), 765, 999999));
    cap_notdeliverable       :=  __common__( if( not_deliverable = 1, 765, 999999));
    cap_city_zip_mismatch    :=  __common__( if( city_zip_mismatch = 1, 765, 999999));
    cap_hr_addr              :=  __common__( if( add1_hr_address = 1 or hirisk_commercial_flag = 1, 765, 999999));
    cap_corrections          :=  __common__( if( corrections = 1, 680, 999999));
		
    cap_ssnprob              :=  __common__( if( (integer)ssnpop=1 and ((dobpop=1 and (deceased=1 or ssn_valid=0 or ssninval_pw=1 or ssnprior_pi=1))
																										or (dobpop=0 and (deceased=1 or ssn_valid=0 or ssninval_pw=1)) ), 610, 999999));
																										
    cap_velo_ssns_per_adl_c6 :=  __common__( if( (integer)ssnpop=1 and ssns_per_adl_c6 >= 2, 765, 999999));
    cap_velo_adls_per_addr   :=  __common__( if( aptflag = 0 and adls_per_addr >= 25, 765, 999999));
    cap_adlperssn            :=  __common__( if( (integer)ssnpop=1 and adlperssn_count >= 6, 680, 999999));
    cap_watchlist            :=  __common__( if( watchlist_hit = 1, 765, 999999));
    cap_dobmismatch          :=  __common__( if( dobpop=1 and dob_mismatch_flag = 1, 610, 999999));
		 

		nugen :=  __common__( true);
		ritmp :=  __common__( riskwise.bdReasonCodesConsumer2( le.bs, le.ip, num_reasons, nugen, criminal ));

				
		rc_IA :=  __common__( Risk_Indicators.rcSet.isCodeIA(le.bs.shell_input.ip_address, le.ip.ipaddr<>''));
		rc_IF :=  __common__( Risk_Indicators.rcSet.isCodeIF(le.ip.countrycode));
		rc_IE :=  __common__( Risk_Indicators.rcSet.isCodeIE(le.ip.ipaddr<>'', le.ip.secondleveldomain, le.ip.iptype));
		rc_IG :=  __common__( Risk_Indicators.rcSet.isCodeIG(le.ip.iptype));
		rc_II :=  __common__( Risk_Indicators.rcSet.isCodeII(le.bs.shell_input.in_state, le.ip.state, le.ip.countrycode, le.ip.ipaddr<>'', le.ip.secondleveldomain, le.ip.iptype));
		rc_IJ :=  __common__( Risk_Indicators.rcSet.isCodeIJ(le.bs.shell_input.in_zipCode, le.ip.zip, le.ip.countrycode, le.ip.ipaddr<>'', le.bs.shell_input.in_state, 
												le.ip.state, le.ip.secondleveldomain,le.ip.iptype, le.ip.areacode, le.bs.shell_input.phone10));
		rc_IK :=  __common__( Risk_Indicators.rcSet.isCodeIK(le.ip.areacode, le.bs.shell_input.phone10, le.bs.shell_input.in_state, le.ip.state, le.ip.countrycode, 
												le.ip.secondleveldomain, le.ip.ipaddr<>'',le.ip.iptype));
												
		
		IP_adj1 :=  __common__( if( ipaddrpop and ( rc_IA OR rc_IF ), 100, 0 ));
		IP_adj2 :=  __common__( if( ipaddrpop and ( rc_IE OR rc_IG OR rc_II OR rc_IJ OR rc_IK ), 20, 0 ));		
				
		cap_IP1 :=  __common__( if( IP_adj1 > 0, 610, 999 ));
		cap_IP2 :=  __common__( if( IP_adj2 > 0, 765, 999 ));
		
		fp3710_b :=  __common__( fp3710_a - ( ip_adj1 + ip_adj2 ));


		FP3710_c :=  __common__( min( FP3710_b,
			cap_ssnver2,
			cap_ssnver3,
			cap_disc,
			cap_pnotpot,
			...
			cap_watchlist,
			cap_dobmismatch,
			cap_IP1,                     /* IP Caps Added 5-23-2008   pk  */
			cap_IP2
		));

		FP3710 :=  __common__( map(
			FP3710_c < 300 => 300,
			FP3710_c > 999 => 999,
			FP3710_c
		));
		...
		reasons :=  __common__( Models.Common.checkFraudPointRC34(fp3710, ritmp, num_reasons));


		self.seq   :=   le.bs.seq;
		self.ri    :=   reasons;
		self.score :=  (string)fp3710;

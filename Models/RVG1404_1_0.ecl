...
iv_riskview_222s := (nas_summary <= 4 and nap_summary <= 4 and add1_naprop <= 3)
                     and not(if(max(property_owned_total, property_sold_total) = NULL, NULL, sum(if(property_owned_total = NULL, 0, property_owned_total), if(property_sold_total = NULL, 0, property_sold_total))) > 0 or 90 <= combo_dobscore AND combo_dobscore <= 100 or indexw(StringLib.StringToUpperCase(trim(ver_sources, ALL)), 'L2', ',') or indexw(StringLib.StringToUpperCase(trim(ver_sources, ALL)), 'LI', ',') or liens_recent_unreleased_count > 0 or liens_historical_unreleased_ct > 0 or criminal_count > 0 or (rc_bansflag in ['1', '2']) or indexw(StringLib.StringToUpperCase(trim(ver_sources, ALL)), 'BA', ',') or (Integer)bankrupt = 1 or filing_count > 0 or bk_recent_count > 0 or (Integer)rc_decsflag = 1 or indexw(StringLib.StringToUpperCase(trim(ver_sources, ALL)), 'DS', ',') or truedid);

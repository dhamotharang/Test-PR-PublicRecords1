...
Layout_ModelOut doModel(clam le) := TRANSFORM
	...
	// Above is aid606.0.0 code
	
	/****************************************************************************************
	* clone code starts here 
	****************************************************************************************/

	AID607 := 542.6721 + -1.087298 * fcra5 + 0.004030549 * fcra5 * fcra5 + -0.000002816005 * fcra5 * fcra5 * fcra5;

	AID607_1_0 := map(fcra5 < 350 => 535,
				   fcra5 > 800 => 811,
				   AID607);

	/****************************************************************************************
	* The following is driveTimeScore.1 override code.
	****************************************************************************************/

	aid := if(le.ssn_verification.validation.deceased and AID607_1_0 > 700, round(700 + (AID607_1_0 - 700)/5), AID607_1_0);

	aid2 := if(((~le.ssn_verification.validation.valid  and trim(le.shell_input.ssn) <> '') or ~le.ssn_verification.validation.issued) and aid > 700, round(700 + (aid - 700)/5), aid);

	aid3 := if(le.ssn_verification.validation.dob_mismatch > 0 and aid2 > 724, round(724 + (aid2 - 724)/5), aid2);

	aid4_pre_cap := (integer)aid3;
	
	// Score Cap change - Bug 36144
	aid4 := map(aid4_pre_cap < 250 => 250,
				   aid4_pre_cap > 999 => 999,
				   aid4_pre_cap);	
	
	self.ri := RiskWiseFCRA.corrReasonCodes(le.consumerflags, 4);
	self.score := if(self.ri[1].hri in ['91','92','93','94','95'], (string)((integer)self.ri[1].hri + 10), intformat(aid4,3,1));
	self.seq := le.seq;
END;

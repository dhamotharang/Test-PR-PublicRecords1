...
nf_seg_fraudpoint_3_0 := map(
  ...
  END;
	...
	fp3_wofdn_scaled := min(if(max(round(base + pts * (FP3_woFDN_LGT - offset - lgt) / ln(2)), 300) = NULL, -NULL, max(round(base + pts * (FP3_woFDN_LGT - offset - lgt) / ln(2)), 300)), 999);
  ...
  // _ver_src_cnt := Models.Common.countw((string)(ver_sources), ARRAY(0x2ef84c0));
	...
	// syntheticid := if(nap_summary <= 4 and nas_summary <= 4 or _ver_src_cnt = 0 or _bureauonly or ~add_curr_pop, 1, 0);	//kh-delta
	...
	// vulnerablevictim := if(0 < inferred_age AND inferred_age <= 17 or inferred_age >= 70, 1, 0);	//kh-delta
	...
	stolenc_fp31505_0_0 := if(stolenid = 1, fp31505_0_0, 299);

	manip2c_fp31505_0_0 := if((boolean)(integer)manipulatedid or (boolean)(integer)manipulatedidpt2, fp31505_0_0, 299);

	synthc_fp31505_0_0 := if(syntheticid = 1, fp31505_0_0, 299);

	suspactc_fp31505_0_0 := if(suspiciousactivity = 1, fp31505_0_0, 299);

	vulnvicc_fp31505_0_0 := if(vulnerablevictim = 1, fp31505_0_0, 299);

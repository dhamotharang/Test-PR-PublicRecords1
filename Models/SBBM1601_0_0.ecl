...
sysdate := common.sas_date(if(history_date=999999 or history_date = 0, (string)ut.getdate, (string6)history_date+'01'));

sysdate_days := common.sas_date(if((string)history_datetime[1..6]='999999' or history_datetime = 0, (string)ut.getdate, (string)history_datetime));
...
_ver_src_id__pl := _ver_src_id_pl_pos > 0;

_ver_src_id_q3_pos := Models.Common.findw_cpp(ver_src_id_list, 'Q3' , '  ,', 'ie');

_ver_src_id__q3 := _ver_src_id_q3_pos > 0;

_ver_src_id_fdate_q3 := if(_ver_src_id_q3_pos > 0, Models.Common.getw(ver_src_id_firstseen_list, _ver_src_id_q3_pos), '0');
...
_ver_src_id__cs := _ver_src_id_cs_pos > 0;

_ne_ver_src_id_count_1 := (integer)_ver_src_id__br +
...
_ne_ver_src_id_count := if((integer)ver_src_id_count = -1, -1, _ne_ver_src_id_count_1);

truebiz := id_seleID != '0' and NOT((_ne_ver_src_id_count in [-1, 0]) and (integer)sbfe_acct_cnt < 1);

ne_bv_truebiz_category := map(
...
vs_ver_src_id__in := (INTEGER)_ver_src_id__in;

vs_adl_util_i := if(util_adl_type_list = '', -1, (integer)(contains_i(util_adl_type_list, '1') > 0));

vs_util_adl_count := if(not(truedid), NULL, util_adl_count);

_util_adl_first_seen := common.sas_date((string)(util_adl_first_seen));

vs_util_adl_mths_fs := map(
    not(truedid)                => NULL,
...
vs_header_mths_fs := map(
    not(truedid)              => NULL,
    _header_first_seen = NULL => -1,
                                 (sysdate - _header_first_seen) / (365.25 / 12));

_pb_first_seen := common.sas_date((string)(pb_first_seen));
...
_pb_last_seen := common.sas_date((string)(pb_last_seen));
...
vs_pb_number_of_sources := map(
		not(truedid) 							=> -1, 
		pb_number_of_sources = '' => NULL,
																 // (INTEGER)pb_number_of_sources);
																 -1);  // reason code edit for iBehavior removal

_br_first_seen_1 := common.sas_date((string)(br_First_seen));
...
vs_br_business_count := if(not(truedid), -1, br_business_count);

vs_br_dead_business_count := if(not(truedid), -1, br_dead_business_count);

vs_br_active_phone_count := if(not(truedid), -1, br_active_phone_count);

vs_email_count := if(not(truedid), -1, email_count);

vs_email_domain_free_count := if(not(truedid), -1, email_domain_free_count);

vs_email_domain_edu_count := if(not(truedid), -1, email_domain_EDU_count);
...
_ver_src_cons_ldate_vo := common.sas_date((string)(ver_src_cons_ldate_vo));

vs_ver_src_cons_vo_mths_ls := map(
    not(truedid)                  => NULL,
    _ver_src_cons_ldate_vo = NULL => -1,
                                     (sysdate - _ver_src_cons_ldate_vo) / (365.25 / 12));

vs_ver_sources_wp_pop := if(not(truedid), NULL, (INTEGER)(ver_src_cons_wp_pos > 0));

vs_ver_sources_am_pop := if(not(truedid), NULL, (INTEGER)(ver_src_cons_am_pos > 0));

vs_ver_sources_e1_pop := if(not(truedid), NULL, (INTEGER)(ver_src_cons_e1_pos > 0));

c_vs_gb_id_mth_fseen_w := map(
...
c_vs_pb_number_of_sources_w := map(
...
c_vs_email_domain_free_count_w := map(
...
c_vs_ver_sources_am_pop_w := map(
    vs_ver_sources_am_pop = NULL => 0.00000,
    vs_ver_sources_am_pop <= 0.5 => 0.00790,
                                    -0.50074);

bv_bus_rep_source_profile := -2.39363071732448 +
...
r_vs_pb_mths_fs_w := map(
    vs_pb_mths_fs = NULL          => 0.00000,
    vs_pb_mths_fs = -1            => 0.23415,
    vs_pb_mths_fs <= 18.858316222 => 0.29443,
                                     -0.15106);

r_vs_pb_mths_ls_w := map(
    vs_pb_mths_ls = NULL           => 0.00000,
    vs_pb_mths_ls = -1             => 0.23415,
    vs_pb_mths_ls <= 0.0164271047  => -0.15414,
...
r_vs_pb_number_of_sources_w := map(
    vs_pb_number_of_sources = NULL => 0.00000,
    vs_pb_number_of_sources = -1   => 0.00000,
    vs_pb_number_of_sources <= 1.5 => 0.13516,
    vs_pb_number_of_sources <= 2.5 => -0.00289,
...
r_vs_br_dead_business_count_w := map(
    vs_br_dead_business_count = NULL => 0.00000,
    vs_br_dead_business_count = -1   => 0.00000,
    vs_br_dead_business_count <= 0.5 => -0.03651,
...
r_vs_br_active_phone_count_w := map(
    vs_br_active_phone_count = NULL => 0.00000,
    vs_br_active_phone_count = -1   => 0.00000,
    vs_br_active_phone_count <= 0.5 => 0.14873,
    vs_br_active_phone_count <= 1.5 => -0.14960,
...
r_vs_email_domain_free_count_w := map(
    vs_email_domain_free_count = NULL => 0.00000,
...
r_vs_email_domain_edu_count_w := map(
    vs_email_domain_edu_count = NULL => 0.00000,
    vs_email_domain_edu_count = -1   => 0.00000,
    vs_email_domain_edu_count <= 0.5 => 0.00745,
                                        -0.39425);

r_vs_historical_count_w := map(
...
r_vs_prof_license_flag_w := map(
    vs_prof_license_flag = NULL => 0.00000,
    vs_prof_license_flag = -1   => 0.00000,
    vs_prof_license_flag <= 0.5 => 0.05818,
                                   -0.38783);

r_vs_ver_src_cons_vo_mths_ls_w := map(
    vs_ver_src_cons_vo_mths_ls = NULL          => 0.00000,
    vs_ver_src_cons_vo_mths_ls = -1            => 0.01379,
    vs_ver_src_cons_vo_mths_ls <= 1.0020533881 => -0.24285,
...
iv_d34_liens_rel_sc_ct := if(not(truedid), NULL, min(if(liens_rel_SC_ct = NULL, -NULL, liens_rel_SC_ct), 999));
...
nf_fp_vardobcountnew := if(not(truedid), NULL, min(if((INTEGER)fp_vardobcountnew = NULL, -NULL, (INTEGER)fp_vardobcountnew), 999));
...
iv_d34_liens_unrel_cj_ct := if(not(truedid), NULL, min(if(liens_unrel_CJ_ct = NULL, -NULL, liens_unrel_CJ_ct), 999));
...
nf_accident_count := if(not(truedid), NULL, min(if(acc_count = NULL, -NULL, acc_count), 999));

nf_fp_srchunvrfdssncount := if(not(truedid), NULL, min(if((integer)fp_srchunvrfdssncount = NULL, -NULL, (integer)fp_srchunvrfdssncount), 999));

nf_fp_srchunvrfdaddrcount := if(not(truedid), NULL, min(if((integer)fp_srchunvrfdaddrcount = NULL, -NULL, (integer)fp_srchunvrfdaddrcount), 999));

nf_fp_curraddractivephonelist := map(
    not(truedid)                      => '',
    fp_curraddractivephonelist = ''   => '',
                                         trim(fp_curraddractivephonelist, LEFT));

td_sbfe_avg_util_pct06 := if(not(truebiz), NULL, (integer)sbfe_avg_util_pct06);

td_acct_dpd_1p_recency := map(
    not(truebiz)               => NULL,
...
_sbfe_acct_datefirstopen := common.sas_date((string)(sbfe_acct_datefirstopen));

td_mth_acct_firstopen := map(
    not(truebiz)                    => NULL,
...
td_sbfe_avg_util_pct_cc := if(not(truebiz), NULL, (integer)sbfe_avg_util_pct_cc);

td_sbfe_util_pct_cc := if(not(truebiz), NULL, (integer)sbfe_util_pct_cc);

_sos_inc_filing_firstseen := common.sas_date((string)(sos_inc_filing_firstseen));
...
num_neg_sources := if(max((integer)source_ba, (integer)source_l2, (integer)source_ut) = 0, 0, sum((integer)source_ba, (integer)source_l2, (integer)source_ut));
...
bv_mth_ver_src_q3_fs := if(not(_ver_src_id__q3), -1, mth__ver_src_id_fdate_q3);

_sbfe_datefirstseen := common.sas_date((string)(sbfe_datefirstseen));

td_mth_sbfe_datefirstseen := map(
    not(truebiz)               		=> NULL,
    (integer)sbfe_acct_cnt = -99  => -99,
...
_sbfe_dlq_datelastseen := common.sas_date((string)(sbfe_dlq_datelastseen));

td_mth_dlq_lastseen := map(
    not(truebiz)                  => NULL,
    (integer)sbfe_acct_cnt = -99  => -99,
...
_inq_lastseen := common.sas_date((string)(inq_lastseen));

bv_mths_inq_lastseen := map(
    not(truebiz)         => NULL,
...
_phn_input_firstseen_id := common.sas_date((string)(phn_input_firstseen_id));

bv_src_id_phn_mth_fs := map(
    not(truebiz)                                              								=> NULL,
...
bv_sos_defunct_status := map(
    not(truebiz)             				 	=> NULL,
    (integer)sos_inc_filing_count = 0 => -1,
																				 (integer)sos_ever_defunct);

bv_lien_filed_count := if(not(truebiz), NULL, (integer)lien_filed_count);

_ucc_lastseen := common.sas_date((string)(ucc_lastseen));
...
td_sbfe_worst_perf06 := if(not(truebiz), NULL, (integer)sbfe_worst_perf06);

td_sbfe_worst_perf60_loan := if(not(truebiz), NULL, (integer)sbfe_worst_perf60_loan);

td_sbfe_worst_perf12_oel := if(not(truebiz), NULL, (integer)sbfe_worst_perf12_oel);

td_acct_dpd_31p_recency_cc := map(
    not(truebiz)                   					=> NULL,
...
td_acct_dpd_91p_recency_lse := map(
    not(truebiz)                    				 => NULL,
...
td_sbfe_util_pct := if(not(truebiz), NULL, (integer)sbfe_util_pct);

td_sbfe_avg_util_pct24 := if(not(truebiz), NULL, (integer)sbfe_avg_util_pct24);

td_sbfe_avg_past_due_amt24 := if(not(truebiz), NULL, (integer)sbfe_avg_past_due_amt24);

td_sbfe_avg_past_due_amt := if(not(truebiz), NULL, (integer)sbfe_avg_past_due_amt);

td_sbfe_avg_util_pct12 := if(not(truebiz), NULL, (integer)sbfe_avg_util_pct12);

td_sbfe_worst_perf84 := if(not(truebiz), NULL, (integer)sbfe_worst_perf84);

bv_assoc_derog_flag := map(
    not(truebiz)    				 => NULL,
    (integer)assoc_count = 0 => -1,
...
s_aa_dist_1 := (0.00000 - s_bv_src_id_phn_mth_fs_w) * 0.23236;
...
s_aa_dist_2 := (0.00000 - s_bv_assoc_bk_tot_w) * 0.27649;
...
s_bv_prop_assessed_value_w := map(
...
s_aa_dist_6 := (0.00000 - s_bv_prop_assessed_value_w) * 0.74352;
...
s_td_mth_sbfe_datefirstseen_w := map(
...
s_aa_dist_7 := (0.00000 - s_td_mth_sbfe_datefirstseen_w) * 0.60977;
...
s_aa_dist_8 := (0.00000 - s_td_mth_acct_firstopen_w) * 0.72726;
...
s_aa_dist_9 := (0.00000 - s_td_mth_dlq_lastseen_w) * 0.23844;
...
s_aa_dist_10 := (0.00000 - s_td_acct_pct_closed_w) * 0.46319;
...
s_aa_dist_11 := (0.00000 - s_td_sbfe_worst_perf06_w) * 0.14616;
...
s_td_sbfe_worst_perf60_loan_w := map(
...
s_aa_dist_12 := (0.00000 - s_td_sbfe_worst_perf60_loan_w) * 0.47246;
...
s_td_sbfe_worst_perf12_oel_w := map(
...
s_aa_dist_13 := (0.00000 - s_td_sbfe_worst_perf12_oel_w) * 0.23160;
...
s_td_acct_dpd_31p_recency_cc_w := map(
...
s_aa_dist_14 := (0.00000 - s_td_acct_dpd_31p_recency_cc_w) * 0.20843;
...
s_td_acct_dpd_91p_recency_lse_w := map(
...
s_aa_dist_15 := (0.00000 - s_td_acct_dpd_91p_recency_lse_w) * 0.37346;
...
s_td_sbfe_closed_pct_invol_w := map(
...
s_aa_dist_16 := (0.00000 - s_td_sbfe_closed_pct_invol_w) * 0.43846;
...
s_aa_dist_17 := (0.00000 - s_td_sbfe_util_pct_w) * 0.13927;
...
s_aa_dist_18 := (0.00000 - s_td_sbfe_util_pct_cc_w) * 0.23574;
...
s_td_sbfe_avg_util_pct06_w := map(
...
s_aa_dist_19 := (0.00000 - s_td_sbfe_avg_util_pct06_w) * 0.20554;
...
s_td_sbfe_avg_util_pct24_w := map(
...
s_aa_dist_20 := (0.00000 - s_td_sbfe_avg_util_pct24_w) * 0.20150;
...
s_td_sbfe_avg_util_pct_cc_w := map(
...
s_aa_dist_21 := (0.00000 - s_td_sbfe_avg_util_pct_cc_w) * 0.13800;
...
s_td_sbfe_avg_past_due_amt24_w := map(
...
s_aa_dist_22 := (0.00000 - s_td_sbfe_avg_past_due_amt24_w) * 0.24473;

s_aa_code_22 := if((integer)sbfe_acct_dpd_91p_cnt24 > 0, 'BF142', 'BF141');

s_td_sbfe_avg_past_due_amt_w := map(
...
s_aa_dist_23 := (0.00000 - s_td_sbfe_avg_past_due_amt_w) * 0.20638;

s_aa_code_23 := if((integer)sbfe_acct_dpd_91p_cnt84 > 0, 'BF142', 'BF141');

s_bv_ver_src_derog_ratio_w := map(
...
s_aa_dist_24 := (0.00000 - s_bv_ver_src_derog_ratio_w) * 0.26224;
...
s_aa_dist_25 := (0.00000 - s_rv_d30_derog_count_w) * 0.28197;
...
s_rv_d32_mos_since_crim_ls_w := map(
...
s_aa_dist_26 := (0.00000 - s_rv_d32_mos_since_crim_ls_w) * 0.37003;
...
s_aa_dist_27 := (0.00000 - s_rv_d31_mostrec_bk_w) * 0.34996;
...
s_rv_a46_curr_avm_autoval_w := map(
...
s_aa_dist_28 := (0.00000 - s_rv_a46_curr_avm_autoval_w) * 0.68191;
...
s_rv_a49_curr_avm_chg_1yr_pct_w := map(
    rv_a49_curr_avm_chg_1yr_pct = NULL   => 0.00000,
    rv_a49_curr_avm_chg_1yr_pct <= 88.15 => 0.16262,
                                            -0.06477);

s_aa_dist_29 := (0.00000 - s_rv_a49_curr_avm_chg_1yr_pct_w) * 0.77100;
...
s_rv_a41_prop_owner_w := map(
    rv_a41_prop_owner = '' 	=> 0.00000,
    rv_a41_prop_owner = '0' => 0.74232,
                                -0.04203);

s_aa_dist_30 := (0.00000 - s_rv_a41_prop_owner_w) * 0.67200;
...
s_rv_e55_college_ind_w := map(
    rv_e55_college_ind = ''  => 0.00000,
    rv_e55_college_ind = '0' => 0.06440,
                                 -0.40475);

s_aa_dist_31 := (0.00000 - s_rv_e55_college_ind_w) * 0.68125;
...
s_rv_i60_inq_hiriskcred_rec_w := map(
    rv_i60_inq_hiriskcred_recency = NULL => 0.00000,
    rv_i60_inq_hiriskcred_recency <= 0.5 => -0.01554,
                                            0.96357);


s_aa_dist_32 := (0.00000 - s_rv_i60_inq_hiriskcred_rec_w) * 0.61014;
...
s_rv_c18_inv_addrs_per_adl_w := map(
    rv_c18_invalid_addrs_per_adl = NULL => 0.00000,
    rv_c18_invalid_addrs_per_adl <= 1.5 => -0.18219,
                                           0.09195);

s_aa_dist_33 := (0.00000 - s_rv_c18_inv_addrs_per_adl_w) * 0.85450;
...
s_rv_e58_br_dead_bus_count_w := map(
...
s_aa_dist_35 := (0.00000 - s_rv_e58_br_dead_bus_count_w) * 0.33511;
...
s_nf_inq_collection_count_w := map(
...
s_aa_dist_36 := (0.00000 - s_nf_inq_collection_count_w) * 0.30703;
...
s_aa_dist_37 := (0.00000 - s_nf_mos_acc_lseen_w) * 0.47679;
...
s_bv_bus_rep_source_profile_w := map(
...
s_aa_dist_38 := (0.00000 - s_bv_bus_rep_source_profile_w) * 0.57257;
...
s_bv_rep_only_source_profile_w := map(
...
s_aa_dist_39 := (0.00000 - s_bv_rep_only_source_profile_w) * 0.16965;
...
s_td_sbfe_avg_util_pct12_w := map(
...
s_aa_dist_40 := (0.00000 - s_td_sbfe_avg_util_pct12_w) * 0.09199;
...
s_td_acct_dpd_1p_recency_w := map(
...
s_aa_dist_41 := (0.00000 - s_td_acct_dpd_1p_recency_w) * 0.17668;
...
s_aa_dist_42 := (0.00000 - s_td_sbfe_worst_perf84_w) * 0.07518;
...
s_rcvalueb037 := (integer)(s_aa_code_1 = 'B037') * s_aa_dist_1 +
...
s_rcvaluep531 := (integer)(s_aa_code_1 = 'P531') * s_aa_dist_1 +
...
s_rcvaluep532 := (integer)(s_aa_code_1 = 'P532') * s_aa_dist_1 +
...
s_rcvalueb034 := (integer)(s_aa_code_1 = 'B034') * s_aa_dist_1 +
...
s_rcvaluep534 := (integer)(s_aa_code_1 = 'P534') * s_aa_dist_1 +
...
s_rcvaluep535 := (integer)(s_aa_code_1 = 'P535') * s_aa_dist_1 +
...
s_rcvalueb031 := (integer)(s_aa_code_1 = 'B031') * s_aa_dist_1 +
...
s_rcvaluep516 := (integer)(s_aa_code_1 = 'P516') * s_aa_dist_1 +
...
s_rcvaluep539 := (integer)(s_aa_code_1 = 'P539') * s_aa_dist_1 +
...
s_rcvalueb017 := (integer)(s_aa_code_1 = 'B017') * s_aa_dist_1 +
...
s_rcvaluep515 := (integer)(s_aa_code_1 = 'P515') * s_aa_dist_1 +
...
s_rcvaluep521 := (integer)(s_aa_code_1 = 'P521') * s_aa_dist_1 +
...
s_rcvaluebf142 := (integer)(s_aa_code_1 = 'BF142') * s_aa_dist_1 +
...
s_rcvaluep511 := (integer)(s_aa_code_1 = 'P511') * s_aa_dist_1 +
...
s_rcvaluebf113 := (integer)(s_aa_code_1 = 'BF113') * s_aa_dist_1 +
...
s_rcvaluebf134 := (integer)(s_aa_code_1 = 'BF134') * s_aa_dist_1 +
...
s_rcvaluebf110 := (integer)(s_aa_code_1 = 'BF110') * s_aa_dist_1 +
...
s_rcvaluebf117 := (integer)(s_aa_code_1 = 'BF117') * s_aa_dist_1 +
...
s_rcvaluebf133 := (integer)(s_aa_code_1 = 'BF133') * s_aa_dist_1 +
...
s_rcvaluebf136 := (integer)(s_aa_code_1 = 'BF136') * s_aa_dist_1 +
...
s_rcvaluep509 := (integer)(s_aa_code_1 = 'P509') * s_aa_dist_1 +
...
s_rcvaluebf141 := (integer)(s_aa_code_1 = 'BF141') * s_aa_dist_1 +
...
s_rcvalueb026 := (integer)(s_aa_code_1 = 'B026') * s_aa_dist_1 +
...
s_rcvaluebf143 := (integer)(s_aa_code_1 = 'BF143') * s_aa_dist_1 +
...
s_rcvaluep526 := (integer)(s_aa_code_1 = 'P526') * s_aa_dist_1 +
...
s_rcvalueb063 := (integer)(s_aa_code_1 = 'B063') * s_aa_dist_1 +
...
s_rcvalueb066 := (integer)(s_aa_code_1 = 'B066') * s_aa_dist_1 +
...
s_rcvaluebf126 := (integer)(s_aa_code_1 = 'BF126') * s_aa_dist_1 +
...
s_rcvaluebf127 := (integer)(s_aa_code_1 = 'BF127') * s_aa_dist_1 +
...
s_rcvaluebf125 := (integer)(s_aa_code_1 = 'BF125') * s_aa_dist_1 +
...
s_rcvaluebf120 := (integer)(s_aa_code_1 = 'BF120') * s_aa_dist_1 +
...
s_rcvaluebf105 := (integer)(s_aa_code_1 = 'BF105') * s_aa_dist_1 +
...
s_rcvaluebf106 := (integer)(s_aa_code_1 = 'BF106') * s_aa_dist_1 +
...
s_rcvaluebf128 := (integer)(s_aa_code_1 = 'BF128') * s_aa_dist_1 +
...
s_rcvaluebf129 := (integer)(s_aa_code_1 = 'BF129') * s_aa_dist_1 +
...
s_rcvaluebf135 := (integer)(s_aa_code_1 = 'BF135') * s_aa_dist_1 +
...
s_rcvaluep502 := (integer)(s_aa_code_1 = 'P502') * s_aa_dist_1 +
...
s_rcvalueb053 := (integer)(s_aa_code_1 = 'B053') * s_aa_dist_1 +
...
s_rcvalueb052 := (integer)(s_aa_code_1 = 'B052') * s_aa_dist_1 +
...
s_rcvalueb055 := (integer)(s_aa_code_1 = 'B055') * s_aa_dist_1 +
...
s_rcvalueb059 := (integer)(s_aa_code_1 = 'B059') * s_aa_dist_1 +
...
s_final_score := -2.76933408596469 +
...
l_aa_dist_1 := (0.00000 - l_bv_src_id_phn_mth_fs_w) * 0.39552;
...
l_bv_lien_judg_index_w := map(
    bv_lien_judg_index = NULL => 0.00000,
    bv_lien_judg_index <= 0   => -0.01731,
                                 1.07247);

l_aa_dist_3 := (0.00000 - l_bv_lien_judg_index_w) * 0.68527;
...
l_bv_prop_assessed_value_w := map(
    bv_prop_assessed_value = NULL    => 0.00000,
...
l_aa_dist_4 := (0.00000 - l_bv_prop_assessed_value_w) * 0.45444;
...
l_aa_dist_6 := (0.00000 - l_bv_mth_ver_src_p_ls_w) * 0.43513;
...
l_aa_dist_7 := (0.00000 - l_bv_mth_ver_src_q3_fs_w) * 0.21837;
...
l_aa_dist_8 := (0.00000 - l_rv_d30_derog_count_w) * 0.21478;
...
l_rv_d31_bk_chapter_w := map(
    rv_d31_bk_chapter = '' 						=> 0.00000,
    (integer)rv_d31_bk_chapter <= 3.5 => -0.08570,
																				 0.99863);

l_aa_dist_9 := (0.00000 - l_rv_d31_bk_chapter_w) * 0.49892;
...
l_aa_dist_10 := (0.00000 - l_rv_d34_unrel_liens_ct_w) * 0.27430;
...
l_rv_a46_curr_avm_autoval_w := map(
    rv_a46_curr_avm_autoval = NULL    => 0.00000,
...
l_aa_dist_11 := (0.00000 - l_rv_a46_curr_avm_autoval_w) * 0.80079;
...
l_rv_a41_a42_prop_owner_hist_w := map(
...
l_aa_dist_12 := (0.00000 - l_rv_a41_a42_prop_owner_hist_w) * 0.30565;
...
l_rv_e55_college_ind_w := map(
    rv_e55_college_ind = '' => 0.00000,
    rv_e55_college_ind = '0' => 0.06972,
                                 -0.33938);

l_aa_dist_13 := (0.00000 - l_rv_e55_college_ind_w) * 0.90180;
...
l_rv_i61_inq_collection_rec_w := map(
...
l_aa_dist_14 := (0.00000 - l_rv_i61_inq_collection_rec_w) * 0.48060;
...
l_rv_i60_inq_auto_recency_w := map(
    rv_i60_inq_auto_recency = NULL => 0.00000,
    rv_i60_inq_auto_recency <= 0.5 => -0.07858,
                                      0.65157);

l_aa_dist_15 := (0.00000 - l_rv_i60_inq_auto_recency_w) * 0.43425;
...
l_rv_d34_attr_liens_recency_w := map(
    rv_d34_attr_liens_recency = NULL  => 0.00000,
...
l_aa_dist_16 := (0.00000 - l_rv_d34_attr_liens_recency_w) * 0.20213;
...
l_rv_mos_since_br_first_seen_w := map(
...
l_aa_dist_17 := (0.00000 - l_rv_mos_since_br_first_seen_w) * 0.33850;
...
l_iv_d34_liens_rel_sc_ct_w := map(
    iv_d34_liens_rel_sc_ct = NULL => 0.00000,
    iv_d34_liens_rel_sc_ct <= 0.5 => -0.01219,
                                     0.91531);

l_aa_dist_18 := (0.00000 - l_iv_d34_liens_rel_sc_ct_w) * 0.47831;
...
l_nf_inq_highriskcredit_count_w := map(
    nf_inq_highriskcredit_count = NULL => 0.00000,
    nf_inq_highriskcredit_count <= 0.5 => -0.02564,
                                          1.20086);

l_aa_dist_19 := (0.00000 - l_nf_inq_highriskcredit_count_w) * 0.42295;
...
l_nf_mos_inq_banko_cm_fseen_w := map(
    nf_mos_inq_banko_cm_fseen = NULL => 0.00000,
    nf_mos_inq_banko_cm_fseen = -1   => -0.14661,
                                        0.79274);

l_aa_dist_20 := (0.00000 - l_nf_mos_inq_banko_cm_fseen_w) * 0.44918;
...
l_nf_mos_acc_lseen_w := map(
    nf_mos_acc_lseen = NULL  => 0.00000,
    nf_mos_acc_lseen = -1    => -0.05444,
    nf_mos_acc_lseen <= 52.5 => 0.42884,
                                0.16057);

l_aa_dist_21 := (0.00000 - l_nf_mos_acc_lseen_w) * 0.48429;
...
l_nf_fp_vardobcountnew_w := map(
    nf_fp_vardobcountnew = NULL => 0.00000,
    nf_fp_vardobcountnew <= 0.5 => -0.10184,
                                   0.50518);

l_aa_dist_22 := (0.00000 - l_nf_fp_vardobcountnew_w) * 0.37684;
...
l_nf_fp_srchfraudsrchcountmo_w := map(
    nf_fp_srchfraudsrchcountmo = NULL => 0.00000,
    nf_fp_srchfraudsrchcountmo <= 0.5 => -0.04952,
                                         0.66454);

l_aa_dist_23 := (0.00000 - l_nf_fp_srchfraudsrchcountmo_w) * 0.46921;
...
l_aa_dist_24 := (0.00000 - l_nf_fp_prevaddrstatus_w) * 0.23004;
...
l_bv_bus_rep_source_profile_w := map(
    bv_bus_rep_source_profile = NULL           => 0.00000,
...
l_aa_dist_25 := (0.00000 - l_bv_bus_rep_source_profile_w) * 0.35584;
...
l_bv_rep_only_source_profile_w := map(
    bv_rep_only_source_profile = NULL           => 0.00000,
...
l_aa_dist_26 := (0.00000 - l_bv_rep_only_source_profile_w) * 0.29460;
...
l_nf_m_bureau_adl_fs_notu_w := map(
...
l_aa_dist_28 := (0.00000 - l_nf_m_bureau_adl_fs_notu_w) * 0.13872;
...
l_rcvalueb037 := (integer)(l_aa_code_1 = 'B037') * l_aa_dist_1 +
...
l_rcvaluep531 := (integer)(l_aa_code_1 = 'P531') * l_aa_dist_1 +
...
l_rcvaluep532 := (integer)(l_aa_code_1 = 'P532') * l_aa_dist_1 +
...
l_rcvalueb034 := (integer)(l_aa_code_1 = 'B034') * l_aa_dist_1 +
...
l_rcvaluep534 := (integer)(l_aa_code_1 = 'P534') * l_aa_dist_1 +
...
l_rcvaluep535 := (integer)(l_aa_code_1 = 'P535') * l_aa_dist_1 +
...
l_rcvalueb031 := (integer)(l_aa_code_1 = 'B031') * l_aa_dist_1 +
...
l_rcvalueb015 := (integer)(l_aa_code_1 = 'B015') * l_aa_dist_1 +
...
l_rcvaluep539 := (integer)(l_aa_code_1 = 'P539') * l_aa_dist_1 +
...
l_rcvalueb017 := (integer)(l_aa_code_1 = 'B017') * l_aa_dist_1 +
...
l_rcvaluep515 := (integer)(l_aa_code_1 = 'P515') * l_aa_dist_1 +
...
l_rcvalueb038 := (integer)(l_aa_code_1 = 'B038') * l_aa_dist_1 +
...
l_rcvalueb040 := (integer)(l_aa_code_1 = 'B040') * l_aa_dist_1 +
...
l_rcvaluep509 := (integer)(l_aa_code_1 = 'P509') * l_aa_dist_1 +
...
l_rcvalueb026 := (integer)(l_aa_code_1 = 'B026') * l_aa_dist_1 +
...
l_rcvaluep526 := (integer)(l_aa_code_1 = 'P526') * l_aa_dist_1 +
...
l_rcvalueb063 := (integer)(l_aa_code_1 = 'B063') * l_aa_dist_1 +
...
l_rcvaluep506 := (integer)(l_aa_code_1 = 'P506') * l_aa_dist_1 +
...
l_rcvaluebf108 := (integer)(l_aa_code_1 = 'BF108') * l_aa_dist_1 +
...
l_rcvaluep562 := (integer)(l_aa_code_1 = 'P562') * l_aa_dist_1 +
...
l_rcvalueb053 := (integer)(l_aa_code_1 = 'B053') * l_aa_dist_1 +
...
l_rcvalueb052 := (integer)(l_aa_code_1 = 'B052') * l_aa_dist_1 +
...
l_rcvalueb055 := (integer)(l_aa_code_1 = 'B055') * l_aa_dist_1 +
...
l_rcvalueb057 := (integer)(l_aa_code_1 = 'B057') * l_aa_dist_1 +
...
l_rcvaluep523 := (integer)(l_aa_code_1 = 'P523') * l_aa_dist_1 +
...
l_final_score := -2.35817084802197 +
...
u_aa_dist_1 := (0.00000 - u_rv_d30_derog_count_w) * 0.42452;
...
u_rv_d31_attr_bk_recency_w := map(
    rv_d31_attr_bankruptcy_recency = NULL => 0.00000,
    rv_d31_attr_bankruptcy_recency <= 6   => -0.06563,
                                             0.75590);

u_aa_dist_2 := (0.00000 - u_rv_d31_attr_bk_recency_w) * 0.23212;
...
u_rv_d31_bk_disposed_hist_cnt_w := map(
    rv_d31_bk_disposed_hist_count = NULL => 0.00000,
    rv_d31_bk_disposed_hist_count <= 0.5 => -0.06479,
                                            0.75860);

u_aa_dist_3 := (0.00000 - u_rv_d31_bk_disposed_hist_cnt_w) * 0.26427;
...
u_rv_c21_stl_inq_count_w := map(
    rv_c21_stl_inq_count = NULL => 0.00000,
    rv_c21_stl_inq_count <= 0.5 => -0.01950,
                                   0.63014);

u_aa_dist_4 := (0.00000 - u_rv_c21_stl_inq_count_w) * 0.75217;
...
u_rv_d34_unrel_lien60_count_w := map(
    rv_d34_unrel_lien60_count = NULL => 0.00000,
    rv_d34_unrel_lien60_count <= 0.5 => -0.04327,
                                        1.03906);

u_aa_dist_6 := (0.00000 - u_rv_d34_unrel_lien60_count_w) * 0.16260;
...
u_rv_c20_m_bureau_adl_fs_w := map(
...
u_aa_dist_7 := (0.00000 - u_rv_c20_m_bureau_adl_fs_w) * 0.17861;
...
u_rv_a46_curr_avm_autoval_w := map(
    rv_a46_curr_avm_autoval = NULL    => 0.00000,
...
u_aa_dist_8 := (0.00000 - u_rv_a46_curr_avm_autoval_w) * 0.69358;
...
u_rv_a49_curr_avm_chg_1yr_pct_w := map(
    rv_a49_curr_avm_chg_1yr_pct = NULL   => 0.00000,
    rv_a49_curr_avm_chg_1yr_pct <= 66.35 => 0.58537,
    rv_a49_curr_avm_chg_1yr_pct <= 75.55 => 0.35086,
                                            -0.04787);

u_aa_dist_9 := (0.00000 - u_rv_a49_curr_avm_chg_1yr_pct_w) * 0.37899;
...
u_rv_a41_prop_owner_w := map(
    rv_a41_prop_owner = ''  => 0.00000,
    rv_a41_prop_owner = '0' => 0.61707,
                                -0.08257);

u_aa_dist_10 := (0.00000 - u_rv_a41_prop_owner_w) * 0.38671;
...
u_rv_e55_college_ind_w := map(
    rv_e55_college_ind = ''  => 0.00000,
    rv_e55_college_ind = '0' => 0.06624,
                                 -0.28886);

u_aa_dist_11 := (0.00000 - u_rv_e55_college_ind_w) * 0.82870;
...
u_rv_i61_inq_collection_cnt12_w := map(
...
u_aa_dist_12 := (0.00000 - u_rv_i61_inq_collection_cnt12_w) * 0.23951;
...
u_rv_l79_adls_per_addr_curr_w := map(
    rv_l79_adls_per_addr_curr = NULL => 0.00000,
    rv_l79_adls_per_addr_curr <= 6.5 => -0.01000,
                                        0.29661);

u_aa_dist_13 := (0.00000 - u_rv_l79_adls_per_addr_curr_w) * 0.49411;
...
u_rv_d34_attr_liens_recency_w := map(
    rv_d34_attr_liens_recency = NULL  => 0.00000,
    rv_d34_attr_liens_recency <= 0.5  => -0.13409,
    rv_d34_attr_liens_recency <= 79.5 => 1.01299,
                                         0.52108);

u_aa_dist_14 := (0.00000 - u_rv_d34_attr_liens_recency_w) * 0.34744;
...
u_rv_a50_pb_average_dollars_w := map(
...
u_aa_dist_15 := if(rv_a50_pb_average_dollars=null, ((0.00000 - u_rv_a50_pb_average_dollars_w) * 0.20553), 999999);  // reason code edit for iBehavior removal
...
u_aa_dist_16 := (0.00000 - u_iv_c13_avg_lres_w) * 0.22932;
...
u_rv_i60_inq_other_recency_w := map(
    rv_i60_inq_other_recency = NULL => 0.00000,
    rv_i60_inq_other_recency <= 18  => -0.03836,
                                       0.29330);

u_aa_dist_17 := (0.00000 - u_rv_i60_inq_other_recency_w) * 0.43471;
...
u_rv_mos_since_br_first_seen_w := map(
    rv_mos_since_br_first_seen = NULL => 0.00000,
    rv_mos_since_br_first_seen = -1   => 0.08255,
    rv_mos_since_br_first_seen <= 76  => -0.04609,
                                         -0.18988);

u_aa_dist_18 := (0.00000 - u_rv_mos_since_br_first_seen_w) * 0.37300;
...
u_iv_d34_liens_unrel_sc_ct_w := map(
    iv_d34_liens_unrel_sc_ct = NULL => 0.00000,
    iv_d34_liens_unrel_sc_ct <= 0.5 => -0.03200,
                                       0.94892);

u_aa_dist_20 := (0.00000 - u_iv_d34_liens_unrel_sc_ct_w) * 0.28703;
...
u_nf_inq_collection_count_w := map(
    nf_inq_collection_count = NULL => 0.00000,
...
u_aa_dist_22 := (0.00000 - u_nf_inq_collection_count_w) * 0.51163;
...
u_nf_inq_communications_cnt24_w := map(
    nf_inq_communications_count24 = NULL => 0.00000,
    nf_inq_communications_count24 <= 0.5 => -0.03134,
                                            0.76570);

u_aa_dist_23 := (0.00000 - u_nf_inq_communications_cnt24_w) * 0.40601;
...
u_nf_inq_highriskcredit_count_w := map(
    nf_inq_highriskcredit_count = NULL => 0.00000,
    nf_inq_highriskcredit_count <= 0.5 => -0.02397,
                                          1.03713);

u_aa_dist_24 := (0.00000 - u_nf_inq_highriskcredit_count_w) * 0.34206;
...
u_nf_mos_inq_banko_cm_fseen_w := map(
    nf_mos_inq_banko_cm_fseen = NULL => 0.00000,
    nf_mos_inq_banko_cm_fseen = -1   => -0.10757,
                                        0.65863);

u_aa_dist_25 := (0.00000 - u_nf_mos_inq_banko_cm_fseen_w) * 0.44534;
...
u_aa_dist_27 := (0.00000 - u_nf_accident_count_w) * 0.59079;
...
u_nf_fp_vardobcountnew_w := map(
    nf_fp_vardobcountnew = NULL => 0.00000,
    nf_fp_vardobcountnew <= 0.5 => -0.08075,
                                   0.33555);

u_aa_dist_28 := (0.00000 - u_nf_fp_vardobcountnew_w) * 0.28112;
...
u_nf_fp_srchunvrfdssncount_w := map(
    nf_fp_srchunvrfdssncount = NULL => 0.00000,
    nf_fp_srchunvrfdssncount <= 0.5 => -0.02241,
                                       0.51171);

u_aa_dist_29 := (0.00000 - u_nf_fp_srchunvrfdssncount_w) * 0.47837;
...
u_nf_fp_srchunvrfdaddrcount_w := map(
    nf_fp_srchunvrfdaddrcount = NULL => 0.00000,
    nf_fp_srchunvrfdaddrcount <= 0.5 => -0.02488,
                                        0.78991);

u_aa_dist_30 := (0.00000 - u_nf_fp_srchunvrfdaddrcount_w) * 0.58133;
...
u_nf_fp_curraddractivephnlist_w := map(
...
u_aa_dist_31 := (0.00000 - u_nf_fp_curraddractivephnlist_w) * 0.42890;
...
u_bv_rep_only_source_profile_w := map(
    bv_rep_only_source_profile = NULL           => 0.00000,
...
u_aa_dist_32 := (0.00000 - u_bv_rep_only_source_profile_w) * 0.63013;
...
u_nf_m_bureau_adl_fs_notu_w := map(
    nf_m_bureau_adl_fs_notu = NULL   => 0.00000,
    nf_m_bureau_adl_fs_notu = -1     => 0.00000,
...
u_aa_dist_33 := (0.00000 - u_nf_m_bureau_adl_fs_notu_w) * 0.30365;
...
u_rcvaluep523 := (integer)(u_aa_code_1 = 'P523') * u_aa_dist_1 +
...
u_rcvaluep509 := (integer)(u_aa_code_1 = 'P509') * u_aa_dist_1 +
...
u_rcvaluep532 := (integer)(u_aa_code_1 = 'P532') * u_aa_dist_1 +
...
u_rcvaluep502 := (integer)(u_aa_code_1 = 'P502') * u_aa_dist_1 +
...
u_rcvaluep534 := (integer)(u_aa_code_1 = 'P534') * u_aa_dist_1 +
...
u_rcvaluep535 := (integer)(u_aa_code_1 = 'P535') * u_aa_dist_1 +
...
u_rcvaluep531 := (integer)(u_aa_code_1 = 'P531') * u_aa_dist_1 +
...
u_rcvaluep524 := (integer)(u_aa_code_1 = 'P524') * u_aa_dist_1 +
...
u_rcvaluep516 := (integer)(u_aa_code_1 = 'P516') * u_aa_dist_1 +
...
u_rcvaluep539 := (integer)(u_aa_code_1 = 'P539') * u_aa_dist_1 +
...
u_rcvaluep515 := (integer)(u_aa_code_1 = 'P515') * u_aa_dist_1 +
...
u_rcvaluep512 := (integer)(u_aa_code_1 = 'P512') * u_aa_dist_1 +
...
u_rcvaluep511 := (integer)(u_aa_code_1 = 'P511') * u_aa_dist_1 +
...
u_rcvaluep538 := (integer)(u_aa_code_1 = 'P538') * u_aa_dist_1 +
...
u_rcvaluep550 := (integer)(u_aa_code_1 = 'P550') * u_aa_dist_1 +
...
u_rcvalueb067 := (integer)(u_aa_code_1 = 'B067') * u_aa_dist_1 +
...
u_rcvaluep549 := (integer)(u_aa_code_1 = 'P549') * u_aa_dist_1 +
...
u_rcvaluep562 := (integer)(u_aa_code_1 = 'P562') * u_aa_dist_1 +
...
u_rcvaluep526 := (integer)(u_aa_code_1 = 'P526') * u_aa_dist_1 +
...
u_final_score := -2.11317180468609 +
...
_sbbm1601_0_0 := round(base + pts * (blend_mod_final_lgt - lgt) / ln(2));
...
//*************************************************************************************//
rc_dataset_l_sorted := sort(rc_dataset_l, value, rc);

l_rc1  := rc_dataset_l_sorted[1].rc;
l_rc2  := rc_dataset_l_sorted[2].rc;
l_rc3  := rc_dataset_l_sorted[3].rc;

do_new_l_sort := l_rc1[1] <> 'B' and l_rc2[1] <> 'B' and l_rc3[1] <> 'B';
rc_business_l := sort((rc_dataset_l_sorted(rc[1] = 'B')), value)[1];

// Use null ds to pad the new sort in order to keep the indexing between the original and new sorts the same.
rc_dataset_l_sorted_new := if(do_new_l_sort, SORT(Null_ds +  rc_dataset_l_sorted[4..], value, rc)(rc <> rc_business_l.rc), rc_dataset_l_sorted);

l_rc4  := IF(do_new_l_sort, rc_business_l.rc, rc_dataset_l_sorted_new[4].rc);
l_rc5  := rc_dataset_l_sorted_new[5].rc;
l_rc6  := rc_dataset_l_sorted_new[6].rc;
l_rc7  := rc_dataset_l_sorted_new[7].rc;
l_rc8  := rc_dataset_l_sorted_new[8].rc;
l_rc9  := rc_dataset_l_sorted_new[9].rc;
l_rc10  := rc_dataset_l_sorted_new[10].rc;
l_rc11  := rc_dataset_l_sorted_new[11].rc;
l_rc12  := rc_dataset_l_sorted_new[12].rc;
l_rc13  := rc_dataset_l_sorted_new[13].rc;
l_rc14  := rc_dataset_l_sorted_new[14].rc;
l_rc15  := rc_dataset_l_sorted_new[15].rc;
l_rc16  := rc_dataset_l_sorted_new[16].rc;
l_rc17  := rc_dataset_l_sorted_new[17].rc;
l_rc18  := rc_dataset_l_sorted_new[18].rc;
l_rc19  := rc_dataset_l_sorted_new[19].rc;
l_rc20  := rc_dataset_l_sorted_new[20].rc;
l_rc21  := rc_dataset_l_sorted_new[21].rc;
l_rc22  := rc_dataset_l_sorted_new[22].rc;
l_rc23  := rc_dataset_l_sorted_new[23].rc;
l_rc24  := rc_dataset_l_sorted_new[24].rc;
l_rc25  := rc_dataset_l_sorted_new[25].rc;
l_rc26  := rc_dataset_l_sorted_new[26].rc;
l_rc27  := rc_dataset_l_sorted_new[27].rc;
l_rc28  := rc_dataset_l_sorted_new[28].rc;
l_rc29  := rc_dataset_l_sorted_new[29].rc;
l_rc30  := rc_dataset_l_sorted_new[30].rc;
l_rc31  := rc_dataset_l_sorted_new[31].rc;
l_rc32  := rc_dataset_l_sorted_new[32].rc;
l_rc33  := rc_dataset_l_sorted_new[33].rc;
l_rc34  := rc_dataset_l_sorted_new[34].rc;
l_rc35  := rc_dataset_l_sorted_new[35].rc;
l_rc36  := rc_dataset_l_sorted_new[36].rc;
l_rc37  := rc_dataset_l_sorted_new[37].rc;
l_rc38  := rc_dataset_l_sorted_new[38].rc;
l_rc39  := rc_dataset_l_sorted_new[39].rc;
l_rc40  := rc_dataset_l_sorted_new[40].rc;
l_rc41  := rc_dataset_l_sorted_new[41].rc;
l_rc42  := rc_dataset_l_sorted_new[42].rc;
l_rc43  := rc_dataset_l_sorted_new[43].rc;
l_rc44  := rc_dataset_l_sorted_new[44].rc;
l_rc45  := rc_dataset_l_sorted_new[45].rc;
l_rc46  := rc_dataset_l_sorted_new[46].rc;
l_rc47  := rc_dataset_l_sorted_new[47].rc;
l_rc48  := rc_dataset_l_sorted_new[48].rc;
l_rc49  := rc_dataset_l_sorted_new[49].rc;
l_rc50  := rc_dataset_l_sorted_new[50].rc;
 
//*************************************************************************************//
s_code_set := [s_aa_code_1,
							 s_aa_code_2,
							 s_aa_code_3,
							 s_aa_code_4,
							 s_aa_code_5,
...
// one position to make room for that reason code.  
//*************************************************************************************//
rc_dataset_s_sorted := sort(rc_dataset_s, value, rc);

s_rc1  := rc_dataset_s_sorted[1].rc;
s_rc2  := rc_dataset_s_sorted[2].rc;
s_rc3  := rc_dataset_s_sorted[3].rc;

do_new_s_sort := s_rc1[1] <> 'B' and s_rc2[1] <> 'B' and s_rc3[1] <> 'B';
rc_business_s := sort((rc_dataset_s_sorted(rc[1] = 'B')), value)[1];

// Use null ds to pad the new sort in order to keep the indexing between the original and new sorts the same.
rc_dataset_s_sorted_new := if(do_new_s_sort, SORT(Null_ds +  rc_dataset_s_sorted[4..], value, rc)(rc <> rc_business_s.rc), rc_dataset_s_sorted);

s_rc4  := IF(do_new_s_sort, rc_business_s.rc, rc_dataset_s_sorted_new[4].rc);
s_rc5  := rc_dataset_s_sorted_new[5].rc;
s_rc6  := rc_dataset_s_sorted_new[6].rc;
s_rc7  := rc_dataset_s_sorted_new[7].rc;
s_rc8  := rc_dataset_s_sorted_new[8].rc;
s_rc9  := rc_dataset_s_sorted_new[9].rc;
s_rc10  := rc_dataset_s_sorted_new[10].rc;
s_rc11  := rc_dataset_s_sorted_new[11].rc;
s_rc12  := rc_dataset_s_sorted_new[12].rc;
s_rc13  := rc_dataset_s_sorted_new[13].rc;
s_rc14  := rc_dataset_s_sorted_new[14].rc;
s_rc15  := rc_dataset_s_sorted_new[15].rc;
s_rc16  := rc_dataset_s_sorted_new[16].rc;
s_rc17  := rc_dataset_s_sorted_new[17].rc;
s_rc18  := rc_dataset_s_sorted_new[18].rc;
s_rc19  := rc_dataset_s_sorted_new[19].rc;
s_rc20  := rc_dataset_s_sorted_new[20].rc;
s_rc21  := rc_dataset_s_sorted_new[21].rc;
s_rc22  := rc_dataset_s_sorted_new[22].rc;
s_rc23  := rc_dataset_s_sorted_new[23].rc;
s_rc24  := rc_dataset_s_sorted_new[24].rc;
s_rc25  := rc_dataset_s_sorted_new[25].rc;
s_rc26  := rc_dataset_s_sorted_new[26].rc;
s_rc27  := rc_dataset_s_sorted_new[27].rc;
s_rc28  := rc_dataset_s_sorted_new[28].rc;
s_rc29  := rc_dataset_s_sorted_new[29].rc;
s_rc30  := rc_dataset_s_sorted_new[30].rc;
s_rc31  := rc_dataset_s_sorted_new[31].rc;
s_rc32  := rc_dataset_s_sorted_new[32].rc;
s_rc33  := rc_dataset_s_sorted_new[33].rc;
s_rc34  := rc_dataset_s_sorted_new[34].rc;
s_rc35  := rc_dataset_s_sorted_new[35].rc;
s_rc36  := rc_dataset_s_sorted_new[36].rc;
s_rc37  := rc_dataset_s_sorted_new[37].rc;
s_rc38  := rc_dataset_s_sorted_new[38].rc;
s_rc39  := rc_dataset_s_sorted_new[39].rc;
s_rc40  := rc_dataset_s_sorted_new[40].rc;
s_rc41  := rc_dataset_s_sorted_new[41].rc;
s_rc42  := rc_dataset_s_sorted_new[42].rc;
s_rc43  := rc_dataset_s_sorted_new[43].rc;
s_rc44  := rc_dataset_s_sorted_new[44].rc;
s_rc45  := rc_dataset_s_sorted_new[45].rc;
s_rc46  := rc_dataset_s_sorted_new[46].rc;
s_rc47  := rc_dataset_s_sorted_new[47].rc;
s_rc48  := rc_dataset_s_sorted_new[48].rc;
s_rc49  := rc_dataset_s_sorted_new[49].rc;
s_rc50  := rc_dataset_s_sorted_new[50].rc;
 
//*************************************************************************************//
u_code_set := [u_aa_code_1,
							 u_aa_code_2,
							 u_aa_code_3,
							 u_aa_code_4,
							 u_aa_code_5,
...
// one position to make room for that reason code.  
//*************************************************************************************//
rc_dataset_u_sorted := sort(rc_dataset_u, value, rc);

u_rc1  := rc_dataset_u_sorted[1].rc;
u_rc2  := rc_dataset_u_sorted[2].rc;
u_rc3  := rc_dataset_u_sorted[3].rc;

do_new_u_sort := u_rc1[1] <> 'B' and u_rc2[1] <> 'B' and u_rc3[1] <> 'B';
rc_business_u := sort((rc_dataset_u_sorted(rc[1] = 'B')), value)[1];

// Use null ds to pad the new sort in order to keep the indexing between the original and new sorts the same.
rc_dataset_u_sorted_new := if(do_new_u_sort, SORT(Null_ds +  rc_dataset_u_sorted[4..], value, rc)(rc <> rc_business_u.rc), rc_dataset_u_sorted);

u_rc4  := IF(do_new_u_sort, rc_business_u.rc, rc_dataset_u_sorted_new[4].rc);
u_rc5  := rc_dataset_u_sorted_new[5].rc;
u_rc6  := rc_dataset_u_sorted_new[6].rc;
u_rc7  := rc_dataset_u_sorted_new[7].rc;
u_rc8  := rc_dataset_u_sorted_new[8].rc;
u_rc9  := rc_dataset_u_sorted_new[9].rc;
u_rc10  := rc_dataset_u_sorted_new[10].rc;
u_rc11  := rc_dataset_u_sorted_new[11].rc;
u_rc12  := rc_dataset_u_sorted_new[12].rc;
u_rc13  := rc_dataset_u_sorted_new[13].rc;
u_rc14  := rc_dataset_u_sorted_new[14].rc;
u_rc15  := rc_dataset_u_sorted_new[15].rc;
u_rc16  := rc_dataset_u_sorted_new[16].rc;
u_rc17  := rc_dataset_u_sorted_new[17].rc;
u_rc18  := rc_dataset_u_sorted_new[18].rc;
u_rc19  := rc_dataset_u_sorted_new[19].rc;
u_rc20  := rc_dataset_u_sorted_new[20].rc;
u_rc21  := rc_dataset_u_sorted_new[21].rc;
u_rc22  := rc_dataset_u_sorted_new[22].rc;
u_rc23  := rc_dataset_u_sorted_new[23].rc;
u_rc24  := rc_dataset_u_sorted_new[24].rc;
u_rc25  := rc_dataset_u_sorted_new[25].rc;
u_rc26  := rc_dataset_u_sorted_new[26].rc;
u_rc27  := rc_dataset_u_sorted_new[27].rc;
u_rc28  := rc_dataset_u_sorted_new[28].rc;
u_rc29  := rc_dataset_u_sorted_new[29].rc;
u_rc30  := rc_dataset_u_sorted_new[30].rc;
u_rc31  := rc_dataset_u_sorted_new[31].rc;
u_rc32  := rc_dataset_u_sorted_new[32].rc;
u_rc33  := rc_dataset_u_sorted_new[33].rc;
u_rc34  := rc_dataset_u_sorted_new[34].rc;
u_rc35  := rc_dataset_u_sorted_new[35].rc;
u_rc36  := rc_dataset_u_sorted_new[36].rc;
u_rc37  := rc_dataset_u_sorted_new[37].rc;
u_rc38  := rc_dataset_u_sorted_new[38].rc;
u_rc39  := rc_dataset_u_sorted_new[39].rc;
u_rc40  := rc_dataset_u_sorted_new[40].rc;
u_rc41  := rc_dataset_u_sorted_new[41].rc;
u_rc42  := rc_dataset_u_sorted_new[42].rc;
u_rc43  := rc_dataset_u_sorted_new[43].rc;
u_rc44  := rc_dataset_u_sorted_new[44].rc;
u_rc45  := rc_dataset_u_sorted_new[45].rc;
u_rc46  := rc_dataset_u_sorted_new[46].rc;
u_rc47  := rc_dataset_u_sorted_new[47].rc;
u_rc48  := rc_dataset_u_sorted_new[48].rc;
u_rc49  := rc_dataset_u_sorted_new[49].rc;
u_rc50  := rc_dataset_u_sorted_new[50].rc;

blend_mod_rc10_1 := map(
    seg_business_score = 'LN & SBFE' => s_rc10,
    seg_business_score = 'LN ONLY'   => l_rc10,
                                        u_rc10);

blend_mod_rc46_1 := map(
    seg_business_score = 'LN & SBFE' => s_rc46,
    seg_business_score = 'LN ONLY'   => l_rc46,
                                        u_rc46);

blend_mod_rc35_1 := map(
...
blend_mod_rc17 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc17_1);

blend_mod_rc11 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc11_1);

blend_mod_rc20 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc20_1);

blend_mod_rc9 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc9_1);

blend_mod_rc24 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc24_1);

blend_mod_rc38 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc38_1);

blend_mod_rc14 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc14_1);

blend_mod_rc27 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc27_1);

blend_mod_rc42 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc42_1);

blend_mod_rc5 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc5_1);

blend_mod_rc12 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc12_1);

blend_mod_rc30 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc30_1);

blend_mod_rc19 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc19_1);

blend_mod_rc49 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc49_1);

blend_mod_rc6 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc6_1);

blend_mod_rc31 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc31_1);

blend_mod_rc48 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc48_1);

blend_mod_rc22 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc22_1);

blend_mod_rc33 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc33_1);

blend_mod_rc15 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc15_1);

blend_mod_rc4 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc4_1);

blend_mod_rc8 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc8_1);

blend_mod_rc32 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc32_1);

blend_mod_rc39 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc39_1);

blend_mod_rc1 := if(blend_mod_final_lgt = -1, 'B068', blend_mod_rc1_1);

blend_mod_rc43 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc43_1);

blend_mod_rc25 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc25_1);

blend_mod_rc29 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc29_1);

blend_mod_rc41 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc41_1);

blend_mod_rc16 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc16_1);

blend_mod_rc28 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc28_1);

blend_mod_rc50 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc50_1);

blend_mod_rc36 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc36_1);

blend_mod_rc34 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc34_1);

blend_mod_rc47 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc47_1);

blend_mod_rc40 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc40_1);

blend_mod_rc13 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc13_1);

blend_mod_rc21 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc21_1);

blend_mod_rc46 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc46_1);

blend_mod_rc10 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc10_1);

blend_mod_rc35 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc35_1);

sbbm1601_0_0 := if(blend_mod_final_lgt = -1, 222, min(if(max(_sbbm1601_0_0, 501) = NULL, -NULL, max(_sbbm1601_0_0, 501)), 900));

blend_mod_rc26 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc26_1);

blend_mod_rc7 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc7_1);

blend_mod_rc2 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc2_1);

blend_mod_rc45 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc45_1);

blend_mod_rc23 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc23_1);

blend_mod_rc44 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc44_1);

blend_mod_rc3 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc3_1);

blend_mod_rc37 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc37_1);

blend_mod_rc18 := if(blend_mod_final_lgt = -1, ' ', blend_mod_rc18_1);
...
sbbm_rc1_1 := blend_mod_rc1;

sbbm_rc2_1 := blend_mod_rc2;

sbbm_rc3_1 := blend_mod_rc3;

sbbm_rc4_1 := blend_mod_rc4;

sbbm_rc4 := if(sbbm1601_0_0 = 900, ' ', sbbm_rc4_1);

sbbm_rc3 := if(sbbm1601_0_0 = 900, ' ', sbbm_rc3_1);

sbbm_rc1 := if(sbbm1601_0_0 = 900, ' ', sbbm_rc1_1);

sbbm_rc2 := if(sbbm1601_0_0 = 900, ' ', sbbm_rc2_1);

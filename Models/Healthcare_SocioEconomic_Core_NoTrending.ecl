...
export Healthcare_SocioEconomic_Core_NoTrending(Socio_TC_Model_Version_in, SuppressResultsForOptOuts, isCoreRequestValid, batch_in, DPPAPurpose_in, GLBPurpose_in, DataRestrictionMask_in, DataPermissionMask_in, Options_in, ofac_version_in, gateways_in_ds, CoreResults,
                                                                        LexIdSourceOptout,
                                                                        TransactionID,
                                                                        BatchUID,
...
/* 	rawResults_append12 := join(rawResults_appendInput,rawResults12,(integer)left.seq=(integer)right.seq,
	...
	//Disabling historydate IID/Boca Shell/LeadIntegrity Calls and setting the trending attributes to blanks
	rawResults_final := PROJECT(rawResults_appendInput, TRANSFORM(Models.Layouts_Healthcare_Core.layout_SocioEconomic_attributes_combined,
									self.v4_CurrAddrBurglaryIndex12   := '';
									self.v4_EstimatedAnnualIncome12   := '';
									self.v4_PropOwnedTaxTotal12       := '';
									self.v4_SFDUAddrIdentitiesCount12 := '';
									self.v4_CurrAddrBurglaryIndex24   := '';
									self.v4_EstimatedAnnualIncome24   := '';
									self.v4_PropOwnedTaxTotal24       := '';
									self.v4_SFDUAddrIdentitiesCount24 := '';
									SELF := LEFT));

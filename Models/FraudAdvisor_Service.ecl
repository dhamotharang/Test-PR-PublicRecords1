...
//=============================================================
attrV1 := 'version1';                       //version1 attributes
...
from_biid        := false;
from_IT1O        := IF(model_name IN Models.FraudAdvisor_Constants.Paro_models or doParoAttributes, true, false); //set to true if Paro is transacting
doScore          := false;
...
Models.Layout_Parameters intoParoAttributes(Models.Layout_FraudAttributes le, integer c) := TRANSFORM
...
name_pairs_IDAttributes := NORMALIZE(pick_attr, NormalizeCount, intoIDAttributes(LEFT, COUNTER));
...
name_pairs_ParoAttributes := NORMALIZE(pick_attr, NormalizeCount, intoParoAttributes(LEFT, COUNTER));
...
Models.layouts.Layout_Score_FP form_Paroscore1(Models.Layout_ModelOut le) := TRANSFORM
	// SELF.seq := le.seq;
	SELF.i := le.Score;
	SELF.description := IF(model_name in ['msn1803_1','msnrsn_1'], '0 to 999', '250 to 999');
  self.index := Models.FraudAdvisor_Constants.getBilling_Index(model_name);
	self := [];
END;

Models.layouts.Layout_Score_FP form_Paroscore2(Models.Layout_ModelOut le) := TRANSFORM
	// SELF.seq := le.seq;
	SELF.i := le.Score;
	SELF.description := '250 to 999';
  self.index := Models.FraudAdvisor_Constants.getBilling_Index(model_name);
	self := [];
END;
...
Models.layouts.FP_Layout_Model form_Paro(Paro_ret le, Paro_ret2 ri) := TRANSFORM
	...
	self.scores := Map(model_name = 'msnrsn_1'  => PROJECT(le,form_Paroscore1(LEFT)) + PROJECT(ri,form_Paroscore2(LEFT)),
                     model_name = 'msn1803_1' => DATASET([form_Paroscore1(le)]),
                     model_name = 'rsn804_1'  => DATASET([form_Paroscore2(ri)]),
...
scores := if((~doAttributesVersion1 AND ~doIDAttributes) or 
                model_name in Models.FraudAdvisor_Constants.XML_custom_models OR  
                isWFS34, 
...
Deltabase_Logging_prep :=  project(joined_results, TRANSFORM(Risk_Reporting.Layouts.LOG_Deltabase_Layout_Record,
                                           ...
                                           self.i_work_phone := wphone_value,
																					 ...
																					 self.i_model_name_2 := '',
																					 self.o_score_1    := IF(model_name != '', left.Scores[1].i, ''),
																					 self.o_reason_1_1 := IF(model_name != '', left.Scores[1].Reason_Codes[1].Reason_Code, ''),
																					 self.o_reason_1_2 := IF(model_name != '', left.Scores[1].Reason_Codes[2].Reason_Code, ''),
																					 self.o_reason_1_3 := IF(model_name != '', left.Scores[1].Reason_Codes[3].Reason_Code, ''),
																					 self.o_reason_1_4 := IF(model_name != '', left.Scores[1].Reason_Codes[4].Reason_Code, ''),
																					 self.o_reason_1_5 := IF(model_name != '', left.Scores[1].Reason_Codes[5].Reason_Code, ''),
																					 self.o_reason_1_6 := IF(model_name != '', left.Scores[1].Reason_Codes[6].Reason_Code, ''),
																					 self.o_score_2    := IF(self.i_model_name_2 != '', left.Scores[2].i, ''),
																					 self.o_reason_2_1 := IF(self.i_model_name_2 != '', left.Scores[2].Reason_Codes[1].Reason_Code, ''),
																					 self.o_reason_2_2 := IF(self.i_model_name_2 != '', left.Scores[2].Reason_Codes[2].Reason_Code, ''),
																					 self.o_reason_2_3 := IF(self.i_model_name_2 != '', left.Scores[2].Reason_Codes[3].Reason_Code, ''),
																					 self.o_reason_2_4 := IF(self.i_model_name_2 != '', left.Scores[2].Reason_Codes[4].Reason_Code, ''),
																					 self.o_reason_2_5 := IF(self.i_model_name_2 != '', left.Scores[2].Reason_Codes[5].Reason_Code, ''),
																					 self.o_reason_2_6 := IF(self.i_model_name_2 != '', left.Scores[2].Reason_Codes[6].Reason_Code, ''),

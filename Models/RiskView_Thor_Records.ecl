...
boolean	IncludeAllScores, boolean IncludeAllAttributes, boolean IncludeScoresAttributes, boolean IncludeAuto, boolean IncludeBankcard,
...
batchinseq := BatchIn; // seq is already assigned.
...
cleanIn_Roxie := project(batchinseq_temp, into_in(left));
...
r_layout_input_PlusRaw	prep_for_AID(cleanIn_Roxie le)	:= transform
...
aid_prepped	:=	PROJECT(cleanIn_Roxie, prep_for_AID(left));
...
clam_to_run := if(adl_clam_model OR IsPreScreen, adl_clam, clam);
...
Layout_working := RECORD
	...
	string3 score6; // need to add on extra fields to put the prescreen score when running on thor because batch layout doesn't allow for 6 scores at once like the xml service does
	string3 reason26;
END;
...
rvAuto := map(AlternateModel='rva707_0' => ungroup( Models.RVA707_0_0( clam_to_run,  false) ),
						  AlternateModel='rva1309_1' => ungroup( Models.RVA1309_1_0( clam_to_run,  false) ),
			IncludeAllScores or IncludeScoresAttributes or IncludeAuto => map(
				FlagshipVersion=4 =>  ungroup(Models.RVA1104_0_0(clam_to_run,false,false)),
				FlagshipVersion=3 =>  ungroup(Models.RVA1003_0_0(clam_to_run,false)),
				FlagshipVersion=2 =>  ungroup(Models.RVA711_0_0(clam_to_run,false)),
				ungroup(Models.RVA611_0_0(clam_to_run,  false))),
			dataset([],Models.Layout_ModelOut)
);
rvBankcard := map(
			AlternateModel='rvb1402_1' => ungroup(Models.RVB1402_1_0(clam_to_run,false)),
			IncludeAllScores or IncludeScoresAttributes or IncludeBankcard => map(
				FlagshipVersion=4 				 => ungroup(Models.RVB1104_0_0(clam_to_run,false,false)),
				FlagshipVersion=3					 => ungroup(Models.RVB1003_0_0(clam_to_run,false)),
				FlagshipVersion=2					 => ungroup(Models.RVB711_0_0(ungroup(clam_to_run),false,false)),
				ungroup(Models.RVB609_0_0(clam_to_run,  false))),
			dataset([],Models.Layout_ModelOut)
);


rvRetail := map(
	AlternateModel='rvr1104_2' => ungroup( Models.RVR1104_2_0( clam_to_run, false, false) ),
	AlternateModel='rvr1210_1' => ungroup( Models.RVR1210_1_0( clam_to_run) ),
	AlternateModel='ex89' => ungroup( Models.TRD605_0_0( clam_to_run,  false, false ) ),
	IncludeAllScores or IncludeScoresAttributes or IncludeRetail => map(
	FlagshipVersion=4 => ungroup(Models.RVR1103_0_0(clam_to_run, false, false)),
	FlagshipVersion=3 => ungroup(Models.RVR1003_0_0(clam_to_run)),
	FlagshipVersion=2 => ungroup(Models.RVR711_0_0(clam_to_run,  false)),
	ungroup(Models.RVR611_0_0(clam_to_run,  false))),
	dataset([],Models.Layout_ModelOut)
);
rvTelecom := map(
			AlternateModel='ex23' => ungroup( Models.AWD606_3_0( clam_to_run,  false, false ) ),
			IncludeAllScores or IncludeScoresAttributes or IncludeTelecom => map(
				FlagshipVersion=4 => ungroup(Models.RVT1104_0_0(clam_to_run,false,false)),
				FlagshipVersion=3 => ungroup(Models.RVT1003_0_0(clam_to_run)),
				FlagshipVersion=2 => ungroup(Models.RVT711_0_0(clam_to_run,false)),
				ungroup(Models.RVT612_0(clam_to_run,  false))),
			dataset([],Models.Layout_ModelOut)
);

rvPreScreen := map(
			IncludePreScreen => map(
        AlternateModel = 'rvp1012_1' => ungroup(models.RVP1012_1_0(clam_to_run)),
        AlternateModel = 'rvp1208_1' => ungroup(models.RVP1208_1_0(clam_to_run)),
				AlternateModel = 'rvp1401_1' => ungroup(models.RVP1401_1_0(clam_to_run)),
				AlternateModel = 'rvp1401_2' => ungroup(models.RVP1401_2_0(clam_to_run)),
				AlternateModel = 'rvp1503_1' => ungroup(models.RVP1503_1_0(clam_to_run)),
				FlagshipVersion=4 => ungroup(Models.RVP1104_0_0(clam_to_run,false,false)),
				FlagshipVersion=3 => ungroup(Models.RVP1003_0_0(clam_to_run,false)),
				ungroup(Models.RVP804_0_0(clam_to_run))),
			dataset([],Models.Layout_ModelOut)
);
...
rvMoney := map(
			IncludeAllScores or IncludeScoresAttributes or IncludeMoney => map(
				...
				AlternateModel = 'rvc1112_0' => UNGROUP(Models.RVC1112_0_0(clam_to_run)),
				AlternateModel = 'rvc1110_1' => Models.RVC1110_1_0(UNGROUP(clam_to_run),false,false),
				AlternateModel = 'rvc1110_2' => Models.RVC1110_2_0(UNGROUP(clam_to_run),false,false),
				AlternateModel = 'rvc1208_1' => ungroup(Models.RVC1208_1_0(clam_to_run,false,false)),  
				AlternateModel = 'rvc1301_1' => ungroup(Models.RVC1301_1_0(clam_to_run)),  
				AlternateModel = 'rvc1307_1' => ungroup(Models.RVC1307_1_0(clam_to_run, false)),  
				...
				AlternateModel = 'rvc1405_3' => ungroup(Models.RVC1405_3_0(clam_to_run, false)), 
				AlternateModel = 'rvc1405_4' => ungroup(Models.RVC1405_4_0(clam_to_run, false)),
				AlternateModel = 'rvc1412_1' => ungroup(Models.RVC1412_1_0(clam_to_run, false)),
				AlternateModel = 'ied1002_0' => ungroup(Models.IED1002_0_9(clam_to_run, false)),
				AlternateModel = 'rvc1412_2' => ungroup(Models.RVC1412_2_0(custom_adl_clam, false)),
				AlternateModel = 'rvc1703_1' => ungroup(Models.RVC1703_1_0(clam_to_run, false)),
				AlternateModel = 'rvc1801_1' => ungroup(Models.RVC1801_1_0(clam_to_run, false)),
				FlagshipVersion=4 => ungroup(Models.RVG1103_0_0(clam_to_run,false,false)),
				FlagshipVersion=3 => ungroup(Models.RVG1003_0_0(clam_to_run)),
...
Layout_working addScorePreScreen(Models.Layout_ModelOut le, attributes rt) := transform
	// PreScreen
	self.seq := le.seq;
	self.score6 := le.score;
	self.reason26 := le.ri[1].hri;
	self := rt;
	self := [];
END;
...
layout_working applySuppression( filtered_pre_screen_recs le, acct_seq_only ri ) := TRANSFORM
...
final := map(
	AlternateModel!='' AND CustomIndex='' => dataset( [], layout_working ), // When an alternate model (not flagship v2) is specified but is not valid, output a blank dataset.
	isPreScreen => after_suppressions,
	filtered_pre_screen_recs
);
#end

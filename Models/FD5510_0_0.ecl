Import risk_indicators,std,ut,RiskWise;
export FD5510_0_0(grouped dataset(Risk_Indicators.Layout_Boca_Shell) clam, boolean OFAC=true, boolean nugen=false, boolean other_watchlists=false) := 
...
Layout_ModelOut doModel(clam le) := transform
	...
	/* Models */
     ...
     Fraud_Score_510_1 := map((ssninval = 1 or ssndead = 1) and mod7_score > 624 => 624,
     ...
     Fraud_Score_510_5 := map(Fraud_Score_510 <= 605 => 10,
						Fraud_Score_510 <= 624 => 20,
						Fraud_Score_510 <= 641 => 30,
						Fraud_Score_510 <= 679 => 40,
						50);


     SELF.score := '0'+(string)Fraud_Score_510_5;
	SELF.seq := le.seq;
	SELF.ri := [];	
...
Layout_ModelOut addReasons(Layout_ModelOut le, iid ri) := transform
	reasons := if( nugen, RiskWise.bdReasonCodesConsumer( ri, 4, OFAC, other_watchlists, nugen ), RiskWise.fdReasonCodes(ri, 4, OFAC, other_watchlists) );
	reason34 := DATASET([{'34',risk_indicators.getHRIDesc('34')},{'00',''},{'00',''},{'00',''}],risk_indicators.Layout_Desc);
	SELF.ri := if((integer)le.score <= 30 and reasons[1].hri='00',reason34,reasons);
	SELF := le;
end;

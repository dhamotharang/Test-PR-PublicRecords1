...
export TBD609_1_0(grouped dataset(Risk_Indicators.Layout_Boca_Shell) clam, boolean OFAC, boolean inCalif) := FUNCTION
...
Layout_ModelOut doModel(clam le) := TRANSFORM
	...
	tbd := (integer)(-104685 + (x3 * 466.95115) + (x3 * x3 * -0.69128) + (x3 * x3 * x3 * 0.00034184));
     	
	tbd60910 := map(le.ssn_verification.validation.deceased and tbd > 625 => 625,
			    tbd > 999 => 999,
			    tbd < 250 => 250,
			    tbd);

	self.ri := RiskWiseFCRA.corrReasonCodes(le.consumerflags, 4);
     self.score := if(self.ri[1].hri in ['91','92','93','94','95'], (string)((integer)self.ri[1].hri + 10), intformat(tbd60910,3,1));
	self.seq := le.seq;
END;

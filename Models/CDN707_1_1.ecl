...
export CDN707_1_1(
	...
	Layout_ModelOut doModel( clam le, ineasi ri ) := TRANSFORM
		...
		v_in_avs := case( cus_avs,
		 ...
		 v_in_cid_match := (INTEGER)(v_in_pmt='P' or v_in_cid = '1');
		...
		vi_in_avsX := map(
			v_in_pmt = 'P' => 5,
			vi_in_avsX1 <= 2 and v_in_cid_match=0 => vi_in_avsX1 - 0.5,
			vi_in_avsX1
		);
		...
		logit := map(
			uv_dataset = 'instore' => instore_logit,
			uv_dataset = 'billto' =>  billto_logit,
			shipto_logit
		);
		...
		CDN707_1_1a := round(point*(log(phat/(1-phat)) - log(odds))/log(2) + base);
		CDN707_1_1  := ut.max2(250,ut.min2(999,CDN707_1_1a));


		self.score := (STRING)cdn707_1_1;

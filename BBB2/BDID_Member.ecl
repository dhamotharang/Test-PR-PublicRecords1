...
export BDID_Member(
	...
	,dataset(Layouts_Files.Base.Member_BIP	)	pBaseFile
	,string																pPersistname = PersistNames.BDIDMember
	
) := 
function

BBB_Member_Base 	:= pBaseFile;

//////////////////////////////////////////////////////////////////////////////////////////////
// -- Clear bdid field
//////////////////////////////////////////////////////////////////////////////////////////////
#if(BBB_Init_Flag = false)
Layouts_Files.Base.Member_BIP ClearBDIDBase(BBB_Member_Base l) := 
transform
	self.bdid 	   := 0;
...
BBB_Member_clear_bdid_base := project(BBB_Member_Base, ClearBDIDBase(left));

BBB_Member_combined := Clean_Member(pversion,pInputFile) + BBB_Member_clear_bdid_base;
#else
BBB_Member_combined := Clean_Member(pversion,pInputFile);
#end

//////////////////////////////////////////////////////////////////////////////////////////////
// -- Clean and Standardizes all Addresses using the AID address cleaning
//////////////////////////////////////////////////////////////////////////////////////////////
HasAddress	:= trim(BBB_Member_combined.prep_addr_line_last, left,right) != '';
								
dWith_address			:= BBB_Member_combined(HasAddress);
dWithout_address	:= BBB_Member_combined(not(HasAddress));
...
AID.MacAppendFromRaw_2Line(dWith_address, prep_addr_line1, prep_addr_line_last, Raw_AID, dMember_withAID, lFlags);

dMember_Aid_Cleaned := project(
	dMember_withAID
	,transform(
...
layout_BBB_Member_seq AddSeqNum(Layouts_Files.Base.Member_BIP l, unsigned4 cnt) :=
transform
	self.seq 		:= cnt;
	self.source_rec_id := HASH64(ut.CleanSpacesAndUpper(l.company_name) + 
															 ...
															 ut.CleanSpacesAndUpper(l.member_title) +
															 stringlib.stringcleanspaces(l.member_since_date) +
															 ut.CleanSpacesAndUpper(l.member_category)
															);
	self.source	:= mdr.sourceTools.src_BBB_Member;                          ;
	self 				:= l;
end;

BBB_Member_seq := sort(distribute(project(dMember_Aid_Cleaned, AddSeqNum(left, counter)),seq),seq,local);

//////////////////////////////////////////////////////////////////////////////////////////////
// -- Slim Record for BDIDing, add BDID field
//////////////////////////////////////////////////////////////////////////////////////////////
layout_BBB_Member_bdid := 
record
	unsigned6 bdid := 0;
	BIPV2.IDlayouts.l_xlink_ids;
	BBB_Member_seq.source;                      
	BBB_Member_seq.source_rec_id;
	BBB_Member_seq.seq;
	BBB_Member_seq.Company_Name;
	BBB_Member_seq.prim_range;
	BBB_Member_seq.prim_name;
	BBB_Member_seq.sec_range;
	BBB_Member_seq.st;
	BBB_Member_seq.zip;
	BBB_Member_seq.phone10;
	BBB_Member_seq.p_city_name;
	BBB_Member_seq.bbb_id;
end;

BBB_Member_to_bdid := table(BBB_Member_seq, layout_BBB_Member_bdid);

//////////////////////////////////////////////////////////////////////////////////////////////
// -- BDID records First Pass
//////////////////////////////////////////////////////////////////////////////////////////////
Business_Header.MAC_Source_Match(BBB_Member_to_bdid, BBB_Member_bdid_init,
                        false, bdid,
                        false, MDR.sourceTools.src_BBB_Member,
                        true,  bbb_id,
...
//////////////////////////////////////////////////////////////////////////////////////////////
Business_Header_SS.MAC_Add_BDID_Flex(BBB_Member_bdid_init     // Input Dataset
                                     ...
                                     ,phone10                 // phone
																		 ,fein_field              // fein
                                     ,bdid                    // bdid
																		 ,layout_BBB_Member_bdid  // Output Layout
                                     ,FALSE                   // output layout has bdid score field?
																		 ,BDID_score_field        // bdid_score
                                     ,BBB_Member_bdid_all     // Output Dataset   
	                                   ,												// deafult threscold
...
BBB_Member_All_sort := sort(distribute(BBB_Member_bdid_all,seq),seq,local);


//////////////////////////////////////////////////////////////////////////////////////////////
// -- Join back to input file and append BDIDs to create base file
//////////////////////////////////////////////////////////////////////////////////////////////
Layouts_Files.Base.Member_BIP JoinBack(layout_BBB_Member_seq L, layout_BBB_Member_bdid R) := 
transform
	self.bdid				:= R.bdid;
	self.ultid      := R.ultid;
  self.orgid      := R.orgid;
...
jBBB_Member    := join(BBB_Member_seq,
								       BBB_Member_All_sort,
										   left.seq = right.seq,
										   JoinBack(left,right),
											 left outer,
...
//////////////////////////////////////////////////////////////////////////////////////////
Layouts_Files.Base.Member_BIP RollupBase(Layouts_Files.Base.Member_BIP L, Layouts_Files.Base.Member_BIP R) := 
transform
...
BBB_Member_dist := distribute(jBBB_Member,hash(bbb_id,company_name));
BBB_Member_sort := sort(BBB_Member_dist, 
						bbb_id,
						company_name,
						address,
						country,
...
BBB_Member_rollup := rollup(BBB_Member_sort,
...
//			  left.report_date 		= right.report_date 		and
			    ...
			    left.member_title		    = right.member_title 		and
			    left.member_since_date	= right.member_since_date,
...
//////////////////////////////////////////////////////////////////////////////////////////
BBB_Member_Grpd 	  	:= GROUP(BBB_Member_rollup, bbb_id, LOCAL);
BBB_Member_Grpd_Sort 	:= SORT(BBB_Member_Grpd, -process_date_last_seen);

Layouts_Files.Base.Member_BIP SetRecordType(BBB_Member_Grpd_Sort l, BBB_Member_Grpd_Sort r) := 
transform
	self.record_type 	  := if(l.record_type = '', 'C', 'H');
	SELF 				        := r;
END;

BBB_Member_Prop := GROUP(ITERATE(BBB_Member_Grpd_Sort, SetRecordType(LEFT, RIGHT)));

BBB_Member_Out  := BBB_Member_Prop : persist(pPersistname);	
 
return BBB_Member_Out;

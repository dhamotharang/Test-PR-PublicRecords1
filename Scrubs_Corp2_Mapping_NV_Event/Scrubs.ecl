...
EXPORT FromNone(DATASET(corp2_mapping.layoutscommon.events) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.event_filing_reference_nbr_Invalid := Fields.InValid_event_filing_reference_nbr((SALT32.StrType)le.event_filing_reference_nbr);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    event_filing_reference_nbr_LENGTH_ErrorCount := COUNT(GROUP,h.event_filing_reference_nbr_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.event_filing_reference_nbr_Invalid,'LENGTH','UNKNOWN')
    ...
    SELF.FieldType := CHOOSE(c,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_mandatory','invalid_date','invalid_date_type_cd','invalid_date_type_desc','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT32.StrType)le.corp_key,(SALT32.StrType)le.corp_vendor,(SALT32.StrType)le.corp_state_origin,(SALT32.StrType)le.corp_process_date,(SALT32.StrType)le.corp_sos_charter_nbr,(SALT32.StrType)le.event_filing_reference_nbr,(SALT32.StrType)le.event_filing_date,(SALT32.StrType)le.event_date_type_cd,(SALT32.StrType)le.event_date_type_desc,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT32.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_event_date_type_desc(1),'UNKNOWN');
      ...
      SELF.rulepcnt := 100 * CHOOSE(c
          ...
          ,le.event_filing_date_ALLOW_ErrorCount,le.event_filing_date_CUSTOM_ErrorCount
          ,le.event_date_type_cd_ENUM_ErrorCount
          ,le.event_date_type_desc_ENUM_ErrorCount,0) / le.TotalCnt + 0.5;
    END;

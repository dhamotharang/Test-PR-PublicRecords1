...
EXPORT FromNone(DATASET(Layout_In_NV) h) := MODULE
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.corp_key_Invalid << 0 ) + ( le.corp_vendor_Invalid << 2 ) + ( le.corp_state_origin_Invalid << 3 ) + ( le.corp_process_date_Invalid << 4 ) + ( le.corp_sos_charter_nbr_Invalid << 6 ) + ( le.event_filing_reference_nbr_Invalid << 8 ) + ( le.event_filing_date_Invalid << 9 ) + ( le.event_date_type_cd_Invalid << 11 ) + ( le.event_date_type_desc_Invalid << 12 );
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    event_filing_reference_nbr_LENGTHS_ErrorCount := COUNT(GROUP,h.event_filing_reference_nbr_Invalid=1);
    ...
    event_date_type_desc_ENUM_ErrorCount := COUNT(GROUP,h.event_date_type_desc_Invalid=1);
    AnyRule_WithErrorsCount := COUNT(GROUP, h.corp_key_Invalid > 0 OR h.corp_vendor_Invalid > 0 OR h.corp_state_origin_Invalid > 0 OR h.corp_process_date_Invalid > 0 OR h.corp_sos_charter_nbr_Invalid > 0 OR h.event_filing_reference_nbr_Invalid > 0 OR h.event_filing_date_Invalid > 0 OR h.event_date_type_cd_Invalid > 0 OR h.event_date_type_desc_Invalid > 0);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.corp_key_Total_ErrorCount > 0, 1, 0) + IF(le.corp_vendor_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_state_origin_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_sos_charter_nbr_Total_ErrorCount > 0, 1, 0) + IF(le.event_filing_reference_nbr_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.event_filing_date_Total_ErrorCount > 0, 1, 0) + IF(le.event_date_type_cd_ENUM_ErrorCount > 0, 1, 0) + IF(le.event_date_type_desc_ENUM_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.corp_key_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_key_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_vendor_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_state_origin_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_sos_charter_nbr_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_sos_charter_nbr_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.event_filing_reference_nbr_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.event_filing_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.event_filing_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.event_date_type_cd_ENUM_ErrorCount > 0, 1, 0) + IF(le.event_date_type_desc_ENUM_ErrorCount > 0, 1, 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.corp_key_Invalid,le.corp_vendor_Invalid,le.corp_state_origin_Invalid,le.corp_process_date_Invalid,le.corp_sos_charter_nbr_Invalid,le.event_filing_reference_nbr_Invalid,le.event_filing_date_Invalid,le.event_date_type_cd_Invalid,le.event_date_type_desc_Invalid,100);
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.event_filing_reference_nbr_Invalid,'LENGTHS','UNKNOWN')
          ,CHOOSE(le.event_filing_date_Invalid,'ALLOW','CUSTOM','UNKNOWN')
          ,CHOOSE(le.event_date_type_cd_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.event_date_type_desc_Invalid,'ENUM','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_sos_charter_nbr','event_filing_reference_nbr','event_filing_date','event_date_type_cd','event_date_type_desc','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_mandatory','invalid_future_date','invalid_date_type_cd','invalid_date_type_desc','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT311.StrType)le.corp_key,(SALT311.StrType)le.corp_vendor,(SALT311.StrType)le.corp_state_origin,(SALT311.StrType)le.corp_process_date,(SALT311.StrType)le.corp_sos_charter_nbr,(SALT311.StrType)le.event_filing_reference_nbr,(SALT311.StrType)le.event_filing_date,(SALT311.StrType)le.event_date_type_cd,(SALT311.StrType)le.event_date_type_desc,'***SALTBUG***');
  END;
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'event_filing_reference_nbr:invalid_mandatory:LENGTHS'
          ,'event_filing_date:invalid_future_date:ALLOW','event_filing_date:invalid_future_date:CUSTOM'
          ,'event_date_type_cd:invalid_date_type_cd:ENUM'
          ,'event_date_type_desc:invalid_date_type_desc:ENUM'
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ...
          ,le.event_filing_date_ALLOW_ErrorCount,le.event_filing_date_CUSTOM_ErrorCount
          ,le.event_date_type_cd_ENUM_ErrorCount
          ,le.event_date_type_desc_ENUM_ErrorCount,0) / le.TotalCnt, CHOOSE(c - NumRules
...
EXPORT StandardStats(DATASET(Layout_In_NV) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Corp2_Mapping_NV_Event, Fields, 'RECORDOF(scrubsSummaryOverall)', '');

...
EXPORT Build_Base_Main ( 
   string pversion
	,dataset(Layouts.Base.main) PrevBase = Files().Base.main.QA
	//,boolean IsUpdateFullFile            = false
	) := 
module
  ...
  // Append Industry type description 
	...
	Combined        := join (addUID , Files().Input.MBSFdnIndType.Sprayed(status = 1), 
																		   ...
																		   self.classification_source.Industry_segment                     := if(left.source<>'GLB5',StringLib.StringToUppercase(right.description),left.classification_source.Industry_segment), 

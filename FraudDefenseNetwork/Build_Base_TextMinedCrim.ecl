...
InCrim     := dedup(sort(distribute(ds_frdtype,hash(did)),did,-off_date,  -convict_dt,  -ct_disp_dt,  -stc_dt,  -inc_adm_dt,  -st_start_dt,  -event_date, -offender_persistent_id,local),did,local);
	 ...
	 FraudDefenseNetwork.Functions.CleanFields(InCrim ,InCrimupper); 
		...
		FraudDefenseNetwork.Layouts.Base.TextMinedCrim	tPrep(FraudDefenseNetwork.Layouts.Input.TextMinedCrim	pInput)	:= 
		transform				
  ...
  pBase_dist    := distribute(inBaseTextMinedCrim, hash(did));	
		...
		FraudDefenseNetwork.Layouts.Base.TextMinedCrim getSrcRid (pDataset_Dist l, pBase_dist r) := transform

...
EXPORT Build_Base_Tiger (
                          string pversion, dataset(Layouts.Base.Tiger) inBaseTiger = Files().Base.Tiger.QA, dataset(Layouts.Input.Tiger)
                          inTigerUpdate = Files().Input.Tiger.Sprayed, boolean UpdateTiger = FraudDefenseNetwork._Flags.Update.Tiger
                        ) := module
   ...
   tools.mac_WriteFile(Filenames(pversion).Base.tiger.New, dBase_RecordID, Build_Base_File);  
    ...
    //Return
      ...
      export All := if(
                        tools.fun_IsValidVersion(pversion), full_build, output('No Valid version parameter passed, skipping Build_Base_Tiger atribute')
                      );
end;

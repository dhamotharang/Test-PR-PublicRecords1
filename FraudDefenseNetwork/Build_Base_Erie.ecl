...
module 
	...
	transform
	 ...
	 Erie_Proj                                     := project(dedup(inERIEUpdateUpper(claimnumber<>'' and (insuredfname <> '' or insuredlname <>'')),all),tPrep(left)):independent;
     	 
   Erie_Individuals                              := Erie_Proj(insuredfirstname <> '');  
...
//Standardize_Name name cannot be used because of the output clean names are coming with "cleaned_name" format which are already used in line62
   ...
   ErieCleanName_Indiv                           := project(ErieCleanName_cp, CName(LEFT));
	 ...
	 Erie_Business                                 := Erie_Proj(insuredfirstname = '');	
   ...
   ErieClean_Proj                                := project(ErieClean, transform(recordof(ErieClean),
																																									...
																																									self.TypeOfMapping       := map(left.responsibleparty not in ['I', 'i']                                                => 'ERIE_PARTY_MAPPING',
	 ...
	 //updates
	 ErieUpdate                                    := distribute(ErieClean_Proj, hash(claimnumber,cleaned_name.fname,cleaned_name.lname,typeofloss, dateofloss, dateio, findings, responsibleparty, policynumber, cleaned_name_cp.fname, cleaned_name_cp.lname));		 

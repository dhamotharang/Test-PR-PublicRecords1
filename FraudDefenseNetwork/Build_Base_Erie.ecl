...
export Build_Base_Erie (
                         string pversion, dataset(Layouts.Base.ERIE) inBaseERIE = Files().Base.ERIE.QA, 
...
//Standardize_Name name cannot be used because of the output clean names are coming with "cleaned_name" format which are already used in line62
   ...
   typeof(ErieCleanName_Siu) CName(ErieCleanName_cp L) :=
            transform
          ...
          ErieCleanName_Indiv_rmDup := dedup(sort(ErieCleanName_Indiv_dist, claimnumber, cleaned_name.fname, cleaned_name.lname, 
                                       typeofloss, dateofloss, dateio, findings, responsibleparty, policynumber, cleaned_name_cp.fname, 
                                       cleaned_name_cp.lname, -ffid, local), claimnumber, cleaned_name.fname, cleaned_name.lname, 
                                       typeofloss, dateofloss, dateio, findings, responsibleparty, policynumber, 
                                       cleaned_name_cp.fname, cleaned_name_cp.lname, local);
          ...
          Entity_Ind := map(left.cleaned_name_cp.fname <>'' and left.cleaned_name_cp.lname <>'' => 'PERSON', left.cleaned_name_cp.fname = '' and left.cleaned_name_cp.lname <>'' => 'BUSINESS', 'UNKNOWN');
          ...
          Type_Of_Mapping := map(left.responsibleparty not in ['I', 'i'] => 'ERIE_PARTY_MAPPING', IsInsuredClaimParty and Entity_Ind <> 'UNKNOWN' => 'ERIE_INSURED_MAPPING', 'ERIE_PARTY_MAPPING');
          self.TypeOfMapping := Type_Of_Mapping;
                                 
          IsErieInsMapIndiv := Type_Of_Mapping = 'ERIE_INSURED_MAPPING' and (left.insuredfirstname <>'' and left.insuredlastName <>'');
          IsErieInsMapBusi := Type_Of_Mapping = 'ERIE_INSURED_MAPPING' and (left.insuredfirstname = '' and left.insuredlastName <>'');
          IsEriePartyMapIndivUnk := Type_Of_Mapping = 'ERIE_PARTY_MAPPING' and(Entity_Ind = 'PERSON' or Entity_Ind = 'UNKNOWN');
          ...
          self.business_name := map(IsErieInsMapBusi => left.insuredlastName, IsEriePartyMapBusi => left.name_last, '');
          self.clean_business_name := map(IsErieInsMapBusi => NID.clnBizName(prepInsLastName), IsEriePartyMapBusi => NID.clnBizName(prepName_last), '');
          self.cleaned_name.title := map(IsErieInsMapIndiv => left.cleaned_name.title, IsEriePartyMapIndivUnk => left.cleaned_name_cp.title, '');
          self.cleaned_name.fname := map(IsErieInsMapIndiv => left.cleaned_name.fname, IsEriePartyMapIndivUnk => left.cleaned_name_cp.fname, '');
          self.cleaned_name.mname := map(IsErieInsMapIndiv => left.cleaned_name.mname, IsEriePartyMapIndivUnk => left.cleaned_name_cp.mname, '');
          self.cleaned_name.lname := map(IsErieInsMapIndiv => left.cleaned_name.lname, IsEriePartyMapIndivUnk => left.cleaned_name_cp.lname, '');
          self.cleaned_name.name_suffix := map(IsErieInsMapIndiv => left.cleaned_name.name_suffix, IsEriePartyMapIndivUnk => left.cleaned_name_cp.name_suffix, '');
          self := left));
  ...
  //Return
    export full_build := sequential(
     ...
     export All := if(
                       tools.fun_IsValidVersion(pversion), full_build, output('No Valid version parameter passed, skipping Erie atribute')
                     );
end;
...
EXPORT Build_Base_AInspection (
                                string pversion, dataset(Layouts.Base.AInspection) inBaseAInspection = Files().Base.AInspection.QA
  ...
  //Rollup Update and previous base
    ...
    pDataset_rollup := rollup(
                               pDataset_sort, RollupUpdate(left, right), Record, except process_date, dt_first_seen,
  ...
  //Return
    ...
    export All := if(
                      tools.fun_IsValidVersion(pversion), full_build, output('No Valid version parameter passed, skipping Build_Base_AInspection atribute')
                    );
end;
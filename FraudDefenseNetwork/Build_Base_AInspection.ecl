...
module 
	...
	unsigned	AInspectionMaxRecordID	:= if(UpdateAInspection, max(inBaseAInspection,source_rec_id), 0)	:	global; // For full initial load as its not update
		...
		FraudDefenseNetwork.Functions.CleanFields(inAInspectionUpdate ,inAInspectionUpdateUpper); 

		FraudDefenseNetwork.Layouts.Base.AInspection	tPrep(Layouts.Input.AInspection	pInput,integer	cnt)	:=
	transform
	...
	AInspectionUpdate	:=	project(dedup(inAInspectionUpdateUpper(prim_name <> '' and zip <>'')  ,all),tPrep(left,counter));
		
	// Rollup Update and previous base 
	Pcombined     := If(UpdateAInspection , inBaseAInspection + AInspectionUpdate , AInspectionUpdate); 	
	pDataset_Dist := distribute(Pcombined, hash(prim_name));	

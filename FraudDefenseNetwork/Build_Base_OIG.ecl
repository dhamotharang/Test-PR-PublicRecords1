...
AppendProviderAttributes, Health_Provider_Services, Health_Facility_Services, HealthCareFacility, BIPV2_Company_Names, address;
    ...
    export Build_Base_OIG (
                            string pversion, dataset(Layouts.Base.OIG) inBaseOIG = Files().Base.OIG.QA
       ...
       Layouts.Base.OIG tPrep(Layouts.Input.OIG pInput) :=   
               transform
                    ...
                    self.sancdesc := map(pInput.sanctype ='1128AA' => 'CIVIL MONETARY PENALTY EXCLUSION',
                                     pInput.sanctype ='1128B1' => 'CONVICTION RELATING TO FRAUD', pInput.sancdesc);
                    self.dob := pInput.dob;
     ...
     //LNPID Append
       Health_Provider_Services.mac_get_best_lnpid_on_thor (oigDIDUpdUnq, lnpid, cleaned_name.fname, cleaned_name.mname, 
       cleaned_name.lname, cleaned_name.name_suffix,, clean_address.prim_range, clean_address.prim_name, clean_address.sec_range, clean_address.v_city_name,
       clean_address.st, clean_address.zip, ssn, dob,,,,,,, npi, upin1,,,,, oigDIDUpdLnpid, false, 38);
  ...
  //Return
    ...
    export All := if(
                      tools.fun_IsValidVersion(pversion), full_build, 

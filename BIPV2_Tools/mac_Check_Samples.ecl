...
allids := 
  ...
  dfilt := table(pdataset,{proxid,prim_range,prim_name,v_city_name,cnp_name,zip})(
                    proxid in allids
                 or (prim_range = '2250' and prim_name = 'WILLIAM D TATE'  and v_city_name  = 'GRAPEVINE'  and regexfind('^(?!.*?[[:digit:]]+.*).*?GAMESTOP.*$'                          ,cnp_name,nocase)  )
                 or (prim_range = '5200' and prim_name = 'BUFFINGTON'      and zip          = '30349'      and regexfind('^(?!.*?([[:digit:]]+| AT ).*)CHIC(K|C)[ ]*?FI[L]?[ ]*A?[ ]*$'  ,cnp_name,nocase)  )
  );
  ...
  dagg := BIPV2_ProxID.AggregateProxidElements(dfilt2);

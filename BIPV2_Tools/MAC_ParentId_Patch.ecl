// This is based on, and can replace, SALT28.MAC_ParentId_Patch
import salt28;
...
import salt28;
  ...
  IdRec := RECORD
	...
	GoodMaps := JOIN(JOIN(OkDupsOut, BadParents, LEFT.id1=RIGHT.ID, TRANSFORM(LEFT), LEFT ONLY, HASH), BadParents, LEFT.id2=RIGHT.ID, TRANSFORM(LEFT), LEFT ONLY, HASH);
	
	// For those BadIds that cannot be combined at the parent level - I need to pull the children out of the parent
	Maps_filt := JOIN(Maps, BadParents, LEFT.id1=RIGHT.ID, TRANSFORM(LEFT), KEEP(1), HASH) + JOIN(Maps, BadParents, LEFT.id2=RIGHT.ID, TRANSFORM(LEFT), KEEP(1), HASH);
	...
	O2 := JOIN(o1,AffectedParents,LEFT.parentid=RIGHT.ID,TRANSFORM(LEFT),LEFT ONLY,HASH) +

...
functionmacro
  import _control,ut, data_services;
	...
	#ELSE
		...
		ds_active_duns_new  := dataset(Data_Services.foreign_prod+Persistname+ '.new' + pPersist_Unique, {string duns_number,string company_name,string date_last_seen,string status}, thor);
	#END

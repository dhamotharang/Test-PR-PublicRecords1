EXPORT macPivotOttoOutput(indata,fieldlist,outputfieldlist):=FUNCTIONMACRO
  IMPORT hipie_ecl, RAMPS_Functions;
  IMPORT Std;
  
  d := DISTRIBUTE(hipie_ecl.macSlimDataset(indata, fieldlist,outputfieldlist)); // strip down to only the columns needed
  
  LOADXML('<xml/>');
  //#DECLARE(EntityHash) #SET(EntityHash, 'HASH64(LEFT.' + REGEXREPLACE(',', fieldlist, ' + \'|\' + LEFT.') + ')');
  #DECLARE(EntityHash) #SET(EntityHash, 'LEFT.' + REGEXREPLACE(',', fieldlist, ' + \'|\' + LEFT.'));
  
  d1 := PROJECT(d, TRANSFORM({RECORDOF(LEFT), STRING EntityHash}, SELF.EntityHash := %EntityHash%, SELF := LEFT));
  d2 := RAMPS_Functions.macPivotLiteV2(d1, EntityHash, outputfieldlist);
  d3 := PROJECT(d2, TRANSFORM({RECORDOF(LEFT), STRING indicatortype, STRING indicatordescription, STRING Label, STRING FieldType, INTEGER RiskLevel}, 

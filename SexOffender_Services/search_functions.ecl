import AutoStandardI, doxie, doxie_raw, FCRA, iesp, moxie_phonesplus_server, paw_services, 
...
EXPORT search_functions := MODULE
	...
	shared fnSearchWeAlsoFoundData(unsigned1 GLBPurpose, unsigned1 DPPAPurpose, dataset(doxie.layout_references) l) := function 
			iesp.sexualoffender.t_OffenderAlsoFound append_wealsofound(doxie.layout_references l) := TRANSFORM
				...
				self.ProfessionalLicense := count(CHOOSEN(Prof_LicenseV2.Key_Proflic_Did ()(keyed(did=l.did)),255));
				self.EmploymentRelated := paw_services.PAW_Raw.getPAWcount(l.did,GLBPurpose,DPPAPurpose,255);
				self.Phones := Count(moxie_phonesplus_server.phonesplus_did_records(dataset([{l.did}],doxie.layout_references),
																					...
																					GLBPurpose,
																					DPPAPurpose,,TRUE).w_timezoneSeenDt);
				self := l;
			END;
	...
	export fnSearchVal(dataset(Sexoffender_Services.Layouts.raw_rec) in_recs, 
		...
		SexOffender_Services.Layouts.t_OffenderRecord_plus spk_file_xform(SexOffender_Services.Layouts.raw_rec l) := transform
			...
			self.WeAlsoFound := if(~isFCRA and in_mod.include_wealsofound,fnSearchWeAlsoFoundData(in_mod.glbpurpose, in_mod.DPPAPurpose, dataset([{(unsigned)l.did}],doxie.layout_references))[1]);

...
EXPORT ConstantsDeltaBase := MODULE
	...
	EXPORT Join_Columns :=
		' k.delta_key_id,k.date_added,k.report_code,k.jurisdiction_state,k.jurisdiction,k.case_identifier,k.state_report_number,' +
		...
		'k.report_link_id,k.report_type_id,k.contrib_source,k.officer_id,k.vendor_report_id,k.date_report_submitted,' +
		'p.delta_key_id as pdelta_key_id,p.person_type,p.name_first,p.name_middle,p.name_last,p.name_suffix,p.dob,p.driver_license_number,' +
		'p.address,p.city,p.state,p.zip_code,p.vehicle_unit_number,k.report_id,k.incident_id,k.releasable,k.is_deleted,' +
		'v.vin,v.tag_number,v.unit_number';

	// Used when we only filter by report fields.
	...
	// We need "OR v.unit_number IS NULL" because according to the logic that builds payload data if vehicle has empty unit_number.
	// We want to assign that vehicle to ALL of the person records
	EXPORT Join_FROM :=
		' FROM delta_ec.delta_key k' +
	...
	// Used when we filter by person and/or vehicle fields and want to also return the non-matching rest of the parties from the reports that
	...
	SHARED ImageRetrievalSqlJoin :=
		'LEFT JOIN delta_ec.delta_person AS p use index(p_delta_key_id_idx) ON p.delta_key_id = k.delta_key_id' +
		' LEFT JOIN delta_ec.delta_vehicle AS v	use index(v_delta_key_id_idx) ON v.delta_key_id = k.delta_key_id' +
		' AND p.vehicle_unit_number = v.unit_number';

...
EXPORT Constants := MODULE
		...
		EXPORT MAX_SOURCES_PER_AGENCY := 20;
		...
		EXPORT VENDOR_HPD_ESP := 'HPD';
		EXPORT VENDOR_HPD := 'HPD';
		EXPORT DATA_SOURCE_IYETEK := 'IyeTek';
		EXPORT DATA_SOURCE_CRU := 'CRU';
		EXPORT REPORT_CODE_ACCIDENT := 'A';
		EXPORT REPORT_CODE_KY  := 'TM';
		EXPORT REPORT_CODE_HPD  := 'TM';
		EXPORT AGENCY_FLAG_APPEND := 'AP';
		EXPORT AGENCY_FLAG_OVERWRITE := 'OW';
		EXPORT KY_APPRISS_SOURCE_ID := 'A';
		EXPORT HPD_SOURCE_ID := 'HP';
		EXPORT KY_STATE_ABBR := 'KY';
		EXPORT TX_STATE_ABBR := 'TX';
		EXPORT ERROR_STRING_SOAP_HPD := 'HPD Gateway Soap Error:';
		EXPORT GW_RETRIES := 1;
        EXPORT GW_TIMEOUT := 5;

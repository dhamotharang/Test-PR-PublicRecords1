...
EXPORT DocumentRetrievalService() := FUNCTION
  ...
  END;
	...
	// The response for non-releasable.
	iesp.retrievedocument.t_ECrashRetrieveDocumentResponse._Header ExceptionImageNonReleasable := TRANSFORM
		SELF.Status := ErrorCodeImageNonReleasable;
		SELF.Exceptions := DATASET([
			{'Roxie', 
				ErrorCodeImageNonReleasable, 
				'eCrashServices.DocumentRetrievalService', 
				'Image is non-releasable'
			}
		], iesp.share.t_WsException);
		SELF := [];
	END;
	
  HeaderImageNonReleasable := ROW(ExceptionImageNonReleasable);
	
	// Figure out if the report is non-releasable.
	...
	IsImageNonReleasable := ReportsAll[1].Releasable != '1';
	
	ResponseHeader := MAP(IsImageTooLarge => HeaderImageOverflow,
	                      IsImageNonReleasable => HeaderImageNonReleasable,
		                    EmptyHeader);

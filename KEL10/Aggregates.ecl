...
EXPORT Aggregates := MODULE
    ...
    ENDMACRO;
     ...
     EXPORT SlopeN(inset, field1, field2) := FUNCTIONMACRO
        ...
        LOCAL __var := Kel.Aggregates.AggN(VARIANCE, __filtered, field1);
    ...
    EXPORT ConcatN(inset, __leftFS, delim) := FUNCTIONMACRO
        ...
        LOCAL __rowSet := ROLLUP(__proj, TRUE, TRANSFORM({STRING out}, SELF.out := LEFT.out + IF(LEFT.out <> '' AND RIGHT.out <> '', __T(delim), '') + RIGHT.out));

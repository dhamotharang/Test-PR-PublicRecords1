...
EXPORT FromNone(DATASET(Layout_in_file) h) := MODULE
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_ln_name_type_desc_Invalid << 22 ) + ( le.corp_address1_effective_date_Invalid << 23 ) + ( le.corp_filing_date_Invalid << 25 ) + ( le.corp_inc_state_Invalid << 27 ) + ( le.corp_inc_date_Invalid << 28 ) + ( le.corp_foreign_domestic_ind_Invalid << 30 ) + ( le.corp_forgn_state_cd_Invalid << 31 ) + ( le.corp_forgn_state_desc_Invalid << 32 ) + ( le.corp_forgn_date_Invalid << 33 ) + ( le.corp_orig_org_structure_desc_Invalid << 35 ) + ( le.corp_for_profit_ind_Invalid << 36 ) + ( le.corp_agent_assign_date_Invalid << 37 ) + ( le.recordorigin_Invalid << 39 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_filing_date_Invalid := (le.ScrubsBits1 >> 25) & 3;
    ...
    SELF.corp_agent_assign_date_Invalid := (le.ScrubsBits1 >> 37) & 3;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_ln_name_type_desc:invalid_name_type_desc:ALLOW'
          ...
          ,'corp_orig_org_structure_desc:invalid_org_struc_desc:ENUM'

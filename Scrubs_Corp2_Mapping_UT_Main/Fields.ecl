IMPORT ut,SALT34;
IMPORT Scrubs_Corp2_Mapping_UT_Main,Scrubs; // Import modules for FieldTypes attribute definitions
EXPORT Fields := MODULE
 
// Processing for each FieldType
EXPORT SALT34.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_charter','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_name_type_code','invalid_name_type_desc','invalid_for_dom_ind','invalid_flag_code','invalid_date','invalid_recordorigin','invalid_alphablank','invalid_org_struc_desc');
EXPORT FieldTypeNum(SALT34.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_charter' => 2,'invalid_corp_vendor' => 3,'invalid_state_origin' => 4,'invalid_mandatory' => 5,'invalid_name_type_code' => 6,'invalid_name_type_desc' => 7,'invalid_for_dom_ind' => 8,'invalid_flag_code' => 9,'invalid_date' => 10,'invalid_recordorigin' => 11,'invalid_alphablank' => 12,'invalid_org_struc_desc' => 13,0);
...
EXPORT MakeFT_invalid_corp_vendor(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_vendor(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['49']);
EXPORT InValidMessageFT_invalid_corp_vendor(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('49'),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_state_origin(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_state_origin(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['UT']);
EXPORT InValidMessageFT_invalid_state_origin(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('UT'),SALT34.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_name_type_code(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_name_type_code(SALT34.StrType s,SALT34.StrType recordorigin) := WHICH(~Scrubs_Corp2_Mapping_UT_Main.Functions.invalid_name_type_code(s,recordorigin)>0);
EXPORT InValidMessageFT_invalid_name_type_code(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.CustomFail('Scrubs_Corp2_Mapping_UT_Main.Functions.invalid_name_type_code'),SALT34.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_name_type_desc(SALT34.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_invalid_name_type_desc(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,' -.ABCDEFGHIJKLMNOPQRSTUVWXYZ'))));
EXPORT InValidMessageFT_invalid_name_type_desc(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars(' -.ABCDEFGHIJKLMNOPQRSTUVWXYZ'),SALT34.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_org_struc_desc(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_org_struc_desc(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN Scrubs_Corp2_Mapping_UT_Main.Functions.set_valid_org_desc);
EXPORT InValidMessageFT_invalid_org_struc_desc(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('Scrubs_Corp2_Mapping_UT_Main.Functions.set_valid_org_desc'),SALT34.HygieneErrors.Good);
...
EXPORT Make_corp_orig_org_structure_desc(SALT34.StrType s0) := MakeFT_invalid_org_struc_desc(s0);
EXPORT InValid_corp_orig_org_structure_desc(SALT34.StrType s) := InValidFT_invalid_org_struc_desc(s);
EXPORT InValidMessage_corp_orig_org_structure_desc(UNSIGNED1 wh) := InValidMessageFT_invalid_org_struc_desc(wh);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT34,Scrubs_Corp2_Mapping_UT_Main;

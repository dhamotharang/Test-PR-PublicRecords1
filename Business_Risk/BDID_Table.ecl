import Business_Header, Gong, mdr;
...
bdid_tbl_init := project(Business_Header.BH_BQI_Stats, transform(Layout_BDID_Risk_Table, self := left));

// Get Gong History Stats
gh := Gong.File_Gong_History_Full(bdid <> 0);
...
bdid_tbl_stat := join(bdid_tbl_gong,
					  ...
					  left outer,
					  hash) : persist('~thor_data400::persist::business_risk::bdid_table');

import risk_indicators, DID_Add, address;

export Business_Shell_Function(dataset(Business_Risk.Layout_Output) biid, unsigned1 glb) := function
...
prof_risk := if(production_realtime_mode, Business_Risk.getBDIDTable(biid), business_risk.getBDIDTable_Hist(biid, glb));

IMPORT DCAV2, BIPV2, iesp, MDR, TopBusiness_Services, ut, std;
...
EXPORT LNCASection := MODULE
 ...
 EXPORT fn_FullView(
  ...
  ds_dca_recs := DCAV2.Key_LinkIds.kFetch(ds_in_unique_ids_only, , FETCH_LEVEL); 
  ...
  comp_lnca_info := 
    JOIN(dca_rec, DCAV2.Keys().EntNumNonFilt.qa,
  ...
  ultParent := DATASET([{ultimateEntNumber,'',0,0}], slim_hierRec);
  ...
  fullHierSet := 
    JOIN(hierSet, DCAV2.Keys().EntNum.qa,
  ...
  outRec := IF(EXISTS(comp_lnca_group),
    DENORMALIZE(outXML,comp_lnca_group,LEFT.dummyGroupField = RIGHT.dummyGroupField,GROUP,

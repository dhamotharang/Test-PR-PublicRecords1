IMPORT AutoStandardI, Workers_Compensation,Insurance_Certification, BIPV2, iesp, MDR, ut, TopBusiness_Services;
...
EXPORT RiskMetricsSection := MODULE
 ...
 EXPORT fn_FullView(
   dataset(SupplierRisk_Services.SupplierRisk_Layouts.rec_input_ids) ds_in_ids
	...
	// *** Key fetch to get insurance cert linking data from BIP2 linkids key file.
  ds_ic_recs := Insurance_Certification.Key_LinkIds.KFetch(
	...
	ic_sort := DEDUP(SORT(ic_recs_level(LNInsCertRecordID != 0),LNInsCertRecordID,-date_firstseen,-dateadded,RECORD),lninscertrecordid);
	...
	// *** Key fetch to get wokers comp linking data from BIP2 linkids key file.
  ds_wc_recs := Workers_Compensation.Key_LinkIds.KFetch(
	                         ds_in_unique_ids_only, // input file to join key with
													 FETCH_LEVEL); // level of ids to join with
							  					 // 3rd parm is ScoreThreshold, take default of 0
	...
	iesp.riskmetrics.t_RiskMetricsRecord format_iesp(riskmet_slim_rec l, DATASET(riskmet_slim_rec) allrows) := TRANSFORM
		SELF.SelfInsurances := PROJECT(CHOOSEN(DEDUP(allrows(itype=SupplierRisk_Services.Constants.risk_insureCert),ALL),iesp.constants.RISK_METRIC.MaxSelfInsurance),format_iesp_selfIns(LEFT));
		workers_recs := PROJECT(allrows(itype=SupplierRisk_Services.Constants.risk_workersComp),format_iesp_workComp(LEFT));
		SELF.WorkersInsComps := CHOOSEN(DEDUP(SORT(workers_recs,-EffectiveDate.year,-EffectiveDate.month,-EffectiveDate.day,RECORD),RECORD),iesp.constants.RISK_METRIC.MaxWorkersComp);

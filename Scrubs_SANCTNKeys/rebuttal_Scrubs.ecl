...
EXPORT FromNone(DATASET(rebuttal_Layout_SANCTNKeys) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.batch_number_Invalid := rebuttal_Fields.InValid_batch_number((SALT38.StrType)le.batch_number);
    SELF.incident_number_Invalid := rebuttal_Fields.InValid_incident_number((SALT38.StrType)le.incident_number);
    SELF.party_number_Invalid := rebuttal_Fields.InValid_party_number((SALT38.StrType)le.party_number);
    SELF.order_number_Invalid := rebuttal_Fields.InValid_order_number((SALT38.StrType)le.order_number);
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    AnyRule_WithErrorsCount := COUNT(GROUP, h.batch_number_Invalid > 0 OR h.incident_number_Invalid > 0 OR h.party_number_Invalid > 0 OR h.order_number_Invalid > 0);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.batch_number_Total_ErrorCount > 0, 1, 0) + IF(le.incident_number_ALLOW_ErrorCount > 0, 1, 0) + IF(le.party_number_ALLOW_ErrorCount > 0, 1, 0) + IF(le.order_number_ALLOW_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.batch_number_ALLOW_ErrorCount > 0, 1, 0) + IF(le.batch_number_LENGTH_ErrorCount > 0, 1, 0) + IF(le.incident_number_ALLOW_ErrorCount > 0, 1, 0) + IF(le.party_number_ALLOW_ErrorCount > 0, 1, 0) + IF(le.order_number_ALLOW_ErrorCount > 0, 1, 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.FieldContents := CHOOSE(c,(SALT38.StrType)le.batch_number,(SALT38.StrType)le.incident_number,(SALT38.StrType)le.party_number,(SALT38.StrType)le.order_number,'***SALTBUG***');
  END;
  EXPORT AllErrors := NORMALIZE(h,4,Into(LEFT,COUNTER));
   bv := TABLE(AllErrors,{FieldContents, FieldName, Cnt := COUNT(GROUP)},FieldContents, FieldName,MERGE);
  EXPORT BadValues := TOPN(bv,1000,-Cnt);
  // Particular form of stats required for Orbit
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(rebuttal_Layout_SANCTNKeys) prevDS = DATASET([], rebuttal_Layout_SANCTNKeys), STRING10 Src='UNK'):= FUNCTION
  // field error stats
    SALT38.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,rebuttal_Fields.InvalidMessage_order_number(1)
          ,'Fields with errors'
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ...
          ,le.incident_number_ALLOW_ErrorCount
          ,le.party_number_ALLOW_ErrorCount
          ,le.order_number_ALLOW_ErrorCount,0) / le.TotalCnt + 0.5, CHOOSE(c - NumRules
    ...
    mod_hygiene := rebuttal_hygiene(PROJECT(h, rebuttal_Layout_SANCTNKeys));
    ...
    SALT38.ScrubsOrbitLayout xNormHygieneStats(hygiene_summaryStats le, UNSIGNED c, STRING suffix) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'party_text:' + getFieldTypeText(h.party_text) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix,'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
      ...
      SELF.rulepcnt := CHOOSE(c
          ...
          ,le.populated_party_text_pcnt,0);
      SELF.ErrorMessage := '';
    END;
  ...
  // record count stats
    ...
    mod_Delta := rebuttal_Delta(prevDS, PROJECT(h, rebuttal_Layout_SANCTNKeys));
...
EXPORT StandardStats(DATASET(rebuttal_Layout_SANCTNKeys) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  myTimeStamp := (UNSIGNED6)SALT38.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
  expandedFile := FromNone(inFile).ExpandedInfile;
  mod_fromexpandedOverall := FromExpanded(expandedFile);
  scrubsSummaryOverall := mod_fromexpandedOverall.SummaryStats;
 
  SALT38.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_SANCTNKeys, rebuttal_Fields, 'RECORDOF(scrubsSummaryOverall)', '');

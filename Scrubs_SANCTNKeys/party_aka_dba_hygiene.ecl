...
SALT33.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT33.StrType)le.batch_number),TRIM((SALT33.StrType)le.incident_number),TRIM((SALT33.StrType)le.party_number),TRIM((SALT33.StrType)le.record_type),TRIM((SALT33.StrType)le.order_number),TRIM((SALT33.StrType)le.name_type),TRIM((SALT33.StrType)le.last_name),TRIM((SALT33.StrType)le.first_name),TRIM((SALT33.StrType)le.middle_name),TRIM((SALT33.StrType)le.aka_dba_text)));
  SELF.FldNo := C;
END;
...
SALT33.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 10);
  SELF.FldNo2 := 1 + (C % 10);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT33.StrType)le.batch_number),TRIM((SALT33.StrType)le.incident_number),TRIM((SALT33.StrType)le.party_number),TRIM((SALT33.StrType)le.record_type),TRIM((SALT33.StrType)le.order_number),TRIM((SALT33.StrType)le.name_type),TRIM((SALT33.StrType)le.last_name),TRIM((SALT33.StrType)le.first_name),TRIM((SALT33.StrType)le.middle_name),TRIM((SALT33.StrType)le.aka_dba_text)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT33.StrType)le.batch_number),TRIM((SALT33.StrType)le.incident_number),TRIM((SALT33.StrType)le.party_number),TRIM((SALT33.StrType)le.record_type),TRIM((SALT33.StrType)le.order_number),TRIM((SALT33.StrType)le.name_type),TRIM((SALT33.StrType)le.last_name),TRIM((SALT33.StrType)le.first_name),TRIM((SALT33.StrType)le.middle_name),TRIM((SALT33.StrType)le.aka_dba_text)));
  END;
...
SHARED FldIds := DATASET([{1,'batch_number'}
      ...
      ,{8,'first_name'}
      ,{9,'middle_name'}
      ,{10,'aka_dba_text'}],SALT33.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    party_aka_dba_Fields.InValid_batch_number((SALT33.StrType)le.batch_number),
    party_aka_dba_Fields.InValid_incident_number((SALT33.StrType)le.incident_number),
    party_aka_dba_Fields.InValid_party_number((SALT33.StrType)le.party_number),
    party_aka_dba_Fields.InValid_record_type((SALT33.StrType)le.record_type),
    party_aka_dba_Fields.InValid_order_number((SALT33.StrType)le.order_number),

...
EXPORT FromNone(DATASET(party_aka_dba_Layout_SANCTNKeys) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.batch_number_Invalid := party_aka_dba_Fields.InValid_batch_number((SALT33.StrType)le.batch_number);
    SELF.incident_number_Invalid := party_aka_dba_Fields.InValid_incident_number((SALT33.StrType)le.incident_number);
    SELF.party_number_Invalid := party_aka_dba_Fields.InValid_party_number((SALT33.StrType)le.party_number);
    SELF.order_number_Invalid := party_aka_dba_Fields.InValid_order_number((SALT33.StrType)le.order_number);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.FieldContents := CHOOSE(c,(SALT33.StrType)le.batch_number,(SALT33.StrType)le.incident_number,(SALT33.StrType)le.party_number,(SALT33.StrType)le.order_number,(SALT33.StrType)le.name_type,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT33.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'name_type:Invalid_NameType:ENUM','UNKNOWN');
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,party_aka_dba_Fields.InvalidMessage_name_type(1),'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.name_type_ENUM_ErrorCount,0);
      SELF.rulepcnt := 100 * CHOOSE(c
          ,le.batch_number_ALLOW_ErrorCount,le.batch_number_LENGTH_ErrorCount
          ,le.incident_number_ALLOW_ErrorCount
          ,le.party_number_ALLOW_ErrorCount
          ,le.order_number_ALLOW_ErrorCount
          ,le.name_type_ENUM_ErrorCount,0) / le.TotalCnt + 0.5;
    END;

...
EXPORT SALT33.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'Invalid_Batch','Invalid_Num','Invalid_LicenseNumber','Invalid_ClnLicenseNumber','Invalid_State','Invalid_LicenseType');
EXPORT FieldTypeNum(SALT33.StrType fn) := CASE(fn,'Invalid_Batch' => 1,'Invalid_Num' => 2,'Invalid_LicenseNumber' => 3,'Invalid_ClnLicenseNumber' => 4,'Invalid_State' => 5,'Invalid_LicenseType' => 6,0);
...
EXPORT MakeFT_Invalid_LicenseNumber(SALT33.StrType s0) := FUNCTION
  s1 := SALT33.stringfilter(s0,'0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz .-#'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_LicenseNumber(SALT33.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz .-#'))));
EXPORT InValidMessageFT_Invalid_LicenseNumber(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInChars('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz .-#'),SALT33.HygieneErrors.Good);
EXPORT MakeFT_Invalid_ClnLicenseNumber(SALT33.StrType s0) := FUNCTION
  s1 := SALT33.stringfilter(s0,'0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_ClnLicenseNumber(SALT33.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'))));
EXPORT InValidMessageFT_Invalid_ClnLicenseNumber(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInChars('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'),SALT33.HygieneErrors.Good);
...
EXPORT MakeFT_Invalid_LicenseType(SALT33.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_Invalid_LicenseType(SALT33.StrType s) := WHICH(~Scrubs_SANCTNKeys.fn_CodeCheck_Licence(s)>0);
EXPORT InValidMessageFT_Invalid_LicenseType(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.CustomFail('Scrubs_SANCTNKeys.fn_CodeCheck_Licence'),SALT33.HygieneErrors.Good);
EXPORT SALT33.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'batch_number','incident_number','party_number','record_type','order_number','license_number','license_type','license_state','cln_license_number','std_type_desc');
EXPORT FieldNum(SALT33.StrType fn) := CASE(fn,'batch_number' => 0,'incident_number' => 1,'party_number' => 2,'record_type' => 3,'order_number' => 4,'license_number' => 5,'license_type' => 6,'license_state' => 7,'cln_license_number' => 8,'std_type_desc' => 9,0);
...
EXPORT Make_license_number(SALT33.StrType s0) := MakeFT_Invalid_LicenseNumber(s0);
EXPORT InValid_license_number(SALT33.StrType s) := InValidFT_Invalid_LicenseNumber(s);
EXPORT InValidMessage_license_number(UNSIGNED1 wh) := InValidMessageFT_Invalid_LicenseNumber(wh);
EXPORT Make_license_type(SALT33.StrType s0) := MakeFT_Invalid_LicenseType(s0);
EXPORT InValid_license_type(SALT33.StrType s) := InValidFT_Invalid_LicenseType(s);
EXPORT InValidMessage_license_type(UNSIGNED1 wh) := InValidMessageFT_Invalid_LicenseType(wh);
EXPORT Make_license_state(SALT33.StrType s0) := MakeFT_Invalid_State(s0);
EXPORT InValid_license_state(SALT33.StrType s) := InValidFT_Invalid_State(s);
EXPORT InValidMessage_license_state(UNSIGNED1 wh) := InValidMessageFT_Invalid_State(wh);
EXPORT Make_cln_license_number(SALT33.StrType s0) := MakeFT_Invalid_ClnLicenseNumber(s0);
EXPORT InValid_cln_license_number(SALT33.StrType s) := InValidFT_Invalid_ClnLicenseNumber(s);

...
#workunit('name','Scrubs_Calbus.raw_BWR_Hygiene - Hygiene & Stats - SALT V3.11.4');
IMPORT Scrubs_Calbus,SALT311;
// First create an instantiated hygiene module
  infile := Scrubs_Calbus.raw_In_Calbus;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_Calbus.raw_hygiene(ip);
  ...
  OUTPUT(SALT311.MAC_Character_Counts.EclRecord(p,'raw_Layout_Calbus'),NAMED('OptimizedLayout'));// File layout suggested by data

...
EXPORT raw_Delta(DATASET(raw_Layout_Calbus)old_s, DATASET(raw_Layout_Calbus) new_s) := MODULE//Routines to compute the differences between two instances of a file
  SHARED inFieldList := ['district_branch','account_number','sub_account_number','district','account_type','firm_name','owner_name','business_street','business_city','business_state','business_zip_5','business_zip_plus_4','business_foreign_zip','business_country_name','start_date','ownership_code'];
  ...
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE) := FUNCTION
    ...
    SALT38.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_Calbus, raw_Fields, 'RECORDOF(hygieneDiffOverall)', FALSE);

...
#else
			MAP(n<=1 => sep+SALT35.GetRangeOfWords(s,n+1,wc,sep),
			    n=wc => SALT35.GetRangeOfWords(s,1,wc-1,sep)+sep,
					n>wc => SALT35.GetRangeOfWords(s,1,wc,sep),
					SALT35.GetRangeOfWords(s,1,n-1,sep)+sep+sep+SALT35.GetRangeOfWords(s,n+1,wc,sep));
#end

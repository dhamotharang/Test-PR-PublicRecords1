...
export File_TA_Valuations(string8 history_date) := function

ta_base1 := avm_v2.File_TA_BaseTable(history_date);
ta_base := distribute(ta_base1, hash(unformatted_apn,zip,prim_range,prim_name,predir,suffix,postdir,sec_range));

// sort by tax year desc to retain the most recent tax year record
ta_sorted := sort(ta_base, unformatted_apn, zip, prim_range, prim_name, predir, suffix, postdir, sec_range,-assessed_value_year, local); 
ta_deduped := dedup(ta_sorted, unformatted_apn, zip, prim_range, prim_name, predir, suffix, postdir, sec_range, local); 

ta_update := avm_v2.File_TA_UpdateTable(history_date);
...
layout_ta_record := record
	...
	// new fields needed for new accuracy table structure
	real market_ratio;
...
layout_ta_record add_valuation(ta_deduped le, ta_update rt) := transform
	...
	assessed_bin := map((integer)le.assessed_total_value<>0 and (integer)le.assessed_total_value <= (integer)rt.assess_bin1_cap => 'A1',
					  (integer)le.assessed_total_value<>0 and (integer)le.assessed_total_value <= (integer)rt.assess_bin2_cap => 'A2',
					  (integer)le.assessed_total_value<>0 and (integer)le.assessed_total_value <= (integer)rt.assess_bin3_cap => 'A3',
					  (integer)le.assessed_total_value<>0 and (integer)rt.assess_bin3_cap<>0 => 'A4',
					  '');		
						
	assessed_ratio := map(assessed_bin='A1' => rt.assessed_value_to_saleprice_ratio1,
				 assessed_bin='A2' => rt.assessed_value_to_saleprice_ratio2,
				 assessed_bin='A3' => rt.assessed_value_to_saleprice_ratio3, 
				 assessed_bin='A4' => rt.assessed_value_to_saleprice_ratio4, 	
				 0);
	self.assessed_ratio := assessed_ratio;
	self.Assessed_Value_Valuation := (integer)le.assessed_total_value * assessed_ratio;
																				 
				 
	market_bin := map((integer)le.market_total_value<>0 and (integer)le.market_total_value <= (integer)rt.market_bin1_cap => 'M1',
					  (integer)le.market_total_value<>0 and (integer)le.market_total_value <= (integer)rt.market_bin2_cap => 'M2',
					  (integer)le.market_total_value<>0 and (integer)le.market_total_value <= (integer)rt.market_bin3_cap => 'M3',
					  (integer)le.market_total_value<>0 and (integer)rt.market_bin3_cap<>0 => 'M4',
					  '');
				 
	market_ratio := map(market_bin='M1' => rt.market_value_to_saleprice_ratio1,
				 market_bin='M2' => rt.market_value_to_saleprice_ratio2,
				 market_bin='M3' => rt.market_value_to_saleprice_ratio3,
				 market_bin='M4' => rt.market_value_to_saleprice_ratio4,
				 0);
	self.market_ratio := market_ratio;
	self.Market_Value_Valuation := (integer)le.market_total_value * market_ratio;
...
output(with_valuation,,'~thor_data400::avm_v2::ta_valuations', __compressed__, overwrite);

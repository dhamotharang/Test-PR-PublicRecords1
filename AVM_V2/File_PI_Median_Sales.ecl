...
export File_PI_Median_Sales(string8 history_date) := function

// median sales price is only calculated after 2005Q2.  base table or ofheo are used prior to that
pi_file1 := avm_v2.File_AVM_PI_Records(history_date)(recording_date[1..6] > '199512');
...
props := group(sort(project(pi_file, transform(layout_msp_calcs, self := left, self := [])), quarter,fips_code,land_use,(integer)sales_price, local), quarter, fips_code, land_use, local);
...
output(final,,'~thor_data400::avm_v2::median_sales_price', __compressed__, overwrite);

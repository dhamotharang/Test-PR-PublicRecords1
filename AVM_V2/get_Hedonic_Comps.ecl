...
export get_Hedonic_Comps(string1 fc, string8 history_date) := function
...
hedonic_base := avm_v2.File_AVM_Hedonic_Base(history_date)(fips_code[1]=fc);

// made the comp candidates their own attribute so that they could be keyed off of fid for the query
comparable_candidates := avm_v2.File_Hedonic_Comps(fc, history_date);
...
layout_geobox_base := record
	...
	real w_Lon;
	real e_Lon;
	avm_v2.File_AVM_Hedonic_Base(history_date);
end;
...
swr add_box2(subject_prop_distr le, stats_distr rt) := transform
	...
	lat := (real)le.lat;
	lon := (real)le.long;
	radius := (real)subject_radius + 0.5;  // make the second box slightly bigger just in case.  we skip any outside the actual radius anyway
	miPerLat := 69.055;	// miles per degree of latitude (24859.82mi/360)
	miPerLong := abs(miPerLat * cos(lon)); 
	deltaLat := radius / miPerLat;
	deltaLon := radius / miPerLong;
...
layout_hedonic_vars add_weights(with_dist le, avm_v2.File_Hedonic_Weights_Table rt) := transform
...
with_all_weights := join(with_dist, avm_v2.File_Hedonic_Weights_Table,

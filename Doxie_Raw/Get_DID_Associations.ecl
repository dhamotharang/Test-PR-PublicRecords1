...
FUNCTION
	...
	Doxie_Raw.Layouts.RelDetailsTemp tRelsAssocFirstDeg(Doxie_Raw.Layouts.RelDetailsIn le,relativeLayout ri,INTEGER depth) :=
	TRANSFORM
		SELF.seq     				:= le.seq;
		SELF.src_did 				:= le.did;
		SELF.depth   				:= depth;
	...
	Doxie_Raw.Layouts.RelDetailsTemp tRels(Doxie_Raw.Layouts.RelDetailsTemp le,relativeLayout ri,INTEGER depth) :=
	TRANSFORM
		SELF.depth 					:= depth;
	...
	Doxie_Raw.Layouts.RelDetailsTemp tRollupSiblings(Doxie_Raw.Layouts.RelDetailsTemp le,Doxie_Raw.Layouts.RelDetailsTemp ri) :=
	TRANSFORM
	...
	Doxie_Raw.Layouts.DIDsOfIntWSubj tDIDsOfIntWSubj(Doxie_Raw.Layouts.RelDetailsIn le,Doxie_Raw.Layouts.RelDetailsIn ri) :=
	TRANSFORM
		SELF.seq     := le.seq;
		SELF.src_did := le.did;
		SELF.did     := ri.did;
	...
	Doxie_Raw.Layouts.RelDetailsTemp tDIDsOfIntFirstDeg(Doxie_Raw.Layouts.DIDsOfIntWSubj le,Doxie_Raw.Layouts.RelDetailsTemp ri) :=
	TRANSFORM
		SELF.seq                   := le.seq;
	...
	dRelsFirstDegSort := PROJECT(dRelsFirstDegGrp,
																TRANSFORM(Doxie_Raw.Layouts.RelDetailsTemp,SELF.p2_sort := COUNTER,SELF := LEFT));
	...
	// Flag records where we found a match in dids_of_interest
	Doxie_Raw.Layouts.RelDetailsTemp tDIDsOfIntSecondDeg(Doxie_Raw.Layouts.RelDetailsTemp le,Doxie_Raw.Layouts.RelDetailsTemp ri) :=
	TRANSFORM
		SELF.seq                   := le.seq;
		SELF.src_did               := le.src_did;
	...
	dRelsSecondDegSort := PROJECT(dRelsSecondDegGrp,
																TRANSFORM(Doxie_Raw.Layouts.RelDetailsTemp,SELF.p3_sort := COUNTER,SELF := LEFT));
  ...
  tdeg_recs := Relationship.proc_GetRelationshipNeutral(tdeg_dids,TRUE,TRUE,FALSE,FALSE,ut.limits.RELATIVES_PER_PERSON,,TRUE).result;
	...
	// Flag records where we found a match in dids_of_interest
	Doxie_Raw.Layouts.RelDetailsTemp tDIDsOfIntThirdDeg(Doxie_Raw.Layouts.RelDetailsTemp le,Doxie_Raw.Layouts.RelDetailsTemp ri) :=
	TRANSFORM
		SELF.seq                   := le.seq;
		SELF.src_did               := le.src_did;
		SELF.did_of_interest       := le.did_of_interest;
	...
	dRelsThirdDegSort := PROJECT(dRelsThirdDegGrp,
																TRANSFORM(Doxie_Raw.Layouts.RelDetailsTemp,SELF.p4_sort := COUNTER,SELF := LEFT));
	...
	Doxie_Raw.Layouts.RelDetailsTemp tDIDsOfIntAssocs(Doxie_Raw.Layouts.RelDetailsTemp le,Doxie_Raw.Layouts.RelDetailsTemp ri) :=
	TRANSFORM
		SELF.seq                   := le.seq;
		SELF.src_did               := le.src_did;
		SELF.did_of_interest       := le.did_of_interest;
	...
	dSubjDIDsNoRels := JOIN(dDIDsOfIntWSubj,dRelAssocs,
													LEFT.seq = RIGHT.seq,
													TRANSFORM(Doxie_Raw.Layouts.RelDetailsOut,SELF.person2 := LEFT.did,SELF := LEFT,SELF := []),
													LEFT ONLY);
	...
	dAllRelAssocs := IF(calculateDIDAssociations,dDIDsOfIntNoRels,dSubjDIDsNoRels) +
										PROJECT(dRelAssocs,
														TRANSFORM(Doxie_Raw.Layouts.RelDetailsOut,

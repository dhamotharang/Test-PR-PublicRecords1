import ut, doxie, doxie_crs, domains;
...
export doxie_crs.layout_report Entrp_Central_records(doxie_crs.layout_report L) := TRANSFORM
  ...
  self.property_children                  := property_entrp;
	...
	plrr_active := L.professional_licenses_children(expiration_date > ut.IndustryClass.sys_date);
	plrr_inactive := L.professional_licenses_children(expiration_date <= ut.IndustryClass.sys_date);
  ...
  // self.liens_judgements_v2_children       := lier2_entrp;
	...
	dea_active := L.dea_children(expiration_date>ut.IndustryClass.sys_date);
	dea_inactive := L.dea_children(expiration_date<=ut.IndustryClass.sys_date);
  ...
  // self.phones_old_children                := global(phOld);
	...
	wtcr_active := L.watercraft_children(registration_expiration_date > ut.IndustryClass.sys_date);
	wtcr_inactive := L.watercraft_children(registration_expiration_date <= ut.IndustryClass.sys_date);
  ...
  self.watercraft_children                := if(ut.industryClass.EntDateVal=1,wtcr_curr,wtcr_active+wtcr_entrp);
	...
	idom_active := idom_dt(expiredate1 >ut.IndustryClass.sys_date);
	idom_inactive := idom_dt(expiredate1 <=ut.IndustryClass.sys_date);
  ...
  self.Email_children                      := email_entrp;
	...
	vehi_active := L.vehicle_children(registration_expiration_date >ut.IndustryClass.sys_date);
	vehi_inactive := L.vehicle_children(registration_expiration_date >ut.IndustryClass.sys_date);
	...
	dlsr_active := L.drivers_licenses_children((string)expiration_Date>ut.IndustryClass.sys_date);
	dlsr_inactive := L.drivers_licenses_children((string)expiration_Date<=ut.IndustryClass.sys_date);
	...
	dlsr2_active := L.drivers_licenses2_children((string)expiration_Date>ut.IndustryClass.sys_date);
	dlsr2_inactive := L.drivers_licenses2_children((string)expiration_Date<=ut.IndustryClass.sys_date);

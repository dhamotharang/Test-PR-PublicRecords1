...
rPopulationStats_Experian_base
 :=
  record
    state										:= pExperian_base.Orig_State;
	...
	aid_raw_CountNonBlank						:= sum(group,if(pExperian_base.rawAIDin<>0,1,0));
	aid_clean_CountNonBlank						:= sum(group,if(pExperian_base.cleanaid<>0,1,0));
	...
	//---Original data
	Encrypted_Experian_PIN_CountNonBlank		:= sum(group,if(pExperian_base.Orig_PID<>'',1,0));
	Address_ID_CountNonBlank					:= sum(group,if(pExperian_base.Address_ID<>'',1,0));
	Orig_Social_Security_Number_CountNonBlank	:= sum(group,if(pExperian_base.Orig_SSN<>'',1,0));
	Orig_Date_of_Birth_CountNonBlank			:= sum(group,if(pExperian_base.Orig_Date_of_Birth<>'',1,0));
	Orig_Telephone_CountNonBlank				:= sum(group,if(pExperian_base.Orig_Phone_Num<>'',1,0));
	Orig_Gender_CountNonBlank					:= sum(group,if(pExperian_base.Orig_Gender<>'',1,0));
...
STRATA.createXMLStats(dPopulationStats_Experian_base
					 ...
					 ,'ExperianIRSG'
					 ,pVersion
					 ,'nirav.bhattlexisnexis.com'
					 ,zExperian_base);

EXPORT Strata_Stat_ExperianIRSG_Build := zExperian_base;
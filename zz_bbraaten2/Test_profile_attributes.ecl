...
EXPORT Test_profile_attributes (ds, at_name, min_, Max_, flag = false, myset = ['','','','','']) := FUNCTIONMACRO
// EXPORT Test_profile_attributes (ds, at_name, min_, Max_, flag = false) := FUNCTIONMACRO
...
output(full_min, named('full_min'));

	neg_one := -1;
	zero := 0;
	// bubble := 0.000001;
	
//some logic like bin all defaults and counds in table.... ah haha.. got it tables are nice. 
	output(myset, named('myset'));


ds_defaults_in_base := ds_slim_p(#expand(at_name) in myset );

output(choosen(ds_defaults_in_base, 5), named('ds_defaults_in_base'));

ds_fine_no_defaults := ds_slim_p(#expand(at_name) not in myset );

sorted_ds_fine_no_defaults := SORT(ds_fine_no_defaults, (real)#expand(at_name));


output(choosen(sorted_ds_fine_no_defaults, 5), named('sorted_ds_fine_no_defaults'));


table_defaults := table(ds_defaults_in_base, {#expand(at_name),
																				Total := count(group)},
																						#expand(at_name));
	
	// output(table_defaults, named('table_defaults'));
...
ds_sort_table2 := sort(table_defaults, #expand(at_name));

table_lay2 table_trans2(ds_sort_table2 le, integer c) := transform
	self.bin_lower_limit := ds_sort_table2[c].#expand(at_name);
	self.bin_upper_limit := ds_sort_table2[c].#expand(at_name);
	self.Bin_cnt_slm := (integer)ds_sort_table2[c].Total;
	self.bin_pct_slm := (real)(ds_sort_table2[c].Total/slim_count_p)*100;
end;

ds_table2 := project(ds_sort_table2, table_trans2(left, counter));


table_lay2 table_trans4(ds_table2 le, integer c) := transform
	self.bin_lower_limit := ds_table2[c].bin_lower_limit;
	self.bin_upper_limit := ds_table2[c].bin_upper_limit;
	self.Bin_cnt_slm := (integer)ds_table2[c].Bin_cnt_slm;
	self.bin_pct_slm := (real)ds_table2[c].bin_pct_slm;
end;
...
slim_profile2 := record
	...
	dataset(table_lay2) tbl2;	
	end;

slim_profile2 trans2(ut.ds_oneRecord le, integer c) :=  TRANSFORM
		SELF.name := at_name;
		SELF.count_total := slim_count_p;
		self.full_range := Min_ + ' ' +'-' + ' ' + max_;  
			  	self.tbl2 := project(ds_table2, table_trans4(left, counter));	
end;


dset_slim2 := project(ut.ds_oneRecord, trans2(left, counter));



slim_profile_quick2 := record
	string name;
	integer count_total;
	string full_range;
		string bin_lower_limit;
...
slim_profile_quick2 NewChildren2(dset_slim le, integer c) := TRANSFORM
		SELF.name := le.name;
...
dset_slim_quick2 := NORMALIZE(dset_slim2,count(left.tbl),NewChildren2(left, counter));

output(dset_slim_quick2, named('default_dset_slim_quick'));
...
// -1 and 0 bin	
	...
	left_count := count(sorted_ds_fine_no_defaults);
	start_bin1 := count(ds_fine_no_defaults);
...
// output(choosen(table_of_vals, all), named('table_of_vals'));
// output(choosen(ds_table, all), named('ds_table'));
// output(choosen(dset_slim, all), named('dset_slim'));
...
// output(Bin1_max_pre, named('Bin1_max_pre'));
	...
	ds_test_1 := sorted_ds_p((real)#expand(at_name) > (real)min_ and (real)#expand(at_name)<= (real)Bin1_max_pre);
...
ds_less := sorted_ds_p((real)#expand(at_name) > (real)min_ and (real)#expand(at_name) < (real)Bin1_max_pre);
...
// output(ds_abs2, named('ds_abs2'));
 ...
 max_test_1 := if((ds_abs1 <= ds_abs2) and ((real)min_ <= max_less) , max_less, Bin1_max_pre);
...
max_1 := map((ds_full_cnt/slim_count_p) < 0.001 => Max_,
	...
	First_GRP := sorted_ds_p((real)#expand(at_name) > min_ and (real)#expand(at_name)<= max_1);	
...
//Bin 2
		...
		left_count2 := slim_count_p - Sum(start_bin1 + First_grp_cnt );

	second_start := Sum(start_bin1 + First_grp_cnt );
...
max_2 := map((ds_full_cnt2/slim_count_p) < 0.001 => Max_,
	...
	//Bin 3
		...
		left_count3 := slim_count_p - Sum(start_bin1 + First_grp_cnt + second_grp_cnt);

	third_start := Sum(start_bin1 + First_grp_cnt + second_grp_cnt);
...
max_3 := map((ds_full_cnt3/slim_count_p) < 0.001 => Max_,
	...
	left_count4 := slim_count_p - Sum(start_bin1 + First_grp_cnt + second_grp_cnt + third_grp_cnt);

	fourth_start := Sum(start_bin1 + First_grp_cnt + second_grp_cnt + third_grp_cnt);
...
// output(choosen(fourth_GRP, all), named('fourth_GRP'));
	...
	left_count5 := slim_count_p - Sum(start_bin1 + First_grp_cnt + second_grp_cnt + third_grp_cnt + fourth_grp_cnt);


		fifth_start := Sum(start_bin1 + First_grp_cnt + second_grp_cnt + third_grp_cnt + fourth_grp_cnt);

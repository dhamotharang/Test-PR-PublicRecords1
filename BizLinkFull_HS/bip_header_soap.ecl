...
EXPORT bip_header_soap := MACRO
  LOADXML(BizLinkFull_HS._Search.sFieldNames);
  ...
  dFormalized:=BizLinkFull_HS._Search.macFormalize(dInput);
  dInputFormatted:=PROJECT(dFormalized,BizLinkFull_HS.Process_Biz_Layouts.InputLayout);
  dResults:=BizLinkFull_HS.MEOW_Biz(dInputFormatted).Data_;
  ...
  dKeysUsed:=TABLE(dProxids,{KeysUsed;STRING keys:=BizLinkFull_HS.Process_Biz_Layouts.KeysUsedToText(KeysUsed);},KeysUsed);
  dKeyLegend:=BizLinkFull_HS.linkpaths;
  
  dRawResults:=BizLinkFull_HS.MEOW_Biz(dInputFormatted).Raw_Results;
  ...
  IF((BOOLEAN)Input_soapcallmode,
    ...
    SEQUENTIAL(
      IF(GroupProxID,OUTPUT(dAggregated,named('Header_Data_Grouped')),IF(bDebug,OUTPUT(BizLinkFull_HS.MEOW_Biz(dInputFormatted)),OUTPUT(dResults,named('Header_Data')))),

...
EXPORT KeyName := BizLinkFull_HS.Filename_keys.L_CNPNAME; /*HACK07*/
...
EXPORT RawFetch_server(TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := 
 ...
 //Generate service attributes for GSS join
    ...
    res := JOIN( steppedmatches, Key, KEYED(RIGHT.GSS_Hash = wds[1].hsh) AND LEFT.proxid = RIGHT.proxid AND LEFT.seleid = RIGHT.seleid AND LEFT.orgid = RIGHT.orgid AND LEFT.ultid = RIGHT.ultid,TRANSFORM(indexOutputRecord,SELF.gss_word_weight := LEFT.gss_word_weight,SELF := RIGHT));
...
#IF(#TEXT(Input_cnp_name)<>'')
  #uniquename(trans)
  BizLinkFull_HS.Key_BizHead_L_CNPNAME.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := BizLinkFull_HS.Key_BizHead_L_CNPNAME.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

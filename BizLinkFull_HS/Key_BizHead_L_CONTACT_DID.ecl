...
EXPORT KeyName := BizLinkFull_HS.Filename_keys.L_CONTACT_DID; /*HACK07*/
...
#IF(#TEXT(Input_contact_did)<>'')
  #uniquename(trans)
  BizLinkFull_HS.Key_BizHead_L_CONTACT_DID.InputLayout_Batch %trans%(InFile le) := TRANSFORM
    SELF.Reference := le.Input_Ref;
    SELF.contact_did := (TYPEOF(SELF.contact_did))le.Input_contact_did;
  END;
  #uniquename(p)
  %p% := PROJECT(Infile,%trans%(left));
  output_file := BizLinkFull_HS.Key_BizHead_L_CONTACT_DID.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

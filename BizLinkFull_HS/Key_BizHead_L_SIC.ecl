...
EXPORT KeyName := BizLinkFull_HS.Filename_keys.L_SIC; /*HACK07*/
...
EXPORT ScoredproxidFetch(TYPEOF(h.company_sic_code1) param_company_sic_code1 = (TYPEOF(h.company_sic_code1))'',DATASET(process_Biz_layouts.layout_zip_cases) param_zip,TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.company_sic_code1_match_code := MAP(le.company_sic_code1 = (TYPEOF(le.company_sic_code1))'' OR le.company_sic_code1 = (TYPEOF(le.company_sic_code1))'' => SALT33.MatchCode.OneSideNull,match_methods(File_BizHead).match_company_sic_code1(le.company_sic_code1,param_company_sic_code1,TRUE));
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.company_sic_code1_match_code := MAP(le.company_sic_code1 = (TYPEOF(le.company_sic_code1))'' OR le.company_sic_code1 = (TYPEOF(le.company_sic_code1))'' => SALT33.MatchCode.OneSideNull,match_methods(File_BizHead).match_company_sic_code1(le.company_sic_code1,ri.company_sic_code1,TRUE));
...
#IF(#TEXT(Input_company_sic_code1)<>'' AND #TEXT(Input_zip)<>'')
  #uniquename(trans)
  BizLinkFull_HS.Key_BizHead_L_SIC.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := BizLinkFull_HS.Key_BizHead_L_SIC.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

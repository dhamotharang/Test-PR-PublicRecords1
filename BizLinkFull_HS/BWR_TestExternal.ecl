...
#workunit('name','BizLinkFull_HS.BWR_TestExternal - Test External Linking - Precision and Recall - SALT V3.3.1');
IMPORT BizLinkFull_HS,SALT33;
// This is the 'thor only' version (no roxie)
  SmallJob := FALSE;
  Infile := BizLinkFull_HS.File_BizHead; // You can make this a different file - but must be in 'as header' format
  Mapping := 'UniqueID:rcid,zip_cases:zip';
  MyInfile := SALT33.FromFlat(InFile,process_Biz_layouts.InputLayout,Mapping);
  BizLinkFull_HS.MAC_Meow_Biz_Batch(myinfile,UniqueId,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,has_lgid,empid,source,source_record_id,source_docid,company_name,company_name_prefix,cnp_name,cnp_number,cnp_btype,cnp_lowv,company_phone,company_phone_3,company_phone_3_ex,company_phone_7,company_fein,company_sic_code1,active_duns_number,prim_range,prim_name,sec_range,city,city_clean,st,zip_cases,company_url,isContact,contact_did,title,fname,fname_preferred,mname,lname,name_suffix,contact_ssn,contact_email,sele_flag,org_flag,ult_flag,fallback_value,CONTACTNAME,STREETADDRESS,MyOutFile,SmallJob,Stats);
  R := { BOOLEAN Present; BOOLEAN Equal; BOOLEAN Resolved };
  R note(Infile le,MyOutFile ri) := TRANSFORM

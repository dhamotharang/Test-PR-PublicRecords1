...
dWithCnpName:=PROJECT(dCnpName,TRANSFORM(lFileBizHead,
  ...
  SELF.company_name_prefix:=BizLinkFull_HS.fn_company_name_prefix(LEFT.cnp_name);//[..5];
...
dBaseFile:=dWithCnpName+dAdditionalCity:PERSIST('~BizLinkFull_HS::persist::file_bizhead',EXPIRE(60));
...
l:=RECORD
  ...
  unsigned1 fallback_value;
 END;
// EXPORT File_BizHead:=DATASET('~BizLinkFull_HS::persist::file_bizhead',l,THOR);

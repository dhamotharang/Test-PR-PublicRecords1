...
//rcid2
EXPORT KeyName := BizLinkFull_HS.Filename_keys.L_RCID;
...
#IF(#TEXT(Input_rcid2)<>'')
  #uniquename(trans)
  BizLinkFull_HS.Key_BizHead_L_RCID.InputLayout_Batch %trans%(InFile le) := TRANSFORM
    SELF.Reference := le.Input_Ref;
    SELF.rcid2 := (TYPEOF(SELF.rcid2))le.Input_rcid2;
  END;
  #uniquename(p)
  %p% := PROJECT(Infile,%trans%(left));
  output_file := BizLinkFull_HS.Key_BizHead_L_RCID.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

...
EXPORT KeyName := BizLinkFull_HS.Filename_keys.L_ADDRESS2; /*HACK07*/
...
EXPORT ScoredproxidFetch(TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',DATASET(process_Biz_layouts.layout_zip_cases) param_zip,TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.company_sic_code1) param_company_sic_code1 = (TYPEOF(h.company_sic_code1))'',TYPEOF(h.cnp_number) param_cnp_number = (TYPEOF(h.cnp_number))'',TYPEOF(h.cnp_btype) param_cnp_btype = (TYPEOF(h.cnp_btype))'',TYPEOF(h.cnp_lowv) param_cnp_lowv = (TYPEOF(h.cnp_lowv))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.parent_proxid) param_parent_proxid = (TYPEOF(h.parent_proxid))'',TYPEOF(h.sele_proxid) param_sele_proxid = (TYPEOF(h.sele_proxid))'',TYPEOF(h.org_proxid) param_org_proxid = (TYPEOF(h.org_proxid))'',TYPEOF(h.ultimate_proxid) param_ultimate_proxid = (TYPEOF(h.ultimate_proxid))'',TYPEOF(h.sele_flag) param_sele_flag = (TYPEOF(h.sele_flag))'',TYPEOF(h.org_flag) param_org_flag = (TYPEOF(h.org_flag))'',TYPEOF(h.ult_flag) param_ult_flag = (TYPEOF(h.ult_flag))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.zip_match_code := MAP(le.zip = (TYPEOF(le.zip))'' OR ~EXISTS(param_zip) => SALT33.MatchCode.OneSideNull,match_methods(File_BizHead).match_zip(le.zip,param_zip,TRUE));
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.prim_nameWeight := (50+MAP ( le.prim_name = ri.prim_name  => le.prim_name_weight100,
          ...
          -1.000*le.prim_name_weight100))/100; 
    SELF.zip_match_code := MAP(le.zip = (TYPEOF(le.zip))'' OR ~EXISTS(ri.zip_cases) => SALT33.MatchCode.OneSideNull,match_methods(File_BizHead).match_zip(le.zip,ri.zip_cases,TRUE));
...
#IF(#TEXT(Input_prim_name)<>'' AND #TEXT(Input_zip)<>'')
  #uniquename(trans)
  BizLinkFull_HS.Key_BizHead_L_ADDRESS2.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := BizLinkFull_HS.Key_BizHead_L_ADDRESS2.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

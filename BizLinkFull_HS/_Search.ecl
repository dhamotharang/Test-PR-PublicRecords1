...
EXPORT _Search:=MODULE
  ...
  EXPORT lInputAugmented:=RECORD
    BizLinkFull_HS.Process_Biz_Layouts.InputLayout;
    UNSIGNED zip_radius;
  ...
  #SET(inChild,0)
  #DECLARE(f) #SET(f,'')
  #EXPORTXML(fields,BizLinkFull_HS.Process_Biz_Layouts.InputLayout)
  #FOR(fields)
    #FOR(Field)
      #IF(%inChild%=0)
        #APPEND(f,','+%'{@label}'%)
  ...
  EXPORT macFieldMap(d,f=''):=FUNCTIONMACRO
    ...
    #IF(#TEXT(f)='')
      a:=BizLinkFull_HS._Search.sFieldnames;
    #ELSE
      a:=REGEXREPLACE('(<fieldnames>)(<)',BizLinkFull_HS._Search.sFieldnames,'$1'+REGEXREPLACE('([^=]+)=([^,]+)',f,'<row field="$1" alternative="$2" placement="force"/>')+'$2');
    #END
    LOADXML(a);
    ...
    // added in shortly.
    #DECLARE(pbl) #SET(pbl,'PROJECT('+#TEXT(d)+',TRANSFORM(BizLinkFull_HS._Search.lInputAugmented,')
    #DECLARE(inChild) #SET(inChild,0)
    #EXPORTXML(fields,BizLinkFull_HS._Search.lInputAugmented)
    #FOR(fields)
      #FOR(Field)
        #IF(%inChild%=0)
          #APPEND(pbl,'SELF.'+%'{@label}'%+':=(TYPEOF(BizLinkFull_HS._Search.lInputAugmented.'+%'{@label}'%+'))')
          #IF(%'{@label}'% IN ['uniqueid','maxids','leadthreshold','zip_cases','matchrecords','fullmatch','entered_rcid','entered_proxid','entered_seleid','entered_orgid','entered_ultid','entered_powid','zip_radius','allow7digitmatch','hierarchicalsort','soapcallmode'])
            #APPEND(pbl,'___;\n')
          #ELSE
            #APPEND(pbl,'BizLinkFull_HS.Fields.Make_'+%'{@label}'%+'((STRING)___);\n')
          #END
        #END
        #IF(%'{@isDataset}'%='1' OR %'{@isEnd}'%='1')
          #SET(inChild,IF(%inChild%=1,0,1))
    ...
    // #SET(pbl,REGEXREPLACE('___',%'pbl'%,'\'\'')+'SELF.zip_weights:=DATASET([],BizLinkFull_HS.Process_Biz_Layouts.Zip_Weights_Layout);))');
    #SET(pbl,REGEXREPLACE('___',%'pbl'%,'\'\'')+'))');
    // fields ending in "_cases" are MULTIPLE fields, so we need to wrap those
    // into a dataset
    #SET(pbl,REGEXREPLACE('(SELF.[^_]+_cases:=)[^)]+[)][)]([^;]+)',%'pbl'%,'$1DATASET([{TRIM($2),0}],BizLinkFull_HS.Process_Biz_Layouts.layout_zip_cases)'))
    RETURN %'pbl'%;
  ENDMACRO;
  // Dataset showing the mappings being performed based on input
  EXPORT macShowMapping(d,f=''):=FUNCTIONMACRO
    sMapped:=BizLinkFull_HS._Search.macFieldMap(d,f);
  ...
  EXPORT macFormalize(d,f='',bAugment=TRUE,bZipExtrapolate=TRUE):=FUNCTIONMACRO
    ...
    dAugmented:=PROJECT(dCompany,TRANSFORM(BizLinkFull_HS._Search.lInputAugmented,
      ...
      SELF.zip_cases:=MAP(
        ~bZipExtrapolate AND TRIM(LEFT.zip_cases[1].zip)='' => DATASET([],BizLinkFull_HS.Process_Biz_Layouts.layout_zip_cases),
        ~bZipExtrapolate => DATASET([{TRIM(LEFT.zip_cases[1].zip),100}],BizLinkFull_HS.Process_Biz_Layouts.layout_zip_cases),
        dZips[1].zip='' => DATASET([],BizLinkFull_HS.Process_Biz_Layouts.layout_zip_cases),
        PROJECT(dZips,TRANSFORM(BizLinkFull_HS.Process_Biz_Layouts.layout_zip_cases,SELF.weight:=100-((LEFT.radius/Input_zip_radius)*50);SELF:=LEFT;))
      );
      SELF.st:=dZips[1].state;
      SELF.company_name_prefix:=BizLinkFull_HS.fn_company_name_prefix(LEFT.cnp_name);
      ...
      SELF.fname_preferred:=BizLinkFull_HS.fn_PreferredName(LEFT.fname);
...
//      SELF.LeadThreshold:=10;
  ...
  EXPORT BatchSubmit(DATASET(lInputAugmented) d,BOOLEAN bIncludeFilter=FALSE):=FUNCTION
    //#EXPAND('BizLinkFull_HS.MAC_MEOW_Biz_Batch('+#TEXT(d)+','+REGEXREPLACE('input_company_name_prefix',sMEOWParameters,'',NOCASE)+',dResults,,,bIncludeFilter)');
    #EXPAND('BizLinkFull_HS.MAC_MEOW_Biz_Batch('+#TEXT(d)+','+REGEXREPLACE(',address_type_derived',REGEXREPLACE('input_company_name_prefix',sMEOWParameters,'',NOCASE),'',NOCASE)+',dResults,,,bIncludeFilter)');
    RETURN dResults;
  END;
  ...
  EXPORT SoapSubmit(DATASET(lInputAugmented) d,STRING IP='http://10.239.190.1:9876'):=FUNCTION
    // lResponse:=RECORDOF(BizLinkFull_HS.MEOW_Biz(DATASET([],BizLinkFull_HS.Process_Biz_Layouts.InputLayout)).Raw_Results);
    // RETURN SOAPCALL(d,IP,'bip_header_soap',{d},DATASET(lResponse),PARALLEL(1));
    #EXPAND('BizLinkFull_HS.MAC_MEOW_Biz_Online('+#TEXT(d)+','+sMEOWParameters+',dResults)');
    RETURN dResults;
  END;
  ...
  EXPORT fMatchResults(DATASET(BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch) d,DATASET(lInputAugmented) dIn=DATASET([],lInputAugmented)):=FUNCTION
  ...
  EXPORT Search_ADDRESS1(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_ADDRESS1.ScoredproxidFetch(d[1].prim_name,d[1].city,d[1].st,d[1].prim_range,cnp_name_spec,d[1].zip_cases,d[1].company_sic_code1,d[1].cnp_number,d[1].cnp_btype,d[1].cnp_lowv,d[1].sec_range,d[1].parent_proxid,d[1].sele_proxid,d[1].org_proxid,d[1].ultimate_proxid,d[1].sele_flag,d[1].org_flag,d[1].ult_flag) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_ADDRESS2(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_ADDRESS2.ScoredproxidFetch(d[1].prim_name,d[1].zip_cases,d[1].prim_range,cnp_name_spec,d[1].st,d[1].city,d[1].company_sic_code1,d[1].cnp_number,d[1].cnp_btype,d[1].cnp_lowv,d[1].sec_range,d[1].parent_proxid,d[1].sele_proxid,d[1].org_proxid,d[1].ultimate_proxid,d[1].sele_flag,d[1].org_flag,d[1].ult_flag) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_ADDRESS3(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_ADDRESS3.ScoredproxidFetch(d[1].prim_name,d[1].prim_range,d[1].zip_cases,cnp_name_spec,d[1].st,d[1].city,d[1].company_sic_code1,d[1].cnp_number,d[1].cnp_btype,d[1].cnp_lowv,d[1].sec_range,d[1].parent_proxid,d[1].sele_proxid,d[1].org_proxid,d[1].ultimate_proxid,d[1].sele_flag,d[1].org_flag,d[1].ult_flag) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_CNPNAME(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_CNPNAME.ScoredproxidFetch(cnp_name_spec,d[1].prim_name,d[1].st,d[1].city,d[1].company_sic_code1,d[1].cnp_number,d[1].cnp_btype,d[1].cnp_lowv,d[1].prim_range,d[1].sec_range,d[1].parent_proxid,d[1].sele_proxid,d[1].org_proxid,d[1].ultimate_proxid,d[1].sele_flag,d[1].org_flag,d[1].ult_flag,d[1].zip_cases) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_CNPNAME_FUZZY(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_CNPNAME_FUZZY.ScoredproxidFetch(d[1].company_name_prefix,cnp_name_spec,d[1].zip_cases,d[1].city,d[1].st,d[1].company_sic_code1,d[1].cnp_number,d[1].cnp_btype,d[1].cnp_lowv,d[1].prim_range,d[1].sec_range,d[1].parent_proxid,d[1].sele_proxid,d[1].org_proxid,d[1].ultimate_proxid,d[1].sele_flag,d[1].org_flag,d[1].ult_flag) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_CNPNAME_ST(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_CNPNAME_ST.ScoredproxidFetch(cnp_name_spec,d[1].st,d[1].prim_name,d[1].zip_cases,d[1].city,d[1].company_sic_code1,d[1].cnp_number,d[1].cnp_btype,d[1].cnp_lowv,d[1].prim_range,d[1].sec_range,d[1].parent_proxid,d[1].sele_proxid,d[1].org_proxid,d[1].ultimate_proxid,d[1].sele_flag,d[1].org_flag,d[1].ult_flag) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_CNPNAME_ZIP(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_CNPNAME_ZIP.ScoredproxidFetch(cnp_name_spec,d[1].zip_cases,d[1].prim_name,d[1].st,d[1].city,d[1].company_sic_code1,d[1].cnp_number,d[1].cnp_btype,d[1].cnp_lowv,d[1].prim_range,d[1].sec_range,d[1].parent_proxid,d[1].sele_proxid,d[1].org_proxid,d[1].ultimate_proxid,d[1].sele_flag,d[1].org_flag,d[1].ult_flag) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_CONTACT(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_CONTACT.ScoredproxidFetch(d[1].fname_preferred,d[1].lname,d[1].mname,cnp_name_spec,d[1].zip_cases,d[1].st,d[1].fname,d[1].company_sic_code1,d[1].cnp_number,d[1].cnp_btype,d[1].cnp_lowv,d[1].prim_name,d[1].city,d[1].prim_range,d[1].sec_range,d[1].parent_proxid,d[1].sele_proxid,d[1].org_proxid,d[1].ultimate_proxid,d[1].sele_flag,d[1].org_flag,d[1].ult_flag) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_CONTACT_DID(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    d01:=BizLinkFull_HS.Key_BizHead_L_CONTACT_DID.ScoredproxidFetch(d[1].contact_did) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_CONTACT_SSN(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_CONTACT_SSN.ScoredproxidFetch(d[1].contact_ssn,d[1].contact_email,d[1].company_sic_code1,cnp_name_spec,d[1].cnp_number,d[1].cnp_btype,d[1].cnp_lowv,d[1].zip_cases,d[1].prim_name,d[1].city,d[1].st,d[1].prim_range,d[1].sec_range,d[1].parent_proxid,d[1].sele_proxid,d[1].org_proxid,d[1].ultimate_proxid,d[1].sele_flag,d[1].org_flag,d[1].ult_flag) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_EMAIL(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_EMAIL.ScoredproxidFetch(d[1].contact_email,d[1].company_sic_code1,cnp_name_spec,d[1].cnp_number,d[1].cnp_btype,d[1].cnp_lowv,d[1].zip_cases,d[1].prim_name,d[1].city,d[1].st,d[1].prim_range,d[1].sec_range,d[1].parent_proxid,d[1].sele_proxid,d[1].org_proxid,d[1].ultimate_proxid,d[1].sele_flag,d[1].org_flag,d[1].ult_flag) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_FEIN(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_FEIN.ScoredproxidFetch(d[1].company_fein,d[1].company_sic_code1,cnp_name_spec,d[1].cnp_number,d[1].cnp_btype,d[1].cnp_lowv,d[1].zip_cases,d[1].prim_name,d[1].city,d[1].st,d[1].prim_range,d[1].sec_range,d[1].parent_proxid,d[1].sele_proxid,d[1].org_proxid,d[1].ultimate_proxid,d[1].sele_flag,d[1].org_flag,d[1].ult_flag) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_PHONE(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_PHONE.ScoredproxidFetch(d[1].company_phone_7,cnp_name_spec,d[1].company_phone_3,d[1].company_phone_3_ex,d[1].company_sic_code1,d[1].cnp_number,d[1].cnp_btype,d[1].cnp_lowv,d[1].zip_cases,d[1].prim_name,d[1].city,d[1].st,d[1].prim_range,d[1].sec_range,d[1].parent_proxid,d[1].sele_proxid,d[1].org_proxid,d[1].ultimate_proxid,d[1].sele_flag,d[1].org_flag,d[1].ult_flag) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_SIC(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_SIC.ScoredproxidFetch(d[1].company_sic_code1,d[1].zip_cases,cnp_name_spec,d[1].prim_name) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_SOURCE(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_SOURCE.ScoredproxidFetch(d[1].source_record_id,d[1].source,cnp_name_spec,d[1].prim_name,d[1].zip_cases,d[1].city,d[1].st,d[1].company_sic_code1,d[1].cnp_number,d[1].cnp_btype,d[1].cnp_lowv,d[1].prim_range,d[1].sec_range,d[1].parent_proxid,d[1].sele_proxid,d[1].org_proxid,d[1].ultimate_proxid,d[1].sele_flag,d[1].org_flag,d[1].ult_flag) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Search_URL(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw);
    SALT30.mac_wordbag_appendspecs_rx(d[1].cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,cnp_name_spec);
    d01:=BizLinkFull_HS.Key_BizHead_L_URL.ScoredproxidFetch(d[1].company_url,d[1].st,d[1].company_sic_code1,cnp_name_spec,d[1].cnp_number,d[1].cnp_btype,d[1].cnp_lowv,d[1].zip_cases,d[1].prim_name,d[1].city,d[1].prim_range,d[1].sec_range,d[1].parent_proxid,d[1].sele_proxid,d[1].org_proxid,d[1].ultimate_proxid,d[1].sele_flag,d[1].org_flag,d[1].ult_flag) ;
    RETURN BizLinkFull_HS._Search.fMatchResults(d01,d);
  ENDMACRO;
  EXPORT Batch_ADDRESS1(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw,,,FALSE);
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_ADDRESS1.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_ADDRESS1.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_ADDRESS2(dRaw):=FUNCTIONMACRO
    d:=BizLinkFull_HS._Search.macFormalize(dRaw,,,FALSE);
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_ADDRESS2.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_ADDRESS2.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_ADDRESS3(dRaw):=FUNCTIONMACRO
    ...
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_ADDRESS3.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_ADDRESS3.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_CNPNAME(dRaw):=FUNCTIONMACRO
    ...
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_CNPNAME.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_CNPNAME.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_CNPNAME_FUZZY(dRaw):=FUNCTIONMACRO
    ...
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_CNPNAME_FUZZY.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_CNPNAME_FUZZY.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_CNPNAME_ST(dRaw):=FUNCTIONMACRO
    ...
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_CNPNAME_ST.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_CNPNAME_ST.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_CNPNAME_ZIP(dRaw):=FUNCTIONMACRO
    ...
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_CNPNAME_ZIP.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_CNPNAME_ZIP.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_CONTACT(dRaw):=FUNCTIONMACRO
    ...
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_CONTACT.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_CONTACT.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_CONTACT_DID(dRaw):=FUNCTIONMACRO
    ...
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_CONTACT_DID.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_CONTACT_DID.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_CONTACT_SSN(dRaw):=FUNCTIONMACRO
    ...
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_CONTACT_SSN.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_CONTACT_SSN.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_EMAIL(dRaw):=FUNCTIONMACRO
    ...
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_EMAIL.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_EMAIL.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_FEIN(dRaw):=FUNCTIONMACRO
    ...
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_FEIN.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_FEIN.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_PHONE(dRaw):=FUNCTIONMACRO
    ...
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_PHONE.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_PHONE.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_SIC(dRaw):=FUNCTIONMACRO
    ...
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_SIC.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_SIC.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_SOURCE(dRaw):=FUNCTIONMACRO
    ...
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_SOURCE.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_SOURCE.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;
  EXPORT Batch_URL(dRaw):=FUNCTIONMACRO
    ...
    d01:=PROJECT(d,TRANSFORM({RECORDOF(LEFT);STRING cnp_name_;},SELF.cnp_name_:=LEFT.cnp_name;SELF.cnp_name:='';SELF:=LEFT;));
    d02:=SALT29.mac_wordbag_appendspecs_th(d01,cnp_name_,cnp_name,BizLinkFull_HS.Specificities(BizLinkFull_HS.File_BizHead).cnp_name_values_key,cnp_name,TRUE);
    d03:=BizLinkFull_HS.Key_BizHead_L_URL.ScoredFetch_Batch(PROJECT(d02,TRANSFORM(BizLinkFull_HS.Key_BizHead_L_URL.InputLayout_Batch,SELF.Reference:=LEFT.uniqueid;SELF:=LEFT;)),TRUE);
    RETURN BizLinkFull_HS._Search.fMatchResults(d03,d);
  ENDMACRO;

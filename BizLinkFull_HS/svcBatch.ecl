...
EXPORT svcBatch := MACRO
  ...
  dInput:=DATASET([],BizLinkFull_HS.lBatchInput):STORED('batch_in');
 
  // For any records where a proxid was not populated, perform a batch append
  dFormalized:=BizLinkFull_HS._Search.macFormalize(dInput,,,FALSE);
  dResults:=BizLinkFull_HS._Search.BatchSubmit(dFormalized,FALSE);
  dBatchSearch:=JOIN(dInput,dResults,LEFT.acctno=RIGHT.reference,TRANSFORM(BizLinkFull_HS.lBatchOutput,
    ...
    SELF.search_matches := BizLinkFull_HS.matchCodes.batch_MatchCode(RIGHT.results[1]);
    SELF:=[];
  ));
  
  // For any records where a proxid was populated, pull results from MEOW_Biz
  dIDFormalized:=PROJECT(BizLinkFull_HS._Search.macFormalize(TABLE(dInput(entered_proxid>0),{acctno;entered_proxid})),BizLinkFull_HS.Process_Biz_Layouts.InputLayout);
  dIDSearch:=PROJECT(DEDUP(SORT(BizLinkFull_HS.MEOW_Biz(dIDFormalized).Data_,uniqueid),uniqueid),TRANSFORM(BizLinkFull_HS.lBatchOutput,
  ...
  MEOW:=BizLinkFull_HS.MEOW_Biz(DATASET([],BizLinkFull_HS.Process_Biz_Layouts.InputLayout));
  ...
  dSeleScoredFormalized:=PROJECT(BizLinkFull_HS._Search.macFormalize(dSeleScored),BizLinkFull_HS.Process_Biz_Layouts.InputLayout);
  dBestOut:=MEOW.ScoreData(dSeleScored,dSeleScoredFormalized);
  
  // Format the best results into the output layout
  dSeleBest:=JOIN(dBestOut,dWithSele,LEFT.orig_uid=RIGHT.acctno AND LEFT.seleid=RIGHT.seleid,TRANSFORM(BizLinkFull_HS.lBatchOutput,
    SELF.acctno:=RIGHT.acctno;
    SELF.proxid:=RIGHT.proxid;
    SELF.seleid:=RIGHT.seleid;
    SELF.orgid:=RIGHT.orgid;
    ...
    SELF.best_matches := BizLinkFull_HS.matchCodes.online_MatchCode(   
          project(left, transform(BizLinkFull_HS.matchCodes.onlineMatchLayout, self := left)), dInput(acctno=RIGHT.acctno)[1]);
    SELF.search_matches := RIGHT.search_matches;
    SELF:=LEFT;
    SELF:=[];
  ),KEEP(1));

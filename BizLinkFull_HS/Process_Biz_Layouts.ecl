...
EXPORT KeyName := BizLinkFull_HS.Filename_keys.meow; /*HACK07meow*/
...
EXPORT KeyproxidUpName := BizLinkFull_HS.Filename_keys.sup_proxid; /*HACK07sup_proxid*/
...
EXPORT KeyseleidUpName := BizLinkFull_HS.Filename_keys.sup_seleid; /*HACK07sup_seleid*/
  s := TABLE(h,{ultid,orgid,seleid},ultid,orgid,seleid,MERGE);
 
EXPORT KeyseleidUp := INDEX(s,{seleid},{s},KeyseleidUpName);
 
EXPORT KeyorgidUpName := BizLinkFull_HS.Filename_keys.sup_orgid; /*HACK07sup_orgid*/
...
EXPORT KeyIDHistoryName := BizLinkFull_HS.Filename_keys.sup_rcid; /*HACK07sup_rcid*/
...
EXPORT id_stream_historic(DATASET(id_stream_layout) id) := FUNCTION
  ...
  END;
  J := JOIN(C,KeyIDHistory,LEFT.rcid=RIGHT.rcid,Load(LEFT,RIGHT),LIMIT(BizLinkFull_HS.Config.JoinLimit));
  ...
  id_stream_layout Load0(id_stream_layout le,KeyIDHistory ri) := TRANSFORM
    ...
    SELF := le; // Remainder of id_stream information
  END;
  J0 := JOIN(C0,KeyIDHistory,LEFT.proxid=RIGHT.rcid,Load0(LEFT,RIGHT),LIMIT(BizLinkFull_HS.Config.JoinLimit));
  ...
  id_stream_layout Load1(id_stream_layout le,KeyIDHistory ri) := TRANSFORM
    ...
    SELF := le; // Remainder of id_stream information
  END;
  J1 := JOIN(C1,KeyIDHistory,LEFT.seleid=RIGHT.rcid,Load1(LEFT,RIGHT),LIMIT(BizLinkFull_HS.Config.JoinLimit));
  ...
  id_stream_layout Load2(id_stream_layout le,KeyIDHistory ri) := TRANSFORM
    ...
    SELF := le; // Remainder of id_stream information
  END;
  J2 := JOIN(C2,KeyIDHistory,LEFT.orgid=RIGHT.rcid,Load2(LEFT,RIGHT),LIMIT(BizLinkFull_HS.Config.JoinLimit));
  ...
  id_stream_layout Load3(id_stream_layout le,KeyIDHistory ri) := TRANSFORM
    ...
    SELF := le; // Remainder of id_stream information
  END;
  J3 := JOIN(C3,KeyIDHistory,LEFT.ultid=RIGHT.rcid,Load3(LEFT,RIGHT),LIMIT(BizLinkFull_HS.Config.JoinLimit));
  ...
  id_stream_layout Load4(id_stream_layout le,KeyIDHistory ri) := TRANSFORM
    ...
    SELF := le; // Remainder of id_stream information
  END;
  J4 := JOIN(C4,KeyIDHistory,LEFT.powid=RIGHT.rcid,Load4(LEFT,RIGHT),LIMIT(BizLinkFull_HS.Config.JoinLimit));
...
END;
EXPORT CombineAllScores(DATASET(LayoutScoredFetch) in_data,BOOLEAN bIncludeFilter=TRUE) := FUNCTION
OutputLayout_Batch Create_Output(LayoutScoredFetch le, DATASET(LayoutScoredFetch) ri) := TRANSFORM
  ...
  SELF.Results_powid := SORT( ROLLUP( SORT( SELF.Results,powid),LEFT.powid=RIGHT.powid,Combine_Scores(LEFT,RIGHT)),-Weight);

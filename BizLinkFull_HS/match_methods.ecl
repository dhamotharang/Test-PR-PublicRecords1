...
EXPORT match_cnp_name(TYPEOF(h.cnp_name) L, TYPEOF(h.cnp_name) R, UNSIGNED1 LL = 0, UNSIGNED1 RL = 0, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT33.MatchCode.ExactMatch,
    ...
    SALT33.MatchBagOfWords(L,R,3177747,1) <> 0 => SALT33.MatchCode.WordBagMatch,
    SALT33.MatchCode.NoMatch),
        MAP(L = R => SALT33.MatchCode.ExactMatch, SALT33.MatchCode.NoMatch)
);
...
EXPORT match_zip(TYPEOF(h.zip) L,DATASET(BizLinkFull_HS.process_Biz_layouts.layout_zip_cases)  R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(EXISTS(R(L=zip)) => SALT33.MatchCode.ExactMatch,
    SALT33.MatchCode.NoMatch),
        MAP(EXISTS(R(L=zip)) => SALT33.MatchCode.ExactMatch, SALT33.MatchCode.NoMatch)
);
EXPORT match_company_url(TYPEOF(h.company_url) L, TYPEOF(h.company_url) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT33.MatchCode.ExactMatch,
    SALT33.MatchBagOfWords(L,R,31744,0) <> 0 => SALT33.MatchCode.WordBagMatch,
    SALT33.MatchCode.NoMatch),
        MAP(L = R => SALT33.MatchCode.ExactMatch, SALT33.MatchCode.NoMatch)
);
...
EXPORT match_name_suffix(TYPEOF(h.name_suffix) L, TYPEOF(h.name_suffix) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT33.MatchCode.ExactMatch,
    fn_normSuffix(L) =  fn_normSuffix(R) => SALT33.MatchCode.CustomFuzzyMatch, // Compare fn_normSuffix values
    SALT33.MatchCode.NoMatch),
        MAP(L = R => SALT33.MatchCode.ExactMatch, SALT33.MatchCode.NoMatch)
);
...
EXPORT match_CONTACTNAME(TYPEOF(h.CONTACTNAME) L,TYPEOF(h.CONTACTNAME) R) := 
   MAP(L = R => SALT33.MatchCode.ExactMatch,
  SALT33.MatchCode.NoMatch);
EXPORT match_STREETADDRESS(TYPEOF(h.STREETADDRESS) L,TYPEOF(h.STREETADDRESS) R) := 
   MAP(L = R => SALT33.MatchCode.ExactMatch,
  SALT33.MatchCode.NoMatch);
END;

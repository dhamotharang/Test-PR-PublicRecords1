EXPORT MAC_MEOW_Biz_Batch(infile,Ref='',Input_parent_proxid = '',Input_sele_proxid = '',Input_org_proxid = '',Input_ultimate_proxid = '',Input_has_lgid = '',Input_empid = '',Input_source = '',Input_source_record_id = '',Input_source_docid = '',Input_company_name = '',Input_company_name_prefix = '',Input_cnp_name = '',Input_cnp_number = '',Input_cnp_btype = '',Input_cnp_lowv = '',Input_company_phone = '',Input_company_phone_3 = '',Input_company_phone_3_ex = '',Input_company_phone_7 = '',Input_company_fein = '',Input_company_sic_code1 = '',Input_active_duns_number = '',Input_prim_range = '',Input_prim_name = '',Input_sec_range = '',Input_city = '',Input_city_clean = '',Input_st = '',Input_zip = '',Input_company_url = '',Input_isContact = '',Input_contact_did = '',Input_title = '',Input_fname = '',Input_fname_preferred = '',Input_mname = '',Input_lname = '',Input_name_suffix = '',Input_contact_ssn = '',Input_contact_email = '',Input_sele_flag = '',Input_org_flag = '',Input_ult_flag = '',Input_fallback_value = '',Input_CONTACTNAME = '',Input_STREETADDRESS = '',OutFile,AsIndex='true',Stats='',bIncludeFilter=TRUE) := MACRO
  ...
  #ELSE
    SELF.zip_cases := DATASET([],BizLinkFull_HS.Process_Biz_layouts.layout_zip_cases);
  #END
  ...
  %TPRec% %CleanT%(%fats0% le) := TRANSFORM
    ...
    SELF.company_name := (TYPEOF(SELF.company_name))BizLinkFull_HS.Fields.Make_company_name((SALT33.StrType)le.company_name);
    SELF.company_name_prefix := (TYPEOF(SELF.company_name_prefix))BizLinkFull_HS.Fields.Make_company_name_prefix((SALT33.StrType)le.company_name_prefix);
    SELF.cnp_name := (TYPEOF(SELF.cnp_name))BizLinkFull_HS.Fields.Make_cnp_name((SALT33.StrType)le.cnp_name);
    ...
    SELF.company_fein := IF ( BizLinkFull_HS.Fields.Invalid_company_fein((SALT33.StrType)le.company_fein)=0,(TYPEOF(SELF.company_fein))le.company_fein,(TYPEOF(SELF.company_fein))'');
    ...
    SELF.prim_name := (TYPEOF(SELF.prim_name))BizLinkFull_HS.Fields.Make_prim_name((SALT33.StrType)le.prim_name);
    SELF.sec_range := (TYPEOF(SELF.sec_range))BizLinkFull_HS.Fields.Make_sec_range((SALT33.StrType)le.sec_range);
    SELF.city := (TYPEOF(SELF.city))BizLinkFull_HS.Fields.Make_city((SALT33.StrType)le.city);
    SELF.city_clean := (TYPEOF(SELF.city_clean))BizLinkFull_HS.Fields.Make_city_clean((SALT33.StrType)le.city_clean);
    SELF.st := (TYPEOF(SELF.st))BizLinkFull_HS.Fields.Make_st((SALT33.StrType)le.st);
    SELF.zip_cases := le.zip_cases;
    SELF.company_url := (TYPEOF(SELF.company_url))BizLinkFull_HS.Fields.Make_company_url((SALT33.StrType)le.company_url);
    ...
    SELF.fname := (TYPEOF(SELF.fname))BizLinkFull_HS.Fields.Make_fname((SALT33.StrType)le.fname);
    SELF.fname_preferred := (TYPEOF(SELF.fname_preferred))BizLinkFull_HS.Fields.Make_fname_preferred((SALT33.StrType)le.fname_preferred);
    SELF.mname := (TYPEOF(SELF.mname))BizLinkFull_HS.Fields.Make_mname((SALT33.StrType)le.mname);
    SELF.lname := (TYPEOF(SELF.lname))BizLinkFull_HS.Fields.Make_lname((SALT33.StrType)le.lname);
    SELF.name_suffix := (TYPEOF(SELF.name_suffix))BizLinkFull_HS.Fields.Make_name_suffix((SALT33.StrType)le.name_suffix);
    SELF.contact_ssn := (TYPEOF(SELF.contact_ssn))le.contact_ssn;
    SELF.contact_email := (TYPEOF(SELF.contact_email))BizLinkFull_HS.Fields.Make_contact_email((SALT33.StrType)le.contact_email);
 ...
 // In case multiple copies of the same indicative are in there - remove them
  SALT33.MAC_Dups_Note(%fats1%,%TPRec%,%fats%,%dups%,,BizLinkFull_HS.Config.meow_dedup);
  #uniquename(key_cnp_name)
  %key_cnp_name% := BizLinkFull_HS.specificities(BizLinkFull_HS.file_BizHead).cnp_name_values_key;
  #uniquename(A_cnp_name)
  %A_cnp_name% := SALT33.mac_wordbag_appendspecs_th(%fats%,cnp_name,cnp_name_wb,%key_cnp_name%,cnp_name,AsIndex);
  #uniquename(key_company_url)
  %key_company_url% := BizLinkFull_HS.specificities(BizLinkFull_HS.file_BizHead).company_url_values_key;
  #uniquename(A_company_url)
  %A_company_url% := SALT33.mac_wordbag_appendspecs_th(%A_cnp_name%,company_url,company_url_wb,%key_company_url%,company_url,AsIndex);
...
#IF(#TEXT(Input_cnp_name)<>'' AND #TEXT(Input_zip)<>'')
  ...
  BizLinkFull_HS.Key_BizHead_L_CNPNAME_ZIP.MAC_ScoredFetch_Batch(%HoldL_CNPNAME_ZIP%,UniqueId,cnp_name_wb,zip_cases,prim_name,st,city,company_sic_code1,cnp_number,cnp_btype,cnp_lowv,prim_range,sec_range,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,sele_flag,org_flag,ult_flag,%OutputL_CNPNAME_ZIP%,AsIndex)
#ELSE
  %OutputL_CNPNAME_ZIP% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
  #uniquename(OutputL_CNPNAME_ST)
#IF(#TEXT(Input_cnp_name)<>'' AND #TEXT(Input_st)<>'')
  #uniquename(HoldL_CNPNAME_ST)
  %HoldL_CNPNAME_ST% := %ToProcess%(~BizLinkFull_HS.Key_BizHead_L_CNPNAME_ZIP.CanSearch(ROW(%ToProcess%)));
  BizLinkFull_HS.Key_BizHead_L_CNPNAME_ST.MAC_ScoredFetch_Batch(%HoldL_CNPNAME_ST%,UniqueId,cnp_name_wb,st,prim_name,zip_cases,city,company_sic_code1,cnp_number,cnp_btype,cnp_lowv,prim_range,sec_range,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,sele_flag,org_flag,ult_flag,%OutputL_CNPNAME_ST%,AsIndex)
#ELSE
  %OutputL_CNPNAME_ST% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
  #uniquename(OutputL_CNPNAME)
#IF(#TEXT(Input_cnp_name)<>'')
  #uniquename(HoldL_CNPNAME)
  %HoldL_CNPNAME% := %ToProcess%(~BizLinkFull_HS.Key_BizHead_L_CNPNAME_ST.CanSearch(ROW(%ToProcess%)) AND ~BizLinkFull_HS.Key_BizHead_L_CNPNAME_ZIP.CanSearch(ROW(%ToProcess%)));
  BizLinkFull_HS.Key_BizHead_L_CNPNAME.MAC_ScoredFetch_Batch(%HoldL_CNPNAME%,UniqueId,cnp_name_wb,prim_name,st,city,company_sic_code1,cnp_number,cnp_btype,cnp_lowv,prim_range,sec_range,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,sele_flag,org_flag,ult_flag,zip_cases,%OutputL_CNPNAME%,AsIndex)
#ELSE
  %OutputL_CNPNAME% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
  #uniquename(OutputL_CNPNAME_FUZZY)
#IF(#TEXT(Input_company_name_prefix)<>'')
  #uniquename(HoldL_CNPNAME_FUZZY)
  %HoldL_CNPNAME_FUZZY% := %ToProcess%;
  BizLinkFull_HS.Key_BizHead_L_CNPNAME_FUZZY.MAC_ScoredFetch_Batch(%HoldL_CNPNAME_FUZZY%,UniqueId,company_name_prefix,cnp_name_wb,zip_cases,city,st,company_sic_code1,cnp_number,cnp_btype,cnp_lowv,prim_range,sec_range,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,sele_flag,org_flag,ult_flag,%OutputL_CNPNAME_FUZZY%,AsIndex)
#ELSE
  %OutputL_CNPNAME_FUZZY% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
...
#IF(#TEXT(Input_prim_name)<>'' AND #TEXT(Input_city)<>'' AND #TEXT(Input_st)<>'')
  ...
  BizLinkFull_HS.Key_BizHead_L_ADDRESS1.MAC_ScoredFetch_Batch(%HoldL_ADDRESS1%,UniqueId,prim_name,city,st,prim_range,cnp_name_wb,zip_cases,company_sic_code1,cnp_number,cnp_btype,cnp_lowv,sec_range,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,sele_flag,org_flag,ult_flag,%OutputL_ADDRESS1%,AsIndex)
#ELSE
  %OutputL_ADDRESS1% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
  #uniquename(OutputL_ADDRESS2)
#IF(#TEXT(Input_prim_name)<>'' AND #TEXT(Input_zip)<>'')
  #uniquename(HoldL_ADDRESS2)
  %HoldL_ADDRESS2% := %ToProcess%;
  BizLinkFull_HS.Key_BizHead_L_ADDRESS2.MAC_ScoredFetch_Batch(%HoldL_ADDRESS2%,UniqueId,prim_name,zip_cases,prim_range,cnp_name_wb,st,city,company_sic_code1,cnp_number,cnp_btype,cnp_lowv,sec_range,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,sele_flag,org_flag,ult_flag,%OutputL_ADDRESS2%,AsIndex)
#ELSE
  %OutputL_ADDRESS2% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
...
#IF(#TEXT(Input_prim_name)<>'' AND #TEXT(Input_prim_range)<>'' AND #TEXT(Input_zip)<>'')
  ...
  BizLinkFull_HS.Key_BizHead_L_ADDRESS3.MAC_ScoredFetch_Batch(%HoldL_ADDRESS3%,UniqueId,prim_name,prim_range,zip_cases,cnp_name_wb,st,city,company_sic_code1,cnp_number,cnp_btype,cnp_lowv,sec_range,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,sele_flag,org_flag,ult_flag,%OutputL_ADDRESS3%,AsIndex)
#ELSE
  %OutputL_ADDRESS3% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
  #uniquename(OutputL_PHONE)
#IF(#TEXT(Input_company_phone_7)<>'')
  #uniquename(HoldL_PHONE)
  %HoldL_PHONE% := %ToProcess%;
  BizLinkFull_HS.Key_BizHead_L_PHONE.MAC_ScoredFetch_Batch(%HoldL_PHONE%,UniqueId,company_phone_7,cnp_name_wb,company_phone_3,company_phone_3_ex,company_sic_code1,cnp_number,cnp_btype,cnp_lowv,zip_cases,prim_name,city,st,prim_range,sec_range,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,sele_flag,org_flag,ult_flag,%OutputL_PHONE%,AsIndex)
#ELSE
  %OutputL_PHONE% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
  #uniquename(OutputL_FEIN)
#IF(#TEXT(Input_company_fein)<>'')
  #uniquename(HoldL_FEIN)
  %HoldL_FEIN% := %ToProcess%;
  BizLinkFull_HS.Key_BizHead_L_FEIN.MAC_ScoredFetch_Batch(%HoldL_FEIN%,UniqueId,company_fein,company_sic_code1,cnp_name_wb,cnp_number,cnp_btype,cnp_lowv,zip_cases,prim_name,city,st,prim_range,sec_range,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,sele_flag,org_flag,ult_flag,%OutputL_FEIN%,AsIndex)
#ELSE
  %OutputL_FEIN% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
  #uniquename(OutputL_URL)
#IF(#TEXT(Input_company_url)<>'')
  #uniquename(HoldL_URL)
  %HoldL_URL% := %ToProcess%;
  BizLinkFull_HS.Key_BizHead_L_URL.MAC_ScoredFetch_Batch(%HoldL_URL%,UniqueId,company_url_wb,st,company_sic_code1,cnp_name_wb,cnp_number,cnp_btype,cnp_lowv,zip_cases,prim_name,city,prim_range,sec_range,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,sele_flag,org_flag,ult_flag,%OutputL_URL%,AsIndex)
#ELSE
  %OutputL_URL% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
...
#IF(#TEXT(Input_fname_preferred)<>'' AND #TEXT(Input_lname)<>'')
  ...
  BizLinkFull_HS.Key_BizHead_L_CONTACT.MAC_ScoredFetch_Batch(%HoldL_CONTACT%,UniqueId,fname_preferred,lname,mname,cnp_name_wb,zip_cases,st,fname,company_sic_code1,cnp_number,cnp_btype,cnp_lowv,prim_name,city,prim_range,sec_range,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,sele_flag,org_flag,ult_flag,%OutputL_CONTACT%,AsIndex)
#ELSE
  %OutputL_CONTACT% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
  #uniquename(OutputL_CONTACT_SSN)
#IF(#TEXT(Input_contact_ssn)<>'')
  #uniquename(HoldL_CONTACT_SSN)
  %HoldL_CONTACT_SSN% := %ToProcess%;
  BizLinkFull_HS.Key_BizHead_L_CONTACT_SSN.MAC_ScoredFetch_Batch(%HoldL_CONTACT_SSN%,UniqueId,contact_ssn,contact_email,company_sic_code1,cnp_name_wb,cnp_number,cnp_btype,cnp_lowv,zip_cases,prim_name,city,st,prim_range,sec_range,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,sele_flag,org_flag,ult_flag,%OutputL_CONTACT_SSN%,AsIndex)
#ELSE
  %OutputL_CONTACT_SSN% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
  #uniquename(OutputL_EMAIL)
#IF(#TEXT(Input_contact_email)<>'')
  #uniquename(HoldL_EMAIL)
  %HoldL_EMAIL% := %ToProcess%;
  BizLinkFull_HS.Key_BizHead_L_EMAIL.MAC_ScoredFetch_Batch(%HoldL_EMAIL%,UniqueId,contact_email,company_sic_code1,cnp_name_wb,cnp_number,cnp_btype,cnp_lowv,zip_cases,prim_name,city,st,prim_range,sec_range,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,sele_flag,org_flag,ult_flag,%OutputL_EMAIL%,AsIndex)
#ELSE
  %OutputL_EMAIL% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
  #uniquename(OutputL_SIC)
#IF(#TEXT(Input_company_sic_code1)<>'' AND #TEXT(Input_zip)<>'')
  #uniquename(HoldL_SIC)
  %HoldL_SIC% := %ToProcess%;
  BizLinkFull_HS.Key_BizHead_L_SIC.MAC_ScoredFetch_Batch(%HoldL_SIC%,UniqueId,company_sic_code1,zip_cases,cnp_name_wb,prim_name,%OutputL_SIC%,AsIndex)
#ELSE
  %OutputL_SIC% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
...
#IF(#TEXT(Input_source_record_id)<>'' AND #TEXT(Input_source)<>'')
  ...
  BizLinkFull_HS.Key_BizHead_L_SOURCE.MAC_ScoredFetch_Batch(%HoldL_SOURCE%,UniqueId,source_record_id,source,cnp_name_wb,prim_name,zip_cases,city,st,company_sic_code1,cnp_number,cnp_btype,cnp_lowv,prim_range,sec_range,parent_proxid,sele_proxid,org_proxid,ultimate_proxid,sele_flag,org_flag,ult_flag,%OutputL_SOURCE%,AsIndex)
#ELSE
  %OutputL_SOURCE% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
  #uniquename(OutputL_CONTACT_DID)
#IF(#TEXT(Input_contact_did)<>'')
  #uniquename(HoldL_CONTACT_DID)
  %HoldL_CONTACT_DID% := %ToProcess%;
  BizLinkFull_HS.Key_BizHead_L_CONTACT_DID.MAC_ScoredFetch_Batch(%HoldL_CONTACT_DID%,UniqueId,contact_did,%OutputL_CONTACT_DID%,AsIndex)
#ELSE
  %OutputL_CONTACT_DID% := DATASET([],BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch);
#END
  #uniquename(AllRes)
  %AllRes% := %OutputL_CNPNAME_ZIP%+%OutputL_CNPNAME_ST%+%OutputL_CNPNAME%+%OutputL_CNPNAME_FUZZY%+%OutputL_ADDRESS1%+%OutputL_ADDRESS2%+%OutputL_ADDRESS3%+%OutputL_PHONE%+%OutputL_FEIN%+%OutputL_URL%+%OutputL_CONTACT%+%OutputL_CONTACT_SSN%+%OutputL_EMAIL%+%OutputL_SIC%+%OutputL_SOURCE%+%OutputL_CONTACT_DID%;
  #uniquename(All)
  %All% := BizLinkFull_HS.Process_Biz_Layouts.CombineAllScores(%AllRes%,bIncludeFilter/*HACK13*/);
  #uniquename(OutFile0)
  ...
  TYPEOF(%OutFile0%) %RestoreChildReference%(%OutFile0% le) := TRANSFORM
    SELF.Results := PROJECT(le.Results, TRANSFORM(BizLinkFull_HS.Process_Biz_Layouts.LayoutScoredFetch,SELF.Reference := le.Reference, SELF := LEFT));
    SELF := le;
  END;
  OutFile := PROJECT(%OutFile0%,%RestoreChildReference%(LEFT));
  #IF (#TEXT(Stats)<>'')
    Stats := BizLinkFull_HS.Process_Biz_Layouts.ScoreSummary(OutFile);
  #END
ENDMACRO;

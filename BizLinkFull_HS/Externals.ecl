EXPORT Externals := MODULE
  IMPORT SALT26,BizLinkFull_HS;// Gather up the UID counts from each of the children - provides 'we also found' capability
  AllEfr0 := ;
...
EXPORT EFR := INDEX(r,{ultid,orgid,seleid,proxid},{r},'~key::BizLinkFull_HS::proxid::efr');
...
END;
EXPORT Fetch(DATASET(BizLinkFull_HS.Process_Biz_Layouts.id_stream_layout) ins,FetchParams P=FetchParams) := FUNCTION
  Layout Get(DATASET(BizLinkFull_HS.Process_Biz_Layouts.id_stream_layout) ids) := TRANSFORM

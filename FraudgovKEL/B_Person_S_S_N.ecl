...
EXPORT B_Person_S_S_N := MODULE
  ...
  SHARED __EE3960809 := __E_Person_S_S_N;
  EXPORT __ST66832_Layout := RECORD
  ...
  END;
  SHARED __ST66832_Layout __ND3960843__Project(E_Person_S_S_N.Layout __PP3960704) := TRANSFORM
    __EE3960742 := __PP3960704.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE3960742,__T(__EE3960742).Event_Date_);
    __EE3960771 := __PP3960704.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE3960771,__T(__EE3960771).Event_Date_);
    SELF := __PP3960704;
  END;
  EXPORT __ENH_Person_S_S_N := PROJECT(__EE3960809,__ND3960843__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_S_S_N::Annotated',EXPIRE(7));
  SHARED __EE4267457 := __ENH_Person_S_S_N;
  SHARED __IDX_Person_S_S_N_Social__Filtered := __EE4267457(__NN(__EE4267457.Social_));
  ...
  EXPORT __ST4267459_Layout := RECORDOF(IDX_Person_S_S_N_Social_);
  EXPORT IDX_Person_S_S_N_Social__Wrapped := PROJECT(IDX_Person_S_S_N_Social_,TRANSFORM(__ST66832_Layout,SELF.Social_ := __CN(LEFT.Social_),SELF:=LEFT));

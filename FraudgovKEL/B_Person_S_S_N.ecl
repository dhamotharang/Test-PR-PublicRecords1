...
EXPORT B_Person_S_S_N := MODULE
  ...
  SHARED __EE3894661 := __E_Person_S_S_N;
  EXPORT __ST66929_Layout := RECORD
  ...
  END;
  SHARED __ST66929_Layout __ND3894695__Project(E_Person_S_S_N.Layout __PP3894556) := TRANSFORM
    __EE3894594 := __PP3894556.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE3894594,__T(__EE3894594).Event_Date_);
    __EE3894623 := __PP3894556.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE3894623,__T(__EE3894623).Event_Date_);
    SELF := __PP3894556;
  END;
  EXPORT __ENH_Person_S_S_N := PROJECT(__EE3894661,__ND3894695__Project(LEFT)) : PERSIST('~fraudgov::temp::KEL::FraudgovKEL::Person_S_S_N::Annotated',EXPIRE(7));
  SHARED __EE4160167 := __ENH_Person_S_S_N;
  SHARED __IDX_Person_S_S_N_Social__Filtered := __EE4160167(__NN(__EE4160167.Social_));
  ...
  EXPORT __ST4160169_Layout := RECORDOF(IDX_Person_S_S_N_Social_);
  EXPORT IDX_Person_S_S_N_Social__Wrapped := PROJECT(IDX_Person_S_S_N_Social_,TRANSFORM(__ST66929_Layout,SELF.Social_ := __CN(LEFT.Social_),SELF:=LEFT));

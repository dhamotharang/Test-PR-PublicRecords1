...
EXPORT B_Person_S_S_N := MODULE
  ...
  SHARED __EE4007153 := __E_Person_S_S_N;
  EXPORT __ST67180_Layout := RECORD
  ...
  END;
  SHARED __ST67180_Layout __ND4007187__Project(E_Person_S_S_N.Layout __PP4007048) := TRANSFORM
    __EE4007086 := __PP4007048.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE4007086,__T(__EE4007086).Event_Date_);
    __EE4007115 := __PP4007048.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE4007115,__T(__EE4007115).Event_Date_);
    SELF := __PP4007048;
  END;
  EXPORT __ENH_Person_S_S_N := PROJECT(__EE4007153,__ND4007187__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_S_S_N::Annotated',EXPIRE(7));
  SHARED __EE4317929 := __ENH_Person_S_S_N;
  SHARED __IDX_Person_S_S_N_Social__Filtered := __EE4317929(__NN(__EE4317929.Social_));
  ...
  EXPORT __ST4317931_Layout := RECORDOF(IDX_Person_S_S_N_Social_);
  EXPORT IDX_Person_S_S_N_Social__Wrapped := PROJECT(IDX_Person_S_S_N_Social_,TRANSFORM(__ST67180_Layout,SELF.Social_ := __CN(LEFT.Social_),SELF:=LEFT));

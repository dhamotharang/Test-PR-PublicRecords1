...
EXPORT B_Person_S_S_N := MODULE
  ...
  SHARED __EE4515658 := __E_Person_S_S_N;
  EXPORT __ST71740_Layout := RECORD
  ...
  END;
  SHARED __ST71740_Layout __ND4515692__Project(E_Person_S_S_N.Layout __PP4515553) := TRANSFORM
    __EE4515591 := __PP4515553.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE4515591,__T(__EE4515591).Event_Date_);
    __EE4515620 := __PP4515553.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE4515620,__T(__EE4515620).Event_Date_);
    SELF := __PP4515553;
  END;
  EXPORT __ENH_Person_S_S_N := PROJECT(__EE4515658,__ND4515692__Project(LEFT)) : PERSIST('~fraudgov::temp::KEL::FraudgovKEL::Person_S_S_N::Annotated',EXPIRE(7));
  SHARED __EE4829915 := __ENH_Person_S_S_N;
  SHARED __IDX_Person_S_S_N_Social__Filtered := __EE4829915(__NN(__EE4829915.Social_));
  ...
  EXPORT __ST4829917_Layout := RECORDOF(IDX_Person_S_S_N_Social_);
  EXPORT IDX_Person_S_S_N_Social__Wrapped := PROJECT(IDX_Person_S_S_N_Social_,TRANSFORM(__ST71740_Layout,SELF.Social_ := __CN(LEFT.Social_),SELF:=LEFT));

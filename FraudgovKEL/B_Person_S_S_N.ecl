...
EXPORT B_Person_S_S_N := MODULE
  ...
  SHARED __EE1110979 := __E_Person_S_S_N;
  ...
  END;
  SHARED __ST31115_Layout __ND1111013__Project(E_Person_S_S_N.Layout __PP1110874) := TRANSFORM
    __EE1110912 := __PP1110874.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1110912,__T(__EE1110912).Event_Date_);
    __EE1110941 := __PP1110874.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1110941,__T(__EE1110941).Event_Date_);
    SELF := __PP1110874;
  END;
  EXPORT __ENH_Person_S_S_N := PROJECT(__EE1110979,__ND1111013__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_S_S_N::Annotated',EXPIRE(7));
  SHARED __EE1380742 := __ENH_Person_S_S_N;
  SHARED __IDX_Person_S_S_N_Social__Filtered := __EE1380742(__NN(__EE1380742.Social_));
  ...
  EXPORT __ST1380744_Layout := RECORDOF(IDX_Person_S_S_N_Social_);
  EXPORT IDX_Person_S_S_N_Social__Wrapped := PROJECT(IDX_Person_S_S_N_Social_,TRANSFORM(__ST31115_Layout,SELF.Social_ := __CN(LEFT.Social_),SELF:=LEFT));

...
EXPORT B_Person_S_S_N := MODULE
  ...
  SHARED __EE2033440 := __E_Person_S_S_N;
  EXPORT __ST43645_Layout := RECORD
  ...
  END;
  SHARED __ST43645_Layout __ND2033474__Project(E_Person_S_S_N.Layout __PP2033335) := TRANSFORM
    __EE2033373 := __PP2033335.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2033373,__T(__EE2033373).Event_Date_);
    __EE2033402 := __PP2033335.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2033402,__T(__EE2033402).Event_Date_);
    SELF := __PP2033335;
  END;
  EXPORT __ENH_Person_S_S_N := PROJECT(__EE2033440,__ND2033474__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_S_S_N::Annotated',EXPIRE(7));
  SHARED __EE2278071 := __ENH_Person_S_S_N;
  SHARED __IDX_Person_S_S_N_Social__Filtered := __EE2278071(__NN(__EE2278071.Social_));
  ...
  EXPORT __ST2278073_Layout := RECORDOF(IDX_Person_S_S_N_Social_);
  EXPORT IDX_Person_S_S_N_Social__Wrapped := PROJECT(IDX_Person_S_S_N_Social_,TRANSFORM(__ST43645_Layout,SELF.Social_ := __CN(LEFT.Social_),SELF:=LEFT));

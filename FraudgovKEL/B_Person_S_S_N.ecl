...
EXPORT B_Person_S_S_N := MODULE
  ...
  SHARED __EE2096286 := __E_Person_S_S_N;
  EXPORT __ST45478_Layout := RECORD
  ...
  END;
  SHARED __ST45478_Layout __ND2096320__Project(E_Person_S_S_N.Layout __PP2096181) := TRANSFORM
    __EE2096219 := __PP2096181.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2096219,__T(__EE2096219).Event_Date_);
    __EE2096248 := __PP2096181.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2096248,__T(__EE2096248).Event_Date_);
    SELF := __PP2096181;
  END;
  EXPORT __ENH_Person_S_S_N := PROJECT(__EE2096286,__ND2096320__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_S_S_N::Annotated',EXPIRE(7));
  SHARED __EE2343769 := __ENH_Person_S_S_N;
  SHARED __IDX_Person_S_S_N_Social__Filtered := __EE2343769(__NN(__EE2343769.Social_));
  ...
  EXPORT __ST2343771_Layout := RECORDOF(IDX_Person_S_S_N_Social_);
  EXPORT IDX_Person_S_S_N_Social__Wrapped := PROJECT(IDX_Person_S_S_N_Social_,TRANSFORM(__ST45478_Layout,SELF.Social_ := __CN(LEFT.Social_),SELF:=LEFT));

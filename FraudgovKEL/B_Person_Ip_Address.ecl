...
EXPORT B_Person_Ip_Address := MODULE
  ...
  SHARED __EE3960445 := __E_Person_Ip_Address;
  EXPORT __ST66786_Layout := RECORD
  ...
  END;
  SHARED __ST66786_Layout __ND3960479__Project(E_Person_Ip_Address.Layout __PP3960340) := TRANSFORM
    __EE3960378 := __PP3960340.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE3960378,__T(__EE3960378).Event_Date_);
    __EE3960407 := __PP3960340.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE3960407,__T(__EE3960407).Event_Date_);
    SELF := __PP3960340;
  END;
  EXPORT __ENH_Person_Ip_Address := PROJECT(__EE3960445,__ND3960479__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Ip_Address::Annotated',EXPIRE(7));
  SHARED __EE4266255 := __ENH_Person_Ip_Address;
  SHARED __IDX_Person_Ip_Address_Ip__Filtered := __EE4266255(__NN(__EE4266255.Ip_));
  ...
  EXPORT __ST4266257_Layout := RECORDOF(IDX_Person_Ip_Address_Ip_);
  EXPORT IDX_Person_Ip_Address_Ip__Wrapped := PROJECT(IDX_Person_Ip_Address_Ip_,TRANSFORM(__ST66786_Layout,SELF.Ip_ := __CN(LEFT.Ip_),SELF:=LEFT));

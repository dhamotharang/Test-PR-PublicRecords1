...
EXPORT B_Person_Ip_Address := MODULE
  ...
  SHARED __EE1110615 := __E_Person_Ip_Address;
  EXPORT __ST31069_Layout := RECORD
  ...
  END;
  SHARED __ST31069_Layout __ND1110649__Project(E_Person_Ip_Address.Layout __PP1110510) := TRANSFORM
    __EE1110548 := __PP1110510.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1110548,__T(__EE1110548).Event_Date_);
    __EE1110577 := __PP1110510.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1110577,__T(__EE1110577).Event_Date_);
    SELF := __PP1110510;
  END;
  EXPORT __ENH_Person_Ip_Address := PROJECT(__EE1110615,__ND1110649__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Ip_Address::Annotated',EXPIRE(7));
  SHARED __EE1379746 := __ENH_Person_Ip_Address;
  SHARED __IDX_Person_Ip_Address_Ip__Filtered := __EE1379746(__NN(__EE1379746.Ip_));
  ...
  EXPORT __ST1379748_Layout := RECORDOF(IDX_Person_Ip_Address_Ip_);
  EXPORT IDX_Person_Ip_Address_Ip__Wrapped := PROJECT(IDX_Person_Ip_Address_Ip_,TRANSFORM(__ST31069_Layout,SELF.Ip_ := __CN(LEFT.Ip_),SELF:=LEFT));

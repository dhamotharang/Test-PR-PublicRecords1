...
EXPORT B_Person_Ip_Address := MODULE
  ...
  SHARED __EE3894297 := __E_Person_Ip_Address;
  EXPORT __ST66883_Layout := RECORD
  ...
  END;
  SHARED __ST66883_Layout __ND3894331__Project(E_Person_Ip_Address.Layout __PP3894192) := TRANSFORM
    __EE3894230 := __PP3894192.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE3894230,__T(__EE3894230).Event_Date_);
    __EE3894259 := __PP3894192.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE3894259,__T(__EE3894259).Event_Date_);
    SELF := __PP3894192;
  END;
  EXPORT __ENH_Person_Ip_Address := PROJECT(__EE3894297,__ND3894331__Project(LEFT)) : PERSIST('~fraudgov::temp::KEL::FraudgovKEL::Person_Ip_Address::Annotated',EXPIRE(7));
  SHARED __EE4161639 := __ENH_Person_Ip_Address;
  SHARED __IDX_Person_Ip_Address_Ip__Filtered := __EE4161639(__NN(__EE4161639.Ip_));
  ...
  EXPORT __ST4161641_Layout := RECORDOF(IDX_Person_Ip_Address_Ip_);
  EXPORT IDX_Person_Ip_Address_Ip__Wrapped := PROJECT(IDX_Person_Ip_Address_Ip_,TRANSFORM(__ST66883_Layout,SELF.Ip_ := __CN(LEFT.Ip_),SELF:=LEFT));

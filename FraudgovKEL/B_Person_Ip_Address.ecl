...
EXPORT B_Person_Ip_Address := MODULE
  ...
  SHARED __EE2537765 := __E_Person_Ip_Address;
  EXPORT __ST53172_Layout := RECORD
  ...
  END;
  SHARED __ST53172_Layout __ND2537799__Project(E_Person_Ip_Address.Layout __PP2537660) := TRANSFORM
    __EE2537698 := __PP2537660.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2537698,__T(__EE2537698).Event_Date_);
    __EE2537727 := __PP2537660.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2537727,__T(__EE2537727).Event_Date_);
    SELF := __PP2537660;
  END;
  EXPORT __ENH_Person_Ip_Address := PROJECT(__EE2537765,__ND2537799__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Ip_Address::Annotated',EXPIRE(7));
  SHARED __EE2805631 := __ENH_Person_Ip_Address;
  SHARED __IDX_Person_Ip_Address_Ip__Filtered := __EE2805631(__NN(__EE2805631.Ip_));
  ...
  EXPORT __ST2805633_Layout := RECORDOF(IDX_Person_Ip_Address_Ip_);
  EXPORT IDX_Person_Ip_Address_Ip__Wrapped := PROJECT(IDX_Person_Ip_Address_Ip_,TRANSFORM(__ST53172_Layout,SELF.Ip_ := __CN(LEFT.Ip_),SELF:=LEFT));

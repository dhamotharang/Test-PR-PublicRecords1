...
EXPORT B_Person_Ip_Address := MODULE
  ...
  SHARED __EE1090002 := __E_Person_Ip_Address;
  EXPORT __ST30640_Layout := RECORD
  ...
  END;
  SHARED __ST30640_Layout __ND1090036__Project(E_Person_Ip_Address.Layout __PP1089897) := TRANSFORM
    __EE1089935 := __PP1089897.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1089935,__T(__EE1089935).Event_Date_);
    __EE1089964 := __PP1089897.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1089964,__T(__EE1089964).Event_Date_);
    SELF := __PP1089897;
  END;
  EXPORT __ENH_Person_Ip_Address := PROJECT(__EE1090002,__ND1090036__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Ip_Address::Annotated',EXPIRE(7));
  SHARED __EE1360674 := __ENH_Person_Ip_Address;
  SHARED __IDX_Person_Ip_Address_Ip__Filtered := __EE1360674(__NN(__EE1360674.Ip_));
  ...
  EXPORT __ST1360676_Layout := RECORDOF(IDX_Person_Ip_Address_Ip_);
  EXPORT IDX_Person_Ip_Address_Ip__Wrapped := PROJECT(IDX_Person_Ip_Address_Ip_,TRANSFORM(__ST30640_Layout,SELF.Ip_ := __CN(LEFT.Ip_),SELF:=LEFT));

...
EXPORT B_Person_Ip_Address := MODULE
  ...
  SHARED __EE4515294 := __E_Person_Ip_Address;
  ...
  END;
  SHARED __ST71694_Layout __ND4515328__Project(E_Person_Ip_Address.Layout __PP4515189) := TRANSFORM
    __EE4515227 := __PP4515189.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE4515227,__T(__EE4515227).Event_Date_);
    __EE4515256 := __PP4515189.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE4515256,__T(__EE4515256).Event_Date_);
    SELF := __PP4515189;
  END;
  EXPORT __ENH_Person_Ip_Address := PROJECT(__EE4515294,__ND4515328__Project(LEFT)) : PERSIST('~fraudgov::temp::KEL::FraudgovKEL::Person_Ip_Address::Annotated',EXPIRE(7));
  SHARED __EE4830786 := __ENH_Person_Ip_Address;
  SHARED __IDX_Person_Ip_Address_Ip__Filtered := __EE4830786(__NN(__EE4830786.Ip_));
  ...
  EXPORT __ST4830788_Layout := RECORDOF(IDX_Person_Ip_Address_Ip_);
  EXPORT IDX_Person_Ip_Address_Ip__Wrapped := PROJECT(IDX_Person_Ip_Address_Ip_,TRANSFORM(__ST71694_Layout,SELF.Ip_ := __CN(LEFT.Ip_),SELF:=LEFT));

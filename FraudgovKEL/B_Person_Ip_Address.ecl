...
EXPORT B_Person_Ip_Address := MODULE
  ...
  SHARED __EE2267936 := __E_Person_Ip_Address;
  EXPORT __ST45790_Layout := RECORD
  ...
  END;
  SHARED __ST45790_Layout __ND2267970__Project(E_Person_Ip_Address.Layout __PP2267831) := TRANSFORM
    __EE2267869 := __PP2267831.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2267869,__T(__EE2267869).Event_Date_);
    __EE2267898 := __PP2267831.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2267898,__T(__EE2267898).Event_Date_);
    SELF := __PP2267831;
  END;
  EXPORT __ENH_Person_Ip_Address := PROJECT(__EE2267936,__ND2267970__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Ip_Address::Annotated',EXPIRE(7));
  SHARED __EE2523626 := __ENH_Person_Ip_Address;
  SHARED __IDX_Person_Ip_Address_Ip__Filtered := __EE2523626(__NN(__EE2523626.Ip_));
  ...
  EXPORT __ST2523628_Layout := RECORDOF(IDX_Person_Ip_Address_Ip_);
  EXPORT IDX_Person_Ip_Address_Ip__Wrapped := PROJECT(IDX_Person_Ip_Address_Ip_,TRANSFORM(__ST45790_Layout,SELF.Ip_ := __CN(LEFT.Ip_),SELF:=LEFT));

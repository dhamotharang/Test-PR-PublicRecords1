...
EXPORT B_Person_Ip_Address := MODULE
  ...
  SHARED __EE4006789 := __E_Person_Ip_Address;
  EXPORT __ST67134_Layout := RECORD
  ...
  END;
  SHARED __ST67134_Layout __ND4006823__Project(E_Person_Ip_Address.Layout __PP4006684) := TRANSFORM
    __EE4006722 := __PP4006684.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE4006722,__T(__EE4006722).Event_Date_);
    __EE4006751 := __PP4006684.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE4006751,__T(__EE4006751).Event_Date_);
    SELF := __PP4006684;
  END;
  EXPORT __ENH_Person_Ip_Address := PROJECT(__EE4006789,__ND4006823__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Ip_Address::Annotated',EXPIRE(7));
  SHARED __EE4316791 := __ENH_Person_Ip_Address;
  SHARED __IDX_Person_Ip_Address_Ip__Filtered := __EE4316791(__NN(__EE4316791.Ip_));
  ...
  EXPORT __ST4316793_Layout := RECORDOF(IDX_Person_Ip_Address_Ip_);
  EXPORT IDX_Person_Ip_Address_Ip__Wrapped := PROJECT(IDX_Person_Ip_Address_Ip_,TRANSFORM(__ST67134_Layout,SELF.Ip_ := __CN(LEFT.Ip_),SELF:=LEFT));

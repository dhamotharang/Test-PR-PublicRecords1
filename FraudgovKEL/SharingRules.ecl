...
CustomerIds := TABLE(FraudgovKEL.fraudgov, {classification_Permissible_use_access.fdn_file_info_id, inclusion_id := (INTEGER)customer_id, classification_Permissible_use_access.ind_type, SourceCustomerHash := HASH32(TRIM(customer_id) + '|' + TRIM((STRING)classification_Permissible_use_access.ind_type))}, classification_Permissible_use_access.fdn_file_info_id, customer_id, classification_Permissible_use_access.ind_type, FEW);
...
EXPORT SharingRules := tempSharing; //OttoSharingUsingInclusionIdWithState;//CustomerIdState;//DEDUP(SORT(OttoSharingUsingInclusionId2, fdn_file_info_id, SourceCustomerHash, TargetCustomerHash), fdn_file_info_id, SourceCustomerHash, TargetCustomerHash); 

...
EXPORT B_Address_2 := MODULE
  ...
  SHARED __EE196923 := __ENH_Address_3;
  SHARED __EE199469 := __E_Address_Event;
  SHARED __EE203417 := __EE199469(__NN(__EE199469.Transaction_) AND __NN(__EE199469.Location_));
  SHARED __EE199471 := __ENH_Event_3;
  SHARED __ST200264_Layout := RECORD
  ...
  END;
  __JC203429(E_Address_Event.Layout __EE203417, B_Event_3.__ST40418_Layout __EE199471) := __EEQP(__EE203417.Transaction_,__EE199471.UID);
  __ST200264_Layout __JT203429(E_Address_Event.Layout __l, B_Event_3.__ST40418_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE203430 := JOIN(__EE203417,__EE199471,__JC203429(LEFT,RIGHT),__JT203429(LEFT,RIGHT),INNER,HASH);
  SHARED __EE199425 := __EE196923;
  __JC203658(__ST200264_Layout __EE203430, B_Address_3.__ST39584_Layout __EE199425) := __EEQP(__EE199425.UID,__EE203430.Location_) AND __T(__AND(__EEQ(__EE199425.UID,__EE203430.Location_),__OP2(__EE203430.Event_Date__1_,>,__EE199425.Kr_Last_Event_Date_)));
  SHARED __EE203659 := JOIN(__EE203430,__EE199425,__JC203658(LEFT,RIGHT),TRANSFORM(__ST200264_Layout,SELF:=LEFT),HASH,KEEP(1));
  SHARED __ST199511_Layout := RECORD
    ...
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __EE203891 := PROJECT(__EE203659,TRANSFORM(__ST199511_Layout,SELF.UID := LEFT.Location_,SELF := LEFT));
  SHARED __ST199535_Layout := RECORD
    KEL.typ.int C_O_U_N_T___Address_Event_ := 0;
    KEL.typ.ntyp(E_Address.Typ) UID;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __EE203904 := PROJECT(__CLEANANDDO(__EE203891,TABLE(__EE203891,{KEL.typ.int C_O_U_N_T___Address_Event_ := COUNT(GROUP),UID},UID,MERGE)),__ST199535_Layout);
  SHARED __ST200774_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC203910(B_Address_3.__ST39584_Layout __EE196923, __ST199535_Layout __EE203904) := __EEQP(__EE196923.UID,__EE203904.UID);
  __ST200774_Layout __JT203910(B_Address_3.__ST39584_Layout __l, __ST199535_Layout __r) := TRANSFORM
    SELF.U_I_D__1_ := __r.UID;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE203911 := JOIN(__EE196923,__EE203904,__JC203910(LEFT,RIGHT),__JT203910(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE197788 := __ENH_Person_3;
  SHARED __EE197790 := __E_Person_Address;
  SHARED __EE203971 := __EE197790(__NN(__EE197790.Location_) AND __NN(__EE197790.Subject_));
  SHARED __ST199707_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.nint Lex_Id_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC203989(B_Person_3.__ST40961_Layout __EE197788, E_Person_Address.Layout __EE203971) := __EEQP(__EE203971.Subject_,__EE197788.UID);
  __ST199707_Layout __JT203989(B_Person_3.__ST40961_Layout __l, E_Person_Address.Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE203990 := JOIN(__EE203971,__EE197788,__JC203989(RIGHT,LEFT),__JT203989(RIGHT,LEFT),INNER,HASH);
  SHARED __ST198663_Layout := RECORD
    KEL.typ.ntyp(E_Address.Typ) UID;
    ...
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __ST198663_Layout __ND204115__Project(__ST199707_Layout __PP203991) := TRANSFORM
    SELF.UID := __PP203991.Location_;
    SELF.U_I_D__1_ := __PP203991.UID;
    SELF := __PP203991;
  END;
  SHARED __EE204518 := PROJECT(__EE203990,__ND204115__Project(LEFT));
  SHARED __ST198926_Layout := RECORD
    KEL.typ.ntyp(E_Address.Typ) UID;
    KEL.typ.int High_Risk_Death_Prior_To_All_Events_ := 0;
    KEL.typ.nint Exp1_;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __ST198926_Layout __ND204801__Project(__ST198663_Layout __PP204519) := TRANSFORM
    SELF.Exp1_ := IF(__PP204519.In_Customer_Population_ = 1,__ECAST(KEL.typ.nint,__CN(__PP204519.High_Risk_Death_Prior_To_All_Events_)),__ECAST(KEL.typ.nint,__N(KEL.typ.int)));
    SELF := __PP204519;
  END;
  SHARED __EE204806 := PROJECT(__EE204518,__ND204801__Project(LEFT));
  SHARED __ST198946_Layout := RECORD
    ...
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __EE204827 := PROJECT(__CLEANANDDO(__EE204806,TABLE(__EE204806,{KEL.typ.int S_U_M___High_Risk_Death_Prior_To_All_Events_ := SUM(GROUP,__EE204806.High_Risk_Death_Prior_To_All_Events_),KEL.typ.int S_U_M___High_Risk_Death_Prior_To_All_Events__1_ := KEL.Aggregates.SumNG(__EE204806.Exp1_),UID},UID,MERGE)),__ST198946_Layout);
  SHARED __ST201022_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ndataset(E_Address.Source_Customers_Layout) Source_Customers_;
    KEL.typ.nbool _addresspobox_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC204833(__ST200774_Layout __EE203911, __ST198946_Layout __EE204827) := __EEQP(__EE203911.UID,__EE204827.UID);
  __ST201022_Layout __JT204833(__ST200774_Layout __l, __ST198946_Layout __r) := TRANSFORM
    SELF.U_I_D__2_ := __r.UID;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE204879 := JOIN(__EE203911,__EE204827,__JC204833(LEFT,RIGHT),__JT204833(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST37569_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ndataset(E_Address.Source_Customers_Layout) Source_Customers_;
    KEL.typ.nbool _addresspobox_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST37569_Layout __ND204884__Project(__ST201022_Layout __PP204880) := TRANSFORM
    SELF.All_High_Risk_Death_Prior_To_All_Events_Person_Count_ := __PP204880.S_U_M___High_Risk_Death_Prior_To_All_Events_;
    SELF.High_Frequency_Flag_ := MAP(__PP204880.Identity_Count_ >= 20=>1,0);
    SELF.High_Risk_Death_Prior_To_All_Events_Person_Count_ := __PP204880.S_U_M___High_Risk_Death_Prior_To_All_Events__1_;
    __BS205003 := __T(__PP204880.Source_Customers_);
    SELF.In_Customer_Population_ := MAP(EXISTS(__BS205003(__T(__OP2(__T(__PP204880.Source_Customers_)._r_Source_Customer_,=,__PP204880._r_Customer_))))=>1,0);
    SELF.Kr_Event_After_Last_Known_Risk_Count_ := __PP204880.C_O_U_N_T___Address_Event_;
    SELF := __PP204880;
  END;
  EXPORT __ENH_Address_2 := PROJECT(__EE204879,__ND204884__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Address::Annotated_2',EXPIRE(7));
END;

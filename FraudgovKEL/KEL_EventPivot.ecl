...
SHARED PivotToEntities :=
            NORMALIZE(InputWithRules,8,NormIt(LEFT,COUNTER))(label != '' AND entitycontextuid[4..] NOT IN NonEntities) : PERSIST('~fraudgov::tempfraudgov::temp::eventpivot'); // exclude entities that didn't exist on the transaction.
...
SHARED PivotWithHistoricalCurrentFlags := JOIN(PivotToEntitiesWithHRICounts, dDedup,
  ...
  SELF := LEFT), LEFT OUTER, HASH) : PERSIST('~fraudgov::temp::PivotWithHistoricalCurrentFlags', EXPIRE(7));

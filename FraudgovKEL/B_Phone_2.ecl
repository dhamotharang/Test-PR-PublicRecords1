...
EXPORT B_Phone_2 := MODULE
  ...
  SHARED __EE250973 := __ENH_Phone_3;
  SHARED __EE251031 := __E_Phone_Event;
  SHARED __EE252462 := __EE251031(__NN(__EE251031.Transaction_) AND __NN(__EE251031.Phone_Number_));
  SHARED __EE251033 := __ENH_Event_3;
  SHARED __ST251196_Layout := RECORD
  ...
  END;
  __JC252474(E_Phone_Event.Layout __EE252462, B_Event_3.__ST40418_Layout __EE251033) := __EEQP(__EE252462.Transaction_,__EE251033.UID);
  __ST251196_Layout __JT252474(E_Phone_Event.Layout __l, B_Event_3.__ST40418_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE252475 := JOIN(__EE252462,__EE251033,__JC252474(LEFT,RIGHT),__JT252474(LEFT,RIGHT),INNER,HASH);
  SHARED __EE250987 := __EE250973;
  __JC252703(__ST251196_Layout __EE252475, B_Phone_4.__ST42669_Layout __EE250987) := __EEQP(__EE250987.UID,__EE252475.Phone_Number_) AND __T(__AND(__EEQ(__EE250987.UID,__EE252475.Phone_Number_),__OP2(__EE252475.Event_Date__1_,>,__EE250987.Kr_Last_Event_Date_)));
  SHARED __EE252704 := JOIN(__EE252475,__EE250987,__JC252703(LEFT,RIGHT),TRANSFORM(__ST251196_Layout,SELF:=LEFT),HASH,KEEP(1));
  SHARED __ST251073_Layout := RECORD
    ...
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __EE252936 := PROJECT(__EE252704,TRANSFORM(__ST251073_Layout,SELF.UID := LEFT.Phone_Number_,SELF := LEFT));
  SHARED __ST251097_Layout := RECORD
    KEL.typ.int C_O_U_N_T___Phone_Event_ := 0;
    KEL.typ.ntyp(E_Phone.Typ) UID;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __EE252949 := PROJECT(__CLEANANDDO(__EE252936,TABLE(__EE252936,{KEL.typ.int C_O_U_N_T___Phone_Event_ := COUNT(GROUP),UID},UID,MERGE)),__ST251097_Layout);
  SHARED __ST251704_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC252955(B_Phone_4.__ST42669_Layout __EE250973, __ST251097_Layout __EE252949) := __EEQP(__EE250973.UID,__EE252949.UID);
  __ST251704_Layout __JT252955(B_Phone_4.__ST42669_Layout __l, __ST251097_Layout __r) := TRANSFORM
    SELF.U_I_D__1_ := __r.UID;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE252956 := JOIN(__EE250973,__EE252949,__JC252955(LEFT,RIGHT),__JT252955(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST39445_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ENH_Phone_2 := PROJECT(__EE252956,TRANSFORM(__ST39445_Layout,SELF.Kr_Event_After_Last_Known_Risk_Count_ := LEFT.C_O_U_N_T___Phone_Event_,SELF := LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Phone::Annotated_2',EXPIRE(7));
END;

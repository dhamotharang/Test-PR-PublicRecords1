...
EXPORT B_Person_Bank_Account := MODULE
  ...
  SHARED __EE4004223 := __E_Person_Bank_Account;
  EXPORT __ST67005_Layout := RECORD
  ...
  END;
  SHARED __ST67005_Layout __ND4004257__Project(E_Person_Bank_Account.Layout __PP4004118) := TRANSFORM
    __EE4004156 := __PP4004118.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE4004156,__T(__EE4004156).Event_Date_);
    __EE4004185 := __PP4004118.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE4004185,__T(__EE4004185).Event_Date_);
    SELF := __PP4004118;
  END;
  EXPORT __ENH_Person_Bank_Account := PROJECT(__EE4004223,__ND4004257__Project(LEFT)) : PERSIST('~fraudgov::temp::KEL::FraudgovKEL::Person_Bank_Account::Annotated',EXPIRE(7));
  ...
  EXPORT __ST4315772_Layout := RECORDOF(IDX_Person_Bank_Account_Account_);
  EXPORT IDX_Person_Bank_Account_Account__Wrapped := PROJECT(IDX_Person_Bank_Account_Account_,TRANSFORM(__ST67005_Layout,SELF.Account_ := __CN(LEFT.Account_),SELF:=LEFT));

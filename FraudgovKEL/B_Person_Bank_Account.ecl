...
EXPORT B_Person_Bank_Account := MODULE
  ...
  SHARED __EE3803317 := __E_Person_Bank_Account;
  EXPORT __ST66071_Layout := RECORD
  ...
  END;
  SHARED __ST66071_Layout __ND3803351__Project(E_Person_Bank_Account.Layout __PP3803212) := TRANSFORM
    __EE3803250 := __PP3803212.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE3803250,__T(__EE3803250).Event_Date_);
    __EE3803279 := __PP3803212.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE3803279,__T(__EE3803279).Event_Date_);
    SELF := __PP3803212;
  END;
  EXPORT __ENH_Person_Bank_Account := PROJECT(__EE3803317,__ND3803351__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Bank_Account::Annotated',EXPIRE(7));
  SHARED __EE4093852 := __ENH_Person_Bank_Account;
  SHARED __IDX_Person_Bank_Account_Account__Filtered := __EE4093852(__NN(__EE4093852.Account_));
  ...
  EXPORT __ST4093854_Layout := RECORDOF(IDX_Person_Bank_Account_Account_);
  EXPORT IDX_Person_Bank_Account_Account__Wrapped := PROJECT(IDX_Person_Bank_Account_Account_,TRANSFORM(__ST66071_Layout,SELF.Account_ := __CN(LEFT.Account_),SELF:=LEFT));

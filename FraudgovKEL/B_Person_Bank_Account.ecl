...
EXPORT B_Person_Bank_Account := MODULE
  ...
  SHARED __EE2088046 := __E_Person_Bank_Account;
  EXPORT __ST45354_Layout := RECORD
  ...
  END;
  SHARED __ST45354_Layout __ND2088080__Project(E_Person_Bank_Account.Layout __PP2087941) := TRANSFORM
    __EE2087979 := __PP2087941.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2087979,__T(__EE2087979).Event_Date_);
    __EE2088008 := __PP2087941.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2088008,__T(__EE2088008).Event_Date_);
    SELF := __PP2087941;
  END;
  EXPORT __ENH_Person_Bank_Account := PROJECT(__EE2088046,__ND2088080__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Bank_Account::Annotated',EXPIRE(7));
  SHARED __EE2334650 := __ENH_Person_Bank_Account;
  SHARED __IDX_Person_Bank_Account_Account__Filtered := __EE2334650(__NN(__EE2334650.Account_));
  ...
  EXPORT __ST2334652_Layout := RECORDOF(IDX_Person_Bank_Account_Account_);
  EXPORT IDX_Person_Bank_Account_Account__Wrapped := PROJECT(IDX_Person_Bank_Account_Account_,TRANSFORM(__ST45354_Layout,SELF.Account_ := __CN(LEFT.Account_),SELF:=LEFT));

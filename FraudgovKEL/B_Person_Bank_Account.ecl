...
EXPORT B_Person_Bank_Account := MODULE
  ...
  SHARED __EE1064227 := __E_Person_Bank_Account;
  EXPORT __ST28955_Layout := RECORD
  ...
  END;
  SHARED __ST28955_Layout __ND1064261__Project(E_Person_Bank_Account.Layout __PP1064122) := TRANSFORM
    __EE1064160 := __PP1064122.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1064160,__T(__EE1064160).Event_Date_);
    __EE1064189 := __PP1064122.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1064189,__T(__EE1064189).Event_Date_);
    SELF := __PP1064122;
  END;
  EXPORT __ENH_Person_Bank_Account := PROJECT(__EE1064227,__ND1064261__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Bank_Account::Annotated',EXPIRE(7));
  SHARED __EE1284074 := __ENH_Person_Bank_Account;
  SHARED __IDX_Person_Bank_Account_Account__Filtered := __EE1284074(__NN(__EE1284074.Account_));
  ...
  EXPORT __ST1284076_Layout := RECORDOF(IDX_Person_Bank_Account_Account_);
  EXPORT IDX_Person_Bank_Account_Account__Wrapped := PROJECT(IDX_Person_Bank_Account_Account_,TRANSFORM(__ST28955_Layout,SELF.Account_ := __CN(LEFT.Account_),SELF:=LEFT));

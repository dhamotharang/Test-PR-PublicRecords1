...
EXPORT B_Person_Bank_Account := MODULE
  ...
  SHARED __EE2155960 := __E_Person_Bank_Account;
  EXPORT __ST45425_Layout := RECORD
  ...
  END;
  SHARED __ST45425_Layout __ND2155994__Project(E_Person_Bank_Account.Layout __PP2155855) := TRANSFORM
    __EE2155893 := __PP2155855.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2155893,__T(__EE2155893).Event_Date_);
    __EE2155922 := __PP2155855.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2155922,__T(__EE2155922).Event_Date_);
    SELF := __PP2155855;
  END;
  EXPORT __ENH_Person_Bank_Account := PROJECT(__EE2155960,__ND2155994__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Bank_Account::Annotated',EXPIRE(7));
  SHARED __EE2406871 := __ENH_Person_Bank_Account;
  SHARED __IDX_Person_Bank_Account_Account__Filtered := __EE2406871(__NN(__EE2406871.Account_));
  ...
  EXPORT __ST2406873_Layout := RECORDOF(IDX_Person_Bank_Account_Account_);
  EXPORT IDX_Person_Bank_Account_Account__Wrapped := PROJECT(IDX_Person_Bank_Account_Account_,TRANSFORM(__ST45425_Layout,SELF.Account_ := __CN(LEFT.Account_),SELF:=LEFT));

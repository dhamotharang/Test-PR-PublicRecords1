...
EXPORT B_Person_Bank_Account := MODULE
  ...
  SHARED __EE3144042 := __E_Person_Bank_Account;
  EXPORT __ST63381_Layout := RECORD
  ...
  END;
  SHARED __ST63381_Layout __ND3144076__Project(E_Person_Bank_Account.Layout __PP3143937) := TRANSFORM
    __EE3143975 := __PP3143937.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE3143975,__T(__EE3143975).Event_Date_);
    __EE3144004 := __PP3143937.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE3144004,__T(__EE3144004).Event_Date_);
    SELF := __PP3143937;
  END;
  EXPORT __ENH_Person_Bank_Account := PROJECT(__EE3144042,__ND3144076__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Bank_Account::Annotated',EXPIRE(7));
  SHARED __EE3429745 := __ENH_Person_Bank_Account;
  SHARED __IDX_Person_Bank_Account_Account__Filtered := __EE3429745(__NN(__EE3429745.Account_));
  ...
  EXPORT __ST3429747_Layout := RECORDOF(IDX_Person_Bank_Account_Account_);
  EXPORT IDX_Person_Bank_Account_Account__Wrapped := PROJECT(IDX_Person_Bank_Account_Account_,TRANSFORM(__ST63381_Layout,SELF.Account_ := __CN(LEFT.Account_),SELF:=LEFT));

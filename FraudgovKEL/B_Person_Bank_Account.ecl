...
EXPORT B_Person_Bank_Account := MODULE
  ...
  SHARED __EE1110069 := __E_Person_Bank_Account;
  EXPORT __ST31000_Layout := RECORD
  ...
  END;
  SHARED __ST31000_Layout __ND1110103__Project(E_Person_Bank_Account.Layout __PP1109964) := TRANSFORM
    __EE1110002 := __PP1109964.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1110002,__T(__EE1110002).Event_Date_);
    __EE1110031 := __PP1109964.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1110031,__T(__EE1110031).Event_Date_);
    SELF := __PP1109964;
  END;
  EXPORT __ENH_Person_Bank_Account := PROJECT(__EE1110069,__ND1110103__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Bank_Account::Annotated',EXPIRE(7));
  SHARED __EE1379668 := __ENH_Person_Bank_Account;
  SHARED __IDX_Person_Bank_Account_Account__Filtered := __EE1379668(__NN(__EE1379668.Account_));
  ...
  EXPORT __ST1379670_Layout := RECORDOF(IDX_Person_Bank_Account_Account_);
  EXPORT IDX_Person_Bank_Account_Account__Wrapped := PROJECT(IDX_Person_Bank_Account_Account_,TRANSFORM(__ST31000_Layout,SELF.Account_ := __CN(LEFT.Account_),SELF:=LEFT));

...
EXPORT B_Person_Bank_Account := MODULE
  ...
  SHARED __EE2095376 := __E_Person_Bank_Account;
  EXPORT __ST45363_Layout := RECORD
  ...
  END;
  SHARED __ST45363_Layout __ND2095410__Project(E_Person_Bank_Account.Layout __PP2095271) := TRANSFORM
    __EE2095309 := __PP2095271.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2095309,__T(__EE2095309).Event_Date_);
    __EE2095338 := __PP2095271.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2095338,__T(__EE2095338).Event_Date_);
    SELF := __PP2095271;
  END;
  EXPORT __ENH_Person_Bank_Account := PROJECT(__EE2095376,__ND2095410__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Bank_Account::Annotated',EXPIRE(7));
  SHARED __EE2342257 := __ENH_Person_Bank_Account;
  SHARED __IDX_Person_Bank_Account_Account__Filtered := __EE2342257(__NN(__EE2342257.Account_));
  ...
  EXPORT __ST2342259_Layout := RECORDOF(IDX_Person_Bank_Account_Account_);
  EXPORT IDX_Person_Bank_Account_Account__Wrapped := PROJECT(IDX_Person_Bank_Account_Account_,TRANSFORM(__ST45363_Layout,SELF.Account_ := __CN(LEFT.Account_),SELF:=LEFT));

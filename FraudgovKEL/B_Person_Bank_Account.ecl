...
EXPORT B_Person_Bank_Account := MODULE
  ...
  SHARED __EE1076803 := __E_Person_Bank_Account;
  EXPORT __ST29003_Layout := RECORD
  ...
  END;
  SHARED __ST29003_Layout __ND1076837__Project(E_Person_Bank_Account.Layout __PP1076698) := TRANSFORM
    __EE1076736 := __PP1076698.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1076736,__T(__EE1076736).Event_Date_);
    __EE1076765 := __PP1076698.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1076765,__T(__EE1076765).Event_Date_);
    SELF := __PP1076698;
  END;
  EXPORT __ENH_Person_Bank_Account := PROJECT(__EE1076803,__ND1076837__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Bank_Account::Annotated',EXPIRE(7));
  SHARED __EE1297996 := __ENH_Person_Bank_Account;
  SHARED __IDX_Person_Bank_Account_Account__Filtered := __EE1297996(__NN(__EE1297996.Account_));
  ...
  EXPORT __ST1297998_Layout := RECORDOF(IDX_Person_Bank_Account_Account_);
  EXPORT IDX_Person_Bank_Account_Account__Wrapped := PROJECT(IDX_Person_Bank_Account_Account_,TRANSFORM(__ST29003_Layout,SELF.Account_ := __CN(LEFT.Account_),SELF:=LEFT));

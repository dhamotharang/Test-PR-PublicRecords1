...
EXPORT B_Person_Phone := MODULE
  ...
  SHARED __EE1110797 := __E_Person_Phone;
  EXPORT __ST31092_Layout := RECORD
  ...
  END;
  SHARED __ST31092_Layout __ND1110831__Project(E_Person_Phone.Layout __PP1110692) := TRANSFORM
    __EE1110730 := __PP1110692.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1110730,__T(__EE1110730).Event_Date_);
    __EE1110759 := __PP1110692.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1110759,__T(__EE1110759).Event_Date_);
    SELF := __PP1110692;
  END;
  EXPORT __ENH_Person_Phone := PROJECT(__EE1110797,__ND1110831__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Phone::Annotated',EXPIRE(7));
  SHARED __EE1379760 := __ENH_Person_Phone;
  SHARED __IDX_Person_Phone_Phone_Number__Filtered := __EE1379760(__NN(__EE1379760.Phone_Number_));
  ...
  EXPORT __ST1379762_Layout := RECORDOF(IDX_Person_Phone_Phone_Number_);
  EXPORT IDX_Person_Phone_Phone_Number__Wrapped := PROJECT(IDX_Person_Phone_Phone_Number_,TRANSFORM(__ST31092_Layout,SELF.Phone_Number_ := __CN(LEFT.Phone_Number_),SELF:=LEFT));

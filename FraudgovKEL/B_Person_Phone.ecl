...
EXPORT B_Person_Phone := MODULE
  ...
  SHARED __EE1064955 := __E_Person_Phone;
  EXPORT __ST29047_Layout := RECORD
  ...
  END;
  SHARED __ST29047_Layout __ND1064989__Project(E_Person_Phone.Layout __PP1064850) := TRANSFORM
    __EE1064888 := __PP1064850.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1064888,__T(__EE1064888).Event_Date_);
    __EE1064917 := __PP1064850.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1064917,__T(__EE1064917).Event_Date_);
    SELF := __PP1064850;
  END;
  EXPORT __ENH_Person_Phone := PROJECT(__EE1064955,__ND1064989__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Phone::Annotated',EXPIRE(7));
  SHARED __EE1285502 := __ENH_Person_Phone;
  SHARED __IDX_Person_Phone_Phone_Number__Filtered := __EE1285502(__NN(__EE1285502.Phone_Number_));
  ...
  EXPORT __ST1285504_Layout := RECORDOF(IDX_Person_Phone_Phone_Number_);
  EXPORT IDX_Person_Phone_Phone_Number__Wrapped := PROJECT(IDX_Person_Phone_Phone_Number_,TRANSFORM(__ST29047_Layout,SELF.Phone_Number_ := __CN(LEFT.Phone_Number_),SELF:=LEFT));

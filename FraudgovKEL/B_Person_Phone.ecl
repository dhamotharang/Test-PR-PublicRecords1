...
EXPORT B_Person_Phone := MODULE
  ...
  SHARED __EE2148559 := __E_Person_Phone;
  EXPORT __ST45507_Layout := RECORD
  ...
  END;
  SHARED __ST45507_Layout __ND2148593__Project(E_Person_Phone.Layout __PP2148454) := TRANSFORM
    __EE2148492 := __PP2148454.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2148492,__T(__EE2148492).Event_Date_);
    __EE2148521 := __PP2148454.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2148521,__T(__EE2148521).Event_Date_);
    SELF := __PP2148454;
  END;
  EXPORT __ENH_Person_Phone := PROJECT(__EE2148559,__ND2148593__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Phone::Annotated',EXPIRE(7));
  SHARED __EE2397490 := __ENH_Person_Phone;
  SHARED __IDX_Person_Phone_Phone_Number__Filtered := __EE2397490(__NN(__EE2397490.Phone_Number_));
  ...
  EXPORT __ST2397492_Layout := RECORDOF(IDX_Person_Phone_Phone_Number_);
  EXPORT IDX_Person_Phone_Phone_Number__Wrapped := PROJECT(IDX_Person_Phone_Phone_Number_,TRANSFORM(__ST45507_Layout,SELF.Phone_Number_ := __CN(LEFT.Phone_Number_),SELF:=LEFT));

...
EXPORT B_Person_Phone := MODULE
  ...
  SHARED __EE2156688 := __E_Person_Phone;
  EXPORT __ST45517_Layout := RECORD
  ...
  END;
  SHARED __ST45517_Layout __ND2156722__Project(E_Person_Phone.Layout __PP2156583) := TRANSFORM
    __EE2156621 := __PP2156583.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2156621,__T(__EE2156621).Event_Date_);
    __EE2156650 := __PP2156583.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2156650,__T(__EE2156650).Event_Date_);
    SELF := __PP2156583;
  END;
  EXPORT __ENH_Person_Phone := PROJECT(__EE2156688,__ND2156722__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Phone::Annotated',EXPIRE(7));
  SHARED __EE2406454 := __ENH_Person_Phone;
  SHARED __IDX_Person_Phone_Phone_Number__Filtered := __EE2406454(__NN(__EE2406454.Phone_Number_));
  ...
  EXPORT __ST2406456_Layout := RECORDOF(IDX_Person_Phone_Phone_Number_);
  EXPORT IDX_Person_Phone_Phone_Number__Wrapped := PROJECT(IDX_Person_Phone_Phone_Number_,TRANSFORM(__ST45517_Layout,SELF.Phone_Number_ := __CN(LEFT.Phone_Number_),SELF:=LEFT));

...
EXPORT B_Person_Phone := MODULE
  ...
  SHARED __EE2088774 := __E_Person_Phone;
  EXPORT __ST45446_Layout := RECORD
  ...
  END;
  SHARED __ST45446_Layout __ND2088808__Project(E_Person_Phone.Layout __PP2088669) := TRANSFORM
    __EE2088707 := __PP2088669.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2088707,__T(__EE2088707).Event_Date_);
    __EE2088736 := __PP2088669.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2088736,__T(__EE2088736).Event_Date_);
    SELF := __PP2088669;
  END;
  EXPORT __ENH_Person_Phone := PROJECT(__EE2088774,__ND2088808__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Phone::Annotated',EXPIRE(7));
  SHARED __EE2336112 := __ENH_Person_Phone;
  SHARED __IDX_Person_Phone_Phone_Number__Filtered := __EE2336112(__NN(__EE2336112.Phone_Number_));
  ...
  EXPORT __ST2336114_Layout := RECORDOF(IDX_Person_Phone_Phone_Number_);
  EXPORT IDX_Person_Phone_Phone_Number__Wrapped := PROJECT(IDX_Person_Phone_Phone_Number_,TRANSFORM(__ST45446_Layout,SELF.Phone_Number_ := __CN(LEFT.Phone_Number_),SELF:=LEFT));

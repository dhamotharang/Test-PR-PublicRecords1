...
EXPORT B_Person_Phone := MODULE
  ...
  SHARED __EE2096104 := __E_Person_Phone;
  EXPORT __ST45455_Layout := RECORD
  ...
  END;
  SHARED __ST45455_Layout __ND2096138__Project(E_Person_Phone.Layout __PP2095999) := TRANSFORM
    __EE2096037 := __PP2095999.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2096037,__T(__EE2096037).Event_Date_);
    __EE2096066 := __PP2095999.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2096066,__T(__EE2096066).Event_Date_);
    SELF := __PP2095999;
  END;
  EXPORT __ENH_Person_Phone := PROJECT(__EE2096104,__ND2096138__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Phone::Annotated',EXPIRE(7));
  SHARED __EE2342539 := __ENH_Person_Phone;
  SHARED __IDX_Person_Phone_Phone_Number__Filtered := __EE2342539(__NN(__EE2342539.Phone_Number_));
  ...
  EXPORT __ST2342541_Layout := RECORDOF(IDX_Person_Phone_Phone_Number_);
  EXPORT IDX_Person_Phone_Phone_Number__Wrapped := PROJECT(IDX_Person_Phone_Phone_Number_,TRANSFORM(__ST45455_Layout,SELF.Phone_Number_ := __CN(LEFT.Phone_Number_),SELF:=LEFT));

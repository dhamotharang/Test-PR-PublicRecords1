...
EXPORT B_Person_Phone := MODULE
  ...
  SHARED __EE2345193 := __E_Person_Phone;
  EXPORT __ST45894_Layout := RECORD
  ...
  END;
  SHARED __ST45894_Layout __ND2345227__Project(E_Person_Phone.Layout __PP2345088) := TRANSFORM
    __EE2345126 := __PP2345088.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2345126,__T(__EE2345126).Event_Date_);
    __EE2345155 := __PP2345088.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2345155,__T(__EE2345155).Event_Date_);
    SELF := __PP2345088;
  END;
  EXPORT __ENH_Person_Phone := PROJECT(__EE2345193,__ND2345227__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Phone::Annotated',EXPIRE(7));
  SHARED __EE2604832 := __ENH_Person_Phone;
  SHARED __IDX_Person_Phone_Phone_Number__Filtered := __EE2604832(__NN(__EE2604832.Phone_Number_));
  ...
  EXPORT __ST2604834_Layout := RECORDOF(IDX_Person_Phone_Phone_Number_);
  EXPORT IDX_Person_Phone_Phone_Number__Wrapped := PROJECT(IDX_Person_Phone_Phone_Number_,TRANSFORM(__ST45894_Layout,SELF.Phone_Number_ := __CN(LEFT.Phone_Number_),SELF:=LEFT));

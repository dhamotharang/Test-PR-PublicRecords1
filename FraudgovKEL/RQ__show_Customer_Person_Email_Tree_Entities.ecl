...
EXPORT RQ__show_Customer_Person_Email_Tree_Entities := MODULE
  SHARED __EE4415295 := PROJECT(B_Person_Email.IDX_Person_Email_Emailof__Wrapped,E_Person_Email.Layout);
  SHARED __EE4415298 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST4312004_Layout := RECORD
  ...
  END;
  __JC4415306(E_Person_Email.Layout __EE4415295, B_Person.__ST66766_Layout __EE4415298) := __EEQP(__EE4415295.Subject_,__EE4415298.UID);
  __ST4312004_Layout __JT4415306(E_Person_Email.Layout __l, B_Person.__ST66766_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE4415307 := JOIN(__EE4415295,__EE4415298,__JC4415306(LEFT,RIGHT),__JT4415306(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4415602 := B_Email.IDX_Email_UID_Wrapped;
  SHARED __ST4312345_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Email.Typ) Emailof_;
    KEL.typ.ndataset(E_Person_Email.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4415610(__ST4312004_Layout __EE4415307, B_Email.__ST48609_Layout __EE4415602) := __EEQP(__EE4415307.Emailof_,__EE4415602.UID);
  __ST4312345_Layout __JT4415610(__ST4312004_Layout __l, B_Email.__ST48609_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4415611 := JOIN(__EE4415307,__EE4415602,__JC4415610(LEFT,RIGHT),__JT4415610(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4416530 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST4312857_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Email.Typ) Emailof_;
    KEL.typ.ndataset(E_Person_Email.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4416544(__ST4312345_Layout __EE4415611, E_Customer.Layout __EE4416530) := __EEQP(__EE4415611._r_Customer_,__EE4416530.UID);
  __ST4312857_Layout __JT4416544(__ST4312345_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4416941 := JOIN(__EE4415611,__EE4416530,__JC4416544(LEFT,RIGHT),__JT4416544(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST4311909_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST4311909_Layout __ND4417329__Project(__ST4312857_Layout __PP4416942) := TRANSFORM
    SELF.Source_Customer_ := __PP4416942._r_Customer_;
    SELF.Customer_Id_ := __PP4416942.Customer_Id__2_;
    SELF.Industry_Type_ := __PP4416942.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP4416942.Entity_Context_Uid__1_;
    SELF := __PP4416942;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE4416941,__ND4417329__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST4311909_Layout));
END;

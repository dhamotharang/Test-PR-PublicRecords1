...
EXPORT RQ__show_Customer_Person_Email_Tree_Entities := MODULE
  SHARED __EE2486253 := PROJECT(B_Person_Email.IDX_Person_Email_Emailof__Wrapped,E_Person_Email.Layout);
  SHARED __EE2486256 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST2393511_Layout := RECORD
  ...
  END;
  __JC2486264(E_Person_Email.Layout __EE2486253, B_Person.__ST45138_Layout __EE2486256) := __EEQP(__EE2486253.Subject_,__EE2486256.UID);
  __ST2393511_Layout __JT2486264(E_Person_Email.Layout __l, B_Person.__ST45138_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE2486265 := JOIN(__EE2486253,__EE2486256,__JC2486264(LEFT,RIGHT),__JT2486264(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE2486535 := B_Email.IDX_Email_UID_Wrapped;
  SHARED __ST2393825_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Email.Typ) Emailof_;
    KEL.typ.ndataset(E_Person_Email.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC2486543(__ST2393511_Layout __EE2486265, B_Email.__ST33532_Layout __EE2486535) := __EEQP(__EE2486265.Emailof_,__EE2486535.UID);
  __ST2393825_Layout __JT2486543(__ST2393511_Layout __l, B_Email.__ST33532_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE2486544 := JOIN(__EE2486265,__EE2486535,__JC2486543(LEFT,RIGHT),__JT2486543(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE2487357 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST2394271_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Email.Typ) Emailof_;
    KEL.typ.ndataset(E_Person_Email.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC2487371(__ST2393825_Layout __EE2486544, E_Customer.Layout __EE2487357) := __EEQP(__EE2486544._r_Customer_,__EE2487357.UID);
  __ST2394271_Layout __JT2487371(__ST2393825_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE2487726 := JOIN(__EE2486544,__EE2487357,__JC2487371(LEFT,RIGHT),__JT2487371(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST2393416_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST2393416_Layout __ND2488072__Project(__ST2394271_Layout __PP2487727) := TRANSFORM
    SELF.Source_Customer_ := __PP2487727._r_Customer_;
    SELF.Customer_Id_ := __PP2487727.Customer_Id__2_;
    SELF.Industry_Type_ := __PP2487727.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP2487727.Entity_Context_Uid__1_;
    SELF := __PP2487727;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE2487726,__ND2488072__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST2393416_Layout));
END;

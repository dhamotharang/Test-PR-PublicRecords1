...
EXPORT RQ__show_Customer_Person_Email_Tree_Entities := MODULE
  SHARED __EE2365116 := PROJECT(B_Person_Email.IDX_Person_Email_Emailof__Wrapped,E_Person_Email.Layout);
  SHARED __EE2365119 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST2273092_Layout := RECORD
  ...
  END;
  __JC2365127(E_Person_Email.Layout __EE2365116, B_Person.__ST43253_Layout __EE2365119) := __EEQP(__EE2365116.Subject_,__EE2365119.UID);
  __ST2273092_Layout __JT2365127(E_Person_Email.Layout __l, B_Person.__ST43253_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE2365128 := JOIN(__EE2365116,__EE2365119,__JC2365127(LEFT,RIGHT),__JT2365127(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE2365398 := B_Email.IDX_Email_UID_Wrapped;
  SHARED __ST2273406_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Email.Typ) Emailof_;
    KEL.typ.ndataset(E_Person_Email.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC2365406(__ST2273092_Layout __EE2365128, B_Email.__ST32271_Layout __EE2365398) := __EEQP(__EE2365128.Emailof_,__EE2365398.UID);
  __ST2273406_Layout __JT2365406(__ST2273092_Layout __l, B_Email.__ST32271_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE2365407 := JOIN(__EE2365128,__EE2365398,__JC2365406(LEFT,RIGHT),__JT2365406(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE2366220 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST2273852_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Email.Typ) Emailof_;
    KEL.typ.ndataset(E_Person_Email.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC2366234(__ST2273406_Layout __EE2365407, E_Customer.Layout __EE2366220) := __EEQP(__EE2365407._r_Customer_,__EE2366220.UID);
  __ST2273852_Layout __JT2366234(__ST2273406_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE2366589 := JOIN(__EE2365407,__EE2366220,__JC2366234(LEFT,RIGHT),__JT2366234(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST2272997_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST2272997_Layout __ND2366935__Project(__ST2273852_Layout __PP2366590) := TRANSFORM
    SELF.Source_Customer_ := __PP2366590._r_Customer_;
    SELF.Customer_Id_ := __PP2366590.Customer_Id__2_;
    SELF.Industry_Type_ := __PP2366590.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP2366590.Entity_Context_Uid__1_;
    SELF := __PP2366590;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE2366589,__ND2366935__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST2272997_Layout));
END;

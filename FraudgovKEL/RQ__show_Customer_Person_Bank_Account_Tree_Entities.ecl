...
EXPORT RQ__show_Customer_Person_Bank_Account_Tree_Entities := MODULE
  SHARED __EE2368006 := PROJECT(B_Person_Bank_Account.IDX_Person_Bank_Account_Account__Wrapped,E_Person_Bank_Account.Layout);
  SHARED __EE2368009 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST2274348_Layout := RECORD
  ...
  END;
  __JC2368017(E_Person_Bank_Account.Layout __EE2368006, B_Person.__ST43253_Layout __EE2368009) := __EEQP(__EE2368006.Subject_,__EE2368009.UID);
  __ST2274348_Layout __JT2368017(E_Person_Bank_Account.Layout __l, B_Person.__ST43253_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE2368018 := JOIN(__EE2368006,__EE2368009,__JC2368017(LEFT,RIGHT),__JT2368017(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE2368288 := B_Bank_Account.IDX_Bank_Account_UID_Wrapped;
  SHARED __ST2274662_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Bank_Account.Typ) Account_;
    KEL.typ.ndataset(E_Person_Bank_Account.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC2368296(__ST2274348_Layout __EE2368018, B_Bank_Account.__ST31055_Layout __EE2368288) := __EEQP(__EE2368018.Account_,__EE2368288.UID);
  __ST2274662_Layout __JT2368296(__ST2274348_Layout __l, B_Bank_Account.__ST31055_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE2368297 := JOIN(__EE2368018,__EE2368288,__JC2368296(LEFT,RIGHT),__JT2368296(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE2369100 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST2275102_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Bank_Account.Typ) Account_;
    KEL.typ.ndataset(E_Person_Bank_Account.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC2369114(__ST2274662_Layout __EE2368297, E_Customer.Layout __EE2369100) := __EEQP(__EE2368297._r_Customer_,__EE2369100.UID);
  __ST2275102_Layout __JT2369114(__ST2274662_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE2369464 := JOIN(__EE2368297,__EE2369100,__JC2369114(LEFT,RIGHT),__JT2369114(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST2274253_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST2274253_Layout __ND2369805__Project(__ST2275102_Layout __PP2369465) := TRANSFORM
    SELF.Source_Customer_ := __PP2369465._r_Customer_;
    SELF.Customer_Id_ := __PP2369465.Customer_Id__2_;
    SELF.Industry_Type_ := __PP2369465.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP2369465.Entity_Context_Uid__1_;
    SELF := __PP2369465;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE2369464,__ND2369805__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST2274253_Layout));
END;

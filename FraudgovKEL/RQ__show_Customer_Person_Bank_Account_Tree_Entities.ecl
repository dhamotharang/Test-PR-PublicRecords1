...
EXPORT RQ__show_Customer_Person_Bank_Account_Tree_Entities := MODULE
  SHARED __EE4755426 := PROJECT(B_Person_Bank_Account.IDX_Person_Bank_Account_Account__Wrapped,E_Person_Bank_Account.Layout);
  SHARED __EE4755429 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST4667554_Layout := RECORD
  ...
  END;
  __JC4755437(E_Person_Bank_Account.Layout __EE4755426, B_Person.__ST69802_Layout __EE4755429) := __EEQP(__EE4755426.Subject_,__EE4755429.UID);
  __ST4667554_Layout __JT4755437(E_Person_Bank_Account.Layout __l, B_Person.__ST69802_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE4755438 := JOIN(__EE4755426,__EE4755429,__JC4755437(LEFT,RIGHT),__JT4755437(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4755667 := B_Bank_Account.IDX_Bank_Account_UID_Wrapped;
  SHARED __ST4667829_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Bank_Account.Typ) Account_;
    KEL.typ.ndataset(E_Person_Bank_Account.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4755675(__ST4667554_Layout __EE4755438, B_Bank_Account.__ST48341_Layout __EE4755667) := __EEQP(__EE4755438.Account_,__EE4755667.UID);
  __ST4667829_Layout __JT4755675(__ST4667554_Layout __l, B_Bank_Account.__ST48341_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4755676 := JOIN(__EE4755438,__EE4755667,__JC4755675(LEFT,RIGHT),__JT4755675(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4756409 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST4668233_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Bank_Account.Typ) Account_;
    KEL.typ.ndataset(E_Person_Bank_Account.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4756424(__ST4667829_Layout __EE4755676, E_Customer.Layout __EE4756409) := __EEQP(__EE4755676._r_Customer_,__EE4756409.UID);
  __ST4668233_Layout __JT4756424(__ST4667829_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4756739 := JOIN(__EE4755676,__EE4756409,__JC4756424(LEFT,RIGHT),__JT4756424(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST4667459_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST4667459_Layout __ND4757045__Project(__ST4668233_Layout __PP4756740) := TRANSFORM
    SELF.Source_Customer_ := __PP4756740._r_Customer_;
    SELF.Customer_Id_ := __PP4756740.Customer_Id__2_;
    SELF.Industry_Type_ := __PP4756740.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP4756740.Entity_Context_Uid__1_;
    SELF := __PP4756740;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE4756739,__ND4757045__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST4667459_Layout));
END;

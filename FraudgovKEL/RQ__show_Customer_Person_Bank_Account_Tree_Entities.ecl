...
EXPORT RQ__show_Customer_Person_Bank_Account_Tree_Entities := MODULE
  SHARED __EE2691095 := PROJECT(B_Person_Bank_Account.IDX_Person_Bank_Account_Account__Wrapped,E_Person_Bank_Account.Layout);
  SHARED __EE2691098 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST2596670_Layout := RECORD
  ...
  END;
  __JC2691106(E_Person_Bank_Account.Layout __EE2691095, B_Person.__ST45521_Layout __EE2691098) := __EEQP(__EE2691095.Subject_,__EE2691098.UID);
  __ST2596670_Layout __JT2691106(E_Person_Bank_Account.Layout __l, B_Person.__ST45521_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE2691107 := JOIN(__EE2691095,__EE2691098,__JC2691106(LEFT,RIGHT),__JT2691106(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE2691377 := B_Bank_Account.IDX_Bank_Account_UID_Wrapped;
  SHARED __ST2596984_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Bank_Account.Typ) Account_;
    KEL.typ.ndataset(E_Person_Bank_Account.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC2691385(__ST2596670_Layout __EE2691107, B_Bank_Account.__ST32562_Layout __EE2691377) := __EEQP(__EE2691107.Account_,__EE2691377.UID);
  __ST2596984_Layout __JT2691385(__ST2596670_Layout __l, B_Bank_Account.__ST32562_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE2691386 := JOIN(__EE2691107,__EE2691377,__JC2691385(LEFT,RIGHT),__JT2691385(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE2692189 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST2597424_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Bank_Account.Typ) Account_;
    KEL.typ.ndataset(E_Person_Bank_Account.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC2692203(__ST2596984_Layout __EE2691386, E_Customer.Layout __EE2692189) := __EEQP(__EE2691386._r_Customer_,__EE2692189.UID);
  __ST2597424_Layout __JT2692203(__ST2596984_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE2692553 := JOIN(__EE2691386,__EE2692189,__JC2692203(LEFT,RIGHT),__JT2692203(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST2596575_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST2596575_Layout __ND2692894__Project(__ST2597424_Layout __PP2692554) := TRANSFORM
    SELF.Source_Customer_ := __PP2692554._r_Customer_;
    SELF.Customer_Id_ := __PP2692554.Customer_Id__2_;
    SELF.Industry_Type_ := __PP2692554.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP2692554.Entity_Context_Uid__1_;
    SELF := __PP2692554;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE2692553,__ND2692894__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST2596575_Layout));
END;

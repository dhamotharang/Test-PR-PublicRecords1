...
EXPORT B_Drivers_License_2 := MODULE
  ...
  SHARED __EE208907 := __ENH_Drivers_License_3;
  SHARED __EE208965 := __E_Drivers_License_Event;
  SHARED __EE210392 := __EE208965(__NN(__EE208965.Transaction_) AND __NN(__EE208965.Licence_));
  SHARED __EE208967 := __ENH_Event_3;
  SHARED __ST209130_Layout := RECORD
  ...
  END;
  __JC210404(E_Drivers_License_Event.Layout __EE210392, B_Event_3.__ST40418_Layout __EE208967) := __EEQP(__EE210392.Transaction_,__EE208967.UID);
  __ST209130_Layout __JT210404(E_Drivers_License_Event.Layout __l, B_Event_3.__ST40418_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF.Event_Date__1_ := __r.Event_Date_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE210405 := JOIN(__EE210392,__EE208967,__JC210404(LEFT,RIGHT),__JT210404(LEFT,RIGHT),INNER,HASH);
  SHARED __EE208921 := __EE208907;
  __JC210631(__ST209130_Layout __EE210405, B_Drivers_License_4.__ST41352_Layout __EE208921) := __EEQP(__EE208921.UID,__EE210405.Licence_) AND __T(__AND(__EEQ(__EE208921.UID,__EE210405.Licence_),__OP2(__EE210405.Event_Date__1_,>,__EE208921.Kr_Last_Event_Date_)));
  SHARED __EE210632 := JOIN(__EE210405,__EE208921,__JC210631(LEFT,RIGHT),TRANSFORM(__ST209130_Layout,SELF:=LEFT),HASH,KEEP(1));
  SHARED __ST209007_Layout := RECORD
    ...
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __EE210864 := PROJECT(__EE210632,TRANSFORM(__ST209007_Layout,SELF.UID := LEFT.Licence_,SELF := LEFT));
  SHARED __ST209031_Layout := RECORD
    KEL.typ.int C_O_U_N_T___Drivers_License_Event_ := 0;
    KEL.typ.ntyp(E_Drivers_License.Typ) UID;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __EE210877 := PROJECT(__CLEANANDDO(__EE210864,TABLE(__EE210864,{KEL.typ.int C_O_U_N_T___Drivers_License_Event_ := COUNT(GROUP),UID},UID,MERGE)),__ST209031_Layout);
  SHARED __ST209636_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC210883(B_Drivers_License_4.__ST41352_Layout __EE208907, __ST209031_Layout __EE210877) := __EEQP(__EE208907.UID,__EE210877.UID);
  __ST209636_Layout __JT210883(B_Drivers_License_4.__ST41352_Layout __l, __ST209031_Layout __r) := TRANSFORM
    SELF.U_I_D__1_ := __r.UID;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE210884 := JOIN(__EE208907,__EE210877,__JC210883(LEFT,RIGHT),__JT210883(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST37763_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ENH_Drivers_License_2 := PROJECT(__EE210884,TRANSFORM(__ST37763_Layout,SELF.Kr_Event_After_Last_Known_Risk_Count_ := LEFT.C_O_U_N_T___Drivers_License_Event_,SELF := LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Drivers_License::Annotated_2',EXPIRE(7));
END;

...
EXPORT Q__show_Customer_Ip_Address_Entities := MODULE
  ...
  SHARED __EE4345283 := __ENH_Internet_Protocol;
  SHARED __EE4345231 := __E_Customer;
  SHARED __ST4345308_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  __JC4345555(B_Internet_Protocol.__ST68443_Layout __EE4345283, E_Customer.Layout __EE4345231) := __EEQP(__EE4345283._r_Customer_,__EE4345231.UID);
  __ST4345308_Layout __JT4345555(B_Internet_Protocol.__ST68443_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4345556 := JOIN(__EE4345283,__EE4345231,__JC4345555(LEFT,RIGHT),__JT4345555(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST46341_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST46341_Layout __ND4345537__Project(__ST4345308_Layout __PP4345536) := TRANSFORM
    SELF.Source_Customer_ := __PP4345536._r_Customer_;
    SELF.Customer_Id_ := __PP4345536.Customer_Id__1_;
    SELF.Industry_Type_ := __PP4345536.Industry_Type__1_;
    SELF.Person_Count_ := __PP4345536.Identity_Count_;
    SELF := __PP4345536;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE4345556,__ND4345537__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Entity_Context_Uid_,Label_,Entity_Type_,Person_Count_,In_Customer_Population_,Contributor_Safe_Flag_,Safe_Flag_,Dt_First_Seen_,Dt_Last_Seen_,Kr_High_Risk_Flag_,Kr_Medium_Risk_Flag_,Kr_Low_Risk_Flag_},Source_Customer_,Customer_Id_,Industry_Type_,Entity_Context_Uid_,Label_,Entity_Type_,Person_Count_,In_Customer_Population_,Contributor_Safe_Flag_,Safe_Flag_,Dt_First_Seen_,Dt_Last_Seen_,Kr_High_Risk_Flag_,Kr_Medium_Risk_Flag_,Kr_Low_Risk_Flag_,MERGE),__ST46341_Layout));
END;

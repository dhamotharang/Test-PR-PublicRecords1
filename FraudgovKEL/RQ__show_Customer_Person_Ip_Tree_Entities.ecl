...
EXPORT RQ__show_Customer_Person_Ip_Tree_Entities := MODULE
  SHARED __EE4922269 := PROJECT(B_Person_Ip_Address.IDX_Person_Ip_Address_Ip__Wrapped,E_Person_Ip_Address.Layout);
  SHARED __EE4922272 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST4822921_Layout := RECORD
  ...
  END;
  __JC4922280(E_Person_Ip_Address.Layout __EE4922269, B_Person.__ST71326_Layout __EE4922272) := __EEQP(__EE4922269.Subject_,__EE4922272.UID);
  __ST4822921_Layout __JT4922280(E_Person_Ip_Address.Layout __l, B_Person.__ST71326_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE4922281 := JOIN(__EE4922269,__EE4922272,__JC4922280(LEFT,RIGHT),__JT4922280(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4922576 := B_Internet_Protocol.IDX_Internet_Protocol_UID_Wrapped;
  SHARED __ST4823262_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Internet_Protocol.Typ) Ip_;
    KEL.typ.ndataset(E_Person_Ip_Address.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4922584(__ST4822921_Layout __EE4922281, B_Internet_Protocol.__ST69217_Layout __EE4922576) := __EEQP(__EE4922281.Ip_,__EE4922576.UID);
  __ST4823262_Layout __JT4922584(__ST4822921_Layout __l, B_Internet_Protocol.__ST69217_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4922585 := JOIN(__EE4922281,__EE4922576,__JC4922584(LEFT,RIGHT),__JT4922584(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4923666 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST4823859_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Internet_Protocol.Typ) Ip_;
    KEL.typ.ndataset(E_Person_Ip_Address.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4923680(__ST4823262_Layout __EE4922585, E_Customer.Layout __EE4923666) := __EEQP(__EE4922585._r_Customer_,__EE4923666.UID);
  __ST4823859_Layout __JT4923680(__ST4823262_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4924153 := JOIN(__EE4922585,__EE4923666,__JC4923680(LEFT,RIGHT),__JT4923680(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST4822826_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST4822826_Layout __ND4924617__Project(__ST4823859_Layout __PP4924154) := TRANSFORM
    SELF.Source_Customer_ := __PP4924154._r_Customer_;
    SELF.Customer_Id_ := __PP4924154.Customer_Id__2_;
    SELF.Industry_Type_ := __PP4924154.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP4924154.Entity_Context_Uid__1_;
    SELF := __PP4924154;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE4924153,__ND4924617__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST4822826_Layout));
END;

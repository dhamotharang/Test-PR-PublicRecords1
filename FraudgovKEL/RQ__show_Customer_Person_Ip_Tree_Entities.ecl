...
EXPORT RQ__show_Customer_Person_Ip_Tree_Entities := MODULE
  SHARED __EE1372987 := PROJECT(B_Person_Ip_Address.IDX_Person_Ip_Address_Ip__Wrapped,E_Person_Ip_Address.Layout);
  SHARED __EE1372990 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST1292131_Layout := RECORD
  ...
  END;
  __JC1372998(E_Person_Ip_Address.Layout __EE1372987, B_Person.__ST28756_Layout __EE1372990) := __EEQP(__EE1372987.Subject_,__EE1372990.UID);
  __ST1292131_Layout __JT1372998(E_Person_Ip_Address.Layout __l, B_Person.__ST28756_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE1372999 := JOIN(__EE1372987,__EE1372990,__JC1372998(LEFT,RIGHT),__JT1372998(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE1373239 := B_Internet_Protocol.IDX_Internet_Protocol_UID_Wrapped;
  SHARED __ST1292415_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Internet_Protocol.Typ) Ip_;
    KEL.typ.ndataset(E_Person_Ip_Address.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC1373247(__ST1292131_Layout __EE1372999, B_Internet_Protocol.__ST27230_Layout __EE1373239) := __EEQP(__EE1372999.Ip_,__EE1373239.UID);
  __ST1292415_Layout __JT1373247(__ST1292131_Layout __l, B_Internet_Protocol.__ST27230_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE1373248 := JOIN(__EE1372999,__EE1373239,__JC1373247(LEFT,RIGHT),__JT1373247(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE1374145 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST1292907_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Internet_Protocol.Typ) Ip_;
    KEL.typ.ndataset(E_Person_Ip_Address.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC1374159(__ST1292415_Layout __EE1373248, E_Customer.Layout __EE1374145) := __EEQP(__EE1373248._r_Customer_,__EE1374145.UID);
  __ST1292907_Layout __JT1374159(__ST1292415_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE1374551 := JOIN(__EE1373248,__EE1374145,__JC1374159(LEFT,RIGHT),__JT1374159(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST1292036_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST1292036_Layout __ND1374934__Project(__ST1292907_Layout __PP1374552) := TRANSFORM
    SELF.Source_Customer_ := __PP1374552._r_Customer_;
    SELF.Customer_Id_ := __PP1374552.Customer_Id__2_;
    SELF.Industry_Type_ := __PP1374552.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP1374552.Entity_Context_Uid__1_;
    SELF := __PP1374552;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE1374551,__ND1374934__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST1292036_Layout));
END;

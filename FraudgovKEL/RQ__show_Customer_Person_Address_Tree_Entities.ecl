...
EXPORT RQ__show_Customer_Person_Address_Tree_Entities := MODULE
  SHARED __EE4175527 := PROJECT(B_Person_Address.IDX_Person_Address_Location__Wrapped,E_Person_Address.Layout);
  SHARED __EE4175530 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST4082489_Layout := RECORD
  ...
  END;
  __JC4175538(E_Person_Address.Layout __EE4175527, B_Person.__ST65777_Layout __EE4175530) := __EEQP(__EE4175527.Subject_,__EE4175530.UID);
  __ST4082489_Layout __JT4175538(E_Person_Address.Layout __l, B_Person.__ST65777_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE4175539 := JOIN(__EE4175527,__EE4175530,__JC4175538(LEFT,RIGHT),__JT4175538(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4175829 := B_Address.IDX_Address_UID_Wrapped;
  SHARED __ST4082825_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Address.Typ) Location_;
    KEL.typ.ndataset(E_Person_Address.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4175837(__ST4082489_Layout __EE4175539, B_Address.__ST45405_Layout __EE4175829) := __EEQP(__EE4175539.Location_,__EE4175829.UID);
  __ST4082825_Layout __JT4175837(__ST4082489_Layout __l, B_Address.__ST45405_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4175838 := JOIN(__EE4175539,__EE4175829,__JC4175837(LEFT,RIGHT),__JT4175837(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4176813 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST4083356_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Address.Typ) Location_;
    KEL.typ.ndataset(E_Person_Address.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4176827(__ST4082825_Layout __EE4175838, E_Customer.Layout __EE4176813) := __EEQP(__EE4175838._r_Customer_,__EE4176813.UID);
  __ST4083356_Layout __JT4176827(__ST4082825_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4177260 := JOIN(__EE4175838,__EE4176813,__JC4176827(LEFT,RIGHT),__JT4176827(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST4082394_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST4082394_Layout __ND4177684__Project(__ST4083356_Layout __PP4177261) := TRANSFORM
    SELF.Source_Customer_ := __PP4177261._r_Customer_;
    SELF.Customer_Id_ := __PP4177261.Customer_Id__2_;
    SELF.Industry_Type_ := __PP4177261.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP4177261.Entity_Context_Uid__1_;
    SELF := __PP4177261;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE4177260,__ND4177684__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST4082394_Layout));
END;

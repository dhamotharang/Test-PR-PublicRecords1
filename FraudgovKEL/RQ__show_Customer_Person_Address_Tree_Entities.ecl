...
EXPORT RQ__show_Customer_Person_Address_Tree_Entities := MODULE
  SHARED __EE4231138 := PROJECT(B_Person_Address.IDX_Person_Address_Location__Wrapped,E_Person_Address.Layout);
  SHARED __EE4231141 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST4151330_Layout := RECORD
  ...
  END;
  __JC4231149(E_Person_Address.Layout __EE4231138, B_Person.__ST66581_Layout __EE4231141) := __EEQP(__EE4231138.Subject_,__EE4231141.UID);
  __ST4151330_Layout __JT4231149(E_Person_Address.Layout __l, B_Person.__ST66581_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE4231150 := JOIN(__EE4231138,__EE4231141,__JC4231149(LEFT,RIGHT),__JT4231149(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4231379 := B_Address.IDX_Address_UID_Wrapped;
  SHARED __ST4151605_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Address.Typ) Location_;
    KEL.typ.ndataset(E_Person_Address.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4231387(__ST4151330_Layout __EE4231150, B_Address.__ST45522_Layout __EE4231379) := __EEQP(__EE4231150.Location_,__EE4231379.UID);
  __ST4151605_Layout __JT4231387(__ST4151330_Layout __l, B_Address.__ST45522_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4231388 := JOIN(__EE4231150,__EE4231379,__JC4231387(LEFT,RIGHT),__JT4231387(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4232213 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST4152054_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Address.Typ) Location_;
    KEL.typ.ndataset(E_Person_Address.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4232227(__ST4151605_Layout __EE4231388, E_Customer.Layout __EE4232213) := __EEQP(__EE4231388._r_Customer_,__EE4232213.UID);
  __ST4152054_Layout __JT4232227(__ST4151605_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4232592 := JOIN(__EE4231388,__EE4232213,__JC4232227(LEFT,RIGHT),__JT4232227(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST4151235_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST4151235_Layout __ND4232948__Project(__ST4152054_Layout __PP4232593) := TRANSFORM
    SELF.Source_Customer_ := __PP4232593._r_Customer_;
    SELF.Customer_Id_ := __PP4232593.Customer_Id__2_;
    SELF.Industry_Type_ := __PP4232593.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP4232593.Entity_Context_Uid__1_;
    SELF := __PP4232593;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE4232592,__ND4232948__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST4151235_Layout));
END;

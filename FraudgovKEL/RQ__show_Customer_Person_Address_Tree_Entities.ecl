...
EXPORT RQ__show_Customer_Person_Address_Tree_Entities := MODULE
  SHARED __EE4783937 := PROJECT(B_Person_Address.IDX_Person_Address_Location__Wrapped,E_Person_Address.Layout);
  SHARED __EE4783940 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST4703851_Layout := RECORD
  ...
  END;
  __JC4783948(E_Person_Address.Layout __EE4783937, B_Person.__ST69071_Layout __EE4783940) := __EEQP(__EE4783937.Subject_,__EE4783940.UID);
  __ST4703851_Layout __JT4783948(E_Person_Address.Layout __l, B_Person.__ST69071_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE4783949 := JOIN(__EE4783937,__EE4783940,__JC4783948(LEFT,RIGHT),__JT4783948(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4784178 := B_Address.IDX_Address_UID_Wrapped;
  SHARED __ST4704126_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Address.Typ) Location_;
    KEL.typ.ndataset(E_Person_Address.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4784186(__ST4703851_Layout __EE4783949, B_Address.__ST46977_Layout __EE4784178) := __EEQP(__EE4783949.Location_,__EE4784178.UID);
  __ST4704126_Layout __JT4784186(__ST4703851_Layout __l, B_Address.__ST46977_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4784187 := JOIN(__EE4783949,__EE4784178,__JC4784186(LEFT,RIGHT),__JT4784186(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4785016 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST4704575_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Address.Typ) Location_;
    KEL.typ.ndataset(E_Person_Address.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4785031(__ST4704126_Layout __EE4784187, E_Customer.Layout __EE4785016) := __EEQP(__EE4784187._r_Customer_,__EE4785016.UID);
  __ST4704575_Layout __JT4785031(__ST4704126_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4785397 := JOIN(__EE4784187,__EE4785016,__JC4785031(LEFT,RIGHT),__JT4785031(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST4703756_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST4703756_Layout __ND4785754__Project(__ST4704575_Layout __PP4785398) := TRANSFORM
    SELF.Source_Customer_ := __PP4785398._r_Customer_;
    SELF.Customer_Id_ := __PP4785398.Customer_Id__2_;
    SELF.Industry_Type_ := __PP4785398.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP4785398.Entity_Context_Uid__1_;
    SELF := __PP4785398;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE4785397,__ND4785754__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST4703756_Layout));
END;

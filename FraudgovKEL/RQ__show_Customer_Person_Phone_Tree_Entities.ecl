...
EXPORT RQ__show_Customer_Person_Phone_Tree_Entities := MODULE
  SHARED __EE4792564 := PROJECT(B_Person_Phone.IDX_Person_Phone_Phone_Number__Wrapped,E_Person_Phone.Layout);
  SHARED __EE4792567 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST4707577_Layout := RECORD
  ...
  END;
  __JC4792575(E_Person_Phone.Layout __EE4792564, B_Person.__ST69071_Layout __EE4792567) := __EEQP(__EE4792564.Subject_,__EE4792567.UID);
  __ST4707577_Layout __JT4792575(E_Person_Phone.Layout __l, B_Person.__ST69071_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE4792576 := JOIN(__EE4792564,__EE4792567,__JC4792575(LEFT,RIGHT),__JT4792575(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4792805 := B_Phone.IDX_Phone_UID_Wrapped;
  SHARED __ST4707852_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Phone.Typ) Phone_Number_;
    KEL.typ.ndataset(E_Person_Phone.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4792813(__ST4707577_Layout __EE4792576, B_Phone.__ST69989_Layout __EE4792805) := __EEQP(__EE4792576.Phone_Number_,__EE4792805.UID);
  __ST4707852_Layout __JT4792813(__ST4707577_Layout __l, B_Phone.__ST69989_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4792814 := JOIN(__EE4792576,__EE4792805,__JC4792813(LEFT,RIGHT),__JT4792813(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4793551 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST4708259_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Phone.Typ) Phone_Number_;
    KEL.typ.ndataset(E_Person_Phone.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4793566(__ST4707852_Layout __EE4792814, E_Customer.Layout __EE4793551) := __EEQP(__EE4792814._r_Customer_,__EE4793551.UID);
  __ST4708259_Layout __JT4793566(__ST4707852_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4793882 := JOIN(__EE4792814,__EE4793551,__JC4793566(LEFT,RIGHT),__JT4793566(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST4707482_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST4707482_Layout __ND4794189__Project(__ST4708259_Layout __PP4793883) := TRANSFORM
    SELF.Source_Customer_ := __PP4793883._r_Customer_;
    SELF.Customer_Id_ := __PP4793883.Customer_Id__2_;
    SELF.Industry_Type_ := __PP4793883.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP4793883.Entity_Context_Uid__1_;
    SELF := __PP4793883;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE4793882,__ND4794189__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST4707482_Layout));
END;

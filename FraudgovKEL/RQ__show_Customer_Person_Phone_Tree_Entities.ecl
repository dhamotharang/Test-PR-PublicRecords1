...
EXPORT RQ__show_Customer_Person_Phone_Tree_Entities := MODULE
  SHARED __EE4239738 := PROJECT(B_Person_Phone.IDX_Person_Phone_Phone_Number__Wrapped,E_Person_Phone.Layout);
  SHARED __EE4239741 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST4155053_Layout := RECORD
  ...
  END;
  __JC4239749(E_Person_Phone.Layout __EE4239738, B_Person.__ST66581_Layout __EE4239741) := __EEQP(__EE4239738.Subject_,__EE4239741.UID);
  __ST4155053_Layout __JT4239749(E_Person_Phone.Layout __l, B_Person.__ST66581_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE4239750 := JOIN(__EE4239738,__EE4239741,__JC4239749(LEFT,RIGHT),__JT4239749(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4239979 := B_Phone.IDX_Phone_UID_Wrapped;
  SHARED __ST4155328_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Phone.Typ) Phone_Number_;
    KEL.typ.ndataset(E_Person_Phone.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4239987(__ST4155053_Layout __EE4239750, B_Phone.__ST67529_Layout __EE4239979) := __EEQP(__EE4239750.Phone_Number_,__EE4239979.UID);
  __ST4155328_Layout __JT4239987(__ST4155053_Layout __l, B_Phone.__ST67529_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4239988 := JOIN(__EE4239750,__EE4239979,__JC4239987(LEFT,RIGHT),__JT4239987(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4240721 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST4155735_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Phone.Typ) Phone_Number_;
    KEL.typ.ndataset(E_Person_Phone.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4240735(__ST4155328_Layout __EE4239988, E_Customer.Layout __EE4240721) := __EEQP(__EE4239988._r_Customer_,__EE4240721.UID);
  __ST4155735_Layout __JT4240735(__ST4155328_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4241050 := JOIN(__EE4239988,__EE4240721,__JC4240735(LEFT,RIGHT),__JT4240735(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST4154958_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST4154958_Layout __ND4241356__Project(__ST4155735_Layout __PP4241051) := TRANSFORM
    SELF.Source_Customer_ := __PP4241051._r_Customer_;
    SELF.Customer_Id_ := __PP4241051.Customer_Id__2_;
    SELF.Industry_Type_ := __PP4241051.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP4241051.Entity_Context_Uid__1_;
    SELF := __PP4241051;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE4241050,__ND4241356__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST4154958_Layout));
END;

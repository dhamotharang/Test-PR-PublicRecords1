...
EXPORT RQ__show_Customer_Person_Phone_Tree_Entities := MODULE
  SHARED __EE1376059 := PROJECT(B_Person_Phone.IDX_Person_Phone_Phone_Number__Wrapped,E_Person_Phone.Layout);
  SHARED __EE1376062 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST1293439_Layout := RECORD
  ...
  END;
  __JC1376070(E_Person_Phone.Layout __EE1376059, B_Person.__ST28756_Layout __EE1376062) := __EEQP(__EE1376059.Subject_,__EE1376062.UID);
  __ST1293439_Layout __JT1376070(E_Person_Phone.Layout __l, B_Person.__ST28756_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE1376071 := JOIN(__EE1376059,__EE1376062,__JC1376070(LEFT,RIGHT),__JT1376070(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE1376311 := B_Phone.IDX_Phone_UID_Wrapped;
  SHARED __ST1293723_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Phone.Typ) Phone_Number_;
    KEL.typ.ndataset(E_Person_Phone.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC1376319(__ST1293439_Layout __EE1376071, B_Phone.__ST29469_Layout __EE1376311) := __EEQP(__EE1376071.Phone_Number_,__EE1376311.UID);
  __ST1293723_Layout __JT1376319(__ST1293439_Layout __l, B_Phone.__ST29469_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE1376320 := JOIN(__EE1376071,__EE1376311,__JC1376319(LEFT,RIGHT),__JT1376319(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE1377049 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST1294127_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Phone.Typ) Phone_Number_;
    KEL.typ.ndataset(E_Person_Phone.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC1377063(__ST1293723_Layout __EE1376320, E_Customer.Layout __EE1377049) := __EEQP(__EE1376320._r_Customer_,__EE1377049.UID);
  __ST1294127_Layout __JT1377063(__ST1293723_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE1377375 := JOIN(__EE1376320,__EE1377049,__JC1377063(LEFT,RIGHT),__JT1377063(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST1293344_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST1293344_Layout __ND1377678__Project(__ST1294127_Layout __PP1377376) := TRANSFORM
    SELF.Source_Customer_ := __PP1377376._r_Customer_;
    SELF.Customer_Id_ := __PP1377376.Customer_Id__2_;
    SELF.Industry_Type_ := __PP1377376.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP1377376.Entity_Context_Uid__1_;
    SELF := __PP1377376;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE1377375,__ND1377678__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST1293344_Layout));
END;

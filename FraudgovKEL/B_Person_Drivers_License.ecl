...
EXPORT B_Person_Drivers_License := MODULE
  ...
  SHARED __EE2156142 := __E_Person_Drivers_License;
  EXPORT __ST45448_Layout := RECORD
  ...
  END;
  SHARED __ST45448_Layout __ND2156176__Project(E_Person_Drivers_License.Layout __PP2156037) := TRANSFORM
    __EE2156075 := __PP2156037.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2156075,__T(__EE2156075).Event_Date_);
    __EE2156104 := __PP2156037.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2156104,__T(__EE2156104).Event_Date_);
    SELF := __PP2156037;
  END;
  EXPORT __ENH_Person_Drivers_License := PROJECT(__EE2156142,__ND2156176__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Drivers_License::Annotated',EXPIRE(7));
  SHARED __EE2406468 := __ENH_Person_Drivers_License;
  SHARED __IDX_Person_Drivers_License_License__Filtered := __EE2406468(__NN(__EE2406468.License_));
  ...
  EXPORT __ST2406470_Layout := RECORDOF(IDX_Person_Drivers_License_License_);
  EXPORT IDX_Person_Drivers_License_License__Wrapped := PROJECT(IDX_Person_Drivers_License_License_,TRANSFORM(__ST45448_Layout,SELF.License_ := __CN(LEFT.License_),SELF:=LEFT));

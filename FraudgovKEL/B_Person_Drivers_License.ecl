...
EXPORT B_Person_Drivers_License := MODULE
  ...
  SHARED __EE3893933 := __E_Person_Drivers_License;
  EXPORT __ST66837_Layout := RECORD
  ...
  END;
  SHARED __ST66837_Layout __ND3893967__Project(E_Person_Drivers_License.Layout __PP3893828) := TRANSFORM
    __EE3893866 := __PP3893828.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE3893866,__T(__EE3893866).Event_Date_);
    __EE3893895 := __PP3893828.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE3893895,__T(__EE3893895).Event_Date_);
    SELF := __PP3893828;
  END;
  EXPORT __ENH_Person_Drivers_License := PROJECT(__EE3893933,__ND3893967__Project(LEFT)) : PERSIST('~fraudgov::temp::KEL::FraudgovKEL::Person_Drivers_License::Annotated',EXPIRE(7));
  SHARED __EE4159973 := __ENH_Person_Drivers_License;
  SHARED __IDX_Person_Drivers_License_License__Filtered := __EE4159973(__NN(__EE4159973.License_));
  ...
  EXPORT __ST4159975_Layout := RECORDOF(IDX_Person_Drivers_License_License_);
  EXPORT IDX_Person_Drivers_License_License__Wrapped := PROJECT(IDX_Person_Drivers_License_License_,TRANSFORM(__ST66837_Layout,SELF.License_ := __CN(LEFT.License_),SELF:=LEFT));

...
EXPORT B_Person_Drivers_License := MODULE
  ...
  SHARED __EE2537401 := __E_Person_Drivers_License;
  EXPORT __ST53126_Layout := RECORD
  ...
  END;
  SHARED __ST53126_Layout __ND2537435__Project(E_Person_Drivers_License.Layout __PP2537296) := TRANSFORM
    __EE2537334 := __PP2537296.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2537334,__T(__EE2537334).Event_Date_);
    __EE2537363 := __PP2537296.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2537363,__T(__EE2537363).Event_Date_);
    SELF := __PP2537296;
  END;
  EXPORT __ENH_Person_Drivers_License := PROJECT(__EE2537401,__ND2537435__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Drivers_License::Annotated',EXPIRE(7));
  SHARED __EE2805182 := __ENH_Person_Drivers_License;
  SHARED __IDX_Person_Drivers_License_License__Filtered := __EE2805182(__NN(__EE2805182.License_));
  ...
  EXPORT __ST2805184_Layout := RECORDOF(IDX_Person_Drivers_License_License_);
  EXPORT IDX_Person_Drivers_License_License__Wrapped := PROJECT(IDX_Person_Drivers_License_License_,TRANSFORM(__ST53126_Layout,SELF.License_ := __CN(LEFT.License_),SELF:=LEFT));

...
EXPORT B_Person_Drivers_License := MODULE
  ...
  SHARED __EE1076985 := __E_Person_Drivers_License;
  EXPORT __ST29026_Layout := RECORD
  ...
  END;
  SHARED __ST29026_Layout __ND1077019__Project(E_Person_Drivers_License.Layout __PP1076880) := TRANSFORM
    __EE1076918 := __PP1076880.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1076918,__T(__EE1076918).Event_Date_);
    __EE1076947 := __PP1076880.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1076947,__T(__EE1076947).Event_Date_);
    SELF := __PP1076880;
  END;
  EXPORT __ENH_Person_Drivers_License := PROJECT(__EE1076985,__ND1077019__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Drivers_License::Annotated',EXPIRE(7));
  SHARED __EE1297982 := __ENH_Person_Drivers_License;
  SHARED __IDX_Person_Drivers_License_License__Filtered := __EE1297982(__NN(__EE1297982.License_));
  ...
  EXPORT __ST1297984_Layout := RECORDOF(IDX_Person_Drivers_License_License_);
  EXPORT IDX_Person_Drivers_License_License__Wrapped := PROJECT(IDX_Person_Drivers_License_License_,TRANSFORM(__ST29026_Layout,SELF.License_ := __CN(LEFT.License_),SELF:=LEFT));

...
EXPORT B_Person_Drivers_License := MODULE
  ...
  SHARED __EE1089638 := __E_Person_Drivers_License;
  EXPORT __ST30594_Layout := RECORD
  ...
  END;
  SHARED __ST30594_Layout __ND1089672__Project(E_Person_Drivers_License.Layout __PP1089533) := TRANSFORM
    __EE1089571 := __PP1089533.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1089571,__T(__EE1089571).Event_Date_);
    __EE1089600 := __PP1089533.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1089600,__T(__EE1089600).Event_Date_);
    SELF := __PP1089533;
  END;
  EXPORT __ENH_Person_Drivers_License := PROJECT(__EE1089638,__ND1089672__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Drivers_License::Annotated',EXPIRE(7));
  SHARED __EE1360926 := __ENH_Person_Drivers_License;
  SHARED __IDX_Person_Drivers_License_License__Filtered := __EE1360926(__NN(__EE1360926.License_));
  ...
  EXPORT __ST1360928_Layout := RECORDOF(IDX_Person_Drivers_License_License_);
  EXPORT IDX_Person_Drivers_License_License__Wrapped := PROJECT(IDX_Person_Drivers_License_License_,TRANSFORM(__ST30594_Layout,SELF.License_ := __CN(LEFT.License_),SELF:=LEFT));

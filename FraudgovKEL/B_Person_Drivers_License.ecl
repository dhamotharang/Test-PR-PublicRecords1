...
EXPORT B_Person_Drivers_License := MODULE
  ...
  SHARED __EE4006425 := __E_Person_Drivers_License;
  EXPORT __ST67088_Layout := RECORD
  ...
  END;
  SHARED __ST67088_Layout __ND4006459__Project(E_Person_Drivers_License.Layout __PP4006320) := TRANSFORM
    __EE4006358 := __PP4006320.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE4006358,__T(__EE4006358).Event_Date_);
    __EE4006387 := __PP4006320.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE4006387,__T(__EE4006387).Event_Date_);
    SELF := __PP4006320;
  END;
  EXPORT __ENH_Person_Drivers_License := PROJECT(__EE4006425,__ND4006459__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Drivers_License::Annotated',EXPIRE(7));
  SHARED __EE4316690 := __ENH_Person_Drivers_License;
  SHARED __IDX_Person_Drivers_License_License__Filtered := __EE4316690(__NN(__EE4316690.License_));
  ...
  EXPORT __ST4316692_Layout := RECORDOF(IDX_Person_Drivers_License_License_);
  EXPORT IDX_Person_Drivers_License_License__Wrapped := PROJECT(IDX_Person_Drivers_License_License_,TRANSFORM(__ST67088_Layout,SELF.License_ := __CN(LEFT.License_),SELF:=LEFT));

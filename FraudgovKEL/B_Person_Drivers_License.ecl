...
EXPORT B_Person_Drivers_License := MODULE
  ...
  SHARED __EE2340892 := __E_Person_Drivers_License;
  EXPORT __ST45821_Layout := RECORD
  ...
  END;
  SHARED __ST45821_Layout __ND2340926__Project(E_Person_Drivers_License.Layout __PP2340787) := TRANSFORM
    __EE2340825 := __PP2340787.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2340825,__T(__EE2340825).Event_Date_);
    __EE2340854 := __PP2340787.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2340854,__T(__EE2340854).Event_Date_);
    SELF := __PP2340787;
  END;
  EXPORT __ENH_Person_Drivers_License := PROJECT(__EE2340892,__ND2340926__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Drivers_License::Annotated',EXPIRE(7));
  SHARED __EE2599570 := __ENH_Person_Drivers_License;
  SHARED __IDX_Person_Drivers_License_License__Filtered := __EE2599570(__NN(__EE2599570.License_));
  ...
  EXPORT __ST2599572_Layout := RECORDOF(IDX_Person_Drivers_License_License_);
  EXPORT IDX_Person_Drivers_License_License__Wrapped := PROJECT(IDX_Person_Drivers_License_License_,TRANSFORM(__ST45821_Layout,SELF.License_ := __CN(LEFT.License_),SELF:=LEFT));

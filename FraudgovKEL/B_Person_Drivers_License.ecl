...
EXPORT B_Person_Drivers_License := MODULE
  ...
  SHARED __EE3803499 := __E_Person_Drivers_License;
  EXPORT __ST66094_Layout := RECORD
  ...
  END;
  SHARED __ST66094_Layout __ND3803533__Project(E_Person_Drivers_License.Layout __PP3803394) := TRANSFORM
    __EE3803432 := __PP3803394.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE3803432,__T(__EE3803432).Event_Date_);
    __EE3803461 := __PP3803394.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE3803461,__T(__EE3803461).Event_Date_);
    SELF := __PP3803394;
  END;
  EXPORT __ENH_Person_Drivers_License := PROJECT(__EE3803499,__ND3803533__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Drivers_License::Annotated',EXPIRE(7));
  SHARED __EE4094195 := __ENH_Person_Drivers_License;
  SHARED __IDX_Person_Drivers_License_License__Filtered := __EE4094195(__NN(__EE4094195.License_));
  ...
  EXPORT __ST4094197_Layout := RECORDOF(IDX_Person_Drivers_License_License_);
  EXPORT IDX_Person_Drivers_License_License__Wrapped := PROJECT(IDX_Person_Drivers_License_License_,TRANSFORM(__ST66094_Layout,SELF.License_ := __CN(LEFT.License_),SELF:=LEFT));

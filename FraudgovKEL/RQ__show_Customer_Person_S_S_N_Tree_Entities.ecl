...
EXPORT RQ__show_Customer_Person_S_S_N_Tree_Entities := MODULE
  SHARED __EE4489586 := PROJECT(B_Person_S_S_N.IDX_Person_S_S_N_Social__Wrapped,E_Person_S_S_N.Layout);
  SHARED __EE4489589 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST4407872_Layout := RECORD
  ...
  END;
  __JC4489597(E_Person_S_S_N.Layout __EE4489586, B_Person.__ST69802_Layout __EE4489589) := __EEQP(__EE4489586.Subject_,__EE4489589.UID);
  __ST4407872_Layout __JT4489597(E_Person_S_S_N.Layout __l, B_Person.__ST69802_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE4489598 := JOIN(__EE4489586,__EE4489589,__JC4489597(LEFT,RIGHT),__JT4489597(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4489827 := B_Social_Security_Number.IDX_Social_Security_Number_UID_Wrapped;
  SHARED __ST4408147_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Social_Security_Number.Typ) Social_;
    KEL.typ.ndataset(E_Person_S_S_N.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4489835(__ST4407872_Layout __EE4489598, B_Social_Security_Number.__ST71402_Layout __EE4489827) := __EEQP(__EE4489598.Social_,__EE4489827.UID);
  __ST4408147_Layout __JT4489835(__ST4407872_Layout __l, B_Social_Security_Number.__ST71402_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4489836 := JOIN(__EE4489598,__EE4489827,__JC4489835(LEFT,RIGHT),__JT4489835(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE4490619 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST4408588_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Social_Security_Number.Typ) Social_;
    KEL.typ.ndataset(E_Person_S_S_N.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4490634(__ST4408147_Layout __EE4489836, E_Customer.Layout __EE4490619) := __EEQP(__EE4489836._r_Customer_,__EE4490619.UID);
  __ST4408588_Layout __JT4490634(__ST4408147_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4490962 := JOIN(__EE4489836,__EE4490619,__JC4490634(LEFT,RIGHT),__JT4490634(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST4407777_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST4407777_Layout __ND4491281__Project(__ST4408588_Layout __PP4490963) := TRANSFORM
    SELF.Source_Customer_ := __PP4490963._r_Customer_;
    SELF.Customer_Id_ := __PP4490963.Customer_Id__2_;
    SELF.Industry_Type_ := __PP4490963.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP4490963.Entity_Context_Uid__1_;
    SELF := __PP4490963;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE4490962,__ND4491281__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST4407777_Layout));
END;

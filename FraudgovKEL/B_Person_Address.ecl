...
EXPORT B_Person_Address := MODULE
  ...
  SHARED __EE3803135 := __E_Person_Address;
  EXPORT __ST66048_Layout := RECORD
  ...
  END;
  SHARED __ST66048_Layout __ND3803169__Project(E_Person_Address.Layout __PP3803030) := TRANSFORM
    __EE3803068 := __PP3803030.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE3803068,__T(__EE3803068).Event_Date_);
    __EE3803097 := __PP3803030.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE3803097,__T(__EE3803097).Event_Date_);
    SELF := __PP3803030;
  END;
  EXPORT __ENH_Person_Address := PROJECT(__EE3803135,__ND3803169__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Address::Annotated',EXPIRE(7));
  SHARED __EE4093885 := __ENH_Person_Address;
  SHARED __IDX_Person_Address_Location__Filtered := __EE4093885(__NN(__EE4093885.Location_));
  ...
  EXPORT __ST4093887_Layout := RECORDOF(IDX_Person_Address_Location_);
  EXPORT IDX_Person_Address_Location__Wrapped := PROJECT(IDX_Person_Address_Location_,TRANSFORM(__ST66048_Layout,SELF.Location_ := __CN(LEFT.Location_),SELF:=LEFT));

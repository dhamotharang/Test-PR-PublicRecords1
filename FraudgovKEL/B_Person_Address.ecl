...
EXPORT B_Person_Address := MODULE
  ...
  SHARED __EE2087864 := __E_Person_Address;
  EXPORT __ST45331_Layout := RECORD
  ...
  END;
  SHARED __ST45331_Layout __ND2087898__Project(E_Person_Address.Layout __PP2087759) := TRANSFORM
    __EE2087797 := __PP2087759.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2087797,__T(__EE2087797).Event_Date_);
    __EE2087826 := __PP2087759.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2087826,__T(__EE2087826).Event_Date_);
    SELF := __PP2087759;
  END;
  EXPORT __ENH_Person_Address := PROJECT(__EE2087864,__ND2087898__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Address::Annotated',EXPIRE(7));
  SHARED __EE2334636 := __ENH_Person_Address;
  SHARED __IDX_Person_Address_Location__Filtered := __EE2334636(__NN(__EE2334636.Location_));
  ...
  EXPORT __ST2334638_Layout := RECORDOF(IDX_Person_Address_Location_);
  EXPORT IDX_Person_Address_Location__Wrapped := PROJECT(IDX_Person_Address_Location_,TRANSFORM(__ST45331_Layout,SELF.Location_ := __CN(LEFT.Location_),SELF:=LEFT));

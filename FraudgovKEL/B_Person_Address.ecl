...
EXPORT B_Person_Address := MODULE
  ...
  SHARED __EE2340528 := __E_Person_Address;
  EXPORT __ST45775_Layout := RECORD
  ...
  END;
  SHARED __ST45775_Layout __ND2340562__Project(E_Person_Address.Layout __PP2340423) := TRANSFORM
    __EE2340461 := __PP2340423.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2340461,__T(__EE2340461).Event_Date_);
    __EE2340490 := __PP2340423.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2340490,__T(__EE2340490).Event_Date_);
    SELF := __PP2340423;
  END;
  EXPORT __ENH_Person_Address := PROJECT(__EE2340528,__ND2340562__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Address::Annotated',EXPIRE(7));
  SHARED __EE2599059 := __ENH_Person_Address;
  SHARED __IDX_Person_Address_Location__Filtered := __EE2599059(__NN(__EE2599059.Location_));
  ...
  EXPORT __ST2599061_Layout := RECORDOF(IDX_Person_Address_Location_);
  EXPORT IDX_Person_Address_Location__Wrapped := PROJECT(IDX_Person_Address_Location_,TRANSFORM(__ST45775_Layout,SELF.Location_ := __CN(LEFT.Location_),SELF:=LEFT));

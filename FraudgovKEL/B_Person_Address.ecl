...
EXPORT B_Person_Address := MODULE
  ...
  SHARED __EE3143860 := __E_Person_Address;
  EXPORT __ST63358_Layout := RECORD
  ...
  END;
  SHARED __ST63358_Layout __ND3143894__Project(E_Person_Address.Layout __PP3143755) := TRANSFORM
    __EE3143793 := __PP3143755.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE3143793,__T(__EE3143793).Event_Date_);
    __EE3143822 := __PP3143755.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE3143822,__T(__EE3143822).Event_Date_);
    SELF := __PP3143755;
  END;
  EXPORT __ENH_Person_Address := PROJECT(__EE3143860,__ND3143894__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Address::Annotated',EXPIRE(7));
  SHARED __EE3429478 := __ENH_Person_Address;
  SHARED __IDX_Person_Address_Location__Filtered := __EE3429478(__NN(__EE3429478.Location_));
  ...
  EXPORT __ST3429480_Layout := RECORDOF(IDX_Person_Address_Location_);
  EXPORT IDX_Person_Address_Location__Wrapped := PROJECT(IDX_Person_Address_Location_,TRANSFORM(__ST63358_Layout,SELF.Location_ := __CN(LEFT.Location_),SELF:=LEFT));

...
EXPORT B_Person_Address := MODULE
  ...
  SHARED __EE1063179 := __E_Person_Address;
  EXPORT __ST28736_Layout := RECORD
  ...
  END;
  SHARED __ST28736_Layout __ND1063213__Project(E_Person_Address.Layout __PP1063074) := TRANSFORM
    __EE1063112 := __PP1063074.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1063112,__T(__EE1063112).Event_Date_);
    __EE1063141 := __PP1063074.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1063141,__T(__EE1063141).Event_Date_);
    SELF := __PP1063074;
  END;
  EXPORT __ENH_Person_Address := PROJECT(__EE1063179,__ND1063213__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Address::Annotated',EXPIRE(7));
  SHARED __EE1283085 := __ENH_Person_Address;
  SHARED __IDX_Person_Address_Location__Filtered := __EE1283085(__NN(__EE1283085.Location_));
  ...
  EXPORT __ST1283087_Layout := RECORDOF(IDX_Person_Address_Location_);
  EXPORT IDX_Person_Address_Location__Wrapped := PROJECT(IDX_Person_Address_Location_,TRANSFORM(__ST28736_Layout,SELF.Location_ := __CN(LEFT.Location_),SELF:=LEFT));

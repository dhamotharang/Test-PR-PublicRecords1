...
EXPORT B_Person_Address := MODULE
  ...
  SHARED __EE4004041 := __E_Person_Address;
  EXPORT __ST66982_Layout := RECORD
  ...
  END;
  SHARED __ST66982_Layout __ND4004075__Project(E_Person_Address.Layout __PP4003936) := TRANSFORM
    __EE4003974 := __PP4003936.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE4003974,__T(__EE4003974).Event_Date_);
    __EE4004003 := __PP4003936.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE4004003,__T(__EE4004003).Event_Date_);
    SELF := __PP4003936;
  END;
  EXPORT __ENH_Person_Address := PROJECT(__EE4004041,__ND4004075__Project(LEFT)) : PERSIST('~fraudgov::temp::KEL::FraudgovKEL::Person_Address::Annotated',EXPIRE(7));
  ...
  EXPORT __ST4314193_Layout := RECORDOF(IDX_Person_Address_Location_);
  EXPORT IDX_Person_Address_Location__Wrapped := PROJECT(IDX_Person_Address_Location_,TRANSFORM(__ST66982_Layout,SELF.Location_ := __CN(LEFT.Location_),SELF:=LEFT));

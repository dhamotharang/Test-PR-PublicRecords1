...
EXPORT B_Person_Address := MODULE
  ...
  SHARED __EE1127284 := __E_Person_Address;
  EXPORT __ST31115_Layout := RECORD
  ...
  END;
  SHARED __ST31115_Layout __ND1127318__Project(E_Person_Address.Layout __PP1127179) := TRANSFORM
    __EE1127217 := __PP1127179.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1127217,__T(__EE1127217).Event_Date_);
    __EE1127246 := __PP1127179.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1127246,__T(__EE1127246).Event_Date_);
    SELF := __PP1127179;
  END;
  EXPORT __ENH_Person_Address := PROJECT(__EE1127284,__ND1127318__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Address::Annotated',EXPIRE(7));
  SHARED __EE1387711 := __ENH_Person_Address;
  SHARED __IDX_Person_Address_Location__Filtered := __EE1387711(__NN(__EE1387711.Location_));
  ...
  EXPORT __ST1387713_Layout := RECORDOF(IDX_Person_Address_Location_);
  EXPORT IDX_Person_Address_Location__Wrapped := PROJECT(IDX_Person_Address_Location_,TRANSFORM(__ST31115_Layout,SELF.Location_ := __CN(LEFT.Location_),SELF:=LEFT));

...
EXPORT B_Person_Address := MODULE
  ...
  SHARED __EE2344283 := __E_Person_Address;
  ...
  END;
  SHARED __ST45779_Layout __ND2344317__Project(E_Person_Address.Layout __PP2344178) := TRANSFORM
    __EE2344216 := __PP2344178.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2344216,__T(__EE2344216).Event_Date_);
    __EE2344245 := __PP2344178.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2344245,__T(__EE2344245).Event_Date_);
    SELF := __PP2344178;
  END;
  EXPORT __ENH_Person_Address := PROJECT(__EE2344283,__ND2344317__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Address::Annotated',EXPIRE(7));
  SHARED __EE2603006 := __ENH_Person_Address;
  SHARED __IDX_Person_Address_Location__Filtered := __EE2603006(__NN(__EE2603006.Location_));
  ...
  EXPORT __ST2603008_Layout := RECORDOF(IDX_Person_Address_Location_);
  EXPORT IDX_Person_Address_Location__Wrapped := PROJECT(IDX_Person_Address_Location_,TRANSFORM(__ST45779_Layout,SELF.Location_ := __CN(LEFT.Location_),SELF:=LEFT));

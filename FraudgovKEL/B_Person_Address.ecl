...
EXPORT B_Person_Address := MODULE
  ...
  SHARED __EE2537037 := __E_Person_Address;
  EXPORT __ST53080_Layout := RECORD
  ...
  END;
  SHARED __ST53080_Layout __ND2537071__Project(E_Person_Address.Layout __PP2536932) := TRANSFORM
    __EE2536970 := __PP2536932.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2536970,__T(__EE2536970).Event_Date_);
    __EE2536999 := __PP2536932.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2536999,__T(__EE2536999).Event_Date_);
    SELF := __PP2536932;
  END;
  EXPORT __ENH_Person_Address := PROJECT(__EE2537037,__ND2537071__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Address::Annotated',EXPIRE(7));
  SHARED __EE2805617 := __ENH_Person_Address;
  SHARED __IDX_Person_Address_Location__Filtered := __EE2805617(__NN(__EE2805617.Location_));
  ...
  EXPORT __ST2805619_Layout := RECORDOF(IDX_Person_Address_Location_);
  EXPORT IDX_Person_Address_Location__Wrapped := PROJECT(IDX_Person_Address_Location_,TRANSFORM(__ST53080_Layout,SELF.Location_ := __CN(LEFT.Location_),SELF:=LEFT));

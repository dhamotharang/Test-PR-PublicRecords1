...
EXPORT B_Person_Address := MODULE
  ...
  SHARED __EE4006061 := __E_Person_Address;
  EXPORT __ST67042_Layout := RECORD
  ...
  END;
  SHARED __ST67042_Layout __ND4006095__Project(E_Person_Address.Layout __PP4005956) := TRANSFORM
    __EE4005994 := __PP4005956.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE4005994,__T(__EE4005994).Event_Date_);
    __EE4006023 := __PP4005956.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE4006023,__T(__EE4006023).Event_Date_);
    SELF := __PP4005956;
  END;
  EXPORT __ENH_Person_Address := PROJECT(__EE4006061,__ND4006095__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Address::Annotated',EXPIRE(7));
  SHARED __EE4316676 := __ENH_Person_Address;
  SHARED __IDX_Person_Address_Location__Filtered := __EE4316676(__NN(__EE4316676.Location_));
  ...
  EXPORT __ST4316678_Layout := RECORDOF(IDX_Person_Address_Location_);
  EXPORT IDX_Person_Address_Location__Wrapped := PROJECT(IDX_Person_Address_Location_,TRANSFORM(__ST67042_Layout,SELF.Location_ := __CN(LEFT.Location_),SELF:=LEFT));

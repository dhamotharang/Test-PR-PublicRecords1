...
EXPORT B_Person_Address := MODULE
  ...
  SHARED __EE1064045 := __E_Person_Address;
  EXPORT __ST28932_Layout := RECORD
  ...
  END;
  SHARED __ST28932_Layout __ND1064079__Project(E_Person_Address.Layout __PP1063940) := TRANSFORM
    __EE1063978 := __PP1063940.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1063978,__T(__EE1063978).Event_Date_);
    __EE1064007 := __PP1063940.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1064007,__T(__EE1064007).Event_Date_);
    SELF := __PP1063940;
  END;
  EXPORT __ENH_Person_Address := PROJECT(__EE1064045,__ND1064079__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Address::Annotated',EXPIRE(7));
  SHARED __EE1284060 := __ENH_Person_Address;
  SHARED __IDX_Person_Address_Location__Filtered := __EE1284060(__NN(__EE1284060.Location_));
  ...
  EXPORT __ST1284062_Layout := RECORDOF(IDX_Person_Address_Location_);
  EXPORT IDX_Person_Address_Location__Wrapped := PROJECT(IDX_Person_Address_Location_,TRANSFORM(__ST28932_Layout,SELF.Location_ := __CN(LEFT.Location_),SELF:=LEFT));

...
EXPORT B_Person_Address := MODULE
  ...
  SHARED __EE1089274 := __E_Person_Address;
  EXPORT __ST30548_Layout := RECORD
  ...
  END;
  SHARED __ST30548_Layout __ND1089308__Project(E_Person_Address.Layout __PP1089169) := TRANSFORM
    __EE1089207 := __PP1089169.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1089207,__T(__EE1089207).Event_Date_);
    __EE1089236 := __PP1089169.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1089236,__T(__EE1089236).Event_Date_);
    SELF := __PP1089169;
  END;
  EXPORT __ENH_Person_Address := PROJECT(__EE1089274,__ND1089308__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Address::Annotated',EXPIRE(7));
  SHARED __EE1361920 := __ENH_Person_Address;
  SHARED __IDX_Person_Address_Location__Filtered := __EE1361920(__NN(__EE1361920.Location_));
  ...
  EXPORT __ST1361922_Layout := RECORDOF(IDX_Person_Address_Location_);
  EXPORT IDX_Person_Address_Location__Wrapped := PROJECT(IDX_Person_Address_Location_,TRANSFORM(__ST30548_Layout,SELF.Location_ := __CN(LEFT.Location_),SELF:=LEFT));

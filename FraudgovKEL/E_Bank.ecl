...
EXPORT E_Bank := MODULE
  ...
  EXPORT Lookup := NullLookupRec + PROJECT(__Table,TRANSFORM(__TabRec,SELF.UID:=COUNTER,SELF:=LEFT)) : PERSIST('~fraudgov::tempKEL::FraudgovKEL::Bank::UidLookup',EXPIRE(7));
  EXPORT UID_IdToText := INDEX(Lookup,{UID},{Lookup},'~fraudgov::tempKEL::IDtoT::FraudgovKEL::Bank');
  EXPORT UID_TextToId := INDEX(Lookup,{ht:=HASH32(KeyVal)},{Lookup},'~fraudgov::tempKEL::TtoID::FraudgovKEL::Bank');
  ...
  EXPORT __Result := __CLEARFLAGS(__PreResult) : PERSIST('~fraudgov::tempKEL::FraudgovKEL::Bank::Result',EXPIRE(7));

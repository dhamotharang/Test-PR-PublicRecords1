...
EXPORT RQ__show_Customer_Ip_Address_Entities := MODULE
  SHARED __EE4383424 := B_Internet_Protocol.IDX_Internet_Protocol_UID_Wrapped;
  SHARED __EE4383693 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST4300178_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  __JC4383707(B_Internet_Protocol.__ST64597_Layout __EE4383424, E_Customer.Layout __EE4383693) := __EEQP(__EE4383424._r_Customer_,__EE4383693.UID);
  __ST4300178_Layout __JT4383707(B_Internet_Protocol.__ST64597_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4383921 := JOIN(__EE4383424,__EE4383693,__JC4383707(LEFT,RIGHT),__JT4383707(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST4300038_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST4300038_Layout __ND4384096__Project(__ST4300178_Layout __PP4383922) := TRANSFORM
    SELF.Source_Customer_ := __PP4383922._r_Customer_;
    SELF.Customer_Id_ := __PP4383922.Customer_Id__1_;
    SELF.Industry_Type_ := __PP4383922.Industry_Type__1_;
    SELF.Person_Count_ := __PP4383922.Identity_Count_;
    SELF := __PP4383922;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE4383921,__ND4384096__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Entity_Context_Uid_,Label_,Entity_Type_,Person_Count_,In_Customer_Population_,Contributor_Safe_Flag_,Safe_Flag_,Dt_First_Seen_,Dt_Last_Seen_,Kr_High_Risk_Flag_,Kr_Medium_Risk_Flag_,Kr_Low_Risk_Flag_},Source_Customer_,Customer_Id_,Industry_Type_,Entity_Context_Uid_,Label_,Entity_Type_,Person_Count_,In_Customer_Population_,Contributor_Safe_Flag_,Safe_Flag_,Dt_First_Seen_,Dt_Last_Seen_,Kr_High_Risk_Flag_,Kr_Medium_Risk_Flag_,Kr_Low_Risk_Flag_,MERGE),__ST4300038_Layout));
END;

...
EXPORT RQ__show_Customer_Ip_Address_Entities := MODULE
  SHARED __EE2465726 := B_Internet_Protocol.IDX_Internet_Protocol_UID_Wrapped;
  SHARED __EE2465978 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST2391163_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  __JC2465992(B_Internet_Protocol.__ST43207_Layout __EE2465726, E_Customer.Layout __EE2465978) := __EEQP(__EE2465726._r_Customer_,__EE2465978.UID);
  __ST2391163_Layout __JT2465992(B_Internet_Protocol.__ST43207_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE2466189 := JOIN(__EE2465726,__EE2465978,__JC2465992(LEFT,RIGHT),__JT2465992(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST2391023_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST2391023_Layout __ND2466347__Project(__ST2391163_Layout __PP2466190) := TRANSFORM
    SELF.Source_Customer_ := __PP2466190._r_Customer_;
    SELF.Customer_Id_ := __PP2466190.Customer_Id__1_;
    SELF.Industry_Type_ := __PP2466190.Industry_Type__1_;
    SELF.Person_Count_ := __PP2466190.Identity_Count_;
    SELF := __PP2466190;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE2466189,__ND2466347__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Entity_Context_Uid_,Label_,Entity_Type_,Person_Count_,In_Customer_Population_,Contributor_Safe_Flag_,Safe_Flag_,Dt_First_Seen_,Dt_Last_Seen_,Kr_High_Risk_Flag_,Kr_Medium_Risk_Flag_,Kr_Low_Risk_Flag_},Source_Customer_,Customer_Id_,Industry_Type_,Entity_Context_Uid_,Label_,Entity_Type_,Person_Count_,In_Customer_Population_,Contributor_Safe_Flag_,Safe_Flag_,Dt_First_Seen_,Dt_Last_Seen_,Kr_High_Risk_Flag_,Kr_Medium_Risk_Flag_,Kr_Low_Risk_Flag_,MERGE),__ST2391023_Layout));
END;

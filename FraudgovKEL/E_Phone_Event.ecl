...
EXPORT E_Phone_Event := MODULE
  ...
  EXPORT __d0_KELfiltered := FraudgovKEL.fraudgovshared(AssociatedCustomerFileInfo > 0 AND (UNSIGNED)record_id > 0 AND clean_phones.phone_number != '');
  SHARED __d0_Phone_Number__Layout := RECORD
    RECORDOF(__d0_KELfiltered);
    KEL.typ.uid Phone_Number_;
  END;
  SHARED __d0_Phone_Number__Mapped := JOIN(__d0_KELfiltered,E_Phone.Lookup,TRIM((STRING)LEFT.AssociatedCustomerFileInfo) + '|' + TRIM((STRING)LEFT.clean_phones.phone_number) = RIGHT.KeyVal,TRANSFORM(__d0_Phone_Number__Layout,SELF.Phone_Number_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,SMART);
  ...
  EXPORT __Result := __CLEARFLAGS(__PreResult) : PERSIST('~fraudgov::temp::KEL::FraudgovKEL::Phone_Event::Result',EXPIRE(7));

...
EXPORT E_Phone_Event := MODULE
  ...
  END;
  SHARED __d0_Phone_Number__Mapped := JOIN(__d0_KELfiltered,E_Phone.Lookup,TRIM((STRING)LEFT.AssociatedCustomerFileInfo) + '|' + TRIM((STRING)LEFT.clean_phones.cell_phone) = RIGHT.KeyVal,TRANSFORM(__d0_Phone_Number__Layout,SELF.Phone_Number_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,SMART);
  ...
  SHARED __d1_Phone_Number__Layout := RECORD
    ...
    KEL.typ.uid Phone_Number_;
  END;
  SHARED __d1_Phone_Number__Mapped := JOIN(__d1_KELfiltered,E_Phone.Lookup,TRIM((STRING)LEFT.AssociatedCustomerFileInfo) + '|' + TRIM((STRING)LEFT.clean_phones.phone_number) = RIGHT.KeyVal,TRANSFORM(__d1_Phone_Number__Layout,SELF.Phone_Number_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,SMART);
  SHARED __d1_Transaction__Layout := RECORD
    RECORDOF(__d1_Phone_Number__Mapped);
    KEL.typ.uid Transaction_;
  END;
  SHARED __d1_Transaction__Mapped := JOIN(__d1_Phone_Number__Mapped,E_Event.Lookup,TRIM((STRING)LEFT.AssociatedCustomerFileInfo) + '|' + TRIM((STRING)LEFT.record_id) = RIGHT.KeyVal,TRANSFORM(__d1_Transaction__Layout,SELF.Transaction_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,SMART);
  ...
  EXPORT __Result := __CLEARFLAGS(__PreResult) : PERSIST('~temp::KEL::FraudgovKEL::Phone_Event::Result',EXPIRE(7));

...
EXPORT AddressPersonAssociations := MODULE
  ...
  SHARED DemoHashes := [3977509724, 2727638882, 1139485299, 2459821998, 3635312545, 1026679856, 4401323, 3005794324, 866735130];
	...
	SHARED Sharing := FraudgovKEL.SharingRules;
  ...
  SHARED MatchElements(dsIn, JoinCondition) := FUNCTIONMACRO 
	...
	SHARED IdentityRec := RECORD
		...
		FraudgovKEL.fraudgov.address_1, 
		FraudgovKEL.fraudgov.address_2,
		FraudgovKEL.fraudgov.email_address,
		FraudgovKEL.fraudgov.clean_phones.cell_phone,
		FraudgovKEL.fraudgov.ssn,
		FraudgovKEL.fraudgov.phone_number,
		FraudgovKEL.fraudgov.ip_address,
		FraudgovKEL.fraudgov.bank_routing_number_1, 
		FraudgovKEL.fraudgov.bank_account_number_1,
		FraudgovKEL.fraudgov.bank_routing_number_2, 
		FraudgovKEL.fraudgov.bank_account_number_2,
		FraudgovKEL.fraudgov.did,
		FraudgovKEL.fraudgov.dob,
		FraudgovKEL.fraudgov.event_date
	END;
  ...
  // Addresses with a LOT of dids
	EXPORT HighFrequencyAddresses := TABLE(
															DEDUP(SORT(FraudgovKEL.fraudgovshared(did > 0), AssociatedCustomerFileInfo, did, OttoAddressId), AssociatedCustomerFileInfo, did, OttoAddressId), 
  ...
  EXPORT FraudgovSharedPrep1 := JOIN(FraudgovKEL.fraudgovshared, HighFrequencyAddresses(HighFrequencyAddressFlag=1), 
  ...
  EXPORT FraudgovSharedPrep3 := JOIN(FraudgovSharedPrep2, HighFrequencyPhones, 
	...
	EXPORT HighFrequencyCells := TABLE(
															DEDUP(SORT(FraudgovKEL.fraudgovshared(did > 0), AssociatedCustomerFileInfo, did, cell_phone), AssociatedCustomerFileInfo, did, cell_phone), 
  ...
  EXPORT FraudgovSharedPrep4 := JOIN(FraudgovSharedPrep3, HighFrequencyCells, 
	...
	EXPORT HighFrequencyBank1 := TABLE(
															DEDUP(SORT(FraudgovKEL.fraudgovshared(did > 0), AssociatedCustomerFileInfo, did, OttoBankAccountId), AssociatedCustomerFileInfo, did, OttoBankAccountId), 
  ...
  EXPORT FraudgovSharedPrep5 := JOIN(FraudgovSharedPrep4, HighFrequencyBank1, 
	...
	EXPORT HighFrequencyBank2 := TABLE(
															DEDUP(SORT(FraudgovKEL.fraudgovshared(did > 0), AssociatedCustomerFileInfo, did, OttoBankAccountId2), AssociatedCustomerFileInfo, did, OttoBankAccountId2), 
...
EXPORT AddressMatch := TABLE(Matches, 
                    {AssociatedCustomerFileInfo,
                     ...
                     industry_type_,											
										 ...
										 Person.event_date, MERGE);// : PERSIST('~fraudgov::tempdeleteme30_1');
  ...
  EXPORT CustomerDids := TABLE(FraudgovKEL.fraudgovshared(did > 0), {AssociatedCustomerFileInfo, INTEGER customer_id_ := customer_id,	industry_type_ := classification_permissible_use_access.Ind_type, did, PersonEntityContextUid := '_01' + (STRING)did}, 
  ...
  SHARED PersonAddressMatchStatsPrep2 := PROJECT(CustomerDids, TRANSFORM(RECORDOF(PersonAddressMatchStatsPrep1), 
	...
	EXPORT PersonAddressMatchStatsPrep3 := PersonAddressMatchStatsPrep1 + PersonAddressMatchStatsPrep2 : PERSIST('~fraudgov::tempdeleteme31_1'); // JP

//  output(FraudgovKEL.AddressPersonAssociations.PersonAddressMatchStatsPrep3,, '~fraudgov::tempPersonAddressMatchStatsPrep3_1', overwrite, EXPIRE(7));

...
EXPORT B_Event_4 := MODULE
  ...
  SHARED __EE121454 := __ENH_Event_5;
  SHARED __EE122238 := __EE121454;
  SHARED __ST123369_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  __ST123369_Layout __JT123366(B_Event_5.__ST40697_Layout __l, E_Event.Event_Types_Layout __r) := TRANSFORM
    SELF.__RecordCount := __r.__RecordCount;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE123367 := NORMALIZE(__EE122238,__T(LEFT.Event_Types_),__JT123366(LEFT,RIGHT));
  SHARED __ST122484_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nstr Event_Type_;
    KEL.typ.ndataset(E_Event.Event_Types_Layout) Event_Types_;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __EE123641 := PROJECT(__EE123367,__ST122484_Layout);
  SHARED __ST122513_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nbool Exp1_;
    KEL.typ.nbool Exp2_;
    KEL.typ.nbool Exp3_;
    ...
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __ST122513_Layout __ND125444__Project(__ST122484_Layout __PP125416) := TRANSFORM
    SELF.Exp1_ := __OP2(__PP125416.Event_Type_,IN,__CN(['100','191']));
    SELF.Exp2_ := __OP2(__PP125416.Event_Type_,IN,__CN(['103','104','105','190','193']));
    SELF.Exp3_ := __OP2(__PP125416.Event_Type_,IN,__CN(['101','192']));
    SELF := __PP125416;
  END;
  SHARED __EE125454 := PROJECT(__EE123641,__ND125444__Project(LEFT));
  SHARED __ST122538_Layout := RECORD
    ...
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __EE126777 := PROJECT(__CLEANANDDO(__EE125454,TABLE(__EE125454,{KEL.typ.int C_O_U_N_T___Event_Types_ := COUNT(GROUP,__T(__EE125454.Exp1_)),KEL.typ.int C_O_U_N_T___Event_Types__1_ := COUNT(GROUP,__T(__EE125454.Exp2_)),KEL.typ.int C_O_U_N_T___Event_Types__2_ := COUNT(GROUP,__T(__EE125454.Exp3_)),UID},UID,MERGE)),__ST122538_Layout);
  SHARED __ST123648_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Customer.Typ) _r_Source_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC126786(B_Event_5.__ST40697_Layout __EE121454, __ST122538_Layout __EE126777) := __EEQP(__EE121454.UID,__EE126777.UID);
  __ST123648_Layout __JT126786(B_Event_5.__ST40697_Layout __l, __ST122538_Layout __r) := TRANSFORM
    SELF.U_I_D__1_ := __r.UID;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE126787 := JOIN(__EE121454,__EE126777,__JC126786(LEFT,RIGHT),__JT126786(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST39445_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Customer.Typ) _r_Source_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST39445_Layout __ND125745__Project(__ST123648_Layout __PP125741) := TRANSFORM
    SELF.Kr_High_Risk_Ssn_Flag_ := MAP(__T(__OP2(__CN(__PP125741.C_O_U_N_T___Event_Types_),<>,__CN(0)))=>1,0);
    SELF.Kr_Identity_Flag_ := MAP(__PP125741.Kr_High_Risk_Identity_Flag_ = 1 OR __PP125741.Kr_Medium_Risk_Identity_Flag_ = 1 OR __PP125741.Kr_Low_Risk_Identity_Flag_ = 1=>1,0);
    SELF.Kr_Low_Risk_Ssn_Flag_ := MAP(__T(__OP2(__CN(__PP125741.C_O_U_N_T___Event_Types__1_),<>,__CN(0)))=>1,0);
    SELF.Kr_Medium_Risk_Ssn_Flag_ := MAP(__T(__OP2(__CN(__PP125741.C_O_U_N_T___Event_Types__2_),<>,__CN(0)))=>1,0);
    SELF := __PP125741;
  END;
  EXPORT __ENH_Event_4 := PROJECT(__EE126787,__ND125745__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Event::Annotated_4',EXPIRE(7));
END;

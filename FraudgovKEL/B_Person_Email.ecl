...
EXPORT B_Person_Email := MODULE
  ...
  SHARED __EE1063725 := __E_Person_Email;
  EXPORT __ST28805_Layout := RECORD
  ...
  END;
  SHARED __ST28805_Layout __ND1063759__Project(E_Person_Email.Layout __PP1063620) := TRANSFORM
    __EE1063658 := __PP1063620.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1063658,__T(__EE1063658).Event_Date_);
    __EE1063687 := __PP1063620.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1063687,__T(__EE1063687).Event_Date_);
    SELF := __PP1063620;
  END;
  EXPORT __ENH_Person_Email := PROJECT(__EE1063725,__ND1063759__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Email::Annotated',EXPIRE(7));
  SHARED __EE1283008 := __ENH_Person_Email;
  SHARED __IDX_Person_Email_Emailof__Filtered := __EE1283008(__NN(__EE1283008.Emailof_));
  ...
  EXPORT __ST1283010_Layout := RECORDOF(IDX_Person_Email_Emailof_);
  EXPORT IDX_Person_Email_Emailof__Wrapped := PROJECT(IDX_Person_Email_Emailof_,TRANSFORM(__ST28805_Layout,SELF.Emailof_ := __CN(LEFT.Emailof_),SELF:=LEFT));

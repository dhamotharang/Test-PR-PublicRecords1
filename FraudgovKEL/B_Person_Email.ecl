...
EXPORT B_Person_Email := MODULE
  ...
  SHARED __EE1103744 := __E_Person_Email;
  EXPORT __ST31110_Layout := RECORD
  ...
  END;
  SHARED __ST31110_Layout __ND1103778__Project(E_Person_Email.Layout __PP1103639) := TRANSFORM
    __EE1103677 := __PP1103639.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE1103677,__T(__EE1103677).Event_Date_);
    __EE1103706 := __PP1103639.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE1103706,__T(__EE1103706).Event_Date_);
    SELF := __PP1103639;
  END;
  EXPORT __ENH_Person_Email := PROJECT(__EE1103744,__ND1103778__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Email::Annotated',EXPIRE(7));
  SHARED __EE1362885 := __ENH_Person_Email;
  SHARED __IDX_Person_Email_Emailof__Filtered := __EE1362885(__NN(__EE1362885.Emailof_));
  ...
  EXPORT __ST1362887_Layout := RECORDOF(IDX_Person_Email_Emailof_);
  EXPORT IDX_Person_Email_Emailof__Wrapped := PROJECT(IDX_Person_Email_Emailof_,TRANSFORM(__ST31110_Layout,SELF.Emailof_ := __CN(LEFT.Emailof_),SELF:=LEFT));

...
EXPORT B_Person_Email := MODULE
  ...
  SHARED __EE4143089 := __E_Person_Email;
  EXPORT __ST70081_Layout := RECORD
  ...
  END;
  SHARED __ST70081_Layout __ND4143123__Project(E_Person_Email.Layout __PP4142984) := TRANSFORM
    __EE4143022 := __PP4142984.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE4143022,__T(__EE4143022).Event_Date_);
    __EE4143051 := __PP4142984.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE4143051,__T(__EE4143051).Event_Date_);
    SELF := __PP4142984;
  END;
  EXPORT __ENH_Person_Email := PROJECT(__EE4143089,__ND4143123__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Email::Annotated',EXPIRE(7));
  SHARED __EE4414827 := __ENH_Person_Email;
  SHARED __IDX_Person_Email_Emailof__Filtered := __EE4414827(__NN(__EE4414827.Emailof_));
  ...
  EXPORT __ST4414829_Layout := RECORDOF(IDX_Person_Email_Emailof_);
  EXPORT IDX_Person_Email_Emailof__Wrapped := PROJECT(IDX_Person_Email_Emailof_,TRANSFORM(__ST70081_Layout,SELF.Emailof_ := __CN(LEFT.Emailof_),SELF:=LEFT));

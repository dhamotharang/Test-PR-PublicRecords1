...
EXPORT B_Person_Email := MODULE
  ...
  SHARED __EE3144406 := __E_Person_Email;
  EXPORT __ST63427_Layout := RECORD
  ...
  END;
  SHARED __ST63427_Layout __ND3144440__Project(E_Person_Email.Layout __PP3144301) := TRANSFORM
    __EE3144339 := __PP3144301.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE3144339,__T(__EE3144339).Event_Date_);
    __EE3144368 := __PP3144301.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE3144368,__T(__EE3144368).Event_Date_);
    SELF := __PP3144301;
  END;
  EXPORT __ENH_Person_Email := PROJECT(__EE3144406,__ND3144440__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Email::Annotated',EXPIRE(7));
  SHARED __EE3429835 := __ENH_Person_Email;
  SHARED __IDX_Person_Email_Emailof__Filtered := __EE3429835(__NN(__EE3429835.Emailof_));
  ...
  EXPORT __ST3429837_Layout := RECORDOF(IDX_Person_Email_Emailof_);
  EXPORT IDX_Person_Email_Emailof__Wrapped := PROJECT(IDX_Person_Email_Emailof_,TRANSFORM(__ST63427_Layout,SELF.Emailof_ := __CN(LEFT.Emailof_),SELF:=LEFT));

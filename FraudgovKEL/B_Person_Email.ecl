...
EXPORT B_Person_Email := MODULE
  ...
  SHARED __EE2156324 := __E_Person_Email;
  EXPORT __ST45471_Layout := RECORD
  ...
  END;
  SHARED __ST45471_Layout __ND2156358__Project(E_Person_Email.Layout __PP2156219) := TRANSFORM
    __EE2156257 := __PP2156219.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2156257,__T(__EE2156257).Event_Date_);
    __EE2156286 := __PP2156219.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2156286,__T(__EE2156286).Event_Date_);
    SELF := __PP2156219;
  END;
  EXPORT __ENH_Person_Email := PROJECT(__EE2156324,__ND2156358__Project(LEFT)) : PERSIST('~temp::KEL::FraudgovKEL::Person_Email::Annotated',EXPIRE(7));
  SHARED __EE2407383 := __ENH_Person_Email;
  SHARED __IDX_Person_Email_Emailof__Filtered := __EE2407383(__NN(__EE2407383.Emailof_));
  ...
  EXPORT __ST2407385_Layout := RECORDOF(IDX_Person_Email_Emailof_);
  EXPORT IDX_Person_Email_Emailof__Wrapped := PROJECT(IDX_Person_Email_Emailof_,TRANSFORM(__ST45471_Layout,SELF.Emailof_ := __CN(LEFT.Emailof_),SELF:=LEFT));

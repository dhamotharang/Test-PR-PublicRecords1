...
EXPORT B_Bank_2 := MODULE
  ...
  SHARED __EE802951 := __E_Bank;
  EXPORT __ST87153_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST87153_Layout __ND802986__Project(E_Bank.Layout __PP802873) := TRANSFORM
    SELF.High_Risk_Routing_ := MAP(__T(__OP2(__PP802873.Routing_Number_,IN,__CN(['031101169','124071889','124303120','073972181','084003997','114924742','031101169','063115194'])))=>1,0);
    SELF := __PP802873;
  END;
  EXPORT __ENH_Bank_2 := PROJECT(__EE802951,__ND802986__Project(LEFT)) : PERSIST('~fraudgov::temp::KEL::FraudgovKEL::Bank::Annotated_2',EXPIRE(7));
END;

...
EXPORT E_Internet_Protocol := MODULE
  ...
  EXPORT Lookup := NullLookupRec + PROJECT(__Table,TRANSFORM(__TabRec,SELF.UID:=COUNTER,SELF:=LEFT)) : PERSIST('~fraudgov::tempKEL::FraudgovKEL::Internet_Protocol::UidLookup',EXPIRE(7));
  EXPORT UID_IdToText := INDEX(Lookup,{UID},{Lookup},'~fraudgov::tempKEL::IDtoT::FraudgovKEL::Internet_Protocol');
  EXPORT UID_TextToId := INDEX(Lookup,{ht:=HASH32(KeyVal)},{Lookup},'~fraudgov::tempKEL::TtoID::FraudgovKEL::Internet_Protocol');
  ...
  EXPORT __Result := __CLEARFLAGS(__PreResult) : PERSIST('~fraudgov::tempKEL::FraudgovKEL::Internet_Protocol::Result',EXPIRE(7));

...
EXPORT RQ__show_Customer_Address_Person_Tree_Entities := MODULE
  SHARED __EE1467727 := PROJECT(B_Person_Address.IDX_Person_Address_Location__Wrapped,E_Person_Address.Layout);
  SHARED __EE1467730 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST1374545_Layout := RECORD
  ...
  END;
  __JC1467738(E_Person_Address.Layout __EE1467727, B_Person.__ST30803_Layout __EE1467730) := __EEQP(__EE1467727.Subject_,__EE1467730.UID);
  __ST1374545_Layout __JT1467738(E_Person_Address.Layout __l, B_Person.__ST30803_Layout __r) := TRANSFORM
    SELF._r_Customer__1_ := __r._r_Customer_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE1467739 := JOIN(__EE1467727,__EE1467730,__JC1467738(LEFT,RIGHT),__JT1467738(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE1468067 := B_Address.IDX_Address_UID_Wrapped;
  SHARED __ST1374917_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Address.Typ) Location_;
    KEL.typ.ndataset(E_Person_Address.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC1468075(__ST1374545_Layout __EE1467739, B_Address.__ST22728_Layout __EE1468067) := __EEQP(__EE1467739.Location_,__EE1468067.UID);
  __ST1374917_Layout __JT1468075(__ST1374545_Layout __l, B_Address.__ST22728_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE1468076 := JOIN(__EE1467739,__EE1468067,__JC1468075(LEFT,RIGHT),__JT1468075(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE1469121 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST1375488_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Address.Typ) Location_;
    KEL.typ.ndataset(E_Person_Address.Event_Dates_Layout) Event_Dates_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC1469135(__ST1374917_Layout __EE1468076, E_Customer.Layout __EE1469121) := __EEQP(__EE1468076._r_Customer_,__EE1469121.UID);
  __ST1375488_Layout __JT1469135(__ST1374917_Layout __l, E_Customer.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE1469596 := JOIN(__EE1468076,__EE1469121,__JC1469135(LEFT,RIGHT),__JT1469135(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST1374450_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST1374450_Layout __ND1470048__Project(__ST1375488_Layout __PP1469597) := TRANSFORM
    SELF.Source_Customer_ := __PP1469597._r_Customer_;
    SELF.Customer_Id_ := __PP1469597.Customer_Id__2_;
    SELF.Industry_Type_ := __PP1469597.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP1469597.Entity_Context_Uid__1_;
    SELF := __PP1469597;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE1469596,__ND1470048__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST1374450_Layout));
END;

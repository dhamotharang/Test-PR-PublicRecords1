//HPCC Systems KEL Compiler Version 0.11.6-2
IMPORT KEL011 AS KEL;
IMPORT B_Customer,B_Customer_4,B_Person,B_Person_1,B_Person_2,B_Person_Person,E_Address,E_Customer,E_Person,E_Person_Person FROM FraudgovKEL;
IMPORT * FROM KEL011.Null;
EXPORT RQ__show_Customer_Person_Tree_Entities := MODULE
  SHARED __EE1430479 := B_Person_Person.IDX_Person_Person_From_Person__Wrapped;
  SHARED __ST1434673_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) From_Person_;
    KEL.typ.ntyp(E_Person.Typ) To_Person_;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __EE1434678 := PROJECT(TABLE(PROJECT(__EE1430479,__ST1434673_Layout),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),_r_Customer_,From_Person_,To_Person_},_r_Customer_,From_Person_,To_Person_,MERGE),__ST1434673_Layout);
  SHARED __EE1430482 := B_Person.IDX_Person_UID_Wrapped;
  SHARED __ST1435202_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer__1_;
    KEL.typ.nint Lex_Id_;
    KEL.typ.ndataset(E_Person.Source_Customers_Layout) Source_Customers_;
    KEL.typ.ndataset(E_Person.Reported_Date_Of_Birth_Layout) Reported_Date_Of_Birth_;
    KEL.typ.ndataset(E_Person.Reported_Ssn_Layout) Reported_Ssn_;
    KEL.typ.ndataset(E_Person.Reported_Email_Address_Layout) Reported_Email_Address_;
    KEL.typ.ndataset(E_Person.Full_Name_Layout) Full_Name_;
    KEL.typ.nkdate Deceased_Date_;
    KEL.typ.nkdate Deceased_Date_Of_Birth_;
    KEL.typ.nstr Deceased_First_;
    KEL.typ.nstr Deceased_Middle_;
    KEL.typ.nstr Deceased_Last_;
    KEL.typ.nstr Deceased_Match_Code_;
    KEL.typ.nbool _isdeepdive_;
    KEL.typ.nstr _county__death_;
    KEL.typ.nstr Deceased_Ssn_;
    KEL.typ.nstr _state__death__flag_;
    KEL.typ.nstr _death__rec__src_;
    KEL.typ.nstr _state__death__id_;
    KEL.typ.nstr _verfirst_;
    KEL.typ.nstr _verlast_;
    KEL.typ.nstr _veraddr_;
    KEL.typ.nstr _vercity_;
    KEL.typ.nstr _verstate_;
    KEL.typ.nstr _verzip_;
    KEL.typ.nstr _verzip4_;
    KEL.typ.nstr _verssn_;
    KEL.typ.nstr _verdob_;
    KEL.typ.nstr _verhphone_;
    KEL.typ.nstr _verify__addr_;
    KEL.typ.nstr _verify__dob_;
    KEL.typ.nstr _valid__ssn_;
    KEL.typ.nint _nas__summary_;
    KEL.typ.nint _nap__summary_;
    KEL.typ.nstr _cvi_;
    KEL.typ.nstr _additional__fname__1_;
    KEL.typ.nstr _additional__lname__1_;
    KEL.typ.nstr _additional__lname__date__last__1_;
    KEL.typ.nstr _additional__fname__2_;
    KEL.typ.nstr _additional__lname__2_;
    KEL.typ.nstr _additional__lname__date__last__2_;
    KEL.typ.nstr _additional__fname__3_;
    KEL.typ.nstr _additional__lname__3_;
    KEL.typ.nstr _additional__lname__date__last__3_;
    KEL.typ.nint _subjectssncount_;
    KEL.typ.nstr _dobmatchlevel_;
    KEL.typ.nbool _ssnfoundforlexid_;
    KEL.typ.nstr _cvicustomscore_;
    KEL.typ.nint Subject_Ssn_Count_;
    KEL.typ.nint Date_Of_Birth_Match_Level_;
    KEL.typ.nint Stolen_Identity_Index_;
    KEL.typ.nint Synthetic_Identity_Index_;
    KEL.typ.nint Manipulated_Identity_Index_;
    KEL.typ.nint Vulnerable_Victim_Index_;
    KEL.typ.nint Friendlyfraud_Index_;
    KEL.typ.nint Suspicious_Activity_Index_;
    KEL.typ.nint _v2__sourcerisklevel_;
    KEL.typ.nint _v2__assocsuspicousidentitiescount_;
    KEL.typ.nint _v2__assoccreditbureauonlycount_;
    KEL.typ.nint _v2__validationaddrproblems_;
    KEL.typ.nint _v2__inputaddrageoldest_;
    KEL.typ.nstr _v2__inputaddrdwelltype_;
    KEL.typ.nstr _v2__divssnidentitycountnew_;
    KEL.typ.nstr _curr__incar__flag_;
    KEL.typ.nint _name__ssn__dob__match_;
    KEL.typ.ndataset(E_Person.Offenses_Layout) Offenses_;
    KEL.typ.ndataset(E_Person.Address_Layout) Address_;
    KEL.typ.int Address_Count_ := 0;
    KEL.typ.int Address_Invalid30_Count_ := 0;
    KEL.typ.int Address_Invalid30_Flag_ := 0;
    KEL.typ.int Address_Is_Cmra30_Count_ := 0;
    KEL.typ.int Address_Is_Cmra30_Flag_ := 0;
    KEL.typ.int Address_Is_Of_State30_Flag_ := 0;
    KEL.typ.int Address_Is_Out_Of_State30_Count_ := 0;
    KEL.typ.nint Age_;
    KEL.typ.nint Age_At_Last_Event_;
    KEL.typ.int All_Death_Prior_To_All_Events_ := 0;
    KEL.typ.int All_Deceased_Event_Count_ := 0;
    KEL.typ.nfloat All_Deceased_Event_Percent_;
    KEL.typ.int All_High_Risk_Death_Prior_To_All_Events_ := 0;
    KEL.typ.nint All_Max_Deceased_To_Event_Diff_;
    KEL.typ.int Associated_With_Fraud_Offenses_Flag_ := 0;
    KEL.typ.int Associated_With_Incarcerated_Flag_ := 0;
    KEL.typ.nstr Best_City_;
    KEL.typ.nstr Best_Email_;
    KEL.typ.nstr Best_First_Name_;
    B_Person_2.__NS215277_Layout Best_Full_Name_;
    KEL.typ.nstr Best_Last_Name_;
    KEL.typ.nstr Best_Middle_Name_;
    KEL.typ.nstr Best_Name_Suffix_;
    B_Person_1.__NS427404_Layout Best_Reported_Address_;
    B_Person_1.__NS427424_Layout Best_Reported_Email_;
    B_Person_1.__NS427432_Layout Best_Reported_Ssn_;
    KEL.typ.nstr Best_Ssn_;
    KEL.typ.nstr Best_State_;
    KEL.typ.nstr Best_Street_Address_;
    KEL.typ.nstr Best_Zip_;
    KEL.typ.int Contributor_Safe_Flag_ := 0;
    KEL.typ.int Currently_Incarcerated_Flag_ := 0;
    KEL.typ.nint Customer_Id_;
    KEL.typ.nkdate Date_Of_Birth_;
    KEL.typ.int Death_Prior_To_All_Events_ := 0;
    KEL.typ.int Deceased_ := 0;
    KEL.typ.int Deceased_Dob_Match_ := 0;
    KEL.typ.int Deceased_Event_Count_ := 0;
    KEL.typ.nfloat Deceased_Event_Percent_;
    KEL.typ.int Deceased_Match_ := 0;
    KEL.typ.nstr Deceased_Match_Date_Of_Death_;
    KEL.typ.str Deceased_Match_Description_ := '';
    KEL.typ.int Deceased_Name_Match_ := 0;
    KEL.typ.nkdate Dt_First_Seen_;
    KEL.typ.nkdate Dt_Last_Seen_;
    KEL.typ.nunk Entity_Context_Uid_;
    KEL.typ.int Entity_Type_ := 0;
    KEL.typ.int Event_Count_ := 0;
    KEL.typ.int Fraud_Offense_Count_ := 0;
    KEL.typ.int Fraud_Offenses_Flag_ := 0;
    KEL.typ.int High_Risk_Death_Prior_To_All_Events_ := 0;
    KEL.typ.int Hri03_Flag_ := 0;
    KEL.typ.int Hri06_Flag_ := 0;
    KEL.typ.int Hri07_Flag_ := 0;
    KEL.typ.int Hri08_Flag_ := 0;
    KEL.typ.int Hri11_Flag_ := 0;
    KEL.typ.int Hri12_Flag_ := 0;
    KEL.typ.int Hri14_Active30_Flag_ := 0;
    KEL.typ.int Hri14_Flag_ := 0;
    KEL.typ.int Hri15_Flag_ := 0;
    KEL.typ.int Hri19_Flag_ := 0;
    KEL.typ.int Hri25_Flag_ := 0;
    KEL.typ.int Hri26_Flag_ := 0;
    KEL.typ.int Hri27_Flag_ := 0;
    KEL.typ.int Hri28_Flag_ := 0;
    KEL.typ.int Hri29_Flag_ := 0;
    KEL.typ.int Hri30_Flag_ := 0;
    KEL.typ.int Hri31_Flag_ := 0;
    KEL.typ.int Hri37_Flag_ := 0;
    KEL.typ.int Hri38_Flag_ := 0;
    KEL.typ.int Hri41_Flag_ := 0;
    KEL.typ.int Hri48_Flag_ := 0;
    KEL.typ.int Hri50_Active30_Flag_ := 0;
    KEL.typ.int Hri50_Flag_ := 0;
    KEL.typ.int Hri51_Flag_ := 0;
    KEL.typ.int Hri52_Flag_ := 0;
    KEL.typ.int Hri71_Flag_ := 0;
    KEL.typ.int Hri83_Flag_ := 0;
    KEL.typ.int Hri90_Flag_ := 0;
    KEL.typ.int Hri_Cl_Flag_ := 0;
    KEL.typ.int Hri_Co_Flag_ := 0;
    KEL.typ.int Hri_Dd_Flag_ := 0;
    KEL.typ.int Hri_Df_Flag_ := 0;
    KEL.typ.int Hri_It_Flag_ := 0;
    KEL.typ.int Hri_Iv_Flag_ := 0;
    KEL.typ.int Hri_Mi_Flag_ := 0;
    KEL.typ.int Hri_Mo_Flag_ := 0;
    KEL.typ.int Hri_Ms_Flag_ := 0;
    KEL.typ.int Hri_Nf_Flag_ := 0;
    KEL.typ.int Hri_Pa_Flag_ := 0;
    KEL.typ.int Hri_Po_Flag_ := 0;
    KEL.typ.int Hri_Va_Flag_ := 0;
    KEL.typ.nint Id_Ssn_Identity_Count_Max_;
    KEL.typ.int In_Customer_Population_ := 0;
    KEL.typ.nint Industry_Type_;
    KEL.typ.int Ip_High_Risk_City_ := 0;
    KEL.typ.int Ip_Hosted_ := 0;
    KEL.typ.int Ip_Not_Us_ := 0;
    KEL.typ.int Ip_Tor_ := 0;
    KEL.typ.int Ip_Vpn_ := 0;
    KEL.typ.int Is_Minor_ := 0;
    KEL.typ.int Kr_Event_After_Known_Risk_Count_ := 0;
    KEL.typ.int Kr_Event_After_Known_Risk_Flag_ := 0;
    KEL.typ.int Kr_Event_After_Last_Known_Risk_Count_ := 0;
    KEL.typ.int Kr_Event_After_Last_Known_Risk_Flag_ := 0;
    KEL.typ.int Kr_Flag_ := 0;
    KEL.typ.int Kr_Has_Known_Risk_Element_Count_ := 0;
    KEL.typ.nint Kr_Has_Known_Risk_Element_Flag_;
    KEL.typ.int Kr_High_Risk_Flag_ := 0;
    KEL.typ.int Kr_High_Risk_Routing_ := 0;
    KEL.typ.int Kr_Id10000_Flag_ := 0;
    KEL.typ.int Kr_Id10001_Flag_ := 0;
    KEL.typ.int Kr_Id10002_Flag_ := 0;
    KEL.typ.int Kr_Id10003_Flag_ := 0;
    KEL.typ.int Kr_Id10004_Flag_ := 0;
    KEL.typ.int Kr_Id10005_Flag_ := 0;
    KEL.typ.int Kr_Id10006_Flag_ := 0;
    KEL.typ.int Kr_Id10007_Flag_ := 0;
    KEL.typ.int Kr_Id1010_Flag_ := 0;
    KEL.typ.int Kr_Id11000_Flag_ := 0;
    KEL.typ.int Kr_Id11001_Flag_ := 0;
    KEL.typ.int Kr_Id11002_Flag_ := 0;
    KEL.typ.int Kr_Id11003_Flag_ := 0;
    KEL.typ.int Kr_Id11004_Flag_ := 0;
    KEL.typ.int Kr_Id11005_Flag_ := 0;
    KEL.typ.int Kr_Id11006_Flag_ := 0;
    KEL.typ.int Kr_Id11007_Flag_ := 0;
    KEL.typ.int Kr_Id11008_Flag_ := 0;
    KEL.typ.int Kr_Id11009_Flag_ := 0;
    KEL.typ.int Kr_Id11010_Flag_ := 0;
    KEL.typ.int Kr_Id11011_Flag_ := 0;
    KEL.typ.int Kr_Id11012_Flag_ := 0;
    KEL.typ.int Kr_Id11013_Flag_ := 0;
    KEL.typ.int Kr_Id11014_Flag_ := 0;
    KEL.typ.int Kr_Id11015_Flag_ := 0;
    KEL.typ.int Kr_Id11016_Flag_ := 0;
    KEL.typ.int Kr_Id11017_Flag_ := 0;
    KEL.typ.int Kr_Id11018_Flag_ := 0;
    KEL.typ.int Kr_Id11019_Flag_ := 0;
    KEL.typ.int Kr_Id12000_Flag_ := 0;
    KEL.typ.int Kr_Id12001_Flag_ := 0;
    KEL.typ.int Kr_Id12002_Flag_ := 0;
    KEL.typ.int Kr_Id12003_Flag_ := 0;
    KEL.typ.int Kr_Id12004_Flag_ := 0;
    KEL.typ.int Kr_Id12006_Flag_ := 0;
    KEL.typ.int Kr_Id12007_Flag_ := 0;
    KEL.typ.int Kr_Id13000_Flag_ := 0;
    KEL.typ.int Kr_Id13001_Flag_ := 0;
    KEL.typ.int Kr_Id13002_Flag_ := 0;
    KEL.typ.int Kr_Id13003_Flag_ := 0;
    KEL.typ.int Kr_Id13005_Flag_ := 0;
    KEL.typ.int Kr_Id13006_Flag_ := 0;
    KEL.typ.int Kr_Id13007_Flag_ := 0;
    KEL.typ.int Kr_Id14000_Flag_ := 0;
    KEL.typ.int Kr_Id14001_Flag_ := 0;
    KEL.typ.int Kr_Id14900_Flag_ := 0;
    KEL.typ.int Kr_Id14901_Flag_ := 0;
    KEL.typ.int Kr_Id14902_Flag_ := 0;
    KEL.typ.int Kr_Id14903_Flag_ := 0;
    KEL.typ.int Kr_Id2025_Flag_ := 0;
    KEL.typ.nkdate Kr_Last_Element_Event_Date_;
    KEL.typ.nkdate Kr_Last_Event_Date_;
    KEL.typ.int Kr_Low_Risk_Flag_ := 0;
    KEL.typ.int Kr_Medium_Risk_Flag_ := 0;
    KEL.typ.int Kr_Not_Known_Risk_Has_Known_Risk_Element_Flag_ := 0;
    KEL.typ.nstr Label_;
    KEL.typ.nkdate Last_Event_Date_;
    KEL.typ.nint Last_Record_Id_;
    KEL.typ.nint Max_Deceased_To_Event_Diff_;
    KEL.typ.int Nap3_Flag_ := 0;
    KEL.typ.int Nas9_Flag_ := 0;
    KEL.typ.int No_Lex_Id_ := 0;
    KEL.typ.int No_Lex_Id_Gt22_ := 0;
    KEL.typ.int Other_Customer_Person_Customer_Count_ := 0;
    KEL.typ.int Safe_Flag_ := 0;
    KEL.typ.nkdate Safe_Flag_Last_Date_;
    KEL.typ.int Safe_Flag_Transaction_Count_ := 0;
    KEL.typ.nint Source_Customer_Count_;
    KEL.typ.int Validation_Addr_Problems30_Count_ := 0;
    KEL.typ.int Validation_Addr_Problems30_Flag_ := 0;
    KEL.typ.int Vl_Event1_All_Count_ := 0;
    KEL.typ.int Vl_Event1_Count_ := 0;
    KEL.typ.int Vl_Event30_Active_Flag_ := 0;
    KEL.typ.int Vl_Event30_All_Active_Flag_ := 0;
    KEL.typ.int Vl_Event30_All_Day_Count_ := 0;
    KEL.typ.int Vl_Event30_Count_ := 0;
    KEL.typ.int Vl_Event365_All_Day_Count_ := 0;
    KEL.typ.int Vl_Event365_Count_ := 0;
    KEL.typ.int Vl_Event7_Active_Flag_ := 0;
    KEL.typ.int Vl_Event7_All_Active_Flag_ := 0;
    KEL.typ.int Vl_Event7_All_Count_ := 0;
    KEL.typ.int Vl_Event7_Count_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __EE1435533 := PROJECT(__EE1430482,TRANSFORM(__ST1435202_Layout,SELF._r_Customer__1_ := LEFT._r_Customer_,SELF := LEFT));
  SHARED __ST1435877_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) From_Person_;
    KEL.typ.ntyp(E_Person.Typ) To_Person_;
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer__1_;
    KEL.typ.nint Lex_Id_;
    KEL.typ.ndataset(E_Person.Source_Customers_Layout) Source_Customers_;
    KEL.typ.ndataset(E_Person.Reported_Date_Of_Birth_Layout) Reported_Date_Of_Birth_;
    KEL.typ.ndataset(E_Person.Reported_Ssn_Layout) Reported_Ssn_;
    KEL.typ.ndataset(E_Person.Reported_Email_Address_Layout) Reported_Email_Address_;
    KEL.typ.ndataset(E_Person.Full_Name_Layout) Full_Name_;
    KEL.typ.nkdate Deceased_Date_;
    KEL.typ.nkdate Deceased_Date_Of_Birth_;
    KEL.typ.nstr Deceased_First_;
    KEL.typ.nstr Deceased_Middle_;
    KEL.typ.nstr Deceased_Last_;
    KEL.typ.nstr Deceased_Match_Code_;
    KEL.typ.nbool _isdeepdive_;
    KEL.typ.nstr _county__death_;
    KEL.typ.nstr Deceased_Ssn_;
    KEL.typ.nstr _state__death__flag_;
    KEL.typ.nstr _death__rec__src_;
    KEL.typ.nstr _state__death__id_;
    KEL.typ.nstr _verfirst_;
    KEL.typ.nstr _verlast_;
    KEL.typ.nstr _veraddr_;
    KEL.typ.nstr _vercity_;
    KEL.typ.nstr _verstate_;
    KEL.typ.nstr _verzip_;
    KEL.typ.nstr _verzip4_;
    KEL.typ.nstr _verssn_;
    KEL.typ.nstr _verdob_;
    KEL.typ.nstr _verhphone_;
    KEL.typ.nstr _verify__addr_;
    KEL.typ.nstr _verify__dob_;
    KEL.typ.nstr _valid__ssn_;
    KEL.typ.nint _nas__summary_;
    KEL.typ.nint _nap__summary_;
    KEL.typ.nstr _cvi_;
    KEL.typ.nstr _additional__fname__1_;
    KEL.typ.nstr _additional__lname__1_;
    KEL.typ.nstr _additional__lname__date__last__1_;
    KEL.typ.nstr _additional__fname__2_;
    KEL.typ.nstr _additional__lname__2_;
    KEL.typ.nstr _additional__lname__date__last__2_;
    KEL.typ.nstr _additional__fname__3_;
    KEL.typ.nstr _additional__lname__3_;
    KEL.typ.nstr _additional__lname__date__last__3_;
    KEL.typ.nint _subjectssncount_;
    KEL.typ.nstr _dobmatchlevel_;
    KEL.typ.nbool _ssnfoundforlexid_;
    KEL.typ.nstr _cvicustomscore_;
    KEL.typ.nint Subject_Ssn_Count_;
    KEL.typ.nint Date_Of_Birth_Match_Level_;
    KEL.typ.nint Stolen_Identity_Index_;
    KEL.typ.nint Synthetic_Identity_Index_;
    KEL.typ.nint Manipulated_Identity_Index_;
    KEL.typ.nint Vulnerable_Victim_Index_;
    KEL.typ.nint Friendlyfraud_Index_;
    KEL.typ.nint Suspicious_Activity_Index_;
    KEL.typ.nint _v2__sourcerisklevel_;
    KEL.typ.nint _v2__assocsuspicousidentitiescount_;
    KEL.typ.nint _v2__assoccreditbureauonlycount_;
    KEL.typ.nint _v2__validationaddrproblems_;
    KEL.typ.nint _v2__inputaddrageoldest_;
    KEL.typ.nstr _v2__inputaddrdwelltype_;
    KEL.typ.nstr _v2__divssnidentitycountnew_;
    KEL.typ.nstr _curr__incar__flag_;
    KEL.typ.nint _name__ssn__dob__match_;
    KEL.typ.ndataset(E_Person.Offenses_Layout) Offenses_;
    KEL.typ.ndataset(E_Person.Address_Layout) Address_;
    KEL.typ.int Address_Count_ := 0;
    KEL.typ.int Address_Invalid30_Count_ := 0;
    KEL.typ.int Address_Invalid30_Flag_ := 0;
    KEL.typ.int Address_Is_Cmra30_Count_ := 0;
    KEL.typ.int Address_Is_Cmra30_Flag_ := 0;
    KEL.typ.int Address_Is_Of_State30_Flag_ := 0;
    KEL.typ.int Address_Is_Out_Of_State30_Count_ := 0;
    KEL.typ.nint Age_;
    KEL.typ.nint Age_At_Last_Event_;
    KEL.typ.int All_Death_Prior_To_All_Events_ := 0;
    KEL.typ.int All_Deceased_Event_Count_ := 0;
    KEL.typ.nfloat All_Deceased_Event_Percent_;
    KEL.typ.int All_High_Risk_Death_Prior_To_All_Events_ := 0;
    KEL.typ.nint All_Max_Deceased_To_Event_Diff_;
    KEL.typ.int Associated_With_Fraud_Offenses_Flag_ := 0;
    KEL.typ.int Associated_With_Incarcerated_Flag_ := 0;
    KEL.typ.nstr Best_City_;
    KEL.typ.nstr Best_Email_;
    KEL.typ.nstr Best_First_Name_;
    B_Person_2.__NS215277_Layout Best_Full_Name_;
    KEL.typ.nstr Best_Last_Name_;
    KEL.typ.nstr Best_Middle_Name_;
    KEL.typ.nstr Best_Name_Suffix_;
    B_Person_1.__NS427404_Layout Best_Reported_Address_;
    B_Person_1.__NS427424_Layout Best_Reported_Email_;
    B_Person_1.__NS427432_Layout Best_Reported_Ssn_;
    KEL.typ.nstr Best_Ssn_;
    KEL.typ.nstr Best_State_;
    KEL.typ.nstr Best_Street_Address_;
    KEL.typ.nstr Best_Zip_;
    KEL.typ.int Contributor_Safe_Flag_ := 0;
    KEL.typ.int Currently_Incarcerated_Flag_ := 0;
    KEL.typ.nint Customer_Id_;
    KEL.typ.nkdate Date_Of_Birth_;
    KEL.typ.int Death_Prior_To_All_Events_ := 0;
    KEL.typ.int Deceased_ := 0;
    KEL.typ.int Deceased_Dob_Match_ := 0;
    KEL.typ.int Deceased_Event_Count_ := 0;
    KEL.typ.nfloat Deceased_Event_Percent_;
    KEL.typ.int Deceased_Match_ := 0;
    KEL.typ.nstr Deceased_Match_Date_Of_Death_;
    KEL.typ.str Deceased_Match_Description_ := '';
    KEL.typ.int Deceased_Name_Match_ := 0;
    KEL.typ.nkdate Dt_First_Seen_;
    KEL.typ.nkdate Dt_Last_Seen_;
    KEL.typ.nunk Entity_Context_Uid_;
    KEL.typ.int Entity_Type_ := 0;
    KEL.typ.int Event_Count_ := 0;
    KEL.typ.int Fraud_Offense_Count_ := 0;
    KEL.typ.int Fraud_Offenses_Flag_ := 0;
    KEL.typ.int High_Risk_Death_Prior_To_All_Events_ := 0;
    KEL.typ.int Hri03_Flag_ := 0;
    KEL.typ.int Hri06_Flag_ := 0;
    KEL.typ.int Hri07_Flag_ := 0;
    KEL.typ.int Hri08_Flag_ := 0;
    KEL.typ.int Hri11_Flag_ := 0;
    KEL.typ.int Hri12_Flag_ := 0;
    KEL.typ.int Hri14_Active30_Flag_ := 0;
    KEL.typ.int Hri14_Flag_ := 0;
    KEL.typ.int Hri15_Flag_ := 0;
    KEL.typ.int Hri19_Flag_ := 0;
    KEL.typ.int Hri25_Flag_ := 0;
    KEL.typ.int Hri26_Flag_ := 0;
    KEL.typ.int Hri27_Flag_ := 0;
    KEL.typ.int Hri28_Flag_ := 0;
    KEL.typ.int Hri29_Flag_ := 0;
    KEL.typ.int Hri30_Flag_ := 0;
    KEL.typ.int Hri31_Flag_ := 0;
    KEL.typ.int Hri37_Flag_ := 0;
    KEL.typ.int Hri38_Flag_ := 0;
    KEL.typ.int Hri41_Flag_ := 0;
    KEL.typ.int Hri48_Flag_ := 0;
    KEL.typ.int Hri50_Active30_Flag_ := 0;
    KEL.typ.int Hri50_Flag_ := 0;
    KEL.typ.int Hri51_Flag_ := 0;
    KEL.typ.int Hri52_Flag_ := 0;
    KEL.typ.int Hri71_Flag_ := 0;
    KEL.typ.int Hri83_Flag_ := 0;
    KEL.typ.int Hri90_Flag_ := 0;
    KEL.typ.int Hri_Cl_Flag_ := 0;
    KEL.typ.int Hri_Co_Flag_ := 0;
    KEL.typ.int Hri_Dd_Flag_ := 0;
    KEL.typ.int Hri_Df_Flag_ := 0;
    KEL.typ.int Hri_It_Flag_ := 0;
    KEL.typ.int Hri_Iv_Flag_ := 0;
    KEL.typ.int Hri_Mi_Flag_ := 0;
    KEL.typ.int Hri_Mo_Flag_ := 0;
    KEL.typ.int Hri_Ms_Flag_ := 0;
    KEL.typ.int Hri_Nf_Flag_ := 0;
    KEL.typ.int Hri_Pa_Flag_ := 0;
    KEL.typ.int Hri_Po_Flag_ := 0;
    KEL.typ.int Hri_Va_Flag_ := 0;
    KEL.typ.nint Id_Ssn_Identity_Count_Max_;
    KEL.typ.int In_Customer_Population_ := 0;
    KEL.typ.nint Industry_Type_;
    KEL.typ.int Ip_High_Risk_City_ := 0;
    KEL.typ.int Ip_Hosted_ := 0;
    KEL.typ.int Ip_Not_Us_ := 0;
    KEL.typ.int Ip_Tor_ := 0;
    KEL.typ.int Ip_Vpn_ := 0;
    KEL.typ.int Is_Minor_ := 0;
    KEL.typ.int Kr_Event_After_Known_Risk_Count_ := 0;
    KEL.typ.int Kr_Event_After_Known_Risk_Flag_ := 0;
    KEL.typ.int Kr_Event_After_Last_Known_Risk_Count_ := 0;
    KEL.typ.int Kr_Event_After_Last_Known_Risk_Flag_ := 0;
    KEL.typ.int Kr_Flag_ := 0;
    KEL.typ.int Kr_Has_Known_Risk_Element_Count_ := 0;
    KEL.typ.nint Kr_Has_Known_Risk_Element_Flag_;
    KEL.typ.int Kr_High_Risk_Flag_ := 0;
    KEL.typ.int Kr_High_Risk_Routing_ := 0;
    KEL.typ.int Kr_Id10000_Flag_ := 0;
    KEL.typ.int Kr_Id10001_Flag_ := 0;
    KEL.typ.int Kr_Id10002_Flag_ := 0;
    KEL.typ.int Kr_Id10003_Flag_ := 0;
    KEL.typ.int Kr_Id10004_Flag_ := 0;
    KEL.typ.int Kr_Id10005_Flag_ := 0;
    KEL.typ.int Kr_Id10006_Flag_ := 0;
    KEL.typ.int Kr_Id10007_Flag_ := 0;
    KEL.typ.int Kr_Id1010_Flag_ := 0;
    KEL.typ.int Kr_Id11000_Flag_ := 0;
    KEL.typ.int Kr_Id11001_Flag_ := 0;
    KEL.typ.int Kr_Id11002_Flag_ := 0;
    KEL.typ.int Kr_Id11003_Flag_ := 0;
    KEL.typ.int Kr_Id11004_Flag_ := 0;
    KEL.typ.int Kr_Id11005_Flag_ := 0;
    KEL.typ.int Kr_Id11006_Flag_ := 0;
    KEL.typ.int Kr_Id11007_Flag_ := 0;
    KEL.typ.int Kr_Id11008_Flag_ := 0;
    KEL.typ.int Kr_Id11009_Flag_ := 0;
    KEL.typ.int Kr_Id11010_Flag_ := 0;
    KEL.typ.int Kr_Id11011_Flag_ := 0;
    KEL.typ.int Kr_Id11012_Flag_ := 0;
    KEL.typ.int Kr_Id11013_Flag_ := 0;
    KEL.typ.int Kr_Id11014_Flag_ := 0;
    KEL.typ.int Kr_Id11015_Flag_ := 0;
    KEL.typ.int Kr_Id11016_Flag_ := 0;
    KEL.typ.int Kr_Id11017_Flag_ := 0;
    KEL.typ.int Kr_Id11018_Flag_ := 0;
    KEL.typ.int Kr_Id11019_Flag_ := 0;
    KEL.typ.int Kr_Id12000_Flag_ := 0;
    KEL.typ.int Kr_Id12001_Flag_ := 0;
    KEL.typ.int Kr_Id12002_Flag_ := 0;
    KEL.typ.int Kr_Id12003_Flag_ := 0;
    KEL.typ.int Kr_Id12004_Flag_ := 0;
    KEL.typ.int Kr_Id12006_Flag_ := 0;
    KEL.typ.int Kr_Id12007_Flag_ := 0;
    KEL.typ.int Kr_Id13000_Flag_ := 0;
    KEL.typ.int Kr_Id13001_Flag_ := 0;
    KEL.typ.int Kr_Id13002_Flag_ := 0;
    KEL.typ.int Kr_Id13003_Flag_ := 0;
    KEL.typ.int Kr_Id13005_Flag_ := 0;
    KEL.typ.int Kr_Id13006_Flag_ := 0;
    KEL.typ.int Kr_Id13007_Flag_ := 0;
    KEL.typ.int Kr_Id14000_Flag_ := 0;
    KEL.typ.int Kr_Id14001_Flag_ := 0;
    KEL.typ.int Kr_Id14900_Flag_ := 0;
    KEL.typ.int Kr_Id14901_Flag_ := 0;
    KEL.typ.int Kr_Id14902_Flag_ := 0;
    KEL.typ.int Kr_Id14903_Flag_ := 0;
    KEL.typ.int Kr_Id2025_Flag_ := 0;
    KEL.typ.nkdate Kr_Last_Element_Event_Date_;
    KEL.typ.nkdate Kr_Last_Event_Date_;
    KEL.typ.int Kr_Low_Risk_Flag_ := 0;
    KEL.typ.int Kr_Medium_Risk_Flag_ := 0;
    KEL.typ.int Kr_Not_Known_Risk_Has_Known_Risk_Element_Flag_ := 0;
    KEL.typ.nstr Label_;
    KEL.typ.nkdate Last_Event_Date_;
    KEL.typ.nint Last_Record_Id_;
    KEL.typ.nint Max_Deceased_To_Event_Diff_;
    KEL.typ.int Nap3_Flag_ := 0;
    KEL.typ.int Nas9_Flag_ := 0;
    KEL.typ.int No_Lex_Id_ := 0;
    KEL.typ.int No_Lex_Id_Gt22_ := 0;
    KEL.typ.int Other_Customer_Person_Customer_Count_ := 0;
    KEL.typ.int Safe_Flag_ := 0;
    KEL.typ.nkdate Safe_Flag_Last_Date_;
    KEL.typ.int Safe_Flag_Transaction_Count_ := 0;
    KEL.typ.nint Source_Customer_Count_;
    KEL.typ.int Validation_Addr_Problems30_Count_ := 0;
    KEL.typ.int Validation_Addr_Problems30_Flag_ := 0;
    KEL.typ.int Vl_Event1_All_Count_ := 0;
    KEL.typ.int Vl_Event1_Count_ := 0;
    KEL.typ.int Vl_Event30_Active_Flag_ := 0;
    KEL.typ.int Vl_Event30_All_Active_Flag_ := 0;
    KEL.typ.int Vl_Event30_All_Day_Count_ := 0;
    KEL.typ.int Vl_Event30_Count_ := 0;
    KEL.typ.int Vl_Event365_All_Day_Count_ := 0;
    KEL.typ.int Vl_Event365_Count_ := 0;
    KEL.typ.int Vl_Event7_Active_Flag_ := 0;
    KEL.typ.int Vl_Event7_All_Active_Flag_ := 0;
    KEL.typ.int Vl_Event7_All_Count_ := 0;
    KEL.typ.int Vl_Event7_Count_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.int __RecordCount := 0;
  END;
  __JC1435540(__ST1434673_Layout __EE1434678, __ST1435202_Layout __EE1435533) := __EEQP(__EE1434678.To_Person_,__EE1435533.UID);
  __ST1435877_Layout __JT1435540(__ST1434673_Layout __l, __ST1435202_Layout __r) := TRANSFORM
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE1435875 := JOIN(__EE1434678,__EE1435533,__JC1435540(LEFT,RIGHT),__JT1435540(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE1432765 := __EE1430482;
  SHARED __ST1346809_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) From_Person_;
    KEL.typ.ntyp(E_Person.Typ) To_Person_;
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer__1_;
    KEL.typ.nint Lex_Id_;
    KEL.typ.ndataset(E_Person.Source_Customers_Layout) Source_Customers_;
    KEL.typ.ndataset(E_Person.Reported_Date_Of_Birth_Layout) Reported_Date_Of_Birth_;
    KEL.typ.ndataset(E_Person.Reported_Ssn_Layout) Reported_Ssn_;
    KEL.typ.ndataset(E_Person.Reported_Email_Address_Layout) Reported_Email_Address_;
    KEL.typ.ndataset(E_Person.Full_Name_Layout) Full_Name_;
    KEL.typ.nkdate Deceased_Date_;
    KEL.typ.nkdate Deceased_Date_Of_Birth_;
    KEL.typ.nstr Deceased_First_;
    KEL.typ.nstr Deceased_Middle_;
    KEL.typ.nstr Deceased_Last_;
    KEL.typ.nstr Deceased_Match_Code_;
    KEL.typ.nbool _isdeepdive_;
    KEL.typ.nstr _county__death_;
    KEL.typ.nstr Deceased_Ssn_;
    KEL.typ.nstr _state__death__flag_;
    KEL.typ.nstr _death__rec__src_;
    KEL.typ.nstr _state__death__id_;
    KEL.typ.nstr _verfirst_;
    KEL.typ.nstr _verlast_;
    KEL.typ.nstr _veraddr_;
    KEL.typ.nstr _vercity_;
    KEL.typ.nstr _verstate_;
    KEL.typ.nstr _verzip_;
    KEL.typ.nstr _verzip4_;
    KEL.typ.nstr _verssn_;
    KEL.typ.nstr _verdob_;
    KEL.typ.nstr _verhphone_;
    KEL.typ.nstr _verify__addr_;
    KEL.typ.nstr _verify__dob_;
    KEL.typ.nstr _valid__ssn_;
    KEL.typ.nint _nas__summary_;
    KEL.typ.nint _nap__summary_;
    KEL.typ.nstr _cvi_;
    KEL.typ.nstr _additional__fname__1_;
    KEL.typ.nstr _additional__lname__1_;
    KEL.typ.nstr _additional__lname__date__last__1_;
    KEL.typ.nstr _additional__fname__2_;
    KEL.typ.nstr _additional__lname__2_;
    KEL.typ.nstr _additional__lname__date__last__2_;
    KEL.typ.nstr _additional__fname__3_;
    KEL.typ.nstr _additional__lname__3_;
    KEL.typ.nstr _additional__lname__date__last__3_;
    KEL.typ.nint _subjectssncount_;
    KEL.typ.nstr _dobmatchlevel_;
    KEL.typ.nbool _ssnfoundforlexid_;
    KEL.typ.nstr _cvicustomscore_;
    KEL.typ.nint Subject_Ssn_Count_;
    KEL.typ.nint Date_Of_Birth_Match_Level_;
    KEL.typ.nint Stolen_Identity_Index_;
    KEL.typ.nint Synthetic_Identity_Index_;
    KEL.typ.nint Manipulated_Identity_Index_;
    KEL.typ.nint Vulnerable_Victim_Index_;
    KEL.typ.nint Friendlyfraud_Index_;
    KEL.typ.nint Suspicious_Activity_Index_;
    KEL.typ.nint _v2__sourcerisklevel_;
    KEL.typ.nint _v2__assocsuspicousidentitiescount_;
    KEL.typ.nint _v2__assoccreditbureauonlycount_;
    KEL.typ.nint _v2__validationaddrproblems_;
    KEL.typ.nint _v2__inputaddrageoldest_;
    KEL.typ.nstr _v2__inputaddrdwelltype_;
    KEL.typ.nstr _v2__divssnidentitycountnew_;
    KEL.typ.nstr _curr__incar__flag_;
    KEL.typ.nint _name__ssn__dob__match_;
    KEL.typ.ndataset(E_Person.Offenses_Layout) Offenses_;
    KEL.typ.ndataset(E_Person.Address_Layout) Address_;
    KEL.typ.int Address_Count_ := 0;
    KEL.typ.int Address_Invalid30_Count_ := 0;
    KEL.typ.int Address_Invalid30_Flag_ := 0;
    KEL.typ.int Address_Is_Cmra30_Count_ := 0;
    KEL.typ.int Address_Is_Cmra30_Flag_ := 0;
    KEL.typ.int Address_Is_Of_State30_Flag_ := 0;
    KEL.typ.int Address_Is_Out_Of_State30_Count_ := 0;
    KEL.typ.nint Age_;
    KEL.typ.nint Age_At_Last_Event_;
    KEL.typ.int All_Death_Prior_To_All_Events_ := 0;
    KEL.typ.int All_Deceased_Event_Count_ := 0;
    KEL.typ.nfloat All_Deceased_Event_Percent_;
    KEL.typ.int All_High_Risk_Death_Prior_To_All_Events_ := 0;
    KEL.typ.nint All_Max_Deceased_To_Event_Diff_;
    KEL.typ.int Associated_With_Fraud_Offenses_Flag_ := 0;
    KEL.typ.int Associated_With_Incarcerated_Flag_ := 0;
    KEL.typ.nstr Best_City_;
    KEL.typ.nstr Best_Email_;
    KEL.typ.nstr Best_First_Name_;
    B_Person_2.__NS215277_Layout Best_Full_Name_;
    KEL.typ.nstr Best_Last_Name_;
    KEL.typ.nstr Best_Middle_Name_;
    KEL.typ.nstr Best_Name_Suffix_;
    B_Person_1.__NS427404_Layout Best_Reported_Address_;
    B_Person_1.__NS427424_Layout Best_Reported_Email_;
    B_Person_1.__NS427432_Layout Best_Reported_Ssn_;
    KEL.typ.nstr Best_Ssn_;
    KEL.typ.nstr Best_State_;
    KEL.typ.nstr Best_Street_Address_;
    KEL.typ.nstr Best_Zip_;
    KEL.typ.int Contributor_Safe_Flag_ := 0;
    KEL.typ.int Currently_Incarcerated_Flag_ := 0;
    KEL.typ.nint Customer_Id_;
    KEL.typ.nkdate Date_Of_Birth_;
    KEL.typ.int Death_Prior_To_All_Events_ := 0;
    KEL.typ.int Deceased_ := 0;
    KEL.typ.int Deceased_Dob_Match_ := 0;
    KEL.typ.int Deceased_Event_Count_ := 0;
    KEL.typ.nfloat Deceased_Event_Percent_;
    KEL.typ.int Deceased_Match_ := 0;
    KEL.typ.nstr Deceased_Match_Date_Of_Death_;
    KEL.typ.str Deceased_Match_Description_ := '';
    KEL.typ.int Deceased_Name_Match_ := 0;
    KEL.typ.nkdate Dt_First_Seen_;
    KEL.typ.nkdate Dt_Last_Seen_;
    KEL.typ.nunk Entity_Context_Uid_;
    KEL.typ.int Entity_Type_ := 0;
    KEL.typ.int Event_Count_ := 0;
    KEL.typ.int Fraud_Offense_Count_ := 0;
    KEL.typ.int Fraud_Offenses_Flag_ := 0;
    KEL.typ.int High_Risk_Death_Prior_To_All_Events_ := 0;
    KEL.typ.int Hri03_Flag_ := 0;
    KEL.typ.int Hri06_Flag_ := 0;
    KEL.typ.int Hri07_Flag_ := 0;
    KEL.typ.int Hri08_Flag_ := 0;
    KEL.typ.int Hri11_Flag_ := 0;
    KEL.typ.int Hri12_Flag_ := 0;
    KEL.typ.int Hri14_Active30_Flag_ := 0;
    KEL.typ.int Hri14_Flag_ := 0;
    KEL.typ.int Hri15_Flag_ := 0;
    KEL.typ.int Hri19_Flag_ := 0;
    KEL.typ.int Hri25_Flag_ := 0;
    KEL.typ.int Hri26_Flag_ := 0;
    KEL.typ.int Hri27_Flag_ := 0;
    KEL.typ.int Hri28_Flag_ := 0;
    KEL.typ.int Hri29_Flag_ := 0;
    KEL.typ.int Hri30_Flag_ := 0;
    KEL.typ.int Hri31_Flag_ := 0;
    KEL.typ.int Hri37_Flag_ := 0;
    KEL.typ.int Hri38_Flag_ := 0;
    KEL.typ.int Hri41_Flag_ := 0;
    KEL.typ.int Hri48_Flag_ := 0;
    KEL.typ.int Hri50_Active30_Flag_ := 0;
    KEL.typ.int Hri50_Flag_ := 0;
    KEL.typ.int Hri51_Flag_ := 0;
    KEL.typ.int Hri52_Flag_ := 0;
    KEL.typ.int Hri71_Flag_ := 0;
    KEL.typ.int Hri83_Flag_ := 0;
    KEL.typ.int Hri90_Flag_ := 0;
    KEL.typ.int Hri_Cl_Flag_ := 0;
    KEL.typ.int Hri_Co_Flag_ := 0;
    KEL.typ.int Hri_Dd_Flag_ := 0;
    KEL.typ.int Hri_Df_Flag_ := 0;
    KEL.typ.int Hri_It_Flag_ := 0;
    KEL.typ.int Hri_Iv_Flag_ := 0;
    KEL.typ.int Hri_Mi_Flag_ := 0;
    KEL.typ.int Hri_Mo_Flag_ := 0;
    KEL.typ.int Hri_Ms_Flag_ := 0;
    KEL.typ.int Hri_Nf_Flag_ := 0;
    KEL.typ.int Hri_Pa_Flag_ := 0;
    KEL.typ.int Hri_Po_Flag_ := 0;
    KEL.typ.int Hri_Va_Flag_ := 0;
    KEL.typ.nint Id_Ssn_Identity_Count_Max_;
    KEL.typ.int In_Customer_Population_ := 0;
    KEL.typ.nint Industry_Type_;
    KEL.typ.int Ip_High_Risk_City_ := 0;
    KEL.typ.int Ip_Hosted_ := 0;
    KEL.typ.int Ip_Not_Us_ := 0;
    KEL.typ.int Ip_Tor_ := 0;
    KEL.typ.int Ip_Vpn_ := 0;
    KEL.typ.int Is_Minor_ := 0;
    KEL.typ.int Kr_Event_After_Known_Risk_Count_ := 0;
    KEL.typ.int Kr_Event_After_Known_Risk_Flag_ := 0;
    KEL.typ.int Kr_Event_After_Last_Known_Risk_Count_ := 0;
    KEL.typ.int Kr_Event_After_Last_Known_Risk_Flag_ := 0;
    KEL.typ.int Kr_Flag_ := 0;
    KEL.typ.int Kr_Has_Known_Risk_Element_Count_ := 0;
    KEL.typ.nint Kr_Has_Known_Risk_Element_Flag_;
    KEL.typ.int Kr_High_Risk_Flag_ := 0;
    KEL.typ.int Kr_High_Risk_Routing_ := 0;
    KEL.typ.int Kr_Id10000_Flag_ := 0;
    KEL.typ.int Kr_Id10001_Flag_ := 0;
    KEL.typ.int Kr_Id10002_Flag_ := 0;
    KEL.typ.int Kr_Id10003_Flag_ := 0;
    KEL.typ.int Kr_Id10004_Flag_ := 0;
    KEL.typ.int Kr_Id10005_Flag_ := 0;
    KEL.typ.int Kr_Id10006_Flag_ := 0;
    KEL.typ.int Kr_Id10007_Flag_ := 0;
    KEL.typ.int Kr_Id1010_Flag_ := 0;
    KEL.typ.int Kr_Id11000_Flag_ := 0;
    KEL.typ.int Kr_Id11001_Flag_ := 0;
    KEL.typ.int Kr_Id11002_Flag_ := 0;
    KEL.typ.int Kr_Id11003_Flag_ := 0;
    KEL.typ.int Kr_Id11004_Flag_ := 0;
    KEL.typ.int Kr_Id11005_Flag_ := 0;
    KEL.typ.int Kr_Id11006_Flag_ := 0;
    KEL.typ.int Kr_Id11007_Flag_ := 0;
    KEL.typ.int Kr_Id11008_Flag_ := 0;
    KEL.typ.int Kr_Id11009_Flag_ := 0;
    KEL.typ.int Kr_Id11010_Flag_ := 0;
    KEL.typ.int Kr_Id11011_Flag_ := 0;
    KEL.typ.int Kr_Id11012_Flag_ := 0;
    KEL.typ.int Kr_Id11013_Flag_ := 0;
    KEL.typ.int Kr_Id11014_Flag_ := 0;
    KEL.typ.int Kr_Id11015_Flag_ := 0;
    KEL.typ.int Kr_Id11016_Flag_ := 0;
    KEL.typ.int Kr_Id11017_Flag_ := 0;
    KEL.typ.int Kr_Id11018_Flag_ := 0;
    KEL.typ.int Kr_Id11019_Flag_ := 0;
    KEL.typ.int Kr_Id12000_Flag_ := 0;
    KEL.typ.int Kr_Id12001_Flag_ := 0;
    KEL.typ.int Kr_Id12002_Flag_ := 0;
    KEL.typ.int Kr_Id12003_Flag_ := 0;
    KEL.typ.int Kr_Id12004_Flag_ := 0;
    KEL.typ.int Kr_Id12006_Flag_ := 0;
    KEL.typ.int Kr_Id12007_Flag_ := 0;
    KEL.typ.int Kr_Id13000_Flag_ := 0;
    KEL.typ.int Kr_Id13001_Flag_ := 0;
    KEL.typ.int Kr_Id13002_Flag_ := 0;
    KEL.typ.int Kr_Id13003_Flag_ := 0;
    KEL.typ.int Kr_Id13005_Flag_ := 0;
    KEL.typ.int Kr_Id13006_Flag_ := 0;
    KEL.typ.int Kr_Id13007_Flag_ := 0;
    KEL.typ.int Kr_Id14000_Flag_ := 0;
    KEL.typ.int Kr_Id14001_Flag_ := 0;
    KEL.typ.int Kr_Id14900_Flag_ := 0;
    KEL.typ.int Kr_Id14901_Flag_ := 0;
    KEL.typ.int Kr_Id14902_Flag_ := 0;
    KEL.typ.int Kr_Id14903_Flag_ := 0;
    KEL.typ.int Kr_Id2025_Flag_ := 0;
    KEL.typ.nkdate Kr_Last_Element_Event_Date_;
    KEL.typ.nkdate Kr_Last_Event_Date_;
    KEL.typ.int Kr_Low_Risk_Flag_ := 0;
    KEL.typ.int Kr_Medium_Risk_Flag_ := 0;
    KEL.typ.int Kr_Not_Known_Risk_Has_Known_Risk_Element_Flag_ := 0;
    KEL.typ.nstr Label_;
    KEL.typ.nkdate Last_Event_Date_;
    KEL.typ.nint Last_Record_Id_;
    KEL.typ.nint Max_Deceased_To_Event_Diff_;
    KEL.typ.int Nap3_Flag_ := 0;
    KEL.typ.int Nas9_Flag_ := 0;
    KEL.typ.int No_Lex_Id_ := 0;
    KEL.typ.int No_Lex_Id_Gt22_ := 0;
    KEL.typ.int Other_Customer_Person_Customer_Count_ := 0;
    KEL.typ.int Safe_Flag_ := 0;
    KEL.typ.nkdate Safe_Flag_Last_Date_;
    KEL.typ.int Safe_Flag_Transaction_Count_ := 0;
    KEL.typ.nint Source_Customer_Count_;
    KEL.typ.int Validation_Addr_Problems30_Count_ := 0;
    KEL.typ.int Validation_Addr_Problems30_Flag_ := 0;
    KEL.typ.int Vl_Event1_All_Count_ := 0;
    KEL.typ.int Vl_Event1_Count_ := 0;
    KEL.typ.int Vl_Event30_Active_Flag_ := 0;
    KEL.typ.int Vl_Event30_All_Active_Flag_ := 0;
    KEL.typ.int Vl_Event30_All_Day_Count_ := 0;
    KEL.typ.int Vl_Event30_Count_ := 0;
    KEL.typ.int Vl_Event365_All_Day_Count_ := 0;
    KEL.typ.int Vl_Event365_Count_ := 0;
    KEL.typ.int Vl_Event7_Active_Flag_ := 0;
    KEL.typ.int Vl_Event7_All_Active_Flag_ := 0;
    KEL.typ.int Vl_Event7_All_Count_ := 0;
    KEL.typ.int Vl_Event7_Count_ := 0;
    KEL.typ.nuid U_I_D__1_;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer__2_;
    KEL.typ.nint Lex_Id__1_;
    KEL.typ.ndataset(E_Person.Source_Customers_Layout) Source_Customers__1_;
    KEL.typ.ndataset(E_Person.Reported_Date_Of_Birth_Layout) Reported_Date_Of_Birth__1_;
    KEL.typ.ndataset(E_Person.Reported_Ssn_Layout) Reported_Ssn__1_;
    KEL.typ.ndataset(E_Person.Reported_Email_Address_Layout) Reported_Email_Address__1_;
    KEL.typ.ndataset(E_Person.Full_Name_Layout) Full_Name__1_;
    KEL.typ.nkdate Deceased_Date__1_;
    KEL.typ.nkdate Deceased_Date_Of_Birth__1_;
    KEL.typ.nstr Deceased_First__1_;
    KEL.typ.nstr Deceased_Middle__1_;
    KEL.typ.nstr Deceased_Last__1_;
    KEL.typ.nstr Deceased_Match_Code__1_;
    KEL.typ.nbool _isdeepdive__1_;
    KEL.typ.nstr _county__death__1_;
    KEL.typ.nstr Deceased_Ssn__1_;
    KEL.typ.nstr _state__death__flag__1_;
    KEL.typ.nstr _death__rec__src__1_;
    KEL.typ.nstr _state__death__id__1_;
    KEL.typ.nstr _verfirst__1_;
    KEL.typ.nstr _verlast__1_;
    KEL.typ.nstr _veraddr__1_;
    KEL.typ.nstr _vercity__1_;
    KEL.typ.nstr _verstate__1_;
    KEL.typ.nstr _verzip__1_;
    KEL.typ.nstr _verzip4__1_;
    KEL.typ.nstr _verssn__1_;
    KEL.typ.nstr _verdob__1_;
    KEL.typ.nstr _verhphone__1_;
    KEL.typ.nstr _verify__addr__1_;
    KEL.typ.nstr _verify__dob__1_;
    KEL.typ.nstr _valid__ssn__1_;
    KEL.typ.nint _nas__summary__1_;
    KEL.typ.nint _nap__summary__1_;
    KEL.typ.nstr _cvi__1_;
    KEL.typ.nstr _additional__fname__4_;
    KEL.typ.nstr _additional__lname__4_;
    KEL.typ.nstr _additional__lname__date__last__4_;
    KEL.typ.nstr _additional__fname__5_;
    KEL.typ.nstr _additional__lname__5_;
    KEL.typ.nstr _additional__lname__date__last__5_;
    KEL.typ.nstr _additional__fname__6_;
    KEL.typ.nstr _additional__lname__6_;
    KEL.typ.nstr _additional__lname__date__last__6_;
    KEL.typ.nint _subjectssncount__1_;
    KEL.typ.nstr _dobmatchlevel__1_;
    KEL.typ.nbool _ssnfoundforlexid__1_;
    KEL.typ.nstr _cvicustomscore__1_;
    KEL.typ.nint Subject_Ssn_Count__1_;
    KEL.typ.nint Date_Of_Birth_Match_Level__1_;
    KEL.typ.nint Stolen_Identity_Index__1_;
    KEL.typ.nint Synthetic_Identity_Index__1_;
    KEL.typ.nint Manipulated_Identity_Index__1_;
    KEL.typ.nint Vulnerable_Victim_Index__1_;
    KEL.typ.nint Friendlyfraud_Index__1_;
    KEL.typ.nint Suspicious_Activity_Index__1_;
    KEL.typ.nint _v2__sourcerisklevel__1_;
    KEL.typ.nint _v2__assocsuspicousidentitiescount__1_;
    KEL.typ.nint _v2__assoccreditbureauonlycount__1_;
    KEL.typ.nint _v2__validationaddrproblems__1_;
    KEL.typ.nint _v2__inputaddrageoldest__1_;
    KEL.typ.nstr _v2__inputaddrdwelltype__1_;
    KEL.typ.nstr _v2__divssnidentitycountnew__1_;
    KEL.typ.nstr _curr__incar__flag__1_;
    KEL.typ.nint _name__ssn__dob__match__1_;
    KEL.typ.ndataset(E_Person.Offenses_Layout) Offenses__1_;
    KEL.typ.ndataset(E_Person.Address_Layout) Address__1_;
    KEL.typ.int Address_Count__1_ := 0;
    KEL.typ.int Address_Invalid30_Count__1_ := 0;
    KEL.typ.int Address_Invalid30_Flag__1_ := 0;
    KEL.typ.int Address_Is_Cmra30_Count__1_ := 0;
    KEL.typ.int Address_Is_Cmra30_Flag__1_ := 0;
    KEL.typ.int Address_Is_Of_State30_Flag__1_ := 0;
    KEL.typ.int Address_Is_Out_Of_State30_Count__1_ := 0;
    KEL.typ.nint Age__1_;
    KEL.typ.nint Age_At_Last_Event__1_;
    KEL.typ.int All_Death_Prior_To_All_Events__1_ := 0;
    KEL.typ.int All_Deceased_Event_Count__1_ := 0;
    KEL.typ.nfloat All_Deceased_Event_Percent__1_;
    KEL.typ.int All_High_Risk_Death_Prior_To_All_Events__1_ := 0;
    KEL.typ.nint All_Max_Deceased_To_Event_Diff__1_;
    KEL.typ.int Associated_With_Fraud_Offenses_Flag__1_ := 0;
    KEL.typ.int Associated_With_Incarcerated_Flag__1_ := 0;
    KEL.typ.nstr Best_City__1_;
    KEL.typ.nstr Best_Email__1_;
    KEL.typ.nstr Best_First_Name__1_;
    B_Person_2.__NS215277_Layout Best_Full_Name__1_;
    KEL.typ.nstr Best_Last_Name__1_;
    KEL.typ.nstr Best_Middle_Name__1_;
    KEL.typ.nstr Best_Name_Suffix__1_;
    B_Person_1.__NS427404_Layout Best_Reported_Address__1_;
    B_Person_1.__NS427424_Layout Best_Reported_Email__1_;
    B_Person_1.__NS427432_Layout Best_Reported_Ssn__1_;
    KEL.typ.nstr Best_Ssn__1_;
    KEL.typ.nstr Best_State__1_;
    KEL.typ.nstr Best_Street_Address__1_;
    KEL.typ.nstr Best_Zip__1_;
    KEL.typ.int Contributor_Safe_Flag__1_ := 0;
    KEL.typ.int Currently_Incarcerated_Flag__1_ := 0;
    KEL.typ.nint Customer_Id__1_;
    KEL.typ.nkdate Date_Of_Birth__1_;
    KEL.typ.int Death_Prior_To_All_Events__1_ := 0;
    KEL.typ.int Deceased__1_ := 0;
    KEL.typ.int Deceased_Dob_Match__1_ := 0;
    KEL.typ.int Deceased_Event_Count__1_ := 0;
    KEL.typ.nfloat Deceased_Event_Percent__1_;
    KEL.typ.int Deceased_Match__1_ := 0;
    KEL.typ.nstr Deceased_Match_Date_Of_Death__1_;
    KEL.typ.str Deceased_Match_Description__1_ := '';
    KEL.typ.int Deceased_Name_Match__1_ := 0;
    KEL.typ.nkdate Dt_First_Seen__1_;
    KEL.typ.nkdate Dt_Last_Seen__1_;
    KEL.typ.nunk Entity_Context_Uid__1_;
    KEL.typ.int Entity_Type__1_ := 0;
    KEL.typ.int Event_Count__1_ := 0;
    KEL.typ.int Fraud_Offense_Count__1_ := 0;
    KEL.typ.int Fraud_Offenses_Flag__1_ := 0;
    KEL.typ.int High_Risk_Death_Prior_To_All_Events__1_ := 0;
    KEL.typ.int Hri03_Flag__1_ := 0;
    KEL.typ.int Hri06_Flag__1_ := 0;
    KEL.typ.int Hri07_Flag__1_ := 0;
    KEL.typ.int Hri08_Flag__1_ := 0;
    KEL.typ.int Hri11_Flag__1_ := 0;
    KEL.typ.int Hri12_Flag__1_ := 0;
    KEL.typ.int Hri14_Active30_Flag__1_ := 0;
    KEL.typ.int Hri14_Flag__1_ := 0;
    KEL.typ.int Hri15_Flag__1_ := 0;
    KEL.typ.int Hri19_Flag__1_ := 0;
    KEL.typ.int Hri25_Flag__1_ := 0;
    KEL.typ.int Hri26_Flag__1_ := 0;
    KEL.typ.int Hri27_Flag__1_ := 0;
    KEL.typ.int Hri28_Flag__1_ := 0;
    KEL.typ.int Hri29_Flag__1_ := 0;
    KEL.typ.int Hri30_Flag__1_ := 0;
    KEL.typ.int Hri31_Flag__1_ := 0;
    KEL.typ.int Hri37_Flag__1_ := 0;
    KEL.typ.int Hri38_Flag__1_ := 0;
    KEL.typ.int Hri41_Flag__1_ := 0;
    KEL.typ.int Hri48_Flag__1_ := 0;
    KEL.typ.int Hri50_Active30_Flag__1_ := 0;
    KEL.typ.int Hri50_Flag__1_ := 0;
    KEL.typ.int Hri51_Flag__1_ := 0;
    KEL.typ.int Hri52_Flag__1_ := 0;
    KEL.typ.int Hri71_Flag__1_ := 0;
    KEL.typ.int Hri83_Flag__1_ := 0;
    KEL.typ.int Hri90_Flag__1_ := 0;
    KEL.typ.int Hri_Cl_Flag__1_ := 0;
    KEL.typ.int Hri_Co_Flag__1_ := 0;
    KEL.typ.int Hri_Dd_Flag__1_ := 0;
    KEL.typ.int Hri_Df_Flag__1_ := 0;
    KEL.typ.int Hri_It_Flag__1_ := 0;
    KEL.typ.int Hri_Iv_Flag__1_ := 0;
    KEL.typ.int Hri_Mi_Flag__1_ := 0;
    KEL.typ.int Hri_Mo_Flag__1_ := 0;
    KEL.typ.int Hri_Ms_Flag__1_ := 0;
    KEL.typ.int Hri_Nf_Flag__1_ := 0;
    KEL.typ.int Hri_Pa_Flag__1_ := 0;
    KEL.typ.int Hri_Po_Flag__1_ := 0;
    KEL.typ.int Hri_Va_Flag__1_ := 0;
    KEL.typ.nint Id_Ssn_Identity_Count_Max__1_;
    KEL.typ.int In_Customer_Population__1_ := 0;
    KEL.typ.nint Industry_Type__1_;
    KEL.typ.int Ip_High_Risk_City__1_ := 0;
    KEL.typ.int Ip_Hosted__1_ := 0;
    KEL.typ.int Ip_Not_Us__1_ := 0;
    KEL.typ.int Ip_Tor__1_ := 0;
    KEL.typ.int Ip_Vpn__1_ := 0;
    KEL.typ.int Is_Minor__1_ := 0;
    KEL.typ.int Kr_Event_After_Known_Risk_Count__1_ := 0;
    KEL.typ.int Kr_Event_After_Known_Risk_Flag__1_ := 0;
    KEL.typ.int Kr_Event_After_Last_Known_Risk_Count__1_ := 0;
    KEL.typ.int Kr_Event_After_Last_Known_Risk_Flag__1_ := 0;
    KEL.typ.int Kr_Flag__1_ := 0;
    KEL.typ.int Kr_Has_Known_Risk_Element_Count__1_ := 0;
    KEL.typ.nint Kr_Has_Known_Risk_Element_Flag__1_;
    KEL.typ.int Kr_High_Risk_Flag__1_ := 0;
    KEL.typ.int Kr_High_Risk_Routing__1_ := 0;
    KEL.typ.int Kr_Id10000_Flag__1_ := 0;
    KEL.typ.int Kr_Id10001_Flag__1_ := 0;
    KEL.typ.int Kr_Id10002_Flag__1_ := 0;
    KEL.typ.int Kr_Id10003_Flag__1_ := 0;
    KEL.typ.int Kr_Id10004_Flag__1_ := 0;
    KEL.typ.int Kr_Id10005_Flag__1_ := 0;
    KEL.typ.int Kr_Id10006_Flag__1_ := 0;
    KEL.typ.int Kr_Id10007_Flag__1_ := 0;
    KEL.typ.int Kr_Id1010_Flag__1_ := 0;
    KEL.typ.int Kr_Id11000_Flag__1_ := 0;
    KEL.typ.int Kr_Id11001_Flag__1_ := 0;
    KEL.typ.int Kr_Id11002_Flag__1_ := 0;
    KEL.typ.int Kr_Id11003_Flag__1_ := 0;
    KEL.typ.int Kr_Id11004_Flag__1_ := 0;
    KEL.typ.int Kr_Id11005_Flag__1_ := 0;
    KEL.typ.int Kr_Id11006_Flag__1_ := 0;
    KEL.typ.int Kr_Id11007_Flag__1_ := 0;
    KEL.typ.int Kr_Id11008_Flag__1_ := 0;
    KEL.typ.int Kr_Id11009_Flag__1_ := 0;
    KEL.typ.int Kr_Id11010_Flag__1_ := 0;
    KEL.typ.int Kr_Id11011_Flag__1_ := 0;
    KEL.typ.int Kr_Id11012_Flag__1_ := 0;
    KEL.typ.int Kr_Id11013_Flag__1_ := 0;
    KEL.typ.int Kr_Id11014_Flag__1_ := 0;
    KEL.typ.int Kr_Id11015_Flag__1_ := 0;
    KEL.typ.int Kr_Id11016_Flag__1_ := 0;
    KEL.typ.int Kr_Id11017_Flag__1_ := 0;
    KEL.typ.int Kr_Id11018_Flag__1_ := 0;
    KEL.typ.int Kr_Id11019_Flag__1_ := 0;
    KEL.typ.int Kr_Id12000_Flag__1_ := 0;
    KEL.typ.int Kr_Id12001_Flag__1_ := 0;
    KEL.typ.int Kr_Id12002_Flag__1_ := 0;
    KEL.typ.int Kr_Id12003_Flag__1_ := 0;
    KEL.typ.int Kr_Id12004_Flag__1_ := 0;
    KEL.typ.int Kr_Id12006_Flag__1_ := 0;
    KEL.typ.int Kr_Id12007_Flag__1_ := 0;
    KEL.typ.int Kr_Id13000_Flag__1_ := 0;
    KEL.typ.int Kr_Id13001_Flag__1_ := 0;
    KEL.typ.int Kr_Id13002_Flag__1_ := 0;
    KEL.typ.int Kr_Id13003_Flag__1_ := 0;
    KEL.typ.int Kr_Id13005_Flag__1_ := 0;
    KEL.typ.int Kr_Id13006_Flag__1_ := 0;
    KEL.typ.int Kr_Id13007_Flag__1_ := 0;
    KEL.typ.int Kr_Id14000_Flag__1_ := 0;
    KEL.typ.int Kr_Id14001_Flag__1_ := 0;
    KEL.typ.int Kr_Id14900_Flag__1_ := 0;
    KEL.typ.int Kr_Id14901_Flag__1_ := 0;
    KEL.typ.int Kr_Id14902_Flag__1_ := 0;
    KEL.typ.int Kr_Id14903_Flag__1_ := 0;
    KEL.typ.int Kr_Id2025_Flag__1_ := 0;
    KEL.typ.nkdate Kr_Last_Element_Event_Date__1_;
    KEL.typ.nkdate Kr_Last_Event_Date__1_;
    KEL.typ.int Kr_Low_Risk_Flag__1_ := 0;
    KEL.typ.int Kr_Medium_Risk_Flag__1_ := 0;
    KEL.typ.int Kr_Not_Known_Risk_Has_Known_Risk_Element_Flag__1_ := 0;
    KEL.typ.nstr Label__1_;
    KEL.typ.nkdate Last_Event_Date__1_;
    KEL.typ.nint Last_Record_Id__1_;
    KEL.typ.nint Max_Deceased_To_Event_Diff__1_;
    KEL.typ.int Nap3_Flag__1_ := 0;
    KEL.typ.int Nas9_Flag__1_ := 0;
    KEL.typ.int No_Lex_Id__1_ := 0;
    KEL.typ.int No_Lex_Id_Gt22__1_ := 0;
    KEL.typ.int Other_Customer_Person_Customer_Count__1_ := 0;
    KEL.typ.int Safe_Flag__1_ := 0;
    KEL.typ.nkdate Safe_Flag_Last_Date__1_;
    KEL.typ.int Safe_Flag_Transaction_Count__1_ := 0;
    KEL.typ.nint Source_Customer_Count__1_;
    KEL.typ.int Validation_Addr_Problems30_Count__1_ := 0;
    KEL.typ.int Validation_Addr_Problems30_Flag__1_ := 0;
    KEL.typ.int Vl_Event1_All_Count__1_ := 0;
    KEL.typ.int Vl_Event1_Count__1_ := 0;
    KEL.typ.int Vl_Event30_Active_Flag__1_ := 0;
    KEL.typ.int Vl_Event30_All_Active_Flag__1_ := 0;
    KEL.typ.int Vl_Event30_All_Day_Count__1_ := 0;
    KEL.typ.int Vl_Event30_Count__1_ := 0;
    KEL.typ.int Vl_Event365_All_Day_Count__1_ := 0;
    KEL.typ.int Vl_Event365_Count__1_ := 0;
    KEL.typ.int Vl_Event7_Active_Flag__1_ := 0;
    KEL.typ.int Vl_Event7_All_Active_Flag__1_ := 0;
    KEL.typ.int Vl_Event7_All_Count__1_ := 0;
    KEL.typ.int Vl_Event7_Count__1_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.int __RecordCount := 0;
  END;
  __JC1436215(__ST1435877_Layout __EE1435875, B_Person.__ST30234_Layout __EE1432765) := __EEQP(__EE1435875.From_Person_,__EE1432765.UID);
  __ST1346809_Layout __JT1436215(__ST1435877_Layout __l, B_Person.__ST30234_Layout __r) := TRANSFORM
    SELF.U_I_D__1_ := __r.UID;
    SELF._r_Customer__2_ := __r._r_Customer_;
    SELF.Lex_Id__1_ := __r.Lex_Id_;
    SELF.Source_Customers__1_ := __r.Source_Customers_;
    SELF.Reported_Date_Of_Birth__1_ := __r.Reported_Date_Of_Birth_;
    SELF.Reported_Ssn__1_ := __r.Reported_Ssn_;
    SELF.Reported_Email_Address__1_ := __r.Reported_Email_Address_;
    SELF.Full_Name__1_ := __r.Full_Name_;
    SELF.Deceased_Date__1_ := __r.Deceased_Date_;
    SELF.Deceased_Date_Of_Birth__1_ := __r.Deceased_Date_Of_Birth_;
    SELF.Deceased_First__1_ := __r.Deceased_First_;
    SELF.Deceased_Middle__1_ := __r.Deceased_Middle_;
    SELF.Deceased_Last__1_ := __r.Deceased_Last_;
    SELF.Deceased_Match_Code__1_ := __r.Deceased_Match_Code_;
    SELF._isdeepdive__1_ := __r._isdeepdive_;
    SELF._county__death__1_ := __r._county__death_;
    SELF.Deceased_Ssn__1_ := __r.Deceased_Ssn_;
    SELF._state__death__flag__1_ := __r._state__death__flag_;
    SELF._death__rec__src__1_ := __r._death__rec__src_;
    SELF._state__death__id__1_ := __r._state__death__id_;
    SELF._verfirst__1_ := __r._verfirst_;
    SELF._verlast__1_ := __r._verlast_;
    SELF._veraddr__1_ := __r._veraddr_;
    SELF._vercity__1_ := __r._vercity_;
    SELF._verstate__1_ := __r._verstate_;
    SELF._verzip__1_ := __r._verzip_;
    SELF._verzip4__1_ := __r._verzip4_;
    SELF._verssn__1_ := __r._verssn_;
    SELF._verdob__1_ := __r._verdob_;
    SELF._verhphone__1_ := __r._verhphone_;
    SELF._verify__addr__1_ := __r._verify__addr_;
    SELF._verify__dob__1_ := __r._verify__dob_;
    SELF._valid__ssn__1_ := __r._valid__ssn_;
    SELF._nas__summary__1_ := __r._nas__summary_;
    SELF._nap__summary__1_ := __r._nap__summary_;
    SELF._cvi__1_ := __r._cvi_;
    SELF._additional__fname__4_ := __r._additional__fname__1_;
    SELF._additional__lname__4_ := __r._additional__lname__1_;
    SELF._additional__lname__date__last__4_ := __r._additional__lname__date__last__1_;
    SELF._additional__fname__5_ := __r._additional__fname__2_;
    SELF._additional__lname__5_ := __r._additional__lname__2_;
    SELF._additional__lname__date__last__5_ := __r._additional__lname__date__last__2_;
    SELF._additional__fname__6_ := __r._additional__fname__3_;
    SELF._additional__lname__6_ := __r._additional__lname__3_;
    SELF._additional__lname__date__last__6_ := __r._additional__lname__date__last__3_;
    SELF._subjectssncount__1_ := __r._subjectssncount_;
    SELF._dobmatchlevel__1_ := __r._dobmatchlevel_;
    SELF._ssnfoundforlexid__1_ := __r._ssnfoundforlexid_;
    SELF._cvicustomscore__1_ := __r._cvicustomscore_;
    SELF.Subject_Ssn_Count__1_ := __r.Subject_Ssn_Count_;
    SELF.Date_Of_Birth_Match_Level__1_ := __r.Date_Of_Birth_Match_Level_;
    SELF.Stolen_Identity_Index__1_ := __r.Stolen_Identity_Index_;
    SELF.Synthetic_Identity_Index__1_ := __r.Synthetic_Identity_Index_;
    SELF.Manipulated_Identity_Index__1_ := __r.Manipulated_Identity_Index_;
    SELF.Vulnerable_Victim_Index__1_ := __r.Vulnerable_Victim_Index_;
    SELF.Friendlyfraud_Index__1_ := __r.Friendlyfraud_Index_;
    SELF.Suspicious_Activity_Index__1_ := __r.Suspicious_Activity_Index_;
    SELF._v2__sourcerisklevel__1_ := __r._v2__sourcerisklevel_;
    SELF._v2__assocsuspicousidentitiescount__1_ := __r._v2__assocsuspicousidentitiescount_;
    SELF._v2__assoccreditbureauonlycount__1_ := __r._v2__assoccreditbureauonlycount_;
    SELF._v2__validationaddrproblems__1_ := __r._v2__validationaddrproblems_;
    SELF._v2__inputaddrageoldest__1_ := __r._v2__inputaddrageoldest_;
    SELF._v2__inputaddrdwelltype__1_ := __r._v2__inputaddrdwelltype_;
    SELF._v2__divssnidentitycountnew__1_ := __r._v2__divssnidentitycountnew_;
    SELF._curr__incar__flag__1_ := __r._curr__incar__flag_;
    SELF._name__ssn__dob__match__1_ := __r._name__ssn__dob__match_;
    SELF.Offenses__1_ := __r.Offenses_;
    SELF.Address__1_ := __r.Address_;
    SELF.Address_Count__1_ := __r.Address_Count_;
    SELF.Address_Invalid30_Count__1_ := __r.Address_Invalid30_Count_;
    SELF.Address_Invalid30_Flag__1_ := __r.Address_Invalid30_Flag_;
    SELF.Address_Is_Cmra30_Count__1_ := __r.Address_Is_Cmra30_Count_;
    SELF.Address_Is_Cmra30_Flag__1_ := __r.Address_Is_Cmra30_Flag_;
    SELF.Address_Is_Of_State30_Flag__1_ := __r.Address_Is_Of_State30_Flag_;
    SELF.Address_Is_Out_Of_State30_Count__1_ := __r.Address_Is_Out_Of_State30_Count_;
    SELF.Age__1_ := __r.Age_;
    SELF.Age_At_Last_Event__1_ := __r.Age_At_Last_Event_;
    SELF.All_Death_Prior_To_All_Events__1_ := __r.All_Death_Prior_To_All_Events_;
    SELF.All_Deceased_Event_Count__1_ := __r.All_Deceased_Event_Count_;
    SELF.All_Deceased_Event_Percent__1_ := __r.All_Deceased_Event_Percent_;
    SELF.All_High_Risk_Death_Prior_To_All_Events__1_ := __r.All_High_Risk_Death_Prior_To_All_Events_;
    SELF.All_Max_Deceased_To_Event_Diff__1_ := __r.All_Max_Deceased_To_Event_Diff_;
    SELF.Associated_With_Fraud_Offenses_Flag__1_ := __r.Associated_With_Fraud_Offenses_Flag_;
    SELF.Associated_With_Incarcerated_Flag__1_ := __r.Associated_With_Incarcerated_Flag_;
    SELF.Best_City__1_ := __r.Best_City_;
    SELF.Best_Email__1_ := __r.Best_Email_;
    SELF.Best_First_Name__1_ := __r.Best_First_Name_;
    SELF.Best_Full_Name__1_ := __r.Best_Full_Name_;
    SELF.Best_Last_Name__1_ := __r.Best_Last_Name_;
    SELF.Best_Middle_Name__1_ := __r.Best_Middle_Name_;
    SELF.Best_Name_Suffix__1_ := __r.Best_Name_Suffix_;
    SELF.Best_Reported_Address__1_ := __r.Best_Reported_Address_;
    SELF.Best_Reported_Email__1_ := __r.Best_Reported_Email_;
    SELF.Best_Reported_Ssn__1_ := __r.Best_Reported_Ssn_;
    SELF.Best_Ssn__1_ := __r.Best_Ssn_;
    SELF.Best_State__1_ := __r.Best_State_;
    SELF.Best_Street_Address__1_ := __r.Best_Street_Address_;
    SELF.Best_Zip__1_ := __r.Best_Zip_;
    SELF.Contributor_Safe_Flag__1_ := __r.Contributor_Safe_Flag_;
    SELF.Currently_Incarcerated_Flag__1_ := __r.Currently_Incarcerated_Flag_;
    SELF.Customer_Id__1_ := __r.Customer_Id_;
    SELF.Date_Of_Birth__1_ := __r.Date_Of_Birth_;
    SELF.Death_Prior_To_All_Events__1_ := __r.Death_Prior_To_All_Events_;
    SELF.Deceased__1_ := __r.Deceased_;
    SELF.Deceased_Dob_Match__1_ := __r.Deceased_Dob_Match_;
    SELF.Deceased_Event_Count__1_ := __r.Deceased_Event_Count_;
    SELF.Deceased_Event_Percent__1_ := __r.Deceased_Event_Percent_;
    SELF.Deceased_Match__1_ := __r.Deceased_Match_;
    SELF.Deceased_Match_Date_Of_Death__1_ := __r.Deceased_Match_Date_Of_Death_;
    SELF.Deceased_Match_Description__1_ := __r.Deceased_Match_Description_;
    SELF.Deceased_Name_Match__1_ := __r.Deceased_Name_Match_;
    SELF.Dt_First_Seen__1_ := __r.Dt_First_Seen_;
    SELF.Dt_Last_Seen__1_ := __r.Dt_Last_Seen_;
    SELF.Entity_Context_Uid__1_ := __r.Entity_Context_Uid_;
    SELF.Entity_Type__1_ := __r.Entity_Type_;
    SELF.Event_Count__1_ := __r.Event_Count_;
    SELF.Fraud_Offense_Count__1_ := __r.Fraud_Offense_Count_;
    SELF.Fraud_Offenses_Flag__1_ := __r.Fraud_Offenses_Flag_;
    SELF.High_Risk_Death_Prior_To_All_Events__1_ := __r.High_Risk_Death_Prior_To_All_Events_;
    SELF.Hri03_Flag__1_ := __r.Hri03_Flag_;
    SELF.Hri06_Flag__1_ := __r.Hri06_Flag_;
    SELF.Hri07_Flag__1_ := __r.Hri07_Flag_;
    SELF.Hri08_Flag__1_ := __r.Hri08_Flag_;
    SELF.Hri11_Flag__1_ := __r.Hri11_Flag_;
    SELF.Hri12_Flag__1_ := __r.Hri12_Flag_;
    SELF.Hri14_Active30_Flag__1_ := __r.Hri14_Active30_Flag_;
    SELF.Hri14_Flag__1_ := __r.Hri14_Flag_;
    SELF.Hri15_Flag__1_ := __r.Hri15_Flag_;
    SELF.Hri19_Flag__1_ := __r.Hri19_Flag_;
    SELF.Hri25_Flag__1_ := __r.Hri25_Flag_;
    SELF.Hri26_Flag__1_ := __r.Hri26_Flag_;
    SELF.Hri27_Flag__1_ := __r.Hri27_Flag_;
    SELF.Hri28_Flag__1_ := __r.Hri28_Flag_;
    SELF.Hri29_Flag__1_ := __r.Hri29_Flag_;
    SELF.Hri30_Flag__1_ := __r.Hri30_Flag_;
    SELF.Hri31_Flag__1_ := __r.Hri31_Flag_;
    SELF.Hri37_Flag__1_ := __r.Hri37_Flag_;
    SELF.Hri38_Flag__1_ := __r.Hri38_Flag_;
    SELF.Hri41_Flag__1_ := __r.Hri41_Flag_;
    SELF.Hri48_Flag__1_ := __r.Hri48_Flag_;
    SELF.Hri50_Active30_Flag__1_ := __r.Hri50_Active30_Flag_;
    SELF.Hri50_Flag__1_ := __r.Hri50_Flag_;
    SELF.Hri51_Flag__1_ := __r.Hri51_Flag_;
    SELF.Hri52_Flag__1_ := __r.Hri52_Flag_;
    SELF.Hri71_Flag__1_ := __r.Hri71_Flag_;
    SELF.Hri83_Flag__1_ := __r.Hri83_Flag_;
    SELF.Hri90_Flag__1_ := __r.Hri90_Flag_;
    SELF.Hri_Cl_Flag__1_ := __r.Hri_Cl_Flag_;
    SELF.Hri_Co_Flag__1_ := __r.Hri_Co_Flag_;
    SELF.Hri_Dd_Flag__1_ := __r.Hri_Dd_Flag_;
    SELF.Hri_Df_Flag__1_ := __r.Hri_Df_Flag_;
    SELF.Hri_It_Flag__1_ := __r.Hri_It_Flag_;
    SELF.Hri_Iv_Flag__1_ := __r.Hri_Iv_Flag_;
    SELF.Hri_Mi_Flag__1_ := __r.Hri_Mi_Flag_;
    SELF.Hri_Mo_Flag__1_ := __r.Hri_Mo_Flag_;
    SELF.Hri_Ms_Flag__1_ := __r.Hri_Ms_Flag_;
    SELF.Hri_Nf_Flag__1_ := __r.Hri_Nf_Flag_;
    SELF.Hri_Pa_Flag__1_ := __r.Hri_Pa_Flag_;
    SELF.Hri_Po_Flag__1_ := __r.Hri_Po_Flag_;
    SELF.Hri_Va_Flag__1_ := __r.Hri_Va_Flag_;
    SELF.Id_Ssn_Identity_Count_Max__1_ := __r.Id_Ssn_Identity_Count_Max_;
    SELF.In_Customer_Population__1_ := __r.In_Customer_Population_;
    SELF.Industry_Type__1_ := __r.Industry_Type_;
    SELF.Ip_High_Risk_City__1_ := __r.Ip_High_Risk_City_;
    SELF.Ip_Hosted__1_ := __r.Ip_Hosted_;
    SELF.Ip_Not_Us__1_ := __r.Ip_Not_Us_;
    SELF.Ip_Tor__1_ := __r.Ip_Tor_;
    SELF.Ip_Vpn__1_ := __r.Ip_Vpn_;
    SELF.Is_Minor__1_ := __r.Is_Minor_;
    SELF.Kr_Event_After_Known_Risk_Count__1_ := __r.Kr_Event_After_Known_Risk_Count_;
    SELF.Kr_Event_After_Known_Risk_Flag__1_ := __r.Kr_Event_After_Known_Risk_Flag_;
    SELF.Kr_Event_After_Last_Known_Risk_Count__1_ := __r.Kr_Event_After_Last_Known_Risk_Count_;
    SELF.Kr_Event_After_Last_Known_Risk_Flag__1_ := __r.Kr_Event_After_Last_Known_Risk_Flag_;
    SELF.Kr_Flag__1_ := __r.Kr_Flag_;
    SELF.Kr_Has_Known_Risk_Element_Count__1_ := __r.Kr_Has_Known_Risk_Element_Count_;
    SELF.Kr_Has_Known_Risk_Element_Flag__1_ := __r.Kr_Has_Known_Risk_Element_Flag_;
    SELF.Kr_High_Risk_Flag__1_ := __r.Kr_High_Risk_Flag_;
    SELF.Kr_High_Risk_Routing__1_ := __r.Kr_High_Risk_Routing_;
    SELF.Kr_Id10000_Flag__1_ := __r.Kr_Id10000_Flag_;
    SELF.Kr_Id10001_Flag__1_ := __r.Kr_Id10001_Flag_;
    SELF.Kr_Id10002_Flag__1_ := __r.Kr_Id10002_Flag_;
    SELF.Kr_Id10003_Flag__1_ := __r.Kr_Id10003_Flag_;
    SELF.Kr_Id10004_Flag__1_ := __r.Kr_Id10004_Flag_;
    SELF.Kr_Id10005_Flag__1_ := __r.Kr_Id10005_Flag_;
    SELF.Kr_Id10006_Flag__1_ := __r.Kr_Id10006_Flag_;
    SELF.Kr_Id10007_Flag__1_ := __r.Kr_Id10007_Flag_;
    SELF.Kr_Id1010_Flag__1_ := __r.Kr_Id1010_Flag_;
    SELF.Kr_Id11000_Flag__1_ := __r.Kr_Id11000_Flag_;
    SELF.Kr_Id11001_Flag__1_ := __r.Kr_Id11001_Flag_;
    SELF.Kr_Id11002_Flag__1_ := __r.Kr_Id11002_Flag_;
    SELF.Kr_Id11003_Flag__1_ := __r.Kr_Id11003_Flag_;
    SELF.Kr_Id11004_Flag__1_ := __r.Kr_Id11004_Flag_;
    SELF.Kr_Id11005_Flag__1_ := __r.Kr_Id11005_Flag_;
    SELF.Kr_Id11006_Flag__1_ := __r.Kr_Id11006_Flag_;
    SELF.Kr_Id11007_Flag__1_ := __r.Kr_Id11007_Flag_;
    SELF.Kr_Id11008_Flag__1_ := __r.Kr_Id11008_Flag_;
    SELF.Kr_Id11009_Flag__1_ := __r.Kr_Id11009_Flag_;
    SELF.Kr_Id11010_Flag__1_ := __r.Kr_Id11010_Flag_;
    SELF.Kr_Id11011_Flag__1_ := __r.Kr_Id11011_Flag_;
    SELF.Kr_Id11012_Flag__1_ := __r.Kr_Id11012_Flag_;
    SELF.Kr_Id11013_Flag__1_ := __r.Kr_Id11013_Flag_;
    SELF.Kr_Id11014_Flag__1_ := __r.Kr_Id11014_Flag_;
    SELF.Kr_Id11015_Flag__1_ := __r.Kr_Id11015_Flag_;
    SELF.Kr_Id11016_Flag__1_ := __r.Kr_Id11016_Flag_;
    SELF.Kr_Id11017_Flag__1_ := __r.Kr_Id11017_Flag_;
    SELF.Kr_Id11018_Flag__1_ := __r.Kr_Id11018_Flag_;
    SELF.Kr_Id11019_Flag__1_ := __r.Kr_Id11019_Flag_;
    SELF.Kr_Id12000_Flag__1_ := __r.Kr_Id12000_Flag_;
    SELF.Kr_Id12001_Flag__1_ := __r.Kr_Id12001_Flag_;
    SELF.Kr_Id12002_Flag__1_ := __r.Kr_Id12002_Flag_;
    SELF.Kr_Id12003_Flag__1_ := __r.Kr_Id12003_Flag_;
    SELF.Kr_Id12004_Flag__1_ := __r.Kr_Id12004_Flag_;
    SELF.Kr_Id12006_Flag__1_ := __r.Kr_Id12006_Flag_;
    SELF.Kr_Id12007_Flag__1_ := __r.Kr_Id12007_Flag_;
    SELF.Kr_Id13000_Flag__1_ := __r.Kr_Id13000_Flag_;
    SELF.Kr_Id13001_Flag__1_ := __r.Kr_Id13001_Flag_;
    SELF.Kr_Id13002_Flag__1_ := __r.Kr_Id13002_Flag_;
    SELF.Kr_Id13003_Flag__1_ := __r.Kr_Id13003_Flag_;
    SELF.Kr_Id13005_Flag__1_ := __r.Kr_Id13005_Flag_;
    SELF.Kr_Id13006_Flag__1_ := __r.Kr_Id13006_Flag_;
    SELF.Kr_Id13007_Flag__1_ := __r.Kr_Id13007_Flag_;
    SELF.Kr_Id14000_Flag__1_ := __r.Kr_Id14000_Flag_;
    SELF.Kr_Id14001_Flag__1_ := __r.Kr_Id14001_Flag_;
    SELF.Kr_Id14900_Flag__1_ := __r.Kr_Id14900_Flag_;
    SELF.Kr_Id14901_Flag__1_ := __r.Kr_Id14901_Flag_;
    SELF.Kr_Id14902_Flag__1_ := __r.Kr_Id14902_Flag_;
    SELF.Kr_Id14903_Flag__1_ := __r.Kr_Id14903_Flag_;
    SELF.Kr_Id2025_Flag__1_ := __r.Kr_Id2025_Flag_;
    SELF.Kr_Last_Element_Event_Date__1_ := __r.Kr_Last_Element_Event_Date_;
    SELF.Kr_Last_Event_Date__1_ := __r.Kr_Last_Event_Date_;
    SELF.Kr_Low_Risk_Flag__1_ := __r.Kr_Low_Risk_Flag_;
    SELF.Kr_Medium_Risk_Flag__1_ := __r.Kr_Medium_Risk_Flag_;
    SELF.Kr_Not_Known_Risk_Has_Known_Risk_Element_Flag__1_ := __r.Kr_Not_Known_Risk_Has_Known_Risk_Element_Flag_;
    SELF.Label__1_ := __r.Label_;
    SELF.Last_Event_Date__1_ := __r.Last_Event_Date_;
    SELF.Last_Record_Id__1_ := __r.Last_Record_Id_;
    SELF.Max_Deceased_To_Event_Diff__1_ := __r.Max_Deceased_To_Event_Diff_;
    SELF.Nap3_Flag__1_ := __r.Nap3_Flag_;
    SELF.Nas9_Flag__1_ := __r.Nas9_Flag_;
    SELF.No_Lex_Id__1_ := __r.No_Lex_Id_;
    SELF.No_Lex_Id_Gt22__1_ := __r.No_Lex_Id_Gt22_;
    SELF.Other_Customer_Person_Customer_Count__1_ := __r.Other_Customer_Person_Customer_Count_;
    SELF.Safe_Flag__1_ := __r.Safe_Flag_;
    SELF.Safe_Flag_Last_Date__1_ := __r.Safe_Flag_Last_Date_;
    SELF.Safe_Flag_Transaction_Count__1_ := __r.Safe_Flag_Transaction_Count_;
    SELF.Source_Customer_Count__1_ := __r.Source_Customer_Count_;
    SELF.Validation_Addr_Problems30_Count__1_ := __r.Validation_Addr_Problems30_Count_;
    SELF.Validation_Addr_Problems30_Flag__1_ := __r.Validation_Addr_Problems30_Flag_;
    SELF.Vl_Event1_All_Count__1_ := __r.Vl_Event1_All_Count_;
    SELF.Vl_Event1_Count__1_ := __r.Vl_Event1_Count_;
    SELF.Vl_Event30_Active_Flag__1_ := __r.Vl_Event30_Active_Flag_;
    SELF.Vl_Event30_All_Active_Flag__1_ := __r.Vl_Event30_All_Active_Flag_;
    SELF.Vl_Event30_All_Day_Count__1_ := __r.Vl_Event30_All_Day_Count_;
    SELF.Vl_Event30_Count__1_ := __r.Vl_Event30_Count_;
    SELF.Vl_Event365_All_Day_Count__1_ := __r.Vl_Event365_All_Day_Count_;
    SELF.Vl_Event365_Count__1_ := __r.Vl_Event365_Count_;
    SELF.Vl_Event7_Active_Flag__1_ := __r.Vl_Event7_Active_Flag_;
    SELF.Vl_Event7_All_Active_Flag__1_ := __r.Vl_Event7_All_Active_Flag_;
    SELF.Vl_Event7_All_Count__1_ := __r.Vl_Event7_All_Count_;
    SELF.Vl_Event7_Count__1_ := __r.Vl_Event7_Count_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE1437389 := JOIN(__EE1435875,__EE1432765,__JC1436215(LEFT,RIGHT),__JT1436215(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE1438552 := PROJECT(B_Customer.IDX_Customer_UID_Wrapped,E_Customer.Layout);
  SHARED __ST1348045_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.ntyp(E_Person.Typ) From_Person_;
    KEL.typ.ntyp(E_Person.Typ) To_Person_;
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer__1_;
    KEL.typ.nint Lex_Id_;
    KEL.typ.ndataset(E_Person.Source_Customers_Layout) Source_Customers_;
    KEL.typ.ndataset(E_Person.Reported_Date_Of_Birth_Layout) Reported_Date_Of_Birth_;
    KEL.typ.ndataset(E_Person.Reported_Ssn_Layout) Reported_Ssn_;
    KEL.typ.ndataset(E_Person.Reported_Email_Address_Layout) Reported_Email_Address_;
    KEL.typ.ndataset(E_Person.Full_Name_Layout) Full_Name_;
    KEL.typ.nkdate Deceased_Date_;
    KEL.typ.nkdate Deceased_Date_Of_Birth_;
    KEL.typ.nstr Deceased_First_;
    KEL.typ.nstr Deceased_Middle_;
    KEL.typ.nstr Deceased_Last_;
    KEL.typ.nstr Deceased_Match_Code_;
    KEL.typ.nbool _isdeepdive_;
    KEL.typ.nstr _county__death_;
    KEL.typ.nstr Deceased_Ssn_;
    KEL.typ.nstr _state__death__flag_;
    KEL.typ.nstr _death__rec__src_;
    KEL.typ.nstr _state__death__id_;
    KEL.typ.nstr _verfirst_;
    KEL.typ.nstr _verlast_;
    KEL.typ.nstr _veraddr_;
    KEL.typ.nstr _vercity_;
    KEL.typ.nstr _verstate_;
    KEL.typ.nstr _verzip_;
    KEL.typ.nstr _verzip4_;
    KEL.typ.nstr _verssn_;
    KEL.typ.nstr _verdob_;
    KEL.typ.nstr _verhphone_;
    KEL.typ.nstr _verify__addr_;
    KEL.typ.nstr _verify__dob_;
    KEL.typ.nstr _valid__ssn_;
    KEL.typ.nint _nas__summary_;
    KEL.typ.nint _nap__summary_;
    KEL.typ.nstr _cvi_;
    KEL.typ.nstr _additional__fname__1_;
    KEL.typ.nstr _additional__lname__1_;
    KEL.typ.nstr _additional__lname__date__last__1_;
    KEL.typ.nstr _additional__fname__2_;
    KEL.typ.nstr _additional__lname__2_;
    KEL.typ.nstr _additional__lname__date__last__2_;
    KEL.typ.nstr _additional__fname__3_;
    KEL.typ.nstr _additional__lname__3_;
    KEL.typ.nstr _additional__lname__date__last__3_;
    KEL.typ.nint _subjectssncount_;
    KEL.typ.nstr _dobmatchlevel_;
    KEL.typ.nbool _ssnfoundforlexid_;
    KEL.typ.nstr _cvicustomscore_;
    KEL.typ.nint Subject_Ssn_Count_;
    KEL.typ.nint Date_Of_Birth_Match_Level_;
    KEL.typ.nint Stolen_Identity_Index_;
    KEL.typ.nint Synthetic_Identity_Index_;
    KEL.typ.nint Manipulated_Identity_Index_;
    KEL.typ.nint Vulnerable_Victim_Index_;
    KEL.typ.nint Friendlyfraud_Index_;
    KEL.typ.nint Suspicious_Activity_Index_;
    KEL.typ.nint _v2__sourcerisklevel_;
    KEL.typ.nint _v2__assocsuspicousidentitiescount_;
    KEL.typ.nint _v2__assoccreditbureauonlycount_;
    KEL.typ.nint _v2__validationaddrproblems_;
    KEL.typ.nint _v2__inputaddrageoldest_;
    KEL.typ.nstr _v2__inputaddrdwelltype_;
    KEL.typ.nstr _v2__divssnidentitycountnew_;
    KEL.typ.nstr _curr__incar__flag_;
    KEL.typ.nint _name__ssn__dob__match_;
    KEL.typ.ndataset(E_Person.Offenses_Layout) Offenses_;
    KEL.typ.ndataset(E_Person.Address_Layout) Address_;
    KEL.typ.int Address_Count_ := 0;
    KEL.typ.int Address_Invalid30_Count_ := 0;
    KEL.typ.int Address_Invalid30_Flag_ := 0;
    KEL.typ.int Address_Is_Cmra30_Count_ := 0;
    KEL.typ.int Address_Is_Cmra30_Flag_ := 0;
    KEL.typ.int Address_Is_Of_State30_Flag_ := 0;
    KEL.typ.int Address_Is_Out_Of_State30_Count_ := 0;
    KEL.typ.nint Age_;
    KEL.typ.nint Age_At_Last_Event_;
    KEL.typ.int All_Death_Prior_To_All_Events_ := 0;
    KEL.typ.int All_Deceased_Event_Count_ := 0;
    KEL.typ.nfloat All_Deceased_Event_Percent_;
    KEL.typ.int All_High_Risk_Death_Prior_To_All_Events_ := 0;
    KEL.typ.nint All_Max_Deceased_To_Event_Diff_;
    KEL.typ.int Associated_With_Fraud_Offenses_Flag_ := 0;
    KEL.typ.int Associated_With_Incarcerated_Flag_ := 0;
    KEL.typ.nstr Best_City_;
    KEL.typ.nstr Best_Email_;
    KEL.typ.nstr Best_First_Name_;
    B_Person_2.__NS215277_Layout Best_Full_Name_;
    KEL.typ.nstr Best_Last_Name_;
    KEL.typ.nstr Best_Middle_Name_;
    KEL.typ.nstr Best_Name_Suffix_;
    B_Person_1.__NS427404_Layout Best_Reported_Address_;
    B_Person_1.__NS427424_Layout Best_Reported_Email_;
    B_Person_1.__NS427432_Layout Best_Reported_Ssn_;
    KEL.typ.nstr Best_Ssn_;
    KEL.typ.nstr Best_State_;
    KEL.typ.nstr Best_Street_Address_;
    KEL.typ.nstr Best_Zip_;
    KEL.typ.int Contributor_Safe_Flag_ := 0;
    KEL.typ.int Currently_Incarcerated_Flag_ := 0;
    KEL.typ.nint Customer_Id_;
    KEL.typ.nkdate Date_Of_Birth_;
    KEL.typ.int Death_Prior_To_All_Events_ := 0;
    KEL.typ.int Deceased_ := 0;
    KEL.typ.int Deceased_Dob_Match_ := 0;
    KEL.typ.int Deceased_Event_Count_ := 0;
    KEL.typ.nfloat Deceased_Event_Percent_;
    KEL.typ.int Deceased_Match_ := 0;
    KEL.typ.nstr Deceased_Match_Date_Of_Death_;
    KEL.typ.str Deceased_Match_Description_ := '';
    KEL.typ.int Deceased_Name_Match_ := 0;
    KEL.typ.nkdate Dt_First_Seen_;
    KEL.typ.nkdate Dt_Last_Seen_;
    KEL.typ.nunk Entity_Context_Uid_;
    KEL.typ.int Entity_Type_ := 0;
    KEL.typ.int Event_Count_ := 0;
    KEL.typ.int Fraud_Offense_Count_ := 0;
    KEL.typ.int Fraud_Offenses_Flag_ := 0;
    KEL.typ.int High_Risk_Death_Prior_To_All_Events_ := 0;
    KEL.typ.int Hri03_Flag_ := 0;
    KEL.typ.int Hri06_Flag_ := 0;
    KEL.typ.int Hri07_Flag_ := 0;
    KEL.typ.int Hri08_Flag_ := 0;
    KEL.typ.int Hri11_Flag_ := 0;
    KEL.typ.int Hri12_Flag_ := 0;
    KEL.typ.int Hri14_Active30_Flag_ := 0;
    KEL.typ.int Hri14_Flag_ := 0;
    KEL.typ.int Hri15_Flag_ := 0;
    KEL.typ.int Hri19_Flag_ := 0;
    KEL.typ.int Hri25_Flag_ := 0;
    KEL.typ.int Hri26_Flag_ := 0;
    KEL.typ.int Hri27_Flag_ := 0;
    KEL.typ.int Hri28_Flag_ := 0;
    KEL.typ.int Hri29_Flag_ := 0;
    KEL.typ.int Hri30_Flag_ := 0;
    KEL.typ.int Hri31_Flag_ := 0;
    KEL.typ.int Hri37_Flag_ := 0;
    KEL.typ.int Hri38_Flag_ := 0;
    KEL.typ.int Hri41_Flag_ := 0;
    KEL.typ.int Hri48_Flag_ := 0;
    KEL.typ.int Hri50_Active30_Flag_ := 0;
    KEL.typ.int Hri50_Flag_ := 0;
    KEL.typ.int Hri51_Flag_ := 0;
    KEL.typ.int Hri52_Flag_ := 0;
    KEL.typ.int Hri71_Flag_ := 0;
    KEL.typ.int Hri83_Flag_ := 0;
    KEL.typ.int Hri90_Flag_ := 0;
    KEL.typ.int Hri_Cl_Flag_ := 0;
    KEL.typ.int Hri_Co_Flag_ := 0;
    KEL.typ.int Hri_Dd_Flag_ := 0;
    KEL.typ.int Hri_Df_Flag_ := 0;
    KEL.typ.int Hri_It_Flag_ := 0;
    KEL.typ.int Hri_Iv_Flag_ := 0;
    KEL.typ.int Hri_Mi_Flag_ := 0;
    KEL.typ.int Hri_Mo_Flag_ := 0;
    KEL.typ.int Hri_Ms_Flag_ := 0;
    KEL.typ.int Hri_Nf_Flag_ := 0;
    KEL.typ.int Hri_Pa_Flag_ := 0;
    KEL.typ.int Hri_Po_Flag_ := 0;
    KEL.typ.int Hri_Va_Flag_ := 0;
    KEL.typ.nint Id_Ssn_Identity_Count_Max_;
    KEL.typ.int In_Customer_Population_ := 0;
    KEL.typ.nint Industry_Type_;
    KEL.typ.int Ip_High_Risk_City_ := 0;
    KEL.typ.int Ip_Hosted_ := 0;
    KEL.typ.int Ip_Not_Us_ := 0;
    KEL.typ.int Ip_Tor_ := 0;
    KEL.typ.int Ip_Vpn_ := 0;
    KEL.typ.int Is_Minor_ := 0;
    KEL.typ.int Kr_Event_After_Known_Risk_Count_ := 0;
    KEL.typ.int Kr_Event_After_Known_Risk_Flag_ := 0;
    KEL.typ.int Kr_Event_After_Last_Known_Risk_Count_ := 0;
    KEL.typ.int Kr_Event_After_Last_Known_Risk_Flag_ := 0;
    KEL.typ.int Kr_Flag_ := 0;
    KEL.typ.int Kr_Has_Known_Risk_Element_Count_ := 0;
    KEL.typ.nint Kr_Has_Known_Risk_Element_Flag_;
    KEL.typ.int Kr_High_Risk_Flag_ := 0;
    KEL.typ.int Kr_High_Risk_Routing_ := 0;
    KEL.typ.int Kr_Id10000_Flag_ := 0;
    KEL.typ.int Kr_Id10001_Flag_ := 0;
    KEL.typ.int Kr_Id10002_Flag_ := 0;
    KEL.typ.int Kr_Id10003_Flag_ := 0;
    KEL.typ.int Kr_Id10004_Flag_ := 0;
    KEL.typ.int Kr_Id10005_Flag_ := 0;
    KEL.typ.int Kr_Id10006_Flag_ := 0;
    KEL.typ.int Kr_Id10007_Flag_ := 0;
    KEL.typ.int Kr_Id1010_Flag_ := 0;
    KEL.typ.int Kr_Id11000_Flag_ := 0;
    KEL.typ.int Kr_Id11001_Flag_ := 0;
    KEL.typ.int Kr_Id11002_Flag_ := 0;
    KEL.typ.int Kr_Id11003_Flag_ := 0;
    KEL.typ.int Kr_Id11004_Flag_ := 0;
    KEL.typ.int Kr_Id11005_Flag_ := 0;
    KEL.typ.int Kr_Id11006_Flag_ := 0;
    KEL.typ.int Kr_Id11007_Flag_ := 0;
    KEL.typ.int Kr_Id11008_Flag_ := 0;
    KEL.typ.int Kr_Id11009_Flag_ := 0;
    KEL.typ.int Kr_Id11010_Flag_ := 0;
    KEL.typ.int Kr_Id11011_Flag_ := 0;
    KEL.typ.int Kr_Id11012_Flag_ := 0;
    KEL.typ.int Kr_Id11013_Flag_ := 0;
    KEL.typ.int Kr_Id11014_Flag_ := 0;
    KEL.typ.int Kr_Id11015_Flag_ := 0;
    KEL.typ.int Kr_Id11016_Flag_ := 0;
    KEL.typ.int Kr_Id11017_Flag_ := 0;
    KEL.typ.int Kr_Id11018_Flag_ := 0;
    KEL.typ.int Kr_Id11019_Flag_ := 0;
    KEL.typ.int Kr_Id12000_Flag_ := 0;
    KEL.typ.int Kr_Id12001_Flag_ := 0;
    KEL.typ.int Kr_Id12002_Flag_ := 0;
    KEL.typ.int Kr_Id12003_Flag_ := 0;
    KEL.typ.int Kr_Id12004_Flag_ := 0;
    KEL.typ.int Kr_Id12006_Flag_ := 0;
    KEL.typ.int Kr_Id12007_Flag_ := 0;
    KEL.typ.int Kr_Id13000_Flag_ := 0;
    KEL.typ.int Kr_Id13001_Flag_ := 0;
    KEL.typ.int Kr_Id13002_Flag_ := 0;
    KEL.typ.int Kr_Id13003_Flag_ := 0;
    KEL.typ.int Kr_Id13005_Flag_ := 0;
    KEL.typ.int Kr_Id13006_Flag_ := 0;
    KEL.typ.int Kr_Id13007_Flag_ := 0;
    KEL.typ.int Kr_Id14000_Flag_ := 0;
    KEL.typ.int Kr_Id14001_Flag_ := 0;
    KEL.typ.int Kr_Id14900_Flag_ := 0;
    KEL.typ.int Kr_Id14901_Flag_ := 0;
    KEL.typ.int Kr_Id14902_Flag_ := 0;
    KEL.typ.int Kr_Id14903_Flag_ := 0;
    KEL.typ.int Kr_Id2025_Flag_ := 0;
    KEL.typ.nkdate Kr_Last_Element_Event_Date_;
    KEL.typ.nkdate Kr_Last_Event_Date_;
    KEL.typ.int Kr_Low_Risk_Flag_ := 0;
    KEL.typ.int Kr_Medium_Risk_Flag_ := 0;
    KEL.typ.int Kr_Not_Known_Risk_Has_Known_Risk_Element_Flag_ := 0;
    KEL.typ.nstr Label_;
    KEL.typ.nkdate Last_Event_Date_;
    KEL.typ.nint Last_Record_Id_;
    KEL.typ.nint Max_Deceased_To_Event_Diff_;
    KEL.typ.int Nap3_Flag_ := 0;
    KEL.typ.int Nas9_Flag_ := 0;
    KEL.typ.int No_Lex_Id_ := 0;
    KEL.typ.int No_Lex_Id_Gt22_ := 0;
    KEL.typ.int Other_Customer_Person_Customer_Count_ := 0;
    KEL.typ.int Safe_Flag_ := 0;
    KEL.typ.nkdate Safe_Flag_Last_Date_;
    KEL.typ.int Safe_Flag_Transaction_Count_ := 0;
    KEL.typ.nint Source_Customer_Count_;
    KEL.typ.int Validation_Addr_Problems30_Count_ := 0;
    KEL.typ.int Validation_Addr_Problems30_Flag_ := 0;
    KEL.typ.int Vl_Event1_All_Count_ := 0;
    KEL.typ.int Vl_Event1_Count_ := 0;
    KEL.typ.int Vl_Event30_Active_Flag_ := 0;
    KEL.typ.int Vl_Event30_All_Active_Flag_ := 0;
    KEL.typ.int Vl_Event30_All_Day_Count_ := 0;
    KEL.typ.int Vl_Event30_Count_ := 0;
    KEL.typ.int Vl_Event365_All_Day_Count_ := 0;
    KEL.typ.int Vl_Event365_Count_ := 0;
    KEL.typ.int Vl_Event7_Active_Flag_ := 0;
    KEL.typ.int Vl_Event7_All_Active_Flag_ := 0;
    KEL.typ.int Vl_Event7_All_Count_ := 0;
    KEL.typ.int Vl_Event7_Count_ := 0;
    KEL.typ.nuid U_I_D__1_;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer__2_;
    KEL.typ.nint Lex_Id__1_;
    KEL.typ.ndataset(E_Person.Source_Customers_Layout) Source_Customers__1_;
    KEL.typ.ndataset(E_Person.Reported_Date_Of_Birth_Layout) Reported_Date_Of_Birth__1_;
    KEL.typ.ndataset(E_Person.Reported_Ssn_Layout) Reported_Ssn__1_;
    KEL.typ.ndataset(E_Person.Reported_Email_Address_Layout) Reported_Email_Address__1_;
    KEL.typ.ndataset(E_Person.Full_Name_Layout) Full_Name__1_;
    KEL.typ.nkdate Deceased_Date__1_;
    KEL.typ.nkdate Deceased_Date_Of_Birth__1_;
    KEL.typ.nstr Deceased_First__1_;
    KEL.typ.nstr Deceased_Middle__1_;
    KEL.typ.nstr Deceased_Last__1_;
    KEL.typ.nstr Deceased_Match_Code__1_;
    KEL.typ.nbool _isdeepdive__1_;
    KEL.typ.nstr _county__death__1_;
    KEL.typ.nstr Deceased_Ssn__1_;
    KEL.typ.nstr _state__death__flag__1_;
    KEL.typ.nstr _death__rec__src__1_;
    KEL.typ.nstr _state__death__id__1_;
    KEL.typ.nstr _verfirst__1_;
    KEL.typ.nstr _verlast__1_;
    KEL.typ.nstr _veraddr__1_;
    KEL.typ.nstr _vercity__1_;
    KEL.typ.nstr _verstate__1_;
    KEL.typ.nstr _verzip__1_;
    KEL.typ.nstr _verzip4__1_;
    KEL.typ.nstr _verssn__1_;
    KEL.typ.nstr _verdob__1_;
    KEL.typ.nstr _verhphone__1_;
    KEL.typ.nstr _verify__addr__1_;
    KEL.typ.nstr _verify__dob__1_;
    KEL.typ.nstr _valid__ssn__1_;
    KEL.typ.nint _nas__summary__1_;
    KEL.typ.nint _nap__summary__1_;
    KEL.typ.nstr _cvi__1_;
    KEL.typ.nstr _additional__fname__4_;
    KEL.typ.nstr _additional__lname__4_;
    KEL.typ.nstr _additional__lname__date__last__4_;
    KEL.typ.nstr _additional__fname__5_;
    KEL.typ.nstr _additional__lname__5_;
    KEL.typ.nstr _additional__lname__date__last__5_;
    KEL.typ.nstr _additional__fname__6_;
    KEL.typ.nstr _additional__lname__6_;
    KEL.typ.nstr _additional__lname__date__last__6_;
    KEL.typ.nint _subjectssncount__1_;
    KEL.typ.nstr _dobmatchlevel__1_;
    KEL.typ.nbool _ssnfoundforlexid__1_;
    KEL.typ.nstr _cvicustomscore__1_;
    KEL.typ.nint Subject_Ssn_Count__1_;
    KEL.typ.nint Date_Of_Birth_Match_Level__1_;
    KEL.typ.nint Stolen_Identity_Index__1_;
    KEL.typ.nint Synthetic_Identity_Index__1_;
    KEL.typ.nint Manipulated_Identity_Index__1_;
    KEL.typ.nint Vulnerable_Victim_Index__1_;
    KEL.typ.nint Friendlyfraud_Index__1_;
    KEL.typ.nint Suspicious_Activity_Index__1_;
    KEL.typ.nint _v2__sourcerisklevel__1_;
    KEL.typ.nint _v2__assocsuspicousidentitiescount__1_;
    KEL.typ.nint _v2__assoccreditbureauonlycount__1_;
    KEL.typ.nint _v2__validationaddrproblems__1_;
    KEL.typ.nint _v2__inputaddrageoldest__1_;
    KEL.typ.nstr _v2__inputaddrdwelltype__1_;
    KEL.typ.nstr _v2__divssnidentitycountnew__1_;
    KEL.typ.nstr _curr__incar__flag__1_;
    KEL.typ.nint _name__ssn__dob__match__1_;
    KEL.typ.ndataset(E_Person.Offenses_Layout) Offenses__1_;
    KEL.typ.ndataset(E_Person.Address_Layout) Address__1_;
    KEL.typ.int Address_Count__1_ := 0;
    KEL.typ.int Address_Invalid30_Count__1_ := 0;
    KEL.typ.int Address_Invalid30_Flag__1_ := 0;
    KEL.typ.int Address_Is_Cmra30_Count__1_ := 0;
    KEL.typ.int Address_Is_Cmra30_Flag__1_ := 0;
    KEL.typ.int Address_Is_Of_State30_Flag__1_ := 0;
    KEL.typ.int Address_Is_Out_Of_State30_Count__1_ := 0;
    KEL.typ.nint Age__1_;
    KEL.typ.nint Age_At_Last_Event__1_;
    KEL.typ.int All_Death_Prior_To_All_Events__1_ := 0;
    KEL.typ.int All_Deceased_Event_Count__1_ := 0;
    KEL.typ.nfloat All_Deceased_Event_Percent__1_;
    KEL.typ.int All_High_Risk_Death_Prior_To_All_Events__1_ := 0;
    KEL.typ.nint All_Max_Deceased_To_Event_Diff__1_;
    KEL.typ.int Associated_With_Fraud_Offenses_Flag__1_ := 0;
    KEL.typ.int Associated_With_Incarcerated_Flag__1_ := 0;
    KEL.typ.nstr Best_City__1_;
    KEL.typ.nstr Best_Email__1_;
    KEL.typ.nstr Best_First_Name__1_;
    B_Person_2.__NS215277_Layout Best_Full_Name__1_;
    KEL.typ.nstr Best_Last_Name__1_;
    KEL.typ.nstr Best_Middle_Name__1_;
    KEL.typ.nstr Best_Name_Suffix__1_;
    B_Person_1.__NS427404_Layout Best_Reported_Address__1_;
    B_Person_1.__NS427424_Layout Best_Reported_Email__1_;
    B_Person_1.__NS427432_Layout Best_Reported_Ssn__1_;
    KEL.typ.nstr Best_Ssn__1_;
    KEL.typ.nstr Best_State__1_;
    KEL.typ.nstr Best_Street_Address__1_;
    KEL.typ.nstr Best_Zip__1_;
    KEL.typ.int Contributor_Safe_Flag__1_ := 0;
    KEL.typ.int Currently_Incarcerated_Flag__1_ := 0;
    KEL.typ.nint Customer_Id__1_;
    KEL.typ.nkdate Date_Of_Birth__1_;
    KEL.typ.int Death_Prior_To_All_Events__1_ := 0;
    KEL.typ.int Deceased__1_ := 0;
    KEL.typ.int Deceased_Dob_Match__1_ := 0;
    KEL.typ.int Deceased_Event_Count__1_ := 0;
    KEL.typ.nfloat Deceased_Event_Percent__1_;
    KEL.typ.int Deceased_Match__1_ := 0;
    KEL.typ.nstr Deceased_Match_Date_Of_Death__1_;
    KEL.typ.str Deceased_Match_Description__1_ := '';
    KEL.typ.int Deceased_Name_Match__1_ := 0;
    KEL.typ.nkdate Dt_First_Seen__1_;
    KEL.typ.nkdate Dt_Last_Seen__1_;
    KEL.typ.nunk Entity_Context_Uid__1_;
    KEL.typ.int Entity_Type__1_ := 0;
    KEL.typ.int Event_Count__1_ := 0;
    KEL.typ.int Fraud_Offense_Count__1_ := 0;
    KEL.typ.int Fraud_Offenses_Flag__1_ := 0;
    KEL.typ.int High_Risk_Death_Prior_To_All_Events__1_ := 0;
    KEL.typ.int Hri03_Flag__1_ := 0;
    KEL.typ.int Hri06_Flag__1_ := 0;
    KEL.typ.int Hri07_Flag__1_ := 0;
    KEL.typ.int Hri08_Flag__1_ := 0;
    KEL.typ.int Hri11_Flag__1_ := 0;
    KEL.typ.int Hri12_Flag__1_ := 0;
    KEL.typ.int Hri14_Active30_Flag__1_ := 0;
    KEL.typ.int Hri14_Flag__1_ := 0;
    KEL.typ.int Hri15_Flag__1_ := 0;
    KEL.typ.int Hri19_Flag__1_ := 0;
    KEL.typ.int Hri25_Flag__1_ := 0;
    KEL.typ.int Hri26_Flag__1_ := 0;
    KEL.typ.int Hri27_Flag__1_ := 0;
    KEL.typ.int Hri28_Flag__1_ := 0;
    KEL.typ.int Hri29_Flag__1_ := 0;
    KEL.typ.int Hri30_Flag__1_ := 0;
    KEL.typ.int Hri31_Flag__1_ := 0;
    KEL.typ.int Hri37_Flag__1_ := 0;
    KEL.typ.int Hri38_Flag__1_ := 0;
    KEL.typ.int Hri41_Flag__1_ := 0;
    KEL.typ.int Hri48_Flag__1_ := 0;
    KEL.typ.int Hri50_Active30_Flag__1_ := 0;
    KEL.typ.int Hri50_Flag__1_ := 0;
    KEL.typ.int Hri51_Flag__1_ := 0;
    KEL.typ.int Hri52_Flag__1_ := 0;
    KEL.typ.int Hri71_Flag__1_ := 0;
    KEL.typ.int Hri83_Flag__1_ := 0;
    KEL.typ.int Hri90_Flag__1_ := 0;
    KEL.typ.int Hri_Cl_Flag__1_ := 0;
    KEL.typ.int Hri_Co_Flag__1_ := 0;
    KEL.typ.int Hri_Dd_Flag__1_ := 0;
    KEL.typ.int Hri_Df_Flag__1_ := 0;
    KEL.typ.int Hri_It_Flag__1_ := 0;
    KEL.typ.int Hri_Iv_Flag__1_ := 0;
    KEL.typ.int Hri_Mi_Flag__1_ := 0;
    KEL.typ.int Hri_Mo_Flag__1_ := 0;
    KEL.typ.int Hri_Ms_Flag__1_ := 0;
    KEL.typ.int Hri_Nf_Flag__1_ := 0;
    KEL.typ.int Hri_Pa_Flag__1_ := 0;
    KEL.typ.int Hri_Po_Flag__1_ := 0;
    KEL.typ.int Hri_Va_Flag__1_ := 0;
    KEL.typ.nint Id_Ssn_Identity_Count_Max__1_;
    KEL.typ.int In_Customer_Population__1_ := 0;
    KEL.typ.nint Industry_Type__1_;
    KEL.typ.int Ip_High_Risk_City__1_ := 0;
    KEL.typ.int Ip_Hosted__1_ := 0;
    KEL.typ.int Ip_Not_Us__1_ := 0;
    KEL.typ.int Ip_Tor__1_ := 0;
    KEL.typ.int Ip_Vpn__1_ := 0;
    KEL.typ.int Is_Minor__1_ := 0;
    KEL.typ.int Kr_Event_After_Known_Risk_Count__1_ := 0;
    KEL.typ.int Kr_Event_After_Known_Risk_Flag__1_ := 0;
    KEL.typ.int Kr_Event_After_Last_Known_Risk_Count__1_ := 0;
    KEL.typ.int Kr_Event_After_Last_Known_Risk_Flag__1_ := 0;
    KEL.typ.int Kr_Flag__1_ := 0;
    KEL.typ.int Kr_Has_Known_Risk_Element_Count__1_ := 0;
    KEL.typ.nint Kr_Has_Known_Risk_Element_Flag__1_;
    KEL.typ.int Kr_High_Risk_Flag__1_ := 0;
    KEL.typ.int Kr_High_Risk_Routing__1_ := 0;
    KEL.typ.int Kr_Id10000_Flag__1_ := 0;
    KEL.typ.int Kr_Id10001_Flag__1_ := 0;
    KEL.typ.int Kr_Id10002_Flag__1_ := 0;
    KEL.typ.int Kr_Id10003_Flag__1_ := 0;
    KEL.typ.int Kr_Id10004_Flag__1_ := 0;
    KEL.typ.int Kr_Id10005_Flag__1_ := 0;
    KEL.typ.int Kr_Id10006_Flag__1_ := 0;
    KEL.typ.int Kr_Id10007_Flag__1_ := 0;
    KEL.typ.int Kr_Id1010_Flag__1_ := 0;
    KEL.typ.int Kr_Id11000_Flag__1_ := 0;
    KEL.typ.int Kr_Id11001_Flag__1_ := 0;
    KEL.typ.int Kr_Id11002_Flag__1_ := 0;
    KEL.typ.int Kr_Id11003_Flag__1_ := 0;
    KEL.typ.int Kr_Id11004_Flag__1_ := 0;
    KEL.typ.int Kr_Id11005_Flag__1_ := 0;
    KEL.typ.int Kr_Id11006_Flag__1_ := 0;
    KEL.typ.int Kr_Id11007_Flag__1_ := 0;
    KEL.typ.int Kr_Id11008_Flag__1_ := 0;
    KEL.typ.int Kr_Id11009_Flag__1_ := 0;
    KEL.typ.int Kr_Id11010_Flag__1_ := 0;
    KEL.typ.int Kr_Id11011_Flag__1_ := 0;
    KEL.typ.int Kr_Id11012_Flag__1_ := 0;
    KEL.typ.int Kr_Id11013_Flag__1_ := 0;
    KEL.typ.int Kr_Id11014_Flag__1_ := 0;
    KEL.typ.int Kr_Id11015_Flag__1_ := 0;
    KEL.typ.int Kr_Id11016_Flag__1_ := 0;
    KEL.typ.int Kr_Id11017_Flag__1_ := 0;
    KEL.typ.int Kr_Id11018_Flag__1_ := 0;
    KEL.typ.int Kr_Id11019_Flag__1_ := 0;
    KEL.typ.int Kr_Id12000_Flag__1_ := 0;
    KEL.typ.int Kr_Id12001_Flag__1_ := 0;
    KEL.typ.int Kr_Id12002_Flag__1_ := 0;
    KEL.typ.int Kr_Id12003_Flag__1_ := 0;
    KEL.typ.int Kr_Id12004_Flag__1_ := 0;
    KEL.typ.int Kr_Id12006_Flag__1_ := 0;
    KEL.typ.int Kr_Id12007_Flag__1_ := 0;
    KEL.typ.int Kr_Id13000_Flag__1_ := 0;
    KEL.typ.int Kr_Id13001_Flag__1_ := 0;
    KEL.typ.int Kr_Id13002_Flag__1_ := 0;
    KEL.typ.int Kr_Id13003_Flag__1_ := 0;
    KEL.typ.int Kr_Id13005_Flag__1_ := 0;
    KEL.typ.int Kr_Id13006_Flag__1_ := 0;
    KEL.typ.int Kr_Id13007_Flag__1_ := 0;
    KEL.typ.int Kr_Id14000_Flag__1_ := 0;
    KEL.typ.int Kr_Id14001_Flag__1_ := 0;
    KEL.typ.int Kr_Id14900_Flag__1_ := 0;
    KEL.typ.int Kr_Id14901_Flag__1_ := 0;
    KEL.typ.int Kr_Id14902_Flag__1_ := 0;
    KEL.typ.int Kr_Id14903_Flag__1_ := 0;
    KEL.typ.int Kr_Id2025_Flag__1_ := 0;
    KEL.typ.nkdate Kr_Last_Element_Event_Date__1_;
    KEL.typ.nkdate Kr_Last_Event_Date__1_;
    KEL.typ.int Kr_Low_Risk_Flag__1_ := 0;
    KEL.typ.int Kr_Medium_Risk_Flag__1_ := 0;
    KEL.typ.int Kr_Not_Known_Risk_Has_Known_Risk_Element_Flag__1_ := 0;
    KEL.typ.nstr Label__1_;
    KEL.typ.nkdate Last_Event_Date__1_;
    KEL.typ.nint Last_Record_Id__1_;
    KEL.typ.nint Max_Deceased_To_Event_Diff__1_;
    KEL.typ.int Nap3_Flag__1_ := 0;
    KEL.typ.int Nas9_Flag__1_ := 0;
    KEL.typ.int No_Lex_Id__1_ := 0;
    KEL.typ.int No_Lex_Id_Gt22__1_ := 0;
    KEL.typ.int Other_Customer_Person_Customer_Count__1_ := 0;
    KEL.typ.int Safe_Flag__1_ := 0;
    KEL.typ.nkdate Safe_Flag_Last_Date__1_;
    KEL.typ.int Safe_Flag_Transaction_Count__1_ := 0;
    KEL.typ.nint Source_Customer_Count__1_;
    KEL.typ.int Validation_Addr_Problems30_Count__1_ := 0;
    KEL.typ.int Validation_Addr_Problems30_Flag__1_ := 0;
    KEL.typ.int Vl_Event1_All_Count__1_ := 0;
    KEL.typ.int Vl_Event1_Count__1_ := 0;
    KEL.typ.int Vl_Event30_Active_Flag__1_ := 0;
    KEL.typ.int Vl_Event30_All_Active_Flag__1_ := 0;
    KEL.typ.int Vl_Event30_All_Day_Count__1_ := 0;
    KEL.typ.int Vl_Event30_Count__1_ := 0;
    KEL.typ.int Vl_Event365_All_Day_Count__1_ := 0;
    KEL.typ.int Vl_Event365_Count__1_ := 0;
    KEL.typ.int Vl_Event7_Active_Flag__1_ := 0;
    KEL.typ.int Vl_Event7_All_Active_Flag__1_ := 0;
    KEL.typ.int Vl_Event7_All_Count__1_ := 0;
    KEL.typ.int Vl_Event7_Count__1_ := 0;
    KEL.typ.nuid U_I_D__2_;
    KEL.typ.nint Customer_Id__2_;
    KEL.typ.nint Industry_Type__2_;
    KEL.typ.ndataset(E_Customer.States_Layout) States_;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.int __RecordCount := 0;
  END;
  __JC1438568(__ST1346809_Layout __EE1437389, E_Customer.Layout __EE1438552) := __EEQP(__EE1437389._r_Customer_,__EE1438552.UID);
  __ST1348045_Layout __JT1438568(__ST1346809_Layout __l, E_Customer.Layout __r) := TRANSFORM
    SELF.U_I_D__2_ := __r.UID;
    SELF.Customer_Id__2_ := __r.Customer_Id_;
    SELF.Industry_Type__2_ := __r.Industry_Type_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE1439235 := JOIN(__EE1437389,__EE1438552,__JC1438568(LEFT,RIGHT),__JT1438568(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __ST1346323_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) Source_Customer_;
    KEL.typ.nint Customer_Id_;
    KEL.typ.nint Industry_Type_;
    KEL.typ.nunk Tree_Uid_;
    KEL.typ.nunk Entity_Context_Uid_;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST1346323_Layout __ND1439893__Project(__ST1348045_Layout __PP1439236) := TRANSFORM
    SELF.Source_Customer_ := __PP1439236._r_Customer_;
    SELF.Customer_Id_ := __PP1439236.Customer_Id__2_;
    SELF.Industry_Type_ := __PP1439236.Industry_Type__2_;
    SELF.Tree_Uid_ := __PP1439236.Entity_Context_Uid__1_;
    SELF := __PP1439236;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE1439235,__ND1439893__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_},Source_Customer_,Customer_Id_,Industry_Type_,Tree_Uid_,Entity_Context_Uid_,MERGE),__ST1346323_Layout));
END;

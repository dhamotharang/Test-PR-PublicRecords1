...
export File_GongVendorIDs := module
	...
	{string VendorName, string LSSiVendorID, string VDecimal, string Vendor, string File_Name, string File_Type, string Install});
	...
	export vendor_translations := dataset([
			...
			{'0xfa','Lssihi',false}], {string vendor, string vendorname, boolean isDuplicateCo});
			
import ut;


export fn_master_vendor_counts(dataset(Gong_v2.layout_gongMaster) master = gong_v2.File_GongMaster, string newest_date = ut.GetDate) := function

					Assign_VoIPs_To_Translations := join(vendor_translations, 
																							 voips,
																								left.vendor = right.vendor, 
																								transform({recordof(vendor_translations), recordof(voips) - vendor},
																															self.vendor := max([left.vendor, right.vendor]),
																															self.vendorname := max([left.vendorname, right.vendorname]),
																															self := left, 
					...
					currentMonth := ut.GetDate[1..6];
					pastMonth := if(ut.GetDate[5..6] = '01', 
																(string)((unsigned)ut.GetDate[1..4] - 1)+ '12' + (string)((unsigned)ut.GetDate[7..8] + 1), 
																ut.GetDate[1..4] + (string)intformat(((unsigned)ut.GetDate[5..6] - 1), 2, 1) + (string)((unsigned)ut.GetDate[7..8] + 1)); 
					past2Year := (string)((unsigned)ut.GetDate[1..4] - 2) +  ut.GetDate[5..8];

					Filter_TrimMaster := join(
																distribute(dedup(sort(project(master(filedate between past2Year and currentDate), trimMaster_layout),record),record), random()), 
					...
					SubtractMonths(unsigned howmany) := (string)intformat((if((unsigned)ut.GetDate[5..6] - howmany = 0, 12 - (unsigned)ut.GetDate[5..6], (unsigned)ut.GetDate[5..6] - howmany)), 2, 1);
					...
					calcKeepMonths(integer pMonths) := function // 2 years only
								...
								gm := (string)intformat(map(mb - (integer)ut.GetDate[5..6] = 0 => 12,
														mb - (integer)ut.GetDate[5..6] < 0 => (integer)ut.GetDate[5..6] - mb,
														mb - (integer)ut.GetDate[5..6] = 12 => 12,
														mb - (integer)ut.GetDate[5..6] < 12 => 12 - (mb - (integer)ut.GetDate[5..6]),
														12 - ((mb - (integer)ut.GetDate[5..6]) - 12)), 2, 1);


								gy := (string)map((integer)ut.GetDate[5..6] - mb > 0 => (integer)ut.GetDate[1..4],
													(integer)ut.GetDate[5..6] - mb > -12 => (integer)ut.GetDate[1..4] - 1,
													(integer)ut.GetDate[1..4] - 2);

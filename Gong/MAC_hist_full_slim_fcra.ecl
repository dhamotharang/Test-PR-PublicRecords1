...
%in_slim_rec% := record
//      in_file.bell_id;					// need to generate source code for fcra keys
			string2		src;
//      in_file.filedate;
...
#uniquename(in_slim)
%in_slim% := project(in_file, TRANSFORM(%in_slim_rec%,
							self.src := if(LEFT.bell_id='NEU',mdr.sourceTools.src_Gong_Neustar,mdr.sourceTools.src_Gong_History);
							self := left;));

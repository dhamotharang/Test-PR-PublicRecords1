...
export fnPropagateADLs(
			dataset(gong.layout_historyaid) gonghistory = dataset('~thor_data400::base::gong_history',layout_historyaid, flat, __compressed__), 
			intmax_nm_match_score = 2, // higher match score allows lenient matches
			dataset( recordof(Watchdog.Layout_Gong_DID)) gongbase = Gong.File_Gong_DID) 
		:= module
...
shared flaggedGdPh := /* flags phones as good for matching (either all 10 digits or last 7 and did populated) */
	project(gonghistory,
		transform({recordof(gonghistory), unsigned6 new_did := 0, unsigned cnt := 1, string2 flag, string60 matchnm := ''}, 
...
shared distHist := distribute(flaggedGdPh(name_first <> '' and name_last <> '' and flag <> '00'), hash(phone10[4..10])); 
...
shared seqDID := table(dedup(srtDID, phone10, flag, name_last, name_first, name_middle, did, local), 
				{phone10, flag, name_last, name_first, name_middle, did, cntDID := count(group), new_did, pDID, cnt, matchnm}, 
					phone10, flag, name_last, name_first, name_middle, local)(cntDID = 1
									);	
...
joinBack := join(distHist(did = 0), rollup_matches(pdid <> ''),
					...
					transform(recordof(gonghistory),

...
export mac_add_disc_cnt(cnt_in_f,update_date,cnt_out_f) := macro
...
%history_dst% := distribute(%history_slim%, hash(phone10, prim_range, predir, prim_name, 
			    			                   ...
			    			                   p_city_name, st, z5, z4, listed_name));

%history_srt% := sort(%history_dst%, phone10, prim_range, predir, prim_name, suffix,
                                     postdir, unit_desig, sec_range, p_city_name,
			   			       st, z5, z4, listed_name, current_record_flag, 
						       deletion_date, local);
							
%history_grp% := group(%history_srt%, phone10, prim_range, predir, prim_name, suffix,
                                      postdir, unit_desig, sec_range, p_city_name,
			   			        st, z5, z4, listed_name, local);

#uniquename(m_6)
#uniquename(m_12)
#uniquename(m_18)							
%m_6% := if((integer)(update_date[5..6])<7, 
                     (string4)((integer)(update_date[1..4])-1) + 
	     	      intformat((integer)(update_date[5..6])+6,2,1) + 
			      update_date[7..8],
				 update_date[1..4] + 
				 intformat((integer)(update_date[5..6])-6,2,1) + 
				 update_date[7..8]);
				 
%m_12% := (string4)((integer)(update_date[1..4])-1) + update_date[5..8];

...
export out_STRATA_population_stats(string version = ut.getdate) := function

ds := Gong.File_GongBase;

rPopulationStats_File_GongBase
 :=
  record
    ...
    phoneno_CountNonBlank                              := sum(group,if(ds.phoneno<>'',1,0));
...
tStats := table(ds,rPopulationStats_File_GongBase,st,few);

strata.createXMLStats(tStats,'Gong','data',version,'',resultsOut);

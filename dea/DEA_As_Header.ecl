...
DEA_file := dea_as_source;

Header.Layout_New_Records Translate_DEA_to_Header(DEA_file l) := transform
	self.did := 0;
	self.rid := 0;
	self.pflag1 := '';
	self.pflag2 := '';
...
DEA_to_header := project(DEA_file,Translate_DEA_to_Header(left));
...
export DEA_as_header := DEA_dedup;
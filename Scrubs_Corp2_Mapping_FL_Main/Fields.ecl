...
EXPORT SALT32.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_charter','invalid_mandatory','invalid_date','invalid_optional_date','invalid_fein','invalid_corp_vendor','invalid_state_origin','invalid_for_profit_ind','invalid_forgn_dom_code','invalid_corp_status_cd','invalid_corp_orig_org_structure_cd','invalid_alphablank');
EXPORT FieldTypeNum(SALT32.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_charter' => 2,'invalid_mandatory' => 3,'invalid_date' => 4,'invalid_optional_date' => 5,'invalid_fein' => 6,'invalid_corp_vendor' => 7,'invalid_state_origin' => 8,'invalid_for_profit_ind' => 9,'invalid_forgn_dom_code' => 10,'invalid_corp_status_cd' => 11,'invalid_corp_orig_org_structure_cd' => 12,'invalid_alphablank' => 13,0);
...
EXPORT MakeFT_invalid_fein(SALT32.StrType s0) := FUNCTION
  s1 := SALT32.stringfilter(s0,'0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_fein(SALT32.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT32.StringFilter(s,'0123456789'))),~(LENGTH(TRIM(s)) = 0 OR LENGTH(TRIM(s)) = 9));
EXPORT InValidMessageFT_invalid_fein(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInChars('0123456789'),SALT32.HygieneErrors.NotLength('0,9'),SALT32.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_corp_status_cd(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_status_cd(SALT32.StrType s) := WHICH(((SALT32.StrType) s) NOT IN ['A','I',' ']);
EXPORT InValidMessageFT_invalid_corp_status_cd(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInEnum('A|I| '),SALT32.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_corp_orig_org_structure_cd(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_orig_org_structure_cd(SALT32.StrType s) := WHICH(((SALT32.StrType) s) NOT IN ['AGENT','DOMLP','DOMNP','DOMP','FLAL','FORL','FORLP','FORNP','FORP','NPREG','TRUST',' ']);
EXPORT InValidMessageFT_invalid_corp_orig_org_structure_cd(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInEnum('AGENT|DOMLP|DOMNP|DOMP|FLAL|FORL|FORLP|FORNP|FORP|NPREG|TRUST| '),SALT32.HygieneErrors.Good);
...
EXPORT FieldNum(SALT32.StrType fn) := CASE(fn,'corp_key' => 0,'corp_orig_sos_charter_nbr' => 1,'corp_legal_name' => 2,'dt_first_seen' => 3,'dt_last_seen' => 4,'corp_process_date' => 5,'dt_vendor_first_reported' => 6,'dt_vendor_last_reported' => 7,'corp_forgn_date' => 8,'corp_inc_date' => 9,'corp_merger_date' => 10,'corp_merger_effective_date' => 11,'corp_ra_dt_first_seen' => 12,'corp_ra_dt_last_seen' => 13,'corp_ra_resign_date' => 14,'corp_status_date' => 15,'corp_vendor' => 16,'corp_state_origin' => 17,'corp_inc_state' => 18,'corp_for_profit_ind' => 19,'corp_foreign_domestic_ind' => 20,'corp_status_cd' => 21,'corp_orig_org_structure_cd' => 22,'corp_fed_tax_id' => 23,'corp_forgn_state_desc' => 24,0);
...
EXPORT Make_corp_fed_tax_id(SALT32.StrType s0) := MakeFT_invalid_fein(s0);
EXPORT InValid_corp_fed_tax_id(SALT32.StrType s) := InValidFT_invalid_fein(s);
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_corp_key,1,0)+ IF( SELF.Diff_corp_orig_sos_charter_nbr,1,0)+ IF( SELF.Diff_corp_legal_name,1,0)+ IF( SELF.Diff_dt_first_seen,1,0)+ IF( SELF.Diff_dt_last_seen,1,0)+ IF( SELF.Diff_corp_process_date,1,0)+ IF( SELF.Diff_dt_vendor_first_reported,1,0)+ IF( SELF.Diff_dt_vendor_last_reported,1,0)+ IF( SELF.Diff_corp_forgn_date,1,0)+ IF( SELF.Diff_corp_inc_date,1,0)+ IF( SELF.Diff_corp_merger_date,1,0)+ IF( SELF.Diff_corp_merger_effective_date,1,0)+ IF( SELF.Diff_corp_ra_dt_first_seen,1,0)+ IF( SELF.Diff_corp_ra_dt_last_seen,1,0)+ IF( SELF.Diff_corp_ra_resign_date,1,0)+ IF( SELF.Diff_corp_status_date,1,0)+ IF( SELF.Diff_corp_vendor,1,0)+ IF( SELF.Diff_corp_state_origin,1,0)+ IF( SELF.Diff_corp_inc_state,1,0)+ IF( SELF.Diff_corp_for_profit_ind,1,0)+ IF( SELF.Diff_corp_foreign_domestic_ind,1,0)+ IF( SELF.Diff_corp_status_cd,1,0)+ IF( SELF.Diff_corp_orig_org_structure_cd,1,0)+ IF( SELF.Diff_corp_fed_tax_id,1,0)+ IF( SELF.Diff_corp_forgn_state_desc,1,0);
  END;

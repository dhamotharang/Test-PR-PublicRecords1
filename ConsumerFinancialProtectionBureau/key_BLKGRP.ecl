...
export key_BLKGRP(filedate, pUseProd = false, isfcra = false, seq_build, seq_move_built)  := macro
    RoxieKeyBuild.Mac_SK_BuildProcess_v2_local($.key_BLKGRP_ID(filedate,pUseProd,isfcra), $.Filenames(,pUseProd,isfcra).keyBLKGRP, $.Filenames(filedate,pUseProd,isfcra).keyBLKGRP, seq_build);

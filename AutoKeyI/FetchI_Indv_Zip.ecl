	...
	export old := module
		export params := module
			export base := interface
				...
				export set of string20 lname_set_value_20;
				...
				export boolean SearchAroundAddress_value;
			end;
		...
		export do(params.full in_mod) := function
			...
			i := autokey.Key_Zip(in_mod.autokey_keyname_root);
		  ...
		  // enable 'starts with' matching on the input fname *and* safeguard against invalid subrange exception
			...
			f_raw := i(in_mod.zip_value<>[] AND (in_mod.pname_value='' OR in_mod.addr_error_value OR in_mod.SearchAroundAddress_value) AND 
						...
						keyed(dph_lname=metaphonelib.DMetaPhone1(in_mod.lname_value)[1..6] OR (in_mod.lname_value='' and in_mod.workHard)),
						keyed((lname in in_mod.lname_set_value_20 OR ((in_mod.phonetics OR in_mod.lname_value='') and in_mod.workHard))),
	...
	export new := module
		export params := module
			export base := interface(
				...
				AutoStandardI.InterfaceTranslator.lname_set_value_20.params,

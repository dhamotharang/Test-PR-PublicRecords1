...
export FetchI_Indv_Name := module
	export old := module
		...
		export do(params.full in_mod) := function
			...
			i := AutoKey.key_name(in_mod.autokey_keyname_root);
		  ...
		  // enable 'starts with' matching on the input fname *and* safeguard against invalid subrange exception
			...
			f := 
				 project(
						 i(in_mod.did_value = '' and in_mod.lname_value != '' AND in_mod.state_value = '' AND 
						   in_mod.city_value = '' AND in_mod.zip_value = [] AND in_mod.can_poscode_value = '' AND in_mod.phone_value = '' AND 
							 length(trim(in_mod.ssn_value)) <= 4,
						keyed(dph_lname=metaphonelib.DMetaPhone1(in_mod.lname_value)[1..6]),
						keyed((lname in in_mod.lname_set_value_20 OR (in_mod.phonetics and in_mod.workHard))),

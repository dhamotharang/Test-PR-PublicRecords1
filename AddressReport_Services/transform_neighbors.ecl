IMPORT doxie,iesp,ut,gong,CriminalRecords_Services, AutoStandardI;
export transform_neighbors(dataset(doxie.layout_nbr_records) neighbors,
	...
	integer max_nbr_phones := if(param.LocationReport, iesp.Constants.BR.MaxAddress_Phones, 1);
	...
	iesp.bpsreport.t_NeighborAddressSlim SetNeighborsAddresses (nbr_phone_layout L, dataset (nbr_phone_layout) R) := transform
		...
		phones			:= if(param.locationReport, phones_loc, phones0);
		...
		residents				:= if(param.locationReport, residents_loc, residents0);
	...
	nbr_phone_layout addgong (nbr l, gong_key r):=transform
	...
	nbr_Phone_tmp0:=join(nbr, gong_key,
	...
	doxie.MAC_Get_GLB_DPPA_PhonesPlus(nbr_pp_in, nbr_pp_out, true,,
																		param.GLBPurpose, param.DPPAPurpose, param.IndustryClass,if(param.locationReport,0,11),
																		'',,false,param.DataRestrictionMask, false, true);
	...
	nbr_Phone_tmp := if(param.locationReport, nbr_Phone_tmp_loc, nbr_Phone_tmp0);

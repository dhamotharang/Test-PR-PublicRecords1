...
import AutoStandardI,PersonReports,iesp,doxie,AddressReport_Services, lib_stringlib;
 ...
 boolean OverrideInclude := false : stored ('OverrideInclude');
  ...
  string1 bus_report_fetch_level		:= 'S' : stored('BusinessReportFetchLevel'); 
	...
	tempmod 	:= module (project (tmp,AddressReport_Services.input._addressreport,  opt))
		  export string6 ssn_mask 											:= 'NONE' : stored('SSNMask');
	    ...
	    export boolean include_businesses 						:= include_stored.include_businesses;
			...
			export boolean glb_ok  												:= AutoStandardI.InterfaceTranslator.glb_ok.val(project(tmp,AutoStandardI.InterfaceTranslator.glb_ok.params));
			export boolean dppa_ok 												:= AutoStandardI.InterfaceTranslator.dppa_ok.val(project(tmp,AutoStandardI.InterfaceTranslator.dppa_ok.params)); 
			export boolean probationoverride 							:= AutoStandardI.InterfaceTranslator.probation_override_value.val(project(tmp,AutoStandardI.InterfaceTranslator.probation_override_value.params)); 
			export boolean no_scrub 											:= AutoStandardI.InterfaceTranslator.no_scrub.val(project(tmp,AutoStandardI.InterfaceTranslator.no_scrub.params)); 
			...
			export string32 applicationType	 							:= AutoStandardI.InterfaceTranslator.application_type_val.val(project(tmp,AutoStandardI.InterfaceTranslator.application_type_val.params));
			export boolean locationReport									:= location_report;
			export boolean useNewBusinessHeader						:= use_new_business_header;
			export string1 BusinessReportFetchLevel 			:= stringlib.stringToUpperCase(bus_report_fetch_level); // overkill?
  end;

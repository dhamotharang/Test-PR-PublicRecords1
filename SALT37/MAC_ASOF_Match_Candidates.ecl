...
However, if effectivedates have been defined in the file (in_dt_usable_first,in_dt_usable_last), the 
in_dt_usable_first is set to in_dt_seen_first, and in_dt_usable_last is not changed.
...
this was when this rid was assigned that did). Set the in_dt_seen_first to the co_date (in the case of a match). NO NEW RECORDS 
in this step.
...
and the dt_first_usable to the earliest date on the record (in_dt_seen_first). This generates NEW records.

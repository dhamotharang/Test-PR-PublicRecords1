EXPORT MOD_Profile(inFile,id_field='',src_field='',CorrelateSampleSize=100000000,out_prefix='') := FUNCTIONMACRO
	...
	#FOR(inFileFields)
		#FOR(field)
			...
			#ELSEIF(%recLevel% = 0)
				...
				#APPEND(transformClauses, 'TRIM((SALT37.StrType)le.' + %'@name'% + ')')
				...
				#APPEND(summaryFields, 'maxlength_'+%'@name'%+' := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)'+#TEXT(inFile)+'.'+%'@name'%+')));')
				#APPEND(summaryFields, 'avelength_'+%'@name'%+' := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)'+#TEXT(inFile)+'.'+%'@name'%+')),'+#TEXT(inFile)+'.'+%'@name'%+'<>(typeof('+#TEXT(inFile)+'.'+%'@name'%+'))\'\');')
	...
	%M% := MODULE
		...
		SALT37.MAC_Character_Counts.X_Data_Layout Into(RECORDOF(inFile) le, UNSIGNED c) := TRANSFORM
		...
		SHARED FldIds := DATASET([%datasetRecords%], SALT37.MAC_Character_Counts.Field_Identification);
		...
		// Field Correlations
		SALT37.MAC_Correlate.Data_Layout IntoP(inFile le, UNSIGNED C) := TRANSFORM
		...
		// Cluster counts
		#IF(#TEXT(id_field)<>'')
			EXPORT ClusterCounts := SALT37.MOD_ClusterStats.Counts(inFile,id_field);
			EXPORT out_ClusterCounts := OUTPUT(ClusterCounts,NAMED(pfx+'ClusterCounts'),ALL);
			#APPEND(outCondition, ',out_ClusterCounts')
			#IF(#TEXT(src_field)<>'')
				EXPORT ClusterSrc := SALT37.MOD_ClusterStats.Sources(inFile,id_field,src_field);

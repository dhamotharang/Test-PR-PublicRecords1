...
#else
			MAP(n<=1 => sep+SALT37.GetRangeOfWords(s,n+1,wc,sep),
			    n=wc => SALT37.GetRangeOfWords(s,1,wc-1,sep)+sep,
					n>wc => SALT37.GetRangeOfWords(s,1,wc,sep),
					SALT37.GetRangeOfWords(s,1,n-1,sep)+sep+sep+SALT37.GetRangeOfWords(s,n+1,wc,sep));
#end

...
EXPORT Records := MODULE
  ...
  EXPORT PersonSummary(PublicProfileServices.IParam.searchParams rptByMod_new) := FUNCTION
    ...
    iesp.public_profile_report.t_PublicProfileIndividual setIndividual() := TRANSFORM
      ...
      SELF.AKAs := CHOOSEN(PROJECT(GLOBAL(PersonReports.Person_records(dids,mod_access,PROJECT(glbMod,PersonReports.IParam.personal,OPT)).AKAs),iesp.public_profile_report.t_PublicProfileIdentity),iesp.Constants.BR.MaxAKA);
      SELF.DriverLicenses := CHOOSEN(GLOBAL(PersonReports.DL_records(dids)),iesp.Constants.DL.MaxCountDL);
      bankruptcy_mod := module (project(rptByMod_new, PersonReports.IParam.bankruptcy, opt)) end;
      SELF.Bankruptcies := CHOOSEN(GLOBAL(PersonReports.bankruptcy_records(dids, mod_access, bankruptcy_mod).bankruptcy_v2),iesp.Constants.BR.MaxBankruptcies);
      ...
      faaCert_mod := module (project(rptByMod_new, PersonReports.IParam.faacerts)) end;
      ...
      proflic_mod := project(rptByMod_new, PersonReports.IParam.proflic, opt);
      SELF.ProfessionalLicenses := CHOOSEN(GLOBAL(PersonReports.proflic_records(dids, proflic_mod).proflicenses_v2),iesp.Constants.BR.MaxProfLicenses);
      ...
      dea_mod := module (project(rptByMod_new, PersonReports.IParam.dea)) end;
      SELF.ControlledSubstances := CHOOSEN(GLOBAL(PersonReports.DEA_records(dids, dea_mod)),iesp.Constants.BR.MaxDEASubstances);
      voters_mod := module (project(rptByMod_new, PersonReports.IParam.voters)) end;
      ...
      ccw_mod := module (project(rptByMod_new, PersonReports.IParam.ccw)) end;
      ...
      watercrafts_mod := module (PROJECT(rptByMod_new, PersonReports.Iparam.watercrafts, OPT)) end;

...
// create median assessed to sales ratio per year, fips, land use and bin
	...
	assessed_props := group(sort(f(assessed_value_to_saleprice_ratio1<>0), assessed_value_year,fips_code,land_use_code,assessed_total_value, local), assessed_value_year, fips_code, land_use_code, local);
	...
	grouped_assess_with_total := group(sort(w_assess_totals(assess_recs_total_count > 50), assessed_value_year,fips_code,land_use_code,assessed_total_value, local), assessed_value_year, fips_code, land_use_code, local);
...
// create median assessed to sales ratio per year, fips, land use and bin
	...
	// group all the properties by tax year, fips, land use and put that grouped result into layout_ta_calcs to iterate through
	market_props := group(sort(f(market_value_to_saleprice_ratio1<>0), assessed_value_year,fips_code,land_use_code,market_total_value, local), assessed_value_year, fips_code, land_use_code, local);
	...
	grouped_market_with_total := group(sort(w_market_totals(market_recs_total_count > 50), assessed_value_year,fips_code,land_use_code,market_total_value, local), assessed_value_year, fips_code, land_use_code, local);

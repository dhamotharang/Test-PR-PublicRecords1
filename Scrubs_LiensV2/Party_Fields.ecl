...
EXPORT SALT33.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'number','alpha','invalid_alpha','invalid_only_alpha','invalid_zip','invalid_date','invalid_phone','invalid_ssn','invalid_name_suffix','invalid_name');
EXPORT FieldTypeNum(SALT33.StrType fn) := CASE(fn,'number' => 1,'alpha' => 2,'invalid_alpha' => 3,'invalid_only_alpha' => 4,'invalid_zip' => 5,'invalid_date' => 6,'invalid_phone' => 7,'invalid_ssn' => 8,'invalid_name_suffix' => 9,'invalid_name' => 10,0);
...
EXPORT MakeFT_alpha(SALT33.StrType s0) := FUNCTION
  s1 := SALT33.stringfilter(s0,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefqhijklmnopqrstuvwxyz -,.'); // Only allow valid symbols
  s2 := SALT33.stringcleanspaces( SALT33.stringsubstituteout(s1,' -,.',' ') ); // Insert spaces but avoid doubles
  s3 := if ( SALT33.StringFind('"\'',s2[1],1)>0 and SALT33.StringFind('"\'',s2[LENGTH(TRIM(s2))],1)>0,s2[2..LENGTH(TRIM(s2))-1],s2 );// Remove quotes if required
  RETURN  s3;
END;
EXPORT InValidFT_alpha(SALT33.StrType s) := WHICH(SALT33.StringFind('"\'',s[1],1)<>0 and SALT33.StringFind('"\'',s[LENGTH(TRIM(s))],1)<>0,LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefqhijklmnopqrstuvwxyz -,.'))));
EXPORT InValidMessageFT_alpha(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.Inquotes('"\''),SALT33.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefqhijklmnopqrstuvwxyz -,.'),SALT33.HygieneErrors.Good);
EXPORT MakeFT_invalid_alpha(SALT33.StrType s0) := FUNCTION
  ...
  RETURN  s2;
END;
EXPORT InValidFT_invalid_alpha(SALT33.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 <>{}[]-^=!+&,.;/#()_"\''))));
EXPORT InValidMessageFT_invalid_alpha(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 <>{}[]-^=!+&,.;/#()_"\''),SALT33.HygieneErrors.Good);
EXPORT MakeFT_invalid_only_alpha(SALT33.StrType s0) := FUNCTION
  s1 := SALT33.stringfilter(s0,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz -,.'); // Only allow valid symbols
  s2 := SALT33.stringcleanspaces( SALT33.stringsubstituteout(s1,' -,.',' ') ); // Insert spaces but avoid doubles
  RETURN  s2;
END;
EXPORT InValidFT_invalid_only_alpha(SALT33.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz -,.'))));
EXPORT InValidMessageFT_invalid_only_alpha(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz -,.'),SALT33.HygieneErrors.Good);
...
END;
EXPORT InValidFT_invalid_zip(SALT33.StrType s) := WHICH(SALT33.StringFind('"\'',s[1],1)<>0 and SALT33.StringFind('"\'',s[LENGTH(TRIM(s))],1)<>0,LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'0123456789-'))),~(LENGTH(TRIM(s)) = 0 OR LENGTH(TRIM(s)) = 5 OR LENGTH(TRIM(s)) = 9 OR LENGTH(TRIM(s)) = 10));
EXPORT InValidMessageFT_invalid_zip(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.Inquotes('"\''),SALT33.HygieneErrors.NotInChars('0123456789-'),SALT33.HygieneErrors.NotLength('0,5,9,10'),SALT33.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_phone(SALT33.StrType s0) := FUNCTION
  ...
  RETURN  MakeFT_number(s2);
END;
EXPORT InValidFT_invalid_phone(SALT33.StrType s) := WHICH(SALT33.StringFind('"\'',s[1],1)<>0 and SALT33.StringFind('"\'',s[LENGTH(TRIM(s))],1)<>0,LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'0123456789'))),~(LENGTH(TRIM(s)) = 0 OR LENGTH(TRIM(s)) = 7 OR LENGTH(TRIM(s)) = 10));
EXPORT InValidMessageFT_invalid_phone(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.Inquotes('"\''),SALT33.HygieneErrors.NotInChars('0123456789'),SALT33.HygieneErrors.NotLength('0,7,10'),SALT33.HygieneErrors.Good);
EXPORT MakeFT_invalid_ssn(SALT33.StrType s0) := FUNCTION
  s1 := SALT33.stringfilter(s0,'0123456789Xx*'); // Only allow valid symbols
  s2 := SALT33.stringcleanspaces( SALT33.stringsubstituteout(s1,'*',' ') ); // Insert spaces but avoid doubles
  RETURN  s2;
END;
EXPORT InValidFT_invalid_ssn(SALT33.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'0123456789Xx*'))));
EXPORT InValidMessageFT_invalid_ssn(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInChars('0123456789Xx*'),SALT33.HygieneErrors.Good);
EXPORT MakeFT_invalid_name_suffix(SALT33.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_name_suffix(SALT33.StrType s) := WHICH(((SALT33.StrType) s) NOT IN ['JR','SR','I','II','III','IV','V','VI','VII','VIII','IX','X','XI','XII','XIII','XVII',' ']);
EXPORT InValidMessageFT_invalid_name_suffix(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInEnum('JR|SR|I|II|III|IV|V|VI|VII|VIII|IX|X|XI|XII|XIII|XVII| '),SALT33.HygieneErrors.Good);
EXPORT MakeFT_invalid_name(SALT33.StrType s0) := FUNCTION
  ...
  RETURN  s2;
END;
EXPORT InValidFT_invalid_name(SALT33.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789\' -,.&()'))),~(LENGTH(TRIM(s)) = 0 OR LENGTH(TRIM(s)) >= 3));
EXPORT InValidMessageFT_invalid_name(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789\' -,.&()'),SALT33.HygieneErrors.NotLength('0,3..'),SALT33.HygieneErrors.Good);
...
EXPORT FieldNum(SALT33.StrType fn) := CASE(fn,'tmsid' => 0,'rmsid' => 1,'orig_rmsid' => 2,'orig_full_debtorname' => 3,'orig_name' => 4,'orig_lname' => 5,'orig_fname' => 6,'orig_mname' => 7,'orig_suffix' => 8,'tax_id' => 9,'ssn' => 10,'title' => 11,'fname' => 12,'mname' => 13,'lname' => 14,'name_suffix' => 15,'name_score' => 16,'cname' => 17,'orig_address1' => 18,'orig_address2' => 19,'orig_city' => 20,'orig_state' => 21,'orig_zip5' => 22,'orig_zip4' => 23,'orig_county' => 24,'orig_country' => 25,'prim_range' => 26,'predir' => 27,'prim_name' => 28,'addr_suffix' => 29,'postdir' => 30,'unit_desig' => 31,'sec_range' => 32,'p_city_name' => 33,'v_city_name' => 34,'st' => 35,'zip' => 36,'zip4' => 37,'cart' => 38,'cr_sort_sz' => 39,'lot' => 40,'lot_order' => 41,'dbpc' => 42,'chk_digit' => 43,'rec_type' => 44,'county' => 45,'geo_lat' => 46,'geo_long' => 47,'msa' => 48,'geo_blk' => 49,'geo_match' => 50,'err_stat' => 51,'phone' => 52,'name_type' => 53,'did' => 54,'bdid' => 55,'date_first_seen' => 56,'date_last_seen' => 57,'date_vendor_first_reported' => 58,'date_vendor_last_reported' => 59,'persistent_record_id' => 60,'source_file' => 61,0);
...
EXPORT Make_orig_full_debtorname(SALT33.StrType s0) := MakeFT_invalid_alpha(s0);
EXPORT InValid_orig_full_debtorname(SALT33.StrType s) := InValidFT_invalid_alpha(s);
EXPORT InValidMessage_orig_full_debtorname(UNSIGNED1 wh) := InValidMessageFT_invalid_alpha(wh);
EXPORT Make_orig_name(SALT33.StrType s0) := MakeFT_invalid_alpha(s0);
EXPORT InValid_orig_name(SALT33.StrType s) := InValidFT_invalid_alpha(s);
...
EXPORT Make_orig_suffix(SALT33.StrType s0) := MakeFT_invalid_name_suffix(s0);
EXPORT InValid_orig_suffix(SALT33.StrType s) := InValidFT_invalid_name_suffix(s);
EXPORT InValidMessage_orig_suffix(UNSIGNED1 wh) := InValidMessageFT_invalid_name_suffix(wh);
EXPORT Make_tax_id(SALT33.StrType s0) := MakeFT_invalid_ssn(s0);
EXPORT InValid_tax_id(SALT33.StrType s) := InValidFT_invalid_ssn(s);
...
EXPORT Make_name_suffix(SALT33.StrType s0) := MakeFT_invalid_name_suffix(s0);
EXPORT InValid_name_suffix(SALT33.StrType s) := InValidFT_invalid_name_suffix(s);
...
EXPORT Make_orig_address1(SALT33.StrType s0) := MakeFT_invalid_alpha(s0);
EXPORT InValid_orig_address1(SALT33.StrType s) := InValidFT_invalid_alpha(s);
EXPORT InValidMessage_orig_address1(UNSIGNED1 wh) := InValidMessageFT_invalid_alpha(wh);
EXPORT Make_orig_address2(SALT33.StrType s0) := MakeFT_invalid_alpha(s0);
EXPORT InValid_orig_address2(SALT33.StrType s) := InValidFT_invalid_alpha(s);
EXPORT InValidMessage_orig_address2(UNSIGNED1 wh) := InValidMessageFT_invalid_alpha(wh);
EXPORT Make_orig_city(SALT33.StrType s0) := MakeFT_invalid_only_alpha(s0);
EXPORT InValid_orig_city(SALT33.StrType s) := InValidFT_invalid_only_alpha(s);
EXPORT InValidMessage_orig_city(UNSIGNED1 wh) := InValidMessageFT_invalid_only_alpha(wh);
EXPORT Make_orig_state(SALT33.StrType s0) := MakeFT_invalid_only_alpha(s0);
EXPORT InValid_orig_state(SALT33.StrType s) := InValidFT_invalid_only_alpha(s);
EXPORT InValidMessage_orig_state(UNSIGNED1 wh) := InValidMessageFT_invalid_only_alpha(wh);
EXPORT Make_orig_zip5(SALT33.StrType s0) := MakeFT_invalid_zip(s0);
EXPORT InValid_orig_zip5(SALT33.StrType s) := InValidFT_invalid_zip(s);
...
EXPORT Make_orig_county(SALT33.StrType s0) := MakeFT_invalid_only_alpha(s0);
EXPORT InValid_orig_county(SALT33.StrType s) := InValidFT_invalid_only_alpha(s);
EXPORT InValidMessage_orig_county(UNSIGNED1 wh) := InValidMessageFT_invalid_only_alpha(wh);
EXPORT Make_orig_country(SALT33.StrType s0) := MakeFT_invalid_only_alpha(s0);
EXPORT InValid_orig_country(SALT33.StrType s) := InValidFT_invalid_only_alpha(s);

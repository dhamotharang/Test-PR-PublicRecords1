...
Incoming_transaction 	:= NOTHOR(fileservices.RemoteDirectory(_control.IPAddress.bctlpedata10,'/data/riskview/sao/in/transaction/','*',)(~REGEXFIND('batch', name, nocase) AND ~REGEXFIND('delta', name, nocase)));//changed
Incoming_intermediate := NOTHOR(fileservices.RemoteDirectory(_control.IPAddress.bctlpedata10,'/data/riskview/sao/in/intermediate/','*',)(~REGEXFIND('batch', name, nocase) AND ~REGEXFIND('delta', name, nocase)));//changed	 
...
spray_intermediate := fileservices.sprayvariable(_control.IPAddress.bctlpedata10, '/data/riskview/sao/in/intermediate/'+filename,500000000 ,'~~',	'~~ENDOFRECORD~~','',	'thor400_44', 	'~thor_data400::in::score_logs::'+wu+'.'+filename, ,,,	true, true, true); //changed
...
movefile_intermediate := FileServices.MoveExternalFile(_control.IPAddress.bctlpedata10,'/data/riskview/sao/in/intermediate/'+filename,'/data/riskview/out/'+wu+'.'+filename);//changed
...
spray_transaction   := fileservices.sprayvariable(_control.IPAddress.bctlpedata10, '/data/riskview/sao/in/transaction/'+filename, 500000000 ,'~~',	'~~^~~','',	'thor400_44', 	'~thor_data400::in::score_logs::'+wu+'.'+filename, ,,,	true, true, true); //changed
...
addsuper := fileservices.addsuperfile('~thor_data400::in::score_tracking::'+ext, '~thor_data400::in::score_logs::'+wu+'.'+filename);
movefile_transaction   := FileServices.MoveExternalFile(_control.IPAddress.bctlpedata10,'/data/riskview/sao/in/transaction/'+filename,'/data/riskview/out/'+wu+'.'+filename);//changed

...
EXPORT SALT34.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_future_date','invalid_charter_nbr','invalid_numericblank','invalid_numericperiodblank','invalid_alphablank');
EXPORT FieldTypeNum(SALT34.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_corp_vendor' => 2,'invalid_state_origin' => 3,'invalid_mandatory' => 4,'invalid_date' => 5,'invalid_future_date' => 6,'invalid_charter_nbr' => 7,'invalid_numericblank' => 8,'invalid_numericperiodblank' => 9,'invalid_alphablank' => 10,0);
...
EXPORT MakeFT_invalid_corp_vendor(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_vendor(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['25']);
EXPORT InValidMessageFT_invalid_corp_vendor(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('25'),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_state_origin(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_state_origin(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['MA']);
EXPORT InValidMessageFT_invalid_state_origin(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('MA'),SALT34.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_numericperiodblank(SALT34.StrType s0) := FUNCTION
  s1 := SALT34.stringfilter(s0,'.0123456789 '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_numericperiodblank(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,'.0123456789 '))));
EXPORT InValidMessageFT_invalid_numericperiodblank(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars('.0123456789 '),SALT34.HygieneErrors.Good);
...
EXPORT Make_stock_class(SALT34.StrType s0) := MakeFT_invalid_alphablank(s0);
EXPORT InValid_stock_class(SALT34.StrType s) := InValidFT_invalid_alphablank(s);
EXPORT InValidMessage_stock_class(UNSIGNED1 wh) := InValidMessageFT_invalid_alphablank(wh);
 
EXPORT Make_stock_shares_issued(SALT34.StrType s0) := MakeFT_invalid_numericblank(s0);
EXPORT InValid_stock_shares_issued(SALT34.StrType s) := InValidFT_invalid_numericblank(s);
EXPORT InValidMessage_stock_shares_issued(UNSIGNED1 wh) := InValidMessageFT_invalid_numericblank(wh);
 
EXPORT Make_stock_authorized_nbr(SALT34.StrType s0) := MakeFT_invalid_numericblank(s0);
EXPORT InValid_stock_authorized_nbr(SALT34.StrType s) := InValidFT_invalid_numericblank(s);
EXPORT InValidMessage_stock_authorized_nbr(UNSIGNED1 wh) := InValidMessageFT_invalid_numericblank(wh);
 
EXPORT Make_stock_par_value(SALT34.StrType s0) := MakeFT_invalid_numericperiodblank(s0);
EXPORT InValid_stock_par_value(SALT34.StrType s) := InValidFT_invalid_numericperiodblank(s);
EXPORT InValidMessage_stock_par_value(UNSIGNED1 wh) := InValidMessageFT_invalid_numericperiodblank(wh);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT34,Scrubs_Corp2_Mapping_MA_Stock;

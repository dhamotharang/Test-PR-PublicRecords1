...
// The module to handle the case where no scrubs exist
  ...
  EXPORT Expanded_Layout := RECORD(Lien_Layout_Vendor_Src)
  ...
  END;
  EXPORT  Bitmap_Layout := RECORD(Lien_Layout_Vendor_Src)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(Lien_Layout_Vendor_Src) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.lncourtcode_Invalid := Lien_Fields.InValid_lncourtcode((SALT311.StrType)le.lncourtcode);
    SELF.court_code_Invalid := Lien_Fields.InValid_court_code((SALT311.StrType)le.court_code);
    SELF.court_name_Invalid := Lien_Fields.InValid_court_name((SALT311.StrType)le.court_name);
    SELF.address1_Invalid := Lien_Fields.InValid_address1((SALT311.StrType)le.address1);
    SELF.address2_Invalid := Lien_Fields.InValid_address2((SALT311.StrType)le.address2);
    SELF.city_Invalid := Lien_Fields.InValid_city((SALT311.StrType)le.city);
    SELF.state_Invalid := Lien_Fields.InValid_state((SALT311.StrType)le.state);
    SELF.zip_Invalid := Lien_Fields.InValid_zip((SALT311.StrType)le.zip);
    SELF.phone_Invalid := Lien_Fields.InValid_phone((SALT311.StrType)le.phone);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Lien_Layout_Vendor_Src);
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,Lien_Layout_Vendor_Src);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    AnyRule_WithErrorsCount := COUNT(GROUP, h.lncourtcode_Invalid > 0 OR h.court_code_Invalid > 0 OR h.court_name_Invalid > 0 OR h.address1_Invalid > 0 OR h.address2_Invalid > 0 OR h.city_Invalid > 0 OR h.state_Invalid > 0 OR h.zip_Invalid > 0 OR h.phone_Invalid > 0);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.lncourtcode_ALLOW_ErrorCount > 0, 1, 0) + IF(le.court_code_ALLOW_ErrorCount > 0, 1, 0) + IF(le.court_name_ALLOW_ErrorCount > 0, 1, 0) + IF(le.address1_ALLOW_ErrorCount > 0, 1, 0) + IF(le.address2_ALLOW_ErrorCount > 0, 1, 0) + IF(le.city_ALLOW_ErrorCount > 0, 1, 0) + IF(le.state_ALLOW_ErrorCount > 0, 1, 0) + IF(le.zip_ALLOW_ErrorCount > 0, 1, 0) + IF(le.phone_ALLOW_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.lncourtcode_ALLOW_ErrorCount > 0, 1, 0) + IF(le.court_code_ALLOW_ErrorCount > 0, 1, 0) + IF(le.court_name_ALLOW_ErrorCount > 0, 1, 0) + IF(le.address1_ALLOW_ErrorCount > 0, 1, 0) + IF(le.address2_ALLOW_ErrorCount > 0, 1, 0) + IF(le.city_ALLOW_ErrorCount > 0, 1, 0) + IF(le.state_ALLOW_ErrorCount > 0, 1, 0) + IF(le.zip_ALLOW_ErrorCount > 0, 1, 0) + IF(le.phone_ALLOW_ErrorCount > 0, 1, 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.lncourtcode_Invalid,le.court_code_Invalid,le.court_name_Invalid,le.address1_Invalid,le.address2_Invalid,le.city_Invalid,le.state_Invalid,le.zip_Invalid,le.phone_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Lien_Fields.InvalidMessage_lncourtcode(le.lncourtcode_Invalid),Lien_Fields.InvalidMessage_court_code(le.court_code_Invalid),Lien_Fields.InvalidMessage_court_name(le.court_name_Invalid),Lien_Fields.InvalidMessage_address1(le.address1_Invalid),Lien_Fields.InvalidMessage_address2(le.address2_Invalid),Lien_Fields.InvalidMessage_city(le.city_Invalid),Lien_Fields.InvalidMessage_state(le.state_Invalid),Lien_Fields.InvalidMessage_zip(le.zip_Invalid),Lien_Fields.InvalidMessage_phone(le.phone_Invalid),'UNKNOWN'));
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(Lien_Layout_Vendor_Src) prevDS = DATASET([], Lien_Layout_Vendor_Src), STRING10 Src='UNK'):= FUNCTION
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ,Lien_Fields.InvalidMessage_lncourtcode(1)
          ,Lien_Fields.InvalidMessage_court_code(1)
          ,Lien_Fields.InvalidMessage_court_name(1)
          ,Lien_Fields.InvalidMessage_address1(1)
          ,Lien_Fields.InvalidMessage_address2(1)
          ,Lien_Fields.InvalidMessage_city(1)
          ,Lien_Fields.InvalidMessage_state(1)
          ,Lien_Fields.InvalidMessage_zip(1)
          ,Lien_Fields.InvalidMessage_phone(1)
          ,'Fields with errors'
    ...
    mod_hygiene := Lien_hygiene(PROJECT(h, Lien_Layout_Vendor_Src));
  ...
  // record count stats
    ...
    mod_Delta := Lien_Delta(prevDS, PROJECT(h, Lien_Layout_Vendor_Src));
...
EXPORT StandardStats(DATASET(Lien_Layout_Vendor_Src) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  myTimeStamp := (UNSIGNED6)SALT311.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
  expandedFile := FromNone(inFile).ExpandedInfile;
  mod_fromexpandedOverall := FromExpanded(expandedFile);
  scrubsSummaryOverall := mod_fromexpandedOverall.SummaryStats;
 
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Vendor_Src, Lien_Fields, 'RECORDOF(scrubsSummaryOverall)', '');

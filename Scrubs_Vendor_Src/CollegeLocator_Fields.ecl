IMPORT SALT311;
EXPORT CollegeLocator_Fields := MODULE
...
EXPORT MakeFT_lnfilecategory(SALT311.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_lnfilecategory(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'()0123456789 ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz'))));
EXPORT InValidMessageFT_lnfilecategory(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('()0123456789 ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_lnsourcetcode(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'0123456789O'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_lnsourcetcode(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789O'))));
EXPORT InValidMessageFT_lnsourcetcode(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789O'),SALT311.HygieneErrors.Good);
...
EXPORT MakeFT_vendorname(SALT311.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_vendorname(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,' &-ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'))));
EXPORT InValidMessageFT_vendorname(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars(' &-ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'),SALT311.HygieneErrors.Good);
...
EXPORT MakeFT_address2(SALT311.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_address2(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'LNU'))));
EXPORT InValidMessageFT_address2(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('LNU'),SALT311.HygieneErrors.Good);

IMPORT SALT311,STD;
EXPORT Bankruptcy_Scrubs := MODULE
...
EXPORT FromNone(DATASET(Bankruptcy_Layout_Vendor_Src) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.court_name_Invalid := Bankruptcy_Fields.InValid_court_name((SALT311.StrType)le.court_name);
    SELF.address1_Invalid := Bankruptcy_Fields.InValid_address1((SALT311.StrType)le.address1);
    SELF.address2_Invalid := Bankruptcy_Fields.InValid_address2((SALT311.StrType)le.address2);
    SELF.city_Invalid := Bankruptcy_Fields.InValid_city((SALT311.StrType)le.city);
    SELF.state_Invalid := Bankruptcy_Fields.InValid_state((SALT311.StrType)le.state);
    SELF.zip_Invalid := Bankruptcy_Fields.InValid_zip((SALT311.StrType)le.zip);
    SELF.phone_Invalid := Bankruptcy_Fields.InValid_phone((SALT311.StrType)le.phone);
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Bankruptcy_Fields.InvalidMessage_lncourtcode(le.lncourtcode_Invalid),Bankruptcy_Fields.InvalidMessage_court_code(le.court_code_Invalid),Bankruptcy_Fields.InvalidMessage_court_name(le.court_name_Invalid),Bankruptcy_Fields.InvalidMessage_address1(le.address1_Invalid),Bankruptcy_Fields.InvalidMessage_address2(le.address2_Invalid),Bankruptcy_Fields.InvalidMessage_city(le.city_Invalid),Bankruptcy_Fields.InvalidMessage_state(le.state_Invalid),Bankruptcy_Fields.InvalidMessage_zip(le.zip_Invalid),Bankruptcy_Fields.InvalidMessage_phone(le.phone_Invalid),'UNKNOWN'));
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ,Bankruptcy_Fields.InvalidMessage_lncourtcode(1)
          ,Bankruptcy_Fields.InvalidMessage_court_code(1)
          ,Bankruptcy_Fields.InvalidMessage_court_name(1)
          ,Bankruptcy_Fields.InvalidMessage_address1(1)
          ,Bankruptcy_Fields.InvalidMessage_address2(1)
          ,Bankruptcy_Fields.InvalidMessage_city(1)
          ,Bankruptcy_Fields.InvalidMessage_state(1)
          ,Bankruptcy_Fields.InvalidMessage_zip(1)
          ,Bankruptcy_Fields.InvalidMessage_phone(1)
          ,'Fields with errors'
...
EXPORT StandardStats(DATASET(Bankruptcy_Layout_Vendor_Src) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Vendor_Src, Bankruptcy_Fields, 'RECORDOF(scrubsSummaryOverall)', '');

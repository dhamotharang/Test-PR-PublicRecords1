IMPORT SALT311,STD;
EXPORT MasterList_Scrubs := MODULE
...
// The module to handle the case where no scrubs exist
  ...
  EXPORT Expanded_Layout := RECORD(MasterList_Layout_Vendor_Src)
  ...
  END;
  EXPORT  Bitmap_Layout := RECORD(MasterList_Layout_Vendor_Src)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(MasterList_Layout_Vendor_Src) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.lnfilecategory_Invalid := MasterList_Fields.InValid_lnfilecategory((SALT311.StrType)le.lnfilecategory);
    SELF.lnsourcetcode_Invalid := MasterList_Fields.InValid_lnsourcetcode((SALT311.StrType)le.lnsourcetcode);
    SELF.vendorname_Invalid := MasterList_Fields.InValid_vendorname((SALT311.StrType)le.vendorname);
    SELF.address1_Invalid := MasterList_Fields.InValid_address1((SALT311.StrType)le.address1);
    SELF.address2_Invalid := MasterList_Fields.InValid_address2((SALT311.StrType)le.address2);
    SELF.city_Invalid := MasterList_Fields.InValid_city((SALT311.StrType)le.city);
    SELF.state_Invalid := MasterList_Fields.InValid_state((SALT311.StrType)le.state);
    SELF.zipcode_Invalid := MasterList_Fields.InValid_zipcode((SALT311.StrType)le.zipcode);
    SELF.phone_Invalid := MasterList_Fields.InValid_phone((SALT311.StrType)le.phone);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),MasterList_Layout_Vendor_Src);
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,MasterList_Layout_Vendor_Src);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,MasterList_Fields.InvalidMessage_lnfilecategory(le.lnfilecategory_Invalid),MasterList_Fields.InvalidMessage_lnsourcetcode(le.lnsourcetcode_Invalid),MasterList_Fields.InvalidMessage_vendorname(le.vendorname_Invalid),MasterList_Fields.InvalidMessage_address1(le.address1_Invalid),MasterList_Fields.InvalidMessage_address2(le.address2_Invalid),MasterList_Fields.InvalidMessage_city(le.city_Invalid),MasterList_Fields.InvalidMessage_state(le.state_Invalid),MasterList_Fields.InvalidMessage_zipcode(le.zipcode_Invalid),MasterList_Fields.InvalidMessage_phone(le.phone_Invalid),'UNKNOWN'));
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(MasterList_Layout_Vendor_Src) prevDS = DATASET([], MasterList_Layout_Vendor_Src), STRING10 Src='UNK'):= FUNCTION
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ,MasterList_Fields.InvalidMessage_lnfilecategory(1)
          ,MasterList_Fields.InvalidMessage_lnsourcetcode(1)
          ,MasterList_Fields.InvalidMessage_vendorname(1)
          ,MasterList_Fields.InvalidMessage_address1(1)
          ,MasterList_Fields.InvalidMessage_address2(1)
          ,MasterList_Fields.InvalidMessage_city(1)
          ,MasterList_Fields.InvalidMessage_state(1)
          ,MasterList_Fields.InvalidMessage_zipcode(1)
          ,MasterList_Fields.InvalidMessage_phone(1)
          ,'Fields with errors'
    ...
    mod_hygiene := MasterList_hygiene(PROJECT(h, MasterList_Layout_Vendor_Src));
  ...
  // record count stats
    ...
    mod_Delta := MasterList_Delta(prevDS, PROJECT(h, MasterList_Layout_Vendor_Src));
...
EXPORT StandardStats(DATASET(MasterList_Layout_Vendor_Src) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  myTimeStamp := (UNSIGNED6)SALT311.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
  expandedFile := FromNone(inFile).ExpandedInfile;
  mod_fromexpandedOverall := FromExpanded(expandedFile);
  scrubsSummaryOverall := mod_fromexpandedOverall.SummaryStats;
 
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Vendor_Src, MasterList_Fields, 'RECORDOF(scrubsSummaryOverall)', '');

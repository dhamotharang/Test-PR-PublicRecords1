IMPORT SALT311,STD;
EXPORT Lien_hygiene(dataset(Lien_layout_Vendor_Src) h) := MODULE
...
invRec invert(summary0 le, INTEGER C) := TRANSFORM
  ...
  SELF.FieldName := CHOOSE(C,'lncourtcode','court_code','court_name','address1','address2','city','state','zip','phone');
  SELF.populated_pcnt := CHOOSE(C,le.populated_lncourtcode_pcnt,le.populated_court_code_pcnt,le.populated_court_name_pcnt,le.populated_address1_pcnt,le.populated_address2_pcnt,le.populated_city_pcnt,le.populated_state_pcnt,le.populated_zip_pcnt,le.populated_phone_pcnt);
  SELF.maxlength := CHOOSE(C,le.maxlength_lncourtcode,le.maxlength_court_code,le.maxlength_court_name,le.maxlength_address1,le.maxlength_address2,le.maxlength_city,le.maxlength_state,le.maxlength_zip,le.maxlength_phone);
  SELF.avelength := CHOOSE(C,le.avelength_lncourtcode,le.avelength_court_code,le.avelength_court_name,le.avelength_address1,le.avelength_address2,le.avelength_city,le.avelength_state,le.avelength_zip,le.avelength_phone);
END;
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    Lien_Fields.InValid_lncourtcode((SALT311.StrType)le.lncourtcode),
    Lien_Fields.InValid_court_code((SALT311.StrType)le.court_code),
    Lien_Fields.InValid_court_name((SALT311.StrType)le.court_name),
    Lien_Fields.InValid_address1((SALT311.StrType)le.address1),
    Lien_Fields.InValid_address2((SALT311.StrType)le.address2),
    Lien_Fields.InValid_city((SALT311.StrType)le.city),
    Lien_Fields.InValid_state((SALT311.StrType)le.state),
    Lien_Fields.InValid_zip((SALT311.StrType)le.zip),
    Lien_Fields.InValid_phone((SALT311.StrType)le.phone),
    0);
...
PrettyErrorTotals := RECORD
  FieldNme := Lien_Fields.FieldName(TotalErrors.FieldNum);
  FieldType := CHOOSE(TotalErrors.FieldNum,'lncourtcode','court_code','court_name','address1','address2','city','state','zip','phone');
  ErrorMessage := CHOOSE(TotalErrors.FieldNum,Lien_Fields.InValidMessage_lncourtcode(TotalErrors.ErrorNum),Lien_Fields.InValidMessage_court_code(TotalErrors.ErrorNum),Lien_Fields.InValidMessage_court_name(TotalErrors.ErrorNum),Lien_Fields.InValidMessage_address1(TotalErrors.ErrorNum),Lien_Fields.InValidMessage_address2(TotalErrors.ErrorNum),Lien_Fields.InValidMessage_city(TotalErrors.ErrorNum),Lien_Fields.InValidMessage_state(TotalErrors.ErrorNum),Lien_Fields.InValidMessage_zip(TotalErrors.ErrorNum),Lien_Fields.InValidMessage_phone(TotalErrors.ErrorNum));
  TotalErrors.Cnt;
END;
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_Vendor_Src, Lien_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

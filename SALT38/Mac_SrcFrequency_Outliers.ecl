...
EXPORT Mac_SrcFrequency_Outliers(infile,infield,srcfield,o1,o21,o22,o31,o32,o41,o42) := MACRO
  #uniquename(r)
  %r% := RECORD
	  SALT38.Str30Type fieldname := #TEXT(infield);
	  SALT38.StrType fieldvalue {MAXLENGTH(200000)} := (SALT38.StrType)infile.infield ;
		SALT38.StrType src {MAXLENGTH(30)} := infile.srcfield ;
...
EXPORT o22 := TABLE(%osu%, {SALT38.Str30Type fieldname := #TEXT(infield);SALT38.Str30Type OutlierType := 'Unique'; SALT38.StrType src := %osu%.src; REAL score := %osu%.score;});
	...
	EXPORT o32 := TABLE(%osd%,{SALT38.Str30Type fieldname := #TEXT(infield);SALT38.Str30Type OutlierType := 'Distinct'; SALT38.StrType src := %osd%.src; REAL score := %osd%.score;});
	...
	EXPORT o42 := TABLE(%ot5s%,{SALT38.Str30Type fieldname := #TEXT(infield);SALT38.Str30Type OutlierType := 'Top5'; SALT38.StrType src := %ot5s%.src; REAL score := %ot5s%.score;});
  ENDMACRO;

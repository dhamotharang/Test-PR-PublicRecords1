...
EXPORT mod_StandardStatsTransforms := MODULE
	...
	EXPORT mac_hygieneSummaryTransform(mymod, fieldsAttr, inLayout, hasSourceField, outputECLText = FALSE) := MACRO
		...
		#SET(currentField, '');
		#SET(currentFlat, '');
		#APPEND(outECL, 'SALT38.Layout_Stats_Standard.Main xSummary(' + inLayout + 'le, INTEGER cnt, UNSIGNED useTimestamp, STRING suffix = \'\', STRING statCategoryMore = \'\') := TRANSFORM\n');
	...
	EXPORT mac_hygieneTotalRecs(dsSummaryIn, useTimestamp, statnameSuffixIn, hasSourceField, statCategoryMoreIn) := FUNCTIONMACRO
		SALT38.Layout_Stats_Standard.Main xTotalRecs(dsSummaryIn le) := TRANSFORM
	...
	EXPORT hygieneAllProfiles(DATASET(SALT38.MAC_Character_Counts.ResultLine_Layout) allProfilesIn, UNSIGNED useTimestamp, UNSIGNED1 numExamples, STRING statCategoryMoreIn = '') := FUNCTION
		SALT38.Layout_Stats_Standard.Main xAllProfiles(allProfilesIn le, INTEGER cnt) := TRANSFORM
			SELF.dateTimeStamp := useTimestamp;
			SELF.wuid := WORKUNIT;
			...
			SELF.dsExamples := IF(cnt = 1, PROJECT(SORT(le.Frequent_Terms, -cnt)[1..numExamples], TRANSFORM(SALT38.Layout_Stats_Standard.Examples, SELF := LEFT)));
			SELF := [];
		END;
	...
	EXPORT mac_scrubsSummaryStatsFieldErrTransform(mymod, fieldsAttr, inLayout, srcField, outputECLText = FALSE) := MACRO
		...
		#APPEND(outECL, 'SALT38.Layout_Stats_Standard.Main xSummaryStats(' + inLayout + 'le, INTEGER cnt, UNSIGNED useTimestamp, STRING suffixIn, STRING statCategoryMoreIn = \'\') := TRANSFORM\n');
		#APPEND(outECL, '  SELF.dateTimeStamp := useTimeStamp;\n');
		#APPEND(outECL, '  SELF.wuid := WORKUNIT;\n');
		#IF(TRIM(srcField) > '')
	...
	EXPORT scrubsSummaryStatsGeneral(scrubsSummaryIn, srcField = '', useTimeStamp, suffixIn, statCategoryMoreIn) := FUNCTIONMACRO
		SALT38.Layout_Stats_Standard.Main xGeneralStats(scrubsSummaryIn le, INTEGER cnt) := TRANSFORM
			SELF.dateTimeStamp := useTimestamp;
			SELF.wuid := WORKUNIT;

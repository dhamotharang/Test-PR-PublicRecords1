...
#else
			MAP(n<=1 => sep+SALT38.GetRangeOfWords(s,n+1,wc,sep),
			    n=wc => SALT38.GetRangeOfWords(s,1,wc-1,sep)+sep,
					n>wc => SALT38.GetRangeOfWords(s,1,wc,sep),
					SALT38.GetRangeOfWords(s,1,n-1,sep)+sep+sep+SALT38.GetRangeOfWords(s,n+1,wc,sep));
#end

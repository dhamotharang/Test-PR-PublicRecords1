...
export Candidates := index(prop_file,{},{prop_file},'~temp::::tin_matching::match_candidates_debug');
ms_temp := sort(mtch,Conf,1,2,skew(1.0)); // Some headers have very skewed IDs
export MatchSample := index(ms_temp,{Conf,1,2},{mtch},'~temp::::tin_matching::match_sample_debug',sort keyed);
export Specificities_Key := index(s,{1},{s},'~temp::::tin_matching::specificities_debug');
prop_file := matches(ih).Patched_Candidates; // Available for External ADL2
export PatchedCandidates := index(prop_file,{},{prop_file},'~temp::::tin_matching::patched_candidates');
end;

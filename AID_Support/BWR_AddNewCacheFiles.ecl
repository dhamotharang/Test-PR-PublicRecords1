...
gWhichCache				:=	AID_Support.Constants.eCache.ForHeader;
	...
	lJobName				:=	AID_Support.Constants.JobName.AddNewCacheFiles;			// This is used in WU search below to confirm only instance
	...
	lJobName				:=	AID_Support.Constants.NonHeaderPrefix + AID_Support.Constants.JobName.AddNewCacheFiles;			// This is used in WU search below to confirm only instance
#else
...
zGo	:=	if(AID_Support.Common.fIsJobActive(lJobName),
					 ...
					 if(not AID_Support.Common.fValidEnvironment(),
							...
							AID_Support.ProcAddNewCacheFiles
						 )
					);

#if(gWhichCache = AID_Support.Constants.eCache.ForHeader)
	zGo : when(cron(AID_Support.Constants.CronTrigger.AddNewCacheFiles));
	zGo : when(event(AID_Support.Constants.EventTrigger.AddNewCacheFiles, '*'));
#elseif(gWhichCache = AID_Support.Constants.eCache.ForNonHeader)
	zGo : when(cron(AID_Support.Constants.CronTrigger.AddNewCacheFiles));
	zGo : when(event(AID_Support.Constants.NonHeaderPrefix + AID_Support.Constants.EventTrigger.AddNewCacheFiles, '*'));
#else
	FAIL('Which "gWhichCache" value are you using?');
#end

...
SHARED match_candidates(ih).layout_matches match_join(match_candidates(ih).layout_candidates le,match_candidates(ih).layout_candidates ri,UNSIGNED2 c,UNSIGNED outside=0) := TRANSFORM
  ...
  INTEGER2 company_name_score := IF ( le.company_name = ri.company_name or company_name_score_temp >= 1300, company_name_score_temp, SKIP ); // Enforce FORCE parameter
...
// Compute the score for the concept company_address
  ...
  iComp := (cnp_number_score + prim_range_score + prim_name_score + st_score + ebr_file_number_score + hist_enterprise_number_score + hist_domestic_corp_key_score + unk_corp_key_score + active_enterprise_number_score + active_domestic_corp_key_score + hist_duns_number_score + active_duns_number_score + company_fein_score + company_phone_score + foreign_corp_key_score + company_name_score + zip_score + company_csz_score + sec_range_score + v_city_name_score + cnp_btype_score + iscorp_score + company_addr1_score + company_address_score) / 100 + outside;
...
//Fixed fields ->:cnp_number(14):prim_range(13):prim_name(15):st(5)
...
mj0 := JOIN( dn0_deduped, dn0_deduped, LEFT.Proxid > RIGHT.Proxid AND ( LEFT.SALT_Partition = RIGHT.SALT_Partition OR LEFT.SALT_Partition='' OR RIGHT.SALT_Partition = '' ) AND LEFT.cnp_number = RIGHT.cnp_number AND LEFT.prim_range = RIGHT.prim_range AND LEFT.prim_name = RIGHT.prim_name AND LEFT.st = RIGHT.st AND ( ~left.prim_name_isnull AND ~right.prim_name_isnull ) AND ( ~left.st_isnull AND ~right.st_isnull ) AND ( left.active_enterprise_number = right.active_enterprise_number OR left.active_enterprise_number_isnull OR right.active_enterprise_number_isnull ) AND ( left.active_domestic_corp_key = right.active_domestic_corp_key OR left.active_domestic_corp_key_isnull OR right.active_domestic_corp_key_isnull ) AND ( left.active_duns_number = right.active_duns_number OR left.active_duns_number_isnull OR right.active_duns_number_isnull ) AND ( ~left.company_name_isnull AND ~right.company_name_isnull ) AND ( ~left.company_address_isnull AND ~right.company_address_isnull ),match_join(LEFT,RIGHT,0),HINT(Parallel_Match),ATMOST(LEFT.cnp_number = RIGHT.cnp_number AND LEFT.prim_range = RIGHT.prim_range AND LEFT.prim_name = RIGHT.prim_name AND LEFT.st = RIGHT.st,20000),SKEW(1));
mj1 := JOIN( dn0_deduped, dn0_deduped, LEFT.Proxid > RIGHT.Proxid AND ( LEFT.SALT_Partition = RIGHT.SALT_Partition OR LEFT.SALT_Partition='' OR RIGHT.SALT_Partition = '' ) AND LEFT.cnp_number = RIGHT.cnp_number AND LEFT.prim_range = RIGHT.prim_range AND LEFT.prim_name = RIGHT.prim_name AND LEFT.st = RIGHT.st  AND LEFT.company_name[1..4] = RIGHT.company_name[1..4] AND ( ~left.prim_name_isnull AND ~right.prim_name_isnull ) AND ( ~left.st_isnull AND ~right.st_isnull ) AND ( left.active_enterprise_number = right.active_enterprise_number OR left.active_enterprise_number_isnull OR right.active_enterprise_number_isnull ) AND ( left.active_domestic_corp_key = right.active_domestic_corp_key OR left.active_domestic_corp_key_isnull OR right.active_domestic_corp_key_isnull ) AND ( left.active_duns_number = right.active_duns_number OR left.active_duns_number_isnull OR right.active_duns_number_isnull ) AND ( ~left.company_name_isnull AND ~right.company_name_isnull ) AND ( ~left.company_address_isnull AND ~right.company_address_isnull ),match_join(LEFT,RIGHT,0),HINT(Parallel_Match),ATMOST(LEFT.cnp_number = RIGHT.cnp_number AND LEFT.prim_range = RIGHT.prim_range AND LEFT.prim_name = RIGHT.prim_name AND LEFT.st = RIGHT.st AND LEFT.company_name[1..4] = RIGHT.company_name[1..4] ,20000),SKEW(1));
mj2 := JOIN( dn0_deduped, dn0_deduped, LEFT.Proxid > RIGHT.Proxid AND ( LEFT.SALT_Partition = RIGHT.SALT_Partition OR LEFT.SALT_Partition='' OR RIGHT.SALT_Partition = '' ) AND LEFT.cnp_number = RIGHT.cnp_number AND LEFT.prim_range = RIGHT.prim_range AND LEFT.prim_name = RIGHT.prim_name AND LEFT.st = RIGHT.st  AND LEFT.sec_range = RIGHT.sec_range AND ( ~left.prim_name_isnull AND ~right.prim_name_isnull ) AND ( ~left.st_isnull AND ~right.st_isnull ) AND ( left.active_enterprise_number = right.active_enterprise_number OR left.active_enterprise_number_isnull OR right.active_enterprise_number_isnull ) AND ( left.active_domestic_corp_key = right.active_domestic_corp_key OR left.active_domestic_corp_key_isnull OR right.active_domestic_corp_key_isnull ) AND ( left.active_duns_number = right.active_duns_number OR left.active_duns_number_isnull OR right.active_duns_number_isnull ) AND ( ~left.company_name_isnull AND ~right.company_name_isnull ) AND ( ~left.company_address_isnull AND ~right.company_address_isnull ),match_join(LEFT,RIGHT,0),HINT(Parallel_Match),ATMOST(LEFT.cnp_number = RIGHT.cnp_number AND LEFT.prim_range = RIGHT.prim_range AND LEFT.prim_name = RIGHT.prim_name AND LEFT.st = RIGHT.st AND LEFT.sec_range = RIGHT.sec_range,20000),SKEW(1));

last_mjs_t :=mj0 + mj1 + mj2;

SALT27.mac_select_best_matches(last_mjs_t,rcid1,rcid2,o);
SHARED all_mjs := o;
export All_Matches := all_mjs : PERSIST('~temp::BIPV2_ProxID_dev4_Proxid_DOT_Base_all_m'); // To by used by rcid and Proxid
SALT27.mac_avoid_transitives(All_Matches,Proxid1,Proxid2,Conf,DateOverlap,Rule,o);
export PossibleMatches := o : PERSIST('~temp::BIPV2_ProxID_dev4_Proxid_DOT_Base_mt');
SALT27.mac_get_BestPerRecord( All_Matches,rcid1,Proxid1,rcid2,Proxid2,o );
EXPORT BestPerRecord := o : PERSIST('~temp::BIPV2_ProxID_dev4_Proxid_DOT_Base_mr');
...
EXPORT ClusterLinkages := o1 : PERSIST('~temp::BIPV2_ProxID_dev4_Proxid_DOT_Base_clu');

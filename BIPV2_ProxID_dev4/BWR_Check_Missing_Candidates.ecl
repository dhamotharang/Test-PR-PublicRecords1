EXPORT BWR_Check_Missing_Candidates := 'todo';output(BIPV2_ProxID_dev4.Files().outfile.qa(dotid in [1786858,4063037]));

UNSIGNED8 Proxidone := 1786858;
UNSIGNED8 Proxidtwo := 4063037;
BFile := BIPV2_ProxID_dev4.In_DOT_Base;
odl := PROJECT(CHOOSEN(BIPV2_ProxID_dev4.Keys(BFile).Candidates(Proxid=Proxidone),100000),BIPV2_ProxID_dev4.match_candidates(BFile).layout_candidates);
odr := PROJECT(CHOOSEN(BIPV2_ProxID_dev4.Keys(BFile).Candidates(Proxid=ProxidTwo),100000),BIPV2_ProxID_dev4.match_candidates(BFile).layout_candidates);
k := BIPV2_ProxID_dev4.Keys(BFile).Specificities_Key;
...
BIPV2_ProxID_dev4.match_candidates(BFile).layout_attribute_matches ainto(BIPV2_ProxID_dev4.Keys(BFile).Attribute_Matches le) := TRANSFORM
SELF := le;
END;
am := PROJECT(BIPV2_ProxID_dev4.Keys(BFile).Attribute_Matches(Proxid1=Proxidone,Proxid2=Proxidtwo)+BIPV2_ProxID_dev4.Keys(BFile).Attribute_Matches(Proxid1=Proxidtwo,Proxid2=Proxidone),ainto(LEFT));
mtch := BIPV2_ProxID_dev4.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,Proxidone,Proxidtwo,0,0}],BIPV2_ProxID_dev4.match_candidates(BFile).layout_matches),am);
...
dit10 				:= BIPV2_ProxID_dev4.Files().outfile.qa;
keycandidates := BIPV2_ProxID_dev4.Keys(BFile).Candidates;
...
BIPV2_ProxID_dev4.output_test_cases(dProxidsInBoth);

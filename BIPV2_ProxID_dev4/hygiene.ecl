...
SALT27.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.id := le.Proxid;
  SELF.Src := le.source;
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT27.StrType)le.cnp_number),TRIM((SALT27.StrType)le.prim_range),TRIM((SALT27.StrType)le.prim_name),TRIM((SALT27.StrType)le.st),TRIM((SALT27.StrType)le.ebr_file_number),TRIM((SALT27.StrType)le.hist_enterprise_number),TRIM((SALT27.StrType)le.hist_domestic_corp_key),TRIM((SALT27.StrType)le.unk_corp_key),TRIM((SALT27.StrType)le.active_enterprise_number),TRIM((SALT27.StrType)le.active_domestic_corp_key),TRIM((SALT27.StrType)le.hist_duns_number),TRIM((SALT27.StrType)le.active_duns_number),TRIM((SALT27.StrType)le.company_fein),TRIM((SALT27.StrType)le.company_phone),TRIM((SALT27.StrType)le.foreign_corp_key),TRIM((SALT27.StrType)le.company_name),TRIM((SALT27.StrType)le.zip),TRIM((SALT27.StrType)le.sec_range),TRIM((SALT27.StrType)le.v_city_name),TRIM((SALT27.StrType)le.cnp_btype),TRIM((SALT27.StrType)le.iscorp),TRIM((SALT27.StrType)le.cnp_hasnumber),TRIM((SALT27.StrType)le.cnp_lowv),TRIM((SALT27.StrType)le.cnp_translated),TRIM((SALT27.StrType)le.cnp_classid),TRIM((SALT27.StrType)le.company_foreign_domestic),TRIM((SALT27.StrType)le.company_bdid),TRIM((SALT27.StrType)le.dt_first_seen),TRIM((SALT27.StrType)le.dt_last_seen)));
  SELF.FldNo := C;
END;
...
SHARED FldIds := DATASET([{1,'cnp_number'}
      ...
      ,{8,'unk_corp_key'}
      ,{9,'active_enterprise_number'}
      ,{10,'active_domestic_corp_key'}
      ,{11,'hist_duns_number'}
      ...
      ,{25,'cnp_translated'}
      ,{26,'cnp_classid'}
      ,{27,'company_foreign_domestic'}
      ,{28,'company_bdid'}
      ,{31,'dt_first_seen'}
      ,{32,'dt_last_seen'}],SALT27.MAC_Character_Counts.Field_Identification);
...
PrettyErrorTotals := record
  ...
  ErrorMessage := CHOOSE(TotalErrors.FieldNum,Fields.InValidMessage_cnp_number(TotalErrors.ErrorNum),Fields.InValidMessage_prim_range(TotalErrors.ErrorNum),Fields.InValidMessage_prim_name(TotalErrors.ErrorNum),Fields.InValidMessage_st(TotalErrors.ErrorNum),Fields.InValidMessage_ebr_file_number(TotalErrors.ErrorNum),Fields.InValidMessage_hist_enterprise_number(TotalErrors.ErrorNum),Fields.InValidMessage_hist_domestic_corp_key(TotalErrors.ErrorNum),Fields.InValidMessage_unk_corp_key(TotalErrors.ErrorNum),Fields.InValidMessage_active_enterprise_number(TotalErrors.ErrorNum),Fields.InValidMessage_active_domestic_corp_key(TotalErrors.ErrorNum),Fields.InValidMessage_hist_duns_number(TotalErrors.ErrorNum),Fields.InValidMessage_active_duns_number(TotalErrors.ErrorNum),Fields.InValidMessage_company_fein(TotalErrors.ErrorNum),Fields.InValidMessage_company_phone(TotalErrors.ErrorNum),Fields.InValidMessage_foreign_corp_key(TotalErrors.ErrorNum),Fields.InValidMessage_company_name(TotalErrors.ErrorNum),Fields.InValidMessage_zip(TotalErrors.ErrorNum),'',Fields.InValidMessage_sec_range(TotalErrors.ErrorNum),Fields.InValidMessage_v_city_name(TotalErrors.ErrorNum),Fields.InValidMessage_cnp_btype(TotalErrors.ErrorNum),Fields.InValidMessage_iscorp(TotalErrors.ErrorNum),Fields.InValidMessage_cnp_hasnumber(TotalErrors.ErrorNum),Fields.InValidMessage_cnp_lowv(TotalErrors.ErrorNum),Fields.InValidMessage_cnp_translated(TotalErrors.ErrorNum),Fields.InValidMessage_cnp_classid(TotalErrors.ErrorNum),Fields.InValidMessage_company_foreign_domestic(TotalErrors.ErrorNum),Fields.InValidMessage_company_bdid(TotalErrors.ErrorNum),'','',Fields.InValidMessage_dt_first_seen(TotalErrors.ErrorNum),Fields.InValidMessage_dt_last_seen(TotalErrors.ErrorNum));
  TotalErrors.Cnt;
end;
...
EXPORT ClusterSrc := SALT27.MOD_ClusterStats.Sources(h,Proxid,source);
end;

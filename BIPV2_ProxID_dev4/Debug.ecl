...
EXPORT layout_sample_matches sample_match_join(match_candidates(ih).layout_candidates le,match_candidates(ih).layout_candidates ri,UNSIGNED outside=0) := TRANSFORM
  ...
  INTEGER2 company_name_score_temp := MAP( le.company_name_isnull OR ri.company_name_isnull OR le.company_name_weight100 = 0 => 0,
                        le.company_name = ri.company_name OR SALT27.fn_hyphen_match(le.company_name,ri.company_name,1)<=1  => MIN(le.company_name_weight100,ri.company_name_weight100),
                        SALT27.MatchBagOfWords(le.company_name,ri.company_name,3,1,1,1,4));
  ...
  SELF.company_name_score := IF ( le.company_name = ri.company_name or company_name_score_temp >= 1300, company_name_score_temp, -9999 ); // Enforce FORCE parameter
...
// Compute the score for the concept company_address
  ...
  SELF.Conf := (SELF.cnp_number_score + SELF.prim_range_score + SELF.prim_name_score + SELF.st_score + SELF.ebr_file_number_score + SELF.hist_enterprise_number_score + SELF.hist_domestic_corp_key_score + SELF.unk_corp_key_score + SELF.active_enterprise_number_score + SELF.active_domestic_corp_key_score + SELF.hist_duns_number_score + SELF.active_duns_number_score + SELF.company_fein_score + SELF.company_phone_score + SELF.foreign_corp_key_score + SELF.company_name_score + SELF.zip_score + SELF.company_csz_score + SELF.sec_range_score + SELF.v_city_name_score + SELF.cnp_btype_score + SELF.iscorp_score + SELF.company_addr1_score + SELF.company_address_score) / 100 + outside;
END;
...
Layout_Chubbies := RECORD,MAXLENGTH(63000)
  t;
  UNSIGNED2 Size := t.cnp_number_size+t.ebr_file_number_size+t.hist_enterprise_number_size+t.hist_domestic_corp_key_size+t.unk_corp_key_size+t.active_enterprise_number_size+t.active_domestic_corp_key_size+t.hist_duns_number_size+t.active_duns_number_size+t.company_fein_size+t.company_phone_size+t.foreign_corp_key_size+t.company_name_size+t.cnp_btype_size+t.iscorp_size+t.company_address_size;
END;

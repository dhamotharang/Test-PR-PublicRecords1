...
SHARED thin_table := DISTRIBUTE(TABLE(h00(SALT_Partition<>'*'),{rcid,active_duns_number,active_enterprise_number,active_domestic_corp_key,hist_enterprise_number,hist_duns_number,hist_domestic_corp_key,foreign_corp_key,unk_corp_key,ebr_file_number,company_name,cnp_hasnumber,cnp_number,cnp_btype,cnp_lowv,cnp_translated,cnp_classid,company_fein,company_foreign_domestic,company_bdid,company_phone,iscorp,prim_range,prim_name,sec_range,v_city_name,st,zip,company_csz,company_addr1,company_address,dt_first_seen,dt_last_seen,SALT_Partition,Proxid}),HASH(Proxid));
...
layout_withpropvars take_cnp_number(with_props le,cnp_number_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj11 := JOIN(pj8,cnp_number_props,left.Proxid=right.Proxid,take_cnp_number(left,right),LEFT OUTER,HASH);
...
layout_withpropvars take_company_fein(with_props le,company_fein_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj16 := JOIN(pj11,company_fein_props,left.Proxid=right.Proxid,take_company_fein(left,right),LEFT OUTER,HASH);
...
layout_withpropvars take_company_phone(with_props le,company_phone_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj19 := JOIN(pj16,company_phone_props,left.Proxid=right.Proxid,take_company_phone(left,right),LEFT OUTER,HASH);
...
layout_withpropvars take_iscorp(with_props le,iscorp_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj20 := JOIN(pj19,iscorp_props,left.Proxid=right.Proxid,take_iscorp(left,right),LEFT OUTER,HASH);
...
layout_withpropvars take_sec_range(with_props le,sec_range_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj23 := JOIN(pj20,sec_range_props,left.Proxid=right.Proxid,take_sec_range(left,right),LEFT OUTER,HASH);
 
pj23 do_computes(pj23 le) := TRANSFORM
  ...
  SELF := le;
END;
SHARED propogated := PROJECT(pj23,do_computes(left)) : PERSIST('~temp::BIPV2_ProxID_dev4_Proxid_DOT_Base_mc_props'); // to allow to 'jump' over an exported value
...
END;
h1 := TABLE(h0,layout_candidates)(~company_name_isnull);
...
layout_candidates add_company_name(layout_candidates le,Specificities(ih).company_name_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF.company_name := IF( ri.field_specificity<>0 or ri.word<>'',SELF.company_name_weight100+' '+ri.word,SALT27.Fn_WordBag_AppendSpecs_Fake(le.company_name, s.company_name_specificity) );// Copy in annotated wordstring
  SELF := le;
END;
SALT27.MAC_Choose_JoinType(j12,s.nulls_company_name,Specificities(ih).company_name_values_persisted,company_name,company_name_weight100,add_company_name,j11);
layout_candidates add_foreign_corp_key(layout_candidates le,Specificities(ih).foreign_corp_key_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT27.MAC_Choose_JoinType(j11,s.nulls_foreign_corp_key,Specificities(ih).foreign_corp_key_values_persisted,foreign_corp_key,foreign_corp_key_weight100,add_foreign_corp_key,j10);
...
layout_candidates add_company_fein(layout_candidates le,Specificities(ih).company_fein_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT27.MAC_Choose_JoinType(j9,s.nulls_company_fein,Specificities(ih).company_fein_values_persisted,company_fein,company_fein_weight100,add_company_fein,j8);
...
layout_candidates add_active_duns_number(layout_candidates le,Specificities(ih).active_duns_number_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT27.MAC_Choose_JoinType(j7,s.nulls_active_duns_number,Specificities(ih).active_duns_number_values_persisted,active_duns_number,active_duns_number_weight100,add_active_duns_number,j6);
...
layout_candidates add_active_enterprise_number(layout_candidates le,Specificities(ih).active_enterprise_number_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT27.MAC_Choose_JoinType(j3,s.nulls_active_enterprise_number,Specificities(ih).active_enterprise_number_values_persisted,active_enterprise_number,active_enterprise_number_weight100,add_active_enterprise_number,j2);
layout_candidates add_ebr_file_number(layout_candidates le,Specificities(ih).ebr_file_number_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT27.MAC_Choose_JoinType(j2,s.nulls_ebr_file_number,Specificities(ih).ebr_file_number_values_persisted,ebr_file_number,ebr_file_number_weight100,add_ebr_file_number,j1);
layout_candidates add_hist_enterprise_number(layout_candidates le,Specificities(ih).hist_enterprise_number_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT27.MAC_Choose_JoinType(j1,s.nulls_hist_enterprise_number,Specificities(ih).hist_enterprise_number_values_persisted,hist_enterprise_number,hist_enterprise_number_weight100,add_hist_enterprise_number,j0);
//Using HASH(did) to get smoother distribution
SHARED Annotated := DISTRIBUTE(j0,hash(Proxid)) : PERSIST('~temp::BIPV2_ProxID_dev4_DOT_Base_mc'); // Distributed for keybuild case
//Now see if these records are actually linkable
TotalWeight := Annotated.hist_enterprise_number_weight100 + Annotated.ebr_file_number_weight100 + Annotated.active_enterprise_number_weight100 + Annotated.active_domestic_corp_key_weight100 + Annotated.hist_domestic_corp_key_weight100 + Annotated.unk_corp_key_weight100 + Annotated.active_duns_number_weight100 + Annotated.hist_duns_number_weight100 + Annotated.company_fein_weight100 + Annotated.company_phone_weight100 + Annotated.foreign_corp_key_weight100 + Annotated.company_name_weight100 + Annotated.company_address_weight100 + Annotated.cnp_number_weight100 + Annotated.cnp_btype_weight100 + Annotated.iscorp_weight100;

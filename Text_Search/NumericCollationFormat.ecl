...
EXPORT NumericCollationFormat := MODULE
	EXPORT UNSIGNED4 StringToNCF(STRING numstr) := BEGINC++
		#option pure;
		const uint32_t digitBytes = 3; //sizeof(unsigned) - sizeof(char);
		const int bufSz = digitBytes << 1;	// number of digits in packed BCD
		const uint32_t digitBits = digitBytes << 3;
		const uint32_t ZERO = 0x80000000;
		const uint32_t N_ZERO = 0x7F999999;
		const int SIGN = 0x80;
		const int EXPMAX = 64;
		const int EXPMIN = -63;
		...
		if (!nzFound)
			rslt = ZERO;  //no non-zero digits, so it must be a real zero even if a minus sign was found
		else
		{
			...
			if (expnt > EXPMAX)
				...
				// too minute; so it is zero or a negative zero
	...
	EXPORT REAL NCFtoREAL(UNSIGNED4 ncf) := BEGINC++
		#include <math.h>
		#option pure
		const unsigned digitBytes = 3; //sizeof(unsigned) - sizeof(char);
		const int bufSz = digitBytes << 1;
		const unsigned digitBits = digitBytes << 3;
		const uint32_t SGN_MASK = 0x80000000;
		const uint32_t EXP_MASK = 0x7F;
		const uint32_t DIGIT_MASK = 0x0F;

...
, DATASET(Layout_DunnData_email) Delta = DATASET([],Layout_DunnData_email)
, DATASET(Layout_DunnData_email) dsBase = In_DunnData_email // Change IN_DunnData_email to change input to ingest process
, DATASET(RECORDOF(DunnData_email.prep_ingest_file))  infile = DunnData_email.prep_ingest_file
) := MODULE
  SHARED NullFile := DATASET([],Layout_DunnData_email); // Use to replace files you wish to remove
  ...
  SHARED WithRT MergeData(WithRT le, WithRT ri) := TRANSFORM // Pick the data for the new record
    ...
    SELF.em_src_cnt := ri.em_src_cnt; // Derived(NEW)
    SELF.num_emails := ri.num_emails; // Derived(NEW)
    SELF.num_indiv := ri.num_indiv; // Derived(NEW)
  ...
  DistIngest0 := DISTRIBUTE(FilesToIngest0, HASH32(dtmatch,email,name_full,address1,address2,city,state,zip5,zip_ext,url,lastdate));
  SortIngest0 := SORT(DistIngest0,dtmatch,email,name_full,address1,address2,city,state,zip5,zip_ext,url,lastdate, __Tpe, RCID, LOCAL);
  GroupIngest0 := GROUP(SortIngest0,dtmatch,email,name_full,address1,address2,city,state,zip5,zip_ext,url,lastdate, LOCAL, ORDERED, STABLE);
  SHARED AllIngestRecs0 := UNGROUP(ROLLUP(GroupIngest0,TRUE,MergeData(LEFT,RIGHT)));
 
  // Existing Base: combine delta with base file
  DistBase0 := DISTRIBUTE(Base0+Delta0, HASH32(dtmatch,email,name_full,address1,address2,city,state,zip5,zip_ext,url,lastdate));
  SortBase0 := SORT(DistBase0,dtmatch,email,name_full,address1,address2,city,state,zip5,zip_ext,url,lastdate, __Tpe, RCID, LOCAL);
  GroupBase0 := GROUP(SortBase0,dtmatch,email,name_full,address1,address2,city,state,zip5,zip_ext,url,lastdate, LOCAL, ORDERED, STABLE);
  SHARED AllBaseRecs0 := UNGROUP(ROLLUP(GroupBase0,TRUE,MergeData(LEFT,RIGHT)));
 
  // Everything: combine ingest and base recs
  Sort0 := SORT(AllBaseRecs0+AllIngestRecs0,dtmatch,email,name_full,address1,address2,city,state,zip5,zip_ext,url,lastdate, __Tpe,RCID,LOCAL);
  Group0 := GROUP(Sort0,dtmatch,email,name_full,address1,address2,city,state,zip5,zip_ext,url,lastdate,LOCAL, ORDERED, STABLE);
  ...
  SHARED AllRecs := ORe+NR1+NR(RCID<>0) : PERSIST('~temp::DunnData_email::Ingest_Cache',EXPIRE(DunnData_email.Config.PersistExpire));
  ...
  EXPORT NewRecords_NoTag := PROJECT(NewRecords,Layout_DunnData_email);
  EXPORT OldRecords :=PROJECT( AllRecs(__Tpe=RecordType.Old), NoFlagsRec);
  EXPORT OldRecords_NoTag := PROJECT(OldRecords,Layout_DunnData_email);
  EXPORT UpdatedRecords := PROJECT(AllRecs(__Tpe=RecordType.Updated), NoFlagsRec);
  EXPORT UpdatedRecords_NoTag := PROJECT(UpdatedRecords,Layout_DunnData_email);
  EXPORT AllRecords := IF(incremental, NewRecords, PROJECT(AllRecs, NoFlagsRec));
  EXPORT AllRecords_NoTag := PROJECT(AllRecords,Layout_DunnData_email); // Records in 'pure' format

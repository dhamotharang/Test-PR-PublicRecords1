IMPORT	ut, AID, AID_Support, DID_Add, Business_Header_SS, address, NID, STD, PromoteSupers,
							PRTE2; //using a cleaning functions in these repositories;
...
EXPORT proc_build_base(STRING8 version) := FUNCTION
	...
	dsBase			:= DunnData_email.Files.Base;
	IngestPrep	:= DunnData_email.prep_ingest_file;

	ingestMod		:= DunnData_email.Ingest(,,dsBase,IngestPrep);
	...
	PopCurrentRec	:= Project(new_base, TRANSFORM(DunnData_email.Layouts.Base, SELF.current_rec := IF(LEFT.__Tpe in [2,3],FALSE,TRUE);
																																				SELF := LEFT;
																																				SELF:= [];));
	
	NID.Mac_CleanFullNames(PopCurrentRec, FileClnName, NAME_FULL
	...
	InvalidName := FileClnName(nametype IN InvName_flags OR (nametype = 'U' AND trim(cln_fname) = '' AND TRIM(cln_lname) = '' AND ~REGEXFIND('TRUST',NAME_FULL)));
	OUTPUT(InvalidName,,'~thor_data400::out::DunnDataEmail_invalid_names_'+version,OVERWRITE,__COMPRESSED__);
	OUTPUT(COUNT(InvalidName),NAMED('TotalInvalidRecords_'+version));
	
	InputFileClnName	:= PROJECT(FileClnName(nametype != 'I'),TRANSFORM(DunnData_email.Layouts.Base,
																																			...
																																			SELF.clean_cname				:=  IF(LEFT.nametype IN business_flags OR (LEFT.nametype = 'U' AND NOT IsName),
																																																		STD.Str.CleanSpaces(LEFT.NAME_FULL),'');
																																			SELF := LEFT));
																										
		//AID process
	unsigned4 lAIDFlags := AID.Common.eReturnValues.RawAID | AID.Common.eReturnValues.ACECacheRecords;
	
	DunnData_email.Layouts.Base tProjectAIDClean_prep(DunnData_email.Layouts.Base pInput) := TRANSFORM
	  clnFullAddr	:= STD.Str.CleanSpaces(TRIM(pInput.address1) +' '+TRIM(pInput.address2));
		...
		self.Append_Prep_Address_Last_Situs	:=	Address.fn_addr_clean_prep(pInput.city
																							...
																							+	' ' + pInput.Zip5+pInput.ZIP_ext, 'last');
		self := pInput;
	END;
	...
	rsAID_NoAddr		:=	rsAIDCleanName(TRIM(Append_Prep_Address_Situs) = '' OR TRIM(Append_Prep_Address_Last_Situs) = '' 
																			OR STD.Str.Find(Append_Prep_Address_Situs, '@', 1) > 0 OR LENGTH(Zip5)<5 OR TRIM(state) = '');
	rsAID_Addr			:=	rsAIDCleanName(TRIM(Append_Prep_Address_Situs) != '' AND TRIM(Append_Prep_Address_Last_Situs) != ''
																			AND STD.Str.Find(Append_Prep_Address_Situs, '@', 1) = 0 AND LENGTH(Zip5)=5 AND TRIM(state) != '');
	...
	DunnData_email.Layouts.Base tProjectClean(rsCleanAID pInput) := TRANSFORM
	...
	DunnData_email.Layouts.Base tProjectNoAddrClean(rsAID_NoAddr pInput) := TRANSFORM
		cl_addr	:= Address.CleanAddress182(pInput.Append_Prep_Address_Situs, TRIM(pInput.city) + ' ' + TRIM(pInput.state) + ' ' + TRIM(pInput.Zip5)+TRIM(pInput.Zip_ext));
	...
	did_Add.MAC_Match_Flex(rsAIDCleanFlipNames								// Input Dataset
													...
													,foo															// Phone
													,DID 															// DID  			
													,DunnData_email.Layouts.Base 			// Output Layout
	...
	PromoteSupers.Mac_SF_BuildProcess(dsBIP_out,DunnData_email.thor_cluster+'base::email::dunndata_email',build_base,3,,true);

...
EXPORT Functions := MODULE
  ...
  EXPORT fn_verify_zip59(STRING zip) := FUNCTION 
	  zip59:= Stringlib.StringFilterOut(Corp2_Mapping.fn_RemoveSpecialChars(trim(zip,left,right)), '-');	//Noticed unprintable letters in vendor zips & Cleaning them to analyze 

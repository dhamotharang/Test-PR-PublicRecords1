...
EXPORT FromNone(DATASET(Corp2_Mapping.LayoutsCommon.Main) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_address2_type_cd_Invalid := Fields.InValid_corp_address2_type_cd((SALT34.StrType)le.corp_address2_type_cd);
    SELF.corp_address2_type_desc_Invalid := Fields.InValid_corp_address2_type_desc((SALT34.StrType)le.corp_address2_type_desc);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_ln_name_type_desc_Invalid << 22 ) + ( le.corp_address1_type_cd_Invalid << 23 ) + ( le.corp_address1_type_desc_Invalid << 25 ) + ( le.corp_address1_line3_Invalid << 26 ) + ( le.corp_address2_type_cd_Invalid << 27 ) + ( le.corp_address2_type_desc_Invalid << 29 ) + ( le.corp_address2_line3_Invalid << 30 ) + ( le.corp_filing_date_Invalid << 31 ) + ( le.corp_status_desc_Invalid << 33 ) + ( le.corp_inc_state_Invalid << 34 ) + ( le.corp_inc_date_Invalid << 35 ) + ( le.corp_term_exist_cd_Invalid << 37 ) + ( le.corp_term_exist_exp_Invalid << 38 ) + ( le.corp_term_exist_desc_Invalid << 40 ) + ( le.corp_foreign_domestic_ind_Invalid << 41 ) + ( le.corp_forgn_state_cd_Invalid << 42 ) + ( le.corp_forgn_state_desc_Invalid << 43 ) + ( le.corp_forgn_date_Invalid << 44 ) + ( le.corp_orig_org_structure_cd_Invalid << 46 ) + ( le.corp_for_profit_ind_Invalid << 47 ) + ( le.recordorigin_Invalid << 48 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_address1_line3_Invalid := (le.ScrubsBits1 >> 26) & 1;
    SELF.corp_address2_type_cd_Invalid := (le.ScrubsBits1 >> 27) & 3;
    SELF.corp_address2_type_desc_Invalid := (le.ScrubsBits1 >> 29) & 1;
    SELF.corp_address2_line3_Invalid := (le.ScrubsBits1 >> 30) & 1;
    SELF.corp_filing_date_Invalid := (le.ScrubsBits1 >> 31) & 3;
    SELF.corp_status_desc_Invalid := (le.ScrubsBits1 >> 33) & 1;
    SELF.corp_inc_state_Invalid := (le.ScrubsBits1 >> 34) & 1;
    SELF.corp_inc_date_Invalid := (le.ScrubsBits1 >> 35) & 3;
    SELF.corp_term_exist_cd_Invalid := (le.ScrubsBits1 >> 37) & 1;
    SELF.corp_term_exist_exp_Invalid := (le.ScrubsBits1 >> 38) & 3;
    SELF.corp_term_exist_desc_Invalid := (le.ScrubsBits1 >> 40) & 1;
    SELF.corp_foreign_domestic_ind_Invalid := (le.ScrubsBits1 >> 41) & 1;
    SELF.corp_forgn_state_cd_Invalid := (le.ScrubsBits1 >> 42) & 1;
    SELF.corp_forgn_state_desc_Invalid := (le.ScrubsBits1 >> 43) & 1;
    SELF.corp_forgn_date_Invalid := (le.ScrubsBits1 >> 44) & 3;
    SELF.corp_orig_org_structure_cd_Invalid := (le.ScrubsBits1 >> 46) & 1;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_status_desc:invalid_alphaparensblank:ALLOW'
          ...
          ,'corp_forgn_state_cd:invalid_forgn_state_cd:CUSTOM'
    ...
    END;
    SummaryInfo := NORMALIZE(SummaryStats,49,Into(LEFT,COUNTER));
    orb_r := RECORD
      AllErrors.Src;

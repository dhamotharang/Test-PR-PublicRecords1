...
SHARED TotalClusters := specificities(h).TotalClusters;
  PHONE_tokens := PROJECT(specificities(h).PHONE_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.PHONE; SELF.TokenType := 1; SELF.Spc := LEFT.field_Specificity ));
  FAX_tokens := PROJECT(specificities(h).FAX_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.FAX; SELF.TokenType := 2; SELF.Spc := LEFT.field_Specificity ));
  NPI_NUMBER_tokens := PROJECT(specificities(h).NPI_NUMBER_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.NPI_NUMBER; SELF.TokenType := 4; SELF.Spc := LEFT.field_Specificity ));
  DEA_NUMBER_tokens := PROJECT(specificities(h).DEA_NUMBER_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.DEA_NUMBER; SELF.TokenType := 5; SELF.Spc := LEFT.field_Specificity ));
  CLIA_NUMBER_tokens := PROJECT(specificities(h).CLIA_NUMBER_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.CLIA_NUMBER; SELF.TokenType := 6; SELF.Spc := LEFT.field_Specificity ));
  MEDICARE_FACILITY_NUMBER_tokens := PROJECT(specificities(h).MEDICARE_FACILITY_NUMBER_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.MEDICARE_FACILITY_NUMBER; SELF.TokenType := 7; SELF.Spc := LEFT.field_Specificity ));
  NCPDP_NUMBER_tokens := PROJECT(specificities(h).NCPDP_NUMBER_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.NCPDP_NUMBER; SELF.TokenType := 8; SELF.Spc := LEFT.field_Specificity ));
  TAX_ID_tokens := PROJECT(specificities(h).TAX_ID_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.TAX_ID; SELF.TokenType := 9; SELF.Spc := LEFT.field_Specificity ));
  FEIN_tokens := PROJECT(specificities(h).FEIN_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.FEIN; SELF.TokenType := 10; SELF.Spc := LEFT.field_Specificity ));
  C_LIC_NBR_tokens := PROJECT(specificities(h).C_LIC_NBR_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.C_LIC_NBR; SELF.TokenType := 11; SELF.Spc := LEFT.field_Specificity ));
  CNP_NAME_tokens := PROJECT(specificities(h).CNP_NAME_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.CNP_NAME; SELF.TokenType := 14; SELF.Spc := LEFT.field_Specificity ));
  ...
  CNP_STORE_NUMBER_tokens := PROJECT(specificities(h).CNP_STORE_NUMBER_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.CNP_STORE_NUMBER; SELF.TokenType := 16; SELF.Spc := LEFT.field_Specificity ));
  CNP_BTYPE_tokens := PROJECT(specificities(h).CNP_BTYPE_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.CNP_BTYPE; SELF.TokenType := 17; SELF.Spc := LEFT.field_Specificity ));
  PRIM_RANGE_tokens := PROJECT(specificities(h).PRIM_RANGE_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.PRIM_RANGE; SELF.TokenType := 23; SELF.Spc := LEFT.field_Specificity ));
  PRIM_NAME_tokens := PROJECT(specificities(h).PRIM_NAME_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.PRIM_NAME; SELF.TokenType := 24; SELF.Spc := LEFT.field_Specificity ));
  SEC_RANGE_tokens := PROJECT(specificities(h).SEC_RANGE_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.SEC_RANGE; SELF.TokenType := 25; SELF.Spc := LEFT.field_Specificity ));
  ST_tokens := PROJECT(specificities(h).ST_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.ST; SELF.TokenType := 26; SELF.Spc := LEFT.field_Specificity ));
  ...
  ZIP_tokens := PROJECT(specificities(h).ZIP_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.ZIP; SELF.TokenType := 28; SELF.Spc := LEFT.field_Specificity ));
...
SHARED all_tokens0 := PHONE_tokens + FAX_tokens + NPI_NUMBER_tokens + DEA_NUMBER_tokens + CLIA_NUMBER_tokens + MEDICARE_FACILITY_NUMBER_tokens + NCPDP_NUMBER_tokens + TAX_ID_tokens + FEIN_tokens + C_LIC_NBR_tokens + CNP_NAME_tokens + CNP_NUMBER_tokens + CNP_STORE_NUMBER_tokens + CNP_BTYPE_tokens + PRIM_RANGE_tokens + PRIM_NAME_tokens + SEC_RANGE_tokens + ST_tokens + V_CITY_NAME_tokens + ZIP_tokens + TAXONOMY_tokens + TAXONOMY_CODE_tokens + MEDICAID_NUMBER_tokens + VENDOR_ID_tokens;
  all_tokens := SALT30.fn_process_multitokens(all_tokens0);
EXPORT TokenKeyName := '~'+'key::HealthCareFacilityHeader_Best::LNPID::Token::TokenKey';
EXPORT TokenKey := INDEX(all_tokens,{UNSIGNED4 TokenHash := HASH32(TokenValue),TokenType},{all_tokens},TokenKeyName);
EXPORT MultiTokenKeyName := '~'+'key::HealthCareFacilityHeader_Best::LNPID::Token::MultiTokenKey';
EXPORT MultiTokenKey := INDEX(all_tokens0(SALT30.WordCount(TokenValue)>1),{UNSIGNED4 TokenHash := HASH32(TokenValue),TokenType},{all_tokens0},MultiTokenKeyName);
  FAC_NAME_tokens := PROJECT(specificities(h).FAC_NAME_values_persisted,TRANSFORM(SALT30.Layout_Classify_Concept,SELF.ConceptHash := LEFT.FAC_NAME; SELF.TokenType := 37; SELF.Spc := LEFT.field_Specificity ));
  ADDR1_tokens := PROJECT(specificities(h).ADDR1_values_persisted,TRANSFORM(SALT30.Layout_Classify_Concept,SELF.ConceptHash := LEFT.ADDR1; SELF.TokenType := 38; SELF.Spc := LEFT.field_Specificity ));
  LOCALE_tokens := PROJECT(specificities(h).LOCALE_values_persisted,TRANSFORM(SALT30.Layout_Classify_Concept,SELF.ConceptHash := LEFT.LOCALE; SELF.TokenType := 39; SELF.Spc := LEFT.field_Specificity ));
  ADDRESS_tokens := PROJECT(specificities(h).ADDRESS_values_persisted,TRANSFORM(SALT30.Layout_Classify_Concept,SELF.ConceptHash := LEFT.ADDRESS; SELF.TokenType := 40; SELF.Spc := LEFT.field_Specificity ));
all_tokens1 := FAC_NAME_tokens + ADDR1_tokens + LOCALE_tokens + ADDRESS_tokens;
EXPORT ConceptKeyName := '~'+'key::HealthCareFacilityHeader_Best::LNPID::Token::ConceptKey';
...
shared FAC_NAME_combinations := o_tot;
  Layout_ConceptTemplate Into(FAC_NAME_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.CNP_NAME_filled => 14, le.CNP_NUMBER_filled => 15, le.CNP_STORE_NUMBER_filled => 16, le.CNP_BTYPE_filled => 17,0);
    SELF.FieldNumber2 := MAP ( le.CNP_NUMBER_filled AND SELF.FieldNumber1 != 15 => 15, le.CNP_STORE_NUMBER_filled AND SELF.FieldNumber1 != 16 => 16, le.CNP_BTYPE_filled AND SELF.FieldNumber1 != 17 => 17,0);
    SELF.FieldNumber3 := MAP ( le.CNP_STORE_NUMBER_filled AND SELF.FieldNumber1 != 16 AND SELF.FieldNumber2 != 16 => 16, le.CNP_BTYPE_filled AND SELF.FieldNumber1 != 17 AND SELF.FieldNumber2 != 17 => 17,0);
    SELF.FieldNumber4 := MAP ( le.CNP_BTYPE_filled AND SELF.FieldNumber1 != 17 AND SELF.FieldNumber2 != 17 AND SELF.FieldNumber3 != 17 => 17,0);
    SELF := le;
  END;
...
shared LOCALE_combinations := o_tot;
  Layout_ConceptTemplate Into(LOCALE_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.V_CITY_NAME_filled => 27, le.ST_filled => 26, le.ZIP_filled => 28,0);
    SELF.FieldNumber2 := MAP ( le.ST_filled AND SELF.FieldNumber1 != 26 => 26, le.ZIP_filled AND SELF.FieldNumber1 != 28 => 28,0);
    SELF.FieldNumber3 := MAP ( le.ZIP_filled AND SELF.FieldNumber1 != 28 AND SELF.FieldNumber2 != 28 => 28,0);
    SELF := le;
  END;
...
shared ADDRESS_combinations := o_tot;
  Layout_ConceptTemplate Into(ADDRESS_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.ADDR1_filled => 38, le.LOCALE_filled => 39,0);
    SELF.FieldNumber2 := MAP ( le.LOCALE_filled AND SELF.FieldNumber1 != 39 => 39,0);
    SELF := le;
  END;
...
EXPORT ConceptTemplatesKey := '~'+'key::HealthCareFacilityHeader_Best::LNPID::Token::ConceptKey';

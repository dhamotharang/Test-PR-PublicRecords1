...
EXPORT LNPIDCompareService := MACRO
  IMPORT SALT30,HealthCareFacilityHeader_Best,ut;
...
odl := PROJECT(CHOOSEN(HealthCareFacilityHeader_Best.Keys(BFile).Candidates(LNPID=LNPIDone),100000),HealthCareFacilityHeader_Best.match_candidates(BFile).layout_candidates);
odr := PROJECT(CHOOSEN(HealthCareFacilityHeader_Best.Keys(BFile).Candidates(LNPID=LNPIDTwo),100000),HealthCareFacilityHeader_Best.match_candidates(BFile).layout_candidates);
k := HealthCareFacilityHeader_Best.Keys(BFile).Specificities_Key;
s := GLOBAL(PROJECT(k,HealthCareFacilityHeader_Best.Layout_Specificities.R)[1]);
odlv := HealthCareFacilityHeader_Best.Debug(BFile,s).RolledEntities(odl);
odrv := HealthCareFacilityHeader_Best.Debug(BFile,s).RolledEntities(odr);
mtch := HealthCareFacilityHeader_Best.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,LNPIDone,LNPIDtwo,0,0}],HealthCareFacilityHeader_Best.match_candidates(BFile).layout_matches));

...
EXPORT layout_sample_matches sample_match_join(match_candidates(ih).layout_candidates le,match_candidates(ih).layout_candidates ri,UNSIGNED c=0,UNSIGNED outside=0) := TRANSFORM
  ...
  SELF.C_LIC_NBR_score := C_LIC_NBR_score_temp*0.75; 
  ...
  INTEGER2 SEC_RANGE_score_temp := MAP(
                        ...
                        0 /* switch0 */)*IF(ADDR1_score_scale=0,1,ADDR1_score_scale)*IF(ADDRESS_score_scale=0,1,ADDRESS_score_scale);
  SELF.CNP_BTYPE_score := IF ( CNP_BTYPE_score_temp >= Config.CNP_BTYPE_Force * 100 OR (SELF.FEIN_score > Config.CNP_BTYPE_OR1_FEIN_Force*100) OR (SELF.NPI_NUMBER_score > Config.CNP_BTYPE_OR2_NPI_NUMBER_Force*100) OR (SELF.C_LIC_NBR_score > Config.CNP_BTYPE_OR3_C_LIC_NBR_Force*100) OR (SELF.CLIA_NUMBER_score > Config.CNP_BTYPE_OR4_CLIA_NUMBER_Force*100) OR (SELF.MEDICARE_FACILITY_NUMBER_score > Config.CNP_BTYPE_OR5_MEDICARE_FACILITY_NUMBER_Force*100) OR (SELF.NCPDP_NUMBER_score > Config.CNP_BTYPE_OR6_NCPDP_NUMBER_Force*100), CNP_BTYPE_score_temp, -9999 ); // Enforce FORCE parameter
  SELF.CNP_BTYPE_skipped := SELF.CNP_BTYPE_score < -5000;// Enforce FORCE parameter
  INTEGER2 TAXONOMY_CODE_score_unweighted := IF ( TAXONOMY_CODE_score_temp >= Config.TAXONOMY_CODE_Force * 100 OR (SELF.NPI_NUMBER_score > Config.TAXONOMY_CODE_OR1_NPI_NUMBER_Force*100), TAXONOMY_CODE_score_temp, -9999 ); // Enforce FORCE parameter
  ...
  SELF.ST_score := IF ( ST_score_temp > Config.ST_Force * 100 OR (SELF.NPI_NUMBER_score > Config.ST_OR1_NPI_NUMBER_Force*100) OR (SELF.NCPDP_NUMBER_score > Config.ST_OR2_NCPDP_NUMBER_Force*100) OR LOCALE_score_pre > 0 OR ADDRESS_score_pre > 0, ST_score_temp, -9999 ); // Enforce FORCE parameter
  SELF.ST_skipped := SELF.ST_score < -5000;// Enforce FORCE parameter
  SELF.CNP_STORE_NUMBER_score := IF ( CNP_STORE_NUMBER_score_temp >= Config.CNP_STORE_NUMBER_Force * 100 OR (SELF.FEIN_score > Config.CNP_STORE_NUMBER_OR1_FEIN_Force*100) OR (SELF.NPI_NUMBER_score > Config.CNP_STORE_NUMBER_OR2_NPI_NUMBER_Force*100) OR (SELF.C_LIC_NBR_score > Config.CNP_STORE_NUMBER_OR3_C_LIC_NBR_Force*100) OR (SELF.CLIA_NUMBER_score > Config.CNP_STORE_NUMBER_OR4_CLIA_NUMBER_Force*100) OR (SELF.MEDICARE_FACILITY_NUMBER_score > Config.CNP_STORE_NUMBER_OR5_MEDICARE_FACILITY_NUMBER_Force*100) OR (SELF.NCPDP_NUMBER_score > Config.CNP_STORE_NUMBER_OR6_NCPDP_NUMBER_Force*100), CNP_STORE_NUMBER_score_temp, -9999 ); // Enforce FORCE parameter
...
// Compute the score for the concept FAC_NAME
  ...
  SELF.PRIM_NAME_score := IF ( PRIM_NAME_score_temp > Config.PRIM_NAME_Force * 100 OR (SELF.NPI_NUMBER_score > Config.PRIM_NAME_OR1_NPI_NUMBER_Force*100) OR (SELF.NCPDP_NUMBER_score > Config.PRIM_NAME_OR2_NCPDP_NUMBER_Force*100) OR ADDR1_score_pre > 0 OR ADDRESS_score_pre > 0, PRIM_NAME_score_temp, -9999 ); // Enforce FORCE parameter
...
// Compute the score for the concept LOCALE
  ...
  SELF.PRIM_RANGE_score := IF ( PRIM_RANGE_score_temp > Config.PRIM_RANGE_Force * 100 OR (SELF.NPI_NUMBER_score > Config.PRIM_RANGE_OR1_NPI_NUMBER_Force*100) OR (SELF.NCPDP_NUMBER_score > Config.PRIM_RANGE_OR2_NCPDP_NUMBER_Force*100) OR ADDR1_score_pre > 0 OR ADDRESS_score_pre > 0, PRIM_RANGE_score_temp, -9999 ); // Enforce FORCE parameter
  SELF.PRIM_RANGE_skipped := SELF.PRIM_RANGE_score < -5000;// Enforce FORCE parameter
  SELF.SEC_RANGE_score := IF ( SEC_RANGE_score_temp >= Config.SEC_RANGE_Force * 100 OR (SELF.NPI_NUMBER_score > Config.SEC_RANGE_OR1_NPI_NUMBER_Force*100) OR (SELF.NCPDP_NUMBER_score > Config.SEC_RANGE_OR2_NCPDP_NUMBER_Force*100), SEC_RANGE_score_temp, -9999 ); // Enforce FORCE parameter
...
// Compute the score for the concept ADDRESS
  ...
  SELF.Conf_Prop := (0
    ...
    +MAX(le.MEDICARE_FACILITY_NUMBER_prop,ri.MEDICARE_FACILITY_NUMBER_prop)*SELF.MEDICARE_FACILITY_NUMBER_score // Score if either field propogated
...
EXPORT Layout_RolledEntity := RECORD,MAXLENGTH(63000)
  ...
  DATASET(SALT30.Layout_FieldValueList) DT_LIC_EXPIRATION_Values := DATASET([],SALT30.Layout_FieldValueList);
  DATASET(SALT30.Layout_FieldValueList) DT_DEA_EXPIRATION_Values := DATASET([],SALT30.Layout_FieldValueList);
...
Layout_RolledEntity RollValues(Layout_RolledEntity le,Layout_RolledEntity ri) := TRANSFORM
  ...
  SELF.DT_LIC_EXPIRATION_values := SALT30.fn_combine_fieldvaluelist(le.DT_LIC_EXPIRATION_values,ri.DT_LIC_EXPIRATION_values);
  SELF.DT_DEA_EXPIRATION_values := SALT30.fn_combine_fieldvaluelist(le.DT_DEA_EXPIRATION_values,ri.DT_DEA_EXPIRATION_values);
...
Layout_RolledEntity into(in_data le) := TRANSFORM
  ...
  SELF.DT_LIC_EXPIRATION_Values := DATASET([{TRIM((SALT30.StrType)le.DT_LIC_EXPIRATION)}],SALT30.Layout_FieldValueList);
  SELF.DT_DEA_EXPIRATION_Values := DATASET([{TRIM((SALT30.StrType)le.DT_DEA_EXPIRATION)}],SALT30.Layout_FieldValueList);
...
EXPORT RemoveProps(DATASET(match_candidates(ih).layout_candidates) im) := FUNCTION
  im rem(im le) := TRANSFORM
    ...
    self.MEDICARE_FACILITY_NUMBER := if ( le.MEDICARE_FACILITY_NUMBER_prop>0, (TYPEOF(le.MEDICARE_FACILITY_NUMBER))'', le.MEDICARE_FACILITY_NUMBER ); // Blank if propogated
    self.MEDICARE_FACILITY_NUMBER_isnull := le.MEDICARE_FACILITY_NUMBER_prop>0 OR le.MEDICARE_FACILITY_NUMBER_isnull;
    self.MEDICARE_FACILITY_NUMBER_prop := 0; // Avoid reducing score later
...
Layout_Chubbies := RECORD,MAXLENGTH(63000)
  t;
  UNSIGNED2 Size := t.NPI_NUMBER_size+t.CLIA_NUMBER_size+t.NCPDP_NUMBER_size+t.MEDICAID_NUMBER_size+t.DEA_NUMBER_size+t.MEDICARE_FACILITY_NUMBER_size+t.TAX_ID_size+t.VENDOR_ID_size+t.FAX_size+t.PHONE_size+t.FAC_NAME_size+t.ADDRESS_size+t.C_LIC_NBR_size+t.FEIN_size+t.TAXONOMY_size+t.TAXONOMY_CODE_size;
END;

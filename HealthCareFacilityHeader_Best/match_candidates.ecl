...
SHARED layout_withpropvars := RECORD
  ...
  INTEGER1 CLIA_NUMBER_prop_method := 0;
  UNSIGNED1 MEDICARE_FACILITY_NUMBER_prop := 0;
  INTEGER1 MEDICARE_FACILITY_NUMBER_prop_method := 0;
  UNSIGNED1 NCPDP_NUMBER_prop := 0;
  INTEGER1 NCPDP_NUMBER_prop_method := 0;
  ...
  INTEGER1 MEDICAID_NUMBER_prop_method := 0;
...
// Now generate code for basis :LNPID,SRC
// There are 12 best types that we can propogate from this basis which means we will not need to use this basis again
  Layout_WithPropVars T_BestNPI_12_0(Layout_WithPropVars le,Best(ih).BestBy_LNPID_best ri) := TRANSFORM
    SELF.NPI_NUMBER_prop_method := MAP ( ri.NPI_NUMBER_method = 0 OR le.NPI_NUMBER_prop_method <> 0 OR le.NPI_NUMBER = ri.NPI_NUMBER => le.NPI_NUMBER_prop_method, // No propogation
      ri.NPI_NUMBER_method = 1 AND (SALT30.StrType)ri.NPI_NUMBER[1..length(trim((SALT30.StrType)le.NPI_NUMBER))] = (SALT30.StrType)le.NPI_NUMBER  => 1, // This field is extensible
      ri.NPI_NUMBER_method = 2 AND (SALT30.StrType)ri.NPI_NUMBER[1..length(trim((SALT30.StrType)le.NPI_NUMBER))] = (SALT30.StrType)le.NPI_NUMBER  => 2, // This field is extensible
    ...
    SELF.NPI_NUMBER := MAP ( le.NPI_NUMBER_prop > 0 OR le.NPI_NUMBER = ri.NPI_NUMBER OR ri.NPI_NUMBER  IN SET(s.nulls_NPI_NUMBER,NPI_NUMBER) => le.NPI_NUMBER, // No propogation
      ri.NPI_NUMBER_method = 1 AND (SALT30.StrType)ri.NPI_NUMBER[1..length(trim((SALT30.StrType)le.NPI_NUMBER))] = (SALT30.StrType)le.NPI_NUMBER  => ri.NPI_NUMBER, // This field is extensible
      ri.NPI_NUMBER_method = 2 AND (SALT30.StrType)ri.NPI_NUMBER[1..length(trim((SALT30.StrType)le.NPI_NUMBER))] = (SALT30.StrType)le.NPI_NUMBER  => ri.NPI_NUMBER, // This field is extensible
      le.NPI_NUMBER);
    ...
    SELF.DEA_NUMBER_prop_method := MAP ( ri.DEA_NUMBER_method = 0 OR le.DEA_NUMBER_prop_method <> 0 OR le.DEA_NUMBER = ri.DEA_NUMBER => le.DEA_NUMBER_prop_method, // No propogation
      ri.DEA_NUMBER_method = 1 AND (SALT30.StrType)ri.DEA_NUMBER[1..length(trim((SALT30.StrType)le.DEA_NUMBER))] = (SALT30.StrType)le.DEA_NUMBER  => 1, // This field is extensible
      ri.DEA_NUMBER_method = 2 AND (SALT30.StrType)ri.DEA_NUMBER[1..length(trim((SALT30.StrType)le.DEA_NUMBER))] = (SALT30.StrType)le.DEA_NUMBER  => 2, // This field is extensible
    ...
    SELF.DEA_NUMBER := MAP ( le.DEA_NUMBER_prop > 0 OR le.DEA_NUMBER = ri.DEA_NUMBER OR ri.DEA_NUMBER  IN SET(s.nulls_DEA_NUMBER,DEA_NUMBER) => le.DEA_NUMBER, // No propogation
      ri.DEA_NUMBER_method = 1 AND (SALT30.StrType)ri.DEA_NUMBER[1..length(trim((SALT30.StrType)le.DEA_NUMBER))] = (SALT30.StrType)le.DEA_NUMBER  => ri.DEA_NUMBER, // This field is extensible
      ri.DEA_NUMBER_method = 2 AND (SALT30.StrType)ri.DEA_NUMBER[1..length(trim((SALT30.StrType)le.DEA_NUMBER))] = (SALT30.StrType)le.DEA_NUMBER  => ri.DEA_NUMBER, // This field is extensible
      le.DEA_NUMBER);
    SELF.DEA_NUMBER_prop := IF ( le.DEA_NUMBER = SELF.DEA_NUMBER, le.DEA_NUMBER_prop,1); // Flag the propagation for sliceouts
    SELF.CLIA_NUMBER_prop_method := MAP ( ri.CLIA_NUMBER_method = 0 OR le.CLIA_NUMBER_prop_method <> 0 OR le.CLIA_NUMBER = ri.CLIA_NUMBER => le.CLIA_NUMBER_prop_method, // No propogation
      ri.CLIA_NUMBER_method = 1 AND (SALT30.StrType)ri.CLIA_NUMBER[1..length(trim((SALT30.StrType)le.CLIA_NUMBER))] = (SALT30.StrType)le.CLIA_NUMBER  => 1, // This field is extensible
      ri.CLIA_NUMBER_method = 2 AND (SALT30.StrType)ri.CLIA_NUMBER[1..length(trim((SALT30.StrType)le.CLIA_NUMBER))] = (SALT30.StrType)le.CLIA_NUMBER  => 2, // This field is extensible
      le.CLIA_NUMBER_prop_method);
    SELF.CLIA_NUMBER := MAP ( le.CLIA_NUMBER_prop > 0 OR le.CLIA_NUMBER = ri.CLIA_NUMBER OR ri.CLIA_NUMBER  IN SET(s.nulls_CLIA_NUMBER,CLIA_NUMBER) => le.CLIA_NUMBER, // No propogation
      ri.CLIA_NUMBER_method = 1 AND (SALT30.StrType)ri.CLIA_NUMBER[1..length(trim((SALT30.StrType)le.CLIA_NUMBER))] = (SALT30.StrType)le.CLIA_NUMBER  => ri.CLIA_NUMBER, // This field is extensible
      ri.CLIA_NUMBER_method = 2 AND (SALT30.StrType)ri.CLIA_NUMBER[1..length(trim((SALT30.StrType)le.CLIA_NUMBER))] = (SALT30.StrType)le.CLIA_NUMBER  => ri.CLIA_NUMBER, // This field is extensible
      le.CLIA_NUMBER);
    SELF.CLIA_NUMBER_prop := IF ( le.CLIA_NUMBER = SELF.CLIA_NUMBER, le.CLIA_NUMBER_prop,1); // Flag the propagation for sliceouts
    SELF.MEDICARE_FACILITY_NUMBER_prop_method := MAP ( ri.MEDICARE_FACILITY_NUMBER_method = 0 OR le.MEDICARE_FACILITY_NUMBER_prop_method <> 0 OR le.MEDICARE_FACILITY_NUMBER = ri.MEDICARE_FACILITY_NUMBER => le.MEDICARE_FACILITY_NUMBER_prop_method, // No propogation
      ri.MEDICARE_FACILITY_NUMBER_method = 1 AND (SALT30.StrType)ri.MEDICARE_FACILITY_NUMBER[1..length(trim((SALT30.StrType)le.MEDICARE_FACILITY_NUMBER))] = (SALT30.StrType)le.MEDICARE_FACILITY_NUMBER  => 1, // This field is extensible
      ri.MEDICARE_FACILITY_NUMBER_method = 2 AND (SALT30.StrType)ri.MEDICARE_FACILITY_NUMBER[1..length(trim((SALT30.StrType)le.MEDICARE_FACILITY_NUMBER))] = (SALT30.StrType)le.MEDICARE_FACILITY_NUMBER  => 2, // This field is extensible
      le.MEDICARE_FACILITY_NUMBER_prop_method);
    SELF.MEDICARE_FACILITY_NUMBER := MAP ( le.MEDICARE_FACILITY_NUMBER_prop > 0 OR le.MEDICARE_FACILITY_NUMBER = ri.MEDICARE_FACILITY_NUMBER OR ri.MEDICARE_FACILITY_NUMBER  IN SET(s.nulls_MEDICARE_FACILITY_NUMBER,MEDICARE_FACILITY_NUMBER) => le.MEDICARE_FACILITY_NUMBER, // No propogation
      ri.MEDICARE_FACILITY_NUMBER_method = 1 AND (SALT30.StrType)ri.MEDICARE_FACILITY_NUMBER[1..length(trim((SALT30.StrType)le.MEDICARE_FACILITY_NUMBER))] = (SALT30.StrType)le.MEDICARE_FACILITY_NUMBER  => ri.MEDICARE_FACILITY_NUMBER, // This field is extensible
      ri.MEDICARE_FACILITY_NUMBER_method = 2 AND (SALT30.StrType)ri.MEDICARE_FACILITY_NUMBER[1..length(trim((SALT30.StrType)le.MEDICARE_FACILITY_NUMBER))] = (SALT30.StrType)le.MEDICARE_FACILITY_NUMBER  => ri.MEDICARE_FACILITY_NUMBER, // This field is extensible
      le.MEDICARE_FACILITY_NUMBER);
    SELF.MEDICARE_FACILITY_NUMBER_prop := IF ( le.MEDICARE_FACILITY_NUMBER = SELF.MEDICARE_FACILITY_NUMBER, le.MEDICARE_FACILITY_NUMBER_prop,1); // Flag the propagation for sliceouts
    SELF.NCPDP_NUMBER_prop_method := MAP ( ri.NCPDP_NUMBER_method = 0 OR le.NCPDP_NUMBER_prop_method <> 0 OR le.NCPDP_NUMBER = ri.NCPDP_NUMBER => le.NCPDP_NUMBER_prop_method, // No propogation
      ri.NCPDP_NUMBER_method = 1 AND (SALT30.StrType)ri.NCPDP_NUMBER[1..length(trim((SALT30.StrType)le.NCPDP_NUMBER))] = (SALT30.StrType)le.NCPDP_NUMBER  => 1, // This field is extensible
      ri.NCPDP_NUMBER_method = 2 AND (SALT30.StrType)ri.NCPDP_NUMBER[1..length(trim((SALT30.StrType)le.NCPDP_NUMBER))] = (SALT30.StrType)le.NCPDP_NUMBER  => 2, // This field is extensible
      le.NCPDP_NUMBER_prop_method);
    SELF.NCPDP_NUMBER := MAP ( le.NCPDP_NUMBER_prop > 0 OR le.NCPDP_NUMBER = ri.NCPDP_NUMBER OR ri.NCPDP_NUMBER  IN SET(s.nulls_NCPDP_NUMBER,NCPDP_NUMBER) => le.NCPDP_NUMBER, // No propogation
      ri.NCPDP_NUMBER_method = 1 AND (SALT30.StrType)ri.NCPDP_NUMBER[1..length(trim((SALT30.StrType)le.NCPDP_NUMBER))] = (SALT30.StrType)le.NCPDP_NUMBER  => ri.NCPDP_NUMBER, // This field is extensible
      ri.NCPDP_NUMBER_method = 2 AND (SALT30.StrType)ri.NCPDP_NUMBER[1..length(trim((SALT30.StrType)le.NCPDP_NUMBER))] = (SALT30.StrType)le.NCPDP_NUMBER  => ri.NCPDP_NUMBER, // This field is extensible
      le.NCPDP_NUMBER);
    SELF.NCPDP_NUMBER_prop := IF ( le.NCPDP_NUMBER = SELF.NCPDP_NUMBER, le.NCPDP_NUMBER_prop,1); // Flag the propagation for sliceouts
    SELF.MEDICAID_NUMBER_prop_method := MAP ( ri.MEDICAID_NUMBER_method = 0 OR le.MEDICAID_NUMBER_prop_method <> 0 OR le.MEDICAID_NUMBER = ri.MEDICAID_NUMBER => le.MEDICAID_NUMBER_prop_method, // No propogation
      ri.MEDICAID_NUMBER_method = 1 AND (SALT30.StrType)ri.MEDICAID_NUMBER[1..length(trim((SALT30.StrType)le.MEDICAID_NUMBER))] = (SALT30.StrType)le.MEDICAID_NUMBER  => 1, // This field is extensible
      ri.MEDICAID_NUMBER_method = 2 AND (SALT30.StrType)ri.MEDICAID_NUMBER[1..length(trim((SALT30.StrType)le.MEDICAID_NUMBER))] = (SALT30.StrType)le.MEDICAID_NUMBER  => 2, // This field is extensible
      le.MEDICAID_NUMBER_prop_method);
    SELF.MEDICAID_NUMBER := MAP ( le.MEDICAID_NUMBER_prop > 0 OR le.MEDICAID_NUMBER = ri.MEDICAID_NUMBER OR ri.MEDICAID_NUMBER  IN SET(s.nulls_MEDICAID_NUMBER,MEDICAID_NUMBER) => le.MEDICAID_NUMBER, // No propogation
      ri.MEDICAID_NUMBER_method = 1 AND (SALT30.StrType)ri.MEDICAID_NUMBER[1..length(trim((SALT30.StrType)le.MEDICAID_NUMBER))] = (SALT30.StrType)le.MEDICAID_NUMBER  => ri.MEDICAID_NUMBER, // This field is extensible
      ri.MEDICAID_NUMBER_method = 2 AND (SALT30.StrType)ri.MEDICAID_NUMBER[1..length(trim((SALT30.StrType)le.MEDICAID_NUMBER))] = (SALT30.StrType)le.MEDICAID_NUMBER  => ri.MEDICAID_NUMBER, // This field is extensible
      le.MEDICAID_NUMBER);
    SELF.MEDICAID_NUMBER_prop := IF ( le.MEDICAID_NUMBER = SELF.MEDICAID_NUMBER, le.MEDICAID_NUMBER_prop,1); // Flag the propagation for sliceouts
    SELF := le;
  END;
SHARED P_BestNPI_12_0 := JOIN(with_props(LNPID <> 0),Best(ih).BestBy_LNPID_best, LEFT.LNPID = RIGHT.LNPID ,T_BestNPI_12_0(LEFT,RIGHT),LEFT OUTER,HASH)
                 + with_props(LNPID = 0);
// Now generate code for basis :LNPID,LIC_STATE
// There are 1 best types that we can propogate from this basis which means we will not need to use this basis again
  Layout_WithPropVars T_MostRecentLIC_1_0(Layout_WithPropVars le,Best(ih).BestBy_LNPID__LIC_STATE_best ri) := TRANSFORM
    SELF.C_LIC_NBR_prop_method := MAP ( ri.C_LIC_NBR_method = 0 OR le.C_LIC_NBR_prop_method <> 0 OR le.C_LIC_NBR = ri.C_LIC_NBR => le.C_LIC_NBR_prop_method, // No propogation
      ri.C_LIC_NBR_method = 1 AND (SALT30.StrType)ri.C_LIC_NBR[1..length(trim((SALT30.StrType)le.C_LIC_NBR))] = (SALT30.StrType)le.C_LIC_NBR  => 1, // This field is extensible
    ...
    SELF.C_LIC_NBR := MAP ( le.C_LIC_NBR_prop > 0 OR le.C_LIC_NBR = ri.C_LIC_NBR OR ri.C_LIC_NBR  IN SET(s.nulls_C_LIC_NBR,C_LIC_NBR) => le.C_LIC_NBR, // No propogation
      ri.C_LIC_NBR_method = 1 AND (SALT30.StrType)ri.C_LIC_NBR[1..length(trim((SALT30.StrType)le.C_LIC_NBR))] = (SALT30.StrType)le.C_LIC_NBR  => ri.C_LIC_NBR, // This field is extensible
      le.C_LIC_NBR);
    SELF.C_LIC_NBR_prop := IF ( le.C_LIC_NBR = SELF.C_LIC_NBR, le.C_LIC_NBR_prop,1); // Flag the propagation for sliceouts
    SELF := le;
  END;
SHARED P_MostRecentLIC_1_0 := JOIN(P_BestNPI_12_0(LNPID <> 0),Best(ih).BestBy_LNPID__LIC_STATE_best, LEFT.LNPID = RIGHT.LNPID  AND ( LEFT.LIC_STATE = (TYPEOF(LEFT.LIC_STATE))'' OR RIGHT.LIC_STATE = (TYPEOF(RIGHT.LIC_STATE))'' OR LEFT.LIC_STATE = RIGHT.LIC_STATE  ),T_MostRecentLIC_1_0(LEFT,RIGHT),LEFT OUTER,HASH)
                 + P_BestNPI_12_0(LNPID = 0);
...
layout_withpropvars take_CNP_NUMBER(with_props le,CNP_NUMBER_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj14 := JOIN(P_MostRecentLIC_1_0,CNP_NUMBER_props,left.LNPID=right.LNPID,take_CNP_NUMBER(left,right),LEFT OUTER,HASH,HINT(parallel_match));
...
layout_withpropvars take_CNP_STORE_NUMBER(with_props le,CNP_STORE_NUMBER_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj15 := JOIN(pj14,CNP_STORE_NUMBER_props,left.LNPID=right.LNPID,take_CNP_STORE_NUMBER(left,right),LEFT OUTER,HASH,HINT(parallel_match));
...
layout_withpropvars take_CNP_BTYPE(with_props le,CNP_BTYPE_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj16 := JOIN(pj15,CNP_BTYPE_props,left.LNPID=right.LNPID,take_CNP_BTYPE(left,right),LEFT OUTER,HASH,HINT(parallel_match));
...
layout_withpropvars take_PRIM_RANGE(with_props le,PRIM_RANGE_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj22 := JOIN(pj16,PRIM_RANGE_props,left.LNPID=right.LNPID,take_PRIM_RANGE(left,right),LEFT OUTER,HASH,HINT(parallel_match));
...
layout_withpropvars take_PRIM_NAME(with_props le,PRIM_NAME_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj23 := JOIN(pj22,PRIM_NAME_props,left.LNPID=right.LNPID AND (left.PRIM_NAME='' OR left.PRIM_NAME[1]=right.PRIM_NAME[1]),take_PRIM_NAME(left,right),LEFT OUTER,HASH,HINT(parallel_match));
...
layout_withpropvars take_SEC_RANGE(with_props le,SEC_RANGE_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj24 := JOIN(pj23,SEC_RANGE_props,left.LNPID=right.LNPID,take_SEC_RANGE(left,right),LEFT OUTER,HASH,HINT(parallel_match));
...
layout_withpropvars take_TAXONOMY(with_props le,TAXONOMY_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj28 := JOIN(pj24,TAXONOMY_props,left.LNPID=right.LNPID,take_TAXONOMY(left,right),LEFT OUTER,HASH,HINT(parallel_match));
...
pj29 do_computes(pj29 le) := TRANSFORM
  ...
  SELF := le;
END;
SHARED propogated := PROJECT(pj29,do_computes(left)) : PERSIST('~temp::LNPID::HealthCareFacilityHeader_Best::mc_props::HealthFacility',EXPIRE(Config.PersistExpire)); // to allow to 'jump' over an exported value
PostPropCounts := RECORD
  ...
  REAL8 CLIA_NUMBER_method_1 := AVE(GROUP,IF(propogated.CLIA_NUMBER_prop_method = 1, 100, 0));
  REAL8 CLIA_NUMBER_method_2 := AVE(GROUP,IF(propogated.CLIA_NUMBER_prop_method = 2, 100, 0));
  ...
  REAL8 MEDICARE_FACILITY_NUMBER_method_1 := AVE(GROUP,IF(propogated.MEDICARE_FACILITY_NUMBER_prop_method = 1, 100, 0));
  REAL8 MEDICARE_FACILITY_NUMBER_method_2 := AVE(GROUP,IF(propogated.MEDICARE_FACILITY_NUMBER_prop_method = 2, 100, 0));
  REAL8 NCPDP_NUMBER_pop := AVE(GROUP,IF(propogated.NCPDP_NUMBER IN SET(s.nulls_NCPDP_NUMBER,NCPDP_NUMBER),0,100));
  REAL8 NCPDP_NUMBER_method_1 := AVE(GROUP,IF(propogated.NCPDP_NUMBER_prop_method = 1, 100, 0));
  REAL8 NCPDP_NUMBER_method_2 := AVE(GROUP,IF(propogated.NCPDP_NUMBER_prop_method = 2, 100, 0));
  ...
  REAL8 MEDICAID_NUMBER_method_1 := AVE(GROUP,IF(propogated.MEDICAID_NUMBER_prop_method = 1, 100, 0));
  REAL8 MEDICAID_NUMBER_method_2 := AVE(GROUP,IF(propogated.MEDICAID_NUMBER_prop_method = 2, 100, 0));
...
EXPORT Layout_Candidates := RECORD // A record to hold weights of each field value
  ...
  UNSIGNED SEC_RANGE_ADDR1_cnt := 0; // Number of name instances matching this one in CONCEPT as BOW
...
layout_candidates add_SEC_RANGE(layout_candidates le,Specificities(ih).SEC_RANGE_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF.SEC_RANGE_ADDR1_weight100 := SALT30.Min0(SEC_RANGE_weight100 + 100*log(ri.e1_cnt/ri.ADDR1_cnt)/log(2)); // Precompute CONCEPT-WORDBAG specificity 
  SELF := le;
END;
...
layout_candidates add_CNP_STORE_NUMBER(layout_candidates le,Specificities(ih).CNP_STORE_NUMBER_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT30.MAC_Choose_JoinType(j11,s.nulls_CNP_STORE_NUMBER,Specificities(ih).CNP_STORE_NUMBER_values_persisted,CNP_STORE_NUMBER,CNP_STORE_NUMBER_weight100,add_CNP_STORE_NUMBER,j10);
layout_candidates add_FAX(layout_candidates le,Specificities(ih).FAX_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT30.MAC_Choose_JoinType(j10,s.nulls_FAX,Specificities(ih).FAX_values_persisted,FAX,FAX_weight100,add_FAX,j9);
layout_candidates add_PHONE(layout_candidates le,Specificities(ih).PHONE_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT30.MAC_Choose_JoinType(j9,s.nulls_PHONE,Specificities(ih).PHONE_values_persisted,PHONE,PHONE_weight100,add_PHONE,j8);
...
SHARED Annotated := DISTRIBUTE(j0,hash(LNPID)) : PERSIST('~temp::LNPID::HealthCareFacilityHeader_Best::mc',EXPIRE(Config.PersistExpire)); // Distributed for keybuild case

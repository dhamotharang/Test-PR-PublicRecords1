...
accidents_by_CT_slim CTslim({files_addl.DedupedKey} L) := TRANSFORM
  SELF.AGENCYID 						:= L.Jurisdiction_nbr;
	SELF := L;
END;

dsSlimmed := PROJECT(files_addl.DedupedKey, CTslim(LEFT));
...
EXPORT file_byCollisionType := dsByCollisionType;
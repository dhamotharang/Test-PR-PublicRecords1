...
accidents_by_slim PerformSlim({files_addl.DedupedKey} L) := TRANSFORM
...
dsSlimmed := PROJECT(files_addl.DedupedKey, PerformSlim(LEFT));
...
accidents_by_slim2 PerformCounts({dsVehiclesRolled} L) := TRANSFORM
	NewBTString 	:= TRIM(L.Report_vehicle_body_type,LEFT,RIGHT);
	NewFHEString 	:= TRIM(L.Report_first_harmful_event,LEFT,RIGHT);
	...
	NewCTString := TRIM(L.Report_Collision_Type,LEFT,RIGHT);

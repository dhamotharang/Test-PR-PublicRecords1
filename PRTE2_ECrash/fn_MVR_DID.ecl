...
export Fn_Mvr_DID(dataset(recordof(Layouts.keybuild_SSv2)) infile) := function

// get only valid vins
main := distribute(PRTE2_Vehicle.files.main_slim(length(trim(vina_vin,left,right)) = 17 ), hash(vehicle_key)); 

// get only owner/registrants
party := distribute(PRTE2_Vehicle.files.party_bip(orig_name_type in ['1', '4'] and fname <> '' and lname <>'' and (unsigned) append_did <>0 ), hash(vehicle_key)); 
...
roll_ := rollup(j_sort, left.fname = right.fname and 
                         left.lname = right.lname and 
												 left.vina_vin = right.vina_vin, t_roll(left,right), local); 

// allow only owners and drivers. 
record_types := ['OWNER','DRIVER','PROPERTY OWNER','VEHICLE DRIVER','VEHICLE OWNER','VEHICLE OWNER-OTHER'];
accidentKeybuild := distribute(infile(trim(record_type,left,right) in record_types),hash(fname,lname,vin)); 


accidentNoOwners := distribute(infile(trim(record_type,left,right) not in record_types),hash(fname,lname,vin)); 

//get did 
jdid:= join(roll_(flag <>'F') , accidentKeybuild,

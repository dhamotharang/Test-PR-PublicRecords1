...
EXPORT Summary(SALT32.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_insurance_type := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.insurance_type)));
    avelength_insurance_type := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.insurance_type)),h.insurance_type<>(typeof(h.insurance_type))'');
...
SALT32.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT32.StrType)le.fname),TRIM((SALT32.StrType)le.lname),TRIM((SALT32.StrType)le.title),TRIM((SALT32.StrType)le.company),TRIM((SALT32.StrType)le.address1),TRIM((SALT32.StrType)le.address2),TRIM((SALT32.StrType)le.city),TRIM((SALT32.StrType)le.state),TRIM((SALT32.StrType)le.zip),TRIM((SALT32.StrType)le.zip4),TRIM((SALT32.StrType)le.cart),TRIM((SALT32.StrType)le.bar),TRIM((SALT32.StrType)le.gender),TRIM((SALT32.StrType)le.country),TRIM((SALT32.StrType)le.postal_cd),TRIM((SALT32.StrType)le.dpv),TRIM((SALT32.StrType)le.addr_type),TRIM((SALT32.StrType)le.county_cd),TRIM((SALT32.StrType)le.insurance_type),TRIM((SALT32.StrType)le.license_type),TRIM((SALT32.StrType)le.msa),TRIM((SALT32.StrType)le.nielsen_county_cd),TRIM((SALT32.StrType)le.list_id),TRIM((SALT32.StrType)le.scno),TRIM((SALT32.StrType)le.keycode),TRIM((SALT32.StrType)le.custno)));
  SELF.FldNo := C;
END;
...
SHARED FldIds := DATASET([{1,'fname'}
      ...
      ,{24,'scno'}
      ,{25,'keycode'}
      ,{26,'custno'}],SALT32.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_insurance_type((SALT32.StrType)le.insurance_type),

...
EXPORT search_records(DATASET(layouts.in_layout) aInputData, IParam._search in_mod) := FUNCTION
  ...
  boolean search_dailies := ~EXISTS(dids_for_header) and ~EXISTS(fetched_dids(err_code = 203));
  ...
  END;
	...
	// Header Search Logic - STEP9: Get errorcodes, fetch hits back from Header search/fetched_dids	
	...
	searched_records := good_recs & bad_recs; //good records(errorcode = 0) + no records(errorcode = 203)

/**
	// SALT Search Logic - STEP9: Get weight and score back from SALT search/fetched_dids
	searched_records_2 := JOIN(recs_formatted,_fetched_dids,
														(integer)LEFT.uniqueid = RIGHT.did AND (integer)LEFT.uniqueid<>0 AND RIGHT.did <>0,
														TRANSFORM(t_IDMSearchRecord_err,
														SELF.score:= RIGHT.score, //SELF.weight:= RIGHT.weight, 
														SELF:= LEFT, SELF:= []), // []:Header fields
														LEFT OUTER,LIMIT(1000,SKIP),KEEP(1000));
**/
// OUTPUT(cleaned_input,NAMED('cleaned_input'));
// OUTPUT(cleaned_input_SALT,NAMED('cleaned_input_SALT'));
// OUTPUT(_fetched_dids,NAMED('_fetched_dids'));
// OUTPUT(fetched_dids_salt,NAMED('fetched_dids_salt'));
// OUTPUT(input_did,NAMED('input_did'));
// OUTPUT(direct_dids,NAMED('direct_dids'));
// OUTPUT(fetched_dids,NAMED('fetched_dids'));
// OUTPUT(dids_selected,named('dids_selected'));
// OUTPUT(dids_for_header,NAMED('dids_for_header'));
// output (header_recs_by_did, named ('header_recs_by_did'), all);
// OUTPUT(header_recs_by_did,NAMED('header_recs_by_did'));
// OUTPUT(recs_frm,NAMED('recs_frm'));
// OUTPUT(recs_formatted,NAMED('recs_formatted'));
// OUTPUT(_good_recs,NAMED('_good_recs'));
// OUTPUT(_bad_recs,NAMED('_bad_recs'));
// OUTPUT(searched_records,NAMED('searched_records'));

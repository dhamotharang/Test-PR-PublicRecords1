...
#uniquename(nulls)
%nulls% := PROJECT(mainfile(field_name IN SET(null_dataset,field_name)),trans_name(LEFT,DATASET([],{valuefile})[1],TRUE));
...
outfile :=	IF ( COUNT(valuefile)*SIZEOF(valuefile,MAX)<SALT25.Config.LookupMaxSize,%sj%,%sjbp%);
  ENDMACRO;

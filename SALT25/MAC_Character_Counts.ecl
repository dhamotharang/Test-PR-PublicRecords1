...
export Data_Layout := RECORD
  SALT25.StrType Fld { MAXLENGTH(200000) };
	UNSIGNED2 FldNo;
  END;
...
shared Pattern_Layout := record
  SALT25.StrType data_pattern {maxlength(200000)};
	unsigned cnt;
	end;
	
shared Value_Layout := record
  SALT25.StrType val  {maxlength(200000)};
	unsigned cnt;
	end;
export Field_Identification := RECORD
  unsigned2 FldNo;
  SALT25.StrType FieldName;
  END;
...
EXPORT EclRecord(DATASET(ResultLine_Layout) d, STRING RName='Record_Layout') := FUNCTION
	...
	r1 WriteEcl(t le) := TRANSFORM
		SELF.Txt := '  '+MAP( ~le.AllNum => 'SALT25.StrType' + IF(le.MaxLen<100,(SALT25.StrType)le.MaxLen, ''),
...
glbl_field_cnts := record
  FldInv1;
	unsigned2 xx_l := length(trim((SALT25.StrType)FldInv1.Fld));
	unsigned2 xx_wc := SALT25.WordCount((SALT25.StrType)FldInv1.Fld);
  end;
...
r tr(FldInv le,unsigned co) := transform
  self.c := ((SALT25.StrType)le.Fld)[co];
	self := le;
  end;
...
dr := record
	  SALT25.StrType Fld  {maxlength(200000)} := SALT25.fn_data_pattern(FldInv.Fld);
		FldInv.Cnt;
		FldInv.FldNo;
  end;
t := table(FldInv,dr);

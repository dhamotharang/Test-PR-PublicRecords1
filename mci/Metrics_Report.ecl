...
EXPORT Metrics_Report (string pVersion, boolean pUseProd, string gcid, dataset (MCI.Layouts_V2.input_processing) pBaseFile, string pHistMode):= MODULE
   ...
   d := dataset([{min_lexid_score, max_lexid_score, ave_lexid_score, cnt_lexid_change, pct_lexid_change, cnt_crk_change, pct_crk_change, guardian_min_lexid_score, guardian_max_lexid_score, guardian_ave_lexid_score, cnt_new_crk}],
	...
	export cnt_new_crk	:= function
		...
		prev_base		:= map(												 
									pHistMode = 'N'	=> dataset([],MCI.Layouts_V2.base),
									IF(NOTHOR(FileServices.GetSuperFileSubCount(MCI.Filenames_V2(pVersion, pUseProd, gcid, pHistMode).member_base.father)) = 0
												 ,dataset([],MCI.Layouts_V2.base)
												 ,MCI.Files_V2(pVersion,pUseProd,gcid,pHistMode).member_base.father)); 

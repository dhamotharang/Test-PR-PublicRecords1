...
EXPORT Convert_Base_toHeader (string pVersion, boolean pUseProd, string gcid, string pHistMode) := MODULE
		...
		EXPORT Build_it := FUNCTION
			...
			pBaseFile		:= map(												 
											...
											NOTHOR(FileServices.GetSuperFileSubCount(MCI.Filenames_V2(pVersion, pUseProd, gcid, pHistMode).member_lBaseTemplate_built)) = 0 =>
														dataset([],MCI.Layouts_V2.base),
											MCI.Files_V2(pVersion,pUseProd,gcid,pHistMode).member_base.qa);
												 
			header_layout := record
				MCI.Layouts_V2.as_header;
				unsigned1	__tpe;
			end;

			header_layout xformHL(MCI.Layouts_V2.base L) := TRANSFORM

...
// 4.  Update your input test file logical name:  test_file_name on line 57
// 5.  RUN a 100 record sample through the roxie version to make sure all keys are working
// 6.  remove the 100 record limit on line 61, change the flag for onThor on line 22 to true and RUN the entire job  
//	 - (wait couple days for it to finish)
// 7.  Copy your output file back to alpha dev
// *****************************************************************************************************************
EXPORT Proc_ProfileBooster_THOR(string Version) := Function
...
test_file_name_tmp := 'in::marketmagnifier::profileboosterin::' + Version;
//test_file_name_in := data_services.foreign_dataland + test_file_name_tmp;
test_file_name_in := test_file_name_tmp;
test_file_name := '~' + test_file_name_tmp;
					
// ds_in := dataset ('~thor400::in::profile_booster::' + test_file_name, layout_file_input, csv(heading(single), quote('"')));		
ds_in := dataset (test_file_name, layout_file_input, csv(heading(single), quote('"')));			
			
	PBSAMPLE := output(choosen(ds_in, eyeball_count), named('sample_input_records'));
	...
	PBSearch := output(choosen(attributes, eyeball_count), named('Search_Function_attributes'));
	...
	PBSearch2 := output(choosen(search_function_with_acctno, eyeball_count), named('search_function_with_acctno'));
	PBOut     := output(search_function_with_acctno,, '~' + test_file_name_tmp + '::profile_booster_attributes_' + if(onThor, 'thor_', 'roxie_') + thorlib.wuid(), 
	csv(heading(single), quote('"')));
	
	RunPB := SEQUENTIAL(PBSAMPLE,PBSearch,PBSearch2,PBOut);		

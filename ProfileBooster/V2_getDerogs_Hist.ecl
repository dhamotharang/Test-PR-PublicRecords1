...
layout_extended get_bankrupt_search (layout_extended le, bans_search ri) := TRANSFORM
	...
	SELF.BJL.disposition := ri.disposition;
 	hit := ri.case_number<>'';
 	hit7y := ri.case_number<>'' AND risk_indicators.iid_constants.checkdays(Risk_Indicators.iid_constants.myGetDate(le.historydate),(STRING8)ri.date_filed,ut.DaysInNYears(7),le.historydate);
	SELF.BJL.filing_count := (INTEGER)hit;
	SELF.BJL.filing_count7y := (INTEGER)hit7y;
...
bankrupt_full_roxie := JOIN (bankrupt_added, bans_search,
                       ...
                       risk_indicators.iid_constants.checkdays(Risk_Indicators.iid_constants.myGetDate(left.historydate),(STRING8)right.date_filed,ut.DaysInNYears(10),left.historydate),
...
layout_extended roll_bankrupt(layout_extended le, layout_extended ri) := TRANSFORM
	...
	SELF.BJL.filing_count7y := le.BJL.filing_count7y + IF(sameBankruptcy, 0, ri.BJL.filing_count7y);
...
liens_full_roxie_unsuppressed := JOIN (liens_added, klr_nonFCRA, 
                    ...
                    risk_indicators.iid_constants.checkdays(Risk_Indicators.iid_constants.myGetDate(left.historydate),(STRING8)right.date_first_seen[1..6],ut.DaysInNYears(7),left.historydate),
...
liens_main_roxie := JOIN(liens_full, liensV2.key_liens_main_ID,
		...
		keyed(left.tmsid=right.tmsid) and keyed(left.rmsid=right.rmsid) and
		trim(stringlib.stringtouppercase(right.filing_type_desc)) not in risk_indicators.iid_constants.set_Invalid_Liens_50 and // ignore certain lien types completely if running shell version 5.0 or higher
    risk_indicators.iid_constants.checkdays(Risk_Indicators.iid_constants.myGetDate(left.historydate),(STRING8)right.process_date[1..6],ut.DaysInNYears(7),left.historydate),
...
liens_main_thor1 := JOIN(
    ...
    risk_indicators.iid_constants.checkdays(Risk_Indicators.iid_constants.myGetDate(left.historydate),(STRING8)right.process_date[1..6],ut.DaysInNYears(7),left.historydate),
		get_evictions(LEFT,RIGHT), left outer, 
		ATMOST(riskwise.max_atmost), local);  //Error:    ATMOST JOIN cannot be evaluated with this join condition (checkDays(myGetDate(LEFT.historydate), (string8) RIGHT.process_date[1..6], DaysInNYears(7), LEFT.historydate)) (547, 21), 4067, 
		// ATMOST(keyed(LEFT.tmsid=RIGHT.tmsid) and keyed(left.rmsid=right.rmsid), riskwise.max_atmost), local);
...
doc_added_roxie := JOIN (liens_rolled, koff_nonFCRA, 
                   (LEFT.did != 0) AND keyed(LEFT.did=RIGHT.sdid) AND
									 ...
									 (unsigned3)(RIGHT.earliest_offense_date[1..6]) < left.historydate AND
                   risk_indicators.iid_constants.checkdays(Risk_Indicators.iid_constants.myGetDate(left.historydate),(STRING8)right.earliest_offense_date[1..6],ut.DaysInNYears(7),left.historydate), 
...
all_foreclosures_roxie_unsuppressed := join(wFID, kforf,
						...
						(unsigned3)(right.recording_date[1..6]) < left.historydate AND right.source=MDR.sourceTools.src_Foreclosures AND
            risk_indicators.iid_constants.checkdays(Risk_Indicators.iid_constants.myGetDate(left.historydate),(STRING8)right.recording_date[1..6],ut.DaysInNYears(7),left.historydate),

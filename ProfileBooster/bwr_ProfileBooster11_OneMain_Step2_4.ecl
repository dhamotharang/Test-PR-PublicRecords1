...
EXPORT bwr_ProfileBooster11_OneMain_Step2_4(string NotifyList ) := FUNCTION
...
ds_in := dataset('~thor400::profileboosterV11::springleaf_full_file_pb_in_layout_final_part4.csv', ProfileBooster.Layouts.LayoutPBInputThor, csv(quote('"')));							
output(choosen(ds_in, eyeball_count), named('sample_input_records_step2_4'));
	
search_function_with_acctno := ProfileBooster.OneMain_PB11_Step2_Function(ds_in, eyeball_count, StepName):independent, FAILURE(FileServices.SendEmail(EmailList,'OneMain PB11 Step2_4 attribute calculation failed', 'The failed workunit is:' + WORKUNIT + FAILMESSAGE));

								
output(choosen(search_function_with_acctno, eyeball_count), named('search_function_with_acctno_Step2_4'));
output(search_function_with_acctno,, '~thor400::out::profile_boosterV11_attributes_' + if(onThor, 'thor_', 'roxie_') + 'part4', csv(quote('"')), EXPIRE(30), OVERWRITE);
	
	
// email results of this bwr
FileServices.SendEmail(EmailList, 'OneMain PB11 Step2_4 finished ' + WORKUNIT, 'Input count  ' + count(ds_in) + '    Output count ' + count(search_function_with_acctno)):
FAILURE(FileServices.SendEmail(EmailList,'OneMain PB11 Step2_4 failed', 'The failed workunit is:' + WORKUNIT + FAILMESSAGE));

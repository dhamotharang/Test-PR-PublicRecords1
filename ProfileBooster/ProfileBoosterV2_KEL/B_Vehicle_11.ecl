...
EXPORT B_Vehicle_11(CFG_Compile __cfg = CFG_Compile) := MODULE
  SHARED VIRTUAL TYPEOF(E_Vehicle(__cfg).__Result) __E_Vehicle := E_Vehicle(__cfg).__Result;
  SHARED __EE16367 := __E_Vehicle;
  EXPORT __ST12438_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    UNSIGNED4 __Part := 0;
  END;
  SHARED __ST12438_Layout __ND16603__Project(E_Vehicle(__cfg).Layout __PP15793) := TRANSFORM
    SELF.Vina_Model_Date_ := __ECAST(KEL.typ.nstr,__FN3(KEL.Routines.DateFromParts,__OP2(__PP15793.Vina_Model_Year_,-,__CN(1)),__CN(1),__CN(1)));
    SELF := __PP15793;
  END;
  EXPORT __ENH_Vehicle_11 := PROJECT(__EE16367,__ND16603__Project(LEFT));
END;

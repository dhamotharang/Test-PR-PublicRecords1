...
EXPORT B_Person_Vehicle_10(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE13039 := __E_Person_Vehicle;
  EXPORT __ST12024_Layout := RECORD
    ...
    UNSIGNED4 __Part := 0;
  END;
  SHARED __ST12024_Layout __ND13268__Project(E_Person_Vehicle(__cfg).Layout __PP12865) := TRANSFORM
    SELF.Current_Date_ := KEL.Routines.MinN(FN_Compile(__cfg).FN_G_E_T_B_U_I_L_D_D_A_T_E(__ECAST(KEL.typ.nstr,__CN('vehicle_build_version'))),__CN((__cfg.CurrentDate)[__PP12865.__Part].__Asof));
    SELF := __PP12865;
  END;
  EXPORT __ENH_Person_Vehicle_10 := PROJECT(__EE13039,__ND13268__Project(LEFT));
END;

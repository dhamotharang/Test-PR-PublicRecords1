...
EXPORT B_Person_7(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE18319 := __E_Person;
  EXPORT __ST10965_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    UNSIGNED4 __Part := 0;
  END;
  SHARED __ST10965_Layout __ND18619__Project(E_Person(__cfg).Layout __PP18120) := TRANSFORM
    __BS18253 := __T(__PP18120.Data_Sources_);
    SELF.P___Lex_I_D_Seen_Flag_ := IF(EXISTS(__BS18253(__T(__T(__PP18120.Data_Sources_).Header_Hit_Flag_))),'1','0');
    SELF := __PP18120;
  END;
  EXPORT __ENH_Person_7 := PROJECT(__EE18319,__ND18619__Project(LEFT));
END;

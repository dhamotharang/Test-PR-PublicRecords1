...
EXPORT B_Person_Vehicle_7(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE19002 := __ENH_Person_Vehicle_8;
  EXPORT __ST11062_Layout := RECORD
    ...
    UNSIGNED4 __Part := 0;
  END;
  SHARED __ST11062_Layout __ND19249__Project(B_Person_Vehicle_8(__cfg).__ST11406_Layout __PP18800) := TRANSFORM
    SELF.Age_Year_At_First_Seen_ := __OP2(__PP18800.Age_At_First_Seen_,/,__CN(365.25));
    __CC1081 := '-99997';
    SELF.Vehicle_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP18800.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP18800.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC1081)));
    SELF := __PP18800;
  END;
  EXPORT __ENH_Person_Vehicle_7 := PROJECT(__EE19002,__ND19249__Project(LEFT));
END;

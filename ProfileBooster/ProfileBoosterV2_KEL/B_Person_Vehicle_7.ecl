...
EXPORT B_Person_Vehicle_7(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE19007 := __ENH_Person_Vehicle_8;
  EXPORT __ST11067_Layout := RECORD
    ...
    UNSIGNED4 __Part := 0;
  END;
  SHARED __ST11067_Layout __ND19254__Project(B_Person_Vehicle_8(__cfg).__ST11411_Layout __PP18805) := TRANSFORM
    SELF.Age_Year_At_First_Seen_ := __OP2(__PP18805.Age_At_First_Seen_,/,__CN(365.25));
    __CC1081 := '-99997';
    SELF.Vehicle_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP18805.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP18805.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC1081)));
    SELF := __PP18805;
  END;
  EXPORT __ENH_Person_Vehicle_7 := PROJECT(__EE19007,__ND19254__Project(LEFT));
END;

...
EXPORT B_Vehicle_1(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE106732 := __ENH_Vehicle_2;
  EXPORT __ST8512_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    UNSIGNED4 __Part := 0;
  END;
  SHARED __ST8512_Layout __ND106980__Project(B_Vehicle_4(__cfg).__ST10319_Layout __PP106115) := TRANSFORM
    __CC1345 := ['Aston Martin','Bentley','Bugatti','Ferrari','Koenigsegg','Lamborghini','Lotus','Maserati','McLaren','Noble','Pagani','Pininfarina','Rolls-Royce'];
    SELF.Flag_Elite_Brand_ := __AND(__CN(__PP106115.Flag_Vina_Make_Desc_ = (KEL.typ.str)1),__OP2(__PP106115.Vina_Make_Description_,IN,__CN(__CC1345)));
    __CC1330 := ['Acura','Alfa Romeo','Audi','BMW','Cadillac','Genesis','Infiniti','Jaguar','Lexus','Lincoln','Mercedes-Benz','Porsche','Saab','Volvo'];
    SELF.Flag_Luxury_Brand_ := __AND(__CN(__PP106115.Flag_Vina_Make_Desc_ = (KEL.typ.str)1),__OP2(__PP106115.Vina_Make_Description_,IN,__CN(__CC1330)));
    SELF.Flag_Vina_Price_ := FN_Compile(__cfg).FN_Is_Echo_Populated_Integer(__ECAST(KEL.typ.nint,__PP106115.Vina_Price_));
    SELF := __PP106115;
  END;
  EXPORT __ENH_Vehicle_1 := PROJECT(__EE106732,__ND106980__Project(LEFT));
END;

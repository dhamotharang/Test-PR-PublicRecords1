...
IMPORT KEL11 AS KEL;
IMPORT CFG_Compile,E_Vehicle,FN_Compile FROM ProfileBooster.ProfileBoosterV2_KEL;
IMPORT * FROM KEL11.Null;
EXPORT B_Vehicle_13(CFG_Compile __cfg = CFG_Compile) := MODULE
  SHARED VIRTUAL TYPEOF(E_Vehicle(__cfg).__Result) __E_Vehicle := E_Vehicle(__cfg).__Result;
  SHARED __EE14382 := __E_Vehicle;
  EXPORT __ST12640_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    UNSIGNED4 __Part := 0;
  END;
  SHARED __ST12640_Layout __ND14616__Project(E_Vehicle(__cfg).Layout __PP13813) := TRANSFORM
    SELF.Current_Date_ := KEL.Routines.MinN(FN_Compile(__cfg).FN_G_E_T_B_U_I_L_D_D_A_T_E(__ECAST(KEL.typ.nstr,__CN('vehicle_build_version'))),__CN((__cfg.CurrentDate)[__PP13813.__Part].__Asof));
    SELF := __PP13813;
  END;
  EXPORT __ENH_Vehicle_13 := PROJECT(__EE14382,__ND14616__Project(LEFT));
END;

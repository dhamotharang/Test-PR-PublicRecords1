...
EXPORT B_Vehicle(CFG_Compile __cfg = CFG_Compile) := MODULE
  SHARED __EE467372 := E_Vehicle(__cfg).PreEntityInternal(true);
  SHARED IDX_Vehicle_UID_Layout := RECORD
    KEL.typ.uid UID;
    TYPEOF(__EE467372.__Payload) __Payload{BLOB} := __EE467372.__Payload;
  END;
  EXPORT IDX_Vehicle_UID_BaseFile := PROJECT(__EE467372,TRANSFORM(IDX_Vehicle_UID_Layout,SELF.UID:=__T(LEFT.UID),SELF:=LEFT));
  ...
  EXPORT __ST468410_Layout := RECORDOF(IDX_Vehicle_UID);

...
EXPORT B_Vehicle(CFG_Compile __cfg = CFG_Compile) := MODULE
  SHARED __EE457720 := E_Vehicle(__cfg).PreEntityInternal(true);
  SHARED IDX_Vehicle_UID_Layout := RECORD
    KEL.typ.uid UID;
    TYPEOF(__EE457720.__Payload) __Payload{BLOB} := __EE457720.__Payload;
  END;
  EXPORT IDX_Vehicle_UID_BaseFile := PROJECT(__EE457720,TRANSFORM(IDX_Vehicle_UID_Layout,SELF.UID:=__T(LEFT.UID),SELF:=LEFT));
  ...
  EXPORT __ST458762_Layout := RECORDOF(IDX_Vehicle_UID);

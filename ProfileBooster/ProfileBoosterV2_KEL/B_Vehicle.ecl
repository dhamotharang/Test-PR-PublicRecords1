...
EXPORT B_Vehicle(CFG_Compile __cfg = CFG_Compile) := MODULE
  SHARED __EE457337 := E_Vehicle(__cfg).PreEntityInternal(true);
  SHARED IDX_Vehicle_UID_Layout := RECORD
    KEL.typ.uid UID;
    TYPEOF(__EE457337.__Payload) __Payload{BLOB} := __EE457337.__Payload;
  END;
  EXPORT IDX_Vehicle_UID_BaseFile := PROJECT(__EE457337,TRANSFORM(IDX_Vehicle_UID_Layout,SELF.UID:=__T(LEFT.UID),SELF:=LEFT));
  ...
  EXPORT __ST458309_Layout := RECORDOF(IDX_Vehicle_UID);

...
EXPORT B_Person_Vehicle_1(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE98950 := __ENH_Person_Vehicle_2;
  SHARED __EE98877 := __ENH_Vehicle_2;
  SHARED __ST99057_Layout := RECORD
    ...
    UNSIGNED4 __Part := 0;
  END;
  __JC99054(B_Person_Vehicle_2(__cfg).__ST12773_Layout __EE98950, B_Vehicle_4(__cfg).__ST9866_Layout __EE98877) := __EEQP(__EE98950.Automobile_,__EE98877.UID) AND __EE98950.__Part = __EE98877.__Part;
  __ST99057_Layout __JT99054(B_Person_Vehicle_2(__cfg).__ST12773_Layout __l, B_Vehicle_4(__cfg).__ST9866_Layout __r) := TRANSFORM
    SELF.Data_Sources__1_ := __r.Data_Sources_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE99055 := JOIN(__EE98950,__EE98877,__JC99054(LEFT,RIGHT),__JT99054(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST7864_Layout := RECORD
    KEL.typ.ntyp(E_Person().Typ) Subject_;
    KEL.typ.ntyp(E_Vehicle().Typ) Automobile_;
    KEL.typ.ndataset(E_Person_Vehicle(__cfg).Registration_Layout) Registration_;
    KEL.typ.ndataset(E_Person_Vehicle(__cfg).Title_Layout) Title_;
    ...
    UNSIGNED4 __Part := 0;
  END;
  SHARED __ST7864_Layout __ND99679__Project(__ST99057_Layout __PP99414) := TRANSFORM
    __CC1081 := '-99997';
    SELF.Flag_New_V_I_N_ := __AND(__AND(__AND(__OP2(__PP99414.Age_At_First_Seen_,<,__CN(365.25 * 3)),__OP2(__PP99414.Age_At_First_Seen_,>=,__CN(0))),__OP2(__PP99414.Vina_Model_Date_,<>,__CN(__CC1081))),__FN1(KEL.Routines.IsValidDate,__PP99414.Date_First_Seen_Capped_));
    SELF.Vehicle_Max_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP99414.Date_Last_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP99414.Date_Last_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC1081)));
    SELF := __PP99414;
  END;
  EXPORT __ENH_Person_Vehicle_1 := PROJECT(__EE99055,__ND99679__Project(LEFT));
END;

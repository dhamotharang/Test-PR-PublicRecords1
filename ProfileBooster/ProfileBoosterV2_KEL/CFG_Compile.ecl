...
EXPORT CFG_Compile := MODULE, VIRTUAL
  ...
  EXPORT Non_F_C_R_A_Profile_Booster_V2_Params_Layout := RECORD
    ...
    UNSIGNED4 __Part := 0;
  END;
  EXPORT Non_F_C_R_A_Profile_Booster_V2_All_Params_Layout := RECORD
    KEL.typ.uid __QueryId := 0;
  ...
  SHARED CurrentDateLayout := {UNSIGNED4 __Part,KEL.typ.kdate __Asof};
  SHARED CurrentDateData := DATASET([{0,KEL.Routines.Today()}],CurrentDateLayout);
  EXPORT CurrentDate := DICTIONARY(CurrentDateData,{__Part=>__Asof});
  // **** These definitions may be overridden as needed ****;
  // Override to set the logicalfile fragment for key file names;
  EXPORT KEL.typ.str LogicalFileFragment := 'ProfileBooster20';
  // Override to set the superfile fragment for key file names;
  EXPORT KEL.typ.str SuperFileFragment := 'ProfileBooster20';

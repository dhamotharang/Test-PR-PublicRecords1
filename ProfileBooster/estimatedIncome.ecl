...
export estimatedIncome(dataset(ProfileBooster.Layouts.Layout_PB_BatchOut) clam) := FUNCTION
  ...
  #else
		ProfileBooster.Layouts.Layout_PB_BatchOut doModel( clam le ) := TRANSFORM
  #end
...
v1_rescurrbusinesscnt            	:= (integer)le.attributes.version1.rescurrbusinesscnt;
v1_crtreclienjudgamtttl         	:= (integer)le.attributes.version1.crtreclienjudgamtttl;
v1_resinputdwelltype             	:= le.attributes.version1.resinputdwelltype;
...
v2_hhcrtrecevictmbrct12mo_ge1 := v1_hhcrtrecevictionmmbrcnt12mo >= 1;
...
//Intermediate variables
		...
		#else
			self.attributes.version1.ProspectEstimatedIncomeRange	:= map(le.attributes.version1.VerifiedProspectFound	= '-1' 	=> '-1',

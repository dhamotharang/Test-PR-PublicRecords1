...
EXPORT bwr_ProfileBooster_OneMain_Step2_1(string NotifyList ) := FUNCTION
...
search_function_with_acctno := ProfileBooster.OneMain_Step2_Function(ds_in, eyeball_count, StepName): independent, FAILURE(FileServices.SendEmail(EmailList,'OneMain Step2_1 attribute calculation failed', 'The failed workunit is:' + WORKUNIT + FAILMESSAGE));
...
output(choosen(checkInputs,50), named('checkInputs_step1'));
output(Count(checkInputs), named('checkInputsCounts_step1'));	
output(Count(checkInputs(lexid = 0)), named('checkInputs_LexidZeroCount_step1'));	

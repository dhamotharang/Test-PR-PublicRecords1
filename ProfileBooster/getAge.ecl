...
EXPORT getAge(DATASET(ProfileBooster.Layouts.Layout_PB_Slim) PBslim, 
							  doxie.IDataAccess mod_access = MODULE (doxie.IDataAccess) END) := FUNCTION

//search header by DID to pick up DOB and calculate age and also append HHID to each rec	
	{ProfileBooster.Layouts.Layout_PB_Slim_header, UNSIGNED4 global_sid} getHeader(PBslim le, Doxie.Key_Header ri) := transform
...
withHeader_THOR := PROJECT(withHeader_THOR_flagged, TRANSFORM(ProfileBooster.Layouts.Layout_PB_Slim_header, 
  ...
  #END
	...
	{ProfileBooster.Layouts.Layout_PB_Slim_header, UNSIGNED4 global_sid} getQHeader(PBslim le, header_quick.key_DID ri) := transform
		self.global_sid			:= ri.global_sid;
		age									:= if(ri.dob = 0 or ri.src=mdr.sourceTools.src_TUCS_Ptrack, 0, risk_indicators.years_apart((unsigned)risk_indicators.iid_constants.myGetDate(le.historydate), (unsigned)ri.dob));
		self.age						:= age;				
	...
	withQHeader_roxie := Suppress.Suppress_ReturnOldLayout(withQHeader_roxie_unsuppressed, mod_access, ProfileBooster.Layouts.Layout_PB_Slim_header);
	...
	withQHeader_thor := Suppress.Suppress_ReturnOldLayout(withQHeader_thor_unsuppressed, mod_access, ProfileBooster.Layouts.Layout_PB_Slim_header);

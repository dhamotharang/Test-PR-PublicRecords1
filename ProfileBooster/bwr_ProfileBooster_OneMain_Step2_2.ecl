...
FileServices.SendEmail(ProfileBooster.Constants.ECL_Developers_Slim, 'OneMain Step2_2 finished ' + WORKUNIT, 'Input count  ' + count(pb_wseq) + '    Output count ' + count(search_function_with_acctno)):
FAILURE(FileServices.SendEmail(ProfileBooster.Constants.ECL_Developers_Slim,'OneMain Step2_2 failed', 'The failed workunit is:' + WORKUNIT + FAILMESSAGE));
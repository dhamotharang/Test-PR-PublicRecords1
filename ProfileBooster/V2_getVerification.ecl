IMPORT _Control, Risk_Indicators, dx_header, RiskWise, InfutorCID, dx_Gong, header_quick, MDR, ut, address, AID_Build, 
	   ProfileBooster, Doxie, Suppress, EmailV2_Services, Gateway;
onThor := _Control.Environment.OnThor;

EXPORT V2_getVerification(DATASET(ProfileBooster.V2_Layouts.Layout_PB2_Shell) PB2Shell,
											doxie.IDataAccess mod_access = MODULE (doxie.IDataAccess) END) := FUNCTION

	nines	 := ProfileBooster.V2_Constants.Max9999999;
...
//search Infutor by DID to verify input name, address, phone 
	{ProfileBooster.V2_Layouts.Layout_PB2_Shell, UNSIGNED4 global_sid} getInfutor(PB2Shell le, infutorcid_key ri) := transform
		OlderErmgRecord := (UNSIGNED6)((ProfileBooster.V2_Common.convertDateTo8((STRING)ri.dt_first_seen))[1..6]) <= (UNSIGNED6)((ProfileBooster.V2_Common.convertDateTo8((STRING)le.EmrgDt_first_seen))[1..6]) OR (UNSIGNED)ProfileBooster.V2_Common.convertDateTo8((STRING)le.EmrgDt_first_seen) <= 0; 
		EmrgDt_first_seen        := IF(OlderErmgRecord,ProfileBooster.V2_Common.convertDateTo8((STRING)ri.dt_first_seen),
		                                               ProfileBooster.V2_Common.convertDateTo8((STRING)le.EmrgDt_first_seen));
		SELF.EmrgDt_first_seen   := (UNSIGNED)EmrgDt_first_seen;
		SELF.EmrgSrc             := IF(OlderErmgRecord,MDR.sourceTools.src_InfutorCID,le.EmrgSrc);
		SELF.EmrgDOB			 := MAX(ProfileBooster.V2_Common.convertDateTo8((STRING)le.dob),
										ProfileBooster.V2_Common.convertDateTo8((STRING)le.EmrgDOB)
...
//search Gong by DID to verify input name, address, phone
	{ProfileBooster.V2_Layouts.Layout_PB2_Shell, UNSIGNED4 global_sid} getGong(PB2Shell le, gonghistorydid_key ri) := transform
		...
		SELF.addrscore 			 := Risk_Indicators.AddrScore.AddressScore(le.prim_range, le.prim_name, le.sec_range,ri.prim_range,ri.prim_name,ri.sec_range,zip_score,cityst_score);
		SELF.addrcount 			 := (INTEGER)Risk_Indicators.iid_constants.ga(SELF.AddrScore);
...
//search header by DID to verify input name, address, phone, SSN
	...
	wHeader_roxie := PROJECT(wHeader_roxie_flagged, TRANSFORM(ProfileBooster.V2_Layouts.Layout_PB2_Shell,
		...
		SELF.dob				 := IF(left.is_suppressed, Suppress.OptOutMessage('STRING'), ProfileBooster.V2_Common.convertDateTo8((STRING)left.dob));
	...
	{ProfileBooster.V2_Layouts.Layout_PB2_Shell, UNSIGNED4 global_sid} getQHeader(ProfileBooster.V2_Layouts.Layout_PB2_Shell le, quickheader_key ri) := transform
    	...
    	historydate := (UNSIGNED)fullhistorydate[1..6];
		SELF.dt_last_seen		 := if(ri.dt_last_seen > HistoryDate, HistoryDate, ri.dt_last_seen);
		SELF.dob				 := if(ri.src=mdr.sourceTools.src_TUCS_Ptrack, '', ProfileBooster.V2_Common.convertDateTo8((STRING)ri.dob));
		SELF.ProspectAge 		 := if(ri.src=mdr.sourceTools.src_TUCS_Ptrack, 0, risk_indicators.years_apart((UNSIGNED)fullhistorydate, (UNSIGNED)ri.dob));
   		SELF.title				 := ri.title;
		SELF.hdr_prim_range		 := ri.prim_range;
		SELF.hdr_predir			 := ri.predir;
	...
	ProfileBooster.V2_Layouts.Layout_PB2_Shell addVerification(PB2Shell le, rolledVer ri) := TRANSFORM
		...
		SELF.dob				 := ProfileBooster.V2_Common.convertDateTo8((STRING)ri.dob);
		...
		SELF.EmrgDob			 := ProfileBooster.V2_Common.convertDateTo8((STRING)ri.EmrgDob);
...
//join all addresses from the header to the address history key to determine current address versus previous address
	ProfileBooster.V2_Layouts.Layout_PB2_Shell getAddrSeq(dedupHdr le, address_rank_key ri) := TRANSFORM
		...
		SELF.LifeMoveNewMsnc  		:= if(ri.date_first_seen <> 0 AND LifeMoveNewMsnc>=0,LifeMoveNewMsnc,nines);
		SELF 						:= le;
	END;
...
//rollup to get current and previous address on same record
  ...
  sortedAddrs := SORT(withAddrs, seq, did, did2, -curr_z5);

  rolledAddrs := rollup(sortedAddrs, rollAddrs(left,right), seq);

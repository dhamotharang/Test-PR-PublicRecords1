...
ProfileBooster.V2_Key_Layouts.Layout_PB2_Slim_student	addStudent(PBslim le, american_student_list.key_DID ri) := transform
    ...
    self.student_college_class      := MAP(ri.college_class='0' => 'FR',
                                           ...
                                           ri.college_class='4' => 'GR',
                                                                   '-99997');
		self.src := ri.source;
    within4Years											:= ri.college_code = '4' and ProfileBooster.Common.MonthsApart_YYYYMMDD(risk_indicators.iid_constants.myGetDate((unsigned3)((STRING)(le.historydate))[1..6]),(string)ri.date_first_seen,true) < 49;
    within2Years											:= ri.college_code = '2' and ProfileBooster.Common.MonthsApart_YYYYMMDD(risk_indicators.iid_constants.myGetDate((unsigned3)((STRING)(le.historydate))[1..6]),(string)ri.date_first_seen,true) < 25;
...
//rollup to accumulate the counts 
  ProfileBooster.V2_Key_Layouts.Layout_PB2_Slim_student rollStudents(ProfileBooster.V2_Key_Layouts.Layout_PB2_Slim_student le, ProfileBooster.V2_Key_Layouts.Layout_PB2_Slim_student ri) := transform
    ...
    self.DemEduCollRecNewMajorTypeEv  := MAP(NewerRecord => ri.DemEduCollRecNewMajorTypeEv,
                                             ...
                                             ProfileBooster.ProfileBoosterV2_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
    self.DemEduCollEvidFlagEv         := MAP(le.student_date_first_seen <> '' => 1,
  ...
  // withInquiry := JOIN(rolledStudents, Inquiry_AccLogs.Key_Inquiry_DID,
                      // KEYED(LEFT.did = RIGHT.s_did) AND
           						// (unsigned)RIGHT.search_info.datetime[1..6] <= LEFT.historydate,
                      // xfm_GetStudentInquiries(LEFT, RIGHT), LEFT OUTER, KEEP(1)
                      // );
/*
...
// output(CHOOSEN(withInquiry,100), named('withInquiry'));

...
ds_in := dataset (test_file_name, layout_file_input, csv(heading(single), quote('"')));			
	...
	PBSAMPLE := output(choosen(ds_in, eyeball_count), named('sample_input_records_' + prt));
	...
	PBSearch2 := output(choosen(search_function_with_acctno, eyeball_count), named('search_function_with_acctno_' + prt));
	outfile   := output(search_function_with_acctno,, '~' + test_file_name_tmp + ProfileBooster.Files.PBSuffix,csv(heading(single), quote('"')));
	
	PBOUT := SEQUENTIAL(PBSAMPLE,PBSearch,PBSearch2,outfile);		

...
EXPORT FromNone(DATASET(Base_Layout_PAW) h) := MODULE
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_first_seen_Invalid << 0 ) + ( le.dt_last_seen_Invalid << 1 ) + ( le.contact_id_Invalid << 2 ) + ( le.did_Invalid << 3 ) + ( le.bdid_Invalid << 4 ) + ( le.ssn_Invalid << 5 ) + ( le.company_name_Invalid << 6 ) + ( le.company_predir_Invalid << 7 ) + ( le.company_postdir_Invalid << 8 ) + ( le.company_state_Invalid << 9 ) + ( le.company_zip_Invalid << 10 ) + ( le.company_zip4_Invalid << 11 ) + ( le.company_phone_Invalid << 12 ) + ( le.fname_Invalid << 13 ) + ( le.lname_Invalid << 14 ) + ( le.county_Invalid << 15 ) + ( le.phone_Invalid << 16 ) + ( le.active_phone_flag_Invalid << 17 ) + ( le.GLB_Invalid << 18 ) + ( le.source_Invalid << 19 ) + ( le.source_count_Invalid << 20 ) + ( le.dead_flag_Invalid << 21 ) + ( le.from_hdr_Invalid << 22 ) + ( le.RawAid_Invalid << 23 ) + ( le.Company_RawAid_Invalid << 24 ) + ( le.global_sid_Invalid << 25 ) + ( le.record_sid_Invalid << 26 ) + ( le.record_type_Invalid << 27 ) + ( le.predir_Invalid << 28 ) + ( le.postdir_Invalid << 29 ) + ( le.state_Invalid << 30 ) + ( le.zip_Invalid << 31 ) + ( le.zip4_Invalid << 32 ) + ( le.geo_lat_Invalid << 33 ) + ( le.geo_long_Invalid << 34 ) + ( le.msa_Invalid << 35 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.contact_id_Invalid := (le.ScrubsBits1 >> 2) & 1;
    SELF.did_Invalid := (le.ScrubsBits1 >> 3) & 1;
    SELF.bdid_Invalid := (le.ScrubsBits1 >> 4) & 1;
    SELF.ssn_Invalid := (le.ScrubsBits1 >> 5) & 1;
    SELF.company_name_Invalid := (le.ScrubsBits1 >> 6) & 1;
    SELF.company_predir_Invalid := (le.ScrubsBits1 >> 7) & 1;
    SELF.company_postdir_Invalid := (le.ScrubsBits1 >> 8) & 1;
    SELF.company_state_Invalid := (le.ScrubsBits1 >> 9) & 1;
    SELF.company_zip_Invalid := (le.ScrubsBits1 >> 10) & 1;
    SELF.company_zip4_Invalid := (le.ScrubsBits1 >> 11) & 1;
    SELF.company_phone_Invalid := (le.ScrubsBits1 >> 12) & 1;
    SELF.fname_Invalid := (le.ScrubsBits1 >> 13) & 1;
    SELF.lname_Invalid := (le.ScrubsBits1 >> 14) & 1;
    SELF.county_Invalid := (le.ScrubsBits1 >> 15) & 1;
    SELF.phone_Invalid := (le.ScrubsBits1 >> 16) & 1;
    SELF.active_phone_flag_Invalid := (le.ScrubsBits1 >> 17) & 1;
    SELF.GLB_Invalid := (le.ScrubsBits1 >> 18) & 1;
    SELF.source_Invalid := (le.ScrubsBits1 >> 19) & 1;
    SELF.source_count_Invalid := (le.ScrubsBits1 >> 20) & 1;
    SELF.dead_flag_Invalid := (le.ScrubsBits1 >> 21) & 1;
    SELF.from_hdr_Invalid := (le.ScrubsBits1 >> 22) & 1;
    SELF.RawAid_Invalid := (le.ScrubsBits1 >> 23) & 1;
    SELF.Company_RawAid_Invalid := (le.ScrubsBits1 >> 24) & 1;
    SELF.global_sid_Invalid := (le.ScrubsBits1 >> 25) & 1;
    SELF.record_sid_Invalid := (le.ScrubsBits1 >> 26) & 1;
    SELF.record_type_Invalid := (le.ScrubsBits1 >> 27) & 1;
    SELF.predir_Invalid := (le.ScrubsBits1 >> 28) & 1;
    SELF.postdir_Invalid := (le.ScrubsBits1 >> 29) & 1;
    SELF.state_Invalid := (le.ScrubsBits1 >> 30) & 1;
    SELF.zip_Invalid := (le.ScrubsBits1 >> 31) & 1;
    SELF.zip4_Invalid := (le.ScrubsBits1 >> 32) & 1;
    SELF.geo_lat_Invalid := (le.ScrubsBits1 >> 33) & 1;
    SELF.geo_long_Invalid := (le.ScrubsBits1 >> 34) & 1;
    SELF.msa_Invalid := (le.ScrubsBits1 >> 35) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    AnyRule_WithErrorsCount := COUNT(GROUP, h.dt_first_seen_Invalid > 0 OR h.dt_last_seen_Invalid > 0 OR h.contact_id_Invalid > 0 OR h.did_Invalid > 0 OR h.bdid_Invalid > 0 OR h.ssn_Invalid > 0 OR h.company_name_Invalid > 0 OR h.company_predir_Invalid > 0 OR h.company_postdir_Invalid > 0 OR h.company_state_Invalid > 0 OR h.company_zip_Invalid > 0 OR h.company_zip4_Invalid > 0 OR h.company_phone_Invalid > 0 OR h.fname_Invalid > 0 OR h.lname_Invalid > 0 OR h.county_Invalid > 0 OR h.phone_Invalid > 0 OR h.active_phone_flag_Invalid > 0 OR h.GLB_Invalid > 0 OR h.source_Invalid > 0 OR h.source_count_Invalid > 0 OR h.dead_flag_Invalid > 0 OR h.from_hdr_Invalid > 0 OR h.RawAid_Invalid > 0 OR h.Company_RawAid_Invalid > 0 OR h.global_sid_Invalid > 0 OR h.record_sid_Invalid > 0 OR h.record_type_Invalid > 0 OR h.predir_Invalid > 0 OR h.postdir_Invalid > 0 OR h.state_Invalid > 0 OR h.zip_Invalid > 0 OR h.zip4_Invalid > 0 OR h.geo_lat_Invalid > 0 OR h.geo_long_Invalid > 0 OR h.msa_Invalid > 0);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.dt_first_seen_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.dt_last_seen_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.contact_id_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.did_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.bdid_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.ssn_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.company_name_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.company_predir_ENUM_ErrorCount > 0, 1, 0) + IF(le.company_postdir_ENUM_ErrorCount > 0, 1, 0) + IF(le.company_state_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.company_zip_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.company_zip4_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.company_phone_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.fname_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.lname_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.county_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.phone_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.active_phone_flag_ENUM_ErrorCount > 0, 1, 0) + IF(le.GLB_ENUM_ErrorCount > 0, 1, 0) + IF(le.source_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.source_count_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.dead_flag_ENUM_ErrorCount > 0, 1, 0) + IF(le.from_hdr_ENUM_ErrorCount > 0, 1, 0) + IF(le.RawAid_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.Company_RawAid_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.global_sid_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.record_sid_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.record_type_ENUM_ErrorCount > 0, 1, 0) + IF(le.predir_ENUM_ErrorCount > 0, 1, 0) + IF(le.postdir_ENUM_ErrorCount > 0, 1, 0) + IF(le.state_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.zip_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.zip4_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.geo_lat_ALLOW_ErrorCount > 0, 1, 0) + IF(le.geo_long_ALLOW_ErrorCount > 0, 1, 0) + IF(le.msa_CUSTOM_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.dt_first_seen_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.dt_last_seen_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.contact_id_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.did_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.bdid_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.ssn_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.company_name_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.company_predir_ENUM_ErrorCount > 0, 1, 0) + IF(le.company_postdir_ENUM_ErrorCount > 0, 1, 0) + IF(le.company_state_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.company_zip_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.company_zip4_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.company_phone_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.fname_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.lname_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.county_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.phone_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.active_phone_flag_ENUM_ErrorCount > 0, 1, 0) + IF(le.GLB_ENUM_ErrorCount > 0, 1, 0) + IF(le.source_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.source_count_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.dead_flag_ENUM_ErrorCount > 0, 1, 0) + IF(le.from_hdr_ENUM_ErrorCount > 0, 1, 0) + IF(le.RawAid_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.Company_RawAid_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.global_sid_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.record_sid_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.record_type_ENUM_ErrorCount > 0, 1, 0) + IF(le.predir_ENUM_ErrorCount > 0, 1, 0) + IF(le.postdir_ENUM_ErrorCount > 0, 1, 0) + IF(le.state_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.zip_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.zip4_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.geo_lat_ALLOW_ErrorCount > 0, 1, 0) + IF(le.geo_long_ALLOW_ErrorCount > 0, 1, 0) + IF(le.msa_CUSTOM_ErrorCount > 0, 1, 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.dt_first_seen_Invalid,le.dt_last_seen_Invalid,le.contact_id_Invalid,le.did_Invalid,le.bdid_Invalid,le.ssn_Invalid,le.company_name_Invalid,le.company_predir_Invalid,le.company_postdir_Invalid,le.company_state_Invalid,le.company_zip_Invalid,le.company_zip4_Invalid,le.company_phone_Invalid,le.fname_Invalid,le.lname_Invalid,le.county_Invalid,le.phone_Invalid,le.active_phone_flag_Invalid,le.GLB_Invalid,le.source_Invalid,le.source_count_Invalid,le.dead_flag_Invalid,le.from_hdr_Invalid,le.RawAid_Invalid,le.Company_RawAid_Invalid,le.global_sid_Invalid,le.record_sid_Invalid,le.record_type_Invalid,le.predir_Invalid,le.postdir_Invalid,le.state_Invalid,le.zip_Invalid,le.zip4_Invalid,le.geo_lat_Invalid,le.geo_long_Invalid,le.msa_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Base_Fields.InvalidMessage_dt_first_seen(le.dt_first_seen_Invalid),Base_Fields.InvalidMessage_dt_last_seen(le.dt_last_seen_Invalid),Base_Fields.InvalidMessage_contact_id(le.contact_id_Invalid),Base_Fields.InvalidMessage_did(le.did_Invalid),Base_Fields.InvalidMessage_bdid(le.bdid_Invalid),Base_Fields.InvalidMessage_ssn(le.ssn_Invalid),Base_Fields.InvalidMessage_company_name(le.company_name_Invalid),Base_Fields.InvalidMessage_company_predir(le.company_predir_Invalid),Base_Fields.InvalidMessage_company_postdir(le.company_postdir_Invalid),Base_Fields.InvalidMessage_company_state(le.company_state_Invalid),Base_Fields.InvalidMessage_company_zip(le.company_zip_Invalid),Base_Fields.InvalidMessage_company_zip4(le.company_zip4_Invalid),Base_Fields.InvalidMessage_company_phone(le.company_phone_Invalid),Base_Fields.InvalidMessage_fname(le.fname_Invalid),Base_Fields.InvalidMessage_lname(le.lname_Invalid),Base_Fields.InvalidMessage_county(le.county_Invalid),Base_Fields.InvalidMessage_phone(le.phone_Invalid),Base_Fields.InvalidMessage_active_phone_flag(le.active_phone_flag_Invalid),Base_Fields.InvalidMessage_GLB(le.GLB_Invalid),Base_Fields.InvalidMessage_source(le.source_Invalid),Base_Fields.InvalidMessage_source_count(le.source_count_Invalid),Base_Fields.InvalidMessage_dead_flag(le.dead_flag_Invalid),Base_Fields.InvalidMessage_from_hdr(le.from_hdr_Invalid),Base_Fields.InvalidMessage_RawAid(le.RawAid_Invalid),Base_Fields.InvalidMessage_Company_RawAid(le.Company_RawAid_Invalid),Base_Fields.InvalidMessage_global_sid(le.global_sid_Invalid),Base_Fields.InvalidMessage_record_sid(le.record_sid_Invalid),Base_Fields.InvalidMessage_record_type(le.record_type_Invalid),Base_Fields.InvalidMessage_predir(le.predir_Invalid),Base_Fields.InvalidMessage_postdir(le.postdir_Invalid),Base_Fields.InvalidMessage_state(le.state_Invalid),Base_Fields.InvalidMessage_zip(le.zip_Invalid),Base_Fields.InvalidMessage_zip4(le.zip4_Invalid),Base_Fields.InvalidMessage_geo_lat(le.geo_lat_Invalid),Base_Fields.InvalidMessage_geo_long(le.geo_long_Invalid),Base_Fields.InvalidMessage_msa(le.msa_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.dead_flag_Invalid,'ENUM','UNKNOWN')
          ...
          ,CHOOSE(le.msa_Invalid,'CUSTOM','UNKNOWN'),'UNKNOWN'));
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ...
          ,le.geo_lat_ALLOW_ErrorCount
          ,le.geo_long_ALLOW_ErrorCount
          ,le.msa_CUSTOM_ErrorCount,0) / le.TotalCnt, CHOOSE(c - NumRules
    ...
    SALT311.ScrubsOrbitLayout xNormHygieneStats(hygiene_summaryStats le, UNSIGNED c, STRING suffix) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'dead_flag:' + getFieldTypeText(h.dead_flag) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix
          ...
          ,'msa:' + getFieldTypeText(h.msa) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix,'UNKNOWN');
      SELF.rulecnt := CHOOSE(c

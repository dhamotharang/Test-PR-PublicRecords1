...
EXPORT FromNone(DATASET(Layout_FILE) h) := MODULE
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.orig_end_user_id_Invalid << 0 ) + ( le.orig_loginid_Invalid << 1 ) + ( le.orig_billing_code_Invalid << 3 ) + ( le.orig_transaction_id_Invalid << 4 ) + ( le.orig_transaction_type_Invalid << 6 ) + ( le.orig_neighbors_Invalid << 8 ) + ( le.orig_relatives_Invalid << 10 ) + ( le.orig_associates_Invalid << 12 ) + ( le.orig_property_Invalid << 14 ) + ( le.orig_company_id_Invalid << 16 ) + ( le.orig_reference_code_Invalid << 18 ) + ( le.orig_fname_Invalid << 19 ) + ( le.orig_mname_Invalid << 21 ) + ( le.orig_lname_Invalid << 23 ) + ( le.orig_address_Invalid << 25 ) + ( le.orig_city_Invalid << 26 ) + ( le.orig_state_Invalid << 28 ) + ( le.orig_zip_Invalid << 30 ) + ( le.orig_zip4_Invalid << 32 ) + ( le.orig_phone_Invalid << 34 ) + ( le.orig_ssn_Invalid << 36 ) + ( le.orig_free_Invalid << 38 ) + ( le.orig_record_count_Invalid << 40 ) + ( le.orig_price_Invalid << 42 ) + ( le.orig_bankruptcy_Invalid << 43 ) + ( le.orig_transaction_code_Invalid << 45 ) + ( le.orig_dateadded_Invalid << 47 ) + ( le.orig_full_name_Invalid << 49 ) + ( le.orig_billingdate_Invalid << 51 ) + ( le.orig_business_name_Invalid << 53 ) + ( le.orig_pricing_error_code_Invalid << 54 ) + ( le.orig_dl_purpose_Invalid << 56 ) + ( le.orig_result_format_Invalid << 58 ) + ( le.orig_dob_Invalid << 60 ) + ( le.orig_unique_id_Invalid << 62 );
    SELF.ScrubsBits2 := ( le.orig_dls_Invalid << 0 ) + ( le.orig_mvs_Invalid << 2 ) + ( le.orig_function_name_Invalid << 4 ) + ( le.orig_response_time_Invalid << 6 ) + ( le.orig_data_source_Invalid << 8 ) + ( le.orig_glb_purpose_Invalid << 10 ) + ( le.orig_report_options_Invalid << 12 ) + ( le.orig_unused_Invalid << 14 ) + ( le.orig_login_history_id_Invalid << 16 ) + ( le.orig_aseid_Invalid << 18 ) + ( le.orig_years_Invalid << 20 ) + ( le.orig_ip_address_Invalid << 22 ) + ( le.orig_source_code_Invalid << 23 ) + ( le.orig_retail_price_Invalid << 25 );
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.orig_billing_code_Invalid := (le.ScrubsBits1 >> 3) & 1;
    SELF.orig_transaction_id_Invalid := (le.ScrubsBits1 >> 4) & 3;
    SELF.orig_transaction_type_Invalid := (le.ScrubsBits1 >> 6) & 3;
    SELF.orig_neighbors_Invalid := (le.ScrubsBits1 >> 8) & 3;
    SELF.orig_relatives_Invalid := (le.ScrubsBits1 >> 10) & 3;
    SELF.orig_associates_Invalid := (le.ScrubsBits1 >> 12) & 3;
    SELF.orig_property_Invalid := (le.ScrubsBits1 >> 14) & 3;
    SELF.orig_company_id_Invalid := (le.ScrubsBits1 >> 16) & 3;
    SELF.orig_reference_code_Invalid := (le.ScrubsBits1 >> 18) & 1;
    SELF.orig_fname_Invalid := (le.ScrubsBits1 >> 19) & 3;
    SELF.orig_mname_Invalid := (le.ScrubsBits1 >> 21) & 3;
    SELF.orig_lname_Invalid := (le.ScrubsBits1 >> 23) & 3;
    SELF.orig_address_Invalid := (le.ScrubsBits1 >> 25) & 1;
    SELF.orig_city_Invalid := (le.ScrubsBits1 >> 26) & 3;
    SELF.orig_state_Invalid := (le.ScrubsBits1 >> 28) & 3;
    SELF.orig_zip_Invalid := (le.ScrubsBits1 >> 30) & 3;
    SELF.orig_zip4_Invalid := (le.ScrubsBits1 >> 32) & 3;
    SELF.orig_phone_Invalid := (le.ScrubsBits1 >> 34) & 3;
    SELF.orig_ssn_Invalid := (le.ScrubsBits1 >> 36) & 3;
    SELF.orig_free_Invalid := (le.ScrubsBits1 >> 38) & 3;
    SELF.orig_record_count_Invalid := (le.ScrubsBits1 >> 40) & 3;
    SELF.orig_price_Invalid := (le.ScrubsBits1 >> 42) & 1;
    SELF.orig_bankruptcy_Invalid := (le.ScrubsBits1 >> 43) & 3;
    SELF.orig_transaction_code_Invalid := (le.ScrubsBits1 >> 45) & 3;
    SELF.orig_dateadded_Invalid := (le.ScrubsBits1 >> 47) & 3;
    SELF.orig_full_name_Invalid := (le.ScrubsBits1 >> 49) & 3;
    SELF.orig_billingdate_Invalid := (le.ScrubsBits1 >> 51) & 3;
    SELF.orig_business_name_Invalid := (le.ScrubsBits1 >> 53) & 1;
    SELF.orig_pricing_error_code_Invalid := (le.ScrubsBits1 >> 54) & 3;
    SELF.orig_dl_purpose_Invalid := (le.ScrubsBits1 >> 56) & 3;
    SELF.orig_result_format_Invalid := (le.ScrubsBits1 >> 58) & 3;
    SELF.orig_dob_Invalid := (le.ScrubsBits1 >> 60) & 3;
    SELF.orig_unique_id_Invalid := (le.ScrubsBits1 >> 62) & 3;
    SELF.orig_dls_Invalid := (le.ScrubsBits2 >> 0) & 3;
    SELF.orig_mvs_Invalid := (le.ScrubsBits2 >> 2) & 3;
    SELF.orig_function_name_Invalid := (le.ScrubsBits2 >> 4) & 3;
    SELF.orig_response_time_Invalid := (le.ScrubsBits2 >> 6) & 3;
    SELF.orig_data_source_Invalid := (le.ScrubsBits2 >> 8) & 3;
    SELF.orig_glb_purpose_Invalid := (le.ScrubsBits2 >> 10) & 3;
    SELF.orig_report_options_Invalid := (le.ScrubsBits2 >> 12) & 3;
    SELF.orig_unused_Invalid := (le.ScrubsBits2 >> 14) & 3;
    SELF.orig_login_history_id_Invalid := (le.ScrubsBits2 >> 16) & 3;
    SELF.orig_aseid_Invalid := (le.ScrubsBits2 >> 18) & 3;
    SELF.orig_years_Invalid := (le.ScrubsBits2 >> 20) & 3;
    SELF.orig_ip_address_Invalid := (le.ScrubsBits2 >> 22) & 1;
    SELF.orig_source_code_Invalid := (le.ScrubsBits2 >> 23) & 3;
    SELF.orig_retail_price_Invalid := (le.ScrubsBits2 >> 25) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    orig_transaction_type_ALLOW_ErrorCount := COUNT(GROUP,h.orig_transaction_type_Invalid=1);
    orig_transaction_type_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_transaction_type_Invalid=1 AND h.orig_transaction_type_wouldClean);
    orig_transaction_type_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_transaction_type_Invalid=2);
    orig_transaction_type_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_transaction_type_Invalid=2 AND h.orig_transaction_type_wouldClean);
    orig_transaction_type_WORDS_ErrorCount := COUNT(GROUP,h.orig_transaction_type_Invalid=3);
    orig_transaction_type_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_transaction_type_Invalid=3 AND h.orig_transaction_type_wouldClean);
    orig_transaction_type_Total_ErrorCount := COUNT(GROUP,h.orig_transaction_type_Invalid>0);
    orig_neighbors_ALLOW_ErrorCount := COUNT(GROUP,h.orig_neighbors_Invalid=1);
    orig_neighbors_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_neighbors_Invalid=1 AND h.orig_neighbors_wouldClean);
    orig_neighbors_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_neighbors_Invalid=2);
    orig_neighbors_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_neighbors_Invalid=2 AND h.orig_neighbors_wouldClean);
    orig_neighbors_WORDS_ErrorCount := COUNT(GROUP,h.orig_neighbors_Invalid=3);
    orig_neighbors_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_neighbors_Invalid=3 AND h.orig_neighbors_wouldClean);
    orig_neighbors_Total_ErrorCount := COUNT(GROUP,h.orig_neighbors_Invalid>0);
    orig_relatives_ALLOW_ErrorCount := COUNT(GROUP,h.orig_relatives_Invalid=1);
    orig_relatives_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_relatives_Invalid=1 AND h.orig_relatives_wouldClean);
    orig_relatives_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_relatives_Invalid=2);
    orig_relatives_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_relatives_Invalid=2 AND h.orig_relatives_wouldClean);
    orig_relatives_WORDS_ErrorCount := COUNT(GROUP,h.orig_relatives_Invalid=3);
    orig_relatives_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_relatives_Invalid=3 AND h.orig_relatives_wouldClean);
    orig_relatives_Total_ErrorCount := COUNT(GROUP,h.orig_relatives_Invalid>0);
    orig_associates_ALLOW_ErrorCount := COUNT(GROUP,h.orig_associates_Invalid=1);
    orig_associates_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_associates_Invalid=1 AND h.orig_associates_wouldClean);
    orig_associates_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_associates_Invalid=2);
    orig_associates_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_associates_Invalid=2 AND h.orig_associates_wouldClean);
    orig_associates_WORDS_ErrorCount := COUNT(GROUP,h.orig_associates_Invalid=3);
    orig_associates_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_associates_Invalid=3 AND h.orig_associates_wouldClean);
    orig_associates_Total_ErrorCount := COUNT(GROUP,h.orig_associates_Invalid>0);
    orig_property_ALLOW_ErrorCount := COUNT(GROUP,h.orig_property_Invalid=1);
    orig_property_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_property_Invalid=1 AND h.orig_property_wouldClean);
    orig_property_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_property_Invalid=2);
    orig_property_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_property_Invalid=2 AND h.orig_property_wouldClean);
    orig_property_WORDS_ErrorCount := COUNT(GROUP,h.orig_property_Invalid=3);
    orig_property_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_property_Invalid=3 AND h.orig_property_wouldClean);
    ...
    orig_company_id_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_company_id_Invalid=2);
    orig_company_id_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_company_id_Invalid=2 AND h.orig_company_id_wouldClean);
    ...
    orig_phone_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_phone_Invalid=1);
    orig_phone_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_phone_Invalid=1 AND h.orig_phone_wouldClean);
    orig_phone_WORDS_ErrorCount := COUNT(GROUP,h.orig_phone_Invalid=2);
    orig_phone_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_phone_Invalid=2 AND h.orig_phone_wouldClean);
    ...
    orig_free_ALLOW_ErrorCount := COUNT(GROUP,h.orig_free_Invalid=1);
    orig_free_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_free_Invalid=1 AND h.orig_free_wouldClean);
    orig_free_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_free_Invalid=2);
    orig_free_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_free_Invalid=2 AND h.orig_free_wouldClean);
    orig_free_WORDS_ErrorCount := COUNT(GROUP,h.orig_free_Invalid=3);
    orig_free_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_free_Invalid=3 AND h.orig_free_wouldClean);
    ...
    orig_record_count_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_record_count_Invalid=2);
    orig_record_count_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_record_count_Invalid=2 AND h.orig_record_count_wouldClean);
    orig_record_count_WORDS_ErrorCount := COUNT(GROUP,h.orig_record_count_Invalid=3);
    orig_record_count_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_record_count_Invalid=3 AND h.orig_record_count_wouldClean);
    orig_record_count_Total_ErrorCount := COUNT(GROUP,h.orig_record_count_Invalid>0);
    orig_price_WORDS_ErrorCount := COUNT(GROUP,h.orig_price_Invalid=1);
    orig_price_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_price_Invalid=1 AND h.orig_price_wouldClean);
    orig_bankruptcy_ALLOW_ErrorCount := COUNT(GROUP,h.orig_bankruptcy_Invalid=1);
    orig_bankruptcy_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_bankruptcy_Invalid=1 AND h.orig_bankruptcy_wouldClean);
    orig_bankruptcy_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_bankruptcy_Invalid=2);
    orig_bankruptcy_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_bankruptcy_Invalid=2 AND h.orig_bankruptcy_wouldClean);
    orig_bankruptcy_WORDS_ErrorCount := COUNT(GROUP,h.orig_bankruptcy_Invalid=3);
    orig_bankruptcy_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_bankruptcy_Invalid=3 AND h.orig_bankruptcy_wouldClean);
    orig_bankruptcy_Total_ErrorCount := COUNT(GROUP,h.orig_bankruptcy_Invalid>0);
    orig_transaction_code_ALLOW_ErrorCount := COUNT(GROUP,h.orig_transaction_code_Invalid=1);
    orig_transaction_code_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_transaction_code_Invalid=1 AND h.orig_transaction_code_wouldClean);
    orig_transaction_code_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_transaction_code_Invalid=2);
    orig_transaction_code_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_transaction_code_Invalid=2 AND h.orig_transaction_code_wouldClean);
    orig_transaction_code_WORDS_ErrorCount := COUNT(GROUP,h.orig_transaction_code_Invalid=3);
    orig_transaction_code_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_transaction_code_Invalid=3 AND h.orig_transaction_code_wouldClean);
    orig_transaction_code_Total_ErrorCount := COUNT(GROUP,h.orig_transaction_code_Invalid>0);
    orig_dateadded_ALLOW_ErrorCount := COUNT(GROUP,h.orig_dateadded_Invalid=1);
    orig_dateadded_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_dateadded_Invalid=1 AND h.orig_dateadded_wouldClean);
    orig_dateadded_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_dateadded_Invalid=2);
    orig_dateadded_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_dateadded_Invalid=2 AND h.orig_dateadded_wouldClean);
    orig_dateadded_WORDS_ErrorCount := COUNT(GROUP,h.orig_dateadded_Invalid=3);
    orig_dateadded_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_dateadded_Invalid=3 AND h.orig_dateadded_wouldClean);
    ...
    orig_billingdate_ALLOW_ErrorCount := COUNT(GROUP,h.orig_billingdate_Invalid=1);
    orig_billingdate_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_billingdate_Invalid=1 AND h.orig_billingdate_wouldClean);
    orig_billingdate_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_billingdate_Invalid=2);
    orig_billingdate_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_billingdate_Invalid=2 AND h.orig_billingdate_wouldClean);
    orig_billingdate_WORDS_ErrorCount := COUNT(GROUP,h.orig_billingdate_Invalid=3);
    orig_billingdate_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_billingdate_Invalid=3 AND h.orig_billingdate_wouldClean);
    ...
    orig_pricing_error_code_ALLOW_ErrorCount := COUNT(GROUP,h.orig_pricing_error_code_Invalid=1);
    orig_pricing_error_code_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_pricing_error_code_Invalid=1 AND h.orig_pricing_error_code_wouldClean);
    orig_pricing_error_code_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_pricing_error_code_Invalid=2);
    orig_pricing_error_code_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_pricing_error_code_Invalid=2 AND h.orig_pricing_error_code_wouldClean);
    orig_pricing_error_code_WORDS_ErrorCount := COUNT(GROUP,h.orig_pricing_error_code_Invalid=3);
    orig_pricing_error_code_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_pricing_error_code_Invalid=3 AND h.orig_pricing_error_code_wouldClean);
    orig_pricing_error_code_Total_ErrorCount := COUNT(GROUP,h.orig_pricing_error_code_Invalid>0);
    orig_dl_purpose_ALLOW_ErrorCount := COUNT(GROUP,h.orig_dl_purpose_Invalid=1);
    orig_dl_purpose_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_dl_purpose_Invalid=1 AND h.orig_dl_purpose_wouldClean);
    orig_dl_purpose_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_dl_purpose_Invalid=2);
    orig_dl_purpose_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_dl_purpose_Invalid=2 AND h.orig_dl_purpose_wouldClean);
    orig_dl_purpose_WORDS_ErrorCount := COUNT(GROUP,h.orig_dl_purpose_Invalid=3);
    orig_dl_purpose_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_dl_purpose_Invalid=3 AND h.orig_dl_purpose_wouldClean);
    orig_dl_purpose_Total_ErrorCount := COUNT(GROUP,h.orig_dl_purpose_Invalid>0);
    orig_result_format_ALLOW_ErrorCount := COUNT(GROUP,h.orig_result_format_Invalid=1);
    orig_result_format_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_result_format_Invalid=1 AND h.orig_result_format_wouldClean);
    orig_result_format_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_result_format_Invalid=2);
    orig_result_format_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_result_format_Invalid=2 AND h.orig_result_format_wouldClean);
    orig_result_format_WORDS_ErrorCount := COUNT(GROUP,h.orig_result_format_Invalid=3);
    orig_result_format_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_result_format_Invalid=3 AND h.orig_result_format_wouldClean);
    ...
    orig_dob_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_dob_Invalid=1 AND h.orig_dob_wouldClean);
    ...
    orig_dls_ALLOW_ErrorCount := COUNT(GROUP,h.orig_dls_Invalid=1);
    orig_dls_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_dls_Invalid=1 AND h.orig_dls_wouldClean);
    orig_dls_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_dls_Invalid=2);
    orig_dls_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_dls_Invalid=2 AND h.orig_dls_wouldClean);
    orig_dls_WORDS_ErrorCount := COUNT(GROUP,h.orig_dls_Invalid=3);
    orig_dls_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_dls_Invalid=3 AND h.orig_dls_wouldClean);
    orig_dls_Total_ErrorCount := COUNT(GROUP,h.orig_dls_Invalid>0);
    orig_mvs_ALLOW_ErrorCount := COUNT(GROUP,h.orig_mvs_Invalid=1);
    orig_mvs_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_mvs_Invalid=1 AND h.orig_mvs_wouldClean);
    orig_mvs_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_mvs_Invalid=2);
    orig_mvs_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_mvs_Invalid=2 AND h.orig_mvs_wouldClean);
    orig_mvs_WORDS_ErrorCount := COUNT(GROUP,h.orig_mvs_Invalid=3);
    orig_mvs_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_mvs_Invalid=3 AND h.orig_mvs_wouldClean);
    ...
    orig_response_time_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_response_time_Invalid=1);
    orig_response_time_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_response_time_Invalid=1 AND h.orig_response_time_wouldClean);
    orig_response_time_WORDS_ErrorCount := COUNT(GROUP,h.orig_response_time_Invalid=2);
    orig_response_time_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_response_time_Invalid=2 AND h.orig_response_time_wouldClean);
    orig_response_time_Total_ErrorCount := COUNT(GROUP,h.orig_response_time_Invalid>0);
    orig_data_source_ALLOW_ErrorCount := COUNT(GROUP,h.orig_data_source_Invalid=1);
    orig_data_source_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_data_source_Invalid=1 AND h.orig_data_source_wouldClean);
    orig_data_source_WORDS_ErrorCount := COUNT(GROUP,h.orig_data_source_Invalid=2);
    orig_data_source_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_data_source_Invalid=2 AND h.orig_data_source_wouldClean);
    orig_data_source_Total_ErrorCount := COUNT(GROUP,h.orig_data_source_Invalid>0);
    orig_glb_purpose_ALLOW_ErrorCount := COUNT(GROUP,h.orig_glb_purpose_Invalid=1);
    orig_glb_purpose_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_glb_purpose_Invalid=1 AND h.orig_glb_purpose_wouldClean);
    orig_glb_purpose_WORDS_ErrorCount := COUNT(GROUP,h.orig_glb_purpose_Invalid=2);
    orig_glb_purpose_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_glb_purpose_Invalid=2 AND h.orig_glb_purpose_wouldClean);
    ...
    orig_unused_ALLOW_ErrorCount := COUNT(GROUP,h.orig_unused_Invalid=1);
    orig_unused_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_unused_Invalid=1 AND h.orig_unused_wouldClean);
    orig_unused_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_unused_Invalid=2);
    orig_unused_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_unused_Invalid=2 AND h.orig_unused_wouldClean);
    orig_unused_WORDS_ErrorCount := COUNT(GROUP,h.orig_unused_Invalid=3);
    orig_unused_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_unused_Invalid=3 AND h.orig_unused_wouldClean);
    orig_unused_Total_ErrorCount := COUNT(GROUP,h.orig_unused_Invalid>0);
    orig_login_history_id_ALLOW_ErrorCount := COUNT(GROUP,h.orig_login_history_id_Invalid=1);
    orig_login_history_id_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_login_history_id_Invalid=1 AND h.orig_login_history_id_wouldClean);
    orig_login_history_id_WORDS_ErrorCount := COUNT(GROUP,h.orig_login_history_id_Invalid=2);
    orig_login_history_id_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_login_history_id_Invalid=2 AND h.orig_login_history_id_wouldClean);
    orig_login_history_id_Total_ErrorCount := COUNT(GROUP,h.orig_login_history_id_Invalid>0);
    orig_aseid_ALLOW_ErrorCount := COUNT(GROUP,h.orig_aseid_Invalid=1);
    orig_aseid_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_aseid_Invalid=1 AND h.orig_aseid_wouldClean);
    orig_aseid_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_aseid_Invalid=2);
    orig_aseid_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_aseid_Invalid=2 AND h.orig_aseid_wouldClean);
    orig_aseid_WORDS_ErrorCount := COUNT(GROUP,h.orig_aseid_Invalid=3);
    orig_aseid_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_aseid_Invalid=3 AND h.orig_aseid_wouldClean);
    orig_aseid_Total_ErrorCount := COUNT(GROUP,h.orig_aseid_Invalid>0);
    orig_years_ALLOW_ErrorCount := COUNT(GROUP,h.orig_years_Invalid=1);
    orig_years_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_years_Invalid=1 AND h.orig_years_wouldClean);
    orig_years_LENGTHS_ErrorCount := COUNT(GROUP,h.orig_years_Invalid=2);
    orig_years_LENGTHS_WouldModifyCount := COUNT(GROUP,h.orig_years_Invalid=2 AND h.orig_years_wouldClean);
    orig_years_WORDS_ErrorCount := COUNT(GROUP,h.orig_years_Invalid=3);
    orig_years_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_years_Invalid=3 AND h.orig_years_wouldClean);
    orig_years_Total_ErrorCount := COUNT(GROUP,h.orig_years_Invalid>0);
    orig_ip_address_WORDS_ErrorCount := COUNT(GROUP,h.orig_ip_address_Invalid=1);
    orig_ip_address_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_ip_address_Invalid=1 AND h.orig_ip_address_wouldClean);
    orig_source_code_ALLOW_ErrorCount := COUNT(GROUP,h.orig_source_code_Invalid=1);
    orig_source_code_ALLOW_WouldModifyCount := COUNT(GROUP,h.orig_source_code_Invalid=1 AND h.orig_source_code_wouldClean);
    orig_source_code_WORDS_ErrorCount := COUNT(GROUP,h.orig_source_code_Invalid=2);
    orig_source_code_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_source_code_Invalid=2 AND h.orig_source_code_wouldClean);
    orig_source_code_Total_ErrorCount := COUNT(GROUP,h.orig_source_code_Invalid>0);
    orig_retail_price_WORDS_ErrorCount := COUNT(GROUP,h.orig_retail_price_Invalid=1);
    orig_retail_price_WORDS_WouldModifyCount := COUNT(GROUP,h.orig_retail_price_Invalid=1 AND h.orig_retail_price_wouldClean);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.orig_end_user_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_loginid_Total_ErrorCount > 0, 1, 0) + IF(le.orig_billing_code_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_transaction_id_Total_ErrorCount > 0, 1, 0) + IF(le.orig_transaction_type_Total_ErrorCount > 0, 1, 0) + IF(le.orig_neighbors_Total_ErrorCount > 0, 1, 0) + IF(le.orig_relatives_Total_ErrorCount > 0, 1, 0) + IF(le.orig_associates_Total_ErrorCount > 0, 1, 0) + IF(le.orig_property_Total_ErrorCount > 0, 1, 0) + IF(le.orig_company_id_Total_ErrorCount > 0, 1, 0) + IF(le.orig_reference_code_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_fname_Total_ErrorCount > 0, 1, 0) + IF(le.orig_mname_Total_ErrorCount > 0, 1, 0) + IF(le.orig_lname_Total_ErrorCount > 0, 1, 0) + IF(le.orig_address_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_city_Total_ErrorCount > 0, 1, 0) + IF(le.orig_state_Total_ErrorCount > 0, 1, 0) + IF(le.orig_zip_Total_ErrorCount > 0, 1, 0) + IF(le.orig_zip4_Total_ErrorCount > 0, 1, 0) + IF(le.orig_phone_Total_ErrorCount > 0, 1, 0) + IF(le.orig_ssn_Total_ErrorCount > 0, 1, 0) + IF(le.orig_free_Total_ErrorCount > 0, 1, 0) + IF(le.orig_record_count_Total_ErrorCount > 0, 1, 0) + IF(le.orig_price_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_bankruptcy_Total_ErrorCount > 0, 1, 0) + IF(le.orig_transaction_code_Total_ErrorCount > 0, 1, 0) + IF(le.orig_dateadded_Total_ErrorCount > 0, 1, 0) + IF(le.orig_full_name_Total_ErrorCount > 0, 1, 0) + IF(le.orig_billingdate_Total_ErrorCount > 0, 1, 0) + IF(le.orig_business_name_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_pricing_error_code_Total_ErrorCount > 0, 1, 0) + IF(le.orig_dl_purpose_Total_ErrorCount > 0, 1, 0) + IF(le.orig_result_format_Total_ErrorCount > 0, 1, 0) + IF(le.orig_dob_Total_ErrorCount > 0, 1, 0) + IF(le.orig_unique_id_Total_ErrorCount > 0, 1, 0) + IF(le.orig_dls_Total_ErrorCount > 0, 1, 0) + IF(le.orig_mvs_Total_ErrorCount > 0, 1, 0) + IF(le.orig_function_name_Total_ErrorCount > 0, 1, 0) + IF(le.orig_response_time_Total_ErrorCount > 0, 1, 0) + IF(le.orig_data_source_Total_ErrorCount > 0, 1, 0) + IF(le.orig_glb_purpose_Total_ErrorCount > 0, 1, 0) + IF(le.orig_report_options_Total_ErrorCount > 0, 1, 0) + IF(le.orig_unused_Total_ErrorCount > 0, 1, 0) + IF(le.orig_login_history_id_Total_ErrorCount > 0, 1, 0) + IF(le.orig_aseid_Total_ErrorCount > 0, 1, 0) + IF(le.orig_years_Total_ErrorCount > 0, 1, 0) + IF(le.orig_ip_address_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_source_code_Total_ErrorCount > 0, 1, 0) + IF(le.orig_retail_price_WORDS_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.orig_end_user_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_loginid_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_loginid_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_billing_code_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_transaction_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_transaction_id_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_transaction_type_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_transaction_type_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_transaction_type_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_neighbors_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_neighbors_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_neighbors_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_relatives_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_relatives_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_relatives_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_associates_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_associates_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_associates_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_property_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_property_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_property_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_company_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_company_id_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_company_id_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_reference_code_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_fname_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_fname_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_mname_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_mname_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_lname_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_lname_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_address_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_city_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_city_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_state_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_state_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_state_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_zip_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_zip_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_zip_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_zip4_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_zip4_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_zip4_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_phone_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_phone_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_ssn_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_ssn_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_ssn_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_free_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_free_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_free_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_record_count_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_record_count_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_record_count_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_price_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_bankruptcy_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_bankruptcy_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_bankruptcy_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_transaction_code_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_transaction_code_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_transaction_code_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_dateadded_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_dateadded_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_dateadded_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_full_name_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_full_name_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_billingdate_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_billingdate_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_billingdate_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_business_name_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_pricing_error_code_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_pricing_error_code_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_pricing_error_code_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_dl_purpose_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_dl_purpose_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_dl_purpose_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_result_format_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_result_format_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_result_format_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_dob_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_dob_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_dob_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_unique_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_unique_id_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_dls_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_dls_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_dls_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_mvs_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_mvs_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_mvs_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_function_name_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_function_name_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_response_time_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_response_time_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_data_source_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_data_source_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_glb_purpose_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_glb_purpose_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_report_options_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_report_options_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_unused_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_unused_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_unused_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_login_history_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_login_history_id_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_aseid_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_aseid_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_aseid_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_years_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_years_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.orig_years_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_ip_address_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_source_code_ALLOW_ErrorCount > 0, 1, 0) + IF(le.orig_source_code_WORDS_ErrorCount > 0, 1, 0) + IF(le.orig_retail_price_WORDS_ErrorCount > 0, 1, 0);
    SELF.Rules_NoErrors := NumRules - SELF.Rules_WithErrors;
    SELF.Rules_WithEdits := IF(le.orig_end_user_id_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_loginid_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_loginid_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_billing_code_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_transaction_id_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_transaction_id_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_transaction_type_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_transaction_type_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_transaction_type_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_neighbors_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_neighbors_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_neighbors_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_relatives_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_relatives_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_relatives_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_associates_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_associates_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_associates_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_property_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_property_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_property_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_company_id_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_company_id_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_company_id_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_reference_code_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_fname_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_fname_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_mname_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_mname_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_lname_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_lname_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_address_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_city_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_city_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_state_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_state_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_state_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_zip_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_zip_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_zip_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_zip4_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_zip4_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_zip4_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_phone_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_phone_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_ssn_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_ssn_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_ssn_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_free_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_free_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_free_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_record_count_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_record_count_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_record_count_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_price_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_bankruptcy_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_bankruptcy_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_bankruptcy_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_transaction_code_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_transaction_code_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_transaction_code_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_dateadded_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_dateadded_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_dateadded_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_full_name_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_full_name_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_billingdate_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_billingdate_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_billingdate_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_business_name_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_pricing_error_code_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_pricing_error_code_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_pricing_error_code_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_dl_purpose_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_dl_purpose_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_dl_purpose_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_result_format_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_result_format_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_result_format_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_dob_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_dob_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_dob_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_unique_id_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_unique_id_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_dls_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_dls_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_dls_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_mvs_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_mvs_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_mvs_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_function_name_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_function_name_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_response_time_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_response_time_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_data_source_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_data_source_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_glb_purpose_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_glb_purpose_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_report_options_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_report_options_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_unused_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_unused_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_unused_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_login_history_id_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_login_history_id_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_aseid_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_aseid_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_aseid_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_years_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_years_LENGTHS_WouldModifyCount > 0, 1, 0) + IF(le.orig_years_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_ip_address_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_source_code_ALLOW_WouldModifyCount > 0, 1, 0) + IF(le.orig_source_code_WORDS_WouldModifyCount > 0, 1, 0) + IF(le.orig_retail_price_WORDS_WouldModifyCount > 0, 1, 0);
    SELF := le;
  END;
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.orig_billing_code_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.orig_transaction_id_Invalid,'ALLOW','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_transaction_type_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_neighbors_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_relatives_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_associates_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_property_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_company_id_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ...
          ,CHOOSE(le.orig_phone_Invalid,'LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_ssn_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_free_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_record_count_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_price_Invalid,'WORDS','UNKNOWN')
          ,CHOOSE(le.orig_bankruptcy_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_transaction_code_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_dateadded_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_full_name_Invalid,'ALLOW','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_billingdate_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_business_name_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.orig_pricing_error_code_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_dl_purpose_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_result_format_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_dob_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_unique_id_Invalid,'ALLOW','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_dls_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_mvs_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_function_name_Invalid,'ALLOW','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_response_time_Invalid,'LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_data_source_Invalid,'ALLOW','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_glb_purpose_Invalid,'ALLOW','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_report_options_Invalid,'ALLOW','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_unused_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_login_history_id_Invalid,'ALLOW','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_aseid_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_years_Invalid,'ALLOW','LENGTHS','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_ip_address_Invalid,'WORDS','UNKNOWN')
          ,CHOOSE(le.orig_source_code_Invalid,'ALLOW','WORDS','UNKNOWN')
          ,CHOOSE(le.orig_retail_price_Invalid,'WORDS','UNKNOWN'),'UNKNOWN'));
  ...
  // field error stats
    SALT39.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'orig_billing_code:orig_billing_code:ALLOW'
          ,'orig_transaction_id:orig_transaction_id:ALLOW','orig_transaction_id:orig_transaction_id:WORDS'
          ,'orig_transaction_type:orig_transaction_type:ALLOW','orig_transaction_type:orig_transaction_type:LENGTHS','orig_transaction_type:orig_transaction_type:WORDS'
          ,'orig_neighbors:orig_neighbors:ALLOW','orig_neighbors:orig_neighbors:LENGTHS','orig_neighbors:orig_neighbors:WORDS'
          ,'orig_relatives:orig_relatives:ALLOW','orig_relatives:orig_relatives:LENGTHS','orig_relatives:orig_relatives:WORDS'
          ,'orig_associates:orig_associates:ALLOW','orig_associates:orig_associates:LENGTHS','orig_associates:orig_associates:WORDS'
          ,'orig_property:orig_property:ALLOW','orig_property:orig_property:LENGTHS','orig_property:orig_property:WORDS'
          ,'orig_company_id:orig_company_id:ALLOW','orig_company_id:orig_company_id:LENGTHS','orig_company_id:orig_company_id:WORDS'
          ...
          ,'orig_phone:orig_phone:LENGTHS','orig_phone:orig_phone:WORDS'
          ,'orig_ssn:orig_ssn:ALLOW','orig_ssn:orig_ssn:LENGTHS','orig_ssn:orig_ssn:WORDS'
          ,'orig_free:orig_free:ALLOW','orig_free:orig_free:LENGTHS','orig_free:orig_free:WORDS'
          ,'orig_record_count:orig_record_count:ALLOW','orig_record_count:orig_record_count:LENGTHS','orig_record_count:orig_record_count:WORDS'
          ,'orig_price:orig_price:WORDS'
          ,'orig_bankruptcy:orig_bankruptcy:ALLOW','orig_bankruptcy:orig_bankruptcy:LENGTHS','orig_bankruptcy:orig_bankruptcy:WORDS'
          ,'orig_transaction_code:orig_transaction_code:ALLOW','orig_transaction_code:orig_transaction_code:LENGTHS','orig_transaction_code:orig_transaction_code:WORDS'
          ,'orig_dateadded:orig_dateadded:ALLOW','orig_dateadded:orig_dateadded:LENGTHS','orig_dateadded:orig_dateadded:WORDS'
          ,'orig_full_name:orig_full_name:ALLOW','orig_full_name:orig_full_name:WORDS'
          ,'orig_billingdate:orig_billingdate:ALLOW','orig_billingdate:orig_billingdate:LENGTHS','orig_billingdate:orig_billingdate:WORDS'
          ,'orig_business_name:orig_business_name:ALLOW'
          ,'orig_pricing_error_code:orig_pricing_error_code:ALLOW','orig_pricing_error_code:orig_pricing_error_code:LENGTHS','orig_pricing_error_code:orig_pricing_error_code:WORDS'
          ,'orig_dl_purpose:orig_dl_purpose:ALLOW','orig_dl_purpose:orig_dl_purpose:LENGTHS','orig_dl_purpose:orig_dl_purpose:WORDS'
          ,'orig_result_format:orig_result_format:ALLOW','orig_result_format:orig_result_format:LENGTHS','orig_result_format:orig_result_format:WORDS'
          ,'orig_dob:orig_dob:ALLOW','orig_dob:orig_dob:LENGTHS','orig_dob:orig_dob:WORDS'
          ,'orig_unique_id:orig_unique_id:ALLOW','orig_unique_id:orig_unique_id:WORDS'
          ,'orig_dls:orig_dls:ALLOW','orig_dls:orig_dls:LENGTHS','orig_dls:orig_dls:WORDS'
          ,'orig_mvs:orig_mvs:ALLOW','orig_mvs:orig_mvs:LENGTHS','orig_mvs:orig_mvs:WORDS'
          ,'orig_function_name:orig_function_name:ALLOW','orig_function_name:orig_function_name:WORDS'
          ,'orig_response_time:orig_response_time:LENGTHS','orig_response_time:orig_response_time:WORDS'
          ,'orig_data_source:orig_data_source:ALLOW','orig_data_source:orig_data_source:WORDS'
          ,'orig_glb_purpose:orig_glb_purpose:ALLOW','orig_glb_purpose:orig_glb_purpose:WORDS'
          ,'orig_report_options:orig_report_options:ALLOW','orig_report_options:orig_report_options:WORDS'
          ,'orig_unused:orig_unused:ALLOW','orig_unused:orig_unused:LENGTHS','orig_unused:orig_unused:WORDS'
          ,'orig_login_history_id:orig_login_history_id:ALLOW','orig_login_history_id:orig_login_history_id:WORDS'
          ,'orig_aseid:orig_aseid:ALLOW','orig_aseid:orig_aseid:LENGTHS','orig_aseid:orig_aseid:WORDS'
          ,'orig_years:orig_years:ALLOW','orig_years:orig_years:LENGTHS','orig_years:orig_years:WORDS'
          ,'orig_ip_address:orig_ip_address:WORDS'
          ,'orig_source_code:orig_source_code:ALLOW','orig_source_code:orig_source_code:WORDS'
          ,'orig_retail_price:orig_retail_price:WORDS'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_orig_billing_code(1)
          ,Fields.InvalidMessage_orig_transaction_id(1),Fields.InvalidMessage_orig_transaction_id(2)
          ,Fields.InvalidMessage_orig_transaction_type(1),Fields.InvalidMessage_orig_transaction_type(2),Fields.InvalidMessage_orig_transaction_type(3)
          ,Fields.InvalidMessage_orig_neighbors(1),Fields.InvalidMessage_orig_neighbors(2),Fields.InvalidMessage_orig_neighbors(3)
          ,Fields.InvalidMessage_orig_relatives(1),Fields.InvalidMessage_orig_relatives(2),Fields.InvalidMessage_orig_relatives(3)
          ,Fields.InvalidMessage_orig_associates(1),Fields.InvalidMessage_orig_associates(2),Fields.InvalidMessage_orig_associates(3)
          ,Fields.InvalidMessage_orig_property(1),Fields.InvalidMessage_orig_property(2),Fields.InvalidMessage_orig_property(3)
          ...
          ,Fields.InvalidMessage_orig_free(1),Fields.InvalidMessage_orig_free(2),Fields.InvalidMessage_orig_free(3)
          ,Fields.InvalidMessage_orig_record_count(1),Fields.InvalidMessage_orig_record_count(2),Fields.InvalidMessage_orig_record_count(3)
          ,Fields.InvalidMessage_orig_price(1)
          ,Fields.InvalidMessage_orig_bankruptcy(1),Fields.InvalidMessage_orig_bankruptcy(2),Fields.InvalidMessage_orig_bankruptcy(3)
          ,Fields.InvalidMessage_orig_transaction_code(1),Fields.InvalidMessage_orig_transaction_code(2),Fields.InvalidMessage_orig_transaction_code(3)
          ,Fields.InvalidMessage_orig_dateadded(1),Fields.InvalidMessage_orig_dateadded(2),Fields.InvalidMessage_orig_dateadded(3)
          ,Fields.InvalidMessage_orig_full_name(1),Fields.InvalidMessage_orig_full_name(2)
          ,Fields.InvalidMessage_orig_billingdate(1),Fields.InvalidMessage_orig_billingdate(2),Fields.InvalidMessage_orig_billingdate(3)
          ,Fields.InvalidMessage_orig_business_name(1)
          ,Fields.InvalidMessage_orig_pricing_error_code(1),Fields.InvalidMessage_orig_pricing_error_code(2),Fields.InvalidMessage_orig_pricing_error_code(3)
          ,Fields.InvalidMessage_orig_dl_purpose(1),Fields.InvalidMessage_orig_dl_purpose(2),Fields.InvalidMessage_orig_dl_purpose(3)
          ,Fields.InvalidMessage_orig_result_format(1),Fields.InvalidMessage_orig_result_format(2),Fields.InvalidMessage_orig_result_format(3)
          ...
          ,Fields.InvalidMessage_orig_dls(1),Fields.InvalidMessage_orig_dls(2),Fields.InvalidMessage_orig_dls(3)
          ,Fields.InvalidMessage_orig_mvs(1),Fields.InvalidMessage_orig_mvs(2),Fields.InvalidMessage_orig_mvs(3)
          ,Fields.InvalidMessage_orig_function_name(1),Fields.InvalidMessage_orig_function_name(2)
          ,Fields.InvalidMessage_orig_response_time(1),Fields.InvalidMessage_orig_response_time(2)
          ,Fields.InvalidMessage_orig_data_source(1),Fields.InvalidMessage_orig_data_source(2)
          ...
          ,Fields.InvalidMessage_orig_unused(1),Fields.InvalidMessage_orig_unused(2),Fields.InvalidMessage_orig_unused(3)
          ,Fields.InvalidMessage_orig_login_history_id(1),Fields.InvalidMessage_orig_login_history_id(2)
          ,Fields.InvalidMessage_orig_aseid(1),Fields.InvalidMessage_orig_aseid(2),Fields.InvalidMessage_orig_aseid(3)
          ,Fields.InvalidMessage_orig_years(1),Fields.InvalidMessage_orig_years(2),Fields.InvalidMessage_orig_years(3)
          ,Fields.InvalidMessage_orig_ip_address(1)
          ,Fields.InvalidMessage_orig_source_code(1),Fields.InvalidMessage_orig_source_code(2)
          ,Fields.InvalidMessage_orig_retail_price(1)
          ,'Fields with errors'
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.orig_billing_code_ALLOW_ErrorCount
          ,le.orig_transaction_id_ALLOW_ErrorCount,le.orig_transaction_id_WORDS_ErrorCount
          ,le.orig_transaction_type_ALLOW_ErrorCount,le.orig_transaction_type_LENGTHS_ErrorCount,le.orig_transaction_type_WORDS_ErrorCount
          ,le.orig_neighbors_ALLOW_ErrorCount,le.orig_neighbors_LENGTHS_ErrorCount,le.orig_neighbors_WORDS_ErrorCount
          ,le.orig_relatives_ALLOW_ErrorCount,le.orig_relatives_LENGTHS_ErrorCount,le.orig_relatives_WORDS_ErrorCount
          ,le.orig_associates_ALLOW_ErrorCount,le.orig_associates_LENGTHS_ErrorCount,le.orig_associates_WORDS_ErrorCount
          ,le.orig_property_ALLOW_ErrorCount,le.orig_property_LENGTHS_ErrorCount,le.orig_property_WORDS_ErrorCount
          ,le.orig_company_id_ALLOW_ErrorCount,le.orig_company_id_LENGTHS_ErrorCount,le.orig_company_id_WORDS_ErrorCount
          ...
          ,le.orig_phone_LENGTHS_ErrorCount,le.orig_phone_WORDS_ErrorCount
          ,le.orig_ssn_ALLOW_ErrorCount,le.orig_ssn_LENGTHS_ErrorCount,le.orig_ssn_WORDS_ErrorCount
          ,le.orig_free_ALLOW_ErrorCount,le.orig_free_LENGTHS_ErrorCount,le.orig_free_WORDS_ErrorCount
          ,le.orig_record_count_ALLOW_ErrorCount,le.orig_record_count_LENGTHS_ErrorCount,le.orig_record_count_WORDS_ErrorCount
          ,le.orig_price_WORDS_ErrorCount
          ,le.orig_bankruptcy_ALLOW_ErrorCount,le.orig_bankruptcy_LENGTHS_ErrorCount,le.orig_bankruptcy_WORDS_ErrorCount
          ,le.orig_transaction_code_ALLOW_ErrorCount,le.orig_transaction_code_LENGTHS_ErrorCount,le.orig_transaction_code_WORDS_ErrorCount
          ,le.orig_dateadded_ALLOW_ErrorCount,le.orig_dateadded_LENGTHS_ErrorCount,le.orig_dateadded_WORDS_ErrorCount
          ,le.orig_full_name_ALLOW_ErrorCount,le.orig_full_name_WORDS_ErrorCount
          ,le.orig_billingdate_ALLOW_ErrorCount,le.orig_billingdate_LENGTHS_ErrorCount,le.orig_billingdate_WORDS_ErrorCount
          ,le.orig_business_name_ALLOW_ErrorCount
          ,le.orig_pricing_error_code_ALLOW_ErrorCount,le.orig_pricing_error_code_LENGTHS_ErrorCount,le.orig_pricing_error_code_WORDS_ErrorCount
          ,le.orig_dl_purpose_ALLOW_ErrorCount,le.orig_dl_purpose_LENGTHS_ErrorCount,le.orig_dl_purpose_WORDS_ErrorCount
          ,le.orig_result_format_ALLOW_ErrorCount,le.orig_result_format_LENGTHS_ErrorCount,le.orig_result_format_WORDS_ErrorCount
          ,le.orig_dob_ALLOW_ErrorCount,le.orig_dob_LENGTHS_ErrorCount,le.orig_dob_WORDS_ErrorCount
          ,le.orig_unique_id_ALLOW_ErrorCount,le.orig_unique_id_WORDS_ErrorCount
          ,le.orig_dls_ALLOW_ErrorCount,le.orig_dls_LENGTHS_ErrorCount,le.orig_dls_WORDS_ErrorCount
          ,le.orig_mvs_ALLOW_ErrorCount,le.orig_mvs_LENGTHS_ErrorCount,le.orig_mvs_WORDS_ErrorCount
          ,le.orig_function_name_ALLOW_ErrorCount,le.orig_function_name_WORDS_ErrorCount
          ,le.orig_response_time_LENGTHS_ErrorCount,le.orig_response_time_WORDS_ErrorCount
          ,le.orig_data_source_ALLOW_ErrorCount,le.orig_data_source_WORDS_ErrorCount
          ,le.orig_glb_purpose_ALLOW_ErrorCount,le.orig_glb_purpose_WORDS_ErrorCount
          ,le.orig_report_options_ALLOW_ErrorCount,le.orig_report_options_WORDS_ErrorCount
          ,le.orig_unused_ALLOW_ErrorCount,le.orig_unused_LENGTHS_ErrorCount,le.orig_unused_WORDS_ErrorCount
          ,le.orig_login_history_id_ALLOW_ErrorCount,le.orig_login_history_id_WORDS_ErrorCount
          ,le.orig_aseid_ALLOW_ErrorCount,le.orig_aseid_LENGTHS_ErrorCount,le.orig_aseid_WORDS_ErrorCount
          ,le.orig_years_ALLOW_ErrorCount,le.orig_years_LENGTHS_ErrorCount,le.orig_years_WORDS_ErrorCount
          ,le.orig_ip_address_WORDS_ErrorCount
          ,le.orig_source_code_ALLOW_ErrorCount,le.orig_source_code_WORDS_ErrorCount
          ,le.orig_retail_price_WORDS_ErrorCount
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ...
          ,le.orig_ip_address_WORDS_ErrorCount
          ,le.orig_source_code_ALLOW_ErrorCount,le.orig_source_code_WORDS_ErrorCount
          ,le.orig_retail_price_WORDS_ErrorCount,0) / le.TotalCnt + 0.5, CHOOSE(c - NumRules
...
EXPORT StandardStats(DATASET(Layout_FILE) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  ...
  SALT39.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Inql_Nonfcra_Accurint, Fields, 'RECORDOF(scrubsSummaryOverall)', '');

...
inFile := DeltabaseGateway.File_Deltabase_Gateway.Historic_Results_Raw;
	...
	DeltabaseGateway.Layout_Deltabase_Gateway.Historic_Results_Base fixFields(inFile l):= transform
		...
		self.optin_timestamp 						:= DeltabaseGateway._Functions.rmNull(trim(l.optin_timestamp, left, right));
		...
		self.cnm_presentation_indicator := DeltabaseGateway._Functions.rmNull(trim(l.cnm_presentation_indicator, left, right));
			...
			rmNull 	:= DeltabaseGateway._Functions.rmNull(trim(l.port_date, left, right));
			pullNum := stringlib.stringfilter(rmNull, '0123456789'); 
		
		self.port_date 									:= pullNum[1..8];
		self.port_time									:= pullNum[9..];	
	...
	ddConcat 		:= dedup(sort(distribute(concatFile, hash(submitted_phonenumber)), submitted_phonenumber, source, -(date_added+time_added), -transaction_id, local), submitted_phonenumber, source, date_file_loaded, local);//keep latest record

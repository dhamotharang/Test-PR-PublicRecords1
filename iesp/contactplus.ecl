...
export t_ContactPlusSearchBy := record
	...
	iesp.progressivephones.t_DedupeInfo DedupeInfo {xpath('DedupeInfo')};
end;
		
export t_ContactPlusDOB := record (iesp.share.t_Date)
	integer Age {xpath('Age')};
end;
		
export t_ContactPlusAddress := record (iesp.share.t_AddressEx)
	iesp.share.t_Date DateFirstSeen {xpath('DateFirstSeen')};
	iesp.share.t_Date DateLastSeen {xpath('DateLastSeen')};
	iesp.share.t_Date UtilityConnectDate {xpath('UtilityConnectDate')};
...
export t_ContactPlusProgPhoneRecord := record (iesp.progressivephones.t_ProgressivePhonesSearchRecord)
...
export t_ContactPlusSearchRecord := record
	string UniqueId {xpath('UniqueId')};
	dataset(iesp.share.t_NameWithGender) Names {xpath('Names/Name'), MAXCOUNT(iesp.Constants.Contact_Plus.MaxCountNameRecords)};
	dataset(iesp.share.t_StringArrayItem) SSNs {xpath('SSNs/SSN'), MAXCOUNT(iesp.Constants.Contact_Plus.MaxCountSSNRecords)};
...
export t_ContactPlusSearchRequest := record (iesp.share.t_BaseRequest)

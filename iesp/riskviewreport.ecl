...
export t_RiskViewReportOptions := record (share.t_BaseOption)
	riskview_share.t_RiskViewModels IncludeModels {xpath('IncludeModels')};
	string50 IntendedPurpose {xpath('IntendedPurpose')}; //values['Application','Prescreening','Portfolio Review','Insurance Application','Insurance Portfolio Review','Disclosure','Government','Collections','Tenant Screening','Healthcare Credit Transaction','Healthcare Legitimate Business Need','Rental Car Loss Damage Waiver','']
...
export t_RvReportScoreHRI := record
	...
	dataset(share.t_SequencedRiskIndicator) HighRiskIndicators {xpath('HighRiskIndicators/HighRiskIndicator'), MAXCOUNT(8)};
end;
...
export t_RiskViewReportRequest := record (share.t_BaseRequest)

...
EXPORT Build_Bases_tmRenewal(
	...
	workingUpdate := PROJECT(pIntmRenewal, standardize_input(LEFT));

	// Combine base and update career to determine what's new.
	combined 			:= workingUpdate + pBasetmRenewal;
	...
	basetmRenewal := ROLLUP( combined_sort,
	...
	// Return
	tools.mac_WriteFile(Corp2_Raw_CO.Filenames(pversion).Base.tmRenewal.New, basetmRenewal, Build_Base);

	EXPORT All := IF(tools.fun_IsValidVersion(pversion),
											Build_Base,
											OUTPUT('No Valid version parameter passed, skipping Corp2_Raw_CO.Build_Bases_tmRenewal attribute')
									 );

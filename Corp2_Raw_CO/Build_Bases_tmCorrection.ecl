...
EXPORT Build_Bases_tmCorrection(
	STRING																						pfiledate,
	STRING																						ptmfiledate,	
	STRING																						pversion,
	BOOLEAN																						puseprod,	
	...
	workingUpdate := PROJECT(pIntmCorrection, standardize_input(LEFT));

	// Combine base and update career to determine what's new.
	combined 			:= workingUpdate + pBasetmCorrection;
	...
	basetmCorrection := ROLLUP(	combined_sort,
	...
	// Return
	tools.mac_WriteFile(Corp2_Raw_CO.Filenames(pversion).Base.tmCorrection.New, basetmCorrection, Build_Base);

	EXPORT All := IF(tools.fun_IsValidVersion(pversion),
											Build_Base,
											OUTPUT('No Valid version parameter passed, skipping Corp2_Raw_CO.Build_Bases_tmCorrection attribute')
									 );

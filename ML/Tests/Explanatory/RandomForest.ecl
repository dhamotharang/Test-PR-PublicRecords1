...
//learner := ML.Classify.RandomForest(100, 5, 1.0, 100, FALSE);  // GiniSplit = FALSE uses Info Gain Ratio as split criteria. 71.3% Accuracy
learner := ML.Classify.RandomForest(250, 5, 1.0, 30, FALSE);  // GiniSplit = FALSE uses Info Gain Ratio as split criteria. 91% Accuracy
...
AUC0:= ML.Classify.AUC_ROC(ClassDist, 0, depData); //Area under ROC Curve for class "0"

...
progressive_phone.layout_progressive_batch_out_sid %get_blue_phones%(%blue_phone_recs% l, %gong_history_phone_key% r) := transform
...
progressive_phone.layout_progressive_batch_out_sid %by_addr_lname%(%f_to_match% l,
...
progressive_phone.layout_progressive_batch_out_sid %trans_did%(f_b_did l,%gong_did% r) := transform
  self.acctno := (string20)l.seq;
  self.subj_first :=l.fname;
  self.subj_middle := l.mname;
  self.subj_last := l.lname;
...
#uniquename(f_combined_optout)
#uniquename(f_combined)
#uniquename(f_b_out_dep)
%f_combined_optout% := Suppress.MAC_SuppressSource(%f_b_out_from_blue% + %f_b_out_ready% + %f_did%, mod_access, key_did);
%f_combined% := PROJECT(%f_combined_optout%, progressive_phone.layout_progressive_batch_out_with_did);
%f_b_out_dep% := dedup(sort(%f_combined%, acctno, subj_phone10, -subj_date_last, -subj_date_first, -subj_phone_date_last, -(p_did IN %f_b_did_set%)), acctno, subj_phone10);

...
progressive_phone.mac_get_blue(f_c_did, %blue_recs%, true, true, false, mod_access);
...
#uniquename(optout_tmp_rec)
%optout_tmp_rec% := record
  %batch_out_ext_rec%;
  unsigned6 key_did := 0;
...
%optout_tmp_rec% %get_type_c_raw%(%f_c_to_match% l,
...
#uniquename(pre_f_c_out_found_precheck)
#uniquename(f_c_out_found_precheck_optout)
#uniquename(f_c_out_found_precheck)
%pre_f_c_out_found_precheck% := join(%f_c_to_match%, %gong_addr_key%,
...
%f_c_out_found_precheck_optout% := Suppress.MAC_SuppressSource(%pre_f_c_out_found_precheck%, mod_access, key_did);
%f_c_out_found_precheck% := PROJECT(%f_c_out_found_precheck_optout%, %batch_out_ext_rec%);
...
#uniquename(pre_f_c_out_input)
#uniquename(f_c_out_input_optout)
#uniquename(f_c_out_input)
%pre_f_c_out_input% := join(%f_c_in%, %gong_addr_key%,
  keyed(left.prim_name = right.prim_name) and
  keyed(left.zip = right.z5) and
  keyed(left.prim_range = right.prim_range) and
  keyed(left.st = right.st) and right.current_flag = true and
...
%f_c_out_input_optout% := Suppress.MAC_SuppressSource(%pre_f_c_out_input%, mod_access);
%f_c_out_input% := PROJECT(%f_c_out_found_precheck_optout%, %batch_out_ext_rec%);

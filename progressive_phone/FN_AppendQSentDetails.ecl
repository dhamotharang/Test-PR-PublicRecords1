IMPORT BatchServices, iesp, Address, Gateway, AutoStandardI, Doxie_Raw,Royalty;
EXPORT FN_AppendQSentDetails(dataset(progressive_phone.layout_progressive_batch_in) f_in_raw,
	...
	layout_hist_phone get_hist_phone(f_in_raw l, unsigned1 cnt) := transform
		...
		self.hist_phone := map(cnt=1 => l.phoneno_1,
													 cnt=2 => l.phoneno_2,
													 cnt=3 => l.phoneno_3,
													 cnt=4 => l.phoneno_4, 
													 cnt=5 => l.phoneno_5,
	...
	gw_out := project(f_in_raw, getGateway(LEFT));
	...
	TrimUpper(string input) := TRIM(StringLib.StringToUpperCase(input),LEFT,RIGHT);
	progressive_phone.layout_progressive_phone_batch_with_details_out batchit(gw_out_flat L) := transform
		...
		self.subj_name_dual := TrimUpper(StringLib.StringCleanSpaces(L.lname + ' ' + L.fname + ' '+ L.mname));
		...
		self.address :=  	TrimUpper(StringLib.StringCleanSpaces(L.prim_range+ ' ' + 
											L.predir +' ' + 
											L.prim_name +' ' + 
											L.suffix + ' ' + 
											L.postdir + ' ' + 
		...
		self.operating_company_address := TrimUpper(StringLib.StringCleanSpaces(
		...
		self.operating_company_zip := TrimUpper(L.RealTimePhone_Ext.OperatingCompany.address.zip5 + 
																						L.RealTimePhone_Ext.OperatingCompany.address.zip4);
		self.operating_company_phone := TrimUpper(L.RealTimePhone_Ext.OperatingCompany.PhoneInfo.PhoneNPA +
																							L.RealTimePhone_Ext.OperatingCompany.PhoneInfo.PhoneNXX +
																							L.RealTimePhone_Ext.OperatingCompany.PhoneInfo.PhoneLine);
		self.operating_company_fax := TrimUpper(L.RealTimePhone_Ext.OperatingCompany.PhoneInfo.FaxNPA +
																						L.RealTimePhone_Ext.OperatingCompany.PhoneInfo.FaxNXX +
																						L.RealTimePhone_Ext.OperatingCompany.PhoneInfo.FaxLine);
		self.operating_company_contact := TrimUpper(if (length(trim(L.RealTimePhone_Ext.OperatingCompany.contact.name.fullname)) > 0 ,
		...
		self.company_contact_address := TrimUpper(StringLib.StringCleanSpaces(
		...
		self.company_contact_zip := TrimUpper(L.RealTimePhone_Ext.OperatingCompany.contact.address.zip5 +
	...
	gw_out_qsent := project(gw_out_flat, batchit(LEFT));

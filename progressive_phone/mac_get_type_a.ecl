export mac_get_type_a(f_a_acctno, f_a_out, use_seq=false) := macro

import STD, gong, ut, NID, progressive_phone, Person_Models;	
...
progressive_phone.layout_progressive_batch_out_with_did %by_gong_address%(f_a_acctno l, %gong_addr_key% r, unsigned sub_rule_num) := transform
...
%f_a_out_1% := join(f_a_acctno, %gong_addr_key%,
			     ...
			     (left.sec_range <> '' and NID.mod_PFirstTools.PFLeqPFR(left.fname,right.name_first)))) 
			     or 
...
progressive_phone.layout_progressive_batch_out_with_did %get_2_only%(%f_a_out_2% l) := transform
	self := l;
end;
			   

#uniquename(f_a_out_ready)
%f_a_out_ready% := %f_a_out_1% + join(%f_a_out_2%, %f_a_out_1%,
                                      left.acctno=right.acctno,
			  		             %get_2_only%(left), left only);

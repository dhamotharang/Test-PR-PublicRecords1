...
export progressive_phone_batch_with_details_service := macro
  ...
  gateways_in := Gateway.Configuration.Get();
	...
	f_in_raw := dataset([],progressive_phone.layout_progressive_batch_in) : stored('batch_in',few);
		...
		f_out := PROJECT(addrbest.Progressive_phone_common(f_in_raw, 
																											 ,
																											 ,
																											gateways_in, 
																											, 
																											...
																											MaxNumSpouse,
																											MaxNumSubject,
																											MaxNumNeighbor), progressive_phone.layout_progressive_batch_out_with_did);
		
	ug_f_out := ungroup(f_out);
	header_recs_w_ssn := progressive_phone.fn_addSSN(ug_f_out);

	/*--- Join back to original result, conserving main method for populating fields ---*/
	progressive_phone.layout_progressive_batch_out_with_did addssn(progressive_phone.layout_progressive_batch_out_with_did l, header_recs_w_ssn r) := transform
	...
	// WITHOUT QSENT
	results_without_qsent := project(f_out_ssn,transform(progressive_phone.layout_progressive_phone_batch_with_details_out,self.did := intformat(left.did,12,1),self.p_did := intformat(left.p_did,12,1),self := left, self := []));
	...
	results_with_qsent := progressive_phone.FN_AppendQSentDetails(f_in_raw, ungroup(results_without_qsent), QSENTV2_GW, QSentV2_SearchType);
...
// progressive_phone.progressive_phone_batch_with_details_service();
/*
<batch_in>
<row>
<acctno>1</acctno>

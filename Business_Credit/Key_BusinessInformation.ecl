...
EXPORT	Key_BusinessInformation(STRING pVersion	=	(STRING8)Std.Date.Today(),
	...
	rBusinessInformation	:=	RECORD	
		...
		STRING2		source;
		//DF-26180 Add CCPA fields to thor_data400::key::sbfe::qa::businessinformation
		UNSIGNED4	global_sid 			:= 0;
		UNSIGNED8   record_sid 			:= 0;
	END;
	
	dBusinessInformation			:=	PROJECT(dLinked,TRANSFORM(rBusinessInformation,
	...
	rBusinessInformation	tFillDates(rBusinessInformation	L,	rBusinessInformation	R)	:=	TRANSFORM
	...
	dKeyResult			:=	IF(	pBuildType	= Constants().buildType.Daily,
													dBIWithDates(Version=pVersion),
													dBIWithDates);

	RETURN	INDEX(dKeyResult,{Sbfe_Contributor_Number,Contract_Account_Number,Account_Type_Reported},{dKeyResult},Business_Credit.keynames().BusinessInformation.QA);
END;
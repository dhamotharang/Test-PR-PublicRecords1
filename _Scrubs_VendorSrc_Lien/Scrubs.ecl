...
// The module to handle the case where no scrubs exist
  ...
  EXPORT Expanded_Layout := RECORD(Layout_Lien)
  ...
  END;
  EXPORT  Bitmap_Layout := RECORD(Layout_Lien)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(Layout_Lien) h) := MODULE
  ...
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Layout_Lien);
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,Layout_Lien);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(Layout_Lien) prevDS = DATASET([], Layout_Lien), STRING10 Src='UNK'):= FUNCTION
  // field error stats
    ...
    mod_hygiene := hygiene(PROJECT(h, Layout_Lien));
  ...
  // record count stats
    ...
    mod_Delta := Delta(prevDS, PROJECT(h, Layout_Lien));
...
EXPORT StandardStats(DATASET(Layout_Lien) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  myTimeStamp := (UNSIGNED6)SALT311.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
  expandedFile := FromNone(inFile).ExpandedInfile;
  mod_fromexpandedOverall := FromExpanded(expandedFile);
  scrubsSummaryOverall := mod_fromexpandedOverall.SummaryStats;
 
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(_Scrubs_VendorSrc_Lien, Fields, 'RECORDOF(scrubsSummaryOverall)', '');

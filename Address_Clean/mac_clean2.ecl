...
EXPORT mac_clean2(inData, UseIndexThreshold=5000000, addr1_field, addr2_with_city_st_zip_field='', addr2_field='', incity='', instate='', inzip='') := FUNCTIONMACRO
    ...
    string AddressLine2 :=
		...
		#ELSE
		   TRIM(l.incity) + ', ' + TRIM(l.instate) + ' ' + TRIM(l.inzip)
		#END
		;
  ...
  results_out :=RECORD
	 rFinal AND NOT [seq, addx1, addx2];
	END;

	Results := PROJECT(cacheHits + cleanerCall, TRANSFORM(results_out, self := LEFT));

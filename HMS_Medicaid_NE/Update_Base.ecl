...
EXPORT Update_Base (string filedate, boolean pUseProd = false) := MODULE
	...
	EXPORT Clean_name (pBaseFile, pLayout_base, useFull = false) := FUNCTIONMACRO
		NID.Mac_CleanFullNames(pBaseFile, CleanNames/*cleaned_name_output*/, Name);
	...
	EXPORT _Base := FUNCTION
		...
		//hist_base	:= Mark_history(HMS_Medicaid_NE.Files(filedate,pUseProd).base.built, HMS_Medicaid_NE.layouts.base);		
		std_input	:= HMS_Medicaid_NE.StandardizeInputFile(filedate,pUseProd).base;
		cleanNames := Clean_name(std_input,HMS_Medicaid_Common.Layouts.base_NE);
// cleanNames;		
		cleanAdd_a	:= Clean_addr(cleanNames, HMS_Medicaid_Common.Layouts.base_NE):PERSIST('~thor400_data::Persist::base::hms::Medicaid::NE::addresses');	
		//cleanAdd_a := Dataset('~thor400_data::Persist::base::hms::Medicaid::NE::addresses',HMS_Medicaid_Common.Layouts.base_NE,THOR);
//CleanAdd_a;

	base_and_update := if(nothor(FileServices.GetSuperFileSubCount(HMS_Medicaid_Common.Filenames('IL',filedate, pUseProd).lBaseTemplate_built)) = 0
													 ,cleanAdd_a
													 ,cleanAdd_a );//+ hist_base); 
														
	new_base_d := distribute(base_and_update, hash(Name,License_Number,NPI,ProviderType,Speciality,Address1,OfficeCity,OfficeState,OfficeZipCode)); 
	...
	HMS_Medicaid_Common.Layouts.base_NE t_rollup (new_base_s  le, new_base_s ri) := transform
	...
	base_f := rollup(new_base_s,
									...
									and left.ProviderType = right.ProviderType
									and left.Speciality = right.Speciality
									and left.Address1 = right.Address1
									and left.OfficeCity = right.OfficeCity
									and left.OfficeState = right.OfficeState
									and left.OfficeZipCode = right.OfficeZipCode
									,t_rollup(left, right)
									,local);
	HMS_Medicaid_Common.Layouts.base_NE  GetSourceRID(base_f L)	:= TRANSFORM
  ...
  d_rid:=project(base_f,GetSourceRID(left));
	...
	did_add.MAC_Match_Flex
		(d_rid, matchset,foo					
		 ...
		 DID, HMS_Medicaid_Common.Layouts.base_NE, true, did_score,
		 75, d_did);
	...
	d_dob:=project(d_dob0
		...
		(
				 d_dob //d_dob0
				,bdid_matchset
				,Name			//Clinic Name,firmname					// Company name field
				,prim_range
				,prim_name
				,zip						//orig_zip
				,sec_range
				...
				,foo
				,bdid
				,HMS_Medicaid_Common.Layouts.base_NE
				,TRUE
				,bdid_score
				,d_bdid
				,
		...
		HMS_Medicaid_Common.Layouts.base_NE xform( MyData L,  NPPES.File_NPPES_Keybuild R) := Transform
			SELF.entity_type_code := 1;
			SELF := L;
		end;

		HMS_Medicaid_Common.Layouts.base_NE xformOrg( MyData L,  NPPES.File_NPPES_Keybuild R) := Transform

...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_orig_org_structure_cd_Invalid := (le.ScrubsBits1 >> 28) & 1;
    SELF.corp_orig_org_structure_desc_Invalid := (le.ScrubsBits1 >> 29) & 1;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_for_profit_ind:invalid_flag_code:ENUM','UNKNOWN');
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_corp_for_profit_ind(1),'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_for_profit_ind_ENUM_ErrorCount,0);
      SELF.rulepcnt := 100 * CHOOSE(c
          ...
          ,le.corp_orig_org_structure_cd_CUSTOM_ErrorCount
          ,le.corp_orig_org_structure_desc_CUSTOM_ErrorCount
          ,le.corp_for_profit_ind_ENUM_ErrorCount,0) / le.TotalCnt + 0.5;
    END;
    SummaryInfo := NORMALIZE(SummaryStats,31,Into(LEFT,COUNTER));
    orb_r := RECORD
      AllErrors.Src;

...
EXPORT RawFileNonFCRA_MAC_PopulationStatistics(infile,Ref='',Input_statement_id = '',Input_orig_fname = '',Input_orig_lname = '',Input_orig_mname = '',Input_orig_cname = '',Input_orig_address = '',Input_orig_city = '',Input_orig_st = '',Input_orig_zip = '',Input_orig_zip4 = '',Input_phone = '',Input_title = '',Input_fname = '',Input_mname = '',Input_lname = '',Input_name_suffix = '',Input_name_score = '',Input_prim_range = '',Input_predir = '',Input_prim_name = '',Input_addr_suffix = '',Input_postdir = '',Input_unit_desig = '',Input_sec_range = '',Input_p_city_name = '',Input_v_city_name = '',Input_st = '',Input_zip = '',Input_zip4 = '',Input_cart = '',Input_cr_sort_sz = '',Input_lot = '',Input_lot_order = '',Input_dbpc = '',Input_chk_digit = '',Input_rec_type = '',Input_county = '',Input_geo_lat = '',Input_geo_long = '',Input_msa = '',Input_geo_blk = '',Input_geo_match = '',Input_err_stat = '',Input_date_submitted = '',Input_date_created = '',Input_did = '',Input_consumer_text = '',Input_override_flag = '',OutFile) := MACRO
  ...
  %of% %ot%(infile le) := TRANSFORM
    SELF.fields :=
    #IF( #TEXT(Input_statement_id)='' )
      '' 
    #ELSE
        IF( le.Input_statement_id = (TYPEOF(le.Input_statement_id))'','',':statement_id')
    #END
...
+    #IF( #TEXT(Input_orig_cname)='' )
      '' 
    #ELSE
        IF( le.Input_orig_cname = (TYPEOF(le.Input_orig_cname))'','',':orig_cname')
    #END
...
+    #IF( #TEXT(Input_date_submitted)='' )
      '' 
    #ELSE
        IF( le.Input_date_submitted = (TYPEOF(le.Input_date_submitted))'','',':date_submitted')
    #END
...
+    #IF( #TEXT(Input_consumer_text)='' )
      '' 
    #ELSE
        IF( le.Input_consumer_text = (TYPEOF(le.Input_consumer_text))'','',':consumer_text')
    #END

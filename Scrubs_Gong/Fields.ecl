...
EXPORT MakeFT_RecordTypes(SALT33.StrType s0) := FUNCTION
  s1 := SALT33.stringfilter(s0,'BGPR'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_RecordTypes(SALT33.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'BGPR'))));
EXPORT InValidMessageFT_RecordTypes(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInChars('BGPR'),SALT33.HygieneErrors.Good);

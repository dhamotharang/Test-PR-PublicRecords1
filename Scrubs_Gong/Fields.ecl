...
EXPORT SALT311.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'alpha','phone','zip','zip4','date','date_alt','Invalid_Date','RecordTypes','PublishCodes','ActionCodes','YesNo','StateAbrv','Numeric','Directional','primname');
EXPORT FieldTypeNum(SALT311.StrType fn) := CASE(fn,'alpha' => 1,'phone' => 2,'zip' => 3,'zip4' => 4,'date' => 5,'date_alt' => 6,'Invalid_Date' => 7,'RecordTypes' => 8,'PublishCodes' => 9,'ActionCodes' => 10,'YesNo' => 11,'StateAbrv' => 12,'Numeric' => 13,'Directional' => 14,'primname' => 15,0);
...
EXPORT MakeFT_alpha(SALT311.StrType s0) := FUNCTION
  ...
  RETURN  s2;
END;
EXPORT InValidFT_alpha(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 <>{}[]-^=?!+&,./#()@\'`"_:;$*'))));
EXPORT InValidMessageFT_alpha(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 <>{}[]-^=?!+&,./#()@\'`"_:;$*'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_phone(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_phone(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789'))),~(LENGTH(TRIM(s)) = 0 OR LENGTH(TRIM(s)) = 10));
EXPORT InValidMessageFT_phone(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789'),SALT311.HygieneErrors.NotLength('0,10'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_zip(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_zip(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789'))),~(LENGTH(TRIM(s)) = 0 OR LENGTH(TRIM(s)) = 5));
EXPORT InValidMessageFT_zip(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789'),SALT311.HygieneErrors.NotLength('0,5'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_zip4(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_zip4(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789'))),~(LENGTH(TRIM(s)) = 0 OR LENGTH(TRIM(s)) = 4));
EXPORT InValidMessageFT_zip4(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789'),SALT311.HygieneErrors.NotLength('0,4'),SALT311.HygieneErrors.Good);
...
EXPORT MakeFT_date_alt(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'0123456789 '); // Only allow valid symbols
  s2 := SALT311.stringcleanspaces( SALT311.stringsubstituteout(s1,' ',' ') ); // Insert spaces but avoid doubles
  RETURN  s2;
END;
EXPORT InValidFT_date_alt(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789 '))),~(LENGTH(TRIM(s)) = 0 OR LENGTH(TRIM(s)) = 1 OR LENGTH(TRIM(s)) = 6 OR LENGTH(TRIM(s)) = 8));
EXPORT InValidMessageFT_date_alt(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789 '),SALT311.HygieneErrors.NotLength('0,1,6,8'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_Invalid_Date(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_Invalid_Date(SALT311.StrType s) := WHICH(~Scrubs.fn_valid_date(s,'F')>0);
...
EXPORT MakeFT_RecordTypes(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'BGPRLCSFZ'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_RecordTypes(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'BGPRLCSFZ'))));
EXPORT InValidMessageFT_RecordTypes(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('BGPRLCSFZ'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_PublishCodes(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'LUP'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_PublishCodes(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'LUP'))));
EXPORT InValidMessageFT_PublishCodes(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('LUP'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_ActionCodes(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'ADI'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_ActionCodes(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'ADI'))));
EXPORT InValidMessageFT_ActionCodes(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('ADI'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_YesNo(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'YN '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_YesNo(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'YN '))));
...
EXPORT MakeFT_Directional(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_Directional(SALT311.StrType s) := WHICH(((SALT311.StrType) s) NOT IN ['N','S','E','W','NW','Nw','SW','Sw','NE','Ne','SE','Se',' ']);
EXPORT InValidMessageFT_Directional(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInEnum('N|S|E|W|NW|Nw|SW|Sw|NE|Ne|SE|Se| '),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_primname(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 \'/.#&?;-'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_primname(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 \'/.#&?;-'))));
EXPORT InValidMessageFT_primname(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 \'/.#&?;-'),SALT311.HygieneErrors.Good);
...
EXPORT Make_ACTION_CODE(SALT311.StrType s0) := MakeFT_ActionCodes(s0);
EXPORT InValid_ACTION_CODE(SALT311.StrType s) := InValidFT_ActionCodes(s);
...
EXPORT Make_RECORD_TYPE(SALT311.StrType s0) := MakeFT_RecordTypes(s0);
EXPORT InValid_RECORD_TYPE(SALT311.StrType s) := InValidFT_RecordTypes(s);
EXPORT InValidMessage_RECORD_TYPE(UNSIGNED1 wh) := InValidMessageFT_RecordTypes(wh);
 
EXPORT Make_TELEPHONE(SALT311.StrType s0) := MakeFT_phone(s0);
EXPORT InValid_TELEPHONE(SALT311.StrType s) := InValidFT_phone(s);
EXPORT InValidMessage_TELEPHONE(UNSIGNED1 wh) := InValidMessageFT_phone(wh);
 
EXPORT Make_LISTING_TYPE(SALT311.StrType s0) := MakeFT_PublishCodes(s0);
EXPORT InValid_LISTING_TYPE(SALT311.StrType s) := InValidFT_PublishCodes(s);
EXPORT InValidMessage_LISTING_TYPE(UNSIGNED1 wh) := InValidMessageFT_PublishCodes(wh);
 
EXPORT Make_BUSINESS_NAME(SALT311.StrType s0) := s0;
EXPORT InValid_BUSINESS_NAME(SALT311.StrType s) := 0;
EXPORT InValidMessage_BUSINESS_NAME(UNSIGNED1 wh) := '';
 
EXPORT Make_BUSINESS_CAPTIONS(SALT311.StrType s0) := s0;
EXPORT InValid_BUSINESS_CAPTIONS(SALT311.StrType s) := 0;
...
EXPORT Make_INDENT(SALT311.StrType s0) := MakeFT_Numeric(s0);
EXPORT InValid_INDENT(SALT311.StrType s) := InValidFT_Numeric(s);
...
EXPORT Make_SUFFIX_NAME(SALT311.StrType s0) := s0;
EXPORT InValid_SUFFIX_NAME(SALT311.StrType s) := 0;
...
EXPORT Make_PRIMARY_STREET_NUMBER(SALT311.StrType s0) := s0;
EXPORT InValid_PRIMARY_STREET_NUMBER(SALT311.StrType s) := 0;
EXPORT InValidMessage_PRIMARY_STREET_NUMBER(UNSIGNED1 wh) := '';
 
EXPORT Make_PRE_DIR(SALT311.StrType s0) := MakeFT_Directional(s0);
EXPORT InValid_PRE_DIR(SALT311.StrType s) := InValidFT_Directional(s);
EXPORT InValidMessage_PRE_DIR(UNSIGNED1 wh) := InValidMessageFT_Directional(wh);
 
EXPORT Make_PRIMARY_STREET_NAME(SALT311.StrType s0) := MakeFT_primname(s0);
EXPORT InValid_PRIMARY_STREET_NAME(SALT311.StrType s) := InValidFT_primname(s);
EXPORT InValidMessage_PRIMARY_STREET_NAME(UNSIGNED1 wh) := InValidMessageFT_primname(wh);
 
EXPORT Make_PRIMARY_STREET_SUFFIX(SALT311.StrType s0) := s0;
EXPORT InValid_PRIMARY_STREET_SUFFIX(SALT311.StrType s) := 0;
EXPORT InValidMessage_PRIMARY_STREET_SUFFIX(UNSIGNED1 wh) := '';
 
EXPORT Make_POST_DIR(SALT311.StrType s0) := MakeFT_Directional(s0);
EXPORT InValid_POST_DIR(SALT311.StrType s) := InValidFT_Directional(s);
EXPORT InValidMessage_POST_DIR(UNSIGNED1 wh) := InValidMessageFT_Directional(wh);
 
EXPORT Make_SECONDARY_ADDRESS_TYPE(SALT311.StrType s0) := s0;
EXPORT InValid_SECONDARY_ADDRESS_TYPE(SALT311.StrType s) := 0;
EXPORT InValidMessage_SECONDARY_ADDRESS_TYPE(UNSIGNED1 wh) := '';
 
EXPORT Make_SECONDARY_RANGE(SALT311.StrType s0) := s0;
EXPORT InValid_SECONDARY_RANGE(SALT311.StrType s) := 0;
...
EXPORT Make_STATE(SALT311.StrType s0) := MakeFT_StateAbrv(s0);
EXPORT InValid_STATE(SALT311.StrType s) := InValidFT_StateAbrv(s);
EXPORT InValidMessage_STATE(UNSIGNED1 wh) := InValidMessageFT_StateAbrv(wh);
 
EXPORT Make_ZIP_CODE(SALT311.StrType s0) := MakeFT_zip(s0);
EXPORT InValid_ZIP_CODE(SALT311.StrType s) := InValidFT_zip(s);
EXPORT InValidMessage_ZIP_CODE(UNSIGNED1 wh) := InValidMessageFT_zip(wh);
 
EXPORT Make_ZIP_PLUS4(SALT311.StrType s0) := MakeFT_zip4(s0);
EXPORT InValid_ZIP_PLUS4(SALT311.StrType s) := InValidFT_zip4(s);
...
EXPORT Make_LAT_LONG_MATCH_LEVEL(SALT311.StrType s0) := s0;
EXPORT InValid_LAT_LONG_MATCH_LEVEL(SALT311.StrType s) := 0;
EXPORT InValidMessage_LAT_LONG_MATCH_LEVEL(UNSIGNED1 wh) := '';
 
EXPORT Make_UNLICENSED(SALT311.StrType s0) := s0;
EXPORT InValid_UNLICENSED(SALT311.StrType s) := 0;
EXPORT InValidMessage_UNLICENSED(UNSIGNED1 wh) := '';
 
EXPORT Make_ADD_DATE(SALT311.StrType s0) := MakeFT_Invalid_Date(s0);
EXPORT InValid_ADD_DATE(SALT311.StrType s) := InValidFT_Invalid_Date(s);
EXPORT InValidMessage_ADD_DATE(UNSIGNED1 wh) := InValidMessageFT_Invalid_Date(wh);
 
EXPORT Make_OMIT_ADDRESS(SALT311.StrType s0) := MakeFT_YesNo(s0);
EXPORT InValid_OMIT_ADDRESS(SALT311.StrType s) := InValidFT_YesNo(s);
EXPORT InValidMessage_OMIT_ADDRESS(UNSIGNED1 wh) := InValidMessageFT_YesNo(wh);
 
EXPORT Make_DATA_SOURCE(SALT311.StrType s0) := s0;
EXPORT InValid_DATA_SOURCE(SALT311.StrType s) := 0;
EXPORT InValidMessage_DATA_SOURCE(UNSIGNED1 wh) := '';
 
EXPORT Make_unknownField(SALT311.StrType s0) := s0;
EXPORT InValid_unknownField(SALT311.StrType s) := 0;
EXPORT InValidMessage_unknownField(UNSIGNED1 wh) := '';
 
EXPORT Make_TransactionID(SALT311.StrType s0) := s0;
EXPORT InValid_TransactionID(SALT311.StrType s) := 0;
EXPORT InValidMessage_TransactionID(UNSIGNED1 wh) := '';
 
EXPORT Make_Original_Suffix(SALT311.StrType s0) := s0;
EXPORT InValid_Original_Suffix(SALT311.StrType s) := 0;
EXPORT InValidMessage_Original_Suffix(UNSIGNED1 wh) := '';
 
EXPORT Make_Original_First_Name(SALT311.StrType s0) := s0;
EXPORT InValid_Original_First_Name(SALT311.StrType s) := 0;
EXPORT InValidMessage_Original_First_Name(UNSIGNED1 wh) := '';
 
EXPORT Make_Original_Middle_Name(SALT311.StrType s0) := s0;
EXPORT InValid_Original_Middle_Name(SALT311.StrType s) := 0;
EXPORT InValidMessage_Original_Middle_Name(UNSIGNED1 wh) := '';
 
EXPORT Make_Original_Last_Name(SALT311.StrType s0) := s0;
EXPORT InValid_Original_Last_Name(SALT311.StrType s) := 0;
EXPORT InValidMessage_Original_Last_Name(UNSIGNED1 wh) := '';
 
EXPORT Make_Original_Address(SALT311.StrType s0) := s0;
EXPORT InValid_Original_Address(SALT311.StrType s) := 0;
EXPORT InValidMessage_Original_Address(UNSIGNED1 wh) := '';
 
EXPORT Make_Original_Last_Line(SALT311.StrType s0) := s0;
EXPORT InValid_Original_Last_Line(SALT311.StrType s) := 0;

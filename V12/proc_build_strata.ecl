 ...
 basefile:=V12.files.V12_base;
  ...
  address_1_Cnt := sum(group,if(basefile.address_1<>'',1,0));
	address_2_Cnt := sum(group,if(basefile.address_2<>'',1,0));
  city_Cnt  := sum(group,if(basefile.city<>'',1,0));
  state_Cnt  := sum(group,if(basefile.state<>'',1,0));
  zip_codeCnt  := sum(group,if(basefile.zip_code<>'',1,0));
  optout_Cnt  := sum(group,if(basefile.optout = true,1,0));
	hardbounce_Cnt  := sum(group,if(basefile.hardbounce = true,1,0));
 ...
 strata.createXMLStats(basestrata,'V12 stats','data',version,'',strataResults);

...
shared BatchParams		:= IParam.getBatchParams();				 										 
 ...
 //---------------------------------------------------------------------------------------------------------//
   //IID
  export getIID(dataset(Layouts.BatchIn) BatchIn ) := function
	
	in_mod := module(project(BatchParams,BatchDatasets.IParams.BatchParams,opt))
				export unsigned1 DPPA_Purpose := BatchParams.DPPAPurpose;
				export unsigned1 GLB_Purpose := BatchParams.GLBPurpose;
				//export STRING5 industry_class_val := '';
				//export boolean ln_branded_value := BatchParams.LnBranded;
				export string Model_in := '';
				export string50 DataRestriction := BatchParams.DataRestrictionMask;
				export string50 DataPermission := BatchParams.DataPermissionMask;
				export boolean ExactSSNMatch := true;
				export boolean IncludeAllRiskIndicators := true;
		end;
		
		IIDBatchIn := project( BatchIn, transform(BatchDatasets.Layouts.layout_batch_in_waddr_status,
			self := left,
			self := []  ));
...
// Set parameter values for calling risk_indicators.reasonCodes( ) --see below.
		...
		Layouts.rec_instantid into_final(BatchDatasets.Layouts.layout_batch_in_waddr_status le, Risk_Indicators.Layout_Output ri) := 
			transform

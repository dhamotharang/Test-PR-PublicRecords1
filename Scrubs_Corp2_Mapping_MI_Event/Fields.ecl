...
EXPORT SALT30.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_future_date','invalid_charter_nbr','invalid_event_date_type_cd','invalid_event_date_type_desc');
EXPORT FieldTypeNum(SALT30.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_corp_vendor' => 2,'invalid_state_origin' => 3,'invalid_mandatory' => 4,'invalid_date' => 5,'invalid_future_date' => 6,'invalid_charter_nbr' => 7,'invalid_event_date_type_cd' => 8,'invalid_event_date_type_desc' => 9,0);
...
EXPORT MakeFT_invalid_event_date_type_cd(SALT30.StrType s0) := FUNCTION
  s1 := SALT30.stringfilter(s0,'FIL'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_event_date_type_cd(SALT30.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT30.StringFilter(s,'FIL'))));
EXPORT InValidMessageFT_invalid_event_date_type_cd(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInChars('FIL'),SALT30.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_event_date_type_desc(SALT30.StrType s0) := FUNCTION
  s1 := SALT30.stringfilter(s0,'FGILN'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_event_date_type_desc(SALT30.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT30.StringFilter(s,'FGILN'))));
EXPORT InValidMessageFT_invalid_event_date_type_desc(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInChars('FGILN'),SALT30.HygieneErrors.Good);
...
EXPORT Make_event_date_type_desc(SALT30.StrType s0) := MakeFT_invalid_event_date_type_desc(s0);
EXPORT InValid_event_date_type_desc(SALT30.StrType s) := InValidFT_invalid_event_date_type_desc(s);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT30,Scrubs_Corp2_Mapping_MI_Event;

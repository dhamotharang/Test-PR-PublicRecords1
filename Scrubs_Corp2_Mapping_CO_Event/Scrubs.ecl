...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.FieldType := CHOOSE(c,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_no','invalid_numericorblank','invalid_date','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT34.StrType)le.corp_key,(SALT34.StrType)le.corp_vendor,(SALT34.StrType)le.corp_state_origin,(SALT34.StrType)le.corp_process_date,(SALT34.StrType)le.corp_sos_charter_nbr,(SALT34.StrType)le.event_filing_reference_nbr,(SALT34.StrType)le.event_filing_date,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'event_filing_reference_nbr:invalid_numericorblank:ALLOW'
          ,'event_filing_date:invalid_date:ALLOW','event_filing_date:invalid_date:CUSTOM','UNKNOWN');
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_event_filing_date(1),Fields.InvalidMessage_event_filing_date(2),'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.event_filing_date_ALLOW_ErrorCount,le.event_filing_date_CUSTOM_ErrorCount,0);
      SELF.rulepcnt := 100 * CHOOSE(c
          ...
          ,le.corp_sos_charter_nbr_ALLOW_ErrorCount,le.corp_sos_charter_nbr_LENGTH_ErrorCount
          ,le.event_filing_reference_nbr_ALLOW_ErrorCount
          ,le.event_filing_date_ALLOW_ErrorCount,le.event_filing_date_CUSTOM_ErrorCount,0) / le.TotalCnt + 0.5;
    END;

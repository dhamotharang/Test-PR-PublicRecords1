...
EXPORT SALT34.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_charter_no','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_numericorblank');
EXPORT FieldTypeNum(SALT34.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_charter_no' => 2,'invalid_corp_vendor' => 3,'invalid_state_origin' => 4,'invalid_mandatory' => 5,'invalid_date' => 6,'invalid_numericorblank' => 7,0);
...
EXPORT MakeFT_invalid_numericorblank(SALT34.StrType s0) := FUNCTION
  s1 := SALT34.stringfilter(s0,' 0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_numericorblank(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,' 0123456789'))));
EXPORT InValidMessageFT_invalid_numericorblank(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars(' 0123456789'),SALT34.HygieneErrors.Good);
...
EXPORT Make_event_filing_reference_nbr(SALT34.StrType s0) := MakeFT_invalid_numericorblank(s0);
EXPORT InValid_event_filing_reference_nbr(SALT34.StrType s) := InValidFT_invalid_numericorblank(s);
EXPORT InValidMessage_event_filing_reference_nbr(UNSIGNED1 wh) := InValidMessageFT_invalid_numericorblank(wh);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT34,Scrubs_Corp2_Mapping_CO_Event;

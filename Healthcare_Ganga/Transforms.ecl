...
EXPORT Transforms := MODULE
 	EXPORT iesp.healthcare_identity.t_HealthCareIdentityScreeningResponse xformWeb (Healthcare_Ganga.Layouts.IdentityInput l,Healthcare_Ganga.Layouts.IdentityOutput R) := TRANSFORM
			self._Record.EntityType := l.EntityType;
			self._Header.TransactionId := R.TransactionId;
			self._Record.RecordIdentifier := l.RecordIdentifier;
			...
			self._Record.Business.TaxId:= R.TaxId;
			...
			self._Record.ResponseDateTime := R.ResponseDateTime;
			self._Record.NameValues := choosen(project(R.NameValues, transform(iesp.healthcare_identity.t_HealthCareIdentityScreeningNVP,
																																						self.name := left.name;
																																						self.value := left.value;)),iesp.Constants.Tista.MAX_NAMEVALUES);
			self._Record.Addresses := project(R,transform(iesp.healthcare_identity.t_HealthCareIdentityScreeningAddress,
																													...
																													self.Zip4 := left.Zip4;
																													self:=[]));
			self.Warnings := choosen(project(R.Warnings, transform(iesp.healthcare_identity.t_HealthCareIdentityScreeningWarning,
																											self.Code := left.Code;
																											self.Source := left.Source;)),iesp.Constants.Tista.MAX_WARNINGS);
			self := [];
	END;
	...
	EXPORT Healthcare_Ganga.Layouts.IdentityOutput xformCommon (Healthcare_Header_Services.Layouts.CombinedHeaderResultsDoxieLayout l,Healthcare_Ganga.Layouts.IdentityInput r) := TRANSFORM
					...
					self.RecordIdentifier := r.RecordIdentifier;
					self.RequestDateTime := r.RequestDateTime;

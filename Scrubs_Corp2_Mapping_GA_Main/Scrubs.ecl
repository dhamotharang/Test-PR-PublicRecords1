...
EXPORT FromNone(DATASET(Layout_in_file) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_address1_effective_date_Invalid := Fields.InValid_corp_address1_effective_date((SALT34.StrType)le.corp_address1_effective_date);
    SELF.corp_phone_number_Invalid := Fields.InValid_corp_phone_number((SALT34.StrType)le.corp_phone_number);
    ...
    SELF.corp_ra_phone_number_Invalid := Fields.InValid_corp_ra_phone_number((SALT34.StrType)le.corp_ra_phone_number);
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_ra_phone_number_Invalid := (le.ScrubsBits1 >> 40) & 3;
    SELF.corp_agent_assign_date_Invalid := (le.ScrubsBits1 >> 42) & 3;
    SELF.recordorigin_Invalid := (le.ScrubsBits1 >> 44) & 1;
    SELF := le;
  END;

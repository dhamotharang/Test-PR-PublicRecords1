...
EXPORT match_dt_vendor_first_reported_year(INTEGER2 L_year,INTEGER2 R_year) := MAP(
	L_year= R_year => SALT34.MatchCode.ExactMatch,
...
EXPORT match_dt_vendor_first_reported_month(INTEGER2 L_month,INTEGER2 R_month, INTEGER2 L_day,INTEGER2 R_day) := MAP(
	L_month = R_month => SALT34.MatchCode.ExactMatch,
	L_month = R_day AND L_day = R_month => SALT34.MatchCode.MonthDaySwitch, // Performing M-D switch
	L_day <= 1 AND L_month = 1 OR R_day <= 1 AND R_month = 1 => SALT34.MatchCode.SoftMatch, // Month may be a soft 1 if day is ... 
	SALT34.MatchCode.NoMatch);
EXPORT match_dt_vendor_first_reported_day(INTEGER2 L_month,INTEGER2 R_month, INTEGER2 L_day,INTEGER2 R_day) := MAP(
	L_day = R_day => SALT34.MatchCode.ExactMatch,
	L_month = R_day AND L_day = R_month => SALT34.MatchCode.MonthDaySwitch, // Performing M-D switch
	L_day = 1 OR R_day = 1 => SALT34.MatchCode.SoftMatch, // Treating as a 'soft' 1 
	SALT34.MatchCode.NoMatch);
EXPORT match_dt_vendor_first_reported(INTEGER2 L_year,INTEGER2 R_year, INTEGER1 L_month,INTEGER1 R_month,INTEGER1 L_day,INTEGER1 R_day) := MAP(
	L_year = R_year AND L_month = R_month AND L_day = R_day => SALT34.MatchCode.ExactMatch,
	L_month = R_day AND L_day = R_month => SALT34.MatchCode.MonthDaySwitch, // Performing M-D switch
	L_day = 1 OR R_day = 1 OR (L_day <= 1 AND L_month = 1) OR (R_day <= 1 AND R_month = 1) => SALT34.MatchCode.SoftMatch, // Treating as a 'soft' 1 
	SALT34.MatchCode.NoMatch);
...
EXPORT match_corp_ra_suffix(TYPEOF(h.corp_ra_suffix) L, TYPEOF(h.corp_ra_suffix) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_cont_suffix(TYPEOF(h.cont_suffix) L, TYPEOF(h.cont_suffix) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_action_pending_cd(TYPEOF(h.corp_action_pending_cd) L, TYPEOF(h.corp_action_pending_cd) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_action_pending_desc(TYPEOF(h.corp_action_pending_desc) L, TYPEOF(h.corp_action_pending_desc) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_agent_assign_date(TYPEOF(h.corp_agent_assign_date) L, TYPEOF(h.corp_agent_assign_date) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_agent_commercial(TYPEOF(h.corp_agent_commercial) L, TYPEOF(h.corp_agent_commercial) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_agent_status_cd(TYPEOF(h.corp_agent_status_cd) L, TYPEOF(h.corp_agent_status_cd) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_agent_status_desc(TYPEOF(h.corp_agent_status_desc) L, TYPEOF(h.corp_agent_status_desc) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_agent_id(TYPEOF(h.corp_agent_id) L, TYPEOF(h.corp_agent_id) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_comment(TYPEOF(h.corp_comment) L, TYPEOF(h.corp_comment) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_farm_status_cd(TYPEOF(h.corp_farm_status_cd) L, TYPEOF(h.corp_farm_status_cd) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_farm_status_desc(TYPEOF(h.corp_farm_status_desc) L, TYPEOF(h.corp_farm_status_desc) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_has_members(TYPEOF(h.corp_has_members) L, TYPEOF(h.corp_has_members) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_has_vested_managers(TYPEOF(h.corp_has_vested_managers) L, TYPEOF(h.corp_has_vested_managers) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_is_non_profit_irs_approved(TYPEOF(h.corp_is_non_profit_irs_approved) L, TYPEOF(h.corp_is_non_profit_irs_approved) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_llc_managed_desc(TYPEOF(h.corp_llc_managed_desc) L, TYPEOF(h.corp_llc_managed_desc) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_management_desc(TYPEOF(h.corp_management_desc) L, TYPEOF(h.corp_management_desc) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_merger_desc(TYPEOF(h.corp_merger_desc) L, TYPEOF(h.corp_merger_desc) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_merger_type_converted_to_cd(TYPEOF(h.corp_merger_type_converted_to_cd) L, TYPEOF(h.corp_merger_type_converted_to_cd) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_merger_type_converted_to_desc(TYPEOF(h.corp_merger_type_converted_to_desc) L, TYPEOF(h.corp_merger_type_converted_to_desc) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_naics_desc(TYPEOF(h.corp_naics_desc) L, TYPEOF(h.corp_naics_desc) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_name_reservation_desc(TYPEOF(h.corp_name_reservation_desc) L, TYPEOF(h.corp_name_reservation_desc) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_name_reservation_nbr(TYPEOF(h.corp_name_reservation_nbr) L, TYPEOF(h.corp_name_reservation_nbr) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_non_profit_irs_approved_purpose(TYPEOF(h.corp_non_profit_irs_approved_purpose) L, TYPEOF(h.corp_non_profit_irs_approved_purpose) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_non_profit_solicit_donations(TYPEOF(h.corp_non_profit_solicit_donations) L, TYPEOF(h.corp_non_profit_solicit_donations) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_nbr_of_amendments(TYPEOF(h.corp_nbr_of_amendments) L, TYPEOF(h.corp_nbr_of_amendments) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_nbr_of_initial_llc_members(TYPEOF(h.corp_nbr_of_initial_llc_members) L, TYPEOF(h.corp_nbr_of_initial_llc_members) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_nbr_of_partners(TYPEOF(h.corp_nbr_of_partners) L, TYPEOF(h.corp_nbr_of_partners) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_operating_agreement(TYPEOF(h.corp_operating_agreement) L, TYPEOF(h.corp_operating_agreement) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_opt_in_llc_act_desc(TYPEOF(h.corp_opt_in_llc_act_desc) L, TYPEOF(h.corp_opt_in_llc_act_desc) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_percentage_voters_required_to_approve_amendments(TYPEOF(h.corp_percentage_voters_required_to_approve_amendments) L, TYPEOF(h.corp_percentage_voters_required_to_approve_amendments) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_termination_cd(TYPEOF(h.corp_termination_cd) L, TYPEOF(h.corp_termination_cd) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_termination_desc(TYPEOF(h.corp_termination_desc) L, TYPEOF(h.corp_termination_desc) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_termination_date(TYPEOF(h.corp_termination_date) L, TYPEOF(h.corp_termination_date) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_trademark_class_desc1(TYPEOF(h.corp_trademark_class_desc1) L, TYPEOF(h.corp_trademark_class_desc1) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_trademark_class_desc2(TYPEOF(h.corp_trademark_class_desc2) L, TYPEOF(h.corp_trademark_class_desc2) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_trademark_class_desc3(TYPEOF(h.corp_trademark_class_desc3) L, TYPEOF(h.corp_trademark_class_desc3) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_trademark_class_desc4(TYPEOF(h.corp_trademark_class_desc4) L, TYPEOF(h.corp_trademark_class_desc4) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_trademark_class_desc5(TYPEOF(h.corp_trademark_class_desc5) L, TYPEOF(h.corp_trademark_class_desc5) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_trademark_class_desc6(TYPEOF(h.corp_trademark_class_desc6) L, TYPEOF(h.corp_trademark_class_desc6) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
EXPORT match_corp_trademark_classification_nbr(TYPEOF(h.corp_trademark_classification_nbr) L, TYPEOF(h.corp_trademark_classification_nbr) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);
...
EXPORT match_corp_trademark_nbr(TYPEOF(h.corp_trademark_nbr) L, TYPEOF(h.corp_trademark_nbr) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT34.MatchCode.ExactMatch,
    SALT34.MatchCode.NoMatch),
        MAP(L = R => SALT34.MatchCode.ExactMatch, SALT34.MatchCode.NoMatch)
);

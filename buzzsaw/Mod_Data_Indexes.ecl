export Mod_Data_Indexes := MODULE
...
//	ds := get_cluster_info();
	...
	*
	*
	* All IDX_U4_xxx should be re-written and integrated with  C_U4_EX
	*
	*/
...
// Changed slightly...
export IDX_U4_ips_Test      := INDEX(
			DS_U4_Idx
			, {srcip_u4, destip_u4, INTEGER4 tod := buzzsaw.Mod_Data_Util.f_tod(ms_start)}
			, {src,doc,
					RecPtr}
			, FN_Idx_U4_ips + '_TEST');		
			
export IDX_U4_destip_u4_TEST      := INDEX(
			DS_U4_Idx
			, {destip_u4, destport}
			, {
					src,doc,
					RecPtr}
			, FN_Idx_U4_destip_u4  + '_TEST');
			
			
	export IDX_U4_Time_Of_Day_TEST := INDEX(
			DS_U4_Idx
			, {INTEGER4 tod := buzzsaw.Mod_Data_Util.f_tod(ms_start),
				dest_cc
			},{
				src,doc,
				RecPtr
			}, FN_Idx_U4_Time_Of_Day + '_TEST');
						

export IDX_U4_destport_TEST := INDEX(
			DS_U4_Idx
			, {destport, sent_packets, rcv_packets, INTEGER4 tod := buzzsaw.Mod_Data_Util.f_tod(ms_start),
			},{src,doc, RecPtr
			}, FN_Idx_U4_DestPort  + '_TEST');
			
export IDX_U4_dest_cc_TEST := INDEX(
			DS_U4_Idx
			, {dest_cc, destport, 
			},{src,doc, RecPtr
			}, FN_Idx_U4_Dest_CC  + '_TEST');
			

export IDX_U4_src_cc_TEST := INDEX(
			DS_U4_Idx
			, {src_cc, destport, 
			},{src,doc, RecPtr
			}, FN_Idx_U4_Src_CC  + '_TEST');
			
export IDX_U4_protocol_TEST := INDEX(
			DS_U4_Idx
			, {protocol, destport, 
			},{src,doc, RecPtr
			}, FN_Idx_U4_Protocol  + '_TEST');			
	...
	//
	export IDX_U4 := INDEX(
						DATASET(Mod_data.FN_U4, {Mod_Data.L_Firewall_U4; UNSIGNED8 fpos{virtual(fileposition)}},	FLAT), 
						{src,doc}, 
						{ms_start, ms_stop, records, srcip, srcport, destip, destport, protocol, sent_packets,
							rcv_packets, duration, srcip_u4, destip_u4,  src_cc, dest_cc, firewall, fpos},
						Mod_data.FN_IDX_U4
						);
						
						
	// export IDX_Unique_Ips_Location := INDEX(
						// DATASET(Mod_data.FN_Unique_Ips_Location, {Mod_Data.L_Unique_IPs_Location; UNSIGNED8 fpos {virtual(fileposition)}},	FLAT), 
						// {ip_u4}, 
						// {country_code, fpos},
						// Mod_Data.FN_Idx_Unique_Ips_Location
						// );
						
	// export IDX_DDC_Srcip_u4 := INDEX(
						// DATASET(Mod_data.FN_ddc, {Mod_Data.L_Firewall_Packed; UNSIGNED8 fpos {virtual(fileposition)}}, FLAT),
						// {srcip_u4}, 
						// {src,doc, fpos},
						// Mod_Data.FN_IDX_DDC_Srcip_u4
						// );
						
	// export IDX_DDC_Destip_u4 := INDEX(
						// DATASET(Mod_data.FN_ddc, {Mod_Data.L_Firewall_Packed; UNSIGNED8 fpos {virtual(fileposition)}}, FLAT),
						// {destip_u4}, 
						// {src,doc, fpos},
						// Mod_Data.FN_IDX_DDC_Destip_u4
						// );						

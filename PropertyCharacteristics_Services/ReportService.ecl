...
export ReportService() := macro
  ...
  clean_addr := PropertyCharacteristics_Services.Get_Clean_Address(Request.ReportBy.AddressInfo);
	...
	iesp.property_info.t_PropertyInformation	tInvalidAcctPropResult()	:= transform
		...
		self.Report.ReportIdSection.OrderDate										:=	iesp.ECL2ESP.toDate((unsigned)ut.GetDate);
		self.Report.ReportIdSection.ReceiptDate									:=	iesp.ECL2ESP.toDate((unsigned)ut.GetDate);
		self.Report.ReportIdSection.CompletionDate							:=	iesp.ECL2ESP.toDate((unsigned)ut.GetDate);

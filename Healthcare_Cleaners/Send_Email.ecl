...
export Send_Email(string pversion, boolean pUseProd = false) := module
	shared SuccessSubject	:= if(VersionControl.IsValidVersion(pversion)
															,_Dataset(pUseProd).name +' Medschool '+ ' Build ' + pversion + ' Completed on ' + _Control.ThisEnvironment.Name
	...
	export BuildFailure		:= fileservices.sendemail(  
														...
														_Dataset(pUseProd).name +' Medschool '+ ' Build ' + pversion + ' Failed on '+ _Control.ThisEnvironment.Name,
														workunit + '\n' + failmessage  );
end;

...
EXPORT LNPIDCompareService := MACRO
  IMPORT SALT29,HealthCareProviderHeader_BEST,ut;
...
odl := PROJECT(CHOOSEN(HealthCareProviderHeader_BEST.Keys(BFile).Candidates(LNPID=LNPIDone),100000),HealthCareProviderHeader_BEST.match_candidates(BFile).layout_candidates);
odr := PROJECT(CHOOSEN(HealthCareProviderHeader_BEST.Keys(BFile).Candidates(LNPID=LNPIDTwo),100000),HealthCareProviderHeader_BEST.match_candidates(BFile).layout_candidates);
k := HealthCareProviderHeader_BEST.Keys(BFile).Specificities_Key;
s := GLOBAL(PROJECT(k,HealthCareProviderHeader_BEST.Layout_Specificities.R)[1]);
odlv := HealthCareProviderHeader_BEST.Debug(BFile,s).RolledEntities(odl);
odrv := HealthCareProviderHeader_BEST.Debug(BFile,s).RolledEntities(odr);
mtch := HealthCareProviderHeader_BEST.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,LNPIDone,LNPIDtwo,0,0}],HealthCareProviderHeader_BEST.match_candidates(BFile).layout_matches));

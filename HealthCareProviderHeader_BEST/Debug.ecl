...
EXPORT Layout_Sample_Matches := RECORD,MAXLENGTH(32000)
  ...
  BOOLEAN BILLING_NPI_NUMBER_skipped := FALSE; // True if FORCE blocks match
  ...
  BOOLEAN CNSMR_DOB_skipped := FALSE; // True if FORCE blocks match
  ...
  BOOLEAN SNAME_skipped := FALSE; // True if FORCE blocks match
  ...
  BOOLEAN DERIVED_GENDER_skipped := FALSE; // True if FORCE blocks match
...
EXPORT layout_sample_matches sample_match_join(match_candidates(ih).layout_candidates le,match_candidates(ih).layout_candidates ri,UNSIGNED outside=0) := TRANSFORM
  ...
  SELF.CNSMR_SSN_score := MAP( le.CNSMR_SSN_isnull OR ri.CNSMR_SSN_isnull => 0,
  ...
  SELF.LIC_NBR_match_code := MAP(
		le.LIC_NBR_isnull OR ri.LIC_NBR_isnull => SALT29.MatchCode.OneSideNull,
		le.LIC_STATE <> ri.LIC_STATE => SALT29.MatchCode.ContextInvolved, // Only valid if the context variable is equal
		match_methods(ih).match_LIC_NBR(le.LIC_NBR,ri.LIC_NBR));
  ...
  SELF.CNSMR_DOB_year_match_code := MAP(
                        ...
                        CNSMR_DOB_year+CNSMR_DOB_month+CNSMR_DOB_day < 0 OR CNSMR_DOB_year+CNSMR_DOB_month+CNSMR_DOB_day<>0 AND CNSMR_DOB_year+CNSMR_DOB_month+CNSMR_DOB_day < 300 => SALT29.MatchCode.ForcedNoMatch,
                        CNSMR_DOB_year < 0 AND ABS(le.CNSMR_DOB_year-ri.CNSMR_DOB_year) > 13 => SALT29.MatchCode.ForcedNoMatch, // Do not allow full generation mis-match
  ...
  SELF.CNSMR_DOB_score := MAP( le.CNSMR_DOB_year  IN SET(s.nulls_CNSMR_DOB_year,CNSMR_DOB_year) AND le.CNSMR_DOB_month  IN SET(s.nulls_CNSMR_DOB_month,CNSMR_DOB_month) AND le.CNSMR_DOB_day  IN SET(s.nulls_CNSMR_DOB_day,CNSMR_DOB_day) or ri.CNSMR_DOB_year  IN SET(s.nulls_CNSMR_DOB_year,CNSMR_DOB_year) AND ri.CNSMR_DOB_month  IN SET(s.nulls_CNSMR_DOB_month,CNSMR_DOB_month) AND ri.CNSMR_DOB_day  IN SET(s.nulls_CNSMR_DOB_day,CNSMR_DOB_day) => 0,
                        CNSMR_DOB_year+CNSMR_DOB_month+CNSMR_DOB_day < 0 or CNSMR_DOB_year+CNSMR_DOB_month+CNSMR_DOB_day<>0 and CNSMR_DOB_year+CNSMR_DOB_month+CNSMR_DOB_day < 300 => -9999,
                        CNSMR_DOB_year < 0 and ABS(le.CNSMR_DOB_year-ri.CNSMR_DOB_year) > 13 => -9999, // Do not allow full generation mis-match
                        CNSMR_DOB_score_temp);
  CNSMR_DOB_skipped := SELF.CNSMR_DOB_score = -9999;// Enforce FORCE parameter
  ...
  SELF.BILLING_TAX_ID_score := MAP( le.BILLING_TAX_ID_isnull OR ri.BILLING_TAX_ID_isnull => 0,
                        le.FNAME <> ri.FNAME => 0, // Only valid if the context variable is equal
  ...
  SELF.BILLING_NPI_NUMBER_score := IF ( BILLING_NPI_NUMBER_score_temp >= Config.BILLING_NPI_NUMBER_Force * 100, BILLING_NPI_NUMBER_score_temp, -9999 ); // Enforce FORCE parameter
  SELF.BILLING_NPI_NUMBER_skipped := SELF.BILLING_NPI_NUMBER_score < -5000;// Enforce FORCE parameter
  SELF.MAINNAME_match_code := MAP(
		...
		FULLNAME_score_pre > 0 => SALT29.MatchCode.ParentInvolved, // Ancestor has found solution so child keeps quiet
		match_methods(ih).match_MAINNAME(le.MAINNAME,ri.MAINNAME,(SALT29.StrType)le.FNAME,le.FNAME_MAINNAME_weight100,true,0,true,1,(SALT29.StrType)le.MNAME,le.MNAME_MAINNAME_weight100,true,0,true,2,(SALT29.StrType)le.LNAME,le.LNAME_MAINNAME_weight100,true,0,true,2,(SALT29.StrType)ri.FNAME,ri.MNAME_MAINNAME_weight100,(SALT29.StrType)ri.MNAME,ri.MNAME_MAINNAME_weight100,(SALT29.StrType)ri.LNAME,ri.MNAME_MAINNAME_weight100));
  ...
  SELF.SNAME_score := IF ( SNAME_score_temp >= Config.SNAME_Force * 100, SNAME_score_temp, -9999 ); // Enforce FORCE parameter
  SELF.SNAME_skipped := SELF.SNAME_score < -5000;// Enforce FORCE parameter
  ...
  SELF.LIC_TYPE_score := LIC_TYPE_score_temp*0.50; 
  SELF.DERIVED_GENDER_score := IF ( DERIVED_GENDER_score_temp >= Config.DERIVED_GENDER_Force * 100, DERIVED_GENDER_score_temp, -9999 ); // Enforce FORCE parameter
  SELF.DERIVED_GENDER_skipped := SELF.DERIVED_GENDER_score < -5000;// Enforce FORCE parameter
...
// Compute the score for the concept ADDRESS
  ...
  SELF.Conf_Prop := (0
    ...
    +MAX(le.CNSMR_SSN_prop,ri.CNSMR_SSN_prop)*SELF.CNSMR_SSN_score // Score if either field propogated
    ...
    +if(le.ADDRESS_prop+ri.ADDRESS_prop>0,self.ADDRESS_score*(0+if(le.ADDR1_prop+ri.ADDR1_prop>0,1,0)+if(le.LOCALE_prop+ri.LOCALE_prop>0,1,0))/2,0)
    +MAX(le.LIC_NBR_prop,ri.LIC_NBR_prop)*SELF.LIC_NBR_score // Score if either field propogated
    +MAX(le.C_LIC_NBR_prop,ri.C_LIC_NBR_prop)*SELF.C_LIC_NBR_score // Score if either field propogated
    +if(le.ADDR1_prop+ri.ADDR1_prop>0,self.ADDR1_score*(0+if(le.SEC_RANGE_prop+ri.SEC_RANGE_prop>0,1,0))/3,0)
    ...
    +self.CNSMR_DOB_score*(IF( (le.CNSMR_DOB_prop|ri.CNSMR_DOB_prop)&4>0,6,0 )+IF( (le.CNSMR_DOB_prop|ri.CNSMR_DOB_prop)&2>0,4,0 )+IF( (le.CNSMR_DOB_prop|ri.CNSMR_DOB_prop)&1>0,5,0 ))/15 // Compute weight of year/month/day propogation independantly
  ...
  SELF.Conf := (SELF.SSN_score + SELF.CNSMR_SSN_score + SELF.UPIN_score + SELF.VENDOR_ID_score + SELF.DID_score + SELF.NPI_NUMBER_score + SELF.BILLING_NPI_NUMBER_score + SELF.DEA_NUMBER_score + MAX(SELF.FULLNAME_score,MAX(SELF.MAINNAME_score,SELF.FNAME_score + SELF.MNAME_score + SELF.LNAME_score) + SELF.SNAME_score) + SELF.PHONE_score + SELF.FAX_score + MAX(SELF.ADDRESS_score,MAX(SELF.ADDR1_score,SELF.PRIM_RANGE_score + SELF.SEC_RANGE_score + SELF.PRIM_NAME_score) + MAX(SELF.LOCALE_score,SELF.V_CITY_NAME_score + SELF.ST_score + SELF.ZIP_score)) + SELF.LIC_NBR_score + SELF.C_LIC_NBR_score + SELF.DOB_score + SELF.CNSMR_DOB_score + SELF.LAT_LONG_score + SELF.TAXONOMY_score + SELF.LIC_TYPE_score + SELF.GENDER_score + SELF.DERIVED_GENDER_score + SELF.TAX_ID_score + SELF.BILLING_TAX_ID_score) / 100 + outside;
END;
...
EXPORT Layout_RolledEntity := RECORD,MAXLENGTH(63000)
  ...
  DATASET(SALT29.Layout_FieldValueList) LIC_NBR_Values := DATASET([],SALT29.Layout_FieldValueList);
...
Layout_RolledEntity RollValues(Layout_RolledEntity le,Layout_RolledEntity ri) := TRANSFORM
  ...
  SELF.LIC_NBR_values := SALT29.fn_combine_fieldvaluelist(le.LIC_NBR_values,ri.LIC_NBR_values);
...
Layout_RolledEntity into(in_data le) := TRANSFORM
  ...
  SELF.LIC_NBR_Values := IF ( le.LIC_NBR  IN SET(s.nulls_LIC_NBR,LIC_NBR),DATASET([],SALT29.Layout_FieldValueList),DATASET([{TRIM((SALT29.StrType)le.LIC_NBR)}],SALT29.Layout_FieldValueList));
...
EXPORT RemoveProps(DATASET(match_candidates(ih).layout_candidates) im) := FUNCTION
  im rem(im le) := TRANSFORM
    ...
    self.CNSMR_SSN := if ( le.CNSMR_SSN_prop>0, (TYPEOF(le.CNSMR_SSN))'', le.CNSMR_SSN ); // Blank if propogated
    self.CNSMR_SSN_isnull := le.CNSMR_SSN_prop>0 OR le.CNSMR_SSN_isnull;
    self.CNSMR_SSN_prop := 0; // Avoid reducing score later
    ...
    self.C_LIC_NBR := if ( le.C_LIC_NBR_prop>0, (TYPEOF(le.C_LIC_NBR))'', le.C_LIC_NBR ); // Blank if propogated
    self.C_LIC_NBR_isnull := le.C_LIC_NBR_prop>0 OR le.C_LIC_NBR_isnull;
    self.C_LIC_NBR_prop := 0; // Avoid reducing score later
    ...
    self.CNSMR_DOB_year := if ( le.CNSMR_DOB_prop&4>0, 0, le.CNSMR_DOB_year ); // Kill year if propogated
    self.CNSMR_DOB_year_isnull := self.CNSMR_DOB_year = 0;
    self.CNSMR_DOB_month := if ( le.CNSMR_DOB_prop&2>0, 0, le.CNSMR_DOB_month ); // Kill month if propogated
    self.CNSMR_DOB_month_isnull := self.CNSMR_DOB_month = 0;
    self.CNSMR_DOB_day := if ( le.CNSMR_DOB_prop&1>0, 0, le.CNSMR_DOB_day ); // Kill day if propogated
    self.CNSMR_DOB_day_isnull := self.CNSMR_DOB_day = 0;
    self.CNSMR_DOB_prop := 0; // Avoid reducing score later
...
Layout_Chubbies0 NoteSize(Layout_Chubbies0 le) := TRANSFORM
  ...
  SELF.LIC_NBR_size := SALT29.Fn_SwitchSpec(s.LIC_NBR_switch,count(le.LIC_NBR_values));
...
Layout_Chubbies := RECORD,MAXLENGTH(63000)
  t;
  UNSIGNED2 Size := t.SSN_size+t.CNSMR_SSN_size+t.UPIN_size+t.VENDOR_ID_size+t.DID_size+t.NPI_NUMBER_size+t.BILLING_NPI_NUMBER_size+t.DEA_NUMBER_size+t.FULLNAME_size+t.PHONE_size+t.FAX_size+t.ADDRESS_size+t.LIC_NBR_size+t.C_LIC_NBR_size+t.DOB_size+t.CNSMR_DOB_size+t.LAT_LONG_size+t.TAXONOMY_size+t.LIC_TYPE_size+t.GENDER_size+t.DERIVED_GENDER_size+t.TAX_ID_size+t.BILLING_TAX_ID_size;
END;

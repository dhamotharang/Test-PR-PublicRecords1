...
EXPORT Functions := MODULE
	...
	EXPORT fn_getHdrAddrRecs(DATASET(Address_Rank.Layouts.Batch_in) ds_batch_in,
		...
		ds_batch_in_w_seq := PROJECT(ds_batch_in, 
    ...
    // recs_with_did is grouped by seq.
		...
		roll_recs  		:= Address_Rank.fn_getHdrRecByDid_wBestdates(GROUP(dids_hh));
		...
		Address_Rank.Layouts.Bestrec restoreAcctno(recs_with_did l, roll_recs r):= TRANSFORM
			...
			SELF := [];
		END;
		hdr_recs := JOIN(recs_with_did_final, roll_recs,LEFT.did = RIGHT.did, restoreAcctno(LEFT, RIGHT), 
										 LIMIT(Address_Rank.Constants.JOIN_LIMIT, SKIP), LEFT OUTER);														

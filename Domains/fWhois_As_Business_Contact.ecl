IMPORT ut, Business_Header, Domains,mdr;

export fWhois_As_Business_Contact(dataset(Layout_Whois_Base_BIP) pwhoisBase) :=
function
	...
	BH_File := Business_Header.Files().base.business_headers.built;
	...
	Layout_Domains_Temp InitDomains(Layout_Whois_Base_BIP L) := TRANSFORM
	SELF := L;
	END;

	Domains_Init := PROJECT(pwhoisBase((UNSIGNED3)zip<>0, registrant_name<>''), InitDomains(LEFT));
	...
	BH_Init := PROJECT(BH_File(MDR.sourceTools.SourceIsWhois_domains(source)), InitBH(LEFT));
	...
	Business_Header.Layout_Business_Contact_Full_New Translate_Whois_to_BCF(Layout_Domains_Temp L, INTEGER C) := TRANSFORM
	...
	SELF.city := CHOOSE(C, L.admin_v_city_name, L.tech_v_city_name);
	...
	return Domains_Contacts_noroll;
end;
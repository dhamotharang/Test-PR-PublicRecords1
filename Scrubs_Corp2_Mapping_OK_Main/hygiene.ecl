IMPORT ut,SALT32;
EXPORT hygiene(dataset(layout_In_OK) h) := MODULE
...
SALT32.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT32.StrType)le.dt_vendor_first_reported),TRIM((SALT32.StrType)le.dt_vendor_last_reported),TRIM((SALT32.StrType)le.dt_first_seen),TRIM((SALT32.StrType)le.dt_last_seen),TRIM((SALT32.StrType)le.corp_ra_dt_first_seen),TRIM((SALT32.StrType)le.corp_ra_dt_last_seen),TRIM((SALT32.StrType)le.corp_key),TRIM((SALT32.StrType)le.corp_vendor),TRIM((SALT32.StrType)le.corp_state_origin),TRIM((SALT32.StrType)le.corp_process_date),TRIM((SALT32.StrType)le.corp_orig_sos_charter_nbr),TRIM((SALT32.StrType)le.corp_legal_name),TRIM((SALT32.StrType)le.corp_filing_date),TRIM((SALT32.StrType)le.corp_status_cd),TRIM((SALT32.StrType)le.corp_status_date),TRIM((SALT32.StrType)le.corp_inc_state),TRIM((SALT32.StrType)le.corp_inc_date),TRIM((SALT32.StrType)le.corp_foreign_domestic_ind),TRIM((SALT32.StrType)le.corp_forgn_date),TRIM((SALT32.StrType)le.corp_orig_org_structure_cd),TRIM((SALT32.StrType)le.corp_for_profit_ind),TRIM((SALT32.StrType)le.corp_ra_effective_date),TRIM((SALT32.StrType)le.corp_ra_resign_date),TRIM((SALT32.StrType)le.cont_effective_date),TRIM((SALT32.StrType)le.corp_merger_date),TRIM((SALT32.StrType)le.corp_merger_indicator),TRIM((SALT32.StrType)le.corp_name_effective_date),TRIM((SALT32.StrType)le.corp_merger_id),TRIM((SALT32.StrType)le.corp_name_reservation_expiration_date),TRIM((SALT32.StrType)le.corp_name_status_cd),TRIM((SALT32.StrType)le.corp_name_status_date),TRIM((SALT32.StrType)le.corp_forgn_state_desc),TRIM((SALT32.StrType)le.corp_ln_name_type_cd),TRIM((SALT32.StrType)le.recordorigin)));
  SELF.FldNo := C;
END;
...
SHARED FldIds := DATASET([{1,'dt_vendor_first_reported'}
      ...
      ,{19,'corp_forgn_date'}
      ,{20,'corp_orig_org_structure_cd'}
      ,{21,'corp_for_profit_ind'}
      ,{22,'corp_ra_effective_date'}
      ,{23,'corp_ra_resign_date'}
      ,{24,'cont_effective_date'}
      ,{25,'corp_merger_date'}
      ,{26,'corp_merger_indicator'}
      ,{27,'corp_name_effective_date'}
      ,{28,'corp_merger_id'}
      ,{29,'corp_name_reservation_expiration_date'}
      ,{30,'corp_name_status_cd'}
      ,{31,'corp_name_status_date'}
      ,{32,'corp_forgn_state_desc'}
      ,{33,'corp_ln_name_type_cd'}
      ,{34,'recordorigin'}],SALT32.MAC_Character_Counts.Field_Identification);

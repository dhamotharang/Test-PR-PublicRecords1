...
EXPORT SALT34.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_date','invalid_optional_date','invalid_corp_key','invalid_charter','invalid_merged_corpid','invalid_mandatory','invalid_corp_vendor','invalid_state_origin','invalid_forgn_dom_code','invalid_for_profit_ind','invalid_name_type_cd','invalid_corp_orig_org_structure_cd','invalid_alphablank');
EXPORT FieldTypeNum(SALT34.StrType fn) := CASE(fn,'invalid_date' => 1,'invalid_optional_date' => 2,'invalid_corp_key' => 3,'invalid_charter' => 4,'invalid_merged_corpid' => 5,'invalid_mandatory' => 6,'invalid_corp_vendor' => 7,'invalid_state_origin' => 8,'invalid_forgn_dom_code' => 9,'invalid_for_profit_ind' => 10,'invalid_name_type_cd' => 11,'invalid_corp_orig_org_structure_cd' => 12,'invalid_alphablank' => 13,0);
...
EXPORT MakeFT_invalid_merged_corpid(SALT34.StrType s0) := FUNCTION
  s1 := SALT34.stringfilter(s0,'0123456789 '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_merged_corpid(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,'0123456789 '))),~(LENGTH(TRIM(s)) >= 0 AND LENGTH(TRIM(s)) <= 7));
EXPORT InValidMessageFT_invalid_merged_corpid(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars('0123456789 '),SALT34.HygieneErrors.NotLength('0..7'),SALT34.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_corp_vendor(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_vendor(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['01']);
EXPORT InValidMessageFT_invalid_corp_vendor(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('01'),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_state_origin(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_state_origin(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['AL']);
EXPORT InValidMessageFT_invalid_state_origin(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('AL'),SALT34.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_name_type_cd(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_name_type_cd(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['01','09','I','P','N',' ']);
EXPORT InValidMessageFT_invalid_name_type_cd(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('01|09|I|P|N| '),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_corp_orig_org_structure_cd(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_orig_org_structure_cd(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['D/C','D/S','D3P','DCP','DGP','DHA','DLL','DLP','DML','DNL','DNP','DNS','DPA','DPC','DPL','DPN','DRP','DRT','DSL','DUA','F/C','F3P','FBE','FCP','FGP','FIC','FLL','FLP','FNP','FPA','FPC','FPL','FPN','FRP','FRT','FSL','FUA',' ']);
EXPORT InValidMessageFT_invalid_corp_orig_org_structure_cd(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('D/C|D/S|D3P|DCP|DGP|DHA|DLL|DLP|DML|DNL|DNP|DNS|DPA|DPC|DPL|DPN|DRP|DRT|DSL|DUA|F/C|F3P|FBE|FCP|FGP|FIC|FLL|FLP|FNP|FPA|FPC|FPL|FPN|FRP|FRT|FSL|FUA| '),SALT34.HygieneErrors.Good);
...
EXPORT FieldNum(SALT34.StrType fn) := CASE(fn,'dt_vendor_first_reported' => 0,'dt_vendor_last_reported' => 1,'dt_first_seen' => 2,'dt_last_seen' => 3,'corp_ra_dt_first_seen' => 4,'corp_ra_dt_last_seen' => 5,'corp_process_date' => 6,'corp_filing_date' => 7,'corp_merger_date' => 8,'corp_dissolved_date' => 9,'corp_forgn_date' => 10,'corp_inc_date' => 11,'corp_ra_resign_date' => 12,'corp_vendor' => 13,'corp_state_origin' => 14,'corp_inc_state' => 15,'corp_legal_name' => 16,'corp_merged_corporation_id' => 17,'corp_key' => 18,'corp_orig_sos_charter_nbr' => 19,'corp_for_profit_ind' => 20,'corp_foreign_domestic_ind' => 21,'corp_ln_name_type_cd' => 22,'corp_orig_org_structure_cd' => 23,'corp_forgn_state_desc' => 24,0);
...
EXPORT Make_corp_merged_corporation_id(SALT34.StrType s0) := MakeFT_invalid_merged_corpid(s0);
EXPORT InValid_corp_merged_corporation_id(SALT34.StrType s) := InValidFT_invalid_merged_corpid(s);
EXPORT InValidMessage_corp_merged_corporation_id(UNSIGNED1 wh) := InValidMessageFT_invalid_merged_corpid(wh);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT34,Scrubs_Corp2_Mapping_AL_Main;
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_dt_vendor_first_reported,1,0)+ IF( SELF.Diff_dt_vendor_last_reported,1,0)+ IF( SELF.Diff_dt_first_seen,1,0)+ IF( SELF.Diff_dt_last_seen,1,0)+ IF( SELF.Diff_corp_ra_dt_first_seen,1,0)+ IF( SELF.Diff_corp_ra_dt_last_seen,1,0)+ IF( SELF.Diff_corp_process_date,1,0)+ IF( SELF.Diff_corp_filing_date,1,0)+ IF( SELF.Diff_corp_merger_date,1,0)+ IF( SELF.Diff_corp_dissolved_date,1,0)+ IF( SELF.Diff_corp_forgn_date,1,0)+ IF( SELF.Diff_corp_inc_date,1,0)+ IF( SELF.Diff_corp_ra_resign_date,1,0)+ IF( SELF.Diff_corp_vendor,1,0)+ IF( SELF.Diff_corp_state_origin,1,0)+ IF( SELF.Diff_corp_inc_state,1,0)+ IF( SELF.Diff_corp_legal_name,1,0)+ IF( SELF.Diff_corp_merged_corporation_id,1,0)+ IF( SELF.Diff_corp_key,1,0)+ IF( SELF.Diff_corp_orig_sos_charter_nbr,1,0)+ IF( SELF.Diff_corp_for_profit_ind,1,0)+ IF( SELF.Diff_corp_foreign_domestic_ind,1,0)+ IF( SELF.Diff_corp_ln_name_type_cd,1,0)+ IF( SELF.Diff_corp_orig_org_structure_cd,1,0)+ IF( SELF.Diff_corp_forgn_state_desc,1,0);
  END;

...
EXPORT Get_payload := MODULE
	...
	EXPORT AlphaPropertyCSVRec := Layouts.AlphaPropertyCSVRec;
	EXPORT Boca_Official_Layout := Layouts.Boca_Official_Layout;
	...
	EXPORT EXISTING_PROPERTYINFOV2 := FUNCTION	
	   RETURN DATASET([], Layouts.Boca_Official_Layout);
	END;	
...
//* Create new payload from Customer Test data	
	EXPORT NEW_PROPERTYINFOV2 := FUNCTION
			...
			// This should now be in the new layout AlphaPropertyCSVRec
			AlphaDS := Files.PII_ALPHA_BASE_SF_DS;

			// next step simply renews clean address info and TRANSFORMS to Boca_Official_Layout
			Reformat_DS 			:= PROJECT(AlphaDS, Transform_Alpha_Data.Prepare_Alpharetta_Data (Left, Counter));
			// After this transform - this should now be in the new layout Boca_Official_Layout
			...
			Dedup_Reformat_DS  Resequence_AlphaDS (allVendorSourceDs L, Integer C) := TRANSFORM

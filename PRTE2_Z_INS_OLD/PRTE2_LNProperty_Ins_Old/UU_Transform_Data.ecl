...
EXPORT UU_Transform_Data(STRING pindexversion) := MODULE

		SHARED LNPropertyRec := Layouts_V2.LN_spreadsheet;
		SHARED string8 today_date := ut.GetDate; 
	
		EXPORT LNPropertyRec  Reformat_PHASE1 (z_Layout_Batch_in L) := TRANSFORM
					...
					// l.address has no unit_desig in it because they have a separate column for Apt#
					SELF.did 				:= (INTEGER)L.did;
					SELF.s_did 			:= (INTEGER)L.did;
					tempUnitString  := IF(L.Apt='','',' UNIT '+L.Apt);
					tempAddrString  := l.address+tempUnitString;
					string clean_address := address.CleanAddress182(tempAddrString, l.city + ' ' + l.st + ' ' + l.zip);
					SELF.prim_name  :=  clean_address [13..40];
					SELF.prim_range := clean_address [1..10];
					SELF.sec_range  := clean_address [57..64];
					tempUnitGen			:= PRTE2_Common.Clean_Address.GenerateUnitDesc(clean_address [1]);
					generatedUnitDesc := IF(L.Apt='','',tempUnitGen);
					SELF.unit_desig := generatedUnitDesc;
					SELF.unitDesc		:= generatedUnitDesc;
					...
					// ERROR CORRECTION:  the clean addresss was not handed any real unit_desig so it doesn't have one!!!!
					SELF.person_addr.unit_desig		:= generatedUnitDesc;

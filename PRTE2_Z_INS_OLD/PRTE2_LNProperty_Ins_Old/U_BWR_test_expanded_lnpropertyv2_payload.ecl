//* PRTE2_LNProperty.test_expanded_lnpropertyv2_payload *//
IMPORT PRTE2_LNProperty,PRTE2, PRTE_CSV;
#WORKUNIT ('name', 'expanded_lnpropertyv2_payload');
STRING Filedate := '20121016';
STRING pIndexVersion := Filedate;
Expanded_LNPROPERTYv2_Payload := PRTE2_LNProperty.Get_payload.EXISTING_LNPROPERTYV2;
OUTPUT(Expanded_LNPROPERTYv2_Payload,,'~prte::ct::ln_propertyv2::join::expanded',OVERWRITE);
OUTPUT(choosen(Expanded_LNPROPERTYv2_Payload,300));
...
OUTPUT(choosen(New_Expanded_LNPROPERTYv2_Payload,300));

F1 := dedup(project(Expanded_LNPROPERTYv2_Payload,PRTE2.layouts.layout_ln_propertyv2_expanded_payload),record,all);
F2 := dedup(project(New_Expanded_LNPROPERTYv2_Payload,PRTE2.layouts.layout_ln_propertyv2_expanded_payload),record,all);
...
OUTPUT (choosen(DS_All_LNPropertyv2_payload,300));
...
Newaddlfarestaxfid			:= 	project(DS_ALL_LNPropertyv2_Payload,TRANSFORM (rKeyLNPropertyV2__addlfarestax_fid,
			SELF := left,
			SELF := [] )); 
dKeyLNPropertyV2__addlfarestax_fid := DEDUP(Newaddlfarestaxfid,RECORD,ALL);			
...
Newaddllegalfid			:= 	project(DS_ALL_LNPropertyv2_Payload,TRANSFORM (rKeyLNPropertyV2__addllegal_fid,
			SELF := left,
			SELF := [] )); 
dKeyLNPropertyV2__addllegal_fid := DEDUP(Newaddllegalfid,RECORD,ALL);			
			
Newsearchfid			:= 	project(DS_ALL_LNPropertyv2_Payload,TRANSFORM (rKeyLNPropertyV2__addr_search_fid,
			SELF := left,
			SELF := [] )); 	
dKeyLNPropertyV2__addr_search_fid		:= DEDUP(Newsearchfid,RECORD,ALL);
...
Newcitystname			:= 	DEDUP(project(DS_ALL_LNPropertyv2_Payload,TRANSFORM (rKeyLNPropertyV2__autokey__citystname,
			...
			SELF := left,
			SELF := [] )),RECORD,ALL);
dKeyLNPropertyV2__autokey__citystname := DEDUP(Newcitystname,RECORD,ALL);
	
Newcitystnameb2			:= 	DEDUP(project(DS_ALL_LNPropertyv2_Payload,TRANSFORM (rKeyLNPropertyV2__autokey__citystnameb2,
			SELF := left,
			SELF := [] )),RECORD,ALL); 
dKeyLNPropertyV2__autokey__citystnameb2 := DEDUP(Newcitystnameb2,RECORD,ALL);
...
Newssn2     			:= 	DEDUP(project(DS_ALL_LNPropertyv2_Payload,TRANSFORM (rKeyLNPropertyV2__autokey__ssn2,
      self.DID := left.fakeid,			
			...
			SELF := left,
			SELF := [] )),RECORD,ALL); 
dKeyLNPropertyV2__autokey__ssn2 := DEDUP(Newssn2,RECORD,ALL);
...
Newstname		    	:= 	DEDUP(project(DS_ALL_LNPropertyv2_Payload,TRANSFORM (rKeyLNPropertyV2__autokey__stname,
      self.DID := left.fakeid,		  
			...
			SELF := left,
			SELF := [] )),RECORD,ALL);
dKeyLNPropertyV2__autokey__stname := DEDUP(Newstname,RECORD,ALL);
	

Newstnameb2 			:= 	DEDUP(project(DS_ALL_LNPropertyv2_Payload,TRANSFORM (rKeyLNPropertyV2__autokey__stnameb2,
			SELF := left,
			SELF := [] )),RECORD,ALL);
dKeyLNPropertyV2__autokey__stnameb2 := DEDUP(Newstnameb2,RECORD,ALL);
...
Newzip      			:= 	DEDUP(project(DS_ALL_LNPropertyv2_Payload,TRANSFORM (rKeyLNPropertyV2__autokey__zip,
      self.DID := left.fakeid,      
			...
			SELF := left,
			SELF := [] )),RECORD,ALL); 
dKeyLNPropertyV2__autokey__zip := DEDUP(Newzip,RECORD,ALL);

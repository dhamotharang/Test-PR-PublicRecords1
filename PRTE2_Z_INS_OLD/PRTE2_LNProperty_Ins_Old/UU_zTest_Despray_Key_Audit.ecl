/*
I want to pull off the current PRTE Dev key contents (filter for our DID range) - before and after the UU conversion
Just doing this to compare the two CSV files to have a good feeling we haven't broken anything 
and/or to know the limits of things we've changed.
*/

IMPORT PRTE2_LNProperty as LNP;
IMPORT PRTE2_Common, ut;
#workunit('name', 'Boca CT LNProperty UU Despray');

keyFileName := '~prte::key::ln_propertyv2::fcra::20160114::assessor.fid';
...
// ELIMINATE THE BOCA TEST CASES FROM THE KEY BEFORE DESPRAY  I'm ignoring the 55 records we have with no DID for auditing purposes
//OUTPUT ON MIN MAX SHOWED: 888809000002 to 888809054118
// dataDS := prct_data_key(did > 888809000000 AND did < 888809054119);
// OOPS, the key doesn't use DID  - it uses ln_fares_id as the key...  our records have ALP in the field: OAALP0100195
dataDS := prct_data_key(ln_fares_id[3..5]='ALP');
OUTPUT(COUNT(dataDS));
output(dataDS);
EXPORT_DS						:= SORT(dataDS, ln_fares_id,state_code,county_name,assessee_name,vendor_source_flag, fips_code);

//---------------------------------------------------------------------------------------------------------
dateString	:= ut.GetDate;
TempCSV							:= LNP.Files.LNP_CSV_FILE + '::' +  WORKUNIT;
// desprayName 				:= 'LN_Property_AV3_KeyAudit_pre_'+dateString+'.csv';
desprayName 				:= 'LN_Property_AV3_KeyAudit_post_'+dateString+'.csv';

IMPORT PRTE2_Email, emailservice, address, ut, mdr, _validate, Entiera, email_data;

EXPORT Map_P1_As_Email(string version) := FUNCTION

			with_email := _files.Phase1_Output_File(length(trim(email,left, right)) > 4 and StringLib.StringFindCount(email,  '@') > 0);

			//apply macro to obtain email domain fields
			emailservice.mac_append_domain_flags(with_email,domain_d_macro_results,email);
			//TODO No idea if that mac in the emailservice module will operate outside of Email_data module
						
			//************Transform to a common email layout
			_layouts.Base t_map_to_common (domain_d_macro_results input) := TRANSFORM
						//Default source to MediaOne, alter to different source if we see two-character "source" field.
						boolean specialSource := length(input.source)=2;
						string MediaOneString := mdr.sourceTools.src_MediaOne;
						string CalcSrcString := if(specialSource, _functions.SpecialCTSourceCalculation(input.source),MediaOneString);
						string calc_date_of_birth := IF(regexfind('/',input.dob),ut.date_slashed_MMDDYYYY_to_YYYYMMDD(input.dob),input.dob);
						
						self.email_src        					:= CalcSrcString;
						self.rec_src_all      					:= Translation_Codes.source_bitmap_code(CalcSrcString);
						self.email_src_all    					:= Translation_Codes.source_bitmap_code(CalcSrcString);						
						...
						self.best_ssn  									:= '';
						self.best_dob  									:= (unsigned) calc_date_of_birth;
						...
						self.append_email_username 			:= stringlib.stringtouppercase(_functions.Clean_Email_Username(self.orig_email));
						...
						self.Email_rec_key    					:= _functions.Email_rec_key(self.clean_email ,
			...
			phase1CommonMapping := project(domain_d_macro_results, t_map_to_common(left));


			RETURN phase1CommonMapping(append_email_username <> '' and append_domain_root <> '' and StringLib.StringFindCount(clean_email,  '.') > 0 and ~Entiera.fn_profanity(clean_email));

...
EXPORT Mod_Corps(BOOLEAN MultiRec = FALSE,SET OF SALT38.UIDType ButNot=[], DATASET(RECORDOF(File_Corp2_Base))  inefile = File_Corp2_Base) := MODULE
  R := RECORD
    inefile;
...
SHARED s := CHOOSEN(Keys(File_BizHead).Specificities_Key,1)[1];
  Mapping0 := 'cnp_name:corp_legal_name,prim_range:corp_addr1_prim_range,prim_name:corp_addr1_prim_name,sec_range:corp_addr1_sec_range,p_city_name:corp_addr1_p_city_name,st:corp_addr1_state,zip_cases:corp_addr1_zip5,company_phone:corp_phone10,company_fein:corp_fed_tax_id,fname:cont_fname,lname:cont_lname,mname:cont_mname,source_record_id:source_rec_id'; // Tweaks for non-standard field names
  UniChange0 := input;
...
// Create the mappings and links for data in the main external file
  Process_Biz_layouts.InputLayout tr(Process_Biz_layouts.InputLayout le) := TRANSFORM,SKIP(~Process_Biz_Layouts.HardKeyMatch(le))
    SELF := le;
  END;
EXPORT Mapped := PROJECT(Mapped0,tr(LEFT)) : PERSIST('~temp::proxid::PRTE2_BIPV2_WAF::Mapped::UID::Corps',EXPIRE(PRTE2_BIPV2_WAF.Config.PersistExpire));
...
EXPORT PayloadByproxidName := '~'+'key::PRTE2_BIPV2_WAF::proxid::Corpsbyproxid::Payload';
...
//EXPORT EFR := PROJECT(cnts,TRANSFORM(Ext_Layouts.EFRR_Layout,SELF.Child_Id := Ext_Layouts.ID_Corps,SELF.ChildId_BMap := 1<<(Ext_Layouts.ID_Corps-1),SELF.proxid := LEFT.Fetch_proxid,SELF.seleid := LEFT.Fetch_seleid,SELF.orgid := LEFT.Fetch_orgid,SELF.ultid := LEFT.Fetch_ultid,SELF := LEFT));
EXPORT EFR := PROJECT(cnts,TRANSFORM(Ext_Layouts.EFRR_Layout,SELF.Child_Id := Ext_Layouts.ID_Corps,SELF.proxid := LEFT.Fetch_proxid,SELF.seleid := LEFT.Fetch_seleid,SELF.orgid := LEFT.Fetch_orgid,SELF.ultid := LEFT.Fetch_ultid,SELF := LEFT));
END;

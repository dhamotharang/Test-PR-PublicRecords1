...
EXPORT Mod_PropertyV2(BOOLEAN MultiRec = FALSE,SET OF SALT38.UIDType ButNot=[], DATASET(RECORDOF(File_Ln_PropertyV2_Base))  inefile = File_Ln_PropertyV2_Base) := MODULE
  R := RECORD
    inefile;
...
SHARED s := CHOOSEN(Keys(File_BizHead).Specificities_Key,1)[1];
  Mapping0 := 'cnp_name:cname,prim_range:prim_range,prim_name:prim_name,sec_range:sec_range,p_city_name:p_city_name,st:st,zip_cases:zip,company_phone:phone_number'; // Tweaks for non-standard field names
  UniChange0 := input;
...
// Create the mappings and links for data in the main external file
  Process_Biz_layouts.InputLayout tr(Process_Biz_layouts.InputLayout le) := TRANSFORM,SKIP(~Process_Biz_Layouts.HardKeyMatch(le))
    SELF := le;
  END;
EXPORT Mapped := PROJECT(Mapped0,tr(LEFT)) : PERSIST('~temp::proxid::PRTE2_BIPV2_WAF::Mapped::UID::PropertyV2',EXPIRE(PRTE2_BIPV2_WAF.Config.PersistExpire));
...
EXPORT PayloadByproxidName := '~'+'key::PRTE2_BIPV2_WAF::proxid::PropertyV2byproxid::Payload';
...
//EXPORT EFR := PROJECT(cnts,TRANSFORM(Ext_Layouts.EFRR_Layout,SELF.Child_Id := Ext_Layouts.ID_PropertyV2,SELF.ChildId_BMap := 1<<(Ext_Layouts.ID_PropertyV2-1),SELF.proxid := LEFT.Fetch_proxid,SELF.seleid := LEFT.Fetch_seleid,SELF.orgid := LEFT.Fetch_orgid,SELF.ultid := LEFT.Fetch_ultid,SELF := LEFT));
EXPORT EFR := PROJECT(cnts,TRANSFORM(Ext_Layouts.EFRR_Layout,SELF.Child_Id := Ext_Layouts.ID_PropertyV2,SELF.proxid := LEFT.Fetch_proxid,SELF.seleid := LEFT.Fetch_seleid,SELF.orgid := LEFT.Fetch_orgid,SELF.ultid := LEFT.Fetch_ultid,SELF := LEFT));
END;

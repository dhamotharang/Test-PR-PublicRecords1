...
SHARED s := Specificities(ih).Specificities[1];
  h00 := PRTE2_BIPV2_WAF.Specificities(ih).input_file;
SHARED thin_table := DISTRIBUTE(TABLE(h00,{rcid,parent_proxid,ultimate_proxid,has_lgid,empid,powid,source,source_record_id,cnp_number,cnp_btype,cnp_lowv,cnp_name,company_phone,company_fein,company_fein_len,company_sic_code1,prim_range,prim_range_len,prim_name,prim_name_len,sec_range,sec_range_len,p_city_name,p_city_name_len,st,zip,company_url,isContact,title,fname,fname_len,mname,mname_len,lname,lname_len,name_suffix,contact_email,CONTACTNAME,STREETADDRESS,ultid,orgid,seleid,proxid}),HASH(proxid));
 
//Prepare for field propagations ...
PrePropCounts := RECORD
  REAL8 empid_pop := AVE(GROUP,IF((thin_table.empid  IN SET(s.nulls_empid,empid) OR thin_table.empid = (TYPEOF(thin_table.empid))''),0,100));
  REAL8 powid_pop := AVE(GROUP,IF((thin_table.powid  IN SET(s.nulls_powid,powid) OR thin_table.powid = (TYPEOF(thin_table.powid))''),0,100));
...
EXPORT poprec := RECORD
	STRING label;
		REAL8 pop;
	END;
EXPORT PrePropogationStats := SALT38.MAC_Pivot(PPS, poprec);
...
SALT38.mac_prop_field(with_props(company_fein NOT IN SET(s.nulls_company_fein,company_fein)),company_fein,proxid,company_fein_props); // For every DID find the best FULL company_fein
...
pj12 do_computes(pj12 le) := TRANSFORM
  ...
  SELF := le;
END;
SHARED propogated := PROJECT(pj12,do_computes(left)) : PERSIST('~temp::proxid::PRTE2_BIPV2_WAF::mc_props::BizHead',EXPIRE(PRTE2_BIPV2_WAF.Config.PersistExpire)); // to allow to 'jump' over an exported value
PostPropCounts := RECORD
  REAL8 empid_pop := AVE(GROUP,IF((propogated.empid  IN SET(s.nulls_empid,empid) OR propogated.empid = (TYPEOF(propogated.empid))''),0,100));
  REAL8 powid_pop := AVE(GROUP,IF((propogated.powid  IN SET(s.nulls_powid,powid) OR propogated.powid = (TYPEOF(propogated.powid))''),0,100));
...
EXPORT PostPropogationStats := SALT38.MAC_Pivot(PoPS, poprec);
  Grpd := GROUP( SORT(
    DISTRIBUTE( TABLE( propogated, { propogated, UNSIGNED2 Buddies := 0 }),HASH(proxid)),
    proxid,empid,powid,source,source_record_id,cnp_number,cnp_btype,cnp_lowv,cnp_name,company_phone,company_fein,company_sic_code1,prim_range,prim_name,sec_range,p_city_name,st,zip,company_url,isContact,title,fname,mname,lname,name_suffix,contact_email, LOCAL),
    proxid,empid,powid,source,source_record_id,cnp_number,cnp_btype,cnp_lowv,cnp_name,company_phone,company_fein,company_sic_code1,prim_range,prim_name,sec_range,p_city_name,st,zip,company_url,isContact,title,fname,mname,lname,name_suffix,contact_email, LOCAL);
...
EXPORT Layout_Matches := RECORD//in this module for because of ,foward bug
  ...
  SALT38.UIDType proxid1;
  SALT38.UIDType proxid2;
  SALT38.UIDType rcid1 := 0;
  SALT38.UIDType rcid2 := 0;
END;
EXPORT Layout_Candidates := RECORD // A record to hold weights of each field value
  ...
  BOOLEAN empid_isnull := (h0.empid  IN SET(s.nulls_empid,empid) OR h0.empid = (TYPEOF(h0.empid))''); // Simplify later processing 
  INTEGER2 powid_weight100 := 0; // Contains 100x the specificity
  BOOLEAN powid_isnull := (h0.powid  IN SET(s.nulls_powid,powid) OR h0.powid = (TYPEOF(h0.powid))''); // Simplify later processing 
...
SHARED Annotated := DISTRIBUTE(j0,hash(proxid)) : PERSIST('~temp::proxid::PRTE2_BIPV2_WAF::mc',EXPIRE(PRTE2_BIPV2_WAF.Config.PersistExpire)); // Distributed for keybuild case

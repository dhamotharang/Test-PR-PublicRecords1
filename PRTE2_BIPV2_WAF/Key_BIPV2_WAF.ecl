IMPORT SALT29;// Gather up the UID counts from each of the children - provides 'we also found' capability
...
EXPORT Key_BIPV2_WAF := Module
  ...
  r := ROLLUP(m,GROUP,TRANSFORM(Ext_Layouts.EFR_Layout,SELF.Hits := PROJECT(ROWS(LEFT),Ext_Layouts.EFR_Child),SELF := LEFT));
	...
	EXPORT Key := INDEX(r,{ultid,orgid,seleid,proxid},{r},'~prte::key::BIPV2_WAF::QA::proxid::efr');

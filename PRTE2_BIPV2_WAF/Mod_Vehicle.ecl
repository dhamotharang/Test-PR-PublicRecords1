...
EXPORT Mod_Vehicle(BOOLEAN MultiRec = FALSE,SET OF SALT38.UIDType ButNot=[], DATASET(RECORDOF(File_VehicleV2_Base))  inefile = File_VehicleV2_Base) := MODULE
  R := RECORD
    inefile;
...
SHARED s := CHOOSEN(Keys(File_BizHead).Specificities_Key,1)[1];
  Mapping0 := 'cnp_name:append_clean_cname,prim_range:append_clean_address.prim_range,prim_name:append_clean_address.prim_name,sec_range:append_clean_address.sec_range,p_city_name:append_clean_address.v_city_name,st:append_clean_address.st,zip_cases:append_clean_address.zip5,company_fein:orig_fein,fname:fname,mname:mname,lname:lname,source_record_id:source_rec_id'; // Tweaks for non-standard field names
  UniChange0 := input;
...
// Create the mappings and links for data in the main external file
  Process_Biz_layouts.InputLayout tr(Process_Biz_layouts.InputLayout le) := TRANSFORM,SKIP(~Process_Biz_Layouts.HardKeyMatch(le))
    SELF := le;
  END;
EXPORT Mapped := PROJECT(Mapped0,tr(LEFT)) : PERSIST('~temp::proxid::PRTE2_BIPV2_WAF::Mapped::UID::Vehicle',EXPIRE(PRTE2_BIPV2_WAF.Config.PersistExpire));
...
EXPORT PayloadByproxidName := '~'+'key::PRTE2_BIPV2_WAF::proxid::Vehiclebyproxid::Payload';
...
//EXPORT EFR := PROJECT(cnts,TRANSFORM(Ext_Layouts.EFRR_Layout,SELF.Child_Id := Ext_Layouts.ID_Vehicle,SELF.ChildId_BMap := 1<<(Ext_Layouts.ID_Vehicle-1),SELF.proxid := LEFT.Fetch_proxid,SELF.seleid := LEFT.Fetch_seleid,SELF.orgid := LEFT.Fetch_orgid,SELF.ultid := LEFT.Fetch_ultid,SELF := LEFT));
EXPORT EFR := PROJECT(cnts,TRANSFORM(Ext_Layouts.EFRR_Layout,SELF.Child_Id := Ext_Layouts.ID_Vehicle,SELF.proxid := LEFT.Fetch_proxid,SELF.seleid := LEFT.Fetch_seleid,SELF.orgid := LEFT.Fetch_orgid,SELF.ultid := LEFT.Fetch_ultid,SELF := LEFT));
END;

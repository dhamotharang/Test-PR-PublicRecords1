...
EXPORT FetchEFR(DATASET(process_Biz_layouts.id_stream_layout) idstream,UNSIGNED User_Permits) := FUNCTION
  ...
  R := RECORD
    ...
    UNSIGNED2 Permits;
  END;
  N := NORMALIZE(Raw,COUNT(LEFT.Hits),TRANSFORM(R,SELF.UniqueID := LEFT.UniqueID, SELF := LEFT.Hits[COUNTER]))(User_Permits|Permits = (1<<Config.SOURCE_Permit_Number)-1);
...
EXPORT FetchParams := MODULE,VIRTUAL // Derive from here to change some of the default values
  ...
  EXPORT MaxCorps := Config.MaxCorps; //Default of 0 fetches all
  EXPORT MaxUCC := Config.MaxUCC; //Default of 0 fetches all
  EXPORT MaxVehicle := Config.MaxVehicle; //Default of 0 fetches all
  EXPORT MaxPropertyV2 := Config.MaxPropertyV2; //Default of 0 fetches all
  EXPORT MaxBizContacts := Config.MaxBizContacts; //Default of 0 fetches all
END;
...
END;
EXPORT Fetch(DATASET(PRTE2_BIPV2_WAF.Process_Biz_Layouts.id_stream_layout) ins,FetchParams P=FetchParams) := FUNCTION
  Layout Get(DATASET(PRTE2_BIPV2_WAF.Process_Biz_Layouts.id_stream_layout) ids) := TRANSFORM

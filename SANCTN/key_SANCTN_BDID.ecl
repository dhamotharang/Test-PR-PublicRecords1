import doxie_files, doxie,ut,Data_Services;


f_sanctn_party := SANCTN.file_base_party(bdid != 0);

slim_sanctn := record
f_sanctn_party.BDID;
f_sanctn_party.BATCH_NUMBER;
f_sanctn_party.INCIDENT_NUMBER;
f_sanctn_party.PARTY_NUMBER;
end;

tbl_did := table(f_sanctn_party,slim_sanctn,BDID,BATCH_NUMBER,INCIDENT_NUMBER,PARTY_NUMBER,few);

KeyName 			:= 'thor_data400::key::sanctn::';

export key_sanctn_BDID := index(tbl_DID
                               ,{BDID}
															 ,{BATCH_NUMBER,INCIDENT_NUMBER,PARTY_NUMBER}
                               ,Data_Services.Data_location.Prefix('sanctn')+KeyName +'bdid_'+doxie.Version_SuperKey);


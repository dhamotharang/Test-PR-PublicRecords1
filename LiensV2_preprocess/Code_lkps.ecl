...
EXPORT Code_lkps := MODULE
	...
	//CA Federal Lookups
	CA_FileDict	:= DICTIONARY(Files_lkp.filing_type, {filing_type_cd => filing_type_desc});
	
	EXPORT CA_FileType(string code) := CA_FileDict[code].filing_type_desc;
	
	CA_StatusDict	:= DICTIONARY(Files_lkp.filing_status, {filing_status_cd => filing_status_desc});
	
	EXPORT CA_FileStatus(string code) := CA_StatusDict[code].filing_status_desc;

	//Hogan Lookups
	HG_FileDict := DICTIONARY(Files_lkp.HGFiling_type, {filetype_cd => filetype_desc});

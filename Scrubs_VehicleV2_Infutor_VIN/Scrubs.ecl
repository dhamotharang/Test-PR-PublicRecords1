...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Layout_Profile_VehicleV2_Infutor_VIN)
  ...
  END;
  EXPORT  Bitmap_Layout := RECORD(Layout_Profile_VehicleV2_Infutor_VIN)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(Layout_Profile_VehicleV2_Infutor_VIN) h) := MODULE
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,Layout_Profile_VehicleV2_Infutor_VIN);
  Expanded_Layout into(h le) := TRANSFORM

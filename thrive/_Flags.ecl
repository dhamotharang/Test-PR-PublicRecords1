...
export _Flags(string pInput_Name='', string pBase_Name='')  := module
	...
	export ExistCurrentSprayed			:= fileservices.superfileexists(filenames(pInput_Name,).lInputLtTemplate) and 
																		 fileservices.superfileexists(filenames(pInput_Name,).lInputPdTemplate) and 
																		 count(fileservices.superfilecontents(filenames(pInput_Name,).lInputLtTemplate  )) > 0 and
																		 count(fileservices.superfilecontents(filenames(pInput_Name,).lInputPdTemplate  )) > 0 ;

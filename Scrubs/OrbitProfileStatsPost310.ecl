import Salt310, Orbit3SOA, ut,_control,std;
EXPORT OrbitProfileStatsPost310 (string pProfileName = '', string pProfileType = 'ScrubsAlerts', dataset(Salt310.ScrubsOrbitLayout)ScrubsStats = dataset([], Salt310.ScrubsOrbitLayout), string versionDate = (STRING8)Std.Date.Today(), string FileType = '', string CustomTag = '', string maxThreshold = '10' , string minThreshold = '-10'):= module
...
output(RemoveBlankRules),
																	output(_control.fSubmitNewWorkunit('#workunit(\'name\',\'Build Scrubs - '+pProfileName+'\');\r\n'+
																																		 'Submission:=dataset(\'~thor_data400::Scrubs::FileToSubmit_'+pProfileName+'_'+workunit+'_'+CustomTag+'\',Salt310.ScrubsOrbitLayout,thor);\r\n'+
...
EXPORT SummaryStats := dedup(sort(project(CleanStats,
																					transform(Salt310.ScrubsOrbitLayout and not [rejectwarning, rawcodemissing, rawcodemissingcnt],

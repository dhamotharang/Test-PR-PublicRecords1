...
EXPORT CallFunctions (string pProfileName = '', string pProfileType = 'ScrubsAlerts', dataset(Salt310.ScrubsOrbitLayout)ScrubsStats = dataset([], Salt310.ScrubsOrbitLayout), string versionDate = (STRING8)Std.Date.Today(), string FileType = '', string CustomTag = '', string maxThreshold = '10' , string minThreshold = '-10'):= module
...
output(RemoveBlankRules),
																	output(_control.fSubmitNewWorkunit('#workunit(\'name\',\'Build Scrubs - '+pProfileName+'\');\r\n'+
																																		 'Submission:=dataset(\'~thor_data400::Scrubs::FileToSubmit_'+pProfileName+'_'+workunit+'_'+version+'\',Salt310.ScrubsOrbitLayout,thor);\r\n'+

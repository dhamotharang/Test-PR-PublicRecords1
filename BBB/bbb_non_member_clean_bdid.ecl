...
layout_bbb_seq SequenceInput(Layout_BBB_Non_Members_In l, unsigned4 cnt) := transform
self.seq := cnt;
...
bbb_seq := project(File_BBB_Non_Members_In, SequenceInput(left, counter));
...
layout_bbb_non_member_base_temp := record
unsigned4 seq;
layout_bbb_non_member_base;
end;

// Project clean fields to base format
layout_bbb_non_member_base_temp InitBase(layout_bbb_clean l) := transform
self.bdid := 0;
...
bbb_clean := project(bbb_clean_init, InitBase(left)) : persist('TEMP::bbb_non_member_clean');

// Project into BDID in batch format
Business_Header_SS.Layout_BDID_OutBatch InitBatch(layout_bbb_non_member_base_temp l) := transform
...
layout_bbb_non_member_base AppendBDID(layout_bbb_non_member_base_temp l, Business_Header_SS.Layout_BDID_OutBatch r) := transform
self.bdid := r.bdid;
self := l;
end;

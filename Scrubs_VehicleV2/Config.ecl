...
EXPORT CorrelateSampleSize := 20661157; // Size of sample used in hygiene.corelations (reduced due to large field count)
...
EXPORT AttrValueType := SALT36.AttrValueType;
EXPORT MaxChildren := 100; // Maximum children allowed for a MULTIPLE(,LIST)
alg := ENUM(UNSIGNED1, Standard=0, NoTrailingHalfEdit);
EXPORT WithinEditN(SALT36.StrType l,UNSIGNED1 ll, SALT36.StrType r,UNSIGNED1 rl,UNSIGNED1 d, UNSIGNED1 edit_threshold=0,UNSIGNED1 mode=alg.Standard, BOOLEAN edFunction(SALT36.StrType l,UNSIGNED1 ll, SALT36.StrType r,UNSIGNED1 rl,UNSIGNED1 d,UNSIGNED1 mode) = SALT36.fn_EditDistance) := 
        SALT36.WithinEditNew(l, ll, r, rl, d, edit_threshold, mode, edFunction);

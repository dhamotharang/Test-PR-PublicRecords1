IMPORT SALT311,STD;
EXPORT Infutor_VIN_Delta(DATASET(Infutor_VIN_Layout_VehicleV2)old_s, DATASET(Infutor_VIN_Layout_VehicleV2) new_s) := MODULE//Routines to compute the differences between two instances of a file
  ...
  EXPORT DifferenceSummary(BOOLEAN Glob = TRUE) := Infutor_VIN_hygiene(old_s).Summary('Old',Glob) + Infutor_VIN_hygiene(new_s).Summary('New',Glob) + Infutor_VIN_hygiene(PROJECT(Differences(deleted), TRANSFORM(Infutor_VIN_Layout_VehicleV2, SELF := LEFT.old_rec))).Summary('Deletions',Glob) + Infutor_VIN_hygiene(PROJECT(Differences(added), TRANSFORM(Infutor_VIN_Layout_VehicleV2, SELF := LEFT.new_rec))).Summary('Additions',Glob);
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE, BOOLEAN doHygieneSummaryPerSrc = TRUE) := FUNCTION
    ...
    SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_VehicleV2, Infutor_VIN_Fields, 'RECORDOF(hygieneDiffOverall)', TRUE);

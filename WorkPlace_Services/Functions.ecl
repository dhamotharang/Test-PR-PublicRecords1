...
export Functions := module
  ...
  export getCompInfoFromGong(
    dataset(WorkPlace_Services.Layouts.poe_didkey_plus) ds_all_recs_in,
    Doxie.IDataAccess mod_access) := function
    ...
    //The transform populates the did from the left, need to keep right one for opt out.
    poe_optout_layout := RECORD
    ...
    pre_ds_all_phone1_gongcomp := join(ds_all_phone1_deduped, dx_Gong.key_history_phone(),
      ...
      transform(poe_optout_layout,
        ...
        self.company_zip := right.z5 + if(right.z4<>'', '-' + right.z4,''),
        self.company_zip5 := right.z5,
        self.company_zip4 := right.z4,
        self.bdid := if(left.bdid<>0, left.bdid,right.bdid),
        self.from_gong := true,
    ...
    ds_all_phone1_gongcomp_optout := Suppress.MAC_SuppressSource(pre_ds_all_phone1_gongcomp, mod_access, key_did);
    ds_all_phone1_gongcomp := PROJECT(ds_all_phone1_gongcomp_optout, WorkPlace_Services.Layouts.poe_didkey_plus);
  ...
  export getPhoneFromGong(
    ...
    //The DID is populated with the left recordset, need to save right for opt out.
    poe_optout_layout := RECORD
      WorkPlace_Services.Layouts.poe_didkey_plus;
      unsigned6 key_did;
    END;
    ...
    ds_all_bdids_gongphone_optout := Suppress.MAC_SuppressSource(pre_ds_all_bdids_gongphone, mod_access, key_did);
    ds_all_bdids_gongphone := PROJECT(ds_all_bdids_gongphone_optout, WorkPlace_Services.Layouts.poe_didkey_plus);

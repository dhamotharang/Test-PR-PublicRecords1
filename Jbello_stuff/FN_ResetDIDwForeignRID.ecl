...
import header;
export FN_ResetDIDwForeignRID(dataset({unsigned8 old_rid,header.Layout_New_Records}) head) :=
...
bad  := ddp(rid <> did);
// output(bad,named('FN_ResetDIDwForeignRID_bad'));
...
dist patch(dist l, dids r) := transform
	...
	self.old_rid := if(r.did > 0, l.did, l.old_rid);
	self.pflag1 := if(r.did > 0, '1', l.pflag1);
	self := l;
end;

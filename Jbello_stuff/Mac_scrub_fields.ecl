...
// more junk removal
	set_junk:='(.*)[,;-]{1,}$';
	set_junk1:='^([^a-z]){1,}(.*)([^a-z]){1,}$';
	set_junk2:='^([^a-z]){1,}(.*)([a-z])$';
	set_junk3:='^([a-z])(.*)([^a-z]){1,}$';
	set_junk4:='^([a-z]{1,})([;,])([a-z]{1,})$';
	set_junk5:='trstee';
	remove_junk(string p):=map(
		 regexfind(set_junk ,trim(p,left,right),nocase) => regexreplace(set_junk ,trim(p,left,right),trim('$1'),nocase)
		,regexfind(set_junk1,trim(p,left,right),nocase) => regexreplace(set_junk1,trim(p,left,right),trim('$2'),nocase)
		,regexfind(set_junk2,trim(p,left,right),nocase) => regexreplace(set_junk2,trim(p,left,right),trim('$2'+'$3'),nocase)
		,regexfind(set_junk3,trim(p,left,right),nocase) => regexreplace(set_junk3,trim(p,left,right),trim('$1'+'$2'),nocase)
		,regexfind(set_junk4,trim(p,left,right),nocase) => regexreplace(set_junk4,trim(p,left,right),trim('$1'),nocase)
		,regexfind(set_junk5,trim(p,left,right),nocase) => regexreplace(set_junk5,trim(p,left,right),'',nocase)
		,p);
 	self.fname			:=remove_junk(fname);
	self.mname			:=remove_junk(mname);
	self.lname			:=remove_junk(lname);

...
export fn_Search(GROUPED DATASET(GlobalWatchlists.Layout_Batch_In) seqd, real threshold, 
				 boolean useDOBFilter, INTEGER2 dobRadius, boolean includeOFAC, SET OF STRING50 categories) :=
FUNCTION

n := globalwatchlists.fn_Search(seqd,worldcheck.File_WorldCheck_Tokens);
...
END;
	...
	// extra filters
	dob_filter(l_work le, worldcheck.Key_WorldCheck_key ri) := is_person(le, ri) AND 
																useDOBFilter AND le.dob<>'' AND ABS(ut.GetAge(le.dob) - ut.GetAge(ri.Date_Of_Birth)) > dobRadius;
...
TRANSFORM
	...
	person_score := ofaclib.personScore(le.name,ri.First_Name,ri.Last_Name);
	company_score := ofaclib.companyScore(le.name,ri.cname);
	country_score := IF(le.country=ri.cname,1,0);
...
s := ITERATE(SORT(GROUP(with_input), seq, -score, uid), iter(LEFT,RIGHT));

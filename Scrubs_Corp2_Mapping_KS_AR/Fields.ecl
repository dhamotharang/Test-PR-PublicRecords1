...
EXPORT SALT34.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_future_date','invalid_charter_nbr','invalid_Y_N','invalid_ar_status','invalid_numeric');
EXPORT FieldTypeNum(SALT34.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_corp_vendor' => 2,'invalid_state_origin' => 3,'invalid_mandatory' => 4,'invalid_date' => 5,'invalid_future_date' => 6,'invalid_charter_nbr' => 7,'invalid_Y_N' => 8,'invalid_ar_status' => 9,'invalid_numeric' => 10,0);
...
EXPORT MakeFT_invalid_ar_status(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_ar_status(SALT34.StrType s) := WHICH(~Scrubs_Corp2_Mapping_KS_AR.Functions.invalid_ar_status(s)>0);
EXPORT InValidMessageFT_invalid_ar_status(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.CustomFail('Scrubs_Corp2_Mapping_KS_AR.Functions.invalid_ar_status'),SALT34.HygieneErrors.Good);
...
EXPORT Make_ar_year(SALT34.StrType s0) := s0;
EXPORT InValid_ar_year(SALT34.StrType s) := FALSE;
...
EXPORT Make_ar_report_dt(SALT34.StrType s0) := MakeFT_invalid_date(s0);
EXPORT InValid_ar_report_dt(SALT34.StrType s) := InValidFT_invalid_date(s);
EXPORT InValidMessage_ar_report_dt(UNSIGNED1 wh) := InValidMessageFT_invalid_date(wh);
 
EXPORT Make_ar_report_nbr(SALT34.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_ar_report_nbr(SALT34.StrType s) := InValidFT_invalid_numeric(s);
...
EXPORT Make_ar_roll(SALT34.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_ar_roll(SALT34.StrType s) := InValidFT_invalid_numeric(s);
EXPORT InValidMessage_ar_roll(UNSIGNED1 wh) := InValidMessageFT_invalid_numeric(wh);
 
EXPORT Make_ar_frame(SALT34.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_ar_frame(SALT34.StrType s) := InValidFT_invalid_numeric(s);
EXPORT InValidMessage_ar_frame(UNSIGNED1 wh) := InValidMessageFT_invalid_numeric(wh);
...
EXPORT Make_ar_status(SALT34.StrType s0) := MakeFT_invalid_ar_status(s0);
EXPORT InValid_ar_status(SALT34.StrType s) := InValidFT_invalid_ar_status(s);
EXPORT InValidMessage_ar_status(UNSIGNED1 wh) := InValidMessageFT_invalid_ar_status(wh);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT34,Scrubs_Corp2_Mapping_KS_AR;

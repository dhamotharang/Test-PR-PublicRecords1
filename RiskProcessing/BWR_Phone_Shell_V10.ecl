...
#WORKUNIT('name', 'Phone_Shell V1.0');
...
PhoneShell_version := 10; // 10 for 1.0; A different script should be used for 2.0.

IncludeFPScores := FALSE; // Default is FALSE (for v1)
...
// Score_Threshold_In := 217;//387 phoneshell v2.1 w/ new combined model //217 phoneshell v1 w/ v3 models //306 older v2 models //orig script had 245
...
layoutSOAPIn := RECORD
	...
	UNSIGNED PhoneShell_Version := 10;
...
OUTPUT(goodResults,, OutputFile + '_' + ThorLib.wuid(), OVERWRITE, named('GoodResults_fullTHOR')); // saves it as a THOR/FLAT file for later use in a script

...
// f := dataset('~jpyon::in::axcess_2408_p1_f_s_in',prii_layout, csv(quote('"')));
f := choosen(dataset('~jpyon::in::axcess_2408_p1_f_s_in',prii_layout, csv(quote('"'))), 100);
//f := choosen(dataset('~jpyon::in::sprint_pii_090709_subset_in', prii_layout, csv(quote('"'))),5);	// slimmed version of above sprint file															
...
//	self.HistoryDateYYYYMM := le.historydateyyyymm; //999999;
	...
	self.DataRestrictionMask := '100001000101'; // to restrict fares, experian and transunion.  Advo also restricted because this is presreen 

// to run in prescreen mode
...
// output(final,,'~jpyon::out::axcess_2408_p1_rvattr_out',CSV(heading(single), quote('"')), overwrite);
// output(final(errorcode<>''),,'~jpyon::out::axcess_2408_p1_rvattr_error',CSV(QUOTE('"')), overwrite);

#workunit('name','NonFCRA-np81-process');
...
// f := DATASET('~jpyon::in::keybank_1212_f_s_in',layout_prii,csv(quote('"')));
f := choosen(dataset('~jpyon::in::keybank_1212_f_s_in',layout_prii, csv(quote('"'))),5);
output(f);

//mapping
Scoring.Layout_NP2O_Soapcall into_NP2O_input(f le) := transform
	self.tribcode := 'np81';  // !! replace the tribcode here !!
...
self.gateways := dataset([{'attus','http://rw_data_prod:Password01@rwgateway.br.seisint.com:8090/wsGateway'},
...
output(s,,'~jpyon::out::keybank_1212_np81-test',csv(heading(single), quote('"')), overwrite);

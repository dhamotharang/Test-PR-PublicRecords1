#workunit('name','FCRA-2x16 Process');
...
f := dataset('~tfuerstenberg::in::t-mobile_589_2x16', layout_prii, csv(QUOTE('"')));
// f := choosen(dataset('~tfuerstenberg::in::t-mobile_589_2x16', layout_prii, csv(QUOTE('"'))),5);
...
Scoring.Layout_SC1O_Soapcall into_SC1O_input(f le) := transform
	self.tribcode := '2x16';  // !! replace the tribcode here !!
...
output(s,,'~tfuerstenberg::out::t-mobile_589_2x16_out',CSV(heading(single), QUOTE('"')), overwrite);
output(s_f(errorcode<>''),,'~tfuerstenberg::out::t-mobile_589_2x16_error',CSV(QUOTE('"')), overwrite);
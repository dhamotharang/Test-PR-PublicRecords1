...
outputfile := '~lweiner::out::fp201_attributes_scores_' + thorlib.wuid();
qa_outputfile := '~lweiner::out::FPA2_QA_' + thorlib.wuid();
...
layout_old_acct into_fdInput(f le, INTEGER c) := TRANSFORM
	self.SuppressCompromisedDLs := SuppressCompromisedDLs;  
...
IF(Include_Internal_Extras, OUTPUT(CHOOSEN(qa_results_rolled, eyeball), NAMED('Sample_QA_Results')));

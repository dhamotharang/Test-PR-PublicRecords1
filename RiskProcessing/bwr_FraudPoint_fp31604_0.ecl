...
#workunit('name','Fraudpoint FP31604_0 DID input only');
...
filename := ut.foreign_prod + 'jpyon::in::navy_7515_001_p1_in';
outputfile := '~khuls::out::FP31604_DID_only_' + thorlib.wuid();

eyeball 								:= 25;	//number of records to display in outputs
unsigned 	record_limit 	:= 25;   //number of records to process from input file; 0 means ALL
...
self.ExcludeIbehavior := true;  // set this back to false if they would like to include this data for their test
	...
	SELF.HistoryDateYYYYMM := (Integer) le.historydateyyyymm[1..6];  //to use history date from input file
	...
	// SELF.RequestedAttributeGroups := dataset([{'fraudpointattrv2'}], layout_attributes_in); //attributes will be returned
	SELF.RequestedAttributeGroups := dataset([{''}], layout_attributes_in); //attributes will not be returned
	self.model := 'fp31604_0';  
	self.IncludeRiskIndices := true; 
 	self.gateways := riskwise.shortcuts.gw_netacuityv4 /*+ riskwise.shortcuts.gw_targus*/; //Can uncomment targus gateway if needed
  
	//for fp31604_0, need to pass in the ModelRequests dataset because there are custom input fields (LexID) that it needs
	DID := (string)le.DID;
	Model_parms := dataset([{'custom','fp31604_0'},{'lexid',DID}], Models.Layouts.Layout_Model_Options);
 	self.ModelRequests := project(ut.ds_oneRecord, transform(models.layouts.Layout_Model_Request_In, 
													self.ModelName := 'customfa_service',
													self.ModelOptions := Model_parms;
			));		
	SELF := le;
	self := [];
end;

#workunit('name','Collection Shell 4.0');
...
//===================  input-output files  ======================
infile_name := '~tfuerstenberg::in::premiere_2038_pii_in';

outfile_name := '~dvstemp::out::collection_shell_premiere_2038_'+ thorlib.wuid();	// this will output your work unit number in your filename
...
//=============  Service settings ====================
//====================================================
// CollectionShell service
service_name := 'Risk_Indicators.Collection_Shell_Service';
...
l assignAccount (ds_input le, INTEGER c) := TRANSFORM
  ...
  SELF.DPPAPurpose := dppa;
	// it is possible to run this shell with a history date, but the waterfall phones piece, ADL append and best append pieces aren't
	// available in archive mode, so it makes the most sense to just run this process as current.  Jim C stated that most collection files
	// with contactability performance would be current files anyway
	SELF.HistoryDateYYYYMM := 999999;  
  SELF.datarestrictionmask := datarestrictionmask;
  self.model := 'csn1007_0';  //add
	self.bsversion := 4;		
  SELF := le;
  SELF := [];
END;
// p_f := PROJECT (ds_input, assignAccount (LEFT,COUNTER))(accountnumber='10041');
...
xlayout := RECORD
	unsigned8 time_s{xpath('_call_latency')} := 0;  // picks up timing
	unsigned8 time_ms{xpath('_call_latency_ms')} := 0;  // picks up timing
	risk_indicators.collection_shell_mod.collection_shell_layout_full;
	STRING200 errorcode;
END;

xlayout myFail(dist_dataset le) := TRANSFORM
	SELF.errorcode := FAILCODE + ' ' + FAILMESSAGE;
	SELF.cs.acctno := le.AccountNumber;
	SELF := [];
END;

roxie_result := SOAPCALL (dist_dataset, roxieIP, 
                    service_name, {dist_dataset}, 
                    DATASET (xlayout),
                    PARALLEL (parallel_calls), // take out parallel calls if using the 50x
										onFail(myFail (Left)));

output(choosen(roxie_result, eyeball), named('roxie_result'));
...
ox getold(roxie_result le, l ri) :=	TRANSFORM
  SELF.AccountNumber := ri.old_account_number;
  SELF := le;
END;

res := JOIN (roxie_result,p_f,LEFT.cs.acctno=RIGHT.accountnumber,getold(LEFT,RIGHT));
...
risk_indicators.collection_shell_mod.collection_shell_layout_edina_v4 convertToEdina_v4(f le) := transform
	// map the collection shell fields to the collection shell fields cs := le.cs, 
	//everything else in this conversion is the same as the shell 3.0 conversion
	self.cs := le.cs;
	...
	self.fd_scores.fraudpointV4 := le.FD_Scores.fraudpointV2;
	self.fd_scores.reason1FPV4 := le.FD_Scores.reason1FPV2;
	self.fd_scores.reason2FPV4 := le.FD_Scores.reason2FPV2;
	self.fd_scores.reason3FPV4 := le.FD_Scores.reason3FPV2;
	self.fd_scores.reason4FPV4 := le.FD_Scores.reason4FPV2;
	self.fd_scores.reason5FPV4 := le.FD_Scores.reason5FPV2;
	self.fd_scores.reason6FPV4 := le.FD_Scores.reason6FPV2;	
...
output(edina,, outfile_name+'_edina_v4',CSV(QUOTE('"')), overwrite);

#workunit('name','Fraudpoint Score and Attributes');
...
inputFile := '~jpyon::in::alerus_1031_newfile_f_s_in';
...
layout_old_acct into_fdInput(f le, INTEGER c) := TRANSFORM
	...
	SELF.DPPAPurpose := 3;
	SELF.GLBPurpose := 1;
	self.model := 'fp3710_0'; // coming in June... fp3904_1
...
Layout_FDAttributesOut := RECORD, maxlength(100000)
	...
	DATASET(models.Layout_Score) Scores;
...
resu := SOAPCALL(dist_dataset, roxieIP,
        ...
        RETRY(5), TIMEOUT(500),
        // We want the Score Results AND Attribute Results to come through, so no XPATH being used here...
...
roll_to_1_record := rollup(attr_plus_scores, true, combine_scores_attributes(left,right));
output(CHOOSEN(roll_to_1_record, eyeball), named('sample_rolled_records'));
output(roll_to_1_record,, outputFile, CSV(heading(single), quote('"')), overwrite);
#workunit('name','FCRA-2x15 Process');
...
f := dataset('~tfuerstenberg::in::t-mobile_589_2x15', layout_prii, csv(QUOTE('"')));
// f := choosen(dataset('~tfuerstenberg::in::t-mobile_589_2x15', layout_prii, csv(QUOTE('"'))),5);
...
Scoring.Layout_SC1O_Soapcall into_SC1O_input(f le) := transform
	self.tribcode := '2x15';  // !! replace the tribcode here !!
...
roxieIP :='http://roxiebatch.br.seisint.com:9876';   // roxiebatch
...
output(s,,'~tfuerstenberg::out::t-mobile_589_2x15_out',CSV(heading(single), QUOTE('"')), overwrite);
output(s_f(errorcode<>''),,'~tfuerstenberg::out::t-mobile_589_2x15_error',CSV(QUOTE('"')), overwrite);
#workunit('name', 'Business_Shell_V2');
...
BusShellVersion := 2; //version 2 blanks out the SBFE enhancement attributes. Should only be run for modelers when requested. v21 is used in production (see bwr_business_shell_v21).

inputFileName := '~hmccarl::in::bshell_test_inputs';

outputFile := '~bpahl::out::business_shell_v2_results_' + ThorLib.wuid();
...
SASBusShellFinal := Business_Risk_BIP.to_OutputLayout_SAS_v2(PROJECT(BusShellFinal, TRANSFORM(Business_Risk_BIP.Layouts.OutputLayout, SELF := LEFT))); //v2 returns slimmed SAS output layout.

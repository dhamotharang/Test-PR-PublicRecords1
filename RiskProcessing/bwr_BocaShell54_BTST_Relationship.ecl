#workunit('name','NonFCRA BT-ST Bocashell 5.4 Process');

/* *** Note that Netacuity is turned ON *** needs to use Cert gateway  */

import risk_indicators, riskwise, ut, _control;
...
//===================  input-output files  ======================
infile_name :=  '~foreign::' + _control.IPAddress.prod_thor_dali + '::' +'tfuerstenberg::in::Elevate_8809_BTST_ipaddr_in2.csv';
// infile_name  := ut.foreign_prod+'tfuerstenberg::in::walmart_8249_btst_in10.csv';
outfile_name := '~tfuerstenberg::out::Elevate_8809_BTST54_No_IPaddr_'+ thorlib.wuid();	// this will output your work unit number in your filename
...
l t_f(ds_input le, INTEGER c) := TRANSFORM
	...
	self.ipaddress := '';  // blanking out IPaddress from input file. Tim
...
billtoshell54 := project(f, transform(riskprocessing.layouts.layout_internal_shell, self.accountnumber := left.accountnumber, self.errorcode := left.errorcode, self := left.bill_to_out));
shiptoshell54 := project(f, transform(riskprocessing.layouts.layout_internal_shell, self.accountnumber := left.accountnumber, self.errorcode := left.errorcode, self := left.ship_to_out));
isFCRA := false;
string IntendedPurpose := '';  // leave blank in nonfcra

billto_edina54 := risk_indicators.ToEdina_54(billtoshell54, isFCRA, DataRestrictionMask, IntendedPurpose);
shipto_edina54 := risk_indicators.ToEdina_54(shiptoshell54, isFCRA, DataRestrictionMask, IntendedPurpose);
...
ox := record
	risk_indicators.Layout_Boca_Shell_Edina_v54 bto;
	risk_indicators.Layout_Boca_Shell_Edina_v54 sto;
	risk_indicators.Layout_EDDO eddo;
	riskwise.Layout_IP2O ip2o;
	risk_indicators.Layout_BocaShell_BtSt.BTST_Fields;
end;

j1 := join(f, billto_edina54, left.bill_to_out.seq=right.seq,
	transform(ox, 
						self.bto := right, 
						self.eddo := left.eddo, 
						self.ip2o := left.ip2o, 
...
edina_with_relationship := join(j1, shipto_edina54, left.sto.seq=right.seq,
	transform(ox, 
						self.sto := right, 
						self.btst_DeviceProvider1_score := left.btst_DeviceProvider1_score;     
						self.btst_DeviceProvider2_score := left.btst_DeviceProvider2_score;     
...
OUTPUT(edina_with_relationship,, outfile_name+'_edina_v54',CSV(QUOTE('"'),maxlength(8192)));

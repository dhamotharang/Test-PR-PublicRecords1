...
outfile_name := '~dvstemp::out::btst53'+ thorlib.wuid();	// this will output your work unit number in your filename
...
f := dataset(outfile_name, final_layout, thor);
...
billto_edina5 := risk_indicators.ToEdina_53(billtoshell5, isFCRA, DataRestrictionMask, IntendedPurpose);
shipto_edina5 := risk_indicators.ToEdina_53(shiptoshell5, isFCRA, DataRestrictionMask, IntendedPurpose);
...
btst_edina53 := join(j1, shipto_edina5, left.sto.seq=right.seq,
	transform(ox, self.sto := right, self := left));

output(btst_edina53, named('btst_edina53'));
output(btst_edina53,, outfile_name+'_edina_v53',CSV(QUOTE('"'),maxlength(8192)));

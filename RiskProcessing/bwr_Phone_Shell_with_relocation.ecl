...
prii_layout := RECORD
     ...
     INTEGER HistoryDateYYYYMM;
		 UNSIGNED8 DID;
END;
//Input := choosen(SORT(DATASET(InputFile, prii_layout, CSV(QUOTE('"'))), Account),RecordsToRun);
Input := SORT(DATASET(InputFile, prii_layout, CSV(QUOTE('"'))), Account);

// Input := IF(RecordsToRun <= 0, 

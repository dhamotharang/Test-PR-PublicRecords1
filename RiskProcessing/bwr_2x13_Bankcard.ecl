#workunit('name','FCRA-2x13 Process');
...
f := dataset('~tfuerstenberg::in::t-mobile_589_2x13', layout_prii, csv(QUOTE('"')));
// f := choosen(dataset('~tfuerstenberg::in::t-mobile_589_2x13', layout_prii, csv(QUOTE('"'))),5);
...
Scoring.Layout_SC1O_Soapcall into_SC1O_input(f le) := transform
	self.tribcode := '2x13';  // !! replace the tribcode here !!
...
output(s,,'~tfuerstenberg::out::t-mobile_589_2x13_out',CSV(heading(single), QUOTE('"')), overwrite);
output(s_f(errorcode<>''),,'~tfuerstenberg::out::t-mobile_589_2x13_error',CSV(QUOTE('"')), overwrite);
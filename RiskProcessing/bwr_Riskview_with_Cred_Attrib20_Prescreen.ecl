...
infile_name := '~tfuerstenberg::in::pnc_7397_in.csv';
outfile_name := '~dvstemp::out::prescreen_test_out_' + thorlib.wuid();

f := choosen(dataset(infile_name,prii_layout, csv(quote('"'))), eyeball);
...
output(choosen(j_f, eyeball), named('j_f'));
output(count(j_f(errorcode<>'')), named('error_count'));
output(choosen(j_f(errorcode<>''), eyeball), named('errors'));

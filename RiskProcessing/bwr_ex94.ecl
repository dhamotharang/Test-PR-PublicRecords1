#workunit('name','ex94 Non-FCRA Process');
...
f := dataset('~jpyon::in::ge_fraud_f_s_in', layout_prii, csv(QUOTE('"')));
//f := choosen(dataset('~jpyon::in::ge_fraud_f_s_in', layout_prii, csv(QUOTE('"'))),5);
...
Scoring.Layout_SC1O_Soapcall into_SC1O_input(f le) := transform
	self.tribcode := 'ex94';  // !! replace the tribcode here !!
...
output(s,,'~jpyon::out::ge_fraud_ex94_out',CSV(heading(single), quote('"')), overwrite);
output(s_f(errorcode<>''),,'~jpyon::out::ge_fraud_ex94_error',CSV(QUOTE('"')), overwrite);

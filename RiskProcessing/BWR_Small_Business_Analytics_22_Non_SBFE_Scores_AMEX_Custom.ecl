...
// same script as ScoringQA.RiskProcessing.BWR_Small_Business_Analytics_22_Non_SBFE_Scores just uses V101 attributes 
...
AttributesRequested := 
		DATASET([{'SmallBusinessAttrV101'}], iesp.share.t_StringArrayItem);//same script as ScoringQA.RiskProcessing.BWR_Small_Business_Analytics_22_Non_SBFE_Scores just AMEX is using V101 attributes  
...
BusinessModelRequested := IF(includeBusinessOnlyModel,
					DATASET([{'SLBO1809_0_0'}], iesp.share.t_StringArrayItem), // do not mix with slbb1702 models 
...
BlendedModelRequested := IF(includeBlendedModel, 
					DATASET([{'SLBB1809_0_0'}], iesp.share.t_StringArrayItem),
     DATASET([], iesp.share.t_StringArrayItem)); 
...
layout_flat_v1 flatten_v1(layout_soap le, SmallBusinessAnalyticsoutput ri) := TRANSFORM
	...
	// #IF(includeLN)
	V1AttributeResults := ri.Result.AttributeGroups(StringLib.StringToLowerCase(Name) = 'smallbusinessattrv101')[1].Attributes;
	...
	SELF.AssociateBankruptCount := V1AttributeResults[186].value;
	SELF.AssociateCountWithBankruptcy := V1AttributeResults[187].value;
	SELF.AssociateBankrupt1YearCount := V1AttributeResults[188].value;
	SELF.AssociateLienCount := V1AttributeResults[189].value;
	SELF.AssociateCountWithLien := V1AttributeResults[190].value;
	SELF.AssociateJudgmentCount := V1AttributeResults[191].value;
	SELF.AssociateCountWithJudgment := V1AttributeResults[192].value;
	SELF.AssociateHighRiskAddrCount := V1AttributeResults[193].value;
	SELF.AssociateWatchlistCount := V1AttributeResults[194].value;
	SELF.AssociateBusinessCount := V1AttributeResults[195].value;
	SELF.AssociateCityCount := V1AttributeResults[196].value;
	SELF.AssociateCountyCount := V1AttributeResults[197].value;
	// #END
 ...
 SLBO_Results := ri.Result.Models(TRIM(Name)='SLBO1809_0_0')[1];
	Model2 := IF(includeBusinessOnlyModel AND TRIM(SELF.Model1Name) <> 'SLBO1809_0_0', SLBO_Results, ri.Result.Models[2]);

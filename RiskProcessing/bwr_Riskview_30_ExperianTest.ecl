#workunit('name','Experian Riskview Test 3.0');
// outputting riskview 3.0 scores and attributes using Experian data
...
// part := 'part5';
part := 'part6';
infile := '~jpyon::in::exp_2549_' + part + '_f_s_in';
outfile := '~dvstemp::out::exp_2549_' + part + '_riskview30_' + thorlib.wuid();

f := dataset(infile,prii_layout, csv(quote('"')));				
// f := choosen(dataset(infile, prii_layout, csv(quote('"'))),5);									
...
//	self.HistoryDateYYYYMM := le.historydateyyyymm; //999999;
	...
	self.gateways := dataset([{'neutralroxie', riskwise.shortcuts.prod_batch_analytics_roxie}], risk_indicators.Layout_Gateways_In);

	self.DataRestrictionMask := '10000001010000'; // to restrict fares, equifax and transunion 
...
output(count(final(errorcode<>'')), named('errors'));

// *************************************************

output(final,,outfile,CSV(heading(single), quote('"')), overwrite);

...
unsigned1 parallel_threads := 30;  //max number of parallel threads = 30
 ...
 f := DATASET('~jpyon::in::fn_1056_23u_f_s_in',layout_prii,csv(quote('"')));
//f := choosen(dataset('~jpyon::in::bofa_807_file2_ciid_v2_in',layout_prii, csv(quote('"'))),15);
output(f);

//mapping
Scoring.Layout_NP2O_Soapcall into_NP2O_input(f le) := transform
	self.tribcode := 'np50';  // !! replace the tribcode here !!
...
output(s,,'~jpyon::out::fn_1056_under_np50_out',csv(heading(single), quote('"')), overwrite);

...
EXPORT SALT34.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_future_date','invalid_charter_nbr','invalid_numeric','invalid_numericblank','invalid_percentage','invalid_money');
EXPORT FieldTypeNum(SALT34.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_corp_vendor' => 2,'invalid_state_origin' => 3,'invalid_mandatory' => 4,'invalid_date' => 5,'invalid_future_date' => 6,'invalid_charter_nbr' => 7,'invalid_numeric' => 8,'invalid_numericblank' => 9,'invalid_percentage' => 10,'invalid_money' => 11,0);
...
EXPORT MakeFT_invalid_percentage(SALT34.StrType s0) := FUNCTION
  s1 := SALT34.stringfilter(s0,' .0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_percentage(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,' .0123456789'))));
EXPORT InValidMessageFT_invalid_percentage(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars(' .0123456789'),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_money(SALT34.StrType s0) := FUNCTION
  s1 := SALT34.stringfilter(s0,' $,.0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_money(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,' $,.0123456789'))));
EXPORT InValidMessageFT_invalid_money(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars(' $,.0123456789'),SALT34.HygieneErrors.Good);
...
EXPORT Make_ar_mailed_dt(SALT34.StrType s0) := MakeFT_invalid_date(s0);
EXPORT InValid_ar_mailed_dt(SALT34.StrType s) := InValidFT_invalid_date(s);
EXPORT InValidMessage_ar_mailed_dt(UNSIGNED1 wh) := InValidMessageFT_invalid_date(wh);
 
EXPORT Make_ar_due_dt(SALT34.StrType s0) := MakeFT_invalid_numericblank(s0);
EXPORT InValid_ar_due_dt(SALT34.StrType s) := InValidFT_invalid_numericblank(s);
...
EXPORT Make_ar_delinquent_dt(SALT34.StrType s0) := MakeFT_invalid_date(s0);
EXPORT InValid_ar_delinquent_dt(SALT34.StrType s) := InValidFT_invalid_date(s);
EXPORT InValidMessage_ar_delinquent_dt(UNSIGNED1 wh) := InValidMessageFT_invalid_date(wh);
 
EXPORT Make_ar_tax_factor(SALT34.StrType s0) := MakeFT_invalid_percentage(s0);
EXPORT InValid_ar_tax_factor(SALT34.StrType s) := InValidFT_invalid_percentage(s);
EXPORT InValidMessage_ar_tax_factor(UNSIGNED1 wh) := InValidMessageFT_invalid_percentage(wh);
...
EXPORT Make_ar_paid_date(SALT34.StrType s0) := MakeFT_invalid_date(s0);
EXPORT InValid_ar_paid_date(SALT34.StrType s) := InValidFT_invalid_date(s);
EXPORT InValidMessage_ar_paid_date(UNSIGNED1 wh) := InValidMessageFT_invalid_date(wh);
...
EXPORT Make_ar_amount_paid(SALT34.StrType s0) := MakeFT_invalid_money(s0);
EXPORT InValid_ar_amount_paid(SALT34.StrType s) := InValidFT_invalid_money(s);
EXPORT InValidMessage_ar_amount_paid(UNSIGNED1 wh) := InValidMessageFT_invalid_money(wh);
 
// This macro will compute and count field level differences based upon a pivot expression
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT34,Scrubs_Corp2_Mapping_IL_LLC_AR;

import address, did_add, didville,ut,header_slimsort,business_header,Business_Header_SS,lib_stringlib;
...
recordof(dFinancingStatement)  Trans_add_Jurisdiction(dFinancingStatement pLeft, dJurisidiction  pRight)
   :=
   TRANSFORM
...
Layout_UCC_Common.layout_ucc_new Trans_Rmsid(Layout_UCC_Common.layout_ucc_new pLeft,string2 c ) 
   :=
   TRANSFORM
     self.Rmsid                      :=if(trim(C,left,right)='1',pLeft.rmsid,trim(C,left,right)+'D'+pLeft.rmsid);
	 self                            :=pLeft;
	 end;
Layout_UCC_Common.layout_ucc_new Trans_rollback_Rmsid(Layout_UCC_Common.layout_ucc_new pInput) 
   :=
   TRANSFORM
...
Layout_UCC_Common.Filing Trans_add_signer(dFinancingStatement pLeft, dSigner pRight) 
   := 
   TRANSFORM
		  ...
		  self.orig_filing_date 	 := if(pLeft.FILG_STMT_TYPE_DEC='INITIAL FILING' Or pLeft.FILG_STMT_TYPE_DEC='ORIGINAL',pLeft.FILG_DT,pLeft.XREF_DT);
		  self.filing_date 		     := pLeft.FILG_DT;
		  self.page 				 := pLeft.FILG_NBR_PG;
		  self.vendor_entry_date 	 := pLeft.DT_ENTD;
		  self.vendor_upd_date 		 := pLeft.DT_LAST_UPD;
		  ...
		  self.expiration_date 		 := pLeft.FILG_EXPN_DT ;
...
Layout_UCC_Common.Filing Trans_cmn_signer(Layout_UCC_Common.Filing pLeft, dFinancingComment  pRight) 
   := 
   TRANSFORM
          ...
          self.cmnt_effective_date   := pRight.CMNT_DT;
          self.description 		     := if(pRight.SRC_INDV_NME>'',trim(pRight.SRC_INDV_NME,right)+';','')+
									    ...
									    if(pRight.CMNT_DESC>'',trim(pRight.CMNT_DESC)+';','')+
									    pRight.TXT ;
     	  self					     := pLeft;
  
END;
layout_collateral tproject(dCollateral	 pLeft)
  :=
   TRANSFORM
...
layout_collateral tRollupDuplicates(layout_collateral pLeft,  layout_collateral pRight)
  :=
   TRANSFORM
...
layout_collateral  TCollateral(layout_collateral pLeft,dCollateralItem pRight) 
   := 
   TRANSFORM
...
dsortSigner						     := distribute(dedup(dSigner,except process_date, ALL),hash(F_FING_ENTY_ID));
dsortStatement				         := distribute(dedup(dAddJurisdiction(~REGEXFIND('CA|MA|IL|TX',Filing_Jurisdiction))
															 ,except process_date,ALL),hash(F_FING_ENTY_ID));
dsortComment                         := distribute(dedup(dFinancingComment,except process_date, ALL),hash(F_FING_ENTY_ID));
dSortCollateral                      := sort(distribute(dedup(dCollateral,except process_date, def_data_length,fing_coll_seq_nbr,ALL),hash(F_FING_ENTY_ID)),F_FING_ENTY_ID,local);
dSortCItem	   						 := distribute(dedup(dCollateralItem,except process_date, ALL),hash(F_FING_ENTY_ID));

dJoinCollateral                      := join(rollup(project(dSortCollateral,tproject(left)),tRollupDuplicates(left,right),rmsid),
											dSortCItem,
											left.rmsid=right.F_FING_ENTY_ID,
											Tcollateral(left,right),
											left outer,local);
											
daddsigner   			             := Join(dsortStatement, 
                                             dsortSigner	,
											 left.F_FING_ENTY_ID = Right.F_FING_ENTY_ID, 
											 Trans_add_signer(left, right),
											 Left outer,local) ;
											 
dFiling                              := distribute(dedup(Join(daddsigner  , 
                                                                  dsortComment,
																	left.Rmsid = Right.F_FING_ENTY_ID, 
																	Trans_cmn_signer(left, right),
																	Left outer,local), 
														except process_date,all),hash(rmsid));

Layout_UCC_Common.layout_ucc_new Trans_add_collateral(dFiling pLeft, layout_collateral pRight)
   :=
   TRANSFORM
...
dAddCollateral                       :=  Join(dFiling  , 
                                              distribute(dJoinCollateral,hash(rmsid)),
											 left.Rmsid = Right.Rmsid, 
											 Trans_add_collateral(left, right),
											 Left outer,local);
dDedup                               :=  dedup(distribute(dAddCollateral+Project(dFile,Trans_rollback_Rmsid(left)),hash(rmsid)),except process_date,vendor_entry_date,vendor_upd_date, all, local);										 
dGroup                               :=  group(sort(distribute(ddedup,hash(tmsid,rmsid)),
                                             tmsid,rmsid,local),
									         tmsid,rmsid,local);

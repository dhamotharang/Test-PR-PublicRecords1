...
layout_party
         :=record
		  string14  Original_filing_number;
...
layout_party tBDName_Address(File_TX_BusinessDebtor_in pInput) 
   := 
   transform
			self.Orig_name 						:=	pInput.bd_name;
			self.Orig_address1				:=	pInput.bd_street_address; 
			self.Orig_city						:=	pInput.bd_city ;
			self.Orig_state						:=	pInput.bd_state;
			self.Orig_zip5						:=	pInput.bd_zip  ;
			self.Orig_zip4						:=	pInput.bd_zip_ext;
...
layout_party tBSName_Address(File_TX_BusinessSecuredParty_in pInput) 
   := 
   transform
			self.Orig_name 						:=	pInput.bs_name;
			self.Orig_address1				:=	pInput.bs_street_address;
			self.Orig_city						:=	pInput.bs_city ;
			self.Orig_state						:=	pInput.bs_state;
			self.Orig_zip5						:=	pInput.bs_zip  ;
			self.Orig_zip4						:=	pInput.bs_zip_ext;
...
layout_party tpDName_Address(File_TX_PersonDebtor_in pInput) 
   := 
   transform
...
layout_party tpSName_Address(File_TX_PersonSecuredParty_in pInput) 
   := 
   transform
...
dMain							:=  File_TX_Main_Base;
dSortmain					:=  sort(distribute(dedup(sort(project(dmain,layout_rmsid_tmsid),filing_date),except filing_date,rmsid,all),
																			hash(tmsid)),tmsid,local);

dBDebtor					:=	project(File_TX_BusinessDebtor_in,tBDName_Address(left));
dBSecuredParty		:=	project(File_TX_BusinessSecuredparty_in,tBSName_Address(left));
dPDebtor					:=  project(File_TX_PersonDebtor_in,tPDName_Address(left));
dPSecuredParty		:=  project(File_TX_PersonSecuredParty_in,tPSName_Address(left));
...
dCnameSort 							:=  sort(dCnamedist, tmsid,Orig_zip5, Orig_address1, company_name,sec_range,fein,
																		dt_vendor_last_reported, dt_vendor_first_reported, dt_last_seen, local);
...
dPartyJoin							:=  Join(	sort(distribute(dpartyout,hash(tmsid)),tmsid,local),
																	dSortmain ,
...
OutParty                :=  output(dPartyJoin   ,,UCCV2.cluster.cluster_out+'base::UCC::Party::TX',overwrite,__compressed__);

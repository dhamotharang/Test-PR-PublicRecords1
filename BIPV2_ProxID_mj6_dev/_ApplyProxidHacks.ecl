...
EXPORT _ApplyProxidHacks(
   string   pModule               = 'BIPV2_ProxID_mj6_dev'
...
module
  ...
  function
    ...
    hackReplaceTransitivesMacroCondition := pAttribute = 'matches' and      regexfind('SALT30[.]mac_avoid_transitives[(]All_Matches,Proxid1,Proxid2,Conf,DateOverlap,Rule,o[)];',EclCode,nocase)
                                                                   and not  regexfind('BIPV2_ProxID_mj6_dev[.]mac_avoid_transitives_scalene'                                    ,EclCode,nocase);
  ...
  function
    ...
    + 'EXPORT MatchHistoryName := \'~keep::BIPV2_ProxID_mj6_dev::Proxid::MatchHistory\';\n'
    + 'EXPORT MatchHistoryFile := DATASET(MatchHistoryName,Matches(In_DOT_Base).id_shift_r,THOR); // Read in all the change history\n'
    + ' \n'
    + 'EXPORT MatchHistoryKeyName := \'~\'+\'key::BIPV2_ProxID_mj6_dev::Proxid::History::Match\';\n'
    + '  MH := MatchHistoryFile;\n'
    + ' \n'
  ...
  function
    ...
    HackKeys  := 
      ...
      + '//  Cands0 := BIPV2_ProxID_mj6_dev.match_candidates(inhead).ForeignCorpkey_candidates;\n'
      + '  Cands0 := BIPV2_ProxID_mj6_dev._file_Foreign_Corpkey;/*HACK*/\n'
...
//////////////    
  ...
  function
    ...
    HackKeys  := 
      ...
      + 'IMPORT SALT24,BIPV2_ProxID_mj6_dev,ut,tools;\n'
      + 'STRING50 Proxidonestr := \'1234\'  : STORED(\'ProxidOne\');\n'
      + 'STRING20 Proxidtwostr := \'12345\'  : STORED(\'Proxidtwo\');\n'
      + 'combo := \'qa\';\n'
      + '\n'
      + 'BIPV2_ProxID_mj6_dev._fun_CompareService((unsigned6)Proxidonestr,(unsigned6)Proxidtwostr,combo);\n'
      + 'ENDMACRO;\n'
      ;

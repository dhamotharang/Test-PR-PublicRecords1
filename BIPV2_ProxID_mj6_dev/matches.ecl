...
//Fixed fields ->:cnp_name(20):cnp_number(12):prim_range(12):prim_name_derived(12):st(1):zip(6)
...
match_candidates(ih).Layout_Attribute_Matches CombineResults(match_candidates(ih).Layout_Attribute_Matches le,match_candidates(ih).Layout_Attribute_Matches ri) := TRANSFORM
  ...
  SELF := le;
END;
EXPORT All_Attribute_Matches := ROLLUP(AllAttrMatches,LEFT.Proxid1=RIGHT.Proxid1 AND LEFT.Proxid2=RIGHT.Proxid2,CombineResults(LEFT,RIGHT),LOCAL) : PERSIST('~temp::Proxid::BIPV2_ProxID_mj6_dev::attribute_matches',EXPIRE(Config.PersistExpire));
...
EXPORT All_Matches := all_matches0 : PERSIST('~temp::Proxid::BIPV2_ProxID_mj6_dev::all_m',EXPIRE(Config.PersistExpire)); // To by used by rcid and Proxid
...
o := BIPV2_ProxID_mj6_dev.mac_avoid_transitives_scalene(All_Matches,Proxid1,Proxid2,Conf,DateOverlap,Rule,MatchThreshold,10); // HACK - Use new transitives macro, bucket size 10*/

EXPORT PossibleMatches := o : PERSIST('~temp::Proxid::BIPV2_ProxID_mj6_dev::mt',EXPIRE(Config.PersistExpire));
SALT30.mac_get_BestPerRecord( All_Matches,rcid1,Proxid1,rcid2,Proxid2,o );
EXPORT BestPerRecord := o : PERSIST('~temp::Proxid::BIPV2_ProxID_mj6_dev::mr',EXPIRE(Config.PersistExpire));
...
EXPORT ClusterLinkages := o1 : PERSIST('~temp::Proxid::BIPV2_ProxID_mj6_dev::clu',EXPIRE(Config.PersistExpire));
...
EXPORT ToSlice := IF ( Config.DoSliceouts, DEDUP(SORT(DISTRIBUTE(BetterElsewhere(Pref_Margin>10),HASH(Proxid)),Proxid,-Pref_Margin,LOCAL),Proxid,LOCAL)); // 1024x better in new place
...
EXPORT PreIDs := BIPV2_ProxID_mj6_dev.Fields.UIDConsistency(ih); // Export whole consistency module
EXPORT PostIDs := BIPV2_ProxID_mj6_dev.Fields.UIDConsistency(Patched_Infile); // Export whole consistency module

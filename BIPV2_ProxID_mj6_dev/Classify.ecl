...
SHARED TotalClusters := specificities(h).TotalClusters;
  ...
  prim_range_tokens := PROJECT(specificities(h).prim_range_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.prim_range; SELF.TokenType := 3; SELF.Spc := LEFT.field_Specificity ));
  prim_name_derived_tokens := PROJECT(specificities(h).prim_name_derived_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.prim_name_derived; SELF.TokenType := 4; SELF.Spc := LEFT.field_Specificity ));
  st_tokens := PROJECT(specificities(h).st_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.st; SELF.TokenType := 5; SELF.Spc := LEFT.field_Specificity ));
  ...
  active_domestic_corp_key_tokens := PROJECT(specificities(h).active_domestic_corp_key_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.active_domestic_corp_key; SELF.TokenType := 14; SELF.Spc := LEFT.field_Specificity ));
  company_fein_tokens := PROJECT(specificities(h).company_fein_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.company_fein; SELF.TokenType := 15; SELF.Spc := LEFT.field_Specificity ));
  company_phone_tokens := PROJECT(specificities(h).company_phone_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.company_phone; SELF.TokenType := 16; SELF.Spc := LEFT.field_Specificity ));
  ...
  v_city_name_tokens := PROJECT(specificities(h).v_city_name_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.v_city_name; SELF.TokenType := 18; SELF.Spc := LEFT.field_Specificity ));
  hist_enterprise_number_tokens := PROJECT(specificities(h).hist_enterprise_number_values_persisted,TRANSFORM(SALT30.Layout_Classify_Token,SELF.TokenValue := (SALT30.StrType)LEFT.hist_enterprise_number; SELF.TokenType := 19; SELF.Spc := LEFT.field_Specificity ));
SHARED all_tokens0 := cnp_name_tokens + cnp_number_tokens + prim_range_tokens + prim_name_derived_tokens + st_tokens + zip_tokens + hist_duns_number_tokens + hist_domestic_corp_key_tokens + foreign_corp_key_tokens + unk_corp_key_tokens + ebr_file_number_tokens + active_duns_number_tokens + active_enterprise_number_tokens + active_domestic_corp_key_tokens + company_fein_tokens + company_phone_tokens + sec_range_tokens + v_city_name_tokens + hist_enterprise_number_tokens;
  all_tokens := SALT30.fn_process_multitokens(all_tokens0);
 
EXPORT TokenKeyName := '~'+'key::BIPV2_ProxID_mj6_dev::Proxid::Token::TokenKey';
 
EXPORT TokenKey := INDEX(all_tokens,{UNSIGNED4 TokenHash := HASH32(TokenValue),TokenType},{all_tokens},TokenKeyName);
 
EXPORT MultiTokenKeyName := '~'+'key::BIPV2_ProxID_mj6_dev::Proxid::Token::MultiTokenKey';
...
EXPORT ConceptKeyName := '~'+'key::BIPV2_ProxID_mj6_dev::Proxid::Token::ConceptKey';
...
shared company_csz_combinations := o_tot;
  Layout_ConceptTemplate Into(company_csz_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.v_city_name_filled => 18, le.st_filled => 5, le.zip_filled => 6,0);
    SELF.FieldNumber2 := MAP ( le.st_filled AND SELF.FieldNumber1 != 5 => 5, le.zip_filled AND SELF.FieldNumber1 != 6 => 6,0);
    SELF.FieldNumber3 := MAP ( le.zip_filled AND SELF.FieldNumber1 != 6 AND SELF.FieldNumber2 != 6 => 6,0);
    SELF := le;
  END;
...
shared company_addr1_combinations := o_tot;
  Layout_ConceptTemplate Into(company_addr1_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.prim_range_filled => 3, le.prim_name_derived_filled => 4, le.sec_range_filled => 17,0);
    SELF.FieldNumber2 := MAP ( le.prim_name_derived_filled AND SELF.FieldNumber1 != 4 => 4, le.sec_range_filled AND SELF.FieldNumber1 != 17 => 17,0);
...
EXPORT ConceptTemplatesKey := '~'+'key::BIPV2_ProxID_mj6_dev::Proxid::Token::ConceptKey';

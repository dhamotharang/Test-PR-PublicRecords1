...
EXPORT SALT30.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'cnp_name','cnp_number','prim_range','prim_name_derived','st','zip','hist_duns_number','hist_domestic_corp_key','foreign_corp_key','unk_corp_key','ebr_file_number','active_duns_number','active_enterprise_number','active_domestic_corp_key','company_fein','company_phone','sec_range','v_city_name','hist_enterprise_number','company_csz','company_addr1','company_address','dt_first_seen','dt_last_seen');
EXPORT FieldNum(SALT30.StrType fn) := CASE(fn,'cnp_name' => 1,'cnp_number' => 2,'prim_range' => 3,'prim_name_derived' => 4,'st' => 5,'zip' => 6,'hist_duns_number' => 7,'hist_domestic_corp_key' => 8,'foreign_corp_key' => 9,'unk_corp_key' => 10,'ebr_file_number' => 11,'active_duns_number' => 12,'active_enterprise_number' => 13,'active_domestic_corp_key' => 14,'company_fein' => 15,'company_phone' => 16,'sec_range' => 17,'v_city_name' => 18,'hist_enterprise_number' => 19,'company_csz' => 20,'company_addr1' => 21,'company_address' => 22,'dt_first_seen' => 23,'dt_last_seen' => 24,0);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT30,BIPV2_ProxID_mj6_dev;
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_cnp_name,1,0)+ IF( SELF.Diff_cnp_number,1,0)+ IF( SELF.Diff_prim_range,1,0)+ IF( SELF.Diff_prim_name_derived,1,0)+ IF( SELF.Diff_st,1,0)+ IF( SELF.Diff_zip,1,0)+ IF( SELF.Diff_hist_duns_number,1,0)+ IF( SELF.Diff_hist_domestic_corp_key,1,0)+ IF( SELF.Diff_foreign_corp_key,1,0)+ IF( SELF.Diff_unk_corp_key,1,0)+ IF( SELF.Diff_ebr_file_number,1,0)+ IF( SELF.Diff_active_duns_number,1,0)+ IF( SELF.Diff_active_enterprise_number,1,0)+ IF( SELF.Diff_active_domestic_corp_key,1,0)+ IF( SELF.Diff_company_fein,1,0)+ IF( SELF.Diff_company_phone,1,0)+ IF( SELF.Diff_sec_range,1,0)+ IF( SELF.Diff_v_city_name,1,0)+ IF( SELF.Diff_hist_enterprise_number,1,0);
  END;

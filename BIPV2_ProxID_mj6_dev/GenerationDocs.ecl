...
//  1. Company
// ------------------------------------
FIELD:cnp_name:FORCE(+):TYPE(string250):20,165
FIELD:cnp_number:FORCE:PROP:12,0
 
FIELD:active_duns_number:SWITCH0:PROP:21,18
FIELD:active_enterprise_number:FORCE(--):PROP:21,4
FIELD:active_domestic_corp_key:FORCE(--):PROP:21,0
 
FIELD:hist_enterprise_number:SWITCH0:PROP:21,5
FIELD:hist_duns_number:SWITCH0:PROP:21,163
FIELD:hist_domestic_corp_key:SWITCH0:PROP:21,9
FIELD:foreign_corp_key:SWITCH0:PROP:21,138
FIELD:unk_corp_key:SWITCH0:PROP:21,16
FIELD:ebr_file_number:SWITCH0:PROP:21,224
 
FIELD:company_fein:SWITCH0:PROP:20,85
FIELD:company_phone:SWITCH0:PROP:20,266
 
// ------------------------------------
//  2. Place
// ------------------------------------
FIELD:prim_range:FORCE:12,0
FIELD:prim_name_derived:FORCE(+):12,13
FIELD:sec_range:SWITCH0:PROP:11,152
FIELD:v_city_name:CONTEXT(st):FORCE(--):2,0
FIELD:st:LIKE(alpha):FORCE(+):1,0
FIELD:zip:LIKE(number):FORCE(+):6,6
 
CONCEPT:company_csz:v_city_name:st:zip:FORCE(+):6,8
CONCEPT:company_addr1:prim_range:prim_name_derived:sec_range:FORCE(+):17,169
CONCEPT:company_address:company_addr1:company_csz:FORCE(+):18,174
...
//  Attribute Files
// ------------------------------------
ATTRIBUTEFILE:ForeignCorpkey:NAMED(_file_Foreign_Corpkey):VALUES(company_charter_number<company_inc_state):FORCE(--,ALL):IDFIELD(Proxid):21,51
...
This process was told to produce 3 nines of accuracy on a population of 200000000 with    82701640 records.
...
but should average 21 points with a failed match substracting 21 points.
...
(This subtraction estimate is based upon 16% of clusters with 2 or more records have 2 or more values for hist_duns_number.)
...
but should average 21 points with a failed match substracting 18 points.
(This subtraction estimate is based upon 13% of clusters with 2 or more records have 2 or more values for foreign_corp_key.)
...
but should average 21 points with a failed match substracting 16 points.
...
but should average 20 points with a failed match substracting 18 points.
(This subtraction estimate is based upon 8% of clusters with 2 or more records have 2 or more values for company_fein.)
...
but should average 20 points with a failed match substracting 14 points.
(This subtraction estimate is based upon 26% of clusters with 2 or more records have 2 or more values for company_phone.)
...
This field is scaled to match with its parent company_addr1, on average that will mean multiplying by 48%
...
(This subtraction estimate is based upon 15% of clusters with 2 or more records have 2 or more values for sec_range.)
...
This field is scaled to match with its parent company_csz, on average that will mean multiplying by 66%
(Although the score is still computed to satisfy the forcing condition.)
...
This field is scaled to match with its parent company_address, on average that will mean multiplying by 78%
(Although the score is still computed to satisfy the forcing condition.)
...
The exact score will depend upon the global scarcity of ForeignCorpkey but should average 21.

import BIPV2_ProxID_mj6_dev,tools;
...
function
  ...
  kFile := BIPV2_ProxID_mj6_dev.Keys(BFile,version);

  odl   := PROJECT(CHOOSEN(KFile.Candidates(Proxid=Proxidone),100000),BIPV2_ProxID_mj6_dev.match_candidates(BFile).layout_candidates);
  odr   := PROJECT(CHOOSEN(KFile.Candidates(Proxid=ProxidTwo),100000),BIPV2_ProxID_mj6_dev.match_candidates(BFile).layout_candidates);
  k     := KFile.Specificities_Key;
  s     := GLOBAL(PROJECT(k,BIPV2_ProxID_mj6_dev.Layout_Specificities.R)[1]);
  odlv  := BIPV2_ProxID_mj6_dev.Debug(BFile,s).RolledEntities(odl);
  odrv  := BIPV2_ProxID_mj6_dev.Debug(BFile,s).RolledEntities(odr);

  BIPV2_ProxID_mj6_dev.match_candidates(BFile).layout_attribute_matches ainto(KFile.Attribute_Matches le) := TRANSFORM
    SELF := le;
  END;
  ...
  mtch  := BIPV2_ProxID_mj6_dev.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,Proxidone,Proxidtwo,0,0}],BIPV2_ProxID_mj6_dev.match_candidates(BFile).layout_matches),am);

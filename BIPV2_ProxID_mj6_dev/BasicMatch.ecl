...
SHARED  h00 := Specificities(ih).input_file;
  ...
  SHARED h00_match := h00( 
      0 + IF( cnp_name  IN SET(s.nulls_cnp_name,cnp_name), 0, 20 ) + IF( cnp_number  IN SET(s.nulls_cnp_number,cnp_number), 0, 12 ) + IF( active_duns_number  IN SET(s.nulls_active_duns_number,active_duns_number), 0, 21 ) + IF( active_enterprise_number  IN SET(s.nulls_active_enterprise_number,active_enterprise_number), 0, 21 ) + IF( active_domestic_corp_key  IN SET(s.nulls_active_domestic_corp_key,active_domestic_corp_key), 0, 21 ) + IF( hist_enterprise_number  IN SET(s.nulls_hist_enterprise_number,hist_enterprise_number), 0, 21 ) + IF( hist_duns_number  IN SET(s.nulls_hist_duns_number,hist_duns_number), 0, 21 ) + IF( hist_domestic_corp_key  IN SET(s.nulls_hist_domestic_corp_key,hist_domestic_corp_key), 0, 21 ) + IF( foreign_corp_key  IN SET(s.nulls_foreign_corp_key,foreign_corp_key), 0, 21 ) + IF( unk_corp_key  IN SET(s.nulls_unk_corp_key,unk_corp_key), 0, 21 ) + IF( ebr_file_number  IN SET(s.nulls_ebr_file_number,ebr_file_number), 0, 21 ) + IF( company_fein  IN SET(s.nulls_company_fein,company_fein), 0, 20 ) + IF( company_phone  IN SET(s.nulls_company_phone,company_phone), 0, 20 ) + IF( prim_range  IN SET(s.nulls_prim_range,prim_range), 0, 12 ) + IF( prim_name_derived  IN SET(s.nulls_prim_name_derived,prim_name_derived), 0, 12 ) + IF( sec_range  IN SET(s.nulls_sec_range,sec_range), 0, 11 ) + IF( v_city_name  IN SET(s.nulls_v_city_name,v_city_name), 0, 2 ) + IF( st  IN SET(s.nulls_st,st), 0, 1 ) + IF( zip  IN SET(s.nulls_zip,zip), 0, 6 ) >= Config.BasicMatchThreshold); // Potentially remove anything that would violate FORCE(+) constraints

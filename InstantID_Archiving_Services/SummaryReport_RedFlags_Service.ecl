/*--SOAP--
<message name="RedFlagsSummaryReportRequest"  wuTimeout="300000">	
	<part name="gateways" 						type="tns:XmlDataSet" cols="110" rows="4"/>
	<part name="_CompanyId"  					type="xsd:string"/>
	<part name="RedFlagsSummaryReportRequest" type="tns:XmlDataSet" cols="80" rows="30" />
</message>
*/

IMPORT InstantId_Archiving_Services, AutoStandardI, iesp;

EXPORT SummaryReport_RedFlags_Service := MACRO

  rec_in     := iesp.iidredflagsummary.t_RedFlagsSummaryReportRequest;
	ds_in      := DATASET([],rec_in) : STORED('RedFlagsSummaryReportRequest',few);	
  ...
  search_options := GLOBAL (first_row.Options);
	...
	redflags := InstantId_Archiving_Services.SummaryReport_RedFlags_Records.doSummaryRedFlagsReport(tempmod, dGateways);
	Results := redflags.Response;

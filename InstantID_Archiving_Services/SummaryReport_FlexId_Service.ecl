/*--SOAP--
<message name="FlexIDSummaryReportRequest"  wuTimeout="300000">	
	<part name="gateways" 						type="tns:XmlDataSet" cols="110" rows="4"/>
	<part name="_CompanyId"  					type="xsd:string"/>
	<part name="FlexIDSummaryReportRequest" type="tns:XmlDataSet" cols="80" rows="30" />
</message>
*/

IMPORT InstantId_Archiving_Services, AutoStandardI, iesp;

EXPORT SummaryReport_FlexId_Service := MACRO

  rec_in     := iesp.flexidsummary.t_FlexIDSummaryReportRequest;
	ds_in      := DATASET([],rec_in) : STORED('FlexIDSummaryReportRequest',few);	
  ...
  search_options := GLOBAL (first_row.Options);
	...
	Results := InstantId_Archiving_Services.SummaryReport_FlexId_Records.doSummaryFlexIDReport(tempmod, dGateways);

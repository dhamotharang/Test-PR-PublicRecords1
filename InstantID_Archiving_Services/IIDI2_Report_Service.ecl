...
<message name="InstantIDArchiveSingleReportRequest"  wuTimeout="300000">	
	...
	<part name="InstantIDIntl2ArchiveSingleReportRequest" type="tns:XmlDataSet" cols="80" rows="30" />
</message>
*/

IMPORT AutoStandardI, iesp, InstantID_Archiving_Services, Gateway;

EXPORT IIDI2_Report_Service := MACRO

  rec_in     := iesp.iidi2singlereport.t_InstantIDIntl2ArchiveSingleReportRequest;
	ds_in      := DATASET([],rec_in) : STORED('InstantIDIntl2ArchiveSingleReportRequest',few);	
  ...
  search_options := GLOBAL (first_row.Options);
	...
	results := InstantID_Archiving_Services.IIDI2_Report_Records.doSingleReport(tempmod, dGateways, search_by);

...
<message name="InstantIDInternationalSummaryReportRequest"  wuTimeout="300000">	
	...
	<part name="InstantIDInternationalSummaryReportRequest" 	type="tns:XmlDataSet" cols="80" rows="30" />
</message>
*/

IMPORT InstantId_Archiving_Services, AutoStandardI, iesp;

EXPORT SummaryReport_InstantIDInternational_Service := MACRO

  rec_in     := iesp.iidisummary.t_InstantIDInternationalSummaryReportRequest;
	ds_in      := DATASET([],rec_in) : STORED('InstantIDInternationalSummaryReportRequest',few);	
  ...
  search_options := GLOBAL (first_row.Options);
	...
	Results := InstantId_Archiving_Services.SummaryReport_InstantIDInternational_Records.doSummaryIIDIReport(tempmod, dGateways);

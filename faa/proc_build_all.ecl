...
export proc_build_all(string version, string pSource, STRING srcIP) :=
function
	#workunit('name','FAA Build ' + version);
	import orbit_report,scrubs_faa;
	spray_all := faa.Spray_In(version, pSource, srcIP) : success(output('SPRAY FAA FILES SUCCESS')), failure(output('SPRAY FAA FILES FAILED'));

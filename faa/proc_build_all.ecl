...
export proc_build_all(string version, string pSource) :=
function
	#workunit('name','FAA Build ' + version);
	import orbit_report,scrubs_faa;
	spray_all := faa.Spray_In(version, pSource) : success(output('SPRAY FAA FILES SUCCESS')), failure(output('SPRAY FAA FILES FAILED'));

import ut, header_services, faa,mdr;

Orig_layout := record
faa.layout_airmen_data_out and not source;
end;

ds := dataset('~thor_data400::base::faa_airmen_building',faa.layout_airmen_data_out,flat);

header_services.Supplemental_Data.mac_verify('file_faa_inj.thor',Orig_layout,attr);
 
supp_data_in := attr();

start := max(ds,(unsigned)unique_id);

faa.layout_airmen_data_out add_id(Orig_layout l) := transform
...
ded_outseq :=dedup(sort( distribute(project(f, transform({f}, self.zip_code 			:= trim(left.zip_code,left,right),

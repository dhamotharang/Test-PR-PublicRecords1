IMPORT Gong_Neustar, Header, DNB_dmi, IRS5500, Corp2, Business_Header_SS, ut, versioncontrol,mdr;

EXPORT BC_Stats(

	 dataset(Layout_Business_Header_Base							) pBusinessHeaders			= Files().Base.Business_Headers.built
	,dataset(Layout_BH_Best														) pBusinessHeadersBest	= Files().Base.Business_Header_Best.built
	,dataset(Layout_Business_Contact_Full_new					) pBusinessContacts			= Files().Base.Business_Contacts.built
	...
	,dataset(dnb_dmi.Layouts.base.CompaniesForBIP2		) pFile_DNB_Base				= DNB_dmi.Files().base.companies.qa
	,dataset(irs5500.Layout_IRS5500_Base							) pFile_IRS5500_Base		= IRS5500.File_IRS5500_Base
	,dataset(Corp2.Layout_Corporate_Direct_Corp_Base	) pCorp2_Base_Corp			= Corp2.Files().Base.Corp.Prod
	,dataset(Layout_Business_Contact_Plus							) pBusinessContactsPlus	= Files().Base.Business_Contacts_Plus.keybuild
...
function
	...
	Layout_Business_Contacts_Stats AddEqFlag(contacts_check_eq l, Layout_Business_Contact_Full_new r) := TRANSFORM
	...
	Layout_Business_Contacts_Stats toLayout_Business_Contacts_Stats(contacts l) := transform
		self := l;
	end;
	...
	Layout_Business_Contacts_Stats AddSourceStats(contacts_eq l, bs_all r) := TRANSFORM
		SELF.dnb_emps := r.dnb_emps;
		SELF.irs5500_emps := r.irs5500_emps;
	...
	Layout_Business_Contacts_Stats AddStats(empf_dist l, addr_all r) := TRANSFORM
		SELF.bdid_per_addr := r.bdid_per_addr;
		SELF.apts := r.apts;
		SELF.ppl := r.ppl;
		SELF.r_phone_per_addr := r.r_phone_per_addr;
	...
	Layout_Business_Contacts_Stats ScoreContact(with_stats_dist r) := TRANSFORM

...
cnf := Business_Header_SS.File_CNP_ForStats;
...
corp_base_recs := 
	Corp.File_Corp_Base(business_header.is_ActiveCorp(record_type, corp_status_cd, corp_status_desc));
...
// from credit headers.
contacts := Business_Header.File_BCP_ForStats;
contacts_check_eq := contacts(did != 0, from_hdr != 'E');

Layout_Business_Contacts_Stats AddEqFlag(contacts_check_eq l, Header.File_Eq_Employer r) := TRANSFORM
...
contacts_eq_j1 := JOIN(
	...
	DISTRIBUTE(Header.File_Eq_Employer, HASH(did)),
	LEFT.did = RIGHT.did AND
	ut.CompanySimilar100(LEFT.company_name, RIGHT.occupation_employer) < 10,
...
ut.MAC_SF_BuildProcess(scored_stats,'BASE::People_At_Work_Stats',theout)

EXPORT BC_Stats := theout;
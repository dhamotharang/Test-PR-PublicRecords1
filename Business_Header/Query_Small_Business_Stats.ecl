import ut, Business_Header, DNB, DCA, Corp, UCC, Busreg, Bankrupt, InfoUSA, NID;
...
sb_contacts_grp_sort := sort(sb_contacts_grp, lname, NID.PreferredFirstVersionedStr(fname, NID.version), if(did <> 0, 0, 1), did, ut.TitleRank(company_title), mname);
...
sb_auth_contacts_did := dedup(table(sb_contacts(did <> 0), layout_sb_contacts_did), all);

...
#workunit ('name', 'Build Business_Header_Output');
...
bh_date_fix := project(bh, fixbhdates(left));

// Rollup base to discard blank addresses if current clean name name, source, vendor_id exists with an address
BH_Base_Dist := distribute(bh_date_fix, hash(source, trim(vendor_id), trim(ut.CleanCompany(company_name))));
...
SELF.gong_flag := IF(L.source in ['GB','GG'], 'Y', 'N');
SELF.yellowpages_flag := IF(L.source = 'Y', 'Y', 'N');
SELF.corp_flag := IF(L.source = 'C', 'Y', 'N');
SELF.ucc_flag := IF(L.source = 'U', 'Y', 'N');
SELF.bankruptcy_flag := IF(L.source = 'B', 'Y', 'N');
SELF.domain_flag := IF(L.source = 'W', 'Y', 'N');
SELF.fbn_flag := IF(L.source = 'F', 'Y', 'N');
SELF.busreg_flag := IF(L.source = 'BR', 'Y', 'N');
SELF.edgar_flag := IF(L.source = 'E', 'Y', 'N');
SELF.dnb_flag := IF(L.source = 'D', 'Y', 'N');
SELF.irs_5500_flag := IF(L.source = 'I', 'Y', 'N');
SELF.employee_directory_flag := IF(L.source = 'ED', 'Y', 'N');
SELF.fdic_flag := IF(L.source = 'FD', 'Y', 'N');
SELF.sales_tax_flag := IF(L.source = 'ST', 'Y', 'N');
SELF.sec_broker_flag := IF(L.source = 'SB', 'Y', 'N');
SELF.fl_non_profit_flag := IF(L.source = 'FN', 'Y', 'N');
SELF.workers_comp_flag := IF(L.source = 'WC', 'Y', 'N');
SELF.vickers_flag := IF(L.source = 'V ', 'Y', 'N');
SELF.irs_non_profit_flag := IF(L.source = 'IN', 'Y', 'N');
SELF.prof_lic_flag := IF(L.source = 'PL', 'Y', 'N');
SELF.fl_fbn_flag := IF(L.source = 'FF', 'Y', 'N');
SELF.dea_flag := IF(L.source = 'DE', 'Y', 'N');
SELF.tradeshow_flag := IF(L.source = 'AT', 'Y', 'N');
SELF.ska_flag := IF(L.source = 'SK', 'Y', 'N');
SELF.liens_flag := IF(L.source = 'LJ', 'Y', 'N');
SELF.credit_union_flag := IF(L.source = 'CU', 'Y', 'N');
SELF.fcc_flag := IF(L.source = 'FC', 'Y', 'N');
SELF.ebr_flag := IF(L.source = 'EB', 'Y', 'N');
SELF.bbb_member_flag := IF(L.source = 'BM', 'Y', 'N');
SELF.bbb_nonmember_flag := IF(L.source = 'BN', 'Y', 'N');
SELF.infousa_abius_flag := IF(L.source = 'IA', 'Y', 'N');
SELF.infousa_idexec_flag := IF(L.source = 'II', 'Y', 'N');
SELF.infousa_deadco_flag := IF(L.source = 'ID', 'Y', 'N');
SELF.infousa_fbns_flag := IF(L.source = 'IF', 'Y', 'N');
SELF.vehicles_flag := IF(L.source = 'MV', 'Y', 'N');
SELF.watercraft_flag := IF(L.source = 'AW', 'Y', 'N');
SELF.ln_property_flag := IF(L.source = 'LP', 'Y', 'N');
...
BHF_Out_DNB := BHF_Out_Init(source='D');
BHF_Out_DNB_Dist := DISTRIBUTE(BHF_Out_DNB, HASH(bdid));

BHF_Out_Not_DNB := BHF_Out_Init(source<>'D');
BHF_Out_Not_DNB_Dist := DISTRIBUTE(BHF_Out_Not_DNB, HASH(bdid));
...
BHF_Out_DNB_NoMatch := JOIN(BHF_Out_DNB_Dist,
...
BHF_Out_Dist := DISTRIBUTE(BHF_Out_Flags, HASH(bdid, TRIM(match_company_name), zip));
BHF_Out_Sort := SORT(BHF_Out_Dist, bdid, match_company_name, zip, prim_name, prim_range,
...
BHF_Out_Filtered := BHF_Out_Rollup(zip <> '' or (zip = '' and (source = 'D' or phone <> '' or fein <> '' or (city <> '' and state <> ''))));
BHF_Out_Blank_Address := BHF_Out_Rollup(not(zip <> '' or (zip = '' and (source = 'D' or phone <> '' or fein <> '' or (city <> '' and state <> '')))));
...
BHF_Out_Combined := BHF_Out_Filtered + 	BHF_Out_Blank_Address_Unique;						 
									 
Business_Header.Layout_Business_Header_Out FormatOutput(Layout_Business_Header_Rollup L) := TRANSFORM
SELF := L;
END;

BHF_Out := PROJECT(BHF_Out_Combined, FormatOutput(LEFT));

out1 := OUTPUT(BHF_Out,,'OUT::Business_Header', OVERWRITE);
...
BH_Relatives_Out := project(Business_Header.File_Business_Relatives(
...
out2 := output(BH_Relatives_Out,,'OUT::Business_Relatives', overwrite);
...
BH_Relatives_Group_Out := project(Business_Header.File_Business_Relatives_Group, FormatBusinessRelativesGroup(left));

out3 := output(BH_Relatives_Group_Out,,'OUT::Business_Relatives_Group', overwrite);
...
BH_Best_Out := project(Business_Header.File_Business_Header_Best, FormatBHBest(left));

out4 := output(BH_Best_Out,,'OUT::Business_Header_Best', overwrite);

string6 format_stat(unsigned4 i) := INTFORMAT(i, 6, 1);

// Generate the Business Header Stats File
Business_Header.Layout_Business_Header_Stat_Out FormatBHStat(Business_Header.Layout_Business_Header_Stat L) := TRANSFORM
SELF.bdid := (STRING12)INTFORMAT(L.bdid, 12, 1);
...
BH_Stat_Out := project(bhstats, FormatBHStat(LEFT));

bh_Stat_overflow := bhstats(business_header.isStatOverflow(bhstats));
out5 := output(bh_Stat_overflow,,'BASE::Business_Header_Stat_overflow', overwrite);
out6 := output(count(bh_Stat_overflow), named('STAT_OVERFLOW_HAD_BETTER_BE_ZERO'));

out7 := output(BH_Stat_Out,,'OUT::Business_Header_Stat', overwrite);

outs := parallel(out1, out2, out3, out4, out5, out6, out7);

export make_business_header_out := outs;	
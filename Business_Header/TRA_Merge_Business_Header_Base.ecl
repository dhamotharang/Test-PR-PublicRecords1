...
EXPORT Layout_Business_Header_Base TRA_Merge_Business_Header_Base(Layout_Business_Header_Base L, Layout_Business_Header_Base R) := TRANSFORM
SELF.source := IF((L.group1_id = 0 AND R.group1_id <> 0) OR
                  L.source = '', R.source, L.source);
...
SELF.current := IF(L.current = TRUE, L.current, R.current);
SELF.match_company_name := IF(L.match_company_name = '', R.match_company_name, L.match_company_name);
SELF.match_branch_unit := IF(L.match_branch_unit = '', L.match_branch_unit, R.match_branch_unit);
SELF.match_geo_city := IF(L.match_geo_city = '', L.match_geo_city, R.match_geo_city);
SELF := L;
END;
...
iesp.WsSearchCore.t_SearchRequest into_req() := transform
	...
	SELF.Input.EntityRecords := project(indata, transform(iesp.WsSearchCore.t_InputEntityRecord,
			...
			iesp.WsSearchCore.t_InputAdditionalInfo SetDOB() := TRANSFORM
				boolean is_dob := left.dob not in ['', '0'] and dob_radius <> -1;
				SELF._Type := IF (is_dob, 'DOB', 'None');
				SELF.Value := IF (is_dob, DOB, ''); //format MM/DD/YYYY
				SELF := [];
      END;
			self.AdditionalInformation := DATASET([SetDOB()]);

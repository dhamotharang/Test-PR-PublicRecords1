...
shared glb_ok := death_params.isValidGlb();
  ...
  export getSpouseDidBatch(dataset($.WorkPlace_Layouts.subjectSpouse) inDids) := FUNCTION
    ...
    doxie_raw.Layout_RelativeRawBatchInput xform_in($.WorkPlace_Layouts.subjectSpouse l) := TRANSFORM
    self.input.did := l.did;
    self.input.seq := (unsigned)l.acctno;
    self.seq := (unsigned)l.acctno;
    self := [];
    end;

    dids_for_rels := GROUP(SORT(PROJECT(indids, xform_in(LEFT)),seq),seq);

    relatives_1st := UNGROUP(Doxie_Raw.relative_raw_batch(dids_for_rels, mod_access, 1, true,true));
    recs_fltrd := relatives_1st(depth=1 AND titleNo in Header.relative_titles.set_Spouse);

    recs_ready := dx_death_master.Exclude(recs_fltrd, person2, death_params);

    // only returning records where spouse info is found
    // account records without spousedid are dropped from results. They are added later in upper layer
    outDids := PROJECT(recs_ready,
                      TRANSFORM($.WorkPlace_Layouts.subjectSpouse,
                          SELF.spousedid := LEFT.person2;
                          SELF.acctno := (STRING) LEFT.input.seq,
                          SELF.did := LEFT.input.did));
  ...
  EXPORT getBusinessListingBatch(dataset($.WorkPlace_Layouts.PhoneListingBatch) phones):= FUNCTION
    optout_layout := RECORD
      $.WorkPlace_Layouts.PhoneListingBatch;
      unsigned6 key_did;
    ...
    dk := PROJECT(dk_optout, $.WorkPlace_Layouts.PhoneListingBatch);
    RETURN dk;
  END;
  ...
  EXPORT getBusinessPhoneBatch(dataset($.WorkPlace_Layouts.bdidPhoneBatch) bdids) := FUNCTION
    ...
    optout_layout := RECORD
      $.WorkPlace_Layouts.bdidPhoneBatch;
      unsigned6 key_did;
      unsigned4 global_sid;
      unsigned8 record_sid;
    END;
    ...
    gh_slim := PROJECT(gh_slim_optout, $.WorkPlace_Layouts.bdidPhoneBatch);
    RETURN gh_slim;
  END;
  ...
  export CleanCompName(dataset(WorkPlace_Services.Layouts.poe_didkey_plus) ds_inRecs,
    ...
    ds_inrecs_compname_checked := project(ds_inrecs,
      transform(WorkPlace_Services.Layouts.poe_didkey_plus,
        ...
        self.company_name := IF(company_name IN $.WorkPlace_Constants.INVALID_COMPANY_NAMES_SET OR
                               (~IncludeSelfRepCompanyName AND company_name IN $.WorkPlace_Constants.SELF_REP_COMPANY_NAMES_SET), '',
                                 LEFT.company_name);

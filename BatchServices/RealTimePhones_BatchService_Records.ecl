import address, doxie_raw, Gateway;

input_layout := BatchServices.Layouts.RTPhones.rec_batch_RTPhones_input;

EXPORT RealTimePhones_BatchService_Records(dataset(input_layout) f_in, 
...
// TEN DIGIT
	...
	tenDigitGW := BatchServices.RealTimePhones_BatchService_Gateway(Pre_tenDigit, in_gateways, gmod,'a');
	tenDigitIH := BatchServices.SearchInhouse(gmod,tenDigitGW);
	tenDigitDeNormed := BatchServices.fillInhouse(gmod,tenDigitIH,in_gateways,'a');
...
// NAME ADDR
	...
	nameAddrGW := BatchServices.RealTimePhones_BatchService_Gateway(Pre_nameAddr, in_gateways, gmod,'b');
	nameAddrIH := BatchServices.SearchInhouse(gmod,nameAddrGW);
	nameAddrDeNormed := BatchServices.fillInhouse(gmod,nameAddrIH,in_gateways,'b');
  ...
  tot_10_na_ds_rolled := BatchServices.RT_Rollup(tot_10_na_ds,gmod.maxResults);
	...
	nameSSNGW := BatchServices.RealTimePhones_BatchService_Gateway(Pre_nameSSN, in_gateways, gmod,'c1');
	nameSSNIH := BatchServices.SearchInhouse(gmod,nameSSNGW);
	nameSSNDeNormed := BatchServices.fillInhouse(gmod,nameSSNIH,in_gateways,'c1');
  ...
  tot_plus_ns_ds_rolled := BatchServices.RT_Rollup(tot_plus_ns_ds,gmod.maxResults);
	...
	nameSSN2GW := BatchServices.RealTimePhones_BatchService_Gateway(Pre_name2SSN, in_gateways, gmod,'c2');
  ...
  tot_plus_ns2_ds_rolled := BatchServices.RT_Rollup(tot_plus_ns2_ds,gmod.maxResults);
	...
	nameSSN3GW := BatchServices.RealTimePhones_BatchService_Gateway(Pre_name3SSN, in_gateways, gmod,'c3');
  ...
  tot_plus_ns3_ds_rolled := BatchServices.RT_Rollup(tot_plus_ns3_ds,gmod.maxResults);
	...
	nameSSN4GW := BatchServices.RealTimePhones_BatchService_Gateway(Pre_name4SSN, in_gateways, gmod,'c4');
  ...
  tot_plus_ns4_ds_rolled := BatchServices.RT_Rollup(tot_plus_ns4_ds,gmod.maxResults);
	...
	LinkIdDeNormed := BatchServices.fillInhouse(gmod,LinkIDIH,in_gateways,'d');
  ...
  tot_plus_lid_ds_rolled := BatchServices.RT_Rollup(tot_plus_lid_ds,gmod.maxResults);
	...
	AddrOnlyDeNormed := BatchServices.fillInhouse(GMOD,AddrOnlyIH,in_gateways,'e');
...
f_ds := project(totalDS_rolled,f_tran(LEFT));
  // output(tenDigitGW,named('tenDigitGW'));
   // output(tenDigitIH,named('tenDigitIH'));
   // output(tenDigitDeNormed,named('tenDigitDeNormed'));
    // output(AddrOnlyih,named('AddrOnlyih'));
    // output(AddrOnlyDeNormed,named('AddrOnlyDeNormed'));
    // output(totalDS,named('totalDS'));
   // output(totalDS_rolled,named('ORIG_OUTPUT'));

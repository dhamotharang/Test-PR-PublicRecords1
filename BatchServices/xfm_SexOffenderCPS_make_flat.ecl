...
SHARED displayOffense(BatchServices.Layouts.SexOffenderCPS.rec_offense_plus_acctno offense) := 
	FUNCTION
		RETURN TRIM( TRIM(offense.offense_code_or_statute) + ' '
							 + TRIM(offense.offense_description) + ' ' 
							 + TRIM(offense.offense_description_2 ) );
	END;

SHARED displayCourt(BatchServices.Layouts.SexOffenderCPS.rec_offense_plus_acctno offense) := 
	FUNCTION
		RETURN TRIM( TRIM(offense.conviction_jurisdiction) + ' ' 
		           + TRIM(offense.court) );
	END;

EXPORT layout_SexOffenderCPS_Batch_out xfm_SexOffenderCPS_make_flat(layout_SexOffenderCPS_Batch_out l, 
                                                                    BatchServices.Layouts.SexOffenderCPS.rec_offense_plus_acctno r, 
																																		INTEGER C) := 
	TRANSFORM

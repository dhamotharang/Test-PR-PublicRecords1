...
EXPORT TaxRefundISv3_BatchService_Functions := MODULE
  ...
  EXPORT ProcessHRICodes(boolean isv3fdn) := FUNCTION 
    string  in_AddressRiskHRICodes  := '' : stored('AddressRiskHRICodes');    
    string  in_IdentityRiskHRICodes := '' : stored('IdentityRiskHRICodes');
    string  in_ReportOnlyHRICodes   := '' : stored('ReportOnlyHRICodes');
    string  in_AllHRICodes          := '';
    //
    ...
    //
    aRisk := Std.Str.ToUppercase(TRIM(in_AddressRiskHRICodes, ALL));
    iRisk := Std.Str.ToUppercase(TRIM(in_IdentityRiskHRICodes, ALL));
    rOnly := Std.Str.ToUppercase(TRIM(in_ReportOnlyHRICodes, ALL));
    ...
    finalResults := MODULE
      EXPORT string AddressRiskHRICodes  := IF (isv3fdn, aRisk2, in_AddressRiskHRICodes);    
      EXPORT string IdentityRiskHRICodes := IF (isv3fdn, iRisk2, in_IdentityRiskHRICodes);
      EXPORT string ReportOnlyHRICodes   := IF (isv3fdn, rOnly2, in_ReportOnlyHRICodes);
      EXPORT string AllHRICodes          := IF (isv3fdn, aRisk2 + ',' + iRisk2 + ',' + rOnly2, in_AllHRICodes);
    END;    
    RETURN finalResults;
  END;

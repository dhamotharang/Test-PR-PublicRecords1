IMPORT Address, Address_Attributes, Gateway, Royalty, STD, ut, DeathV2_Services, RiskWise;
...
EXPORT TaxRefundISv3_BatchService_Records(
  ...
  // booleans used in multiple places to make the coding more understandable
  ...
  ds_addrBest_res := BatchServices.TaxRefundISv3_BatchService_Functions.getBestAddress(ds_clean_batch_w_did,
   																																											 args_in);
   
   	// *--- Run through Deceased batch service ---* //
  set_deceased_match_codes := ['ANSZC', 'ANSZ', 'ANSC', 'ANS', 'SNCZ', 'SNC', 'SNZ', 
   	                             'SN', 'S'];
  ...
  ds_phone_res := BatchServices.TaxRefundISv3_BatchService_Functions.getPhoneRecords(
   	...
   	// *--- Unless input option IncludeAddrGW was intentionally turned off or
    	//      the input "Gateways" dataset is empty, 
   	// *--- Run input subjects through netacuity gateway to get ip address info ---* //
  ds_ip_address_res := 
                       if(IncludeGateway and GatewaysInIsNotEmpty,
  ...
  rec_out xformOutRecs(ds_clean_batch_w_did L) := transform
       ...
       self.best_ssn := if((ssn_rec.best_ssn <> L.ssn) and 
   		                     (ssn_rec.score >= args_in.DIDScoreThreshold), ssn_rec.best_ssn, '');
   		...
   		// req 4.1.21
   		self.did := if(ssn_rec.score >= args_in.DIDScoreThreshold, (string)L.did, ''); // return if score >= 80 ???
      ...
      best_lat   := (REAL) addr_rec.latitude;
    	best_long  := (REAL) addr_rec.longitude;
   		best_dist  := (unsigned) Address_Attributes.functions.GeoDist(
   		                                 input_lat,input_long,best_lat,best_long);
   	
      self.BestAddrChangeDistance := if(clean_batch_rec.prim_name ='' or 
   		                                  (input_lat = 0 and input_long = 0) or 
   		                                  (best_lat = 0  and best_long = 0),
   																			'-1',
   																			if(best_dist < 10000, (string4) best_dist,
   																			   '9999'));
      ...
      self.doc_state_origin := if (use_crim_rec, crim_rec.state_origin,'');
       ...
       // req. 4.1.46 (Revised for TRIS v3 2015 enhancements req 4.1.3) - Moved coding to set 
   		//   ValidationIpProblems from the xformFinal transform below to here and revised logic 
   		//   to add these lines cloned from Models.Attributes_Master, lines 1669 & 1702-1705.
   	  //    Note: have to match on seq (---v) because ds_ip_address_res does not have acctno
   		ip_address_rec  := ds_ip_address_res(seq = L.seq)[1];
      string ip_addr_trimmed := trim(ip_address_rec.ipaddr);
   		string2 ipcountry := if(ip_addr_trimmed = '', 
   		                        '', // '-1' in Models.Attributes_Master, but if set to -1 here, 
   														    // the value '2' in the map below would never come out. 
   													  trim(StringLib.StringToUpperCase(ip_address_rec.countrycode[1..2])));
      self.ValidationIpProblems := map(// if NA gateway was not requested to be used or the input
   		                                 // "Gateways" dataset was empty, blank out this field.
   																		 // Added due to 2015 enhancements, req 4.1.7 new input option.
   																		 DontIncludeGateway or GatewaysInIsEmpty => '', 
   		                                 // v--- have to check input ip_address instead of 
   																		 //   ip_address_rec.ipaddr, because if the input 
   																		 //   ip_address is non-blank but invalid, the ipaddr
   																		 //   field out of NA gateway results gets blanked out.
   		                                 clean_batch_rec.ip_address = ''         => '-1',
   	                                   ipcountry  = 'US'                       => '0',
   	                                   ipcountry <> 'US' and ipcountry <>''    => '1',
   	                                   // anything else is '2'
   																		                                            '2');
   	
       // IP Address indicator req. 4.1.47
   		ip_lat     := (REAL)ip_address_rec.latitude;
   		ip_long    := (REAL)ip_address_rec.longitude;
   		geo_dist   := IF(clean_batch_rec.ip_address <> '' AND 
   		                 IncludeGateway AND GatewaysInIsNotEmpty,
   		                 Address_Attributes.functions.GeoDist(input_lat,  // previously set above in this transform
   									                                        input_long, // previously set above in this transform
   																												  ip_lat,
   																												  ip_long),
   									   0);
    
      ipInvalid := if(// line below added due to 2015 enhancements, req 4.1.7 new input option
   		                DontIncludeGateway or GatewaysInIsEmpty or
   		                Stringlib.StringFilterOut(clean_batch_rec.ip_address[1],'0123456789')<>'' or 
   										clean_batch_rec.ip_address='' or clean_batch_rec.ip_address = '0' or
   										(ip_lat = 0 and ip_long = 0), TRUE, FALSE);	
											
							
   		self.IPAddrExceedsInputAddr := MAP(ipInvalid       => '',
     ...
     //  Save input
      self := L;
    // Null everything else not already assigned for now
      self := [];
  end;
  ...
  ds_recs_pre_res := join (ds_recs_pre_res_temp, ds_hri_res,
														left.acctno = right.acctno,
														transform(rec_out,
  ...
  rec_out xformFinal(rec_out L) := transform
      ...
      ssn_iss_rec := ds_w_ssn_issuance_res(acctno = L.acctno)[1];
		  // req 4.1.26
      self.possible_age_ssn_issuance := if(L.possible_age_dob = '' AND ssn_iss_rec.issued_start_date <> '', 
  ...
  // *--- Unless input option IncludeDebtEvasion was intentionally turned off, 
  // *--- Req 4.1.49, run data through the Liens Batch Service---* //
	// Currently not used in prod, but might be needed at a later time
	// #CONSTANT('SearchGoodSSNOnly',TRUE) //liens used
	// #CONSTANT('SearchIgnoresAddressOnly',TRUE) //liens used
	// #STORED('ScoreThreshold',10)   //liens used
  ds_final_lj := 
              if(args_in.IncludeDebtEvasion AND NOT (StringLib.StringToUpperCase(args_in.FilterRule)='V3FDN_FILTER'),
  ...
  ds_hri_cnt_join := join(ds_recs_join,dep_cnt,left.acctno = right.acctno,
   												 transform(rec_out,
  ...
  //	Filter rules changed per TRIS Rewrite (V3) Rules
      ...
      SELF.identity_risk := if((L.hri_1_code in ACCEPTABLE_HRI_CODES_ID_risk) or (L.hri_2_code in ACCEPTABLE_HRI_CODES_ID_risk),'Y','');
	  SELF.address_risk := if((L.hri_1_code in ACCEPTABLE_HRI_CODES_ADDR_risk) or (L.hri_2_code in ACCEPTABLE_HRI_CODES_ADDR_risk),'Y','');
	  SELF := L;
	  SELF := [];
  ...
  END;
	...
	ds_final := if(StringLib.StringToUpperCase(args_in.FilterRule)='V3FDN_FILTER',ds_final_DeNormedRecs,ds_final_lj);				

  // Call function to apply the main filter and fill in output field named "output_status"
	
  ds_final_mainfilt := BatchServices.TaxRefundISv3_BatchService_Functions.MainFilter(ds_final,args_in);

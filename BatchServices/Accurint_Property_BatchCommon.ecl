IMPORT AutokeyB2_batch, BatchServices, LN_PropertyV2_Services, LN_PropertyV2, BatchShare, Autokey_batch, Gateway, Suppress, ut, STD, FFD;
...
EXPORT Accurint_Property_BatchCommon(boolean isFCRA, unsigned1 nss, boolean useCannedRecs) := FUNCTION
    ...
    rec1 := LN_PropertyV2_Services.layouts.batch_in;
	  ...
	  BatchShare.MAC_AppendPicklistDID (ds_batch_in, ds_batch_did, batch_params, true);		
		...
		pc_recs := if(isFCRA, FFD.FetchPersonContext(ds_dids, gw_config, FFD.Constants.DataGroupSet.Property));
  ...
  temp_layout add_matchcodeValues(BatchServices.Layouts.LN_Property.rec_widest_plus_acctnos L,
    ...
    ds_flat_final_recs := PROJECT(UNGROUP(ds_top_property_recs), 
		...
		ds_all_sort := sort(ds_flat_final_recs,record_type,LN_PropertyV2.fn_strip_pnum(parcel_number),-sortby_date,fares_source_id,acctno);
		...
		BatchShare.MAC_RestoreAcctno(ds_batch_in, consumer_statements_prep, consumer_statements_acctno, false, false);

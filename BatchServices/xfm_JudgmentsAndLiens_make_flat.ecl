...
SHARED STRING fn_DisplayAddress1( DATASET(liensv2_services.layout_lien_party_address) address) :=
	FUNCTION
...
SHARED STRING fn_DisplayAddress2( DATASET(liensv2_services.layout_lien_party_address) address) :=
	FUNCTION
		RETURN TRIM(address[1].unit_desig) + ' ' + TRIM(address[1].sec_range);
	END;
	
SHARED STRING fn_FormatSSN(STRING9 ssn) :=
	FUNCTION
    ...
    EXPORT BatchServices.layout_JudgmentsAndLiens_Batch_out xfm_JudgmentsAndLiens_make_flat(batchservices.Layouts.Liens.layout_lien_rollup_error_code_and_matchcodes l) :=
	TRANSFORM

...
EXPORT GetIds(dataset(Layouts.rEntity) infile) := FUNCTION
	...
	dsWithoutVessel := Normalize(infile(EntryType <> 'Vessel'), 4, xformWithoutVessel(LEFT, COUNTER));
	
	return SORT(Distribute((dsWithVessel&dsWithoutVessel)(number<>''), Ent_id), Ent_id, type, number, LOCAL);
END;
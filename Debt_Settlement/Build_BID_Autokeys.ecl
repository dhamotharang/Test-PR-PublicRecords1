...
FUNCTION
	...
	llgname := Keynames(pversion).bidautokeyroot.new	;
	newBase := PROJECT(pBase, TRANSFORM(Layouts.Keybuild,
	...
	module(AutokeyB2.Fn_Build.params) 
		...
		export string														L_inlogical						:= Keynames(pversion).bidautokeyroot.new;
		export set of string1 									L_build_skip_set			:= _Constants().ak_skipSet; 
	end; 

	Build_BID_autokeys := AutokeyB2.Fn_Build.Do(
							 mod_AKB

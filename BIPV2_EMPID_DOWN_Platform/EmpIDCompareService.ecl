...
EXPORT EmpIDCompareService := MACRO
  IMPORT SALT32,BIPV2_EMPID_DOWN_Platform,ut;
...
odl := PROJECT(CHOOSEN(BIPV2_EMPID_DOWN_Platform.Keys(BFile).Candidates(EmpID=EmpIDone),100000),BIPV2_EMPID_DOWN_Platform.match_candidates(BFile).layout_candidates);
odr := PROJECT(CHOOSEN(BIPV2_EMPID_DOWN_Platform.Keys(BFile).Candidates(EmpID=EmpIDTwo),100000),BIPV2_EMPID_DOWN_Platform.match_candidates(BFile).layout_candidates);
k := BIPV2_EMPID_DOWN_Platform.Keys(BFile).Specificities_Key;
s := GLOBAL(PROJECT(k,BIPV2_EMPID_DOWN_Platform.Layout_Specificities.R)[1]);
odlv := BIPV2_EMPID_DOWN_Platform.Debug(BFile,s).RolledEntities(odl);
odrv := BIPV2_EMPID_DOWN_Platform.Debug(BFile,s).RolledEntities(odr);
mtch := BIPV2_EMPID_DOWN_Platform.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,EmpIDone,EmpIDtwo,0,0}],BIPV2_EMPID_DOWN_Platform.match_candidates(BFile).layout_matches));
// Put out easy to read versions of the EmpID data

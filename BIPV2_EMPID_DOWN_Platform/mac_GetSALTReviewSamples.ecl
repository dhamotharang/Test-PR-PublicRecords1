...
iter := '25';
#workunit('name','BIPV2_EMPID_DOWN_Platform Iter ' + iter + ' review samples');
psetReviewers           := ['CM','LB','TL','JL','FN','DW'];
pNumSamplesPerReviewer  := 15;

kmtch      := index(BIPV2_EMPID_DOWN_Platform.Keys(BIPV2_EMPID_DOWN_Platform.In_EmpID).MatchSample ,BIPV2_EMPID_DOWN_Platform.keynames('20130330' + iter).match_sample_debug.logical);

tools.mac_GetReviewSamples(kmtch,EmpID,'21',pNumSamplesPerReviewer,psetReviewers,true);
*/
import tools;
...
EXPORT mac_GetSALTReviewSamples(
  ...
  ,pInFile                            //Infile for matching(BIPV2_EMPID_DOWN_Platform.In_EmpID)
  ,pID                                //Internal linking id(EmpID, dotid, bdid, did, etc)
...
////
  #APPEND(ECL    ,'allsamplerecs     := sort(samplerecseq + samplerecsgt,EmpID1 + EmpID2);\n')
...
////
  #APPEND(ECL    ,'allsampleEmpIDs :=   table(table(samplerecseq + samplerecsgt  ,{unsigned6 ' + %'lID'% + ' := ' + %'lID'% + '1})\n')
  #APPEND(ECL    ,'                          + table(samplerecseq + samplerecsgt  ,{unsigned6 ' + %'lID'% + ' := ' + %'lID'% + '2})\n')
  #APPEND(ECL    ,'                      ,{' + %'lID'% + '},' + %'lID'% + ',few);\n')
  #APPEND(ECL    ,'setsampleEmpIDs  := set(allsampleEmpIDs  ,' + %'lID'% + ');\n')
  #APPEND(ECL    ,'allEmpIDcands    := project(kcand(' + %'lID'% + ' in setsampleEmpIDs)  ,' + %'lModule'% + '.match_candidates(' + %'lInfile'% + ').layout_candidates);\n')
  #APPEND(ECL    ,'s := dataset([],' + %'lModule'% + '.Layout_Specificities.R);\n')
  #APPEND(ECL    ,'allEmpIDsrolled := ' + %'lModule'% + '.Debug(' + %'lInfile'% + ',s).RolledEntities(allEmpIDcands);\n')
  #APPEND(ECL    ,'layrolled := recordof(allEmpIDsrolled);\n\n')
  ...
  #LOOP
    ...
    #APPEND(NormSamples ,psetReviewers[%CNTR%] + '_norm := project(normalize(' + psetReviewers[%CNTR%] + ',3,transform({integer2 conf,unsigned6 EmpID},self.EmpID := choose(counter  ,left.EmpID1,left.EmpID2,0),self.conf := left.conf)) ,transform({unsigned cnt,recordof(left)},self.cnt := counter + ((startrec' + %'CNTR'% + ' - 1) * 3),self := left));\n')
    #APPEND(RollSamples ,psetReviewers[%CNTR%] + '_rolled := sort(join(' + psetReviewers[%CNTR%] + '_norm ,allEmpIDsrolled ,left.EmpID = right.EmpID ,transform({unsigned cnt,integer2 conf,recordof(right)},self.cnt := left.cnt,self.conf := if(right.EmpID != 0  ,left.conf,0),self := right),left outer,lookup),cnt);\n')

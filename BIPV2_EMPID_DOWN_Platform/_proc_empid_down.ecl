...
BIPV2_EMPID_DOWN_Platform._proc_Empid_Down().MultIter_run (EmpStartIteration     ,EmpNumIterations      ,doEmpid_DownInit ,doEmpid_DownSpecs  ,doEmpid_DownIters  ,doEmpid_DownPost ,pversion  
...
EXPORT _proc_empid_down(
  ...
  shared _files_empid_down := BIPV2_Files.files_empid_down('BIPV2_EMPID_DOWN_Platform');
	/* ---------------------- Logical Files ------------------------------- */
  ...
  /* ---------------------- Init From POWID ---------------------------- */
	...
	/* ---------------------- SALT Specificities ------------------------- */
	// shared specMod := BIPV2_EMPID_DOWN_Platform.specificities(input);
	...
	/* ---------------------- SALT Iteration ----------------------------- */
	shared possibleMatches  := output(BIPV2_EMPID_DOWN_Platform.matches(input).PossibleMatches,,f_hist(iter),overwrite,compressed);
	shared saltMod 	        := BIPV2_EMPID_DOWN_Platform.Proc_Iterate(Build_Date,iter, input, f_out(''));
	...
	/* ---------------------- Review Samples ----------------------------- */
	export outputReviewSamples  := BIPV2_EMPID_DOWN_Platform._Samples().out;
	/* ---------------------- Take Action -------------------------------- */
	...
	export MultIter_wuName(unsigned startIter, unsigned numIters) := 
			'BIPV2_EMPID_DOWN_Platform Controller ' + Build_Date + ' ' + (string)startiter + '-' + (string)(startiter + numiters - 1);
	export MultIter_run(startIter, numIters, doInit = 'true',dospec = 'true',doIters = 'true',doPost = 'true',pversion = 'bipv2.keysuffix',p_Init_File = 'BIPV2_Files.files_powid().DS_BASE') := functionmacro
			...
			eclInit		:= 'import BIPV2_Files,BIPV2_build;\niteration := \'@iteration@\';\npversion  := \'@version@\';\n#workunit(\'name\',\'BIPV2_EMPID_DOWN_Platform \' + pversion + \' Init \');\n#OPTION(\'multiplePersistInstances\',FALSE);\n#workunit(\'priority\',\'high\');\n' + 'BIPV2_EMPID_DOWN_Platform._proc_empid_down(,,\'' + pversion + '\').init(' + #TEXT(p_Init_File)+ ');';
		  // eclSpec		:= 'import BIPV2_Files,BIPV2_build;iteration := \'@iteration@\';pversion  := \'@version@\';lih := BIPV2_Files.files_empid_down(\'BIPV2_EMPID_DOWN_Platform\').DS_BUILDING;#workunit(\'name\',\'BIPV2_EMPID_DOWN_Platform \' + pversion + \' Specificities \');#workunit(\'priority\',\'high\');BIPV2_EMPID_DOWN_Platform._proc_empid_down(lih,iteration,\'' + pversion + '\').runSpecBuild;';
			eclIter		:= 'import BIPV2_Files,BIPV2_build;\niteration := \'@iteration@\';\npversion  := \'@version@\';\nlih := BIPV2_Files.files_empid_down(\'BIPV2_EMPID_DOWN_Platform\').DS_BUILDING;\n#OPTION(\'multiplePersistInstances\',FALSE);\n#workunit(\'name\',\'BIPV2_EMPID_DOWN_Platform \' + pversion + \' iter \' + iteration);\n#workunit(\'priority\',\'high\');\n' + 'BIPV2_EMPID_DOWN_Platform._proc_empid_down(lih,iteration,\'' + pversion + '\').runIter;';
      eclPost		:= 'import BIPV2_build;\n#workunit(\'name\',\'BIPV2_EMPID_DOWN_Platform @version@ PostProcess\');\n' + 'BIPV2_EMPID_DOWN_Platform._proc_empid_down(,\'' + lastIter + '\',\'' + pversion + '\').updateSuperfiles();';
	...
	//		proc := BIPV2_EMPID_DOWN_Platform._proc_empid_down();

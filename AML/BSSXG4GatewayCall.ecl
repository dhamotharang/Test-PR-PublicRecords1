...
export BSSXG4GatewayCall (DATASET(Layouts.BridgerSearchInput) indata,
...
// 'US-OFAC - Non-SDN Iranian Sanctions Act (NS-ISA) List'
			...
			gateway_cfg  := Gateway.Configuration.get()(Gateway.Configuration.IsSearchCore(servicename))[1];
        ...
        SELF.Config.DataFiles := project(ut.ds_oneRecord, transform(iesp.gateway_bridgerxg5.t_ConfigDataFile,
				...
				self.Config.GeneralOptions.FirstNameWeight  := 'LOW';
				self.Config.GeneralOptions.LastnameWeight   := 'HIGH';
	...
	outBridger := Gateway.SoapCall_BridgerXG4(in_req, gateway_cfg);
...
entityDetailsRecord	breakoutentity(iesp.gateway_bridgerxg5.t_ResultMatch le) := TRANSFORM
	...
	self.SearchCriteria := if(trim(le.entity.EntityUniqueID) = '', '', le.Entity.SearchCoreCriteria);
...
ResponseRec	breakoutmain(iesp.gateway_bridgerxg5.t_SearchCoreResponse le) := TRANSFORM
  // self.blockid := le.searchresult.EntityRecords[1].InputRecord.id;
  self.blockid := le.blockid;
	self.errormessage := le.errormessage;
  
	self.EDRec := project(le.EntityRecords.Matches, breakoutentity(left));
end;	
...
NewsMain := project( outBridger, breakoutmain(left.response));
...
ResponseRecSlim   GetCategories(ResponseRec le) := TRANSFORM
  self.blockid := le.blockid;
	self.errormessage := le.errormessage;
...
CategoryRec := project(NewsMain, GetCategories(left));
...
// output(gateways, named('gatewaysGateCall'), overwrite);
 // output(gateway_cfg, named('gateway_cfgGateCall4'), overwrite);
 // output(outBridger, named('outBridger4'), overwrite);
 // output(NewsMain, named('NewsMain4'), overwrite);

...
layout_PropertyRecordPlus join_did(ids L, kponf R) := transform
	...
	self.applicant_sale_found := self.applicant_sold;
	self.applicant_buy_found := self.applicant_owned;
	fname_match := risk_indicators.g(risk_indicators.FnameScore(l.fname,r.fname));
	lname_match := risk_indicators.g(Risk_Indicators.LnameScore(l.lname,r.lname));
...
together := group(sort(ungroup(by_addr + by_did), seq, prim_range, prim_name, sec_range, zip5, datasrce, did),seq, did);
...
risk_indicators.Layouts.layout_relat_prop_plusv4 to_relat_prop(addDidtoAddr le) :=
TRANSFORM
...
SingleProp := SORT(Property,prim_name,prim_range,zip5,sec_range,census_loose, dataSrce); 

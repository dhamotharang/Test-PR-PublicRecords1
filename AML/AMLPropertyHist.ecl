...
proprec := record
	Risk_Indicators.Layouts.Layout_PropertyRecordv2 - risk_indicators.layout_overrides;
...
property_by_address :=  property_by_address_all;
...
property_by_did :=  property_by_did_all;
...
pre_property_fid := group(property_by_address, seq, did) + group(property_by_did, seq,did);
...
assessments_added := rollup(sort(pre_assessments_added, own_fares_id, isRelat, prim_name, prim_range, zip5, sec_range, dataSrce), roll_prop_searched(LEFT,RIGHT), seq,did,own_fares_id, prim_name, prim_range, zip5, sec_range);
...
deeds_added := rollup(sort(pre_deeds_added,seq, did, prim_name,prim_range,zip5,sec_range, dataSrce),roll_prop_searched(LEFT,RIGHT),seq, did, prim_name,prim_range,zip5,sec_range);
...
layout_out to_relat_prop(Deeds_added le) :=
TRANSFORM
myGetDate := ut.GetDate;
	...
	SELF.property_status_applicant := MAP(le.applicant_sale_found => 'S',
								   ...
								   //le.applicant_owned OR le.applicant_sold => 'A',   //AML does not want family owned
								   ' ');
	SELF.property_status_family := MAP(le.family_sale_found => 'S',
								le.family_buy_found  => 'O',
								//le.family_owned OR le.family_sold => 'A',  //AML does not want family owned
								' ');
...
Single_Property := group(sort(Property,seq),seq);

...
EXPORT FromNone(DATASET(Layout_In_NV) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.stock_nbr_par_shares_Invalid := Fields.InValid_stock_nbr_par_shares((SALT311.StrType)le.stock_nbr_par_shares);
    SELF.stock_total_capital_Invalid := Fields.InValid_stock_total_capital((SALT311.StrType)le.stock_total_capital);
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    AnyRule_WithErrorsCount := COUNT(GROUP, h.corp_key_Invalid > 0 OR h.corp_vendor_Invalid > 0 OR h.corp_state_origin_Invalid > 0 OR h.corp_process_date_Invalid > 0 OR h.corp_sos_charter_nbr_Invalid > 0 OR h.stock_authorized_nbr_Invalid > 0 OR h.stock_par_value_Invalid > 0 OR h.stock_nbr_par_shares_Invalid > 0 OR h.stock_total_capital_Invalid > 0);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.corp_key_Total_ErrorCount > 0, 1, 0) + IF(le.corp_vendor_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_state_origin_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_sos_charter_nbr_Total_ErrorCount > 0, 1, 0) + IF(le.stock_authorized_nbr_ALLOW_ErrorCount > 0, 1, 0) + IF(le.stock_par_value_ALLOW_ErrorCount > 0, 1, 0) + IF(le.stock_nbr_par_shares_ALLOW_ErrorCount > 0, 1, 0) + IF(le.stock_total_capital_ALLOW_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.corp_key_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_key_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_vendor_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_state_origin_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_sos_charter_nbr_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_sos_charter_nbr_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.stock_authorized_nbr_ALLOW_ErrorCount > 0, 1, 0) + IF(le.stock_par_value_ALLOW_ErrorCount > 0, 1, 0) + IF(le.stock_nbr_par_shares_ALLOW_ErrorCount > 0, 1, 0) + IF(le.stock_total_capital_ALLOW_ErrorCount > 0, 1, 0);
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'stock_total_capital:invalid_stock_total_capital:ALLOW'
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ...
          ,le.stock_par_value_ALLOW_ErrorCount
          ,le.stock_nbr_par_shares_ALLOW_ErrorCount
          ,le.stock_total_capital_ALLOW_ErrorCount,0) / le.TotalCnt, CHOOSE(c - NumRules
...
EXPORT StandardStats(DATASET(Layout_In_NV) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Corp2_Mapping_NV_Stock, Fields, 'RECORDOF(scrubsSummaryOverall)', '');

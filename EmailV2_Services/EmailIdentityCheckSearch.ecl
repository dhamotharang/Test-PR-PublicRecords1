...
EXPORT EmailIdentityCheckSearch(DATASET($.Layouts.batch_in_rec) batch_in,
  ...
  Suppress.Mac_mask(ds_results_with_relations, ds_results_best_ssnmasked, bestinfo.ssn, null, TRUE, FALSE, maskVal:=ssn_mask_use);
  ...
  ds_results_ready := $.Functions.SortResults(ds_results_ssndob_masked + ds_rejected_input_email, in_mod);

...
EXPORT E_Sele_Property_Event(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __d0_Event__Layout := RECORD
    ...
    KEL.typ.uid Event_;
  END;
  SHARED __d0_Event__Mapped := JOIN(KEL.Intake.AppendNonExistUidComponents(__d0_Legal__Mapped,'LNFaresID','__in'),E_Property_Event(__in,__cfg).Lookup,TRIM((STRING)LEFT.LNFaresID) = RIGHT.KeyVal,TRANSFORM(__d0_Event__Layout,SELF.Event_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,HASH);
  ...
  EXPORT Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __PostFilter := __GroupedFilter(GROUP(InData,Legal_,Event_,Ult_I_D_,Org_I_D_,Sele_I_D_,L_N_Fares_I_D_,Person_Number_,Conjunctive_Name_Sequence_,Party_Is_Buyer_Or_Owner_,Party_Is_Borrower_,Party_Is_Seller_,Party_Is_Care_Of_,Process_Date_,Date_Vendor_First_Reported_,Date_Vendor_Last_Reported_,Vendor_Source_Code_,ALL));
  ...
  EXPORT NullCounts := DATASET([
    ...
    {'SelePropertyEvent','PublicRecords_KEL.ECL_Functions.Dataset_FDC','LNFaresID',COUNT(__d0(__NL(L_N_Fares_I_D_))),COUNT(__d0(__NN(L_N_Fares_I_D_)))},
    ...
    {'SelePropertyEvent','PublicRecords_KEL.ECL_Functions.Dataset_FDC','DateVendorFirstReported',COUNT(__d0(__NL(Date_Vendor_First_Reported_))),COUNT(__d0(__NN(Date_Vendor_First_Reported_)))},
    {'SelePropertyEvent','PublicRecords_KEL.ECL_Functions.Dataset_FDC','DateVendorLastReported',COUNT(__d0(__NL(Date_Vendor_Last_Reported_))),COUNT(__d0(__NN(Date_Vendor_Last_Reported_)))},
    ...
    {'SelePropertyEvent','PublicRecords_KEL.ECL_Functions.Dataset_FDC','Source',COUNT(__d0(__NL(Source_))),COUNT(__d0(__NN(Source_)))},

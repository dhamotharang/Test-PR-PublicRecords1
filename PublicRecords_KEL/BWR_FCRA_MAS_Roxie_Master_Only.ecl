...
Include_InferredPerformance := TRUE;
...
OutputFile := '~akoenen::out::PersonFCRA_Roxie_10k_archive_after_'+ ThorLib.wuid();
...
//  STRING CustomerId; // This is used only for failed transactions here; it's ignored by the ECL service.
	...
	BOOLEAN IncludeInferredPerformance;
	BOOLEAN appendpii;
	BOOLEAN isprescreen;
...
Settings := MODULE(PublicRecords_KEL.Interface_BWR_Settings)
	...
	EXPORT BOOLEAN IncludeInferredPerformance := Include_InferredPerformance;
END;
...
// SELF.CustomerId := le.CustomerId;
    ...
    SELF.OutputMasterResults := Output_Master_Results;
		...
		self.IncludeInferredPerformance := Settings.IncludeInferredPerformance;
...
Passed_with_Extras := 
	JOIN(p, Passed, LEFT.Account = RIGHT.P_InpAcct, 
...
SALT_AttributeResults := IF(Output_SALT_Profile, SALTRoutines.SALT_Profile_Run_Everything(Passed_with_Extras, 'SALT_Results'), 0);

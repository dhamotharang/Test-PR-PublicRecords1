...
EXPORT Q_Non_F_C_R_A_Person_Attributes_V1(KEL.typ.uid __PLexID_in, DATASET(RECORDOF(PublicRecords_KEL.ECL_Functions.Layouts.LayoutInputPII)) __PInputPIIDataset, KEL.typ.kdate __PP_InpClnArchDt, UNSIGNED8 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED __EE2688503 := __ENH_Person;
  SHARED __EE2696141 := __EE2688503(__T(__OP2(__EE2688503.UID,=,__CN(__PLexID_in))));
  SHARED __ST54284_Layout := RECORD
    KEL.typ.nuid Lex_I_D_;
    KEL.typ.str P___Lex_I_D_Seen_Flag_ := '';
    KEL.typ.int P_L___Ast_Veh_Auto_Cnt_Ev_ := 0;
    KEL.typ.ndataset(B_Person(__in,__cfg_Local).__ST2209933_Layout) P_L___Ast_Veh_Auto_Emrg_Dt_List_Ev_;
    KEL.typ.ndataset(B_Person(__in,__cfg_Local).__ST2209953_Layout) P_L___Ast_Veh_Auto_Last_Dt_List_Ev_;
  ...
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(__EE2696141,TRANSFORM(__ST54284_Layout,SELF.Lex_I_D_ := LEFT.UID,SELF := LEFT)));
END;

...
EXPORT Q_Non_F_C_R_A_Person_Attributes_V1(SET OF KEL.typ.uid __PLexIDs, KEL.typ.kdate __PArchiveDate, UNSIGNED1 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED __EE408304 := __ENH_Person;
  SHARED __EE411979 := __EE408304(__T(__OP2(__EE408304.UID,IN,__CN(__PLexIDs))));
  SHARED __ST17364_Layout := RECORD
  ...
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(__EE411979,TRANSFORM(__ST17364_Layout,SELF.Lex_I_D_ := LEFT.UID,SELF := LEFT)));
END;

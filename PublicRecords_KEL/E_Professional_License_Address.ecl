...
EXPORT E_Professional_License_Address(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __Mapping := 'Prof_Lic_(Prof_Lic_:0),Location_(Location_:0),source(Source_:\'\'),datefirstseen(Date_First_Seen_:EPOCH),datelastseen(Date_Last_Seen_:EPOCH)';
  ...
  EXPORT Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __PostFilter := __GroupedFilter(GROUP(InData,Prof_Lic_,Location_,ALL));
  ...
  EXPORT SanityCheck := DATASET([{COUNT(Prof_Lic__Orphan),COUNT(Location__Orphan)}],{KEL.typ.int Prof_Lic__Orphan,KEL.typ.int Location__Orphan});

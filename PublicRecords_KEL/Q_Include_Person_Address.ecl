...
IMPORT KEL12 AS KEL;
IMPORT CFG_Compile,E_Address,E_Aircraft_Owner,E_Bankruptcy,E_Criminal_Offense,E_Geo_Link,E_Lien_Judgment,E_Person,E_Person_Address,E_Person_Bankruptcy,E_Person_Lien_Judgment,E_Person_Offenses,E_Person_Vehicle,E_Professional_License,E_Professional_License_Person,E_Watercraft_Owner,E_Zip_Code FROM PublicRecords_KEL;
IMPORT * FROM KEL12.Null;
EXPORT Q_Include_Person_Address(KEL.typ.uid __PLexID_in, KEL.typ.kdate __PArchiveDate, UNSIGNED8 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  SHARED __cfg_Local := MODULE(CFG_Compile)
    EXPORT KEL.typ.kdate CurrentDate := __PArchiveDate;
  END;
  SHARED E_Person_Filtered_1 := MODULE(E_Person(__in,__cfg_Local))
  ...
  SHARED __EE31400816 := __E_Person_Address;
  SHARED __EE31400965 := __EE31400816(__NN(__EE31400816.Subject_));
  SHARED __EE31400810 := __E_Person;
  SHARED __EE31400970 := __EE31400810(__T(__OP2(__EE31400810.UID,=,__CN(__PLexID_in))));
  __JC31400976(E_Person_Address(__in,__cfg_Local).Layout __EE31400965, E_Person(__in,__cfg_Local).Layout __EE31400970) := __EEQP(__EE31400970.UID,__EE31400965.Subject_);
  SHARED __EE31401023 := JOIN(__EE31400965,__EE31400970,__JC31400976(LEFT,RIGHT),TRANSFORM(E_Person_Address(__in,__cfg_Local).Layout,SELF:=LEFT),SMART,KEEP(1));
  EXPORT Res0 := __UNWRAP(__EE31401023);
END;

...
EXPORT B_Email_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1040047 := __E_Email;
  EXPORT __ST240275_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST240275_Layout __ND6296781__Project(E_Email(__in,__cfg).Layout __PP1039852) := TRANSFORM
    ...
    __CC34197 := 2556;
    SELF.Emails7y_ := __OP2(FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__CC13243),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP1039852.Date_Last_Seen_))),<,__CN(__CC34197));
    SELF := __PP1039852;
  END;
  EXPORT __ENH_Email_3 := PROJECT(__EE1040047,__ND6296781__Project(LEFT));
END;

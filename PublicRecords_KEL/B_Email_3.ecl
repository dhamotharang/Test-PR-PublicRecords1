...
EXPORT B_Email_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1043215 := __E_Email;
  EXPORT __ST240362_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST240362_Layout __ND6300668__Project(E_Email(__in,__cfg).Layout __PP1043020) := TRANSFORM
    ...
    __CC34216 := 2556;
    SELF.Emails7y_ := __OP2(FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__CC13243),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP1043020.Date_Last_Seen_))),<,__CN(__CC34216));
    SELF := __PP1043020;
  END;
  EXPORT __ENH_Email_3 := PROJECT(__EE1043215,__ND6300668__Project(LEFT));
END;

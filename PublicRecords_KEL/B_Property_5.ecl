...
EXPORT B_Property_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE594405 := __E_Property;
  EXPORT __ST256148_Layout := RECORD
  ...
  EXPORT __ST256130_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST256148_Layout) Automated_Valuation_Model_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST256130_Layout __ND594643__Project(E_Property(__in,__cfg).Layout __PP594216) := TRANSFORM
    __EE594286 := __PP594216.Automated_Valuation_Model_;
    __ST256148_Layout __ND594563__Project(E_Property(__in,__cfg).Automated_Valuation_Model_Layout __PP594559) := TRANSFORM
      SELF.A_V_M_Value_Date_ := KEL.era.ToDate(__PP594559.Date_First_Seen_);
      SELF := __PP594559;
    END;
    SELF.Automated_Valuation_Model_ := __PROJECT(__EE594286,__ND594563__Project(LEFT));
    SELF := __PP594216;
  END;
  EXPORT __ENH_Property_5 := PROJECT(__EE594405,__ND594643__Project(LEFT));
END;

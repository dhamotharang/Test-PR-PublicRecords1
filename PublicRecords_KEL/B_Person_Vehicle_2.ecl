...
EXPORT B_Person_Vehicle_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7201212 := __ENH_Person_Vehicle_3;
  EXPORT __ST209860_Layout := RECORD
  ...
  END;
  SHARED __ST209860_Layout __ND7201151__Project(B_Person_Vehicle_3(__in,__cfg).__ST224345_Layout __PP7200882) := TRANSFORM
    __EE7201126 := __PP7200882.Counts_Model_;
    ...
    __EE7201146 := __PP7200882.Counts_Model_;
    SELF.Date_Last_Seen_Capped_ := IF(__T(__OP2(KEL.Aggregates.MaxNN(__EE7201126,KEL.era.ToDate(__T(__EE7201126).Date_Last_Seen_)),>,__CC13109)),__ECAST(KEL.typ.nkdate,__CC13109),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MaxNN(__EE7201146,KEL.era.ToDate(__T(__EE7201146).Date_Last_Seen_))));
    __CC13423 := '-99997';
    SELF.Vehicle_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP7200882.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP7200882.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13423)));
    SELF := __PP7200882;
  END;
  EXPORT __ENH_Person_Vehicle_2 := PROJECT(__EE7201212,__ND7201151__Project(LEFT));
END;

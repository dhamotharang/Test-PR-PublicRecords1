...
EXPORT B_Address_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE453502 := __E_Address;
  EXPORT __ST262584_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST262584_Layout __ND457199__Project(E_Address(__in,__cfg).Layout __PP452788) := TRANSFORM
    __EE453291 := __PP452788.A_D_V_O_Summary_;
    __BS453649 := __T(__EE453291);
    __EE453669 := __BN(TOPN(__BS453649(__NN(__T(__EE453291).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE453291).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE453291));
    SELF.Bestchild_Advo_ := __EE453669;
    SELF := __PP452788;
  END;
  EXPORT __ENH_Address_6 := PROJECT(__EE453502,__ND457199__Project(LEFT));
END;

...
EXPORT B_Address_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  END;
  SHARED __ST257331_Layout __ND451587__Project(E_Address(__in,__cfg).Layout __PP447176) := TRANSFORM
    __EE447679 := __PP447176.A_D_V_O_Summary_;
    __BS448037 := __T(__EE447679);
    __EE448057 := __BN(TOPN(__BS448037(__NN(__T(__EE447679).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE447679).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE447679));
    SELF.Bestchild_Advo_ := __EE448057;
    SELF := __PP447176;
  END;
  EXPORT __ENH_Address_6 := PROJECT(__EE447890,__ND451587__Project(LEFT));
END;

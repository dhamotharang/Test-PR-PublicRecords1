...
EXPORT B_Address_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE382352 := __E_Address;
  EXPORT __ST243738_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST243738_Layout __ND386049__Project(E_Address(__in,__cfg).Layout __PP381638) := TRANSFORM
    __EE382141 := __PP381638.A_D_V_O_Summary_;
    __BS382499 := __T(__EE382141);
    __EE382519 := __BN(TOPN(__BS382499(__NN(__T(__EE382141).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE382141).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE382141));
    SELF.Bestchild_Advo_ := __EE382519;
    SELF := __PP381638;
  END;
  EXPORT __ENH_Address_6 := PROJECT(__EE382352,__ND386049__Project(LEFT));
END;

...
EXPORT B_Person_Vehicle_3(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6525489 := __E_Person_Vehicle;
  EXPORT __ST231479_Layout := RECORD
  ...
  END;
  SHARED __ST231479_Layout __ND6525441__Project(E_Person_Vehicle(__cfg).Layout __PP1217354) := TRANSFORM
    __EE6525416 := __PP1217354.Counts_Model_;
    ...
    __EE6525436 := __PP1217354.Counts_Model_;
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.Aggregates.MinNN(__EE6525416,KEL.era.ToDate(__T(__EE6525416).Date_First_Seen_)),>,__CC14514)),__ECAST(KEL.typ.nkdate,__CC14514),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MinNN(__EE6525436,KEL.era.ToDate(__T(__EE6525436).Date_First_Seen_))));
    SELF := __PP1217354;
  END;
  EXPORT __ENH_Person_Vehicle_3 := PROJECT(__EE6525489,__ND6525441__Project(LEFT));
END;

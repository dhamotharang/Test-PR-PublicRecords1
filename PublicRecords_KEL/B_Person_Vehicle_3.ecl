...
EXPORT B_Person_Vehicle_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6054832 := __E_Person_Vehicle;
  EXPORT __ST224325_Layout := RECORD
  ...
  END;
  SHARED __ST224325_Layout __ND6054784__Project(E_Person_Vehicle(__in,__cfg).Layout __PP1129321) := TRANSFORM
    __EE6054759 := __PP1129321.Counts_Model_;
    ...
    __EE6054779 := __PP1129321.Counts_Model_;
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.Aggregates.MinNN(__EE6054759,KEL.era.ToDate(__T(__EE6054759).Date_First_Seen_)),>,__CC13114)),__ECAST(KEL.typ.nkdate,__CC13114),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MinNN(__EE6054779,KEL.era.ToDate(__T(__EE6054779).Date_First_Seen_))));
    SELF := __PP1129321;
  END;
  EXPORT __ENH_Person_Vehicle_3 := PROJECT(__EE6054832,__ND6054784__Project(LEFT));
END;

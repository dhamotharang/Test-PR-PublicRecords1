...
EXPORT B_Person_Vehicle_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6805106 := __E_Person_Vehicle;
  EXPORT __ST238703_Layout := RECORD
  ...
  END;
  SHARED __ST238703_Layout __ND6805058__Project(E_Person_Vehicle(__in,__cfg).Layout __PP1265577) := TRANSFORM
    __EE6805033 := __PP1265577.Counts_Model_;
    ...
    __EE6805053 := __PP1265577.Counts_Model_;
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.Aggregates.MinNN(__EE6805033,KEL.era.ToDate(__T(__EE6805033).Date_First_Seen_)),>,__CC13339)),__ECAST(KEL.typ.nkdate,__CC13339),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MinNN(__EE6805053,KEL.era.ToDate(__T(__EE6805053).Date_First_Seen_))));
    SELF := __PP1265577;
  END;
  EXPORT __ENH_Person_Vehicle_3 := PROJECT(__EE6805106,__ND6805058__Project(LEFT));
END;

...
EXPORT B_Person_Vehicle_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6988629 := __E_Person_Vehicle;
  EXPORT __ST245703_Layout := RECORD
  ...
  END;
  SHARED __ST245703_Layout __ND6988581__Project(E_Person_Vehicle(__in,__cfg).Layout __PP1280732) := TRANSFORM
    __EE6988556 := __PP1280732.Counts_Model_;
    ...
    __EE6988576 := __PP1280732.Counts_Model_;
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.Aggregates.MinNN(__EE6988556,KEL.era.ToDate(__T(__EE6988556).Date_First_Seen_)),>,__CC13360)),__ECAST(KEL.typ.nkdate,__CC13360),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MinNN(__EE6988576,KEL.era.ToDate(__T(__EE6988576).Date_First_Seen_))));
    SELF := __PP1280732;
  END;
  EXPORT __ENH_Person_Vehicle_3 := PROJECT(__EE6988629,__ND6988581__Project(LEFT));
END;

...
EXPORT B_Person_Vehicle_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6825473 := __E_Person_Vehicle;
  EXPORT __ST239005_Layout := RECORD
  ...
  END;
  SHARED __ST239005_Layout __ND6825425__Project(E_Person_Vehicle(__in,__cfg).Layout __PP1265916) := TRANSFORM
    __EE6825400 := __PP1265916.Counts_Model_;
    ...
    __EE6825420 := __PP1265916.Counts_Model_;
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.Aggregates.MinNN(__EE6825400,KEL.era.ToDate(__T(__EE6825400).Date_First_Seen_)),>,__CC13339)),__ECAST(KEL.typ.nkdate,__CC13339),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MinNN(__EE6825420,KEL.era.ToDate(__T(__EE6825420).Date_First_Seen_))));
    SELF := __PP1265916;
  END;
  EXPORT __ENH_Person_Vehicle_3 := PROJECT(__EE6825473,__ND6825425__Project(LEFT));
END;

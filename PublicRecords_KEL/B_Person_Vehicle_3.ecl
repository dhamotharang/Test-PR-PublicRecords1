...
EXPORT B_Person_Vehicle_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6745289 := __E_Person_Vehicle;
  EXPORT __ST235909_Layout := RECORD
  ...
  END;
  SHARED __ST235909_Layout __ND6745241__Project(E_Person_Vehicle(__in,__cfg).Layout __PP1257290) := TRANSFORM
    __EE6745216 := __PP1257290.Counts_Model_;
    ...
    __EE6745236 := __PP1257290.Counts_Model_;
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.Aggregates.MinNN(__EE6745216,KEL.era.ToDate(__T(__EE6745216).Date_First_Seen_)),>,__CC13339)),__ECAST(KEL.typ.nkdate,__CC13339),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MinNN(__EE6745236,KEL.era.ToDate(__T(__EE6745236).Date_First_Seen_))));
    SELF := __PP1257290;
  END;
  EXPORT __ENH_Person_Vehicle_3 := PROJECT(__EE6745289,__ND6745241__Project(LEFT));
END;

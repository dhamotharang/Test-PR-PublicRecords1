...
EXPORT B_Person_Vehicle_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6956822 := __E_Person_Vehicle;
  EXPORT __ST245682_Layout := RECORD
  ...
  END;
  SHARED __ST245682_Layout __ND6956774__Project(E_Person_Vehicle(__in,__cfg).Layout __PP1280711) := TRANSFORM
    __EE6956749 := __PP1280711.Counts_Model_;
    ...
    __EE6956769 := __PP1280711.Counts_Model_;
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.Aggregates.MinNN(__EE6956749,KEL.era.ToDate(__T(__EE6956749).Date_First_Seen_)),>,__CC13339)),__ECAST(KEL.typ.nkdate,__CC13339),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MinNN(__EE6956769,KEL.era.ToDate(__T(__EE6956769).Date_First_Seen_))));
    SELF := __PP1280711;
  END;
  EXPORT __ENH_Person_Vehicle_3 := PROJECT(__EE6956822,__ND6956774__Project(LEFT));
END;

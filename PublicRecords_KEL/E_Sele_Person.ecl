...
EXPORT E_Sele_Person(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __Mapping := 'Legal_(DEFAULT:Legal_:0),contact(DEFAULT:Contact_:0),datefirstseencontact(DEFAULT:Date_First_Seen_Contact_:DATE),datelastseencontact(DEFAULT:Date_Last_Seen_Contact_:DATE),contacttype(DEFAULT:Contact_Type_:\'\'),jobtitle(DEFAULT:Job_Title_:\'\'),contactstatus(DEFAULT:Contact_Status_:\'\'),source(DEFAULT:Source_:\'\'),datefirstseen(DEFAULT:Date_First_Seen_:EPOCH),datelastseen(DEFAULT:Date_Last_Seen_:EPOCH)';
  SHARED __Mapping0 := 'Legal_(DEFAULT:Legal_:0),contact_did(OVERRIDE:Contact_:0),dt_first_seen_contact(OVERRIDE:Date_First_Seen_Contact_:DATE),dt_last_seen_contact(OVERRIDE:Date_Last_Seen_Contact_:DATE),contact_type_derived(OVERRIDE:Contact_Type_:\'\'),contact_job_title_derived(OVERRIDE:Job_Title_:\'\'),contact_status_derived(OVERRIDE:Contact_Status_:\'\'),source(OVERRIDE:Source_:\'\'),datefirstseen(DEFAULT:Date_First_Seen_:EPOCH),datelastseen(DEFAULT:Date_Last_Seen_:EPOCH),DPMBitmap(DEFAULT:__Permits:PERMITS)';
  ...
  SHARED __d0_Legal__Layout := RECORD
    ...
    KEL.typ.uid Legal_;
  END;
  SHARED __d0_Legal__Mapped := JOIN(KEL.Intake.AppendNonExistUidComponents(__d0_Norm,'ultid,orgid,seleid','__in'),E_Business_Sele(__in,__cfg).Lookup,TRIM((STRING)LEFT.ultid) + '|' + TRIM((STRING)LEFT.orgid) + '|' + TRIM((STRING)LEFT.seleid) = RIGHT.KeyVal,TRANSFORM(__d0_Legal__Layout,SELF.Legal_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,HASH);

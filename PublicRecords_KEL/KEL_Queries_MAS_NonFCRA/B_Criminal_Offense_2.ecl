...
IMPORT KEL15 AS KEL;
IMPORT B_Criminal_Offense_3,CFG_Compile,E_Criminal_Offense FROM PublicRecords_KEL.KEL_Queries_MAS_NonFCRA;
...
EXPORT B_Criminal_Offense_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5501724 := __ENH_Criminal_Offense_3;
  EXPORT __ST180599_Layout := RECORD
  ...
  END;
  EXPORT __ST180552_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST180599_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST180552_Layout __ND5502181__Project(B_Criminal_Offense_3(__in,__cfg).__ST190156_Layout __PP5501725) := TRANSFORM
    __EE5501885 := __PP5501725.Offense_Charges_;
    __ST180599_Layout __ND5502146__Project(B_Criminal_Offense_3(__in,__cfg).__ST190203_Layout __PP5501886) := TRANSFORM
      SELF.Is_Arrest_ := __OP2(__PP5501886.Data_Type_,=,__CN('5'));
      SELF.Is_Non_Felony_Records_F_C_R_A_ := __AND(__AND(__AND(__NOT(__PP5501886.Dismissed_Charges_),__OP2(__PP5501886.Offense_Score_,=,__CN('M'))),__OP2(__PP5501886.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP5501886.Traffic_Flag_,=,__CN('N')));
      SELF.Is_Non_Felony_Records_Non_F_C_R_A_ := __AND(__NOT(__PP5501886.Dismissed_Charges_),__NOT(__OP2(__PP5501886.Offense_Score_,=,__CN('F'))));
      SELF := __PP5501886;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE5501885,__ND5502146__Project(LEFT));
    SELF := __PP5501725;
  END;
  EXPORT __ENH_Criminal_Offense_2 := PROJECT(__EE5501724,__ND5502181__Project(LEFT));
END;

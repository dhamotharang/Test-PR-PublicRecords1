...
IMPORT KEL16 AS KEL;
IMPORT B_Input_P_I_I_7,B_Input_P_I_I_8,CFG_Compile,E_Address,E_Address_Slim,E_Address_Summary,E_Email,E_Geo_Link,E_Name_Summary,E_Person,E_Phone,E_Phone_Summary,E_Property,E_S_S_N_Summary,E_Social_Security_Number,E_Surname,E_Zip_Code,FN_Compile FROM PublicRecords_KEL.KEL_Queries_MAS_NonFCRA;
...
EXPORT B_Input_P_I_I_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1590558 := __ENH_Input_P_I_I_7;
  EXPORT __ST186685_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST186685_Layout __ND1591126__Project(B_Input_P_I_I_8(__in,__cfg).__ST182009_Layout __PP1590559) := TRANSFORM
    __CC13537 := '-99999';
    SELF.Input_First_Name_Value_ := FN_Compile(__cfg).FN_Is_Blank(__ECAST(KEL.typ.nstr,__PP1590559.P___Inp_Name_First_),__ECAST(KEL.typ.nstr,__CN(__CC13537)));
    SELF.Input_Last_Name_Value_ := FN_Compile(__cfg).FN_Is_Blank(__ECAST(KEL.typ.nstr,__PP1590559.P___Inp_Name_Last_),__ECAST(KEL.typ.nstr,__CN(__CC13537)));
    SELF.Input_Middle_Name_Value_ := FN_Compile(__cfg).FN_Is_Blank(__ECAST(KEL.typ.nstr,__PP1590559.P___Inp_Name_Mid_),__ECAST(KEL.typ.nstr,__CN(__CC13537)));
    SELF.P___Inp_Addr_ := __OP2(IF(__T(__NT(__PP1590559.P___Inp_Addr_Line1_)),__ECAST(KEL.typ.nstr,__CN('')),__ECAST(KEL.typ.nstr,__OP2(__PP1590559.P___Inp_Addr_Line1_,+,__CN(' ')))),+,IF(__T(__NT(__PP1590559.P___Inp_Addr_Line2_)),__ECAST(KEL.typ.nstr,__CN('')),__ECAST(KEL.typ.nstr,__PP1590559.P___Inp_Addr_Line2_)));
    SELF := __PP1590559;
  END;
  EXPORT __ENH_Input_P_I_I_6 := PROJECT(__EE1590558,__ND1591126__Project(LEFT));
END;

...
EXPORT B_Person_Vehicle_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4684698 := __E_Person_Vehicle;
  ...
  END;
  SHARED __ST186321_Layout __ND4684650__Project(E_Person_Vehicle(__in,__cfg).Layout __PP694663) := TRANSFORM
    __EE4684625 := __PP694663.Counts_Model_;
    ...
    __EE4684645 := __PP694663.Counts_Model_;
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.Aggregates.MinNN(__EE4684625,KEL.era.ToDate(__T(__EE4684625).Date_First_Seen_)),>,__CC13362)),__ECAST(KEL.typ.nkdate,__CC13362),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MinNN(__EE4684645,KEL.era.ToDate(__T(__EE4684645).Date_First_Seen_))));
    SELF := __PP694663;
  END;
  EXPORT __ENH_Person_Vehicle_3 := PROJECT(__EE4684698,__ND4684650__Project(LEFT));
END;

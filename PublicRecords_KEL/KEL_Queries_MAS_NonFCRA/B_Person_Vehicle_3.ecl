...
EXPORT B_Person_Vehicle_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5493087 := __E_Person_Vehicle;
  EXPORT __ST195765_Layout := RECORD
  ...
  END;
  SHARED __ST195765_Layout __ND5493039__Project(E_Person_Vehicle(__in,__cfg).Layout __PP784223) := TRANSFORM
    __EE5493014 := __PP784223.Counts_Model_;
    ...
    __EE5493034 := __PP784223.Counts_Model_;
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.Aggregates.MinNN(__EE5493014,KEL.era.ToDate(__T(__EE5493014).Date_First_Seen_)),>,__CC13435)),__ECAST(KEL.typ.nkdate,__CC13435),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MinNN(__EE5493034,KEL.era.ToDate(__T(__EE5493034).Date_First_Seen_))));
    SELF := __PP784223;
  END;
  EXPORT __ENH_Person_Vehicle_3 := PROJECT(__EE5493087,__ND5493039__Project(LEFT));
END;

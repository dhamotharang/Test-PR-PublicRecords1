...
EXPORT B_Person_Vehicle_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2061197 := __E_Person_Vehicle;
  EXPORT __ST174196_Layout := RECORD
  ...
  END;
  SHARED __ST174196_Layout __ND2061149__Project(E_Person_Vehicle(__in,__cfg).Layout __PP496690) := TRANSFORM
    __EE2061124 := __PP496690.Counts_Model_;
    ...
    __EE2061144 := __PP496690.Counts_Model_;
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.Aggregates.MinNN(__EE2061124,KEL.era.ToDate(__T(__EE2061124).Date_First_Seen_)),>,__CC13312)),__ECAST(KEL.typ.nkdate,__CC13312),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MinNN(__EE2061144,KEL.era.ToDate(__T(__EE2061144).Date_First_Seen_))));
    SELF := __PP496690;
  END;
  EXPORT __ENH_Person_Vehicle_3 := PROJECT(__EE2061197,__ND2061149__Project(LEFT));
END;

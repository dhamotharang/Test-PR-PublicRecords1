...
IMPORT KEL15 AS KEL;
IMPORT B_Input_P_I_I_9,B_Person_9,CFG_Compile,E_Address,E_Address_Slim,E_Address_Summary,E_Email,E_Geo_Link,E_Input_P_I_I,E_Name_Summary,E_Person,E_Phone,E_Phone_Summary,E_Property,E_S_S_N_Summary,E_Social_Security_Number,E_Surname,E_Zip_Code,FN_Compile FROM PublicRecords_KEL.KEL_Queries_MAS_NonFCRA;
...
EXPORT B_Person_8(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE3950694 := __ENH_Person_9;
  SHARED __EE3950696 := __ENH_Input_P_I_I_9;
  SHARED __ST273275_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(B_Person_9(__in,__cfg).__ST265026_Layout) All_Lien_Data_;
  ...
  END;
  __JC3950705(B_Person_9(__in,__cfg).__ST200291_Layout __EE3950694, B_Input_P_I_I_9(__in,__cfg).__ST199833_Layout __EE3950696) := __EEQP(__EE3950694.P_I_I_,__EE3950696.UID);
  __ST273275_Layout __JT3950705(B_Person_9(__in,__cfg).__ST200291_Layout __l, B_Input_P_I_I_9(__in,__cfg).__ST199833_Layout __r) := TRANSFORM
    SELF.U_I_D__1_ := __r.UID;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE3950936 := JOIN(__EE3950694,__EE3950696,__JC3950705(LEFT,RIGHT),__JT3950705(LEFT,RIGHT),LEFT OUTER,SMART);
  EXPORT __ST201696_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST272085_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST201664_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nstr Gender_;
    KEL.typ.nstr Lex_I_D_Segment_;
    KEL.typ.nstr Lex_I_D_Segment2_;
    ...
    KEL.typ.nstr Race_;
    KEL.typ.nstr Race_Description_;
    KEL.typ.ndataset(__ST201696_Layout) Data_Sources_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Dunn_Data_Layout) Dunn_Data_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Conceal_Carry_Layout) Conceal_Carry_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Hunt_Fish_Layout) Hunt_Fish_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Consumer_Statement_Flags_Layout) Consumer_Statement_Flags_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Thrive_Layout) Thrive_;
    KEL.typ.ndataset(__ST272085_Layout) All_Lien_Data_;
    KEL.typ.ntyp(E_Input_P_I_I().Typ) P_I_I_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST201664_Layout __ND3950586__Project(__ST273275_Layout __PP3949094) := TRANSFORM
    __EE3950939 := __PP3949094.Data_Sources_;
    __ST201696_Layout __ND3949435__Project(E_Person(__in,__cfg).Data_Sources_Layout __PP3949431) := TRANSFORM
      SELF.Translated_Source_Code_ := FN_Compile(__cfg).FN_Consumer_Source_Group(__ECAST(KEL.typ.nstr,__PP3949431.Source_));
      SELF := __PP3949431;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE3950939,__ND3949435__Project(LEFT));
    __EE3950584 := __PP3949094.All_Lien_Data_;
    __ST272085_Layout __ND3950471__Project(B_Person_9(__in,__cfg).__ST265026_Layout __PP3949466) := TRANSFORM
      ...
      SELF.Is_Landlord_Tenant_Dispute_ := __AND(__OR(__CN(__PP3949466.Landlord_Tenant_Dispute_Flag_ = TRUE),__OP2(__PP3949466.Filing_Type_Description_,IN,__CN(__CC32744))),__NOT(__OP2(__PP3949466.Filing_Type_Description_,IN,__CN(__CC32757))));
      SELF.Is_Over_All_Lien_Judgment_ := __OR(__PP3949466.Is_Over_All_Judgment_,__PP3949466.Is_Over_All_Lien_);
      __CC32736 := 365;
      SELF.Seen___In___One___Year_ := __OP2(__PP3949466.Age_In_Days_,<=,__CN(__CC32736));
      SELF := __PP3949466;
    END;
    SELF.All_Lien_Data_ := __PROJECT(__EE3950584,__ND3950471__Project(LEFT));
    SELF.P___Inp_Cln_Name_First_Raw_ := __PP3949094.P___Inp_Cln_Name_First_;
    SELF.P___Inp_Cln_Name_Last_Raw_ := __PP3949094.P___Inp_Cln_Name_Last_;
    SELF.P___Inp_Cln_S_S_N_ := __PP3949094.Input_S_S_N_Clean_Value_;
    __BS3950596 := __T(__PP3949094.Data_Sources_);
    SELF.P___Lex_I_D_Seen_Flag_ := IF(EXISTS(__BS3950596(__T(__T(__PP3949094.Data_Sources_).Header_Hit_Flag_))),'1','0');
    SELF := __PP3949094;
  END;
  EXPORT __ENH_Person_8 := PROJECT(__EE3950936,__ND3950586__Project(LEFT));
END;

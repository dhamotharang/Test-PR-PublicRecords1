...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4160655 := __ENH_Criminal_Offense_4;
  EXPORT __ST181566_Layout := RECORD
  ...
  END;
  EXPORT __ST181519_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST181566_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST181519_Layout __ND4161119__Project(B_Criminal_Offense_4(__in,__cfg).__ST187760_Layout __PP4160656) := TRANSFORM
    __EE4160816 := __PP4160656.Offense_Charges_;
    __ST181566_Layout __ND4161091__Project(B_Criminal_Offense_4(__in,__cfg).__ST187807_Layout __PP4160817) := TRANSFORM
      SELF.Is_Felony_F_C_R_A_ := __AND(__AND(__AND(__AND(__NOT(__PP4160817.Dismissed_Charges_),IF(__T(__NT(__PP4160817.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP4160817.Offense_Score_,=,__CN('F'))))),__NT(__PP4160817.Offender_Level_)),__OP2(__PP4160817.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP4160817.Traffic_Flag_,=,__CN('N')));
      SELF.Is_Felony_Non_F_C_R_A_ := __AND(__AND(__NOT(__PP4160817.Dismissed_Charges_),IF(__T(__NT(__PP4160817.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP4160817.Offense_Score_,=,__CN('F'))))),__OP2(__PP4160817.Offender_Level_,=,__CN('4')));
      SELF := __PP4160817;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE4160816,__ND4161091__Project(LEFT));
    SELF := __PP4160656;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE4160655,__ND4161119__Project(LEFT));
END;

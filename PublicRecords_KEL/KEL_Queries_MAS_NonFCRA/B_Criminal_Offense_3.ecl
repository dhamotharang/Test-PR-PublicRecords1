...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4932731 := __ENH_Criminal_Offense_4;
  EXPORT __ST190203_Layout := RECORD
  ...
  END;
  EXPORT __ST190156_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST190203_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST190156_Layout __ND4933195__Project(B_Criminal_Offense_4(__in,__cfg).__ST197204_Layout __PP4932732) := TRANSFORM
    __EE4932892 := __PP4932732.Offense_Charges_;
    __ST190203_Layout __ND4933167__Project(B_Criminal_Offense_4(__in,__cfg).__ST197251_Layout __PP4932893) := TRANSFORM
      SELF.Is_Felony_F_C_R_A_ := __AND(__AND(__AND(__AND(__NOT(__PP4932893.Dismissed_Charges_),IF(__T(__NT(__PP4932893.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP4932893.Offense_Score_,=,__CN('F'))))),__NT(__PP4932893.Offender_Level_)),__OP2(__PP4932893.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP4932893.Traffic_Flag_,=,__CN('N')));
      SELF.Is_Felony_Non_F_C_R_A_ := __AND(__AND(__NOT(__PP4932893.Dismissed_Charges_),IF(__T(__NT(__PP4932893.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP4932893.Offense_Score_,=,__CN('F'))))),__OP2(__PP4932893.Offender_Level_,=,__CN('4')));
      SELF := __PP4932893;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE4932892,__ND4933167__Project(LEFT));
    SELF := __PP4932732;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE4932731,__ND4933195__Project(LEFT));
END;

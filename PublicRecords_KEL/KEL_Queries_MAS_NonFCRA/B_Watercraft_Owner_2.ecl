...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2549905 := __ENH_Watercraft_Owner_3;
  EXPORT __ST163298_Layout := RECORD
  ...
  END;
  SHARED __ST163298_Layout __ND2549950__Project(B_Watercraft_Owner_3(__in,__cfg).__ST167995_Layout __PP2549906) := TRANSFORM
    __CC14087 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP2549906.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP2549906.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14087)));
    SELF := __PP2549906;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE2549905,__ND2549950__Project(LEFT));
END;

...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2571498 := __ENH_Watercraft_Owner_3;
  ...
  END;
  SHARED __ST170174_Layout __ND2571539__Project(B_Watercraft_Owner_3(__in,__cfg).__ST174531_Layout __PP2571499) := TRANSFORM
    __CC14102 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP2571499.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP2571499.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14102)));
    SELF := __PP2571499;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE2571498,__ND2571539__Project(LEFT));
END;

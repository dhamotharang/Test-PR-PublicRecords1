...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2577288 := __ENH_Watercraft_Owner_3;
  EXPORT __ST173777_Layout := RECORD
  ...
  END;
  SHARED __ST173777_Layout __ND2577329__Project(B_Watercraft_Owner_3(__in,__cfg).__ST178134_Layout __PP2577289) := TRANSFORM
    __CC14111 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP2577289.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP2577289.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14111)));
    SELF := __PP2577289;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE2577288,__ND2577329__Project(LEFT));
END;

...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2556664 := __ENH_Watercraft_Owner_3;
  EXPORT __ST167876_Layout := RECORD
  ...
  END;
  SHARED __ST167876_Layout __ND2556705__Project(B_Watercraft_Owner_3(__in,__cfg).__ST172223_Layout __PP2556665) := TRANSFORM
    __CC13913 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP2556665.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP2556665.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13913)));
    SELF := __PP2556665;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE2556664,__ND2556705__Project(LEFT));
END;

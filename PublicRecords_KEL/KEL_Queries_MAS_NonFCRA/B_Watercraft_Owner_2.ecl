...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2557482 := __ENH_Watercraft_Owner_3;
  EXPORT __ST168694_Layout := RECORD
  ...
  END;
  SHARED __ST168694_Layout __ND2557523__Project(B_Watercraft_Owner_3(__in,__cfg).__ST173041_Layout __PP2557483) := TRANSFORM
    __CC13964 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP2557483.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP2557483.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13964)));
    SELF := __PP2557483;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE2557482,__ND2557523__Project(LEFT));
END;

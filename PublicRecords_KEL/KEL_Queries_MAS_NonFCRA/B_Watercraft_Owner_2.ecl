...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2617950 := __ENH_Watercraft_Owner_3;
  EXPORT __ST170420_Layout := RECORD
  ...
  END;
  SHARED __ST170420_Layout __ND2617995__Project(B_Watercraft_Owner_3(__in,__cfg).__ST175128_Layout __PP2617951) := TRANSFORM
    __CC13964 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP2617951.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP2617951.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13964)));
    SELF := __PP2617951;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE2617950,__ND2617995__Project(LEFT));
END;

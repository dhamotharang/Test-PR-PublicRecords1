...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2570824 := __ENH_Watercraft_Owner_3;
  EXPORT __ST169500_Layout := RECORD
  ...
  END;
  SHARED __ST169500_Layout __ND2570865__Project(B_Watercraft_Owner_3(__in,__cfg).__ST173857_Layout __PP2570825) := TRANSFORM
    __CC13886 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP2570825.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP2570825.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13886)));
    SELF := __PP2570825;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE2570824,__ND2570865__Project(LEFT));
END;

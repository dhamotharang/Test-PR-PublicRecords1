...
EXPORT B_Bankruptcy_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4703850 := __ENH_Bankruptcy_8;
  EXPORT __ST313811_Layout := RECORD
  ...
  EXPORT __ST208135_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST313811_Layout) Best_Child_Record_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST208135_Layout __ND4703807__Project(B_Bankruptcy_8(__in,__cfg).__ST210045_Layout __PP4703472) := TRANSFORM
    __EE4703805 := __PP4703472.Best_Child_Record_;
    __ST313811_Layout __ND4703740__Project(B_Bankruptcy_9(__in,__cfg).__ST211660_Layout __PP4703530) := TRANSFORM
      ...
      SELF.Filing_Age_In_Days_ := FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP4703530.Date_Filed_),__ECAST(KEL.typ.nkdate,__CC13283));
      __BS4703597 := __T(__PP4703472.Case_Details_);
      SELF.Is_Bankruptcy_ := __AND(__PP4703472.Has_Case_Number_,__CN(EXISTS(__BS4703597(__T(__OP2(__CAST(KEL.typ.str,__T(__PP4703472.Case_Details_).Case_I_D_),<>,__CN('')))))));
      SELF := __PP4703530;
    END;
    SELF.Best_Child_Record_ := __FILTER(__PROJECT(__EE4703805,__ND4703740__Project(LEFT)),__NN(Source_Description_) OR __NN(Original_Chapter_) OR __NN(Filing_Type_) OR __NN(Business_Flag_) OR __NN(Corporate_Flag_) OR __NN(Discharged_Date_) OR __NN(Disposition_) OR __NN(Debtor_Type_) OR __NN(Debtor_Sequence_) OR __NN(Disposition_Type_) OR __NN(Disposition_Reason_) OR __NN(Disposition_Type_Description_) OR __NN(Name_Type_) OR __NN(Screen_Description_) OR __NN(Decoded_Description_) OR __NN(Date_Filed_) OR __NN(Record_Type_) OR __NN(Last_Status_Update_) OR __NN(Filing_Age_In_Days_) OR __NN(Is_Bankruptcy_));
    SELF := __PP4703472;
  END;
  EXPORT __ENH_Bankruptcy_7 := PROJECT(__EE4703850,__ND4703807__Project(LEFT));
END;

...
EXPORT B_Bankruptcy_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1596415 := __ENH_Bankruptcy_7;
  EXPORT __ST243328_Layout := RECORD
  ...
  END;
  EXPORT __ST178153_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST243328_Layout) Best_Child_Record_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST178153_Layout __ND1596372__Project(B_Bankruptcy_7(__in,__cfg).__ST179856_Layout __PP1596037) := TRANSFORM
    __EE1596370 := __PP1596037.Best_Child_Record_;
    __ST243328_Layout __ND1596305__Project(B_Bankruptcy_8(__in,__cfg).__ST181032_Layout __PP1596095) := TRANSFORM
      ...
      SELF.Filing_Age_In_Days_ := FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP1596095.Date_Filed_),__ECAST(KEL.typ.nkdate,__CC13082));
      __BS1596162 := __T(__PP1596037.Case_Details_);
      SELF.Is_Bankruptcy_ := __AND(__PP1596037.Has_Case_Number_,__CN(EXISTS(__BS1596162(__T(__OP2(__CAST(KEL.typ.str,__T(__PP1596037.Case_Details_).Case_I_D_),<>,__CN('')))))));
      SELF := __PP1596095;
    END;
    SELF.Best_Child_Record_ := __FILTER(__PROJECT(__EE1596370,__ND1596305__Project(LEFT)),__NN(Source_Description_) OR __NN(Original_Chapter_) OR __NN(Filing_Type_) OR __NN(Business_Flag_) OR __NN(Corporate_Flag_) OR __NN(Discharged_Date_) OR __NN(Disposition_) OR __NN(Debtor_Type_) OR __NN(Debtor_Sequence_) OR __NN(Disposition_Type_) OR __NN(Disposition_Reason_) OR __NN(Disposition_Type_Description_) OR __NN(Name_Type_) OR __NN(Screen_Description_) OR __NN(Decoded_Description_) OR __NN(Date_Filed_) OR __NN(Record_Type_) OR __NN(Last_Status_Update_) OR __NN(Filing_Age_In_Days_) OR __NN(Is_Bankruptcy_));
    SELF := __PP1596037;
  END;
  EXPORT __ENH_Bankruptcy_6 := PROJECT(__EE1596415,__ND1596372__Project(LEFT));
END;

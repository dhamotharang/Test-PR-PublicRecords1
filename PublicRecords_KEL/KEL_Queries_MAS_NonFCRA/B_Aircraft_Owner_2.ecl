...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2003716 := __ENH_Aircraft_Owner_3;
  EXPORT __ST159195_Layout := RECORD
  ...
  END;
  SHARED __ST159195_Layout __ND2003776__Project(B_Aircraft_Owner_3(__in,__cfg).__ST164679_Layout __PP2003717) := TRANSFORM
    __CC13943 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP2003717.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP2003717.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13943)));
    SELF := __PP2003717;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE2003716,__ND2003776__Project(LEFT));
END;

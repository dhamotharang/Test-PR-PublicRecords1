...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1919591 := __ENH_Aircraft_Owner_3;
  EXPORT __ST165255_Layout := RECORD
  ...
  END;
  SHARED __ST165255_Layout __ND1919647__Project(B_Aircraft_Owner_3(__in,__cfg).__ST170531_Layout __PP1919592) := TRANSFORM
    __CC14087 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1919592.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1919592.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14087)));
    SELF := __PP1919592;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE1919591,__ND1919647__Project(LEFT));
END;

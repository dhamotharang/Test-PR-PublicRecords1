...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2004541 := __ENH_Aircraft_Owner_3;
  EXPORT __ST158180_Layout := RECORD
  ...
  END;
  SHARED __ST158180_Layout __ND2004601__Project(B_Aircraft_Owner_3(__in,__cfg).__ST163678_Layout __PP2004542) := TRANSFORM
    __CC14072 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP2004542.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP2004542.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14072)));
    SELF := __PP2004542;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE2004541,__ND2004601__Project(LEFT));
END;

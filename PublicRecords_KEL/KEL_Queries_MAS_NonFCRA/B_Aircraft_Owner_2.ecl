...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2069008 := __ENH_Aircraft_Owner_3;
  EXPORT __ST165266_Layout := RECORD
  ...
  END;
  SHARED __ST165266_Layout __ND2069068__Project(B_Aircraft_Owner_3(__in,__cfg).__ST170778_Layout __PP2069009) := TRANSFORM
    __CC13949 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP2069009.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP2069009.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13949)));
    SELF := __PP2069009;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE2069008,__ND2069068__Project(LEFT));
END;

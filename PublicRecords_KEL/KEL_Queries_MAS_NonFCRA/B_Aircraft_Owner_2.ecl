...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5501113 := __ENH_Aircraft_Owner_3;
  EXPORT __ST180101_Layout := RECORD
  ...
  END;
  SHARED __ST180101_Layout __ND5501173__Project(B_Aircraft_Owner_3(__in,__cfg).__ST189750_Layout __PP5501114) := TRANSFORM
    __CC14072 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP5501114.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP5501114.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14072)));
    SELF := __PP5501114;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE5501113,__ND5501173__Project(LEFT));
END;

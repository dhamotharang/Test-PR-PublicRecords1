...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4692431 := __ENH_Aircraft_Owner_3;
  EXPORT __ST172310_Layout := RECORD
  ...
  END;
  SHARED __ST172310_Layout __ND4692491__Project(B_Aircraft_Owner_3(__in,__cfg).__ST181113_Layout __PP4692432) := TRANSFORM
    __CC13835 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP4692432.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP4692432.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13835)));
    SELF := __PP4692432;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE4692431,__ND4692491__Project(LEFT));
END;

...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1919104 := __ENH_Aircraft_Owner_3;
  EXPORT __ST163136_Layout := RECORD
  ...
  END;
  SHARED __ST163136_Layout __ND1919160__Project(B_Aircraft_Owner_3(__in,__cfg).__ST168412_Layout __PP1919105) := TRANSFORM
    __CC13871 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1919105.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1919105.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13871)));
    SELF := __PP1919105;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE1919104,__ND1919160__Project(LEFT));
END;

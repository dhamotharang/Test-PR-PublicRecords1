...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1918917 := __ENH_Aircraft_Owner_3;
  EXPORT __ST164581_Layout := RECORD
  ...
  END;
  SHARED __ST164581_Layout __ND1918973__Project(B_Aircraft_Owner_3(__in,__cfg).__ST169857_Layout __PP1918918) := TRANSFORM
    __CC13871 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1918918.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1918918.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13871)));
    SELF := __PP1918918;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE1918917,__ND1918973__Project(LEFT));
END;

...
EXPORT B_Professional_License_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1662643 := __E_Professional_License;
  EXPORT __ST173572_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST173572_Layout __ND1662570__Project(E_Professional_License(__in,__cfg).Layout __PP286740) := TRANSFORM
    __BS286889 := __T(__PP286740.License_Dates_);
    ...
    SELF.B_U_I_L_D___D_A_T_E_ := KEL.Routines.MinN(IF(EXISTS(__BS286889(__T(__OP2(__T(__PP286740.License_Dates_).Source_,=,__CN('PL'))))),__ECAST(KEL.typ.nkdate,__CC13387),__ECAST(KEL.typ.nkdate,__CC13395)),__CN(__cfg.CurrentDate));
    __EE1662565 := __PP286740.License_Dates_;
    SELF.Max_Expire_Date_ := KEL.Aggregates.MaxNN(__EE1662565,__T(__EE1662565).Date_Of_Expiration_);
    __EE1662590 := __PP286740.License_Dates_;
    SELF.Max_Issue_Date_ := KEL.Aggregates.MaxNN(__EE1662590,IF(__T(__OP2(__T(__EE1662590).Original_Date_Of_Issuance_,<,KEL.Routines.CastStringToDate(__CN('19000101')))),__ECAST(KEL.typ.nkdate,KEL.Routines.CastStringToDate(__CN(''))),__ECAST(KEL.typ.nkdate,__T(__EE1662590).Original_Date_Of_Issuance_)));
    SELF := __PP286740;
  END;
  EXPORT __ENH_Professional_License_5 := PROJECT(__EE1662643,__ND1662570__Project(LEFT));
END;

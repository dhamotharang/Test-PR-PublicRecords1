...
EXPORT B_Address_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE340084 := __E_Address;
  EXPORT __ST163347_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST163347_Layout __ND343781__Project(E_Address(__in,__cfg).Layout __PP339370) := TRANSFORM
    __EE339873 := __PP339370.A_D_V_O_Summary_;
    __BS340231 := __T(__EE339873);
    __EE340251 := __BN(TOPN(__BS340231(__NN(__T(__EE339873).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE339873).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE339873));
    SELF.Bestchild_Advo_ := __EE340251;
    SELF := __PP339370;
  END;
  EXPORT __ENH_Address_3 := PROJECT(__EE340084,__ND343781__Project(LEFT));
END;

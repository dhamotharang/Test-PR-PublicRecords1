...
EXPORT B_Education_8(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE222868 := __E_Education;
  ...
  END;
  SHARED __ST181021_Layout __ND1565797__Project(E_Education(__in,__cfg).Layout __PP222742) := TRANSFORM
    __BS222818 := __T(__PP222742.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS222818(__T(__OP2(__T(__PP222742.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP222742;
  END;
  EXPORT __ENH_Education_8 := PROJECT(__EE222868,__ND1565797__Project(LEFT));
END;

...
EXPORT B_Education_8(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE304962 := __E_Education;
  EXPORT __ST210253_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST210253_Layout __ND4691800__Project(E_Education(__in,__cfg).Layout __PP304836) := TRANSFORM
    __BS304912 := __T(__PP304836.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS304912(__T(__OP2(__T(__PP304836.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP304836;
  END;
  EXPORT __ENH_Education_8 := PROJECT(__EE304962,__ND4691800__Project(LEFT));
END;

...
EXPORT B_Person_Inquiry_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4719565 := __ENH_Person_Inquiry_8;
  EXPORT __ST329385_Layout := RECORD
  ...
  EXPORT __ST209727_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST209727_Layout __ND4719570__Project(B_Person_Inquiry_8(__in,__cfg).__ST211383_Layout __PP4719566) := TRANSFORM
    __EE4719590 := __PP4719566.Gather_Inquiries_;
    __ST329385_Layout __ND4719595__Project(B_Person_Inquiry_8(__in,__cfg).__ST94366_Layout __PP4719591) := TRANSFORM
      SELF.Exclude_Function_Description_ := KEL.Routines.ToUpperCase(TRIM(__PP4719591.Function_Description_)) <> 'BANKO BATCH';
      __CC39802 := ['CARDS - SUBPRIME','MSB','REFUND ANTICIPATION LOAN','RENT TO OWN','PAYDAY LOANS','PAY DAY LOANS','TITLE LOANS','PAY DAY','PAYDAY'];
      SELF.Is_High_Risk_ := KEL.Routines.ToUpperCase(TRIM(__PP4719591.Industry_)) IN __CC39802;
      SELF := __PP4719591;
    END;
    SELF.Gather_Inquiries_ := __PROJECT(__EE4719590,__ND4719595__Project(LEFT));
    SELF := __PP4719566;
  END;
  EXPORT __ENH_Person_Inquiry_7 := PROJECT(__EE4719565,__ND4719570__Project(LEFT));
END;

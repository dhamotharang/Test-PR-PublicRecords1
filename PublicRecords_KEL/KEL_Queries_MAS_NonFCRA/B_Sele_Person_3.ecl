...
EXPORT B_Sele_Person_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5497670 := __E_Sele_Person;
  EXPORT __ST196709_Layout := RECORD
  ...
  END;
  SHARED __ST196709_Layout __ND5497634__Project(E_Sele_Person(__in,__cfg).Layout __PP790356) := TRANSFORM
    __EE5497609 := __PP790356.Contact_Info_;
    ...
    __EE5497629 := __PP790356.Contact_Info_;
    SELF.Assoc_Date_ := IF(__T(__OP2(KEL.Aggregates.MaxNN(__EE5497609,KEL.era.ToDate(__T(__EE5497609).Date_Last_Seen_)),>,__CC13531)),__ECAST(KEL.typ.nkdate,__CC13531),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MaxNN(__EE5497629,KEL.era.ToDate(__T(__EE5497629).Date_Last_Seen_))));
    SELF := __PP790356;
  END;
  EXPORT __ENH_Sele_Person_3 := PROJECT(__EE5497670,__ND5497634__Project(LEFT));
END;

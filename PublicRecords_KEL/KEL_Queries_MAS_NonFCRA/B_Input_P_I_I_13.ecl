...
IMPORT KEL15 AS KEL;
IMPORT B_Input_P_I_I_14,CFG_Compile,E_Address,E_Address_Slim,E_Address_Summary,E_Email,E_Geo_Link,E_Name_Summary,E_Person,E_Phone,E_Phone_Summary,E_Property,E_S_S_N_Summary,E_Social_Security_Number,E_Surname,E_Zip_Code,FN_Compile FROM PublicRecords_KEL.KEL_Queries_MAS_NonFCRA;
IMPORT * FROM KEL15.Null;
EXPORT B_Input_P_I_I_13(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  SHARED VIRTUAL TYPEOF(B_Input_P_I_I_14(__in,__cfg).__ENH_Input_P_I_I_14) __ENH_Input_P_I_I_14 := B_Input_P_I_I_14(__in,__cfg).__ENH_Input_P_I_I_14;
  SHARED __EE4675611 := __ENH_Input_P_I_I_14;
  EXPORT __ST214888_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST214888_Layout __ND4675616__Project(B_Input_P_I_I_14(__in,__cfg).__ST215290_Layout __PP4675612) := TRANSFORM
    SELF.Addr_Not_Populated_ := FN_Compile(__cfg).FN_Is_Not_Enough_To_Clean(__ECAST(KEL.typ.nstr,__PP4675612.P___Inp_Addr_));
    SELF.City_State_Zip_Not_Populated_ := FN_Compile(__cfg).FN_City_State_Zip_Not_Populated_Check(__ECAST(KEL.typ.nstr,__PP4675612.P___Inp_Addr_City_),__ECAST(KEL.typ.nstr,__PP4675612.P___Inp_Addr_State_),__ECAST(KEL.typ.nstr,__PP4675612.P___Inp_Addr_Zip_));
    SELF.P___Inp_Addr_City_Flag_Value_ := FN_Compile(__cfg).FN_Is_Echo_Populated(__ECAST(KEL.typ.nstr,__PP4675612.P___Inp_Addr_City_));
    SELF.P___Inp_Addr_Zip_Flag_Value_ := FN_Compile(__cfg).FN_Is_Echo_Populated(__ECAST(KEL.typ.nstr,__PP4675612.P___Inp_Addr_Zip_));
    SELF := __PP4675612;
  END;
  EXPORT __ENH_Input_P_I_I_13 := PROJECT(__EE4675611,__ND4675616__Project(LEFT));
END;

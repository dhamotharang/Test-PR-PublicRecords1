...
IMPORT KEL15 AS KEL;
IMPORT B_Input_P_I_I_3,B_Input_P_I_I_4,CFG_Compile,E_Address,E_Address_Slim,E_Address_Summary,E_Email,E_Geo_Link,E_Name_Summary,E_Person,E_Phone,E_Phone_Summary,E_Property,E_S_S_N_Summary,E_Social_Security_Number,E_Surname,E_Zip_Code,FN_Compile FROM PublicRecords_KEL.KEL_Queries_MAS_NonFCRA;
...
EXPORT B_Input_P_I_I_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5502343 := __ENH_Input_P_I_I_3;
  EXPORT __ST234931_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST234931_Layout __ND5502348__Project(B_Input_P_I_I_4(__in,__cfg).__ST197957_Layout __PP5502344) := TRANSFORM
    SELF.C_U_R_R_E_N_T___D_A_T_E__1_Y_E_A_R_S_ := KEL.Routines.AdjustCalendar(__cfg.CurrentDate, -1,0,0);
    SELF.P___Inp_Cln_Arch_Dt_Flag_Value_ := FN_Compile(__cfg).FN_Is_Echo_Populated(__ECAST(KEL.typ.nstr,__PP5502344.P___Inp_Cln_Arch_Dt_));
    SELF := __PP5502344;
  END;
  EXPORT __ENH_Input_P_I_I_2 := PROJECT(__EE5502343,__ND5502348__Project(LEFT));
END;

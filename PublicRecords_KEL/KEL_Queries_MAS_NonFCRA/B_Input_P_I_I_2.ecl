...
IMPORT KEL15 AS KEL;
IMPORT B_Input_P_I_I_3,CFG_Compile,E_Address,E_Address_Slim,E_Address_Summary,E_Email,E_Geo_Link,E_Name_Summary,E_Person,E_Phone,E_Phone_Summary,E_Property,E_S_S_N_Summary,E_Social_Security_Number,E_Surname,E_Zip_Code,FN_Compile FROM PublicRecords_KEL.KEL_Queries_MAS_NonFCRA;
...
EXPORT B_Input_P_I_I_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4693661 := __ENH_Input_P_I_I_3;
  EXPORT __ST173258_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST173258_Layout __ND4694276__Project(B_Input_P_I_I_3(__in,__cfg).__ST182008_Layout __PP4693662) := TRANSFORM
    SELF.C_U_R_R_E_N_T___D_A_T_E__1_Y_E_A_R_S_ := KEL.Routines.AdjustCalendar(__cfg.CurrentDate, -1,0,0);
    __CC13587 := '-99999';
    __CC13592 := '-99998';
    SELF.Input_State_Clean_Value_ := MAP(__T(__AND(__CN(__PP4693662.P___Inp_Addr_State_Flag_Value_ = '0'),__OR(__CN(__PP4693662.Addr_Not_Populated_),__PP4693662.City_State_Zip_Not_Populated_)))=>__ECAST(KEL.typ.nstr,__CN(__CC13587)),__T(__AND(__CN(__PP4693662.P___Inp_Addr_State_Flag_Value_ = '1'),__OR(__CN(__PP4693662.Addr_Not_Populated_),__PP4693662.City_State_Zip_Not_Populated_)))=>__ECAST(KEL.typ.nstr,__CN(__CC13592)),__ECAST(KEL.typ.nstr,FN_Compile(__cfg).FN_Is_Blank(__ECAST(KEL.typ.nstr,__PP4693662.P___Inp_Cln_Addr_State_),__ECAST(KEL.typ.nstr,__CN(__CC13592)))));
    SELF.P___Inp_Cln_Arch_Dt_Flag_Value_ := FN_Compile(__cfg).FN_Is_Echo_Populated(__ECAST(KEL.typ.nstr,__PP4693662.P___Inp_Cln_Arch_Dt_));
    SELF := __PP4693662;
  END;
  EXPORT __ENH_Input_P_I_I_2 := PROJECT(__EE4693661,__ND4694276__Project(LEFT));
END;

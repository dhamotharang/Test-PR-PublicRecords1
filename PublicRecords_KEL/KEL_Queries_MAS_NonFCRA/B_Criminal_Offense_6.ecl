...
EXPORT B_Criminal_Offense_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4724882 := __ENH_Criminal_Offense_7;
  EXPORT __ST205745_Layout := RECORD
  ...
  END;
  EXPORT __ST205698_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST205745_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST205698_Layout __ND4724802__Project(B_Criminal_Offense_7(__in,__cfg).__ST208269_Layout __PP4724250) := TRANSFORM
    __EE4724885 := __PP4724250.Offense_Charges_;
    __ST205745_Layout __ND4724760__Project(B_Criminal_Offense_7(__in,__cfg).__ST208316_Layout __PP4724411) := TRANSFORM
      __EE4724755 := __PP4724250.Offense_Charges_;
      SELF.Criminal_Date_ := KEL.Aggregates.MinNN(__EE4724755,KEL.Routines.CastStringToDate(__T(__EE4724755).Valid_Date_));
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP4724250.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP4724250.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP4724250.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP4724250.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP4724411;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE4724885,__ND4724760__Project(LEFT));
    SELF := __PP4724250;
  END;
  EXPORT __ENH_Criminal_Offense_6 := PROJECT(__EE4724882,__ND4724802__Project(LEFT));
END;

...
EXPORT B_Person_Inquiry_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4750997 := __ENH_Person_Inquiry_7;
  EXPORT __ST355128_Layout := RECORD
  ...
  EXPORT __ST207663_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST207663_Layout __ND4750983__Project(B_Person_Inquiry_7(__in,__cfg).__ST209727_Layout __PP4750780) := TRANSFORM
    __EE4750981 := __PP4750780.Gather_Inquiries_;
    __ST355128_Layout __ND4750945__Project(B_Person_Inquiry_7(__in,__cfg).__ST329385_Layout __PP4750805) := TRANSFORM
      ...
      SELF.Is_Banking_ := KEL.Routines.ToUpperCase(TRIM(__PP4750805.Industry_)) IN __CC39813;
      __CC39816 := ['COMMUNICATIONS'];
      SELF.Is_Communications_ := KEL.Routines.ToUpperCase(TRIM(__PP4750805.Industry_)) IN __CC39816;
      SELF.Valid_Inquiries_ := __AND(__AND(__AND(__AND(__PP4750805.Is_Non_Fcra_Ok_,__CN(NOT (__PP4750805.Is_Batch_Monitoring_Method_))),__CN(__PP4750805.Exclude_Function_Description_)),__CN(NOT (__PP4750805.Is_Collection_))),__CN(NOT (__PP4750805.Is_High_Risk_)));
      SELF := __PP4750805;
    END;
    SELF.Gather_Inquiries_ := __PROJECT(__EE4750981,__ND4750945__Project(LEFT));
    SELF := __PP4750780;
  END;
  EXPORT __ENH_Person_Inquiry_6 := PROJECT(__EE4750997,__ND4750983__Project(LEFT));
END;

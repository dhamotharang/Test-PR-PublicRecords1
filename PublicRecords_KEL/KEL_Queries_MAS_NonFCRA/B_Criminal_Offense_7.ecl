...
IMPORT KEL16 AS KEL;
IMPORT B_Criminal_Offense_8,CFG_Compile,E_Criminal_Offense FROM PublicRecords_KEL.KEL_Queries_MAS_NonFCRA;
IMPORT * FROM KEL16.Null;
EXPORT B_Criminal_Offense_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  SHARED VIRTUAL TYPEOF(B_Criminal_Offense_8(__in,__cfg).__ENH_Criminal_Offense_8) __ENH_Criminal_Offense_8 := B_Criminal_Offense_8(__in,__cfg).__ENH_Criminal_Offense_8;
  SHARED __EE1578915 := __ENH_Criminal_Offense_8;
  EXPORT __ST182134_Layout := RECORD
  ...
  END;
  EXPORT __ST182087_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST182134_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST182087_Layout __ND1579293__Project(B_Criminal_Offense_8(__in,__cfg).__ST183248_Layout __PP1578916) := TRANSFORM
    __EE1579076 := __PP1578916.Offense_Charges_;
    __ST182134_Layout __ND1579280__Project(B_Criminal_Offense_8(__in,__cfg).__ST183295_Layout __PP1579077) := TRANSFORM
      __CC14112 := '-99998';
      SELF.Valid_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1579077.My_Crim_Date_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1579077.My_Crim_Date_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14112)));
      SELF := __PP1579077;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE1579076,__ND1579280__Project(LEFT));
    SELF := __PP1578916;
  END;
  EXPORT __ENH_Criminal_Offense_7 := PROJECT(__EE1578915,__ND1579293__Project(LEFT));
END;

...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE237016 := __E_Education;
  EXPORT __ST183115_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST183115_Layout __ND1609144__Project(E_Education(__in,__cfg).Layout __PP236890) := TRANSFORM
    __BS236966 := __T(__PP236890.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS236966(__T(__OP2(__T(__PP236890.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP236890;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE237016,__ND1609144__Project(LEFT));
END;

...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE230421 := __E_Education;
  ...
  END;
  SHARED __ST176949_Layout __ND1587487__Project(E_Education(__in,__cfg).Layout __PP230295) := TRANSFORM
    __BS230371 := __T(__PP230295.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS230371(__T(__OP2(__T(__PP230295.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP230295;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE230421,__ND1587487__Project(LEFT));
END;

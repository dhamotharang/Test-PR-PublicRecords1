...
EXPORT B_Aircraft_Owner_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE345616 := __E_Aircraft_Owner;
  EXPORT __ST163678_Layout := RECORD
  ...
  END;
  SHARED __ST163678_Layout __ND345628__Project(E_Aircraft_Owner(__in,__cfg).Layout __PP345554) := TRANSFORM
    ...
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.era.ToDate(__PP345554.Date_First_Seen_),>,__CC13275)),__ECAST(KEL.typ.nkdate,__CC13275),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP345554.Date_First_Seen_)));
    SELF := __PP345554;
  END;
  EXPORT __ENH_Aircraft_Owner_3 := PROJECT(__EE345616,__ND345628__Project(LEFT));
END;

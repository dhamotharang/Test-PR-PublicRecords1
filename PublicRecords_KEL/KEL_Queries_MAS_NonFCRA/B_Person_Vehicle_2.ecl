...
EXPORT B_Person_Vehicle_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2543679 := __ENH_Person_Vehicle_3;
  EXPORT __ST162253_Layout := RECORD
  ...
  END;
  SHARED __ST162253_Layout __ND2543618__Project(B_Person_Vehicle_3(__in,__cfg).__ST167063_Layout __PP2543349) := TRANSFORM
    __EE2543593 := __PP2543349.Counts_Model_;
    ...
    __EE2543613 := __PP2543349.Counts_Model_;
    SELF.Date_Last_Seen_Capped_ := IF(__T(__OP2(KEL.Aggregates.MaxNN(__EE2543593,KEL.era.ToDate(__T(__EE2543593).Date_Last_Seen_)),>,__CC13435)),__ECAST(KEL.typ.nkdate,__CC13435),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MaxNN(__EE2543613,KEL.era.ToDate(__T(__EE2543613).Date_Last_Seen_))));
    __CC13929 := '-99997';
    SELF.Vehicle_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP2543349.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP2543349.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13929)));
    SELF := __PP2543349;
  END;
  EXPORT __ENH_Person_Vehicle_2 := PROJECT(__EE2543679,__ND2543618__Project(LEFT));
END;

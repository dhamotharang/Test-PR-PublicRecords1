...
EXPORT E_Neighborhood(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  Layout Neighborhood__Rollup(InLayout __r, DATASET(InLayout) __recs) := TRANSFORM
    ...
    SELF.History_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),History_Date_,Median_Valuation_},History_Date_,Median_Valuation_),History_Layout)(__NN(History_Date_) OR __NN(Median_Valuation_)));
  ...
  Layout Neighborhood__Single_Rollup(InLayout __r) := TRANSFORM
    SELF.History_ := __CN(PROJECT(DATASET(__r),TRANSFORM(History_Layout,SELF.__RecordCount:=1;,SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF:=LEFT))(__NN(History_Date_) OR __NN(Median_Valuation_)));

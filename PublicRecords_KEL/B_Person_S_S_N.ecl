...
EXPORT B_Person_S_S_N(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1415459 := __E_Person_S_S_N;
  EXPORT __ST58740_Layout := RECORD
  ...
  END;
  SHARED __ST58740_Layout __ND1415528__Project(E_Person_S_S_N(__in,__cfg).Layout __PP1415381) := TRANSFORM
    SELF.Legacy_S_S_N_ := EXISTS(__T(__PP1415381.Temp_Legacy_S_S_N_));
    SELF := __PP1415381;
  END;
  EXPORT __ENH_Person_S_S_N := PROJECT(__EE1415459,__ND1415528__Project(LEFT));
END;

...
EXPORT B_Person_S_S_N(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE791504 := __E_Person_S_S_N;
  EXPORT __ST47442_Layout := RECORD
  ...
  END;
  SHARED __ST47442_Layout __ND791569__Project(E_Person_S_S_N(__in,__cfg).Layout __PP791429) := TRANSFORM
    SELF.Legacy_S_S_N_ := EXISTS(__T(__PP791429.Temp_Legacy_S_S_N_));
    SELF := __PP791429;
  END;
  EXPORT __ENH_Person_S_S_N := PROJECT(__EE791504,__ND791569__Project(LEFT));
END;

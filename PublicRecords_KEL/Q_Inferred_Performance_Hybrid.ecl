...
EXPORT Q_Inferred_Performance_Hybrid(KEL.typ.uid __PLexID_in, DATASET(RECORDOF(PublicRecords_KEL.ECL_Functions.Layouts.LayoutInputPII)) __PInputPIIDataset, KEL.typ.kdate __PP_InpClnArchDt, DATA57 __PDPM) := MODULE
  ...
  END;
  SHARED E_Person_Filtered_2 := MODULE(E_Person(__cfg_Local))
  ...
  SHARED E_Bankruptcy_Filtered := MODULE(E_Bankruptcy(__cfg_Local))
    SHARED __AsofFitler(DATASET(InLayout) __ds) := __ds(__T(__OP2(KEL.era.ToDateMinNull(__ds.Hybrid_Archive_Date_),<=,__CN(KEL.Routines.AdjustCalendar(__PP_InpClnArchDt,2,0,0)))));
    SHARED __UsingFitler(DATASET(InLayout) __ds) := __ds(KEL.Permits.BitAnd(__ds.__Permits,__PDPM) = __ds.__Permits);
    SHARED __GroupTest(DATASET(InLayout) __g) := EXISTS(__g(__T(FN_Compile(__cfg_Local).FN_Within_F1_Y(__ECAST(KEL.typ.nkdate,__g.Date_Filed_),__ECAST(KEL.typ.nkdate,__CN(__cfg_Local.CurrentDate))))));
    SHARED __SourceFilter(DATASET(InLayout) __ds) := __UsingFitler(__AsofFitler(__ds));
    SHARED __GroupedFilter(GROUPED DATASET(InLayout) __ds) := JOIN(HAVING(__ds,__GroupTest(ROWS(LEFT))),E_Person_Bankruptcy_Filtered_1.__PostFilter,__EEQP(LEFT.UID,RIGHT.Bankrupt_),TRANSFORM(InLayout,SELF:=LEFT),HASH,GROUPED,KEEP(1));
  END;
  ...
  SHARED __EE12839093 := __ENH_Person;
  SHARED __EE12841005 := __EE12839093(__T(__OP2(__EE12839093.UID,=,__CN(__PLexID_in))));
  ...
  SHARED __ST127345_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT Res1 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE12841035,__ST127345_Layout),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),P___Inp_Cln_Arch_Dt_F6_M_,P___Inp_Cln_Arch_Dt_F1_Y_,P___Inp_Cln_Arch_Dt_F2_Y_},P___Inp_Cln_Arch_Dt_F6_M_,P___Inp_Cln_Arch_Dt_F1_Y_,P___Inp_Cln_Arch_Dt_F2_Y_,MERGE),__ST127345_Layout));

...
EXPORT B_Sele_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE9691556 := __ENH_Sele_Vehicle_2;
  EXPORT __ST213289_Layout := RECORD
  ...
  END;
  SHARED __ST213289_Layout __ND9691762__Project(B_Sele_Vehicle_2(__in,__cfg).__ST232447_Layout __PP9691557) := TRANSFORM
    __CC13780 := ['AG','AR','CLG','CML','DE','DLR','EA','EMR','ENV','FGV','FNL','FOR','LGV','LIV','MFG','MIL','MUB','OFF','POL','SCB','SGV','TAX'];
    SELF.Commercial_Type_ := __OP2(__PP9691557.Vehicle_Type_Code_,IN,__CN(__CC13780));
    __CC13790 := ['BOT','EXT','MOT','OTH','TRL','UNK','VAN','XSR'];
    SELF.Other_Type_ := __OR(__OR(__OP2(__PP9691557.Vehicle_Type_Code_,IN,__CN(__CC13790)),__OP2(__PP9691557.Vehicle_Type_Code_,=,__CN(''))),__NT(__PP9691557.Vehicle_Type_Code_));
    __CC13756 := ['ANQ','DAV','HCP','MH','PRV'];
    SELF.Private_Type_ := __OP2(__PP9691557.Vehicle_Type_Code_,IN,__CN(__CC13756));
    SELF := __PP9691557;
  END;
  EXPORT __ENH_Sele_Vehicle_1 := PROJECT(__EE9691556,__ND9691762__Project(LEFT));
END;

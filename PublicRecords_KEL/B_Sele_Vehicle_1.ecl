...
EXPORT B_Sele_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE9556069 := __ENH_Sele_Vehicle_2;
  ...
  END;
  SHARED __ST209280_Layout __ND9556275__Project(B_Sele_Vehicle_2(__in,__cfg).__ST228584_Layout __PP9556070) := TRANSFORM
    __CC13743 := ['AG','AR','CLG','CML','DE','DLR','EA','EMR','ENV','FGV','FNL','FOR','LGV','LIV','MFG','MIL','MUB','OFF','POL','SCB','SGV','TAX'];
    SELF.Commercial_Type_ := __OP2(__PP9556070.Vehicle_Type_Code_,IN,__CN(__CC13743));
    __CC13753 := ['BOT','EXT','MOT','OTH','TRL','UNK','VAN','XSR'];
    SELF.Other_Type_ := __OR(__OR(__OP2(__PP9556070.Vehicle_Type_Code_,IN,__CN(__CC13753)),__OP2(__PP9556070.Vehicle_Type_Code_,=,__CN(''))),__NT(__PP9556070.Vehicle_Type_Code_));
    __CC13719 := ['ANQ','DAV','HCP','MH','PRV'];
    SELF.Private_Type_ := __OP2(__PP9556070.Vehicle_Type_Code_,IN,__CN(__CC13719));
    SELF := __PP9556070;
  END;
  EXPORT __ENH_Sele_Vehicle_1 := PROJECT(__EE9556069,__ND9556275__Project(LEFT));
END;

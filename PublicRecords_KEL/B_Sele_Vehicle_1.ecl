...
EXPORT B_Sele_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7941282 := __ENH_Sele_Vehicle_2;
  EXPORT __ST192960_Layout := RECORD
  ...
  END;
  SHARED __ST192960_Layout __ND7941488__Project(B_Sele_Vehicle_2(__in,__cfg).__ST211255_Layout __PP7941283) := TRANSFORM
    __CC13514 := ['AG','AR','CLG','CML','DE','DLR','EA','EMR','ENV','FGV','FNL','FOR','LGV','LIV','MFG','MIL','MUB','OFF','POL','SCB','SGV','TAX'];
    SELF.Commercial_Type_ := __OP2(__PP7941283.Vehicle_Type_Code_,IN,__CN(__CC13514));
    __CC13524 := ['BOT','EXT','MOT','OTH','TRL','UNK','VAN','XSR'];
    SELF.Other_Type_ := __OR(__OR(__OP2(__PP7941283.Vehicle_Type_Code_,IN,__CN(__CC13524)),__OP2(__PP7941283.Vehicle_Type_Code_,=,__CN(''))),__NT(__PP7941283.Vehicle_Type_Code_));
    __CC13490 := ['ANQ','DAV','HCP','MH','PRV'];
    SELF.Private_Type_ := __OP2(__PP7941283.Vehicle_Type_Code_,IN,__CN(__CC13490));
    SELF := __PP7941283;
  END;
  EXPORT __ENH_Sele_Vehicle_1 := PROJECT(__EE7941282,__ND7941488__Project(LEFT));
END;

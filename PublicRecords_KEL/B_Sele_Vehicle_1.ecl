...
EXPORT B_Sele_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7953362 := __ENH_Sele_Vehicle_2;
  EXPORT __ST193475_Layout := RECORD
  ...
  END;
  SHARED __ST193475_Layout __ND7953568__Project(B_Sele_Vehicle_2(__in,__cfg).__ST211796_Layout __PP7953363) := TRANSFORM
    __CC13534 := ['AG','AR','CLG','CML','DE','DLR','EA','EMR','ENV','FGV','FNL','FOR','LGV','LIV','MFG','MIL','MUB','OFF','POL','SCB','SGV','TAX'];
    SELF.Commercial_Type_ := __OP2(__PP7953363.Vehicle_Type_Code_,IN,__CN(__CC13534));
    __CC13544 := ['BOT','EXT','MOT','OTH','TRL','UNK','VAN','XSR'];
    SELF.Other_Type_ := __OR(__OR(__OP2(__PP7953363.Vehicle_Type_Code_,IN,__CN(__CC13544)),__OP2(__PP7953363.Vehicle_Type_Code_,=,__CN(''))),__NT(__PP7953363.Vehicle_Type_Code_));
    __CC13510 := ['ANQ','DAV','HCP','MH','PRV'];
    SELF.Private_Type_ := __OP2(__PP7953363.Vehicle_Type_Code_,IN,__CN(__CC13510));
    SELF := __PP7953363;
  END;
  EXPORT __ENH_Sele_Vehicle_1 := PROJECT(__EE7953362,__ND7953568__Project(LEFT));
END;

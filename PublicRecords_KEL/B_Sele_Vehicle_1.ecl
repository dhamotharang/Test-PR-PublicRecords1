...
EXPORT B_Sele_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7946288 := __ENH_Sele_Vehicle_2;
  EXPORT __ST192996_Layout := RECORD
  ...
  END;
  SHARED __ST192996_Layout __ND7946494__Project(B_Sele_Vehicle_2(__in,__cfg).__ST211295_Layout __PP7946289) := TRANSFORM
    __CC13503 := ['AG','AR','CLG','CML','DE','DLR','EA','EMR','ENV','FGV','FNL','FOR','LGV','LIV','MFG','MIL','MUB','OFF','POL','SCB','SGV','TAX'];
    SELF.Commercial_Type_ := __OP2(__PP7946289.Vehicle_Type_Code_,IN,__CN(__CC13503));
    __CC13513 := ['BOT','EXT','MOT','OTH','TRL','UNK','VAN','XSR'];
    SELF.Other_Type_ := __OR(__OR(__OP2(__PP7946289.Vehicle_Type_Code_,IN,__CN(__CC13513)),__OP2(__PP7946289.Vehicle_Type_Code_,=,__CN(''))),__NT(__PP7946289.Vehicle_Type_Code_));
    __CC13479 := ['ANQ','DAV','HCP','MH','PRV'];
    SELF.Private_Type_ := __OP2(__PP7946289.Vehicle_Type_Code_,IN,__CN(__CC13479));
    SELF := __PP7946289;
  END;
  EXPORT __ENH_Sele_Vehicle_1 := PROJECT(__EE7946288,__ND7946494__Project(LEFT));
END;

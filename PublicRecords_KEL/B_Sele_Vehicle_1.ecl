...
EXPORT B_Sele_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE9607859 := __ENH_Sele_Vehicle_2;
  ...
  END;
  SHARED __ST211757_Layout __ND9608065__Project(B_Sele_Vehicle_2(__in,__cfg).__ST230865_Layout __PP9607860) := TRANSFORM
    __CC13795 := ['AG','AR','CLG','CML','DE','DLR','EA','EMR','ENV','FGV','FNL','FOR','LGV','LIV','MFG','MIL','MUB','OFF','POL','SCB','SGV','TAX'];
    SELF.Commercial_Type_ := __OP2(__PP9607860.Vehicle_Type_Code_,IN,__CN(__CC13795));
    __CC13805 := ['BOT','EXT','MOT','OTH','TRL','UNK','VAN','XSR'];
    SELF.Other_Type_ := __OR(__OR(__OP2(__PP9607860.Vehicle_Type_Code_,IN,__CN(__CC13805)),__OP2(__PP9607860.Vehicle_Type_Code_,=,__CN(''))),__NT(__PP9607860.Vehicle_Type_Code_));
    __CC13771 := ['ANQ','DAV','HCP','MH','PRV'];
    SELF.Private_Type_ := __OP2(__PP9607860.Vehicle_Type_Code_,IN,__CN(__CC13771));
    SELF := __PP9607860;
  END;
  EXPORT __ENH_Sele_Vehicle_1 := PROJECT(__EE9607859,__ND9608065__Project(LEFT));
END;

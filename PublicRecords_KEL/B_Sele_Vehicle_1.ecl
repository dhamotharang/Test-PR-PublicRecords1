...
EXPORT B_Sele_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1130642 := __ENH_Sele_Vehicle_2;
  ...
  END;
  SHARED __ST87340_Layout __ND1130986__Project(B_Sele_Vehicle_2(__in,__cfg).__ST95439_Layout __PP1130392) := TRANSFORM
    __CC9124 := ['AG','AR','CLG','CML','DE','DLR','EA','EMR','ENV','FGV','FNL','FOR','LGV','LIV','MFG','MIL','MUB','OFF','POL','SCB','SGV','TAX'];
    SELF.Commercial_Type_ := __OP2(__PP1130392.Vehicle_Type_Code_,IN,__CN(__CC9124));
    __CC9134 := ['BOT','EXT','MOT','OTH','TRL','UNK','VAN','XSR'];
    SELF.Other_Type_ := __OR(__OR(__OP2(__PP1130392.Vehicle_Type_Code_,IN,__CN(__CC9134)),__OP2(__PP1130392.Vehicle_Type_Code_,=,__CN(''))),__NT(__PP1130392.Vehicle_Type_Code_));
    __CC9100 := ['ANQ','DAV','HCP','MH','PRV'];
    SELF.Private_Type_ := __OP2(__PP1130392.Vehicle_Type_Code_,IN,__CN(__CC9100));
    SELF := __PP1130392;
  END;
  EXPORT __ENH_Sele_Vehicle_1 := PROJECT(__EE1130642,__ND1130986__Project(LEFT));
END;

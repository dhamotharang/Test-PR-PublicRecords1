...
EXPORT B_Sele_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7953367 := __ENH_Sele_Vehicle_2;
  EXPORT __ST193480_Layout := RECORD
  ...
  END;
  SHARED __ST193480_Layout __ND7953573__Project(B_Sele_Vehicle_2(__in,__cfg).__ST211801_Layout __PP7953368) := TRANSFORM
    __CC13539 := ['AG','AR','CLG','CML','DE','DLR','EA','EMR','ENV','FGV','FNL','FOR','LGV','LIV','MFG','MIL','MUB','OFF','POL','SCB','SGV','TAX'];
    SELF.Commercial_Type_ := __OP2(__PP7953368.Vehicle_Type_Code_,IN,__CN(__CC13539));
    __CC13549 := ['BOT','EXT','MOT','OTH','TRL','UNK','VAN','XSR'];
    SELF.Other_Type_ := __OR(__OR(__OP2(__PP7953368.Vehicle_Type_Code_,IN,__CN(__CC13549)),__OP2(__PP7953368.Vehicle_Type_Code_,=,__CN(''))),__NT(__PP7953368.Vehicle_Type_Code_));
    __CC13515 := ['ANQ','DAV','HCP','MH','PRV'];
    SELF.Private_Type_ := __OP2(__PP7953368.Vehicle_Type_Code_,IN,__CN(__CC13515));
    SELF := __PP7953368;
  END;
  EXPORT __ENH_Sele_Vehicle_1 := PROJECT(__EE7953367,__ND7953573__Project(LEFT));
END;

...
EXPORT B_Sele_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8108929 := __ENH_Sele_Vehicle_2;
  EXPORT __ST197434_Layout := RECORD
  ...
  END;
  SHARED __ST197434_Layout __ND8109135__Project(B_Sele_Vehicle_2(__in,__cfg).__ST215672_Layout __PP8108930) := TRANSFORM
    __CC13795 := ['AG','AR','CLG','CML','DE','DLR','EA','EMR','ENV','FGV','FNL','FOR','LGV','LIV','MFG','MIL','MUB','OFF','POL','SCB','SGV','TAX'];
    SELF.Commercial_Type_ := __OP2(__PP8108930.Vehicle_Type_Code_,IN,__CN(__CC13795));
    __CC13805 := ['BOT','EXT','MOT','OTH','TRL','UNK','VAN','XSR'];
    SELF.Other_Type_ := __OR(__OR(__OP2(__PP8108930.Vehicle_Type_Code_,IN,__CN(__CC13805)),__OP2(__PP8108930.Vehicle_Type_Code_,=,__CN(''))),__NT(__PP8108930.Vehicle_Type_Code_));
    __CC13771 := ['ANQ','DAV','HCP','MH','PRV'];
    SELF.Private_Type_ := __OP2(__PP8108930.Vehicle_Type_Code_,IN,__CN(__CC13771));
    SELF := __PP8108930;
  END;
  EXPORT __ENH_Sele_Vehicle_1 := PROJECT(__EE8108929,__ND8109135__Project(LEFT));
END;

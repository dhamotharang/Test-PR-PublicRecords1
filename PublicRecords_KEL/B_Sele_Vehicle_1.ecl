...
EXPORT B_Sele_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8443336 := __ENH_Sele_Vehicle_2;
  EXPORT __ST200888_Layout := RECORD
  ...
  END;
  SHARED __ST200888_Layout __ND8443542__Project(B_Sele_Vehicle_2(__in,__cfg).__ST219614_Layout __PP8443337) := TRANSFORM
    __CC13608 := ['AG','AR','CLG','CML','DE','DLR','EA','EMR','ENV','FGV','FNL','FOR','LGV','LIV','MFG','MIL','MUB','OFF','POL','SCB','SGV','TAX'];
    SELF.Commercial_Type_ := __OP2(__PP8443337.Vehicle_Type_Code_,IN,__CN(__CC13608));
    __CC13618 := ['BOT','EXT','MOT','OTH','TRL','UNK','VAN','XSR'];
    SELF.Other_Type_ := __OR(__OR(__OP2(__PP8443337.Vehicle_Type_Code_,IN,__CN(__CC13618)),__OP2(__PP8443337.Vehicle_Type_Code_,=,__CN(''))),__NT(__PP8443337.Vehicle_Type_Code_));
    __CC13584 := ['ANQ','DAV','HCP','MH','PRV'];
    SELF.Private_Type_ := __OP2(__PP8443337.Vehicle_Type_Code_,IN,__CN(__CC13584));
    SELF := __PP8443337;
  END;
  EXPORT __ENH_Sele_Vehicle_1 := PROJECT(__EE8443336,__ND8443542__Project(LEFT));
END;

...
EXPORT B_Sele_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE10290779 := __ENH_Sele_Vehicle_2;
  EXPORT __ST219263_Layout := RECORD
  ...
  END;
  SHARED __ST219263_Layout __ND10290985__Project(B_Sele_Vehicle_2(__in,__cfg).__ST239698_Layout __PP10290780) := TRANSFORM
    __CC13745 := ['AG','AR','CLG','CML','DE','DLR','EA','EMR','ENV','FGV','FNL','FOR','LGV','LIV','MFG','MIL','MUB','OFF','POL','SCB','SGV','TAX'];
    SELF.Commercial_Type_ := __OP2(__PP10290780.Vehicle_Type_Code_,IN,__CN(__CC13745));
    __CC13755 := ['BOT','EXT','MOT','OTH','TRL','UNK','VAN','XSR'];
    SELF.Other_Type_ := __OR(__OR(__OP2(__PP10290780.Vehicle_Type_Code_,IN,__CN(__CC13755)),__OP2(__PP10290780.Vehicle_Type_Code_,=,__CN(''))),__NT(__PP10290780.Vehicle_Type_Code_));
    __CC13721 := ['ANQ','DAV','HCP','MH','PRV'];
    SELF.Private_Type_ := __OP2(__PP10290780.Vehicle_Type_Code_,IN,__CN(__CC13721));
    SELF := __PP10290780;
  END;
  EXPORT __ENH_Sele_Vehicle_1 := PROJECT(__EE10290779,__ND10290985__Project(LEFT));
END;

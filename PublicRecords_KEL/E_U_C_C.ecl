...
EXPORT E_U_C_C(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  Layout U_C_C__Rollup(InLayout __r, DATASET(InLayout) __recs) := TRANSFORM
    ...
    SELF.Sub_Filing_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Date_Vendor_First_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_First_Reported_,MIN,FALSE),KEL.typ.epoch Date_Vendor_Last_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_Last_Reported_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,FALSE),R_M_S_I_D_,Filing_Jurisdiction_,Filing_Number_,Filing_Type_,Filing_Date_,Original_Filing_Date_,Filing_Status_,Filing_Time_,Status_Type_,Filing_Agency_,Expiration_Date_,Contract_Type_,Vendor_Entry_Date_,Vendor_Update_Date_,Statements_Filed_,Foreign_Flag_,Process_Date_},R_M_S_I_D_,Filing_Jurisdiction_,Filing_Number_,Filing_Type_,Filing_Date_,Original_Filing_Date_,Filing_Status_,Filing_Time_,Status_Type_,Filing_Agency_,Expiration_Date_,Contract_Type_,Vendor_Entry_Date_,Vendor_Update_Date_,Statements_Filed_,Foreign_Flag_,Process_Date_),Sub_Filing_Layout)(__NN(R_M_S_I_D_) OR __NN(Filing_Jurisdiction_) OR __NN(Filing_Number_) OR __NN(Filing_Type_) OR __NN(Filing_Date_) OR __NN(Original_Filing_Date_) OR __NN(Filing_Status_) OR __NN(Filing_Time_) OR __NN(Status_Type_) OR __NN(Filing_Agency_) OR __NN(Expiration_Date_) OR __NN(Contract_Type_) OR __NN(Vendor_Entry_Date_) OR __NN(Vendor_Update_Date_) OR __NN(Statements_Filed_) OR __NN(Foreign_Flag_) OR __NN(Process_Date_)));
    SELF.Collateral_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Date_Vendor_First_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_First_Reported_,MIN,FALSE),KEL.typ.epoch Date_Vendor_Last_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_Last_Reported_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,FALSE),Collateral_Desc_,Collateral_Machine_},Collateral_Desc_,Collateral_Machine_),Collateral_Layout)(__NN(Collateral_Desc_) OR __NN(Collateral_Machine_)));
  ...
  EXPORT NullCounts := DATASET([
    ...
    {'UCC','PublicRecords_KEL.ECL_Functions.Dataset_FDC','VaultDateFirstSeen',COUNT(__d0(Vault_Date_First_Seen_=0)),COUNT(__d0(Vault_Date_First_Seen_!=0))},
    ...
    {'UCC','PublicRecords_KEL.ECL_Functions.Dataset_FDC','VaultDateFirstSeen',COUNT(__d1(Vault_Date_First_Seen_=0)),COUNT(__d1(Vault_Date_First_Seen_!=0))},
    ...
    {'UCC','PublicRecords_KEL.ECL_Functions.Dataset_FDC','VaultDateFirstSeen',COUNT(__d2(Vault_Date_First_Seen_=0)),COUNT(__d2(Vault_Date_First_Seen_!=0))},

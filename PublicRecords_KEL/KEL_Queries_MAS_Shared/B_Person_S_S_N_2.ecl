...
EXPORT B_Person_S_S_N_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE163604 := __E_Person_S_S_N;
  EXPORT __ST143373_Layout := RECORD
  ...
  END;
  SHARED __ST143373_Layout __ND186030__Project(E_Person_S_S_N(__in,__cfg).Layout __PP163510) := TRANSFORM
    __BS163562 := __T(__PP163510.Data_Sources_);
    SELF.Is_Best_S_S_N_ := EXISTS(__BS163562(__T(__OP2(__T(__PP163510.Data_Sources_).Source_,IN,__CN(['BP','NEN','NEQ'])))));
    SELF := __PP163510;
  END;
  EXPORT __ENH_Person_S_S_N_2 := PROJECT(__EE163604,__ND186030__Project(LEFT));
END;

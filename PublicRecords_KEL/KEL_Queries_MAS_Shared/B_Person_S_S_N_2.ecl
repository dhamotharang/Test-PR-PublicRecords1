...
EXPORT B_Person_S_S_N_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE160431 := __E_Person_S_S_N;
  EXPORT __ST137775_Layout := RECORD
  ...
  END;
  SHARED __ST137775_Layout __ND184974__Project(E_Person_S_S_N(__in,__cfg).Layout __PP160337) := TRANSFORM
    __BS160389 := __T(__PP160337.Data_Sources_);
    SELF.Is_Best_S_S_N_ := EXISTS(__BS160389(__T(__OP2(__T(__PP160337.Data_Sources_).Source_,IN,__CN(['BP','NEN','NEQ'])))));
    SELF := __PP160337;
  END;
  EXPORT __ENH_Person_S_S_N_2 := PROJECT(__EE160431,__ND184974__Project(LEFT));
END;

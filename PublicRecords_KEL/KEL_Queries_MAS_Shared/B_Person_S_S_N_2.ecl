...
EXPORT B_Person_S_S_N_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE169597 := __E_Person_S_S_N;
  EXPORT __ST149366_Layout := RECORD
  ...
  END;
  SHARED __ST149366_Layout __ND192023__Project(E_Person_S_S_N(__in,__cfg).Layout __PP169503) := TRANSFORM
    __BS169555 := __T(__PP169503.Data_Sources_);
    SELF.Is_Best_S_S_N_ := EXISTS(__BS169555(__T(__OP2(__T(__PP169503.Data_Sources_).Source_,IN,__CN(['BP','NEN','NEQ'])))));
    SELF := __PP169503;
  END;
  EXPORT __ENH_Person_S_S_N_2 := PROJECT(__EE169597,__ND192023__Project(LEFT));
END;

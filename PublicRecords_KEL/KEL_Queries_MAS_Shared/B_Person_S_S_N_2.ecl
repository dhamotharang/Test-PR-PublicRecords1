...
EXPORT B_Person_S_S_N_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE186867 := __E_Person_S_S_N;
  EXPORT __ST151630_Layout := RECORD
  ...
  END;
  SHARED __ST151630_Layout __ND218185__Project(E_Person_S_S_N(__in,__cfg).Layout __PP186773) := TRANSFORM
    __BS186825 := __T(__PP186773.Data_Sources_);
    SELF.Is_Best_S_S_N_ := EXISTS(__BS186825(__T(__OP2(__T(__PP186773.Data_Sources_).Source_,IN,__CN(['BP','NEN','NEQ'])))));
    SELF := __PP186773;
  END;
  EXPORT __ENH_Person_S_S_N_2 := PROJECT(__EE186867,__ND218185__Project(LEFT));
END;

...
EXPORT B_Person_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE206227 := __ENH_Person_5;
  EXPORT __ST154100_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST154100_Layout __ND206232__Project(B_Person_5(__in,__cfg).__ST153306_Layout __PP206228) := TRANSFORM
    __EE206403 := __PP206228.Recent_Addr_Full_Set_;
    __BS206404 := __T(__EE206403);
    __EE206415 := __BN(TOPN(__BS206404(__NN(__T(__EE206403).Address_Rank_) AND __NN(__T(__EE206403).Sort_Field_)),1,__T(__T(__EE206403).Address_Rank_), -__T(__T(__EE206403).Sort_Field_),__T(Address_Type_),__T(Address_Status_),__T(State_Code_),__T(County_Code_),__T(Latitude_),__T(Longitude_),__T(Geo_Blk_),__T(Addr_Full_),__T(Primary_Range_),__T(Predirectional_),__T(Primary_Name_),__T(Suffix_),__T(Postdirectional_),__T(Unit_Designation_),__T(Secondary_Range_),__T(Postal_City_),__T(State_),__T(Z_I_P5_),__T(Z_I_P4_),__T(Addr1_From_Components_),__T(Hierarchy_Date_First_Seen_),__T(Hierarchy_Date_Last_Seen_)),__NL(__EE206403));
    SELF.Curr_Addr_Full_Set_ := __EE206415;
    SELF := __PP206228;
  END;
  EXPORT __ENH_Person_4 := PROJECT(__EE206227,__ND206232__Project(LEFT));
END;

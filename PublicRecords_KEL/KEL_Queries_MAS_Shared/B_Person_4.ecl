...
EXPORT B_Person_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE178528 := __ENH_Person_5;
  EXPORT __ST144003_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST144003_Layout __ND178533__Project(B_Person_5(__in,__cfg).__ST143668_Layout __PP178529) := TRANSFORM
    __EE178704 := __PP178529.Recent_Addr_Full_Set_;
    __BS178705 := __T(__EE178704);
    __EE178716 := __BN(TOPN(__BS178705(__NN(__T(__EE178704).Address_Rank_) AND __NN(__T(__EE178704).Sort_Field_)),1,__T(__T(__EE178704).Address_Rank_), -__T(__T(__EE178704).Sort_Field_),__T(Address_Type_),__T(Address_Status_),__T(State_Code_),__T(County_Code_),__T(Latitude_),__T(Longitude_),__T(Geo_Blk_),__T(Addr_Full_),__T(Primary_Range_),__T(Predirectional_),__T(Primary_Name_),__T(Suffix_),__T(Postdirectional_),__T(Unit_Designation_),__T(Secondary_Range_),__T(Postal_City_),__T(State_),__T(Z_I_P5_),__T(Z_I_P4_),__T(Addr1_From_Components_),__T(Hierarchy_Date_First_Seen_),__T(Hierarchy_Date_Last_Seen_)),__NL(__EE178704));
    SELF.Curr_Addr_Full_Set_ := __EE178716;
    SELF := __PP178529;
  END;
  EXPORT __ENH_Person_4 := PROJECT(__EE178528,__ND178533__Project(LEFT));
END;

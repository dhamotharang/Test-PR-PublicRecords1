...
EXPORT B_Person_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE207844 := __ENH_Person_5;
  EXPORT __ST155717_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST155717_Layout __ND207849__Project(B_Person_5(__in,__cfg).__ST154923_Layout __PP207845) := TRANSFORM
    __EE208020 := __PP207845.Recent_Addr_Full_Set_;
    __BS208021 := __T(__EE208020);
    __EE208032 := __BN(TOPN(__BS208021(__NN(__T(__EE208020).Address_Rank_) AND __NN(__T(__EE208020).Sort_Field_)),1,__T(__T(__EE208020).Address_Rank_), -__T(__T(__EE208020).Sort_Field_),__T(Address_Type_),__T(Address_Status_),__T(State_Code_),__T(County_Code_),__T(Latitude_),__T(Longitude_),__T(Geo_Blk_),__T(Addr_Full_),__T(Primary_Range_),__T(Predirectional_),__T(Primary_Name_),__T(Suffix_),__T(Postdirectional_),__T(Unit_Designation_),__T(Secondary_Range_),__T(Postal_City_),__T(State_),__T(Z_I_P5_),__T(Z_I_P4_),__T(Addr1_From_Components_),__T(Hierarchy_Date_First_Seen_),__T(Hierarchy_Date_Last_Seen_)),__NL(__EE208020));
    SELF.Curr_Addr_Full_Set_ := __EE208032;
    SELF := __PP207845;
  END;
  EXPORT __ENH_Person_4 := PROJECT(__EE207844,__ND207849__Project(LEFT));
END;

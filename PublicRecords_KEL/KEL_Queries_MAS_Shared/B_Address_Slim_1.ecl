...
EXPORT B_Address_Slim_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE217053 := __ENH_Address_Slim_2;
  SHARED __EE217055 := __ENH_Input_P_I_I_2;
  SHARED __ST185486_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  __JC217064(B_Address_Slim_2(__in,__cfg).__ST148851_Layout __EE217053, B_Input_P_I_I_2(__in,__cfg).__ST149020_Layout __EE217055) := __EEQP(__EE217053.P_I_I_,__EE217055.UID);
  __ST185486_Layout __JT217064(B_Address_Slim_2(__in,__cfg).__ST148851_Layout __l, B_Input_P_I_I_2(__in,__cfg).__ST149020_Layout __r) := TRANSFORM
    SELF.U_I_D__1_ := __r.UID;
    SELF.Z_I_P5__1_ := __r.Z_I_P5_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE217238 := JOIN(__EE217053,__EE217055,__JC217064(LEFT,RIGHT),__JT217064(LEFT,RIGHT),LEFT OUTER,SMART);
  EXPORT __ST148577_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nstr Primary_Range_;
    KEL.typ.nstr Predirectional_;
    KEL.typ.nstr Primary_Name_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST148577_Layout __ND216938__Project(__ST185486_Layout __PP215936) := TRANSFORM
    __EE216928 := __PP215936.Address_N_A_I_C_S_Sorted_List_;
    SELF.Address_N_A_I_C_S_Not_Contains_ := __NOT(__FN2(KEL.Routines.Contains,KEL.Aggregates.ConcatNN(__EE216928,LEFT.High_Risk_N_A_I_C_S_,__CN('|')),__CN('922140')));
    __EE216950 := __PP215936.Address_S_I_C_Sorted_List_;
    SELF.Address_S_I_C_Not_Contains_ := __NOT(__FN2(KEL.Routines.Contains,KEL.Aggregates.ConcatNN(__EE216950,LEFT.High_Risk_S_I_C_,__CN('|')),__CN('9223')));
    __BS216970 := __T(__PP215936.Address_N_A_I_C_S_Sorted_List_);
    SELF.N_A_I_C_S_List_Not_Foind_ := __OR(__NOT(__CN(EXISTS(__T(__PP215936.Address_N_A_I_C_S_Sorted_List_)))),__CN(NOT EXISTS(__BS216970(__T(__NOT(__OR(__OP2(__CAST(KEL.typ.str,__T(__PP215936.Address_N_A_I_C_S_Sorted_List_).High_Risk_N_A_I_C_S_),=,__CN('')),__NT(__T(__PP215936.Address_N_A_I_C_S_Sorted_List_).High_Risk_N_A_I_C_S_))))))));
    SELF.P___Cln_Addr_Full_Flag_ := __PP215936.P___Inp_Cln_Addr_Full_Flag_Value_;
    __BS216999 := __T(__PP215936.Address_S_I_C_Sorted_List_);
    SELF.S_I_C_List_Not_Found_ := __OR(__NOT(__CN(EXISTS(__T(__PP215936.Address_S_I_C_Sorted_List_)))),__CN(NOT EXISTS(__BS216999(__T(__NOT(__OR(__OP2(__CAST(KEL.typ.str,__T(__PP215936.Address_S_I_C_Sorted_List_).High_Risk_S_I_C_),=,__CN('')),__NT(__T(__PP215936.Address_S_I_C_Sorted_List_).High_Risk_S_I_C_))))))));
    SELF := __PP215936;
  END;
  EXPORT __ENH_Address_Slim_1 := PROJECT(__EE217238,__ND216938__Project(LEFT));
END;

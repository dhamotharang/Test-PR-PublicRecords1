...
EXPORT E_Sele_Email(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __d0__r_Email__Mapped := IF(__d0_Missing__r_Email__UIDComponents = 'contact_email',PROJECT(__d0_Legal__Mapped,TRANSFORM(__d0__r_Email__Layout,SELF._r_Email_:=0,SELF:=LEFT)),JOIN(KEL.Intake.AppendFields(__d0_Legal__Mapped,__d0_Missing__r_Email__UIDComponents),E_Email(__in,__cfg).Lookup,TRIM((STRING)LEFT.contact_email) = RIGHT.KeyVal,TRANSFORM(__d0__r_Email__Layout,SELF._r_Email_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,SMART));

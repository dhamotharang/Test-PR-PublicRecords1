...
EXPORT E_Business_Pow(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __Trimmed := RECORD, MAXLENGTH(5000)
    STRING KeyVal;
  END;
  SHARED __d0_Trim := PROJECT(PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key,TRANSFORM(__Trimmed,SELF.KeyVal:=TRIM((STRING)LEFT.ultid) + '|' + TRIM((STRING)LEFT.orgid) + '|' + TRIM((STRING)LEFT.seleid) + '|' + TRIM((STRING)LEFT.proxid) + '|' + TRIM((STRING)LEFT.powid)));
  ...
  EXPORT Lookup := NullLookupRec + PROJECT(__Table,TRANSFORM(__TabRec,SELF.UID:=COUNTER,SELF:=LEFT)) : PERSIST('~temp::KEL::PublicRecords_KEL::Business_Pow::UidLookup',EXPIRE(7));
  EXPORT UID_IdToText := INDEX(Lookup,{UID},{Lookup},'~temp::KEL::IDtoT::PublicRecords_KEL::Business_Pow');
  EXPORT UID_TextToId := INDEX(Lookup,{ht:=HASH32(KeyVal)},{Lookup},'~temp::KEL::TtoID::PublicRecords_KEL::Business_Pow');
  ...
  SHARED __d0_Out := RECORD
    ...
    KEL.typ.uid UID := 0;
  END;
  SHARED __d0_UID_Mapped := JOIN(PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key,Lookup,TRIM((STRING)LEFT.ultid) + '|' + TRIM((STRING)LEFT.orgid) + '|' + TRIM((STRING)LEFT.seleid) + '|' + TRIM((STRING)LEFT.proxid) + '|' + TRIM((STRING)LEFT.powid) = RIGHT.KeyVal,TRANSFORM(__d0_Out,SELF.UID:=RIGHT.UID,SELF:=LEFT),SMART);
  SHARED __d0_Pow_Org__Layout := RECORD
    RECORDOF(__d0_UID_Mapped);
    KEL.typ.uid Pow_Org_;
  END;
  SHARED __d0_Missing_Pow_Org__UIDComponents := KEL.Intake.ConstructMissingFieldList(__d0_UID_Mapped,'ultid,orgid','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key');
  SHARED __d0_Pow_Org__Mapped := IF(__d0_Missing_Pow_Org__UIDComponents = 'ultid,orgid',PROJECT(__d0_UID_Mapped,TRANSFORM(__d0_Pow_Org__Layout,SELF.Pow_Org_:=0,SELF:=LEFT)),JOIN(KEL.Intake.AppendFields(__d0_UID_Mapped,__d0_Missing_Pow_Org__UIDComponents),E_Business_Org(__cfg).Lookup,TRIM((STRING)LEFT.ultid) + '|' + TRIM((STRING)LEFT.orgid) = RIGHT.KeyVal,TRANSFORM(__d0_Pow_Org__Layout,SELF.Pow_Org_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,SMART));
  EXPORT PublicRecords_KEL_Files_NonFCRA_BIPV2__Key_BH_Linking_Ids_Vault__Key_Invalid := __d0_Pow_Org__Mapped(UID = 0);
  ...
  EXPORT Pow_Org__Orphan := JOIN(InData(__NN(Pow_Org_)),E_Business_Org(__cfg).__Result,__EEQP(LEFT.Pow_Org_, RIGHT.UID),TRANSFORM(InLayout,SELF := LEFT,SELF:=[]),LEFT ONLY, HASH);
  EXPORT SanityCheck := DATASET([{COUNT(Pow_Org__Orphan),COUNT(PublicRecords_KEL_Files_NonFCRA_BIPV2__Key_BH_Linking_Ids_Vault__Key_Invalid),COUNT(Ult_I_D__SingleValue_Invalid),COUNT(Org_I_D__SingleValue_Invalid),COUNT(Sele_I_D__SingleValue_Invalid),COUNT(Prox_I_D__SingleValue_Invalid),COUNT(Pow_I_D__SingleValue_Invalid),COUNT(Pow_Org__SingleValue_Invalid),COUNT(Pow_Segment__SingleValue_Invalid),TopSourcedUIDs(1)}],{KEL.typ.int Pow_Org__Orphan,KEL.typ.int PublicRecords_KEL_Files_NonFCRA_BIPV2__Key_BH_Linking_Ids_Vault__Key_Invalid,KEL.typ.int Ult_I_D__SingleValue_Invalid,KEL.typ.int Org_I_D__SingleValue_Invalid,KEL.typ.int Sele_I_D__SingleValue_Invalid,KEL.typ.int Prox_I_D__SingleValue_Invalid,KEL.typ.int Pow_I_D__SingleValue_Invalid,KEL.typ.int Pow_Org__SingleValue_Invalid,KEL.typ.int Pow_Segment__SingleValue_Invalid,DATASET(RECORDOF(UIDSourceCounts)) topSourcedUID});
  EXPORT NullCounts := DATASET([
    {'BusinessPow','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key','UID',COUNT(PublicRecords_KEL_Files_NonFCRA_BIPV2__Key_BH_Linking_Ids_Vault__Key_Invalid),COUNT(__d0)},
    {'BusinessPow','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key','ultid',COUNT(__d0(__NL(Ult_I_D_))),COUNT(__d0(__NN(Ult_I_D_)))},
    {'BusinessPow','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key','orgid',COUNT(__d0(__NL(Org_I_D_))),COUNT(__d0(__NN(Org_I_D_)))},
    {'BusinessPow','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key','seleid',COUNT(__d0(__NL(Sele_I_D_))),COUNT(__d0(__NN(Sele_I_D_)))},
    {'BusinessPow','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key','proxid',COUNT(__d0(__NL(Prox_I_D_))),COUNT(__d0(__NN(Prox_I_D_)))},
    {'BusinessPow','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key','powid',COUNT(__d0(__NL(Pow_I_D_))),COUNT(__d0(__NN(Pow_I_D_)))},
    {'BusinessPow','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key','PowOrg',COUNT(__d0(__NL(Pow_Org_))),COUNT(__d0(__NN(Pow_Org_)))},
    {'BusinessPow','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key','pow_seg',COUNT(__d0(__NL(Pow_Segment_))),COUNT(__d0(__NN(Pow_Segment_)))},
    {'BusinessPow','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key','source',COUNT(__d0(__NL(Source_))),COUNT(__d0(__NN(Source_)))},
    {'BusinessPow','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key','Archive_Date',COUNT(__d0(Archive___Date_=0)),COUNT(__d0(Archive___Date_!=0))},
    {'BusinessPow','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key','DateFirstSeen',COUNT(__d0(Date_First_Seen_=0)),COUNT(__d0(Date_First_Seen_!=0))},
    {'BusinessPow','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key','DateLastSeen',COUNT(__d0(Date_Last_Seen_=0)),COUNT(__d0(Date_Last_Seen_!=0))},
    {'BusinessPow','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key','HybridArchiveDate',COUNT(__d0(Hybrid_Archive_Date_=0)),COUNT(__d0(Hybrid_Archive_Date_!=0))},
    {'BusinessPow','PublicRecords_KEL.Files.NonFCRA.BIPV2__Key_BH_Linking_Ids_Vault__Key','VaultDateLastSeen',COUNT(__d0(Vault_Date_Last_Seen_=0)),COUNT(__d0(Vault_Date_Last_Seen_!=0))}]

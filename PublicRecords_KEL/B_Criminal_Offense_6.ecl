...
EXPORT B_Criminal_Offense_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6198715 := __ENH_Criminal_Offense_7;
  EXPORT __ST276064_Layout := RECORD
  ...
  END;
  EXPORT __ST276017_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST276064_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST276017_Layout __ND6198635__Project(B_Criminal_Offense_7(__in,__cfg).__ST280514_Layout __PP6198083) := TRANSFORM
    __EE6198718 := __PP6198083.Offense_Charges_;
    __ST276064_Layout __ND6198593__Project(B_Criminal_Offense_7(__in,__cfg).__ST280561_Layout __PP6198244) := TRANSFORM
      __EE6198588 := __PP6198083.Offense_Charges_;
      SELF.Criminal_Date_ := KEL.Aggregates.MinNN(__EE6198588,KEL.Routines.CastStringToDate(__T(__EE6198588).Valid_Date_));
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP6198083.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP6198083.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP6198083.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP6198083.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP6198244;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE6198718,__ND6198593__Project(LEFT));
    SELF := __PP6198083;
  END;
  EXPORT __ENH_Criminal_Offense_6 := PROJECT(__EE6198715,__ND6198635__Project(LEFT));
END;

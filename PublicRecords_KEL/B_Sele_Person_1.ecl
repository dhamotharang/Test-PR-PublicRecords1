...
EXPORT B_Sele_Person_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7945671 := __ENH_Sele_Person_2;
  EXPORT __ST193000_Layout := RECORD
  ...
  END;
  SHARED __ST193000_Layout __ND7945820__Project(B_Sele_Person_2(__in,__cfg).__ST211232_Layout __PP7945672) := TRANSFORM
    __CC13680 := '-99997';
    SELF.Assoc_Valid_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP7945672.Assoc_Fs_Date_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP7945672.Assoc_Fs_Date_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13680)));
    SELF := __PP7945672;
  END;
  EXPORT __ENH_Sele_Person_1 := PROJECT(__EE7945671,__ND7945820__Project(LEFT));
END;

...
EXPORT B_Sele_Person_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8448662 := __ENH_Sele_Person_2;
  EXPORT __ST200745_Layout := RECORD
  ...
  END;
  SHARED __ST200745_Layout __ND8448811__Project(B_Sele_Person_2(__in,__cfg).__ST219404_Layout __PP8448663) := TRANSFORM
    __CC13819 := '-99997';
    SELF.Assoc_Valid_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP8448663.Assoc_Fs_Date_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP8448663.Assoc_Fs_Date_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13819)));
    SELF := __PP8448663;
  END;
  EXPORT __ENH_Sele_Person_1 := PROJECT(__EE8448662,__ND8448811__Project(LEFT));
END;

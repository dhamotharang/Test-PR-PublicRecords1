...
EXPORT B_Sele_Person_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8101170 := __ENH_Sele_Person_2;
  EXPORT __ST196954_Layout := RECORD
  ...
  END;
  SHARED __ST196954_Layout __ND8101319__Project(B_Sele_Person_2(__in,__cfg).__ST215103_Layout __PP8101171) := TRANSFORM
    __CC13936 := '-99997';
    SELF.Assoc_Valid_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP8101171.Assoc_Fs_Date_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP8101171.Assoc_Fs_Date_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13936)));
    SELF := __PP8101171;
  END;
  EXPORT __ENH_Sele_Person_1 := PROJECT(__EE8101170,__ND8101319__Project(LEFT));
END;

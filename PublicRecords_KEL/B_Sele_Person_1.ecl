...
EXPORT B_Sele_Person_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE9548247 := __ENH_Sele_Person_2;
  EXPORT __ST208800_Layout := RECORD
  ...
  END;
  SHARED __ST208800_Layout __ND9548396__Project(B_Sele_Person_2(__in,__cfg).__ST228015_Layout __PP9548248) := TRANSFORM
    __CC13887 := '-99997';
    SELF.Assoc_Valid_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP9548248.Assoc_Fs_Date_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP9548248.Assoc_Fs_Date_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13887)));
    SELF := __PP9548248;
  END;
  EXPORT __ENH_Sele_Person_1 := PROJECT(__EE9548247,__ND9548396__Project(LEFT));
END;

...
EXPORT B_Sele_Person_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8448632 := __ENH_Sele_Person_2;
  EXPORT __ST200715_Layout := RECORD
  ...
  END;
  SHARED __ST200715_Layout __ND8448781__Project(B_Sele_Person_2(__in,__cfg).__ST219374_Layout __PP8448633) := TRANSFORM
    __CC13789 := '-99997';
    SELF.Assoc_Valid_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP8448633.Assoc_Fs_Date_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP8448633.Assoc_Fs_Date_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13789)));
    SELF := __PP8448633;
  END;
  EXPORT __ENH_Sele_Person_1 := PROJECT(__EE8448632,__ND8448781__Project(LEFT));
END;

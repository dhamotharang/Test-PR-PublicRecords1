...
EXPORT B_Sele_Person_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE9710890 := __ENH_Sele_Person_2;
  EXPORT __ST212769_Layout := RECORD
  ...
  END;
  SHARED __ST212769_Layout __ND9711039__Project(B_Sele_Person_2(__in,__cfg).__ST231825_Layout __PP9710891) := TRANSFORM
    __CC13945 := '-99997';
    SELF.Assoc_Valid_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP9710891.Assoc_Fs_Date_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP9710891.Assoc_Fs_Date_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13945)));
    SELF := __PP9710891;
  END;
  EXPORT __ENH_Sele_Person_1 := PROJECT(__EE9710890,__ND9711039__Project(LEFT));
END;

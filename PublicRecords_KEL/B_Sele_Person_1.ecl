...
EXPORT B_Sele_Person_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE9600037 := __ENH_Sele_Person_2;
  EXPORT __ST211277_Layout := RECORD
  ...
  END;
  SHARED __ST211277_Layout __ND9600186__Project(B_Sele_Person_2(__in,__cfg).__ST230296_Layout __PP9600038) := TRANSFORM
    __CC13936 := '-99997';
    SELF.Assoc_Valid_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP9600038.Assoc_Fs_Date_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP9600038.Assoc_Fs_Date_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13936)));
    SELF := __PP9600038;
  END;
  EXPORT __ENH_Sele_Person_1 := PROJECT(__EE9600037,__ND9600186__Project(LEFT));
END;

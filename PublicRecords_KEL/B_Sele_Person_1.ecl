...
EXPORT B_Sele_Person_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE9484892 := __ENH_Sele_Person_2;
  EXPORT __ST210096_Layout := RECORD
  ...
  END;
  SHARED __ST210096_Layout __ND9485041__Project(B_Sele_Person_2(__in,__cfg).__ST229029_Layout __PP9484893) := TRANSFORM
    __CC13933 := '-99997';
    SELF.Assoc_Valid_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP9484893.Assoc_Fs_Date_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP9484893.Assoc_Fs_Date_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13933)));
    SELF := __PP9484893;
  END;
  EXPORT __ENH_Sele_Person_1 := PROJECT(__EE9484892,__ND9485041__Project(LEFT));
END;

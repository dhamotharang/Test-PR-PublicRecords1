...
EXPORT B_Sele_Person_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE10843529 := __ENH_Sele_Person_2;
  EXPORT __ST225558_Layout := RECORD
  ...
  END;
  SHARED __ST225558_Layout __ND10843678__Project(B_Sele_Person_2(__in,__cfg).__ST245857_Layout __PP10843530) := TRANSFORM
    __CC13953 := '-99997';
    SELF.Assoc_Valid_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP10843530.Assoc_Fs_Date_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP10843530.Assoc_Fs_Date_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13953)));
    SELF := __PP10843530;
  END;
  EXPORT __ENH_Sele_Person_1 := PROJECT(__EE10843529,__ND10843678__Project(LEFT));
END;

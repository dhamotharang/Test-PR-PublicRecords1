...
EXPORT E_Address(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __d3_KELfiltered := __in.Dataset_BIPV2__Key_BH_Linking_Ids((STRING10)prim_range != '' AND (STRING28)prim_name != '' AND (UNSIGNED3)zip != 0);
  ...
  SHARED __Mapping3 := 'UID(UID),prim_range(Primary_Range_),predir(Predirectional_),prim_name(Primary_Name_),addr_suffix(Suffix_),postdir(Postdirectional_),unit_desig(Unit_Designation_),sec_range(Secondary_Range_),p_city_name(Postal_City_),v_city_name(Vanity_City_),st(State_),zip(Z_I_P5_:0),zip4(Z_I_P4_),carrierroutenumber(Carrier_Route_Number_),carrierroutesortationatzip(Carrier_Route_Sortation_At_Z_I_P_),lineoftravel(Line_Of_Travel_),lineoftravelorder(Line_Of_Travel_Order_),deliverypointbarcode(Delivery_Point_Barcode_),deliverypointbarcodecheckdigit(Delivery_Point_Barcode_Check_Digit_),typecode(Type_Code_),county(County_),latitude(Latitude_),longitude(Longitude_),msa(Metropolitan_Statistical_Area_),geoblock(Geo_Block_),geomatch(Geo_Match_),acecleanererrorcode(A_C_E_Cleaner_Error_Code_),advodatefirstseen(A_D_V_O_Date_First_Seen_:DATE),advodatelastseen(A_D_V_O_Date_Last_Seen_:DATE),advodatevendorfirstreported(A_D_V_O_Date_Vendor_First_Reported_:DATE),advodatevendorlastreported(A_D_V_O_Date_Vendor_Last_Reported_:DATE),vacancyindicator(Vacancy_Indicator_),throwbackindicator(Throw_Back_Indicator_),seasonaldeliveryindicator(Seasonal_Delivery_Indicator_),seasonalstartsuppressiondate(Seasonal_Start_Suppression_Date_),seasonalendsuppressiondate(Seasonal_End_Suppression_Date_),donotdeliverindicator(Do_Not_Deliver_Indicator_),donotmailindicator(Do_Not_Mail_Indicator_),deadcoindicator(Dead_C_O_Indicator_),hotlistindicator(Hot_List_Indicator_),collegeindicator(College_Indicator_),collegestartsuppressiondate(College_Start_Suppression_Date_),collegeendsuppressiondate(College_End_Suppression_Date_),stylecode(Style_Code_),simplifycount(Simplify_Count_),dropindicator(Drop_Indicator_),residentialorbusinessindicator(Residential_Or_Business_Indicator_),onlywaytogetmailindicator(Only_Way_To_Get_Mail_Indicator_),recordtypecode(Record_Type_Code_),addresstypecode(Address_Type_Code_),mixedusagecode(Mixed_Usage_Code_),vacationbegindate(Vacation_Begin_Date_:DATE),vacationenddate(Vacation_End_Date_:DATE),numberofcurrentvacationmonths(Number_Of_Current_Vacation_Months_),maxvacationmonths(Max_Vacation_Months_),vacationperiodscount(Vacation_Periods_Count_),company_address_type_raw(Company_Address_Type_Raw_:\'\'),company_address_type_derived(Company_Address_Type_Derived_:\'\'),address_type_derived(Address_Type_Derived_:\'\'),addressmethod(Address_Method_:\'\'),isactive(Is_Active_:\'\'),isdefunct(Is_Defunct_:\'\'),address_type(Address___Type_:\'\'),address_desc(Address___Desc_:\'\'),source(Source_:\'\'),dt_last_seen_company_address(Date_First_Seen_:EPOCH),dt_first_seen_company_address(Date_Last_Seen_:EPOCH),DPMBitmap(__Permits:PERMITS)';
  SHARED __d3_Norm := NORMALIZE(__in,LEFT.Dataset_BIPV2__Key_BH_Linking_Ids,TRANSFORM(RECORDOF(__in.Dataset_BIPV2__Key_BH_Linking_Ids),SELF:=RIGHT));
  ...
  EXPORT PublicRecords_KEL_ECL_Functions_Dataset_FDC_Dataset_BIPV2__Key_BH_Linking_Ids_Invalid := __d3_UID_Mapped(UID = 0);
  ...
  Layout Address__Rollup(InLayout __r, DATASET(InLayout) __recs) := TRANSFORM
    ...
    SELF.Business_Characteristics_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Company_Address_Type_Raw_,Company_Address_Type_Derived_,Address_Type_Derived_,Address___Type_,Address___Desc_},Company_Address_Type_Raw_,Company_Address_Type_Derived_,Address_Type_Derived_,Address___Type_,Address___Desc_),Business_Characteristics_Layout)(__NN(Company_Address_Type_Raw_) OR __NN(Company_Address_Type_Derived_) OR __NN(Address_Type_Derived_) OR __NN(Address___Type_) OR __NN(Address___Desc_)));
    SELF.B_I_P_V2_Best_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Address_Method_,Is_Active_,Is_Defunct_},Address_Method_,Is_Active_,Is_Defunct_),B_I_P_V2_Best_Layout)(__NN(Address_Method_) OR __NN(Is_Active_) OR __NN(Is_Defunct_)));
    ...
    SELF.Do_Not_Mail_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Do_Not_Mail_Indicator_},Do_Not_Mail_Indicator_),Do_Not_Mail_Layout)(__NN(Do_Not_Mail_Indicator_)));
    SELF.Dead_C_O_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Dead_C_O_Indicator_},Dead_C_O_Indicator_),Dead_C_O_Layout)(__NN(Dead_C_O_Indicator_)));
    SELF.Hot_List_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Hot_List_Indicator_},Hot_List_Indicator_),Hot_List_Layout)(__NN(Hot_List_Indicator_)));
  ...
  Layout Address__Single_Rollup(InLayout __r) := TRANSFORM
    ...
    SELF.Business_Characteristics_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Business_Characteristics_Layout,SELF.__RecordCount:=1;,SELF:=LEFT))(__NN(Company_Address_Type_Raw_) OR __NN(Company_Address_Type_Derived_) OR __NN(Address_Type_Derived_) OR __NN(Address___Type_) OR __NN(Address___Desc_)));
    SELF.B_I_P_V2_Best_ := __CN(PROJECT(DATASET(__r),TRANSFORM(B_I_P_V2_Best_Layout,SELF.__RecordCount:=1;,SELF:=LEFT))(__NN(Address_Method_) OR __NN(Is_Active_) OR __NN(Is_Defunct_)));
    ...
    SELF.Do_Not_Mail_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Do_Not_Mail_Layout,SELF.__RecordCount:=1;,SELF:=LEFT))(__NN(Do_Not_Mail_Indicator_)));
    SELF.Dead_C_O_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Dead_C_O_Layout,SELF.__RecordCount:=1;,SELF:=LEFT))(__NN(Dead_C_O_Indicator_)));
    SELF.Hot_List_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Hot_List_Layout,SELF.__RecordCount:=1;,SELF:=LEFT))(__NN(Hot_List_Indicator_)));
  ...
  EXPORT NullCounts := DATASET([
    ...
    {'Address','PublicRecords_KEL.ECL_Functions.Dataset_FDC','UID',COUNT(PublicRecords_KEL_ECL_Functions_Dataset_FDC_Dataset_Doxie_Files__Key_Offenders_Invalid),COUNT(__d2)},
    ...
    {'Address','PublicRecords_KEL.ECL_Functions.Dataset_FDC','UID',COUNT(PublicRecords_KEL_ECL_Functions_Dataset_FDC_Dataset_BIPV2__Key_BH_Linking_Ids_Invalid),COUNT(__d3)},
    ...
    {'Address','PublicRecords_KEL.ECL_Functions.Dataset_FDC','company_address_type_raw',COUNT(__d3(__NL(Company_Address_Type_Raw_))),COUNT(__d3(__NN(Company_Address_Type_Raw_)))},
    {'Address','PublicRecords_KEL.ECL_Functions.Dataset_FDC','company_address_type_derived',COUNT(__d3(__NL(Company_Address_Type_Derived_))),COUNT(__d3(__NN(Company_Address_Type_Derived_)))},
    {'Address','PublicRecords_KEL.ECL_Functions.Dataset_FDC','address_type_derived',COUNT(__d3(__NL(Address_Type_Derived_))),COUNT(__d3(__NN(Address_Type_Derived_)))},

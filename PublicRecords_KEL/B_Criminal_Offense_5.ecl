...
EXPORT B_Criminal_Offense_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5614002 := __ENH_Criminal_Offense_6;
  EXPORT __ST254143_Layout := RECORD
  ...
  EXPORT __ST254096_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST254143_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST254096_Layout __ND5613922__Project(B_Criminal_Offense_6(__in,__cfg).__ST260045_Layout __PP5613370) := TRANSFORM
    __EE5614005 := __PP5613370.Offense_Charges_;
    __ST254143_Layout __ND5613880__Project(B_Criminal_Offense_6(__in,__cfg).__ST260092_Layout __PP5613531) := TRANSFORM
      __EE5613875 := __PP5613370.Offense_Charges_;
      SELF.Criminal_Date_ := KEL.Aggregates.MinNN(__EE5613875,KEL.Routines.CastStringToDate(__T(__EE5613875).Valid_Date_));
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP5613370.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP5613370.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP5613370.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP5613370.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP5613531;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE5614005,__ND5613880__Project(LEFT));
    SELF := __PP5613370;
  END;
  EXPORT __ENH_Criminal_Offense_5 := PROJECT(__EE5614002,__ND5613922__Project(LEFT));
END;

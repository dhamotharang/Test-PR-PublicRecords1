...
EXPORT B_Criminal_Offense_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5634369 := __ENH_Criminal_Offense_6;
  EXPORT __ST254445_Layout := RECORD
  ...
  END;
  EXPORT __ST254398_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST254445_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST254398_Layout __ND5634289__Project(B_Criminal_Offense_6(__in,__cfg).__ST260347_Layout __PP5633737) := TRANSFORM
    __EE5634372 := __PP5633737.Offense_Charges_;
    __ST254445_Layout __ND5634247__Project(B_Criminal_Offense_6(__in,__cfg).__ST260394_Layout __PP5633898) := TRANSFORM
      __EE5634242 := __PP5633737.Offense_Charges_;
      SELF.Criminal_Date_ := KEL.Aggregates.MinNN(__EE5634242,KEL.Routines.CastStringToDate(__T(__EE5634242).Valid_Date_));
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP5633737.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP5633737.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP5633737.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP5633737.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP5633898;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE5634372,__ND5634247__Project(LEFT));
    SELF := __PP5633737;
  END;
  EXPORT __ENH_Criminal_Offense_5 := PROJECT(__EE5634369,__ND5634289__Project(LEFT));
END;

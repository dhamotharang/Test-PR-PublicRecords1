...
EXPORT B_Criminal_Offense_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE488276 := __E_Criminal_Offense;
  ...
  END;
  EXPORT __ST238499_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST238546_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST238499_Layout __ND4923500__Project(E_Criminal_Offense(__in,__cfg).Layout __PP488656) := TRANSFORM
    __EE488078 := __PP488656.Offense_Charges_;
    __ST238546_Layout __ND4923487__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP488658) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP488658.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP488656.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP488656.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP488656.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP488656.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP488658;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE488078,__ND4923487__Project(LEFT));
    SELF := __PP488656;
  END;
  EXPORT __ENH_Criminal_Offense_5 := PROJECT(__EE488276,__ND4923500__Project(LEFT));
END;

...
EXPORT B_Criminal_Offense_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5554870 := __ENH_Criminal_Offense_6;
  EXPORT __ST251360_Layout := RECORD
  ...
  END;
  EXPORT __ST251313_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST251360_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST251313_Layout __ND5554790__Project(B_Criminal_Offense_6(__in,__cfg).__ST257262_Layout __PP5554238) := TRANSFORM
    __EE5554873 := __PP5554238.Offense_Charges_;
    __ST251360_Layout __ND5554748__Project(B_Criminal_Offense_6(__in,__cfg).__ST257309_Layout __PP5554399) := TRANSFORM
      __EE5554743 := __PP5554238.Offense_Charges_;
      SELF.Criminal_Date_ := KEL.Aggregates.MinNN(__EE5554743,KEL.Routines.CastStringToDate(__T(__EE5554743).Valid_Date_));
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP5554238.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP5554238.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP5554238.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP5554238.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP5554399;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE5554873,__ND5554748__Project(LEFT));
    SELF := __PP5554238;
  END;
  EXPORT __ENH_Criminal_Offense_5 := PROJECT(__EE5554870,__ND5554790__Project(LEFT));
END;

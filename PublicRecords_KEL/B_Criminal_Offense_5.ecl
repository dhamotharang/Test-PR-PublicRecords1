...
EXPORT B_Criminal_Offense_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE488956 := __E_Criminal_Offense;
  ...
  END;
  EXPORT __ST239179_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST239226_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST239179_Layout __ND4925784__Project(E_Criminal_Offense(__in,__cfg).Layout __PP489336) := TRANSFORM
    __EE488758 := __PP489336.Offense_Charges_;
    __ST239226_Layout __ND4925771__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP489338) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP489338.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP489336.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP489336.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP489336.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP489336.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP489338;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE488758,__ND4925771__Project(LEFT));
    SELF := __PP489336;
  END;
  EXPORT __ENH_Criminal_Offense_5 := PROJECT(__EE488956,__ND4925784__Project(LEFT));
END;

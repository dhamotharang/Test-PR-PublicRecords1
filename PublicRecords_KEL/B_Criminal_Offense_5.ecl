...
EXPORT B_Criminal_Offense_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE497076 := __E_Criminal_Offense;
  EXPORT __ST243018_Layout := RECORD
  ...
  END;
  EXPORT __ST242971_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST243018_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST242971_Layout __ND4767130__Project(E_Criminal_Offense(__in,__cfg).Layout __PP497456) := TRANSFORM
    __EE496878 := __PP497456.Offense_Charges_;
    __ST243018_Layout __ND4767117__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP497458) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP497458.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP497456.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP497456.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP497456.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP497456.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP497458;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE496878,__ND4767117__Project(LEFT));
    SELF := __PP497456;
  END;
  EXPORT __ENH_Criminal_Offense_5 := PROJECT(__EE497076,__ND4767130__Project(LEFT));
END;

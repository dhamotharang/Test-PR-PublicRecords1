...
EXPORT B_Criminal_Offense_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE488555 := __E_Criminal_Offense;
  EXPORT __ST238738_Layout := RECORD
  ...
  END;
  EXPORT __ST238691_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST238738_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST238691_Layout __ND4922813__Project(E_Criminal_Offense(__in,__cfg).Layout __PP488935) := TRANSFORM
    __EE488357 := __PP488935.Offense_Charges_;
    __ST238738_Layout __ND4922800__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP488937) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP488937.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP488935.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP488935.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP488935.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP488935.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP488937;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE488357,__ND4922800__Project(LEFT));
    SELF := __PP488935;
  END;
  EXPORT __ENH_Criminal_Offense_5 := PROJECT(__EE488555,__ND4922813__Project(LEFT));
END;

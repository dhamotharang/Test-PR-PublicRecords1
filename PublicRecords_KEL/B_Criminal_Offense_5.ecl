...
EXPORT B_Criminal_Offense_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE571644 := __E_Criminal_Offense;
  EXPORT __ST261118_Layout := RECORD
  ...
  END;
  EXPORT __ST261071_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST261118_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST261071_Layout __ND5685089__Project(E_Criminal_Offense(__in,__cfg).Layout __PP572024) := TRANSFORM
    __EE571446 := __PP572024.Offense_Charges_;
    __ST261118_Layout __ND5685076__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP572026) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP572026.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP572024.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP572024.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP572024.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP572024.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP572026;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE571446,__ND5685076__Project(LEFT));
    SELF := __PP572024;
  END;
  EXPORT __ENH_Criminal_Offense_5 := PROJECT(__EE571644,__ND5685089__Project(LEFT));
END;

...
EXPORT B_Criminal_Offense_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE569458 := __E_Criminal_Offense;
  ...
  END;
  EXPORT __ST259469_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST259516_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST259469_Layout __ND5627142__Project(E_Criminal_Offense(__in,__cfg).Layout __PP569838) := TRANSFORM
    __EE569260 := __PP569838.Offense_Charges_;
    __ST259516_Layout __ND5627129__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP569840) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP569840.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP569838.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP569838.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP569838.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP569838.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP569840;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE569260,__ND5627129__Project(LEFT));
    SELF := __PP569838;
  END;
  EXPORT __ENH_Criminal_Offense_5 := PROJECT(__EE569458,__ND5627142__Project(LEFT));
END;

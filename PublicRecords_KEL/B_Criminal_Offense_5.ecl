...
EXPORT B_Criminal_Offense_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE488951 := __E_Criminal_Offense;
  EXPORT __ST239221_Layout := RECORD
  ...
  END;
  EXPORT __ST239174_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST239221_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST239174_Layout __ND4925779__Project(E_Criminal_Offense(__in,__cfg).Layout __PP489331) := TRANSFORM
    __EE488753 := __PP489331.Offense_Charges_;
    __ST239221_Layout __ND4925766__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP489333) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP489333.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP489331.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP489331.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP489331.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP489331.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP489333;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE488753,__ND4925766__Project(LEFT));
    SELF := __PP489331;
  END;
  EXPORT __ENH_Criminal_Offense_5 := PROJECT(__EE488951,__ND4925779__Project(LEFT));
END;

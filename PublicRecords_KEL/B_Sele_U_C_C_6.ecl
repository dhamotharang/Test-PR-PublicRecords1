...
EXPORT B_Sele_U_C_C_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5328099 := __ENH_Sele_U_C_C_7;
  ...
  EXPORT __ST253891_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST253891_Layout __ND5328204__Project(B_Sele_U_C_C_7(__in,__cfg).__ST257043_Layout __PP5328100) := TRANSFORM
    __EE5328135 := __PP5328100.Sub_Filing_;
    SELF.Sub_Filing_ := __BN(PROJECT(__T(__EE5328135),__ST253899_Layout),__NL(__EE5328135));
    SELF.Is_Filing_ := IF(__T(__OP2(__FN1(KEL.Routines.ToUpperCase,__FN1(TRIM,__PP5328100.Filtered_Party_Type_)),IN,__CN(['D','S','C','A']))),TRUE,FALSE);
    SELF := __PP5328100;
  END;
  EXPORT __ENH_Sele_U_C_C_6 := PROJECT(__EE5328099,__ND5328204__Project(LEFT));
END;

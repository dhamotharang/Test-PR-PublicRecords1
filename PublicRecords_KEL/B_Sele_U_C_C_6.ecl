...
EXPORT B_Sele_U_C_C_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5511461 := __ENH_Sele_U_C_C_7;
  EXPORT __ST260224_Layout := RECORD
  ...
  END;
  EXPORT __ST260216_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST260224_Layout) Sub_Filing_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST260216_Layout __ND5511566__Project(B_Sele_U_C_C_7(__in,__cfg).__ST263546_Layout __PP5511462) := TRANSFORM
    __EE5511497 := __PP5511462.Sub_Filing_;
    SELF.Sub_Filing_ := __BN(PROJECT(__T(__EE5511497),__ST260224_Layout),__NL(__EE5511497));
    SELF.Is_Filing_ := IF(__T(__OP2(__FN1(KEL.Routines.ToUpperCase,__FN1(TRIM,__PP5511462.Filtered_Party_Type_)),IN,__CN(['D','S','C','A']))),TRUE,FALSE);
    SELF := __PP5511462;
  END;
  EXPORT __ENH_Sele_U_C_C_6 := PROJECT(__EE5511461,__ND5511566__Project(LEFT));
END;

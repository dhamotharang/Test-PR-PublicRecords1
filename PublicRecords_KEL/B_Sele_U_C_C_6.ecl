...
EXPORT B_Sele_U_C_C_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4880637 := __ENH_Sele_U_C_C_7;
  EXPORT __ST246694_Layout := RECORD
  ...
  END;
  EXPORT __ST246686_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST246694_Layout) Sub_Filing_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST246686_Layout __ND4880742__Project(B_Sele_U_C_C_7(__in,__cfg).__ST249590_Layout __PP4880638) := TRANSFORM
    __EE4880673 := __PP4880638.Sub_Filing_;
    SELF.Sub_Filing_ := __BN(PROJECT(__T(__EE4880673),__ST246694_Layout),__NL(__EE4880673));
    SELF.Is_Filing_ := IF(__T(__OP2(__FN1(KEL.Routines.ToUpperCase,__FN1(TRIM,__PP4880638.Filtered_Party_Type_)),IN,__CN(['D','S','C','A']))),TRUE,FALSE);
    SELF := __PP4880638;
  END;
  EXPORT __ENH_Sele_U_C_C_6 := PROJECT(__EE4880637,__ND4880742__Project(LEFT));
END;

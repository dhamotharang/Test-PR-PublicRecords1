...
EXPORT B_Sele_U_C_C_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5355080 := __ENH_Sele_U_C_C_7;
  EXPORT __ST255310_Layout := RECORD
  ...
  END;
  EXPORT __ST255302_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST255310_Layout) Sub_Filing_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST255302_Layout __ND5355185__Project(B_Sele_U_C_C_7(__in,__cfg).__ST258454_Layout __PP5355081) := TRANSFORM
    __EE5355116 := __PP5355081.Sub_Filing_;
    SELF.Sub_Filing_ := __BN(PROJECT(__T(__EE5355116),__ST255310_Layout),__NL(__EE5355116));
    SELF.Is_Filing_ := IF(__T(__OP2(__FN1(KEL.Routines.ToUpperCase,__FN1(TRIM,__PP5355081.Filtered_Party_Type_)),IN,__CN(['D','S','C','A']))),TRUE,FALSE);
    SELF := __PP5355081;
  END;
  EXPORT __ENH_Sele_U_C_C_6 := PROJECT(__EE5355080,__ND5355185__Project(LEFT));
END;

...
EXPORT B_Sele_U_C_C_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5161485 := __ENH_Sele_U_C_C_7;
  EXPORT __ST256615_Layout := RECORD
  ...
  EXPORT __ST256607_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST256615_Layout) Sub_Filing_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST256607_Layout __ND5161590__Project(B_Sele_U_C_C_7(__in,__cfg).__ST259852_Layout __PP5161486) := TRANSFORM
    __EE5161521 := __PP5161486.Sub_Filing_;
    SELF.Sub_Filing_ := __BN(PROJECT(__T(__EE5161521),__ST256615_Layout),__NL(__EE5161521));
    SELF.Is_Filing_ := IF(__T(__OP2(__FN1(KEL.Routines.ToUpperCase,__FN1(TRIM,__PP5161486.Filtered_Party_Type_)),IN,__CN(['D','S','C','A']))),TRUE,FALSE);
    SELF := __PP5161486;
  END;
  EXPORT __ENH_Sele_U_C_C_6 := PROJECT(__EE5161485,__ND5161590__Project(LEFT));
END;

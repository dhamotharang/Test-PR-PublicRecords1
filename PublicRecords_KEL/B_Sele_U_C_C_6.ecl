...
EXPORT B_Sele_U_C_C_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5717656 := __ENH_Sele_U_C_C_7;
  EXPORT __ST266195_Layout := RECORD
  ...
  END;
  EXPORT __ST266187_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST266195_Layout) Sub_Filing_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST266187_Layout __ND5717761__Project(B_Sele_U_C_C_7(__in,__cfg).__ST269517_Layout __PP5717657) := TRANSFORM
    __EE5717692 := __PP5717657.Sub_Filing_;
    SELF.Sub_Filing_ := __BN(PROJECT(__T(__EE5717692),__ST266195_Layout),__NL(__EE5717692));
    SELF.Is_Filing_ := IF(__T(__OP2(__FN1(KEL.Routines.ToUpperCase,__FN1(TRIM,__PP5717657.Filtered_Party_Type_)),IN,__CN(['D','S','C','A']))),TRUE,FALSE);
    SELF := __PP5717657;
  END;
  EXPORT __ENH_Sele_U_C_C_6 := PROJECT(__EE5717656,__ND5717761__Project(LEFT));
END;

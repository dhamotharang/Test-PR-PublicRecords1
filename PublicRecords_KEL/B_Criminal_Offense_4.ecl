...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE28446 := __E_Criminal_Offense;
  EXPORT __ST25783_Layout := RECORD
  ...
  END;
  EXPORT __ST25736_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST25783_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST25736_Layout __ND28371__Project(E_Criminal_Offense(__in,__cfg).Layout __PP26870) := TRANSFORM
    __EE28449 := __PP26870.Offense_Charges_;
    __ST25783_Layout __ND28260__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP27734) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP27734.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP26870.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP26870.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP26870.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP26870.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP27734;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE28449,__ND28260__Project(LEFT));
    ...
    __EE28363 := __PP26870.Offender_Level_;
    __BS28350 := __T(__EE28363);
    __EE28367 := __BN(TOPN(__BS28350(__NN(KEL.era.ToDate(__T(__EE28363).Date_First_Seen_))),1, -__T(KEL.era.ToDate(__T(__EE28363).Date_First_Seen_)),__T(Offender_Level_)),__NL(__EE28363));
    SELF.Recent_Offender_Level_ := (__T(__EE28367))[1].Offender_Level_;
    SELF := __PP26870;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE28446,__ND28371__Project(LEFT));
END;

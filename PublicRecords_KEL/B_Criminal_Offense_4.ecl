...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE21060 := __E_Criminal_Offense;
  EXPORT __ST19369_Layout := RECORD
  ...
  END;
  EXPORT __ST19322_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST19369_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST19322_Layout __ND20985__Project(E_Criminal_Offense(__in,__cfg).Layout __PP19484) := TRANSFORM
    __EE21063 := __PP19484.Offense_Charges_;
    __ST19369_Layout __ND20874__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP20348) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP20348.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP19484.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP19484.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP19484.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP19484.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP20348;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE21063,__ND20874__Project(LEFT));
    ...
    __EE20977 := __PP19484.Offender_Level_;
    __BS20964 := __T(__EE20977);
    __EE20981 := __BN(TOPN(__BS20964(__NN(KEL.era.ToDate(__T(__EE20977).Date_First_Seen_))),1, -__T(KEL.era.ToDate(__T(__EE20977).Date_First_Seen_)),__T(Offender_Level_)),__NL(__EE20977));
    SELF.Recent_Offender_Level_ := (__T(__EE20981))[1].Offender_Level_;
    SELF := __PP19484;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE21060,__ND20985__Project(LEFT));
END;

...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE22892 := __E_Criminal_Offense;
  EXPORT __ST20996_Layout := RECORD
  ...
  EXPORT __ST20949_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST20996_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST20949_Layout __ND22817__Project(E_Criminal_Offense(__in,__cfg).Layout __PP21316) := TRANSFORM
    __EE22895 := __PP21316.Offense_Charges_;
    __ST20996_Layout __ND22706__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP22180) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP22180.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP21316.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP21316.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP21316.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP21316.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP22180;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE22895,__ND22706__Project(LEFT));
    ...
    __EE22809 := __PP21316.Offender_Level_;
    __BS22796 := __T(__EE22809);
    __EE22813 := __BN(TOPN(__BS22796(__NN(KEL.era.ToDate(__T(__EE22809).Date_First_Seen_))),1, -__T(KEL.era.ToDate(__T(__EE22809).Date_First_Seen_)),__T(Offender_Level_)),__NL(__EE22809));
    SELF.Recent_Offender_Level_ := (__T(__EE22813))[1].Offender_Level_;
    SELF := __PP21316;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE22892,__ND22817__Project(LEFT));
END;

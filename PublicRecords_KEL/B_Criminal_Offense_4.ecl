...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE49219 := __E_Criminal_Offense;
  EXPORT __ST38448_Layout := RECORD
  ...
  END;
  EXPORT __ST38401_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST38448_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST38401_Layout __ND49144__Project(E_Criminal_Offense(__in,__cfg).Layout __PP47643) := TRANSFORM
    __EE49222 := __PP47643.Offense_Charges_;
    __ST38448_Layout __ND49033__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP48507) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP48507.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP47643.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP47643.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP47643.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP47643.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP48507;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE49222,__ND49033__Project(LEFT));
    ...
    __EE49136 := __PP47643.Offender_Level_;
    __BS49123 := __T(__EE49136);
    __EE49140 := __BN(TOPN(__BS49123(__NN(KEL.era.ToDate(__T(__EE49136).Date_First_Seen_))),1, -__T(KEL.era.ToDate(__T(__EE49136).Date_First_Seen_)),__T(Offender_Level_)),__NL(__EE49136));
    SELF.Recent_Offender_Level_ := (__T(__EE49140))[1].Offender_Level_;
    SELF := __PP47643;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE49219,__ND49144__Project(LEFT));
END;

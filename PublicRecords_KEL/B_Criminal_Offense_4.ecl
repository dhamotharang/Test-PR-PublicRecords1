...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE400612 := __E_Criminal_Offense;
  EXPORT __ST139534_Layout := RECORD
  ...
  END;
  EXPORT __ST139487_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST139534_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST139487_Layout __ND401317__Project(E_Criminal_Offense(__in,__cfg).Layout __PP400878) := TRANSFORM
    __EE400414 := __PP400878.Offense_Charges_;
    __ST139534_Layout __ND401304__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP400880) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP400880.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP400878.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP400878.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP400878.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP400878.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP400880;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE400414,__ND401304__Project(LEFT));
    SELF := __PP400878;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE400612,__ND401317__Project(LEFT));
END;

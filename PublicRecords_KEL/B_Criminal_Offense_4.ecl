...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE155821 := __E_Criminal_Offense;
  ...
  END;
  EXPORT __ST68963_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST69010_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST68963_Layout __ND155746__Project(E_Criminal_Offense(__in,__cfg).Layout __PP154245) := TRANSFORM
    __EE155824 := __PP154245.Offense_Charges_;
    __ST69010_Layout __ND155635__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP155109) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP155109.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP154245.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP154245.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP154245.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP154245.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP155109;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE155824,__ND155635__Project(LEFT));
    ...
    __EE155738 := __PP154245.Offender_Level_;
    __BS155725 := __T(__EE155738);
    __EE155742 := __BN(TOPN(__BS155725(__NN(KEL.era.ToDate(__T(__EE155738).Date_First_Seen_))),1, -__T(KEL.era.ToDate(__T(__EE155738).Date_First_Seen_)),__T(Offender_Level_)),__NL(__EE155738));
    SELF.Recent_Offender_Level_ := (__T(__EE155742))[1].Offender_Level_;
    SELF := __PP154245;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE155821,__ND155746__Project(LEFT));
END;

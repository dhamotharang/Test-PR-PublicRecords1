...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE667793 := __ENH_Criminal_Offense_5;
  EXPORT __ST153060_Layout := RECORD
  ...
  END;
  EXPORT __ST153013_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST153060_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST153013_Layout __ND668447__Project(B_Criminal_Offense_5(__in,__cfg).__ST158388_Layout __PP667345) := TRANSFORM
    __EE667570 := __PP667345.Offense_Charges_;
    __ST153060_Layout __ND668427__Project(B_Criminal_Offense_5(__in,__cfg).__ST158435_Layout __PP667797) := TRANSFORM
      ...
      SELF.Age_In_Days_ := FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP667797.Criminal_Date_),__ECAST(KEL.typ.nkdate,__CC9472));
      SELF.Is_Felony_ := __AND(__AND(__NOT(__PP667797.Dismissed_Charges_),IF(__T(__NT(__PP667797.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP667797.Offense_Score_,=,__CN('F'))))),__OR(__OP2(__PP667797.Offender_Level_,=,__CN('4')),__NT(__PP667797.Offender_Level_)));
      SELF.Is_Non_Felony_Records_ := __AND(__NOT(__PP667797.Dismissed_Charges_),__NOT(__OP2(__PP667797.Offense_Score_,=,__CN('F'))));
      SELF := __PP667797;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE667570,__ND668427__Project(LEFT));
    SELF := __PP667345;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE667793,__ND668447__Project(LEFT));
END;

...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5713230 := __ENH_Criminal_Offense_5;
  EXPORT __ST243610_Layout := RECORD
  ...
  END;
  EXPORT __ST243563_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST243610_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST243563_Layout __ND5713665__Project(B_Criminal_Offense_5(__in,__cfg).__ST251283_Layout __PP5713231) := TRANSFORM
    __EE5713391 := __PP5713231.Offense_Charges_;
    __ST243610_Layout __ND5713642__Project(B_Criminal_Offense_5(__in,__cfg).__ST251330_Layout __PP5713392) := TRANSFORM
      ...
      SELF.Age_In_Days_ := FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP5713392.Criminal_Date_),__ECAST(KEL.typ.nkdate,__CC13219));
      SELF.Is_Criminal_Count_ := __NOT(__PP5713392.Dismissed_Charges_);
      SELF.Is_Felony_Non_F_C_R_A_ := __AND(__AND(__NOT(__PP5713392.Dismissed_Charges_),IF(__T(__NT(__PP5713392.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP5713392.Offense_Score_,=,__CN('F'))))),__OP2(__PP5713392.Offender_Level_,=,__CN('4')));
      SELF.Is_Non_Felony_Records_ := __AND(__NOT(__PP5713392.Dismissed_Charges_),__NOT(__OP2(__PP5713392.Offense_Score_,=,__CN('F'))));
      SELF := __PP5713392;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE5713391,__ND5713642__Project(LEFT));
    SELF := __PP5713231;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE5713230,__ND5713665__Project(LEFT));
END;

...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE218997 := __E_Criminal_Offense;
  EXPORT __ST92438_Layout := RECORD
  ...
  EXPORT __ST92391_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST92438_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST92391_Layout __ND218922__Project(E_Criminal_Offense(__in,__cfg).Layout __PP218276) := TRANSFORM
    __EE219000 := __PP218276.Offense_Charges_;
    __ST92438_Layout __ND218803__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP218278) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP218278.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP218276.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP218276.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP218276.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP218276.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP218278;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE219000,__ND218803__Project(LEFT));
    ...
    __EE218913 := __PP218276.Offender_Level_;
    __BS218900 := __T(__EE218913);
    __EE218917 := __BN(TOPN(__BS218900(__NN(KEL.era.ToDate(__T(__EE218913).Date_First_Seen_))),1, -__T(KEL.era.ToDate(__T(__EE218913).Date_First_Seen_)),__T(Offender_Level_)),__NL(__EE218913));
    SELF.Recent_Offender_Level_ := (__T(__EE218917))[1].Offender_Level_;
    SELF := __PP218276;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE218997,__ND218922__Project(LEFT));
END;

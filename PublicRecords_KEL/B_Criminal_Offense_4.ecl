...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE41476 := __E_Criminal_Offense;
  EXPORT __ST33229_Layout := RECORD
  ...
  EXPORT __ST33182_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST33229_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST33182_Layout __ND41401__Project(E_Criminal_Offense(__in,__cfg).Layout __PP39900) := TRANSFORM
    __EE41479 := __PP39900.Offense_Charges_;
    __ST33229_Layout __ND41290__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP40764) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP40764.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP39900.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP39900.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP39900.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP39900.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP40764;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE41479,__ND41290__Project(LEFT));
    ...
    __EE41393 := __PP39900.Offender_Level_;
    __BS41380 := __T(__EE41393);
    __EE41397 := __BN(TOPN(__BS41380(__NN(KEL.era.ToDate(__T(__EE41393).Date_First_Seen_))),1, -__T(KEL.era.ToDate(__T(__EE41393).Date_First_Seen_)),__T(Offender_Level_)),__NL(__EE41393));
    SELF.Recent_Offender_Level_ := (__T(__EE41397))[1].Offender_Level_;
    SELF := __PP39900;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE41476,__ND41401__Project(LEFT));
END;

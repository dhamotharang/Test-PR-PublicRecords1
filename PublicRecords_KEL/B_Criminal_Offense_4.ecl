...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE42121 := __E_Criminal_Offense;
  EXPORT __ST33834_Layout := RECORD
  ...
  END;
  EXPORT __ST33787_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST33834_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST33787_Layout __ND42046__Project(E_Criminal_Offense(__in,__cfg).Layout __PP40545) := TRANSFORM
    __EE42124 := __PP40545.Offense_Charges_;
    __ST33834_Layout __ND41935__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP41409) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP41409.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP40545.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP40545.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP40545.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP40545.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP41409;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE42124,__ND41935__Project(LEFT));
    ...
    __EE42038 := __PP40545.Offender_Level_;
    __BS42025 := __T(__EE42038);
    __EE42042 := __BN(TOPN(__BS42025(__NN(KEL.era.ToDate(__T(__EE42038).Date_First_Seen_))),1, -__T(KEL.era.ToDate(__T(__EE42038).Date_First_Seen_)),__T(Offender_Level_)),__NL(__EE42038));
    SELF.Recent_Offender_Level_ := (__T(__EE42042))[1].Offender_Level_;
    SELF := __PP40545;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE42121,__ND42046__Project(LEFT));
END;

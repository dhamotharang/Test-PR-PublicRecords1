...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5791143 := __ENH_Criminal_Offense_5;
  EXPORT __ST246724_Layout := RECORD
  ...
  END;
  EXPORT __ST246677_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST246724_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST246677_Layout __ND5791605__Project(B_Criminal_Offense_5(__in,__cfg).__ST254398_Layout __PP5791144) := TRANSFORM
    __EE5791304 := __PP5791144.Offense_Charges_;
    __ST246724_Layout __ND5791573__Project(B_Criminal_Offense_5(__in,__cfg).__ST254445_Layout __PP5791305) := TRANSFORM
      ...
      SELF.Age_In_Days_ := FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP5791305.Criminal_Date_),__ECAST(KEL.typ.nkdate,__CC13219));
      SELF.Is_Criminal_Count_F_C_R_A_ := __AND(__AND(__AND(__NOT(__PP5791305.Dismissed_Charges_),__OP2(__PP5791305.Offense_Score_,IN,__CN(['M','F']))),__OP2(__PP5791305.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP5791305.Traffic_Flag_,=,__CN('N')));
      SELF.Is_Criminal_Count_Non_F_C_R_A_ := __NOT(__PP5791305.Dismissed_Charges_);
      SELF.Is_Felony_Non_F_C_R_A_ := __AND(__AND(__NOT(__PP5791305.Dismissed_Charges_),IF(__T(__NT(__PP5791305.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP5791305.Offense_Score_,=,__CN('F'))))),__OP2(__PP5791305.Offender_Level_,=,__CN('4')));
      SELF.Is_Non_Felony_Records_Non_F_C_R_A_ := __AND(__NOT(__PP5791305.Dismissed_Charges_),__NOT(__OP2(__PP5791305.Offense_Score_,=,__CN('F'))));
      SELF := __PP5791305;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE5791304,__ND5791573__Project(LEFT));
    SELF := __PP5791144;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE5791143,__ND5791605__Project(LEFT));
END;

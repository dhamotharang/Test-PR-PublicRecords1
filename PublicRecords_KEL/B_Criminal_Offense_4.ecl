...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE21055 := __E_Criminal_Offense;
  EXPORT __ST19364_Layout := RECORD
  ...
  END;
  EXPORT __ST19317_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST19364_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST19317_Layout __ND20980__Project(E_Criminal_Offense(__in,__cfg).Layout __PP19479) := TRANSFORM
    __EE21058 := __PP19479.Offense_Charges_;
    __ST19364_Layout __ND20869__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP20343) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP20343.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP19479.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP19479.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP19479.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP19479.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP20343;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE21058,__ND20869__Project(LEFT));
    ...
    __EE20972 := __PP19479.Offender_Level_;
    __BS20959 := __T(__EE20972);
    __EE20976 := __BN(TOPN(__BS20959(__NN(KEL.era.ToDate(__T(__EE20972).Date_First_Seen_))),1, -__T(KEL.era.ToDate(__T(__EE20972).Date_First_Seen_)),__T(Offender_Level_)),__NL(__EE20972));
    SELF.Recent_Offender_Level_ := (__T(__EE20976))[1].Offender_Level_;
    SELF := __PP19479;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE21055,__ND20980__Project(LEFT));
END;

...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE3694668 := __ENH_Criminal_Offense_5;
  ...
  END;
  EXPORT __ST179989_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST180036_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST179989_Layout __ND3695020__Project(B_Criminal_Offense_5(__in,__cfg).__ST185963_Layout __PP3694669) := TRANSFORM
    __EE3694829 := __PP3694669.Offense_Charges_;
    __ST180036_Layout __ND3695000__Project(B_Criminal_Offense_5(__in,__cfg).__ST186010_Layout __PP3694830) := TRANSFORM
      ...
      SELF.Age_In_Days_ := FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP3694830.Criminal_Date_),__ECAST(KEL.typ.nkdate,__CC10826));
      SELF.Is_Felony_ := __AND(__AND(__NOT(__PP3694830.Dismissed_Charges_),IF(__T(__NT(__PP3694830.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP3694830.Offense_Score_,=,__CN('F'))))),__OR(__OP2(__PP3694830.Offender_Level_,=,__CN('4')),__NT(__PP3694830.Offender_Level_)));
      SELF.Is_Non_Felony_Records_ := __AND(__NOT(__PP3694830.Dismissed_Charges_),__NOT(__OP2(__PP3694830.Offense_Score_,=,__CN('F'))));
      SELF := __PP3694830;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE3694829,__ND3695000__Project(LEFT));
    SELF := __PP3694669;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE3694668,__ND3695020__Project(LEFT));
END;

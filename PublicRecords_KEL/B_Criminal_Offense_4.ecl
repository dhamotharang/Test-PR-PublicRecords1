...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE17813 := __E_Criminal_Offense;
  EXPORT __ST16122_Layout := RECORD
  ...
  END;
  EXPORT __ST16075_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST16122_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST16075_Layout __ND17738__Project(E_Criminal_Offense(__in,__cfg).Layout __PP16237) := TRANSFORM
    __EE17816 := __PP16237.Offense_Charges_;
    __ST16122_Layout __ND17627__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP17101) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP17101.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP16237.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP16237.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP16237.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP16237.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP17101;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE17816,__ND17627__Project(LEFT));
    ...
    __EE17730 := __PP16237.Offender_Level_;
    __BS17717 := __T(__EE17730);
    __EE17734 := __BN(TOPN(__BS17717(__NN(KEL.era.ToDate(__T(__EE17730).Date_First_Seen_))),1, -__T(KEL.era.ToDate(__T(__EE17730).Date_First_Seen_)),__T(Offender_Level_)),__NL(__EE17730));
    SELF.Recent_Offender_Level_ := (__T(__EE17734))[1].Offender_Level_;
    SELF := __PP16237;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE17813,__ND17738__Project(LEFT));
END;

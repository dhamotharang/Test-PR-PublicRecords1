...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE217454 := __E_Criminal_Offense;
  ...
  END;
  EXPORT __ST90981_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST91028_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST90981_Layout __ND217379__Project(E_Criminal_Offense(__in,__cfg).Layout __PP216733) := TRANSFORM
    __EE217457 := __PP216733.Offense_Charges_;
    __ST91028_Layout __ND217260__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP216735) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP216735.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP216733.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP216733.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP216733.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP216733.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP216735;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE217457,__ND217260__Project(LEFT));
    ...
    __EE217370 := __PP216733.Offender_Level_;
    __BS217357 := __T(__EE217370);
    __EE217374 := __BN(TOPN(__BS217357(__NN(KEL.era.ToDate(__T(__EE217370).Date_First_Seen_))),1, -__T(KEL.era.ToDate(__T(__EE217370).Date_First_Seen_)),__T(Offender_Level_)),__NL(__EE217370));
    SELF.Recent_Offender_Level_ := (__T(__EE217374))[1].Offender_Level_;
    SELF := __PP216733;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE217454,__ND217379__Project(LEFT));
END;

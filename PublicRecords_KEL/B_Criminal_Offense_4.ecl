...
EXPORT B_Criminal_Offense_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE25724 := __E_Criminal_Offense;
  EXPORT __ST23061_Layout := RECORD
  ...
  END;
  EXPORT __ST23014_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST23061_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST23014_Layout __ND25649__Project(E_Criminal_Offense(__in,__cfg).Layout __PP24148) := TRANSFORM
    __EE25727 := __PP24148.Offense_Charges_;
    __ST23061_Layout __ND25538__Project(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __PP25012) := TRANSFORM
      SELF.Criminal_Date_ := KEL.era.ToDate(__PP25012.Date_First_Seen_);
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP24148.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP24148.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP24148.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP24148.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP25012;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE25727,__ND25538__Project(LEFT));
    ...
    __EE25641 := __PP24148.Offender_Level_;
    __BS25628 := __T(__EE25641);
    __EE25645 := __BN(TOPN(__BS25628(__NN(KEL.era.ToDate(__T(__EE25641).Date_First_Seen_))),1, -__T(KEL.era.ToDate(__T(__EE25641).Date_First_Seen_)),__T(Offender_Level_)),__NL(__EE25641));
    SELF.Recent_Offender_Level_ := (__T(__EE25645))[1].Offender_Level_;
    SELF := __PP24148;
  END;
  EXPORT __ENH_Criminal_Offense_4 := PROJECT(__EE25724,__ND25649__Project(LEFT));
END;

...
EXPORT B_Aircraft_Owner_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE698886 := __E_Aircraft_Owner;
  EXPORT __ST212243_Layout := RECORD
  ...
  END;
  SHARED __ST212243_Layout __ND698898__Project(E_Aircraft_Owner(__in,__cfg).Layout __PP698824) := TRANSFORM
    ...
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.era.ToDate(__PP698824.Date_First_Seen_),>,__CC12929)),__ECAST(KEL.typ.nkdate,__CC12929),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP698824.Date_First_Seen_)));
    SELF := __PP698824;
  END;
  EXPORT __ENH_Aircraft_Owner_3 := PROJECT(__EE698886,__ND698898__Project(LEFT));
END;

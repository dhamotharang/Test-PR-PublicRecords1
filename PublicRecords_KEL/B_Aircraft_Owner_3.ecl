...
EXPORT B_Aircraft_Owner_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE648136 := __E_Aircraft_Owner;
  ...
  END;
  SHARED __ST174267_Layout __ND648148__Project(E_Aircraft_Owner(__in,__cfg).Layout __PP648074) := TRANSFORM
    ...
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.era.ToDate(__PP648074.Date_First_Seen_),>,__CC10251)),__ECAST(KEL.typ.nkdate,__CC10251),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP648074.Date_First_Seen_)));
    SELF := __PP648074;
  END;
  EXPORT __ENH_Aircraft_Owner_3 := PROJECT(__EE648136,__ND648148__Project(LEFT));
END;

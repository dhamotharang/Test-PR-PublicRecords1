...
EXPORT B_Sele_T_I_N_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE435919 := __E_Sele_T_I_N;
  SHARED __EE4881500 := __ENH_Business_Sele_7;
  SHARED __ST435991_Layout := RECORD
  ...
  END;
  __JC4881506(E_Sele_T_I_N(__in,__cfg).Layout __EE435919, B_Business_Sele_7(__in,__cfg).__ST248338_Layout __EE4881500) := __EEQP(__EE435919.Legal_,__EE4881500.UID);
  __ST435991_Layout __JT4881506(E_Sele_T_I_N(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST248338_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4881507 := JOIN(__EE435919,__EE4881500,__JC4881506(LEFT,RIGHT),__JT4881506(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST247298_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST247289_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_T_I_N().Typ) Tax_I_D_;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    ...
    KEL.typ.nint Best_T_I_N_;
    KEL.typ.nint Best_T_I_N_Rank_;
    KEL.typ.ndataset(__ST247298_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST247289_Layout __ND4882223__Project(__ST435991_Layout __PP4881508) := TRANSFORM
    __EE4881869 := __PP4881508.Data_Sources_;
    __ST247298_Layout __ND4881874__Project(E_Sele_T_I_N(__in,__cfg).Data_Sources_Layout __PP4881870) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP4881870.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP4881870.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP4881870.Source_));
      SELF := __PP4881870;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE4881869,__ND4881874__Project(LEFT));
    SELF.Input_T_I_N_Match_ := FN_Compile(__cfg).FN_Edit_Distance_Within_Radius(__ECAST(KEL.typ.nstr,__PP4881508.Tax_I_D_),__ECAST(KEL.typ.nstr,__PP4881508.B___Inp_Cln_T_I_N_),__ECAST(KEL.typ.nint,__CN(2)));
    SELF := __PP4881508;
  END;
  EXPORT __ENH_Sele_T_I_N_6 := PROJECT(__EE4881507,__ND4882223__Project(LEFT));
END;

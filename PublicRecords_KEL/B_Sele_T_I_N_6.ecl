...
EXPORT B_Sele_T_I_N_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE435449 := __E_Sele_T_I_N;
  SHARED __EE4873257 := __ENH_Business_Sele_7;
  SHARED __ST435521_Layout := RECORD
  ...
  END;
  __JC4873263(E_Sele_T_I_N(__in,__cfg).Layout __EE435449, B_Business_Sele_7(__in,__cfg).__ST247821_Layout __EE4873257) := __EEQP(__EE435449.Legal_,__EE4873257.UID);
  __ST435521_Layout __JT4873263(E_Sele_T_I_N(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST247821_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4873264 := JOIN(__EE435449,__EE4873257,__JC4873263(LEFT,RIGHT),__JT4873263(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST246781_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST246772_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_T_I_N().Typ) Tax_I_D_;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    ...
    KEL.typ.nint Best_T_I_N_;
    KEL.typ.nint Best_T_I_N_Rank_;
    KEL.typ.ndataset(__ST246781_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST246772_Layout __ND4873980__Project(__ST435521_Layout __PP4873265) := TRANSFORM
    __EE4873626 := __PP4873265.Data_Sources_;
    __ST246781_Layout __ND4873631__Project(E_Sele_T_I_N(__in,__cfg).Data_Sources_Layout __PP4873627) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP4873627.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP4873627.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP4873627.Source_));
      SELF := __PP4873627;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE4873626,__ND4873631__Project(LEFT));
    SELF.Input_T_I_N_Match_ := FN_Compile(__cfg).FN_Edit_Distance_Within_Radius(__ECAST(KEL.typ.nstr,__PP4873265.Tax_I_D_),__ECAST(KEL.typ.nstr,__PP4873265.B___Inp_Cln_T_I_N_),__ECAST(KEL.typ.nint,__CN(2)));
    SELF := __PP4873265;
  END;
  EXPORT __ENH_Sele_T_I_N_6 := PROJECT(__EE4873264,__ND4873980__Project(LEFT));
END;

...
EXPORT B_Sele_T_I_N_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE509184 := __E_Sele_T_I_N;
  SHARED __EE5538324 := __ENH_Business_Sele_7;
  SHARED __ST509256_Layout := RECORD
  ...
  END;
  __JC5538330(E_Sele_T_I_N(__in,__cfg).Layout __EE509184, B_Business_Sele_7(__in,__cfg).__ST262739_Layout __EE5538324) := __EEQP(__EE509184.Legal_,__EE5538324.UID);
  __ST509256_Layout __JT5538330(E_Sele_T_I_N(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST262739_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE5538331 := JOIN(__EE509184,__EE5538324,__JC5538330(LEFT,RIGHT),__JT5538330(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST261699_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST261690_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_T_I_N().Typ) Tax_I_D_;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    ...
    KEL.typ.nint Best_T_I_N_;
    KEL.typ.nint Best_T_I_N_Rank_;
    KEL.typ.ndataset(__ST261699_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST261690_Layout __ND5539047__Project(__ST509256_Layout __PP5538332) := TRANSFORM
    __EE5538693 := __PP5538332.Data_Sources_;
    __ST261699_Layout __ND5538698__Project(E_Sele_T_I_N(__in,__cfg).Data_Sources_Layout __PP5538694) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP5538694.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP5538694.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP5538694.Source_));
      SELF := __PP5538694;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE5538693,__ND5538698__Project(LEFT));
    SELF.Input_T_I_N_Match_ := FN_Compile(__cfg).FN_Edit_Distance_Within_Radius(__ECAST(KEL.typ.nstr,__PP5538332.Tax_I_D_),__ECAST(KEL.typ.nstr,__PP5538332.B___Inp_Cln_T_I_N_),__ECAST(KEL.typ.nint,__CN(2)));
    SELF := __PP5538332;
  END;
  EXPORT __ENH_Sele_T_I_N_6 := PROJECT(__EE5538331,__ND5539047__Project(LEFT));
END;

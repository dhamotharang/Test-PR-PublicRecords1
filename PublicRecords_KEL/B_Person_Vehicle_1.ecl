...
EXPORT B_Person_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5173317 := __ENH_Person_Vehicle_2;
  EXPORT __ST155619_Layout := RECORD
  ...
  END;
  SHARED __ST155619_Layout __ND5173445__Project(B_Person_Vehicle_2(__in,__cfg).__ST171364_Layout __PP5173318) := TRANSFORM
    ...
    __CC10656 := 3652;
    SELF.Seen___In___Last___Ten___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP5173318.Date_Last_Seen_Capped_,__CC10411),<=,__CN(__CC10656));
    __CC10658 := 730;
    SELF.Seen___In___Last___Two___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP5173318.Date_Last_Seen_Capped_,__CC10411),<=,__CN(__CC10658));
    __CC10654 := '-99997';
    SELF.Vehicle_Max_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP5173318.Date_Last_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP5173318.Date_Last_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC10654)));
    SELF := __PP5173318;
  END;
  EXPORT __ENH_Person_Vehicle_1 := PROJECT(__EE5173317,__ND5173445__Project(LEFT));
END;

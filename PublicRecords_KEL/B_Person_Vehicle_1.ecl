...
EXPORT B_Person_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1762311 := __ENH_Person_Vehicle_2;
  EXPORT __ST115448_Layout := RECORD
  ...
  END;
  SHARED __ST115448_Layout __ND1762553__Project(B_Person_Vehicle_2(__in,__cfg).__ST126266_Layout __PP1762111) := TRANSFORM
    ...
    __CC9349 := 3652;
    SELF.Seen___In___Last___Ten___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP1762111.Date_Last_Seen_Capped_,__CC9163),<=,__CN(__CC9349));
    __CC9351 := 730;
    SELF.Seen___In___Last___Two___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP1762111.Date_Last_Seen_Capped_,__CC9163),<=,__CN(__CC9351));
    __CC9347 := '-99997';
    SELF.Vehicle_Max_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1762111.Date_Last_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1762111.Date_Last_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC9347)));
    SELF := __PP1762111;
  END;
  EXPORT __ENH_Person_Vehicle_1 := PROJECT(__EE1762311,__ND1762553__Project(LEFT));
END;

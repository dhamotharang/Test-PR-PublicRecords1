...
EXPORT B_Person_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE9481745 := __ENH_Person_Vehicle_2;
  ...
  END;
  SHARED __ST208489_Layout __ND9481913__Project(B_Person_Vehicle_2(__in,__cfg).__ST227649_Layout __PP9481746) := TRANSFORM
    ...
    __CC13683 := 3652;
    SELF.Seen___In___Last___Ten___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP9481746.Date_Last_Seen_Capped_,__CC13351),<=,__CN(__CC13683));
    __CC13685 := 730;
    SELF.Seen___In___Last___Two___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP9481746.Date_Last_Seen_Capped_,__CC13351),<=,__CN(__CC13685));
    __CC13681 := '-99997';
    SELF.Vehicle_Max_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP9481746.Date_Last_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP9481746.Date_Last_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13681)));
    SELF := __PP9481746;
  END;
  EXPORT __ENH_Person_Vehicle_1 := PROJECT(__EE9481745,__ND9481913__Project(LEFT));
END;

...
EXPORT B_Person_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5496712 := __ENH_Person_Vehicle_2;
  EXPORT __ST146286_Layout := RECORD
  ...
  END;
  SHARED __ST146286_Layout __ND5496840__Project(B_Person_Vehicle_2(__in,__cfg).__ST160898_Layout __PP5496713) := TRANSFORM
    ...
    __CC11135 := 3652;
    SELF.Seen___In___Last___Ten___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP5496713.Date_Last_Seen_Capped_,__CC10922),<=,__CN(__CC11135));
    __CC11137 := 730;
    SELF.Seen___In___Last___Two___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP5496713.Date_Last_Seen_Capped_,__CC10922),<=,__CN(__CC11137));
    __CC11133 := '-99997';
    SELF.Vehicle_Max_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP5496713.Date_Last_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP5496713.Date_Last_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC11133)));
    SELF := __PP5496713;
  END;
  EXPORT __ENH_Person_Vehicle_1 := PROJECT(__EE5496712,__ND5496840__Project(LEFT));
END;

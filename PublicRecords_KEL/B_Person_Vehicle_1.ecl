...
EXPORT B_Person_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE937633 := __ENH_Person_Vehicle_2;
  EXPORT __ST74566_Layout := RECORD
  ...
  END;
  SHARED __ST74566_Layout __ND937880__Project(B_Person_Vehicle_2(__in,__cfg).__ST81819_Layout __PP937424) := TRANSFORM
    __CC7984 := 3652;
    SELF.Seen___In___Last___Ten___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP937424.Date_Last_Seen_Capped_,__PP937424.Current_Date_),<=,__CN(__CC7984));
    __CC7986 := 730;
    SELF.Seen___In___Last___Two___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP937424.Date_Last_Seen_Capped_,__PP937424.Current_Date_),<=,__CN(__CC7986));
    __CC7982 := '-99997';
    SELF.Vehicle_Max_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP937424.Date_Last_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP937424.Date_Last_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC7982)));
    SELF := __PP937424;
  END;
  EXPORT __ENH_Person_Vehicle_1 := PROJECT(__EE937633,__ND937880__Project(LEFT));
END;

...
EXPORT B_Person_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8098023 := __ENH_Person_Vehicle_2;
  EXPORT __ST195345_Layout := RECORD
  ...
  END;
  SHARED __ST195345_Layout __ND8098191__Project(B_Person_Vehicle_2(__in,__cfg).__ST213721_Layout __PP8098024) := TRANSFORM
    ...
    __CC13686 := 3652;
    SELF.Seen___In___Last___Ten___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP8098024.Date_Last_Seen_Capped_,__CC13354),<=,__CN(__CC13686));
    __CC13688 := 730;
    SELF.Seen___In___Last___Two___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP8098024.Date_Last_Seen_Capped_,__CC13354),<=,__CN(__CC13688));
    __CC13684 := '-99997';
    SELF.Vehicle_Max_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP8098024.Date_Last_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP8098024.Date_Last_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13684)));
    SELF := __PP8098024;
  END;
  EXPORT __ENH_Person_Vehicle_1 := PROJECT(__EE8098023,__ND8098191__Project(LEFT));
END;

...
EXPORT B_Person_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE9680575 := __ENH_Person_Vehicle_2;
  EXPORT __ST211197_Layout := RECORD
  ...
  END;
  SHARED __ST211197_Layout __ND9680743__Project(B_Person_Vehicle_2(__in,__cfg).__ST230485_Layout __PP9680576) := TRANSFORM
    ...
    __CC13671 := 3652;
    SELF.Seen___In___Last___Ten___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP9680576.Date_Last_Seen_Capped_,__CC13339),<=,__CN(__CC13671));
    __CC13673 := 730;
    SELF.Seen___In___Last___Two___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP9680576.Date_Last_Seen_Capped_,__CC13339),<=,__CN(__CC13673));
    __CC13669 := '-99997';
    SELF.Vehicle_Max_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP9680576.Date_Last_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP9680576.Date_Last_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13669)));
    SELF := __PP9680576;
  END;
  EXPORT __ENH_Person_Vehicle_1 := PROJECT(__EE9680575,__ND9680743__Project(LEFT));
END;

...
EXPORT B_Person_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2794453 := __ENH_Person_Vehicle_2;
  EXPORT __ST124116_Layout := RECORD
  ...
  END;
  SHARED __ST124116_Layout __ND2794695__Project(B_Person_Vehicle_2(__in,__cfg).__ST135491_Layout __PP2794253) := TRANSFORM
    ...
    __CC9770 := 3652;
    SELF.Seen___In___Last___Ten___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP2794253.Date_Last_Seen_Capped_,__CC9568),<=,__CN(__CC9770));
    __CC9772 := 730;
    SELF.Seen___In___Last___Two___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP2794253.Date_Last_Seen_Capped_,__CC9568),<=,__CN(__CC9772));
    __CC9768 := '-99997';
    SELF.Vehicle_Max_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP2794253.Date_Last_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP2794253.Date_Last_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC9768)));
    SELF := __PP2794253;
  END;
  EXPORT __ENH_Person_Vehicle_1 := PROJECT(__EE2794453,__ND2794695__Project(LEFT));
END;

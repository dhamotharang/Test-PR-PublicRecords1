...
EXPORT B_Person_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7942519 := __ENH_Person_Vehicle_2;
  EXPORT __ST191401_Layout := RECORD
  ...
  END;
  SHARED __ST191401_Layout __ND7942687__Project(B_Person_Vehicle_2(__in,__cfg).__ST209860_Layout __PP7942520) := TRANSFORM
    ...
    __CC13425 := 3652;
    SELF.Seen___In___Last___Ten___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP7942520.Date_Last_Seen_Capped_,__CC13109),<=,__CN(__CC13425));
    __CC13427 := 730;
    SELF.Seen___In___Last___Two___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP7942520.Date_Last_Seen_Capped_,__CC13109),<=,__CN(__CC13427));
    __CC13423 := '-99997';
    SELF.Vehicle_Max_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP7942520.Date_Last_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP7942520.Date_Last_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13423)));
    SELF := __PP7942520;
  END;
  EXPORT __ENH_Person_Vehicle_1 := PROJECT(__EE7942519,__ND7942687__Project(LEFT));
END;

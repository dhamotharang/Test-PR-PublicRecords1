...
EXPORT B_Person_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8055725 := __ENH_Person_Vehicle_2;
  EXPORT __ST191843_Layout := RECORD
  ...
  END;
  SHARED __ST191843_Layout __ND8055893__Project(B_Person_Vehicle_2(__in,__cfg).__ST210342_Layout __PP8055726) := TRANSFORM
    ...
    __CC13418 := 3652;
    SELF.Seen___In___Last___Ten___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP8055726.Date_Last_Seen_Capped_,__CC13102),<=,__CN(__CC13418));
    __CC13420 := 730;
    SELF.Seen___In___Last___Two___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP8055726.Date_Last_Seen_Capped_,__CC13102),<=,__CN(__CC13420));
    __CC13416 := '-99997';
    SELF.Vehicle_Max_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP8055726.Date_Last_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP8055726.Date_Last_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13416)));
    SELF := __PP8055726;
  END;
  EXPORT __ENH_Person_Vehicle_1 := PROJECT(__EE8055725,__ND8055893__Project(LEFT));
END;

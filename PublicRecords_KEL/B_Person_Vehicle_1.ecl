...
EXPORT B_Person_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8995643 := __ENH_Person_Vehicle_2;
  EXPORT __ST203236_Layout := RECORD
  ...
  END;
  SHARED __ST203236_Layout __ND8995811__Project(B_Person_Vehicle_2(__in,__cfg).__ST222396_Layout __PP8995644) := TRANSFORM
    ...
    __CC13613 := 3652;
    SELF.Seen___In___Last___Ten___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP8995644.Date_Last_Seen_Capped_,__CC13281),<=,__CN(__CC13613));
    __CC13615 := 730;
    SELF.Seen___In___Last___Two___Years_ := __OP2(__FN2(KEL.Routines.DaysBetween,__PP8995644.Date_Last_Seen_Capped_,__CC13281),<=,__CN(__CC13615));
    __CC13611 := '-99997';
    SELF.Vehicle_Max_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP8995644.Date_Last_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP8995644.Date_Last_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13611)));
    SELF := __PP8995644;
  END;
  EXPORT __ENH_Person_Vehicle_1 := PROJECT(__EE8995643,__ND8995811__Project(LEFT));
END;

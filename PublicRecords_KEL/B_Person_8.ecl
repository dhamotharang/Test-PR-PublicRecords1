...
EXPORT B_Person_8(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5554486 := __ENH_Person_9;
  SHARED __EE5554489 := __ENH_Input_P_I_I_9;
  SHARED __ST416534_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(B_Person_9(__in,__cfg).__ST392032_Layout) All_Lien_Data_;
  ...
  END;
  __JC5555208(B_Person_9(__in,__cfg).__ST278231_Layout __EE5554486, B_Input_P_I_I_9(__in,__cfg).__ST277439_Layout __EE5554489) := __EEQP(__EE5554486.P_I_I_,__EE5554489.UID);
  __ST416534_Layout __JT5555208(B_Person_9(__in,__cfg).__ST278231_Layout __l, B_Input_P_I_I_9(__in,__cfg).__ST277439_Layout __r) := TRANSFORM
    SELF.U_I_D__1_ := __r.UID;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE5555209 := JOIN(__EE5554486,__EE5554489,__JC5555208(LEFT,RIGHT),__JT5555208(LEFT,RIGHT),LEFT OUTER,SMART);
  EXPORT __ST414739_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST275965_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nstr Gender_;
    KEL.typ.nstr Lex_I_D_Segment_;
    KEL.typ.nstr Lex_I_D_Segment2_;
    ...
    KEL.typ.ndataset(E_Person(__in,__cfg).Thrive_Layout) Thrive_;
    KEL.typ.ndataset(B_Person_10(__in,__cfg).__ST85371_Layout) Address_Hierarchy_Set_;
    KEL.typ.ndataset(__ST414739_Layout) All_Lien_Data_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST275965_Layout __ND5554740__Project(__ST416534_Layout __PP5554499) := TRANSFORM
    __EE5554936 := __PP5554499.All_Lien_Data_;
    __ST414739_Layout __ND5554941__Project(B_Person_9(__in,__cfg).__ST392032_Layout __PP5554937) := TRANSFORM
      ...
      SELF.Age_In_Days_ := FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP5554937.Original_Filing_Date_),__ECAST(KEL.typ.nkdate,__CC13427));
      SELF.Is_Over_All_Judgment_ := __OR(__OR(__PP5554937.Is_Civil_Court_Judgment_,__PP5554937.Is_Foreclosure_Judgment_),__PP5554937.Is_Small_Cliams_Judgment_);
      SELF.Is_Over_All_Lien_ := __OR(__PP5554937.Is_Total_Tax_Lien_,__PP5554937.Is_Other_Lien_);
      SELF := __PP5554937;
    END;
    SELF.All_Lien_Data_ := __PROJECT(__EE5554936,__ND5554941__Project(LEFT));
    __EE5555020 := __PP5554499.Recent_Addr_Full_Set_;
    __BS5555021 := __T(__EE5555020);
    __EE5555032 := __BN(TOPN(__BS5555021(__NN(__T(__EE5555020).Address_Rank_) AND __NN(__T(__EE5555020).Sort_Field_)),1,__T(__T(__EE5555020).Address_Rank_), -__T(__T(__EE5555020).Sort_Field_),__T(Address_Type_),__T(Address_Status_),__T(State_Code_),__T(County_Code_),__T(Latitude_),__T(Longitude_),__T(Geo_Blk_),__T(Addr_Full_),__T(Primary_Range_),__T(Predirectional_),__T(Primary_Name_),__T(Suffix_),__T(Postdirectional_),__T(Unit_Designation_),__T(Secondary_Range_),__T(Postal_City_),__T(State_),__T(Z_I_P5_),__T(Z_I_P4_),__T(Addr1_From_Components_),__T(Hierarchy_Date_First_Seen_),__T(Hierarchy_Date_Last_Seen_)),__NL(__EE5555020));
    SELF.Curr_Addr_Full_Set_ := __EE5555032;
    SELF.P___Inp_Cln_Name_First_Raw_ := __PP5554499.P___Inp_Cln_Name_First_;
    SELF.P___Inp_Cln_Name_Last_Raw_ := __PP5554499.P___Inp_Cln_Name_Last_;
    SELF := __PP5554499;
  END;
  EXPORT __ENH_Person_8 := PROJECT(__EE5555209,__ND5554740__Project(LEFT));
END;

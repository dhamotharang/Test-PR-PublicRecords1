...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8037561 := __ENH_Watercraft_Owner_3;
  EXPORT __ST223348_Layout := RECORD
  ...
  END;
  SHARED __ST223348_Layout __ND8037606__Project(B_Watercraft_Owner_3(__in,__cfg).__ST238157_Layout __PP8037562) := TRANSFORM
    __CC13827 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP8037562.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP8037562.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13827)));
    SELF := __PP8037562;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE8037561,__ND8037606__Project(LEFT));
END;

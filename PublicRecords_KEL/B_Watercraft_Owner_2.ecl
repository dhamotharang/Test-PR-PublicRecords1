...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE675529 := __ENH_Watercraft_Owner_3;
  EXPORT __ST83869_Layout := RECORD
  ...
  END;
  SHARED __ST83869_Layout __ND675555__Project(B_Watercraft_Owner_3(__in,__cfg).__ST89256_Layout __PP675473) := TRANSFORM
    __CC8093 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP675473.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP675473.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC8093)));
    SELF := __PP675473;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE675529,__ND675555__Project(LEFT));
END;

...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8215506 := __ENH_Watercraft_Owner_3;
  EXPORT __ST231471_Layout := RECORD
  ...
  END;
  SHARED __ST231471_Layout __ND8215551__Project(B_Watercraft_Owner_3(__in,__cfg).__ST246311_Layout __PP8215507) := TRANSFORM
    __CC13842 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP8215507.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP8215507.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13842)));
    SELF := __PP8215507;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE8215506,__ND8215551__Project(LEFT));
END;

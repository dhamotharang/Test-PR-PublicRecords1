...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE9307968 := __ENH_Watercraft_Owner_3;
  EXPORT __ST247032_Layout := RECORD
  ...
  END;
  SHARED __ST247032_Layout __ND9308013__Project(B_Watercraft_Owner_3(__in,__cfg).__ST262490_Layout __PP9307969) := TRANSFORM
    __CC13851 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP9307969.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP9307969.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13851)));
    SELF := __PP9307969;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE9307968,__ND9308013__Project(LEFT));
END;

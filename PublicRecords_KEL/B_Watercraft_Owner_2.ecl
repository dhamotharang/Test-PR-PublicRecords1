...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8039147 := __ENH_Watercraft_Owner_3;
  EXPORT __ST223318_Layout := RECORD
  ...
  END;
  SHARED __ST223318_Layout __ND8039192__Project(B_Watercraft_Owner_3(__in,__cfg).__ST238127_Layout __PP8039148) := TRANSFORM
    __CC13827 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP8039148.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP8039148.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13827)));
    SELF := __PP8039148;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE8039147,__ND8039192__Project(LEFT));
END;

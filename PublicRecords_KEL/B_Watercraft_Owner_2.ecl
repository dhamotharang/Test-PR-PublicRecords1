...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7615715 := __ENH_Watercraft_Owner_3;
  EXPORT __ST220549_Layout := RECORD
  ...
  END;
  SHARED __ST220549_Layout __ND7615760__Project(B_Watercraft_Owner_3(__in,__cfg).__ST235078_Layout __PP7615716) := TRANSFORM
    __CC13695 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP7615716.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP7615716.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13695)));
    SELF := __PP7615716;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE7615715,__ND7615760__Project(LEFT));
END;

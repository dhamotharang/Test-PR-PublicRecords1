...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8890123 := __ENH_Watercraft_Owner_3;
  EXPORT __ST240304_Layout := RECORD
  ...
  END;
  SHARED __ST240304_Layout __ND8890168__Project(B_Watercraft_Owner_3(__in,__cfg).__ST255986_Layout __PP8890124) := TRANSFORM
    __CC13792 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP8890124.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP8890124.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13792)));
    SELF := __PP8890124;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE8890123,__ND8890168__Project(LEFT));
END;

...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8252373 := __ENH_Watercraft_Owner_3;
  EXPORT __ST229190_Layout := RECORD
  ...
  END;
  SHARED __ST229190_Layout __ND8252418__Project(B_Watercraft_Owner_3(__in,__cfg).__ST244109_Layout __PP8252374) := TRANSFORM
    __CC13790 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP8252374.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP8252374.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13790)));
    SELF := __PP8252374;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE8252373,__ND8252418__Project(LEFT));
END;

...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7575196 := __ENH_Watercraft_Owner_3;
  EXPORT __ST218767_Layout := RECORD
  ...
  END;
  SHARED __ST218767_Layout __ND7575241__Project(B_Watercraft_Owner_3(__in,__cfg).__ST233146_Layout __PP7575197) := TRANSFORM
    __CC13612 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP7575197.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP7575197.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13612)));
    SELF := __PP7575197;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE7575196,__ND7575241__Project(LEFT));
END;

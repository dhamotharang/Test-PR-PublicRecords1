...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8319637 := __ENH_Watercraft_Owner_3;
  EXPORT __ST233000_Layout := RECORD
  ...
  END;
  SHARED __ST233000_Layout __ND8319682__Project(B_Watercraft_Owner_3(__in,__cfg).__ST247921_Layout __PP8319638) := TRANSFORM
    __CC13848 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP8319638.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP8319638.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13848)));
    SELF := __PP8319638;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE8319637,__ND8319682__Project(LEFT));
END;

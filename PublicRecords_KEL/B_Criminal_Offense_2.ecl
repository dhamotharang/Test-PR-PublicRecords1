...
EXPORT B_Criminal_Offense_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE20673 := __E_Criminal_Offense;
  ...
  END;
  EXPORT __ST13203_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST13250_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST13203_Layout __ND20584__Project(E_Criminal_Offense(__in,__cfg).Layout __PP19133) := TRANSFORM
    __EE20569 := __PP19133.Offense_Charges_;
    __EE20566 := __PP19133.Offender_Level_;
    __ST19770_Layout := RECORD
      KEL.typ.nstr Case_Number_;
      KEL.typ.nkdate Case_Date_;
      KEL.typ.nstr Case_Type_Description_;
      KEL.typ.nkdate Offense_Date_;
      ...
      KEL.typ.int __RecordCount := 0;
    END;
    __JC20574(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __EE20569, E_Criminal_Offense(__in,__cfg).Offender_Level_Layout __EE20566) := TRUE;
    __ST19770_Layout __JT20574(E_Criminal_Offense(__in,__cfg).Offense_Charges_Layout __l, E_Criminal_Offense(__in,__cfg).Offender_Level_Layout __r) := TRANSFORM
      SELF := __l;
      SELF := __r;
    END;
    __EE20575 := JOIN(__T(__EE20569),__T(__EE20566),__JC20574(LEFT,RIGHT),__JT20574(LEFT,RIGHT),INNER,ALL);
    __EE20582 := IF(TRUE AND TRUE,__EE20575,DATASET([],__ST19770_Layout));
    __ST13250_Layout __ND20479__Project(__ST19770_Layout __PP19930) := TRANSFORM
      SELF.Is_Felony_ := __AND(__OP2(__PP19930.Offender_Level_,=,__CN(4)),__OP2(__PP19930.Offense_Score_,=,__CN('F')));
      SELF := __PP19930;
    END;
    SELF.Offense_Charges_ := __FILTER(__CN(PROJECT(__EE20582,__ND20479__Project(LEFT))),__NN(Case_Number_) OR __NN(Case_Date_) OR __NN(Case_Type_Description_) OR __NN(Offense_Date_) OR __NN(Offense_Score_) OR __NN(Data_Type_) OR __NN(Traffic_Flag_) OR __NN(Conviction_Flag_) OR __NN(Is_Felony_));
    ...
    SELF := __PP19133;
  END;
  EXPORT __ENH_Criminal_Offense_2 := PROJECT(__EE20673,__ND20584__Project(LEFT));
END;

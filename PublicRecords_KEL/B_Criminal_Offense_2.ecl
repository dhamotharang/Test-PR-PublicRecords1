...
EXPORT B_Criminal_Offense_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7206295 := __ENH_Criminal_Offense_3;
  EXPORT __ST216429_Layout := RECORD
  ...
  END;
  EXPORT __ST216382_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST216429_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST216382_Layout __ND7206748__Project(B_Criminal_Offense_3(__in,__cfg).__ST233472_Layout __PP7206296) := TRANSFORM
    __EE7206456 := __PP7206296.Offense_Charges_;
    __ST216429_Layout __ND7206716__Project(B_Criminal_Offense_3(__in,__cfg).__ST233519_Layout __PP7206457) := TRANSFORM
      SELF.Is_Arrest_ := __OP2(__PP7206457.Data_Type_,=,__CN('5'));
      SELF.Is_Non_Felony_Records_F_C_R_A_ := __AND(__AND(__AND(__NOT(__PP7206457.Dismissed_Charges_),__OP2(__PP7206457.Offense_Score_,=,__CN('M'))),__OP2(__PP7206457.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP7206457.Traffic_Flag_,=,__CN('N')));
      SELF := __PP7206457;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE7206456,__ND7206716__Project(LEFT));
    SELF := __PP7206296;
  END;
  EXPORT __ENH_Criminal_Offense_2 := PROJECT(__EE7206295,__ND7206748__Project(LEFT));
END;

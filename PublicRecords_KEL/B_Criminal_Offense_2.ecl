...
EXPORT B_Criminal_Offense_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8355212 := __ENH_Criminal_Offense_3;
  EXPORT __ST235929_Layout := RECORD
  ...
  END;
  EXPORT __ST235882_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST235929_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST235882_Layout __ND8355665__Project(B_Criminal_Offense_3(__in,__cfg).__ST254156_Layout __PP8355213) := TRANSFORM
    __EE8355373 := __PP8355213.Offense_Charges_;
    __ST235929_Layout __ND8355633__Project(B_Criminal_Offense_3(__in,__cfg).__ST254203_Layout __PP8355374) := TRANSFORM
      SELF.Is_Arrest_ := __OP2(__PP8355374.Data_Type_,=,__CN('5'));
      SELF.Is_Non_Felony_Records_F_C_R_A_ := __AND(__AND(__AND(__NOT(__PP8355374.Dismissed_Charges_),__OP2(__PP8355374.Offense_Score_,=,__CN('M'))),__OP2(__PP8355374.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP8355374.Traffic_Flag_,=,__CN('N')));
      SELF := __PP8355374;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE8355373,__ND8355633__Project(LEFT));
    SELF := __PP8355213;
  END;
  EXPORT __ENH_Criminal_Offense_2 := PROJECT(__EE8355212,__ND8355665__Project(LEFT));
END;

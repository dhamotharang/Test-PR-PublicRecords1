...
EXPORT B_Sele_Property_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE870115 := __E_Sele_Property;
  SHARED __EE870678 := __E_Sele_Property_Event;
  SHARED __EE6950415 := __EE870678(__NN(__EE870678.Legal_) AND __NN(__EE870678.Event_));
  SHARED __EE870577 := __EE870115;
  SHARED __EE6951948 := __EE870577(__NN(__EE870577.Prop_) AND __NN(__EE870577.Legal_));
  SHARED __EE6949261 := __ENH_Property_Event_5;
  SHARED __EE6949268 := __EE6949261(__NN(__EE6949261.Prop_));
  SHARED __ST871678_Layout := RECORD
  ...
  END;
  __JC6952027(E_Sele_Property(__in,__cfg).Layout __EE6951948, B_Property_Event_6(__in,__cfg).__ST285339_Layout __EE6949268) := __EEQP(__EE6951948.Prop_,__EE6949268.Prop_);
  __ST871678_Layout __JT6952027(E_Sele_Property(__in,__cfg).Layout __l, B_Property_Event_6(__in,__cfg).__ST285339_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE6952028 := JOIN(__EE6951948,__EE6949268,__JC6952027(LEFT,RIGHT),__JT6952027(LEFT,RIGHT),INNER,HASH);
  SHARED __ST871832_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC6952156(E_Sele_Property_Event(__in,__cfg).Layout __EE6950415, __ST871678_Layout __EE6952028) := __EEQP(__EE6952028.UID,__EE6950415.Event_) AND __NNEQ(__EE6950415.Legal_,__EE6952028.Legal_) AND __T(__AND(__EEQ(__EE6952028.UID,__EE6950415.Event_),__OP2(__EE6950415.Legal_,=,__EE6952028.Legal_)));
  __ST871832_Layout __JT6952156(E_Sele_Property_Event(__in,__cfg).Layout __l, __ST871678_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE6952157 := JOIN(__EE6950415,__EE6952028,__JC6952156(LEFT,RIGHT),__JT6952156(LEFT,RIGHT),INNER,HASH);
  SHARED __ST871114_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Property().Typ) Prop_;
    KEL.typ.ndataset(E_Sele_Property_Event(__in,__cfg).Party_Details_Layout) Party_Details_;
    KEL.typ.nbool Is_Current_Assessment_Record_;
    ...
    KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
  END;
  SHARED __EE6952327 := PROJECT(__EE6952157,TRANSFORM(__ST871114_Layout,SELF.Legal_ := LEFT.Legal__1_,SELF := LEFT));
  SHARED __ST871201_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Property().Typ) Prop_;
    ...
    KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
  END;
  SHARED __ST871201_Layout __ND6952332__Project(__ST871114_Layout __PP6952328) := TRANSFORM
    __BS6952349 := __T(__PP6952328.Party_Details_);
    SELF.Exp1_ := EXISTS(__BS6952349(__T(__OR(__T(__PP6952328.Party_Details_).Party_Is_Buyer_Or_Owner_,__T(__PP6952328.Party_Details_).Party_Is_Seller_))));
    __BS6952368 := __T(__PP6952328.Party_Details_);
    SELF.Exp2_ := EXISTS(__BS6952368(__T(__AND(__PP6952328.Is_Current_Assessment_Record_,__T(__PP6952328.Party_Details_).Party_Is_Buyer_Or_Owner_))));
    __BS6952386 := __T(__PP6952328.Party_Details_);
    SELF.Exp3_ := EXISTS(__BS6952386(__T(__AND(__PP6952328.Is_Deed_,__T(__PP6952328.Party_Details_).Party_Is_Buyer_Or_Owner_))));
    __BS6952404 := __T(__PP6952328.Party_Details_);
    SELF.Exp4_ := EXISTS(__BS6952404(__T(__T(__PP6952328.Party_Details_).Party_Is_Seller_)));
    SELF := __PP6952328;
  END;
  SHARED __EE6952419 := PROJECT(__EE6952327,__ND6952332__Project(LEFT));
  SHARED __ST871235_Layout := RECORD
    ...
    KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
  END;
  SHARED __EE6952454 := PROJECT(__CLEANANDDO(__EE6952419,TABLE(__EE6952419,{KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),KEL.typ.int C_O_U_N_T___Sele_Property_Event_ := COUNT(GROUP,__EE6952419.Exp1_),KEL.typ.int C_O_U_N_T___Sele_Property_Event__1_ := COUNT(GROUP,__EE6952419.Exp2_),KEL.typ.int C_O_U_N_T___Sele_Property_Event__2_ := COUNT(GROUP,__EE6952419.Exp3_),KEL.typ.int C_O_U_N_T___Sele_Property_Event__3_ := COUNT(GROUP,__EE6952419.Exp4_),Legal_,Prop_},Legal_,Prop_,MERGE)),__ST871235_Layout);
  SHARED __ST872035_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Property().Typ) Prop_;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC6952464(E_Sele_Property(__in,__cfg).Layout __EE870115, __ST871235_Layout __EE6952454) := __EEQP(__EE870115.Prop_,__EE6952454.Prop_) AND __EEQP(__EE870115.Legal_,__EE6952454.Legal_);
  __ST872035_Layout __JT6952464(E_Sele_Property(__in,__cfg).Layout __l, __ST871235_Layout __r) := TRANSFORM
    SELF.Legal__1_ := __r.Legal_;
    SELF.Prop__1_ := __r.Prop_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE6952465 := JOIN(__EE870115,__EE6952454,__JC6952464(LEFT,RIGHT),__JT6952464(LEFT,RIGHT),LEFT OUTER,SMART);
  EXPORT __ST273513_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST273513_Layout __ND6952506__Project(__ST872035_Layout __PP6952466) := TRANSFORM
    SELF.Is_Ever_Owned_ := __PP6952466.C_O_U_N_T___Sele_Property_Event_ <> 0;
    SELF.Property_Is_Owned_Assessment_ := __PP6952466.C_O_U_N_T___Sele_Property_Event__1_ <> 0;
    SELF.Property_Is_Owned_Deed_ := __PP6952466.C_O_U_N_T___Sele_Property_Event__2_ <> 0;
    SELF.Property_Is_Sold_ := __PP6952466.C_O_U_N_T___Sele_Property_Event__3_ <> 0;
    SELF := __PP6952466;
  END;
  EXPORT __ENH_Sele_Property_4 := PROJECT(__EE6952465,__ND6952506__Project(LEFT));
END;

...
EXPORT B_Sele_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE3221856 := __ENH_Sele_Vehicle_2;
  EXPORT __ST180456_Layout := RECORD
  ...
  END;
  SHARED __ST180456_Layout __ND3222062__Project(B_Sele_Vehicle_2(__in,__cfg).__ST191108_Layout __PP3221857) := TRANSFORM
    __CC13872 := ['AG','AR','CLG','CML','DE','DLR','EA','EMR','ENV','FGV','FNL','FOR','LGV','LIV','MFG','MIL','MUB','OFF','POL','SCB','SGV','TAX'];
    SELF.Commercial_Type_ := __OP2(__PP3221857.Vehicle_Type_Code_,IN,__CN(__CC13872));
    __CC13882 := ['BOT','EXT','MOT','OTH','TRL','UNK','VAN','XSR'];
    SELF.Other_Type_ := __OR(__OR(__OP2(__PP3221857.Vehicle_Type_Code_,IN,__CN(__CC13882)),__OP2(__PP3221857.Vehicle_Type_Code_,=,__CN(''))),__NT(__PP3221857.Vehicle_Type_Code_));
    __CC13848 := ['ANQ','DAV','HCP','MH','PRV'];
    SELF.Private_Type_ := __OP2(__PP3221857.Vehicle_Type_Code_,IN,__CN(__CC13848));
    SELF := __PP3221857;
  END;
  EXPORT __ENH_Sele_Vehicle_1 := PROJECT(__EE3221856,__ND3222062__Project(LEFT));
END;

...
EXPORT B_Sele_Vehicle_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE3221045 := __ENH_Sele_Vehicle_2;
  ...
  END;
  SHARED __ST179687_Layout __ND3221251__Project(B_Sele_Vehicle_2(__in,__cfg).__ST190343_Layout __PP3221046) := TRANSFORM
    __CC13866 := ['AG','AR','CLG','CML','DE','DLR','EA','EMR','ENV','FGV','FNL','FOR','LGV','LIV','MFG','MIL','MUB','OFF','POL','SCB','SGV','TAX'];
    SELF.Commercial_Type_ := __OP2(__PP3221046.Vehicle_Type_Code_,IN,__CN(__CC13866));
    __CC13876 := ['BOT','EXT','MOT','OTH','TRL','UNK','VAN','XSR'];
    SELF.Other_Type_ := __OR(__OR(__OP2(__PP3221046.Vehicle_Type_Code_,IN,__CN(__CC13876)),__OP2(__PP3221046.Vehicle_Type_Code_,=,__CN(''))),__NT(__PP3221046.Vehicle_Type_Code_));
    __CC13842 := ['ANQ','DAV','HCP','MH','PRV'];
    SELF.Private_Type_ := __OP2(__PP3221046.Vehicle_Type_Code_,IN,__CN(__CC13842));
    SELF := __PP3221046;
  END;
  EXPORT __ENH_Sele_Vehicle_1 := PROJECT(__EE3221045,__ND3221251__Project(LEFT));
END;

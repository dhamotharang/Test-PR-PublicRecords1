...
EXPORT B_Business_Prox_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2127715 := __E_Business_Prox;
  SHARED __EE2127726 := __E_Prox_Address;
  SHARED __EE2127736 := __EE2127726(__NN(__EE2127726.Business_Location_));
  SHARED __ST557833_Layout := RECORD
  ...
  END;
  __ST557833_Layout __JT2127744(E_Prox_Address(__in,__cfg).Layout __l, E_Prox_Address(__in,__cfg).Best_Addresses_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE2127813 := NORMALIZE(__EE2127736,__T(LEFT.Best_Addresses_),__JT2127744(LEFT,RIGHT));
  SHARED __ST557433_Layout := RECORD
  ...
  SHARED __ST561733_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST557433_Layout) Exp1_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC2127899(E_Business_Prox(__in,__cfg).Layout __EE2127715, __ST557433_Layout __EE2127890) := __EEQP(__EE2127715.UID,__EE2127890.____grp___U_I_D_);
  __ST561733_Layout __Join__ST561733_Layout(E_Business_Prox(__in,__cfg).Layout __r, DATASET(__ST557433_Layout) __recs) := TRANSFORM
    SELF := __r;
    SELF.Exp1_ := __CN(__recs);
  END;
  SHARED __EE2128093 := DENORMALIZE(DISTRIBUTE(__EE2127715,HASH(UID)),DISTRIBUTE(__EE2127890,HASH(____grp___U_I_D_)),__JC2127899(LEFT,RIGHT),GROUP,__Join__ST561733_Layout(LEFT,ROWS(RIGHT)),LOCAL);
  SHARED __ST180587_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Contacts_Layout) Contacts_;
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Data_Sources_Layout) Data_Sources_;
    KEL.typ.ndataset(__ST557433_Layout) Best_Business_Prox_Address_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST180587_Layout __ND2128101__Project(__ST561733_Layout __PP2128097) := TRANSFORM
    __EE2128096 := __PP2128097.Exp1_;
    SELF.Best_Business_Prox_Address_ := __EE2128096;
    SELF := __PP2128097;
  END;
  SHARED __EE2128557 := PROJECT(__EE2128093,__ND2128101__Project(LEFT));
  EXPORT __ST557194_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST557194_Layout __ND2127229__Project(__ST180587_Layout __PP2127225) := TRANSFORM
    __EE2128560 := __PP2127225.Best_Business_Prox_Address_;
    SELF.Best_Business_Prox_Address_ := __PROJECT(__EE2128560,E_Prox_Address(__in,__cfg).Best_Addresses_Layout);
    SELF := __PP2127225;
  END;
  EXPORT __ENH_Business_Prox_3 := PROJECT(__EE2128557,__ND2127229__Project(LEFT));
END;

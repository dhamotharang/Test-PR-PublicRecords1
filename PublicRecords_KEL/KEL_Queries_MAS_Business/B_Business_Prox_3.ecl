...
EXPORT B_Business_Prox_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2129792 := __E_Business_Prox;
  SHARED __EE2129803 := __E_Prox_Address;
  SHARED __EE2129813 := __EE2129803(__NN(__EE2129803.Business_Location_));
  SHARED __ST565822_Layout := RECORD
  ...
  END;
  __ST565822_Layout __JT2129821(E_Prox_Address(__in,__cfg).Layout __l, E_Prox_Address(__in,__cfg).Best_Addresses_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE2129890 := NORMALIZE(__EE2129813,__T(LEFT.Best_Addresses_),__JT2129821(LEFT,RIGHT));
  SHARED __ST565422_Layout := RECORD
  ...
  SHARED __ST569722_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST565422_Layout) Exp1_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC2129976(E_Business_Prox(__in,__cfg).Layout __EE2129792, __ST565422_Layout __EE2129967) := __EEQP(__EE2129792.UID,__EE2129967.____grp___U_I_D_);
  __ST569722_Layout __Join__ST569722_Layout(E_Business_Prox(__in,__cfg).Layout __r, DATASET(__ST565422_Layout) __recs) := TRANSFORM
    SELF := __r;
    SELF.Exp1_ := __CN(__recs);
  END;
  SHARED __EE2130170 := DENORMALIZE(DISTRIBUTE(__EE2129792,HASH(UID)),DISTRIBUTE(__EE2129967,HASH(____grp___U_I_D_)),__JC2129976(LEFT,RIGHT),GROUP,__Join__ST569722_Layout(LEFT,ROWS(RIGHT)),LOCAL);
  SHARED __ST192162_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Contacts_Layout) Contacts_;
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Data_Sources_Layout) Data_Sources_;
    KEL.typ.ndataset(__ST565422_Layout) Best_Business_Prox_Address_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST192162_Layout __ND2130178__Project(__ST569722_Layout __PP2130174) := TRANSFORM
    __EE2130173 := __PP2130174.Exp1_;
    SELF.Best_Business_Prox_Address_ := __EE2130173;
    SELF := __PP2130174;
  END;
  SHARED __EE2130634 := PROJECT(__EE2130170,__ND2130178__Project(LEFT));
  EXPORT __ST565183_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST565183_Layout __ND2129306__Project(__ST192162_Layout __PP2129302) := TRANSFORM
    __EE2130637 := __PP2129302.Best_Business_Prox_Address_;
    SELF.Best_Business_Prox_Address_ := __PROJECT(__EE2130637,E_Prox_Address(__in,__cfg).Best_Addresses_Layout);
    SELF := __PP2129302;
  END;
  EXPORT __ENH_Business_Prox_3 := PROJECT(__EE2130634,__ND2129306__Project(LEFT));
END;

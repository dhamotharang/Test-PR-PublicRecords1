...
EXPORT B_Business_Sele_Overflow_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE359061 := __E_Business_Sele_Overflow;
  EXPORT __ST219484_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST219484_Layout __ND1934848__Project(E_Business_Sele_Overflow(__in,__cfg).Layout __PP358593) := TRANSFORM
    __EE358904 := __PP358593.S_O_S_Incorporation_Details_;
    __BS360438 := __T(__EE358904);
    __EE1934846 := __BS360438(__T(__AND(__AND(__NOT(__NT(__T(__EE358904).S_O_S_Incorporation_State_)),__OP2(__T(__EE358904).S_O_S_Incorporation_State_,<>,__CN(''))),__OP2(__T(__EE358904).S_O_S_Foreign_Domestic_Indicator_,=,__CN('D')))));
    SELF.S_O_S_Domestic_Filing_ := __CN(__EE1934846);
    __EE358935 := __PP358593.S_O_S_Incorporation_Details_;
    __BS360640 := __T(__EE358935);
    __EE1934856 := __BS360640(__T(__AND(__AND(__NOT(__NT(__T(__EE358935).S_O_S_Foreign_State_Code_)),__OP2(__T(__EE358935).S_O_S_Foreign_State_Code_,<>,__CN(''))),__OP2(__T(__EE358935).S_O_S_Foreign_Domestic_Indicator_,=,__CN('F')))));
    SELF.S_O_S_Foreign_Filing_ := __CN(__EE1934856);
    SELF := __PP358593;
  END;
  EXPORT __ENH_Business_Sele_Overflow_6 := PROJECT(__EE359061,__ND1934848__Project(LEFT));
END;

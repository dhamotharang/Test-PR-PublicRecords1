...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE315083 := __E_Education;
  EXPORT __ST216506_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST216506_Layout __ND1904093__Project(E_Education(__in,__cfg).Layout __PP314957) := TRANSFORM
    __BS315033 := __T(__PP314957.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS315033(__T(__OP2(__T(__PP314957.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP314957;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE315083,__ND1904093__Project(LEFT));
END;

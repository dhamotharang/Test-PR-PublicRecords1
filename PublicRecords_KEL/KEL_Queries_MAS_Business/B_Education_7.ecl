...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE315696 := __E_Education;
  ...
  END;
  SHARED __ST217119_Layout __ND1904405__Project(E_Education(__in,__cfg).Layout __PP315570) := TRANSFORM
    __BS315646 := __T(__PP315570.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS315646(__T(__OP2(__T(__PP315570.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP315570;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE315696,__ND1904405__Project(LEFT));
END;

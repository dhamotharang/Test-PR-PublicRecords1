...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE315956 := __E_Education;
  EXPORT __ST217379_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST217379_Layout __ND1906064__Project(E_Education(__in,__cfg).Layout __PP315830) := TRANSFORM
    __BS315906 := __T(__PP315830.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS315906(__T(__OP2(__T(__PP315830.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP315830;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE315956,__ND1906064__Project(LEFT));
END;

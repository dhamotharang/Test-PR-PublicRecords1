...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE304503 := __E_Education;
  EXPORT __ST205908_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST205908_Layout __ND1902030__Project(E_Education(__in,__cfg).Layout __PP304377) := TRANSFORM
    __BS304453 := __T(__PP304377.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS304453(__T(__OP2(__T(__PP304377.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP304377;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE304503,__ND1902030__Project(LEFT));
END;

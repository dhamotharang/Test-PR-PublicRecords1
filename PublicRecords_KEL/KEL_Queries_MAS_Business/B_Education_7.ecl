...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE314902 := __E_Education;
  EXPORT __ST216325_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST216325_Layout __ND1903548__Project(E_Education(__in,__cfg).Layout __PP314776) := TRANSFORM
    __BS314852 := __T(__PP314776.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS314852(__T(__OP2(__T(__PP314776.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP314776;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE314902,__ND1903548__Project(LEFT));
END;

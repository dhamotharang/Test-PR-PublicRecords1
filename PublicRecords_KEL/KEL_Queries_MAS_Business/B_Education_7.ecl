...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE320985 := __E_Education;
  EXPORT __ST222402_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST222402_Layout __ND1910023__Project(E_Education(__in,__cfg).Layout __PP320859) := TRANSFORM
    __BS320935 := __T(__PP320859.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS320935(__T(__OP2(__T(__PP320859.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP320859;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE320985,__ND1910023__Project(LEFT));
END;

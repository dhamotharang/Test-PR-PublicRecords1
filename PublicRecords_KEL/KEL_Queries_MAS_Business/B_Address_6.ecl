...
EXPORT B_Address_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE320011 := __E_Address;
  EXPORT __ST206214_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST206214_Layout __ND323708__Project(E_Address(__in,__cfg).Layout __PP319297) := TRANSFORM
    __EE319800 := __PP319297.A_D_V_O_Summary_;
    __BS320158 := __T(__EE319800);
    __EE320178 := __BN(TOPN(__BS320158(__NN(__T(__EE319800).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE319800).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE319800));
    SELF.Bestchild_Advo_ := __EE320178;
    SELF := __PP319297;
  END;
  EXPORT __ENH_Address_6 := PROJECT(__EE320011,__ND323708__Project(LEFT));
END;

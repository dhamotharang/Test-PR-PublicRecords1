...
EXPORT B_Address_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE325851 := __E_Address;
  EXPORT __ST212108_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST212108_Layout __ND328755__Project(E_Address(__in,__cfg).Layout __PP325212) := TRANSFORM
    __EE325656 := __PP325212.A_D_V_O_Summary_;
    __BS325982 := __T(__EE325656);
    __EE326002 := __BN(TOPN(__BS325982(__NN(__T(__EE325656).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE325656).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE325656));
    SELF.Bestchild_Advo_ := __EE326002;
    SELF := __PP325212;
  END;
  EXPORT __ENH_Address_6 := PROJECT(__EE325851,__ND328755__Project(LEFT));
END;

...
EXPORT B_Address_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE320913 := __E_Address;
  EXPORT __ST207170_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST207170_Layout __ND323817__Project(E_Address(__in,__cfg).Layout __PP320274) := TRANSFORM
    __EE320718 := __PP320274.A_D_V_O_Summary_;
    __BS321044 := __T(__EE320718);
    __EE321064 := __BN(TOPN(__BS321044(__NN(__T(__EE320718).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE320718).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE320718));
    SELF.Bestchild_Advo_ := __EE321064;
    SELF := __PP320274;
  END;
  EXPORT __ENH_Address_6 := PROJECT(__EE320913,__ND323817__Project(LEFT));
END;

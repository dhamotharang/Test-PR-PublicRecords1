...
EXPORT B_Address_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  END;
  SHARED __ST213906_Layout __ND330563__Project(E_Address(__in,__cfg).Layout __PP327020) := TRANSFORM
    __EE327464 := __PP327020.A_D_V_O_Summary_;
    __BS327790 := __T(__EE327464);
    __EE327810 := __BN(TOPN(__BS327790(__NN(__T(__EE327464).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE327464).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE327464));
    SELF.Bestchild_Advo_ := __EE327810;
    SELF := __PP327020;
  END;
  EXPORT __ENH_Address_6 := PROJECT(__EE327659,__ND330563__Project(LEFT));
END;

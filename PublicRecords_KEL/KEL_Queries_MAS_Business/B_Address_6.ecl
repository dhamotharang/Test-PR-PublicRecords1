...
EXPORT B_Address_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE326032 := __E_Address;
  EXPORT __ST212285_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST212285_Layout __ND328936__Project(E_Address(__in,__cfg).Layout __PP325393) := TRANSFORM
    __EE325837 := __PP325393.A_D_V_O_Summary_;
    __BS326163 := __T(__EE325837);
    __EE326183 := __BN(TOPN(__BS326163(__NN(__T(__EE325837).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE325837).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE325837));
    SELF.Bestchild_Advo_ := __EE326183;
    SELF := __PP325393;
  END;
  EXPORT __ENH_Address_6 := PROJECT(__EE326032,__ND328936__Project(LEFT));
END;

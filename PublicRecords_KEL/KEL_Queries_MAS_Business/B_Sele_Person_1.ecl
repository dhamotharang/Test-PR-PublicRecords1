...
EXPORT B_Sele_Person_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE3215952 := __ENH_Sele_Person_2;
  EXPORT __ST181056_Layout := RECORD
  ...
  END;
  SHARED __ST181056_Layout __ND3216087__Project(B_Sele_Person_2(__in,__cfg).__ST191594_Layout __PP3215953) := TRANSFORM
    __CC13988 := '-99997';
    SELF.Assoc_Valid_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP3215953.Assoc_Fs_Date_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP3215953.Assoc_Fs_Date_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13988)));
    SELF := __PP3215953;
  END;
  EXPORT __ENH_Sele_Person_1 := PROJECT(__EE3215952,__ND3216087__Project(LEFT));
END;

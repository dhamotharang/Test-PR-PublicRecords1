...
EXPORT B_Sele_Person_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE3210546 := __ENH_Sele_Person_2;
  EXPORT __ST174083_Layout := RECORD
  ...
  END;
  SHARED __ST174083_Layout __ND3210681__Project(B_Sele_Person_2(__in,__cfg).__ST184654_Layout __PP3210547) := TRANSFORM
    __CC14060 := '-99997';
    SELF.Assoc_Valid_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP3210547.Assoc_Fs_Date_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP3210547.Assoc_Fs_Date_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14060)));
    SELF := __PP3210547;
  END;
  EXPORT __ENH_Sele_Person_1 := PROJECT(__EE3210546,__ND3210681__Project(LEFT));
END;

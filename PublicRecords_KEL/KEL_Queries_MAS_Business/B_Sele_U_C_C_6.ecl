...
EXPORT B_Sele_U_C_C_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1937970 := __ENH_Sele_U_C_C_7;
  EXPORT __ST210042_Layout := RECORD
  ...
  END;
  EXPORT __ST210034_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST210042_Layout) Sub_Filing_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST210034_Layout __ND1938075__Project(B_Sele_U_C_C_7(__in,__cfg).__ST212244_Layout __PP1937971) := TRANSFORM
    __EE1938006 := __PP1937971.Sub_Filing_;
    SELF.Sub_Filing_ := __BN(PROJECT(__T(__EE1938006),__ST210042_Layout),__NL(__EE1938006));
    SELF.Is_Filing_ := IF(__T(__OP2(__FN1(KEL.Routines.ToUpperCase,__FN1(TRIM,__PP1937971.Filtered_Party_Type_)),IN,__CN(['D','S','C','A']))),TRUE,FALSE);
    SELF := __PP1937971;
  END;
  EXPORT __ENH_Sele_U_C_C_6 := PROJECT(__EE1937970,__ND1938075__Project(LEFT));
END;

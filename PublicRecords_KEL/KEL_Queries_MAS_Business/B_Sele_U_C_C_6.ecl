...
EXPORT B_Sele_U_C_C_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1942297 := __ENH_Sele_U_C_C_7;
  EXPORT __ST215774_Layout := RECORD
  ...
  END;
  EXPORT __ST215766_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST215774_Layout) Sub_Filing_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST215766_Layout __ND1942402__Project(B_Sele_U_C_C_7(__in,__cfg).__ST217976_Layout __PP1942298) := TRANSFORM
    __EE1942333 := __PP1942298.Sub_Filing_;
    SELF.Sub_Filing_ := __BN(PROJECT(__T(__EE1942333),__ST215774_Layout),__NL(__EE1942333));
    SELF.Is_Filing_ := IF(__T(__OP2(__FN1(KEL.Routines.ToUpperCase,__FN1(TRIM,__PP1942298.Filtered_Party_Type_)),IN,__CN(['D','S','C','A']))),TRUE,FALSE);
    SELF := __PP1942298;
  END;
  EXPORT __ENH_Sele_U_C_C_6 := PROJECT(__EE1942297,__ND1942402__Project(LEFT));
END;

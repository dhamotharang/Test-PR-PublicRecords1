...
EXPORT B_Criminal_Offense_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1991031 := __ENH_Criminal_Offense_6;
  EXPORT __ST215953_Layout := RECORD
  ...
  END;
  EXPORT __ST215906_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST215953_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST215906_Layout __ND1990951__Project(B_Criminal_Offense_6(__in,__cfg).__ST219664_Layout __PP1990418) := TRANSFORM
    __EE1991034 := __PP1990418.Offense_Charges_;
    __ST215953_Layout __ND1990915__Project(B_Criminal_Offense_6(__in,__cfg).__ST219711_Layout __PP1990579) := TRANSFORM
      __EE1990910 := __PP1990418.Offense_Charges_;
      SELF.Criminal_Date_ := KEL.Aggregates.MinNN(__EE1990910,KEL.Routines.CastStringToDate(__T(__EE1990910).Valid_Date_));
      SELF.Dismissed_Charges_ := __OR(IF(__T(__NT(__PP1990418.Court_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP1990418.Court_Disposition_Description_,=,__CN('DISMISSED')))),IF(__T(__NT(__PP1990418.Court_Additional_Disposition_Description_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP1990418.Court_Additional_Disposition_Description_,=,__CN('DISMISSED')))));
      SELF := __PP1990579;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE1991034,__ND1990915__Project(LEFT));
    SELF := __PP1990418;
  END;
  EXPORT __ENH_Criminal_Offense_5 := PROJECT(__EE1991031,__ND1990951__Project(LEFT));
END;

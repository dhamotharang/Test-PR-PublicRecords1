...
EXPORT B_Sele_Phone_Number_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE367112 := __E_Sele_Phone_Number;
  SHARED __EE1940836 := __ENH_Business_Sele_7;
  SHARED __ST367256_Layout := RECORD
  ...
  END;
  __JC1940842(E_Sele_Phone_Number(__in,__cfg).Layout __EE367112, B_Business_Sele_7(__in,__cfg).__ST217706_Layout __EE1940836) := __EEQP(__EE367112.Legal_,__EE1940836.UID);
  __ST367256_Layout __JT1940842(E_Sele_Phone_Number(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST217706_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE1940843 := JOIN(__EE367112,__EE1940836,__JC1940842(LEFT,RIGHT),__JT1940842(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST216667_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST216629_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Phone().Typ) Phone_Number_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Best_Phone_Details_Layout) Best_Phone_Details_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Phone_Details_Layout) Phone_Details_;
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).S_I_C_Codes_Layout) S_I_C_Codes_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(__ST216667_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST216629_Layout __ND1941202__Project(__ST367256_Layout __PP1940844) := TRANSFORM
    __EE1941251 := __PP1940844.Data_Sources_;
    __ST216667_Layout __ND1941256__Project(E_Sele_Phone_Number(__in,__cfg).Data_Sources_Layout __PP1941252) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP1941252.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP1941252.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP1941252.Source_));
      SELF := __PP1941252;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE1941251,__ND1941256__Project(LEFT));
    SELF.Input_Phone_Match_ := FN_Compile(__cfg).FN_Is_Phone_Match(__ECAST(KEL.typ.nstr,__ECAST(KEL.typ.nstr,__PP1940844.Phone_Number_)),__ECAST(KEL.typ.nstr,__PP1940844.B___Inp_Cln_Phone_));
    SELF := __PP1940844;
  END;
  EXPORT __ENH_Sele_Phone_Number_6 := PROJECT(__EE1940843,__ND1941202__Project(LEFT));
END;

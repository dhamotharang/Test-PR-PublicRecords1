...
EXPORT B_Sele_Phone_Number_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE365485 := __E_Sele_Phone_Number;
  SHARED __EE1939181 := __ENH_Business_Sele_7;
  ...
  END;
  __JC1939187(E_Sele_Phone_Number(__in,__cfg).Layout __EE365485, B_Business_Sele_7(__in,__cfg).__ST216085_Layout __EE1939181) := __EEQP(__EE365485.Legal_,__EE1939181.UID);
  __ST365629_Layout __JT1939187(E_Sele_Phone_Number(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST216085_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE1939188 := JOIN(__EE365485,__EE1939181,__JC1939187(LEFT,RIGHT),__JT1939187(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST215046_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST215008_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Phone().Typ) Phone_Number_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Best_Phone_Details_Layout) Best_Phone_Details_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Phone_Details_Layout) Phone_Details_;
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).S_I_C_Codes_Layout) S_I_C_Codes_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(__ST215046_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST215008_Layout __ND1939547__Project(__ST365629_Layout __PP1939189) := TRANSFORM
    __EE1939596 := __PP1939189.Data_Sources_;
    __ST215046_Layout __ND1939601__Project(E_Sele_Phone_Number(__in,__cfg).Data_Sources_Layout __PP1939597) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP1939597.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP1939597.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP1939597.Source_));
      SELF := __PP1939597;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE1939596,__ND1939601__Project(LEFT));
    SELF.Input_Phone_Match_ := FN_Compile(__cfg).FN_Is_Phone_Match(__ECAST(KEL.typ.nstr,__ECAST(KEL.typ.nstr,__PP1939189.Phone_Number_)),__ECAST(KEL.typ.nstr,__PP1939189.B___Inp_Cln_Phone_));
    SELF := __PP1939189;
  END;
  EXPORT __ENH_Sele_Phone_Number_6 := PROJECT(__EE1939188,__ND1939547__Project(LEFT));
END;

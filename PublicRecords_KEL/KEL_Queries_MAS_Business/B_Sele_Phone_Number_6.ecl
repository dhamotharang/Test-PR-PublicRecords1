...
EXPORT B_Sele_Phone_Number_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE365828 := __E_Sele_Phone_Number;
  SHARED __EE1939257 := __ENH_Business_Sele_7;
  SHARED __ST365972_Layout := RECORD
  ...
  END;
  __JC1939263(E_Sele_Phone_Number(__in,__cfg).Layout __EE365828, B_Business_Sele_7(__in,__cfg).__ST216653_Layout __EE1939257) := __EEQP(__EE365828.Legal_,__EE1939257.UID);
  __ST365972_Layout __JT1939263(E_Sele_Phone_Number(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST216653_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE1939264 := JOIN(__EE365828,__EE1939257,__JC1939263(LEFT,RIGHT),__JT1939263(LEFT,RIGHT),LEFT OUTER,HASH);
  ...
  EXPORT __ST215576_Layout := RECORD
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).S_I_C_Codes_Layout) S_I_C_Codes_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(__ST215614_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST215576_Layout __ND1939623__Project(__ST365972_Layout __PP1939265) := TRANSFORM
    __EE1939672 := __PP1939265.Data_Sources_;
    __ST215614_Layout __ND1939677__Project(E_Sele_Phone_Number(__in,__cfg).Data_Sources_Layout __PP1939673) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP1939673.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP1939673.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP1939673.Source_));
      SELF := __PP1939673;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE1939672,__ND1939677__Project(LEFT));
    SELF.Input_Phone_Match_ := FN_Compile(__cfg).FN_Is_Phone_Match(__ECAST(KEL.typ.nstr,__ECAST(KEL.typ.nstr,__PP1939265.Phone_Number_)),__ECAST(KEL.typ.nstr,__PP1939265.B___Inp_Cln_Phone_));
    SELF := __PP1939265;
  END;
  EXPORT __ENH_Sele_Phone_Number_6 := PROJECT(__EE1939264,__ND1939623__Project(LEFT));
END;

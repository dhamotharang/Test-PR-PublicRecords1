...
EXPORT B_Input_B_I_I_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE2856199 := __ENH_Input_B_I_I_3;
  SHARED __EE2856202 := __ENH_Input_P_I_I_3;
  SHARED __EE1077718 := __E_Input_B_I_I_Input_P_I_I;
  SHARED __EE2859632 := __EE1077718(__NN(__EE1077718.B_I_I_) AND __NN(__EE1077718.P_I_I_));
  SHARED __ST1082386_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  __JC2859650(B_Input_P_I_I_3(__in,__cfg).__ST187734_Layout __EE2856202, E_Input_B_I_I_Input_P_I_I(__in,__cfg).Layout __EE2859632) := __EEQP(__EE2859632.P_I_I_,__EE2856202.UID);
  __ST1082386_Layout __JT2859650(B_Input_P_I_I_3(__in,__cfg).__ST187734_Layout __l, E_Input_B_I_I_Input_P_I_I(__in,__cfg).Layout __r) := TRANSFORM
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE2859651 := JOIN(__EE2859632,__EE2856202,__JC2859650(RIGHT,LEFT),__JT2859650(RIGHT,LEFT),INNER,SMART);
  SHARED __ST1081183_Layout := RECORD
    ...
    KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
  END;
  SHARED __ST1081183_Layout __ND2859811__Project(__ST1082386_Layout __PP2859652) := TRANSFORM
    SELF.UID := __PP2859652.B_I_I_;
    SELF.U_I_D__1_ := __PP2859652.UID;
    SELF := __PP2859652;
  END;
  SHARED __EE2860432 := PROJECT(__EE2859651,__ND2859811__Project(LEFT));
  SHARED __ST1081533_Layout := RECORD
    ...
    KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
  END;
  SHARED __ST1081533_Layout __ND2861083__Project(__ST1081183_Layout __PP2860433) := TRANSFORM
    SELF.Exp1_ := IF(__T(__OP2(__PP2860433.Rep_Number_,=,__CN(1))),__ECAST(KEL.typ.nuid,__PP2860433.U_I_D__1_),__ECAST(KEL.typ.nuid,__N(KEL.typ.uid)));
    SELF.Exp2_ := IF(__T(__OP2(__PP2860433.Rep_Number_,=,__CN(2))),__ECAST(KEL.typ.nuid,__PP2860433.U_I_D__1_),__ECAST(KEL.typ.nuid,__N(KEL.typ.uid)));
    SELF.Exp3_ := IF(__T(__OP2(__PP2860433.Rep_Number_,=,__CN(3))),__ECAST(KEL.typ.nuid,__PP2860433.U_I_D__1_),__ECAST(KEL.typ.nuid,__N(KEL.typ.uid)));
    SELF.Exp4_ := IF(__T(__OP2(__PP2860433.Rep_Number_,=,__CN(4))),__ECAST(KEL.typ.nuid,__PP2860433.U_I_D__1_),__ECAST(KEL.typ.nuid,__N(KEL.typ.uid)));
    SELF.Exp5_ := IF(__T(__OP2(__PP2860433.Rep_Number_,=,__CN(5))),__ECAST(KEL.typ.nuid,__PP2860433.U_I_D__1_),__ECAST(KEL.typ.nuid,__N(KEL.typ.uid)));
    SELF := __PP2860433;
  END;
  SHARED __EE2861103 := PROJECT(__EE2860432,__ND2861083__Project(LEFT));
  SHARED __ST1081568_Layout := RECORD
    ...
    KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
  END;
  SHARED __EE2861139 := PROJECT(__CLEANANDDO(__EE2861103,TABLE(__EE2861103,{KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),KEL.Aggregates.MaxNG(__EE2861103.Exp1_) O_N_L_Y___U_I_D_,KEL.Aggregates.MaxNG(__EE2861103.Exp2_) O_N_L_Y___U_I_D__1_,KEL.Aggregates.MaxNG(__EE2861103.Exp3_) O_N_L_Y___U_I_D__2_,KEL.Aggregates.MaxNG(__EE2861103.Exp4_) O_N_L_Y___U_I_D__3_,KEL.Aggregates.MaxNG(__EE2861103.Exp5_) O_N_L_Y___U_I_D__4_,UID},UID,MERGE)),__ST1081568_Layout);
  SHARED __ST1083336_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC2861145(B_Input_B_I_I_3(__in,__cfg).__ST187203_Layout __EE2856199, __ST1081568_Layout __EE2861139) := __EEQP(__EE2856199.UID,__EE2861139.UID);
  __ST1083336_Layout __JT2861145(B_Input_B_I_I_3(__in,__cfg).__ST187203_Layout __l, __ST1081568_Layout __r) := TRANSFORM
    SELF.U_I_D__1_ := __r.UID;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE2861248 := JOIN(__EE2856199,__EE2861139,__JC2861145(LEFT,RIGHT),__JT2861145(LEFT,RIGHT),LEFT OUTER,SMART);
  EXPORT __ST177302_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.nint G___Proc_Bus_U_I_D_;
    KEL.typ.nint B___Inp_Lex_I_D_Ult_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST177302_Layout __ND2861253__Project(__ST1083336_Layout __PP2861249) := TRANSFORM
    SELF.Auth_Rep1_ := __PP2861249.O_N_L_Y___U_I_D_;
    SELF.Auth_Rep2_ := __PP2861249.O_N_L_Y___U_I_D__1_;
    SELF.Auth_Rep3_ := __PP2861249.O_N_L_Y___U_I_D__2_;
    SELF.Auth_Rep4_ := __PP2861249.O_N_L_Y___U_I_D__3_;
    SELF.Auth_Rep5_ := __PP2861249.O_N_L_Y___U_I_D__4_;
    __CC13602 := '-99999';
    __CC13607 := '-99998';
    SELF.Bus_Input_Alternate_Name_Clean_Value_ := FN_Compile(__cfg).FN_Is_Blank2_Fields(__ECAST(KEL.typ.nstr,__PP2861249.B___Inp_Alt_Name_),__ECAST(KEL.typ.nstr,__CN(__CC13602)),__ECAST(KEL.typ.nstr,__PP2861249.B___Inp_Cln_Alt_Name_),__ECAST(KEL.typ.nstr,__CN(__CC13607)));
    SELF.Bus_Input_Email_Clean_Value_ := FN_Compile(__cfg).FN_Is_Blank2_Fields(__ECAST(KEL.typ.nstr,__PP2861249.B___Inp_Email_),__ECAST(KEL.typ.nstr,__CN(__CC13602)),__ECAST(KEL.typ.nstr,__PP2861249.B___Inp_Cln_Email_),__ECAST(KEL.typ.nstr,__CN(__CC13607)));
    SELF.Bus_Input_Email_Echo_Value_ := FN_Compile(__cfg).FN_Is_Blank(__ECAST(KEL.typ.nstr,__PP2861249.B___Inp_Email_),__ECAST(KEL.typ.nstr,__CN(__CC13602)));
    SELF.Bus_Input_I_P_Address_Echo_Value_ := FN_Compile(__cfg).FN_Is_Blank(__ECAST(KEL.typ.nstr,__PP2861249.B___Inp_I_P_Addr_),__ECAST(KEL.typ.nstr,__CN(__CC13602)));
    SELF.Bus_Input_State_Echo_Value_ := FN_Compile(__cfg).FN_Is_Blank(__ECAST(KEL.typ.nstr,__PP2861249.B___Inp_Addr_State_),__ECAST(KEL.typ.nstr,__CN(__CC13602)));
    SELF.Bus_Input_T_I_N_Echo_Value_ := FN_Compile(__cfg).FN_Is_Blank(__ECAST(KEL.typ.nstr,__PP2861249.B___Inp_T_I_N_),__ECAST(KEL.typ.nstr,__CN(__CC13602)));
    SELF.Bus_Input_Zip_Echo_Value_ := FN_Compile(__cfg).FN_Is_Blank(__ECAST(KEL.typ.nstr,__PP2861249.B___Inp_Addr_Zip_),__ECAST(KEL.typ.nstr,__CN(__CC13602)));
    SELF.Clean_Email_Username_ := FN_Compile(__cfg).FN_Get_Clean_Email_Username(__ECAST(KEL.typ.nstr,__PP2861249.B___Inp_Email_));
    SELF.Email_Domain_Length_ := __FN1(LENGTH,__PP2861249.Email_Domain_);
    SELF.Email_Username_Length_ := __FN1(LENGTH,__PP2861249.Email_Username_);
    SELF := __PP2861249;
  END;
  EXPORT __ENH_Input_B_I_I_2 := PROJECT(__EE2861248,__ND2861253__Project(LEFT));
END;

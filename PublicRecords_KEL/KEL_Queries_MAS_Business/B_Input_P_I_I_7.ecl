...
IMPORT KEL16 AS KEL;
IMPORT CFG_Compile,E_Address,E_Address_Slim,E_Address_Summary,E_Email,E_Geo_Link,E_Input_P_I_I,E_Name_Summary,E_Person,E_Phone,E_Phone_Summary,E_Property,E_S_S_N_Summary,E_Social_Security_Number,E_Surname,E_Zip_Code FROM PublicRecords_KEL.KEL_Queries_MAS_Business;
...
EXPORT B_Input_P_I_I_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE316572 := __E_Input_P_I_I;
  EXPORT __ST216726_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST216726_Layout __ND1904156__Project(E_Input_P_I_I(__in,__cfg).Layout __PP315892) := TRANSFORM
    SELF.P___Inp_Addr_ := __OP2(IF(__T(__NT(__PP315892.P___Inp_Addr_Line1_)),__ECAST(KEL.typ.nstr,__CN('')),__ECAST(KEL.typ.nstr,__OP2(__PP315892.P___Inp_Addr_Line1_,+,__CN(' ')))),+,IF(__T(__NT(__PP315892.P___Inp_Addr_Line2_)),__ECAST(KEL.typ.nstr,__CN('')),__ECAST(KEL.typ.nstr,__PP315892.P___Inp_Addr_Line2_)));
    SELF := __PP315892;
  END;
  EXPORT __ENH_Input_P_I_I_7 := PROJECT(__EE316572,__ND1904156__Project(LEFT));
END;

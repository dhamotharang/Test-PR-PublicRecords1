...
EXPORT B_Input_P_I_I_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE317321 := __E_Input_P_I_I;
  EXPORT __ST217475_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST217475_Layout __ND1904968__Project(E_Input_P_I_I(__in,__cfg).Layout __PP316641) := TRANSFORM
    SELF.P___Inp_Addr_ := __OP2(IF(__T(__NT(__PP316641.P___Inp_Addr_Line1_)),__ECAST(KEL.typ.nstr,__CN('')),__ECAST(KEL.typ.nstr,__OP2(__PP316641.P___Inp_Addr_Line1_,+,__CN(' ')))),+,IF(__T(__NT(__PP316641.P___Inp_Addr_Line2_)),__ECAST(KEL.typ.nstr,__CN('')),__ECAST(KEL.typ.nstr,__PP316641.P___Inp_Addr_Line2_)));
    SELF := __PP316641;
  END;
  EXPORT __ENH_Input_P_I_I_7 := PROJECT(__EE317321,__ND1904968__Project(LEFT));
END;

...
EXPORT B_Input_B_I_I_8(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1895302 := __ENH_Input_B_I_I_9;
  EXPORT __ST220828_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST220828_Layout __ND1895608__Project(B_Input_B_I_I_9(__in,__cfg).__ST221868_Layout __PP1895303) := TRANSFORM
    SELF.Addr_Not_Populated_ := FN_Compile(__cfg).FN_Is_Not_Enough_To_Clean(__ECAST(KEL.typ.nstr,__PP1895303.B___Inp_Addr_));
    __CC13690 := '-99999';
    __CC13695 := '-99998';
    SELF.Bus_Input_Phone_Clean_Value_ := FN_Compile(__cfg).FN_Is_Blank2_Fields(__ECAST(KEL.typ.nstr,__PP1895303.B___Inp_Phone_),__ECAST(KEL.typ.nstr,__CN(__CC13690)),__ECAST(KEL.typ.nstr,__PP1895303.B___Inp_Cln_Phone_),__ECAST(KEL.typ.nstr,__CN(__CC13695)));
    SELF.Bus_Input_T_I_N_Clean_Value_ := FN_Compile(__cfg).FN_Is_Blank2_Fields(__ECAST(KEL.typ.nstr,__PP1895303.B___Inp_T_I_N_),__ECAST(KEL.typ.nstr,__CN(__CC13690)),__ECAST(KEL.typ.nstr,__PP1895303.B___Inp_Cln_T_I_N_),__ECAST(KEL.typ.nstr,__CN(__CC13695)));
    SELF.City_State_Zip_Not_Populated_ := FN_Compile(__cfg).FN_City_State_Zip_Not_Populated_Check(__ECAST(KEL.typ.nstr,__PP1895303.B___Inp_Addr_City_),__ECAST(KEL.typ.nstr,__PP1895303.B___Inp_Addr_State_),__ECAST(KEL.typ.nstr,__PP1895303.B___Inp_Addr_Zip_));
    SELF := __PP1895303;
  END;
  EXPORT __ENH_Input_B_I_I_8 := PROJECT(__EE1895302,__ND1895608__Project(LEFT));
END;

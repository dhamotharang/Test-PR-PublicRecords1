...
EXPORT B_Sele_Person_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1907524 := __E_Sele_Person;
  EXPORT __ST206699_Layout := RECORD
  ...
  END;
  SHARED __ST206699_Layout __ND1907488__Project(E_Sele_Person(__in,__cfg).Layout __PP312782) := TRANSFORM
    __EE1907463 := __PP312782.Contact_Info_;
    ...
    __EE1907483 := __PP312782.Contact_Info_;
    SELF.Assoc_Date_ := IF(__T(__OP2(KEL.Aggregates.MaxNN(__EE1907463,KEL.era.ToDate(__T(__EE1907463).Date_Last_Seen_)),>,__CC13458)),__ECAST(KEL.typ.nkdate,__CC13458),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MaxNN(__EE1907483,KEL.era.ToDate(__T(__EE1907483).Date_Last_Seen_))));
    SELF := __PP312782;
  END;
  EXPORT __ENH_Sele_Person_7 := PROJECT(__EE1907524,__ND1907488__Project(LEFT));
END;

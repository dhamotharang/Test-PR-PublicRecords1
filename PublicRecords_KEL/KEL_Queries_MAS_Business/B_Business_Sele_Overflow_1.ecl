...
EXPORT B_Business_Sele_Overflow_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE3159882 := __ENH_Business_Sele_Overflow_2;
  EXPORT __ST165919_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST165919_Layout __ND3159896__Project(B_Business_Sele_Overflow_2(__in,__cfg).__ST176607_Layout __PP3159167) := TRANSFORM
    __EE3159712 := __PP3159167.S_O_S_Statuses_;
    __EE3159727 := __PP3159167.S_O_S_Statuses_;
    __ST94310_Layout := RECORD
      ...
      KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
    END;
    __EE3159737 := PROJECT(TABLE(PROJECT(__T(__EE3159727),__ST94310_Layout),{KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),Days_Since_Status_},Days_Since_Status_,MERGE),__ST94310_Layout);
    __BS3159713 := __T(__EE3159712);
    __EE3159888 := __BS3159713(__T(__OP2(__T(__EE3159712).Days_Since_Status_,=,KEL.Aggregates.MinN(__EE3159737,__EE3159737.Days_Since_Status_))));
    SELF.Most_Recent_Status_Filing_Description_ := (__EE3159888)[1].S_O_S_Status_Description_;
    SELF.S_O_S_Unique_State_Count_ := __OP2(__OP2(KEL.Aggregates.CountN(__PP3159167.S_O_S_Foreign_States_),+,KEL.Aggregates.CountN(__PP3159167.S_O_S_Domestic_States_)),-,__CN(__PP3159167.S_O_S_Filing_States_Count_Dups_));
    SELF := __PP3159167;
  END;
  EXPORT __ENH_Business_Sele_Overflow_1 := PROJECT(__EE3159882,__ND3159896__Project(LEFT));
END;

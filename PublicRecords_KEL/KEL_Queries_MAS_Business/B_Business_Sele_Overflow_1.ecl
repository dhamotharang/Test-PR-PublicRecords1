...
EXPORT B_Business_Sele_Overflow_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE3157777 := __ENH_Business_Sele_Overflow_2;
  EXPORT __ST177500_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST177500_Layout __ND3157791__Project(B_Business_Sele_Overflow_2(__in,__cfg).__ST188174_Layout __PP3157062) := TRANSFORM
    __EE3157607 := __PP3157062.S_O_S_Statuses_;
    __EE3157622 := __PP3157062.S_O_S_Statuses_;
    __ST100656_Layout := RECORD
      ...
      KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
    END;
    __EE3157632 := PROJECT(TABLE(PROJECT(__T(__EE3157622),__ST100656_Layout),{KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),Days_Since_Status_},Days_Since_Status_,MERGE),__ST100656_Layout);
    __BS3157608 := __T(__EE3157607);
    __EE3157783 := __BS3157608(__T(__OP2(__T(__EE3157607).Days_Since_Status_,=,KEL.Aggregates.MinN(__EE3157632,__EE3157632.Days_Since_Status_))));
    SELF.Most_Recent_Status_Filing_Description_ := (__EE3157783)[1].S_O_S_Status_Description_;
    SELF.S_O_S_Unique_State_Count_ := __OP2(__OP2(KEL.Aggregates.CountN(__PP3157062.S_O_S_Foreign_States_),+,KEL.Aggregates.CountN(__PP3157062.S_O_S_Domestic_States_)),-,__CN(__PP3157062.S_O_S_Filing_States_Count_Dups_));
    SELF := __PP3157062;
  END;
  EXPORT __ENH_Business_Sele_Overflow_1 := PROJECT(__EE3157777,__ND3157791__Project(LEFT));
END;

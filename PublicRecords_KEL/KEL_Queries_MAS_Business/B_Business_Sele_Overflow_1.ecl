...
EXPORT B_Business_Sele_Overflow_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE3154093 := __ENH_Business_Sele_Overflow_2;
  EXPORT __ST171548_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST171548_Layout __ND3154107__Project(B_Business_Sele_Overflow_2(__in,__cfg).__ST182222_Layout __PP3153378) := TRANSFORM
    __EE3153923 := __PP3153378.S_O_S_Statuses_;
    __EE3153938 := __PP3153378.S_O_S_Statuses_;
    __ST97392_Layout := RECORD
      ...
      KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
    END;
    __EE3153948 := PROJECT(TABLE(PROJECT(__T(__EE3153938),__ST97392_Layout),{KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),Days_Since_Status_},Days_Since_Status_,MERGE),__ST97392_Layout);
    __BS3153924 := __T(__EE3153923);
    __EE3154099 := __BS3153924(__T(__OP2(__T(__EE3153923).Days_Since_Status_,=,KEL.Aggregates.MinN(__EE3153948,__EE3153948.Days_Since_Status_))));
    SELF.Most_Recent_Status_Filing_Description_ := (__EE3154099)[1].S_O_S_Status_Description_;
    SELF.S_O_S_Unique_State_Count_ := __OP2(__OP2(KEL.Aggregates.CountN(__PP3153378.S_O_S_Foreign_States_),+,KEL.Aggregates.CountN(__PP3153378.S_O_S_Domestic_States_)),-,__CN(__PP3153378.S_O_S_Filing_States_Count_Dups_));
    SELF := __PP3153378;
  END;
  EXPORT __ENH_Business_Sele_Overflow_1 := PROJECT(__EE3154093,__ND3154107__Project(LEFT));
END;

...
EXPORT B_Business_Sele_Overflow_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE3160173 := __ENH_Business_Sele_Overflow_2;
  EXPORT __ST179191_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST179191_Layout __ND3160187__Project(B_Business_Sele_Overflow_2(__in,__cfg).__ST189871_Layout __PP3159458) := TRANSFORM
    __EE3160003 := __PP3159458.S_O_S_Statuses_;
    __EE3160018 := __PP3159458.S_O_S_Statuses_;
    __ST101419_Layout := RECORD
      ...
      KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
    END;
    __EE3160028 := PROJECT(TABLE(PROJECT(__T(__EE3160018),__ST101419_Layout),{KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),Days_Since_Status_},Days_Since_Status_,MERGE),__ST101419_Layout);
    __BS3160004 := __T(__EE3160003);
    __EE3160179 := __BS3160004(__T(__OP2(__T(__EE3160003).Days_Since_Status_,=,KEL.Aggregates.MinN(__EE3160028,__EE3160028.Days_Since_Status_))));
    SELF.Most_Recent_Status_Filing_Description_ := (__EE3160179)[1].S_O_S_Status_Description_;
    SELF.S_O_S_Unique_State_Count_ := __OP2(__OP2(KEL.Aggregates.CountN(__PP3159458.S_O_S_Foreign_States_),+,KEL.Aggregates.CountN(__PP3159458.S_O_S_Domestic_States_)),-,__CN(__PP3159458.S_O_S_Filing_States_Count_Dups_));
    SELF := __PP3159458;
  END;
  EXPORT __ENH_Business_Sele_Overflow_1 := PROJECT(__EE3160173,__ND3160187__Project(LEFT));
END;

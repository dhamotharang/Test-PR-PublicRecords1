...
EXPORT E_S_S_N_Address(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  EXPORT __d2_KELfiltered := __d2_Norm((UNSIGNED)person_q.ssn != 0 AND (STRING) person_q.prim_name !='' AND (STRING) person_q.prim_range != '' AND(UNSIGNED)person_q.zip != 0);
  ...
  SHARED __Mapping4 := 'ssn(OVERRIDE:Social_:0),Location_(DEFAULT:Location_:0),prim_range(OVERRIDE:Primary_Range_:\'\'),predir(OVERRIDE:Predirectional_:\'\'),prim_name(OVERRIDE:Primary_Name_:\'\'),suffix(OVERRIDE:Suffix_:\'\'),postdir(OVERRIDE:Postdirectional_:\'\'),sec_range(OVERRIDE:Secondary_Range_:\'\'),zip(OVERRIDE:Z_I_P5_:0),headerhitflag(DEFAULT:Header_Hit_Flag_),source(DEFAULT:Source_:\'\'),datefirstseen(DEFAULT:Date_First_Seen_:EPOCH),datelastseen(DEFAULT:Date_Last_Seen_:EPOCH),datevendorfirstreported(DEFAULT:Date_Vendor_First_Reported_:EPOCH),datevendorlastreported(DEFAULT:Date_Vendor_Last_Reported_:EPOCH),DPMBitmap(OVERRIDE:__Permits:PERMITS)';
  ...
  EXPORT __d4_KELfiltered := __d4_Norm((STRING10)prim_range != '' AND (STRING28)prim_name != '' AND (UNSIGNED)zip != 0 AND (UNSIGNED)ssn != 0);
  ...
  EXPORT __d5_KELfiltered := __d5_Norm((UNSIGNED)ssn != 0 AND (STRING) person_q.prim_name !='' AND (STRING) person_q.prim_range != '' AND (unsigned) person_q.ssn != 0);
  ...
  EXPORT __d6_KELfiltered := __d6_Norm((UNSIGNED)ssn != 0 AND (STRING) person_q.prim_name !='' AND (STRING) person_q.prim_range != '' AND (UNSIGNED)person_q.zip != 0);
  ...
  EXPORT __d7_KELfiltered := __d7_Norm((UNSIGNED)ssn != 0 AND (STRING) person_q.prim_name !='' AND (STRING) person_q.prim_range != '' AND(UNSIGNED)person_q.zip != 0);

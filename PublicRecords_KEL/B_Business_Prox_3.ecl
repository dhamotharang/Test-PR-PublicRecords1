...
EXPORT B_Business_Prox_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5406617 := __E_Business_Prox;
  SHARED __EE5406628 := __E_Prox_Address;
  SHARED __EE5406638 := __EE5406628(__NN(__EE5406628.Business_Location_));
  SHARED __ST734232_Layout := RECORD
  ...
  END;
  __ST734232_Layout __JT5406646(E_Prox_Address(__in,__cfg).Layout __l, E_Prox_Address(__in,__cfg).Best_Addresses_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE5406715 := NORMALIZE(__EE5406638,__T(LEFT.Best_Addresses_),__JT5406646(LEFT,RIGHT));
  SHARED __ST733832_Layout := RECORD
  ...
  SHARED __ST738132_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST733832_Layout) Exp1_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC5406801(E_Business_Prox(__in,__cfg).Layout __EE5406617, __ST733832_Layout __EE5406792) := __EEQP(__EE5406617.UID,__EE5406792.____grp___U_I_D_);
  __ST738132_Layout __Join__ST738132_Layout(E_Business_Prox(__in,__cfg).Layout __r, DATASET(__ST733832_Layout) __recs) := TRANSFORM
    SELF := __r;
    SELF.Exp1_ := __CN(__recs);
  END;
  SHARED __EE5406995 := DENORMALIZE(DISTRIBUTE(__EE5406617,HASH(UID)),DISTRIBUTE(__EE5406792,HASH(____grp___U_I_D_)),__JC5406801(LEFT,RIGHT),GROUP,__Join__ST738132_Layout(LEFT,ROWS(RIGHT)),LOCAL);
  SHARED __ST219503_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Contacts_Layout) Contacts_;
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Data_Sources_Layout) Data_Sources_;
    KEL.typ.ndataset(__ST733832_Layout) Best_Business_Prox_Address_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST219503_Layout __ND5407003__Project(__ST738132_Layout __PP5406999) := TRANSFORM
    __EE5406998 := __PP5406999.Exp1_;
    SELF.Best_Business_Prox_Address_ := __EE5406998;
    SELF := __PP5406999;
  END;
  SHARED __EE5407459 := PROJECT(__EE5406995,__ND5407003__Project(LEFT));
  EXPORT __ST733593_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST733593_Layout __ND5406131__Project(__ST219503_Layout __PP5406127) := TRANSFORM
    __EE5407462 := __PP5406127.Best_Business_Prox_Address_;
    SELF.Best_Business_Prox_Address_ := __PROJECT(__EE5407462,E_Prox_Address(__in,__cfg).Best_Addresses_Layout);
    SELF := __PP5406127;
  END;
  EXPORT __ENH_Business_Prox_3 := PROJECT(__EE5407459,__ND5406131__Project(LEFT));
END;

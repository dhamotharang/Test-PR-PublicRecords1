...
EXPORT B_Business_Prox_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5821682 := __E_Business_Prox;
  SHARED __EE5821693 := __E_Prox_Address;
  SHARED __EE5821703 := __EE5821693(__NN(__EE5821693.Business_Location_));
  ...
  END;
  __ST801820_Layout __JT5821711(E_Prox_Address(__in,__cfg).Layout __l, E_Prox_Address(__in,__cfg).Best_Addresses_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE5821780 := NORMALIZE(__EE5821703,__T(LEFT.Best_Addresses_),__JT5821711(LEFT,RIGHT));
  SHARED __ST801420_Layout := RECORD
  ...
  SHARED __ST805720_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST801420_Layout) Exp1_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC5821866(E_Business_Prox(__in,__cfg).Layout __EE5821682, __ST801420_Layout __EE5821857) := __EEQP(__EE5821682.UID,__EE5821857.____grp___U_I_D_);
  __ST805720_Layout __Join__ST805720_Layout(E_Business_Prox(__in,__cfg).Layout __r, DATASET(__ST801420_Layout) __recs) := TRANSFORM
    SELF := __r;
    SELF.Exp1_ := __CN(__recs);
  END;
  SHARED __EE5822060 := DENORMALIZE(DISTRIBUTE(__EE5821682,HASH(UID)),DISTRIBUTE(__EE5821857,HASH(____grp___U_I_D_)),__JC5821866(LEFT,RIGHT),GROUP,__Join__ST805720_Layout(LEFT,ROWS(RIGHT)),LOCAL);
  SHARED __ST225794_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Contacts_Layout) Contacts_;
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Data_Sources_Layout) Data_Sources_;
    KEL.typ.ndataset(__ST801420_Layout) Best_Business_Prox_Address_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST225794_Layout __ND5822068__Project(__ST805720_Layout __PP5822064) := TRANSFORM
    __EE5822063 := __PP5822064.Exp1_;
    SELF.Best_Business_Prox_Address_ := __EE5822063;
    SELF := __PP5822064;
  END;
  SHARED __EE5822524 := PROJECT(__EE5822060,__ND5822068__Project(LEFT));
  EXPORT __ST801181_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST801181_Layout __ND5821196__Project(__ST225794_Layout __PP5821192) := TRANSFORM
    __EE5822527 := __PP5821192.Best_Business_Prox_Address_;
    SELF.Best_Business_Prox_Address_ := __PROJECT(__EE5822527,E_Prox_Address(__in,__cfg).Best_Addresses_Layout);
    SELF := __PP5821192;
  END;
  EXPORT __ENH_Business_Prox_3 := PROJECT(__EE5822524,__ND5821196__Project(LEFT));
END;

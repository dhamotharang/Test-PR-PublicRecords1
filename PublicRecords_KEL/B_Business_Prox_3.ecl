...
EXPORT B_Business_Prox_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5439316 := __E_Business_Prox;
  SHARED __EE5439327 := __E_Prox_Address;
  SHARED __EE5439337 := __EE5439327(__NN(__EE5439327.Business_Location_));
  SHARED __ST742593_Layout := RECORD
  ...
  END;
  __ST742593_Layout __JT5439345(E_Prox_Address(__in,__cfg).Layout __l, E_Prox_Address(__in,__cfg).Best_Addresses_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE5439414 := NORMALIZE(__EE5439337,__T(LEFT.Best_Addresses_),__JT5439345(LEFT,RIGHT));
  SHARED __ST742193_Layout := RECORD
  ...
  SHARED __ST746493_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST742193_Layout) Exp1_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC5439500(E_Business_Prox(__in,__cfg).Layout __EE5439316, __ST742193_Layout __EE5439491) := __EEQP(__EE5439316.UID,__EE5439491.____grp___U_I_D_);
  __ST746493_Layout __Join__ST746493_Layout(E_Business_Prox(__in,__cfg).Layout __r, DATASET(__ST742193_Layout) __recs) := TRANSFORM
    SELF := __r;
    SELF.Exp1_ := __CN(__recs);
  END;
  SHARED __EE5439694 := DENORMALIZE(DISTRIBUTE(__EE5439316,HASH(UID)),DISTRIBUTE(__EE5439491,HASH(____grp___U_I_D_)),__JC5439500(LEFT,RIGHT),GROUP,__Join__ST746493_Layout(LEFT,ROWS(RIGHT)),LOCAL);
  SHARED __ST221392_Layout := RECORD
    ...
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Contacts_Layout) Contacts_;
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Data_Sources_Layout) Data_Sources_;
    KEL.typ.ndataset(__ST742193_Layout) Best_Business_Prox_Address_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST221392_Layout __ND5439702__Project(__ST746493_Layout __PP5439698) := TRANSFORM
    __EE5439697 := __PP5439698.Exp1_;
    SELF.Best_Business_Prox_Address_ := __EE5439697;
    SELF := __PP5439698;
  END;
  SHARED __EE5440158 := PROJECT(__EE5439694,__ND5439702__Project(LEFT));
  EXPORT __ST741954_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST741954_Layout __ND5438830__Project(__ST221392_Layout __PP5438826) := TRANSFORM
    __EE5440161 := __PP5438826.Best_Business_Prox_Address_;
    SELF.Best_Business_Prox_Address_ := __PROJECT(__EE5440161,E_Prox_Address(__in,__cfg).Best_Addresses_Layout);
    SELF := __PP5438826;
  END;
  EXPORT __ENH_Business_Prox_3 := PROJECT(__EE5440158,__ND5438830__Project(LEFT));
END;

...
EXPORT B_Business_Prox_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6965437 := __E_Business_Prox;
  SHARED __EE6965448 := __E_Prox_Address;
  SHARED __EE6965458 := __EE6965448(__NN(__EE6965448.Business_Location_));
  SHARED __ST884198_Layout := RECORD
  ...
  END;
  __ST884198_Layout __JT6965466(E_Prox_Address(__in,__cfg).Layout __l, E_Prox_Address(__in,__cfg).Best_Addresses_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE6965535 := NORMALIZE(__EE6965458,__T(LEFT.Best_Addresses_),__JT6965466(LEFT,RIGHT));
  SHARED __ST883798_Layout := RECORD
  ...
  SHARED __ST888098_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST883798_Layout) Exp1_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC6965621(E_Business_Prox(__in,__cfg).Layout __EE6965437, __ST883798_Layout __EE6965612) := __EEQP(__EE6965437.UID,__EE6965612.____grp___U_I_D_);
  __ST888098_Layout __Join__ST888098_Layout(E_Business_Prox(__in,__cfg).Layout __r, DATASET(__ST883798_Layout) __recs) := TRANSFORM
    SELF := __r;
    SELF.Exp1_ := __CN(__recs);
  END;
  SHARED __EE6965815 := DENORMALIZE(DISTRIBUTE(__EE6965437,HASH(UID)),DISTRIBUTE(__EE6965612,HASH(____grp___U_I_D_)),__JC6965621(LEFT,RIGHT),GROUP,__Join__ST888098_Layout(LEFT,ROWS(RIGHT)),LOCAL);
  SHARED __ST247837_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Contacts_Layout) Contacts_;
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Data_Sources_Layout) Data_Sources_;
    KEL.typ.ndataset(__ST883798_Layout) Best_Business_Prox_Address_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST247837_Layout __ND6965823__Project(__ST888098_Layout __PP6965819) := TRANSFORM
    __EE6965818 := __PP6965819.Exp1_;
    SELF.Best_Business_Prox_Address_ := __EE6965818;
    SELF := __PP6965819;
  END;
  SHARED __EE6966279 := PROJECT(__EE6965815,__ND6965823__Project(LEFT));
  EXPORT __ST883559_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST883559_Layout __ND6964951__Project(__ST247837_Layout __PP6964947) := TRANSFORM
    __EE6966282 := __PP6964947.Best_Business_Prox_Address_;
    SELF.Best_Business_Prox_Address_ := __PROJECT(__EE6966282,E_Prox_Address(__in,__cfg).Best_Addresses_Layout);
    SELF := __PP6964947;
  END;
  EXPORT __ENH_Business_Prox_3 := PROJECT(__EE6966279,__ND6964951__Project(LEFT));
END;

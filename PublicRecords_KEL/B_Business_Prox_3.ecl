...
EXPORT B_Business_Prox_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6556217 := __E_Business_Prox;
  SHARED __EE6556228 := __E_Prox_Address;
  SHARED __EE6556238 := __EE6556228(__NN(__EE6556228.Business_Location_));
  SHARED __ST871373_Layout := RECORD
  ...
  END;
  __ST871373_Layout __JT6556246(E_Prox_Address(__in,__cfg).Layout __l, E_Prox_Address(__in,__cfg).Best_Addresses_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE6556315 := NORMALIZE(__EE6556238,__T(LEFT.Best_Addresses_),__JT6556246(LEFT,RIGHT));
  SHARED __ST870973_Layout := RECORD
  ...
  SHARED __ST875273_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST870973_Layout) Exp1_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC6556401(E_Business_Prox(__in,__cfg).Layout __EE6556217, __ST870973_Layout __EE6556392) := __EEQP(__EE6556217.UID,__EE6556392.____grp___U_I_D_);
  __ST875273_Layout __Join__ST875273_Layout(E_Business_Prox(__in,__cfg).Layout __r, DATASET(__ST870973_Layout) __recs) := TRANSFORM
    SELF := __r;
    SELF.Exp1_ := __CN(__recs);
  END;
  SHARED __EE6556595 := DENORMALIZE(DISTRIBUTE(__EE6556217,HASH(UID)),DISTRIBUTE(__EE6556392,HASH(____grp___U_I_D_)),__JC6556401(LEFT,RIGHT),GROUP,__Join__ST875273_Layout(LEFT,ROWS(RIGHT)),LOCAL);
  SHARED __ST241042_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Contacts_Layout) Contacts_;
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Data_Sources_Layout) Data_Sources_;
    KEL.typ.ndataset(__ST870973_Layout) Best_Business_Prox_Address_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST241042_Layout __ND6556603__Project(__ST875273_Layout __PP6556599) := TRANSFORM
    __EE6556598 := __PP6556599.Exp1_;
    SELF.Best_Business_Prox_Address_ := __EE6556598;
    SELF := __PP6556599;
  END;
  SHARED __EE6557059 := PROJECT(__EE6556595,__ND6556603__Project(LEFT));
  EXPORT __ST870734_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST870734_Layout __ND6555731__Project(__ST241042_Layout __PP6555727) := TRANSFORM
    __EE6557062 := __PP6555727.Best_Business_Prox_Address_;
    SELF.Best_Business_Prox_Address_ := __PROJECT(__EE6557062,E_Prox_Address(__in,__cfg).Best_Addresses_Layout);
    SELF := __PP6555727;
  END;
  EXPORT __ENH_Business_Prox_3 := PROJECT(__EE6557059,__ND6555731__Project(LEFT));
END;

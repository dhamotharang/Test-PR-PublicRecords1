...
EXPORT B_Property_Event_8(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE421950 := __E_Property_Event;
  EXPORT __ST275938_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST275938_Layout __ND422018__Project(E_Property_Event(__in,__cfg).Layout __PP421613) := TRANSFORM
    SELF.Dt_First_Seen_ := KEL.era.ToDate(__PP421613.Date_First_Seen_);
    SELF := __PP421613;
  END;
  EXPORT __ENH_Property_Event_8 := PROJECT(__EE421950,__ND422018__Project(LEFT));
END;

...
EXPORT B_Sele_Person_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5582863 := __E_Sele_Person;
  EXPORT __ST273660_Layout := RECORD
  ...
  END;
  SHARED __ST273660_Layout __ND5582827__Project(E_Sele_Person(__in,__cfg).Layout __PP455704) := TRANSFORM
    __EE5582802 := __PP455704.Contact_Info_;
    ...
    __EE5582822 := __PP455704.Contact_Info_;
    SELF.Assoc_Date_ := IF(__T(__OP2(KEL.Aggregates.MaxNN(__EE5582802,KEL.era.ToDate(__T(__EE5582802).Date_Last_Seen_)),>,__CC13435)),__ECAST(KEL.typ.nkdate,__CC13435),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MaxNN(__EE5582822,KEL.era.ToDate(__T(__EE5582822).Date_Last_Seen_))));
    SELF := __PP455704;
  END;
  EXPORT __ENH_Sele_Person_7 := PROJECT(__EE5582863,__ND5582827__Project(LEFT));
END;

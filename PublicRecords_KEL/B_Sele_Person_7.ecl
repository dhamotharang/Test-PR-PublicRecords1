...
EXPORT B_Sele_Person_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5654936 := __E_Sele_Person;
  ...
  END;
  SHARED __ST269441_Layout __ND5654900__Project(E_Sele_Person(__in,__cfg).Layout __PP450816) := TRANSFORM
    __EE5654875 := __PP450816.Contact_Info_;
    ...
    __EE5654895 := __PP450816.Contact_Info_;
    SELF.Assoc_Date_ := IF(__T(__OP2(KEL.Aggregates.MaxNN(__EE5654875,KEL.era.ToDate(__T(__EE5654875).Date_Last_Seen_)),>,__CC13398)),__ECAST(KEL.typ.nkdate,__CC13398),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MaxNN(__EE5654895,KEL.era.ToDate(__T(__EE5654895).Date_Last_Seen_))));
    SELF := __PP450816;
  END;
  EXPORT __ENH_Sele_Person_7 := PROJECT(__EE5654936,__ND5654900__Project(LEFT));
END;

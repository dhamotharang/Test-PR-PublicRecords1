...
EXPORT B_Sele_Person_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5580011 := __E_Sele_Person;
  EXPORT __ST273241_Layout := RECORD
  ...
  END;
  SHARED __ST273241_Layout __ND5579975__Project(E_Sele_Person(__in,__cfg).Layout __PP454041) := TRANSFORM
    __EE5579950 := __PP454041.Contact_Info_;
    ...
    __EE5579970 := __PP454041.Contact_Info_;
    SELF.Assoc_Date_ := IF(__T(__OP2(KEL.Aggregates.MaxNN(__EE5579950,KEL.era.ToDate(__T(__EE5579950).Date_Last_Seen_)),>,__CC13435)),__ECAST(KEL.typ.nkdate,__CC13435),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MaxNN(__EE5579970,KEL.era.ToDate(__T(__EE5579970).Date_Last_Seen_))));
    SELF := __PP454041;
  END;
  EXPORT __ENH_Sele_Person_7 := PROJECT(__EE5580011,__ND5579975__Project(LEFT));
END;

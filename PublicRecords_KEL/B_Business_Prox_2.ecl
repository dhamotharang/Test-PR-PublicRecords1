...
EXPORT B_Business_Prox_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6578451 := __ENH_Business_Prox_3;
  SHARED __EE6578453 := __E_Prox_Phone_Number;
  SHARED __EE6578460 := __EE6578453(__NN(__EE6578453.Business_Location_));
  SHARED __ST1244426_Layout := RECORD
  ...
  END;
  __ST1244426_Layout __JT6578471(E_Prox_Phone_Number(__in,__cfg).Layout __l, E_Prox_Phone_Number(__in,__cfg).Best_Phone_Details_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE6578510 := NORMALIZE(__EE6578460,__T(LEFT.Best_Phone_Details_),__JT6578471(LEFT,RIGHT));
  SHARED __ST1249007_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST1244426_Layout) Prox_Phone_Number_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC6578519(B_Business_Prox_3(__in,__cfg).__ST773503_Layout __EE6578451, __ST1244426_Layout __EE6578510) := __EEQP(__EE6578451.UID,__EE6578510.Business_Location_);
  __ST1249007_Layout __Join__ST1249007_Layout(B_Business_Prox_3(__in,__cfg).__ST773503_Layout __r, DATASET(__ST1244426_Layout) __recs) := TRANSFORM
    SELF := __r;
    SELF.Prox_Phone_Number_ := __CN(__recs);
  END;
  SHARED __EE6578751 := DENORMALIZE(DISTRIBUTE(__EE6578451,HASH(UID)),DISTRIBUTE(__EE6578510,HASH(Business_Location_)),__JC6578519(LEFT,RIGHT),GROUP,__Join__ST1249007_Layout(LEFT,ROWS(RIGHT)),LOCAL);
  SHARED __EE6578762 := __E_Prox_T_I_N;
  SHARED __EE6578769 := __EE6578762(__NN(__EE6578762.Business_Location_));
  SHARED __ST1244103_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __EE6578821 := PROJECT(__EE6578769,TRANSFORM(__ST1244103_Layout,SELF.____grp___U_I_D_ := LEFT.Business_Location_,SELF := LEFT));
  SHARED __EE6578832 := GROUP(KEL.Routines.SortChildren(__EE6578821,'Data_Sources_'),____grp___U_I_D_,ALL);
  SHARED __EE6578836 := UNGROUP(TOPN(__EE6578832(__NN(__EE6578832.Best_T_I_N_Rank_)),1,__T(__EE6578832.Best_T_I_N_Rank_),__T(____grp___U_I_D_),__T(Tax_I_D_),__T(Ult_I_D_),__T(Org_I_D_),__T(Sele_I_D_),__T(Prox_I_D_),__T(Best_T_I_N_)));
  SHARED __ST1250327_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.ndataset(E_Prox_Address(__in,__cfg).Best_Addresses_Layout) Best_Business_Prox_Address_;
    KEL.typ.ndataset(__ST1244426_Layout) Prox_Phone_Number_;
    KEL.typ.ndataset(__ST1244103_Layout) Exp1_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC6578845(__ST1249007_Layout __EE6578751, __ST1244103_Layout __EE6578836) := __EEQP(__EE6578751.UID,__EE6578836.____grp___U_I_D_);
  __ST1250327_Layout __Join__ST1250327_Layout(__ST1249007_Layout __r, DATASET(__ST1244103_Layout) __recs) := TRANSFORM
    SELF := __r;
    SELF.Exp1_ := __CN(__recs);
  END;
  SHARED __EE6579092 := DENORMALIZE(DISTRIBUTE(__EE6578751,HASH(UID)),DISTRIBUTE(__EE6578836,HASH(____grp___U_I_D_)),__JC6578845(LEFT,RIGHT),GROUP,__Join__ST1250327_Layout(LEFT,ROWS(RIGHT)),LOCAL);
  SHARED __ST201496_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.ndataset(__ST1244103_Layout) Best_Business_Prox_Tin_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST201496_Layout __ND6579103__Project(__ST1250327_Layout __PP6579099) := TRANSFORM
    __EE6579404 := __PP6579099.Best_Company_Names_;
    __BS6579394 := __T(__EE6579404);
    __EE6579408 := __BN(TOPN(__BS6579394(__NN(__T(__EE6579404).Best_Company_Name_Rank_)),1,__T(__T(__EE6579404).Best_Company_Name_Rank_),__T(Best_Company_Name_)),__NL(__EE6579404));
    SELF.Best_Business_Prox_Names_Sorted_ := __EE6579408;
    __EE6579095 := __PP6579099.Prox_Phone_Number_;
    __EE6579436 := PROJECT(TABLE(PROJECT(__T(__EE6579095),E_Prox_Phone_Number(__in,__cfg).Best_Phone_Details_Layout),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),Best_Phone_,Best_Phone_Rank_},Best_Phone_,Best_Phone_Rank_,MERGE),E_Prox_Phone_Number(__in,__cfg).Best_Phone_Details_Layout);
    __EE6579440 := TOPN(__EE6579436(__NN(__EE6579436.Best_Phone_Rank_)),1,__T(__EE6579436.Best_Phone_Rank_),__T(Best_Phone_));
    SELF.Best_Business_Prox_Phone_ := __CN(__EE6579440(__NN(Best_Phone_) OR __NN(Best_Phone_Rank_)));
    __EE6579098 := __PP6579099.Exp1_;
    SELF.Best_Business_Prox_Tin_ := __EE6579098;
    __EE6579444 := __PP6579099.Best_Business_Prox_Address_;
    SELF.Only_Best_Business_Prox_Address_ := (__T(__EE6579444))[1];
    SELF := __PP6579099;
  END;
  SHARED __EE6579662 := PROJECT(__EE6579092,__ND6579103__Project(LEFT));
  EXPORT __ST1243831_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST1243831_Layout __ND6577891__Project(__ST201496_Layout __PP6577887) := TRANSFORM
    __EE6579665 := __PP6577887.Best_Business_Prox_Tin_;
    SELF.Best_Business_Prox_Tin_ := __PROJECT(__EE6579665,E_Prox_T_I_N(__in,__cfg).Layout);
    SELF := __PP6577887;
  END;
  EXPORT __ENH_Business_Prox_2 := PROJECT(__EE6579662,__ND6577891__Project(LEFT));
END;

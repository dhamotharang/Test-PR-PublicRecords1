...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6067552 := __ENH_Aircraft_Owner_3;
  EXPORT __ST194068_Layout := RECORD
  ...
  END;
  SHARED __ST194068_Layout __ND6067612__Project(B_Aircraft_Owner_3(__in,__cfg).__ST212283_Layout __PP6067553) := TRANSFORM
    __CC13535 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP6067553.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP6067553.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13535)));
    SELF := __PP6067553;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE6067552,__ND6067612__Project(LEFT));
END;

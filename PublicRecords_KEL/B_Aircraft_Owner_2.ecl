...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6441333 := __ENH_Aircraft_Owner_3;
  EXPORT __ST202372_Layout := RECORD
  ...
  END;
  SHARED __ST202372_Layout __ND6441393__Project(B_Aircraft_Owner_3(__in,__cfg).__ST221042_Layout __PP6441334) := TRANSFORM
    __CC13680 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP6441334.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP6441334.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13680)));
    SELF := __PP6441334;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE6441333,__ND6441393__Project(LEFT));
END;

...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6841857 := __ENH_Aircraft_Owner_3;
  EXPORT __ST207689_Layout := RECORD
  ...
  END;
  SHARED __ST207689_Layout __ND6841917__Project(B_Aircraft_Owner_3(__in,__cfg).__ST226803_Layout __PP6841858) := TRANSFORM
    __CC13812 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP6841858.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP6841858.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13812)));
    SELF := __PP6841858;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE6841857,__ND6841917__Project(LEFT));
END;

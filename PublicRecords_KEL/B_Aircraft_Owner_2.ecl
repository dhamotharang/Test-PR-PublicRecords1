...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5783290 := __ENH_Aircraft_Owner_3;
  EXPORT __ST198516_Layout := RECORD
  ...
  END;
  SHARED __ST198516_Layout __ND5783350__Project(B_Aircraft_Owner_3(__in,__cfg).__ST216664_Layout __PP5783291) := TRANSFORM
    __CC13827 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP5783291.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP5783291.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13827)));
    SELF := __PP5783291;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE5783290,__ND5783350__Project(LEFT));
END;

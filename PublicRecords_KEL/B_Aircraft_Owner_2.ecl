...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1125405 := __ENH_Aircraft_Owner_3;
  EXPORT __ST125534_Layout := RECORD
  ...
  END;
  SHARED __ST125534_Layout __ND1125435__Project(B_Aircraft_Owner_3(__in,__cfg).__ST137719_Layout __PP1125339) := TRANSFORM
    __CC9911 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1125339.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1125339.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC9911)));
    SELF := __PP1125339;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE1125405,__ND1125435__Project(LEFT));
END;

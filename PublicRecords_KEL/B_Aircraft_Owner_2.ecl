...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6968596 := __ENH_Aircraft_Owner_3;
  EXPORT __ST210459_Layout := RECORD
  ...
  END;
  SHARED __ST210459_Layout __ND6968656__Project(B_Aircraft_Owner_3(__in,__cfg).__ST229685_Layout __PP6968597) := TRANSFORM
    __CC13775 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP6968597.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP6968597.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13775)));
    SELF := __PP6968597;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE6968596,__ND6968656__Project(LEFT));
END;

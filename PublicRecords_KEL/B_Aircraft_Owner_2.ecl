...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE360280 := __ENH_Aircraft_Owner_3;
  EXPORT __ST69836_Layout := RECORD
  ...
  END;
  SHARED __ST69836_Layout __ND360312__Project(B_Aircraft_Owner_3(__in,__cfg).__ST76570_Layout __PP360209) := TRANSFORM
    __CC8115 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP360209.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP360209.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC8115)));
    SELF := __PP360209;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE360280,__ND360312__Project(LEFT));
END;

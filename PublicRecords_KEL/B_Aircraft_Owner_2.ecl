...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6760087 := __ENH_Aircraft_Owner_3;
  EXPORT __ST204927_Layout := RECORD
  ...
  END;
  SHARED __ST204927_Layout __ND6760147__Project(B_Aircraft_Owner_3(__in,__cfg).__ST223843_Layout __PP6760088) := TRANSFORM
    __CC13812 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP6760088.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP6760088.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13812)));
    SELF := __PP6760088;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE6760087,__ND6760147__Project(LEFT));
END;

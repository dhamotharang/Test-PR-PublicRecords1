...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7005341 := __ENH_Aircraft_Owner_3;
  EXPORT __ST214428_Layout := RECORD
  ...
  END;
  SHARED __ST214428_Layout __ND7005401__Project(B_Aircraft_Owner_3(__in,__cfg).__ST233495_Layout __PP7005342) := TRANSFORM
    __CC13833 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP7005342.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP7005342.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13833)));
    SELF := __PP7005342;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE7005341,__ND7005401__Project(LEFT));
END;

...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6911406 := __ENH_Aircraft_Owner_3;
  EXPORT __ST212934_Layout := RECORD
  ...
  END;
  SHARED __ST212934_Layout __ND6911466__Project(B_Aircraft_Owner_3(__in,__cfg).__ST231964_Layout __PP6911407) := TRANSFORM
    __CC13827 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP6911407.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP6911407.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13827)));
    SELF := __PP6911407;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE6911406,__ND6911466__Project(LEFT));
END;

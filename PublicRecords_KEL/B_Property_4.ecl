...
EXPORT B_Property_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE3698265 := __ENH_Property_5;
  EXPORT __ST182170_Layout := RECORD
  ...
  END;
  EXPORT __ST182152_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST182170_Layout) Automated_Valuation_Model_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST182152_Layout __ND3698270__Project(B_Property_5(__in,__cfg).__ST187794_Layout __PP3698266) := TRANSFORM
    __EE3698309 := __PP3698266.Automated_Valuation_Model_;
    __ST182170_Layout __ND3698314__Project(B_Property_5(__in,__cfg).__ST187812_Layout __PP3698310) := TRANSFORM
      __CC10882 := KEL.Routines.MinN(FN_Compile(__cfg).FN_G_E_T_B_U_I_L_D_D_A_T_E(__ECAST(KEL.typ.nstr,__CN('property_build_version'))),__CN(__cfg.CurrentDate));
      SELF.A_V_M_Years_ := FN_Compile(__cfg).FN_A_B_S_Y_E_A_R_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP3698310.A_V_M_Value_Date_),__ECAST(KEL.typ.nkdate,__CC10882));
      SELF := __PP3698310;
    END;
    SELF.Automated_Valuation_Model_ := __PROJECT(__EE3698309,__ND3698314__Project(LEFT));
    SELF := __PP3698266;
  END;
  EXPORT __ENH_Property_4 := PROJECT(__EE3698265,__ND3698270__Project(LEFT));
END;

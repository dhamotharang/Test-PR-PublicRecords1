...
EXPORT B_Sele_Address_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE513029 := __E_Sele_Address;
  SHARED __EE5579214 := __ENH_Business_Sele_7;
  SHARED __ST513329_Layout := RECORD
  ...
  END;
  __JC5579220(E_Sele_Address(__in,__cfg).Layout __EE513029, B_Business_Sele_7(__in,__cfg).__ST269390_Layout __EE5579214) := __EEQP(__EE513029.Legal_,__EE5579214.UID);
  __ST513329_Layout __JT5579220(E_Sele_Address(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST269390_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE5579221 := JOIN(__EE513029,__EE5579214,__JC5579220(LEFT,RIGHT),__JT5579220(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST268169_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST268113_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Address().Typ) Location_;
    KEL.typ.nstr Primary_Range_;
    KEL.typ.nstr Predirectional_;
    ...
    KEL.typ.ndataset(E_Sele_Address(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(E_Sele_Address(__in,__cfg).Best_Addresses_Layout) Best_Addresses_;
    KEL.typ.ndataset(__ST268169_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST268113_Layout __ND5580270__Project(__ST513329_Layout __PP5579222) := TRANSFORM
    __EE5579684 := __PP5579222.Data_Sources_;
    __ST268169_Layout __ND5579689__Project(E_Sele_Address(__in,__cfg).Data_Sources_Layout __PP5579685) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP5579685.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP5579685.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP5579685.Source_));
      SELF := __PP5579685;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE5579684,__ND5579689__Project(LEFT));
    SELF.Input_Address_Match_ := __AND(__AND(__AND(__AND(__AND(__AND(FN_Compile(__cfg).FN_Is_Null_Or_Equal(__ECAST(KEL.typ.nunk,__PP5579222.Primary_Range_),__ECAST(KEL.typ.nunk,__PP5579222.B___Inp_Cln_Addr_Prim_Rng_)),FN_Compile(__cfg).FN_Is_Null_Or_Equal(__ECAST(KEL.typ.nunk,__PP5579222.Predirectional_),__ECAST(KEL.typ.nunk,__PP5579222.B___Inp_Cln_Addr_Pre_Dir_))),FN_Compile(__cfg).FN_Is_Null_Or_Equal(__ECAST(KEL.typ.nunk,__PP5579222.Primary_Name_),__ECAST(KEL.typ.nunk,__PP5579222.B___Inp_Cln_Addr_Prim_Name_))),FN_Compile(__cfg).FN_Is_Null_Or_Equal(__ECAST(KEL.typ.nunk,__PP5579222.Suffix_),__ECAST(KEL.typ.nunk,__PP5579222.B___Inp_Cln_Addr_Sffx_))),FN_Compile(__cfg).FN_Is_Null_Or_Equal(__ECAST(KEL.typ.nunk,__PP5579222.Postdirectional_),__ECAST(KEL.typ.nunk,__PP5579222.B___Inp_Cln_Addr_Post_Dir_))),FN_Compile(__cfg).FN_Is_Null_Or_Equal(__ECAST(KEL.typ.nunk,__PP5579222.Z_I_P5_),__ECAST(KEL.typ.nunk,__PP5579222.B___Inp_Cln_Addr_Zip5_))),FN_Compile(__cfg).FN_Is_Null_Or_Equal(__ECAST(KEL.typ.nunk,__PP5579222.Secondary_Range_),__ECAST(KEL.typ.nunk,__PP5579222.B___Inp_Cln_Addr_Sec_Rng_)));
    SELF.Matches_Is_Best_Helper_Attr_ := __AND(__AND(__AND(__AND(__AND(__AND(__OP2(__PP5579222.Only_Best_Address_.Best_Primary_Range_,=,__PP5579222.Primary_Range_),__CN(__DEFAULT(__PP5579222.Only_Best_Address_.Best_Predirectional_,'') = __DEFAULT(__PP5579222.Predirectional_,''))),__OP2(__PP5579222.Only_Best_Address_.Best_Primary_Name_,=,__PP5579222.Primary_Name_)),__CN(__DEFAULT(__PP5579222.Only_Best_Address_.Best_Suffix_,'') = __DEFAULT(__PP5579222.Suffix_,''))),__CN(__DEFAULT(__PP5579222.Only_Best_Address_.Best_Postdirectional_,'') = __DEFAULT(__PP5579222.Postdirectional_,''))),__OP2(__PP5579222.Z_I_P5_,=,__PP5579222.Only_Best_Address_.Best_Zip5_)),__CN(__DEFAULT(__PP5579222.Only_Best_Address_.Best_Secondary_Range_,'') = __DEFAULT(__PP5579222.Secondary_Range_,'')));
    SELF := __PP5579222;
  END;
  EXPORT __ENH_Sele_Address_6 := PROJECT(__EE5579221,__ND5580270__Project(LEFT));
END;

...
EXPORT B_Sele_Address_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE432503 := __E_Sele_Address;
  SHARED __EE4876993 := __ENH_Business_Sele_7;
  SHARED __ST432803_Layout := RECORD
    ...
    KEL.typ.ndataset(B_Business_Sele_7(__in,__cfg).__ST248385_Layout) S_I_C_Codes__1_;
    KEL.typ.ndataset(B_Business_Sele_7(__in,__cfg).__ST248396_Layout) N_A_I_C_S_Codes__1_;
    ...
    KEL.typ.ndataset(B_Business_Sele_7(__in,__cfg).__ST248501_Layout) Data_Sources__1_;
  ...
  END;
  __JC4876999(E_Sele_Address(__in,__cfg).Layout __EE432503, B_Business_Sele_7(__in,__cfg).__ST248318_Layout __EE4876993) := __EEQP(__EE432503.Legal_,__EE4876993.UID);
  __ST432803_Layout __JT4876999(E_Sele_Address(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST248318_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4877000 := JOIN(__EE432503,__EE4876993,__JC4876999(LEFT,RIGHT),__JT4876999(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST247097_Layout := RECORD
  ...
  EXPORT __ST247041_Layout := RECORD
    ...
    KEL.typ.ndataset(E_Sele_Address(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(E_Sele_Address(__in,__cfg).Best_Addresses_Layout) Best_Addresses_;
    KEL.typ.ndataset(__ST247097_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST247041_Layout __ND4878049__Project(__ST432803_Layout __PP4877001) := TRANSFORM
    __EE4877463 := __PP4877001.Data_Sources_;
    __ST247097_Layout __ND4877468__Project(E_Sele_Address(__in,__cfg).Data_Sources_Layout __PP4877464) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP4877464.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP4877464.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP4877464.Source_));
      SELF := __PP4877464;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE4877463,__ND4877468__Project(LEFT));
    SELF.Input_Address_Match_ := __AND(__AND(__AND(__AND(__AND(__AND(FN_Compile(__cfg).FN_Is_Null_Or_Equal(__ECAST(KEL.typ.nunk,__PP4877001.Primary_Range_),__ECAST(KEL.typ.nunk,__PP4877001.B___Inp_Cln_Addr_Prim_Rng_)),FN_Compile(__cfg).FN_Is_Null_Or_Equal(__ECAST(KEL.typ.nunk,__PP4877001.Predirectional_),__ECAST(KEL.typ.nunk,__PP4877001.B___Inp_Cln_Addr_Pre_Dir_))),FN_Compile(__cfg).FN_Is_Null_Or_Equal(__ECAST(KEL.typ.nunk,__PP4877001.Primary_Name_),__ECAST(KEL.typ.nunk,__PP4877001.B___Inp_Cln_Addr_Prim_Name_))),FN_Compile(__cfg).FN_Is_Null_Or_Equal(__ECAST(KEL.typ.nunk,__PP4877001.Suffix_),__ECAST(KEL.typ.nunk,__PP4877001.B___Inp_Cln_Addr_Sffx_))),FN_Compile(__cfg).FN_Is_Null_Or_Equal(__ECAST(KEL.typ.nunk,__PP4877001.Postdirectional_),__ECAST(KEL.typ.nunk,__PP4877001.B___Inp_Cln_Addr_Post_Dir_))),FN_Compile(__cfg).FN_Is_Null_Or_Equal(__ECAST(KEL.typ.nunk,__PP4877001.Z_I_P5_),__ECAST(KEL.typ.nunk,__PP4877001.B___Inp_Cln_Addr_Zip5_))),FN_Compile(__cfg).FN_Is_Null_Or_Equal(__ECAST(KEL.typ.nunk,__PP4877001.Secondary_Range_),__ECAST(KEL.typ.nunk,__PP4877001.B___Inp_Cln_Addr_Sec_Rng_)));
    SELF.Matches_Is_Best_Helper_Attr_ := __AND(__AND(__AND(__AND(__AND(__AND(__OP2(__PP4877001.Only_Best_Address_.Best_Primary_Range_,=,__PP4877001.Primary_Range_),__CN(__DEFAULT(__PP4877001.Only_Best_Address_.Best_Predirectional_,'') = __DEFAULT(__PP4877001.Predirectional_,''))),__OP2(__PP4877001.Only_Best_Address_.Best_Primary_Name_,=,__PP4877001.Primary_Name_)),__CN(__DEFAULT(__PP4877001.Only_Best_Address_.Best_Suffix_,'') = __DEFAULT(__PP4877001.Suffix_,''))),__CN(__DEFAULT(__PP4877001.Only_Best_Address_.Best_Postdirectional_,'') = __DEFAULT(__PP4877001.Postdirectional_,''))),__OP2(__PP4877001.Z_I_P5_,=,__PP4877001.Only_Best_Address_.Best_Zip5_)),__CN(__DEFAULT(__PP4877001.Only_Best_Address_.Best_Secondary_Range_,'') = __DEFAULT(__PP4877001.Secondary_Range_,'')));
    SELF := __PP4877001;
  END;
  EXPORT __ENH_Sele_Address_6 := PROJECT(__EE4877000,__ND4878049__Project(LEFT));
END;

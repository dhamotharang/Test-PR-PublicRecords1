...
EXPORT B_Input_B_I_I_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE26332 := __E_Input_B_I_I;
  EXPORT __ST23289_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST23289_Layout __ND26330__Project(E_Input_B_I_I(__in,__cfg).Layout __PP26076) := TRANSFORM
    SELF.Addr_Not_Populated_ := IF(FN_Compile.FN_Addr_Not_Populated_Check(__ECAST(KEL.typ.nstr,__PP26076.Bus_Input_Street_Echo_),__ECAST(KEL.typ.nstr,__PP26076.Bus_Input_City_Echo_),__ECAST(KEL.typ.nstr,__PP26076.Bus_Input_State_Echo_),__ECAST(KEL.typ.nstr,__PP26076.Bus_Input_Zip_Echo_)),TRUE,FALSE);
    SELF := __PP26076;
  END;
  EXPORT __ENH_Input_B_I_I_4 := PROJECT(__EE26332,__ND26330__Project(LEFT));
END;

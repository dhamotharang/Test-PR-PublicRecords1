...
EXPORT Q_Non_F_C_R_A_Mini_Attributes_V1(KEL.typ.uid __PLexID_in, KEL.typ.kdate __PP_InpClnArchDt, UNSIGNED8 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED __EE2748697 := __ENH_Person;
  SHARED __EE2749186 := __EE2748697(__T(__OP2(__EE2748697.UID,=,__CN(__PLexID_in))));
  SHARED __ST71171_Layout := RECORD
  ...
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(__EE2749186,TRANSFORM(__ST71171_Layout,SELF.Lex_I_D_ := LEFT.UID,SELF := LEFT)));
END;

...
EXPORT B_Input_P_I_I(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE80473 := __ENH_Input_P_I_I_1;
  EXPORT __ST12277_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST12277_Layout __ND80547__Project(B_Input_P_I_I_1(__in,__cfg).__ST13523_Layout __PP80118) := TRANSFORM
    SELF.I_Rep_Number_Value_ := __PP80118.Rep_Number_;
    __CC2585 := '-99';
    SELF.Input_Archive_Date_Value_ := FN_Compile.FN_Is_Blank(__ECAST(KEL.typ.nstr,__PP80118.Input_Archive_Date_Echo_),__ECAST(KEL.typ.nstr,__CN(__CC2585)));
    SELF.Input_Work_Phone_Value_ := FN_Compile.FN_Is_Blank(__ECAST(KEL.typ.nstr,__PP80118.Input_Work_Phone_Echo_),__ECAST(KEL.typ.nstr,__CN(__CC2585)));
    SELF := __PP80118;
  END;
  EXPORT __ENH_Input_P_I_I := PROJECT(__EE80473,__ND80547__Project(LEFT));
END;

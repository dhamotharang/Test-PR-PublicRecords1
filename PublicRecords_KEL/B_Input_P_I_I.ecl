...
EXPORT B_Input_P_I_I(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE79849 := __ENH_Input_P_I_I_1;
  EXPORT __ST11531_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST11531_Layout __ND79923__Project(B_Input_P_I_I_1(__in,__cfg).__ST12777_Layout __PP79494) := TRANSFORM
    SELF.I_Rep_Number_Value_ := __PP79494.Rep_Number_;
    __CC1831 := '-99';
    SELF.Input_Archive_Date_Value_ := FN_Compile.FN_Is_Blank(__ECAST(KEL.typ.nstr,__PP79494.Input_Archive_Date_Echo_),__ECAST(KEL.typ.nstr,__CN(__CC1831)));
    SELF.Input_Work_Phone_Value_ := FN_Compile.FN_Is_Blank(__ECAST(KEL.typ.nstr,__PP79494.Input_Work_Phone_Echo_),__ECAST(KEL.typ.nstr,__CN(__CC1831)));
    SELF := __PP79494;
  END;
  EXPORT __ENH_Input_P_I_I := PROJECT(__EE79849,__ND79923__Project(LEFT));
END;

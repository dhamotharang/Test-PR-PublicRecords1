...
IMPORT CFG_Compile,E_Business_Org,E_Business_Sele,E_Business_Sele_Overflow,E_Business_Ult,E_Watercraft FROM PublicRecords_KEL;
IMPORT * FROM KEL15.Null;
EXPORT E_Sele_Watercraft(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  EXPORT __d0_KELfiltered := PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault((UNSIGNED)ultid<>0 AND (UNSIGNED)orgid<>0 AND (UNSIGNED)seleid<>0);
  SHARED __d0_Legal__Layout := RECORD
    RECORDOF(__d0_KELfiltered);
    KEL.typ.uid Legal_;
  END;
  SHARED __d0_Missing_Legal__UIDComponents := KEL.Intake.ConstructMissingFieldList(__d0_KELfiltered,'ultid,orgid,seleid','PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault');
  ...
  END;
  SHARED __d0_Missing_W_Craft__UIDComponents := KEL.Intake.ConstructMissingFieldList(__d0_Legal__Mapped,'watercraft_key','PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault');
  SHARED __d0_W_Craft__Mapped := IF(__d0_Missing_W_Craft__UIDComponents = 'watercraft_key',PROJECT(__d0_Legal__Mapped,TRANSFORM(__d0_W_Craft__Layout,SELF.W_Craft_:=0,SELF:=LEFT)),JOIN(KEL.Intake.AppendFields(__d0_Legal__Mapped,__d0_Missing_W_Craft__UIDComponents),E_Watercraft(__cfg).Lookup,TRIM((STRING)LEFT.watercraft_key) = RIGHT.KeyVal,TRANSFORM(__d0_W_Craft__Layout,SELF.W_Craft_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,SMART));
  SHARED __d0_Prefiltered := __d0_W_Craft__Mapped;
  SHARED __d0 := __SourceFilter(KEL.FromFlat.Convert(__d0_Prefiltered,InLayout,__Mapping0,'PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault'));
  EXPORT InData := __d0;
  ...
  EXPORT Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __PostFilter := __GroupedFilter(GROUP(InData,Legal_,W_Craft_,Ult_I_D_,Org_I_D_,Sele_I_D_,Watercraft_Key_,ALL));
  ...
  EXPORT NullCounts := DATASET([
    {'SeleWatercraft','PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault','Legal',COUNT(__d0(__NL(Legal_))),COUNT(__d0(__NN(Legal_)))},
    {'SeleWatercraft','PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault','WCraft',COUNT(__d0(__NL(W_Craft_))),COUNT(__d0(__NN(W_Craft_)))},
    {'SeleWatercraft','PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault','ultid',COUNT(__d0(__NL(Ult_I_D_))),COUNT(__d0(__NN(Ult_I_D_)))},
    {'SeleWatercraft','PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault','orgid',COUNT(__d0(__NL(Org_I_D_))),COUNT(__d0(__NN(Org_I_D_)))},
    {'SeleWatercraft','PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault','seleid',COUNT(__d0(__NL(Sele_I_D_))),COUNT(__d0(__NN(Sele_I_D_)))},
    {'SeleWatercraft','PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault','watercraft_key',COUNT(__d0(__NL(Watercraft_Key_))),COUNT(__d0(__NN(Watercraft_Key_)))},
    {'SeleWatercraft','PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault','Src',COUNT(__d0(__NL(Source_))),COUNT(__d0(__NN(Source_)))},
    {'SeleWatercraft','PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault','Archive_Date',COUNT(__d0(Archive___Date_=0)),COUNT(__d0(Archive___Date_!=0))},
    {'SeleWatercraft','PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault','DateFirstSeen',COUNT(__d0(Date_First_Seen_=0)),COUNT(__d0(Date_First_Seen_!=0))},
    {'SeleWatercraft','PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault','DateLastSeen',COUNT(__d0(Date_Last_Seen_=0)),COUNT(__d0(Date_Last_Seen_!=0))},
    {'SeleWatercraft','PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault','HybridArchiveDate',COUNT(__d0(Hybrid_Archive_Date_=0)),COUNT(__d0(Hybrid_Archive_Date_!=0))},
    {'SeleWatercraft','PublicRecords_KEL.Files.NonFCRA.Watercraft__Key_LinkIds_Vault','VaultDateLastSeen',COUNT(__d0(Vault_Date_Last_Seen_=0)),COUNT(__d0(Vault_Date_Last_Seen_!=0))}]

...
EXPORT E_Sele_Watercraft(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __d0_W_Craft__Mapped := IF(__d0_Missing_W_Craft__UIDComponents = 'watercraft_key',PROJECT(__d0_Legal__Mapped,TRANSFORM(__d0_W_Craft__Layout,SELF.W_Craft_:=0,SELF:=LEFT)),JOIN(KEL.Intake.AppendFields(__d0_Legal__Mapped,__d0_Missing_W_Craft__UIDComponents),E_Watercraft(__in,__cfg).Lookup,TRIM((STRING)LEFT.watercraft_key) = RIGHT.KeyVal,TRANSFORM(__d0_W_Craft__Layout,SELF.W_Craft_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,HASH));

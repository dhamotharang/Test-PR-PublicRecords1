...
EXPORT E_Second_Degree_Associations(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  EXPORT Second_Degree_Association__Orphan := JOIN(InData(__NN(Second_Degree_Association_)),E_Person(__cfg).__Result,__EEQP(LEFT.Second_Degree_Association_, RIGHT.UID),TRANSFORM(InLayout,SELF := LEFT,SELF:=[]),LEFT ONLY, HASH);

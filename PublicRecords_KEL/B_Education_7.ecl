...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE437358 := __E_Education;
  EXPORT __ST267585_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST267585_Layout __ND5642157__Project(E_Education(__in,__cfg).Layout __PP437232) := TRANSFORM
    __BS437308 := __T(__PP437232.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS437308(__T(__OP2(__T(__PP437232.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP437232;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE437358,__ND5642157__Project(LEFT));
END;

...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE429933 := __E_Education;
  EXPORT __ST261614_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST261614_Layout __ND5435962__Project(E_Education(__in,__cfg).Layout __PP429807) := TRANSFORM
    __BS429883 := __T(__PP429807.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS429883(__T(__OP2(__T(__PP429807.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP429807;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE429933,__ND5435962__Project(LEFT));
END;

...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE369967 := __E_Education;
  EXPORT __ST248093_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST248093_Layout __ND4815676__Project(E_Education(__in,__cfg).Layout __PP369841) := TRANSFORM
    __BS369917 := __T(__PP369841.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS369917(__T(__OP2(__T(__PP369841.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP369841;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE369967,__ND4815676__Project(LEFT));
END;

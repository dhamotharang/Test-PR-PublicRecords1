...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE370442 := __E_Education;
  EXPORT __ST248615_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST248615_Layout __ND4823924__Project(E_Education(__in,__cfg).Layout __PP370316) := TRANSFORM
    __BS370392 := __T(__PP370316.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS370392(__T(__OP2(__T(__PP370316.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP370316;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE370442,__ND4823924__Project(LEFT));
END;

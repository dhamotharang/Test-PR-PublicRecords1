...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE433994 := __E_Education;
  EXPORT __ST264699_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST264699_Layout __ND5515461__Project(E_Education(__in,__cfg).Layout __PP433868) := TRANSFORM
    __BS433944 := __T(__PP433868.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS433944(__T(__OP2(__T(__PP433868.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP433868;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE433994,__ND5515461__Project(LEFT));
END;

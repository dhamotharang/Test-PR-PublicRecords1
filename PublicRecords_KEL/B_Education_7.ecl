...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE388042 := __E_Education;
  EXPORT __ST257886_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST257886_Layout __ND5092219__Project(E_Education(__in,__cfg).Layout __PP387916) := TRANSFORM
    __BS387992 := __T(__PP387916.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS387992(__T(__OP2(__T(__PP387916.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP387916;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE388042,__ND5092219__Project(LEFT));
END;

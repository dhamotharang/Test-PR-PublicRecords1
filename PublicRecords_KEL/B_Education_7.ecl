...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE388012 := __E_Education;
  EXPORT __ST257856_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST257856_Layout __ND5092189__Project(E_Education(__in,__cfg).Layout __PP387886) := TRANSFORM
    __BS387962 := __T(__PP387886.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS387962(__T(__OP2(__T(__PP387886.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP387886;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE388012,__ND5092189__Project(LEFT));
END;

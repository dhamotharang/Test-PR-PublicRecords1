...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE433655 := __E_Education;
  ...
  END;
  SHARED __ST264397_Layout __ND5495094__Project(E_Education(__in,__cfg).Layout __PP433529) := TRANSFORM
    __BS433605 := __T(__PP433529.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS433605(__T(__OP2(__T(__PP433529.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP433529;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE433655,__ND5495094__Project(LEFT));
END;

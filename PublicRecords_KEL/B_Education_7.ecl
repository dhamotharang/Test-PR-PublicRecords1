...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE416663 := __E_Education;
  EXPORT __ST255140_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST255140_Layout __ND5254201__Project(E_Education(__in,__cfg).Layout __PP416537) := TRANSFORM
    __BS416613 := __T(__PP416537.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS416613(__T(__OP2(__T(__PP416537.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP416537;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE416663,__ND5254201__Project(LEFT));
END;

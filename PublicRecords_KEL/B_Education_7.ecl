...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE370761 := __E_Education;
  EXPORT __ST249141_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST249141_Layout __ND4938629__Project(E_Education(__in,__cfg).Layout __PP370635) := TRANSFORM
    __BS370711 := __T(__PP370635.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS370711(__T(__OP2(__T(__PP370635.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP370635;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE370761,__ND4938629__Project(LEFT));
END;

...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE286149 := __E_Education;
  EXPORT __ST165484_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST165484_Layout __ND286296__Project(E_Education(__in,__cfg).Layout __PP286023) := TRANSFORM
    __BS286099 := __T(__PP286023.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS286099(__T(__OP2(__T(__PP286023.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP286023;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE286149,__ND286296__Project(LEFT));
END;

...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE418257 := __E_Education;
  EXPORT __ST256551_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST256551_Layout __ND5281152__Project(E_Education(__in,__cfg).Layout __PP418131) := TRANSFORM
    __BS418207 := __T(__PP418131.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS418207(__T(__OP2(__T(__PP418131.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP418131;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE418257,__ND5281152__Project(LEFT));
END;

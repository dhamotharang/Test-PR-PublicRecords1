...
EXPORT B_Education_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE429903 := __E_Education;
  EXPORT __ST261584_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST261584_Layout __ND5437548__Project(E_Education(__in,__cfg).Layout __PP429777) := TRANSFORM
    __BS429853 := __T(__PP429777.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS429853(__T(__OP2(__T(__PP429777.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP429777;
  END;
  EXPORT __ENH_Education_7 := PROJECT(__EE429903,__ND5437548__Project(LEFT));
END;

...
EXPORT E_Person_Sex_Offender(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  EXPORT InLayout := RECORD
    ...
    KEL.typ.ntyp(E_Sex_Offender().Typ) Offender_;
    KEL.typ.nstr Seisint_Primary_Key_;
    KEL.typ.nkdate F_C_R_A_Date_;
    KEL.typ.nstr Fcra_Conviction_Flag_;
    KEL.typ.nstr Fcra_Traffic_Flag_;
    KEL.typ.nstr Sex_Offender_Registry_Number_;
  ...
  EXPORT Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __PostFilter := __GroupedFilter(GROUP(InData,Subject_,Offender_,Seisint_Primary_Key_,F_C_R_A_Date_,Fcra_Conviction_Flag_,Fcra_Traffic_Flag_,Sex_Offender_Registry_Number_,ALL));
  Person_Sex_Offender_Group := __PostFilter;
  Layout Person_Sex_Offender__Rollup(InLayout __r, DATASET(InLayout) __recs) := TRANSFORM
    ...
    SELF := __r;
  END;
  Layout Person_Sex_Offender__Single_Rollup(InLayout __r) := TRANSFORM
  ...
  EXPORT NullCounts := DATASET([
    ...
    {'PersonSexOffender','PublicRecords_KEL.ECL_Functions.Dataset_FDC','DateVendorFirstReported',COUNT(__d0(Date_Vendor_First_Reported_=0)),COUNT(__d0(Date_Vendor_First_Reported_!=0))},
    {'PersonSexOffender','PublicRecords_KEL.ECL_Functions.Dataset_FDC','DateVendorLastReported',COUNT(__d0(Date_Vendor_Last_Reported_=0)),COUNT(__d0(Date_Vendor_Last_Reported_!=0))},
    ...
    {'PersonSexOffender','PublicRecords_KEL.ECL_Functions.Dataset_FDC','VaultDateFirstSeen',COUNT(__d0(Vault_Date_First_Seen_=0)),COUNT(__d0(Vault_Date_First_Seen_!=0))},

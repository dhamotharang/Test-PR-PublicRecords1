...
EXPORT B_Business_Sele_Overflow_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8618762 := __ENH_Business_Sele_Overflow_2;
  EXPORT __ST198611_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST198611_Layout __ND8618776__Project(B_Business_Sele_Overflow_2(__in,__cfg).__ST222787_Layout __PP8618047) := TRANSFORM
    __EE8618592 := __PP8618047.S_O_S_Statuses_;
    __EE8618607 := __PP8618047.S_O_S_Statuses_;
    __ST121329_Layout := RECORD
      ...
      KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
    END;
    __EE8618617 := PROJECT(TABLE(PROJECT(__T(__EE8618607),__ST121329_Layout),{KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),Days_Since_Status_},Days_Since_Status_,MERGE),__ST121329_Layout);
    __BS8618593 := __T(__EE8618592);
    __EE8618768 := __BS8618593(__T(__OP2(__T(__EE8618592).Days_Since_Status_,=,KEL.Aggregates.MinN(__EE8618617,__EE8618617.Days_Since_Status_))));
    SELF.Most_Recent_Status_Filing_Description_ := (__EE8618768)[1].S_O_S_Status_Description_;
    SELF.S_O_S_Unique_State_Count_ := __OP2(__OP2(KEL.Aggregates.CountN(__PP8618047.S_O_S_Foreign_States_),+,KEL.Aggregates.CountN(__PP8618047.S_O_S_Domestic_States_)),-,__CN(__PP8618047.S_O_S_Filing_States_Count_Dups_));
    SELF := __PP8618047;
  END;
  EXPORT __ENH_Business_Sele_Overflow_1 := PROJECT(__EE8618762,__ND8618776__Project(LEFT));
END;

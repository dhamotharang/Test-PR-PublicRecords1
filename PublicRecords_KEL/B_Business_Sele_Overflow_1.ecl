...
EXPORT B_Business_Sele_Overflow_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE9642735 := __ENH_Business_Sele_Overflow_2;
  EXPORT __ST209221_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST209221_Layout __ND9642749__Project(B_Business_Sele_Overflow_2(__in,__cfg).__ST235569_Layout __PP9642020) := TRANSFORM
    __EE9642565 := __PP9642020.S_O_S_Statuses_;
    __EE9642580 := __PP9642020.S_O_S_Statuses_;
    __ST124285_Layout := RECORD
      ...
      KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
    END;
    __EE9642590 := PROJECT(TABLE(PROJECT(__T(__EE9642580),__ST124285_Layout),{KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),Days_Since_Status_},Days_Since_Status_,MERGE),__ST124285_Layout);
    __BS9642566 := __T(__EE9642565);
    __EE9642741 := __BS9642566(__T(__OP2(__T(__EE9642565).Days_Since_Status_,=,KEL.Aggregates.MinN(__EE9642590,__EE9642590.Days_Since_Status_))));
    SELF.Most_Recent_Status_Filing_Description_ := (__EE9642741)[1].S_O_S_Status_Description_;
    SELF.S_O_S_Unique_State_Count_ := __OP2(__OP2(KEL.Aggregates.CountN(__PP9642020.S_O_S_Foreign_States_),+,KEL.Aggregates.CountN(__PP9642020.S_O_S_Domestic_States_)),-,__CN(__PP9642020.S_O_S_Filing_States_Count_Dups_));
    SELF := __PP9642020;
  END;
  EXPORT __ENH_Business_Sele_Overflow_1 := PROJECT(__EE9642735,__ND9642749__Project(LEFT));
END;

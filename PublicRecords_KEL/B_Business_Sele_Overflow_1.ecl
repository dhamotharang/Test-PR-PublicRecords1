...
EXPORT B_Business_Sele_Overflow_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7567816 := __ENH_Business_Sele_Overflow_2;
  EXPORT __ST180712_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST180712_Layout __ND7567830__Project(B_Business_Sele_Overflow_2(__in,__cfg).__ST202938_Layout __PP7567101) := TRANSFORM
    __EE7567646 := __PP7567101.S_O_S_Statuses_;
    __EE7567661 := __PP7567101.S_O_S_Statuses_;
    __ST108673_Layout := RECORD
      ...
      KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
    END;
    __EE7567671 := PROJECT(TABLE(PROJECT(__T(__EE7567661),__ST108673_Layout),{KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),Days_Since_Status_},Days_Since_Status_,MERGE),__ST108673_Layout);
    __BS7567647 := __T(__EE7567646);
    __EE7567822 := __BS7567647(__T(__OP2(__T(__EE7567646).Days_Since_Status_,=,KEL.Aggregates.MinN(__EE7567671,__EE7567671.Days_Since_Status_))));
    SELF.Most_Recent_Status_Filing_Description_ := (__EE7567822)[1].S_O_S_Status_Description_;
    SELF.S_O_S_Unique_State_Count_ := __OP2(__OP2(KEL.Aggregates.CountN(__PP7567101.S_O_S_Foreign_States_),+,KEL.Aggregates.CountN(__PP7567101.S_O_S_Domestic_States_)),-,__CN(__PP7567101.S_O_S_Filing_States_Count_Dups_));
    SELF := __PP7567101;
  END;
  EXPORT __ENH_Business_Sele_Overflow_1 := PROJECT(__EE7567816,__ND7567830__Project(LEFT));
END;

...
EXPORT B_Business_Sele_Overflow_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7899887 := __ENH_Business_Sele_Overflow_2;
  EXPORT __ST186088_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST186088_Layout __ND7899901__Project(B_Business_Sele_Overflow_2(__in,__cfg).__ST209009_Layout __PP7899172) := TRANSFORM
    __EE7899717 := __PP7899172.S_O_S_Statuses_;
    __EE7899732 := __PP7899172.S_O_S_Statuses_;
    __ST112504_Layout := RECORD
      ...
      KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
    END;
    __EE7899742 := PROJECT(TABLE(PROJECT(__T(__EE7899732),__ST112504_Layout),{KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),Days_Since_Status_},Days_Since_Status_,MERGE),__ST112504_Layout);
    __BS7899718 := __T(__EE7899717);
    __EE7899893 := __BS7899718(__T(__OP2(__T(__EE7899717).Days_Since_Status_,=,KEL.Aggregates.MinN(__EE7899742,__EE7899742.Days_Since_Status_))));
    SELF.Most_Recent_Status_Filing_Description_ := (__EE7899893)[1].S_O_S_Status_Description_;
    SELF.S_O_S_Unique_State_Count_ := __OP2(__OP2(KEL.Aggregates.CountN(__PP7899172.S_O_S_Foreign_States_),+,KEL.Aggregates.CountN(__PP7899172.S_O_S_Domestic_States_)),-,__CN(__PP7899172.S_O_S_Filing_States_Count_Dups_));
    SELF := __PP7899172;
  END;
  EXPORT __ENH_Business_Sele_Overflow_1 := PROJECT(__EE7899887,__ND7899901__Project(LEFT));
END;

...
EXPORT B_Business_Sele_Overflow_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7942887 := __ENH_Business_Sele_Overflow_2;
  EXPORT __ST187717_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST187717_Layout __ND7942901__Project(B_Business_Sele_Overflow_2(__in,__cfg).__ST210782_Layout __PP7942172) := TRANSFORM
    __EE7942717 := __PP7942172.S_O_S_Statuses_;
    __EE7942732 := __PP7942172.S_O_S_Statuses_;
    __ST113686_Layout := RECORD
      ...
      KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
    END;
    __EE7942742 := PROJECT(TABLE(PROJECT(__T(__EE7942732),__ST113686_Layout),{KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),Days_Since_Status_},Days_Since_Status_,MERGE),__ST113686_Layout);
    __BS7942718 := __T(__EE7942717);
    __EE7942893 := __BS7942718(__T(__OP2(__T(__EE7942717).Days_Since_Status_,=,KEL.Aggregates.MinN(__EE7942742,__EE7942742.Days_Since_Status_))));
    SELF.Most_Recent_Status_Filing_Description_ := (__EE7942893)[1].S_O_S_Status_Description_;
    SELF.S_O_S_Unique_State_Count_ := __OP2(__OP2(KEL.Aggregates.CountN(__PP7942172.S_O_S_Foreign_States_),+,KEL.Aggregates.CountN(__PP7942172.S_O_S_Domestic_States_)),-,__CN(__PP7942172.S_O_S_Filing_States_Count_Dups_));
    SELF := __PP7942172;
  END;
  EXPORT __ENH_Business_Sele_Overflow_1 := PROJECT(__EE7942887,__ND7942901__Project(LEFT));
END;

...
EXPORT B_Business_Sele_Overflow_1(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE8086297 := __ENH_Business_Sele_Overflow_2;
  EXPORT __ST186618_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST186618_Layout __ND8086311__Project(B_Business_Sele_Overflow_2(__cfg).__ST209423_Layout __PP8085582) := TRANSFORM
    __EE8086127 := __PP8085582.S_O_S_Statuses_;
    __EE8086142 := __PP8085582.S_O_S_Statuses_;
    __ST113204_Layout := RECORD
      ...
      KEL.typ.epoch Vault_Date_Last_Seen_ := 0;
    END;
    __EE8086152 := PROJECT(TABLE(PROJECT(__T(__EE8086142),__ST113204_Layout),{KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),Days_Since_Status_},Days_Since_Status_,MERGE),__ST113204_Layout);
    __BS8086128 := __T(__EE8086127);
    __EE8086303 := __BS8086128(__T(__OP2(__T(__EE8086127).Days_Since_Status_,=,KEL.Aggregates.MinN(__EE8086152,__EE8086152.Days_Since_Status_))));
    SELF.Most_Recent_Status_Filing_Description_ := (__EE8086303)[1].S_O_S_Status_Description_;
    SELF.S_O_S_Unique_State_Count_ := __OP2(__OP2(KEL.Aggregates.CountN(__PP8085582.S_O_S_Foreign_States_),+,KEL.Aggregates.CountN(__PP8085582.S_O_S_Domestic_States_)),-,__CN(__PP8085582.S_O_S_Filing_States_Count_Dups_));
    SELF := __PP8085582;
  END;
  EXPORT __ENH_Business_Sele_Overflow_1 := PROJECT(__EE8086297,__ND8086311__Project(LEFT));
END;

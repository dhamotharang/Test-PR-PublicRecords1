...
EXPORT Q_Non_F_C_R_A_Business_Sele_I_D_No_Dates_Attributes_V1(KEL.typ.int __PUltID_in, KEL.typ.int __POrgID_in, KEL.typ.int __PSeleID_in, UNSIGNED8 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED __EE510493 := __ENH_Business;
  SHARED __EE510590 := __EE510493(__T(__AND(__OP2(__EE510493.Ult_I_D_,=,__CN(__PUltID_in)),__AND(__OP2(__EE510493.Org_I_D_,=,__CN(__POrgID_in)),__OP2(__EE510493.Sele_I_D_,=,__CN(__PSeleID_in))))));
  SHARED __ST20773_Layout := RECORD
  ...
  END;
  SHARED __ST20773_Layout __ND510595__Project(B_Business(__in).__ST24328_Layout __PP510591) := TRANSFORM
    SELF.Lex_I_D_Bus_Extended_Family_Append_ := __PP510591.Ult_I_D_;
    SELF.Lex_I_D_Bus_Legal_Family_Append_ := __PP510591.Org_I_D_;
    SELF.Lex_I_D_Bus_Legal_Entity_Append_ := __PP510591.Sele_I_D_;
    SELF := __PP510591;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE510590,__ND510595__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Lex_I_D_Bus_Extended_Family_Append_,Lex_I_D_Bus_Legal_Family_Append_,Lex_I_D_Bus_Legal_Entity_Append_,Lex_I_D_Bus_Legal_Entity_Restricted_},Lex_I_D_Bus_Extended_Family_Append_,Lex_I_D_Bus_Legal_Family_Append_,Lex_I_D_Bus_Legal_Entity_Append_,Lex_I_D_Bus_Legal_Entity_Restricted_,MERGE),__ST20773_Layout));
END;

...
EXPORT Q_Non_F_C_R_A_Sixth_Rep_Attributes_V1_Hybrid(KEL.typ.uid __PLexID_in, DATASET(RECORDOF(PublicRecords_KEL.ECL_Functions.Layouts.LayoutInputPII)) __PInputPIIDataset, KEL.typ.kdate __PP_InpClnArchDt, KEL.typ.kdate __PRunDateToMimic, DATA57 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED __EE5470917 := __E_Person;
  SHARED __EE12216299 := __EE5470917(__T(__OP2(__EE5470917.UID,=,__CN(__PLexID_in))));
  SHARED __ST136515_Layout := RECORD
  ...
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(__EE12216299,TRANSFORM(__ST136515_Layout,SELF.Lex_I_D_ := LEFT.UID,SELF := LEFT)));

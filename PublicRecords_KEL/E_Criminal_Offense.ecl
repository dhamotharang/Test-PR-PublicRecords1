...
EXPORT E_Criminal_Offense(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  Layout Criminal_Offense__Rollup(InLayout __r, DATASET(InLayout) __recs) := TRANSFORM
    ...
    SELF.Offense_Charges_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Case_Number_,Case_Date_,Case_Type_Description_,Offense_Date_,Offense_Score_,Data_Type_,Traffic_Flag_,Conviction_Flag_},Case_Number_,Case_Date_,Case_Type_Description_,Offense_Date_,Offense_Score_,Data_Type_,Traffic_Flag_,Conviction_Flag_),Offense_Charges_Layout)(__NN(Case_Number_) OR __NN(Case_Date_) OR __NN(Case_Type_Description_) OR __NN(Offense_Date_) OR __NN(Offense_Score_) OR __NN(Data_Type_) OR __NN(Traffic_Flag_) OR __NN(Conviction_Flag_)));
    SELF.Criminal_Data_Sources_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Data_Source_},Data_Source_),Criminal_Data_Sources_Layout)(__NN(Data_Source_)));
    SELF.Court_Offense_Level_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Court_Offense_Level_},Court_Offense_Level_),Court_Offense_Level_Layout)(__NN(Court_Offense_Level_)));
    SELF.Offender_Level_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Offender_Level_},Offender_Level_),Offender_Level_Layout)(__NN(Offender_Level_)));
    SELF.Fcra_Data_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Fcra_Offense_Key_,Fcra_Date_,Fcra_Date_Type_},Fcra_Offense_Key_,Fcra_Date_,Fcra_Date_Type_),Fcra_Data_Layout)(__NN(Fcra_Offense_Key_) OR __NN(Fcra_Date_) OR __NN(Fcra_Date_Type_)));
    SELF.Conviction_Overrides_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Conviction_Override_Date_,Conviction_Override_Date_Type_},Conviction_Override_Date_,Conviction_Override_Date_Type_),Conviction_Overrides_Layout)(__NN(Conviction_Override_Date_) OR __NN(Conviction_Override_Date_Type_)));
  ...
  Layout Criminal_Offense__Single_Rollup(InLayout __r) := TRANSFORM
    SELF.Offense_Charges_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Offense_Charges_Layout,SELF.__RecordCount:=1;,SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF:=LEFT))(__NN(Case_Number_) OR __NN(Case_Date_) OR __NN(Case_Type_Description_) OR __NN(Offense_Date_) OR __NN(Offense_Score_) OR __NN(Data_Type_) OR __NN(Traffic_Flag_) OR __NN(Conviction_Flag_)));
    SELF.Criminal_Data_Sources_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Criminal_Data_Sources_Layout,SELF.__RecordCount:=1;,SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF:=LEFT))(__NN(Data_Source_)));
    SELF.Court_Offense_Level_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Court_Offense_Level_Layout,SELF.__RecordCount:=1;,SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF:=LEFT))(__NN(Court_Offense_Level_)));
    SELF.Offender_Level_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Offender_Level_Layout,SELF.__RecordCount:=1;,SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF:=LEFT))(__NN(Offender_Level_)));
    SELF.Fcra_Data_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Fcra_Data_Layout,SELF.__RecordCount:=1;,SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF:=LEFT))(__NN(Fcra_Offense_Key_) OR __NN(Fcra_Date_) OR __NN(Fcra_Date_Type_)));
    SELF.Conviction_Overrides_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Conviction_Overrides_Layout,SELF.__RecordCount:=1;,SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF:=LEFT))(__NN(Conviction_Override_Date_) OR __NN(Conviction_Override_Date_Type_)));

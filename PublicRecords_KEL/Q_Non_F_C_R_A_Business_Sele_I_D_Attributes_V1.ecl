...
EXPORT Q_Non_F_C_R_A_Business_Sele_I_D_Attributes_V1(KEL.typ.int __PUltID_in, KEL.typ.int __POrgID_in, KEL.typ.int __PSeleID_in, DATASET(RECORDOF(PublicRecords_KEL.ECL_Functions.Layouts.LayoutInputPII)) __PInputPIIDataset, DATASET(RECORDOF(PublicRecords_KEL.ECL_Functions.Layouts.LayoutInputBII)) __PInputBIIDataset, KEL.typ.kdate __PP_InpClnArchDt, UNSIGNED8 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED __EE3982217 := __ENH_Business_Sele;
  SHARED __EE3988944 := __EE3982217(__T(__AND(__OP2(__EE3982217.Ult_I_D_,=,__CN(__PUltID_in)),__AND(__OP2(__EE3982217.Org_I_D_,=,__CN(__POrgID_in)),__OP2(__EE3982217.Sele_I_D_,=,__CN(__PSeleID_in))))));
  SHARED __ST83224_Layout := RECORD
  ...
  SHARED __ST83759_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST83759_Layout __ND8489551__Project(B_Business_Sele(__in,__cfg_Local).__ST101043_Layout __PP8489547) := TRANSFORM
    SELF.B___Lex_I_D_Ult_ := __PP8489547.Ult_I_D_;
    SELF.B___Lex_I_D_Org_ := __PP8489547.Org_I_D_;
    SELF.B___Lex_I_D_Legal_ := __PP8489547.Sele_I_D_;
    SELF := __PP8489547;
  END;
  EXPORT Res1 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE8489546,__ND8489551__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Date_Vendor_First_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_First_Reported_,MIN,FALSE),KEL.typ.epoch Date_Vendor_Last_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_Last_Reported_,MAX,FALSE),B___Lex_I_D_Ult_,B___Lex_I_D_Org_,B___Lex_I_D_Legal_,B___Lex_I_D_Legal_Rstd_Only_Flag_},B___Lex_I_D_Ult_,B___Lex_I_D_Org_,B___Lex_I_D_Legal_,B___Lex_I_D_Legal_Rstd_Only_Flag_,MERGE),__ST83759_Layout));
END;

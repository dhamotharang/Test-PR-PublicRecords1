...
EXPORT B_Sele_Person_4(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5946126 := __ENH_Sele_Person_5;
  SHARED __EE5946129 := __ENH_First_Degree_Relative_5;
  SHARED __ST5946483_Layout := RECORD
  ...
  SHARED __ST5946502_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST5946483_Layout) First_Degree_Relative_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC5946663(B_Sele_Person_5(__in,__cfg).__ST264885_Layout __EE5946126, __ST5946483_Layout __EE5946487) := __EEQP(__EE5946126.Contact_,__EE5946487.Subject_) AND __T(__AND(__EEQ(__EE5946126.Contact_,__EE5946487.Subject_),__CN(__NN(__EE5946126.Contact_))));
  __ST5946502_Layout __Join__ST5946502_Layout(B_Sele_Person_5(__in,__cfg).__ST264885_Layout __r, DATASET(__ST5946483_Layout) __recs) := TRANSFORM
    SELF := __r;
    SELF.First_Degree_Relative_ := __CN(__recs);
  END;
  SHARED __EE5946701 := DENORMALIZE(DISTRIBUTE(__EE5946126,HASH(Contact_)),DISTRIBUTE(__EE5946487,HASH(Subject_)),__JC5946663(LEFT,RIGHT),GROUP,__Join__ST5946502_Layout(LEFT,ROWS(RIGHT)),LOCAL);
  EXPORT __ST124636_Layout := RECORD
    KEL.typ.ntyp(E_Person().Typ) Relative_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST258144_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Person().Typ) Contact_;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    ...
    KEL.typ.ndataset(__ST124636_Layout) Relatives_;
    KEL.typ.nbool Two_Years_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST258144_Layout __ND5946710__Project(__ST5946502_Layout __PP5946706) := TRANSFORM
    __BS5946763 := __T(__PP5946706.Contact_Info_);
    SELF.Is_Executive_Ever_ := EXISTS(__BS5946763(__T(__OP2(__T(__PP5946706.Contact_Info_).Is_Executive_,=,__CN(TRUE)))));
    __EE5946704 := __PP5946706.First_Degree_Relative_;
    SELF.Relatives_ := __PROJECT(__EE5946704,__ST124636_Layout);
    SELF := __PP5946706;
  END;
  EXPORT __ENH_Sele_Person_4 := PROJECT(__EE5946701,__ND5946710__Project(LEFT));
END;

...
EXPORT E_Utility(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  Layout Utility__Rollup(InLayout __r, DATASET(InLayout) __recs) := TRANSFORM
    ...
    SELF.Account_Holder_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Date_Vendor_First_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_First_Reported_,MIN,FALSE),KEL.typ.epoch Date_Vendor_Last_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_Last_Reported_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,FALSE),First_Name_,Last_Name_,Original_First_Name_,Original_Last_Name_,Primary_Range_,Primary_Name_,Postdirectional_,Predirectional_,Suffix_,Z_I_P5_,Secondary_Range_,Phone_,Work_Phone_},First_Name_,Last_Name_,Original_First_Name_,Original_Last_Name_,Primary_Range_,Primary_Name_,Postdirectional_,Predirectional_,Suffix_,Z_I_P5_,Secondary_Range_,Phone_,Work_Phone_),Account_Holder_Layout)(__NN(First_Name_) OR __NN(Last_Name_) OR __NN(Original_First_Name_) OR __NN(Original_Last_Name_) OR __NN(Primary_Range_) OR __NN(Primary_Name_) OR __NN(Postdirectional_) OR __NN(Predirectional_) OR __NN(Suffix_) OR __NN(Z_I_P5_) OR __NN(Secondary_Range_) OR __NN(Phone_) OR __NN(Work_Phone_)));
  ...
  Layout Utility__Single_Rollup(InLayout __r) := TRANSFORM
    SELF.Account_Holder_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Account_Holder_Layout,SELF.__RecordCount:=1;,SELF.Archive___Date_:=KEL.era.SimpleRollSingleRow(LEFT,Archive___Date_,FALSE),SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF.Date_Vendor_First_Reported_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Vendor_First_Reported_,FALSE),SELF.Date_Vendor_Last_Reported_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Vendor_Last_Reported_,FALSE),SELF.Hybrid_Archive_Date_:=KEL.era.SimpleRollSingleRow(LEFT,Hybrid_Archive_Date_,FALSE),SELF.Vault_Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Vault_Date_First_Seen_,FALSE),SELF.Vault_Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Vault_Date_Last_Seen_,FALSE),SELF:=LEFT))(__NN(First_Name_) OR __NN(Last_Name_) OR __NN(Original_First_Name_) OR __NN(Original_Last_Name_) OR __NN(Primary_Range_) OR __NN(Primary_Name_) OR __NN(Postdirectional_) OR __NN(Predirectional_) OR __NN(Suffix_) OR __NN(Z_I_P5_) OR __NN(Secondary_Range_) OR __NN(Phone_) OR __NN(Work_Phone_)));
  ...
  EXPORT NullCounts := DATASET([
    ...
    {'Utility','PublicRecords_KEL.ECL_Functions.Dataset_FDC','VaultDateFirstSeen',COUNT(__d0(Vault_Date_First_Seen_=0)),COUNT(__d0(Vault_Date_First_Seen_!=0))},
    ...
    {'Utility','PublicRecords_KEL.ECL_Functions.Dataset_FDC','VaultDateFirstSeen',COUNT(__d1(Vault_Date_First_Seen_=0)),COUNT(__d1(Vault_Date_First_Seen_!=0))},
    ...
    {'Utility','PublicRecords_KEL.ECL_Functions.Dataset_FDC','VaultDateFirstSeen',COUNT(__d2(Vault_Date_First_Seen_=0)),COUNT(__d2(Vault_Date_First_Seen_!=0))},

...
EXPORT B_Input_B_I_I_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE233392 := __E_Input_B_I_I;
  EXPORT __ST144817_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST144817_Layout __ND233467__Project(E_Input_B_I_I(__in,__cfg).Layout __PP233047) := TRANSFORM
    SELF.B___Inp_Addr_ := __OP2(IF(__T(__NT(__PP233047.B___Inp_Addr_Line1_)),__ECAST(KEL.typ.nstr,__CN('')),__ECAST(KEL.typ.nstr,__OP2(__PP233047.B___Inp_Addr_Line1_,+,__CN(' ')))),+,IF(__T(__NT(__PP233047.B___Inp_Addr_Line2_)),__ECAST(KEL.typ.nstr,__CN('')),__ECAST(KEL.typ.nstr,__PP233047.B___Inp_Addr_Line2_)));
    SELF := __PP233047;
  END;
  EXPORT __ENH_Input_B_I_I_6 := PROJECT(__EE233392,__ND233467__Project(LEFT));
END;

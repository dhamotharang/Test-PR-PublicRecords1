...
EXPORT E_Sex_Offender(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  EXPORT InLayout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nstr Seisint_Primary_Key_;
  ...
  SHARED __Mapping0 := 'UID(DEFAULT:UID),sspk(OVERRIDE:Seisint_Primary_Key_:\'\'),src(OVERRIDE:Source_:\'\'),archive_date(DEFAULT:Archive___Date_:EPOCH),dt_first_reported(OVERRIDE:Date_First_Seen_:EPOCH),dt_last_reported(OVERRIDE:Date_Last_Seen_:EPOCH),datevendorfirstreported(DEFAULT:Date_Vendor_First_Reported_:EPOCH),datevendorlastreported(DEFAULT:Date_Vendor_Last_Reported_:EPOCH),hybridarchivedate(DEFAULT:Hybrid_Archive_Date_:EPOCH),vaultdatefirstseen(DEFAULT:Vault_Date_First_Seen_:EPOCH),vaultdatelastseen(DEFAULT:Vault_Date_Last_Seen_:EPOCH),DPMBitmap(OVERRIDE:__Permits:PERMITS)';
  ...
  Layout Sex_Offender__Rollup(InLayout __r, DATASET(InLayout) __recs) := TRANSFORM
    ...
    SELF := __r;
  END;
  Layout Sex_Offender__Single_Rollup(InLayout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  EXPORT __PreResult := ROLLUP(HAVING(Sex_Offender_Group,COUNT(ROWS(LEFT))=1),GROUP,Sex_Offender__Single_Rollup(LEFT)) + ROLLUP(HAVING(Sex_Offender_Group,COUNT(ROWS(LEFT))>1),GROUP,Sex_Offender__Rollup(LEFT, ROWS(LEFT)));
  ...
  EXPORT NullCounts := DATASET([
    ...
    {'SexOffender','PublicRecords_KEL.ECL_Functions.Dataset_FDC','DateVendorFirstReported',COUNT(__d0(Date_Vendor_First_Reported_=0)),COUNT(__d0(Date_Vendor_First_Reported_!=0))},
    {'SexOffender','PublicRecords_KEL.ECL_Functions.Dataset_FDC','DateVendorLastReported',COUNT(__d0(Date_Vendor_Last_Reported_=0)),COUNT(__d0(Date_Vendor_Last_Reported_!=0))},
    ...
    {'SexOffender','PublicRecords_KEL.ECL_Functions.Dataset_FDC','VaultDateFirstSeen',COUNT(__d0(Vault_Date_First_Seen_=0)),COUNT(__d0(Vault_Date_First_Seen_!=0))},

...
EXPORT E_S_S_N_Inquiry(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  EXPORT __d0_KELfiltered := __d0_Norm(person_q.appended_ssn <> '');
  ...
  SHARED __d0_Inquiry__Mapped := IF(__d0_Missing_Inquiry__UIDComponents = 'search_info.transaction_id,search_info.sequence_number',PROJECT(__d0_KELfiltered,TRANSFORM(__d0_Inquiry__Layout,SELF.Inquiry_:=0,SELF:=LEFT)),JOIN(KEL.Intake.AppendFields(__d0_KELfiltered,__d0_Missing_Inquiry__UIDComponents),E_Inquiry(__in,__cfg).Lookup,TRIM((STRING)LEFT.search_info.transaction_id) + '|' + TRIM((STRING)LEFT.search_info.sequence_number) = RIGHT.KeyVal,TRANSFORM(__d0_Inquiry__Layout,SELF.Inquiry_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,HASH));
  ...
  EXPORT Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __PostFilter := __GroupedFilter(GROUP(InData,S_S_N_,Inquiry_,ALL));
  ...
  EXPORT SanityCheck := DATASET([{COUNT(S_S_N__Orphan),COUNT(Inquiry__Orphan)}],{KEL.typ.int S_S_N__Orphan,KEL.typ.int Inquiry__Orphan});
  EXPORT NullCounts := DATASET([
    {'SSNInquiry','PublicRecords_KEL.ECL_Functions.Dataset_FDC','person_q.appended_ssn',COUNT(__d0(__NL(S_S_N_))),COUNT(__d0(__NN(S_S_N_)))},
    {'SSNInquiry','PublicRecords_KEL.ECL_Functions.Dataset_FDC','Inquiry',COUNT(__d0(__NL(Inquiry_))),COUNT(__d0(__NN(Inquiry_)))},

...
IMPORT B_Address_4,B_Address_5,B_Address_6,B_Input_P_I_I_3,B_Input_P_I_I_4,B_Input_P_I_I_5,B_Input_P_I_I_6,B_Input_P_I_I_7,B_Input_P_I_I_8,B_Input_P_I_I_9,B_Inquiry_10,B_Inquiry_11,B_Inquiry_4,B_Inquiry_5,B_Inquiry_6,B_Inquiry_7,B_Inquiry_8,B_Inquiry_9,B_Name_Summary,B_Name_Summary_1,B_Name_Summary_2,B_Name_Summary_3,B_Name_Summary_4,B_Name_Summary_5,B_Property_4,B_Property_5,CFG_Compile,E_Address,E_Address_Inquiry,E_Address_Property,E_Email,E_Email_Inquiry,E_Geo_Link,E_Input_P_I_I,E_Inquiry,E_Name_Summary,E_Person,E_Phone,E_Phone_Inquiry,E_Property,E_S_S_N_Inquiry,E_Social_Security_Number,E_Surname,E_Zip_Code FROM PublicRecords_KEL;
IMPORT * FROM KEL15.Null;
EXPORT Q_Non_F_C_R_A_Name_Summary_Attributes_V1_Dynamic(KEL.typ.str __PFirstName_in, KEL.typ.str __PLastName_in, KEL.typ.kdate __PDateOfBirth_in, DATASET(RECORDOF(PublicRecords_KEL.ECL_Functions.Layouts.LayoutInputPII)) __PInputPIIDataset, KEL.typ.kdate __PP_InpClnArchDt, DATA57 __PDPM) := MODULE
  ...
  SHARED E_Name_Summary_Filtered := MODULE(E_Name_Summary(__cfg_Local))
    SHARED __AsofFitler(DATASET(InLayout) __ds) := __ds(__T(__OP2(KEL.era.ToDateMinNull(__ds.Archive___Date_),<=,__CN(__PP_InpClnArchDt))));
    SHARED __UsingFitler(DATASET(InLayout) __ds) := __ds(KEL.Permits.BitAnd(__ds.__Permits,__PDPM) = __ds.__Permits);
    SHARED __GroupTest(DATASET(InLayout) __g) := EXISTS(__g(__T(__OP2(__g.First_Name_,=,__CN(__PFirstName_in))))) AND EXISTS(__g(__T(__OP2(__g.Last_Name_,=,__CN(__PLastName_in))))) AND EXISTS(__g(__T(__OP2(__g.Date_Of_Birth_,=,__CN(__PDateOfBirth_in)))));
    SHARED __SourceFilter(DATASET(InLayout) __ds) := __UsingFitler(__AsofFitler(__ds));
  ...
  SHARED __EE11562659 := __ENH_Name_Summary;
  SHARED __EE11562739 := __EE11562659(__T(__AND(__OP2(__EE11562659.First_Name_,=,__CN(__PFirstName_in)),__AND(__OP2(__EE11562659.Last_Name_,=,__CN(__PLastName_in)),__OP2(__EE11562659.Date_Of_Birth_,=,__CN(__PDateOfBirth_in))))));
  SHARED __ST79758_Layout := RECORD
  ...
  END;
  SHARED __ST79758_Layout __ND11562744__Project(B_Name_Summary(__cfg_Local).__ST157572_Layout __PP11562740) := TRANSFORM
    SELF.P___Inp_Cln_Name_First_ := __PP11562740.First_Name_;
    SELF.P___Inp_Cln_Name_Last_ := __PP11562740.Last_Name_;
    SELF.P___Inp_Cln_D_O_B_ := __PP11562740.Date_Of_Birth_;
    SELF := __PP11562740;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(TABLE(PROJECT(__EE11562739,__ND11562744__Project(LEFT)),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),P___Inp_Cln_Name_First_,P___Inp_Cln_Name_Last_,P___Inp_Cln_D_O_B_,P_I___Src_W_Inp_F_L_D_List_Ev_,P_I___Src_W_Inp_F_L_D_Emrg_Dt_List_Ev_,P_I___Src_W_Inp_F_L_D_Last_Dt_List_Ev_},P___Inp_Cln_Name_First_,P___Inp_Cln_Name_Last_,P___Inp_Cln_D_O_B_,P_I___Src_W_Inp_F_L_D_List_Ev_,P_I___Src_W_Inp_F_L_D_Emrg_Dt_List_Ev_,P_I___Src_W_Inp_F_L_D_Last_Dt_List_Ev_,MERGE),__ST79758_Layout));

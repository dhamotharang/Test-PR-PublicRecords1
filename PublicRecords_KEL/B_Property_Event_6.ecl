...
EXPORT B_Property_Event_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE431373 := __E_Property_Event;
  EXPORT __ST246354_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST246354_Layout __ND4869473__Project(E_Property_Event(__in,__cfg).Layout __PP431030) := TRANSFORM
    SELF.Dt_First_Seen_ := KEL.era.ToDate(__PP431030.Date_First_Seen_);
    ...
    SELF.Is_Current_Assessment_Record_ := __AND(__PP431030.Is_Assessment_,__OP2(FN_Compile(__cfg).FN_A_B_S_Y_E_A_R_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP431030.Date_First_Seen_)),__ECAST(KEL.typ.nkdate,__CC13049)),<=,__CN(1)));
    SELF := __PP431030;
  END;
  EXPORT __ENH_Property_Event_6 := PROJECT(__EE431373,__ND4869473__Project(LEFT));
END;

...
EXPORT B_Property_Event_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE431848 := __E_Property_Event;
  EXPORT __ST246876_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST246876_Layout __ND4877721__Project(E_Property_Event(__in,__cfg).Layout __PP431505) := TRANSFORM
    SELF.Dt_First_Seen_ := KEL.era.ToDate(__PP431505.Date_First_Seen_);
    ...
    SELF.Is_Current_Assessment_Record_ := __AND(__PP431505.Is_Assessment_,__OP2(FN_Compile(__cfg).FN_A_B_S_Y_E_A_R_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP431505.Date_First_Seen_)),__ECAST(KEL.typ.nkdate,__CC13074)),<=,__CN(1)));
    SELF := __PP431505;
  END;
  EXPORT __ENH_Property_Event_6 := PROJECT(__EE431848,__ND4877721__Project(LEFT));
END;

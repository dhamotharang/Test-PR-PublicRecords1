...
EXPORT E_Drivers_License(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __d0_Out := RECORD
    ...
    KEL.typ.uid UID := 0;
  END;
  SHARED __d0_UID_Mapped := JOIN(__d0_KELfiltered,Lookup,TRIM((STRING)LEFT.dl_number) + '|' + TRIM((STRING)LEFT.orig_state) = RIGHT.KeyVal,TRANSFORM(__d0_Out,SELF.UID:=RIGHT.UID,SELF:=LEFT),HASH);
  ...
  SHARED __d1_Out := RECORD
    ...
    KEL.typ.uid UID := 0;
  END;
  SHARED __d1_UID_Mapped := JOIN(__d1_KELfiltered,Lookup,TRIM((STRING)LEFT.dl_number) + '|' + TRIM((STRING)LEFT.orig_state) = RIGHT.KeyVal,TRANSFORM(__d1_Out,SELF.UID:=RIGHT.UID,SELF:=LEFT),HASH);
  ...
  Layout Drivers_License__Rollup(InLayout __r, DATASET(InLayout) __recs) := TRANSFORM
    ...
    SELF.Previous_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Date_Vendor_First_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_First_Reported_,MIN,FALSE),KEL.typ.epoch Date_Vendor_Last_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_Last_Reported_,MAX,FALSE),O_O_S_Previous_Drivers_License_Number_,Previous_State_,Drivers_License_Key_Number_,Previous_Drivers_License_Number_},O_O_S_Previous_Drivers_License_Number_,Previous_State_,Drivers_License_Key_Number_,Previous_Drivers_License_Number_),Previous_Layout)(__NN(O_O_S_Previous_Drivers_License_Number_) OR __NN(Previous_State_) OR __NN(Drivers_License_Key_Number_) OR __NN(Previous_Drivers_License_Number_)));
    SELF.Change_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Date_Vendor_First_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_First_Reported_,MIN,FALSE),KEL.typ.epoch Date_Vendor_Last_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_Last_Reported_,MAX,FALSE),Address_Change_,Name_Change_,Date_Of_Birth_Change_,Sex_Change_},Address_Change_,Name_Change_,Date_Of_Birth_Change_,Sex_Change_),Change_Layout)(__NN(Address_Change_) OR __NN(Name_Change_) OR __NN(Date_Of_Birth_Change_) OR __NN(Sex_Change_)));
    SELF.Person_Info_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Date_Vendor_First_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_First_Reported_,MIN,FALSE),KEL.typ.epoch Date_Vendor_Last_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_Last_Reported_,MAX,FALSE),Height_,Weight_,Race_,Race_Code_,Sex_,Sex_Code_,Hair_Color_,Hair_Color_Code_,Eye_Color_,Eye_Color_Code_},Height_,Weight_,Race_,Race_Code_,Sex_,Sex_Code_,Hair_Color_,Hair_Color_Code_,Eye_Color_,Eye_Color_Code_),Person_Info_Layout)(__NN(Height_) OR __NN(Weight_) OR __NN(Race_) OR __NN(Race_Code_) OR __NN(Sex_) OR __NN(Sex_Code_) OR __NN(Hair_Color_) OR __NN(Hair_Color_Code_) OR __NN(Eye_Color_) OR __NN(Eye_Color_Code_)));
  ...
  Layout Drivers_License__Single_Rollup(InLayout __r) := TRANSFORM
    SELF.Previous_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Previous_Layout,SELF.__RecordCount:=1;,SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF.Date_Vendor_First_Reported_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Vendor_First_Reported_,FALSE),SELF.Date_Vendor_Last_Reported_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Vendor_Last_Reported_,FALSE),SELF:=LEFT))(__NN(O_O_S_Previous_Drivers_License_Number_) OR __NN(Previous_State_) OR __NN(Drivers_License_Key_Number_) OR __NN(Previous_Drivers_License_Number_)));
    SELF.Change_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Change_Layout,SELF.__RecordCount:=1;,SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF.Date_Vendor_First_Reported_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Vendor_First_Reported_,FALSE),SELF.Date_Vendor_Last_Reported_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Vendor_Last_Reported_,FALSE),SELF:=LEFT))(__NN(Address_Change_) OR __NN(Name_Change_) OR __NN(Date_Of_Birth_Change_) OR __NN(Sex_Change_)));
    SELF.Person_Info_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Person_Info_Layout,SELF.__RecordCount:=1;,SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF.Date_Vendor_First_Reported_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Vendor_First_Reported_,FALSE),SELF.Date_Vendor_Last_Reported_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Vendor_Last_Reported_,FALSE),SELF:=LEFT))(__NN(Height_) OR __NN(Weight_) OR __NN(Race_) OR __NN(Race_Code_) OR __NN(Sex_) OR __NN(Sex_Code_) OR __NN(Hair_Color_) OR __NN(Hair_Color_Code_) OR __NN(Eye_Color_) OR __NN(Eye_Color_Code_)));

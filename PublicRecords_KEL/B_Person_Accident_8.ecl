...
EXPORT B_Person_Accident_8(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE419427 := __E_Person_Accident;
  SHARED __ST419752_Layout := RECORD
  ...
  END;
  SHARED __EE419341 := __E_Accident;
  SHARED __EE419353 := __EE419341.Report_Codes_;
  SHARED __CC14049 := ['FA','A','IA','EA','TF'];
  __JC5586045(E_Accident(__in,__cfg).Report_Codes_Layout __EE419353) := __T(__OP2(__EE419353.Report_Code_,IN,__CN(__CC14049)));
  SHARED __EE5586046 := __EE419341(EXISTS(__CHILDJOINFILTER(__EE419353,__JC5586045)));
  __JC5586079(E_Person_Accident(__in,__cfg).Layout __EE419427, E_Accident(__in,__cfg).Layout __EE5586046) := __EEQP(__EE419427.Acc_,__EE5586046.UID);
  __JF5586079(E_Accident(__in,__cfg).Layout __EE5586046) := __NN(__EE5586046.UID);
  SHARED __EE5586128 := JOIN(__EE419427,__EE5586046,__JC5586079(LEFT,RIGHT),TRANSFORM(__ST419752_Layout,SELF:=LEFT,SELF.Acc__1_:=__JF5586079(RIGHT)),HASH,LEFT OUTER,KEEP(1));
  EXPORT __ST275736_Layout := RECORD
    KEL.typ.ntyp(E_Person().Typ) Subject_;
    KEL.typ.ntyp(E_Accident().Typ) Acc_;
    KEL.typ.nstr Point_Of_Impact_;
    KEL.typ.nstr Driver_B_A_C_Test_Type_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ENH_Person_Accident_8 := PROJECT(__EE5586128,TRANSFORM(__ST275736_Layout,SELF.Is_Accident_Record_ := LEFT.Acc__1_,SELF := LEFT));
END;

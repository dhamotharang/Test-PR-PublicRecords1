...
EXPORT B_Person_Accident_8(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE350231 := __E_Person_Accident;
  SHARED __ST350556_Layout := RECORD
  ...
  END;
  SHARED __EE350145 := __E_Accident;
  SHARED __EE350157 := __EE350145.Report_Codes_;
  SHARED __CC13744 := ['FA','A','IA','EA','TF'];
  __JC4810804(E_Accident(__in,__cfg).Report_Codes_Layout __EE350157) := __T(__OP2(__EE350157.Report_Code_,IN,__CN(__CC13744)));
  SHARED __EE4810805 := __EE350145(EXISTS(__CHILDJOINFILTER(__EE350157,__JC4810804)));
  __JC4810838(E_Person_Accident(__in,__cfg).Layout __EE350231, E_Accident(__in,__cfg).Layout __EE4810805) := __EEQP(__EE350231.Acc_,__EE4810805.UID);
  __JF4810838(E_Accident(__in,__cfg).Layout __EE4810805) := __NN(__EE4810805.UID);
  SHARED __EE4810887 := JOIN(__EE350231,__EE4810805,__JC4810838(LEFT,RIGHT),TRANSFORM(__ST350556_Layout,SELF:=LEFT,SELF.Acc__1_:=__JF4810838(RIGHT)),HASH,LEFT OUTER,KEEP(1));
  EXPORT __ST251881_Layout := RECORD
    KEL.typ.ntyp(E_Person().Typ) Subject_;
    KEL.typ.ntyp(E_Accident().Typ) Acc_;
    KEL.typ.nstr Point_Of_Impact_;
    KEL.typ.nstr Driver_B_A_C_Test_Type_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ENH_Person_Accident_8 := PROJECT(__EE4810887,TRANSFORM(__ST251881_Layout,SELF.Is_Accident_Record_ := LEFT.Acc__1_,SELF := LEFT));
END;

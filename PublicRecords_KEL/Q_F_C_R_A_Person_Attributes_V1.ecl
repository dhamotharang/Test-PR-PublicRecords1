...
EXPORT Q_F_C_R_A_Person_Attributes_V1(KEL.typ.uid __PLexID_in, DATASET(RECORDOF(PublicRecords_KEL.ECL_Functions.Layouts.LayoutInputPII)) __PInputPIIDataset, KEL.typ.kdate __PP_InpClnArchDt, DATA100 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED __EE6445342 := __ENH_Person;
  SHARED __EE6446173 := __EE6445342(__T(__OP2(__EE6445342.UID,=,__CN(__PLexID_in))));
  SHARED __ST67498_Layout := RECORD
  ...
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(__EE6446173,TRANSFORM(__ST67498_Layout,SELF.Lex_I_D_ := LEFT.UID,SELF := LEFT)));
END;

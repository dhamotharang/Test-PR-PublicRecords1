...
EXPORT Q_F_C_R_A_Person_Attributes_V1(KEL.typ.uid __PLexID_in, DATASET(RECORDOF(PublicRecords_KEL.ECL_Functions.Layouts.LayoutInputPII)) __PInputPIIDataset, KEL.typ.kdate __PP_InpClnArchDt, DATA57 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED __EE6229426 := __ENH_Person;
  SHARED __EE6230369 := __EE6229426(__T(__OP2(__EE6229426.UID,=,__CN(__PLexID_in))));
  SHARED __ST74723_Layout := RECORD
  ...
  END;
  SHARED __ST74723_Layout __ND6230374__Project(B_Person(__in,__cfg_Local).__ST133569_Layout __PP6230370) := TRANSFORM
    SELF.Lex_I_D_ := __PP6230370.UID;
    ...
    SELF.P_L___Email_Cnt_Ev_ := __PP6230370.P_L___Email_Cnt_Ev_F_C_R_A_;
    SELF := __PP6230370;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(__EE6230369,__ND6230374__Project(LEFT)));
END;

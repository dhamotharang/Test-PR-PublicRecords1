...
EXPORT Q_F_C_R_A_Person_Attributes_V1(KEL.typ.uid __PLexID_in, KEL.typ.kdate __PP_InpClnArchDt, UNSIGNED8 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED __EE1439722 := __ENH_Person;
  SHARED __EE1445976 := __EE1439722(__T(__OP2(__EE1439722.UID,=,__CN(__PLexID_in))));
  SHARED __ST35623_Layout := RECORD
  ...
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(__EE1445976,TRANSFORM(__ST35623_Layout,SELF.Lex_I_D_ := LEFT.UID,SELF := LEFT)));
END;

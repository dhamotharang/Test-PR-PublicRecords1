...
EXPORT Q_F_C_R_A_Person_Attributes_V1(SET OF KEL.typ.uid __PLexIDs, KEL.typ.kdate __PArchiveDate, UNSIGNED1 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED __EE195949 := __ENH_Person;
  SHARED __EE197061 := __EE195949(__T(__OP2(__EE195949.UID,IN,__CN(__PLexIDs))));
  SHARED __ST14508_Layout := RECORD
    ...
    KEL.typ.ndataset(B_Person(__in,__cfg_Local).__ST14120_Layout) Dt_Of_Bks_List1_Y_;
    KEL.typ.ndataset(B_Person(__in,__cfg_Local).__ST14146_Layout) Dt_Of_Bks_List7_Y_;
    KEL.typ.ndataset(B_Person(__in,__cfg_Local).__ST14172_Layout) Dt_Of_Bks_List10_Y_;
  ...
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(__EE197061,TRANSFORM(__ST14508_Layout,SELF.Lex_I_D_ := LEFT.UID,SELF := LEFT)));
END;

...
IMPORT KEL15 AS KEL;
IMPORT B_Input_B_I_I_9,CFG_Compile,E_Business_Org,E_Business_Sele,E_Business_Sele_Overflow,E_Business_Ult,FN_Compile FROM PublicRecords_KEL;
IMPORT * FROM KEL15.Null;
EXPORT B_Input_B_I_I_8(CFG_Compile __cfg = CFG_Compile) := MODULE
  SHARED VIRTUAL TYPEOF(B_Input_B_I_I_9(__cfg).__ENH_Input_B_I_I_9) __ENH_Input_B_I_I_9 := B_Input_B_I_I_9(__cfg).__ENH_Input_B_I_I_9;
  SHARED __EE5239342 := __ENH_Input_B_I_I_9;
  EXPORT __ST259595_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST259595_Layout __ND5239648__Project(B_Input_B_I_I_9(__cfg).__ST261863_Layout __PP5239343) := TRANSFORM
    SELF.Addr_Not_Populated_ := FN_Compile(__cfg).FN_Is_Not_Enough_To_Clean(__ECAST(KEL.typ.nstr,__PP5239343.B___Inp_Addr_));
    __CC14738 := '-99999';
    __CC14743 := '-99998';
    SELF.Bus_Input_Phone_Clean_Value_ := FN_Compile(__cfg).FN_Is_Blank2_Fields(__ECAST(KEL.typ.nstr,__PP5239343.B___Inp_Phone_),__ECAST(KEL.typ.nstr,__CN(__CC14738)),__ECAST(KEL.typ.nstr,__PP5239343.B___Inp_Cln_Phone_),__ECAST(KEL.typ.nstr,__CN(__CC14743)));
    SELF.Bus_Input_T_I_N_Clean_Value_ := FN_Compile(__cfg).FN_Is_Blank2_Fields(__ECAST(KEL.typ.nstr,__PP5239343.B___Inp_T_I_N_),__ECAST(KEL.typ.nstr,__CN(__CC14738)),__ECAST(KEL.typ.nstr,__PP5239343.B___Inp_Cln_T_I_N_),__ECAST(KEL.typ.nstr,__CN(__CC14743)));
    SELF.City_State_Zip_Not_Populated_ := FN_Compile(__cfg).FN_City_State_Zip_Not_Populated_Check(__ECAST(KEL.typ.nstr,__PP5239343.B___Inp_Addr_City_),__ECAST(KEL.typ.nstr,__PP5239343.B___Inp_Addr_State_),__ECAST(KEL.typ.nstr,__PP5239343.B___Inp_Addr_Zip_));
    SELF := __PP5239343;
  END;
  EXPORT __ENH_Input_B_I_I_8 := PROJECT(__EE5239342,__ND5239648__Project(LEFT));
END;

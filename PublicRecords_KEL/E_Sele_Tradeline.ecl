...
IMPORT CFG_Compile,E_Business_Org,E_Business_Sele,E_Business_Sele_Overflow,E_Business_Ult,E_Tradeline FROM PublicRecords_KEL;
IMPORT * FROM KEL15.Null;
EXPORT E_Sele_Tradeline(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __Mapping0 := 'Legal_(DEFAULT:Legal_:0),Account_(DEFAULT:Account_:0),source(OVERRIDE:Source_:\'\'),archive_date(OVERRIDE:Archive___Date_:EPOCH),dt_first_seen(OVERRIDE:Date_First_Seen_:EPOCH),dt_last_seen(OVERRIDE:Date_Last_Seen_:EPOCH),hybrid_archive_date(OVERRIDE:Hybrid_Archive_Date_:EPOCH:Hybrid_Archive_Date_0Rule),vaultdatelastseen(DEFAULT:Vault_Date_Last_Seen_:EPOCH),DPMBitmap(OVERRIDE:__Permits:PERMITS)';
  EXPORT __d0_KELfiltered := PublicRecords_KEL.Files.NonFCRA.Cortera_Tradeline__Key_LinkIds_Vault(status NOT IN ['D', 'R']);
  SHARED __d0_Legal__Layout := RECORD
    RECORDOF(__d0_KELfiltered);
    KEL.typ.uid Legal_;
  END;
  SHARED __d0_Missing_Legal__UIDComponents := KEL.Intake.ConstructMissingFieldList(__d0_KELfiltered,'ultid,orgid,seleid','PublicRecords_KEL.Files.NonFCRA.Cortera_Tradeline__Key_LinkIds_Vault');
  ...
  END;
  SHARED __d0_Missing_Account__UIDComponents := KEL.Intake.ConstructMissingFieldList(__d0_Legal__Mapped,'ultid,orgid,seleid,account_key','PublicRecords_KEL.Files.NonFCRA.Cortera_Tradeline__Key_LinkIds_Vault');
  SHARED __d0_Account__Mapped := IF(__d0_Missing_Account__UIDComponents = 'ultid,orgid,seleid,account_key',PROJECT(__d0_Legal__Mapped,TRANSFORM(__d0_Account__Layout,SELF.Account_:=0,SELF:=LEFT)),JOIN(KEL.Intake.AppendFields(__d0_Legal__Mapped,__d0_Missing_Account__UIDComponents),E_Tradeline(__cfg).Lookup,TRIM((STRING)LEFT.ultid) + '|' + TRIM((STRING)LEFT.orgid) + '|' + TRIM((STRING)LEFT.seleid) + '|' + TRIM((STRING)LEFT.account_key) = RIGHT.KeyVal,TRANSFORM(__d0_Account__Layout,SELF.Account_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,SMART));
  ...
  EXPORT Account__Orphan := JOIN(InData(__NN(Account_)),E_Tradeline(__cfg).__Result,__EEQP(LEFT.Account_, RIGHT.UID),TRANSFORM(InLayout,SELF := LEFT,SELF:=[]),LEFT ONLY, HASH);
  ...
  EXPORT NullCounts := DATASET([
    {'SeleTradeline','PublicRecords_KEL.Files.NonFCRA.Cortera_Tradeline__Key_LinkIds_Vault','Legal',COUNT(__d0(__NL(Legal_))),COUNT(__d0(__NN(Legal_)))},
    {'SeleTradeline','PublicRecords_KEL.Files.NonFCRA.Cortera_Tradeline__Key_LinkIds_Vault','Account',COUNT(__d0(__NL(Account_))),COUNT(__d0(__NN(Account_)))},
    {'SeleTradeline','PublicRecords_KEL.Files.NonFCRA.Cortera_Tradeline__Key_LinkIds_Vault','source',COUNT(__d0(__NL(Source_))),COUNT(__d0(__NN(Source_)))},
    {'SeleTradeline','PublicRecords_KEL.Files.NonFCRA.Cortera_Tradeline__Key_LinkIds_Vault','Archive_Date',COUNT(__d0(Archive___Date_=0)),COUNT(__d0(Archive___Date_!=0))},
    {'SeleTradeline','PublicRecords_KEL.Files.NonFCRA.Cortera_Tradeline__Key_LinkIds_Vault','DateFirstSeen',COUNT(__d0(Date_First_Seen_=0)),COUNT(__d0(Date_First_Seen_!=0))},
    {'SeleTradeline','PublicRecords_KEL.Files.NonFCRA.Cortera_Tradeline__Key_LinkIds_Vault','DateLastSeen',COUNT(__d0(Date_Last_Seen_=0)),COUNT(__d0(Date_Last_Seen_!=0))},
    {'SeleTradeline','PublicRecords_KEL.Files.NonFCRA.Cortera_Tradeline__Key_LinkIds_Vault','HybridArchiveDate',COUNT(__d0(Hybrid_Archive_Date_=0)),COUNT(__d0(Hybrid_Archive_Date_!=0))},
    {'SeleTradeline','PublicRecords_KEL.Files.NonFCRA.Cortera_Tradeline__Key_LinkIds_Vault','VaultDateLastSeen',COUNT(__d0(Vault_Date_Last_Seen_=0)),COUNT(__d0(Vault_Date_Last_Seen_!=0))}]

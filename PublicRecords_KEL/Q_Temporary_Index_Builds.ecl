...
IMPORT KEL11 AS KEL;
IMPORT B_Bankruptcy_1,B_Bankruptcy_2,B_Bankruptcy_3,B_Bankruptcy_4,B_Bankruptcy_5,B_Bankruptcy_6,B_Bankruptcy_7,B_Business_Prox,B_Business_Prox_1,B_Business_Prox_2,B_Business_Prox_3,B_Business_Prox_4,B_Business_Sele,B_Business_Sele_1,B_Business_Sele_2,B_Business_Sele_3,B_Business_Sele_4,B_Business_Sele_5,B_Business_Sele_6,B_Lien_Judgment_6,B_Phone,B_Sele_Aircraft_1,B_Sele_Bankruptcy_1,B_Sele_Lien_Judgment_6,B_Sele_Tradeline_1,B_Sele_Tradeline_2,B_Sele_Tradeline_3,B_Sele_Vehicle_1,B_Sele_Vehicle_2,B_Sele_Vehicle_3,B_Sele_Vehicle_4,B_Sele_Watercraft_1,B_Tradeline_1,B_Tradeline_10,B_Tradeline_11,B_Tradeline_2,B_Tradeline_3,B_Tradeline_4,B_Tradeline_5,B_Tradeline_6,B_Tradeline_7,B_Tradeline_8,B_Tradeline_9,CFG_Compile,E_Address,E_Bankruptcy,E_Business_Org,E_Business_Prox,E_Business_Sele,E_Business_Ult,E_Lien_Judgment,E_Phone,E_Prox_Address,E_Prox_Phone_Number,E_Prox_T_I_N,E_Sele_Address,E_Sele_Aircraft,E_Sele_Bankruptcy,E_Sele_Lien_Judgment,E_Sele_Phone_Number,E_Sele_T_I_N,E_Sele_Tradeline,E_Sele_Vehicle,E_Sele_Watercraft,E_T_I_N,E_Tradeline,E_Vehicle,E_Zip_Code FROM PublicRecords_KEL;
...
EXPORT Q_Temporary_Index_Builds(KEL.typ.int __PUltID_in, KEL.typ.int __POrgID_in, KEL.typ.int __PSeleID_in, KEL.typ.int __PProxID_in, KEL.typ.str __PPrimaryRange_in, KEL.typ.str __PPredirectional_in, KEL.typ.str __PPrimaryName_in, KEL.typ.str __PAddrSuffix_in, KEL.typ.str __PPostdirectional_in, KEL.typ.uid __PZIP5_in, KEL.typ.str __PSecondaryRange_in, KEL.typ.str __PPhone_in, KEL.typ.kdate __PP_InpClnArchDt, UNSIGNED8 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED E_Sele_Lien_Judgment_Filtered_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE(E_Sele_Lien_Judgment(__in,__cfg))
    ...
    SHARED __FilterPart1(GROUPED DATASET(__InLayoutExtended) __ds) := JOIN(__ds,E_Business_Sele_Filtered_3(__in,__cfg).__PostFilter,__NNEQ(LEFT.T_M_S_I_D_,RIGHT.UID) AND __T(__OP2(LEFT.T_M_S_I_D_,=,__CAST(KEL.typ.str,RIGHT.UID))),TRANSFORM(__InLayoutExtended,SELF.__Link1:=__NN(RIGHT.UID),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
  ...
  SHARED __EE2241731 := __ENH_Business_Sele;
  SHARED __EE2241752 := __EE2241731(__T(__AND(__OP2(__EE2241731.Ult_I_D_,=,__CN(__PUltID_in)),__AND(__OP2(__EE2241731.Org_I_D_,=,__CN(__POrgID_in)),__OP2(__EE2241731.Sele_I_D_,=,__CN(__PSeleID_in))))));
  SHARED __EE2241754 := __EE2241752;
  EXPORT Res0 := __UNWRAP(__EE2241754);
  SHARED __EE2243349 := __ENH_Business_Prox;
  SHARED __EE2243374 := __EE2243349(__T(__AND(__OP2(__EE2243349.Ult_I_D_,=,__CN(__PUltID_in)),__AND(__OP2(__EE2243349.Org_I_D_,=,__CN(__POrgID_in)),__AND(__OP2(__EE2243349.Sele_I_D_,=,__CN(__PSeleID_in)),__OP2(__EE2243349.Prox_I_D_,=,__CN(__PProxID_in)))))));
  SHARED __EE2243376 := __EE2243374;
  EXPORT Res1 := __UNWRAP(__EE2243376);
  SHARED __EE2243397 := __ENH_Phone;
  SHARED __EE2243409 := __EE2243397(__T(__OP2(__EE2243397.Phone10_,=,__CN(__PPhone_in))));
  SHARED __EE2243411 := __EE2243409;
  EXPORT Res2 := __UNWRAP(__EE2243411);
  SHARED __EE2243451 := __E_Address;
  SHARED __EE2243488 := __EE2243451(__T(__AND(__OP2(__EE2243451.Primary_Range_,=,__CN(__PPrimaryRange_in)),__AND(__OP2(__EE2243451.Predirectional_,=,__CN(__PPredirectional_in)),__AND(__OP2(__EE2243451.Primary_Name_,=,__CN(__PPrimaryName_in)),__AND(__OP2(__EE2243451.Suffix_,=,__CN(__PAddrSuffix_in)),__AND(__OP2(__EE2243451.Postdirectional_,=,__CN(__PPostdirectional_in)),__AND(__OP2(__EE2243451.Z_I_P5_,=,__CN(__PZIP5_in)),__OP2(__EE2243451.Secondary_Range_,=,__CN(__PSecondaryRange_in))))))))));
  EXPORT Res3 := __UNWRAP(__EE2243488);
END;

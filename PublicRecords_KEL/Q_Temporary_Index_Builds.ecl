...
EXPORT Q_Temporary_Index_Builds(KEL.typ.int __PUltID_in, KEL.typ.int __POrgID_in, KEL.typ.int __PSeleID_in, KEL.typ.int __PProxID_in, KEL.typ.str __PPrimaryRange_in, KEL.typ.str __PPredirectional_in, KEL.typ.str __PPrimaryName_in, KEL.typ.str __PAddrSuffix_in, KEL.typ.str __PPostdirectional_in, KEL.typ.uid __PZIP5_in, KEL.typ.str __PSecondaryRange_in, KEL.typ.str __PPhone_in, KEL.typ.kdate __PP_InpClnArchDt, DATA57 __PDPM) := MODULE
  ...
  END;
  SHARED E_Property_Filtered_2 := MODULE(E_Property(__cfg_Local))
    SHARED __AsofFitler(DATASET(InLayout) __ds) := __ds(__T(__OP2(KEL.era.ToDateMinNull(__ds.Archive___Date_),<=,__CN(__PP_InpClnArchDt))));
    SHARED __UsingFitler(DATASET(InLayout) __ds) := __ds(KEL.Permits.BitAnd(__ds.__Permits,__PDPM) = __ds.__Permits);
  ...
  SHARED E_Address_Property_Filtered_1 := MODULE(E_Address_Property(__cfg_Local))
    ...
    SHARED __UsingFitler(DATASET(InLayout) __ds) := __ds(KEL.Permits.BitAnd(__ds.__Permits,__PDPM) = __ds.__Permits);
    SHARED __SourceFilter(DATASET(InLayout) __ds) := __UsingFitler(__AsofFitler(__ds));
    SHARED __GroupedFilter(GROUPED DATASET(InLayout) __ds) := JOIN(__ds,E_Property_Filtered_2.__PostFilter,__EEQP(LEFT.Prop_,RIGHT.UID),TRANSFORM(InLayout,SELF:=LEFT),HASH,GROUPED,KEEP(1));
  END;
  ...
  SHARED E_Business_Prox_Filtered := MODULE(E_Business_Prox(__cfg_Local))
    ...
    SHARED __InLayoutExtended := RECORD
      ...
      KEL.typ.bool __Link0 := FALSE;
    END;
    SHARED __FilterPart0(GROUPED DATASET(InLayout) __ds) := JOIN(__ds,JOIN(__ds,E_Business_Sele_Filtered_3.__PostFilter,__EEQP(LEFT.Prox_Sele_,RIGHT.UID),TRANSFORM({KEL.typ.nuid UID},SELF:=LEFT,SELF:=RIGHT),HASH,KEEP(1)),__EEQP(LEFT.UID,RIGHT.UID),TRANSFORM(__InLayoutExtended,SELF.__Link0:=__NN(RIGHT.UID),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
  ...
  SHARED TYPEOF(E_Property(__cfg_Local).__Result) __E_Property := E_Property_Filtered.__Result;
  SHARED __EE13739294 := __ENH_Business_Sele;
  SHARED __EE13740632 := __EE13739294(__T(__AND(__OP2(__EE13739294.Ult_I_D_,=,__CN(__PUltID_in)),__AND(__OP2(__EE13739294.Org_I_D_,=,__CN(__POrgID_in)),__OP2(__EE13739294.Sele_I_D_,=,__CN(__PSeleID_in))))));
  SHARED __EE13740634 := __EE13740632;
  EXPORT Res0 := __UNWRAP(__EE13740634);
  SHARED __EE13740637 := __ENH_Business_Prox;
  SHARED __EE13740906 := __EE13740637(__T(__AND(__OP2(__EE13740637.Ult_I_D_,=,__CN(__PUltID_in)),__AND(__OP2(__EE13740637.Org_I_D_,=,__CN(__POrgID_in)),__AND(__OP2(__EE13740637.Sele_I_D_,=,__CN(__PSeleID_in)),__OP2(__EE13740637.Prox_I_D_,=,__CN(__PProxID_in)))))));
  SHARED __EE13740908 := __EE13740906;
  EXPORT Res1 := __UNWRAP(__EE13740908);
  SHARED __EE5199808 := __E_Phone;
  SHARED __EE13740985 := __EE5199808(__T(__OP2(__EE5199808.Phone10_,=,__CN(__PPhone_in))));
  SHARED __EE13740987 := __EE13740985;
  EXPORT Res2 := __UNWRAP(__EE13740987);
  SHARED __EE13740990 := __ENH_Address;
  SHARED __EE13741176 := __EE13740990(__T(__AND(__OP2(__EE13740990.Primary_Range_,=,__CN(__PPrimaryRange_in)),__AND(__OP2(__EE13740990.Predirectional_,=,__CN(__PPredirectional_in)),__AND(__OP2(__EE13740990.Primary_Name_,=,__CN(__PPrimaryName_in)),__AND(__OP2(__EE13740990.Suffix_,=,__CN(__PAddrSuffix_in)),__AND(__OP2(__EE13740990.Postdirectional_,=,__CN(__PPostdirectional_in)),__AND(__OP2(__EE13740990.Z_I_P5_,=,__CN(__PZIP5_in)),__OP2(__EE13740990.Secondary_Range_,=,__CN(__PSecondaryRange_in))))))))));
  SHARED __EE13741178 := __EE13741176;
  EXPORT Res3 := __UNWRAP(__EE13741178);
  SHARED __EE13741181 := __ENH_Property;
  SHARED __EE13741269 := __EE13741181(__T(__AND(__OP2(__EE13741181.Primary_Range_,=,__CN(__PPrimaryRange_in)),__AND(__OP2(__EE13741181.Predirectional_,=,__CN(__PPredirectional_in)),__AND(__OP2(__EE13741181.Primary_Name_,=,__CN(__PPrimaryName_in)),__AND(__OP2(__EE13741181.Suffix_,=,__CN(__PAddrSuffix_in)),__AND(__OP2(__EE13741181.Postdirectional_,=,__CN(__PPostdirectional_in)),__AND(__OP2(__EE13741181.Z_I_P5_,=,__CN(__PZIP5_in)),__OP2(__EE13741181.Secondary_Range_,=,__CN(__PSecondaryRange_in))))))))));
  EXPORT Res4 := __UNWRAP(__EE13741269);
  ...
  EXPORT DBG_OutputAll := PARALLEL(
    ...
    OUTPUT(DBG_E_Person_Property_Intermediate_8,NAMED('DBG_E_Person_Property_Intermediate_8_Q_Temporary_Index_Builds')),
    OUTPUT(DBG_E_Property_Event_Intermediate_8,NAMED('DBG_E_Property_Event_Intermediate_8_Q_Temporary_Index_Builds')),
    ...
    OUTPUT(DBG_E_Person_Property_Intermediate_7,NAMED('DBG_E_Person_Property_Intermediate_7_Q_Temporary_Index_Builds')),
    OUTPUT(DBG_E_Property_Event_Intermediate_7,NAMED('DBG_E_Property_Event_Intermediate_7_Q_Temporary_Index_Builds')),
    ...
    OUTPUT(DBG_E_Person_S_S_N_Intermediate_6,NAMED('DBG_E_Person_S_S_N_Intermediate_6_Q_Temporary_Index_Builds')),

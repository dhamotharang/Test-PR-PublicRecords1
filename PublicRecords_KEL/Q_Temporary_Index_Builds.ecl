...
EXPORT Q_Temporary_Index_Builds(KEL.typ.int __PUltID_in, KEL.typ.int __POrgID_in, KEL.typ.int __PSeleID_in, KEL.typ.int __PProxID_in, KEL.typ.str __PPrimaryRange_in, KEL.typ.str __PPredirectional_in, KEL.typ.str __PPrimaryName_in, KEL.typ.str __PAddrSuffix_in, KEL.typ.str __PPostdirectional_in, KEL.typ.uid __PZIP5_in, KEL.typ.str __PSecondaryRange_in, KEL.typ.str __PPhone_in, KEL.typ.kdate __PP_InpClnArchDt, DATA57 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED TYPEOF(E_Property(__in,__cfg_Local).__Result) __E_Property := E_Property_Filtered.__Result;
  SHARED __EE11954233 := __ENH_Business_Sele;
  SHARED __EE11955571 := __EE11954233(__T(__AND(__OP2(__EE11954233.Ult_I_D_,=,__CN(__PUltID_in)),__AND(__OP2(__EE11954233.Org_I_D_,=,__CN(__POrgID_in)),__OP2(__EE11954233.Sele_I_D_,=,__CN(__PSeleID_in))))));
  SHARED __EE11955573 := __EE11955571;
  EXPORT Res0 := __UNWRAP(__EE11955573);
  SHARED __EE11955576 := __ENH_Business_Prox;
  SHARED __EE11955845 := __EE11955576(__T(__AND(__OP2(__EE11955576.Ult_I_D_,=,__CN(__PUltID_in)),__AND(__OP2(__EE11955576.Org_I_D_,=,__CN(__POrgID_in)),__AND(__OP2(__EE11955576.Sele_I_D_,=,__CN(__PSeleID_in)),__OP2(__EE11955576.Prox_I_D_,=,__CN(__PProxID_in)))))));
  SHARED __EE11955847 := __EE11955845;
  EXPORT Res1 := __UNWRAP(__EE11955847);
  SHARED __EE4765052 := __E_Phone;
  SHARED __EE11955924 := __EE4765052(__T(__OP2(__EE4765052.Phone10_,=,__CN(__PPhone_in))));
  SHARED __EE11955926 := __EE11955924;
  EXPORT Res2 := __UNWRAP(__EE11955926);
  SHARED __EE11955929 := __ENH_Address;
  SHARED __EE11956115 := __EE11955929(__T(__AND(__OP2(__EE11955929.Primary_Range_,=,__CN(__PPrimaryRange_in)),__AND(__OP2(__EE11955929.Predirectional_,=,__CN(__PPredirectional_in)),__AND(__OP2(__EE11955929.Primary_Name_,=,__CN(__PPrimaryName_in)),__AND(__OP2(__EE11955929.Suffix_,=,__CN(__PAddrSuffix_in)),__AND(__OP2(__EE11955929.Postdirectional_,=,__CN(__PPostdirectional_in)),__AND(__OP2(__EE11955929.Z_I_P5_,=,__CN(__PZIP5_in)),__OP2(__EE11955929.Secondary_Range_,=,__CN(__PSecondaryRange_in))))))))));
  SHARED __EE11956117 := __EE11956115;
  EXPORT Res3 := __UNWRAP(__EE11956117);
  SHARED __EE11956120 := __ENH_Property;
  SHARED __EE11956208 := __EE11956120(__T(__AND(__OP2(__EE11956120.Primary_Range_,=,__CN(__PPrimaryRange_in)),__AND(__OP2(__EE11956120.Predirectional_,=,__CN(__PPredirectional_in)),__AND(__OP2(__EE11956120.Primary_Name_,=,__CN(__PPrimaryName_in)),__AND(__OP2(__EE11956120.Suffix_,=,__CN(__PAddrSuffix_in)),__AND(__OP2(__EE11956120.Postdirectional_,=,__CN(__PPostdirectional_in)),__AND(__OP2(__EE11956120.Z_I_P5_,=,__CN(__PZIP5_in)),__OP2(__EE11956120.Secondary_Range_,=,__CN(__PSecondaryRange_in))))))))));
  EXPORT Res4 := __UNWRAP(__EE11956208);

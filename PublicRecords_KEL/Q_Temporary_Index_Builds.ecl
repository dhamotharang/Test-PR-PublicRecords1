...
EXPORT Q_Temporary_Index_Builds(KEL.typ.int __PUltID_in, KEL.typ.int __POrgID_in, KEL.typ.int __PSeleID_in, KEL.typ.int __PProxID_in, KEL.typ.str __PPrimaryRange_in, KEL.typ.str __PPredirectional_in, KEL.typ.str __PPrimaryName_in, KEL.typ.str __PAddrSuffix_in, KEL.typ.str __PPostdirectional_in, KEL.typ.uid __PZIP5_in, KEL.typ.str __PSecondaryRange_in, KEL.typ.str __PPhone_in, KEL.typ.kdate __PP_InpClnArchDt, UNSIGNED8 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED E_Sele_Person_Filtered_2 := MODULE(E_Sele_Person(__in,__cfg_Local))
    ...
    SHARED __InLayoutExtended := RECORD
      ...
      KEL.typ.bool __Link1 := FALSE;
    END;
    SHARED __FilterPart0(GROUPED DATASET(InLayout) __ds) := JOIN(__ds,E_Business_Sele_Filtered_8.__PostFilter,__EEQP(LEFT.Legal_,RIGHT.UID),TRANSFORM(__InLayoutExtended,SELF.__Link0:=__NN(RIGHT.UID),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
    SHARED __FilterPart1(GROUPED DATASET(__InLayoutExtended) __ds) := JOIN(__ds,E_Sele_Address_Filtered_3.__PostFilter,__EEQP(LEFT.Legal_,RIGHT.Legal_),TRANSFORM(__InLayoutExtended,SELF.__Link1:=__NN(RIGHT.Legal_),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
    SHARED __GroupTest(DATASET(__InLayoutExtended) __g) := EXISTS(__g(__g.__Link0)) OR EXISTS(__g(__g.__Link1));
    SHARED __SourceFilter(DATASET(InLayout) __ds) := __UsingFitler(__AsofFitler(__ds));
    SHARED __GroupedFilter(GROUPED DATASET(InLayout) __ds) := PROJECT(HAVING(__FilterPart1(__FilterPart0(__ds)),__GroupTest(ROWS(LEFT))),InLayout);
  END;
  ...
  SHARED E_Business_Prox_Filtered_1 := MODULE(E_Business_Prox(__in,__cfg_Local))
    ...
    SHARED __InLayoutExtended := RECORD
      ...
      KEL.typ.bool __Link1 := FALSE;
    END;
    SHARED __FilterPart0(GROUPED DATASET(InLayout) __ds) := JOIN(__ds,JOIN(__ds,E_Business_Sele_Filtered_8.__PostFilter,__EEQP(LEFT.Prox_Sele_,RIGHT.UID),TRANSFORM({KEL.typ.nuid UID},SELF:=LEFT,SELF:=RIGHT),HASH,KEEP(1)),__EEQP(LEFT.UID,RIGHT.UID),TRANSFORM(__InLayoutExtended,SELF.__Link0:=__NN(RIGHT.UID),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
    ...
    SHARED __GroupTest(DATASET(__InLayoutExtended) __g) := EXISTS(__g(__g.__Link0)) OR EXISTS(__g(__g.__Link1)) OR EXISTS(__g(__T(__OP2(__g.Ult_I_D_,=,__CN(__PUltID_in))))) AND EXISTS(__g(__T(__OP2(__g.Org_I_D_,=,__CN(__POrgID_in))))) AND EXISTS(__g(__T(__OP2(__g.Sele_I_D_,=,__CN(__PSeleID_in))))) AND EXISTS(__g(__T(__OP2(__g.Prox_I_D_,=,__CN(__PProxID_in)))));
    SHARED __SourceFilter(DATASET(InLayout) __ds) := __UsingFitler(__AsofFitler(__ds));
    SHARED __GroupedFilter(GROUPED DATASET(InLayout) __ds) := PROJECT(HAVING(__FilterPart1(__FilterPart0(__ds)),__GroupTest(ROWS(LEFT))),InLayout);
  END;
  ...
  SHARED E_Sele_Bankruptcy_Filtered_1 := MODULE(E_Sele_Bankruptcy(__in,__cfg_Local))
    ...
    END;
    SHARED __FilterPart0(GROUPED DATASET(InLayout) __ds) := JOIN(__ds,E_Business_Sele_Filtered_8.__PostFilter,__EEQP(LEFT.Legal_,RIGHT.UID),TRANSFORM(__InLayoutExtended,SELF.__Link0:=__NN(RIGHT.UID),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
    SHARED __FilterPart1(GROUPED DATASET(__InLayoutExtended) __ds) := JOIN(__ds,E_Sele_Address_Filtered_2.__PostFilter,__EEQP(LEFT.Legal_,RIGHT.Legal_),TRANSFORM(__InLayoutExtended,SELF.__Link1:=__NN(RIGHT.Legal_),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
    SHARED __GroupTest(DATASET(__InLayoutExtended) __g) := EXISTS(__g(__g.__Link0)) OR EXISTS(__g(__g.__Link1));
    SHARED __SourceFilter(DATASET(InLayout) __ds) := __UsingFitler(__AsofFitler(__ds));
    SHARED __GroupedFilter(GROUPED DATASET(InLayout) __ds) := PROJECT(HAVING(__FilterPart1(__FilterPart0(__ds)),__GroupTest(ROWS(LEFT))),InLayout);
  END;
  SHARED E_Sele_Lien_Judgment_Filtered_1 := MODULE(E_Sele_Lien_Judgment(__in,__cfg_Local))
    ...
    SHARED __InLayoutExtended := RECORD
      ...
      KEL.typ.bool __Link1 := FALSE;
    END;
    SHARED __FilterPart0(GROUPED DATASET(InLayout) __ds) := JOIN(__ds,E_Business_Sele_Filtered_8.__PostFilter,__EEQP(LEFT.Sele_,RIGHT.UID),TRANSFORM(__InLayoutExtended,SELF.__Link0:=__NN(RIGHT.UID),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
    SHARED __FilterPart1(GROUPED DATASET(__InLayoutExtended) __ds) := JOIN(__ds,E_Sele_Address_Filtered_2.__PostFilter,__EEQP(LEFT.Sele_,RIGHT.Legal_),TRANSFORM(__InLayoutExtended,SELF.__Link1:=__NN(RIGHT.Legal_),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
    SHARED __GroupTest(DATASET(__InLayoutExtended) __g) := EXISTS(__g(__g.__Link0)) OR EXISTS(__g(__g.__Link1));
    SHARED __SourceFilter(DATASET(InLayout) __ds) := __UsingFitler(__AsofFitler(__ds));
    SHARED __GroupedFilter(GROUPED DATASET(InLayout) __ds) := PROJECT(HAVING(__FilterPart1(__FilterPart0(__ds)),__GroupTest(ROWS(LEFT))),InLayout);
  END;
  ...
  SHARED E_Address_Filtered := MODULE(E_Address(__in,__cfg_Local))
    ...
    SHARED __InLayoutExtended := RECORD
      ...
      KEL.typ.bool __Link1 := FALSE;
    END;
    SHARED __FilterPart0(GROUPED DATASET(InLayout) __ds) := JOIN(__ds,E_Address_Property_Event_Filtered_5.__PostFilter,__EEQP(LEFT.UID,RIGHT.Location_),TRANSFORM(__InLayoutExtended,SELF.__Link0:=__NN(RIGHT.Location_),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
  ...
  SHARED E_Aircraft_Owner_Filtered := MODULE(E_Aircraft_Owner(__in,__cfg_Local))
    ...
    SHARED __UsingFitler(DATASET(InLayout) __ds) := __ds((__ds.__Permits & __PDPM) = __ds.__Permits);
    SHARED __SourceFilter(DATASET(InLayout) __ds) := __UsingFitler(__AsofFitler(__ds));
    SHARED __GroupedFilter(GROUPED DATASET(InLayout) __ds) := JOIN(__ds,E_Sele_Person_Filtered_1.__PostFilter,__EEQP(LEFT.Owner_,RIGHT.Contact_),TRANSFORM(InLayout,SELF:=LEFT),HASH,GROUPED,KEEP(1));
  END;
  ...
  SHARED E_Business_Sele_Filtered := MODULE(E_Business_Sele(__in,__cfg_Local))
    ...
    SHARED __GroupTest(DATASET(__InLayoutExtended) __g) := EXISTS(__g(__g.__Link0)) OR EXISTS(__g(__T(__OP2(__g.Ult_I_D_,=,__CN(__PUltID_in))))) AND EXISTS(__g(__T(__OP2(__g.Org_I_D_,=,__CN(__POrgID_in))))) AND EXISTS(__g(__T(__OP2(__g.Sele_I_D_,=,__CN(__PSeleID_in)))));
    SHARED __SourceFilter(DATASET(InLayout) __ds) := __UsingFitler(__AsofFitler(__ds));
    SHARED __GroupedFilter(GROUPED DATASET(InLayout) __ds) := PROJECT(HAVING(__FilterPart0(__ds),__GroupTest(ROWS(LEFT))),InLayout);
  END;
  ...
  SHARED E_First_Degree_Associations_Filtered := MODULE(E_First_Degree_Associations(__in,__cfg_Local))
    ...
    SHARED __SourceFilter(DATASET(InLayout) __ds) := JOIN(__SimpleFilter(__UsingFitler(__AsofFitler(__ds))),E_Sele_Person_Filtered_1.__PostFilter,__EEQP(LEFT.Subject_,RIGHT.Contact_),TRANSFORM(InLayout,SELF:=LEFT),HASH,KEEP(1));
  END;
  ...
  SHARED E_Person_Filtered := MODULE(E_Person(__in,__cfg_Local))
    ...
    SHARED __UsingFitler(DATASET(InLayout) __ds) := __ds((__ds.__Permits & __PDPM) = __ds.__Permits);
    SHARED __SourceFilter(DATASET(InLayout) __ds) := __UsingFitler(__AsofFitler(__ds));
    SHARED __GroupedFilter(GROUPED DATASET(InLayout) __ds) := JOIN(__ds,E_Sele_Person_Filtered_1.__PostFilter,__EEQP(LEFT.UID,RIGHT.Contact_),TRANSFORM(InLayout,SELF:=LEFT),HASH,GROUPED,KEEP(1));
  END;
  SHARED E_Person_Address_Filtered := MODULE(E_Person_Address(__in,__cfg_Local))
    SHARED __AsofFitler(DATASET(InLayout) __ds) := __ds(__T(__OP2(KEL.era.ToDateMinNull(__ds.Archive___Date_),<=,__CN(__PP_InpClnArchDt))));
    SHARED __UsingFitler(DATASET(InLayout) __ds) := __ds((__ds.__Permits & __PDPM) = __ds.__Permits);
    SHARED __SourceFilter(DATASET(InLayout) __ds) := __UsingFitler(__AsofFitler(__ds));
    SHARED __GroupedFilter(GROUPED DATASET(InLayout) __ds) := JOIN(__ds,E_Sele_Person_Filtered_1.__PostFilter,__EEQP(LEFT.Subject_,RIGHT.Contact_),TRANSFORM(InLayout,SELF:=LEFT),HASH,GROUPED,KEEP(1));
  END;
  ...
  SHARED E_Property_Filtered := MODULE(E_Property(__in,__cfg_Local))
    ...
    SHARED __InLayoutExtended := RECORD
      ...
      KEL.typ.bool __Link0 := FALSE;
    END;
    SHARED __FilterPart0(GROUPED DATASET(InLayout) __ds) := JOIN(__ds,E_Sele_Property_Filtered_1.__PostFilter,__EEQP(LEFT.UID,RIGHT.Prop_),TRANSFORM(__InLayoutExtended,SELF.__Link0:=__NN(RIGHT.Prop_),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
    SHARED __GroupTest(DATASET(__InLayoutExtended) __g) := EXISTS(__g(__g.__Link0)) OR EXISTS(__g(__T(__OP2(__g.Primary_Range_,=,__CN(__PPrimaryRange_in))))) AND EXISTS(__g(__T(__OP2(__g.Predirectional_,=,__CN(__PPredirectional_in))))) AND EXISTS(__g(__T(__OP2(__g.Primary_Name_,=,__CN(__PPrimaryName_in))))) AND EXISTS(__g(__T(__OP2(__g.Suffix_,=,__CN(__PAddrSuffix_in))))) AND EXISTS(__g(__T(__OP2(__g.Postdirectional_,=,__CN(__PPostdirectional_in))))) AND EXISTS(__g(__T(__OP2(__g.Z_I_P5_,=,__CN(__PZIP5_in))))) AND EXISTS(__g(__T(__OP2(__g.Secondary_Range_,=,__CN(__PSecondaryRange_in)))));
    SHARED __SourceFilter(DATASET(InLayout) __ds) := __UsingFitler(__AsofFitler(__ds));
    SHARED __GroupedFilter(GROUPED DATASET(InLayout) __ds) := PROJECT(HAVING(__FilterPart0(__ds),__GroupTest(ROWS(LEFT))),InLayout);
  END;
  SHARED E_Property_Event_Filtered := MODULE(E_Property_Event(__in,__cfg_Local))
    ...
    SHARED __FilterPart1(GROUPED DATASET(__InLayoutExtended) __ds) := JOIN(__ds,JOIN(__ds,E_Sele_Property_Filtered_1.__PostFilter,__EEQP(LEFT.Prop_,RIGHT.Prop_),TRANSFORM({KEL.typ.nuid UID},SELF:=LEFT,SELF:=RIGHT),HASH,KEEP(1)),__EEQP(LEFT.UID,RIGHT.UID),TRANSFORM(__InLayoutExtended,SELF.__Link1:=__NN(RIGHT.UID),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
    SHARED __FilterPart2(GROUPED DATASET(__InLayoutExtended) __ds) := JOIN(__ds,E_Address_Property_Event_Filtered_5.__PostFilter,__EEQP(LEFT.UID,RIGHT.Event_),TRANSFORM(__InLayoutExtended,SELF.__Link2:=__NN(RIGHT.Event_),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
    SHARED __FilterPart3(GROUPED DATASET(__InLayoutExtended) __ds) := JOIN(__ds,E_Property_Event_Filtered_1.__PostFilter,__EEQP(LEFT.UID,RIGHT.UID),TRANSFORM(__InLayoutExtended,SELF.__Link3:=__NN(RIGHT.UID),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
  ...
  SHARED E_Sele_Property_Event_Filtered := MODULE(E_Sele_Property_Event(__in,__cfg_Local))
    ...
    SHARED __FilterPart1(GROUPED DATASET(__InLayoutExtended) __ds) := JOIN(__ds,E_Address_Property_Event_Filtered_5.__PostFilter,__EEQP(LEFT.Event_,RIGHT.Event_),TRANSFORM(__InLayoutExtended,SELF.__Link1:=__NN(RIGHT.Event_),SELF:=LEFT),HASH,GROUPED,KEEP(1),LEFT OUTER);
  ...
  SHARED TYPEOF(E_Property().__Result) __E_Property := E_Property_Filtered.__Result;
  SHARED __EE178890799 := __ENH_Business_Sele;
  SHARED __EE178890820 := __EE178890799(__T(__AND(__OP2(__EE178890799.Ult_I_D_,=,__CN(__PUltID_in)),__AND(__OP2(__EE178890799.Org_I_D_,=,__CN(__POrgID_in)),__OP2(__EE178890799.Sele_I_D_,=,__CN(__PSeleID_in))))));
  SHARED __EE178890822 := __EE178890820;
  EXPORT Res0 := __UNWRAP(__EE178890822);
  SHARED __EE182724671 := __ENH_Business_Prox;
  SHARED __EE182724696 := __EE182724671(__T(__AND(__OP2(__EE182724671.Ult_I_D_,=,__CN(__PUltID_in)),__AND(__OP2(__EE182724671.Org_I_D_,=,__CN(__POrgID_in)),__AND(__OP2(__EE182724671.Sele_I_D_,=,__CN(__PSeleID_in)),__OP2(__EE182724671.Prox_I_D_,=,__CN(__PProxID_in)))))));
  SHARED __EE182724698 := __EE182724696;
  EXPORT Res1 := __UNWRAP(__EE182724698);
  SHARED __EE182724718 := __E_Phone;
  SHARED __EE182724730 := __EE182724718(__T(__OP2(__EE182724718.Phone10_,=,__CN(__PPhone_in))));
  SHARED __EE182724732 := __EE182724730;
  EXPORT Res2 := __UNWRAP(__EE182724732);
  SHARED __EE182724773 := __ENH_Address;
  SHARED __EE182724810 := __EE182724773(__T(__AND(__OP2(__EE182724773.Primary_Range_,=,__CN(__PPrimaryRange_in)),__AND(__OP2(__EE182724773.Predirectional_,=,__CN(__PPredirectional_in)),__AND(__OP2(__EE182724773.Primary_Name_,=,__CN(__PPrimaryName_in)),__AND(__OP2(__EE182724773.Suffix_,=,__CN(__PAddrSuffix_in)),__AND(__OP2(__EE182724773.Postdirectional_,=,__CN(__PPostdirectional_in)),__AND(__OP2(__EE182724773.Z_I_P5_,=,__CN(__PZIP5_in)),__OP2(__EE182724773.Secondary_Range_,=,__CN(__PSecondaryRange_in))))))))));
  SHARED __EE182724812 := __EE182724810;
  EXPORT Res3 := __UNWRAP(__EE182724812);
  SHARED __EE182724862 := __E_Property;
  SHARED __EE182724899 := __EE182724862(__T(__AND(__OP2(__EE182724862.Primary_Range_,=,__CN(__PPrimaryRange_in)),__AND(__OP2(__EE182724862.Predirectional_,=,__CN(__PPredirectional_in)),__AND(__OP2(__EE182724862.Primary_Name_,=,__CN(__PPrimaryName_in)),__AND(__OP2(__EE182724862.Suffix_,=,__CN(__PAddrSuffix_in)),__AND(__OP2(__EE182724862.Postdirectional_,=,__CN(__PPostdirectional_in)),__AND(__OP2(__EE182724862.Z_I_P5_,=,__CN(__PZIP5_in)),__OP2(__EE182724862.Secondary_Range_,=,__CN(__PSecondaryRange_in))))))))));
  EXPORT Res4 := __UNWRAP(__EE182724899);
END;

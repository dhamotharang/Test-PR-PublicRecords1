...
EXPORT B_Person_Inquiry_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE10839257 := __ENH_Person_Inquiry_2;
  EXPORT __ST223596_Layout := RECORD
    KEL.typ.ntyp(E_Person().Typ) Subject_;
    KEL.typ.ntyp(E_Inquiry().Typ) Transaction_;
    KEL.typ.nstr Transaction_I_D_;
    KEL.typ.nstr Sequence_Number_;
    KEL.typ.ndataset(E_Person_Inquiry(__in,__cfg).Data_Sources_Layout) Data_Sources_;
    KEL.typ.ndataset(B_Person_Inquiry_3(__in,__cfg).__ST1365602_Layout) Gather_Inquiries_;
  ...
  END;
  EXPORT __ENH_Person_Inquiry_1 := PROJECT(__EE10839257,__ST223596_Layout);
END;

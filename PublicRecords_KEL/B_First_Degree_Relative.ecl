...
EXPORT B_First_Degree_Relative(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1143595 := __E_First_Degree_Associations;
  SHARED __EE1143990 := __EE1143595(__T(__AND(__CN(TRUE),__AND(__OP2(__EE1143595.Title_,>=,__CN(1)),__OP2(__EE1143595.Title_,<=,__CN(45))))));
  EXPORT __ENH_First_Degree_Relative := PROJECT(__EE1143990,TRANSFORM(E_First_Degree_Relative(__in,__cfg).Layout,SELF.Relative_ := LEFT.First_Degree_Association_,SELF := LEFT));
END;

...
EXPORT B_First_Degree_Relative(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE583616 := __E_First_Degree_Relative;
  SHARED __EE583664 := __E_First_Degree_Associations;
  SHARED __EE583682 := __EE583664(__T(__AND(__CN(TRUE),__AND(__OP2(__EE583664.Title_,>=,__CN(1)),__OP2(__EE583664.Title_,<=,__CN(45))))));
  SHARED __EE583684 := __EE583616 + PROJECT(__EE583682,TRANSFORM(E_First_Degree_Relative(__in,__cfg).Layout,SELF.Relative_ := LEFT.First_Degree_Association_,SELF := LEFT));
  EXPORT __ENH_First_Degree_Relative := PROJECT(TABLE(PROJECT(__EE583684,E_First_Degree_Relative(__in,__cfg).Layout),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Subject_,Relative_,Title_,Relationship_Type_,Relationship_Confidence_,Relationship_Score_,Generation_,Relationship_Date_First_Seen_,Relationship_Date_Last_Seen_},Subject_,Relative_,Title_,Relationship_Type_,Relationship_Confidence_,Relationship_Score_,Generation_,Relationship_Date_First_Seen_,Relationship_Date_Last_Seen_,MERGE),E_First_Degree_Relative(__in,__cfg).Layout);
END;

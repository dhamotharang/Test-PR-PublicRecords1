...
EXPORT FnRoxie_Get6thRepAttributes(DATASET(PublicRecords_KEL.ECL_Functions.Layouts.LayoutInputPII) InputData,
	...
	NonFCRAPersonAttributesRaw := NOCOMBINE(JOIN(RecordsWithLexID, FDCDataset, 
		...
		TRANSFORM({INTEGER G_ProcUID, LayoutNonFCRAPersonAttributes},
			SELF.G_ProcUID := LEFT.G_ProcUID;
			NonFCRAPersonResults := PublicRecords_KEL.Q_Non_F_C_R_A_Sixth_Rep_Attributes_V1_Dynamic(LEFT.P_LexID , DATASET(LEFT), (INTEGER)(LEFT.P_InpClnArchDt[1..8]), Options.KEL_Permissions_Mask, DATASET(RIGHT)).res0;	

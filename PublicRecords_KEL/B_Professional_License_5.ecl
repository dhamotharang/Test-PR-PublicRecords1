...
EXPORT B_Professional_License_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4781053 := __E_Professional_License;
  EXPORT __ST245745_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST245745_Layout __ND4780980__Project(E_Professional_License(__in,__cfg).Layout __PP515088) := TRANSFORM
    __BS515237 := __T(__PP515088.License_Dates_);
    ...
    SELF.B_U_I_L_D___D_A_T_E_ := KEL.Routines.MinN(IF(EXISTS(__BS515237(__T(__OP2(__T(__PP515088.License_Dates_).Source_,=,__CN('PL'))))),__ECAST(KEL.typ.nkdate,__CC13306),__ECAST(KEL.typ.nkdate,__CC13314)),__CN(__cfg.CurrentDate));
    __EE4780975 := __PP515088.License_Dates_;
    SELF.Max_Expire_Date_ := KEL.Aggregates.MaxNN(__EE4780975,__T(__EE4780975).Date_Of_Expiration_);
    __EE4781000 := __PP515088.License_Dates_;
    SELF.Max_Issue_Date_ := KEL.Aggregates.MaxNN(__EE4781000,IF(__T(__OP2(__T(__EE4781000).Original_Date_Of_Issuance_,<,KEL.Routines.CastStringToDate(__CN('19000101')))),__ECAST(KEL.typ.nkdate,KEL.Routines.CastStringToDate(__CN(''))),__ECAST(KEL.typ.nkdate,__T(__EE4781000).Original_Date_Of_Issuance_)));
    SELF := __PP515088;
  END;
  EXPORT __ENH_Professional_License_5 := PROJECT(__EE4781053,__ND4780980__Project(LEFT));
END;

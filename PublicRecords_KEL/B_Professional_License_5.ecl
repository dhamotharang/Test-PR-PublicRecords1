...
EXPORT B_Professional_License_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4936589 := __E_Professional_License;
  EXPORT __ST241445_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST241445_Layout __ND4936518__Project(E_Professional_License(__in,__cfg).Layout __PP507471) := TRANSFORM
    __EE4936513 := __PP507471.License_Dates_;
    SELF.Max_Expire_Date_ := KEL.Aggregates.MaxNN(__EE4936513,__T(__EE4936513).Date_Of_Expiration_);
    __EE4936538 := __PP507471.License_Dates_;
    SELF.Max_Issue_Date_ := KEL.Aggregates.MaxNN(__EE4936538,IF(__T(__OP2(__T(__EE4936538).Original_Date_Of_Issuance_,<,KEL.Routines.CastStringToDate(__CN('19000101')))),__ECAST(KEL.typ.nkdate,KEL.Routines.CastStringToDate(__CN(''))),__ECAST(KEL.typ.nkdate,__T(__EE4936538).Original_Date_Of_Issuance_)));
    SELF := __PP507471;
  END;
  EXPORT __ENH_Professional_License_5 := PROJECT(__EE4936589,__ND4936518__Project(LEFT));
END;

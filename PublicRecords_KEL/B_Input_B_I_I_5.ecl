...
EXPORT B_Input_B_I_I_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE36034 := __E_Input_B_I_I;
  EXPORT __ST32349_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST32349_Layout __ND36032__Project(E_Input_B_I_I(__in,__cfg).Layout __PP35740) := TRANSFORM
    SELF.Addr_Not_Populated_ := FN_Compile.FN_Is_Not_Enough_To_Clean(__ECAST(KEL.typ.nstr,__PP35740.Bus_Input_Street_Echo_));
    SELF.City_State_Zip_Not_Populated_ := FN_Compile.FN_City_State_Zip_Not_Populated_Check(__ECAST(KEL.typ.nstr,__PP35740.Bus_Input_City_Echo_),__ECAST(KEL.typ.nstr,__PP35740.Bus_Input_State_Echo_),__ECAST(KEL.typ.nstr,__PP35740.Bus_Input_Zip_Echo_));
    SELF := __PP35740;
  END;
  EXPORT __ENH_Input_B_I_I_5 := PROJECT(__EE36034,__ND36032__Project(LEFT));
END;

...
EXPORT Q_Non_F_C_R_A_Person_Attributes_V1_Dynamic(KEL.typ.uid __PLexID_in, DATASET(RECORDOF(PublicRecords_KEL.ECL_Functions.Layouts.LayoutInputPII)) __PInputPIIDataset, KEL.typ.kdate __PP_InpClnArchDt, DATA57 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED __EE11407581 := __ENH_Person;
  SHARED __EE11412934 := __EE11407581(__T(__OP2(__EE11407581.UID,=,__CN(__PLexID_in))));
  SHARED __ST101003_Layout := RECORD
  ...
  END;
  SHARED __ST101003_Layout __ND11412939__Project(B_Person(__in,__cfg_Local).__ST171389_Layout __PP11412935) := TRANSFORM
    SELF.Lex_I_D_ := __PP11412935.UID;
    SELF.P_L___Email_Cnt_Ev_ := __PP11412935.P_L___Email_Cnt_Ev_Non_F_C_R_A_;
    SELF := __PP11412935;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(__EE11412934,__ND11412939__Project(LEFT)));

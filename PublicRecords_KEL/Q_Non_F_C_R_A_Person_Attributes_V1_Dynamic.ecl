...
EXPORT Q_Non_F_C_R_A_Person_Attributes_V1_Dynamic(KEL.typ.uid __PLexID_in, DATASET(RECORDOF(PublicRecords_KEL.ECL_Functions.Layouts.LayoutInputPII)) __PInputPIIDataset, KEL.typ.kdate __PP_InpClnArchDt, DATA57 __PDPM, CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault) := MODULE
  ...
  SHARED __EE11545262 := __ENH_Person;
  SHARED __EE11550387 := __EE11545262(__T(__OP2(__EE11545262.UID,=,__CN(__PLexID_in))));
  SHARED __ST96028_Layout := RECORD
  ...
  END;
  SHARED __ST96028_Layout __ND11550392__Project(B_Person(__in,__cfg_Local).__ST164109_Layout __PP11550388) := TRANSFORM
    SELF.Lex_I_D_ := __PP11550388.UID;
    SELF.P_L___Email_Cnt_Ev_ := __PP11550388.P_L___Email_Cnt_Ev_Non_F_C_R_A_;
    SELF := __PP11550388;
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(__EE11550387,__ND11550392__Project(LEFT)));

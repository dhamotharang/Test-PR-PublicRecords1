...
EXPORT E_Education(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  Layout Education__Rollup(InLayout __r, DATASET(InLayout) __recs) := TRANSFORM
    ...
    SELF.College_Characteristics_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Date_Vendor_First_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_First_Reported_,MIN,FALSE),KEL.typ.epoch Date_Vendor_Last_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_Last_Reported_,MAX,FALSE),College_Code_,College_Type_,File_Type_,School_Size_Code_,Competitive_Code_,Tuition_Code_,Tier_,Tier2_},College_Code_,College_Type_,File_Type_,School_Size_Code_,Competitive_Code_,Tuition_Code_,Tier_,Tier2_),College_Characteristics_Layout)(__NN(College_Code_) OR __NN(College_Type_) OR __NN(File_Type_) OR __NN(School_Size_Code_) OR __NN(Competitive_Code_) OR __NN(Tuition_Code_) OR __NN(Tier_) OR __NN(Tier2_)));

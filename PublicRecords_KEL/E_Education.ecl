...
EXPORT E_Education(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __d0_Out := RECORD
    ...
    KEL.typ.uid UID := 0;
  END;
  SHARED __d0_UID_Mapped := JOIN(__d0_Norm,Lookup,'' + '|' + TRIM((STRING)LEFT.Key) + '|' + '' = RIGHT.KeyVal,TRANSFORM(__d0_Out,SELF.UID:=RIGHT.UID,SELF:=LEFT),HASH);
  ...
  SHARED __Mapping1 := 'UID(DEFAULT:UID),sequence_number(OVERRIDE:Sequence_:\'\'),key_code(OVERRIDE:Key_:\'\'),rawaid(OVERRIDE:Raw_A_I_D_:\'\'),school_name(OVERRIDE:College_Name_:\'\'),ln_college_name(OVERRIDE:L_N_College_Name_:\'\'),public_private_code(OVERRIDE:College_Code_:\'\'|OVERRIDE:College_Type_:\'\'),file_type(OVERRIDE:File_Type_:\'\'),school_size_code(OVERRIDE:School_Size_Code_:\'\'),competitive_code(OVERRIDE:Competitive_Code_:\'\'),tuition_code(OVERRIDE:Tuition_Code_:\'\'),tier(OVERRIDE:Tier_:\'\'),tier2(OVERRIDE:Tier2_:\'\'),source(OVERRIDE:Source_:\'\'),date_first_seen(OVERRIDE:Date_First_Seen_:EPOCH),date_last_seen(OVERRIDE:Date_Last_Seen_:EPOCH),date_vendor_first_reported(OVERRIDE:Date_Vendor_First_Reported_:EPOCH),date_vendor_last_reported(OVERRIDE:Date_Vendor_Last_Reported_:EPOCH),DPMBitmap(OVERRIDE:__Permits:PERMITS)';
  ...
  SHARED __d1_Out := RECORD
    ...
    KEL.typ.uid UID := 0;
  END;
  SHARED __d1_UID_Mapped := JOIN(__d1_Norm,Lookup,TRIM((STRING)LEFT.Sequence_Number) + '|' + TRIM((STRING)LEFT.key_code) + '|' + TRIM((STRING)LEFT.rawaid) = RIGHT.KeyVal,TRANSFORM(__d1_Out,SELF.UID:=RIGHT.UID,SELF:=LEFT),HASH);
  ...
  Layout Education__Rollup(InLayout __r, DATASET(InLayout) __recs) := TRANSFORM
    ...
    SELF.College_Characteristics_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Date_Vendor_First_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_First_Reported_,MIN,FALSE),KEL.typ.epoch Date_Vendor_Last_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_Last_Reported_,MAX,FALSE),College_Code_,College_Type_,File_Type_,School_Size_Code_,Competitive_Code_,Tuition_Code_,Tier_,Tier2_},College_Code_,College_Type_,File_Type_,School_Size_Code_,Competitive_Code_,Tuition_Code_,Tier_,Tier2_),College_Characteristics_Layout)(__NN(College_Code_) OR __NN(College_Type_) OR __NN(File_Type_) OR __NN(School_Size_Code_) OR __NN(Competitive_Code_) OR __NN(Tuition_Code_) OR __NN(Tier_) OR __NN(Tier2_)));
  ...
  Layout Education__Single_Rollup(InLayout __r) := TRANSFORM
    SELF.College_Characteristics_ := __CN(PROJECT(DATASET(__r),TRANSFORM(College_Characteristics_Layout,SELF.__RecordCount:=1;,SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF.Date_Vendor_First_Reported_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Vendor_First_Reported_,FALSE),SELF.Date_Vendor_Last_Reported_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Vendor_Last_Reported_,FALSE),SELF:=LEFT))(__NN(College_Code_) OR __NN(College_Type_) OR __NN(File_Type_) OR __NN(School_Size_Code_) OR __NN(Competitive_Code_) OR __NN(Tuition_Code_) OR __NN(Tier_) OR __NN(Tier2_)));

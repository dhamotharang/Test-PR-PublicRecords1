...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1511854 := __ENH_Watercraft_Owner_3;
  EXPORT __ST158869_Layout := RECORD
  ...
  END;
  SHARED __ST158869_Layout __ND1511899__Project(B_Watercraft_Owner_3(__in,__cfg).__ST162185_Layout __PP1511855) := TRANSFORM
    __CC13958 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1511855.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1511855.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13958)));
    SELF := __PP1511855;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE1511854,__ND1511899__Project(LEFT));
END;

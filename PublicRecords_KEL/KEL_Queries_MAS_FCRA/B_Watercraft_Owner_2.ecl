...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1512824 := __ENH_Watercraft_Owner_3;
  ...
  END;
  SHARED __ST157838_Layout __ND1512869__Project(B_Watercraft_Owner_3(__in,__cfg).__ST161168_Layout __PP1512825) := TRANSFORM
    __CC14087 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1512825.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1512825.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14087)));
    SELF := __PP1512825;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE1512824,__ND1512869__Project(LEFT));
END;

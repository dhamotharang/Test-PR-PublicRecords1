...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6326565 := __ENH_Watercraft_Owner_3;
  EXPORT __ST188594_Layout := RECORD
  ...
  END;
  SHARED __ST188594_Layout __ND6326610__Project(B_Watercraft_Owner_3(__in,__cfg).__ST195277_Layout __PP6326566) := TRANSFORM
    __CC14087 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP6326566.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP6326566.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14087)));
    SELF := __PP6326566;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE6326565,__ND6326610__Project(LEFT));
END;

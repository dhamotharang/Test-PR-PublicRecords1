...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1536587 := __ENH_Watercraft_Owner_3;
  EXPORT __ST163993_Layout := RECORD
  ...
  END;
  SHARED __ST163993_Layout __ND1536628__Project(B_Watercraft_Owner_3(__in,__cfg).__ST167058_Layout __PP1536588) := TRANSFORM
    __CC13964 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1536588.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1536588.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13964)));
    SELF := __PP1536588;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE1536587,__ND1536628__Project(LEFT));
END;

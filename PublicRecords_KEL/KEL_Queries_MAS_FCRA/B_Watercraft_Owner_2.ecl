...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1543776 := __ENH_Watercraft_Owner_3;
  EXPORT __ST164961_Layout := RECORD
  ...
  END;
  SHARED __ST164961_Layout __ND1543821__Project(B_Watercraft_Owner_3(__in,__cfg).__ST168302_Layout __PP1543777) := TRANSFORM
    __CC13964 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1543777.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1543777.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13964)));
    SELF := __PP1543777;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE1543776,__ND1543821__Project(LEFT));
END;

...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1547476 := __ENH_Watercraft_Owner_3;
  ...
  END;
  SHARED __ST165018_Layout __ND1547517__Project(B_Watercraft_Owner_3(__in,__cfg).__ST168093_Layout __PP1547477) := TRANSFORM
    __CC13886 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1547477.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1547477.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13886)));
    SELF := __PP1547477;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE1547476,__ND1547517__Project(LEFT));
END;

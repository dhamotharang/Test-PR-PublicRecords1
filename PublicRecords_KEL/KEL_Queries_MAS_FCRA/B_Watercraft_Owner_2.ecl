...
EXPORT B_Watercraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1553098 := __ENH_Watercraft_Owner_3;
  EXPORT __ST169295_Layout := RECORD
  ...
  END;
  SHARED __ST169295_Layout __ND1553139__Project(B_Watercraft_Owner_3(__in,__cfg).__ST172370_Layout __PP1553099) := TRANSFORM
    __CC14111 := '-99997';
    SELF.Watercraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1553099.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1553099.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14111)));
    SELF := __PP1553099;
  END;
  EXPORT __ENH_Watercraft_Owner_2 := PROJECT(__EE1553098,__ND1553139__Project(LEFT));
END;

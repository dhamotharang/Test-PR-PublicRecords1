...
EXPORT B_Person_Vehicle_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5443845 := __E_Person_Vehicle;
  EXPORT __ST194818_Layout := RECORD
  ...
  END;
  SHARED __ST194818_Layout __ND5443797__Project(E_Person_Vehicle(__in,__cfg).Layout __PP766467) := TRANSFORM
    __EE5443772 := __PP766467.Counts_Model_;
    ...
    __EE5443792 := __PP766467.Counts_Model_;
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.Aggregates.MinNN(__EE5443772,KEL.era.ToDate(__T(__EE5443772).Date_First_Seen_)),>,__CC13435)),__ECAST(KEL.typ.nkdate,__CC13435),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MinNN(__EE5443792,KEL.era.ToDate(__T(__EE5443792).Date_First_Seen_))));
    SELF := __PP766467;
  END;
  EXPORT __ENH_Person_Vehicle_3 := PROJECT(__EE5443845,__ND5443797__Project(LEFT));
END;

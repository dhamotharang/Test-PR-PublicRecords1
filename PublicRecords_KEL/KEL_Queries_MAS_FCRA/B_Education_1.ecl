...
EXPORT B_Education_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1537342 := __ENH_Education_2;
  EXPORT __ST156090_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST156090_Layout __ND1537474__Project(B_Education_8(__in,__cfg).__ST173638_Layout __PP1537343) := TRANSFORM
    __BS1537385 := __T(__PP1537343.College_Characteristics_);
    SELF.Coll_Rec_Flag_ := EXISTS(__BS1537385(__T(__OP2(__T(__PP1537343.College_Characteristics_).File_Type_,IN,__CN(['C','H','O'])))));
    SELF := __PP1537343;
  END;
  EXPORT __ENH_Education_1 := PROJECT(__EE1537342,__ND1537474__Project(LEFT));
END;

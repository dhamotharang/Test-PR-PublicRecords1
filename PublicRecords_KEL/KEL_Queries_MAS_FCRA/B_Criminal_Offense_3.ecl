...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1166654 := __ENH_Criminal_Offense_4;
  ...
  END;
  EXPORT __ST163944_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST163991_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST163944_Layout __ND1167086__Project(B_Criminal_Offense_4(__in,__cfg).__ST166696_Layout __PP1166655) := TRANSFORM
    __EE1166815 := __PP1166655.Offense_Charges_;
    __ST163991_Layout __ND1167068__Project(B_Criminal_Offense_4(__in,__cfg).__ST166743_Layout __PP1166816) := TRANSFORM
      SELF.Is_Felony_F_C_R_A_ := __AND(__AND(__AND(__AND(__NOT(__PP1166816.Dismissed_Charges_),IF(__T(__NT(__PP1166816.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP1166816.Offense_Score_,=,__CN('F'))))),__NT(__PP1166816.Offender_Level_)),__OP2(__PP1166816.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP1166816.Traffic_Flag_,=,__CN('N')));
      SELF := __PP1166816;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE1166815,__ND1167068__Project(LEFT));
    SELF := __PP1166655;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE1166654,__ND1167086__Project(LEFT));
END;

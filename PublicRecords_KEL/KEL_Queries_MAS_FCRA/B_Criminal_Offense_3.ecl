...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1168810 := __ENH_Criminal_Offense_4;
  EXPORT __ST165655_Layout := RECORD
  ...
  END;
  EXPORT __ST165608_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST165655_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST165608_Layout __ND1169242__Project(B_Criminal_Offense_4(__in,__cfg).__ST168360_Layout __PP1168811) := TRANSFORM
    __EE1168971 := __PP1168811.Offense_Charges_;
    __ST165655_Layout __ND1169224__Project(B_Criminal_Offense_4(__in,__cfg).__ST168407_Layout __PP1168972) := TRANSFORM
      SELF.Is_Felony_F_C_R_A_ := __AND(__AND(__AND(__AND(__NOT(__PP1168972.Dismissed_Charges_),IF(__T(__NT(__PP1168972.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP1168972.Offense_Score_,=,__CN('F'))))),__NT(__PP1168972.Offender_Level_)),__OP2(__PP1168972.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP1168972.Traffic_Flag_,=,__CN('N')));
      SELF := __PP1168972;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE1168971,__ND1169224__Project(LEFT));
    SELF := __PP1168811;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE1168810,__ND1169242__Project(LEFT));
END;

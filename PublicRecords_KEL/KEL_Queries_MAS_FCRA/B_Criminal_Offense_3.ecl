...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1148511 := __ENH_Criminal_Offense_4;
  EXPORT __ST158498_Layout := RECORD
  ...
  END;
  EXPORT __ST158451_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST158498_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST158451_Layout __ND1148943__Project(B_Criminal_Offense_4(__in,__cfg).__ST161484_Layout __PP1148512) := TRANSFORM
    __EE1148672 := __PP1148512.Offense_Charges_;
    __ST158498_Layout __ND1148925__Project(B_Criminal_Offense_4(__in,__cfg).__ST161531_Layout __PP1148673) := TRANSFORM
      SELF.Is_Felony_F_C_R_A_ := __AND(__AND(__AND(__AND(__NOT(__PP1148673.Dismissed_Charges_),IF(__T(__NT(__PP1148673.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP1148673.Offense_Score_,=,__CN('F'))))),__NT(__PP1148673.Offender_Level_)),__OP2(__PP1148673.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP1148673.Traffic_Flag_,=,__CN('N')));
      SELF := __PP1148673;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE1148672,__ND1148925__Project(LEFT));
    SELF := __PP1148512;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE1148511,__ND1148943__Project(LEFT));
END;

...
EXPORT B_Property_Event_8(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE217534 := __E_Property_Event;
  EXPORT __ST173770_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST173770_Layout __ND217602__Project(E_Property_Event(__in,__cfg).Layout __PP217197) := TRANSFORM
    SELF.Dt_First_Seen_ := KEL.era.ToDate(__PP217197.Date_First_Seen_);
    SELF := __PP217197;
  END;
  EXPORT __ENH_Property_Event_8 := PROJECT(__EE217534,__ND217602__Project(LEFT));
END;

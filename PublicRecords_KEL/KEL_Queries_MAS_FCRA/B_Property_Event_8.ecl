...
IMPORT KEL16 AS KEL;
IMPORT CFG_Compile,E_Property,E_Property_Event,E_Zip_Code FROM PublicRecords_KEL.KEL_Queries_MAS_FCRA;
...
EXPORT B_Property_Event_8(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE217189 := __E_Property_Event;
  EXPORT __ST173559_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST173559_Layout __ND217257__Project(E_Property_Event(__in,__cfg).Layout __PP216852) := TRANSFORM
    SELF.Dt_First_Seen_ := KEL.era.ToDate(__PP216852.Date_First_Seen_);
    SELF := __PP216852;
  END;
  EXPORT __ENH_Property_Event_8 := PROJECT(__EE217189,__ND217257__Project(LEFT));
END;

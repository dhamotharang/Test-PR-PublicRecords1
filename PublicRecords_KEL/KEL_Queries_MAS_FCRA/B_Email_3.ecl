...
EXPORT B_Email_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE296284 := __E_Email;
  EXPORT __ST165669_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST165669_Layout __ND1162442__Project(E_Email(__in,__cfg).Layout __PP296089) := TRANSFORM
    ...
    __CC39788 := 2556;
    SELF.Emails7y_ := __OP2(FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__CC13216),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP296089.Date_Last_Seen_))),<,__CN(__CC39788));
    SELF := __PP296089;
  END;
  EXPORT __ENH_Email_3 := PROJECT(__EE296284,__ND1162442__Project(LEFT));
END;

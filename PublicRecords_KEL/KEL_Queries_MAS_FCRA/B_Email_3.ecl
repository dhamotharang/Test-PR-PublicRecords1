...
EXPORT B_Email_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE506996 := __E_Email;
  EXPORT __ST189467_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST189467_Layout __ND4884910__Project(E_Email(__in,__cfg).Layout __PP506801) := TRANSFORM
    ...
    __CC39298 := 2556;
    SELF.Emails7y_ := __OP2(FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__CC13339),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP506801.Date_Last_Seen_))),<,__CN(__CC39298));
    SELF := __PP506801;
  END;
  EXPORT __ENH_Email_3 := PROJECT(__EE506996,__ND4884910__Project(LEFT));
END;

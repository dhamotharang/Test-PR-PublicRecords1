...
EXPORT B_Email_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE307168 := __E_Email;
  EXPORT __ST164700_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST164700_Layout __ND1161657__Project(E_Email(__in,__cfg).Layout __PP306973) := TRANSFORM
    __CC13216 := KEL.Routines.MinN(FN_Compile(__cfg).FN_G_E_T_B_U_I_L_D_D_A_T_E(__ECAST(KEL.typ.nstr,__CN('email_build_version'))),__CN(__cfg.CurrentDate));
    __CC39836 := 2556;
    SELF.Emails7y_ := __OP2(FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__CC13216),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP306973.Date_Last_Seen_))),<,__CN(__CC39836));
    SELF := __PP306973;
  END;
  EXPORT __ENH_Email_3 := PROJECT(__EE307168,__ND1161657__Project(LEFT));
END;

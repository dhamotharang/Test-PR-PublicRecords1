...
EXPORT B_Email_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  END;
  SHARED __ST180821_Layout __ND4109397__Project(E_Email(__in,__cfg).Layout __PP447553) := TRANSFORM
    ...
    __CC38646 := 2556;
    SELF.Emails7y_ := __OP2(FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__CC13266),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP447553.Date_Last_Seen_))),<,__CN(__CC38646));
    SELF := __PP447553;
  END;
  EXPORT __ENH_Email_3 := PROJECT(__EE447748,__ND4109397__Project(LEFT));
END;

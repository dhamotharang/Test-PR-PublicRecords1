...
EXPORT B_Education_8(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE212364 := __E_Education;
  EXPORT __ST172865_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST172865_Layout __ND1063082__Project(E_Education(__in,__cfg).Layout __PP212238) := TRANSFORM
    __BS212314 := __T(__PP212238.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS212314(__T(__OP2(__T(__PP212238.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP212238;
  END;
  EXPORT __ENH_Education_8 := PROJECT(__EE212364,__ND1063082__Project(LEFT));
END;

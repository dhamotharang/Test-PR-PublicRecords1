...
EXPORT B_Education_8(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE212709 := __E_Education;
  EXPORT __ST173076_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST173076_Layout __ND1068609__Project(E_Education(__in,__cfg).Layout __PP212583) := TRANSFORM
    __BS212659 := __T(__PP212583.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS212659(__T(__OP2(__T(__PP212583.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP212583;
  END;
  EXPORT __ENH_Education_8 := PROJECT(__EE212709,__ND1068609__Project(LEFT));
END;

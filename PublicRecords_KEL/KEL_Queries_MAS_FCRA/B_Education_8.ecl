...
EXPORT B_Education_8(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE213137 := __E_Education;
  EXPORT __ST173638_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST173638_Layout __ND1063861__Project(E_Education(__in,__cfg).Layout __PP213011) := TRANSFORM
    __BS213087 := __T(__PP213011.College_Characteristics_);
    SELF.Edu_Rec_Flag_ := EXISTS(__BS213087(__T(__OP2(__T(__PP213011.College_Characteristics_).File_Type_,IN,__CN(['M','C','H','O'])))));
    SELF := __PP213011;
  END;
  EXPORT __ENH_Education_8 := PROJECT(__EE213137,__ND1063861__Project(LEFT));
END;

...
EXPORT B_Sele_Person_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4630191 := __E_Sele_Person;
  ...
  END;
  SHARED __ST185736_Layout __ND4630155__Project(E_Sele_Person(__in,__cfg).Layout __PP675984) := TRANSFORM
    __EE4630130 := __PP675984.Contact_Info_;
    ...
    __EE4630150 := __PP675984.Contact_Info_;
    SELF.Assoc_Date_ := IF(__T(__OP2(KEL.Aggregates.MaxNN(__EE4630130,KEL.era.ToDate(__T(__EE4630130).Date_Last_Seen_)),>,__CC13458)),__ECAST(KEL.typ.nkdate,__CC13458),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MaxNN(__EE4630150,KEL.era.ToDate(__T(__EE4630150).Date_Last_Seen_))));
    SELF := __PP675984;
  END;
  EXPORT __ENH_Sele_Person_3 := PROJECT(__EE4630191,__ND4630155__Project(LEFT));
END;

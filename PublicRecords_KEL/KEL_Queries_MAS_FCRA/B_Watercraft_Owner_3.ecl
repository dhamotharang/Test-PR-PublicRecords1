...
EXPORT B_Watercraft_Owner_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE370036 := __E_Watercraft_Owner;
  EXPORT __ST161168_Layout := RECORD
  ...
  END;
  SHARED __ST161168_Layout __ND370045__Project(E_Watercraft_Owner(__in,__cfg).Layout __PP369989) := TRANSFORM
    ...
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.era.ToDate(__PP369989.Date_First_Seen_),>,__CC13443)),__ECAST(KEL.typ.nkdate,__CC13443),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP369989.Date_First_Seen_)));
    SELF := __PP369989;
  END;
  EXPORT __ENH_Watercraft_Owner_3 := PROJECT(__EE370036,__ND370045__Project(LEFT));
END;

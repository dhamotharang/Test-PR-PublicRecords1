...
EXPORT B_Watercraft_Owner_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE388796 := __E_Watercraft_Owner;
  EXPORT __ST168302_Layout := RECORD
  ...
  END;
  SHARED __ST168302_Layout __ND388805__Project(E_Watercraft_Owner(__in,__cfg).Layout __PP388749) := TRANSFORM
    ...
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.era.ToDate(__PP388749.Date_First_Seen_),>,__CC13320)),__ECAST(KEL.typ.nkdate,__CC13320),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP388749.Date_First_Seen_)));
    SELF := __PP388749;
  END;
  EXPORT __ENH_Watercraft_Owner_3 := PROJECT(__EE388796,__ND388805__Project(LEFT));
END;

...
EXPORT B_Person_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1089425 := __ENH_Person_8;
  SHARED __EE1089427 := __ENH_Input_P_I_I_8;
  SHARED __ST222377_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(B_Person_8(__in,__cfg).__ST213710_Layout) All_Lien_Data_;
  ...
  END;
  __JC1089436(B_Person_8(__in,__cfg).__ST175170_Layout __EE1089425, B_Input_P_I_I_8(__in,__cfg).__ST174868_Layout __EE1089427) := __EEQP(__EE1089425.P_I_I_,__EE1089427.UID);
  __ST222377_Layout __JT1089436(B_Person_8(__in,__cfg).__ST175170_Layout __l, B_Input_P_I_I_8(__in,__cfg).__ST174868_Layout __r) := TRANSFORM
    SELF.U_I_D__1_ := __r.UID;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE1089660 := JOIN(__EE1089425,__EE1089427,__JC1089436(LEFT,RIGHT),__JT1089436(LEFT,RIGHT),LEFT OUTER,SMART);
  EXPORT __ST179718_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST221357_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST179713_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nstr Lex_I_D_Segment_;
    KEL.typ.nstr Lex_I_D_Segment2_;
    KEL.typ.ndataset(__ST179718_Layout) Full_Name_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Reported_Dates_Of_Birth_Layout) Reported_Dates_Of_Birth_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Reported_Dates_Of_Death_Layout) Reported_Dates_Of_Death_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Data_Sources_Layout) Data_Sources_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Conceal_Carry_Layout) Conceal_Carry_;
    ...
    KEL.typ.ndataset(E_Person(__in,__cfg).Consumer_Statement_Flags_Layout) Consumer_Statement_Flags_;
    KEL.typ.ndataset(E_Person(__in,__cfg).Thrive_Layout) Thrive_;
    KEL.typ.ndataset(__ST221357_Layout) All_Lien_Data_;
    KEL.typ.ntyp(E_Input_P_I_I().Typ) P_I_I_;
    KEL.typ.nstr P___Inp_Cln_Name_First_Raw_;
    KEL.typ.nstr P___Inp_Cln_Name_Last_Raw_;
    KEL.typ.nstr P___Inp_Cln_S_S_N_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST179713_Layout __ND1089336__Project(__ST222377_Layout __PP1087910) := TRANSFORM
    __EE1089663 := __PP1087910.Full_Name_;
    __ST179718_Layout __ND1088213__Project(E_Person(__in,__cfg).Full_Name_Layout __PP1088209) := TRANSFORM
      SELF.Translated_Source_Code_ := FN_Compile(__cfg).FN_Consumer_Source_Group(__ECAST(KEL.typ.nstr,__PP1088209.Source_));
      SELF.Verified_First_Name_ := FN_Compile(__cfg).FN_Is_First_Name_Match(__ECAST(KEL.typ.nstr,__PP1088209.First_Name_),__ECAST(KEL.typ.nstr,__PP1087910.P___Inp_Cln_Name_First_Raw_));
      SELF.Verified_Last_Name_ := FN_Compile(__cfg).FN_Is_Last_Name_Match(__ECAST(KEL.typ.nstr,__PP1088209.Last_Name_),__ECAST(KEL.typ.nstr,__PP1087910.P___Inp_Cln_Name_Last_Raw_));
      SELF := __PP1088209;
    END;
    SELF.Full_Name_ := __PROJECT(__EE1089663,__ND1088213__Project(LEFT));
    __EE1089334 := __PP1087910.All_Lien_Data_;
    __ST221357_Layout __ND1089240__Project(B_Person_8(__in,__cfg).__ST213710_Layout __PP1088286) := TRANSFORM
      ...
      SELF.Is_Landlord_Tenant_Dispute_ := __AND(__OR(__CN(__PP1088286.Landlord_Tenant_Dispute_Flag_ = TRUE),__OP2(__PP1088286.Filing_Type_Description_,IN,__CN(__CC33874))),__NOT(__OP2(__PP1088286.Filing_Type_Description_,IN,__CN(__CC33887))));
      SELF.Is_Over_All_Lien_Judgment_ := __OR(__PP1088286.Is_Over_All_Judgment_,__PP1088286.Is_Over_All_Lien_);
      __CC33868 := 2556;
      SELF.Seen___In___Seven___Years_ := __OP2(__PP1088286.Age_In_Days_,<=,__CN(__CC33868));
      SELF := __PP1088286;
    END;
    SELF.All_Lien_Data_ := __PROJECT(__EE1089334,__ND1089240__Project(LEFT));
    SELF.P___Inp_Cln_S_S_N_ := __PP1087910.Input_S_S_N_Clean_Value_;
    __BS1089346 := __T(__PP1087910.Data_Sources_);
    SELF.P___Lex_I_D_Seen_Flag_ := IF(EXISTS(__BS1089346(__T(__T(__PP1087910.Data_Sources_).Header_Hit_Flag_))),'1','0');
    SELF := __PP1087910;
  END;
  EXPORT __ENH_Person_7 := PROJECT(__EE1089660,__ND1089336__Project(LEFT));
END;

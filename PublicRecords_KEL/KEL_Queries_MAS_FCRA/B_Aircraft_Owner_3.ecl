...
EXPORT B_Aircraft_Owner_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE283607 := __E_Aircraft_Owner;
  EXPORT __ST158065_Layout := RECORD
  ...
  END;
  SHARED __ST158065_Layout __ND283619__Project(E_Aircraft_Owner(__in,__cfg).Layout __PP283545) := TRANSFORM
    ...
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.era.ToDate(__PP283545.Date_First_Seen_),>,__CC13275)),__ECAST(KEL.typ.nkdate,__CC13275),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP283545.Date_First_Seen_)));
    SELF := __PP283545;
  END;
  EXPORT __ENH_Aircraft_Owner_3 := PROJECT(__EE283607,__ND283619__Project(LEFT));
END;

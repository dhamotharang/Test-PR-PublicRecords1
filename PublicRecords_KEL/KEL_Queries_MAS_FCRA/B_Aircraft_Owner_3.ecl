...
EXPORT B_Aircraft_Owner_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE293725 := __E_Aircraft_Owner;
  EXPORT __ST165166_Layout := RECORD
  ...
  END;
  SHARED __ST165166_Layout __ND293737__Project(E_Aircraft_Owner(__in,__cfg).Layout __PP293663) := TRANSFORM
    ...
    SELF.Date_First_Seen_Capped_ := IF(__T(__OP2(KEL.era.ToDate(__PP293663.Date_First_Seen_),>,__CC13152)),__ECAST(KEL.typ.nkdate,__CC13152),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP293663.Date_First_Seen_)));
    SELF := __PP293663;
  END;
  EXPORT __ENH_Aircraft_Owner_3 := PROJECT(__EE293725,__ND293737__Project(LEFT));
END;

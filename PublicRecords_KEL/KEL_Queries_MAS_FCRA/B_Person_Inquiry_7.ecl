...
EXPORT B_Person_Inquiry_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4677164 := __ENH_Person_Inquiry_8;
  EXPORT __ST327034_Layout := RECORD
  ...
  EXPORT __ST207425_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST207425_Layout __ND4677169__Project(B_Person_Inquiry_8(__in,__cfg).__ST209081_Layout __PP4677165) := TRANSFORM
    __EE4677189 := __PP4677165.Gather_Inquiries_;
    __ST327034_Layout __ND4677194__Project(B_Person_Inquiry_8(__in,__cfg).__ST101763_Layout __PP4677190) := TRANSFORM
      SELF.Exclude_Function_Description_ := KEL.Routines.ToUpperCase(TRIM(__PP4677190.Function_Description_)) <> 'BANKO BATCH';
      __CC39802 := ['CARDS - SUBPRIME','MSB','REFUND ANTICIPATION LOAN','RENT TO OWN','PAYDAY LOANS','PAY DAY LOANS','TITLE LOANS','PAY DAY','PAYDAY'];
      SELF.Is_High_Risk_ := KEL.Routines.ToUpperCase(TRIM(__PP4677190.Industry_)) IN __CC39802;
      SELF := __PP4677190;
    END;
    SELF.Gather_Inquiries_ := __PROJECT(__EE4677189,__ND4677194__Project(LEFT));
    SELF := __PP4677165;
  END;
  EXPORT __ENH_Person_Inquiry_7 := PROJECT(__EE4677164,__ND4677169__Project(LEFT));
END;

...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1253880 := __ENH_Aircraft_Owner_3;
  EXPORT __ST159752_Layout := RECORD
  ...
  END;
  SHARED __ST159752_Layout __ND1253936__Project(B_Aircraft_Owner_3(__in,__cfg).__ST163558_Layout __PP1253881) := TRANSFORM
    __CC13871 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1253881.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1253881.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13871)));
    SELF := __PP1253881;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE1253880,__ND1253936__Project(LEFT));
END;

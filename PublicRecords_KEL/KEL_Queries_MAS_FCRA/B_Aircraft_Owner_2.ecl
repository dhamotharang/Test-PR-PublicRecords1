...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE4631496 := __ENH_Aircraft_Owner_3;
  EXPORT __ST172346_Layout := RECORD
  ...
  END;
  SHARED __ST172346_Layout __ND4631556__Project(B_Aircraft_Owner_3(__in,__cfg).__ST180242_Layout __PP4631497) := TRANSFORM
    __CC13835 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP4631497.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP4631497.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13835)));
    SELF := __PP4631497;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE4631496,__ND4631556__Project(LEFT));
END;

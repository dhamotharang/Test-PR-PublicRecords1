...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1259549 := __ENH_Aircraft_Owner_3;
  EXPORT __ST155222_Layout := RECORD
  ...
  END;
  SHARED __ST155222_Layout __ND1259609__Project(B_Aircraft_Owner_3(__in,__cfg).__ST159074_Layout __PP1259550) := TRANSFORM
    __CC13943 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1259550.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1259550.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC13943)));
    SELF := __PP1259550;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE1259549,__ND1259609__Project(LEFT));
END;

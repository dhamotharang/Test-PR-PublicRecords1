...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1261207 := __ENH_Aircraft_Owner_3;
  EXPORT __ST154199_Layout := RECORD
  ...
  END;
  SHARED __ST154199_Layout __ND1261267__Project(B_Aircraft_Owner_3(__in,__cfg).__ST158065_Layout __PP1261208) := TRANSFORM
    __CC14072 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1261208.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1261208.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14072)));
    SELF := __PP1261208;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE1261207,__ND1261267__Project(LEFT));
END;

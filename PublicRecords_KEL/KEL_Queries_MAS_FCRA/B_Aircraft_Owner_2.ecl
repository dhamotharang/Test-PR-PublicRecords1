...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5445572 := __ENH_Aircraft_Owner_3;
  EXPORT __ST180137_Layout := RECORD
  ...
  END;
  SHARED __ST180137_Layout __ND5445632__Project(B_Aircraft_Owner_3(__in,__cfg).__ST188888_Layout __PP5445573) := TRANSFORM
    __CC14072 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP5445573.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP5445573.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14072)));
    SELF := __PP5445573;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE5445572,__ND5445632__Project(LEFT));
END;

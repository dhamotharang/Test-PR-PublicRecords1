...
EXPORT B_Aircraft_Owner_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1256704 := __ENH_Aircraft_Owner_3;
  EXPORT __ST162090_Layout := RECORD
  ...
  END;
  SHARED __ST162090_Layout __ND1256760__Project(B_Aircraft_Owner_3(__in,__cfg).__ST165896_Layout __PP1256705) := TRANSFORM
    __CC14087 := '-99997';
    SELF.Aircraft_Min_Date_ := IF(__T(__FN1(KEL.Routines.IsValidDate,__PP1256705.Date_First_Seen_Capped_)),__ECAST(KEL.typ.nstr,__FN2(KEL.Routines.DateToString,__PP1256705.Date_First_Seen_Capped_,__CN('%Y%m%d'))),__ECAST(KEL.typ.nstr,__CN(__CC14087)));
    SELF := __PP1256705;
  END;
  EXPORT __ENH_Aircraft_Owner_2 := PROJECT(__EE1256704,__ND1256760__Project(LEFT));
END;

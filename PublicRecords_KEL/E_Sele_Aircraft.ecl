...
EXPORT E_Sele_Aircraft(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __d0_Plane__Mapped := IF(__d0_Missing_Plane__UIDComponents = 'n_number',PROJECT(__d0_Legal__Mapped,TRANSFORM(__d0_Plane__Layout,SELF.Plane_:=0,SELF:=LEFT)),JOIN(KEL.Intake.AppendFields(__d0_Legal__Mapped,__d0_Missing_Plane__UIDComponents),E_Aircraft(__in,__cfg).Lookup,TRIM((STRING)LEFT.n_number) = RIGHT.KeyVal,TRANSFORM(__d0_Plane__Layout,SELF.Plane_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,HASH));
  ...
  Layout Sele_Aircraft__Rollup(InLayout __r, DATASET(InLayout) __recs) := TRANSFORM
    SELF.Registration_Details_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Date_Vendor_First_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_First_Reported_,MIN,FALSE),KEL.typ.epoch Date_Vendor_Last_Reported_ := KEL.era.SimpleRoll(GROUP,Date_Vendor_Last_Reported_,MAX,FALSE),Registrant_Type_,Fractional_Owner_,Certificate_Issue_Date_,Certification_},Registrant_Type_,Fractional_Owner_,Certificate_Issue_Date_,Certification_),Registration_Details_Layout)(__NN(Registrant_Type_) OR __NN(Fractional_Owner_) OR __NN(Certificate_Issue_Date_) OR __NN(Certification_)));
  ...
  Layout Sele_Aircraft__Single_Rollup(InLayout __r) := TRANSFORM
    SELF.Registration_Details_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Registration_Details_Layout,SELF.__RecordCount:=1;,SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF.Date_Vendor_First_Reported_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Vendor_First_Reported_,FALSE),SELF.Date_Vendor_Last_Reported_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Vendor_Last_Reported_,FALSE),SELF:=LEFT))(__NN(Registrant_Type_) OR __NN(Fractional_Owner_) OR __NN(Certificate_Issue_Date_) OR __NN(Certification_)));

...
EXPORT E_Email_Inquiry(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __Mapping0 := 'Email_(DEFAULT:Email_:0),Transaction_(DEFAULT:Transaction_:0),person_q.email_address(OVERRIDE:Email_Address_:\'\'),search_info.transaction_id(OVERRIDE:Transaction_I_D_:\'\'),search_info.sequence_number(OVERRIDE:Sequence_Number_:\'\'),src(OVERRIDE:Source_:\'\'),dateofinquiry(OVERRIDE:Date_First_Seen_:EPOCHTIMESTAMP:Date_First_Seen_0Rule|OVERRIDE:Date_Last_Seen_:EPOCHTIMESTAMP:Date_Last_Seen_0Rule),hybrid_archive_date(OVERRIDE:Hybrid_Archive_Date_:EPOCHTIMESTAMP:Hybrid_Archive_Date_0Rule),archive_date(OVERRIDE:Archive___Date_:EPOCHTIMESTAMP:Archive___Date_0Rule),vaultdatelastseen(DEFAULT:Vault_Date_Last_Seen_:EPOCH),DPMBitmap(OVERRIDE:__Permits:PERMITS)';
  EXPORT __d0_KELfiltered := PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault((STRING)person_q.email_address <> '');
  SHARED __d0_Email__Layout := RECORD
    RECORDOF(__d0_KELfiltered);
    KEL.typ.uid Email_;
  END;
  SHARED __d0_Missing_Email__UIDComponents := KEL.Intake.ConstructMissingFieldList(__d0_KELfiltered,'person_q.email_address','PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault');
  SHARED __d0_Email__Mapped := IF(__d0_Missing_Email__UIDComponents = 'person_q.email_address',PROJECT(__d0_KELfiltered,TRANSFORM(__d0_Email__Layout,SELF.Email_:=0,SELF:=LEFT)),JOIN(KEL.Intake.AppendFields(__d0_KELfiltered,__d0_Missing_Email__UIDComponents),E_Email(__cfg).Lookup,TRIM((STRING)LEFT.person_q.email_address) = RIGHT.KeyVal,TRANSFORM(__d0_Email__Layout,SELF.Email_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,SMART));
  SHARED __d0_Transaction__Layout := RECORD
    RECORDOF(__d0_Email__Mapped);
    KEL.typ.uid Transaction_;
  END;
  SHARED __d0_Missing_Transaction__UIDComponents := KEL.Intake.ConstructMissingFieldList(__d0_Email__Mapped,'search_info.transaction_id,search_info.sequence_number','PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault');
  SHARED __d0_Transaction__Mapped := IF(__d0_Missing_Transaction__UIDComponents = 'search_info.transaction_id,search_info.sequence_number',PROJECT(__d0_Email__Mapped,TRANSFORM(__d0_Transaction__Layout,SELF.Transaction_:=0,SELF:=LEFT)),JOIN(KEL.Intake.AppendFields(__d0_Email__Mapped,__d0_Missing_Transaction__UIDComponents),E_Inquiry(__cfg).Lookup,TRIM((STRING)LEFT.search_info.transaction_id) + '|' + TRIM((STRING)LEFT.search_info.sequence_number) = RIGHT.KeyVal,TRANSFORM(__d0_Transaction__Layout,SELF.Transaction_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,SMART));
  SHARED __d0_Prefiltered := __d0_Transaction__Mapped;
  SHARED __d0 := __SourceFilter(KEL.FromFlat.Convert(__d0_Prefiltered,InLayout,__Mapping0,'PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault'));
  EXPORT InData := __d0;
  ...
  EXPORT NullCounts := DATASET([
    {'EmailInquiry','PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault','Email',COUNT(__d0(__NL(Email_))),COUNT(__d0(__NN(Email_)))},
    {'EmailInquiry','PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault','Transaction',COUNT(__d0(__NL(Transaction_))),COUNT(__d0(__NN(Transaction_)))},
    {'EmailInquiry','PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault','person_q.email_address',COUNT(__d0(__NL(Email_Address_))),COUNT(__d0(__NN(Email_Address_)))},
    {'EmailInquiry','PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault','search_info.transaction_id',COUNT(__d0(__NL(Transaction_I_D_))),COUNT(__d0(__NN(Transaction_I_D_)))},
    {'EmailInquiry','PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault','search_info.sequence_number',COUNT(__d0(__NL(Sequence_Number_))),COUNT(__d0(__NN(Sequence_Number_)))},
    {'EmailInquiry','PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault','Src',COUNT(__d0(__NL(Source_))),COUNT(__d0(__NN(Source_)))},
    {'EmailInquiry','PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault','Archive_Date',COUNT(__d0(Archive___Date_=0)),COUNT(__d0(Archive___Date_!=0))},
    {'EmailInquiry','PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault','DateFirstSeen',COUNT(__d0(Date_First_Seen_=0)),COUNT(__d0(Date_First_Seen_!=0))},
    {'EmailInquiry','PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault','DateLastSeen',COUNT(__d0(Date_Last_Seen_=0)),COUNT(__d0(Date_Last_Seen_!=0))},
    {'EmailInquiry','PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault','HybridArchiveDate',COUNT(__d0(Hybrid_Archive_Date_=0)),COUNT(__d0(Hybrid_Archive_Date_!=0))},
    {'EmailInquiry','PublicRecords_KEL.files.NonFCRA.Inquiry_AccLogs__Inquiry_ALL_Email_Vault','VaultDateLastSeen',COUNT(__d0(Vault_Date_Last_Seen_=0)),COUNT(__d0(Vault_Date_Last_Seen_!=0))}]

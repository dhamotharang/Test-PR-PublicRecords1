...
EXPORT B_Person_Property_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5519449 := __ENH_Person_Property_8;
  SHARED __EE5519451 := __ENH_Property_Event_8;
  SHARED __EE5519453 := __E_Person_Property_Event;
  SHARED __EE5520287 := __EE5519453(__NN(__EE5519453.Event_) AND __NN(__EE5519453.Subject_));
  SHARED __EE5520301 := __EE5520287.Party_Details_;
  __JC5520304(E_Person_Property_Event(__in,__cfg).Party_Details_Layout __EE5520301) := __T(__EE5520301.Party_Is_Seller_);
  SHARED __EE5520305 := __EE5520287(EXISTS(__CHILDJOINFILTER(__EE5520301,__JC5520304)));
  SHARED __ST450601_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  __JC5520335(B_Property_Event_8(__in,__cfg).__ST274319_Layout __EE5519451, E_Person_Property_Event(__in,__cfg).Layout __EE5520305) := __EEQP(__EE5519451.UID,__EE5520305.Event_);
  __ST450601_Layout __JT5520335(B_Property_Event_8(__in,__cfg).__ST274319_Layout __l, E_Person_Property_Event(__in,__cfg).Layout __r) := TRANSFORM
    SELF.L_N_Fares_I_D__1_ := __r.L_N_Fares_I_D_;
    SELF.Data_Sources__1_ := __r.Data_Sources_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE5520336 := JOIN(__EE5520305,__EE5519451,__JC5520335(RIGHT,LEFT),__JT5520335(RIGHT,LEFT),INNER,HASH);
  SHARED __ST450970_Layout := RECORD
    KEL.typ.ntyp(E_Person().Typ) Subject_;
    KEL.typ.ntyp(E_Property().Typ) Prop_;
    KEL.typ.nstr Primary_Range_;
    KEL.typ.nstr Predirectional_;
    ...
    KEL.typ.ndataset(__ST450601_Layout) Property_Event_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC5520898(B_Person_Property_8(__in,__cfg).__ST274224_Layout __EE5519449, __ST450601_Layout __EE5520336) := __EEQP(__EE5519449.Prop_,__EE5520336.Prop_) AND __NNEQ(__EE5520336.Subject_,__EE5519449.Subject_) AND __T(__AND(__EEQ(__EE5519449.Prop_,__EE5520336.Prop_),__AND(__OP2(__EE5520336.Subject_,=,__EE5519449.Subject_),__CN(__NN(__EE5519449.Prop_)))));
  __ST450970_Layout __Join__ST450970_Layout(B_Person_Property_8(__in,__cfg).__ST274224_Layout __r, DATASET(__ST450601_Layout) __recs) := TRANSFORM
    SELF := __r;
    SELF.Property_Event_ := __CN(__recs);
  END;
  SHARED __EE5521007 := DENORMALIZE(DISTRIBUTE(__EE5519449,HASH(Prop_)),DISTRIBUTE(__EE5520336,HASH(Prop_)),__JC5520898(LEFT,RIGHT),GROUP,__Join__ST450970_Layout(LEFT,ROWS(RIGHT)),LOCAL);
  EXPORT __ST100467_Layout := RECORD
    KEL.typ.nstr L_N_Fares_I_D_;
    KEL.typ.nkdate Dt_First_Seen_;
    KEL.typ.nint Sale_Price_;
    KEL.typ.epoch Archive___Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST271433_Layout := RECORD
    KEL.typ.ntyp(E_Person().Typ) Subject_;
    KEL.typ.ntyp(E_Property().Typ) Prop_;
    KEL.typ.nstr Primary_Range_;
    KEL.typ.nstr Predirectional_;
    ...
    KEL.typ.ndataset(E_Person_Property(__in,__cfg).Data_Sources_Layout) Data_Sources_;
    KEL.typ.bool Property_Is_Sold_ := FALSE;
    KEL.typ.ndataset(__ST100467_Layout) Property_Sale_Info_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST271433_Layout __ND5521016__Project(__ST450970_Layout __PP5521012) := TRANSFORM
    __EE5521010 := __PP5521012.Property_Event_;
    __EE5521093 := IF(__PP5521012.Property_Is_Sold_,__CN(PROJECT(TABLE(PROJECT(__T(__EE5521010),__ST100467_Layout),{KEL.typ.int __RecordCount := SUM(GROUP,__RecordCount),KEL.typ.epoch Archive___Date_ := KEL.era.SimpleRoll(GROUP,Archive___Date_,MIN,FALSE),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),KEL.typ.epoch Hybrid_Archive_Date_ := KEL.era.SimpleRoll(GROUP,Hybrid_Archive_Date_,MIN,FALSE),KEL.typ.epoch Vault_Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Vault_Date_Last_Seen_,MAX,NMAX),L_N_Fares_I_D_,Dt_First_Seen_,Sale_Price_},L_N_Fares_I_D_,Dt_First_Seen_,Sale_Price_,MERGE),__ST100467_Layout)),__N(DATASET(__ST100467_Layout)));
    SELF.Property_Sale_Info_ := __FILTER(__EE5521093,__NN(L_N_Fares_I_D_) OR __NN(Dt_First_Seen_) OR __NN(Sale_Price_));
    SELF := __PP5521012;
  END;
  EXPORT __ENH_Person_Property_7 := PROJECT(__EE5521007,__ND5521016__Project(LEFT));
END;

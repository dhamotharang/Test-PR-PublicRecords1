...
EXPORT B_Email_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1469827 := __E_Email;
  EXPORT __ST203347_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST203347_Layout __ND6434257__Project(E_Email(__in,__cfg).Layout __PP1469632) := TRANSFORM
    ...
    __CC31713 := 2556;
    SELF.Emails7y_ := __OP2(FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__CC13018),__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP1469632.Date_Last_Seen_))),<,__CN(__CC31713));
    SELF := __PP1469632;
  END;
  EXPORT __ENH_Email_2 := PROJECT(__EE1469827,__ND6434257__Project(LEFT));
END;

...
EXPORT E_Person(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __Mapping4 := 'did(UID),title(Title_),fname(First_Name_),mname(Middle_Name_),lname(Last_Name_),name_suffix(Name_Suffix_),lexidsegment(Lex_I_D_Segment_:\'\'),dateofbirth(Date_Of_Birth_:DATE),dateofdeath(Date_Of_Death_:DATE),gender(Gender_:\'\'),race(Race_:\'\'),racedescription(Race_Description_:\'\'),date_first_seen(Date_First_Seen_:EPOCH),date_last_seen(Date_Last_Seen_:EPOCH),DPMBitmap(__Permits:PERMITS)';
  ...
  EXPORT PublicRecords_KEL_ECL_Functions_Dataset_FDC_Dataset_Bankruptcy_Files__Key_Search_Invalid := __d4_Norm((KEL.typ.uid)did = 0);
  ...
  EXPORT NullCounts := DATASET([
    ...
    {'Person','PublicRecords_KEL.ECL_Functions.Dataset_FDC','UID',COUNT(PublicRecords_KEL_ECL_Functions_Dataset_FDC_Dataset_Bankruptcy_Files__Key_Search_Invalid),COUNT(__d4)},

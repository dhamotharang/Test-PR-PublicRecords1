...
EXPORT E_Utility_Person(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __Mapping0 := 'id(OVERRIDE:Util_:0),did(OVERRIDE:Subject_:0),src(OVERRIDE:Source_:\'\'),archive_date(OVERRIDE:Archive___Date_:EPOCH),date_first_seen(OVERRIDE:Date_First_Seen_:EPOCH),datelastseen(DEFAULT:Date_Last_Seen_:EPOCH),hybrid_archive_date(OVERRIDE:Hybrid_Archive_Date_:EPOCH:Hybrid_Archive_Date_0Rule),vaultdatelastseen(DEFAULT:Vault_Date_Last_Seen_:EPOCH),DPMBitmap(OVERRIDE:__Permits:PERMITS)';
  EXPORT __d0_KELfiltered := PublicRecords_KEL.Files.NonFCRA.UtilFile__Key_DID_Vault((UNSIGNED)did != 0);
  SHARED __d0_Prefiltered := __d0_KELfiltered;
  SHARED __d0 := __SourceFilter(KEL.FromFlat.Convert(__d0_Prefiltered,InLayout,__Mapping0,'PublicRecords_KEL.Files.NonFCRA.UtilFile__Key_DID_Vault'));
  EXPORT InData := __d0;
  ...
  EXPORT NullCounts := DATASET([
    {'UtilityPerson','PublicRecords_KEL.Files.NonFCRA.UtilFile__Key_DID_Vault','id',COUNT(__d0(__NL(Util_))),COUNT(__d0(__NN(Util_)))},
    {'UtilityPerson','PublicRecords_KEL.Files.NonFCRA.UtilFile__Key_DID_Vault','did',COUNT(__d0(__NL(Subject_))),COUNT(__d0(__NN(Subject_)))},
    {'UtilityPerson','PublicRecords_KEL.Files.NonFCRA.UtilFile__Key_DID_Vault','src',COUNT(__d0(__NL(Source_))),COUNT(__d0(__NN(Source_)))},
    {'UtilityPerson','PublicRecords_KEL.Files.NonFCRA.UtilFile__Key_DID_Vault','Archive_Date',COUNT(__d0(Archive___Date_=0)),COUNT(__d0(Archive___Date_!=0))},
    {'UtilityPerson','PublicRecords_KEL.Files.NonFCRA.UtilFile__Key_DID_Vault','DateFirstSeen',COUNT(__d0(Date_First_Seen_=0)),COUNT(__d0(Date_First_Seen_!=0))},
    {'UtilityPerson','PublicRecords_KEL.Files.NonFCRA.UtilFile__Key_DID_Vault','DateLastSeen',COUNT(__d0(Date_Last_Seen_=0)),COUNT(__d0(Date_Last_Seen_!=0))},
    {'UtilityPerson','PublicRecords_KEL.Files.NonFCRA.UtilFile__Key_DID_Vault','HybridArchiveDate',COUNT(__d0(Hybrid_Archive_Date_=0)),COUNT(__d0(Hybrid_Archive_Date_!=0))},
    {'UtilityPerson','PublicRecords_KEL.Files.NonFCRA.UtilFile__Key_DID_Vault','VaultDateLastSeen',COUNT(__d0(Vault_Date_Last_Seen_=0)),COUNT(__d0(Vault_Date_Last_Seen_!=0))}]

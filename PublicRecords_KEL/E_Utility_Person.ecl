...
EXPORT E_Utility_Person(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __Mapping := 'util(Util_:0),subject(Subject_:0),source(Source_:\'\'),datefirstseen(Date_First_Seen_:EPOCH),datelastseen(Date_Last_Seen_:EPOCH)';
  ...
  EXPORT NullCounts := DATASET([
    {'UtilityPerson','PublicRecords_KEL.ECL_Functions.Dataset_FDC','Util',COUNT(__d0(__NL(Util_))),COUNT(__d0(__NN(Util_)))},
    {'UtilityPerson','PublicRecords_KEL.ECL_Functions.Dataset_FDC','Subject',COUNT(__d0(__NL(Subject_))),COUNT(__d0(__NN(Subject_)))},
    {'UtilityPerson','PublicRecords_KEL.ECL_Functions.Dataset_FDC','Source',COUNT(__d0(__NL(Source_))),COUNT(__d0(__NN(Source_)))},

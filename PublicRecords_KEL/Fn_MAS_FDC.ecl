...
EXPORT Fn_MAS_FDC(DATASET(PublicRecords_KEL.ECL_Functions.Layouts.LayoutInputPII) Input,
  ...
  unsigned1 iType := IF(Options.IsFCRA, data_services.data_env.iFCRA, data_services.data_env.iNonFCRA);
	...
	Input_Address_Business_recs :=
		...
		Previous_Address_Consumer_recs :=
		PROJECT( Input,
			TRANSFORM( Layouts_FDC.LayoutAddressGeneric,
				SELF.UIDAppend       := LEFT.G_ProcUID,
				...
				// SELF.AddressGeoLink  := (LEFT.previousAddrStateCode + trim(left.previousAddrCnty, left, right) + left.previousAddrGeo),//previousAddrCnty is string6 but 3 digits so needs trimming
	...
	Current_Address_Consumer_recs :=
		PROJECT( Input,
			TRANSFORM( Layouts_FDC.LayoutAddressGeneric,
				...
				// SELF.AddressGeoLink  := (LEFT.currentAddrStCode + trim(left.currentAddrCnty, left, right) + left.currentAddrGeo),//currentAddrCnty is string6 but 3 digits so needs trimming
				// SELF.CityCode        := Doxie.Make_CityCode(LEFT.P_InpClnAddrCity), // doxie.Make_CityCodes(LEFT.InputCityClean).rox) ???
				SELF := LEFT,
				SELF := []
			)
		...
		PROJECT( Input,
			TRANSFORM( Layouts_FDC.LayoutAddressGeneric,
				...
				// SELF.AddressGeoLink  := (LEFT.EmergingAddrStateCode + trim(left.EmergingAddrCnty, left, right) + left.EmergingAddrGeo),//EmergingAddrCnty is string6 but 3 digits so needs trimming
				// SELF.CityCode        := Doxie.Make_CityCode(LEFT.P_InpClnAddrCity), // doxie.Make_CityCodes(LEFT.InputCityClean).rox) ???
				SELF := LEFT,
				SELF := []
			)
...
//First call to contact key with business to get lexid's associated with businesses
	...
	BIPV2_Build__kfetch_contact_linkids_Gathered :=
		PublicRecords_KEL.ecl_functions.DateSelector(IF(Common.DoFDCJoin_BIPV2_Build__kfetch_contact_linkids = TRUE AND COUNT(Unique_Raw_Lexid_Matches_Filtered) < 300, PublicRecords_KEL.Fetch_Data_From.Contact_Linkids(Unique_Raw_Lexid_Matches_Filtered, PublicRecords_KEL.ECL_Functions.Constants.LinkSearch.PowID, linkingOptions, mod_access/*, 1000*/)),FALSE,TRUE);
...
BIPV2.IDAppendLayouts.AppendInput PrepBIPInput(Layouts_FDC.Layout_FDC le) := TRANSFORM
	...
	Key_Aircraft_did_Records :=
			PublicRecords_KEL.ecl_functions.DateSelector(JOIN(Input_FDC, FAA.key_aircraft_did(Options.isFCRA),
	...
	Key_Watercraft_did_Records :=
			PublicRecords_KEL.ecl_functions.DateSelector(JOIN(Input_FDC, Watercraft.key_watercraft_did(Options.isFCRA),
	...
	With_Business_Header_Key_Linking := DENORMALIZE(With_Email_Payload_Records, Business_Header_Key_Linking,
			LEFT.G_ProcBusUID = RIGHT.UniqueID AND
			LEFT.B_LexIDUlt = RIGHT.ULTID AND
			LEFT.B_LexIDOrg = RIGHT.ORGID AND
			LEFT.B_LexIDLegal = RIGHT.SELEID, GROUP,
	...
	Key_DNM_Name_Address_Records :=
		PublicRecords_KEL.ecl_functions.DateSelector(JOIN(Input_Address_All, DMA.Key_DNM_Name_Address,
	...
	Key_Fraudpoint3_Address_Records :=
		PublicRecords_KEL.ecl_functions.DateSelector(JOIN(Input_Address_All, Fraudpoint3.Key_Address,
	...
	Key_Fraudpoint3_SSN_Records :=
		PublicRecords_KEL.ecl_functions.DateSelector(JOIN(Input_FDC, Fraudpoint3.Key_SSN,
	...
	Key_UtilFile_Address :=
			PublicRecords_KEL.ecl_functions.DateSelector(JOIN(Input_Address_Current, UtilFile.Key_Address, 				Common.DoFDCJoin_UtilFile__Key_Address = TRUE AND
				LEFT.PrimaryName != '' AND LEFT.ZIP5 != '' AND
...
/* I Verification Records - By Lexid and Phone*/
	...
	Key_CellPhone__Key_Neustar_Phone :=
			PublicRecords_KEL.ecl_functions.DateSelector(JOIN(Input_Phone_All, CellPhone.key_neustar_phone,
...
//Get phone autokeys to fetch all the DID's associated for the input phone number
	...
	************************************************************************** */
	...
	Key_Vehicle_did_Records :=
			PublicRecords_KEL.ecl_functions.DateSelector(JOIN(Input_FDC, VehicleV2.Key_Vehicle_DID,
...
/* Lookup by did (both FCRA and NonFCRA)*/
	...
	PropertyV2__Key_Property_Did_Records :=
			PublicRecords_KEL.ecl_functions.DateSelector(JOIN(Clean_Input_Plus_Contacts, LN_PropertyV2.key_Property_did(Options.isFCRA),
				...
				TRANSFORM(Layouts_FDC.Layout_PropertyV2_Key_Property_Did_Records,
					...
					SELF := LEFT,
					SELF := []),
				ATMOST(PublicRecords_KEL.ECL_Functions.Constants.PROPERTY_DID_JOIN_LIMIT)),FALSE,FALSE);
...
/* Lookup by address (both FCRA and NonFCRA)*/
	...
	PropertyV2__Key_Addr_Fid_Records :=
			PublicRecords_KEL.ecl_functions.DateSelector(JOIN(Input_and_Best_Address, LN_PropertyV2.key_addr_fid(Options.isFCRA),
...
/* Consumer and Business */
	...
	PropertyV2__Key_Search_Fid_Records :=
			PublicRecords_KEL.ecl_functions.DateSelector(JOIN(Property_lookup_search_records, LN_PropertyV2.key_search_fid(Options.isFCRA),
	...
	PropertyV2__Key_Addl_Fares_Deed_Fid_Records :=
			PublicRecords_KEL.ecl_functions.DateSelector(JOIN(Property_lookup_search_records, LN_PropertyV2.key_addl_fares_deed_fid,
...
/*key_liens_main_id Keys have a parameter to say if FCRA or nonFCRA - same file layout*/
	...
	//gather lexids from input ssn
		Key_wild_SSN :=
			PublicRecords_KEL.ecl_functions.DateSelector(JOIN(Input_FDC, dx_Header.key_wild_SSN(),//non FCRA only
				Common.DoFDCJoin_Dx_Header__key_wild_SSN = TRUE AND
				(INTEGER)LEFT.P_InpClnSSN > 0 AND
				KEYED(LEFT.P_InpClnSSN[1] = RIGHT.s1 AND
							LEFT.P_InpClnSSN[2] = RIGHT.s2 AND
	...
	With_Best_Person__Key_Watchdog := DENORMALIZE(With_Doxie__Key_Header, Best_Person__Key_Watchdog_Records,
				...
				TRANSFORM(Layouts_FDC.Layout_FDC,
						SELF.Dataset_Best_Person__Key_Watchdog := project(rows(right),transform(Layouts_FDC.Layout_Best_Person__Key_Watchdog,
																																	...
																																	SELF.DPMBitmap := SetDPMBitmap( Source := BlankString, FCRA_Restricted := Options.isFCRA ,  KELPermissions := CFG_File);
																																	self.rec  := left._best,
																																	self := []));
																													SELF := LEFT,
																													SELF := []), ALL);
	...
	temp_contacts_surnames_dedup :=DEDUP(SORT(temp_contacts_surnames(P_InpClnNameLast<>''),P_InpClnNameLast));

 Input_surnames := Input_FDC + temp_contacts_surnames_dedup;
...
dx_CFPB__key_Census_Surnames := IF( Options.isFCRA, dx_ConsumerFinancialProtectionBureau.key_census_surnames(TRUE), dx_ConsumerFinancialProtectionBureau.key_census_surnames(False));
	...
	RETURN With_Key_CFPB__key_Census_Surnames;

...
EXPORT B_Address_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE178766 := __E_Address;
  EXPORT __ST154787_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST154787_Layout __ND181670__Project(E_Address(__in,__cfg).Layout __PP178127) := TRANSFORM
    __EE178571 := __PP178127.A_D_V_O_Summary_;
    __BS178897 := __T(__EE178571);
    __EE178917 := __BN(TOPN(__BS178897(__NN(__T(__EE178571).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE178571).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE178571));
    SELF.Bestchild_Advo_ := __EE178917;
    SELF := __PP178127;
  END;
  EXPORT __ENH_Address_3 := PROJECT(__EE178766,__ND181670__Project(LEFT));
END;

...
IMPORT KEL16 AS KEL;
IMPORT CFG_Compile,E_Address,E_Geo_Link,E_Zip_Code FROM PublicRecords_KEL.KEL_Queries_MAS_Consumer;
...
EXPORT B_Address_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE176978 := __E_Address;
  EXPORT __ST152999_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST152999_Layout __ND179882__Project(E_Address(__in,__cfg).Layout __PP176339) := TRANSFORM
    __EE176783 := __PP176339.A_D_V_O_Summary_;
    __BS177109 := __T(__EE176783);
    __EE177129 := __BN(TOPN(__BS177109(__NN(__T(__EE176783).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE176783).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE176783));
    SELF.Bestchild_Advo_ := __EE177129;
    SELF := __PP176339;
  END;
  EXPORT __ENH_Address_3 := PROJECT(__EE176978,__ND179882__Project(LEFT));
END;

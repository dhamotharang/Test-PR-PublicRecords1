...
EXPORT B_Address_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE174166 := __E_Address;
  EXPORT __ST147957_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST147957_Layout __ND177070__Project(E_Address(__in,__cfg).Layout __PP173527) := TRANSFORM
    __EE173971 := __PP173527.A_D_V_O_Summary_;
    __BS174297 := __T(__EE173971);
    __EE174317 := __BN(TOPN(__BS174297(__NN(__T(__EE173971).A_D_V_O_Date_First_Seen_)),1, -__T(__T(__EE173971).A_D_V_O_Date_First_Seen_),__T(Vacancy_Indicator_),__T(Throw_Back_Indicator_),__T(Seasonal_Delivery_Indicator_),__T(Style_Code_),__T(Drop_Indicator_),__T(College_Indicator_),__T(Only_Way_To_Get_Mail_Indicator_),__T(Residential_Or_Business_Indicator_),__T(Do_Not_Deliver_Indicator_),__T(A_D_V_O_Date_Last_Seen_)),__NL(__EE173971));
    SELF.Bestchild_Advo_ := __EE174317;
    SELF := __PP173527;
  END;
  EXPORT __ENH_Address_3 := PROJECT(__EE174166,__ND177070__Project(LEFT));
END;

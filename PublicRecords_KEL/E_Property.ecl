...
EXPORT E_Property(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  Layout Property__Single_Rollup(InLayout __r) := TRANSFORM
    ...
    SELF.Automated_Valuation_Model_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Automated_Valuation_Model_Layout,SELF.__RecordCount:=1;,SELF.Archive___Date_:=KEL.era.SimpleRollSingleRow(LEFT,Archive___Date_,FALSE),SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF.Date_Vendor_First_Reported_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Vendor_First_Reported_,FALSE),SELF.Date_Vendor_Last_Reported_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Vendor_Last_Reported_,FALSE),SELF:=LEFT))(__NN(A_V_M_Unformatted_A_P_N_) OR __NN(A_V_M_Land_Use_Code_) OR __NN(A_V_M_Recording_Date_) OR __NN(A_V_M_Assessed_Value_Year_) OR __NN(A_V_M_Sales_Price_) OR __NN(A_V_M_Assessed_Total_Value_) OR __NN(A_V_M_Market_Total_Value_) OR __NN(A_V_M_Tax_Assessment_Valuation_) OR __NN(A_V_M_Price_Index_Valuation_) OR __NN(A_V_M_Hedonic_Valuation_) OR __NN(A_V_M_Automated_Valuation_) OR __NN(A_V_M_Confidence_Score_) OR __NN(A_V_M_Current_Flag_)));

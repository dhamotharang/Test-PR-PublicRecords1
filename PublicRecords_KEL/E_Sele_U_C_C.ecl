...
IMPORT CFG_Compile,E_Business_Org,E_Business_Sele,E_Business_Sele_Overflow,E_Business_Ult,E_U_C_C FROM PublicRecords_KEL;
IMPORT * FROM KEL15.Null;
EXPORT E_Sele_U_C_C(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  EXPORT __d0_KELfiltered := PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault((UNSIGNED)ultid<>0 AND (UNSIGNED)orgid<>0 AND (UNSIGNED)seleid<>0);
  SHARED __d0_Legal__Layout := RECORD
    RECORDOF(__d0_KELfiltered);
    KEL.typ.uid Legal_;
  END;
  SHARED __d0_Missing_Legal__UIDComponents := KEL.Intake.ConstructMissingFieldList(__d0_KELfiltered,'ultid,orgid,seleid','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault');
  ...
  SHARED __d0_Filing__Layout := RECORD
    ...
    KEL.typ.uid Filing_;
  END;
  SHARED __d0_Missing_Filing__UIDComponents := KEL.Intake.ConstructMissingFieldList(__d0_Legal__Mapped,'tmsid','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault');
  SHARED __d0_Filing__Mapped := IF(__d0_Missing_Filing__UIDComponents = 'tmsid',PROJECT(__d0_Legal__Mapped,TRANSFORM(__d0_Filing__Layout,SELF.Filing_:=0,SELF:=LEFT)),JOIN(KEL.Intake.AppendFields(__d0_Legal__Mapped,__d0_Missing_Filing__UIDComponents),E_U_C_C(__cfg).Lookup,TRIM((STRING)LEFT.tmsid) = RIGHT.KeyVal,TRANSFORM(__d0_Filing__Layout,SELF.Filing_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,SMART));
  ...
  EXPORT __d1_KELfiltered := PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault((UNSIGNED)ultid<>0 AND (UNSIGNED)orgid<>0 AND (UNSIGNED)seleid<>0);
  SHARED __d1_Legal__Layout := RECORD
    RECORDOF(__d1_KELfiltered);
    KEL.typ.uid Legal_;
  END;
  SHARED __d1_Missing_Legal__UIDComponents := KEL.Intake.ConstructMissingFieldList(__d1_KELfiltered,'ultid,orgid,seleid','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault');
  ...
  SHARED __d1_Filing__Layout := RECORD
    ...
    KEL.typ.uid Filing_;
  END;
  SHARED __d1_Missing_Filing__UIDComponents := KEL.Intake.ConstructMissingFieldList(__d1_Legal__Mapped,'tmsid','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault');
  SHARED __d1_Filing__Mapped := IF(__d1_Missing_Filing__UIDComponents = 'tmsid',PROJECT(__d1_Legal__Mapped,TRANSFORM(__d1_Filing__Layout,SELF.Filing_:=0,SELF:=LEFT)),JOIN(KEL.Intake.AppendFields(__d1_Legal__Mapped,__d1_Missing_Filing__UIDComponents),E_U_C_C(__cfg).Lookup,TRIM((STRING)LEFT.tmsid) = RIGHT.KeyVal,TRANSFORM(__d1_Filing__Layout,SELF.Filing_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,SMART));
  SHARED __d1_Prefiltered := __d1_Filing__Mapped;
  SHARED __d1 := __SourceFilter(KEL.FromFlat.Convert(__d1_Prefiltered,InLayout,__Mapping1,'PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault'));
  ...
  EXPORT Filing__Orphan := JOIN(InData(__NN(Filing_)),E_U_C_C(__cfg).__Result,__EEQP(LEFT.Filing_, RIGHT.UID),TRANSFORM(InLayout,SELF := LEFT,SELF:=[]),LEFT ONLY, HASH);
  ...
  EXPORT NullCounts := DATASET([
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault','Legal',COUNT(__d0(__NL(Legal_))),COUNT(__d0(__NN(Legal_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault','Filing',COUNT(__d0(__NL(Filing_))),COUNT(__d0(__NN(Filing_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault','ultid',COUNT(__d0(__NL(Ult_I_D_))),COUNT(__d0(__NN(Ult_I_D_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault','orgid',COUNT(__d0(__NL(Org_I_D_))),COUNT(__d0(__NN(Org_I_D_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault','seleid',COUNT(__d0(__NL(Sele_I_D_))),COUNT(__d0(__NN(Sele_I_D_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault','tmsid',COUNT(__d0(__NL(T_M_S_I_D_))),COUNT(__d0(__NN(T_M_S_I_D_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault','rmsid',COUNT(__d0(__NL(R_M_S_I_D_))),COUNT(__d0(__NN(R_M_S_I_D_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault','party_type',COUNT(__d0(__NL(Party_Type_))),COUNT(__d0(__NN(Party_Type_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault','src',COUNT(__d0(__NL(Source_))),COUNT(__d0(__NN(Source_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault','Archive_Date',COUNT(__d0(Archive___Date_=0)),COUNT(__d0(Archive___Date_!=0))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault','DateFirstSeen',COUNT(__d0(Date_First_Seen_=0)),COUNT(__d0(Date_First_Seen_!=0))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault','DateLastSeen',COUNT(__d0(Date_Last_Seen_=0)),COUNT(__d0(Date_Last_Seen_!=0))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault','HybridArchiveDate',COUNT(__d0(Hybrid_Archive_Date_=0)),COUNT(__d0(Hybrid_Archive_Date_!=0))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_LinkIds_Vault','VaultDateLastSeen',COUNT(__d0(Vault_Date_Last_Seen_=0)),COUNT(__d0(Vault_Date_Last_Seen_!=0))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault','Legal',COUNT(__d1(__NL(Legal_))),COUNT(__d1(__NN(Legal_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault','Filing',COUNT(__d1(__NL(Filing_))),COUNT(__d1(__NN(Filing_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault','ultid',COUNT(__d1(__NL(Ult_I_D_))),COUNT(__d1(__NN(Ult_I_D_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault','orgid',COUNT(__d1(__NL(Org_I_D_))),COUNT(__d1(__NN(Org_I_D_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault','seleid',COUNT(__d1(__NL(Sele_I_D_))),COUNT(__d1(__NN(Sele_I_D_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault','tmsid',COUNT(__d1(__NL(T_M_S_I_D_))),COUNT(__d1(__NN(T_M_S_I_D_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault','rmsid',COUNT(__d1(__NL(R_M_S_I_D_))),COUNT(__d1(__NN(R_M_S_I_D_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault','party_type',COUNT(__d1(__NL(Party_Type_))),COUNT(__d1(__NN(Party_Type_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault','src',COUNT(__d1(__NL(Source_))),COUNT(__d1(__NN(Source_)))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault','Archive_Date',COUNT(__d1(Archive___Date_=0)),COUNT(__d1(Archive___Date_!=0))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault','DateFirstSeen',COUNT(__d1(Date_First_Seen_=0)),COUNT(__d1(Date_First_Seen_!=0))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault','DateLastSeen',COUNT(__d1(Date_Last_Seen_=0)),COUNT(__d1(Date_Last_Seen_!=0))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault','HybridArchiveDate',COUNT(__d1(Hybrid_Archive_Date_=0)),COUNT(__d1(Hybrid_Archive_Date_!=0))},
    {'SeleUCC','PublicRecords_KEL.Files.NonFCRA.UCCV2__Key_rmsid_party_Vault','VaultDateLastSeen',COUNT(__d1(Vault_Date_Last_Seen_=0)),COUNT(__d1(Vault_Date_Last_Seen_!=0))}]

...
EXPORT Q_Non_F_C_R_A_Mini_Attributes_V1_Roxie_Dynamic(SET OF KEL.typ.uid __PLexID_in, KEL.typ.kdate __PP_InpClnArchDt, DATA57 __PDPM) := MODULE
  SHARED __cfg_Local := MODULE(CFG_Compile)
    EXPORT KEL.typ.kdate CurrentDate := __PP_InpClnArchDt;
  END;
  SHARED E_Person_Filtered_1 := MODULE(E_Person(__cfg_Local))
    ...
    SHARED __GroupTest(DATASET(InLayout) __g) := EXISTS(__g(__T(__OP2(__g.UID,IN,__CN(__PLexID_in)))));
  ...
  SHARED E_Person_Address_Filtered := MODULE(E_Person_Address(__cfg_Local))
    ...
    SHARED __UsingFitler(DATASET(InLayout) __ds) := __ds(KEL.Permits.BitAnd(__ds.__Permits,__PDPM) = __ds.__Permits);
    SHARED __SourceFilter(DATASET(InLayout) __ds) := __UsingFitler(__AsofFitler(__ds));
    SHARED __GroupedFilter(GROUPED DATASET(InLayout) __ds) := JOIN(__ds,E_Person_Filtered_1.__PostFilter,__EEQP(LEFT.Subject_,RIGHT.UID),TRANSFORM(InLayout,SELF:=LEFT),HASH,GROUPED,KEEP(1));
  END;
  ...
  SHARED __EE12862748 := __ENH_Person;
  SHARED __EE12864808 := __EE12862748(__T(__OP2(__EE12862748.UID,IN,__CN(__PLexID_in))));
  SHARED __ST127615_Layout := RECORD
  ...
  END;
  EXPORT Res0 := __UNWRAP(PROJECT(__EE12864808,TRANSFORM(__ST127615_Layout,SELF.Lex_I_D_ := LEFT.UID,SELF := LEFT)));
  ...
  EXPORT DBG_OutputAll := PARALLEL(
    ...
    OUTPUT(DBG_E_Lien_Judgment_Intermediate_13,NAMED('DBG_E_Lien_Judgment_Intermediate_13_Q_Non_F_C_R_A_Mini_Attributes_V1_Roxie_Dynamic')),
    OUTPUT(DBG_E_Person_Lien_Judgment_Intermediate_12,NAMED('DBG_E_Person_Lien_Judgment_Intermediate_12_Q_Non_F_C_R_A_Mini_Attributes_V1_Roxie_Dynamic')),

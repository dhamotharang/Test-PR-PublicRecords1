...
IMPORT KEL15 AS KEL;
IMPORT CFG_Compile,E_Address,E_Business_Org,E_Business_Sele,E_Business_Sele_Overflow,E_Business_Ult,E_Email,E_Geo_Link,E_Input_B_I_I,E_Input_P_I_I,E_Person,E_Phone,E_Property,E_Social_Security_Number,E_Surname,E_Zip_Code FROM PublicRecords_KEL;
IMPORT * FROM KEL15.Null;
EXPORT E_Input_B_I_I_Input_P_I_I(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  EXPORT P_I_I__Orphan := JOIN(InData(__NN(P_I_I_)),E_Input_P_I_I(__cfg).__Result,__EEQP(LEFT.P_I_I_, RIGHT.UID),TRANSFORM(InLayout,SELF := LEFT,SELF:=[]),LEFT ONLY, HASH);
  EXPORT B_I_I__Orphan := JOIN(InData(__NN(B_I_I_)),E_Input_B_I_I(__cfg).__Result,__EEQP(LEFT.B_I_I_, RIGHT.UID),TRANSFORM(InLayout,SELF := LEFT,SELF:=[]),LEFT ONLY, HASH);

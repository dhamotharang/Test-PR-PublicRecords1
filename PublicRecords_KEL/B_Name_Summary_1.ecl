...
EXPORT B_Name_Summary_1(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7626331 := __ENH_Name_Summary_2;
  EXPORT __ST183748_Layout := RECORD
  ...
  EXPORT __ST183741_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST183748_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST183741_Layout __ND7626293__Project(B_Name_Summary_2(__in,__cfg).__ST205005_Layout __PP7626094) := TRANSFORM
    __EE7626334 := __PP7626094.Data_Sources_;
    SELF.Data_Sources_ := __BN(PROJECT(__T(__EE7626334),__ST183748_Layout),__NL(__EE7626334));
    __CC13691 := '-99999';
    __CC13693 := '-99998';
    __EE7626286 := __PP7626094.Name_Summary_Source_List_Sorted_;
    SELF.P_I___Src_W_Inp_F_L_D_List_Ev_ := MAP(__T(__OR(__OR(__OP2(__PP7626094.P___Inp_Cln_Name_First_,IN,__CN([__CC13691,__CC13693])),__OP2(__PP7626094.P___Inp_Cln_Name_Last_,IN,__CN([__CC13691,__CC13693]))),__OP2(__PP7626094.P___Inp_Cln_D_O_B_,IN,__CN([__CC13691,__CC13693]))))=>__ECAST(KEL.typ.nstr,__CN(__CC13691)),__T(__NOT(__CN(EXISTS(__T(__PP7626094.Name_Summary_Source_List_Sorted_)))))=>__ECAST(KEL.typ.nstr,__CN(__CC13693)),__ECAST(KEL.typ.nstr,KEL.Aggregates.ConcatNN(__EE7626286,LEFT.Translated_Source_Code_,__CN('|'))));
    SELF := __PP7626094;
  END;
  EXPORT __ENH_Name_Summary_1 := PROJECT(__EE7626331,__ND7626293__Project(LEFT));
END;

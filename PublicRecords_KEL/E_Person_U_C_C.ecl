...
EXPORT E_Person_U_C_C(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __d0_Filing__Layout := RECORD
    ...
    KEL.typ.uid Filing_;
  END;
  SHARED __d0_Filing__Mapped := JOIN(KEL.Intake.AppendNonExistUidComponents(__d0_KELfiltered,'tmsid','__in'),E_U_C_C(__in,__cfg).Lookup,TRIM((STRING)LEFT.tmsid) = RIGHT.KeyVal,TRANSFORM(__d0_Filing__Layout,SELF.Filing_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,HASH);
  ...
  SHARED __d1_Filing__Layout := RECORD
    ...
    KEL.typ.uid Filing_;
  END;
  SHARED __d1_Filing__Mapped := JOIN(KEL.Intake.AppendNonExistUidComponents(__d1_KELfiltered,'tmsid','__in'),E_U_C_C(__in,__cfg).Lookup,TRIM((STRING)LEFT.tmsid) = RIGHT.KeyVal,TRANSFORM(__d1_Filing__Layout,SELF.Filing_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,HASH);
  ...
  EXPORT NullCounts := DATASET([
    ...
    {'PersonUCC','PublicRecords_KEL.ECL_Functions.Dataset_FDC','dt_vendor_last_reported',COUNT(__d0(__NL(Date_Vendor_Last_Reported_))),COUNT(__d0(__NN(Date_Vendor_Last_Reported_)))},
    {'PersonUCC','PublicRecords_KEL.ECL_Functions.Dataset_FDC','dt_vendor_first_reported',COUNT(__d0(__NL(Date_Vendor_First_Reported_))),COUNT(__d0(__NN(Date_Vendor_First_Reported_)))},
    ...
    {'PersonUCC','PublicRecords_KEL.ECL_Functions.Dataset_FDC','dt_vendor_last_reported',COUNT(__d1(__NL(Date_Vendor_Last_Reported_))),COUNT(__d1(__NN(Date_Vendor_Last_Reported_)))},
    {'PersonUCC','PublicRecords_KEL.ECL_Functions.Dataset_FDC','dt_vendor_first_reported',COUNT(__d1(__NL(Date_Vendor_First_Reported_))),COUNT(__d1(__NN(Date_Vendor_First_Reported_)))},

...
EXPORT E_Person_U_C_C(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __d0_Filing__Mapped := IF(__d0_Missing_Filing__UIDComponents = 'tmsid',PROJECT(__d0_KELfiltered,TRANSFORM(__d0_Filing__Layout,SELF.Filing_:=0,SELF:=LEFT)),JOIN(KEL.Intake.AppendFields(__d0_KELfiltered,__d0_Missing_Filing__UIDComponents),E_U_C_C(__in,__cfg).Lookup,TRIM((STRING)LEFT.tmsid) = RIGHT.KeyVal,TRANSFORM(__d0_Filing__Layout,SELF.Filing_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,HASH));
  ...
  SHARED __d1_Filing__Mapped := IF(__d1_Missing_Filing__UIDComponents = 'tmsid',PROJECT(__d1_KELfiltered,TRANSFORM(__d1_Filing__Layout,SELF.Filing_:=0,SELF:=LEFT)),JOIN(KEL.Intake.AppendFields(__d1_KELfiltered,__d1_Missing_Filing__UIDComponents),E_U_C_C(__in,__cfg).Lookup,TRIM((STRING)LEFT.tmsid) = RIGHT.KeyVal,TRANSFORM(__d1_Filing__Layout,SELF.Filing_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,HASH));

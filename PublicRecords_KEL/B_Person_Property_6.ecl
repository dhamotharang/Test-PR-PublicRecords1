...
EXPORT B_Person_Property_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE430048 := __E_Person_Property;
  SHARED __ST430576_Layout := RECORD
  ...
  END;
  SHARED __EE429978 := __E_Person_Property_Event;
  SHARED __EE4876673 := __EE429978(__NN(__EE429978.Event_) AND __NN(__EE429978.Subject_));
  SHARED __EE429956 := __E_Property_Event;
  SHARED __EE430177 := __EE429956(__T(__EE429956.Is_Deed_));
  SHARED __ST430214_Layout := RECORD
    KEL.typ.ntyp(E_Person().Typ) Subject_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC4876747(E_Person_Property_Event(__in,__cfg).Layout __EE4876673, E_Property_Event(__in,__cfg).Layout __EE430177) := __EEQP(__EE430177.UID,__EE4876673.Event_);
  __ST430214_Layout __JT4876747(E_Person_Property_Event(__in,__cfg).Layout __l, E_Property_Event(__in,__cfg).Layout __r) := TRANSFORM
    SELF.L_N_Fares_I_D__1_ := __r.L_N_Fares_I_D_;
    SELF.Data_Sources__1_ := __r.Data_Sources_;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE4876748 := JOIN(__EE4876673,__EE430177,__JC4876747(LEFT,RIGHT),__JT4876747(LEFT,RIGHT),INNER,HASH);
  SHARED __ST430334_Layout := RECORD
    KEL.typ.ntyp(E_Person().Typ) Subject_;
    KEL.typ.ntyp(E_Property_Event().Typ) Event_;
    KEL.typ.nstr L_N_Fares_I_D_;
    KEL.typ.ndataset(E_Person_Property_Event(__in,__cfg).Party_Details_Layout) Party_Details_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __ST430334_Layout __JT4876847(__ST430214_Layout __l, E_Person_Property_Event(__in,__cfg).Party_Details_Layout __r) := TRANSFORM
    SELF.__RecordCount := __r.__RecordCount;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE4876848 := NORMALIZE(__EE4876748,__T(LEFT.Party_Details_),__JT4876847(LEFT,RIGHT));
  SHARED __EE4876941 := __EE4876848(__T(__EE4876848.Party_Is_Seller_));
  __JC4877065(E_Person_Property(__in,__cfg).Layout __EE430048, __ST430334_Layout __EE4876941) := __EEQP(__EE430048.Prop_,__EE4876941.Prop_) AND __NNEQ(__EE4876941.Subject_,__EE430048.Subject_) AND __T(__AND(__EEQ(__EE430048.Prop_,__EE4876941.Prop_),__AND(__OP2(__EE4876941.Subject_,=,__EE430048.Subject_),__CN(__NN(__EE430048.Prop_)))));
  __JF4877065(__ST430334_Layout __EE4876941) := __NN(__EE4876941.Prop_) OR __NN(__EE4876941.Subject_);
  SHARED __EE4877090 := JOIN(__EE430048,__EE4876941,__JC4877065(LEFT,RIGHT),TRANSFORM(__ST430576_Layout,SELF:=LEFT,SELF.Person_Property_Event_:=__JF4877065(RIGHT)),HASH,LEFT OUTER,KEEP(1));
  EXPORT __ST246695_Layout := RECORD
    KEL.typ.ntyp(E_Person().Typ) Subject_;
    KEL.typ.ntyp(E_Property().Typ) Prop_;
    KEL.typ.nstr Primary_Range_;
    KEL.typ.nstr Predirectional_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ENH_Person_Property_6 := PROJECT(__EE4877090,TRANSFORM(__ST246695_Layout,SELF.Property_Is_Sold_ := LEFT.Person_Property_Event_,SELF := LEFT));
END;

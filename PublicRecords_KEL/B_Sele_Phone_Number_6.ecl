...
EXPORT B_Sele_Phone_Number_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE375015 := __E_Sele_Phone_Number;
  SHARED __EE3557349 := __ENH_Business_Sele_7;
  SHARED __ST375159_Layout := RECORD
  ...
  END;
  __JC3557355(E_Sele_Phone_Number(__in,__cfg).Layout __EE375015, B_Business_Sele_7(__in,__cfg).__ST193589_Layout __EE3557349) := __EEQP(__EE375015.Legal_,__EE3557349.UID);
  __ST375159_Layout __JT3557355(E_Sele_Phone_Number(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST193589_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE3557356 := JOIN(__EE375015,__EE3557349,__JC3557355(LEFT,RIGHT),__JT3557355(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST192495_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST192457_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Phone().Typ) Phone_Number_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Best_Phone_Details_Layout) Best_Phone_Details_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Phone_Details_Layout) Phone_Details_;
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).S_I_C_Codes_Layout) S_I_C_Codes_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(__ST192495_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST192457_Layout __ND3557784__Project(__ST375159_Layout __PP3557357) := TRANSFORM
    __EE3557833 := __PP3557357.Data_Sources_;
    __ST192495_Layout __ND3557838__Project(E_Sele_Phone_Number(__in,__cfg).Data_Sources_Layout __PP3557834) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP3557834.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP3557834.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP3557834.Source_));
      SELF := __PP3557834;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE3557833,__ND3557838__Project(LEFT));
    SELF.Input_Phone_Match_ := FN_Compile(__cfg).FN_Is_Phone_Match(__ECAST(KEL.typ.nstr,__ECAST(KEL.typ.nstr,__PP3557357.Phone_Number_)),__ECAST(KEL.typ.nstr,__PP3557357.B___Inp_Cln_Phone_));
    SELF := __PP3557357;
  END;
  EXPORT __ENH_Sele_Phone_Number_6 := PROJECT(__EE3557356,__ND3557784__Project(LEFT));
END;

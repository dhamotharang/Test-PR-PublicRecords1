...
EXPORT B_Sele_Phone_Number_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE548586 := __E_Sele_Phone_Number;
  SHARED __EE6240755 := __ENH_Business_Sele_7;
  ...
  END;
  __JC6240761(E_Sele_Phone_Number(__in,__cfg).Layout __EE548586, B_Business_Sele_7(__in,__cfg).__ST280189_Layout __EE6240755) := __EEQP(__EE548586.Legal_,__EE6240755.UID);
  __ST548730_Layout __JT6240761(E_Sele_Phone_Number(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST280189_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE6240762 := JOIN(__EE548586,__EE6240755,__JC6240761(LEFT,RIGHT),__JT6240761(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST279060_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST279022_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Phone().Typ) Phone_Number_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Best_Phone_Details_Layout) Best_Phone_Details_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Phone_Details_Layout) Phone_Details_;
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).S_I_C_Codes_Layout) S_I_C_Codes_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(__ST279060_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST279022_Layout __ND6241121__Project(__ST548730_Layout __PP6240763) := TRANSFORM
    __EE6241170 := __PP6240763.Data_Sources_;
    __ST279060_Layout __ND6241175__Project(E_Sele_Phone_Number(__in,__cfg).Data_Sources_Layout __PP6241171) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP6241171.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP6241171.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP6241171.Source_));
      SELF := __PP6241171;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE6241170,__ND6241175__Project(LEFT));
    SELF.Input_Phone_Match_ := FN_Compile(__cfg).FN_Is_Phone_Match(__ECAST(KEL.typ.nstr,__ECAST(KEL.typ.nstr,__PP6240763.Phone_Number_)),__ECAST(KEL.typ.nstr,__PP6240763.B___Inp_Cln_Phone_));
    SELF := __PP6240763;
  END;
  EXPORT __ENH_Sele_Phone_Number_6 := PROJECT(__EE6240762,__ND6241121__Project(LEFT));
END;

...
EXPORT B_Sele_Phone_Number_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE434252 := __E_Sele_Phone_Number;
  SHARED __EE4871678 := __ENH_Business_Sele_7;
  SHARED __ST434396_Layout := RECORD
  ...
  END;
  __JC4871684(E_Sele_Phone_Number(__in,__cfg).Layout __EE434252, B_Business_Sele_7(__in,__cfg).__ST247821_Layout __EE4871678) := __EEQP(__EE434252.Legal_,__EE4871678.UID);
  __ST434396_Layout __JT4871684(E_Sele_Phone_Number(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST247821_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE4871685 := JOIN(__EE434252,__EE4871678,__JC4871684(LEFT,RIGHT),__JT4871684(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST246737_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST246699_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Phone().Typ) Phone_Number_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Best_Phone_Details_Layout) Best_Phone_Details_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Phone_Details_Layout) Phone_Details_;
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).S_I_C_Codes_Layout) S_I_C_Codes_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(__ST246737_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST246699_Layout __ND4872044__Project(__ST434396_Layout __PP4871686) := TRANSFORM
    __EE4872093 := __PP4871686.Data_Sources_;
    __ST246737_Layout __ND4872098__Project(E_Sele_Phone_Number(__in,__cfg).Data_Sources_Layout __PP4872094) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP4872094.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP4872094.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP4872094.Source_));
      SELF := __PP4872094;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE4872093,__ND4872098__Project(LEFT));
    SELF.Input_Phone_Match_ := FN_Compile(__cfg).FN_Is_Phone_Match(__ECAST(KEL.typ.nstr,__ECAST(KEL.typ.nstr,__PP4871686.Phone_Number_)),__ECAST(KEL.typ.nstr,__PP4871686.B___Inp_Cln_Phone_));
    SELF := __PP4871686;
  END;
  EXPORT __ENH_Sele_Phone_Number_6 := PROJECT(__EE4871685,__ND4872044__Project(LEFT));
END;

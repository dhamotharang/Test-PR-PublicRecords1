...
EXPORT B_Sele_Phone_Number_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE458815 := __E_Sele_Phone_Number;
  SHARED __EE5158520 := __ENH_Business_Sele_7;
  SHARED __ST458959_Layout := RECORD
  ...
  END;
  __JC5158526(E_Sele_Phone_Number(__in,__cfg).Layout __EE458815, B_Business_Sele_7(__in,__cfg).__ST257614_Layout __EE5158520) := __EEQP(__EE458815.Legal_,__EE5158520.UID);
  __ST458959_Layout __JT5158526(E_Sele_Phone_Number(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST257614_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE5158527 := JOIN(__EE458815,__EE5158520,__JC5158526(LEFT,RIGHT),__JT5158526(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST256530_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST256492_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Phone().Typ) Phone_Number_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Best_Phone_Details_Layout) Best_Phone_Details_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Phone_Details_Layout) Phone_Details_;
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).S_I_C_Codes_Layout) S_I_C_Codes_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(__ST256530_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST256492_Layout __ND5158886__Project(__ST458959_Layout __PP5158528) := TRANSFORM
    __EE5158935 := __PP5158528.Data_Sources_;
    __ST256530_Layout __ND5158940__Project(E_Sele_Phone_Number(__in,__cfg).Data_Sources_Layout __PP5158936) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP5158936.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP5158936.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP5158936.Source_));
      SELF := __PP5158936;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE5158935,__ND5158940__Project(LEFT));
    SELF.Input_Phone_Match_ := FN_Compile(__cfg).FN_Is_Phone_Match(__ECAST(KEL.typ.nstr,__ECAST(KEL.typ.nstr,__PP5158528.Phone_Number_)),__ECAST(KEL.typ.nstr,__PP5158528.B___Inp_Cln_Phone_));
    SELF := __PP5158528;
  END;
  EXPORT __ENH_Sele_Phone_Number_6 := PROJECT(__EE5158527,__ND5158886__Project(LEFT));
END;

...
EXPORT B_Sele_Phone_Number_6(CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED VIRTUAL TYPEOF(E_Sele_Phone_Number(__cfg).__Result) __E_Sele_Phone_Number := E_Sele_Phone_Number(__cfg).__Result;
  SHARED __EE493863 := __E_Sele_Phone_Number;
  SHARED __EE5326321 := __ENH_Business_Sele_7;
  SHARED __ST494007_Layout := RECORD
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__cfg).Data_Sources_Layout) Data_Sources_;
    KEL.typ.nuid UID;
  ...
  END;
  __JC5326327(E_Sele_Phone_Number(__cfg).Layout __EE493863, B_Business_Sele_7(__cfg).__ST256250_Layout __EE5326321) := __EEQP(__EE493863.Legal_,__EE5326321.UID);
  __ST494007_Layout __JT5326327(E_Sele_Phone_Number(__cfg).Layout __l, B_Business_Sele_7(__cfg).__ST256250_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE5326328 := JOIN(__EE493863,__EE5326321,__JC5326327(LEFT,RIGHT),__JT5326327(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST255166_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST255128_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Phone().Typ) Phone_Number_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__cfg).Best_Phone_Details_Layout) Best_Phone_Details_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__cfg).Phone_Details_Layout) Phone_Details_;
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__cfg).S_I_C_Codes_Layout) S_I_C_Codes_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(__ST255166_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST255128_Layout __ND5326687__Project(__ST494007_Layout __PP5326329) := TRANSFORM
    __EE5326736 := __PP5326329.Data_Sources_;
    __ST255166_Layout __ND5326741__Project(E_Sele_Phone_Number(__cfg).Data_Sources_Layout __PP5326737) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP5326737.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP5326737.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP5326737.Source_));
      SELF := __PP5326737;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE5326736,__ND5326741__Project(LEFT));
    SELF.Input_Phone_Match_ := FN_Compile(__cfg).FN_Is_Phone_Match(__ECAST(KEL.typ.nstr,__ECAST(KEL.typ.nstr,__PP5326329.Phone_Number_)),__ECAST(KEL.typ.nstr,__PP5326329.B___Inp_Cln_Phone_));
    SELF := __PP5326329;
  END;
  EXPORT __ENH_Sele_Phone_Number_6 := PROJECT(__EE5326328,__ND5326687__Project(LEFT));
END;

...
EXPORT B_Sele_Phone_Number_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE394278 := __E_Sele_Phone_Number;
  SHARED __EE3562071 := __ENH_Business_Sele_7;
  SHARED __ST394422_Layout := RECORD
  ...
  END;
  __JC3562077(E_Sele_Phone_Number(__in,__cfg).Layout __EE394278, B_Business_Sele_7(__in,__cfg).__ST201962_Layout __EE3562071) := __EEQP(__EE394278.Legal_,__EE3562071.UID);
  __ST394422_Layout __JT3562077(E_Sele_Phone_Number(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST201962_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE3562078 := JOIN(__EE394278,__EE3562071,__JC3562077(LEFT,RIGHT),__JT3562077(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST200868_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST200830_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Phone().Typ) Phone_Number_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Best_Phone_Details_Layout) Best_Phone_Details_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Phone_Details_Layout) Phone_Details_;
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).S_I_C_Codes_Layout) S_I_C_Codes_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(__ST200868_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST200830_Layout __ND3562506__Project(__ST394422_Layout __PP3562079) := TRANSFORM
    __EE3562555 := __PP3562079.Data_Sources_;
    __ST200868_Layout __ND3562560__Project(E_Sele_Phone_Number(__in,__cfg).Data_Sources_Layout __PP3562556) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP3562556.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP3562556.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP3562556.Source_));
      SELF := __PP3562556;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE3562555,__ND3562560__Project(LEFT));
    SELF.Input_Phone_Match_ := FN_Compile(__cfg).FN_Is_Phone_Match(__ECAST(KEL.typ.nstr,__ECAST(KEL.typ.nstr,__PP3562079.Phone_Number_)),__ECAST(KEL.typ.nstr,__PP3562079.B___Inp_Cln_Phone_));
    SELF := __PP3562079;
  END;
  EXPORT __ENH_Sele_Phone_Number_6 := PROJECT(__EE3562078,__ND3562506__Project(LEFT));
END;

...
EXPORT B_Sele_Phone_Number_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE458785 := __E_Sele_Phone_Number;
  SHARED __EE5158490 := __ENH_Business_Sele_7;
  SHARED __ST458929_Layout := RECORD
  ...
  END;
  __JC5158496(E_Sele_Phone_Number(__in,__cfg).Layout __EE458785, B_Business_Sele_7(__in,__cfg).__ST257584_Layout __EE5158490) := __EEQP(__EE458785.Legal_,__EE5158490.UID);
  __ST458929_Layout __JT5158496(E_Sele_Phone_Number(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST257584_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE5158497 := JOIN(__EE458785,__EE5158490,__JC5158496(LEFT,RIGHT),__JT5158496(LEFT,RIGHT),LEFT OUTER,HASH);
  ...
  EXPORT __ST256462_Layout := RECORD
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).S_I_C_Codes_Layout) S_I_C_Codes_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(__ST256500_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST256462_Layout __ND5158856__Project(__ST458929_Layout __PP5158498) := TRANSFORM
    __EE5158905 := __PP5158498.Data_Sources_;
    __ST256500_Layout __ND5158910__Project(E_Sele_Phone_Number(__in,__cfg).Data_Sources_Layout __PP5158906) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP5158906.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP5158906.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP5158906.Source_));
      SELF := __PP5158906;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE5158905,__ND5158910__Project(LEFT));
    SELF.Input_Phone_Match_ := FN_Compile(__cfg).FN_Is_Phone_Match(__ECAST(KEL.typ.nstr,__ECAST(KEL.typ.nstr,__PP5158498.Phone_Number_)),__ECAST(KEL.typ.nstr,__PP5158498.B___Inp_Cln_Phone_));
    SELF := __PP5158498;
  END;
  EXPORT __ENH_Sele_Phone_Number_6 := PROJECT(__EE5158497,__ND5158856__Project(LEFT));
END;

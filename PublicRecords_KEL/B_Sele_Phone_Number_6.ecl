...
EXPORT B_Sele_Phone_Number_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE452342 := __E_Sele_Phone_Number;
  SHARED __EE5126751 := __ENH_Business_Sele_7;
  SHARED __ST452486_Layout := RECORD
  ...
  END;
  __JC5126757(E_Sele_Phone_Number(__in,__cfg).Layout __EE452342, B_Business_Sele_7(__in,__cfg).__ST255522_Layout __EE5126751) := __EEQP(__EE452342.Legal_,__EE5126751.UID);
  __ST452486_Layout __JT5126757(E_Sele_Phone_Number(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST255522_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE5126758 := JOIN(__EE452342,__EE5126751,__JC5126757(LEFT,RIGHT),__JT5126757(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST254438_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST254400_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Phone().Typ) Phone_Number_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Best_Phone_Details_Layout) Best_Phone_Details_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Phone_Details_Layout) Phone_Details_;
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).S_I_C_Codes_Layout) S_I_C_Codes_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(__ST254438_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST254400_Layout __ND5127117__Project(__ST452486_Layout __PP5126759) := TRANSFORM
    __EE5127166 := __PP5126759.Data_Sources_;
    __ST254438_Layout __ND5127171__Project(E_Sele_Phone_Number(__in,__cfg).Data_Sources_Layout __PP5127167) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP5127167.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP5127167.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP5127167.Source_));
      SELF := __PP5127167;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE5127166,__ND5127171__Project(LEFT));
    SELF.Input_Phone_Match_ := FN_Compile(__cfg).FN_Is_Phone_Match(__ECAST(KEL.typ.nstr,__ECAST(KEL.typ.nstr,__PP5126759.Phone_Number_)),__ECAST(KEL.typ.nstr,__PP5126759.B___Inp_Cln_Phone_));
    SELF := __PP5126759;
  END;
  EXPORT __ENH_Sele_Phone_Number_6 := PROJECT(__EE5126758,__ND5127117__Project(LEFT));
END;

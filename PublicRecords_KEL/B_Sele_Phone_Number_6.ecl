...
EXPORT B_Sele_Phone_Number_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE454106 := __E_Sele_Phone_Number;
  SHARED __EE5151163 := __ENH_Business_Sele_7;
  SHARED __ST454250_Layout := RECORD
  ...
  END;
  __JC5151169(E_Sele_Phone_Number(__in,__cfg).Layout __EE454106, B_Business_Sele_7(__in,__cfg).__ST257196_Layout __EE5151163) := __EEQP(__EE454106.Legal_,__EE5151163.UID);
  __ST454250_Layout __JT5151169(E_Sele_Phone_Number(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST257196_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE5151170 := JOIN(__EE454106,__EE5151163,__JC5151169(LEFT,RIGHT),__JT5151169(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST256112_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST256074_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Phone().Typ) Phone_Number_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Best_Phone_Details_Layout) Best_Phone_Details_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Phone_Details_Layout) Phone_Details_;
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).S_I_C_Codes_Layout) S_I_C_Codes_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(__ST256112_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST256074_Layout __ND5151529__Project(__ST454250_Layout __PP5151171) := TRANSFORM
    __EE5151578 := __PP5151171.Data_Sources_;
    __ST256112_Layout __ND5151583__Project(E_Sele_Phone_Number(__in,__cfg).Data_Sources_Layout __PP5151579) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP5151579.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP5151579.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP5151579.Source_));
      SELF := __PP5151579;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE5151578,__ND5151583__Project(LEFT));
    SELF.Input_Phone_Match_ := FN_Compile(__cfg).FN_Is_Phone_Match(__ECAST(KEL.typ.nstr,__ECAST(KEL.typ.nstr,__PP5151171.Phone_Number_)),__ECAST(KEL.typ.nstr,__PP5151171.B___Inp_Cln_Phone_));
    SELF := __PP5151171;
  END;
  EXPORT __ENH_Sele_Phone_Number_6 := PROJECT(__EE5151170,__ND5151529__Project(LEFT));
END;

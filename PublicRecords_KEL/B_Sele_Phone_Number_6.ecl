...
EXPORT B_Sele_Phone_Number_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE507989 := __E_Sele_Phone_Number;
  SHARED __EE5508466 := __ENH_Business_Sele_7;
  SHARED __ST508133_Layout := RECORD
  ...
  END;
  __JC5508472(E_Sele_Phone_Number(__in,__cfg).Layout __EE507989, B_Business_Sele_7(__in,__cfg).__ST261193_Layout __EE5508466) := __EEQP(__EE507989.Legal_,__EE5508466.UID);
  __ST508133_Layout __JT5508472(E_Sele_Phone_Number(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST261193_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE5508473 := JOIN(__EE507989,__EE5508466,__JC5508472(LEFT,RIGHT),__JT5508472(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST260109_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST260071_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_Phone().Typ) Phone_Number_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Best_Phone_Details_Layout) Best_Phone_Details_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).Phone_Details_Layout) Phone_Details_;
    ...
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).S_I_C_Codes_Layout) S_I_C_Codes_;
    KEL.typ.ndataset(E_Sele_Phone_Number(__in,__cfg).N_A_I_C_S_Codes_Layout) N_A_I_C_S_Codes_;
    KEL.typ.ndataset(__ST260109_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST260071_Layout __ND5508832__Project(__ST508133_Layout __PP5508474) := TRANSFORM
    __EE5508881 := __PP5508474.Data_Sources_;
    __ST260109_Layout __ND5508886__Project(E_Sele_Phone_Number(__in,__cfg).Data_Sources_Layout __PP5508882) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP5508882.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP5508882.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP5508882.Source_));
      SELF := __PP5508882;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE5508881,__ND5508886__Project(LEFT));
    SELF.Input_Phone_Match_ := FN_Compile(__cfg).FN_Is_Phone_Match(__ECAST(KEL.typ.nstr,__ECAST(KEL.typ.nstr,__PP5508474.Phone_Number_)),__ECAST(KEL.typ.nstr,__PP5508474.B___Inp_Cln_Phone_));
    SELF := __PP5508474;
  END;
  EXPORT __ENH_Sele_Phone_Number_6 := PROJECT(__EE5508473,__ND5508832__Project(LEFT));
END;

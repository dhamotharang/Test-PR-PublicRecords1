...
EXPORT B_Business_Sele_Overflow_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE461016 := __E_Business_Sele_Overflow;
  EXPORT __ST250955_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST250955_Layout __ND5286292__Project(E_Business_Sele_Overflow(__in,__cfg).Layout __PP460548) := TRANSFORM
    __EE460859 := __PP460548.S_O_S_Incorporation_Details_;
    __BS462393 := __T(__EE460859);
    __EE5286290 := __BS462393(__T(__AND(__AND(__NOT(__NT(__T(__EE460859).S_O_S_Incorporation_State_)),__OP2(__T(__EE460859).S_O_S_Incorporation_State_,<>,__CN(''))),__OP2(__T(__EE460859).S_O_S_Foreign_Domestic_Indicator_,=,__CN('D')))));
    SELF.S_O_S_Domestic_Filing_ := __CN(__EE5286290);
    __EE460890 := __PP460548.S_O_S_Incorporation_Details_;
    __BS462595 := __T(__EE460890);
    __EE5286300 := __BS462595(__T(__AND(__AND(__NOT(__NT(__T(__EE460890).S_O_S_Foreign_State_Code_)),__OP2(__T(__EE460890).S_O_S_Foreign_State_Code_,<>,__CN(''))),__OP2(__T(__EE460890).S_O_S_Foreign_Domestic_Indicator_,=,__CN('F')))));
    SELF.S_O_S_Foreign_Filing_ := __CN(__EE5286300);
    SELF := __PP460548;
  END;
  EXPORT __ENH_Business_Sele_Overflow_6 := PROJECT(__EE461016,__ND5286292__Project(LEFT));
END;

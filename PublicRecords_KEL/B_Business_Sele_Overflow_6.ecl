...
EXPORT B_Business_Sele_Overflow_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE410118 := __E_Business_Sele_Overflow;
  EXPORT __ST244584_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST244584_Layout __ND4849809__Project(E_Business_Sele_Overflow(__in,__cfg).Layout __PP409650) := TRANSFORM
    __EE409961 := __PP409650.S_O_S_Incorporation_Details_;
    __BS411495 := __T(__EE409961);
    __EE4849807 := __BS411495(__T(__AND(__AND(__NOT(__NT(__T(__EE409961).S_O_S_Incorporation_State_)),__OP2(__T(__EE409961).S_O_S_Incorporation_State_,<>,__CN(''))),__OP2(__T(__EE409961).S_O_S_Foreign_Domestic_Indicator_,=,__CN('D')))));
    SELF.S_O_S_Domestic_Filing_ := __CN(__EE4849807);
    __EE409992 := __PP409650.S_O_S_Incorporation_Details_;
    __BS411697 := __T(__EE409992);
    __EE4849817 := __BS411697(__T(__AND(__AND(__NOT(__NT(__T(__EE409992).S_O_S_Foreign_State_Code_)),__OP2(__T(__EE409992).S_O_S_Foreign_State_Code_,<>,__CN(''))),__OP2(__T(__EE409992).S_O_S_Foreign_Domestic_Indicator_,=,__CN('F')))));
    SELF.S_O_S_Foreign_Filing_ := __CN(__EE4849817);
    SELF := __PP409650;
  END;
  EXPORT __ENH_Business_Sele_Overflow_6 := PROJECT(__EE410118,__ND4849809__Project(LEFT));
END;

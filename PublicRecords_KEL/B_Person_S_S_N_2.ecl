...
EXPORT B_Person_S_S_N_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1813208 := __E_Person_S_S_N;
  EXPORT __ST213662_Layout := RECORD
  ...
  END;
  SHARED __ST213662_Layout __ND6908872__Project(E_Person_S_S_N(__in,__cfg).Layout __PP1813114) := TRANSFORM
    __BS1813166 := __T(__PP1813114.Data_Sources_);
    SELF.Is_Best_S_S_N_ := EXISTS(__BS1813166(__T(__OP2(__T(__PP1813114.Data_Sources_).Source_,IN,__CN(['BP','NEN','NEQ'])))));
    SELF := __PP1813114;
  END;
  EXPORT __ENH_Person_S_S_N_2 := PROJECT(__EE1813208,__ND6908872__Project(LEFT));
END;

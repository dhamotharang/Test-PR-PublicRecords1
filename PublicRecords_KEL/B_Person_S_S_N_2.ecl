...
EXPORT B_Person_S_S_N_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1901168 := __E_Person_S_S_N;
  EXPORT __ST216166_Layout := RECORD
  ...
  END;
  SHARED __ST216166_Layout __ND7531551__Project(E_Person_S_S_N(__in,__cfg).Layout __PP1901074) := TRANSFORM
    __BS1901126 := __T(__PP1901074.Data_Sources_);
    SELF.Is_Best_S_S_N_ := EXISTS(__BS1901126(__T(__OP2(__T(__PP1901074.Data_Sources_).Source_,IN,__CN(['BP','NEN','NEQ'])))));
    SELF := __PP1901074;
  END;
  EXPORT __ENH_Person_S_S_N_2 := PROJECT(__EE1901168,__ND7531551__Project(LEFT));
END;

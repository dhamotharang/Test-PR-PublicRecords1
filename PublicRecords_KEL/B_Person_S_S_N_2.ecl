...
EXPORT B_Person_S_S_N_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE1914324 := __E_Person_S_S_N;
  EXPORT __ST217935_Layout := RECORD
  ...
  END;
  SHARED __ST217935_Layout __ND7571986__Project(E_Person_S_S_N(__in,__cfg).Layout __PP1914230) := TRANSFORM
    __BS1914282 := __T(__PP1914230.Data_Sources_);
    SELF.Is_Best_S_S_N_ := EXISTS(__BS1914282(__T(__OP2(__T(__PP1914230.Data_Sources_).Source_,IN,__CN(['BP','NEN','NEQ'])))));
    SELF := __PP1914230;
  END;
  EXPORT __ENH_Person_S_S_N_2 := PROJECT(__EE1914324,__ND7571986__Project(LEFT));
END;

...
EXPORT B_Education_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6767641 := __ENH_Education_3;
  EXPORT __ST209396_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST209396_Layout __ND6767773__Project(B_Education_7(__in,__cfg).__ST255794_Layout __PP6767642) := TRANSFORM
    __BS6767684 := __T(__PP6767642.College_Characteristics_);
    SELF.Coll_Rec_Flag_ := EXISTS(__BS6767684(__T(__OP2(__T(__PP6767642.College_Characteristics_).File_Type_,IN,__CN(['C','H','O'])))));
    SELF := __PP6767642;
  END;
  EXPORT __ENH_Education_2 := PROJECT(__EE6767641,__ND6767773__Project(LEFT));
END;

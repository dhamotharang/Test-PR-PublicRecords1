...
EXPORT B_Education_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7125059 := __ENH_Education_3;
  EXPORT __ST213707_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST213707_Layout __ND7125191__Project(B_Education_7(__in,__cfg).__ST261614_Layout __PP7125060) := TRANSFORM
    __BS7125102 := __T(__PP7125060.College_Characteristics_);
    SELF.Coll_Rec_Flag_ := EXISTS(__BS7125102(__T(__OP2(__T(__PP7125060.College_Characteristics_).File_Type_,IN,__CN(['C','H','O'])))));
    SELF := __PP7125060;
  END;
  EXPORT __ENH_Education_2 := PROJECT(__EE7125059,__ND7125191__Project(LEFT));
END;

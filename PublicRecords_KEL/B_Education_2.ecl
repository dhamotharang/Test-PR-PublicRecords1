...
EXPORT B_Education_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE7206903 := __ENH_Education_3;
  EXPORT __ST216505_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST216505_Layout __ND7207035__Project(B_Education_7(__in,__cfg).__ST264699_Layout __PP7206904) := TRANSFORM
    __BS7206946 := __T(__PP7206904.College_Characteristics_);
    SELF.Coll_Rec_Flag_ := EXISTS(__BS7206946(__T(__OP2(__T(__PP7206904.College_Characteristics_).File_Type_,IN,__CN(['C','H','O'])))));
    SELF := __PP7206904;
  END;
  EXPORT __ENH_Education_2 := PROJECT(__EE7206903,__ND7207035__Project(LEFT));
END;

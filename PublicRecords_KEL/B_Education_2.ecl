...
EXPORT B_Education_2(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6432625 := __ENH_Education_3;
  EXPORT __ST202639_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST202639_Layout __ND6432757__Project(B_Education_7(__in,__cfg).__ST247935_Layout __PP6432626) := TRANSFORM
    __BS6432668 := __T(__PP6432626.College_Characteristics_);
    SELF.Coll_Rec_Flag_ := EXISTS(__BS6432668(__T(__OP2(__T(__PP6432626.College_Characteristics_).File_Type_,IN,__CN(['C','H','O'])))));
    SELF := __PP6432626;
  END;
  EXPORT __ENH_Education_2 := PROJECT(__EE6432625,__ND6432757__Project(LEFT));
END;

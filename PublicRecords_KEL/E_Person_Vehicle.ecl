...
EXPORT E_Person_Vehicle(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  END;
  SHARED __d0_Automobile__Mapped := JOIN(KEL.Intake.AppendNonExistUidComponents(__d0_KELfiltered,'vehicle_key','__in'),E_Vehicle(__in,__cfg).Lookup,TRIM((STRING)LEFT.vehicle_key) = RIGHT.KeyVal,TRANSFORM(__d0_Automobile__Layout,SELF.Automobile_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,HASH);
  ...
  Layout Person_Vehicle__Rollup(InLayout __r, DATASET(InLayout) __recs) := TRANSFORM
    ...
    SELF.Counts_Model_ := __CN(PROJECT(TABLE(__recs,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,FALSE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),Sequence_Key_,History_,History_Source_},Sequence_Key_,History_,History_Source_),Counts_Model_Layout)(__NN(Sequence_Key_) OR __NN(History_) OR __NN(History_Source_)));
  ...
  Layout Person_Vehicle__Single_Rollup(InLayout __r) := TRANSFORM
    ...
    SELF.Counts_Model_ := __CN(PROJECT(DATASET(__r),TRANSFORM(Counts_Model_Layout,SELF.__RecordCount:=1;,SELF.Date_First_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_First_Seen_,FALSE),SELF.Date_Last_Seen_:=KEL.era.SimpleRollSingleRow(LEFT,Date_Last_Seen_,FALSE),SELF:=LEFT))(__NN(Sequence_Key_) OR __NN(History_) OR __NN(History_Source_)));

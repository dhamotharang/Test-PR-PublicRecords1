...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE39655 := __ENH_Criminal_Offense_4;
  EXPORT __ST29872_Layout := RECORD
  ...
  EXPORT __ST29825_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST29872_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST29825_Layout __ND40430__Project(B_Criminal_Offense_4(__in,__cfg).__ST30765_Layout __PP39187) := TRANSFORM
    __EE39422 := __PP39187.Offense_Charges_;
    __ST29872_Layout __ND40415__Project(B_Criminal_Offense_4(__in,__cfg).__ST30812_Layout __PP39886) := TRANSFORM
      SELF.Age_In_Days_ := FN_Compile.FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP39886.Criminal_Date_),__ECAST(KEL.typ.nkdate,__PP39187.Current_Date_));
      SELF.Is_Criminal_Count_ := __NOT(__PP39886.Dismissed_Charges_);
      SELF.Is_Felony_ := __AND(__AND(__NOT(__PP39886.Dismissed_Charges_),IF(__T(__NT(__PP39886.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP39886.Offense_Score_,=,__CN('F'))))),__OR(__OP2(__PP39187.Recent_Offender_Level_,=,__CN('4')),__NT(__PP39187.Recent_Offender_Level_)));
      SELF := __PP39886;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE39422,__ND40415__Project(LEFT));
    SELF := __PP39187;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE39655,__ND40430__Project(LEFT));
END;

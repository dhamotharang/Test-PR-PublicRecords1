...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5809977 := __ENH_Criminal_Offense_4;
  EXPORT __ST227759_Layout := RECORD
  ...
  END;
  EXPORT __ST227712_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST227759_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST227712_Layout __ND5810409__Project(B_Criminal_Offense_4(__in,__cfg).__ST240531_Layout __PP5809978) := TRANSFORM
    __EE5810138 := __PP5809978.Offense_Charges_;
    __ST227759_Layout __ND5810391__Project(B_Criminal_Offense_4(__in,__cfg).__ST240578_Layout __PP5810139) := TRANSFORM
      SELF.Is_Felony_F_C_R_A_ := __AND(__AND(__AND(__AND(__NOT(__PP5810139.Dismissed_Charges_),IF(__T(__NT(__PP5810139.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP5810139.Offense_Score_,=,__CN('F'))))),__NT(__PP5810139.Offender_Level_)),__OP2(__PP5810139.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP5810139.Traffic_Flag_,=,__CN('N')));
      SELF := __PP5810139;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE5810138,__ND5810391__Project(LEFT));
    SELF := __PP5809978;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE5809977,__ND5810409__Project(LEFT));
END;

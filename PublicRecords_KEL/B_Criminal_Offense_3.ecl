...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE33408 := __ENH_Criminal_Offense_4;
  EXPORT __ST26680_Layout := RECORD
  ...
  EXPORT __ST26633_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST26680_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST26633_Layout __ND34183__Project(B_Criminal_Offense_4(__in,__cfg).__ST27413_Layout __PP32940) := TRANSFORM
    __EE33175 := __PP32940.Offense_Charges_;
    __ST26680_Layout __ND34168__Project(B_Criminal_Offense_4(__in,__cfg).__ST27460_Layout __PP33639) := TRANSFORM
      SELF.Age_In_Days_ := FN_Compile.FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP33639.Criminal_Date_),__ECAST(KEL.typ.nkdate,__PP32940.Current_Date_));
      SELF.Is_Criminal_Count_ := __NOT(__PP33639.Dismissed_Charges_);
      SELF.Is_Felony_ := __AND(__AND(__NOT(__PP33639.Dismissed_Charges_),IF(__T(__NT(__PP33639.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP33639.Offense_Score_,=,__CN('F'))))),__OR(__OP2(__PP32940.Recent_Offender_Level_,=,__CN('4')),__NT(__PP32940.Recent_Offender_Level_)));
      SELF := __PP33639;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE33175,__ND34168__Project(LEFT));
    SELF := __PP32940;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE33408,__ND34183__Project(LEFT));
END;

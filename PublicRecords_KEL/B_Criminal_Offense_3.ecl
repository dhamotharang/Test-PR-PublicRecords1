...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE23523 := __ENH_Criminal_Offense_4;
  EXPORT __ST20056_Layout := RECORD
  ...
  END;
  EXPORT __ST20009_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST20056_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST20009_Layout __ND24298__Project(B_Criminal_Offense_4(__in,__cfg).__ST20381_Layout __PP23055) := TRANSFORM
    __EE23290 := __PP23055.Offense_Charges_;
    __ST20056_Layout __ND24283__Project(B_Criminal_Offense_4(__in,__cfg).__ST20428_Layout __PP23754) := TRANSFORM
      SELF.Age_In_Days_ := FN_Compile.FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP23754.Criminal_Date_),__ECAST(KEL.typ.nkdate,__PP23055.Current_Date_));
      SELF.Is_Criminal_Count_ := __NOT(__PP23754.Dismissed_Charges_);
      SELF.Is_Felony_ := __AND(__AND(__NOT(__PP23754.Dismissed_Charges_),IF(__T(__NT(__PP23754.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP23754.Offense_Score_,=,__CN('F'))))),__OR(__OP2(__PP23055.Recent_Offender_Level_,=,__CN(4)),__NT(__PP23055.Recent_Offender_Level_)));
      SELF := __PP23754;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE23290,__ND24283__Project(LEFT));
    SELF := __PP23055;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE23523,__ND24298__Project(LEFT));
END;

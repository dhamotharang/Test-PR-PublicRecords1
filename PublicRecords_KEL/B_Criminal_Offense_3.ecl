...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6176632 := __ENH_Criminal_Offense_4;
  EXPORT __ST237384_Layout := RECORD
  ...
  END;
  EXPORT __ST237337_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST237384_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST237337_Layout __ND6177064__Project(B_Criminal_Offense_4(__in,__cfg).__ST250410_Layout __PP6176633) := TRANSFORM
    __EE6176793 := __PP6176633.Offense_Charges_;
    __ST237384_Layout __ND6177046__Project(B_Criminal_Offense_4(__in,__cfg).__ST250457_Layout __PP6176794) := TRANSFORM
      SELF.Is_Felony_F_C_R_A_ := __AND(__AND(__AND(__AND(__NOT(__PP6176794.Dismissed_Charges_),IF(__T(__NT(__PP6176794.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP6176794.Offense_Score_,=,__CN('F'))))),__NT(__PP6176794.Offender_Level_)),__OP2(__PP6176794.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP6176794.Traffic_Flag_,=,__CN('N')));
      SELF := __PP6176794;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE6176793,__ND6177046__Project(LEFT));
    SELF := __PP6176633;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE6176632,__ND6177064__Project(LEFT));
END;

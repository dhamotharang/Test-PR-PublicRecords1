...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6211288 := __ENH_Criminal_Offense_4;
  EXPORT __ST233217_Layout := RECORD
  ...
  END;
  EXPORT __ST233170_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST233217_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST233170_Layout __ND6211738__Project(B_Criminal_Offense_4(__in,__cfg).__ST246375_Layout __PP6211289) := TRANSFORM
    __EE6211449 := __PP6211289.Offense_Charges_;
    __ST233217_Layout __ND6211717__Project(B_Criminal_Offense_4(__in,__cfg).__ST246422_Layout __PP6211450) := TRANSFORM
      SELF.Is_Felony_F_C_R_A_ := __AND(__AND(__AND(__AND(__NOT(__PP6211450.Dismissed_Charges_),IF(__T(__NT(__PP6211450.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP6211450.Offense_Score_,=,__CN('F'))))),__NT(__PP6211450.Offender_Level_)),__OP2(__PP6211450.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP6211450.Traffic_Flag_,=,__CN('N')));
      SELF := __PP6211450;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE6211449,__ND6211717__Project(LEFT));
    SELF := __PP6211289;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE6211288,__ND6211738__Project(LEFT));
END;

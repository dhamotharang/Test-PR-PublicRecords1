...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE18526 := __ENH_Criminal_Offense_4;
  EXPORT __ST15936_Layout := RECORD
  ...
  END;
  EXPORT __ST15889_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST15936_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST15889_Layout __ND19302__Project(B_Criminal_Offense_4(__in,__cfg).__ST16075_Layout __PP18062) := TRANSFORM
    __EE18297 := __PP18062.Offense_Charges_;
    __ST15936_Layout __ND19287__Project(B_Criminal_Offense_4(__in,__cfg).__ST16122_Layout __PP18757) := TRANSFORM
      SELF.Age_In_Days_ := FN_Compile.FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP18757.Criminal_Date_),__ECAST(KEL.typ.nkdate,__PP18062.Current_Date_));
      SELF.Is_Criminal_Count_ := __NOT(__PP18757.Dismissed_Charges_);
      SELF.Is_Felony_ := __AND(__AND(__OP2(__PP18757.Dismissed_Charges_,=,__CN(FALSE)),IF(__T(__NT(__PP18757.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP18757.Offense_Score_,=,__CN('F'))))),__OP2(__PP18062.Recent_Offender_Level_,=,__CN(4)));
      SELF := __PP18757;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE18297,__ND19287__Project(LEFT));
    SELF := __PP18062;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE18526,__ND19302__Project(LEFT));
END;

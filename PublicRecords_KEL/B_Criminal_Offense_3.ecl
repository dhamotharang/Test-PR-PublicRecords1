...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6152128 := __ENH_Criminal_Offense_4;
  EXPORT __ST230541_Layout := RECORD
  ...
  END;
  EXPORT __ST230494_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST230541_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST230494_Layout __ND6152572__Project(B_Criminal_Offense_4(__in,__cfg).__ST243593_Layout __PP6152129) := TRANSFORM
    __EE6152289 := __PP6152129.Offense_Charges_;
    __ST230541_Layout __ND6152552__Project(B_Criminal_Offense_4(__in,__cfg).__ST243640_Layout __PP6152290) := TRANSFORM
      SELF.Is_Felony_F_C_R_A_ := __AND(__AND(__AND(__AND(__NOT(__PP6152290.Dismissed_Charges_),IF(__T(__NT(__PP6152290.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP6152290.Offense_Score_,=,__CN('F'))))),__NT(__PP6152290.Offender_Level_)),__OP2(__PP6152290.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP6152290.Traffic_Flag_,=,__CN('N')));
      SELF := __PP6152290;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE6152289,__ND6152552__Project(LEFT));
    SELF := __PP6152129;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE6152128,__ND6152572__Project(LEFT));
END;

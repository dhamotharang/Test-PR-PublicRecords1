...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE31107 := __ENH_Criminal_Offense_4;
  EXPORT __ST25151_Layout := RECORD
  ...
  END;
  EXPORT __ST25104_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST25151_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST25104_Layout __ND31882__Project(B_Criminal_Offense_4(__in,__cfg).__ST25785_Layout __PP30639) := TRANSFORM
    __EE30874 := __PP30639.Offense_Charges_;
    __ST25151_Layout __ND31867__Project(B_Criminal_Offense_4(__in,__cfg).__ST25832_Layout __PP31338) := TRANSFORM
      SELF.Age_In_Days_ := FN_Compile.FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP31338.Criminal_Date_),__ECAST(KEL.typ.nkdate,__PP30639.Current_Date_));
      SELF.Is_Criminal_Count_ := __NOT(__PP31338.Dismissed_Charges_);
      SELF.Is_Felony_ := __AND(__AND(__NOT(__PP31338.Dismissed_Charges_),IF(__T(__NT(__PP31338.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP31338.Offense_Score_,=,__CN('F'))))),__OR(__OP2(__PP30639.Recent_Offender_Level_,=,__CN('4')),__NT(__PP30639.Recent_Offender_Level_)));
      SELF := __PP31338;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE30874,__ND31867__Project(LEFT));
    SELF := __PP30639;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE31107,__ND31882__Project(LEFT));
END;

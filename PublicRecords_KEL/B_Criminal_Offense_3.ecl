...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE42481 := __ENH_Criminal_Offense_4;
  EXPORT __ST30301_Layout := RECORD
  ...
  END;
  EXPORT __ST30254_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST30301_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST30254_Layout __ND43256__Project(B_Criminal_Offense_4(__in,__cfg).__ST31391_Layout __PP42013) := TRANSFORM
    __EE42248 := __PP42013.Offense_Charges_;
    __ST30301_Layout __ND43241__Project(B_Criminal_Offense_4(__in,__cfg).__ST31438_Layout __PP42712) := TRANSFORM
      SELF.Age_In_Days_ := FN_Compile.FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP42712.Criminal_Date_),__ECAST(KEL.typ.nkdate,__PP42013.Current_Date_));
      SELF.Is_Criminal_Count_ := __NOT(__PP42712.Dismissed_Charges_);
      SELF.Is_Felony_ := __AND(__AND(__NOT(__PP42712.Dismissed_Charges_),IF(__T(__NT(__PP42712.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP42712.Offense_Score_,=,__CN('F'))))),__OR(__OP2(__PP42013.Recent_Offender_Level_,=,__CN('4')),__NT(__PP42013.Recent_Offender_Level_)));
      SELF := __PP42712;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE42248,__ND43241__Project(LEFT));
    SELF := __PP42013;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE42481,__ND43256__Project(LEFT));
END;

...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5802203 := __ENH_Criminal_Offense_4;
  EXPORT __ST227400_Layout := RECORD
  ...
  END;
  EXPORT __ST227353_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST227400_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST227353_Layout __ND5802635__Project(B_Criminal_Offense_4(__in,__cfg).__ST240146_Layout __PP5802204) := TRANSFORM
    __EE5802364 := __PP5802204.Offense_Charges_;
    __ST227400_Layout __ND5802617__Project(B_Criminal_Offense_4(__in,__cfg).__ST240193_Layout __PP5802365) := TRANSFORM
      SELF.Is_Felony_F_C_R_A_ := __AND(__AND(__AND(__AND(__NOT(__PP5802365.Dismissed_Charges_),IF(__T(__NT(__PP5802365.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP5802365.Offense_Score_,=,__CN('F'))))),__NT(__PP5802365.Offender_Level_)),__OP2(__PP5802365.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP5802365.Traffic_Flag_,=,__CN('N')));
      SELF := __PP5802365;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE5802364,__ND5802617__Project(LEFT));
    SELF := __PP5802204;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE5802203,__ND5802635__Project(LEFT));
END;

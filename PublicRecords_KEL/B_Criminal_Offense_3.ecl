...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6238006 := __ENH_Criminal_Offense_4;
  EXPORT __ST238651_Layout := RECORD
  ...
  END;
  EXPORT __ST238604_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST238651_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST238604_Layout __ND6238438__Project(B_Criminal_Offense_4(__in,__cfg).__ST251734_Layout __PP6238007) := TRANSFORM
    __EE6238167 := __PP6238007.Offense_Charges_;
    __ST238651_Layout __ND6238420__Project(B_Criminal_Offense_4(__in,__cfg).__ST251781_Layout __PP6238168) := TRANSFORM
      SELF.Is_Felony_F_C_R_A_ := __AND(__AND(__AND(__AND(__NOT(__PP6238168.Dismissed_Charges_),IF(__T(__NT(__PP6238168.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP6238168.Offense_Score_,=,__CN('F'))))),__NT(__PP6238168.Offender_Level_)),__OP2(__PP6238168.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP6238168.Traffic_Flag_,=,__CN('N')));
      SELF := __PP6238168;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE6238167,__ND6238420__Project(LEFT));
    SELF := __PP6238007;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE6238006,__ND6238438__Project(LEFT));
END;

...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6153714 := __ENH_Criminal_Offense_4;
  EXPORT __ST230511_Layout := RECORD
  ...
  END;
  EXPORT __ST230464_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST230511_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST230464_Layout __ND6154158__Project(B_Criminal_Offense_4(__in,__cfg).__ST243563_Layout __PP6153715) := TRANSFORM
    __EE6153875 := __PP6153715.Offense_Charges_;
    __ST230511_Layout __ND6154138__Project(B_Criminal_Offense_4(__in,__cfg).__ST243610_Layout __PP6153876) := TRANSFORM
      SELF.Is_Felony_F_C_R_A_ := __AND(__AND(__AND(__AND(__NOT(__PP6153876.Dismissed_Charges_),IF(__T(__NT(__PP6153876.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP6153876.Offense_Score_,=,__CN('F'))))),__NT(__PP6153876.Offender_Level_)),__OP2(__PP6153876.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP6153876.Traffic_Flag_,=,__CN('N')));
      SELF := __PP6153876;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE6153875,__ND6154138__Project(LEFT));
    SELF := __PP6153715;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE6153714,__ND6154158__Project(LEFT));
END;

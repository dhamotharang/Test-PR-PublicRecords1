...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6231655 := __ENH_Criminal_Offense_4;
  EXPORT __ST233519_Layout := RECORD
  ...
  END;
  EXPORT __ST233472_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST233519_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST233472_Layout __ND6232105__Project(B_Criminal_Offense_4(__in,__cfg).__ST246677_Layout __PP6231656) := TRANSFORM
    __EE6231816 := __PP6231656.Offense_Charges_;
    __ST233519_Layout __ND6232084__Project(B_Criminal_Offense_4(__in,__cfg).__ST246724_Layout __PP6231817) := TRANSFORM
      SELF.Is_Felony_F_C_R_A_ := __AND(__AND(__AND(__AND(__NOT(__PP6231817.Dismissed_Charges_),IF(__T(__NT(__PP6231817.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP6231817.Offense_Score_,=,__CN('F'))))),__NT(__PP6231817.Offender_Level_)),__OP2(__PP6231817.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP6231817.Traffic_Flag_,=,__CN('N')));
      SELF := __PP6231817;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE6231816,__ND6232084__Project(LEFT));
    SELF := __PP6231656;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE6231655,__ND6232105__Project(LEFT));
END;

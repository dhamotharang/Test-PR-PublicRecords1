...
EXPORT B_Criminal_Offense_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE6358351 := __ENH_Criminal_Offense_4;
  EXPORT __ST236401_Layout := RECORD
  ...
  END;
  EXPORT __ST236354_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST236401_Layout) Offense_Charges_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST236354_Layout __ND6358801__Project(B_Criminal_Offense_4(__in,__cfg).__ST249563_Layout __PP6358352) := TRANSFORM
    __EE6358512 := __PP6358352.Offense_Charges_;
    __ST236401_Layout __ND6358780__Project(B_Criminal_Offense_4(__in,__cfg).__ST249610_Layout __PP6358513) := TRANSFORM
      SELF.Is_Felony_F_C_R_A_ := __AND(__AND(__AND(__AND(__NOT(__PP6358513.Dismissed_Charges_),IF(__T(__NT(__PP6358513.Offense_Score_)),__ECAST(KEL.typ.nbool,__CN(FALSE)),__ECAST(KEL.typ.nbool,__OP2(__PP6358513.Offense_Score_,=,__CN('F'))))),__NT(__PP6358513.Offender_Level_)),__OP2(__PP6358513.Conviction_Flag_,IN,__CN(['Y','D']))),__OP2(__PP6358513.Traffic_Flag_,=,__CN('N')));
      SELF := __PP6358513;
    END;
    SELF.Offense_Charges_ := __PROJECT(__EE6358512,__ND6358780__Project(LEFT));
    SELF := __PP6358352;
  END;
  EXPORT __ENH_Criminal_Offense_3 := PROJECT(__EE6358351,__ND6358801__Project(LEFT));
END;

...
EXPORT B_Person_Inquiry_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE5445025 := __ENH_Person_Inquiry_8;
  EXPORT __ST441179_Layout := RECORD
  ...
  EXPORT __ST263203_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST263203_Layout __ND5445030__Project(B_Person_Inquiry_8(__in,__cfg).__ST265956_Layout __PP5445026) := TRANSFORM
    __EE5445050 := __PP5445026.Gather_Inquiries_;
    __ST441179_Layout __ND5445055__Project(B_Person_Inquiry_8(__in,__cfg).__ST95717_Layout __PP5445051) := TRANSFORM
      SELF.Exclude_Function_Description_ := KEL.Routines.ToUpperCase(TRIM(__PP5445051.Function_Description_)) <> 'BANKO BATCH';
      __CC34720 := ['CARDS - SUBPRIME','MSB','REFUND ANTICIPATION LOAN','RENT TO OWN','PAYDAY LOANS','PAY DAY LOANS','TITLE LOANS','PAY DAY','PAYDAY'];
      SELF.Is_High_Risk_ := KEL.Routines.ToUpperCase(TRIM(__PP5445051.Industry_)) IN __CC34720;
      SELF := __PP5445051;
    END;
    SELF.Gather_Inquiries_ := __PROJECT(__EE5445050,__ND5445055__Project(LEFT));
    SELF := __PP5445026;
  END;
  EXPORT __ENH_Person_Inquiry_7 := PROJECT(__EE5445025,__ND5445030__Project(LEFT));
END;

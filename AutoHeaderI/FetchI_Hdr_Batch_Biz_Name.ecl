...
export FetchI_Hdr_Batch_Biz_Name(
	dataset(Layouts.Fetch_Hdr_Biz_Working_Layout) indata) := function

	return denormalize(indata,Business_Header.RoxieKeys().NewFetch.Key_Name_QA,
		...
		left.city_value = '' and left.state_value = '' and
		left.company_name_value <> '' and
		JoinConditions.CNAME_DIAL_KEYED() and
		JoinConditions.FEIN_OPT_KEYED(),
		group,

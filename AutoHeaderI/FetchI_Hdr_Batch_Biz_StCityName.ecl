...
export FetchI_Hdr_Batch_Biz_StCityName(
	dataset(Layouts.Fetch_Hdr_Biz_Working_Layout) indata) := function

	return denormalize(indata,Business_Header.RoxieKeys().NewFetch.Key_StCityName_QA,
		...
		JoinConditions.NEGATIVE_STNAME() and
		left.company_name_value != '' and
		JoinConditions.CITY_KEYED() and
		JoinConditions.STATE_OPT_KEYED() and
		JoinConditions.CNAME_DIAL_KEYED(),
		group,

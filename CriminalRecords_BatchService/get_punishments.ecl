...
export get_punishments(DATASET(Assorted_Layouts.key) ofdkey_field = DATASET([],Assorted_Layouts.Key)) := FUNCTION
  //Punishment
	punish := doxie_files.key_punishment;
	
  pun_field_3 := JOIN(ofdkey_field,punish,KEYED(LEFT.offender_key = RIGHT.ok));
 ...
 // OUTPUT(pun_field_2);											
  pun_field_1  := GROUP(SORT(pun_field_2
	...
	CriminalRecords_BatchService.Assorted_Layouts.Layout_out xfm_punishment(recordof(pun_field_1) L, DATASET(recordof(pun_field_1)) R) := TRANSFORM

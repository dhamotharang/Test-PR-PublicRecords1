...
EXPORT Mod_Corps(BOOLEAN MultiRec = FALSE,SET OF SALT29.UIDType ButNot=[]) := MODULE
 
  infile := File_Corp2_Base;
  R := RECORD
    infile;
...
// Create the mappings and links for data in the main external file
  Process_Biz_layouts.InputLayout tr(Process_Biz_layouts.InputLayout le) := TRANSFORM,SKIP(~Process_Biz_Layouts.HardKeyMatch(le))
    SELF := le;
  END;
EXPORT Mapped := PROJECT(Mapped0,tr(LEFT)) : PERSIST('~thor_data400::persist::proxid::BIPV2_WAF::Mapped::UID::Corps',EXPIRE(30));
...
EXPORT PayloadByproxid := INDEX(T,{Fetch_ultid,Fetch_orgid,Fetch_seleid,Fetch_proxid,Fetch_rcid},{T},'~thor_data400::key::BIPV2_WAF::proxid::Corpsbyproxid::Payload');

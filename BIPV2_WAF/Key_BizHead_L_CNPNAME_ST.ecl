...
DataForKey0 := DEDUP(SORT(TABLE(h(cnp_name NOT IN SET(s.nulls_cnp_name,cnp_name),st NOT IN SET(s.nulls_st,st)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,proxid,cnp_name,st,zip,prim_name,p_city_name,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT RawFetch(TYPEOF(h.cnp_name) param_cnp_name,TYPEOF(h.st) param_st,SET OF TYPEOF(h.zip) param_zip,TYPEOF(h.prim_name) param_prim_name,TYPEOF(h.p_city_name) param_p_city_name) := 
 ...
 //Generate service attributes for GSS join
    ...
    doIndexRead(UNSIGNED4 search,UNSIGNED2 spc) := STEPPED(KEY( GSS_hash = search AND (GSS_bloom & BloomF) = BloomF
      ...
      AND ( p_city_name = (TYPEOF(p_city_name))'' OR param_p_city_name = (TYPEOF(p_city_name))'' OR SALT29.WithinEditN(p_city_name,param_p_city_name,1, 0)  )),ultid,orgid,seleid,proxid,PRIORITY(40-spc)); // Filter for each row of index fetch
    ...
    res := JOIN( steppedmatches, Key, RIGHT.GSS_Hash = wds[1].hsh
      AND ( RIGHT.st = param_st AND param_st <> (TYPEOF(RIGHT.st))'' ) AND LEFT.proxid = RIGHT.proxid AND LEFT.seleid = RIGHT.seleid AND LEFT.orgid = RIGHT.orgid AND LEFT.ultid = RIGHT.ultid,TRANSFORM(indexOutputRecord,SELF.gss_word_weight := LEFT.gss_word_weight,SELF := RIGHT));
...
EXPORT ScoredproxidFetch(TYPEOF(h.cnp_name) param_cnp_name,TYPEOF(h.st) param_st,SET OF TYPEOF(h.zip) param_zip,TYPEOF(h.prim_name) param_prim_name,TYPEOF(h.p_city_name) param_p_city_name,TYPEOF(h.company_sic_code1) param_company_sic_code1,TYPEOF(h.cnp_number) param_cnp_number,TYPEOF(h.cnp_btype) param_cnp_btype,TYPEOF(h.cnp_lowv) param_cnp_lowv,TYPEOF(h.prim_range) param_prim_range,TYPEOF(h.sec_range) param_sec_range) := FUNCTION
  RawData := RawFetch(param_cnp_name,param_st,param_zip,param_prim_name,param_p_city_name);
  ...
  SELF.cnp_name_gss_cases := DATASET([{le.gss_hash}],SALT29.layout_GSS_cases);
    SELF.st_match_code := MAP(
		le.st = (TYPEOF(le.st))'' OR le.st = (TYPEOF(le.st))'' => SALT29.MatchCode.OneSideNull,match_methods(File_BizHead).match_st(le.st,param_st,TRUE));
    ...
    SELF.Weight :=MAX(0,SELF.cnp_nameWeight) + MAX(0,SELF.stWeight) + MAX(0,SELF.zipWeight) + MAX(0,SELF.prim_nameWeight) + MAX(0,SELF.p_city_nameWeight) + MAX(0,SELF.company_sic_code1Weight) + MAX(0,SELF.cnp_numberWeight) + MAX(0,SELF.cnp_btypeWeight) + MAX(0,SELF.cnp_lowvWeight) + MAX(0,SELF.prim_rangeWeight) + MAX(0,SELF.sec_rangeWeight);
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.st_match_code := MAP(
		le.st = (TYPEOF(le.st))'' OR le.st = (TYPEOF(le.st))'' => SALT29.MatchCode.OneSideNull,match_methods(File_BizHead).match_st(le.st,ri.st,TRUE));
  ...
  J0 := JOIN(Recs1,Key,HASH32(SALT29.fn_bow_bestword(LEFT.cnp_name))=RIGHT.gss_hash AND SALT29.MatchBagOfWords(LEFT.cnp_name,RIGHT.cnp_name,31744,0) > Config.cnp_name_Force * 100
     ...
     AND ( LEFT.p_city_name = (TYPEOF(LEFT.p_city_name))'' OR RIGHT.p_city_name = (TYPEOF(RIGHT.p_city_name))'' OR SALT29.WithinEditN(LEFT.p_city_name,RIGHT.p_city_name,1, 0)  ),Score_Batch(RIGHT,LEFT),
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_cnp_name='',Input_st='',Input_zip='',Input_prim_name='',Input_p_city_name='',Input_company_sic_code1='',Input_cnp_number='',Input_cnp_btype='',Input_cnp_lowv='',Input_prim_range='',Input_sec_range='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,BIPV2_WAF;
#IF(#TEXT(Input_cnp_name)<>'' AND #TEXT(Input_st)<>'')
  #uniquename(trans)
  BIPV2_WAF.Key_BizHead_L_CNPNAME_ST.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := BIPV2_WAF.Key_BizHead_L_CNPNAME_ST.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

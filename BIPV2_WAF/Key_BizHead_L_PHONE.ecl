...
EXPORT KeyName := data_services.data_location.prefix('bipv2') +'key::BIPV2_WAF::proxid::Refs::L_PHONE';
...
//Scores for various field components
  h.company_phone_weight100 ; // Contains 100x the specificity
...
DataForKey0 := DEDUP(SORT(TABLE(h(company_phone NOT IN SET(s.nulls_company_phone,company_phone)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
 
EXPORT Key := INDEX(DataForKey0,,KeyName);
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,proxid,company_phone,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'L_PHONE','company_sic_code1',CntRed_company_sic_code1*100,CntRed_company_sic_code1*TSize},{'L_PHONE','cnp_name',CntRed_cnp_name*100,CntRed_cnp_name*TSize},{'L_PHONE','cnp_number',CntRed_cnp_number*100,CntRed_cnp_number*TSize},{'L_PHONE','cnp_btype',CntRed_cnp_btype*100,CntRed_cnp_btype*TSize},{'L_PHONE','cnp_lowv',CntRed_cnp_lowv*100,CntRed_cnp_lowv*TSize},{'L_PHONE','zip',CntRed_zip*100,CntRed_zip*TSize},{'L_PHONE','prim_name',CntRed_prim_name*100,CntRed_prim_name*TSize},{'L_PHONE','p_city_name',CntRed_p_city_name*100,CntRed_p_city_name*TSize},{'L_PHONE','st',CntRed_st*100,CntRed_st*TSize},{'L_PHONE','prim_range',CntRed_prim_range*100,CntRed_prim_range*TSize},{'L_PHONE','sec_range',CntRed_sec_range*100,CntRed_sec_range*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.company_phone <> (typeof(le.company_phone))'';
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch(TYPEOF(h.company_phone) param_company_phone) := 
    STEPPED( LIMIT( Key(
          ( company_phone = param_company_phone AND param_company_phone <> (TYPEOF(company_phone))'' )),10000,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),ultid,orgid,seleid,proxid);
 
 
EXPORT ScoredproxidFetch(TYPEOF(h.company_phone) param_company_phone,TYPEOF(h.company_sic_code1) param_company_sic_code1,TYPEOF(h.cnp_name) param_cnp_name,TYPEOF(h.cnp_number) param_cnp_number,TYPEOF(h.cnp_btype) param_cnp_btype,TYPEOF(h.cnp_lowv) param_cnp_lowv,SET OF TYPEOF(h.zip) param_zip,TYPEOF(h.prim_name) param_prim_name,TYPEOF(h.p_city_name) param_p_city_name,TYPEOF(h.st) param_st,TYPEOF(h.prim_range) param_prim_range,TYPEOF(h.sec_range) param_sec_range) := FUNCTION
  RawData := RawFetch(param_company_phone);
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.company_phone_match_code := MAP(
		le.company_phone = (TYPEOF(le.company_phone))'' OR le.company_phone = (TYPEOF(le.company_phone))'' => SALT29.MatchCode.OneSideNull,match_methods(File_BizHead).match_company_phone(le.company_phone,param_company_phone,TRUE));
    SELF.company_phoneWeight := (50+MAP ( le.company_phone = param_company_phone  => le.company_phone_weight100,
          le.company_phone = (TYPEOF(le.company_phone))'' OR param_company_phone = (TYPEOF(le.company_phone))'' => 0,
           -1.000*le.company_phone_weight100))/100; 
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.company_phone_match_code := MAP(
		le.company_phone = (TYPEOF(le.company_phone))'' OR le.company_phone = (TYPEOF(le.company_phone))'' => SALT29.MatchCode.OneSideNull,match_methods(File_BizHead).match_company_phone(le.company_phone,ri.company_phone,TRUE));
    SELF.company_phoneWeight := (50+MAP ( le.company_phone = ri.company_phone  => le.company_phone_weight100,
          le.company_phone = (TYPEOF(le.company_phone))'' OR ri.company_phone = (TYPEOF(le.company_phone))'' => 0,
           -1.000*le.company_phone_weight100))/100; 
    SELF.company_sic_code1_match_code := MAP(
    ...
    SELF := le;
  END;
  Recs0 := Recs(company_phone <> (typeof(company_phone))'');
  ...
  J0 := JOIN(Recs1,Key,LEFT.company_phone = RIGHT.company_phone,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.company_phone = RIGHT.company_phone,10000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.company_phone = RIGHT.company_phone,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.company_phone = RIGHT.company_phone,10000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_company_phone='',Input_company_sic_code1='',Input_cnp_name='',Input_cnp_number='',Input_cnp_btype='',Input_cnp_lowv='',Input_zip='',Input_prim_name='',Input_p_city_name='',Input_st='',Input_prim_range='',Input_sec_range='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,BIPV2_WAF;
#IF(#TEXT(Input_company_phone)<>'')
  #uniquename(trans)
  BIPV2_WAF.Key_BizHead_L_PHONE.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := BIPV2_WAF.Key_BizHead_L_PHONE.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

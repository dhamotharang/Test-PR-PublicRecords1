...
EXPORT KeyName := data_services.data_location.prefix('bipv2') +'key::BIPV2_WAF::proxid::Refs::L_ADDRESS1';
...
DataForKey0 := DEDUP(SORT(TABLE(h(prim_name NOT IN SET(s.nulls_prim_name,prim_name),p_city_name NOT IN SET(s.nulls_p_city_name,p_city_name),st NOT IN SET(s.nulls_st,st)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
 
EXPORT Key := INDEX(DataForKey0,,KeyName);
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,proxid,prim_name,p_city_name,st,prim_range,cnp_name,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'L_ADDRESS1','zip',CntRed_zip*100,CntRed_zip*TSize},{'L_ADDRESS1','company_sic_code1',CntRed_company_sic_code1*100,CntRed_company_sic_code1*TSize},{'L_ADDRESS1','cnp_number',CntRed_cnp_number*100,CntRed_cnp_number*TSize},{'L_ADDRESS1','cnp_btype',CntRed_cnp_btype*100,CntRed_cnp_btype*TSize},{'L_ADDRESS1','cnp_lowv',CntRed_cnp_lowv*100,CntRed_cnp_lowv*TSize},{'L_ADDRESS1','sec_range',CntRed_sec_range*100,CntRed_sec_range*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.prim_name <> (typeof(le.prim_name))'' AND le.p_city_name <> (typeof(le.p_city_name))'' AND le.st <> (typeof(le.st))'';
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch(TYPEOF(h.prim_name) param_prim_name,TYPEOF(h.p_city_name) param_p_city_name,TYPEOF(h.st) param_st,TYPEOF(h.prim_range) param_prim_range,TYPEOF(h.cnp_name) param_cnp_name) := 
    STEPPED( LIMIT( Key(
          ( prim_name = param_prim_name AND param_prim_name <> (TYPEOF(prim_name))'' )
      AND ( p_city_name = param_p_city_name AND param_p_city_name <> (TYPEOF(p_city_name))'' )
...
EXPORT ScoredproxidFetch(TYPEOF(h.prim_name) param_prim_name,TYPEOF(h.p_city_name) param_p_city_name,TYPEOF(h.st) param_st,TYPEOF(h.prim_range) param_prim_range,TYPEOF(h.cnp_name) param_cnp_name,SET OF TYPEOF(h.zip) param_zip,TYPEOF(h.company_sic_code1) param_company_sic_code1,TYPEOF(h.cnp_number) param_cnp_number,TYPEOF(h.cnp_btype) param_cnp_btype,TYPEOF(h.cnp_lowv) param_cnp_lowv,TYPEOF(h.sec_range) param_sec_range) := FUNCTION
  RawData := RawFetch(param_prim_name,param_p_city_name,param_st,param_prim_range,param_cnp_name);
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.p_city_name_match_code := MAP(
		le.p_city_name = (TYPEOF(le.p_city_name))'' OR le.p_city_name = (TYPEOF(le.p_city_name))'' => SALT29.MatchCode.OneSideNull,match_methods(File_BizHead).match_p_city_name(le.p_city_name,param_p_city_name,TRUE));
    ...
    SELF.Weight :=MAX(0,SELF.prim_nameWeight) + MAX(0,SELF.p_city_nameWeight) + MAX(0,SELF.stWeight) + MAX(0,SELF.prim_rangeWeight) + MAX(0,SELF.cnp_nameWeight) + MAX(0,SELF.zipWeight) + MAX(0,SELF.company_sic_code1Weight) + MAX(0,SELF.cnp_numberWeight) + MAX(0,SELF.cnp_btypeWeight) + MAX(0,SELF.cnp_lowvWeight) + MAX(0,SELF.sec_rangeWeight);
    SELF := le;
  END;
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.prim_nameWeight := (50+MAP ( le.prim_name = ri.prim_name  => le.prim_name_weight100,
          le.prim_name = (TYPEOF(le.prim_name))'' OR ri.prim_name = (TYPEOF(le.prim_name))'' => 0,
           -1.000*le.prim_name_weight100))/100; 
    SELF.p_city_name_match_code := MAP(
		le.p_city_name = (TYPEOF(le.p_city_name))'' OR le.p_city_name = (TYPEOF(le.p_city_name))'' => SALT29.MatchCode.OneSideNull,match_methods(File_BizHead).match_p_city_name(le.p_city_name,ri.p_city_name,TRUE));
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_prim_name='',Input_p_city_name='',Input_st='',Input_prim_range='',Input_cnp_name='',Input_zip='',Input_company_sic_code1='',Input_cnp_number='',Input_cnp_btype='',Input_cnp_lowv='',Input_sec_range='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,BIPV2_WAF;
#IF(#TEXT(Input_prim_name)<>'' AND #TEXT(Input_p_city_name)<>'' AND #TEXT(Input_st)<>'')
  #uniquename(trans)
  BIPV2_WAF.Key_BizHead_L_ADDRESS1.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := BIPV2_WAF.Key_BizHead_L_ADDRESS1.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

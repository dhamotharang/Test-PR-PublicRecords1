...
<part name="MaxCorps" type="unsignedInt"/>
<part name="NoCorps" type="xsd:boolean"/>
<part name="MaxUCC" type="unsignedInt"/>
<part name="NoUCC" type="xsd:boolean"/>
<part name="MaxVehicle" type="unsignedInt"/>
<part name="NoVehicle" type="xsd:boolean"/>
<part name="MaxPropertyV2" type="unsignedInt"/>
<part name="NoPropertyV2" type="xsd:boolean"/>
<part name="MaxBizContacts" type="unsignedInt"/>
<part name="NoBizContacts" type="xsd:boolean"/>
...
// Options to override the data-bomb fetch
  UNSIGNED4 s_MaxCorps := 0 : STORED('MaxCorps');
  BOOLEAN s_NoCorps := FALSE : STORED('NoCorps');
  UNSIGNED4 s_MaxUCC := 0 : STORED('MaxUCC');
  BOOLEAN s_NoUCC := FALSE : STORED('NoUCC');
  UNSIGNED4 s_MaxVehicle := 0 : STORED('MaxVehicle');
  BOOLEAN s_NoVehicle := FALSE : STORED('NoVehicle');
  UNSIGNED4 s_MaxPropertyV2 := 0 : STORED('MaxPropertyV2');
  BOOLEAN s_NoPropertyV2 := FALSE : STORED('NoPropertyV2');
  UNSIGNED4 s_MaxBizContacts := 0 : STORED('MaxBizContacts');
  BOOLEAN s_NoBizContacts := FALSE : STORED('NoBizContacts');
  ...
  pm := BIPV2_WAF.MEOW_Biz(Input_Data); // This module performs regular xproxid functions
  Options := MODULE(BIPV2_WAF.Externals.FetchParams) // Interface to pass parameters to fetch
    EXPORT SET OF INTEGER2 ToFetch := [] + IF ( s_NoCorps, [], [BIPV2_WAF.Ext_Layouts.Id_Corps] ) + IF ( s_NoUCC, [], [BIPV2_WAF.Ext_Layouts.Id_UCC] ) + IF ( s_NoVehicle, [], [BIPV2_WAF.Ext_Layouts.Id_Vehicle] ) + IF ( s_NoPropertyV2, [], [BIPV2_WAF.Ext_Layouts.Id_PropertyV2] ) + IF ( s_NoBizContacts, [], [BIPV2_WAF.Ext_Layouts.Id_BizContacts] );
    EXPORT MaxCorps := s_MaxCorps;
    EXPORT MaxUCC := s_MaxUCC;
    EXPORT MaxVehicle := s_MaxVehicle;
    EXPORT MaxPropertyV2 := s_MaxPropertyV2;
    EXPORT MaxBizContacts := s_MaxBizContacts;
  END;
  e := BIPV2_WAF.Externals.Fetch(pm.Uid_Results,Options); // Perform data-bomb fetch

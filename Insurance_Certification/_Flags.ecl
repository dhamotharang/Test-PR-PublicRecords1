...
export _Flags(string pInput_Name='', string pBase_Name='')  := module
	...
	export ExistCurrentSprayed			:= nothor(fileservices.superfileexists(filenames(pInput_Name,).input.using)) and 
																		 count(nothor(fileservices.superfilecontents(filenames(pInput_Name,).input.using  ))) > 0;

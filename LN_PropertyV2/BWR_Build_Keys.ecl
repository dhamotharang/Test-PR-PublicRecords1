EXPORT BWR_Build_Keys := module

//Initial Creation on FCRA keys for Project July
//key_addl_legal_fid
layout_legal := record
...
infile_addllegal := pull(ln_propertyv2.key_addl_legal_fid()(ln_fares_id[1] !='R'));
fcra_addllegal := PROJECT(infile_addllegal, TRANSFORM(layout_legal,SELF := LEFT; self := []));
legal_index_fcra := INDEX(fcra_addllegal,{ln_fares_id},{fcra_addllegal},'~thor_data400::key::ln_propertyv2::fcra::20121109b::addllegal.fid');
export bld_fcra_legal := BUILDINDEX(legal_index_fcra);

 
 layout_addr_search :=  RECORD
  string28 prim_name;
  string10 prim_range;
  string5 zip;
  string2 predir;
 ...
 infile_addr_srch_fcra := pull(ln_propertyv2.Key_Addr_Fid_FCRA);
 fcra_addr_search := PROJECT( infile_addr_srch_fcra, TRANSFORM(layout_addr_search,SELF := LEFT; self := []));
 addr_fid_index_fcra := INDEX(fcra_addr_search,{prim_name,prim_range,zip,predir,postdir,suffix,sec_range,source_code_2, LN_owner,owner,nofares_owner, source_code_1},
														{ln_fares_id, lname, fname, name_suffix},
                            '~thor_data400::key::ln_propertyv2::fcra::20121109b::addr_search.fid');
export bld_addr_fid_fcra := BUILDINDEX(addr_fid_index_fcra);
...
infile_assessor_fcra := pull(ln_propertyv2.Key_Assessor_FID_FCRA);
fcra_assessor_fid := PROJECT( infile_assessor_fcra, TRANSFORM(layout_assessor,SELF := LEFT; self := []));
assessor_fid_index_fcra := INDEX(fcra_assessor_fid,{ln_fares_id, proc_date},{fcra_assessor_fid},'~thor_data400::key::ln_propertyv2::fcra::20121109b::assessor.fid');
export bld_assessor_fid_fcra := BUILDINDEX(assessor_fid_index_fcra);
...
infile_deed_fid_fcra :=pull(ln_propertyv2.Key_Deed_FID_FCRA);
fcra_deed_fid := PROJECT(infile_deed_fid_fcra, TRANSFORM(layout_deed_fid,SELF := LEFT; self := []));
deed_fid_index_fcra := INDEX(fcra_deed_fid,{ln_fares_id, proc_date},{fcra_deed_fid},'~thor_data400::key::ln_propertyv2::fcra::20121109b::deed.fid');
export bld_deed_fid_fcra := BUILDINDEX(deed_fid_index_fcra);


layout_search_fid := RECORD
  string12 ln_fares_id;
  string1 which_orig;
  string1 source_code_2;
  string1 source_code_1;
...
infile_search_fid_fcra := pull(ln_propertyv2.key_fid_search_FCRA);
fcra_search_fid := PROJECT(infile_search_fid_fcra, TRANSFORM(layout_search_fid,
                                                             self.persistent_record_id := 0;
                                                             SELF := LEFT; 
                                                             self := []));

seach_fid_index_fcra := INDEX(fcra_search_fid,{ln_fares_id, which_orig, source_code_2, source_code_1},
                                {fcra_search_fid},'~thor_data400::key::ln_propertyv2::fcra::20121109b::search.fid');

export bld_search_fid_fcra := BUILDINDEX(seach_fid_index_fcra);




//key_search_fid
layout_search_fid_new := RECORD
  string12 ln_fares_id;
  string1 which_orig;
  string1 source_code_2;
  string1 source_code_1;
...
infile_search_fid := pull(ln_propertyv2.key_search_fid());
new_search_fid := PROJECT(infile_search_fid, TRANSFORM(layout_search_fid_new,
                                                             self.persistent_record_id := 0;
                                                             SELF := LEFT; 
                                                             self := []));

search_fid_index := INDEX(new_search_fid,{ln_fares_id, which_orig, source_code_2, source_code_1},
                                {new_search_fid},'~thor_data400::key::ln_propertyv2::20121109b::search.fid');
export bld_search_fid := BUILDINDEX(search_fid_index);
...
infile_search_bdid := pull(ln_propertyv2.Key_Search_BDID);
new_search_bdid := PROJECT(infile_search_bdid, TRANSFORM(search_bdid,
                                                         self.persistent_record_id := 0;
                                                         SELF := LEFT; 
                                                         self := []));
search_bdid_index := INDEX(new_search_bdid,{s_bid},{new_search_bdid},'~thor_data400::key::ln_propertyv2::20121109b::search.bdid');
export bld_search_bdid := BUILDINDEX(search_bdid_index);                                
...
infile_search_bdid_fcra := pull(ln_propertyv2.Key_Search_BDID_FCRA);
fcra_search_bdid := PROJECT(infile_search_bdid_fcra, TRANSFORM(search_bdid_fcra,
                                                                self.persistent_record_id := 0;
                                                                SELF := LEFT; 
                                                                self := []));
search_bdid_index_fcra := INDEX(fcra_search_bdid,{bd},{fcra_search_bdid},'~thor_data400::key::ln_propertyv2::fcra::20121109b::bdid');
export bld_search_bdid_fcra := BUILDINDEX(search_bdid_index_fcra);    

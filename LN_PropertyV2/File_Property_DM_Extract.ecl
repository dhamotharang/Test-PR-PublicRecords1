...
import LN_PropertyV2, text_search, Codes, ut,LN_PropertyV2_Services;
	...
	ds_deed := SORT(LN_Propertyv2.File_Deed(ln_fares_id <> ' '),ln_fares_id);
	ds_addl := SORT(LN_PropertyV2.File_addl_fares_deed(ln_fares_id <> ' '),ln_fares_id);	
	ds_search := SORT(File_Property_DMCleanName(ln_fares_id <> ' '),ln_fares_id);
  ...
  ut.Mac_Convert_Codes_To_Desc(ds_main,main_rec,legal_lot_ds,'FARES_1080',
	...
	ds_addl_rec := record
		...
		Layout_Property_DM_Extract.mortgage_deed_type_desc;
		Layout_Property_DM_Extract.fares_transaction_type_desc;
  ...
  ds_main_out := project(n_ds, transform(Layout_Property_DM_Extract, self:=left, self:=[]));
	ds_addl_out := project(trans_type_ds, transform(Layout_Property_DM_Extract, self:=left, self:=[]));
	ds_search_out := project(ds_search, transform(Layout_Property_DM_Extract, self:=left, self:=[]));

	export File_Property_DM_Extract := MERGE(ds_main_out,ds_addl_out,ds_search_out,SORTED(ln_fares_id)):INDEPENDENT;

...
export File_Prop_Address_Prep_V2(boolean isFCRA, boolean filter_fares=false) := function
...
with_assess get_deeds(with_assess L, ln_propertyv2.File_Deed R) := transform
	...
	purchasePicker := map(l.purchase_date = (unsigned)r.contract_date => 1, // both the same, use either one
												(unsigned)r.contract_date > l.purchase_date => 2, // use the right one
												3);	// use the left one

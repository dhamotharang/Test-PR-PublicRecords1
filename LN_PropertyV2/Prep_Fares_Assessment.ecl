...
import ut, aid, nid, property;

export Prep_Fares_Assessment(string pVersionDate)	:= function
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
		...
		ds_sort_fips_lookup := sort(distribute(ds_fips_lookup, hash(fips_code)), fips_code, local);
			...
			ds_assessment findFips(ds_sort_assessment l, ds_sort_fips_lookup r):= transform
				self := l;
			end;

			unmatched_fips := join(ds_sort_assessment, ds_sort_fips_lookup,
	...
	//Add Sequence Number///////////////////////////////////////////////////////////////////////////////////////////////	
		LN_PropertyV2.Layout_Raw_Fares.Clean_Address_Assessor seqAssess(ds_assessment l, unsigned4 cnt):= transform

				maxAssessFaresID									:= max(LN_PropertyV2.File_Fares_assessor_in(fares_id[1..2]	=	'RA'), (unsigned)fares_id[3..])	:	global;
	...
	//Prep Addresses for Cleaner & Add Fares ID
		LN_PropertyV2.Layout_Raw_Fares.Clean_Address_Assessor addOwnName(ds_assess_sequence l):= transform
		...
		ds_assess_mod := record
			LN_PropertyV2.Layout_Raw_Fares.Clean_Address_Assessor;
			string5   vendor										:= '';
			string1   which_orig								:= '';
			string70	nameasis 									:= '';
			string2   source_code								:= '';
	...
	//Normalize File by Address
		ds_assess_mod	tAssessNormAddr(ds_prep_assessor_address pInput, integer cnt) := transform
	...
	//Apply Clean Address Macro - Add AIDs
		LN_Propertyv2.Append_AID(assessFilter, dSearchCleanAddr, false);
	
	//Populate Clean Address Fields by Source Code
		ds_assess_mod addressAppended(dSearchCleanAddr pInput) := transform
	...
	cleanAddress 	:= project(dSearchCleanAddr,addressAppended(left));//:persist('~thor_data::persist::clean_assessor');
	...
	//Apply Name Clean Macro - Add NIDs
		...
		cleanfullnames_sort := sort(distribute(cleanfullnames, hash(fips_code)), fips_code, local):persist('~thor40_241_11::persist::fares_assess_clean_names_test');
		
	///////////////////////////////////////////////////////////////////////////////////////////////////////////
	//Create Assessment File///////////////////////////////////////////////////////////////////////////////////
	///////////////////////////////////////////////////////////////////////////////////////////////////////////
		property.Layout_Fares_Assessor f2580form(cleanfullnames_sort l, ds_sort_fips_lookup r):= transform
		...
		dAssess := dedup(sort(join(cleanfullnames_sort, ds_sort_fips_lookup,
													left.fips_code = right.fips_code,
	...
	//Populate Clean Names by Source Code & Orig Name Type	 
		ds_assess_mod addVendor(cleanfullnames_sort l, ds_sort_fips_lookup r):= transform
			self.vendor 						:= if(r.reid_tax_roll_coverage = 'YES-SUP',
																		'FAR_S',
																		'FAR_F');
			self.own1_name1_title		:= if(l.source_code[1]='O' and l.which_orig='1' and l.nametype<>'B',
																		l.cln_title,
																		'');
			self.own1_name1_first		:= if(l.source_code[1]='O' and l.which_orig='1' and l.nametype<>'B',
																		l.cln_fname,
																		'');
			self.own1_name1_middle	:= if(l.source_code[1]='O' and l.which_orig='1' and l.nametype<>'B',
																		l.cln_mname,
																		'');
			self.own1_name1_last		:= if(l.source_code[1]='O' and l.which_orig='1' and l.nametype<>'B',
																		l.cln_lname,
																		'');
			self.own1_name1_suffix	:= if(l.source_code[1]='O' and l.which_orig='1' and l.nametype<>'B',
																		l.cln_suffix,
																		'');
			self.own1_name1_company	:= if(l.source_code[1]='O' and l.which_orig='1' and l.nametype='B',
																		l.nameasis,
																		'');
			self.own1_name2_title		:= if(l.source_code[1]='O' and l.which_orig='1' and l.nametype<>'B',
																		l.cln_title2,
																		'');
			self.own1_name2_first		:= if(l.source_code[1]='O' and l.which_orig='1' and l.nametype<>'B',
																		l.cln_fname2,
																		'');
			self.own1_name2_middle	:= if(l.source_code[1]='O' and l.which_orig='1' and l.nametype<>'B',
																		l.cln_mname2,
																		'');
			self.own1_name2_last		:= if(l.source_code[1]='O' and l.which_orig='1' and l.nametype<>'B',
																		l.cln_lname2,
																		'');
			self.own1_name2_suffix	:= if(l.source_code[1]='O' and l.which_orig='1' and l.nametype<>'B',
																		l.cln_suffix2,
																		'');
			self.own1_name2_company	:= if(l.source_code[1]='O' and l.which_orig='1' and l.nametype='B',
																		l.nameasis,
																		'');
			self.own2_name1_title		:= if(l.source_code[1]='O' and l.which_orig='2' and l.nametype<>'B',
																		l.cln_title,
																		'');
			self.own2_name1_first		:= if(l.source_code[1]='O' and l.which_orig='2' and l.nametype<>'B',
																		l.cln_fname,
																		'');
			self.own2_name1_middle	:= if(l.source_code[1]='O' and l.which_orig='2' and l.nametype<>'B',
																		l.cln_mname,
																		'');
			self.own2_name1_last		:= if(l.source_code[1]='O' and l.which_orig='2' and l.nametype<>'B',
																		l.cln_lname,
																		'');
			self.own2_name1_suffix	:= if(l.source_code[1]='O' and l.which_orig='2' and l.nametype<>'B',
																		l.cln_suffix,
																		'');
			self.own2_name1_company	:= if(l.source_code[1]='O' and l.which_orig='2' and l.nametype='B',
																		l.nameasis,
																		'');	
			self.own2_name2_title		:= if(l.source_code[1]='O' and l.which_orig='2' and l.nametype<>'B',
																		l.cln_title2,
																		'');
			self.own2_name2_first		:= if(l.source_code[1]='O' and l.which_orig='2' and l.nametype<>'B',
																		l.cln_fname2,
																		'');
			self.own2_name2_middle	:= if(l.source_code[1]='O' and l.which_orig='2' and l.nametype<>'B',
																		l.cln_mname2,
																		'');
			self.own2_name2_last		:= if(l.source_code[1]='O' and l.which_orig='2' and l.nametype<>'B',
																		l.cln_lname2,
																		'');
			self.own2_name2_suffix	:= if(l.source_code[1]='O' and l.which_orig='2' and l.nametype<>'B',
																		l.cln_suffix2,
																		'');
			self.own2_name2_company	:= if(l.source_code[1]='O' and l.which_orig='2' and l.nametype='B',
																		l.nameasis,
																		'');	
			...
			self.seller2_suffix			:= if(l.source_code[1]='S' and l.nametype<>'B',
																		l.cln_suffix2,
																		'');
			self.seller2_company		:= if(l.source_code[1]='S' and l.which_orig='2' and l.nametype='B',
																		l.nameasis,
																		'');	
			self										:= l;
		end;
	...
	//Normalize Search File
		...
		layout_deed_mortgage_property_search_temp assessNorm(cleanNames l, integer	c)	:= transform
			self.ln_fares_id								:= l.fares_id;	
			self.conjunctive_name_seq 			:= choose(c, '1','2','1','2','1','2','1','2','1','2');
				
					date_temp 	:=  regexreplace('^[0]*$',StringLib.StringFindReplace(StringLib.StringRepad(ut.fnTrim2Upper(l.tax_year),6),' ','0'),'');
			...
			self.fname 											:= choose(c, l.own1_name1_first, l.own1_name2_first, l.own2_name1_first, l.own2_name2_first, l.own1_name1_first, l.own1_name2_first, l.own2_name1_first, l.own2_name2_first, l.seller1_first, l.seller2_first);
		...
		assess_norm_filter 	:= assess_norm(trim(lname, left, right)<>'' or trim(cname, left, right)<>'');	
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		bldUnmatch					:= output(unmatched_fips_project 	,,	vFilePrefix	+	'::fares_assess_unmatched' 	,__compressed__);	
		bldAssess						:= output(dAssess									,,	vFilePrefix	+	'::fares_assess'						,__compressed__);
		bldSearch						:= output(dSearch									,,	vFilePrefix	+	'::fares_assess_search' 		,__compressed__);
	
		bldPrepFiles				:= sequential(bldUnmatch, bldAssess, bldSearch);
		
		addSuperFiles				:= sequential(fileservices.startsuperfiletransaction(),
																			fileservices.addsuperfile('~thor_data400::in::ln_propertyv2::fares_assessor_unmatched', vFilePrefix	+	'::fares_assess_unmatched'),
																			fileservices.addsuperfile('~thor_data400::in::ln_propertyv2::fares_assessor', vFilePrefix	+	'::fares_assess'),
																			fileservices.addsuperfile('~thor_data400::in::ln_propertyv2::fares_search_converted', vFilePrefix	+	'::fares_assess_search'),
																			fileservices.addsuperfile('~thor_data400::in::property::raw::frs::assessment_archive_full','~thor_data400::in::property::raw::frs::assessment_archive',,true),
																			fileservices.clearsuperfile('~thor_data400::in::property::raw::frs::assessment_archive'),

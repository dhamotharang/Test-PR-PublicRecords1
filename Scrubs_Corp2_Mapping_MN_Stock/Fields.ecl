...
EXPORT MakeFT_invalid_corp_key(SALT30.StrType s0) := FUNCTION
  s1 := SALT30.stringfilter(s0,'-/=.+_0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_corp_key(SALT30.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT30.StringFilter(s,'-/=.+_0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ '))),~(LENGTH(TRIM(s)) >= 4));
EXPORT InValidMessageFT_invalid_corp_key(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInChars('-/=.+_0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ '),SALT30.HygieneErrors.NotLength('4..'),SALT30.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_corp_vendor(SALT30.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_vendor(SALT30.StrType s) := WHICH(((SALT30.StrType) s) NOT IN ['27']);
EXPORT InValidMessageFT_invalid_corp_vendor(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInEnum('27'),SALT30.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_state_origin(SALT30.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_state_origin(SALT30.StrType s) := WHICH(((SALT30.StrType) s) NOT IN ['MN']);
EXPORT InValidMessageFT_invalid_state_origin(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInEnum('MN'),SALT30.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_charter_nbr(SALT30.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_invalid_charter_nbr(SALT30.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT30.StringFilter(s,'-/=.+_0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ '))),~(LENGTH(TRIM(s)) >= 1));
EXPORT InValidMessageFT_invalid_charter_nbr(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInChars('-/=.+_0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ '),SALT30.HygieneErrors.NotLength('1..'),SALT30.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_stock_authorized_nbr(SALT30.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_invalid_stock_authorized_nbr(SALT30.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT30.StringFilter(s,'$.,/0123456789ABDEFGHILMNORSTQUWX '))),~(LENGTH(TRIM(s)) >= 1));
EXPORT InValidMessageFT_invalid_stock_authorized_nbr(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInChars('$.,/0123456789ABDEFGHILMNORSTQUWX '),SALT30.HygieneErrors.NotLength('1..'),SALT30.HygieneErrors.Good);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT30,Scrubs_Corp2_Mapping_MN_Stock;

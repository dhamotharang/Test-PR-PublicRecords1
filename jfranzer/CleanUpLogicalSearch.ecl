import ut,STD,VersionControl;
EXPORT CleanUpLogicalSearch(string lClusterFilter='thor400_30') := function
...
dname	:=	if(VersionControl._Flags.IsDataland,
							CleanUpLogicalConstants(lClusterFilter).dname_dataland,
							CleanUpLogicalConstants(lClusterFilter).dname_prod
						 );

d1:=join(s,dname,left.owner=right.owner,lookup);
d2:=table(d1,{d1,spaceUsed:=sum(group,(unsigned8)size)},email,few);
d:=join(d1,d2,left.owner=right.owner,lookup,left outer);

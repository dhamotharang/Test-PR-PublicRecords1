IMPORT  _control, std, tools, SICCodes;
...
EXPORT Spray(
	...
	,STRING		pNameOutput		= SICCodes._Constants().Name + ' Spray Info'	
  ) := FUNCTION
	...
	FilesToSpray := DATASET([
		...
		,SICCodes._Constants().max_record_size            // MAX RECORD SIZE
	 	}
	
	], tools.Layout_Sprays.Info);
		
	RETURN IF( pDirectory != '' AND NOT pExistSprayed
						,tools.fun_Spray(FilesToSpray,,,pOverwrite,,FALSE,pIsTesting,,SICCodes._Constants().Name + ' ' + pversion,pNameOutput));

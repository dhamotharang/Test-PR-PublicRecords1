...
#workunit('name','Scrubs_AVM.Base_BWR_Hygiene - Hygiene & Stats - SALT V3.11.9');
IMPORT Scrubs_AVM,SALT311;
// First create an instantiated hygiene module
  infile := Scrubs_AVM.Base_In_AVM;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_AVM.Base_hygiene(ip);
  ...
  OUTPUT(SALT311.MAC_Character_Counts.EclRecord(p,'Base_Layout_AVM'),NAMED('OptimizedLayout'));// File layout suggested by data

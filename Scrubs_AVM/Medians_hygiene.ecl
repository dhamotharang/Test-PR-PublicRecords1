...
EXPORT Summary(SALT311.Str30Type  txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_fips_geo_12 := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.fips_geo_12)));
    avelength_fips_geo_12 := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.fips_geo_12)),h.fips_geo_12<>(typeof(h.fips_geo_12))'');
    ...
    maxlength_median_valuation := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.median_valuation)));
    avelength_median_valuation := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.median_valuation)),h.median_valuation<>(typeof(h.median_valuation))'');
    ...
    maxlength_history_median_valuation := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.history_median_valuation)));
    avelength_history_median_valuation := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.history_median_valuation)),h.history_median_valuation<>(typeof(h.history_median_valuation))'');
  END;
...
SALT311.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT311.StrType)le.history_date),TRIM((SALT311.StrType)le.fips_geo_12),IF (le.median_valuation <> 0,TRIM((SALT311.StrType)le.median_valuation), ''),TRIM((SALT311.StrType)le.history_history_date),IF (le.history_median_valuation <> 0,TRIM((SALT311.StrType)le.history_median_valuation), '')));
  SELF.FldNo := C;
END;
...
SALT311.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 5);
  SELF.FldNo2 := 1 + (C % 5);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT311.StrType)le.history_date),TRIM((SALT311.StrType)le.fips_geo_12),IF (le.median_valuation <> 0,TRIM((SALT311.StrType)le.median_valuation), ''),TRIM((SALT311.StrType)le.history_history_date),IF (le.history_median_valuation <> 0,TRIM((SALT311.StrType)le.history_median_valuation), '')));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT311.StrType)le.history_date),TRIM((SALT311.StrType)le.fips_geo_12),IF (le.median_valuation <> 0,TRIM((SALT311.StrType)le.median_valuation), ''),TRIM((SALT311.StrType)le.history_history_date),IF (le.history_median_valuation <> 0,TRIM((SALT311.StrType)le.history_median_valuation), '')));
  END;
...
SHARED FldIds := DATASET([{1,'history_date'}
      ...
      ,{5,'history_median_valuation'}],SALT311.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    Medians_Fields.InValid_history_date((SALT311.StrType)le.history_date),
    Medians_Fields.InValid_fips_geo_12((SALT311.StrType)le.fips_geo_12),
    Medians_Fields.InValid_median_valuation((SALT311.StrType)le.median_valuation),
    Medians_Fields.InValid_history_history_date((SALT311.StrType)le.history_history_date),
    Medians_Fields.InValid_history_median_valuation((SALT311.StrType)le.history_median_valuation),
    0);
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_AVM, Medians_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

...
EXPORT Comps_Fields := MODULE
 
EXPORT NumFields := 32;
 
// Processing for each FieldType
EXPORT SALT311.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'Invalid_Date','Invalid_AlphaNum','Invalid_Chars','Invalid_Alpha','Invalid_Num','Invalid_Comps','Invalid_SalesPriceCode','Invalid_LandUseCode');
EXPORT FieldTypeNum(SALT311.StrType fn) := CASE(fn,'Invalid_Date' => 1,'Invalid_AlphaNum' => 2,'Invalid_Chars' => 3,'Invalid_Alpha' => 4,'Invalid_Num' => 5,'Invalid_Comps' => 6,'Invalid_SalesPriceCode' => 7,'Invalid_LandUseCode' => 8,0);
...
EXPORT MakeFT_Invalid_Chars(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 .+/'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_Chars(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 .+/'))));
EXPORT InValidMessageFT_Invalid_Chars(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 .+/'),SALT311.HygieneErrors.Good);
...
EXPORT MakeFT_Invalid_Comps(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'0123456789OAD'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_Comps(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789OAD'))));
EXPORT InValidMessageFT_Invalid_Comps(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789OAD'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_Invalid_SalesPriceCode(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_Invalid_SalesPriceCode(SALT311.StrType s) := WHICH(((SALT311.StrType) s) NOT IN ['A','Q','U','F','Z','D','C','P','X','']);
EXPORT InValidMessageFT_Invalid_SalesPriceCode(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInEnum('A|Q|U|F|Z|D|C|P|X|'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_Invalid_LandUseCode(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_Invalid_LandUseCode(SALT311.StrType s) := WHICH(((SALT311.StrType) s) NOT IN ['1','2','']);
EXPORT InValidMessageFT_Invalid_LandUseCode(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInEnum('1|2|'),SALT311.HygieneErrors.Good);
...
EXPORT SALT311.StrType FlatName(UNSIGNED2 i) := CHOOSE(i,'seq','ln_fares_id','unformatted_apn','prim_range','predir','prim_name','suffix','postdir','unit_desig','sec_range','p_city_name','st','zip','zip4','lat','long','geo_blk','fips_code','land_use_code','sales_price','sales_price_code','recording_date','assessed_value_year','assessed_total_value','market_total_value','lot_size','building_area','year_built','no_of_stories','no_of_rooms','no_of_bedrooms','no_of_baths');
EXPORT FieldNum(SALT311.StrType fn) := CASE(fn,'seq' => 0,'ln_fares_id' => 1,'unformatted_apn' => 2,'prim_range' => 3,'predir' => 4,'prim_name' => 5,'suffix' => 6,'postdir' => 7,'unit_desig' => 8,'sec_range' => 9,'p_city_name' => 10,'st' => 11,'zip' => 12,'zip4' => 13,'lat' => 14,'long' => 15,'geo_blk' => 16,'fips_code' => 17,'land_use_code' => 18,'sales_price' => 19,'sales_price_code' => 20,'recording_date' => 21,'assessed_value_year' => 22,'assessed_total_value' => 23,'market_total_value' => 24,'lot_size' => 25,'building_area' => 26,'year_built' => 27,'no_of_stories' => 28,'no_of_rooms' => 29,'no_of_bedrooms' => 30,'no_of_baths' => 31,0);
EXPORT SET OF SALT311.StrType FieldRules(UNSIGNED2 i) := CHOOSE(i,['ALLOW'],['ALLOW'],['ALLOW'],[],[],[],[],[],[],[],[],['ALLOW'],['ALLOW'],['ALLOW'],[],[],[],['ALLOW'],['ENUM'],['ALLOW'],['ENUM'],['CUSTOM'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],[]);
...
EXPORT Make_seq(SALT311.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_seq(SALT311.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_seq(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
 
EXPORT Make_ln_fares_id(SALT311.StrType s0) := MakeFT_Invalid_Comps(s0);
EXPORT InValid_ln_fares_id(SALT311.StrType s) := InValidFT_Invalid_Comps(s);
EXPORT InValidMessage_ln_fares_id(UNSIGNED1 wh) := InValidMessageFT_Invalid_Comps(wh);
 
EXPORT Make_unformatted_apn(SALT311.StrType s0) := MakeFT_Invalid_AlphaNum(s0);
EXPORT InValid_unformatted_apn(SALT311.StrType s) := InValidFT_Invalid_AlphaNum(s);
...
EXPORT Make_land_use_code(SALT311.StrType s0) := MakeFT_Invalid_LandUseCode(s0);
EXPORT InValid_land_use_code(SALT311.StrType s) := InValidFT_Invalid_LandUseCode(s);
...
EXPORT Make_sales_price_code(SALT311.StrType s0) := MakeFT_Invalid_SalesPriceCode(s0);
EXPORT InValid_sales_price_code(SALT311.StrType s) := InValidFT_Invalid_SalesPriceCode(s);
...
EXPORT Make_lot_size(SALT311.StrType s0) := MakeFT_Invalid_Chars(s0);
EXPORT InValid_lot_size(SALT311.StrType s) := InValidFT_Invalid_Chars(s);
EXPORT InValidMessage_lot_size(UNSIGNED1 wh) := InValidMessageFT_Invalid_Chars(wh);
 
EXPORT Make_building_area(SALT311.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_building_area(SALT311.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_building_area(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
 
EXPORT Make_year_built(SALT311.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_year_built(SALT311.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_year_built(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
 
EXPORT Make_no_of_stories(SALT311.StrType s0) := MakeFT_Invalid_Chars(s0);
EXPORT InValid_no_of_stories(SALT311.StrType s) := InValidFT_Invalid_Chars(s);
EXPORT InValidMessage_no_of_stories(UNSIGNED1 wh) := InValidMessageFT_Invalid_Chars(wh);
 
EXPORT Make_no_of_rooms(SALT311.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_no_of_rooms(SALT311.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_no_of_rooms(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
 
EXPORT Make_no_of_bedrooms(SALT311.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_no_of_bedrooms(SALT311.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_no_of_bedrooms(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
 
EXPORT Make_no_of_baths(SALT311.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_no_of_baths(SALT311.StrType s) := InValidFT_Invalid_Num(s);

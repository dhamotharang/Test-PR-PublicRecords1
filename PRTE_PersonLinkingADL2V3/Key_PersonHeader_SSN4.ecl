...
export Key := index(DataForKey0,,'~key::PRTE_PersonLinkingADL2V3PersonHeaderSSN4Refs');
export RawFetch( typeof(h.SSN4) param_SSN4, typeof(h.FNAME) param_FNAME, typeof(h.LNAME) param_LNAME) := 
    STEPPED( LIMIT( Key(
      ...
      AND ( LNAME = param_LNAME or LNAME = (typeof(LNAME))'' or param_LNAME = (typeof(LNAME))'' or metaphonelib.DMetaPhone1(LNAME)=metaphonelib.DMetaPhone1(param_LNAME) or ut.WithinEditN(LNAME,param_LNAME,2)  )),5000,skip,keyed),DID);
...
J0 := JOIN(Recs(SSN4 <> (typeof(SSN4))'',FNAME <> (typeof(FNAME))''),Key,LEFT.SSN4 = RIGHT.SSN4
     ...
     AND ( LEFT.LNAME = RIGHT.LNAME OR LEFT.LNAME = (TYPEOF(LEFT.LNAME))'' OR RIGHT.LNAME = (TYPEOF(RIGHT.LNAME))'' OR metaphonelib.DMetaPhone1(LEFT.LNAME)=metaphonelib.DMetaPhone1(RIGHT.LNAME) OR ut.WithinEditN(LEFT.LNAME,RIGHT.LNAME,2) ),Score_Batch(RIGHT,LEFT),
...
#IF(#TEXT(Input_SSN4)<>'' AND #TEXT(Input_FNAME)<>'')
  #uniquename(trans)
  PRTE_PersonLinkingADL2V3.Key_PersonHeader_SSN4.InputLayout_Batch %trans%(InFile le) := transform
  ...
  output_file := PRTE_PersonLinkingADL2V3.Key_PersonHeader_SSN4.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

export Key_PersonHeader_S := MODULE
import SALT20,ut;
...
DataForKey0 := dedup(sort(table(h(SSN5 NOT IN SET(s.nulls_SSN5,SSN5),SSN4 NOT IN SET(s.nulls_SSN4,SSN4)),layout),whole record,local),whole record,local); // Project out the fields in match candidates required for this linkpath
export Key := index(DataForKey0,,'~key::PRTE_PersonLinkingADL2V3PersonHeaderSRefs');
export RawFetch( typeof(h.SSN5) param_SSN5, typeof(h.SSN4) param_SSN4, typeof(h.FNAME) param_FNAME, typeof(h.MNAME) param_MNAME, typeof(h.LNAME) param_LNAME) := 
    STEPPED( LIMIT( Key(
          ( SSN5 = param_SSN5 and param_SSN5 <> (typeof(SSN5))'' )
      AND ( SSN4 = param_SSN4 and param_SSN4 <> (typeof(SSN4))'' )
      AND (( FNAME = param_FNAME  OR FNAME_PreferredName = PersonLinkingADL2.FNPreferName(param_FNAME) or FNAME[1..length(trim(param_FNAME))] = param_FNAME or param_FNAME[1..length(trim(FNAME))] = FNAME or metaphonelib.DMetaPhone1(FNAME)=metaphonelib.DMetaPhone1(param_FNAME) or ut.WithinEditN(FNAME,param_FNAME,2)  )
        ...
        AND ( LNAME = param_LNAME or LNAME = (typeof(LNAME))'' or param_LNAME = (typeof(LNAME))'' or metaphonelib.DMetaPhone1(LNAME)=metaphonelib.DMetaPhone1(param_LNAME) or ut.WithinEditN(LNAME,param_LNAME,2)  )
...
export ScoredDIDFetch( typeof(h.SSN5) param_SSN5, typeof(h.SSN4) param_SSN4, typeof(h.FNAME) param_FNAME, typeof(h.MNAME) param_MNAME, typeof(h.LNAME) param_LNAME,UNSIGNED4 param_DOB, typeof(h.CITY) param_CITY, typeof(h.STATE) param_STATE) := FUNCTION
  RawData := RawFetch(param_SSN5,param_SSN4,param_FNAME,param_MNAME,param_LNAME);
  Process_xADL2_Layouts.LayoutScoredFetch Score(RawData le) := transform
    self.keys_used := 1 << 4; // Set bitmap for key used
    self.SSN5Weight := IF ( le.SSN5 <> param_SSN5,0,(le.SSN5_weight100+50)/100);
    ...
    self.STATEWeight := (50+MAP ( le.STATE = param_STATE  => le.STATE_weight100,
           ...
           0))/100;
    self.Weight :=self.SSN5Weight +self.SSN4Weight +MAX(self.MAINNAMEWeight,self.FNAMEWeight + self.MNAMEWeight + self.LNAMEWeight) + self.DOBWeight_year + self.DOBWeight_month + self.DOBWeight_day +self.CITYWeight +self.STATEWeight;
    self := le;
  end;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xADL2_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := transform
    ...
    self.keys_used := 1 << 4; // Set bitmap for key used
    self.SSN5Weight := IF ( le.SSN5 <> ri.SSN5,0,(le.SSN5_weight100+50)/100);
...
J0 := JOIN(Recs(SSN5 <> (typeof(SSN5))'',SSN4 <> (typeof(SSN4))''),Key,LEFT.SSN5 = RIGHT.SSN5
     ...
     AND 
      (( LEFT.FNAME = RIGHT.FNAME OR LEFT.FNAME[1..LENGTH(TRIM(RIGHT.FNAME))] = RIGHT.FNAME OR RIGHT.FNAME[1..LENGTH(TRIM(LEFT.FNAME))] = LEFT.FNAME OR metaphonelib.DMetaPhone1(LEFT.FNAME)=metaphonelib.DMetaPhone1(RIGHT.FNAME) OR ut.WithinEditN(LEFT.FNAME,RIGHT.FNAME,2)  OR RIGHT.FNAME_PreferredName = PersonLinkingADL2.FNPreferName(LEFT.FNAME) )
        ...
        AND ( LEFT.LNAME = RIGHT.LNAME OR LEFT.LNAME = (TYPEOF(LEFT.LNAME))'' OR RIGHT.LNAME = (TYPEOF(RIGHT.LNAME))'' OR metaphonelib.DMetaPhone1(LEFT.LNAME)=metaphonelib.DMetaPhone1(RIGHT.LNAME) OR ut.WithinEditN(LEFT.LNAME,RIGHT.LNAME,2) )
    ...
    ATMOST(LEFT.SSN5 = RIGHT.SSN5
     AND LEFT.SSN4 = RIGHT.SSN4,5000)); // Use indexed join (used for smaller batches
J1 := JOIN(Recs(SSN5 <> (typeof(SSN5))'',SSN4 <> (typeof(SSN4))''),PULL(Key),LEFT.SSN5 = RIGHT.SSN5
     AND LEFT.SSN4 = RIGHT.SSN4
     AND 
      (( LEFT.FNAME = RIGHT.FNAME OR LEFT.FNAME[1..LENGTH(TRIM(RIGHT.FNAME))] = RIGHT.FNAME OR RIGHT.FNAME[1..LENGTH(TRIM(LEFT.FNAME))] = LEFT.FNAME OR metaphonelib.DMetaPhone1(LEFT.FNAME)=metaphonelib.DMetaPhone1(RIGHT.FNAME) OR ut.WithinEditN(LEFT.FNAME,RIGHT.FNAME,2)  OR RIGHT.FNAME_PreferredName = PersonLinkingADL2.FNPreferName(LEFT.FNAME) )
        AND ( LEFT.MNAME = RIGHT.MNAME OR LEFT.MNAME[1..LENGTH(TRIM(RIGHT.MNAME))] = RIGHT.MNAME OR RIGHT.MNAME[1..LENGTH(TRIM(LEFT.MNAME))] = LEFT.MNAME OR ut.WithinEditN(LEFT.MNAME,RIGHT.MNAME,2) )
        ...
        OR                         SALT20.fn_concept_wordbag_oneweight.Match3((string)RIGHT.FNAME,RIGHT.FNAME_weight100,true,0,true,(string)RIGHT.MNAME,RIGHT.MNAME_weight100,true,0,true,(string)RIGHT.LNAME,RIGHT.LNAME_weight100,true,0,false,(string)LEFT.FNAME,0,(string)LEFT.MNAME,0,(string)LEFT.LNAME,0)>= 0 ),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.SSN5 = RIGHT.SSN5
     AND LEFT.SSN4 = RIGHT.SSN4,5000),HASH); // PULL used to cause non-indexed join
  RETURN IF(AsIndex,J0,J1);
END;
// Now the sloppier macro to allow processing of an 'arbitrary' file
export MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_SSN5='',Input_SSN4='',Input_FNAME='',Input_MNAME='',Input_LNAME='',Input_DOB='',Input_CITY='',Input_STATE='',output_file,AsIndex='true') := MACRO
#IF(#TEXT(Input_SSN5)<>'' AND #TEXT(Input_SSN4)<>'')
  #uniquename(trans)
  PRTE_PersonLinkingADL2V3.Key_PersonHeader_S.InputLayout_Batch %trans%(InFile le) := transform
  ...
  output_file := PRTE_PersonLinkingADL2V3.Key_PersonHeader_S.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

...
pj15 do_computes(pj15 le) := TRANSFORM
  ...
  SELF := le;
END;
SHARED propogated := PROJECT(pj15,do_computes(left)) : PERSIST('temp::PRTE_PersonLinkingADL2V3_DID_PersonHeader_mc_props'); // to allow to 'jump' over an exported value
...
SHARED Annotated := DISTRIBUTE(j0,hash(DID)) : PERSIST('temp::PRTE_PersonLinkingADL2V3_PersonHeader_mc'); // Distributed for keybuild case

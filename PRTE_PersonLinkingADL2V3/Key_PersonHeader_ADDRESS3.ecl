...
export Key := index(DataForKey0,,'~key::PRTE_PersonLinkingADL2V3PersonHeaderADDRESS3Refs');
...
#IF(#TEXT(Input_PRIM_RANGE)<>'' AND #TEXT(Input_PRIM_NAME)<>'' AND #TEXT(Input_ZIP)<>'')
  #uniquename(trans)
  PRTE_PersonLinkingADL2V3.Key_PersonHeader_ADDRESS3.InputLayout_Batch %trans%(InFile le) := transform
  ...
  output_file := PRTE_PersonLinkingADL2V3.Key_PersonHeader_ADDRESS3.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

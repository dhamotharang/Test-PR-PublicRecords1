...
export Key := index(DataForKey7,,'~key::PRTE_PersonLinkingADL2V3PersonHeaderDORefs');
export RawFetch(UNSIGNED4 param_DOB, typeof(h.LNAME) param_LNAME, typeof(h.FNAME) param_FNAME) := 
    STEPPED( LIMIT( Key(
          ( param_DOB <> 0 AND ( param_DOB div 10000 = DOB_year AND DOB_year<>0 ) AND ( param_DOB div 100 % 100 = DOB_month AND DOB_month<>0 ) AND ( param_DOB % 100 = DOB_day AND DOB_day<>0 ))
...
#IF(#TEXT(Input_DOB)<>'' AND #TEXT(Input_LNAME)<>'')
  #uniquename(trans)
  PRTE_PersonLinkingADL2V3.Key_PersonHeader_DO.InputLayout_Batch %trans%(InFile le) := transform
  ...
  output_file := PRTE_PersonLinkingADL2V3.Key_PersonHeader_DO.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

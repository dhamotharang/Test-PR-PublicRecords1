...
#uniquename(ofat)	
	...
	%ofat% := DISTRIBUTE(JOIN(infile,patchfile,left.rid_name=right.rid_name,%tr%(left,right),LEFT OUTER),HASH(did_name));
//	o := IF( COUNT(patchfile)<50000000, %ot%, %ofat% );
  o := IF( COUNT(patchfile)<SALT310.Config.LookupMaxSize, %ot%, %ofat% );
  ENDMACRO;

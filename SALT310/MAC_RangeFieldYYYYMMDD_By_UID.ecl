...
%idata% := TABLE(infile((UNSIGNED6)low_infield<>null_field,(UNSIGNED6)high_infield<>null_field),{ufield,UNSIGNED4 xx_low := SALT310.fn_YYYYMMDD_to_Int((UNSIGNED6)low_infield),UNSIGNED4 xx_high := SALT310.fn_YYYYMMDD_to_Int((UNSIGNED6)high_infield)})(xx_low<=xx_high);
#uniquename(r)
%r% := RECORD
  SALT310.UIDType ufield;
	UNSIGNED3 infield;
  END;
#uniquename(tr)

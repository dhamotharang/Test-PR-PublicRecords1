// NOTE: This macro references SALT310.Config.LookupMaxSize when determining whether
// it's possible to use a LOOKUP JOIN given the data provided.  If users experience
// OOM issues in that JOIN ("j1s" below), it likely indicates LookupMaxSize has been
// set incorrectly for the environment -- it is a measure of the amount of RAM
// available per node.
...
t := TABLE(infile,ifp);
SALT310.utMAC_Sequence_Records(t,xxx_unc,nif)	
r := RECORD
  UNSIGNED xxx_unc;
  UNSIGNED2 pos;
  SALT310.StrType s;
...
r tr(nif s_in, unsigned2 c) := transform
  self.pos := c;
	self.s := SALT310.GetNthWord(s_in.instring,c);
	self.xxx_unc := s_in.xxx_unc;
  end;
	
// Split out the words for each wordbag
n := NORMALIZE(	nif, SALT310.WordCount(LEFT.instring), tr(LEFT,COUNTER) );
r2 := RECORD(r)
  INTEGER2 spec;
	END;
...
j1 := IF ( COUNT(keyname)*SIZEOF(pk,max) < SALT310.Config.LookupMaxSize, j1s, j1b_1+j1b_2 ); // Use a pure lookup if you can
j0 := JOIN( n, keyname, LEFT.s = RIGHT.keyfield, tr2(LEFT,RIGHT), LEFT OUTER, LIMIT(SALT310._Config.Constants.JoinLimit) );
j := IF(AsIndex,j0,j1);
ifp addw(ifp le, j ri) := TRANSFORM
  SELF.new_instring := TRIM(le.new_instring)+' '+TRIM(ri.s)+' '+(SALT310.StrType)ri.spec;
...
TYPEOF(infile) sb(sf le) := TRANSFORM
  SELF.new_instring := IF(le.instring='','',(SALT310.StrType)le.tot_spec+le.new_instring); // Newstring has space pre-pended by %addw%
  SELF := le;
  END;

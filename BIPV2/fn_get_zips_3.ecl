...
EXPORT fn_get_zips_3(STRING sCity,STRING sState,STRING sZip,UNSIGNED iRadius):=FUNCTION
  ...
  RETURN IF(iRadius>0,dWithRadius,dDerivedNoRadius);
END;


/*

BIPV2.fn_get_zips_3('AUSTIN','TX','76710',0);   // Valid city/st with a valid zip code that is not in that city/st, without radius : should have all Austin zips as well as the one enterd
BIPV2.fn_get_zips_3('AUSTIN','TX','76710',15);  // Valid city/st with a valid zip code that is not in that city/st, with radius    : all zips within the radius of Austin as well as 76710
BIPV2.fn_get_zips_3('AUSTINI','TX','76710',0);  // Invalid city with a valid zip code, without radius                              : should have just the entered zip
BIPV2.fn_get_zips_3('AUSTINI','TX','76710',15); // Invalid city with a valid zip code, with radius                                 : all zips within the radius of 76710
BIPV2.fn_get_zips_3('AUSTIN','TX','',0);        // valid city/st without or radius                                                 : all zips in Austin
BIPV2.fn_get_zips_3('AUSTIN','TX','',15);       // valid city/st without zip, with radius                                          : all zips within the radius of Austin
BIPV2.fn_get_zips_3('AUSTIN','XX','',0);        // valid city but not state without zip or radius                                  : no results
BIPV2.fn_get_zips_3('AUSTIN','TX','78710',0);   // valid city/st with zip in Austin, without radius                                : all Austin zips (entered one on top)
BIPV2.fn_get_zips_3('AUSTIN','TX','78710',15);  // valid city/st with zip in Austin, with radius                                   : all zips within the radius of Austin (entered one on top)
BIPV2.fn_get_zips_3('','','76710',0);           // Valid valid zip only, without radius                                            : only the zip entered
BIPV2.fn_get_zips_3('','','76710',15);          // Valid valid zip only, with radius                                               : all zips within the radius of 76710

...
// Layout_case_details_Offender_key := RECORD
// INTEGER ecl_cade_id;
...
Layout_case_details_Offender_key   tr_PrimOffenderToCaseDetails(File_Primary_Offender_ECL_Cade_id L) := TRANSFORM 
  ...
  SELF.CADE_ID := '';
	SELF.CASE_DT_C := //IF(LENGTH(TRIM(L.CASE_FILING_DT))=8,TRIM(L.CASE_FILING_DT),'');
	                  IF(_validate.date.fIsValid(L.CASE_FILING_DT)=true,L.CASE_FILING_DT,'');
	...
	SELF.LAST_UPDATE_DT := '';
	SELF.RECORD_SUPPLIER_CD :=      Map(l.source_file = 'NJ_DOC' and l.case_type =  'ADM' => '9041',
	...
	SELF.OTH_PERSONAL_INFO := 'ProcDt: ' + TRIM(L.PROCESS_DATE) + '; ' +
	                          'OffKey: ' + TRIM(L.OFFENDER_KEY) + '; ' + 
														...
														'SF: ' + TRIM(L.SOURCE_FILE)  + '; ECL_CADE_ID: ' + L.ECL_CADE_ID;
	SELF.NC_CASE_DT := //IF(LENGTH(TRIM(L.CASE_FILING_DT))<>8,L.CASE_FILING_DT,''); 
	                   IF(_validate.date.fIsValid(L.CASE_FILING_DT)=false,L.CASE_FILING_DT,'');
	SELF.NC_SUBJECT_DOB := //IF(LENGTH(TRIM(L.DOB))<>8,L.DOB,'');

...
Layout_ecl_cade_id_ecl_charge_id_offender_key  tr_ecl_cade_id_ecl_charge_id_offender_key(mi8.File_Crim_Offenses_ECL_Cade_id L) := TRANSFORM
SELF := L;
END;		

ds_ecl_cade_id_ecl_charge_id_offender_key := PROJECT(mi8.File_Crim_Offenses_ECL_Cade_id,tr_ecl_cade_id_ecl_charge_id_offender_key(LEFT));
...
//distribute_File_Crim_Punishment :=  DISTRIBUTE(mi8.File_Crim_Punishment,HASH32(offender_key));
distribute_File_Crim_Punishment :=  DISTRIBUTE(File_DOC_Crim_Punishment,HASH32(offender_key));
...
Layout_Normalize_Sentences tr_Normalize_Sentences(sort_Crim_Punishment_ECL_CadeCharge_id L, INTEGER C) := TRANSFORM 
  ...
  sent_length_desc_clean :=  IF(L.sent_length_desc<>'',
	...
	// latest_adm_dt *************************************************************************************//	
	latest_adm_dt :=  IF(L.latest_adm_dt<>'' and L.OLD_RECORD_SUPPLIER_CD_C in ['FLDO','KYDO'],
  ...
  SELF.ecl_cade_id := L.ecl_cade_id;
	...
	SELF.SENTENCE_COMMENT_C := 
	                     CHOOSE(C,IF(LENGTH(TRIM(sent_length))>0,TRIM(sent_length,LEFT,RIGHT),''),
	                                    IF(LENGTH(TRIM(cur_stat_inm))>0,TRIM(cur_stat_inm,LEFT,RIGHT),''),
										                	...
										                	IF(LENGTH(TRIM(L.cur_sec_class_dt))>0,'Security Class Date: '+  TRIM(L.cur_sec_class_dt,LEFT,RIGHT) + ' (YYYYMMDD)',''),								 
																			...
																			IF(LENGTH(TRIM(ctl_rel_dt))>0,TRIM(ctl_rel_dt,LEFT,RIGHT) + ' (YYYYMMDD)',''),
																			...
																			IF(LENGTH(TRIM(par_sch_end_dt))>0,TRIM(par_sch_end_dt,LEFT,RIGHT) + ' (YYYYMMDD)',''),
																			IF(LENGTH(TRIM(par_act_end_dt))>0,TRIM(par_act_end_dt,LEFT,RIGHT) + ' (YYYYMMDD)',''),
																			IF(LENGTH(TRIM(par_cty))>0,TRIM(par_cty,LEFT,RIGHT),''),	
																			IF(LENGTH(TRIM(L.pro_st_dt))>0,TRIM(L.pro_st_dt,LEFT,RIGHT),''),	
																			IF(LENGTH(TRIM(L.pro_end_dt))>0,TRIM(L.pro_end_dt,LEFT,RIGHT),''),	
																			IF(LENGTH(TRIM(L.pro_status))>0,TRIM(L.pro_status,LEFT,RIGHT),'')	
																				 );
	SELF.SETY_SENTENCE_TYPE_CD_C := 
	                           CHOOSE(C,IF(LENGTH(TRIM(sent_length))>0,'32',''),                    //Sety Code for sent_length,
																						...
																						IF(LENGTH(TRIM(sch_rel_dt))>0,'02',''),                     //Sety Code for sch_rel_dt,
																						IF(LENGTH(TRIM(act_rel_dt))>0,'03',''),                     //Sety Code for act_rel_dt,
																						IF(LENGTH(TRIM(ctl_rel_dt))>0,'01',''),                     //Sety Code for ctl_rel_dt,
																						
																						IF(LENGTH(TRIM(presump_par_rel_dt))>0,'PP',''),             //Sety Code for presump_par_rel_dt,
																						IF(LENGTH(TRIM(mutl_part_pgm_dt))>0,'MP',''),               //Sety Code for mutl_part_pgm_dt,
																						...
																						IF(LENGTH(TRIM(L.pro_st_dt))>0,'64',''),                    //Sety Code for L.pro_st_dt,
																						IF(LENGTH(TRIM(L.pro_end_dt))>0,'65',''),                  //Sety Code for L.pro_end_dt,
																						IF(LENGTH(TRIM(L.pro_status))>0,'PR','')									   //Sety Code for L.pro_status,													
																						);     
		...
		//MP  Mutual Participation Prgm Dt
		//64 Probation Start
		//65 	Probation End
		//PR 	Probation
		
		//'01','Control Release Date'
		//'02','Scheduled Release Date'
		//'03','Actual Release Date'
...
ds_NormSentences := NORMALIZE(sort_Crim_Punishment_ECL_CadeCharge_id,22,tr_Normalize_Sentences(LEFT,COUNTER));

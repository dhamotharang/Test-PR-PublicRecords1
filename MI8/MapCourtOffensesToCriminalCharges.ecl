...
dist_MapPrimOffenderToCaseMaster := DISTRIBUTE(MapPrimOffenderToCaseMaster,HASH32(offender_key));
dist_File_Crim_Offenses_ECL_Cade_id := DISTRIBUTE(File_Crim_Offenses_ECL_Cade_id,HASH32(offender_key));


/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

// make left record set bigger due to error:
//Error: System error: 101: Graph[60], csvwrite[67]: SLAVE 10.173.89.6:16600: 101, LOCALjoin(66): CThorRowArray: Group too large: 67108123 bytes : ProcessSlaveActivity exception id=67

// Layout_Case_Master_and_Moxie_Court_Offenses tr_Attach_CM_Constants_Court_offenses(dist_MapPrimOffenderToCaseMaster L, dist_File_Crim_Offenses_ECL_Cade_id R) := TRANSFORM
...
// ds_CaseMasterCrimOffenses := JOIN(dist_MapPrimOffenderToCaseMaster,dist_File_Crim_Offenses_ECL_Cade_id,
																			// LEFT.ecl_cade_id=RIGHT.ecl_cade_id,
																			 // tr_Attach_CM_Constants_Court_offenses(LEFT,RIGHT),local);
																			 
	
Layout_Case_Master_and_Moxie_Court_Offenses tr_Attach_CM_Constants_Court_offenses(dist_File_Crim_Offenses_ECL_Cade_id L, dist_MapPrimOffenderToCaseMaster R) := TRANSFORM
SELF.CM_CASE_NUM := R.CASE_NUM_C;
SELF.CM_DOB := R.DOB;																																	 
SELF.CM_SSN := R.SSN;
SELF.CM_NC_DOB := R.NC_DOB;
SELF.CM_SOURCE_UID := R.SOURCE_UID_C;
SELF := L;
END;


ds_CaseMasterCrimOffenses := JOIN(dist_File_Crim_Offenses_ECL_Cade_id,dist_MapPrimOffenderToCaseMaster,
																			LEFT.ecl_cade_id=RIGHT.ecl_cade_id,
																			 tr_Attach_CM_Constants_Court_offenses(LEFT,RIGHT),local);	
...
ORIGINAL_CASE_TYPE_C := '';
                                 //CASE(TRIM(L.off_typ,LEFT,RIGHT),'F' => 'F', 'M'=> 'M', '');
...
SELF.AMENDED_CHARGE_C :=  AMENDED_CHARGE_C;
SELF.AMENDED_CHARGE_SHORT := '';
SELF.AMENDED_CODE_SECTION := IF(l.chg <>'',l.chg,L.off_code);
SELF.AMENDED_CASE_TYPE := l.off_lev; 
...
SELF.OFFENSE_TOWN_C := l.OffenseTown;
SELF.OFFENSE_DESCRIPTION_C := L.add_off_desc;
																													   ...
																													   => TRIM(L.arr_disp_desc_1),
																													  	 ...
																													  	 => TRIM(L.arr_disp_desc_2),
																													          ...
																													          => TRIM(L.arr_disp_desc_3),
																															      	L.arr_disp_cd <> ''
																																       => TRIM(L.arr_disp_cd),''));															    
SELF.COURT_LOCATION_C := Map(l.cty_conv <> ''  =>l.cty_conv,l.court_desc);
...
SELF.REFERENCE_INFO := '';
SELF.CUSTODY_DT_C := '';
SELF.ADMIT_DT_C := IF(_validate.date.fIsValid(l.inc_adm_dt)=true,l.inc_adm_dt,'');  
SELF.RELEASE_DT_C := '';
...
//export MapCourtOffensesToCriminalCharges := SORT(ds_CriminalCharges,ecl_cade_id,ecl_charge_id); -- removed for hygenics....add back in tp


export MapCourtOffensesToCriminalCharges := ds_CriminalCharges;

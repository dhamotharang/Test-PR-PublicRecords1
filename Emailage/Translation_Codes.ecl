...
//Translate Source Code Values
//-----Bit map translation for EQ											
export	string	fGet_eq_sources_from_bitmap2(unsigned bitmap_src) := function
		boolean		fFlagIsOn(unsigned pValue, unsigned bitmap_src)	:=	pValue & bitmap_src = bitmap_src;
		string		translated_value		:=	if(bitmap_src = 0,
											   '',											   
											   if(fFlagIsOn(bitmap_src, source_bitmap_code(mdr.sourceTools.src_Equifax)),			' ' + 'Equifax',''));
		return		lib_stringlib.stringlib.stringfindreplace(trim(lib_stringlib.stringlib.stringcleanspaces(translated_value),left,right),'  ',' ');
end;

//-----Bit map translation for Other header sources									
export	string	fGet_other_hdr_sources_from_bitmap2(unsigned bitmap_src) := function
		boolean		fFlagIsOn(unsigned pValue, unsigned bitmap_src)	:=	pValue & bitmap_src = bitmap_src;
		string		translated_value		:=	if(bitmap_src = 0,
											   '',							if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_Utilities 										)),' ' + 	'Utility' 								,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_LnPropV2_Fares_Asrs 					)),' ' + 	'LNPropertyFaresAssessors' 			,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_Voters_v2											)),' ' + 	'Voters'									,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_Util_Work_Phone								)),' ' + 	'UtiltyWork'						,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_American_Students_List				)),' ' + 	'AmericanStudentList'		,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_Professional_License					)),' ' + 	'ProfessionalLicense'			,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_Certegy 											)),' ' + 	'Certegy' 									,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_EMerge_Hunt										)),' ' + 	'EMerge_Hunt'								,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_EMerge_Master									)),' ' + 	'EMerge_Master'							,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_LnPropV2_Lexis_Deeds_Mtgs			)),' ' + 	'LNPropertyDeedsMtgs'	,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_LnPropV2_Lexis_Asrs						)),' ' + 	'LNPropertyAssessors'				,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_KY_Watercraft									)),' ' + 	'KY_Watercraft'							,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_EMerge_Boat										)),' ' + 	'EMerge_Boat'								,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_VA_Watercraft									)),' ' + 	'VA_Watercraft'							,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_NC_Watercraft									)),' ' + 	'NC_Watercraft'							,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_TUCS_Ptrack										)),' ' + 	'TUCS_Ptrack'								,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_EMerge_Cens										)),' ' + 	'EMerge_Cens'								,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_Federal_Firearms							)),' ' + 	'Federal_Firearms'					,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_EMerge_Fish										)),' ' + 	'EMerge_Fish'								,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_Federal_Explosives						)),' ' + 	'Federal_Explosives'				,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_MD_Watercraft									)),' ' + 	'MD_Watercraft'							,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_Miscellaneous									)),' ' + 	'Miscellaneous'							,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_MO_Veh												)),' ' + 	'MO_Veh'										,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_MO_DL													)),' ' + 	'MO_DL'											,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_MO_Experian_Veh								)),' ' + 	'MO_Experian_Veh'						,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_Experian_Credit_Header				)),' ' + 	'ExperianCreditHeader'		,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_MO_Watercraft									)),' ' + 	'MO_Watercraft'							,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_ZUtil_Work_Phone							)),' ' + 	'ZUtilWorkPhone'					,'')
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_ZUtilities										)),' ' + 	'ZUtilities'								,'')	
																					+if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_TU_CreditHeader							  )),' ' + 	'TUCreditHeader'				    ,'')			
											  );
		return		lib_stringlib.stringlib.stringfindreplace(trim(lib_stringlib.stringlib.stringcleanspaces(translated_value),left,right),'  ',' ');
end;	

//-----Bit map translation for Other sources
export	string	fGet_other_sources_from_bitmap2(unsigned bitmap_src) := function
		boolean		fFlagIsOn(unsigned pValue, unsigned bitmap_src)	:=	pValue & bitmap_src = bitmap_src;
		string		translated_value		:=	if(bitmap_src = 0,
											   '',											   
											   if(fFlagIsOn(bitmap_src, source_bitmap_code(mdr.sourceTools.src_InfutorCID							)),	' ' + 'InfutorCID'						,'')
										+	   if(fFlagIsOn(bitmap_src, source_bitmap_code(mdr.sourceTools.src_Cellphones_Kroll				)),	' ' + 'CellphonesKroll'			,'')
										+	   if(fFlagIsOn(bitmap_src, source_bitmap_code(mdr.sourceTools.src_Cellphones_Traffix			)),	' ' + 'CellphonesTraffix'		,'')
										+	   if(fFlagIsOn(bitmap_src, source_bitmap_code(mdr.sourceTools.src_Cellphones_Nextones		)),	' ' + 'CellphonesNextones'		,'')
										+	   if(fFlagIsOn(bitmap_src, source_bitmap_code(mdr.sourceTools.src_Targus_White_Pages			)),	' ' + 'TargusWhitePages'		,'')
										+	   if(fFlagIsOn(bitmap_src, source_bitmap_code(mdr.sourceTools.src_Pcnsr									)),	' ' + 'PCNSR'									,'')
										+	   if(fFlagIsOn(bitmap_src, source_bitmap_code(mdr.sourceTools.src_Gong_History						)),	' ' + 'GongHistory'					,'')
										+	   if(fFlagIsOn(bitmap_src, source_bitmap_code(mdr.sourceTools.src_Intrado								)),	' ' + 'Intrado'								,'')
										+	   if(fFlagIsOn(bitmap_src, source_bitmap_code(mdr.sourceTools.src_Wired_Assets_Owned			)),	' ' + 'WiredAssetsOwned'		,'')										
										+	   if(fFlagIsOn(bitmap_src, source_bitmap_code(mdr.sourceTools.src_Wired_Assets_Royalty		)),	' ' + 'WiredAssetsRoyalty'	,'')	
									  +	   if(fFlagIsOn(bitmap_src, source_bitmap_code(mdr.sourceTools.src_InquiryAcclogs					)),	' ' + 'Inquiry'	,'')	
										+		 if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_Ibehavior							)),' ' + 	'Ibehavior','')	
										+    if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_Thrive_LT							)),' ' + 	'ThriveLT','')	
										+    if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_Thrive_PD							)),' ' + 	'Thrive_PD','')		
										+    if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_AlloyMedia_Consumer)),' ' + 	'AlloyMedia_Consumer','')
										+    if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_Link2Tek              )),' ' + 	'Link2Tek','')
										+    if(fFlagIsOn(bitmap_src, source_bitmap_code(	mdr.sourceTools.src_NeustarWireless       )),' ' + 	'NeustarWireless','') //Jira DF-24336										
										);
...
export	string fGet_all_sources2 (unsigned bitmap_src) := function
return  StringLib.StringCleanSpaces(fGet_other_sources_from_bitmap2(bitmap_src) + ' ' +
             fGet_other_hdr_sources_from_bitmap2(bitmap_src) + ' ' + 
   	         fGet_eq_sources_from_bitmap2(bitmap_src));
end;
...
export rules_bitmap_code(string rules = '')  := map(
															...
															rules = 'NeustarWireless-Verified-A' => ut.bit_set(0,31),
															rules = 'NeustarWireless-Verified-B' => ut.bit_set(0,32),
															rules = 'NeustarWireless-Verified-C' => ut.bit_set(0,33),
															rules = 'NeustarWireless-Verified-D' => ut.bit_set(0,34),
															rules = 'NeustarWireless-Verified-E' => ut.bit_set(0,35),
															rules = 'NeustarWireless-Activity-Status-A1' => ut.bit_set(0,36),
															rules = 'NeustarWireless-Activity-Status-A2' => ut.bit_set(0,37),
															rules = 'NeustarWireless-Activity-Status-A3' => ut.bit_set(0,38),
															rules = 'NeustarWireless-Activity-Status-A4' => ut.bit_set(0,39),
															rules = 'NeustarWireless-Activity-Status-A5' => ut.bit_set(0,40),
															rules = 'NeustarWireless-Activity-Status-A6' => ut.bit_set(0,41),
															rules = 'NeustarWireless-Activity-Status-A7' => ut.bit_set(0,42),
															rules = 'NeustarWireless-Activity-Status-I1' => ut.bit_set(0,43),
															rules = 'NeustarWireless-Activity-Status-I2' => ut.bit_set(0,44),
															rules = 'NeustarWireless-Activity-Status-I3' => ut.bit_set(0,45),
															rules = 'NeustarWireless-Activity-Status-I4' => ut.bit_set(0,46),
															rules = 'NeustarWireless-Activity-Status-I5' => ut.bit_set(0,47),
															rules = 'NeustarWireless-Activity-Status-I6' => ut.bit_set(0,48),
															rules = 'NeustarWireless-Activity-Status-I7' => ut.bit_set(0,49),
															rules = 'NeustarWireless-Activity-Status-U' => ut.bit_set(0,50),
															rules = 'NeustarWireless-Prepaid-Y' => ut.bit_set(0,51),
															rules = 'NeustarWireless-Prepaid-N' => ut.bit_set(0,52),
															rules = 'NeustarWireless-Cord-Cutter-Y' => ut.bit_set(0,53),
															rules = 'NeustarWireless-Cord-Cutter-N' => ut.bit_set(0,54),
...
export source_file(string vendor = '')   := map(vendor = mdr.sourceTools.src_Pcnsr  		=> 'PConsumer',
												...
												//cellphone vendors
												vendor = mdr.sourceTools.src_Cellphones_Kroll		=> 'Cellphones_KROLL',
												vendor = mdr.sourceTools.src_Cellphones_Traffix		=> 'Cellphones_TRAFFIX',
												vendor = mdr.sourceTools.src_Cellphones_Nextones		=> 'Cellphones_NEXTONES',
												...
												vendor = mdr.sourceTools.src_Utilities => 'Utility',
												vendor = mdr.sourceTools.src_LnPropV2_Fares_Asrs => 'LN Property Fares Assessor',
												vendor = mdr.sourceTools.src_Voters_v2=> 'Voters',
												vendor = mdr.sourceTools.src_Util_Work_Phone=> 'Utility Work',
												vendor = mdr.sourceTools.src_American_Students_List=> 'American Student List',
												vendor = mdr.sourceTools.src_Professional_License=> 'Professional License',
												vendor = mdr.sourceTools.src_Certegy => 'Certegy',
												vendor = mdr.sourceTools.src_EMerge_Hunt=> 'Emerges Hunt',
												vendor = mdr.sourceTools.src_EMerge_Master=> 'Emerges Master',
												vendor = mdr.sourceTools.src_LnPropV2_Lexis_Deeds_Mtgs=> 'LN Property Deeds Mortgages',
												vendor = mdr.sourceTools.src_LnPropV2_Lexis_Asrs=> 'LN Property Assessor',
												vendor = mdr.sourceTools.src_KY_Watercraft=> 'KY Watercraft',
												vendor = mdr.sourceTools.src_EMerge_Boat=> 'Emerges Boat',
												vendor = mdr.sourceTools.src_VA_Watercraft=> 'VA Watercraft',
												vendor = mdr.sourceTools.src_NC_Watercraft=> 'NC Watercraft',
												vendor = mdr.sourceTools.src_TUCS_Ptrack=> 'TUCS Ptrack',
												vendor = mdr.sourceTools.src_EMerge_Cens=> 'Emerges Cens',
												vendor = mdr.sourceTools.src_Federal_Firearms=> 'Federal Firearms',
												vendor = mdr.sourceTools.src_EMerge_Fish=> 'Emerges Fish',
												vendor = mdr.sourceTools.src_Federal_Explosives=> 'Federal Explosives',
												vendor = mdr.sourceTools.src_MD_Watercraft=> 'MD Watercraft',
												vendor = mdr.sourceTools.src_Miscellaneous=> 'Misc',
												vendor = mdr.sourceTools.src_MO_Veh=> 'MO Vehicles',
												vendor = mdr.sourceTools.src_MO_DL=> 'MO Drivers License',
												vendor = mdr.sourceTools.src_MO_Experian_Veh=> 'MO Experian Vehicles',
												vendor = mdr.sourceTools.src_Experian_Credit_Header=> 'Experian Credit Header',
												vendor = mdr.sourceTools.src_MO_Watercraft=> 'MO Watercraft',
												vendor = mdr.sourceTools.src_ZUtil_Work_Phone=> 'Z Utilities Work Phone',
												vendor = mdr.sourceTools.src_ZUtilities=> 'Z Utilities',
												vendor = mdr.sourceTools.src_TU_CreditHeader=> 'TransUnion Credit Header',
												'Header');

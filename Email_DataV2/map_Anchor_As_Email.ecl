IMPORT Anchor, address, ut, emailservice, mdr, _validate, Entiera, lib_StringLib, std, Email_Data;

EXPORT map_Anchor_As_Email(version) := FUNCTION

with_email := Anchor.Files.Base_BIP(TRIM(EmailAddress) <> '');

//apply macro to obtain email domain fields
emailservice.mac_append_domain_flags(with_email,domain_d,EmailAddress);
			
//************Transform to a common email layout
Email_DataV2.Layouts.Base_BIP t_map_to_common (domain_d input) := TRANSFORM
	SELF.email_src        					:= mdr.sourceTools.src_Anchor;
	SELF.current_rec      					:= input.current_rec;
	SELF.activecode     						:= '';
	SELF.did_type         					:= '';
	SELF.orig_pmghousehold_id  			:= '';
	SELF.orig_pmgindividual_id 			:= '';
	SELF.orig_First_Name  					:= STD.Str.CleanSpaces(input.FirstName);
	SELF.orig_Last_Name  						:= STD.Str.CleanSpaces(input.LastName);
	SELF.Orig_Address  							:= IF(TRIM(input.Address_2,LEFT,RIGHT) = '', STD.Str.CleanSpaces(input.Address_1),
																				STD.Str.CleanSpaces(input.Address_1 +' '+ input.Address_2));
	SELF.Orig_City  								:= input.city;
	SELF.Orig_State  								:= input.state;
	SELF.orig_ZIP  									:= input.ZipCode;
	SELF.orig_zip4  								:= '';
	SELF.orig_email 								:= STD.Str.CleanSpaces(input.EmailAddress);
	SELF.orig_ip  									:= STD.Str.CleanSpaces(input.IPAddress);
	SELF.orig_site  								:= STD.Str.CleanSpaces(input.SourceURL);
	SELF.orig_e360_id  							:= '';
	...
	SELF.orig_CompanyName						:= IF(TRIM(input.clean_cname) != '',STD.Str.CleanSpaces(input.FirstName+' '+input.LastName),'');
	SELF.cln_CompanyName						:= STD.Str.CleanSpaces(input.clean_cname);
	SELF.rules											:= 0;
	SELF.global_sid                 := 27161;
	SELF := input;
	SELF := [];
END;

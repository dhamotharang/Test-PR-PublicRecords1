IMPORT Infutor_NARE, address, ut,emailservice, mdr, _validate, Entiera, lib_StringLib, std, Email_Data;

EXPORT map_InfutorNARE_AS_Email (version) := FUNCTION

with_email := Infutor_NARE.file_base(TRIM(email) <> '');
...
Email_DataV2.Layouts.Base_BIP t_map_to_common (domain_d input) := TRANSFORM
	...
	SELF.current_rec      					:= IF(input.current_rec_flag = 'C', TRUE,FALSE);
	SELF.activecode     						:= '';
	SELF.did_type         					:= '';
	...
	SELF.orig_name_suffix						:= ut.CleanSpacesAndUpper(input.Suffix);
	SELF.Orig_Address  							:= ut.CleanSpacesAndUpper(input.AddrLine1);
	...
	SELF.clean_ssn 									:= '';
	SELF.clean_dob 									:= (integer)STD.date.ConvertDateFormat(input.dob,'%Y%m%d', '%Y%m%d'); //blanks out dob that are not full dob's

IMPORT Acquireweb_Plus, ut, emailservice, mdr, _validate, entiera, STD, Email_Data;

EXPORT Map_AcquireWebPlus_As_Email(version) := FUNCTION

with_email := Acquireweb_Plus.files.Business_Base(TRIM(email)<> '');
...
Email_DataV2.Layouts.Base_BIP t_map_to_common (domain_d input) := TRANSFORM
	SELF.email_src        					:= mdr.sourceTools.src_Acquiredweb_plus;
	...
	SELF.Orig_Address  							:= STD.Str.CleanSpaces(input.address1 + ' ' + input.address2);
	...
	SELF.email_rec_key 							:= HASH64((data)TRIM(SELF.orig_first_name, LEFT, RIGHT) +
																						...
																						(data)TRIM(input.CompanyName, LEFT, RIGHT) +
																						(data)TRIM(SELF.email_src, LEFT, RIGHT));
																						
	SELF.orig_CompanyName						:= input.CompanyName;
	SELF.cln_CompanyName						:= input.clean_cname;
	SELF.CompanyTitle								:= input.Title;
	SELF.rules											:= 0;
	SELF := input;
	SELF := [];
END;

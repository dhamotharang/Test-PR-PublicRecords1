export macro_run_bs_4_1_cert_nonfcra(archive_date) := macro
...
outfile_name    := '~nmontpetit::out::bs_41_cert_tracking';

today := ut.GetDate[3..];
outfile_name_full   := outfile_name + '_full_nonfcra_' + today + '_' + (string)archive_date;
outfile_name_edina  := outfile_name + '_edina_nonfcra_' + today + '_' + (string)archive_date;
...
OUTPUT (res, , outfile_name_full, CSV(QUOTE('"')), overwrite);
...
f := dataset(outfile_name_full, ox, csv(quote('"'), maxlength(20000)));
...
output(edina,, outfile_name_edina, CSV(QUOTE('"')), overwrite);
endmacro;
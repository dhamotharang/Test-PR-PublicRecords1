EXPORT macro_run_rva_3_0_prescreen(archive_date) := macro
...
f := dataset('~nmontpetit::in::rva_prescreen_tracking_pii',prii_layout, csv(quote('"')));
...
output(final,,'~nmontpetit::out::rva_30_pre_tracking_' + today + '_' + (string)archive_date,CSV(heading(single), quote('"')), overwrite);
output(final(errorcode<>''),,'~nmontpetit::out::rva_30_pre_tracking_err',CSV(QUOTE('"')), overwrite);

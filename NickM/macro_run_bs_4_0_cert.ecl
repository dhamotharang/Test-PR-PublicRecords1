export macro_run_bs_4_0_cert() := macro
...
neutral_roxieIP := RiskWise.Shortcuts.staging_neutral_roxieIP;
roxieIP_fcra := RiskWise.Shortcuts.staging_fcra_roxieIP;
roxieIP_nonfcra :=  RiskWise.Shortcuts.staging_neutral_roxieIP;

//input_file := '~nmontpetit::in::first_inv_857_bs_input';
input_file := '~nmontpetit::in::ge_cap_100302_pii';
output_file := '~nmontpetit::out::bs_40_cert_tracking';
...
unsigned1 glba := 1;
unsigned1 dppa := 3;
string nonFCRADataRestrictionMask 		:= '00000000000001';
string fcraDataRestrictionMask 	        := '10000100010001';
string history_date_archive := '201003';
...
l t_f_nonfcra_arch(p_f le) := TRANSFORM
	...
	SELF.datarestrictionmask := nonfcradatarestrictionmask;
...
s_fcra_arch := Risk_Indicators.test_BocaShell_SoapCall (				PROJECT (p_f_fcra_arch, 
...
s_fcra_curr := Risk_Indicators.test_BocaShell_SoapCall (				PROJECT (p_f_fcra_curr, 
																		TRANSFORM (Risk_Indicators.Layout_InstID_SoapCall, 
																		SELF := LEFT)),
                                                bs_service_fcra, 
												roxieIP_FCRA, 
...
s_nonfcra_arch := Risk_Indicators.test_BocaShell_SoapCall (				PROJECT (p_f_nonfcra_arch, 
																		TRANSFORM (Risk_Indicators.Layout_InstID_SoapCall, 
																		SELF := LEFT)),
                                                bs_service_nonfcra, 
												roxieIP_nonfcra, 
...
s_nonfcra_curr := Risk_Indicators.test_BocaShell_SoapCall (				PROJECT (p_f_nonfcra_curr, 
																		TRANSFORM (Risk_Indicators.Layout_InstID_SoapCall, 
																		SELF := LEFT)),
                                                bs_service_nonfcra, 
												roxieIP_nonfcra, 
...
ox getold(s_fcra_arch le, l ri) :=	TRANSFORM
  SELF.AccountNumber := ri.old_account_number;
  SELF := le;
END;
	
res_fcra_arch := 	JOIN (s_fcra_arch,    p_f, LEFT.seq=(unsigned)RIGHT.accountnumber,getold(LEFT,RIGHT));
res_fcra_curr := 	JOIN (s_fcra_curr,    p_f, LEFT.seq=(unsigned)RIGHT.accountnumber,getold(LEFT,RIGHT));
res_nonfcra_arch := JOIN (s_nonfcra_arch, p_f, LEFT.seq=(unsigned)RIGHT.accountnumber,getold(LEFT,RIGHT));
res_nonfcra_curr := JOIN (s_nonfcra_curr, p_f, LEFT.seq=(unsigned)RIGHT.accountnumber,getold(LEFT,RIGHT));

OUTPUT (res_fcra_arch, ,    output_file + '_full_fcra_' + 	  today + '_' + history_date_archive, CSV(QUOTE('"')), overwrite);
OUTPUT (res_fcra_curr, ,    output_file + '_full_fcra_' + 	  today + '_' + '999999', CSV(QUOTE('"')), overwrite);
OUTPUT (res_nonfcra_arch, , output_file + '_full_nonfcra_' + 	today + '_' + history_date_archive, CSV(QUOTE('"')), overwrite);
OUTPUT (res_nonfcra_curr, , output_file + '_full_nonfcra_' + 	today + '_' + '999999', CSV(QUOTE('"')), overwrite);
...
risk_indicators.Layout_Boca_Shell_Edina_v4 convertToEdina_v4(res_fcra_arch le) := transform
...
edina_fcra_arch_temp := 	project(res_fcra_arch, 		convertToEdina_v4(left));
edina_fcra_curr_temp := 	project(res_fcra_curr, 		convertToEdina_v4(left));
edina_nonfcra_arch_temp := 	project(res_nonfcra_arch, 	convertToEdina_v4(left));
edina_nonfcra_curr_temp := 	project(res_nonfcra_curr, 	convertToEdina_v4(left));

risk_indicators.Layout_Boca_Shell_Edina_v4 set_fcra(risk_indicators.Layout_Boca_Shell_Edina_v4 le) := transform
	self.fd_scores := [];	
	self.cb_allowed := map(	FCRAdataRestrictionMask[6]<>'1' and FCRAdataRestrictionMask[8]<>'1' and FCRAdataRestrictionMask[10]<>'1' => 'ALL',
								FCRAdataRestrictionMask[8]<>'1' => 'EQ',
								FCRAdataRestrictionMask[6]<>'1' => 'EN',
								FCRAdataRestrictionMask[10]<>'1' => 'TN',
								'NONE');		// this should not happen
		self := le;
end;

risk_indicators.Layout_Boca_Shell_Edina_v4 set_nonfcra(risk_indicators.Layout_Boca_Shell_Edina_v4 le) := transform
	self.rv_scores := [];	
	self.isFCRA := '0';
	self.cb_allowed := map(	nonFCRAdataRestrictionMask[6]<>'1' and nonFCRAdataRestrictionMask[8]<>'1' and nonFCRAdataRestrictionMask[10]<>'1' => 'ALL',
								nonFCRAdataRestrictionMask[8]<>'1' => 'EQ',
								nonFCRAdataRestrictionMask[6]<>'1' => 'EN',
								nonFCRAdataRestrictionMask[10]<>'1' => 'TN',
								'NONE');		// this should not happen
    self := le;
end;

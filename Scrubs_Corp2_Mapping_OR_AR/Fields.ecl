...
EXPORT SALT32.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_charter_nbr','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_ar_type');
EXPORT FieldTypeNum(SALT32.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_charter_nbr' => 2,'invalid_corp_vendor' => 3,'invalid_state_origin' => 4,'invalid_mandatory' => 5,'invalid_date' => 6,'invalid_ar_type' => 7,0);
...
EXPORT MakeFT_invalid_corp_vendor(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_vendor(SALT32.StrType s) := WHICH(((SALT32.StrType) s) NOT IN ['41']);
EXPORT InValidMessageFT_invalid_corp_vendor(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInEnum('41'),SALT32.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_state_origin(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_state_origin(SALT32.StrType s) := WHICH(((SALT32.StrType) s) NOT IN ['OR']);
EXPORT InValidMessageFT_invalid_state_origin(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInEnum('OR'),SALT32.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_ar_type(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_ar_type(SALT32.StrType s) := WHICH(((SALT32.StrType) s) NOT IN ['ANNUAL REPORT','ANNUAL REPORT PAYMENT','AMENDED ANNUAL REPORT','AMNDMT TO ANNUAL RPT/INFO STATEMENT','CORRECTION OF ANNUAL','IMAGE ANNUAL REPORT','NOTICE LATE ANNUAL']);
EXPORT InValidMessageFT_invalid_ar_type(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInEnum('ANNUAL REPORT|ANNUAL REPORT PAYMENT|AMENDED ANNUAL REPORT|AMNDMT TO ANNUAL RPT/INFO STATEMENT|CORRECTION OF ANNUAL|IMAGE ANNUAL REPORT|NOTICE LATE ANNUAL'),SALT32.HygieneErrors.Good);
...
EXPORT Make_ar_type(SALT32.StrType s0) := MakeFT_invalid_ar_type(s0);
EXPORT InValid_ar_type(SALT32.StrType s) := InValidFT_invalid_ar_type(s);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT32,Scrubs_Corp2_Mapping_OR_AR;

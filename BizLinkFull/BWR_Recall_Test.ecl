#workunit('name','BIP External Append Testing');
...
macSamples(s):=MACRO
  ...
  %dMissing%:=PROJECT(dBatchJoined,TRANSFORM(
    ...
    sSearchParameters:=REGEXREPLACE(' ','uniqueid='+LEFT.rid+
      IF(LEFT.company_name='','','&copmany_name='+TRIM(LEFT.company_name))+
  ...
  %dHighPairs%:=JOIN(dBatchResult(Results[1].score>10 AND Results[2].score>10),%dAmbiguous%,LEFT.reference=RIGHT.rid,TRANSFORM(
    {STRING proxid_01__html;STRING proxid_02__html;STRING proxid_compare__html;STRING seleid_compare__html;UNSIGNED1 weight_01;UNSIGNED1 score_01;UNSIGNED1 weight_02;UNSIGNED1 score_02;STRING address1__html;STRING address2__html;},
    ...
    SELF.seleid_compare__html:=BIPV2.Hyperlink().LGIDCompare(LEFT.Results[1].seleid,LEFT.Results[2].seleid);

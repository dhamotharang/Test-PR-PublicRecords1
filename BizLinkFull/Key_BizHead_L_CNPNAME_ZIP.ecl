...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  J0 := JOIN(Recs2,Key,((HASH32(SALT44.fn_bow_bestword(LEFT.cnp_name))=RIGHT.gss_hash AND SALT44.MatchBagOfWords(RIGHT.cnp_name,LEFT.cnp_name,3177747,1) > BizLinkFull.Config_BIP.cnp_name_Force * 100))
  ...
  J1 := JOIN(Recs2,PULL(Key),((HASH32(SALT44.fn_bow_bestword(LEFT.cnp_name))=RIGHT.gss_hash AND SALT44.MatchBagOfWords(RIGHT.cnp_name,LEFT.cnp_name,3177747,1) > BizLinkFull.Config_BIP.cnp_name_Force * 100))
  AND ((RIGHT.zip = LEFT.zip_cases[1].zip))
  AND ((LEFT.prim_name = (TYPEOF(RIGHT.prim_name))'' OR RIGHT.prim_name = (TYPEOF(RIGHT.prim_name))'') OR (RIGHT.prim_name = LEFT.prim_name) OR ((Config_BIP.WithinEditN(RIGHT.prim_name,RIGHT.prim_name_len,LEFT.prim_name,LEFT.prim_name_len,1, 0))))
  AND ((LEFT.st = (TYPEOF(RIGHT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'') OR (RIGHT.st = LEFT.st)),Score_Batch(RIGHT,LEFT),
    ATMOST(((HASH32(SALT44.fn_bow_bestword(LEFT.cnp_name))=RIGHT.gss_hash))

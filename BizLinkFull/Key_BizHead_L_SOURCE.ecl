...
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.source_record_id <> (TYPEOF(le.source_record_id))'' AND Fields.InValid_source_record_id((SALT37.StrType)le.source_record_id)=0 AND le.source <> (TYPEOF(le.source))'' AND Fields.InValid_source((SALT37.StrType)le.source)=0;
KeyRec := RECORDOF(Key);
...
EXPORT RawFetch_server(TYPEOF(h.source_record_id) param_source_record_id = (TYPEOF(h.source_record_id))'',TYPEOF(h.source) param_source = (TYPEOF(h.source))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.prim_name_len) param_prim_name_len = (TYPEOF(h.prim_name_len))'',DATASET(process_Biz_layouts.layout_zip_cases) param_zip,TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.city_len) param_city_len = (TYPEOF(h.city_len))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := 
    STEPPED( LIMIT( Key(
      ...
      AND ( city = (TYPEOF(city))'' OR param_city = (TYPEOF(city))'' OR metaphonelib.DMetaPhone1(city)=metaphonelib.DMetaPhone1(param_city) OR Config_BIP.WithinEditN(city,city_len,param_city,param_city_len,2, 0) )
...
le.zip = (TYPEOF(le.zip))'' OR ~EXISTS(ri.zip_cases) => SALT37.MatchCode.OneSideNull,
  ...
  J0 := JOIN(Recs1,Key,LEFT.source_record_id = RIGHT.source_record_id
    ...
    ATMOST(LEFT.source_record_id = RIGHT.source_record_id
     AND LEFT.source = RIGHT.source,Config_BIP.L_SOURCE_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.source_record_id = RIGHT.source_record_id
     ...
     AND ( LEFT.st = (TYPEOF(LEFT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'' OR LEFT.st = RIGHT.st  ),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.source_record_id = RIGHT.source_record_id
     AND LEFT.source = RIGHT.source,Config_BIP.L_SOURCE_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);

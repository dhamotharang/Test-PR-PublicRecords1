...
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.source_record_id <> (TYPEOF(le.source_record_id))'' AND Fields.InValid_source_record_id((SALT311.StrType)le.source_record_id)=0 AND le.source <> (TYPEOF(le.source))'' AND Fields.InValid_source((SALT311.StrType)le.source)=0;
KeyRec := RECORDOF(Key);

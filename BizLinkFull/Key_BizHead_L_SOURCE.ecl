...
EXPORT KeyName := BizLinkFull.Filename_keys.L_SOURCE; /*HACK07*/
...
EXPORT RawFetch_server(TYPEOF(h.source_record_id) param_source_record_id = (TYPEOF(h.source_record_id))'',TYPEOF(h.source) param_source = (TYPEOF(h.source))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.prim_name_len) param_prim_name_len = (TYPEOF(h.prim_name_len))'',DATASET(Process_Biz_Layouts.layout_zip_cases) param_zip,TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.city_len) param_city_len = (TYPEOF(h.city_len))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'',UNSIGNED4 param_efr_bitmap = 0) := 
    STEPPED( LIMIT( Key(
      ...
      AND ( param_efr_bitmap=0 OR (EFR_BMap & param_efr_bitmap)>0 )),Config_BIP.L_SOURCE_MAXBLOCKLIMIT,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),ultid,orgid,seleid,proxid);
...
// Why not LOOP? - Because I am expecting FIRST one to win 99+ percent of the time - and don't want to impact it
  RawData0 := RawFetch_server(param_source_record_id,param_source,param_cnp_name,param_prim_name,param_prim_name_len,param_zip,param_city,param_city_len,param_st,0,param_efr_bitmap);
  RawData1 := RawFetch_server(param_source_record_id,param_source,param_cnp_name,param_prim_name,param_prim_name_len,param_zip,param_city,param_city_len,param_st,1,param_efr_bitmap);
  RawData2 := RawFetch_server(param_source_record_id,param_source,param_cnp_name,param_prim_name,param_prim_name_len,param_zip,param_city,param_city_len,param_st,2,param_efr_bitmap);
  Returnable(DATASET(RECORDOF(RawData0)) d) := COUNT(NOFOLD(d))<>1 OR EXISTS(NOFOLD(d((TYPEOF(source_record_id))source_record_id != (TYPEOF(source_record_id))'')));
  res := MAP (
...
EXPORT ScoredproxidFetch(TYPEOF(h.source_record_id) param_source_record_id = (TYPEOF(h.source_record_id))'',TYPEOF(h.source) param_source = (TYPEOF(h.source))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.prim_name_len) param_prim_name_len = (TYPEOF(h.prim_name_len))'',DATASET(Process_Biz_Layouts.layout_zip_cases) param_zip,TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.city_len) param_city_len = (TYPEOF(h.city_len))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.company_sic_code1) param_company_sic_code1 = (TYPEOF(h.company_sic_code1))'',TYPEOF(h.cnp_number) param_cnp_number = (TYPEOF(h.cnp_number))'',TYPEOF(h.cnp_btype) param_cnp_btype = (TYPEOF(h.cnp_btype))'',TYPEOF(h.cnp_lowv) param_cnp_lowv = (TYPEOF(h.cnp_lowv))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.prim_range_len) param_prim_range_len = (TYPEOF(h.prim_range_len))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.sec_range_len) param_sec_range_len = (TYPEOF(h.sec_range_len))'',TYPEOF(h.parent_proxid) param_parent_proxid = (TYPEOF(h.parent_proxid))'',TYPEOF(h.sele_proxid) param_sele_proxid = (TYPEOF(h.sele_proxid))'',TYPEOF(h.org_proxid) param_org_proxid = (TYPEOF(h.org_proxid))'',TYPEOF(h.ultimate_proxid) param_ultimate_proxid = (TYPEOF(h.ultimate_proxid))'',TYPEOF(h.sele_flag) param_sele_flag = (TYPEOF(h.sele_flag))'',TYPEOF(h.org_flag) param_org_flag = (TYPEOF(h.org_flag))'',TYPEOF(h.ult_flag) param_ult_flag = (TYPEOF(h.ult_flag))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'',UNSIGNED4 param_efr_bitmap = 0,BOOLEAN param_disableForce = FALSE) := FUNCTION
  RawData := RawFetch(param_source_record_id,param_source,param_cnp_name,param_prim_name,param_prim_name_len,param_zip,param_city,param_city_len,param_st,param_fallback_value,param_efr_bitmap);
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.source_record_id_match_code := match_methods(File_BizHead).match_source_record_id(le.source_record_id,ri.source_record_id,TRUE);
 
    SELF.source_record_idWeight := (50+MAP (
         SELF.source_record_id_match_code = SALT44.MatchCode.ExactMatch =>le.source_record_id_weight100,
         -0.244*le.source_record_id_weight100))/100;
 
    SELF.source_match_code := match_methods(File_BizHead).match_source(le.source,ri.source,TRUE);
  ...
  J0 := JOIN(Recs1,Key,((RIGHT.source_record_id = LEFT.source_record_id))
  ...
  AND ((LEFT.st = (TYPEOF(RIGHT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'') OR (RIGHT.st = LEFT.st)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.source_record_id = LEFT.source_record_id))
      AND ((RIGHT.source = LEFT.source)),Config_BIP.L_SOURCE_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((RIGHT.source_record_id = LEFT.source_record_id))
  AND ((RIGHT.source = LEFT.source))
  AND ((LEFT.cnp_name = (TYPEOF(RIGHT.cnp_name))'' OR RIGHT.cnp_name = (TYPEOF(RIGHT.cnp_name))'') OR (SALT44.MatchBagOfWords(RIGHT.cnp_name,LEFT.cnp_name,3177747,1) > BizLinkFull.Config_BIP.cnp_name_Force * 100))
  AND ((LEFT.prim_name = (TYPEOF(RIGHT.prim_name))'' OR RIGHT.prim_name = (TYPEOF(RIGHT.prim_name))'') OR (RIGHT.prim_name = LEFT.prim_name) OR ((Config_BIP.WithinEditN(RIGHT.prim_name,RIGHT.prim_name_len,LEFT.prim_name,LEFT.prim_name_len,1, 0))))
  AND ((~EXISTS(LEFT.zip_cases) OR RIGHT.zip = (TYPEOF(RIGHT.zip))'') OR (RIGHT.zip IN SET(LEFT.zip_cases,zip)))
  ...
  AND ((LEFT.st = (TYPEOF(RIGHT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'') OR (RIGHT.st = LEFT.st)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.source_record_id = LEFT.source_record_id))
      AND ((RIGHT.source = LEFT.source)),Config_BIP.L_SOURCE_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_source_record_id='',Input_source='',Input_cnp_name='',Input_prim_name='',Input_zip='',Input_city='',Input_st='',Input_company_sic_code1='',Input_cnp_number='',Input_cnp_btype='',Input_cnp_lowv='',Input_prim_range='',Input_sec_range='',Input_parent_proxid='',Input_sele_proxid='',Input_org_proxid='',Input_ultimate_proxid='',Input_sele_flag='',Input_org_flag='',Input_ult_flag='',output_file,AsIndex='true', In_disableForce = 'false') := MACRO
  ...
  #IF(#TEXT(Input_source_record_id)<>'' AND #TEXT(Input_source)<>'')
    ...
    output_file := BizLinkFull.Key_BizHead_L_SOURCE.ScoredFetch_Batch(%p%,AsIndex, In_disableForce);
  #ELSE

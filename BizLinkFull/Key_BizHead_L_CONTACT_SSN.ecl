...
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.contact_ssn <> (TYPEOF(le.contact_ssn))'' AND Fields.InValid_contact_ssn((SALT37.StrType)le.contact_ssn)=0;
KeyRec := RECORDOF(Key);
...
le.zip = (TYPEOF(le.zip))'' OR ~EXISTS(ri.zip_cases) => SALT37.MatchCode.OneSideNull,
  ...
  J0 := JOIN(Recs1,Key,LEFT.contact_ssn = RIGHT.contact_ssn,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.contact_ssn = RIGHT.contact_ssn,Config_BIP.L_CONTACT_SSN_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.contact_ssn = RIGHT.contact_ssn,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.contact_ssn = RIGHT.contact_ssn,Config_BIP.L_CONTACT_SSN_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);

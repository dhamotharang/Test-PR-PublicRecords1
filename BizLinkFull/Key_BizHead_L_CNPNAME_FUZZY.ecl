...
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.company_name_prefix <> (TYPEOF(le.company_name_prefix))'' AND Fields.InValid_company_name_prefix((SALT37.StrType)le.company_name_prefix)=0;
KeyRec := RECORDOF(Key);
...
le.zip = (TYPEOF(le.zip))'' OR ~EXISTS(ri.zip_cases) => SALT37.MatchCode.OneSideNull,
  ...
  J0 := JOIN(Recs1,Key,LEFT.company_name_prefix = RIGHT.company_name_prefix
    ...
    ATMOST(LEFT.company_name_prefix = RIGHT.company_name_prefix,Config_BIP.L_CNPNAME_FUZZY_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.company_name_prefix = RIGHT.company_name_prefix
     AND ( LEFT.cnp_name = (TYPEOF(LEFT.cnp_name))'' OR RIGHT.cnp_name = (TYPEOF(RIGHT.cnp_name))'' OR SALT37.MatchBagOfWords(LEFT.cnp_name,RIGHT.cnp_name,3177747,1) > BizLinkFull.Config_BIP.cnp_name_Force * 100 )
     AND ( LEFT.zip_cases[1].zip = (TYPEOF(LEFT.zip_cases[1].zip))'' OR RIGHT.zip = (TYPEOF(RIGHT.zip))'' OR LEFT.zip_cases[1].zip = RIGHT.zip  )
     AND ( LEFT.city = (TYPEOF(LEFT.city))'' OR RIGHT.city = (TYPEOF(RIGHT.city))''  OR metaphonelib.DMetaPhone1(LEFT.city)=metaphonelib.DMetaPhone1(RIGHT.city) OR BizLinkFull.Config_BIP.WithinEditN(LEFT.city,LEFT.city_len,RIGHT.city,RIGHT.city_len,2, 0)  )
     AND ( LEFT.st = (TYPEOF(LEFT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'' OR LEFT.st = RIGHT.st  ),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.company_name_prefix = RIGHT.company_name_prefix,Config_BIP.L_CNPNAME_FUZZY_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);

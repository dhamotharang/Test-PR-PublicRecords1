IMPORT SALT44,std;EXPORT Key_BizHead_L_CNPNAME_FUZZY := MODULE
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,proxid,company_name_prefix,cnp_name,st,zip,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'L_CNPNAME_FUZZY','city',CntRed_city*100,CntRed_city*TSize},{'L_CNPNAME_FUZZY','company_sic_code1',CntRed_company_sic_code1*100,CntRed_company_sic_code1*TSize},{'L_CNPNAME_FUZZY','cnp_number',CntRed_cnp_number*100,CntRed_cnp_number*TSize},{'L_CNPNAME_FUZZY','cnp_btype',CntRed_cnp_btype*100,CntRed_cnp_btype*TSize},{'L_CNPNAME_FUZZY','cnp_lowv',CntRed_cnp_lowv*100,CntRed_cnp_lowv*TSize},{'L_CNPNAME_FUZZY','prim_range',CntRed_prim_range*100,CntRed_prim_range*TSize},{'L_CNPNAME_FUZZY','sec_range',CntRed_sec_range*100,CntRed_sec_range*TSize},{'L_CNPNAME_FUZZY','parent_proxid',CntRed_parent_proxid*100,CntRed_parent_proxid*TSize},{'L_CNPNAME_FUZZY','sele_proxid',CntRed_sele_proxid*100,CntRed_sele_proxid*TSize},{'L_CNPNAME_FUZZY','org_proxid',CntRed_org_proxid*100,CntRed_org_proxid*TSize},{'L_CNPNAME_FUZZY','ultimate_proxid',CntRed_ultimate_proxid*100,CntRed_ultimate_proxid*TSize},{'L_CNPNAME_FUZZY','sele_flag',CntRed_sele_flag*100,CntRed_sele_flag*TSize},{'L_CNPNAME_FUZZY','org_flag',CntRed_org_flag*100,CntRed_org_flag*TSize},{'L_CNPNAME_FUZZY','ult_flag',CntRed_ult_flag*100,CntRed_ult_flag*TSize}],SALT44.ShrinkLayout);
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.company_name_prefix <> (TYPEOF(le.company_name_prefix))'' AND Fields.InValid_company_name_prefix((SALT44.StrType)le.company_name_prefix)=0;
KeyRec := RECORDOF(Key);
EXPORT RawFetch_server(TYPEOF(h.company_name_prefix) param_company_name_prefix = (TYPEOF(h.company_name_prefix))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',DATASET(Process_Biz_Layouts.layout_zip_cases) param_zip,TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'',UNSIGNED4 param_efr_bitmap = 0) := 
    STEPPED( LIMIT( Key(
...
// Why not LOOP? - Because I am expecting FIRST one to win 99+ percent of the time - and don't want to impact it
  RawData0 := RawFetch_server(param_company_name_prefix,param_cnp_name,param_st,param_zip,0,param_efr_bitmap);
  RawData1 := RawFetch_server(param_company_name_prefix,param_cnp_name,param_st,param_zip,1,param_efr_bitmap);
  RawData2 := RawFetch_server(param_company_name_prefix,param_cnp_name,param_st,param_zip,2,param_efr_bitmap);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_company_name_prefix='',Input_cnp_name='',Input_st='',Input_zip='',Input_city='',Input_company_sic_code1='',Input_cnp_number='',Input_cnp_btype='',Input_cnp_lowv='',Input_prim_range='',Input_sec_range='',Input_parent_proxid='',Input_sele_proxid='',Input_org_proxid='',Input_ultimate_proxid='',Input_sele_flag='',Input_org_flag='',Input_ult_flag='',output_file,AsIndex='true', In_disableForce = 'false') := MACRO

...
SHARED city_cleanDS := TABLE(SpecMod.city_clean_values_persisted, {SALT44.Str30Type prefix := SpecMod.city_clean_values_persisted.city_clean;	REAL8 specificity := SpecMod.city_clean_values_persisted.field_specificity;});
EXPORT Key_city_clean := INDEX(city_cleanDS,{prefix},{city_cleanDS},KeyName_city_clean);
EXPORT Fetch_city_clean_Regular(SALT44.StrType s, UNSIGNED n, UNSIGNED t) := TABLE(TOPN(Key_city_clean(prefix[1..length(s)] = s AND specificity<=t), n, specificity), {SALT44.StrType word_list := prefix, REAL8 specificity := specificity});
...
EXPORT FetchQuick_city_clean_Regular(SALT44.StrType s, UNSIGNED n, UNSIGNED t) := TABLE(TOPN(Key_city_clean(prefix[1..length(s)] = s AND specificity<=t), n, specificity), {SALT44.StrType word_list := prefix, REAL8 specificity := specificity});
EXPORT Fetch_city_clean(SALT44.StrType s, UNSIGNED n, UNSIGNED t) := FUNCTION

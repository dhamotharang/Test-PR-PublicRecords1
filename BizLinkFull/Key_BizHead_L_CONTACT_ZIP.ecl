IMPORT SALT44,std;
EXPORT Key_BizHead_L_CONTACT_ZIP := MODULE
//fname_preferred:lname:zip:?:mname:cnp_name:st:+:fname:company_sic_code1:cnp_number:cnp_btype:cnp_lowv:prim_name:city:prim_range:sec_range:parent_proxid:sele_proxid:org_proxid:ultimate_proxid:sele_flag:org_flag:ult_flag
EXPORT KeyName := BizLinkFull.Filename_keys.L_CONTACT_ZIP; /*HACK07*/
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,proxid,fname_preferred,lname,zip,mname,cnp_name,st,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'L_CONTACT_ZIP','fname',CntRed_fname*100,CntRed_fname*TSize},{'L_CONTACT_ZIP','company_sic_code1',CntRed_company_sic_code1*100,CntRed_company_sic_code1*TSize},{'L_CONTACT_ZIP','cnp_number',CntRed_cnp_number*100,CntRed_cnp_number*TSize},{'L_CONTACT_ZIP','cnp_btype',CntRed_cnp_btype*100,CntRed_cnp_btype*TSize},{'L_CONTACT_ZIP','cnp_lowv',CntRed_cnp_lowv*100,CntRed_cnp_lowv*TSize},{'L_CONTACT_ZIP','prim_name',CntRed_prim_name*100,CntRed_prim_name*TSize},{'L_CONTACT_ZIP','city',CntRed_city*100,CntRed_city*TSize},{'L_CONTACT_ZIP','prim_range',CntRed_prim_range*100,CntRed_prim_range*TSize},{'L_CONTACT_ZIP','sec_range',CntRed_sec_range*100,CntRed_sec_range*TSize},{'L_CONTACT_ZIP','parent_proxid',CntRed_parent_proxid*100,CntRed_parent_proxid*TSize},{'L_CONTACT_ZIP','sele_proxid',CntRed_sele_proxid*100,CntRed_sele_proxid*TSize},{'L_CONTACT_ZIP','org_proxid',CntRed_org_proxid*100,CntRed_org_proxid*TSize},{'L_CONTACT_ZIP','ultimate_proxid',CntRed_ultimate_proxid*100,CntRed_ultimate_proxid*TSize},{'L_CONTACT_ZIP','sele_flag',CntRed_sele_flag*100,CntRed_sele_flag*TSize},{'L_CONTACT_ZIP','org_flag',CntRed_org_flag*100,CntRed_org_flag*TSize},{'L_CONTACT_ZIP','ult_flag',CntRed_ult_flag*100,CntRed_ult_flag*TSize}],SALT44.ShrinkLayout);
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.fname_preferred <> (TYPEOF(le.fname_preferred))'' AND Fields.InValid_fname_preferred((SALT44.StrType)le.fname_preferred)=0 AND le.lname <> (TYPEOF(le.lname))'' AND Fields.InValid_lname((SALT44.StrType)le.lname)=0 AND EXISTS(le.zip_cases);
KeyRec := RECORDOF(Key);
EXPORT RawFetch_server(TYPEOF(h.fname_preferred) param_fname_preferred = (TYPEOF(h.fname_preferred))'',TYPEOF(h.lname) param_lname = (TYPEOF(h.lname))'',TYPEOF(h.lname_len) param_lname_len = (TYPEOF(h.lname_len))'',DATASET(Process_Biz_Layouts.layout_zip_cases) param_zip,TYPEOF(h.mname) param_mname = (TYPEOF(h.mname))'',TYPEOF(h.mname_len) param_mname_len = (TYPEOF(h.mname_len))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'',UNSIGNED4 param_efr_bitmap = 0) := 
    STEPPED( LIMIT( Key(
      ...
      AND ( param_efr_bitmap=0 OR (EFR_BMap & param_efr_bitmap)>0 )),Config_BIP.L_CONTACT_ZIP_MAXBLOCKLIMIT,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),ultid,orgid,seleid,proxid);
...
// Why not LOOP? - Because I am expecting FIRST one to win 99+ percent of the time - and don't want to impact it
  RawData0 := RawFetch_server(param_fname_preferred,param_lname,param_lname_len,param_zip,param_mname,param_mname_len,param_cnp_name,param_st,0,param_efr_bitmap);
  RawData1 := RawFetch_server(param_fname_preferred,param_lname,param_lname_len,param_zip,param_mname,param_mname_len,param_cnp_name,param_st,1,param_efr_bitmap);
  RawData2 := RawFetch_server(param_fname_preferred,param_lname,param_lname_len,param_zip,param_mname,param_mname_len,param_cnp_name,param_st,2,param_efr_bitmap);
...
EXPORT ScoredproxidFetch(TYPEOF(h.fname_preferred) param_fname_preferred = (TYPEOF(h.fname_preferred))'',TYPEOF(h.lname) param_lname = (TYPEOF(h.lname))'',TYPEOF(h.lname_len) param_lname_len = (TYPEOF(h.lname_len))'',DATASET(Process_Biz_Layouts.layout_zip_cases) param_zip,TYPEOF(h.mname) param_mname = (TYPEOF(h.mname))'',TYPEOF(h.mname_len) param_mname_len = (TYPEOF(h.mname_len))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.fname) param_fname = (TYPEOF(h.fname))'',TYPEOF(h.fname_len) param_fname_len = (TYPEOF(h.fname_len))'',TYPEOF(h.company_sic_code1) param_company_sic_code1 = (TYPEOF(h.company_sic_code1))'',TYPEOF(h.cnp_number) param_cnp_number = (TYPEOF(h.cnp_number))'',TYPEOF(h.cnp_btype) param_cnp_btype = (TYPEOF(h.cnp_btype))'',TYPEOF(h.cnp_lowv) param_cnp_lowv = (TYPEOF(h.cnp_lowv))'',TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.prim_name_len) param_prim_name_len = (TYPEOF(h.prim_name_len))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.city_len) param_city_len = (TYPEOF(h.city_len))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.prim_range_len) param_prim_range_len = (TYPEOF(h.prim_range_len))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.sec_range_len) param_sec_range_len = (TYPEOF(h.sec_range_len))'',TYPEOF(h.parent_proxid) param_parent_proxid = (TYPEOF(h.parent_proxid))'',TYPEOF(h.sele_proxid) param_sele_proxid = (TYPEOF(h.sele_proxid))'',TYPEOF(h.org_proxid) param_org_proxid = (TYPEOF(h.org_proxid))'',TYPEOF(h.ultimate_proxid) param_ultimate_proxid = (TYPEOF(h.ultimate_proxid))'',TYPEOF(h.sele_flag) param_sele_flag = (TYPEOF(h.sele_flag))'',TYPEOF(h.org_flag) param_org_flag = (TYPEOF(h.org_flag))'',TYPEOF(h.ult_flag) param_ult_flag = (TYPEOF(h.ult_flag))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'',UNSIGNED4 param_efr_bitmap = 0,BOOLEAN param_disableForce = FALSE) := FUNCTION
  RawData := RawFetch(param_fname_preferred,param_lname,param_lname_len,param_zip,param_mname,param_mname_len,param_cnp_name,param_st,param_fallback_value,param_efr_bitmap);
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF := le;
  END;
  Recs0 := Recs(fname_preferred <> (TYPEOF(fname_preferred))'',lname <> (TYPEOF(lname))'',EXISTS(zip_cases));
  ...
  J0 := JOIN(Recs2,Key,((RIGHT.fname_preferred = LEFT.fname_preferred))
  ...
  AND ((LEFT.st = (TYPEOF(RIGHT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'') OR (RIGHT.st = LEFT.st)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.fname_preferred = LEFT.fname_preferred))
      AND ((RIGHT.lname = LEFT.lname))
      AND ((RIGHT.zip = LEFT.zip_cases[1].zip)),Config_BIP.L_CONTACT_ZIP_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs2,PULL(Key),((RIGHT.fname_preferred = LEFT.fname_preferred))
  AND ((RIGHT.lname = LEFT.lname))
  AND ((RIGHT.zip = LEFT.zip_cases[1].zip))
  AND ((LEFT.mname = (TYPEOF(RIGHT.mname))'' OR RIGHT.mname = (TYPEOF(RIGHT.mname))'') OR (RIGHT.mname = LEFT.mname) OR ((RIGHT.mname[1..LENGTH(TRIM(LEFT.mname))] = LEFT.mname OR LEFT.mname[1..LENGTH(TRIM(RIGHT.mname))] = RIGHT.mname) OR (Config_BIP.WithinEditN(RIGHT.mname,RIGHT.mname_len,LEFT.mname,LEFT.mname_len,2, 0))))
  AND ((LEFT.cnp_name = (TYPEOF(RIGHT.cnp_name))'' OR RIGHT.cnp_name = (TYPEOF(RIGHT.cnp_name))'') OR (SALT44.MatchBagOfWords(RIGHT.cnp_name,LEFT.cnp_name,3177747,1) > BizLinkFull.Config_BIP.cnp_name_Force * 100))
  AND ((LEFT.st = (TYPEOF(RIGHT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'') OR (RIGHT.st = LEFT.st)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.fname_preferred = LEFT.fname_preferred))
      AND ((RIGHT.lname = LEFT.lname))
      AND ((RIGHT.zip = LEFT.zip_cases[1].zip)),Config_BIP.L_CONTACT_ZIP_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_fname_preferred='',Input_lname='',Input_zip='',Input_mname='',Input_cnp_name='',Input_st='',Input_fname='',Input_company_sic_code1='',Input_cnp_number='',Input_cnp_btype='',Input_cnp_lowv='',Input_prim_name='',Input_city='',Input_prim_range='',Input_sec_range='',Input_parent_proxid='',Input_sele_proxid='',Input_org_proxid='',Input_ultimate_proxid='',Input_sele_flag='',Input_org_flag='',Input_ult_flag='',output_file,AsIndex='true', In_disableForce = 'false') := MACRO
  IMPORT SALT44,BizLinkFull;
  #IF(#TEXT(Input_fname_preferred)<>'' AND #TEXT(Input_lname)<>'' AND #TEXT(Input_zip)<>'')
    #UNIQUENAME(trans)
    BizLinkFull.Key_BizHead_L_CONTACT_ZIP.InputLayout_Batch %trans%(InFile le) := TRANSFORM
    ...
    output_file := BizLinkFull.Key_BizHead_L_CONTACT_ZIP.ScoredFetch_Batch(%p%,AsIndex, In_disableForce);
  #ELSE

...
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.prim_name <> (TYPEOF(le.prim_name))'' AND Fields.InValid_prim_name((SALT311.StrType)le.prim_name)=0 AND EXISTS(le.zip_cases);
KeyRec := RECORDOF(Key);
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  SALT311.MAC_Dups_Note(Recs1,InputLayout_Batch,Recs2,outdups,Reference,Config_BIP.meow_dedup) // Whilst duplicates have been removed for the whole input; there may still be dups on a per linkpath basis

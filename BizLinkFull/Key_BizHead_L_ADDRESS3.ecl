...
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.prim_name <> (TYPEOF(le.prim_name))'' AND Fields.InValid_prim_name((SALT37.StrType)le.prim_name)=0 AND le.prim_range <> (TYPEOF(le.prim_range))'' AND Fields.InValid_prim_range((SALT37.StrType)le.prim_range)=0 AND EXISTS(le.zip_cases);
KeyRec := RECORDOF(Key);
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  J0 := JOIN(Recs1,Key,LEFT.prim_name = RIGHT.prim_name
    ...
    ATMOST(LEFT.prim_name = RIGHT.prim_name
     AND LEFT.prim_range = RIGHT.prim_range
     AND LEFT.zip_cases[1].zip = RIGHT.zip,Config_BIP.L_ADDRESS3_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.prim_name = RIGHT.prim_name
    ...
    ATMOST(LEFT.prim_name = RIGHT.prim_name
     AND LEFT.prim_range = RIGHT.prim_range
     AND LEFT.zip_cases[1].zip = RIGHT.zip,Config_BIP.L_ADDRESS3_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);

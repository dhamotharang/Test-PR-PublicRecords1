...
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.prim_name <> (TYPEOF(le.prim_name))'' AND Fields.InValid_prim_name((SALT311.StrType)le.prim_name)=0 AND le.prim_range <> (TYPEOF(le.prim_range))'' AND Fields.InValid_prim_range((SALT311.StrType)le.prim_range)=0 AND EXISTS(le.zip_cases);
KeyRec := RECORDOF(Key);

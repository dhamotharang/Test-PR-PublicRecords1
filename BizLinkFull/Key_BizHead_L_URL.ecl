...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.company_urlWeight := IF(SALT311.MatchBagOfWords(le.company_url,ri.company_url,31744,0)<=Config_BIP.company_url_force,SKIP,SALT311.MatchBagOfWords(le.company_url,ri.company_url,31744,0)/100); 
    SELF.st_match_code := MAP(
  ...
  J0 := JOIN(Recs1,Key,((HASH32(SALT311.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash AND SALT311.MatchBagOfWords(RIGHT.company_url,LEFT.company_url,31744,0) > BizLinkFull.Config_BIP.company_url_Force * 100))
     AND ((LEFT.st = (TYPEOF(RIGHT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'') OR (RIGHT.st = LEFT.st)),Score_Batch(RIGHT,LEFT),
    ATMOST(((HASH32(SALT311.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash)),Config_BIP.L_URL_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((HASH32(SALT311.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash AND SALT311.MatchBagOfWords(RIGHT.company_url,LEFT.company_url,31744,0) > BizLinkFull.Config_BIP.company_url_Force * 100))
     AND ((LEFT.st = (TYPEOF(RIGHT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'') OR (RIGHT.st = LEFT.st)),Score_Batch(RIGHT,LEFT),
    ATMOST(((HASH32(SALT311.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash)),Config_BIP.L_URL_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);

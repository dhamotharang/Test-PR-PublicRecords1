...
EXPORT KeyName := BizLinkFull.Filename_keys.L_URL; /*HACK07*/
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.company_urlWeight := IF(SALT33.MatchBagOfWords(le.company_url,ri.company_url,31744,0)<=Config_BIP.company_url_force,SKIP,SALT33.MatchBagOfWords(le.company_url,ri.company_url,31744,0)/100); 
  ...
  J0 := JOIN(Recs1,Key,HASH32(SALT33.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash AND SALT33.MatchBagOfWords(LEFT.company_url,RIGHT.company_url,31744,0) > Config_BIP.company_url_Force * 100
  ...
  J1 := JOIN(Recs1,PULL(Key),HASH32(SALT33.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash AND SALT33.MatchBagOfWords(LEFT.company_url,RIGHT.company_url,31744,0) > Config_BIP.company_url_Force * 100
     AND ( LEFT.st = (TYPEOF(LEFT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'' OR LEFT.st = RIGHT.st  ),Score_Batch(RIGHT,LEFT),
...
#IF(#TEXT(Input_company_url)<>'')
  ...
  output_file := BizLinkFull.Key_BizHead_L_URL.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

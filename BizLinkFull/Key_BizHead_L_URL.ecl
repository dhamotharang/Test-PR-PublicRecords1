...
EXPORT RawFetch(TYPEOF(h.company_url) param_company_url = (TYPEOF(h.company_url))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'',UNSIGNED4 param_efr_bitmap = 0) := FUNCTION
// Why not LOOP? - Because I am expecting FIRST one to win 99+ percent of the time - and don't want to impact it
  RawData0 := RawFetch_server(param_company_url,param_st,0,param_efr_bitmap);
  RawData1 := RawFetch_server(param_company_url,param_st,1,param_efr_bitmap);
  RawData2 := RawFetch_server(param_company_url,param_st,2,param_efr_bitmap);
  Returnable(DATASET(RECORDOF(RawData0)) d) := COUNT(NOFOLD(d))<>1 OR EXISTS(NOFOLD(d((TYPEOF(company_url))company_url != (TYPEOF(company_url))'')));
  res := MAP (
...
EXPORT ScoredproxidFetch(TYPEOF(h.company_url) param_company_url = (TYPEOF(h.company_url))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.company_sic_code1) param_company_sic_code1 = (TYPEOF(h.company_sic_code1))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.cnp_number) param_cnp_number = (TYPEOF(h.cnp_number))'',TYPEOF(h.cnp_btype) param_cnp_btype = (TYPEOF(h.cnp_btype))'',TYPEOF(h.cnp_lowv) param_cnp_lowv = (TYPEOF(h.cnp_lowv))'',DATASET(Process_Biz_Layouts.layout_zip_cases) param_zip,TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.prim_name_len) param_prim_name_len = (TYPEOF(h.prim_name_len))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.city_len) param_city_len = (TYPEOF(h.city_len))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.prim_range_len) param_prim_range_len = (TYPEOF(h.prim_range_len))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.sec_range_len) param_sec_range_len = (TYPEOF(h.sec_range_len))'',TYPEOF(h.parent_proxid) param_parent_proxid = (TYPEOF(h.parent_proxid))'',TYPEOF(h.sele_proxid) param_sele_proxid = (TYPEOF(h.sele_proxid))'',TYPEOF(h.org_proxid) param_org_proxid = (TYPEOF(h.org_proxid))'',TYPEOF(h.ultimate_proxid) param_ultimate_proxid = (TYPEOF(h.ultimate_proxid))'',TYPEOF(h.sele_flag) param_sele_flag = (TYPEOF(h.sele_flag))'',TYPEOF(h.org_flag) param_org_flag = (TYPEOF(h.org_flag))'',TYPEOF(h.ult_flag) param_ult_flag = (TYPEOF(h.ult_flag))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'',UNSIGNED4 param_efr_bitmap = 0,BOOLEAN param_disableForce = FALSE) := FUNCTION
  RawData := RawFetch(param_company_url,param_st,param_fallback_value,param_efr_bitmap);
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.company_urlWeight := IF(SALT44.MatchBagOfWords(le.company_url,ri.company_url,31744,0)<=Config_BIP.company_url_force,SKIP,SALT44.MatchBagOfWords(le.company_url,ri.company_url,31744,0)/100);
  ...
  J0 := JOIN(Recs1,Key,((HASH32(SALT44.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash AND SALT44.MatchBagOfWords(RIGHT.company_url,LEFT.company_url,31744,0) > BizLinkFull.Config_BIP.company_url_Force * 100))
  AND ((LEFT.st = (TYPEOF(RIGHT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'') OR (RIGHT.st = LEFT.st)),Score_Batch(RIGHT,LEFT),
    ATMOST(((HASH32(SALT44.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash)),Config_BIP.L_URL_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((HASH32(SALT44.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash AND SALT44.MatchBagOfWords(RIGHT.company_url,LEFT.company_url,31744,0) > BizLinkFull.Config_BIP.company_url_Force * 100))
  AND ((LEFT.st = (TYPEOF(RIGHT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'') OR (RIGHT.st = LEFT.st)),Score_Batch(RIGHT,LEFT),
    ATMOST(((HASH32(SALT44.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash)),Config_BIP.L_URL_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_company_url='',Input_st='',Input_company_sic_code1='',Input_cnp_name='',Input_cnp_number='',Input_cnp_btype='',Input_cnp_lowv='',Input_zip='',Input_prim_name='',Input_city='',Input_prim_range='',Input_sec_range='',Input_parent_proxid='',Input_sele_proxid='',Input_org_proxid='',Input_ultimate_proxid='',Input_sele_flag='',Input_org_flag='',Input_ult_flag='',output_file,AsIndex='true', In_disableForce = 'false') := MACRO
  ...
  #IF(#TEXT(Input_company_url)<>'')
    ...
    output_file := BizLinkFull.Key_BizHead_L_URL.ScoredFetch_Batch(%p%,AsIndex, In_disableForce);
  #ELSE

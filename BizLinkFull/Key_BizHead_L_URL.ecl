...
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.company_url <> (TYPEOF(le.company_url))'' AND Fields.InValid_company_url((SALT37.StrType)le.company_url)=0;
KeyRec := RECORDOF(Key);
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.company_urlWeight := IF(SALT37.MatchBagOfWords(le.company_url,ri.company_url,31744,0)<=Config_BIP.company_url_force,SKIP,SALT37.MatchBagOfWords(le.company_url,ri.company_url,31744,0)/100); 
    SELF.st_match_code := MAP(
...
le.zip = (TYPEOF(le.zip))'' OR ~EXISTS(ri.zip_cases) => SALT37.MatchCode.OneSideNull,
  ...
  J0 := JOIN(Recs1,Key,HASH32(SALT37.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash AND SALT37.MatchBagOfWords(LEFT.company_url,RIGHT.company_url,31744,0) > BizLinkFull.Config_BIP.company_url_Force * 100
    ...
    ATMOST(HASH32(SALT37.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash,Config_BIP.L_URL_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),HASH32(SALT37.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash AND SALT37.MatchBagOfWords(LEFT.company_url,RIGHT.company_url,31744,0) > BizLinkFull.Config_BIP.company_url_Force * 100
     AND ( LEFT.st = (TYPEOF(LEFT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'' OR LEFT.st = RIGHT.st  ),Score_Batch(RIGHT,LEFT),
    ATMOST(HASH32(SALT37.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash,Config_BIP.L_URL_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);

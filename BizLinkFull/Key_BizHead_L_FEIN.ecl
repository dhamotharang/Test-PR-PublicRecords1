...
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.company_fein <> (TYPEOF(le.company_fein))'' AND Fields.InValid_company_fein((SALT37.StrType)le.company_fein)=0;
KeyRec := RECORDOF(Key);
...
le.zip = (TYPEOF(le.zip))'' OR ~EXISTS(ri.zip_cases) => SALT37.MatchCode.OneSideNull,
  ...
  J0 := JOIN(Recs1,Key,LEFT.company_fein = RIGHT.company_fein,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.company_fein = RIGHT.company_fein,Config_BIP.L_FEIN_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.company_fein = RIGHT.company_fein,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.company_fein = RIGHT.company_fein,Config_BIP.L_FEIN_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);

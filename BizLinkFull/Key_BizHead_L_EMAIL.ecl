...
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.contact_email <> (TYPEOF(le.contact_email))'' AND Fields.InValid_contact_email((SALT37.StrType)le.contact_email)=0;
KeyRec := RECORDOF(Key);
...
le.zip = (TYPEOF(le.zip))'' OR ~EXISTS(ri.zip_cases) => SALT37.MatchCode.OneSideNull,
  ...
  J0 := JOIN(Recs1,Key,LEFT.contact_email = RIGHT.contact_email,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.contact_email = RIGHT.contact_email,Config_BIP.L_EMAIL_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.contact_email = RIGHT.contact_email,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.contact_email = RIGHT.contact_email,Config_BIP.L_EMAIL_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);

...
EXPORT LayoutScoredFetch update_forcefailed(LayoutScoredFetch le, BOOLEAN disableForce = FALSE) := TRANSFORM
  SELF.ForceFailed := IF(disableForce, FALSE, (le.prim_rangeWeight < Config_BIP.prim_range_Force)/*HACK03_a*/ OR le.cnp_nameWeight < BizLinkFull.Config_BIP.HACK03_a_forceCnpnameWeight);
  SELF := le;
END;
...
EXPORT LayoutScoredFetch combine_scores(LayoutScoredFetch le,LayoutScoredFetch ri, BOOLEAN In_disableForce = FALSE) := TRANSFORM
  ...
  SELF.ForceFailed := IF(In_disableForce, FALSE, (SELF.prim_rangeWeight < Config_BIP.prim_range_Force)/*HACK03_b*/ OR SELF.cnp_nameWeight < BizLinkFull.Config_BIP.HACK03_b_forceCnpnameWeight);
...
EXPORT CombineAllScores(DATASET(LayoutScoredFetch) in_data, BOOLEAN In_bGetAllScores = TRUE, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  SALT44.MAC_External_AddPcnt(R7,LayoutScoredFetch,Results,OutputLayout_Batch,27,R8);
  SALT44.MAC_External_AddPcnt(R8,LayoutScoredFetch,Results_seleid,OutputLayout_Batch,27,R9);
  SALT44.MAC_External_AddPcnt(R9,LayoutScoredFetch,Results_orgid,OutputLayout_Batch,27,R10);
  SALT44.MAC_External_AddPcnt(R10,LayoutScoredFetch,Results_ultid,OutputLayout_Batch,27,R11);
  SALT44.MAC_External_AddPcnt(R11,LayoutScoredFetch,Results_powid,OutputLayout_Batch,27,R12);
  	 RETURN r12;
END;

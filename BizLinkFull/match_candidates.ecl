...
pj20 do_computes(pj20 le) := TRANSFORM
  ...
  SELF := le;
END;
SHARED propogated := PROJECT(pj20,do_computes(left)) : PERSIST('~temp::proxid::BizLinkFull::mc_props::BizHead',EXPIRE(Config_BIP.PersistExpire)); // to allow to 'jump' over an exported value
...
SHARED Annotated := DISTRIBUTE(j0,hash(proxid)) : PERSIST('~temp::proxid::BizLinkFull::mc',EXPIRE(Config_BIP.PersistExpire)); // Distributed for keybuild case

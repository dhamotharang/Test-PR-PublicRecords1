...
SALT311.mac_prop_field(thin_table(cnp_btype NOT IN SET(s.nulls_cnp_btype,cnp_btype)),cnp_btype,proxid,cnp_btype_props); // For every DID find the best FULL cnp_btype
...
SALT311.mac_prop_field(thin_table(company_sic_code1 NOT IN SET(s.nulls_company_sic_code1,company_sic_code1)),company_sic_code1,proxid,company_sic_code1_props); // For every DID find the best FULL company_sic_code1
...
EXPORT PostPropogationStats := SALT311.MAC_Pivot(PoPS, poprec);
  SHARED MAC_RollupCandidates(inCandidates, sortFields, groupFields, addBuddies) := FUNCTIONMACRO
    Grpd0 := GROUP(SORT(
      ...
      proxid, #EXPAND(sortFields), LOCAL),
      proxid, #EXPAND(groupFields), LOCAL);
  ...
  ENDMACRO;
  SHARED fieldList := 'parent_proxid,sele_proxid,org_proxid,ultimate_proxid,source,source_record_id,company_name,company_name_prefix,cnp_name,cnp_number,cnp_btype,cnp_lowv,company_phone_3,company_phone_3_ex,company_phone_7,company_fein,company_sic_code1,prim_range,prim_name,sec_range,city,city_clean,st,zip,company_url,isContact,contact_did,title,fname,fname_preferred,mname,lname,name_suffix,contact_ssn,contact_email,sele_flag,org_flag,ult_flag,fallback_value';
  SHARED fieldListWithPropFlags := fieldList + ',cnp_btype_prop,company_fein_prop,company_sic_code1_prop';

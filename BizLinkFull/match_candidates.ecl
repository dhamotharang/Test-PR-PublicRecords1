...
EXPORT poprec := RECORD
	STRING label;
		REAL8 pop;
	END;
EXPORT PrePropogationStats := SALT44.MAC_Pivot(PPS, poprec);
...
SALT44.mac_prop_field(thin_table(cnp_btype NOT IN SET(s.nulls_cnp_btype,cnp_btype)),cnp_btype,proxid,cnp_btype_props); // For every DID find the best FULL cnp_btype
...
SALT44.mac_prop_field(thin_table(company_fein NOT IN SET(s.nulls_company_fein,company_fein)),company_fein,proxid,company_fein_props); // For every DID find the best FULL company_fein
...
SALT44.mac_prop_field(thin_table(company_sic_code1 NOT IN SET(s.nulls_company_sic_code1,company_sic_code1)),company_sic_code1,proxid,company_sic_code1_props); // For every DID find the best FULL company_sic_code1
...
pj20 do_computes(pj20 le) := TRANSFORM
  ...
  SELF := le;
END;
SHARED propogated := PROJECT(pj20,do_computes(left)) : PERSIST(Config_BIP.DefaultTempPrefix + 'proxid::BizLinkFull::mc_props::BizHead',EXPIRE(BizLinkFull.Config_BIP.PersistExpire)); // to allow to 'jump' over an exported value
...
EXPORT PostPropogationStats := SALT44.MAC_Pivot(PoPS, poprec);
...
EXPORT Layout_Matches := RECORD//in this module for because of ,foward bug
  ...
  SALT44.UIDType proxid1;
  SALT44.UIDType proxid2;
  SALT44.UIDType rcid1 := 0;
  SALT44.UIDType rcid2 := 0;
END;
...
SHARED Annotated := DISTRIBUTE(j0, HASH(proxid)) : PERSIST(Config_BIP.DefaultTempPrefix + 'proxid::BizLinkFull::mc',EXPIRE(BizLinkFull.Config_BIP.PersistExpire)); // Distributed for keybuild case

...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,proxid,cnp_name,st,prim_name,zip,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'L_CNPNAME_ST','city',CntRed_city*100,CntRed_city*TSize},{'L_CNPNAME_ST','company_sic_code1',CntRed_company_sic_code1*100,CntRed_company_sic_code1*TSize},{'L_CNPNAME_ST','cnp_number',CntRed_cnp_number*100,CntRed_cnp_number*TSize},{'L_CNPNAME_ST','cnp_btype',CntRed_cnp_btype*100,CntRed_cnp_btype*TSize},{'L_CNPNAME_ST','cnp_lowv',CntRed_cnp_lowv*100,CntRed_cnp_lowv*TSize},{'L_CNPNAME_ST','prim_range',CntRed_prim_range*100,CntRed_prim_range*TSize},{'L_CNPNAME_ST','sec_range',CntRed_sec_range*100,CntRed_sec_range*TSize},{'L_CNPNAME_ST','parent_proxid',CntRed_parent_proxid*100,CntRed_parent_proxid*TSize},{'L_CNPNAME_ST','sele_proxid',CntRed_sele_proxid*100,CntRed_sele_proxid*TSize},{'L_CNPNAME_ST','org_proxid',CntRed_org_proxid*100,CntRed_org_proxid*TSize},{'L_CNPNAME_ST','ultimate_proxid',CntRed_ultimate_proxid*100,CntRed_ultimate_proxid*TSize},{'L_CNPNAME_ST','sele_flag',CntRed_sele_flag*100,CntRed_sele_flag*TSize},{'L_CNPNAME_ST','org_flag',CntRed_org_flag*100,CntRed_org_flag*TSize},{'L_CNPNAME_ST','ult_flag',CntRed_ult_flag*100,CntRed_ult_flag*TSize}],SALT44.ShrinkLayout);
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.cnp_name <> (TYPEOF(le.cnp_name))'' AND Fields.InValid_cnp_name((SALT44.StrType)le.cnp_name)=0 AND le.st <> (TYPEOF(le.st))'' AND Fields.InValid_st((SALT44.StrType)le.st)=0;
KeyRec := RECORDOF(Key);
EXPORT RawFetch_server(TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.prim_name_len) param_prim_name_len = (TYPEOF(h.prim_name_len))'',DATASET(Process_Biz_Layouts.layout_zip_cases) param_zip,TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'',UNSIGNED4 param_efr_bitmap = 0) := 
  FUNCTION
 //Generate service attributes for GSS join
	...
	slimKeyRec := recordof(Key_BizHead_L_CNPNAME.SlimKey);/*HACK29a*/
    ...
    doIndexRead(UNSIGNED4 search,UNSIGNED2 spc) := STEPPED(LIMIT(Key_BizHead_L_CNPNAME.SlimKey( KEYED(GSS_hash = search) // ADDED LIMIT
                                                           ...
                                                           seleid,
                                                           proxid,
                                                           PRIORITY(40-spc)); /*HACK29b*/ // Filter for each row of index fetch
    ...
    res := JOIN( steppedmatches, Key,KEYED(RIGHT.GSS_Hash = wds[1].hsh)
                ...
                TRANSFORM(indexOutputRecord,
                          ...
                          SELF := RIGHT),
                LIMIT(Config_BIP.L_CNPNAME_ST_MAXBLOCKLIMIT,SKIP)); /*HACK29c*/
...
// Why not LOOP? - Because I am expecting FIRST one to win 99+ percent of the time - and don't want to impact it
  RawData0 := RawFetch_server(param_cnp_name,param_st,param_prim_name,param_prim_name_len,param_zip,0,param_efr_bitmap);
  RawData1 := RawFetch_server(param_cnp_name,param_st,param_prim_name,param_prim_name_len,param_zip,1,param_efr_bitmap);
  RawData2 := RawFetch_server(param_cnp_name,param_st,param_prim_name,param_prim_name_len,param_zip,2,param_efr_bitmap);
...
EXPORT ScoredproxidFetch(TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.prim_name_len) param_prim_name_len = (TYPEOF(h.prim_name_len))'',DATASET(Process_Biz_Layouts.layout_zip_cases) param_zip,TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.city_len) param_city_len = (TYPEOF(h.city_len))'',TYPEOF(h.company_sic_code1) param_company_sic_code1 = (TYPEOF(h.company_sic_code1))'',TYPEOF(h.cnp_number) param_cnp_number = (TYPEOF(h.cnp_number))'',TYPEOF(h.cnp_btype) param_cnp_btype = (TYPEOF(h.cnp_btype))'',TYPEOF(h.cnp_lowv) param_cnp_lowv = (TYPEOF(h.cnp_lowv))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.prim_range_len) param_prim_range_len = (TYPEOF(h.prim_range_len))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.sec_range_len) param_sec_range_len = (TYPEOF(h.sec_range_len))'',TYPEOF(h.parent_proxid) param_parent_proxid = (TYPEOF(h.parent_proxid))'',TYPEOF(h.sele_proxid) param_sele_proxid = (TYPEOF(h.sele_proxid))'',TYPEOF(h.org_proxid) param_org_proxid = (TYPEOF(h.org_proxid))'',TYPEOF(h.ultimate_proxid) param_ultimate_proxid = (TYPEOF(h.ultimate_proxid))'',TYPEOF(h.sele_flag) param_sele_flag = (TYPEOF(h.sele_flag))'',TYPEOF(h.org_flag) param_org_flag = (TYPEOF(h.org_flag))'',TYPEOF(h.ult_flag) param_ult_flag = (TYPEOF(h.ult_flag))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'',UNSIGNED4 param_efr_bitmap = 0,BOOLEAN param_disableForce = FALSE) := FUNCTION
  RawData := RawFetch(param_cnp_name,param_st,param_prim_name,param_prim_name_len,param_zip,param_fallback_value,param_efr_bitmap);
  Process_Biz_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.zipWeight := (50+MAP (
         ...
         SELF.zip_match_code = SALT44.MatchCode.ExactMatch => /*HACK16 le.zip_weight100 */ 1100 * param_zip(zip=le.zip)[1].weight/100.0,
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.cnp_nameWeight := IF(SALT44.MatchBagOfWords(le.cnp_name,ri.cnp_name,3177747,1)<=Config_BIP.cnp_name_force,SKIP,SALT44.MatchBagOfWords(le.cnp_name,ri.cnp_name,3177747,1)/100);
    ...
    SELF.zip_match_code := MAP(
         le.zip = (TYPEOF(le.zip))'' OR ~EXISTS(ri.zip_cases) => SALT44.MatchCode.OneSideNull,
  ...
  J0 := JOIN(Recs1,Key,((HASH32(SALT44.fn_bow_bestword(LEFT.cnp_name))=RIGHT.gss_hash AND SALT44.MatchBagOfWords(RIGHT.cnp_name,LEFT.cnp_name,3177747,1) > BizLinkFull.Config_BIP.cnp_name_Force * 100))
  ...
  AND ((~EXISTS(LEFT.zip_cases) OR RIGHT.zip = (TYPEOF(RIGHT.zip))'') OR (RIGHT.zip IN SET(LEFT.zip_cases,zip))),Score_Batch(RIGHT,LEFT),
    ATMOST(((HASH32(SALT44.fn_bow_bestword(LEFT.cnp_name))=RIGHT.gss_hash))
      AND ((RIGHT.st = LEFT.st)),Config_BIP.L_CNPNAME_ST_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((HASH32(SALT44.fn_bow_bestword(LEFT.cnp_name))=RIGHT.gss_hash AND SALT44.MatchBagOfWords(RIGHT.cnp_name,LEFT.cnp_name,3177747,1) > BizLinkFull.Config_BIP.cnp_name_Force * 100))
  AND ((RIGHT.st = LEFT.st))
  AND ((LEFT.prim_name = (TYPEOF(RIGHT.prim_name))'' OR RIGHT.prim_name = (TYPEOF(RIGHT.prim_name))'') OR (RIGHT.prim_name = LEFT.prim_name) OR ((Config_BIP.WithinEditN(RIGHT.prim_name,RIGHT.prim_name_len,LEFT.prim_name,LEFT.prim_name_len,1, 0))))
  AND ((~EXISTS(LEFT.zip_cases) OR RIGHT.zip = (TYPEOF(RIGHT.zip))'') OR (RIGHT.zip IN SET(LEFT.zip_cases,zip))),Score_Batch(RIGHT,LEFT),
    ATMOST(((HASH32(SALT44.fn_bow_bestword(LEFT.cnp_name))=RIGHT.gss_hash))
      AND ((RIGHT.st = LEFT.st)),Config_BIP.L_CNPNAME_ST_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join

...
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.prim_name <> (TYPEOF(le.prim_name))'' AND Fields.InValid_prim_name((SALT37.StrType)le.prim_name)=0 AND le.city <> (TYPEOF(le.city))'' AND Fields.InValid_city((SALT37.StrType)le.city)=0 AND le.st <> (TYPEOF(le.st))'' AND Fields.InValid_st((SALT37.StrType)le.st)=0;
KeyRec := RECORDOF(Key);
...
EXPORT RawFetch_server(TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.prim_name_len) param_prim_name_len = (TYPEOF(h.prim_name_len))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.city_len) param_city_len = (TYPEOF(h.city_len))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.prim_range_len) param_prim_range_len = (TYPEOF(h.prim_range_len))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := 
    STEPPED( LIMIT( Key(
      ...
      AND ( prim_range = (TYPEOF(prim_range))'' OR param_prim_range = (TYPEOF(prim_range))'' OR Config_BIP.WithinEditN(prim_range,prim_range_len,param_prim_range,param_prim_range_len,1, 0) )
      AND ( cnp_name = (TYPEOF(cnp_name))'' OR param_cnp_name = (TYPEOF(cnp_name))'' OR SALT37.MatchBagOfWords(cnp_name,param_cnp_name,3177747,1)+400/*HACK08*/ > Config_BIP.cnp_name_Force * 100)
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.city_match_code := MAP(
           ...
           match_methods(File_BizHead).match_city(le.city,ri.city,le.city_len,ri.city_len,TRUE));
    SELF.cityWeight := (50+MAP (
           le.st = (TYPEOF(le.st))'' OR ri.st = (TYPEOF(ri.st))'' OR le.st <> ri.st => 0, // Only valid if the context variable is equal
...
le.zip = (TYPEOF(le.zip))'' OR ~EXISTS(ri.zip_cases) => SALT37.MatchCode.OneSideNull,
  ...
  J0 := JOIN(Recs1,Key,LEFT.prim_name = RIGHT.prim_name
    ...
    ATMOST(LEFT.prim_name = RIGHT.prim_name
     AND LEFT.city = RIGHT.city
     AND LEFT.st = RIGHT.st,Config_BIP.L_ADDRESS1_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.prim_name = RIGHT.prim_name
    ...
    ATMOST(LEFT.prim_name = RIGHT.prim_name
     AND LEFT.city = RIGHT.city
     AND LEFT.st = RIGHT.st,Config_BIP.L_ADDRESS1_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
#IF(#TEXT(Input_prim_name)<>'' AND #TEXT(Input_city)<>'' AND #TEXT(Input_st)<>'')
  ...
  output_file := BizLinkFull.Key_BizHead_L_ADDRESS1.ScoredFetch_Batch(%p%,AsIndex, In_disableForce);
#ELSE

...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  AND ((LEFT.cnp_name = (TYPEOF(RIGHT.cnp_name))'' OR RIGHT.cnp_name = (TYPEOF(RIGHT.cnp_name))'') OR (SALT44.MatchBagOfWords(RIGHT.cnp_name,LEFT.cnp_name,3177747,1)+BizLinkFull.Config_BIP.HACK08_ThorAddrCnpNameBonus/*HACK08_b*/ > BizLinkFull.Config_BIP.cnp_name_Force * 100)),Score_Batch(RIGHT,LEFT),
  ...
  AND ((RIGHT.st = LEFT.st))
  AND ((LEFT.prim_range = (TYPEOF(RIGHT.prim_range))'' OR RIGHT.prim_range = (TYPEOF(RIGHT.prim_range))'') OR (RIGHT.prim_range = LEFT.prim_range) OR ((Config_BIP.WithinEditN(RIGHT.prim_range,RIGHT.prim_range_len,LEFT.prim_range,LEFT.prim_range_len,1, 0))))
  AND ((LEFT.cnp_name = (TYPEOF(RIGHT.cnp_name))'' OR RIGHT.cnp_name = (TYPEOF(RIGHT.cnp_name))'') OR (SALT44.MatchBagOfWords(RIGHT.cnp_name,LEFT.cnp_name,3177747,1)+BizLinkFull.Config_BIP.HACK08_ThorAddrCnpNameBonus/*HACK08_c*/ > BizLinkFull.Config_BIP.cnp_name_Force * 100)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.prim_name = LEFT.prim_name))
      AND ((RIGHT.city = LEFT.city))

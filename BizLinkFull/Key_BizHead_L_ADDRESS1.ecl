...
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.prim_name <> (TYPEOF(le.prim_name))'' AND Fields.InValid_prim_name((SALT311.StrType)le.prim_name)=0 AND le.city <> (TYPEOF(le.city))'' AND Fields.InValid_city((SALT311.StrType)le.city)=0 AND le.st <> (TYPEOF(le.st))'' AND Fields.InValid_st((SALT311.StrType)le.st)=0;
KeyRec := RECORDOF(Key);

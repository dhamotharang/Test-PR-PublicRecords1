...
// Transform to process 'real' results
  ...
  EXPORT ScoreData(RD,Inv) := FUNCTIONMACRO
    ...
    SELF.Match_cnp_name := MAP ( ri.cnp_name = (typeof(ri.cnp_name))'' => 0, le.cnp_name = (typeof(ri.cnp_name))'' => -1, ri.cnp_name = le.cnp_name => 2,SALT33.MatchBagOfWords(le_cnp_name,ri_cnp_name,3177747,1) > BizLinkFull.Config_BIP.cnp_name_Force * 100 => 1,-2);
    ...
    SELF.Match_company_url := MAP ( ri.company_url = (typeof(ri.company_url))'' => 0, le.company_url = (typeof(ri.company_url))'' => -1, ri.company_url = le.company_url => 2,SALT33.MatchBagOfWords(le_company_url,ri_company_url,31744,0) > BizLinkFull.Config_BIP.company_url_Force * 100 => 1,-2);

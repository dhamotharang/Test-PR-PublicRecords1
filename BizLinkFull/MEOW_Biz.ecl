...
// Transform to process 'real' results
  ...
  SELF.keys_tried := MAP(
        ...
        LinkPathName = 'L_CONTACT' =>  + IF (Key_BizHead_L_CONTACT.CanSearch(le),1 << 13,0),
        LinkPathName = 'L_CONTACT_SSN' =>  + IF (Key_BizHead_L_CONTACT_SSN.CanSearch(le),1 << 14,0),
        LinkPathName = 'L_EMAIL' =>  + IF (Key_BizHead_L_EMAIL.CanSearch(le),1 << 15,0),
        LinkPathName = 'L_SIC' =>  + IF (Key_BizHead_L_SIC.CanSearch(le),1 << 16,0),
        LinkPathName = 'L_SOURCE' =>  + IF (Key_BizHead_L_SOURCE.CanSearch(le),1 << 17,0),
        LinkPathName = 'L_CONTACT_DID' =>  + IF (Key_BizHead_L_CONTACT_DID.CanSearch(le),1 << 18,0),0);
  ...
  fetchResults := MAP(
        ...
        LinkPathName = 'L_CONTACT' => IF(Key_BizHead_L_CONTACT.CanSearch(le),SORTED(IF((~BizLinkFull.Key_BizHead_L_CONTACT_ST.CanSearch(le) AND ~BizLinkFull.Key_BizHead_L_CONTACT_ZIP.CanSearch(le)),IF(BizLinkFull.Key_BizHead_L_CONTACT.CanSearch(le),Key_BizHead_L_CONTACT.ScoredproxidFetch(param_fname_preferred := le.fname_preferred,param_lname := le.lname,param_lname_len := lname_len,param_mname := le.mname,param_mname_len := mname_len,param_cnp_name := cnp_name_spec,param_zip := le.zip_cases,param_st := le.st,param_fname := le.fname,param_fname_len := fname_len,param_company_sic_code1 := le.company_sic_code1,param_cnp_number := le.cnp_number,param_cnp_btype := le.cnp_btype,param_cnp_lowv := le.cnp_lowv,param_prim_name := le.prim_name,param_prim_name_len := prim_name_len,param_city := le.city,param_city_len := city_len,param_prim_range := le.prim_range,param_prim_range_len := prim_range_len,param_sec_range := le.sec_range,param_sec_range_len := sec_range_len,param_parent_proxid := le.parent_proxid,param_sele_proxid := le.sele_proxid,param_org_proxid := le.org_proxid,param_ultimate_proxid := le.ultimate_proxid,param_sele_flag := le.sele_flag,param_org_flag := le.org_flag,param_ult_flag := le.ult_flag,param_fallback_value := le.fallback_value,param_efr_bitmap := (UNSIGNED4)le.efr_bitmap,param_disableForce := In_disableForce))),ultid,orgid,seleid,proxid),DATASET([],Process_Biz_Layouts.LayoutScoredFetch)),

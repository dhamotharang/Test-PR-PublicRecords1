...
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.contact_did <> (TYPEOF(le.contact_did))'' AND Fields.InValid_contact_did((SALT37.StrType)le.contact_did)=0;
KeyRec := RECORDOF(Key);
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  J0 := JOIN(Recs1,Key,LEFT.contact_did = RIGHT.contact_did,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.contact_did = RIGHT.contact_did,Config_BIP.L_CONTACT_DID_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.contact_did = RIGHT.contact_did,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.contact_did = RIGHT.contact_did,Config_BIP.L_CONTACT_DID_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);

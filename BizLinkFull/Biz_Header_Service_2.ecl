...
END;
dSlimmed:=PROJECT(dLinkIDs(proxid <> 0),TRANSFORM(lSlimmed, // 20180329 JA: added filter proxid <> 0 because change to KeyseleidUp means dLinkIDs includes rollup record at sele level, and we don't need hyperlink to non-exist
  SELF.proxid__html:=BIPV2.Hyperlink().BIPHeader(LEFT.proxid);

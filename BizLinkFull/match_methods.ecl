...
EXPORT match_cnp_name(TYPEOF(h.cnp_name) L, TYPEOF(h.cnp_name) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(SALT311.WordBagsEqual(L,R) => SALT311.MatchCode.ExactMatch,
	    SALT311.MatchBagOfWords(L,R,3177747,1) <> 0 => SALT311.MatchCode.WordBagMatch,
    SALT311.MatchCode.NoMatch),
...
EXPORT match_company_fein(TYPEOF(h.company_fein) L, TYPEOF(h.company_fein) R, UNSIGNED1 LL = 0, UNSIGNED1 RL = 0, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT311.MatchCode.ExactMatch,
	Config_BIP.WithinEditN(L,LL,R,RL,1,0)=> SALT311.MatchCode.EditDistanceMatch,
    SALT311.MatchCode.NoMatch),
     MAP(L = R => SALT311.MatchCode.ExactMatch, SALT311.MatchCode.NoMatch)
);
...
EXPORT match_city(TYPEOF(h.city) L, TYPEOF(h.city) R, UNSIGNED1 LL = 0, UNSIGNED1 RL = 0, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT311.MatchCode.ExactMatch,
	Config_BIP.WithinEditN(L,LL,R,RL,2,0)=> SALT311.MatchCode.EditDistanceMatch,
...
EXPORT match_company_url(TYPEOF(h.company_url) L, TYPEOF(h.company_url) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(SALT311.WordBagsEqual(L,R) => SALT311.MatchCode.ExactMatch,
	    SALT311.MatchBagOfWords(L,R,31744,0) <> 0 => SALT311.MatchCode.WordBagMatch,
    SALT311.MatchCode.NoMatch),
     MAP(SALT311.WordBagsEqual(L,R) => SALT311.MatchCode.ExactMatch, SALT311.MatchCode.NoMatch)
);
...
EXPORT match_name_suffix(TYPEOF(h.name_suffix) L, TYPEOF(h.name_suffix) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT311.MatchCode.ExactMatch,
    fn_normSuffix(L) =  fn_normSuffix(R) => SALT311.MatchCode.CustomFuzzyMatch, // Compare fn_normSuffix values
    SALT311.MatchCode.NoMatch),
     MAP(L = R => SALT311.MatchCode.ExactMatch, SALT311.MatchCode.NoMatch)
);

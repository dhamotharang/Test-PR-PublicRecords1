...
EXPORT KeyName := BizLinkFull.Filename_keys.L_PHONE; /*HACK07*/
...
EXPORT RawFetch_server(TYPEOF(h.company_phone_7) param_company_phone_7 = (TYPEOF(h.company_phone_7))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.company_phone_3) param_company_phone_3 = (TYPEOF(h.company_phone_3))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'',UNSIGNED4 param_efr_bitmap = 0) := 
    STEPPED( LIMIT( Key(
      ...
      AND ( param_efr_bitmap=0 OR (EFR_BMap & param_efr_bitmap)>0 )),Config_BIP.L_PHONE_MAXBLOCKLIMIT,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),ultid,orgid,seleid,proxid);
 
EXPORT RawFetch(TYPEOF(h.company_phone_7) param_company_phone_7 = (TYPEOF(h.company_phone_7))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.company_phone_3) param_company_phone_3 = (TYPEOF(h.company_phone_3))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'',UNSIGNED4 param_efr_bitmap = 0) := FUNCTION
// Why not LOOP? - Because I am expecting FIRST one to win 99+ percent of the time - and don't want to impact it
  RawData0 := RawFetch_server(param_company_phone_7,param_cnp_name,param_company_phone_3,0,param_efr_bitmap);
  RawData1 := RawFetch_server(param_company_phone_7,param_cnp_name,param_company_phone_3,1,param_efr_bitmap);
  RawData2 := RawFetch_server(param_company_phone_7,param_cnp_name,param_company_phone_3,2,param_efr_bitmap);
  Returnable(DATASET(RECORDOF(RawData0)) d) := COUNT(NOFOLD(d))<>1 OR EXISTS(NOFOLD(d((TYPEOF(company_phone_7))company_phone_7 != (TYPEOF(company_phone_7))'')));
  res := MAP (
...
EXPORT ScoredproxidFetch(TYPEOF(h.company_phone_7) param_company_phone_7 = (TYPEOF(h.company_phone_7))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.company_phone_3) param_company_phone_3 = (TYPEOF(h.company_phone_3))'',TYPEOF(h.company_phone_3_ex) param_company_phone_3_ex = (TYPEOF(h.company_phone_3_ex))'',TYPEOF(h.company_sic_code1) param_company_sic_code1 = (TYPEOF(h.company_sic_code1))'',TYPEOF(h.cnp_number) param_cnp_number = (TYPEOF(h.cnp_number))'',TYPEOF(h.cnp_btype) param_cnp_btype = (TYPEOF(h.cnp_btype))'',TYPEOF(h.cnp_lowv) param_cnp_lowv = (TYPEOF(h.cnp_lowv))'',DATASET(Process_Biz_Layouts.layout_zip_cases) param_zip,TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.prim_name_len) param_prim_name_len = (TYPEOF(h.prim_name_len))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.city_len) param_city_len = (TYPEOF(h.city_len))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.prim_range_len) param_prim_range_len = (TYPEOF(h.prim_range_len))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.sec_range_len) param_sec_range_len = (TYPEOF(h.sec_range_len))'',TYPEOF(h.parent_proxid) param_parent_proxid = (TYPEOF(h.parent_proxid))'',TYPEOF(h.sele_proxid) param_sele_proxid = (TYPEOF(h.sele_proxid))'',TYPEOF(h.org_proxid) param_org_proxid = (TYPEOF(h.org_proxid))'',TYPEOF(h.ultimate_proxid) param_ultimate_proxid = (TYPEOF(h.ultimate_proxid))'',TYPEOF(h.sele_flag) param_sele_flag = (TYPEOF(h.sele_flag))'',TYPEOF(h.org_flag) param_org_flag = (TYPEOF(h.org_flag))'',TYPEOF(h.ult_flag) param_ult_flag = (TYPEOF(h.ult_flag))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'',UNSIGNED4 param_efr_bitmap = 0,BOOLEAN param_disableForce = FALSE) := FUNCTION
  RawData := RawFetch(param_company_phone_7,param_cnp_name,param_company_phone_3,param_fallback_value,param_efr_bitmap);
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.company_phone_7_match_code := match_methods(File_BizHead).match_company_phone_7(le.company_phone_7,ri.company_phone_7,TRUE);
    SELF.company_phone_7Weight := (50+MAP (
           SELF.company_phone_7_match_code = SALT311.MatchCode.ExactMatch =>le.company_phone_7_weight100,
           -0.798*le.company_phone_7_weight100))/100; 
    SELF.cnp_name_match_code := MAP(
    ...
    SELF.company_phone_3_match_code := MAP(
           le.company_phone_3 = (TYPEOF(le.company_phone_3))'' OR ri.company_phone_3 = (TYPEOF(ri.company_phone_3))'' => SALT311.MatchCode.OneSideNull,
           match_methods(File_BizHead).match_company_phone_3(le.company_phone_3,ri.company_phone_3,FALSE));
    SELF.company_phone_3Weight := (50+MAP (
           SELF.company_phone_3_match_code = SALT311.MatchCode.OneSideNull => 0,
           SELF.company_phone_3_match_code = SALT311.MatchCode.ExactMatch =>le.company_phone_3_weight100,
           -0.952*le.company_phone_3_weight100))/100; 
    SELF.company_phone_3_ex_match_code := MAP(
           le.company_phone_3_ex = (TYPEOF(le.company_phone_3_ex))'' OR ri.company_phone_3_ex = (TYPEOF(ri.company_phone_3_ex))'' => SALT311.MatchCode.OneSideNull,
           match_methods(File_BizHead).match_company_phone_3_ex(le.company_phone_3_ex,ri.company_phone_3_ex,FALSE));
    SELF.company_phone_3_exWeight := (50+MAP (
           SELF.company_phone_3_ex_match_code = SALT311.MatchCode.OneSideNull => 0,
           SELF.company_phone_3_ex_match_code = SALT311.MatchCode.ExactMatch =>le.company_phone_3_ex_weight100,
           -0.952*le.company_phone_3_ex_weight100))/100; 
  ...
  J0 := JOIN(Recs1,Key,((RIGHT.company_phone_7 = LEFT.company_phone_7))
     ...
     AND ((LEFT.company_phone_3 = (TYPEOF(RIGHT.company_phone_3))'' OR RIGHT.company_phone_3 = (TYPEOF(RIGHT.company_phone_3))'') OR (RIGHT.company_phone_3 = LEFT.company_phone_3)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.company_phone_7 = LEFT.company_phone_7)),Config_BIP.L_PHONE_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),((RIGHT.company_phone_7 = LEFT.company_phone_7))
     AND ((LEFT.cnp_name = (TYPEOF(RIGHT.cnp_name))'' OR RIGHT.cnp_name = (TYPEOF(RIGHT.cnp_name))'') OR (SALT311.MatchBagOfWords(RIGHT.cnp_name,LEFT.cnp_name,3177747,1) > BizLinkFull.Config_BIP.cnp_name_Force * 100))
     AND ((LEFT.company_phone_3 = (TYPEOF(RIGHT.company_phone_3))'' OR RIGHT.company_phone_3 = (TYPEOF(RIGHT.company_phone_3))'') OR (RIGHT.company_phone_3 = LEFT.company_phone_3)),Score_Batch(RIGHT,LEFT),
    ATMOST(((RIGHT.company_phone_7 = LEFT.company_phone_7)),Config_BIP.L_PHONE_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_company_phone_7='',Input_cnp_name='',Input_company_phone_3='',Input_company_phone_3_ex='',Input_company_sic_code1='',Input_cnp_number='',Input_cnp_btype='',Input_cnp_lowv='',Input_zip='',Input_prim_name='',Input_city='',Input_st='',Input_prim_range='',Input_sec_range='',Input_parent_proxid='',Input_sele_proxid='',Input_org_proxid='',Input_ultimate_proxid='',Input_sele_flag='',Input_org_flag='',Input_ult_flag='',output_file,AsIndex='true', In_disableForce = 'false') := MACRO
  ...
  #IF(#TEXT(Input_company_phone_7)<>'')
    ...
    output_file := BizLinkFull.Key_BizHead_L_PHONE.ScoredFetch_Batch(%p%,AsIndex, In_disableForce);
  #ELSE

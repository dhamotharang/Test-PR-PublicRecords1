...
R ProcessConceptScaling(cfk le) := TRANSFORM
  ...
  SELF.lname_e2_Weight100 := SALT33.Min0(le.lname_weight100 + 100*log(le.lname_cnt/le.lname_e2_cnt)/log(2)); // Precompute edit-distance specificity
  REAL STREETADDRESS_score_scale := 1.0; // No scaling required
  SELF.prim_range_weight100 := SALT33.Min0(le.prim_range_weight100); // Precompute the scale
  SELF.prim_range_e1_Weight100 := SALT33.Min0(le.prim_range_weight100 + 100*log(le.prim_range_cnt/le.prim_range_e1_cnt)/log(2)); // Precompute edit-distance specificity
  SELF.prim_name_weight100 := SALT33.Min0(le.prim_name_weight100); // Precompute the scale
  SELF.prim_name_e1_Weight100 := SALT33.Min0(le.prim_name_weight100 + 100*log(le.prim_name_cnt/le.prim_name_e1_cnt)/log(2)); // Precompute edit-distance specificity
  SELF.sec_range_weight100 := SALT33.Min0(le.sec_range_weight100); // Precompute the scale
  SELF.sec_range_e1_Weight100 := SALT33.Min0(le.sec_range_weight100 + 100*log(le.sec_range_cnt/le.sec_range_e1_cnt)/log(2)); // Precompute edit-distance specificity
  SELF := le;
END;

...
Business_Header.Layout_Business_Contact_Full Translate_Vickers_13g13d_To_BCF(Layout_13d13g_Base l, unsigned1 c) := transform
self.source := 'V';
...
Business_Header.Layout_Business_Contact_Full Translate_Vickers_Insider_Filing_To_BCF(layout_insider_filing_temp l, unsigned1 ctyp) := transform
self.source := 'V';
self.vendor_id := trim(l.cusip) + ',' + trim(l.form_id) + ',' + trim(l.filer_id) + ',' + l.form_type;
...
v_insider_filing_issuer_contact_init := project(v_insider_filing_name(not(datalib.companyclean(filer_name)[41..120] <> '' or
...
v_insider_filing_street_contact_init := project(v_insider_filing_name(not(datalib.companyclean(filer_name)[41..120] <> '' or
                                        ((integer)(Business_Header.CleanName(name_first, name_middle, name_last, name_suffix)[142]) >= 3 and
                                        (integer)name_score < 85)),
...
Business_Header.Layout_Business_Contact_Full Translate_Vickers_Blank_Address_To_BCF(layout_insider_filing_temp l) := transform
self.source := 'V';
self.vendor_id := trim(l.cusip) + ',' + trim(l.form_id) + ',' + trim(l.filer_id) + ',' + l.form_type;
self.dt_first_seen := (unsigned4)map(length(trim(l.trans_date_from)) = 8 => l.trans_date_from,
                          length(trim(l.date_entered)) = 8 => l.date_entered,
...
v_insider_filing_issuer_blank_contact_init := project(v_insider_filing_name(not(datalib.companyclean(filer_name)[41..120] <> '' or
                                        ((integer)(Business_Header.CleanName(name_first, name_middle, name_last, name_suffix)[142]) >= 3 and
                                        (integer)name_score < 85)),
...
Business_Header.Layout_Business_Contact_Full BlankAddress(Business_Header.Layout_Business_Contact_Full l, layout_nameaddr r) := transform
...
export Vickers_As_Business_Contact := vickers_contact_noroll : PERSIST('TEMP::Vickers_Contacts_As_BC');
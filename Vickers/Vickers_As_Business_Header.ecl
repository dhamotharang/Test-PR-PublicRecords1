import Business_Header, ut, Vickers;
...
v_13g13d := File_13d13g_Base(filer_name <> '',
                          datalib.companyclean(filer_name)[41..120] <> '' or
                          (integer)(datalib.NameClean(filer_name)[142]) >= 3 or
						  (integer)filer_name_score <= 85);

Business_Header.Layout_Business_Header Translate_Vickers_13g13d_To_BHF(Layout_13d13g_Base l) := transform
self.source := 'V';
self.source_group := '';
self.vendor_id := trim(l.cusip) + ',' + trim(l.form_id) + ',' + trim(l.filer_id) + ',' + l.form_type;
self.dt_first_seen := (unsigned4)map(length(trim(l.filing_date)) = 8 => l.filing_date,
                          length(trim(l.entered_date)) = 8 => l.entered_date,
						  length(trim(l.upd_date)) = 8 => l.upd_date,
						  '');
self.dt_last_seen := (unsigned4)map(length(trim(l.upd_date)) = 8 => l.upd_date,
                          length(trim(l.entered_date)) = 8 => l.entered_date,
						  length(trim(l.filing_date)) = 8 => l.filing_date,
						  '');
self.dt_vendor_first_reported := (unsigned4)map(length(trim(l.entered_date)) = 8 => l.entered_date,
                          length(trim(l.upd_date)) = 8 => l.upd_date,
						  length(trim(l.filing_date)) = 8 => l.filing_date,
						  '');						  
self.dt_vendor_last_reported := (unsigned4)map(length(trim(l.upd_date)) = 8 => l.upd_date,
                          length(trim(l.entered_date)) = 8 => l.entered_date,
						  length(trim(l.filing_date)) = 8 => l.filing_date,
						  '');
...
v_insider_filing := File_Insider_Filing_Base(cusip <> '');
v_insider_filing_dist := distribute(v_insider_filing, hash(cusip));

v_insider_security := dedup(File_Insider_Security_In(cusip <> ''), cusip, all);
...
Business_Header.Layout_Business_Header Translate_Vickers_Insider_Filing_To_BHF(layout_insider_filing_temp l, unsigned1 ctyp) := transform
self.source := 'V';
self.source_group := '';
self.vendor_id := trim(l.cusip) + ',' + trim(l.form_id) + ',' + trim(l.filer_id) + ',' + l.form_type;
self.dt_first_seen := (unsigned4)map(length(trim(l.trans_date_from)) = 8 => l.trans_date_from,
...
self.dt_last_seen := (unsigned4)map(length(trim(l.upd_date)) = 8 => l.upd_date,
						  ...
						  '');
self.dt_vendor_first_reported := (unsigned4)map(length(trim(l.date_entered)) = 8 => l.date_entered,
                          length(trim(l.upd_date)) = 8 => l.upd_date,
						  length(trim(l.trans_date_from)) = 8 => l.trans_date_from,
						  '');						  
self.dt_vendor_last_reported := (unsigned4)map(length(trim(l.upd_date)) = 8 => l.upd_date,
...
v_insider_filing_filer_init := project(v_insider_filing_name(datalib.companyclean(filer_name)[41..120] <> '' or
                                        ((integer)(Business_Header.CleanName(name_first, name_middle, name_last, name_suffix)[142]) >= 3 and
                                        (integer)name_score < 85)), 
...
v_insider_filing_issuer_init := project(v_insider_filing_name(not(datalib.companyclean(filer_name)[41..120] <> '' or
                                        ((integer)(Business_Header.CleanName(name_first, name_middle, name_last, name_suffix)[142]) >= 3 and
                                        (integer)name_score < 85)),
...
v_insider_filing_street_init := project(v_insider_filing_name(not(datalib.companyclean(filer_name)[41..120] <> '' or
                                        ((integer)(Business_Header.CleanName(name_first, name_middle, name_last, name_suffix)[142]) >= 3 and
                                        (integer)name_score < 85)),
...
Business_Header.Layout_Business_Header Translate_Vickers_Blank_Address_To_BHF(layout_insider_filing_temp l) := transform
self.source := 'V';
self.source_group := '';
self.vendor_id := trim(l.cusip) + ',' + trim(l.form_id) + ',' + trim(l.filer_id) + ',' + l.form_type;
self.dt_first_seen := (unsigned4)map(length(trim(l.trans_date_from)) = 8 => l.trans_date_from,
...
v_insider_filing_issuer_blank_init := project(v_insider_filing_name(not(datalib.companyclean(filer_name)[41..120] <> '' or
                                        ((integer)(Business_Header.CleanName(name_first, name_middle, name_last, name_suffix)[142]) >= 3 and
                                        (integer)name_score < 85)),
...
bh := Business_Header.File_Business_Header(source <> 'V', zip <> 0, prim_name <> '');
...
Business_Header.Layout_Business_Header BlankAddress(Business_Header.Layout_Business_Header l, layout_nameaddr r) := transform
...
// Rollup
Business_Header.Layout_Business_Header RollupVickers(Business_Header.Layout_Business_Header L, Business_Header.Layout_Business_Header R) := TRANSFORM
SELF.dt_first_seen := 
...
Vickers_Init_Rollup := ROLLUP(Vickers_Init_Sort,
                    ...
                    LOCAL) : PERSIST('TEMP::Vickers_Company_Rollup');
					
export Vickers_As_Business_Header := Vickers_Init_Rollup;
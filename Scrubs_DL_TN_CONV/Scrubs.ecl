...
// The module to handle the case where no scrubs exist
  ...
  END;
EXPORT FromNone(DATASET(Layout_In_tn_conv) h) := MODULE
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(Layout_In_tn_conv) prevDS = DATASET([], Layout_In_tn_conv), STRING10 Src='UNK'):= FUNCTION
  // field error stats
    ...
    mod_hygiene := hygiene(PROJECT(h, Layout_In_tn_conv));
...
EXPORT StandardStats(DATASET(Layout_In_tn_conv) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  myTimeStamp := (UNSIGNED6)SALT38.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
  expandedFile := FromNone(inFile).ExpandedInfile;
  mod_fromexpandedOverall := FromExpanded(expandedFile);
  scrubsSummaryOverall := mod_fromexpandedOverall.SummaryStats;
 
  SALT38.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_DL_TN_CONV, Fields, 'RECORDOF(scrubsSummaryOverall)', '');

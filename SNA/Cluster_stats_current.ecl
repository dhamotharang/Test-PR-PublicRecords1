...
export Cluster_stats_current(dataset(sna.layouts.cluster_stat_input) dids, unsigned1 caller )  := Function
...
prof_lic_did add_proflic(dids le, profLic_key ri) := Transform
    self.did := le.did;
		self.seq := le.seq;
...
did_proflic := join(dids, profLic_key, keyed(right.did=left.did),
...
owner_recs_sd := dedup(sort(owner_recs, prim_range,prim_name,suffix,postdir,st,p_city_name,zip),prim_range,prim_name,suffix,postdir,st,p_city_name,zip);
seller_recs_sd := dedup(sort(seller_recs, prim_range,prim_name,suffix,postdir,st,p_city_name,zip),prim_range,prim_name,suffix,postdir,st,p_city_name,zip);
...
prof_lic_did add_prop_count(did_proflic_sd le, owned_prop_t ri) := TRANSFORM
...
prof_prop_join := join(did_proflic_sd, owned_prop_t,
                 right.did=left.did,
								 add_prop_count(left,right), left outer,
								 atmost(riskwise.max_atmost));



Layout_MC_Attributes_Person getPersonAttributes( prof_prop_join le, SNA.Key_prop_cluster_stats ri ) := TRANSFORM
  ...
  END;
		...
		cluster_attrib  :=	join(prof_prop_join, SNA.Key_prop_cluster_stats, 

...
export AutoKey_IDs(IParam.autokey_search in_mod, boolean noFail=false) := function
	  ...
	  AutokeyB2.mac_get_payload(ids,ak_keyname,ak_dataset,outpl,did,b_did,ak_typeStr)
		...
		by_auto := dedup(sort(project(outpl,transform(layouts.r_accnbr,self.accnbr:=left.accident_nbr)),record),record);

...
EXPORT Functions := MODULE
  ...
  EXPORT ResolveResults(dataset(iesp.RelationshipIdentifierSearch.t_RelationshipIdentifierSearchMatchRecord) in_recs) :=
   ...
   return(ds_relResultFinal);	 
	 ...
	 EXPORT AddPhoneMetaData(Dataset(iesp.RelationshipIdentifierSearch.t_RelationshipIdentifierSearchMatchRecord) InRecs) := FUNCTION	  				 
			Slim_recs := PROJECT(inRecs, TRANSFORM({STRING20 AccountNumber;
		                                         UNSIGNED2 SequenceNumber;
																						 STRING10 phone;},
																						 SELF.phone := LEFT.PhoneInfo.Phone10;
																						 SELF := LEFT));
    						    																	
			 base_recs := JOIN(inRecs, gong.Key_History_phone, 
		                    KEYED(LEFT.PhoneInfo.phone10[4..10] = RIGHT.p7) AND 
					              KEYED(LEFT.PhoneInfo.phone10[1..3]  = RIGHT.p3),
											  TRANSFORM({string20 AccountNumber; Unsigned2 sequenceNumber; string10 phone;
																RECORDOF(RIGHT);},
											     SELF.accountNumber :=  LEFT.AccountNumber;
										    ...
										    LIMIT(RelationshipIdentifier_Services.Constants.limitvalue,skip)); 

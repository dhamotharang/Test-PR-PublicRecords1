...
r2 t2(tucs_ln le) := transform
 self.in_tucs := '1';
 self         := le;
end;

p_experian := project(experian(did>0),t1(left));
p_tucs     := project(tucs_ln(did>0),t2(left));

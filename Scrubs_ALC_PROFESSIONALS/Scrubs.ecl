...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Layout_ALC_PROFESSIONALS)
  ...
  END;
  EXPORT  Bitmap_Layout := RECORD(Layout_ALC_PROFESSIONALS)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(Layout_ALC_PROFESSIONALS) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.license_class_Invalid := Fields.InValid_license_class((SALT37.StrType)le.license_class);
    ...
    SELF.license_no_Invalid := Fields.InValid_license_no((SALT37.StrType)le.license_no,(SALT37.StrType)le.license_state);
  ...
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Layout_ALC_PROFESSIONALS);
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.fname_Invalid << 0 ) + ( le.lname_Invalid << 1 ) + ( le.title_Invalid << 2 ) + ( le.state_Invalid << 3 ) + ( le.zip_Invalid << 4 ) + ( le.zip4_Invalid << 5 ) + ( le.cart_Invalid << 6 ) + ( le.bar_Invalid << 7 ) + ( le.gender_Invalid << 8 ) + ( le.addr_type_Invalid << 9 ) + ( le.county_cd_Invalid << 10 ) + ( le.license_class_Invalid << 11 ) + ( le.license_state_Invalid << 12 ) + ( le.license_type_Invalid << 13 ) + ( le.msa_Invalid << 14 ) + ( le.nielsen_county_cd_Invalid << 15 ) + ( le.phone_Invalid << 16 ) + ( le.list_id_Invalid << 17 ) + ( le.scno_Invalid << 18 ) + ( le.custno_Invalid << 19 ) + ( le.license_no_Invalid << 20 ) + ( le.dob_Invalid << 22 ) + ( le.orig_date_Invalid << 23 ) + ( le.exp_date_Invalid << 24 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,Layout_ALC_PROFESSIONALS);
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.license_class_Invalid := (le.ScrubsBits1 >> 11) & 1;
    SELF.license_state_Invalid := (le.ScrubsBits1 >> 12) & 1;
    SELF.license_type_Invalid := (le.ScrubsBits1 >> 13) & 1;
    ...
    SELF.list_id_Invalid := (le.ScrubsBits1 >> 17) & 1;
    SELF.scno_Invalid := (le.ScrubsBits1 >> 18) & 1;
    SELF.custno_Invalid := (le.ScrubsBits1 >> 19) & 1;
    SELF.license_no_Invalid := (le.ScrubsBits1 >> 20) & 3;
    SELF.dob_Invalid := (le.ScrubsBits1 >> 22) & 1;
    SELF.orig_date_Invalid := (le.ScrubsBits1 >> 23) & 1;
    SELF.exp_date_Invalid := (le.ScrubsBits1 >> 24) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.fname_Invalid,le.lname_Invalid,le.title_Invalid,le.state_Invalid,le.zip_Invalid,le.zip4_Invalid,le.cart_Invalid,le.bar_Invalid,le.gender_Invalid,le.addr_type_Invalid,le.county_cd_Invalid,le.license_class_Invalid,le.license_state_Invalid,le.license_type_Invalid,le.msa_Invalid,le.nielsen_county_cd_Invalid,le.phone_Invalid,le.list_id_Invalid,le.scno_Invalid,le.custno_Invalid,le.license_no_Invalid,le.dob_Invalid,le.orig_date_Invalid,le.exp_date_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_fname(le.fname_Invalid),Fields.InvalidMessage_lname(le.lname_Invalid),Fields.InvalidMessage_title(le.title_Invalid),Fields.InvalidMessage_state(le.state_Invalid),Fields.InvalidMessage_zip(le.zip_Invalid),Fields.InvalidMessage_zip4(le.zip4_Invalid),Fields.InvalidMessage_cart(le.cart_Invalid),Fields.InvalidMessage_bar(le.bar_Invalid),Fields.InvalidMessage_gender(le.gender_Invalid),Fields.InvalidMessage_addr_type(le.addr_type_Invalid),Fields.InvalidMessage_county_cd(le.county_cd_Invalid),Fields.InvalidMessage_license_class(le.license_class_Invalid),Fields.InvalidMessage_license_state(le.license_state_Invalid),Fields.InvalidMessage_license_type(le.license_type_Invalid),Fields.InvalidMessage_msa(le.msa_Invalid),Fields.InvalidMessage_nielsen_county_cd(le.nielsen_county_cd_Invalid),Fields.InvalidMessage_phone(le.phone_Invalid),Fields.InvalidMessage_list_id(le.list_id_Invalid),Fields.InvalidMessage_scno(le.scno_Invalid),Fields.InvalidMessage_custno(le.custno_Invalid),Fields.InvalidMessage_license_no(le.license_no_Invalid),Fields.InvalidMessage_dob(le.dob_Invalid),Fields.InvalidMessage_orig_date(le.orig_date_Invalid),Fields.InvalidMessage_exp_date(le.exp_date_Invalid),'UNKNOWN'));
    ...
    SELF.FieldName := CHOOSE(c,'fname','lname','title','state','zip','zip4','cart','bar','gender','addr_type','county_cd','license_class','license_state','license_type','msa','nielsen_county_cd','phone','list_id','scno','custno','license_no','dob','orig_date','exp_date','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_alphaspacequote','invalid_alphaspacequote','invalid_alphaspace','invalid_alpha','invalid_numeric','invalid_numeric','invalid_alphanumeric','invalid_numeric','invalid_gender','invalid_addr_type','invalid_alphanumeric','invalid_numericpound','invalid_alpha','invalid_alphapound','invalid_numeric','invalid_alphanumeric','invalid_numeric','invalid_numeric','invalid_numeric','invalid_alphanumeric','invalid_license_number','invalid_slashspace_date','invalid_slashOspace_date','invalid_slashOspace_date','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT37.StrType)le.fname,(SALT37.StrType)le.lname,(SALT37.StrType)le.title,(SALT37.StrType)le.state,(SALT37.StrType)le.zip,(SALT37.StrType)le.zip4,(SALT37.StrType)le.cart,(SALT37.StrType)le.bar,(SALT37.StrType)le.gender,(SALT37.StrType)le.addr_type,(SALT37.StrType)le.county_cd,(SALT37.StrType)le.license_class,(SALT37.StrType)le.license_state,(SALT37.StrType)le.license_type,(SALT37.StrType)le.msa,(SALT37.StrType)le.nielsen_county_cd,(SALT37.StrType)le.phone,(SALT37.StrType)le.list_id,(SALT37.StrType)le.scno,(SALT37.StrType)le.custno,(SALT37.StrType)le.license_no,(SALT37.StrType)le.dob,(SALT37.StrType)le.orig_date,(SALT37.StrType)le.exp_date,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT37.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'license_class:invalid_numericpound:ALLOW'
          ...
          ,'dob:invalid_slashspace_date:ALLOW'
          ,'orig_date:invalid_slashOspace_date:ALLOW'
          ,'exp_date:invalid_slashOspace_date:ALLOW','UNKNOWN');

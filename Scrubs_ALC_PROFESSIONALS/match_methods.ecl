IMPORT SALT32,std;
EXPORT match_methods(DATASET(layout_ALC_PROFESSIONALS) ih) := MODULE
...
EXPORT match_license_class(TYPEOF(h.license_class) L, TYPEOF(h.license_class) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT32.MatchCode.ExactMatch,
    SALT32.MatchCode.NoMatch),
  MAP(L = R => SALT32.MatchCode.ExactMatch, SALT32.MatchCode.NoMatch)
);

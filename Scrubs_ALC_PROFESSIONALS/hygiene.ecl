...
EXPORT Summary(SALT37.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_license_class := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.license_class)));
    avelength_license_class := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.license_class)),h.license_class<>(typeof(h.license_class))'');
...
SALT37.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT37.StrType)le.fname),TRIM((SALT37.StrType)le.lname),TRIM((SALT37.StrType)le.title),TRIM((SALT37.StrType)le.company),TRIM((SALT37.StrType)le.address1),TRIM((SALT37.StrType)le.address2),TRIM((SALT37.StrType)le.city),TRIM((SALT37.StrType)le.state),TRIM((SALT37.StrType)le.zip),TRIM((SALT37.StrType)le.zip4),TRIM((SALT37.StrType)le.cart),TRIM((SALT37.StrType)le.bar),TRIM((SALT37.StrType)le.gender),TRIM((SALT37.StrType)le.country),TRIM((SALT37.StrType)le.postal_cd),TRIM((SALT37.StrType)le.dpv),TRIM((SALT37.StrType)le.addr_type),TRIM((SALT37.StrType)le.county_cd),TRIM((SALT37.StrType)le.license_class),TRIM((SALT37.StrType)le.license_state),TRIM((SALT37.StrType)le.license_type),TRIM((SALT37.StrType)le.msa),TRIM((SALT37.StrType)le.nielsen_county_cd),TRIM((SALT37.StrType)le.phone),TRIM((SALT37.StrType)le.email),TRIM((SALT37.StrType)le.list_id),TRIM((SALT37.StrType)le.scno),TRIM((SALT37.StrType)le.keycode),TRIM((SALT37.StrType)le.custno),TRIM((SALT37.StrType)le.license_no),TRIM((SALT37.StrType)le.dob),TRIM((SALT37.StrType)le.orig_date),TRIM((SALT37.StrType)le.exp_date),TRIM((SALT37.StrType)le.degree),TRIM((SALT37.StrType)le.specialty)));
  SELF.FldNo := C;
END;
...
SHARED FldIds := DATASET([{1,'fname'}
      ...
      ,{33,'exp_date'}
      ,{34,'degree'}
      ,{35,'specialty'}],SALT37.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_license_class((SALT37.StrType)le.license_class),
    ...
    Fields.InValid_license_no((SALT37.StrType)le.license_no,(SALT37.StrType)le.license_state),

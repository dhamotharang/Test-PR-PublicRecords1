...
Watercraft.Macro_Clean_Hull_ID(watercraft_preprocess.file_AK_clean_in, Watercraft.layout_ak, hull_clean_in)
...
watercraft.Layout_Watercraft_Main_Base main_mapping_format(wDatasetwithflag L) := transform
	...
	IsValidStatusDate									:=	STD.DATE.IsValidDate((integer)L.DMVStatusDate1);
	self.registration_status_date			:=	IF(IsValidStatusDate,L.DMVStatusDate1,'');
	...
	self.lien_1_name					:=	'';
	self.lien_1_date					:=	'';
	tempLienAddress1					:=	StringLib.StringCleanSpaces(StringLib.Stringfindreplace(L.LienAddress1,'.', ' '));
	self.lien_1_address_1			:=	StringLib.StringCleanSpaces(MAP(regexfind('(P O BOX BOX )',tempLienAddress1) => regexreplace('(P O BOX BOX )',tempLienAddress1,'PO BOX '),
																																regexfind('^(P O BOX|POB |BOX)',tempLienAddress1) => regexreplace('^(P O BOX|POB |BOX|)',tempLienAddress1,'PO BOX '),tempLienAddress1));
	self.lien_1_address_2			:=	L.LienAddress2;
	self.lien_1_city					:=	L.LienCity;
	self.lien_1_state					:=	L.LienState;
	self.lien_1_zip						:=	L.LienZip + IF(trim(L.LienZip4) <> '','-','')+ L.LienZip4;
	...
	self.purchase_price						:=	'';
	self.new_used_flag						:=	'';
	self.watercraft_status_code		:=	L.licstatus;
	self.watercraft_status_description	:=	IF(L.licstatus = 'C','CURRENT','');
	self.history_flag							:=	'';
	self.coastguard_flag					:=	'';
	self.signatory                := case(L.ConJuntnCode1, 'AND'=> 'YES', 'OR'=>'NO', '');
	end;

EXPORT Map_AK_raw_to_Main := project(wDatasetwithflag, main_mapping_format(left));
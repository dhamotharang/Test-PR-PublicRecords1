...
EXPORT SALT34.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_future_date','invalid_charter_nbr','invalid_numeric','invalid_percentage','invalid_money');
EXPORT FieldTypeNum(SALT34.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_corp_vendor' => 2,'invalid_state_origin' => 3,'invalid_mandatory' => 4,'invalid_date' => 5,'invalid_future_date' => 6,'invalid_charter_nbr' => 7,'invalid_numeric' => 8,'invalid_percentage' => 9,'invalid_money' => 10,0);
...
EXPORT InValid_ar_due_dt(SALT34.StrType s) := FALSE;
...
EXPORT Make_ar_tax_amount_paid(SALT34.StrType s0) := MakeFT_invalid_money(s0);
EXPORT InValid_ar_tax_amount_paid(SALT34.StrType s) := InValidFT_invalid_money(s);
EXPORT InValidMessage_ar_tax_amount_paid(UNSIGNED1 wh) := InValidMessageFT_invalid_money(wh);
...
EXPORT Make_ar_illinois_capital(SALT34.StrType s0) := MakeFT_invalid_money(s0);
EXPORT InValid_ar_illinois_capital(SALT34.StrType s) := InValidFT_invalid_money(s);
EXPORT InValidMessage_ar_illinois_capital(UNSIGNED1 wh) := InValidMessageFT_invalid_money(wh);
...
EXPORT Make_ar_amount_paid(SALT34.StrType s0) := s0;
EXPORT InValid_ar_amount_paid(SALT34.StrType s) := FALSE;
EXPORT InValidMessage_ar_amount_paid(UNSIGNED1 wh) := '';
 
// This macro will compute and count field level differences based upon a pivot expression
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT34,Scrubs_Corp2_Mapping_IL_AR;

...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(corp2_mapping.layoutscommon.ar)
    ...
    UNSIGNED1 ar_illinois_capital_Invalid;
...
EXPORT FromNone(DATASET(corp2_mapping.layoutscommon.ar) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.ar_tax_amount_paid_Invalid := Fields.InValid_ar_tax_amount_paid((SALT34.StrType)le.ar_tax_amount_paid);
    SELF.ar_illinois_capital_Invalid := Fields.InValid_ar_illinois_capital((SALT34.StrType)le.ar_illinois_capital);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.corp_key_Invalid << 0 ) + ( le.corp_vendor_Invalid << 2 ) + ( le.corp_state_origin_Invalid << 3 ) + ( le.corp_process_date_Invalid << 4 ) + ( le.corp_sos_charter_nbr_Invalid << 6 ) + ( le.ar_year_Invalid << 8 ) + ( le.ar_mailed_dt_Invalid << 9 ) + ( le.ar_filed_dt_Invalid << 11 ) + ( le.ar_delinquent_dt_Invalid << 13 ) + ( le.ar_tax_factor_Invalid << 15 ) + ( le.ar_tax_amount_paid_Invalid << 16 ) + ( le.ar_illinois_capital_Invalid << 17 ) + ( le.ar_paid_date_Invalid << 18 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.ar_delinquent_dt_Invalid := (le.ScrubsBits1 >> 13) & 3;
    SELF.ar_tax_factor_Invalid := (le.ScrubsBits1 >> 15) & 1;
    SELF.ar_tax_amount_paid_Invalid := (le.ScrubsBits1 >> 16) & 1;
    SELF.ar_illinois_capital_Invalid := (le.ScrubsBits1 >> 17) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    ar_illinois_capital_ALLOW_ErrorCount := COUNT(GROUP,h.ar_illinois_capital_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.corp_key_Invalid,le.corp_vendor_Invalid,le.corp_state_origin_Invalid,le.corp_process_date_Invalid,le.corp_sos_charter_nbr_Invalid,le.ar_year_Invalid,le.ar_mailed_dt_Invalid,le.ar_filed_dt_Invalid,le.ar_delinquent_dt_Invalid,le.ar_tax_factor_Invalid,le.ar_tax_amount_paid_Invalid,le.ar_illinois_capital_Invalid,le.ar_paid_date_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_corp_key(le.corp_key_Invalid),Fields.InvalidMessage_corp_vendor(le.corp_vendor_Invalid),Fields.InvalidMessage_corp_state_origin(le.corp_state_origin_Invalid),Fields.InvalidMessage_corp_process_date(le.corp_process_date_Invalid),Fields.InvalidMessage_corp_sos_charter_nbr(le.corp_sos_charter_nbr_Invalid),Fields.InvalidMessage_ar_year(le.ar_year_Invalid),Fields.InvalidMessage_ar_mailed_dt(le.ar_mailed_dt_Invalid),Fields.InvalidMessage_ar_filed_dt(le.ar_filed_dt_Invalid),Fields.InvalidMessage_ar_delinquent_dt(le.ar_delinquent_dt_Invalid),Fields.InvalidMessage_ar_tax_factor(le.ar_tax_factor_Invalid),Fields.InvalidMessage_ar_tax_amount_paid(le.ar_tax_amount_paid_Invalid),Fields.InvalidMessage_ar_illinois_capital(le.ar_illinois_capital_Invalid),Fields.InvalidMessage_ar_paid_date(le.ar_paid_date_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.ar_illinois_capital_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.ar_paid_date_Invalid,'ALLOW','CUSTOM','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_sos_charter_nbr','ar_year','ar_mailed_dt','ar_filed_dt','ar_delinquent_dt','ar_tax_factor','ar_tax_amount_paid','ar_illinois_capital','ar_paid_date','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_numeric','invalid_date','invalid_date','invalid_date','invalid_percentage','invalid_money','invalid_money','invalid_date','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT34.StrType)le.corp_key,(SALT34.StrType)le.corp_vendor,(SALT34.StrType)le.corp_state_origin,(SALT34.StrType)le.corp_process_date,(SALT34.StrType)le.corp_sos_charter_nbr,(SALT34.StrType)le.ar_year,(SALT34.StrType)le.ar_mailed_dt,(SALT34.StrType)le.ar_filed_dt,(SALT34.StrType)le.ar_delinquent_dt,(SALT34.StrType)le.ar_tax_factor,(SALT34.StrType)le.ar_tax_amount_paid,(SALT34.StrType)le.ar_illinois_capital,(SALT34.StrType)le.ar_paid_date,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'ar_tax_amount_paid:invalid_money:ALLOW'
          ,'ar_illinois_capital:invalid_money:ALLOW'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_ar_illinois_capital(1)
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.ar_illinois_capital_ALLOW_ErrorCount

...
EXPORT GetIDs(BairRx_Common.IParam.SearchParam inMod) := FUNCTION
		...
		MC := inMod.ModeContext(mID=BairRx_Common.Constants.Mode.SHO)[1];
		DR := MC.data_relation;
		...
		dBoolSrchRecs := IF(booleanSearch,BairRx_Common.BooleanFilterSearch(inMod,BairRx_Common.Constants.SearchType.ShotSpotter,MC.filters,GH.SearchGrid,BoolEIDs,booleanSearch));						

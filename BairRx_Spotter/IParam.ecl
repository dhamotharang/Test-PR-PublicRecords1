...
EXPORT IParam := MODULE
	...
	EXPORT getSearchParams(iesp.bair_shotspotter.t_BAIRShotSpotterSearchBy searchBy) := FUNCTION
		
		base_params := BairRx_Common.IParam.getSearchParams(searchBy, BairRx_Common.Constants.SearchType.ShotSpotter);
		
		BairRx_Common.Layouts.ModeContext xtModeContext() := TRANSFORM						
			SELF.mID := BairRx_Common.Constants.Mode.SHO,
			SELF.codes := BairRx_Common.Functions.SplitClassCodes(BairRx_Common.Constants.Mode.SHO, searchBy.Classification),
			SELF.clean_codes := BairRx_Common.Functions.SplitClassCodes(BairRx_Common.Constants.Mode.SHO, searchBy.Classification, TRUE),
		...
		BairRx_Common.Layouts.ActiveModes xtActiveMode() := TRANSFORM
			SELF.SHO := exists(inModeContext[1].codes);
		END;

...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Layout_POWID_Down)
    UNSIGNED1 st_Invalid;
    UNSIGNED1 zip_Invalid;
  END;
  EXPORT  Bitmap_Layout := RECORD(Layout_POWID_Down)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(Layout_POWID_Down) h) := MODULE
  ...
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Layout_POWID_Down);
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,Layout_POWID_Down);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.FieldContents := CHOOSE(c,(SALT35.StrType)le.st,(SALT35.StrType)le.zip,'***SALTBUG***');
  END;

...
EXPORT input_file := h0  : PERSIST('~temp::POWID::BIPV2_POWID_Down::Specificities_Cache');
//We have POWID specified - so we can compute statistics on the cluster counts
  r0 := RECORD
    input_file.POWID;
    UNSIGNED6 InCluster := COUNT(GROUP);
  end;
EXPORT ClusterSizes := TABLE(input_file,r0,POWID,LOCAL)  : PERSIST('~temp::POWID::BIPV2_POWID_Down::Cluster_Sizes');

...
pj3 do_computes(pj3 le) := TRANSFORM
  ...
  SELF := le;
END;
SHARED propogated := PROJECT(pj3,do_computes(left)) : PERSIST('~temp::BIPV2_POWID_Down_POWID_POWID_Down_mc_props'); // to allow to 'jump' over an exported value
...
SHARED Annotated := DISTRIBUTE(j0,hash(POWID)) : PERSIST('~temp::BIPV2_POWID_Down_POWID_Down_mc'); // Distributed for keybuild case

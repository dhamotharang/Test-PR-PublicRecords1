...
EXPORT POWIDCompareService := MACRO
  IMPORT SALT27,BIPV2_POWID_Down,ut;
...
odl := PROJECT(CHOOSEN(BIPV2_POWID_Down.Keys(BFile).Candidates(POWID=POWIDone),100000),BIPV2_POWID_Down.match_candidates(BFile).layout_candidates);
odr := PROJECT(CHOOSEN(BIPV2_POWID_Down.Keys(BFile).Candidates(POWID=POWIDTwo),100000),BIPV2_POWID_Down.match_candidates(BFile).layout_candidates);
k := BIPV2_POWID_Down.Keys(BFile).Specificities_Key;
s := GLOBAL(PROJECT(k,BIPV2_POWID_Down.Layout_Specificities.R)[1]);
odlv := BIPV2_POWID_Down.Debug(BFile,s).RolledEntities(odl);
odrv := BIPV2_POWID_Down.Debug(BFile,s).RolledEntities(odr);
mtch := BIPV2_POWID_Down.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,POWIDone,POWIDtwo,0,0}],BIPV2_POWID_Down.match_candidates(BFile).layout_matches));

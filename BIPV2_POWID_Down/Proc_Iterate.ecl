//Executable code
EXPORT Proc_Iterate(STRING iter,DATASET(Layout_POWID_Down) InFile0 = BIPV2_POWID_Down.In_POWID_Down,STRING OutFileNameP = '~temp::POWID::BIPV2_POWID_Down::it',UNSIGNED MatchThreshold = 38,BOOLEAN Debugging = true) := MODULE
...
MP := PARALLEL( OUTPUT(MM.MatchesPerformed,NAMED('MatchesPerformed')),OUTPUT(BIPV2_POWID_Down.BasicMatch(InFile).basic_match_count,NAMED('BasicMatchesPerformed')));
...
d := DATASET([{MM.PatchingError0,MM.DidsNoRid0,MM.DidsAboveRid0,MM.DuplicateRids0,COUNT(BIPV2_POWID_Down.match_candidates(InFile).Unlinkables)}],{INTEGER PatchingError0, INTEGER DidsNoRid0, INTEGER DidsAboveRid0, INTEGER DuplicateRids0, UNSIGNED UnlinkableRecords0});
...
EXPORT OutputChanges := OUTPUT(MM.IdChanges,,'~temp::POWID::BIPV2_POWID_Down::changes_it'+iter,OVERWRITE,COMPRESSED);// Changes made
//EXPORT LoopN(UNSIGNED N,UNSIGNED mt=38) := LOOP(BIPV2_POWID_Down.In_POWID_Down,N,BIPV2_POWID_Down.matches(ROWS(LEFT),mt).patched_infile); // Loop N times
//EXPORT LoopThisN(DATASET(BIPV2_POWID_Down.Layout_POWID_Down)d,UNSIGNED N,UNSIGNED mt=38) := LOOP(d,N,BIPV2_POWID_Down.matches(ROWS(LEFT),mt).patched_infile);
//EXPORT LoopThisK(DATASET(BIPV2_POWID_Down.Layout_POWID_Down)d,UNSIGNED K,UNSIGNED NMax=100) := LOOP(d,COUNTER<=NMax AND COUNT(DEDUP(ROWS(LEFT),POWID,ALL))>K,BIPV2_POWID_Down.matches(ROWS(LEFT),0).patched_infile); // Loop until K clusters left

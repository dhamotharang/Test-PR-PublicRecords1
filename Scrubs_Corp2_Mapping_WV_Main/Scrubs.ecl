...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Layout_In_File)
    ...
    UNSIGNED1 corp_action_pending_cd_Invalid;
    UNSIGNED1 corp_action_pending_desc_Invalid;
    UNSIGNED1 corp_country_of_formation_Invalid;
    UNSIGNED1 corp_termination_cd_Invalid;
    UNSIGNED1 corp_termination_desc_Invalid;
...
EXPORT FromNone(DATASET(Layout_In_File) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_action_pending_cd_Invalid := Fields.InValid_corp_action_pending_cd((SALT34.StrType)le.corp_action_pending_cd);
    SELF.corp_action_pending_desc_Invalid := Fields.InValid_corp_action_pending_desc((SALT34.StrType)le.corp_action_pending_desc);
    ...
    SELF.corp_termination_cd_Invalid := Fields.InValid_corp_termination_cd((SALT34.StrType)le.corp_termination_cd);
    SELF.corp_termination_desc_Invalid := Fields.InValid_corp_termination_desc((SALT34.StrType)le.corp_termination_desc);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_inc_state_Invalid << 22 ) + ( le.corp_inc_county_Invalid << 23 ) + ( le.corp_inc_date_Invalid << 24 ) + ( le.corp_foreign_domestic_ind_Invalid << 26 ) + ( le.corp_forgn_state_cd_Invalid << 27 ) + ( le.corp_forgn_state_desc_Invalid << 28 ) + ( le.corp_forgn_date_Invalid << 29 ) + ( le.corp_orig_org_structure_cd_Invalid << 31 ) + ( le.corp_orig_org_structure_desc_Invalid << 32 ) + ( le.corp_for_profit_ind_Invalid << 33 ) + ( le.corp_action_pending_cd_Invalid << 34 ) + ( le.corp_action_pending_desc_Invalid << 35 ) + ( le.corp_country_of_formation_Invalid << 36 ) + ( le.corp_termination_cd_Invalid << 37 ) + ( le.corp_termination_desc_Invalid << 38 ) + ( le.recordorigin_Invalid << 39 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_inc_state_Invalid := (le.ScrubsBits1 >> 22) & 1;
    SELF.corp_inc_county_Invalid := (le.ScrubsBits1 >> 23) & 1;
    ...
    SELF.corp_action_pending_cd_Invalid := (le.ScrubsBits1 >> 34) & 1;
    SELF.corp_action_pending_desc_Invalid := (le.ScrubsBits1 >> 35) & 1;
    SELF.corp_country_of_formation_Invalid := (le.ScrubsBits1 >> 36) & 1;
    SELF.corp_termination_cd_Invalid := (le.ScrubsBits1 >> 37) & 1;
    SELF.corp_termination_desc_Invalid := (le.ScrubsBits1 >> 38) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_action_pending_cd_ENUM_ErrorCount := COUNT(GROUP,h.corp_action_pending_cd_Invalid=1);
    corp_action_pending_desc_ALLOW_ErrorCount := COUNT(GROUP,h.corp_action_pending_desc_Invalid=1);
    corp_country_of_formation_ALLOW_ErrorCount := COUNT(GROUP,h.corp_country_of_formation_Invalid=1);
    corp_termination_cd_ENUM_ErrorCount := COUNT(GROUP,h.corp_termination_cd_Invalid=1);
    corp_termination_desc_ALLOW_ErrorCount := COUNT(GROUP,h.corp_termination_desc_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_action_pending_cd_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.corp_action_pending_desc_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.corp_country_of_formation_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.corp_termination_cd_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.corp_termination_desc_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.recordorigin_Invalid,'ENUM','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'dt_vendor_first_reported','dt_vendor_last_reported','dt_first_seen','dt_last_seen','corp_ra_dt_first_seen','corp_ra_dt_last_seen','corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_orig_sos_charter_nbr','corp_legal_name','corp_ln_name_type_cd','corp_inc_state','corp_inc_county','corp_inc_date','corp_foreign_domestic_ind','corp_forgn_state_cd','corp_forgn_state_desc','corp_forgn_date','corp_orig_org_structure_cd','corp_orig_org_structure_desc','corp_for_profit_ind','corp_action_pending_cd','corp_action_pending_desc','corp_country_of_formation','corp_termination_cd','corp_termination_desc','recordorigin','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter','invalid_mandatory','invalid_name_type_code','invalid_state_origin','invalid_alphablank','invalid_date','invalid_foreign_domestic_ind','invalid_alphablank','invalid_alphablank','invalid_date','invalid_org_structure_cd','invalid_org_structure_desc','invalid_flag_code','invalid_Termination_Cd','invalid_Termination_Desc','invalid_country_of_formation','invalid_Termination_Cd','invalid_Termination_Desc','invalid_recordorigin','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT34.StrType)le.dt_vendor_first_reported,(SALT34.StrType)le.dt_vendor_last_reported,(SALT34.StrType)le.dt_first_seen,(SALT34.StrType)le.dt_last_seen,(SALT34.StrType)le.corp_ra_dt_first_seen,(SALT34.StrType)le.corp_ra_dt_last_seen,(SALT34.StrType)le.corp_key,(SALT34.StrType)le.corp_vendor,(SALT34.StrType)le.corp_state_origin,(SALT34.StrType)le.corp_process_date,(SALT34.StrType)le.corp_orig_sos_charter_nbr,(SALT34.StrType)le.corp_legal_name,(SALT34.StrType)le.corp_ln_name_type_cd,(SALT34.StrType)le.corp_inc_state,(SALT34.StrType)le.corp_inc_county,(SALT34.StrType)le.corp_inc_date,(SALT34.StrType)le.corp_foreign_domestic_ind,(SALT34.StrType)le.corp_forgn_state_cd,(SALT34.StrType)le.corp_forgn_state_desc,(SALT34.StrType)le.corp_forgn_date,(SALT34.StrType)le.corp_orig_org_structure_cd,(SALT34.StrType)le.corp_orig_org_structure_desc,(SALT34.StrType)le.corp_for_profit_ind,(SALT34.StrType)le.corp_action_pending_cd,(SALT34.StrType)le.corp_action_pending_desc,(SALT34.StrType)le.corp_country_of_formation,(SALT34.StrType)le.corp_termination_cd,(SALT34.StrType)le.corp_termination_desc,(SALT34.StrType)le.recordorigin,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_action_pending_cd:invalid_Termination_Cd:ENUM'
          ,'corp_action_pending_desc:invalid_Termination_Desc:ALLOW'
          ,'corp_country_of_formation:invalid_country_of_formation:ALLOW'
          ,'corp_termination_cd:invalid_Termination_Cd:ENUM'
          ,'corp_termination_desc:invalid_Termination_Desc:ALLOW'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_corp_action_pending_cd(1)
          ,Fields.InvalidMessage_corp_action_pending_desc(1)
          ,Fields.InvalidMessage_corp_country_of_formation(1)
          ,Fields.InvalidMessage_corp_termination_cd(1)
          ,Fields.InvalidMessage_corp_termination_desc(1)
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_action_pending_cd_ENUM_ErrorCount
          ,le.corp_action_pending_desc_ALLOW_ErrorCount
          ,le.corp_country_of_formation_ALLOW_ErrorCount
          ,le.corp_termination_cd_ENUM_ErrorCount
          ,le.corp_termination_desc_ALLOW_ErrorCount

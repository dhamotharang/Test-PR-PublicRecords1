...
EXPORT FromNone(DATASET(Corp2_Mapping.LayoutsCommon.Events) h) := MODULE
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.corp_key_Invalid << 0 ) + ( le.corp_vendor_Invalid << 2 ) + ( le.corp_state_origin_Invalid << 3 ) + ( le.corp_process_date_Invalid << 4 ) + ( le.corp_sos_charter_nbr_Invalid << 6 ) + ( le.event_filing_reference_nbr_Invalid << 7 ) + ( le.event_filing_cd_Invalid << 8 ) + ( le.event_filing_desc_Invalid << 9 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_sos_charter_nbr_Invalid := (le.ScrubsBits1 >> 6) & 1;
    SELF.event_filing_reference_nbr_Invalid := (le.ScrubsBits1 >> 7) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_sos_charter_nbr_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.event_filing_reference_nbr_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.event_filing_cd_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.event_filing_desc_Invalid,'ALLOW','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_sos_charter_nbr','event_filing_reference_nbr','event_filing_cd','event_filing_desc','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_alphanumericblank','invalid_numericblank','invalid_alphaslashblank','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT32.StrType)le.corp_key,(SALT32.StrType)le.corp_vendor,(SALT32.StrType)le.corp_state_origin,(SALT32.StrType)le.corp_process_date,(SALT32.StrType)le.corp_sos_charter_nbr,(SALT32.StrType)le.event_filing_reference_nbr,(SALT32.StrType)le.event_filing_cd,(SALT32.StrType)le.event_filing_desc,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT32.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_sos_charter_nbr:invalid_charter_nbr:ALLOW'
          ,'event_filing_reference_nbr:invalid_alphanumericblank:ALLOW'
          ,'event_filing_cd:invalid_numericblank:ALLOW'
          ,'event_filing_desc:invalid_alphaslashblank:ALLOW','UNKNOWN');
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_corp_sos_charter_nbr(1)
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_sos_charter_nbr_ALLOW_ErrorCount
          ,le.event_filing_reference_nbr_ALLOW_ErrorCount
          ,le.event_filing_cd_ALLOW_ErrorCount

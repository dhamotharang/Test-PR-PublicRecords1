...
EXPORT SALT32.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_future_date','invalid_charter_nbr','invalid_numericblank','invalid_numericperiodblank','invalid_alphablank','invalid_alphanumericblank','invalid_alphaslashblank');
EXPORT FieldTypeNum(SALT32.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_corp_vendor' => 2,'invalid_state_origin' => 3,'invalid_mandatory' => 4,'invalid_date' => 5,'invalid_future_date' => 6,'invalid_charter_nbr' => 7,'invalid_numericblank' => 8,'invalid_numericperiodblank' => 9,'invalid_alphablank' => 10,'invalid_alphanumericblank' => 11,'invalid_alphaslashblank' => 12,0);
...
EXPORT MakeFT_invalid_charter_nbr(SALT32.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_invalid_charter_nbr(SALT32.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT32.StringFilter(s,' -0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'))));
EXPORT InValidMessageFT_invalid_charter_nbr(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInChars(' -0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'),SALT32.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_alphaslashblank(SALT32.StrType s0) := FUNCTION
  s1 := SALT32.stringfilter(s0,'\'-/().ABCDEFGHIJKLMNOPQRSTUVWXYZ '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_alphaslashblank(SALT32.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT32.StringFilter(s,'\'-/().ABCDEFGHIJKLMNOPQRSTUVWXYZ '))));
EXPORT InValidMessageFT_invalid_alphaslashblank(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInChars('\'-/().ABCDEFGHIJKLMNOPQRSTUVWXYZ '),SALT32.HygieneErrors.Good);
...
EXPORT Make_event_filing_reference_nbr(SALT32.StrType s0) := MakeFT_invalid_alphanumericblank(s0);
EXPORT InValid_event_filing_reference_nbr(SALT32.StrType s) := InValidFT_invalid_alphanumericblank(s);
EXPORT InValidMessage_event_filing_reference_nbr(UNSIGNED1 wh) := InValidMessageFT_invalid_alphanumericblank(wh);
 
EXPORT Make_event_amendment_nbr(SALT32.StrType s0) := s0;
EXPORT InValid_event_amendment_nbr(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_amendment_nbr(UNSIGNED1 wh) := '';
 
EXPORT Make_event_filing_date(SALT32.StrType s0) := s0;
EXPORT InValid_event_filing_date(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_filing_date(UNSIGNED1 wh) := '';
 
EXPORT Make_event_date_type_cd(SALT32.StrType s0) := s0;
EXPORT InValid_event_date_type_cd(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_date_type_cd(UNSIGNED1 wh) := '';
 
EXPORT Make_event_date_type_desc(SALT32.StrType s0) := s0;
EXPORT InValid_event_date_type_desc(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_date_type_desc(UNSIGNED1 wh) := '';
 
EXPORT Make_event_filing_cd(SALT32.StrType s0) := MakeFT_invalid_numericblank(s0);
EXPORT InValid_event_filing_cd(SALT32.StrType s) := InValidFT_invalid_numericblank(s);
EXPORT InValidMessage_event_filing_cd(UNSIGNED1 wh) := InValidMessageFT_invalid_numericblank(wh);
 
EXPORT Make_event_filing_desc(SALT32.StrType s0) := MakeFT_invalid_alphaslashblank(s0);
EXPORT InValid_event_filing_desc(SALT32.StrType s) := InValidFT_invalid_alphaslashblank(s);
EXPORT InValidMessage_event_filing_desc(UNSIGNED1 wh) := InValidMessageFT_invalid_alphaslashblank(wh);
 
EXPORT Make_event_corp_nbr(SALT32.StrType s0) := s0;
EXPORT InValid_event_corp_nbr(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_corp_nbr(UNSIGNED1 wh) := '';
 
EXPORT Make_event_corp_nbr_cd(SALT32.StrType s0) := s0;
EXPORT InValid_event_corp_nbr_cd(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_corp_nbr_cd(UNSIGNED1 wh) := '';
 
EXPORT Make_event_corp_nbr_desc(SALT32.StrType s0) := s0;
EXPORT InValid_event_corp_nbr_desc(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_corp_nbr_desc(UNSIGNED1 wh) := '';
 
EXPORT Make_event_roll(SALT32.StrType s0) := s0;
EXPORT InValid_event_roll(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_roll(UNSIGNED1 wh) := '';
 
EXPORT Make_event_frame(SALT32.StrType s0) := s0;
EXPORT InValid_event_frame(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_frame(UNSIGNED1 wh) := '';
 
EXPORT Make_event_start(SALT32.StrType s0) := s0;
EXPORT InValid_event_start(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_start(UNSIGNED1 wh) := '';
 
EXPORT Make_event_end(SALT32.StrType s0) := s0;
EXPORT InValid_event_end(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_end(UNSIGNED1 wh) := '';
 
EXPORT Make_event_microfilm_nbr(SALT32.StrType s0) := s0;
EXPORT InValid_event_microfilm_nbr(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_microfilm_nbr(UNSIGNED1 wh) := '';
 
EXPORT Make_event_desc(SALT32.StrType s0) := s0;
EXPORT InValid_event_desc(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_desc(UNSIGNED1 wh) := '';
 
EXPORT Make_event_revocation_comment1(SALT32.StrType s0) := s0;
EXPORT InValid_event_revocation_comment1(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_revocation_comment1(UNSIGNED1 wh) := '';
 
EXPORT Make_event_revocation_comment2(SALT32.StrType s0) := s0;
EXPORT InValid_event_revocation_comment2(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_revocation_comment2(UNSIGNED1 wh) := '';
 
EXPORT Make_event_book_number(SALT32.StrType s0) := s0;
EXPORT InValid_event_book_number(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_book_number(UNSIGNED1 wh) := '';
 
EXPORT Make_event_page_number(SALT32.StrType s0) := s0;
EXPORT InValid_event_page_number(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_page_number(UNSIGNED1 wh) := '';
 
EXPORT Make_event_certification_number(SALT32.StrType s0) := s0;
EXPORT InValid_event_certification_number(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_event_certification_number(UNSIGNED1 wh) := '';
 
// This macro will compute and count field level differences based upon a pivot expression
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT32,Scrubs_Corp2_Mapping_MA_Event;

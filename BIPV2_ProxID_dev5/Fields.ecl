...
EXPORT SALT27.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'cnp_number','prim_range','prim_name','st','active_enterprise_number','hist_enterprise_number','hist_domestic_corp_key','unk_corp_key','ebr_file_number','active_duns_number','active_domestic_corp_key','hist_duns_number','foreign_corp_key','company_fein','company_phone','cnp_name','zip','company_csz','sec_range','v_city_name','cnp_btype','iscorp','company_name','company_name_type_raw','company_name_type_derived','cnp_hasnumber','cnp_lowv','cnp_translated','cnp_classid','company_foreign_domestic','company_bdid','company_addr1','company_address','dt_first_seen','dt_last_seen');
EXPORT FieldNum(SALT27.StrType fn) := CASE(fn,'cnp_number' => 1,'prim_range' => 2,'prim_name' => 3,'st' => 4,'active_enterprise_number' => 5,'hist_enterprise_number' => 6,'hist_domestic_corp_key' => 7,'unk_corp_key' => 8,'ebr_file_number' => 9,'active_duns_number' => 10,'active_domestic_corp_key' => 11,'hist_duns_number' => 12,'foreign_corp_key' => 13,'company_fein' => 14,'company_phone' => 15,'cnp_name' => 16,'zip' => 17,'company_csz' => 18,'sec_range' => 19,'v_city_name' => 20,'cnp_btype' => 21,'iscorp' => 22,'company_name' => 23,'company_name_type_raw' => 24,'company_name_type_derived' => 25,'cnp_hasnumber' => 26,'cnp_lowv' => 27,'cnp_translated' => 28,'cnp_classid' => 29,'company_foreign_domestic' => 30,'company_bdid' => 31,'company_addr1' => 32,'company_address' => 33,'dt_first_seen' => 34,'dt_last_seen' => 35,0);
...
EXPORT Make_company_name_type_raw(SALT27.StrType s0) := s0;
EXPORT InValid_company_name_type_raw(SALT27.StrType s) := FALSE;
EXPORT InValidMessage_company_name_type_raw(UNSIGNED1 wh) := '';
 
EXPORT Make_company_name_type_derived(SALT27.StrType s0) := s0;
EXPORT InValid_company_name_type_derived(SALT27.StrType s) := FALSE;
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT27,BIPV2_ProxID_dev5;
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_cnp_number,1,0)+ IF( SELF.Diff_prim_range,1,0)+ IF( SELF.Diff_prim_name,1,0)+ IF( SELF.Diff_st,1,0)+ IF( SELF.Diff_active_enterprise_number,1,0)+ IF( SELF.Diff_hist_enterprise_number,1,0)+ IF( SELF.Diff_hist_domestic_corp_key,1,0)+ IF( SELF.Diff_unk_corp_key,1,0)+ IF( SELF.Diff_ebr_file_number,1,0)+ IF( SELF.Diff_active_duns_number,1,0)+ IF( SELF.Diff_active_domestic_corp_key,1,0)+ IF( SELF.Diff_hist_duns_number,1,0)+ IF( SELF.Diff_foreign_corp_key,1,0)+ IF( SELF.Diff_company_fein,1,0)+ IF( SELF.Diff_company_phone,1,0)+ IF( SELF.Diff_cnp_name,1,0)+ IF( SELF.Diff_zip,1,0)+ IF( SELF.Diff_sec_range,1,0)+ IF( SELF.Diff_v_city_name,1,0)+ IF( SELF.Diff_cnp_btype,1,0)+ IF( SELF.Diff_iscorp,1,0)+ IF( SELF.Diff_company_name,1,0)+ IF( SELF.Diff_company_name_type_raw,1,0)+ IF( SELF.Diff_company_name_type_derived,1,0)+ IF( SELF.Diff_cnp_hasnumber,1,0)+ IF( SELF.Diff_cnp_lowv,1,0)+ IF( SELF.Diff_cnp_translated,1,0)+ IF( SELF.Diff_cnp_classid,1,0)+ IF( SELF.Diff_company_foreign_domestic,1,0)+ IF( SELF.Diff_company_bdid,1,0);
  END;

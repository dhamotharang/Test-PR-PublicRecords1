...
END;
EXPORT layout_sample_matches sample_match_join(match_candidates(ih).layout_candidates le,match_candidates(ih).layout_candidates ri,UNSIGNED outside=0,UNSIGNED cnp_name_support = 0) := TRANSFORM
  ...
  INTEGER2 cnp_name_score_supp := MIN(IF(cnp_name_support>0,MAX(cnp_name_score_temp,cnp_name_support),cnp_name_score_temp),s.cnp_name_MAX*100); // Add support
  ...
  SELF.cnp_number_score := IF ( le.cnp_number = ri.cnp_number or cnp_number_score_temp >= 0, cnp_number_score_temp, -9999 ); // Enforce FORCE parameter
  ...
  self.cnp_name_score := IF ( le.cnp_name = ri.cnp_name 
    or (cnp_name_score_supp >= 1300 and cnp_name_support = 0) 
    or (cnp_name_score_supp >= 1300 and cnp_name_score_temp < 1300 and cnp_name_support > 0 and regexfind('fbn|dba|fictitious|assumed|trade',le.company_name_type_raw + le.company_name_type_derived + ri.company_name_type_raw + ri.company_name_type_derived,nocase))
    
    , cnp_name_score_supp, -9999 ); // Enforce FORCE parameter
  ...
  SELF.prim_range_score := IF ( le.prim_range = ri.prim_range or prim_range_score_temp >= 0 OR company_addr1_score_pre > 0 OR company_address_score_pre > 0, prim_range_score_temp, -9999 ); // Enforce FORCE parameter
...
// Compute the score for the concept company_address
  ...
  SELF.Conf := (SELF.cnp_number_score + SELF.prim_range_score + SELF.prim_name_score + SELF.st_score + SELF.active_enterprise_number_score + SELF.hist_enterprise_number_score + SELF.hist_duns_number_score + SELF.hist_domestic_corp_key_score + SELF.unk_corp_key_score + SELF.ebr_file_number_score + SELF.active_duns_number_score + SELF.foreign_corp_key_score + SELF.company_phone_score + SELF.active_domestic_corp_key_score + SELF.cnp_name_score + SELF.company_fein_score + SELF.zip_score + SELF.company_csz_score + SELF.sec_range_score + SELF.v_city_name_score + SELF.cnp_btype_score + SELF.iscorp_score + SELF.company_addr1_score + SELF.company_address_score) / 100 + outside;
END;
...
EXPORT Layout_RolledEntity := RECORD,MAXLENGTH(63000)
  ...
  DATASET(SALT27.Layout_FieldValueList) company_name_type_raw_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) company_name_type_derived_Values := DATASET([],SALT27.Layout_FieldValueList);
...
Layout_RolledEntity RollValues(Layout_RolledEntity le,Layout_RolledEntity ri) := TRANSFORM
  ...
  SELF.company_name_type_raw_values := SALT27.fn_combine_fieldvaluelist(le.company_name_type_raw_values,ri.company_name_type_raw_values);
  SELF.company_name_type_derived_values := SALT27.fn_combine_fieldvaluelist(le.company_name_type_derived_values,ri.company_name_type_derived_values);
...
Layout_RolledEntity into(in_data le) := TRANSFORM
  ...
  SELF.company_name_type_raw_Values := DATASET([{TRIM((SALT27.StrType)le.company_name_type_raw)}],SALT27.Layout_FieldValueList);
  SELF.company_name_type_derived_Values := DATASET([{TRIM((SALT27.StrType)le.company_name_type_derived)}],SALT27.Layout_FieldValueList);

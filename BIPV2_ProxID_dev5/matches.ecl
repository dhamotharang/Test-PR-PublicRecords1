...
SHARED match_candidates(ih).layout_matches match_join(match_candidates(ih).layout_candidates le,match_candidates(ih).layout_candidates ri,UNSIGNED2 c,UNSIGNED outside=0,UNSIGNED cnp_name_support = 0) := TRANSFORM
  SELF.Rule := c;
  ...
  INTEGER2 cnp_name_score := IF ( le.cnp_name = ri.cnp_name 
    ...
    , cnp_name_score_supp, SKIP ); // Enforce FORCE parameter
...
match_candidates(ih).Layout_Attribute_Matches ScoreSrcRidVlid(match_candidates(ih).SrcRidVlid_candidates le,match_candidates(ih).SrcRidVlid_candidates ri,UNSIGNED cnp_name_support = 0) := TRANSFORM
  ...
  SELF.Conf_Prop := (cnp_number_score + active_enterprise_number_score + active_duns_number_score + active_domestic_corp_key_score) / 100; // Score based on forced fields
  SELF.support_cnp_name := ri.Basis_weight100 *  1.00;
end;
...
SrcRidVlid_matches := DEDUP( SORT(SrcRidVlid_matches0,Proxid1,Proxid2,-(Conf+Conf_Prop+support_cnp_name),Source_Id,LOCAL),Proxid1,Proxid2,KEEP(1),LOCAL ); // Keep 1 source_ids per match
 
AllAttrMatches := SORT(SrcRidVlid_Matches,Proxid1,Proxid2,Rule,-(Conf+Conf_Prop+support_cnp_name),LOCAL);
match_candidates(ih).Layout_Attribute_Matches CombineResults(match_candidates(ih).Layout_Attribute_Matches le,match_candidates(ih).Layout_Attribute_Matches ri) := TRANSFORM
  ...
  SELF := le;
END;
EXPORT All_Attribute_Matches := ROLLUP(AllAttrMatches,LEFT.Proxid1=RIGHT.Proxid1 AND LEFT.Proxid2=RIGHT.Proxid2,CombineResults(LEFT,RIGHT),LOCAL):PERSIST('~temp::attribute_matches::Proxid::BIPV2_ProxID_dev5');
...
match_candidates(ih).layout_candidates strim(j1 le) := TRANSFORM
  SELF := le;
END;
attr_match := JOIN(DISTRIBUTE(j1,HASH(Proxid1)),hd,LEFT.Proxid1 = RIGHT.Proxid,match_join( RIGHT,PROJECT(LEFT,strim(LEFT)),LEFT.Rule, LEFT.Conf,LEFT.support_cnp_name),LOCAL); // Will be distributed by DID1
with_attr := attr_match + all_mjs;
export All_Matches := with_attr : persist('~temp::BIPV2_ProxID_dev5_Proxid_DOT_Base_all_m'); // To by used by rcid and Proxid
SALT27.mac_avoid_transitives(All_Matches,Proxid1,Proxid2,Conf,DateOverlap,Rule,o);
export PossibleMatches := o : persist('~temp::BIPV2_ProxID_dev5_Proxid_DOT_Base_mt');
SALT27.mac_get_BestPerRecord( All_Matches,rcid1,Proxid1,rcid2,Proxid2,o );
EXPORT BestPerRecord := o : PERSIST('~temp::BIPV2_ProxID_dev5_Proxid_DOT_Base_mr');
...
EXPORT ClusterLinkages := o1 : PERSIST('~temp::BIPV2_ProxID_dev5_Proxid_DOT_Base_clu');

...
import bipv2,BIPV2_ProxID_dev5,Business_DOT_SALT_micro12,BIPV2_Tools;

dinput 						:= BIPV2_ProxID_dev5.In_DOT_Base;

dlegalentityrels 	:= BIPV2_ProxID_dev5.relationships(dinput).SAMELEGALENTITY_links	;
...
BIPV2_ProxID_dev5.Proc_Iterate('1').RelationshipKeys;
BIPV2_ProxID_dev5.Proc_Iterate('1').DebugKeys;
...
output(dpatchlegalentitycluster_proj		,,'~thor_data400::base::BIPV2_ProxID_dev5::legalentity_patched_file_filt'	,__compressed__,overwrite);
output(dbadlegalentityrels 							,,'~thor_data400::base::BIPV2_ProxID_dev5::badlegalentity_rels'						,__compressed__,overwrite);

////////////////extra outputs
p := project(dpatchlegalentitycluster_proj, transform(BIPV2_ProxID_dev5.layouts.dot_base, self.dotid := left.proxid, self := left));

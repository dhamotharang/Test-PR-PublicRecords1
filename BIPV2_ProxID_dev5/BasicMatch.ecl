...
// It is important that this is an EQUIVALENCE relationship - it allows us to form an implicit transitive closure
  h01 := SORT(h00,active_duns_number,active_enterprise_number,active_domestic_corp_key,hist_enterprise_number,hist_duns_number,hist_domestic_corp_key,foreign_corp_key,unk_corp_key,ebr_file_number,cnp_name,cnp_number,cnp_btype,company_fein,company_phone,iscorp,prim_range,prim_name,sec_range,v_city_name,st,zip,Proxid);
  h02 := DEDUP(h01,active_duns_number,active_enterprise_number,active_domestic_corp_key,hist_enterprise_number,hist_duns_number,hist_domestic_corp_key,foreign_corp_key,unk_corp_key,ebr_file_number,cnp_name,cnp_number,cnp_btype,company_fein,company_phone,iscorp,prim_range,prim_name,sec_range,v_city_name,st,zip,LOCAL); // ,LOCAL ok - we don't need a perfect dedup - this is an optimization
  Match := JOIN(h02,MatchCands,LEFT.active_duns_number = RIGHT.active_duns_number AND LEFT.active_enterprise_number = RIGHT.active_enterprise_number AND LEFT.active_domestic_corp_key = RIGHT.active_domestic_corp_key AND LEFT.hist_enterprise_number = RIGHT.hist_enterprise_number AND LEFT.hist_duns_number = RIGHT.hist_duns_number
       ...
       AND LEFT.zip = RIGHT.zip AND LEFT.Proxid < RIGHT.Proxid,TRANSFORM(Rec,SELF.Proxid2 := LEFT.Proxid,SELF.Proxid1 := RIGHT.Proxid));

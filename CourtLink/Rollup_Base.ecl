...
export Rollup_Base(dataset(Layouts.Base) pDataset) := function
	pDataset_Dist := distribute(pDataset, hash(courtID, DocketNumber));
	...
	dnotfull_sort        := sort  (pDataset_rollup, courtID, DocketNumber, local);
	dnotfull_group       := group (dnotfull_sort, courtID, DocketNumber, local);

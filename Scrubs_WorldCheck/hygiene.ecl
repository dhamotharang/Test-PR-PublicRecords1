...
EXPORT Summary(SALT311.Str30Type  txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    populated_companies_cnt := COUNT(GROUP,h.companies <> (TYPEOF(h.companies))'');
    populated_companies_pcnt := AVE(GROUP,IF(h.companies = (TYPEOF(h.companies))'',0,100));
    maxlength_companies := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.companies)));
    avelength_companies := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.companies)),h.companies<>(typeof(h.companies))'');
    ...
    populated_linked_tos_cnt := COUNT(GROUP,h.linked_tos <> (TYPEOF(h.linked_tos))'');
    populated_linked_tos_pcnt := AVE(GROUP,IF(h.linked_tos = (TYPEOF(h.linked_tos))'',0,100));
    maxlength_linked_tos := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.linked_tos)));
    avelength_linked_tos := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.linked_tos)),h.linked_tos<>(typeof(h.linked_tos))'');
...
SHARED FldIds := DATASET([{1,'uid'}
      ...
      ,{24,'updated'}
      ,{25,'editor'}
      ,{26,'age_as_of_date'}],SALT311.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_companies((SALT311.StrType)le.companies),
    Fields.InValid_e_i_ind((SALT311.StrType)le.e_i_ind),
    Fields.InValid_linked_tos((SALT311.StrType)le.linked_tos),

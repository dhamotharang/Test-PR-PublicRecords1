...
#workunit('name','Scrubs_CityStateZip.BWR_Hygiene - Hygiene & Stats - SALT V3.11.0');
IMPORT Scrubs_CityStateZip,SALT311;
// First create an instantiated hygiene module
  infile := Scrubs_CityStateZip.In_CityStateZip;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_CityStateZip.hygiene(ip);
  ...
  OUTPUT(SALT311.MAC_Character_Counts.EclRecord(p,'Layout_CityStateZip'),NAMED('OptimizedLayout'));// File layout suggested by data

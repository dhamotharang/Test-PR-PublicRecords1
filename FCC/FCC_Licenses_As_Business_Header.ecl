...
FCC.Layout_FCC_Licenses_in;
END;

//--------------------------------------------
// Add unique record id to FCC_Licenses file
//--------------------------------------------
Layout_FCC_Licenses_Local AddRecordID(Layout_FCC_Licenses_in L) := TRANSFORM
SELF := L;
END;
...
bh_layout Translate_FCC_Licenses_To_BHF(Layout_FCC_Licenses_Local L, integer CTR) := transform
  ...
  SELF.source := 'FC';
  SELF.source_group := '';
  SELF.company_name := choose(CTR, L.licensees_name, L.DBA_name);
  ...
  SELF.dt_vendor_first_reported := (UNSIGNED4)(if(validatedate(L.date_of_last_change) <> '', validatedate(L.date_of_last_change),
							''));
...
FCC_Licenses_clean_rollup := ROLLUP(FCC_Licenses_clean_sort,
                    ...
                    LOCAL) : PERSIST('TEMP::FCC_Licenses_Company_Rollup');

EXPORT FCC_licenses_As_Business_Header := FCC_Licenses_clean_rollup;

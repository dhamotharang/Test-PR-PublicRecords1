...
layout_payload forAutokey(fcc.Layout_FCC_base L, integer cnt) := transform
...
search_file := normalize(fcc.File_FCC_base,3,forAutokey(left,counter));
...
AutoKeyB2.MAC_AcceptSK_to_QA(Constant(filedate).ak_keyname, mymove,false,fcc.Constant('').ak_skip_set)

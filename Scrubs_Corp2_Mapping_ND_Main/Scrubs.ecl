...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Corp2_Mapping.LayoutsCommon.Main)
    ...
    UNSIGNED1 corp_last_renewal_date_Invalid;
...
EXPORT FromNone(DATASET(Corp2_Mapping.LayoutsCommon.Main) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_trademark_renewal_date_Invalid := Fields.InValid_corp_trademark_renewal_date((SALT32.StrType)le.corp_trademark_renewal_date);
    SELF.corp_last_renewal_date_Invalid := Fields.InValid_corp_last_renewal_date((SALT32.StrType)le.corp_last_renewal_date);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_trademark_filing_date_Invalid << 12 ) + ( le.corp_key_Invalid << 14 ) + ( le.corp_vendor_Invalid << 16 ) + ( le.corp_state_origin_Invalid << 17 ) + ( le.corp_process_date_Invalid << 18 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 20 ) + ( le.corp_legal_name_Invalid << 22 ) + ( le.corp_filing_date_Invalid << 23 ) + ( le.corp_status_cd_Invalid << 25 ) + ( le.corp_status_date_Invalid << 26 ) + ( le.corp_inc_state_Invalid << 28 ) + ( le.corp_inc_date_Invalid << 29 ) + ( le.corp_foreign_domestic_ind_Invalid << 31 ) + ( le.corp_forgn_date_Invalid << 32 ) + ( le.corp_orig_org_structure_cd_Invalid << 34 ) + ( le.corp_for_profit_ind_Invalid << 35 ) + ( le.corp_trademark_renewal_date_Invalid << 36 ) + ( le.corp_last_renewal_date_Invalid << 38 ) + ( le.corp_ln_name_type_cd_Invalid << 40 ) + ( le.corp_ln_name_type_desc_Invalid << 41 ) + ( le.corp_forgn_state_desc_Invalid << 42 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_trademark_renewal_date_Invalid := (le.ScrubsBits1 >> 36) & 3;
    SELF.corp_last_renewal_date_Invalid := (le.ScrubsBits1 >> 38) & 3;
    SELF.corp_ln_name_type_cd_Invalid := (le.ScrubsBits1 >> 40) & 1;
    SELF.corp_ln_name_type_desc_Invalid := (le.ScrubsBits1 >> 41) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_last_renewal_date_ALLOW_ErrorCount := COUNT(GROUP,h.corp_last_renewal_date_Invalid=1);
    corp_last_renewal_date_CUSTOM_ErrorCount := COUNT(GROUP,h.corp_last_renewal_date_Invalid=2);
    corp_last_renewal_date_LENGTH_ErrorCount := COUNT(GROUP,h.corp_last_renewal_date_Invalid=3);
    corp_last_renewal_date_Total_ErrorCount := COUNT(GROUP,h.corp_last_renewal_date_Invalid>0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_last_renewal_date_Invalid,'ALLOW','CUSTOM','LENGTH','UNKNOWN')
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT32.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_last_renewal_date:invalid_optional_date:ALLOW','corp_last_renewal_date:invalid_optional_date:CUSTOM','corp_last_renewal_date:invalid_optional_date:LENGTH'
          ,'corp_ln_name_type_cd:invalid_name_type_cd:ENUM'
          ,'corp_ln_name_type_desc:invalid_name_type_desc:ENUM'
          ,'corp_forgn_state_desc:invalid_alphablankhyphencommaperiod:ALLOW','UNKNOWN');
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_corp_last_renewal_date(1),Fields.InvalidMessage_corp_last_renewal_date(2),Fields.InvalidMessage_corp_last_renewal_date(3)
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_last_renewal_date_ALLOW_ErrorCount,le.corp_last_renewal_date_CUSTOM_ErrorCount,le.corp_last_renewal_date_LENGTH_ErrorCount

IMPORT SALT311,STD;
IMPORT Scrubs,Scrubs_Corp2_Mapping_ND_Main; // Import modules for FieldTypes attribute definitions
...
// The module to handle the case where no scrubs exist
  ...
  EXPORT Expanded_Layout := RECORD(Layout_In_ND)
  ...
  END;
  EXPORT  Bitmap_Layout := RECORD(Layout_In_ND)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(Layout_In_ND) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_delayed_effective_date_Invalid := Fields.InValid_corp_delayed_effective_date((SALT311.StrType)le.corp_delayed_effective_date);
  ...
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Layout_In_ND);
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_trademark_filing_date_Invalid << 12 ) + ( le.corp_key_Invalid << 14 ) + ( le.corp_vendor_Invalid << 16 ) + ( le.corp_state_origin_Invalid << 17 ) + ( le.corp_process_date_Invalid << 18 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 20 ) + ( le.corp_legal_name_Invalid << 22 ) + ( le.corp_filing_date_Invalid << 23 ) + ( le.corp_status_date_Invalid << 25 ) + ( le.corp_status_desc_Invalid << 27 ) + ( le.corp_inc_state_Invalid << 28 ) + ( le.corp_inc_date_Invalid << 29 ) + ( le.corp_foreign_domestic_ind_Invalid << 31 ) + ( le.corp_forgn_date_Invalid << 32 ) + ( le.corp_for_profit_ind_Invalid << 34 ) + ( le.corp_ln_name_type_cd_Invalid << 35 ) + ( le.corp_ln_name_type_desc_Invalid << 36 ) + ( le.corp_delayed_effective_date_Invalid << 37 ) + ( le.corp_term_exist_exp_Invalid << 39 ) + ( le.corp_trademark_expiration_date_Invalid << 41 ) + ( le.recordorigin_Invalid << 43 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,Layout_In_ND);
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_trademark_filing_date_Invalid := (le.ScrubsBits1 >> 12) & 3;
    SELF.corp_key_Invalid := (le.ScrubsBits1 >> 14) & 3;
    SELF.corp_vendor_Invalid := (le.ScrubsBits1 >> 16) & 1;
    SELF.corp_state_origin_Invalid := (le.ScrubsBits1 >> 17) & 1;
    SELF.corp_process_date_Invalid := (le.ScrubsBits1 >> 18) & 3;
    SELF.corp_orig_sos_charter_nbr_Invalid := (le.ScrubsBits1 >> 20) & 3;
    SELF.corp_legal_name_Invalid := (le.ScrubsBits1 >> 22) & 1;
    SELF.corp_filing_date_Invalid := (le.ScrubsBits1 >> 23) & 3;
    SELF.corp_status_date_Invalid := (le.ScrubsBits1 >> 25) & 3;
    SELF.corp_status_desc_Invalid := (le.ScrubsBits1 >> 27) & 1;
    ...
    SELF.corp_forgn_date_Invalid := (le.ScrubsBits1 >> 32) & 3;
    SELF.corp_for_profit_ind_Invalid := (le.ScrubsBits1 >> 34) & 1;
    SELF.corp_ln_name_type_cd_Invalid := (le.ScrubsBits1 >> 35) & 1;
    SELF.corp_ln_name_type_desc_Invalid := (le.ScrubsBits1 >> 36) & 1;
    SELF.corp_delayed_effective_date_Invalid := (le.ScrubsBits1 >> 37) & 3;
    SELF.corp_term_exist_exp_Invalid := (le.ScrubsBits1 >> 39) & 3;
    SELF.corp_trademark_expiration_date_Invalid := (le.ScrubsBits1 >> 41) & 3;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_trademark_filing_date_LENGTHS_ErrorCount := COUNT(GROUP,h.corp_trademark_filing_date_Invalid=3);
    ...
    corp_delayed_effective_date_LENGTHS_ErrorCount := COUNT(GROUP,h.corp_delayed_effective_date_Invalid=3);
    ...
    corp_term_exist_exp_LENGTHS_ErrorCount := COUNT(GROUP,h.corp_term_exist_exp_Invalid=3);
    ...
    corp_trademark_expiration_date_LENGTHS_ErrorCount := COUNT(GROUP,h.corp_trademark_expiration_date_Invalid=3);
    ...
    AnyRule_WithErrorsCount := COUNT(GROUP, h.dt_vendor_first_reported_Invalid > 0 OR h.dt_vendor_last_reported_Invalid > 0 OR h.dt_first_seen_Invalid > 0 OR h.dt_last_seen_Invalid > 0 OR h.corp_ra_dt_first_seen_Invalid > 0 OR h.corp_ra_dt_last_seen_Invalid > 0 OR h.corp_trademark_filing_date_Invalid > 0 OR h.corp_key_Invalid > 0 OR h.corp_vendor_Invalid > 0 OR h.corp_state_origin_Invalid > 0 OR h.corp_process_date_Invalid > 0 OR h.corp_orig_sos_charter_nbr_Invalid > 0 OR h.corp_legal_name_Invalid > 0 OR h.corp_filing_date_Invalid > 0 OR h.corp_status_date_Invalid > 0 OR h.corp_status_desc_Invalid > 0 OR h.corp_inc_state_Invalid > 0 OR h.corp_inc_date_Invalid > 0 OR h.corp_foreign_domestic_ind_Invalid > 0 OR h.corp_forgn_date_Invalid > 0 OR h.corp_for_profit_ind_Invalid > 0 OR h.corp_ln_name_type_cd_Invalid > 0 OR h.corp_ln_name_type_desc_Invalid > 0 OR h.corp_delayed_effective_date_Invalid > 0 OR h.corp_term_exist_exp_Invalid > 0 OR h.corp_trademark_expiration_date_Invalid > 0 OR h.recordorigin_Invalid > 0);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.dt_vendor_first_reported_Total_ErrorCount > 0, 1, 0) + IF(le.dt_vendor_last_reported_Total_ErrorCount > 0, 1, 0) + IF(le.dt_first_seen_Total_ErrorCount > 0, 1, 0) + IF(le.dt_last_seen_Total_ErrorCount > 0, 1, 0) + IF(le.corp_ra_dt_first_seen_Total_ErrorCount > 0, 1, 0) + IF(le.corp_ra_dt_last_seen_Total_ErrorCount > 0, 1, 0) + IF(le.corp_trademark_filing_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_key_Total_ErrorCount > 0, 1, 0) + IF(le.corp_vendor_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_state_origin_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_orig_sos_charter_nbr_Total_ErrorCount > 0, 1, 0) + IF(le.corp_legal_name_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_filing_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_status_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_status_desc_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_inc_state_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_inc_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_foreign_domestic_ind_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_forgn_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_for_profit_ind_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_ln_name_type_cd_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_ln_name_type_desc_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_delayed_effective_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_term_exist_exp_Total_ErrorCount > 0, 1, 0) + IF(le.corp_trademark_expiration_date_Total_ErrorCount > 0, 1, 0) + IF(le.recordorigin_ENUM_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.dt_vendor_first_reported_ALLOW_ErrorCount > 0, 1, 0) + IF(le.dt_vendor_first_reported_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.dt_vendor_first_reported_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.dt_vendor_last_reported_ALLOW_ErrorCount > 0, 1, 0) + IF(le.dt_vendor_last_reported_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.dt_vendor_last_reported_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.dt_first_seen_ALLOW_ErrorCount > 0, 1, 0) + IF(le.dt_first_seen_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.dt_first_seen_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.dt_last_seen_ALLOW_ErrorCount > 0, 1, 0) + IF(le.dt_last_seen_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.dt_last_seen_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_ra_dt_first_seen_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_ra_dt_first_seen_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_ra_dt_first_seen_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_ra_dt_last_seen_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_ra_dt_last_seen_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_ra_dt_last_seen_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_trademark_filing_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_trademark_filing_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_trademark_filing_date_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_key_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_key_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_vendor_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_state_origin_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_orig_sos_charter_nbr_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_orig_sos_charter_nbr_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_legal_name_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_filing_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_filing_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_filing_date_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_status_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_status_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_status_date_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_status_desc_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_inc_state_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_inc_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_inc_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_inc_date_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_foreign_domestic_ind_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_forgn_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_forgn_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_forgn_date_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_for_profit_ind_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_ln_name_type_cd_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_ln_name_type_desc_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_delayed_effective_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_delayed_effective_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_delayed_effective_date_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_term_exist_exp_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_term_exist_exp_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_term_exist_exp_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_trademark_expiration_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_trademark_expiration_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_trademark_expiration_date_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.recordorigin_ENUM_ErrorCount > 0, 1, 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_trademark_filing_date_Invalid,'ALLOW','CUSTOM','LENGTHS','UNKNOWN')
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_trademark_filing_date:invalid_optional_date:ALLOW','corp_trademark_filing_date:invalid_optional_date:CUSTOM','corp_trademark_filing_date:invalid_optional_date:LENGTHS'
          ...
          ,'corp_status_date:invalid_optional_date:ALLOW','corp_status_date:invalid_optional_date:CUSTOM','corp_status_date:invalid_optional_date:LENGTHS'
          ...
          ,'corp_delayed_effective_date:invalid_optional_date:ALLOW','corp_delayed_effective_date:invalid_optional_date:CUSTOM','corp_delayed_effective_date:invalid_optional_date:LENGTHS'
          ,'corp_term_exist_exp:invalid_optional_date:ALLOW','corp_term_exist_exp:invalid_optional_date:CUSTOM','corp_term_exist_exp:invalid_optional_date:LENGTHS'
          ,'corp_trademark_expiration_date:invalid_optional_date:ALLOW','corp_trademark_expiration_date:invalid_optional_date:CUSTOM','corp_trademark_expiration_date:invalid_optional_date:LENGTHS'
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_trademark_filing_date_ALLOW_ErrorCount,le.corp_trademark_filing_date_CUSTOM_ErrorCount,le.corp_trademark_filing_date_LENGTHS_ErrorCount
          ...
          ,le.corp_delayed_effective_date_ALLOW_ErrorCount,le.corp_delayed_effective_date_CUSTOM_ErrorCount,le.corp_delayed_effective_date_LENGTHS_ErrorCount
          ,le.corp_term_exist_exp_ALLOW_ErrorCount,le.corp_term_exist_exp_CUSTOM_ErrorCount,le.corp_term_exist_exp_LENGTHS_ErrorCount
          ,le.corp_trademark_expiration_date_ALLOW_ErrorCount,le.corp_trademark_expiration_date_CUSTOM_ErrorCount,le.corp_trademark_expiration_date_LENGTHS_ErrorCount
    ...
    mod_hygiene := hygiene(PROJECT(h, Layout_In_ND));
  ...
  // record count stats
    ...
    mod_Delta := Delta(prevDS, PROJECT(h, Layout_In_ND));
...
EXPORT StandardStats(DATASET(Layout_In_ND) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  myTimeStamp := (UNSIGNED6)SALT311.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
  expandedFile := FromNone(inFile).ExpandedInfile;
  mod_fromexpandedOverall := FromExpanded(expandedFile);
  scrubsSummaryOverall := mod_fromexpandedOverall.SummaryStats;
 
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Corp2_Mapping_ND_Main, Fields, 'RECORDOF(scrubsSummaryOverall)', '');

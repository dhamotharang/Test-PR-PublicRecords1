...
EXPORT MAC_PopulationStatistics(infile,Ref='',Input_dt_vendor_first_reported = '',Input_dt_vendor_last_reported = '',Input_dt_first_seen = '',Input_dt_last_seen = '',Input_corp_ra_dt_first_seen = '',Input_corp_ra_dt_last_seen = '',Input_corp_trademark_filing_date = '',Input_corp_key = '',Input_corp_vendor = '',Input_corp_state_origin = '',Input_corp_process_date = '',Input_corp_orig_sos_charter_nbr = '',Input_corp_legal_name = '',Input_corp_filing_date = '',Input_corp_status_date = '',Input_corp_status_desc = '',Input_corp_inc_state = '',Input_corp_inc_date = '',Input_corp_foreign_domestic_ind = '',Input_corp_forgn_date = '',Input_corp_for_profit_ind = '',Input_corp_ln_name_type_cd = '',Input_corp_ln_name_type_desc = '',Input_corp_delayed_effective_date = '',Input_corp_term_exist_exp = '',Input_corp_trademark_expiration_date = '',Input_recordorigin = '',OutFile) := MACRO
...
+    #IF( #TEXT(Input_corp_trademark_filing_date)='' )
      '' 
    #ELSE
        IF( (unsigned)le.Input_corp_trademark_filing_date = 0,'', ':corp_trademark_filing_date(' + SALT311.fn_date_valid_as_text((unsigned)le.Input_corp_trademark_filing_date) + ')' )
    #END
...
+    #IF( #TEXT(Input_corp_status_date)='' )
      '' 
    #ELSE
        IF( (unsigned)le.Input_corp_status_date = 0,'', ':corp_status_date(' + SALT311.fn_date_valid_as_text((unsigned)le.Input_corp_status_date) + ')' )
    #END
...
+    #IF( #TEXT(Input_corp_delayed_effective_date)='' )
      '' 
    #ELSE
        IF( (unsigned)le.Input_corp_delayed_effective_date = 0,'', ':corp_delayed_effective_date(' + SALT311.fn_date_valid_as_text((unsigned)le.Input_corp_delayed_effective_date) + ')' )
    #END
 
+    #IF( #TEXT(Input_corp_term_exist_exp)='' )
      '' 
    #ELSE
        IF( (unsigned)le.Input_corp_term_exist_exp = 0,'', ':corp_term_exist_exp(' + SALT311.fn_date_valid_as_text((unsigned)le.Input_corp_term_exist_exp) + ')' )
    #END
 
+    #IF( #TEXT(Input_corp_trademark_expiration_date)='' )
      '' 
    #ELSE
        IF( (unsigned)le.Input_corp_trademark_expiration_date = 0,'', ':corp_trademark_expiration_date(' + SALT311.fn_date_valid_as_text((unsigned)le.Input_corp_trademark_expiration_date) + ')' )
    #END

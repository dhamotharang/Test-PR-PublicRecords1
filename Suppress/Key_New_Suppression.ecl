...
New_Suppression      := DATASET('~thor_data400::base::new_suppression_file', Suppress.Layout_New_Suppression,THOR);
New_Suppression_FCRA := DATASET('~thor_data400::base::new_suppression_file_fcra', Suppress.Layout_New_Suppression,THOR);

get_recs := if(isFCRA, New_Suppression_FCRA,
                New_Suppression(Linking_ID not in Suppress.UNsuppress_DID())
															);

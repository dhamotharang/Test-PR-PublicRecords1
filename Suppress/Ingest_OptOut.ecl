...
EXPORT Ingest_OptOut(BOOLEAN incremental=FALSE
...
, DATASET(RECORDOF(Suppress.In_OptOut_Base))  infile = Suppress.In_OptOut_Base
  ...
  DistIngest0 := DISTRIBUTE(FilesToIngest0, HASH32(src,entry_type,lexid,domain_id,state_act));
  SortIngest0 := SORT(DistIngest0,src,entry_type,lexid,domain_id,state_act, __Tpe, rcid, LOCAL);
  GroupIngest0 := GROUP(SortIngest0,src,entry_type,lexid,domain_id,state_act, LOCAL, ORDERED, STABLE);
  SHARED AllIngestRecs0 := UNGROUP(ROLLUP(GroupIngest0,TRUE,MergeData(LEFT,RIGHT)));

  // Existing Base: combine delta with base file
  DistBase0 := DISTRIBUTE(Base0+Delta0, HASH32(src,entry_type,lexid,domain_id,state_act));
  SortBase0 := SORT(DistBase0,src,entry_type,lexid,domain_id,state_act, __Tpe, rcid, LOCAL);
  GroupBase0 := GROUP(SortBase0,src,entry_type,lexid,domain_id,state_act, LOCAL, ORDERED, STABLE);
  SHARED AllBaseRecs0 := UNGROUP(ROLLUP(GroupBase0,TRUE,MergeData(LEFT,RIGHT)));

  // Everything: combine ingest and base recs
  Sort0 := SORT(AllBaseRecs0+AllIngestRecs0,src,entry_type,lexid,domain_id,state_act, __Tpe,rcid,LOCAL);
  Group0 := GROUP(Sort0,src,entry_type,lexid,domain_id,state_act,LOCAL, ORDERED, STABLE);
  ...
  SHARED AllRecs := ORe+NR1+NR(rcid<>0) : PERSIST('~temp::Suppress::Ingest_OptOut_Cache',EXPIRE(Suppress.Config.PersistExpire));
  ...
  SHARED S0 := OUTPUT(UpdateStats, {{UpdateStats} AND NOT __Tpe}, ALL, NAMED('UpdateStats_OptOut'));
...
EXPORT ValidityStats := OUTPUT(d,NAMED('ValidityStatistics_OptOut'));
  EXPORT DoStats := S0;

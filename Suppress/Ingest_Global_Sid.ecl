...
EXPORT Ingest_Global_Sid(BOOLEAN incremental=FALSE
, DATASET(Layout_Global_Sid_Base) Delta = DATASET([],Layout_Global_Sid_Base)
, DATASET(Layout_Global_Sid_Base) dsBase = In_Global_Sid_Base // Change IN_Global_Sid_Base to change input to ingest process
, DATASET(RECORDOF(Suppress.In_Global_Sid_Base))  infile = Suppress.In_Global_Sid_Base
) := MODULE
  SHARED NullFile := DATASET([],Layout_Global_Sid_Base); // Use to replace files you wish to remove
  ...
  DistIngest0 := DISTRIBUTE(FilesToIngest0, HASH32(domain_id,prof_data_only,global_sids));
  SortIngest0 := SORT(DistIngest0,domain_id,prof_data_only,global_sids, __Tpe, rcid, LOCAL);
  GroupIngest0 := GROUP(SortIngest0,domain_id,prof_data_only,global_sids, LOCAL, ORDERED, STABLE);
  SHARED AllIngestRecs0 := UNGROUP(ROLLUP(GroupIngest0,TRUE,MergeData(LEFT,RIGHT)));

  // Existing Base: combine delta with base file
  DistBase0 := DISTRIBUTE(Base0+Delta0, HASH32(domain_id,prof_data_only,global_sids));
  SortBase0 := SORT(DistBase0,domain_id,prof_data_only,global_sids, __Tpe, rcid, LOCAL);
  GroupBase0 := GROUP(SortBase0,domain_id,prof_data_only,global_sids, LOCAL, ORDERED, STABLE);
  SHARED AllBaseRecs0 := UNGROUP(ROLLUP(GroupBase0,TRUE,MergeData(LEFT,RIGHT)));

  // Everything: combine ingest and base recs
  Sort0 := SORT(AllBaseRecs0+AllIngestRecs0,domain_id,prof_data_only,global_sids, __Tpe,rcid,LOCAL);
  Group0 := GROUP(Sort0,domain_id,prof_data_only,global_sids,LOCAL, ORDERED, STABLE);
  ...
  SHARED AllRecs := ORe+NR1+NR(rcid<>0) : PERSIST('~temp::Suppress::Ingest_Global_Sid_Cache',EXPIRE(Suppress.Config.PersistExpire));
  ...
  SHARED S0 := OUTPUT(UpdateStats, {{UpdateStats} AND NOT __Tpe}, ALL, NAMED('UpdateStats_Global_Sid'));
  ...
  EXPORT NewRecords_NoTag := PROJECT(NewRecords,Layout_Global_Sid_Base);
  EXPORT OldRecords :=PROJECT( AllRecs(__Tpe=RecordType.Old), NoFlagsRec);
  EXPORT OldRecords_NoTag := PROJECT(OldRecords,Layout_Global_Sid_Base);
  EXPORT UpdatedRecords := PROJECT(AllRecs(__Tpe=RecordType.Updated), NoFlagsRec);
  EXPORT UpdatedRecords_NoTag := PROJECT(UpdatedRecords,Layout_Global_Sid_Base);
  EXPORT AllRecords := IF(incremental, NewRecords, PROJECT(AllRecs, NoFlagsRec));
  EXPORT AllRecords_NoTag := PROJECT(AllRecords,Layout_Global_Sid_Base); // Records in 'pure' format
...
EXPORT ValidityStats := OUTPUT(d,NAMED('ValidityStatistics_Global_Sid'));
  EXPORT DoStats := S0;

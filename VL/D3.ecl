...
EXPORT D3:=MODULE
  ...
  SHARED VerticalTreeTemplate(STRING sData):=''+
    '<html><head><meta http-equiv="Content-Type" content="text/html;charset=utf-8">'+
    '<script type="text/javascript" src="http://mbostock.github.com/d3/d3.js?1.27.2"></script>'+
    '<script type="text/javascript" src="http://mbostock.github.com/d3/d3.layout.js?1.27.2"></script>'+
    '<style type="text/css">'+
    '.node rect {cursor: pointer;fill: #fff;fill-opacity: .5;stroke: #3182bd;stroke-width: 1.5px;}'+
    ...
    '<script type="text/javascript">'+
		'var data ='+sData+';'+
    'var w = 960,h = 800,i = 0,barHeight = 20,barWidth = w * .8,duration = 400;'+
    ...
    'var node = vis.selectAll("g.node").data(nodes, function(d) { return d.id || (d.id = ++i); });'+
    'var nodeEnter = node.enter().append("svg:g").attr("class", "node").attr("transform", function(d) { return "translate(" + source.y0 + "," + source.x0 + ")"; }).style("opacity", 1e-6);'+
    'nodeEnter.append("svg:rect").attr("y", -barHeight / 2).attr("height", barHeight).attr("width", barWidth).style("fill", color).on("click", click);'+
    'nodeEnter.append("svg:text").attr("dy", 3.5).attr("dx", 5.5).text(function(d) { return d.name; });'+
  ...
  EXPORT STRING ConvertToTree(DATASET(VL.Types.GraphLabels) dLabels,DATASET(VL.Types.GraphRelationships) dRelationships):=FUNCTION
  ...
  EXPORT Graph(STRING sChartType,STRING sChartName,DATASET(VL.Types.GraphLabels) dLabels,DATASET(VL.Types.GraphRelationships) dRelationships,VL.Styles.Default s=VL.Styles.Default):=FUNCTION
    RETURN MAP(
      sChartType='VerticalTree'=>OUTPUT(DATASET([{'CHARTCODE',VerticalTreeTemplate(ConvertToTree(dLabels,dRelationships))}],VL.Types.ChartInterface),NAMED('CHART_VERTICALTREE_'+sChartName)),
      sChartType='Tree'=>OUTPUT(DATASET([{'CHARTCODE',TreeTemplate(ConvertToTree(dLabels,dRelationships))}],VL.Types.ChartInterface),NAMED('CHART_TREE_'+sChartName)),
      sChartType='Chord'=>OUTPUT(DATASET([{'CHARTCODE',ChordTemplate(ConvertToChord(dLabels,dRelationships))}],VL.Types.ChartInterface),NAMED('CHART_CHORD_'+sChartName)),
      OUTPUT('Blah')
    );
  END;
END;
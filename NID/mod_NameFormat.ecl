...
export rgxLastStrong := '(DI |DA |DEL |DE LA |DELA |DE LOS |DES |DE |DELA |DELLA |DOS |DU |LA |MC |MAC |VON |VAN DEN |VAN DER |VAN DE |VAN DEN |VAN |VANDER |VANDEN |ST |SAN |EL |LE |[ODANL]\'|(A|ABU-EL|ABU|AL|ABDUL|BAR|BEN|BOU|EL|VAN-DEN)-)([A-Z][A-Z-]+)';
...
export rgxFiMiL := '^([A-Z]) +[A-Z] +' + '(' + rgxLast + ')$';
...
export rgxSpanishLName1 := '([A-Z]+ +(DE LA|DE LOS|DE) +[A-Z]+)';
...
export InvalidNameFormat(string s) := MAP(
...
export set of NameFormat StandAloneNameFormats := [
...
shared string1 GetNameTypeI(string nm) :=
	types[IF ($.NameTester.IsFirstNameOrInitial(nm) OR LENGTH(TRIM(nm,LEFT,RIGHT))=1,1,0) +
	IF ($.NameTester.IsValidLastName(nm),2,0) + 1];	
...
shared string1 GetNameOrderFML(string rgx, string s, integer2 posf1, integer2 posf2, 
		...
		return	MAP(
				...
				$.NameTester.InvalidLastName(REGEXFIND(rgx, s, posl1)) => 'L',
				$.NameTester.InvalidLastName(REGEXFIND(rgx, s, posl2)) => 'F',
...
shared string1 GetNameOrderFL(string rgx, string s, integer2 posf1, integer2 posf2, 
		...
		FUNCTION
			...
			return MAP(
				$.NameTester.InvalidLastName1(REGEXFIND(rgx, s, posl1)) => 'L',
				$.NameTester.InvalidLastName1(REGEXFIND(rgx, s, posl2)) => 'F',
...
shared string1 GetNameOrderLFMi(string rgx, string s, integer2 posf1, integer2 posl1) := FUNCTION
			//segs := ExtractNameTypes(rgx, s, posf1, posl1);
			return MAP(
				$.NameTester.InvalidLastName(REGEXFIND(rgx, s, posl1)) => 'F',
				$.NameTester.IsLastNameExOrHyphenated(REGEXFIND(rgx, s, posl1)) => 'L',
				$.NameTester.IsFirstName(REGEXFIND(rgx, s, posl1)) AND $.NameTester.IsLastNameExOrHyphenated(REGEXFIND(rgx, s, posf1))
						=> 'F',
...
shared boolean StandaloneFirstLast(string rgx, string s, integer2 posf, integer2 posl) :=
			$.NameTester.IsFirstName(REGEXFIND(rgx, s, posf)) AND $.NameTester.IsLastNameEx(REGEXFIND(rgx, s, posl));
...
shared string20 FilterMName(string s) := 
			TRIM(firstPartOnly(
					IF(s in ['NMI','NMN','NULL','\''] OR $.NameTester.IsSureGen(s),'',
...
shared string GenToAlpha(string suffix) :=
		IF($.NameTester.IsGeneration(suffix), suffix,
		CASE(suffix,
			'' => '',
			'1' => 'SR',
			'2' => 'II',
...
shared string65 FormatNameFMLorFLS(string rgx, string name, integer fn, integer mn, integer lnm, integer sfx) := 
	IF($.NameTester.IsSureSuffix(REGEXFIND(rgx, name, sfx)), FormatNameFLS(rgx, name, fn, lnm, sfx), FormatNameFML(rgx, name, fn, mn, lnm)); 
...
export string70 FormatName(string s, NameFormat fmt = 0, string1 clue='U') := FUNCTION
	return 
		...
		CASE (fmt,
			...
			NameFormat.LF =>		MAP(
									TRIM(REGEXFIND(rgxLF, s, 2)) in ['MAC','VAN'] and 
											$.NameTester.IsSureSuffix(REGEXFIND(rgxLF, s, 5)) =>
													FormatNameFLS(rgxLF, s, 2, 4, 5),
									StandaloneFirstLast(rgxLF, s, 1, 5) => FormatNameFL(rgxLF, s, 1, 5),
										FormatNameFL(rgxLF, s, 5, 1)),
			NameFormat.LFM =>	IF($.NameTester.IsSureGen(REGEXFIND(rgxLFM, s, 5)),
			...
			NameFormat.FLL =>	IF($.NameTester.IsSuffix(REGEXFIND(rgxFLL, s, 6)),
			...
			NameFormat.FMLL => IF($.NameTester.IsSureSuffix(REGEXFIND(rgxFMLL, s, 1)),
			...
			NameFormat.FMiMiL =>	IF($.NameTester.IsSureSuffix(REGEXFIND(rgxFMiMiL, s, 1)),
			...
			NameFormat.FMiLI =>	IF($.NameTester.IsSureGen(REGEXFIND(rgxFMiLI, s, 3)),
			...
			NameFormat.LcFcM => 	IF($.NameTester.IsSureGen(REGEXFIND(rgxLcFcM, s, 1)),
			...
			NameFormat.LLcFM =>		MAP(
															$.NameTester.IsSuffix(TRIM(REGEXFIND(rgxLLcFM, s, 7))) => FormatNameFMLS(rgxLLcFM, s, 1, 4, 9, 7),
	...
	return blank5 +
		CASE (fmt,
			...
			NameFormat.LFMaLFM  =>	MAP(
																$.NameTester.IsSureGen(REGEXFIND(rgxLFMaLFM, s, 4)) => FormatNameFLS(rgxLFMaLFM, s, 5, 1, 4),
			...
			NameFormat.LFMaLFM2 => 	IF($.NameTester.IsSureGen(REGEXFIND(rgxLFMaLFM2, s, 5)),
			...
			NameFormat.LcFMaF =>	if($.NameTester.IsSureSuffix(REGEXFIND(rgxLcFMaF, s, 4)),
															FormatNameFLS(rgxLcFMaF, s, 5, 1, 4),
															FormatNameFML(rgxLcFMaF, s, 4, 5, 1)),
			NameFormat.LcFMaFMM =>	if($.NameTester.IsSureSuffix(REGEXFIND(rgxLcFMaFMM, s, 5)),
			...
			NameFormat.LcFMaLcF =>	if($.NameTester.IsSureSuffix(REGEXFIND(rgxLcFMaLcF, s, 4)),
...
//														FormatNameFMML(rgxLFaFM, s, 6, 7, 8, 1)), 
			...
			NameFormat.FLSaFM => MAP(
											$.NameTester.IsSureGen(REGEXFIND(rgxFLSaFM, s, 9)) => 
											...
											$.NameTester.IsFirstNameOrInitial(REGEXFIND(rgxFLSaFM, s, 7)) AND 
																$.NameTester.IsFirstNameOrInitialOrBlank(REGEXFIND(rgxFLSaFM, s, 9)) =>
												CASE(GetNameOrderFL(rgxFLSaFM, s, 1, 2, 2, 1),
													'F' => FormatNameFML(rgxFLSaFM, s, 7, 9, 2),
													'L' => FormatNameFML(rgxFLSaFM, s, 7, 9, 1),
													FormatNameFML(rgxFLSaFM, s, 7, 9, 2)
												),
											$.NameTester.IsLastNameEx(REGEXFIND(rgxFLSaFM, s, 9)) => FormatNameFL(rgxFLSaFM, s, 7, 9),
			...
			NameFormat.LFMaFM =>	MAP(
															...
															NOT $.NameTester.IsFirstNameOrInitialOrBlank(REGEXFIND(rgxLFMaFM, s, 11)) => FormatNameFL(rgxLFMaFM, s, 8, 11),
			...
			NameFormat.LFMiaFM  =>	MAP(
																$.NameTester.IsSureGen(REGEXFIND(rgxLFMiaFM, s, 8)) => FormatNameFLS(rgxLFMiaFM, s, 7, 1, 8),
			...
			NameFormat.LcFMaFMM =>	MAP(
																$.NameTester.IsSureSuffix(REGEXFIND(rgxLcFMaFMM, s, 7)) => FormatNameFLS(rgxLcFMaFMM, s, 8, 1, 7),
			...
			NameFormat.LcFaLcF =>	IF($.NameTester.IsSureSuffix(REGEXFIND(rgxLcFaLcF, s, 9)),

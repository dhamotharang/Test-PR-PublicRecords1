...
#workunit('name','Scrubs_Business_Credit.BS_BWR_Hygiene - Hygiene & Stats - SALT V3.11.9');
IMPORT Scrubs_Business_Credit,SALT311;
// First create an instantiated hygiene module
  infile := Scrubs_Business_Credit.BS_In_Business_Credit;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_Business_Credit.BS_hygiene(ip);
  ...
  OUTPUT(SALT311.MAC_Character_Counts.EclRecord(p,'BS_Layout_Business_Credit'),NAMED('OptimizedLayout'));// File layout suggested by data

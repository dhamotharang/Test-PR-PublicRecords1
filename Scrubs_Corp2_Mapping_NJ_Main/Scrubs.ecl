...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(corp2_Mapping.LayoutsCommon.Main)
    ...
    UNSIGNED1 corp_ra_full_name_Invalid;
...
EXPORT FromNone(DATASET(corp2_Mapping.LayoutsCommon.Main) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_orig_org_structure_desc_Invalid := Fields.InValid_corp_orig_org_structure_desc((SALT32.StrType)le.corp_orig_org_structure_desc);
    SELF.corp_for_profit_ind_Invalid := Fields.InValid_corp_for_profit_ind((SALT32.StrType)le.corp_for_profit_ind);
    SELF.corp_ra_full_name_Invalid := Fields.InValid_corp_ra_full_name((SALT32.StrType)le.corp_ra_full_name);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_ln_name_type_desc_Invalid << 22 ) + ( le.corp_address1_line3_Invalid << 23 ) + ( le.corp_status_cd_Invalid << 24 ) + ( le.corp_status_desc_Invalid << 25 ) + ( le.corp_inc_state_Invalid << 26 ) + ( le.corp_inc_date_Invalid << 27 ) + ( le.corp_foreign_domestic_ind_Invalid << 29 ) + ( le.corp_forgn_state_cd_Invalid << 30 ) + ( le.corp_forgn_date_Invalid << 31 ) + ( le.corp_orig_org_structure_cd_Invalid << 33 ) + ( le.corp_orig_org_structure_desc_Invalid << 34 ) + ( le.corp_for_profit_ind_Invalid << 35 ) + ( le.corp_ra_full_name_Invalid << 36 ) + ( le.corp_ra_address_line3_Invalid << 37 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_forgn_state_cd_Invalid := (le.ScrubsBits1 >> 30) & 1;
    SELF.corp_forgn_date_Invalid := (le.ScrubsBits1 >> 31) & 3;
    SELF.corp_orig_org_structure_cd_Invalid := (le.ScrubsBits1 >> 33) & 1;
    SELF.corp_orig_org_structure_desc_Invalid := (le.ScrubsBits1 >> 34) & 1;
    SELF.corp_for_profit_ind_Invalid := (le.ScrubsBits1 >> 35) & 1;
    SELF.corp_ra_full_name_Invalid := (le.ScrubsBits1 >> 36) & 1;
    SELF.corp_ra_address_line3_Invalid := (le.ScrubsBits1 >> 37) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_address1_line3_ENUM_ErrorCount := COUNT(GROUP,h.corp_address1_line3_Invalid=1);
    corp_status_cd_ENUM_ErrorCount := COUNT(GROUP,h.corp_status_cd_Invalid=1);
    corp_status_desc_LENGTH_ErrorCount := COUNT(GROUP,h.corp_status_desc_Invalid=1);
    ...
    corp_orig_org_structure_desc_LENGTH_ErrorCount := COUNT(GROUP,h.corp_orig_org_structure_desc_Invalid=1);
    corp_for_profit_ind_ENUM_ErrorCount := COUNT(GROUP,h.corp_for_profit_ind_Invalid=1);
    corp_ra_full_name_LENGTH_ErrorCount := COUNT(GROUP,h.corp_ra_full_name_Invalid=1);
    corp_ra_address_line3_ENUM_ErrorCount := COUNT(GROUP,h.corp_ra_address_line3_Invalid=1);
  END;
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.dt_vendor_first_reported_Invalid,le.dt_vendor_last_reported_Invalid,le.dt_first_seen_Invalid,le.dt_last_seen_Invalid,le.corp_ra_dt_first_seen_Invalid,le.corp_ra_dt_last_seen_Invalid,le.corp_key_Invalid,le.corp_vendor_Invalid,le.corp_state_origin_Invalid,le.corp_process_date_Invalid,le.corp_orig_sos_charter_nbr_Invalid,le.corp_legal_name_Invalid,le.corp_ln_name_type_cd_Invalid,le.corp_ln_name_type_desc_Invalid,le.corp_address1_line3_Invalid,le.corp_status_cd_Invalid,le.corp_status_desc_Invalid,le.corp_inc_state_Invalid,le.corp_inc_date_Invalid,le.corp_foreign_domestic_ind_Invalid,le.corp_forgn_state_cd_Invalid,le.corp_forgn_date_Invalid,le.corp_orig_org_structure_cd_Invalid,le.corp_orig_org_structure_desc_Invalid,le.corp_for_profit_ind_Invalid,le.corp_ra_full_name_Invalid,le.corp_ra_address_line3_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_dt_vendor_first_reported(le.dt_vendor_first_reported_Invalid),Fields.InvalidMessage_dt_vendor_last_reported(le.dt_vendor_last_reported_Invalid),Fields.InvalidMessage_dt_first_seen(le.dt_first_seen_Invalid),Fields.InvalidMessage_dt_last_seen(le.dt_last_seen_Invalid),Fields.InvalidMessage_corp_ra_dt_first_seen(le.corp_ra_dt_first_seen_Invalid),Fields.InvalidMessage_corp_ra_dt_last_seen(le.corp_ra_dt_last_seen_Invalid),Fields.InvalidMessage_corp_key(le.corp_key_Invalid),Fields.InvalidMessage_corp_vendor(le.corp_vendor_Invalid),Fields.InvalidMessage_corp_state_origin(le.corp_state_origin_Invalid),Fields.InvalidMessage_corp_process_date(le.corp_process_date_Invalid),Fields.InvalidMessage_corp_orig_sos_charter_nbr(le.corp_orig_sos_charter_nbr_Invalid),Fields.InvalidMessage_corp_legal_name(le.corp_legal_name_Invalid),Fields.InvalidMessage_corp_ln_name_type_cd(le.corp_ln_name_type_cd_Invalid),Fields.InvalidMessage_corp_ln_name_type_desc(le.corp_ln_name_type_desc_Invalid),Fields.InvalidMessage_corp_address1_line3(le.corp_address1_line3_Invalid),Fields.InvalidMessage_corp_status_cd(le.corp_status_cd_Invalid),Fields.InvalidMessage_corp_status_desc(le.corp_status_desc_Invalid),Fields.InvalidMessage_corp_inc_state(le.corp_inc_state_Invalid),Fields.InvalidMessage_corp_inc_date(le.corp_inc_date_Invalid),Fields.InvalidMessage_corp_foreign_domestic_ind(le.corp_foreign_domestic_ind_Invalid),Fields.InvalidMessage_corp_forgn_state_cd(le.corp_forgn_state_cd_Invalid),Fields.InvalidMessage_corp_forgn_date(le.corp_forgn_date_Invalid),Fields.InvalidMessage_corp_orig_org_structure_cd(le.corp_orig_org_structure_cd_Invalid),Fields.InvalidMessage_corp_orig_org_structure_desc(le.corp_orig_org_structure_desc_Invalid),Fields.InvalidMessage_corp_for_profit_ind(le.corp_for_profit_ind_Invalid),Fields.InvalidMessage_corp_ra_full_name(le.corp_ra_full_name_Invalid),Fields.InvalidMessage_corp_ra_address_line3(le.corp_ra_address_line3_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_address1_line3_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.corp_status_cd_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.corp_status_desc_Invalid,'LENGTH','UNKNOWN')
          ...
          ,CHOOSE(le.corp_orig_org_structure_desc_Invalid,'LENGTH','UNKNOWN')
          ,CHOOSE(le.corp_for_profit_ind_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.corp_ra_full_name_Invalid,'LENGTH','UNKNOWN')
          ,CHOOSE(le.corp_ra_address_line3_Invalid,'ENUM','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'dt_vendor_first_reported','dt_vendor_last_reported','dt_first_seen','dt_last_seen','corp_ra_dt_first_seen','corp_ra_dt_last_seen','corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_orig_sos_charter_nbr','corp_legal_name','corp_ln_name_type_cd','corp_ln_name_type_desc','corp_address1_line3','corp_status_cd','corp_status_desc','corp_inc_state','corp_inc_date','corp_foreign_domestic_ind','corp_forgn_state_cd','corp_forgn_date','corp_orig_org_structure_cd','corp_orig_org_structure_desc','corp_for_profit_ind','corp_ra_full_name','corp_ra_address_line3','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter','invalid_mandatory','invalid_name_type_code','invalid_name_type_desc','invalid_country','invalid_corp_status_code','invalid_mandatory','invalid_state_origin','invalid_optional_date','invalid_forgn_dom_code','invalid_alphablank','invalid_optional_date','invalid_corp_orig_org_structure_cd','invalid_mandatory','invalid_flag_code','invalid_mandatory','invalid_country','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT32.StrType)le.dt_vendor_first_reported,(SALT32.StrType)le.dt_vendor_last_reported,(SALT32.StrType)le.dt_first_seen,(SALT32.StrType)le.dt_last_seen,(SALT32.StrType)le.corp_ra_dt_first_seen,(SALT32.StrType)le.corp_ra_dt_last_seen,(SALT32.StrType)le.corp_key,(SALT32.StrType)le.corp_vendor,(SALT32.StrType)le.corp_state_origin,(SALT32.StrType)le.corp_process_date,(SALT32.StrType)le.corp_orig_sos_charter_nbr,(SALT32.StrType)le.corp_legal_name,(SALT32.StrType)le.corp_ln_name_type_cd,(SALT32.StrType)le.corp_ln_name_type_desc,(SALT32.StrType)le.corp_address1_line3,(SALT32.StrType)le.corp_status_cd,(SALT32.StrType)le.corp_status_desc,(SALT32.StrType)le.corp_inc_state,(SALT32.StrType)le.corp_inc_date,(SALT32.StrType)le.corp_foreign_domestic_ind,(SALT32.StrType)le.corp_forgn_state_cd,(SALT32.StrType)le.corp_forgn_date,(SALT32.StrType)le.corp_orig_org_structure_cd,(SALT32.StrType)le.corp_orig_org_structure_desc,(SALT32.StrType)le.corp_for_profit_ind,(SALT32.StrType)le.corp_ra_full_name,(SALT32.StrType)le.corp_ra_address_line3,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT32.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_address1_line3:invalid_country:ENUM'
          ,'corp_status_cd:invalid_corp_status_code:ENUM'
          ,'corp_status_desc:invalid_mandatory:LENGTH'
          ...
          ,'corp_orig_org_structure_desc:invalid_mandatory:LENGTH'
          ,'corp_for_profit_ind:invalid_flag_code:ENUM'
          ,'corp_ra_full_name:invalid_mandatory:LENGTH'
          ,'corp_ra_address_line3:invalid_country:ENUM','UNKNOWN');
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_corp_ra_full_name(1)
          ,Fields.InvalidMessage_corp_ra_address_line3(1),'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_address1_line3_ENUM_ErrorCount
          ,le.corp_status_cd_ENUM_ErrorCount
          ,le.corp_status_desc_LENGTH_ErrorCount
          ...
          ,le.corp_orig_org_structure_desc_LENGTH_ErrorCount
          ,le.corp_for_profit_ind_ENUM_ErrorCount
          ,le.corp_ra_full_name_LENGTH_ErrorCount
          ,le.corp_ra_address_line3_ENUM_ErrorCount,0);
      SELF.rulepcnt := 100 * CHOOSE(c
          ...
          ,le.corp_for_profit_ind_ENUM_ErrorCount
          ,le.corp_ra_full_name_LENGTH_ErrorCount
          ,le.corp_ra_address_line3_ENUM_ErrorCount,0) / le.TotalCnt + 0.5;
    END;

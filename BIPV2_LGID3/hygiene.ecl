...
EXPORT Summary(SALT311.Str30Type  txt,BOOLEAN Glob=TRUE) := FUNCTION
  SummaryLayout := RECORD
    ...
    populated_cortera_id_cnt := COUNT(GROUP,h.cortera_id <> (TYPEOF(h.cortera_id))'');
    populated_cortera_id_pcnt := AVE(GROUP,IF(h.cortera_id = (TYPEOF(h.cortera_id))'',0,100));
    maxlength_cortera_id := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.cortera_id)));
    avelength_cortera_id := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.cortera_id)),h.cortera_id<>(typeof(h.cortera_id))'');
...
SHARED FldIds := DATASET([{1,'sbfe_id'}
      ...
      ,{41,'cortera_id'}
      ,{42,'dt_first_seen'}
      ,{43,'dt_last_seen'}],SALT311.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_cortera_id((SALT311.StrType)le.cortera_id),

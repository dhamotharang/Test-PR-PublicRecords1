...
// First step is to convert the 'match criteria' in the block file into a LGID3 stream
  SHARED IdRec := RECORD
    SALT30.UIDType LGID3; // The cluster id
...
// If and when that becomes an issue we will need to give the option of pulling out certain high specificity fields
  AllIds0 := JOIN(h,bp,(LEFT.sbfe_id = RIGHT.sbfe_id OR RIGHT.sbfe_id = (TYPEOF(RIGHT.sbfe_id))'')
     ...
     AND (LEFT.cnp_btype = RIGHT.cnp_btype OR RIGHT.cnp_btype = (TYPEOF(RIGHT.cnp_btype))''),FilterHits(LEFT,RIGHT),MANY LOOKUP);
...
SHARED InSeq := SORT(WithIssues,RuleNum,LGID3,Good,rcid,LOCAL);
  IdRec FormDid(IdRec le,IdRec ri) := TRANSFORM
    SELF.LGID3 := IF ( le.RuleNum=ri.RuleNum and le.Good = ri.Good, le.LGID3, ri.rcid );
    SELF := ri;
  END;
EXPORT Patches := ITERATE(InSeq,FormDid(LEFT,RIGHT),LOCAL) : PERSIST('~temp::LGID3::BIPV2_LGID3::BlockLink::patches',EXPIRE(Config.PersistExpire));
...
SALT30.MAC_Reassign_UID(h,Patches,LGID3,rcid,h1);

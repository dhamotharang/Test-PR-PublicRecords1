...
Command line options: -MBIPV2_LGID3 -eC:\Users\wilmotkx\AppData\Local\Temp\TFR3F70.tmp 
...
This process was told to produce 3 nines of accuracy on a population of 225000000 with  7082984960 records.
...
and the degree of fuzziness required but should average 26 points with a failed match substracting 15 points.
(This subtraction estimate is based upon 39% of clusters with 2 or more records have 2 or more values for company_name.)
...
(This subtraction estimate is based upon 19% of clusters with 2 or more records have 2 or more values for company_fein.)
...
cortera_id Scoring: This field is not used for scoring; rather it is carried along for context and debugging
...
Additional Scoring for field account_id from file CorteraAccounts:
 A score will be allocated for CorteraAccounts only in the event of an exact match, no penalty attaches to a no-match
The exact score will depend upon the global scarcity of CorteraAccounts but should average 27.
If multiple CorteraAccounts match for the same cluster pairing only the highest scoring 1 will apply.
There is no force condition on this attributefile so all mis matches are ignored.

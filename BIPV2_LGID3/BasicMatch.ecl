...
SHARED  h00 := LinkBlockers(ih).input_file;
  ...
  SHARED h00_match := h00( 
      0 + IF( sbfe_id  IN SET(s.nulls_sbfe_id,sbfe_id), 0, 27 ) + IF( Lgid3IfHrchy  IN SET(s.nulls_Lgid3IfHrchy,Lgid3IfHrchy), 0, 27 ) + IF( company_name  IN SET(s.nulls_company_name,company_name), 0, 26 ) + IF( cnp_number  IN SET(s.nulls_cnp_number,cnp_number), 0, 13 ) + IF( active_duns_number  IN SET(s.nulls_active_duns_number,active_duns_number), 0, 27 ) + IF( duns_number  IN SET(s.nulls_duns_number,duns_number), 0, 27 ) + IF( company_fein  IN SET(s.nulls_company_fein,company_fein), 0, 26 ) + IF( company_inc_state  IN SET(s.nulls_company_inc_state,company_inc_state), 0, 6 ) + IF( company_charter_number  IN SET(s.nulls_company_charter_number,company_charter_number), 0, 26 ) + IF( cnp_btype  IN SET(s.nulls_cnp_btype,cnp_btype), 0, 3 ) >= Config.BasicMatchThreshold); // Potentially remove anything that would violate FORCE(+) constraints
...
SHARED NotBlocked := JOIN(PickOne,Block,LEFT.LGID31=RIGHT.LGID31 AND LEFT.LGID32=RIGHT.LGID32,TRANSFORM(LEFT),LEFT ONLY, LOOKUP) : PERSIST('~temp::LGID3::BIPV2_LGID3::BasicMatch',EXPIRE(Config.PersistExpire));
EXPORT patch_file := NotBlocked;
  ut.MAC_Patch_Id(h00,LGID3,NotBlocked,LGID31,LGID32,o1); // Patch the input file

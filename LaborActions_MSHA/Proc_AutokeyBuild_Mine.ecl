...
EXPORT Proc_AutokeyBuild_Mine(STRING filedate,STRING filetype) := FUNCTION
		...
		ak_keyname 				:= c.ak_keyname_mine; 		 	/* AutoKey Superfile container name											*/
		ak_logical  			:= c.ak_logical_mine;  			/* AutoKey Logical file name														*/        
	...
	ds_inLayoutMaster_AKB := PROJECT(ds_forLayoutMaster_AKB, 
		...
		SELF.inp.bname      := LEFT.Mine_Name; 
		SELF.inp.fein       := '';
		SELF.inp.bphone     := '';  
		SELF.inp.bprim_name := ''; 
		SELF.inp.bprim_range:= ''; 
		SELF.inp.bst        := LEFT.Mine_State; 
		SELF.inp.bcity_name := ''; 
		SELF.inp.bzip       := ''; 
		SELF.inp.bsec_range := ''; 
	...
	mod_AKB := module(AutokeyB2.Fn_Build.params) 
		...
		EXPORT STRING L_inkeyname := 	LaborActions_MSHA.constants(,filetype).ak_keyname_mine; 
		EXPORT STRING L_inlogical := 	LaborActions_MSHA.constants(filedate,filetype).ak_logical_mine; 

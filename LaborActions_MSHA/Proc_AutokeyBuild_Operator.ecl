...
EXPORT Proc_AutokeyBuild_Operator(STRING filedate,STRING filetype) := FUNCTION
		...
		ak_keyname  					:= c.ak_keyname_operator; 			/* AutoKey Superfile container name											*/
		ak_logical  					:= c.ak_logical_operator; 			/* AutoKey Logical file name														*/        
	...
	ds_inLayoutMaster_AKB := PROJECT(ds_forLayoutMaster_AKB, 
		...
		SELF.inp.fname      := LEFT.Operator_Name_CLN_FName; 
		SELF.inp.mname      := LEFT.Operator_Name_CLN_MName; 
		SELF.inp.lname      := LEFT.Operator_Name_CLN_LName; 
		SELF.inp.ssn        := ''; 
		SELF.inp.dob        := 0; 
		SELF.inp.phone      := ''; 
		SELF.inp.prim_name  := ''; 
		...
		SELF.inp.bname      := LEFT.Operator_Name_Business; 
		SELF.inp.fein       := '';
		SELF.inp.bphone     := '';  
		SELF.inp.bprim_name := ''; 
	...
	mod_AKB := module(AutokeyB2.Fn_Build.params) 
		...
		EXPORT STRING L_inkeyname := 	LaborActions_MSHA.constants(,filetype).ak_keyname_operator; 
		EXPORT STRING L_inlogical := 	LaborActions_MSHA.constants(filedate,filetype).ak_logical_operator; 

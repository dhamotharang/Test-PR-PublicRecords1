...
EXPORT Proc_AutokeyBuild_Controller(STRING filedate,STRING filetype) := FUNCTION

		c := LaborActions_MSHA.Constants(filedate,filetype);
		ak_dataset_mine 	:= c.ak_dataset_mine;  					/* Base file projected as new layout with did & bdid */
		ak_keyname				:= c.ak_keyname_controller;  		/* AutoKey Superfile container name											*/
		ak_logical				:= c.ak_logical_controller;  		/* AutoKey Logical file name														*/        
	...
	ds_inLayoutMaster_AKB := PROJECT(ds_forLayoutMaster_AKB, 
		...
		SELF.inp.fname      := LEFT.Controller_Name_CLN_FName; 
		SELF.inp.mname      := LEFT.Controller_Name_CLN_MName; 
		SELF.inp.lname      := LEFT.Controller_Name_CLN_LName; 
		SELF.inp.ssn        := ''; 
		SELF.inp.dob        := 0; 
		SELF.inp.phone      := ''; 
		SELF.inp.prim_name  := ''; 
		...
		SELF.inp.bname      := LEFT.Controller_Name_Business; 
		SELF.inp.fein       := '';
		SELF.inp.bphone     := '';  
		SELF.inp.bprim_name := ''; 
	...
	mod_AKB := module(AutokeyB2.Fn_Build.params) 
		...
		EXPORT STRING L_inkeyname := 	LaborActions_MSHA.constants(,filetype).ak_keyname_controller; 
		EXPORT STRING L_inlogical := 	LaborActions_MSHA.constants(filedate,filetype).ak_logical_controller; 

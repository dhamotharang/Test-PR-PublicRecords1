...
EXPORT AddAllComponentstoBuild(string			pLoginToken,
																	string			pBuildName,
...
rstuff := record
  ...
  Inputrec := record
		...
		dataset(rstuff)				ReceiveInstanceIds			{xpath('ReceiveInstanceIds')}		:=	project(pBuildCandidates ( trim(ComponentType) = 'ReceiveInstance' ) ,transform(rstuff,self.arrlong := left.ID));
		dataset(rstuff)				BuildInstanceIds				{xpath('BuildInstanceIds')}			:=	project(pBuildCandidates  ( trim(ComponentType) = 'BuildInstance' ),transform(rstuff,self.arrlong := left.ID));
	end;

...
export proc_Orbit3_CreateBuild_AddItem_npf_sp(string buildname,string Buildvs, string email_list = '',boolean skipcreatebuild = false,boolean skipupdatebuild = false,   boolean skipaddcomponents = false, boolean runcreatebuild = true, boolean runaddcomponentsonly = false,string wuid) := function
...
get_build_candidates := 	Orbit3.GetBuildCandidates(buildname,
									Buildvs,
									tokenval,
									get_build.BuildId) ; //( Name = 'OFAC*' and version = Buildvs[5..6]+'-'+Buildvs[7..8]+'-'+Buildvs[1..4]);

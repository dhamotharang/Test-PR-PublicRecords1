...
Export  ShortForm_20151117(String UpdateDate) := Function
...
//      self.lend_score := 100 - ut.CompanySimilar100(l.lendername, l.lender_last_name);
...
//fnMatchScore := mch_join(lend_score >= 75);

//MatchFile_DtFilter := fnMatchScore((Integer)Recording_date_yyyymmdd > (Integer) Cln_CompleteDate);
MatchFile_DtFilter := fAfterJoinDeed((Integer)Recording_date_yyyymmdd > (Integer) 20151001);
output(choosen(fAfterJoinDeed,1000));
output(count(MatchFile_DtFilter));
...
SrtFile := Sort(MatchFile,QFormUID,lendername,lender_last_name);
DedupFile := Dedup(MatchFile,QFormUID,lendername,lender_last_name);

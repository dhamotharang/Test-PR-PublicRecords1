IMPORT BIPV2, Business_Risk_BIP, Cortera, MDR, Risk_Indicators, UT, STD, PublicRecords_KEL;
...
EXPORT Business_Shell_Function(DATASET(Business_Risk_BIP.Layouts.Input) InputOrig,
  ...
  shell_version := Options.BusShellVersion; // just to minimize Options references.

  Using_BusShell31andHigher := shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v31;
  ...
  checkBlank(STRING field, STRING default_val, UNSIGNED minVersion=2) :=
      MAP(shell_version < minVersion => '',  field = '' => default_val,  field);

  // Function to blank out attributes that are not to be returned in the requested business shell version.
  checkVersion(STRING field, UNSIGNED minVersion=2) := IF(shell_version < minVersion, '', field);
  ...
  input_V31 := Business_Risk_BIP.fn_getBestInfoBySeleID(InputOrig, Options, linkingOptions, AllowedSourcesSet);

  // Specify type of input based on Business Shell version.
  Input := MAP(
      shell_version < Business_Risk_BIP.Constants.BusShellVersion_v22 => InputOrig,
      shell_version BETWEEN Business_Risk_BIP.Constants.BusShellVersion_v22 AND Business_Risk_BIP.Constants.BusShellVersion_v30 => modInp.InputOrigPlusAltCompanyNames,
      shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v31 => Business_Risk_BIP.mod_InputDataset(input_V31, Options).InputOrigPlusAltCompanyNames,
      InputOrig);

  cleanedInput := Business_Risk_BIP.fn_CleanInput(Input, Options);

  withAuthRepLexIDs := Business_Risk_BIP.getAuthRepLexIDs(cleanedInput, Options);
  ...
  withDIDwithSeleID := PROJECT(withDID(Using_BusShell31andHigher and input_echo.SeleID <> 0),
  ...
  prepBIPAppend := PROJECT(withDID(shell_version <> Business_Risk_BIP.Constants.BusShellVersion_v31 or input_echo.SeleID = 0), TRANSFORM(Business_Risk_BIP.Layouts.Input, SELF := LEFT.Clean_Input));
  ...
  mod_b2b := PROJECT(Options, Business_Risk_BIP.LIB_B2B_options);
  ...
  withKELB2B := JOIN(withBIP, ds_KELB2B_Attributes, LEFT.Seq = RIGHT.g_procbusuid, TRANSFORM(Business_Risk_BIP.Layouts.Shell,
                        SELF.KELB2BAttributes.G_BuildB2BDt := IF(Using_BusShell31andHigher,RIGHT.G_BuildB2BDt, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BCntEv := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCntEv, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BCnt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCnt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BCarrCnt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCarrCnt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BFltCnt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BFltCnt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BMatCnt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BMatCnt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOpsCnt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOpsCnt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOthCnt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOthCnt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BCarrPct2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCarrPct2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BFltPct2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BFltPct2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BMatPct2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BMatPct2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOpsPct2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOpsPct2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOthPct2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOthPct2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOldDtEv := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOldDtEv, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BOldMsncEv := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOldMsncEv, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOldDt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOldDt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BNewDt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BNewDt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BOldMsnc2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOldMsnc2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BNewMsnc2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BNewMsnc2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvCnt := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCnt, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvCarrCnt := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCarrCnt, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvFltCnt := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvFltCnt, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvMatCnt := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvMatCnt, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOpsCnt := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOpsCnt, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOthCnt := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOthCnt, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvCarrPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCarrPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvFltPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvFltPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvMatPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvMatPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOpsPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOpsPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOthPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOthPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvCntA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCntA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvCarrCntA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCarrCntA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvFltCntA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvFltCntA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvMatCntA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvMatCntA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOpsCntA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOpsCntA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOthCntA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOthCntA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvCntGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCntGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvCarrCntGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCarrCntGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvFltCntGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvFltCntGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvMatCntGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvMatCntGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOpsCntGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOpsCntGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOthCntGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOthCntGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvBalTot := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvBalTot, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvCarrBalTot := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCarrBalTot, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvFltBalTot := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvFltBalTot, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvMatBalTot := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvMatBalTot, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOpsBalTot := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOpsBalTot, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOthBalTot := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOthBalTot, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvCarrBalTotPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCarrBalTotPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvFltBalPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvFltBalPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvMatBalPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvMatBalPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOpsBalPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOpsBalPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOthBalPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOthBalPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvBalTotRnge := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvBalTotRnge, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvCarrBalTotRnge := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCarrBalTotRnge, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvFltBalTotRnge := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvFltBalTotRnge, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvMatBalTotRnge := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvMatBalTotRnge, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvOpsBalTotRnge := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOpsBalTotRnge, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvOthBalTotRnge := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOthBalTotRnge, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvBalAvg := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvBalAvg, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvCarrBalAvg := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCarrBalAvg, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvFltBalAvg := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvFltBalAvg, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvMatBalAvg := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvMatBalAvg, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOpsBalAvg := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOpsBalAvg, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOthBalAvg := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOthBalAvg, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvBalTotA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvBalTotA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvCarrBalTotA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCarrBalTotA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvFltBalTotA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvFltBalTotA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvMatBalTotA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvMatBalTotA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOpsBalTotA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOpsBalTotA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOthBalTotA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOthBalTotA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvBalTotGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvBalTotGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvCarrBalTotGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCarrBalTotGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvFltBalTotGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvFltBalTotGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvMatBalTotGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvMatBalTotGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOpsBalTotGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOpsBalTotGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvOthBalTotGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOthBalTotGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActvBalTotGrowIndx1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvBalTotGrowIndx1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvCarrBalTotGrowIndx1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCarrBalTotGrowIndx1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvFltBalTotGrowIndx1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvFltBalTotGrowIndx1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvMatBalTotGrowIndx1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvMatBalTotGrowIndx1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvOpsBalTotGrowIndx1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOpsBalTotGrowIndx1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvOthBalTotGrowIndx1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOthBalTotGrowIndx1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BBalMax2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BBalMax2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BCarrBalMax2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCarrBalMax2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BFltBalMax2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BFltBalMax2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BMatBalMax2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BMatBalMax2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOpsBalMax2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOpsBalMax2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOthBalMax2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOthBalMax2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BBalMaxDt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BBalMaxDt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BCarrBalMaxDt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCarrBalMaxDt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BFltBalMaxDt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BFltBalMaxDt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BMatBalMaxDt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BMatBalMaxDt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BOpsBalMaxDt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOpsBalMaxDt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BOthBalMaxDt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOthBalMaxDt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BBalMaxMsnc2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BBalMaxMsnc2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BCarrBalMaxMsnc2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCarrBalMaxMsnc2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BFltBalMaxMsnc2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BFltBalMaxMsnc2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BMatBalMaxMsnc2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BMatBalMaxMsnc2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOpsBalMaxMsnc2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOpsBalMaxMsnc2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOthBalMaxMsnc2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOthBalMaxMsnc2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BBalMaxSegType2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BBalMaxSegType2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvWorstPerfIndx := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvWorstPerfIndx, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvCarrWorstPerfIndx := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvCarrWorstPerfIndx, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvFltWorstPerfIndx := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvFltWorstPerfIndx, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvMatWorstPerfIndx := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvMatWorstPerfIndx, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvOpsWorstPerfIndx := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOpsWorstPerfIndx, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActvOthWorstPerfIndx := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActvOthWorstPerfIndx, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BActv1pDpdCnt := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv1pDpdCnt, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv31pDpdCnt := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv31pDpdCnt, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv61pDpdCnt := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv61pDpdCnt, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv91pDpdCnt := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv91pDpdCnt, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv1pDpdPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv1pDpdPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv31pDpdPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv31pDpdPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv61pDpdPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv61pDpdPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv91pDpdPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv91pDpdPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv1pDpdBalTot := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv1pDpdBalTot, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv31pDpdBalTot := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv31pDpdBalTot, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv61pDpdBalTot := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv61pDpdBalTot, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv91pDpdBalTot := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv91pDpdBalTot, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv1pDpdBalTotPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv1pDpdBalTotPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv31pDpdBalTotPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv31pDpdBalTotPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv61pDpdBalTotPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv61pDpdBalTotPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv91pDpdBalTotPct := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv91pDpdBalTotPct, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv1pDpdBalTotA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv1pDpdBalTotA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv31pDpdBalTotA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv31pDpdBalTotA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv61pDpdBalTotA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv61pDpdBalTotA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv91pDpdBalTotA1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv91pDpdBalTotA1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv1pDpdBalTotGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv1pDpdBalTotGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv31pDpdBalTotGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv31pDpdBalTotGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv61pDpdBalTotGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv61pDpdBalTotGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BActv91pDpdBalTotGrow1Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BActv91pDpdBalTotGrow1Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BWorstPerfIndx2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BWorstPerfIndx2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BCarrWorstPerfIndx2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCarrWorstPerfIndx2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BFltWorstPerfIndx2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BFltWorstPerfIndx2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BMatWorstPerfIndx2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BMatWorstPerfIndx2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BOpsWorstPerfIndx2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOpsWorstPerfIndx2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BOthWorstPerfIndx2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOthWorstPerfIndx2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BWorstPerfDt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BWorstPerfDt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BCarrWorstPerfDt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCarrWorstPerfDt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BFltWorstPerfDt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BFltWorstPerfDt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BMatWorstPerfDt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BMatWorstPerfDt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BOpsWorstPerfDt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOpsWorstPerfDt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BOthWorstPerfDt2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOthWorstPerfDt2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BWorstPerfMsnc2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BWorstPerfMsnc2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BCarrWorstPerfMsnc2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCarrWorstPerfMsnc2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BFltWorstPerfMsnc2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BFltWorstPerfMsnc2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BMatWorstPerfMsnc2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BMatWorstPerfMsnc2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOpsWorstPerfMsnc2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOpsWorstPerfMsnc2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOthWorstPerfMsnc2Y := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOthWorstPerfMsnc2Y, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BCnt24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCnt24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BCarrCnt24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCarrCnt24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BFltCnt24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BFltCnt24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BMatCnt24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BMatCnt24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOpsCnt24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOpsCnt24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOthCnt24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOthCnt24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BRecFlagByMonStr24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BRecFlagByMonStr24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BCarrRecFlagByMonStr24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCarrRecFlagByMonStr24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BFltRecFlagByMonStr24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BFltRecFlagByMonStr24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BMatRecFlagByMonStr24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BMatRecFlagByMonStr24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BOpsRecFlagByMonStr24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOpsRecFlagByMonStr24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BOthRecFlagByMonStr24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOthRecFlagByMonStr24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA);
                        SELF.KELB2BAttributes.BE_B2BRecFlagByMonSum24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BRecFlagByMonSum24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BCarrRecFlagByMonSum24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCarrRecFlagByMonSum24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BFltRecFlagByMonSum24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BFltRecFlagByMonSum24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BMatRecFlagByMonSum24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BMatRecFlagByMonSum24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOpsRecFlagByMonSum24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOpsRecFlagByMonSum24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOthRecFlagByMonSum24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOthRecFlagByMonSum24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BBalVol24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BBalVol24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BCarrBalVol24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BCarrBalVol24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BFltBalVol24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BFltBalVol24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BMatBalVol24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BMatBalVol24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOpsBalVol24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOpsBalVol24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF.KELB2BAttributes.BE_B2BOthBalVol24Mc := IF(Using_BusShell31andHigher,RIGHT.BE_B2BOthBalVol24Mc, Business_Risk_BIP.Constants.MISSING_INPUT_DATA_INT);
                        SELF := LEFT),
                    LEFT OUTER, KEEP(1), ATMOST(100), PARALLEL, FEW);
  ...
  withWatchlists := JOIN(withKELB2B, WatchlistHit, LEFT.Seq = RIGHT.Seq, TRANSFORM(Business_Risk_BIP.Layouts.Shell,
   ...
   withPhone := JOIN(withWatchlists, Phone, LEFT.Seq = RIGHT.Seq, TRANSFORM(Business_Risk_BIP.Layouts.Shell,
                                                ...
                                                SELF.Input_Characteristics.InputPhoneEntityCount := IF(shell_version <= Business_Risk_BIP.Constants.BusShellVersion_v22, RIGHT.Input_Characteristics.InputPhoneEntityCount, LEFT.Input_Characteristics.InputPhoneEntityCount),
  ...
  withInputBasedSources := IF(shell_version <= Business_Risk_BIP.Constants.BusShellVersion_v21, withPhone, // unless running version 2.2 or higher we don't need any of the 'input based' attributes
  ...
  withBusinessByFEIN := IF(shell_version <= Business_Risk_BIP.Constants.BusShellVersion_v21, withInputBasedSources, // unless running version 2.2 or higher we don't need any of the 'input based' attributes
  ...
  withBusinessByPhone := IF(shell_version <= Business_Risk_BIP.Constants.BusShellVersion_v22, withBusinessByFEIN, // unless running version 3.0 or higher we don't need any of the 'input based' attributes
  ...
  withBusinessByAddress := IF(shell_version <= Business_Risk_BIP.Constants.BusShellVersion_v22, withBusinessByPhone, // unless running version 3.0 or higher we don't need any of the 'input based' attributes
 ...
 withInquiriesByInputs := JOIN(withBusinessByAddress, InquiriesByInputs, LEFT.Seq = RIGHT.Seq, TRANSFORM(Business_Risk_BIP.Layouts.Shell,
  ...
  withBestBusinessInfo :=  IF(shell_version <= Business_Risk_BIP.Constants.BusShellVersion_v22, LinkIDsFound,
  ...
  consumerHeader_all := Business_Risk_BIP.getConsumerHeader(businessHeader + NoLinkIDsFound, Options, AllowedSourcesSet); // Pass in Business Header results for companies with BIP IDs assigned. Also pass in records with no BIP IDs assigned.
  ...
  Inquiries := Business_Risk_BIP.getInquiries(LinkIDsFound, Options, AllowedSourcesSet);
  ...
  withBusinessHeader := JOIN(withBestBusinessInfo, businessHeader, LEFT.Seq = RIGHT.Seq, TRANSFORM(Business_Risk_BIP.Layouts.Shell,
                                                ...
                                                SELF.Organizational_Structure.OrgAddrLegalEntityCount := IF(shell_version <= Business_Risk_BIP.Constants.BusShellVersion_v22,
  ...
  withAssociates := JOIN(withConsumerHeader, associates, LEFT.Seq = RIGHT.Seq, TRANSFORM(Business_Risk_BIP.Layouts.Shell,
	...
	withAuthRepTitles := if(options.IsBIID20, withAssociates, Business_Risk_BIP.getAuthRepTitles(withAssociates, Options, linkingOptions, AllowedSourcesSet));
  ...
  withAddrAndPropertyData := JOIN(withTradelines, AddrAndPropertyData, LEFT.Seq = RIGHT.Seq, TRANSFORM(Business_Risk_BIP.Layouts.Shell,
                                                SELF.Sources := LEFT.Sources + RIGHT.Sources;
                                                SELF.Verification.AddrIsBest                       := IF(shell_version <= Business_Risk_BIP.Constants.BusShellVersion_v22, RIGHT.Verification.AddrIsBest, LEFT.Verification.AddrIsBest);
  ...
  withOther := JOIN(withInquiries, otherSources, LEFT.Seq = RIGHT.Seq, TRANSFORM(Business_Risk_BIP.Layouts.Shell,
                                                ...
                                                SELF.SICNAICSources := LEFT.SICNAICSources + RIGHT.SICNAICSources;
                                                SELF.SalesSources := LEFT.SalesSources + RIGHT.SalesSources;
                                                SELF.Verification.PhoneDisconnected := IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30, LEFT.Verification.PhoneDisconnected, checkBlank(RIGHT.Verification.PhoneDisconnected, '2')); // A 2 indicates "Unknown"
                                                SELF.Verification.BNAP := (STRING)MAX((INTEGER)LEFT.Verification.BNAP, (INTEGER)RIGHT.Verification.BNAP); // Keep the MAX of the Business Header, Gong, and Phones Plus BNAP
                                                SELF.Verification.BNAP2 := mergeBNAP(LEFT.Verification.BNAP2, RIGHT.Verification.BNAP2); // Merge the BNAP2 from Business Header, Gong, and Phones Plus
                                                SELF.Verification.PhoneMatch := (STRING)MAX((INTEGER)LEFT.Verification.PhoneMatch, (INTEGER)RIGHT.Verification.PhoneMatch);
 ...
 withMotorVehicles := JOIN(withAirWatercraft, MotorVehicleData, LEFT.Seq = RIGHT.Seq, TRANSFORM(Business_Risk_BIP.Layouts.Shell,
  ...
  withPhoneAddrDistance := IF(shell_version <= Business_Risk_BIP.Constants.BusShellVersion_v21, withGovernmentDebarred, //PhoneAddrDistance attributes only needed for v2.2 and above.
  ...
  withCortera := IF(shell_version <= Business_Risk_BIP.Constants.BusShellVersion_v22, withProfLic,
  ...
  Business_Risk_BIP.Layouts.Shell finalizeDelimitedFields(Business_Risk_BIP.Layouts.Shell le) := TRANSFORM
    ...
    // SELF.PhoneSourcesRaw := le.PhoneSources + IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30, le.PhoneIDSources);
    ...
    GroupedPhoneSources := Business_Risk_BIP.Common.groupSources(Business_Risk_BIP.Layouts.LayoutSources, le.PhoneSources +
                                            IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30, le.PhoneIDSources));
    ...
    SeqEmployeeSources := SORT(UniqueEmployeeSources, -DateLastSeen, -DateFirstSeen, Source, -RecordCount)
              (IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30, RecordCount > -1, RecordCount > 0));

    gong_src := IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v22, MDR.sourceTools.set_Gong_History, MDR.sourceTools.set_Gong_Business);
    ...
    SELF.Input_Characteristics.InputAddrConsumerCount := IF(le.Input.InputCheckBusAddrZip = '0' AND shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', le.Input_Characteristics.InputAddrConsumerCount);
    ...
    InputAddrLengthOfResidence := IF(NOT cantVerifyAddress AND newestAddrDate <> Business_Risk_BIP.Constants.MissingDate AND oldestAddrDate <> Business_Risk_BIP.Constants.MissingDate, IF((Using_BusShell31andHigher) AND (months_apart='0'),'1',months_apart),
		                                                                                                                                                                                     IF(Using_BusShell31andHigher, '0','-1'));
    ...
    SELF.Verification.PhoneMatchSourceCount := IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30 AND le.Input.InputCheckBusPhone = '0', '-1',
    ...
    SELF.Verification.FEINMatchSourceCount := IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30 AND le.Input.InputCheckBusFEIN = '0', '-1',
    ...
    AR2BBusPAWRep1 := IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30 AND (TRIM(le.Clean_Input.Rep_FirstName) = '' AND TRIM(le.Clean_Input.Rep_LastName) = ''), '-1', le.Business_To_Executive_Link.AR2BBusPAWRep1);
    ...
    AR2BBusPAWRep2 := IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30 AND (TRIM(le.Clean_Input.Rep2_FirstName) = '' AND TRIM(le.Clean_Input.Rep2_LastName) = ''), '-1', le.Business_To_Executive_Link.AR2BBusPAWRep2);
    ...
    AR2BBusPAWRep3 := IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30 AND (TRIM(le.Clean_Input.Rep3_FirstName) = '' AND TRIM(le.Clean_Input.Rep3_LastName) = ''), '-1', le.Business_To_Executive_Link.AR2BBusPAWRep3);
    ...
    AR2BBusPAWRep4 := IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30 AND (TRIM(le.Clean_Input.Rep4_FirstName) = '' AND TRIM(le.Clean_Input.Rep4_LastName) = ''), '-1', le.Business_To_Executive_Link.AR2BBusPAWRep4);
    ...
    AR2BBusPAWRep5 := IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30 AND (TRIM(le.Clean_Input.Rep5_FirstName) = '' AND TRIM(le.Clean_Input.Rep5_LastName) = ''), '-1', le.Business_To_Executive_Link.AR2BBusPAWRep5);
  ...
  //  SELF.Verification.AddrMiskey := Business_Risk_BIP.Common.SetBoolean(FALSE);
    ...
    SourceNonDerogCount24MonthID := (STRING)Business_Risk_BIP.Common.CapNum(COUNT(
                            IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30, NonDerogSourceRecords24MonthID, NonDerogSourceRecords12MonthID)), -1, 99999);
    ...
    SELF.Business_Activity.BusinessActivity12MonthID := MAP((INTEGER)MaxDateLastSeenID <= 0                             => '-1',
                                                        ...
                                                        COUNT(NonDerogSourceRecords12MonthID) >= 4                    => '5',
                                                                                                                       '-1');
    sourcelistID := Business_Risk_BIP.Common.convertDelimited(SeqSourcesLinkIds, Source, Business_Risk_BIP.Constants.FieldDelimiter);
    SELF.Verification.sourcelistID := sourcelistID;
    ...
    SELF.Verification.PhoneMatchSourceCountID :=  IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30 AND le.Input.InputCheckBusPhone = '0', '-1',
    ...
    SELF.Data_Fetch_Indicators.LargeBusiness := IF(
      ...
      '1', '0');
        ...
        VerifiedOnShell := VerInputIDTruebiz='1'; 
        ShellCorteraResultsFound := (INTEGER)(STD.Str.Find(SourceListID,MDR.SourceTools.src_Cortera))>0 AND VerifiedOnShell;
        
        SELF.KELB2BAttributes.BE_B2BCntEv := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCntEv<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BCntEv,
                                                ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,
                                                                    0);
        SELF.KELB2BAttributes.BE_B2BCnt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCntEv<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BCnt2Y, 
                                                 ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,
                                                                     0/*-99998 on other fields*/);
        SELF.KELB2BAttributes.BE_B2BCarrCnt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCarrCnt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BCarrCnt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,0);
        SELF.KELB2BAttributes.BE_B2BFltCnt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BFltCnt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BFltCnt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,0);
        SELF.KELB2BAttributes.BE_B2BMatCnt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BMatCnt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BMatCnt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,0);
        SELF.KELB2BAttributes.BE_B2BOpsCnt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOpsCnt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOpsCnt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,0);
        SELF.KELB2BAttributes.BE_B2BOthCnt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOthCnt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOthCnt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,0);
        SELF.KELB2BAttributes.BE_B2BCarrPct2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCarrPct2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BCarrPct2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BFltPct2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BFltPct2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BFltPct2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BMatPct2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BMatPct2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BMatPct2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOpsPct2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOpsPct2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOpsPct2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOthPct2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOthPct2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOthPct2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOldDtEv := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOldDtEv<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BOldDtEv, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BOldMsncEv := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOldMsncEv<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOldMsncEv, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOldDt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOldDt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BOldDt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BNewDt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BNewDt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BNewDt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BOldMsnc2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOldMsnc2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOldMsnc2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BNewMsnc2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BNewMsnc2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BNewMsnc2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvCnt := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCnt<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvCnt, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,0);
        SELF.KELB2BAttributes.BE_B2BActvCarrCnt := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCarrCnt<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvCarrCnt, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,0);
        SELF.KELB2BAttributes.BE_B2BActvFltCnt := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvFltCnt<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvFltCnt, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,0);
        SELF.KELB2BAttributes.BE_B2BActvMatCnt := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvMatCnt<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvMatCnt, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,0);
        SELF.KELB2BAttributes.BE_B2BActvOpsCnt := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOpsCnt<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOpsCnt, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,0);
        SELF.KELB2BAttributes.BE_B2BActvOthCnt := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOthCnt<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOthCnt, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,0);
        SELF.KELB2BAttributes.BE_B2BActvCarrPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCarrPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvCarrPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvFltPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvFltPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvFltPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvMatPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvMatPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvMatPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOpsPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOpsPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOpsPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOthPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOthPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOthPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvCntA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCntA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvCntA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvCarrCntA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCarrCntA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvCarrCntA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvFltCntA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvFltCntA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvFltCntA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvMatCntA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvMatCntA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvMatCntA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOpsCntA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOpsCntA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOpsCntA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOthCntA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOthCntA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOthCntA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvCntGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCntGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvCntGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvCarrCntGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCarrCntGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvCarrCntGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvFltCntGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvFltCntGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvFltCntGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvMatCntGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvMatCntGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvMatCntGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOpsCntGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOpsCntGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOpsCntGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOthCntGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOthCntGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOthCntGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvBalTot := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvBalTot<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvBalTot, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvCarrBalTot := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCarrBalTot<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvCarrBalTot, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvFltBalTot := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvFltBalTot<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvFltBalTot, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvMatBalTot := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvMatBalTot<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvMatBalTot, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOpsBalTot := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOpsBalTot<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOpsBalTot, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOthBalTot := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOthBalTot<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOthBalTot, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvCarrBalTotPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCarrBalTotPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvCarrBalTotPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvFltBalPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvFltBalPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvFltBalPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvMatBalPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvMatBalPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvMatBalPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOpsBalPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOpsBalPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOpsBalPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOthBalPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOthBalPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOthBalPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvBalTotRnge := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvBalTotRnge<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvBalTotRnge, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvCarrBalTotRnge := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCarrBalTotRnge<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvCarrBalTotRnge, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvFltBalTotRnge := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvFltBalTotRnge<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvFltBalTotRnge, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvMatBalTotRnge := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvMatBalTotRnge<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvMatBalTotRnge, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvOpsBalTotRnge := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOpsBalTotRnge<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvOpsBalTotRnge, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvOthBalTotRnge := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOthBalTotRnge<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvOthBalTotRnge, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvBalAvg := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvBalAvg<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvBalAvg, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvCarrBalAvg := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCarrBalAvg<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvCarrBalAvg, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvFltBalAvg := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvFltBalAvg<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvFltBalAvg, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvMatBalAvg := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvMatBalAvg<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvMatBalAvg, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOpsBalAvg := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOpsBalAvg<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOpsBalAvg, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOthBalAvg := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOthBalAvg<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOthBalAvg, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvBalTotA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvBalTotA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvBalTotA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvCarrBalTotA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCarrBalTotA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvCarrBalTotA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvFltBalTotA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvFltBalTotA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvFltBalTotA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvMatBalTotA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvMatBalTotA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvMatBalTotA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOpsBalTotA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOpsBalTotA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOpsBalTotA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOthBalTotA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOthBalTotA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOthBalTotA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvBalTotGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvBalTotGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvBalTotGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvCarrBalTotGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCarrBalTotGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvCarrBalTotGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvFltBalTotGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvFltBalTotGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvFltBalTotGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvMatBalTotGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvMatBalTotGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvMatBalTotGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOpsBalTotGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOpsBalTotGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOpsBalTotGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvOthBalTotGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOthBalTotGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActvOthBalTotGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActvBalTotGrowIndx1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvBalTotGrowIndx1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvBalTotGrowIndx1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvCarrBalTotGrowIndx1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCarrBalTotGrowIndx1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvCarrBalTotGrowIndx1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvFltBalTotGrowIndx1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvFltBalTotGrowIndx1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvFltBalTotGrowIndx1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvMatBalTotGrowIndx1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvMatBalTotGrowIndx1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvMatBalTotGrowIndx1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvOpsBalTotGrowIndx1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOpsBalTotGrowIndx1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvOpsBalTotGrowIndx1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvOthBalTotGrowIndx1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOthBalTotGrowIndx1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvOthBalTotGrowIndx1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BBalMax2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BBalMax2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BBalMax2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BCarrBalMax2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCarrBalMax2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BCarrBalMax2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BFltBalMax2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BFltBalMax2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BFltBalMax2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BMatBalMax2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BMatBalMax2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BMatBalMax2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOpsBalMax2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOpsBalMax2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOpsBalMax2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOthBalMax2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOthBalMax2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOthBalMax2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BBalMaxDt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BBalMaxDt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BBalMaxDt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BCarrBalMaxDt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCarrBalMaxDt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BCarrBalMaxDt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BFltBalMaxDt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BFltBalMaxDt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BFltBalMaxDt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BMatBalMaxDt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BMatBalMaxDt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BMatBalMaxDt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BOpsBalMaxDt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOpsBalMaxDt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BOpsBalMaxDt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BOthBalMaxDt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOthBalMaxDt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BOthBalMaxDt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BBalMaxMsnc2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BBalMaxMsnc2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BBalMaxMsnc2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BCarrBalMaxMsnc2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCarrBalMaxMsnc2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BCarrBalMaxMsnc2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BFltBalMaxMsnc2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BFltBalMaxMsnc2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BFltBalMaxMsnc2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BMatBalMaxMsnc2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BMatBalMaxMsnc2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BMatBalMaxMsnc2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOpsBalMaxMsnc2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOpsBalMaxMsnc2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOpsBalMaxMsnc2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOthBalMaxMsnc2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOthBalMaxMsnc2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOthBalMaxMsnc2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BBalMaxSegType2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BBalMaxSegType2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BBalMaxSegType2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvWorstPerfIndx := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvWorstPerfIndx<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvWorstPerfIndx, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvCarrWorstPerfIndx := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvCarrWorstPerfIndx<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvCarrWorstPerfIndx, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvFltWorstPerfIndx := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvFltWorstPerfIndx<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvFltWorstPerfIndx, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvMatWorstPerfIndx := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvMatWorstPerfIndx<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvMatWorstPerfIndx, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvOpsWorstPerfIndx := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOpsWorstPerfIndx<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvOpsWorstPerfIndx, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActvOthWorstPerfIndx := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActvOthWorstPerfIndx<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BActvOthWorstPerfIndx, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BActv1pDpdCnt := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv1pDpdCnt<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv1pDpdCnt, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv31pDpdCnt := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv31pDpdCnt<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv31pDpdCnt, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv61pDpdCnt := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv61pDpdCnt<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv61pDpdCnt, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv91pDpdCnt := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv91pDpdCnt<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv91pDpdCnt, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv1pDpdPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv1pDpdPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv1pDpdPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv31pDpdPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv31pDpdPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv31pDpdPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv61pDpdPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv61pDpdPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv61pDpdPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv91pDpdPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv91pDpdPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv91pDpdPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv1pDpdBalTot := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv1pDpdBalTot<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv1pDpdBalTot, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv31pDpdBalTot := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv31pDpdBalTot<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv31pDpdBalTot, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv61pDpdBalTot := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv61pDpdBalTot<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv61pDpdBalTot, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv91pDpdBalTot := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv91pDpdBalTot<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv91pDpdBalTot, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv1pDpdBalTotPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv1pDpdBalTotPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv1pDpdBalTotPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv31pDpdBalTotPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv31pDpdBalTotPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv31pDpdBalTotPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv61pDpdBalTotPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv61pDpdBalTotPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv61pDpdBalTotPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv91pDpdBalTotPct := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv91pDpdBalTotPct<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv91pDpdBalTotPct, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv1pDpdBalTotA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv1pDpdBalTotA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv1pDpdBalTotA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv31pDpdBalTotA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv31pDpdBalTotA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv31pDpdBalTotA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv61pDpdBalTotA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv61pDpdBalTotA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv61pDpdBalTotA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv91pDpdBalTotA1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv91pDpdBalTotA1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv91pDpdBalTotA1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv1pDpdBalTotGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv1pDpdBalTotGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv1pDpdBalTotGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv31pDpdBalTotGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv31pDpdBalTotGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv31pDpdBalTotGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv61pDpdBalTotGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv61pDpdBalTotGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv61pDpdBalTotGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BActv91pDpdBalTotGrow1Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BActv91pDpdBalTotGrow1Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BActv91pDpdBalTotGrow1Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BWorstPerfIndx2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BWorstPerfIndx2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BWorstPerfIndx2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BCarrWorstPerfIndx2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCarrWorstPerfIndx2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BCarrWorstPerfIndx2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BFltWorstPerfIndx2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BFltWorstPerfIndx2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BFltWorstPerfIndx2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BMatWorstPerfIndx2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BMatWorstPerfIndx2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BMatWorstPerfIndx2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BOpsWorstPerfIndx2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOpsWorstPerfIndx2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BOpsWorstPerfIndx2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BOthWorstPerfIndx2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOthWorstPerfIndx2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING6)le.KELB2BAttributes.BE_B2BOthWorstPerfIndx2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BWorstPerfDt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BWorstPerfDt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BWorstPerfDt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BCarrWorstPerfDt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCarrWorstPerfDt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BCarrWorstPerfDt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BFltWorstPerfDt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BFltWorstPerfDt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BFltWorstPerfDt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BMatWorstPerfDt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BMatWorstPerfDt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BMatWorstPerfDt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BOpsWorstPerfDt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOpsWorstPerfDt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BOpsWorstPerfDt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BOthWorstPerfDt2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOthWorstPerfDt2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING10)le.KELB2BAttributes.BE_B2BOthWorstPerfDt2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BWorstPerfMsnc2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BWorstPerfMsnc2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BWorstPerfMsnc2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BCarrWorstPerfMsnc2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCarrWorstPerfMsnc2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BCarrWorstPerfMsnc2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BFltWorstPerfMsnc2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BFltWorstPerfMsnc2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BFltWorstPerfMsnc2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BMatWorstPerfMsnc2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BMatWorstPerfMsnc2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BMatWorstPerfMsnc2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOpsWorstPerfMsnc2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOpsWorstPerfMsnc2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOpsWorstPerfMsnc2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOthWorstPerfMsnc2Y := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOthWorstPerfMsnc2Y<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOthWorstPerfMsnc2Y, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BCnt24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCnt24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BCnt24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BCarrCnt24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCarrCnt24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BCarrCnt24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BFltCnt24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BFltCnt24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BFltCnt24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BMatCnt24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BMatCnt24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BMatCnt24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOpsCnt24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOpsCnt24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOpsCnt24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOthCnt24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOthCnt24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOthCnt24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BRecFlagByMonStr24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BRecFlagByMonStr24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING30)le.KELB2BAttributes.BE_B2BRecFlagByMonStr24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BCarrRecFlagByMonStr24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCarrRecFlagByMonStr24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING30)le.KELB2BAttributes.BE_B2BCarrRecFlagByMonStr24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BFltRecFlagByMonStr24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BFltRecFlagByMonStr24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING30)le.KELB2BAttributes.BE_B2BFltRecFlagByMonStr24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BMatRecFlagByMonStr24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BMatRecFlagByMonStr24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING30)le.KELB2BAttributes.BE_B2BMatRecFlagByMonStr24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BOpsRecFlagByMonStr24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOpsRecFlagByMonStr24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING30)le.KELB2BAttributes.BE_B2BOpsRecFlagByMonStr24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BOthRecFlagByMonStr24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOthRecFlagByMonStr24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA => (STRING30)le.KELB2BAttributes.BE_B2BOthRecFlagByMonStr24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND);
        SELF.KELB2BAttributes.BE_B2BRecFlagByMonSum24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BRecFlagByMonSum24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BRecFlagByMonSum24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BCarrRecFlagByMonSum24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCarrRecFlagByMonSum24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BCarrRecFlagByMonSum24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BFltRecFlagByMonSum24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BFltRecFlagByMonSum24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BFltRecFlagByMonSum24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BMatRecFlagByMonSum24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BMatRecFlagByMonSum24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BMatRecFlagByMonSum24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOpsRecFlagByMonSum24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOpsRecFlagByMonSum24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOpsRecFlagByMonSum24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOthRecFlagByMonSum24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOthRecFlagByMonSum24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOthRecFlagByMonSum24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BBalVol24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BBalVol24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BBalVol24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BCarrBalVol24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BCarrBalVol24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BCarrBalVol24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BFltBalVol24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BFltBalVol24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BFltBalVol24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BMatBalVol24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BMatBalVol24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BMatBalVol24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOpsBalVol24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOpsBalVol24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOpsBalVol24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);
        SELF.KELB2BAttributes.BE_B2BOthBalVol24Mc := MAP(ShellCorteraResultsFound AND le.KELB2BAttributes.BE_B2BOthBalVol24Mc<>PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT => le.KELB2BAttributes.BE_B2BOthBalVol24Mc, ~VerifiedOnShell => PublicRecords_KEL.ECL_Functions.Constants.MISSING_INPUT_DATA_INT,PublicRecords_KEL.ECL_Functions.Constants.NO_DATA_FOUND_INT);      
 ...
 TempShell := Business_Risk_BIP.getScoresAndIndicators( withVerifiedInputElements, Options );
  ...
  withConsumerHeader_noIds := JOIN(NoLinkIDsFound, consumerHeader_noIds, LEFT.Seq = RIGHT.Seq, TRANSFORM(Business_Risk_BIP.Layouts.Shell,
                                                ...
                                                SELF.Verification.VerifiedConsumerFEIN := RIGHT.Verification.VerifiedConsumerFEIN;
                                                SELF.Verification.VerifiedConsumerPhone := RIGHT.Verification.VerifiedConsumerPhone;
                                                SELF.Input_Characteristics.InputAddrConsumerCount := IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30,
                                                                                            checkBlank(RIGHT.Input_Characteristics.InputAddrConsumerCount, '0'), '-1');
                                                SELF := LEFT),
                                          LEFT OUTER, KEEP(1), ATMOST(100), PARALLEL, FEW);

  TempBlankShell := PROJECT(withConsumerHeader_noIds, Business_Risk_BIP.xfm_finalizeBlankShellFields(LEFT,shell_version));
  ...
  FinalShell := MAP( restrict_sbfe                                                                => FinalShell_restricted, // If SBFE data not allowed, blank all SBFE fields
                     shell_version <= Business_Risk_BIP.Constants.BusShellVersion_v20 => FinalShell_v20,        // If Business shell v2 is requested, blank out SBFE enhancement (v2.1) attributes
                                                                                                   FinalShell_pre );

  FinalShell_rolled := IF(shell_version < Business_Risk_BIP.Constants.BusShellVersion_v22, FinalShell, modInp.fn_PopulateAltCompanyNameFields(FinalShell));

  WithSBA_Patch := Business_Risk_BIP.fn_SASPatch(FinalShell_rolled);

  FinalShell_rolled_WithV21patch := IF(Using_BusShell31andHigher,WithSBA_Patch,FinalShell_rolled);

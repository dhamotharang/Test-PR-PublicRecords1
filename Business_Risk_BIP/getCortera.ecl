  ...
  CorteraUltimateIDStats := TABLE(CorteraUltimateIDData,
	...
	CorteraSIC := TABLE(CorteraRecs,
			{Seq,
			 ...
			 STRING6 DateLastSeen := Business_Risk_BIP.Common.groupMaxDate6(dt_last_seen, HistoryDate),
			 UNSIGNED4 RecordCount := COUNT(GROUP),
			 STRING10 SICCode := (StringLib.StringFilter((STRING)primary_sic, '0123456789'))[1..4],
			 BOOLEAN IsPrimary := TRUE
			 },
			 Seq, Business_Risk_BIP.Common.GetLinkSearchLevel(Options.LinkSearchLevel, SeleID), ((STRING)primary_sic)[1..4]
			 );
	...
	withCorteraSIC := JOIN(withCorteraUltimateIDStats, CorteraSICRolled, LEFT.Seq = RIGHT.Seq,
		...
		CorteraNAIC := TABLE(CorteraRecs,
			{Seq,
			 ...
			 STRING6 DateLastSeen := Business_Risk_BIP.Common.groupMaxDate6(dt_last_seen, HistoryDate),
			 UNSIGNED4 RecordCount := COUNT(GROUP),
			 STRING10 NAICCode := (StringLib.StringFilter((STRING)primary_naics, '0123456789'))[1..6],
			 BOOLEAN IsPrimary := TRUE
			 },
			 Seq, Business_Risk_BIP.Common.GetLinkSearchLevel(Options.LinkSearchLevel, SeleID), ((STRING)primary_naics)[1..6]
			 );

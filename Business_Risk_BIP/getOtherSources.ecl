IMPORT AutoKey, BBB2, BIPV2, Business_Risk, Business_Risk_BIP, CalBus, Data_Services, DNB_FEINV2,
    ...
    Risk_Indicators, TXBUS, UtilFile, STD, YellowPages;
...
EXPORT getOtherSources(DATASET(Business_Risk_BIP.Layouts.Shell) Shell,
	...
	keep_large_business := Options.KeepLargeBusinesses;
	...
	IRS990StatsTemp := JOIN(IRS990Stats, SortTaxIRS990, LEFT.Seq = RIGHT.Seq,
                                        ...
                                        // For v30 and up, need to differentiate between 0 and '' for FirmReportedSales. Set missing records to -1.
																				SELF.SalesSources := DATASET([{LEFT.Source,
																																	...
																																	LEFT.DateLastSeen,
																																	LEFT.DateVendorLastSeen,
																																	IF(RIGHT.Income_Amount = '' AND shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30, -1,
  ...
  // moving filter condition out of join since it was causing Packet Length Exceeded error.
    ...
    Sources := IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v31 AND Options.MarketingMode = 1,
		...
		SELF.NameSources := IF(shell_version >= 22 AND BNAPCalc IN ['5', '8'], Sources, DATASET([], Business_Risk_BIP.Layouts.LayoutSources));
		SELF.AddressVerSources := IF(shell_version >= 22 AND BNAPCalc IN ['4', '8'], Sources, DATASET([], Business_Risk_BIP.Layouts.LayoutSources));
		SELF.BestAddrPhones := IF(shell_version >= Business_Risk_BIP.Constants.BusShellVersion_v30 AND BestAddressMatched AND TRIM(ri.CellPhone) <> '',
	...
	// ---------------- Utility Data ---------------------
	UtilRaw := IF(~Doxie.Compliance.isUtilityRestricted(mod_access.industry_class), UtilFile.Key_LinkIds.kFetch2(Business_Risk_BIP.Common.GetLinkIDs(Shell),
																						 Business_Risk_BIP.Common.SetLinkSearchLevel(link_search_level),
																							0, /*ScoreThreshold --> 0 = Give me everything*/
																							Business_Risk_BIP.Constants.Limit_Default,
																							keep_large_business));
  ...
  BNAPCalc := Business_Risk_BIP.Common.calcBNAP_narrow(NameMatched, AddressMatched, PhoneMatched);
		...
		SELF.NameSources       := IF(shell_version >= 22 AND BNAPCalc IN ['5', '8']     , temp_Sources, DATASET([], Business_Risk_BIP.Layouts.LayoutSources));
		SELF.AddressVerSources := IF(shell_version >= 22 AND BNAPCalc IN ['4', '8']     , temp_Sources, DATASET([], Business_Risk_BIP.Layouts.LayoutSources));
		SELF.BestAddressSources := IF(shell_version >= 30 AND BestAddressMatched    		, temp_Sources, DATASET([], Business_Risk_BIP.Layouts.LayoutSources));
  ...
  withUtil := JOIN(withPhonesPlus, UtilAttrsStatsSources, LEFT.Seq = RIGHT.Seq,
	...
	CalBusNAIC := TABLE(CalBus,
			{Seq,
			 ...
			 Seq, Business_Risk_BIP.Common.GetLinkSearchLevel(link_search_level, SeleID), ((STRING)NAICs_Code)[1..6]
			 );
  ...
  END;
	...
	withPAWAttributes := IF(shell_version <= Business_Risk_BIP.Constants.BusShellVersion_v21, withYellowPages,
  ...
  dataBridge_raw := dx_DataBridge.Key_LinkIds.kFetch2(Business_Risk_BIP.Common.GetLinkIDs(Shell),
	...
	Business_Risk_BIP.Common.AppendSeq(dataBridge_raw, Shell, dataBridge_seq, link_search_level);
  ...
  withDataBridge :=
	...
	Business_Risk_BIP.Common.AppendSeq(TXBus_raw, Shell, TXBus_seq, link_search_level);
  ...
  TXBusNAIC := TABLE(TXBus,
    ...
    Seq, Business_Risk_BIP.Common.GetLinkSearchLevel(link_search_level, SeleID), ((STRING)outlet_naics_code)[1..6]
  );
  ...
  Frandx_raw := Frandx.Key_Linkids.kFetch2(Business_Risk_BIP.Common.GetLinkIDs(Shell),
	...
	Business_Risk_BIP.Common.AppendSeq(Frandx_raw, Shell, Frandx_seq, link_search_level);
  ...
  DNBFEIN_raw := DNB_FEINV2.Key_LinkIds.kFetch2(Business_Risk_BIP.Common.GetLinkIDs(Shell),
	...
	Business_Risk_BIP.Common.AppendSeq(DNBFEIN_raw, Shell, DNBFEIN_seq, link_search_level);

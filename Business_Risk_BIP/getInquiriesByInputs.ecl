...
EXPORT getInquiriesByInputs(DATASET(Business_Risk_BIP.Layouts.Shell) Shell_pre, 
	...
	inquiryTransformMac (TransformName, KeyName) := MACRO
		{tempConsumerInquiry, unsigned4 global_sid, unsigned6 did} TransformName(Business_Risk_BIP.Layouts.Shell le, KeyName ri) := TRANSFORM
			...
			RawIndustry := StringLib.StringToUpperCase(TRIM(ri.Bus_Intel.Industry, LEFT, RIGHT));
			Vertical := StringLib.StringToUpperCase(TRIM(ri.Bus_Intel.Vertical, LEFT, RIGHT));
			SubMarket := StringLib.StringToUpperCase(TRIM(ri.Bus_Intel.Sub_Market, LEFT, RIGHT));
			FunctionDescription := StringLib.StringToUpperCase(TRIM(ri.Search_Info.Function_Description, LEFT, RIGHT));
			ProductCode := StringLib.StringToUpperCase(TRIM(ri.Search_Info.Product_Code, LEFT, RIGHT));
			BusUse := StringLib.StringToUpperCase(TRIM(ri.Bus_Intel.Use, LEFT, RIGHT));
			
			Date := Business_Risk_BIP.Common.checkInvalidDate(TRIM(StringLib.StringFilter(ri.search_info.datetime[1..8], '0123456789')), Business_Risk_BIP.Constants.MissingDate, (INTEGER)Business_Risk_BIP.Constants.NinesDate);
	...
	withConsumerAddressRolled := JOIN(Shell, consumerAddressRolled, LEFT.Seq = RIGHT.Seq,
                                              ...
                                              FEINNotInput := TRIM(LEFT.Clean_Input.FEIN) = '';
																							SELF.Inquiry.InquiryConsumerAddress := IF(AddrNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount, -1, 99999));
																							SELF.Inquiry.InquiryConsumerAddress12Mos := IF(AddrNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount12, -1, 99999));
																							SELF.Inquiry.InquiryConsumerAddress06Mos := IF(AddrNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount06, -1, 99999));
																							SELF.Inquiry.InquiryConsumerAddress03Mos := IF(AddrNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount03, -1, 99999));
																							SELF.Inquiry.InquiryConsumerAddress01Mos := IF(AddrNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount01, -1, 99999));
																							SELF.Inquiry.InquiryConsumerAddressSSN := IF((AddrNotInput OR FEINNotInput) AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquirySSNCount, -1, 99999));
																							SELF.Inquiry.InquiryConsumerAddressSSN12Mos := IF((AddrNotInput OR FEINNotInput) AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquirySSNCount12, -1, 99999));
																							SELF.Inquiry.InquiryConsumerAddressSSN06Mos := IF((AddrNotInput OR FEINNotInput) AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquirySSNCount06, -1, 99999));
																							SELF.Inquiry.InquiryConsumerAddressSSN03Mos := IF((AddrNotInput OR FEINNotInput) AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquirySSNCount03, -1, 99999));
																							SELF.Inquiry.InquiryConsumerAddressSSN01Mos := IF((AddrNotInput OR FEINNotInput) AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquirySSNCount01, -1, 99999));
																							
																							SELF.Inquiry.InquiryBusAddress := IF(AddrNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount, -1, 99999));
																							SELF.Inquiry.InquiryBusAddress24Mos := IF(AddrNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount24, -1, 99999));
																							SELF.Inquiry.InquiryBusAddress12Mos := IF(AddrNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount12, -1, 99999));
																							SELF.Inquiry.InquiryBusAddress06Mos := IF(AddrNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount06, -1, 99999));
																							SELF.Inquiry.InquiryBusAddress03Mos := IF(AddrNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount03, -1, 99999));
																							SELF.Inquiry.InquiryBusAddress01Mos := IF(AddrNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
	...
	withConsumerPhoneRolled := JOIN(withConsumerAddressRolled, consumerPhoneRolled, LEFT.Seq = RIGHT.Seq,
                                              ...
                                              PhoneNotInput := TRIM(LEFT.Clean_Input.Phone10) = '';
																							SELF.Inquiry.InquiryConsumerPhone := IF(PhoneNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount, -1, 99999));
																							SELF.Inquiry.InquiryConsumerPhone12Mos := IF(PhoneNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount12, -1, 99999));
																							SELF.Inquiry.InquiryConsumerPhone06Mos := IF(PhoneNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount06, -1, 99999));
																							SELF.Inquiry.InquiryConsumerPhone03Mos := IF(PhoneNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount03, -1, 99999));
																							SELF.Inquiry.InquiryConsumerPhone01Mos := IF(PhoneNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount01, -1, 99999));
																							
																							SELF.Inquiry.InquiryBusPhone := IF(PhoneNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount, -1, 99999));
																							SELF.Inquiry.InquiryBusPhone24Mos := IF(PhoneNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount24, -1, 99999));
																							SELF.Inquiry.InquiryBusPhone12Mos := IF(PhoneNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount12, -1, 99999));
																							SELF.Inquiry.InquiryBusPhone06Mos := IF(PhoneNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount06, -1, 99999));
																							SELF.Inquiry.InquiryBusPhone03Mos := IF(PhoneNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount03, -1, 99999));
																							SELF.Inquiry.InquiryBusPhone01Mos := IF(PhoneNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount01, -1, 99999));
	...
	withConsumerSSNRolled := JOIN(withConsumerPhoneRolled, consumerSSNRolled, LEFT.Seq = RIGHT.Seq,
																	TRANSFORM(Business_Risk_BIP.Layouts.Shell,
                                              FEINNotInput := TRIM(LEFT.Clean_Input.FEIN) = '';
																							SELF.Inquiry.InquiryConsumerSSN := IF(FEINNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount, -1, 99999));
																							SELF.Inquiry.InquiryConsumerSSN12Mos := IF(FEINNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount12, -1, 99999));
																							SELF.Inquiry.InquiryConsumerSSN06Mos := IF(FEINNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount06, -1, 99999));
																							SELF.Inquiry.InquiryConsumerSSN03Mos := IF(FEINNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount03, -1, 99999));
																							SELF.Inquiry.InquiryConsumerSSN01Mos := IF(FEINNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.InquiryCount01, -1, 99999));
																							SELF.Data_Build_Dates.InquiriesBuildDate := InqBuildDate;
																							SELF := LEFT),
																	LEFT OUTER, KEEP(1), ATMOST(100), FEW);	
	...
	withBusinessFEINRolled := JOIN(withConsumerSSNRolled, businessFEINRolled, LEFT.Seq = RIGHT.Seq,
																	TRANSFORM(Business_Risk_BIP.Layouts.Shell,
                                              FEINNotInput := TRIM(LEFT.Clean_Input.FEIN) = '';
																							SELF.Inquiry.InquiryBusFEIN := IF(FEINNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount, -1, 99999));
																							SELF.Inquiry.InquiryBusFEIN24Mos := IF(FEINNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount24, -1, 99999));
																							SELF.Inquiry.InquiryBusFEIN12Mos := IF(FEINNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount12, -1, 99999));
																							SELF.Inquiry.InquiryBusFEIN06Mos := IF(FEINNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount06, -1, 99999));
																							SELF.Inquiry.InquiryBusFEIN03Mos := IF(FEINNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount03, -1, 99999));
																							SELF.Inquiry.InquiryBusFEIN01Mos := IF(FEINNotInput AND Options.BusShellVersion >= Business_Risk_BIP.Constants.BusShellVersion_v30, '-1', 
                                                                                       (STRING)Business_Risk_BIP.Common.capNum(RIGHT.BusInquiryCount01, -1, 99999));

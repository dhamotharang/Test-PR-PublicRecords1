...
EXPORT PhoneAttributes_BatchRecords(
	...
	dPortedPhonesSorted := SORT(dPortedPhoneswLineType,acctno,phone,-dt_last_reported,-dt_first_reported,source<>Consts.ATT_LIDB_RealTime,record);	
	Layout_BatchOut tOutputLayout(Layout_BatchRaw L) := TRANSFORM
		...
		SELF.line_type_last_seen				:= CASE(L.source,
																					...
																					Consts.ICONECTIV_SRC	=> L.port_start_dt, 
																					0);
	...
	Layout_BatchOut tRollRecords(Layout_BatchOut L, Layout_BatchOut R) := TRANSFORM
		SELF.event_type 							:= STD.STR.combineWords(set(
																			dedup(
																				sort(ut.WordTokenizer(L.event_type+R.event_type)(char<>''),char)
																			,char),char), '');
		SELF 													:= L;
	END;	

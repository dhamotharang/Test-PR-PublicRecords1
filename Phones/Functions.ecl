...
MODULE
  ...
  EXPORT ParseAccudataCallingName(STRING callingName) := FUNCTION
    ...
    isBlank := callingNameUpper = '';
    isState := LENGTH(lastWord) = 2 AND ut.valid_st(lastWord);
    isValidCallingName := NOT isBlank and NOT isState;

    fname := STD.Str.GetNthWord(callingNameUpper, 1);
    lname := STD.Str.GetNthWord(callingNameUpper, 2);
    result := ROW({fname, lname}, {string fname, string lname});
    RETURN IF(isValidCallingName, result);
  END;
END;

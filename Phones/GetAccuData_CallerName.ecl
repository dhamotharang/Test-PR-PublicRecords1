...
EXPORT GetAccuData_CallerName(DATASET(Phones.Layouts.PhoneAcctno) inPhones,
  ...
  dsAccuDataCNAM := Gateway.AccudataCallerID.Records(uniquePhones, mod_access, gateways);
  ...
  Phones.Layouts.AccuDataCNAM resolveCallerID(iesp.accudata_accuname.t_AccudataCnamResponseEx l) := TRANSFORM
    ...
    callerName	:= IF(l.response.AccudataReport.ErrorMessage <> '',
      Phones.Functions.ParseAccudataCallingName(l.response.AccudataReport.Reply.CallingName));

    SELF.fname := callerName.fname;
    SELF.lname := callerName.lname;

...
EXPORT FromNone(DATASET(Layout_LocationId) h) := MODULE
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.prim_name_Invalid << 0 ) + ( le.st_Invalid << 1 ) + ( le.zip5_Invalid << 3 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    SELF.prim_name_Invalid := (le.ScrubsBits1 >> 0) & 1;
    SELF.st_Invalid := (le.ScrubsBits1 >> 1) & 3;
    SELF.zip5_Invalid := (le.ScrubsBits1 >> 3) & 3;
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,Into(LEFT));
    FromNew := FromNone(InFile).ExpandedInfile;
  EXPORT Changed := JOIN(ExpandedInfile,FromNew, LEFT.rid=RIGHT.rid AND (LEFT.prim_name_Invalid <> RIGHT.prim_name_Invalid OR LEFT.st_Invalid <> RIGHT.st_Invalid OR LEFT.zip5_Invalid <> RIGHT.zip5_Invalid),TRANSFORM(RIGHT));
END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.prim_name_Invalid,le.st_Invalid,le.zip5_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_prim_name(le.prim_name_Invalid),Fields.InvalidMessage_st(le.st_Invalid),Fields.InvalidMessage_zip5(le.zip5_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ,CHOOSE(le.prim_name_Invalid,'LEFTTRIM','UNKNOWN')
          ,CHOOSE(le.st_Invalid,'ALLOW','LENGTH','UNKNOWN')
          ,CHOOSE(le.zip5_Invalid,'ALLOW','LENGTH','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'prim_name','st','zip5','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'DFLT','alpha_st','zip5','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT37.StrType)le.prim_name,(SALT37.StrType)le.st,(SALT37.StrType)le.zip5,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT37.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.rulecnt := CHOOSE(c
          ,le.prim_name_LEFTTRIM_ErrorCount

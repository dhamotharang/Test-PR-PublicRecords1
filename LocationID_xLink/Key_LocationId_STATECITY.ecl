IMPORT SALT37,std;
EXPORT Key_LocationId_STATECITY := MODULE
 
//v_city_name:st:prim_range:prim_name:?:sec_range:+:unit_desig:postdir:addr_suffix:predir
EXPORT KeyName := '~'+KeyPrefix+'::'+'key::LocationId_xLink'+'::'+_CFG.KeyInfix+'::LocId::Refs::STATECITY';
 
EXPORT KeyName_sf := '~'+KeyPrefix+'::'+'key::LocationId_xLink'+'::'+KeySuperfile+'::LocId::Refs::STATECITY';
...
//Scores for various field components
  ...
  INTEGER2 v_city_name_e1_Weight100 := SALT37.Min0(h.v_city_name_weight100 + 100*log(h.v_city_name_cnt/h.v_city_name_e1_cnt)/log(2)); // Precompute edit-distance specificity
...
DataForKey0 := DEDUP(SORT(TABLE(h((v_city_name NOT IN SET(s.nulls_v_city_name,v_city_name) AND v_city_name <> (TYPEOF(v_city_name))''),(st NOT IN SET(s.nulls_st,st) AND st <> (TYPEOF(st))''),(prim_range NOT IN SET(s.nulls_prim_range,prim_range) AND prim_range <> (TYPEOF(prim_range))''),(prim_name NOT IN SET(s.nulls_prim_name,prim_name) AND prim_name <> (TYPEOF(prim_name))'')),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
SHARED DataForKey := DataForKey0;
 
EXPORT Key := INDEX(DataForKey,{DataForKey},{},KeyName_sf); 
EXPORT BuildKey := INDEX(DataForKey,{DataForKey},{},KeyName);  /* HACK01 */
 
EXPORT BuildAll := BUILDINDEX(BuildKey, OVERWRITE); /* HACK02 */
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,LocId,v_city_name,st,prim_range,prim_name,sec_range,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'STATECITY','unit_desig',CntRed_unit_desig*100,CntRed_unit_desig*TSize},{'STATECITY','postdir',CntRed_postdir*100,CntRed_postdir*TSize},{'STATECITY','addr_suffix',CntRed_addr_suffix*100,CntRed_addr_suffix*TSize},{'STATECITY','predir',CntRed_predir*100,CntRed_predir*TSize}],SALT37.ShrinkLayout);
EXPORT CanSearch(Process_LocationID_Layouts.InputLayout le) := le.v_city_name <> (TYPEOF(le.v_city_name))'' AND Fields.InValid_v_city_name((SALT37.StrType)le.v_city_name)=0 AND le.st <> (TYPEOF(le.st))'' AND Fields.InValid_st((SALT37.StrType)le.st)=0 AND le.prim_range <> (TYPEOF(le.prim_range))'' AND Fields.InValid_prim_range((SALT37.StrType)le.prim_range)=0 AND le.prim_name <> (TYPEOF(le.prim_name))'' AND Fields.InValid_prim_name((SALT37.StrType)le.prim_name)=0;
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch(TYPEOF(h.v_city_name) param_v_city_name = (TYPEOF(h.v_city_name))'',TYPEOF(h.v_city_name_len) param_v_city_name_len = (TYPEOF(h.v_city_name_len))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.prim_name_len) param_prim_name_len = (TYPEOF(h.prim_name_len))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'') := 
    STEPPED( LIMIT( Key(
          KEYED(( v_city_name = param_v_city_name AND param_v_city_name <> (TYPEOF(v_city_name))''))
      ...
      AND ( sec_range = (TYPEOF(sec_range))'' OR param_sec_range = (TYPEOF(sec_range))'' OR SALT37.HyphenMatch(sec_range,param_sec_range,1)<=1 )),_CFG.STATECITY_MAXBLOCKLIMIT,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LocId);
 
 
EXPORT ScoredLocIdFetch(TYPEOF(h.v_city_name) param_v_city_name = (TYPEOF(h.v_city_name))'',TYPEOF(h.v_city_name_len) param_v_city_name_len = (TYPEOF(h.v_city_name_len))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.prim_name_len) param_prim_name_len = (TYPEOF(h.prim_name_len))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.unit_desig) param_unit_desig = (TYPEOF(h.unit_desig))'',TYPEOF(h.unit_desig_len) param_unit_desig_len = (TYPEOF(h.unit_desig_len))'',TYPEOF(h.postdir) param_postdir = (TYPEOF(h.postdir))'',TYPEOF(h.postdir_len) param_postdir_len = (TYPEOF(h.postdir_len))'',TYPEOF(h.addr_suffix) param_addr_suffix = (TYPEOF(h.addr_suffix))'',TYPEOF(h.addr_suffix_len) param_addr_suffix_len = (TYPEOF(h.addr_suffix_len))'',TYPEOF(h.predir) param_predir = (TYPEOF(h.predir))'',TYPEOF(h.predir_len) param_predir_len = (TYPEOF(h.predir_len))'') := FUNCTION
  RawData := RawFetch(param_v_city_name,param_v_city_name_len,param_st,param_prim_range,param_prim_name,param_prim_name_len,param_sec_range);
 
  Process_LocationID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    SELF.keys_used := 1 << 1; // Set bitmap for keys used
    SELF.keys_failed := IF(le.LocId = 0, 1 << 1, 0); // Set bitmap for key failed
    SELF.v_city_name_match_code := match_methods(File_LocationId).match_v_city_name(le.v_city_name,param_v_city_name,le.v_city_name_len,param_v_city_name_len,TRUE);
    SELF.v_city_nameWeight := (50+MAP (
           le.v_city_name = param_v_city_name  => le.v_city_name_weight100,
          le.v_city_name = (TYPEOF(le.v_city_name))'' OR param_v_city_name = (TYPEOF(le.v_city_name))'' => 0,
          -1.000*le.v_city_name_weight100))/100; 
    SELF.st_match_code := match_methods(File_LocationId).match_st(le.st,param_st,TRUE);
    ...
    SELF.Weight := IF(le.LocId = 0, 100, MAX(0,SELF.v_city_nameWeight) + MAX(0,SELF.stWeight) + MAX(0,SELF.prim_rangeWeight) + MAX(0,SELF.prim_nameWeight) + MAX(0,SELF.sec_rangeWeight) + MAX(0,SELF.unit_desigWeight) + MAX(0,SELF.postdirWeight) + MAX(0,SELF.addr_suffixWeight) + MAX(0,SELF.predirWeight));
    SELF := le;
  END;
...
EXPORT InputLayout_Batch := RECORD
  ...
  TYPEOF(h.v_city_name_len) v_city_name_len := (TYPEOF(h.v_city_name_len))'';
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_LocationID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.v_city_name_match_code := match_methods(File_LocationId).match_v_city_name(le.v_city_name,ri.v_city_name,le.v_city_name_len,ri.v_city_name_len,TRUE);
    SELF.v_city_nameWeight := (50+MAP (
           le.v_city_name = ri.v_city_name  => le.v_city_name_weight100,
          le.v_city_name = (TYPEOF(le.v_city_name))'' OR ri.v_city_name = (TYPEOF(le.v_city_name))'' => 0,
          -1.000*le.v_city_name_weight100))/100; 
    SELF.st_match_code := match_methods(File_LocationId).match_st(le.st,ri.st,TRUE);
    SELF.stWeight := (50+MAP (
    ...
    SELF := le;
  END;
  Recs0 := Recs(v_city_name <> (TYPEOF(v_city_name))'',st <> (TYPEOF(st))'',prim_range <> (TYPEOF(prim_range))'',prim_name <> (TYPEOF(prim_name))'');
  ...
  J0 := JOIN(Recs1,Key,LEFT.v_city_name = RIGHT.v_city_name
    ...
    ATMOST(LEFT.v_city_name = RIGHT.v_city_name
     AND LEFT.st = RIGHT.st
     AND LEFT.prim_range = RIGHT.prim_range
     AND LEFT.prim_name = RIGHT.prim_name,_CFG.STATECITY_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.v_city_name = RIGHT.v_city_name
     AND LEFT.st = RIGHT.st
     AND LEFT.prim_range = RIGHT.prim_range
     AND LEFT.prim_name = RIGHT.prim_name
     AND ( LEFT.sec_range = (TYPEOF(LEFT.sec_range))'' OR RIGHT.sec_range = (TYPEOF(RIGHT.sec_range))'' OR SALT37.HyphenMatch(LEFT.sec_range,RIGHT.sec_range,1)<=1  ),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.v_city_name = RIGHT.v_city_name
     AND LEFT.st = RIGHT.st
     AND LEFT.prim_range = RIGHT.prim_range
     AND LEFT.prim_name = RIGHT.prim_name,_CFG.STATECITY_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_v_city_name='',Input_st='',Input_prim_range='',Input_prim_name='',Input_sec_range='',Input_unit_desig='',Input_postdir='',Input_addr_suffix='',Input_predir='',output_file,AsIndex='true') := MACRO
...
#IF(#TEXT(Input_v_city_name)<>'' AND #TEXT(Input_st)<>'' AND #TEXT(Input_prim_range)<>'' AND #TEXT(Input_prim_name)<>'')
  #uniquename(trans)
  LocationId_xLink.Key_LocationId_STATECITY.InputLayout_Batch %trans%(InFile le) := TRANSFORM
    SELF.Reference := le.Input_Ref;
    SELF.v_city_name := (TYPEOF(SELF.v_city_name))le.Input_v_city_name;
    SELF.v_city_name_len := LENGTH(TRIM((TYPEOF(SELF.v_city_name))le.Input_v_city_name));
  ...
  output_file := LocationId_xLink.Key_LocationId_STATECITY.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

...
//v_city_name:st:prim_range:prim_name_derived:?:sec_range:predir:postdir:addr_suffix_derived:+:unit_desig:err_stat
...
EXPORT ForeignKeyName_sf(string daliAddress) := '~foreign::' + daliAddress +'::'+KeyPrefix+'::'+'key::LocationId_xLink'+'::'+KeySuperfile+'::LocId::Refs::STATECITY';
...
DataForKey0 := DEDUP(SORT(TABLE(h((v_city_name NOT IN SET(s.nulls_v_city_name,v_city_name) AND v_city_name <> (TYPEOF(v_city_name))''),(st NOT IN SET(s.nulls_st,st) AND st <> (TYPEOF(st))''),(prim_range NOT IN SET(s.nulls_prim_range,prim_range) AND prim_range <> (TYPEOF(prim_range))''),(prim_name_derived NOT IN SET(s.nulls_prim_name_derived,prim_name_derived) AND prim_name_derived <> (TYPEOF(prim_name_derived))'')),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
export DataForKey := DataForKey0;

EXPORT ForeignKey(string daliAddress) := INDEX(DataForKey,{DataForKey},{},ForeignKeyName_sf(daliAddress)); /*Key_LocationId_STATECITY01*/
EXPORT Key        := INDEX(DataForKey,{DataForKey},{},KeyName_sf); /*Key_LocationId_STATECITY01*/
EXPORT BuildKey(dataset(recordof(DataForKey)) inData)   := INDEX(inData,{inData},{},KeyName); /*Key_LocationId_STATECITY01*/
 
EXPORT BuildAll := BUILDINDEX(BuildKey(DataForKey), OVERWRITE); /*Key_LocationId_STATECITY02*/
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,LocId,v_city_name,st,prim_range,prim_name_derived,sec_range,predir,postdir,addr_suffix_derived,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'STATECITY','unit_desig',CntRed_unit_desig*100,CntRed_unit_desig*TSize},{'STATECITY','err_stat',CntRed_err_stat*100,CntRed_err_stat*TSize}],SALT37.ShrinkLayout);
...
EXPORT RawFetch(TYPEOF(h.v_city_name) param_v_city_name = (TYPEOF(h.v_city_name))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.prim_name_derived) param_prim_name_derived = (TYPEOF(h.prim_name_derived))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.predir) param_predir = (TYPEOF(h.predir))'',TYPEOF(h.postdir) param_postdir = (TYPEOF(h.postdir))'',TYPEOF(h.addr_suffix_derived) param_addr_suffix_derived = (TYPEOF(h.addr_suffix_derived))'') := 
    STEPPED( LIMIT( Key(
      ...
      AND KEYED(( addr_suffix_derived = (TYPEOF(addr_suffix_derived))'' OR param_addr_suffix_derived = (TYPEOF(addr_suffix_derived))'' OR addr_suffix_derived = param_addr_suffix_derived ))),_CFG.STATECITY_MAXBLOCKLIMIT,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LocId);
 
 
EXPORT ScoredLocIdFetch(TYPEOF(h.v_city_name) param_v_city_name = (TYPEOF(h.v_city_name))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.prim_name_derived) param_prim_name_derived = (TYPEOF(h.prim_name_derived))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.predir) param_predir = (TYPEOF(h.predir))'',TYPEOF(h.postdir) param_postdir = (TYPEOF(h.postdir))'',TYPEOF(h.addr_suffix_derived) param_addr_suffix_derived = (TYPEOF(h.addr_suffix_derived))'',TYPEOF(h.unit_desig) param_unit_desig = (TYPEOF(h.unit_desig))'',TYPEOF(h.err_stat) param_err_stat = (TYPEOF(h.err_stat))'') := FUNCTION
  RawData := RawFetch(param_v_city_name,param_st,param_prim_range,param_prim_name_derived,param_sec_range,param_predir,param_postdir,param_addr_suffix_derived);
  ...
  Process_LocationID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.v_city_name_match_code := match_methods(File_LocationId).match_v_city_name(le.v_city_name,param_v_city_name,TRUE);
    SELF.v_city_nameWeight := (50+MAP (
          ...
          -0.987*le.v_city_name_weight100))/100; 
    ...
    SELF.Weight := IF(le.LocId = 0, 100, MAX(0,SELF.v_city_nameWeight) + MAX(0,SELF.stWeight) + MAX(0,SELF.prim_rangeWeight) + MAX(0,SELF.prim_name_derivedWeight) + MAX(0,SELF.sec_rangeWeight) + MAX(0,SELF.predirWeight) + MAX(0,SELF.postdirWeight) + MAX(0,SELF.addr_suffix_derivedWeight) + MAX(0,SELF.unit_desigWeight) + MAX(0,SELF.err_statWeight));
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_LocationID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.v_city_name_match_code := match_methods(File_LocationId).match_v_city_name(le.v_city_name,ri.v_city_name,TRUE);
    SELF.v_city_nameWeight := (50+MAP (
    ...
    SELF := le;
  END;
  Recs0 := Recs(v_city_name <> (TYPEOF(v_city_name))'',st <> (TYPEOF(st))'',prim_range <> (TYPEOF(prim_range))'',prim_name_derived <> (TYPEOF(prim_name_derived))'');
  ...
  J0 := JOIN(Recs1,Key,LEFT.v_city_name = RIGHT.v_city_name
    ...
    ATMOST(LEFT.v_city_name = RIGHT.v_city_name
     AND LEFT.st = RIGHT.st
     AND LEFT.prim_range = RIGHT.prim_range
     AND LEFT.prim_name_derived = RIGHT.prim_name_derived,_CFG.STATECITY_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.v_city_name = RIGHT.v_city_name
    ...
    ATMOST(LEFT.v_city_name = RIGHT.v_city_name
     AND LEFT.st = RIGHT.st
     AND LEFT.prim_range = RIGHT.prim_range
     AND LEFT.prim_name_derived = RIGHT.prim_name_derived,_CFG.STATECITY_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_v_city_name='',Input_st='',Input_prim_range='',Input_prim_name_derived='',Input_sec_range='',Input_predir='',Input_postdir='',Input_addr_suffix_derived='',Input_unit_desig='',Input_err_stat='',output_file,AsIndex='true') := MACRO

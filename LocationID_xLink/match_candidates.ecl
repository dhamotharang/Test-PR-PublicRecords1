...
SHARED thin_table := DISTRIBUTE(TABLE(h00,{rid,prim_range,predir,predir_len,prim_name,prim_name_len,addr_suffix,addr_suffix_len,postdir,postdir_len,unit_desig,unit_desig_len,sec_range,v_city_name,v_city_name_len,st,zip5,LocId}),HASH(LocId));
 
  Grpd := GROUP( SORT(
    DISTRIBUTE( TABLE( thin_table, { thin_table, UNSIGNED2 Buddies := 0 }),HASH(LocId)),
    LocId,prim_range,predir,prim_name,addr_suffix,postdir,unit_desig,sec_range,v_city_name,st,zip5, LOCAL),
    LocId,prim_range,predir,prim_name,addr_suffix,postdir,unit_desig,sec_range,v_city_name,st,zip5, LOCAL);
...
EXPORT Layout_Candidates := RECORD // A record to hold weights of each field value
  ...
  UNSIGNED predir_cnt := 0; // Number of instances with this particular field value
  UNSIGNED predir_e1_cnt := 0; // Number of names instances matching this one by edit distance
  ...
  UNSIGNED addr_suffix_cnt := 0; // Number of instances with this particular field value
  UNSIGNED addr_suffix_e1_cnt := 0; // Number of names instances matching this one by edit distance
  ...
  UNSIGNED postdir_cnt := 0; // Number of instances with this particular field value
  UNSIGNED postdir_e1_cnt := 0; // Number of names instances matching this one by edit distance
  ...
  UNSIGNED unit_desig_cnt := 0; // Number of instances with this particular field value
  UNSIGNED unit_desig_e1_cnt := 0; // Number of names instances matching this one by edit distance
  ...
  UNSIGNED v_city_name_e1_cnt := 0; // Number of names instances matching this one by edit distance
...
layout_candidates add_addr_suffix(layout_candidates le,Specificities(ih).addr_suffix_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  SELF.addr_suffix_cnt := ri.cnt;
  SELF.addr_suffix_e1_cnt := ri.e1_cnt;
  ...
  SELF := le;
END;
j9 := JOIN(h1,PULL(Specificities(ih).addr_suffix_values_persisted),LEFT.addr_suffix=RIGHT.addr_suffix,add_addr_suffix(LEFT,RIGHT,TRUE),LOOKUP,FEW,LEFT OUTER);
...
layout_candidates add_predir(layout_candidates le,Specificities(ih).predir_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  SELF.predir_cnt := ri.cnt;
  SELF.predir_e1_cnt := ri.e1_cnt;
...
layout_candidates add_unit_desig(layout_candidates le,Specificities(ih).unit_desig_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  SELF.unit_desig_cnt := ri.cnt;
  SELF.unit_desig_e1_cnt := ri.e1_cnt;
...
layout_candidates add_postdir(layout_candidates le,Specificities(ih).postdir_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  SELF.postdir_cnt := ri.cnt;
  SELF.postdir_e1_cnt := ri.e1_cnt;
...
layout_candidates add_v_city_name(layout_candidates le,Specificities(ih).v_city_name_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF.v_city_name_e1_cnt := ri.e1_cnt;
...
layout_candidates add_sec_range(layout_candidates le,Specificities(ih).sec_range_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT37.MAC_Choose_JoinType(j4,s.nulls_sec_range,Specificities(ih).sec_range_values_persisted,sec_range,sec_range_weight100,add_sec_range,j3);
...
layout_candidates add_zip5(layout_candidates le,Specificities(ih).zip5_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT37.MAC_Choose_JoinType(j2,s.nulls_zip5,Specificities(ih).zip5_values_persisted,zip5,zip5_weight100,add_zip5,j1);
...
TotalWeight := Annotated.prim_name_weight100 + Annotated.zip5_weight100 + Annotated.prim_range_weight100 + Annotated.sec_range_weight100 + Annotated.v_city_name_weight100 + Annotated.postdir_weight100 + Annotated.unit_desig_weight100 + Annotated.predir_weight100 + Annotated.st_weight100 + Annotated.addr_suffix_weight100;

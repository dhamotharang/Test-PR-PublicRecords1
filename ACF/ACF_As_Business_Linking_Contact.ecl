...
EXPORT ACF_As_Business_Linking_Contact (
	  DATASET(ACF.Layout_base) pBase  = ACF.File_ACF_Base) := FUNCTION
  	
	  //CONTACT MAPPING
		BIPV2.Layout_Business_Linking_Full trfMAPBLInterface(ACF.Layout_base l) := TRANSFORM																							 
		...
		from_acf_proj	:= PROJECT(pBase, trfMAPBLInterface(LEFT));
																	
		from_acf_dedp  := DEDUP(SORT(DISTRIBUTE(from_acf_proj,HASH(company_bdid, company_name, company_fein)),RECORD, LOCAL),RECORD, LOCAL);

		RETURN from_acf_dedp;
END;

...
, DATASET(RECORDOF(OPM.in_file)) 	infile 
  ...
  SHARED WithRT MergeData(WithRT le, WithRT ri) := TRANSFORM // Pick the data for the new record
    ...
    SELF.occu_series_desc := ri.occu_series_desc; // Derived(NEW)
    SELF.state_code := ri.state_code; // Derived(NEW)
  ...
  DistIngest0 := DISTRIBUTE(FilesToIngest0, HASH32(employee_name,duty_station
             ,country,state,city,county,agency,agency_sub_element,computation_date,occupational_series,File_Date));
  SortIngest0 := SORT(DistIngest0,employee_name,duty_station
             ,country,state,city,county,agency,agency_sub_element,computation_date,occupational_series,File_Date, __Tpe, record_sid, LOCAL);
  GroupIngest0 := GROUP(SortIngest0,employee_name,duty_station
             ,country,state,city,county,agency,agency_sub_element,computation_date,occupational_series,File_Date, LOCAL, ORDERED, STABLE);
  SHARED AllIngestRecs0 := UNGROUP(ROLLUP(GroupIngest0,TRUE,MergeData(LEFT,RIGHT)));
 
  // Existing Base: combine delta with base file
  DistBase0 := DISTRIBUTE(Base0+Delta0, HASH32(employee_name,duty_station
             ,country,state,city,county,agency,agency_sub_element,computation_date,occupational_series,File_Date));
  SortBase0 := SORT(DistBase0,employee_name,duty_station
             ,country,state,city,county,agency,agency_sub_element,computation_date,occupational_series,File_Date, __Tpe, record_sid, LOCAL);
  GroupBase0 := GROUP(SortBase0,employee_name,duty_station
             ,country,state,city,county,agency,agency_sub_element,computation_date,occupational_series,File_Date, LOCAL, ORDERED, STABLE);
  SHARED AllBaseRecs0 := UNGROUP(ROLLUP(GroupBase0,TRUE,MergeData(LEFT,RIGHT)));
 
  // Everything: combine ingest and base recs
  Sort0 := SORT(AllBaseRecs0+AllIngestRecs0,employee_name,duty_station
             ,country,state,city,county,agency,agency_sub_element,computation_date,occupational_series,File_Date, __Tpe,record_sid,LOCAL);
  Group0 := GROUP(Sort0,employee_name,duty_station
             ,country,state,city,county,agency,agency_sub_element,computation_date,occupational_series,File_Date,LOCAL, ORDERED, STABLE);
  ...
  SHARED AllRecs := ORe+NR1+NR(record_sid<>0) : PERSIST('~temp::seleid::OPM::Ingest_Cache',EXPIRE(OPM.Config.PersistExpire));

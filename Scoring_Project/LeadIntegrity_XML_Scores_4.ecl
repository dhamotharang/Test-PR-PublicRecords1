...
// outputFile := '~tfuerstenberg::out::chase_1878_feb10_F-S_out';
...
// String date := (STRING)((integer)ut.GetDate - 1);

outputFile := '~sghatti::out::nonFcra_LIScores_v4_msn1106_0'  + ut.GetDate ;
...
f := IF(recordsToRun <= 0, DATASET(inputFile, prii_layout, CSV(QUOTE('"'))), CHOOSEN(DATASET(inputFile, prii_layout, CSV(QUOTE('"'))), all));
...
ita_input_temp := IF(runADL, appended_ssns + p_f(ssn<>''), p_f);  // add back the records that already had the SSN on the customer file if TRUE, skip if FALSE
...
layout_slimmed := RECORD
	...
	string name;
	string3 LI_score;
	string2 reason_code_1;
	string2 reason_code_2;
	string2 reason_code_3;
	string2 reason_code_4;
//	string2 rc5;
//	string2 rc6;
	// #if(include_internal_extras)        						/********Commenting out the internal extras fields as ********/
		// RiskProcessing.layout_internal_extras;				/***********they are of no use in common format**************/
	// #end																						/**********Suman Burjukindi*******************************/
END;
...
final := JOIN(ita_input, ita_attributes, LEFT.LeadIntegrityRequest[1].SearchBy.Seq=RIGHT.Result.InputEcho.Seq,
	TRANSFORM(layout_slimmed,
		...
		self.LI_score := (string3)right.Result.Models[1].Scores[1].Value;
		self.reason_code_1 := right.Result.Models[1].Scores[1].WarningCodeIndicators[1].WarningCode;
		self.reason_code_2 := right.Result.Models[1].Scores[1].WarningCodeIndicators[2].WarningCode;
		self.reason_code_3 := right.Result.Models[1].Scores[1].WarningCodeIndicators[3].WarningCode;
		self.reason_code_4 := right.Result.Models[1].Scores[1].WarningCodeIndicators[4].WarningCode;
...
output(final,, outputFile, CSV(HEADING(single), SEPARATOR('|'), QUOTE('"')), OVERWRITE);

EXPORT LeadIntegrity_XML_Scores_4 := final;
...
EXPORT LI_4_XML_despray(filename) := functionmacro
import ut;
...
string score_file := 	'~sghatti::out::nonfcra_liscores_v4_msn1106_0' + date_suffix;													
string attribute_file := '~sghatti::out::nonfcra_liattributes_v4_' +  date_suffix;


LI_v4_XML_scores := dataset(score_file, sghatti.Scores_layouts.nonfcra_liscores_v4_msn1106_0,CSV(heading(1), quote('"'), MAXLENGTH(8192)));


LI_v4_XML_scores_project := PROJECT(LI_v4_XML_scores, TRANSFORM(sghatti.Scores_layouts.nonfcra_liscores_v4_msn1106_0_project,self.LI_score := left.score;
																					self := left;));
																					
////////////////////////////LEAD INTEGRITY version-4 Mode-XML////////////////////////////////////////////////////////////////////////////////////////
LI_scores_v4 := sghatti.Normalized_code_attributes_Scores(LI_v4_XML_scores_project, 'accountnumber', 'LEAD INTEGRITY', 'XML', '4.0', 'NON-FCRA', 'CERT', 'FLAGSHIP', datetime);
...
LI_v4_XML_attributes := dataset(attribute_file, Scoring_Project.Attributes_Layouts.XML_LI_4, CSV(heading(1), quote('"'), MAXLENGTH(8192)));

LI_v4_XML_attributes_project := PROJECT(LI_v4_XML_attributes,TRANSFORM(Scoring_Project.Attributes_Layouts.XML_LI_4_project, self := left;));

LI_XML_attributes_v4 := sghatti.Normalized_code_attributes_Scores(LI_v4_XML_attributes_project, 'accountnumber', 'LEAD INTEGRITY', 'XML', '4.0', 'NON-FCRA', 'CERT', 'FLAGSHIP', datetime);

ALL_SCORES :=  LI_v4_SCORES;

ALL_REASON_CODES := LI_v4_REASON_CODES;

ALL_ATTRIBUTES := LI_XML_attributes_v4;

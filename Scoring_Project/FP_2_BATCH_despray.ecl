...
EXPORT FP_2_BATCH_despray(filename) := functionmacro
...
string attribute_file := '~sghatti::out::fp_attributes_and_scores_batch_v2_' +  date_suffix;


FP_2_BATCH_scores_attributes := dataset(attribute_file,Scoring_Project.Attributes_Layouts.XML_FP_2,CSV(heading(1), quote('"'), MAXLENGTH(8192)));


fp_v2_BATCH_scores := PROJECt(FP_2_BATCH_scores_attributes, TRANSFORM(sghatti.Scores_layouts.nonfcra_fpscores_v2, self := left;));

////////////////////////////FRAUD POINT version-2 Mode-XML////////////////////////////////////////////////////////////////////////////////////////
FP_scores_v2 := sghatti.Normalized_code_attributes_Scores(fp_v2_BATCH_scores, 'accountnumber', 'FRAUD POINT', 'BATCH', '2.0', 'NON-FCRA', 'CERT', 'FLAGSHIP', datetime);
...
FP_2_BATCH_attributes_project := PROJECT(FP_2_BATCH_scores_attributes,TRANSFORM(Scoring_Project.Attributes_Layouts.XML_FP_2_project, self := left;));

FP_BATCH_attributes_v2 := sghatti.Normalized_code_attributes_Scores(FP_2_BATCH_attributes_project, 'accountnumber', 'FRAUD POINT', 'BATCH', '2.0', 'NON-FCRA', 'CERT', 'FLAGSHIP', datetime);

ALL_SCORES :=  FP_v2_SCORES;

ALL_REASON_CODES := FP_v2_REASON_CODES;

ALL_ATTRIBUTES := FP_BATCH_attributes_v2;

EXPORT BocaShell_54_NONFCRA_MACRO(dateyymmdd_curr,dateyymmdd_prev) := functionmacro
...
bs_cert_curr := dataset('~scoringqa::out::nonfcra::bocashell_54_historydate_999999_cert_' + dateyymmdd_curr + '_1', lay, thor);

bs_cert_prev := dataset('~scoringqa::out::nonfcra::bocashell_54_historydate_999999_cert_' + dateyymmdd_prev + '_1', lay,thor);
...
// bs_cert := dataset(ut.foreign_prod +  'bweiner::out::tracking::bocashell50::cert_bs_50_nonfcra_999999_'  + dateyyyymmdd + 'no_edina' , lay, CSV(QUOTE('"')));
...
distwphoneaddr := Scoring_Project.MACRO_disthphonewphone(pj, 'distwphoneaddr', 'iid');	
disthphonewphone := Scoring_Project.MACRO_disthphonewphone(pj, 'disthphonewphone', 'iid');
...
dirs_predir +  dirsaddr_lastscore + dirs_unit_desig + dirs_postdir + cityzipflag + statezipflag + drlcgender + drlcdob +
...
last_nonfelony_criminal_date := Scoring_Project.Macro_Dates_Within_30(pj, 'last_nonfelony_criminal_date', 'bjl', 30);
...
previous_addr_sources_first_seen_date := Scoring_Project.MACRO_Parsing_Numbers(pj, 'previous_addr_sources_first_seen_date', 'address_sources_summary');//waiting for Ben's confirmation
...
current_addr_sources_first_seen_date := Scoring_Project.MACRO_Parsing_Numbers(pj, 'current_addr_sources_first_seen_date', 'address_sources_summary');//waiting for Ben's confirmation
...
input_addr_sources_first_seen_date := Scoring_Project.MACRO_Parsing_Numbers(pj, 'input_addr_sources_first_seen_date', 'address_sources_summary');//waiting for Ben's confirmation
...
n_number := Scoring_Project.MACRO_Populating_Value(pj, 'n_number', 'Aircraft');				
...
PR_count := Scoring_Project.MACRO_Range_1_to_10(pj, 'PR_count', 'Source_Verification');
...
Error_Codes := Scoring_Project.MACRO_Populating_Value(pj, 'Error_Codes', 'Address_Validation');//tricky field...has many error codes
...
total_number_derogs := Scoring_Project.MACRO_Range_1_2_3_4_5(pj, 'total_number_derogs', 'total_number_derogs');
...
Employment + advo_addr_hist2 + advo_addr_hist1 + advo_input_addr + ADL_Shell_Flags + ConsumerFlags +  Professional_License + Student + AirCraft + Accident_Data +
 ...
 res := output(fields_sum,,'~scoring_project::bocashell_54_cert_nonFCRA_stats_' + dateyymmdd_curr, thor, overwrite);

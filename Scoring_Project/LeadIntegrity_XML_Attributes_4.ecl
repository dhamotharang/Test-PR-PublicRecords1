...
outputFile := '~sghatti::out::nonFcra_LIAttributes_v4_'  + ut.GetDate  ;
...
OUTPUT(CHOOSEN(LeadIntegrity_attributes(errorcode<>''), eyeball), NAMED('LeadIntegrity_errors'));
	...
	layout_slimmed slim_v3( LeadIntegrity_input le, LeadIntegrity_attributes ri ) := TRANSFORM
		...
		#if(include_internal_extras)
			self.DID := ri.did; 
			// self.historydate := (string)le.HistoryDateYYYYMM; /*******commented by Suman*****************/
...
EXPORT LIattributes := final;

EXPORT LeadIntegrity_XML_Attributes_4 := 'todo';
...
EXPORT RV_4_XML_despray(filename) := functionmacro
...
String Score_file := '~sghatti::out::fcra_rvscores_v0_' + date_suffix;

string attribute_file := '~sghatti::out::fcra_rvattributes_v4_' + date_suffix;


rv_v4_XML_scores := dataset(Score_file, Scoring_Project.Scores_layouts.fcra_rvscores_v0, CSV(heading(1), quote('"'), MAXLENGTH(8192)));

rv_v4_XML_scores_project := PROJECT(rv_v4_XML_scores,TRANSFORM(Scoring_Project.Scores_layouts.fcra_rvscores_v0_project, self := left;));

RV_scores_v4 := sghatti.Normalized_code_attributes_Scores(rv_v4_XML_scores_project, 'acctno', 'RISK VIEW', 'XML', '4.0', 'FCRA', 'CERT', 'FLAGSHIP', datetime);

///////////////////////FUNCTION MACRO CALL to separate reason codes from scores/////////////////////////////////////////////////////////////////
RV_v4_SCORES_prj := sghatti.Splitting_Scores_ReasonCodes(RV_scores_v4);	

RV_v4_SCORES := RV_v4_SCORES_prj(flag = 1);
RV_v4_REASON_CODES := RV_v4_SCORES_prj(flag = 0);


RV_v4_attributes := dataset(attribute_file,Scoring_Project.Attributes_Layouts.XML_RV_4, CSV(heading(1), quote('"'), MAXLENGTH(8192)));
RV_v4_attributes_proj := PROJECT(RV_v4_attributes, TRANSFORM(Scoring_Project.Attributes_Layouts.XML_RV_4_project, self := left;));

RV_attributes_v4 := sghatti.Normalized_code_attributes_Scores(RV_v4_attributes_proj, 'accountnumber', 'RISK VIEW', 'XML', '4.0', 'FCRA', 'CERT', 'FLAGSHIP', datetime);

ALL_SCORES :=  RV_v4_SCORES;

ALL_REASON_CODES := RV_v4_REASON_CODES;

ALL_ATTRIBUTES := RV_attributes_v4;

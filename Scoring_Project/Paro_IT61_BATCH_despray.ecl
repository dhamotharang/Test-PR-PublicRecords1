...
EXPORT Paro_IT61_BATCH_despray(filename) := functionmacro
...
String Score_file := '~ssghatti::out::paro_it61_batch_score_' + date_suffix;


Paro_IT61_BATCH_scores := dataset(Score_file, Scoring_Project.Scores_layouts.Paro_IT61_XML_layout, CSV(heading(1), quote('"'), MAXLENGTH(8192)));
...
Paro_IT61_BATCH_scores_project := PROJECT(Paro_IT61_BATCH_scores,TRANSFORM(paro_score, self := left;));

Paro_IT61_scores := sghatti.Normalized_code_attributes_Scores(Paro_IT61_BATCH_scores_project, 'acctno', 'PARO-IT61', 'BATCH', '2.0', 'NON-FCRA', 'CERT', 'CUSTOM', datetime);
...
Paro_attributes_proj := PROJECT(Paro_IT61_BATCH_scores, TRANSFORM(Paro_attributes, self := left;));

...
EXPORT Chase_PIO2_BATCH_despray(filename) := functionmacro
...
String Score_file := '~sghatti::out::chase_pi02_batch_cust_rec_' + date_suffix;

string attribute_file := '~sghatti::out::chase_pi02_batch_cust_rec_' + date_suffix;


chase_pio2_BATCH_scores := dataset(Score_file, Scoring_Project.Scores_layouts.chase_pio2_BATCH, CSV(heading(1), quote('"'), MAXLENGTH(8192)));
...
chase_pio2_BATCH_scores_project := PROJECT(chase_pio2_BATCH_scores,TRANSFORM(chase_score, self := left;));

chase_pio2_scores1 := sghatti.Normalized_code_attributes_Scores(chase_pio2_BATCH_scores_project, 'acctno', 'CHASE-PIO2', 'BATCH', '2.0', 'NON-FCRA', 'CERT', 'CUSTOM', datetime);
...
chase_pio2_BATCH_attributes_proj := PROJECT(chase_pio2_BATCH_scores, TRANSFORM(chase_attributes, self := left;));

chase_pio2_attributes := sghatti.Normalized_code_attributes_Scores(chase_pio2_BATCH_attributes_proj, 'acctno', 'CHASE-PIO2', 'BATCH', '2.0', 'NON-FCRA', 'CERT', 'CUSTOM', datetime);

...
EXPORT Paro_IT60_XML_despray(filename) := functionmacro
...
String Score_file := '~sghatti::out::paro_it60_' + date_suffix;


Paro_IT60_XML_scores := dataset(Score_file, Scoring_Project.Scores_layouts.Paro_IT60_XML_layout, CSV(heading(1), quote('"'), MAXLENGTH(8192)));
...
Paro_IT60_XML_scores_project := PROJECT(Paro_IT60_XML_scores,TRANSFORM(paro_score, self := left;));

Paro_IT60_scores := sghatti.Normalized_code_attributes_Scores(Paro_IT60_XML_scores_project, 'acctno', 'PARO-IT60', 'XML', '2.0', 'NON-FCRA', 'CERT', 'CUSTOM', datetime);
...
Paro_attributes_proj := PROJECT(Paro_IT60_XML_scores, TRANSFORM(Paro_attributes, self := left;));

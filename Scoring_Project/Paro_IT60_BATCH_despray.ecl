...
EXPORT Paro_IT60_BATCH_despray(filename) := functionmacro
...
String Score_file := '~sghatti::out::paro_it60_batch_score_' + date_suffix;


Paro_IT60_BATCH_scores := dataset(Score_file, Scoring_Project.Scores_layouts.Paro_IT60_XML_layout, CSV(heading(1), quote('"'), MAXLENGTH(8192)));
...
Paro_IT60_BATCH_scores_project := PROJECT(Paro_IT60_BATCH_scores,TRANSFORM(paro_score, self := left;));

Paro_IT60_scores := sghatti.Normalized_code_attributes_Scores(Paro_IT60_BATCH_scores_project, 'acctno', 'PARO-IT60', 'BATCH', '2.0', 'NON-FCRA', 'CERT', 'CUSTOM', datetime);
...
Paro_attributes_proj := PROJECT(Paro_IT60_BATCH_scores, TRANSFORM(Paro_attributes, self := left;));

Paro_IT60_attributes := sghatti.Normalized_code_attributes_Scores(Paro_attributes_proj, 'acctno', 'PARO-IT60', 'XML', '2.0', 'NON-FCRA', 'CERT', 'CUSTOM', datetime);

ALL_SCORES :=  Paro_IT60_scores;

ALL_ATTRIBUTES := Paro_IT60_attributes;

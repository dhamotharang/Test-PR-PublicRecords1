...
EXPORT Chase_BNK4_XML_despray(filename) := functionmacro
...
String Score_file := '~sghatti::out::chase_bnk4_cust_rec_' + date_suffix;

string attribute_file := '~sghatti::out::chase_bnk4_cust_rec_' + date_suffix;


chase_bnk4_xml := dataset(score_file,Scoring_Project.Scores_layouts.chase_bnk4_xml, CSV(heading(1), quote('"'), MAXLENGTH(8192)));
...
chase_bnk4_xml_scores_project := PROJECT(chase_bnk4_xml,TRANSFORM(chase_score, self := left;));

chase_bnk4_scores1 := sghatti.Normalized_code_attributes_Scores(chase_bnk4_xml_scores_project, 'acctno', 'CHASE-BNK4', 'XML', '4.0', 'NON-FCRA', 'CERT', 'CUSTOM', datetime);

///////////////////////FUNCTION MACRO CALL to separate reason codes from scores/////////////////////////////////////////////////////////////////
chase_bnk4_scores_prj := sghatti.Splitting_Scores_ReasonCodes(chase_bnk4_scores1);	

chase_bnk4_scores := chase_bnk4_scores_prj(flag = 0);
...
chase_bnk4_xml_attributes_project := PROJECT(chase_bnk4_xml,TRANSFORM(chase_attributes, self := left;));

chase_bnk4_attributes := sghatti.Normalized_code_attributes_Scores(chase_bnk4_xml_attributes_project, 'acctno', 'CHASE-BNK4', 'XML', '4.0', 'NON-FCRA', 'CERT', 'CUSTOM', datetime);

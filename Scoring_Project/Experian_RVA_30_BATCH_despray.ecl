...
EXPORT Experian_RVA_30_BATCH_despray(filename) := functionmacro
...
string attribute_file := '~sghatti::out::fcra_experian_rva_30_batch_cust_rec_' + date_suffix;

 experian_rva_30_batch := dataset(attribute_file, Scoring_Project.Attributes_Layouts.BATCH_Experian_rva_30, CSV(heading(1), quote('"'), MAXLENGTH(8192)));

experian_rva_30_batch_project := PROJECT(experian_rva_30_batch, TRANSFORM(Scoring_Project.Attributes_Layouts.BATCH_Experian_rva_30_project, self := left;));

experian_rva_BATCH_attributes_v3 := sghatti.Normalized_code_attributes_Scores(experian_rva_30_batch_project, 'accountnumber', 'EXPERIAN RVA-3', 'BATCH', '3.0', 'FCRA', 'CERT', 'FLAGSHIP', datetime);

ALL_ATTRIBUTES := experian_rva_BATCH_attributes_v3;

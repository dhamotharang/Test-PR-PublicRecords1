...
EXPORT JudgmentsAndLiens_BatchServiceFCRA(useCannedRecs = 'FALSE') :=
  MACRO
    ...
    ds_xml_in := IF( NOT useCannedRecs, ds_xml_in_raw,
    ...
    jl_batch_params := MODULE(PROJECT(batch_params, LiensV2_Services.IParam.batch_params, OPT))
      ...
      // we need to override 1st bit below to make sure records with statements are flagged in Batch_records. Dempsey Hits filtering is done later if needed.
      EXPORT INTEGER8 FFDOptionsMask := inFFDOptionsMask | FFD.Constants.ConsumerOptions.SHOW_CONSUMER_STATEMENTS; 

...
export fn_format_parties (
  ...
  boolean IsFCRA = FALSE,
    ...
    input_companyName_only_E2 := input_company_E2 <> '' AND input_ssn_E2 = '' AND input_fname_E2 = '' AND 
	  	                          input_lname_E2 = '' AND input_did_E2 = '';
	  input_name_only_E2 := (input_fname_E2 <> '' OR input_lname_E2 <> '' OR input_ssn_E2 <> '' OR input_did_E2 <> '') AND
		                      input_company_E2 = '';					
		...
		penaltyDID(inner_params in_mod, deduped_names r) := FUNCTION
			did_ :=	MODULE(PROJECT(gm,AutoStandardI.LIBIN.PenaltyI_DID.full,opt))	
		...
		penaltySSN(inner_params in_mod, deduped_names r) := FUNCTION
			ssn :=	MODULE(PROJECT(gm,AutoStandardI.LIBIN.PenaltyI_SSN.full,opt))
		...
		penaltyBizName(inner_params in_mod, deduped_names r) := FUNCTION
			cname :=	MODULE(PROJECT(gm,AutoStandardI.LIBIN.PenaltyI_Biz_Name.full,opt))

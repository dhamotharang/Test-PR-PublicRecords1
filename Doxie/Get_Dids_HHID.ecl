...
EXPORT dataset(doxie.layout_references_hh) 
       Get_Dids_HHID(boolean forceLocal = true, boolean noFail = false) := function
  ...
  search_code := if (noFail, AutoHeaderV2.Constants.SearchCode.NOFAIL, 0);
	dids := AutoHeaderV2.get_dids (ds_search, search_code, forceLocal, ShowMessages := true);

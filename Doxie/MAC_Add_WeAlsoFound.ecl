...
MACRO
  ...
  integer GetPhonesPlusCount (unsigned6 udid, doxie.IDataAccess mod_access) := FUNCTION
  ...
  inputs tra(inputs l) := TRANSFORM
    ...
    phonesplus_count := GetPhonesPlusCount (udid, mod_access);

    email_count := IF(
                      EXISTS(Email_Data.Key_Did(KEYED(udid=did) AND 
                              ~(isCNSMR AND email_src IN D2C.Constants.EmailRestrictedSources )
                                                                      )),1,0);

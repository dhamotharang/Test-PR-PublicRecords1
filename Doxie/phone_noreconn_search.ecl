...
EXPORT phone_noreconn_search := MACRO
  ...
  boolean Use_QSENTV2 := false : stored('UseQSENTV2');
  ...
  resultOut_w_tzone := doxie.phone_noreconn_records(srchMod, filteredDids, gateways_in);
  ...
  // apply ccpa opt-out logic before calling the gateway
  // use same criteria: phone, addr + ssn to resolve lexid
  progressive_phone.layout_progressive_batch_in xtPVSIn() := TRANSFORM
    SELF.ssn := ssn_value;
    SELF.phoneno := phone_value;
    SELF.prim_range := prange_value;
    SELF.predir := predir_value;
    SELF.prim_name := pname_value;
    SELF.suffix := addr_suffix_value;
    SELF.postdir := postdir_value;
    SELF.sec_range := sec_range_value;
    SELF.p_city_name := city_value;
    SELF.st := state_value;
    SELF.z5 := zip_value_cleaned;
    SELF := [];
  END;
  PVS_gw_in := DATASET([xtPVSIn()]);
  PVS_gw_in_clean := IF(use_PVS and Call_PVS, dx_gateway.parser_qsent_raw.CleanRawRequest(PVS_gw_in, mod_access));

  // do not call the gateway with all criteria
  rtp_mod := MODULE(PROJECT(globalmod, BatchServices.RealTimePhones_Params.params, OPT))
    export string15 phone := phone_value;
    ...
    export boolean UseQSENTV2 := Use_QSENTV2;
  end;

  PVS_gw_rslt_raw := doxie_Raw.RealTimePhones_Raw(rtp_mod, gateways_in,3,, use_PVS and Call_PVS);
  // apply ccpa opt-out logic to clean response
  PVS_gw_rslt_clean := dx_gateway.parser_qsent_raw.CleanRawResponse(PVS_gw_rslt_raw, mod_access, /* retain_lexid */ TRUE);
  PVS_gw_rslt := if(use_PVS and Call_PVS AND exists(PVS_gw_in_clean), PVS_gw_rslt_clean);
  ...
  PVS_sort := sort(project(PVS_GW_w_tzone,
    transform(Doxie_Raw.PhonesPlus_Layouts.t_PhoneplusSearchResponse,
      ...
      self := left)),
    -dt_last_seen,-dt_first_seen,record);

  cmp_res := if(use_PVS and Call_PVS, PVS_sort) &
    project(resultOut_w_tzone,transform(
  ...
  appendDid_ready := project(without_did,
  ...
  Death_source_info :=
    iterate(Death_source_grp,
      transform(out_layout_info,
        self.IsLimitedAccessDMF :=
          if(counter = 1, right.dod != 0 and right.is_src_restricted, left.IsLimitedAccessDMF),
        self :=right));
  ...
  resultsPlusDeath := Project(resultsPlusSSA, transform(out_layout, self := left));
  ...
  prepGovSSN := project(resultsFilterBus,
    TRANSFORM(RecprepGovSSN, self.did := (unsigned)left.did, self:=left));

  _withGovBestSSN := SSNBest_Services.Functions.fetchSSNs_generic(prepGovSSN, ssnBestParams, ssn, did, false);
  withGovBestSSN := PROJECT(_withGovBestSSN,
    TRANSFORM(recordof(resultsFilterBus), self.did := (string)left.did, self:=left));
  ...
  ds_ported_dt_match := join(cmp_res_out, ds_ppmd_key_recs,
    ...
    (unsigned) ut.date_math(left.dt_last_seen, doxie.phone_noreconn_constants.PortingMarginOfError)
  ...
  ds_results_ported_checked_gatewayOnly := ds_results_ported_checked(
    vendor_id = 'TG' OR
    vendor_id ='MN' OR
    typeflag = 'I' OR
    typeflag = 'P');
  ds_results_ported_checked_nonGateway := ds_results_ported_checked(
    vendor_id <> 'TG' AND
    vendor_iD <> 'MN' AND
    typeflag <> 'I' AND
    typeflag <> 'P');
  ...
  ds_results_ported_checkedFilteredDate :=
    if (dt_filterOk, ds_results_ported_checkedFilteredDateTmp & ds_results_ported_checked_gatewayOnly,

...
boolean use_LR  := doxie.compliance.use_LastResort(mod_access.DataPermissionMask) and inMod.IncludeLastResort;
boolean use_qt  := doxie.compliance.use_qsent(mod_access.DataPermissionMask) and ~use_LR;
boolean use_cfm := doxie.compliance.use_confirm(mod_access.DataPermissionMask);
...
h0_tf := doxie.MAC_Get_GLB_DPPA_PhonesPlus(dids, mod_access, true,,
                                  if(inMod.IncludeFullPhonesPlus,0,11),
                                  inMod.CompanyName,,false);
...
//In house QSent
h_qsent := doxie.MAC_Get_GLB_DPPA_Qsent(dids, mod_access, true, , inMod.CompanyName);

h_Last_Resort := PhonesPlus_Services.AppendLR_bydid(dids,mod_access.DataRestrictionMask);
...
Suppress.MAC_Suppress(h3_raw,h3_pulled,mod_access.application_type,Suppress.Constants.LinkTypes.DID,did);

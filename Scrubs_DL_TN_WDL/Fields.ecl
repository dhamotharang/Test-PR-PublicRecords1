...
EXPORT SALT38.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_past_date','invalid_dl_nbr','invalid_action_code','invalid_lname','invalid_county_code','invalid_action_type','invalid_filler_data');
EXPORT FieldTypeNum(SALT38.StrType fn) := CASE(fn,'invalid_past_date' => 1,'invalid_dl_nbr' => 2,'invalid_action_code' => 3,'invalid_lname' => 4,'invalid_county_code' => 5,'invalid_action_type' => 6,'invalid_filler_data' => 7,0);
...
EXPORT MakeFT_invalid_county_code(SALT38.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_county_code(SALT38.StrType s) := WHICH(~Scrubs_DL_TN_WDL.Functions.fn_county_code(s)>0);
EXPORT InValidMessageFT_invalid_county_code(UNSIGNED1 wh) := CHOOSE(wh,SALT38.HygieneErrors.CustomFail('Scrubs_DL_TN_WDL.Functions.fn_county_code'),SALT38.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_action_type(SALT38.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_action_type(SALT38.StrType s) := WHICH(~Scrubs_DL_TN_WDL.Functions.fn_action_type(s)>0);
EXPORT InValidMessageFT_invalid_action_type(UNSIGNED1 wh) := CHOOSE(wh,SALT38.HygieneErrors.CustomFail('Scrubs_DL_TN_WDL.Functions.fn_action_type'),SALT38.HygieneErrors.Good);
...
EXPORT SALT38.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'process_date','dl_number','action_code','event_date','last_name','birthdate','post_date','county_code','action_type','filler');
EXPORT SALT38.StrType FlatName(UNSIGNED2 i) := CHOOSE(i,'process_date','dl_number','action_code','event_date','last_name','birthdate','post_date','county_code','action_type','filler');
EXPORT FieldNum(SALT38.StrType fn) := CASE(fn,'process_date' => 0,'dl_number' => 1,'action_code' => 2,'event_date' => 3,'last_name' => 4,'birthdate' => 5,'post_date' => 6,'county_code' => 7,'action_type' => 8,'filler' => 9,0);
EXPORT SET OF SALT38.StrType FieldRules(UNSIGNED2 i) := CHOOSE(i,['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],[]);
...
EXPORT Make_action_type(SALT38.StrType s0) := MakeFT_invalid_action_type(s0);
EXPORT InValid_action_type(SALT38.StrType s) := InValidFT_invalid_action_type(s);
EXPORT InValidMessage_action_type(UNSIGNED1 wh) := InValidMessageFT_invalid_action_type(wh);
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Diff_action_type := le.action_type <> ri.action_type;
    SELF.Diff_filler := le.filler <> ri.filler;
    SELF.Val := (SALT38.StrType)evaluate(le,pivot_exp);
    SELF.Num_Diffs := 0+ IF( SELF.Diff_process_date,1,0)+ IF( SELF.Diff_dl_number,1,0)+ IF( SELF.Diff_action_code,1,0)+ IF( SELF.Diff_event_date,1,0)+ IF( SELF.Diff_last_name,1,0)+ IF( SELF.Diff_birthdate,1,0)+ IF( SELF.Diff_post_date,1,0)+ IF( SELF.Diff_county_code,1,0)+ IF( SELF.Diff_action_type,1,0)+ IF( SELF.Diff_filler,1,0);
  END;
...
#uniquename(AggRec)
  %AggRec% := RECORD
    ...
    Count_Diff_action_type := COUNT(GROUP,%Closest%.Diff_action_type);

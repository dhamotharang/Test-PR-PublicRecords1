...
EXPORT SALT38.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'process_date','dl_number','action_code','event_date','last_name','birthdate','post_date','county_code','action_type');
EXPORT SALT38.StrType FlatName(UNSIGNED2 i) := CHOOSE(i,'process_date','dl_number','action_code','event_date','last_name','birthdate','post_date','county_code','action_type');
EXPORT FieldNum(SALT38.StrType fn) := CASE(fn,'process_date' => 0,'dl_number' => 1,'action_code' => 2,'event_date' => 3,'last_name' => 4,'birthdate' => 5,'post_date' => 6,'county_code' => 7,'action_type' => 8,0);
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_process_date,1,0)+ IF( SELF.Diff_dl_number,1,0)+ IF( SELF.Diff_action_code,1,0)+ IF( SELF.Diff_event_date,1,0)+ IF( SELF.Diff_last_name,1,0)+ IF( SELF.Diff_birthdate,1,0)+ IF( SELF.Diff_post_date,1,0)+ IF( SELF.Diff_county_code,1,0)+ IF( SELF.Diff_action_type,1,0);
  END;

...
%r% := record
  unsigned xxx_unc;
  unsigned2 pos;
  SALT21.Str4000Type s;
  end;
	
#uniquename(tr)
%r% %tr%(%nif% s_in, unsigned2 c) := transform
  self.pos := c;
	self.s := SALT21.GetNthWord(s_in.instring,c);
	self.xxx_unc := s_in.xxx_unc;
  end;
	
#uniquename(n)
// Split out the words for each wordbag
%n% := normalize(	%nif%, SALT21.WordCount(left.instring), %tr%(left,counter) );
#uniquename(r2)
%r2% := record(%r%)
  integer2 spec;
	end;
...
%ifp% %addw%(%ifp% le, %j% ri) := transform
  self.new_instring := trim(le.new_instring)+' '+trim(ri.s)+' '+(SALT21.StrType)ri.spec;
...
typeof(infile) %sb%(%sf% le) := transform
  self.new_instring := IF(le.instring='','',(SALT21.StrType)le.tot_spec+le.new_instring); // Newstring has space pre-pended by %addw%
  self := le;
  end;

...
EXPORT CalculateStatsAlerts(string SpecPackageName, string SpecKeyNickname, string SpecVersion, decimal6_3 NumRecsThresholdMin, decimal6_3 NumRecsThresholdMax, decimal6_3 UniqueThresholdMin, decimal6_3 UniqueThresholdMax, decimal6_3 PIDThresholdMax, boolean publish=true):= function;
	...
	UniquePersistentRecIDAlerts:=if(exists(SpecificInstancesPersist(Stat_Name='UniquePersistentRecID' and Results='')),false,if(((((real)SpecificInstancesPersist(Stat_Name='UniquePersistentRecID')[1].results)/((real)SpecificInstancesPersist(Stat_Name='UniquePersistentRecID')[1].results))*100) <= PIDThresholdMax,false,true));
	...
	AddUniquePersistentRecIDAlerts:=if(UniquePersistentRecIDAlerts,AddUniqueSeleIDAlerts + 'UniquePersistentRecID Growth: '+ SpecificInstances(Stat_Name='UniquePersistentRecID')[1].results + 'Max Threshold: ' + PIDThresholdMax + '\n',AddUniqueSeleIDAlerts);
	...
	ToNotPublish:=output(AddUniqueFEINAlerts);

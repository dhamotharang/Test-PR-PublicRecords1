...
EXPORT SALT32.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_numericblank');
EXPORT FieldTypeNum(SALT32.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_corp_vendor' => 2,'invalid_state_origin' => 3,'invalid_date' => 4,'invalid_charter_nbr' => 5,'invalid_numericblank' => 6,0);
 
EXPORT MakeFT_invalid_corp_key(SALT32.StrType s0) := FUNCTION
  s1 := SALT32.stringfilter(s0,'-CLPS0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_corp_key(SALT32.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT32.StringFilter(s,'-CLPS0123456789'))),~(LENGTH(TRIM(s)) >= 4));
EXPORT InValidMessageFT_invalid_corp_key(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInChars('-CLPS0123456789'),SALT32.HygieneErrors.NotLength('4..'),SALT32.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_corp_vendor(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_vendor(SALT32.StrType s) := WHICH(((SALT32.StrType) s) NOT IN ['17']);
EXPORT InValidMessageFT_invalid_corp_vendor(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInEnum('17'),SALT32.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_state_origin(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_state_origin(SALT32.StrType s) := WHICH(((SALT32.StrType) s) NOT IN ['IL']);
EXPORT InValidMessageFT_invalid_state_origin(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInEnum('IL'),SALT32.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_charter_nbr(SALT32.StrType s0) := FUNCTION
  s1 := SALT32.stringfilter(s0,'CS0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_charter_nbr(SALT32.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT32.StringFilter(s,'CS0123456789'))),~(LENGTH(TRIM(s)) >= 1));
EXPORT InValidMessageFT_invalid_charter_nbr(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInChars('CS0123456789'),SALT32.HygieneErrors.NotLength('1..'),SALT32.HygieneErrors.Good);
...
EXPORT Make_ar_mailed_dt(SALT32.StrType s0) := MakeFT_invalid_date(s0);
EXPORT InValid_ar_mailed_dt(SALT32.StrType s) := InValidFT_invalid_date(s);
EXPORT InValidMessage_ar_mailed_dt(UNSIGNED1 wh) := InValidMessageFT_invalid_date(wh);
 
EXPORT Make_ar_due_dt(SALT32.StrType s0) := MakeFT_invalid_numericblank(s0);
EXPORT InValid_ar_due_dt(SALT32.StrType s) := InValidFT_invalid_numericblank(s);
EXPORT InValidMessage_ar_due_dt(UNSIGNED1 wh) := InValidMessageFT_invalid_numericblank(wh);
...
EXPORT Make_ar_delinquent_dt(SALT32.StrType s0) := MakeFT_invalid_date(s0);
EXPORT InValid_ar_delinquent_dt(SALT32.StrType s) := InValidFT_invalid_date(s);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT32,Scrubs_Corp2_Mapping_IL_LP_AR;

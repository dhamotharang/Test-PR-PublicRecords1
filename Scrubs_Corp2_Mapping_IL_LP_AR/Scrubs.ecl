...
EXPORT FromNone(DATASET(corp2_mapping.layoutscommon.ar) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.ar_mailed_dt_Invalid := Fields.InValid_ar_mailed_dt((SALT32.StrType)le.ar_mailed_dt);
    SELF.ar_due_dt_Invalid := Fields.InValid_ar_due_dt((SALT32.StrType)le.ar_due_dt);
    SELF.ar_filed_dt_Invalid := Fields.InValid_ar_filed_dt((SALT32.StrType)le.ar_filed_dt);
    SELF.ar_delinquent_dt_Invalid := Fields.InValid_ar_delinquent_dt((SALT32.StrType)le.ar_delinquent_dt);
    SELF := le;
  END;
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.corp_key_Invalid << 0 ) + ( le.corp_vendor_Invalid << 2 ) + ( le.corp_state_origin_Invalid << 3 ) + ( le.corp_process_date_Invalid << 4 ) + ( le.corp_sos_charter_nbr_Invalid << 6 ) + ( le.ar_mailed_dt_Invalid << 8 ) + ( le.ar_due_dt_Invalid << 10 ) + ( le.ar_filed_dt_Invalid << 11 ) + ( le.ar_delinquent_dt_Invalid << 13 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.ar_mailed_dt_Invalid := (le.ScrubsBits1 >> 8) & 3;
    SELF.ar_due_dt_Invalid := (le.ScrubsBits1 >> 10) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.corp_key_Invalid,le.corp_vendor_Invalid,le.corp_state_origin_Invalid,le.corp_process_date_Invalid,le.corp_sos_charter_nbr_Invalid,le.ar_mailed_dt_Invalid,le.ar_due_dt_Invalid,le.ar_filed_dt_Invalid,le.ar_delinquent_dt_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_corp_key(le.corp_key_Invalid),Fields.InvalidMessage_corp_vendor(le.corp_vendor_Invalid),Fields.InvalidMessage_corp_state_origin(le.corp_state_origin_Invalid),Fields.InvalidMessage_corp_process_date(le.corp_process_date_Invalid),Fields.InvalidMessage_corp_sos_charter_nbr(le.corp_sos_charter_nbr_Invalid),Fields.InvalidMessage_ar_mailed_dt(le.ar_mailed_dt_Invalid),Fields.InvalidMessage_ar_due_dt(le.ar_due_dt_Invalid),Fields.InvalidMessage_ar_filed_dt(le.ar_filed_dt_Invalid),Fields.InvalidMessage_ar_delinquent_dt(le.ar_delinquent_dt_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.ar_delinquent_dt_Invalid,'ALLOW','CUSTOM','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_sos_charter_nbr','ar_mailed_dt','ar_due_dt','ar_filed_dt','ar_delinquent_dt','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_date','invalid_numericblank','invalid_date','invalid_date','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT32.StrType)le.corp_key,(SALT32.StrType)le.corp_vendor,(SALT32.StrType)le.corp_state_origin,(SALT32.StrType)le.corp_process_date,(SALT32.StrType)le.corp_sos_charter_nbr,(SALT32.StrType)le.ar_mailed_dt,(SALT32.StrType)le.ar_due_dt,(SALT32.StrType)le.ar_filed_dt,(SALT32.StrType)le.ar_delinquent_dt,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT32.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'ar_delinquent_dt:invalid_date:ALLOW','ar_delinquent_dt:invalid_date:CUSTOM','UNKNOWN');
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_ar_delinquent_dt(1),Fields.InvalidMessage_ar_delinquent_dt(2),'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.ar_delinquent_dt_ALLOW_ErrorCount,le.ar_delinquent_dt_CUSTOM_ErrorCount,0);
      SELF.rulepcnt := 100 * CHOOSE(c
          ...
          ,le.ar_due_dt_ALLOW_ErrorCount
          ,le.ar_filed_dt_ALLOW_ErrorCount,le.ar_filed_dt_CUSTOM_ErrorCount
          ,le.ar_delinquent_dt_ALLOW_ErrorCount,le.ar_delinquent_dt_CUSTOM_ErrorCount,0) / le.TotalCnt + 0.5;
    END;

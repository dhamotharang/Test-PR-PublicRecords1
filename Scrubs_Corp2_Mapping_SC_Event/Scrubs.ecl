IMPORT CORP2_MAPPING,SALT36,UT;
...
EXPORT FromNone(DATASET(Corp2_mapping.LayoutsCommon.Events) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_sos_charter_nbr_Invalid := Fields.InValid_corp_sos_charter_nbr((SALT36.StrType)le.corp_sos_charter_nbr);
    SELF.event_filing_date_Invalid := Fields.InValid_event_filing_date((SALT36.StrType)le.event_filing_date);
    SELF.event_filing_cd_Invalid := Fields.InValid_event_filing_cd((SALT36.StrType)le.event_filing_cd);
    SELF.event_filing_desc_Invalid := Fields.InValid_event_filing_desc((SALT36.StrType)le.event_filing_desc);
    SELF := le;
  END;
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.corp_key_Invalid << 0 ) + ( le.corp_vendor_Invalid << 2 ) + ( le.corp_state_origin_Invalid << 3 ) + ( le.corp_process_date_Invalid << 4 ) + ( le.corp_sos_charter_nbr_Invalid << 6 ) + ( le.event_filing_date_Invalid << 8 ) + ( le.event_filing_cd_Invalid << 10 ) + ( le.event_filing_desc_Invalid << 11 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.event_filing_cd_Invalid := (le.ScrubsBits1 >> 10) & 1;
    SELF.event_filing_desc_Invalid := (le.ScrubsBits1 >> 11) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    event_filing_cd_CUSTOM_ErrorCount := COUNT(GROUP,h.event_filing_cd_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.event_filing_cd_Invalid,'CUSTOM','UNKNOWN')
          ,CHOOSE(le.event_filing_desc_Invalid,'ALLOW','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_sos_charter_nbr','event_filing_date','event_filing_cd','event_filing_desc','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_date','invalid_event_filing_cd','invalid_alphablank','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT36.StrType)le.corp_key,(SALT36.StrType)le.corp_vendor,(SALT36.StrType)le.corp_state_origin,(SALT36.StrType)le.corp_process_date,(SALT36.StrType)le.corp_sos_charter_nbr,(SALT36.StrType)le.event_filing_date,(SALT36.StrType)le.event_filing_cd,(SALT36.StrType)le.event_filing_desc,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT36.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'event_filing_cd:invalid_event_filing_cd:CUSTOM'
          ,'event_filing_desc:invalid_alphablank:ALLOW','UNKNOWN');
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.event_filing_cd_CUSTOM_ErrorCount

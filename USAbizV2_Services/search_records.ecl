...
ids := USAbizV2_services.Get_ids (false, , IsDeepDive, is_CompSearchL); //this is layout [layouts.id + isDeepDive]

rec_ids := project (ids, layouts.id);

//NOTE: no ssn_mask_value; deadco doesn't have SSNs
ds_recs := USAbizV2_services.raw.SEARCH_VIEW.by_id (rec_ids);
...
rsrt := sort (rdd (penalt <= pt), if (isDeepDive, 1, 0), penalt, COMPANY_NAME, -process_date);
//rsrt := sort (rdd, if (isDeepDive, 1, 0), penalt, COMPANY_NAME);

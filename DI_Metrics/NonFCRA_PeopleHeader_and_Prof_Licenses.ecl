...
EXPORT NonFCRA_PeopleHeader_and_Prof_Licenses(string pHostname, string pTarget, string pContact ='\' \'') := function
...
//DID count overall:
Key_DID_PHDR_dedup := DEDUP(sort(distribute(Key_DID_PHDR(did <> 0), hash(did)),did, local, skew(1.0)), did, all);
...
Key_DID_PHDR_dedup_vendor := DEDUP(sort(distribute(Key_DID_PHDR(did <> 0), hash(src, did)),src, did, local,skew(1.0)),src, did, all,local);
...
PHDR_key_src_first_reported := DEDUP(sort(distribute(Key_DID_PHDR(did <> 0), hash(did)), src,did, -dt_vendor_first_reported, local), src,did,dt_vendor_first_reported, all,local)((integer) dt_vendor_first_reported >= since_date);
PHDR_key_src_last_reported 	:= DEDUP(sort(distribute(Key_DID_PHDR(did <> 0), hash(did)), src,did, -dt_vendor_last_reported, local), src,did,dt_vendor_last_reported, all,local)((integer) dt_vendor_last_reported >= since_date);
...
tbl_src_dates_first_reported := TABLE(PHDR_key_src_first_reported, rec_src_dates_first_reported, src,dt_vendor_first_reported, few);	
...
tbl_src_dates_last_reported := TABLE(PHDR_key_src_last_reported, rec_src_dates_last_reported, src,dt_vendor_last_reported, few);	


PHDR_key_src_first_seen := DEDUP(sort(distribute(Key_DID_PHDR(did <> 0), hash(did)), src,did, -dt_first_seen, local), src,did,dt_first_seen, all,local)((integer) dt_first_seen >= since_date);
PHDR_key_src_last_seen 	:= DEDUP(sort(distribute(Key_DID_PHDR(did <> 0), hash(did)), src,did, -dt_last_seen, local), src,did,dt_last_seen, all,local)((integer) dt_last_seen >= since_date);
...
tbl_src_dates_first_seen := TABLE(PHDR_key_src_first_seen, rec_src_dates_first_seen, src,dt_first_seen, few);	
...
tbl_src_dates_last_seen := TABLE(PHDR_key_src_last_seen, rec_src_dates_last_seen, src,dt_last_seen, few);	

...
export NonFCRA_Liens_Judgments(string pHostname, string pTarget, string pContact ='\' \'') := function

filedate := (STRING8)Std.Date.Today();
rpt_yyyymmdd := filedate[1..8];

//thor_data400::key::liensv2::fcra::20141219::main::tmsid.rmsid
Key_LiensV2_main := LiensV2.key_liens_main_ID;
...
Key_LiensV2_2010_filings := DEDUP(sort(distribute(Key_LiensV2_2010, hash(tmsid)), filing_period,release_period, filing_jurisdiction, tmsid,rmsid,filing_type_name, local), filing_period,filing_jurisdiction, tmsid,filing_type_name, all,local);
tbl_Key_LiensV2_2010_filings := TABLE(Key_LiensV2_2010_filings, {filing_period,release_period,filing_jurisdiction, filing_type_name, eviction, tmsid_count := count(group)}, filing_period,release_period,filing_jurisdiction, filing_type_name, eviction, few);
...
email_alert := SEQUENTIAL(
					...
					,despray_liens_tbl):
					Success(FileServices.SendEmail(_control.MyInfo.EmailAddressNotify + pContact, 'NonFCRA Group: NonFCRA_Liens_Judgements Build Succeeded', workunit + ': Build complete.' + filedate)),
					Failure(FileServices.SendEmail(_control.MyInfo.EmailAddressNotify + pContact, 'NonFCRA Group: NonFCRA_Liens_Judgements Build Failed', workunit + filedate + '\n' + FAILMESSAGE)
					);
return email_alert;

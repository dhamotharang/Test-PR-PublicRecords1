...
export InquiryRpts_bwrNonFCRAInq50(string pHostname, string pTarget, string pContact ='\' \'', STRING today = (STRING8)STD.Date.Today()) := function
...
key := PULL(Inquiry_AccLogs.Key_Inquiry_DID);
...
nonfcra_inquiries := project(Inquiry_AccLogs.Key_Inquiry_DID((unsigned)search_info.datetime[1..4]>=2008), transform_raw(left));
nonfcra_inquiries_update := project(Inquiry_AccLogs.Key_Inquiry_DID_Update((unsigned)search_info.datetime[1..4]>=2008), transform_raw(left));
nonfcra_inquiries_all := nonfcra_inquiries + nonfcra_inquiries_update;

deduped_inquiries := dedup(sort(nonfcra_inquiries_all, person_q.Appended_ADL, transaction_id, sequence_number, local), 
														person_q.Appended_ADL, transaction_id, sequence_number, local);

InqADLs := table(deduped_inquiries, {log_month := logdate[1..6], 
														...
														PrepaidCards_inquiries := count(group,PrepaidCards),
														RetailPayments_inquiries := count(group,RetailPayments),
														Utilities_inquiries := count(group,Utilities),
														QuizProvider_inquiries := count(group,QuizProvider),
														StudentLoan_inquiries := count(group, StudentLoan),
														Other_inquiries := count(group, Other)
														}, logdate[1..6], person_q.appended_ADL );
														
//output(sort(InqADLs, -total), named('InqADLs'));
...
InqADLsByMonth := table(InqADLs, {
														...
														PrepaidCards := count(group,PrepaidCards_inquiries>0),
														RetailPayments := count(group,RetailPayments_inquiries>0),
														Utilities := count(group,Utilities_inquiries>0),
														QuizProvider := count(group,QuizProvider_inquiries>0),
...
InqByMonth := table(InqADLs, {
														...
														PrepaidCards := sum(group,PrepaidCards_inquiries),
														RetailPayments := sum(group,RetailPayments_inquiries),
														Utilities := sum(group,Utilities_inquiries),
														QuizProvider := sum(group,QuizProvider_inquiries),
...
despray_InqADLsByMonth := STD.File.DeSpray('~thor_data400::data_insight::data_metrics::tbl_InquiryRpts_NonFCRAInq50_InqADLsByMonth_'+ filedate +'.csv',
																		pHostname, 
																		pTarget+ '/tbl_InquiryRpts_NonFCRAInq50_InqADLsByMonth_'+ filedate +'.csv'
																		,,,,true);

despray_InqByMonth := STD.File.DeSpray('~thor_data400::data_insight::data_metrics::tbl_InquiryRpts_NonFCRAInq50_InqByMonth_'+ filedate +'.csv',
																		pHostname, 
																		pTarget+ '/tbl_InquiryRpts_NonFCRAInq50_InqByMonth_'+ filedate +'.csv'
																		,,,,true);

//if everything in the Sequential statement runs, it will send the Success email, else it will send the Failure email
email_alert := SEQUENTIAL(
					 output(srt_InqADLsByMonth,,'~thor_data400::data_insight::data_metrics::tbl_InquiryRpts_NonFCRAInq50_InqADLsByMonth_'+ filedate +'.csv', csv(heading(single), separator('|'),terminator('\r\n'),quote('\"')), overwrite, expire(10))
					,output(srt_InqByMonth,,'~thor_data400::data_insight::data_metrics::tbl_InquiryRpts_NonFCRAInq50_InqByMonth_'+ filedate +'.csv', csv(heading(single), separator('|'),terminator('\r\n'),quote('\"')), overwrite, expire(10))					
					,despray_InqADLsByMonth
					,despray_InqByMonth):
					Success(FileServices.SendEmail(pContact, 'InquiryRpts Group: InquiryRpts_bwrNonFCRAInq50 Build Succeeded', workunit + ': Build complete.' + filedate)),
					Failure(FileServices.SendEmail(pContact, 'InquiryRpts Group: InquiryRpts_bwrNonFCRAInq50 Build Failed', workunit + filedate + '\n' + FAILMESSAGE)
													);
return email_alert;

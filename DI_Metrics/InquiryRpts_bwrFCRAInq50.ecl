...
export InquiryRpts_bwrFCRAInq50(string pHostname, string pTarget, string pContact ='\' \'', STRING today = (STRING8)STD.Date.Today()) := function
...
s_fcra_inquiries := sort(fcra_inquiries,person_q.appended_adl, isAuto_temp, isMortgage_temp, isHighRiskCredit_temp, -logdate, skew(1.0));
...
InqADLs := table(dedup_fcra_inq, {log_month := logdate[1..6], 
...
despray_InqADLsByMonth := STD.File.DeSpray('~thor_data400::data_insight::data_metrics::tbl_InquiryRpts_FCRAInq50_InqADLsByMonth_'+ filedate +'.csv',
																		pHostname, 
																		pTarget+ '/tbl_InquiryRpts_FCRAInq50_InqADLsByMonth_'+ filedate +'.csv'
																		,,,,true);

despray_InqByMonth := STD.File.DeSpray('~thor_data400::data_insight::data_metrics::tbl_InquiryRpts_FCRAInq50_InqByMonth_'+ filedate +'.csv',
																		pHostname, 
																		pTarget+ '/tbl_InquiryRpts_FCRAInq50_InqByMonth_'+ filedate +'.csv'
																		,,,,true);

//if everything in the Sequential statement runs, it will send the Success email, else it will send the Failure email
email_alert := SEQUENTIAL(
					 output(srt_InqADLsByMonth,,'~thor_data400::data_insight::data_metrics::tbl_InquiryRpts_FCRAInq50_InqADLsByMonth_'+ filedate +'.csv', csv(heading(single), separator('|'),terminator('\r\n'),quote('\"')), overwrite, expire(10))
					,output(srt_InqByMonth,,'~thor_data400::data_insight::data_metrics::tbl_InquiryRpts_FCRAInq50_InqByMonth_'+ filedate +'.csv', csv(heading(single), separator('|'),terminator('\r\n'),quote('\"')), overwrite, expire(10))					
					,despray_InqADLsByMonth
					,despray_InqByMonth):
					Success(FileServices.SendEmail(pContact, 'InquiryRpts Group: InquiryRpts_bwrFCRAInq50 Build Succeeded', workunit + ': Build complete.' + filedate)),
					Failure(FileServices.SendEmail(pContact, 'InquiryRpts Group: InquiryRpts_bwrFCRAInq50 Build Failed', workunit + filedate + '\n' + FAILMESSAGE)
													);
return email_alert;

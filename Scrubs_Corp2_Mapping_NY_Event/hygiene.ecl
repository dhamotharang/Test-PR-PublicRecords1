...
SALT32.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT32.StrType)le.corp_key),TRIM((SALT32.StrType)le.corp_sos_charter_nbr),TRIM((SALT32.StrType)le.event_filing_cd)));
  SELF.FldNo := C;
END;
...
SALT32.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 3);
  SELF.FldNo2 := 1 + (C % 3);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT32.StrType)le.corp_key),TRIM((SALT32.StrType)le.corp_sos_charter_nbr),TRIM((SALT32.StrType)le.event_filing_cd)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT32.StrType)le.corp_key),TRIM((SALT32.StrType)le.corp_sos_charter_nbr),TRIM((SALT32.StrType)le.event_filing_cd)));
  END;
...
SHARED FldIds := DATASET([{1,'corp_key'}
      ...
      ,{3,'event_filing_cd'}],SALT32.MAC_Character_Counts.Field_Identification);

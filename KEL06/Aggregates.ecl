...
EXPORT Aggregates := MODULE
    ...
    EXPORT CalcRangeNN(inset, field) := FUNCTIONMACRO
        LOCAL __s := __T(inset);
        RETURN IF(NOT __NN(inset), __N(TYPEOF(__T(field))), __OP2(Kel.Aggregates.AggN(MAX, __s, field, false),-,Kel.Aggregates.AggN(MIN, __s, field, false)));
    ENDMACRO;
    ...
    EXPORT AggNG(agg, field, minmax) := FUNCTIONMACRO
      ...
      #IF(minmax!=0)
        LOCAL __val := IF(__empty, 0, Kel.Aggregates.MinMaxExp(agg, field, minmax));
      #ELSE
    ...
    EXPORT WAveNZNG(field) := FUNCTIONMACRO
        ...
        LOCAL __den := SUM(GROUP, __RecordCount, __NN(field), __T(field)!=0);
        LOCAL __val := __num / __den;
        RETURN { TYPEOF(__val) v := __val, Kel.Typ.flags f := __flag };
    ENDMACRO;
    ...
    EXPORT RankingPrepareGrouped(inset, groupFields, rankFields, nullable, weighted) := FUNCTIONMACRO
      ...
      #IF(groupFields)
        #APPEND(groupSpec,  ',')
      #END
      LOCAL __Mins := TABLE(__Counted, { #EXPAND(%'groupSpec'%) number, KEL.typ.int minpos := MIN(GROUP, pos),
    ...
    EXPORT RankingGroupedSmall(dIn, rankFields, groupFields, lWeights, lResult, tWeights, tNormalize, nullable, weighted) := FUNCTIONMACRO
      LOCAL __Numbered := KEL.Aggregates.RankingPrepareGrouped(dIn, groupFields, rankFields, nullable, weighted);
      LOCAL __Weights := KEL.Aggregates.RankingSelect(__Numbered, lWeights, tWeights, nullable);
    ...
    EXPORT RankingGroupedLarge(dIn, rankFields, groupFields, lWeights, lResult, tWeights, tNormalize, nullable) := FUNCTIONMACRO
      lResult RollGroup(RECORDOF(dIn) r, DATASET(RECORDOF(dIn)) g) := TRANSFORM
        ...
        #IF(nullable)
            Numbered := KEL.Aggregates.RankingPrepareGrouped(g, '', rankFields, TRUE);
        #ELSE
    ...
    // about the dataset.
    EXPORT RankingGrouped(dIn, rankFields, groupFields, lWeights, lResult, tWeights, tNormalize, nullable) := FUNCTIONMACRO
      ...
      LOCAL __LargeResult := KEL.Aggregates.RankingGroupedLarge(dIn, rankFields, groupFields, lWeights, lResult, tWeights, tNormalize, nullable);
      LOCAL __SmallResult := KEL.Aggregates.RankingGroupedSmall(dIn, rankFields, groupFields, lWeights, lResult, tWeights, tNormalize, nullable);

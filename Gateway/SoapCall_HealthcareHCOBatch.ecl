...
EXPORT SoapCall_HealthcareHCOBatch(DATASET(iesp.healthcare_orgaccurint.t_HealthCareAccurintHCOBatchRequestEx) batch_in,
															string40 servicename = Gateway.Constants.ServiceName.IsHCHCOBatch) := FUNCTION

  queryName := 'healthcare_accurint.searchservicehcobatch';
	out_rec := iesp.healthcare_orgaccurint.t_HealthCareAccurintHCOSearchResponse;
  dGWCfg := Gateway.Configuration.Get();
  dHCPGWCfg := dGWCfg(Gateway.Configuration.IsHCHCOBatch(servicename));
  ...
  out_rec SetError (batch_in inn) := TRANSFORM
    Self._Header.Exceptions := dataset ([transform (iesp.share.t_WsException, 
                                                    Self.Source := 'SearchServiceHCOBatch',
                                                    Self.Code := FAILCODE,
                                                    Self.Location := '',
  ...
  sc_results := map(count(batch_in)>0 and serviceURL <>'' => SOAPCALL (batch_in, 
																																				...
																																				{batch_in},
																																				dataset (out_rec), 
																																				XPATH('healthcare_accurint.searchservicehcobatchResponse/Results/Result/Dataset[@name=\'Results\']/Row'),

import ML;
EXPORT mac_compute_suspect_provider_address_score (Infile, 
																									 LNPID = '',
																									 PrimaryRange = '',
																									 PreDirectional = '',
																									 PrimaryName = '',
    ...
    LOCAL ScoreLayout ScoreXForm(RECORDOF(Infile) L) := TRANSFORM
        ...
        evalStudentNPI 										:= ComputeSuspectProviderAddressScore.ScoringAlgorithms.EvalStudentNPI								(L.TotalNPICount, L.StudentNPICount, StudentNPISwitch, StudentNPIScore);	
	        ...
	        // Score each element
				...
				/* Fix this line */
				evalReinstatedSanctionRevocation 	:= ComputeSuspectProviderAddressScore.ScoringAlgorithms.EvalPastLicenseRevocation			(L.PastLicenseRevocation, PastLicenseRevocationSwitch, PastLicenseRevocationScore, L.LicenseRevocationReinstatedState, L.OPMExclusionReinstatedDate);				
				evalLicenseExpired 								:= ComputeSuspectProviderAddressScore.ScoringAlgorithms.EvalLicenseExpired 						(L.LicenseExpiredFlag, LicenseExpiredSwitch, ExpiredLicenseScore, L.ExpiredLicenseNumberDate, L.ExpiredLicenseNumber, L.ExpiredLicenseState, L.TotalChargeAmount);		
				evalDeaExpired 										:= ComputeSuspectProviderAddressScore.ScoringAlgorithms.EvalDEAExpired								(L.DEAExpiredFlag, DEAExpiredSwitch, DEAExpiredScore, L.ExpiredDEANumber, L.ExpiredDEADate, L.TotalChargeAmount);												

				evalDeceased 											:= ComputeSuspectProviderAddressScore.ScoringAlgorithms.EvalDeceased									(L.DeceasedFlag, L.DateofDeath, DeceasedSwitch, DeceasedScore , L.TotalChargeAmount, L.LNPID);												
				...
				evalLargePatientGroups						:= ComputeSuspectProviderAddressScore.ScoringAlgorithms.EvalLargePatientGroups				(L.LargePatientGroupFlag, LargePatientGroupSwitch, LargePatientGroupScore, L.PatientCount, L.LargePatientAddress);	
				evalLicenseInactive 							:= ComputeSuspectProviderAddressScore.ScoringAlgorithms.EvalLicenseInactive						(L.LicenseInactiveFlag, LicenseInactiveSwitch, LicenseInactiveScore, L.InactiveLicenseState, L.TotalChargeAmount);		
				evalNPIDeactivated	 							:= ComputeSuspectProviderAddressScore.ScoringAlgorithms.EvalNPIDeactivated						(L.NPIDeactivatedFlag, NPIDeactivatedSwitch, NPIDeactivatedScore, L.NPIDeactivatedDate, L.TotalChargeAmount);			

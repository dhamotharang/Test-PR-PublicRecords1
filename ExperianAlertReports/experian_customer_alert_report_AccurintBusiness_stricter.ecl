...
export experian_customer_alert_report_AccurintBusiness_stricter(string filedate) := 
function
#workunit('name','Accurint Business Company Experian Alerts');
//contains stricter code( no matches based on telephone)
...
// dJoin_MainPhone1			:=	join(dCompanySlimClean,dAlertListClean,
									 // left.MAIN_PHONE_VOICE		=	right.phone
...
// dJoin_BillPhone1			:=	join(dCompanySlimClean,dAlertListClean,
									 // left.BILLING_PHONE_VOICE		=	right.phone

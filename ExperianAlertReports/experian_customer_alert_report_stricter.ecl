...
export experian_customer_alert_report_stricter(string filedate) := function
#workunit('name','Accurint Company Experian Alerts');
//contains stricter logic ( no matches based on telephone) 

lAlertListDate		:=	filedate;
lCustomerListDate	:=	filedate;
...
dAllJoins	:=	dJoin_Company_Address
			...
			+ dJoin_DBAComp_Address
			+ dJoin_DBACompany_MainAddress
			+ dJoin_DBACompany_BillAddress
  ...
  transform
	...
	self.MatchCount					:=	(string2)length(lib_stringlib.StringLib.stringfilter(self.Company_Address
																						   ...
																						   + self.BillContact_BillAddress
																							 + self.DBAAssumed_Address
																							 + self.DBAAssumed_MainAddress
																							 + self.DBAAssumed_BillAddress
...
dCompanyRolledUpSortFinal := output(dCompanyRolledUpSort,all,named('All_With_Match_Flags'));
return dCompanyRolledUpSortFinal;
end;
/*

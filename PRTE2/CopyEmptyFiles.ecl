//*EXPORT CopyEmptyFiles := 'todo';
EXPORT   CopyEmptyFiles  (string filedate)   :=              FUNCTION
r1 := RECORD
  string12 geo12;
 ...
 END;
d1:=dataset([],r1);
i1:=index(d1,{geo12},{d1},'~prte::key::foreclosure::' + filedate + '::index_geo12');
...
r2  := RECORD
 ...
 END;
d2:=dataset([],r2);
i2:=index(d2,{geo11},{d2},'~prte::key::foreclosure::' + filedate + '::index_geo11');
...
d3:=dataset([],r3);
i3:=index(d3,{fips},{d3},'~prte::key::foreclosure::' + filedate + '::index_fips');
...
END;
d4:=dataset([],r4);
i4:=index(d4,{situs1_zip,situs1_prim_range,situs1_prim_name,situs1_addr_suffix,situs1_predir},{d4},'~prte::key::foreclosure::' + filedate + '::address');

IMPORT ut,SALT34;
IMPORT Scrubs_Corp2_Mapping_HI_Event,Scrubs; // Import modules for FieldTypes attribute definitions
EXPORT Fields := MODULE
 
// Processing for each FieldType
EXPORT SALT34.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_charter_nbr','invalid_corp_vendor','invalid_state_origin','invalid_date_type_cd','invalid_date_type_desc','invalid_mandatory','invalid_date','invalid_numeric');
EXPORT FieldTypeNum(SALT34.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_charter_nbr' => 2,'invalid_corp_vendor' => 3,'invalid_state_origin' => 4,'invalid_date_type_cd' => 5,'invalid_date_type_desc' => 6,'invalid_mandatory' => 7,'invalid_date' => 8,'invalid_numeric' => 9,0);
...
EXPORT MakeFT_invalid_corp_key(SALT34.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_invalid_corp_key(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,'-ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'))),~Scrubs_Corp2_Mapping_HI_Event.Functions.invalid_corp_key(s)>0,~(LENGTH(TRIM(s)) >= 4));
EXPORT InValidMessageFT_invalid_corp_key(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars('-ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'),SALT34.HygieneErrors.CustomFail('Scrubs_Corp2_Mapping_HI_Event.Functions.invalid_corp_key'),SALT34.HygieneErrors.NotLength('4..'),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_charter_nbr(SALT34.StrType s0) := FUNCTION
  s1 := SALT34.stringfilter(s0,' -ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_charter_nbr(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,' -ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'))),~Scrubs_Corp2_Mapping_HI_Event.Functions.invalid_corp_sos_charter_nbr(s)>0,~(LENGTH(TRIM(s)) >= 1));
EXPORT InValidMessageFT_invalid_charter_nbr(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars(' -ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'),SALT34.HygieneErrors.CustomFail('Scrubs_Corp2_Mapping_HI_Event.Functions.invalid_corp_sos_charter_nbr'),SALT34.HygieneErrors.NotLength('1..'),SALT34.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_date_type_cd(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_date_type_cd(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['EFF']);
EXPORT InValidMessageFT_invalid_date_type_cd(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('EFF'),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_date_type_desc(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_date_type_desc(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['EFFECTIVE']);
EXPORT InValidMessageFT_invalid_date_type_desc(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('EFFECTIVE'),SALT34.HygieneErrors.Good);
...
EXPORT Make_event_filing_date(SALT34.StrType s0) := MakeFT_invalid_date(s0);
EXPORT InValid_event_filing_date(SALT34.StrType s) := InValidFT_invalid_date(s);
...
EXPORT Make_event_filing_cd(SALT34.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_event_filing_cd(SALT34.StrType s) := InValidFT_invalid_numeric(s);
EXPORT InValidMessage_event_filing_cd(UNSIGNED1 wh) := InValidMessageFT_invalid_numeric(wh);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT34,Scrubs_Corp2_Mapping_HI_Event;

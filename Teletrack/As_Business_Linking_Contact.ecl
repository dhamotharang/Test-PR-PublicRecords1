...
Export As_Business_Linking_Contact (DATASET(teletrack.layouts.base) pBase = teletrack.Files().base.qa) := FUNCTION

	  //CONTACT MAPPING
		BIPV2.Layout_Business_Linking_Full trfMAPBLInterface(teletrack.layouts.base l) := TRANSFORM
        SELF.source                      	:= 	mdr.sourceTools.src_Teletrack; 
				...
				SELF.company_phone              	:= 	l.clean_wphone;
				SELF.phone_type                 	:= 	'';
				...
				SELF.contact_email              	:=	'';
				SELF.contact_phone              	:= 	l.clean_hphone;
				SELF 							   							:= 	l;
				SELF 							   							:= [];
		end;																										

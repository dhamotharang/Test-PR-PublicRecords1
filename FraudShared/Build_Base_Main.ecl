...
module
  ...
  // Append Industry type description 
	...
	Combined        := join (addUID , Files().Input.MBSFdnIndType.Sprayed(status = 1), 
																		   ...
																		   self.classification_source.Industry_segment                     := if(platform.source = 'FDN' and left.source not in ['ERIE','GLB5'],StringLib.StringToUppercase(right.description),left.classification_source.Industry_segment), 

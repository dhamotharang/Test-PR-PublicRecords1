...
shared 	MsIndTypIncl_ds		 			:= Files().Input.mbsfdnmasteridindtypeinclusion.sprayed;
				...
				gcid_excl_proj     			:= project(newGCExcl_ds(exclusion_id_type='GCID'), transform(FraudShared.Layouts.Input.FDNMasterID,
				...
				gcid_incl_proj		 			:= Project(MsIndTypIncl_ds(inclusion_type ='GCID'),transform(FraudShared.Layouts.Input.FDNMasterID,
																									self.gc_id               := left.inclusion_id;
																									self.gc_cc_hh_id         := left.inclusion_id;
																									self.id_type             := left.inclusion_type;
																									self.FdnmasterId_type    := left.inclusion_type;
																									self.FdnmasterId         := hashmd5(left.inclusion_id,trim(left.inclusion_type,left,right));
																									self                     := left;
																									self                     := []));

				ccid_proj          			:= project(ccid_ds(gc_id<>0),transform(FraudShared.Layouts.Input.FDNMasterID, 
				...
				hhid_proj          			:= project(hhid_ds(gc_id<>0),transform(FraudShared.Layouts.Input.FDNMasterID,
       ...
       FdnmasterId_Fab     			:= gcid_excl_proj + gcid_incl_proj + ccid_proj + hhid_proj;
...
shared jn_mastID_Excl      			:= join(FDNMasterID, newGCExcl_ds(status=1), left.gc_cc_hh_id = right.exclusion_id and left.FDNMasterID_Type = right.exclusion_id_type,
																																				transform(FraudShared.Layouts.Input.MbsFdnMasterIdExcl, 
...
shared jn_mastIDIndTyp_Incl    	:= join(FDNMasterID, MsIndTypIncl_ds(status=1), left.gc_cc_hh_id = right.inclusion_id and left.FDNMasterID_Type = right.inclusion_type,
																																				transform(FraudShared.Layouts.Input.MbsFdnMasterIDIndTypeIncl, 
																																									self.FdnmasterId :=left.FdnmasterId,
																																									self :=right),right outer);

...
export BWR_buildautokeyB_Bid(string pversion) := 
	function

d1       := distribute(project(corp2.files('').AID.Corp.built, 
																 transform(Corp2.Layout_Corporate_Direct_Corp_Base, 
																					 self.bdid := left.bid,
																					 self := left)), hash(corp_key));

d1_dedup :=dedup(sort(d1,corp_key,local),corp_key,local);

pre_d2   :=distribute(project(corp2.files('').AID.cont.Built,
															transform(Corp2.Layout_Corporate_Direct_Cont_Base, 
																				self.bdid := left.bid,
																				self := left)),hash(corp_key));
...
persistname := corp2.Thor_Cluster + 'persist::Corp2_services::BWR_buildautokeyB2_Bid';
...
t := corp2.keynames('').Bid_AutokeyRoot;
inlogical := corp2.keynames(pversion).Autokeys.Bid_Root.new;

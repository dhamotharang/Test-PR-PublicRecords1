d	:= CellPhone.file_cellphones_base;
...
MAXdate := MAX(group,if(d.DateLastSeen<>'',d.DateLastSeen,'00000000'));

has_DateFirstSeen := AVE(group,IF(stringlib.stringfilterout(d.DateFirstSeen ,'0')<>'',100,0));
has_DateLastSeen := AVE(group,IF(stringlib.stringfilterout(d.DateLastSeen ,'0')<>'',100,0));
...
has_RegistrationDate := AVE(group,IF(stringlib.stringfilterout(d.RegistrationDate ,'0')<>'',100,0));
...
has_name_did := AVE(group,IF(stringlib.stringfilterout(d.did ,'0')<>'',100,0));
...
export Build_Field_Population := output(choosen(stats_sorted,all)); 
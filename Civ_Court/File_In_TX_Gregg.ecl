...
Civ_Court.Layout_In_TX_Gregg.l_out xfrmFields(xfrmLine L)	:= TRANSFORM
	...
	self.CaseTitle		:= L.line_data[STD.Str.Find(L.line_data, '\t',4) + 1..STD.Str.Find(L.line_data, '\t',5) - 1];
	self.CaseType			:= L.line_data[STD.Str.Find(L.line_data, '\t',5) + 1..STD.Str.Find(L.line_data, '\t',6) - 1];
	self.CaseSubType	:= L.line_data[STD.Str.Find(L.line_data, '\t',6) + 1..STD.Str.Find(L.line_data, '\t',7) - 1];
	self.CaseNumber		:= L.line_data[STD.Str.Find(L.line_data, '\t',7) + 1..STD.Str.Find(L.line_data, '\t',8) - 1];
	self.FileDate			:= L.line_data[STD.Str.Find(L.line_data, '\t',8) + 1..STD.Str.Find(L.line_data, '\t',9) - 1];
	self.CaseStatusDate		:= L.line_data[STD.Str.Find(L.line_data, '\t',9) + 1..STD.Str.Find(L.line_data, '\t',10) - 1];
	self.ActiveCaseStatus	:= L.line_data[STD.Str.Find(L.line_data, '\t',10) + 1..STD.Str.Find(L.line_data, '\t',11) - 1];
	self.LastEventDate		:= L.line_data[STD.Str.Find(L.line_data, '\t',11) + 1..STD.Str.Find(L.line_data, '\t',12) - 1];
	self.LastEventType		:= STD.Str.FindReplace(L.line_data[STD.Str.Find(L.line_data, '\t',12) + 1..],'"','');
END;

...
fpHancockPlan 	:= Civ_Court.File_In_OH_Hancock(plaintiff<>''and plaintiff<>'PLAINTIFF' and plaintiff<>'Plaintiff');

Civil_Court.Layout_In_Party tMICiv1(fpHancockPlan input) := Transform
...
self.entity_1						:=if(trim(input.party_name,left,right)<>'',
										trim(input.party_name,left,right),
										'');
self.entity_nm_format_1				:='L';
self.entity_type_code_1_orig		:='';
self.entity_type_description_1_orig	:=if(regexfind('PLAINT|DEFEND',StringLib.StringToUpperCase(trim(input.party_type,left,right)),0)<>'',
										StringLib.StringToUpperCase(trim(input.party_type,left,right)),
										'OTHER');
self.entity_type_code_1_master		:=if(StringLib.StringToUpperCase(trim(input.party_type,left,right))='PLAINTIFF',
										'10',
										if(StringLib.StringToUpperCase(trim(input.party_type,left,right))='DEFENDANT',
										'30',
										'70'));
...
pHancockPlan 	:= project(fpHancockPlan,tMICiv1(left));

Civ_court.Civ_Court_Cleaner(pHancockPlan,cleanHancock);

ddHancock 	:= dedup(sort(distribute(cleanHancock,hash(case_key)),
									...
									PERSIST('~thor_200::in::civil_OH_Hancock_party');

export Map_OH_Hancock_Party := ddHancock;
...
nonfcra_ds_curr := dataset('~scoringqa::out::nonfcra::addressshell_batch_generic_attributes_v1_' + dt + '_1', zz_bbraaten2.NonFCRA_AddressShell_batch_layout_v1, thor)(length(trim(errorcode,left,right))= 0 );
...
nonfcra_ds_prev := dataset('~'+ nonfcra_p_file_name, zz_bbraaten2.NonFCRA_AddressShell_batch_layout_v1, thor)(length(trim(errorcode,left,right))= 0 );
...
// ******** END: NonFCRA CURRENT MODE CALCULATIONS *********************************************************************************************************************
  	...
  	// OUTPUT(XtabOut, NAMED('XtabOut'));
		...
		// final := FileServices.SendEmail(Scoring_Project_DailyTracking.email_distribution.fail_list, 'TEST......LeadIntegrity 4.1 Daily Tracking Report: MaxDiff ' + max_diff, XtabOut[COUNT(XtabOut)].line):
									...
									// FAILURE(FileServices.SendEmail('Bridgett.braaten@lexisnexis.com','AddressShell Cert Tracking job failed','The failed workunit is:' + WORKUNIT + FAILMESSAGE));

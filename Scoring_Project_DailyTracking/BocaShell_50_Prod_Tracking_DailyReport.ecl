...
tag:='~';
ds_curr := distribute(dataset(tag + 'scoringqa::out::fcra::bocashell_50_historydate_999999_prod_' + dt + '_1', Scoring_Project_Macros.Global_Output_Layouts.BocaShell_Global_Layout, thor)(length(trim(errorcode,left,right))= 0 ),(integer)accountnumber);
...
ashirey.flatten(ds_curr_join, curr_output, , , ,);
ashirey.flatten(ds_prev_join, prev_output, , , ,);
...
ds_prev_archive := dataset(tag + p_arch_file_name, Scoring_Project_Macros.Global_Output_Layouts.BocaShell_Global_Layout, thor)(length(trim(errorcode,left,right))= 0 );
// ds_prev_archive := dataset(tag + p_arch_file_name, zz_bbraaten2.Boca_50_Cert_NonFCRA, thor)(length(trim(errorcode,left,right))= 0 );
...
ashirey.flatten(ds_curr_arch_join, fcra_arch_curr_output, , , ,);
ashirey.flatten(ds_prev_arch_join, fcra_arch_prev_output, , , ,);
...
nonfcra_ds_curr := dataset(tag + 'scoringqa::out::nonfcra::bocashell_50_historydate_999999_prod_' + dt + '_1', Scoring_Project_Macros.Global_Output_Layouts.BocaShell_Global_Layout, thor)(length(trim(errorcode,left,right))= 0 );
...
nonfcra_ds_prev := dataset(tag + nonfcra_p_file_name, Scoring_Project_Macros.Global_Output_Layouts.BocaShell_Global_Layout, thor)(length(trim(errorcode,left,right))= 0 );
// nonfcra_ds_prev := dataset(tag + nonfcra_p_file_name, zz_bbraaten2.Boca_50_Cert_NonFCRA, thor)(length(trim(errorcode,left,right))= 0 );
...
/* LAYOUT FLATTENING */ 
				 ashirey.flatten(nonfcra_ds_curr_join, nonfcra_curr_output, , , ,);
         ashirey.flatten(nonfcra_ds_prev_join, nonfcra_prev_output, , , ,);
...
nonfcra_ds_curr_arch := dataset(tag + 'scoringqa::out::nonfcra::bocashell_50_historydate_201207_prod_' + dt + '_1', Scoring_Project_Macros.Global_Output_Layouts.BocaShell_Global_Layout, thor)(length(trim(errorcode,left,right))= 0 );
...
nonfcra_ds_prev_arch := dataset(tag + nonfcra_p_file_name_arch, Scoring_Project_Macros.Global_Output_Layouts.BocaShell_Global_Layout, thor)(length(trim(errorcode,left,right))= 0 );
// nonfcra_ds_prev_arch := dataset(tag + nonfcra_p_file_name_arch, zz_bbraaten2.Boca_50_Cert_NonFCRA, thor)(length(trim(errorcode,left,right))= 0 );
...
/* LAYOUT FLATTENING */ 
				 ashirey.flatten(nonfcra_ds_curr_join_arch, nonfcra_curr_output_arch, , , ,);
         ashirey.flatten(nonfcra_ds_prev_join_arch, nonfcra_prev_output_arch, , , ,);
...
// ******** END: NonFCRA ARCHIVE MODE CALCULATIONS *********************************************************************************************************************
		...
		fcra_prod_archive := IF(COUNT(re_filter2_archive) > 0,
																			PROJECT(re_filter2_archive, TRANSFORM(MyRec, SELF.order := 2;
																			SELF.line := (TRIM(LEFT.Field_Name) + filler)[1..70]  + (LEFT.Difference_count + filler)[1..25]  + LEFT.Difference_Percent + '%')),
																			ds_no_diff);
																			
		nonfcra_prod_realtime := IF(COUNT(re_filter2_nonfcra) > 0,
																			PROJECT(re_filter2_nonfcra, TRANSFORM(MyRec, SELF.order := 2;
																			SELF.line := (TRIM(LEFT.Field_Name) + filler)[1..70]  + (LEFT.Difference_count + filler)[1..25]  + LEFT.Difference_Percent + '%')),
																			ds_no_diff);
																			
		nonfcra_prod_archive := IF(COUNT(re_filter2_nonfcra_arch) > 0,
		...
		main_head := DATASET([{1,   'BocaShell 5.0 Prod Attributes Tracking Report' + '\n'
	...
	head_nonfcra_prod_realtime := DATASET([{1,    
	...
	head_nonfcra_prod_archive := DATASET([{1,    
		...
		output_fcra_prod_archive := PROJECT(SORT(head_fcra_prod_archive + fcra_prod_archive, order), TRANSFORM(MyRec, SELF.order := 4; SELF := LEFT));
		output_nonfcra_prod_realtime := PROJECT(SORT(head_nonfcra_prod_realtime + nonfcra_prod_realtime, order), TRANSFORM(MyRec, SELF.order := 6; SELF := LEFT));
		output_nonfcra_prod_archive := PROJECT(SORT(head_nonfcra_prod_archive + nonfcra_prod_archive, order), TRANSFORM(MyRec, SELF.order := 8; SELF := LEFT));
		// OUTPUT(output_cert_realtime, NAMED('output_cert_realtime'));
		// OUTPUT(output_cert_archive, NAMED('output_cert_archive'));

		output_append := main_head + output_fcra_prod_realtime + spacer + output_fcra_prod_archive + spacer2 + output_nonfcra_prod_realtime + spacer3 + output_nonfcra_prod_archive;
  	...
  	// OUTPUT(XtabOut, NAMED('XtabOut'));
		...
		final := FileServices.SendEmail(Scoring_Project_DailyTracking.email_distribution.general_list_all, 'BocaShell 5.0 Prod Tracking Report: MaxDiff ' + max_diff, XtabOut[COUNT(XtabOut)].line):
									FAILURE(FileServices.SendEmail(Scoring_Project_DailyTracking.email_distribution.Bocashell_collections_fail_list,'BocaShell 5.0 Prod Tracking CRON job failed','The failed workunit is:' + WORKUNIT + FAILMESSAGE));

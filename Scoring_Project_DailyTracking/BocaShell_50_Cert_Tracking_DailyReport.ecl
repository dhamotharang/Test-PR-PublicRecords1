EXPORT BocaShell_50_Cert_Tracking_DailyReport := FUNCTION
...
ds_curr := distribute(dataset('~scoringqa::out::fcra::bocashell_50_historydate_999999_cert_' + dt + '_1', Scoring_Project_Macros.Global_Output_Layouts.BocaShell_Global_Layout, thor)(length(trim(errorcode,left,right))= 0 ),(integer)accountnumber);
// bs_cert_curr_choosen := choosen(bs_cert_curr, 10);
// bs_cert_curr_choosen;

filenames_details :=  nothor(STD.File.LogicalFileList('scoringqa::out::fcra::bocashell_50_historydate_999999_cert_*_1'));
...
ds_curr_archive := dataset('~scoringqa::out::fcra::bocashell_50_historydate_201207_cert_' + dt + '_1', Scoring_Project_Macros.Global_Output_Layouts.BocaShell_Global_Layout, thor)(length(trim(errorcode,left,right))= 0 );

filenames_details_arch :=  nothor(STD.File.LogicalFileList('scoringqa::out::fcra::bocashell_50_historydate_201207_cert_*_1'));
...
nonfcra_ds_curr := dataset('~scoringqa::out::nonfcra::bocashell_50_historydate_999999_cert_' + dt + '_1', Scoring_Project_Macros.Global_Output_Layouts.BocaShell_Global_Layout, thor)(length(trim(errorcode,left,right))= 0 );


nonfcra_filenames_details :=  nothor(STD.File.LogicalFileList('scoringqa::out::nonfcra::bocashell_50_historydate_999999_cert_*_1'));
...
nonfcra_ds_curr_arch := dataset('~scoringqa::out::nonfcra::bocashell_50_historydate_201207_cert_' + dt + '_1', Scoring_Project_Macros.Global_Output_Layouts.BocaShell_Global_Layout, thor)(length(trim(errorcode,left,right))= 0 );


nonfcra_filenames_details_arch :=  nothor(STD.File.LogicalFileList('scoringqa::out::nonfcra::bocashell_50_historydate_201207_cert_*_1'));
...
// ******** END: NonFCRA ARCHIVE MODE CALCULATIONS *********************************************************************************************************************
		...
		main_head := DATASET([{1,   'BocaShell 5.0 Cert Attributes Tracking Report' + '\n'
  	...
  	// OUTPUT(XtabOut, NAMED('XtabOut'));
		...
		final := FileServices.SendEmail(Scoring_Project_DailyTracking.email_distribution.general_list_all, 'BocaShell 5.0 Cert Tracking Report: MaxDiff ' + max_diff, XtabOut[COUNT(XtabOut)].line):
									FAILURE(FileServices.SendEmail(Scoring_Project_DailyTracking.email_distribution.Bocashell_collections_fail_list,'BocaShell 5.0 Cert Tracking CRON job failed','The failed workunit is:' + WORKUNIT + FAILMESSAGE));

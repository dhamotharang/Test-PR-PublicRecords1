...
nonfcra_ds_curr := dataset(tag +'scoringqa::out::nonfcra::bocashell_41_historydate_999999_prod_' + dt + '_1', Scoring_Project_Macros.Global_Output_Layouts.BocaShell_Global_Layout, thor)(length(trim(errorcode,left,right))= 0 );
...
nonfcra_ds_curr_arch := dataset(tag + 'scoringqa::out::nonfcra::bocashell_41_historydate_201207_prod_' + dt + '_1', Scoring_Project_Macros.Global_Output_Layouts.BocaShell_Global_Layout, thor)(length(trim(errorcode,left,right))= 0 );
...
// ******** END: NonFCRA ARCHIVE MODE CALCULATIONS *********************************************************************************************************************
	...
	MyRec := RECORD
		...
		main_head := DATASET([{1,   'BocaShell 4.1 Prod Attributes Tracking Report' + '\n'
	...
	head_nonfcra_prod_archive := DATASET([{1,    
  	...
  	// OUTPUT(XtabOut, NAMED('XtabOut'));
		...
		final := FileServices.SendEmail(Scoring_Project_DailyTracking.email_distribution.general_list_all, 'BocaShell 4.1 Prod Tracking Report: MaxDiff ' + max_diff, XtabOut[COUNT(XtabOut)].line):
		// final := FileServices.SendEmail(Scoring_Project_DailyTracking.email_distribution.fail_list, 'TEST......LeadIntegrity 4.1 Daily Tracking Report: MaxDiff ' + max_diff, XtabOut[COUNT(XtabOut)].line):
									...
									FAILURE(FileServices.SendEmail(Scoring_Project_DailyTracking.email_distribution.Bocashell_collections_fail_list,'BocaShell 4.1 Prod Tracking CRON job failed','The failed workunit is:' + WORKUNIT + FAILMESSAGE));

...
, DATASET(Layout_WhoIs) Delta = DATASET([],Layout_WhoIs)
, DATASET(Layout_WhoIs) dsBase = In_WhoIs // Change IN_WhoIs to change input to ingest process
, DATASET(RECORDOF(WhoIs.prep_ingest_file))  infile = WhoIs.prep_ingest_file
) := MODULE
  SHARED NullFile := DATASET([],Layout_WhoIs); // Use to replace files you wish to remove
  ...
  DistIngest0 := DISTRIBUTE(FilesToIngest0, HASH32(domainname,emailtype,email,name,organization, street1,street2,street3,street4,city,state,postalcode,country, phone));
  SortIngest0 := SORT(DistIngest0,domainname,emailtype,email,name, organization, street1,street2,street3,street4,city,state,postalcode,country, phone, __Tpe, RCID, LOCAL);
  GroupIngest0 := GROUP(SortIngest0,domainname,emailtype,email,name, organization, street1,street2,street3,street4,city,state,postalcode,country, phone, LOCAL, ORDERED, STABLE);
  SHARED AllIngestRecs0 := UNGROUP(ROLLUP(GroupIngest0,TRUE,MergeData(LEFT,RIGHT)));
 
  // Existing Base: combine delta with base file
  DistBase0 := DISTRIBUTE(Base0+Delta0, HASH32(domainname,emailtype,email,name, organization, street1,street2,street3,street4,city,state,postalcode,country, phone));
  SortBase0 := SORT(DistBase0,domainname,emailtype,email,name, organization, street1,street2,street3,street4,city,state,postalcode,country, phone, __Tpe, RCID, LOCAL);
  GroupBase0 := GROUP(SortBase0,domainname,emailtype,email,name, organization, street1,street2,street3,street4,city,state,postalcode,country, phone, LOCAL, ORDERED, STABLE);
  SHARED AllBaseRecs0 := UNGROUP(ROLLUP(GroupBase0,TRUE,MergeData(LEFT,RIGHT)));
 
  // Everything: combine ingest and base recs
  Sort0 := SORT(AllBaseRecs0+AllIngestRecs0,domainname,emailtype,email,name, organization, street1,street2,street3,street4,city,state,postalcode,country, phone, __Tpe,RCID,LOCAL);
  Group0 := GROUP(Sort0,domainname,emailtype,email,name, organization, street1,street2,street3,street4,city,state,postalcode,country, phone,LOCAL, ORDERED, STABLE);
  ...
  SHARED AllRecs := ORe+NR1+NR(RCID<>0) : PERSIST('~temp::WhoIs::Ingest_Cache',EXPIRE(WhoIs.Config.PersistExpire));
  ...
  EXPORT NewRecords_NoTag := PROJECT(NewRecords,Layout_WhoIs);
  EXPORT OldRecords :=PROJECT( AllRecs(__Tpe=RecordType.Old), NoFlagsRec);
  EXPORT OldRecords_NoTag := PROJECT(OldRecords,Layout_WhoIs);
  EXPORT UpdatedRecords := PROJECT(AllRecs(__Tpe=RecordType.Updated), NoFlagsRec);
  EXPORT UpdatedRecords_NoTag := PROJECT(UpdatedRecords,Layout_WhoIs);
  EXPORT AllRecords := IF(incremental, NewRecords, PROJECT(AllRecs, NoFlagsRec));
  EXPORT AllRecords_NoTag := PROJECT(AllRecords,Layout_WhoIs); // Records in 'pure' format

...
fcra_base := dm_base(trim(src) = mdr.sourceTools.src_Death_Master and glb_flag != 'Y');

base := if(isFCRA, fcra_base, dm_base);

key_name := if(isFCRA, 
							data_services.data_location.prefix('Death')+'thor_data400::key::fcra::death_master::ssn_'+ doxie.Version_SuperKey,
							data_services.data_location.prefix('Death')+'thor_data400::key::death_master::ssn_'+ doxie.Version_SuperKey
							);

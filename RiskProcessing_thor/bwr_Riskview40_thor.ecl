IMPORT FFD, Gateway, iesp, Risk_Indicators, RiskView, Ut, _Control, RiskProcessing, models, Riskwise;
onThor := _Control.Environment.OnThor;
settingsOK := OnThor and _Control.Environment.OnVault and _Control.LibraryUse.ForceOff_AllLibraries;
// settingsOK := true;

#WORKUNIT('name', 'Riskview 4.0' + 	if(settingsOK,  ' THOR ', ERROR(9,'Toggle OnThor OnVault LibraryUse') ) );  // throw an error here if the controls aren't set correctly
...
infile_name := '~foreign::' + _control.IPAddress.prod_thor_dali + '::tfuerstenberg::in::la_11091_boca_input.csv';
// infile_name := '~rbao::in::smartpay_9025_f2_cons1_input.csv';

outfile_name := '~dvstemp::out::riskview40_thor_' + thorlib.wuid();
...
risk_indicators.Layout_Batch_In_Plus_Custom t_f(f le) := TRANSFORM
	SELF.HistoryDateYYYYMM := (unsigned)((string)LE.HistoryDateYYYYMM)[1..6];
...
batchin := project(f, t_f(left)) ;
output(choosen(batchin, eyeball), named('batchin'));

gateways := Gateway.Constants.void_gateway;
string DataRestriction := '10000100010001000000000000000000000000000'; // to restrict fares, experian and transunion -- returns liens and judgments
//string DataRestriction := '10000100010001000000000000000000000000001';//to restrict fares, LIENS/Jdgmts, experian and transunion 
  
// string IntendedPurpose := 'PRESCREENING';	// to run in prescreen mode
string IntendedPurpose := 'APPLICATION';

boolean	IncludeAllScores := false;
boolean	IncludeAllAttributes := false;
boolean	IncludeScoresAttributes := true;
boolean	IncludeAuto := true;
boolean	IncludeBankcard := true;
boolean	IncludeRetail := true;
boolean	IncludeTelecom := true;
boolean	IncludeMoney_temp   := true;
boolean	IncludePreScreenTemp := false;
boolean	IncludeLifestyle := false;
boolean	IncludeDemographic := false;
boolean	IncludeFinancial := false;
boolean	IncludeProperty := false;
boolean	IncludeDerogatory := false;
boolean IncludeVersion2 := false;
boolean IncludeVersion3 := false;
boolean IncludeVersion4 := TRUE;
string5 industry_class_val := '';
string    AlternateModel_in := '';
boolean	IsPreScreenTemp := if(IntendedPurpose='PRESCREENING', true, false);
integer FlagshipVersion := 1;
...
Risk_Indicators.Layout_Batch_In_Plus_Custom into_seq(batchin le, integer C) := TRANSFORM
	...
	self := le;
END;
batchinseq := project(batchin, into_seq(left,counter)) : PERSIST('~BOCASHELLFCRA::riskview40_batchin_with_seq', expire(5));  // use persist instead of independent;
...
output(CHOOSEN(cleanIn, eyeball));
...
custom_adl_clam := group(join(adl_clam, batchinseq, left.seq=right.seq, getcustom(left, right)),seq);
...
OUTPUT(choosen(final,eyeball), NAMED('Results'));
OUTPUT(final,,outfile_name, CSV(heading(single), QUOTE('"')) ) ;	

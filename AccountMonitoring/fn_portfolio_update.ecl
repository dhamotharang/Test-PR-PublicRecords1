...
// -- NOTE: when new portfolio (2 mutually exclusive files) update takes place, remove -timestamp from SORTED here
      ...
      // project the layout back to the original base layout (without the reverse time stamp field)
		...
		mac_update_documents_file(pseudo_environment,email,spray_ip_address,spray_ip_path,fnms.documents.email.remote,write_updated_email_documents_file);
		mac_update_documents_file(pseudo_environment,corteratradeline,spray_ip_address,spray_ip_path,fnms.documents.corteratradeline.remote,write_updated_corteratradeline_documents_file);
      ...
      main_files_to_update := PARALLEL( write_updated_portfolio_file,
                                        ...
                                        write_updated_email_documents_file,
                                        write_updated_corteratradeline_documents_file
                                      );

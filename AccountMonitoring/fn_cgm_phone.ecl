...
EXPORT DATASET(layouts.history) fn_cgm_phone(
  ...
  FUNCTION
    ...
    file_history_address := 
      DISTRIBUTED(
        AccountMonitoring.product_files.phone.file_history_address, 
        HASH64(name_last,prim_range,prim_name,z5)
      );
    ...
    file_history_phone := DISTRIBUTED(AccountMonitoring.product_files.phone.file_history_phone, HASH64(name_last, name_first, phone7, area_code));
    ...
    file_history_name_city_st := DISTRIBUTED(AccountMonitoring.product_files.phone.file_history_name_city_st, HASH64(city_code,st,dph_name_last,name_last,p_name_first,name_first));
    ...
    temp_base_dist_cell := DISTRIBUTED(AccountMonitoring.product_files.phone.base_file_cell, HASH64(did));  
    ...
    temp_base_dist_neighbor := DISTRIBUTED(AccountMonitoring.product_files.phone.base_file_neighbor, HASH64(did));
    ...
    base_file_paw_did := DISTRIBUTED(AccountMonitoring.product_files.phone.base_file_paw_did, HASH64(did));
    ...
    key_phones_type           := DISTRIBUTED(AccountMonitoring.product_files.phone.key_phones_type_dist, HASH64(phone));
    temp_add_phone_type_full_10 := DISTRIBUTE(temp_all_deduped(LENGTH(TRIM(phone)) = 10), HASH64(phone));
		
		layout_phone_type := RECORD
		temp_layout;
  ...
  fn_GetServiceType(STRING1 serv, STRING1 line) :=	FUNCTION//look at both serv and line
    ...
    ds_PhoneType_join := JOIN(temp_add_phone_type_full_10, key_phones_type,
                               LEFT.phone = RIGHT.phone,
                               TRANSFORM(layout_phone_type,
    ...
    ds_PhoneType_lerg := ds_PhoneType(phone_type_code = '');
    ds_PhoneType_no_lerg := ds_PhoneType(phone_type_code != '');

    //Should look up Lerg6 index only if that data is not available in key_phones_type index
    Lerg6 := DISTRIBUTED(AccountMonitoring.product_files.phone.key_Phones_Lerg6_dist, HASH64(npa, nxx, block_id));
    ds_PhoneType_redist := DISTRIBUTE(ds_PhoneType_lerg, HASH64(phone[1..3], phone[4..6], phone[7]));
    ...
    temp_add_lerg6 := JOIN(ds_PhoneType_redist, Lerg6,

...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Layout_in_file)
    ...
    UNSIGNED1 corp_farm_exemptions_Invalid;
...
EXPORT FromNone(DATASET(Layout_in_file) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_term_exist_desc_Invalid := Fields.InValid_corp_term_exist_desc((SALT35.StrType)le.corp_term_exist_desc);
    ...
    SELF.corp_country_of_formation_Invalid := Fields.InValid_corp_country_of_formation((SALT35.StrType)le.corp_country_of_formation);
    SELF.corp_farm_exemptions_Invalid := Fields.InValid_corp_farm_exemptions((SALT35.StrType)le.corp_farm_exemptions);
    SELF.corp_trademark_class_desc1_Invalid := Fields.InValid_corp_trademark_class_desc1((SALT35.StrType)le.corp_trademark_class_desc1);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_address1_effective_date_Invalid << 22 ) + ( le.corp_filing_date_Invalid << 24 ) + ( le.corp_status_cd_Invalid << 26 ) + ( le.corp_inc_state_Invalid << 27 ) + ( le.corp_inc_date_Invalid << 28 ) + ( le.corp_term_exist_exp_Invalid << 30 ) + ( le.corp_term_exist_desc_Invalid << 31 ) + ( le.corp_foreign_domestic_ind_Invalid << 32 ) + ( le.corp_forgn_state_desc_Invalid << 33 ) + ( le.corp_forgn_date_Invalid << 34 ) + ( le.corp_orig_org_structure_cd_Invalid << 36 ) + ( le.corp_for_profit_ind_Invalid << 37 ) + ( le.corp_country_of_formation_Invalid << 38 ) + ( le.corp_farm_exemptions_Invalid << 39 ) + ( le.corp_trademark_class_desc1_Invalid << 40 ) + ( le.recordorigin_Invalid << 41 ) + ( le.InternalField1_Invalid << 42 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_status_cd_Invalid := (le.ScrubsBits1 >> 26) & 1;
    SELF.corp_inc_state_Invalid := (le.ScrubsBits1 >> 27) & 1;
    SELF.corp_inc_date_Invalid := (le.ScrubsBits1 >> 28) & 3;
    SELF.corp_term_exist_exp_Invalid := (le.ScrubsBits1 >> 30) & 1;
    ...
    SELF.corp_forgn_date_Invalid := (le.ScrubsBits1 >> 34) & 3;
    SELF.corp_orig_org_structure_cd_Invalid := (le.ScrubsBits1 >> 36) & 1;
    SELF.corp_for_profit_ind_Invalid := (le.ScrubsBits1 >> 37) & 1;
    SELF.corp_country_of_formation_Invalid := (le.ScrubsBits1 >> 38) & 1;
    SELF.corp_farm_exemptions_Invalid := (le.ScrubsBits1 >> 39) & 1;
    SELF.corp_trademark_class_desc1_Invalid := (le.ScrubsBits1 >> 40) & 1;
    SELF.recordorigin_Invalid := (le.ScrubsBits1 >> 41) & 1;
    SELF.InternalField1_Invalid := (le.ScrubsBits1 >> 42) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_farm_exemptions_ALLOW_ErrorCount := COUNT(GROUP,h.corp_farm_exemptions_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_farm_exemptions_Invalid,'ALLOW','UNKNOWN')
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT35.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_farm_exemptions:invalid_farm_exemptions:ALLOW'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_corp_farm_exemptions(1)
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_farm_exemptions_ALLOW_ErrorCount

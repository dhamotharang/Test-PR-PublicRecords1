...
EXPORT RealTime_Batch_Service_Records(DATASET(Batch_Layout.RealTime_InLayout) inputData,UNSIGNED1 Operation,BOOLEAN GatewayNameMatch = FALSE,BOOLEAN Is_UseDate = FALSE) := FUNCTION
  ...
  layoutAcctNoChildRpt vehicleGateway(Batch_Layout.RealTime_InLayout L) := TRANSFORM
    ...
    tempmod := MODULE(PROJECT(inputParams,VehicleV2_Services.IParam.polkParams,opt))
      ...
      EXPORT STRING make := STD.STR.ToUpperCase(TRIM(L.make, LEFT, RIGHT));
      EXPORT STRING model := STD.STR.ToUpperCase(TRIM(L.model, LEFT, RIGHT));
   ...
   filtered := IF(GatewayNameMatch AND L.name_first<>'' AND L.name_last<>'',filtered_gateways(
       STD.STR.ToUpperCase(TRIM(L.name_first[1])) IN [registrants[1].fname[1],registrants[2].fname[1]]
   AND STD.STR.ToUpperCase(TRIM(L.name_last)) IN [registrants[1].lname ,registrants[2].lname]),filtered_gateways);
    ...
    makeFilter := IF(~restricted AND L.make<>'',
      yearFilter(STD.STR.Find(STD.STR.ToUpperCase(make_desc),
                 STD.STR.ToUpperCase(TRIM(L.make)),1)=1),
      yearFilter);
    modelFilter := IF(~restricted AND L.model<>'',
      makeFilter(STD.STR.Find(STD.STR.ToUpperCase(model_desc),
                 STD.STR.ToUpperCase(TRIM(L.model)),1)=1),
      makeFilter);
    SELF.acctNo := L.acctNo;
  ...
  Batch_Layout.RealTime_OutLayout gatewayJoin(Batch_Layout.RealTime_InLayout L, layoutAcctNoRpt R) := TRANSFORM
    ...
    SELF.reg_1_addr1 := STD.STR.CleanSpaces(reg_1.prim_range+' '+reg_1.predir+' '
      +reg_1.prim_name+' '+reg_1.addr_suffix+' '+reg_1.postdir);
    SELF.reg_1_addr2 := STD.STR.CleanSpaces(reg_1.unit_desig+' '+reg_1.sec_range);
    ...
    SELF.reg_2_addr1 := STD.STR.CleanSpaces(reg_2.prim_range+' '+reg_2.predir+' '
      +reg_2.prim_name+' '+reg_2.addr_suffix+' '+reg_2.postdir);
    SELF.reg_2_addr2 := STD.STR.CleanSpaces(reg_2.unit_desig+' '+reg_2.sec_range);

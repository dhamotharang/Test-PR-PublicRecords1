IMPORT doxie, Autokey_batch, Address, VehicleV2, BatchServices, AutokeyB2, AutoStandardI,VehicleV2_Services;
...
EXPORT LicPlate_Batch_Service_records(dataset(Batch_Layout.LicPlate_InLayout) data_in,
	...
	descUnmatch(STRING lStr, STRING rStr) := lStr <> BLNK AND StringLib.StringFind(toUpper(rStr), toUpper(lStr), 1) = 0;
	...
	// Output(withDIDSet,NAMED('withDIDSet'));
	// Output(fromDIDLkup,NAMED('fromDIDLkup'));
	// output(withLPInput,NAMED('withLPInput'));
	...
	// output(rslt,NAMED('rslt_licplate'));
	RETURN rslt;

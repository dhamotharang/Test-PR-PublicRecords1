IMPORT $, iesp,AutoStandardI, Address, doxie, Gateway, VehicleV2_Services, STD;
...
EXPORT Get_Experian_Data := MODULE
  ...
  EXPORT val($.IParam.polkParams in_mod, BOOLEAN doCombined=FALSE) := FUNCTION
    ...
    dataSource := TRIM(STD.STR.touppercase(in_mod.DataSource));
    ...
    VehicleV2_Services.assorted_layouts.Layout_registrant xformRegistrants(vinaLayout L,INTEGER C) := TRANSFORM
      ...
      SELF.reg_latest_effective_date := STD.STR.Filter(TRIM(L.lastTitleRegDate),'0123456789');
      SELF.reg_latest_expiration_date := STD.STR.Filter(TRIM(L.registrationExpireDate),'0123456789');
      ...
      SELF.title_issue_date := STD.STR.Filter(TRIM(L.titleIssueDate),'0123456789');
    ...
    VehicleV2_Services.assorted_layouts.layout_brand xformBrand(iesp.experian_vin.t_VehicleBrandsStruct L) := TRANSFORM
      SELF.Brand_Date := STD.STR.Filter(TRIM(L.brandDate),'0123456789'),

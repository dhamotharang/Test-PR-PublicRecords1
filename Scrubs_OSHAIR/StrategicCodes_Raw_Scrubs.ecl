...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    prog_type_ENUM_ErrorCount := COUNT(GROUP,h.prog_type_Invalid=1);
    AnyRule_WithErrorsCount := COUNT(GROUP, h.activity_nr_Invalid > 0 OR h.prog_type_Invalid > 0);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.activity_nr_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.prog_type_ENUM_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.activity_nr_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.prog_type_ENUM_ErrorCount > 0, 1, 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.activity_nr_Invalid,le.prog_type_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,StrategicCodes_Raw_Fields.InvalidMessage_activity_nr(le.activity_nr_Invalid),StrategicCodes_Raw_Fields.InvalidMessage_prog_type(le.prog_type_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ,CHOOSE(le.activity_nr_Invalid,'CUSTOM','UNKNOWN')
          ,CHOOSE(le.prog_type_Invalid,'ENUM','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'activity_nr','prog_type','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_numeric','Invalid_prog_type','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT311.StrType)le.activity_nr,(SALT311.StrType)le.prog_type,'***SALTBUG***');
  END;
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'prog_type:Invalid_prog_type:ENUM'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ,StrategicCodes_Raw_Fields.InvalidMessage_activity_nr(1)
          ,StrategicCodes_Raw_Fields.InvalidMessage_prog_type(1)
          ,'Fields with errors'
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ,le.activity_nr_CUSTOM_ErrorCount
          ,le.prog_type_ENUM_ErrorCount,0) / le.TotalCnt, CHOOSE(c - NumRules
    ...
    SALT311.ScrubsOrbitLayout xNormHygieneStats(hygiene_summaryStats le, UNSIGNED c, STRING suffix) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'prog_type:' + getFieldTypeText(h.prog_type) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix,'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
          ,le.populated_activity_nr_cnt
          ,le.populated_prog_type_cnt,0);
      SELF.rulepcnt := CHOOSE(c
          ,le.populated_activity_nr_pcnt
          ,le.populated_prog_type_pcnt,0);
      SELF.ErrorMessage := '';
    END;
...
EXPORT StandardStats(DATASET(Scrubs_OSHAIR.StrategicCodes_Raw_Layout) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_OSHAIR, StrategicCodes_Raw_Fields, 'RECORDOF(scrubsSummaryOverall)', '');

...
EXPORT SALT311.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_numeric','Invalid_opt_type','Invalid_opt_info_id');
EXPORT FieldTypeNum(SALT311.StrType fn) := CASE(fn,'invalid_numeric' => 1,'Invalid_opt_type' => 2,'Invalid_opt_info_id' => 3,0);
...
EXPORT MakeFT_Invalid_opt_info_id(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'_dfinopt'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_opt_info_id(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'_dfinopt'))));
EXPORT InValidMessageFT_Invalid_opt_info_id(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('_dfinopt'),SALT311.HygieneErrors.Good);
 
EXPORT SALT311.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'activity_nr','opt_type','opt_id','opt_info_id');
EXPORT SALT311.StrType FlatName(UNSIGNED2 i) := CHOOSE(i,'activity_nr','opt_type','opt_id','opt_info_id');
EXPORT FieldNum(SALT311.StrType fn) := CASE(fn,'activity_nr' => 0,'opt_type' => 1,'opt_id' => 2,'opt_info_id' => 3,0);
EXPORT SET OF SALT311.StrType FieldRules(UNSIGNED2 i) := CHOOSE(i,['CUSTOM'],['ENUM'],['CUSTOM'],['ALLOW'],[]);
...
EXPORT Make_opt_info_id(SALT311.StrType s0) := MakeFT_Invalid_opt_info_id(s0);
EXPORT InValid_opt_info_id(SALT311.StrType s) := InValidFT_Invalid_opt_info_id(s);
EXPORT InValidMessage_opt_info_id(UNSIGNED1 wh) := InValidMessageFT_Invalid_opt_info_id(wh);
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Diff_opt_info_id := le.opt_info_id <> ri.opt_info_id;
    SELF.Val := (SALT311.StrType)evaluate(le,pivot_exp);
    SELF.Num_Diffs := 0+ IF( SELF.Diff_activity_nr,1,0)+ IF( SELF.Diff_opt_type,1,0)+ IF( SELF.Diff_opt_id,1,0)+ IF( SELF.Diff_opt_info_id,1,0);
  END;
...
#uniquename(AggRec)
  %AggRec% := RECORD
    ...
    Count_Diff_opt_info_id := COUNT(GROUP,%Closest%.Diff_opt_info_id);
  END;

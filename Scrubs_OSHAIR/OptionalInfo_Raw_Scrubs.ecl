...
// The module to handle the case where no scrubs exist
  ...
  END;
EXPORT FromNone(DATASET(OptionalInfo_Raw_Layout) h) := MODULE
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,OptionalInfo_Raw_Layout);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(OptionalInfo_Raw_Layout) prevDS = DATASET([], OptionalInfo_Raw_Layout), STRING10 Src='UNK'):= FUNCTION
  // field error stats
    ...
    mod_hygiene := OptionalInfo_Raw_hygiene(PROJECT(h, OptionalInfo_Raw_Layout));
...
EXPORT StandardStats(DATASET(OptionalInfo_Raw_Layout) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  myTimeStamp := (UNSIGNED6)SALT311.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
  expandedFile := FromNone(inFile).ExpandedInfile;
  mod_fromexpandedOverall := FromExpanded(expandedFile);
  scrubsSummaryOverall := mod_fromexpandedOverall.SummaryStats;
 
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_OSHAIR, OptionalInfo_Raw_Fields, 'RECORDOF(scrubsSummaryOverall)', '');

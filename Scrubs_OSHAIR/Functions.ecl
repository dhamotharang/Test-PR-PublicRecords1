...
EXPORT Functions := MODULE
  ...
  END;
	...
	//*******************************************************************************
  //fn_general_date: 	Returns true if valid date
  //*******************************************************************************
  EXPORT fn_general_date(STRING sDate) := FUNCTION
    clean_date  := TRIM(sDate, ALL);
    isValidDate := IF(LENGTH(clean_date) = 8, Scrubs.fn_valid_Date(clean_date) > 0, FALSE);
    RETURN IF(isValidDate or LENGTH(clean_date) = 0, 1, 0);
  END;	
  ...
  EXPORT fn_past_yyyymmdd(STRING sDate) := FUNCTION
    ...
    RETURN IF(isValidDate or trim(sDate)='0', 1, 0);
  END;	
  ...
  END;	
	...
	//********************************************************************************
	//fn_alpha_numeric:  returns true if all the characters are alphanumeric
	//********************************************************************************
	EXPORT fn_alpha_numeric(STRING s) := function    
	  RETURN IF(Stringlib.StringFilterOut(s, '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ') = '', 1, 0);
  END;	
  ...
  END;
	...
	EXPORT fn_union_status(STRING code) := function    
	  RETURN IF(Stringlib.StringFilterOut(code, ' ABNUY') = '' ,1 ,0);
	END;
	...
	//********************************************************************************
	//fn_alpha_numeric_blank:  returns true if all the characters are alphanumeric or blank
	//********************************************************************************
	EXPORT fn_alpha_numeric_blank(STRING s) := function    
	  RETURN IF(Stringlib.StringFilterOut(s, ' 0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ') = '', 1, 0);
  END;	

...
IMPORT SANCTN, ut, doxie, codes, suppress, census_data, standard;
...
EXPORT out_rec GetDataSourceByID (GROUPED DATASET (Sanctn_Services.layouts.id) in_ids,
                                  string in_ssn_mask_type = '', string32 apptype) := FUNCTION
  ...
  layouts.Incident RollIncidentInfo (inc_grp L, DATASET (in_rec) all_recs) := TRANSFORM
    // roll incident info: concatenate text which comes without breaks in order_number;
    IncInfo := layouts.IncidentInfo;
  ...
  layouts.rec_party RollPartyInfo (SANCTN_Services.layouts.layoutSanctnClean L, DATASET (SANCTN_Services.layouts.layoutSanctnClean) all_recs) := transform
    SELF.name := L;
    ...
    SELF.SSNUMBER := IF (L.SSNUMBER != '', Stringlib.StringFilterOut (L.SSNUMBER,'- '), '');
  ...
  // pull, mask, add county name
	Suppress.MAC_Suppress(party_info_rolled,party_pulled,apptype,Suppress.Constants.LinkTypes.SSN,SSNUMBER);
	Suppress.MAC_Mask(party_pulled, party_masked, SSNUMBER, blank, true, false, maskVal:=in_ssn_mask_type);
  ...
  rec_parties_rolled := RECORD
    ...
    unsigned1 penalt;
    DATASET (layouts.Party) parties {MAXCOUNT (Constants.PARTY_PER_INCIDENT)};
  END; 

  rec_parties_rolled RollParties (prt_grp L, DATASET (recordof (prt_grp)) all_recs) := transform
    Self.parties := project (all_recs, layouts.Party);
  ...
  // add party info
  OUT_REC AppendParty (layouts.Incident L, rec_parties_rolled R) := TRANSFORM

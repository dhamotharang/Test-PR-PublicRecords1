...
export Mapping_DL_OH_As_ConvPoints( string  pversion, dataset(DriversV2.Layouts_DL_OH_In.Layout_OH_CP_PDate) in_file) := module
	...
	DriversV2.Layouts_DL_OH_In.Layout_OH_CP_PDate trfFixChars(DriversV2.Layouts_DL_OH_In.Layout_OH_CP_PDate l)	:=	transform
		self.blob 	 		:= fn_RemoveSpecialChars(l.blob);
		self            := l;
	end;
  ...
  //**************** CONVICTIONS Mapping **********************************************************************************
	...
	Layout_Conviction_Common trfOHToConvictions(file_Oh_Conviction l) := transform
		...
		self.type_cd              := TrimUpper(l.Record_Type_Code);
		self.type_desc            := DriversV2.Tables_CP_OH.record_type_code(TrimUpper(self.type_cd));
		self.violation_date       := ValidateDate(l.Offense_Violation_Date);
		self.plate_nbr            := TrimUpper(l.plate_number);
		self.conviction_date      := ValidateDate(l.conviction_date);
		self.court_county         := TrimUpper(l.Court_Code);
		self.court_name_cd        := TrimUpper(l.Court_Code);
		self.court_name_desc      := DriversV2.Tables_CP_OH.court_code(self.court_name_cd);
		self.court_type_cd        := TrimUpper(l.Court_Type_Code);
		self.court_type_desc      := DriversV2.Tables_CP_OH.court_type_code(TrimUpper(self.court_type_cd));
		self.st_offense_conv_cd   := TrimUpper(l.Conviction_Type_Code);
		self.st_offense_conv_desc := DriversV2.Tables_CP_OH.offense_conv_code(TrimUpper(self.st_offense_conv_cd));
		self.sentence             := TrimUpper(l.Sentence_Code);
		self.sentence_desc        := DriversV2.Tables_CP_OH.sentence_code(TrimUpper(self.sentence));
		self.points               := TrimUpper(l.Court_Assessed_Points);
		self.hazardous_cd         := TrimUpper(l.Had_Hazardous_Materials);
		self.hazardous_desc       := DriversV2.Tables_CP_OH.hazardous_mat_flag(TrimUpper(self.hazardous_cd));
		self.plea_cd              := TrimUpper(l.conviction_plea);
		self.plea_desc            := DriversV2.Tables_CP_OH.conviction_plea_flag(TrimUpper(self.plea_cd));
		self.court_case_nbr       := TrimUpper(l.court_case_number);
		self.acd_offense_cd       := TrimUpper(l.ACD_Offense_Code);
		self.acd_offense_desc     := DriversV2.Tables_CP_OH.acd_offense_codes(TrimUpper(self.acd_offense_cd));
		self.reduced_cd           := TrimUpper(l.offense_reduced_from_code);
		self.reduced_desc         := '';
		self.create_date          := ValidateDate(l.create_date);
		self.bmv_case_nbr_1       := TrimUpper(l.s1_case_number);
		self.bmv_case_nbr_2       := TrimUpper(l.p2_case_number);
		self.bmv_case_nbr_3       := TrimUpper(l.p3_case_number);
		self.habitual_case_nbr    := TrimUpper(l.habitual_case_number);
		self.soa_conviction       := TrimUpper(l.SOA_ConvWithOffense_Code);
		self.bmv_sp_case_nbr      := TrimUpper(l.sp_case_number);
		self.out_of_state_dl_nbr  := TrimUpper(l.Out_of_State_Lic_Nbr);
		self.state_of_origin      := TrimUpper(l.state_of_origin);
		self                      := l;
		self                      := [];	  
	end;
	...
	Layout_Suspension_Common trfOHToSuspension(file_Oh_Suspension l) := transform
		  self.dt_first_seen               := l.process_date;
   		...
   		self.type_desc              		 := DriversV2.Tables_CP_OH.record_type_code(TrimUpper(self.type_cd));
   		self.violation_date         		 := ValidateDate(l.Offense_Violation_Date);
   		self.filed_date             		 := ValidateDate(l.Proof_Filing_Shown_Date);  
   		self.clear_date             		 := ValidateDate(l.Clear_Date);
   		self.action_date            		 := ValidateDate(l.Action_Date);
   		self.start_date             		 := ValidateDate(l.Start_Date);
   		self.end_date               		 := ValidateDate(l.end_Date);	  
   		self.bmv_case_nbr_1         		 := TrimUpper(l.BMV_Case_Number);
   		self.court_name_cd          		 := TrimUpper(l.Court_Code);
   		self.court_name_desc        		 := DriversV2.Tables_CP_OH.COURT_CODE(self.court_name_cd);	
			self.court_county           		 := TrimUpper(l.Court_Code);
			self.court_type             		 := TrimUpper(l.Court_Code);
   		self.st_offense_conv_cd     		 := TrimUpper(l.Ohio_Offense_Code);
   		self.st_offense_conv_desc   		 := DriversV2.Tables_CP_OH.offense_conv_code(TrimUpper(self.st_offense_conv_cd));
   		self.vehicle_no_cd          		 := TrimUpper(l.Is_Commercial_Vehicle);
   		self.vehicle_no_desc        		 := DriversV2.Tables_CP_OH.vehicle_ind_code(TrimUpper(self.vehicle_no_cd));	  
   		self.hazardous_cd           		 := TrimUpper(l.Had_Hazardous_Material);
   		self.hazardous_desc         		 := DriversV2.Tables_CP_OH.hazardous_mat_flag(TrimUpper(self.hazardous_cd));
   		self.jurisdiction           		 := TrimUpper(l.Jurisdiction_Code);
   		self.fee_pd_date            		 := ValidateDate(l.Fee_Paid_Date);
   		self.plate_nbr              		 := TrimUpper(l.Plate_Number);
   		self.cdl_disq_reason_cd    			 := TrimUpper(l.CDL_Disqualification_Reason_Code);
   		self.cdl_disq_reason_desc   		 := DriversV2.Tables_CP_OH.cdi_disq_reason_code(TrimUpper(self.cdl_disq_reason_cd));
   		self.cdl_ofs_disqual_reason_cd   := TrimUpper(l.CDL_Out_of_State_Disqualification_Type);
   		self.cdl_ofs_disqual_reason_desc := DriversV2.Tables_CP_OH.cdi_oos_disq_type(TrimUpper(self.cdl_ofs_disqual_reason_cd));
   		self.disq_ext_cd            		 := TrimUpper(l.Disqualification_Extent_Flag);						
   		self.disq_reason_ref        		 := ValidateDate(l.Disqualification_Reason_Reference);				
   		self.sd_nbr                 		 := TrimUpper(l.School_District_Number);
   		self.wd_clear_reason_cd     		 := TrimUpper(l.Withdrawal_Type_Detail_Code);
   		self.wd_clear_reason_desc   		 := DriversV2.Tables_CP_OH.wd_cle_reason_code(TrimUpper(self.wd_clear_reason_cd));						
   		self.cleared_date           		 := ValidateDate(l.Cleared_for_Restricted_Driving_Date);
   		self.Conviction_Date             := ValidateDate(l.Conviction_Date);
   		self.wd_reason_cd          			 := TrimUpper(l.Withdrawal_Reason_Code);			
   		self.wd_reason_desc         		 := DriversV2.Tables_CP_OH.wd_reason_code(TrimUpper(self.wd_reason_cd));
   		self.remedial_drv_crs_dt    		 := TrimUpper(l.Remedial_Driving_Course_Date);						
   		self.appeal_file_date       		 := ValidateDate(l.Appeal_Filed_Date);
   		self.appeal_deny_date       		 := ValidateDate(l.Appeal_Denied_Date);
   		self.appeal_granted_date    		 := ValidateDate(l.Appeal_Granted_Date);
   		self.plea_cd                		 := TrimUpper(l.Conviction_Plea_Flag);
   		self.plea_desc              		 := DriversV2.Tables_CP_OH.conviction_plea_flag(TrimUpper(self.plea_cd));
   		self.county_cd              		 := TrimUpper(l.County_Number);
   		self.create_date            		 := ValidateDate(l.Create_Date);
   		self.license_rec_date       		 := ValidateDate(l.License_Received_Date);
   		self.plate_rec_date         		 := ValidateDate(l.Plate_Received_Date);
   		self.fra_sup_start_date     		 := ValidateDate(l.FRA_Start_Date);
   		self.fra_sup_end_date       		 := ValidateDate(l.FRA_end_Date);
   		self.accident_date          		 := ValidateDate(l.Accident_Date);
   		self.related_bmv_case_nbr   		 := TrimUpper(l.Primary_Related_BMV_Case);
   		self.narrative_cd           		 := TrimUpper(l.Narrative_Literal_Code);
   		self.narrative_desc         		 := DriversV2.Tables_CP_OH.narrative_code(TrimUpper(self.narrative_cd));
   		self.fee_required           		 := TrimUpper(l.Is_Fee_Required);
   		self.vehicle_owner_ind      		 := TrimUpper(l.Is_Vehicle_Owner);
   		self.soa_conviction         		 := TrimUpper(l.SOA_ConvWithOffense_Code);
   		self.modified_drv_end_dt    		 := ValidateDate(l.Modified_Driving_end_Date);
   		self.appeal_sup_stay        		 := TrimUpper(l.Is_Appeal_Stay); 
   		self.wd_type_detail         		 := TrimUpper(l.Withdrawal_Type_Detail_Code);
   		self.bmv_dl_case_nbr        		 := TrimUpper(l.BMV_Related_DL_Case);
   		self.related_bmv_case_nbr_2 		 := TrimUpper(l.Additional_Related_BMV_Case);
   		self.fine_paid_date         		 := ValidateDate(l.Fine_Paid_Date);
   		self.csea                   		 := TrimUpper(l.CSEA_Number);
   		self.csea_case_nbr          		 := TrimUpper(l.CSEA_Case_Number);
   		self.csea_order_nbr         		 := TrimUpper(l.CSEA_Order_Number);
   		self.csea_part_nbr          		 := TrimUpper(l.CSEA_Agency_Participant_Number);
   		self                             := l;
   		self                        		 := [];	  
	end;
	...
	Layout_DR_Info_Common trfOHToDR_Info(DriversV2.Layouts_DL_OH_In.Temp_Combined_RecLayout4_DrInfo l) := transform
		...
		self.action_date             := ValidateDate(l.record_type_action_date);
		self.bmv_case_nbr_1          := TrimUpper(l.bmv_case_number);
		self.clear_date              := ValidateDate(l.clear_date);	  
		self.cosigners_dl_nbr        := TrimUpper(l.cosigners_drivers_license_nbr);      
		self.cosigners_name          := TrimUpper(l.drivers_cosigners_name);	  
		self.cosigners_relationship  := TrimUpper(l.drivers_cosigners_relationship);
		self.create_date             := ValidateDate(l.create_date);
		self.out_of_state_dl_nbr     := TrimUpper(l.OutOfStateDLN);
		self.remedial_require_comp   := ValidateDate(l.RemedialReqCompletionDate);            
		self.remedial_require_denied := ValidateDate(l.requirementsDeniedDate);
		self.warrant_eff_date        := ValidateDate(l.warrant_block_effective_date);
		self                         := l;
		self                         := [];
	end;
	...
	Layout_DR_Info_Common trfOH_LCTODR_Info(ds_Cancellation l) := transform
		...
		self.type_cd                 := TrimUpper(l.record_type_code);
		self.type_desc               := DriversV2.Tables_CP_OH.record_type_code(self.type_cd);
		self.action_date             := ValidateDate(l.dateEntered );
		self.bmv_case_nbr_1          := TrimUpper(l.BMVcaseNum);
		self.court_case_nbr          := TrimUpper(l.courtCaseNum);
		self                         := l;
		self                         := [];
	end;
	...
	Layout_Accident_Common trfOHToAccident(file_Oh_Accidents l) := transform
		...
		self.type_cd            := TrimUpper(l.record_type_code);
		self.type_desc          := DriversV2.Tables_CP_OH.record_type_code(TrimUpper(self.type_cd));
		self.county             := TrimUpper(l.county_number);      
		self.jurisdiction       := TrimUpper(l.jurisdiction_code);
		self.severity_cd        := TrimUpper(l.Accident_Severity_Indicator);	  
		self.severity_desc      := DriversV2.Tables_CP_OH.accident_severity_code(TrimUpper(self.severity_cd));
		self.accident_date      := ValidateDate(l.accident_date);
		self.vehicle_no         := TrimUpper(l.Is_Commercial_Vehicle);
		self.hazardous_cd       := TrimUpper(l.Has_Hazardous_Materials);
		self.hazardous_desc     := DriversV2.Tables_CP_OH.hazardous_mat_flag(TrimUpper(self.hazardous_cd));            
		self.create_date        := ValidateDate(l.create_date);	
	  self                    := l;		
		self                    :=[];
	...
	Layout_FRA_Common trfOHToFRA(file_Oh_FRA_Insurance l) := transform
		...
		self.src_state             := 'OH';	
		self.dlcp_key              := trim(l.key_number, left, right);      
		self.cancel_posted_date    := ValidateDate(l.Proof_Filing_Cancelled_Posted_Date);
		self.filed_date            := ValidateDate(l.proof_filing_posted_date);
		self.ins_cancel_dt         := ValidateDate(l.Proof_Filing_Cancellation_Date);	  
		self.ins_policy_nbr        := TrimUpper(l.Insurance_Policy);           
		self.latest_proof_start_dt := ValidateDate(l.Proof_Filing_Late_Start_Date);
		self.proof_start_date      := ValidateDate(l.Proof_Filing_Start_Date);
		self.proof_end_date        := ValidateDate(l.Proof_Filing_end_Date);
		self                       := l;
		self                       := [];	
	end;

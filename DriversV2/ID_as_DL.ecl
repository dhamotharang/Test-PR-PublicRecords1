...
DriversV2.Layout_DL_Extended lTransform_ID_To_Common(Drivers.File_ID_Full pInput)
 := transform
	self.orig_state 						  := 'ID';
	...
	self.license_class 			  		:= pInput.orig_LICENSE_CLASS;
	self.license_type 			  		:= '';
	self.OrigLicenseClass					:= pInput.orig_LICENSE_CLASS;
	self.OrigLicenseType					:= '';	
	self.moxie_license_type 			:= pInput.orig_LICENSE_CLASS;
	...
	self.expiration_date	     		:= (unsigned4)lInsertDDinMMCCYY(pInput.orig_EXPIRATION_DATE);
	...
	self.dl_number 				 				:= '';  //modified as per bug# 26788 to stop populating LNID numbers.

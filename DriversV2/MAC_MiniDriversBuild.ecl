...
%sequenced% := DriversFileIn;
// Slim down for processing
#uniquename(layout_slim)
%layout_slim% := RECORD
	%sequenced%.dl_seq;
	UNSIGNED4 newest_seq_no := %sequenced%.dl_seq;
...
%j_orig% := JOIN(
	SORT(
		DISTRIBUTE(%drivers_hist%, %get_node%(dl_seq)),
		dl_seq, LOCAL),
	%sequenced%,
...
%j_latest% := JOIN(
	SORT(
		DISTRIBUTE(%j_orig%(dl_seq != newest_seq_no), %get_node%(newest_seq_no)),
		newest_seq_no, LOCAL),
	%sequenced%,
	LEFT.newest_seq_no = RIGHT.dl_seq,
...
#uniquename(j_all)
%j_all% := PROJECT(%j_orig%(dl_seq = newest_seq_no), %strip_seq%(LEFT))
			+ %j_latest%;
...
%j_did% := JOIN(
	...
	SORT(%j_all%, dl_seq, LOCAL),
	LEFT.seq = RIGHT.dl_seq,

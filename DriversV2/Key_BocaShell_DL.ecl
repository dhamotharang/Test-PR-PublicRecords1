...
dlrec get_dl_num(DriversV2.File_dl_search ri) :=
TRANSFORM
...
dl_id := group(SORT(distribute(project(DriversV2.File_dl_search(did != 0), get_dl_num(LEFT)), hash(did)),did, dl_number, source_code, orig_state, local),did, dl_number,  source_code, orig_state, local);
...
export Key_BocaShell_DL := index (dl_rolled, {did}, {dl_rolled}, '~thor_data400::key::dl2::'+doxie.Version_SuperKey+'::bocaShell_DL_DID' );

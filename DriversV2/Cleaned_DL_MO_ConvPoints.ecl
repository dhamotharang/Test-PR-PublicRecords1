...
EXPORT Cleaned_DL_MO_ConvPoints(STRING processDate, STRING fileDate) := FUNCTION
  ...
  layout_out  trans1(in_file l) := TRANSFORM
	  ...
	  SELF.Last_Name            := l.Line[111..135];              
	  SELF.First_Name           := l.Line[136..150];              
	  SELF.Middle_Name          := l.Line[151..162];              
	  SELF.Suffix               := l.Line[163..165];              
	  SELF.Birthdate            := l.Line[166..173];              
	  SELF.Gender               := l.Line[174..174];              
	  SELF.Address              := l.Line[175..199];              
	  SELF.City                 := l.Line[200..219];              
	  SELF.State                := l.Line[220..221];              
	  SELF.Zip5                 := l.Line[222..226];              
	  SELF.Zip4                 := l.Line[227..230];              
	  SELF.Mailing_Address1     := l.Line[231..255];              
	  SELF.Mailing_Address2     := l.Line[256..280];              
	  SELF.Mailing_City         := l.Line[281..300];              
	  SELF.Mailing_State        := l.Line[301..302];               
	  SELF.Mailing_Zip          := l.Line[303..311];              
	  SELF.Height               := l.Line[312..314];              
	  SELF.Weight               := l.Line[315..317];              
	  SELF.Eye_Color            := l.Line[318..318];              
	  SELF.Operator_Status      := l.Line[319..320];              
	  SELF.Commercial_Status    := l.Line[321..322];              
	  SELF.Sch_Bus_Status       := l.Line[323..324];              
	  SELF.Pending_Act_Code     := l.Line[325..325];              
	  SELF.Must_Test_Ind        := l.Line[326..326];              
	  SELF.Deceased_Ind         := l.Line[327..327];               
	  SELF.Prev_CDL_Class       := l.Line[328..329];              
	  SELF.CDL_Ptr              := l.Line[330..330];              
	  SELF.PDPS_Ptr             := l.Line[331..331];              
	  SELF.MVR_Privacy_Code     := l.Line[332..332];              
	  SELF.BRC_Status_Code      := l.Line[333..333];              
	  SELF.BRC_Status_Date      := l.Line[334..341];              
	  SELF.Filler2              := l.Line[342..350];              
	  //********* License Issuance Record              ****************
    SELF.Lic_Iss_Code         := l.Line[351..351];              
	  SELF.Lic_Class            := l.Line[352..353];              
	  SELF.Lic_Exp_Date         := l.Line[354..361];              
	  SELF.Lic_Seq_Number       := l.Line[362..373];              
	  SELF.Lic_Surrender_From   := l.Line[374..375];              
	  SELF.Lic_Surrender_To     := l.Line[376..377];              
	  SELF.New_Out_of_St_DL_Num := l.Line[378..402];              
	  SELF.Lic_Endorsements     := l.Line[403..412];              
	  SELF.Lic_Restrictions     := l.Line[413..417];              
	  SELF.Filler3              := l.Line[418..422];              
	  SELF.Lic_Trans_Type       := l.Line[423..424];              
	  SELF.Lic_Print_Date       := l.Line[425..432];              
	  SELF.Filler4              := l.Line[433..442];              
	  SELF.Permit_Iss_Code      := l.Line[443..443];              
	  SELF.Permit_Class         := l.Line[444..445];              
	  SELF.Permit_Exp_Date      := l.Line[446..453];              
	  SELF.Permit_Seq_Number    := l.Line[454..465];              
	  SELF.Permit_Endorse_Codes := l.Line[466..475];              
	  SELF.Permit_Restric_Codes := l.Line[476..480];              
	  SELF.Filler5              := l.Line[481..485];              
	  SELF.Permit_Trans_Type    := l.Line[486..487];              
	  SELF.Permit_Print_Date    := l.Line[488..495];              
	  SELF.Filler6              := l.Line[496..497];              
	  SELF.Non_Drvr_Code        := l.Line[498..498];              
	  SELF.Non_Drvr_Exp_Date    := l.Line[499..506];              
	  SELF.Non_Drvr_Seq_Num     := l.Line[507..518];              
	  SELF.Non_Drvr_Trans_Type  := l.Line[519..520];              
	  SELF.Non_Drvr_Print_Date  := l.Line[521..528];              
	  SELF.Filler7              := l.Line[529..540];              
	  //********* Other Records    ****************
	  SELF.Action_Surrender_Date:= l.Line[541..548];             
	  SELF.Action_Counts        := (UNSIGNED4)l.Line[549..551];  
	  SELF.Driv_Priv_Counts     := (UNSIGNED4)l.Line[552..554];  
	  SELF.Conv_Counts          := (UNSIGNED4)l.Line[555..557];  
	  SELF.Accidents_Counts     := (UNSIGNED4)l.Line[558..560];  
	  SELF.AKA_Counts           := (UNSIGNED4)l.Line[561..563];  
	  SELF.Filler8              := l.Line[564..569];    
  ...
  inrecs := PROJECT(in_file, trans1(left));
	...
	Layouts_DL_MO_New_In.Layout_MO_Actions transActions1(inrecs l, UNSIGNED4 C) := TRANSFORM,SKIP(l.Action_Recs = '')
	 ...
	 Layouts_DL_MO_New_In.Layout_MO_AKA transAKA1(inrecs l, UNSIGNED4 C) := TRANSFORM,SKIP(l.AKA_Recs = '')
	   ...
	   SELF.AKA_Lic_State := trimUpper(CHOOSE(C,l.AKA_Recs[1..2], l.AKA_Recs[101..102],l.AKA_Recs[201..202]));
		 ...
		 SELF.AKA_DOB       := trimUpper(CHOOSE(C,l.AKA_Recs[83..90],l.AKA_Recs[183..190],l.AKA_Recs[283..290]));
		 SELF.AKA_Filler2   := lib_stringlib.stringlib.stringfilter(CHOOSE(C,l.AKA_Recs[91..100],l.AKA_Recs[191..200],l.AKA_Recs[291..300]),'0123456789');
   END;
	 ...
	 Layouts_DL_MO_New_In.Layout_MO_AKA_Pdate transAKA2(akarecs l) := TRANSFORM
		 SELF.process_date     := IF(_Validate.Date.fIsValid(processDate),processDate,'');
		 SELF.Unique_Key       := trimUpper(l.Unique_Key);   
   ...
   OUTPUT(MO_DL_Action_Recs,,DriversV2.Constants.cluster+'in::dl2::mo_action_cp_update::'+ filedate,OVERWRITE);
	 ...
	 OUTPUT(MO_DL_AKA_Recs,,DriversV2.Constants.cluster+'in::dl2::mo_aka_cp_update::'+ filedate,OVERWRITE);
	 
	 Cleaned_MO_DL_CP_Recs   := SEQUENTIAL(
              ...
              FileServices.AddSuperFile(DriversV2.Constants.cluster + 'in::dl2::mo_new_aka_cp_updates::superfile', 
						         	DriversV2.Constants.cluster +'in::dl2::mo_aka_cp_update::'+filedate), 

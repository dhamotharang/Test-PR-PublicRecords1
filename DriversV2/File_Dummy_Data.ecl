...
export File_Dummy_Data(string filedate) := function
	...
	DriversV2.Layout_DL_Extended	tFixDummyDates(dInlineDummyData_Input pInput)
	 :=
		transform
		self.dt_first_seen						:=	if(pInput.dt_first_seen = 0,			(unsigned3)((string6)filedate[1..6]),pInput.dt_first_seen);
		self.dt_last_seen							:=	if(pInput.dt_last_seen = 0,				(unsigned3)((string6)filedate[1..6]),pInput.dt_last_seen);
		self.dt_vendor_first_reported	:=	if(pInput.dt_vendor_first_reported = 0,	(unsigned3)((string6)filedate[1..6]),pInput.dt_vendor_first_reported);
		self.dt_vendor_last_reported	:=	if(pInput.dt_vendor_last_reported = 0,	(unsigned3)((string6)filedate[1..6]),pInput.dt_vendor_last_reported);
	...
	return project(dInlineDummyData_Input,tFixDummyDates(left));
end;

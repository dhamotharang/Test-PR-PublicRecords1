...
export fVehicle_as_Business_Contact(dataset(layout_vehicles) pVehicles)
 :=
  function

	//Filter IRS Dummy DID's
	dVehicles := pVehicles(own_1_did < vehlic.irs_dummy_cutoff
	...
	dVehiclesAsBusCon				:=	normalize(dVehiclesWithCompany,4, tNormalizeVehiclesAsBusinessContacts(left,counter));
	dVehiclesAsBusConPersons		:=	distribute(dVehiclesAsBusCon(lname != '', company_name = '', (integer)name_score < 3), hash(vendor_id, temp_name_type_code));
	dVehiclesAsBusConCompanies		:=	distribute(dVehiclesAsBusCon(company_name != '' and company_name[01..09] != 'VAULT NOM' and company_name[01..16] != 'VAULT AS NOMINEE'), hash(vendor_id, temp_name_type_code));

	business_header.Layout_Business_Contact_Full_New tGetMatchingCompany(dVehiclesAsBusConPersons pPerson, dVehiclesAsBusConCompanies pCompany)
	 :=
	  transform
	...
	dContactsWithMatchingCompany	:=	join(dVehiclesAsBusConCompanies,dVehiclesAsBusConPersons,
	...
	dContactsOnlySort				:=	sort(distribute(dContactsWithMatchingCompany,hash(vendor_id,lname,company_name)),vendor_id,lname,company_name,local);
	...
	asbus := vehlic.fVehicle_as_Business_Header(pVehicles);
	...
	dContactsWithMatchingAsBus	:=	join(distribute(dContactsOnlyDedup, hash(vendor_id)),distribute(asbus, hash(vendor_id)),
											left.vendor_id = right.vendor_id,
											 matchtoasbus(left,right),
											 local
											);
	
	dContactsWithMatchingAsBus_Filtered	:=	dContactsWithMatchingAsBus((integer)name_score < 3, Business_Header.CheckPersonName(fname, mname, lname, name_suffix));

	return dContactsWithMatchingAsBus_Filtered;

...
export	Vehicles_as_header(dataset(Vehlic.layout_vehicles) pVehicles = dataset([],Vehlic.layout_vehicles), boolean pForHeaderBuild=false)
 :=
  function
	h := header.Layout_New_Records;
	dVehiclesAsSource	:=	VehLic.Vehicles_as_Source(pVehicles,pForHeaderBuild);
	...
	hp trans(dVehiclesAsSource le,integer cnt) := transform
	  self.did := 0;
	  self.rid := 0;
	...
	from_vr := normalize(dVehiclesAsSource,4,trans(left,counter));

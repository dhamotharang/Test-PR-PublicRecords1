...
input := IDA.Files(version,pUseProd).input;

Ida.layouts.base tMapping(ida.layouts.input L, C) := TRANSFORM

	SELF.src    := '';							
	SELF.persistent_record_id         := version+ '_'+ intformat(c,9,1);
	SELF.Dt_first_seen                := (unsigned)version;
	SELF.Dt_last_seen                 := (unsigned)version;
...
std_input := project(IDA.Files(version,pUseProd).input, tMapping(LEFT, counter));
...
accumulative_base := if(nothor(FileServices.GetSuperFileSubCount(IDA.Filenames(version, pUseProd).lBaseTemplate_built)) = 0
											 ,Updated_Base
											 ,Updated_Base + IDA.Files(version, pUseProd).base.built);

...
export _Flags(string pInput_Name='', string pBase_Name='',boolean pUseProd=false)  := module
	...
	export name                           := trim('thor_data400::in::ida::header::flag');	                                                                            	
    export headerflaglogicalfile          := NOTHOR(STD.File.LogicalFileList(name,true,false,false));
    export headerflag                     := EXISTS(headerflaglogicalfile);
...
EXPORT IDA_Monthly_Base_Reappend_ActiveWU 	:= GetActiveWU('IDA MONTHLY BUILD'+' 2*')[1].wuid;
EXPORT IDA_Monthly_Base_Reappend_State 	    := GetActiveWUState('IDA MONTHLY BUILD'+' 2*')[1].state;

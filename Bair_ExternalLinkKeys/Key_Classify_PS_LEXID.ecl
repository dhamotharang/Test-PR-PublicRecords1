...
EXPORT KeyName := KeyPrefix+'::'+'key::Bair_ExternalLinkKeys'+'::'+Config.KeyInfix+'::EID_HASH::Refs::LEXID';
 
EXPORT KeyName_sf := KeyPrefix+'::'+'key::Bair_ExternalLinkKeys'+'::'+KeySuperfile+'::EID_HASH::Refs::LEXID';
...
EXPORT Shrinkage := DATASET([{'LEXID','NAME_SUFFIX',CntRed_NAME_SUFFIX*100,CntRed_NAME_SUFFIX*TSize},{'LEXID','DOB',CntRed_DOB*100,CntRed_DOB*TSize},{'LEXID','DL',CntRed_DL*100,CntRed_DL*TSize}],SALT33.ShrinkLayout);
EXPORT CanSearch(Process_PS_Layouts.InputLayout le) := le.LEXID <> (typeof(le.LEXID))'';
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch(TYPEOF(h.LEXID) param_LEXID = (TYPEOF(h.LEXID))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.MNAME) param_MNAME = (TYPEOF(h.MNAME))'',TYPEOF(h.LNAME) param_LNAME = (TYPEOF(h.LNAME))'') := 
    STEPPED( LIMIT( Key(
...
EXPORT ScoredEID_HASHFetch(TYPEOF(h.LEXID) param_LEXID = (TYPEOF(h.LEXID))'',TYPEOF(h.FNAME) param_FNAME = (TYPEOF(h.FNAME))'',TYPEOF(h.MNAME) param_MNAME = (TYPEOF(h.MNAME))'',TYPEOF(h.LNAME) param_LNAME = (TYPEOF(h.LNAME))'',TYPEOF(h.NAME_SUFFIX) param_NAME_SUFFIX = (TYPEOF(h.NAME_SUFFIX))'',UNSIGNED4 param_DOB,TYPEOF(h.DL) param_DL = (TYPEOF(h.DL))'') := FUNCTION
  ...
  Process_PS_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.DL_match_code := MAP(le.DL = (TYPEOF(le.DL))'' OR param_DL = (TYPEOF(param_DL))'' => SALT33.MatchCode.OneSideNull,match_methods(File_Classify_PS).match_DL(le.DL,param_DL,FALSE));
    SELF.DLWeight := (50+MAP ( le.DL = (TYPEOF(le.DL))'' OR param_DL = (TYPEOF(param_DL))'' => 0,
           le.DL = param_DL  => le.DL_weight100,
           -0.876*le.DL_weight100))/100; 
    SELF.Weight := IF(le.EID_HASH = 0, 100, MAX(0,SELF.LEXIDWeight) + MAX(self.MAINNAMEWeight,MAX(0,SELF.FNAMEWeight) + MAX(0,SELF.MNAMEWeight) + MAX(0,SELF.LNAMEWeight)) + MAX(0,SELF.NAME_SUFFIXWeight) + MAX(0,SELF.DOBWeight) + MAX(0,SELF.DLWeight));
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_PS_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.DOB_match_code := MAP(
      ...
      SELF.DOBWeight = SELF.DOBWeight_year+SELF.DOBWeight_month+SELF.DOBWeight_day => SALT33.MatchCode.DateAggregate,
      SALT33.MatchCode.NoMatch);
    SELF.DL_match_code := MAP(le.DL = (TYPEOF(le.DL))'' OR ri.DL = (TYPEOF(ri.DL))'' => SALT33.MatchCode.OneSideNull,match_methods(File_Classify_PS).match_DL(le.DL,ri.DL,FALSE));
    SELF.DLWeight := (50+MAP ( le.DL = (TYPEOF(le.DL))'' OR ri.DL = (TYPEOF(ri.DL))'' => 0,
  ...
  J0 := JOIN(Recs1,Key,LEFT.LEXID = RIGHT.LEXID
    ...
    ATMOST(LEFT.LEXID = RIGHT.LEXID,5000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.LEXID = RIGHT.LEXID
     AND (( ( 
        (LEFT.FNAME[1..LENGTH(TRIM(RIGHT.FNAME))] = RIGHT.FNAME OR RIGHT.FNAME[1..LENGTH(TRIM(LEFT.FNAME))] = LEFT.FNAME OR metaphonelib.DMetaPhone1(LEFT.FNAME)=metaphonelib.DMetaPhone1(RIGHT.FNAME) OR SALT33.WithinEditN(LEFT.FNAME,RIGHT.FNAME,1, 0)  OR RIGHT.FNAME_PreferredName = fn_PreferredName(LEFT.FNAME) ) AND (LEFT.MNAME[1..LENGTH(TRIM(RIGHT.MNAME))] = RIGHT.MNAME OR RIGHT.MNAME[1..LENGTH(TRIM(LEFT.MNAME))] = LEFT.MNAME OR SALT33.WithinEditN(LEFT.MNAME,RIGHT.MNAME,2, 0) ) AND (LEFT.LNAME = (TYPEOF(LEFT.LNAME))'' OR RIGHT.LNAME = (TYPEOF(RIGHT.LNAME))'' OR metaphonelib.DMetaPhone1(LEFT.LNAME)=metaphonelib.DMetaPhone1(RIGHT.LNAME) OR SALT33.WithinEditN(LEFT.LNAME,RIGHT.LNAME,1, 0) OR SALT33.HyphenMatch(LEFT.LNAME,RIGHT.LNAME,1)<=2 )
          OR SALT33.fn_concept_wordbag_EditN_EL.Match3((SALT33.StrType)RIGHT.FNAME,RIGHT.FNAME_MAINNAME_weight100,true,0,true,RIGHT.FNAME_initial_char_weight100,1,(SALT33.StrType)RIGHT.MNAME,RIGHT.MNAME_MAINNAME_weight100,true,0,true,RIGHT.MNAME_initial_char_weight100,2,(SALT33.StrType)RIGHT.LNAME,RIGHT.LNAME_MAINNAME_weight100,true,0,false,0,1,(SALT33.StrType)LEFT.FNAME,(SALT33.StrType)LEFT.MNAME,(SALT33.StrType)LEFT.LNAME) > 0) )),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.LEXID = RIGHT.LEXID,5000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_LEXID='',Input_FNAME='',Input_MNAME='',Input_LNAME='',Input_NAME_SUFFIX='',Input_DOB='',Input_DL='',output_file,AsIndex='true') := MACRO
IMPORT SALT33,Bair_ExternalLinkKeys;
#IF(#TEXT(Input_LEXID)<>'')
  #uniquename(trans)
  Bair_ExternalLinkKeys.Key_Classify_PS_LEXID.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := Bair_ExternalLinkKeys.Key_Classify_PS_LEXID.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

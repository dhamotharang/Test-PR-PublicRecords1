// EXPORT BWR_Boca_Runway_Transform_Compare := 'todo';
import scoring_project_pip, zz_bbraaten2;
...
// scoring_project_pip.CROSSTAB_MACRO(ds_base, ds_prop, ['seq'], 'aid605_1_0_score');

		// j1 := join(ds_base, ds_prop, 
			// left.seq = right.seq 
			// and left.seq = right.seq ,
			// j_trans2(left, right));		
	

				// output(choosen(j1, all), named('ds_join'));
Base_stats := zz_bbraaten2.compare_results_macro(ds_base_clean);
Prop_stats := zz_bbraaten2.compare_results_macro(ds_prop_clean);

// output(base_stats,all);
// output(Prop_stats,all);
...
trans_lay2 := record
		...
		// Decimal7_3 Base_miss_rate	;
		// Decimal7_3 prop_miss_rate;	
		// Decimal7_3 miss_rate_diff;
		// Decimal7_3 Base_hit_rate	;
		// Decimal7_3 prop_hit_rate;
		// Decimal7_3 hit_rate_diff;
end;
...
trans_lay2 j_trans2(base_stats le, Prop_stats ri) := transform
		...
		// self.Base_miss_rate	:= le.miss_rate;
		// self.prop_miss_rate  := ri.miss_rate;	
		// self.miss_rate_diff  := (real)ri.miss_rate - (real)le.miss_rate;

		// self.Base_hit_rate	:= le.hit_rate	;
		// self.prop_hit_rate   := ri.hit_rate	;
		// self.hit_rate_diff  := (real)ri.hit_rate - (real)le.hit_rate;
...
rec_temp := record
	...
	out_rec := record
		...
		// decimal7_4 score_change_pct := 0;
end;
	
macro_stats(ds1, ds2, at_name) := FUNCTIONMACRO

		// rec_temp calc_results(rec rec1, rec rec2) := transform
		rec_temp calc_results(lay rec1, lay rec2) := transform
				...
				// self.flag_score_change := if((decimal7_4) rec1.#expand(at_name) <> (decimal7_4) rec2.#expand(at_name), 1, 0 );
		...
		// filter_join := (ds_temp.flag_score_change = 1);	
		...
		// decimal7_4 score_change_pct := count(filter_join)/count(ds_temp);
		// return dataset([{at_name, d_mean, d_median, score_change_pct}], out_rec);
		return dataset([{at_name, per_diff, per_up, per_down, d_mean, d_median}], out_rec);		
ENDMACRO;
	
dataset median_calc :=	
	
// macro_stats(ds_base_clean, ds_prop_clean, 'nap')+
// macro_stats(ds_base_clean, ds_prop_clean, 'nas')+
...
// macro_stats(ds_base_clean, ds_prop_clean, 'rva1503_0_0_score')+
// macro_stats(ds_base_clean, ds_prop_clean, 'rva1504_1_0_score')+
// macro_stats(ds_base_clean, ds_prop_clean, 'rva1504_2_0_score')+
// macro_stats(ds_base_clean, ds_prop_clean, 'rva1603_1_0_score')+
// macro_stats(ds_base_clean, ds_prop_clean, 'rva1605_1_0_score')+
// macro_stats(ds_base_clean, ds_prop_clean, 'rva1607_1_0_score')+
// macro_stats(ds_base_clean, ds_prop_clean, 'rva611_0_0_score')+
...
// macro_stats(ds_base_clean, ds_prop_clean, 'rvb1503_0_0_score')+
...
// macro_stats(ds_base_clean, ds_prop_clean, 'rvg1502_0_0_score')+
// macro_stats(ds_base_clean, ds_prop_clean, 'rvg1511_1_0_score')+
// macro_stats(ds_base_clean, ds_prop_clean, 'rvg1601_1_0_score')+
// macro_stats(ds_base_clean, ds_prop_clean, 'rvg1605_1_0_score')+
...
// macro_stats(ds_base_clean, ds_prop_clean, 'rvp1605_1_0_score')+
...
// macro_stats(ds_base_clean, ds_prop_clean, 'rvt1503_0_0_score');
// macro_stats(ds_base_clean, ds_prop_clean, 'rvt1601_1_0_score')+
...
output(choosen(j2, all), named('ds_join_end'));
// output(choosen(median_calc, all), named('ds_join_end'));

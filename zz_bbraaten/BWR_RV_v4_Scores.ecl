...
basefilename := '~scoringqa::out::fcra::riskview_xml_generic_allflagships_v4_' + b1; 
testfilename := '~scoringqa::out::fcra::riskview_xml_generic_allflagships_v4_' + a1;       
...
Avescorebase_auto := clean_ds_baseline(RV_score_auto	 >= '300');
Avescorenew_auto := clean_ds_new(RV_score_auto	 >= '300');

ab:= ave(Avescorebase_auto, (integer)RV_score_auto	 );
// output(ab);
ap := ave(Avescorenew_auto, (integer)RV_score_auto	);
// output(ap);

output(ap-ab, named('RV_score_auto'));
...
// gp - gb;
output(gp - gb, named('RV_score_money'));
...
// bp - bb;
output(bp - bb, named('RV_score_bank'));

Avescorebase := clean_ds_baseline(RV_score_telecom	 >= '300');
Avescorenew := clean_ds_new(RV_score_telecom	 >= '300');

tb:= ave(Avescorebase, (integer)RV_score_telecom	 );
// output(tb);
tp := ave(Avescorenew, (integer)RV_score_telecom	);
// output(tp);

// output(tp-tb);
output(tp-tb, named('RV_score_telecom'));
...
// scoring_project_pip.CROSSTAB_MACRO(clean_ds_baseline, clean_ds_new, ['acctno'], 'rv_score_retail');

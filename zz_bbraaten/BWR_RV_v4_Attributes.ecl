...
basefilename := '~scoringqa::out::fcra::riskview_xml_generic_attributes_v4_'+b1;         // v3 or v4   XML or Batch
testfilename := '~scoringqa::out::fcra::riskview_xml_generic_attributes_v4_'+a1;        
...
j1 := join(ds_join_baseline, ds_join_second, left.results.accountnumber = right.results.accountnumber
					AND LEFT.results.emailaddress < right.results.emailaddress,
					// AND (integer)right.results.phoneotheragenewestrecord = 0,
					TRANSFORM(cmpr, SELF.res := LEFT + right));

OUTPUT(count(j1), NAMED('emailaddress_count'));
OUTPUT(CHOOSEN(j1, 25), named('emailaddress'));



scoring_project_pip.COMPARE_DSETS_MACRO(clean_ds_baseline, clean_ds_new, ['accountnumber'], 0);
// scoring_project_pip.CROSSTAB_MACRO(clean_ds_baseline, clean_ds_new, ['accountnumber'], 'v4_liencount');

...
export BestAddr_common(dataset(AddrBest.Layout_BestAddr.Batch_in) f_in_raw,
  Doxie.IDataAccess mod_access,
  UNSIGNED YYMM=1000,
  BOOLEAN rollup_lastname = false,
  integer min_threshold = 75,
...
AddrBest.layout_header_ext get_input_addrs(f_in_raw l, integer C) :=transform
  ...
  alt_prim_range := addr_line_first[1..STD.STR.Find(addr_line_first,' ', 1)-1];
  alt_prim_name := trim(addr_line_first[STD.STR.Find(addr_line_first,' ', 1)+1..]);
...
f_in := project(f_in_raw, transform(didville.Layout_BestInfo_BatchIn_w_did,
                                     self.name_first :=
                                     AlphasOnly(STD.STR.touppercase(left.name_first)),
                                     self.name_middle :=
                                     AlphasOnly(STD.STR.touppercase(left.name_middle)),
                                     self.name_last :=
                                     AlphasOnly(STD.STR.touppercase(left.name_last)),
                                     self := left,self:=[]));
...
best_w_gong := doxie.Append_Gong(pre_gong_w_unique_rids, rels, mod_access);
...
AddrBest.Layout_BestAddr.service_Out_matchcodes get_outrc(final_recs l,f_in r) := transform
  ...
  self.name_dual := if(STD.STR.find(l.listed_name,' & ',1)>0,l.listed_name,'');

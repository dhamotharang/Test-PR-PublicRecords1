...
file_in:= dataset('~thor::base::demo_data_file_fL_crash5_prodcopy',flaccidents.Layout_FLCrash5_Base,flat);
...
mac_scramble_pii_v2( //** THE ORDER OF THE FIELD NAMES IS CRITICAL ***
         ...
         scrambled_file, 	//scrambled output data set attribute
		  false,phone_nbr,
		  false,dl_nbr ,   // dl_number field name
...
self.passenger_full_name :=  trim(l.fname)+' '+trim(l.mname)+ ' '+ trim(l.lname);
self.passenger_st_city := trim(clean_addr[1..8],left,right)+' '+ TRIM(clean_addr[10..38],LEFT,RIGHT)+', '+l.v_city_name;
...
export scramble_fl_crash5 := scrambled1;

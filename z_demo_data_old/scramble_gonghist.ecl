...
file_in:= dataset('~thor::base::demo_data_file_gonghist_prodcopy',Gong.layout_history ,flat);
...
mac_scramble_pii_v2( //** THE ORDER OF THE FIELD NAMES IS CRITICAL ***
         ...
         scrambled_file, 	//scrambled output data set attribute
		  ...
		  //
		  false,listed_name, // businesName Field
		  //
		  true,street,
		  true,v_city_name,
		  true,st,
...
scrambled_listed_name  := if(l.listing_type_bus<>'' or l.listing_type_gov<>'' ,demo_data_scrambler.fn_scrambleBizName(l.listed_name),'');
//
...
self.v_prim_name  := TRIM(clean_addr[10..38],LEFT,RIGHT);
self.p_city_name := L.v_city_name;
self.listed_name := if(scrambled_listed_name<>'', scrambled_listed_name, trim(l.name_last)+' '+trim(l.name_first)+' '+trim(l.name_middle));
self.bdid := IF(l.bdid <> 0,(integer)fn_scramblePII('DID',(string)l.bdid),0);
self.hhid := IF(l.hhid <> 0,(integer)fn_scramblePII('DID',(string)l.hhid),0);
self:=l;
END;


//
export scramble_gonghist := project(scrambled_file(phone10<>''), reformat(LEFT));
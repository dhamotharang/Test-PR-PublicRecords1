...
export proc_watchdog_delta(string var1) := function
//Compare the previous best file with the current one



// curr := dataset('persist::watchdog_joined',watchdog.layout_best,flat);

curr := watchdog.File_Best;
...
self.title := if(l.title=r.title,'',if(l.title='' and r.title<>'','*NULL',l.title));
...
self.prim_range := if(l.prim_range=r.prim_range,'',if(l.prim_range='' and r.prim_range<>'','*NULL',l.prim_range));
self.predir := if(l.predir=r.predir,'',if(l.predir='' and r.predir<>'','*NULL',l.predir));
self.prim_name := if(l.prim_name=r.prim_name,'',if(l.prim_name='' and r.prim_name<>'','*NULL',l.prim_name));
self.suffix := if(l.suffix=r.suffix,'',if(l.suffix='' and r.suffix<>'','*NULL',l.suffix));
self.postdir := if(l.postdir=r.postdir,'',if(l.postdir='' and r.postdir<>'','*NULL',l.postdir));
self.unit_desig := if(l.unit_desig=r.unit_desig,'',if(l.unit_desig='' and r.unit_desig<>'','*NULL',l.unit_desig));
self.sec_range := if(l.sec_range=r.sec_range,'',if(l.sec_range='' and r.sec_range<>'','*NULL',l.sec_range));
self.city_name := if(l.city_name=r.city_name,'',if(l.city_name='' and r.city_name<>'','*NULL',l.city_name));
self.st := if(l.st=r.st,'',if(l.st='' and r.st<>'','*NULL',l.st));
self.zip := if(l.zip=r.zip,'',if(l.zip='' and r.zip<>'','*NULL',l.zip));
self.zip4 := if(l.zip4=r.zip4,'',if(l.zip4='' and r.zip4<>'','*NULL',l.zip4));
...
watchdog.Layout_Delta getDID(watchdog.Layout_Delta l, f r) := transform
...
result := get_deltas(assigned_did!=0 or phone!='' or ssn!='' or dob!=0 or fname!='' or

...
layout_persist_from_prod := RECORD
  string20 name_first;
  ...
  string5 site_zip;
  string4 site_zip4;
  unsigned integer1 name_indicator;
  unsigned integer6 did;
  unsigned integer1 did_score;
  string9 ssn;
  unsigned integer6 bdid;
  unsigned integer1 bdid_score;
  unsigned integer8 sequence;
...
file_in:= dataset('~thor::base::demo_data_file_foreclosure2_prodcopy',layout_persist_from_prod,flat);
...
mac_scramble_pii_v2( //** THE ORDER OF THE FIELD NAMES IS CRITICAL ***
      ...
      scrambled_file, 	//scrambled output data set attribute
		  ...
		  false,name1_ssn, 		//SSN field name
		  false,name1_did,  		// DID field name
		  false,name1_bdid,		// BDID field name
		  false,name1_first,
		  false,name1_middle,
		  false,name1_last,
...
self.situs_address_indicator_1 := '';
self.situs_house_number_prefix_1 :='';
self.situs_house_number_1 := clean_addr[1..8];
self.situs_house_number_suffix_1 := '';
self.situs_street_name_1 :=TRIM(clean_addr[10..38],LEFT,RIGHT);
self.situs_mode_1 := '';
self.situs_direction_1 := '';
self.apartment_unit := l.apartment_unit;
self.property_city_1 := l.situs1_v_city_name;
self.property_state_1 := l.situs1_st;
self.property_address_zip_code_1:= l.situs1_zip4;
self.full_site_address_unparsed_1 := trim(clean_addr[1..8])+  ' ' + TRIM(clean_addr[10..38],LEFT,RIGHT);
self:=l;
END;

export scramble_foreclosure2a := project(scrambled_file,reformat(LEFT));

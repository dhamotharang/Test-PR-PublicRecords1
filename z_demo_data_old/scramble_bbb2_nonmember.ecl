...
file_in:= dataset('~thor::base::demo_data_file_bbb2_nonmember_prodcopy', my_layout,flat);	// bbb2.Layout_nonmember_Base,flat);
...
self.non_member_title := l.company_name;
self := l;
end;

scrambled := project(scrambled_file, to_fix_bdids(left));

export scramble_bbb2_nonmember := dedup(sort(scrambled,record),all);
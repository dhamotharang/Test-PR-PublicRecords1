...
file_in:= dataset('~thor::base::demo_data_file_business_header_base_prodcopy', business_header.Layout_business_header_base,flat);
...
self.match_company_name := l.company_name;
self := l;
end;

scrambled := project(scrambled_file, to_wrap_up(left));

export scramble_business_header_base := dedup(sort(scrambled,record),all);

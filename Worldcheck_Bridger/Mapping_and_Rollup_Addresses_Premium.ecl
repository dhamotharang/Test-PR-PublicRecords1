...
export Mapping_and_Rollup_Addresses_Premium(dataset(Layout_WorldCheck_Premium) in_f):= function
...
CountriesOnly := concatAddr(Country <> '' and Street_1 = '' and Street_2 = '' and City = '' and State = '' and Postal_Code = '');
OtherAddr 	  := concatAddr(Street_1 <> '' or Street_2 <> '' or City <> '' or State <> '' or Postal_Code <> '');

	Layout_temp Xform(Layout_temp L,Layout_temp R) := TRANSFORM
		self.Type					:= '';
		self.Street_1			:= '';
		self.Street_2			:= '';
		self.City					:= '';
	...
	uniqueCountries := rollup(sort(CountriesOnly, ID, Country), left.id = right.id and left.country = right.country, Xform(LEFT, RIGHT));
	
	uniqueaddr1 := OtherAddr + uniqueCountries;	
	
	Layout_temp Xform1(Layout_temp L, Layout_temp R) := TRANSFORM
		self.Type					:= if(L.Type = '', R.Type, L.Type);
		self.Street_1			:= if(L.Street_1 = '', R.Street_1, L.Street_1);
		self.Street_2			:= if(L.Street_2 = '', R.Street_2, L.Street_2);
		self.City					:= if(L.City = '', R.City, L.City);
		self.State				:= if(L.State = '', R.State, L.State);
		self.Postal_Code	:= if(L.Postal_Code = '', R.Postal_Code, L.Postal_Code);
		self.Country			:= if(L.Country = '', R.Country, L.Country);
		self.Comments			:= if(L.Comments = '', R.Comments, L.Comments);
		self.ID						:= l.ID;
		self.addr_info		:= if(L.addr_info = '', R.addr_info, L.addr_info);
		self 							:= L;
	END;
...
/////////////////////////////////////////////////////////////////////////////////////////////////
	...
	Addr_rollup t_Addr(uniqueaddr L) := transform
		self.Address 	:= row(L, Layout_Addresses);
		self := L;
	end;

	p_Addr := project(uniqueaddr, t_Addr(left));

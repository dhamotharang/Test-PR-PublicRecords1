...
export Normalize_AKAs_and_AltSpell(dataset(Layout_WorldCheck_Premium) in_f):= function
...
//POPULATE ALTERNATIVE SPELLINGS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
	...
	Layout_temp aSpellTran(ds_NormASpell l):= transform
		self.First_Name		:= if(l.e_i_ind <> 'E' and regexfind(',', l.name_alias, 0)<>'',
									trim(l.name_alias[stringlib.stringfind(l.name_alias, ',', 1)+1..length(l.name_alias)], left, right),
									'');
		self.Last_Name 		:= if(l.e_i_ind <> 'E' and regexfind(',', l.name_alias, 0)<>'',
...
ds_reformASpell := project(ds_NormASpell, aSpellTran(left)): persist('~thor_200::persist::worldcheck::normalize_akas_altspell');

...
export Mapping_Search_Criteria_Headers_Premium_Plus(string filedate) := function
...
//BUILDINFO/////////////////////////////////////////////////////////////
	...
	//Sub-Category
	ds_SCategory := Worldcheck_Bridger.File_In_SubCategory_Premium;
	...
	ds_SCategory_p 		:= project(ds_SCategory, scatTran(left));
	ds_SCategory_val	:= sort(ds_SCategory_p(id<>'20'), name);
	ds_SCategory_nval	:= ds_SCategory_p(id='20');
	
	ds_SCategory_proj := ds_SCategory_nval + ds_SCategory_val; //'No Value' on top
...
//Country -------------------------------------------------------------------------
	...
	File_In_User := dataset([{'1', 0, reform_ds, ds_clientID, 'W64Bit'}], 
...
p_Build := project(slimInfo, t_Build(left)):persist('~thor_data200::persist::worldcheck_bridger::premium_plus_search'+filedate);

return output(p_Build,,'~thor_data200::base::worldcheck_bridger::premium_plus_search::'+filedate+'.xml', XML('MYROW', 

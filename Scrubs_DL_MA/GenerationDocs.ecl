...
Command line options: -MScrubs_DL_MA -eC:\Users\wolfkg\AppData\Local\Temp\TFR4EA9.tmp 
...
FIELDTYPE:invalid_8date:LENGTHS(8):CUSTOM(Scrubs.fn_valid_GeneralDate>0)
...
FIELDTYPE:invalid_pers_surrogate:LENGTHS(9):LIKE(invalid_numeric)
...
FIELDTYPE:invalid_mailzip:LENGTHS(9):LIKE(invalid_numeric)
FIELDTYPE:invalid_licresi_state:LENGTHS(0,2):CUSTOM(Scrubs_DL_MA.Functions.fn_verify_state>0)
FIELDTYPE:invalid_resizip:LENGTHS(0,9):LIKE(invalid_numeric)
FIELDTYPE:invalid_license_status:ALLOW(/ACENPRSTUVX):LENGTHS(3,7)
 
//FIELD DEFINITIONS
FIELD:pers_surrogate:TYPE(STRING9):LIKE(invalid_pers_surrogate):0,0

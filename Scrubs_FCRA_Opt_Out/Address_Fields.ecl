...
EXPORT MakeFT_Invalid_SSN_append(SALT311.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_SSN_append(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789-xX'))),~(LENGTH(TRIM(s)) >= 9 AND LENGTH(TRIM(s)) <= 11));
EXPORT InValidMessageFT_Invalid_SSN_append(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789-xX'),SALT311.HygieneErrors.NotLength('9..11'),SALT311.HygieneErrors.Good);

...
EXPORT SALT311.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'Invalid_SSN','Invalid_JullianDate','Invalid_Inname','Invalid_State','Invalid_Zip','Invalid_SSN_append','Invalid_Flag','Invalid_Date','Invalid_Blank');
EXPORT FieldTypeNum(SALT311.StrType fn) := CASE(fn,'Invalid_SSN' => 1,'Invalid_JullianDate' => 2,'Invalid_Inname' => 3,'Invalid_State' => 4,'Invalid_Zip' => 5,'Invalid_SSN_append' => 6,'Invalid_Flag' => 7,'Invalid_Date' => 8,'Invalid_Blank' => 9,0);
 
EXPORT MakeFT_Invalid_SSN(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'0123456789-xX'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_SSN(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789-xX'))));
EXPORT InValidMessageFT_Invalid_SSN(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789-xX'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_Invalid_JullianDate(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_JullianDate(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789'))));
EXPORT InValidMessageFT_Invalid_JullianDate(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_Invalid_Inname(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'ABCDEFGHIJKLMNOPQRSTUVWXYZ -\'/'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_Inname(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZ -\'/'))));
EXPORT InValidMessageFT_Invalid_Inname(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZ -\'/'),SALT311.HygieneErrors.Good);
...
EXPORT MakeFT_Invalid_Zip(SALT311.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_Zip(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789'))),~(LENGTH(TRIM(s)) = 5));
EXPORT InValidMessageFT_Invalid_Zip(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789'),SALT311.HygieneErrors.NotLength('5'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_Invalid_SSN_append(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'0123456789-xX'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_SSN_append(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789-xX'))),~(LENGTH(TRIM(s)) = 0 OR LENGTH(TRIM(s)) >= 9 AND LENGTH(TRIM(s)) <= 11));
EXPORT InValidMessageFT_Invalid_SSN_append(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789-xX'),SALT311.HygieneErrors.NotLength('0,9..11'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_Invalid_Flag(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_Invalid_Flag(SALT311.StrType s) := WHICH(((SALT311.StrType) s) NOT IN ['Y','N','y','n','']);
EXPORT InValidMessageFT_Invalid_Flag(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInEnum('Y|N|y|n|'),SALT311.HygieneErrors.Good);
...
EXPORT SALT311.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'z5','prim_range','prim_name','sec_range','ssn','did','source_flag','julian_date','inname_first','inname_last','address','city','state','zip5','did_score','ssn_append','permanent_flag','opt_back_in','date_yyyymmdd');
EXPORT SALT311.StrType FlatName(UNSIGNED2 i) := CHOOSE(i,'z5','prim_range','prim_name','sec_range','ssn','did','source_flag','julian_date','inname_first','inname_last','address','city','state','zip5','did_score','ssn_append','permanent_flag','opt_back_in','date_yyyymmdd');
EXPORT FieldNum(SALT311.StrType fn) := CASE(fn,'z5' => 0,'prim_range' => 1,'prim_name' => 2,'sec_range' => 3,'ssn' => 4,'did' => 5,'source_flag' => 6,'julian_date' => 7,'inname_first' => 8,'inname_last' => 9,'address' => 10,'city' => 11,'state' => 12,'zip5' => 13,'did_score' => 14,'ssn_append' => 15,'permanent_flag' => 16,'opt_back_in' => 17,'date_yyyymmdd' => 18,0);
EXPORT SET OF SALT311.StrType FieldRules(UNSIGNED2 i) := CHOOSE(i,['ALLOW','LENGTHS'],['LENGTHS'],['LENGTHS'],[],['ALLOW'],[],[],['ALLOW'],['ALLOW'],['ALLOW'],[],[],['ALLOW','LENGTHS'],['ALLOW','LENGTHS'],[],['ALLOW','LENGTHS'],['ENUM'],['ENUM'],['CUSTOM'],[]);
...
EXPORT Make_z5(SALT311.StrType s0) := MakeFT_Invalid_Zip(s0);
EXPORT InValid_z5(SALT311.StrType s) := InValidFT_Invalid_Zip(s);
EXPORT InValidMessage_z5(UNSIGNED1 wh) := InValidMessageFT_Invalid_Zip(wh);
 
EXPORT Make_prim_range(SALT311.StrType s0) := MakeFT_Invalid_Blank(s0);
EXPORT InValid_prim_range(SALT311.StrType s) := InValidFT_Invalid_Blank(s);
EXPORT InValidMessage_prim_range(UNSIGNED1 wh) := InValidMessageFT_Invalid_Blank(wh);
 
EXPORT Make_prim_name(SALT311.StrType s0) := MakeFT_Invalid_Blank(s0);
EXPORT InValid_prim_name(SALT311.StrType s) := InValidFT_Invalid_Blank(s);
...
EXPORT Make_source_flag(SALT311.StrType s0) := s0;
EXPORT InValid_source_flag(SALT311.StrType s) := 0;
EXPORT InValidMessage_source_flag(UNSIGNED1 wh) := '';
 
EXPORT Make_julian_date(SALT311.StrType s0) := MakeFT_Invalid_JullianDate(s0);
EXPORT InValid_julian_date(SALT311.StrType s) := InValidFT_Invalid_JullianDate(s);
EXPORT InValidMessage_julian_date(UNSIGNED1 wh) := InValidMessageFT_Invalid_JullianDate(wh);
 
EXPORT Make_inname_first(SALT311.StrType s0) := MakeFT_Invalid_Inname(s0);
EXPORT InValid_inname_first(SALT311.StrType s) := InValidFT_Invalid_Inname(s);
EXPORT InValidMessage_inname_first(UNSIGNED1 wh) := InValidMessageFT_Invalid_Inname(wh);
 
EXPORT Make_inname_last(SALT311.StrType s0) := MakeFT_Invalid_Inname(s0);
EXPORT InValid_inname_last(SALT311.StrType s) := InValidFT_Invalid_Inname(s);
...
EXPORT Make_ssn_append(SALT311.StrType s0) := MakeFT_Invalid_SSN_append(s0);
EXPORT InValid_ssn_append(SALT311.StrType s) := InValidFT_Invalid_SSN_append(s);
EXPORT InValidMessage_ssn_append(UNSIGNED1 wh) := InValidMessageFT_Invalid_SSN_append(wh);
 
EXPORT Make_permanent_flag(SALT311.StrType s0) := MakeFT_Invalid_Flag(s0);
EXPORT InValid_permanent_flag(SALT311.StrType s) := InValidFT_Invalid_Flag(s);
EXPORT InValidMessage_permanent_flag(UNSIGNED1 wh) := InValidMessageFT_Invalid_Flag(wh);
 
EXPORT Make_opt_back_in(SALT311.StrType s0) := MakeFT_Invalid_Flag(s0);
EXPORT InValid_opt_back_in(SALT311.StrType s) := InValidFT_Invalid_Flag(s);
EXPORT InValidMessage_opt_back_in(UNSIGNED1 wh) := InValidMessageFT_Invalid_Flag(wh);
 
EXPORT Make_date_yyyymmdd(SALT311.StrType s0) := MakeFT_Invalid_Date(s0);
EXPORT InValid_date_yyyymmdd(SALT311.StrType s) := InValidFT_Invalid_Date(s);

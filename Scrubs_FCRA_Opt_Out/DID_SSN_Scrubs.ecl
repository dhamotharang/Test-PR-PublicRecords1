...
// The module to handle the case where no scrubs exist
  ...
  SHARED toErrorMessage(UNSIGNED c) := CHOOSE(c
          ...
          ,DID_SSN_Fields.InvalidMessage_ssn_append(1)
...
EXPORT FromNone(DATASET(DID_SSN_Layout_FCRA_Opt_Out) h) := MODULE
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.ssn_Invalid << 0 ) + ( le.julian_date_Invalid << 1 ) + ( le.inname_first_Invalid << 3 ) + ( le.inname_last_Invalid << 4 ) + ( le.state_Invalid << 5 ) + ( le.zip5_Invalid << 7 ) + ( le.ssn_append_Invalid << 9 ) + ( le.permanent_flag_Invalid << 10 ) + ( le.opt_back_in_Invalid << 11 ) + ( le.date_yyyymmdd_Invalid << 12 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.ssn_append_Invalid := (le.ScrubsBits1 >> 9) & 1;
    SELF.permanent_flag_Invalid := (le.ScrubsBits1 >> 10) & 1;
    SELF.opt_back_in_Invalid := (le.ScrubsBits1 >> 11) & 1;
    SELF.date_yyyymmdd_Invalid := (le.ScrubsBits1 >> 12) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    ...
    SELF.Rules_WithErrors := IF(le.ssn_ALLOW_ErrorCount > 0, 1, 0) + IF(le.julian_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.julian_date_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.inname_first_ALLOW_ErrorCount > 0, 1, 0) + IF(le.inname_last_ALLOW_ErrorCount > 0, 1, 0) + IF(le.state_ALLOW_ErrorCount > 0, 1, 0) + IF(le.state_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.zip5_ALLOW_ErrorCount > 0, 1, 0) + IF(le.zip5_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.ssn_append_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.permanent_flag_ENUM_ErrorCount > 0, 1, 0) + IF(le.opt_back_in_ENUM_ErrorCount > 0, 1, 0) + IF(le.date_yyyymmdd_CUSTOM_ErrorCount > 0, 1, 0);

...
dsDocketSearchRecs 		:= normalize(base_recs_seeded, SearchTypeInUse, norm2Search(left, counter),local);

dsDocketMPHSearchRecs := dedup(sort(project(	dsDocketSearchRecs, 
																							transform(MXDocketSearchMPH,
...
export Key_DocketMPHSearch := index(dsDocketMPHSearchRecs,
																		{search_type, n1_m1, n1_m2, n2_m1, n2_m2, n3_m1, n3_m2, minit, nature_type, year, state, gender},															 
																		{entity_id, rec_id},
																		Data_Services.Data_location.Prefix('MXData')+'thor_data400::key::mxd_mxdocket::'+doxie.Version_SuperKey+'::docket_mph_search_idx');
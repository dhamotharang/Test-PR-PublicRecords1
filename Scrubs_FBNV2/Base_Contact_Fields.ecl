...
EXPORT Base_Contact_Fields := MODULE
...
EXPORT SALT37.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'contact_name','contact_addr','contact_phone','prep_addr_line1','prep_addr_line_last');
EXPORT FieldNum(SALT37.StrType fn) := CASE(fn,'contact_name' => 0,'contact_addr' => 1,'contact_phone' => 2,'prep_addr_line1' => 3,'prep_addr_line_last' => 4,0);
 
//Individual field level validation
 
EXPORT Make_contact_name(SALT37.StrType s0) := MakeFT_invalid_mandatory(s0);
EXPORT InValid_contact_name(SALT37.StrType s) := InValidFT_invalid_mandatory(s);
EXPORT InValidMessage_contact_name(UNSIGNED1 wh) := InValidMessageFT_invalid_mandatory(wh);
 
EXPORT Make_contact_addr(SALT37.StrType s0) := MakeFT_invalid_mandatory(s0);
EXPORT InValid_contact_addr(SALT37.StrType s) := InValidFT_invalid_mandatory(s);
EXPORT InValidMessage_contact_addr(UNSIGNED1 wh) := InValidMessageFT_invalid_mandatory(wh);
 
EXPORT Make_contact_phone(SALT37.StrType s0) := MakeFT_invalid_mandatory(s0);
EXPORT InValid_contact_phone(SALT37.StrType s) := InValidFT_invalid_mandatory(s);
EXPORT InValidMessage_contact_phone(UNSIGNED1 wh) := InValidMessageFT_invalid_mandatory(wh);
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Diff_contact_addr := le.contact_addr <> ri.contact_addr;
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_contact_name,1,0)+ IF( SELF.Diff_contact_addr,1,0)+ IF( SELF.Diff_contact_phone,1,0)+ IF( SELF.Diff_prep_addr_line1,1,0)+ IF( SELF.Diff_prep_addr_line_last,1,0);
  END;
...
#uniquename(AggRec)
  %AggRec% := RECORD
    ...
    Count_Diff_contact_addr := COUNT(GROUP,%Closest%.Diff_contact_addr);

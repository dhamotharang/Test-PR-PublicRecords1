...
EXPORT Summary(SALT311.Str30Type  txt) := FUNCTION
  ...
  R1 := RECORD
    UNSIGNED LinkingPotential :=  + T.Populated_FILE_NUMBER_pcnt *   0.00 / 100 + T.Populated_NAME1_pcnt *   0.00 / 100 + T.Populated_NAME2_pcnt *   0.00 / 100 + T.Populated_prep_addr1_line1_pcnt *   0.00 / 100 + T.Populated_prep_addr1_line_last_pcnt *   0.00 / 100 + T.Populated_prep_addr2_line1_pcnt *   0.00 / 100 + T.Populated_prep_addr2_line_last_pcnt *   0.00 / 100;
    T;
  END;
  RETURN TABLE(T,R1);
END;
...
invRec invert(summary0 le, INTEGER C) := TRANSFORM
  ...
  SELF.FieldName := CHOOSE(C,'FILE_NUMBER','NAME1','NAME2','prep_addr1_line1','prep_addr1_line_last','prep_addr2_line1','prep_addr2_line_last');
  SELF.populated_pcnt := CHOOSE(C,le.populated_FILE_NUMBER_pcnt,le.populated_NAME1_pcnt,le.populated_NAME2_pcnt,le.populated_prep_addr1_line1_pcnt,le.populated_prep_addr1_line_last_pcnt,le.populated_prep_addr2_line1_pcnt,le.populated_prep_addr2_line_last_pcnt);
  SELF.maxlength := CHOOSE(C,le.maxlength_FILE_NUMBER,le.maxlength_NAME1,le.maxlength_NAME2,le.maxlength_prep_addr1_line1,le.maxlength_prep_addr1_line_last,le.maxlength_prep_addr2_line1,le.maxlength_prep_addr2_line_last);
  SELF.avelength := CHOOSE(C,le.avelength_FILE_NUMBER,le.avelength_NAME1,le.avelength_NAME2,le.avelength_prep_addr1_line1,le.avelength_prep_addr1_line_last,le.avelength_prep_addr2_line1,le.avelength_prep_addr2_line_last);
END;
EXPORT invSummary := NORMALIZE(summary0, 7, invert(LEFT,COUNTER));
// The character counts
// Move everything into 'inverted list' form so processing can be done 'in library'
SALT311.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT311.StrType)le.FILE_NUMBER),TRIM((SALT311.StrType)le.NAME1),TRIM((SALT311.StrType)le.NAME2),TRIM((SALT311.StrType)le.prep_addr1_line1),TRIM((SALT311.StrType)le.prep_addr1_line_last),TRIM((SALT311.StrType)le.prep_addr2_line1),TRIM((SALT311.StrType)le.prep_addr2_line_last)));
  SELF.FldNo := C;
END;
...
SHARED FldIds := DATASET([{1,'FILE_NUMBER'}
      ...
      ,{3,'NAME2'}
      ,{4,'prep_addr1_line1'}
      ,{5,'prep_addr1_line_last'}
      ,{6,'prep_addr2_line1'}
      ,{7,'prep_addr2_line_last'}],SALT311.MAC_Character_Counts.Field_Identification);
...
PrettyErrorTotals := RECORD
  ...
  FieldType := CHOOSE(TotalErrors.FieldNum,'invalid_mandatory','invalid_mandatory','invalid_mandatory','invalid_mandatory','invalid_mandatory','invalid_mandatory','invalid_mandatory');
  ErrorMessage := CHOOSE(TotalErrors.FieldNum,Input_TX_Harris_Fields.InValidMessage_FILE_NUMBER(TotalErrors.ErrorNum),Input_TX_Harris_Fields.InValidMessage_NAME1(TotalErrors.ErrorNum),Input_TX_Harris_Fields.InValidMessage_NAME2(TotalErrors.ErrorNum),Input_TX_Harris_Fields.InValidMessage_prep_addr1_line1(TotalErrors.ErrorNum),Input_TX_Harris_Fields.InValidMessage_prep_addr1_line_last(TotalErrors.ErrorNum),Input_TX_Harris_Fields.InValidMessage_prep_addr2_line1(TotalErrors.ErrorNum),Input_TX_Harris_Fields.InValidMessage_prep_addr2_line_last(TotalErrors.ErrorNum));
  TotalErrors.Cnt;
END;

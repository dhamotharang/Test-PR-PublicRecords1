...
#workunit('name','Scrubs_FBNV2.Input_FL_Event_BWR_Hygiene - Hygiene & Stats - SALT V3.11.4');
IMPORT Scrubs_FBNV2,SALT311;
// First create an instantiated hygiene module
  infile := Scrubs_FBNV2.Input_FL_Event_In_FBNV2;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_FBNV2.Input_FL_Event_hygiene(ip);
  ...
  OUTPUT(SALT311.MAC_Character_Counts.EclRecord(p,'Input_FL_Event_Layout_FBNV2'),NAMED('OptimizedLayout'));// File layout suggested by data

...
EXPORT SALT37.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'business_name','file_date','regis_nbr','prep_bus_addr_line1','prep_bus_addr_line_last','prep_owner_addr_line1','prep_owner_addr_line_last','cname','first_name','middle_name','last_name_company','owner_address','owner_city','owner_state','phone_nbr');
EXPORT FieldNum(SALT37.StrType fn) := CASE(fn,'business_name' => 0,'file_date' => 1,'regis_nbr' => 2,'prep_bus_addr_line1' => 3,'prep_bus_addr_line_last' => 4,'prep_owner_addr_line1' => 5,'prep_owner_addr_line_last' => 6,'cname' => 7,'first_name' => 8,'middle_name' => 9,'last_name_company' => 10,'owner_address' => 11,'owner_city' => 12,'owner_state' => 13,'phone_nbr' => 14,0);
...
EXPORT Make_owner_city(SALT37.StrType s0) := MakeFT_invalid_mandatory(s0);
EXPORT InValid_owner_city(SALT37.StrType s) := InValidFT_invalid_mandatory(s);
EXPORT InValidMessage_owner_city(UNSIGNED1 wh) := InValidMessageFT_invalid_mandatory(wh);
 
EXPORT Make_owner_state(SALT37.StrType s0) := MakeFT_invalid_mandatory(s0);
EXPORT InValid_owner_state(SALT37.StrType s) := InValidFT_invalid_mandatory(s);
EXPORT InValidMessage_owner_state(UNSIGNED1 wh) := InValidMessageFT_invalid_mandatory(wh);

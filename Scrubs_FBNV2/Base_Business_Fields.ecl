...
EXPORT Base_Business_Fields := MODULE
...
EXPORT SALT37.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'bus_name','filing_jurisdiction','filing_date','filing_number','filing_type','prep_addr_line1','prep_addr_line_last');
EXPORT FieldNum(SALT37.StrType fn) := CASE(fn,'bus_name' => 0,'filing_jurisdiction' => 1,'filing_date' => 2,'filing_number' => 3,'filing_type' => 4,'prep_addr_line1' => 5,'prep_addr_line_last' => 6,0);
 
//Individual field level validation
 
EXPORT Make_bus_name(SALT37.StrType s0) := MakeFT_invalid_mandatory(s0);
EXPORT InValid_bus_name(SALT37.StrType s) := InValidFT_invalid_mandatory(s);
EXPORT InValidMessage_bus_name(UNSIGNED1 wh) := InValidMessageFT_invalid_mandatory(wh);
 
EXPORT Make_filing_jurisdiction(SALT37.StrType s0) := MakeFT_invalid_mandatory(s0);
EXPORT InValid_filing_jurisdiction(SALT37.StrType s) := InValidFT_invalid_mandatory(s);
EXPORT InValidMessage_filing_jurisdiction(UNSIGNED1 wh) := InValidMessageFT_invalid_mandatory(wh);
 
EXPORT Make_filing_date(SALT37.StrType s0) := MakeFT_invalid_general_date(s0);
EXPORT InValid_filing_date(SALT37.StrType s) := InValidFT_invalid_general_date(s);
EXPORT InValidMessage_filing_date(UNSIGNED1 wh) := InValidMessageFT_invalid_general_date(wh);
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    SELF.Diff_bus_name := le.bus_name <> ri.bus_name;
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_bus_name,1,0)+ IF( SELF.Diff_filing_jurisdiction,1,0)+ IF( SELF.Diff_filing_date,1,0)+ IF( SELF.Diff_filing_number,1,0)+ IF( SELF.Diff_filing_type,1,0)+ IF( SELF.Diff_prep_addr_line1,1,0)+ IF( SELF.Diff_prep_addr_line_last,1,0);
  END;
...
#uniquename(AggRec)
  %AggRec% := RECORD
    Count_Diff_bus_name := COUNT(GROUP,%Closest%.Diff_bus_name);

...
EXPORT SALT30.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'multiword','Noblanks','not_hrchy');
EXPORT FieldTypeNum(SALT30.StrType fn) := CASE(fn,'multiword' => 1,'Noblanks' => 2,'not_hrchy' => 3,0);
...
EXPORT MakeFT_not_hrchy(SALT30.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_not_hrchy(SALT30.StrType s) := WHICH(((SALT30.StrType) s) NOT IN ['0','1']);
EXPORT InValidMessageFT_not_hrchy(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInEnum('0|1'),SALT30.HygieneErrors.Good);
 
EXPORT SALT30.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'nodes_total','sbfe_id','source_record_id','company_name','cnp_number','active_duns_number','duns_number','duns_number_concept','company_fein','company_inc_state','company_charter_number','cnp_btype','company_name_type_derived','hist_duns_number','active_domestic_corp_key','hist_domestic_corp_key','foreign_corp_key','unk_corp_key','cnp_name','cnp_hasNumber','cnp_lowv','cnp_translated','cnp_classid','prim_range','prim_name','sec_range','v_city_name','st','zip','dt_first_seen','dt_last_seen');
EXPORT FieldNum(SALT30.StrType fn) := CASE(fn,'nodes_total' => 1,'sbfe_id' => 2,'source_record_id' => 3,'company_name' => 4,'cnp_number' => 5,'active_duns_number' => 6,'duns_number' => 7,'duns_number_concept' => 8,'company_fein' => 9,'company_inc_state' => 10,'company_charter_number' => 11,'cnp_btype' => 12,'company_name_type_derived' => 13,'hist_duns_number' => 14,'active_domestic_corp_key' => 15,'hist_domestic_corp_key' => 16,'foreign_corp_key' => 17,'unk_corp_key' => 18,'cnp_name' => 19,'cnp_hasNumber' => 20,'cnp_lowv' => 21,'cnp_translated' => 22,'cnp_classid' => 23,'prim_range' => 24,'prim_name' => 25,'sec_range' => 26,'v_city_name' => 27,'st' => 28,'zip' => 29,'dt_first_seen' => 30,'dt_last_seen' => 31,0);
 
//Individual field level validation
 
EXPORT Make_nodes_total(SALT30.StrType s0) := MakeFT_not_hrchy(s0);
EXPORT InValid_nodes_total(SALT30.StrType s) := InValidFT_not_hrchy(s);
EXPORT InValidMessage_nodes_total(UNSIGNED1 wh) := InValidMessageFT_not_hrchy(wh);
...
EXPORT Make_source_record_id(SALT30.StrType s0) := s0;
EXPORT InValid_source_record_id(SALT30.StrType s) := FALSE;
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT30,BIPV2_LGID3_dev2;
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_nodes_total,1,0)+ IF( SELF.Diff_sbfe_id,1,0)+ IF( SELF.Diff_source_record_id,1,0)+ IF( SELF.Diff_company_name,1,0)+ IF( SELF.Diff_cnp_number,1,0)+ IF( SELF.Diff_active_duns_number,1,0)+ IF( SELF.Diff_duns_number,1,0)+ IF( SELF.Diff_company_fein,1,0)+ IF( SELF.Diff_company_inc_state,1,0)+ IF( SELF.Diff_company_charter_number,1,0)+ IF( SELF.Diff_cnp_btype,1,0)+ IF( SELF.Diff_company_name_type_derived,1,0)+ IF( SELF.Diff_hist_duns_number,1,0)+ IF( SELF.Diff_active_domestic_corp_key,1,0)+ IF( SELF.Diff_hist_domestic_corp_key,1,0)+ IF( SELF.Diff_foreign_corp_key,1,0)+ IF( SELF.Diff_unk_corp_key,1,0)+ IF( SELF.Diff_cnp_name,1,0)+ IF( SELF.Diff_cnp_hasNumber,1,0)+ IF( SELF.Diff_cnp_lowv,1,0)+ IF( SELF.Diff_cnp_translated,1,0)+ IF( SELF.Diff_cnp_classid,1,0)+ IF( SELF.Diff_prim_range,1,0)+ IF( SELF.Diff_prim_name,1,0)+ IF( SELF.Diff_sec_range,1,0)+ IF( SELF.Diff_v_city_name,1,0)+ IF( SELF.Diff_st,1,0)+ IF( SELF.Diff_zip,1,0);
  END;

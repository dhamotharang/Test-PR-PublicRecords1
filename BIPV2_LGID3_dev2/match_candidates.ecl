...
SHARED thin_table := DISTRIBUTE(TABLE(h00(SALT_Partition<>'*'),{rcid,nodes_total,sbfe_id,source_record_id,company_name,cnp_number,active_duns_number,duns_number,duns_number_concept,company_fein,company_inc_state,company_charter_number,cnp_btype,company_name_type_derived,hist_duns_number,active_domestic_corp_key,hist_domestic_corp_key,foreign_corp_key,unk_corp_key,cnp_name,cnp_hasNumber,cnp_lowv,cnp_translated,cnp_classid,prim_range,prim_name,sec_range,v_city_name,st,zip,dt_first_seen,dt_last_seen,SALT_Partition,ultid,orgid,seleid,LGID3}),HASH(LGID3));
...
layout_withpropvars take_company_name(with_props le,company_name_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj3 := JOIN(with_props,company_name_props,left.LGID3=right.LGID3,take_company_name(left,right),LEFT OUTER,HASH/*,HINT(parallel_match)*//*HACK to prevent memory limit exceeded error*/);
...
layout_withpropvars take_cnp_number(with_props le,cnp_number_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj4 := JOIN(pj3,cnp_number_props,left.LGID3=right.LGID3,take_cnp_number(left,right),LEFT OUTER,HASH/*,HINT(parallel_match)*//*HACK to prevent memory limit exceeded error*/);
...
layout_withpropvars take_active_duns_number(with_props le,active_duns_number_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj5 := JOIN(pj4,active_duns_number_props,left.LGID3=right.LGID3,take_active_duns_number(left,right),LEFT OUTER,HASH/*,HINT(parallel_match)*//*HACK to prevent memory limit exceeded error*/);
...
layout_withpropvars take_duns_number(with_props le,duns_number_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj6 := JOIN(pj5,duns_number_props,left.LGID3=right.LGID3,take_duns_number(left,right),LEFT OUTER,HASH/*,HINT(parallel_match)*//*HACK to prevent memory limit exceeded error*/);
...
layout_withpropvars take_company_inc_state(with_props le,company_inc_state_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj9 := JOIN(pj6,company_inc_state_props,left.LGID3=right.LGID3,take_company_inc_state(left,right),LEFT OUTER,HASH/*,HINT(parallel_match)*//*HACK to prevent memory limit exceeded error*/);
...
layout_withpropvars take_company_charter_number(with_props le,company_charter_number_props ri) := TRANSFORM
  ...
  SELF := le;
  END;
SHARED pj10 := JOIN(pj9,company_charter_number_props,left.LGID3=right.LGID3,take_company_charter_number(left,right),LEFT OUTER,HASH/*,HINT(parallel_match)*//*HACK to prevent memory limit exceeded error*/);
 
pj10 do_computes(pj10 le) := TRANSFORM
  ...
  SELF := le;
END;
SHARED propogated := PROJECT(pj10,do_computes(left)) : PERSIST('~temp::LGID3::BIPV2_LGID3_dev2::mc_props::LGID3',EXPIRE(Config.PersistExpire)); // to allow to 'jump' over an exported value
...
layout_candidates add_source_record_id(layout_candidates le,Specificities(ih).source_record_id_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT30.MAC_Choose_JoinType(j3,s.nulls_source_record_id,Specificities(ih).source_record_id_values_persisted,source_record_id,source_record_id_weight100,add_source_record_id,j2);
layout_candidates add_sbfe_id(layout_candidates le,Specificities(ih).sbfe_id_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT30.MAC_Choose_JoinType(j2,s.nulls_sbfe_id,Specificities(ih).sbfe_id_values_persisted,sbfe_id,sbfe_id_weight100,add_sbfe_id,j1);

...
SHARED  h00 := Specificities(ih).input_file;
  ...
  SHARED h00_match := h00( 
      0 + IF( sbfe_id  IN SET(s.nulls_sbfe_id,sbfe_id), 0, 26 ) + IF( source_record_id  IN SET(s.nulls_source_record_id,source_record_id), 0, 26 ) + IF( company_name  IN SET(s.nulls_company_name,company_name), 0, 26 ) + IF( cnp_number  IN SET(s.nulls_cnp_number,cnp_number), 0, 13 ) + IF( active_duns_number  IN SET(s.nulls_active_duns_number,active_duns_number), 0, 27 ) + IF( duns_number  IN SET(s.nulls_duns_number,duns_number), 0, 26 ) + IF( company_fein  IN SET(s.nulls_company_fein,company_fein), 0, 26 ) + IF( company_inc_state  IN SET(s.nulls_company_inc_state,company_inc_state), 0, 6 ) + IF( company_charter_number  IN SET(s.nulls_company_charter_number,company_charter_number), 0, 26 ) + IF( cnp_btype  IN SET(s.nulls_cnp_btype,cnp_btype), 0, 3 ) >= Config.BasicMatchThreshold); // Potentially remove anything that would violate FORCE(+) constraints
...
/* // It is important that this is an EQUIVALENCE relationship - it allows us to form an implicit transitive closure
  h01 := SORT(h00_match,SALT_Partition,sbfe_id,source_record_id,company_name,cnp_number,active_duns_number,duns_number,company_fein,company_inc_state,company_charter_number,cnp_btype,LGID3);
  h02 := DEDUP(h01,SALT_Partition,sbfe_id,source_record_id,company_name,cnp_number,active_duns_number,duns_number,company_fein,company_inc_state,company_charter_number,cnp_btype,LOCAL); // ,LOCAL ok - we don't need a perfect dedup - this is an optimization
  Match := JOIN(h02,MatchCands,LEFT.sbfe_id = RIGHT.sbfe_id AND LEFT.source_record_id = RIGHT.source_record_id AND LEFT.sbfe_id = RIGHT.sbfe_id AND LEFT.company_name = RIGHT.company_name AND LEFT.cnp_number = RIGHT.cnp_number AND LEFT.active_duns_number = RIGHT.active_duns_number

EXPORT Delta(DATASET(Layout_LGID3)old_s, DATASET(Layout_LGID3) new_s) := MODULE//Routines to compute the differences between two instances of a file
  ...
  Diff_Layout Take_Record(old_s le, new_s ri) := TRANSFORM
    ...
    SELF.Changed := MAP ( le.rcid = (TYPEOF(le.rcid))'' OR ri.rcid = (TYPEOF(ri.rcid))'' => FALSE,
       le.nodes_total<>ri.nodes_total OR  le.sbfe_id<>ri.sbfe_id OR  le.source_record_id<>ri.source_record_id OR  le.company_name<>ri.company_name OR  le.cnp_number<>ri.cnp_number OR  le.active_duns_number<>ri.active_duns_number OR  le.duns_number<>ri.duns_number OR  le.company_fein<>ri.company_fein OR  le.company_inc_state<>ri.company_inc_state OR  le.company_charter_number<>ri.company_charter_number OR  le.cnp_btype<>ri.cnp_btype OR  le.company_name_type_derived<>ri.company_name_type_derived OR  le.hist_duns_number<>ri.hist_duns_number OR  le.active_domestic_corp_key<>ri.active_domestic_corp_key OR  le.hist_domestic_corp_key<>ri.hist_domestic_corp_key OR  le.foreign_corp_key<>ri.foreign_corp_key OR  le.unk_corp_key<>ri.unk_corp_key OR  le.cnp_name<>ri.cnp_name OR  le.cnp_hasNumber<>ri.cnp_hasNumber OR  le.cnp_lowv<>ri.cnp_lowv OR  le.cnp_translated<>ri.cnp_translated OR  le.cnp_classid<>ri.cnp_classid OR  le.prim_range<>ri.prim_range OR  le.prim_name<>ri.prim_name OR  le.sec_range<>ri.sec_range OR  le.v_city_name<>ri.v_city_name OR  le.st<>ri.st OR  le.zip<>ri.zip OR  le.dt_first_seen<>ri.dt_first_seen OR  le.dt_last_seen<>ri.dt_last_seen => TRUE,
      SKIP );

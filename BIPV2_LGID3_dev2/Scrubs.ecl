...
EXPORT FromNone(DATASET(Layout_LGID3) h) := MODULE
  Expanded_Layout into(h le) := TRANSFORM
    SELF.nodes_total_Invalid := Fields.InValid_nodes_total((SALT30.StrType)le.nodes_total);
    SELF.company_name_Invalid := Fields.InValid_company_name((SALT30.StrType)le.company_name);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,Into(LEFT));
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.nodes_total_Invalid << 0 ) + ( le.company_name_Invalid << 1 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    SELF.nodes_total_Invalid := (le.ScrubsBits1 >> 0) & 1;
    SELF.company_name_Invalid := (le.ScrubsBits1 >> 1) & 1;
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,Into(LEFT));
    FromNew := FromNone(InFile).ExpandedInfile;
  EXPORT Changed := JOIN(ExpandedInfile,FromNew, LEFT.rcid=RIGHT.rcid AND (LEFT.nodes_total_Invalid <> RIGHT.nodes_total_Invalid OR LEFT.company_name_Invalid <> RIGHT.company_name_Invalid),TRANSFORM(RIGHT));
END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    SELF.Src :=  le.source;
    UNSIGNED1 ErrNum := CHOOSE(c,le.nodes_total_Invalid,le.company_name_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_nodes_total(le.nodes_total_Invalid),Fields.InvalidMessage_company_name(le.company_name_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ,CHOOSE(le.nodes_total_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.company_name_Invalid,'LENGTH','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'nodes_total','company_name','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'not_hrchy','Noblanks','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT30.StrType)le.nodes_total,(SALT30.StrType)le.company_name,'***SALTBUG***');
  END;

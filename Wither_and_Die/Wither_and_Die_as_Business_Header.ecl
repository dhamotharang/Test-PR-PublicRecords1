...
wither_in := wither_and_die.File_Wither_and_Die_In;
...
Business_Header.Layout_Business_Header  Translate_Wither_to_BHF(Layout_wither_Local l, integer c) := transform
...
self.city 		:= choose(c, l.bus_p_city_name,      l.mail_p_city_name,     l.bus_p_city_name,     l.mail_p_city_name);
...
Layout_wither_entity := record
Business_Header.Layout_Business_Header;
unsigned2 entity_type;
end;


Layout_wither_entity AddEntity(Business_Header.Layout_Business_Header L) := transform
self := L;
...
Wither_Entity := project(from_wither_norm(company_name != ''), AddEntity(left));

// ----------------------------------------
// -- Back to bus header layout
// ----------------------------------------
Business_Header.Layout_Business_Header  Translate_Entity_to_BHF(Layout_wither_entity l) := transform
	self := L;
end;
Wither_bus := project(Wither_entity(entity_type = 1 or entity_type = 2), Translate_Entity_to_BHF(left));
...
Business_Header.Layout_Business_Header RollupWitherNorm(Business_Header.Layout_Business_Header l, Business_Header.Layout_Business_Header r) := transform
self := l;
end;
...
// Rollup
Business_Header.Layout_Business_Header Rollupwither(Business_Header.Layout_Business_Header L, Business_Header.Layout_Business_Header R) := TRANSForM
self.dt_first_seen := 
...
export wither_and_die_As_Business_Header := wither_clean_rollup  : persist('TEMP::wither_As_Business_Header');
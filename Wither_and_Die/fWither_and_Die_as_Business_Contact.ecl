...
export fWither_and_Die_as_Business_Contact(dataset(wither_and_die.Layout_Wither_and_Die_In) pWither_And_Die)
 :=
  function
...
//*************************************************************************
	...
	Wither_init := pWither_And_Die;

	Business_Header.Layout_Business_Contact_Full_New Translate_Wither_to_BCF(Layout_Wither_and_Die_In L, INTEGER C) := TRANSFORM
	SELF.bdid := 0;
	self.vl_id := '';
	...
	SELF.company_city 		:= CHOOSE(C, L.bus_v_city_name, 	L.mail_v_city_name,    L.bus_v_city_name, 	L.mail_v_city_name, 
								   L.bus_v_city_name, 	L.mail_v_city_name,    L.bus_v_city_name, 	L.mail_v_city_name);
	...
	wither_contacts_filtered	:=	Wither_Contacts((INTEGER)name_score < 3, Business_Header.CheckPersonName(fname, mname, lname, name_suffix), company_name != '', company_prim_name != '');
	
	return wither_contacts_filtered;
  end
 ;

IMPORT ut,SALT33;
EXPORT incident_text_Fields := MODULE
...
EXPORT MakeFT_Invalid_Field(SALT33.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_Invalid_Field(SALT33.StrType s) := WHICH(((SALT33.StrType) s) NOT IN ['OTHERINFO','INCIDENT_TEXT','INCIDENT_RESPONSE','']);
EXPORT InValidMessageFT_Invalid_Field(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInEnum('OTHERINFO|INCIDENT_TEXT|INCIDENT_RESPONSE|'),SALT33.HygieneErrors.Good);
EXPORT SALT33.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'batch','dbcode','incident_num','seq','field_name','field_txt');
EXPORT FieldNum(SALT33.StrType fn) := CASE(fn,'batch' => 0,'dbcode' => 1,'incident_num' => 2,'seq' => 3,'field_name' => 4,'field_txt' => 5,0);
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_batch,1,0)+ IF( SELF.Diff_dbcode,1,0)+ IF( SELF.Diff_incident_num,1,0)+ IF( SELF.Diff_seq,1,0)+ IF( SELF.Diff_field_name,1,0)+ IF( SELF.Diff_field_txt,1,0);
  END;

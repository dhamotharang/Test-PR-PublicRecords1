IMPORT ut,SALT33;
EXPORT party_text_hygiene(dataset(party_text_layout_SANCTN_NPKeys) h) := MODULE
...
EXPORT Summary(SALT33.Str30Type txt,BOOLEAN Glob=TRUE) := FUNCTION
  ...
  R1 := RECORD
    UNSIGNED LinkingPotential :=  + T.Populated_batch_pcnt *   0.00 / 100 + T.Populated_dbcode_pcnt *   0.00 / 100 + T.Populated_incident_num_pcnt *   0.00 / 100 + T.Populated_party_num_pcnt *   0.00 / 100 + T.Populated_seq_pcnt *   0.00 / 100 + T.Populated_field_name_pcnt *   0.00 / 100 + T.Populated_field_txt_pcnt *   0.00 / 100;
    T;
  END;
  RETURN TABLE(T,R1);
END;
...
invRec invert(summary0 le, INTEGER C) := TRANSFORM
  ...
  SELF.FieldName := CHOOSE(C,'batch','dbcode','incident_num','party_num','seq','field_name','field_txt');
  SELF.populated_pcnt := CHOOSE(C,le.populated_batch_pcnt,le.populated_dbcode_pcnt,le.populated_incident_num_pcnt,le.populated_party_num_pcnt,le.populated_seq_pcnt,le.populated_field_name_pcnt,le.populated_field_txt_pcnt);
  SELF.maxlength := CHOOSE(C,le.maxlength_batch,le.maxlength_dbcode,le.maxlength_incident_num,le.maxlength_party_num,le.maxlength_seq,le.maxlength_field_name,le.maxlength_field_txt);
  SELF.avelength := CHOOSE(C,le.avelength_batch,le.avelength_dbcode,le.avelength_incident_num,le.avelength_party_num,le.avelength_seq,le.avelength_field_name,le.avelength_field_txt);
END;
...
SALT33.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Src := le.dbcode;
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT33.StrType)le.batch),TRIM((SALT33.StrType)le.dbcode),TRIM((SALT33.StrType)le.incident_num),TRIM((SALT33.StrType)le.party_num),TRIM((SALT33.StrType)le.seq),TRIM((SALT33.StrType)le.field_name),TRIM((SALT33.StrType)le.field_txt)));
  SELF.FldNo := C;
END;
...
SALT33.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 7);
  SELF.FldNo2 := 1 + (C % 7);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT33.StrType)le.batch),TRIM((SALT33.StrType)le.dbcode),TRIM((SALT33.StrType)le.incident_num),TRIM((SALT33.StrType)le.party_num),TRIM((SALT33.StrType)le.seq),TRIM((SALT33.StrType)le.field_name),TRIM((SALT33.StrType)le.field_txt)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT33.StrType)le.batch),TRIM((SALT33.StrType)le.dbcode),TRIM((SALT33.StrType)le.incident_num),TRIM((SALT33.StrType)le.party_num),TRIM((SALT33.StrType)le.seq),TRIM((SALT33.StrType)le.field_name),TRIM((SALT33.StrType)le.field_txt)));
  END;
...
SHARED FldIds := DATASET([{1,'batch'}
      ...
      ,{5,'seq'}
      ,{6,'field_name'}
      ,{7,'field_txt'}],SALT33.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    party_text_Fields.InValid_batch((SALT33.StrType)le.batch),
    party_text_Fields.InValid_dbcode((SALT33.StrType)le.dbcode),
    party_text_Fields.InValid_incident_num((SALT33.StrType)le.incident_num),
    party_text_Fields.InValid_party_num((SALT33.StrType)le.party_num),
    party_text_Fields.InValid_seq((SALT33.StrType)le.seq),
    party_text_Fields.InValid_field_name((SALT33.StrType)le.field_name),
    party_text_Fields.InValid_field_txt((SALT33.StrType)le.field_txt),
    0);
...
PrettyErrorTotals := RECORD
  TotalErrors.dbcode;
  FieldNme := party_text_Fields.FieldName(TotalErrors.FieldNum);
  FieldType := CHOOSE(TotalErrors.FieldNum,'Invalid_Batch','Invalid_DBCode','Invalid_Num','Invalid_Num','Invalid_Num','Unknown','Unknown');
  ErrorMessage := CHOOSE(TotalErrors.FieldNum,party_text_Fields.InValidMessage_batch(TotalErrors.ErrorNum),party_text_Fields.InValidMessage_dbcode(TotalErrors.ErrorNum),party_text_Fields.InValidMessage_incident_num(TotalErrors.ErrorNum),party_text_Fields.InValidMessage_party_num(TotalErrors.ErrorNum),party_text_Fields.InValidMessage_seq(TotalErrors.ErrorNum),party_text_Fields.InValidMessage_field_name(TotalErrors.ErrorNum),party_text_Fields.InValidMessage_field_txt(TotalErrors.ErrorNum));
  TotalErrors.Cnt;
END;

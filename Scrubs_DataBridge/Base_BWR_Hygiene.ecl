...
#workunit('name','Scrubs_DataBridge.Base_BWR_Hygiene - Hygiene & Stats - SALT V3.11.8');
IMPORT Scrubs_DataBridge,SALT311;
// First create an instantiated hygiene module
  infile := Scrubs_DataBridge.Base_In_DataBridge;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_DataBridge.Base_hygiene(ip);
  ...
  OUTPUT(SALT311.MAC_Character_Counts.EclRecord(p,'Base_Layout_DataBridge'),NAMED('OptimizedLayout'));// File layout suggested by data

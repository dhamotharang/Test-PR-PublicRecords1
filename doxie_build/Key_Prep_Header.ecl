import header, Census_data, Gong, Watchdog, did_add, ut, doxie;
...
Combined_Layout startLookups(doxie.Lookups le, Header.File_HHID_Current ri) :=
TRANSFORM
	...
	SELF.ssn := '';
END;
with_lookups := JOIN(DISTRIBUTE(doxie.Lookups, HASH((unsigned6)did)), 
...
TRANSFORM
	...
	SELF := le;
END;
with_best := JOIN(with_death, Watchdog.File_Best, LEFT.did = RIGHT.did, getBest(LEFT, RIGHT), FULL OUTER, LOCAL);
...
TRANSFORM
	SELF.did := IF(ri.head_cnt >= 10000, SKIP, le.did);
...
addrMatch2 := MACRO
	...
	DID_Add.Address_Match_Score(LEFT.prim_range, LEFT.prim_name, LEFT.sec_range, LEFT.zip, 
							RIGHT.prim_range, RIGHT.prim_name, RIGHT.sec_range, RIGHT.z5) < 255
ENDMACRO;
...
TRANSFORM
	...
	SELF.gong_score := IF(ri.listed_name = '', 101, 
...
TRANSFORM
	SELF := le;
END;
rm_score := PROJECT(ddp, backOne(LEFT));

export Key_Prep_Header := INDEX(rm_score, {unsigned6 s_did := did}, {rm_score}, '~thor_data400::key::header' + thorlib.WUID(), OPT);
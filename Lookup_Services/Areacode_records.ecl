import Advo,ut,Risk_Indicators,iesp,gong;
export Areacode_records (Lookup_Services.Areacode_IParam.searchParams inrec) := FUNCTION
	...
	areacodeData:=Project(Gong.Key_Npa(keyed(areacode=inrec.areacode)),area_rec);
	areacodeDataAll:=Project(Gong.Key_Zip(keyed(zip in set(areacodeData,zip))),area_rec);
	//Handle Zipcode searches
	zipcodeData:=Project(Gong.Key_Zip(keyed(zip=inrec.zip)),area_rec);
  ...
  getCityZips := dedup(sort(getCityZipsADVO&getCityZipsUT,zip),zip);
	cityData:=Project(Gong.Key_Zip(keyed(zip in set(getCityZips,zip))),area_rec);
	//Handle City Only Searches
	getCityOnlyZips:=Advo.Key_Addr_City(keyed(city=inrec.city));
	cityOnlyData:=Project(Gong.Key_Zip(keyed(zip in set(getCityOnlyZips,zip))),area_rec);
	//Handle State Only Searches
	getStateOnlyZips := dedup(sort(Advo.Key_Addr_City(state=inrec.State),zip),zip);
	stateOnlyData:=Project(Gong.Key_Zip(keyed(zip in set(getStateOnlyZips,zip))),area_rec);

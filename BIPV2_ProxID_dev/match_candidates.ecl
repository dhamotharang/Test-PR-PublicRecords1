...
pj30 do_computes(pj30 le) := TRANSFORM
  ...
  SELF := le;
END;
SHARED propogated := PROJECT(pj30,do_computes(left)) : PERSIST('~temp::Proxid::BIPV2_ProxID_dev::mc_props::DOT_Base',EXPIRE(Config.PersistExpire)); // to allow to 'jump' over an exported value
...
SHARED Annotated := DISTRIBUTE(j0,hash(Proxid)) : PERSIST('~temp::Proxid::BIPV2_ProxID_dev::mc',EXPIRE(Config.PersistExpire)); // Distributed for keybuild case
...
EXPORT SrcRidVlid_candidates := jSrcRidVlid_21 : PERSIST('~temp::Proxid::BIPV2_ProxID_dev::mc::SrcRidVlid',EXPIRE(Config.PersistExpire));
...
EXPORT ForeignCorpkey_candidates := jForeignCorpkey_21 : PERSIST('~temp::Proxid::BIPV2_ProxID_dev::mc::ForeignCorpkey',EXPIRE(Config.PersistExpire));
...
EXPORT RAAddresses_candidates := jRAAddresses_21 : PERSIST('~temp::Proxid::BIPV2_ProxID_dev::mc::RAAddresses',EXPIRE(Config.PersistExpire));
...
EXPORT FilterPrimNames_candidates := jFilterPrimNames_21 : PERSIST('~temp::Proxid::BIPV2_ProxID_dev::mc::FilterPrimNames',EXPIRE(Config.PersistExpire));

...
/*
  ...
  4. hack BIPV2_ProxID_dev.MOD_Attr_FilterPrimNames to fix it so it runs -- check
  
*/

EXPORT _ApplyProxidHacks(
   string   pModule               = 'BIPV2_ProxID_dev'
...
/*
  BIPV2_ProxID_dev.matches: hack default force for cnp_number,cnp_name & prim_range(make sure they equal each other) in matchjoin attribute
  ...
  BIPV2_ProxID_dev.Debug:   hack sample match join to match hacks in matchjoin in matches.            
...
//  INTEGER2 prim_range_score := IF ( le.prim_range = ri.prim_range or prim_range_score_temp >= 0 OR company_addr1_score_pre > 0 OR company_address_score_pre > 0, prim_range_score_temp, SKIP ); // Enforce FORCE parameter
    ...
    cnp_name_regex        := 'cnp_name_score := IF [(] cnp_name_score_supp >= Config.cnp_name_Force [*] 100 OR [(][ ]*' + activedomesticcorpkey + ' > Config.cnp_name_OR1_active_domestic_corp_key_Force[*]100[)] OR [(][ ]*' + activedunsnumber      + ' > Config.cnp_name_OR2_active_duns_number_Force[*]100[)], cnp_name_score_supp, ' + SkipOr9999 + ' [)];';
...
/////////hack debug scores for attribute files
    ...
    HackRules_message := if(hackRulesCondition ,HackAddPrimName_message,HackAddPrimName_message + ',Did not Hack ' + pAttribute + ' Adding Rules for extra mjs\n');
    ...
    hackReplaceTransitivesMacroCondition := pAttribute = 'matches' and      regexfind('salt30[.]mac_avoid_transitives[(]All_Matches,Proxid1,Proxid2,Conf,DateOverlap,Rule,o[)];',HackPrimNameExact,nocase)
                                                                   and not  regexfind('BIPV2_ProxID_dev[.]mac_avoid_transitives_scalene'                                            ,HackPrimNameExact,nocase);
    ...
    HackReplaceTransitivesMacro_message := if(hackReplaceTransitivesMacroCondition ,HackPrimNameExact_message,HackPrimNameExact_message + 'Did not Hack ' + pAttribute + ' replacing transitives macro\n');
    ...
    HackRollEntitiesReturn_message := if(hackRollEntitiesReturnCondition ,HackReplaceTransitivesMacro_message,HackReplaceTransitivesMacro_message + 'Did not Hack ' + pAttribute + ' replacing return for RollEntities\n');

       
    //////////////
    // 5)	In the î€šmatchesî€š attribute, where you see î€šsalt30.MAC_ParentId_Patchî€š change it to î€šBIPV2_Tools.MAC_ParentId_Patchî€š.  Immediately before the first of these calls, add this line:
  // o_thin := TABLE(o,{ultid,orgid,lgid3,proxid,dotid,rcid}); // HACK
// and then on the first MAC_ParentId_Patch call change the î€šoî€š parameter to î€šo_thinî€š
  ...
  // Patchultid := salt30.MAC_ParentId_Patch(Patchorgid,ultid,orgid);  // Collapse any ultid now joined by orgid
    ...
    HackSlimPatchDs_message := if(hackSlimPatchDsCondition ,HackRollEntitiesReturn_message,HackRollEntitiesReturn_message + ' ,Did not Hack ' + pAttribute + ' replacing Parentid patch\n');
    ...
    HackMacParentIdPatch_message := if(hackMacParentIdPatchCondition ,HackSlimPatchDs_message,HackSlimPatchDs_message + ' ,Did not Hack ' + pAttribute + ' replacing Parentid patch call\n');
    ...
    HackMacChildIdPatch_message := if(hackMacChildIdPatchCondition ,HackMacParentIdPatch_message,HackMacParentIdPatch_message + ' ,Did not Hack ' + pAttribute + ' replacing Childid patch call\n');
    ...
    HackPatchedInfile_message := if(hackPatchedInfileCondition ,hackMacChildIdPatch_message,hackMacChildIdPatch_message + ' ,Did not Hack ' + pAttribute + ' adding join to patched_infile\n');
  ...
  function
    ...
    + 'EXPORT MatchHistoryName := \'~keep::BIPV2_ProxID_dev::Proxid::MatchHistory\';\n'
    + 'EXPORT MatchHistoryFile := DATASET(MatchHistoryName,Matches(In_DOT_Base).id_shift_r,THOR); // Read in all the change history\n'
    + 'EXPORT MatchHistoryKeyName := \'~\'+\'key::BIPV2_ProxID_dev::Proxid::History::Match\';\n'
    + '  MH := MatchHistoryFile;\n'
    ...
    + '// EXPORT InDataKeyName := \'~\'+\'key::BIPV2_ProxID_dev::Proxid::Datafile::in_data\';\n'
  ...
  function
    ...
    HackKeys  := 
      ...
      + '//  Cands0 := BIPV2_ProxID_dev.match_candidates(inhead).ForeignCorpkey_candidates;\n'
      + '  Cands0 := BIPV2_ProxID_dev.file_Foreign_Corpkey;/*HACK*/\n'
  ...
  function
    ...
    HackKeys  := 
      ...
      + '  Cands0 := BIPV2_ProxID_dev.file_RA_Addresses;/*HACK*/\n'
      + '// We are going to create a record for each candidate pair; this record will have a child dataset for the attribute values of each side\n'
      + '////\n'
      + 'childrec1 := \n'
      + 'record\n'
  ...
  function
    ...
    HackKeys  := 
      ...
      + 'EXPORT ForceFilter(inhead,infile,id1,id2) := FUNCTIONMACRO\n'
      + '// Every attribute value has to exactly match every attribute value of the corresponding identifier or both have no values\n'
      + '  Cands0 := BIPV2_ProxID_dev.file_filter_Prim_names;/*HACK*/\n'
      + '// We are going to create a record for each candidate pair; this record will have a child dataset for the attribute values of each side\n'
      + '////\n'
      + 'childrec1 := \n'
      + 'record\n'
...
//////////////    
  ...
  function
    ...
    HackKeys  := 
      ...
      + 'IMPORT SALT24,BIPV2_ProxID_dev,ut,tools;\n'
      + 'STRING50 Proxidonestr := \'1234\'  : STORED(\'ProxidOne\');\n'
      + 'STRING20 Proxidtwostr := \'12345\'  : STORED(\'Proxidtwo\');\n'
      + 'combo := \'qa\';\n'
      + '\n'
      + 'BIPV2_ProxID_dev._fun_CompareService((unsigned6)Proxidonestr,(unsigned6)Proxidtwostr,combo);\n'
      + 'ENDMACRO;\n'
      ;
...
//SHARED SrcRidVlid_prop5 := JOIN(SrcRidVlid_prop4,cnp_number_props,left.Proxid=right.Proxid,LOCAL);
    ...
    hackcnpNumber_Left_outerCondition := pAttribute = 'match_candidates' and regexfind('SHARED SrcRidVlid_prop5 := JOIN[(]SrcRidVlid_prop4,cnp_number_props,left[.]Proxid=right[.]Proxid,LOCAL[)];',HackMatchCandidates,nocase);

    HackcnpNumber_Left_outer := if(hackcnpNumber_Left_outerCondition 
                        ,regexreplace('SHARED SrcRidVlid_prop5 := JOIN[(]SrcRidVlid_prop4,cnp_number_props,left[.]Proxid=right[.]Proxid,LOCAL[)];',HackMatchCandidates
    ...
    saveatt := fPutAttribute(pAttribute,HackcnpNumber_Left_outer);
    
    return sequential(iff(pShouldSaveAttribute  and hackcnpNumber_Left_outerCondition,saveatt  ,output(dataset([{HackcnpNumber_Left_outer}],{string ecl}),named(pAttribute + 'HackedEcl'))),output(HackcnpNumber_Left_outer_message  ,named(pAttribute + 'Hack_message')));
  END;

...
import bipv2,BIPV2_ProxID_dev,Business_DOT_SALT_micro12,BIPV2_Tools;
dinput 						:= BIPV2_ProxID_dev.In_DOT_Base;
dlegalentityrels 	:= BIPV2_ProxID_dev.relationships(dinput).SAMELEGALENTITY_links	;
...
BIPV2_ProxID_dev.Proc_Iterate('1').RelationshipKeys;
BIPV2_ProxID_dev.Proc_Iterate('1').DebugKeys;
...
output(dpatchlegalentitycluster_proj		,,'~thor_data400::base::BIPV2_ProxID_dev::legalentity_patched_file_filt'	,__compressed__,overwrite);
output(dbadlegalentityrels 							,,'~thor_data400::base::BIPV2_ProxID_dev::badlegalentity_rels'						,__compressed__,overwrite);
////////////////extra outputs
p := project(dpatchlegalentitycluster_proj, transform(BIPV2_ProxID_dev.layouts.dot_base, self.dotid := left.proxid, self := left));

ih      := BIPV2_ProxID_dev_dev.In_DOT_Base;
laymtch := BIPV2_ProxID_dev_dev.match_candidates(ih).layout_matches;

All_Matches   := dataset('~temp::BIPV2_ProxID_dev_dev_Proxid_DOT_Base_all_m',laymtch,flat); // all matches before processing
remove_trans  := dataset('~temp::BIPV2_ProxID_dev_dev_Proxid_DOT_Base_mt'   ,laymtch,flat); // remove transitives
BestPerRecord := dataset('~temp::BIPV2_ProxID_dev_dev_Proxid_DOT_Base_mr'   ,laymtch,flat); // best per record

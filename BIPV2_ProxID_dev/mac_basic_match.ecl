...
ds_agginbigs  := sort(BIPV2_ProxID_dev.AggregateProxidElements(oldproxids),proxid) : global;
ds_aggoutbigs := sort(BIPV2_ProxID_dev.AggregateProxidElements(newproxids),proxid) : global;
return
sequential(
  parallel(
    ...
    ,output(ds_doublecheck,,'~temp::lbentley::BIPV2_ProxID_dev::basic_match::patch'          ,compressed,overwrite)
    ,output(ds_out   ,,BIPV2_ProxID_dev.filenames(bipv2.KeySuffix + 'a_27').base.logical,compressed,overwrite)

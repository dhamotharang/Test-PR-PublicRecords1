import BIPV2;
dbase35 := BIPV2_ProxID_dev.files().base.qa(proxid in [5705425,5532372]) : global;
setdots := set(dbase35,dotid);
dbase34 := BIPV2_ProxID_dev.files(BIPV2.KeySuffix + '_34').base.logical(dotid in setdots) : global;
dbase33 := BIPV2_ProxID_dev.files(BIPV2.KeySuffix + '_33').base.logical(dotid in setdots) : global;
dagg    := BIPV2_ProxID_dev.AggregateProxidElements(dbase35);
dagg_34 := BIPV2_ProxID_dev.AggregateProxidElements(dbase34);
dagg_33 := BIPV2_ProxID_dev.AggregateProxidElements(dbase33);
...
dagg_join := BIPV2_ProxID_dev.AggregateProxidElements(djoin);

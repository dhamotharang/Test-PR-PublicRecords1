...
iteration := '@iteration@';
pversion  := '@version@';
lih       := BIPV2_ProxID_dev.files().base.built;
names     :=    BIPV2_ProxID_dev.filenames(pversion + '_' + iteration).dall_filenames
              + BIPV2_ProxID_dev.keynames(pversion + '_' + iteration).dall_filenames
              ;
#workunit('name','BIPV2_ProxID_dev.BWR_Iterate ' + iteration + '- Internal Linking - SALT V2.5 Beta 1');
h         := BIPV2_ProxID_dev.Hygiene(BIPV2_ProxID_dev.Files().base.qa);
p         := h.AllProfiles; // Detailed profile of every field
combo     := if(iteration = '1' ,(string)((unsigned)iteration - 1)  ,pversion + '_' + (string)((unsigned)iteration - 1));
sequential(
	 output(BIPV2_ProxID_dev.In_DOT_Base	,named('InputFile'))
  ,nothor(Tools.fun_ClearfilesFromSupers(project(names, transform(Tools.Layout_Names,self.name := left.logicalname)), false)) //clear supers if needed
	,BIPV2_ProxID_dev.Promote(combo).new2built
	,BIPV2_ProxID_dev.Proc_Iterate(iteration,pversion,lih).DoAll // Change '1' for subsequent iterations
  ...
  ,BIPV2_ProxID_dev.output_test_cases(BIPV2_ProxID_dev.Files().base.qa)
);

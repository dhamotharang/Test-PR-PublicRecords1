...
function
  ...
  kFile := BIPV2_ProxID_dev.Keys(BFile,version);

  odl   := PROJECT(CHOOSEN(KFile.Candidates(Proxid=Proxidone,rcid = prcid1),100000),BIPV2_ProxID_dev.match_candidates(BFile).layout_candidates);
  odr   := PROJECT(CHOOSEN(KFile.Candidates(Proxid=ProxidTwo,rcid = prcid2),100000),BIPV2_ProxID_dev.match_candidates(BFile).layout_candidates);
  ...
  BIPV2_ProxID_dev.match_candidates(BFile).layout_attribute_matches ainto(KFile.Attribute_Matches le) := TRANSFORM
    SELF := le;
  END;
  ...
  mtch  := BIPV2_ProxID_dev.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,Proxidone,Proxidtwo,prcid1,prcid2}],BIPV2_ProxID_dev.match_candidates(BFile).layout_matches),am);

...
dsSearch	:=	distribute(BankruptcyV2.file_bankruptcy_search_v3,hash(tmsid));

BankruptcyV2.layout_bankruptcy_search trfSearch(BankruptcyV2.layout_bankruptcy_search_v3_supp input) := transform
	self := input;
end;

BankruptcyV2.layout_bankruptcy_search trfSearch2(BankruptcyV2.layout_bankruptcy_main_v3.layout_bankruptcy_main_filing_supp input) := transform,skip(trim(input.TrusteeName,left,right)='' and trim(input.trusteeAddress,left,right) = '')
...
export Map_BK_V3_V2_Search := sort(distribute((fileSearch1 + fileSearch2),hash(tmsid)), tmsid,local);
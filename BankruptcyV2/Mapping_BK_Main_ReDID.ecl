/*2010-11-22T15:35:48Z (Ananth Vankatachalam)
use supplemental layout to apply deletes
*/
//////////////////////////////////////////////////////////////////////////////////////////
// DEPENDENT ON : bankruptcyV2.Layout_bankruptcy_main_temp,
//				  bankruptcyV2.Layout_bankruptcy_main,
//				  
// PURPOSE	 	: This function is used only for full-ReDID. Creates child datasets, distribute,sort and mapped main daily 
//                bankruptcy/okc file to main layout, refresh full base file with update file and   
//			      rollup full file.
...
export Mapping_BK_Main_ReDID(boolean boolean_value,boolean useaid = true) := function
...
main_daily := main_daily_with_address + main_daily_with_no_address;

// if useaid is false then don't use data from AID instead use the 
// regular address cleaner data

BankruptcyV2.Layout_bankruptcy_main_v3.layout_bankruptcy_main_filing_supp dontuseaidaddress(main_daily_temp l) := transform
	self := l;
end;
...
preDIDFile	:=	if ( 	boolean_value = true and 
						(ut.Weekday((integer)ut.GetDate) = 'SATURDAY' or ut.Weekday((integer)ut.GetDate) = 'SUNDAY'),
							if(useaid,main_daily + Full_BK_Main_nondist,distribute(fullbasefile,hash(tmsid)) + main_daily_noaid),
							if(useaid,main_daily,main_daily_noaid)
						); 

PreDID_Rec := record,maxlength(32766)
	bankruptcyV2.Layout_bankruptcy_main_v3.layout_bankruptcy_main_filing_supp;
	integer8	temp_DID		  := 0;
...
////////////////////////////////////////////////////////////////////////////////////////
ut.mac_flipnames(preDIDFile,fname,mname,lname,DIDFile,32766);
...
Full_BK_Main_refreshed := if(useaid,join(Full_BK_Main_nondist , BK_Main_WithDID, left.tmsid = right.tmsid /*and left.source = right.source*/,
                     	 transform({recordof(bankruptcyV2.Layout_bankruptcy_main_v3.layout_bankruptcy_main_filing_supp)},
				         self := left),left only,local),
								 join(distribute(fullbasefile,hash(tmsid)) , BK_Main_WithDID, left.tmsid = right.tmsid /*and left.source = right.source*/,
                     	 transform({recordof(bankruptcyV2.Layout_bankruptcy_main_v3.layout_bankruptcy_main_filing_supp)},
				         self := left),left only,local)) ;
...
daily_plus_full := Full_BK_Main + BK_Main_WithDID;

...
EXPORT SALT34.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_supp_key','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_charter','invalid_filing_cd','invalid_alphablankSp');
EXPORT FieldTypeNum(SALT34.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_supp_key' => 2,'invalid_corp_vendor' => 3,'invalid_state_origin' => 4,'invalid_mandatory' => 5,'invalid_date' => 6,'invalid_charter' => 7,'invalid_filing_cd' => 8,'invalid_alphablankSp' => 9,0);
...
EXPORT MakeFT_invalid_filing_cd(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_filing_cd(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN set_valid_filing);
EXPORT InValidMessageFT_invalid_filing_cd(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('set_valid_filing'),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_alphablankSp(SALT34.StrType s0) := FUNCTION
  s1 := SALT34.stringfilter(s0,' ABCDEFGHIJKLMNOPQRSTUVWXYZ/'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_alphablankSp(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,' ABCDEFGHIJKLMNOPQRSTUVWXYZ/'))));
EXPORT InValidMessageFT_invalid_alphablankSp(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars(' ABCDEFGHIJKLMNOPQRSTUVWXYZ/'),SALT34.HygieneErrors.Good);
...
EXPORT Make_corp_sos_charter_nbr(SALT34.StrType s0) := MakeFT_invalid_charter(s0);
EXPORT InValid_corp_sos_charter_nbr(SALT34.StrType s) := InValidFT_invalid_charter(s);
...
EXPORT Make_event_filing_desc(SALT34.StrType s0) := MakeFT_invalid_alphablankSp(s0);
EXPORT InValid_event_filing_desc(SALT34.StrType s) := InValidFT_invalid_alphablankSp(s);
EXPORT InValidMessage_event_filing_desc(UNSIGNED1 wh) := InValidMessageFT_invalid_alphablankSp(wh);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT34,Scrubs_Corp2_Mapping_IN_Event;

...
// **** commit using bug 177807 ****

export Prop_Address_Prep_v4(boolean isFCRA, boolean filter_fares=false, boolean isFast) := function

// ADDITIONAL INPUTS
fdf0 				:= if(isFast,LN_PropertyV2_Fast.CleanSearch(true)
											  +LN_PropertyV2_Fast.CleanSearch(false)
									,LN_PropertyV2_Fast.CleanSearch(false));

fdf					:= fdf0(source_code[2] = 'P', prim_name != '', zip != '');

assessments := if(isFast,LN_PropertyV2_Fast.Files.base.assessment
												+LN_PropertyV2.File_Assessment
									,LN_PropertyV2.File_Assessment);
									
deeds 			:= if(isFast,LN_PropertyV2_Fast.Files.base.deed_mortg
...
LN_PropertyV2_Fast.FCRA_compliance_MAC(isFCRA,fdf,file_out);
//df := file_out;
df := if(filter_fares,no_fares_df,file_out);
...
LN_PropertyV2_Fast.FCRA_compliance_MAC(isFCRA,assessments,file_out2);
assessment_file :=  if(filter_fares, no_fares_assessments, file_out2);
...
LN_PropertyV2_Fast.FCRA_compliance_MAC(isFCRA,deeds,file_out3);
deed_file := if(filter_fares,no_fares_deeds,file_out3);

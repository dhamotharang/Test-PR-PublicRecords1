...
export File_Property_Ownership_V4(boolean filter_fares,boolean isFast) := function
...
pfileF := LN_PropertyV2_Fast.CleanSearch(true);
pfile1 := if(isFast,pfile0+pfileF,pfile0);

assessor0 := ln_propertyV2.File_Assessment;
assessorF	:= LN_PropertyV2_Fast.Files.base.assessment;
assessor1 := if(isFast,assessor0+assessorF,assessor0);

deeds0 := ln_propertyV2.File_Deed;
deedsF := LN_PropertyV2_Fast.Files.base.deed_Mortg;
deeds1 := if(isFast,deeds0+deedsF,deeds0);

...
EXPORT Key_Prop_Ownership_FCRA_V4(boolean isFast) := FUNCTION;
...
file_a0					:=	ln_propertyv2.File_Assessment;
file_a1					:=	LN_PropertyV2_Fast.Files.base.assessment;
file_a					:=  if(isFast,file_a0+file_a1, file_a0);
file_d0					:=	ln_propertyv2.File_Deed;
file_d1					:=	LN_PropertyV2_Fast.Files.base.deed_mortg;
file_d					:=  if(isFast,file_d0+file_d1, file_d0);
fa_1						:= 	distribute(file_a,hash(ln_fares_id));
fd_1						:=	distribute(file_d,hash(ln_fares_id));
file_assessment	:=	dedup(sort(fa_1,ln_fares_id,local),local);
file_deed				:= 	dedup(sort(fd_1,ln_fares_id,local),local);

//filtering by [ln_fares_id[1] != 'R'] produces FCRA compliant data

pfile0	:=	LN_PropertyV2_Fast.CleanSearch(false);
pfile1	:=	LN_PropertyV2_Fast.CleanSearch(true);

ps_2		:= if(isFast,pfile0+pfile1,pfile0);

ps_1		:=	distribute(ps_2,hash(ln_fares_id));
pfile2	:=	dedup(sort(ps_1,ln_fares_id,local),local);

pfile := pfile2 (ln_fares_id[1] != 'R', did != 0, source_code[2] = 'P', prim_name != '', zip != '');
...
//Add assessments
proprec add_assess(proprec le, file_assessment ri) := TRANSFORM
...
pre_Assessments_added := JOIN(distribute(df3,hash(own_fares_id)),
															distribute(file_assessment (ln_fares_id[1] != 'R'), hash(ln_fares_id)),
...
assessments_added := rollup
												 ...
												 ;//: persist ('thor_data400::persist::property_fast::fcra::assessments_rolled_v2');

//Add deeds
proprec add_deeds(proprec le, file_deed Ri) := TRANSFORM
...
pre_Deeds_added := JOIN (assessments_added,
                         distribute (file_deed(ln_fares_id[1] != 'R', ln_fares_id != ''), hash(ln_fares_id)),
...
deeds_added := ROLLUP (
												SORT (ds, did, prim_range, prim_name, zip5, sec_range, own_fares_id, local),
...
return index (Prop, {did}, {Prop},
																	Constants.keyServerPointer+'thor_data400::key::'+keyPrefix+'::fcra::'+doxie.Version_SuperKey+'::did.ownership_v4');

...
export File_Property_A_Extract (boolean isFast) := FUNCTION

	assessment := if(isFast,LN_PropertyV2_Fast.Files.base.assessment,LN_PropertyV2.File_Assessment);
	addl_frs_a := if(isFast,LN_PropertyV2_Fast.Files.base.addl_frs_a,LN_PropertyV2.File_addl_Fares_tax);
	addl_legal := if(isFast,LN_PropertyV2_Fast.Files.base.addl_legal,LN_PropertyV2.File_addl_legal);
	
	ds_asse := 	SORT(assessment(ln_fares_id <>''),ln_fares_id);
	ds_tax := 	SORT(addl_frs_a(ln_fares_id <>''),ln_fares_id);
	ds_leg := 	SORT(addl_legal(ln_fares_id <>''),ln_fares_id);	
 	//Parsed, cleaned, enhanced data of interest

	search := if(isFast,LN_PropertyV2_Fast.File_Property_ACleanName,LN_PropertyV2.File_Property_ACleanName);
	
	ds_search := SORT(search,ln_fares_id);
  ...
  ut.Mac_Convert_Codes_To_Desc(ds_main,main_rec,legal_lot_ds,'FARES_2580',
	...
	//the whole dataset
	ret := MERGE(ds_main_out,ds_tax_out,ds_leg_out,ds_search_out,SORTED(ln_fares_id)):INDEPENDENT;
	RETURN ret;

...
export mod_ownership(boolean isFast) := module
...
f_search0	:= LN_PropertyV2.File_Search_DID(did<>0 or bdid<>0, source_code[1..2] in ['OP','BP'], (st_restrict=[] or st in st_restrict));
f_search1	:= LN_PropertyV2_Fast.Files.base.search_prp(did<>0 or bdid<>0, source_code[1..2] in ['OP','BP'], (st_restrict=[] or st in st_restrict));
f_search	:= if(isFast,f_search0 + f_search1,f_search0);
...
l_hist_thin toHistThin(f_search L) := transform
	...
	ds_oneRecord			:= DATASET([transform({unsigned a},self.a:=1)]);
	self.owners				:= project(ds_oneRecord, toOwnerThin(L));
end;
...
l_work toWork(f_search L) := transform
	...
	ds_oneRecord									:= DATASET([transform({unsigned a},self.a:=1)]);
	self.hist											:= project(ds_oneRecord, toHistThin(L));
	self.cur_owners								:= project(ds_oneRecord, toOwnerFull(L));
...
f_assess0	:= LN_PropertyV2.File_Assessment(fips_code<>'', (st_restrict=[] or state_code in st_restrict));
f_assess1	:= LN_PropertyV2_Fast.Files.base.assessment(fips_code<>'', (st_restrict=[] or state_code in st_restrict));
...
// output(c-hoosen(ds_assess,150),named('ds_assess'));


// --------------------------------------------------------------------------
//	3. Extract data from deed file, join with search data, and rollup owners
// --------------------------------------------------------------------------
f_deed0	:= LN_PropertyV2.File_Deed(fips_code<>'', (st_restrict=[] or state in st_restrict));
f_deed1	:= LN_PropertyV2_Fast.Files.base.deed_mortg(fips_code<>'', (st_restrict=[] or state in st_restrict));
...
//	5. Merge ownership records into datasets with and without aid
// ---------------------------------------------------------------
shared ds_combined_aid		:= ds_assess_ace + ds_deed_ace							;//: persist('~thor_data400::persist::property_fast::own_combined_aid');
shared ds_combined_noaid	:= ds_assess(rawaid=0) + ds_deed(rawaid=0)	;//: persist('~thor_data400::persist::property_fast::own_combined_noaid');

...
EXPORT File_Property_DMCleanName(boolean isFast) := FUNCTION

search_prp := if(isFast,LN_PropertyV2_Fast.Files.base.search_prp, LN_PropertyV2.File_Search_DID );

ds_search := search_prp(ln_fares_id<>'' AND err_stat[1] = 'S' AND which_orig='1' AND ln_fares_id[2] <>'A' AND source_code[1] IN ['B', 'O']);	
rec := record
	LN_PropertyV2.Layout_Property_KeyExtract;
	unsigned1 keepit;
end;
...
RETURN project(ds_search_out(keepit>0), LN_PropertyV2.Layout_Property_KeyExtract);
END;
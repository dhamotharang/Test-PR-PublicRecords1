import Address,AID,LN_Mortgage,LN_Property,ut,LN_Propertyv2,nid,Data_Services;
...
transform
	self.fares_unformatted_apn	:=	if(	L.ln_fares_id[1]	!=	'R',
																			LN_PropertyV2.fn_strip_pnum(L.apnt_or_pin_number),
...
Deeds_Wflag	:=	LN_PropertyV2_Fast.Mac_LN_latest_Recflag(search_in0).current_deeds(deed_result);

// bug 25396 - nameasis in search would begin w/ a leading ", "

export 	Deeds_resultWflag(dataset(recordof(LN_PropertyV2.layout_addl_fares_deed)) in_addl_fares_deed0)
					:=	LN_PropertyV2.fn_patch_deed(Deeds_Wflag,in_addl_fares_deed0); 
// **** https://bugzilla.seisint.com/show_bug.cgi?id=177796	 ****/
...
export new_search_records := dCleanAddress	:	persist('~thor_data400::persist::'+preFix+'::deeds_address_propagated_fp'+if(isFast,'_d','f'));
			 //dataset('~thor_data400::persist::'+preFix+'::deeds_address_propagated_fp',recordof(dCleanAddress),thor);
end;
...
*/
export MAC_autokeyb2_Build (indataset,infname,inmname,inlname,
						inssn,
						indob,
						phone,
...
#uniquename(normalizecompanynameswith_preferred)
%ds_normedB% normed_company_preferred(%ds_normedB% l, integer C) := transform
  self.inbname := choose(C,l.inbname, l.inbname1);
  self := l;
END;
%normalizecompanynameswith_preferred% := normalize(%ds_normedB%,2,normed_company_preferred(left, counter);

#uniquename(normalizecompanynameswith_the)
%normalizecompanynameswith_preferred% split_company_names(%normalizecompanynameswith_preferred% l, integer C) := transform
  self.inbname := choose(C,l.inbname, l.inbname[5..length(l.inbname)]);
  self := l;
END;
%normalizecompanynameswith_the% := normalize(%normalizecompanynameswith_preferred%,if(left.inbname[1..4]='THE ',2,1),split_company_names(left,counter));

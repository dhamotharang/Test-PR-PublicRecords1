...
seq_name := 
	    #if (one_node)
			buildindex(theindexprep,lkeyname,update,few)
		#else 
			if(
				~diffing,
				buildindex(theindexprep,lkeyname,update),
				buildindex(theindexprep,lkeyname,DISTRIBUTE(index(theindexprep,superkeyname+'_Prod')),update)
			);
		#end			
endmacro;
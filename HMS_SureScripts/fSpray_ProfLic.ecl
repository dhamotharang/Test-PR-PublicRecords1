...
export fSpray_ProfLic(string version, boolean pUseProd = false)	:=	DATASET([
 	{_control.IPAddress.bctlpedata10	                    //SourceIP			 Remote Server's IP address									
 	,'/data/hds_180/SureScripts/Prof_licenses/'+version[..8]+'/'         //SourceDirectory	 Absolute path of directory on Remote Server where files are located                
 	...
 	,_Dataset(pUseProd).thor_cluster_Files+ 'in::' + _Dataset().Prof_Lic_Name + '::' + 'in_prof_lic_taxonomy_map'      //Thor_filename_template	-- template filename for files to be sprayed, ex. '~thor_data400::in::corp2::@version@::cont'
 	,[{_Dataset(pUseProd).thor_cluster_Files+'in::' + _Dataset().Prof_Lic_Name  }]            //dSuperfilenames			-- dataset of superfiles to add the sprayed files to.

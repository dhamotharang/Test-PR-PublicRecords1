...
EXPORT Summary(SALT39.Str30Type  txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    populated_inquiry_type_cnt := COUNT(GROUP,h.inquiry_type <> (TYPEOF(h.inquiry_type))'');
    populated_inquiry_type_pcnt := AVE(GROUP,IF(h.inquiry_type = (TYPEOF(h.inquiry_type))'',0,100));
    maxlength_inquiry_type := MAX(GROUP,LENGTH(TRIM((SALT39.StrType)h.inquiry_type)));
    avelength_inquiry_type := AVE(GROUP,LENGTH(TRIM((SALT39.StrType)h.inquiry_type)),h.inquiry_type<>(typeof(h.inquiry_type))'');
    ...
    maxlength_lex_id := MAX(GROUP,LENGTH(TRIM((SALT39.StrType)h.lex_id)));
    avelength_lex_id := AVE(GROUP,LENGTH(TRIM((SALT39.StrType)h.lex_id)),h.lex_id<>(typeof(h.lex_id))'');
    populated_reprice_batch_number_cnt := COUNT(GROUP,h.reprice_batch_number <> (TYPEOF(h.reprice_batch_number))'');
    populated_reprice_batch_number_pcnt := AVE(GROUP,IF(h.reprice_batch_number = (TYPEOF(h.reprice_batch_number))'',0,100));
    maxlength_reprice_batch_number := MAX(GROUP,LENGTH(TRIM((SALT39.StrType)h.reprice_batch_number)));
    avelength_reprice_batch_number := AVE(GROUP,LENGTH(TRIM((SALT39.StrType)h.reprice_batch_number)),h.reprice_batch_number<>(typeof(h.reprice_batch_number))'');
    ...
    populated_fcra_purpose_cnt := COUNT(GROUP,h.fcra_purpose <> (TYPEOF(h.fcra_purpose))'');
    populated_fcra_purpose_pcnt := AVE(GROUP,IF(h.fcra_purpose = (TYPEOF(h.fcra_purpose))'',0,100));
    maxlength_fcra_purpose := MAX(GROUP,LENGTH(TRIM((SALT39.StrType)h.fcra_purpose)));
    avelength_fcra_purpose := AVE(GROUP,LENGTH(TRIM((SALT39.StrType)h.fcra_purpose)),h.fcra_purpose<>(typeof(h.fcra_purpose))'');
    ...
    populated_orig_jobid_cnt := COUNT(GROUP,h.orig_jobid <> (TYPEOF(h.orig_jobid))'');
    populated_orig_jobid_pcnt := AVE(GROUP,IF(h.orig_jobid = (TYPEOF(h.orig_jobid))'',0,100));
    maxlength_orig_jobid := MAX(GROUP,LENGTH(TRIM((SALT39.StrType)h.orig_jobid)));
    avelength_orig_jobid := AVE(GROUP,LENGTH(TRIM((SALT39.StrType)h.orig_jobid)),h.orig_jobid<>(typeof(h.orig_jobid))'');
    populated_orig_acctno_cnt := COUNT(GROUP,h.orig_acctno <> (TYPEOF(h.orig_acctno))'');
    populated_orig_acctno_pcnt := AVE(GROUP,IF(h.orig_acctno = (TYPEOF(h.orig_acctno))'',0,100));
    maxlength_orig_acctno := MAX(GROUP,LENGTH(TRIM((SALT39.StrType)h.orig_acctno)));
    avelength_orig_acctno := AVE(GROUP,LENGTH(TRIM((SALT39.StrType)h.orig_acctno)),h.orig_acctno<>(typeof(h.orig_acctno))'');
...
SALT39.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  ...
  END;
SHARED Pairs0 := NORMALIZE(ENTH(h,Config.CorrelateSampleSize),68*68,IntoP(LEFT,COUNTER))(FldNo1<FldNo2);
SHARED FldIds := DATASET([{1,'orig_end_user_id'}
      ...
      ,{47,'orig_ip_address'}
      ,{48,'orig_source_code'}
      ,{49,'orig_retail_price'}
      ,{50,'inquiry_type'}
      ,{51,'lex_id'}
      ,{52,'reprice_batch_number'}
      ,{53,'user_changed'}
      ,{54,'date_changed'}
      ,{55,'fcra_purpose'}
      ,{56,'orig_address_2'}
      ...
      ,{61,'orig_jobid'}
      ,{62,'orig_acctno'}
      ,{63,'orig_end_user_name'}
      ,{64,'orig_end_user_address_1'}
      ,{65,'orig_end_user_address_2'}
      ,{66,'orig_end_user_city'}
      ,{67,'orig_end_user_state'}
      ,{68,'orig_end_user_zip'}],SALT39.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_inquiry_type((SALT39.StrType)le.inquiry_type),
    Fields.InValid_lex_id((SALT39.StrType)le.lex_id),
    Fields.InValid_reprice_batch_number((SALT39.StrType)le.reprice_batch_number),
    Fields.InValid_user_changed((SALT39.StrType)le.user_changed),
    Fields.InValid_date_changed((SALT39.StrType)le.date_changed),
    Fields.InValid_fcra_purpose((SALT39.StrType)le.fcra_purpose),
    ...
    Fields.InValid_orig_jobid((SALT39.StrType)le.orig_jobid),
    Fields.InValid_orig_acctno((SALT39.StrType)le.orig_acctno),
...
END;
Errors := NORMALIZE(h,68,NoteErrors(LEFT,COUNTER));
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT39.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_Inql_fcra_Accurint, Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

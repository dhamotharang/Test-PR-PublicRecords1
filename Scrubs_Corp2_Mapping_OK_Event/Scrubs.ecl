...
EXPORT FromNone(DATASET(corp2_Mapping.LayoutsCommon.Events) h) := MODULE
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.corp_key_Invalid << 0 ) + ( le.corp_sos_charter_nbr_Invalid << 2 ) + ( le.event_filing_date_Invalid << 4 ) + ( le.event_filing_cd_Invalid << 6 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.event_filing_date_Invalid := (le.ScrubsBits1 >> 4) & 3;
    SELF.event_filing_cd_Invalid := (le.ScrubsBits1 >> 6) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.corp_key_Invalid,le.corp_sos_charter_nbr_Invalid,le.event_filing_date_Invalid,le.event_filing_cd_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_corp_key(le.corp_key_Invalid),Fields.InvalidMessage_corp_sos_charter_nbr(le.corp_sos_charter_nbr_Invalid),Fields.InvalidMessage_event_filing_date(le.event_filing_date_Invalid),Fields.InvalidMessage_event_filing_cd(le.event_filing_cd_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ,CHOOSE(le.corp_key_Invalid,'ALLOW','LENGTH','UNKNOWN')
          ,CHOOSE(le.corp_sos_charter_nbr_Invalid,'ALLOW','LENGTH','UNKNOWN')
          ,CHOOSE(le.event_filing_date_Invalid,'ALLOW','CUSTOM','LENGTH','UNKNOWN')
          ,CHOOSE(le.event_filing_cd_Invalid,'ENUM','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'corp_key','corp_sos_charter_nbr','event_filing_date','event_filing_cd','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_corp_key','invalid_charter','invalid_optional_date','invalid_filing_cd','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT32.StrType)le.corp_key,(SALT32.StrType)le.corp_sos_charter_nbr,(SALT32.StrType)le.event_filing_date,(SALT32.StrType)le.event_filing_cd,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT32.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'event_filing_date:invalid_optional_date:ALLOW','event_filing_date:invalid_optional_date:CUSTOM','event_filing_date:invalid_optional_date:LENGTH'

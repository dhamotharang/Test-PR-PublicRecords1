...
*/
  SHARED EarthRadius := 3963.3453; // In synch with UT.LL_Dist
	SHARED PI := 3.14159;
	EXPORT Distance(REAL8 Lat1,REAL8 Long1, REAL8 Lat2, REAL8 Long2 ) := ut.LL_Dist(lat1,long1,lat2,long2);
...
// Geohash
	EXPORT Encode(REAL8 lat,REAL8 long,INTEGER1 Prec=12) := fn_LatLongEncode(lat,long,Prec);
	EXPORT Decode(STRING inc) := fn_LatLongDecode(inc);
	EXPORT Neighbor(STRING12 inc, STRING2 dir) := FUNCTION
	  INTEGER1 Dir0 := MAP ( dir IN ['N ','NE','NW'] => 1, dir IN ['S ','SW','SE'] => -1, 0 );
		...
		RETURN fn_latlongneighbor(TRIM(inc),Dir0,Dir1);
	END;
  ...
  SHARED Rect(STRING12 tl, STRING12 tr, STRING12 bl, STRING12 br, STRING12 mid, SetOverlap ov) := FUNCTION
	  ...
	  Vals := DATASET( MAP ( trm = best AND brm = best => [ tre ],
		...
		RETURN ov ( AppendBox( LevelNP1( PROJECT(Vals,LayoutGeoList))))(Overlap<2);						 
	END;
	...
	EXPORT PointRadius(STRING12 fr,REAL8 radius) := FUNCTION
	  D := Decode(fr);
    ...
    IRect( DATASET(EGL) p ) := FUNCTION
		...
		br := Encode(bra,bro);
		bl := Encode(bra,tlo);
		RETURN Rect(tl,tr,bl,br,fr,IRect);
	END;

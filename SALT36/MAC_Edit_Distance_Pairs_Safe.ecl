export MAC_Edit_Distance_Pairs(infile,infield,countfield,thresh,phonetics,outfile) := MACRO
	...
	%matchrec% %trans%(infile le,infile ri) := transform
	  self.id1 := le.infield;
		...
		self.distance := MAP( le.infield=ri.infield => 0,
													~SALT36.utWithinEditN(le.infield,ri.infield,2) => 10,
													SALT36.utWithinEditN(le.infield,ri.infield,1) => 1, 2 );
  ...
  // Use ,all for <= 4; allows the 'tiddlers' to get correct (and v.low) specificities
	...
	#uniquename(j0)
	%j0% := join(%inf4%,%inf4%,SALT36.utWithinEditN(left.infield,right.infield,thresh),%trans%(left,right),all);	
	...
	#uniquename(j1)
	%j1% := join(%inf7%,%inf7%,left.infield[1..2]=right.infield[1..2] and  left.infield[3..*]=right.infield[3..*] and SALT36.utWithinEditN(left.infield,right.infield,thresh),%trans%(left,right),atmost(left.infield[1..2]=right.infield[1..2] and  left.infield[3..*]=right.infield[3..*],3500),hash);
	#uniquename(j2)
	%j2% := join(%inf%,%inf%,left.infield[1..3]=right.infield[1..3] and  left.infield[4..*]=right.infield[4..*] and SALT36.utWithinEditN(left.infield,right.infield,thresh),%trans%(left,right),atmost(left.infield[1..3]=right.infield[1..3] and  left.infield[4..*]=right.infield[4..*],2500),hash);
	#uniquename(j3)
	%j3% := join(%inf7%,%inf7%,left.infield[3..*]=right.infield[3..*] and left.infield[length(trim(left.infield))-2..] = right.infield[length(trim(right.infield))-2..] and SALT36.utWithinEditN(left.infield,right.infield,thresh),%trans%(left,right),atmost(left.infield[3..*]=right.infield[3..*] and left.infield[length(trim(left.infield))-2..] = right.infield[length(trim(right.infield))-2..],3500),hash);
	#uniquename(j4)
	%j4% := join(%inf%,%inf%,left.infield[4..*]=right.infield[4..*] and left.infield[length(trim(left.infield))-3..] = right.infield[length(trim(right.infield))-3..] and SALT36.utWithinEditN(left.infield,right.infield,thresh),%trans%(left,right),atmost(left.infield[4..*]=right.infield[4..*] and left.infield[length(trim(left.infield))-3..] = right.infield[length(trim(right.infield))-3..],2500),hash);
	#uniquename(j5)
	// Allow positions 4&5 to vary
	%j5% := join(%inf%,%inf%,left.infield[1..*]=right.infield[1..*] and left.infield[length(trim(left.infield))-2..] = right.infield[length(trim(right.infield))-2..] and SALT36.utWithinEditN(left.infield,right.infield,thresh),%trans%(left,right),atmost(left.infield[1..*]=right.infield[1..*] and left.infield[length(trim(left.infield))-2..] = right.infield[length(trim(right.infield))-2..],2500),hash);
	#uniquename(jnull)

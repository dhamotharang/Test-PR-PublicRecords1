...
the co_old_did and set the dt_last_usable to the co_date (minus one month/day/second, depending on date format of YYYYMM/YYYYMMDD/YYYYMMDDHHMMSS
so that the rid does not have two did associations on the same date)
and the dt_first_usable to the earliest date on the record (in_dt_early). This generates NEW records.
...
Note that the format of co_date will be expanded (dangerous- hopefully we don't store changes at a lower resolution than in_dt_early/in_dt_late- we can miss applying changes
in this case if there are multiple changes for the same in_rid_field/co_date)
or truncated to match the date format of in_dt_early/in_dt_late (also dangerous because we'd be muting intermediate changes within a day,
...
EXPORT MAC_ASOF_Match_Candidates(infile, in_did_field, in_rid_field, in_dt_early, in_dt_late, co_collapses, co_old_did, co_new_did, co_rid, co_date) := FUNCTIONMACRO
  ...
  END;
	...
	// date type of in_dt_early and in_dt_late from infile
	...
	infileSampleEarlyDate := (UNSIGNED6)(infile(in_dt_early > 0)[1].in_dt_early);
	infileSampleLateDate  := (UNSIGNED6)(infile(in_dt_late > 0)[1].in_dt_late);
	...
	// function to convert collapse date to same date format as in_dt_early/in_dt_late when setting start or end dates
	getDate(UNSIGNED6 collapseDateIn, BOOLEAN isEnd) := FUNCTION
	  ...
	  useDateStart	:= MAP(infileDateIsYYYYMMDDHHMMSS => (UNSIGNED6)(collapseDateInYr*10000000000 + collapseDateInMonth*100000000 + collapseDateInDay*1000000 + collapseDateInHr*10000 + collapseDateInMin*100 + collapseDateInSec),
		...
		RETURN (TYPEOF(infile.in_dt_early))useDate;
	END;

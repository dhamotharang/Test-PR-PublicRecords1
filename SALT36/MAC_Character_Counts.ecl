...
export Data_Layout := RECORD
  SALT36.StrType Fld { MAXLENGTH(200000) };
	UNSIGNED2 FldNo;
  END;
...
shared Pattern_Layout := record
  SALT36.StrType data_pattern {maxlength(200000)};
	unsigned cnt;
	REAL pcnt;
	end;
	
shared Value_Layout := record
  SALT36.StrType val  {maxlength(200000)};
	unsigned cnt;
	REAL pcnt;
	end;
...
export ResultLine_Layout := record(Field_Identification)
	...
	SALT36.Str30Type 					MinVal30;
	SALT36.Str30Type 					MaxVal30;
...
EXPORT EclRecord(DATASET(ResultLine_Layout) d, STRING RName='Record_Layout') := FUNCTION
	...
	r1 WriteEcl(t le) := TRANSFORM
		...
		SELF.Txt := '  '+MAP( ~le.AllNum => 'UNICODE' + IF(le.MaxLen<MaxFixedLen,(SALT36.StrType)le.MaxLen, ''),
#else
		SELF.Txt := '  '+MAP( ~le.AllNum => 'STRING' + IF(le.MaxLen<MaxFixedLen,(SALT36.StrType)le.MaxLen, ''),
#end
...
EXPORT FieldTypes(DATASET(ResultLine_Layout) d, REAL Recall = 99.9) := FUNCTION
  PruneOutliers(file,cnt_field) := FUNCTIONMACRO
	...
	ENDMACRO;
  SALT36.StrType GetChars(DATASET(Character_Layout) gc) := FUNCTION
	  T := TABLE(SORT(PruneOutliers(gc(),cnt),c),{ SALT36.StrType s := c; });
		RETURN ROLLUP(t,TRUE,TRANSFORM(RECORDOF(T),SELF.S := LEFT.S+RIGHT.S))[1].s;
	END;
	
	SALT36.StrType LenList(DATASET(Length_Layout) lens) := FUNCTION
	  T := TABLE(PruneOutliers(lens,cnt),{ SALT36.StrType s := ','+len; });
		s := ROLLUP(t,TRUE,TRANSFORM(RECORDOF(T),SELF.S := LEFT.S+RIGHT.S))[1].s[2..];
	  RETURN ':LENGTHS('+s+')';
	END;
	SALT36.StrType WordList(DATASET(Words_Layout) wds) := FUNCTION
	  T := TABLE(PruneOutliers(wds,cnt),{ SALT36.StrType s := ','+words; });
		s := ROLLUP(t,TRUE,TRANSFORM(RECORDOF(T),SELF.S := LEFT.S+RIGHT.S))[1].s[2..];
	  RETURN ':WORDS('+s+')';
	END;
	...
	R := RECORD
	  d.FldNo;
	  d.FieldName;
		SALT36.StrType Allow := ':ALLOW('+GetChars(d.Characters)+')';
		SALT36.StrType LTrim := IF ( EXISTS(d.patterns(data_pattern[1]=' ',TRIM(data_pattern[2..])<>'')),'',':LEFTTRIM');
		SALT36.StrType Lens := LenList(d.len);
		SALT36.StrType Words := WordList(d.words);
		SALT36.StrType Spaces := ':SPACES( )';
	END;
	t := TABLE(d,r);
	r1 := RECORD
	  t.FldNo;
...
glbl_field_cnts := record
  FldInv1;
	unsigned2 xx_l := length(trim((SALT36.StrType)FldInv1.Fld));
	unsigned2 xx_wc := SALT36.WordCount((SALT36.StrType)FldInv1.Fld);
  end;
...
card_rec := RECORD
  FldInv.FldNo;
	...
	SALT36.Str30Type MaxVal30 := MAX(GROUP,FldInv.Fld);
	SALT36.Str30Type MinVal30 := MIN(GROUP,IF(FldInv.Fld='','ZZ--BOGUSVALUE--ZZ',FldInv.Fld));
  END;	
...
r tr(FldInv le,unsigned co) := transform
  self.c := ((SALT36.StrType)le.Fld)[co];
	self := le;
  end;
...
dr := record
	  SALT36.StrType Fld  {maxlength(200000)} := SALT36.fn_data_pattern(FldInv.Fld);
		FldInv.Cnt;
		FldInv.FldNo;
  end;
t := table(FldInv,dr);

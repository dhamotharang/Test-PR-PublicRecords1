...
EXPORT proc_build_autokeys_ContLegalPhys(string filedate) := function

	c := prte2_ncpdp._Constants(filedate);
 
	/* Base file, which is a slimmed down version of the full keybuild layout */
	ak_dataset := prte2_ncpdp.Files.autokey_ContLegalPhys;

	ak_keyname := c.ak_keyname_ContLegalPhys; /* AutoKey Superfile container name */
	ak_logical := c.ak_logical_ContLegalPhys; /* AutoKey Logical file name */
	ak_skipSet := c.ak_skipSet_ContLegalPhys; /* Parms for autokeys to bypass */

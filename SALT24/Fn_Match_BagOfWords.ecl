export integer4 fn_match_bagofwords(const string l, const string r, unsigned1 mode=0, unsigned1 score_mode=0) := BEGINC++

namespace ns_SALT24_Fn_Match_Bagofwords {
...
using namespace ns_SALT24_Fn_Match_Bagofwords;
#body
#option pure
	// Now deal with leading 'uber' weight
#define MAX_WORDS 32
	const char* words[MAX_WORDS];
	unsigned short word_lengths[MAX_WORDS];
	unsigned short specs[MAX_WORDS];
	bool  used[MAX_WORDS];
	...
	for ( int i = 0; i < nfails; i++ )
	{
		for ( int j = 0; j < nwords; j++ )
			...
			||   (mode == 2 && ns_SALT24_Fn_Match_Bagofwords::abbr_eq(fail_words[i],fail_lengths[i],words[j],word_lengths[j]))
			) )
			{
	...
	if ( (score_mode >= 2) && (score_mode != 3) ) // applies to ALL[n] and TOTAL
	...
	case 4: // TOTAL - like ALL, but fail on mismatch
		return mismatching_weight>0 ? -9900 : spec * matching_weight / total_weight;
	case 5: // ALL2 - like ALL, but a mismatch is 2x as bad as a match is good
		return (double)spec * ( matching_weight - 2*mismatching_weight ) / total_weight;
	case 6: // ALL4 - like ALL, but a mismatch is 4x as bad as a match is good
		return (double)spec * ( matching_weight - 4*mismatching_weight ) / total_weight;
	default:
		return 0;
	}
ENDC++;

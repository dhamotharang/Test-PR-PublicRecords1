...
export Data_Layout := RECORD
  SALT24.StrType Fld { MAXLENGTH(200000) };
	UNSIGNED2 FldNo;
  END;
...
shared Pattern_Layout := record
  SALT24.StrType data_pattern {maxlength(200000)};
	unsigned cnt;
	end;
	
shared Value_Layout := record
  SALT24.StrType val  {maxlength(200000)};
	unsigned cnt;
	end;
export Field_Identification := RECORD
  unsigned2 FldNo;
  SALT24.StrType FieldName;
  END;
...
glbl_field_cnts := record
  FldInv1;
	unsigned2 xx_l := length(trim((SALT24.StrType)FldInv1.Fld));
	unsigned2 xx_wc := SALT24.WordCount((SALT24.StrType)FldInv1.Fld);
  end;
...
r tr(FldInv le,unsigned co) := transform
  self.c := ((SALT24.StrType)le.Fld)[co];
	self := le;
  end;
...
dr := record
	  SALT24.StrType Fld  {maxlength(200000)} := SALT24.fn_data_pattern(FldInv.Fld);
		FldInv.Cnt;
		FldInv.FldNo;
  end;
t := table(FldInv,dr);

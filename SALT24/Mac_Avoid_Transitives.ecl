export mac_avoid_transitives(ifile,did1,did2,conf,dover,rule_num,ofile) := macro
  ...
  #uniquename(matchrec)	
	...
	SALT24.UIDType id1;
	SALT24.UIDType id2;
	SALT24.UIDType targetid; // target did
	//%j1%.conf;
	end;
	...
	%j3% := dedup(sort(join(%j2%, %add_reverse%, left.id1 = right.did1 and left.id2 = right.did2, transform(left), left only, hash),id1),id1);
	...
	#uniquename(tidrec)
	%tidrec% := RECORD
	SALT24.UIDType target_id; // target did
	end;

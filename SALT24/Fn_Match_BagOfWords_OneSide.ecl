...
namespace ns_SALT24_Fn_Match_Bagofwords_OneSide {
...
using namespace ns_SALT24_Fn_Match_Bagofwords_OneSide;
#body
#option pure
	// Now deal with leading 'uber' weight
#define MAX_WORDS 32
	...
	for ( int i = 0; i < nfails; i++ )
	{
		for ( int j = 0; j < nwords; j++ )
			...
			||   (mode == 2 && ns_SALT24_Fn_Match_Bagofwords_OneSide::abbr_eq(fail_words[i],fail_lengths[i],words[j],word_lengths[j]))
			) )
			{

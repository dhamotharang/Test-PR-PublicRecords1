export Mac_Choose_JoinType(mainfile,null_dataset,valuefile,field_name,fieldweight_name,trans_name,outfile,forceSmall=false) := MACRO
   
// outfile := join(mainfile,PULL(valuefile),left.field_name=right.field_name,trans_name(left,right,true),lookup, left outer);
...
outfile :=	if (forceSmall OR count(valuefile)*sizeof(valuefile,max)<1000000000,%sj%,%sjbp%);
  endmacro;

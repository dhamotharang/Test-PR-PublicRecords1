...
IMPORT B_Person_8,E_Address,E_Customer,E_Customer_Person_Event,E_Event,E_Person FROM KELOtto;
IMPORT * FROM KEL011.Null;
EXPORT B_Customer_Person_Event_7 := MODULE
  SHARED VIRTUAL TYPEOF(E_Customer_Person_Event.__Result) __E_Customer_Person_Event := E_Customer_Person_Event.__Result;
  SHARED VIRTUAL TYPEOF(B_Person_8.__ENH_Person_8) __ENH_Person_8 := B_Person_8.__ENH_Person_8;
  SHARED __EE6732 := __E_Customer_Person_Event;
  SHARED __EE6769 := __ENH_Person_8;
  SHARED __ST6819_Layout := RECORD
  ...
  END;
  __JC6816(E_Customer_Person_Event.Layout __EE6732, B_Person_8.__ST5321_Layout __EE6769) := __EEQP(__EE6732.Subject_,__EE6769.UID);
  __ST6819_Layout __JT6816(E_Customer_Person_Event.Layout __l, B_Person_8.__ST5321_Layout __r) := TRANSFORM
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE6817 := JOIN(__EE6732,__EE6769,__JC6816(LEFT,RIGHT),__JT6816(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST4977_Layout := RECORD
    KEL.typ.ntyp(E_Customer.Typ) Source_Customer_;
    KEL.typ.ntyp(E_Customer.Typ) Associated_Customer_;
    KEL.typ.ntyp(E_Person.Typ) Subject_;
    KEL.typ.ntyp(E_Address.Typ) Location_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST4977_Layout __ND7000__Project(__ST6819_Layout __PP6986) := TRANSFORM
    SELF.Deceased_Prior_To_Event_ := MAP(__T(__AND(__CN(__PP6986.Deceased_ = 1),__OP2(__PP6986.Deceased_Date_,<,__PP6986.Event_Date_)))=>1,0);
    SELF := __PP6986;
  END;
  EXPORT __ENH_Customer_Person_Event_7 := PROJECT(__EE6817,__ND7000__Project(LEFT));
END;

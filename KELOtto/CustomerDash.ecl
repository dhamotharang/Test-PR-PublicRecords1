EXPORT CustomerDash := MODULE
  ...
  //topclusters
  OttoFullGraph := KELOtto.KelFiles.FullCluster; //DATASET('~gov::otto::fullgraph', RECORDOF(KELOtto.KelFiles.FullCluster), THOR);  
  ...
  topstuff := tempFullCluster((cl_high_risk_pattern1_flag_ = 1 OR cl_high_risk_pattern2_flag_ = 1 OR cl_high_risk_pattern3_flag_ = 1 OR cl_high_risk_pattern4_flag_ = 1 OR cl_high_risk_pattern5_flag_ = 1) AND cl_identity_count_ < 30 AND tree_uid_=entity_context_uid_ AND cl_event_count_percentile_ > 85);
  //count(topstuff);

  topnormalstuff := topn(GROUP(SORT(tempFullCluster((cl_high_risk_pattern1_flag_ = 0 AND cl_high_risk_pattern2_flag_ = 1 AND cl_high_risk_pattern3_flag_ = 1) AND tree_uid_=entity_context_uid_ AND cl_event_count_percentile_ > 85), customer_id_), customer_id_), 10000, -cl_impact_weight_);
  ...
  ts2 := topn(GROUP(SORT(topstuff((cl_high_risk_pattern1_flag_ = 0 AND (cl_high_risk_pattern2_flag_ = 1 OR cl_high_risk_pattern3_flag_ = 1) )), customer_id_), customer_id_), 10000, -cl_impact_weight_);
  //count(ts2);

  ts3 := topn(GROUP(SORT(topstuff((cl_high_risk_pattern2_flag_ = 0 AND (cl_high_risk_pattern1_flag_ = 1 OR cl_high_risk_pattern3_flag_ = 1) )), customer_id_), customer_id_), 10000, -cl_impact_weight_);
  //count(ts3);

  ts4 := topn(GROUP(SORT(topstuff((cl_high_risk_pattern3_flag_ = 0 AND (cl_high_risk_pattern1_flag_ = 1 OR cl_high_risk_pattern2_flag_ = 1) )), customer_id_), customer_id_), 10000, -cl_impact_weight_);
  //count(ts4);

  MainClustersPrep1 := DEDUP(SORT(topnormalstuff + ts1 + ts2 + ts3 + ts4, customer_id_, entity_context_uid_), customer_id_, entity_context_uid_);
  ...
  TopIdentitiesPrep1 := tempFullCluster(tree_uid_=entity_context_uid_ AND entity_type_ = 1 AND score_ > 80 AND cl_event_count_percentile_ > 85 AND cl_identity_count_ < 20);
  TopIdentitiesPrep2 := topn(GROUP(SORT(TopIdentitiesPrep1, customer_id_, skew(1)), customer_id_), 100, customer_id_, -cl_impact_weight_);
  //count(TopIdentitiesPrep2);

  // Top Elements

  TopElementsPrep1 := tempFullCluster(tree_uid_=entity_context_uid_ AND entity_type_ != 1);
  TopElementsPrep2 := topn(GROUP(SORT(TopElementsPrep1, customer_id_, entity_type_, skew(1)), customer_id_, entity_type_), 500, customer_id_, entity_type_, -cl_impact_weight_, -score_);

  ClustersAndElements := DEDUP(SORT(MainClusters + TopIdentitiesPrep2 + TopElementsPrep2, customer_id_, entity_context_uid_), customer_id_, entity_context_uid_);

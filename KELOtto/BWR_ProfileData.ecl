IMPORT KELOtto, SALTRoutines;

fraudgov := KELOtto.fraudgov;

OUTPUT(CHOOSEN(fraudgov, 25), NAMED('Sample_Raw_Input'));
...
split_1 := RECORD,maxlength(60000)
  unsigned8 record_id;
...
split_2 := RECORD,maxlength(60000)
...
split_3 := RECORD,maxlength(60000)
		string100 address_1;
  string50 address_2;
  unsigned8 rawaid;
  unsigned8 aceaid;
...
fraudgov_part1 := PROJECT(fraudgov,TRANSFORM(split_1, SELF := LEFT));
fraudgov_part2 := PROJECT(fraudgov,TRANSFORM(split_2, SELF := LEFT));
fraudgov_part3 := PROJECT(fraudgov,TRANSFORM(split_3, SELF := LEFT));

// OUTPUT(SALTRoutines.SALT_Profile_Run_Everything(fraudgov, 'FRAUDGOV'),NAMED('FraudGov_SALT_profile'));
OUTPUT(SALTRoutines.SALT_Profile_Run_Everything(fraudgov_part1, 'FRAUDGOV_1'),NAMED('FraudGov_SALT_profile_part1'));
OUTPUT(SALTRoutines.SALT_Profile_Run_Everything(fraudgov_part2, 'FRAUDGOV_2'),NAMED('FraudGov_SALT_profile_part2'));
OUTPUT(SALTRoutines.SALT_Profile_Run_Everything(fraudgov_part3, 'FRAUDGOV_3'),NAMED('FraudGov_SALT_profile_part3'));

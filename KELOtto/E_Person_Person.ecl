...
EXPORT E_Person_Person := MODULE
  ...
  END;
  SHARED __d0_From_Person__Mapped := JOIN(KELOtto.AddressPersonAssociations.PersonAddressMatchStats,E_Person.Lookup,TRIM((STRING)LEFT.fdn_ind_type_gc_id_inclusion) + '|' + TRIM((STRING)LEFT.frompersonlexid) = RIGHT.KeyVal,TRANSFORM(__d0_From_Person__Layout,SELF.From_Person_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,HASH);
  SHARED __d0_To_Person__Layout := RECORD
    RECORDOF(__d0_From_Person__Mapped);
    KEL.typ.uid To_Person_;
  END;
  SHARED __d0_To_Person__Mapped := JOIN(__d0_From_Person__Mapped,E_Person.Lookup,TRIM((STRING)LEFT.fdn_ind_type_gc_id_inclusion) + '|' + TRIM((STRING)LEFT.topersonlexid) = RIGHT.KeyVal,TRANSFORM(__d0_To_Person__Layout,SELF.To_Person_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,HASH);
  ...
  END;
  EXPORT __PreResult := PROJECT(TABLE(InData,{KEL.typ.int __RecordCount := COUNT(GROUP),KEL.typ.epoch Date_First_Seen_ := KEL.era.SimpleRoll(GROUP,Date_First_Seen_,MIN,TRUE),KEL.typ.epoch Date_Last_Seen_ := KEL.era.SimpleRoll(GROUP,Date_Last_Seen_,MAX,FALSE),_r_Customer_,From_Person_,To_Person_,Public_Records_,Contributory_Records_,Same_Address_Email_Match_,Same_Address_Ssn_Match_,Same_Address_Phone_Number_Match_,Same_Address_Same_Day_Count_,High_Frequency_Same_Address_Same_Day_Count_,Non_High_Frequency_Address_Count_,Non_High_Frequency_Same_Address_Same_Day_Count_,Shared_Address_Count_},_r_Customer_,From_Person_,To_Person_,Public_Records_,Contributory_Records_,Same_Address_Email_Match_,Same_Address_Ssn_Match_,Same_Address_Phone_Number_Match_,Same_Address_Same_Day_Count_,High_Frequency_Same_Address_Same_Day_Count_,Non_High_Frequency_Address_Count_,Non_High_Frequency_Same_Address_Same_Day_Count_,Shared_Address_Count_,MERGE),Layout);
  ...
  EXPORT NullCounts := DATASET([
    {'PersonPerson','KELOtto.AddressPersonAssociations.PersonAddressMatchStats','fdn_ind_type_gc_id_inclusion',COUNT(__d0(__NL(_r_Customer_))),COUNT(__d0(__NN(_r_Customer_)))},

...
EXPORT B_Person_Address := MODULE
  ...
  SHARED __EE2628396 := __E_Person_Address;
  EXPORT __ST46017_Layout := RECORD
  ...
  END;
  SHARED __ST46017_Layout __ND2628430__Project(E_Person_Address.Layout __PP2628291) := TRANSFORM
    __EE2628329 := __PP2628291.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2628329,__T(__EE2628329).Event_Date_);
    __EE2628358 := __PP2628291.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2628358,__T(__EE2628358).Event_Date_);
    SELF := __PP2628291;
  END;
  EXPORT __ENH_Person_Address := PROJECT(__EE2628396,__ND2628430__Project(LEFT)) : PERSIST('~temp::KEL::KELOtto::Person_Address::Annotated',EXPIRE(7));
  SHARED __EE3312725 := __ENH_Person_Address;
  SHARED __IDX_Person_Address_Location__Filtered := __EE3312725(__NN(__EE3312725.Location_));
  ...
  EXPORT __ST3312727_Layout := RECORDOF(IDX_Person_Address_Location_);
  EXPORT IDX_Person_Address_Location__Wrapped := PROJECT(IDX_Person_Address_Location_,TRANSFORM(__ST46017_Layout,SELF.Location_ := __CN(LEFT.Location_),SELF:=LEFT));

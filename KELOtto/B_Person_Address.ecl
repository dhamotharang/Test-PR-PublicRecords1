...
EXPORT B_Person_Address := MODULE
  ...
  SHARED __EE2633368 := __E_Person_Address;
  EXPORT __ST46059_Layout := RECORD
  ...
  END;
  SHARED __ST46059_Layout __ND2633402__Project(E_Person_Address.Layout __PP2633263) := TRANSFORM
    __EE2633301 := __PP2633263.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2633301,__T(__EE2633301).Event_Date_);
    __EE2633330 := __PP2633263.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2633330,__T(__EE2633330).Event_Date_);
    SELF := __PP2633263;
  END;
  EXPORT __ENH_Person_Address := PROJECT(__EE2633368,__ND2633402__Project(LEFT)) : PERSIST('~temp::KEL::KELOtto::Person_Address::Annotated',EXPIRE(7));
  SHARED __EE3317553 := __ENH_Person_Address;
  SHARED __IDX_Person_Address_Location__Filtered := __EE3317553(__NN(__EE3317553.Location_));
  ...
  EXPORT __ST3317555_Layout := RECORDOF(IDX_Person_Address_Location_);
  EXPORT IDX_Person_Address_Location__Wrapped := PROJECT(IDX_Person_Address_Location_,TRANSFORM(__ST46059_Layout,SELF.Location_ := __CN(LEFT.Location_),SELF:=LEFT));

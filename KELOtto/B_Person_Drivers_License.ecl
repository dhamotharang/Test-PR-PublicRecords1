...
EXPORT B_Person_Drivers_License := MODULE
  ...
  SHARED __EE2534674 := __E_Person_Drivers_License;
  EXPORT __ST43136_Layout := RECORD
  ...
  END;
  SHARED __ST43136_Layout __ND2534708__Project(E_Person_Drivers_License.Layout __PP2534569) := TRANSFORM
    __EE2534607 := __PP2534569.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2534607,__T(__EE2534607).Event_Date_);
    __EE2534636 := __PP2534569.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2534636,__T(__EE2534636).Event_Date_);
    SELF := __PP2534569;
  END;
  EXPORT __ENH_Person_Drivers_License := PROJECT(__EE2534674,__ND2534708__Project(LEFT)) : PERSIST('~temp::KEL::KELOtto::Person_Drivers_License::Annotated',EXPIRE(7));
  SHARED __EE3175808 := __ENH_Person_Drivers_License;
  SHARED __IDX_Person_Drivers_License_License__Filtered := __EE3175808(__NN(__EE3175808.License_));
  ...
  EXPORT __ST3175810_Layout := RECORDOF(IDX_Person_Drivers_License_License_);
  EXPORT IDX_Person_Drivers_License_License__Wrapped := PROJECT(IDX_Person_Drivers_License_License_,TRANSFORM(__ST43136_Layout,SELF.License_ := __CN(LEFT.License_),SELF:=LEFT));

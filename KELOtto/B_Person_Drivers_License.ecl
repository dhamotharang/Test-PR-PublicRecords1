...
EXPORT B_Person_Drivers_License := MODULE
  ...
  SHARED __EE2114020 := __E_Person_Drivers_License;
  EXPORT __ST39889_Layout := RECORD
  ...
  END;
  SHARED __ST39889_Layout __ND2114054__Project(E_Person_Drivers_License.Layout __PP2113915) := TRANSFORM
    __EE2113953 := __PP2113915.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2113953,__T(__EE2113953).Event_Date_);
    __EE2113982 := __PP2113915.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2113982,__T(__EE2113982).Event_Date_);
    SELF := __PP2113915;
  END;
  EXPORT __ENH_Person_Drivers_License := PROJECT(__EE2114020,__ND2114054__Project(LEFT)) : PERSIST('~temp::KEL::KELOtto::Person_Drivers_License::Annotated',EXPIRE(7));
  SHARED __EE2632098 := __ENH_Person_Drivers_License;
  SHARED __IDX_Person_Drivers_License_License__Filtered := __EE2632098(__NN(__EE2632098.License_));
  ...
  EXPORT __ST2632100_Layout := RECORDOF(IDX_Person_Drivers_License_License_);
  EXPORT IDX_Person_Drivers_License_License__Wrapped := PROJECT(IDX_Person_Drivers_License_License_,TRANSFORM(__ST39889_Layout,SELF.License_ := __CN(LEFT.License_),SELF:=LEFT));

...
EXPORT B_Person_Drivers_License := MODULE
  ...
  SHARED __EE2628760 := __E_Person_Drivers_License;
  EXPORT __ST46063_Layout := RECORD
  ...
  END;
  SHARED __ST46063_Layout __ND2628794__Project(E_Person_Drivers_License.Layout __PP2628655) := TRANSFORM
    __EE2628693 := __PP2628655.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2628693,__T(__EE2628693).Event_Date_);
    __EE2628722 := __PP2628655.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2628722,__T(__EE2628722).Event_Date_);
    SELF := __PP2628655;
  END;
  EXPORT __ENH_Person_Drivers_License := PROJECT(__EE2628760,__ND2628794__Project(LEFT)) : PERSIST('~temp::KEL::KELOtto::Person_Drivers_License::Annotated',EXPIRE(7));
  SHARED __EE3312513 := __ENH_Person_Drivers_License;
  SHARED __IDX_Person_Drivers_License_License__Filtered := __EE3312513(__NN(__EE3312513.License_));
  ...
  EXPORT __ST3312515_Layout := RECORDOF(IDX_Person_Drivers_License_License_);
  EXPORT IDX_Person_Drivers_License_License__Wrapped := PROJECT(IDX_Person_Drivers_License_License_,TRANSFORM(__ST46063_Layout,SELF.License_ := __CN(LEFT.License_),SELF:=LEFT));

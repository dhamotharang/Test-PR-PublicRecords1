...
EXPORT E_Address := MODULE
  ...
  SHARED __Mapping := 'UID(UID),associatedcustomerfileinfo(_r_Customer_:0),sourcecustomerfileinfo(_r_Source_Customer_:0),ottoaddressid(Otto_Address_Id_:0),primaryrange(Primary_Range_),predirectional(Predirectional_),primaryname(Primary_Name_),suffix(Suffix_),postdirectional(Postdirectional_),unitdesignation(Unit_Designation_),secondaryrange(Secondary_Range_),postalcity(Postal_City_),vanitycity(Vanity_City_),state(State_),zip(Z_I_P_),zip4(Z_I_P4_),carrierroutenumber(Carrier_Route_Number_),carrierroutesortationatzip(Carrier_Route_Sortation_At_Z_I_P_),lineoftravel(Line_Of_Travel_),lineoftravelorder(Line_Of_Travel_Order_),deliverypointbarcode(Delivery_Point_Barcode_),deliverypointbarcodecheckdigit(Delivery_Point_Barcode_Check_Digit_),typecode(Type_Code_),county(County_),latitude(Latitude_),longitude(Longitude_),metropolitanstatisticalarea(Metropolitan_Statistical_Area_),geoblock(Geo_Block_),geomatch(Geo_Match_),acecleanererrorcode(A_C_E_Cleaner_Error_Code_),isadditional(_is_Additional_),datefirstseen(Date_First_Seen_:EPOCH),datelastseen(Date_Last_Seen_:EPOCH)';
  ...
  Layout Address__Rollup(InLayout __r, DATASET(InLayout) __recs) := TRANSFORM
    ...
    SELF.Z_I_P_ := KEL.Intake.SingleValue(__recs,Z_I_P_);
  ...
  EXPORT Z_I_P__SingleValue_Invalid := KEL.Intake.DetectMultipleValues(__PreResult,Z_I_P_);
  ...
  EXPORT NullCounts := DATASET([
    ...
    {'Address','KELOtto.fraudgovshared','clean_address.zip',COUNT(__d0(__NL(Z_I_P_))),COUNT(__d0(__NN(Z_I_P_)))},
    {'Address','KELOtto.fraudgovshared','clean_address.zip4',COUNT(__d0(__NL(Z_I_P4_))),COUNT(__d0(__NN(Z_I_P4_)))},

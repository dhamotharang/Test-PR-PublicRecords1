...
EXPORT B_Person_Email := MODULE
  ...
  SHARED __EE2633914 := __E_Person_Email;
  EXPORT __ST46128_Layout := RECORD
  ...
  END;
  SHARED __ST46128_Layout __ND2633948__Project(E_Person_Email.Layout __PP2633809) := TRANSFORM
    __EE2633847 := __PP2633809.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2633847,__T(__EE2633847).Event_Date_);
    __EE2633876 := __PP2633809.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2633876,__T(__EE2633876).Event_Date_);
    SELF := __PP2633809;
  END;
  EXPORT __ENH_Person_Email := PROJECT(__EE2633914,__ND2633948__Project(LEFT)) : PERSIST('~temp::KEL::KELOtto::Person_Email::Annotated',EXPIRE(7));
  SHARED __EE3316949 := __ENH_Person_Email;
  SHARED __IDX_Person_Email_Emailof__Filtered := __EE3316949(__NN(__EE3316949.Emailof_));
  ...
  EXPORT __ST3316951_Layout := RECORDOF(IDX_Person_Email_Emailof_);
  EXPORT IDX_Person_Email_Emailof__Wrapped := PROJECT(IDX_Person_Email_Emailof_,TRANSFORM(__ST46128_Layout,SELF.Emailof_ := __CN(LEFT.Emailof_),SELF:=LEFT));

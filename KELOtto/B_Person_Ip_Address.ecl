...
IMPORT E_Address,E_Customer,E_Internet_Protocol,E_Person,E_Person_Ip_Address FROM KELOtto;
...
EXPORT B_Person_Ip_Address := MODULE
  ...
  SHARED __EE2634096 := __E_Person_Ip_Address;
  EXPORT __ST46151_Layout := RECORD
  ...
  END;
  SHARED __ST46151_Layout __ND2634130__Project(E_Person_Ip_Address.Layout __PP2633991) := TRANSFORM
    __EE2634029 := __PP2633991.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2634029,__T(__EE2634029).Event_Date_);
    __EE2634058 := __PP2633991.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2634058,__T(__EE2634058).Event_Date_);
    SELF := __PP2633991;
  END;
  EXPORT __ENH_Person_Ip_Address := PROJECT(__EE2634096,__ND2634130__Project(LEFT)) : PERSIST('~temp::KEL::KELOtto::Person_Ip_Address::Annotated',EXPIRE(7));
  SHARED __EE3316963 := __ENH_Person_Ip_Address;
  SHARED __IDX_Person_Ip_Address_Ip__Filtered := __EE3316963(__NN(__EE3316963.Ip_));
  ...
  EXPORT __ST3316965_Layout := RECORDOF(IDX_Person_Ip_Address_Ip_);
  EXPORT IDX_Person_Ip_Address_Ip__Wrapped := PROJECT(IDX_Person_Ip_Address_Ip_,TRANSFORM(__ST46151_Layout,SELF.Ip_ := __CN(LEFT.Ip_),SELF:=LEFT));

...
EXPORT B_Person_Ip_Address := MODULE
  ...
  SHARED __EE2629124 := __E_Person_Ip_Address;
  EXPORT __ST46109_Layout := RECORD
  ...
  END;
  SHARED __ST46109_Layout __ND2629158__Project(E_Person_Ip_Address.Layout __PP2629019) := TRANSFORM
    __EE2629057 := __PP2629019.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2629057,__T(__EE2629057).Event_Date_);
    __EE2629086 := __PP2629019.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2629086,__T(__EE2629086).Event_Date_);
    SELF := __PP2629019;
  END;
  EXPORT __ENH_Person_Ip_Address := PROJECT(__EE2629124,__ND2629158__Project(LEFT)) : PERSIST('~temp::KEL::KELOtto::Person_Ip_Address::Annotated',EXPIRE(7));
  SHARED __EE3311345 := __ENH_Person_Ip_Address;
  SHARED __IDX_Person_Ip_Address_Ip__Filtered := __EE3311345(__NN(__EE3311345.Ip_));
  ...
  EXPORT __ST3311347_Layout := RECORDOF(IDX_Person_Ip_Address_Ip_);
  EXPORT IDX_Person_Ip_Address_Ip__Wrapped := PROJECT(IDX_Person_Ip_Address_Ip_,TRANSFORM(__ST46109_Layout,SELF.Ip_ := __CN(LEFT.Ip_),SELF:=LEFT));

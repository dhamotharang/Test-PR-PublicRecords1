...
IMPORT KELOtto;
IMPORT E_Address,E_Customer,E_Email,E_Event,E_Internet_Protocol,E_Person,E_Phone,E_Social_Security_Number FROM KELOtto;
...
EXPORT E_Person_Event := MODULE
  ...
  SHARED __Mapping := 'associatedcustomerfileinfo(_r_Customer_:0),Subject_(Subject_:0),Social_(Social_:0),Phone_Number_(Phone_Number_:0),Emailof_(Emailof_:0),Location_(Location_:0),Ip_(Ip_:0),eventdate(Event_Date_:DATE),Transaction_(Transaction_:0),datefirstseen(Date_First_Seen_:EPOCH),datelastseen(Date_Last_Seen_:EPOCH)';
  SHARED __Mapping0 := 'associatedcustomerfileinfo(_r_Customer_:0),Subject_(Subject_:0),Social_(Social_:0),Phone_Number_(Phone_Number_:0),Emailof_(Emailof_:0),Location_(Location_:0),Ip_(Ip_:0),event_date(Event_Date_:DATE),Transaction_(Transaction_:0),datefirstseen(Date_First_Seen_:EPOCH),datelastseen(Date_Last_Seen_:EPOCH)';
  ...
  SHARED __Mapping1 := 'associatedcustomerfileinfo(_r_Customer_:0),Subject_(Subject_:0),Social_(Social_:0),Phone_Number_(Phone_Number_:0),Emailof_(Emailof_:0),Location_(Location_:0),Ip_(Ip_:0),event_date(Event_Date_:DATE),Transaction_(Transaction_:0),datefirstseen(Date_First_Seen_:EPOCH),datelastseen(Date_Last_Seen_:EPOCH)';
  ...
  SHARED __d1_Transaction__Layout := RECORD
    ...
    KEL.typ.uid Transaction_;
  END;
  SHARED __d1_Transaction__Mapped := JOIN(__d1_Ip__Mapped,E_Event.Lookup,TRIM((STRING)LEFT.AssociatedCustomerFileInfo) + '|' + TRIM((STRING)LEFT.record_id) = RIGHT.KeyVal,TRANSFORM(__d1_Transaction__Layout,SELF.Transaction_:=RIGHT.UID,SELF:=LEFT),LEFT OUTER,HASH);
  ...
  EXPORT SanityCheck := DATASET([{COUNT(_r_Customer__Orphan),COUNT(Subject__Orphan),COUNT(Social__Orphan),COUNT(Phone_Number__Orphan),COUNT(Emailof__Orphan),COUNT(Location__Orphan),COUNT(Ip__Orphan),COUNT(Transaction__Orphan)}],{KEL.typ.int _r_Customer__Orphan,KEL.typ.int Subject__Orphan,KEL.typ.int Social__Orphan,KEL.typ.int Phone_Number__Orphan,KEL.typ.int Emailof__Orphan,KEL.typ.int Location__Orphan,KEL.typ.int Ip__Orphan,KEL.typ.int Transaction__Orphan});

...
SelfEntities := PROJECT(DEDUP(SORT(FullTreePrep1 , source_customer_, tree_uid_, LOCAL), source_customer_, tree_uid_, LOCAL), TRANSFORM(RECORDOF(LEFT), SELF.entity_context_uid_ := LEFT.tree_uid_, SELF := LEFT));
  
TreeToEntity := DEDUP(SORT(DISTRIBUTE(FullTreePrep1+SelfEntities, HASH32(entity_context_uid_)), source_customer_, tree_uid_, LOCAL), source_customer_, tree_uid_, LOCAL);
EntityToTree := DISTRIBUTE(FullTreePrep1, HASH32(entity_context_uid_));  
...
FullPersonTreeEntities1 := JOIN(TreeToEntity, EntityToTree,
                            ...
                            TRANSFORM(
                              ...
                              SELF.entity_context_uid_ := RIGHT.tree_uid_,
                              SELF := LEFT), LOCAL);

//output(FullPersonTreeEntities(tree_uid_='_01166099968980'));
//output(FullPersonTreeEntities(tree_uid_='_092989276523'));
...
ConsolidatedPersonEntities := PROJECT(KELOtto.Q__show_Customer_Person_Entities.Res0, TRANSFORM({RECORDOF(LEFT) AND NOT [exp2_,__exp2__flags,exp3_,__exp3__flags,exp4_,__exp4__flags,exp5_,__exp5__flags]}, SELF.exp1_ := LEFT.exp1_ + LEFT.exp2_ + LEFT.exp3_ + LEFT.exp4_ + LEFT.exp5_, SELF := LEFT));
...
ElementCountAggregation := DISTRIBUTE(TABLE(FullGraphPrep5(entity_type_ != 1), {source_customer_, tree_uid_, UNSIGNED cl_element_count_ := COUNT(GROUP)}, source_customer_, tree_uid_, MERGE), HASH32(tree_uid_)); 
// Join the element count back to each entity within all the trees.
FullGraphElementCount := JOIN(FullGraphPrep5, ElementCountAggregation, LEFT.source_customer_ = RIGHT.source_customer_ AND LEFT.entity_context_uid_ = RIGHT.tree_uid_, TRANSFORM(RECORDOF(LEFT), SELF.cl_element_count_ := RIGHT.cl_element_count_, SELF := LEFT), LEFT OUTER, LOCAL);

FinalGraph := PROJECT(FullGraphElementCount, 
                    TRANSFORM(FinalRec, 
										           SELF.Flags := DATASET([

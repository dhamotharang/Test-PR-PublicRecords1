...
EXPORT B_Person_3 := MODULE
  ...
  SHARED __EE63850 := __ENH_Person_4;
  SHARED __EE69267 := __ENH_Event_4;
  SHARED __EE69242 := __E_Person_Event;
  SHARED __EE70372 := __EE69242;
  SHARED __EE70385 := __EE70372(__NN(__EE70372.Subject_) AND __NN(__EE70372.Transaction_));
  SHARED __ST66342_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  __JC70391(B_Event_4.__ST22007_Layout __EE69267, E_Person_Event.Layout __EE70385) := __EEQP(__EE70385.Transaction_,__EE69267.UID);
  __ST66342_Layout __JT70391(B_Event_4.__ST22007_Layout __l, E_Person_Event.Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE70392 := JOIN(__EE70385,__EE69267,__JC70391(RIGHT,LEFT),__JT70391(RIGHT,LEFT),INNER,HASH);
  SHARED __ST65832_Layout := RECORD
    ...
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __ST65832_Layout __ND70565__Project(__ST66342_Layout __PP70393) := TRANSFORM
    SELF.UID := __PP70393.Subject__1_;
    SELF._r_Customer_ := __PP70393._r_Customer__1_;
    SELF.Subject_ := __PP70393.Subject__1_;
    SELF.Event_Date_ := __PP70393.Event_Date__1_;
    SELF.U_I_D__1_ := __PP70393.UID;
    SELF := __PP70393;
  END;
  SHARED __EE70610 := PROJECT(__EE70392,__ND70565__Project(LEFT));
  SHARED __ST65878_Layout := RECORD
    KEL.typ.ntyp(E_Person.Typ) UID;
    ...
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __ST65878_Layout __ND70615__Project(__ST65832_Layout __PP70611) := TRANSFORM
    SELF.Exp1_ := __PP70611.Ip_Hosted_ = 1;
    SELF.Exp2_ := __PP70611.Ip_Not_Us_ = 1;
    SELF.Exp3_ := __PP70611.Ip_Tor_ = 1;
    SELF.Exp4_ := __PP70611.Ip_Vpn_ = 1;
    SELF.Exp5_ := __PP70611.Kr_High_Risk_Identity_Flag_ = 1;
    SELF := __PP70611;
  END;
  SHARED __EE70645 := PROJECT(__EE70610,__ND70615__Project(LEFT));
  SHARED __ST65913_Layout := RECORD
    ...
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __EE70681 := PROJECT(__CLEANANDDO(__EE70645,TABLE(__EE70645,{KEL.typ.int C_O_U_N_T___Exp1_ := COUNT(GROUP,__EE70645.Exp1_),KEL.typ.int C_O_U_N_T___Exp1__1_ := COUNT(GROUP,__EE70645.Exp2_),KEL.typ.int C_O_U_N_T___Exp1__2_ := COUNT(GROUP,__EE70645.Exp3_),KEL.typ.int C_O_U_N_T___Exp1__3_ := COUNT(GROUP,__EE70645.Exp4_),KEL.typ.int C_O_U_N_T___Exp1__4_ := COUNT(GROUP,__EE70645.Exp5_),UID},UID,MERGE)),__ST65913_Layout);
  SHARED __ST66576_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC70687(B_Person_4.__ST22236_Layout __EE63850, __ST65913_Layout __EE70681) := __EEQP(__EE63850.UID,__EE70681.UID);
  __ST66576_Layout __JT70687(B_Person_4.__ST22236_Layout __l, __ST65913_Layout __r) := TRANSFORM
    SELF.U_I_D__1_ := __r.UID;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE70688 := JOIN(__EE63850,__EE70681,__JC70687(LEFT,RIGHT),__JT70687(LEFT,RIGHT),LEFT OUTER,HASH);
  SHARED __EE69249 := __EE69242(__NN(__EE69242.Subject_));
  SHARED __ST65084_Layout := RECORD
    KEL.typ.ntyp(E_Person.Typ) UID;
    KEL.typ.ntyp(E_Event.Typ) Transaction_;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __EE69263 := DEDUP(PROJECT(__EE69249,TRANSFORM(__ST65084_Layout,SELF.UID := LEFT.Subject_,SELF := LEFT)),ALL);
  SHARED __ST65102_Layout := RECORD
    ...
    KEL.typ.epoch Date_Last_Seen_ := 0;
  END;
  SHARED __EE69240 := PROJECT(__CLEANANDDO(__EE69263,TABLE(__EE69263,{KEL.typ.int C_O_U_N_T___Person_Event_ := COUNT(GROUP),UID},UID,MERGE)),__ST65102_Layout);
  SHARED __ST67112_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.nint Lex_Id_;
    KEL.typ.ndataset(E_Person.Source_Customers_Layout) Source_Customers_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC70790(__ST66576_Layout __EE70688, __ST65102_Layout __EE69240) := __EEQP(__EE70688.UID,__EE69240.UID);
  __ST67112_Layout __JT70790(__ST66576_Layout __l, __ST65102_Layout __r) := TRANSFORM
    SELF.U_I_D__2_ := __r.UID;
    SELF := __l;
    SELF := __r;
  END;
  SHARED __EE70791 := JOIN(__EE70688,__EE69240,__JC70790(LEFT,RIGHT),__JT70790(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST21752_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.ntyp(E_Customer.Typ) _r_Customer_;
    KEL.typ.nint Lex_Id_;
    KEL.typ.ndataset(E_Person.Source_Customers_Layout) Source_Customers_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST21752_Layout __ND71287__Project(__ST67112_Layout __PP70792) := TRANSFORM
    SELF.Event_Count_ := __PP70792.C_O_U_N_T___Person_Event_;
    SELF.High_Risk_Death_Prior_To_All_Events_ := MAP(__T(__AND(__CN(__PP70792.Death_Prior_To_All_Events_ = 1),__OP2(__PP70792.Max_Deceased_To_Event_Diff_,<,__CN(-4))))=>1,0);
    SELF.Ip_Hosted_ := MAP(__PP70792.C_O_U_N_T___Exp1_ <> 0=>1,0);
    SELF.Ip_Not_Us_ := MAP(__PP70792.C_O_U_N_T___Exp1__1_ <> 0=>1,0);
    SELF.Ip_Tor_ := MAP(__PP70792.C_O_U_N_T___Exp1__2_ <> 0=>1,0);
    SELF.Ip_Vpn_ := MAP(__PP70792.C_O_U_N_T___Exp1__3_ <> 0=>1,0);
    SELF.Kr_High_Risk_Flag_ := MAP(__PP70792.C_O_U_N_T___Exp1__4_ <> 0=>1,0);
    SELF.Nas9_Flag_ := MAP(__T(__OP2(__CAST(KEL.typ.str,__PP70792._nas__summary_),=,__CN('9')))=>1,0);
    SELF.Vl_Event30_Active_Flag_ := MAP(__PP70792.Vl_Event30_Count_ > 0=>1,0);
    SELF.Vl_Event7_Active_Flag_ := MAP(__PP70792.Vl_Event7_Count_ > 0=>1,0);
    SELF := __PP70792;
  END;
  EXPORT __ENH_Person_3 := PROJECT(__EE70791,__ND71287__Project(LEFT));
END;

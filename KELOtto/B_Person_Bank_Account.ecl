...
IMPORT E_Address,E_Bank,E_Bank_Account,E_Customer,E_Person,E_Person_Bank_Account FROM KELOtto;
...
EXPORT B_Person_Bank_Account := MODULE
  ...
  SHARED __EE2534492 := __E_Person_Bank_Account;
  EXPORT __ST43113_Layout := RECORD
  ...
  END;
  SHARED __ST43113_Layout __ND2534526__Project(E_Person_Bank_Account.Layout __PP2534387) := TRANSFORM
    __EE2534425 := __PP2534387.Event_Dates_;
    SELF.Dt_First_Seen_ := KEL.Aggregates.MinNN(__EE2534425,__T(__EE2534425).Event_Date_);
    __EE2534454 := __PP2534387.Event_Dates_;
    SELF.Dt_Last_Seen_ := KEL.Aggregates.MaxNN(__EE2534454,__T(__EE2534454).Event_Date_);
    SELF := __PP2534387;
  END;
  EXPORT __ENH_Person_Bank_Account := PROJECT(__EE2534492,__ND2534526__Project(LEFT)) : PERSIST('~temp::KEL::KELOtto::Person_Bank_Account::Annotated',EXPIRE(7));
  SHARED __EE3176527 := __ENH_Person_Bank_Account;
  SHARED __IDX_Person_Bank_Account_Account__Filtered := __EE3176527(__NN(__EE3176527.Account_));
  ...
  EXPORT __ST3176529_Layout := RECORDOF(IDX_Person_Bank_Account_Account_);
  EXPORT IDX_Person_Bank_Account_Account__Wrapped := PROJECT(IDX_Person_Bank_Account_Account_,TRANSFORM(__ST43113_Layout,SELF.Account_ := __CN(LEFT.Account_),SELF:=LEFT));

...
#END
  ...
  // Create those fields with BestType: AnyFirstName
  // First step is to get all of the data slimmed and row-reduced
  EXPORT AnyFirstName_tab_ := DISTRIBUTE(TABLE(h_bm(did <> 0),{did,data_permits,fname,UNSIGNED Row_Cnt := COUNT(GROUP)},did,data_permits,fname,MERGE),HASH(did)); // Slim and reduce row-count

  Slim := TABLE(AnyFirstName_tab_((fname NOT IN SET(s.nulls_fname,fname) AND fname <> (TYPEOF(fname))'')),{did,data_permits,fname,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
  END;
  EXPORT AnyFirstName_tab_fname := ROLLUP( SORT(Slim,EXCEPT Row_Cnt,data_permits,LOCAL),TR(LEFT,RIGHT), EXCEPT Row_Cnt,data_permits,LOCAL);

  // Now actually find the best value
  grp := GROUP( AnyFirstName_tab_fname,did,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
  EXPORT AnyFirstName_method_fname := cmn;
  ...
  // Create those fields with BestType: AnyMiddleName
  // First step is to get all of the data slimmed and row-reduced
  EXPORT AnyMiddleName_tab_ := DISTRIBUTE(TABLE(h_bm(did <> 0),{did,data_permits,mname,UNSIGNED Row_Cnt := COUNT(GROUP)},did,data_permits,mname,MERGE),HASH(did)); // Slim and reduce row-count

  Slim := TABLE(AnyMiddleName_tab_((mname NOT IN SET(s.nulls_mname,mname) AND mname <> (TYPEOF(mname))'')),{did,data_permits,mname,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
  END;
  EXPORT AnyMiddleName_tab_mname := ROLLUP( SORT(Slim,EXCEPT Row_Cnt,data_permits,LOCAL),TR(LEFT,RIGHT), EXCEPT Row_Cnt,data_permits,LOCAL);

  // Now actually find the best value
  grp := GROUP( AnyMiddleName_tab_mname,did,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
  EXPORT AnyMiddleName_method_mname := cmn;



  // Create those fields with BestType: BestLastName
  // First step is to get all of the data slimmed and row-reduced
  EXPORT BestLastName_tab_ := DISTRIBUTE(TABLE(h_bm(did <> 0),{did,src,data_permits,lname,name_ind,UNSIGNED Row_Cnt := COUNT(GROUP)},did,src,data_permits,lname,name_ind,MERGE),HASH(did)); // Slim and reduce row-count

  Slim := TABLE(BestLastName_tab_(~((lname  IN SET(s.nulls_lname,lname) OR lname = (TYPEOF(lname))'') AND (name_ind  IN SET(s.nulls_name_ind,name_ind) OR name_ind = (TYPEOF(name_ind))'')),Watchdog_Best.fn_best_lname(TRIM((SALT311.StrType)lname) + '|' + TRIM((SALT311.StrType)name_ind),src)),{did,src,data_permits,lname,name_ind,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
  END;
  EXPORT BestLastName_tab_lastname := ROLLUP( SORT(Slim,EXCEPT Row_Cnt,src,data_permits,LOCAL),TR(LEFT,RIGHT), EXCEPT Row_Cnt,src,data_permits,LOCAL);

  // Now actually find the best value
  grp := GROUP( BestLastName_tab_lastname,did,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
  EXPORT BestLastName_method_lastname := cmn;



  // Create those fields with BestType: CommonLastName
  // First step is to get all of the data slimmed and row-reduced
  EXPORT CommonLastName_tab_ := DISTRIBUTE(TABLE(h_bm(did <> 0),{did,src,data_permits,lname,name_ind,UNSIGNED Row_Cnt := COUNT(GROUP)},did,src,data_permits,lname,name_ind,MERGE),HASH(did)); // Slim and reduce row-count

  Slim := TABLE(CommonLastName_tab_(~((lname  IN SET(s.nulls_lname,lname) OR lname = (TYPEOF(lname))'') AND (name_ind  IN SET(s.nulls_name_ind,name_ind) OR name_ind = (TYPEOF(name_ind))'')),Watchdog_Best.fn_valid_lname(TRIM((SALT311.StrType)lname) + '|' + TRIM((SALT311.StrType)name_ind),src)),{did,src,data_permits,lname,name_ind,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
  END;
  EXPORT CommonLastName_tab_lastname := ROLLUP( SORT(Slim,EXCEPT Row_Cnt,src,data_permits,LOCAL),TR(LEFT,RIGHT), EXCEPT Row_Cnt,src,data_permits,LOCAL);

  // Now actually find the best value
  grp := GROUP( CommonLastName_tab_lastname,did,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
  EXPORT CommonLastName_method_lastname := cmn;



  // Create those fields with BestType: AnyLastName
  // First step is to get all of the data slimmed and row-reduced
  EXPORT AnyLastName_tab_ := DISTRIBUTE(TABLE(h_bm(did <> 0),{did,data_permits,lname,name_ind,UNSIGNED Row_Cnt := COUNT(GROUP)},did,data_permits,lname,name_ind,MERGE),HASH(did)); // Slim and reduce row-count

  Slim := TABLE(AnyLastName_tab_(~((lname  IN SET(s.nulls_lname,lname) OR lname = (TYPEOF(lname))'') AND (name_ind  IN SET(s.nulls_name_ind,name_ind) OR name_ind = (TYPEOF(name_ind))''))),{did,data_permits,lname,name_ind,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
  END;
  EXPORT AnyLastName_tab_lastname := ROLLUP( SORT(Slim,EXCEPT Row_Cnt,data_permits,LOCAL),TR(LEFT,RIGHT), EXCEPT Row_Cnt,data_permits,LOCAL);

  // Now actually find the best value
  grp := GROUP( AnyLastName_tab_lastname,did,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
  EXPORT AnyLastName_method_lastname := cmn;
  ...
  // Now actually find the best value
  grp := GROUP( BestSSN_tab_ssnum,did,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
  ...
  // Now actually find the best value
  grp := GROUP( SecondBestSSN_tab_ssnum,did,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
  ...
  // Create those fields with BestType: PrefferedDOB
  // First step is to get all of the data slimmed and row-reduced
  EXPORT PrefferedDOB_tab_ := DISTRIBUTE(TABLE(h_bm(did <> 0),{did,src,data_permits,dob,UNSIGNED Row_Cnt := COUNT(GROUP)},did,src,data_permits,dob,MERGE),HASH(did)); // Slim and reduce row-count

  Slim := TABLE(PrefferedDOB_tab_((dob NOT IN SET(s.nulls_dob,dob) AND dob <> (TYPEOF(dob))''),Watchdog_Best.fn_preffered_dob(TRIM((SALT311.StrType)dob),src)),{did,src,data_permits,dob,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
  END;
  EXPORT PrefferedDOB_tab_dob := ROLLUP( SORT(Slim,EXCEPT Row_Cnt,src,data_permits,LOCAL),TR(LEFT,RIGHT), EXCEPT Row_Cnt,src,data_permits,LOCAL);

  // Now actually find the best value
  grp := GROUP( PrefferedDOB_tab_dob,did,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
  EXPORT PrefferedDOB_method_dob := cmn;
  ...
  // 1 - Gathering type:BestFirstName Entries:1
  R1 := RECORD
    J0; // The data so far
  ...
  END;
  R1 T1(J0 le,BestFirstName_method_fname ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J1 := JOIN(J0,BestFirstName_method_fname,LEFT.did = RIGHT.did,T1(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 2 - Gathering type:CommonFirstName Entries:1
  R2 := RECORD
    J1; // The data so far
  ...
  END;
  R2 T2(J1 le,CommonFirstName_method_fname ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J2 := JOIN(J1,CommonFirstName_method_fname,LEFT.did = RIGHT.did,T2(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 3 - Gathering type:AnyFirstName Entries:1
  R3 := RECORD
    J2; // The data so far
    TYPEOF(AnyFirstName_method_fname.fname) AnyFirstName_fname;
    UNSIGNED fname_AnyFirstName_Row_Cnt;
    UNSIGNED4 fname_AnyFirstName_data_permits;
  END;
  R3 T3(J2 le,AnyFirstName_method_fname ri) := TRANSFORM
    SELF.AnyFirstName_fname := ri.fname;
    SELF.fname_AnyFirstName_Row_Cnt := ri.Row_Cnt;
    SELF.fname_AnyFirstName_data_permits := ri.data_permits;
    BOOLEAN has_left := le.did <> (TYPEOF(le.did))''; // See if LHS is null
    SELF.did := IF( has_left, le.did, ri.did );
    SELF := le;
  END;
  J3 := JOIN(J2,AnyFirstName_method_fname,LEFT.did = RIGHT.did,T3(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 4 - Gathering type:BestMiddleName Entries:1
  R4 := RECORD
    J3; // The data so far
  ...
  END;
  R4 T4(J3 le,BestMiddleName_method_mname ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J4 := JOIN(J3,BestMiddleName_method_mname,LEFT.did = RIGHT.did,T4(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 5 - Gathering type:CommonMiddleName Entries:1
  R5 := RECORD
    J4; // The data so far
  ...
  END;
  R5 T5(J4 le,CommonMiddleName_method_mname ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J5 := JOIN(J4,CommonMiddleName_method_mname,LEFT.did = RIGHT.did,T5(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 6 - Gathering type:AnyMiddleName Entries:1
  R6 := RECORD
    J5; // The data so far
    TYPEOF(AnyMiddleName_method_mname.mname) AnyMiddleName_mname;
    UNSIGNED mname_AnyMiddleName_Row_Cnt;
    UNSIGNED4 mname_AnyMiddleName_data_permits;
  END;
  R6 T6(J5 le,AnyMiddleName_method_mname ri) := TRANSFORM
    SELF.AnyMiddleName_mname := ri.mname;
    SELF.mname_AnyMiddleName_Row_Cnt := ri.Row_Cnt;
    SELF.mname_AnyMiddleName_data_permits := ri.data_permits;
    BOOLEAN has_left := le.did <> (TYPEOF(le.did))''; // See if LHS is null
    SELF.did := IF( has_left, le.did, ri.did );
    SELF := le;
  END;
  J6 := JOIN(J5,AnyMiddleName_method_mname,LEFT.did = RIGHT.did,T6(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 7 - Gathering type:BestLastName Entries:1
  R7 := RECORD
    J6; // The data so far
    TYPEOF(BestLastName_method_lastname.lname) BestLastName_lastname_lname;
    TYPEOF(BestLastName_method_lastname.name_ind) BestLastName_lastname_name_ind;
    UNSIGNED lastname_BestLastName_Row_Cnt;
    UNSIGNED4 lastname_BestLastName_data_permits;
  END;
  R7 T7(J6 le,BestLastName_method_lastname ri) := TRANSFORM
    SELF.BestLastName_lastname_lname := ri.lname;
    SELF.BestLastName_lastname_name_ind := ri.name_ind;
    SELF.lastname_BestLastName_Row_Cnt := ri.Row_Cnt;
    SELF.lastname_BestLastName_data_permits := ri.data_permits;
    BOOLEAN has_left := le.did <> (TYPEOF(le.did))''; // See if LHS is null
    SELF.did := IF( has_left, le.did, ri.did );
    SELF := le;
  END;
  J7 := JOIN(J6,BestLastName_method_lastname,LEFT.did = RIGHT.did,T7(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 8 - Gathering type:CommonLastName Entries:1
  R8 := RECORD
    J7; // The data so far
    TYPEOF(CommonLastName_method_lastname.lname) CommonLastName_lastname_lname;
    TYPEOF(CommonLastName_method_lastname.name_ind) CommonLastName_lastname_name_ind;
    UNSIGNED lastname_CommonLastName_Row_Cnt;
    UNSIGNED4 lastname_CommonLastName_data_permits;
  END;
  R8 T8(J7 le,CommonLastName_method_lastname ri) := TRANSFORM
    SELF.CommonLastName_lastname_lname := ri.lname;
    SELF.CommonLastName_lastname_name_ind := ri.name_ind;
    SELF.lastname_CommonLastName_Row_Cnt := ri.Row_Cnt;
    SELF.lastname_CommonLastName_data_permits := ri.data_permits;
    BOOLEAN has_left := le.did <> (TYPEOF(le.did))''; // See if LHS is null
    SELF.did := IF( has_left, le.did, ri.did );
    SELF := le;
  END;
  J8 := JOIN(J7,CommonLastName_method_lastname,LEFT.did = RIGHT.did,T8(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 9 - Gathering type:AnyLastName Entries:1
  R9 := RECORD
    J8; // The data so far
    TYPEOF(AnyLastName_method_lastname.lname) AnyLastName_lastname_lname;
    TYPEOF(AnyLastName_method_lastname.name_ind) AnyLastName_lastname_name_ind;
    UNSIGNED lastname_AnyLastName_Row_Cnt;
    UNSIGNED4 lastname_AnyLastName_data_permits;
  END;
  R9 T9(J8 le,AnyLastName_method_lastname ri) := TRANSFORM
    SELF.AnyLastName_lastname_lname := ri.lname;
    SELF.AnyLastName_lastname_name_ind := ri.name_ind;
    SELF.lastname_AnyLastName_Row_Cnt := ri.Row_Cnt;
    SELF.lastname_AnyLastName_data_permits := ri.data_permits;
    BOOLEAN has_left := le.did <> (TYPEOF(le.did))''; // See if LHS is null
    SELF.did := IF( has_left, le.did, ri.did );
    SELF := le;
  END;
  J9 := JOIN(J8,AnyLastName_method_lastname,LEFT.did = RIGHT.did,T9(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 10 - Gathering type:BestSuffix Entries:1
  R10 := RECORD
    J9; // The data so far
  ...
  END;
  R10 T10(J9 le,BestSuffix_method_name_suffix ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J10 := JOIN(J9,BestSuffix_method_name_suffix,LEFT.did = RIGHT.did,T10(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 11 - Gathering type:BestSSN Entries:1
  R11 := RECORD
    J10; // The data so far
  ...
  END;
  R11 T11(J10 le,BestSSN_method_ssnum ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J11 := JOIN(J10,BestSSN_method_ssnum,LEFT.did = RIGHT.did,T11(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 12 - Gathering type:SecondBestSSN Entries:1
  R12 := RECORD
    J11; // The data so far
  ...
  END;
  R12 T12(J11 le,SecondBestSSN_method_ssnum ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J12 := JOIN(J11,SecondBestSSN_method_ssnum,LEFT.did = RIGHT.did,T12(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 13 - Gathering type:BestPhoneCurrentWithNpa Entries:1
  R13 := RECORD
    J12; // The data so far
  ...
  END;
  R13 T13(J12 le,BestPhoneCurrentWithNpa_method_phone ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J13 := JOIN(J12,BestPhoneCurrentWithNpa_method_phone,LEFT.did = RIGHT.did,T13(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 14 - Gathering type:PrefferedDOB Entries:1
  R14 := RECORD
    J13; // The data so far
    TYPEOF(PrefferedDOB_method_dob.dob) PrefferedDOB_dob;
    UNSIGNED dob_PrefferedDOB_Row_Cnt;
    UNSIGNED4 dob_PrefferedDOB_data_permits;
  END;
  R14 T14(J13 le,PrefferedDOB_method_dob ri) := TRANSFORM
    SELF.PrefferedDOB_dob := ri.dob;
    SELF.dob_PrefferedDOB_Row_Cnt := ri.Row_Cnt;
    SELF.dob_PrefferedDOB_data_permits := ri.data_permits;
    BOOLEAN has_left := le.did <> (TYPEOF(le.did))''; // See if LHS is null
    SELF.did := IF( has_left, le.did, ri.did );
    SELF := le;
  END;
  J14 := JOIN(J13,PrefferedDOB_method_dob,LEFT.did = RIGHT.did,T14(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 15 - Gathering type:BestDOB Entries:1
  R15 := RECORD
    J14; // The data so far
  ...
  END;
  R15 T15(J14 le,BestDOB_method_dob ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J15 := JOIN(J14,BestDOB_method_dob,LEFT.did = RIGHT.did,T15(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 16 - Gathering type:NoDayDOB Entries:1
  R16 := RECORD
    J15; // The data so far
  ...
  END;
  R16 T16(J15 le,NoDayDOB_method_dob ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J16 := JOIN(J15,NoDayDOB_method_dob,LEFT.did = RIGHT.did,T16(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 17 - Gathering type:NoMonthDOB Entries:1
  R17 := RECORD
    J16; // The data so far
  ...
  END;
  R17 T17(J16 le,NoMonthDOB_method_dob ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J17 := JOIN(J16,NoMonthDOB_method_dob,LEFT.did = RIGHT.did,T17(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 18 - Gathering type:BestAddress Entries:1
  R18 := RECORD
    J17; // The data so far
  ...
  END;
  R18 T18(J17 le,BestAddress_method_address ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J18 := JOIN(J17,BestAddress_method_address,LEFT.did = RIGHT.did,T18(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 19 - Gathering type:GongAddressBySeen Entries:1
  R19 := RECORD
    J18; // The data so far
  ...
  END;
  R19 T19(J18 le,GongAddressBySeen_method_address ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J19 := JOIN(J18,GongAddressBySeen_method_address,LEFT.did = RIGHT.did,T19(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 20 - Gathering type:BetterAddressBySeen Entries:1
  R20 := RECORD
    J19; // The data so far
  ...
  END;
  R20 T20(J19 le,BetterAddressBySeen_method_address ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J20 := JOIN(J19,BetterAddressBySeen_method_address,LEFT.did = RIGHT.did,T20(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 21 - Gathering type:RecentAddressBySeen Entries:1
  R21 := RECORD
    J20; // The data so far
  ...
  END;
  R21 T21(J20 le,RecentAddressBySeen_method_address ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J21 := JOIN(J20,RecentAddressBySeen_method_address,LEFT.did = RIGHT.did,T21(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 22 - Gathering type:BetterAddressByReported Entries:1
  R22 := RECORD
    J21; // The data so far
  ...
  END;
  R22 T22(J21 le,BetterAddressByReported_method_address ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J22 := JOIN(J21,BetterAddressByReported_method_address,LEFT.did = RIGHT.did,T22(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 23 - Gathering type:RecentAddressByReported Entries:1
  R23 := RECORD
    J22; // The data so far
  ...
  END;
  R23 T23(J22 le,RecentAddressByReported_method_address ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J23 := JOIN(J22,RecentAddressByReported_method_address,LEFT.did = RIGHT.did,T23(LEFT,RIGHT),FULL OUTER,LOCAL);
  // 24 - Gathering type:CommonestAddress Entries:1
  R24 := RECORD
    J23; // The data so far
  ...
  END;
  R24 T24(J23 le,CommonestAddress_method_address ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J24 := JOIN(J23,CommonestAddress_method_address,LEFT.did = RIGHT.did,T24(LEFT,RIGHT),FULL OUTER,LOCAL);
  EXPORT BestBy_did_np := J24;
  ...
  R := RECORD
    ...
    REAL8 AnyFirstName_fname_pcnt := AVE(GROUP,IF(BestBy_did.AnyFirstName_fname=(typeof(BestBy_did.AnyFirstName_fname))'',0,100));
    UNSIGNED AnyFirstName_fname_permit1_cnt := COUNT(GROUP,BestBy_did.fname_AnyFirstName_data_permits&1<>0);
    ...
    REAL8 AnyMiddleName_mname_pcnt := AVE(GROUP,IF(BestBy_did.AnyMiddleName_mname=(typeof(BestBy_did.AnyMiddleName_mname))'',0,100));
    UNSIGNED AnyMiddleName_mname_permit1_cnt := COUNT(GROUP,BestBy_did.mname_AnyMiddleName_data_permits&1<>0);
    REAL8 BestLastName_lastname_lname_pcnt := AVE(GROUP,IF(BestBy_did.BestLastName_lastname_lname=(typeof(BestBy_did.BestLastName_lastname_lname))'',0,100));
    REAL8 BestLastName_lastname_name_ind_pcnt := AVE(GROUP,IF(BestBy_did.BestLastName_lastname_name_ind=(typeof(BestBy_did.BestLastName_lastname_name_ind))'',0,100));
    UNSIGNED BestLastName_lastname_permit1_cnt := COUNT(GROUP,BestBy_did.lastname_BestLastName_data_permits&1<>0);
    REAL8 CommonLastName_lastname_lname_pcnt := AVE(GROUP,IF(BestBy_did.CommonLastName_lastname_lname=(typeof(BestBy_did.CommonLastName_lastname_lname))'',0,100));
    REAL8 CommonLastName_lastname_name_ind_pcnt := AVE(GROUP,IF(BestBy_did.CommonLastName_lastname_name_ind=(typeof(BestBy_did.CommonLastName_lastname_name_ind))'',0,100));
    UNSIGNED CommonLastName_lastname_permit1_cnt := COUNT(GROUP,BestBy_did.lastname_CommonLastName_data_permits&1<>0);
    REAL8 AnyLastName_lastname_lname_pcnt := AVE(GROUP,IF(BestBy_did.AnyLastName_lastname_lname=(typeof(BestBy_did.AnyLastName_lastname_lname))'',0,100));
    REAL8 AnyLastName_lastname_name_ind_pcnt := AVE(GROUP,IF(BestBy_did.AnyLastName_lastname_name_ind=(typeof(BestBy_did.AnyLastName_lastname_name_ind))'',0,100));
    UNSIGNED AnyLastName_lastname_permit1_cnt := COUNT(GROUP,BestBy_did.lastname_AnyLastName_data_permits&1<>0);
    ...
    REAL8 PrefferedDOB_dob_pcnt := AVE(GROUP,IF(BestBy_did.PrefferedDOB_dob=(typeof(BestBy_did.PrefferedDOB_dob))'',0,100));
    UNSIGNED PrefferedDOB_dob_permit1_cnt := COUNT(GROUP,BestBy_did.dob_PrefferedDOB_data_permits&1<>0);
  ...
  lastname_case_layout := RECORD
      TYPEOF(h_bm.lname) lastname_lname;
      TYPEOF(h_bm.name_ind) lastname_name_ind;
    UNSIGNED4 lastname_data_permits;
    UNSIGNED1 lastname_method; // This value could come from multiple BESTTYPE; track which one
  END;
  ...
  END;
    ...
    R T(BestBy_did le) := TRANSFORM
      ...
      SELF.fname_cases := DATASET([
        ...
        {le.AnyFirstName_fname,le.fname_AnyFirstName_data_permits,3}
      ...
      SELF.mname_cases := DATASET([
        ...
        {le.AnyMiddleName_mname,le.mname_AnyMiddleName_data_permits,3}
          ],mname_case_layout)((mname NOT IN SET(s.nulls_mname,mname) AND mname <> (TYPEOF(mname))''));
      SELF.lastname_cases := DATASET([
        {le.BestLastName_lastname_lname,le.BestLastName_lastname_name_ind,le.lastname_BestLastName_data_permits,1},
        {le.CommonLastName_lastname_lname,le.CommonLastName_lastname_name_ind,le.lastname_CommonLastName_data_permits,2},
        {le.AnyLastName_lastname_lname,le.AnyLastName_lastname_name_ind,le.lastname_AnyLastName_data_permits,3}
          ],lastname_case_layout)((lastname_lname NOT IN SET(s.nulls_lname,lname) AND lastname_lname <> (TYPEOF(lastname_lname))'') OR (lastname_name_ind NOT IN SET(s.nulls_name_ind,name_ind) AND lastname_name_ind <> (TYPEOF(lastname_name_ind))''));
      ...
      SELF.dob_cases := DATASET([
        {le.PrefferedDOB_dob,le.dob_PrefferedDOB_data_permits,1},
        {le.BestDOB_dob,le.dob_BestDOB_data_permits,2},
        {le.NoDayDOB_dob,le.dob_NoDayDOB_data_permits,3},
        {le.NoMonthDOB_dob,le.dob_NoMonthDOB_data_permits,4}

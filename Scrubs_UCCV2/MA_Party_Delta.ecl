IMPORT SALT39,STD;
EXPORT MA_Party_Delta(DATASET(MA_Party_Layout_UCCV2)old_s, DATASET(MA_Party_Layout_UCCV2) new_s) := MODULE//Routines to compute the differences between two instances of a file
  ...
  EXPORT DifferenceSummary := MA_Party_hygiene(old_s).Summary('Old') + MA_Party_hygiene(new_s).Summary('New') + MA_Party_hygiene(PROJECT(Differences(deleted), TRANSFORM(MA_Party_Layout_UCCV2, SELF := LEFT.old_rec))).Summary('Deletions') + MA_Party_hygiene(PROJECT(Differences(added), TRANSFORM(MA_Party_Layout_UCCV2, SELF := LEFT.new_rec))).Summary('Additions');
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE) := FUNCTION
    myTimeStamp := (UNSIGNED6)SALT39.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
    hygieneDiffOverall := DifferenceSummary;
    SALT39.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_UCCV2, MA_Party_Fields, 'RECORDOF(hygieneDiffOverall)', FALSE);

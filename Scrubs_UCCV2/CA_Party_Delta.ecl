IMPORT SALT39,STD;
EXPORT CA_Party_Delta(DATASET(CA_Party_Layout_UCCV2)old_s, DATASET(CA_Party_Layout_UCCV2) new_s) := MODULE//Routines to compute the differences between two instances of a file
  ...
  EXPORT DifferenceSummary := CA_Party_hygiene(old_s).Summary('Old') + CA_Party_hygiene(new_s).Summary('New') + CA_Party_hygiene(PROJECT(Differences(deleted), TRANSFORM(CA_Party_Layout_UCCV2, SELF := LEFT.old_rec))).Summary('Deletions') + CA_Party_hygiene(PROJECT(Differences(added), TRANSFORM(CA_Party_Layout_UCCV2, SELF := LEFT.new_rec))).Summary('Additions');
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE) := FUNCTION
    myTimeStamp := (UNSIGNED6)SALT39.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
    hygieneDiffOverall := DifferenceSummary;
    SALT39.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_UCCV2, CA_Party_Fields, 'RECORDOF(hygieneDiffOverall)', FALSE);

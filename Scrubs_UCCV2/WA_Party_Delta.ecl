IMPORT SALT39,STD;
EXPORT WA_Party_Delta(DATASET(WA_Party_Layout_UCCV2)old_s, DATASET(WA_Party_Layout_UCCV2) new_s) := MODULE//Routines to compute the differences between two instances of a file
  ...
  EXPORT DifferenceSummary := WA_Party_hygiene(old_s).Summary('Old') + WA_Party_hygiene(new_s).Summary('New') + WA_Party_hygiene(PROJECT(Differences(deleted), TRANSFORM(WA_Party_Layout_UCCV2, SELF := LEFT.old_rec))).Summary('Deletions') + WA_Party_hygiene(PROJECT(Differences(added), TRANSFORM(WA_Party_Layout_UCCV2, SELF := LEFT.new_rec))).Summary('Additions');
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE) := FUNCTION
    myTimeStamp := (UNSIGNED6)SALT39.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
    hygieneDiffOverall := DifferenceSummary;
    SALT39.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_UCCV2, WA_Party_Fields, 'RECORDOF(hygieneDiffOverall)', FALSE);

IMPORT SALT39,STD;
EXPORT IL_Party_Delta(DATASET(IL_Party_Layout_UCCV2)old_s, DATASET(IL_Party_Layout_UCCV2) new_s) := MODULE//Routines to compute the differences between two instances of a file
  ...
  EXPORT DifferenceSummary := IL_Party_hygiene(old_s).Summary('Old') + IL_Party_hygiene(new_s).Summary('New') + IL_Party_hygiene(PROJECT(Differences(deleted), TRANSFORM(IL_Party_Layout_UCCV2, SELF := LEFT.old_rec))).Summary('Deletions') + IL_Party_hygiene(PROJECT(Differences(added), TRANSFORM(IL_Party_Layout_UCCV2, SELF := LEFT.new_rec))).Summary('Additions');
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE) := FUNCTION
    myTimeStamp := (UNSIGNED6)SALT39.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
    hygieneDiffOverall := DifferenceSummary;
    SALT39.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_UCCV2, IL_Party_Fields, 'RECORDOF(hygieneDiffOverall)', FALSE);

...
EXPORT MakeFT_invalid_tmsid(SALT39.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_tmsid(SALT39.StrType s) := WHICH(~Scrubs_UCCV2.Functions.fn_state_tmsid_rmsid(s,'IL')>0);
...
EXPORT MakeFT_invalid_rmsid(SALT39.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_rmsid(SALT39.StrType s) := WHICH(~Scrubs_UCCV2.Functions.fn_state_tmsid_rmsid(s,'IL')>0);
...
EXPORT Make_orig_country(SALT39.StrType s0) := s0;
EXPORT InValid_orig_country(SALT39.StrType s) := 0;

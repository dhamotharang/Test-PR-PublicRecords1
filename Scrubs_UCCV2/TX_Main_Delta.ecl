IMPORT SALT38,STD;
EXPORT TX_Main_Delta(DATASET(TX_Main_Layout_UCCV2)old_s, DATASET(TX_Main_Layout_UCCV2) new_s) := MODULE//Routines to compute the differences between two instances of a file
  ...
  EXPORT DifferenceSummary := TX_Main_hygiene(old_s).Summary('Old') + TX_Main_hygiene(new_s).Summary('New') + TX_Main_hygiene(PROJECT(Differences(deleted), TRANSFORM(TX_Main_Layout_UCCV2, SELF := LEFT.old_rec))).Summary('Deletions') + TX_Main_hygiene(PROJECT(Differences(added), TRANSFORM(TX_Main_Layout_UCCV2, SELF := LEFT.new_rec))).Summary('Additions');
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE) := FUNCTION
    myTimeStamp := (UNSIGNED6)SALT38.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
    hygieneDiffOverall := DifferenceSummary;
    SALT38.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_UCCV2, TX_Main_Fields, 'RECORDOF(hygieneDiffOverall)', FALSE);

...
EXPORT MakeFT_invalid_tmsid(SALT39.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_tmsid(SALT39.StrType s) := WHICH(~Scrubs_UCCV2.Functions.fn_state_tmsid_rmsid(s,'MA')>0);
...
EXPORT MakeFT_invalid_rmsid(SALT39.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_rmsid(SALT39.StrType s) := WHICH(~Scrubs_UCCV2.Functions.fn_state_tmsid_rmsid(s,'MA')>0);
...
EXPORT MakeFT_invalid_orig_country(SALT39.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_orig_country(SALT39.StrType s) := WHICH(~Scrubs_UCCV2.Functions.fn_country(s)>0);
EXPORT InValidMessageFT_invalid_orig_country(UNSIGNED1 wh) := CHOOSE(wh,SALT39.HygieneErrors.CustomFail('Scrubs_UCCV2.Functions.fn_country'),SALT39.HygieneErrors.Good);
...
EXPORT Make_incorp_state(SALT39.StrType s0) := MakeFT_invalid_state(s0);
EXPORT InValid_incorp_state(SALT39.StrType s) := InValidFT_invalid_state(s);

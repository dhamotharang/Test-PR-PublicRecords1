IMPORT SALT39,STD;
EXPORT NYC_Party_Delta(DATASET(NYC_Party_Layout_UCCV2)old_s, DATASET(NYC_Party_Layout_UCCV2) new_s) := MODULE//Routines to compute the differences between two instances of a file
  ...
  EXPORT DifferenceSummary := NYC_Party_hygiene(old_s).Summary('Old') + NYC_Party_hygiene(new_s).Summary('New') + NYC_Party_hygiene(PROJECT(Differences(deleted), TRANSFORM(NYC_Party_Layout_UCCV2, SELF := LEFT.old_rec))).Summary('Deletions') + NYC_Party_hygiene(PROJECT(Differences(added), TRANSFORM(NYC_Party_Layout_UCCV2, SELF := LEFT.new_rec))).Summary('Additions');
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE) := FUNCTION
    myTimeStamp := (UNSIGNED6)SALT39.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
    hygieneDiffOverall := DifferenceSummary;
    SALT39.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_UCCV2, NYC_Party_Fields, 'RECORDOF(hygieneDiffOverall)', FALSE);

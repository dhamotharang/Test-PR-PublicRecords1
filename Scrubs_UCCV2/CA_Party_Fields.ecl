...
EXPORT MakeFT_invalid_orig_zip4(SALT37.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_orig_zip4(SALT37.StrType s) := WHICH(~Scrubs_UCCV2.Functions.fn_verify_zip4(s)>0);
EXPORT InValidMessageFT_invalid_orig_zip4(UNSIGNED1 wh) := CHOOSE(wh,SALT37.HygieneErrors.CustomFail('Scrubs_UCCV2.Functions.fn_verify_zip4'),SALT37.HygieneErrors.Good);
...
EXPORT Make_orig_zip4(SALT37.StrType s0) := MakeFT_invalid_orig_zip4(s0);
EXPORT InValid_orig_zip4(SALT37.StrType s) := InValidFT_invalid_orig_zip4(s);
EXPORT InValidMessage_orig_zip4(UNSIGNED1 wh) := InValidMessageFT_invalid_orig_zip4(wh);
 
EXPORT Make_orig_country(SALT37.StrType s0) := MakeFT_invalid_mandatory(s0);
EXPORT InValid_orig_country(SALT37.StrType s) := InValidFT_invalid_mandatory(s);

IMPORT SALT38,STD;
EXPORT CA_Main_Delta(DATASET(CA_Main_Layout_UCCV2)old_s, DATASET(CA_Main_Layout_UCCV2) new_s) := MODULE//Routines to compute the differences between two instances of a file
  ...
  EXPORT DifferenceSummary := CA_Main_hygiene(old_s).Summary('Old') + CA_Main_hygiene(new_s).Summary('New') + CA_Main_hygiene(PROJECT(Differences(deleted), TRANSFORM(CA_Main_Layout_UCCV2, SELF := LEFT.old_rec))).Summary('Deletions') + CA_Main_hygiene(PROJECT(Differences(added), TRANSFORM(CA_Main_Layout_UCCV2, SELF := LEFT.new_rec))).Summary('Additions');
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE) := FUNCTION
    myTimeStamp := (UNSIGNED6)SALT38.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
    hygieneDiffOverall := DifferenceSummary;
    SALT38.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_UCCV2, CA_Main_Fields, 'RECORDOF(hygieneDiffOverall)', FALSE);

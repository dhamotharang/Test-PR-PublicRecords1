IMPORT SALT39,STD;
EXPORT TX_Party_Delta(DATASET(TX_Party_Layout_UCCV2)old_s, DATASET(TX_Party_Layout_UCCV2) new_s) := MODULE//Routines to compute the differences between two instances of a file
  ...
  EXPORT DifferenceSummary := TX_Party_hygiene(old_s).Summary('Old') + TX_Party_hygiene(new_s).Summary('New') + TX_Party_hygiene(PROJECT(Differences(deleted), TRANSFORM(TX_Party_Layout_UCCV2, SELF := LEFT.old_rec))).Summary('Deletions') + TX_Party_hygiene(PROJECT(Differences(added), TRANSFORM(TX_Party_Layout_UCCV2, SELF := LEFT.new_rec))).Summary('Additions');
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE) := FUNCTION
    myTimeStamp := (UNSIGNED6)SALT39.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
    hygieneDiffOverall := DifferenceSummary;
    SALT39.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_UCCV2, TX_Party_Fields, 'RECORDOF(hygieneDiffOverall)', FALSE);

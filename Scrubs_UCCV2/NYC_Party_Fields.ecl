...
EXPORT MakeFT_invalid_alphanum_specials(SALT39.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_invalid_alphanum_specials(SALT39.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT39.StringFilter(s,'0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ @-./,&\'#'))));
EXPORT InValidMessageFT_invalid_alphanum_specials(UNSIGNED1 wh) := CHOOSE(wh,SALT39.HygieneErrors.NotInChars('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ @-./,&\'#'),SALT39.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_tmsid(SALT39.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_tmsid(SALT39.StrType s) := WHICH(~Scrubs_UCCV2.Functions.fn_state_tmsid_rmsid(s,'NYC')>0);
...
EXPORT MakeFT_invalid_orig_country(SALT39.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_orig_country(SALT39.StrType s) := WHICH(~Scrubs_UCCV2.Functions.fn_country2(s)>0);
EXPORT InValidMessageFT_invalid_orig_country(UNSIGNED1 wh) := CHOOSE(wh,SALT39.HygieneErrors.CustomFail('Scrubs_UCCV2.Functions.fn_country2'),SALT39.HygieneErrors.Good);
...
EXPORT Make_rmsid(SALT39.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_rmsid(SALT39.StrType s) := InValidFT_invalid_numeric(s);
...
EXPORT Make_corp_number(SALT39.StrType s0) := s0;
EXPORT InValid_corp_number(SALT39.StrType s) := 0;
EXPORT InValidMessage_corp_number(UNSIGNED1 wh) := '';
 
EXPORT Make_corp_type(SALT39.StrType s0) := s0;
EXPORT InValid_corp_type(SALT39.StrType s) := 0;
...
EXPORT Make_orig_postal_code(SALT39.StrType s0) := MakeFT_invalid_mandatory(s0);
EXPORT InValid_orig_postal_code(SALT39.StrType s) := InValidFT_invalid_mandatory(s);

/*--SOAP--
<message name="St. Cloud Main Service SD1O">
...
/* **********************************************
   ...
   **********************************************/
	...
	Industry_Search := Inquiry_AccLogs.Key_Inquiry_industry_use_vertical_login(FALSE)(s_company_id = CompanyID and s_product_id = (String)Risk_Reporting.ProductID.RiskWise__RiskWiseMainSD1O);
/* ************* End Scout Fields **************/
...
targusGatewaySet := ['ex03','ex22','ex70','sd01','cb61','cb62'];
...
final_SD10 := RiskWise.SD1O_Function(f, gateways, GLB_Purpose, DPPA_Purpose, tribCode, DataRestriction, DataPermission);

dRoyalties := DATASET([], Royalty.Layouts.Royalty) : STORED('Royalties');

finalAnswer := if(tribCode in productSet,
				if(count(final_seed)>0 and runSeed_value, final_seed, final_SD10), 
				   dataset([], riskwise.Layout_SD1O));

map_score1 := map(tribcode in ['ex06','ex07','ex08','ex09','ex10','ex22',
													 'ex70','sd01','sd50','cb61','cb62','2x08','2x10'] => 1,
									tribcode in ['ex02','ex03']                                => 2,
																																								0);
map_score2 := map(tribcode in ['cb61','cb62']                      => 2,
									tribcode in ['ex06','ex07','ex08','ex09',
															 'ex10','ex22','sd50','2x08','2x10'] => 3,
									tribcode in ['ex02','ex03']                      => 4,
																																			0);
...
Deltabase_Logging_prep := project(finalAnswer, transform(Risk_Reporting.Layouts.LOG_Deltabase_Layout_Record,
																								 ...
																								 self.o_score_1    := map(map_score1 = 1 => (Integer)left.score,
																																					map_score1 = 2 => (Integer)left.score2,
																																														0),
																								 self.o_reason_1_1 := map(map_score1 = 1 => left.reason11,
																																					map_score1 = 2 => left.reason12,
																																														''),
																								 self.o_reason_1_2 := map(map_score1 = 1 => left.reason21,
																																					map_score1 = 2 => left.reason22,
																																														''),
																								 self.o_reason_1_3 := map(map_score1 = 1 => left.reason31,
																																					map_score1 = 2 => left.reason32,
																																														''),
																								 self.o_reason_1_4 := map(map_score1 = 1 => left.reason41,
																																					map_score1 = 2 => left.reason42,
																																														''),
																								 self.o_score_2    := map(map_score2 = 2 => (Integer)left.score2,
																																					map_score2 = 3 => (Integer)left.score3,
																																					map_score2 = 4 => (Integer)left.score4,
																																														0),
																								 self.o_reason_2_1 := map(map_score2 = 2 => left.reason12,
																																					map_score2 = 3 => left.reason13,
																																					map_score2 = 4 => left.reason14,
																																														''),
																								 self.o_reason_2_2 := map(map_score2 = 2 => left.reason22,
																																					map_score2 = 3 => left.reason23,
																																					map_score2 = 4 => left.reason24,
																																														''),
																								 self.o_reason_2_3 := map(map_score2 = 2 => left.reason32,
																																					map_score2 = 3 => left.reason33,
																																					map_score2 = 4 => left.reason34,
																																														''),
																								 self.o_reason_2_4 := map(map_score2 = 2 => left.reason42,
																																					map_score2 = 3 => left.reason43,
																																					map_score2 = 4 => left.reason44,
																																														''),

...
export Data_Layout := RECORD
  SALT26.StrType Fld { MAXLENGTH(200000) };
	UNSIGNED2 FldNo;
  END;
...
shared Pattern_Layout := record
  SALT26.StrType data_pattern {maxlength(200000)};
	unsigned cnt;
	end;
	
shared Value_Layout := record
  SALT26.StrType val  {maxlength(200000)};
	unsigned cnt;
	end;
export Field_Identification := RECORD
  unsigned2 FldNo;
  SALT26.StrType FieldName;
  END;
...
#if (UseUnicode)
		SELF.Txt := '  '+MAP( ~le.AllNum => 'UNICODE' + IF(le.MaxLen<100,(SALT26.StrType)le.MaxLen, ''),
#else
		SELF.Txt := '  '+MAP( ~le.AllNum => 'STRING' + IF(le.MaxLen<100,(SALT26.StrType)le.MaxLen, ''),
#end
...
glbl_field_cnts := record
  FldInv1;
	unsigned2 xx_l := length(trim((SALT26.StrType)FldInv1.Fld));
	unsigned2 xx_wc := SALT26.WordCount((SALT26.StrType)FldInv1.Fld);
  end;
...
r tr(FldInv le,unsigned co) := transform
  self.c := ((SALT26.StrType)le.Fld)[co];
	self := le;
  end;
...
dr := record
	  SALT26.StrType Fld  {maxlength(200000)} := SALT26.fn_data_pattern(FldInv.Fld);
		FldInv.Cnt;
		FldInv.FldNo;
  end;
t := table(FldInv,dr);

...
doJoin(SET OF DATASET(indexOutputRecord) inputs) := FUNCTION
    indexOutputRecord createMatchRecord(indexOutputRecord firstMatch, DATASET(indexOutputRecord) allMatches) := TRANSFORM
        ...
        SELF := firstMatch;
    END;
    RETURN JOIN(inputs, STEPPED(LEFT.joinfield = RIGHT.joinfield AND LEFT.joinfieldp = RIGHT.joinfieldp), createMatchRecord(LEFT, ROWS(LEFT)), SORTED(joinfieldp,joinfield));
END;
#uniquename(r)
%r% := RECORDOF(n);

...
doJoin(SET OF DATASET(indexOutputRecord) inputs) := FUNCTION
    indexOutputRecord createMatchRecord(indexOutputRecord firstMatch, DATASET(indexOutputRecord) allMatches) := TRANSFORM
        ...
        SELF := firstMatch;
    END;
    RETURN JOIN(inputs, STEPPED(LEFT.joinfield = RIGHT.joinfield AND LEFT.joinfieldp = RIGHT.joinfieldp AND LEFT.joinfieldpp = RIGHT.joinfieldpp), createMatchRecord(LEFT, ROWS(LEFT)), SORTED(joinfieldpp,joinfieldp,joinfield));
END;
#uniquename(r)
%r% := RECORDOF(n);

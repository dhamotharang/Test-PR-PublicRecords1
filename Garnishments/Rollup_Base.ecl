...
function
	...
	pDataset_Dist := distribute(pDataset, hash64(rawfields.Courtid,rawfields.Filetypeid,rawfields.CaseNumber));
	pDataset_sort := sort(pDataset_Dist	,except 
		...
		,record_type
		,rawfields.FilingDate
		,clean_dates.FilingDate
		,UltScore
		,OrgScore
		,SELEScore
		,ProxScore
	...
	dnotfull_sort 			:= sort	(pDataset_rollup, rawfields.Courtid,rawfields.Filetypeid,rawfields.CaseNumber, local	);
	dnotfull_group			:= group(pDataset_sort	, rawfields.Courtid,rawfields.Filetypeid,rawfields.CaseNumber, local	);

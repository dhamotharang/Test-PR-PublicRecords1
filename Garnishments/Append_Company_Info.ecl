...
/*
	take the defendant address and do a radius search and see if there is a business nearby that matches
	that name
*/
export Append_Company_Info(

	 dataset(Layouts.Base																	) pGarnishments
	...
	,string																									pPersistname			= persistnames().AppendCompanyInfo
...
function
	...
	*/
	...
	dgarn_prep 						:= project(pGarnishments	,transform({layouts.base,string clean_company_name,unsigned8 rid}, self := left;self.clean_company_name := ut.CleanCompany(left.rawfields.AttorneyName); self.rid := counter)) : global;
	dgarn_prep_withzip		:= dgarn_prep((unsigned)Clean_Defendant_address.zip != 0);
	dgarn_prep_withoutzip := dgarn_prep((unsigned)Clean_Defendant_address.zip  = 0);
	...
	dfirstjoin := join(
		 dgarn_prep_withzip
		,dbh_back
		,			left.Clean_Defendant_address.zip	= right.zip
			and stringlib.stringtouppercase(trim(left.rawfields.AttorneyName)) = stringlib.stringtouppercase(trim(RIGHT.company_name))
		,transform(
			 recordof(dgarn_prep)
	...
	dnomatches2_zipradius := project(dnomatches2	
		,transform(
			...
			,self 					:= left;
			 self.ziprad5 	:= ziplib.ZipsWithinRadius(left.Clean_Defendant_address.zip,5	);
			 self.ziprad10	:= ziplib.ZipsWithinRadius(left.Clean_Defendant_address.zip,10	);
			 self.ziprad15	:= ziplib.ZipsWithinRadius(left.Clean_Defendant_address.zip,15	);
			 self.ziprad20	:= ziplib.ZipsWithinRadius(left.Clean_Defendant_address.zip,20	);
	));
	...
	dnomatches2_zipradius_norm5 := normalize(dnomatches2_zipradius,count(left.ziprad5)
		,transform(
			...
			,self.zipdist 										:= 5;
			 self.Clean_Defendant_address.zip := intformat(left.ziprad5[counter],5,1);
			 self 														:= left;
	));
	...
	dnomatches2_zipradius_norm10 := normalize(dnomatches2_zipradius,count(left.ziprad10)
		,transform(
			 {recordof(dnomatches2),unsigned2 zipdist}
			,self.zipdist 										:= 10;
			 self.Clean_Defendant_address.zip := intformat(left.ziprad10[counter],5,1);
			 self 														:= left;
	));
	...
	dnomatches2_zipradius_norm15 := normalize(dnomatches2_zipradius,count(left.ziprad15)
		,transform(
			 {recordof(dnomatches2),unsigned2 zipdist}
			,self.zipdist 										:= 15;
			 self.Clean_Defendant_address.zip := intformat(left.ziprad15[counter],5,1);
			 self 														:= left;
	));
	...
	dnomatches2_zipradius_norm20 := normalize(dnomatches2_zipradius,count(left.ziprad20)
		,transform(
			 {recordof(dnomatches2),unsigned2 zipdist}
			,self.zipdist 										:= 20;
			 self.Clean_Defendant_address.zip := intformat(left.ziprad20[counter],5,1);
			 self 														:= left;
	));
	...
	dnomatches2_zipradius_all := 
		...
		;
	dnomatches2_zipradius_all_dedup := dedup(sort(dnomatches2_zipradius_all,rid,Clean_Defendant_address.zip, zipdist), rid, Clean_Defendant_address.zip);
	...
	dziprad_dedup := project(dedup(sort(dzipradmatches2 + dzipradnomatches2, rid, zipdist), rid),recordof(dgarn_prep));
	
	returndataset := project(dmatches2 + dziprad_dedup + dgarn_prep_withoutzip,layouts.base);

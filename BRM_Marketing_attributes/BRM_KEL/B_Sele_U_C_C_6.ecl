...
EXPORT B_Sele_U_C_C_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE837984 := __ENH_Sele_U_C_C_7;
  EXPORT __ST154590_Layout := RECORD
  ...
  END;
  EXPORT __ST154582_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST154590_Layout) Sub_Filing_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST154582_Layout __ND838089__Project(B_Sele_U_C_C_7(__in,__cfg).__ST155951_Layout __PP837985) := TRANSFORM
    __EE838020 := __PP837985.Sub_Filing_;
    SELF.Sub_Filing_ := __BN(PROJECT(__T(__EE838020),__ST154590_Layout),__NL(__EE838020));
    SELF.Is_Filing_ := IF(__T(__OP2(__FN1(KEL.Routines.ToUpperCase,__FN1(TRIM,__PP837985.Filtered_Party_Type_)),IN,__CN(['D','S','C','A']))),TRUE,FALSE);
    SELF := __PP837985;
  END;
  EXPORT __ENH_Sele_U_C_C_6 := PROJECT(__EE837984,__ND838089__Project(LEFT));
END;

...
EXPORT B_Sele_U_C_C_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE845794 := __ENH_Sele_U_C_C_7;
  ...
  END;
  EXPORT __ST160362_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST160370_Layout) Sub_Filing_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST160362_Layout __ND845899__Project(B_Sele_U_C_C_7(__in,__cfg).__ST161731_Layout __PP845795) := TRANSFORM
    __EE845830 := __PP845795.Sub_Filing_;
    SELF.Sub_Filing_ := __BN(PROJECT(__T(__EE845830),__ST160370_Layout),__NL(__EE845830));
    SELF.Is_Filing_ := IF(__T(__OP2(__FN1(KEL.Routines.ToUpperCase,__FN1(TRIM,__PP845795.Filtered_Party_Type_)),IN,__CN(['D','S','C','A']))),TRUE,FALSE);
    SELF := __PP845795;
  END;
  EXPORT __ENH_Sele_U_C_C_6 := PROJECT(__EE845794,__ND845899__Project(LEFT));
END;

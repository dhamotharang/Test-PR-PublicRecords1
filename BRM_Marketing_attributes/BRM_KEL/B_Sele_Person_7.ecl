...
EXPORT B_Sele_Person_7(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE814631 := __E_Sele_Person;
  EXPORT __ST155875_Layout := RECORD
  ...
  END;
  SHARED __ST155875_Layout __ND814595__Project(E_Sele_Person(__in,__cfg).Layout __PP223243) := TRANSFORM
    __EE814570 := __PP223243.Contact_Info_;
    ...
    __EE814590 := __PP223243.Contact_Info_;
    SELF.Assoc_Date_ := IF(__T(__OP2(KEL.Aggregates.MaxNN(__EE814570,KEL.era.ToDate(__T(__EE814570).Date_Last_Seen_)),>,__CC13398)),__ECAST(KEL.typ.nkdate,__CC13398),__ECAST(KEL.typ.nkdate,KEL.Aggregates.MaxNN(__EE814590,KEL.era.ToDate(__T(__EE814590).Date_Last_Seen_))));
    SELF := __PP223243;
  END;
  EXPORT __ENH_Sele_Person_7 := PROJECT(__EE814631,__ND814595__Project(LEFT));
END;

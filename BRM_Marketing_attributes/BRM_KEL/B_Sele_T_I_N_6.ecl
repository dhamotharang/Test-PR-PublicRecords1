...
EXPORT B_Sele_T_I_N_6(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE251063 := __E_Sele_T_I_N;
  SHARED __EE836766 := __ENH_Business_Sele_7;
  SHARED __ST251135_Layout := RECORD
  ...
  END;
  __JC836772(E_Sele_T_I_N(__in,__cfg).Layout __EE251063, B_Business_Sele_7(__in,__cfg).__ST155330_Layout __EE836766) := __EEQP(__EE251063.Legal_,__EE836766.UID);
  __ST251135_Layout __JT836772(E_Sele_T_I_N(__in,__cfg).Layout __l, B_Business_Sele_7(__in,__cfg).__ST155330_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE836773 := JOIN(__EE251063,__EE836766,__JC836772(LEFT,RIGHT),__JT836772(LEFT,RIGHT),LEFT OUTER,HASH);
  EXPORT __ST154519_Layout := RECORD
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  EXPORT __ST154510_Layout := RECORD
    KEL.typ.ntyp(E_Business_Sele().Typ) Legal_;
    KEL.typ.ntyp(E_T_I_N().Typ) Tax_I_D_;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    ...
    KEL.typ.nint Best_T_I_N_;
    KEL.typ.nint Best_T_I_N_Rank_;
    KEL.typ.ndataset(__ST154519_Layout) Data_Sources_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST154510_Layout __ND837391__Project(__ST251135_Layout __PP836774) := TRANSFORM
    __EE837085 := __PP836774.Data_Sources_;
    __ST154519_Layout __ND837090__Project(E_Sele_T_I_N(__in,__cfg).Data_Sources_Layout __PP837086) := TRANSFORM
      SELF.My_Date_First_Seen_ := KEL.era.ToDate(__PP837086.Date_First_Seen_);
      SELF.My_Date_Last_Seen_ := KEL.era.ToDate(__PP837086.Date_Last_Seen_);
      SELF.Translated_Source_ := FN_Compile(__cfg).FN_Source_Group(__ECAST(KEL.typ.nstr,__PP837086.Source_));
      SELF := __PP837086;
    END;
    SELF.Data_Sources_ := __PROJECT(__EE837085,__ND837090__Project(LEFT));
    SELF.Input_T_I_N_Match_ := FN_Compile(__cfg).FN_Edit_Distance_Within_Radius(__ECAST(KEL.typ.nstr,__PP836774.Tax_I_D_),__ECAST(KEL.typ.nstr,__PP836774.B___Inp_Cln_T_I_N_),__ECAST(KEL.typ.nint,__CN(2)));
    SELF := __PP836774;
  END;
  EXPORT __ENH_Sele_T_I_N_6 := PROJECT(__EE836773,__ND837391__Project(LEFT));
END;

...
IMPORT KEL15 AS KEL;
IMPORT CFG_Compile,E_Property,E_Property_Event,E_Zip_Code,FN_Compile FROM BRM_Marketing_attributes.BRM_KEL;
...
EXPORT B_Property_Event_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE300336 := __E_Property_Event;
  EXPORT __ST157793_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST157793_Layout __ND875888__Project(E_Property_Event(__in,__cfg).Layout __PP299994) := TRANSFORM
    ...
    SELF.Is_Current_Assessment_Record_ := __AND(__PP299994.Is_Assessment_,__OP2(FN_Compile(__cfg).FN_A_B_S_Y_E_A_R_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,KEL.era.ToDate(__PP299994.Date_First_Seen_)),__ECAST(KEL.typ.nkdate,__CC13330)),<=,__CN(1)));
    SELF := __PP299994;
  END;
  EXPORT __ENH_Property_Event_5 := PROJECT(__EE300336,__ND875888__Project(LEFT));
END;

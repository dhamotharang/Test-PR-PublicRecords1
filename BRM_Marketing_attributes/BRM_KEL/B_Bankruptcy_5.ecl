...
EXPORT B_Bankruptcy_5(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE846930 := __ENH_Bankruptcy_6;
  EXPORT __ST263829_Layout := RECORD
  ...
  END;
  EXPORT __ST155958_Layout := RECORD
    KEL.typ.nuid UID;
    ...
    KEL.typ.ndataset(__ST263829_Layout) Best_Child_Record_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST155958_Layout __ND846887__Project(B_Bankruptcy_6(__in,__cfg).__ST158637_Layout __PP846552) := TRANSFORM
    __EE846885 := __PP846552.Best_Child_Record_;
    __ST263829_Layout __ND846820__Project(B_Bankruptcy_7(__in,__cfg).__ST160665_Layout __PP846610) := TRANSFORM
      ...
      SELF.Filing_Age_In_Days_ := FN_Compile(__cfg).FN_A_B_S_D_A_Y_S_B_E_T_W_E_E_N(__ECAST(KEL.typ.nkdate,__PP846610.Date_Filed_),__ECAST(KEL.typ.nkdate,__CC13210));
      __BS846677 := __T(__PP846552.Case_Details_);
      SELF.Is_Bankruptcy_ := __AND(__PP846552.Has_Case_Number_,__CN(EXISTS(__BS846677(__T(__OP2(__CAST(KEL.typ.str,__T(__PP846552.Case_Details_).Case_I_D_),<>,__CN('')))))));
      SELF := __PP846610;
    END;
    SELF.Best_Child_Record_ := __FILTER(__PROJECT(__EE846885,__ND846820__Project(LEFT)),__NN(Source_Description_) OR __NN(Original_Chapter_) OR __NN(Filing_Type_) OR __NN(Business_Flag_) OR __NN(Corporate_Flag_) OR __NN(Discharged_Date_) OR __NN(Disposition_) OR __NN(Debtor_Type_) OR __NN(Debtor_Sequence_) OR __NN(Disposition_Type_) OR __NN(Disposition_Reason_) OR __NN(Disposition_Type_Description_) OR __NN(Name_Type_) OR __NN(Screen_Description_) OR __NN(Decoded_Description_) OR __NN(Date_Filed_) OR __NN(Record_Type_) OR __NN(Last_Status_Update_) OR __NN(Filing_Age_In_Days_) OR __NN(Is_Bankruptcy_));
    SELF := __PP846552;
  END;
  EXPORT __ENH_Bankruptcy_5 := PROJECT(__EE846930,__ND846887__Project(LEFT));
END;

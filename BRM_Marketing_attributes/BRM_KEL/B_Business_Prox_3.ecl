...
EXPORT B_Business_Prox_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE915504 := __E_Business_Prox;
  SHARED __EE915515 := __E_Prox_Address;
  SHARED __EE915525 := __EE915515(__NN(__EE915515.Business_Location_));
  SHARED __ST367885_Layout := RECORD
  ...
  END;
  __ST367885_Layout __JT915533(E_Prox_Address(__in,__cfg).Layout __l, E_Prox_Address(__in,__cfg).Best_Addresses_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE915602 := NORMALIZE(__EE915525,__T(LEFT.Best_Addresses_),__JT915533(LEFT,RIGHT));
  SHARED __ST367485_Layout := RECORD
  ...
  SHARED __ST371785_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST367485_Layout) Exp1_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC915688(E_Business_Prox(__in,__cfg).Layout __EE915504, __ST367485_Layout __EE915679) := __EEQP(__EE915504.UID,__EE915679.____grp___U_I_D_);
  __ST371785_Layout __Join__ST371785_Layout(E_Business_Prox(__in,__cfg).Layout __r, DATASET(__ST367485_Layout) __recs) := TRANSFORM
    SELF := __r;
    SELF.Exp1_ := __CN(__recs);
  END;
  SHARED __EE915882 := DENORMALIZE(DISTRIBUTE(__EE915504,HASH(UID)),DISTRIBUTE(__EE915679,HASH(____grp___U_I_D_)),__JC915688(LEFT,RIGHT),GROUP,__Join__ST371785_Layout(LEFT,ROWS(RIGHT)),LOCAL);
  SHARED __ST158993_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Contacts_Layout) Contacts_;
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Data_Sources_Layout) Data_Sources_;
    KEL.typ.ndataset(__ST367485_Layout) Best_Business_Prox_Address_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST158993_Layout __ND915890__Project(__ST371785_Layout __PP915886) := TRANSFORM
    __EE915885 := __PP915886.Exp1_;
    SELF.Best_Business_Prox_Address_ := __EE915885;
    SELF := __PP915886;
  END;
  SHARED __EE916346 := PROJECT(__EE915882,__ND915890__Project(LEFT));
  EXPORT __ST367246_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST367246_Layout __ND915018__Project(__ST158993_Layout __PP915014) := TRANSFORM
    __EE916349 := __PP915014.Best_Business_Prox_Address_;
    SELF.Best_Business_Prox_Address_ := __PROJECT(__EE916349,E_Prox_Address(__in,__cfg).Best_Addresses_Layout);
    SELF := __PP915014;
  END;
  EXPORT __ENH_Business_Prox_3 := PROJECT(__EE916346,__ND915018__Project(LEFT));
END;

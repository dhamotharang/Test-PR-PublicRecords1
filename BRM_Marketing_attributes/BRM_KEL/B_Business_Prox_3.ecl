...
EXPORT B_Business_Prox_3(CFG_Compile.FDCDataset __in = CFG_Compile.FDCDefault, CFG_Compile __cfg = CFG_Compile) := MODULE
  ...
  SHARED __EE901679 := __E_Business_Prox;
  SHARED __EE901690 := __E_Prox_Address;
  SHARED __EE901700 := __EE901690(__NN(__EE901690.Business_Location_));
  SHARED __ST353905_Layout := RECORD
  ...
  END;
  __ST353905_Layout __JT901708(E_Prox_Address(__in,__cfg).Layout __l, E_Prox_Address(__in,__cfg).Best_Addresses_Layout __r) := TRANSFORM
    ...
    SELF := __r;
  END;
  SHARED __EE901777 := NORMALIZE(__EE901700,__T(LEFT.Best_Addresses_),__JT901708(LEFT,RIGHT));
  SHARED __ST353505_Layout := RECORD
  ...
  SHARED __ST357805_Layout := RECORD
    ...
    KEL.typ.ndataset(__ST353505_Layout) Exp1_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  __JC901863(E_Business_Prox(__in,__cfg).Layout __EE901679, __ST353505_Layout __EE901854) := __EEQP(__EE901679.UID,__EE901854.____grp___U_I_D_);
  __ST357805_Layout __Join__ST357805_Layout(E_Business_Prox(__in,__cfg).Layout __r, DATASET(__ST353505_Layout) __recs) := TRANSFORM
    SELF := __r;
    SELF.Exp1_ := __CN(__recs);
  END;
  SHARED __EE902057 := DENORMALIZE(DISTRIBUTE(__EE901679,HASH(UID)),DISTRIBUTE(__EE901854,HASH(____grp___U_I_D_)),__JC901863(LEFT,RIGHT),GROUP,__Join__ST357805_Layout(LEFT,ROWS(RIGHT)),LOCAL);
  SHARED __ST142266_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Contacts_Layout) Contacts_;
    KEL.typ.ndataset(E_Business_Prox(__in,__cfg).Data_Sources_Layout) Data_Sources_;
    KEL.typ.ndataset(__ST353505_Layout) Best_Business_Prox_Address_;
    KEL.typ.epoch Archive___Date_ := 0;
    KEL.typ.epoch Date_First_Seen_ := 0;
    KEL.typ.epoch Date_Last_Seen_ := 0;
    KEL.typ.epoch Hybrid_Archive_Date_ := 0;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST142266_Layout __ND902065__Project(__ST357805_Layout __PP902061) := TRANSFORM
    __EE902060 := __PP902061.Exp1_;
    SELF.Best_Business_Prox_Address_ := __EE902060;
    SELF := __PP902061;
  END;
  SHARED __EE902521 := PROJECT(__EE902057,__ND902065__Project(LEFT));
  EXPORT __ST353266_Layout := RECORD
    KEL.typ.nuid UID;
    KEL.typ.nint Ult_I_D_;
    KEL.typ.nint Org_I_D_;
    KEL.typ.nint Sele_I_D_;
    ...
    KEL.typ.int __RecordCount := 0;
  END;
  SHARED __ST353266_Layout __ND901193__Project(__ST142266_Layout __PP901189) := TRANSFORM
    __EE902524 := __PP901189.Best_Business_Prox_Address_;
    SELF.Best_Business_Prox_Address_ := __PROJECT(__EE902524,E_Prox_Address(__in,__cfg).Best_Addresses_Layout);
    SELF := __PP901189;
  END;
  EXPORT __ENH_Business_Prox_3 := PROJECT(__EE902521,__ND901193__Project(LEFT));
END;

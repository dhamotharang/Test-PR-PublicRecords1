...
EXPORT Compliance.Layout_Parms ExtractParmsB(Compliance.Layout_In srch) := 
	TRANSFORM
		SELF.fname := MAP(REGEXFIND('FNAME',srch.search_str) 			=> REGEXFIND('FNAME=\\(([0-9A-Z* ,#./-]+)\\)', ToCaps(srch.search_str), 1),
											REGEXFIND('first-name',srch.search_str) => REGEXFIND('FIRST-NAME\\(([0-9A-Z* ,#./-]+)\\)', ToCaps(srch.search_str), 1),
											''
										 );
		SELF.mname := MAP(REGEXFIND('MNAME',srch.search_str) 				=> REGEXFIND('MNAME=\\(([0-9A-Z* ,#./-]+)\\)', ToCaps(srch.search_str), 1),
											REGEXFIND('middle-name',srch.search_str) 	=> REGEXFIND('MIDDLE-NAME\\(([0-9A-Z* ,#./-]+)\\)', ToCaps(srch.search_str), 1),
											''
										 );								 
		...
		SELF.lname := MAP(REGEXFIND('LNAME',srch.search_str) 			=> TRIM(Stringlib.Stringfilterout(
											...
											REGEXFIND('last-name',srch.search_str) 	=> TRIM(Stringlib.Stringfilterout(
...
//										  REGEXFIND('state',srch.search_str) 	=> REGEXFIND('STATE\\(([A-Z]+)\\)', ToCaps(srch.search_str), 1),
		...
		SELF.zip := MAP(REGEXFIND('ZIP',srch.search_str) 			=> REGEXFIND('ZIP=\\(([0-9]+)\\)', ToCaps(srch.search_str), 1),
										REGEXFIND('zip code',srch.search_str) => REGEXFIND('ZIP CODE\\(([0-9]+)\\)', ToCaps(srch.search_str), 1),
										''
									 );
		
		SELF.ssn := MAP(REGEXFIND('SSN',srch.search_str) 	=> REGEXFIND('SSN=\\(([0-9]+)\\)', ToCaps(srch.search_str), 1),
										REGEXFIND('ssn',srch.search_str) 	=> REGEXFIND('SSN\\(([0-9]+)\\)', ToCaps(srch.search_str), 1),
										''
									 );
		...
		SELF.UID := MAP(REGEXFIND('LexID',srch.search_str) 				=> (unsigned6) REGEXFIND('LEXID\\(([0-9]+)\\)', ToCaps(srch.search_str), 1),
										REGEXFIND('businessid',srch.search_str) 	=> (unsigned6) REGEXFIND('BUSINESSID\\(([0-9]+)\\)', ToCaps(srch.search_str), 1),

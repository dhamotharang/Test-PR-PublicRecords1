//Change D$ records in header.file_header_raw into the Header Key format	//
//See doxie.header_pre_keybuild	//

IMPORT Header, Census_data, Gong, Watchdog, did_add, ut, Data_Services,doxie_build,mdr;


head := Header.file_header_raw(fname<>'',lname<>'',src='D$');
...
xHead_Layout :=
	RECORD
		...
		unsigned4 dod := 0;					//really just need this
		STRING1 death_code := '';
...
//tbl_exp_lookups := DATASET(Data_Services.foreign_prod+'thor400_60::persist::exp_lookups',rec_exp_lookups,thor);
tbl_exp_lookups := DATASET(Data_Services.foreign_prod+'thor400_30::persist::exp_lookups__p1268229782',rec_exp_lookups,thor);
...
Combined_Layout :=
	RECORD
		...
		Header.File_Did_Death_Master_SSA.dod8;
		Header.File_Did_Death_Master_SSA.VorP_code;

	END;

Combined_Layout startLookups(tbl_exp_lookups le, Header.File_HHID_Current ri) :=
	TRANSFORM
...
with_lookups := JOIN(DISTRIBUTE(tbl_exp_lookups, HASH((unsigned6)did)), 
...
Combined_Layout getDeath(Combined_Layout le, Header.File_Did_Death_Master_SSA ri) :=
	TRANSFORM
...
death0:=sort(DISTRIBUTE(header.File_Did_Death_Master_SSA((unsigned6)did != 0), HASH((unsigned6)did)),did,local);
death:=rollup(death0
				,left.did=right.did
				,transform({death0}
...
addrMatch1 := 
	MACRO
		DID_Add.Address_Match_Score(LE.prim_range, LE.prim_name, LE.sec_range, LE.zip, 
								RI.prim_range, RI.prim_name, RI.sec_range, RI.zip) > 75 
								AND	DID_Add.Address_Match_Score(LE.prim_range, LE.prim_name, LE.sec_range, LE.zip, 
								RI.prim_range, RI.prim_name, RI.sec_range, RI.zip) < 255 
...
with_appends := JOIN(with_county, with_death, 
										 LEFT.did = RIGHT.did
										 ,getHHid(LEFT, RIGHT), LEFT OUTER, LOCAL);


EXPORT	file_SSA_restricted_as_PHDR_key := with_appends; 

...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(corp2_mapping.layoutscommon.main)
    ...
    UNSIGNED1 cont_filing_cd_Invalid;
...
EXPORT FromNone(DATASET(corp2_mapping.layoutscommon.main) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.cont_filing_date_Invalid := Fields.InValid_cont_filing_date((SALT34.StrType)le.cont_filing_date);
    SELF.cont_filing_cd_Invalid := Fields.InValid_cont_filing_cd((SALT34.StrType)le.cont_filing_cd);
    SELF.cont_filing_desc_Invalid := Fields.InValid_cont_filing_desc((SALT34.StrType)le.cont_filing_desc);
    SELF.cont_type_cd_Invalid := Fields.InValid_cont_type_cd((SALT34.StrType)le.cont_type_cd);
    SELF.cont_type_desc_Invalid := Fields.InValid_cont_type_desc((SALT34.StrType)le.cont_type_desc);
    SELF.corp_renewal_date_Invalid := Fields.InValid_corp_renewal_date((SALT34.StrType)le.corp_renewal_date);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_ln_name_type_desc_Invalid << 22 ) + ( le.corp_filing_desc_Invalid << 23 ) + ( le.corp_status_cd_Invalid << 24 ) + ( le.corp_status_desc_Invalid << 25 ) + ( le.corp_standing_Invalid << 26 ) + ( le.corp_inc_date_Invalid << 27 ) + ( le.corp_term_exist_cd_Invalid << 29 ) + ( le.corp_term_exist_exp_Invalid << 30 ) + ( le.corp_term_exist_desc_Invalid << 32 ) + ( le.corp_foreign_domestic_ind_Invalid << 33 ) + ( le.corp_forgn_state_cd_Invalid << 34 ) + ( le.corp_forgn_state_desc_Invalid << 35 ) + ( le.corp_forgn_date_Invalid << 36 ) + ( le.corp_forgn_term_exist_cd_Invalid << 38 ) + ( le.corp_forgn_term_exist_exp_Invalid << 39 ) + ( le.corp_forgn_term_exist_desc_Invalid << 41 ) + ( le.corp_orig_org_structure_cd_Invalid << 42 ) + ( le.corp_for_profit_ind_Invalid << 43 ) + ( le.cont_filing_date_Invalid << 44 ) + ( le.cont_filing_cd_Invalid << 46 ) + ( le.cont_filing_desc_Invalid << 47 ) + ( le.cont_type_cd_Invalid << 48 ) + ( le.cont_type_desc_Invalid << 49 ) + ( le.corp_renewal_date_Invalid << 50 ) + ( le.recordorigin_Invalid << 52 ) + ( le.internalfield1_Invalid << 53 ) + ( le.internalfield2_Invalid << 54 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_filing_desc_Invalid := (le.ScrubsBits1 >> 23) & 1;
    ...
    SELF.corp_term_exist_desc_Invalid := (le.ScrubsBits1 >> 32) & 1;
    SELF.corp_foreign_domestic_ind_Invalid := (le.ScrubsBits1 >> 33) & 1;
    SELF.corp_forgn_state_cd_Invalid := (le.ScrubsBits1 >> 34) & 1;
    SELF.corp_forgn_state_desc_Invalid := (le.ScrubsBits1 >> 35) & 1;
    SELF.corp_forgn_date_Invalid := (le.ScrubsBits1 >> 36) & 3;
    SELF.corp_forgn_term_exist_cd_Invalid := (le.ScrubsBits1 >> 38) & 1;
    SELF.corp_forgn_term_exist_exp_Invalid := (le.ScrubsBits1 >> 39) & 3;
    SELF.corp_forgn_term_exist_desc_Invalid := (le.ScrubsBits1 >> 41) & 1;
    SELF.corp_orig_org_structure_cd_Invalid := (le.ScrubsBits1 >> 42) & 1;
    SELF.corp_for_profit_ind_Invalid := (le.ScrubsBits1 >> 43) & 1;
    SELF.cont_filing_date_Invalid := (le.ScrubsBits1 >> 44) & 3;
    SELF.cont_filing_cd_Invalid := (le.ScrubsBits1 >> 46) & 1;
    SELF.cont_filing_desc_Invalid := (le.ScrubsBits1 >> 47) & 1;
    SELF.cont_type_cd_Invalid := (le.ScrubsBits1 >> 48) & 1;
    SELF.cont_type_desc_Invalid := (le.ScrubsBits1 >> 49) & 1;
    SELF.corp_renewal_date_Invalid := (le.ScrubsBits1 >> 50) & 3;
    SELF.recordorigin_Invalid := (le.ScrubsBits1 >> 52) & 1;
    SELF.internalfield1_Invalid := (le.ScrubsBits1 >> 53) & 1;
    SELF.internalfield2_Invalid := (le.ScrubsBits1 >> 54) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_forgn_term_exist_exp_LENGTH_ErrorCount := COUNT(GROUP,h.corp_forgn_term_exist_exp_Invalid=3);
    ...
    cont_filing_cd_ENUM_ErrorCount := COUNT(GROUP,h.cont_filing_cd_Invalid=1);
    cont_filing_desc_ENUM_ErrorCount := COUNT(GROUP,h.cont_filing_desc_Invalid=1);
    ...
    corp_renewal_date_LENGTH_ErrorCount := COUNT(GROUP,h.corp_renewal_date_Invalid=3);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_forgn_term_exist_exp_Invalid,'ALLOW','CUSTOM','LENGTH','UNKNOWN')
          ...
          ,CHOOSE(le.cont_filing_cd_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.cont_filing_desc_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.cont_type_cd_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.cont_type_desc_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.corp_renewal_date_Invalid,'ALLOW','CUSTOM','LENGTH','UNKNOWN')
    ...
    SELF.FieldName := CHOOSE(c,'dt_vendor_first_reported','dt_vendor_last_reported','dt_first_seen','dt_last_seen','corp_ra_dt_first_seen','corp_ra_dt_last_seen','corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_orig_sos_charter_nbr','corp_legal_name','corp_ln_name_type_cd','corp_ln_name_type_desc','corp_filing_desc','corp_status_cd','corp_status_desc','corp_standing','corp_inc_date','corp_term_exist_cd','corp_term_exist_exp','corp_term_exist_desc','corp_foreign_domestic_ind','corp_forgn_state_cd','corp_forgn_state_desc','corp_forgn_date','corp_forgn_term_exist_cd','corp_forgn_term_exist_exp','corp_forgn_term_exist_desc','corp_orig_org_structure_cd','corp_for_profit_ind','cont_filing_date','cont_filing_cd','cont_filing_desc','cont_type_cd','cont_type_desc','corp_renewal_date','recordorigin','internalfield1','internalfield2','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_mandatory','invalid_ln_name_type_cd','invalid_ln_name_type_desc','invalid_filing_desc','invalid_status_cd','invalid_status_desc','invalid_flag_code','invalid_date','invalid_term_cd','invalid_future_date','invalid_term_desc','invalid_for_dom_ind','invalid_corp_forgn_state_cd','invalid_corp_forgn_state_desc','invalid_date','invalid_term_cd','invalid_future_date','invalid_term_desc','invalid_corp_orig_org_structure_cd','invalid_flag_code','invalid_date','invalid_cont_filing_cd','invalid_cont_filing_desc','invalid_cont_type_cd','invalid_cont_type_desc','invalid_future_date','invalid_recordorigin','invalid_comptype','invalid_type1','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT34.StrType)le.dt_vendor_first_reported,(SALT34.StrType)le.dt_vendor_last_reported,(SALT34.StrType)le.dt_first_seen,(SALT34.StrType)le.dt_last_seen,(SALT34.StrType)le.corp_ra_dt_first_seen,(SALT34.StrType)le.corp_ra_dt_last_seen,(SALT34.StrType)le.corp_key,(SALT34.StrType)le.corp_vendor,(SALT34.StrType)le.corp_state_origin,(SALT34.StrType)le.corp_process_date,(SALT34.StrType)le.corp_orig_sos_charter_nbr,(SALT34.StrType)le.corp_legal_name,(SALT34.StrType)le.corp_ln_name_type_cd,(SALT34.StrType)le.corp_ln_name_type_desc,(SALT34.StrType)le.corp_filing_desc,(SALT34.StrType)le.corp_status_cd,(SALT34.StrType)le.corp_status_desc,(SALT34.StrType)le.corp_standing,(SALT34.StrType)le.corp_inc_date,(SALT34.StrType)le.corp_term_exist_cd,(SALT34.StrType)le.corp_term_exist_exp,(SALT34.StrType)le.corp_term_exist_desc,(SALT34.StrType)le.corp_foreign_domestic_ind,(SALT34.StrType)le.corp_forgn_state_cd,(SALT34.StrType)le.corp_forgn_state_desc,(SALT34.StrType)le.corp_forgn_date,(SALT34.StrType)le.corp_forgn_term_exist_cd,(SALT34.StrType)le.corp_forgn_term_exist_exp,(SALT34.StrType)le.corp_forgn_term_exist_desc,(SALT34.StrType)le.corp_orig_org_structure_cd,(SALT34.StrType)le.corp_for_profit_ind,(SALT34.StrType)le.cont_filing_date,(SALT34.StrType)le.cont_filing_cd,(SALT34.StrType)le.cont_filing_desc,(SALT34.StrType)le.cont_type_cd,(SALT34.StrType)le.cont_type_desc,(SALT34.StrType)le.corp_renewal_date,(SALT34.StrType)le.recordorigin,(SALT34.StrType)le.internalfield1,(SALT34.StrType)le.internalfield2,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_filing_desc:invalid_filing_desc:ENUM'
          ,'corp_status_cd:invalid_status_cd:CUSTOM'
          ...
          ,'corp_term_exist_exp:invalid_future_date:ALLOW','corp_term_exist_exp:invalid_future_date:CUSTOM','corp_term_exist_exp:invalid_future_date:LENGTH'
          ...
          ,'corp_forgn_term_exist_cd:invalid_term_cd:ENUM'
          ,'corp_forgn_term_exist_exp:invalid_future_date:ALLOW','corp_forgn_term_exist_exp:invalid_future_date:CUSTOM','corp_forgn_term_exist_exp:invalid_future_date:LENGTH'
          ,'corp_forgn_term_exist_desc:invalid_term_desc:ENUM'
          ...
          ,'cont_filing_cd:invalid_cont_filing_cd:ENUM'
          ,'cont_filing_desc:invalid_cont_filing_desc:ENUM'
          ,'cont_type_cd:invalid_cont_type_cd:ENUM'
          ,'cont_type_desc:invalid_cont_type_desc:ENUM'
          ,'corp_renewal_date:invalid_future_date:ALLOW','corp_renewal_date:invalid_future_date:CUSTOM','corp_renewal_date:invalid_future_date:LENGTH'
          ,'recordorigin:invalid_recordorigin:ENUM'
          ,'internalfield1:invalid_comptype:CUSTOM'
          ,'internalfield2:invalid_type1:CUSTOM','UNKNOWN');
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_corp_forgn_term_exist_exp(1),Fields.InvalidMessage_corp_forgn_term_exist_exp(2),Fields.InvalidMessage_corp_forgn_term_exist_exp(3)
          ...
          ,Fields.InvalidMessage_cont_filing_date(1),Fields.InvalidMessage_cont_filing_date(2),Fields.InvalidMessage_cont_filing_date(3)
          ,Fields.InvalidMessage_cont_filing_cd(1)
          ,Fields.InvalidMessage_cont_filing_desc(1)
          ,Fields.InvalidMessage_cont_type_cd(1)
          ,Fields.InvalidMessage_cont_type_desc(1)
          ,Fields.InvalidMessage_corp_renewal_date(1),Fields.InvalidMessage_corp_renewal_date(2),Fields.InvalidMessage_corp_renewal_date(3)
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_forgn_term_exist_exp_ALLOW_ErrorCount,le.corp_forgn_term_exist_exp_CUSTOM_ErrorCount,le.corp_forgn_term_exist_exp_LENGTH_ErrorCount
          ...
          ,le.cont_filing_cd_ENUM_ErrorCount
          ,le.cont_filing_desc_ENUM_ErrorCount
          ,le.cont_type_cd_ENUM_ErrorCount
          ,le.cont_type_desc_ENUM_ErrorCount
          ,le.corp_renewal_date_ALLOW_ErrorCount,le.corp_renewal_date_CUSTOM_ErrorCount,le.corp_renewal_date_LENGTH_ErrorCount
    ...
    END;
    SummaryInfo := NORMALIZE(SummaryStats,68,Into(LEFT,COUNTER));
    orb_r := RECORD
      AllErrors.Src;

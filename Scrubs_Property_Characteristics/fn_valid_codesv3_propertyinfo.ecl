import Codes,STD;
EXPORT fn_valid_codesv3_propertyinfo(string code = '', string source = '', string field_name_in = '', string file_name_in =''):= function
codes_v3 := Codes.File_Codes_V3_In(file_name = file_name_in);
string to_find := file_name_in + '|' +  field_name_in + '|' + source + '|'+ TRIM(code,left,right);

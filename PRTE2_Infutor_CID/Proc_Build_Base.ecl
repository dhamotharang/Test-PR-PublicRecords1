IMPORT PRTE2_Infutor_CID, PhonesPlus_V2, ut, PromoteSupers, PRTE2, STD, Address, AID_Support, AID, NID;

  //Project Alpharetta file into a the base layout
  AlphaBase := PROJECT(Files.Alpharetta_base_in,
                       TRANSFORM(PRTE2_infutor_cid.Layouts.Base_ext,
                                 self.cellphoneidkey := [],
...
tempLayout := record
PRTE2_infutor_cid.Layouts.Base_ext;
...
string dummy;
  ...
  pBocaGE  := PROJECT(Files.Boca_GE_in,
                       TRANSFORM(tempLayout,
                                 ...
                                 self.src_all := phonesplus_v2.translation_codes.source_bitmap_code(left.src);
                                 self.concat_address1 := std.str.CleanSpaces(trim(left.address1)+ if(left.address2 
                                                                             <> '', ', '+ trim(left.address2), ''));
                                 self := left;
                                 self := []
                                 ));
  ...
  BocaGE_prev := project(CleanGEBase(DID <> 0 and trim(origname) <> 'ORIGNAME' and link_ssn not in PRTE2_infutor_cid.constants.ssn_set_exception), PRTE2_infutor_cid.Layouts.Base_ext);
  ...
  BocaGE_xmatch := CleanGEBase(DID <> 0 and trim(cust_name) <> '' and trim(origname) <> 'ORIGNAME' and link_ssn in PRTE2_infutor_cid.constants.ssn_set_exception);
  ...
  PRTE2_infutor_cid.Layouts.Base_ext xFrmBase(dAddressCleaned L) :=
                                    TRANSFORM
                                       //Map Address
  ...
  PRTE2_infutor_cid.Layouts.Base_ext xFrmCleanName(FileClnName L) :=
                                    TRANSFORM
  ...
  PRTE2_Infutor_CID.Layouts.Base_ext AddLinkID(NewBocaBase L) :=
                                    TRANSFORM
                                        SELF.did := if(L.did > 0, L.did, prte2.fn_AppendFakeID.did(L.fname, 
                                                                                                   L.lname, 
                                                                                                   L.link_ssn, 
                                                                                                   trim(L.link_dob), 
                                                                                                   L.cust_name));                                                                                                   

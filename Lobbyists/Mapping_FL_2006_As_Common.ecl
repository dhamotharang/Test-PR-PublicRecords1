...
PLayout := record
	...
	Lobbyists.Layout_Lobbyists_FL_2006_Combined;
end;

PLayout  AddSequenceTrf(Lobbyists.Layout_Lobbyists_FL_2006_Combined l, INTEGER c) := transform
	self.seq_num   := c;
	self.group_num := 0;
	self := l;
end;

pout := project(Lobbyists.File_Lobbyists_FL_2006, AddSequenceTrf(LEFT, COUNTER));
...
Layout_Lobbyist := record
	unsigned group_num := 0;
	Lobbyists.Layout_Lobbyists_FL_2006_Rec1;
end;
...
Layout_Association := record
	unsigned group_num := 0;
	Lobbyists.Layout_Lobbyists_FL_2006_Rec2;
end;
...
export Mapping_FL_2006_As_Common := project(ds_with_lobbyist_Assoc,MyTransform(left));
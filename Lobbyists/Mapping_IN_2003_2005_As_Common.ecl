MyInitialDS := Lobbyists.File_Lobbyists_IN_2003_2005; 
...
Layout_Lobbyists_Common MyTransform(MyRolledDS input) := transform
	...
	self.Key := 'IN' + hash64(input.Name,input.Registered,input.CompleteName);
	self.Process_Date := '20050621';
	self.Source_State := 'IN';
	self.Lobby_Registration_Date := input.Registered;
...
export Mapping_IN_2003_2005_As_Common := project(MyRolledDS,MyTransform(left));
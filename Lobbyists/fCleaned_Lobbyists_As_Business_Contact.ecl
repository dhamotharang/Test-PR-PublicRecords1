...
export fCleaned_Lobbyists_As_Business_Contact(dataset(Layout_Lobbyists_Common) pLobbyists)
 :=
  function
	...
	Business_Header.Layout_Business_Contact_Full_New Translate_RA_To_BCF(Layout_Cleaned_Lobbyists_Base l, unsigned1 c) := transform
	  ...
	  self.city             := choose(c,
										l.Lobbyist_Address_Clean_v_city_name,
																		l.Lobbyist_Address_Clean_v_city_name,
																		l.Association_Address_Clean_v_city_name);
	  self.state            := choose(c,
	  ...
	  self.company_city         := choose(c,
											l.Association_Address_Clean_v_city_name,
											l.Firm_Address_Clean_v_city_name,
											l.Association_Address_Clean_v_city_name);
	...
	Cleaned_Lobbyists_Contact_Ra_Init_Filtered	:=	Cleaned_Lobbyists_Contact_Ra_Init((integer)name_score < 3,
																					  Business_Header.CheckPersonName(fname, mname,lname,name_suffix)
																					 );

	return Cleaned_Lobbyists_Contact_Ra_Init_Filtered;

...
My_Extra_Phone_Layout MyExtraPhone1Transform(Layout_Lobbyists_MO_2000_2005 InputRecord, integer PassNumber) := transform
...
MyExtraPhone1DS := normalize(File_Lobbyists_MO_2000_2005,
...
My_Extra_Phone_Layout MyExtraPhone2Transform(My_Extra_Phone_Layout InputRecord, integer PassNumber) := transform
...
Layout_Lobbyists_Common MyTransform(My_Extra_Phone_Layout InputRecord, integer PassNumber) := transform
  self.Key := 'MO' + hash64(InputRecord.Lobbyist_State_ID,InputRecord.Association_Name_Full);
  ...
  self.Lobbyist_State_ID := InputRecord.Lobbyist_State_ID;
	...
	self.Lobby_Status := if(InputRecord.Lobby_Status = 'A',
	                        'ACTIVE',
													if(InputRecord.Lobby_Status = 'D',
													   'TERMINATED',
														 ''));
	DateFinder := '^([0-9]{1,2})/([0-9]{1,2})/([0-9]{4})$';
	self.Lobby_Registration_Date := if(regexfind(DateFinder,InputRecord.Lobby_Registration_Date),
	                                   intformat((integer)regexfind(DateFinder,InputRecord.Lobby_Registration_Date,3),4,1) +
																		 intformat((integer)regexfind(DateFinder,InputRecord.Lobby_Registration_Date,1),2,1) +
																		 intformat((integer)regexfind(DateFinder,InputRecord.Lobby_Registration_Date,2),2,1),
																		 '');
	...
	self.Lobby_Legislative_Year_Start := if(regexfind(DateFinder,InputRecord.Lobby_Registration_Date),
	                                        regexfind(DateFinder,InputRecord.Lobby_Registration_Date,3),
																					'');
	self.Lobby_Legislative_Year_End := if(regexfind(DateFinder,InputRecord.Lobby_Termination_Date),
	                                      regexfind(DateFinder,InputRecord.Lobby_Termination_Date,3),
																				'');
	self := [];
end;

export Mapping_MO_2000_2005_As_Common := normalize(MyExtraPhone2DS,

...
Layout_Lobbyists_Common MyTransform(Layout_Lobbyists_MN_2006 InputRecord) 
											:= transform
	...
	self.Lobby_Registration_Date 			:= if(regexfind(DateFinder,InputRecord.Lobbyist_Registration_Date),
											intformat((integer)regexfind(DateFinder,InputRecord.Lobbyist_Registration_Date,3),4,1) +
																		 intformat((integer)regexfind(DateFinder,InputRecord.Lobbyist_Registration_Date,1),2,1) +
																		 intformat((integer)regexfind(DateFinder,InputRecord.Lobbyist_Registration_Date,2),2,1),
																		 '');
	self.Lobby_Legislative_Year_Start 		:= if(regexfind(DateFinder,InputRecord.Lobbyist_Registration_Date),
	                                        regexfind(DateFinder,InputRecord.Lobbyist_Registration_Date,3),
																					'');
	self 									:= [];
end;

export Mapping_MN_2006_As_Common 			:= project(File_Lobbyists_MN_2006,MyTransform(left));
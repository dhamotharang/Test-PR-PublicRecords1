...
Layout_Lobbyists_Common MyTransform(Layout_Lobbyists_TX_1997_2000 input) := transform
	...
	self.Key := 'TX' + '1999' + hash64(input.Lob_Name,input.Concern);
	...
	self.Lobby_Termination_Date :=  if(regexfind(DateFinder,input.termdate),
	                                   intformat((integer)regexfind(DateFinder,input.termdate,3),4,1) +
																		 ...
																		 '');               
	self.Lobby_Legislative_Year_Start := '1999';
	self.Lobby_Legislative_Year_End := '1999';
	self := [];
end;

export Mapping_TX_1999_As_Common := project(File_Lobbyists_TX_1999,MyTransform(left));
...
export Data_Layout := RECORD
  SALT27.StrType Fld { MAXLENGTH(200000) };
	UNSIGNED2 FldNo;
  END;
...
shared Pattern_Layout := record
  SALT27.StrType data_pattern {maxlength(200000)};
	unsigned cnt;
	end;
	
shared Value_Layout := record
  SALT27.StrType val  {maxlength(200000)};
	unsigned cnt;
	end;
export Field_Identification := RECORD
  unsigned2 FldNo;
  SALT27.StrType FieldName;
  END;
shared ResultLine_Layout := record(Field_Identification)
	unsigned                  Cardinality;
	SALT27.Str30Type 					MinVal30;
	SALT27.Str30Type 					MaxVal30;
...
#if (UseUnicode)
		SELF.Txt := '  '+MAP( ~le.AllNum => 'UNICODE' + IF(le.MaxLen<100,(SALT27.StrType)le.MaxLen, ''),
#else
		SELF.Txt := '  '+MAP( ~le.AllNum => 'STRING' + IF(le.MaxLen<100,(SALT27.StrType)le.MaxLen, ''),
#end
...
glbl_field_cnts := record
  FldInv1;
	unsigned2 xx_l := length(trim((SALT27.StrType)FldInv1.Fld));
	unsigned2 xx_wc := SALT27.WordCount((SALT27.StrType)FldInv1.Fld);
  end;
...
card_rec := RECORD
  FldInv.FldNo;
	...
	SALT27.Str30Type MaxVal30 := MAX(GROUP,FldInv.Fld);
	SALT27.Str30Type MinVal30 := MIN(GROUP,IF(FldInv.Fld='','ZZ--BOGUSVALUE--ZZ',FldInv.Fld));
  END;	
...
r tr(FldInv le,unsigned co) := transform
  self.c := ((SALT27.StrType)le.Fld)[co];
	self := le;
  end;
...
dr := record
	  SALT27.StrType Fld  {maxlength(200000)} := SALT27.fn_data_pattern(FldInv.Fld);
		FldInv.Cnt;
		FldInv.FldNo;
  end;
t := table(FldInv,dr);

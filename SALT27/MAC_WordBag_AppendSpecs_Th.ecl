export MAC_WordBag_AppendSpecs_Th(infile,instring,new_instring,keyname,keyfield,o,AsIndex='true') := MACRO
#uniquename(ifp)
%ifp% := record
  infile;
	unsigned tot_spec := 0;
...
%r% := record
  unsigned xxx_unc;
  unsigned2 pos;
  SALT27.Str4000Type s;
  end;
	
#uniquename(tr)
%r% %tr%(%nif% s_in, unsigned2 c) := transform
  self.pos := c;
	self.s := SALT27.GetNthWord(s_in.instring,c);
	self.xxx_unc := s_in.xxx_unc;
  end;
	
#uniquename(n)
// Split out the words for each wordbag
%n% := normalize(	%nif%, SALT27.WordCount(left.instring), %tr%(left,counter) );
#uniquename(r2)
%r2% := record(%r%)
  integer2 spec;
	end;
...
%ifp% %addw%(%ifp% le, %j% ri) := transform
  self.new_instring := trim(le.new_instring)+' '+trim(ri.s)+' '+(SALT27.StrType)ri.spec;
...
typeof(infile) %sb%(%sf% le) := transform
  self.new_instring := IF(le.instring='','',(SALT27.StrType)le.tot_spec+le.new_instring); // Newstring has space pre-pended by %addw%
  self := le;
  end;

...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Layout_in_file)
    ...
    UNSIGNED1 corp_consent_flag_for_protected_name_Invalid;
    UNSIGNED1 corp_license_type_Invalid;
...
EXPORT FromNone(DATASET(Layout_in_file) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_consent_flag_for_protected_name_Invalid := Fields.InValid_corp_consent_flag_for_protected_name((SALT34.StrType)le.corp_consent_flag_for_protected_name);
    SELF.corp_license_type_Invalid := Fields.InValid_corp_license_type((SALT34.StrType)le.corp_license_type);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_status_cd_Invalid << 22 ) + ( le.corp_status_date_Invalid << 23 ) + ( le.corp_inc_state_Invalid << 25 ) + ( le.corp_inc_date_Invalid << 26 ) + ( le.corp_term_exist_exp_Invalid << 28 ) + ( le.corp_term_exist_desc_Invalid << 29 ) + ( le.corp_foreign_domestic_ind_Invalid << 30 ) + ( le.corp_forgn_state_cd_Invalid << 31 ) + ( le.corp_forgn_state_desc_Invalid << 32 ) + ( le.corp_forgn_date_Invalid << 33 ) + ( le.corp_orig_org_structure_cd_Invalid << 35 ) + ( le.corp_for_profit_ind_Invalid << 36 ) + ( le.corp_ra_effective_date_Invalid << 37 ) + ( le.corp_agent_status_cd_Invalid << 39 ) + ( le.corp_consent_flag_for_protected_name_Invalid << 40 ) + ( le.corp_license_type_Invalid << 41 ) + ( le.corp_trademark_class_desc1_Invalid << 42 ) + ( le.recordorigin_Invalid << 43 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_status_cd_Invalid := (le.ScrubsBits1 >> 22) & 1;
    SELF.corp_status_date_Invalid := (le.ScrubsBits1 >> 23) & 3;
    ...
    SELF.corp_forgn_state_desc_Invalid := (le.ScrubsBits1 >> 32) & 1;
    SELF.corp_forgn_date_Invalid := (le.ScrubsBits1 >> 33) & 3;
    SELF.corp_orig_org_structure_cd_Invalid := (le.ScrubsBits1 >> 35) & 1;
    SELF.corp_for_profit_ind_Invalid := (le.ScrubsBits1 >> 36) & 1;
    SELF.corp_ra_effective_date_Invalid := (le.ScrubsBits1 >> 37) & 3;
    SELF.corp_agent_status_cd_Invalid := (le.ScrubsBits1 >> 39) & 1;
    SELF.corp_consent_flag_for_protected_name_Invalid := (le.ScrubsBits1 >> 40) & 1;
    SELF.corp_license_type_Invalid := (le.ScrubsBits1 >> 41) & 1;
    SELF.corp_trademark_class_desc1_Invalid := (le.ScrubsBits1 >> 42) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_consent_flag_for_protected_name_ENUM_ErrorCount := COUNT(GROUP,h.corp_consent_flag_for_protected_name_Invalid=1);
    corp_license_type_ENUM_ErrorCount := COUNT(GROUP,h.corp_license_type_Invalid=1);
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_forgn_state_desc:invalid_forgn_state_desc:ALLOW'
          ...
          ,'corp_agent_status_cd:invalid_agent_status_cd:ENUM'
          ,'corp_consent_flag_for_protected_name:invalid_flag_code:ENUM'
          ,'corp_license_type:invalid_license_type:ENUM'
          ,'corp_trademark_class_desc1:invalid_trademark_class:ALLOW'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_corp_consent_flag_for_protected_name(1)
          ,Fields.InvalidMessage_corp_license_type(1)
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_consent_flag_for_protected_name_ENUM_ErrorCount
          ,le.corp_license_type_ENUM_ErrorCount

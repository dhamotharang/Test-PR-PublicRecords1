...
shared thin_table := table(h00,{VENDOR_ID,SSN,FULLNAME,PHONE,DOB_year,DOB_month,DOB_day,ADDR1,LNAME,PRIM_NAME,PRIM_RANGE,CITY_NAME,LOCALE,FNAME,SEC_RANGE,MNAME,ST,NAME_SUFFIX,GENDER,SRC,RID,DID});// Already distributed by specificities module
...
Gordon.mac_prop_field(with_bools(SSN NOT IN SET(s.nulls_SSN,SSN)),SSN,DID,SSN_props); // For every DID find the best FULL SSN
...
shared pj1 := join(with_bools,SSN_props,left.DID=right.DID,take_SSN(left,right),local,left outer);
Gordon.mac_prop_field(with_bools(PHONE NOT IN SET(s.nulls_PHONE,PHONE)),PHONE,DID,PHONE_props); // For every DID find the best FULL PHONE
layout_withpropvars take_PHONE(with_bools le,PHONE_props ri) := transform
  ...
  self.PHONE_prop := if ( le.PHONE IN SET(s.nulls_PHONE,PHONE) and ri.PHONE NOT IN SET(s.nulls_PHONE,PHONE) and ri.DID<>(typeof(ri.DID))'', true, le.PHONE_prop );
  self := le;
  end;
shared pj3 := join(pj1,PHONE_props,left.DID=right.DID,take_PHONE(left,right),local,left outer);
Gordon.mac_prop_field(with_bools(DOB_year NOT IN SET(s.nulls_DOB_year,DOB_year)),DOB_year,DID,DOB_year_props); // For every DID find the best FULL DOB_year
...
shared pj400 := join(pj3,DOB_year_props,left.DID=right.DID,take_DOB_year(left,right),local,left outer);
Gordon.mac_prop_field(with_bools(DOB_month NOT IN SET(s.nulls_DOB_month,DOB_month)),DOB_month,DID,DOB_month_props); // For every DID find the best FULL DOB_month
...
shared pj401 := join(pj400,DOB_month_props,left.DID=right.DID,take_DOB_month(left,right),local,left outer);
Gordon.mac_prop_field(with_bools(DOB_day NOT IN SET(s.nulls_DOB_day,DOB_day)),DOB_day,DID,DOB_day_props); // For every DID find the best FULL DOB_day
...
shared pj402 := join(pj401,DOB_day_props,left.DID=right.DID,take_DOB_day(left,right),local,left outer);
Gordon.mac_prop_field_init(with_bools(FNAME NOT IN SET(s.nulls_FNAME,FNAME)),FNAME,DID,FNAME_props); // For every DID find the best FULL FNAME
...
shared pj11 := join(pj402,FNAME_props,left.DID=right.DID,take_FNAME(left,right),local,left outer);
Gordon.mac_prop_field_init(with_bools(MNAME NOT IN SET(s.nulls_MNAME,MNAME)),MNAME,DID,MNAME_props); // For every DID find the best FULL MNAME
...
shared pj13 := join(pj11,MNAME_props,left.DID=right.DID,take_MNAME(left,right),local,left outer);
Gordon.mac_prop_field(with_bools(NAME_SUFFIX NOT IN SET(s.nulls_NAME_SUFFIX,NAME_SUFFIX)),NAME_SUFFIX,DID,NAME_SUFFIX_props); // For every DID find the best FULL NAME_SUFFIX
...
shared pj15 := join(pj13,NAME_SUFFIX_props,left.DID=right.DID,take_NAME_SUFFIX(left,right),local,left outer);
shared propogated := pj15; // to allow to 'jump' over an exported value
...
j16 := join(h1,Specificities(ih).GENDER_values_persisted,left.GENDER=right.GENDER,add_GENDER(left,right),lookup,left outer);
...
j15 := join(j16,Specificities(ih).NAME_SUFFIX_values_persisted,left.NAME_SUFFIX=right.NAME_SUFFIX,add_NAME_SUFFIX(left,right),lookup,left outer);
...
j14 := join(j15,Specificities(ih).ST_values_persisted,left.ST=right.ST,add_ST(left,right),lookup,left outer);
...
j13 := join(j14,Specificities(ih).MNAME_values_persisted,left.MNAME=right.MNAME,add_MNAME(left,right),lookup,left outer);
...
j12 := join(j13,Specificities(ih).SEC_RANGE_values_persisted,left.SEC_RANGE=right.SEC_RANGE,add_SEC_RANGE(left,right),lookup,left outer);
...
Gordon.MAC_ChooseJoinType(j12,s.nulls_FNAME,Specificities(ih).FNAME_values_persisted,FNAME,FNAME_weight100,add_FNAME,j11);
...
Gordon.MAC_ChooseJoinType(j11,s.nulls_LOCALE,Specificities(ih).LOCALE_values_persisted,LOCALE,LOCALE_weight100,add_LOCALE,j10);
...
Gordon.MAC_ChooseJoinType(j10,s.nulls_CITY_NAME,Specificities(ih).CITY_NAME_values_persisted,CITY_NAME,CITY_NAME_weight100,add_CITY_NAME,j9);
...
Gordon.MAC_ChooseJoinType(j9,s.nulls_PRIM_RANGE,Specificities(ih).PRIM_RANGE_values_persisted,PRIM_RANGE,PRIM_RANGE_weight100,add_PRIM_RANGE,j8);
...
Gordon.MAC_ChooseJoinType(j8,s.nulls_PRIM_NAME,Specificities(ih).PRIM_NAME_values_persisted,PRIM_NAME,PRIM_NAME_weight100,add_PRIM_NAME,j7);
...
Gordon.MAC_ChooseJoinType(j7,s.nulls_LNAME,Specificities(ih).LNAME_values_persisted,LNAME,LNAME_weight100,add_LNAME,j6);
...
Gordon.MAC_ChooseJoinType(j6,s.nulls_ADDR1,Specificities(ih).ADDR1_values_persisted,ADDR1,ADDR1_weight100,add_ADDR1,j5);
...
j400 := join(j5,Specificities(ih).DOB_year_values_persisted,left.DOB_year=right.DOB_year,add_DOB_year(left,right),lookup,left outer);
...
j401 := join(j400,Specificities(ih).DOB_month_values_persisted,left.DOB_month=right.DOB_month,add_DOB_month(left,right),lookup,left outer);
...
j402 := join(j401,Specificities(ih).DOB_day_values_persisted,left.DOB_day=right.DOB_day,add_DOB_day(left,right),lookup,left outer);
...
Gordon.MAC_ChooseJoinType(j402,s.nulls_PHONE,Specificities(ih).PHONE_values_persisted,PHONE,PHONE_weight100,add_PHONE,j3);
...
Gordon.MAC_ChooseJoinType(j3,s.nulls_FULLNAME,Specificities(ih).FULLNAME_values_persisted,FULLNAME,FULLNAME_weight100,add_FULLNAME,j2);
...
Gordon.MAC_ChooseJoinType(j2,s.nulls_SSN,Specificities(ih).SSN_values_persisted,SSN,SSN_weight100,add_SSN,j1);
...
Gordon.MAC_ChooseJoinType(j1,s.nulls_VENDOR_ID,Specificities(ih).VENDOR_ID_values_persisted,VENDOR_ID,VENDOR_ID_weight100,add_VENDOR_ID,j0);
shared Annotated := distribute(j0,DID); // Distributed for keybuild case
//Now see if these records are actually linkable
TotalWeight := Annotated.VENDOR_ID_weight100 + Annotated.SSN_weight100 + Annotated.FULLNAME_weight100 + Annotated.PHONE_weight100 + Annotated.DOB_year_weight100 + Annotated.DOB_month_weight100 + Annotated.DOB_day_weight100 + Annotated.ADDR1_weight100 + Annotated.LOCALE_weight100 + Annotated.SEC_RANGE_weight100 + Annotated.GENDER_weight100;
shared Linkable := TotalWeight >= 5;

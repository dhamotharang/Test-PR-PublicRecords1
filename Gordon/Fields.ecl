...
export InValid_PHONE(string s) := WHICH(stringlib.stringfind('"\'',s[1],1)<>0 and stringlib.stringfind('"\'',s[length(trim(s))],1)<>0,length(trim(s))<>length(trim(stringlib.stringfilter(s,'0123456789'))));
export InValidMessage_PHONE(unsigned1 wh) := CHOOSE(wh,'String is quoted using one of:"\'','Contains characters not in:0123456789','GOOD');

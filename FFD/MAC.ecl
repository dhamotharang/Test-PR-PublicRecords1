EXPORT MAC := MODULE
	...
	EXPORT PrepareConsumerRecord(__lexId, __isESDL = TRUE, __searchby = '', __reportby_uniqueId = '') := FUNCTIONMACRO
    ...
    iesp.share_fcra.t_FcraConsumer SetConsumerRecord() := TRANSFORM
	  ...
	  END;
		...
		// return result only if there is resolved LexId
    RETURN 	IF((UNSIGNED) __lexId > 0, __outrec);	
	ENDMACRO;

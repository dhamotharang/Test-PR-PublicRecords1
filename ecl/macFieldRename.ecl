...
EXPORT macFieldRename(d,sNewPrefix,sOldPrefix='',sFieldList='',bIsDataset=FALSE):=FUNCTIONMACRO
  ...
  #FOR(fields)
    #FOR(Field)
      #IF((#TEXT(sFieldList)='' OR REGEXFIND('(,|\')'+%'{@label}'%+'(,|\')',#TEXT(sFieldList),NOCASE)) AND (#TEXT(sOldPrefix)='' OR REGEXFIND('^'+REGEXREPLACE('\'',#TEXT(sOldPrefix),'',NOCASE),%'{@label}'%,NOCASE)))
        #APPEND(f,':')
      #END
  ...
  #ELSE
      #SET(f,'{'+REGEXREPLACE(':[^.]+[.]('+REGEXREPLACE('\'',#TEXT(sOldPrefix),'', NOCASE)+')([^:;]+)',%'f'%,'TYPEOF('+#TEXT(d)+'.$1$2) '+sNewPrefix+'$2', NOCASE)+'}');
  #END

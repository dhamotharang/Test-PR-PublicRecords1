...
IMPORT SALT26b2,HealthCareProviderHeader_prod;// Gather up the UID counts from each of the children - provides 'we also found' capability
AllEfr0 := ;
m := GROUP(AllEfr0(~(LNPID = 0)),LNPID);
r := ROLLUP(m,GROUP,TRANSFORM(Ext_Layouts.EFR_Layout,SELF.Hits := PROJECT(ROWS(LEFT),Ext_Layouts.EFR_Child),SELF := LEFT));
EXPORT EFR := INDEX(r,{LNPID},{r},'~key::HealthCareProviderHeader_prod::LNPID::efr');
...
END;
EXPORT Fetch(DATASET(HealthCareProviderHeader_prod.Process_HealthCareProviderHeader_SVC_Layouts.id_stream_layout) ins,FetchParams P=FetchParams) := FUNCTION
Layout Get(DATASET(HealthCareProviderHeader_prod.Process_HealthCareProviderHeader_SVC_Layouts.id_stream_layout) ids) := TRANSFORM

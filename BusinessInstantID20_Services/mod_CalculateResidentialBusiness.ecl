...
EXPORT mod_CalculateResidentialBusiness( Business_Risk_BIP.Layouts.Shell le, BOOLEAN useSBFE = FALSE ) :=
	MODULE
		...
		EXPORT srcos                   := _srcos;
		EXPORT srcda                   := _srcda;
		EXPORT srcexclude              := _srcexclude;
		EXPORT residentialaddr         := _residentialaddr;
		EXPORT personnamessn           := _personnamessn;
		EXPORT personaddrssn           := _personaddrssn;
		EXPORT personind               := _personind;
		EXPORT ResidentialBusIndicator := _ResidentialBusIndicator;
										
		EXPORT rw_result := IF( BusinessInstantID20_Services.fn_DoesNotMeetMinInputBusinessReqs( le ), rw_null_ResidentialBusiness, rw_ResidentialBusiness );

...
Email_Data.Layout_Email.Base t_map_to_common (domain_d input) := transform
	...
	self.orig_First_Name  					:= ut.CleanSpacesAndUpper(TRIM(input.First_Name,RIGHT,LEFT));
	self.orig_Last_Name  						:= ut.CleanSpacesAndUpper(TRIM(input.Last_Name,RIGHT,LEFT));
	self.Orig_Address  							:= ut.CleanSpacesAndUpper(TRIM(StringLib.StringCleanSpaces(input.address_1 + ' ' + input.address_2), LEFT, RIGHT));

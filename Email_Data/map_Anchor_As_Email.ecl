IMPORT Anchor, address, ut, emailservice, mdr, _validate, Entiera, lib_StringLib, std;

EXPORT map_Anchor_As_Email(version) := FUNCTION

with_email := Anchor.Files.Base_out(length(trim(EmailAddress,LEFT,RIGHT)) > 4 and STD.Str.FindCount(EmailAddress,  '@') > 0 and current_rec = TRUE);
...
Email_Data.Layout_Email.Base t_map_to_common (domain_d input) := TRANSFORM
	...
	self.rec_src_all      					:= translation_codes.source_bitmap_code(mdr.sourceTools.src_Anchor);
	self.email_src_all    					:= translation_codes.source_bitmap_code(mdr.sourceTools.src_Anchor);
	...
	self.Orig_Address  							:= IF(TRIM(input.Address_2,LEFT,RIGHT) = '', TRIM(input.Address_1,LEFT,RIGHT),
																				TRIM(input.Address_1,LEFT,RIGHT) + ' '+TRIM(input.Address_2,LEFT,RIGHT));
	self.Orig_City  								:= input.city;
	self.Orig_State  								:= input.state;
	self.orig_ZIP  									:= input.ZipCode;
	self.orig_zip4  								:= '';
	self.orig_email 								:= TRIM(input.EmailAddress,LEFT,RIGHT);
	self.orig_ip  									:= TRIM(input.IPAddress,LEFT,RIGHT);
	self.orig_site  								:= TRIM(input.SourceURL,LEFT,RIGHT);
	self.orig_e360_id  							:= '';

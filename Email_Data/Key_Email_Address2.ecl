...
emailAddr_ready  := dedp_emailAddr(TRIM(email_src) <> 'IB');	
//enter the first part of emailaddr or enter the full email addr get the rest of the record back				   
export Key_Email_Address2 := 
index(emailAddr_ready
      ,{clean_email}
			,{dedp_emailAddr}
			,Data_Services.Data_location.Prefix('Email_Data')+'thor_200::key::email_data::'+doxie.Version_SuperKey+'::email_addresses2');

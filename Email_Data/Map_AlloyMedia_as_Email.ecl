...
Email_Data.Layout_Email.Base t_map_to_common (domain_d input) := transform
	...
	self.rec_src_all      					:= translation_codes.source_bitmap_code(mdr.sourceTools.src_alloymedia_consumer);
	self.email_src_all    					:= translation_codes.source_bitmap_code(mdr.sourceTools.src_alloymedia_consumer);
...
return t_mappend_f(append_email_username <> '' and StringLib.StringFindCount(clean_email,  '.') > 0 and ~Entiera.fn_profanity(clean_email));
end;
...
<xsl:template match="/child::node()/Results/Result/Dataset[@name = 'CRS_result']/Row/property_children">
  ...
  <H2>Active or Previous Address(es) - Property Info</H2>
    ...
    <!-- Notice of Defaults (iesp format)-->
<xsl:template match="/child::node()/Results/Result/Dataset[@name = 'CRS_result']/Row/nod_children">
  <h2>Notice of Defaults</h2>
  <xsl:for-each select="Row">
      <b>ForeclosureID:</b>&space;<xsl:value-of select="ForeclosureId"/>&cr;
      <xsl:if test="RecordingDate/Year>0">
        RecordingDate:&space;<xsl:value-of select="RecordingDate/Year"/>/<xsl:value-of select="format-number(RecordingDate/Month,'00')"/>/<xsl:value-of select="format-number(RecordingDate/Day,'00')"/>&cr;
      </xsl:if>
      DeedType:&space;<xsl:value-of select="DeedType"/>&cr;
      <xsl:if test="normalize-space(DocumentType)!=''">
        DocumentType:&space;<xsl:value-of select="DocumentType"/>&cr;
      </xsl:if>
      SiteAddress:&space;<xsl:value-of select="SiteAddress/StreetNumber"/>&space;
                         <xsl:value-of select="SiteAddress/StreetPreDirection"/>&space;
                         <xsl:value-of select="SiteAddress/StreetName"/>&space;
                         <xsl:value-of select="SiteAddress/StreetSuffix"/>&space;
                         <xsl:value-of select="SiteAddress/StreetPostDirection"/>&space;
                         <xsl:value-of select="SiteAddress/UnitDesignation"/>&space;
                         <xsl:value-of select="SiteAddress/UnitNumber"/>,&space;
                         <xsl:value-of select="SiteAddress/City"/>&space;
                         <xsl:value-of select="SiteAddress/State"/>,&space;
                         <xsl:value-of select="SiteAddress/Zip5"/>-<xsl:value-of select="SiteAddress/Zip4"/>&space;
                         <xsl:value-of select="SiteAddress/StreetAddress1"/>&space;
                         <xsl:value-of select="SiteAddress/StreetAddress2"/>&space;
                         <xsl:value-of select="SiteAddress/PostalCode"/>&space;
                         <xsl:value-of select="SiteAddress/StateCityZip"/>&space;
                         County:&space;<xsl:value-of select="SiteAddress/County"/>&cr;
      Site2Address:&space;<xsl:value-of select="Site2Address/StreetNumber"/>&space;
                         <xsl:value-of select="Site2Address/StreetPreDirection"/>&space;
                         <xsl:value-of select="Site2Address/StreetName"/>&space;
                         <xsl:value-of select="Site2Address/StreetSuffix"/>&space;
                         <xsl:value-of select="Site2Address/StreetPostDirection"/>&space;
                         <xsl:value-of select="Site2Address/UnitDesignation"/>&space;
                         <xsl:value-of select="Site2Address/UnitNumber"/>,&space;
                         <xsl:value-of select="Site2Address/City"/>&space;
                         <xsl:value-of select="Site2Address/State"/>,&space;
                         <xsl:value-of select="Site2Address/Zip5"/>-<xsl:value-of select="Site2Address/Zip4"/>&space;
                         <xsl:value-of select="Site2Address/StreetAddress1"/>&space;
                         <xsl:value-of select="Site2Address/StreetAddress2"/>&space;
                         <xsl:value-of select="Site2Address/PostalCode"/>&space;
                         <xsl:value-of select="Site2Address/StateCityZip"/>&space;
                         County:&space;<xsl:value-of select="Site2Address/County"/>&cr;
      <xsl:if test="normalize-space(Plaintiffs/Plaintiff)!=''">
        <b>Plaintiffs:</b>&cr;
        <xsl:for-each select="Plaintiffs">
          <xsl:number value="position()" format="1" />)&space;
          <xsl:value-of select="Plaintiff"/>&cr;
        </xsl:for-each>
      </xsl:if>
      <b>Defendants:</b>&cr;
      <xsl:for-each select="Defendants/Defendant">
        <xsl:number value="position()" format="1" />)&space;
        ID:&space;<xsl:value-of select="UniqueId"/>&space;
        <xsl:choose>
          <xsl:when test="normalize-space(CompanyName)!=''">
            CompanyName:&space;<xsl:value-of select="CompanyName"/>&space;
          </xsl:when>
          <xsl:otherwise>
            Name:&space;<xsl:value-of select="Name/Full"/>&space;
                        <xsl:value-of select="Name/Prefix"/>&space;
                        <xsl:value-of select="Name/First"/>&space;
                        <xsl:value-of select="Name/Middle"/>&space;
                        <xsl:value-of select="Name/Last"/>&space;
                        <xsl:value-of select="Name/Suffix"/>&space;
            <xsl:if test="normalize-space(SSN)!=''">
              -&space;SSN:&space;<xsl:value-of select="SSN"/>&space;
            </xsl:if>
          </xsl:otherwise>
        </xsl:choose>&cr;
    </xsl:for-each>&cr;
...
<xsl:template match="/child::node()/Results/Result/Dataset[@name = 'CRS_result']/Row/corporate_affiliations_children">
	...
	<!-- Drivers at Address -->
<xsl:template match="/child::node()/Results/Result/Dataset[@name = 'CRS_result']/Row/driversataddress_children">
	<h2>Drivers At Address:</h2>
	<xsl:for-each select="Row">
		<b>License:</b>&cr;
		<xsl:call-template name="name_dob"/>
		<xsl:call-template name="name_address"/>
		Gender: <xsl:value-of select="sex_name"/>&cr;
		DL: <xsl:value-of select="dl_number"/>&cr;
		DL State: <xsl:value-of select="orig_state"/>&cr;
		Expiration Date: <xsl:value-of select="expiration_date"/>&cr;
     </xsl:for-each>
</xsl:template>

	<!-- Patriot Search -->
<xsl:template match="/child::node()/Results/Result/Dataset[@name = 'CRS_result']/Row/patriotsearch_children">
	<h2>Patriot Search:</h2>
	<xsl:for-each select="Row">
		partyName:  <xsl:value-of select="PartyName"/>&cr;
		Source: <xsl:value-of select="DataSource"/>&cr;
		AKA:
  	<xsl:for-each select="AKAs/AKA">
	  	<xsl:value-of select="PartyName"/>&cr;
    </xsl:for-each>
		Remarks: 
	...
	<!-- FBN Search -->
<xsl:template match="/child::node()/Results/Result/Dataset[@name = 'CRS_result']/Row/fbnsearch_children">
	<h2>FBN Search:</h2>
	<xsl:for-each select="Row">
		   Filing Number      <xsl:value-of select="FilingNumber"/>&cr;
		   Business Name      <xsl:value-of select="Business/Name"/>&cr;
  	<xsl:for-each select="Owners/Owner">
			 Owners							<xsl:value-of select="Name"/>&cr;
     </xsl:for-each>
     </xsl:for-each>
</xsl:template>

<xsl:template match="/child::node()/Results/Result/Dataset[@name = 'CRS_result']/Row/realtime_vehicle_children">
  <H2>Real-Time Motor Vehicles :</H2>
  <xsl:for-each select="Row">
	<b>Vehicles ( polk gateway) </b>:&cr;
		   vin		    <xsl:value-of select="VID"/>&cr;
		   Make       <xsl:value-of select="Make"/>&cr;
		   Model      <xsl:value-of select="Model"/>&cr;
     <xsl:for-each select="Registrants/Registrant/PersonInfo">
		   Name       <xsl:value-of select="Name/First"/>&cr;
     </xsl:for-each>
		 &cr;
     </xsl:for-each>
</xsl:template>

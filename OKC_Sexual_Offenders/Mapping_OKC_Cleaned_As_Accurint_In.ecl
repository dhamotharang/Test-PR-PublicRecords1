...
//In_Base_file := OKC_Sexual_Offenders.Cleaned_OKC_Sex_Offender(name_type <> '2');
In_Base_file := OKC_Sexual_Offenders.File_OKC_Sex_Offender_DID(name_type = '0');
...
//ds_Sorted_AccurintFile := sort(dsAccurint_In, seisint_primary_key, orig_state, name_orig);
...
Lookup_file := sort(Okc_sexual_offenders.File_OKC_Pics_Lookup(trim(image_file_name,left,right) <> ''), state_of_origin, image_file_name);
...
Clean_ds := join(ds_Sorted_AccurintFile, Lookup_file, trim(left.orig_state,left,right) = stringlib.StringToUpperCase(trim(right.state_of_origin,left,right)) and
																											trim(left.image_link,left,right) = trim(right.image_file_name,left,right), 
...
output(ds_AccurintMoxieFile,,'~thor_200::base::OKC_Sex_Offender_File_Accurint_In_Moxie',overwrite);

/////////////////////////////////////////////////////////////////////////////////////////////

export Mapping_OKC_Cleaned_As_Accurint_In := output(sort_clean_ds,,'~thor_200::base::OKC_Sex_Offender_File_Accurint_In',overwrite);
//output(ds_Sorted_AccurintFile,,'~thor_data400::base::OKC_Sex_Offender_File_Accurint_In',overwrite);
//export Mapping_OKC_Cleaned_As_Accurint_In := output(ds_Sorted_AccurintFile,,OKC_Sexual_Offenders.Cluster + 'base::OKC_Sex_Offender_File_Accurint_In',overwrite);
...
<message name="CarrierSearchService" wuTimeout="300000">
  ...
  <part name="HealthcareAccidentIDSearchRequest" type="tns:XmlDataSet" cols="80" rows="30" />
</message>
*/
...
export AccidentIDService :=  MACRO
  #constant('SearchLibraryVersion', AutoheaderV2.Constants.LibVersion.LEGACY);
    ...
    ds_in     := dataset([],iesp.healthcareaccidentidsearch.t_HealthcareAccidentIDSearchRequest) : stored ('HealthcareAccidentIDSearchRequest', FEW);
    ...
    search_by 			 := global(first_row.searchBy);
		...
		//Clean Input:
		iesp.bpsreport.t_BpsReportBy xformCleanInput(iesp.healthcareaccidentidsearch.t_HealthcareAccidentIDSearchBy aInputData) := transform
				/* --- Name Information ---*/
    ...
    tempmodvin := module(project(AutoStandardI.GlobalModule(), 
		...
		final_recs := MAP (~isValidOrder => DATASET ([],iesp.healthcaresubrogationsearch.t_HealthcareSubrogationSearchRespData), isValidOrder AND isNoHit => recsvin,recs);

		string8 Date_of_Loss := if(final_recs[1].incident.DateOfLoss.year>0,(string)final_recs[1].incident.DateOfLoss.Year+(intformat(final_recs[1].incident.DateOfLoss.Month,2,1))+(intformat(final_recs[1].incident.DateOfLoss.Day,2,1)),'');			
		...
		apply_restriction_ds := filter_recs (DataSource in HealthCareSubrogation.Constants.Non_CRU_DataSource AND Incident.State NOT IN  HealthCareSubrogation.Constants.Restricted_States);
    ...
    iesp.healthcareaccidentidsearch.t_HealthcareAccidentIDSearchResponse marshall() := transform	
      ...
      self.RecordCount               := count(carrier_service_resp);
			...
			self.did := (string)final_recs[1].driver.DID;
		end;

...
EXPORT fieldStats_IDcounts_HS(

   dataset(BIPV2.CommonBase.layout ) pInfile  = BIPV2.CommonBase.DS_CLEAN,
	 dataset(BIPV2.CommonBase.layout ) oldF=BIPV2.CommonBase.DS_FATHER
) := module
...
old0 := oldF;
//old0 := project(bipv2.CommonBase.DS_FATHER_STATIC_CLEAN,transform(bipv2.CommonBase.layout,self := left,self := []));//change to static so it works when we change the layout, but project to new layout//the old header is the one in the superfile this time.  the new one is just now building cuz we are inside the build.
new := pInfile;

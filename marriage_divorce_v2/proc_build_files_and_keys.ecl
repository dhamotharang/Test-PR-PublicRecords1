...
marriage_divorce_v2.layout_mar_div_intermediate OffTerms(concat_them1 L) := Transform
		Self.Party1_Race	:= 	If(L.Party1_Race in ['NEGRO'], '', L.Party1_Race);
		Self.Party2_Race	:= 	If(L.Party2_Race in ['NEGRO'], '', L.Party2_Race);
		Self := L;
End;

Concat_them := Project(concat_them1, OffTerms(Left));

PromoteSupers.MAC_SF_BuildProcess(marriage_divorce_v2.proc_clean_and_did(concat_them).base_file_intermediate,'~thor_data400::base::mar_div::intermediate',build_intermediate,3,,true);
PromoteSupers.MAC_SF_BuildProcess(marriage_divorce_v2.proc_clean_and_did(concat_them).search_file           ,'~thor_data400::base::mar_div::search'      ,build_search,3,,true);

...
SHARED FldIds := DATASET([{1,'did'}
      ...
      ,{44,'address_ind'}
      ,{45,'name_ind'}
      ,{46,'persistent_record_id'}],SALT311.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_vendor_id((SALT311.StrType)le.vendor_id),
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_Headers_Monthly, Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

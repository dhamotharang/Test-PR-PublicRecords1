/* *****************************************************************************************************************
PRTE2_Liens_Ins_DataPrep.BWR_Add_Equifax
filtering by hit_no_hit='1' to remove any no hits
...
CSVName := 'Equifax_CTEST_40K_Test_Files_BLAH_02_2015_subset.csv';
...
speadsheetIncoming := PROJECT(SORT(Files.Spray_EQ_DS(hit_no_hit='1'),state),
															TRANSFORM(Layouts.New_EQ_Layout,
															SELF.JoinInt := COUNTER, 
															SELF.AddressLine1 := AppendIF4(LEFT.Street_Number,LEFT.Street_Name,LEFT.Street_Suffix,''),
															SELF := LEFT));
howMany := 10;																		
...
PromoteSupers.Mac_SF_BuildProcess(speadsheetIncoming, Files.Save_EQ_Name, buildBase1,2);
PromoteSupers.Mac_SF_BuildProcess(StateSamples, Files.TMP_EQ_Name, buildBase2,2);
																			 
// --------------------------------------------------
delSprayedFile  := FileServices.DeleteLogicalFile (Files.Spray_EQ_Name);
// --------------------------------------------------
BaseBuiltDS := Files.Save_EQ_DS;

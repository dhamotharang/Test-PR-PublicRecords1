...
// ------------------------------------------------------------------------------------------------------
ReadOnly_Prod_SF_MainName	:= '~thor_data400::base::liensv2::main::all_sources';
ReadOnly_Prod_SF_Main_DS		:= DATASET(ReadOnly_Prod_SF_MainName,Prod_Main_Layout,THOR);
...
Analy_name := Data_Services.foreign_dataland+'prct::TMP::ct_Gather::LiensV2_Alpha::Analytics_Main_2';
Analy_DS			:= DATASET(Analy_name, BaseAnaly_Joinable, THOR);

Analy_Set := SET(Analy_DS,tmsid);
OUTPUT(Analy_set);
OUTPUT(COUNT(Analy_set));
// ------------------------------------------------------------------------------------------------------
OurRecs1 := ReadOnly_Prod_SF_Main_DS(tmsid <>'' AND tmsid in Analy_Set);
OurRecs := DEDUP(SORT(OurRecs1,tmsid,rmsid),tmsid,rmsid);

// OUTPUT(OurRecs,NAMED('OurRecs'));
Tmp_Main_Prod_File0 := '~prct::TMP::ct_Gather::LiensV2_Alpha::Prod_Main0';
...
OUTPUT(OurRecs,,Tmp_Main_Prod_File0,overwrite);

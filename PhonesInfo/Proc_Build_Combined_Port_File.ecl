...
EXPORT Proc_Build_Combined_Port_File(string version, const varstring eclsourceip, string thor_name):= function
	...
	concatiCRawHistory		:= output(dedup(sort(distribute(ds_telo_daily + PhonesInfo.File_Port.In_Port_Daily_History, hash(tn)), record, local), record, local),,'~thor_data400::in::phones::port_daily_history_'+version, __compressed__);
	
	moveiCRawHistory			:= STD.File.PromoteSuperFileList(['~thor_data400::in::phones::port_daily_history',

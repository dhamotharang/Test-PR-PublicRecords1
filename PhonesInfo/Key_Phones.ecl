...
import Data_Services, doxie;
	...
	export Ported 					:= index(inFCurrP
																					,{phone}
																					,{inFCurrP}
																					,/*Data_Services.Data_location.Prefix('NONAMEGIVEN')+*/'~thor_data400::key::phones_ported_'+doxie.Version_SuperKey);
	...
	export Ported_Father 		:= index(inFPrevP
																					,{phone}
																					,{inFPrevP}
																					,/*Data_Services.Data_location.Prefix('NONAMEGIVEN')+*/'~thor_data400::key::phones_ported_father');

	inFPL 									:= PhonesInfo.File_Metadata.PortedMetadata_Main;
	export Ported_Metadata	:= index(inFPL
																				,{phone}
																				,{inFPL}
																				,/*Data_Services.Data_location.Prefix('NONAMEGIVEN')+*/'~thor_data400::key::phones_ported_metadata_'+doxie.Version_SuperKey);

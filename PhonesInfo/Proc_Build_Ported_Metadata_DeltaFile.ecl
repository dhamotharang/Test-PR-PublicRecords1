...
EXPORT Proc_Build_Ported_Metadata_DeltaFile(string version, string filename, string newDay, const varstring eclsourceip, string thor_name):= function

	//Spray Raw Delta Files and Place Into DailyDelta Superfile
	sprayDailyDelta 	:= PhonesInfo.Spray_iConectiv_DailyDelta(version, filename,eclsourceip,thor_name);
	
	//Build DeltaBaseFile, Using the Raw Delta Files
	buildBaseDelta		:= output(PhonesInfo.Map_Ported_Metadata_DeltaFile(version),,'~thor_data400::base::phones::ported_metadata_deltamain_'+version, csv(heading(1), terminator('\n'), separator('\t')), overwrite, __compressed__);

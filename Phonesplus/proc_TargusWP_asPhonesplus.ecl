...
//Filter on DateVendorLastReported						
...
matchCurrDisconn t_isCurrPhone(f_matchCurrDisconn L ,f_currGong R) := transform
self := L;
end;					
...
self.InitScore		:= if(L.InitScore = 0,6,L.InitScore);
self.InitScoreType	:= if(L.InitScoreType = '', 'TARGUS OTHER',L.InitScoreType);
self := L;
end;					
...
export proc_TargusWP_asPhonesplus := ActiveDiffName : PERSIST('~thor400_30::persist::targus_asphonesplus');

IMPORT SALT311,STD;
EXPORT Outfile_Delta(DATASET(Outfile_Layout_IDA)old_s, DATASET(Outfile_Layout_IDA) new_s) := MODULE//Routines to compute the differences between two instances of a file
  ...
  EXPORT DifferenceSummary := Outfile_hygiene(old_s).Summary('Old') + Outfile_hygiene(new_s).Summary('New') + Outfile_hygiene(PROJECT(Differences(deleted), TRANSFORM(Outfile_Layout_IDA, SELF := LEFT.old_rec))).Summary('Deletions') + Outfile_hygiene(PROJECT(Differences(added), TRANSFORM(Outfile_Layout_IDA, SELF := LEFT.new_rec))).Summary('Additions');
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE) := FUNCTION
    myTimeStamp := (UNSIGNED6)SALT311.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
    hygieneDiffOverall := DifferenceSummary;
    SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_IDA, Outfile_Fields, 'RECORDOF(hygieneDiffOverall)', FALSE);

...
EXPORT Homephone_High_Fields := MODULE

EXPORT NumFields := 20;

// Processing for each FieldType
EXPORT SALT311.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'Invalid_Num','Invalid_Alpha','Invalid_AlphaNum','Invalid_FName','Invalid_MName','Invalid_LName','Invalid_Suffix','Invalid_Address1','Invalid_Address2','Invalid_City','Invalid_State','Invalid_Zip','Invalid_DOB','Invalid_SSN','Invalid_DL','Invalid_Phone','Invalid_Phone2','Invalid_Phone3','Invalid_Clientassigneduniquerecordid','Invalid_Emailaddress','Invalid_Ipaddress');
EXPORT FieldTypeNum(SALT311.StrType fn) := CASE(fn,'Invalid_Num' => 1,'Invalid_Alpha' => 2,'Invalid_AlphaNum' => 3,'Invalid_FName' => 4,'Invalid_MName' => 5,'Invalid_LName' => 6,'Invalid_Suffix' => 7,'Invalid_Address1' => 8,'Invalid_Address2' => 9,'Invalid_City' => 10,'Invalid_State' => 11,'Invalid_Zip' => 12,'Invalid_DOB' => 13,'Invalid_SSN' => 14,'Invalid_DL' => 15,'Invalid_Phone' => 16,'Invalid_Phone2' => 17,'Invalid_Phone3' => 18,'Invalid_Clientassigneduniquerecordid' => 19,'Invalid_Emailaddress' => 20,'Invalid_Ipaddress' => 21,0);
...
EXPORT MakeFT_Invalid_MName(SALT311.StrType s0) := FUNCTION
  ...
  RETURN  MakeFT_Invalid_Alpha(s2);
END;
EXPORT InValidFT_Invalid_MName(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZ. '))),~(LENGTH(TRIM(s)) = 0 OR LENGTH(TRIM(s)) = 1 OR LENGTH(TRIM(s)) = 2),~(SALT311.WordCount(SALT311.StringSubstituteOut(s,'. ',' ')) = 0 OR SALT311.WordCount(SALT311.StringSubstituteOut(s,'. ',' ')) = 1));
EXPORT InValidMessageFT_Invalid_MName(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZ. '),SALT311.HygieneErrors.NotLength('0,1,2'),SALT311.HygieneErrors.NotWords('0,1'),SALT311.HygieneErrors.Good);
...
EXPORT MakeFT_Invalid_LName(SALT311.StrType s0) := FUNCTION
  ...
  RETURN  MakeFT_Invalid_Alpha(s2);
END;
EXPORT InValidFT_Invalid_LName(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZ '))),~(LENGTH(TRIM(s)) >= 2 AND LENGTH(TRIM(s)) <= 20),~(SALT311.WordCount(SALT311.StringSubstituteOut(s,' ',' ')) = 0 OR SALT311.WordCount(SALT311.StringSubstituteOut(s,' ',' ')) = 1));
EXPORT InValidMessageFT_Invalid_LName(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZ '),SALT311.HygieneErrors.NotLength('2..20'),SALT311.HygieneErrors.NotWords('0,1'),SALT311.HygieneErrors.Good);

EXPORT MakeFT_Invalid_Suffix(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'.123IVXJRSr '); // Only allow valid symbols
  s2 := SALT311.stringcleanspaces( SALT311.stringsubstituteout(s1,' ',' ') ); // Insert spaces but avoid doubles
  RETURN  s2;
END;
EXPORT InValidFT_Invalid_Suffix(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'.123IVXJRSr '))),~(LENGTH(TRIM(s)) = 0 OR LENGTH(TRIM(s)) = 1 OR LENGTH(TRIM(s)) = 2 OR LENGTH(TRIM(s)) = 3),~(SALT311.WordCount(SALT311.StringSubstituteOut(s,' ',' ')) = 0 OR SALT311.WordCount(SALT311.StringSubstituteOut(s,' ',' ')) = 1));
EXPORT InValidMessageFT_Invalid_Suffix(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('.123IVXJRSr '),SALT311.HygieneErrors.NotLength('0,1,2,3'),SALT311.HygieneErrors.NotWords('0,1'),SALT311.HygieneErrors.Good);
...
END;
EXPORT InValidFT_Invalid_Zip(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789'))),~(LENGTH(TRIM(s)) = 0 OR LENGTH(TRIM(s)) = 4 OR LENGTH(TRIM(s)) = 5 OR LENGTH(TRIM(s)) = 9));
EXPORT InValidMessageFT_Invalid_Zip(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789'),SALT311.HygieneErrors.NotLength('0,4,5,9'),SALT311.HygieneErrors.Good);
...
EXPORT MakeFT_Invalid_DL(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_Invalid_DL(SALT311.StrType s) := WHICH(~Scrubs.Functions.FN_valid_DL(s)>0);
EXPORT InValidMessageFT_Invalid_DL(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.CustomFail('Scrubs.Functions.FN_valid_DL'),SALT311.HygieneErrors.Good);
...
EXPORT MakeFT_Invalid_Phone2(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'0123456789X'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_Phone2(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789X'))),~(LENGTH(TRIM(s)) = 0 OR LENGTH(TRIM(s)) = 7 OR LENGTH(TRIM(s)) = 10));
EXPORT InValidMessageFT_Invalid_Phone2(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789X'),SALT311.HygieneErrors.NotLength('0,7,10'),SALT311.HygieneErrors.Good);

EXPORT MakeFT_Invalid_Phone3(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_Invalid_Phone3(SALT311.StrType s) := WHICH(~Scrubs.Functions.fn_Valid_Phone(s)>0);
EXPORT InValidMessageFT_Invalid_Phone3(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.CustomFail('Scrubs.Functions.fn_Valid_Phone'),SALT311.HygieneErrors.Good);

EXPORT MakeFT_Invalid_Clientassigneduniquerecordid(SALT311.StrType s0) := FUNCTION
  s1 := SALT311.stringfilter(s0,'0123456789'); // Only allow valid symbols
  RETURN  MakeFT_Invalid_Num(s1);
END;
EXPORT InValidFT_Invalid_Clientassigneduniquerecordid(SALT311.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT311.StringFilter(s,'0123456789'))));
EXPORT InValidMessageFT_Invalid_Clientassigneduniquerecordid(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInChars('0123456789'),SALT311.HygieneErrors.Good);
...
EXPORT SALT311.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'firstname','middlename','lastname','suffix','addressline1','addressline2','city','state','zip','dob','ssn','dl','dlstate','phone','phone2','phone3','clientassigneduniquerecordid','emailaddress','ipaddress','filecategory');
EXPORT SALT311.StrType FlatName(UNSIGNED2 i) := CHOOSE(i,'firstname','middlename','lastname','suffix','addressline1','addressline2','city','state','zip','dob','ssn','dl','dlstate','phone','phone2','phone3','clientassigneduniquerecordid','emailaddress','ipaddress','filecategory');
EXPORT FieldNum(SALT311.StrType fn) := CASE(fn,'firstname' => 0,'middlename' => 1,'lastname' => 2,'suffix' => 3,'addressline1' => 4,'addressline2' => 5,'city' => 6,'state' => 7,'zip' => 8,'dob' => 9,'ssn' => 10,'dl' => 11,'dlstate' => 12,'phone' => 13,'phone2' => 14,'phone3' => 15,'clientassigneduniquerecordid' => 16,'emailaddress' => 17,'ipaddress' => 18,'filecategory' => 19,0);
EXPORT SET OF SALT311.StrType FieldRules(UNSIGNED2 i) := CHOOSE(i,['ALLOW','LENGTHS','WORDS'],['ALLOW','LENGTHS','WORDS'],['ALLOW','LENGTHS','WORDS'],['ALLOW','LENGTHS','WORDS'],['ALLOW','WORDS'],['ALLOW','WORDS'],['ALLOW','WORDS'],['CUSTOM'],['ALLOW','LENGTHS'],['CUSTOM'],['ALLOW','LENGTHS','WORDS'],['CUSTOM'],['CUSTOM'],['ALLOW','LENGTHS'],['ALLOW','LENGTHS'],['CUSTOM'],['ALLOW'],['CUSTOM'],['CUSTOM'],[],[]);
...
EXPORT Make_phone2(SALT311.StrType s0) := MakeFT_Invalid_Phone2(s0);
EXPORT InValid_phone2(SALT311.StrType s) := InValidFT_Invalid_Phone2(s);
EXPORT InValidMessage_phone2(UNSIGNED1 wh) := InValidMessageFT_Invalid_Phone2(wh);


EXPORT Make_phone3(SALT311.StrType s0) := MakeFT_Invalid_Phone3(s0);
EXPORT InValid_phone3(SALT311.StrType s) := InValidFT_Invalid_Phone3(s);
EXPORT InValidMessage_phone3(UNSIGNED1 wh) := InValidMessageFT_Invalid_Phone3(wh);
...
EXPORT Make_filecategory(SALT311.StrType s0) := s0;
EXPORT InValid_filecategory(SALT311.StrType s) := 0;
EXPORT InValidMessage_filecategory(UNSIGNED1 wh) := '';
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Diff_filecategory := le.filecategory <> ri.filecategory;
    SELF.Val := (SALT311.StrType)evaluate(le,pivot_exp);
    SELF.Num_Diffs := 0+ IF( SELF.Diff_firstname,1,0)+ IF( SELF.Diff_middlename,1,0)+ IF( SELF.Diff_lastname,1,0)+ IF( SELF.Diff_suffix,1,0)+ IF( SELF.Diff_addressline1,1,0)+ IF( SELF.Diff_addressline2,1,0)+ IF( SELF.Diff_city,1,0)+ IF( SELF.Diff_state,1,0)+ IF( SELF.Diff_zip,1,0)+ IF( SELF.Diff_dob,1,0)+ IF( SELF.Diff_ssn,1,0)+ IF( SELF.Diff_dl,1,0)+ IF( SELF.Diff_dlstate,1,0)+ IF( SELF.Diff_phone,1,0)+ IF( SELF.Diff_phone2,1,0)+ IF( SELF.Diff_phone3,1,0)+ IF( SELF.Diff_clientassigneduniquerecordid,1,0)+ IF( SELF.Diff_emailaddress,1,0)+ IF( SELF.Diff_ipaddress,1,0)+ IF( SELF.Diff_filecategory,1,0);
  END;
...
#uniquename(AggRec)
  %AggRec% := RECORD
    ...
    Count_Diff_filecategory := COUNT(GROUP,%Closest%.Diff_filecategory);
  END;

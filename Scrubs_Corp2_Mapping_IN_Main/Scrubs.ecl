...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_fed_tax_id_Invalid := (le.ScrubsBits1 >> 31) & 3;
    SELF.corp_term_exist_cd_Invalid := (le.ScrubsBits1 >> 33) & 1;
    SELF.corp_term_exist_exp_Invalid := (le.ScrubsBits1 >> 34) & 3;
    SELF.corp_term_exist_desc_Invalid := (le.ScrubsBits1 >> 36) & 1;
    SELF.corp_foreign_domestic_ind_Invalid := (le.ScrubsBits1 >> 37) & 1;
    SELF.corp_forgn_state_cd_Invalid := (le.ScrubsBits1 >> 38) & 1;
    SELF.corp_forgn_state_desc_Invalid := (le.ScrubsBits1 >> 39) & 1;
    SELF.corp_forgn_date_Invalid := (le.ScrubsBits1 >> 40) & 3;
    SELF.corp_orig_org_structure_cd_Invalid := (le.ScrubsBits1 >> 42) & 1;
    SELF.corp_orig_org_structure_desc_Invalid := (le.ScrubsBits1 >> 43) & 1;
    SELF.corp_for_profit_ind_Invalid := (le.ScrubsBits1 >> 44) & 1;
    SELF.corp_acts_Invalid := (le.ScrubsBits1 >> 45) & 1;
    SELF.cont_title1_desc_Invalid := (le.ScrubsBits1 >> 46) & 1;
    SELF.corp_llc_managed_ind_Invalid := (le.ScrubsBits1 >> 47) & 1;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_supp_key:invalid_supp_key:ALLOW'

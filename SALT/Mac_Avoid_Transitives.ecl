export mac_avoid_transitives(ifile,did1,did2,conf,dover,rule_num,ofile) := macro
 ...
 output(choosen(%add_reverse%,100));
 
 #uniquename(only_top_conf)
 %only_top_conf% := rollup(sort(distribute(%add_reverse%,hash(did1)),did1,-conf,did2,rule_num,local),left.did1=right.did1 and left.conf>right.conf,transform(left),local);
 
 output(choosen(%only_top_conf%,100));

...
#uniquename(intoval)
salt.mac_character_counts.Value_Layout %intoval%(%glbl_flds_cnt% le) := transform
...
#uniquename(rcp)
salt.mac_character_counts.character_layout %rcp%(%rc% le) := transform
  self := le;
  end;
#uniquename(o_cc)
...
#uniquename(rcl)
salt.mac_character_counts.length_layout %rcl%(%rl% le) := transform
  self := le;
  end;
#uniquename(o_sl)
...
#uniquename(rcw)
salt.mac_character_counts.words_layout %rcw%(%rw% le) := transform
  self := le;
  end;
#uniquename(o_wc)
%o_wc% := project(sort(table(%t%,%rw%,xx_wc),-cnt),%rcw%(left));
#uniquename(dpo)
salt.mac_find_data_patterns(%t%,infield,%dpo%)
#uniquename(rcd)
salt.mac_character_counts.pattern_layout %rcd%(%dpo% le) := transform
...
export o := dataset( [{fieldname,%ca%,global(%o_sl%,few),global(%o_wc%,few),global(%o_cc%,few),global(topn(%o_dp%,100,-cnt),few),global(%vls%,few)}], SALT.mac_character_counts.ResultLine_Layout );
  endmacro;

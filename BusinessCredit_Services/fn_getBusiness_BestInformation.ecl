		IMPORT Address, Business_Credit, BIPV2, BIPV2_Best, BIPV2_Best_SBFE, Codes, Doxie, DCAV2, iesp, TopBusiness_Services, Cortera,
  DX_Equifax_Business_Data;
    ...
    // RQ-12976 - If the record is an SBFE singleton, we want to show the best info from SBFE
		...
		// get cortera header info
		 corteraLinkIdInfo := Cortera.Key_LinkIds.kfetch2(project(inmod.BusinessIds, TRANSFORM(BIPV2.IDlayouts.l_xlink_ids2,
					                                                                                                     SELF := LEFT; SELF := [])), inmod.fetchLevel, ,BusinessCredit_Services.Constants.KFETCH_MAX_LIMIT );
		...
		equifaxBusDataLinkidInfo := DX_Equifax_Business_Data.Key_LinkIDs.kfetch(project(inmod.BusinessIds, TRANSFORM(BIPV2.IDlayouts.l_xlink_ids,
					                                                                                                     SELF := LEFT; SELF := [])), inmod.fetchLevel, ,BusinessCredit_Services.Constants.KFETCH_MAX_LIMIT);
		...
		// *** new added code here use below in 
		 dt_first_seenValue :=  bipv2BestRecs[1].company_name[1].dt_first_seen;
      ...
      csz := Address.Addr2FromComponents(AuthRepBestRec[1].city_name, AuthRepBestRec[1].st, AuthRepBestRec[1].zip);
			...
			SELF.EstablishedDate						:= IF(inMod.BusinessCreditReportType IN BusinessCredit_Services.Constants.LNOnlyCreditSet, 
       ...
       // output(ParentInfoCname, named('ParentInfoCname'));
			 ...
			 // output(equifaxBusDataIncome, named('equifaxBusDataIncome'));

IMPORT BIPV2,Business_Credit,Business_Credit_Scoring;
...
EXPORT Batch_Functions := MODULE
		...
		EXPORT getTradelineInfo(DATASET(RECORDOF(Business_Credit.Key_LinkIds().kFetch2(dataset([],BIPV2.IDlayouts.l_xlink_ids2)))) inAcctRecs,
				...
				TradeRecs_Raw :=JOIN(inAcctRecs,Business_Credit.key_tradeline(), 
																...
																TRANSFORM(BusinessCredit_Services.Batch_layouts.TradelineInfo,
																					...
																					SELF.Past_Due_Amount_01to30 := (STRING) (100 * (INTEGER) (RIGHT.Past_Due_Aging_Amount_Bucket_1));
																					SELF.Past_Due_Amount_31to60 := (STRING) (100 * (INTEGER) (RIGHT.Past_Due_Aging_Amount_Bucket_2));
																					SELF.Past_Due_Amount_61to90 := (STRING) (100 * (INTEGER) (RIGHT.Past_Due_Aging_Amount_Bucket_3));
																					SELF.Past_Due_Amount_91to120 := (STRING) (100 * (INTEGER) (RIGHT.Past_Due_Aging_Amount_Bucket_4));
																					SELF.Past_Due_Amount_120Plus := (STRING) ((100 * (INTEGER) (RIGHT.Past_Due_Aging_Amount_Bucket_5)) 
																																								  + (100 * (INTEGER) (RIGHT.Past_Due_Aging_Amount_Bucket_6))
																																								  + (100 * (INTEGER) (RIGHT.Past_Due_Aging_Amount_Bucket_7)));
																					SELF := RIGHT,
																					SELF := []),
		...
		EXPORT getCreditScore(DATASET(BIPV2.IDlayouts.l_xlink_ids2) inBusRecs,
				...
				ScoringRecsMask := PROJECT(ScoringRecs,TRANSFORM(ScoreRecLayout,
																								SELF.UltID := LEFT.UltID,
																								SELF.OrgID := IF(inmod.BIPFetchLevel IN BIPV2.IDconstants.Set_Fetch_Level_OrgID_And_Down,  LEFT.OrgID, 0),
																								SELF.SeleID := IF(inmod.BIPFetchLevel IN BIPV2.IDconstants.Set_Fetch_Level_SELEID_And_Down, LEFT.SeleID, 0),
																								SELF.ProxID := IF(inmod.BIPFetchLevel IN BIPV2.IDconstants.Set_Fetch_Level_ProxID_And_Down, LEFT.ProxID, 0),

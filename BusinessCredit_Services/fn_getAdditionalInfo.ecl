IMPORT Address, Business_Credit, BusinessCredit_Services, doxie, iesp, Suppress;

EXPORT fn_getAdditionalInfo (DATASET(BusinessCredit_Services.Layouts.TopBusinessRecord) topBusinessRecs,															
  ...
  topBusiness_bestrecs 	:= 	PROJECT(topBusinessRecs, BusinessCredit_Services.Layouts.TopBusiness_BestSection)[1].BestSection.OtherCompanyNames;
	...
	buzCreditInformationOrg	:= JOIN(buzCreditHeader_recs , Business_Credit.Key_BusinessInformation(), 
																 BusinessCredit_Services.Macros.mac_JoinBusAccounts(), 
																	TRANSFORM(RIGHT), 
																	LIMIT(BusinessCredit_Services.Constants.Join_Limit, SKIP));
	buzCreditInformation := Suppress.MAC_SuppressSource(buzCreditInformationOrg, mod_access, did);

IMPORT SALT311,STD;
IMPORT Scrubs,Scrubs_Corp2_Mapping_NH_AR,Corp2_mapping; // Import modules for FieldTypes attribute definitions
...
EXPORT FromNone(DATASET(Corp2_mapping.LayoutsCommon.AR) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.ar_year_Invalid := Fields.InValid_ar_year((SALT311.StrType)le.ar_year);
    SELF.ar_filed_dt_Invalid := Fields.InValid_ar_filed_dt((SALT311.StrType)le.ar_filed_dt);
    SELF.ar_report_dt_Invalid := Fields.InValid_ar_report_dt((SALT311.StrType)le.ar_report_dt);
    SELF.ar_report_nbr_Invalid := Fields.InValid_ar_report_nbr((SALT311.StrType)le.ar_report_nbr);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Corp2_mapping.LayoutsCommon.AR);
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.corp_key_Invalid << 0 ) + ( le.corp_vendor_Invalid << 2 ) + ( le.corp_state_origin_Invalid << 3 ) + ( le.corp_process_date_Invalid << 4 ) + ( le.corp_sos_charter_nbr_Invalid << 6 ) + ( le.ar_year_Invalid << 8 ) + ( le.ar_filed_dt_Invalid << 9 ) + ( le.ar_report_dt_Invalid << 10 ) + ( le.ar_report_nbr_Invalid << 12 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.ar_filed_dt_Invalid := (le.ScrubsBits1 >> 9) & 1;
    SELF.ar_report_dt_Invalid := (le.ScrubsBits1 >> 10) & 3;
    SELF.ar_report_nbr_Invalid := (le.ScrubsBits1 >> 12) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    ar_report_nbr_CUSTOM_ErrorCount := COUNT(GROUP,h.ar_report_nbr_Invalid=1);
    AnyRule_WithErrorsCount := COUNT(GROUP, h.corp_key_Invalid > 0 OR h.corp_vendor_Invalid > 0 OR h.corp_state_origin_Invalid > 0 OR h.corp_process_date_Invalid > 0 OR h.corp_sos_charter_nbr_Invalid > 0 OR h.ar_year_Invalid > 0 OR h.ar_filed_dt_Invalid > 0 OR h.ar_report_dt_Invalid > 0 OR h.ar_report_nbr_Invalid > 0);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.corp_key_Total_ErrorCount > 0, 1, 0) + IF(le.corp_vendor_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_state_origin_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_sos_charter_nbr_Total_ErrorCount > 0, 1, 0) + IF(le.ar_year_ALLOW_ErrorCount > 0, 1, 0) + IF(le.ar_filed_dt_ALLOW_ErrorCount > 0, 1, 0) + IF(le.ar_report_dt_Total_ErrorCount > 0, 1, 0) + IF(le.ar_report_nbr_CUSTOM_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.corp_key_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_key_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_vendor_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_state_origin_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_sos_charter_nbr_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_sos_charter_nbr_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.ar_year_ALLOW_ErrorCount > 0, 1, 0) + IF(le.ar_filed_dt_ALLOW_ErrorCount > 0, 1, 0) + IF(le.ar_report_dt_ALLOW_ErrorCount > 0, 1, 0) + IF(le.ar_report_dt_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.ar_report_nbr_CUSTOM_ErrorCount > 0, 1, 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.corp_key_Invalid,le.corp_vendor_Invalid,le.corp_state_origin_Invalid,le.corp_process_date_Invalid,le.corp_sos_charter_nbr_Invalid,le.ar_year_Invalid,le.ar_filed_dt_Invalid,le.ar_report_dt_Invalid,le.ar_report_nbr_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_corp_key(le.corp_key_Invalid),Fields.InvalidMessage_corp_vendor(le.corp_vendor_Invalid),Fields.InvalidMessage_corp_state_origin(le.corp_state_origin_Invalid),Fields.InvalidMessage_corp_process_date(le.corp_process_date_Invalid),Fields.InvalidMessage_corp_sos_charter_nbr(le.corp_sos_charter_nbr_Invalid),Fields.InvalidMessage_ar_year(le.ar_year_Invalid),Fields.InvalidMessage_ar_filed_dt(le.ar_filed_dt_Invalid),Fields.InvalidMessage_ar_report_dt(le.ar_report_dt_Invalid),Fields.InvalidMessage_ar_report_nbr(le.ar_report_nbr_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.ar_filed_dt_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.ar_report_dt_Invalid,'ALLOW','CUSTOM','UNKNOWN')
          ,CHOOSE(le.ar_report_nbr_Invalid,'CUSTOM','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_sos_charter_nbr','ar_year','ar_filed_dt','ar_report_dt','ar_report_nbr','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_numeric','invalid_numeric','invalid_date','invalid_ar_report_nbr','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT311.StrType)le.corp_key,(SALT311.StrType)le.corp_vendor,(SALT311.StrType)le.corp_state_origin,(SALT311.StrType)le.corp_process_date,(SALT311.StrType)le.corp_sos_charter_nbr,(SALT311.StrType)le.ar_year,(SALT311.StrType)le.ar_filed_dt,(SALT311.StrType)le.ar_report_dt,(SALT311.StrType)le.ar_report_nbr,'***SALTBUG***');
  END;
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'ar_filed_dt:invalid_numeric:ALLOW'
          ,'ar_report_dt:invalid_date:ALLOW','ar_report_dt:invalid_date:CUSTOM'
          ,'ar_report_nbr:invalid_ar_report_nbr:CUSTOM'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_ar_filed_dt(1)
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ...
          ,le.ar_filed_dt_ALLOW_ErrorCount
          ,le.ar_report_dt_ALLOW_ErrorCount,le.ar_report_dt_CUSTOM_ErrorCount
          ,le.ar_report_nbr_CUSTOM_ErrorCount,0) / le.TotalCnt, CHOOSE(c - NumRules
...
EXPORT StandardStats(DATASET(Corp2_mapping.LayoutsCommon.AR) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Corp2_Mapping_NH_AR, Fields, 'RECORDOF(scrubsSummaryOverall)', '');

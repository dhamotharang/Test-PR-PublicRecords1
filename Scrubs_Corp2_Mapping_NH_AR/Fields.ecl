IMPORT ut,SALT32;
IMPORT Scrubs,Scrubs_Corp2_Mapping_NH_AR; // Import modules for FieldTypes attribute definitions
EXPORT Fields := MODULE
 
// Processing for each FieldType
EXPORT SALT32.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_future_date','invalid_charter_nbr','invalid_Y_N','invalid_ar_report_nbr','invalid_numeric');
EXPORT FieldTypeNum(SALT32.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_corp_vendor' => 2,'invalid_state_origin' => 3,'invalid_mandatory' => 4,'invalid_date' => 5,'invalid_future_date' => 6,'invalid_charter_nbr' => 7,'invalid_Y_N' => 8,'invalid_ar_report_nbr' => 9,'invalid_numeric' => 10,0);
...
EXPORT MakeFT_invalid_corp_vendor(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_vendor(SALT32.StrType s) := WHICH(((SALT32.StrType) s) NOT IN ['33']);
EXPORT InValidMessageFT_invalid_corp_vendor(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInEnum('33'),SALT32.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_state_origin(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_state_origin(SALT32.StrType s) := WHICH(((SALT32.StrType) s) NOT IN ['NH']);
EXPORT InValidMessageFT_invalid_state_origin(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInEnum('NH'),SALT32.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_ar_report_nbr(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_ar_report_nbr(SALT32.StrType s) := WHICH(~Scrubs_Corp2_Mapping_NH_AR.Functions.valid_documentid(s)>0);
EXPORT InValidMessageFT_invalid_ar_report_nbr(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.CustomFail('Scrubs_Corp2_Mapping_NH_AR.Functions.valid_documentid'),SALT32.HygieneErrors.Good);
...
EXPORT Make_ar_report_dt(SALT32.StrType s0) := MakeFT_invalid_date(s0);
EXPORT InValid_ar_report_dt(SALT32.StrType s) := InValidFT_invalid_date(s);
EXPORT InValidMessage_ar_report_dt(UNSIGNED1 wh) := InValidMessageFT_invalid_date(wh);
 
EXPORT Make_ar_report_nbr(SALT32.StrType s0) := MakeFT_invalid_ar_report_nbr(s0);
EXPORT InValid_ar_report_nbr(SALT32.StrType s) := InValidFT_invalid_ar_report_nbr(s);
EXPORT InValidMessage_ar_report_nbr(UNSIGNED1 wh) := InValidMessageFT_invalid_ar_report_nbr(wh);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT32,Scrubs_Corp2_Mapping_NH_AR;

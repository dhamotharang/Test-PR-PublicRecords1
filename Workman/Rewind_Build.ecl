...
function
  ...
  ds_workman_files_written            := project(ds_workman  ,transform({string esp,dataset(WsWorkunits.layouts.WsFileWritten) files} ,self.files := WsWorkunits.get_FilesWritten(left.wuid,left.esp,false),self := left));

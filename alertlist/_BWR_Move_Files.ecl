...
/* Open attribute as BWR and hit SUBMIT OR find previous workunit on ECLWatch and hit RESTART */
...
#WORKUNIT('name','SNA Person Cluster and Business Contacts File Monitor');
...
Prod_Business_Contacts2 := fileservices.superfilecontents(ut.foreign_prod + Prod_Business_Contacts1[stringlib.stringfind(Prod_Business_Contacts1, '::', 2) + 2..])[1].name;
	Business_Contacts1_IsSuper := fileservices.LogicalFileList(Prod_Business_Contacts1[stringlib.stringfind(Prod_Business_Contacts1, '::', 2) + 2..],true,true,,
	_control.IPAddress.prod_thor_dali)[1].superfile;
...
SNA_Person_Cluster_Prod_File := DISTRIBUTE(SNA.Key_Person_Cluster, HASH(cluster_id));
...
PARALLEL(
	...
	: WHEN(CRON('30 13 * * *')),
		FAILURE(FileServices.SendEmail('jo.prichard@lexisnexis.com', 

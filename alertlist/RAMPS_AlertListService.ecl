...
EXPORT RAMPS_AlertListService(String inputLogicalFile,
		...
		fun_calculate_overlap_output := '~thordev_socialthor_50::out::social_alert_results::' + FileID;
			
		//Call Alert_Network.fun_calculate_overlap
		FUN_CALCULATE_OVERLAP := Alert_Network.fun_calculate_overlap(inputLogicalFile, pMindidscore, pMinbdidscore, ActivateThreshold, Test);
		
		//Copy the Output Logical File from the fun_calculate_overlap to the one passed in (OutputLogialFile)
		RETURN sequential(FUN_CALCULATE_OVERLAP, FileServices.RenameLogicalFile(fun_calculate_overlap_output, outputLogicalFile));

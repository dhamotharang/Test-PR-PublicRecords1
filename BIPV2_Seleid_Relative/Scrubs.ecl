...
EXPORT FromNone(DATASET(Layout_Base) h) := MODULE
  Expanded_Layout into(h le) := TRANSFORM
    SELF.cnp_name_Invalid := Fields.InValid_cnp_name((SALT31.StrType)le.cnp_name);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.FieldContents := CHOOSE(c,(SALT31.StrType)le.cnp_name,(SALT31.StrType)le.st,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT31.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_st(1),Fields.InvalidMessage_st(2),'UNKNOWN');

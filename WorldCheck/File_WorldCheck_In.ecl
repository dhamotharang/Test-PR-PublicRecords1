...
CompanyFind1 := JOIN(sort(distribute(norm_out,hash(companies)),companies,local),sort(distribute(in_file,hash(uid)),uid,local),
...
CompanyFind2 := JOIN(sort(distribute(norm_out1,hash(linked_tos)),linked_tos,local),sort(distribute(final_out,hash(uid)),uid,local),
...
CompanyFind21 := JOIN(sort(distribute(final_out1(linked_tos<>''),hash(linked_tos)),linked_tos,local),sort(distribute(final_out(uid<>''),hash(uid)),uid,local),
LEFT.linked_tos = RIGHT.UID,
...
CompanyFind22 := JOIN(sort(distribute(companyfind21a(companies<>''),hash(companies)),companies,local),sort(distribute(companyfind21a(uid<>''),hash(uid)),uid,local),
LEFT.companies = RIGHT.UID,
...
export File_WorldCheck_In := DISTRIBUTE(CompanyFind22a, SKEW(0.01)) : persist('~thor_200::persist::worldcheck::clean');
export mac_edit_distance_pairs(infile,infield,countfield,thresh,phonetics,outfile) := MACRO
	...
	#uniquename(j9)
	%j9% := join(%inf8p%,%rhs%,length(trim(left.infield))=right.rl and left.infield[1..5]=right.infield[1..5] and  ut.WithinEditN(left.infield,right.infield,thresh),%trans1%(left,right));
	#uniquename(j7)
	%j7% := join(%inf8p%,%rhs%,length(trim(left.infield))=right.rl and left.infield[1..2]=right.infield[1..2] and left.infield[length(trim(left.infield))-2..] = right.infield[length(trim(right.infield))-2..] and ut.WithinEditN(left.infield,right.infield,thresh),%trans1%(left,right));
	#uniquename(j8)
	%j8% := join(%inf8p%,%rhs%,length(trim(left.infield))=right.rl and left.infield[length(trim(left.infield))-4..] = right.infield[length(trim(right.infield))-4..] and ut.WithinEditN(left.infield,right.infield,thresh),%trans1%(left,right));
...
#uniquename(j)
%j% := join(infile,%t%,left.infield=right.id1);

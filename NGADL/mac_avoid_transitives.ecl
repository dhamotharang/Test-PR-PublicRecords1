export mac_avoid_transitives(ifile,did1,did2,conf,rule_num,ofile) := macro
	...
	%one_only_b% := dedup( sort( distribute( %one_only_a%, hash(did2) ), did2, -conf, did1, rule_num, local ), did2, local );
	// Make sure no did1 has two bests (still)
	#uniquename(one_only)
	%one_only% := dedup( sort( distribute( %one_only_b%, hash(did1) ), did1, -conf, did2, rule_num, local ), did1, local );

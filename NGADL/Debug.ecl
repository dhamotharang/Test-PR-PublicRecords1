...
export layout_sample_matches sample_match_join(match_candidates(ih).layout_candidates le,match_candidates(ih).layout_candidates ri, Layout_Specificities(ih) s) := transform
  self.DID1 := le.DID;
  self.DID2 := ri.DID;
  self.RID1 := le.RID;
  self.RID2 := ri.RID;
  ...
  self.FULLNAME_score := MAP( le.FULLNAME  IN SET(s.nulls_FULLNAME,FULLNAME) or ri.FULLNAME  IN SET(s.nulls_FULLNAME,FULLNAME) => 0,
  ...
  self.ADDR1_score := MAP( le.ADDR1  IN SET(s.nulls_ADDR1,ADDR1) or ri.ADDR1  IN SET(s.nulls_ADDR1,ADDR1) => 0,
                        le.ADDR1 = ri.ADDR1  => le.ADDR1_weight100,
                        0);
  self.left_ADDR1 := le.ADDR1;
  self.right_ADDR1 := ri.ADDR1;
  self.LOCALE_score := MAP( le.LOCALE  IN SET(s.nulls_LOCALE,LOCALE) or ri.LOCALE  IN SET(s.nulls_LOCALE,LOCALE) => 0,
  ...
  self.PRIM_NAME_score := MAP( le.PRIM_NAME  IN SET(s.nulls_PRIM_NAME,PRIM_NAME) or ri.PRIM_NAME  IN SET(s.nulls_PRIM_NAME,PRIM_NAME) => 0,
                        self.ADDR1_score <> 0 => 0, // Parent has value so child keeps quiet
  ...
  self.Conf_Prop := (if(le.SSN_prop or ri.SSN_prop,self.SSN_score,0) + if(le.FULLNAME_prop or ri.FULLNAME_prop,self.FULLNAME_score,0) + if(le.DOB_prop or ri.DOB_prop,self.DOB_score,0) + if(le.FNAME_prop or ri.FNAME_prop,self.FNAME_score,0) + if(le.MNAME_prop or ri.MNAME_prop,self.MNAME_score,0) + if(le.NAME_SUFFIX_prop or ri.NAME_SUFFIX_prop,self.NAME_SUFFIX_score,0)) / 100; // Score based on propogated fields
  self.Conf := (self.VENDOR_ID_score + self.SSN_score + self.FULLNAME_score + self.DOB_score + self.ADDR1_score + self.LNAME_score + self.PRIM_NAME_score + self.PRIM_RANGE_score + self.CITY_NAME_score + self.LOCALE_score + self.FNAME_score + self.SEC_RANGE_score + self.MNAME_score + self.ST_score + self.NAME_SUFFIX_score + self.GENDER_score) / 100;
end;
export AnnotateMatchesFromData(dataset(match_candidates(ih).layout_candidates) in_data,dataset(match_candidates(ih).layout_matches)  im, Layout_Specificities(ih) s) := function
  j1 := join(in_data,im,left.DID = right.DID1,hash);
  match_candidates(ih).layout_candidates strim(j1 le) := transform
    self := le;
  end;
  r := join(j1,in_data,left.DID2 = right.DID,sample_match_join( project(left,strim(left)),right,s),hash);
  return dedup( sort( r, DID1, DID2, -Conf, local ), DID1, DID2, local ); // DID2 distributed by join
end;
export AnnotateMatchesFromRecordData(dataset(match_candidates(ih).layout_candidates) in_data,dataset(match_candidates(ih).layout_matches)  im, Layout_Specificities(ih) s) := function//Faster form when RID known
  j1 := join(in_data,im,left.RID = right.RID1,hash);
  match_candidates(ih).layout_candidates strim(j1 le) := transform
    self := le;
  end;
  return join(j1,in_data,left.RID2 = right.RID,sample_match_join( project(left,strim(left)),right,s),hash);
end;
export AnnotateMatches(dataset(match_candidates(ih).layout_matches)  im, Layout_Specificities(ih) s) := function
  return AnnotateMatchesFromRecordData(h,im,s);
end;
...
export RolledEntities(dataset(match_candidates(ih).layout_candidates) in_data, Layout_Specificities(ih) s) := function
Layout_RolledEntity into(in_data le) := transform
  ...
  self.ADDR1_Values := IF ( le.PRIM_NAME IN SET(s.nulls_PRIM_NAME,PRIM_NAME) AND le.PRIM_RANGE IN SET(s.nulls_PRIM_RANGE,PRIM_RANGE),dataset([],Layout_FieldValueList),dataset([{trim((string)le.PRIM_NAME) + ' ' + trim((string)le.PRIM_RANGE)}],Layout_FieldValueList));

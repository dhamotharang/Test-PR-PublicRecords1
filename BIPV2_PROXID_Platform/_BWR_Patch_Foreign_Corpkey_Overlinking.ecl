...
dbaseqa     := BIPV2_ProxID_Platform.files().base.qa;
dbasebuilt  := BIPV2_ProxID_Platform.files().base.built;
...
//dbasebuilt  := BIPV2_ProxID_Platform.files('20130808_43').base.logical(proxid in setsampleproxidsfromiter41) : persist('~thor::persist::lbentley::BIPSprint2.dbasebuilt');
dbasebuilt  := BIPV2_ProxID_Platform.files('20131001_46',true).base.logical;
...
dsDotidPatched2 := BIPV2_ProxID_Platform._fPatch_Proxids(dsDotidPatched);
...
/////////////////////////////////
newversion := '20130905';
writefile   := tools.macf_writefile(BIPV2_ProxID_Platform.filenames(newversion).base.logical,dsproxidproj,poverwrite := true,pDescription := 'fix foreign_corp_keys, patch overlinked dotids and proxids');
promotefile := BIPV2_ProxID_Platform.promote(newversion).new2built;
dbasebeforepatch  := dbasebuilt;
dbaseafterpatch   := BIPV2_ProxID_Platform.files(newversion).base.logical;

...
EXPORT ProxidCompareService_iter29 := MACRO
IMPORT SALT24,BIPV2_ProxID_Platform,ut,tools,BIPV2;
STRING50 Proxidonestr := ''   : STORED('ProxidOne');
STRING20 Proxidtwostr := '*'  : STORED('Proxidtwo');
string   iteration    := '29'  ;
string version        := BIPV2.KeySuffix + 'c';
...
odl := PROJECT(CHOOSEN(BIPV2_ProxID_Platform.Keys(BFile,both).Candidates(Proxid=Proxidone),100000),BIPV2_ProxID_Platform.match_candidates(BFile).layout_candidates);
odr := PROJECT(CHOOSEN(BIPV2_ProxID_Platform.Keys(BFile,both).Candidates(Proxid=ProxidTwo),100000),BIPV2_ProxID_Platform.match_candidates(BFile).layout_candidates);
k := BIPV2_ProxID_Platform.Keys(BFile,both).Specificities_Key;

...
SHARED match_candidates(ih).layout_matches match_join(match_candidates(ih).layout_candidates le,match_candidates(ih).layout_candidates ri,UNSIGNED c=0,UNSIGNED outside=0,UNSIGNED cnp_name_support = 0) := TRANSFORM
  SELF.Rule := c;
  ...
  INTEGER2 active_domestic_corp_key_score := MAP(
  ...
  INTEGER2 company_address_score_pre := MAP( (le.company_address_isnull OR (le.company_addr1_isnull OR le.prim_range_derived_isnull AND le.prim_name_derived_isnull AND le.sec_range_isnull) AND (le.company_csz_isnull OR le.v_city_name_isnull AND le.st_isnull AND le.zip_isnull)) OR (ri.company_address_isnull OR (ri.company_addr1_isnull OR ri.prim_range_derived_isnull AND ri.prim_name_derived_isnull AND ri.sec_range_isnull) AND (ri.company_csz_isnull OR ri.v_city_name_isnull AND ri.st_isnull AND ri.zip_isnull)) OR le.company_address_weight100 = 0 => 0,
                        ...
                        0);
  INTEGER2 cnp_number_score := IF ( le.cnp_number = ri.cnp_number /*  cnp_number_score_temp >= Config.cnp_number_Force * 100 */ , cnp_number_score_temp, SKIP ); // Enforce FORCE parameter
  ...
  INTEGER2 cnp_name_score := MAP ( le.cnp_name = ri.cnp_name
    ...
    => cnp_name_score_supp
    ,active_domestic_corp_key_score > Config.active_domestic_corp_key_Force*100 and regexfind('fbn|dba|fictitious|assumed|trade',le.company_name_type_raw + le.company_name_type_derived + ri.company_name_type_raw + ri.company_name_type_derived,nocase)
    => active_domestic_corp_key_score
    ,active_duns_number_score > Config.active_duns_number_Force      *100 and regexfind('fbn|dba|fictitious|assumed|trade',le.company_name_type_raw + le.company_name_type_derived + ri.company_name_type_raw + ri.company_name_type_derived,nocase)
  ...
  INTEGER2 prim_name_derived_score := IF ( le.prim_name_derived = ri.prim_name_derived/* HACK */ or  prim_name_derived_score_temp >= Config.prim_name_derived_Force * 100 OR company_addr1_score_pre > 0 OR company_address_score_pre > 0, prim_name_derived_score_temp, SKIP ); // Enforce FORCE parameter
  ...
  INTEGER2 prim_range_derived_score := IF ( le.prim_range_derived = ri.prim_range_derived or  prim_range_derived_score_temp >= Config.prim_range_derived_Force * 100 OR company_addr1_score_pre > 0 OR company_address_score_pre > 0, prim_range_derived_score_temp, SKIP ); // Enforce FORCE parameter
// Compute the score for the concept company_addr1
  INTEGER2 company_addr1_score_ext := SALT30.ClipScore(MAX(company_addr1_score_pre,0) + prim_range_derived_score + prim_name_derived_score + sec_range_score + MAX(company_address_score_pre,0));// Score in surrounding context
...
// Compute the score for the concept company_address
  INTEGER2 company_address_score_ext := SALT30.ClipScore(MAX(company_address_score_pre,0)+ company_addr1_score + prim_range_derived_score + prim_name_derived_score + sec_range_score+ company_csz_score + v_city_name_score + st_score + zip_score);// Score in surrounding context
  ...
  SELF.Conf_Prop := (0
    ...
    +if(le.company_addr1_prop+ri.company_addr1_prop>0,company_addr1_score*(0+if(le.prim_range_derived_prop+ri.prim_range_derived_prop>0,1,0)+if(le.sec_range_prop+ri.sec_range_prop>0,1,0))/3,0)
...
//Fixed fields ->:cnp_number(15):st(5):prim_range_derived(13)
dn0 := hfile(~st_isnull);
...
mj0 := JOIN( dn0_deduped, dn0_deduped, LEFT.Proxid > RIGHT.Proxid AND ( LEFT.SALT_Partition = RIGHT.SALT_Partition OR LEFT.SALT_Partition='' OR RIGHT.SALT_Partition = '' ) AND LEFT.cnp_number = RIGHT.cnp_number AND LEFT.prim_name_derived = RIGHT.prim_name_derived AND LEFT.st = RIGHT.st AND LEFT.prim_range_derived = RIGHT.prim_range_derived AND ( ~left.st_isnull AND ~right.st_isnull ) AND ( left.active_enterprise_number = right.active_enterprise_number OR left.active_enterprise_number_isnull OR right.active_enterprise_number_isnull ) AND (( ~left.cnp_name_isnull AND ~right.cnp_name_isnull ) OR ( ~left.active_domestic_corp_key_isnull AND ~right.active_domestic_corp_key_isnull ) OR ( ~left.active_duns_number_isnull AND ~right.active_duns_number_isnull )) AND ( ~left.prim_name_derived_isnull AND ~right.prim_name_derived_isnull ) AND ( ~left.company_address_isnull AND ~right.company_address_isnull )AND ~(( left.cnp_name = right.cnp_name ) AND ( left.company_address = right.company_address )),trans(LEFT,RIGHT,0),HINT(unsorted_output),ATMOST(LEFT.cnp_number = RIGHT.cnp_number AND LEFT.prim_name_derived = RIGHT.prim_name_derived AND LEFT.st = RIGHT.st AND LEFT.prim_range_derived = RIGHT.prim_range_derived,20000),HASH);
...
match_candidates(ih).Layout_Attribute_Matches CombineResults(match_candidates(ih).Layout_Attribute_Matches le,match_candidates(ih).Layout_Attribute_Matches ri) := TRANSFORM
  ...
  SELF := le;
END;
EXPORT All_Attribute_Matches := ROLLUP(AllAttrMatches,LEFT.Proxid1=RIGHT.Proxid1 AND LEFT.Proxid2=RIGHT.Proxid2,CombineResults(LEFT,RIGHT),LOCAL) : PERSIST('~temp::Proxid::BIPV2_ProxID_Platform::attribute_matches',EXPIRE(Config.PersistExpire));
...
EXPORT All_Matches := all_matches3 : PERSIST('~temp::Proxid::BIPV2_ProxID_Platform::all_m',EXPIRE(Config.PersistExpire)); // To by used by rcid and Proxid
...
o := BIPV2_ProxID_Platform.mac_avoid_transitives_scalene(All_Matches,Proxid1,Proxid2,Conf,DateOverlap,Rule,MatchThreshold,10); // HACK - Use new transitives macro, bucket size 5*/
EXPORT PossibleMatches := o : PERSIST('~temp::Proxid::BIPV2_ProxID_Platform::mt',EXPIRE(Config.PersistExpire));
SALT30.mac_get_BestPerRecord( All_Matches,rcid1,Proxid1,rcid2,Proxid2,o );
EXPORT BestPerRecord := o : PERSIST('~temp::Proxid::BIPV2_ProxID_Platform::mr',EXPIRE(Config.PersistExpire));
...
EXPORT ClusterLinkages := o1 : PERSIST('~temp::Proxid::BIPV2_ProxID_Platform::clu',EXPIRE(Config.PersistExpire));
...
EXPORT ToSlice := IF ( Config.DoSliceouts, DEDUP(SORT(DISTRIBUTE(BetterElsewhere(Pref_Margin>10),HASH(Proxid)),Proxid,-Pref_Margin,LOCAL),Proxid,LOCAL)) : PERSIST('~temp::Proxid::BIPV2_ProxID_Platform::Matches::ToSlice',EXPIRE(Config.PersistExpire));
...
EXPORT PreIDs := BIPV2_ProxID_Platform.Fields.UIDConsistency(ih); // Export whole consistency module
EXPORT PostIDs := BIPV2_ProxID_Platform.Fields.UIDConsistency(Patched_Infile); // Export whole consistency module

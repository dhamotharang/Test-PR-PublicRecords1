...
/*
  ...
  4. hack BIPV2_ProxID_Platform.MOD_Attr_FilterPrimNames to fix it so it runs -- check
  
*/

EXPORT _ApplyProxidHacks(
   string   pModule               = 'BIPV2_ProxID_Platform'
...
/*
  BIPV2_ProxID_Platform.matches: hack default force for cnp_number,cnp_name & prim_range(make sure they equal each other) in matchjoin attribute
  ...
  BIPV2_ProxID_Platform.Debug:   hack sample match join to match hacks in matchjoin in matches.            
...
/////////hack debug scores for attribute files
    ...
    hackReplaceTransitivesMacroCondition := pAttribute = 'matches' and      regexfind('salt30[.]mac_avoid_transitives[(]All_Matches,Proxid1,Proxid2,Conf,DateOverlap,Rule,o[)];',HackPrimNameExact,nocase)
                                                                   and not  regexfind('BIPV2_ProxID_Platform[.]mac_avoid_transitives_scalene'                                            ,HackPrimNameExact,nocase);
...
// and then on the first MAC_ParentId_Patch call change the o parameter to o_thin
  ...
  function
    ...
    + 'EXPORT MatchHistoryName := \'~keep::BIPV2_ProxID_Platform::Proxid::MatchHistory\';\n'
    + 'EXPORT MatchHistoryFile := DATASET(MatchHistoryName,Matches(In_DOT_Base).id_shift_r,THOR); // Read in all the change history\n'
    + 'EXPORT MatchHistoryKeyName := \'~\'+\'key::BIPV2_ProxID_Platform::Proxid::History::Match\';\n'
    + '  MH := MatchHistoryFile;\n'
    ...
    + '// EXPORT InDataKeyName := \'~\'+\'key::BIPV2_ProxID_Platform::Proxid::Datafile::in_data\';\n'
  ...
  function
    ...
    HackKeys  := 
      ...
      + '//  Cands0 := BIPV2_ProxID_Platform.match_candidates(inhead).ForeignCorpkey_candidates;\n'
      + '  Cands0 := BIPV2_ProxID_Platform.file_Foreign_Corpkey;/*HACK*/\n'
  ...
  function
    ...
    HackKeys  := 
      ...
      + '  Cands0 := BIPV2_ProxID_Platform.file_RA_Addresses;/*HACK*/\n'
      + '// We are going to create a record for each candidate pair; this record will have a child dataset for the attribute values of each side\n'
      + '////\n'
      + 'childrec1 := \n'
      + 'record\n'
  ...
  function
    ...
    HackKeys  := 
      ...
      + 'EXPORT ForceFilter(inhead,infile,id1,id2) := FUNCTIONMACRO\n'
      + '// Every attribute value has to exactly match every attribute value of the corresponding identifier or both have no values\n'
      + '  Cands0 := BIPV2_ProxID_Platform.file_filter_Prim_names;/*HACK*/\n'
      + '// We are going to create a record for each candidate pair; this record will have a child dataset for the attribute values of each side\n'
      + '////\n'
      + 'childrec1 := \n'
      + 'record\n'
...
//////////////    
  ...
  function
    ...
    HackKeys  := 
      ...
      + 'IMPORT SALT24,BIPV2_ProxID_Platform,ut,tools;\n'
      + 'STRING50 Proxidonestr := \'1234\'  : STORED(\'ProxidOne\');\n'
      + 'STRING20 Proxidtwostr := \'12345\'  : STORED(\'Proxidtwo\');\n'
      + 'combo := \'qa\';\n'
      + '\n'
      + 'BIPV2_ProxID_Platform._fun_CompareService((unsigned6)Proxidonestr,(unsigned6)Proxidtwostr,combo);\n'
      + 'ENDMACRO;\n'
      ;

//Executable code
EXPORT Proc_Iterate(STRING iter,string keyversion/*HACK -- add keyversion*/,DATASET(Layout_DOT_Base) InFile0 = BIPV2_ProxID_Platform.In_DOT_Base,STRING OutFileNameP = '~temp::Proxid::BIPV2_ProxID_Platform::it',UNSIGNED MatchThreshold = Config.MatchThreshold,BOOLEAN Debugging = true) := MODULE
SHARED InFile := InFile0;
Export MM := BIPV2_ProxID_Platform.matches(InFile,MatchThreshold); // Get the matching module
...
SHARED BM := BIPV2_ProxID_Platform.BasicMatch(InFile);
...
OS := OUTPUT(BIPV2_ProxID_Platform.specificities(InFile).specificities,NAMED('Specificities'));
OPRP := OUTPUT(BIPV2_ProxID_Platform.match_candidates(InFile).prepropogationstats,NAMED('PrePopStats'));
OPP := OUTPUT(BIPV2_ProxID_Platform.match_candidates(InFile).postpropogationstats,NAMED('PostPopStats'));
...
MP := PARALLEL( OUTPUT(MM.MatchesPerformed,NAMED('MatchesPerformed')),OUTPUT(BIPV2_ProxID_Platform.BasicMatch(InFile).basic_match_count,NAMED('BasicMatchesPerformed')));
...
SpcS := OUTPUT(BIPV2_ProxID_Platform.specificities(InFile).SpcShift,NAMED('SPCShift'));
EXPORT ExecutionStats := PARALLEL(OS,SpcS,PRPP,PPP,PRPPS,PPPS,MP,SP,RE,CB,MPPA,MPPR,OPRP,OPP);
d := DATASET([{MM.PatchingError0,MM.DuplicateRids0,COUNT(BIPV2_ProxID_Platform.match_candidates(InFile).Unlinkables)}],{INTEGER PatchingError0, INTEGER DuplicateRids0, UNSIGNED UnlinkableRecords0});
...
ChangeName := '~temp::Proxid::BIPV2_ProxID_Platform::changes_it'+keyversion;/* HACK use keyversion for changes file*/
...
//EXPORT LoopN(UNSIGNED N,UNSIGNED mt=Config.MatchThreshold) := LOOP(InFile,N,BIPV2_ProxID_Platform.matches(ROWS(LEFT),mt).patched_infile); // Loop N times
//EXPORT LoopThisN(DATASET(BIPV2_ProxID_Platform.Layout_DOT_Base)d,UNSIGNED N,UNSIGNED mt=Config.MatchThreshold) := LOOP(d,N,BIPV2_ProxID_Platform.matches(ROWS(LEFT),mt).patched_infile);
//EXPORT LoopThisK(DATASET(BIPV2_ProxID_Platform.Layout_DOT_Base)d,UNSIGNED K,UNSIGNED NMax=100) := LOOP(d,COUNTER<=NMax AND COUNT(DEDUP(ROWS(LEFT),Proxid,ALL))>K,BIPV2_ProxID_Platform.matches(ROWS(LEFT),0).patched_infile); // Loop until K clusters left

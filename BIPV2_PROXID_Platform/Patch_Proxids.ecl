...
import bipv2,bipv2_files,BIPV2_ProxID_Platform,ut,salt26,tools;
...
function
  ...
  filename  := BIPV2_ProxID_Platform.filenames(combo).base.logical;
  ddefaultproxids := BIPV2_ProxID_Platform._Filters.Explode(project(pDataset,transform(BIPV2.CommonBase.Layout,self.proxid := if(left.proxid = 0 ,left.dotid,left.proxid),self := left)));
  ...
  sequential(
    ...
    ,iff(pAdd2Super = true  ,BIPV2_ProxID_Platform.Promote(combo).new2built)
  )
  ;

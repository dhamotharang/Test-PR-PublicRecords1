dbasebuilt  := BIPV2_ProxID_Platform.files('20130925a_45',true).base.logical;  //this was 1 iteration run on top of a completely fixed input file(no foreign corpkey overlinks)
...
//RA stuff
dbase   := BIPV2_ProxID_Platform.files('20130905',true).base.logical;
...
dbaseaggTop100RA_Addresses    := BIPV2_ProxID_Platform.AggregateProxidElements(dBIP_top100_RA_Addresses ) : persist('~persist::lbentley::dbaseaggTop100RA_Addresses'     );
dbaseaggTop100NotRA_Addresses := BIPV2_ProxID_Platform.AggregateProxidElements(dget_other_recs          ) : persist('~persist::lbentley::dbaseaggTop100NotRA_Addresses'  );

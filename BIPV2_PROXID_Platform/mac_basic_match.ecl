...
ds_agginbigs  := sort(BIPV2_ProxID_Platform.AggregateProxidElements(oldproxids),proxid) : global;
ds_aggoutbigs := sort(BIPV2_ProxID_Platform.AggregateProxidElements(newproxids),proxid) : global;
return
sequential(
  parallel(
    ...
    ,output(ds_doublecheck,,'~temp::lbentley::BIPV2_ProxID_Platform::basic_match::patch'          ,compressed,overwrite)
    ,output(ds_out   ,,BIPV2_ProxID_Platform.filenames(bipv2.KeySuffix + 'a_27').base.logical,compressed,overwrite)

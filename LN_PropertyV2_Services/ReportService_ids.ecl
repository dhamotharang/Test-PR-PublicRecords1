export ReportService_ids(
	...
	// lookup by DID
	dids := dataset([{did}], layouts.search_did) + in_dids;

	by_did := Raw.get_fids_from_dids(dids,isFCRA);

	// lookup by BDID
	bdids := dataset([{bdid}], layouts.search_bdid) + in_bdids;

	by_bdid := Raw.get_fids_from_bdids(bdids);

	// lookup by ParcelId
	pnum := dataset([{parcelID}], layouts.search_pnum);
	by_pnum := Raw.get_fids_from_pnum(pnum);

	// lookup by FaresId
	fid				:= dataset([{faresId}], layouts.fid);

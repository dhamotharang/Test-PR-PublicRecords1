...
export ReportService_records (boolean isFCRA = false,
                              ...
                              FCRA.iRules in_params = module(FCRA.iRules) end, boolean includeVendorSourceB=false ) := function
    ...
    fids := LN_PropertyV2_Services.ReportService_ids(input.did, input.bdid, input.parcelID, 
				...
				// generate the report
    results_pre := LN_PropertyV2_Services.resultFmt.widest_view.get_by_fid(fids,,,nonSS,isFCRA,slim_pc_recs,in_params.FFDOptionsMask, ds_flags, includeVendorSourceB);

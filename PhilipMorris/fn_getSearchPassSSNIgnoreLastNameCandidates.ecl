...
export fn_getSearchPassSSNIgnoreLastNameCandidates(DATASET(LT.Clean.FullRecordNorm) SearchData, UNSIGNED2 minAgeValue, CN.SortSearchPassEnum searchpasslog, boolean isDerived, UNSIGNED1 DPPA_Purpose, UNSIGNED1 GLB_Purpose, boolean SkipRestrictionsCall, boolean AllowProbationSources) := FUNCTION
	...
	dirty_header_records	:= join (searchpass_candidates_dids_deduped, dx_Header.key_header(),
													  ...
													  left.SearchName.lname != right.lname and
														//(Left.DOB_Numeric = right.dob) and
														right.valid_ssn <> 'M',
														TRANSFORM (LT.Search.FullRecordNormWithHeaderData,
		...
		// output(searchpass_candidates_dids_deduped, named('scdd_ignore'), extend);
		return final_output;

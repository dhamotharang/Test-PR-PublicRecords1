...
BR_Init := PROJECT(File_BusReg_Company, AddRecordID(LEFT));

ut.MAC_Sequence_Records(BR_Init, record_id, BR_Seq)

Layout_BHF_Local  Translate_BR_to_BHF(Layout_BR_Local L, INTEGER CTR) := TRANSFORM
...
SELF.city := CHOOSE(CTR, L.mail_p_city_name, L.loc_p_city_name);
...
SELF.county := CHOOSE(CTR, L.mail_fipscounty, L.loc_fipscounty);
...
Business_Header.Layout_Business_Header  Translate_BR_Owner_to_BHF(Layout_BR_Local L) := TRANSFORM
SELF.group1_id := 0;
...
busreg_owner := br_seq(OWNR_CODE='C', OFC1_NAME <> '', OFC1_TITLE <> 'AGT', ofc1_prim_name <> '', ofc1_p_city_name <> '');

busreg_owner_no_agents := busreg.remove_agents(busreg_owner,1000);

// Business Registration Owner Information
//busreg_owner := PROJECT(br_seq(OWNR_CODE='C', OFC1_NAME <> '', OFC1_TITLE <> 'AGT', ofc1_prim_name <> '', ofc1_p_city_name <> ''), Translate_BR_Owner_to_BHF(LEFT));
busreg_owner_filtered := PROJECT(busreg_owner_no_agents, Translate_BR_Owner_to_BHF(LEFT));

// Rollup
Business_Header.Layout_Business_Header RollupBR(Business_Header.Layout_Business_Header L, Business_Header.Layout_Business_Header R) := TRANSFORM
SELF.dt_first_seen := 
...
export BusReg_As_Business_Header := br_clean_rollup : PERSIST('TMTEMP::BR_Company_Rollup');
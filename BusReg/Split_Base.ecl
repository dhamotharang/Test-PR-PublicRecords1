...
module
	...
	function
		...
		Layout_BusReg_Company t2CompanyLayout(layouts.base.AID L) :=
		transform
			...
			self.START_DATE				:= if(l.clean_dates.START_DATE	= 0	,''	,(string)l.clean_dates.START_DATE	);
			self.file_date				:= if(l.clean_dates.file_date	 	= 0	,''	,(string)l.clean_dates.file_date	);
			self.CCYYMMDD					:= if(l.clean_dates.CCYYMMDD		= 0	,''	,(string)l.clean_dates.CCYYMMDD		);
			self.FORM_DATE				:= if(l.clean_dates.FORM_DATE	 	= 0	,''	,(string)l.clean_dates.FORM_DATE	);
			self.EXP_DATE					:= if(l.clean_dates.EXP_DATE		= 0	,''	,(string)l.clean_dates.EXP_DATE		);
			self.DISOL_DATE				:= if(l.clean_dates.DISOL_DATE	= 0	,''	,(string)l.clean_dates.DISOL_DATE	);
			self.RPT_DATE					:= if(l.clean_dates.RPT_DATE		= 0	,''	,(string)l.clean_dates.RPT_DATE		);
			self.RENEW_DATE				:= if(l.clean_dates.RENEW_DATE	= 0	,''	,(string)l.clean_dates.RENEW_DATE	);
			self.CHANG_DATE				:= if(l.clean_dates.CHANG_DATE	= 0	,''	,(string)l.clean_dates.CHANG_DATE	);
			self.APPT_DATE				:= if(l.clean_dates.APPT_DATE	 	= 0	,''	,(string)l.clean_dates.APPT_DATE	);
			self.FISC_DATE_				:= if(l.clean_dates.FISC_DATE_	= 0	,''	,(string)l.clean_dates.FISC_DATE_	);
			self.proc_date				:= if(l.clean_dates.proc_date	 	= 0	,''	,(string)l.clean_dates.proc_date	);
			self := l.rawfields;
			self := l;
	...
	function
		...
		Layout_BusReg_Contact tNorm2ContactLayout(layouts.base.AID L, unsigned4 cnt) :=
		transform
			...
			busreg.Layout_BusReg_Contact propagate(busreg.Layout_BusReg_Contact L, layouts.base.AID R) := 
			transform

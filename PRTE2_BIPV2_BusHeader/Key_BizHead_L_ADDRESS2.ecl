...
//prim_name:zip:?:prim_range:cnp_name:st:+:city:company_sic_code1:cnp_number:cnp_btype:cnp_lowv:sec_range:parent_proxid:sele_proxid:org_proxid:ultimate_proxid:sele_flag:org_flag:ult_flag
 
EXPORT KeyName := PRTE2_BIPV2_BusHeader.Filename_keys.L_ADDRESS2; /*HACK07*/
...
DataForKey0 := DEDUP(SORT(TABLE(h(/*HACK05*/ st!='',(prim_name NOT IN SET(s.nulls_prim_name,prim_name) AND prim_name <> (TYPEOF(prim_name))''),(zip NOT IN SET(s.nulls_zip,zip) AND zip <> (TYPEOF(zip))''),(cnp_name NOT IN SET(s.nulls_cnp_name,cnp_name) AND cnp_name <> (TYPEOF(cnp_name))''),(st NOT IN SET(s.nulls_st,st) AND st <> (TYPEOF(st))'')),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,proxid,prim_name,zip,prim_range,cnp_name,st,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'L_ADDRESS2','city',CntRed_city*100,CntRed_city*TSize},{'L_ADDRESS2','company_sic_code1',CntRed_company_sic_code1*100,CntRed_company_sic_code1*TSize},{'L_ADDRESS2','cnp_number',CntRed_cnp_number*100,CntRed_cnp_number*TSize},{'L_ADDRESS2','cnp_btype',CntRed_cnp_btype*100,CntRed_cnp_btype*TSize},{'L_ADDRESS2','cnp_lowv',CntRed_cnp_lowv*100,CntRed_cnp_lowv*TSize},{'L_ADDRESS2','sec_range',CntRed_sec_range*100,CntRed_sec_range*TSize},{'L_ADDRESS2','parent_proxid',CntRed_parent_proxid*100,CntRed_parent_proxid*TSize},{'L_ADDRESS2','sele_proxid',CntRed_sele_proxid*100,CntRed_sele_proxid*TSize},{'L_ADDRESS2','org_proxid',CntRed_org_proxid*100,CntRed_org_proxid*TSize},{'L_ADDRESS2','ultimate_proxid',CntRed_ultimate_proxid*100,CntRed_ultimate_proxid*TSize},{'L_ADDRESS2','sele_flag',CntRed_sele_flag*100,CntRed_sele_flag*TSize},{'L_ADDRESS2','org_flag',CntRed_org_flag*100,CntRed_org_flag*TSize},{'L_ADDRESS2','ult_flag',CntRed_ult_flag*100,CntRed_ult_flag*TSize}],SALT33.ShrinkLayout);
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.prim_name <> (typeof(le.prim_name))'' AND EXISTS(le.zip_cases);
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch_server(TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',DATASET(process_Biz_layouts.layout_zip_cases) param_zip,TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := 
    STEPPED( LIMIT( Key(
...
EXPORT RawFetch(TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',DATASET(process_Biz_layouts.layout_zip_cases) param_zip,TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
// Why not LOOP? - Because I am expecting FIRST one to win 990f the time - and don't want to impact it
  RawData0 := RawFetch_server(param_prim_name,param_zip,param_prim_range,param_cnp_name,param_st,0);
  RawData1 := RawFetch_server(param_prim_name,param_zip,param_prim_range,param_cnp_name,param_st,1);
  RawData2 := RawFetch_server(param_prim_name,param_zip,param_prim_range,param_cnp_name,param_st,2);
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF := le;
  END;
  Recs0 := Recs(prim_name <> (typeof(prim_name))'',EXISTS(zip_cases));
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_prim_name='',Input_zip='',Input_prim_range='',Input_cnp_name='',Input_st='',Input_city='',Input_company_sic_code1='',Input_cnp_number='',Input_cnp_btype='',Input_cnp_lowv='',Input_sec_range='',Input_parent_proxid='',Input_sele_proxid='',Input_org_proxid='',Input_ultimate_proxid='',Input_sele_flag='',Input_org_flag='',Input_ult_flag='',output_file,AsIndex='true') := MACRO
IMPORT SALT33,BizLinkFull;
#IF(#TEXT(Input_prim_name)<>'' AND #TEXT(Input_zip)<>'')
  #uniquename(trans)
  PRTE2_BIPV2_BusHeader.Key_BizHead_L_ADDRESS2.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE2_BIPV2_BusHeader.Key_BizHead_L_ADDRESS2.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

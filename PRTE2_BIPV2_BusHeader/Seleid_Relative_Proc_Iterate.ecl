...
//Executable code
EXPORT Seleid_Relative_Proc_Iterate(STRING iter,DATASET(BIPV2_Seleid_Relative.Layout_Base) InFile0 = PRTE2_BIPV2_BusHeader.File_DS_CLEAN,STRING OutFileNameP = '~prte::temp::Seleid::PRTE2_BIPV2_BusHeader::Seleid_Relative::it',UNSIGNED MatchThreshold = BIPV2_Seleid_Relative.Config.MatchThreshold,BOOLEAN Debugging = true) := MODULE
...
EXPORT DebugKeys := BIPV2_Seleid_Relative.Keys(InFile).BuildAll;
Rel8 := BUILDINDEX(BIPV2_Seleid_Relative.keys(InFile).ASSOC ,PRTE2_BIPV2_BusHeader.keynames(BIPV2_Seleid_Relative.KeyInfix).assoc.logical,OVERWRITE);
...
SHARED ChangeName := '~prte::temp::Seleid::PRTE2_BIPV2_BusHeader::Seleid_Relative::changes_it'+iter;
EXPORT OutputChanges := SEQUENTIAL( OUTPUT(MM.IdChanges,,ChangeName,OVERWRITE,COMPRESSED),FileServices.AddSuperFile(BIPV2_Seleid_Relative.Keys(InFile).MatchHistoryName,ChangeName));
...
//EXPORT LoopN(UNSIGNED N,UNSIGNED mt=BIPV2_Seleid_Relative.Config.MatchThreshold) := LOOP(InFile,N,BIPV2_Seleid_Relative.matches(ROWS(LEFT),mt).patched_infile); // Loop N times
//EXPORT LoopThisN(d,N,mt=BIPV2_Seleid_Relative.Config.MatchThreshold,ThinLoop=FALSE) := FUNCTIONMACRO
//	IMPORT BIPV2_Seleid_Relative;
//	#IF (ThinLoop=FALSE)

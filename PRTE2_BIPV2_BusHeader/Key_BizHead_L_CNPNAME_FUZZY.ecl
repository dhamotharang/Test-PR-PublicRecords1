...
EXPORT RawFetch_server(TYPEOF(h.company_name_prefix) param_company_name_prefix = (TYPEOF(h.company_name_prefix))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',DATASET(process_Biz_layouts.layout_zip_cases) param_zip,TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := 
    STEPPED( LIMIT( Key(
      ...
      AND KEYED(fallback_value >= param_fallback_value)),10000,/*HACK11 ONFAIL removed*/ SKIP,KEYED),ultid,orgid,seleid,proxid);
 
EXPORT RawFetch(TYPEOF(h.company_name_prefix) param_company_name_prefix = (TYPEOF(h.company_name_prefix))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',DATASET(process_Biz_layouts.layout_zip_cases) param_zip,TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
// Why not LOOP? - Because I am expecting FIRST one to win 990f the time - and don't want to impact it
  RawData0 := RawFetch_server(param_company_name_prefix,param_cnp_name,param_zip,param_city,param_st,0);
  RawData1 := RawFetch_server(param_company_name_prefix,param_cnp_name,param_zip,param_city,param_st,1);
  RawData2 := RawFetch_server(param_company_name_prefix,param_cnp_name,param_zip,param_city,param_st,2);
  Returnable(DATASET(RECORDOF(RawData0)) d) := COUNT(NOFOLD(d))<>1 OR EXISTS(NOFOLD(d((SALT33.StrType)company_name_prefix != '')));
  res := MAP (
...
EXPORT ScoredproxidFetch(TYPEOF(h.company_name_prefix) param_company_name_prefix = (TYPEOF(h.company_name_prefix))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',DATASET(process_Biz_layouts.layout_zip_cases) param_zip,TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.company_sic_code1) param_company_sic_code1 = (TYPEOF(h.company_sic_code1))'',TYPEOF(h.cnp_number) param_cnp_number = (TYPEOF(h.cnp_number))'',TYPEOF(h.cnp_btype) param_cnp_btype = (TYPEOF(h.cnp_btype))'',TYPEOF(h.cnp_lowv) param_cnp_lowv = (TYPEOF(h.cnp_lowv))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.parent_proxid) param_parent_proxid = (TYPEOF(h.parent_proxid))'',TYPEOF(h.sele_proxid) param_sele_proxid = (TYPEOF(h.sele_proxid))'',TYPEOF(h.org_proxid) param_org_proxid = (TYPEOF(h.org_proxid))'',TYPEOF(h.ultimate_proxid) param_ultimate_proxid = (TYPEOF(h.ultimate_proxid))'',TYPEOF(h.sele_flag) param_sele_flag = (TYPEOF(h.sele_flag))'',TYPEOF(h.org_flag) param_org_flag = (TYPEOF(h.org_flag))'',TYPEOF(h.ult_flag) param_ult_flag = (TYPEOF(h.ult_flag))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.company_name_prefix_match_code := MAP(le.company_name_prefix = (TYPEOF(le.company_name_prefix))'' OR le.company_name_prefix = (TYPEOF(le.company_name_prefix))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_company_name_prefix(le.company_name_prefix,param_company_name_prefix,TRUE));
    SELF.company_name_prefixWeight := (50+MAP ( le.company_name_prefix = param_company_name_prefix  => le.company_name_prefix_weight100,
          le.company_name_prefix = (TYPEOF(le.company_name_prefix))'' OR param_company_name_prefix = (TYPEOF(le.company_name_prefix))'' => 0,
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.company_name_prefix_match_code := MAP(le.company_name_prefix = (TYPEOF(le.company_name_prefix))'' OR le.company_name_prefix = (TYPEOF(le.company_name_prefix))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_company_name_prefix(le.company_name_prefix,ri.company_name_prefix,TRUE));
    SELF.company_name_prefixWeight := (50+MAP ( le.company_name_prefix = ri.company_name_prefix  => le.company_name_prefix_weight100,
          le.company_name_prefix = (TYPEOF(le.company_name_prefix))'' OR ri.company_name_prefix = (TYPEOF(le.company_name_prefix))'' => 0,
          -0.915*le.company_name_prefix_weight100))/100*0.10; 
  ...
  J0 := JOIN(Recs1,Key,LEFT.company_name_prefix = RIGHT.company_name_prefix
    ...
    ATMOST(LEFT.company_name_prefix = RIGHT.company_name_prefix,/*HACK01*/2000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.company_name_prefix = RIGHT.company_name_prefix
     AND ( LEFT.cnp_name = (TYPEOF(LEFT.cnp_name))'' OR RIGHT.cnp_name = (TYPEOF(RIGHT.cnp_name))'' OR SALT33.MatchBagOfWords(LEFT.cnp_name,RIGHT.cnp_name,3177747,1) > BizLinkFull.Config_BIP.cnp_name_Force * 100 )
     AND ( LEFT.zip_cases[1].zip = (TYPEOF(LEFT.zip_cases[1].zip))'' OR RIGHT.zip = (TYPEOF(RIGHT.zip))'' OR LEFT.zip_cases[1].zip = RIGHT.zip  )
     AND ( LEFT.city = (TYPEOF(LEFT.city))'' OR RIGHT.city = (TYPEOF(RIGHT.city))'' OR metaphonelib.DMetaPhone1(LEFT.city)=metaphonelib.DMetaPhone1(RIGHT.city) OR SALT33.WithinEditN(LEFT.city,RIGHT.city,2, 0)  )
     AND ( LEFT.st = (TYPEOF(LEFT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'' OR LEFT.st = RIGHT.st  ),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.company_name_prefix = RIGHT.company_name_prefix,/*HACK01*/2000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_company_name_prefix='',Input_cnp_name='',Input_zip='',Input_city='',Input_st='',Input_company_sic_code1='',Input_cnp_number='',Input_cnp_btype='',Input_cnp_lowv='',Input_prim_range='',Input_sec_range='',Input_parent_proxid='',Input_sele_proxid='',Input_org_proxid='',Input_ultimate_proxid='',Input_sele_flag='',Input_org_flag='',Input_ult_flag='',output_file,AsIndex='true') := MACRO
IMPORT SALT33,BizLinkFull;
#IF(#TEXT(Input_company_name_prefix)<>'')
  #uniquename(trans)
  PRTE2_BIPV2_BusHeader.Key_BizHead_L_CNPNAME_FUZZY.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE2_BIPV2_BusHeader.Key_BizHead_L_CNPNAME_FUZZY.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

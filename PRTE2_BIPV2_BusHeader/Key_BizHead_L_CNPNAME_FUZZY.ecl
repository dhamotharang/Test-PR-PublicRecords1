...
EXPORT CanSearch(PRTE2_BIPV2_BusHeader.Process_Biz_Layouts.InputLayout le) := le.company_name_prefix <> (TYPEOF(le.company_name_prefix))'' AND BizLinkFull.Fields.InValid_company_name_prefix((SALT311.StrType)le.company_name_prefix)=0;
KeyRec := RECORDOF(Key);
...
// Why not LOOP? - Because I am expecting FIRST one to win 990f the time - and don't want to impact it
  ...
  Returnable(DATASET(RECORDOF(RawData0)) d) := COUNT(NOFOLD(d))<>1 OR EXISTS(NOFOLD(d((SALT311.StrType)company_name_prefix != '')));
  res := MAP (

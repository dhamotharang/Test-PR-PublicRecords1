...
EXPORT ScoredproxidFetch(TYPEOF(h.contact_did) param_contact_did = (TYPEOF(h.contact_did))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.contact_did_match_code := MAP(le.contact_did = (TYPEOF(le.contact_did))'' OR le.contact_did = (TYPEOF(le.contact_did))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_contact_did(le.contact_did,param_contact_did,TRUE));
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.contact_did_match_code := MAP(le.contact_did = (TYPEOF(le.contact_did))'' OR le.contact_did = (TYPEOF(le.contact_did))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_contact_did(le.contact_did,ri.contact_did,TRUE));
...
#IF(#TEXT(Input_contact_did)<>'')
  ...
  output_file := PRTE2_BIPV2_BusHeader.Key_BizHead_L_CONTACT_DID.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

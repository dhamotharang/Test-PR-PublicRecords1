...
//contact_email:+:company_sic_code1:cnp_name:cnp_number:cnp_btype:cnp_lowv:zip:prim_name:city:st:prim_range:sec_range:parent_proxid:sele_proxid:org_proxid:ultimate_proxid:sele_flag:org_flag:ult_flag
 
EXPORT KeyName := PRTE2_BIPV2_BusHeader.Filename_keys.L_EMAIL; /*HACK07*/
...
DataForKey0 := DEDUP(SORT(TABLE(h((contact_email NOT IN SET(s.nulls_contact_email,contact_email) AND contact_email <> (TYPEOF(contact_email))'')),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,proxid,contact_email,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'L_EMAIL','company_sic_code1',CntRed_company_sic_code1*100,CntRed_company_sic_code1*TSize},{'L_EMAIL','cnp_name',CntRed_cnp_name*100,CntRed_cnp_name*TSize},{'L_EMAIL','cnp_number',CntRed_cnp_number*100,CntRed_cnp_number*TSize},{'L_EMAIL','cnp_btype',CntRed_cnp_btype*100,CntRed_cnp_btype*TSize},{'L_EMAIL','cnp_lowv',CntRed_cnp_lowv*100,CntRed_cnp_lowv*TSize},{'L_EMAIL','zip',CntRed_zip*100,CntRed_zip*TSize},{'L_EMAIL','prim_name',CntRed_prim_name*100,CntRed_prim_name*TSize},{'L_EMAIL','city',CntRed_city*100,CntRed_city*TSize},{'L_EMAIL','st',CntRed_st*100,CntRed_st*TSize},{'L_EMAIL','prim_range',CntRed_prim_range*100,CntRed_prim_range*TSize},{'L_EMAIL','sec_range',CntRed_sec_range*100,CntRed_sec_range*TSize},{'L_EMAIL','parent_proxid',CntRed_parent_proxid*100,CntRed_parent_proxid*TSize},{'L_EMAIL','sele_proxid',CntRed_sele_proxid*100,CntRed_sele_proxid*TSize},{'L_EMAIL','org_proxid',CntRed_org_proxid*100,CntRed_org_proxid*TSize},{'L_EMAIL','ultimate_proxid',CntRed_ultimate_proxid*100,CntRed_ultimate_proxid*TSize},{'L_EMAIL','sele_flag',CntRed_sele_flag*100,CntRed_sele_flag*TSize},{'L_EMAIL','org_flag',CntRed_org_flag*100,CntRed_org_flag*TSize},{'L_EMAIL','ult_flag',CntRed_ult_flag*100,CntRed_ult_flag*TSize}],SALT33.ShrinkLayout);
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.contact_email <> (typeof(le.contact_email))'';
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch_server(TYPEOF(h.contact_email) param_contact_email = (TYPEOF(h.contact_email))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := 
    STEPPED( LIMIT( Key(
          KEYED(( contact_email = param_contact_email AND param_contact_email <> (TYPEOF(contact_email))''))
...
EXPORT RawFetch(TYPEOF(h.contact_email) param_contact_email = (TYPEOF(h.contact_email))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
// Why not LOOP? - Because I am expecting FIRST one to win 990f the time - and don't want to impact it
  RawData0 := RawFetch_server(param_contact_email,0);
  RawData1 := RawFetch_server(param_contact_email,1);
  RawData2 := RawFetch_server(param_contact_email,2);
  Returnable(DATASET(RECORDOF(RawData0)) d) := COUNT(NOFOLD(d))<>1 OR EXISTS(NOFOLD(d((SALT33.StrType)contact_email != '')));
  res := MAP (
...
EXPORT ScoredproxidFetch(TYPEOF(h.contact_email) param_contact_email = (TYPEOF(h.contact_email))'',TYPEOF(h.company_sic_code1) param_company_sic_code1 = (TYPEOF(h.company_sic_code1))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.cnp_number) param_cnp_number = (TYPEOF(h.cnp_number))'',TYPEOF(h.cnp_btype) param_cnp_btype = (TYPEOF(h.cnp_btype))'',TYPEOF(h.cnp_lowv) param_cnp_lowv = (TYPEOF(h.cnp_lowv))'',DATASET(process_Biz_layouts.layout_zip_cases) param_zip,TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.parent_proxid) param_parent_proxid = (TYPEOF(h.parent_proxid))'',TYPEOF(h.sele_proxid) param_sele_proxid = (TYPEOF(h.sele_proxid))'',TYPEOF(h.org_proxid) param_org_proxid = (TYPEOF(h.org_proxid))'',TYPEOF(h.ultimate_proxid) param_ultimate_proxid = (TYPEOF(h.ultimate_proxid))'',TYPEOF(h.sele_flag) param_sele_flag = (TYPEOF(h.sele_flag))'',TYPEOF(h.org_flag) param_org_flag = (TYPEOF(h.org_flag))'',TYPEOF(h.ult_flag) param_ult_flag = (TYPEOF(h.ult_flag))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
  RawData := RawFetch(param_contact_email,param_fallback_value);
 
  Process_Biz_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    SELF.keys_used := 1 << 13; // Set bitmap for key used
    SELF.keys_failed := IF(le.proxid = 0 AND le.seleid = 0 AND le.orgid = 0 AND le.ultid = 0, 1 << 13, 0); // Set bitmap for key failed
    SELF.contact_email_match_code := MAP(le.contact_email = (TYPEOF(le.contact_email))'' OR le.contact_email = (TYPEOF(le.contact_email))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_contact_email(le.contact_email,param_contact_email,TRUE));
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.contact_email_match_code := MAP(le.contact_email = (TYPEOF(le.contact_email))'' OR le.contact_email = (TYPEOF(le.contact_email))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_contact_email(le.contact_email,ri.contact_email,TRUE));
  ...
  J0 := JOIN(Recs1,Key,LEFT.contact_email = RIGHT.contact_email,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.contact_email = RIGHT.contact_email,/*HACK01*/2000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.contact_email = RIGHT.contact_email,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.contact_email = RIGHT.contact_email,/*HACK01*/2000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_contact_email='',Input_company_sic_code1='',Input_cnp_name='',Input_cnp_number='',Input_cnp_btype='',Input_cnp_lowv='',Input_zip='',Input_prim_name='',Input_city='',Input_st='',Input_prim_range='',Input_sec_range='',Input_parent_proxid='',Input_sele_proxid='',Input_org_proxid='',Input_ultimate_proxid='',Input_sele_flag='',Input_org_flag='',Input_ult_flag='',output_file,AsIndex='true') := MACRO
IMPORT SALT33,BizLinkFull;
#IF(#TEXT(Input_contact_email)<>'')
  #uniquename(trans)
  PRTE2_BIPV2_BusHeader.Key_BizHead_L_EMAIL.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE2_BIPV2_BusHeader.Key_BizHead_L_EMAIL.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

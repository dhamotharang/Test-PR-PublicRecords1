...
EXPORT KeyName := PRTE2_BIPV2_BusHeader.Filename_keys.L_URL; /*HACK07*/
...
// Extra credit fields
  ...
  h.company_url;
  UNSIGNED8 gss_bloom := SALT33.Fn_Wordbag_To_Bloom(h.company_url); // To allow pruning of positives from rawfetch//Scores for various field components
...
DataForKey0 := DEDUP(SORT(TABLE(h((company_url NOT IN SET(s.nulls_company_url,company_url) AND company_url <> (TYPEOF(company_url))'')),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
// Now need to 'blow out' the fixed word-bag fields to create the pseudo-inversion
SALT33.mac_expand_wordbag_key(DataForKey0,GSS_hash,company_url,DataForKey1,GSS_word_weight)
...
EXPORT Shrinkage := DATASET([{'L_URL','company_sic_code1',CntRed_company_sic_code1*100,CntRed_company_sic_code1*TSize},{'L_URL','cnp_name',CntRed_cnp_name*100,CntRed_cnp_name*TSize},{'L_URL','cnp_number',CntRed_cnp_number*100,CntRed_cnp_number*TSize},{'L_URL','cnp_btype',CntRed_cnp_btype*100,CntRed_cnp_btype*TSize},{'L_URL','cnp_lowv',CntRed_cnp_lowv*100,CntRed_cnp_lowv*TSize},{'L_URL','zip',CntRed_zip*100,CntRed_zip*TSize},{'L_URL','prim_name',CntRed_prim_name*100,CntRed_prim_name*TSize},{'L_URL','city',CntRed_city*100,CntRed_city*TSize},{'L_URL','prim_range',CntRed_prim_range*100,CntRed_prim_range*TSize},{'L_URL','sec_range',CntRed_sec_range*100,CntRed_sec_range*TSize},{'L_URL','parent_proxid',CntRed_parent_proxid*100,CntRed_parent_proxid*TSize},{'L_URL','sele_proxid',CntRed_sele_proxid*100,CntRed_sele_proxid*TSize},{'L_URL','org_proxid',CntRed_org_proxid*100,CntRed_org_proxid*TSize},{'L_URL','ultimate_proxid',CntRed_ultimate_proxid*100,CntRed_ultimate_proxid*TSize},{'L_URL','sele_flag',CntRed_sele_flag*100,CntRed_sele_flag*TSize},{'L_URL','org_flag',CntRed_org_flag*100,CntRed_org_flag*TSize},{'L_URL','ult_flag',CntRed_ult_flag*100,CntRed_ult_flag*TSize}],SALT33.ShrinkLayout);
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.company_url <> (typeof(le.company_url))'';
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch_server(TYPEOF(h.company_url) param_company_url = (TYPEOF(h.company_url))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := 
  FUNCTION
 //Generate service attributes for GSS join
    wds := SALT33.fn_string_to_wordstream(param_company_url);
    indexOutputRecord := RECORDOF(Key);
    slimrec := { Key.gss_word_weight, Key.proxid, Key.seleid, Key.orgid, Key.ultid };
    BloomF := SALT33.Fn_Wordbag_To_Bloom(param_company_url); // Use for extra index filtering
...
EXPORT RawFetch(TYPEOF(h.company_url) param_company_url = (TYPEOF(h.company_url))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
// Why not LOOP? - Because I am expecting FIRST one to win 990f the time - and don't want to impact it
  RawData0 := RawFetch_server(param_company_url,param_st,0);
  RawData1 := RawFetch_server(param_company_url,param_st,1);
  RawData2 := RawFetch_server(param_company_url,param_st,2);
  Returnable(DATASET(RECORDOF(RawData0)) d) := COUNT(NOFOLD(d))<>1 OR EXISTS(NOFOLD(d((SALT33.StrType)company_url != '')));
  res := MAP (
...
EXPORT ScoredproxidFetch(TYPEOF(h.company_url) param_company_url = (TYPEOF(h.company_url))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.company_sic_code1) param_company_sic_code1 = (TYPEOF(h.company_sic_code1))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.cnp_number) param_cnp_number = (TYPEOF(h.cnp_number))'',TYPEOF(h.cnp_btype) param_cnp_btype = (TYPEOF(h.cnp_btype))'',TYPEOF(h.cnp_lowv) param_cnp_lowv = (TYPEOF(h.cnp_lowv))'',DATASET(process_Biz_layouts.layout_zip_cases) param_zip,TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.parent_proxid) param_parent_proxid = (TYPEOF(h.parent_proxid))'',TYPEOF(h.sele_proxid) param_sele_proxid = (TYPEOF(h.sele_proxid))'',TYPEOF(h.org_proxid) param_org_proxid = (TYPEOF(h.org_proxid))'',TYPEOF(h.ultimate_proxid) param_ultimate_proxid = (TYPEOF(h.ultimate_proxid))'',TYPEOF(h.sele_flag) param_sele_flag = (TYPEOF(h.sele_flag))'',TYPEOF(h.org_flag) param_org_flag = (TYPEOF(h.org_flag))'',TYPEOF(h.ult_flag) param_ult_flag = (TYPEOF(h.ult_flag))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
  RawData := RawFetch(param_company_url,param_st,param_fallback_value);
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.company_url_match_code := BizLinkFull.match_methods(File_BizHead).match_company_url(le.company_url,param_company_url,TRUE);
  ...
  SELF.company_url_gss_cases := DATASET([{le.gss_hash}],SALT33.layout_GSS_cases);
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.company_url_match_code := BizLinkFull.match_methods(File_BizHead).match_company_url(le.company_url,ri.company_url,TRUE);
    SELF.company_urlWeight := IF(SALT33.MatchBagOfWords(le.company_url,ri.company_url,31744,0)<=BizLinkFull.Config_BIP.company_url_force,SKIP,SALT33.MatchBagOfWords(le.company_url,ri.company_url,31744,0)/100); 
  ...
  J0 := JOIN(Recs1,Key,HASH32(SALT33.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash AND SALT33.MatchBagOfWords(LEFT.company_url,RIGHT.company_url,31744,0) > BizLinkFull.Config_BIP.company_url_Force * 100
     AND ( LEFT.st = (TYPEOF(LEFT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'' OR LEFT.st = RIGHT.st  ),Score_Batch(RIGHT,LEFT),
    ATMOST(HASH32(SALT33.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash,/*HACK01*/2000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),HASH32(SALT33.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash AND SALT33.MatchBagOfWords(LEFT.company_url,RIGHT.company_url,31744,0) > BizLinkFull.Config_BIP.company_url_Force * 100
     AND ( LEFT.st = (TYPEOF(LEFT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'' OR LEFT.st = RIGHT.st  ),Score_Batch(RIGHT,LEFT),
    ATMOST(HASH32(SALT33.fn_bow_bestword(LEFT.company_url))=RIGHT.gss_hash,/*HACK01*/2000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_company_url='',Input_st='',Input_company_sic_code1='',Input_cnp_name='',Input_cnp_number='',Input_cnp_btype='',Input_cnp_lowv='',Input_zip='',Input_prim_name='',Input_city='',Input_prim_range='',Input_sec_range='',Input_parent_proxid='',Input_sele_proxid='',Input_org_proxid='',Input_ultimate_proxid='',Input_sele_flag='',Input_org_flag='',Input_ult_flag='',output_file,AsIndex='true') := MACRO
IMPORT SALT33,BizLinkFull;
#IF(#TEXT(Input_company_url)<>'')
  #uniquename(trans)
  PRTE2_BIPV2_BusHeader.Key_BizHead_L_URL.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE2_BIPV2_BusHeader.Key_BizHead_L_URL.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

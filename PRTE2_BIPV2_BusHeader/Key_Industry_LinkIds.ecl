import BIPV2, AutoStandardI, tools;

export Key_Industry_LinkIds := MODULE

   // DEFINE THE INDEX
	shared superfile_name		:= keynames().Industry_LinkIds.qa;

	shared Base				:= Files().base.Industry.built(siccode<>'' or
																									 naics<>'' or
	...
	export Key := k;
  export keyvs(string pversion = '' ,boolean	pUseOtherEnvironment = false) := tools.macf_FilesIndex('Key' ,keynames(pversion,pUseOtherEnvironment).Industry_LinkIds);

...
EXPORT KeyName := PRTE2_BIPV2_BusHeader.Filename_keys.L_CONTACT_SSN; /*HACK07*/
...
DataForKey0 := DEDUP(SORT(TABLE(h((contact_ssn NOT IN SET(s.nulls_contact_ssn,contact_ssn) AND contact_ssn <> (TYPEOF(contact_ssn))'')),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,proxid,contact_ssn,LOCAL); // Not perfect, does not need to be - for statistics
  Rem_contact_email := GROUP( DEDUP( SORT( Grpd, EXCEPT contact_email), EXCEPT contact_email));
  CntRed_contact_email := (KeyCnt-COUNT(Rem_contact_email))/KeyCnt;
...
EXPORT RawFetch_server(TYPEOF(h.contact_ssn) param_contact_ssn = (TYPEOF(h.contact_ssn))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := 
    STEPPED( LIMIT( Key(
          KEYED(( contact_ssn = param_contact_ssn AND param_contact_ssn <> (TYPEOF(contact_ssn))''))
...
EXPORT RawFetch(TYPEOF(h.contact_ssn) param_contact_ssn = (TYPEOF(h.contact_ssn))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
// Why not LOOP? - Because I am expecting FIRST one to win 990f the time - and don't want to impact it
  ...
  Returnable(DATASET(RECORDOF(RawData0)) d) := COUNT(NOFOLD(d))<>1 OR EXISTS(NOFOLD(d((SALT33.StrType)contact_ssn != '')));
  res := MAP (
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.contact_ssn_match_code := MAP(le.contact_ssn = (TYPEOF(le.contact_ssn))'' OR le.contact_ssn = (TYPEOF(le.contact_ssn))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_contact_ssn(le.contact_ssn,ri.contact_ssn,TRUE));
    SELF.contact_ssnWeight := (50+MAP ( le.contact_ssn = ri.contact_ssn  => le.contact_ssn_weight100,
          le.contact_ssn = (TYPEOF(le.contact_ssn))'' OR ri.contact_ssn = (TYPEOF(le.contact_ssn))'' => 0,
          -0.866*le.contact_ssn_weight100))/100; 
    SELF.contact_email_match_code := MAP(le.contact_email = (TYPEOF(le.contact_email))'' OR ri.contact_email = (TYPEOF(ri.contact_email))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_contact_email(le.contact_email,ri.contact_email,FALSE));
    SELF.contact_emailWeight := (50+MAP ( le.contact_email = (TYPEOF(le.contact_email))'' OR ri.contact_email = (TYPEOF(ri.contact_email))'' => 0,
  ...
  J0 := JOIN(Recs1,Key,LEFT.contact_ssn = RIGHT.contact_ssn,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.contact_ssn = RIGHT.contact_ssn,/*HACK01*/2000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.contact_ssn = RIGHT.contact_ssn,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.contact_ssn = RIGHT.contact_ssn,/*HACK01*/2000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
...
#IF(#TEXT(Input_contact_ssn)<>'')
  #uniquename(trans)
  BizLinkFull.Key_BizHead_L_CONTACT_SSN.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE2_BIPV2_BusHeader.Key_BizHead_L_CONTACT_SSN.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

...
EXPORT Key_BizHead_L_ADDRESS1 := MODULE
 
//prim_name:city:st:?:prim_range:cnp_name:+:zip:company_sic_code1:cnp_number:cnp_btype:cnp_lowv:sec_range:parent_proxid:sele_proxid:org_proxid:ultimate_proxid:sele_flag:org_flag:ult_flag
 
EXPORT KeyName := PRTE2_BIPV2_BusHeader.Filename_keys.L_ADDRESS1; /*HACK07*/
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,proxid,prim_name,city,st,prim_range,cnp_name,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'L_ADDRESS1','zip',CntRed_zip*100,CntRed_zip*TSize},{'L_ADDRESS1','company_sic_code1',CntRed_company_sic_code1*100,CntRed_company_sic_code1*TSize},{'L_ADDRESS1','cnp_number',CntRed_cnp_number*100,CntRed_cnp_number*TSize},{'L_ADDRESS1','cnp_btype',CntRed_cnp_btype*100,CntRed_cnp_btype*TSize},{'L_ADDRESS1','cnp_lowv',CntRed_cnp_lowv*100,CntRed_cnp_lowv*TSize},{'L_ADDRESS1','sec_range',CntRed_sec_range*100,CntRed_sec_range*TSize},{'L_ADDRESS1','parent_proxid',CntRed_parent_proxid*100,CntRed_parent_proxid*TSize},{'L_ADDRESS1','sele_proxid',CntRed_sele_proxid*100,CntRed_sele_proxid*TSize},{'L_ADDRESS1','org_proxid',CntRed_org_proxid*100,CntRed_org_proxid*TSize},{'L_ADDRESS1','ultimate_proxid',CntRed_ultimate_proxid*100,CntRed_ultimate_proxid*TSize},{'L_ADDRESS1','sele_flag',CntRed_sele_flag*100,CntRed_sele_flag*TSize},{'L_ADDRESS1','org_flag',CntRed_org_flag*100,CntRed_org_flag*TSize},{'L_ADDRESS1','ult_flag',CntRed_ult_flag*100,CntRed_ult_flag*TSize}],SALT33.ShrinkLayout);
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.prim_name <> (typeof(le.prim_name))'' AND le.city <> (typeof(le.city))'' AND le.st <> (typeof(le.st))'';
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch_server(TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := 
    STEPPED( LIMIT( Key(
          KEYED(( prim_name = param_prim_name AND param_prim_name <> (TYPEOF(prim_name))''))
      AND KEYED(( city = param_city AND param_city <> (TYPEOF(city))''))
      ...
      AND ( SALT33.MatchBagOfWords(cnp_name,param_cnp_name,3177747,1)+400 /*HACK08*/ > BizLinkFull.Config_BIP.cnp_name_Force * 100 OR cnp_name = (TYPEOF(cnp_name))'' OR param_cnp_name = (TYPEOF(cnp_name))'' )
...
EXPORT RawFetch(TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
// Why not LOOP? - Because I am expecting FIRST one to win 990f the time - and don't want to impact it
  RawData0 := RawFetch_server(param_prim_name,param_city,param_st,param_prim_range,param_cnp_name,0);
  RawData1 := RawFetch_server(param_prim_name,param_city,param_st,param_prim_range,param_cnp_name,1);
  RawData2 := RawFetch_server(param_prim_name,param_city,param_st,param_prim_range,param_cnp_name,2);
  Returnable(DATASET(RECORDOF(RawData0)) d) := COUNT(NOFOLD(d))<>1 OR EXISTS(NOFOLD(d((SALT33.StrType)prim_name != '')));
  res := MAP (
...
EXPORT ScoredproxidFetch(TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',DATASET(process_Biz_layouts.layout_zip_cases) param_zip,TYPEOF(h.company_sic_code1) param_company_sic_code1 = (TYPEOF(h.company_sic_code1))'',TYPEOF(h.cnp_number) param_cnp_number = (TYPEOF(h.cnp_number))'',TYPEOF(h.cnp_btype) param_cnp_btype = (TYPEOF(h.cnp_btype))'',TYPEOF(h.cnp_lowv) param_cnp_lowv = (TYPEOF(h.cnp_lowv))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.parent_proxid) param_parent_proxid = (TYPEOF(h.parent_proxid))'',TYPEOF(h.sele_proxid) param_sele_proxid = (TYPEOF(h.sele_proxid))'',TYPEOF(h.org_proxid) param_org_proxid = (TYPEOF(h.org_proxid))'',TYPEOF(h.ultimate_proxid) param_ultimate_proxid = (TYPEOF(h.ultimate_proxid))'',TYPEOF(h.sele_flag) param_sele_flag = (TYPEOF(h.sele_flag))'',TYPEOF(h.org_flag) param_org_flag = (TYPEOF(h.org_flag))'',TYPEOF(h.ult_flag) param_ult_flag = (TYPEOF(h.ult_flag))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
  RawData := RawFetch(param_prim_name,param_city,param_st,param_prim_range,param_cnp_name,param_fallback_value);
 
  Process_Biz_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    SELF.keys_used := 1 << 5; // Set bitmap for key used
    SELF.keys_failed := IF(le.proxid = 0 AND le.seleid = 0 AND le.orgid = 0 AND le.ultid = 0, 1 << 5, 0); // Set bitmap for key failed
    SELF.prim_name_match_code := MAP(le.prim_name = (TYPEOF(le.prim_name))'' OR le.prim_name = (TYPEOF(le.prim_name))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_prim_name(le.prim_name,param_prim_name,0,0,TRUE));
    ...
    SELF.city_match_code := MAP(le.city = (TYPEOF(le.city))'' OR le.city = (TYPEOF(le.city))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_city(le.city,param_city,0,0,TRUE));
    SELF.cityWeight := (50+MAP ( le.city = param_city  => le.city_weight100,
          le.city = (TYPEOF(le.city))'' OR param_city = (TYPEOF(le.city))'' => 0,
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.prim_name_match_code := MAP(le.prim_name = (TYPEOF(le.prim_name))'' OR le.prim_name = (TYPEOF(le.prim_name))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_prim_name(le.prim_name,ri.prim_name,0,0,TRUE));
    SELF.prim_nameWeight := (50+MAP ( le.prim_name = ri.prim_name  => le.prim_name_weight100,
          ...
          -1.000*le.prim_name_weight100))/100; 
    SELF.city_match_code := MAP(le.city = (TYPEOF(le.city))'' OR le.city = (TYPEOF(le.city))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_city(le.city,ri.city,0,0,TRUE));
    SELF.cityWeight := (50+MAP ( le.city = ri.city  => le.city_weight100,
          le.city = (TYPEOF(le.city))'' OR ri.city = (TYPEOF(le.city))'' => 0,
          -0.947*le.city_weight100))/100; 
    ...
    SELF := le;
  END;
  Recs0 := Recs(prim_name <> (typeof(prim_name))'',city <> (typeof(city))'',st <> (typeof(st))'');
  ...
  J0 := JOIN(Recs1,Key,LEFT.prim_name = RIGHT.prim_name
     ...
     AND ( LEFT.cnp_name = (TYPEOF(LEFT.cnp_name))'' OR RIGHT.cnp_name = (TYPEOF(RIGHT.cnp_name))'' OR SALT33.MatchBagOfWords(LEFT.cnp_name,RIGHT.cnp_name,3177747,1) > BizLinkFull.Config_BIP.cnp_name_Force * 100 ),Score_Batch(RIGHT,LEFT),
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_prim_name='',Input_city='',Input_st='',Input_prim_range='',Input_cnp_name='',Input_zip='',Input_company_sic_code1='',Input_cnp_number='',Input_cnp_btype='',Input_cnp_lowv='',Input_sec_range='',Input_parent_proxid='',Input_sele_proxid='',Input_org_proxid='',Input_ultimate_proxid='',Input_sele_flag='',Input_org_flag='',Input_ult_flag='',output_file,AsIndex='true') := MACRO
IMPORT SALT33,BizLinkFull;
#IF(#TEXT(Input_prim_name)<>'' AND #TEXT(Input_city)<>'' AND #TEXT(Input_st)<>'')
  #uniquename(trans)
  PRTE2_BIPV2_BusHeader.Key_BizHead_L_ADDRESS1.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE2_BIPV2_BusHeader.Key_BizHead_L_ADDRESS1.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

...
EXPORT InputToMC(DATASET(InputLayout) Inp) := FUNCTION
  r := RECORD
    ...
    inp.lname;
    inp.name_suffix;
    STRING5 name_suffix_NormSuffix := BizLinkFull.fn_normSuffix(inp.name_suffix);
    inp.contact_ssn;
    inp.contact_email;
    inp.sele_flag;
    inp.org_flag;
...
EXPORT LayoutScoredFetch update_forcefailed(LayoutScoredFetch le) := TRANSFORM
  SELF.ForceFailed := (le.prim_rangeWeight < BizLinkFull.Config_BIP.prim_range_Force);
  SELF := le;
END;
...
EXPORT LayoutScoredFetch combine_scores(LayoutScoredFetch le,LayoutScoredFetch ri) := TRANSFORM
  ...
  SELF.ForceFailed := (SELF.prim_rangeWeight < BizLinkFull.Config_BIP.prim_range_Force)/*HACK03*/ OR SELF.cnp_nameWeight < 4;

...
EXPORT CanSearch(PRTE2_BIPV2_BusHeader.Process_Biz_Layouts.InputLayout le) := le.company_phone_7 <> (TYPEOF(le.company_phone_7))'' AND BizLinkFull.Fields.InValid_company_phone_7((SALT311.StrType)le.company_phone_7)=0;
KeyRec := RECORDOF(Key);
...
// Why not LOOP? - Because I am expecting FIRST one to win 990f the time - and don't want to impact it
  ...
  Returnable(DATASET(RECORDOF(RawData0)) d) := COUNT(NOFOLD(d))<>1 OR EXISTS(NOFOLD(d((SALT311.StrType)company_phone_7 != '')));
  res := MAP (

IMPORT BIPV2, BIPV2_Best, AutoStandardI, BizLinkFull, tools;
EXPORT Key_Best_LinkIds := MODULE
  // DEFINE THE INDEX
	shared superfile_name		      := keynames(,pUseOtherEnvironment := tools._Constants.IsDataland).Best_LinkIds.qa;
	shared superfile_name_Built		:= keynames(,pUseOtherEnvironment := tools._Constants.IsDataland).Best_LinkIds.built;
	shared superfile_name_father	:= keynames(,pUseOtherEnvironment := tools._Constants.IsDataland).Best_LinkIds.father;
	shared Base_prep			:= PRTE2_BIPV2_BusHeader.Files().Base.BestFile.Built;
  shared ds_commonbase  := table(PRTE2_BIPV2_BusHeader.File_DS_CLEAN,{seleid,seleid_status_private},seleid,seleid_status_private,merge);
  shared Base           := join(Base_prep ,ds_commonbase  ,left.seleid = right.seleid,transform(
	...
	shared dkeybuild	:= project(Base, transform(BIPV2_Best.layouts.key, self := left, self := []));
	...
	shared dkeybuild_static	:= project(Base, transform(BIPV2_Best.layouts.key_static, self := left, self := []));

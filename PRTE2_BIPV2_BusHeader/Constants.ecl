...
_PRTE_BUILD := true;  // CONFIGURE HEADER FOR PRTE BUILD BY CHNAGING THIS VALUE.

/* How to use:
   ----------
   1) Add the following 2 lines to the attribute that will conditionally input PRTE data:
   import PRTE2_BIPV2_BusHeader;
   #IF (PRTE2_BIPV2_BusHeader.constants.PRTE_BUILD) #WARNING(PRTE2_BIPV2_BusHeader.constants.PRTE_BUILD_WARN_MSG);
...
EXPORT Constants := MODULE
	      ...
	      str := '** WARNING: PRTE BUILD!! (CHECK: PRTE2_BIPV2_BusHeader.constants _PRTE_BUILD) ** ';
        EXPORT PRTE_BUILD_WARN_MSG  := str+str+str+str+str+str+str+str+str+str;
        #IF (_PRTE_BUILD) #WARNING(PRTE_BUILD_WARN_MSG) #END;
        EXPORT check_prte_build := '#IF (_PRTE_BUILD) #WARNING(PRTE2_BIPV2_BusHeader.constants.PRTE_BUILD_WARN_MSG) #END';

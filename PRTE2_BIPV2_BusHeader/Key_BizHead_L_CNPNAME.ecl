...
// Extra credit fields
  ...
  h.cnp_name;
  UNSIGNED8 gss_bloom := SALT33.Fn_Wordbag_To_Bloom(h.cnp_name); // To allow pruning of positives from rawfetch//Scores for various field components
...
DataForKey0 := DEDUP(SORT(TABLE(h((cnp_name NOT IN SET(s.nulls_cnp_name,cnp_name) AND cnp_name <> (TYPEOF(cnp_name))''),(zip NOT IN SET(s.nulls_zip,zip) AND zip <> (TYPEOF(zip))'')),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
// Now need to 'blow out' the fixed word-bag fields to create the pseudo-inversion
SALT33.mac_expand_wordbag_key(DataForKey0,GSS_hash,cnp_name,DataForKey1,GSS_word_weight)
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,proxid,cnp_name,prim_name,st,city,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'L_CNPNAME','company_sic_code1',CntRed_company_sic_code1*100,CntRed_company_sic_code1*TSize},{'L_CNPNAME','cnp_number',CntRed_cnp_number*100,CntRed_cnp_number*TSize},{'L_CNPNAME','cnp_btype',CntRed_cnp_btype*100,CntRed_cnp_btype*TSize},{'L_CNPNAME','cnp_lowv',CntRed_cnp_lowv*100,CntRed_cnp_lowv*TSize},{'L_CNPNAME','prim_range',CntRed_prim_range*100,CntRed_prim_range*TSize},{'L_CNPNAME','sec_range',CntRed_sec_range*100,CntRed_sec_range*TSize},{'L_CNPNAME','parent_proxid',CntRed_parent_proxid*100,CntRed_parent_proxid*TSize},{'L_CNPNAME','sele_proxid',CntRed_sele_proxid*100,CntRed_sele_proxid*TSize},{'L_CNPNAME','org_proxid',CntRed_org_proxid*100,CntRed_org_proxid*TSize},{'L_CNPNAME','ultimate_proxid',CntRed_ultimate_proxid*100,CntRed_ultimate_proxid*TSize},{'L_CNPNAME','sele_flag',CntRed_sele_flag*100,CntRed_sele_flag*TSize},{'L_CNPNAME','org_flag',CntRed_org_flag*100,CntRed_org_flag*TSize},{'L_CNPNAME','ult_flag',CntRed_ult_flag*100,CntRed_ult_flag*TSize},{'L_CNPNAME','zip',CntRed_zip*100,CntRed_zip*TSize}],SALT33.ShrinkLayout);
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.cnp_name <> (typeof(le.cnp_name))'';
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch_server(TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := 
  FUNCTION
 //Generate service attributes for GSS join
    wds := SALT33.fn_string_to_wordstream(param_cnp_name);
    indexOutputRecord := RECORDOF(Key);
    slimrec := { Key.gss_word_weight, Key.proxid, Key.seleid, Key.orgid, Key.ultid };
    BloomF := SALT33.Fn_Wordbag_To_Bloom(param_cnp_name); // Use for extra index filtering
    doIndexRead(UNSIGNED4 search,UNSIGNED2 spc) := STEPPED(KEY( KEYED(GSS_hash = search) AND (GSS_bloom & BloomF) = BloomF
      ...
      AND ( city = (TYPEOF(city))'' OR param_city = (TYPEOF(city))'' OR metaphonelib.DMetaPhone1(city)=metaphonelib.DMetaPhone1(param_city) OR SALT33.WithinEditN(city,param_city,2, 0) )),ultid,orgid,seleid,proxid,PRIORITY(40-spc)); // Filter for each row of index fetch
    SALT33.MAC_collate_wordbag_matches4(wds,slimrec,doIndexRead,ultid,orgid,seleid,proxid,steppedmatches) // Perform N-way join
    res := JOIN( steppedmatches, Key, KEYED(RIGHT.GSS_Hash = wds[1].hsh) AND WILD(RIGHT.fallback_value) AND KEYED(LEFT.proxid = RIGHT.proxid AND LEFT.seleid = RIGHT.seleid AND LEFT.orgid = RIGHT.orgid AND LEFT.ultid = RIGHT.ultid),TRANSFORM(indexOutputRecord,SELF.gss_word_weight := LEFT.gss_word_weight,SELF := RIGHT));
...
EXPORT RawFetch(TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
// Why not LOOP? - Because I am expecting FIRST one to win 990f the time - and don't want to impact it
  RawData0 := RawFetch_server(param_cnp_name,param_prim_name,param_st,param_city,0);
  RawData1 := RawFetch_server(param_cnp_name,param_prim_name,param_st,param_city,1);
  RawData2 := RawFetch_server(param_cnp_name,param_prim_name,param_st,param_city,2);
  Returnable(DATASET(RECORDOF(RawData0)) d) := COUNT(NOFOLD(d))<>1 OR EXISTS(NOFOLD(d((SALT33.StrType)cnp_name != '')));
  res := MAP (
...
EXPORT ScoredproxidFetch(TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.company_sic_code1) param_company_sic_code1 = (TYPEOF(h.company_sic_code1))'',TYPEOF(h.cnp_number) param_cnp_number = (TYPEOF(h.cnp_number))'',TYPEOF(h.cnp_btype) param_cnp_btype = (TYPEOF(h.cnp_btype))'',TYPEOF(h.cnp_lowv) param_cnp_lowv = (TYPEOF(h.cnp_lowv))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.parent_proxid) param_parent_proxid = (TYPEOF(h.parent_proxid))'',TYPEOF(h.sele_proxid) param_sele_proxid = (TYPEOF(h.sele_proxid))'',TYPEOF(h.org_proxid) param_org_proxid = (TYPEOF(h.org_proxid))'',TYPEOF(h.ultimate_proxid) param_ultimate_proxid = (TYPEOF(h.ultimate_proxid))'',TYPEOF(h.sele_flag) param_sele_flag = (TYPEOF(h.sele_flag))'',TYPEOF(h.org_flag) param_org_flag = (TYPEOF(h.org_flag))'',TYPEOF(h.ult_flag) param_ult_flag = (TYPEOF(h.ult_flag))'',DATASET(process_Biz_layouts.layout_zip_cases) param_zip,TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
  RawData := RawFetch(param_cnp_name,param_prim_name,param_st,param_city,param_fallback_value);
 
  Process_Biz_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    SELF.keys_used := 1 << 3; // Set bitmap for key used
    SELF.keys_failed := IF(le.proxid = 0 AND le.seleid = 0 AND le.orgid = 0 AND le.ultid = 0, 1 << 3, 0); // Set bitmap for key failed
    SELF.cnp_name_match_code := BizLinkFull.match_methods(File_BizHead).match_cnp_name(le.cnp_name,param_cnp_name,0,0,TRUE);
  ...
  SELF.cnp_name_gss_cases := DATASET([{le.gss_hash}],SALT33.layout_GSS_cases);
    ...
    SELF.st_match_code := MAP(le.st = (TYPEOF(le.st))'' OR param_st = (TYPEOF(param_st))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_st(le.st,param_st,FALSE));
    ...
    SELF.city_match_code := MAP(le.city = (TYPEOF(le.city))'' OR param_city = (TYPEOF(param_city))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_city(le.city,param_city,0,0,FALSE));
    ...
    SELF.company_sic_code1_match_code := MAP(le.company_sic_code1 = (TYPEOF(le.company_sic_code1))'' OR param_company_sic_code1 = (TYPEOF(param_company_sic_code1))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_company_sic_code1(le.company_sic_code1,param_company_sic_code1,FALSE));
    ...
    SELF.cnp_number_match_code := MAP(le.cnp_number = (TYPEOF(le.cnp_number))'' OR param_cnp_number = (TYPEOF(param_cnp_number))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_cnp_number(le.cnp_number,param_cnp_number,FALSE));
    ...
    SELF.cnp_btype_match_code := MAP(le.cnp_btype = (TYPEOF(le.cnp_btype))'' OR param_cnp_btype = (TYPEOF(param_cnp_btype))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_cnp_btype(le.cnp_btype,param_cnp_btype,FALSE));
    ...
    SELF.cnp_lowv_match_code := MAP(le.cnp_lowv = (TYPEOF(le.cnp_lowv))'' OR param_cnp_lowv = (TYPEOF(param_cnp_lowv))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_cnp_lowv(le.cnp_lowv,param_cnp_lowv,FALSE));
    ...
    SELF.prim_range_match_code := MAP(le.prim_range = (TYPEOF(le.prim_range))'' OR param_prim_range = (TYPEOF(param_prim_range))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_prim_range(le.prim_range,param_prim_range,0,0,FALSE));
    ...
    SELF.sec_range_match_code := MAP(le.sec_range = (TYPEOF(le.sec_range))'' OR param_sec_range = (TYPEOF(param_sec_range))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_sec_range(le.sec_range,param_sec_range,0,0,FALSE));
    ...
    SELF.parent_proxid_match_code := MAP(le.parent_proxid = (TYPEOF(le.parent_proxid))'' OR param_parent_proxid = (TYPEOF(param_parent_proxid))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_parent_proxid(le.parent_proxid,param_parent_proxid,FALSE));
    ...
    SELF.sele_proxid_match_code := MAP(le.sele_proxid = (TYPEOF(le.sele_proxid))'' OR param_sele_proxid = (TYPEOF(param_sele_proxid))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_sele_proxid(le.sele_proxid,param_sele_proxid,FALSE));
    ...
    SELF.org_proxid_match_code := MAP(le.org_proxid = (TYPEOF(le.org_proxid))'' OR param_org_proxid = (TYPEOF(param_org_proxid))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_org_proxid(le.org_proxid,param_org_proxid,FALSE));
    ...
    SELF.ultimate_proxid_match_code := MAP(le.ultimate_proxid = (TYPEOF(le.ultimate_proxid))'' OR param_ultimate_proxid = (TYPEOF(param_ultimate_proxid))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_ultimate_proxid(le.ultimate_proxid,param_ultimate_proxid,FALSE));
    ...
    SELF.sele_flag_match_code := MAP(le.sele_flag = (TYPEOF(le.sele_flag))'' OR param_sele_flag = (TYPEOF(param_sele_flag))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_sele_flag(le.sele_flag,param_sele_flag,FALSE));
    ...
    SELF.org_flag_match_code := MAP(le.org_flag = (TYPEOF(le.org_flag))'' OR param_org_flag = (TYPEOF(param_org_flag))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_org_flag(le.org_flag,param_org_flag,FALSE));
    ...
    SELF.ult_flag_match_code := MAP(le.ult_flag = (TYPEOF(le.ult_flag))'' OR param_ult_flag = (TYPEOF(param_ult_flag))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_ult_flag(le.ult_flag,param_ult_flag,FALSE));
    ...
    SELF.zip_match_code := MAP(le.zip = (TYPEOF(le.zip))'' OR ~EXISTS(param_zip) => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_zip(le.zip,param_zip,FALSE));
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.cnp_name_match_code := BizLinkFull.match_methods(File_BizHead).match_cnp_name(le.cnp_name,ri.cnp_name,0,0,TRUE);
    SELF.cnp_nameWeight := IF(SALT33.MatchBagOfWords(le.cnp_name,ri.cnp_name,3177747,1)<=BizLinkFull.Config_BIP.cnp_name_force,SKIP,SALT33.MatchBagOfWords(le.cnp_name,ri.cnp_name,3177747,1)/100); 
    ...
    SELF.prim_nameWeight := (50+MAP ( le.prim_name = (TYPEOF(le.prim_name))'' OR ri.prim_name = (TYPEOF(ri.prim_name))'' => 0,
           ...
           -1.000*le.prim_name_weight100))/100; 
    SELF.st_match_code := MAP(le.st = (TYPEOF(le.st))'' OR ri.st = (TYPEOF(ri.st))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_st(le.st,ri.st,FALSE));
    SELF.stWeight := (50+MAP ( le.st = (TYPEOF(le.st))'' OR ri.st = (TYPEOF(ri.st))'' => 0,
           le.st = ri.st  => le.st_weight100,
           -1.000*le.st_weight100))/100; 
    SELF.city_match_code := MAP(le.city = (TYPEOF(le.city))'' OR ri.city = (TYPEOF(ri.city))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_city(le.city,ri.city,0,0,FALSE));
    SELF.cityWeight := (50+MAP ( le.city = (TYPEOF(le.city))'' OR ri.city = (TYPEOF(ri.city))'' => 0,
           ...
           -0.947*le.city_weight100))/100; 
    SELF.company_sic_code1_match_code := MAP(le.company_sic_code1 = (TYPEOF(le.company_sic_code1))'' OR ri.company_sic_code1 = (TYPEOF(ri.company_sic_code1))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_company_sic_code1(le.company_sic_code1,ri.company_sic_code1,FALSE));
    SELF.company_sic_code1Weight := (50+MAP ( le.company_sic_code1 = (TYPEOF(le.company_sic_code1))'' OR ri.company_sic_code1 = (TYPEOF(ri.company_sic_code1))'' => 0,
           ...
           -0.727*le.company_sic_code1_weight100))/100; 
    SELF.cnp_number_match_code := MAP(le.cnp_number = (TYPEOF(le.cnp_number))'' OR ri.cnp_number = (TYPEOF(ri.cnp_number))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_cnp_number(le.cnp_number,ri.cnp_number,FALSE));
    SELF.cnp_numberWeight := (50+MAP ( le.cnp_number = (TYPEOF(le.cnp_number))'' OR ri.cnp_number = (TYPEOF(ri.cnp_number))'' => 0,
           ...
           -0.996*le.cnp_number_weight100))/100; 
    SELF.cnp_btype_match_code := MAP(le.cnp_btype = (TYPEOF(le.cnp_btype))'' OR ri.cnp_btype = (TYPEOF(ri.cnp_btype))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_cnp_btype(le.cnp_btype,ri.cnp_btype,FALSE));
    SELF.cnp_btypeWeight := (50+MAP ( le.cnp_btype = (TYPEOF(le.cnp_btype))'' OR ri.cnp_btype = (TYPEOF(ri.cnp_btype))'' => 0,
           ...
           -0.958*le.cnp_btype_weight100))/100; 
    SELF.cnp_lowv_match_code := MAP(le.cnp_lowv = (TYPEOF(le.cnp_lowv))'' OR ri.cnp_lowv = (TYPEOF(ri.cnp_lowv))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_cnp_lowv(le.cnp_lowv,ri.cnp_lowv,FALSE));
    SELF.cnp_lowvWeight := (50+MAP ( le.cnp_lowv = (TYPEOF(le.cnp_lowv))'' OR ri.cnp_lowv = (TYPEOF(ri.cnp_lowv))'' => 0,
           ...
           -0.962*le.cnp_lowv_weight100))/100; 
    SELF.prim_range_match_code := MAP(le.prim_range = (TYPEOF(le.prim_range))'' OR ri.prim_range = (TYPEOF(ri.prim_range))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_prim_range(le.prim_range,ri.prim_range,0,0,FALSE));
    SELF.prim_rangeWeight := (50+MAP ( le.prim_range = (TYPEOF(le.prim_range))'' OR ri.prim_range = (TYPEOF(ri.prim_range))'' => 0,
           ...
           -1.000*le.prim_range_weight100))/100; 
    SELF.sec_range_match_code := MAP(le.sec_range = (TYPEOF(le.sec_range))'' OR ri.sec_range = (TYPEOF(ri.sec_range))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_sec_range(le.sec_range,ri.sec_range,0,0,FALSE));
    SELF.sec_rangeWeight := (50+MAP ( le.sec_range = (TYPEOF(le.sec_range))'' OR ri.sec_range = (TYPEOF(ri.sec_range))'' => 0,
           ...
           -0.888*le.sec_range_weight100))/100; 
    SELF.parent_proxid_match_code := MAP(le.parent_proxid = (TYPEOF(le.parent_proxid))'' OR ri.parent_proxid = (TYPEOF(ri.parent_proxid))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_parent_proxid(le.parent_proxid,ri.parent_proxid,FALSE));
    SELF.parent_proxidWeight := (50+MAP ( le.parent_proxid = (TYPEOF(le.parent_proxid))'' OR ri.parent_proxid = (TYPEOF(ri.parent_proxid))'' => 0,
           ...
           -1.000*le.parent_proxid_weight100))/100*0.00; 
    SELF.sele_proxid_match_code := MAP(le.sele_proxid = (TYPEOF(le.sele_proxid))'' OR ri.sele_proxid = (TYPEOF(ri.sele_proxid))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_sele_proxid(le.sele_proxid,ri.sele_proxid,FALSE));
    SELF.sele_proxidWeight := (50+MAP ( le.sele_proxid = (TYPEOF(le.sele_proxid))'' OR ri.sele_proxid = (TYPEOF(ri.sele_proxid))'' => 0,
           ...
           -1.000*le.sele_proxid_weight100))/100*0.00; 
    SELF.org_proxid_match_code := MAP(le.org_proxid = (TYPEOF(le.org_proxid))'' OR ri.org_proxid = (TYPEOF(ri.org_proxid))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_org_proxid(le.org_proxid,ri.org_proxid,FALSE));
    SELF.org_proxidWeight := (50+MAP ( le.org_proxid = (TYPEOF(le.org_proxid))'' OR ri.org_proxid = (TYPEOF(ri.org_proxid))'' => 0,
           ...
           -1.000*le.org_proxid_weight100))/100*0.00; 
    SELF.ultimate_proxid_match_code := MAP(le.ultimate_proxid = (TYPEOF(le.ultimate_proxid))'' OR ri.ultimate_proxid = (TYPEOF(ri.ultimate_proxid))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_ultimate_proxid(le.ultimate_proxid,ri.ultimate_proxid,FALSE));
    SELF.ultimate_proxidWeight := (50+MAP ( le.ultimate_proxid = (TYPEOF(le.ultimate_proxid))'' OR ri.ultimate_proxid = (TYPEOF(ri.ultimate_proxid))'' => 0,
           ...
           -1.000*le.ultimate_proxid_weight100))/100*0.00; 
    SELF.sele_flag_match_code := MAP(le.sele_flag = (TYPEOF(le.sele_flag))'' OR ri.sele_flag = (TYPEOF(ri.sele_flag))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_sele_flag(le.sele_flag,ri.sele_flag,FALSE));
    SELF.sele_flagWeight := (50+MAP ( le.sele_flag = (TYPEOF(le.sele_flag))'' OR ri.sele_flag = (TYPEOF(ri.sele_flag))'' => 0,
           ...
           -1.000*le.sele_flag_weight100))/100*0.00; 
    SELF.org_flag_match_code := MAP(le.org_flag = (TYPEOF(le.org_flag))'' OR ri.org_flag = (TYPEOF(ri.org_flag))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_org_flag(le.org_flag,ri.org_flag,FALSE));
    SELF.org_flagWeight := (50+MAP ( le.org_flag = (TYPEOF(le.org_flag))'' OR ri.org_flag = (TYPEOF(ri.org_flag))'' => 0,
           ...
           -1.000*le.org_flag_weight100))/100*0.00; 
    SELF.ult_flag_match_code := MAP(le.ult_flag = (TYPEOF(le.ult_flag))'' OR ri.ult_flag = (TYPEOF(ri.ult_flag))'' => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_ult_flag(le.ult_flag,ri.ult_flag,FALSE));
    SELF.ult_flagWeight := (50+MAP ( le.ult_flag = (TYPEOF(le.ult_flag))'' OR ri.ult_flag = (TYPEOF(ri.ult_flag))'' => 0,
           ...
           -1.000*le.ult_flag_weight100))/100*0.00; 
    SELF.zip_match_code := MAP(le.zip = (TYPEOF(le.zip))'' OR ~EXISTS(ri.zip_cases) => SALT33.MatchCode.OneSideNull,BizLinkFull.match_methods(File_BizHead).match_zip(le.zip,ri.zip_cases,FALSE));
  ...
  J0 := JOIN(Recs1,Key,HASH32(SALT33.fn_bow_bestword(LEFT.cnp_name))=RIGHT.gss_hash AND SALT33.MatchBagOfWords(LEFT.cnp_name,RIGHT.cnp_name,3177747,1) > BizLinkFull.Config_BIP.cnp_name_Force * 100
     ...
     AND ( LEFT.city = (TYPEOF(LEFT.city))'' OR RIGHT.city = (TYPEOF(RIGHT.city))'' OR metaphonelib.DMetaPhone1(LEFT.city)=metaphonelib.DMetaPhone1(RIGHT.city) OR SALT33.WithinEditN(LEFT.city,RIGHT.city,2, 0)  ),Score_Batch(RIGHT,LEFT),
    ATMOST(HASH32(SALT33.fn_bow_bestword(LEFT.cnp_name))=RIGHT.gss_hash,/*HACK01*/2000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),HASH32(SALT33.fn_bow_bestword(LEFT.cnp_name))=RIGHT.gss_hash AND SALT33.MatchBagOfWords(LEFT.cnp_name,RIGHT.cnp_name,3177747,1) > BizLinkFull.Config_BIP.cnp_name_Force * 100
     AND ( LEFT.prim_name = (TYPEOF(LEFT.prim_name))'' OR RIGHT.prim_name = (TYPEOF(RIGHT.prim_name))'' OR SALT33.WithinEditN(LEFT.prim_name,RIGHT.prim_name,1, 0)  )
     AND ( LEFT.st = (TYPEOF(LEFT.st))'' OR RIGHT.st = (TYPEOF(RIGHT.st))'' OR LEFT.st = RIGHT.st  )
     AND ( LEFT.city = (TYPEOF(LEFT.city))'' OR RIGHT.city = (TYPEOF(RIGHT.city))'' OR metaphonelib.DMetaPhone1(LEFT.city)=metaphonelib.DMetaPhone1(RIGHT.city) OR SALT33.WithinEditN(LEFT.city,RIGHT.city,2, 0)  ),Score_Batch(RIGHT,LEFT),
    ATMOST(HASH32(SALT33.fn_bow_bestword(LEFT.cnp_name))=RIGHT.gss_hash,/*HACK01*/2000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_cnp_name='',Input_prim_name='',Input_st='',Input_city='',Input_company_sic_code1='',Input_cnp_number='',Input_cnp_btype='',Input_cnp_lowv='',Input_prim_range='',Input_sec_range='',Input_parent_proxid='',Input_sele_proxid='',Input_org_proxid='',Input_ultimate_proxid='',Input_sele_flag='',Input_org_flag='',Input_ult_flag='',Input_zip='',output_file,AsIndex='true') := MACRO
IMPORT SALT33,BizLinkFull;
#IF(#TEXT(Input_cnp_name)<>'')
  #uniquename(trans)
  PRTE2_BIPV2_BusHeader.Key_BizHead_L_CNPNAME.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE2_BIPV2_BusHeader.Key_BizHead_L_CNPNAME.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

...
//Scores for various field components
  ...
  INTEGER2 company_fein_e1_Weight100 := SALT37.Min0(h.company_fein_weight100 + 100*log(h.company_fein_cnt/h.company_fein_e1_cnt)/log(2)); // Precompute edit-distance specificity
...
EXPORT Shrinkage := DATASET([{'L_FEIN','company_sic_code1',CntRed_company_sic_code1*100,CntRed_company_sic_code1*TSize},{'L_FEIN','cnp_name',CntRed_cnp_name*100,CntRed_cnp_name*TSize},{'L_FEIN','cnp_number',CntRed_cnp_number*100,CntRed_cnp_number*TSize},{'L_FEIN','cnp_btype',CntRed_cnp_btype*100,CntRed_cnp_btype*TSize},{'L_FEIN','cnp_lowv',CntRed_cnp_lowv*100,CntRed_cnp_lowv*TSize},{'L_FEIN','zip',CntRed_zip*100,CntRed_zip*TSize},{'L_FEIN','prim_name',CntRed_prim_name*100,CntRed_prim_name*TSize},{'L_FEIN','city',CntRed_city*100,CntRed_city*TSize},{'L_FEIN','st',CntRed_st*100,CntRed_st*TSize},{'L_FEIN','prim_range',CntRed_prim_range*100,CntRed_prim_range*TSize},{'L_FEIN','sec_range',CntRed_sec_range*100,CntRed_sec_range*TSize},{'L_FEIN','parent_proxid',CntRed_parent_proxid*100,CntRed_parent_proxid*TSize},{'L_FEIN','sele_proxid',CntRed_sele_proxid*100,CntRed_sele_proxid*TSize},{'L_FEIN','org_proxid',CntRed_org_proxid*100,CntRed_org_proxid*TSize},{'L_FEIN','ultimate_proxid',CntRed_ultimate_proxid*100,CntRed_ultimate_proxid*TSize},{'L_FEIN','sele_flag',CntRed_sele_flag*100,CntRed_sele_flag*TSize},{'L_FEIN','org_flag',CntRed_org_flag*100,CntRed_org_flag*TSize},{'L_FEIN','ult_flag',CntRed_ult_flag*100,CntRed_ult_flag*TSize}],SALT37.ShrinkLayout);
EXPORT CanSearch(Process_Biz_Layouts.InputLayout le) := le.company_fein <> (TYPEOF(le.company_fein))'' AND BizLinkFull.Fields.InValid_company_fein((SALT37.StrType)le.company_fein)=0;
KeyRec := RECORDOF(Key);
 
EXPORT RawFetch_server(TYPEOF(h.company_fein) param_company_fein = (TYPEOF(h.company_fein))'',TYPEOF(h.company_fein_len) param_company_fein_len = (TYPEOF(h.company_fein_len))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := 
    STEPPED( LIMIT( Key(
          KEYED(( company_fein = param_company_fein AND param_company_fein <> (TYPEOF(company_fein))''))
      AND KEYED(fallback_value >= param_fallback_value)),BizLinkFull.Config_BIP.L_FEIN_MAXBLOCKLIMIT,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),ultid,orgid,seleid,proxid);
 
EXPORT RawFetch(TYPEOF(h.company_fein) param_company_fein = (TYPEOF(h.company_fein))'',TYPEOF(h.company_fein_len) param_company_fein_len = (TYPEOF(h.company_fein_len))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'') := FUNCTION
// Why not LOOP? - Because I am expecting FIRST one to win 990f the time - and don't want to impact it
  RawData0 := RawFetch_server(param_company_fein,param_company_fein_len,0);
  RawData1 := RawFetch_server(param_company_fein,param_company_fein_len,1);
  RawData2 := RawFetch_server(param_company_fein,param_company_fein_len,2);
  Returnable(DATASET(RECORDOF(RawData0)) d) := COUNT(NOFOLD(d))<>1 OR EXISTS(NOFOLD(d((SALT37.StrType)company_fein != '')));
  res := MAP (
...
EXPORT ScoredproxidFetch(TYPEOF(h.company_fein) param_company_fein = (TYPEOF(h.company_fein))'',TYPEOF(h.company_fein_len) param_company_fein_len = (TYPEOF(h.company_fein_len))'',TYPEOF(h.company_sic_code1) param_company_sic_code1 = (TYPEOF(h.company_sic_code1))'',TYPEOF(h.cnp_name) param_cnp_name = (TYPEOF(h.cnp_name))'',TYPEOF(h.cnp_number) param_cnp_number = (TYPEOF(h.cnp_number))'',TYPEOF(h.cnp_btype) param_cnp_btype = (TYPEOF(h.cnp_btype))'',TYPEOF(h.cnp_lowv) param_cnp_lowv = (TYPEOF(h.cnp_lowv))'',DATASET(process_Biz_layouts.layout_zip_cases) param_zip,TYPEOF(h.prim_name) param_prim_name = (TYPEOF(h.prim_name))'',TYPEOF(h.prim_name_len) param_prim_name_len = (TYPEOF(h.prim_name_len))'',TYPEOF(h.city) param_city = (TYPEOF(h.city))'',TYPEOF(h.city_len) param_city_len = (TYPEOF(h.city_len))'',TYPEOF(h.st) param_st = (TYPEOF(h.st))'',TYPEOF(h.prim_range) param_prim_range = (TYPEOF(h.prim_range))'',TYPEOF(h.prim_range_len) param_prim_range_len = (TYPEOF(h.prim_range_len))'',TYPEOF(h.sec_range) param_sec_range = (TYPEOF(h.sec_range))'',TYPEOF(h.sec_range_len) param_sec_range_len = (TYPEOF(h.sec_range_len))'',TYPEOF(h.parent_proxid) param_parent_proxid = (TYPEOF(h.parent_proxid))'',TYPEOF(h.sele_proxid) param_sele_proxid = (TYPEOF(h.sele_proxid))'',TYPEOF(h.org_proxid) param_org_proxid = (TYPEOF(h.org_proxid))'',TYPEOF(h.ultimate_proxid) param_ultimate_proxid = (TYPEOF(h.ultimate_proxid))'',TYPEOF(h.sele_flag) param_sele_flag = (TYPEOF(h.sele_flag))'',TYPEOF(h.org_flag) param_org_flag = (TYPEOF(h.org_flag))'',TYPEOF(h.ult_flag) param_ult_flag = (TYPEOF(h.ult_flag))'',TYPEOF(h.fallback_value) param_fallback_value = (TYPEOF(h.fallback_value))'',BOOLEAN param_disableForce = FALSE) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.company_fein_match_code := BizLinkFull.match_methods(File_BizHead).match_company_fein(le.company_fein,param_company_fein,le.company_fein_len,param_company_fein_len,TRUE);
    SELF.company_feinWeight := (50+MAP (
           le.company_fein = param_company_fein  => le.company_fein_weight100,
...
EXPORT InputLayout_Batch := RECORD
  ...
  TYPEOF(h.company_fein_len) company_fein_len := (TYPEOF(h.company_fein_len))'';
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex, BOOLEAN In_disableForce = FALSE) := FUNCTION
  ...
  Process_Biz_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.company_fein_match_code := BizLinkFull.match_methods(File_BizHead).match_company_fein(le.company_fein,ri.company_fein,le.company_fein_len,ri.company_fein_len,TRUE);
    SELF.company_feinWeight := (50+MAP (
...
le.zip = (TYPEOF(le.zip))'' OR ~EXISTS(ri.zip_cases) => SALT37.MatchCode.OneSideNull,
  ...
  J0 := JOIN(Recs1,Key,LEFT.company_fein = RIGHT.company_fein,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.company_fein = RIGHT.company_fein,BizLinkFull.Config_BIP.L_FEIN_MAXBLOCKSIZE)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.company_fein = RIGHT.company_fein,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.company_fein = RIGHT.company_fein,BizLinkFull.Config_BIP.L_FEIN_MAXBLOCKSIZE),HASH,UNORDERED); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
#IF(#TEXT(Input_company_fein)<>'')
  ...
  PRTE2_BIPV2_BusHeader.Key_BizHead_L_FEIN.InputLayout_Batch %trans%(InFile le) := TRANSFORM
    ...
    SELF.company_fein_len := LENGTH(TRIM((TYPEOF(SELF.company_fein))le.Input_company_fein));
  ...
  output_file := PRTE2_BIPV2_BusHeader.Key_BizHead_L_FEIN.ScoredFetch_Batch(%p%,AsIndex, In_disableForce);
#ELSE

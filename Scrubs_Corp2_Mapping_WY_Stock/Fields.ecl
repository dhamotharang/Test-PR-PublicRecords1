IMPORT ut,SALT34;
IMPORT Scrubs,Scrubs_Corp2_Mapping_WY_Stock; // Import modules for FieldTypes attribute definitions
EXPORT Fields := MODULE
 
// Processing for each FieldType
EXPORT SALT34.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_charter_nbr','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_stock_type','invalid_shares','invalid_stock_par_value','invalid_stock_addl_info');
EXPORT FieldTypeNum(SALT34.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_charter_nbr' => 2,'invalid_corp_vendor' => 3,'invalid_state_origin' => 4,'invalid_mandatory' => 5,'invalid_date' => 6,'invalid_stock_type' => 7,'invalid_shares' => 8,'invalid_stock_par_value' => 9,'invalid_stock_addl_info' => 10,0);
...
EXPORT MakeFT_invalid_corp_vendor(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_vendor(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['56']);
EXPORT InValidMessageFT_invalid_corp_vendor(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('56'),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_state_origin(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_state_origin(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['WY']);
EXPORT InValidMessageFT_invalid_state_origin(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('WY'),SALT34.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_stock_type(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_stock_type(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['COMMON','PREFERRED','']);
EXPORT InValidMessageFT_invalid_stock_type(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('COMMON|PREFERRED|'),SALT34.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_shares(SALT34.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_invalid_shares(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,' .,0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'))));
EXPORT InValidMessageFT_invalid_shares(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars(' .,0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_stock_par_value(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_stock_par_value(SALT34.StrType s) := WHICH(~Scrubs_Corp2_Mapping_WY_Stock.Functions.invalid_stock_par_value(s)>0);
EXPORT InValidMessageFT_invalid_stock_par_value(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.CustomFail('Scrubs_Corp2_Mapping_WY_Stock.Functions.invalid_stock_par_value'),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_stock_addl_info(SALT34.StrType s0) := FUNCTION
  s1 := SALT34.stringfilter(s0,' :ABCDEFGHIJKLMNOPQRSTUVWXYZ'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_stock_addl_info(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,' :ABCDEFGHIJKLMNOPQRSTUVWXYZ'))));
EXPORT InValidMessageFT_invalid_stock_addl_info(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars(' :ABCDEFGHIJKLMNOPQRSTUVWXYZ'),SALT34.HygieneErrors.Good);
...
EXPORT Make_stock_class(SALT34.StrType s0) := s0;
EXPORT InValid_stock_class(SALT34.StrType s) := FALSE;
...
EXPORT Make_stock_authorized_nbr(SALT34.StrType s0) := s0;
EXPORT InValid_stock_authorized_nbr(SALT34.StrType s) := FALSE;
...
EXPORT Make_stock_addl_info(SALT34.StrType s0) := MakeFT_invalid_stock_addl_info(s0);
EXPORT InValid_stock_addl_info(SALT34.StrType s) := InValidFT_invalid_stock_addl_info(s);
EXPORT InValidMessage_stock_addl_info(UNSIGNED1 wh) := InValidMessageFT_invalid_stock_addl_info(wh);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT34,Scrubs_Corp2_Mapping_WY_Stock;

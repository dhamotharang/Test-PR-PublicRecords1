...
EXPORT Summary(SALT32.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_job_code := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.job_code)));
    avelength_job_code := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.job_code)),h.job_code<>(typeof(h.job_code))'');
    ...
    maxlength_number_of_employees := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.number_of_employees)));
    avelength_number_of_employees := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.number_of_employees)),h.number_of_employees<>(typeof(h.number_of_employees))'');
    ...
    maxlength_source_code := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.source_code)));
    avelength_source_code := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.source_code)),h.source_code<>(typeof(h.source_code))'');
    populated_company_type_pcnt := AVE(GROUP,IF(h.company_type = (TYPEOF(h.company_type))'',0,100));
    maxlength_company_type := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.company_type)));
    avelength_company_type := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.company_type)),h.company_type<>(typeof(h.company_type))'');
...
SALT32.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT32.StrType)le.fname),TRIM((SALT32.StrType)le.lname),TRIM((SALT32.StrType)le.title),TRIM((SALT32.StrType)le.company),TRIM((SALT32.StrType)le.address1),TRIM((SALT32.StrType)le.address2),TRIM((SALT32.StrType)le.city),TRIM((SALT32.StrType)le.state),TRIM((SALT32.StrType)le.zip),TRIM((SALT32.StrType)le.zip4),TRIM((SALT32.StrType)le.cart),TRIM((SALT32.StrType)le.bar),TRIM((SALT32.StrType)le.gender),TRIM((SALT32.StrType)le.country),TRIM((SALT32.StrType)le.postal_cd),TRIM((SALT32.StrType)le.dpv),TRIM((SALT32.StrType)le.addr_type),TRIM((SALT32.StrType)le.county_cd),TRIM((SALT32.StrType)le.job_code),TRIM((SALT32.StrType)le.nielsen_county_cd),TRIM((SALT32.StrType)le.number_of_employees),TRIM((SALT32.StrType)le.msa),TRIM((SALT32.StrType)le.source_code),TRIM((SALT32.StrType)le.company_type),TRIM((SALT32.StrType)le.phone),TRIM((SALT32.StrType)le.email),TRIM((SALT32.StrType)le.list_id),TRIM((SALT32.StrType)le.scno),TRIM((SALT32.StrType)le.keycode),TRIM((SALT32.StrType)le.custno),TRIM((SALT32.StrType)le.license_no),TRIM((SALT32.StrType)le.dob),TRIM((SALT32.StrType)le.orig_date),TRIM((SALT32.StrType)le.exp_date),TRIM((SALT32.StrType)le.degree),TRIM((SALT32.StrType)le.specialty)));
  SELF.FldNo := C;
END;
...
SHARED FldIds := DATASET([{1,'fname'}
      ...
      ,{34,'exp_date'}
      ,{35,'degree'}
      ,{36,'specialty'}],SALT32.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_job_code((SALT32.StrType)le.job_code),
    Fields.InValid_nielsen_county_cd((SALT32.StrType)le.nielsen_county_cd),
    Fields.InValid_number_of_employees((SALT32.StrType)le.number_of_employees),
    Fields.InValid_msa((SALT32.StrType)le.msa),
    Fields.InValid_source_code((SALT32.StrType)le.source_code),
    Fields.InValid_company_type((SALT32.StrType)le.company_type),
    ...
    Fields.InValid_license_no((SALT32.StrType)le.license_no,(SALT32.StrType)le.state),

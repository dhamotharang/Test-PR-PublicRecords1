...
EXPORT FromNone(DATASET(Layout_ALC_ACCOUNTANTS) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.job_code_Invalid := Fields.InValid_job_code((SALT37.StrType)le.job_code);
    SELF.nielsen_county_cd_Invalid := Fields.InValid_nielsen_county_cd((SALT37.StrType)le.nielsen_county_cd);
    SELF.number_of_employees_Invalid := Fields.InValid_number_of_employees((SALT37.StrType)le.number_of_employees);
    SELF.msa_Invalid := Fields.InValid_msa((SALT37.StrType)le.msa);
    SELF.company_type_Invalid := Fields.InValid_company_type((SALT37.StrType)le.company_type);
    ...
    SELF.license_no_Invalid := Fields.InValid_license_no((SALT37.StrType)le.license_no,(SALT37.StrType)le.state);
    ...
    SELF.degree_Invalid := Fields.InValid_degree((SALT37.StrType)le.degree);
    SELF := le;
  END;

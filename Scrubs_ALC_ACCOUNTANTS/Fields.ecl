...
EXPORT MakeFT_invalid_alphaspaceperiod(SALT32.StrType s0) := FUNCTION
  s1 := SALT32.stringfilter(s0,' .ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_alphaspaceperiod(SALT32.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT32.StringFilter(s,' .ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'))));
EXPORT InValidMessageFT_invalid_alphaspaceperiod(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInChars(' .ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'),SALT32.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_job_code(SALT32.StrType s0) := FUNCTION
  s1 := SALT32.stringfilter(s0,'#14'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_job_code(SALT32.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT32.StringFilter(s,'#14'))));
EXPORT InValidMessageFT_invalid_job_code(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInChars('#14'),SALT32.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_company_type(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_company_type(SALT32.StrType s) := WHICH(((SALT32.StrType) s) NOT IN ['2','3','4','6','7','8','A','B','E','F','G','H','I',' ']);
EXPORT InValidMessageFT_invalid_company_type(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInEnum('2|3|4|6|7|8|A|B|E|F|G|H|I| '),SALT32.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_license_number(SALT32.StrType s0) := FUNCTION
  s1 := SALT32.stringfilter(s0,' .-ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_license_number(SALT32.StrType s,SALT32.StrType state) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT32.StringFilter(s,' .-ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'))),~ALC.Custom_Scrubs.fn_valid_mt_acct_license_no(s,state)<1);
EXPORT InValidMessageFT_invalid_license_number(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInChars(' .-ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'),SALT32.HygieneErrors.CustomFail('ALC.Custom_Scrubs.fn_valid_mt_acct_license_no'),SALT32.HygieneErrors.Good);
...
EXPORT FieldNum(SALT32.StrType fn) := CASE(fn,'fname' => 0,'lname' => 1,'title' => 2,'company' => 3,'address1' => 4,'address2' => 5,'city' => 6,'state' => 7,'zip' => 8,'zip4' => 9,'cart' => 10,'bar' => 11,'gender' => 12,'country' => 13,'postal_cd' => 14,'dpv' => 15,'addr_type' => 16,'county_cd' => 17,'job_code' => 18,'nielsen_county_cd' => 19,'number_of_employees' => 20,'msa' => 21,'source_code' => 22,'company_type' => 23,'phone' => 24,'email' => 25,'list_id' => 26,'scno' => 27,'keycode' => 28,'custno' => 29,'license_no' => 30,'dob' => 31,'orig_date' => 32,'exp_date' => 33,'degree' => 34,'specialty' => 35,0);
...
EXPORT Make_job_code(SALT32.StrType s0) := MakeFT_invalid_job_code(s0);
EXPORT InValid_job_code(SALT32.StrType s) := InValidFT_invalid_job_code(s);
...
EXPORT Make_number_of_employees(SALT32.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_number_of_employees(SALT32.StrType s) := InValidFT_invalid_numeric(s);
...
EXPORT Make_source_code(SALT32.StrType s0) := s0;
EXPORT InValid_source_code(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_source_code(UNSIGNED1 wh) := '';
 
EXPORT Make_company_type(SALT32.StrType s0) := MakeFT_invalid_company_type(s0);
EXPORT InValid_company_type(SALT32.StrType s) := InValidFT_invalid_company_type(s);
...
EXPORT InValid_license_no(SALT32.StrType s,SALT32.StrType state) := InValidFT_invalid_license_number(s,state);
...
EXPORT Make_degree(SALT32.StrType s0) := MakeFT_invalid_alphaspaceperiod(s0);
EXPORT InValid_degree(SALT32.StrType s) := InValidFT_invalid_alphaspaceperiod(s);

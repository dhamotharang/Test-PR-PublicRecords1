...
////////////phone_table_v2//////////////////////////////////////////////////////
	EXPORT file_phone_table_fcra_v2(Boolean isFCRA = false,boolean gongFilter = false) := FUNCTION
		goodphone(string10 s) := (INTEGER)s<>0 AND LENGTH(Stringlib.stringfilter(s,'0123456789'))=10;
		Layouts.phone_table_rec_fcra slimg(Files.File_History_Full_Prepped_for_Keys le) := TRANSFORM
		...
		with_did_counts := join(all_phones, did_counts, left.phone10 = right.phone10,
						transform(Layouts.phone_table_rec, 
						          self.did_ct := right.did_ct, 
											self.did_ct_c6 := right.did_ct_c6, 
											self := left, self := []), 
						left outer, keep(1));
		Layouts.phone_table_rec_fcra roller(Layouts.phone_table_rec_fcra le, Layouts.phone_table_rec_fcra ri) := TRANSFORM
		...
		Layouts.phone_table_rec_fcra iterator2(Layouts.phone_table_rec_fcra le, Layouts.phone_table_rec_fcra ri) := TRANSFORM

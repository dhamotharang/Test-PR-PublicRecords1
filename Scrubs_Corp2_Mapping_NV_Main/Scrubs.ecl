...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Corp2_Mapping.LayoutsCommon.Main)
    ...
    UNSIGNED1 corp_ra_title_desc_Invalid;
...
EXPORT FromNone(DATASET(Corp2_Mapping.LayoutsCommon.Main) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_supp_key_Invalid := Fields.InValid_corp_supp_key((SALT32.StrType)le.corp_supp_key);
    ...
    SELF.corp_ln_name_type_desc_Invalid := Fields.InValid_corp_ln_name_type_desc((SALT32.StrType)le.corp_ln_name_type_desc);
    ...
    SELF.corp_ra_title_desc_Invalid := Fields.InValid_corp_ra_title_desc((SALT32.StrType)le.corp_ra_title_desc);
    ...
    SELF.corp_trademark_class_desc1_Invalid := Fields.InValid_corp_trademark_class_desc1((SALT32.StrType)le.corp_trademark_class_desc1);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_supp_key_Invalid << 14 ) + ( le.corp_vendor_Invalid << 15 ) + ( le.corp_state_origin_Invalid << 16 ) + ( le.corp_process_date_Invalid << 17 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 19 ) + ( le.corp_legal_name_Invalid << 21 ) + ( le.corp_ln_name_type_cd_Invalid << 22 ) + ( le.corp_ln_name_type_desc_Invalid << 23 ) + ( le.corp_status_date_Invalid << 24 ) + ( le.corp_standing_Invalid << 26 ) + ( le.corp_status_comment_Invalid << 27 ) + ( le.corp_inc_date_Invalid << 28 ) + ( le.corp_term_exist_cd_Invalid << 30 ) + ( le.corp_term_exist_exp_Invalid << 31 ) + ( le.corp_foreign_domestic_ind_Invalid << 33 ) + ( le.corp_forgn_state_cd_Invalid << 34 ) + ( le.corp_forgn_state_desc_Invalid << 35 ) + ( le.corp_forgn_date_Invalid << 36 ) + ( le.corp_for_profit_ind_Invalid << 38 ) + ( le.corp_ra_title_desc_Invalid << 39 ) + ( le.corp_ra_resign_date_Invalid << 40 ) + ( le.corp_ra_address_type_desc_Invalid << 42 ) + ( le.corp_country_of_formation_Invalid << 43 ) + ( le.corp_trademark_class_desc1_Invalid << 44 ) + ( le.recordorigin_Invalid << 45 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_for_profit_ind_Invalid := (le.ScrubsBits1 >> 38) & 1;
    SELF.corp_ra_title_desc_Invalid := (le.ScrubsBits1 >> 39) & 1;
    SELF.corp_ra_resign_date_Invalid := (le.ScrubsBits1 >> 40) & 3;
    SELF.corp_ra_address_type_desc_Invalid := (le.ScrubsBits1 >> 42) & 1;
    SELF.corp_country_of_formation_Invalid := (le.ScrubsBits1 >> 43) & 1;
    SELF.corp_trademark_class_desc1_Invalid := (le.ScrubsBits1 >> 44) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_ra_title_desc_ENUM_ErrorCount := COUNT(GROUP,h.corp_ra_title_desc_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_ra_title_desc_Invalid,'ENUM','UNKNOWN')
    ...
    SELF.FieldName := CHOOSE(c,'dt_vendor_first_reported','dt_vendor_last_reported','dt_first_seen','dt_last_seen','corp_ra_dt_first_seen','corp_ra_dt_last_seen','corp_key','corp_supp_key','corp_vendor','corp_state_origin','corp_process_date','corp_orig_sos_charter_nbr','corp_legal_name','corp_ln_name_type_cd','corp_ln_name_type_desc','corp_status_date','corp_standing','corp_status_comment','corp_inc_date','corp_term_exist_cd','corp_term_exist_exp','corp_foreign_domestic_ind','corp_forgn_state_cd','corp_forgn_state_desc','corp_forgn_date','corp_for_profit_ind','corp_ra_title_desc','corp_ra_resign_date','corp_ra_address_type_desc','corp_country_of_formation','corp_trademark_class_desc1','recordorigin','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_corp_key','invalid_corp_supp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_mandatory','invalid_corp_type','invalid_corp_type_desc','invalid_date','invalid_corp_standing','invalid_corp_status_comment','invalid_date','invalid_corp_term_exist_cd','invalid_future_date','invalid_for_dom_ind','invalid_alpha_spchar_blank','invalid_alpha_spchar_blank','invalid_date','invalid_Y_N','invalid_corp_ra_title_desc','invalid_date','invalid_corp_ra_address_type_desc','invalid_alpha_spchar_blank','invalid_alpha_spchar_blank','invalid_recordorigin','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT32.StrType)le.dt_vendor_first_reported,(SALT32.StrType)le.dt_vendor_last_reported,(SALT32.StrType)le.dt_first_seen,(SALT32.StrType)le.dt_last_seen,(SALT32.StrType)le.corp_ra_dt_first_seen,(SALT32.StrType)le.corp_ra_dt_last_seen,(SALT32.StrType)le.corp_key,(SALT32.StrType)le.corp_supp_key,(SALT32.StrType)le.corp_vendor,(SALT32.StrType)le.corp_state_origin,(SALT32.StrType)le.corp_process_date,(SALT32.StrType)le.corp_orig_sos_charter_nbr,(SALT32.StrType)le.corp_legal_name,(SALT32.StrType)le.corp_ln_name_type_cd,(SALT32.StrType)le.corp_ln_name_type_desc,(SALT32.StrType)le.corp_status_date,(SALT32.StrType)le.corp_standing,(SALT32.StrType)le.corp_status_comment,(SALT32.StrType)le.corp_inc_date,(SALT32.StrType)le.corp_term_exist_cd,(SALT32.StrType)le.corp_term_exist_exp,(SALT32.StrType)le.corp_foreign_domestic_ind,(SALT32.StrType)le.corp_forgn_state_cd,(SALT32.StrType)le.corp_forgn_state_desc,(SALT32.StrType)le.corp_forgn_date,(SALT32.StrType)le.corp_for_profit_ind,(SALT32.StrType)le.corp_ra_title_desc,(SALT32.StrType)le.corp_ra_resign_date,(SALT32.StrType)le.corp_ra_address_type_desc,(SALT32.StrType)le.corp_country_of_formation,(SALT32.StrType)le.corp_trademark_class_desc1,(SALT32.StrType)le.recordorigin,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT32.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_ln_name_type_cd:invalid_corp_type:CUSTOM'
          ,'corp_ln_name_type_desc:invalid_corp_type_desc:ENUM'
          ...
          ,'corp_forgn_state_cd:invalid_alpha_spchar_blank:ALLOW'
          ,'corp_forgn_state_desc:invalid_alpha_spchar_blank:ALLOW'
          ,'corp_forgn_date:invalid_date:ALLOW','corp_forgn_date:invalid_date:CUSTOM'
          ,'corp_for_profit_ind:invalid_Y_N:ENUM'
          ,'corp_ra_title_desc:invalid_corp_ra_title_desc:ENUM'
          ...
          ,'corp_country_of_formation:invalid_alpha_spchar_blank:ALLOW'
          ,'corp_trademark_class_desc1:invalid_alpha_spchar_blank:ALLOW'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_corp_ra_title_desc(1)
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_ra_title_desc_ENUM_ErrorCount

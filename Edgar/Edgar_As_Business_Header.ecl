...
EdgarCompanies := File_Edgar_Company_Base;
//EdgarCompanies := CHOOSEN(File_Edgar_Company_Base, 1000);

Layout_temp := RECORD
	UNSIGNED8 rcid := 0;
	Layout_Edgar_Company;
END;

// Append id field to edgar company records.
Layout_temp AddField(Layout_Edgar_Company l) := TRANSFORM
	SELF := l;
END;
...
Business_Header.Layout_Business_Header EdgarToBH(
...
export Edgar_As_Business_Header := Edgar_As_BH : persist('TEMP::Edgar_As_BH');
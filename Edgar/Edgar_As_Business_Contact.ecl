...
Business_Header.Layout_Business_Contact_Full Translate_Edgar_to_BCF(Layout_Edgar_Contact_Base L) := TRANSFORM
...
SELF.company_city := L.bus_p_city_name;
...
Edgar_Contacts := PROJECT(File_Edgar_Contacts_Base(lname <> ''), Translate_Edgar_to_BCF(LEFT));

export Edgar_As_Business_Contact := Edgar_Contacts((INTEGER)name_score < 3, Business_Header.CheckPersonName(fname, mname, lname, name_suffix)) : PERSIST('TEMP::Edgar_Contacts_As_BC');
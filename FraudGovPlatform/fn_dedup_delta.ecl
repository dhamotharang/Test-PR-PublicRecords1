EXPORT fn_dedup_delta(inputs):=FUNCTIONMACRO
	...
	in_ddp := rollup( project(in_srt, transform(new_rec, 
		 self.dt_first_seen:=(unsigned4) ut.GetDate; 
		 self.dt_last_seen:=(unsigned4) ut.GetDate; 
		 self.dt_vendor_last_reported:=(unsigned4) ut.GetDate; 
		 self.dt_vendor_first_reported:=(unsigned4) ut.GetDate;		 
		 self := left))

...
EXPORT Append_RID(
    ...
    RollupRec := ROLLUP(SortedRec,
               ...
               ut.CleanSpacesandUpper(left.head_of_household_indicator)= ut.CleanSpacesandUpper(right.head_of_household_indicator) and
                ...
                getPreviousRID(LEFT,RIGHT),
                local);

                old_RIDs := RollupRec(record_id > 0);
                new_recs := RollupRec(record_id = 0); 

                max_rid := max(Previous_Build, Previous_Build.Record_ID);
                ...
                sort_Final_RIDs := sort(prj_new_RIDs + old_RIDs, record_id, -reported_date,local);

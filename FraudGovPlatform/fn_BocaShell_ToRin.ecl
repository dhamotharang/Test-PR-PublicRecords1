...
export fn_BocaShell_ToRin( dataset(riskprocessing.layouts.layout_internal_shell) bs, boolean isFCRA=false, string dataRestrictionMask, string Intended_Purpose ) := FUNCTION
	FraudGovPlatform.Layouts.BocaShell convertToRin(bs le) := transform
...
// blank these out for 5.0 FCRA shell	
  ...
  self.acc_logs.bus_inq_other_count12           := le.BIP_Header54.bus_inq_other_count12;
	self.input_dl_number	:=le.shell_input.dl_number;
	self.input_dl_state	:=le.shell_input.dl_state;
	self.best_fname	:=le.best_flags.fname;
	self.best_lname	:=le.best_flags.lname;
	self.best_ssn		:=le.best_flags.ssn;
	self.add_curr_prim_range	 :=le.address_verification.address_history_1.prim_range;
	self.add_curr_predir	 :=le.address_verification.address_history_1.predir;
	self.add_curr_prim_name	 :=le.address_verification.address_history_1.prim_name;
	self.add_curr_addr_suffix	 :=le.address_verification.address_history_1.addr_suffix;
	self.add_curr_postdir	 :=le.address_verification.address_history_1.postdir;
	self.add_curr_unit_desig	 :=le.address_verification.address_history_1.unit_desig;	
	self.add_curr_sec_range	 :=le.address_verification.address_history_1.sec_range;
	self.add_curr_city_name	 :=le.address_verification.address_history_1.city_name;
	self.add_curr_st	 :=le.address_verification.address_history_1.st;
	self.add_curr_zip5	 :=le.address_verification.address_history_1.zip5;	
	self.add_curr_lat	 :=le.address_verification.address_history_1.lat;
	self.add_curr_long	 :=le.address_verification.address_history_1.long;
	self.record_id	:=(unsigned8)le.accountnumber;
	self.fdn_file_info_id	:=0;
	self := le;
	end;

	RinShell_out := project( bs, convertToRin(left) );

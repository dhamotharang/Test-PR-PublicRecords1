...
MODULE
	...
	IdentityData MapIDDT(Sprayed_NAC L) := TRANSFORM 
			...
			SELF.Source_RIN := if(l.ActivityType, '1', 8, 10); //https://confluence.rsi.lexisnexis.com/display/GTG/Data+Source+Identification
			SELF := L;
			SELF := [];
	END;
	...
	KnownFraud MapKNFD(Sprayed_NAC L) := TRANSFORM 
			...
			SELF.Source_RIN := 11; //https://confluence.rsi.lexisnexis.com/display/GTG/Data+Source+Identification
			SELF := L;
			SELF := [];
	END;

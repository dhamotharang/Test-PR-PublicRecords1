...
IMPORT Risk_Indicators, RiskWise, zz_Koubsky;
...
//===================  input-output files  ======================
// infile_name := '~scoringqa::out::bs_41_tracking_edina_nonfcra_no_edina_qh_bestaddr_base_20150311';
// outfile_name := '~nkoubsky::out::bs_41_tracking_nonfcra_qh_bestaddr_base_' + thorlib.wuid();
infile_name := '~scoringqa::out::bs_41_tracking_edina_nonfcra_no_edina_qh_bestaddr_test_20150312';
outfile_name := '~nkoubsky::out::bs_41_tracking_nonfcra_qh_bestaddr_test_' + thorlib.wuid();
...
ds_baseline := PROJECT(dataset(infile_name, input_lay, thor), TRANSFORM(input_lay, SELF.time_ms := 0; SELF := LEFT));
...
f := project(ds_baseline, transform(ox, self := left));
// output(choosen(f,eyeball), named('infile'));


risk_indicators.Layout_Boca_Shell_Edina_v41 convertToEdina_v4(f le) := transform

// EXPORT PRCT_Instant_ID_Collection := 'todo';
...
String outfile_name :=  'nkoubsky::prct::test' ;
...
layout_soap_input append_settings(in_file le, INTEGER c) := TRANSFORM
		SELF.Accountnumber := (STRING) c;	
		
		SELF.FirstName := le.fname;
		SELF.MiddleName := le.mname;
		SELF.LastName := le.lname;
		SELF.NameSuffix := le.suffix;
		SELF.DateOfBirth := le.dob;
		SELF.SSN := le.ssn;
		SELF.StreetAddress := le.addr1+' '+le.addr2;
		SELF.City := le.city;
		SELF.State := le.st;
		SELF.Zip := le.zip;
		SELF.HomePhone := le.phone;

		SELF.DPPAPurpose := Scoring_Project_PIP.User_Settings_Module.IID_Scores_V0_XML_Generic_settings.DPPA;
		SELF.GLBPurpose := Scoring_Project_PIP.User_Settings_Module.IID_Scores_V0_XML_Generic_settings.GLB;
		self.DataRestrictionMask := Scoring_Project_PIP.User_Settings_Module.IID_Scores_V0_XML_Generic_settings.DRM;	
...
//ds_soap_in
soap_in := DISTRIBUTE(PROJECT(in_file, append_settings(LEFT, counter)), Random());
output(soap_in, named('soap_in'));
    ...
    END;
	  ...
	  ds_with_extras:=join(ds_slim,soap_in,left.acctno=(string)right.accountnumber ,internal_extras_append(left, right));	
		output(ds_with_extras(nap_summary <> 0), named('nap_cases'));
		output(count(ds_with_extras(nap_summary <> 0)), named('nap_cases_count'));

	  //final file out to thor
	  final_output := output(ds_with_extras,, outfile_name+'_'+thorlib.wuid(), csv(heading(single), quote('"')), OVERWRITE);

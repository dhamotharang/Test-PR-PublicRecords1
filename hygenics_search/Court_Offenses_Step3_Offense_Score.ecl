import crimsrch, lib_stringlib;
	...
	CrimSrch.Layout_Moxie_Offenses_temp tBlankScoreIfNotConvicted(Court_Offenses_Step2_Conviction pInput) := transform
...
//***** START Set traffic-only offenses to traffic score first
	CrimSrch.Layout_Moxie_Offenses_temp tSetTrafficScoreIfTraffic(CrimSrch.Layout_Moxie_Offenses_temp pInput) := transform
...
//***** END Set traffic offenses to traffic score first
	...
	CrimSrch.Layout_Moxie_Offenses_temp tTranslateOffenseLevel(CrimSrch.Layout_Moxie_Offenses_temp pInput) := transform
...
dCourtOffensesTranslatedLevel	:= project(dCourtOffensesTrafficScored,tTranslateOffenseLevel(left));
	...
	CrimSrch.Layout_Moxie_Offenses_temp tScoreOffenseCodes(CrimSrch.Layout_Moxie_Offenses_temp pInput) := transform
...
dCourtOffensesScoredOffCode		:= project(dCourtOffensesTranslatedLevel,tScoreOffenseCodes(left));
	...
	export Court_Offenses_Step3_Offense_Score := dCourtOffensesScoredOffCode : persist('Persist::CrimSrch_Court_Offenses_Scored');
#else
	export Court_Offenses_Step3_Offense_Score := dCourtOffensesScoredOffCode;
#end
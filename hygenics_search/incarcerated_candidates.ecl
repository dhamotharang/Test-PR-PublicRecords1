...
export incarcerated_candidates := module
	...
	toffender 		:=	table(distribute(File_Crim_Offender2, hash(offender_key)), {offender_key, data_type, case_filing_dt, party_status_desc});
	toffenses 		:=	/* no doc */ table(distribute(File_Court_Offenses, hash(offender_key)), {offender_key, off_date, arr_date, court_disp_date, court_disp_desc_1, court_disp_code, sent_date, sent_jail}); //using accurint, lost incarceration adm date and orig offense key
	tdocoffenses 	:=	table(distribute(File_DOC_Offenses, hash(offender_key)), {offender_key, offense_key, arr_date, off_date, ct_disp_dt, ct_disp_desc_1, stc_dt, stc_lgth, stc_lgth_desc, inc_adm_dt, min_term, min_term_desc, max_term, max_term_desc});
	tpunishment 	:=	table(distribute(File_DOC_Punishment, hash(offender_key)), {offender_key, punishment_type, offense_key, event_dt, sent_length, sent_length_desc, inmate_cur_status_desc := cur_stat_inm_desc, inmate_cur_loc := cur_loc_inm, admit_latest_date := latest_adm_dt, sch_rel_dt, ctl_rel_dt, release_actual_date := act_rel_dt, parole_start_date := par_st_dt});
...
comboutput2 := join(comboutput1, tpunishment(punishment_type = 'I'), left.offender_key = right.offender_key, combine_files2(left,right));
	...
	comb_layout combine_files3(comb_layout l, tdocoffenses r) := transform
		...
		self.conviction_override_date_type := case(self.conviction_override_date,
													...
													'U');
		self.fcra_conviction_flag 			:= if(l.offender_key[1..2]  not in sDOC_Vendors_Conviction_Unknown and l.data_type = '1',
													if(l.Data_Type='1' and
														l.offender_key[1..2] not in sDOC_Vendors_Conviction_Unknown and
														l.offender_key[1..2] not in sDOC_Vendors_NoOffense_NoConviction,
														'Y',
														'D'), 
												if(l.offender_key[1..2] in sDOC_Vendors_Conviction_Unknown and l.data_type = '1', 
													'U',
													if(l.Data_Type='1' and
														l.offender_key[1..2] not in sDOC_Vendors_Conviction_Unknown 	and
														l.offender_key[1..2] not in sDOC_Vendors_NoOffense_NoConviction,
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	shared flag_incadmdt(string input) := function
		testFail := if(input not between '19000101' and Version.Development, true, false);
	...
	shared flag_admitldate(string input) := function
		testFail := if(input between '19000101' and Version.Development, true, false);
		testPass := map(testFail => 0, 50);
		return testPass;
	end;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	comb_layout_with_flags addincflagses(comboutput3 l) := transform
		...
		prestatus_date					:= ut.date_add( time_to_add, MAP(
									vrPUN_admit_latest_date between '19010101' and  Version.Development=> vrPUN_admit_latest_date,
									vrDSES_inc_adm_dt between '19010101' and  Version.Development=> vrDSES_inc_adm_dt,
									vrSES_sent_date between '19010101' and  Version.Development=> vrSES_sent_date,
									vrDSES_stc_dt between '19010101' and  Version.Development=> vrDSES_stc_dt,
									vrSES_court_disp_date between '19010101' and  Version.Development=> vrSES_court_disp_date,
									vrDSES_ct_disp_dt between '19010101' and  Version.Development  => vrDSES_ct_disp_dt,
									vrder_party_status_desc between '19010101' and  Version.Development  => vrder_party_status_desc,
									vrDER_case_filing_dt between '19010101' and  Version.Development=> vrDER_case_filing_dt,
									vrSES_arr_date between '19010101' and  Version.Development  => vrSES_arr_date,
									vrDSES_arr_date between '19010101' and  Version.Development  => vrDSES_arr_date,
									vrSES_off_date between '19010101' and  Version.Development  => vrSES_off_date,
									vrDSES_off_date between '19010101' and  Version.Development  => vrDSES_off_date,
									vrPUN_event_date between '19010101' and  Version.Development  => vrPUN_event_date, '')) + 
									map(vrPUN_admit_latest_date between '19010101' and  Version.Development  =>  'PUNadmitlatestdate',
									vrDSES_inc_adm_dt between '19010101' and  Version.Development  =>  'DSESincadmdt',
									vrSES_sent_date between '19010101' and  Version.Development  =>  'SESsentdate',
									vrDSES_stc_dt between '19010101' and  Version.Development  =>  'DSESstcdt',
									vrSES_court_disp_date between '19010101' and  Version.Development  =>  'SEScourtdispdate',
									vrDSES_ct_disp_dt between '19010101' and  Version.Development  =>  'DSESctdispdt',
									vrder_party_status_desc  between '19010101' and  Version.Development  =>  'DERpartystatusdesc',
									vrDER_case_filing_dt between '19010101' and  Version.Development  =>  'DERcasefilingdt',
									vrSES_arr_date between '19010101' and  Version.Development  =>  'SESarrdate',
									vrDSES_arr_date between '19010101' and  Version.Development  =>  'DSESarrdate',
									vrSES_off_date between '19010101' and  Version.Development  =>  'SESoffdate',
									vrDSES_off_date  between '19010101' and  Version.Development  =>  'DSESoffdate',
									vrPUN_event_date between '19010101' and  Version.Development =>  'PUNeventdate', '');
		...
		autoDATE :=	if(stringlib.stringtolowercase(offender_key[1..2]) in refresh_list and punishment_type = 'I' and fcra_conviction_flag in ['Y', 'D'] and vrPUN_release_actual_date + vrPUN_sch_rel_dt + vrPUN_ctl_rel_dt +  vrPUN_parole_start_date > '19010101',
						if(vrPUN_parole_start_date between '19010101' and  Version.Development, vrPUN_parole_start_date,
							if(vrPUN_release_actual_date between '19010101' and  Version.Development, vrPUN_release_actual_date,
								if(vrPUN_sch_rel_dt between '19010101' and  Version.Development, vrPUN_sch_rel_dt,
									if(vrPUN_ctl_rel_dt between '19010101' and  Version.Development, vrPUN_ctl_rel_dt, '')))),
							'');
		...
		self.incar_appr_rel_date			:= choose_incar_appr_rel_date( l.offender_key,

...
Layout_Moxie_Activity tDOCtoCrimSrchActivity(Crim_Common.Layout_In_Court_Activity pInput):= transform
...
lActivityProcessed := project(File_In_Court_Activity,tDOCtoCrimSrchActivity(left));

#if(CrimSrch.Switch_ShouldUsePersists = CrimSrch.Switch_YesValue)
	export Activity_Joined := lActivityProcessed : persist('persist::CrimSrch_Activity_Joined');
#else

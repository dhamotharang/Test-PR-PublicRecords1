...
export Filenames(string pVersion = '', boolean pFCRA = false, boolean pDaily = true, string source='') := module
	...
	shared prefix_new                            := if(pFCRA
	                                                   ,INQL_V2._Constants.NEW_FCRA_PREFIX
													   ,INQL_V2._Constants.NEW_NONFCRA_PREFIX);

    shared prefix_batchr3                        := if(pFCRA
	                                                   ,INQL_v2._Constants.FCRA_PREFIX
													   ,INQL_v2._Constants.NONFCRA_PREFIX);
												   

	//INQL_V2 Inputs
	export Input                                 := prefix_new +'inql'+fcra+'in::'+source;

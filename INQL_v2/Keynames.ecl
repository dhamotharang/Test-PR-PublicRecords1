...
EXPORT Address								:= tools.mod_FilenamesBuild(if(isSF,addressSFTemplate,addressLGTemplate) ,pVersion);
...
EXPORT DID										:= tools.mod_FilenamesBuild(if(isSF,didSFTemplate,didLGTemplate) ,pVersion);
...
EXPORT FEIN										:= tools.mod_FilenamesBuild(if(isSF,feinSFTemplate,feinLGTemplate) ,pVersion);
...
EXPORT IPADDR									:= tools.mod_FilenamesBuild(if(isSF,ipaddrSFTemplate,ipaddrLGTemplate) ,pVersion);
...
EXPORT Linkids								:= tools.mod_FilenamesBuild(if(isSF,linkidsSFTemplate,linkidsLGTemplate) ,pVersion);
...
EXPORT Name										:= tools.mod_FilenamesBuild(if(isSF,nameSFTemplate,nameLGTemplate) ,pVersion);
...
EXPORT Phone								:= tools.mod_FilenamesBuild(if(isSF,phoneSFTemplate,phoneLGTemplate) ,pVersion);
...
EXPORT SSN								:= tools.mod_FilenamesBuild(if(isSF,ssnSFTemplate,ssnLGTemplate) ,pVersion);
...
EXPORT Transaction_id								:= tools.mod_FilenamesBuild(if(isSF,transactionidSFTemplate,transactionidLGTemplate) ,pVersion);
...
EXPORT Email								  := tools.mod_FilenamesBuild(if(isSF,emailSFTemplate,emailLGTemplate) ,pVersion);

EXPORT billgroupsSFTemplate				:= prefixSF + '@version@::billgroups_did';
EXPORT billgroupsLGTemplate				:= prefixLG + '@version@::billgroups_did';
EXPORT Billgroups_did						  := tools.mod_FilenamesBuild(if(isSF,billgroupsSFTemplate,billgroupsLGTemplate) ,pVersion);
...
EXPORT Industry_use_vertical			:= tools.mod_FilenamesBuild(if(isSF,induseverticalSFTemplate,induseverticalLGTemplate) ,pVersion);

EXPORT induseverticallogSFTemplate		:= prefixSF + 'industry_use_vertical_login@version@';
EXPORT induseverticallogLGTemplate		:= prefixSF + '@version@::industry_use_vertical_login';
EXPORT Industry_use_vertical_login		:= tools.mod_FilenamesBuild(if(isSF,induseverticallogSFTemplate,induseverticallogLGTemplate) ,pVersion);

EXPORT lookupfuncdescSFTemplate				:= prefixSF + 'lookup_function_desc@version@';
EXPORT lookupfuncdescLGTemplate				:= prefixSF + '@version@::lookup_function_desc';
EXPORT Lookup_function_desc						:= tools.mod_FilenamesBuild(if(isSF,lookupfuncdescSFTemplate,lookupfuncdescLGTemplate) ,pVersion);

EXPORT didriskSFtemplate     					:= STD.STr.RemoveSuffix(prefixSF,'::')  + '_did_qa';
EXPORT didriskLGtemplate							:= prefixSF + '@version@::did';
EXPORT DID_Risk												:= tools.mod_FilenamesBuild(if(isSF,didriskSFtemplate,didriskLGtemplate) ,pVersion);

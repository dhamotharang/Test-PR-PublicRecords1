...
EXPORT fnMap_Batch_PIIs(boolean fcra = false, unsigned logType = 0) := function

	// n := INQL_v2.test_count; /* n - to test a sample set, 0 to run all */		
	n := 0;
	blankRefCode := choosen(INQL_v2.Standardize_input(fcra,logType).Batch
									(length(orig_address1_z5 + orig_address1_z4) < 12	and
									 orig_process_id in Inql_V2._Constants.SBFE_REPORT_BATCH_PROCESS_ID_FILTER), IF(n > 0, n, choosen:ALL));
...
cleanCompany := project(input_data_dedup,
									...
									self.company_fax_number        		:=	Inql_V2.fncleanfunctions.clean_phone(left.orig_company_fax_number);         
...
subjectNormRec fSubjectNorm(recordof(INQL_v2.Standardize_input(fcra,logType).Batch) L,INTEGER C ) := TRANSFORM
SELF := L;
SELF.subj_nbr  := C;
...
cleanSubject := project(SubjectNorm,
									...
									self.subj_ssn       				:= Inql_V2.fncleanfunctions.clean_ssn(left.orig_subj_ssn);
									self.subj_dob       				:= Inql_V2.fncleanfunctions.clean_dob(left.orig_subj_dob);
								  ...
								  self.subj_former_last_name 	:= left.orig_subj_former_last_name;
									...
									self.subj_phone 									:= Inql_V2.fncleanfunctions.clean_phone(left.orig_subj_phone);
									self.subj_work_phone							:= Inql_V2.fncleanfunctions.clean_phone(left.orig_subj_work_phone);
...
AppendTransCompanySubjects := project(DeNormCleanInput, transform(Inql_V2.Layouts.rBatch_PIIs_Base, 

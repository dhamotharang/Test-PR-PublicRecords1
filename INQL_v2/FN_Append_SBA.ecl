...
EXPORT FN_Append_SBA(dataset({Inquiry_AccLogs.Layout.Common_ThorAdditions_non_fcra,bipv2.IDlayouts.l_xlink_ids}) pInFile) := function

pInFile_dist := distribute(project(pInFile, transform({Inquiry_AccLogs.Layout.Common_ThorAdditions_SBA,bipv2.IDlayouts.l_xlink_ids},
self := left, self := [])), hash(search_info.transaction_id));

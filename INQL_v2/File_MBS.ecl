...
export File := dataset(data_services.foreign_new_logs + 'thor100_21::out::inquiry_acclogs::file_mbs', INQL_v2.layouts.FIDO_new_MBS, thor);

export CreateFile(string param_version = ut.GetDate) := function

//apply filter on healthcare CIDs

FlagHealthcareCIDs := JOIN(INQL_v2.Files().file_FIDO, INQL_v2.File_Lookups.HealthcareCIDs, 
...
transform(INQL_v2.layouts.FIDO_new_MBS,
...
self.internal_flag := MAP(left.mbs_company_id = INQL_v2.accounts_exclude.company_id_not_internal and left.mbs_product_id = 1 => '', 
left.mbs_company_id in INQL_v2.accounts_exclude.set_company_id_internal and left.mbs_product_id = 1 => 'Y',
...
self.allowflags := 																			
																										...
																										map(self.disable_observation = '1' or (left.mbs_product_id = 1 and self.company_id in INQL_v2.File_Lookups.set_BridgerDisable) => ut.bit_set(0,12), ut.bit_set(0,0))| //0 - enable observation/ 1 - disable observation
																										map(left.opt_out = '1' or self.company_id in INQL_v2.File_Lookups.set_BridgerDisable  => ut.bit_set(0,13), ut.bit_set(0,0)) |
																										map(self.internal_flag = 'Y' and self.gc_id <> INQL_v2.accounts_exclude.gc_id_not_internal => ut.bit_set(0,14), ut.bit_set(0,0)) |
																										map(left.exclude_from_access = 1 => ut.bit_set(0,15), ut.bit_set(0,0)) |
																										map(self.vertical in INQL_v2.fnCleanFunctions.FilterCds => ut.bit_set(0,17), ut.bit_set(0,0)) |
																										map(self.vertical = 'AUTO FINANCE' => ut.bit_set(0,0),
																												regexfind(INQL_v2.fnCleanFunctions.FilterCds_extra,self.vertical,nocase) => ut.bit_set(0,17), ut.bit_set(0,0)) |
																										map(self.industry in INQL_v2.fnCleanFunctions.Industry_FilterCds => ut.bit_set(0,18), ut.bit_set(0,0)) |
																										map(self.sub_market in INQL_v2.fnCleanFunctions.SubMarket_FilterCds => ut.bit_set(0,19), ut.bit_set(0,0))|
																										map(regexfind(INQL_v2.fnCleanFunctions.healthcare_FilterCds_extra,self.mbs_company_name) and 
																										...
																										self.translation := INQL_v2.fnTranslations.allowflags_str(allowflag_val);
...
fido_stats := table(df_fido_mbs_dedup, {gc_id, company_id,product_id,sub_product_id,priority_flag, id_cnt := count(group)}, 
	...
	FileServices.SendEmail(Email_Notification_Lists.BuildFailure,'FIDO has issues on logs thor '+ workunit ,'FIDO ID not unique' + failmessage)); 
	
	pValidate_subaccountID := if(count(df_fido_mbs_dedup(sub_acct_id = '')) > 1, 
	FileServices.SendEmail(Email_Notification_Lists.BuildFailure,'FIDO has issues on logs thor '+ workunit ,'FIDO subaccountID blank' + failmessage)); 

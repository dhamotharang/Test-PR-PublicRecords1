...
EXPORT CitizenshipBatchMain(DATASET(DueDiligence.Layouts.Input) input, 
  ...
  validatedCitizenshipRequests := Citizenship.Validation.ValidateInput(input, inGLBA, inDPPA, in_modelName);
  
  //clean the data
  cleanedCitizenshipData := Citizenship.Validation.CleanData(validatedCitizenshipRequests(validRequest));

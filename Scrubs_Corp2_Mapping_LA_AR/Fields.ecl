...
EXPORT SALT30.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_future_date','invalid_charter_nbr','invalid_numericblank','invalid_numericperiodblank','invalid_alphablank','invalid_alphanumericblank');
EXPORT FieldTypeNum(SALT30.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_corp_vendor' => 2,'invalid_state_origin' => 3,'invalid_mandatory' => 4,'invalid_date' => 5,'invalid_future_date' => 6,'invalid_charter_nbr' => 7,'invalid_numericblank' => 8,'invalid_numericperiodblank' => 9,'invalid_alphablank' => 10,'invalid_alphanumericblank' => 11,0);
...
EXPORT MakeFT_invalid_corp_vendor(SALT30.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_vendor(SALT30.StrType s) := WHICH(((SALT30.StrType) s) NOT IN ['22']);
EXPORT InValidMessageFT_invalid_corp_vendor(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInEnum('22'),SALT30.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_state_origin(SALT30.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_state_origin(SALT30.StrType s) := WHICH(((SALT30.StrType) s) NOT IN ['LA']);
EXPORT InValidMessageFT_invalid_state_origin(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInEnum('LA'),SALT30.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_numericblank(SALT30.StrType s0) := FUNCTION
  s1 := SALT30.stringfilter(s0,'0123456789 '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_numericblank(SALT30.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT30.StringFilter(s,'0123456789 '))));
EXPORT InValidMessageFT_invalid_numericblank(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInChars('0123456789 '),SALT30.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_numericperiodblank(SALT30.StrType s0) := FUNCTION
  s1 := SALT30.stringfilter(s0,'.0123456789 '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_numericperiodblank(SALT30.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT30.StringFilter(s,'.0123456789 '))));
EXPORT InValidMessageFT_invalid_numericperiodblank(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInChars('.0123456789 '),SALT30.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_alphablank(SALT30.StrType s0) := FUNCTION
  s1 := SALT30.stringfilter(s0,'ABCDEFGHIJKLMNOPQRSTUVWXYZ '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_alphablank(SALT30.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT30.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZ '))));
EXPORT InValidMessageFT_invalid_alphablank(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZ '),SALT30.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_alphanumericblank(SALT30.StrType s0) := FUNCTION
  s1 := SALT30.stringfilter(s0,'0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_alphanumericblank(SALT30.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT30.StringFilter(s,'0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ '))));
EXPORT InValidMessageFT_invalid_alphanumericblank(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInChars('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ '),SALT30.HygieneErrors.Good);
...
EXPORT FieldNum(SALT30.StrType fn) := CASE(fn,'corp_key' => 1,'corp_vendor' => 2,'corp_vendor_county' => 3,'corp_vendor_subcode' => 4,'corp_state_origin' => 5,'corp_process_date' => 6,'corp_sos_charter_nbr' => 7,'ar_year' => 8,'ar_mailed_dt' => 9,'ar_due_dt' => 10,'ar_filed_dt' => 11,'ar_report_dt' => 12,'ar_report_nbr' => 13,'ar_franchise_tax_paid_dt' => 14,'ar_delinquent_dt' => 15,'ar_tax_factor' => 16,'ar_tax_amount_paid' => 17,'ar_annual_report_cap' => 18,'ar_illinois_capital' => 19,'ar_roll' => 20,'ar_frame' => 21,'ar_extension' => 22,'ar_microfilm_nbr' => 23,'ar_comment' => 24,'ar_type' => 25,'ar_exempt' => 26,'ar_license_tax_amount' => 27,'ar_status' => 28,'ar_paid_date' => 29,'ar_prev_paid_date' => 30,'ar_prev_tax_factor' => 31,'ar_extension_date' => 32,'ar_report_mail_date' => 33,'ar_deliquent_report_mail_date' => 34,'ar_report_filed_date' => 35,'ar_year_and_month_due' => 36,0);
...
EXPORT Make_ar_year(SALT30.StrType s0) := MakeFT_invalid_numericblank(s0);
EXPORT InValid_ar_year(SALT30.StrType s) := InValidFT_invalid_numericblank(s);
EXPORT InValidMessage_ar_year(UNSIGNED1 wh) := InValidMessageFT_invalid_numericblank(wh);
 
EXPORT Make_ar_mailed_dt(SALT30.StrType s0) := s0;
EXPORT InValid_ar_mailed_dt(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_mailed_dt(UNSIGNED1 wh) := '';
 
EXPORT Make_ar_due_dt(SALT30.StrType s0) := s0;
EXPORT InValid_ar_due_dt(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_due_dt(UNSIGNED1 wh) := '';
 
EXPORT Make_ar_filed_dt(SALT30.StrType s0) := MakeFT_invalid_date(s0);
EXPORT InValid_ar_filed_dt(SALT30.StrType s) := InValidFT_invalid_date(s);
EXPORT InValidMessage_ar_filed_dt(UNSIGNED1 wh) := InValidMessageFT_invalid_date(wh);
 
EXPORT Make_ar_report_dt(SALT30.StrType s0) := MakeFT_invalid_numericblank(s0);
EXPORT InValid_ar_report_dt(SALT30.StrType s) := InValidFT_invalid_numericblank(s);
...
EXPORT Make_ar_franchise_tax_paid_dt(SALT30.StrType s0) := s0;
EXPORT InValid_ar_franchise_tax_paid_dt(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_franchise_tax_paid_dt(UNSIGNED1 wh) := '';
 
EXPORT Make_ar_delinquent_dt(SALT30.StrType s0) := s0;
EXPORT InValid_ar_delinquent_dt(SALT30.StrType s) := FALSE;
...
EXPORT Make_ar_exempt(SALT30.StrType s0) := s0;
EXPORT InValid_ar_exempt(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_exempt(UNSIGNED1 wh) := '';
 
EXPORT Make_ar_license_tax_amount(SALT30.StrType s0) := s0;
EXPORT InValid_ar_license_tax_amount(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_license_tax_amount(UNSIGNED1 wh) := '';
 
EXPORT Make_ar_status(SALT30.StrType s0) := s0;
EXPORT InValid_ar_status(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_status(UNSIGNED1 wh) := '';
 
EXPORT Make_ar_paid_date(SALT30.StrType s0) := s0;
EXPORT InValid_ar_paid_date(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_paid_date(UNSIGNED1 wh) := '';
 
EXPORT Make_ar_prev_paid_date(SALT30.StrType s0) := s0;
EXPORT InValid_ar_prev_paid_date(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_prev_paid_date(UNSIGNED1 wh) := '';
 
EXPORT Make_ar_prev_tax_factor(SALT30.StrType s0) := s0;
EXPORT InValid_ar_prev_tax_factor(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_prev_tax_factor(UNSIGNED1 wh) := '';
 
EXPORT Make_ar_extension_date(SALT30.StrType s0) := s0;
EXPORT InValid_ar_extension_date(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_extension_date(UNSIGNED1 wh) := '';
 
EXPORT Make_ar_report_mail_date(SALT30.StrType s0) := s0;
EXPORT InValid_ar_report_mail_date(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_report_mail_date(UNSIGNED1 wh) := '';
 
EXPORT Make_ar_deliquent_report_mail_date(SALT30.StrType s0) := s0;
EXPORT InValid_ar_deliquent_report_mail_date(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_deliquent_report_mail_date(UNSIGNED1 wh) := '';
 
EXPORT Make_ar_report_filed_date(SALT30.StrType s0) := s0;
EXPORT InValid_ar_report_filed_date(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_report_filed_date(UNSIGNED1 wh) := '';
 
EXPORT Make_ar_year_and_month_due(SALT30.StrType s0) := s0;
EXPORT InValid_ar_year_and_month_due(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_ar_year_and_month_due(UNSIGNED1 wh) := '';
// This macro will compute and count field level differences based upon a pivot expression
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT30,Scrubs_Corp2_Mapping_LA_AR;

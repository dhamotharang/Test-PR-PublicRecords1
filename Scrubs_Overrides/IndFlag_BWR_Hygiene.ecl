...
#workunit('name','Scrubs_Overrides.IndFlag_BWR_Hygiene - Hygiene & Stats - SALT V3.11.4');
IMPORT Scrubs_Overrides,SALT311;
// First create an instantiated hygiene module
  infile := Scrubs_Overrides.IndFlag_In_Overrides;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_Overrides.IndFlag_hygiene(ip);
  ...
  OUTPUT(SALT311.MAC_Character_Counts.EclRecord(p,'IndFlag_Layout_Overrides'),NAMED('OptimizedLayout'));// File layout suggested by data
  ...
  // IF you find yourself using ALL of these a LOT - let me know, I can make the 'all' case faster
   Examples := 10;
  //  OUTPUT(SALT311.MAC_CrossTab(infile,Key_Ind,flag_file_id,Examples),NAMED('flag_file_idByKey_Ind'));

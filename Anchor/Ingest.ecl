IMPORT STD,SALT38,Anchor;
EXPORT Ingest(BOOLEAN incremental=FALSE
, DATASET(Layout_Anchor) Delta = DATASET([],Layout_Anchor)
, DATASET(Layout_Anchor) dsBase = In_Anchor // Change IN_Anchor to change input to ingest process
, DATASET(RECORDOF(Anchor.prep_ingest_file))  infile = Anchor.prep_ingest_file
) := MODULE
  SHARED NullFile := DATASET([],Layout_Anchor); // Use to replace files you wish to remove
  ...
  SHARED WithRT MergeData(WithRT le, WithRT ri) := TRANSFORM // Pick the data for the new record
    SELF.address_1 := ri.address_1; // Derived(NEW)
    SELF.address_2 := ri.address_2; // Derived(NEW)
    SELF.sourceurl := ri.sourceurl; // Derived(NEW)
    SELF.ipaddress := ri.ipaddress; // Derived(NEW)
    SELF.optindate := ri.optindate; // Derived(NEW)
    SELF.anchorinternalcode := ri.anchorinternalcode; // Derived(NEW)
    SELF.addresstype := ri.addresstype; // Derived(NEW)
    SELF.latitude := ri.latitude; // Derived(NEW)
    SELF.longitude := ri.longitude; // Derived(NEW)
  ...
  DistIngest0 := DISTRIBUTE(FilesToIngest0, HASH32(firstname,lastname,city,state,zipcode,emailaddress,dob));
  SortIngest0 := SORT(DistIngest0,firstname,lastname,city,state,zipcode,emailaddress,dob, __Tpe, RCID, LOCAL);
  GroupIngest0 := GROUP(SortIngest0,firstname,lastname,city,state,zipcode,emailaddress,dob, LOCAL, ORDERED, STABLE);
  SHARED AllIngestRecs0 := UNGROUP(ROLLUP(GroupIngest0,TRUE,MergeData(LEFT,RIGHT)));
 
  // Existing Base: combine delta with base file
  DistBase0 := DISTRIBUTE(Base0+Delta0, HASH32(firstname,lastname,city,state,zipcode,emailaddress,dob));
  SortBase0 := SORT(DistBase0,firstname,lastname,city,state,zipcode,emailaddress,dob, __Tpe, RCID, LOCAL);
  GroupBase0 := GROUP(SortBase0,firstname,lastname,city,state,zipcode,emailaddress,dob, LOCAL, ORDERED, STABLE);
  SHARED AllBaseRecs0 := UNGROUP(ROLLUP(GroupBase0,TRUE,MergeData(LEFT,RIGHT)));
 
  // Everything: combine ingest and base recs
  Sort0 := SORT(AllBaseRecs0+AllIngestRecs0,firstname,lastname,city,state,zipcode,emailaddress,dob, __Tpe,RCID,LOCAL);
  Group0 := GROUP(Sort0,firstname,lastname,city,state,zipcode,emailaddress,dob,LOCAL, ORDERED, STABLE);
  ...
  SHARED AllRecs := ORe+NR1+NR(RCID<>0) : PERSIST('~temp::Anchor::Ingest_Cache',EXPIRE(Anchor.Config.PersistExpire));
  ...
  EXPORT NewRecords_NoTag := PROJECT(NewRecords,Layout_Anchor);
  EXPORT OldRecords :=PROJECT( AllRecs(__Tpe=RecordType.Old), NoFlagsRec);
  EXPORT OldRecords_NoTag := PROJECT(OldRecords,Layout_Anchor);
  EXPORT UpdatedRecords := PROJECT(AllRecs(__Tpe=RecordType.Updated), NoFlagsRec);
  EXPORT UpdatedRecords_NoTag := PROJECT(UpdatedRecords,Layout_Anchor);
  EXPORT AllRecords := IF(incremental, NewRecords, PROJECT(AllRecs, NoFlagsRec));
  EXPORT AllRecords_NoTag := PROJECT(AllRecords,Layout_Anchor); // Records in 'pure' format

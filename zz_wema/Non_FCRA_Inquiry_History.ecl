EXPORT Non_FCRA_Inquiry_History(Par1,Par2):=FUNCTIONMACRO
...
nonfcra_inquiries :=project(choosen(Inquiry_AccLogs.Key_Inquiry_DID((unsigned)search_info.datetime[1..8]<=BuildVersion),eyeball), transform_raw(left));
nonfcra_inquiries_update := project(choosen(Inquiry_AccLogs.Key_Inquiry_DID_Update((unsigned)search_info.datetime[1..8]<=BuildVersion2),eyeball), transform_raw(left));

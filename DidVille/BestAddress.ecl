...
export best_from_best_logic(dataset(doxie.layout_presentation) pres_recs) :=FUNCTION
// start best logic 
...
Export Best_Recs(dataset(doxie.layout_presentation) header_recs0,dataset(doxie.layout_references) did, unsigned1 dppa_purpose,unsigned1 glb_purpose,
string5 input_zip,string10 prange_value, unsigned1 fi_value, unsigned1 li_value)  := FUNCTION
...
w_gong_append := doxie.append_gong(pres_recs,rels);
...
srt_gong_verified := if(ut.glb_ok(glb_purpose),srt_gong_verified0,srt_gong_verified1); 

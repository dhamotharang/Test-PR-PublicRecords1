...
zero := 0;
	...
	lskname := Constants().autokeyBid_template				;
	llgname := Keynames(pversion).bid_autokeyroot.new	;
	
	Addr_file:=Diversity_Certification.Address_Norm(pversion);
	new_pBase:= project(Addr_file,transform(Diversity_Certification.Layouts.Auto_KeyBuild-bid, self.bdid:=left.bid,self:=left));
	...
	module(AutokeyB2.Fn_Build.params) 
		...
		export string											L_inkeyname						:= Constants().autokeyBid_template	;
	...
	Build_autokeys := AutokeyB2.Fn_Build.Do(mod_AKB

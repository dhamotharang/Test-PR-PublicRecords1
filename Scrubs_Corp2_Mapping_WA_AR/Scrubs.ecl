...
EXPORT FromNone(DATASET(Layout_In_WA) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.ar_comment_Invalid := Fields.InValid_ar_comment((SALT311.StrType)le.ar_comment);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Layout_In_WA);
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.corp_key_Invalid << 0 ) + ( le.corp_vendor_Invalid << 2 ) + ( le.corp_state_origin_Invalid << 3 ) + ( le.corp_process_date_Invalid << 4 ) + ( le.corp_sos_charter_nbr_Invalid << 6 ) + ( le.ar_due_dt_Invalid << 8 ) + ( le.ar_report_dt_Invalid << 10 ) + ( le.ar_comment_Invalid << 12 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.ar_comment_Invalid := (le.ScrubsBits1 >> 12) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    ar_comment_CUSTOM_ErrorCount := COUNT(GROUP,h.ar_comment_Invalid=1);
    AnyRule_WithErrorsCount := COUNT(GROUP, h.corp_key_Invalid > 0 OR h.corp_vendor_Invalid > 0 OR h.corp_state_origin_Invalid > 0 OR h.corp_process_date_Invalid > 0 OR h.corp_sos_charter_nbr_Invalid > 0 OR h.ar_due_dt_Invalid > 0 OR h.ar_report_dt_Invalid > 0 OR h.ar_comment_Invalid > 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.ar_comment_Invalid,'CUSTOM','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_sos_charter_nbr','ar_due_dt','ar_report_dt','ar_comment','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_future_date','invalid_date','invalid_ar_comment','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT311.StrType)le.corp_key,(SALT311.StrType)le.corp_vendor,(SALT311.StrType)le.corp_state_origin,(SALT311.StrType)le.corp_process_date,(SALT311.StrType)le.corp_sos_charter_nbr,(SALT311.StrType)le.ar_due_dt,(SALT311.StrType)le.ar_report_dt,(SALT311.StrType)le.ar_comment,'***SALTBUG***');
  END;
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'ar_comment:invalid_ar_comment:CUSTOM'
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.ar_comment_CUSTOM_ErrorCount
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ...
          ,le.ar_due_dt_ALLOW_ErrorCount,le.ar_due_dt_CUSTOM_ErrorCount
          ,le.ar_report_dt_ALLOW_ErrorCount,le.ar_report_dt_CUSTOM_ErrorCount
          ,le.ar_comment_CUSTOM_ErrorCount,0) / le.TotalCnt, CHOOSE(c - NumRules
...
EXPORT StandardStats(DATASET(Layout_In_WA) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Corp2_Mapping_WA_AR, Fields, 'RECORDOF(scrubsSummaryOverall)', '');

import property, lib_fileservices, ut, Business_Header, Targus, PRTE2_Targus, MDR, Header;

export consumer_as_header(dataset(targus.layout_consumer_out) pTargusConsumer = files.targus_header)
 :=
  function
  
  dConsumerbase := pTargusConsumer(lname != ''
                   ,prim_name != ''
                   ,zip != ''
                   ,length(trim(fname))>1
                   );

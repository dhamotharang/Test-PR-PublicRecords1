...
export dataset(l_narrow) by_DL_src(
  ...
  return results;
	...
	// ------------------------------------------
  // Return DL data in the moxie report format
	// ------------------------------------------
  export moxie_view := module
	
		shared l_moxieTmp toMoxie(l_wide L) := transform
			self.did 										:= if(L.did=0,'', intformat(L.did,12,1));
			self.pgid										:= if(L.Preglb_did<>0, (string)L.Preglb_did, '' ); // STUB - is Preglb_did right?
			self.history								:= if(L.orig_state='MI','U',L.History); // Drivers.BWR_Make_FLDL_DID line 62
			self.state_origin						:= L.orig_state;
			...
			self.race										:= L.race;
			self.sex_flag								:= L.sex_flag;
			self.license_type						:= L.moxie_license_type;
			self.attention_flag					:= L.attention_flag;
			self.dod 										:= L.dod;
			self.restrictions						:= L.restrictions;
			...
			self.motorcycle_code 				:= L.motorcycle_code;
			self.dl_number							:= if(L.dl_number[1..5]='LNID:','',L.dl_number); // Drivers.BWR_Make_FLDL_DID line 58
			self.ssn 										:= L.ssn;
			self.age 										:= L.age;
			self.privacy_flag						:= L.privacy_flag;
			self.dl_orig_issue_date			:= if( L.orig_issue_date<>0, (string)L.orig_issue_date, '' );
			self.height									:= L.height;
			self.oos_previous_dl_number	:= L.oos_previous_dl_number;
			self.oos_previous_st 				:= L.oos_previous_st;
			self.title 									:= L.title;
			self.fname 									:= L.fname;
			self.mname 									:= L.mname;
			self.lname 									:= L.lname;
			self.name_suffix 						:= L.name_suffix;
			self.cleaning_score					:= L.cleaning_score;
			self.addr_fix_flag 					:= L.addr_fix_flag;
			...
			self.status									:= if( L.history='','A','I' ); // Drivers.BWR_Make_FLDL_DID line 56
			self.issuance								:= L.issuance;
			self.address_change					:= L.address_change;
			self.name_change 						:= L.name_change;
			self.dob_change							:= L.dob_change;
			self.sex_change							:= L.sex_change;
			self.old_dl_number 					:= L.old_dl_number;
			self.driver_edu_code 				:= L.driver_edu_code;
			self.dup_lic_count 					:= L.dup_lic_count;
			self.rcd_stat_flag 					:= L.rcd_stat_flag;
			self.hair_color							:= L.hair_color;
			self.eye_color 							:= L.eye_color;
			self.weight									:= L.weight;
			self.dl_key_number 					:= L.dl_key_number;
			
			self.history_name						:= L.history_name;
			
			self := []; // STUB
		end;
		
	  // ...using seq as the lookup mechanism
		export dataset(l_moxie) by_seq(
			dataset(l_seq) in_seqs
		) := function
		  recs			:= DriversV2_Services.fn_getDL_report(in_seqs);
		  recs_tmp	:= project(recs(penalt <= moxie_pThresh), toMoxie(left));
			
		  recs_d := dedup(
				recs_tmp,
				except status, oos_previous_st, oos_previous_dl_number, // undisplayed
				history, // history:history_name is many:1 so we dedup by history_name
				all
			);
			
		  results		:= project(recs_d, l_moxie);
		  return results;
		end;
		
	  // ...using DID as the lookup mechanism
	  export dataset(l_moxie) by_did(
			dataset(l_did) in_dids
		) := function
		  return by_seq(get_seq_from_dids(in_dids));
		end;
		...
		// ...using dl_number as the lookup mechanism
		export dataset(l_moxie) by_num(
			dataset(l_num) in_nums
		) := function
		  return by_seq(get_seq_from_num(in_nums));
		end;
		
		export dataset(l_moxie) state_filt(
			dataset(l_moxie)	ds_in,
			string99					state_in
		) := function
			return ds_in(
				state_in=''
				or state_in in [ state_origin, state, st ]
				or (state_origin='' and state='' and st='')
			);
		end;

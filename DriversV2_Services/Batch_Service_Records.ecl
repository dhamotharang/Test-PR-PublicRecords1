IMPORT Address, Autokey_batch, AutokeyB2, AutoStandardI, BatchServices, Census_Data, doxie, DriversV2, DriversV2_Services;
...
EXPORT Batch_Service_Records (DATASET(Autokey_batch.Layouts.rec_inBatchMaster) data_in, 
	...
	currentDt := (UNSIGNED4) StringLib.getDateYYYYMMDD() : GLOBAL;
 ...
 // OUTPUT(dl_recs_with_ids,NAMED('dl_recs_with_ids'));
 // OUTPUT(filterbypenalty,NAMED('filterbypenalty'));
 // OUTPUT(current_rolled_up_TX,NAMED('current_rolled_up_TX'));
 // OUTPUT(current_nonTX,NAMED('current_nonTX'));
 // OUTPUT(matches_pnlt,NAMED('matches_pnlt'));
 // OUTPUT(rsltByDLNum,NAMED('rsltByDLNum'));
 // OUTPUT(rsltByDID,NAMED('rsltByDID'));

...
EXPORT wildcard := MODULE;
	...
	EXPORT get_seq_from_num(params input) := FUNCTION
		...
		UNSIGNED1 state := IF(input.DLState='',-1,ut.St2Code(stringlib.stringtouppercase(input.DLState)));
		hasWild := LENGTH(StringLib.StringFilter(input.dl_num,'*?'))>0;

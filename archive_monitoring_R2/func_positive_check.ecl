import doxie, header, didville, ut, monitoring;

export func_positive_check(dataset(monitoring.layout_header_ext) hdr_data,
                           dataset(monitoring.Layout_Best_Addr_In) in_data,
					  unsigned1 scr_thr_val = 10) := function
				
monitoring.layout_header_ext refine_hdr(hdr_data l) := transform
	self := l;
end;				

	
	
w_in_ssn := if(count(hdr_data)>0,
                 join(hdr_data, in_data((unsigned)ssn<>0),
                 left.rid = (unsigned6)right.acctno and 
			  (header.ssn_value(left.ssn, right.ssn)>0 or 
			  (unsigned)left.ssn = 0), refine_hdr(left), local));				
...
w_penalt := if(count(hdr_data)>0,
               join(hdr_data, in_data((unsigned)ssn=0),
                    left.rid = (unsigned6)right.acctno,
			     get_penalt(left, right), local));			   

w_penalt_srt := sort(w_penalt, rid, if(ssn<>'',1,2), penalt, map(valid_ssn='G'=>1,valid_ssn=''=>2,3),
                               if(dob<>0,1,2), -dt_last_seen, doxie.tnt_score(tnt), -ssn, local);

w_penalt_dep := dedup(w_penalt_srt, rid, local);

/*
//use the found SSN + NAME to re-fetch more records				    
ssn_to_rt := project(w_penalt_dep, 
                     transform({monitoring.Layout_Best_Addr_In},
				           self.acctno := (qstring20)left.rid, 
				           self.name_first := left.fname,
						 self.name_middle := left.mname,
				           self.name_last := left.lname,
				           self.ssn := left.ssn,
						 self.best_address_number := left.best_address_number,
				           self := []));				    

did_to_rt_raw := distribute(monitoring.Dids_Batch_Call(ssn_to_rt), hash((unsigned6)acctno));

did_to_rt := join(did_to_rt_raw, hdr_data,
                  (unsigned6)left.acctno = right.rid and  
                  left.did = right.did, 
		        transform({did_to_rt_raw}, self := left), local, left only);

monitoring.layout_header_ext get_hdr_rt(did_to_rt l, doxie.key_header r) := transform

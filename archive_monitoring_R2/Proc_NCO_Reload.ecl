...
// Makes a reload of a particualr source from the scratch. Can be done from multiple
// input files. Code should be almost the same as in Proc_NCO_UpdateBase,
// main differencies are due to the possible huge size of an input file(s).
// Some possible efficiency shortcuts are commented with "// !!!"
// (based on some simplifying assumptions, like all input records are unique, 
// short-term history can be just appended, etc).

// Main Differencies from regular processing:
// * Location of an input file(s);
// * If RELOAD is done from NCO files (vs BANKO files), then
//   address/phone Normalize transforms can be called with smaller COUNT parameter;
//   (also: IsBatchOutput always = false (not an overhead, indeed), addr_dt_last_seen always = '', etc.);
// * Files are always distributed (LOOKUP_THRESHOLD is redundant, not used)
// * STH files are just appended to current ones;
// --------------------------------------------------------------------

#workunit('name','NCO Recon')
import address, standard;
boolean RECON_NCO := TRUE;
...
EXPORT Proc_NCO_Reload (string vendorName, string fileName, boolean do_spray = true) := FUNCTION
  ...
  //                    SPRAY BATCH 
  // ============================================================================
  fname_src := Environment.LandingZone.reconPath + nco_id + '/' + fileName;
  ...
  ds_batch_in := DATASET (file_in_name, layouts_NCO.batch, CSV (separator('|'), maxlength (16384)));
  ...
  act_UpdateArchieve := SEQUENTIAL (
    FileUtils.CreateAllSuperFiles (fname_archive_version, true),
  ...
  // ============================================================================
  //         VERIFY BATCH, UPDATE CLIENT "RAW MONITOR" (IN ORIGINAL FORMAT)
  // ============================================================================
  ...
  // delete records marked as "delete" or "update" (choose distribution or lookup)
  ...
  act_UpdateRawMonitor := SEQUENTIAL (
    FileUtils.CreateAllSuperFiles (fname_raw_version, true),
  ...
  ds_batch_new_normed := NORMALIZE (DS_BATCH_VALID, IF (RECON_NCO, 3, 9), Client_NCO.NormalizeToBase (Left, COUNTER)) 
  ...
  ds_subm_distr := JOIN (DISTRIBUTE (ds_submonitor, HASH(customer_id, record_id)), DS_BATCH_EXEC,
                         ...
                         (Left.record_id = Right.record_id),
                         transform (layouts.monitor, Self := Left),
                         LEFT ONLY, LOCAL);  //TODO: check if lookup can be used

  ds_submonitor_clean := IF (DO_DISTRIBUTE, ds_subm_distr, ds_subm_lookup);
  // !!!shortcut: ds_submonitor_clean := IF (RECON_NCO, ds_submonitor, ds_subm_distr);

  // append new- and update- records:
  ds_submonitor_new := DEDUP (PROJECT (ds_batch_new_normed (executed AND ~IsBatchOutput), layouts.monitor), record, all);
  // !!!shortcut: ds_submonitor_new := PROJECT (ds_batch_new_normed (executed AND ~IsBatchOutput), layouts.monitor);
  ...
  act_UpdateMonitor := SEQUENTIAL (
    FileUtils.CreateAllSuperFiles (fname_submonitor_version, true), // create if not exist
  ...
  ds_addr_hist_new := 
          DEDUP (SORT (ds_addr_pr,
            ...
            customer_id, record_id, prim_range, predir, prim_name, addr_suffix, postdir, unit_desig, sec_range,
            p_city_name, v_city_name, st, zip5, zip4, name_first, name_middle, name_last, name_suffix);
  // !!!shortcut: ds_addr_hist_new := ds_addr_pr;

  ds_addr_hist := ds_addr_hist_current + ds_addr_hist_new;
  ...
  // update all superfiles: assign current to QA, move older ones down the chain
  act_updateHistorySuperfiles := FileUtils.MoveSuperFiles (fname_addr_st_version, fname_addr_st, true);
  ...
  act_UpdateAddrHistory := SEQUENTIAL (
    FileUtils.CreateAllSuperFiles (fname_addr_st_version, true), // create if not exist
    act_updateHistorySubfile,
    act_updateHistorySuperfiles,
    update_history_addr
  );
  ...
  ds_phones_normed := NORMALIZE (DS_BATCH_VALID, IF (RECON_NCO, 12, 22), Client_NCO.NormalizeToPhoneHist (Left, COUNTER)); 
  ...
  // !!!shortcut: ds_phone_hist_new := ds_phones_normed;
  
  ds_phone_hist := ds_phone_hist_current + ds_phone_hist_new;
  act_updatePhoneSubfile := OUTPUT (ds_phone_hist, , fname_phone_st);

  // update all superfiles: assign current to QA, move older ones down the chain
  act_updatePhoneSSuperfiles := FileUtils.MoveSuperFiles (fname_phone_st_version, fname_phone_st, true);
  ...
  act_UpdatePhoneHistory := SEQUENTIAL (
    FileUtils.CreateAllSuperFiles (fname_phone_st_version, true), // create if not exist
    act_updatePhoneSubfile,
    act_updatePhoneSSuperfiles,
    update_history_phone
  );
  ...
  act_UpdateReport := SEQUENTIAL (
    FileUtils.CreateAllSuperFiles (fname_report_version, true),
...
//    act_despray_report
 ...
 );// : FAILURE (FileServices.sendemail ('vmyullyari@seisint.com,dqi@seisint.com,vikram.dhawan@lexisnexis.com,mluber@seisint.com', 'monitor update failure: ' + WUID, failmessage));
  return action;
END;
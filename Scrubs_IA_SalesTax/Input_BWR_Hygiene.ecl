...
#workunit('name','Scrubs_IA_SalesTax.Input_BWR_Hygiene - Hygiene & Stats - SALT V3.11.11');
IMPORT Scrubs_IA_SalesTax,SALT311;
// First create an instantiated hygiene module
  infile := Scrubs_IA_SalesTax.Input_In_IA_SalesTax;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_IA_SalesTax.Input_hygiene(ip);
  ...
  OUTPUT(SALT311.MAC_Character_Counts.EclRecord(p,'Input_Layout_IA_SalesTax'),NAMED('OptimizedLayout'));// File layout suggested by data

...
export fn_address_hier_byKey (dataset(layout_addr.addrsegrec) insHeader) := MODULE

  //project into slim layout  
	shared slimHdr     := project(insHeader,transform(Layout_Addr.slimrec,self:=left,self:=[]));	
  // output(slimHdr,named('slimHdr'),extend);
	
	// Collapse like addresses (for zip) by setting addr_ind to same value prior to process  
	shared iHdr  := iterate(sort(slimHdr(addr_ind NOT IN ['','0']),did,address_group_id,segment,local),
  ...
  shared outliers    := fn_address_outlier(sHdr(not (dt_last_seen < Constants.OutlierMinDt and zip4<>''))).outliers;
	...
	// output(ccconflicts,named('ccconflicts'),extend);
	shared CCValidated   := fn_address_cc(Goodrecs).GoodInsRecs;
	shared PropValidated := fn_address_prop_byKey(sHdr).GoodPropRecs;
	shared PropConflicts := fn_address_prop_byKey(sHdr).BadPropRecs;
	Export ADVOrecs      := fn_address_ADVO_byKey(sHdr).BadADVOrecs;
	Export Seasonrecs    := fn_address_ADVO_byKey(sHdr).Seasonrecs;
  ...
  shared statrecs    := rollup(sort(outliers + CCConflicts + CCValidated,did,rid,addressstatus,local),
	                                    left.did = right.did and
                                                ...
                                                self := left),
												 local);
	// output(statrecs,named('staterecs'),extend);

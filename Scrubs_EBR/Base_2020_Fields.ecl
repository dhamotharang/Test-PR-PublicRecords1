...
EXPORT Base_2020_Fields := MODULE
 
EXPORT NumFields := 25;
 
// Processing for each FieldType
EXPORT SALT311.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_mandatory','invalid_numeric','invalid_numeric_or_allzeros','invalid_pastdate','invalid_dt_first_seen','invalid_record_type','invalid_month','invalid_segment','invalid_mask','invalid_file_number');
EXPORT FieldTypeNum(SALT311.StrType fn) := CASE(fn,'invalid_mandatory' => 1,'invalid_numeric' => 2,'invalid_numeric_or_allzeros' => 3,'invalid_pastdate' => 4,'invalid_dt_first_seen' => 5,'invalid_record_type' => 6,'invalid_month' => 7,'invalid_segment' => 8,'invalid_mask' => 9,'invalid_file_number' => 10,0);
...
EXPORT MakeFT_invalid_segment(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_segment(SALT311.StrType s) := WHICH(((SALT311.StrType) s) NOT IN ['2020','2020']);
EXPORT InValidMessageFT_invalid_segment(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInEnum('2020|2020'),SALT311.HygieneErrors.Good);
...
EXPORT Make_trend_mm(SALT311.StrType s0) := MakeFT_invalid_month(s0);
EXPORT InValid_trend_mm(SALT311.StrType s) := InValidFT_invalid_month(s);
EXPORT InValidMessage_trend_mm(UNSIGNED1 wh) := InValidMessageFT_invalid_month(wh);
 
EXPORT Make_trend_yy(SALT311.StrType s0) := MakeFT_invalid_numeric_or_allzeros(s0);
EXPORT InValid_trend_yy(SALT311.StrType s) := InValidFT_invalid_numeric_or_allzeros(s);
EXPORT InValidMessage_trend_yy(UNSIGNED1 wh) := InValidMessageFT_invalid_numeric_or_allzeros(wh);
 
EXPORT Make_dbt(SALT311.StrType s0) := MakeFT_invalid_numeric_or_allzeros(s0);
EXPORT InValid_dbt(SALT311.StrType s) := InValidFT_invalid_numeric_or_allzeros(s);
EXPORT InValidMessage_dbt(UNSIGNED1 wh) := InValidMessageFT_invalid_numeric_or_allzeros(wh);
...
EXPORT Make_acct_bal(SALT311.StrType s0) := MakeFT_invalid_numeric_or_allzeros(s0);
EXPORT InValid_acct_bal(SALT311.StrType s) := InValidFT_invalid_numeric_or_allzeros(s);
EXPORT InValidMessage_acct_bal(UNSIGNED1 wh) := InValidMessageFT_invalid_numeric_or_allzeros(wh);
 
EXPORT Make_current_balance_pct(SALT311.StrType s0) := MakeFT_invalid_numeric_or_allzeros(s0);
EXPORT InValid_current_balance_pct(SALT311.StrType s) := InValidFT_invalid_numeric_or_allzeros(s);
EXPORT InValidMessage_current_balance_pct(UNSIGNED1 wh) := InValidMessageFT_invalid_numeric_or_allzeros(wh);
...
#uniquename(dl)
  %dl% := RECORD
    ...
    BOOLEAN Diff_trend_mm;
    BOOLEAN Diff_trend_yy;
    BOOLEAN Diff_dbt;
    BOOLEAN Diff_acct_bal_mask;
    BOOLEAN Diff_acct_bal;
    BOOLEAN Diff_current_balance_pct;
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Diff_trend_mm := le.trend_mm <> ri.trend_mm;
    SELF.Diff_trend_yy := le.trend_yy <> ri.trend_yy;
    SELF.Diff_dbt := le.dbt <> ri.dbt;
    SELF.Diff_acct_bal_mask := le.acct_bal_mask <> ri.acct_bal_mask;
    SELF.Diff_acct_bal := le.acct_bal <> ri.acct_bal;
    SELF.Diff_current_balance_pct := le.current_balance_pct <> ri.current_balance_pct;
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_powid,1,0)+ IF( SELF.Diff_proxid,1,0)+ IF( SELF.Diff_seleid,1,0)+ IF( SELF.Diff_orgid,1,0)+ IF( SELF.Diff_ultid,1,0)+ IF( SELF.Diff_bdid,1,0)+ IF( SELF.Diff_date_first_seen,1,0)+ IF( SELF.Diff_date_last_seen,1,0)+ IF( SELF.Diff_process_date_first_seen,1,0)+ IF( SELF.Diff_process_date_last_seen,1,0)+ IF( SELF.Diff_record_type,1,0)+ IF( SELF.Diff_process_date,1,0)+ IF( SELF.Diff_file_number,1,0)+ IF( SELF.Diff_segment_code,1,0)+ IF( SELF.Diff_sequence_number,1,0)+ IF( SELF.Diff_trend_mm,1,0)+ IF( SELF.Diff_trend_yy,1,0)+ IF( SELF.Diff_dbt,1,0)+ IF( SELF.Diff_acct_bal_mask,1,0)+ IF( SELF.Diff_acct_bal,1,0)+ IF( SELF.Diff_current_balance_pct,1,0)+ IF( SELF.Diff_dbt_01_30_pct,1,0)+ IF( SELF.Diff_dbt_31_60_pct,1,0)+ IF( SELF.Diff_dbt_61_90_pct,1,0)+ IF( SELF.Diff_dbt_91_plus_pct,1,0);
  END;
...
#uniquename(AggRec)
  %AggRec% := RECORD
    ...
    Count_Diff_trend_mm := COUNT(GROUP,%Closest%.Diff_trend_mm);
    Count_Diff_trend_yy := COUNT(GROUP,%Closest%.Diff_trend_yy);
    Count_Diff_dbt := COUNT(GROUP,%Closest%.Diff_dbt);
    Count_Diff_acct_bal_mask := COUNT(GROUP,%Closest%.Diff_acct_bal_mask);
    Count_Diff_acct_bal := COUNT(GROUP,%Closest%.Diff_acct_bal);
    Count_Diff_current_balance_pct := COUNT(GROUP,%Closest%.Diff_current_balance_pct);

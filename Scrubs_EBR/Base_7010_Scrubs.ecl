...
EXPORT Base_7010_Scrubs := MODULE
...
// The module to handle the case where no scrubs exist
  ...
  EXPORT Expanded_Layout := RECORD(Base_7010_Layout_EBR)
    ...
    UNSIGNED1 data_print_line_Invalid;
  END;
  EXPORT  Bitmap_Layout := RECORD(Base_7010_Layout_EBR)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(Base_7010_Layout_EBR) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.powid_Invalid := Base_7010_Fields.InValid_powid((SALT311.StrType)le.powid);
    SELF.proxid_Invalid := Base_7010_Fields.InValid_proxid((SALT311.StrType)le.proxid);
    SELF.seleid_Invalid := Base_7010_Fields.InValid_seleid((SALT311.StrType)le.seleid);
    SELF.orgid_Invalid := Base_7010_Fields.InValid_orgid((SALT311.StrType)le.orgid);
    SELF.ultid_Invalid := Base_7010_Fields.InValid_ultid((SALT311.StrType)le.ultid);
    SELF.bdid_Invalid := Base_7010_Fields.InValid_bdid((SALT311.StrType)le.bdid);
    SELF.date_first_seen_Invalid := Base_7010_Fields.InValid_date_first_seen((SALT311.StrType)le.date_first_seen);
    SELF.date_last_seen_Invalid := Base_7010_Fields.InValid_date_last_seen((SALT311.StrType)le.date_last_seen);
    SELF.process_date_first_seen_Invalid := Base_7010_Fields.InValid_process_date_first_seen((SALT311.StrType)le.process_date_first_seen);
    SELF.process_date_last_seen_Invalid := Base_7010_Fields.InValid_process_date_last_seen((SALT311.StrType)le.process_date_last_seen);
    SELF.record_type_Invalid := Base_7010_Fields.InValid_record_type((SALT311.StrType)le.record_type);
    SELF.process_date_Invalid := Base_7010_Fields.InValid_process_date((SALT311.StrType)le.process_date);
    SELF.file_number_Invalid := Base_7010_Fields.InValid_file_number((SALT311.StrType)le.file_number);
    SELF.segment_code_Invalid := Base_7010_Fields.InValid_segment_code((SALT311.StrType)le.segment_code);
    SELF.sequence_number_Invalid := Base_7010_Fields.InValid_sequence_number((SALT311.StrType)le.sequence_number);
    SELF.data_print_line_Invalid := Base_7010_Fields.InValid_data_print_line((SALT311.StrType)le.data_print_line);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Base_7010_Layout_EBR);
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.powid_Invalid << 0 ) + ( le.proxid_Invalid << 1 ) + ( le.seleid_Invalid << 2 ) + ( le.orgid_Invalid << 3 ) + ( le.ultid_Invalid << 4 ) + ( le.bdid_Invalid << 5 ) + ( le.date_first_seen_Invalid << 6 ) + ( le.date_last_seen_Invalid << 7 ) + ( le.process_date_first_seen_Invalid << 8 ) + ( le.process_date_last_seen_Invalid << 9 ) + ( le.record_type_Invalid << 10 ) + ( le.process_date_Invalid << 11 ) + ( le.file_number_Invalid << 12 ) + ( le.segment_code_Invalid << 14 ) + ( le.sequence_number_Invalid << 15 ) + ( le.data_print_line_Invalid << 16 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,Base_7010_Layout_EBR);
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.data_print_line_Invalid := (le.ScrubsBits1 >> 16) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.powid_Invalid,le.proxid_Invalid,le.seleid_Invalid,le.orgid_Invalid,le.ultid_Invalid,le.bdid_Invalid,le.date_first_seen_Invalid,le.date_last_seen_Invalid,le.process_date_first_seen_Invalid,le.process_date_last_seen_Invalid,le.record_type_Invalid,le.process_date_Invalid,le.file_number_Invalid,le.segment_code_Invalid,le.sequence_number_Invalid,le.data_print_line_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Base_7010_Fields.InvalidMessage_powid(le.powid_Invalid),Base_7010_Fields.InvalidMessage_proxid(le.proxid_Invalid),Base_7010_Fields.InvalidMessage_seleid(le.seleid_Invalid),Base_7010_Fields.InvalidMessage_orgid(le.orgid_Invalid),Base_7010_Fields.InvalidMessage_ultid(le.ultid_Invalid),Base_7010_Fields.InvalidMessage_bdid(le.bdid_Invalid),Base_7010_Fields.InvalidMessage_date_first_seen(le.date_first_seen_Invalid),Base_7010_Fields.InvalidMessage_date_last_seen(le.date_last_seen_Invalid),Base_7010_Fields.InvalidMessage_process_date_first_seen(le.process_date_first_seen_Invalid),Base_7010_Fields.InvalidMessage_process_date_last_seen(le.process_date_last_seen_Invalid),Base_7010_Fields.InvalidMessage_record_type(le.record_type_Invalid),Base_7010_Fields.InvalidMessage_process_date(le.process_date_Invalid),Base_7010_Fields.InvalidMessage_file_number(le.file_number_Invalid),Base_7010_Fields.InvalidMessage_segment_code(le.segment_code_Invalid),Base_7010_Fields.InvalidMessage_sequence_number(le.sequence_number_Invalid),Base_7010_Fields.InvalidMessage_data_print_line(le.data_print_line_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.data_print_line_Invalid,'LENGTHS','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'powid','proxid','seleid','orgid','ultid','bdid','date_first_seen','date_last_seen','process_date_first_seen','process_date_last_seen','record_type','process_date','file_number','segment_code','sequence_number','data_print_line','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_mandatory','invalid_mandatory','invalid_mandatory','invalid_mandatory','invalid_mandatory','invalid_numeric','invalid_dt_first_seen','invalid_pastdate','invalid_pastdate','invalid_pastdate','invalid_record_type','invalid_pastdate','invalid_file_number','invalid_segment','invalid_numeric_or_allzeros','invalid_mandatory','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT311.StrType)le.powid,(SALT311.StrType)le.proxid,(SALT311.StrType)le.seleid,(SALT311.StrType)le.orgid,(SALT311.StrType)le.ultid,(SALT311.StrType)le.bdid,(SALT311.StrType)le.date_first_seen,(SALT311.StrType)le.date_last_seen,(SALT311.StrType)le.process_date_first_seen,(SALT311.StrType)le.process_date_last_seen,(SALT311.StrType)le.record_type,(SALT311.StrType)le.process_date,(SALT311.StrType)le.file_number,(SALT311.StrType)le.segment_code,(SALT311.StrType)le.sequence_number,(SALT311.StrType)le.data_print_line,'***SALTBUG***');
  END;
  EXPORT AllErrors := NORMALIZE(h,16,Into(LEFT,COUNTER));
   bv := TABLE(AllErrors,{FieldContents, FieldName, Cnt := COUNT(GROUP)},FieldContents, FieldName,MERGE);
  EXPORT BadValues := TOPN(bv,1000,-Cnt);
  // Particular form of stats required for Orbit
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(Base_7010_Layout_EBR) prevDS = DATASET([], Base_7010_Layout_EBR), STRING10 Src='UNK'):= FUNCTION
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'data_print_line:invalid_mandatory:LENGTHS'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ,Base_7010_Fields.InvalidMessage_powid(1)
          ,Base_7010_Fields.InvalidMessage_proxid(1)
          ,Base_7010_Fields.InvalidMessage_seleid(1)
          ,Base_7010_Fields.InvalidMessage_orgid(1)
          ,Base_7010_Fields.InvalidMessage_ultid(1)
          ,Base_7010_Fields.InvalidMessage_bdid(1)
          ,Base_7010_Fields.InvalidMessage_date_first_seen(1)
          ,Base_7010_Fields.InvalidMessage_date_last_seen(1)
          ,Base_7010_Fields.InvalidMessage_process_date_first_seen(1)
          ,Base_7010_Fields.InvalidMessage_process_date_last_seen(1)
          ,Base_7010_Fields.InvalidMessage_record_type(1)
          ,Base_7010_Fields.InvalidMessage_process_date(1)
          ,Base_7010_Fields.InvalidMessage_file_number(1),Base_7010_Fields.InvalidMessage_file_number(2)
          ,Base_7010_Fields.InvalidMessage_segment_code(1)
          ,Base_7010_Fields.InvalidMessage_sequence_number(1)
          ,Base_7010_Fields.InvalidMessage_data_print_line(1)
          ,'Fields with errors'
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.data_print_line_LENGTHS_ErrorCount
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ...
          ,le.segment_code_ENUM_ErrorCount
          ,le.sequence_number_CUSTOM_ErrorCount
          ,le.data_print_line_LENGTHS_ErrorCount,0) / le.TotalCnt, CHOOSE(c - NumRules
    ...
    mod_hygiene := Base_7010_hygiene(PROJECT(h, Base_7010_Layout_EBR));
    ...
    SALT311.ScrubsOrbitLayout xNormHygieneStats(hygiene_summaryStats le, UNSIGNED c, STRING suffix) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'data_print_line:' + getFieldTypeText(h.data_print_line) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix,'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.populated_data_print_line_cnt,0);
      SELF.rulepcnt := CHOOSE(c
          ...
          ,le.populated_data_print_line_pcnt,0);
      SELF.ErrorMessage := '';
    END;
  ...
  // record count stats
    ...
    mod_Delta := Base_7010_Delta(prevDS, PROJECT(h, Base_7010_Layout_EBR));
...
EXPORT StandardStats(DATASET(Base_7010_Layout_EBR) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  myTimeStamp := (UNSIGNED6)SALT311.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
  expandedFile := FromNone(inFile).ExpandedInfile;
  mod_fromexpandedOverall := FromExpanded(expandedFile);
  scrubsSummaryOverall := mod_fromexpandedOverall.SummaryStats;
 
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_EBR, Base_7010_Fields, 'RECORDOF(scrubsSummaryOverall)', '');

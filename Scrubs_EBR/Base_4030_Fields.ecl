...
EXPORT Base_4030_Fields := MODULE
...
EXPORT MakeFT_invalid_segment(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_segment(SALT311.StrType s) := WHICH(((SALT311.StrType) s) NOT IN ['4030','4030']);
EXPORT InValidMessageFT_invalid_segment(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInEnum('4030|4030'),SALT311.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_type_code(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_type_code(SALT311.StrType s) := WHICH(((SALT311.StrType) s) NOT IN ['05','05']);
EXPORT InValidMessageFT_invalid_type_code(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInEnum('05|05'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_type_desc(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_type_desc(SALT311.StrType s) := WHICH(((SALT311.StrType) s) NOT IN ['JUDG-','JUDG-']);
EXPORT InValidMessageFT_invalid_type_desc(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInEnum('JUDG-|JUDG-'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_action_code(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_action_code(SALT311.StrType s) := WHICH(((SALT311.StrType) s) NOT IN ['01','07','10','21']);
EXPORT InValidMessageFT_invalid_action_code(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInEnum('01|07|10|21'),SALT311.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_action_desc(SALT311.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_action_desc(SALT311.StrType s) := WHICH(((SALT311.StrType) s) NOT IN ['FILED','REL','SAT','VACAT']);
EXPORT InValidMessageFT_invalid_action_desc(UNSIGNED1 wh) := CHOOSE(wh,SALT311.HygieneErrors.NotInEnum('FILED|REL|SAT|VACAT'),SALT311.HygieneErrors.Good);
...
EXPORT FieldNum(SALT311.StrType fn) := CASE(fn,'powid' => 0,'proxid' => 1,'seleid' => 2,'orgid' => 3,'ultid' => 4,'bdid' => 5,'date_first_seen' => 6,'date_last_seen' => 7,'process_date_first_seen' => 8,'process_date_last_seen' => 9,'record_type' => 10,'process_date' => 11,'file_number' => 12,'segment_code' => 13,'sequence_number' => 14,'orig_date_filed_ymd' => 15,'type_code' => 16,'type_description' => 17,'action_code' => 18,'action_description' => 19,'document_number' => 20,'filing_location' => 21,'liability_amount' => 22,'plaintiff_name' => 23,'date_filed' => 24,0);
EXPORT SET OF SALT311.StrType FieldRules(UNSIGNED2 i) := CHOOSE(i,['LENGTHS'],['LENGTHS'],['LENGTHS'],['LENGTHS'],['LENGTHS'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['CUSTOM'],['ENUM'],['CUSTOM'],['ALLOW','LENGTHS'],['ENUM'],['CUSTOM'],['CUSTOM'],['ENUM'],['ENUM'],['ENUM'],['ENUM'],['LENGTHS'],['LENGTHS'],['CUSTOM'],['LENGTHS'],['CUSTOM'],[]);
...
EXPORT Make_plaintiff_name(SALT311.StrType s0) := MakeFT_invalid_mandatory(s0);
EXPORT InValid_plaintiff_name(SALT311.StrType s) := InValidFT_invalid_mandatory(s);
EXPORT InValidMessage_plaintiff_name(UNSIGNED1 wh) := InValidMessageFT_invalid_mandatory(wh);
...
#uniquename(AggRec)
  %AggRec% := RECORD
    ...
    Count_Diff_plaintiff_name := COUNT(GROUP,%Closest%.Diff_plaintiff_name);

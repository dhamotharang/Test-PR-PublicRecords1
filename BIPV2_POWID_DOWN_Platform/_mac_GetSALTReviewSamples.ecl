...
iter := '25';
#workunit('name','BIPV2_POWID_DOWN_Platform Iter ' + iter + ' review samples');
psetReviewers           := ['CM','LB','TL','JL','FN','DW'];
pNumSamplesPerReviewer  := 15;

kmtch      := index(BIPV2_POWID_DOWN_Platform.Keys(BIPV2_POWID_DOWN_Platform.In_POWID_Down).MatchSample ,BIPV2_POWID_DOWN_Platform.keynames('20130330' + iter).match_sample_debug.logical);

tools.mac_GetReviewSamples(kmtch,powid,'21',pNumSamplesPerReviewer,psetReviewers,true);
*/
import tools;

EXPORT _mac_GetSALTReviewSamples(

   pMatchSampleKey                    //Match sample debug index
  ,pMatchCandidatesKey                //Match Candidates key
  ,pInFile                            //Infile for matching(BIPV2_POWID_DOWN_Platform.In_POWID_Down)
...
////
  #APPEND(ECL    ,'allsamplerecs     := sort(samplerecseq + samplerecsgt,powid1 + powid2);\n')
...
////
  #APPEND(ECL    ,'allsamplepowids :=   table(table(samplerecseq + samplerecsgt  ,{unsigned6 ' + %'lID'% + ' := ' + %'lID'% + '1})\n')
  #APPEND(ECL    ,'                          + table(samplerecseq + samplerecsgt  ,{unsigned6 ' + %'lID'% + ' := ' + %'lID'% + '2})\n')
  #APPEND(ECL    ,'                      ,{' + %'lID'% + '},' + %'lID'% + ',few);\n')
  #APPEND(ECL    ,'setsamplepowids  := set(allsamplepowids  ,' + %'lID'% + ');\n')
  #APPEND(ECL    ,'allpowidcands    := project(kcand(' + %'lID'% + ' in setsamplepowids)  ,' + %'lModule'% + '.match_candidates(' + %'lInfile'% + ').layout_candidates);\n')
  #APPEND(ECL    ,'s := dataset([],' + %'lModule'% + '.Layout_Specificities.R);\n')
  #APPEND(ECL    ,'allpowidsrolled := ' + %'lModule'% + '.Debug(' + %'lInfile'% + ',s).RolledEntities(allpowidcands);\n')
  #APPEND(ECL    ,'layrolled := recordof(allpowidsrolled);\n\n')
  ...
  #LOOP
    ...
    #APPEND(NormSamples ,psetReviewers[%CNTR%] + '_norm := project(normalize(' + psetReviewers[%CNTR%] + ',3,transform({integer2 conf,unsigned6 powid},self.powid := choose(counter  ,left.powid1,left.powid2,0),self.conf := left.conf)) ,transform({unsigned cnt,recordof(left)},self.cnt := counter + ((startrec' + %'CNTR'% + ' - 1) * 3),self := left));\n')
    #APPEND(RollSamples ,psetReviewers[%CNTR%] + '_rolled := sort(join(' + psetReviewers[%CNTR%] + '_norm ,allpowidsrolled ,left.powid = right.powid ,transform({unsigned cnt,integer2 conf,recordof(right)},self.cnt := left.cnt,self.conf := if(right.powid != 0  ,left.conf,0),self := right),left outer,lookup),cnt);\n')

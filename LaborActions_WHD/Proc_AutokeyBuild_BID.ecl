...
EXPORT proc_autokeybuild_bid(STRING filedate) := FUNCTION
		...
		c := LaborActions_WHD.Constants(filedate, 'BID');
	...
	mod_AKB := module(AutokeyB2.Fn_Build.params) 
		...
		EXPORT STRING L_inkeyname := 	LaborActions_WHD.constants().ak_keynamebid; 
		EXPORT STRING L_inlogical := 	LaborActions_WHD.constants(filedate).ak_logicalbid; 

...
EXPORT batch_service(BOOLEAN useCannedRecs = false) := 
	FUNCTION
		...
		rec_results_plus_summary_data := RECORD
			layouts.layout_for_batch_records AND NOT
				[
					TotalViolations,
		...
		ds_xml_in   := DATASET([], LaborActions_WHD.layouts.layout_batch_in) : STORED('batch_in', FEW);
		ds_batch_in := IF( NOT useCannedRecs, ds_xml_in, LaborActions_WHD._canned_records );
		
		// 2. Get matching records, penalized.
		ds_matching_recs := LaborActions_WHD.batch_service_records(ds_batch_in);
		...
		rec_results_plus_summary_data doRollup_1( rec_results_plus_summary_data l, DATASET(rec_results_plus_summary_data) allRows) :=
			TRANSFORM
				...
				SELF.bid                  := IF( EXISTS(allRows(bid != 0)), allRows(bid != 0)[1].bid, 0 );
		...
		// 5. Regroup (since a Rollup breaks the previous grouping) and perform a Group Rollup, projecting
		// the raw records into the final layout.
		LaborActions_WHD.layouts.layout_batch_out doRollup_2( rec_results_plus_summary_data l, DATASET(rec_results_plus_summary_data) allRows) :=
			TRANSFORM
		...
		ds_recs_rolled_with_summary_totals := 
			ROLLUP(
				GROUP(SORT(ds_recs_rolled, acctno), acctno), // regroup, since the prior rollup broke the previous grouping
				GROUP,

...
get_companyname_penalty(LaborActions_WHD.layouts.layout_batch_in le, layouts.layout_for_batch_records ri) :=
	FUNCTION
		RETURN ut.mod_penalize.company_name(le.comp_name, ri.employername);
	END;
	
get_address_penalty(LaborActions_WHD.layouts.layout_batch_in le, layouts.layout_for_batch_records ri) :=
	FUNCTION
...
EXPORT batch_service_records( DATASET(LaborActions_WHD.layouts.layout_batch_in) ds_batch_in = DATASET([], LaborActions_WHD.layouts.layout_batch_in) ) :=
	FUNCTION
		...
		by_Autokey := 
			join(
				...
				TRANSFORM( layouts.layout_for_batch_records,

...
EXPORT fn_Data_Deletion_Individual_Incident_List (STRING logicalfilename) := FUNCTION

  //Incident File
	incident := FLAccidents_Ecrash.Infiles.Incident;
	d_incident := DISTRIBUTE(incident, HASH32(incident_id));
	
	//Update Incident_List 									
  incident_to_delete := d_incident(incident_id IN Incident_List);

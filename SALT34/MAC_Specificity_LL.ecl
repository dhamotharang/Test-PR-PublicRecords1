...
%temp_georec_pairs% := record
  ...
  import ut;
	...
	%ff% := record,maxlength(2048)
	  SALT34.Str20Type infield;
		dataset(SALT34.Layout_LL_Ranges) ranges;
		end;
	// finally, dedup to eliminate all but the last instance of each specificity
	#uniquename(o)
	%o% := project(dedup(%i%,lat1,long1,specificity,right,local),transform(%ff%,self.ranges := dataset([{left.dist,left.specificity}],SALT34.layout_ll_ranges),self.infield := left.infield));
	#uniquename(o2)

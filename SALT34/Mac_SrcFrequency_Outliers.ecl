...
EXPORT Mac_SrcFrequency_Outliers(infile,infield,srcfield,o1,o21,o22,o31,o32,o41,o42) := MACRO
  #uniquename(r)
  %r% := RECORD
	  SALT34.Str30Type fieldname := #TEXT(infield);
	  SALT34.StrType fieldvalue {MAXLENGTH(200000)} := (SALT34.StrType)infile.infield ;
		SALT34.StrType src {MAXLENGTH(30)} := infile.srcfield ;
...
EXPORT o22 := TABLE(%osu%, {SALT34.Str30Type fieldname := #TEXT(infield);SALT34.Str30Type OutlierType := 'Unique'; SALT34.StrType src := %osu%.src; REAL score := %osu%.score;});
	...
	EXPORT o32 := TABLE(%osd%,{SALT34.Str30Type fieldname := #TEXT(infield);SALT34.Str30Type OutlierType := 'Distinct'; SALT34.StrType src := %osd%.src; REAL score := %osd%.score;});
	...
	EXPORT o42 := TABLE(%ot5s%,{SALT34.Str30Type fieldname := #TEXT(infield);SALT34.Str30Type OutlierType := 'Top5'; SALT34.StrType src := %ot5s%.src; REAL score := %ot5s%.score;});
  ENDMACRO;
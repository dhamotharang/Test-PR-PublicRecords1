...
module
	...
	shared fLogicalVersion	(string pversion) := if(			regexfind('@version@', pTemplatefilename,nocase)
																								and not regexfind('@version@', pNewTemplatefilename,nocase)
																						,regexreplace('@version@', pTemplatefilename, pversion,nocase)
																						,regexreplace('@version@', pNewTemplatefilename, pversion,nocase)
																					);
	...
	shared lbuiltdelete				:= 'builtdelete'							;
	...
	export Base							:= fSuperVersion(lbase							);
	...
	export BuiltDelete			:= fSuperVersion(lbuiltdelete				);
	...
	export NewBuiltDelete				:= fLogicalVersion(lbuiltdelete						);
	...
	export BuiltDeleteSub				:= GetLogicalSubFile(lbuiltdelete				);
	...
	export BuiltDeleteVersion				:= GetFileVersion(lbuiltdelete			);
	...
	DATASET([
		...
		,(Delete)
		,(BuiltDelete)
		,(if(pNewTemplatefilename != '' and not regexfind('@version@',pNewTemplatefilename,nocase), pTemplatefilename,''))
...
//,dQAProdEtc			
		...
		,(Template), (TemplateNew), (pdate), (pnGenerations),regexfind('@version@', pTemplatefilename,nocase)}

/*2013-04-16T14:04:34Z (Vern Bentley)
fix for new platform.  old fix from a month ago doesn't work anymore
*/
/*2013-03-15T13:42:00Z (Vern Bentley)
add pversion to parameters so you can override any version in any file and stamp all files with this version
*/
...
function
	...
	transform
		self.SourceIP := mod_IPAddresses.fGetIPAddressFromServerName(l.SourceIP);
...
//																		fileservices.superfilecontents(l.name),dataset([],lib_fileservices.FsLogicalFileNameRecord));
	...
	transform
		lDate := map(	 pversion     != '' =>  pversion
                  ,l.FileDate		!= '' =>	l.Filedate
									,l.date_regex	!= '' =>	l.date_regex
									,												''
						);
		lFlag := map(	 l.FileDate		!= '' or pversion != '' => 'F'
									,l.date_regex	!= '' => 'R'
									,''
						);
		lDateAs1 := map(	
									 lFlag = 'F' and lDate    != '' 																								=> 	lDate
									,lFlag = 'F' and pversion != '' 																								=> 	pversion
	...
	outputwork := if(pSprayInfoLogicalfile != ''
                          ...
                          or pOverwrite
											...
											and not pIsTesting
											,sequential(
												 if(fileservices.FileExists(pSprayInfoLogicalfile) ,nothor(Tools.fun_ClearfilesFromSupers(dataset([{pSprayInfoLogicalfile}], Layout_Names), false)))
	...
	SprayFixedFiles(string pSourceIP
			...
			apply(pfilestospray, 
				sequential(
				if(FileExists,
					Tools.fun_ClearfilesFromSupers(dataset([{ThorLogicalFilename}], Layout_Names), false)),
...
//			,fileservices.finishSuperFileTransaction()
	...
	send_email := if(pEmailNotificationList != '' and pSplitEmails = false
										,fun_SendEmail(	 pEmailNotificationList,email_subject_header + email_subject,email_body[1].emailine)
										,sequential(
											 if(numFilesSprayed > 0,fun_SendEmail( pEmailNotificationList,email_subject_header + email_subject_sprayed,email_body[2].emailine))
											,if(numFilesSkipped > 0  or numFilesSprayed <= 0 , fun_SendEmail( pEmailNotificationList,email_subject_header + email_subject_skipped,email_body[3].emailine))
										)
								);
	
	fail_email := if(pEmailNotificationList != '', fun_SendEmail(

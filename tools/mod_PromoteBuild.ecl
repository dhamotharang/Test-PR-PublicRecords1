...
module
	...
	export odd2built						:= if(pIsTesting = false ,nothor(apply(lnames	,mod_Promote.fOdd2Built					(templatename,templatenameNew,pOddFilename,pversion,pDelete							))),output(lnames(filter),all));
	...
	export built2qa 						:= if(pIsTesting = false ,nothor(apply(lnames	,mod_Promote.fBuilt2QA					(templatename,pnGenerations,pDelete,,IsNewNamingConvention	))),output(lnames(filter),all));

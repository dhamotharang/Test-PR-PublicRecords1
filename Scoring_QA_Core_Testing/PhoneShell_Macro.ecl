EXPORT PhoneShell_Macro(roxie_ip, model, Gateway_dummy, Thread, Timeout,Retry,Input_file_name,Output_file_name,records_ToRun) := FUNCTIONMACRO   
...
BocaShell_Watchlist_Threshold := '0.8399999737739563';
GLBPurpose := 1;
DPPAPurpose := 3;
...
layoutSOAPIn := RECORD
	...
	UNSIGNED BocaShell_Version := 41;                       //SET the BS Version!!!!
...
SELF.EnableInsuranceGateway := EnableInsuranceAttributes;
 ...
 SELF.IncludeLastResort := IncludeLastResort;
	SELF.BocaShell_Version := 41; // version pulled from above
...
soapResults := SOAPCALL(
													soap, 
													RoxieIP, 
													// 'Phone_Shell.Phone_Shell_service.6',    //to select .6 query on Dev156

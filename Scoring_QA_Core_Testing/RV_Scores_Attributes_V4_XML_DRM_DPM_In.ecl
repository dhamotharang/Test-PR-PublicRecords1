EXPORT RV_Scores_Attributes_V4_XML_DRM_DPM_In (fcraroxie_IP, neutralroxie_IP, Thread, Timeout, Retry, Input_file_name,  Output_file_name, records_ToRun,In_DRM,in_DPM ):= FUNCTIONMACRO
...
// output(soap_in);
 ...
 output(ds_final,, outfile_name, thor, compressed, overwrite);	
 output(ds_final,, outfile_name + '_csv',CSV(heading(single), quote('"')), overwrite);   
			 // output(Soap_output,named('Soap_outputv4'));
		// output(soap_in,named('soap_inv4'));
return 0;
endmacro;
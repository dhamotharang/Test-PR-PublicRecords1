EXPORT Modeling_RV_Scores_Attributes_V3_XML_Macro(fcraroxie_IP, neutralroxie_IP, Thread, Timeout, Retry, Input_file_name, Output_file_name1_score, Output_file_name2_attr, records_ToRun):= FUNCTIONMACRO
...
get_group(recordof(layout_rvattributesout) groups, string name_i) := FUNCTION
    ...
    END;
		...
		//Appeding additional internal extras to Soap output file
		Global_output_lay2 v2_trans(Soap_output L, soap_in R) := transform
	...
	output(ds_with_extras,,outfile_name1+'CSV_COPY', CSV(heading(single), quote('"')), overwrite);
		//final file out to thor
	output(ds_with_extras2,, outfile_name2, thor, compressed, overwrite);	
	output(ds_with_extras2,, outfile_name2+'CSV_COPY', CSV(heading(single), quote('"')), overwrite);

...
outputfile := '~scoringqa::out::exphone_fraudpoint2_attributes_scores_after' + thorlib.wuid();
qa_outputfile := '~mludewig::out::FPA2_QA_' + thorlib.wuid();

eyeball := 10;

//f := choosen(dataset(filename,prii_layout,thor),eyeball);
f := dataset(filename,prii_layout, thor);

output(choosen(f,eyeball), named('original_input4'));
...
output(choosen(fdInput, eyeball), named('FD_Input4'));
...
output(choosen(resu, eyeball), named('roxie_result4'));
...
output(choosen(normed, eyeball), named('sample4'));
output(choosen(normed(errorcode<>''), eyeball), named('errors4'));
output(count(normed(errorcode<>'')), named('err_count4'));
...
output(choosen(rolled_to_1_record, eyeball), named('rolled_records4'));

output(rolled_to_1_record,,outputfile,CSV(heading(single), quote('"')), overwrite);

IF(Include_Internal_Extras, OUTPUT(CHOOSEN(qa_results_rolled, eyeball), NAMED('Sample_QA_Results4')));

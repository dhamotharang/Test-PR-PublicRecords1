EXPORT BocaShell_41_FCRA_cert_MACRO_DRM_DPM (  fcraroxie_IP,neutralroxie_IP, Thread, Timeout, Retry, Input_file_name,Output_file_name, records_ToRun, in_DRM, in_DPM):= functionmacro
...
output(ds_soap_in);
			...
			OUTPUT (ds_soap_output_pjt, , out_name_head , thor,compressed, overwrite);
			 OUTPUT (ds_soap_output_pjt, , out_name_head +'_csv', CSV(heading(single), quote('"')), overwrite);   

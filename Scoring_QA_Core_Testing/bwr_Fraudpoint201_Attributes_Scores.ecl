...
//#workunit('name','Fraudpoint Score and Attributes 201');
...
filename := '~scoring_project::in::ndobmeier::fp30dev::fp3_modelval_pii';
outputfile := '~scoringqa::out::exphone_fraudpoint201_attributes_scores_after' + thorlib.wuid();
qa_outputfile := '~mludewig::out::FPA201_QA_' + thorlib.wuid();
...
f := if(record_limit = 0, //depending on the record_limit, either process the entire input file or the number of records specified
				dataset(filename,prii_layout,thor),
				choosen(dataset(filename,prii_layout,thor), record_limit));
...
layout_old_acct into_fdInput(f le, INTEGER c) := TRANSFORM
  ...
  // self.historyDateTimeStamp := map(
	...
	self.DataPermissionMask  := '0000000000001'; //to allow access to Insurance DL data, set position 13 to '1'
																							 //FDN (position 11) applies only to Boca shell versions >= 51.  FP201 attributes
																							 //run BS 50 and all other FP2 scores/attributes run BS < 50 so FDN is N/A for FP2
																								
	SELF.RequestedAttributeGroups := dataset([{'fraudpointattrv201'}], layout_attributes_in);
self.model := 'fp1109_0';  // 
...
fd_attributes_norm_with_extras := RECORD
	string30 AccountNumber;
...
fd_attributes_norm_with_extras normit(resu L, fdInput R) := transform
	self.AccountNumber := r.old_account_number;
		
	// version2 output
	v := get_group(l, 'Version201');
...
self.SourceDriversLicense	:= v225.attribute[1].value;
	...
	self.SuspiciousActivityIndex    := L.scores[1].SuspiciousActivityIndex;
		...
		self.LexID := l.input.did; 
		// self.historydatetimestamp := r.historydatetimestamp;
...
normed_with_extras := JOIN(resu(accountnumber <> ''), fdInput,LEFT.accountnumber=RIGHT.accountnumber, normit(LEFT,RIGHT));
normed := PROJECT(normed_with_extras, TRANSFORM(fd_attributes_norm, SELF := LEFT));

qa_results := GROUP(SORT(PROJECT(normed_with_extras, TRANSFORM({string accountnumber, layout_internal_extras}, SELF := LEFT)),accountnumber), accountnumber);

qa_results_rolled := ROLLUP(qa_results, true, TRANSFORM(RECORDOF(LEFT), SELF := RIGHT));

output(choosen(normed, eyeball), named('sample1'));
output(choosen(normed(errorcode<>''), eyeball), named('errors3'));
output(count(normed(errorcode<>'')), named('err_count3'));
...
output(choosen(rolled_to_1_record, eyeball), named('rolled_records3'));


output(rolled_to_1_record,,outputfile,CSV(heading(single), quote('"')), overwrite);

IF(Include_Internal_Extras, OUTPUT(CHOOSEN(qa_results_rolled, eyeball), NAMED('Sample_QA_Results3')));
IF(Include_Internal_Extras, OUTPUT(qa_results_rolled,,qa_outputfile, CSV(heading(single), quote('"')), overwrite));
...
// #workunit('name','Fraudpoint Score and Attributes 3.0');
...
outputfile := '~scoringqa::out::exphone_fraudpoint3_attributes_scores_after' + thorlib.wuid();
qa_outputfile := '~mludewig::out::FPA3_QA_' + thorlib.wuid();
...
output(choosen(f,eyeball), named('original_input5'));
...
output(choosen(fdInput, eyeball), named('FD_Input5'));
...
output(choosen(resu, eyeball), named('roxie_result5'));
...
output(choosen(normed, eyeball), named('sample5'));
output(choosen(normed(errorcode<>''), eyeball), named('errors5'));
output(count(normed(errorcode<>'')), named('err_count5'));
...
output(choosen(rolled_to_1_record, eyeball), named('rolled_records5'));


output(rolled_to_1_record,,outputfile,CSV(heading(single), quote('"')), overwrite);

IF(Include_Internal_Extras, OUTPUT(CHOOSEN(qa_results_rolled, eyeball), NAMED('Sample_QA_Results5')));

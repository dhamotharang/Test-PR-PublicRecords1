/* updated: Ben Weiner - 09-15-2016 */
/* updated: Andi Koenen - 06-21-2016 */
/* updated mgl */
...
// bwr_phoneshell-metronet_OFF-equifax_OFF
#WORKUNIT('name', 'Phone_Shell_oct17_1p');
...
RoxieIP := riskwise.shortcuts.core_97_roxieIP; // Production Roxie
...
// InputFile := '~bweiner::in::pp_sources_test_input.csv';
InputFile := '~scoringqa::pp_sources_test_input-samp_188k.csv';
OutputFile := '~mludewig::out::phone_shell_NoExp_NoEquifax_pp_sources_188Ktest';
//OutputFileModelShell := '~akoenen::out::phone_shell_modelingLayout_'  + thorlib.wuid();
/* *****************************************
 * Phone Shell Input Options               *
 *******************************************/
DataRestrictionMask := '000000000000000000000001'; //If bit 24 is 1 that means NO Equifax data can be used
...
prii_layout := RECORD
     ...
     INTEGER HistoryDateYYYYMM;
		 UNSIGNED8 DID;
END;
ii := SORT(DATASET(InputFile, prii_layout, CSV(QUOTE('"'))), acctno);
// ii := choosen(SORT(DATASET(InputFile, prii_layout, CSV(QUOTE('"'))), acctno), RecordsToRun);
...
output(sortedSlim,,OutputFile+ ThorLib.wuid(),CSV(heading(single), quote('"')),overwrite);
...
EXPORT bwr_phoneshell_metronet_OFF_equifax_OFF := 'todo';
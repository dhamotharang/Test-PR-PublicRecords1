EXPORT BWR_Phone_Shell_Flex(
														...
														Targus_Gateway_URL,
														Experian_Gateway,
														Experian_Gateway_URL) := FUNCTIONMACRO   
 ...
 *******************************************/
string DataRestrictionMask := DRM ; //If bit 24 is 1 that means NO Equifax data can be used
//set Premium A flag here
boolean PremiumAFlag := Prem;

boolean EnableInsuranceAttributes := INS_ATTR; // Should probably always be TRUE - turns on the Insurance Verification Attributes
String ScoringModelName := Model;//'PHONESCORE_V2' = OLD phone model; // Set to BLANK to turn off the scoring model
integer Score_Threshold_In := Score_Thresh;

/* *****************************************
 * Gateway Information                     *
 *******************************************/
boolean EnableQSentV2_TransUnion_Gateway := TU_Gateway; // Set to TRUE to run the QSentV2 TransUnion Gateway (Source_List == 'TU')
String QSentV2_TransUnion_Gateway_URL := TU_Gateway_URL; // NOTE: THIS URL IS ONLY FOR TRANSUNION TEST SEEDS - WILL NOT RUN LIVE TRANSACTIONS
...
boolean EnableTargus_Gateway := Targus_Gateway; // Set to TRUE to run the Targus Gateway (Source_List == 'PDE')
String Targus_Gateway_URL := Targus_Gateway_URL;
// Targus_Gateway_URL := 'http://rw_score_dev:Password01@10.176.68.164:7726/WsGateway/?ver_=1.70';

boolean EnableMetronet_Experian_Gateway := Experian_Gateway; // Set to TRUE to run the Metronet Experian Gateway (Source_List == 'EXP')
String Metronet_Experian_Gateway_URL := Experian_Gateway_URL;
...
OUTPUT(goodResults,, outfile_name  + '.csv', CSV(HEADING(single), QUOTE('"')), OVERWRITE);
OUTPUT(clean_ds_baseline_full,, outfile_name  , thor, OVERWRITE);

EXPORT Modeling_RV_Scores_Attributes_V5_XML_Macro(fcraroxie_IP, neutralroxie_IP, Thread, Timeout, Retry, Input_file_name, Output_file_name, records_ToRun):= FUNCTIONMACRO
  ...
  string other4;
    ...
    //Mapping Version5 attributes 	
	  ...
	  //Appeding internal extras to Soap output file 
		...
		output(ds_with_extras ,,outfile_name+'CSV_COPY', CSV(heading(single), quote('"')), overwrite);

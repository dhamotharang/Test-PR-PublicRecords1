...
bs := File_BK_Search(debtor_lname = '', debtor_company <> '', court_code <> '', case_number <> '');
...
bs_companies_rollup := rollup(bs_companies_sort,
                       ...
                       RollupBS(left, right),
                         local) : persist('TEMP::BK_Company_Rollup');

export Bankrupt_As_Business_Header := project(bs_append, Translate_Bankruptcy_to_BHF(left)) : persist('TEMP::BK_As_BH');
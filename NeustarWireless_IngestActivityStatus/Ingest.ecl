...
, DATASET(Layout_Activity_Status) Delta = DATASET([],Layout_Activity_Status)
, DATASET(Layout_Activity_Status) dsBase = In_Activity_Status // Change IN_Activity_Status to change input to ingest process
, DATASET(RECORDOF(Prep_Activity_Status_Ingest))  infile = Prep_Activity_Status_Ingest
) := MODULE
  SHARED NullFile := DATASET([],Layout_Activity_Status); // Use to replace files you wish to remove
  ...
  DistIngest0 := DISTRIBUTE(FilesToIngest0, HASH32(phone,activity_status));
  SortIngest0 := SORT(DistIngest0,phone,activity_status, __Tpe, RCID, LOCAL);
  GroupIngest0 := GROUP(SortIngest0,phone,activity_status, LOCAL, ORDERED, STABLE);
  SHARED AllIngestRecs0 := UNGROUP(ROLLUP(GroupIngest0,TRUE,MergeData(LEFT,RIGHT)));
 
  // Existing Base: combine delta with base file
  DistBase0 := DISTRIBUTE(Base0+Delta0, HASH32(phone,activity_status));
  SortBase0 := SORT(DistBase0,phone,activity_status, __Tpe, RCID, LOCAL);
  GroupBase0 := GROUP(SortBase0,phone,activity_status, LOCAL, ORDERED, STABLE);
  SHARED AllBaseRecs0 := UNGROUP(ROLLUP(GroupBase0,TRUE,MergeData(LEFT,RIGHT)));
 
  // Everything: combine ingest and base recs
  Sort0 := SORT(AllBaseRecs0+AllIngestRecs0,phone,activity_status, __Tpe,RCID,LOCAL);
  Group0 := GROUP(Sort0,phone,activity_status,LOCAL, ORDERED, STABLE);
  ...
  SHARED AllRecs := ORe+NR1+NR(RCID<>0) : PERSIST('~temp::NeustarWireless_IngestActivityStatus::Ingest_Cache',EXPIRE(NeustarWireless_IngestActivityStatus.Config.PersistExpire));
  ...
  SHARED S0 := OUTPUT(UpdateStats, {{UpdateStats} AND NOT __Tpe}, ALL, NAMED('ActivityStatusUpdateStats'));
  ...
  EXPORT NewRecords_NoTag := PROJECT(NewRecords,Layout_Activity_Status);
  EXPORT OldRecords :=PROJECT( AllRecs(__Tpe=RecordType.Old), NoFlagsRec);
  EXPORT OldRecords_NoTag := PROJECT(OldRecords,Layout_Activity_Status);
  EXPORT UpdatedRecords := PROJECT(AllRecs(__Tpe=RecordType.Updated), NoFlagsRec);
  EXPORT UpdatedRecords_NoTag := PROJECT(UpdatedRecords,Layout_Activity_Status);
  EXPORT AllRecords := IF(incremental, NewRecords, PROJECT(AllRecs, NoFlagsRec));
  EXPORT AllRecords_NoTag := PROJECT(AllRecords,Layout_Activity_Status); // Records in 'pure' format
...
EXPORT ValidityStats := OUTPUT(d,NAMED('ActivityStatusValidityStatistics'));
  EXPORT DoStats := S0;

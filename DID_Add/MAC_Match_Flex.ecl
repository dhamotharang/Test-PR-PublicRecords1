...
#uniquename(roxin)
%roxin% := distribute(project(%pre_infile_id%, %roxprep%(left)),hash(random()));
...
#uniquename(preclean)
%preclean% := if(%force%='thor' or (count(infile) > 15000000 and %force% <> 'roxi' and thorlib.nodes() >= 400),
			     project(%all_recs_clean%,%strip_id%(LEFT)),
			     project(%roxplus%,%strip_id_roxie%(LEFT)));

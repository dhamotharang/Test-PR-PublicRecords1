...
export	MS_Worker_as_header(dataset(Layout_MS_Workers_Comp_In) pMS_Worker = dataset([],Layout_MS_Workers_Comp_In), boolean pForHeaderBuild=false)
 :=
  function
	dMSWorkerAsSource	:=	header.Files_SeqdSrc().WC;

	header.Layout_New_Records into(dMSWorkerAsSource L) := transform
		self.did := 0;
		self.rid := 0;
	...
	MS_Worker_In := project(dMSWorkerAsSource,into(left))(prim_name!='' and lname!='');

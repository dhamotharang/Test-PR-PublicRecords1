...
export CA_Sales_Tax_As_Business_Contact := project(f(owner_name <> '', firm_name <> '',
           (integer)score > 85 and datalib.CompanyClean(owner_name)[41..120] = '',
           (integer)(Business_Header.CleanName(name_first, name_middle, name_last, name_suffix)[142]) < 3),
               Translate_CST_To_BCF(left)) : PERSIST('TEMP::CA_Sales_Tax_Contacts_Rollup');
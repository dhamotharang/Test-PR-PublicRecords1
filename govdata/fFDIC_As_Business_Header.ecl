...
export fFDIC_As_Business_Header(dataset(Layouts_FDIC.Base_AID) pBasefile) :=
function

FDIC_In := pBasefile;

Business_Header.Layout_Business_Header_New Translate_FDIC_To_BHF(Layouts_FDIC.Base_AID l) := transform
self.source := MDR.sourceTools.src_FDIC;          // Source file type
self.source_group := '';
self.vl_id := l.FED_RSSD;

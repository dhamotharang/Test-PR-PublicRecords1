...
CA_ST_Owners := normalize(CA_ST_Seq((owner_name <> '' and firm_name = '') or (owner_name <> '' and firm_name <> '' and
                     not ((integer)score > 85 and datalib.CompanyClean(owner_name)[41..120] = ''))), 2, Translate_CASalesTax_To_BHF(left, counter, 2));
...
CA_ST_Clean_Companies_Rollup := ROLLUP(CA_ST_Clean_Companies_Sort,
                    ...
                    LOCAL) : PERSIST('TMTEMP::CA_SalesTax_Company_Rollup');

export CA_Sales_Tax_As_Business_Header := CA_ST_Clean_Companies_Rollup;
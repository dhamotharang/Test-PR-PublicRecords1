...
gp_base := File_Gov_Phones_Base; //(zip != '' AND prim_name != '');

Business_Header.Layout_Business_Header Proj(gp_base l) := TRANSFORM
...
// Rollup
Business_Header.Layout_Business_Header RollupGP(Business_Header.Layout_Business_Header L, Business_Header.Layout_Business_Header R) := TRANSFORM
SELF.dt_first_seen := 
...
gp_companies_rollup := ROLLUP(gp_companies_sort,
                    ...
                    RollupGP(LEFT, RIGHT),
                    LOCAL) : PERSIST('TEMP::Gov_Phones_Company_Rollup');


EXPORT Gov_Phones_As_Business_Header := gp_companies_rollup;
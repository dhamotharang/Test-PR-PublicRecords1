...
function
	...
	dCA_Sales_Tax_BDID			 := govdata.File_CA_Sales_Tax_BDID			(bdid != 0, start_date			 != '', (unsigned6)start_date			 	<= (unsigned6)Std.Date.Today() );
	dCA_Sales_Tax_LinkIDS 	 := govdata.File_CA_Sales_Tax_BDID			((dotid<>0 OR	empid<>0 OR	powid<>0 OR	proxid<>0 OR seleid<>0 OR	orgid<>0 OR	ultid<>0), start_date	!= '', (unsigned6)start_date <= (unsigned6)Std.Date.Today() );		
	dIA_SalesTax_Base				 := govdata.File_IA_SalesTax_Base				(bdid != 0, issue_date			 != '', (unsigned6)issue_date			 	<= (unsigned6)Std.Date.Today());
	dFDIC_BDID							 := govdata.File_FDIC_BDID							(bdid != 0, process_date		 != '', (unsigned6)process_date		 	<= (unsigned6)Std.Date.Today());
	dfl_fbn_base						 := govdata.file_fl_fbn_base						(bdid != 0, fic_fil_date		 != '', (unsigned6)fic_fil_date		 	<= (unsigned6)Std.Date.Today());
	dFL_NonProfit_Corp			 := govdata.File_FL_NonProfit_Corp			(bdid != 0, process_date		 != '', (unsigned6)process_date		 	<= (unsigned6)Std.Date.Today());
	dGov_Phones_Base				 := govdata.File_Gov_Phones_Base				(bdid != 0, record_date			 != '', (unsigned6)record_date			<= (unsigned6)Std.Date.Today());
	dIRS_NonProfit_Base			 := govdata.File_IRS_NonProfit_Base			(bdid != 0, Process_Date		 != '', (unsigned6)Process_Date		 	<= (unsigned6)Std.Date.Today());
	dMS_Workers_Comp_BDID		 := govdata.File_MS_Workers_Comp_BDID		(bdid != 0, date_last_seen	 != '', (unsigned6)date_last_seen	 	<= (unsigned6)Std.Date.Today());
	dOR_Workers_Comp_BDID		 := govdata.File_OR_Workers_Comp_BDID		(bdid != 0, date_last_seen	 != '', (unsigned6)date_last_seen	 	<= (unsigned6)Std.Date.Today());
	dSEC_Broker_Dealer_BDID	 := govdata.File_SEC_Broker_Dealer_BDID	(bdid != 0, dt_last_reported != '', (unsigned6)dt_last_reported <= (unsigned6)Std.Date.Today());

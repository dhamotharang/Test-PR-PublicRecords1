...
EXPORT Explosion_Table := MODULE

  //Explode the Industry codes for Subsection and Classification
	EXPORT SubSectClass(STRING3 subsectclass_code) := FUNCTION    
    RETURN CASE(subsectclass_code,
  ...
  //Explode the Industry codes for Activities
	EXPORT Activity(STRING3 activity_code) := FUNCTION
    RETURN CASE(activity_code,
  ...
  //Explode the Industry codes for National Taxonomy Exemptions
	EXPORT NatlTaxExempt(STRING3 national_taxonomy_exempt_code) := FUNCTION
    RETURN CASE(national_taxonomy_exempt_code,
  ...
  //Take all the codes for the Industry Description and explode them and then
  //return those exploded values concatenated together
	EXPORT GetIndustryDesc_IRS990(STRING2 subsection_code, STRING4 classification_code,
                                STRING9 activity_codes, STRING4 national_taxonomy_exempt) := FUNCTION
    
    IndustryDesc1 := TRIM(SubSectClass(subsection_code + classification_code),LEFT,RIGHT);
    
    IndustryDesc2a := TRIM(Activity(activity_codes[1..3]),LEFT,RIGHT);
    IndustryDesc2b := TRIM(Activity(activity_codes[4..6]),LEFT,RIGHT);
    IndustryDesc2c := TRIM(Activity(activity_codes[7..9]),LEFT,RIGHT);
   
    IndustryDesc3 := TRIM(NatlTaxExempt(national_taxonomy_exempt[1..3]),LEFT,RIGHT);
	    
    All_Industry := IndustryDesc1 + 
                    IF(IndustryDesc2a != '', ';' + IndustryDesc2a, '') +
                    IF(IndustryDesc2b != '', ';' + IndustryDesc2b, '') +
                    IF(IndustryDesc2c != '', ';' + IndustryDesc2c, '') +
                    IF(IndustryDesc3 != '', ';' + IndustryDesc3, '');
           
    RETURN REGEXREPLACE('^;+', All_Industry, '');
	END;

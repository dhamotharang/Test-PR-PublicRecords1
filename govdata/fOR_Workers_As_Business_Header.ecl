...
export fOR_Workers_As_Business_Header(dataset(Layout_OR_Workers_Comp_Base) pBaseFile) :=
function

f := pBaseFile;

Business_Header.Layout_Business_Header_New TranslateORWC(f l, unsigned1 cnt, unsigned1 addrtype) := transform
self.bdid := L.bdid;
self.vl_id := 'OR' + l.employer_number;
...
self.city := choose(addrtype, l.mailing_v_city_name, l.ppb_v_city_name);

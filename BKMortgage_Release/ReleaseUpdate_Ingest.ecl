IMPORT STD,SALT311;
EXPORT ReleaseUpdate_Ingest(BOOLEAN incremental=FALSE
, DATASET(ReleaseUpdate_Layout_BKMortgage) Delta = DATASET([],ReleaseUpdate_Layout_BKMortgage)
, DATASET(ReleaseUpdate_Layout_BKMortgage) dsBase = ReleaseUpdate_In_BKMortgage // Change IN_BKMortgage to change input to ingest process
, DATASET(RECORDOF(BKMortgage_Release.ReleaseUpdate_prep_ingest_file))  infile = BKMortgage_Release.ReleaseUpdate_prep_ingest_file
) := MODULE
  SHARED NullFile := DATASET([],ReleaseUpdate_Layout_BKMortgage); // Use to replace files you wish to remove
  ...
  SHARED WithRT := RECORD
    ReleaseUpdate_Layout_BKMortgage;
  ...
  SHARED AllRecs := ORe+NR1+NR(record_id<>0) : PERSIST('~temp::BKMortgage_Release::Update_Ingest_Cache',EXPIRE(BKMortgage_Release.Config.PersistExpire));
  ...
  EXPORT NewRecords_NoTag := PROJECT(NewRecords,ReleaseUpdate_Layout_BKMortgage);
  EXPORT OldRecords :=PROJECT( AllRecs(__Tpe=RecordType.Old), NoFlagsRec);
  EXPORT OldRecords_NoTag := PROJECT(OldRecords,ReleaseUpdate_Layout_BKMortgage);
  EXPORT UpdatedRecords := PROJECT(AllRecs(__Tpe=RecordType.Updated), NoFlagsRec);
  EXPORT UpdatedRecords_NoTag := PROJECT(UpdatedRecords,ReleaseUpdate_Layout_BKMortgage);
  EXPORT AllRecords := IF(incremental, NewRecords, PROJECT(AllRecs, NoFlagsRec));
  EXPORT AllRecords_NoTag := PROJECT(AllRecords,ReleaseUpdate_Layout_BKMortgage); // Records in 'pure' format

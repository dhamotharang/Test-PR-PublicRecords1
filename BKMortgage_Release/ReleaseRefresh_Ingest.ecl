IMPORT STD,SALT311;
EXPORT ReleaseRefresh_Ingest(BOOLEAN incremental=FALSE
, DATASET(ReleaseRefresh_Layout_BKMortgage) Delta = DATASET([],ReleaseRefresh_Layout_BKMortgage)
, DATASET(ReleaseRefresh_Layout_BKMortgage) dsBase = ReleaseRefresh_In_BKMortgage // Change IN_BKMortgage to change input to ingest process
, DATASET(RECORDOF(BKMortgage_Release.ReleaseRefresh_prep_ingest_file))  infile = BKMortgage_Release.ReleaseRefresh_prep_ingest_file
) := MODULE
  SHARED NullFile := DATASET([],ReleaseRefresh_Layout_BKMortgage); // Use to replace files you wish to remove
  ...
  SHARED WithRT := RECORD
    ReleaseRefresh_Layout_BKMortgage;
  ...
  DistIngest0 := DISTRIBUTE(FilesToIngest0, HASH32(rectype
             ,documenttype,fipscode,mainaddendum,releaserecdate,releaseeffecdate,mortgagepayoffdate,releasedoc,releasebk,releasepg,multiplepageimage
             ,bkfsimageid,origdotrecdate,origdotcontractdate,origdotdoc,origdotbk,origdotpg,origlenderben,origloanamnt,loannumber,currentlenderben
             ,mers,mersvalidation,mspsvrloan,currentlenderpool,borrowername,borrmailfulladdress,borrmailunit,borrmailcity,borrmailstate,borrmailzip
             ,borrmailzip4,apn,multiapncode,taxacctid,propertyfulladd,propertyunit,propertycity,propertystate,propertyzip,propertyzip4
             ,dataentrydate,dataentryopercode,vendorsourcecode,hids_recordingflag,hids_docnumber,transfercertificateoftitle,hi_condo_cpr_hpr,hi_situs_unit_number,hids_previous_docnumber,previoustransfercertificateoftitle
             ,pid,matchedororphan,deed_pid,sam_pid,assessorparcelnumber_matched,assessorpropertyfulladd,assessorpropertyunittype,assessorpropertyunit,assessorpropertycity,assessorpropertystate
             ,assessorpropertyzip,assessorpropertyzip4,assessorpropertyaddrsource));
  SortIngest0 := SORT(DistIngest0,rectype
             ...
             ,dataentrydate,dataentryopercode,vendorsourcecode,hids_recordingflag,hids_docnumber,transfercertificateoftitle,hi_condo_cpr_hpr,hi_situs_unit_number,hids_previous_docnumber,previoustransfercertificateoftitle
             ,pid,matchedororphan,deed_pid,sam_pid,assessorparcelnumber_matched,assessorpropertyfulladd,assessorpropertyunittype,assessorpropertyunit,assessorpropertycity,assessorpropertystate
             ,assessorpropertyzip,assessorpropertyzip4,assessorpropertyaddrsource, __Tpe, record_id, LOCAL);
  GroupIngest0 := GROUP(SortIngest0,rectype
             ...
             ,dataentrydate,dataentryopercode,vendorsourcecode,hids_recordingflag,hids_docnumber,transfercertificateoftitle,hi_condo_cpr_hpr,hi_situs_unit_number,hids_previous_docnumber,previoustransfercertificateoftitle
             ,pid,matchedororphan,deed_pid,sam_pid,assessorparcelnumber_matched,assessorpropertyfulladd,assessorpropertyunittype,assessorpropertyunit,assessorpropertycity,assessorpropertystate
             ,assessorpropertyzip,assessorpropertyzip4,assessorpropertyaddrsource, LOCAL, ORDERED, STABLE);
  ...
  SHARED AllRecs := ORe+NR1+NR(record_id<>0) : PERSIST('~temp::BKMortgage_Release::Refresh_Ingest_Cache',EXPIRE(BKMortgage_Release.Config.PersistExpire));
  ...
  SHARED S0 := OUTPUT(UpdateStats, {{UpdateStats} AND NOT __Tpe}, ALL, NAMED('RefreshUpdateStats'));
  ...
  EXPORT NewRecords_NoTag := PROJECT(NewRecords,ReleaseRefresh_Layout_BKMortgage);
  EXPORT OldRecords :=PROJECT( AllRecs(__Tpe=RecordType.Old), NoFlagsRec);
  EXPORT OldRecords_NoTag := PROJECT(OldRecords,ReleaseRefresh_Layout_BKMortgage);
  EXPORT UpdatedRecords := PROJECT(AllRecs(__Tpe=RecordType.Updated), NoFlagsRec);
  EXPORT UpdatedRecords_NoTag := PROJECT(UpdatedRecords,ReleaseRefresh_Layout_BKMortgage);
  EXPORT AllRecords := IF(incremental, NewRecords, PROJECT(AllRecs, NoFlagsRec));
  EXPORT AllRecords_NoTag := PROJECT(AllRecords,ReleaseRefresh_Layout_BKMortgage); // Records in 'pure' format

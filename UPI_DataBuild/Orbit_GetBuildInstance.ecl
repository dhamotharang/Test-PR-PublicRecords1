...
EXPORT Orbit_GetBuildInstance		 (	STRING BuildName
	...
	OrbitCallQA	:=	SOAPCALL(		UPI_DataBuild.Orbit_Tracking.TargetURL
														,	UPI_DataBuild.Orbit_Tracking.SOAPService(sService)
														,	rRequest
														,	rResponse
														,	XPATH(UPI_DataBuild.Orbit_Tracking.OrbitRR(sService)	)
														,	NAMESPACE(UPI_DataBuild.Orbit_Tracking.Namespace_B)
														,	LITERAL
														,	SOAPACTION(UPI_DataBuild.Orbit_Tracking.SoapPath(sService) )
														, RETRY(2)
														, LOG
													)	;

	OrbitCallPROD	:=	SOAPCALL(		UPI_DataBuild.Orbit_TrackingPROD.TargetURL
														,	UPI_DataBuild.Orbit_TrackingPROD.SOAPService(sService)
														,	rRequest
														,	rResponse
														,	XPATH(UPI_DataBuild.Orbit_TrackingPROD.OrbitRR(sService)	)
														,	NAMESPACE(UPI_DataBuild.Orbit_TrackingPROD.Namespace_B)
														,	LITERAL
														,	SOAPACTION(UPI_DataBuild.Orbit_TrackingPROD.SoapPath(sService) )
														, RETRY(2)
														, LOG
													)	;

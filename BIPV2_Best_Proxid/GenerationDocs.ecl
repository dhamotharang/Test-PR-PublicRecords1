...
//SIC
BESTTYPE:BestSicCommon:BASIS(Proxid):COMMONEST:MINIMUM(2):EXTEND
BESTTYPE:BestSicCurrent:BASIS(Proxid):RECENT(dt_last_seen):MINIMUM(2):EXTEND
//NAICS
BESTTYPE:BestNaicsCommon:BASIS(Proxid):COMMONEST:MINIMUM(2):EXTEND
BESTTYPE:BestNaicsCurrent:BASIS(Proxid):RECENT(dt_last_seen):MINIMUM(2):EXTEND
// DBA Name
BESTTYPE:BestDbaNameLegal:BASIS(Proxid):VOTED(fn_Best_Name_Legal_Votes):FUZZY:VALID(fn_valid_cname):EXTEND
BESTTYPE:BestDbaNameCommon:BASIS(Proxid):COMMONEST:FUZZY:MINIMUM(2):VALID(fn_valid_cname):EXTEND
BESTTYPE:BestDbaNameCurrent:BASIS(Proxid):RECENT(dt_last_seen):FUZZY:MINIMUM(2):VALID(fn_valid_cname):EXTEND
BESTTYPE:BestDbaNameVoted:BASIS(Proxid):VOTED(fn_Best_Name_Source_Votes,2):VALID(fn_valid_cname):EXTEND
BESTTYPE:BestDbaNameLength:BASIS(Proxid):LONGEST:FUZZY:VALID(fn_valid_cname):EXTEND
BESTTYPE:BestDbaNameStrong:BASIS(Proxid):UNIQUE:FUZZY:VALID(fn_valid_cname):EXTEND
BESTTYPE:BestDbaNameCurrent2:BASIS(Proxid):RECENT(dt_last_seen):VALID(fn_valid_cname):EXTEND
BESTTYPE:BestDbaNameVotedUnrestricted:BASIS(Proxid):VOTED(fn_Best_Source_Unrestricted_Votes,2):VALID(fn_valid_cname):EXTEND
...
FIELD:dba_name:LIKE(cname):BestDbaNameLegal:BestDbaNameCommon:BestDbaNameCurrent:BestDbaNameVoted:BestDbaNameLength:BestDbaNameStrong:BestDbaNameCurrent2:BestDbaNameVotedUnrestricted:FLAG:26,373
...
// 2.   Scenario 1: if I am a branch location of ArbyÃƒÂ¢??s, my SELE is the corporate office I report to.
// 3.   Scenario 2: if I am a Subsidiary of Reed Elsevier, I am my own SELE.
// 4    The SELEID above ProxidID and below Org and Ult.
 
Total available specificity:333
...
Prior to matching the following rules are applied in a cascade in an attempt to fix or complete field values.
 1. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cname.
 ...
 2. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cname.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 ...
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 3. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cname.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 ...
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 4. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cname.
 ...
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 5. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cname.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which has the most complete (non-null) value of all the field values.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 6. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cname.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which is unique or at least fuzzily-equal to every value gathered.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 7. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cname.
 The number of each field value are then counted.
 The best is then defined to be the one which has the most recent date recorded in:dt_last_seen.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 8. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cname.
 The number of each field value are then counted.
...
Prior to matching the following rules are applied in a cascade in an attempt to fix or complete field values.
 1. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cfein.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which is unique or at least fuzzily-equal to every value gathered.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 2. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cfein.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which is the most commonly occurring field value and which has at least 2 records with that field value.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 3. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cfein.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which has the most recent date recorded in:dt_last_seen.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 4. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cfein.
 The number of each field value are then counted.
 The best is then defined to be the one which recieved most votes based upon the attribute:fn_Best_Source_Unrestricted_Votes which takes count and source into account by a margin of at least 2.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 5. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cfein.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 ...
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 6. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cfein.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
...
and the degree of fuzziness required but should average 26 points with a failed match substracting 23 points.
(This subtraction estimate is based upon 10% of clusters with 2 or more records have 2 or more values for company_fein.)
 
company_phone Scoring: 
Prior to matching the following rules are applied in a cascade in an attempt to fix or complete field values.
 1. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cphone10.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which has the most recent date recorded in:dt_last_seen.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 2. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cphone7.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which has the most recent date recorded in:dt_last_seen.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 3. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cphone7.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 ...
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 4. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cphone7.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which has the most complete (non-null) value of all the field values.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 5. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cphone7.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which is unique or at least fuzzily-equal to every value gathered.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 6. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cphone10.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which recieved most votes based upon the attribute:fn_Best_Source_Unrestricted_Votes which takes count and source into account by a margin of at least 2.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 7. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_cphone10.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which is the most commonly occurring field value and which has at least 2 records with that field value.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
...
company_url Scoring: 
Prior to matching the following rules are applied in a cascade in an attempt to fix or complete field values.
 1. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_curl.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which is the most commonly occurring field value and which has at least 2 records with that field value.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 2. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_curl.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which has the most recent date recorded in:dt_last_seen.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 3. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_curl.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which has the most complete (non-null) value of all the field values.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 4. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_curl.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which is unique or at least fuzzily-equal to every value gathered.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 5. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_curl.
 The number of each field value are then counted.
 The best is then defined to be the one which recieved most votes based upon the attribute:fn_Best_Source_Unrestricted_Votes which takes count and source into account by a margin of at least 2.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
...
(This subtraction estimate is based upon 1% of clusters with 2 or more records have 2 or more values for company_url.)
 
duns_number Scoring: 
Prior to matching the following rules are applied in a cascade in an attempt to fix or complete field values.
 1. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_duns.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which is the most commonly occurring field value and which has at least 2 records with that field value.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 2. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_duns.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which has the most recent date recorded in:dt_last_seen and which has at least 2 records with that field value.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 3. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_duns.
 The number of each field value are then counted.
 The best is then defined to be the one which has the most recent date recorded in:dt_last_seen.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 4. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_duns.
 The number of each field value are then counted.
 The best is then defined to be the one which recieved most votes based upon the attribute:fn_Best_Source_Unrestricted_Votes which takes count and source into account by a margin of at least 2.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
...
Prior to matching the following rules are applied in a cascade in an attempt to fix or complete field values.
 1. Gather together all records that match on (Proxid,data_permits).
 The number of each field value are then counted.
 The best is then defined to be the one which is the most commonly occurring field value and which has at least 2 records with that field value.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 2. Gather together all records that match on (Proxid,data_permits).
 The number of each field value are then counted.
 The best is then defined to be the one which has the most recent date recorded in:dt_last_seen and which has at least 2 records with that field value.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
...
Prior to matching the following rules are applied in a cascade in an attempt to fix or complete field values.
 ...
 The best is then defined to be the one which recieved most votes based upon the attribute:fn_Best_Source_Unrestricted_Votes which takes count and source into account by a margin of at least 2.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
Two dba_name fields will be considered to match if:
  - they are identical
The exact number of points allocated to a match will depend upon the global scarcity of the dba_name
but should average 26 points with a failed match substracting 16 points.
(This subtraction estimate is based upon 37% of clusters with 2 or more records have 2 or more values for dba_name.)
...
This field is scaled to match with its parent address, on average that will mean multiplying by 31%
...
(This subtraction estimate is based upon 1% of clusters with 2 or more records have 2 or more values for postdir.)
...
but should average 5 points with a failed match substracting 4 points.
(This subtraction estimate is based upon 3% of clusters with 2 or more records have 2 or more values for unit_desig.)
...
(This subtraction estimate is based upon 3% of clusters with 2 or more records have 2 or more values for p_city_name.)
...
(This subtraction estimate is based upon 6% of clusters with 2 or more records have 2 or more values for zip4.)
...
but should average 5 points with a failed match substracting 5 points.
(This subtraction estimate is based upon 0% of clusters with 2 or more records have 2 or more values for fips_state.)
...
but should average 7 points with a failed match substracting 6 points.
(This subtraction estimate is based upon 4% of clusters with 2 or more records have 2 or more values for fips_county.)
 
address Scoring: 
Prior to matching the following rules are applied in a cascade in an attempt to fix or complete field values.
 1. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_caddress.
 The number of each field value are then counted.
 ...
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 2. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_caddress.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which has the most recent date recorded in:dt_last_seen and which has at least 2 records with that field value.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 3. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_caddress.
 The number of each field value are then counted.
 ...
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 4. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_caddress.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which is the most commonly occurring field value and which has at least 2 records with that field value.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 5. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_caddress.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which is unique or at least fuzzily-equal to every value gathered.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 6. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_caddress.
 The number of each field value are then counted.
 The best is then defined to be the one which has the most recent date recorded in:dt_last_seen.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.
 7. Gather together all records that match on (Proxid,source_for_votes,data_permits) removing any which are invalid according to fn_valid_caddress.
 The number of each field value are then counted, field values with a smaller count are used to re-enforce higher count fields that they fuzzy-match to.
 The best is then defined to be the one which recieved most votes based upon the attribute:fn_Best_Source_Unrestricted_Votes which takes count and source into account by a margin of at least 2.
 If the value generated is non-null and the value currently in the field is a shorter form of it then the value currently in the field will be replaced.

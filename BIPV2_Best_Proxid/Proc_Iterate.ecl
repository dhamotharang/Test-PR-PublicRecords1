...
SHARED BM := BIPV2_Best_Proxid.BasicMatch(InFile);
...
OS := OUTPUT(BIPV2_Best_Proxid.specificities(InFile).specificities,NAMED('Specificities'));
OPRP := OUTPUT(BIPV2_Best_Proxid.match_candidates(InFile).prepropogationstats,NAMED('PrePopStats'));
OPP := OUTPUT(BIPV2_Best_Proxid.match_candidates(InFile).postpropogationstats,NAMED('PostPopStats'));
...
MP := PARALLEL( OUTPUT(MM.MatchesPerformed,NAMED('MatchesPerformed')),OUTPUT(BIPV2_Best_Proxid.BasicMatch(InFile).basic_match_count,NAMED('BasicMatchesPerformed')));
...
SpcS := OUTPUT(BIPV2_Best_Proxid.specificities(InFile).SpcShift,NAMED('SPCShift'));
...
d := DATASET([{MM.PatchingError0,MM.DuplicateRids0,COUNT(BIPV2_Best_Proxid.match_candidates(InFile).Unlinkables)}],{INTEGER PatchingError0, INTEGER DuplicateRids0, UNSIGNED UnlinkableRecords0});
...
ChangeName := '~temp::Proxid::BIPV2_Best_Proxid::changes_it'+iter;
...
//EXPORT LoopN(UNSIGNED N,UNSIGNED mt=Config.MatchThreshold) := LOOP(InFile,N,BIPV2_Best_Proxid.matches(ROWS(LEFT),mt).patched_infile); // Loop N times
//EXPORT LoopThisN(DATASET(BIPV2_Best_Proxid.Layout_Base)d,UNSIGNED N,UNSIGNED mt=Config.MatchThreshold) := LOOP(d,N,BIPV2_Best_Proxid.matches(ROWS(LEFT),mt).patched_infile);
//EXPORT LoopThisK(DATASET(BIPV2_Best_Proxid.Layout_Base)d,UNSIGNED K,UNSIGNED NMax=100) := LOOP(d,COUNTER<=NMax AND COUNT(DEDUP(ROWS(LEFT),Proxid,ALL))>K,BIPV2_Best_Proxid.matches(ROWS(LEFT),0).patched_infile); // Loop until K clusters left

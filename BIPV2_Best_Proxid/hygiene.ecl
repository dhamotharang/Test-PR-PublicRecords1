...
EXPORT Summary(SALT30.Str30Type txt,BOOLEAN Glob=TRUE) := FUNCTION
  ...
  R1 := RECORD
    UNSIGNED LinkingPotential :=  + T.Populated_dt_first_seen_pcnt *   0.00 / 100 + T.Populated_dt_last_seen_pcnt *   0.00 / 100 + T.Populated_source_for_votes_pcnt *   0.00 / 100 + T.Populated_company_name_pcnt *  26.00 / 100 + T.Populated_company_fein_pcnt *  26.00 / 100 + T.Populated_company_phone_pcnt *  26.00 / 100 + T.Populated_company_url_pcnt *  27.00 / 100 + T.Populated_duns_number_pcnt *  27.00 / 100 + T.Populated_company_sic_code1_pcnt *  11.00 / 100 + T.Populated_company_naics_code1_pcnt *  11.00 / 100 + T.Populated_prim_range_pcnt *  13.00 / 100 + T.Populated_predir_pcnt *   5.00 / 100 + T.Populated_prim_name_pcnt *  15.00 / 100 + T.Populated_addr_suffix_pcnt *   4.00 / 100 + T.Populated_postdir_pcnt *   7.00 / 100 + T.Populated_unit_desig_pcnt *   5.00 / 100 + T.Populated_sec_range_pcnt *  12.00 / 100 + T.Populated_p_city_name_pcnt *  12.00 / 100 + T.Populated_v_city_name_pcnt *  11.00 / 100 + T.Populated_st_pcnt *   5.00 / 100 + T.Populated_zip_pcnt *  14.00 / 100 + T.Populated_zip4_pcnt *  13.00 / 100 + T.Populated_fips_state_pcnt *   5.00 / 100 + T.Populated_fips_county_pcnt *   7.00 / 100;
    T;
  END;
  RETURN TABLE(T,R1);
END;
EXPORT SourceCounts := Summary('SummaryBySource',FALSE);  R := RECORD
  ...
  R T(SourceCounts le, SourceCounts ri) := TRANSFORM
    ...
    SELF.LinkingPotential := 0 + le.Populated_dt_first_seen_pcnt*ri.Populated_dt_first_seen_pcnt *   0.00 / 10000 + le.Populated_dt_last_seen_pcnt*ri.Populated_dt_last_seen_pcnt *   0.00 / 10000 + le.Populated_source_for_votes_pcnt*ri.Populated_source_for_votes_pcnt *   0.00 / 10000 + le.Populated_company_name_pcnt*ri.Populated_company_name_pcnt *  26.00 / 10000 + le.Populated_company_fein_pcnt*ri.Populated_company_fein_pcnt *  26.00 / 10000 + le.Populated_company_phone_pcnt*ri.Populated_company_phone_pcnt *  26.00 / 10000 + le.Populated_company_url_pcnt*ri.Populated_company_url_pcnt *  27.00 / 10000 + le.Populated_duns_number_pcnt*ri.Populated_duns_number_pcnt *  27.00 / 10000 + le.Populated_company_sic_code1_pcnt*ri.Populated_company_sic_code1_pcnt *  11.00 / 10000 + le.Populated_company_naics_code1_pcnt*ri.Populated_company_naics_code1_pcnt *  11.00 / 10000 + le.Populated_prim_range_pcnt*ri.Populated_prim_range_pcnt *  13.00 / 10000 + le.Populated_predir_pcnt*ri.Populated_predir_pcnt *   5.00 / 10000 + le.Populated_prim_name_pcnt*ri.Populated_prim_name_pcnt *  15.00 / 10000 + le.Populated_addr_suffix_pcnt*ri.Populated_addr_suffix_pcnt *   4.00 / 10000 + le.Populated_postdir_pcnt*ri.Populated_postdir_pcnt *   7.00 / 10000 + le.Populated_unit_desig_pcnt*ri.Populated_unit_desig_pcnt *   5.00 / 10000 + le.Populated_sec_range_pcnt*ri.Populated_sec_range_pcnt *  12.00 / 10000 + le.Populated_p_city_name_pcnt*ri.Populated_p_city_name_pcnt *  12.00 / 10000 + le.Populated_v_city_name_pcnt*ri.Populated_v_city_name_pcnt *  11.00 / 10000 + le.Populated_st_pcnt*ri.Populated_st_pcnt *   5.00 / 10000 + le.Populated_zip_pcnt*ri.Populated_zip_pcnt *  14.00 / 10000 + le.Populated_zip4_pcnt*ri.Populated_zip4_pcnt *  13.00 / 10000 + le.Populated_fips_state_pcnt*ri.Populated_fips_state_pcnt *   5.00 / 10000 + le.Populated_fips_county_pcnt*ri.Populated_fips_county_pcnt *   7.00 / 10000;
  END;
...
invRec invert(summary0 le, INTEGER C) := TRANSFORM
  ...
  SELF.FieldName := CHOOSE(C,'dt_first_seen','dt_last_seen','source_for_votes','company_name','company_fein','company_phone','company_url','duns_number','company_sic_code1','company_naics_code1','prim_range','predir','prim_name','addr_suffix','postdir','unit_desig','sec_range','p_city_name','v_city_name','st','zip','zip4','fips_state','fips_county');
  SELF.populated_pcnt := CHOOSE(C,le.populated_dt_first_seen_pcnt,le.populated_dt_last_seen_pcnt,le.populated_source_for_votes_pcnt,le.populated_company_name_pcnt,le.populated_company_fein_pcnt,le.populated_company_phone_pcnt,le.populated_company_url_pcnt,le.populated_duns_number_pcnt,le.populated_company_sic_code1_pcnt,le.populated_company_naics_code1_pcnt,le.populated_prim_range_pcnt,le.populated_predir_pcnt,le.populated_prim_name_pcnt,le.populated_addr_suffix_pcnt,le.populated_postdir_pcnt,le.populated_unit_desig_pcnt,le.populated_sec_range_pcnt,le.populated_p_city_name_pcnt,le.populated_v_city_name_pcnt,le.populated_st_pcnt,le.populated_zip_pcnt,le.populated_zip4_pcnt,le.populated_fips_state_pcnt,le.populated_fips_county_pcnt);
...
SALT30.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  ...
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT30.StrType)le.dt_first_seen),TRIM((SALT30.StrType)le.dt_last_seen),TRIM((SALT30.StrType)le.source_for_votes),TRIM((SALT30.StrType)le.company_name),TRIM((SALT30.StrType)le.company_fein),TRIM((SALT30.StrType)le.company_phone),TRIM((SALT30.StrType)le.company_url),TRIM((SALT30.StrType)le.duns_number),TRIM((SALT30.StrType)le.company_sic_code1),TRIM((SALT30.StrType)le.company_naics_code1),TRIM((SALT30.StrType)le.prim_range),TRIM((SALT30.StrType)le.predir),TRIM((SALT30.StrType)le.prim_name),TRIM((SALT30.StrType)le.addr_suffix),TRIM((SALT30.StrType)le.postdir),TRIM((SALT30.StrType)le.unit_desig),TRIM((SALT30.StrType)le.sec_range),TRIM((SALT30.StrType)le.p_city_name),TRIM((SALT30.StrType)le.v_city_name),TRIM((SALT30.StrType)le.st),TRIM((SALT30.StrType)le.zip),TRIM((SALT30.StrType)le.zip4),TRIM((SALT30.StrType)le.fips_state),TRIM((SALT30.StrType)le.fips_county)));
  SELF.FldNo := C;
END;
...
SHARED FldIds := DATASET([{1,'dt_first_seen'}
      ...
      ,{22,'zip4'}
      ,{23,'fips_state'}
      ,{24,'fips_county'}],SALT30.MAC_Character_Counts.Field_Identification);
...
PrettyErrorTotals := RECORD
  ...
  FieldType := CHOOSE(TotalErrors.FieldNum,'Unknown','Unknown','Unknown','cname','number','number','Unknown','number','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','Unknown','alpha','number','number','number','number','Unknown');
...
EXPORT ClusterSrc := SALT30.MOD_ClusterStats.Sources(h,Proxid,source_for_votes);
END;
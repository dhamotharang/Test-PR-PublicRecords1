...
// There are 41 best types that we can propogate from this basis which means we will not need to use this basis again
  Layout_WithPropVars T_BestCompanyNameLegal_41_0(Layout_WithPropVars le,Best(ih).BestBy_Proxid_best ri) := TRANSFORM
    ...
    SELF := le;
  END;
SHARED P_BestCompanyNameLegal_41_0 := JOIN(with_props(Proxid <> 0),Best(ih).BestBy_Proxid_best, LEFT.Proxid = RIGHT.Proxid ,T_BestCompanyNameLegal_41_0(LEFT,RIGHT),LEFT OUTER,HASH)
                 + with_props(Proxid = 0);
...
layout_candidates add_predir(layout_candidates le,Specificities(ih).predir_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
j17 := JOIN(j18,PULL(Specificities(ih).predir_values_persisted),LEFT.predir=RIGHT.predir,add_predir(LEFT,RIGHT,TRUE),LOOKUP,FEW,LEFT OUTER);

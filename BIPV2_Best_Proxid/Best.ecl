...
EXPORT BestCompanyNameLegal_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_name,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_name,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestCompanyNameLegal_tab_(company_name NOT IN SET(s.nulls_company_name,company_name),fn_valid_cname(TRIM((SALT30.StrType)company_name),source_for_votes)),{Proxid,source_for_votes,data_permits,company_name,Row_Cnt});
...
// Voting is supported by altering the Row Count to be 100 * the (real) vote returned from the attribute
  Voted := TABLE( BestCompanyNameLegal_tab_company_name,{Proxid,data_permits,company_name,UNSIGNED Row_Cnt := 100 * fn_Best_Name_Legal_Votes(source_for_votes,Row_Cnt)}); // Use fn_Best_Name_Legal_Votes to vote
...
//Now actually find the best value
  grp := GROUP( BestCompanyNameLegal_vote_company_name,Proxid,ALL,LOCAL);
...
EXPORT BestCompanyNameCommon_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_name,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_name,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestCompanyNameCommon_tab_(company_name NOT IN SET(s.nulls_company_name,company_name),fn_valid_cname(TRIM((SALT30.StrType)company_name),source_for_votes)),{Proxid,source_for_votes,data_permits,company_name,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value enforcing minimum
  grp := GROUP( BestCompanyNameCommon_tab_company_name,Proxid,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
...
EXPORT BestCompanyNameCurrent_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_name,UNSIGNED Early_Date := MIN(GROUP,IF(dt_last_seen=0,99999999,dt_last_seen)),UNSIGNED Late_Date := MAX(GROUP,dt_last_seen),UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_name,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestCompanyNameCurrent_tab_(company_name NOT IN SET(s.nulls_company_name,company_name),fn_valid_cname(TRIM((SALT30.StrType)company_name),source_for_votes)),{Proxid,source_for_votes,data_permits,company_name,Early_Date,Late_Date,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
...
EXPORT BestCompanyNameVoted_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_name,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_name,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestCompanyNameVoted_tab_(company_name NOT IN SET(s.nulls_company_name,company_name),fn_valid_cname(TRIM((SALT30.StrType)company_name),source_for_votes)),{Proxid,source_for_votes,data_permits,company_name,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestCompanyNameLength_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_name,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_name,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestCompanyNameLength_tab_(company_name NOT IN SET(s.nulls_company_name,company_name),fn_valid_cname(TRIM((SALT30.StrType)company_name),source_for_votes)),{Proxid,source_for_votes,data_permits,company_name,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestCompanyNameStrong_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_name,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_name,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestCompanyNameStrong_tab_(company_name NOT IN SET(s.nulls_company_name,company_name),fn_valid_cname(TRIM((SALT30.StrType)company_name),source_for_votes)),{Proxid,source_for_votes,data_permits,company_name,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value
  grp := GROUP( BestCompanyNameStrong_tab_company_name,Proxid,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
...
EXPORT BestCompanyNameCurrent2_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_name,UNSIGNED Early_Date := MIN(GROUP,IF(dt_last_seen=0,99999999,dt_last_seen)),UNSIGNED Late_Date := MAX(GROUP,dt_last_seen),UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_name,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestCompanyNameCurrent2_tab_(company_name NOT IN SET(s.nulls_company_name,company_name),fn_valid_cname(TRIM((SALT30.StrType)company_name),source_for_votes)),{Proxid,source_for_votes,data_permits,company_name,Early_Date,Late_Date,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
EXPORT BestCompanyNameVotedUnrestricted_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_name,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_name,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestCompanyNameVotedUnrestricted_tab_(company_name NOT IN SET(s.nulls_company_name,company_name),fn_valid_cname(TRIM((SALT30.StrType)company_name),source_for_votes)),{Proxid,source_for_votes,data_permits,company_name,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestCompanyAddressCurrent_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,prim_range,predir,prim_name,addr_suffix,postdir,unit_desig,sec_range,st,zip,UNSIGNED Early_Date := MIN(GROUP,IF(dt_last_seen=0,99999999,dt_last_seen)),UNSIGNED Late_Date := MAX(GROUP,dt_last_seen),UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,prim_range,predir,prim_name,addr_suffix,postdir,unit_desig,sec_range,st,zip,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestCompanyAddressCurrent_tab_(~(prim_range  IN SET(s.nulls_prim_range,prim_range) AND predir  IN SET(s.nulls_predir,predir) AND prim_name  IN SET(s.nulls_prim_name,prim_name) AND addr_suffix  IN SET(s.nulls_addr_suffix,addr_suffix) AND postdir  IN SET(s.nulls_postdir,postdir) AND unit_desig  IN SET(s.nulls_unit_desig,unit_desig) AND sec_range  IN SET(s.nulls_sec_range,sec_range) AND st  IN SET(s.nulls_st,st) AND zip  IN SET(s.nulls_zip,zip)),fn_valid_caddress(TRIM((SALT30.StrType)prim_range) + '|' + TRIM((SALT30.StrType)predir) + '|' + TRIM((SALT30.StrType)prim_name) + '|' + TRIM((SALT30.StrType)addr_suffix) + '|' + TRIM((SALT30.StrType)postdir) + '|' + TRIM((SALT30.StrType)unit_desig) + '|' + TRIM((SALT30.StrType)sec_range) + '|' + TRIM((SALT30.StrType)st) + '|' + TRIM((SALT30.StrType)zip),source_for_votes)),{Proxid,source_for_votes,data_permits,prim_range,predir,prim_name,addr_suffix,postdir,unit_desig,sec_range,st,zip,Early_Date,Late_Date,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
EXPORT BestPhoneCurrentWithNpa_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_phone,UNSIGNED Early_Date := MIN(GROUP,IF(dt_last_seen=0,99999999,dt_last_seen)),UNSIGNED Late_Date := MAX(GROUP,dt_last_seen),UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_phone,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestPhoneCurrentWithNpa_tab_(company_phone NOT IN SET(s.nulls_company_phone,company_phone),fn_valid_cphone10(TRIM((SALT30.StrType)company_phone),source_for_votes)),{Proxid,source_for_votes,data_permits,company_phone,Early_Date,Late_Date,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
Fuzzy_layout NoteSupport(BestPhoneCurrentWithNpa_tab_company_phone le,BestPhoneCurrentWithNpa_tab_company_phone ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestPhoneCurrentWithNpa_tab_company_phone,BestPhoneCurrentWithNpa_tab_company_phone, LEFT.Proxid = RIGHT.Proxid  AND ( LEFT.company_phone = (TYPEOF(LEFT.company_phone))'' OR RIGHT.company_phone = (TYPEOF(RIGHT.company_phone))'' OR SALT30.WithinEditN(LEFT.company_phone,RIGHT.company_phone,1, 0)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
EXPORT BestPhoneCurrent_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_phone,UNSIGNED Early_Date := MIN(GROUP,IF(dt_last_seen=0,99999999,dt_last_seen)),UNSIGNED Late_Date := MAX(GROUP,dt_last_seen),UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_phone,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestPhoneCurrent_tab_(company_phone NOT IN SET(s.nulls_company_phone,company_phone),fn_valid_cphone7(TRIM((SALT30.StrType)company_phone),source_for_votes)),{Proxid,source_for_votes,data_permits,company_phone,Early_Date,Late_Date,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
Fuzzy_layout NoteSupport(BestPhoneCurrent_tab_company_phone le,BestPhoneCurrent_tab_company_phone ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestPhoneCurrent_tab_company_phone,BestPhoneCurrent_tab_company_phone, LEFT.Proxid = RIGHT.Proxid  AND ( LEFT.company_phone = (TYPEOF(LEFT.company_phone))'' OR RIGHT.company_phone = (TYPEOF(RIGHT.company_phone))'' OR SALT30.WithinEditN(LEFT.company_phone,RIGHT.company_phone,1, 0)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
EXPORT BestPhoneVoted_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_phone,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_phone,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestPhoneVoted_tab_(company_phone NOT IN SET(s.nulls_company_phone,company_phone),fn_valid_cphone7(TRIM((SALT30.StrType)company_phone),source_for_votes)),{Proxid,source_for_votes,data_permits,company_phone,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
Fuzzy_layout NoteSupport(BestPhoneVoted_vote_company_phone le,BestPhoneVoted_vote_company_phone ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestPhoneVoted_vote_company_phone,BestPhoneVoted_vote_company_phone, LEFT.Proxid = RIGHT.Proxid  AND ( LEFT.company_phone = (TYPEOF(LEFT.company_phone))'' OR RIGHT.company_phone = (TYPEOF(RIGHT.company_phone))'' OR SALT30.WithinEditN(LEFT.company_phone,RIGHT.company_phone,1, 0)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestPhoneLongest_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_phone,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_phone,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestPhoneLongest_tab_(company_phone NOT IN SET(s.nulls_company_phone,company_phone),fn_valid_cphone7(TRIM((SALT30.StrType)company_phone),source_for_votes)),{Proxid,source_for_votes,data_permits,company_phone,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
Fuzzy_layout NoteSupport(BestPhoneLongest_tab_company_phone le,BestPhoneLongest_tab_company_phone ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestPhoneLongest_tab_company_phone,BestPhoneLongest_tab_company_phone, LEFT.Proxid = RIGHT.Proxid  AND ( LEFT.company_phone = (TYPEOF(LEFT.company_phone))'' OR RIGHT.company_phone = (TYPEOF(RIGHT.company_phone))'' OR SALT30.WithinEditN(LEFT.company_phone,RIGHT.company_phone,1, 0)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestPhoneStrong_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_phone,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_phone,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestPhoneStrong_tab_(company_phone NOT IN SET(s.nulls_company_phone,company_phone),fn_valid_cphone7(TRIM((SALT30.StrType)company_phone),source_for_votes)),{Proxid,source_for_votes,data_permits,company_phone,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
Fuzzy_layout NoteSupport(BestPhoneStrong_tab_company_phone le,BestPhoneStrong_tab_company_phone ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestPhoneStrong_tab_company_phone,BestPhoneStrong_tab_company_phone, LEFT.Proxid = RIGHT.Proxid  AND ( LEFT.company_phone = (TYPEOF(LEFT.company_phone))'' OR RIGHT.company_phone = (TYPEOF(RIGHT.company_phone))'' OR SALT30.WithinEditN(LEFT.company_phone,RIGHT.company_phone,1, 0)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestPhoneVotedUnrestricted_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_phone,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_phone,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestPhoneVotedUnrestricted_tab_(company_phone NOT IN SET(s.nulls_company_phone,company_phone),fn_valid_cphone10(TRIM((SALT30.StrType)company_phone),source_for_votes)),{Proxid,source_for_votes,data_permits,company_phone,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
Fuzzy_layout NoteSupport(BestPhoneVotedUnrestricted_vote_company_phone le,BestPhoneVotedUnrestricted_vote_company_phone ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestPhoneVotedUnrestricted_vote_company_phone,BestPhoneVotedUnrestricted_vote_company_phone, LEFT.Proxid = RIGHT.Proxid  AND ( LEFT.company_phone = (TYPEOF(LEFT.company_phone))'' OR RIGHT.company_phone = (TYPEOF(RIGHT.company_phone))'' OR SALT30.WithinEditN(LEFT.company_phone,RIGHT.company_phone,1, 0)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestPhoneCommon_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_phone,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_phone,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestPhoneCommon_tab_(company_phone NOT IN SET(s.nulls_company_phone,company_phone),fn_valid_cphone10(TRIM((SALT30.StrType)company_phone),source_for_votes)),{Proxid,source_for_votes,data_permits,company_phone,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
Fuzzy_layout NoteSupport(BestPhoneCommon_tab_company_phone le,BestPhoneCommon_tab_company_phone ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestPhoneCommon_tab_company_phone,BestPhoneCommon_tab_company_phone, LEFT.Proxid = RIGHT.Proxid  AND ( LEFT.company_phone = (TYPEOF(LEFT.company_phone))'' OR RIGHT.company_phone = (TYPEOF(RIGHT.company_phone))'' OR SALT30.WithinEditN(LEFT.company_phone,RIGHT.company_phone,1, 0)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestFeinStrong_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_fein,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_fein,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestFeinStrong_tab_(company_fein NOT IN SET(s.nulls_company_fein,company_fein),fn_valid_cfein(TRIM((SALT30.StrType)company_fein),source_for_votes)),{Proxid,source_for_votes,data_permits,company_fein,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
Fuzzy_layout NoteSupport(BestFeinStrong_tab_company_fein le,BestFeinStrong_tab_company_fein ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestFeinStrong_tab_company_fein,BestFeinStrong_tab_company_fein, LEFT.Proxid = RIGHT.Proxid  AND ( LEFT.company_fein = (TYPEOF(LEFT.company_fein))'' OR RIGHT.company_fein = (TYPEOF(RIGHT.company_fein))'' OR SALT30.WithinEditN(LEFT.company_fein,RIGHT.company_fein,1, 0)  OR fn_Right4(RIGHT.company_fein) = fn_Right4(LEFT.company_fein)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestFeinCommon_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_fein,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_fein,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestFeinCommon_tab_(company_fein NOT IN SET(s.nulls_company_fein,company_fein),fn_valid_cfein(TRIM((SALT30.StrType)company_fein),source_for_votes)),{Proxid,source_for_votes,data_permits,company_fein,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
Fuzzy_layout NoteSupport(BestFeinCommon_tab_company_fein le,BestFeinCommon_tab_company_fein ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestFeinCommon_tab_company_fein,BestFeinCommon_tab_company_fein, LEFT.Proxid = RIGHT.Proxid  AND ( LEFT.company_fein = (TYPEOF(LEFT.company_fein))'' OR RIGHT.company_fein = (TYPEOF(RIGHT.company_fein))'' OR SALT30.WithinEditN(LEFT.company_fein,RIGHT.company_fein,1, 0)  OR fn_Right4(RIGHT.company_fein) = fn_Right4(LEFT.company_fein)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestFeinCurrent_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_fein,UNSIGNED Early_Date := MIN(GROUP,IF(dt_last_seen=0,99999999,dt_last_seen)),UNSIGNED Late_Date := MAX(GROUP,dt_last_seen),UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_fein,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestFeinCurrent_tab_(company_fein NOT IN SET(s.nulls_company_fein,company_fein),fn_valid_cfein(TRIM((SALT30.StrType)company_fein),source_for_votes)),{Proxid,source_for_votes,data_permits,company_fein,Early_Date,Late_Date,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
EXPORT BestFeinVotedUnrestricted_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_fein,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_fein,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestFeinVotedUnrestricted_tab_(company_fein NOT IN SET(s.nulls_company_fein,company_fein),fn_valid_cfein(TRIM((SALT30.StrType)company_fein),source_for_votes)),{Proxid,source_for_votes,data_permits,company_fein,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestFeinMin_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_fein,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_fein,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestFeinMin_tab_(company_fein NOT IN SET(s.nulls_company_fein,company_fein),fn_valid_cfein(TRIM((SALT30.StrType)company_fein),source_for_votes)),{Proxid,source_for_votes,data_permits,company_fein,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
Fuzzy_layout NoteSupport(BestFeinMin_vote_company_fein le,BestFeinMin_vote_company_fein ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestFeinMin_vote_company_fein,BestFeinMin_vote_company_fein, LEFT.Proxid = RIGHT.Proxid  AND ( LEFT.company_fein = (TYPEOF(LEFT.company_fein))'' OR RIGHT.company_fein = (TYPEOF(RIGHT.company_fein))'' OR SALT30.WithinEditN(LEFT.company_fein,RIGHT.company_fein,1, 0)  OR fn_Right4(RIGHT.company_fein) = fn_Right4(LEFT.company_fein)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestFeinMax_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_fein,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_fein,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestFeinMax_tab_(company_fein NOT IN SET(s.nulls_company_fein,company_fein),fn_valid_cfein(TRIM((SALT30.StrType)company_fein),source_for_votes)),{Proxid,source_for_votes,data_permits,company_fein,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
Fuzzy_layout NoteSupport(BestFeinMax_vote_company_fein le,BestFeinMax_vote_company_fein ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestFeinMax_vote_company_fein,BestFeinMax_vote_company_fein, LEFT.Proxid = RIGHT.Proxid  AND ( LEFT.company_fein = (TYPEOF(LEFT.company_fein))'' OR RIGHT.company_fein = (TYPEOF(RIGHT.company_fein))'' OR SALT30.WithinEditN(LEFT.company_fein,RIGHT.company_fein,1, 0)  OR fn_Right4(RIGHT.company_fein) = fn_Right4(LEFT.company_fein)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestUrlCommon_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_url,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_url,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestUrlCommon_tab_(company_url NOT IN SET(s.nulls_company_url,company_url),fn_valid_curl(TRIM((SALT30.StrType)company_url),source_for_votes)),{Proxid,source_for_votes,data_permits,company_url,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value enforcing minimum
  grp := GROUP( BestUrlCommon_tab_company_url,Proxid,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
...
EXPORT BestUrlCurrent_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_url,UNSIGNED Early_Date := MIN(GROUP,IF(dt_last_seen=0,99999999,dt_last_seen)),UNSIGNED Late_Date := MAX(GROUP,dt_last_seen),UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_url,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestUrlCurrent_tab_(company_url NOT IN SET(s.nulls_company_url,company_url),fn_valid_curl(TRIM((SALT30.StrType)company_url),source_for_votes)),{Proxid,source_for_votes,data_permits,company_url,Early_Date,Late_Date,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
EXPORT BestUrlLength_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_url,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_url,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestUrlLength_tab_(company_url NOT IN SET(s.nulls_company_url,company_url),fn_valid_curl(TRIM((SALT30.StrType)company_url),source_for_votes)),{Proxid,source_for_votes,data_permits,company_url,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestUrlStrong_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_url,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_url,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestUrlStrong_tab_(company_url NOT IN SET(s.nulls_company_url,company_url),fn_valid_curl(TRIM((SALT30.StrType)company_url),source_for_votes)),{Proxid,source_for_votes,data_permits,company_url,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value
  grp := GROUP( BestUrlStrong_tab_company_url,Proxid,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
// A best solution is unique if its Row_Cnt is the same as the sum of the row_cnts.
  Totals := TABLE(GROUP(grp),{Proxid,Tot := SUM(GROUP,Row_Cnt)},Proxid);
...
EXPORT BestUrlVotedUnrestricted_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,company_url,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,company_url,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestUrlVotedUnrestricted_tab_(company_url NOT IN SET(s.nulls_company_url,company_url),fn_valid_curl(TRIM((SALT30.StrType)company_url),source_for_votes)),{Proxid,source_for_votes,data_permits,company_url,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
EXPORT BestDunsCommon_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,duns_number,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,duns_number,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestDunsCommon_tab_(duns_number NOT IN SET(s.nulls_duns_number,duns_number),fn_valid_duns(TRIM((SALT30.StrType)duns_number),source_for_votes)),{Proxid,source_for_votes,data_permits,duns_number,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
Fuzzy_layout NoteSupport(BestDunsCommon_tab_duns_number le,BestDunsCommon_tab_duns_number ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestDunsCommon_tab_duns_number,BestDunsCommon_tab_duns_number, LEFT.Proxid = RIGHT.Proxid  AND ( LEFT.duns_number = (TYPEOF(LEFT.duns_number))'' OR RIGHT.duns_number = (TYPEOF(RIGHT.duns_number))'' OR SALT30.WithinEditN(LEFT.duns_number,RIGHT.duns_number,1, 0)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value enforcing minimum
  grp := GROUP( BestDunsCommon_fuzz_duns_number,Proxid,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt,-Orig_Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
...
EXPORT BestDunsCurrent_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,duns_number,UNSIGNED Early_Date := MIN(GROUP,IF(dt_last_seen=0,99999999,dt_last_seen)),UNSIGNED Late_Date := MAX(GROUP,dt_last_seen),UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,duns_number,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestDunsCurrent_tab_(duns_number NOT IN SET(s.nulls_duns_number,duns_number),fn_valid_duns(TRIM((SALT30.StrType)duns_number),source_for_votes)),{Proxid,source_for_votes,data_permits,duns_number,Early_Date,Late_Date,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
Fuzzy_layout NoteSupport(BestDunsCurrent_tab_duns_number le,BestDunsCurrent_tab_duns_number ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestDunsCurrent_tab_duns_number,BestDunsCurrent_tab_duns_number, LEFT.Proxid = RIGHT.Proxid  AND ( LEFT.duns_number = (TYPEOF(LEFT.duns_number))'' OR RIGHT.duns_number = (TYPEOF(RIGHT.duns_number))'' OR SALT30.WithinEditN(LEFT.duns_number,RIGHT.duns_number,1, 0)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
EXPORT BestDunsCurrent2_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,duns_number,UNSIGNED Early_Date := MIN(GROUP,IF(dt_last_seen=0,99999999,dt_last_seen)),UNSIGNED Late_Date := MAX(GROUP,dt_last_seen),UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,duns_number,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestDunsCurrent2_tab_(duns_number NOT IN SET(s.nulls_duns_number,duns_number),fn_valid_duns(TRIM((SALT30.StrType)duns_number),source_for_votes)),{Proxid,source_for_votes,data_permits,duns_number,Early_Date,Late_Date,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
EXPORT BestDunsVotedUnrestricted_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,source_for_votes,data_permits,duns_number,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,source_for_votes,data_permits,duns_number,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestDunsVotedUnrestricted_tab_(duns_number NOT IN SET(s.nulls_duns_number,duns_number),fn_valid_duns(TRIM((SALT30.StrType)duns_number),source_for_votes)),{Proxid,source_for_votes,data_permits,duns_number,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
// Voting is supported by altering the Row Count to be 100 * the (real) vote returned from the attribute
  Voted := TABLE( BestDunsVotedUnrestricted_tab_duns_number,{Proxid,data_permits,duns_number,UNSIGNED Row_Cnt := 100 * fn_Best_Source_Unrestricted_Votes(source_for_votes,Row_Cnt)}); // Use fn_Best_Source_Unrestricted_Votes to vote
  Voted TR(Voted le, Voted ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value
  grp := GROUP( BestDunsVotedUnrestricted_vote_duns_number,Proxid,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  SALT30.MAC_Apply_Threshold(srt,200,o);
...
EXPORT BestSicCommon_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,data_permits,company_sic_code1,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,data_permits,company_sic_code1,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestSicCommon_tab_(company_sic_code1 NOT IN SET(s.nulls_company_sic_code1,company_sic_code1)),{Proxid,data_permits,company_sic_code1,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value enforcing minimum
  grp := GROUP( BestSicCommon_tab_company_sic_code1,Proxid,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
...
EXPORT BestSicCurrent_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,data_permits,company_sic_code1,UNSIGNED Early_Date := MIN(GROUP,IF(dt_last_seen=0,99999999,dt_last_seen)),UNSIGNED Late_Date := MAX(GROUP,dt_last_seen),UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,data_permits,company_sic_code1,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestSicCurrent_tab_(company_sic_code1 NOT IN SET(s.nulls_company_sic_code1,company_sic_code1)),{Proxid,data_permits,company_sic_code1,Early_Date,Late_Date,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
EXPORT BestNaicsCommon_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,data_permits,company_naics_code1,UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,data_permits,company_naics_code1,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestNaicsCommon_tab_(company_naics_code1 NOT IN SET(s.nulls_company_naics_code1,company_naics_code1)),{Proxid,data_permits,company_naics_code1,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value enforcing minimum
  grp := GROUP( BestNaicsCommon_tab_company_naics_code1,Proxid,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
...
EXPORT BestNaicsCurrent_tab_ := DISTRIBUTE(TABLE(h(Proxid <> 0),{Proxid,data_permits,company_naics_code1,UNSIGNED Early_Date := MIN(GROUP,IF(dt_last_seen=0,99999999,dt_last_seen)),UNSIGNED Late_Date := MAX(GROUP,dt_last_seen),UNSIGNED Row_Cnt := COUNT(GROUP)},Proxid,data_permits,company_naics_code1,MERGE),HASH(Proxid)); // Slim and reduce row-count
  Slim := TABLE(BestNaicsCurrent_tab_(company_naics_code1 NOT IN SET(s.nulls_company_naics_code1,company_naics_code1)),{Proxid,data_permits,company_naics_code1,Early_Date,Late_Date,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
// 0 - Gathering type:BestCompanyNameLegal Entries:1
  R0 := RECORD
    typeof(BestCompanyNameLegal_method_company_name.Proxid) Proxid; // Need to copy in basis
...
// 1 - Gathering type:BestCompanyNameCommon Entries:1
  R1 := RECORD
    J0; // The data so far
  ...
  END;
  R1 T1(J0 le,BestCompanyNameCommon_method_company_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J1 := JOIN(J0,BestCompanyNameCommon_method_company_name,LEFT.Proxid = RIGHT.Proxid,T1(LEFT,RIGHT),FULL OUTER,LOCAL);
// 2 - Gathering type:BestCompanyNameCurrent Entries:1
  R2 := RECORD
    J1; // The data so far
  ...
  END;
  R2 T2(J1 le,BestCompanyNameCurrent_method_company_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J2 := JOIN(J1,BestCompanyNameCurrent_method_company_name,LEFT.Proxid = RIGHT.Proxid,T2(LEFT,RIGHT),FULL OUTER,LOCAL);
// 3 - Gathering type:BestCompanyNameVoted Entries:1
  R3 := RECORD
    J2; // The data so far
  ...
  END;
  R3 T3(J2 le,BestCompanyNameVoted_method_company_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J3 := JOIN(J2,BestCompanyNameVoted_method_company_name,LEFT.Proxid = RIGHT.Proxid,T3(LEFT,RIGHT),FULL OUTER,LOCAL);
// 4 - Gathering type:BestCompanyNameLength Entries:1
  R4 := RECORD
    J3; // The data so far
  ...
  END;
  R4 T4(J3 le,BestCompanyNameLength_method_company_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J4 := JOIN(J3,BestCompanyNameLength_method_company_name,LEFT.Proxid = RIGHT.Proxid,T4(LEFT,RIGHT),FULL OUTER,LOCAL);
// 5 - Gathering type:BestCompanyNameStrong Entries:1
  R5 := RECORD
    J4; // The data so far
  ...
  END;
  R5 T5(J4 le,BestCompanyNameStrong_method_company_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J5 := JOIN(J4,BestCompanyNameStrong_method_company_name,LEFT.Proxid = RIGHT.Proxid,T5(LEFT,RIGHT),FULL OUTER,LOCAL);
// 6 - Gathering type:BestCompanyNameCurrent2 Entries:1
  R6 := RECORD
    J5; // The data so far
  ...
  END;
  R6 T6(J5 le,BestCompanyNameCurrent2_method_company_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J6 := JOIN(J5,BestCompanyNameCurrent2_method_company_name,LEFT.Proxid = RIGHT.Proxid,T6(LEFT,RIGHT),FULL OUTER,LOCAL);
// 7 - Gathering type:BestCompanyNameVotedUnrestricted Entries:1
  R7 := RECORD
    J6; // The data so far
  ...
  END;
  R7 T7(J6 le,BestCompanyNameVotedUnrestricted_method_company_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J7 := JOIN(J6,BestCompanyNameVotedUnrestricted_method_company_name,LEFT.Proxid = RIGHT.Proxid,T7(LEFT,RIGHT),FULL OUTER,LOCAL);
// 8 - Gathering type:BestCompanyAddressVoted Entries:1
  R8 := RECORD
    J7; // The data so far
  ...
  END;
  R8 T8(J7 le,BestCompanyAddressVoted_method_address ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J8 := JOIN(J7,BestCompanyAddressVoted_method_address,LEFT.Proxid = RIGHT.Proxid,T8(LEFT,RIGHT),FULL OUTER,LOCAL);
// 9 - Gathering type:BestCompanyAddressCurrent Entries:1
  R9 := RECORD
    J8; // The data so far
  ...
  END;
  R9 T9(J8 le,BestCompanyAddressCurrent_method_address ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J9 := JOIN(J8,BestCompanyAddressCurrent_method_address,LEFT.Proxid = RIGHT.Proxid,T9(LEFT,RIGHT),FULL OUTER,LOCAL);
// 10 - Gathering type:BestCompanyAddressVotedSrc Entries:1
  R10 := RECORD
    J9; // The data so far
  ...
  END;
  R10 T10(J9 le,BestCompanyAddressVotedSrc_method_address ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J10 := JOIN(J9,BestCompanyAddressVotedSrc_method_address,LEFT.Proxid = RIGHT.Proxid,T10(LEFT,RIGHT),FULL OUTER,LOCAL);
// 11 - Gathering type:BestCompanyAddressCommon Entries:1
  R11 := RECORD
    J10; // The data so far
  ...
  END;
  R11 T11(J10 le,BestCompanyAddressCommon_method_address ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J11 := JOIN(J10,BestCompanyAddressCommon_method_address,LEFT.Proxid = RIGHT.Proxid,T11(LEFT,RIGHT),FULL OUTER,LOCAL);
// 12 - Gathering type:BestCompanyAddressStrong Entries:1
  R12 := RECORD
    J11; // The data so far
  ...
  END;
  R12 T12(J11 le,BestCompanyAddressStrong_method_address ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J12 := JOIN(J11,BestCompanyAddressStrong_method_address,LEFT.Proxid = RIGHT.Proxid,T12(LEFT,RIGHT),FULL OUTER,LOCAL);
// 13 - Gathering type:BestCompanyAddressCurrent2 Entries:1
  R13 := RECORD
    J12; // The data so far
  ...
  END;
  R13 T13(J12 le,BestCompanyAddressCurrent2_method_address ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J13 := JOIN(J12,BestCompanyAddressCurrent2_method_address,LEFT.Proxid = RIGHT.Proxid,T13(LEFT,RIGHT),FULL OUTER,LOCAL);
// 14 - Gathering type:BestCompanyAddressVotedUnrestricted Entries:1
  R14 := RECORD
    J13; // The data so far
  ...
  END;
  R14 T14(J13 le,BestCompanyAddressVotedUnrestricted_method_address ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J14 := JOIN(J13,BestCompanyAddressVotedUnrestricted_method_address,LEFT.Proxid = RIGHT.Proxid,T14(LEFT,RIGHT),FULL OUTER,LOCAL);
// 15 - Gathering type:BestPhoneCurrentWithNpa Entries:1
  R15 := RECORD
    J14; // The data so far
  ...
  END;
  R15 T15(J14 le,BestPhoneCurrentWithNpa_method_company_phone ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J15 := JOIN(J14,BestPhoneCurrentWithNpa_method_company_phone,LEFT.Proxid = RIGHT.Proxid,T15(LEFT,RIGHT),FULL OUTER,LOCAL);
// 16 - Gathering type:BestPhoneCurrent Entries:1
  R16 := RECORD
    J15; // The data so far
  ...
  END;
  R16 T16(J15 le,BestPhoneCurrent_method_company_phone ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J16 := JOIN(J15,BestPhoneCurrent_method_company_phone,LEFT.Proxid = RIGHT.Proxid,T16(LEFT,RIGHT),FULL OUTER,LOCAL);
// 17 - Gathering type:BestPhoneVoted Entries:1
  R17 := RECORD
    J16; // The data so far
  ...
  END;
  R17 T17(J16 le,BestPhoneVoted_method_company_phone ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J17 := JOIN(J16,BestPhoneVoted_method_company_phone,LEFT.Proxid = RIGHT.Proxid,T17(LEFT,RIGHT),FULL OUTER,LOCAL);
// 18 - Gathering type:BestPhoneLongest Entries:1
  R18 := RECORD
    J17; // The data so far
  ...
  END;
  R18 T18(J17 le,BestPhoneLongest_method_company_phone ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J18 := JOIN(J17,BestPhoneLongest_method_company_phone,LEFT.Proxid = RIGHT.Proxid,T18(LEFT,RIGHT),FULL OUTER,LOCAL);
// 19 - Gathering type:BestPhoneStrong Entries:1
  R19 := RECORD
    J18; // The data so far
  ...
  END;
  R19 T19(J18 le,BestPhoneStrong_method_company_phone ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J19 := JOIN(J18,BestPhoneStrong_method_company_phone,LEFT.Proxid = RIGHT.Proxid,T19(LEFT,RIGHT),FULL OUTER,LOCAL);
// 20 - Gathering type:BestPhoneVotedUnrestricted Entries:1
  R20 := RECORD
    J19; // The data so far
  ...
  END;
  R20 T20(J19 le,BestPhoneVotedUnrestricted_method_company_phone ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J20 := JOIN(J19,BestPhoneVotedUnrestricted_method_company_phone,LEFT.Proxid = RIGHT.Proxid,T20(LEFT,RIGHT),FULL OUTER,LOCAL);
// 21 - Gathering type:BestPhoneCommon Entries:1
  R21 := RECORD
    J20; // The data so far
  ...
  END;
  R21 T21(J20 le,BestPhoneCommon_method_company_phone ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J21 := JOIN(J20,BestPhoneCommon_method_company_phone,LEFT.Proxid = RIGHT.Proxid,T21(LEFT,RIGHT),FULL OUTER,LOCAL);
// 22 - Gathering type:BestFeinStrong Entries:1
  R22 := RECORD
    J21; // The data so far
  ...
  END;
  R22 T22(J21 le,BestFeinStrong_method_company_fein ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J22 := JOIN(J21,BestFeinStrong_method_company_fein,LEFT.Proxid = RIGHT.Proxid,T22(LEFT,RIGHT),FULL OUTER,LOCAL);
// 23 - Gathering type:BestFeinCommon Entries:1
  R23 := RECORD
    J22; // The data so far
  ...
  END;
  R23 T23(J22 le,BestFeinCommon_method_company_fein ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J23 := JOIN(J22,BestFeinCommon_method_company_fein,LEFT.Proxid = RIGHT.Proxid,T23(LEFT,RIGHT),FULL OUTER,LOCAL);
// 24 - Gathering type:BestFeinCurrent Entries:1
  R24 := RECORD
    J23; // The data so far
  ...
  END;
  R24 T24(J23 le,BestFeinCurrent_method_company_fein ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J24 := JOIN(J23,BestFeinCurrent_method_company_fein,LEFT.Proxid = RIGHT.Proxid,T24(LEFT,RIGHT),FULL OUTER,LOCAL);
// 25 - Gathering type:BestFeinVotedUnrestricted Entries:1
  R25 := RECORD
    J24; // The data so far
  ...
  END;
  R25 T25(J24 le,BestFeinVotedUnrestricted_method_company_fein ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J25 := JOIN(J24,BestFeinVotedUnrestricted_method_company_fein,LEFT.Proxid = RIGHT.Proxid,T25(LEFT,RIGHT),FULL OUTER,LOCAL);
// 26 - Gathering type:BestFeinMin Entries:1
  R26 := RECORD
    J25; // The data so far
  ...
  END;
  R26 T26(J25 le,BestFeinMin_method_company_fein ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J26 := JOIN(J25,BestFeinMin_method_company_fein,LEFT.Proxid = RIGHT.Proxid,T26(LEFT,RIGHT),FULL OUTER,LOCAL);
// 27 - Gathering type:BestFeinMax Entries:1
  R27 := RECORD
    J26; // The data so far
  ...
  END;
  R27 T27(J26 le,BestFeinMax_method_company_fein ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J27 := JOIN(J26,BestFeinMax_method_company_fein,LEFT.Proxid = RIGHT.Proxid,T27(LEFT,RIGHT),FULL OUTER,LOCAL);
// 28 - Gathering type:BestUrlCommon Entries:1
  R28 := RECORD
    J27; // The data so far
  ...
  END;
  R28 T28(J27 le,BestUrlCommon_method_company_url ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J28 := JOIN(J27,BestUrlCommon_method_company_url,LEFT.Proxid = RIGHT.Proxid,T28(LEFT,RIGHT),FULL OUTER,LOCAL);
// 29 - Gathering type:BestUrlCurrent Entries:1
  R29 := RECORD
    J28; // The data so far
  ...
  END;
  R29 T29(J28 le,BestUrlCurrent_method_company_url ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J29 := JOIN(J28,BestUrlCurrent_method_company_url,LEFT.Proxid = RIGHT.Proxid,T29(LEFT,RIGHT),FULL OUTER,LOCAL);
// 30 - Gathering type:BestUrlLength Entries:1
  R30 := RECORD
    J29; // The data so far
  ...
  END;
  R30 T30(J29 le,BestUrlLength_method_company_url ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J30 := JOIN(J29,BestUrlLength_method_company_url,LEFT.Proxid = RIGHT.Proxid,T30(LEFT,RIGHT),FULL OUTER,LOCAL);
// 31 - Gathering type:BestUrlStrong Entries:1
  R31 := RECORD
    J30; // The data so far
  ...
  END;
  R31 T31(J30 le,BestUrlStrong_method_company_url ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J31 := JOIN(J30,BestUrlStrong_method_company_url,LEFT.Proxid = RIGHT.Proxid,T31(LEFT,RIGHT),FULL OUTER,LOCAL);
// 32 - Gathering type:BestUrlVotedUnrestricted Entries:1
  R32 := RECORD
    J31; // The data so far
  ...
  END;
  R32 T32(J31 le,BestUrlVotedUnrestricted_method_company_url ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J32 := JOIN(J31,BestUrlVotedUnrestricted_method_company_url,LEFT.Proxid = RIGHT.Proxid,T32(LEFT,RIGHT),FULL OUTER,LOCAL);
// 33 - Gathering type:BestDunsCommon Entries:1
  R33 := RECORD
    J32; // The data so far
  ...
  END;
  R33 T33(J32 le,BestDunsCommon_method_duns_number ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J33 := JOIN(J32,BestDunsCommon_method_duns_number,LEFT.Proxid = RIGHT.Proxid,T33(LEFT,RIGHT),FULL OUTER,LOCAL);
// 34 - Gathering type:BestDunsCurrent Entries:1
  R34 := RECORD
    J33; // The data so far
  ...
  END;
  R34 T34(J33 le,BestDunsCurrent_method_duns_number ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J34 := JOIN(J33,BestDunsCurrent_method_duns_number,LEFT.Proxid = RIGHT.Proxid,T34(LEFT,RIGHT),FULL OUTER,LOCAL);
// 35 - Gathering type:BestDunsCurrent2 Entries:1
  R35 := RECORD
    J34; // The data so far
  ...
  END;
  R35 T35(J34 le,BestDunsCurrent2_method_duns_number ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J35 := JOIN(J34,BestDunsCurrent2_method_duns_number,LEFT.Proxid = RIGHT.Proxid,T35(LEFT,RIGHT),FULL OUTER,LOCAL);
// 36 - Gathering type:BestDunsVotedUnrestricted Entries:1
  R36 := RECORD
    J35; // The data so far
  ...
  END;
  R36 T36(J35 le,BestDunsVotedUnrestricted_method_duns_number ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J36 := JOIN(J35,BestDunsVotedUnrestricted_method_duns_number,LEFT.Proxid = RIGHT.Proxid,T36(LEFT,RIGHT),FULL OUTER,LOCAL);
// 37 - Gathering type:BestSicCommon Entries:1
  R37 := RECORD
    J36; // The data so far
  ...
  END;
  R37 T37(J36 le,BestSicCommon_method_company_sic_code1 ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J37 := JOIN(J36,BestSicCommon_method_company_sic_code1,LEFT.Proxid = RIGHT.Proxid,T37(LEFT,RIGHT),FULL OUTER,LOCAL);
// 38 - Gathering type:BestSicCurrent Entries:1
  R38 := RECORD
    J37; // The data so far
  ...
  END;
  R38 T38(J37 le,BestSicCurrent_method_company_sic_code1 ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J38 := JOIN(J37,BestSicCurrent_method_company_sic_code1,LEFT.Proxid = RIGHT.Proxid,T38(LEFT,RIGHT),FULL OUTER,LOCAL);
// 39 - Gathering type:BestNaicsCommon Entries:1
  R39 := RECORD
    J38; // The data so far
  ...
  END;
  R39 T39(J38 le,BestNaicsCommon_method_company_naics_code1 ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J39 := JOIN(J38,BestNaicsCommon_method_company_naics_code1,LEFT.Proxid = RIGHT.Proxid,T39(LEFT,RIGHT),FULL OUTER,LOCAL);
// 40 - Gathering type:BestNaicsCurrent Entries:1
  R40 := RECORD
    J39; // The data so far
  ...
  END;
  R40 T40(J39 le,BestNaicsCurrent_method_company_naics_code1 ri) := TRANSFORM
...
// Now gather some statistics to see how we did
  R := RECORD
    ...
    REAL8 BestCompanyNameLegal_company_name_pcnt := AVE(GROUP,IF(BestBy_Proxid.BestCompanyNameLegal_company_name=(typeof(BestBy_Proxid.BestCompanyNameLegal_company_name))'',0,100));
    UNSIGNED BestCompanyNameLegal_company_name_permit1_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameLegal_data_permits&1<>0);
    UNSIGNED BestCompanyNameLegal_company_name_permit2_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameLegal_data_permits&2<>0);
    UNSIGNED BestCompanyNameLegal_company_name_permit3_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameLegal_data_permits&4<>0);
    UNSIGNED BestCompanyNameLegal_company_name_permit4_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameLegal_data_permits&8<>0);
    UNSIGNED BestCompanyNameLegal_company_name_permit5_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameLegal_data_permits&16<>0);
    UNSIGNED BestCompanyNameLegal_company_name_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameLegal_data_permits&32<>0);
    UNSIGNED BestCompanyNameLegal_company_name_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameLegal_data_permits&64<>0);
    UNSIGNED BestCompanyNameLegal_company_name_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameLegal_data_permits&128<>0);
    UNSIGNED BestCompanyNameLegal_company_name_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameLegal_data_permits&256<>0);
    UNSIGNED BestCompanyNameLegal_company_name_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameLegal_data_permits&512<>0);
    ...
    UNSIGNED BestCompanyNameVoted_company_name_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameVoted_data_permits&32<>0);
    UNSIGNED BestCompanyNameVoted_company_name_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameVoted_data_permits&64<>0);
    UNSIGNED BestCompanyNameVoted_company_name_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameVoted_data_permits&128<>0);
    UNSIGNED BestCompanyNameVoted_company_name_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameVoted_data_permits&256<>0);
    UNSIGNED BestCompanyNameVoted_company_name_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameVoted_data_permits&512<>0);
    ...
    UNSIGNED BestCompanyNameStrong_company_name_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameStrong_data_permits&32<>0);
    UNSIGNED BestCompanyNameStrong_company_name_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameStrong_data_permits&64<>0);
    UNSIGNED BestCompanyNameStrong_company_name_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameStrong_data_permits&128<>0);
    UNSIGNED BestCompanyNameStrong_company_name_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameStrong_data_permits&256<>0);
    UNSIGNED BestCompanyNameStrong_company_name_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameStrong_data_permits&512<>0);
    ...
    UNSIGNED BestCompanyNameCurrent2_company_name_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameCurrent2_data_permits&32<>0);
    UNSIGNED BestCompanyNameCurrent2_company_name_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameCurrent2_data_permits&64<>0);
    UNSIGNED BestCompanyNameCurrent2_company_name_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameCurrent2_data_permits&128<>0);
    UNSIGNED BestCompanyNameCurrent2_company_name_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameCurrent2_data_permits&256<>0);
    UNSIGNED BestCompanyNameCurrent2_company_name_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameCurrent2_data_permits&512<>0);
    ...
    UNSIGNED BestCompanyNameVotedUnrestricted_company_name_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameVotedUnrestricted_data_permits&32<>0);
    UNSIGNED BestCompanyNameVotedUnrestricted_company_name_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameVotedUnrestricted_data_permits&64<>0);
    UNSIGNED BestCompanyNameVotedUnrestricted_company_name_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameVotedUnrestricted_data_permits&128<>0);
    UNSIGNED BestCompanyNameVotedUnrestricted_company_name_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameVotedUnrestricted_data_permits&256<>0);
    UNSIGNED BestCompanyNameVotedUnrestricted_company_name_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_name_BestCompanyNameVotedUnrestricted_data_permits&512<>0);
    ...
    UNSIGNED BestCompanyAddressVoted_address_permit6_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVoted_data_permits&32<>0);
    UNSIGNED BestCompanyAddressVoted_address_permit7_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVoted_data_permits&64<>0);
    UNSIGNED BestCompanyAddressVoted_address_permit8_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVoted_data_permits&128<>0);
    UNSIGNED BestCompanyAddressVoted_address_permit9_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVoted_data_permits&256<>0);
    UNSIGNED BestCompanyAddressVoted_address_permit10_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVoted_data_permits&512<>0);
    ...
    UNSIGNED BestCompanyAddressCurrent_address_permit6_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCurrent_data_permits&32<>0);
    UNSIGNED BestCompanyAddressCurrent_address_permit7_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCurrent_data_permits&64<>0);
    UNSIGNED BestCompanyAddressCurrent_address_permit8_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCurrent_data_permits&128<>0);
    UNSIGNED BestCompanyAddressCurrent_address_permit9_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCurrent_data_permits&256<>0);
    UNSIGNED BestCompanyAddressCurrent_address_permit10_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCurrent_data_permits&512<>0);
    ...
    UNSIGNED BestCompanyAddressVotedSrc_address_permit6_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVotedSrc_data_permits&32<>0);
    UNSIGNED BestCompanyAddressVotedSrc_address_permit7_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVotedSrc_data_permits&64<>0);
    UNSIGNED BestCompanyAddressVotedSrc_address_permit8_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVotedSrc_data_permits&128<>0);
    UNSIGNED BestCompanyAddressVotedSrc_address_permit9_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVotedSrc_data_permits&256<>0);
    UNSIGNED BestCompanyAddressVotedSrc_address_permit10_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVotedSrc_data_permits&512<>0);
    ...
    UNSIGNED BestCompanyAddressCommon_address_permit6_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCommon_data_permits&32<>0);
    UNSIGNED BestCompanyAddressCommon_address_permit7_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCommon_data_permits&64<>0);
    UNSIGNED BestCompanyAddressCommon_address_permit8_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCommon_data_permits&128<>0);
    UNSIGNED BestCompanyAddressCommon_address_permit9_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCommon_data_permits&256<>0);
    UNSIGNED BestCompanyAddressCommon_address_permit10_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCommon_data_permits&512<>0);
    ...
    UNSIGNED BestCompanyAddressStrong_address_permit6_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressStrong_data_permits&32<>0);
    UNSIGNED BestCompanyAddressStrong_address_permit7_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressStrong_data_permits&64<>0);
    UNSIGNED BestCompanyAddressStrong_address_permit8_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressStrong_data_permits&128<>0);
    UNSIGNED BestCompanyAddressStrong_address_permit9_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressStrong_data_permits&256<>0);
    UNSIGNED BestCompanyAddressStrong_address_permit10_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressStrong_data_permits&512<>0);
    ...
    UNSIGNED BestCompanyAddressCurrent2_address_permit6_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCurrent2_data_permits&32<>0);
    UNSIGNED BestCompanyAddressCurrent2_address_permit7_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCurrent2_data_permits&64<>0);
    UNSIGNED BestCompanyAddressCurrent2_address_permit8_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCurrent2_data_permits&128<>0);
    UNSIGNED BestCompanyAddressCurrent2_address_permit9_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCurrent2_data_permits&256<>0);
    UNSIGNED BestCompanyAddressCurrent2_address_permit10_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressCurrent2_data_permits&512<>0);
    ...
    UNSIGNED BestCompanyAddressVotedUnrestricted_address_permit6_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVotedUnrestricted_data_permits&32<>0);
    UNSIGNED BestCompanyAddressVotedUnrestricted_address_permit7_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVotedUnrestricted_data_permits&64<>0);
    UNSIGNED BestCompanyAddressVotedUnrestricted_address_permit8_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVotedUnrestricted_data_permits&128<>0);
    UNSIGNED BestCompanyAddressVotedUnrestricted_address_permit9_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVotedUnrestricted_data_permits&256<>0);
    UNSIGNED BestCompanyAddressVotedUnrestricted_address_permit10_cnt := COUNT(GROUP,BestBy_Proxid.address_BestCompanyAddressVotedUnrestricted_data_permits&512<>0);
    ...
    UNSIGNED BestPhoneCurrentWithNpa_company_phone_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneCurrentWithNpa_data_permits&32<>0);
    UNSIGNED BestPhoneCurrentWithNpa_company_phone_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneCurrentWithNpa_data_permits&64<>0);
    UNSIGNED BestPhoneCurrentWithNpa_company_phone_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneCurrentWithNpa_data_permits&128<>0);
    UNSIGNED BestPhoneCurrentWithNpa_company_phone_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneCurrentWithNpa_data_permits&256<>0);
    UNSIGNED BestPhoneCurrentWithNpa_company_phone_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneCurrentWithNpa_data_permits&512<>0);
    ...
    UNSIGNED BestPhoneVoted_company_phone_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneVoted_data_permits&32<>0);
    UNSIGNED BestPhoneVoted_company_phone_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneVoted_data_permits&64<>0);
    UNSIGNED BestPhoneVoted_company_phone_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneVoted_data_permits&128<>0);
    UNSIGNED BestPhoneVoted_company_phone_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneVoted_data_permits&256<>0);
    UNSIGNED BestPhoneVoted_company_phone_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneVoted_data_permits&512<>0);
    ...
    UNSIGNED BestPhoneLongest_company_phone_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneLongest_data_permits&32<>0);
    UNSIGNED BestPhoneLongest_company_phone_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneLongest_data_permits&64<>0);
    UNSIGNED BestPhoneLongest_company_phone_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneLongest_data_permits&128<>0);
    UNSIGNED BestPhoneLongest_company_phone_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneLongest_data_permits&256<>0);
    UNSIGNED BestPhoneLongest_company_phone_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneLongest_data_permits&512<>0);
    ...
    UNSIGNED BestPhoneVotedUnrestricted_company_phone_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneVotedUnrestricted_data_permits&32<>0);
    UNSIGNED BestPhoneVotedUnrestricted_company_phone_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneVotedUnrestricted_data_permits&64<>0);
    UNSIGNED BestPhoneVotedUnrestricted_company_phone_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneVotedUnrestricted_data_permits&128<>0);
    UNSIGNED BestPhoneVotedUnrestricted_company_phone_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneVotedUnrestricted_data_permits&256<>0);
    UNSIGNED BestPhoneVotedUnrestricted_company_phone_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneVotedUnrestricted_data_permits&512<>0);
    ...
    UNSIGNED BestPhoneCommon_company_phone_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneCommon_data_permits&32<>0);
    UNSIGNED BestPhoneCommon_company_phone_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneCommon_data_permits&64<>0);
    UNSIGNED BestPhoneCommon_company_phone_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneCommon_data_permits&128<>0);
    UNSIGNED BestPhoneCommon_company_phone_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneCommon_data_permits&256<>0);
    UNSIGNED BestPhoneCommon_company_phone_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_phone_BestPhoneCommon_data_permits&512<>0);
    ...
    UNSIGNED BestFeinCurrent_company_fein_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinCurrent_data_permits&32<>0);
    UNSIGNED BestFeinCurrent_company_fein_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinCurrent_data_permits&64<>0);
    UNSIGNED BestFeinCurrent_company_fein_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinCurrent_data_permits&128<>0);
    UNSIGNED BestFeinCurrent_company_fein_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinCurrent_data_permits&256<>0);
    UNSIGNED BestFeinCurrent_company_fein_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinCurrent_data_permits&512<>0);
    ...
    UNSIGNED BestFeinVotedUnrestricted_company_fein_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinVotedUnrestricted_data_permits&32<>0);
    UNSIGNED BestFeinVotedUnrestricted_company_fein_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinVotedUnrestricted_data_permits&64<>0);
    UNSIGNED BestFeinVotedUnrestricted_company_fein_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinVotedUnrestricted_data_permits&128<>0);
    UNSIGNED BestFeinVotedUnrestricted_company_fein_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinVotedUnrestricted_data_permits&256<>0);
    UNSIGNED BestFeinVotedUnrestricted_company_fein_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinVotedUnrestricted_data_permits&512<>0);
    ...
    UNSIGNED BestFeinMin_company_fein_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinMin_data_permits&32<>0);
    UNSIGNED BestFeinMin_company_fein_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinMin_data_permits&64<>0);
    UNSIGNED BestFeinMin_company_fein_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinMin_data_permits&128<>0);
    UNSIGNED BestFeinMin_company_fein_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinMin_data_permits&256<>0);
    UNSIGNED BestFeinMin_company_fein_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinMin_data_permits&512<>0);
    ...
    UNSIGNED BestFeinMax_company_fein_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinMax_data_permits&32<>0);
    UNSIGNED BestFeinMax_company_fein_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinMax_data_permits&64<>0);
    UNSIGNED BestFeinMax_company_fein_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinMax_data_permits&128<>0);
    UNSIGNED BestFeinMax_company_fein_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinMax_data_permits&256<>0);
    UNSIGNED BestFeinMax_company_fein_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_fein_BestFeinMax_data_permits&512<>0);
    ...
    UNSIGNED BestUrlCurrent_company_url_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlCurrent_data_permits&32<>0);
    UNSIGNED BestUrlCurrent_company_url_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlCurrent_data_permits&64<>0);
    UNSIGNED BestUrlCurrent_company_url_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlCurrent_data_permits&128<>0);
    UNSIGNED BestUrlCurrent_company_url_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlCurrent_data_permits&256<>0);
    UNSIGNED BestUrlCurrent_company_url_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlCurrent_data_permits&512<>0);
    ...
    UNSIGNED BestUrlLength_company_url_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlLength_data_permits&32<>0);
    UNSIGNED BestUrlLength_company_url_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlLength_data_permits&64<>0);
    UNSIGNED BestUrlLength_company_url_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlLength_data_permits&128<>0);
    UNSIGNED BestUrlLength_company_url_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlLength_data_permits&256<>0);
    UNSIGNED BestUrlLength_company_url_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlLength_data_permits&512<>0);
    ...
    UNSIGNED BestUrlStrong_company_url_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlStrong_data_permits&32<>0);
    UNSIGNED BestUrlStrong_company_url_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlStrong_data_permits&64<>0);
    UNSIGNED BestUrlStrong_company_url_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlStrong_data_permits&128<>0);
    UNSIGNED BestUrlStrong_company_url_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlStrong_data_permits&256<>0);
    UNSIGNED BestUrlStrong_company_url_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlStrong_data_permits&512<>0);
    ...
    UNSIGNED BestUrlVotedUnrestricted_company_url_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlVotedUnrestricted_data_permits&32<>0);
    UNSIGNED BestUrlVotedUnrestricted_company_url_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlVotedUnrestricted_data_permits&64<>0);
    UNSIGNED BestUrlVotedUnrestricted_company_url_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlVotedUnrestricted_data_permits&128<>0);
    UNSIGNED BestUrlVotedUnrestricted_company_url_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlVotedUnrestricted_data_permits&256<>0);
    UNSIGNED BestUrlVotedUnrestricted_company_url_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_url_BestUrlVotedUnrestricted_data_permits&512<>0);
    REAL8 BestDunsCommon_duns_number_pcnt := AVE(GROUP,IF(BestBy_Proxid.BestDunsCommon_duns_number=(typeof(BestBy_Proxid.BestDunsCommon_duns_number))'',0,100));
    UNSIGNED BestDunsCommon_duns_number_permit1_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCommon_data_permits&1<>0);
    UNSIGNED BestDunsCommon_duns_number_permit2_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCommon_data_permits&2<>0);
    UNSIGNED BestDunsCommon_duns_number_permit3_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCommon_data_permits&4<>0);
    UNSIGNED BestDunsCommon_duns_number_permit4_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCommon_data_permits&8<>0);
    UNSIGNED BestDunsCommon_duns_number_permit5_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCommon_data_permits&16<>0);
    UNSIGNED BestDunsCommon_duns_number_permit6_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCommon_data_permits&32<>0);
    UNSIGNED BestDunsCommon_duns_number_permit7_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCommon_data_permits&64<>0);
    UNSIGNED BestDunsCommon_duns_number_permit8_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCommon_data_permits&128<>0);
    UNSIGNED BestDunsCommon_duns_number_permit9_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCommon_data_permits&256<>0);
    UNSIGNED BestDunsCommon_duns_number_permit10_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCommon_data_permits&512<>0);
    REAL8 BestDunsCurrent_duns_number_pcnt := AVE(GROUP,IF(BestBy_Proxid.BestDunsCurrent_duns_number=(typeof(BestBy_Proxid.BestDunsCurrent_duns_number))'',0,100));
    UNSIGNED BestDunsCurrent_duns_number_permit1_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent_data_permits&1<>0);
    UNSIGNED BestDunsCurrent_duns_number_permit2_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent_data_permits&2<>0);
    UNSIGNED BestDunsCurrent_duns_number_permit3_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent_data_permits&4<>0);
    UNSIGNED BestDunsCurrent_duns_number_permit4_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent_data_permits&8<>0);
    UNSIGNED BestDunsCurrent_duns_number_permit5_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent_data_permits&16<>0);
    UNSIGNED BestDunsCurrent_duns_number_permit6_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent_data_permits&32<>0);
    UNSIGNED BestDunsCurrent_duns_number_permit7_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent_data_permits&64<>0);
    UNSIGNED BestDunsCurrent_duns_number_permit8_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent_data_permits&128<>0);
    UNSIGNED BestDunsCurrent_duns_number_permit9_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent_data_permits&256<>0);
    UNSIGNED BestDunsCurrent_duns_number_permit10_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent_data_permits&512<>0);
    REAL8 BestDunsCurrent2_duns_number_pcnt := AVE(GROUP,IF(BestBy_Proxid.BestDunsCurrent2_duns_number=(typeof(BestBy_Proxid.BestDunsCurrent2_duns_number))'',0,100));
    UNSIGNED BestDunsCurrent2_duns_number_permit1_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent2_data_permits&1<>0);
    UNSIGNED BestDunsCurrent2_duns_number_permit2_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent2_data_permits&2<>0);
    UNSIGNED BestDunsCurrent2_duns_number_permit3_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent2_data_permits&4<>0);
    UNSIGNED BestDunsCurrent2_duns_number_permit4_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent2_data_permits&8<>0);
    UNSIGNED BestDunsCurrent2_duns_number_permit5_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent2_data_permits&16<>0);
    UNSIGNED BestDunsCurrent2_duns_number_permit6_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent2_data_permits&32<>0);
    UNSIGNED BestDunsCurrent2_duns_number_permit7_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent2_data_permits&64<>0);
    UNSIGNED BestDunsCurrent2_duns_number_permit8_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent2_data_permits&128<>0);
    UNSIGNED BestDunsCurrent2_duns_number_permit9_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent2_data_permits&256<>0);
    UNSIGNED BestDunsCurrent2_duns_number_permit10_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsCurrent2_data_permits&512<>0);
    REAL8 BestDunsVotedUnrestricted_duns_number_pcnt := AVE(GROUP,IF(BestBy_Proxid.BestDunsVotedUnrestricted_duns_number=(typeof(BestBy_Proxid.BestDunsVotedUnrestricted_duns_number))'',0,100));
    UNSIGNED BestDunsVotedUnrestricted_duns_number_permit1_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsVotedUnrestricted_data_permits&1<>0);
    UNSIGNED BestDunsVotedUnrestricted_duns_number_permit2_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsVotedUnrestricted_data_permits&2<>0);
    UNSIGNED BestDunsVotedUnrestricted_duns_number_permit3_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsVotedUnrestricted_data_permits&4<>0);
    UNSIGNED BestDunsVotedUnrestricted_duns_number_permit4_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsVotedUnrestricted_data_permits&8<>0);
    UNSIGNED BestDunsVotedUnrestricted_duns_number_permit5_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsVotedUnrestricted_data_permits&16<>0);
    UNSIGNED BestDunsVotedUnrestricted_duns_number_permit6_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsVotedUnrestricted_data_permits&32<>0);
    UNSIGNED BestDunsVotedUnrestricted_duns_number_permit7_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsVotedUnrestricted_data_permits&64<>0);
    UNSIGNED BestDunsVotedUnrestricted_duns_number_permit8_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsVotedUnrestricted_data_permits&128<>0);
    UNSIGNED BestDunsVotedUnrestricted_duns_number_permit9_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsVotedUnrestricted_data_permits&256<>0);
    UNSIGNED BestDunsVotedUnrestricted_duns_number_permit10_cnt := COUNT(GROUP,BestBy_Proxid.duns_number_BestDunsVotedUnrestricted_data_permits&512<>0);
    REAL8 BestSicCommon_company_sic_code1_pcnt := AVE(GROUP,IF(BestBy_Proxid.BestSicCommon_company_sic_code1=(typeof(BestBy_Proxid.BestSicCommon_company_sic_code1))'',0,100));
    UNSIGNED BestSicCommon_company_sic_code1_permit1_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCommon_data_permits&1<>0);
    UNSIGNED BestSicCommon_company_sic_code1_permit2_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCommon_data_permits&2<>0);
    UNSIGNED BestSicCommon_company_sic_code1_permit3_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCommon_data_permits&4<>0);
    UNSIGNED BestSicCommon_company_sic_code1_permit4_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCommon_data_permits&8<>0);
    UNSIGNED BestSicCommon_company_sic_code1_permit5_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCommon_data_permits&16<>0);
    UNSIGNED BestSicCommon_company_sic_code1_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCommon_data_permits&32<>0);
    UNSIGNED BestSicCommon_company_sic_code1_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCommon_data_permits&64<>0);
    UNSIGNED BestSicCommon_company_sic_code1_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCommon_data_permits&128<>0);
    UNSIGNED BestSicCommon_company_sic_code1_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCommon_data_permits&256<>0);
    UNSIGNED BestSicCommon_company_sic_code1_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCommon_data_permits&512<>0);
    REAL8 BestSicCurrent_company_sic_code1_pcnt := AVE(GROUP,IF(BestBy_Proxid.BestSicCurrent_company_sic_code1=(typeof(BestBy_Proxid.BestSicCurrent_company_sic_code1))'',0,100));
    UNSIGNED BestSicCurrent_company_sic_code1_permit1_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCurrent_data_permits&1<>0);
    UNSIGNED BestSicCurrent_company_sic_code1_permit2_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCurrent_data_permits&2<>0);
    UNSIGNED BestSicCurrent_company_sic_code1_permit3_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCurrent_data_permits&4<>0);
    UNSIGNED BestSicCurrent_company_sic_code1_permit4_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCurrent_data_permits&8<>0);
    UNSIGNED BestSicCurrent_company_sic_code1_permit5_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCurrent_data_permits&16<>0);
    UNSIGNED BestSicCurrent_company_sic_code1_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCurrent_data_permits&32<>0);
    UNSIGNED BestSicCurrent_company_sic_code1_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCurrent_data_permits&64<>0);
    UNSIGNED BestSicCurrent_company_sic_code1_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCurrent_data_permits&128<>0);
    UNSIGNED BestSicCurrent_company_sic_code1_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCurrent_data_permits&256<>0);
    UNSIGNED BestSicCurrent_company_sic_code1_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_sic_code1_BestSicCurrent_data_permits&512<>0);
    REAL8 BestNaicsCommon_company_naics_code1_pcnt := AVE(GROUP,IF(BestBy_Proxid.BestNaicsCommon_company_naics_code1=(typeof(BestBy_Proxid.BestNaicsCommon_company_naics_code1))'',0,100));
    UNSIGNED BestNaicsCommon_company_naics_code1_permit1_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCommon_data_permits&1<>0);
    UNSIGNED BestNaicsCommon_company_naics_code1_permit2_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCommon_data_permits&2<>0);
    UNSIGNED BestNaicsCommon_company_naics_code1_permit3_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCommon_data_permits&4<>0);
    UNSIGNED BestNaicsCommon_company_naics_code1_permit4_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCommon_data_permits&8<>0);
    UNSIGNED BestNaicsCommon_company_naics_code1_permit5_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCommon_data_permits&16<>0);
    UNSIGNED BestNaicsCommon_company_naics_code1_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCommon_data_permits&32<>0);
    UNSIGNED BestNaicsCommon_company_naics_code1_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCommon_data_permits&64<>0);
    UNSIGNED BestNaicsCommon_company_naics_code1_permit8_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCommon_data_permits&128<>0);
    UNSIGNED BestNaicsCommon_company_naics_code1_permit9_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCommon_data_permits&256<>0);
    UNSIGNED BestNaicsCommon_company_naics_code1_permit10_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCommon_data_permits&512<>0);
    REAL8 BestNaicsCurrent_company_naics_code1_pcnt := AVE(GROUP,IF(BestBy_Proxid.BestNaicsCurrent_company_naics_code1=(typeof(BestBy_Proxid.BestNaicsCurrent_company_naics_code1))'',0,100));
    UNSIGNED BestNaicsCurrent_company_naics_code1_permit1_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCurrent_data_permits&1<>0);
    UNSIGNED BestNaicsCurrent_company_naics_code1_permit2_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCurrent_data_permits&2<>0);
    UNSIGNED BestNaicsCurrent_company_naics_code1_permit3_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCurrent_data_permits&4<>0);
    UNSIGNED BestNaicsCurrent_company_naics_code1_permit4_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCurrent_data_permits&8<>0);
    UNSIGNED BestNaicsCurrent_company_naics_code1_permit5_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCurrent_data_permits&16<>0);
    UNSIGNED BestNaicsCurrent_company_naics_code1_permit6_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCurrent_data_permits&32<>0);
    UNSIGNED BestNaicsCurrent_company_naics_code1_permit7_cnt := COUNT(GROUP,BestBy_Proxid.company_naics_code1_BestNaicsCurrent_data_permits&64<>0);
...
SHARED F_BestBy_Proxid(DATASET({BestBy_Proxid}) d) := FUNCTION
  ...
  R T(BestBy_Proxid le) := TRANSFORM
    SELF.company_name_cases := DATASET([
        ...
        {le.BestCompanyNameCommon_company_name,le.company_name_BestCompanyNameCommon_data_permits,2},
        {le.BestCompanyNameCurrent_company_name,le.company_name_BestCompanyNameCurrent_data_permits,3},
        {le.BestCompanyNameVoted_company_name,le.company_name_BestCompanyNameVoted_data_permits,4},
        {le.BestCompanyNameLength_company_name,le.company_name_BestCompanyNameLength_data_permits,5},
        {le.BestCompanyNameStrong_company_name,le.company_name_BestCompanyNameStrong_data_permits,6},
        {le.BestCompanyNameCurrent2_company_name,le.company_name_BestCompanyNameCurrent2_data_permits,7},
        {le.BestCompanyNameVotedUnrestricted_company_name,le.company_name_BestCompanyNameVotedUnrestricted_data_permits,8}

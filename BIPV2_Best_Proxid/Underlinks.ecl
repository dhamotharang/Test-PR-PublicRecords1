...
EXPORT ForceFails := TABLE(RRF,R);
  ...
  R tosummary(ds le) := TRANSFORM
    SELF := le;
    SELF.Summary := IF(le.company_url_Score = 0,'','|'+IF(le.company_url_Score < 0,'-','')+'company_url')+IF(le.duns_number_Score = 0,'','|'+IF(le.duns_number_Score < 0,'-','')+'duns_number')+IF(le.company_name_Score = 0,'','|'+IF(le.company_name_Score < 0,'-','')+'company_name')+IF(le.company_fein_Score = 0,'','|'+IF(le.company_fein_Score < 0,'-','')+'company_fein')+IF(le.company_phone_Score = 0,'','|'+IF(le.company_phone_Score < 0,'-','')+'company_phone')+IF(le.dba_name_Score = 0,'','|'+IF(le.dba_name_Score < 0,'-','')+'dba_name')+IF(le.address_Score = 0,'','|'+IF(le.address_Score < 0,'-','')+'address')+IF(le.prim_name_Score = 0,'','|'+IF(le.prim_name_Score < 0,'-','')+'prim_name')+IF(le.zip_Score = 0,'','|'+IF(le.zip_Score < 0,'-','')+'zip')+IF(le.prim_range_Score = 0,'','|'+IF(le.prim_range_Score < 0,'-','')+'prim_range')+IF(le.zip4_Score = 0,'','|'+IF(le.zip4_Score < 0,'-','')+'zip4')+IF(le.sec_range_Score = 0,'','|'+IF(le.sec_range_Score < 0,'-','')+'sec_range')+IF(le.p_city_name_Score = 0,'','|'+IF(le.p_city_name_Score < 0,'-','')+'p_city_name')+IF(le.company_sic_code1_Score = 0,'','|'+IF(le.company_sic_code1_Score < 0,'-','')+'company_sic_code1')+IF(le.company_naics_code1_Score = 0,'','|'+IF(le.company_naics_code1_Score < 0,'-','')+'company_naics_code1')+IF(le.v_city_name_Score = 0,'','|'+IF(le.v_city_name_Score < 0,'-','')+'v_city_name')+IF(le.postdir_Score = 0,'','|'+IF(le.postdir_Score < 0,'-','')+'postdir')+IF(le.fips_county_Score = 0,'','|'+IF(le.fips_county_Score < 0,'-','')+'fips_county')+IF(le.predir_Score = 0,'','|'+IF(le.predir_Score < 0,'-','')+'predir')+IF(le.unit_desig_Score = 0,'','|'+IF(le.unit_desig_Score < 0,'-','')+'unit_desig')+IF(le.st_Score = 0,'','|'+IF(le.st_Score < 0,'-','')+'st')+IF(le.fips_state_Score = 0,'','|'+IF(le.fips_state_Score < 0,'-','')+'fips_state')+IF(le.addr_suffix_Score = 0,'','|'+IF(le.addr_suffix_Score < 0,'-','')+'addr_suffix');
  END;

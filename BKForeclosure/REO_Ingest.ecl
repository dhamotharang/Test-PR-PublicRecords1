IMPORT STD,SALT311;
EXPORT REO_Ingest(BOOLEAN incremental=FALSE
, DATASET(REO_Layout_BKForeclosure) Delta = DATASET([],REO_Layout_BKForeclosure)
, DATASET(REO_Layout_BKForeclosure) dsBase = REO_In_BKForeclosure // Change IN_BKForeclosure to change input to ingest process
, DATASET(RECORDOF(BKForeclosure.REO_prep_ingest_file))  infile = BKForeclosure.REO_prep_ingest_file
) := MODULE
  SHARED NullFile := DATASET([],REO_Layout_BKForeclosure); // Use to replace files you wish to remove
  ...
  SHARED AllRecs := ORe+NR1+NR(record_id<>0) : PERSIST('~temp::BKForeclosure::REO_Ingest_Cache',EXPIRE(BKForeclosure.Config.PersistExpire));
  ...
  SHARED S0 := OUTPUT(UpdateStats, {{UpdateStats} AND NOT __Tpe}, ALL, NAMED('REOUpdateStats'));
  ...
  EXPORT NewRecords_NoTag := PROJECT(NewRecords,REO_Layout_BKForeclosure);
  EXPORT OldRecords :=PROJECT( AllRecs(__Tpe=RecordType.Old), NoFlagsRec);
  EXPORT OldRecords_NoTag := PROJECT(OldRecords,REO_Layout_BKForeclosure);
  EXPORT UpdatedRecords := PROJECT(AllRecs(__Tpe=RecordType.Updated), NoFlagsRec);
  EXPORT UpdatedRecords_NoTag := PROJECT(UpdatedRecords,REO_Layout_BKForeclosure);
  EXPORT AllRecords := IF(incremental, NewRecords, PROJECT(AllRecs, NoFlagsRec));
  EXPORT AllRecords_NoTag := PROJECT(AllRecords,REO_Layout_BKForeclosure); // Records in 'pure' format

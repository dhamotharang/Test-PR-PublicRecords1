...
ClnName_NOD      := PROJECT(FileClnName,TRANSFORM(Layout_BK.CleanFields_NOD,
...
layout_address_nod		:= RECORD
	STRING		address1;
	STRING		address2;
	Layout_BK.CleanFields_NOD;
END;

layout_address_nod t_clean_address_nod (ClnName_NOD le)		:= TRANSFORM
...
AddrPrep	:= project(ClnName_NOD,t_clean_address_nod(left));
...
Layout_BK.CleanFields_NOD tNormAidAppends(addr_rec_out le):= TRANSFORM
...
END;
 ...
 Cln_GoodAddr := PROJECT(addr_rec_out, tNormAidAppends(LEFT)): PERSIST('~thor_data400::in::BKForeclosure::Cln_Addr_Nod');
 
Layout_BK.CleanFields_NOD tNoAddrClean(rsAID_NoAddr le):= TRANSFORM

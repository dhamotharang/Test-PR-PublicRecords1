...
bf4:=project(basefile_flcrash4,transform(	{
											basefile_flcrash4
											,unsigned6 temp_did
											},self.temp_did:=(integer)left.did,self.driver_st_city := trim(trim(left.driver_address,left,right)+' '+ trim(left.driver_st_city,left,right),left,right), self:=left));
...
export flc4_Keybuild := project(flc4,transform({slim_layout},self:=left));
...
flc2v :=project( basefile_flcrash2v,transform({slim_layout},
  self.vehicle_owner_st_city := trim(trim(left.vehicle_owner_address,left,right)+' '+ trim(left.vehicle_owner_st_city,left,right),left,right),
 self:= left));

export key_flcrash2v := index(flc2v
                             ,{unsigned6 l_acc_nbr := (integer)accident_nbr}
							 ,{flc2v}
							 ,Data_Services.Data_location.Prefix('NONAMEGIVEN') +'~thor_data400::key::flcrash2v_' + doxie.Version_SuperKey);

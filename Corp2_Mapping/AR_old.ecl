...
export AR := MODULE;
	...
	export CorpData := record,MAXLENGTH(999999)
	...
	export CorpNames := record,MAXLENGTH(999999)
		string 	Filing_Number;                  	
	...
	export CorpOfficer	:= record,MAXLENGTH(999999)
		string	Filing_Number;                  	
		string	Officer_ID	;		
		string	Address_ID	;		
	...
	export CorpWithOfficer	:= record,MAXLENGTH(999999)
	...
	export NewCorpOfficer	:= record,MAXLENGTH(999999)
...
export Files_Raw_Input := MODULE;
		...
		export CorpData (string fileDate)        := dataset('~thor_data400::in::corp2::'+fileDate+'::CorpData::AR',
														     layouts_Raw_Input.CorpData,CSV(SEPARATOR(['\t']),MAXLENGTH(999999) , quote(''), TERMINATOR(['\r\n', '\n'])));
															 
																		 
		export CorpNames(string fileDate)        := dataset('~thor_data400::in::corp2::'+fileDate+'::CorpNames::AR',
														     layouts_Raw_Input.CorpNames,CSV(SEPARATOR(['\t']),MAXLENGTH(999999) , quote(''), TERMINATOR(['\r\n', '\n'])));
															 
		export CorpOfficer (string fileDate)     := dataset('~thor_data400::in::corp2::'+fileDate+'::CorpOfficer::AR',
														     layouts_Raw_Input.CorpOfficer,CSV(SEPARATOR(['\t']),MAXLENGTH(999999), quote(''), TERMINATOR(['\r\n', '\n'])));
...
export Update(string fileDate, string version, boolean pShouldSpray = _Dataset().bShouldSpray, boolean pOverwrite = false) := function
	...
	TitleIdTable := dataset(_dataset().foreign_prod + 'thor_data400::lookup::corp2::officerid::ar', TitleIdTableLayout, CSV(SEPARATOR(['\t']), quote('"'), TERMINATOR(['\r\n', '\n'])));			
	...
	Corp2.Layout_Corporate_Direct_Cont_In CleanContNameAddr(PrecleanedCommonLayouts.contPre input) := transform
		...
		self.cont_prep_addr_line1       := if (trim(input.cont_address_line1,left,right) <> '' or
																					 ...
																					 StringLib.StringCleanSpaces(trim(input.cont_address_line1,left,right) + ' ' +
																																			 trim(input.cont_address_line2,left,right))
																					 ,'');
		self.cont_prep_addr_last_line   := if (trim(input.cont_address_line1,left,right) <> '' or
																					 trim(input.cont_address_line2,left,right) <> '' or
																					 trim(input.cont_address_line3,left,right) <> '' or
																					 trim(input.cont_address_line4,left,right) <> '' or
																					 trim(input.cont_address_line5,left,right) <> '',
																					 if (trim(input.cont_address_line3,left,right) != '',
																							 regexreplace(',$',
																							 StringLib.StringCleanSpaces(regexreplace(',',trim(input.cont_address_line3,left,right),'') + ', ' +
																																					 Corp2.Rewrite_Common.StateCodeFips_Reverse(
																																								StringLib.StringFilter(input.cont_address_line4,'ABCDEFGHIJKLMNOPQRSTUVWXYZ ')).StateCode + ' ' + 
																																					 StringLib.StringFilter(input.cont_address_line5,'1234567890')[1..5]),''), 
																							 StringLib.StringCleanSpaces(Corp2.Rewrite_Common.StateCodeFips_Reverse(
																																								StringLib.StringFilter(input.cont_address_line4,'ABCDEFGHIJKLMNOPQRSTUVWXYZ ')).StateCode + ' ' + 
																																					 StringLib.StringFilter(input.cont_address_line5,'1234567890')[1..5])
																							)
																					,''); 
	...
	corp2.layout_corporate_direct_corp_in CleanCorpNameAddr(PrecleanedCommonLayouts.corpPre input) := transform
		...
		self.corp_prep_addr1_line1      := if (trim(input.corp_address1_line1,left,right) <> '' and
																					 trim(input.corp_address1_line2,left,right) = '' and
																					 trim(input.corp_address1_line3,left,right) = '' and
																					 trim(input.corp_address1_line4,left,right) = '' and
																					 trim(input.corp_address1_line5,left,right) = '',
																					 Corp2_Mapping.Functions.fParseAddr(trim(input.corp_address1_line1,left,right)).Var_Street,
																					 StringLib.StringCleanSpaces(trim(input.corp_address1_line1,left,right) + ' '+
																																			 trim(input.corp_address1_line2,left,right)));
																																			 
		self.corp_prep_addr1_last_line 	:= if (trim(input.corp_address1_line1,left,right) <> '' and
																					 trim(input.corp_address1_line2,left,right) = '' and
																					 trim(input.corp_address1_line3,left,right) = '' and
																					 trim(input.corp_address1_line4,left,right) = '' and
																					 trim(input.corp_address1_line5,left,right) = '',
																					 regexreplace('^,|,$',																						 
																						 ...
																						 Corp2_Mapping.Functions.fParseAddr(trim(input.corp_address1_line1,left,right)).Var_city + ', '+
																						 Corp2_Mapping.Functions.fParseAddr(trim(input.corp_address1_line1,left,right)).Var_st + ' '+
																						 Corp2_Mapping.Functions.fParseAddr(trim(input.corp_address1_line1,left,right)).Var_zip),''),
																					 if (trim(input.corp_address1_line3,left,right) != '',
																							 regexreplace(',$',
																							 StringLib.StringCleanSpaces(regexreplace(',',trim(input.corp_address1_line3,left,right),'') + ', ' +
																																					 Corp2.Rewrite_Common.StateCodeFips_Reverse(
																																								StringLib.StringFilter(input.corp_address1_line4,'ABCDEFGHIJKLMNOPQRSTUVWXYZ ')).StateCode + ' ' + 
																																					 StringLib.StringFilter(input.corp_address1_line5,'1234567890')[1..5]),''), 
																							 StringLib.StringCleanSpaces(Corp2.Rewrite_Common.StateCodeFips_Reverse(
																																								StringLib.StringFilter(input.corp_address1_line4,'ABCDEFGHIJKLMNOPQRSTUVWXYZ ')).StateCode + ' ' + 
																																					 StringLib.StringFilter(input.corp_address1_line5,'1234567890')[1..5])
																							)
		...
		self.RA_prep_addr_line1         := if (trim(input.corp_ra_address_line1,left,right) <> '' and
																					 trim(input.corp_ra_address_line2,left,right) = '' and
																					 trim(input.corp_ra_address_line3,left,right) = '' and
																					 trim(input.corp_ra_address_line4,left,right) <> '' and
																					 trim(input.corp_ra_address_line5,left,right) = '',
																					 Corp2_Mapping.Functions.fParseAddr(trim(input.corp_ra_address_line1,left,right)).Var_Street,
																					 StringLib.StringCleanSpaces(trim(input.corp_ra_address_line1,left,right) + ' ' +
																																			 trim(input.corp_ra_address_line2,left,right)));
																																			 
		self.RA_prep_addr_last_line     := if (trim(input.corp_ra_address_line1,left,right) <> '' and
																					 trim(input.corp_ra_address_line2,left,right) = '' and
																					 trim(input.corp_ra_address_line3,left,right) = '' and
																					 trim(input.corp_ra_address_line4,left,right) <> '' and
																					 trim(input.corp_ra_address_line5,left,right) = '',
																					 regexreplace('^,|,$',																						 
																						 StringLib.StringCleanSpaces(
																						 Corp2_Mapping.Functions.fParseAddr(trim(input.corp_ra_address_line1,left,right)).Var_city + ', '+
																						 //Corp2_Mapping.Functions.fParseAddr(trim(input.corp_ra_address_line1,left,right)).Var_st + ' '+
																						 Corp2.Rewrite_Common.StateCodeFips_Reverse(StringLib.StringFilter(input.corp_ra_address_line4,'ABCDEFGHIJKLMNOPQRSTUVWXYZ ')).StateCode + ' ' +
																						 Corp2_Mapping.Functions.fParseAddr(trim(input.corp_ra_address_line1,left,right)).Var_zip),''),
																					 if (trim(input.corp_ra_address_line3,left,right) != '',
																							 regexreplace(',$',
																							 StringLib.StringCleanSpaces(regexreplace(',',trim(input.corp_ra_address_line3,left,right),'') + ', ' +
																																					 Corp2.Rewrite_Common.StateCodeFips_Reverse(
																																								StringLib.StringFilter(input.corp_ra_address_line4,'ABCDEFGHIJKLMNOPQRSTUVWXYZ ')).StateCode + ' ' +
																																					 StringLib.StringFilter(input.corp_ra_address_line5,'1234567890')[1..5]),''),
																							 StringLib.StringCleanSpaces(Corp2.Rewrite_Common.StateCodeFips_Reverse(
																																								StringLib.StringFilter(input.corp_ra_address_line4,'ABCDEFGHIJKLMNOPQRSTUVWXYZ ')).StateCode + ' ' +
																																					 StringLib.StringFilter(input.corp_ra_address_line5,'1234567890')[1..5])
																							)

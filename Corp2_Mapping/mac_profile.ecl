...
// changed to use SALT22.MAC_Character_Counts.FN_Profile because we don't have SALT23 in 
// boca prod.
...
export mac_profile( dIn ) := functionmacro
  ...
  %d_norm%:=
		normalize(dIn,
		          %field_cnt%,
	            transform(SALT22.MAC_Character_Counts.Data_Layout,
									      self.fldno:=counter,
												self.fld:=choose(#expand(%'field_values'%))
												)
							);
						
	return SALT22.MAC_Character_Counts.FN_Profile(%d_norm%,
		dataset([#expand(%'field_set'%)]
		        , SALT22.MAC_Character_Counts.Field_Identification ) );

Import _Control, Corp2, Corp2_Raw_NM, lib_stringlib, Scrubs, Scrubs_Corp2_Mapping_NM_Main, STD, Tools, UT, VersionControl;
...
Export NM 	:= Module 
 ...
 Export Update(String filedate, string version, boolean pShouldSpray = Corp2_mapping._Dataset().bShouldSpray, boolean pOverwrite = false, pUseProd = Tools._Constants.IsDataland) := Function
		...
		MasterFile				:= sort(distribute(Corp2_Raw_NM.Files(filedate,puseprod).Input.ImportMaster.Logical,hash(corp_nmscc)),corp_nmscc,local) : independent;
		...
		Corp2_Mapping.LayoutsCommon.Main corpTransform(Corp2_Raw_NM.Layouts.ImportMasterLayoutIn l):= transform
				...
				self.corp_key										 	:= state_fips + '-' + corp2.t2u(l.corp_nmscc);
				self.corp_vendor								 	:= state_fips;
				self.corp_state_origin					 	:= state_origin;
				self.corp_process_date					 	:= fileDate;
				self.corp_orig_sos_charter_nbr	 	:= corp2.t2u(l.corp_nmscc);
				...
				self.corp_status_cd							 	:= Corp2_Raw_NM.Functions.CorpStatusCD(l.corp_stat);
				self.corp_status_desc						 	:= Corp2_Raw_NM.Functions.CorpStatusDesc(l.corp_stat);
				//Note: Vendor stopped providing state of incorporaton for foreign business.
				//			Therefore, no state check is being done on corp_inc_date.
				self.corp_inc_date							  := if(Corp2_Raw_NM.Functions.CorpForeignDomesticInd(l.corp_type) in ['D'],
																								map(length(l.corp_doi_date) not in [0] 	=> Corp2_Mapping.fValidateDate(l.corp_doi_date,Corp2_Mapping.fFormatOfDate(l.corp_doi_date)).PastDate,
																										''
																									 ),
																								''
																							 );
				self.corp_inc_state								:= state_origin;
				Self.corp_orig_org_structure_cd 	:= Corp2_Raw_NM.Functions.CorpOrigOrgStructureCD(l.corp_type);
				self.corp_orig_org_structure_desc	:= Corp2_Raw_NM.Functions.CorpOrigOrgStructureDesc(l.corp_type);
				self.corp_for_profit_ind					:= Corp2_Raw_NM.Functions.CorpForProfitInd(l.corp_type);
				self.corp_foreign_domestic_ind  	:= Corp2_Raw_NM.Functions.CorpForeignDomesticInd(l.corp_type);
				self.corp_forgn_date							:= if(Corp2_Raw_NM.Functions.CorpForeignDomesticInd(l.corp_type) in ['F'],
																								map(length(l.corp_doi_date) in [8] 	=> Corp2_Mapping.fValidateDate(l.corp_doi_date,'MM-DD-YY').PastDate,
																										length(l.corp_doi_date) in [10] => Corp2_Mapping.fValidateDate(l.corp_doi_date,'CCYY-MM-DD').PastDate,
																										''
																									 ),
																								''
																							 );				
				self.corp_ra_full_name            := Corp2_Raw_NM.Functions.CorpRAFullName(state_origin,state_desc,l.corp_agent_name);
				self.corp_ra_address_type_cd			:= if(Corp2_Mapping.fAddressExists(state_origin,state_desc,l.corp_street1,'',l.corp_city,l.corp_state,l.corp_zip).ifAddressExists,'R','');				
				self.corp_ra_address_type_desc		:= if(Corp2_Mapping.fAddressExists(state_origin,state_desc,l.corp_street1,'',l.corp_city,l.corp_state,l.corp_zip).ifAddressExists,'REGISTERED OFFICE','');
				self.corp_ra_address_line1			 	:= Corp2_Mapping.fCleanAddress(state_origin,state_desc,l.corp_street1,'',l.corp_city,l.corp_state,l.corp_zip,l.corp_country).AddressLine1;
				self.corp_ra_address_line2			 	:= Corp2_Mapping.fCleanAddress(state_origin,state_desc,l.corp_street1,'',l.corp_city,l.corp_state,l.corp_zip,l.corp_country).AddressLine2;
				self.corp_ra_address_line3				:= Corp2_Mapping.fCleanAddress(state_origin,state_desc,l.corp_street1,'',l.corp_city,l.corp_state,l.corp_zip,l.corp_country).AddressLine3;
				self.ra_prep_addr_line1						:= Corp2_Mapping.fCleanAddress(state_origin,state_desc,l.corp_street1,'',l.corp_city,l.corp_state,l.corp_zip,l.corp_country).PrepAddrLine1;
				self.ra_prep_addr_last_line				:= Corp2_Mapping.fCleanAddress(state_origin,state_desc,l.corp_street1,'',l.corp_city,l.corp_state,l.corp_zip,l.corp_country).PrepAddrLastLine;
				self.recordorigin									:= 'C';
				self													  	:= [];
		End;
		...
		Main_SubmitStats 			  := Scrubs.OrbitProfileStats('Scrubs_Corp2_Mapping_NM_Main','ScrubsAlerts', Main_OrbitStats, version,'Corp_NM_Main').SubmitStats;
		Main_ScrubsWithExamples := Scrubs.OrbitProfileStats('Scrubs_Corp2_Mapping_NM_Main','ScrubsAlerts', Main_OrbitStats, version,'Corp_NM_Main').CompareToProfile_with_Examples;
		...
		Main_FailBuild					:= map( corp2_mapping.fCalcPercent(count(Main_N.ExpandedInFile(corp_key_invalid<>0)),count(Main_N.ExpandedInFile),false) 											> Scrubs_Corp2_Mapping_NM_Main.Threshold_Percent.CORP_KEY										 	=> true,
																		...
																		corp2_mapping.fCalcPercent(count(Main_N.ExpandedInFile(corp_status_cd_invalid<>0)),count(Main_N.ExpandedInFile),false) 								> Scrubs_Corp2_Mapping_NM_Main.Threshold_Percent.CORP_STATUS_CD 						 	=> true,
																		...
																		corp2_mapping.fCalcPercent(count(Main_N.ExpandedInFile(corp_orig_org_structure_cd_invalid<>0)),count(Main_N.ExpandedInFile),false) 		> Scrubs_Corp2_Mapping_NM_Main.Threshold_Percent.CORP_ORIG_ORG_STRUCTURE_CD  	=> true,
																		...
																		corp2_mapping.fCalcPercent(count(Main_N.ExpandedInFile(corp_inc_date='' and corp_forgn_date='')),count(Main_N.ExpandedInFile),false)	> Scrubs_Corp2_Mapping_NM_Main.Threshold_Percent.BLANK_INC_FOREIGN_DATES		 	=> true,

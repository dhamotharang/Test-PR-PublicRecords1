...
*/
/*--INFO-- Small Business for Marketing Batch Service - This service returns Small Business Attributes and Scores for Marketing */
/*--HELP-- 
<pre>
&lt;Batch_In&gt;
  &lt;Row&gt;
...
IMPORT Business_Risk_BIP, Gateway, IESP, MDR, Phones, Royalty, OFAC_XG5;
...
EXPORT SmallBusiness_Marketing_Batch_Service() := FUNCTION
	 ...
	 ************************************************************************ */
	SBA_Results_with_PhoneSources := LNSmallBusiness.SmallBusiness_BIP_Function(searchInputById,
																														DPPA_Purpose,
																														GLBA_Purpose,
																														DataRestrictionMask,
																														DataPermissionMask,
	...
	Final_Results_with_BillingHit :=
			MAP(
				NOT allow_SBA_attrs  => PROJECT( Final_Results_pre, xfm_allow_none(LEFT)),
				/* default: */    PROJECT( Final_Results_pre, xfm_allow_SBA_only(LEFT))
			);

	// Prevent BillingHit from displaying. It'll only cause confusion and questions.
	layout_SBA4Marketing_BatchOutput := LNSmallBusiness.BIP_Layouts.BatchOutput AND NOT BillingHit;
	
	Final_Results := PROJECT( Final_Results_with_BillingHit, layout_SBA4Marketing_BatchOutput );

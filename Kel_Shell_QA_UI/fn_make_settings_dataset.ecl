...
EXPORT fn_make_settings_dataset(PublicRecords_KEL.Interface_BWR_Settings Settings)
	...
	Settings_Dataset := DATASET([
		...
		{IF((INTEGER)Settings.ArchiveDate = 0, 'Archive Date on file :', 'ArchiveDate: '),IF((INTEGER)Settings.ArchiveDate = 0, '0', Settings.ArchiveDate)},
		...
		{IF(Settings.GLBAPurpose = 0, 'GLBA: ', 'GLBA: '), IF(Settings.GLBAPurpose = 0, 'Not Permitted 0', 'Permitted' + (STRING)Settings.GLBAPurpose) },
		{IF(Settings.DPPAPurpose = 0, 'DPPA: ', 'DPPA: '),IF(Settings.DPPAPurpose = 0, 'Not Permitted 0', 'Permitted' + (STRING)Settings.DPPAPurpose)},
		...
		], {STRING Attributes_Setting, STRING Value});

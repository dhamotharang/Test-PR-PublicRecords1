EXPORT	STRING	mWorkunitHandlers(STRING pECLText, STRING pCluster, STRING pQueue)	:=	FUNCTION
		STRING fWUCreateAndUpdate(STRING pECLText)	:=		FUNCTION
            ...
            dsValues := dataset([{'Insurance-branch','RoxieDev'}],rDebugValue);
			rDebugValues :=
			record
			...
			dWUCreateAndUpdateResult	:=	SOAPCALL('http://10.194.90.203:8010' + '/WsWorkunits',//'http://10.193.211.1:8010/WsWorkunits/',
																						 'WUUpdate',
		...
		fWUSubmit(STRING pWUID, STRING pCluster, STRING pQueue)	:= FUNCTION
			...
			dWUSubmitResult	:=	SOAPCALL('http://10.194.90.203:8010' + '/WsWorkunits',//'http://10.193.211.1:8010/WsWorkunits/',
																	 'WUSubmit',
																	 rWUSubmitRequest,

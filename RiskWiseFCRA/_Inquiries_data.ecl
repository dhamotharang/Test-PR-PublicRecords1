...
EXPORT _Inquiries_data(	dataset (doxie.layout_references) bshell_dids, 
  ...
  unsigned1 iType := data_services.data_env.iFCRA;
	...
	with_gong_address := join(with_seq, gong.Key_FCRA_History_Address,
...
with_phone_indicator := join(with_telcordia, Gong.Key_FCRA_Business_Header_Phone_Table_Filtered_V2,
	LEFT.person_q.personal_phone !='' and
...
with_phone_listing := join(phone_indicator_deduped, gong.Key_FCRA_History_Phone, 

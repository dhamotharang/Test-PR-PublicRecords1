...
EXPORT getIndKRIResidency(DueDiligence.Layouts.Indv_Internal indv) := FUNCTION
		...
		usResidencyFlag4 := IF(reportedAge < ut.DaysInNYears(3) AND (indv.registeredVoter = FALSE OR indv.stateVotingSourceAvailable = FALSE), 'T', 'F'); 
		usResidencyFlag3 := IF(reportedAge BETWEEN ut.DaysInNYears(3) AND ut.DaysInNYears(10) AND (indv.registeredVoter = FALSE OR indv.stateVotingSourceAvailable = FALSE), 'T', 'F');   
		usResidencyFlag2 := IF(reportedAge > ut.DaysInNYears(10) AND (indv.registeredVoter = FALSE OR indv.stateVotingSourceAvailable = FALSE), 'T', 'F');  

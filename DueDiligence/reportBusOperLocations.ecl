EXPORT reportBusOperLocations := 'todo';

IMPORT Address, BIPV2, Business_Risk_BIP, LN_PropertyV2, MDR, DueDiligence, UT, iesp, Census_Data;

EXPORT reportBusOperLocations(DATASET(DueDiligence.layouts.Busn_Internal) BusinessInternal, 
 ...
 // ------ define the ChildDataset                                               ------
	...
	BusLiensChildDatasetLayout    := RECORD
	  ...
	  DATASET(iesp.duediligencereport.t_DDRBusinessAddressRisk) BusOperLocationsChild;
	END;
 ...
 // ------ as a 'WHOLE' into the DATASET defined within the PARENT               ------
	// ------                                                                       ------
	iesp.duediligencereport.t_DDRBusinessAddressRisk   FormatTheListOfLIENS(RECORDOF(BusinessLiens_unreleased) le, Integer LienSeq) := TRANSFORM 

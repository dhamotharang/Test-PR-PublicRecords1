IMPORT Business_Risk_BIP, doxie, DueDiligence, dx_header, Header, iesp, Suppress;
...
EXPORT reportIndIdentitySSNVariation(DATASET(DueDiligence.Layouts.Indv_Internal) inData,
    ...
    ssnVariations := NORMALIZE(inData, LEFT.ssnOnFile, TRANSFORM(DueDiligence.LayoutsInternalReport.PersonSSNDeviation,
		...
		getLexIDsBySSNs := JOIN(ssnVariations, dx_header.key_wild_SSN(),
                            ...
                            TRANSFORM(DueDiligence.LayoutsInternalReport.PersonSSNDeviation,
                                      ...
                                      SELF.relation := IF(inquired, Header.relative_titles.num_Subject, 0);
    ...
    uniqueInd := DEDUP(SORT(uniqueSSNsPerInquired(relation <> Header.relative_titles.num_Subject), didTiedToSSN), didTiedToSSN);
    ...
    othersBestData := DueDiligence.getIndInformation(options, mod_access).GetIndividualBestDataWithLexID(others);
    
    addOthersDetails := JOIN(uniqueSSNsPerInquired, othersBestData,
                             LEFT.didTiedToSSN = RIGHT.individual.did,
                             TRANSFORM(DueDiligence.LayoutsInternalReport.PersonSSNDeviation,
                                        
                                        isSelf := LEFT.relation = Header.relative_titles.num_Subject;
    ...
    //mask SSNs
    Suppress.MAC_Mask(addOthersDetails, maskedInputSSNData, maskedInputSSN, '', TRUE, FALSE,,,, ssnMask);
    Suppress.MAC_Mask(maskedInputSSNData, maskedSSNOnFileData, maskedSSNOnFile, '', TRUE, FALSE,,,, ssnMask);  
    Suppress.MAC_Mask(maskedSSNOnFileData, maskedBestSSNData, maskedBestSSN, '', TRUE, FALSE,,,, ssnMask);  
    ...
    addRelationship := JOIN(uniqueLexIDsPerInquired, allAssociations,
                            ...
                            TRANSFORM(DueDiligence.LayoutsInternalReport.PersonSSNDeviation,
                                      ...
                                      isInquired := LEFT.relation = Header.relative_titles.num_Subject;
                                      
                                      SELF.relation := IF(isInquired, LEFT.relation, RIGHT.relationship);
                                      SELF.relationText := IF(isInquired, LEFT.relationText, Header.relative_titles.fn_get_str_title(RIGHT.relationship));
                                      SELF := LEFT;),
                            LEFT OUTER,
                            ATMOST(1));
    ...
    finalResults := ROLLUP(SORT(convertLayout, seq, inquiredDID, didTiedToSSN),
                            ...
                            TRANSFORM(DueDiligence.LayoutsInternalReport.PersonSSNDeviation,
                                      ...
                                      keepData := IF(LEFT.relation = Header.relative_titles.num_Subject, LEFT, RIGHT);

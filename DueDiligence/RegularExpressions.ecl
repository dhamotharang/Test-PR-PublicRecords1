...
EXPORT RegularExpressions(STRING textToSearch, STRING offenseScore) := MODULE
		...
		SHARED EXPRESSION_EXCLUDE_RELATED_TRAFFIC := '(?!.*(ONE-WAY|SIGNAL|OFFENSE|STOP|HABIT|LANE|PEDES|MOPED|DEVICE|SPEED|BLOCK|CHAPTER|IMPEDE|(NON)?MOVING|OBSTRUCTING|DRUNK))';
		SHARED EXPRESSION_SMUGGLING := '^(?=.*(SMUG))' + EXPRESSION_EXCLUDE_RELATED_TRAFFIC + '.*'; 
		SHARED EXPRESSION_TRAFFICKING := '^(?=.*(TRAF))' + EXPRESSION_EXCLUDE_RELATED_TRAFFIC + '.*'; ; 

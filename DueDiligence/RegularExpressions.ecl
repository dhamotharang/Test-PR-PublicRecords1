...
EXPORT RegularExpressions(STRING textToSearch) := MODULE
		...
		SHARED EXPRESSION_TRAFFICKING_SMUGGLING := '^(?=.*(TRAF|SMUG))(?!.*(ONE-WAY|SIGNAL|OFFENSE|STOP|HABIT|LANE|PEDES|MOPED|DEVICE|SPEED|BLOCK|CHAPTER|IMPEDE|(NON)?MOVING|OBSTRUCTING|DRUNK)).*'; 

IMPORT Marriage_Divorce_v2, ConsumerDisclosure, doxie, FCRA, FFD, Suppress;

EXPORT RawMarriageDivorce := MODULE

	SHARED MD_party_raw := RECORD(marriage_divorce_v2.layout_mar_div_search) // RECORDOF(marriage_divorce_v2.key_mar_div_id_search)
	END; 
	
	SHARED MD_raw := RECORD(marriage_divorce_v2.layout_mar_div_base_slim) // RECORDOF(marriage_divorce_v2.key_mar_div_id_main)
	END; 
	
	SHARED MD_party_rawrec := RECORD(MD_party_raw)
		ConsumerDisclosure.Layouts.InternalMetadata;
	END;
	
	SHARED MD_rawrec := RECORD(MD_raw)
		ConsumerDisclosure.Layouts.InternalMetadata;
	END;
	
	EXPORT MD_party_out := RECORD(ConsumerDisclosure.Layouts.Metadata)
		MD_party_raw;
	END;

	EXPORT MD_main_out := RECORD(ConsumerDisclosure.Layouts.Metadata)
		MD_raw;
	END;

	EXPORT MD_out := RECORD
		UNSIGNED6 record_id;
		DATASET(MD_main_out) Main;
		DATASET(MD_party_out) Parties;
	END;
 ...
 FUNCTION
	...
	id_recs := JOIN(in_dids,marriage_divorce_v2.key_mar_div_did(isFCRA),
			            KEYED(left.did=right.did),
									TRANSFORM(RIGHT),
									LIMIT(0),KEEP(ConsumerDisclosure.Constants.Limits.MaxMDPerDID)); 
	...
	main_recs_filt:= main_recs_all(
		...
		MD_rawrec xformStatements( MD_rawrec l,	FFD.Layouts.PersonContextBatchSlim r ) := TRANSFORM,
		...
		main_recs_out := PROJECT(main_recs_final_ds, TRANSFORM(MD_main_out,			
		...
		MD_party_suppressed_recs := MD_party_flag_recs(compliance_flags.isSuppressed);		
		...
		MD_party_payload_recs := JOIN(id_recs, marriage_divorce_v2.key_mar_div_id_search(IsFCRA),
													...
													TRANSFORM(MD_party_rawrec,
																		...
																		SELF := RIGHT,
																		SELF := []),
																		LIMIT(0), KEEP(ConsumerDisclosure.Constants.Limits.MaxMDParties));
		...
		MD_party_rawrec xformPartyStatements(MD_party_rawrec l,	FFD.Layouts.PersonContextBatchSlim r) := TRANSFORM,
			SKIP(~ShowDisputedRecords AND r.isDisputed)
					SELF.statement_ids := r.StatementIDs;
					SELF.compliance_flags.IsDisputed := r.isDisputed;
					SELF := l;
		...
		MD_party_rawrec xformNSS(MD_party_rawrec L) := TRANSFORM
      ...
      SELF := IF (IsSubject, L);
		...
		md_out xfRollMain(MD_main_out le, 
												DATASET(MD_main_out) main_rows) := 
		TRANSFORM
		...
		md_main_rolled := ROLLUP(GROUP(SORT(main_recs_out, record_id), record_id), 
																	GROUP, xfRollMain(LEFT, ROWS(LEFT)));	

		md_out xfAddParties(md_out le, DATASET(MD_party_out) party_rows) := 
		TRANSFORM
			SELF.Parties := party_rows;
			SELF := le;
		END;
		...
		recs_out := DENORMALIZE(md_main_rolled, party_recs_out, 
			...
			IF(ConsumerDisclosure.Debug,OUTPUT(id_recs, NAMED('MD_ids')));
			IF(ConsumerDisclosure.Debug,OUTPUT(MD_party_suppressed_recs, NAMED('MD_party_suppressed_recs')));
			IF(ConsumerDisclosure.Debug,OUTPUT(MD_party_override_recs, NAMED('MD_party_override_recs')));
			IF(ConsumerDisclosure.Debug,OUTPUT(MD_party_payload_recs, NAMED('MD_party_payload_recs')));
			IF(ConsumerDisclosure.Debug,OUTPUT(main_suppressed_recs, NAMED('MD_main_suppressed_recs')));
			IF(ConsumerDisclosure.Debug,OUTPUT(main_override_recs, NAMED('MD_main_override_recs')));
			IF(ConsumerDisclosure.Debug,OUTPUT(main_payload_recs, NAMED('MD_main_payload_recs')));
			IF(ConsumerDisclosure.Debug,OUTPUT(recs_out, NAMED('MD_combined_recs')));

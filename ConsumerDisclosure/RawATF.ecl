...
EXPORT RawATF := MODULE
  //Alcohol, Tobacco & Firearms (ATF) - Firearms and Explosives
	...
	EXPORT ATF_raw := RECORD
		ATF.ATF_Layout_SearchFile AND NOT [persistent_record_id, lf];
	END; 
	
	EXPORT ATF_rawrec := RECORD(ATF_raw)
		ConsumerDisclosure.Layouts.InternalMetadata;
	END;
	
	EXPORT ATF_out := RECORD(ConsumerDisclosure.Layouts.Metadata)
		ATF_raw;
	END;
  ...
  FUNCTION
	...
	ATF_rawrec xformStatements( ATF_rawrec l,	FFD.Layouts.PersonContextBatchSlim r ) := TRANSFORM,
	...
	ATF_rawrec xformNSS(ATF_rawrec L) := TRANSFORM
	...
	IF(ConsumerDisclosure.Debug, OUTPUT(id_recs, NAMED('ATF_ids')));
	IF(ConsumerDisclosure.Debug, OUTPUT(suppressed_recs, NAMED('ATF_suppressed_recs')));
	IF(ConsumerDisclosure.Debug, OUTPUT(override_recs, NAMED('ATF_override_recs')));
	IF(ConsumerDisclosure.Debug, OUTPUT(main_recs, NAMED('ATF_main_recs')));
	IF(ConsumerDisclosure.Debug, OUTPUT(recs_out, NAMED('ATF_combined_recs')));

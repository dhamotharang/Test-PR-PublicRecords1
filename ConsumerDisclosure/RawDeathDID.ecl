...
EXPORT RawDeathDID := MODULE
	...
	EXPORT DeathDID_raw := RECORD
		UNSIGNED6 l_did;
		Header.Layout_Did_Death_MasterV3;
		STRING18 county_name := '';
	END;

	EXPORT DeathDID_rawrec := RECORD(DeathDID_raw)
		ConsumerDisclosure.Layouts.InternalMetadata;
	END;
	
	EXPORT DeathDID_out := RECORD(ConsumerDisclosure.Layouts.Metadata)
		DeathDID_raw;
	END;
	...
	FUNCTION
		...
		main_recs := JOIN(in_dids, Doxie.key_death_masterV2_ssa_DID_fcra,
													...
													TRANSFORM(DeathDID_rawrec,
																		...
																		SELF := LEFT,
																		SELF := []),
																		LIMIT(0), KEEP(ConsumerDisclosure.Constants.Limits.MaxDeathPerDID));
		...
		DeathDID_rawrec xformStatements( DeathDID_rawrec l,	FFD.Layouts.PersonContextBatchSlim r ) := TRANSFORM,
		...
		recs_out := PROJECT(recs_final_ds, TRANSFORM(DeathDID_out,			
		...
		IF(ConsumerDisclosure.Debug,OUTPUT(suppressed_recs, NAMED('DeathDID_suppressed_recs')));
		IF(ConsumerDisclosure.Debug,OUTPUT(override_recs, NAMED('DeathDID_override_recs')));
		IF(ConsumerDisclosure.Debug,OUTPUT(main_recs, NAMED('DeathDID_main_recs')));
		IF(ConsumerDisclosure.Debug,OUTPUT(recs_out, NAMED('DeathDID_recs')));

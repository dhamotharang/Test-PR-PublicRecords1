...
EXPORT RawLiens := MODULE
	...
	EXPORT layout_liens_party_out := RECORD(ConsumerDisclosure.Layouts.Metadata)
		layout_liens_party_raw;
	END;

	EXPORT layout_liens_main_out := RECORD(ConsumerDisclosure.Layouts.Metadata)
 ...
 FUNCTION
	...
	id_recs := JOIN(in_dids,liensv2.key_liens_DID_FCRA,
			            KEYED(left.did=right.did),
									TRANSFORM(RIGHT),
									LIMIT(0),KEEP(ConsumerDisclosure.Constants.Limits.MaxLiensPerDID)); 
	...
	main_payload_recs := JOIN(id_recs,liensv2.key_liens_main_id_FCRA,
												...
												TRANSFORM(layout_liens_main_rawrec,
																	...
																	SELF := LEFT,
																	SELF := []),
																	LIMIT(0), KEEP(ConsumerDisclosure.Constants.Limits.MaxLiens));
	...
	main_recs_filt:= main_recs_all(
		...
		ConsumerDisclosure.Functions.FCRADateIsOk(orig_filing_date, todaysdate)
		);   

...
EXPORT RecordsTier2(UNSIGNED6 in_lexid, $.IParam.IReportParam in_mod) := FUNCTION
  ...
  exp_recs := Experian_CRDB.Key_LinkIDs.kFetch(bus_link_ids,, BIPV2.IDconstants.Fetch_Level_SELEID)(did = in_lexid);
  bip_recs := BIPV2.Key_BH_Linking_Ids.kFetch2(bus_link_ids, BIPV2.IDconstants.Fetch_Level_SELEID)(contact_did = in_lexid);
  ...
  recs := 
    ...
    + $.MAC.GetCollectionFromRaw(exp_recs, in_mod, $.Constants.Collection.EXPERIAN_CRDB, experian_crdb.Key_LinkIDs.Key,, dt_last_seen)

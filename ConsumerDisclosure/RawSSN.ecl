...
EXPORT RawSSN := MODULE
	...
	EXPORT layout_ssn_out := RECORD(ConsumerDisclosure.Layouts.Metadata)
		layout_SSN_raw;
		BOOLEAN isInquirySource := FALSE;
		BOOLEAN isHeaderSource := FALSE;
	END;

	EXPORT GetData(DATASET(ConsumerDisclosure.Layouts.ssn_rec) in_ssn,
	...
	FUNCTION
		...
		main_recs := JOIN(ssn_deduped, Risk_Indicators.Key_SSN_Table_v4_filtered_FCRA,
													...
													TRANSFORM(layout_SSN_rawrec,
																		...
																		SELF := LEFT,
																		SELF := []),
																		LIMIT(0), KEEP(ConsumerDisclosure.Constants.Limits.MaxSSN));

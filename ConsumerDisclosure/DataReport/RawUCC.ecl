/*
  ...
  ***********************************************************************************************************
*/
IMPORT $, ConsumerDisclosure, doxie, FCRA, FFD, UCCV2, BIPV2, MDR, STD, UT, Suppress;
...
EXPORT RawUCC := MODULE
  ...
  EXPORT layout_UCC_main_out := RECORD($.Layouts.Metadata)
    layout_UCC_main_raw;
  END;

  EXPORT layout_UCC_party_out := RECORD($.Layouts.Metadata)
  ...
  FUNCTION
    ...
    id_recs := JOIN(in_dids,UCCV2.key_did_w_Type(IsFCRA),
                  ...
                  TRANSFORM(RIGHT),
                  LIMIT(0),KEEP($.Constants.Limits.MaxUCCPerDID)); 
    ...
    party_payload_recs := JOIN(id_recs_ddp, UCCV2.key_rmsid_party(isFCRA),
                          KEYED(LEFT.tmsid = RIGHT.tmsid AND LEFT.rmsid = RIGHT.rmsid),
                           TRANSFORM(layout_UCC_party_rawrec,
                                    ...
                                    SELF := LEFT,
                                    SELF := []),
                                    LIMIT(0), KEEP($.Constants.Limits.MaxUCCPerTmsid));

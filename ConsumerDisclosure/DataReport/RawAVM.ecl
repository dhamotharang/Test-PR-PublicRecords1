/*
  ...
  ***********************************************************************************************************
*/
IMPORT $, FCRA, FFD, AVM_v2, ConsumerDisclosure, UT,
...
layout_avm_medians_disclosure_out := RECORD($.Layouts.Metadata)
...
layout_avm_medians_raw_out := RECORD($.Layouts.Metadata)
...
EXPORT RawAVM := MODULE
  ...
  EXPORT layout_avm_address_out := RECORD($.Layouts.Metadata)
    layout_avm_base_raw;
    layout_avm_valuation SnapshotData;
  END;
  ...
  EXPORT GetAVMMediansData(DATASET($.Layouts.address_rec) in_addresses,
    ...
    flag_recs :=
      JOIN(all_flags, FCRA.Key_Override_AVM.avm_medians,
        ...
        TRANSFORM(layout_avm_medians_disclosure_internal,
          ...
          SELF.compliance_flags.isSuppressed := LEFT.isSuppressed AND (RIGHT.geolink='' OR LEFT.record_id=RIGHT.geolink);
    ...
    TRANSFORM
      SELF.raw_medians := PROJECT(rt, TRANSFORM(layout_avm_medians_raw_out,
                        SELF.Metadata := $.Functions.GetMetadataESDL(
                                                ROW({FALSE,FALSE,FALSE,FALSE},$.Layouts.ComplianceFlags),
                                                ROW({LEFT.raw_payload.fips_geo_12,'','',''},$.Layouts.RecordIds),
  ...
  EXPORT GetAVMAddressData(DATASET($.Layouts.address_rec) in_addresses,
                DATASET (FFD.Layouts.flag_ext_rec) flag_file,
                DATASET (FFD.Layouts.PersonContextBatchSlim) slim_pc_recs,
                $.IParams.IParam in_mod) :=
  FUNCTION
    ...
    main_recs := JOIN(in_addresses, AVM_V2.Key_AVM_Address_FCRA,
                        KEYED(LEFT.prim_name = RIGHT.prim_name AND
                          ...
                          TRANSFORM(layout_avm_base_rawrec,
                                    ...
                                    SELF := LEFT,
                                    SELF := []),
                                    LIMIT(0), KEEP($.Constants.Limits.MaxAVMPerAddress));

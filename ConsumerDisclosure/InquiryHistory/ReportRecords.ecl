IMPORT $, doxie, FFD, FCRA, ut, iesp, Gateway, STD;
...
FUNCTION
 ...
 // prepare results for output - we need to add status/message per input subject
  $.Layouts.inquiry_history_out xform_indv(doxie.layout_references L,
            ...
            StatusCode := IF(EXISTS(R), $.Constants.StatusCodes.ResultsFound,
                                           $.Constants.StatusCodes.NoResultsFound);
            SELF.UniqueId := L.did;
            SELF.SearchStatus := StatusCode;
            SELF.Message := $.Constants.GetStatusMessage(StatusCode);
  ...
  isPCSoapFail := ~in_mod.SkipPersonContextCall AND pc_resp._Header.Status = $.Constants.StatusCodes.SOAPError; 
  isIHSoapFail := delta_IH_results._Header.Status = $.Constants.StatusCodes.SOAPError;
  ...
  $.Layouts.inquiry_history_out xform_onfail() := TRANSFORM 
      ...
      SELF.Message := $.Constants.GetStatusMessage(_err_code);
      SELF := [];
  END;

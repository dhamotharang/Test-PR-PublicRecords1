...
EXPORT RawHuntingFishing := MODULE
	...
	EXPORT HuntFish_raw := RECORD(emerges.layout_hunters_out)
		UNSIGNED6 rid;
	END;
		//should we add mail_county_name column to the layout?
	
	EXPORT HuntFish_rawrec := RECORD(HuntFish_raw)
		ConsumerDisclosure.Layouts.InternalMetadata;
	END;
	
	EXPORT HuntFish_out := RECORD(ConsumerDisclosure.Layouts.Metadata)
		HuntFish_raw;
	END;
 ...
 FUNCTION
	...
	recs_filt:= recs_all(
		...
		HuntFish_rawrec xformStatements( HuntFish_rawrec l,	FFD.Layouts.PersonContextBatchSlim r ) := TRANSFORM,
		...
		recs_out := PROJECT(recs_final_ds, TRANSFORM(HuntFish_out,			
			...
			IF(ConsumerDisclosure.Debug,OUTPUT(id_recs, NAMED('huntfish_ids')));
			IF(ConsumerDisclosure.Debug,OUTPUT(suppressed_recs, NAMED('huntfish_suppressed_recs')));
			IF(ConsumerDisclosure.Debug,OUTPUT(override_recs, NAMED('huntfish_override_recs')));
			IF(ConsumerDisclosure.Debug,OUTPUT(main_recs, NAMED('huntfish_main_recs')));
		RETURN recs_out;
	END;

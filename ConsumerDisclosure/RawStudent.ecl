...
EXPORT RawStudent := MODULE
	...
	SHARED ASL_raw := RECORD(American_student_list.layout_american_student_base_v2) //RECORDOF(American_Student_List.key_DID_FCRA)
		UNSIGNED6 l_did;	
	END;

	SHARED AlloyMS_raw := AlloyMedia_student_list.layouts.layout_base; //RECORDOF(AlloyMedia_student_list.key_DID_FCRA);

	SHARED ASL_rawrec := RECORD(ASL_raw)
		ConsumerDisclosure.Layouts.InternalMetadata;
	END;
	
	SHARED AlloyMS_rawrec := RECORD(AlloyMS_raw)
		ConsumerDisclosure.Layouts.InternalMetadata;
	END;
	
	EXPORT ASL_out := RECORD(ConsumerDisclosure.Layouts.Metadata)
		ASL_raw;
	END;

	EXPORT AlloyMS_out := RECORD(ConsumerDisclosure.Layouts.Metadata)
		AlloyMS_raw;
	END;
	...
	FUNCTION
		...
		main_recs := JOIN(in_dids, American_Student_List.Key_DID_FCRA,
													...
													TRANSFORM(ASL_rawrec,
																		...
																		SELF := LEFT,
																		SELF := []),
																		LIMIT(0), KEEP(ConsumerDisclosure.Constants.Limits.MaxStudentPerDID));
		...
		ASL_rawrec xformStatements(ASL_rawrec l,	FFD.Layouts.PersonContextBatchSlim r) := TRANSFORM,
		...
		recs_out := PROJECT(recs_final_ds, TRANSFORM(ASL_out,			
		...
		IF(ConsumerDisclosure.Debug,OUTPUT(suppressed_recs, NAMED('ASL_suppressed_recs')));
		IF(ConsumerDisclosure.Debug,OUTPUT(override_recs, NAMED('ASL_override_recs')));
		IF(ConsumerDisclosure.Debug,OUTPUT(main_recs, NAMED('ASL_main_recs')));
		IF(ConsumerDisclosure.Debug,OUTPUT(recs_out, NAMED('AmericanStudentList_recs')));
	...
	BOOLEAN showDisputedRecords := in_mod.ReturnDisputed;
		...
		AlloyMS_rawrec xformStatements(AlloyMS_rawrec l,	FFD.Layouts.PersonContextBatchSlim r) := TRANSFORM,
			SKIP(~ShowDisputedRecords AND r.isDisputed)
					SELF.statement_ids := r.StatementIDs;
					SELF.compliance_flags.IsDisputed := r.isDisputed;
					SELF := l;
		...
		recs_out := PROJECT(recs_final_ds, TRANSFORM(AlloyMS_out,			
												SELF.Metadata := ConsumerDisclosure.Functions.GetMetadataESDL(LEFT.compliance_flags,
																								LEFT.record_ids,
																								LEFT.statement_ids,
																								LEFT.subject_did, 
		...
		IF(ConsumerDisclosure.Debug,OUTPUT(suppressed_recs, NAMED('AlloyMS_suppressed_recs')));
		IF(ConsumerDisclosure.Debug,OUTPUT(override_recs, NAMED('AlloyMS_override_recs')));
		IF(ConsumerDisclosure.Debug,OUTPUT(main_recs, NAMED('AlloyMS_main_recs')));
		IF(ConsumerDisclosure.Debug,OUTPUT(recs_out, NAMED('AlloyMediaStudent_recs')));

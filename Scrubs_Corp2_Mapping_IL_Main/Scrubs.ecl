...
EXPORT FromNone(DATASET(corp2_mapping.layoutscommon.main) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_orig_bus_type_desc_Invalid := Fields.InValid_corp_orig_bus_type_desc((SALT34.StrType)le.corp_orig_bus_type_desc,(SALT34.StrType)le.recordorigin);
    ...
    SELF.internalfield1_Invalid := Fields.InValid_internalfield1((SALT34.StrType)le.internalfield1,(SALT34.StrType)le.recordorigin);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),corp2_mapping.layoutscommon.main);
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_ln_name_type_desc_Invalid << 22 ) + ( le.corp_status_cd_Invalid << 23 ) + ( le.corp_status_date_Invalid << 25 ) + ( le.corp_inc_state_Invalid << 27 ) + ( le.corp_inc_date_Invalid << 28 ) + ( le.corp_term_exist_cd_Invalid << 30 ) + ( le.corp_term_exist_exp_Invalid << 31 ) + ( le.corp_term_exist_desc_Invalid << 33 ) + ( le.corp_foreign_domestic_ind_Invalid << 34 ) + ( le.corp_forgn_state_cd_Invalid << 35 ) + ( le.corp_forgn_state_desc_Invalid << 36 ) + ( le.corp_forgn_date_Invalid << 37 ) + ( le.corp_orig_org_structure_desc_Invalid << 39 ) + ( le.corp_for_profit_ind_Invalid << 40 ) + ( le.corp_orig_bus_type_desc_Invalid << 41 ) + ( le.corp_ra_effective_date_Invalid << 42 ) + ( le.corp_agent_county_Invalid << 44 ) + ( le.corp_country_of_formation_Invalid << 45 ) + ( le.corp_merger_date_Invalid << 46 ) + ( le.recordorigin_Invalid << 48 ) + ( le.internalfield1_Invalid << 49 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_status_cd_Invalid := (le.ScrubsBits1 >> 23) & 3;
    ...
    SELF.corp_orig_bus_type_desc_Invalid := (le.ScrubsBits1 >> 41) & 1;
    SELF.corp_ra_effective_date_Invalid := (le.ScrubsBits1 >> 42) & 3;
    SELF.corp_agent_county_Invalid := (le.ScrubsBits1 >> 44) & 1;
    SELF.corp_country_of_formation_Invalid := (le.ScrubsBits1 >> 45) & 1;
    SELF.corp_merger_date_Invalid := (le.ScrubsBits1 >> 46) & 3;
    SELF.recordorigin_Invalid := (le.ScrubsBits1 >> 48) & 1;
    SELF.internalfield1_Invalid := (le.ScrubsBits1 >> 49) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_orig_bus_type_desc_CUSTOM_ErrorCount := COUNT(GROUP,h.corp_orig_bus_type_desc_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_orig_bus_type_desc_Invalid,'CUSTOM','UNKNOWN')
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_status_date:invalid_future_date:ALLOW','corp_status_date:invalid_future_date:CUSTOM'
          ...
          ,'corp_orig_org_structure_desc:invalid_orig_org_structure_desc:CUSTOM'
          ,'corp_for_profit_ind:invalid_for_profit_ind:ENUM'
          ,'corp_orig_bus_type_desc:invalid_orig_bus_type_desc:CUSTOM'
          ...
          ,'corp_merger_date:invalid_future_date:ALLOW','corp_merger_date:invalid_future_date:CUSTOM'
          ,'recordorigin:invalid_recordorigin:ENUM'
          ,'internalfield1:invalid_corp_types:CUSTOM','UNKNOWN');
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_orig_bus_type_desc_CUSTOM_ErrorCount
    ...
    END;
    SummaryInfo := NORMALIZE(SummaryStats,50,Into(LEFT,COUNTER));
    orb_r := RECORD
      AllErrors.Src;

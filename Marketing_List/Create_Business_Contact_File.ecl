...
functionmacro
  ...
  ds_fix_hierarchy_with_lexid     := ds_fix_hierarchy(lexid != 0);
  ds_fix_hierarchy_without_lexid  := ds_fix_hierarchy(lexid  = 0);

  // -----------------------------------------------------------------------------------
  // -- append address for lexid records
  ds_lexids_prep        := project(table(ds_fix_hierarchy(lexid != 0) ,{lexid} ,lexid ,merge) ,transform(doxie.layout_references  ,self.did := left.lexid));
  ...
  ds_concat_address := project(ds_append_county_info2  ,Marketing_List.Layouts.business_contact) + ds_fix_hierarchy_without_address;
  ...
  ds_result := ds_blank_address_fields;
  ...
  output_debug := parallel(
   ...
   ,output(choosen(ds_prep_with_lexid               (count(pSampleProxids) = 0 or proxid in pSampleProxids ),300) ,named('Create_Business_Contact_File_ds_prep_with_lexid'                ),all)
   ,output(choosen(ds_prep_without_lexid            (count(pSampleProxids) = 0 or proxid in pSampleProxids ),300) ,named('Create_Business_Contact_File_ds_prep_without_lexid'             ),all)
   ,output(choosen(ds_lexid_valid_segs_no_minors    (count(pSampleProxids) = 0 or proxid in pSampleProxids ),300) ,named('Create_Business_Contact_File_ds_lexid_valid_segs_no_minors'     ),all)
   ,output(choosen(ds_concat_lexid_segs             (count(pSampleProxids) = 0 or proxid in pSampleProxids ),300) ,named('Create_Business_Contact_File_ds_concat_lexid_segs'              ),all)
   ,output(choosen(ds_validate_contact_name         (count(pSampleProxids) = 0 or proxid in pSampleProxids ),300) ,named('Create_Business_Contact_File_ds_validate_contact_name'          ),all)
   ...
   ,output(choosen(ds_fix_hierarchy_with_lexid      (count(pSampleProxids) = 0 or proxid in pSampleProxids ),300) ,named('Create_Business_Contact_File_ds_fix_hierarchy_with_lexid'       ),all)
   ,output(choosen(ds_fix_hierarchy_without_lexid   (count(pSampleProxids) = 0 or proxid in pSampleProxids ),300) ,named('Create_Business_Contact_File_ds_fix_hierarchy_without_lexid'    ),all)
   ...
   ,output(choosen(ds_get_lexid_addresses           (count(pSampleProxids) = 0 or proxid in pSampleProxids ),300) ,named('Create_Business_Contact_File_ds_get_lexid_addresses'            ),all)
   ,output(choosen(ds_concat_lexids                 (count(pSampleProxids) = 0 or proxid in pSampleProxids ),300) ,named('Create_Business_Contact_File_ds_concat_lexids'                  ),all)
   ...
   ,output(choosen(ds_append_county_info1           (count(pSampleProxids) = 0 or proxid in pSampleProxids ),300) ,named('Create_Business_Contact_File_ds_append_county_info1'            ),all)
   ,output(choosen(ds_append_county_info2           (count(pSampleProxids) = 0 or proxid in pSampleProxids ),300) ,named('Create_Business_Contact_File_ds_append_county_info2'            ),all)

   ,output(choosen(ds_concat_address                (count(pSampleProxids) = 0 or proxid in pSampleProxids ),300) ,named('Create_Business_Contact_File_ds_concat_address'                 ),all)
   ,output(choosen(ds_blank_address_fields          (count(pSampleProxids) = 0 or proxid in pSampleProxids ),300) ,named('Create_Business_Contact_File_ds_blank_address_fields'           ),all)

   ,output(choosen(ds_result                        (count(pSampleProxids) = 0 or proxid in pSampleProxids ),300) ,named('Create_Business_Contact_File_ds_return_result'                  ),all)

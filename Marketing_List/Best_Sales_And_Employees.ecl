...
*/
  ...
  ds_dca_table       := table(ds_dca_prep      ,{pID ,dt_last_seen,unsigned number_of_employees := max(group,(unsigned)emp_num                ) ,unsigned annual_revenue := max(group,(unsigned)sales                )   ,string2 source := mdr.sourcetools.src_DCA                  } ,pID ,dt_last_seen,merge) : persist('~persist::Marketing_List::Get_Sales_And_Employees::ds_dca_table'      );
  ds_eq_biz_table    := table(ds_eq_biz_prep   ,{pID ,dt_last_seen,unsigned number_of_employees := max(group,(unsigned)emp_num                ) ,unsigned annual_revenue := max(group,(unsigned)efx_corpamount * 1000)   ,string2 source := mdr.sourcetools.src_Equifax_Business_Data} ,pID ,dt_last_seen,merge) : persist('~persist::Marketing_List::Get_Sales_And_Employees::ds_eq_biz_table'   );
  ds_oshair_table    := table(ds_oshair_prep   ,{pID ,dt_last_seen,unsigned number_of_employees := max(group,(unsigned)Number_In_Establishment) ,unsigned annual_revenue := 0                                            ,string2 source := mdr.sourcetools.src_OSHAIR               } ,pID ,dt_last_seen,merge) : persist('~persist::Marketing_List::Get_Sales_And_Employees::ds_oshair_table'   );
  ds_cortera_table   := table(ds_cortera_prep  ,{pID ,dt_last_seen,unsigned number_of_employees := max(group,(unsigned)TOTAL_EMPLOYEES        ) ,unsigned annual_revenue := max(group,(unsigned)TOTAL_SALES          )   ,string2 source := mdr.sourcetools.src_Cortera              } ,pID ,dt_last_seen,merge) : persist('~persist::Marketing_List::Get_Sales_And_Employees::ds_cortera_table'  );
  ds_infutor_table   := table(ds_infutor_prep  ,{pID ,dt_last_seen,unsigned number_of_employees := max(group,(unsigned)number_of_employees    ) ,unsigned annual_revenue := 0                                            ,string2 source := mdr.sourcetools.src_Infutor_NARB         } ,pID ,dt_last_seen,merge) : persist('~persist::Marketing_List::Get_Sales_And_Employees::ds_infutor_table'  );
  ds_accutrend_table := table(ds_accutrend_prep,{pID ,dt_last_seen,unsigned number_of_employees := max(group,(unsigned)emp_size               ) ,unsigned annual_revenue := 0                                            ,string2 source := mdr.sourcetools.src_Business_Registration} ,pID ,dt_last_seen,merge) : persist('~persist::Marketing_List::Get_Sales_And_Employees::ds_accutrend_table');
  ...
  best_emp_seleid   := dedup(sort(distribute(ds_add_sales_ranking(number_of_employees != 0)  ,hash(pID))  ,pID,emp_rank_order           ,-dt_last_seen,-number_of_employees ,local) ,pID,local) : persist('~persist::Marketing_List::Get_Sales_And_Employees::best_emp_seleid'        );
  best_sales_seleid := dedup(sort(distribute(ds_add_sales_ranking(annual_revenue      != 0)  ,hash(pID))  ,pID,annual_revenue_rank_order,-dt_last_seen,-annual_revenue      ,local) ,pID,local) : persist('~persist::Marketing_List::Get_Sales_And_Employees::best_sales_seleid'      );

  ds_out := join(best_emp_seleid  ,best_sales_seleid  ,left.pID = right.pID ,transform(recordof(left)
    ,self.number_of_employees       := left.number_of_employees

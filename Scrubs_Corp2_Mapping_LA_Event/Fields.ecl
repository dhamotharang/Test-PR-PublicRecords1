...
EXPORT SALT30.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_future_date','invalid_charter_nbr','invalid_event_date_type_cd','invalid_event_filing_cd','invalid_alphanumericblank');
EXPORT FieldTypeNum(SALT30.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_corp_vendor' => 2,'invalid_state_origin' => 3,'invalid_mandatory' => 4,'invalid_date' => 5,'invalid_future_date' => 6,'invalid_charter_nbr' => 7,'invalid_event_date_type_cd' => 8,'invalid_event_filing_cd' => 9,'invalid_alphanumericblank' => 10,0);
...
EXPORT MakeFT_invalid_event_date_type_cd(SALT30.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_event_date_type_cd(SALT30.StrType s) := WHICH(((SALT30.StrType) s) NOT IN ['FIL','MER','']);
EXPORT InValidMessageFT_invalid_event_date_type_cd(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInEnum('FIL|MER|'),SALT30.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_event_filing_cd(SALT30.StrType s0) := FUNCTION
  s1 := SALT30.stringfilter(s0,'?0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_event_filing_cd(SALT30.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT30.StringFilter(s,'?0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ '))));
EXPORT InValidMessageFT_invalid_event_filing_cd(UNSIGNED1 wh) := CHOOSE(wh,SALT30.HygieneErrors.NotInChars('?0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ '),SALT30.HygieneErrors.Good);
...
EXPORT FieldNum(SALT30.StrType fn) := CASE(fn,'corp_key' => 1,'corp_supp_key' => 2,'corp_vendor' => 3,'corp_vendor_county' => 4,'corp_vendor_subcode' => 5,'corp_state_origin' => 6,'corp_process_date' => 7,'corp_sos_charter_nbr' => 8,'event_filing_reference_nbr' => 9,'event_amendment_nbr' => 10,'event_filing_date' => 11,'event_date_type_cd' => 12,'event_date_type_desc' => 13,'event_filing_cd' => 14,'event_filing_desc' => 15,'event_corp_nbr' => 16,'event_corp_nbr_cd' => 17,'event_corp_nbr_desc' => 18,'event_roll' => 19,'event_frame' => 20,'event_start' => 21,'event_end' => 22,'event_microfilm_nbr' => 23,'event_desc' => 24,'event_revocation_comment1' => 25,'event_revocation_comment2' => 26,'event_book_number' => 27,'event_page_number' => 28,'event_certification_number' => 29,0);
...
EXPORT Make_event_filing_reference_nbr(SALT30.StrType s0) := MakeFT_invalid_alphanumericblank(s0);
EXPORT InValid_event_filing_reference_nbr(SALT30.StrType s) := InValidFT_invalid_alphanumericblank(s);
...
EXPORT Make_event_filing_date(SALT30.StrType s0) := MakeFT_invalid_date(s0);
EXPORT InValid_event_filing_date(SALT30.StrType s) := InValidFT_invalid_date(s);
EXPORT InValidMessage_event_filing_date(UNSIGNED1 wh) := InValidMessageFT_invalid_date(wh);
 
EXPORT Make_event_date_type_cd(SALT30.StrType s0) := MakeFT_invalid_event_date_type_cd(s0);
EXPORT InValid_event_date_type_cd(SALT30.StrType s) := InValidFT_invalid_event_date_type_cd(s);
...
EXPORT Make_event_filing_cd(SALT30.StrType s0) := MakeFT_invalid_event_filing_cd(s0);
EXPORT InValid_event_filing_cd(SALT30.StrType s) := InValidFT_invalid_event_filing_cd(s);
...
EXPORT Make_event_revocation_comment1(SALT30.StrType s0) := s0;
EXPORT InValid_event_revocation_comment1(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_event_revocation_comment1(UNSIGNED1 wh) := '';
 
EXPORT Make_event_revocation_comment2(SALT30.StrType s0) := s0;
EXPORT InValid_event_revocation_comment2(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_event_revocation_comment2(UNSIGNED1 wh) := '';
 
EXPORT Make_event_book_number(SALT30.StrType s0) := s0;
EXPORT InValid_event_book_number(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_event_book_number(UNSIGNED1 wh) := '';
 
EXPORT Make_event_page_number(SALT30.StrType s0) := s0;
EXPORT InValid_event_page_number(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_event_page_number(UNSIGNED1 wh) := '';
 
EXPORT Make_event_certification_number(SALT30.StrType s0) := s0;
EXPORT InValid_event_certification_number(SALT30.StrType s) := FALSE;
EXPORT InValidMessage_event_certification_number(UNSIGNED1 wh) := '';
// This macro will compute and count field level differences based upon a pivot expression
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT30,Scrubs_Corp2_Mapping_LA_Event;

...
EXPORT FromNone(DATASET(Corp2_Mapping.LayoutsCommon.Events) h) := MODULE
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.event_filing_reference_nbr_Invalid := Fields.InValid_event_filing_reference_nbr((SALT30.StrType)le.event_filing_reference_nbr);
    SELF.event_filing_date_Invalid := Fields.InValid_event_filing_date((SALT30.StrType)le.event_filing_date);
    SELF.event_date_type_cd_Invalid := Fields.InValid_event_date_type_cd((SALT30.StrType)le.event_date_type_cd);
    SELF.event_filing_cd_Invalid := Fields.InValid_event_filing_cd((SALT30.StrType)le.event_filing_cd);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,Into(LEFT)) : independent;
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.corp_key_Invalid << 0 ) + ( le.corp_vendor_Invalid << 2 ) + ( le.corp_state_origin_Invalid << 3 ) + ( le.corp_process_date_Invalid << 4 ) + ( le.corp_sos_charter_nbr_Invalid << 6 ) + ( le.event_filing_reference_nbr_Invalid << 8 ) + ( le.event_filing_date_Invalid << 9 ) + ( le.event_date_type_cd_Invalid << 11 ) + ( le.event_filing_cd_Invalid << 12 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.event_filing_cd_Invalid := (le.ScrubsBits1 >> 12) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    event_filing_cd_ALLOW_ErrorCount := COUNT(GROUP,h.event_filing_cd_Invalid=1);
  END;
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.event_filing_cd_Invalid,'ALLOW','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_sos_charter_nbr','event_filing_reference_nbr','event_filing_date','event_date_type_cd','event_filing_cd','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_alphanumericblank','invalid_date','invalid_event_date_type_cd','invalid_event_filing_cd','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT30.StrType)le.corp_key,(SALT30.StrType)le.corp_vendor,(SALT30.StrType)le.corp_state_origin,(SALT30.StrType)le.corp_process_date,(SALT30.StrType)le.corp_sos_charter_nbr,(SALT30.StrType)le.event_filing_reference_nbr,(SALT30.StrType)le.event_filing_date,(SALT30.StrType)le.event_date_type_cd,(SALT30.StrType)le.event_filing_cd,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT30.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'event_filing_reference_nbr:invalid_alphanumericblank:ALLOW'
          ,'event_filing_date:invalid_date:ALLOW','event_filing_date:invalid_date:CUSTOM'
          ,'event_date_type_cd:invalid_event_date_type_cd:ENUM'
          ,'event_filing_cd:invalid_event_filing_cd:ALLOW','UNKNOWN');
      SELF.ErrorMessage := '';
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.event_filing_cd_ALLOW_ErrorCount,0);
      SELF.rulepcnt := 100 * CHOOSE(c
          ...
          ,le.event_filing_date_ALLOW_ErrorCount,le.event_filing_date_CUSTOM_ErrorCount
          ,le.event_date_type_cd_ENUM_ErrorCount
          ,le.event_filing_cd_ALLOW_ErrorCount,0) / le.TotalCnt + 0.5;
    END;

...
export proc_map_rozlin(string filedate) := function

Layout_Vendor_Rozlin := record
string orig_DebtorID;
...
string orig_Zip;
string orig_Phone;
string orig_ResultCode;
string orig_ContactDate;
end;
...
spray := FileServices.SprayVariable(_control.IPAddress.edata12,   
								   '/hds_2/phones_feedback/sources/customer/rozlin/raw/'+filedate+'/*',,,,,groupName, 								   
								   '~thor_data400::in::phonesfeedback_rozlin::'+filedate+'::raw',,,,true,true,true); 
								  
ds_in    := dataset('~thor_data400::in::phonesfeedback_rozlin::'+filedate+'::raw',
						    Layout_Vendor_Rozlin,csv(quote('"'),heading(1),terminator(['\r\n','\n'])));
								 
checkDS_IN := if(~exists(ds_in),
						fail('Unable to find vendor file on unix. Please investigate.'),
						output('Continuing'));								 
								 
temp_common_layout prepRecs(Layout_Vendor_Rozlin L) := transform	
fullName	:=	trimUpper(trim(l.orig_FirstName,left,right) + ' ' +
													  trim(l.orig_MiddleName,left,right) + ' ' + 
...
self.alt_phone := '';
self.other_info := '';
self.phone_contact_type := getContactType(l.orig_ResultCode);
...
writeFormat := output(flippedRecs,,'~thor_data400::in::phonesfeedback::rozlin_formatted::'+filedate,__COMPRESSED__,overwrite);

despray := lib_fileservices.fileservices.Despray('~thor_data400::in::phonesfeedback::rozlin_formatted::'+filedate,_control.IPAddress.edata12,
 									'/hds_2/phones_feedback/sources/customer/rozlin/clean/rozlin_'+filedate+'_'+hhmmss+'.txt',,,,TRUE);

deleteLogicals := sequential(FileServices.DeleteLogicalFile('~thor_data400::in::phonesfeedback_rozlin::'+filedate+'::raw'),
							 FileServices.DeleteLogicalFile('~thor_data400::in::phonesfeedback::rozlin_formatted::'+filedate));

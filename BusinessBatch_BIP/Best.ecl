...
EXPORT Best := MODULE
 ...
 rec_seleBestSlimFein := Record
  ...
  //
  tds_parents_all := BIPV2_Best.Key_LinkIds.kfetch2(ds_parentsIdsForBestKfetch,
                      BIPV2.IDconstants.Fetch_Level_PROXID,,,FALSE);

  // Use the marketing kfetch to get the marketing-allowed version of the data (this will effectively filter experian as well)
  tds_parents_mark := BIPV2_Best.Key_LinkIds.kfetch2Marketing(ds_parentsIdsForBestKfetch,
                      BIPV2.IDconstants.Fetch_Level_PROXID,,,FALSE);

  // JIRA RR-10621: Conditionallly remove Experian only records
  tds_parents_raw := IF( inMod.ExcludeExperian,
                    Business_Risk_BIP.fnMac_ExcludeExperian_ChildDataset(tds_parents_all, company_name[1].sources, source),
                    tds_parents_all );

  // optionally apply marketing restriction - note marketing-restriction will include experian restriction
  tds_parents := IF (inMod.ExcludeMarketing, tds_parents_mark, tds_parents_raw);
  ...
  ds_seleBestIds := PROJECT(ds_seleBest, TRANSFORM(
  ...
  ds_seleBestRaw := bipv2_best.Key_LinkIds.kfetch2(ds_seleBestIds);
  ds_seleBestMark := bipv2_best.Key_LinkIds.kfetch2Marketing(ds_seleBestIds);

  // optionally use marketing-restricted data here
  ds_seleBestData := IF (inMod.ExcludeMarketing, ds_seleBestMark, ds_seleBestRaw);

  CompanyIncorporationDate := PROJECT(ds_seleBestData(proxid = 0),

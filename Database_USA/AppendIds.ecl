...
EXPORT AppendIds := MODULE

	//////////////////////////////////////////////////////////////////////////////////////
	// -- FUNCTION: fAppendDid
	// -- Standardizes names
	//////////////////////////////////////////////////////////////////////////////////////
	EXPORT fAppendDid(DATASET(Database_USA.Layouts.Base) pDataset) :=	FUNCTION

		Database_USA.Layouts.Temp.StandardizeInput tAddUniqueId(Database_USA.Layouts.Base L, UNSIGNED8 cnt) :=	TRANSFORM
			SELF.unique_id		:= cnt;
			SELF							:= L;
		END;   
    ...
    Database_USA.Layouts.Temp.DidSlim taddDID(Database_USA.Layouts.Temp.StandardizeInput L) :=  TRANSFORM
	    SELF.fname            := L.fname;
	    SELF.mname            := L.mname;
	    SELF.lname            := L.lname;
	    SELF.name_suffix      := L.name_suffix;
			SELF.prim_range       := L.DB_cons_prim_range;
			SELF.prim_name        := L.DB_cons_prim_name;
			SELF.sec_range        := L.DB_cons_sec_range;
			SELF.zip5             := L.DB_cons_zip;
			SELF.state            := L.DB_cons_st;
      SELF.phone            := L.DB_cons_phone;      
			SELF.did              := 0;
			SELF.did_score		    := 0;
			SELF			            := L;
			SELF							    := [];
    ...
    did_Add.MAC_Match_Flex(
	    ...
	    prim_range,                          //Prim_range  
			...
			phone,                               //Phone
	    did,   			                         //DID
	    Database_USA.Layouts.Temp.DidSlim,   //Output rec layout
    ...
    dGoodDIDs := DISTRIBUTE(postDID(did != 0), unique_id);
		...
		Database_USA.Layouts.Base tAssignDIDs(Database_USA.Layouts.Temp.UniqueId L,Database_USA.Layouts.Temp.DidSlim R) := TRANSFORM
			SELF.did          := R.did;
	...
	// -- Creates the linkIDs
	//////////////////////////////////////////////////////////////////////////////////////
	EXPORT fAppendBIP(DATASET(Database_USA.Layouts.Base) pDataset) :=	FUNCTION
		...
		//////////////////////////////////////////////////////////////////////////////////////
		Database_USA.Layouts.Temp.BIPSlim tSlimForBIPing(Database_USA.Layouts.Temp.StandardizeInput L) := TRANSFORM
			SELF.unique_id		    := L.unique_id;
			SELF.company  		    := L.company_name;
	    SELF.fname            := L.fname;
	    SELF.mname            := L.mname;
	    SELF.lname            := L.lname;
			SELF.prim_range       := L.Phy_prim_range;
			SELF.prim_name        := L.Phy_prim_name;
			SELF.sec_range        := L.Phy_sec_range;
			SELF.city       	    := L.Phy_p_city_name;
			SELF.zip5             := L.Phy_zip;
			SELF.state            := L.Phy_st;
      SELF.phone            := L.phone;     
			SELF.email            := L.email;
    ...
    dSlimInput	:= project(dAddUniqueId(company_name <> '') ,tSlimForBIPing(LEFT));
		...
		Business_Header_SS.MAC_Add_BDID_Flex(
			...
			,fein_not_used                        // fein              
			,bdid_not_used 								        // bdid												
			,Database_USA.Layouts.Temp.BIPSlim    // Output Layout 
    ...
    dBIP_dist           := DISTRIBUTE(dBIPOut(Ultid 	!= 0 OR 
		...
		Database_USA.Layouts.Base tAssignLinkids(Database_USA.Layouts.Temp.StandardizeInput L,Database_USA.Layouts.Temp.BIPSlim R) := TRANSFORM
			SELF.Ultid				:= R.Ultid;
	...
	EXPORT fAll(DATASET(Database_USA.Layouts.Base) pDataset) := FUNCTION
    
		dAppendDid    := fAppendDid(pDataset);
	
		dAppendLinkID	:= fAppendBIP(dAppendDid) : PERSIST(Database_USA.Persist_Names().Append_Ids);

EXPORT FP_Scores_and_Attributes_XML_Macro (roxie_ip,Gateway, Thread, Timeout, Retry, Input_file_name,Output_file_name, records_ToRun):= functionmacro
...
f := IF(no_of_records <= 0, DATASET(ut.foreign_prod +  infile_name, layout_input, CSV(heading(single), quote('"'))),
...
layout_old_acct into_fdInput(f le, INTEGER c) := TRANSFORM
	...
	self.DataRestrictionMask := '0000000000000';  // to allow use of both Equifax and Experian, this is the default value for all legacy scoring products
...
// output(choosen(fdInput, eyeball), named('fpsa_FD_Input'));
...
// output(choosen(resu, eyeball), named('fpsa_roxie_result'));
...
// output(choosen(normed, eyeball), named('fpsa_sample'));
// output(choosen(normed(errorcode<>''), eyeball), named('fpsa_errors'));
// output(count(normed(errorcode<>'')), named('fpsA_err_count'));
...
// output(choosen(rolled_to_1_record, eyeball), named('fpsa_rolled_records'));
...
NewRecs := PROJECT(rolled_to_1_record,TRANSFORM(lay,self.acctno:=left.accountnumber;
...
op_final :=   output(NewRecs,,Output_file_name,CSV(heading(single), quote('"')), overwrite );


// op_despray := FileServices.DeSpray('~sghatti::out::nonFcra_FPScoresAttributes_v2_' + ut.GetDate,'10.48.72.175', 'G:\\Scoring Project\\landing\\'+ 'nonFcra_FPScoresAttributes_v2_' + ut.GetDate  + '_' + ut.getTime()  + '.csv',,,,TRUE);

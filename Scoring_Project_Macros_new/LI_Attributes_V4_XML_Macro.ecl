EXPORT LI_Attributes_V4_XML_Macro (roxie_ip,Gateway, Thread, Timeout, Retry, Input_file_name,Output_file_name, records_ToRun):= functionmacro
...
f_all := DATASET( infile_name, layout_input, CSV(QUOTE('"')));
...
op_shell := output(adlappendshell, , outputshell, CSV(HEADING(single), QUOTE('"')), OVERWRITE);
...
fin_out := sequential(op,op_final);

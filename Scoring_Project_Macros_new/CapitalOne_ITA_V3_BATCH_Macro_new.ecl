EXPORT CapitalOne_ITA_V3_BATCH_Macro_new (roxie_ip,Gateway, Thread, Timeout, Retry, Input_file_name,Output_file_name, records_ToRun):= functionmacro
...
inputData1 := dataset(ut.foreign_prod + infile_name, newlayout, csv(quote('"'), HEADING(SINGLE)));
...
layout_soap_input make_rv_in(ds_input le, integer c) := TRANSFORM
	...
	SELF.adl_based_shell := true;
  SELF.DataRestrictionMask := '000000000000010000000000000000' ;  // to allow use of both Equifax and Experian, this is the default value for all legacy scoring products
	SELF.HistoryDateYYYYMM := 99999 ; //IF(histDateYYYYMM=0, le.historydateyyyymm, histDateYYYYMM);
	END;
...
// f1 := IF(recordsToRun = 0, dataset('~nkoubsky::in::capone_ita3batch_20131126', newlayout1, csv(quote('"'))), choosen(dataset('~nkoubsky::in::capone_ita3batch_20131126', newlayout, csv(quote('"'))), recordsToRun));

...
indata := IF(no_of_records <= 0, dataset(ut.foreign_prod + infile_name, rec_input, CSV(HEADING(single), QUOTE('"')) ),
                            CHOOSEN(DATASET(ut.foreign_prod + infile_name, rec_input, CSV(HEADING(single), QUOTE('"'))), no_of_records));	
...
f1 := PROJECT(indata, TRANSFORM(lay_input, self.account := left.account_number;
...
// output(choosen(j_f,all), named('rvs_j_f'));

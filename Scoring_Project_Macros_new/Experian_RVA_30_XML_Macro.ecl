...
f1 := IF(no_of_records <= 0, dataset(ut.foreign_prod + infile_name, layout_input, thor ),
                            CHOOSEN(DATASET(ut.foreign_prod + infile_name, layout_input, thor), no_of_records));	


f := distribute(f1, hash64(account));  // Comment out when testing only one account
...
// fcraroxieIP := riskwise.shortcuts.staging_fcra_roxieIP; //calling from a MACRO and pointing to RiskWise.Shortcuts.prod_batch_fcra; 
...
//	self.HistoryDateYYYYMM := le.historydateyyyymm; //999999;
  ...
  // self.gateways := dataset([{'neutralroxie', riskwise.shortcuts.prod_batch_neutral}], risk_indicators.Layout_Gateways_In);
	...
	// self.gateways := dataset([{'neutralroxie', riskwise.shortcuts.staging_neutral_roxieIP}], risk_indicators.Layout_Gateways_In);
			self.gateways := dataset([{'neutralroxie', neutralroxieIP}], risk_indicators.Layout_Gateways_In);//calling from a MACRO and pointing to RiskWise.shortcuts.prod_batch_neutral; 
		// self.gateways := dataset([{'neutralroxie', riskwise.shortcuts.QA_neutral_roxieIP}], risk_indicators.Layout_Gateways_In); //pointing to QA Roxie as per Nathan's request
...
// output(choosen(j_f, eyeball), named('join'));

// ******* Resort output to original order ********

// layout that matches documentation from Brad
layout_final := RECORD
	string30 AccountNumber;
	string4 AgeOldestRecord; 
	string4 AgeNewestRecord; 
...
// mappings to fields in the documented layout
layout_final intoFinal(s le) := transform
	self.RecentUpdate	:= le.isrecentupdate		;
	self.SrcsConfirmIDAddrCount	:= le.numsources		;
	self.VerificationFailure	:= le.isnover		;
	self.SSNDateDeceased	:= le.deceaseddate		;
	self.SSNRecent	:= le.recentissue		;
	self.SSNLowIssueDate	:= le.lowissuedate		;
	self.SSNHighIssueDate	:= le.highissuedate		;
	self.SSNIssueState	:= le.issuestate		;
	self.SSNNonUS	:= le.nonus		;
	self.SSN3Years	:= le.issued3		;
	self.SSNAfter5	:= le.issuedage5		;
	self.InputAddrAgeOldestRecord	:= le.iaageoldestrecord		;
	self.InputAddrAgeNewestRecord	:= le.iaagenewestrecord		;
	self.InputAddrALenOfRes	:= le.ialenofres		;
	self.InputAddrDwellType	:= le.iadwelltype		;
	self.InputAddrLandUseCode	:= le.ialandusecode		;
	self.InputAddrOwnedBySubject	:= le.iaownedbysubject		;
	self.InputAddrFamilyOwned	:= le.iafamilyowned		;
	self.InputAddrOccupantOwned	:= le.iaoccupantowned		;
	self.InputAddrAgeLastSale	:= le.iaagelastsale		;
	self.InputAddrLastSalesPrice	:= le.ialastsaleamount		;
	self.InputAddrNotPrimaryRes	:= le.ianotprimaryres		;
	self.InputAddrActivePhoneList	:= le.iaphonelisted		;
	self.InputAddrActivePhoneNumber	:= le.iaphonenumber		;
	self.InputAddrTaxValue	:= le.iaassessedvalue		;
	self.CurrAddrAgeOldestRecord	:= le.caageoldestrecord		;
	self.CurrAddrAgeNewestRecord	:= le.caagenewestrecord		;
	self.CurrAddrLenOfRes	:= le.calenofres		;
	self.CurrAddrDwellType	:= le.cadwelltype		;
	self.CurrAddrALandUseCode	:= le.calandusecode		;
	self.CurrAddrApplicantOwned	:= le.caownedbysubject		;
	self.CurrAddrFamilyOwned	:= le.cafamilyowned		;
	self.CurrAddrOccupantOwned	:= le.caoccupantowned		;
	self.CurrAddrAgeLastSale	:= le.caagelastsale		;
	self.CurrAddrLastSalesPrice	:= le.calastsaleamount		;
	self.CurrAddrNotPrimaryRes	:= le.canotprimaryres		;
	self.CurrAddrActivePhoneList	:= le.caphonelisted		;
	self.CurrAddrActivePhoneNumber	:= le.caphonenumber		;
	self.CurrAddrTaxValue	:= le.caassessedvalue		;
	self.PrevAddrAgeOldestRecord	:= le.paageoldestrecord		;
	self.PrevAddrAgeNewestRecord	:= le.paagenewestrecord		;
	self.PrevAddrLenOfRes	:= le.palenofres		;
	self.PrevAddrDwellType	:= le.padwelltype		;
	self.PrevAddrLandUseCode	:= le.palandusecode		;
	self.PrevAddrApplicantOwned	:= le.paownedbysubject		;
	self.PrevAddrFamilyOwned	:= le.pafamilyowned		;
	self.PrevAddrOccupantOwned	:= le.paoccupantowned		;
	self.PrevAddrAgeLastSale	:= le.paagelastsale		;
	self.PrevAddrLastSalesPrice	:= le.palastsaleamount		;
	self.PrevAddrPhoneListed	:= le.paphonelisted		;
	self.PrevAddrPhoneNumber	:= le.paphonenumber		;
	self.PrevAddrAssessedValue	:= le.paassessedvalue		;
	self.InputCurrAddrMatch	:= le.inputcurrmatch		;
	self.InputCurrAddrDistance	:= le.distinputcurr		;
	self.InputCurrAddrStateDiff	:= le.diffstate		;
	self.InputCurrAddrTaxDiff	:= le.assesseddiff		;
	self.InputCurrEconTrajectory	:= le.ecotrajectory		;
	self.InputPrevAddrMatch	:= le.inputprevmatch		;
	self.CurrPrevAddrDistance	:= le.distcurrprev		;
	self.CurrPrevAddrStateDiff	:= le.diffstate2		;
	self.CurrPrevAddrTaxDiff	:= le.assesseddiff2		;
	self.PrevCurrEconTrajectory	:= le.ecotrajectory2		;
	self.AddrStability	:= le.mobility_indicator		;
	self.StatusMostRecent	:= le.statusaddr		;
	self.StatusPrevious	:= le.statusaddr2		;
	self.StatusNextPrevious	:= le.statusaddr3		;
	self.AddrChangeCount01	:= le.addrchanges30		;
	self.AddrChangeCount03	:= le.addrchanges90		;
	self.AddrChangeCount06	:= le.addrchanges180		;
	self.AddrChangeCount12	:= le.addrchanges12		;
	self.AddrChangeCount24	:= le.addrchanges24		;
	self.AddrChangeCount36	:= le.addrchanges36		;
	self.AddrChangeCount60	:= le.addrchanges60		;
	self.PropOwnedCount	:= le.property_owned_total		;
	self.PropOwnedTaxTotal	:= le.property_owned_assessed_total		;
	self.PropOwnedHistoricalCount	:= le.property_historically_owned		;
	self.PropPurchasedCount01	:= le.numpurchase30		;
	self.PropPurchasedCount03	:= le.numpurchase90		;
	self.PropPurchasedCount06	:= le.numpurchase180		;
	self.PropPurchasedCount12	:= le.numpurchase12		;
	self.PropPurchasedCount24	:= le.numpurchase24		;
	self.PropPurchasedCount36	:= le.numpurchase36		;
	self.PropPurchasedCount60	:= le.numpurchase60		;
	self.PropSoldCount01	:= le.numsold30		;
	self.PropSoldCount03	:= le.numsold90		;
	self.PropSoldCount06	:= le.numsold180		;
	self.PropSoldCount12	:= le.numsold12		;
	self.PropSoldCount24	:= le.numsold24		;
	self.PropSoldCount36	:= le.numsold36		;
	self.PropSoldCount60	:= le.numsold60		;
	self.WatercraftCount	:= le.numwatercraft		;
	self.WatercraftCount01	:= le.numwatercraft30		;
	self.WatercraftCount03	:= le.numwatercraft90		;
	self.WatercraftCount06	:= le.numwatercraft180		;
	self.WatercraftCount12	:= le.numwatercraft12		;
	self.WatercraftCount24	:= le.numwatercraft24		;
	self.WatercraftCount36	:= le.numwatercraft36		;
	self.WatercraftCount60	:= le.numwatercraft60		;
	self.AircraftCount	:= le.numaircraft		;
	self.AircraftCount01	:= le.numaircraft30		;
	self.AircraftCount03	:= le.numaircraft90		;
	self.AircraftCount06	:= le.numaircraft180		;
	self.AircraftCount12	:= le.numaircraft12		;
	self.AircraftCount24	:= le.numaircraft24		;
	self.AircraftCount36	:= le.numaircraft36		;
	self.AircraftCount60	:= le.numaircraft60		;
	self.WealthIndex	:= le.wealth_indicator		;
	self.DerogCount	:= le.total_number_derogs		;
	self.FelonyCount	:= le.felonies		;
	self.FelonyCount01	:= le.felonies30		;
	self.FelonyCount03	:= le.felonies90		;
	self.FelonyCount06	:= le.felonies180		;
	self.FelonyCount12	:= le.felonies12		;
	self.FelonyCount24	:= le.felonies24		;
	self.FelonyCount36	:= le.felonies36		;
	self.FelonyCount60	:= le.felonies60		;
	self.LienCount	:= le.num_liens		;
	self.LienFiledCount	:= le.num_unreleased_liens		;
	self.LienFiledCount01	:= le.num_unreleased_liens30		;
	self.LienFiledCount03	:= le.num_unreleased_liens90		;
	self.LienFiledCount06	:= le.num_unreleased_liens180		;
	self.LienFiledCount12	:= le.num_unreleased_liens12		;
	self.LienFiledCount24	:= le.num_unreleased_liens24		;
	self.LienFiledCount36	:= le.num_unreleased_liens36		;
	self.LienFiledCount60	:= le.num_unreleased_liens60		;
	self.LienReleasedCount	:= le.num_released_liens		;
	self.LienReleasedCount01	:= le.num_released_liens30		;
	self.LienReleasedCount03	:= le.num_released_liens90		;
	self.LienReleasedCount06	:= le.num_released_liens180		;
	self.LienReleasedCount12	:= le.num_released_liens12		;
	self.LienReleasedCount24	:= le.num_released_liens24		;
	self.LienReleasedCount36	:= le.num_released_liens36		;
	self.LienReleasedCount60	:= le.num_released_liens60		;
	self.BankruptcyCount	:= le.bankruptcy_count		;
	self.BankruptcyType	:= le.filing_type		;
	self.BankruptcyStatus	:= le.disposition		;
	self.BankruptcyCount01	:= le.bankruptcy_count30		;
	self.BankruptcyCount03	:= le.bankruptcy_count90		;
	self.BankruptcyCount06	:= le.bankruptcy_count180		;
	self.BankruptcyCount12	:= le.bankruptcy_count12		;
	self.BankruptcyCount24	:= le.bankruptcy_count24		;
	self.BankruptcyCount36	:= le.bankruptcy_count36		;
	self.BankruptcyCount60	:= le.bankruptcy_count60		;
	self.EvictionCount	:= le.eviction_count		;
	self.EvictionCount01	:= le.eviction_count30		;
	self.EvictionCount03	:= le.eviction_count90		;
	self.EvictionCount06	:= le.eviction_count180		;
	self.EvictionCount12	:= le.eviction_count12		;
	self.EvictionCount24	:= le.eviction_count24		;
	self.EvictionCount36	:= le.eviction_count36		;
	self.EvictionCount60	:= le.eviction_count60		;
	self.NonDerogCount	:= le.num_nonderogs		;
	self.NonDerogCount01	:= le.num_nonderogs30		;
	self.NonDerogCount03	:= le.num_nonderogs90		;
	self.NonDerogCount06	:= le.num_nonderogs180		;
	self.NonDerogCount12	:= le.num_nonderogs12		;
	self.NonDerogCount24	:= le.num_nonderogs24		;
	self.NonDerogCount36	:= le.num_nonderogs36		;
	self.NonDerogCount60	:= le.num_nonderogs60		;
	self.ProfLicCount	:= le.num_proflic		;
	self.ProfLicCount01	:= le.num_proflic30		;
	self.ProfLicCount03	:= le.num_proflic90		;
	self.ProfLicCount06	:= le.num_proflic180		;
	self.ProfLicCount12	:= le.num_proflic12		;
	self.ProfLicCount24	:= le.num_proflic24		;
	self.ProfLicCount36	:= le.num_proflic36		;
	self.ProfLicCount60	:= le.num_proflic60		;
	self.ProflicExpireCount01	:= le.num_proflic_exp30		;
	self.ProflicExpireCount03	:= le.num_proflic_exp90		;
	self.ProflicExpireCount06	:= le.num_proflic_exp180		;
	self.ProflicExpireCount12	:= le.num_proflic_exp12		;
	self.ProflicExpireCount24	:= le.num_proflic_exp24		;
	self.ProflicExpireCount36	:= le.num_proflic_exp36		;
	self.ProflicExpireCount60	:= le.num_proflic_exp60		;
	self.InputPhoneStatus	:= le.phonestatus		;
	self.InputPhonePager	:= le.phonepager		;
	self.InputPhoneMobile	:= le.phonemobile		;
	self.InvalidPhoneZip	:= le.phonezipmismatch		;
	self.InputPhoneAddrDist	:= le.phoneaddrdist		;
	self.InputAddrHighRisk	:= le.addrhighrisk		;
	self.InputPhoneHighRisk	:= le.phonehighrisk		;
	self.InputAddrPrison	:= le.addrprison		;
	self.InputZipPOBox	:= le.zippobox		;
	self.InputZipCorpMil	:= le.zipcorpmil		;
	self := le;
end;
...
//output(count(j_f(errorcode<>'')),named('rva30_error_count'));

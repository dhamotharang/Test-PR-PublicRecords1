...
EXPORT Instant_ID_XML_Macro (roxie_ip,Gateway, Thread, Timeout, Retry, Input_file_name,Output_file_name, records_ToRun):= functionmacro
...
f1 := IF(no_of_records = 0, dataset(Infile_name, layout_input, CSV(HEADING(1), QUOTE('"'))), choosen(dataset (Infile_name, layout_input, CSV(HEADING(1), QUOTE('"'))), no_of_records));
// OUTPUT(CHOOSEN(f1, 20), NAMED('f1'));
...
xlayout myFail(dist_dataset le) := TRANSFORM
	...
	SELF := [];
END;
// OUTPUT(CHOOSEN(dist_dataset, 20), NAMED('dist_dataset'));
...
// OUTPUT(CHOOSEN(resu, 20), NAMED('resu'));

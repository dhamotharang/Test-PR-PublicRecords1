...
export proc_Orbit3_CreateBuild(string buildname,string Buildvs,string Envmt = 'N', boolean skipcreatebuild = false,boolean skipupdatebuild = false, boolean skipaddcomponents = false, boolean runcreatebuild = true, boolean runaddcomponentsonly = false) := function
	...
	return Orbit3.AddComponentstoBuild(tokenval,
		...
		add_components := 		apply(get_new_build_candidates, output(fn_add_components( ComponentType,DataType,Family,Id,Name,Status,Version)));

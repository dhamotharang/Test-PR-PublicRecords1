...
eCrashSearchRecs := DISTRIBUTE(PROJECT(SearchRecs, Layouts.key_search_layout), HASH32(accident_nbr));
EXPORT EcrashSearchRecsIn := eCrashSearchRecs:INDEPENDENT;
...
EXPORT uSlimDlnNbrDLState := DEDUP(sSlimDlnNbrDLState, accident_nbr, driver_license_nbr, dlnbr_st, report_code, jurisdiction_state, jurisdiction,
                                   accident_date, report_type_id, LOCAL);
...
sSlimLicensePlateNbr := SORT(dSlimLicensePlateNbr, accident_nbr, tag_nbr, tagnbr_st, report_code, jurisdiction_state, jurisdiction,
                             accident_date, report_type_id, LOCAL);
EXPORT uSlimLicensePlateNbr := DEDUP(sSlimLicensePlateNbr, accident_nbr, tag_nbr, tagnbr_st, report_code, jurisdiction_state, jurisdiction,
                                     accident_date, report_type_id, LOCAL); 
...
sSlimOfficerBadgeNbr := SORT(dSlimOfficerBadgeNbr, accident_nbr, officer_id, report_code, jurisdiction_state, jurisdiction,
                             accident_date, report_type_id, LOCAL);
EXPORT uSlimOfficerBadgeNbr := DEDUP(sSlimOfficerBadgeNbr, accident_nbr, officer_id, report_code, jurisdiction_state, jurisdiction, 
                                     accident_date, report_type_id, LOCAL);
			
//***************************************************************
//                Key_EcrashV2_LastName
//***************************************************************
dsLastName := EcrashSearchRecsIn(lname <> '');
fLastName := dsLastName(TRIM(lname, LEFT, RIGHT) <> TRIM(orig_lname, LEFT, RIGHT) AND  
...
Layouts.key_slim_layout	tModifyLayout(Layouts.key_search_layout L) := TRANSFORM
  SELF.lname := L.lname;
	...
	SELF := L;
END;
pSlimLastName := PROJECT(pLastName + dsLastName, tModifyLayout(LEFT));

dSlimLastName	:= DISTRIBUTED(pSlimLastName, HASH32(accident_nbr)); 
sSlimLastName	:= SORT(dSlimLastName, accident_nbr, lname, report_code, jurisdiction_state, jurisdiction, accident_date, report_type_id, LOCAL); 
EXPORT uSlimLastName	:= DEDUP(sSlimLastName, accident_nbr, lname, report_code, jurisdiction_state, jurisdiction, accident_date, report_type_id, LOCAL); 
...
//***************************************************************
dsStAndLocation := EcrashSearchRecsIn(Accident_Location <> ''); 

Layout_PrepEcrashSearchKeys.InterimAccidentLocLayout tParseAccidentLocation(dsStAndLocation L) := TRANSFORM
...
sAccidentLocation	:= SORT(dAccidentLocation, accident_nbr, Partial_Accident_Location, report_code, jurisdiction_state, jurisdiction,
                          accident_date, report_type_id, LOCAL);
EXPORT uAccidentLocation	:= DEDUP(sAccidentLocation, accident_nbr, Partial_Accident_Location, report_code, jurisdiction_state, jurisdiction, 
                                   accident_date, report_type_id, LOCAL);

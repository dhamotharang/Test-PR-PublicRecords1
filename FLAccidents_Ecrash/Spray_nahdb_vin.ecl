...
EXPORT Spray_nahdb_vin(string filedate,	 boolean	pIsTesting	= false ) := function

lfile := '~thor_data::in::nahdb::raw_'+filedate;
	sfile := '~thor_data::in::nahdb::vin' ;
	...
	nahdb_sp :=  if( EXISTS(FileServices.RemoteDirectory(_control.IPAddress.bctlpedata10,'/data/super_credit/ecrash/fulfilment/nhdb/raw/'+filedate,'*.csv')),
                  sequential(fileservices.clearsuperfile('~thor_data::in::nahdb::vin'),VersionControl.fSprayInputFiles(spry_raw,pIsTesting := pIsTesting)), 

...
EXPORT fn_FL_Removal_Extraction(STRING Pdate = (STRING) STD.Date.CurrentDate(TRUE)) := FUNCTION
...
//Incident input file 
 incident := dataset(Data_Services.foreign_prod+'thor_data400::in::ecrash::incidnt_raw_new'
  ...
  fl_incident := incident(loss_state_abbr = 'FL' AND source_id IN ['TM','TF'] 
	                         AND STD.Str.ToUpperCase(TRIM(vendor_code, ALL)) = 'CRASHLOGIC') :PERSIST('~THOR::BASE::PERSIST::ECRASH::FL_DATA_REMOVAL::INCIDENT', EXPIRE(30));
												 
  //Count FL Incidents
	OUTPUT(COUNT(fl_incident));
	
	d_fl_incident := DISTRIBUTE(fl_incident(incident_id <> ''), HASH32(incident_id));
	unq_fl_incident := DEDUP(SORT(d_fl_incident, incident_id, -(sent_to_hpcc_datetime), LOCAL), incident_id, LOCAL)  :PERSIST('~THOR::BASE::PERSIST::ECRASH::FL_DATA_REMOVAL::UNIQUE_INCIDENTS', EXPIRE(30));

  SequencingRecSummary xformCtr_inc(fl_incident L, INTEGER CTR) := TRANSFORM
	   SELF.recid            	:= CTR;
	...
	END;
	ExtractData_inc          := PROJECT(fl_incident, xformCtr_inc(LEFT, COUNTER));
	ExtractHeaderRec_inc  		:= DATASET([{0, IncidentHeader}], SequencingRecSummary);
	FORMATTEDFINALA_inc 		  := BigLineFormatDespray(ExtractData_inc & ExtractHeaderRec_inc, 'FL_Ecrash_Incident_100.csv');
...
fl_vehicl := JOIN(DISTRIBUTE(vehicl, HASH32(incident_id)), unq_fl_incident,
								     TRIM(LEFT.incident_id, ALL) = TRIM(RIGHT.incident_id,ALL),
								     TRANSFORM(LEFT), LOCAL) :PERSIST('~THOR::BASE::PERSIST::ECRASH::FL_DATA_REMOVAL::VEHICLE', EXPIRE(30));	

// Count FL Vehicle 
OUTPUT(COUNT(fl_vehicl));

SequencingRecSummary xformCtr_veh(fl_vehicl L, INTEGER CTR) := TRANSFORM
	   SELF.recid            	:= CTR;
		 SELF.LINE             	:= TRIM(TRIM(L.Vehicle_ID, LEFT, RIGHT) + ',' +  
		                                TRIM(L.Creation_Date, LEFT, RIGHT) + ',' + 
																...
																, LEFT, RIGHT); 
	END;
	ExtractData_veh           := PROJECT(fl_vehicl, xformCtr_veh(LEFT, COUNTER));
	ExtractHeaderRec_veh  		:= DATASET([{0, VehicleHeader}], SequencingRecSummary);
	FORMATTEDFINALA_veh 		  := BigLineFormatDespray(ExtractData_veh & ExtractHeaderRec_veh, 'FL_Ecrash_Vehicle_100.csv');
  ...
  fl_persn := JOIN(DISTRIBUTE(persn, HASH32(incident_id)), unq_fl_incident,
								   TRIM(LEFT.incident_id, ALL) = TRIM(RIGHT.incident_id, ALL),
								   TRANSFORM(LEFT), LOCAL) :PERSIST('~THOR::BASE::PERSIST::ECRASH::FL_DATA_REMOVAL::PERSON', EXPIRE(30));	

	//Count FL Person
	OUTPUT(COUNT(fl_persn));

  SequencingRecSummary xformCtr_per(fl_persn L, INTEGER CTR) := TRANSFORM
	   SELF.recid            	:= CTR;
	...
	END;
	ExtractData_per           := PROJECT(fl_persn, xformCtr_per(LEFT, COUNTER));
	ExtractHeaderRec_per  		:= DATASET([{0, PersonHeader}], SequencingRecSummary);
	FORMATTEDFINALA_per 		  := BigLineFormatDespray(ExtractData_per & ExtractHeaderRec_per, 'FL_Ecrash_Person_100.csv');
  ...
  fl_citation := JOIN(DISTRIBUTE(citation, HASH32(incident_id)), unq_fl_incident,
								   TRIM(LEFT.incident_id, ALL) = TRIM(RIGHT.incident_id, ALL),
								   TRANSFORM(LEFT), LOCAL) :PERSIST('~THOR::BASE::PERSIST::ECRASH::FL_DATA_REMOVAL::CITATION', EXPIRE(30));	

	//Count FL Citation
	OUTPUT(COUNT(fl_citation));

  SequencingRecSummary xformCtr_cit(fl_citation L, INTEGER CTR) := TRANSFORM
	   SELF.recid            	:= CTR;
	...
	END;
	ExtractData_cit           := PROJECT(fl_citation, xformCtr_cit(LEFT, COUNTER));
	ExtractHeaderRec_cit  		:= DATASET([{0, CitationHeader}], SequencingRecSummary);
	FORMATTEDFINALA_cit 		  := BigLineFormatDespray(ExtractData_cit & ExtractHeaderRec_cit, 'FL_Ecrash_Citation_100.csv');

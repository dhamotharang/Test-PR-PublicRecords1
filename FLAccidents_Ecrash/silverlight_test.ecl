...
,maxLength(40000)
  ...
  string8 traffic_control_damage_notify_time;
...
BaseFile :=dedup( dataset('~thor_data400::persist::ecrash_did_20120912',t,thor),all); 


output(BaseFile,all) ; 


 citation1 := dedup(project(BaseFile(citation_id <>'') , transform (
													 FLAccidents_Ecrash.Layout_Infiles.citation , self := left,self:=[])),citation_id ,creation_date,incident_id,citation_issued, citation_number1,section_number1,citation_detail1,violator_name,all); 
													 
								 
						output(citation1,all);
 citation := dataset(ut.foreign_prod+'~thor_data400::in::ecrash::citatn_20120912'
													 ,FLAccidents_Ecrash.Layout_Infiles.citation
													 ,csv(terminator('\n'), separator(','),quote('"')));	
		output(		citation ,all);
	

		incident1   := dedup(project(BaseFile , transform (
													FLAccidents_Ecrash.Layout_Infiles.incident_new,
													self := left, self := [])),incident_id, all); 
													
     		output(incident1,all); 		
 incident   := dataset(ut.foreign_prod+'~thor_data400::in::ecrash::incidnt_20120912'
													,FLAccidents_Ecrash.Layout_Infiles.incident_new
 ...
 persn1      :=  dedup(project(BaseFile, transform (
													FLAccidents_Ecrash.Layout_Infiles.persn_new,
													self := left)),all); 
													
				output(persn1,all); 

 persn      :=  dataset(ut.foreign_prod+'~thor_data400::in::ecrash::persn_20120912'
													,FLAccidents_Ecrash.Layout_Infiles.persn_new
													,csv(terminator('\n'), separator(','),quote('"')));
output(		persn ,all);
	 ...
	 vehicl     :=  dedup(project(BaseFile(  vehicle_id <>'') , transform (
													FLAccidents_Ecrash.Layout_Infiles.vehicl_new,
													
														self := left, self := [])),all); 			
		output(vehicl);
				
 vehicl1     :=  dataset(ut.foreign_prod+'~thor_data400::in::ecrash::vehicl_20120912'
													,FLAccidents_Ecrash.Layout_Infiles.vehicl_new
...
Property_damage1     :=  dedup(project(BaseFile(person_type ='PROPERTY OWNER' ) , transform (
													property_damage,
												self := left, 
												

	 self.property_owner_last_name             := left.last_name; 
		self.property_owner_first_name  := left.first_name ; 
		self.property_owner_middle_name := left.middle_name ; 
		
    self.property_owner_address            := left.Address ; 
	  self.property_owner_city               := left.City; 
	  self.property_owner_state               := left.State ; 
		self.property_owner_phone                := left.Home_Phone; 
		self.property_owner_zip_code             := left.Zip_Code;
		self := []
 )),all); 

output(Property_damage1); 

 Property_damage2     :=  dataset(ut.foreign_prod+'~thor_data400::in::ecrash::property_20120912'
													,property_damage
													,csv(terminator('\n'), separator(','),quote('"'),maxlength(50000)));

output(Property_damage2); 


 commercl   := dataset(ut.foreign_prod+'~thor_data400::in::ecrash::commercl_20120912'
													,FLAccidents_Ecrash.Layout_Infiles.commercial
													,csv(terminator('\n'), separator(','),quote('"')));	

output(commercl); 

	 commercl1     :=  dedup(project(BaseFile , transform (
													FLAccidents_Ecrash.Layout_Infiles.commercial,

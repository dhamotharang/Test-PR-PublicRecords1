...
EXPORT SALT34.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_supp_key','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_future_date','invalid_charter_nbr','invalid_type_cd','invalid_fed_tax_id','invalid_term_exist_cd','invalid_term_exist_desc','invalid_address_type_cd','invalid_address_type_desc','invalid_for_dom_ind','invalid_for_profit_ind','invalid_managed_ind','invalid_recordorigin','invalid_alphablankperiod','invalid_alphablank','invalid_alphadashblank','invalid_numeric','invalid_numblank');
EXPORT FieldTypeNum(SALT34.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_supp_key' => 2,'invalid_corp_vendor' => 3,'invalid_state_origin' => 4,'invalid_mandatory' => 5,'invalid_date' => 6,'invalid_future_date' => 7,'invalid_charter_nbr' => 8,'invalid_type_cd' => 9,'invalid_fed_tax_id' => 10,'invalid_term_exist_cd' => 11,'invalid_term_exist_desc' => 12,'invalid_address_type_cd' => 13,'invalid_address_type_desc' => 14,'invalid_for_dom_ind' => 15,'invalid_for_profit_ind' => 16,'invalid_managed_ind' => 17,'invalid_recordorigin' => 18,'invalid_alphablankperiod' => 19,'invalid_alphablank' => 20,'invalid_alphadashblank' => 21,'invalid_numeric' => 22,'invalid_numblank' => 23,0);
 
EXPORT MakeFT_invalid_corp_key(SALT34.StrType s0) := FUNCTION
  s1 := SALT34.stringfilter(s0,'-CL0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_corp_key(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,'-CL0123456789'))),~(LENGTH(TRIM(s)) >= 4));
EXPORT InValidMessageFT_invalid_corp_key(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars('-CL0123456789'),SALT34.HygieneErrors.NotLength('4..'),SALT34.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_type_cd(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_type_cd(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['01','06','P',' ']);
EXPORT InValidMessageFT_invalid_type_cd(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('01|06|P| '),SALT34.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_managed_ind(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_managed_ind(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['0','1','2','3',' ']);
EXPORT InValidMessageFT_invalid_managed_ind(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('0|1|2|3| '),SALT34.HygieneErrors.Good);
...
EXPORT Make_corp_llc_managed_ind(SALT34.StrType s0) := MakeFT_invalid_managed_ind(s0);
EXPORT InValid_corp_llc_managed_ind(SALT34.StrType s) := InValidFT_invalid_managed_ind(s);
EXPORT InValidMessage_corp_llc_managed_ind(UNSIGNED1 wh) := InValidMessageFT_invalid_managed_ind(wh);
...
EXPORT Make_cont_country(SALT34.StrType s0) := MakeFT_invalid_alphablank(s0);
EXPORT InValid_cont_country(SALT34.StrType s) := InValidFT_invalid_alphablank(s);
EXPORT InValidMessage_cont_country(UNSIGNED1 wh) := InValidMessageFT_invalid_alphablank(wh);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT34,Scrubs_Corp2_Mapping_IL_LLC_Main;

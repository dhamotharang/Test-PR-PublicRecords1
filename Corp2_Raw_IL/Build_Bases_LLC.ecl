...
EXPORT Build_Bases_LLC(
	...
	STRING																					pversion,
	BOOLEAN																					puseprod,	
	DATASET(Corp2_Raw_IL.Layouts.StringLayoutIn)		pInMaster   = Corp2_Raw_IL.Files(pfiledate,pUseProd).Input.LLC.MasterString.Logical,
	DATASET(Corp2_Raw_IL.Layouts.StringLayoutBase)	pBaseMaster = IF(Corp2_Raw_IL._Flags.Base.LLC.Master, Corp2_Raw_IL.Files(,pUseOtherEnvironment := FALSE).Base.LLC.Master.qa, DATASET([], Corp2_Raw_IL.Layouts.StringLayoutBase))
) := MODULE
	...
	// Return
	tools.mac_WriteFile(Corp2_Raw_IL.Filenames(pversion).Base.LLC.Master.New, basefile, Build_Corporation_Base);

	EXPORT full_build := SEQUENTIAL(	Build_Corporation_Base,
																		Promote(pversion).LLC.BuildFiles.New2Built,
																		Promote().LLC.BuildFiles.Built2QA);

	EXPORT All := IF(tools.fun_IsValidVersion(pversion),
									 full_build,
									 OUTPUT('No Valid version parameter passed, skipping Corp2_Raw_IL.Build_Bases_LLC attribute')
									);

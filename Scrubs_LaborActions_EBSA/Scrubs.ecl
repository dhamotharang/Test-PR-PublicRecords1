...
// The module to handle the case where no scrubs exist
  ...
  SHARED toRuleDesc(UNSIGNED c) := CHOOSE(c
          ...
          ,'admin_zip_code:Invalid_Zip:ALLOW','admin_zip_code:Invalid_Zip:LENGTHS'
          ,'admin_zip_code4:Invalid_No:ALLOW'
          ,'closing_reason:Invalid_Alpha:ALLOW'
          ,'closing_date:Invalid_Date:CUSTOM'
          ,'penalty_amount:Invalid_Float:ALLOW'
  ...
  SHARED toErrorMessage(UNSIGNED c) := CHOOSE(c
          ...
          ,Fields.InvalidMessage_admin_zip_code(1),Fields.InvalidMessage_admin_zip_code(2)
          ,Fields.InvalidMessage_admin_zip_code4(1)
          ,Fields.InvalidMessage_closing_reason(1)
          ,Fields.InvalidMessage_closing_date(1)
          ,Fields.InvalidMessage_penalty_amount(1)
          ,'Fields with errors'
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.admin_zip_code_Invalid := (le.ScrubsBits1 >> 13) & 3;
    SELF.admin_zip_code4_Invalid := (le.ScrubsBits1 >> 15) & 1;
    SELF.closing_reason_Invalid := (le.ScrubsBits1 >> 16) & 1;
    SELF.closing_date_Invalid := (le.ScrubsBits1 >> 17) & 1;
    SELF.penalty_amount_Invalid := (le.ScrubsBits1 >> 18) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    admin_zip_code_LENGTHS_ErrorCount := COUNT(GROUP,h.admin_zip_code_Invalid=2);
    admin_zip_code_Total_ErrorCount := COUNT(GROUP,h.admin_zip_code_Invalid>0);
    ...
    penalty_amount_ALLOW_ErrorCount := COUNT(GROUP,h.penalty_amount_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.admin_zip_code_Invalid,'ALLOW','LENGTHS','UNKNOWN')
          ...
          ,CHOOSE(le.penalty_amount_Invalid,'ALLOW','UNKNOWN'),'UNKNOWN'));
    ...
    SELF.FieldType := CHOOSE(c,'Invalid_No','Invalid_Date','Invalid_Date','Invalid_Alpha','Invalid_State','Invalid_Alpha','Invalid_Float','Invalid_No','Invalid_Float','Invalid_AlphaNumChar','Invalid_AlphaNumChar','Invalid_Alpha','Invalid_Zip','Invalid_No','Invalid_Alpha','Invalid_Date','Invalid_Float','UNKNOWN');
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.admin_zip_code_ALLOW_ErrorCount,le.admin_zip_code_LENGTHS_ErrorCount
          ,le.admin_zip_code4_ALLOW_ErrorCount
          ,le.closing_reason_ALLOW_ErrorCount
          ,le.closing_date_CUSTOM_ErrorCount
          ,le.penalty_amount_ALLOW_ErrorCount
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ...
          ,le.closing_reason_ALLOW_ErrorCount
          ,le.closing_date_CUSTOM_ErrorCount
          ,le.penalty_amount_ALLOW_ErrorCount,0) / le.TotalCnt, CHOOSE(c - NumRules

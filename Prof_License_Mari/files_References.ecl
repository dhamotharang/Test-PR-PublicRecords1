...
export cmvtranslation	  := dataset('~thor_data400::in::proflic_mari::using::cmvtrans',Prof_License_Mari.layouts_Reference.cmvtranslation,CSV(HEADING(1),SEPARATOR('\t')));	
...
export MARIcmvLicStatus	:= dataset('~thor_data400::in::proflic_mari::using::cmvlicstatus',Prof_License_Mari.layouts_Reference.MARIcmvLicStatus,CSV(HEADING(1),SEPARATOR(','),QUOTE('"'))); 
export MARIcmvLicType	  := dataset('~thor_data400::in::proflic_mari::using::cmvlictype',Prof_License_Mari.layouts_Reference.MARIcmvLicType,CSV(HEADING(1),SEPARATOR('\t'))); 
export MARIcmvProf		  := dataset('~thor_data400::in::proflic_mari::using::cmvprof',Prof_License_Mari.layouts_Reference.MARIcmvProf,CSV(HEADING(1),QUOTE('"'))); 
export MARIcmvSrc	      := dataset('~thor_data400::in::proflic_mari::using::cmvsrc',Prof_License_Mari.layouts_Reference.MARIcmvSrc,CSV(HEADING(1),QUOTE('"'),SEPARATOR(','))); 
...
export county_names			:= dataset('~thor_data400::in::proflic_mari::using::county_names',Prof_License_Mari.layouts_reference.county_names,CSV(HEADING(1),SEPARATOR('\t'),QUOTE('"'))); 

...
#workunit('name','Prof License MARI- USS0404 ' + pVersion);
...
TrimLender				:= STD.Str.FilterOut(ut.fnTrim2Upper(pInput.LENDER_NAME),'"');
...
maribase_plus_dbas 		xformToCommon(FilterRec pInput) 
	:= 
	 TRANSFORM
		self.PRIMARY_KEY	    := 0;  
		...
		//Standardize Fields
		NAME_ORG_temp		:= ut.fnTrim2Upper(pInput.LENDER_NAME);
		TrimNAME_ORG    := STD.Str.FilterOut(NAME_ORG_temp, '"');
		ConvertedName_Org := STD.Str.FilterOut(ut.fnTrim2Upper(pInput.Convert_Lender_Name),'"');
		TrimCity				:= ut.fnTrim2Upper(pInput.CITY);
		TrimState				:= ut.fnTrim2Upper(pInput.STATE);
...
//Use address cleaner to clean address
	...
	RemovePattern	  := '(^.* LLC[.]?$|^.* INC[.]?$|^.* COMPANY$|^.* CORP[.]?$|^.*APPRAISAL[S]?$|^.* MORTGAGE$|^HOMEFINDERSCENTER.COM$|^AGENT$|^BANK OF .*$|^CENTRAL REGION.*$|' +
		...
		TrimAddress1		:= ut.fnTrim2Upper(pInput.corrected_address1);
		TrimAddress2		:= ut.fnTrim2Upper(pInput.corrected_address2);
...
END;
inFileLic	:= project(FilterRec,xformToCommon(left));
...
Prof_License_Mari.layouts.base xTransToBase(FilteredRecs L) := transform
  self.NAME_ORG 				:= IF(regexfind('(%|")',L.NAME_ORG),
	...
	self.NAME_DBA 				:= IF(length(trim(L.TMP_DBA,left,right)) < 3,'',tmpNameDBA);

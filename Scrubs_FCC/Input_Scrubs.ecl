...
// The module to handle the case where no scrubs exist
  ...
  EXPORT Expanded_Layout := RECORD(Input_Layout_FCC)
    ...
    UNSIGNED1 licensees_attention_line_Invalid;
    ...
    UNSIGNED1 firm_preparing_application_Invalid;
  ...
  SHARED toRuleDesc(UNSIGNED c) := CHOOSE(c
          ...
          ,'licensees_attention_line:Invalid_AlphaChar:ALLOW'
          ,'dba_name:Invalid_AlphaNumChar:ALLOW'
          ,'licensees_street:Invalid_AlphaNumChar:ALLOW'
          ,'licensees_city:Invalid_AlphaChar:ALLOW'
          ,'licensees_state:Invalid_State:ALLOW','licensees_state:Invalid_State:LENGTHS'
          ...
          ,'type_of_last_change:Invalid_Alpha:ALLOW'
          ,'latitude:Invalid_Float:ALLOW'
          ,'longitude:Invalid_Float:ALLOW'
          ,'transmitters_street:Invalid_AlphaNumChar:ALLOW'
          ,'transmitters_city:Invalid_AlphaChar:ALLOW'
          ,'transmitters_county:Invalid_AlphaChar:ALLOW'
          ,'transmitters_state:Invalid_State:ALLOW','transmitters_state:Invalid_State:LENGTHS'
          ...
          ,'class_of_station:Invalid_AlphaNum:ALLOW'
          ...
          ,'firm_preparing_application:Invalid_AlphaChar:ALLOW'
          ,'contact_firms_street_address:Invalid_AlphaNumChar:ALLOW'
          ,'contact_firms_city:Invalid_Alpha:ALLOW'
          ,'contact_firms_state:Invalid_State:ALLOW','contact_firms_state:Invalid_State:LENGTHS'
          ...
          ,'unique_key:Invalid_AlphaNumChar:ALLOW'
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.callsign_of_license_Invalid := (le.ScrubsBits1 >> 2) & 1;
    SELF.radio_service_code_Invalid := (le.ScrubsBits1 >> 3) & 1;
    SELF.licensees_name_Invalid := (le.ScrubsBits1 >> 4) & 1;
    SELF.licensees_attention_line_Invalid := (le.ScrubsBits1 >> 5) & 1;
    SELF.dba_name_Invalid := (le.ScrubsBits1 >> 6) & 1;
    SELF.licensees_street_Invalid := (le.ScrubsBits1 >> 7) & 1;
    SELF.licensees_city_Invalid := (le.ScrubsBits1 >> 8) & 1;
    SELF.licensees_state_Invalid := (le.ScrubsBits1 >> 9) & 3;
    SELF.licensees_zip_Invalid := (le.ScrubsBits1 >> 11) & 3;
    SELF.licensees_phone_Invalid := (le.ScrubsBits1 >> 13) & 3;
    SELF.date_application_received_at_fcc_Invalid := (le.ScrubsBits1 >> 15) & 1;
    SELF.date_license_issued_Invalid := (le.ScrubsBits1 >> 16) & 1;
    SELF.date_license_expires_Invalid := (le.ScrubsBits1 >> 17) & 1;
    SELF.date_of_last_change_Invalid := (le.ScrubsBits1 >> 18) & 1;
    SELF.type_of_last_change_Invalid := (le.ScrubsBits1 >> 19) & 1;
    SELF.latitude_Invalid := (le.ScrubsBits1 >> 20) & 1;
    SELF.longitude_Invalid := (le.ScrubsBits1 >> 21) & 1;
    SELF.transmitters_street_Invalid := (le.ScrubsBits1 >> 22) & 1;
    SELF.transmitters_city_Invalid := (le.ScrubsBits1 >> 23) & 1;
    SELF.transmitters_county_Invalid := (le.ScrubsBits1 >> 24) & 1;
    SELF.transmitters_state_Invalid := (le.ScrubsBits1 >> 25) & 3;
    SELF.transmitters_antenna_height_Invalid := (le.ScrubsBits1 >> 27) & 1;
    SELF.transmitters_height_above_avg_terra_Invalid := (le.ScrubsBits1 >> 28) & 1;
    SELF.transmitters_height_above_ground_le_Invalid := (le.ScrubsBits1 >> 29) & 1;
    SELF.power_of_this_frequency_Invalid := (le.ScrubsBits1 >> 30) & 1;
    SELF.frequency_mhz_Invalid := (le.ScrubsBits1 >> 31) & 1;
    SELF.class_of_station_Invalid := (le.ScrubsBits1 >> 32) & 1;
    SELF.number_of_units_authorized_on_freq_Invalid := (le.ScrubsBits1 >> 33) & 1;
    SELF.effective_radiated_power_Invalid := (le.ScrubsBits1 >> 34) & 1;
    SELF.emissions_codes_Invalid := (le.ScrubsBits1 >> 35) & 1;
    SELF.frequency_coordination_number_Invalid := (le.ScrubsBits1 >> 36) & 1;
    SELF.paging_license_status_Invalid := (le.ScrubsBits1 >> 37) & 1;
    SELF.control_point_for_the_system_Invalid := (le.ScrubsBits1 >> 38) & 1;
    SELF.pending_or_granted_Invalid := (le.ScrubsBits1 >> 39) & 1;
    SELF.firm_preparing_application_Invalid := (le.ScrubsBits1 >> 40) & 1;
    SELF.contact_firms_street_address_Invalid := (le.ScrubsBits1 >> 41) & 1;
    SELF.contact_firms_city_Invalid := (le.ScrubsBits1 >> 42) & 1;
    SELF.contact_firms_state_Invalid := (le.ScrubsBits1 >> 43) & 3;
    SELF.contact_firms_zipcode_Invalid := (le.ScrubsBits1 >> 45) & 3;
    SELF.contact_firms_phone_number_Invalid := (le.ScrubsBits1 >> 47) & 3;
    SELF.contact_firms_fax_number_Invalid := (le.ScrubsBits1 >> 49) & 3;
    SELF.unique_key_Invalid := (le.ScrubsBits1 >> 51) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    callsign_of_license_ALLOW_ErrorCount := COUNT(GROUP,h.callsign_of_license_Invalid=1);
    radio_service_code_ALLOW_ErrorCount := COUNT(GROUP,h.radio_service_code_Invalid=1);
    licensees_name_ALLOW_ErrorCount := COUNT(GROUP,h.licensees_name_Invalid=1);
    licensees_attention_line_ALLOW_ErrorCount := COUNT(GROUP,h.licensees_attention_line_Invalid=1);
    dba_name_ALLOW_ErrorCount := COUNT(GROUP,h.dba_name_Invalid=1);
    licensees_street_ALLOW_ErrorCount := COUNT(GROUP,h.licensees_street_Invalid=1);
    licensees_city_ALLOW_ErrorCount := COUNT(GROUP,h.licensees_city_Invalid=1);
    licensees_state_ALLOW_ErrorCount := COUNT(GROUP,h.licensees_state_Invalid=1);
    licensees_state_LENGTHS_ErrorCount := COUNT(GROUP,h.licensees_state_Invalid=2);
    licensees_state_Total_ErrorCount := COUNT(GROUP,h.licensees_state_Invalid>0);
    ...
    type_of_last_change_ALLOW_ErrorCount := COUNT(GROUP,h.type_of_last_change_Invalid=1);
    latitude_ALLOW_ErrorCount := COUNT(GROUP,h.latitude_Invalid=1);
    longitude_ALLOW_ErrorCount := COUNT(GROUP,h.longitude_Invalid=1);
    transmitters_street_ALLOW_ErrorCount := COUNT(GROUP,h.transmitters_street_Invalid=1);
    transmitters_city_ALLOW_ErrorCount := COUNT(GROUP,h.transmitters_city_Invalid=1);
    transmitters_county_ALLOW_ErrorCount := COUNT(GROUP,h.transmitters_county_Invalid=1);
    transmitters_state_ALLOW_ErrorCount := COUNT(GROUP,h.transmitters_state_Invalid=1);
    transmitters_state_LENGTHS_ErrorCount := COUNT(GROUP,h.transmitters_state_Invalid=2);
    transmitters_state_Total_ErrorCount := COUNT(GROUP,h.transmitters_state_Invalid>0);
    ...
    class_of_station_ALLOW_ErrorCount := COUNT(GROUP,h.class_of_station_Invalid=1);
    ...
    emissions_codes_ALLOW_ErrorCount := COUNT(GROUP,h.emissions_codes_Invalid=1);
    frequency_coordination_number_ALLOW_ErrorCount := COUNT(GROUP,h.frequency_coordination_number_Invalid=1);
    paging_license_status_ALLOW_ErrorCount := COUNT(GROUP,h.paging_license_status_Invalid=1);
    control_point_for_the_system_ALLOW_ErrorCount := COUNT(GROUP,h.control_point_for_the_system_Invalid=1);
    pending_or_granted_ALLOW_ErrorCount := COUNT(GROUP,h.pending_or_granted_Invalid=1);
    firm_preparing_application_ALLOW_ErrorCount := COUNT(GROUP,h.firm_preparing_application_Invalid=1);
    contact_firms_street_address_ALLOW_ErrorCount := COUNT(GROUP,h.contact_firms_street_address_Invalid=1);
    contact_firms_city_ALLOW_ErrorCount := COUNT(GROUP,h.contact_firms_city_Invalid=1);
    contact_firms_state_ALLOW_ErrorCount := COUNT(GROUP,h.contact_firms_state_Invalid=1);
    contact_firms_state_LENGTHS_ErrorCount := COUNT(GROUP,h.contact_firms_state_Invalid=2);
    contact_firms_state_Total_ErrorCount := COUNT(GROUP,h.contact_firms_state_Invalid>0);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.license_type_ALLOW_ErrorCount > 0, 1, 0) + IF(le.file_number_ALLOW_ErrorCount > 0, 1, 0) + IF(le.callsign_of_license_ALLOW_ErrorCount > 0, 1, 0) + IF(le.radio_service_code_ALLOW_ErrorCount > 0, 1, 0) + IF(le.licensees_name_ALLOW_ErrorCount > 0, 1, 0) + IF(le.licensees_attention_line_ALLOW_ErrorCount > 0, 1, 0) + IF(le.dba_name_ALLOW_ErrorCount > 0, 1, 0) + IF(le.licensees_street_ALLOW_ErrorCount > 0, 1, 0) + IF(le.licensees_city_ALLOW_ErrorCount > 0, 1, 0) + IF(le.licensees_state_Total_ErrorCount > 0, 1, 0) + IF(le.licensees_zip_Total_ErrorCount > 0, 1, 0) + IF(le.licensees_phone_Total_ErrorCount > 0, 1, 0) + IF(le.date_application_received_at_fcc_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.date_license_issued_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.date_license_expires_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.date_of_last_change_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.type_of_last_change_ALLOW_ErrorCount > 0, 1, 0) + IF(le.latitude_ALLOW_ErrorCount > 0, 1, 0) + IF(le.longitude_ALLOW_ErrorCount > 0, 1, 0) + IF(le.transmitters_street_ALLOW_ErrorCount > 0, 1, 0) + IF(le.transmitters_city_ALLOW_ErrorCount > 0, 1, 0) + IF(le.transmitters_county_ALLOW_ErrorCount > 0, 1, 0) + IF(le.transmitters_state_Total_ErrorCount > 0, 1, 0) + IF(le.transmitters_antenna_height_ALLOW_ErrorCount > 0, 1, 0) + IF(le.transmitters_height_above_avg_terra_ALLOW_ErrorCount > 0, 1, 0) + IF(le.transmitters_height_above_ground_le_ALLOW_ErrorCount > 0, 1, 0) + IF(le.power_of_this_frequency_ALLOW_ErrorCount > 0, 1, 0) + IF(le.frequency_mhz_ALLOW_ErrorCount > 0, 1, 0) + IF(le.class_of_station_ALLOW_ErrorCount > 0, 1, 0) + IF(le.number_of_units_authorized_on_freq_ALLOW_ErrorCount > 0, 1, 0) + IF(le.effective_radiated_power_ALLOW_ErrorCount > 0, 1, 0) + IF(le.emissions_codes_ALLOW_ErrorCount > 0, 1, 0) + IF(le.frequency_coordination_number_ALLOW_ErrorCount > 0, 1, 0) + IF(le.paging_license_status_ALLOW_ErrorCount > 0, 1, 0) + IF(le.control_point_for_the_system_ALLOW_ErrorCount > 0, 1, 0) + IF(le.pending_or_granted_ALLOW_ErrorCount > 0, 1, 0) + IF(le.firm_preparing_application_ALLOW_ErrorCount > 0, 1, 0) + IF(le.contact_firms_street_address_ALLOW_ErrorCount > 0, 1, 0) + IF(le.contact_firms_city_ALLOW_ErrorCount > 0, 1, 0) + IF(le.contact_firms_state_Total_ErrorCount > 0, 1, 0) + IF(le.contact_firms_zipcode_Total_ErrorCount > 0, 1, 0) + IF(le.contact_firms_phone_number_Total_ErrorCount > 0, 1, 0) + IF(le.contact_firms_fax_number_Total_ErrorCount > 0, 1, 0) + IF(le.unique_key_ALLOW_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.license_type_ALLOW_ErrorCount > 0, 1, 0) + IF(le.file_number_ALLOW_ErrorCount > 0, 1, 0) + IF(le.callsign_of_license_ALLOW_ErrorCount > 0, 1, 0) + IF(le.radio_service_code_ALLOW_ErrorCount > 0, 1, 0) + IF(le.licensees_name_ALLOW_ErrorCount > 0, 1, 0) + IF(le.licensees_attention_line_ALLOW_ErrorCount > 0, 1, 0) + IF(le.dba_name_ALLOW_ErrorCount > 0, 1, 0) + IF(le.licensees_street_ALLOW_ErrorCount > 0, 1, 0) + IF(le.licensees_city_ALLOW_ErrorCount > 0, 1, 0) + IF(le.licensees_state_ALLOW_ErrorCount > 0, 1, 0) + IF(le.licensees_state_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.licensees_zip_ALLOW_ErrorCount > 0, 1, 0) + IF(le.licensees_zip_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.licensees_phone_ALLOW_ErrorCount > 0, 1, 0) + IF(le.licensees_phone_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.date_application_received_at_fcc_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.date_license_issued_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.date_license_expires_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.date_of_last_change_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.type_of_last_change_ALLOW_ErrorCount > 0, 1, 0) + IF(le.latitude_ALLOW_ErrorCount > 0, 1, 0) + IF(le.longitude_ALLOW_ErrorCount > 0, 1, 0) + IF(le.transmitters_street_ALLOW_ErrorCount > 0, 1, 0) + IF(le.transmitters_city_ALLOW_ErrorCount > 0, 1, 0) + IF(le.transmitters_county_ALLOW_ErrorCount > 0, 1, 0) + IF(le.transmitters_state_ALLOW_ErrorCount > 0, 1, 0) + IF(le.transmitters_state_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.transmitters_antenna_height_ALLOW_ErrorCount > 0, 1, 0) + IF(le.transmitters_height_above_avg_terra_ALLOW_ErrorCount > 0, 1, 0) + IF(le.transmitters_height_above_ground_le_ALLOW_ErrorCount > 0, 1, 0) + IF(le.power_of_this_frequency_ALLOW_ErrorCount > 0, 1, 0) + IF(le.frequency_mhz_ALLOW_ErrorCount > 0, 1, 0) + IF(le.class_of_station_ALLOW_ErrorCount > 0, 1, 0) + IF(le.number_of_units_authorized_on_freq_ALLOW_ErrorCount > 0, 1, 0) + IF(le.effective_radiated_power_ALLOW_ErrorCount > 0, 1, 0) + IF(le.emissions_codes_ALLOW_ErrorCount > 0, 1, 0) + IF(le.frequency_coordination_number_ALLOW_ErrorCount > 0, 1, 0) + IF(le.paging_license_status_ALLOW_ErrorCount > 0, 1, 0) + IF(le.control_point_for_the_system_ALLOW_ErrorCount > 0, 1, 0) + IF(le.pending_or_granted_ALLOW_ErrorCount > 0, 1, 0) + IF(le.firm_preparing_application_ALLOW_ErrorCount > 0, 1, 0) + IF(le.contact_firms_street_address_ALLOW_ErrorCount > 0, 1, 0) + IF(le.contact_firms_city_ALLOW_ErrorCount > 0, 1, 0) + IF(le.contact_firms_state_ALLOW_ErrorCount > 0, 1, 0) + IF(le.contact_firms_state_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.contact_firms_zipcode_ALLOW_ErrorCount > 0, 1, 0) + IF(le.contact_firms_zipcode_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.contact_firms_phone_number_ALLOW_ErrorCount > 0, 1, 0) + IF(le.contact_firms_phone_number_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.contact_firms_fax_number_ALLOW_ErrorCount > 0, 1, 0) + IF(le.contact_firms_fax_number_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.unique_key_ALLOW_ErrorCount > 0, 1, 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.callsign_of_license_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.radio_service_code_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.licensees_name_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.licensees_attention_line_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.dba_name_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.licensees_street_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.licensees_city_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.licensees_state_Invalid,'ALLOW','LENGTHS','UNKNOWN')
          ...
          ,CHOOSE(le.type_of_last_change_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.latitude_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.longitude_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.transmitters_street_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.transmitters_city_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.transmitters_county_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.transmitters_state_Invalid,'ALLOW','LENGTHS','UNKNOWN')
          ...
          ,CHOOSE(le.class_of_station_Invalid,'ALLOW','UNKNOWN')
          ...
          ,CHOOSE(le.emissions_codes_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.frequency_coordination_number_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.paging_license_status_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.control_point_for_the_system_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.pending_or_granted_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.firm_preparing_application_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.contact_firms_street_address_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.contact_firms_city_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.contact_firms_state_Invalid,'ALLOW','LENGTHS','UNKNOWN')
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.file_number_ALLOW_ErrorCount
          ,le.callsign_of_license_ALLOW_ErrorCount
          ,le.radio_service_code_ALLOW_ErrorCount
          ,le.licensees_name_ALLOW_ErrorCount
          ,le.licensees_attention_line_ALLOW_ErrorCount
          ,le.dba_name_ALLOW_ErrorCount
          ,le.licensees_street_ALLOW_ErrorCount
          ,le.licensees_city_ALLOW_ErrorCount
          ,le.licensees_state_ALLOW_ErrorCount,le.licensees_state_LENGTHS_ErrorCount
          ...
          ,le.type_of_last_change_ALLOW_ErrorCount
          ,le.latitude_ALLOW_ErrorCount
          ,le.longitude_ALLOW_ErrorCount
          ,le.transmitters_street_ALLOW_ErrorCount
          ,le.transmitters_city_ALLOW_ErrorCount
          ,le.transmitters_county_ALLOW_ErrorCount
          ,le.transmitters_state_ALLOW_ErrorCount,le.transmitters_state_LENGTHS_ErrorCount
          ...
          ,le.class_of_station_ALLOW_ErrorCount
          ,le.number_of_units_authorized_on_freq_ALLOW_ErrorCount
          ,le.effective_radiated_power_ALLOW_ErrorCount
          ,le.emissions_codes_ALLOW_ErrorCount
          ,le.frequency_coordination_number_ALLOW_ErrorCount
          ,le.paging_license_status_ALLOW_ErrorCount
          ,le.control_point_for_the_system_ALLOW_ErrorCount
          ,le.pending_or_granted_ALLOW_ErrorCount
          ,le.firm_preparing_application_ALLOW_ErrorCount
          ,le.contact_firms_street_address_ALLOW_ErrorCount
          ,le.contact_firms_city_ALLOW_ErrorCount
          ,le.contact_firms_state_ALLOW_ErrorCount,le.contact_firms_state_LENGTHS_ErrorCount
          ...
          ,le.unique_key_ALLOW_ErrorCount
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ...
          ,le.contact_firms_phone_number_ALLOW_ErrorCount,le.contact_firms_phone_number_LENGTHS_ErrorCount
          ,le.contact_firms_fax_number_ALLOW_ErrorCount,le.contact_firms_fax_number_LENGTHS_ErrorCount
          ,le.unique_key_ALLOW_ErrorCount,0) / le.TotalCnt, CHOOSE(c - NumRules

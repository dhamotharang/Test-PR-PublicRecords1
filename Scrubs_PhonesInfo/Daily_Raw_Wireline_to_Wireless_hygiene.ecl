...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    Daily_Raw_Wireline_to_Wireless_Fields.InValid_phone((SALT38.StrType)le.phone),
    Daily_Raw_Wireline_to_Wireless_Fields.InValid_lf((SALT38.StrType)le.lf),
    Daily_Raw_Wireline_to_Wireless_Fields.InValid_filename((SALT38.StrType)le.filename),
    0);
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT38.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_PhonesInfo, Daily_Raw_Wireline_to_Wireless_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

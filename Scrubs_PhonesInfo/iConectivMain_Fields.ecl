...
EXPORT SALT310.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'Invalid_Num','Invalid_Num_Blank','Invalid_DCT','Invalid_TOS','Invalid_Port_Date','Invalid_ISO2','Invalid_Filename');
EXPORT FieldTypeNum(SALT310.StrType fn) := CASE(fn,'Invalid_Num' => 1,'Invalid_Num_Blank' => 2,'Invalid_DCT' => 3,'Invalid_TOS' => 4,'Invalid_Port_Date' => 5,'Invalid_ISO2' => 6,'Invalid_Filename' => 7,0);
...
EXPORT MakeFT_Invalid_DCT(SALT310.StrType s0) := FUNCTION
  s1 := SALT310.stringfilter(s0,'ECTBN'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_DCT(SALT310.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT310.StringFilter(s,'ECTBN'))));
EXPORT InValidMessageFT_Invalid_DCT(UNSIGNED1 wh) := CHOOSE(wh,SALT310.HygieneErrors.NotInChars('ECTBN'),SALT310.HygieneErrors.Good);
 
EXPORT MakeFT_Invalid_TOS(SALT310.StrType s0) := FUNCTION
  s1 := SALT310.stringfilter(s0,'MGOU'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_TOS(SALT310.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT310.StringFilter(s,'MGOU'))));
EXPORT InValidMessageFT_Invalid_TOS(UNSIGNED1 wh) := CHOOSE(wh,SALT310.HygieneErrors.NotInChars('MGOU'),SALT310.HygieneErrors.Good);
 
EXPORT MakeFT_Invalid_Port_Date(SALT310.StrType s0) := FUNCTION
  s1 := SALT310.stringfilter(s0,'0123456789 /:-'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_Port_Date(SALT310.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT310.StringFilter(s,'0123456789 /:-'))));
EXPORT InValidMessageFT_Invalid_Port_Date(UNSIGNED1 wh) := CHOOSE(wh,SALT310.HygieneErrors.NotInChars('0123456789 /:-'),SALT310.HygieneErrors.Good);
...
EXPORT SALT310.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'country_code','phone','dial_type','spid','service_provider','service_type','routing_code','porting_dt','country_abbr','filename','file_dt_time','vendor_first_reported_dt','vendor_last_reported_dt','port_start_dt','port_end_dt','remove_port_dt','is_ported');
EXPORT SALT310.StrType FlatName(UNSIGNED2 i) := CHOOSE(i,'country_code','phone','dial_type','spid','service_provider','service_type','routing_code','porting_dt','country_abbr','filename','file_dt_time','vendor_first_reported_dt','vendor_last_reported_dt','port_start_dt','port_end_dt','remove_port_dt','is_ported');
EXPORT FieldNum(SALT310.StrType fn) := CASE(fn,'country_code' => 0,'phone' => 1,'dial_type' => 2,'spid' => 3,'service_provider' => 4,'service_type' => 5,'routing_code' => 6,'porting_dt' => 7,'country_abbr' => 8,'filename' => 9,'file_dt_time' => 10,'vendor_first_reported_dt' => 11,'vendor_last_reported_dt' => 12,'port_start_dt' => 13,'port_end_dt' => 14,'remove_port_dt' => 15,'is_ported' => 16,0);
EXPORT SET OF SALT310.StrType FieldRules(UNSIGNED2 i) := CHOOSE(i,['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],[],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],[],[]);
...
EXPORT Make_dial_type(SALT310.StrType s0) := MakeFT_Invalid_DCT(s0);
EXPORT InValid_dial_type(SALT310.StrType s) := InValidFT_Invalid_DCT(s);
...
EXPORT Make_service_provider(SALT310.StrType s0) := s0;
EXPORT InValid_service_provider(SALT310.StrType s) := 0;
EXPORT InValidMessage_service_provider(UNSIGNED1 wh) := '';
 
EXPORT Make_service_type(SALT310.StrType s0) := MakeFT_Invalid_TOS(s0);
EXPORT InValid_service_type(SALT310.StrType s) := InValidFT_Invalid_TOS(s);
EXPORT InValidMessage_service_type(UNSIGNED1 wh) := InValidMessageFT_Invalid_TOS(wh);
 
EXPORT Make_routing_code(SALT310.StrType s0) := MakeFT_Invalid_Num_Blank(s0);
EXPORT InValid_routing_code(SALT310.StrType s) := InValidFT_Invalid_Num_Blank(s);
EXPORT InValidMessage_routing_code(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num_Blank(wh);
 
EXPORT Make_porting_dt(SALT310.StrType s0) := MakeFT_Invalid_Port_Date(s0);
EXPORT InValid_porting_dt(SALT310.StrType s) := InValidFT_Invalid_Port_Date(s);
...
EXPORT Make_file_dt_time(SALT310.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_file_dt_time(SALT310.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_file_dt_time(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
 
EXPORT Make_vendor_first_reported_dt(SALT310.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_vendor_first_reported_dt(SALT310.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_vendor_first_reported_dt(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
 
EXPORT Make_vendor_last_reported_dt(SALT310.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_vendor_last_reported_dt(SALT310.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_vendor_last_reported_dt(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
 
EXPORT Make_port_start_dt(SALT310.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_port_start_dt(SALT310.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_port_start_dt(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
 
EXPORT Make_port_end_dt(SALT310.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_port_end_dt(SALT310.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_port_end_dt(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
 
EXPORT Make_remove_port_dt(SALT310.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_remove_port_dt(SALT310.StrType s) := InValidFT_Invalid_Num(s);

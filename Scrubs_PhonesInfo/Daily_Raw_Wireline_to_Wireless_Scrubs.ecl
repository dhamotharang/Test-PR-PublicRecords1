IMPORT SALT310,STD;
EXPORT Daily_Raw_Wireline_to_Wireless_Scrubs := MODULE
...
// The module to handle the case where no scrubs exist
  ...
  EXPORT Expanded_Layout := RECORD(Daily_Raw_Wireline_to_Wireless_Layout_PhonesInfo)
    UNSIGNED1 phone_Invalid;
    UNSIGNED1 filename_Invalid;
  END;
  EXPORT  Bitmap_Layout := RECORD(Daily_Raw_Wireline_to_Wireless_Layout_PhonesInfo)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(Daily_Raw_Wireline_to_Wireless_Layout_PhonesInfo) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.phone_Invalid := Daily_Raw_Wireline_to_Wireless_Fields.InValid_phone((SALT310.StrType)le.phone);
    SELF.filename_Invalid := Daily_Raw_Wireline_to_Wireless_Fields.InValid_filename((SALT310.StrType)le.filename);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Daily_Raw_Wireline_to_Wireless_Layout_PhonesInfo);
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,Daily_Raw_Wireline_to_Wireless_Layout_PhonesInfo);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    AnyRule_WithErrorsCount := COUNT(GROUP, h.phone_Invalid > 0 OR h.filename_Invalid > 0);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.phone_ALLOW_ErrorCount > 0, 1, 0) + IF(le.filename_ALLOW_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.phone_ALLOW_ErrorCount > 0, 1, 0) + IF(le.filename_ALLOW_ErrorCount > 0, 1, 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Daily_Raw_Wireline_to_Wireless_Fields.InvalidMessage_phone(le.phone_Invalid),Daily_Raw_Wireline_to_Wireless_Fields.InvalidMessage_filename(le.filename_Invalid),'UNKNOWN'));
    ...
    SELF.FieldContents := CHOOSE(c,(SALT310.StrType)le.phone,(SALT310.StrType)le.filename,'***SALTBUG***');
  END;
  EXPORT AllErrors := NORMALIZE(h,2,Into(LEFT,COUNTER));
   bv := TABLE(AllErrors,{FieldContents, FieldName, Cnt := COUNT(GROUP)},FieldContents, FieldName,MERGE);
  EXPORT BadValues := TOPN(bv,1000,-Cnt);
  // Particular form of stats required for Orbit
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(Daily_Raw_Wireline_to_Wireless_Layout_PhonesInfo) prevDS = DATASET([], Daily_Raw_Wireline_to_Wireless_Layout_PhonesInfo), STRING10 Src='UNK'):= FUNCTION
  // field error stats
    SALT310.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ,Daily_Raw_Wireline_to_Wireless_Fields.InvalidMessage_phone(1)
          ,Daily_Raw_Wireline_to_Wireless_Fields.InvalidMessage_filename(1)
          ,'Fields with errors'
    ...
    mod_hygiene := Daily_Raw_Wireline_to_Wireless_hygiene(PROJECT(h, Daily_Raw_Wireline_to_Wireless_Layout_PhonesInfo));
  ...
  // record count stats
    ...
    mod_Delta := Daily_Raw_Wireline_to_Wireless_Delta(prevDS, PROJECT(h, Daily_Raw_Wireline_to_Wireless_Layout_PhonesInfo));
...
EXPORT StandardStats(DATASET(Daily_Raw_Wireline_to_Wireless_Layout_PhonesInfo) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  myTimeStamp := (UNSIGNED6)SALT310.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
  expandedFile := FromNone(inFile).ExpandedInfile;
  mod_fromexpandedOverall := FromExpanded(expandedFile);
  scrubsSummaryOverall := mod_fromexpandedOverall.SummaryStats;
 
  SALT310.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_PhonesInfo, Daily_Raw_Wireline_to_Wireless_Fields, 'RECORDOF(scrubsSummaryOverall)', '');

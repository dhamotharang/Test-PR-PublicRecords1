...
EXPORT FromNone(DATASET(Daily_Raw_Wireline_to_Wireless_Layout_PhonesInfo) h) := MODULE
  Expanded_Layout into(h le) := TRANSFORM
    SELF.phone_Invalid := Daily_Raw_Wireline_to_Wireless_Fields.InValid_phone((SALT31.StrType)le.phone);
    SELF.filename_Invalid := Daily_Raw_Wireline_to_Wireless_Fields.InValid_filename((SALT31.StrType)le.filename);
    SELF := le;
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT31.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Daily_Raw_Wireline_to_Wireless_Fields.InvalidMessage_filename(1),'UNKNOWN');

...
EXPORT Summary(SALT310.Str30Type  txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_activation_dt := MAX(GROUP,LENGTH(TRIM((SALT310.StrType)h.activation_dt)));
    avelength_activation_dt := AVE(GROUP,LENGTH(TRIM((SALT310.StrType)h.activation_dt)),h.activation_dt<>(typeof(h.activation_dt))'');
...
SALT310.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT310.StrType)le.reference_id),IF (le.dt_first_reported <> 0,TRIM((SALT310.StrType)le.dt_first_reported), ''),IF (le.dt_last_reported <> 0,TRIM((SALT310.StrType)le.dt_last_reported), ''),TRIM((SALT310.StrType)le.phone),TRIM((SALT310.StrType)le.reply_code),TRIM((SALT310.StrType)le.local_routing_number),TRIM((SALT310.StrType)le.account_owner),TRIM((SALT310.StrType)le.carrier_name),TRIM((SALT310.StrType)le.carrier_category),TRIM((SALT310.StrType)le.local_area_transport_area),TRIM((SALT310.StrType)le.point_code),TRIM((SALT310.StrType)le.serv),TRIM((SALT310.StrType)le.line),TRIM((SALT310.StrType)le.spid),TRIM((SALT310.StrType)le.operator_fullname),IF (le.activation_dt <> 0,TRIM((SALT310.StrType)le.activation_dt), ''),TRIM((SALT310.StrType)le.number_in_service),TRIM((SALT310.StrType)le.high_risk_indicator),TRIM((SALT310.StrType)le.prepaid)));
  SELF.FldNo := C;
END;
...
SALT310.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 19);
  SELF.FldNo2 := 1 + (C % 19);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT310.StrType)le.reference_id),IF (le.dt_first_reported <> 0,TRIM((SALT310.StrType)le.dt_first_reported), ''),IF (le.dt_last_reported <> 0,TRIM((SALT310.StrType)le.dt_last_reported), ''),TRIM((SALT310.StrType)le.phone),TRIM((SALT310.StrType)le.reply_code),TRIM((SALT310.StrType)le.local_routing_number),TRIM((SALT310.StrType)le.account_owner),TRIM((SALT310.StrType)le.carrier_name),TRIM((SALT310.StrType)le.carrier_category),TRIM((SALT310.StrType)le.local_area_transport_area),TRIM((SALT310.StrType)le.point_code),TRIM((SALT310.StrType)le.serv),TRIM((SALT310.StrType)le.line),TRIM((SALT310.StrType)le.spid),TRIM((SALT310.StrType)le.operator_fullname),IF (le.activation_dt <> 0,TRIM((SALT310.StrType)le.activation_dt), ''),TRIM((SALT310.StrType)le.number_in_service),TRIM((SALT310.StrType)le.high_risk_indicator),TRIM((SALT310.StrType)le.prepaid)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT310.StrType)le.reference_id),IF (le.dt_first_reported <> 0,TRIM((SALT310.StrType)le.dt_first_reported), ''),IF (le.dt_last_reported <> 0,TRIM((SALT310.StrType)le.dt_last_reported), ''),TRIM((SALT310.StrType)le.phone),TRIM((SALT310.StrType)le.reply_code),TRIM((SALT310.StrType)le.local_routing_number),TRIM((SALT310.StrType)le.account_owner),TRIM((SALT310.StrType)le.carrier_name),TRIM((SALT310.StrType)le.carrier_category),TRIM((SALT310.StrType)le.local_area_transport_area),TRIM((SALT310.StrType)le.point_code),TRIM((SALT310.StrType)le.serv),TRIM((SALT310.StrType)le.line),TRIM((SALT310.StrType)le.spid),TRIM((SALT310.StrType)le.operator_fullname),IF (le.activation_dt <> 0,TRIM((SALT310.StrType)le.activation_dt), ''),TRIM((SALT310.StrType)le.number_in_service),TRIM((SALT310.StrType)le.high_risk_indicator),TRIM((SALT310.StrType)le.prepaid)));
  END;
...
SHARED FldIds := DATASET([{1,'reference_id'}
      ...
      ,{19,'prepaid'}],SALT310.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    LIDBProcessed_Fields.InValid_reference_id((SALT310.StrType)le.reference_id),
    LIDBProcessed_Fields.InValid_dt_first_reported((SALT310.StrType)le.dt_first_reported),
    LIDBProcessed_Fields.InValid_dt_last_reported((SALT310.StrType)le.dt_last_reported),
    LIDBProcessed_Fields.InValid_phone((SALT310.StrType)le.phone),
    LIDBProcessed_Fields.InValid_reply_code((SALT310.StrType)le.reply_code),
    LIDBProcessed_Fields.InValid_local_routing_number((SALT310.StrType)le.local_routing_number),
    LIDBProcessed_Fields.InValid_account_owner((SALT310.StrType)le.account_owner),
    LIDBProcessed_Fields.InValid_carrier_name((SALT310.StrType)le.carrier_name),
    LIDBProcessed_Fields.InValid_carrier_category((SALT310.StrType)le.carrier_category),
    LIDBProcessed_Fields.InValid_local_area_transport_area((SALT310.StrType)le.local_area_transport_area),
    LIDBProcessed_Fields.InValid_point_code((SALT310.StrType)le.point_code),
    LIDBProcessed_Fields.InValid_serv((SALT310.StrType)le.serv),
    LIDBProcessed_Fields.InValid_line((SALT310.StrType)le.line),
    LIDBProcessed_Fields.InValid_spid((SALT310.StrType)le.spid),
    LIDBProcessed_Fields.InValid_operator_fullname((SALT310.StrType)le.operator_fullname),
    LIDBProcessed_Fields.InValid_activation_dt((SALT310.StrType)le.activation_dt),
    LIDBProcessed_Fields.InValid_number_in_service((SALT310.StrType)le.number_in_service),
    LIDBProcessed_Fields.InValid_high_risk_indicator((SALT310.StrType)le.high_risk_indicator),
    LIDBProcessed_Fields.InValid_prepaid((SALT310.StrType)le.prepaid),
    0);
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT310.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_PhonesInfo, LIDBProcessed_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

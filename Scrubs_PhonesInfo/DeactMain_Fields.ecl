...
EXPORT SALT33.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'Invalid_Action_Code','Invalid_Deact_Code','Invalid_Num','Invalid_Num_Blank','Invalid_Filename','Invalid_IS');
EXPORT FieldTypeNum(SALT33.StrType fn) := CASE(fn,'Invalid_Action_Code' => 1,'Invalid_Deact_Code' => 2,'Invalid_Num' => 3,'Invalid_Num_Blank' => 4,'Invalid_Filename' => 5,'Invalid_IS' => 6,0);
EXPORT MakeFT_Invalid_Action_Code(SALT33.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_Invalid_Action_Code(SALT33.StrType s) := WHICH(((SALT33.StrType) s) NOT IN ['DE','SW','SU','RE'],~(LENGTH(TRIM(s)) >= 1));
EXPORT InValidMessageFT_Invalid_Action_Code(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInEnum('DE|SW|SU|RE'),SALT33.HygieneErrors.NotLength('1..'),SALT33.HygieneErrors.Good);
EXPORT MakeFT_Invalid_Deact_Code(SALT33.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_Invalid_Deact_Code(SALT33.StrType s) := WHICH(((SALT33.StrType) s) NOT IN ['DE','SU',' ']);
EXPORT InValidMessageFT_Invalid_Deact_Code(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInEnum('DE|SU| '),SALT33.HygieneErrors.Good);
...
EXPORT MakeFT_Invalid_Filename(SALT33.StrType s0) := FUNCTION
  s1 := SALT33.stringfilter(s0,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_.:'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_Filename(SALT33.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_.:'))));
EXPORT InValidMessageFT_Invalid_Filename(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_.:'),SALT33.HygieneErrors.Good);
EXPORT MakeFT_Invalid_IS(SALT33.StrType s0) := FUNCTION
  s1 := SALT33.stringfilter(s0,'YNP '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_IS(SALT33.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'YNP '))));
EXPORT InValidMessageFT_Invalid_IS(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInChars('YNP '),SALT33.HygieneErrors.Good);
EXPORT SALT33.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'vendor_first_reported_dt','vendor_last_reported_dt','action_code','timestamp','phone','phone_swap','filename','carrier_name','filedate','swap_start_dt','swap_end_dt','deact_code','deact_start_dt','deact_end_dt','react_start_dt','react_end_dt','is_react','is_deact','porting_dt');
EXPORT FieldNum(SALT33.StrType fn) := CASE(fn,'vendor_first_reported_dt' => 0,'vendor_last_reported_dt' => 1,'action_code' => 2,'timestamp' => 3,'phone' => 4,'phone_swap' => 5,'filename' => 6,'carrier_name' => 7,'filedate' => 8,'swap_start_dt' => 9,'swap_end_dt' => 10,'deact_code' => 11,'deact_start_dt' => 12,'deact_end_dt' => 13,'react_start_dt' => 14,'react_end_dt' => 15,'is_react' => 16,'is_deact' => 17,'porting_dt' => 18,0);
//Individual field level validation
EXPORT Make_vendor_first_reported_dt(SALT33.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_vendor_first_reported_dt(SALT33.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_vendor_first_reported_dt(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
EXPORT Make_vendor_last_reported_dt(SALT33.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_vendor_last_reported_dt(SALT33.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_vendor_last_reported_dt(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
EXPORT Make_action_code(SALT33.StrType s0) := MakeFT_Invalid_Action_Code(s0);
EXPORT InValid_action_code(SALT33.StrType s) := InValidFT_Invalid_Action_Code(s);
EXPORT InValidMessage_action_code(UNSIGNED1 wh) := InValidMessageFT_Invalid_Action_Code(wh);
EXPORT Make_timestamp(SALT33.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_timestamp(SALT33.StrType s) := InValidFT_Invalid_Num(s);
...
EXPORT Make_phone_swap(SALT33.StrType s0) := MakeFT_Invalid_Num_Blank(s0);
EXPORT InValid_phone_swap(SALT33.StrType s) := InValidFT_Invalid_Num_Blank(s);
EXPORT InValidMessage_phone_swap(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num_Blank(wh);
EXPORT Make_filename(SALT33.StrType s0) := MakeFT_Invalid_Filename(s0);
EXPORT InValid_filename(SALT33.StrType s) := InValidFT_Invalid_Filename(s);
EXPORT InValidMessage_filename(UNSIGNED1 wh) := InValidMessageFT_Invalid_Filename(wh);
EXPORT Make_carrier_name(SALT33.StrType s0) := s0;
EXPORT InValid_carrier_name(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_carrier_name(UNSIGNED1 wh) := '';
EXPORT Make_filedate(SALT33.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_filedate(SALT33.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_filedate(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
EXPORT Make_swap_start_dt(SALT33.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_swap_start_dt(SALT33.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_swap_start_dt(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
EXPORT Make_swap_end_dt(SALT33.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_swap_end_dt(SALT33.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_swap_end_dt(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
EXPORT Make_deact_code(SALT33.StrType s0) := MakeFT_Invalid_Deact_Code(s0);
EXPORT InValid_deact_code(SALT33.StrType s) := InValidFT_Invalid_Deact_Code(s);
EXPORT InValidMessage_deact_code(UNSIGNED1 wh) := InValidMessageFT_Invalid_Deact_Code(wh);
EXPORT Make_deact_start_dt(SALT33.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_deact_start_dt(SALT33.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_deact_start_dt(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
EXPORT Make_deact_end_dt(SALT33.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_deact_end_dt(SALT33.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_deact_end_dt(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
EXPORT Make_react_start_dt(SALT33.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_react_start_dt(SALT33.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_react_start_dt(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
EXPORT Make_react_end_dt(SALT33.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_react_end_dt(SALT33.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_react_end_dt(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
EXPORT Make_is_react(SALT33.StrType s0) := MakeFT_Invalid_IS(s0);
EXPORT InValid_is_react(SALT33.StrType s) := InValidFT_Invalid_IS(s);
EXPORT InValidMessage_is_react(UNSIGNED1 wh) := InValidMessageFT_Invalid_IS(wh);
EXPORT Make_is_deact(SALT33.StrType s0) := MakeFT_Invalid_IS(s0);
EXPORT InValid_is_deact(SALT33.StrType s) := InValidFT_Invalid_IS(s);
EXPORT InValidMessage_is_deact(UNSIGNED1 wh) := InValidMessageFT_Invalid_IS(wh);
EXPORT Make_porting_dt(SALT33.StrType s0) := MakeFT_Invalid_Num_Blank(s0);
EXPORT InValid_porting_dt(SALT33.StrType s) := InValidFT_Invalid_Num_Blank(s);

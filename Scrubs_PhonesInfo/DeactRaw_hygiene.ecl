...
SALT33.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT33.StrType)le.action_code),TRIM((SALT33.StrType)le.timestamp),TRIM((SALT33.StrType)le.phone),TRIM((SALT33.StrType)le.phone_swap),TRIM((SALT33.StrType)le.filename)));
  SELF.FldNo := C;
END;
...
SALT33.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 5);
  SELF.FldNo2 := 1 + (C % 5);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT33.StrType)le.action_code),TRIM((SALT33.StrType)le.timestamp),TRIM((SALT33.StrType)le.phone),TRIM((SALT33.StrType)le.phone_swap),TRIM((SALT33.StrType)le.filename)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT33.StrType)le.action_code),TRIM((SALT33.StrType)le.timestamp),TRIM((SALT33.StrType)le.phone),TRIM((SALT33.StrType)le.phone_swap),TRIM((SALT33.StrType)le.filename)));
  END;
...
SHARED FldIds := DATASET([{1,'action_code'}
      ...
      ,{3,'phone'}
      ,{4,'phone_swap'}
      ,{5,'filename'}],SALT33.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    DeactRaw_Fields.InValid_action_code((SALT33.StrType)le.action_code),
    DeactRaw_Fields.InValid_timestamp((SALT33.StrType)le.timestamp),
    DeactRaw_Fields.InValid_phone((SALT33.StrType)le.phone),
    DeactRaw_Fields.InValid_phone_swap((SALT33.StrType)le.phone_swap),
    DeactRaw_Fields.InValid_filename((SALT33.StrType)le.filename),
    0);

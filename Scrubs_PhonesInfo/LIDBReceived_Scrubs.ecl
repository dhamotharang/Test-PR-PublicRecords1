...
EXPORT FromNone(DATASET(LIDBReceived_Layout_PhonesInfo) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.reference_id_Invalid := LIDBReceived_Fields.InValid_reference_id((SALT38.StrType)le.reference_id);
    SELF.phone_Invalid := LIDBReceived_Fields.InValid_phone((SALT38.StrType)le.phone);
    SELF.reply_code_Invalid := LIDBReceived_Fields.InValid_reply_code((SALT38.StrType)le.reply_code);
    SELF.local_routing_number_Invalid := LIDBReceived_Fields.InValid_local_routing_number((SALT38.StrType)le.local_routing_number);
    SELF.account_owner_Invalid := LIDBReceived_Fields.InValid_account_owner((SALT38.StrType)le.account_owner);
    SELF.carrier_name_Invalid := LIDBReceived_Fields.InValid_carrier_name((SALT38.StrType)le.carrier_name);
    SELF.carrier_category_Invalid := LIDBReceived_Fields.InValid_carrier_category((SALT38.StrType)le.carrier_category);
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.FieldContents := CHOOSE(c,(SALT38.StrType)le.reference_id,(SALT38.StrType)le.phone,(SALT38.StrType)le.reply_code,(SALT38.StrType)le.local_routing_number,(SALT38.StrType)le.account_owner,(SALT38.StrType)le.carrier_name,(SALT38.StrType)le.carrier_category,'***SALTBUG***');
  END;
...
EXPORT StandardStats(DATASET(LIDBReceived_Layout_PhonesInfo) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  ...
  SALT38.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_PhonesInfo, LIDBReceived_Fields, 'RECORDOF(scrubsSummaryOverall)', '');

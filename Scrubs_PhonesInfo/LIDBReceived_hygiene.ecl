...
SALT38.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT38.StrType)le.reference_id),TRIM((SALT38.StrType)le.phone),TRIM((SALT38.StrType)le.reply_code),TRIM((SALT38.StrType)le.local_routing_number),TRIM((SALT38.StrType)le.account_owner),TRIM((SALT38.StrType)le.carrier_name),TRIM((SALT38.StrType)le.carrier_category),TRIM((SALT38.StrType)le.local_area_transport_area),TRIM((SALT38.StrType)le.point_code)));
  SELF.FldNo := C;
END;
...
SALT38.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 9);
  SELF.FldNo2 := 1 + (C % 9);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT38.StrType)le.reference_id),TRIM((SALT38.StrType)le.phone),TRIM((SALT38.StrType)le.reply_code),TRIM((SALT38.StrType)le.local_routing_number),TRIM((SALT38.StrType)le.account_owner),TRIM((SALT38.StrType)le.carrier_name),TRIM((SALT38.StrType)le.carrier_category),TRIM((SALT38.StrType)le.local_area_transport_area),TRIM((SALT38.StrType)le.point_code)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT38.StrType)le.reference_id),TRIM((SALT38.StrType)le.phone),TRIM((SALT38.StrType)le.reply_code),TRIM((SALT38.StrType)le.local_routing_number),TRIM((SALT38.StrType)le.account_owner),TRIM((SALT38.StrType)le.carrier_name),TRIM((SALT38.StrType)le.carrier_category),TRIM((SALT38.StrType)le.local_area_transport_area),TRIM((SALT38.StrType)le.point_code)));
  END;
...
SHARED FldIds := DATASET([{1,'reference_id'}
      ...
      ,{9,'point_code'}],SALT38.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    LIDBReceived_Fields.InValid_reference_id((SALT38.StrType)le.reference_id),
    LIDBReceived_Fields.InValid_phone((SALT38.StrType)le.phone),
    LIDBReceived_Fields.InValid_reply_code((SALT38.StrType)le.reply_code),
    LIDBReceived_Fields.InValid_local_routing_number((SALT38.StrType)le.local_routing_number),
    LIDBReceived_Fields.InValid_account_owner((SALT38.StrType)le.account_owner),
    LIDBReceived_Fields.InValid_carrier_name((SALT38.StrType)le.carrier_name),
    LIDBReceived_Fields.InValid_carrier_category((SALT38.StrType)le.carrier_category),
    LIDBReceived_Fields.InValid_local_area_transport_area((SALT38.StrType)le.local_area_transport_area),
    LIDBReceived_Fields.InValid_point_code((SALT38.StrType)le.point_code),
    0);
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT38.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_PhonesInfo, LIDBReceived_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

...
SALT33.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT33.StrType)le.reference_id),TRIM((SALT33.StrType)le.phone),TRIM((SALT33.StrType)le.reply_code),TRIM((SALT33.StrType)le.local_routing_number),TRIM((SALT33.StrType)le.account_owner),TRIM((SALT33.StrType)le.carrier_name),TRIM((SALT33.StrType)le.carrier_category),TRIM((SALT33.StrType)le.local_area_transport_area),TRIM((SALT33.StrType)le.point_code)));
  SELF.FldNo := C;
END;
...
SALT33.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 9);
  SELF.FldNo2 := 1 + (C % 9);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT33.StrType)le.reference_id),TRIM((SALT33.StrType)le.phone),TRIM((SALT33.StrType)le.reply_code),TRIM((SALT33.StrType)le.local_routing_number),TRIM((SALT33.StrType)le.account_owner),TRIM((SALT33.StrType)le.carrier_name),TRIM((SALT33.StrType)le.carrier_category),TRIM((SALT33.StrType)le.local_area_transport_area),TRIM((SALT33.StrType)le.point_code)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT33.StrType)le.reference_id),TRIM((SALT33.StrType)le.phone),TRIM((SALT33.StrType)le.reply_code),TRIM((SALT33.StrType)le.local_routing_number),TRIM((SALT33.StrType)le.account_owner),TRIM((SALT33.StrType)le.carrier_name),TRIM((SALT33.StrType)le.carrier_category),TRIM((SALT33.StrType)le.local_area_transport_area),TRIM((SALT33.StrType)le.point_code)));
  END;
...
SHARED FldIds := DATASET([{1,'reference_id'}
      ...
      ,{9,'point_code'}],SALT33.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    LIDBReceived_Fields.InValid_reference_id((SALT33.StrType)le.reference_id),
    LIDBReceived_Fields.InValid_phone((SALT33.StrType)le.phone),
    LIDBReceived_Fields.InValid_reply_code((SALT33.StrType)le.reply_code),
    LIDBReceived_Fields.InValid_local_routing_number((SALT33.StrType)le.local_routing_number),
    LIDBReceived_Fields.InValid_account_owner((SALT33.StrType)le.account_owner),
    LIDBReceived_Fields.InValid_carrier_name((SALT33.StrType)le.carrier_name),
    LIDBReceived_Fields.InValid_carrier_category((SALT33.StrType)le.carrier_category),
    LIDBReceived_Fields.InValid_local_area_transport_area((SALT33.StrType)le.local_area_transport_area),
    LIDBReceived_Fields.InValid_point_code((SALT33.StrType)le.point_code),
    0);

...
EXPORT DeactGHMain_MAC_PopulationStatistics(infile,Ref='',Input_groupid = '',Input_vendor_first_reported_dt = '',Input_vendor_last_reported_dt = '',Input_action_code = '',Input_timestamp = '',Input_phone = '',Input_phone_swap = '',Input_filename = '',Input_carrier_name = '',Input_filedate = '',Input_swap_start_dt = '',Input_swap_end_dt = '',Input_deact_code = '',Input_deact_start_dt = '',Input_deact_end_dt = '',Input_react_start_dt = '',Input_react_end_dt = '',Input_is_react = '',Input_is_deact = '',Input_porting_dt = '',Input_pk_carrier_name = '',Input_days_apart = '',OutFile) := MACRO
  ...
  %of% %ot%(infile le) := TRANSFORM
    SELF.fields :=
    #IF( #TEXT(Input_groupid)='' )
      '' 
    #ELSE
        IF( le.Input_groupid = (TYPEOF(le.Input_groupid))'','',':groupid')
    #END
...
+    #IF( #TEXT(Input_pk_carrier_name)='' )
      '' 
    #ELSE
        IF( le.Input_pk_carrier_name = (TYPEOF(le.Input_pk_carrier_name))'','',':pk_carrier_name')
    #END

+    #IF( #TEXT(Input_days_apart)='' )
      '' 
    #ELSE
        IF( le.Input_days_apart = (TYPEOF(le.Input_days_apart))'','',':days_apart')
    #END
;
  END;
  #uniquename(op)

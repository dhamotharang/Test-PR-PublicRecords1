IMPORT ut,SALT31;
EXPORT Daily_Raw_Wireless_to_Wireline_hygiene(dataset(Daily_Raw_Wireless_to_Wireline_layout_PhonesInfo) h) := MODULE
...
EXPORT Summary(SALT31.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_lf := MAX(GROUP,LENGTH(TRIM((SALT31.StrType)h.lf)));
    avelength_lf := AVE(GROUP,LENGTH(TRIM((SALT31.StrType)h.lf)),h.lf<>(typeof(h.lf))'');
...
SALT31.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT31.StrType)le.phone),TRIM((SALT31.StrType)le.lf),TRIM((SALT31.StrType)le.filename)));
  SELF.FldNo := C;
END;
...
SALT31.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 3);
  SELF.FldNo2 := 1 + (C % 3);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT31.StrType)le.phone),TRIM((SALT31.StrType)le.lf),TRIM((SALT31.StrType)le.filename)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT31.StrType)le.phone),TRIM((SALT31.StrType)le.lf),TRIM((SALT31.StrType)le.filename)));
  END;
...
SHARED FldIds := DATASET([{1,'phone'}
      ,{2,'lf'}
      ,{3,'filename'}],SALT31.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    Daily_Raw_Wireless_to_Wireline_Fields.InValid_phone((SALT31.StrType)le.phone),
    Daily_Raw_Wireless_to_Wireline_Fields.InValid_lf((SALT31.StrType)le.lf),
    Daily_Raw_Wireless_to_Wireline_Fields.InValid_filename((SALT31.StrType)le.filename),
    0);
...
PrettyErrorTotals := RECORD
  FieldNme := Daily_Raw_Wireless_to_Wireline_Fields.FieldName(TotalErrors.FieldNum);
  FieldType := CHOOSE(TotalErrors.FieldNum,'Invalid_Phone','Unknown','Invalid_Filename');
  ErrorMessage := CHOOSE(TotalErrors.FieldNum,Daily_Raw_Wireless_to_Wireline_Fields.InValidMessage_phone(TotalErrors.ErrorNum),Daily_Raw_Wireless_to_Wireline_Fields.InValidMessage_lf(TotalErrors.ErrorNum),Daily_Raw_Wireless_to_Wireline_Fields.InValidMessage_filename(TotalErrors.ErrorNum));
  TotalErrors.Cnt;
END;

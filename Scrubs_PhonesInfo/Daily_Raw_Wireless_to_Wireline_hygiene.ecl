...
SALT38.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT38.StrType)le.phone),TRIM((SALT38.StrType)le.lf),TRIM((SALT38.StrType)le.filename)));
  SELF.FldNo := C;
END;
...
SALT38.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 3);
  SELF.FldNo2 := 1 + (C % 3);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT38.StrType)le.phone),TRIM((SALT38.StrType)le.lf),TRIM((SALT38.StrType)le.filename)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT38.StrType)le.phone),TRIM((SALT38.StrType)le.lf),TRIM((SALT38.StrType)le.filename)));
  END;
...
SHARED FldIds := DATASET([{1,'phone'}
      ,{2,'lf'}
      ,{3,'filename'}],SALT38.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    Daily_Raw_Wireless_to_Wireline_Fields.InValid_phone((SALT38.StrType)le.phone),
    Daily_Raw_Wireless_to_Wireline_Fields.InValid_lf((SALT38.StrType)le.lf),
    Daily_Raw_Wireless_to_Wireline_Fields.InValid_filename((SALT38.StrType)le.filename),
    0);
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT38.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_PhonesInfo, Daily_Raw_Wireless_to_Wireline_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

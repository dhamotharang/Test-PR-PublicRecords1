...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    Daily_Raw_Wireless_to_Wireline_Fields.InValid_phone((SALT39.StrType)le.phone),
    Daily_Raw_Wireless_to_Wireline_Fields.InValid_lf((SALT39.StrType)le.lf),
    Daily_Raw_Wireless_to_Wireline_Fields.InValid_filename((SALT39.StrType)le.filename),
    0);
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT39.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_PhonesInfo, Daily_Raw_Wireless_to_Wireline_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

...
EXPORT Summary(SALT39.Str30Type  txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_groupid := MAX(GROUP,LENGTH(TRIM((SALT39.StrType)h.groupid)));
    avelength_groupid := AVE(GROUP,LENGTH(TRIM((SALT39.StrType)h.groupid)),h.groupid<>(typeof(h.groupid))'');
    ...
    maxlength_filedate := MAX(GROUP,LENGTH(TRIM((SALT39.StrType)h.filedate)));
    avelength_filedate := AVE(GROUP,LENGTH(TRIM((SALT39.StrType)h.filedate)),h.filedate<>(typeof(h.filedate))'');
    ...
    maxlength_pk_carrier_name := MAX(GROUP,LENGTH(TRIM((SALT39.StrType)h.pk_carrier_name)));
    avelength_pk_carrier_name := AVE(GROUP,LENGTH(TRIM((SALT39.StrType)h.pk_carrier_name)),h.pk_carrier_name<>(typeof(h.pk_carrier_name))'');
    ...
    maxlength_days_apart := MAX(GROUP,LENGTH(TRIM((SALT39.StrType)h.days_apart)));
    avelength_days_apart := AVE(GROUP,LENGTH(TRIM((SALT39.StrType)h.days_apart)),h.days_apart<>(typeof(h.days_apart))'');
  END;
...
SALT39.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,IF (le.groupid <> 0,TRIM((SALT39.StrType)le.groupid), ''),IF (le.vendor_first_reported_dt <> 0,TRIM((SALT39.StrType)le.vendor_first_reported_dt), ''),IF (le.vendor_last_reported_dt <> 0,TRIM((SALT39.StrType)le.vendor_last_reported_dt), ''),TRIM((SALT39.StrType)le.action_code),TRIM((SALT39.StrType)le.timestamp),TRIM((SALT39.StrType)le.phone),TRIM((SALT39.StrType)le.phone_swap),TRIM((SALT39.StrType)le.filename),TRIM((SALT39.StrType)le.carrier_name),TRIM((SALT39.StrType)le.filedate),IF (le.swap_start_dt <> 0,TRIM((SALT39.StrType)le.swap_start_dt), ''),IF (le.swap_end_dt <> 0,TRIM((SALT39.StrType)le.swap_end_dt), ''),TRIM((SALT39.StrType)le.deact_code),IF (le.deact_start_dt <> 0,TRIM((SALT39.StrType)le.deact_start_dt), ''),IF (le.deact_end_dt <> 0,TRIM((SALT39.StrType)le.deact_end_dt), ''),IF (le.react_start_dt <> 0,TRIM((SALT39.StrType)le.react_start_dt), ''),IF (le.react_end_dt <> 0,TRIM((SALT39.StrType)le.react_end_dt), ''),TRIM((SALT39.StrType)le.is_react),TRIM((SALT39.StrType)le.is_deact),IF (le.porting_dt <> 0,TRIM((SALT39.StrType)le.porting_dt), ''),TRIM((SALT39.StrType)le.pk_carrier_name),IF (le.days_apart <> 0,TRIM((SALT39.StrType)le.days_apart), '')));
  SELF.FldNo := C;
END;
...
SALT39.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 22);
  SELF.FldNo2 := 1 + (C % 22);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,IF (le.groupid <> 0,TRIM((SALT39.StrType)le.groupid), ''),IF (le.vendor_first_reported_dt <> 0,TRIM((SALT39.StrType)le.vendor_first_reported_dt), ''),IF (le.vendor_last_reported_dt <> 0,TRIM((SALT39.StrType)le.vendor_last_reported_dt), ''),TRIM((SALT39.StrType)le.action_code),TRIM((SALT39.StrType)le.timestamp),TRIM((SALT39.StrType)le.phone),TRIM((SALT39.StrType)le.phone_swap),TRIM((SALT39.StrType)le.filename),TRIM((SALT39.StrType)le.carrier_name),TRIM((SALT39.StrType)le.filedate),IF (le.swap_start_dt <> 0,TRIM((SALT39.StrType)le.swap_start_dt), ''),IF (le.swap_end_dt <> 0,TRIM((SALT39.StrType)le.swap_end_dt), ''),TRIM((SALT39.StrType)le.deact_code),IF (le.deact_start_dt <> 0,TRIM((SALT39.StrType)le.deact_start_dt), ''),IF (le.deact_end_dt <> 0,TRIM((SALT39.StrType)le.deact_end_dt), ''),IF (le.react_start_dt <> 0,TRIM((SALT39.StrType)le.react_start_dt), ''),IF (le.react_end_dt <> 0,TRIM((SALT39.StrType)le.react_end_dt), ''),TRIM((SALT39.StrType)le.is_react),TRIM((SALT39.StrType)le.is_deact),IF (le.porting_dt <> 0,TRIM((SALT39.StrType)le.porting_dt), ''),TRIM((SALT39.StrType)le.pk_carrier_name),IF (le.days_apart <> 0,TRIM((SALT39.StrType)le.days_apart), '')));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,IF (le.groupid <> 0,TRIM((SALT39.StrType)le.groupid), ''),IF (le.vendor_first_reported_dt <> 0,TRIM((SALT39.StrType)le.vendor_first_reported_dt), ''),IF (le.vendor_last_reported_dt <> 0,TRIM((SALT39.StrType)le.vendor_last_reported_dt), ''),TRIM((SALT39.StrType)le.action_code),TRIM((SALT39.StrType)le.timestamp),TRIM((SALT39.StrType)le.phone),TRIM((SALT39.StrType)le.phone_swap),TRIM((SALT39.StrType)le.filename),TRIM((SALT39.StrType)le.carrier_name),TRIM((SALT39.StrType)le.filedate),IF (le.swap_start_dt <> 0,TRIM((SALT39.StrType)le.swap_start_dt), ''),IF (le.swap_end_dt <> 0,TRIM((SALT39.StrType)le.swap_end_dt), ''),TRIM((SALT39.StrType)le.deact_code),IF (le.deact_start_dt <> 0,TRIM((SALT39.StrType)le.deact_start_dt), ''),IF (le.deact_end_dt <> 0,TRIM((SALT39.StrType)le.deact_end_dt), ''),IF (le.react_start_dt <> 0,TRIM((SALT39.StrType)le.react_start_dt), ''),IF (le.react_end_dt <> 0,TRIM((SALT39.StrType)le.react_end_dt), ''),TRIM((SALT39.StrType)le.is_react),TRIM((SALT39.StrType)le.is_deact),IF (le.porting_dt <> 0,TRIM((SALT39.StrType)le.porting_dt), ''),TRIM((SALT39.StrType)le.pk_carrier_name),IF (le.days_apart <> 0,TRIM((SALT39.StrType)le.days_apart), '')));
  END;
...
SHARED FldIds := DATASET([{1,'groupid'}
      ...
      ,{20,'porting_dt'}
      ,{21,'pk_carrier_name'}
      ,{22,'days_apart'}],SALT39.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    DeactMain2_Fields.InValid_groupid((SALT39.StrType)le.groupid),
    DeactMain2_Fields.InValid_vendor_first_reported_dt((SALT39.StrType)le.vendor_first_reported_dt),
    DeactMain2_Fields.InValid_vendor_last_reported_dt((SALT39.StrType)le.vendor_last_reported_dt),
    DeactMain2_Fields.InValid_action_code((SALT39.StrType)le.action_code),
    DeactMain2_Fields.InValid_timestamp((SALT39.StrType)le.timestamp),
    DeactMain2_Fields.InValid_phone((SALT39.StrType)le.phone),
    DeactMain2_Fields.InValid_phone_swap((SALT39.StrType)le.phone_swap),
    DeactMain2_Fields.InValid_filename((SALT39.StrType)le.filename),
    DeactMain2_Fields.InValid_carrier_name((SALT39.StrType)le.carrier_name),
    DeactMain2_Fields.InValid_filedate((SALT39.StrType)le.filedate),
    DeactMain2_Fields.InValid_swap_start_dt((SALT39.StrType)le.swap_start_dt),
    DeactMain2_Fields.InValid_swap_end_dt((SALT39.StrType)le.swap_end_dt),
    DeactMain2_Fields.InValid_deact_code((SALT39.StrType)le.deact_code),
    DeactMain2_Fields.InValid_deact_start_dt((SALT39.StrType)le.deact_start_dt),
    DeactMain2_Fields.InValid_deact_end_dt((SALT39.StrType)le.deact_end_dt),
    DeactMain2_Fields.InValid_react_start_dt((SALT39.StrType)le.react_start_dt),
    DeactMain2_Fields.InValid_react_end_dt((SALT39.StrType)le.react_end_dt),
    DeactMain2_Fields.InValid_is_react((SALT39.StrType)le.is_react),
    DeactMain2_Fields.InValid_is_deact((SALT39.StrType)le.is_deact),
    DeactMain2_Fields.InValid_porting_dt((SALT39.StrType)le.porting_dt),
    DeactMain2_Fields.InValid_pk_carrier_name((SALT39.StrType)le.pk_carrier_name),
    DeactMain2_Fields.InValid_days_apart((SALT39.StrType)le.days_apart),
    0);
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT39.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_Phonesinfo, DeactMain2_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

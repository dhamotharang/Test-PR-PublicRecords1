...
SALT310.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT310.StrType)le.action_code),TRIM((SALT310.StrType)le.country_code),TRIM((SALT310.StrType)le.phone),TRIM((SALT310.StrType)le.dial_type),TRIM((SALT310.StrType)le.spid),TRIM((SALT310.StrType)le.service_type),TRIM((SALT310.StrType)le.routing_code),TRIM((SALT310.StrType)le.porting_dt),TRIM((SALT310.StrType)le.country_abbr),TRIM((SALT310.StrType)le.filename)));
  SELF.FldNo := C;
END;
...
SALT310.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 10);
  SELF.FldNo2 := 1 + (C % 10);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT310.StrType)le.action_code),TRIM((SALT310.StrType)le.country_code),TRIM((SALT310.StrType)le.phone),TRIM((SALT310.StrType)le.dial_type),TRIM((SALT310.StrType)le.spid),TRIM((SALT310.StrType)le.service_type),TRIM((SALT310.StrType)le.routing_code),TRIM((SALT310.StrType)le.porting_dt),TRIM((SALT310.StrType)le.country_abbr),TRIM((SALT310.StrType)le.filename)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT310.StrType)le.action_code),TRIM((SALT310.StrType)le.country_code),TRIM((SALT310.StrType)le.phone),TRIM((SALT310.StrType)le.dial_type),TRIM((SALT310.StrType)le.spid),TRIM((SALT310.StrType)le.service_type),TRIM((SALT310.StrType)le.routing_code),TRIM((SALT310.StrType)le.porting_dt),TRIM((SALT310.StrType)le.country_abbr),TRIM((SALT310.StrType)le.filename)));
  END;
...
SHARED FldIds := DATASET([{1,'action_code'}
      ...
      ,{8,'porting_dt'}
      ,{9,'country_abbr'}
      ,{10,'filename'}],SALT310.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    In_Port_Daily_Fields.InValid_action_code((SALT310.StrType)le.action_code),
    In_Port_Daily_Fields.InValid_country_code((SALT310.StrType)le.country_code),
    In_Port_Daily_Fields.InValid_phone((SALT310.StrType)le.phone),
    In_Port_Daily_Fields.InValid_dial_type((SALT310.StrType)le.dial_type),
    In_Port_Daily_Fields.InValid_spid((SALT310.StrType)le.spid),
    In_Port_Daily_Fields.InValid_service_type((SALT310.StrType)le.service_type),
    In_Port_Daily_Fields.InValid_routing_code((SALT310.StrType)le.routing_code),
    In_Port_Daily_Fields.InValid_porting_dt((SALT310.StrType)le.porting_dt),
    In_Port_Daily_Fields.InValid_country_abbr((SALT310.StrType)le.country_abbr),
    In_Port_Daily_Fields.InValid_filename((SALT310.StrType)le.filename),
    0);
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT310.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_PhonesInfo, In_Port_Daily_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

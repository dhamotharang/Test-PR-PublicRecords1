...
EXPORT Summary(SALT31.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_action_code := MAX(GROUP,LENGTH(TRIM((SALT31.StrType)h.action_code)));
    avelength_action_code := AVE(GROUP,LENGTH(TRIM((SALT31.StrType)h.action_code)),h.action_code<>(typeof(h.action_code))'');
    ...
    maxlength_dial_type := MAX(GROUP,LENGTH(TRIM((SALT31.StrType)h.dial_type)));
    avelength_dial_type := AVE(GROUP,LENGTH(TRIM((SALT31.StrType)h.dial_type)),h.dial_type<>(typeof(h.dial_type))'');
    populated_spid_pcnt := AVE(GROUP,IF(h.spid = (TYPEOF(h.spid))'',0,100));
    maxlength_spid := MAX(GROUP,LENGTH(TRIM((SALT31.StrType)h.spid)));
    avelength_spid := AVE(GROUP,LENGTH(TRIM((SALT31.StrType)h.spid)),h.spid<>(typeof(h.spid))'');
    ...
    maxlength_routing_code := MAX(GROUP,LENGTH(TRIM((SALT31.StrType)h.routing_code)));
    avelength_routing_code := AVE(GROUP,LENGTH(TRIM((SALT31.StrType)h.routing_code)),h.routing_code<>(typeof(h.routing_code))'');
    populated_porting_dt_pcnt := AVE(GROUP,IF(h.porting_dt = (TYPEOF(h.porting_dt))'',0,100));
    maxlength_porting_dt := MAX(GROUP,LENGTH(TRIM((SALT31.StrType)h.porting_dt)));
    avelength_porting_dt := AVE(GROUP,LENGTH(TRIM((SALT31.StrType)h.porting_dt)),h.porting_dt<>(typeof(h.porting_dt))'');
    populated_country_abbr_pcnt := AVE(GROUP,IF(h.country_abbr = (TYPEOF(h.country_abbr))'',0,100));
    maxlength_country_abbr := MAX(GROUP,LENGTH(TRIM((SALT31.StrType)h.country_abbr)));
    avelength_country_abbr := AVE(GROUP,LENGTH(TRIM((SALT31.StrType)h.country_abbr)),h.country_abbr<>(typeof(h.country_abbr))'');
    populated_filename_pcnt := AVE(GROUP,IF(h.filename = (TYPEOF(h.filename))'',0,100));
    maxlength_filename := MAX(GROUP,LENGTH(TRIM((SALT31.StrType)h.filename)));
    avelength_filename := AVE(GROUP,LENGTH(TRIM((SALT31.StrType)h.filename)),h.filename<>(typeof(h.filename))'');
  END;
...
SALT31.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT31.StrType)le.action_code),TRIM((SALT31.StrType)le.country_code),TRIM((SALT31.StrType)le.phone),TRIM((SALT31.StrType)le.dial_type),TRIM((SALT31.StrType)le.spid),TRIM((SALT31.StrType)le.service_type),TRIM((SALT31.StrType)le.routing_code),TRIM((SALT31.StrType)le.porting_dt),TRIM((SALT31.StrType)le.country_abbr),TRIM((SALT31.StrType)le.filename)));
  SELF.FldNo := C;
END;
...
SALT31.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 10);
  SELF.FldNo2 := 1 + (C % 10);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT31.StrType)le.action_code),TRIM((SALT31.StrType)le.country_code),TRIM((SALT31.StrType)le.phone),TRIM((SALT31.StrType)le.dial_type),TRIM((SALT31.StrType)le.spid),TRIM((SALT31.StrType)le.service_type),TRIM((SALT31.StrType)le.routing_code),TRIM((SALT31.StrType)le.porting_dt),TRIM((SALT31.StrType)le.country_abbr),TRIM((SALT31.StrType)le.filename)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT31.StrType)le.action_code),TRIM((SALT31.StrType)le.country_code),TRIM((SALT31.StrType)le.phone),TRIM((SALT31.StrType)le.dial_type),TRIM((SALT31.StrType)le.spid),TRIM((SALT31.StrType)le.service_type),TRIM((SALT31.StrType)le.routing_code),TRIM((SALT31.StrType)le.porting_dt),TRIM((SALT31.StrType)le.country_abbr),TRIM((SALT31.StrType)le.filename)));
  END;
...
SHARED FldIds := DATASET([{1,'action_code'}
      ...
      ,{8,'porting_dt'}
      ,{9,'country_abbr'}
      ,{10,'filename'}],SALT31.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    In_Port_Daily_Fields.InValid_action_code((SALT31.StrType)le.action_code),
    In_Port_Daily_Fields.InValid_country_code((SALT31.StrType)le.country_code),
    In_Port_Daily_Fields.InValid_phone((SALT31.StrType)le.phone),
    In_Port_Daily_Fields.InValid_dial_type((SALT31.StrType)le.dial_type),
    In_Port_Daily_Fields.InValid_spid((SALT31.StrType)le.spid),
    In_Port_Daily_Fields.InValid_service_type((SALT31.StrType)le.service_type),
    In_Port_Daily_Fields.InValid_routing_code((SALT31.StrType)le.routing_code),
    In_Port_Daily_Fields.InValid_porting_dt((SALT31.StrType)le.porting_dt),
    In_Port_Daily_Fields.InValid_country_abbr((SALT31.StrType)le.country_abbr),
    In_Port_Daily_Fields.InValid_filename((SALT31.StrType)le.filename),
    0);

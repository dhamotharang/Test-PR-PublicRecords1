...
SALT33.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT33.StrType)le.reference_id),TRIM((SALT33.StrType)le.phone)));
  SELF.FldNo := C;
END;
...
SALT33.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 2);
  SELF.FldNo2 := 1 + (C % 2);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT33.StrType)le.reference_id),TRIM((SALT33.StrType)le.phone)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT33.StrType)le.reference_id),TRIM((SALT33.StrType)le.phone)));
  END;
...
SHARED FldIds := DATASET([{1,'reference_id'}
      ,{2,'phone'}],SALT33.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    LIDBCurrent_Fields.InValid_reference_id((SALT33.StrType)le.reference_id),
    LIDBCurrent_Fields.InValid_phone((SALT33.StrType)le.phone),
    0);

...
SALT38.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT38.StrType)le.reference_id),TRIM((SALT38.StrType)le.phone)));
  SELF.FldNo := C;
END;
...
SALT38.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 2);
  SELF.FldNo2 := 1 + (C % 2);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT38.StrType)le.reference_id),TRIM((SALT38.StrType)le.phone)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT38.StrType)le.reference_id),TRIM((SALT38.StrType)le.phone)));
  END;
...
SHARED FldIds := DATASET([{1,'reference_id'}
      ,{2,'phone'}],SALT38.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    LIDBCurrent_Fields.InValid_reference_id((SALT38.StrType)le.reference_id),
    LIDBCurrent_Fields.InValid_phone((SALT38.StrType)le.phone),
    0);
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT38.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_PhonesInfo, LIDBCurrent_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

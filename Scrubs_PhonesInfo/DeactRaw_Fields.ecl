...
EXPORT SALT33.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'Invalid_Action_Code','Invalid_Num','Invalid_Num_Blank','Invalid_Filename');
EXPORT FieldTypeNum(SALT33.StrType fn) := CASE(fn,'Invalid_Action_Code' => 1,'Invalid_Num' => 2,'Invalid_Num_Blank' => 3,'Invalid_Filename' => 4,0);
EXPORT MakeFT_Invalid_Action_Code(SALT33.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_Invalid_Action_Code(SALT33.StrType s) := WHICH(((SALT33.StrType) s) NOT IN ['DE','SW','SU','RE']);
EXPORT InValidMessageFT_Invalid_Action_Code(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInEnum('DE|SW|SU|RE'),SALT33.HygieneErrors.Good);
...
EXPORT SALT33.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'action_code','timestamp','phone','phone_swap','filename');
EXPORT FieldNum(SALT33.StrType fn) := CASE(fn,'action_code' => 0,'timestamp' => 1,'phone' => 2,'phone_swap' => 3,'filename' => 4,0);

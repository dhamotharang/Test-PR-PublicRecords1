...
EXPORT SALT310.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'Invalid_Num','Invalid_AccOwn','Invalid_RefID','Invalid_Char','Invalid_Binary','Invalid_Num_In_Service','Invalid_Line_Serv');
EXPORT FieldTypeNum(SALT310.StrType fn) := CASE(fn,'Invalid_Num' => 1,'Invalid_AccOwn' => 2,'Invalid_RefID' => 3,'Invalid_Char' => 4,'Invalid_Binary' => 5,'Invalid_Num_In_Service' => 6,'Invalid_Line_Serv' => 7,0);
...
EXPORT MakeFT_Invalid_AccOwn(SALT310.StrType s0) := FUNCTION
  s1 := SALT310.stringfilter(s0,'ABHGCDFE0123456789 '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_AccOwn(SALT310.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT310.StringFilter(s,'ABHGCDFE0123456789 '))));
EXPORT InValidMessageFT_Invalid_AccOwn(UNSIGNED1 wh) := CHOOSE(wh,SALT310.HygieneErrors.NotInChars('ABHGCDFE0123456789 '),SALT310.HygieneErrors.Good);
 
EXPORT MakeFT_Invalid_RefID(SALT310.StrType s0) := FUNCTION
  s1 := SALT310.stringfilter(s0,'MGP0123456789 '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_RefID(SALT310.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT310.StringFilter(s,'MGP0123456789 '))));
EXPORT InValidMessageFT_Invalid_RefID(UNSIGNED1 wh) := CHOOSE(wh,SALT310.HygieneErrors.NotInChars('MGP0123456789 '),SALT310.HygieneErrors.Good);
 
EXPORT MakeFT_Invalid_Char(SALT310.StrType s0) := FUNCTION
  s1 := SALT310.stringfilter(s0,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 ()&-.,/-#@\'!*'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_Char(SALT310.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT310.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 ()&-.,/-#@\'!*'))));
EXPORT InValidMessageFT_Invalid_Char(UNSIGNED1 wh) := CHOOSE(wh,SALT310.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 ()&-.,/-#@\'!*'),SALT310.HygieneErrors.Good);
 
EXPORT MakeFT_Invalid_Binary(SALT310.StrType s0) := FUNCTION
  s1 := SALT310.stringfilter(s0,'1 '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_Binary(SALT310.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT310.StringFilter(s,'1 '))));
EXPORT InValidMessageFT_Invalid_Binary(UNSIGNED1 wh) := CHOOSE(wh,SALT310.HygieneErrors.NotInChars('1 '),SALT310.HygieneErrors.Good);
 
EXPORT MakeFT_Invalid_Num_In_Service(SALT310.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_Invalid_Num_In_Service(SALT310.StrType s) := WHICH(((SALT310.StrType) s) NOT IN ['1A','2A','3A','4A','7A','10A','12A','1I','2I','3I','4I','7I','10I','12I','U',' ']);
EXPORT InValidMessageFT_Invalid_Num_In_Service(UNSIGNED1 wh) := CHOOSE(wh,SALT310.HygieneErrors.NotInEnum('1A|2A|3A|4A|7A|10A|12A|1I|2I|3I|4I|7I|10I|12I|U| '),SALT310.HygieneErrors.Good);
 
EXPORT MakeFT_Invalid_Line_Serv(SALT310.StrType s0) := FUNCTION
  s1 := SALT310.stringfilter(s0,'0123 '); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_Line_Serv(SALT310.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT310.StringFilter(s,'0123 '))));
EXPORT InValidMessageFT_Invalid_Line_Serv(UNSIGNED1 wh) := CHOOSE(wh,SALT310.HygieneErrors.NotInChars('0123 '),SALT310.HygieneErrors.Good);
...
EXPORT SALT310.StrType FlatName(UNSIGNED2 i) := CHOOSE(i,'reference_id','dt_first_reported','dt_last_reported','phone','reply_code','local_routing_number','account_owner','carrier_name','carrier_category','local_area_transport_area','point_code','serv','line','spid','operator_fullname','activation_dt','number_in_service','high_risk_indicator','prepaid');
...
EXPORT SET OF SALT310.StrType FieldRules(UNSIGNED2 i) := CHOOSE(i,['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],[],[],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ALLOW'],['ENUM'],['ALLOW'],['ALLOW'],[]);
...
EXPORT Make_reference_id(SALT310.StrType s0) := MakeFT_Invalid_RefID(s0);
EXPORT InValid_reference_id(SALT310.StrType s) := InValidFT_Invalid_RefID(s);
EXPORT InValidMessage_reference_id(UNSIGNED1 wh) := InValidMessageFT_Invalid_RefID(wh);
 
EXPORT Make_dt_first_reported(SALT310.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_dt_first_reported(SALT310.StrType s) := InValidFT_Invalid_Num(s);
EXPORT InValidMessage_dt_first_reported(UNSIGNED1 wh) := InValidMessageFT_Invalid_Num(wh);
 
EXPORT Make_dt_last_reported(SALT310.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_dt_last_reported(SALT310.StrType s) := InValidFT_Invalid_Num(s);
...
EXPORT Make_account_owner(SALT310.StrType s0) := MakeFT_Invalid_AccOwn(s0);
EXPORT InValid_account_owner(SALT310.StrType s) := InValidFT_Invalid_AccOwn(s);
EXPORT InValidMessage_account_owner(UNSIGNED1 wh) := InValidMessageFT_Invalid_AccOwn(wh);
 
EXPORT Make_carrier_name(SALT310.StrType s0) := MakeFT_Invalid_Char(s0);
EXPORT InValid_carrier_name(SALT310.StrType s) := InValidFT_Invalid_Char(s);
...
EXPORT Make_local_area_transport_area(SALT310.StrType s0) := s0;
EXPORT InValid_local_area_transport_area(SALT310.StrType s) := 0;
...
EXPORT Make_serv(SALT310.StrType s0) := MakeFT_Invalid_Line_Serv(s0);
EXPORT InValid_serv(SALT310.StrType s) := InValidFT_Invalid_Line_Serv(s);
EXPORT InValidMessage_serv(UNSIGNED1 wh) := InValidMessageFT_Invalid_Line_Serv(wh);
 
EXPORT Make_line(SALT310.StrType s0) := MakeFT_Invalid_Line_Serv(s0);
EXPORT InValid_line(SALT310.StrType s) := InValidFT_Invalid_Line_Serv(s);
...
EXPORT Make_activation_dt(SALT310.StrType s0) := MakeFT_Invalid_Num(s0);
EXPORT InValid_activation_dt(SALT310.StrType s) := InValidFT_Invalid_Num(s);
...
EXPORT Make_high_risk_indicator(SALT310.StrType s0) := MakeFT_Invalid_Binary(s0);
EXPORT InValid_high_risk_indicator(SALT310.StrType s) := InValidFT_Invalid_Binary(s);
EXPORT InValidMessage_high_risk_indicator(UNSIGNED1 wh) := InValidMessageFT_Invalid_Binary(wh);
 
EXPORT Make_prepaid(SALT310.StrType s0) := MakeFT_Invalid_Binary(s0);
EXPORT InValid_prepaid(SALT310.StrType s) := InValidFT_Invalid_Binary(s);

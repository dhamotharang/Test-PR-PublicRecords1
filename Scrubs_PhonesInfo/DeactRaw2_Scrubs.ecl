...
// The module to handle the case where no scrubs exist
  ...
  END;
  EXPORT Rule_Layout := RECORD(DeactRaw2_Layout_PhonesInfo)
...
EXPORT FromNone(DATASET(DeactRaw2_Layout_PhonesInfo) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.msisdn_Invalid := DeactRaw2_Fields.InValid_msisdn((SALT311.StrType)le.msisdn);
    SELF.timestamp_Invalid := DeactRaw2_Fields.InValid_timestamp((SALT311.StrType)le.timestamp);
    SELF.changeid_Invalid := DeactRaw2_Fields.InValid_changeid((SALT311.StrType)le.changeid);
    SELF.operatorid_Invalid := DeactRaw2_Fields.InValid_operatorid((SALT311.StrType)le.operatorid);
    SELF.msisdneid_Invalid := DeactRaw2_Fields.InValid_msisdneid((SALT311.StrType)le.msisdneid);
    SELF.msisdnnew_Invalid := DeactRaw2_Fields.InValid_msisdnnew((SALT311.StrType)le.msisdnnew);
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.FieldContents := CHOOSE(c,(SALT311.StrType)le.msisdn,(SALT311.StrType)le.timestamp,(SALT311.StrType)le.changeid,(SALT311.StrType)le.operatorid,(SALT311.StrType)le.msisdneid,(SALT311.StrType)le.msisdnnew,'***SALTBUG***');
  END;
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ...
          ,le.operatorid_ALLOW_ErrorCount
          ,le.msisdneid_ALLOW_ErrorCount
          ,le.msisdnnew_ALLOW_ErrorCount,0) / le.TotalCnt, CHOOSE(c - NumRules
...
EXPORT StandardStats(DATASET(DeactRaw2_Layout_PhonesInfo) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_PhonesInfo, DeactRaw2_Fields, 'RECORDOF(scrubsSummaryOverall)', '');

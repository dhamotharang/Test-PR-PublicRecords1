...
export fDCA_as_Business_Contact(dataset(Layout_Pub_In) pDCA_Base) :=
function

	dca_contacts := pDCA_Base;
	...
	Business_Header.Layout_Business_Contact_Full_New Translate_dca_to_BCF(Layout_pub_slim l, integer CTR) := TRANSFORM
	...
	SELF.city := CHOOSE(CTR, L.v_city_name, L.v_city_nameA, L.v_city_name, L.v_city_nameA);
	...
	SELF.company_city := CHOOSE(CTR, L.v_city_name, L.v_city_nameA, L.v_city_nameA, L.v_city_name);
	...
	self.company_phone := (UNSIGNED6)((UNSIGNED8)if(L.state <> '' or L.country = '', Address.CleanPhone(L.Phone), ''));
	self.company_fein := 0;
	...
	return from_dca_dedup((integer)name_score < 3, Business_Header.CheckPersonName(fname, mname, lname, name_suffix));

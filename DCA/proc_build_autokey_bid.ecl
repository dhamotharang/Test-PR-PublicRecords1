...
EXPORT Proc_Build_Autokey_bid(STRING filedate) :=
	FUNCTION

		cnst       := DCA.Constants(filedate);
		ak_dataset := cnst.ak_dataset_bid;
		ak_keyname := cnst.ak_keyname_bid; // @version@
		ak_logical := cnst.ak_logical_bid;
		ak_skipSet := cnst.ak_skipSet_bid;
		ak_typeStr := cnst.ak_typeStr_bid;
		
		jobComplete := FileServices.sendemail(_control.MyInfo.EmailAddressNotify, ' DCA Roxie BID autokey build succeeded.', '');									 
		jobFailed   := FileServices.sendemail(_control.MyInfo.EmailAddressNotify, ' DCA Roxie BID autokey build failed - ' + filedate, failmessage);		

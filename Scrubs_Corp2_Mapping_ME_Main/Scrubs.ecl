...
EXPORT FromNone(DATASET(Corp2_Mapping.LayoutsCommon.Main) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_status_cd_Invalid := Fields.InValid_corp_status_cd((SALT34.StrType)le.corp_status_cd,(SALT34.StrType)le.recordorigin);
    SELF.corp_status_desc_Invalid := Fields.InValid_corp_status_desc((SALT34.StrType)le.corp_status_desc,(SALT34.StrType)le.recordorigin);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_ln_name_type_desc_Invalid << 22 ) + ( le.corp_address1_line3_Invalid << 23 ) + ( le.corp_filing_date_Invalid << 24 ) + ( le.corp_status_cd_Invalid << 26 ) + ( le.corp_status_desc_Invalid << 27 ) + ( le.corp_inc_state_Invalid << 28 ) + ( le.corp_inc_date_Invalid << 29 ) + ( le.corp_term_exist_exp_Invalid << 31 ) + ( le.corp_forgn_state_cd_Invalid << 33 ) + ( le.corp_forgn_state_desc_Invalid << 34 ) + ( le.corp_forgn_date_Invalid << 35 ) + ( le.corp_orig_org_structure_desc_Invalid << 37 ) + ( le.corp_country_of_formation_Invalid << 38 ) + ( le.recordorigin_Invalid << 39 ) + ( le.internalfield1_Invalid << 40 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_orig_org_structure_desc_Invalid := (le.ScrubsBits1 >> 37) & 1;
    SELF.corp_country_of_formation_Invalid := (le.ScrubsBits1 >> 38) & 1;
    SELF.recordorigin_Invalid := (le.ScrubsBits1 >> 39) & 1;
    SELF.internalfield1_Invalid := (le.ScrubsBits1 >> 40) & 1;
    SELF := le;
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT34.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_forgn_state_desc:invalid_alpha:ALLOW'
          ...
          ,'corp_country_of_formation:invalid_alpha:ALLOW'

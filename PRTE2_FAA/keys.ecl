...
export key_Certifications(boolean IsFCRA = false) := index(fAirmenCert,{uid := fAirmenCert.unique_id},{fAirmenCert},
...
//Airmen Keys
export key_airmen_did(boolean IsFCRA = false)		:= index(files.Airmen_key((integer)did_out != 0), 
																													{UNSIGNED8 did := (UNSIGNED8)did_out, airmen_id}, {files.Airmen_key}, 
...
export key_aircraft_id(boolean IsFCRA = false)				:= index(fAircraftReg_withPersist, {aircraft_id}, {fAircraftReg_withPersist}, 
																																Data_Services.Data_location.Prefix('FAA')+ if(isFCRA, Constants.KEY_PREFIX, Constants.KEY_PREFIX) + doxie.Version_SuperKey + '::aircraft_id');

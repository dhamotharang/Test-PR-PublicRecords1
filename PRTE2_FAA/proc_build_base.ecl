 import ut, PromoteSupers, Data_Services, prte2, BIPV2,prte_bip, AID, Address, AID_Support,std, mdr;
...
//Appending source_rec_id for new Aircraft records
ut.MAC_Append_Rcid(dsClnAircraft,source_rec_id,Append_Aircraft);


//Existing Records
dsAirmenExistingRecords  	:= dsClnAirmen(cust_name = '');
dsAircraftExistingRecords := Append_Aircraft(cust_name = '');


//New Records
dsAirmenNewRecords 		:= dsClnAirmen(cust_name <> '');
dsAircraftNewRecords 	:= Append_Aircraft(cust_name <> '');

//Prepping Airmen/Aircraft Records for Address Cleaner
PrepAddrLayout := record
	// string2   filetype; //AR- aircraft; AM- airmen
	string33 	street;
	string33 	street2;
	string18 	city;
	string5 	state;
...
Layouts.Airmen  AppendCleanAddrAirmen(dsAirmenNewRecords l, dCleanAddressAppended r) := transform
	self.dpbc 				:= r.dbpc;
...
j1 := join(dsAirmenNewRecords, dCleanAddressAppended,
...
Layouts.aircraft_registration  AppendCleanAddrAircraft(dsAircraftNewRecords l,  dCleanAddressAppended r) := transform
	self.dpbc 				:= r.dbpc;
	self.ace_fips_st	:= r.county[1..2];
	self.county				:= r.county[3..5];
	self := r;
...
j2 := join(dsAircraftNewRecords, dCleanAddressAppended,
...
//Append ID(s)
self.did	 						:= prte2.fn_AppendFakeID.did(self.fname, self.lname, pInput.best_ssn, pInput.link_dob, pInput.cust_name);
...
//Appending aircraft_id
dsAircraft_srt := sort(dsSearchAircraftReg, source_rec_id);
ut.MAC_Sequence_Records_NewRec(dsAircraft_srt,Layouts.Search_Aircraft,aircraft_id,dsAircraftRegSeq);

PromoteSupers.Mac_SF_BuildProcess(dsSearchAirmen,Constants.base_prefix_name+'airmen',airmen_out,2,,true);
PromoteSupers.Mac_SF_BuildProcess(dsAircraftRegSeq,Constants.base_prefix_name+'aircraft_reg',aircraft_reg_out,2,,true);

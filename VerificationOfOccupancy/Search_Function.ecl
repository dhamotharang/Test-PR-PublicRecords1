...
EXPORT Search_Function(DATASET(VerificationOfOccupancy.Layouts.Layout_VOOIn) VOO_In, 
																						...
																						boolean PAR_request = FALSE) := MODULE
...
// ********************************************************************************************************************************
	...
	SHARED VerificationOfOccupancy.Layouts.Layout_VOOShell getQHeaderDates(Risk_Indicators.Layout_Output l, header_quick.key_DID r) := transform
		addrmatch							:= trim(l.z5) = trim(r.zip) and
														 trim(l.prim_range) = trim(r.prim_range) and
														 ut.NNEQ(trim(l.sec_range), trim(r.sec_range)) and
														 trim(l.prim_name) = trim(r.prim_name) and
...
//end new join
	...
	SHARED VerificationOfOccupancy.Layouts.Layout_VOOShell getQHeader(Risk_Indicators.Layout_Output l, header_quick.key_DID r) := transform
		addrmatch							:= trim(l.z5) = trim(r.zip) and
														 trim(l.prim_range) = trim(r.prim_range) and
														 ut.NNEQ(trim(l.sec_range), trim(r.sec_range)) and
														 trim(l.prim_name) = trim(r.prim_name) and
...
// *******************************************************************************************************************
	...
	SHARED layout_inq get_inquiry(with_srcSeen le, Inquiry_AccLogs.Key_Inquiry_DID ri) := TRANSFORM
		self.seq 	:= le.seq;
		self 			:= ri;
	END;	
	...
	SHARED layout_inq get_inquiry_update(with_srcSeen le, Inquiry_AccLogs.Key_Inquiry_DID_Update ri) := TRANSFORM
		self.seq 	:= le.seq;
		self 			:= ri;
	END;
	...
	SHARED inquiry_update_Recs := join(with_srcSeen, Inquiry_AccLogs.Key_Inquiry_DID_Update, 
						...
						trim(right.search_info.function_description) in Inquiry_AccLogs.shell_constants.VOO_search_functions and
						// filter out 'collections' to coincide with Shell 5 (mimic Risk_Indicators.Boca_Shell_Inquiries.isCollection)
						trim(StringLib.StringToUpperCase(right.bus_intel.vertical)) <> 'COLLECTIONS' and 
						trim(StringLib.StringToUpperCase(right.bus_intel.vertical)) <> 'RECEIVABLES MANAGEMENT' and 
						trim(StringLib.StringToUpperCase(right.bus_intel.industry)) not IN Inquiry_AccLogs.shell_constants.collection_industry and
						StringLib.StringFind(StringLib.StringToUpperCase(trim(StringLib.StringToUpperCase(right.bus_intel.sub_market))),'FIRST PARTY', 1) < 1 and 
						// filter out 'highriskcredit' to coincide with Shell 5 (mimic Risk_Indicators.Boca_Shell_Inquiries.isHighRiskCredit)
						trim(StringLib.StringToUpperCase(right.bus_intel.industry)) not IN Inquiry_AccLogs.shell_constants.HighRiskCredit_industry5,
...
//keep only one inquiry record per unique address
	SHARED dedup_inquiries := dedup(sort(ungroup(inquiry_recs) & ungroup(inquiry_update_Recs), seq, s_did, person_q.zip5, person_q.prim_range, person_q.sec_range, person_q.prim_name, person_q.addr_suffix, person_q.predir, person_q.postdir), 
...
// Go get utility data and append to VOO shell
	...
	SHARED utility_data := VerificationOfOccupancy.getUtility(roll_allHeader, glba, isUtility);
...
// Go get prior resident data and append to VOO shell
	SHARED priorRes_data := VerificationOfOccupancy.getPriorResident(roll_allHeader, DataRestrictionMask, glb_ok, dppa, isUtility, dppa_ok, fares_ok);
...
// Go get necessary data for determining the 'PriorAddressMoveIndex' attribute and append to VOOShell
	SHARED priorAddr_data := VerificationOfOccupancy.getPriorAddress(roll_allHeader, sort_allHeader, DataRestrictionMask, glb_ok, dppa, isUtility, dppa_ok, fares_ok);
...
// *******************************************************************************************************************
	...
	SHARED VerificationOfOccupancy.Layouts.Layout_VOOShell getDOD(with_otherProximity l, Doxie.key_death_masterV2_DID r) := transform
...
// *******************************************************************************************************************
	...
	SHARED VerificationOfOccupancy.Layouts.Layout_VOOShell getRelatives(roll_allHeader le, rolledHdrdids ri) := TRANSFORM
		self.relativeTitle	:= StringLib.StringToUpperCase(TRIM(Header.relative_titles.fn_get_str_title(ri.title)));
	...
	SHARED VerificationOfOccupancy.Layouts.Layout_VOOShell getQHeaderRel(VerificationOfOccupancy.Layouts.Layout_VOOShell l, header_quick.key_DID r) := transform
		addrmatch							:= trim(l.z5) = trim(r.zip) and
														 trim(l.prim_range) = trim(r.prim_range) and
														 ut.NNEQ(trim(l.sec_range), trim(r.sec_range)) and
														 trim(l.prim_name) = trim(r.prim_name) and
...
// *******************************************************************************************************************
  SHARED attributes := if(StringLib.StringToUpperCase(AttributesVersion) in ['VOOATTRV1', 'PARATTRV1'] and glb_ok, //if valid attributes version requested, go get attributes
									 VerificationOfOccupancy.getAttributes(final_VOO_Shell, DataRestrictionMask, glb_ok, dppa_ok),
									 emptyAttr);  

	SHARED wScore := if(StringLib.StringToUpperCase(AttributesVersion) in ['VOOATTRV1', 'PARATTRV1'] and IncludeModel and glb_ok,		//if score is requested, go append it and the Inferred Ownership attribute, else just return attributes
							 VerificationOfOccupancy.getscore(attributes, final_VOO_Shell, DataRestrictionMask, glb_ok, dppa, isUtility, dppa_ok), 
							 attributes);
							 
	SHARED wVOOReport := if(IncludeReport,
								VerificationOfOccupancy.getReport(final_VOO_Shell, wScore, DataRestrictionMask, glba, dppa, isUtility, IncludeReport, fares_ok),
	...
	SHARED wPARReport := if(IncludeReport,
								VerificationOfOccupancy.getReportPAR(final_VOO_Shell, wScore, DataRestrictionMask, glba, dppa, isUtility, IncludeReport, fares_ok),

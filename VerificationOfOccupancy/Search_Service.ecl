...
/*--SOAP--
<message name="Verification Of Occupancy">
	<part name="VerificationOfOccupancyRequest" type="tns:XmlDataSet" cols="80" rows="50"/>
...
/* **********************************************
	...
	Industry_Search := Inquiry_AccLogs.Key_Inquiry_industry_use_vertical(FALSE)(s_company_id = CompanyID and s_product_id = (String)Risk_Reporting.ProductID.VerificationOfOccupancy__Search_Service);
/* ************* End Scout Fields **************/
  ...
  STRING50 DataPermission := MAP(TRIM(userIn.DataPermissionMask) <> '' => userIn.DataPermissionMask,
	...
	boolean RequestValid 	:= attributesVersion in ['VOOATTRV1'];  //version 1 is the initial version
	...
	VOO_Input := PROJECT(wseq, into(left));	
...
iesp.share.t_NameValuePair createrec(searchResults le, integer C) := TRANSFORM
			self.Name:= case( C,
				...
				5 	=> 'AddressOwnershipHistoryIndex',
				6 	=> 'AddressPropertyTypeIndex', 
				7 	=> 'AddressValidityIndex', 
				8 	=> 'RelativesConfirmingAddressIndex', 
				9 	=> 'AddressOwnerMailingAddressIndex', 
				10 	=> 'PriorAddressMoveIndex',  
				11 	=> 'PriorResidentMoveIndex',  
				12 	=> 'AddressDateFirstSeen',  
				13 	=> 'AddressDateLastSeen',  
				14 	=> 'OccupancyOverride',  
				15 	=> 'InferredOwnershipTypeIndex',  
				16 	=> 'OtherOwnedPropertyProximity',  
				17 	=> 'VerificationOfOccupancyScore',  
               'Invalid');
			self.Value:=  case(C,
				...
				5 	=> le.attributes.version1.AddressOwnershipHistoryIndex ,
				6 	=> le.attributes.version1.AddressPropertyTypeIndex ,
				7 	=> le.attributes.version1.AddressValidityIndex ,
				8 	=> le.attributes.version1.RelativesConfirmingAddressIndex ,
				9 	=> le.attributes.version1.AddressOwnerMailingAddressIndex ,
				10 	=> le.attributes.version1.PriorAddressMoveIndex ,
				11 	=> le.attributes.version1.PriorResidentMoveIndex ,
				12 	=> le.attributes.version1.AddressDateFirstSeen ,
				13 	=> le.attributes.version1.AddressDateLastSeen ,
				14 	=> le.attributes.version1.OccupancyOverride ,
				15 	=> le.attributes.version1.InferredOwnershipTypeIndex ,
				16 	=> le.attributes.version1.OtherOwnedPropertyProximity,
				17 	=> le.attributes.version1.VerificationOfOccupancyScore,
							 'Invalid');
		
	END;
 	
	IndIndex := normalize(ungroup(searchResults), 17, createrec(LEFT,COUNTER ));
	
	iesp.verificationofoccupancy.t_VerificationOfOccupancyResponse IntoResults(wseq le, searchResults ri ) := Transform
	...
	VOOResults := join(wseq, searchResults,
	                     right.seq = left.seq,
											 IntoResults(LEFT, RIGHT));

	if(~InputValid and ~TestDataEnabled, FAIL( 'Insufficient input criteria supplied.'));
	if(~RequestValid, FAIL('Invalid attributes version requested - eg VOOATTRV1.'));
	...
	//Log to Deltabase
	Deltabase_Logging_prep := project(VOOResults, transform(Risk_Reporting.Layouts.LOG_Deltabase_Layout_Record,
																									 self.company_id := (Integer)CompanyID,
																									 self.login_id := _LoginID,
																									 self.product_id := Risk_Reporting.ProductID.VerificationOfOccupancy__Search_Service,

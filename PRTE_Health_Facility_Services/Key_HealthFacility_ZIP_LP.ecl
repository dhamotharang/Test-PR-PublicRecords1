EXPORT Key_HealthFacility_ZIP_LP := MODULE
IMPORT SALT29,ut,std;
//PRIM_NAME:ZIP:?:PRIM_RANGE:ST:+:SEC_RANGE:V_CITY_NAME:FAC_NAME:TAXONOMY:TAXONOMY_CODE
EXPORT KeyName := PRTE_Health_Facility_Services.Files.FILE_ZIP; //'~'+'key::PRTE_Health_Facility_Services::LNPID::Refs::ZIP_LP';
...
DataForKey0 := DEDUP(SORT(TABLE(h(PRIM_NAME NOT IN SET(s.nulls_PRIM_NAME,PRIM_NAME),ZIP NOT IN SET(s.nulls_ZIP,ZIP),ST NOT IN SET(s.nulls_ST,ST)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
EXPORT Key := INDEX(DataForKey0,,KeyName);
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,LNPID,PRIM_NAME,ZIP,PRIM_RANGE,ST,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'ZIP_LP','SEC_RANGE',CntRed_SEC_RANGE*100,CntRed_SEC_RANGE*TSize},{'ZIP_LP','V_CITY_NAME',CntRed_V_CITY_NAME*100,CntRed_V_CITY_NAME*TSize},{'ZIP_LP','FAC_NAME',CntRed_FAC_NAME*100,CntRed_FAC_NAME*TSize},{'ZIP_LP','TAXONOMY',CntRed_TAXONOMY*100,CntRed_TAXONOMY*TSize},{'ZIP_LP','TAXONOMY_CODE',CntRed_TAXONOMY_CODE*100,CntRed_TAXONOMY_CODE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.PRIM_NAME <> (typeof(le.PRIM_NAME))'' AND le.ZIP <> (typeof(le.ZIP))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.ST) param_ST) := 
    STEPPED( LIMIT( Key(
...
EXPORT ScoredLNPIDFetch(TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.ST) param_ST,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.CNP_NUMBER) param_CNP_NUMBER,TYPEOF(h.CNP_STORE_NUMBER) param_CNP_STORE_NUMBER,TYPEOF(h.CNP_BTYPE) param_CNP_BTYPE,TYPEOF(h.CNP_LOWV) param_CNP_LOWV,TYPEOF(h.TAXONOMY) param_TAXONOMY,TYPEOF(h.TAXONOMY_CODE) param_TAXONOMY_CODE) := FUNCTION
  RawData := RawFetch(param_PRIM_NAME,param_ZIP,param_PRIM_RANGE,param_ST);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_PRIM_NAME='',Input_ZIP='',Input_PRIM_RANGE='',Input_ST='',Input_SEC_RANGE='',Input_V_CITY_NAME='',Input_CNP_NAME='',Input_CNP_NUMBER='',Input_CNP_STORE_NUMBER='',Input_CNP_BTYPE='',Input_CNP_LOWV='',Input_TAXONOMY='',Input_TAXONOMY_CODE='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,PRTE_Health_Facility_Services;
#IF(#TEXT(Input_PRIM_NAME)<>'' AND #TEXT(Input_ZIP)<>'')
  #uniquename(trans)
  PRTE_Health_Facility_Services.Key_HealthFacility_ZIP_LP.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Facility_Services.Key_HealthFacility_ZIP_LP.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

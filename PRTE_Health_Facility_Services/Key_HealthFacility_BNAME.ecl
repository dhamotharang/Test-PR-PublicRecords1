EXPORT Key_HealthFacility_BNAME := MODULE
...
EXPORT KeyName := PRTE_Health_Facility_Services.Files.FILE_BName; //'~'+'key::PRTE_Health_Facility_Services::LNPID::Refs::BNAME';
...
DataForKey0 := DEDUP(SORT(TABLE(h(CNP_NAME NOT IN SET(s.nulls_CNP_NAME,CNP_NAME),ZIP NOT IN SET(s.nulls_ZIP,ZIP)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,LNPID,CNP_NAME,PRIM_NAME,ST,V_CITY_NAME,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'BNAME','CNP_NUMBER',CntRed_CNP_NUMBER*100,CntRed_CNP_NUMBER*TSize},{'BNAME','CNP_STORE_NUMBER',CntRed_CNP_STORE_NUMBER*100,CntRed_CNP_STORE_NUMBER*TSize},{'BNAME','CNP_BTYPE',CntRed_CNP_BTYPE*100,CntRed_CNP_BTYPE*TSize},{'BNAME','CNP_LOWV',CntRed_CNP_LOWV*100,CntRed_CNP_LOWV*TSize},{'BNAME','PRIM_RANGE',CntRed_PRIM_RANGE*100,CntRed_PRIM_RANGE*TSize},{'BNAME','SEC_RANGE',CntRed_SEC_RANGE*100,CntRed_SEC_RANGE*TSize},{'BNAME','ZIP',CntRed_ZIP*100,CntRed_ZIP*TSize},{'BNAME','TAXONOMY',CntRed_TAXONOMY*100,CntRed_TAXONOMY*TSize},{'BNAME','TAXONOMY_CODE',CntRed_TAXONOMY_CODE*100,CntRed_TAXONOMY_CODE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.CNP_NAME <> (typeof(le.CNP_NAME))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME) := 
  FUNCTION
 //Generate service attributes for GSS join
    ...
    res := JOIN( steppedmatches, Key, RIGHT.GSS_Hash = wds[1].hsh AND LEFT.LNPID = RIGHT.LNPID,TRANSFORM(indexOutputRecord,SELF.gss_word_weight := LEFT.gss_word_weight,SELF := RIGHT));
    RETURN IF(SUM(wds,spec) > 10,res,IF(SUM(wds,spec) = 0,DATASET([],indexOutputRecord) ,DATASET(ROW([],indexOutputRecord)))); // Ensure at least spc of specificity in gss portion
  END;
EXPORT ScoredLNPIDFetch(TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.CNP_NUMBER) param_CNP_NUMBER,TYPEOF(h.CNP_STORE_NUMBER) param_CNP_STORE_NUMBER,TYPEOF(h.CNP_BTYPE) param_CNP_BTYPE,TYPEOF(h.CNP_LOWV) param_CNP_LOWV,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.TAXONOMY) param_TAXONOMY,TYPEOF(h.TAXONOMY_CODE) param_TAXONOMY_CODE) := FUNCTION
  RawData := RawFetch(param_CNP_NAME,param_PRIM_NAME,param_ST,param_V_CITY_NAME);
  ...
  SELF.CNP_NAME_gss_cases := DATASET([{le.gss_hash}],SALT29.layout_GSS_cases);
    ...
    SELF.Weight :=MAX(0,SELF.CNP_NAMEWeight) + MAX(0,SELF.PRIM_NAMEWeight) + MAX(0,SELF.STWeight) + MAX(0,SELF.V_CITY_NAMEWeight) + MAX(0,SELF.CNP_NUMBERWeight) + MAX(0,SELF.CNP_STORE_NUMBERWeight) + MAX(0,SELF.CNP_BTYPEWeight) + MAX(0,SELF.CNP_LOWVWeight) + MAX(0,SELF.PRIM_RANGEWeight) + MAX(0,SELF.SEC_RANGEWeight) + MAX(0,SELF.ZIPWeight) + MAX(0,SELF.TAXONOMYWeight) + MAX(0,SELF.TAXONOMY_CODEWeight);
    SELF := le;
  END;
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_CNP_NAME='',Input_PRIM_NAME='',Input_ST='',Input_V_CITY_NAME='',Input_CNP_NUMBER='',Input_CNP_STORE_NUMBER='',Input_CNP_BTYPE='',Input_CNP_LOWV='',Input_PRIM_RANGE='',Input_SEC_RANGE='',Input_ZIP='',Input_TAXONOMY='',Input_TAXONOMY_CODE='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,PRTE_Health_Facility_Services;
#IF(#TEXT(Input_CNP_NAME)<>'')
  #uniquename(trans)
  PRTE_Health_Facility_Services.Key_HealthFacility_BNAME.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Facility_Services.Key_HealthFacility_BNAME.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

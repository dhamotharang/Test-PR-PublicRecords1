EXPORT Key_HealthFacility_MEDICARE := MODULE
IMPORT SALT29,ut,std;
//MEDICARE_FACILITY_NUMBER:+:FAC_NAME:ADDR1:LOCALE:TAXONOMY:TAXONOMY_CODE
EXPORT KeyName := PRTE_Health_Facility_Services.Files.FILE_MEDICARE; //'~'+'key::PRTE_Health_Facility_Services::LNPID::Refs::MEDICARE';
...
//Scores for various field components
  h.MEDICARE_FACILITY_NUMBER_weight100 ; // Contains 100x the specificity
...
DataForKey0 := DEDUP(SORT(TABLE(h(MEDICARE_FACILITY_NUMBER NOT IN SET(s.nulls_MEDICARE_FACILITY_NUMBER,MEDICARE_FACILITY_NUMBER)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
EXPORT Key := INDEX(DataForKey0,,KeyName);
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,LNPID,MEDICARE_FACILITY_NUMBER,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'MEDICARE','FAC_NAME',CntRed_FAC_NAME*100,CntRed_FAC_NAME*TSize},{'MEDICARE','ADDR1',CntRed_ADDR1*100,CntRed_ADDR1*TSize},{'MEDICARE','LOCALE',CntRed_LOCALE*100,CntRed_LOCALE*TSize},{'MEDICARE','TAXONOMY',CntRed_TAXONOMY*100,CntRed_TAXONOMY*TSize},{'MEDICARE','TAXONOMY_CODE',CntRed_TAXONOMY_CODE*100,CntRed_TAXONOMY_CODE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.MEDICARE_FACILITY_NUMBER <> (typeof(le.MEDICARE_FACILITY_NUMBER))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.MEDICARE_FACILITY_NUMBER) param_MEDICARE_FACILITY_NUMBER) := 
    STEPPED( LIMIT( Key(
          ( MEDICARE_FACILITY_NUMBER = param_MEDICARE_FACILITY_NUMBER AND param_MEDICARE_FACILITY_NUMBER <> (TYPEOF(MEDICARE_FACILITY_NUMBER))'' )),10000,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
EXPORT ScoredLNPIDFetch(TYPEOF(h.MEDICARE_FACILITY_NUMBER) param_MEDICARE_FACILITY_NUMBER,TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.CNP_NUMBER) param_CNP_NUMBER,TYPEOF(h.CNP_STORE_NUMBER) param_CNP_STORE_NUMBER,TYPEOF(h.CNP_BTYPE) param_CNP_BTYPE,TYPEOF(h.CNP_LOWV) param_CNP_LOWV,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.TAXONOMY) param_TAXONOMY,TYPEOF(h.TAXONOMY_CODE) param_TAXONOMY_CODE) := FUNCTION
  RawData := RawFetch(param_MEDICARE_FACILITY_NUMBER);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.MEDICARE_FACILITY_NUMBER_match_code := MAP(
		le.MEDICARE_FACILITY_NUMBER = (TYPEOF(le.MEDICARE_FACILITY_NUMBER))'' OR le.MEDICARE_FACILITY_NUMBER = (TYPEOF(le.MEDICARE_FACILITY_NUMBER))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_MEDICARE_FACILITY_NUMBER(le.MEDICARE_FACILITY_NUMBER,param_MEDICARE_FACILITY_NUMBER,TRUE));
    SELF.MEDICARE_FACILITY_NUMBERWeight := (50+MAP ( le.MEDICARE_FACILITY_NUMBER = param_MEDICARE_FACILITY_NUMBER  => le.MEDICARE_FACILITY_NUMBER_weight100,
          le.MEDICARE_FACILITY_NUMBER = (TYPEOF(le.MEDICARE_FACILITY_NUMBER))'' OR param_MEDICARE_FACILITY_NUMBER = (TYPEOF(le.MEDICARE_FACILITY_NUMBER))'' => 0,
           -0.986*le.MEDICARE_FACILITY_NUMBER_weight100))/100; 
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.MEDICARE_FACILITY_NUMBER_match_code := MAP(
		le.MEDICARE_FACILITY_NUMBER = (TYPEOF(le.MEDICARE_FACILITY_NUMBER))'' OR le.MEDICARE_FACILITY_NUMBER = (TYPEOF(le.MEDICARE_FACILITY_NUMBER))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_MEDICARE_FACILITY_NUMBER(le.MEDICARE_FACILITY_NUMBER,ri.MEDICARE_FACILITY_NUMBER,TRUE));
    SELF.MEDICARE_FACILITY_NUMBERWeight := (50+MAP ( le.MEDICARE_FACILITY_NUMBER = ri.MEDICARE_FACILITY_NUMBER  => le.MEDICARE_FACILITY_NUMBER_weight100,
          le.MEDICARE_FACILITY_NUMBER = (TYPEOF(le.MEDICARE_FACILITY_NUMBER))'' OR ri.MEDICARE_FACILITY_NUMBER = (TYPEOF(le.MEDICARE_FACILITY_NUMBER))'' => 0,
           -0.986*le.MEDICARE_FACILITY_NUMBER_weight100))/100; 
    SELF.CNP_NAME_match_code := MAP(
    ...
    SELF := le;
  END;
  Recs0 := Recs(MEDICARE_FACILITY_NUMBER <> (typeof(MEDICARE_FACILITY_NUMBER))'');
  ...
  J0 := JOIN(Recs1,Key,LEFT.MEDICARE_FACILITY_NUMBER = RIGHT.MEDICARE_FACILITY_NUMBER,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.MEDICARE_FACILITY_NUMBER = RIGHT.MEDICARE_FACILITY_NUMBER,10000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.MEDICARE_FACILITY_NUMBER = RIGHT.MEDICARE_FACILITY_NUMBER,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.MEDICARE_FACILITY_NUMBER = RIGHT.MEDICARE_FACILITY_NUMBER,10000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_MEDICARE_FACILITY_NUMBER='',Input_CNP_NAME='',Input_CNP_NUMBER='',Input_CNP_STORE_NUMBER='',Input_CNP_BTYPE='',Input_CNP_LOWV='',Input_PRIM_RANGE='',Input_SEC_RANGE='',Input_PRIM_NAME='',Input_V_CITY_NAME='',Input_ST='',Input_ZIP='',Input_TAXONOMY='',Input_TAXONOMY_CODE='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,PRTE_Health_Facility_Services;
#IF(#TEXT(Input_MEDICARE_FACILITY_NUMBER)<>'')
  #uniquename(trans)
  PRTE_Health_Facility_Services.Key_HealthFacility_MEDICARE.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Facility_Services.Key_HealthFacility_MEDICARE.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

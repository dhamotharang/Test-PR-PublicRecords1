EXPORT Key_HealthFacility_ADDRESS := MODULE
IMPORT SALT29,ut,std;
//PRIM_NAME:PRIM_RANGE:ZIP:?:V_CITY_NAME:ST:+:SEC_RANGE:FAC_NAME:TAXONOMY:TAXONOMY_CODE
EXPORT KeyName := PRTE_Health_Facility_Services.Files.FILE_Address; //'~'+'key::PRTE_Health_Facility_Services::LNPID::Refs::ADDRESS';
...
DataForKey0 := DEDUP(SORT(TABLE(h(PRIM_NAME NOT IN SET(s.nulls_PRIM_NAME,PRIM_NAME),PRIM_RANGE NOT IN SET(s.nulls_PRIM_RANGE,PRIM_RANGE),ZIP NOT IN SET(s.nulls_ZIP,ZIP)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
EXPORT Key := INDEX(DataForKey0,,KeyName);
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,LNPID,PRIM_NAME,PRIM_RANGE,ZIP,V_CITY_NAME,ST,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'ADDRESS','SEC_RANGE',CntRed_SEC_RANGE*100,CntRed_SEC_RANGE*TSize},{'ADDRESS','FAC_NAME',CntRed_FAC_NAME*100,CntRed_FAC_NAME*TSize},{'ADDRESS','TAXONOMY',CntRed_TAXONOMY*100,CntRed_TAXONOMY*TSize},{'ADDRESS','TAXONOMY_CODE',CntRed_TAXONOMY_CODE*100,CntRed_TAXONOMY_CODE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.PRIM_NAME <> (typeof(le.PRIM_NAME))'' AND le.PRIM_RANGE <> (typeof(le.PRIM_RANGE))'' AND le.ZIP <> (typeof(le.ZIP))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST) := 
    STEPPED( LIMIT( Key(
      ...
      AND ( V_CITY_NAME = (TYPEOF(V_CITY_NAME))'' OR param_V_CITY_NAME = (TYPEOF(V_CITY_NAME))'' OR metaphonelib.DMetaPhone1(V_CITY_NAME)=metaphonelib.DMetaPhone1(param_V_CITY_NAME) OR SALT29.WithinEditN(V_CITY_NAME,param_V_CITY_NAME,2, 0)  )
      AND ( ST = (TYPEOF(ST))'' OR param_ST = (TYPEOF(ST))'' OR ST = param_ST  )),10000,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
EXPORT ScoredLNPIDFetch(TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.CNP_NUMBER) param_CNP_NUMBER,TYPEOF(h.CNP_STORE_NUMBER) param_CNP_STORE_NUMBER,TYPEOF(h.CNP_BTYPE) param_CNP_BTYPE,TYPEOF(h.CNP_LOWV) param_CNP_LOWV,TYPEOF(h.TAXONOMY) param_TAXONOMY,TYPEOF(h.TAXONOMY_CODE) param_TAXONOMY_CODE) := FUNCTION
  RawData := RawFetch(param_PRIM_NAME,param_PRIM_RANGE,param_ZIP,param_V_CITY_NAME,param_ST);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.PRIM_RANGE_match_code := MAP(
		le.PRIM_RANGE = (TYPEOF(le.PRIM_RANGE))'' OR le.PRIM_RANGE = (TYPEOF(le.PRIM_RANGE))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_PRIM_RANGE(le.PRIM_RANGE,param_PRIM_RANGE,TRUE));
    ...
    SELF.Weight :=MAX(0,SELF.PRIM_NAMEWeight) + MAX(0,SELF.PRIM_RANGEWeight) + MAX(0,SELF.ZIPWeight) + MAX(0,SELF.V_CITY_NAMEWeight) + MAX(0,SELF.STWeight) + MAX(0,SELF.SEC_RANGEWeight) + MAX(self.FAC_NAMEWeight,MAX(0,SELF.CNP_NAMEWeight) + MAX(0,SELF.CNP_NUMBERWeight) + MAX(0,SELF.CNP_STORE_NUMBERWeight) + MAX(0,SELF.CNP_BTYPEWeight) + MAX(0,SELF.CNP_LOWVWeight)) + MAX(0,SELF.TAXONOMYWeight) + MAX(0,SELF.TAXONOMY_CODEWeight);
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.PRIM_NAMEWeight := (50+MAP ( le.PRIM_NAME = ri.PRIM_NAME  => le.PRIM_NAME_weight100,
          le.PRIM_NAME = (TYPEOF(le.PRIM_NAME))'' OR ri.PRIM_NAME = (TYPEOF(le.PRIM_NAME))'' => 0,
           -0.715*le.PRIM_NAME_weight100))/100; 
    SELF.PRIM_RANGE_match_code := MAP(
		le.PRIM_RANGE = (TYPEOF(le.PRIM_RANGE))'' OR le.PRIM_RANGE = (TYPEOF(le.PRIM_RANGE))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_PRIM_RANGE(le.PRIM_RANGE,ri.PRIM_RANGE,TRUE));
    ...
    SELF := le;
  END;
  Recs0 := Recs(PRIM_NAME <> (typeof(PRIM_NAME))'',PRIM_RANGE <> (typeof(PRIM_RANGE))'',ZIP <> (typeof(ZIP))'');
  ...
  J0 := JOIN(Recs1,Key,LEFT.PRIM_NAME = RIGHT.PRIM_NAME
     ...
     AND ( LEFT.V_CITY_NAME = (TYPEOF(LEFT.V_CITY_NAME))'' OR RIGHT.V_CITY_NAME = (TYPEOF(RIGHT.V_CITY_NAME))'' OR metaphonelib.DMetaPhone1(LEFT.V_CITY_NAME)=metaphonelib.DMetaPhone1(RIGHT.V_CITY_NAME) OR SALT29.WithinEditN(LEFT.V_CITY_NAME,RIGHT.V_CITY_NAME,2, 0)  )
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_PRIM_NAME='',Input_PRIM_RANGE='',Input_ZIP='',Input_V_CITY_NAME='',Input_ST='',Input_SEC_RANGE='',Input_CNP_NAME='',Input_CNP_NUMBER='',Input_CNP_STORE_NUMBER='',Input_CNP_BTYPE='',Input_CNP_LOWV='',Input_TAXONOMY='',Input_TAXONOMY_CODE='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,PRTE_Health_Facility_Services;
#IF(#TEXT(Input_PRIM_NAME)<>'' AND #TEXT(Input_PRIM_RANGE)<>'' AND #TEXT(Input_ZIP)<>'')
  #uniquename(trans)
  PRTE_Health_Facility_Services.Key_HealthFacility_ADDRESS.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Facility_Services.Key_HealthFacility_ADDRESS.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

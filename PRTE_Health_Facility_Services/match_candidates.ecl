...
pj13 do_computes(pj13 le) := TRANSFORM
  ...
  SELF := le;
END;
SHARED propogated := PROJECT(pj13,do_computes(left)) : PERSIST('~temp::LNPID::PRTE_Health_Facility_Services::mc_props::HealthFacility',EXPIRE(30)); // to allow to 'jump' over an exported value
...
SHARED Annotated := DISTRIBUTE(j0,hash(LNPID)) : PERSIST('~temp::LNPID::PRTE_Health_Facility_Services::mc',EXPIRE(30)); // Distributed for keybuild case

EXPORT Key_HealthFacility_VEN := MODULE
IMPORT SALT29,ut,std;
//VENDOR_ID:?:SRC:+:FAC_NAME:ADDR1:LOCALE:TAXONOMY:TAXONOMY_CODE
EXPORT KeyName := PRTE_Health_Facility_Services.Files.FILE_VendorID; //'~'+'key::PRTE_Health_Facility_Services::LNPID::Refs::VEN';
...
DataForKey0 := DEDUP(SORT(TABLE(h(VENDOR_ID NOT IN SET(s.nulls_VENDOR_ID,VENDOR_ID)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
EXPORT Key := INDEX(DataForKey0,,KeyName);
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,LNPID,VENDOR_ID,SRC,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'VEN','FAC_NAME',CntRed_FAC_NAME*100,CntRed_FAC_NAME*TSize},{'VEN','ADDR1',CntRed_ADDR1*100,CntRed_ADDR1*TSize},{'VEN','LOCALE',CntRed_LOCALE*100,CntRed_LOCALE*TSize},{'VEN','TAXONOMY',CntRed_TAXONOMY*100,CntRed_TAXONOMY*TSize},{'VEN','TAXONOMY_CODE',CntRed_TAXONOMY_CODE*100,CntRed_TAXONOMY_CODE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.VENDOR_ID <> (typeof(le.VENDOR_ID))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.VENDOR_ID) param_VENDOR_ID,TYPEOF(h.SRC) param_SRC) := 
    STEPPED( LIMIT( Key(
          ( VENDOR_ID = param_VENDOR_ID AND param_VENDOR_ID <> (TYPEOF(VENDOR_ID))'' )
      AND ( SRC = (TYPEOF(SRC))'' OR param_SRC = (TYPEOF(SRC))'' OR SRC = param_SRC  )),10000,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
EXPORT ScoredLNPIDFetch(TYPEOF(h.VENDOR_ID) param_VENDOR_ID,TYPEOF(h.SRC) param_SRC,TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.CNP_NUMBER) param_CNP_NUMBER,TYPEOF(h.CNP_STORE_NUMBER) param_CNP_STORE_NUMBER,TYPEOF(h.CNP_BTYPE) param_CNP_BTYPE,TYPEOF(h.CNP_LOWV) param_CNP_LOWV,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.TAXONOMY) param_TAXONOMY,TYPEOF(h.TAXONOMY_CODE) param_TAXONOMY_CODE) := FUNCTION
  RawData := RawFetch(param_VENDOR_ID,param_SRC);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.VENDOR_ID_match_code := MAP(
		le.VENDOR_ID = (TYPEOF(le.VENDOR_ID))'' OR le.VENDOR_ID = (TYPEOF(le.VENDOR_ID))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_VENDOR_ID(le.VENDOR_ID,param_VENDOR_ID,TRUE));
    SELF.VENDOR_IDWeight := (50+MAP ( le.VENDOR_ID = param_VENDOR_ID  => le.VENDOR_ID_weight100,
          le.VENDOR_ID = (TYPEOF(le.VENDOR_ID))'' OR param_VENDOR_ID = (TYPEOF(le.VENDOR_ID))'' => 0,
           -0.080*le.VENDOR_ID_weight100))/100; 
    SELF.SRC_match_code := MAP(
		le.SRC = (TYPEOF(le.SRC))'' OR param_SRC = (TYPEOF(param_SRC))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_SRC(le.SRC,param_SRC,FALSE));
    SELF.SRCWeight := (50+MAP ( le.SRC = param_SRC  => le.SRC_weight100,
           le.SRC = (TYPEOF(le.SRC))'' OR param_SRC = (TYPEOF(param_SRC))'' => 0,
           -0.120*le.SRC_weight100))/100; 
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.VENDOR_ID_match_code := MAP(
		le.VENDOR_ID = (TYPEOF(le.VENDOR_ID))'' OR le.VENDOR_ID = (TYPEOF(le.VENDOR_ID))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_VENDOR_ID(le.VENDOR_ID,ri.VENDOR_ID,TRUE));
    SELF.VENDOR_IDWeight := (50+MAP ( le.VENDOR_ID = ri.VENDOR_ID  => le.VENDOR_ID_weight100,
          le.VENDOR_ID = (TYPEOF(le.VENDOR_ID))'' OR ri.VENDOR_ID = (TYPEOF(le.VENDOR_ID))'' => 0,
           -0.080*le.VENDOR_ID_weight100))/100; 
    SELF.SRC_match_code := MAP(
		le.SRC = (TYPEOF(le.SRC))'' OR ri.SRC = (TYPEOF(ri.SRC))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_SRC(le.SRC,ri.SRC,FALSE));
    ...
    SELF := le;
  END;
  Recs0 := Recs(VENDOR_ID <> (typeof(VENDOR_ID))'');
  ...
  J0 := JOIN(Recs1,Key,LEFT.VENDOR_ID = RIGHT.VENDOR_ID
     AND ( LEFT.SRC = (TYPEOF(LEFT.SRC))'' OR RIGHT.SRC = (TYPEOF(RIGHT.SRC))'' OR LEFT.SRC = RIGHT.SRC  ),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.VENDOR_ID = RIGHT.VENDOR_ID,10000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.VENDOR_ID = RIGHT.VENDOR_ID
     AND ( LEFT.SRC = (TYPEOF(LEFT.SRC))'' OR RIGHT.SRC = (TYPEOF(RIGHT.SRC))'' OR LEFT.SRC = RIGHT.SRC  ),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.VENDOR_ID = RIGHT.VENDOR_ID,10000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
#IF(#TEXT(Input_VENDOR_ID)<>'')
  ...
  output_file := PRTE_Health_Facility_Services.Key_HealthFacility_VEN.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

EXPORT Key_HealthFacility_MEDICAID := MODULE
IMPORT SALT29,ut,std;
//MEDICAID_NUMBER:+:FAC_NAME:ADDR1:LOCALE:TAXONOMY:TAXONOMY_CODE
EXPORT KeyName := PRTE_Health_Facility_Services.Files.FILE_MEDICAID; //'~'+'key::PRTE_Health_Facility_Services::LNPID::Refs::MEDICAID';
...
//Scores for various field components
  h.MEDICAID_NUMBER_weight100 ; // Contains 100x the specificity
...
DataForKey0 := DEDUP(SORT(TABLE(h(MEDICAID_NUMBER NOT IN SET(s.nulls_MEDICAID_NUMBER,MEDICAID_NUMBER)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
EXPORT Key := INDEX(DataForKey0,,KeyName);
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,LNPID,MEDICAID_NUMBER,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'MEDICAID','FAC_NAME',CntRed_FAC_NAME*100,CntRed_FAC_NAME*TSize},{'MEDICAID','ADDR1',CntRed_ADDR1*100,CntRed_ADDR1*TSize},{'MEDICAID','LOCALE',CntRed_LOCALE*100,CntRed_LOCALE*TSize},{'MEDICAID','TAXONOMY',CntRed_TAXONOMY*100,CntRed_TAXONOMY*TSize},{'MEDICAID','TAXONOMY_CODE',CntRed_TAXONOMY_CODE*100,CntRed_TAXONOMY_CODE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.MEDICAID_NUMBER <> (typeof(le.MEDICAID_NUMBER))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.MEDICAID_NUMBER) param_MEDICAID_NUMBER) := 
    STEPPED( LIMIT( Key(
          ( MEDICAID_NUMBER = param_MEDICAID_NUMBER AND param_MEDICAID_NUMBER <> (TYPEOF(MEDICAID_NUMBER))'' )),10000,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
EXPORT ScoredLNPIDFetch(TYPEOF(h.MEDICAID_NUMBER) param_MEDICAID_NUMBER,TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.CNP_NUMBER) param_CNP_NUMBER,TYPEOF(h.CNP_STORE_NUMBER) param_CNP_STORE_NUMBER,TYPEOF(h.CNP_BTYPE) param_CNP_BTYPE,TYPEOF(h.CNP_LOWV) param_CNP_LOWV,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.TAXONOMY) param_TAXONOMY,TYPEOF(h.TAXONOMY_CODE) param_TAXONOMY_CODE) := FUNCTION
  RawData := RawFetch(param_MEDICAID_NUMBER);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    SELF.keys_used := 1 << 18; // Set bitmap for key used
    SELF.keys_failed := IF(le.LNPID = 0, 1 << 18, 0); // Set bitmap for key failed
    SELF.MEDICAID_NUMBER_match_code := MAP(
		le.MEDICAID_NUMBER = (TYPEOF(le.MEDICAID_NUMBER))'' OR le.MEDICAID_NUMBER = (TYPEOF(le.MEDICAID_NUMBER))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_MEDICAID_NUMBER(le.MEDICAID_NUMBER,param_MEDICAID_NUMBER,TRUE));
    SELF.MEDICAID_NUMBERWeight := (50+MAP ( le.MEDICAID_NUMBER = param_MEDICAID_NUMBER  => le.MEDICAID_NUMBER_weight100,
          le.MEDICAID_NUMBER = (TYPEOF(le.MEDICAID_NUMBER))'' OR param_MEDICAID_NUMBER = (TYPEOF(le.MEDICAID_NUMBER))'' => 0,
           -0.984*le.MEDICAID_NUMBER_weight100))/100; 
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.MEDICAID_NUMBER_match_code := MAP(
		le.MEDICAID_NUMBER = (TYPEOF(le.MEDICAID_NUMBER))'' OR le.MEDICAID_NUMBER = (TYPEOF(le.MEDICAID_NUMBER))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_MEDICAID_NUMBER(le.MEDICAID_NUMBER,ri.MEDICAID_NUMBER,TRUE));
    SELF.MEDICAID_NUMBERWeight := (50+MAP ( le.MEDICAID_NUMBER = ri.MEDICAID_NUMBER  => le.MEDICAID_NUMBER_weight100,
          le.MEDICAID_NUMBER = (TYPEOF(le.MEDICAID_NUMBER))'' OR ri.MEDICAID_NUMBER = (TYPEOF(le.MEDICAID_NUMBER))'' => 0,
           -0.984*le.MEDICAID_NUMBER_weight100))/100; 
    SELF.CNP_NAME_match_code := MAP(
    ...
    SELF := le;
  END;
  Recs0 := Recs(MEDICAID_NUMBER <> (typeof(MEDICAID_NUMBER))'');
  ...
  J0 := JOIN(Recs1,Key,LEFT.MEDICAID_NUMBER = RIGHT.MEDICAID_NUMBER,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.MEDICAID_NUMBER = RIGHT.MEDICAID_NUMBER,10000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.MEDICAID_NUMBER = RIGHT.MEDICAID_NUMBER,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.MEDICAID_NUMBER = RIGHT.MEDICAID_NUMBER,10000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
#IF(#TEXT(Input_MEDICAID_NUMBER)<>'')
  ...
  output_file := PRTE_Health_Facility_Services.Key_HealthFacility_MEDICAID.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

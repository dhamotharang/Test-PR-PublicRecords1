EXPORT Key_HealthFacility_CITY_LP := MODULE
IMPORT SALT29,ut,std;
//PRIM_NAME:V_CITY_NAME:ST:?:PRIM_RANGE:+:ZIP:SEC_RANGE:FAC_NAME:TAXONOMY:TAXONOMY_CODE
EXPORT KeyName := PRTE_Health_Facility_Services.Files.FILE_City; //'~'+'key::PRTE_Health_Facility_Services::LNPID::Refs::CITY_LP';
...
DataForKey0 := DEDUP(SORT(TABLE(h(PRIM_NAME NOT IN SET(s.nulls_PRIM_NAME,PRIM_NAME),V_CITY_NAME NOT IN SET(s.nulls_V_CITY_NAME,V_CITY_NAME),ST NOT IN SET(s.nulls_ST,ST)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
EXPORT Key := INDEX(DataForKey0,,KeyName);
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,LNPID,PRIM_NAME,V_CITY_NAME,ST,PRIM_RANGE,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'CITY_LP','ZIP',CntRed_ZIP*100,CntRed_ZIP*TSize},{'CITY_LP','SEC_RANGE',CntRed_SEC_RANGE*100,CntRed_SEC_RANGE*TSize},{'CITY_LP','FAC_NAME',CntRed_FAC_NAME*100,CntRed_FAC_NAME*TSize},{'CITY_LP','TAXONOMY',CntRed_TAXONOMY*100,CntRed_TAXONOMY*TSize},{'CITY_LP','TAXONOMY_CODE',CntRed_TAXONOMY_CODE*100,CntRed_TAXONOMY_CODE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.PRIM_NAME <> (typeof(le.PRIM_NAME))'' AND le.V_CITY_NAME <> (typeof(le.V_CITY_NAME))'' AND le.ST <> (typeof(le.ST))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE) := 
    STEPPED( LIMIT( Key(
          ( PRIM_NAME = param_PRIM_NAME AND param_PRIM_NAME <> (TYPEOF(PRIM_NAME))'' )
      AND ( V_CITY_NAME = param_V_CITY_NAME AND param_V_CITY_NAME <> (TYPEOF(V_CITY_NAME))'' )
      AND ( ST = param_ST AND param_ST <> (TYPEOF(ST))'' )
      AND ( PRIM_RANGE = (TYPEOF(PRIM_RANGE))'' OR param_PRIM_RANGE = (TYPEOF(PRIM_RANGE))'' OR SALT29.WithinEditN(PRIM_RANGE,param_PRIM_RANGE,1, 0)  )),10000,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
EXPORT ScoredLNPIDFetch(TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.CNP_NUMBER) param_CNP_NUMBER,TYPEOF(h.CNP_STORE_NUMBER) param_CNP_STORE_NUMBER,TYPEOF(h.CNP_BTYPE) param_CNP_BTYPE,TYPEOF(h.CNP_LOWV) param_CNP_LOWV,TYPEOF(h.TAXONOMY) param_TAXONOMY,TYPEOF(h.TAXONOMY_CODE) param_TAXONOMY_CODE) := FUNCTION
  RawData := RawFetch(param_PRIM_NAME,param_V_CITY_NAME,param_ST,param_PRIM_RANGE);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.PRIM_NAME_match_code := MAP(
		le.PRIM_NAME = (TYPEOF(le.PRIM_NAME))'' OR le.PRIM_NAME = (TYPEOF(le.PRIM_NAME))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_PRIM_NAME(le.PRIM_NAME,param_PRIM_NAME,TRUE));
    ...
    SELF.V_CITY_NAME_match_code := MAP(
		le.V_CITY_NAME = (TYPEOF(le.V_CITY_NAME))'' OR le.V_CITY_NAME = (TYPEOF(le.V_CITY_NAME))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_V_CITY_NAME(le.V_CITY_NAME,param_V_CITY_NAME,TRUE));
    ...
    SELF.Weight :=MAX(0,SELF.PRIM_NAMEWeight) + MAX(0,SELF.V_CITY_NAMEWeight) + MAX(0,SELF.STWeight) + MAX(0,SELF.PRIM_RANGEWeight) + MAX(0,SELF.ZIPWeight) + MAX(0,SELF.SEC_RANGEWeight) + MAX(self.FAC_NAMEWeight,MAX(0,SELF.CNP_NAMEWeight) + MAX(0,SELF.CNP_NUMBERWeight) + MAX(0,SELF.CNP_STORE_NUMBERWeight) + MAX(0,SELF.CNP_BTYPEWeight) + MAX(0,SELF.CNP_LOWVWeight)) + MAX(0,SELF.TAXONOMYWeight) + MAX(0,SELF.TAXONOMY_CODEWeight);
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.PRIM_NAME_match_code := MAP(
		le.PRIM_NAME = (TYPEOF(le.PRIM_NAME))'' OR le.PRIM_NAME = (TYPEOF(le.PRIM_NAME))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_PRIM_NAME(le.PRIM_NAME,ri.PRIM_NAME,TRUE));
    SELF.PRIM_NAMEWeight := (50+MAP ( le.PRIM_NAME = ri.PRIM_NAME  => le.PRIM_NAME_weight100,
          le.PRIM_NAME = (TYPEOF(le.PRIM_NAME))'' OR ri.PRIM_NAME = (TYPEOF(le.PRIM_NAME))'' => 0,
           -0.715*le.PRIM_NAME_weight100))/100; 
    SELF.V_CITY_NAME_match_code := MAP(
		le.V_CITY_NAME = (TYPEOF(le.V_CITY_NAME))'' OR le.V_CITY_NAME = (TYPEOF(le.V_CITY_NAME))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_V_CITY_NAME(le.V_CITY_NAME,ri.V_CITY_NAME,TRUE));
    ...
    SELF := le;
  END;
  Recs0 := Recs(PRIM_NAME <> (typeof(PRIM_NAME))'',V_CITY_NAME <> (typeof(V_CITY_NAME))'',ST <> (typeof(ST))'');
  ...
  J0 := JOIN(Recs1,Key,LEFT.PRIM_NAME = RIGHT.PRIM_NAME
     ...
     AND ( LEFT.PRIM_RANGE = (TYPEOF(LEFT.PRIM_RANGE))'' OR RIGHT.PRIM_RANGE = (TYPEOF(RIGHT.PRIM_RANGE))'' OR SALT29.WithinEditN(LEFT.PRIM_RANGE,RIGHT.PRIM_RANGE,1, 0)  ),Score_Batch(RIGHT,LEFT),
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_PRIM_NAME='',Input_V_CITY_NAME='',Input_ST='',Input_PRIM_RANGE='',Input_ZIP='',Input_SEC_RANGE='',Input_CNP_NAME='',Input_CNP_NUMBER='',Input_CNP_STORE_NUMBER='',Input_CNP_BTYPE='',Input_CNP_LOWV='',Input_TAXONOMY='',Input_TAXONOMY_CODE='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,PRTE_Health_Facility_Services;
#IF(#TEXT(Input_PRIM_NAME)<>'' AND #TEXT(Input_V_CITY_NAME)<>'' AND #TEXT(Input_ST)<>'')
  #uniquename(trans)
  PRTE_Health_Facility_Services.Key_HealthFacility_CITY_LP.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Facility_Services.Key_HealthFacility_CITY_LP.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

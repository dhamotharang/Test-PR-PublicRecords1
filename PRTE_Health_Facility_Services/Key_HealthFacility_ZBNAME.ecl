EXPORT Key_HealthFacility_ZBNAME := MODULE
...
EXPORT KeyName := PRTE_Health_Facility_Services.Files.FILE_BName_Zip; //'~'+'key::PRTE_Health_Facility_Services::LNPID::Refs::ZBNAME';
...
DataForKey0 := DEDUP(SORT(TABLE(h(CNP_NAME NOT IN SET(s.nulls_CNP_NAME,CNP_NAME),ZIP NOT IN SET(s.nulls_ZIP,ZIP),ST NOT IN SET(s.nulls_ST,ST)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,LNPID,CNP_NAME,ZIP,PRIM_NAME,ST,V_CITY_NAME,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'ZBNAME','CNP_NUMBER',CntRed_CNP_NUMBER*100,CntRed_CNP_NUMBER*TSize},{'ZBNAME','CNP_STORE_NUMBER',CntRed_CNP_STORE_NUMBER*100,CntRed_CNP_STORE_NUMBER*TSize},{'ZBNAME','CNP_BTYPE',CntRed_CNP_BTYPE*100,CntRed_CNP_BTYPE*TSize},{'ZBNAME','CNP_LOWV',CntRed_CNP_LOWV*100,CntRed_CNP_LOWV*TSize},{'ZBNAME','PRIM_RANGE',CntRed_PRIM_RANGE*100,CntRed_PRIM_RANGE*TSize},{'ZBNAME','SEC_RANGE',CntRed_SEC_RANGE*100,CntRed_SEC_RANGE*TSize},{'ZBNAME','TAXONOMY',CntRed_TAXONOMY*100,CntRed_TAXONOMY*TSize},{'ZBNAME','TAXONOMY_CODE',CntRed_TAXONOMY_CODE*100,CntRed_TAXONOMY_CODE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.CNP_NAME <> (typeof(le.CNP_NAME))'' AND le.ZIP <> (typeof(le.ZIP))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME) := 
  FUNCTION
 //Generate service attributes for GSS join
    ...
    res := JOIN( steppedmatches, Key, RIGHT.GSS_Hash = wds[1].hsh
      AND ( RIGHT.ZIP = param_ZIP AND param_ZIP <> (TYPEOF(RIGHT.ZIP))'' ) AND LEFT.LNPID = RIGHT.LNPID,TRANSFORM(indexOutputRecord,SELF.gss_word_weight := LEFT.gss_word_weight,SELF := RIGHT));
    RETURN IF(SUM(wds,spec) > 0,res,IF(SUM(wds,spec) = 0,DATASET([],indexOutputRecord) ,DATASET(ROW([],indexOutputRecord)))); // Ensure at least spc of specificity in gss portion
  END;
EXPORT ScoredLNPIDFetch(TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.CNP_NUMBER) param_CNP_NUMBER,TYPEOF(h.CNP_STORE_NUMBER) param_CNP_STORE_NUMBER,TYPEOF(h.CNP_BTYPE) param_CNP_BTYPE,TYPEOF(h.CNP_LOWV) param_CNP_LOWV,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.TAXONOMY) param_TAXONOMY,TYPEOF(h.TAXONOMY_CODE) param_TAXONOMY_CODE) := FUNCTION
  RawData := RawFetch(param_CNP_NAME,param_ZIP,param_PRIM_NAME,param_ST,param_V_CITY_NAME);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    SELF.keys_used := 1 << 1; // Set bitmap for key used
    SELF.keys_failed := IF(le.LNPID = 0, 1 << 1, 0); // Set bitmap for key failed
  ...
  SELF.CNP_NAME_gss_cases := DATASET([{le.gss_hash}],SALT29.layout_GSS_cases);
    SELF.ZIP_match_code := MAP(
		le.ZIP = (TYPEOF(le.ZIP))'' OR le.ZIP = (TYPEOF(le.ZIP))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_ZIP(le.ZIP,param_ZIP,TRUE));
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.ZIP_match_code := MAP(
		le.ZIP = (TYPEOF(le.ZIP))'' OR le.ZIP = (TYPEOF(le.ZIP))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_ZIP(le.ZIP,ri.ZIP,TRUE));
    ...
    SELF := le;
  END;
  Recs0 := Recs(CNP_NAME <> (typeof(CNP_NAME))'',ZIP <> (typeof(ZIP))'');
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_CNP_NAME='',Input_ZIP='',Input_PRIM_NAME='',Input_ST='',Input_V_CITY_NAME='',Input_CNP_NUMBER='',Input_CNP_STORE_NUMBER='',Input_CNP_BTYPE='',Input_CNP_LOWV='',Input_PRIM_RANGE='',Input_SEC_RANGE='',Input_TAXONOMY='',Input_TAXONOMY_CODE='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,PRTE_Health_Facility_Services;
#IF(#TEXT(Input_CNP_NAME)<>'' AND #TEXT(Input_ZIP)<>'')
  #uniquename(trans)
  PRTE_Health_Facility_Services.Key_HealthFacility_ZBNAME.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Facility_Services.Key_HealthFacility_ZBNAME.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

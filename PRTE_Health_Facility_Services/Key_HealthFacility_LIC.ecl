EXPORT Key_HealthFacility_LIC := MODULE
IMPORT SALT29,ut,std;
//C_LIC_NBR:?:LIC_STATE:+:FAC_NAME:ADDR1:LOCALE:TAXONOMY:TAXONOMY_CODE
EXPORT KeyName := PRTE_Health_Facility_Services.Files.FILE_LIC; //'~'+'key::PRTE_Health_Facility_Services::LNPID::Refs::LIC';
...
//Scores for various field components
  ...
  INTEGER2 C_LIC_NBR_e2_Weight100 := SALT29.Min0(h.C_LIC_NBR_weight100 + 100*log(h.C_LIC_NBR_cnt/h.C_LIC_NBR_e2_cnt)/log(2)); // Precompute edit-distance specificity
...
DataForKey0 := DEDUP(SORT(TABLE(h(C_LIC_NBR NOT IN SET(s.nulls_C_LIC_NBR,C_LIC_NBR)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
EXPORT Key := INDEX(DataForKey0,,KeyName);
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,LNPID,C_LIC_NBR,LIC_STATE,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'LIC','FAC_NAME',CntRed_FAC_NAME*100,CntRed_FAC_NAME*TSize},{'LIC','ADDR1',CntRed_ADDR1*100,CntRed_ADDR1*TSize},{'LIC','LOCALE',CntRed_LOCALE*100,CntRed_LOCALE*TSize},{'LIC','TAXONOMY',CntRed_TAXONOMY*100,CntRed_TAXONOMY*TSize},{'LIC','TAXONOMY_CODE',CntRed_TAXONOMY_CODE*100,CntRed_TAXONOMY_CODE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.C_LIC_NBR <> (typeof(le.C_LIC_NBR))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR,TYPEOF(h.LIC_STATE) param_LIC_STATE) := 
    STEPPED( LIMIT( Key(
          ( C_LIC_NBR = param_C_LIC_NBR AND param_C_LIC_NBR <> (TYPEOF(C_LIC_NBR))'' )
      AND ( LIC_STATE = (TYPEOF(LIC_STATE))'' OR param_LIC_STATE = (TYPEOF(LIC_STATE))'' OR LIC_STATE = param_LIC_STATE  )),10000,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
EXPORT ScoredLNPIDFetch(TYPEOF(h.C_LIC_NBR) param_C_LIC_NBR,TYPEOF(h.LIC_STATE) param_LIC_STATE,TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.CNP_NUMBER) param_CNP_NUMBER,TYPEOF(h.CNP_STORE_NUMBER) param_CNP_STORE_NUMBER,TYPEOF(h.CNP_BTYPE) param_CNP_BTYPE,TYPEOF(h.CNP_LOWV) param_CNP_LOWV,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.TAXONOMY) param_TAXONOMY,TYPEOF(h.TAXONOMY_CODE) param_TAXONOMY_CODE) := FUNCTION
  RawData := RawFetch(param_C_LIC_NBR,param_LIC_STATE);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.C_LIC_NBR_match_code := MAP(
		le.C_LIC_NBR = (TYPEOF(le.C_LIC_NBR))'' OR le.C_LIC_NBR = (TYPEOF(le.C_LIC_NBR))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_C_LIC_NBR(le.C_LIC_NBR,param_C_LIC_NBR,TRUE));
    SELF.C_LIC_NBRWeight := (50+MAP ( le.C_LIC_NBR = param_C_LIC_NBR  => le.C_LIC_NBR_weight100,
          le.C_LIC_NBR = (TYPEOF(le.C_LIC_NBR))'' OR param_C_LIC_NBR = (TYPEOF(le.C_LIC_NBR))'' => 0,
           -0.907*le.C_LIC_NBR_weight100))/100*2.00; 
    SELF.LIC_STATE_match_code := MAP(
		le.LIC_STATE = (TYPEOF(le.LIC_STATE))'' OR param_LIC_STATE = (TYPEOF(param_LIC_STATE))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_LIC_STATE(le.LIC_STATE,param_LIC_STATE,FALSE));
    SELF.LIC_STATEWeight := (50+MAP ( le.LIC_STATE = param_LIC_STATE  => le.LIC_STATE_weight100,
           ...
           -0.991*le.LIC_STATE_weight100))/100; 
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.C_LIC_NBR_match_code := MAP(
		le.C_LIC_NBR = (TYPEOF(le.C_LIC_NBR))'' OR le.C_LIC_NBR = (TYPEOF(le.C_LIC_NBR))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_C_LIC_NBR(le.C_LIC_NBR,ri.C_LIC_NBR,TRUE));
    SELF.C_LIC_NBRWeight := (50+MAP ( le.C_LIC_NBR = ri.C_LIC_NBR  => le.C_LIC_NBR_weight100,
          le.C_LIC_NBR = (TYPEOF(le.C_LIC_NBR))'' OR ri.C_LIC_NBR = (TYPEOF(le.C_LIC_NBR))'' => 0,
           -0.907*le.C_LIC_NBR_weight100))/100*2.00; 
    SELF.LIC_STATE_match_code := MAP(
		le.LIC_STATE = (TYPEOF(le.LIC_STATE))'' OR ri.LIC_STATE = (TYPEOF(ri.LIC_STATE))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_LIC_STATE(le.LIC_STATE,ri.LIC_STATE,FALSE));
    ...
    SELF := le;
  END;
  Recs0 := Recs(C_LIC_NBR <> (typeof(C_LIC_NBR))'');
  ...
  J0 := JOIN(Recs1,Key,LEFT.C_LIC_NBR = RIGHT.C_LIC_NBR
     AND ( LEFT.LIC_STATE = (TYPEOF(LEFT.LIC_STATE))'' OR RIGHT.LIC_STATE = (TYPEOF(RIGHT.LIC_STATE))'' OR LEFT.LIC_STATE = RIGHT.LIC_STATE  ),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.C_LIC_NBR = RIGHT.C_LIC_NBR,10000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.C_LIC_NBR = RIGHT.C_LIC_NBR
     AND ( LEFT.LIC_STATE = (TYPEOF(LEFT.LIC_STATE))'' OR RIGHT.LIC_STATE = (TYPEOF(RIGHT.LIC_STATE))'' OR LEFT.LIC_STATE = RIGHT.LIC_STATE  ),Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.C_LIC_NBR = RIGHT.C_LIC_NBR,10000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
#IF(#TEXT(Input_C_LIC_NBR)<>'')
  ...
  output_file := PRTE_Health_Facility_Services.Key_HealthFacility_LIC.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

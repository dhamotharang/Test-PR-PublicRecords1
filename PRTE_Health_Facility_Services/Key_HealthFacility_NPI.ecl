EXPORT Key_HealthFacility_NPI := MODULE
IMPORT SALT29,ut,std;
//NPI_NUMBER:+:FAC_NAME:ADDR1:LOCALE:TAXONOMY:TAXONOMY_CODE
EXPORT KeyName := PRTE_Health_Facility_Services.Files.FILE_NPI; //'~'+'key::PRTE_Health_Facility_Services::LNPID::Refs::NPI';
...
EXPORT Shrinkage := DATASET([{'NPI','FAC_NAME',CntRed_FAC_NAME*100,CntRed_FAC_NAME*TSize},{'NPI','ADDR1',CntRed_ADDR1*100,CntRed_ADDR1*TSize},{'NPI','LOCALE',CntRed_LOCALE*100,CntRed_LOCALE*TSize},{'NPI','TAXONOMY',CntRed_TAXONOMY*100,CntRed_TAXONOMY*TSize},{'NPI','TAXONOMY_CODE',CntRed_TAXONOMY_CODE*100,CntRed_TAXONOMY_CODE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.NPI_NUMBER <> (typeof(le.NPI_NUMBER))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.NPI_NUMBER) param_NPI_NUMBER) := 
    STEPPED( LIMIT( Key(
          ( NPI_NUMBER = param_NPI_NUMBER AND param_NPI_NUMBER <> (TYPEOF(NPI_NUMBER))'' )),10000,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
EXPORT ScoredLNPIDFetch(TYPEOF(h.NPI_NUMBER) param_NPI_NUMBER,TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.CNP_NUMBER) param_CNP_NUMBER,TYPEOF(h.CNP_STORE_NUMBER) param_CNP_STORE_NUMBER,TYPEOF(h.CNP_BTYPE) param_CNP_BTYPE,TYPEOF(h.CNP_LOWV) param_CNP_LOWV,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.TAXONOMY) param_TAXONOMY,TYPEOF(h.TAXONOMY_CODE) param_TAXONOMY_CODE) := FUNCTION
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  ...
  J0 := JOIN(Recs1,Key,LEFT.NPI_NUMBER = RIGHT.NPI_NUMBER,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.NPI_NUMBER = RIGHT.NPI_NUMBER,10000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.NPI_NUMBER = RIGHT.NPI_NUMBER,Score_Batch(RIGHT,LEFT),
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_NPI_NUMBER='',Input_CNP_NAME='',Input_CNP_NUMBER='',Input_CNP_STORE_NUMBER='',Input_CNP_BTYPE='',Input_CNP_LOWV='',Input_PRIM_RANGE='',Input_SEC_RANGE='',Input_PRIM_NAME='',Input_V_CITY_NAME='',Input_ST='',Input_ZIP='',Input_TAXONOMY='',Input_TAXONOMY_CODE='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,PRTE_Health_Facility_Services;
#IF(#TEXT(Input_NPI_NUMBER)<>'')
  #uniquename(trans)
  PRTE_Health_Facility_Services.Key_HealthFacility_NPI.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Facility_Services.Key_HealthFacility_NPI.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

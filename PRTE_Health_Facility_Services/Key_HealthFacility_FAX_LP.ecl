EXPORT Key_HealthFacility_FAX_LP := MODULE
IMPORT SALT29,ut,std;
//FAX:+:FAC_NAME:ADDR1:LOCALE:TAXONOMY:TAXONOMY_CODE
EXPORT KeyName := PRTE_Health_Facility_Services.Files.FILE_FAX; //'~'+'key::PRTE_Health_Facility_Services::LNPID::Refs::FAX_LP';
...
//Scores for various field components
  h.FAX_weight100 ; // Contains 100x the specificity
  INTEGER2 FAX_CleanPhone_Weight100 := SALT29.Min0(h.FAX_weight100 + 100*log(h.FAX_cnt/h.FAX_CleanPhone_cnt)/log(2)); // Precompute CleanPhone specificity
...
DataForKey0 := DEDUP(SORT(TABLE(h(FAX NOT IN SET(s.nulls_FAX,FAX)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
EXPORT Key := INDEX(DataForKey0,,KeyName);
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  KeyCnt := COUNT(Key);
  TSize := KeyCnt * SIZEOF(RECORDOF(Key)) / 1000000000; // Key size in gigs
  Grpd := GROUP(Key,LNPID,FAX,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'FAX_LP','FAC_NAME',CntRed_FAC_NAME*100,CntRed_FAC_NAME*TSize},{'FAX_LP','ADDR1',CntRed_ADDR1*100,CntRed_ADDR1*TSize},{'FAX_LP','LOCALE',CntRed_LOCALE*100,CntRed_LOCALE*TSize},{'FAX_LP','TAXONOMY',CntRed_TAXONOMY*100,CntRed_TAXONOMY*TSize},{'FAX_LP','TAXONOMY_CODE',CntRed_TAXONOMY_CODE*100,CntRed_TAXONOMY_CODE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.FAX <> (typeof(le.FAX))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.FAX) param_FAX) := 
    STEPPED( LIMIT( Key(
          ( FAX = param_FAX AND param_FAX <> (TYPEOF(FAX))'' )),10000,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
EXPORT ScoredLNPIDFetch(TYPEOF(h.FAX) param_FAX,TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.CNP_NUMBER) param_CNP_NUMBER,TYPEOF(h.CNP_STORE_NUMBER) param_CNP_STORE_NUMBER,TYPEOF(h.CNP_BTYPE) param_CNP_BTYPE,TYPEOF(h.CNP_LOWV) param_CNP_LOWV,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.TAXONOMY) param_TAXONOMY,TYPEOF(h.TAXONOMY_CODE) param_TAXONOMY_CODE) := FUNCTION
  RawData := RawFetch(param_FAX);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.FAX_match_code := MAP(
		le.FAX = (TYPEOF(le.FAX))'' OR le.FAX = (TYPEOF(le.FAX))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_FAX(le.FAX,param_FAX,TRUE));
    SELF.FAXWeight := (50+MAP ( le.FAX = param_FAX  => le.FAX_weight100,
          le.FAX = (TYPEOF(le.FAX))'' OR param_FAX = (TYPEOF(le.FAX))'' => 0,
           -0.815*le.FAX_weight100))/100*0.25; 
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.FAX_match_code := MAP(
		le.FAX = (TYPEOF(le.FAX))'' OR le.FAX = (TYPEOF(le.FAX))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_FAX(le.FAX,ri.FAX,TRUE));
    SELF.FAXWeight := (50+MAP ( le.FAX = ri.FAX  => le.FAX_weight100,
          le.FAX = (TYPEOF(le.FAX))'' OR ri.FAX = (TYPEOF(le.FAX))'' => 0,
           -0.815*le.FAX_weight100))/100*0.25; 
    SELF.CNP_NAME_match_code := MAP(
  ...
  J0 := JOIN(Recs1,Key,LEFT.FAX = RIGHT.FAX,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.FAX = RIGHT.FAX,10000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.FAX = RIGHT.FAX,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.FAX = RIGHT.FAX,10000),HASH,HINT(unsorted_output)); // PULL used to cause non-indexed join
  J2 := IF(AsIndex,J0,J1);
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_FAX='',Input_CNP_NAME='',Input_CNP_NUMBER='',Input_CNP_STORE_NUMBER='',Input_CNP_BTYPE='',Input_CNP_LOWV='',Input_PRIM_RANGE='',Input_SEC_RANGE='',Input_PRIM_NAME='',Input_V_CITY_NAME='',Input_ST='',Input_ZIP='',Input_TAXONOMY='',Input_TAXONOMY_CODE='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,PRTE_Health_Facility_Services;
#IF(#TEXT(Input_FAX)<>'')
  #uniquename(trans)
  PRTE_Health_Facility_Services.Key_HealthFacility_FAX_LP.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Facility_Services.Key_HealthFacility_FAX_LP.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

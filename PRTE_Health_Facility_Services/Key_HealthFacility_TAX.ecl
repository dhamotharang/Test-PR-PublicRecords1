EXPORT Key_HealthFacility_TAX := MODULE
IMPORT SALT29,ut,std;
//TAX_ID:+:FAC_NAME:ADDR1:LOCALE:TAXONOMY:TAXONOMY_CODE
EXPORT KeyName := PRTE_Health_Facility_Services.Files.FILE_Tax; //'~'+'key::PRTE_Health_Facility_Services::LNPID::Refs::TAX';
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,LNPID,TAX_ID,LOCAL); // Not perfect, does not need to be - for statistics
...
EXPORT Shrinkage := DATASET([{'TAX','FAC_NAME',CntRed_FAC_NAME*100,CntRed_FAC_NAME*TSize},{'TAX','ADDR1',CntRed_ADDR1*100,CntRed_ADDR1*TSize},{'TAX','LOCALE',CntRed_LOCALE*100,CntRed_LOCALE*TSize},{'TAX','TAXONOMY',CntRed_TAXONOMY*100,CntRed_TAXONOMY*TSize},{'TAX','TAXONOMY_CODE',CntRed_TAXONOMY_CODE*100,CntRed_TAXONOMY_CODE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.TAX_ID <> (typeof(le.TAX_ID))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.TAX_ID) param_TAX_ID) := 
    STEPPED( LIMIT( Key(
          ( TAX_ID = param_TAX_ID AND param_TAX_ID <> (TYPEOF(TAX_ID))'' )),10000,ONFAIL(TRANSFORM(KeyRec,SELF := ROW([],KeyRec))),KEYED),LNPID);
EXPORT ScoredLNPIDFetch(TYPEOF(h.TAX_ID) param_TAX_ID,TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.CNP_NUMBER) param_CNP_NUMBER,TYPEOF(h.CNP_STORE_NUMBER) param_CNP_STORE_NUMBER,TYPEOF(h.CNP_BTYPE) param_CNP_BTYPE,TYPEOF(h.CNP_LOWV) param_CNP_LOWV,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.TAXONOMY) param_TAXONOMY,TYPEOF(h.TAXONOMY_CODE) param_TAXONOMY_CODE) := FUNCTION
  RawData := RawFetch(param_TAX_ID);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.TAX_ID_match_code := MAP(
		le.TAX_ID = (TYPEOF(le.TAX_ID))'' OR le.TAX_ID = (TYPEOF(le.TAX_ID))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_TAX_ID(le.TAX_ID,param_TAX_ID,TRUE));
    SELF.TAX_IDWeight := (50+MAP ( le.TAX_ID = param_TAX_ID  => le.TAX_ID_weight100,
          le.TAX_ID = (TYPEOF(le.TAX_ID))'' OR param_TAX_ID = (TYPEOF(le.TAX_ID))'' => 0,
           -1.000*le.TAX_ID_weight100))/100*0.25; 
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.TAX_ID_match_code := MAP(
		le.TAX_ID = (TYPEOF(le.TAX_ID))'' OR le.TAX_ID = (TYPEOF(le.TAX_ID))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_TAX_ID(le.TAX_ID,ri.TAX_ID,TRUE));
  ...
  J0 := JOIN(Recs1,Key,LEFT.TAX_ID = RIGHT.TAX_ID,Score_Batch(RIGHT,LEFT),
    ATMOST(LEFT.TAX_ID = RIGHT.TAX_ID,10000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),LEFT.TAX_ID = RIGHT.TAX_ID,Score_Batch(RIGHT,LEFT),
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_TAX_ID='',Input_CNP_NAME='',Input_CNP_NUMBER='',Input_CNP_STORE_NUMBER='',Input_CNP_BTYPE='',Input_CNP_LOWV='',Input_PRIM_RANGE='',Input_SEC_RANGE='',Input_PRIM_NAME='',Input_V_CITY_NAME='',Input_ST='',Input_ZIP='',Input_TAXONOMY='',Input_TAXONOMY_CODE='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,PRTE_Health_Facility_Services;
#IF(#TEXT(Input_TAX_ID)<>'')
  #uniquename(trans)
  PRTE_Health_Facility_Services.Key_HealthFacility_TAX.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Facility_Services.Key_HealthFacility_TAX.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

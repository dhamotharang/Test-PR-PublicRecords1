EXPORT Key_HealthFacility_SBNAME := MODULE
...
EXPORT KeyName := PRTE_Health_Facility_Services.Files.FILE_BName_St; //'~'+'key::PRTE_Health_Facility_Services::LNPID::Refs::SBNAME';
...
DataForKey0 := DEDUP(SORT(TABLE(h(CNP_NAME NOT IN SET(s.nulls_CNP_NAME,CNP_NAME),ST NOT IN SET(s.nulls_ST,ST),ZIP NOT IN SET(s.nulls_ZIP,ZIP)),layout),WHOLE RECORD,LOCAL),WHOLE RECORD,LOCAL); // Project out the fields in match candidates required for this Name()
...
// Compute shrinkage stats; the amount we could shrink the key for each extra credit removal
  ...
  Grpd := GROUP(Key,LNPID,CNP_NAME,ST,PRIM_NAME,ZIP,V_CITY_NAME,LOCAL); // Not perfect, does not need to be - for statistics
  ...
  Rem_CNP_STORE_NUMBER := GROUP( DEDUP( SORT( Grpd, EXCEPT CNP_STORE_NUMBER), EXCEPT CNP_STORE_NUMBER));
  CntRed_CNP_STORE_NUMBER := (KeyCnt-COUNT(Rem_CNP_STORE_NUMBER))/KeyCnt;
...
EXPORT Shrinkage := DATASET([{'SBNAME','CNP_NUMBER',CntRed_CNP_NUMBER*100,CntRed_CNP_NUMBER*TSize},{'SBNAME','CNP_STORE_NUMBER',CntRed_CNP_STORE_NUMBER*100,CntRed_CNP_STORE_NUMBER*TSize},{'SBNAME','CNP_BTYPE',CntRed_CNP_BTYPE*100,CntRed_CNP_BTYPE*TSize},{'SBNAME','CNP_LOWV',CntRed_CNP_LOWV*100,CntRed_CNP_LOWV*TSize},{'SBNAME','PRIM_RANGE',CntRed_PRIM_RANGE*100,CntRed_PRIM_RANGE*TSize},{'SBNAME','SEC_RANGE',CntRed_SEC_RANGE*100,CntRed_SEC_RANGE*TSize},{'SBNAME','TAXONOMY',CntRed_TAXONOMY*100,CntRed_TAXONOMY*TSize},{'SBNAME','TAXONOMY_CODE',CntRed_TAXONOMY_CODE*100,CntRed_TAXONOMY_CODE*TSize}],SALT29.ShrinkLayout);
EXPORT CanSearch(Process_xLNPID_Layouts.InputLayout le) := le.CNP_NAME <> (typeof(le.CNP_NAME))'' AND le.ST <> (typeof(le.ST))'';
KeyRec := RECORDOF(Key);
EXPORT RawFetch(TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME) := 
  FUNCTION
 //Generate service attributes for GSS join
    wds := SALT29.fn_string_to_wordstream(param_CNP_NAME);
    indexOutputRecord := RECORDOF(Key);
    slimrec := { Key.gss_word_weight, Key.LNPID };
    ...
    doIndexRead(UNSIGNED4 search,UNSIGNED2 spc) := STEPPED(KEY( GSS_hash = search AND (GSS_bloom & BloomF) = BloomF
      ...
      AND ( ZIP = (TYPEOF(ZIP))'' OR param_ZIP = (TYPEOF(ZIP))'' OR ZIP = param_ZIP  )
      AND ( V_CITY_NAME = (TYPEOF(V_CITY_NAME))'' OR param_V_CITY_NAME = (TYPEOF(V_CITY_NAME))'' OR metaphonelib.DMetaPhone1(V_CITY_NAME)=metaphonelib.DMetaPhone1(param_V_CITY_NAME) OR SALT29.WithinEditN(V_CITY_NAME,param_V_CITY_NAME,2, 0)  )),LNPID,PRIORITY(40-spc)); // Filter for each row of index fetch
    SALT29.MAC_collate_wordbag_matches1(wds,slimrec,doIndexRead,LNPID,steppedmatches) // Perform N-way join
    res := JOIN( steppedmatches, Key, RIGHT.GSS_Hash = wds[1].hsh
      AND ( RIGHT.ST = param_ST AND param_ST <> (TYPEOF(RIGHT.ST))'' ) AND LEFT.LNPID = RIGHT.LNPID,TRANSFORM(indexOutputRecord,SELF.gss_word_weight := LEFT.gss_word_weight,SELF := RIGHT));
    RETURN IF(SUM(wds,spec) > 5,res,IF(SUM(wds,spec) = 0,DATASET([],indexOutputRecord) ,DATASET(ROW([],indexOutputRecord)))); // Ensure at least spc of specificity in gss portion
  END;
EXPORT ScoredLNPIDFetch(TYPEOF(h.CNP_NAME) param_CNP_NAME,TYPEOF(h.ST) param_ST,TYPEOF(h.PRIM_NAME) param_PRIM_NAME,TYPEOF(h.ZIP) param_ZIP,TYPEOF(h.V_CITY_NAME) param_V_CITY_NAME,TYPEOF(h.CNP_NUMBER) param_CNP_NUMBER,TYPEOF(h.CNP_STORE_NUMBER) param_CNP_STORE_NUMBER,TYPEOF(h.CNP_BTYPE) param_CNP_BTYPE,TYPEOF(h.CNP_LOWV) param_CNP_LOWV,TYPEOF(h.PRIM_RANGE) param_PRIM_RANGE,TYPEOF(h.SEC_RANGE) param_SEC_RANGE,TYPEOF(h.TAXONOMY) param_TAXONOMY,TYPEOF(h.TAXONOMY_CODE) param_TAXONOMY_CODE) := FUNCTION
  RawData := RawFetch(param_CNP_NAME,param_ST,param_PRIM_NAME,param_ZIP,param_V_CITY_NAME);
  Process_xLNPID_Layouts.LayoutScoredFetch Score(RawData le) := TRANSFORM
    ...
    SELF.CNP_NAME_match_code := match_methods(File_HealthFacility).match_CNP_NAME(le.CNP_NAME,param_CNP_NAME,TRUE);
  ...
  SELF.CNP_NAME_gss_cases := DATASET([{le.gss_hash}],SALT29.layout_GSS_cases);
    SELF.ST_match_code := MAP(
		le.ST = (TYPEOF(le.ST))'' OR le.ST = (TYPEOF(le.ST))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_ST(le.ST,param_ST,TRUE));
    ...
    SELF.Weight :=MAX(0,SELF.CNP_NAMEWeight) + MAX(0,SELF.STWeight) + MAX(0,SELF.PRIM_NAMEWeight) + MAX(0,SELF.ZIPWeight) + MAX(0,SELF.V_CITY_NAMEWeight) + MAX(0,SELF.CNP_NUMBERWeight) + MAX(0,SELF.CNP_STORE_NUMBERWeight) + MAX(0,SELF.CNP_BTYPEWeight) + MAX(0,SELF.CNP_LOWVWeight) + MAX(0,SELF.PRIM_RANGEWeight) + MAX(0,SELF.SEC_RANGEWeight) + MAX(0,SELF.TAXONOMYWeight) + MAX(0,SELF.TAXONOMY_CODEWeight);
    SELF := le;
  END;
...
EXPORT ScoredFetch_Batch(DATASET(InputLayout_Batch) recs,BOOLEAN AsIndex) := FUNCTION
  Process_xLNPID_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := TRANSFORM
    ...
    SELF.CNP_NAME_match_code := match_methods(File_HealthFacility).match_CNP_NAME(le.CNP_NAME,ri.CNP_NAME,TRUE);
    SELF.CNP_NAMEWeight := IF(SALT29.MatchBagOfWords(le.CNP_NAME,ri.CNP_NAME,3407639,0)<=Config.CNP_NAME_force,SKIP,SALT29.MatchBagOfWords(le.CNP_NAME,ri.CNP_NAME,3407639,0)/100); 
    SELF.ST_match_code := MAP(
		le.ST = (TYPEOF(le.ST))'' OR le.ST = (TYPEOF(le.ST))'' => SALT29.MatchCode.OneSideNull,match_methods(File_HealthFacility).match_ST(le.ST,ri.ST,TRUE));
  ...
  J0 := JOIN(Recs1,Key,HASH32(SALT29.fn_bow_bestword(LEFT.CNP_NAME))=RIGHT.gss_hash AND SALT29.MatchBagOfWords(LEFT.CNP_NAME,RIGHT.CNP_NAME,3407639,0) > Config.CNP_NAME_Force * 100
     ...
     AND ( LEFT.ZIP = (TYPEOF(LEFT.ZIP))'' OR RIGHT.ZIP = (TYPEOF(RIGHT.ZIP))'' OR LEFT.ZIP = RIGHT.ZIP  )
     AND ( LEFT.V_CITY_NAME = (TYPEOF(LEFT.V_CITY_NAME))'' OR RIGHT.V_CITY_NAME = (TYPEOF(RIGHT.V_CITY_NAME))'' OR metaphonelib.DMetaPhone1(LEFT.V_CITY_NAME)=metaphonelib.DMetaPhone1(RIGHT.V_CITY_NAME) OR SALT29.WithinEditN(LEFT.V_CITY_NAME,RIGHT.V_CITY_NAME,2, 0)  ),Score_Batch(RIGHT,LEFT),
    ATMOST(HASH32(SALT29.fn_bow_bestword(LEFT.CNP_NAME))=RIGHT.gss_hash
     AND LEFT.ST = RIGHT.ST,10000)); // Use indexed join (used for smaller batches
  J1 := JOIN(Recs1,PULL(Key),HASH32(SALT29.fn_bow_bestword(LEFT.CNP_NAME))=RIGHT.gss_hash AND SALT29.MatchBagOfWords(LEFT.CNP_NAME,RIGHT.CNP_NAME,3407639,0) > Config.CNP_NAME_Force * 100
     AND LEFT.ST = RIGHT.ST
     AND ( LEFT.PRIM_NAME = (TYPEOF(LEFT.PRIM_NAME))'' OR RIGHT.PRIM_NAME = (TYPEOF(RIGHT.PRIM_NAME))'' OR SALT29.WithinEditN(LEFT.PRIM_NAME,RIGHT.PRIM_NAME,1, 0)  )
     AND ( LEFT.ZIP = (TYPEOF(LEFT.ZIP))'' OR RIGHT.ZIP = (TYPEOF(RIGHT.ZIP))'' OR LEFT.ZIP = RIGHT.ZIP  )
     AND ( LEFT.V_CITY_NAME = (TYPEOF(LEFT.V_CITY_NAME))'' OR RIGHT.V_CITY_NAME = (TYPEOF(RIGHT.V_CITY_NAME))'' OR metaphonelib.DMetaPhone1(LEFT.V_CITY_NAME)=metaphonelib.DMetaPhone1(RIGHT.V_CITY_NAME) OR SALT29.WithinEditN(LEFT.V_CITY_NAME,RIGHT.V_CITY_NAME,2, 0)  ),Score_Batch(RIGHT,LEFT),
...
EXPORT MAC_ScoredFetch_Batch(InFile,Input_Ref,Input_CNP_NAME='',Input_ST='',Input_PRIM_NAME='',Input_ZIP='',Input_V_CITY_NAME='',Input_CNP_NUMBER='',Input_CNP_STORE_NUMBER='',Input_CNP_BTYPE='',Input_CNP_LOWV='',Input_PRIM_RANGE='',Input_SEC_RANGE='',Input_TAXONOMY='',Input_TAXONOMY_CODE='',output_file,AsIndex='true') := MACRO
IMPORT SALT29,PRTE_Health_Facility_Services;
#IF(#TEXT(Input_CNP_NAME)<>'' AND #TEXT(Input_ST)<>'')
  #uniquename(trans)
  PRTE_Health_Facility_Services.Key_HealthFacility_SBNAME.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := PRTE_Health_Facility_Services.Key_HealthFacility_SBNAME.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

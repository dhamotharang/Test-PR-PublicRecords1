...
IMPORT SALT24,BIPV2_ProxID_dev2,ut,tools;
...
BIPV2_ProxID_dev2.match_candidates(BFile).layout_attribute_matches ainto(BIPV2_ProxID_dev2.Keys(BFile).Attribute_Matches le) := TRANSFORM
  SELF := le;
END;
am := PROJECT(BIPV2_ProxID_dev2.Keys(BFile).Attribute_Matches(Proxid1=Proxidone,Proxid2=Proxidtwo)+BIPV2_ProxID_dev2.Keys(BFile).Attribute_Matches(Proxid1=Proxidtwo,Proxid2=Proxidone),ainto(LEFT));
mtch := BIPV2_ProxID_dev2.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,Proxidone,Proxidtwo,0,0}],BIPV2_ProxID_dev2.match_candidates(BFile).layout_matches),am);
// Put out easy to read versions of the 5055296 data

//mtch := BIPV2_ProxID_dev2.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,Proxidone,Proxidtwo,0,0}],BIPV2_ProxID_dev2.match_candidates(BFile).layout_matches));

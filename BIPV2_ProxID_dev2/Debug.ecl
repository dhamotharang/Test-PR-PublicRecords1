...
EXPORT Layout_Sample_Matches := RECORD,MAXLENGTH(32000)
  ...
  SALT27.StrType   Matching_Attributes := ''; // Keys from attribute files which match
  ...
  typeof(h.company_address_type_raw) left_company_address_type_raw;
  typeof(h.company_address_type_raw) right_company_address_type_raw;
  typeof(h.company_address_type_derived) left_company_address_type_derived;
  typeof(h.company_address_type_derived) right_company_address_type_derived;
  ...
  UNSIGNED2 support_company_address := 0; // External support for company_address
END;
EXPORT layout_sample_matches sample_match_join(match_candidates(ih).layout_candidates le,match_candidates(ih).layout_candidates ri,UNSIGNED outside=0,UNSIGNED company_address_support = 0) := TRANSFORM
  SELF.support_company_address := company_address_support;
  ...
  SELF.left_company_address_type_raw := le.company_address_type_raw;
  SELF.right_company_address_type_raw := ri.company_address_type_raw;
  SELF.left_company_address_type_derived := le.company_address_type_derived;
  SELF.right_company_address_type_derived := ri.company_address_type_derived;
  ...
  SELF.prim_range_score := IF ( prim_range_score_temp >= 0 OR company_addr1_score_pre > 0 OR company_address_score_pre > 0 OR company_address_support > 0, prim_range_score_temp, -9999 ); // Enforce FORCE parameter
  SELF.prim_range_skipped := SELF.prim_range_score < -5000;// Enforce FORCE parameter
 
  SELF.prim_name_score := IF ( prim_name_score_temp > 0 OR company_addr1_score_pre > 0 OR company_address_score_pre > 0 OR company_address_support > 0, prim_name_score_temp, -9999 ); // Enforce FORCE parameter
  SELF.prim_name_skipped := SELF.prim_name_score < -5000;// Enforce FORCE parameter
 
  SELF.st_score := IF ( st_score_temp > 0 OR company_csz_score_pre > 0 OR company_address_score_pre > 0 OR company_address_support > 0, st_score_temp, -9999 ); // Enforce FORCE parameter
...
// Compute the score for the concept company_address
  ...
  SELF.company_address_score := IF ( company_address_score_eext > 0,company_address_score_res,-9999);
  ...
  SELF.Conf := (SELF.cnp_number_score + SELF.prim_range_score + SELF.prim_name_score + SELF.st_score + SELF.active_enterprise_number_score + SELF.hist_enterprise_number_score + SELF.hist_duns_number_score + SELF.hist_domestic_corp_key_score + SELF.unk_corp_key_score + SELF.ebr_file_number_score + SELF.active_duns_number_score + SELF.foreign_corp_key_score + SELF.company_phone_score + SELF.active_domestic_corp_key_score + SELF.cnp_name_score + SELF.company_fein_score + SELF.company_address_score + SELF.company_addr1_score + SELF.zip_score + SELF.company_csz_score + SELF.sec_range_score + SELF.v_city_name_score + SELF.cnp_btype_score + SELF.iscorp_score) / 100 + outside;
END;
...
EXPORT Layout_RolledEntity := RECORD,MAXLENGTH(63000)
  ...
  DATASET(SALT27.Layout_FieldValueList) cnp_number_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) active_enterprise_number_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) hist_enterprise_number_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) hist_duns_number_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) hist_domestic_corp_key_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) unk_corp_key_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) ebr_file_number_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) active_duns_number_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) foreign_corp_key_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) company_phone_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) active_domestic_corp_key_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) cnp_name_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) company_fein_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) company_address_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) cnp_btype_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) iscorp_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) company_name_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) cnp_hasnumber_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) cnp_lowv_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) cnp_translated_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) cnp_classid_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) company_foreign_domestic_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) company_bdid_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) company_address_type_raw_Values := DATASET([],SALT27.Layout_FieldValueList);
  DATASET(SALT27.Layout_FieldValueList) company_address_type_derived_Values := DATASET([],SALT27.Layout_FieldValueList);
...
Layout_RolledEntity RollValues(Layout_RolledEntity le,Layout_RolledEntity ri) := TRANSFORM
  ...
  SELF.cnp_number_values := SALT27.fn_combine_fieldvaluelist(le.cnp_number_values,ri.cnp_number_values);
  SELF.active_enterprise_number_values := SALT27.fn_combine_fieldvaluelist(le.active_enterprise_number_values,ri.active_enterprise_number_values);
  SELF.hist_enterprise_number_values := SALT27.fn_combine_fieldvaluelist(le.hist_enterprise_number_values,ri.hist_enterprise_number_values);
  SELF.hist_duns_number_values := SALT27.fn_combine_fieldvaluelist(le.hist_duns_number_values,ri.hist_duns_number_values);
  SELF.hist_domestic_corp_key_values := SALT27.fn_combine_fieldvaluelist(le.hist_domestic_corp_key_values,ri.hist_domestic_corp_key_values);
  SELF.unk_corp_key_values := SALT27.fn_combine_fieldvaluelist(le.unk_corp_key_values,ri.unk_corp_key_values);
  SELF.ebr_file_number_values := SALT27.fn_combine_fieldvaluelist(le.ebr_file_number_values,ri.ebr_file_number_values);
  SELF.active_duns_number_values := SALT27.fn_combine_fieldvaluelist(le.active_duns_number_values,ri.active_duns_number_values);
  SELF.foreign_corp_key_values := SALT27.fn_combine_fieldvaluelist(le.foreign_corp_key_values,ri.foreign_corp_key_values);
  SELF.company_phone_values := SALT27.fn_combine_fieldvaluelist(le.company_phone_values,ri.company_phone_values);
  SELF.active_domestic_corp_key_values := SALT27.fn_combine_fieldvaluelist(le.active_domestic_corp_key_values,ri.active_domestic_corp_key_values);
  SELF.cnp_name_values := SALT27.fn_combine_fieldvaluelist(le.cnp_name_values,ri.cnp_name_values);
  SELF.company_fein_values := SALT27.fn_combine_fieldvaluelist(le.company_fein_values,ri.company_fein_values);
  SELF.company_address_values := SALT27.fn_combine_fieldvaluelist(le.company_address_values,ri.company_address_values);
  SELF.cnp_btype_values := SALT27.fn_combine_fieldvaluelist(le.cnp_btype_values,ri.cnp_btype_values);
  SELF.iscorp_values := SALT27.fn_combine_fieldvaluelist(le.iscorp_values,ri.iscorp_values);
  SELF.company_name_values := SALT27.fn_combine_fieldvaluelist(le.company_name_values,ri.company_name_values);
  SELF.cnp_hasnumber_values := SALT27.fn_combine_fieldvaluelist(le.cnp_hasnumber_values,ri.cnp_hasnumber_values);
  SELF.cnp_lowv_values := SALT27.fn_combine_fieldvaluelist(le.cnp_lowv_values,ri.cnp_lowv_values);
  SELF.cnp_translated_values := SALT27.fn_combine_fieldvaluelist(le.cnp_translated_values,ri.cnp_translated_values);
  SELF.cnp_classid_values := SALT27.fn_combine_fieldvaluelist(le.cnp_classid_values,ri.cnp_classid_values);
  SELF.company_foreign_domestic_values := SALT27.fn_combine_fieldvaluelist(le.company_foreign_domestic_values,ri.company_foreign_domestic_values);
  SELF.company_bdid_values := SALT27.fn_combine_fieldvaluelist(le.company_bdid_values,ri.company_bdid_values);
  SELF.company_address_type_raw_values := SALT27.fn_combine_fieldvaluelist(le.company_address_type_raw_values,ri.company_address_type_raw_values);
  SELF.company_address_type_derived_values := SALT27.fn_combine_fieldvaluelist(le.company_address_type_derived_values,ri.company_address_type_derived_values);
...
Layout_RolledEntity into(in_data le) := TRANSFORM
  SELF.Proxid := le.Proxid;
  SELF.cnp_number_Values := IF ( le.cnp_number  IN SET(s.nulls_cnp_number,cnp_number),DATASET([],SALT27.Layout_FieldValueList),DATASET([{TRIM((SALT27.StrType)le.cnp_number)}],SALT27.Layout_FieldValueList));
  SELF.active_enterprise_number_Values := IF ( le.active_enterprise_number  IN SET(s.nulls_active_enterprise_number,active_enterprise_number),DATASET([],SALT27.Layout_FieldValueList),DATASET([{TRIM((SALT27.StrType)le.active_enterprise_number)}],SALT27.Layout_FieldValueList));
  SELF.hist_enterprise_number_Values := IF ( le.hist_enterprise_number  IN SET(s.nulls_hist_enterprise_number,hist_enterprise_number),DATASET([],SALT27.Layout_FieldValueList),DATASET([{TRIM((SALT27.StrType)le.hist_enterprise_number)}],SALT27.Layout_FieldValueList));
  SELF.hist_duns_number_Values := IF ( le.hist_duns_number  IN SET(s.nulls_hist_duns_number,hist_duns_number),DATASET([],SALT27.Layout_FieldValueList),DATASET([{TRIM((SALT27.StrType)le.hist_duns_number)}],SALT27.Layout_FieldValueList));
  SELF.hist_domestic_corp_key_Values := IF ( le.hist_domestic_corp_key  IN SET(s.nulls_hist_domestic_corp_key,hist_domestic_corp_key),DATASET([],SALT27.Layout_FieldValueList),DATASET([{TRIM((SALT27.StrType)le.hist_domestic_corp_key)}],SALT27.Layout_FieldValueList));
  SELF.unk_corp_key_Values := IF ( le.unk_corp_key  IN SET(s.nulls_unk_corp_key,unk_corp_key),DATASET([],SALT27.Layout_FieldValueList),DATASET([{TRIM((SALT27.StrType)le.unk_corp_key)}],SALT27.Layout_FieldValueList));
  SELF.ebr_file_number_Values := IF ( le.ebr_file_number  IN SET(s.nulls_ebr_file_number,ebr_file_number),DATASET([],SALT27.Layout_FieldValueList),DATASET([{TRIM((SALT27.StrType)le.ebr_file_number)}],SALT27.Layout_FieldValueList));
  SELF.active_duns_number_Values := IF ( le.active_duns_number  IN SET(s.nulls_active_duns_number,active_duns_number),DATASET([],SALT27.Layout_FieldValueList),DATASET([{TRIM((SALT27.StrType)le.active_duns_number)}],SALT27.Layout_FieldValueList));
  SELF.foreign_corp_key_Values := IF ( le.foreign_corp_key  IN SET(s.nulls_foreign_corp_key,foreign_corp_key),DATASET([],SALT27.Layout_FieldValueList),DATASET([{TRIM((SALT27.StrType)le.foreign_corp_key)}],SALT27.Layout_FieldValueList));
  ...
  SELF.active_domestic_corp_key_Values := IF ( le.active_domestic_corp_key  IN SET(s.nulls_active_domestic_corp_key,active_domestic_corp_key),DATASET([],SALT27.Layout_FieldValueList),DATASET([{TRIM((SALT27.StrType)le.active_domestic_corp_key)}],SALT27.Layout_FieldValueList));
  SELF.cnp_name_Values := IF ( le.cnp_name  IN SET(s.nulls_cnp_name,cnp_name),DATASET([],SALT27.Layout_FieldValueList),DATASET([{TRIM((SALT27.StrType)le.cnp_name)}],SALT27.Layout_FieldValueList));
  ...
  self.company_address_Values := IF ( le.prim_range  IN SET(s.nulls_prim_range,prim_range) AND le.prim_name  IN SET(s.nulls_prim_name,prim_name) AND le.sec_range  IN SET(s.nulls_sec_range,sec_range) AND le.v_city_name  IN SET(s.nulls_v_city_name,v_city_name) AND le.st  IN SET(s.nulls_st,st) AND le.zip  IN SET(s.nulls_zip,zip),dataset([],SALT27.Layout_FieldValueList),dataset([{TRIM((SALT27.StrType)le.prim_range) + ' ' + TRIM((SALT27.StrType)le.prim_name) + ' ' + TRIM((SALT27.StrType)le.sec_range) + ' ' + TRIM((SALT27.StrType)le.v_city_name) + ' ' + TRIM((SALT27.StrType)le.st) + ' ' + TRIM((SALT27.StrType)le.zip)}],SALT27.Layout_FieldValueList));
  SELF.cnp_btype_Values := IF ( le.cnp_btype  IN SET(s.nulls_cnp_btype,cnp_btype),DATASET([],SALT27.Layout_FieldValueList),DATASET([{TRIM((SALT27.StrType)le.cnp_btype)}],SALT27.Layout_FieldValueList));
  SELF.iscorp_Values := IF ( le.iscorp  IN SET(s.nulls_iscorp,iscorp),DATASET([],SALT27.Layout_FieldValueList),DATASET([{TRIM((SALT27.StrType)le.iscorp)}],SALT27.Layout_FieldValueList));
  SELF.company_name_Values := DATASET([{TRIM((SALT27.StrType)le.company_name)}],SALT27.Layout_FieldValueList);
  SELF.cnp_hasnumber_Values := DATASET([{TRIM((SALT27.StrType)le.cnp_hasnumber)}],SALT27.Layout_FieldValueList);
  SELF.cnp_lowv_Values := DATASET([{TRIM((SALT27.StrType)le.cnp_lowv)}],SALT27.Layout_FieldValueList);
  SELF.cnp_translated_Values := DATASET([{TRIM((SALT27.StrType)le.cnp_translated)}],SALT27.Layout_FieldValueList);
  SELF.cnp_classid_Values := DATASET([{TRIM((SALT27.StrType)le.cnp_classid)}],SALT27.Layout_FieldValueList);
  SELF.company_foreign_domestic_Values := DATASET([{TRIM((SALT27.StrType)le.company_foreign_domestic)}],SALT27.Layout_FieldValueList);
  SELF.company_bdid_Values := DATASET([{TRIM((SALT27.StrType)le.company_bdid)}],SALT27.Layout_FieldValueList);
  SELF.company_address_type_raw_Values := DATASET([{TRIM((SALT27.StrType)le.company_address_type_raw)}],SALT27.Layout_FieldValueList);
  SELF.company_address_type_derived_Values := DATASET([{TRIM((SALT27.StrType)le.company_address_type_derived)}],SALT27.Layout_FieldValueList);
...
Layout_Chubbies0 NoteSize(Layout_Chubbies0 le) := TRANSFORM
  SELF.cnp_number_size := SALT27.Fn_SwitchSpec(s.cnp_number_switch,count(le.cnp_number_values));
  SELF.active_enterprise_number_size := SALT27.Fn_SwitchSpec(s.active_enterprise_number_switch,count(le.active_enterprise_number_values));
  SELF.hist_enterprise_number_size := SALT27.Fn_SwitchSpec(s.hist_enterprise_number_switch,count(le.hist_enterprise_number_values));
  SELF.hist_duns_number_size := SALT27.Fn_SwitchSpec(s.hist_duns_number_switch,count(le.hist_duns_number_values));
  SELF.hist_domestic_corp_key_size := SALT27.Fn_SwitchSpec(s.hist_domestic_corp_key_switch,count(le.hist_domestic_corp_key_values));
  SELF.unk_corp_key_size := SALT27.Fn_SwitchSpec(s.unk_corp_key_switch,count(le.unk_corp_key_values));
  SELF.ebr_file_number_size := SALT27.Fn_SwitchSpec(s.ebr_file_number_switch,count(le.ebr_file_number_values));
  SELF.active_duns_number_size := SALT27.Fn_SwitchSpec(s.active_duns_number_switch,count(le.active_duns_number_values));
  SELF.foreign_corp_key_size := SALT27.Fn_SwitchSpec(s.foreign_corp_key_switch,count(le.foreign_corp_key_values));
  SELF.company_phone_size := SALT27.Fn_SwitchSpec(s.company_phone_switch,count(le.company_phone_values));
  SELF.active_domestic_corp_key_size := SALT27.Fn_SwitchSpec(s.active_domestic_corp_key_switch,count(le.active_domestic_corp_key_values));
  SELF.cnp_name_size := SALT27.Fn_SwitchSpec(s.cnp_name_switch,count(le.cnp_name_values));
  SELF.company_fein_size := SALT27.Fn_SwitchSpec(s.company_fein_switch,count(le.company_fein_values));
  SELF.company_address_size := SALT27.Fn_SwitchSpec(s.company_address_switch,count(le.company_address_values));
  SELF.cnp_btype_size := SALT27.Fn_SwitchSpec(s.cnp_btype_switch,count(le.cnp_btype_values));
  SELF.iscorp_size := SALT27.Fn_SwitchSpec(s.iscorp_switch,count(le.iscorp_values));
  SELF := le;
END;  t := PROJECT(t0,NoteSize(LEFT));
Layout_Chubbies := RECORD,MAXLENGTH(63000)
  t;
  UNSIGNED2 Size := t.cnp_number_size+t.active_enterprise_number_size+t.hist_enterprise_number_size+t.hist_duns_number_size+t.hist_domestic_corp_key_size+t.unk_corp_key_size+t.ebr_file_number_size+t.active_duns_number_size+t.foreign_corp_key_size+t.company_phone_size+t.active_domestic_corp_key_size+t.cnp_name_size+t.company_fein_size+t.company_address_size+t.cnp_btype_size+t.iscorp_size;
END;

...
EXPORT SALT27.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'alpha','number','upper');
EXPORT FieldTypeNum(SALT27.StrType fn) := CASE(fn,'alpha' => 1,'number' => 2,'upper' => 3,0);
...
EXPORT SALT27.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'cnp_number','prim_range','prim_name','st','active_enterprise_number','hist_enterprise_number','hist_duns_number','hist_domestic_corp_key','unk_corp_key','ebr_file_number','active_duns_number','foreign_corp_key','company_phone','active_domestic_corp_key','cnp_name','company_fein','company_address','company_addr1','zip','company_csz','sec_range','v_city_name','cnp_btype','iscorp','company_name','cnp_hasnumber','cnp_lowv','cnp_translated','cnp_classid','company_foreign_domestic','company_bdid','company_address_type_raw','company_address_type_derived','dt_first_seen','dt_last_seen');
EXPORT FieldNum(SALT27.StrType fn) := CASE(fn,'cnp_number' => 1,'prim_range' => 2,'prim_name' => 3,'st' => 4,'active_enterprise_number' => 5,'hist_enterprise_number' => 6,'hist_duns_number' => 7,'hist_domestic_corp_key' => 8,'unk_corp_key' => 9,'ebr_file_number' => 10,'active_duns_number' => 11,'foreign_corp_key' => 12,'company_phone' => 13,'active_domestic_corp_key' => 14,'cnp_name' => 15,'company_fein' => 16,'company_address' => 17,'company_addr1' => 18,'zip' => 19,'company_csz' => 20,'sec_range' => 21,'v_city_name' => 22,'cnp_btype' => 23,'iscorp' => 24,'company_name' => 25,'cnp_hasnumber' => 26,'cnp_lowv' => 27,'cnp_translated' => 28,'cnp_classid' => 29,'company_foreign_domestic' => 30,'company_bdid' => 31,'company_address_type_raw' => 32,'company_address_type_derived' => 33,'dt_first_seen' => 34,'dt_last_seen' => 35,0);
...
EXPORT Make_cnp_name(SALT27.StrType s0) := s0;
EXPORT InValid_cnp_name(SALT27.StrType s) := FALSE;
...
EXPORT Make_company_address_type_raw(SALT27.StrType s0) := s0;
EXPORT InValid_company_address_type_raw(SALT27.StrType s) := FALSE;
EXPORT InValidMessage_company_address_type_raw(UNSIGNED1 wh) := '';
 
EXPORT Make_company_address_type_derived(SALT27.StrType s0) := s0;
EXPORT InValid_company_address_type_derived(SALT27.StrType s) := FALSE;
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT27,BIPV2_ProxID_dev2;
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_cnp_number,1,0)+ IF( SELF.Diff_prim_range,1,0)+ IF( SELF.Diff_prim_name,1,0)+ IF( SELF.Diff_st,1,0)+ IF( SELF.Diff_active_enterprise_number,1,0)+ IF( SELF.Diff_hist_enterprise_number,1,0)+ IF( SELF.Diff_hist_duns_number,1,0)+ IF( SELF.Diff_hist_domestic_corp_key,1,0)+ IF( SELF.Diff_unk_corp_key,1,0)+ IF( SELF.Diff_ebr_file_number,1,0)+ IF( SELF.Diff_active_duns_number,1,0)+ IF( SELF.Diff_foreign_corp_key,1,0)+ IF( SELF.Diff_company_phone,1,0)+ IF( SELF.Diff_active_domestic_corp_key,1,0)+ IF( SELF.Diff_cnp_name,1,0)+ IF( SELF.Diff_company_fein,1,0)+ IF( SELF.Diff_zip,1,0)+ IF( SELF.Diff_sec_range,1,0)+ IF( SELF.Diff_v_city_name,1,0)+ IF( SELF.Diff_cnp_btype,1,0)+ IF( SELF.Diff_iscorp,1,0)+ IF( SELF.Diff_company_name,1,0)+ IF( SELF.Diff_cnp_hasnumber,1,0)+ IF( SELF.Diff_cnp_lowv,1,0)+ IF( SELF.Diff_cnp_translated,1,0)+ IF( SELF.Diff_cnp_classid,1,0)+ IF( SELF.Diff_company_foreign_domestic,1,0)+ IF( SELF.Diff_company_bdid,1,0)+ IF( SELF.Diff_company_address_type_raw,1,0)+ IF( SELF.Diff_company_address_type_derived,1,0);
  END;

...
IMPORT SALT25,BIPV2_ProxID_dev2;
STRING20 Proxidstr := ''  : STORED('Proxid');
SALT25.UIDType uid := (SALT25.UIDType)Proxidstr;
TheData := PROJECT(BIPV2_ProxID_dev2.Keys(BIPV2_ProxID_dev2.In_DOT_Base).InData(Proxid=uid),BIPV2_ProxID_dev2.Layout_DOT_Base);
s := GLOBAL(PROJECT(BIPV2_ProxID_dev2.Keys(TheData).Specificities_Key,BIPV2_ProxID_dev2.Layout_Specificities.R)[1]);
CM := BIPV2_ProxID_dev2.Best(TheData,s,TRUE);

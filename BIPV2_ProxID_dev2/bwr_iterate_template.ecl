...
lih       := BIPV2_ProxID_dev2.files().base.built;
...
#workunit('name','BIPV2_ProxID_dev2.BWR_Iterate ' + iteration + '- Internal Linking - SALT V2.5 Beta 1');

h         := BIPV2_ProxID_dev2.Hygiene(BIPV2_ProxID_dev2.Files().base.qa);
p         := h.AllProfiles; // Detailed profile of every field
combo     := if(iteration = '1' ,(string)((unsigned)iteration - 1)  ,pversion + '_' + (string)((unsigned)iteration - 1));

sequential(
	 output(BIPV2_ProxID_dev2.In_DOT_Base	,named('InputFile'))
  ,nothor(Tools.fun_ClearfilesFromSupers(project(names, transform(Tools.Layout_Names,self.name := left.logicalname)), false)) //clear supers if needed
	,BIPV2_ProxID_dev2.Promote(combo).new2built
	,BIPV2_ProxID_dev2.Proc_Iterate(iteration,pversion,lih).DoAll // Change '1' for subsequent iterations
  ...
  ,BIPV2_ProxID_dev2.output_test_cases(BIPV2_ProxID_dev2.Files().base.qa)
);
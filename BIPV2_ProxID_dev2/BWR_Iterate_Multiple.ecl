...
hacked matches and keys attributes to factor it in support for company_address

added back attribute file srcridvlid, 
*/
import wk_ut,tools,bipv2,mdr,BIPV2_Proxid_dev2;
...
#workunit('name','BIPV2_Proxid_dev2 Specificites + kick off iterations ' + (string)startiter + '-' + (string)(startiter + numiters - 1));
#workunit('priority','10');
#workunit('protect' ,true);

ecltext := 'import BIPV2,BIPV2_Build,BIPV2_Proxid_dev2,tools,mdr;iteration := \'@iteration@\';pversion  := \'@version@\';lih       := if((unsigned)iteration = 1   ,BIPV2_Proxid_dev2.In_DOT_Base    ,BIPV2_Proxid_dev2.files().base.built )(not mdr.sourceTools.sourceisZoom(source));#workunit(\'name\',\'BIPV2_Proxid_dev2 \' + pversion + \' iter \' + iteration);#workunit(\'priority\',\'10\');#workunit(\'protect\' ,true);BIPV2_Proxid_dev2.Proc_Iterate(iteration,pversion,lih).doall;';
...
//#workunit('name','BIPV2_Proxid_dev2.BWR_Specificities - Specificities');
IMPORT BIPV2_Proxid_dev2,SALT25;
IMPORT SALTTOOLS22;

dotbase := BIPV2_Proxid_dev2.files().base.built;

SpecMod := BIPV2_Proxid_dev2.specificities(dotbase);
...
sequential(
 ...
 // ,BIPV2_Proxid_dev2.Promote('0').new2built    //promote it as zeroth iteration to superfile(for attribute file use)
  ,SpecMod.Build
  ,parallel(
     OUTPUT(SpecMod.Specificities ,NAMED('Specificities'))
    ,OUTPUT(SpecMod.SpcShift      ,NAMED('SpcShift'     ))
    ,SALTTOOLS22.mac_Patch_SPC(SpecMod.Specificities,'BIPV2_Proxid_dev2._SPC',dotbase,,false)
//    ,SALTTOOLS22.mac_Patch_SPC(SpecMod.Specificities,'BIPV2_Proxid_dev2._SPC',dotbase,,false,false)
  )
  ,kickiters
);

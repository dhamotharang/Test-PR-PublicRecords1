...
IMPORT Gateway, Phone_Shell, Relocations, Risk_Indicators, Doxie, STD;
...
EXPORT Phone_Shell_Service() := FUNCTION
 ...
 // Boca Shell Options, Phone Shell version uses 2-digit notation so 10 value = phone shell 1.0, 21 = v2.1, 30 = v3.0
 UNSIGNED1 PhoneShell_Version         := 30 : STORED('PhoneShell_Version');
 ...
 input := DEDUP(SORT(realtime + batch, AcctNo, FullName, FirstName, MiddleName, LastName, TitleName, SuffixName, 
                                       StreetAddress1, StreetAddress2, City, State, Zip, 
                                       Prim_Range, Predir, Prim_Name, Addr_Suffix, Postdir, Unit_Desig, Sec_Range, Zip5, Zip4, 
                                       SSN, DateOfBirth, Age, HomePhone, WorkPhone, 
                                       TransUnionGatewayEnabled, InsuranceGatewayEnabled),
                AcctNo, FullName, FirstName, MiddleName, LastName, TitleName, SuffixName, StreetAddress1, StreetAddress2, City, State, Zip, 
                Prim_Range, Predir, Prim_Name, Addr_Suffix, Postdir, Unit_Desig, Sec_Range, Zip5, Zip4, SSN, DateOfBirth, Age, HomePhone, WorkPhone, 
                TransUnionGatewayEnabled, InsuranceGatewayEnabled);
 ...
 // If batch has 1 using gateway and one not at the batch level they will all go against the gateway.
 // As we don't have a way to call the gateway at the record level. It's at the dataset level.

 model_results := MAP(Phone_Score_Model NOT IN ['PHONESCORE_V2','COLLECTIONSCORE_V3','COMMON_SCORE'] => results,	
                      // If not one of the above, no models are being called, just return the shell and attributes (phone_model_score will be blank)
                      ModelVersion IN ['V2'] AND allowPremiumSource_A                 => Phone_Shell.PhoneScore_cp3_v2(results, Score_Threshold), // default score threshold back then was 306 
                      ModelVersion IN ['V2'] AND NOT allowPremiumSource_A             => Phone_Shell.PhoneScore_wf8_v2(results, Score_Threshold), // default score threshold back then was 306
                      // ModelVersion = V2 above calls the old v2 split models for Phone Shell v1.0. This is the ONLY way to call the old v2 models.
                      ModelVersion IN ['V3'] AND allowPremiumSource_A                 => Phone_Shell.PhoneScore_cp3_v3(results, Score_Threshold), // default score threshold back then was 217
                      ModelVersion IN ['V3'] AND NOT allowPremiumSource_A             => Phone_Shell.PhoneScore_wf8_v3(results, Score_Threshold), // default score threshold back then was 217
 ...
 // model_results := Phone_Shell.PhoneScore_cp3_v3(results, Score_Threshold); // default score_threshold was 217
 ...
 // STD.Str.Find(Phone_Shell.Sources.Source_List, 'EQP', 1) > 0;

...
IMPORT Phone_Shell, Risk_Indicators, RiskWise, UT, Header, STD;

EXPORT Phone_Shell.Layout_Phone_Shell.Layout_Phone_Shell_Plus Search_RelativeCloseProximity (DATASET(Phone_Shell.Layout_Phone_Shell.Layout_Phone_Shell_Plus) input, DATASET(Phone_Shell.Layouts.Layout_Parent_Spouse_Relative_RawData) phones, UNSIGNED1 PhoneRestrictionMask,
 ...
 sortedCloseRelatives := SORT(closestRelativePhones, Clean_Input.seq, Gathered_Phone, Sources.Source_List, -Sources.Source_List_Last_Seen, -Sources.Source_List_First_Seen, -LENGTH(TRIM(Raw_Phone_Characteristics.Phone_Match_Code)));

	final := if(IncludeLexIDCounts,
             ROLLUP(sortedCloseRelatives,
               ...
               get_Alls(left,right)),
             DEDUP(sortedCloseRelatives, Clean_Input.seq, Gathered_Phone, Sources.Source_List)
            );

...
IMPORT Doxie, Gong, NID, Phone_Shell, Progressive_Phone, RiskWise, UT, Watchdog, AutoStandardI, STD, Suppress;
...
EXPORT Phone_Shell.Layout_Phone_Shell.Layout_Phone_Shell_Plus Search_Extended_Skip_Trace (DATASET(Phone_Shell.Layout_Phone_Shell.Layout_Phone_Shell_Plus) input, UNSIGNED1 PhoneRestrictionMask, STRING50 DataRestrictionMask, UNSIGNED2 sx_match_restriction_limit = 10, BOOLEAN Strict_APSX = FALSE,
 ...
 dppa_ok := mod_access.isValidDPPA();
	...
	{b_out_plus, unsigned4 global_sid} by_rule_1(sixMonths le, Gong.Key_history_Address ri) := TRANSFORM
		...
		SELF := [];
	END;
	f_rule_1_raw_unsuppressed := JOIN(sixMonths, Gong.Key_history_Address, KEYED(LEFT.prim_name = RIGHT.prim_name) AND
	...
	{b_out_plus, unsigned4 global_sid} by_rule_2(sixMonths le, Gong.key_history_city_st_name ri) := TRANSFORM
		SELF.global_sid := ri.global_sid;
	...
	f_rule_2_input_unsuppressed := JOIN(IF(sx_match_restriction_limit	=	0, prepInput, f_best_city_st_raw_input), Gong.key_history_city_st_name, 
	...
	f_rule_2_raw_unsuppressed := JOIN(IF(sx_match_restriction_limit	=	0, sixMonths, f_best_city_st_raw), Gong.key_history_city_st_name, 
																				KEYED(RIGHT.city_code IN Doxie.Make_CityCodes(LEFT.city_name).rox) AND             
																				KEYED(LEFT.st = RIGHT.st) AND       
																				KEYED(metaphonelib.DMetaPhone1(LEFT.lname)[1..6] = RIGHT.dph_name_last) AND
																				KEYED(LEFT.lname <> '' AND LEFT.lname = RIGHT.name_last) AND
	...
	{b_out_plus, unsigned4 global_sid} by_rule_3(sixMonths le, Gong.key_history_city_st_name ri) := TRANSFORM
		SELF.global_sid := ri.global_sid;
		SELF.AcctNo := (STRING20)le.seq;
		SELF.Subj_First := ri.name_first;
		SELF.Subj_Middle := ri.name_middle;
	...
	f_rule_3_input_unsuppressed := JOIN(IF(sx_match_restriction_limit = 0, prepInput, f_best_city_st_raw_input), Gong.key_history_city_st_name,
																				KEYED(RIGHT.city_code IN doxie.Make_CityCodes(LEFT.city_name).rox) AND
	...
	f_rule_3_raw_unsuppressed := JOIN(IF(sx_match_restriction_limit = 0, sixMonths, f_best_city_st_raw), Gong.key_history_city_st_name,
																				KEYED(RIGHT.city_code IN doxie.Make_CityCodes(LEFT.city_name).rox) AND
																				(LEFT.st = RIGHT.st) AND
																				(metaphonelib.DMetaPhone1(LEFT.lname)[1..6] = RIGHT.dph_name_last) AND
																				(LEFT.lname <> '' AND LEFT.lname = RIGHT.name_last) AND
	...
	skiptrace_all_unsuppressed := IF(strict_apsx, JOIN(skiptrace_all_precheck, Gong.Key_History_phone,

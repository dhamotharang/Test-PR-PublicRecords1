/*
	This modeling shell is for use by the Modeling team in Minneapolis.  SAS requires that all field names be less than 32 characters
	long, and not have any '.' (DOT) notation in the name.  Because of this we must project the normal Phone Shell into this
	modeling layout which SAS can read in.  Phone_Shell.To_Modeling_Shell is used to convert the Phone Shell to this layout.
*/
...
Layout_Subject_Level_Modeling := RECORD
	...
	UNSIGNED1 Exp_Num_Duplicate 					:= 0;
	UNSIGNED1 Exp_Num_Insufficient_Score	:= 0;
END;
...
Layout_Experian_File_One_ver_Modeling := RECORD
	BOOLEAN Exp_Verified := FALSE;
	STRING1 Exp_Type := '';
	STRING1 Exp_Source := '';
	STRING8 Exp_Last_Update := '';
	STRING3 Exp_ph_Score_V1 := '';
END;
...
EXPORT Layout_Modeling_Shell := RECORD
	...
	Layout_Experian_File_One_ver_Modeling;
	Layout_EDA_Characteristics_Modeling;
	Risk_Indicators.Layout_Boca_Shell_Edina_v41; // Boca Shell v4.1 Fields

...
EXPORT Phone_Shell.Layout_Phone_Shell.Phone_Shell_Layout Phone_Shell_Function (DATASET(Phone_Shell.Layout_Phone_Shell.Input) Input,
  ...
  *  Note that as of Phone Shell v3.0 Boca Shell is no longer run.         *
	 ************************************************************************ */
	nugen      := TRUE;
	from_IT1O  := FALSE;
 ...
 runDLVerification := if(PhoneShellVersion >= 20,
	 ...
	 ************************************************************************ */
	InstantID := Risk_Indicators.InstantID_Function(iid_prep, BocaShellGateways, DPPAPurpose, GLBPurpose, isUtility, 
                      ln_branded, ofac_only, suppressNearDups, require2ele, isFCRA, from_biid, 
																						ExcludeWatchLists, from_IT1O, ofac_version, include_ofac, addtl_watchlists, 
                      watchlist_threshold, dob_radius, BocaShellVersion, 
 ...
 // the 'iff' makes sure that consumer shell is not still run in the background when running Phone Shell 3.0+
	 ...
	 ************************************************************************ */
	withPhones := Phone_Shell.Gather_Phones(withBocaShell, Gateways, GLBPurpose, DPPAPurpose, 
                     DataRestrictionMask, DataPermissionMask, PhoneRestrictionMask, MaxPhones, 
                     InsuranceVerificationAgeLimit, SPIIAccessLevel, VerticalMarket, IndustryClass, 
                     RelocationsMaxDaysBefore, RelocationsMaxDaysAfter, RelocationsTargetRadius, 
                     IncludeLastResort, IncludePhonesFeedback, TestAccount, Batch, SX_Match_Restriction_Limit, 
                     Strict_APSX, BlankOutDuplicatePhones, 
																					UsePremiumSource_A, RunRelocation, PhoneShellVersion, mod_access);

	/* ************************************************************************
	 *  Gather attributes for the discovered phones                           *
	 ************************************************************************ */
	withAttributes := Phone_Shell.Gather_Attributes(withPhones, GLBPurpose, DPPAPurpose, DataRestrictionMask, 
                     InsuranceVerificationAgeLimit, IndustryClass, PhoneShellVersion, mod_access);

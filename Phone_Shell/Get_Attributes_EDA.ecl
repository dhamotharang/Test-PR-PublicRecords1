...
IMPORT dx_Gong, Phone_Shell, RiskWise, UT, STD, doxie, Suppress;
...
EXPORT Phone_Shell.Layout_Phone_Shell.Layout_Phone_Shell_Plus Get_Attributes_EDA (DATASET(Phone_Shell.Layout_Phone_Shell.Layout_Phone_Shell_Plus) input,
  ...
  {layoutEDA, unsigned4 global_sid} getEDAAttributes(Phone_Shell.Layout_Phone_Shell.Layout_Phone_Shell_Plus le, key_history_phone ri) := TRANSFORM
  ...
  edaAttributes_unsuppressed := JOIN(Input, key_history_phone, TRIM(LEFT.Gathered_Phone) NOT IN ['', '0'] AND KEYED(LEFT.Gathered_Phone[1..3] = RIGHT.p3 AND LEFT.Gathered_Phone[4..10] = RIGHT.p7) AND RIGHT.current_flag = TRUE,
   ...
   ************************************************************************ */
  {layoutEDA, unsigned4 global_sid} getEDAAttributesHist(Phone_Shell.Layout_Phone_Shell.Layout_Phone_Shell_Plus le, key_history_phone ri) := TRANSFORM
    SELF.global_sid := ri.global_sid;
    SELF.dt_first_seen := ri.dt_first_seen;
    SELF.dt_last_seen := ri.dt_last_seen;
    SELF.Prim_Range := ri.prim_range;
  ...
  edaAttributesHist_unsuppressed := JOIN(Input, key_history_phone, TRIM(LEFT.Gathered_Phone) NOT IN ['', '0'] AND KEYED(LEFT.Gathered_Phone[4..10] = RIGHT.p7 AND LEFT.Gathered_Phone[1..3] = RIGHT.p3) AND RIGHT.current_flag = FALSE,

...
EXPORT  AssetReport (
 ...
 // DID should be atmost one (do we keep layout_references for legacy reasons?)
  ...
  ds_flags := if (IsFCRA, FCRA.GetFlagFile (ds_best));
	...
	// 2) Call the person context	and filter out consumer statements if they were not requested	
	pc_recs := if(isFCRA, FFD.FetchPersonContext(ds_dids, Gateway.Configuration.Get(),
																							FFD.Constants.DataGroupSet.AssetReport)(ShowConsumerStatements OR (RecordType = FFD.Constants.RecordType.DR)));

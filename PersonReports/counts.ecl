...
export counts (
  ...
  PersonReports.input.count_param in_params = module (PersonReports.input.count_param) end,
  ...
  export dl := module
    shared data_cnt := if (~IsFCRA, doxie.Fn_dl_count (did, 0, in_params.DPPApurpose, in_params.GLBpurpose, in_params.ln_branded), 0);
    export unsigned GetCount := data_cnt;
    export boolean exists := data_cnt > 0;
  end;

  export property := module
    data_cnt := doxie.Fn_comp_prop_count (did, 0, in_params.DPPApurpose, in_params.GLBpurpose, in_params.ln_branded);
    export unsigned GetCount := data_cnt;
//    export boolean exists := data_cnt > 0;

    // unfortunately, I have to do this extremely heavy calculations to ensure consistency
		prop_recs := PersonReports.property_records(dids, module (project (in_params, input.property)) end, IsFCRA, flagfile);

...
EXPORT bankruptcy_records (
  ...
  PersonReports.input.bankruptcy in_params = module (PersonReports.input.bankruptcy) end,
...
// V1 ----------------------------------------------------------------------------------------
  bk := Doxie_Raw.bk_legacy_raw (dids, , , , in_params.ssn_mask, in_params.bk_party_type);
...
// V2 ----------------------------------------------------------------------------------------
  ...
  bankruptcies := BankruptcyV2_Services.bankruptcy_raw(IsFCRA).source_view (dids,in_ssn_mask := in_params.ssn_mask,in_party_type := in_params.bk_party_type); 
...
// V3 ----------------------------------------------------------------------------------------
  // flagfile is ignored for this call; it is recalculated further down in the core code
  bk_v3 := doxie.bk_records_v3 (dids, in_params.ssn_mask, in_params.bk_party_type, IsFCRA, flagfile, nonSS, slim_pc_recs, in_params.FFDOptionsMask, in_params.bk_suppress_withdrawn).format_v3;

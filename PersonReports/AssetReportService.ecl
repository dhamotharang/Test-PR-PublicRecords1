...
<message name="AssetReportService" wuTimeout="300000">
  ...
  <part name="ExactSecondRangeMatch" type="xsd:boolean" default="false"/>
  <separator />
  ...
  <separator />
  <part name="AssetReportRequest" type="tns:XmlDataSet" cols="80" rows="30" />
</message>
*/
/*--INFO-- Searches for personal assets (property, vehicles, etc.)<p/><p/>*/
/*--USES-- ut.input_xslt */
...
#constant('IncludeNonRegulatedWatercraftSources', true);
  ...
  GetInputOptions (iesp.assetreport.t_AssetReportOption tag) := function
    options := module
  ...
  rec_in := iesp.assetreport.t_AssetReportRequest;
  ds_in := DATASET ([], rec_in) : STORED ('AssetReportRequest', FEW);
  ...
  options_stored := module (SR.relatives_options, SR.imposters_options, SR.global_options)
...
//  output (recs);
  ...
  iesp.assetreport.t_AssetReportResponse SetResponse (PersonReports.layouts.CommonAssetReportIndividual L) := transform

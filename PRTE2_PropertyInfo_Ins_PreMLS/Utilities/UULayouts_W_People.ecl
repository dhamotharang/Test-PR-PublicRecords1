EXPORT Layouts_W_People := MODULE

// Jan 2016 - once we can run PRTE2_PropertyInfo_Ins.U_Convert_V2_Expanded, then remove the following layout and instead use
// the 	AlphaPropertyCSVRec_V2 below.  BUT RENAME it to the name without the _V2 so all the code works without modification.
// ALSO - change Fn_Spray_Alpharetta_Spreadsheet - to begin auto-cleaning addresses during the spray.
	EXPORT AlphaPropertyCSVRec := z_AlphaPropertyCSVRec;

	EXPORT PersonSection := RECORD
			unsigned6 did;
			STRING fname;
			STRING mname;
			STRING lname;
			...
			integer4 dob;
	END;
// 6/22/16 ... altered to be the exact same order as AlphaPropertyCSVRec above except the first fields up thru property_city_state_zip
// not in use until I convert everything over and add people info.  (maybe use Anna's spreadsheet to grab right person per her list)
	EXPORT AlphaPropertyCSVRec_V2 := RECORD
		unsigned6		property_rid;
		string1			vendor_source;
		PersonSection reference_personInfo;			// nothing in there is used for Property Info, just a help for data team

...
EXPORT SALT34.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_charter_no','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_desc');
EXPORT FieldTypeNum(SALT34.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_charter_no' => 2,'invalid_corp_vendor' => 3,'invalid_state_origin' => 4,'invalid_mandatory' => 5,'invalid_date' => 6,'invalid_desc' => 7,0);
...
EXPORT MakeFT_invalid_charter_no(SALT34.StrType s0) := FUNCTION
  s1 := SALT34.stringfilter(s0,'0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_charter_no(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,'0123456789'))),~(LENGTH(TRIM(s)) >= 1));
EXPORT InValidMessageFT_invalid_charter_no(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars('0123456789'),SALT34.HygieneErrors.NotLength('1..'),SALT34.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_desc(SALT34.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_invalid_desc(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,' -()0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'))));
EXPORT InValidMessageFT_invalid_desc(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars(' -()0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'),SALT34.HygieneErrors.Good);
...
EXPORT Make_corp_sos_charter_nbr(SALT34.StrType s0) := MakeFT_invalid_charter_no(s0);
EXPORT InValid_corp_sos_charter_nbr(SALT34.StrType s) := InValidFT_invalid_charter_no(s);
EXPORT InValidMessage_corp_sos_charter_nbr(UNSIGNED1 wh) := InValidMessageFT_invalid_charter_no(wh);
...
EXPORT Make_event_desc(SALT34.StrType s0) := MakeFT_invalid_desc(s0);
EXPORT InValid_event_desc(SALT34.StrType s) := InValidFT_invalid_desc(s);
EXPORT InValidMessage_event_desc(UNSIGNED1 wh) := InValidMessageFT_invalid_desc(wh);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT34,Scrubs_Corp2_Mapping_IL_Event;

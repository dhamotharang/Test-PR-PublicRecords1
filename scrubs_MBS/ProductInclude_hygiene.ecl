...
EXPORT Summary(SALT37.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_fdn_file_product_include_id := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.fdn_file_product_include_id)));
    avelength_fdn_file_product_include_id := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.fdn_file_product_include_id)),h.fdn_file_product_include_id<>(typeof(h.fdn_file_product_include_id))'');
    ...
    maxlength_product_id := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.product_id)));
    avelength_product_id := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.product_id)),h.product_id<>(typeof(h.product_id))'');
...
SALT37.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,IF (le.fdn_file_product_include_id <> 0,TRIM((SALT37.StrType)le.fdn_file_product_include_id), ''),IF (le.fdn_file_info_id <> 0,TRIM((SALT37.StrType)le.fdn_file_info_id), ''),IF (le.product_id <> 0,TRIM((SALT37.StrType)le.product_id), ''),IF (le.status <> 0,TRIM((SALT37.StrType)le.status), ''),TRIM((SALT37.StrType)le.date_added),TRIM((SALT37.StrType)le.user_added),TRIM((SALT37.StrType)le.date_changed),TRIM((SALT37.StrType)le.user_changed)));
  SELF.FldNo := C;
END;
...
SALT37.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 8);
  SELF.FldNo2 := 1 + (C % 8);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,IF (le.fdn_file_product_include_id <> 0,TRIM((SALT37.StrType)le.fdn_file_product_include_id), ''),IF (le.fdn_file_info_id <> 0,TRIM((SALT37.StrType)le.fdn_file_info_id), ''),IF (le.product_id <> 0,TRIM((SALT37.StrType)le.product_id), ''),IF (le.status <> 0,TRIM((SALT37.StrType)le.status), ''),TRIM((SALT37.StrType)le.date_added),TRIM((SALT37.StrType)le.user_added),TRIM((SALT37.StrType)le.date_changed),TRIM((SALT37.StrType)le.user_changed)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,IF (le.fdn_file_product_include_id <> 0,TRIM((SALT37.StrType)le.fdn_file_product_include_id), ''),IF (le.fdn_file_info_id <> 0,TRIM((SALT37.StrType)le.fdn_file_info_id), ''),IF (le.product_id <> 0,TRIM((SALT37.StrType)le.product_id), ''),IF (le.status <> 0,TRIM((SALT37.StrType)le.status), ''),TRIM((SALT37.StrType)le.date_added),TRIM((SALT37.StrType)le.user_added),TRIM((SALT37.StrType)le.date_changed),TRIM((SALT37.StrType)le.user_changed)));
  END;
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ProductInclude_Fields.InValid_fdn_file_product_include_id((SALT37.StrType)le.fdn_file_product_include_id),
    ProductInclude_Fields.InValid_fdn_file_info_id((SALT37.StrType)le.fdn_file_info_id),
    ProductInclude_Fields.InValid_product_id((SALT37.StrType)le.product_id),
    ProductInclude_Fields.InValid_status((SALT37.StrType)le.status),
    ProductInclude_Fields.InValid_date_added((SALT37.StrType)le.date_added),
    ProductInclude_Fields.InValid_user_added((SALT37.StrType)le.user_added),
    ProductInclude_Fields.InValid_date_changed((SALT37.StrType)le.date_changed),
    ProductInclude_Fields.InValid_user_changed((SALT37.StrType)le.user_changed),
    0);

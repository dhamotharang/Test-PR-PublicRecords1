...
EXPORT SALT37.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'cc_id','gc_id','account_id');
EXPORT FieldNum(SALT37.StrType fn) := CASE(fn,'cc_id' => 0,'gc_id' => 1,'account_id' => 2,0);
 
//Individual field level validation
 
EXPORT Make_cc_id(SALT37.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_cc_id(SALT37.StrType s) := InValidFT_invalid_numeric(s);
...
EXPORT Make_account_id(SALT37.StrType s0) := MakeFT_invalid_alphanumeric(s0);
EXPORT InValid_account_id(SALT37.StrType s) := InValidFT_invalid_alphanumeric(s);

...
EXPORT Summary(SALT37.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_table_name := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.table_name)));
    avelength_table_name := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.table_name)),h.table_name<>(typeof(h.table_name))'');
    populated_column_name_pcnt := AVE(GROUP,IF(h.column_name = (TYPEOF(h.column_name))'',0,100));
    maxlength_column_name := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.column_name)));
    avelength_column_name := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.column_name)),h.column_name<>(typeof(h.column_name))'');
    populated_is_column_value_pcnt := AVE(GROUP,IF(h.is_column_value = (TYPEOF(h.is_column_value))'',0,100));
    maxlength_is_column_value := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.is_column_value)));
    avelength_is_column_value := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.is_column_value)),h.is_column_value<>(typeof(h.is_column_value))'');
  END;
...
SALT37.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,IF (le.table_column_id <> 0,TRIM((SALT37.StrType)le.table_column_id), ''),TRIM((SALT37.StrType)le.table_name),TRIM((SALT37.StrType)le.column_name),IF (le.is_column_value <> 0,TRIM((SALT37.StrType)le.is_column_value), '')));
  SELF.FldNo := C;
END;
...
SALT37.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 4);
  SELF.FldNo2 := 1 + (C % 4);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,IF (le.table_column_id <> 0,TRIM((SALT37.StrType)le.table_column_id), ''),TRIM((SALT37.StrType)le.table_name),TRIM((SALT37.StrType)le.column_name),IF (le.is_column_value <> 0,TRIM((SALT37.StrType)le.is_column_value), '')));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,IF (le.table_column_id <> 0,TRIM((SALT37.StrType)le.table_column_id), ''),TRIM((SALT37.StrType)le.table_name),TRIM((SALT37.StrType)le.column_name),IF (le.is_column_value <> 0,TRIM((SALT37.StrType)le.is_column_value), '')));
  END;
...
SHARED FldIds := DATASET([{1,'table_column_id'}
      ,{2,'table_name'}
      ,{3,'column_name'}
      ,{4,'is_column_value'}],SALT37.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    TableCol_Fields.InValid_table_column_id((SALT37.StrType)le.table_column_id),
    TableCol_Fields.InValid_table_name((SALT37.StrType)le.table_name),
    TableCol_Fields.InValid_column_name((SALT37.StrType)le.column_name),
    TableCol_Fields.InValid_is_column_value((SALT37.StrType)le.is_column_value),
    0);

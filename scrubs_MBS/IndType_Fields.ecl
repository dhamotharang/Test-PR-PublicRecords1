...
EXPORT SALT37.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'ind_type','description','status','date_added','user_added','date_changed','user_changed');
EXPORT FieldNum(SALT37.StrType fn) := CASE(fn,'ind_type' => 0,'description' => 1,'status' => 2,'date_added' => 3,'user_added' => 4,'date_changed' => 5,'user_changed' => 6,0);
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_ind_type,1,0)+ IF( SELF.Diff_description,1,0)+ IF( SELF.Diff_status,1,0)+ IF( SELF.Diff_date_added,1,0)+ IF( SELF.Diff_user_added,1,0)+ IF( SELF.Diff_date_changed,1,0)+ IF( SELF.Diff_user_changed,1,0);
  END;

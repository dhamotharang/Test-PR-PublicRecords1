IMPORT SALT39;
EXPORT NewGcIdExcl_Fields := MODULE
...
EXPORT SALT39.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'fdn_file_gc_exclusion_id','fdn_file_info_id','exclusion_id','exclusion_id_type','status','date_added','user_added','date_changed','user_changed');
EXPORT SALT39.StrType FlatName(UNSIGNED2 i) := CHOOSE(i,'fdn_file_gc_exclusion_id','fdn_file_info_id','exclusion_id','exclusion_id_type','status','date_added','user_added','date_changed','user_changed');
EXPORT FieldNum(SALT39.StrType fn) := CASE(fn,'fdn_file_gc_exclusion_id' => 0,'fdn_file_info_id' => 1,'exclusion_id' => 2,'exclusion_id_type' => 3,'status' => 4,'date_added' => 5,'user_added' => 6,'date_changed' => 7,'user_changed' => 8,0);
...
EXPORT Make_fdn_file_gc_exclusion_id(SALT39.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_fdn_file_gc_exclusion_id(SALT39.StrType s) := InValidFT_invalid_numeric(s);
EXPORT InValidMessage_fdn_file_gc_exclusion_id(UNSIGNED1 wh) := InValidMessageFT_invalid_numeric(wh);
...
EXPORT Make_exclusion_id(SALT39.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_exclusion_id(SALT39.StrType s) := InValidFT_invalid_numeric(s);
EXPORT InValidMessage_exclusion_id(UNSIGNED1 wh) := InValidMessageFT_invalid_numeric(wh);
 
EXPORT Make_exclusion_id_type(SALT39.StrType s0) := MakeFT_invalid_alphanumeric(s0);
EXPORT InValid_exclusion_id_type(SALT39.StrType s) := InValidFT_invalid_alphanumeric(s);
EXPORT InValidMessage_exclusion_id_type(UNSIGNED1 wh) := InValidMessageFT_invalid_alphanumeric(wh);
...
#uniquename(dl)
  %dl% := RECORD
    BOOLEAN Diff_fdn_file_gc_exclusion_id;
    BOOLEAN Diff_fdn_file_info_id;
    BOOLEAN Diff_exclusion_id;
    BOOLEAN Diff_exclusion_id_type;
    BOOLEAN Diff_status;
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    SELF.Diff_fdn_file_gc_exclusion_id := le.fdn_file_gc_exclusion_id <> ri.fdn_file_gc_exclusion_id;
    SELF.Diff_fdn_file_info_id := le.fdn_file_info_id <> ri.fdn_file_info_id;
    SELF.Diff_exclusion_id := le.exclusion_id <> ri.exclusion_id;
    SELF.Diff_exclusion_id_type := le.exclusion_id_type <> ri.exclusion_id_type;
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_fdn_file_gc_exclusion_id,1,0)+ IF( SELF.Diff_fdn_file_info_id,1,0)+ IF( SELF.Diff_exclusion_id,1,0)+ IF( SELF.Diff_exclusion_id_type,1,0)+ IF( SELF.Diff_status,1,0)+ IF( SELF.Diff_date_added,1,0)+ IF( SELF.Diff_user_added,1,0)+ IF( SELF.Diff_date_changed,1,0)+ IF( SELF.Diff_user_changed,1,0);
  END;
...
#uniquename(AggRec)
  %AggRec% := RECORD
    Count_Diff_fdn_file_gc_exclusion_id := COUNT(GROUP,%Closest%.Diff_fdn_file_gc_exclusion_id);
    Count_Diff_fdn_file_info_id := COUNT(GROUP,%Closest%.Diff_fdn_file_info_id);
    Count_Diff_exclusion_id := COUNT(GROUP,%Closest%.Diff_exclusion_id);
    Count_Diff_exclusion_id_type := COUNT(GROUP,%Closest%.Diff_exclusion_id_type);

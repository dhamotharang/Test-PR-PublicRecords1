...
EXPORT SALT37.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'fdn_file_gc_exclusion_id','fdn_file_info_id','exclusion_id','exclusion_id_type','status','date_added','user_added','date_changed','user_changed');
EXPORT FieldNum(SALT37.StrType fn) := CASE(fn,'fdn_file_gc_exclusion_id' => 0,'fdn_file_info_id' => 1,'exclusion_id' => 2,'exclusion_id_type' => 3,'status' => 4,'date_added' => 5,'user_added' => 6,'date_changed' => 7,'user_changed' => 8,0);
 
//Individual field level validation
 
EXPORT Make_fdn_file_gc_exclusion_id(SALT37.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_fdn_file_gc_exclusion_id(SALT37.StrType s) := InValidFT_invalid_numeric(s);
...
EXPORT Make_exclusion_id(SALT37.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_exclusion_id(SALT37.StrType s) := InValidFT_invalid_numeric(s);
EXPORT InValidMessage_exclusion_id(UNSIGNED1 wh) := InValidMessageFT_invalid_numeric(wh);
 
EXPORT Make_exclusion_id_type(SALT37.StrType s0) := MakeFT_invalid_alphanumeric(s0);
EXPORT InValid_exclusion_id_type(SALT37.StrType s) := InValidFT_invalid_alphanumeric(s);

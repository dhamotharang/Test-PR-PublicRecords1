...
SALT39.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,IF (le.ind_type <> 0,TRIM((SALT39.StrType)le.ind_type), ''),TRIM((SALT39.StrType)le.description),IF (le.status <> 0,TRIM((SALT39.StrType)le.status), ''),TRIM((SALT39.StrType)le.date_added),TRIM((SALT39.StrType)le.user_added),TRIM((SALT39.StrType)le.date_changed),TRIM((SALT39.StrType)le.user_changed)));
  SELF.FldNo := C;
END;
...
SALT39.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 7);
  SELF.FldNo2 := 1 + (C % 7);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,IF (le.ind_type <> 0,TRIM((SALT39.StrType)le.ind_type), ''),TRIM((SALT39.StrType)le.description),IF (le.status <> 0,TRIM((SALT39.StrType)le.status), ''),TRIM((SALT39.StrType)le.date_added),TRIM((SALT39.StrType)le.user_added),TRIM((SALT39.StrType)le.date_changed),TRIM((SALT39.StrType)le.user_changed)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,IF (le.ind_type <> 0,TRIM((SALT39.StrType)le.ind_type), ''),TRIM((SALT39.StrType)le.description),IF (le.status <> 0,TRIM((SALT39.StrType)le.status), ''),TRIM((SALT39.StrType)le.date_added),TRIM((SALT39.StrType)le.user_added),TRIM((SALT39.StrType)le.date_changed),TRIM((SALT39.StrType)le.user_changed)));
  END;
...
SHARED FldIds := DATASET([{1,'ind_type'}
      ...
      ,{5,'user_added'}
      ,{6,'date_changed'}
      ,{7,'user_changed'}],SALT39.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    IndType_Fields.InValid_ind_type((SALT39.StrType)le.ind_type),
    IndType_Fields.InValid_description((SALT39.StrType)le.description),
    IndType_Fields.InValid_status((SALT39.StrType)le.status),
    IndType_Fields.InValid_date_added((SALT39.StrType)le.date_added),
    IndType_Fields.InValid_user_added((SALT39.StrType)le.user_added),
    IndType_Fields.InValid_date_changed((SALT39.StrType)le.date_changed),
    IndType_Fields.InValid_user_changed((SALT39.StrType)le.user_changed),
    0);
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT39.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_MBS, IndType_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

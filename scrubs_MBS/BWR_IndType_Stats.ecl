...
F := Scrubs_MBS.IndType_In_IndType; // Input file
S := Scrubs_MBS.IndType_Scrubs; // My scrubs module
N := S.FromNone(F); // Generate the error flags
B := N.BitmapInfile;
//OUTPUT(B,,'~thor_data400::scrubs::fraudgov::20170522::scrubs_IndType', overwrite, compressed);
...
// Scrubs.OrbitProfileStats('Scrubs_MBS_IndType','Scrubs',stats).ProfileTemplate;
//Generating a template to upload rules to Orbit
//Submits stats to Orbit
submitStats:=Scrubs.OrbitProfileStats('Scrubs_MBS_IndType','ScrubsAlerts',stats,versionDate,'MBS').SubmitStats;
submitStats;			

...
EXPORT FromNone(DATASET(NewGcIdExcl_Layout_NewGcIdExcl) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.fdn_file_gc_exclusion_id_Invalid := NewGcIdExcl_Fields.InValid_fdn_file_gc_exclusion_id((SALT39.StrType)le.fdn_file_gc_exclusion_id);
    SELF.fdn_file_info_id_Invalid := NewGcIdExcl_Fields.InValid_fdn_file_info_id((SALT39.StrType)le.fdn_file_info_id);
    SELF.exclusion_id_Invalid := NewGcIdExcl_Fields.InValid_exclusion_id((SALT39.StrType)le.exclusion_id);
    SELF.exclusion_id_type_Invalid := NewGcIdExcl_Fields.InValid_exclusion_id_type((SALT39.StrType)le.exclusion_id_type);
    SELF.status_Invalid := NewGcIdExcl_Fields.InValid_status((SALT39.StrType)le.status);
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    AnyRule_WithErrorsCount := COUNT(GROUP, h.fdn_file_gc_exclusion_id_Invalid > 0 OR h.fdn_file_info_id_Invalid > 0 OR h.exclusion_id_Invalid > 0 OR h.exclusion_id_type_Invalid > 0 OR h.status_Invalid > 0);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.fdn_file_gc_exclusion_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.fdn_file_info_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.exclusion_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.exclusion_id_type_ALLOW_ErrorCount > 0, 1, 0) + IF(le.status_ALLOW_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.fdn_file_gc_exclusion_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.fdn_file_info_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.exclusion_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.exclusion_id_type_ALLOW_ErrorCount > 0, 1, 0) + IF(le.status_ALLOW_ErrorCount > 0, 1, 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.FieldContents := CHOOSE(c,(SALT39.StrType)le.fdn_file_gc_exclusion_id,(SALT39.StrType)le.fdn_file_info_id,(SALT39.StrType)le.exclusion_id,(SALT39.StrType)le.exclusion_id_type,(SALT39.StrType)le.status,'***SALTBUG***');
  END;
  EXPORT AllErrors := NORMALIZE(h,5,Into(LEFT,COUNTER));
   bv := TABLE(AllErrors,{FieldContents, FieldName, Cnt := COUNT(GROUP)},FieldContents, FieldName,MERGE);
  EXPORT BadValues := TOPN(bv,1000,-Cnt);
  // Particular form of stats required for Orbit
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(NewGcIdExcl_Layout_NewGcIdExcl) prevDS = DATASET([], NewGcIdExcl_Layout_NewGcIdExcl), STRING10 Src='UNK'):= FUNCTION
  // field error stats
    SALT39.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,NewGcIdExcl_Fields.InvalidMessage_status(1)
          ,'Fields with errors'
    ...
    mod_hygiene := NewGcIdExcl_hygiene(PROJECT(h, NewGcIdExcl_Layout_NewGcIdExcl));
    ...
    SALT39.ScrubsOrbitLayout xNormHygieneStats(hygiene_summaryStats le, UNSIGNED c, STRING suffix) := TRANSFORM
      ...
      SELF.sourcecode := src;
      SELF.ruledesc := CHOOSE(c
          ,'fdn_file_gc_exclusion_id:' + getFieldTypeText(h.fdn_file_gc_exclusion_id) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix
          ,'fdn_file_info_id:' + getFieldTypeText(h.fdn_file_info_id) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix
          ,'exclusion_id:' + getFieldTypeText(h.exclusion_id) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix
          ,'exclusion_id_type:' + getFieldTypeText(h.exclusion_id_type) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix
      ...
      SELF.rulecnt := CHOOSE(c
          ,le.populated_fdn_file_gc_exclusion_id_cnt
          ,le.populated_fdn_file_info_id_cnt
          ,le.populated_exclusion_id_cnt
          ,le.populated_exclusion_id_type_cnt
      ...
      SELF.rulepcnt := CHOOSE(c
          ,le.populated_fdn_file_gc_exclusion_id_pcnt
          ,le.populated_fdn_file_info_id_pcnt
          ,le.populated_exclusion_id_pcnt
          ,le.populated_exclusion_id_type_pcnt
  ...
  // record count stats
    ...
    mod_Delta := NewGcIdExcl_Delta(prevDS, PROJECT(h, NewGcIdExcl_Layout_NewGcIdExcl));
...
EXPORT StandardStats(DATASET(NewGcIdExcl_Layout_NewGcIdExcl) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  myTimeStamp := (UNSIGNED6)SALT39.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
  expandedFile := FromNone(inFile).ExpandedInfile;
  mod_fromexpandedOverall := FromExpanded(expandedFile);
  scrubsSummaryOverall := mod_fromexpandedOverall.SummaryStats;
 
  SALT39.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_MBS, NewGcIdExcl_Fields, 'RECORDOF(scrubsSummaryOverall)', '');

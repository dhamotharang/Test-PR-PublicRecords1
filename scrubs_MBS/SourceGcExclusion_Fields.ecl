...
EXPORT SALT39.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'gc_id','product_id','company_id','status','date_added','user_added','date_changed','user_changed');
EXPORT SALT39.StrType FlatName(UNSIGNED2 i) := CHOOSE(i,'gc_id','product_id','company_id','status','date_added','user_added','date_changed','user_changed');
EXPORT FieldNum(SALT39.StrType fn) := CASE(fn,'gc_id' => 0,'product_id' => 1,'company_id' => 2,'status' => 3,'date_added' => 4,'user_added' => 5,'date_changed' => 6,'user_changed' => 7,0);
...
EXPORT Make_company_id(SALT39.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_company_id(SALT39.StrType s) := InValidFT_invalid_numeric(s);

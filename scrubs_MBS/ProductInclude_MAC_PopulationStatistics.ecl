...
EXPORT ProductInclude_MAC_PopulationStatistics(infile,Ref='',Input_fdn_file_product_include_id = '',Input_fdn_file_info_id = '',Input_product_id = '',Input_status = '',Input_date_added = '',Input_user_added = '',Input_date_changed = '',Input_user_changed = '',OutFile) := MACRO
  ...
  %of% %ot%(infile le) := TRANSFORM
    SELF.fields :=
    #IF( #TEXT(Input_fdn_file_product_include_id)='' )
      '' 
    #ELSE
        IF( le.Input_fdn_file_product_include_id = (TYPEOF(le.Input_fdn_file_product_include_id))'','',':fdn_file_product_include_id')
    #END

...
EXPORT Summary(SALT37.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_fdn_ind_type_gc_id_inclusion := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.fdn_ind_type_gc_id_inclusion)));
    avelength_fdn_ind_type_gc_id_inclusion := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.fdn_ind_type_gc_id_inclusion)),h.fdn_ind_type_gc_id_inclusion<>(typeof(h.fdn_ind_type_gc_id_inclusion))'');
    ...
    maxlength_inclusion_id := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.inclusion_id)));
    avelength_inclusion_id := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.inclusion_id)),h.inclusion_id<>(typeof(h.inclusion_id))'');
    populated_inclusion_type_pcnt := AVE(GROUP,IF(h.inclusion_type = (TYPEOF(h.inclusion_type))'',0,100));
    maxlength_inclusion_type := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.inclusion_type)));
    avelength_inclusion_type := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.inclusion_type)),h.inclusion_type<>(typeof(h.inclusion_type))'');
...
SALT37.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,IF (le.fdn_ind_type_gc_id_inclusion <> 0,TRIM((SALT37.StrType)le.fdn_ind_type_gc_id_inclusion), ''),IF (le.fdn_file_info_id <> 0,TRIM((SALT37.StrType)le.fdn_file_info_id), ''),IF (le.ind_type <> 0,TRIM((SALT37.StrType)le.ind_type), ''),IF (le.inclusion_id <> 0,TRIM((SALT37.StrType)le.inclusion_id), ''),TRIM((SALT37.StrType)le.inclusion_type),IF (le.status <> 0,TRIM((SALT37.StrType)le.status), ''),TRIM((SALT37.StrType)le.date_added),TRIM((SALT37.StrType)le.user_added),TRIM((SALT37.StrType)le.date_changed),TRIM((SALT37.StrType)le.user_changed)));
  SELF.FldNo := C;
END;
...
SALT37.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 10);
  SELF.FldNo2 := 1 + (C % 10);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,IF (le.fdn_ind_type_gc_id_inclusion <> 0,TRIM((SALT37.StrType)le.fdn_ind_type_gc_id_inclusion), ''),IF (le.fdn_file_info_id <> 0,TRIM((SALT37.StrType)le.fdn_file_info_id), ''),IF (le.ind_type <> 0,TRIM((SALT37.StrType)le.ind_type), ''),IF (le.inclusion_id <> 0,TRIM((SALT37.StrType)le.inclusion_id), ''),TRIM((SALT37.StrType)le.inclusion_type),IF (le.status <> 0,TRIM((SALT37.StrType)le.status), ''),TRIM((SALT37.StrType)le.date_added),TRIM((SALT37.StrType)le.user_added),TRIM((SALT37.StrType)le.date_changed),TRIM((SALT37.StrType)le.user_changed)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,IF (le.fdn_ind_type_gc_id_inclusion <> 0,TRIM((SALT37.StrType)le.fdn_ind_type_gc_id_inclusion), ''),IF (le.fdn_file_info_id <> 0,TRIM((SALT37.StrType)le.fdn_file_info_id), ''),IF (le.ind_type <> 0,TRIM((SALT37.StrType)le.ind_type), ''),IF (le.inclusion_id <> 0,TRIM((SALT37.StrType)le.inclusion_id), ''),TRIM((SALT37.StrType)le.inclusion_type),IF (le.status <> 0,TRIM((SALT37.StrType)le.status), ''),TRIM((SALT37.StrType)le.date_added),TRIM((SALT37.StrType)le.user_added),TRIM((SALT37.StrType)le.date_changed),TRIM((SALT37.StrType)le.user_changed)));
  END;
...
SHARED FldIds := DATASET([{1,'fdn_ind_type_gc_id_inclusion'}
      ...
      ,{8,'user_added'}
      ,{9,'date_changed'}
      ,{10,'user_changed'}],SALT37.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    MasterIdIndTypeIncl_Fields.InValid_fdn_ind_type_gc_id_inclusion((SALT37.StrType)le.fdn_ind_type_gc_id_inclusion),
    MasterIdIndTypeIncl_Fields.InValid_fdn_file_info_id((SALT37.StrType)le.fdn_file_info_id),
    MasterIdIndTypeIncl_Fields.InValid_ind_type((SALT37.StrType)le.ind_type),
    MasterIdIndTypeIncl_Fields.InValid_inclusion_id((SALT37.StrType)le.inclusion_id),
    MasterIdIndTypeIncl_Fields.InValid_inclusion_type((SALT37.StrType)le.inclusion_type),
    MasterIdIndTypeIncl_Fields.InValid_status((SALT37.StrType)le.status),
    MasterIdIndTypeIncl_Fields.InValid_date_added((SALT37.StrType)le.date_added),
    MasterIdIndTypeIncl_Fields.InValid_user_added((SALT37.StrType)le.user_added),
    MasterIdIndTypeIncl_Fields.InValid_date_changed((SALT37.StrType)le.date_changed),
    MasterIdIndTypeIncl_Fields.InValid_user_changed((SALT37.StrType)le.user_changed),
    0);

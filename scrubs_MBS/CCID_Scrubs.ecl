...
EXPORT FromNone(DATASET(CCID_Layout_CCID) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.cc_id_Invalid := CCID_Fields.InValid_cc_id((SALT39.StrType)le.cc_id);
    SELF.gc_id_Invalid := CCID_Fields.InValid_gc_id((SALT39.StrType)le.gc_id);
    SELF.account_id_Invalid := CCID_Fields.InValid_account_id((SALT39.StrType)le.account_id);
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    AnyRule_WithErrorsCount := COUNT(GROUP, h.cc_id_Invalid > 0 OR h.gc_id_Invalid > 0 OR h.account_id_Invalid > 0);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.cc_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.gc_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.account_id_ALLOW_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.cc_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.gc_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.account_id_ALLOW_ErrorCount > 0, 1, 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.FieldContents := CHOOSE(c,(SALT39.StrType)le.cc_id,(SALT39.StrType)le.gc_id,(SALT39.StrType)le.account_id,'***SALTBUG***');
  END;
  EXPORT AllErrors := NORMALIZE(h,3,Into(LEFT,COUNTER));
   bv := TABLE(AllErrors,{FieldContents, FieldName, Cnt := COUNT(GROUP)},FieldContents, FieldName,MERGE);
  EXPORT BadValues := TOPN(bv,1000,-Cnt);
  // Particular form of stats required for Orbit
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(CCID_Layout_CCID) prevDS = DATASET([], CCID_Layout_CCID), STRING10 Src='UNK'):= FUNCTION
  // field error stats
    SALT39.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'account_id:invalid_alphanumeric:ALLOW'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,CCID_Fields.InvalidMessage_account_id(1)
          ,'Fields with errors'
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ,le.cc_id_ALLOW_ErrorCount
          ,le.gc_id_ALLOW_ErrorCount
          ,le.account_id_ALLOW_ErrorCount,0) / le.TotalCnt + 0.5, CHOOSE(c - NumRules
    ...
    mod_hygiene := CCID_hygiene(PROJECT(h, CCID_Layout_CCID));
    ...
    SALT39.ScrubsOrbitLayout xNormHygieneStats(hygiene_summaryStats le, UNSIGNED c, STRING suffix) := TRANSFORM
      ...
      SELF.sourcecode := src;
      SELF.ruledesc := CHOOSE(c
          ,'cc_id:' + getFieldTypeText(h.cc_id) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix
          ,'gc_id:' + getFieldTypeText(h.gc_id) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix
          ,'account_id:' + getFieldTypeText(h.account_id) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix,'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
          ,le.populated_cc_id_cnt
          ,le.populated_gc_id_cnt
          ,le.populated_account_id_cnt,0);
      SELF.rulepcnt := CHOOSE(c
          ,le.populated_cc_id_pcnt
          ,le.populated_gc_id_pcnt
          ,le.populated_account_id_pcnt,0);
      SELF.ErrorMessage := '';
    END;
  ...
  // record count stats
    ...
    mod_Delta := CCID_Delta(prevDS, PROJECT(h, CCID_Layout_CCID));
...
EXPORT StandardStats(DATASET(CCID_Layout_CCID) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  myTimeStamp := (UNSIGNED6)SALT39.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
  expandedFile := FromNone(inFile).ExpandedInfile;
  mod_fromexpandedOverall := FromExpanded(expandedFile);
  scrubsSummaryOverall := mod_fromexpandedOverall.SummaryStats;
 
  SALT39.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_MBS, CCID_Fields, 'RECORDOF(scrubsSummaryOverall)', '');

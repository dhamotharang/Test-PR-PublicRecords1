...
EXPORT Summary(SALT311.Str30Type  txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_column_value_desc_id := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.column_value_desc_id)));
    avelength_column_value_desc_id := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.column_value_desc_id)),h.column_value_desc_id<>(typeof(h.column_value_desc_id))'');
    ...
    maxlength_table_column_id := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.table_column_id)));
    avelength_table_column_id := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.table_column_id)),h.table_column_id<>(typeof(h.table_column_id))'');
    ...
    maxlength_desc_value := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.desc_value)));
    avelength_desc_value := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.desc_value)),h.desc_value<>(typeof(h.desc_value))'');
...
SALT311.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,IF (le.column_value_desc_id <> 0,TRIM((SALT311.StrType)le.column_value_desc_id), ''),IF (le.table_column_id <> 0,TRIM((SALT311.StrType)le.table_column_id), ''),TRIM((SALT311.StrType)le.desc_value),IF (le.status <> 0,TRIM((SALT311.StrType)le.status), ''),TRIM((SALT311.StrType)le.description)));
  SELF.FldNo := C;
END;
...
SALT311.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 5);
  SELF.FldNo2 := 1 + (C % 5);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,IF (le.column_value_desc_id <> 0,TRIM((SALT311.StrType)le.column_value_desc_id), ''),IF (le.table_column_id <> 0,TRIM((SALT311.StrType)le.table_column_id), ''),TRIM((SALT311.StrType)le.desc_value),IF (le.status <> 0,TRIM((SALT311.StrType)le.status), ''),TRIM((SALT311.StrType)le.description)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,IF (le.column_value_desc_id <> 0,TRIM((SALT311.StrType)le.column_value_desc_id), ''),IF (le.table_column_id <> 0,TRIM((SALT311.StrType)le.table_column_id), ''),TRIM((SALT311.StrType)le.desc_value),IF (le.status <> 0,TRIM((SALT311.StrType)le.status), ''),TRIM((SALT311.StrType)le.description)));
  END;
...
SHARED FldIds := DATASET([{1,'column_value_desc_id'}
      ...
      ,{3,'desc_value'}
      ,{4,'status'}
      ,{5,'description'}],SALT311.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ColValDesc_Fields.InValid_column_value_desc_id((SALT311.StrType)le.column_value_desc_id),
    ColValDesc_Fields.InValid_table_column_id((SALT311.StrType)le.table_column_id),
    ColValDesc_Fields.InValid_desc_value((SALT311.StrType)le.desc_value),
    ColValDesc_Fields.InValid_status((SALT311.StrType)le.status),
    ColValDesc_Fields.InValid_description((SALT311.StrType)le.description),
    0);
...
PrettyErrorTotals := RECORD
  ...
  FieldType := CHOOSE(TotalErrors.FieldNum,'invalid_numeric','invalid_numeric','invalid_text','invalid_numeric','invalid_text');
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_MBS, ColValDesc_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

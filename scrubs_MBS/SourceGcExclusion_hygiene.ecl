...
SALT37.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,IF (le.gc_id <> 0,TRIM((SALT37.StrType)le.gc_id), ''),IF (le.product_id <> 0,TRIM((SALT37.StrType)le.product_id), ''),IF (le.company_id <> 0,TRIM((SALT37.StrType)le.company_id), ''),IF (le.status <> 0,TRIM((SALT37.StrType)le.status), ''),TRIM((SALT37.StrType)le.date_added),TRIM((SALT37.StrType)le.user_added),TRIM((SALT37.StrType)le.date_changed),TRIM((SALT37.StrType)le.user_changed)));
  SELF.FldNo := C;
END;
...
SALT37.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 8);
  SELF.FldNo2 := 1 + (C % 8);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,IF (le.gc_id <> 0,TRIM((SALT37.StrType)le.gc_id), ''),IF (le.product_id <> 0,TRIM((SALT37.StrType)le.product_id), ''),IF (le.company_id <> 0,TRIM((SALT37.StrType)le.company_id), ''),IF (le.status <> 0,TRIM((SALT37.StrType)le.status), ''),TRIM((SALT37.StrType)le.date_added),TRIM((SALT37.StrType)le.user_added),TRIM((SALT37.StrType)le.date_changed),TRIM((SALT37.StrType)le.user_changed)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,IF (le.gc_id <> 0,TRIM((SALT37.StrType)le.gc_id), ''),IF (le.product_id <> 0,TRIM((SALT37.StrType)le.product_id), ''),IF (le.company_id <> 0,TRIM((SALT37.StrType)le.company_id), ''),IF (le.status <> 0,TRIM((SALT37.StrType)le.status), ''),TRIM((SALT37.StrType)le.date_added),TRIM((SALT37.StrType)le.user_added),TRIM((SALT37.StrType)le.date_changed),TRIM((SALT37.StrType)le.user_changed)));
  END;
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    SourceGcExclusion_Fields.InValid_gc_id((SALT37.StrType)le.gc_id),
    SourceGcExclusion_Fields.InValid_product_id((SALT37.StrType)le.product_id),
    SourceGcExclusion_Fields.InValid_company_id((SALT37.StrType)le.company_id),
    SourceGcExclusion_Fields.InValid_status((SALT37.StrType)le.status),
    SourceGcExclusion_Fields.InValid_date_added((SALT37.StrType)le.date_added),
    SourceGcExclusion_Fields.InValid_user_added((SALT37.StrType)le.user_added),
    SourceGcExclusion_Fields.InValid_date_changed((SALT37.StrType)le.date_changed),
    SourceGcExclusion_Fields.InValid_user_changed((SALT37.StrType)le.user_changed),
    0);

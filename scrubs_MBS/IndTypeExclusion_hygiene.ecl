...
EXPORT Summary(SALT39.Str30Type  txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    populated_fdn_file_ind_type_exclusion_id_cnt := COUNT(GROUP,h.fdn_file_ind_type_exclusion_id <> (TYPEOF(h.fdn_file_ind_type_exclusion_id))'');
    ...
    maxlength_fdn_file_ind_type_exclusion_id := MAX(GROUP,LENGTH(TRIM((SALT39.StrType)h.fdn_file_ind_type_exclusion_id)));
    avelength_fdn_file_ind_type_exclusion_id := AVE(GROUP,LENGTH(TRIM((SALT39.StrType)h.fdn_file_ind_type_exclusion_id)),h.fdn_file_ind_type_exclusion_id<>(typeof(h.fdn_file_ind_type_exclusion_id))'');
...
SALT39.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,IF (le.fdn_file_ind_type_exclusion_id <> 0,TRIM((SALT39.StrType)le.fdn_file_ind_type_exclusion_id), ''),IF (le.fdn_file_info_id <> 0,TRIM((SALT39.StrType)le.fdn_file_info_id), ''),IF (le.ind_type <> 0,TRIM((SALT39.StrType)le.ind_type), ''),IF (le.status <> 0,TRIM((SALT39.StrType)le.status), ''),TRIM((SALT39.StrType)le.date_added),TRIM((SALT39.StrType)le.user_added),TRIM((SALT39.StrType)le.date_changed),TRIM((SALT39.StrType)le.user_changed)));
  SELF.FldNo := C;
END;
...
SALT39.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 8);
  SELF.FldNo2 := 1 + (C % 8);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,IF (le.fdn_file_ind_type_exclusion_id <> 0,TRIM((SALT39.StrType)le.fdn_file_ind_type_exclusion_id), ''),IF (le.fdn_file_info_id <> 0,TRIM((SALT39.StrType)le.fdn_file_info_id), ''),IF (le.ind_type <> 0,TRIM((SALT39.StrType)le.ind_type), ''),IF (le.status <> 0,TRIM((SALT39.StrType)le.status), ''),TRIM((SALT39.StrType)le.date_added),TRIM((SALT39.StrType)le.user_added),TRIM((SALT39.StrType)le.date_changed),TRIM((SALT39.StrType)le.user_changed)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,IF (le.fdn_file_ind_type_exclusion_id <> 0,TRIM((SALT39.StrType)le.fdn_file_ind_type_exclusion_id), ''),IF (le.fdn_file_info_id <> 0,TRIM((SALT39.StrType)le.fdn_file_info_id), ''),IF (le.ind_type <> 0,TRIM((SALT39.StrType)le.ind_type), ''),IF (le.status <> 0,TRIM((SALT39.StrType)le.status), ''),TRIM((SALT39.StrType)le.date_added),TRIM((SALT39.StrType)le.user_added),TRIM((SALT39.StrType)le.date_changed),TRIM((SALT39.StrType)le.user_changed)));
  END;
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    IndTypeExclusion_Fields.InValid_fdn_file_ind_type_exclusion_id((SALT39.StrType)le.fdn_file_ind_type_exclusion_id),
    IndTypeExclusion_Fields.InValid_fdn_file_info_id((SALT39.StrType)le.fdn_file_info_id),
    IndTypeExclusion_Fields.InValid_ind_type((SALT39.StrType)le.ind_type),
    IndTypeExclusion_Fields.InValid_status((SALT39.StrType)le.status),
    IndTypeExclusion_Fields.InValid_date_added((SALT39.StrType)le.date_added),
    IndTypeExclusion_Fields.InValid_user_added((SALT39.StrType)le.user_added),
    IndTypeExclusion_Fields.InValid_date_changed((SALT39.StrType)le.date_changed),
    IndTypeExclusion_Fields.InValid_user_changed((SALT39.StrType)le.user_changed),
    0);
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT39.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_MBS, IndTypeExclusion_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

...
EXPORT SALT37.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'fdn_file_product_include_id','fdn_file_info_id','product_id','status','date_added','user_added','date_changed','user_changed');
EXPORT FieldNum(SALT37.StrType fn) := CASE(fn,'fdn_file_product_include_id' => 0,'fdn_file_info_id' => 1,'product_id' => 2,'status' => 3,'date_added' => 4,'user_added' => 5,'date_changed' => 6,'user_changed' => 7,0);
 
//Individual field level validation
 
EXPORT Make_fdn_file_product_include_id(SALT37.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_fdn_file_product_include_id(SALT37.StrType s) := InValidFT_invalid_numeric(s);

...
EXPORT Summary(SALT37.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_sub_account_id := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.sub_account_id)));
    avelength_sub_account_id := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.sub_account_id)),h.sub_account_id<>(typeof(h.sub_account_id))'');
    populated_hh_id_pcnt := AVE(GROUP,IF(h.hh_id = (TYPEOF(h.hh_id))'',0,100));
    maxlength_hh_id := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.hh_id)));
    avelength_hh_id := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.hh_id)),h.hh_id<>(typeof(h.hh_id))'');
  END;
...
SALT37.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,IF (le.gc_id <> 0,TRIM((SALT37.StrType)le.gc_id), ''),TRIM((SALT37.StrType)le.sub_account_id),IF (le.hh_id <> 0,TRIM((SALT37.StrType)le.hh_id), '')));
  SELF.FldNo := C;
END;
...
SALT37.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 3);
  SELF.FldNo2 := 1 + (C % 3);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,IF (le.gc_id <> 0,TRIM((SALT37.StrType)le.gc_id), ''),TRIM((SALT37.StrType)le.sub_account_id),IF (le.hh_id <> 0,TRIM((SALT37.StrType)le.hh_id), '')));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,IF (le.gc_id <> 0,TRIM((SALT37.StrType)le.gc_id), ''),TRIM((SALT37.StrType)le.sub_account_id),IF (le.hh_id <> 0,TRIM((SALT37.StrType)le.hh_id), '')));
  END;
...
SHARED FldIds := DATASET([{1,'gc_id'}
      ,{2,'sub_account_id'}
      ,{3,'hh_id'}],SALT37.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    HHID_Fields.InValid_gc_id((SALT37.StrType)le.gc_id),
    HHID_Fields.InValid_sub_account_id((SALT37.StrType)le.sub_account_id),
    HHID_Fields.InValid_hh_id((SALT37.StrType)le.hh_id),
    0);

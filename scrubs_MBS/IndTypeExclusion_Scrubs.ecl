...
EXPORT FromNone(DATASET(IndTypeExclusion_Layout_IndTypeExclusion) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.fdn_file_ind_type_exclusion_id_Invalid := IndTypeExclusion_Fields.InValid_fdn_file_ind_type_exclusion_id((SALT39.StrType)le.fdn_file_ind_type_exclusion_id);
    SELF.fdn_file_info_id_Invalid := IndTypeExclusion_Fields.InValid_fdn_file_info_id((SALT39.StrType)le.fdn_file_info_id);
    SELF.ind_type_Invalid := IndTypeExclusion_Fields.InValid_ind_type((SALT39.StrType)le.ind_type);
    SELF.status_Invalid := IndTypeExclusion_Fields.InValid_status((SALT39.StrType)le.status);
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    AnyRule_WithErrorsCount := COUNT(GROUP, h.fdn_file_ind_type_exclusion_id_Invalid > 0 OR h.fdn_file_info_id_Invalid > 0 OR h.ind_type_Invalid > 0 OR h.status_Invalid > 0);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.fdn_file_ind_type_exclusion_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.fdn_file_info_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.ind_type_ALLOW_ErrorCount > 0, 1, 0) + IF(le.status_ALLOW_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.fdn_file_ind_type_exclusion_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.fdn_file_info_id_ALLOW_ErrorCount > 0, 1, 0) + IF(le.ind_type_ALLOW_ErrorCount > 0, 1, 0) + IF(le.status_ALLOW_ErrorCount > 0, 1, 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.FieldContents := CHOOSE(c,(SALT39.StrType)le.fdn_file_ind_type_exclusion_id,(SALT39.StrType)le.fdn_file_info_id,(SALT39.StrType)le.ind_type,(SALT39.StrType)le.status,'***SALTBUG***');
  END;
  EXPORT AllErrors := NORMALIZE(h,4,Into(LEFT,COUNTER));
   bv := TABLE(AllErrors,{FieldContents, FieldName, Cnt := COUNT(GROUP)},FieldContents, FieldName,MERGE);
  EXPORT BadValues := TOPN(bv,1000,-Cnt);
  // Particular form of stats required for Orbit
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(IndTypeExclusion_Layout_IndTypeExclusion) prevDS = DATASET([], IndTypeExclusion_Layout_IndTypeExclusion), STRING10 Src='UNK'):= FUNCTION
  // field error stats
    SALT39.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,IndTypeExclusion_Fields.InvalidMessage_status(1)
          ,'Fields with errors'
    ...
    mod_hygiene := IndTypeExclusion_hygiene(PROJECT(h, IndTypeExclusion_Layout_IndTypeExclusion));
    ...
    SALT39.ScrubsOrbitLayout xNormHygieneStats(hygiene_summaryStats le, UNSIGNED c, STRING suffix) := TRANSFORM
      ...
      SELF.sourcecode := src;
      SELF.ruledesc := CHOOSE(c
          ,'fdn_file_ind_type_exclusion_id:' + getFieldTypeText(h.fdn_file_ind_type_exclusion_id) + IF(TRIM(le.txt) > '', '_' + TRIM(le.txt), '') + ':' + suffix
      ...
      SELF.rulecnt := CHOOSE(c
          ,le.populated_fdn_file_ind_type_exclusion_id_cnt
      ...
      SELF.rulepcnt := CHOOSE(c
          ,le.populated_fdn_file_ind_type_exclusion_id_pcnt
  ...
  // record count stats
    ...
    mod_Delta := IndTypeExclusion_Delta(prevDS, PROJECT(h, IndTypeExclusion_Layout_IndTypeExclusion));
...
EXPORT StandardStats(DATASET(IndTypeExclusion_Layout_IndTypeExclusion) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  myTimeStamp := (UNSIGNED6)SALT39.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
  expandedFile := FromNone(inFile).ExpandedInfile;
  mod_fromexpandedOverall := FromExpanded(expandedFile);
  scrubsSummaryOverall := mod_fromexpandedOverall.SummaryStats;
 
  SALT39.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_MBS, IndTypeExclusion_Fields, 'RECORDOF(scrubsSummaryOverall)', '');

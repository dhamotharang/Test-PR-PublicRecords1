...
F := Scrubs_MBS.ProductInclude_In_ProductInclude; // Input file
S := Scrubs_MBS.ProductInclude_Scrubs; // My scrubs module
N := S.FromNone(F); // Generate the error flags
B := N.BitmapInfile;
//OUTPUT(B,,'~thor_data400::scrubs::fraudgov::20170522::scrubs_ProductInclude', overwrite, compressed);
...
// Scrubs.OrbitProfileStats('Scrubs_MBS_ProductInclude','Scrubs',stats).ProfileTemplate;
//Generating a template to upload rules to Orbit
//Submits stats to Orbit
submitStats:=Scrubs.OrbitProfileStats('Scrubs_MBS_ProductInclude','ScrubsAlerts',stats,versionDate,'MBS').SubmitStats;
submitStats;			

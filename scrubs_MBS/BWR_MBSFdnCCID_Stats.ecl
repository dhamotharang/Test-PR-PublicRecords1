...
F := Scrubs_MBS.CCID_In_CCID; // Input file
S := Scrubs_MBS.CCID_Scrubs; // My scrubs module
N := S.FromNone(F); // Generate the error flags
B := N.BitmapInfile;
//OUTPUT(B,,'~thor_data400::scrubs::fraudgov::20170522::scrubs_CCID', overwrite, compressed);
...
// Scrubs.OrbitProfileStats('Scrubs_MBS_CCID','Scrubs',stats).ProfileTemplate;
//Generating a template to upload rules to Orbit
//Submits stats to Orbit
submitStats:=Scrubs.OrbitProfileStats('Scrubs_MBS_CCID','ScrubsAlerts',stats,versionDate,'MBS').SubmitStats;
submitStats;			

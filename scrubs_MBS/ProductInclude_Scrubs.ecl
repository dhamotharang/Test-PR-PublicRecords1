...
EXPORT FromNone(DATASET(ProductInclude_Layout_ProductInclude) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.fdn_file_product_include_id_Invalid := ProductInclude_Fields.InValid_fdn_file_product_include_id((SALT37.StrType)le.fdn_file_product_include_id);
    SELF.fdn_file_info_id_Invalid := ProductInclude_Fields.InValid_fdn_file_info_id((SALT37.StrType)le.fdn_file_info_id);
    SELF.product_id_Invalid := ProductInclude_Fields.InValid_product_id((SALT37.StrType)le.product_id);
    SELF.status_Invalid := ProductInclude_Fields.InValid_status((SALT37.StrType)le.status);
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.FieldContents := CHOOSE(c,(SALT37.StrType)le.fdn_file_product_include_id,(SALT37.StrType)le.fdn_file_info_id,(SALT37.StrType)le.product_id,(SALT37.StrType)le.status,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT37.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,ProductInclude_Fields.InvalidMessage_status(1),'UNKNOWN');

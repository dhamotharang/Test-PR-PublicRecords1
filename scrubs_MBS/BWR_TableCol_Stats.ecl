...
F := Scrubs_MBS.TableCol_In_TableCol; // Input file
S := Scrubs_MBS.TableCol_Scrubs; // My scrubs module
N := S.FromNone(F); // Generate the error flags
B := N.BitmapInfile;
//OUTPUT(B,,'~thor_data400::scrubs::fraudgov::20170522::scrubs_TableCol', overwrite, compressed);
...
// Scrubs.OrbitProfileStats('Scrubs_MBS_TableCol','Scrubs',stats).ProfileTemplate;
//Generating a template to upload rules to Orbit
//Submits stats to Orbit
submitStats:=Scrubs.OrbitProfileStats('Scrubs_MBS_TableCol','ScrubsAlerts',stats,versionDate,'MBS').SubmitStats;
submitStats;			

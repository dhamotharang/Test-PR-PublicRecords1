...
out3 := sort(table(j1,     r2,st,county,type_,coverage_type,few),st,county,type_);
out4 := sort(table(j1_filt,r3,st,county,type_,few),              st,county,type_);
...
final_out := sort(join(out3,out4,(left.st=right.st and left.county=right.county and left.type_=right.type_),t_j1(left,right),left outer),type_,st,county)(st_co_count>10);
		  
export coverage_overall := output(final_out(st<>'' and st in ln_property.valid_states and county<>''),all,named('Property_Coverage'));
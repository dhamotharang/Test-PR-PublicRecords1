IMPORT ut,SALT32;
IMPORT Scrubs_Corp2_Mapping_HI_AR,Scrubs; // Import modules for FieldTypes attribute definitions
EXPORT Fields := MODULE
 
// Processing for each FieldType
EXPORT SALT32.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_corp_key','invalid_charter_nbr','invalid_corp_vendor','invalid_state_origin','invalid_mandatory','invalid_date','invalid_future_date','invalid_numeric','invalid_alphablank');
EXPORT FieldTypeNum(SALT32.StrType fn) := CASE(fn,'invalid_corp_key' => 1,'invalid_charter_nbr' => 2,'invalid_corp_vendor' => 3,'invalid_state_origin' => 4,'invalid_mandatory' => 5,'invalid_date' => 6,'invalid_future_date' => 7,'invalid_numeric' => 8,'invalid_alphablank' => 9,0);
...
EXPORT MakeFT_invalid_corp_key(SALT32.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_invalid_corp_key(SALT32.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT32.StringFilter(s,'-ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'))),~Scrubs_Corp2_Mapping_HI_AR.Functions.invalid_corp_key(s)>0,~(LENGTH(TRIM(s)) >= 4));
EXPORT InValidMessageFT_invalid_corp_key(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInChars('-ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'),SALT32.HygieneErrors.CustomFail('Scrubs_Corp2_Mapping_HI_AR.Functions.invalid_corp_key'),SALT32.HygieneErrors.NotLength('4..'),SALT32.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_charter_nbr(SALT32.StrType s0) := FUNCTION
  s1 := SALT32.stringfilter(s0,' -ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_charter_nbr(SALT32.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT32.StringFilter(s,' -ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'))),~Scrubs_Corp2_Mapping_HI_AR.Functions.invalid_corp_sos_charter_nbr(s)>0,~(LENGTH(TRIM(s)) >= 1));
EXPORT InValidMessageFT_invalid_charter_nbr(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInChars(' -ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'),SALT32.HygieneErrors.CustomFail('Scrubs_Corp2_Mapping_HI_AR.Functions.invalid_corp_sos_charter_nbr'),SALT32.HygieneErrors.NotLength('1..'),SALT32.HygieneErrors.Good);
...
EXPORT Make_ar_comment(SALT32.StrType s0) := MakeFT_invalid_alphablank(s0);
EXPORT InValid_ar_comment(SALT32.StrType s) := InValidFT_invalid_alphablank(s);
EXPORT InValidMessage_ar_comment(UNSIGNED1 wh) := InValidMessageFT_invalid_alphablank(wh);
...
EXPORT Make_ar_status(SALT32.StrType s0) := MakeFT_invalid_alphablank(s0);
EXPORT InValid_ar_status(SALT32.StrType s) := InValidFT_invalid_alphablank(s);
EXPORT InValidMessage_ar_status(UNSIGNED1 wh) := InValidMessageFT_invalid_alphablank(wh);
...
EXPORT Make_ar_amount_paid(SALT32.StrType s0) := s0;
EXPORT InValid_ar_amount_paid(SALT32.StrType s) := FALSE;
EXPORT InValidMessage_ar_amount_paid(UNSIGNED1 wh) := '';
 
// This macro will compute and count field level differences based upon a pivot expression
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT32,Scrubs_Corp2_Mapping_HI_AR;
...
#uniquename(AggRec)
  %AggRec% := RECORD
    ...
    Count_Diff_ar_amount_paid := COUNT(GROUP,%Closest%.Diff_ar_amount_paid);
  END;

...
EXPORT Mac_SrcFrequency_Outliers(infile,infield,srcfield,o1,o21,o22,o31,o32,o41,o42) := MACRO
  #uniquename(r)
  %r% := RECORD
	  SALT39.Str30Type fieldname := #TEXT(infield);
	  SALT39.StrType fieldvalue {MAXLENGTH(200000)} := (SALT39.StrType)infile.infield ;
		SALT39.StrType src {MAXLENGTH(30)} := infile.srcfield ;
...
EXPORT o22 := TABLE(%osu%, {SALT39.Str30Type fieldname := #TEXT(infield);SALT39.Str30Type OutlierType := 'Unique'; SALT39.StrType src := %osu%.src; REAL score := %osu%.score;});
	...
	EXPORT o32 := TABLE(%osd%,{SALT39.Str30Type fieldname := #TEXT(infield);SALT39.Str30Type OutlierType := 'Distinct'; SALT39.StrType src := %osd%.src; REAL score := %osd%.score;});
	...
	EXPORT o42 := TABLE(%ot5s%,{SALT39.Str30Type fieldname := #TEXT(infield);SALT39.Str30Type OutlierType := 'Top5'; SALT39.StrType src := %ot5s%.src; REAL score := %ot5s%.score;});
  ENDMACRO;

...
NB(SALT39.StrType s) := SALT39.StringFilterOut(s,' -');
...
Fix(SALT39.StrType s) := SALT39.StringCleanSpaces(SALT39.StringSubstituteOut(s,'-',' '));
TYPEOF(fieldvals) apfw(fieldvals le,UNSIGNED c) := TRANSFORM
  SELF.infield := SALT39.GetNthWord(Fix(le.infield),c);
  SELF := le;
  END;
sw0 := DISTRIBUTED(NORMALIZE(fieldvals,SALT39.WordCount(Fix(LEFT.infield)),apfw(LEFT,COUNTER)),ufield); // Get each word

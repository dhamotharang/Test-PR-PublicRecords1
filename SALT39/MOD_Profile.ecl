EXPORT MOD_Profile(inFile_raw,id_field='',src_field='',CorrelateSampleSize=100000000,out_prefix='',maxFieldCorr=50) := FUNCTIONMACRO
	...
	#FOR(inFileFields)
		#FOR(field)
			...
			#ELSEIF(%recLevel% = 0 OR (%recLevel%=1 AND %'recName'%<>''))
				...
				#APPEND(transformClauses, 'TRIM((SALT39.StrType)le.' +%'recName'% + %'@name'% + ')')
				...
				#APPEND(summaryFields, 'maxlength_'+%'recFlat'%+%'@name'%+' := MAX(GROUP,LENGTH(TRIM((SALT39.StrType)'+#TEXT(inFile)+'.'+%'recName'%+%'@name'%+')));')
				#APPEND(summaryFields, 'avelength_'+%'recFlat'%+%'@name'%+' := AVE(GROUP,LENGTH(TRIM((SALT39.StrType)'+#TEXT(inFile)+'.'+%'recName'%+%'@name'%+')),'+#TEXT(inFile)+'.'+%'recName'%+%'@name'%+'<>(typeof('+#TEXT(inFile)+'.'+%'recName'%+%'@name'%+'))\'\');')
	...
	%M% := MODULE
		...
		SALT39.MAC_Character_Counts.X_Data_Layout Into(RECORDOF(inFile) le, UNSIGNED c) := TRANSFORM
		...
		SHARED FldIds := DATASET([%datasetRecords%], SALT39.MAC_Character_Counts.Field_Identification);
		...
		SALT39.MAC_Correlate.Data_Layout IntoP(inFile le, UNSIGNED C) := TRANSFORM
		...
		Pairs00 := IF(okCorrelations, Pairs0, DATASET([],SALT39.MAC_Correlate.Data_Layout));
		EXPORT Correlations := SALT39.MAC_Correlate.Fn_Profile(Pairs00,FldIds);
		...
		// Cluster counts
		#IF(#TEXT(id_field)<>'')
			EXPORT ClusterCounts := SALT39.MOD_ClusterStats.Counts(inFile,id_field);
			EXPORT out_ClusterCounts := OUTPUT(ClusterCounts,NAMED(pfx+'ClusterCounts'),ALL);
			#APPEND(outCondition, ',out_ClusterCounts')
			#IF(#TEXT(src_field)<>'')
				EXPORT ClusterSrc := SALT39.MOD_ClusterStats.Sources(inFile,id_field,src_field);

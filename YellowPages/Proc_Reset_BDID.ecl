...
YP_Base_BusName_NonBlank 	:= Files().Base.QA(business_name <> '');
YP_Base_BusName_Blank		  := Files().Base.QA(business_name = '');
...
//ut.MAC_SF_BuildProcess(yellowpages_base_bdid_all,Filenames.BaseSuperfile,do_it,2)

ut.MAC_SF_Build_standard(yellowpages_base_bdid_all,	Filenames().Base.Template,	Reset_Base,	YellowPages_Reset_Date);



do_count := output(COUNT(YellowPages_Base_BDID_All(bdid <> 0)), named('BaseYellowPagesBdids'));

export Proc_Reset_BDID := sequential(Reset_Base,do_count);

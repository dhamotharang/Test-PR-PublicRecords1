...
Gong_Match_Init := PROJECT(Gong.File_Gong_full(listing_type_bus<>'', listed_name <> '', err_stat[1]='S'), InitGongMatch(LEFT));
...
Layout_YellowPages_Seq InitYellowPagesMatch(YellowPages.Layout_YellowPages L) := TRANSFORM
...
YellowPages_Init := PROJECT(YellowPages.File_YellowPages(business_name <> ''), InitYellowPagesMatch(LEFT));
...
SELF.phone_type := MAP(L.phone10[1..3] IN Toll_Free_Area_Codes => 'FREE',
                       datalib.NPA_PhoneType(L.phone10));
SELF := L;
END;

YellowPages_Type_Init := PROJECT(YellowPages_Combined_Dedup, InitYellowPagesType(LEFT));

YellowPages_Type_Addr_Dist := DISTRIBUTE(YellowPages_Type_Init(phone_type='****', (INTEGER)zip<>0, prim_name<>''), HASH(zip, prim_name, prim_range));
COUNT(YellowPages_Type_Addr_Dist);

// Replace YellowPage Listing address with Gong Address
Layout_YellowPages_Seq MatchYPPhone(Layout_Gong_Slim L, Layout_YellowPages_Seq R) := TRANSFORM
SELF.phone10 := IF(R.phone10 <> L.phone10, L.phone10, R.phone10);
SELF.phone_override := IF(R.phone10 <> L.phone10, 'G', R.phone_override);
SELF.phone_type := IF(R.phone10 <> L.phone10, datalib.NPA_PhoneType(L.phone10), R.phone_type);
SELF := R;
END;


YellowPages_Type_Addr_Primary := JOIN(Gong_Match_Primary_Addr_Dist,
...
YellowPages.Layout_YellowPages_Base FormatYPBase(Layout_YellowPages_Seq L) := TRANSFORM
SELF := L;
END;

YellowPages_Formatted := PROJECT(YellowPages_Type_Combined_Dedup, FormatYPBase(LEFT));

EXPORT YellowPages_Base_YP := YellowPages_Formatted : PERSIST('TEMP::YellowPages_Base_YP');
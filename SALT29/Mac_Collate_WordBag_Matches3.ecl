EXPORT Mac_Collate_WordBag_Matches3(n,indexOutputRecord,doIndexRead,joinfieldpp,joinfieldp,joinfield,steppedmatches) := MACRO
doJoin(SET OF DATASET(indexOutputRecord) inputs) := FUNCTION
    indexOutputRecord createMatchRecord(indexOutputRecord firstMatch, DATASET(indexOutputRecord) allMatches) := TRANSFORM
        ...
        SELF := firstMatch;
    END;
    RETURN MERGEJOIN(inputs, STEPPED(LEFT.joinfield = RIGHT.joinfield AND LEFT.joinfieldp = RIGHT.joinfieldp AND LEFT.joinfieldpp = RIGHT.joinfieldpp)
		//, createMatchRecord(LEFT, ROWS(LEFT))
		, SORTED(joinfieldpp,joinfieldp,joinfield)
		, ASSERT SORTED);
END;
#uniquename(r)
%r% := RECORDOF(n);
...
steppedMatches1 := GROUP(steppedMatches0,joinfieldpp,joinfieldp,joinfield);

...
import ut;
#uniquename(s0)
shared %s0% := SALT29.MAC_Field_By_UID(infile,ufield,infield);
#uniquename(s)
#if(hyphenate)
SALT29.MAC_Field_Variants_Hyphen(%s0%,ufield,infield,sv)
shared %s% := sv;
#elsif(add_initials)
SALT29.MAC_Field_Variants_Initials(%s0%,ufield,infield,sv)
shared %s% := sv;
#elsif(word_bag)
SALT29.MAC_Field_Variants_WordBag(%s0%,ufield,infield,sv)
shared %s% := sv;
#else
shared %s% := group(%s0%);
#end
...
SALT29.MAC_Field_Count_UID(%s%,infield,ufield,sv,countedclusters);
...
#uniquename(t0)
SALT29.MAC_Field_Specificities(t1,sv1)
...
#else
SALT29.MAC_Field_Nulls(spec_values,null_value_type,nv)
...
#end
SALT29.MAC_Field_Bfoul(%s0%,infield,ufield,null_value_label,ClusterSizes,add_initials,hyphenate,bf)
...
SALT29.MAC_Field_Specificity(Spec_Values,infield,null_value_label,ol)
export label := ol;
  ENDMACRO;

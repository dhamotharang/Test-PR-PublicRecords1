...
export  Match1L( StrType L1, unsigned2 lspec1, boolean lnne1, integer lfor1, Boolean linit1, integer edit1, StrType R1, unsigned2 rspec1) := FUNCTION
	...
	result1 := map( SALT29.WithinEditN(L1,R1,edit1)  => if(weight > lfor1, if(lspec1 > 0 and rspec1 > 0, min(lspec1,rspec1), weight), FailedForce), 
								 length(R1) > length(L1) and L1 = R1[1..length(TRIM(L1))] => init_weight,
								 length(L1) > length(R1) and R1 = L1[1..length(TRIM(R1))] => init_weight,
								 Failed);
								 
	result := MAP ( 
							 not linit1 and not lnne1 and SALT29.WithinEditN(L1,R1,edit1) => if(weight >  lfor1, minWeight, FailedForce),	// boolean linit - initial flag,  boolean lnnel - nulls not allowed flag
							 not linit1 and lnne1 and ( L1 = '' or R1 = '' ) 			=> 0,
							 not linit1 and lnne1 and SALT29.WithinEditN(L1,R1,edit1) => if(weight >  lfor1, minWeight, FailedForce),
...
export Match2L(
    ...
    L1BlankCost := IF ( lfor1 > 0, FailedForce, Failed );
	 ...
	 result := MAP(not lnne1  and not linit1  and lfor1 > 0 	     => r0,
								...
								linit1 or linit2                                			=> r0,
								SALT29.WithinEditN(L1,R1,edit1) and SALT29.WithinEditN(L2,R2,edit2) => if(weight1+weight2 > lfor1, min(lspec1, rspec1) + min(lspec2, rspec2), FailedForce), 
								SALT29.WithinEditN(L1,R2,edit1) and SALT29.WithinEditN(L2,R1,edit2) => if(weight1+weight2 > lfor1, min(lspec1, rspec2) + min(lspec2, rspec1), FailedForce), 
...
shared stringCompare(StrType L1, boolean init, boolean nne, integer edit, StrType R1) := function
	...
	result := map(not nne and (L1 = '' or R1 = '') => false,
							  ...
							  SALT29.WithinEditN(L1,R1,edit) => true , 
							  false);

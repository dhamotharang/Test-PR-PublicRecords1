...
NB(SALT29.StrType s) := SALT29.StringFilterOut(s,' -');
...
Fix(SALT29.StrType s) := SALT29.StringCleanSpaces(SALT29.StringSubstituteOut(s,'-',' '));
TYPEOF(fieldvals) apfw(fieldvals le,UNSIGNED c) := TRANSFORM
  SELF.infield := SALT29.GetNthWord(Fix(le.infield),c);
  SELF.ufield := le.ufield;
  END;
sw0 := DISTRIBUTED(NORMALIZE(fieldvals,SALT29.WordCount(Fix(LEFT.infield)),apfw(LEFT,COUNTER)),ufield); // Get each word

...
namespace ns_SALT29_Fn_Match_Bagofwords_Fuzzy {
/*
...
enum MatchBagOfWordsAbbrType {LEGACY, FIRST, INITIAL};
	...
	bool acronym_equals(unsigned l, wb &r,unsigned rl,unsigned &total, int &matching,unsigned abbrType)
  {
      ...
      if ( l == nwords-1 || used[l+1] ) // next token has to exist
          return false;
      unsigned count;
      if (isAcronym(l, r.word_lengths[rl],r.words[rl],count,abbrType))
      {
...
using namespace ns_SALT29_Fn_Match_Bagofwords_Fuzzy;
#body
{
    ...
    ns_SALT29_Fn_Match_Bagofwords_Fuzzy::wb L(lenL,l,stem);
    ns_SALT29_Fn_Match_Bagofwords_Fuzzy::wb R(lenR,r,stem);
    if ( L.error || R.error )
    ...
    // Short & Long
    ns_SALT29_Fn_Match_Bagofwords_Fuzzy::wb &SH = L.nwords < R.nwords || lenL < lenR ? L : R;
    ns_SALT29_Fn_Match_Bagofwords_Fuzzy::wb &LO = L.nwords < R.nwords || lenL < lenR ? R : L;

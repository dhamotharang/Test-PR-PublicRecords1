...
export Data_Layout := RECORD
  SALT29.StrType Fld { MAXLENGTH(200000) };
	UNSIGNED2 FldNo;
  END;
...
shared Pattern_Layout := record
  SALT29.StrType data_pattern {maxlength(200000)};
	unsigned cnt;
	REAL pcnt;
	end;
	
shared Value_Layout := record
  SALT29.StrType val  {maxlength(200000)};
	unsigned cnt;
	REAL pcnt;
	end;
...
EXPORT EclRecord(DATASET(ResultLine_Layout) d, STRING RName='Record_Layout') := FUNCTION
	...
	r1 WriteEcl(t le) := TRANSFORM
		...
		SELF.Txt := '  '+MAP( ~le.AllNum => 'UNICODE' + IF(le.MaxLen<MaxFixedLen,(SALT29.StrType)le.MaxLen, ''),
#else
		SELF.Txt := '  '+MAP( ~le.AllNum => 'STRING' + IF(le.MaxLen<MaxFixedLen,(SALT29.StrType)le.MaxLen, ''),
#end
...
EXPORT FieldTypes(DATASET(ResultLine_Layout) d, REAL Recall = 99.9) := FUNCTION
  PruneOutliers(file,cnt_field) := FUNCTIONMACRO
	...
	ENDMACRO;
  SALT29.StrType GetChars(DATASET(Character_Layout) gc) := FUNCTION
	  T := TABLE(SORT(PruneOutliers(gc(),cnt),c),{ SALT29.StrType s := c; });
		RETURN ROLLUP(t,TRUE,TRANSFORM(RECORDOF(T),SELF.S := LEFT.S+RIGHT.S))[1].s;
	END;
	
	SALT29.StrType LenList(DATASET(Length_Layout) lens) := FUNCTION
	  T := TABLE(PruneOutliers(lens,cnt),{ SALT29.StrType s := ','+len; });
		s := ROLLUP(t,TRUE,TRANSFORM(RECORDOF(T),SELF.S := LEFT.S+RIGHT.S))[1].s[2..];
	  RETURN ':LENGTHS('+s+')';
	END;
	SALT29.StrType WordList(DATASET(Words_Layout) wds) := FUNCTION
	  T := TABLE(PruneOutliers(wds,cnt),{ SALT29.StrType s := ','+words; });
		s := ROLLUP(t,TRUE,TRANSFORM(RECORDOF(T),SELF.S := LEFT.S+RIGHT.S))[1].s[2..];
	  RETURN ':WORDS('+s+')';
	END;
	...
	R := RECORD
	  d.FldNo;
	  d.FieldName;
		SALT29.StrType Allow := ':ALLOW('+GetChars(d.Characters)+')';
		SALT29.StrType LTrim := IF ( EXISTS(d.patterns(data_pattern[1]=' ',TRIM(data_pattern[2..])<>'')),'',':LEFTTRIM');
		SALT29.StrType Lens := LenList(d.len);
		SALT29.StrType Words := WordList(d.words);
		SALT29.StrType Spaces := ':SPACES( )';
	END;
	t := TABLE(d,r);
	r1 := RECORD
	  t.FldNo;
...
glbl_field_cnts := record
  FldInv1;
	unsigned2 xx_l := length(trim((SALT29.StrType)FldInv1.Fld));
	unsigned2 xx_wc := SALT29.WordCount((SALT29.StrType)FldInv1.Fld);
  end;
...
card_rec := RECORD
  FldInv.FldNo;
	...
	SALT29.Str30Type MaxVal30 := MAX(GROUP,FldInv.Fld);
	SALT29.Str30Type MinVal30 := MIN(GROUP,IF(FldInv.Fld='','ZZ--BOGUSVALUE--ZZ',FldInv.Fld));
  END;	
...
r tr(FldInv le,unsigned co) := transform
  self.c := ((SALT29.StrType)le.Fld)[co];
	self := le;
  end;
...
dr := record
	  SALT29.StrType Fld  {maxlength(200000)} := SALT29.fn_data_pattern(FldInv.Fld);
		FldInv.Cnt;
		FldInv.FldNo;
  end;
t := table(FldInv,dr);

...
r tr(nif s_in, unsigned2 c) := transform
  self.pos := c;
	self.s := SALT29.GetNthWord(s_in.instring,c);
	self.xxx_unc := s_in.xxx_unc;
  end;
	
// Split out the words for each wordbag
n := NORMALIZE(	nif, SALT29.WordCount(LEFT.instring), tr(LEFT,COUNTER) );
r2 := RECORD(r)
  INTEGER2 spec;
	END;
...
j1 := IF ( COUNT(keyname)*SIZEOF(pk,max) < SALT29.Config.LookupMaxSize, j1s, j1b_1+j1b_2 ); // Use a pure lookup if you can
j0 := JOIN( n, keyname, LEFT.s = RIGHT.keyfield, tr2(LEFT,RIGHT), LEFT OUTER );
j := IF(AsIndex,j0,j1);
ifp addw(ifp le, j ri) := TRANSFORM
  SELF.new_instring := TRIM(le.new_instring)+' '+TRIM(ri.s)+' '+(SALT29.StrType)ri.spec;
...
TYPEOF(infile) sb(sf le) := TRANSFORM
  SELF.new_instring := IF(le.instring='','',(SALT29.StrType)le.tot_spec+le.new_instring); // Newstring has space pre-pended by %addw%
  SELF := le;
  END;

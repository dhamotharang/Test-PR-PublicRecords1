...
EXPORT Summary(SALT311.Str30Type  txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    populated_rawfields_description_cnt := COUNT(GROUP,h.rawfields_description <> (TYPEOF(h.rawfields_description))'');
    populated_rawfields_description_pcnt := AVE(GROUP,IF(h.rawfields_description = (TYPEOF(h.rawfields_description))'',0,100));
    maxlength_rawfields_description := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.rawfields_description)));
    avelength_rawfields_description := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.rawfields_description)),h.rawfields_description<>(typeof(h.rawfields_description))'');
...
SHARED FldIds := DATASET([{1,'dotid'}
      ...
      ,{84,'clean_dates_entrydate'}
      ,{85,'clean_dates_lastupdate'}
      ,{86,'clean_phones_phone'}
      ,{87,'global_sid'}
      ,{88,'record_sid'}],SALT311.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Companies_Fields.InValid_rawfields_description((SALT311.StrType)le.rawfields_description),

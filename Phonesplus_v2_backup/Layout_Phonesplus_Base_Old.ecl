export Layout_Phonesplus_Base_Old := RECORD
  boolean in_flag;
  unsigned8 rules;
  string3 npa;
  string7 phone7;
  string7 phone10;
  data16 phone7_rec_key;
  data16 phone7_did_key;
  unsigned6 pdid;
  unsigned6 did;
  unsigned1 did_score;
  unsigned3 dt_first_seen;
  unsigned3 dt_last_seen;
  unsigned3 dt_vendor_last_reported;
  unsigned3 dt_vendor_first_reported;
  unsigned3 dt_nonglb_last_seen;
  string1 glb_dppa_flag;
  string glb_dppa_all;
  unsigned8 src;
  unsigned8 src_cnt;
  unsigned8 append_avg_source_conf;
  unsigned8 append_max_source_conf;
  unsigned8 append_min_source_conf;
  unsigned8 append_total_source_conf;
  unsigned8 append_velocity;
  unsigned3 orig_dt_last_seen;
  string10 did_type;
  string90 orig_name;
  string25 orig_address1;
  string25 orig_address2;
  string25 orig_address3;
  string20 orig_city;
  string2 orig_state;
  string9 orig_zip;
  string10 orig_phone;
  string8 orig_dob;
  string10 orig_agegroup;
  string8 orig_gender;
  string50 orig_email;
  string orig_listing_type;
  string orig_publish_code;
  string orig_phone_type;
  string orig_phone_usage;
  string80 orig_company;
  unsigned3 orig_phone_reg_dt;
  string20 orig_carrier_code;
  string60 orig_carrier_name;
  string orig_conf_score;
  unsigned1 orig_rec_type;
  string100 clean_company;
  string10 prim_range;
  string2 predir;
  string28 prim_name;
  string4 addr_suffix;
  string2 postdir;
  string10 unit_desig;
  string8 sec_range;
  string25 p_city_name;
  string25 v_city_name;
  string2 state;
  string5 zip5;
  string4 zip4;
  string4 cart;
  string1 cr_sort_sz;
  string4 lot;
  string1 lot_order;
  string2 dpbc;
  string1 chk_digit;
  string2 rec_type;
  string2 ace_fips_st;
  string3 ace_fips_county;
  string10 geo_lat;
  string11 geo_long;
  string4 msa;
  string7 geo_blk;
  string1 geo_match;
  string4 err_stat;
  string5 title;
  string20 fname;
  string20 mname;
  string20 lname;
  string5 name_suffix;
  unsigned1 name_score;
  unsigned6 bdid;
  unsigned1 bdid_score;
  unsigned8 append_npa_effective_dt;
  unsigned8 append_npa_last_change_dt;
  string1 append_dialable_ind;
  string30 append_place_name;
  string1 append_portability_indicator;
  string append_prior_area_code;
  unsigned8 append_nonpublished_match;
  string append_ocn;
  string append_time_zone;
  string2 append_nxx_type;
  string3 append_coctype;
  string4 append_scc;
  string append_phone_type;
  string append_company_type;
  string append_phone_use;
  string5 agreg_listing_type;
  unsigned1 max_orig_conf_score;
  unsigned1 min_orig_conf_score;
  unsigned1 cur_orig_conf_score;
  boolean eda_active_flag;
  unsigned8 eda_match;
  unsigned4 eda_phone_dt;
  unsigned4 eda_did_dt;
  unsigned4 eda_nm_addr_dt;
  unsigned8 eda_hist_match;
  unsigned4 eda_hist_phone_dt;
  unsigned4 eda_hist_did_dt;
  unsigned4 eda_hist_nm_addr_dt;
  string1 append_feedback_phone;
  unsigned4 append_feedback_phone_dt;
  string1 append_feedback_phone7_did;
  unsigned4 append_feedback_phone7_did_dt;
  string1 append_feedback_phone7_nm_addr;
  unsigned4 append_feedback_phone7_nm_addr_dt;
  unsigned8 append_ported_match;
  boolean append_seen_once_ind;
  unsigned8 append_indiv_phone_cnt;
  boolean append_indiv_has_active_eda_phone_flag;
  boolean append_latest_phone_owner_flag;
  unsigned6 hhid;
  unsigned1 hhid_score;
  data16 phone7_hhid_key;
  boolean append_best_addr_match_flag;
  boolean append_best_nm_match_flag;
  unsigned8 cleanaid;
  string1 addresstype;
  string182 previous_cleanaddress;
 END;

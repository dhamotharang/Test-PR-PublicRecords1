...
Layout_In_Phonesplus.Layout_In_Common t_map_common_layout(phone_f input) := Transform
	self.dt_first_seen 				:= map(input.addr_type = 'O' and (unsigned3) _validate.date.fCorrectedDateString(input.effective_dt) [1..6] > 0 =>  (unsigned3) _validate.date.fCorrectedDateString(input.effective_dt) [1..6],
	...
	self.dt_last_seen 				:= map(input.addr_type = 'O' and (unsigned3) _validate.date.fCorrectedDateString(input.last_activity_dt) [1..6] > 0 =>  (unsigned3) _validate.date.fCorrectedDateString(input.last_activity_dt) [1..6],
	...
	self.src						:= translation_codes.source_bitmap_code('IF');

import phonesplus,_control,ut,watchdog;
...
//phones_plus_file    := dataset(ut.foreign_prod+ '~thor_data400::base::phonesplusv2_20100809', phonesplus_v2.Layout_Phonesplus_Base, thor);
//phones_plus_file    := dataset(ut.foreign_prod+ '~thor_data400::base::phonesplusv2_20100816', phonesplus_v2.Layout_Phonesplus_Base, thor);
	
ds_phonesplus := distribute(phones_plus_file (did <> 0 and current_rec =true),HASH((INTEGER)did)); 
...
// string v_process_date := '20100817';
 Temp_rec := record
...
Temp_rec Append_dob_ssn (ds_phonesplus l	,recordof(Filebest) r) :=
		transform
		self.EXTERNAL_ID    := StringLib.Data2String(l.phone7_rec_key);
		self.SSN   			    := r.ssn;
		self.DOB_MONTH      := MAP( regexfind('^([0-9]{8})$',trim(l.orig_dob))   =>l.orig_dob[5..6],
		                           trim((string) r.dob[5..6]) not in ['0','00'] => (string) r.dob[5..6],
															 '');
    self.DOB_DAY        := MAP( regexfind('^([0-9]{8})$',trim(l.orig_dob))  =>l.orig_dob[7..8],
		                           trim((string) r.dob[7..8]) not in ['0','00'] => (string) r.dob[7..8] ,
															 '');
		self.DOB_YEAR       := MAP( regexfind('^([0-9]{8})$',trim(l.orig_dob))   =>l.orig_dob[1..4],
    ...
    self.PAGER                := Map( pager ='Y' => l.phone10,
		                                  '');
																			
    self.FAX_DATA             := Map( FAX_DATA ='Y' => l.phone10,
		                                  '');
    ...
    string home               := Map( cell ='Y' or PAGER ='Y' or FAX_DATA ='Y' or WORK ='Y' => '',
		...
		self.HOME_PHONE_NUMBER    := Map(HOME ='Y' => l.phone7,
																		 ...
																		 '');
    self.other                := Map(	cell ='' and PAGER ='' and FAX_DATA ='' and WORK ='' and home ='' => l.phone10,
		                                  '');
		self.LINK_ID              := l.did;
...
// export Promonitor_despray :=  sequential(out_phonesplus,phonesplus_d);  
 ut.MAC_SF_BuildProcess(phonesplus_ds( street_1 <>'' and city <> '' and state <> '' and postal_code<> '' and 
                                       (ssn <> '' or dob_month+dob_day+dob_year <> '') ),'~thor_Data400::out::Promonitor::PhonesPlus',OutPhoneplus,3);
 
 //sort_phonesplus_ds := Sort(phonesplus_ds,record,local);
 old_phonesplus_ds  := distribute(dataset('~thor_Data400::out::Promonitor::PhonesPlus_father', Temp_rec, thor),HASH(LAST_NAME,FIRST_NAME,MIDDLE_NAME,SSN,DOB_MONTH,DOB_DAY,DOB_YEAR)) ; 
 new_phonesplus_ds  := distribute(dataset('~thor_Data400::out::Promonitor::PhonesPlus'       , Temp_rec, thor),HASH(LAST_NAME,FIRST_NAME,MIDDLE_NAME,SSN,DOB_MONTH,DOB_DAY,DOB_YEAR)) ; 
// comb_phonesplus_ds := Sort(new_phonesplus_ds+old_phonesplus_ds,link_id,external_id,last_name,first_name,middle_name,process_date,local);
	comb_phonesplus_ds := If (FileServices.GetSuperFileSubCount('~thor_Data400::out::Promonitor::PhonesPlus_father') <> 0, 
...
output_phonesplus := output(out_changedrecords,
 ...
 phonesplus_d := fileservices.despray('~thor_Data400::out::promonitor::PhonesPlusExtract_'+v_process_date, DestinationIP, '/hds_180/pro_monitor/build/phonesplus/'+'PhonesPlusExtract_'+v_process_date+'.txt',,,,TRUE); 

//output(phonesplus_ds(Link_id = 23686));
export Proc_build_Promonitor_extract := sequential(OutPhoneplus,
                                                   output_phonesplus,phonesplus_d);
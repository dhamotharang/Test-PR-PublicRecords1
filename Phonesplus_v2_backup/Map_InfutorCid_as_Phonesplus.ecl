...
//G=GLB, D=DPPA, B=Both, ''=Neither, 'U'=Utility
	...
	self.src						:= translation_codes.source_bitmap_code('IC') | translation_codes.source_bitmap_code(hdr_source);
	self.append_avg_source_conf 	:= phonesplus_v2.Translation_Codes.source_conf('IC');
	...
	self.orig_name					:= StringLib.StringCleanSpaces(input.fname + ' ' + input.mname + ' ' + input.lname);
	self.orig_address1				:= orig_addr1;
	self.orig_address2				:= orig_addr2;
...
//************Fields appended by address cleaner.  Appended addresses don't have those fields
	...
	self.phone7_did_key         	:= hashmd5((data)input.phone [length(input.phone) - 6 ..length(input.phone)] + (unsigned)self.did);
	self 							:= input; 

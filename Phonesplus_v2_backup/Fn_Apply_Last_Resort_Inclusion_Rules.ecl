...
latest_include_flagged := dedup(sort(distribute(last_resort(in_flag and cur_orig_conf_score <> 4), hash(npa+phone7)), npa+phone7,-append_latest_phone_owner_flag, -dt_last_seen, local), npa+phone7, local);
...
cellphones_no_dates := project(phplus_in(append_phone_type = 'CELL' and dt_last_seen = 0 and ~Translation_Codes.fFlagIsOn(rules, Translation_Codes.rules_bitmap_code('Low-Vendor-Conf'))),

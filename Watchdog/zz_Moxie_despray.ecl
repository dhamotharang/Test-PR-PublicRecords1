...
best_file := map(var1='nonglb'=>watchdog.file_best_nonglb,
...
//-- This one needs to be converted
    ...
    self.vehicle_change := if(fieldChange(r.vehicle_vehnum) and r.vehicle_vehnum[1..2]!='MO','Y','N');
...
setDates := record
	...
	string8	vehicle_date := max(group,if(~fieldChange(delta_history.vehicle_vehnum) or delta_history.vehicle_vehnum[1..2]='MO',EarliestDate,convert_date8(delta_history.run_date)));

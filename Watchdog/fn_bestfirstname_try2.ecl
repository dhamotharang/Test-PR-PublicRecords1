...
export fn_bestfirstname_try2(dataset(recordof(header.layout_header)) fhf, string pst_sfx) := function

f0  := fhf;
bfn := watchdog.fn_bestfirstname_try1(fhf, pst_sfx)(fname = '');
...
bls1 := watchdog.fn_bestlastname_try1(fhf,pst_sfx);
...
p1 := project(sorted_duped(fname <> ''),into_out(LEFT))
      +
	  watchdog.fn_bestfirstname_try1(fhf, pst_sfx)(fname <> '')
	  ;

p1_pst := p1 : Persist('persist::Watchdog_BestFirstName_try2'+pst_sfx);

outf := if(NoPersist, p1, p1_pst);

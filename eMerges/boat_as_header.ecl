...
export	boat_as_header(dataset(layout_boats_in) peMergesBoats = dataset([],layout_boats_in), boolean pForHeaderBuild=false)
 :=
  function
	deMergesBoatsAsSource	:=	if(pForHeaderBuild,header.Files_SeqdSrc().BO,eMerges.Boat_as_Source(peMergesBoats,pForHeaderBuild));

	header.Layout_New_Records header_trans(deMergesBoatsAsSource L, integer c) := transform
		self.did := 0;
		self.rid := 0;
		self.pflag1 := '';
		self.pflag2 := '';
	...
	Boat_norm := NORMALIZE(deMergesBoatsAsSource, 8, header_trans(LEFT, COUNTER));
	Boat_Filtered := Boat_norm(lname != '' and prim_name != '' and zip != '' and length(trim(fname)) > 1);

    return Boat_Filtered;
  end
 ;

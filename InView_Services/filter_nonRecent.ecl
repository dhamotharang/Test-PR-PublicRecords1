...
isLien(string desc) := stringlib.stringFind(stringlib.stringToUpperCase(desc), 'LIEN',1)>0 or 
											stringlib.stringFind(stringlib.stringToUpperCase(desc), 'WARRANT',1)>0 or
											stringlib.stringFind(stringlib.stringToUpperCase(desc), 'TAX',1)>0;
											
isPaid(string status, string desc) := (status = '' and desc = '') or
																			stringlib.stringFind(stringlib.stringToUpperCase(desc), 'SATISFIED',1)>0 or
																			stringlib.stringFind(stringlib.stringToUpperCase(desc), 'RELEASED',1)>0 or
																			stringlib.stringFind(stringlib.stringToUpperCase(desc), 'SETTLED',1)>0;

// sometimes a released lien is indicated in the filing_type rather than the filing_status (FEDERAL TAX LIEN RELEASE)
isReleased(string desc) := stringlib.stringFind(stringlib.stringToUpperCase(desc),'RELEASE',1)>0;
											 
isRemoved(string desc) := stringlib.stringFind(stringlib.stringToUpperCase(desc), 'REMOVED',1)>0 or
											 stringlib.stringFind(stringlib.stringToUpperCase(desc), 'DISMISSED',1)>0 or
											 stringlib.stringFind(stringlib.stringToUpperCase(desc), 'TRANSFER',1)>0;


comb_layout := recordof(doxie_cbrs.all_base_records_prs().liens_judgments_v2);

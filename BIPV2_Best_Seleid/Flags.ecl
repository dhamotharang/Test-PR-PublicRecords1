...
// Append flags to the regular file
  TYPEOF(ih) NoteFlags(ih le,b.BestBy_Seleid_Best ri) := TRANSFORM
    ...
    SELF := le;
  END;
  j := JOIN(ih,b.BestBy_Seleid_Best,LEFT.Seleid=RIGHT.Seleid,NoteFlags(LEFT,RIGHT),LEFT OUTER,HASH);
SHARED In_Flagged0 := SORT(DISTRIBUTE(j,HASH(Seleid)),Seleid,LOCAL);
...
// Append flags to the input file
  TYPEOF(h) NoteFlags(h le,b.BestBy_Seleid_Best ri) := TRANSFORM
    SELF.company_name_flag := MAP ( ri.company_name  IN SET(s.nulls_company_name,company_name) => SALT30.Flags.Null,
      le.company_name  IN SET(s.nulls_company_name,company_name) => SALT30.Flags.Missing,
      le.company_name = ri.company_name => SALT30.Flags.Equal,
      SALT30.Flags.Bad);
    ...
    SELF := le;
  END;
  j := JOIN(h,b.BestBy_Seleid_Best,LEFT.Seleid=RIGHT.Seleid,NoteFlags(LEFT,RIGHT),LEFT OUTER,HASH);
SHARED Input_Flagged0 := SORT(DISTRIBUTE(j,HASH(Seleid)),Seleid,LOCAL);

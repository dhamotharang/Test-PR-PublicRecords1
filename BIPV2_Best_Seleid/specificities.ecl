...
EXPORT input_layout := RECORD // project out required fields
  SALT30.UIDType Seleid := h.Seleid; // using existing id field
  h.rcid;//RIDfield 
...
EXPORT input_file := h0 : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::Specificities_Cache',EXPIRE(Config.PersistExpire));
//We have Seleid specified - so we can compute statistics on the cluster counts
  r0 := RECORD
    input_file.Seleid;
    UNSIGNED6 InCluster := COUNT(GROUP);
  end;
EXPORT ClusterSizes := TABLE(input_file,r0,Seleid,LOCAL) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::Cluster_Sizes',EXPIRE(Config.PersistExpire));
EXPORT TotalClusters := COUNT(ClusterSizes);
 
EXPORT  company_name_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,company_name) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::company_name',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(company_name_deduped,company_name,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
...
EXPORT  company_fein_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,company_fein) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::company_fein',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(company_fein_deduped,company_fein,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
...
EXPORT  company_phone_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,company_phone) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::company_phone',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(company_phone_deduped,company_phone,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  company_url_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,company_url) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::company_url',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(company_url_deduped,company_url,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  duns_number_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,duns_number) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::duns_number',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(duns_number_deduped,duns_number,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  company_sic_code1_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,company_sic_code1) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::company_sic_code1',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(company_sic_code1_deduped,company_sic_code1,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  company_naics_code1_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,company_naics_code1) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::company_naics_code1',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(company_naics_code1_deduped,company_naics_code1,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  dba_name_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,dba_name) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::dba_name',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(dba_name_deduped,dba_name,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  prim_range_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,prim_range) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::prim_range',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(prim_range_deduped,prim_range,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  predir_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,predir) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::predir',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(predir_deduped,predir,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  prim_name_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,prim_name) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::prim_name',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(prim_name_deduped,prim_name,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  addr_suffix_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,addr_suffix) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::addr_suffix',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(addr_suffix_deduped,addr_suffix,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  postdir_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,postdir) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::postdir',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(postdir_deduped,postdir,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  unit_desig_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,unit_desig) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::unit_desig',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(unit_desig_deduped,unit_desig,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  sec_range_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,sec_range) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::sec_range',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(sec_range_deduped,sec_range,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  p_city_name_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,p_city_name) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::p_city_name',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(p_city_name_deduped,p_city_name,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  v_city_name_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,v_city_name) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::v_city_name',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(v_city_name_deduped,v_city_name,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  st_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,st) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::st',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(st_deduped,st,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  zip_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,zip) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::zip',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(zip_deduped,zip,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  zip4_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,zip4) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::zip4',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(zip4_deduped,zip4,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  fips_state_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,fips_state) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::fips_state',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(fips_state_deduped,fips_state,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  fips_county_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,fips_county) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::fips_county',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(fips_county_deduped,fips_county,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
EXPORT  address_deduped := SALT30.MAC_Field_By_UID(input_file,Seleid,address) : PERSIST('~temp::Seleid::BIPV2_Best_Seleid::dedups::address',EXPIRE(Config.PersistExpire)); // Reduce to field values by UID
  SALT30.Mac_Field_Count_UID(address_deduped,address,Seleid,counted,counted_clusters) // count the number of UIDs with each field value
  r1 := RECORD
    counted;
    UNSIGNED4 id := 0; // Used to identify value later
  end;
...
SALT30.MAC_Field_Bfoul(company_name_deduped,company_name,Seleid,company_name_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(company_fein_deduped,company_fein,Seleid,company_fein_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(company_phone_deduped,company_phone,Seleid,company_phone_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(company_url_deduped,company_url,Seleid,company_url_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(duns_number_deduped,duns_number,Seleid,duns_number_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(company_sic_code1_deduped,company_sic_code1,Seleid,company_sic_code1_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(company_naics_code1_deduped,company_naics_code1,Seleid,company_naics_code1_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(dba_name_deduped,dba_name,Seleid,dba_name_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(prim_range_deduped,prim_range,Seleid,prim_range_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(predir_deduped,predir,Seleid,predir_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(prim_name_deduped,prim_name,Seleid,prim_name_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(addr_suffix_deduped,addr_suffix,Seleid,addr_suffix_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(postdir_deduped,postdir,Seleid,postdir_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(unit_desig_deduped,unit_desig,Seleid,unit_desig_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(sec_range_deduped,sec_range,Seleid,sec_range_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(p_city_name_deduped,p_city_name,Seleid,p_city_name_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(v_city_name_deduped,v_city_name,Seleid,v_city_name_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
EXPORT st_nulls := nv;
SALT30.MAC_Field_Bfoul(st_deduped,st,Seleid,st_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
EXPORT st_switch := bf;
...
EXPORT zip_nulls := nv;
SALT30.MAC_Field_Bfoul(zip_deduped,zip,Seleid,zip_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(zip4_deduped,zip4,Seleid,zip4_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(fips_state_deduped,fips_state,Seleid,fips_state_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(fips_county_deduped,fips_county,Seleid,fips_county_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SALT30.MAC_Field_Bfoul(address_deduped,address,Seleid,address_nulls,ClusterSizes,false,false,bf) // Compute the chances of a field having 2 values for one entity
...
SpcShiftR := RECORD
  ...
  integer2 company_name_switch_shift0 := ROUND(1000*Specificities[1].company_name_switch - 371);
  integer1 company_fein_shift0 := ROUND(Specificities[1].company_fein_specificity - 27);
  integer2 company_fein_switch_shift0 := ROUND(1000*Specificities[1].company_fein_switch - 136);
  integer1 company_phone_shift0 := ROUND(Specificities[1].company_phone_specificity - 26);
  integer2 company_phone_switch_shift0 := ROUND(1000*Specificities[1].company_phone_switch - 295);
  integer1 company_url_shift0 := ROUND(Specificities[1].company_url_specificity - 27);
  integer2 company_url_switch_shift0 := ROUND(1000*Specificities[1].company_url_switch - 27);
  ...
  integer2 dba_name_switch_shift0 := ROUND(1000*Specificities[1].dba_name_switch - 148);
  integer1 prim_range_shift0 := ROUND(Specificities[1].prim_range_specificity - 13);
  integer2 prim_range_switch_shift0 := ROUND(1000*Specificities[1].prim_range_switch - 111);
  integer1 predir_shift0 := ROUND(Specificities[1].predir_specificity - 4);
  integer2 predir_switch_shift0 := ROUND(1000*Specificities[1].predir_switch - 56);
  integer1 prim_name_shift0 := ROUND(Specificities[1].prim_name_specificity - 15);
  integer2 prim_name_switch_shift0 := ROUND(1000*Specificities[1].prim_name_switch - 128);
  integer1 addr_suffix_shift0 := ROUND(Specificities[1].addr_suffix_specificity - 3);
  integer2 addr_suffix_switch_shift0 := ROUND(1000*Specificities[1].addr_suffix_switch - 89);
  integer1 postdir_shift0 := ROUND(Specificities[1].postdir_specificity - 7);
  integer2 postdir_switch_shift0 := ROUND(1000*Specificities[1].postdir_switch - 42);
  integer1 unit_desig_shift0 := ROUND(Specificities[1].unit_desig_specificity - 5);
  integer2 unit_desig_switch_shift0 := ROUND(1000*Specificities[1].unit_desig_switch - 64);
  integer1 sec_range_shift0 := ROUND(Specificities[1].sec_range_specificity - 12);
  integer2 sec_range_switch_shift0 := ROUND(1000*Specificities[1].sec_range_switch - 167);
  integer1 p_city_name_shift0 := ROUND(Specificities[1].p_city_name_specificity - 12);
  integer2 p_city_name_switch_shift0 := ROUND(1000*Specificities[1].p_city_name_switch - 100);
  integer1 v_city_name_shift0 := ROUND(Specificities[1].v_city_name_specificity - 11);
  integer2 v_city_name_switch_shift0 := ROUND(1000*Specificities[1].v_city_name_switch - 77);
  integer1 st_shift0 := ROUND(Specificities[1].st_specificity - 5);
  integer2 st_switch_shift0 := ROUND(1000*Specificities[1].st_switch - 16);
  integer1 zip_shift0 := ROUND(Specificities[1].zip_specificity - 14);
  integer2 zip_switch_shift0 := ROUND(1000*Specificities[1].zip_switch - 99);
  integer1 zip4_shift0 := ROUND(Specificities[1].zip4_specificity - 13);
  integer2 zip4_switch_shift0 := ROUND(1000*Specificities[1].zip4_switch - 177);
  integer1 fips_state_shift0 := ROUND(Specificities[1].fips_state_specificity - 5);
  integer2 fips_state_switch_shift0 := ROUND(1000*Specificities[1].fips_state_switch - 16);
  integer1 fips_county_shift0 := ROUND(Specificities[1].fips_county_specificity - 7);
  integer2 fips_county_switch_shift0 := ROUND(1000*Specificities[1].fips_county_switch - 89);
  integer1 address_shift0 := ROUND(Specificities[1].address_specificity - 25);
  integer2 address_switch_shift0 := ROUND(1000*Specificities[1].address_switch - 226);
  END;

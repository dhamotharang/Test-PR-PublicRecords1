...
// First step is to get all of the data slimmed and row-reduced
EXPORT BestDbaNameLegal_tab_ := DISTRIBUTE(TABLE(h(Seleid <> 0),{Seleid,source_for_votes,data_permits,dba_name,UNSIGNED Row_Cnt := COUNT(GROUP)},Seleid,source_for_votes,data_permits,dba_name,MERGE),HASH(Seleid)); // Slim and reduce row-count
  Slim := TABLE(BestDbaNameLegal_tab_(dba_name NOT IN SET(s.nulls_dba_name,dba_name),fn_valid_cname(TRIM((SALT30.StrType)dba_name),source_for_votes)),{Seleid,source_for_votes,data_permits,dba_name,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
// Voting is supported by altering the Row Count to be 100 * the (real) vote returned from the attribute
  Voted := TABLE( BestDbaNameLegal_tab_dba_name,{Seleid,data_permits,dba_name,UNSIGNED Row_Cnt := 100 * fn_Best_Name_Legal_Votes(source_for_votes,Row_Cnt)}); // Use fn_Best_Name_Legal_Votes to vote
  Voted TR(Voted le, Voted ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value
  grp := GROUP( BestDbaNameLegal_vote_dba_name,Seleid,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
EXPORT BestDbaNameLegal_method_dba_name := cmn;
 
//Create those fields with BestType: BestDbaNameVotedSeleLevel
// First step is to get all of the data slimmed and row-reduced
EXPORT BestDbaNameVotedSeleLevel_tab_ := DISTRIBUTE(TABLE(h(Seleid <> 0),{Seleid,source_for_votes,data_permits,dba_name,UNSIGNED Row_Cnt := COUNT(GROUP)},Seleid,source_for_votes,data_permits,dba_name,MERGE),HASH(Seleid)); // Slim and reduce row-count
  Slim := TABLE(BestDbaNameVotedSeleLevel_tab_(dba_name NOT IN SET(s.nulls_dba_name,dba_name),fn_valid_cname(TRIM((SALT30.StrType)dba_name),source_for_votes)),{Seleid,source_for_votes,data_permits,dba_name,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
  END;
EXPORT BestDbaNameVotedSeleLevel_tab_dba_name := ROLLUP( SORT(Slim,EXCEPT Row_Cnt,data_permits,LOCAL),TR(LEFT,RIGHT), EXCEPT Row_Cnt,data_permits,LOCAL);
// Voting is supported by altering the Row Count to be 100 * the (real) vote returned from the attribute
  Voted := TABLE( BestDbaNameVotedSeleLevel_tab_dba_name,{Seleid,data_permits,dba_name,UNSIGNED Row_Cnt := 100 * fn_Best_Sele_Level_Votes(source_for_votes,Row_Cnt)}); // Use fn_Best_Sele_Level_Votes to vote
  Voted TR(Voted le, Voted ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
  END;
EXPORT BestDbaNameVotedSeleLevel_vote_dba_name := ROLLUP( SORT(Voted,EXCEPT Row_Cnt,data_permits,LOCAL),TR(LEFT,RIGHT), EXCEPT Row_Cnt,data_permits,LOCAL);
//Now actually find the best value
  grp := GROUP( BestDbaNameVotedSeleLevel_vote_dba_name,Seleid,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  SALT30.MAC_Apply_Threshold(srt,200,o);
EXPORT BestDbaNameVotedSeleLevel_method_dba_name := o;
 
//Create those fields with BestType: BestDbaNameCommon
// First step is to get all of the data slimmed and row-reduced
EXPORT BestDbaNameCommon_tab_ := DISTRIBUTE(TABLE(h(Seleid <> 0),{Seleid,source_for_votes,data_permits,dba_name,UNSIGNED Row_Cnt := COUNT(GROUP)},Seleid,source_for_votes,data_permits,dba_name,MERGE),HASH(Seleid)); // Slim and reduce row-count
  Slim := TABLE(BestDbaNameCommon_tab_(dba_name NOT IN SET(s.nulls_dba_name,dba_name),fn_valid_cname(TRIM((SALT30.StrType)dba_name),source_for_votes)),{Seleid,source_for_votes,data_permits,dba_name,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value enforcing minimum
  grp := GROUP( BestDbaNameCommon_tab_dba_name,Seleid,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
...
// First step is to get all of the data slimmed and row-reduced
EXPORT BestDbaNameCurrent_tab_ := DISTRIBUTE(TABLE(h(Seleid <> 0),{Seleid,source_for_votes,data_permits,dba_name,UNSIGNED Early_Date := MIN(GROUP,IF(dt_last_seen=0,99999999,dt_last_seen)),UNSIGNED Late_Date := MAX(GROUP,dt_last_seen),UNSIGNED Row_Cnt := COUNT(GROUP)},Seleid,source_for_votes,data_permits,dba_name,MERGE),HASH(Seleid)); // Slim and reduce row-count
  Slim := TABLE(BestDbaNameCurrent_tab_(dba_name NOT IN SET(s.nulls_dba_name,dba_name),fn_valid_cname(TRIM((SALT30.StrType)dba_name),source_for_votes)),{Seleid,source_for_votes,data_permits,dba_name,Early_Date,Late_Date,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
//Now actually find the best value enforcing minimum
EXPORT BestDbaNameCurrent_method_dba_name := DEDUP( SORT( BestDbaNameCurrent_tab_dba_name(Late_Date>0,Early_Date<99999999),Seleid,-Late_Date,-Early_Date,LOCAL)(Row_Cnt >= 2),Seleid,LOCAL);
 
//Create those fields with BestType: BestDbaNameVoted
// First step is to get all of the data slimmed and row-reduced
EXPORT BestDbaNameVoted_tab_ := DISTRIBUTE(TABLE(h(Seleid <> 0),{Seleid,source_for_votes,data_permits,dba_name,UNSIGNED Row_Cnt := COUNT(GROUP)},Seleid,source_for_votes,data_permits,dba_name,MERGE),HASH(Seleid)); // Slim and reduce row-count
  Slim := TABLE(BestDbaNameVoted_tab_(dba_name NOT IN SET(s.nulls_dba_name,dba_name),fn_valid_cname(TRIM((SALT30.StrType)dba_name),source_for_votes)),{Seleid,source_for_votes,data_permits,dba_name,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
// Voting is supported by altering the Row Count to be 100 * the (real) vote returned from the attribute
  Voted := TABLE( BestDbaNameVoted_tab_dba_name,{Seleid,data_permits,dba_name,UNSIGNED Row_Cnt := 100 * fn_Best_Name_Source_Votes(source_for_votes,Row_Cnt)}); // Use fn_Best_Name_Source_Votes to vote
  Voted TR(Voted le, Voted ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value
  grp := GROUP( BestDbaNameVoted_vote_dba_name,Seleid,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  SALT30.MAC_Apply_Threshold(srt,200,o);
...
// First step is to get all of the data slimmed and row-reduced
EXPORT BestDbaNameLength_tab_ := DISTRIBUTE(TABLE(h(Seleid <> 0),{Seleid,source_for_votes,data_permits,dba_name,UNSIGNED Row_Cnt := COUNT(GROUP)},Seleid,source_for_votes,data_permits,dba_name,MERGE),HASH(Seleid)); // Slim and reduce row-count
  Slim := TABLE(BestDbaNameLength_tab_(dba_name NOT IN SET(s.nulls_dba_name,dba_name),fn_valid_cname(TRIM((SALT30.StrType)dba_name),source_for_votes)),{Seleid,source_for_votes,data_permits,dba_name,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value
EXPORT BestDbaNameLength_method_dba_name := DEDUP( SORT( BestDbaNameLength_tab_dba_name,Seleid,-LENGTH(TRIM((SALT30.StrType)dba_name)),-Row_Cnt,LOCAL),Seleid,LOCAL);
 
//Create those fields with BestType: BestDbaNameStrong
// First step is to get all of the data slimmed and row-reduced
EXPORT BestDbaNameStrong_tab_ := DISTRIBUTE(TABLE(h(Seleid <> 0),{Seleid,source_for_votes,data_permits,dba_name,UNSIGNED Row_Cnt := COUNT(GROUP)},Seleid,source_for_votes,data_permits,dba_name,MERGE),HASH(Seleid)); // Slim and reduce row-count
  Slim := TABLE(BestDbaNameStrong_tab_(dba_name NOT IN SET(s.nulls_dba_name,dba_name),fn_valid_cname(TRIM((SALT30.StrType)dba_name),source_for_votes)),{Seleid,source_for_votes,data_permits,dba_name,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value
  grp := GROUP( BestDbaNameStrong_tab_dba_name,Seleid,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
// A best solution is unique if its Row_Cnt is the same as the sum of the row_cnts.
  Totals := TABLE(GROUP(grp),{Seleid,Tot := SUM(GROUP,Row_Cnt)},Seleid);
export BestDbaNameStrong_method_dba_name := JOIN( cmn,Totals,left.Seleid = right.Seleid AND LEFT.Row_Cnt = RIGHT.Tot,TRANSFORM(LEFT),LOCAL);
 
//Create those fields with BestType: BestDbaNameCurrent2
// First step is to get all of the data slimmed and row-reduced
EXPORT BestDbaNameCurrent2_tab_ := DISTRIBUTE(TABLE(h(Seleid <> 0),{Seleid,source_for_votes,data_permits,dba_name,UNSIGNED Early_Date := MIN(GROUP,IF(dt_last_seen=0,99999999,dt_last_seen)),UNSIGNED Late_Date := MAX(GROUP,dt_last_seen),UNSIGNED Row_Cnt := COUNT(GROUP)},Seleid,source_for_votes,data_permits,dba_name,MERGE),HASH(Seleid)); // Slim and reduce row-count
  Slim := TABLE(BestDbaNameCurrent2_tab_(dba_name NOT IN SET(s.nulls_dba_name,dba_name),fn_valid_cname(TRIM((SALT30.StrType)dba_name),source_for_votes)),{Seleid,source_for_votes,data_permits,dba_name,Early_Date,Late_Date,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
//Now actually find the best value
EXPORT BestDbaNameCurrent2_method_dba_name := DEDUP( SORT( BestDbaNameCurrent2_tab_dba_name(Late_Date>0,Early_Date<99999999),Seleid,-Late_Date,-Early_Date,LOCAL),Seleid,LOCAL);
 
//Create those fields with BestType: BestDbaNameVotedUnrestricted
// First step is to get all of the data slimmed and row-reduced
EXPORT BestDbaNameVotedUnrestricted_tab_ := DISTRIBUTE(TABLE(h(Seleid <> 0),{Seleid,source_for_votes,data_permits,dba_name,UNSIGNED Row_Cnt := COUNT(GROUP)},Seleid,source_for_votes,data_permits,dba_name,MERGE),HASH(Seleid)); // Slim and reduce row-count
  Slim := TABLE(BestDbaNameVotedUnrestricted_tab_(dba_name NOT IN SET(s.nulls_dba_name,dba_name),fn_valid_cname(TRIM((SALT30.StrType)dba_name),source_for_votes)),{Seleid,source_for_votes,data_permits,dba_name,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
// Voting is supported by altering the Row Count to be 100 * the (real) vote returned from the attribute
  Voted := TABLE( BestDbaNameVotedUnrestricted_tab_dba_name,{Seleid,data_permits,dba_name,UNSIGNED Row_Cnt := 100 * fn_Best_Source_Unrestricted_Votes(source_for_votes,Row_Cnt)}); // Use fn_Best_Source_Unrestricted_Votes to vote
  Voted TR(Voted le, Voted ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value
  grp := GROUP( BestDbaNameVotedUnrestricted_vote_dba_name,Seleid,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  SALT30.MAC_Apply_Threshold(srt,200,o);
...
// 44 - Gathering type:BestDbaNameLegal Entries:1
  R44 := RECORD
    J43; // The data so far
  ...
  END;
  R44 T44(J43 le,BestDbaNameLegal_method_dba_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J44 := JOIN(J43,BestDbaNameLegal_method_dba_name,LEFT.Seleid = RIGHT.Seleid,T44(LEFT,RIGHT),FULL OUTER,LOCAL);
// 45 - Gathering type:BestDbaNameVotedSeleLevel Entries:1
  R45 := RECORD
    J44; // The data so far
    TYPEOF(BestDbaNameVotedSeleLevel_method_dba_name.dba_name) BestDbaNameVotedSeleLevel_dba_name;
    UNSIGNED dba_name_BestDbaNameVotedSeleLevel_Row_Cnt;
    UNSIGNED2 dba_name_BestDbaNameVotedSeleLevel_data_permits;
  END;
  R45 T45(J44 le,BestDbaNameVotedSeleLevel_method_dba_name ri) := TRANSFORM
    SELF.BestDbaNameVotedSeleLevel_dba_name := ri.dba_name;
    SELF.dba_name_BestDbaNameVotedSeleLevel_Row_Cnt := ri.Row_Cnt;
    SELF.dba_name_BestDbaNameVotedSeleLevel_data_permits := ri.data_permits;
    BOOLEAN has_left := le.Seleid <> (TYPEOF(le.Seleid))''; // See if LHS is null
    SELF.Seleid := IF( has_left, le.Seleid, ri.Seleid );
    SELF := le;
  END;
  J45 := JOIN(J44,BestDbaNameVotedSeleLevel_method_dba_name,LEFT.Seleid = RIGHT.Seleid,T45(LEFT,RIGHT),FULL OUTER,LOCAL);
// 46 - Gathering type:BestDbaNameCommon Entries:1
  R46 := RECORD
    J45; // The data so far
  ...
  END;
  R46 T46(J45 le,BestDbaNameCommon_method_dba_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J46 := JOIN(J45,BestDbaNameCommon_method_dba_name,LEFT.Seleid = RIGHT.Seleid,T46(LEFT,RIGHT),FULL OUTER,LOCAL);
// 47 - Gathering type:BestDbaNameCurrent Entries:1
  R47 := RECORD
    J46; // The data so far
  ...
  END;
  R47 T47(J46 le,BestDbaNameCurrent_method_dba_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J47 := JOIN(J46,BestDbaNameCurrent_method_dba_name,LEFT.Seleid = RIGHT.Seleid,T47(LEFT,RIGHT),FULL OUTER,LOCAL);
// 48 - Gathering type:BestDbaNameVoted Entries:1
  R48 := RECORD
    J47; // The data so far
  ...
  END;
  R48 T48(J47 le,BestDbaNameVoted_method_dba_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J48 := JOIN(J47,BestDbaNameVoted_method_dba_name,LEFT.Seleid = RIGHT.Seleid,T48(LEFT,RIGHT),FULL OUTER,LOCAL);
// 49 - Gathering type:BestDbaNameLength Entries:1
  R49 := RECORD
    J48; // The data so far
  ...
  END;
  R49 T49(J48 le,BestDbaNameLength_method_dba_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J49 := JOIN(J48,BestDbaNameLength_method_dba_name,LEFT.Seleid = RIGHT.Seleid,T49(LEFT,RIGHT),FULL OUTER,LOCAL);
// 50 - Gathering type:BestDbaNameStrong Entries:1
  R50 := RECORD
    J49; // The data so far
  ...
  END;
  R50 T50(J49 le,BestDbaNameStrong_method_dba_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J50 := JOIN(J49,BestDbaNameStrong_method_dba_name,LEFT.Seleid = RIGHT.Seleid,T50(LEFT,RIGHT),FULL OUTER,LOCAL);
// 51 - Gathering type:BestDbaNameCurrent2 Entries:1
  R51 := RECORD
    J50; // The data so far
  ...
  END;
  R51 T51(J50 le,BestDbaNameCurrent2_method_dba_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J51 := JOIN(J50,BestDbaNameCurrent2_method_dba_name,LEFT.Seleid = RIGHT.Seleid,T51(LEFT,RIGHT),FULL OUTER,LOCAL);
// 52 - Gathering type:BestDbaNameVotedUnrestricted Entries:1
  R52 := RECORD
    J51; // The data so far
  ...
  END;
  R52 T52(J51 le,BestDbaNameVotedUnrestricted_method_dba_name ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J52 := JOIN(J51,BestDbaNameVotedUnrestricted_method_dba_name,LEFT.Seleid = RIGHT.Seleid,T52(LEFT,RIGHT),FULL OUTER,LOCAL);
EXPORT BestBy_Seleid_np := J52;
...
// Now gather some statistics to see how we did
  R := RECORD
    ...
    REAL8 BestDbaNameLegal_dba_name_pcnt := AVE(GROUP,IF(BestBy_Seleid.BestDbaNameLegal_dba_name=(typeof(BestBy_Seleid.BestDbaNameLegal_dba_name))'',0,100));
    UNSIGNED BestDbaNameLegal_dba_name_permit1_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLegal_data_permits&1<>0);
    UNSIGNED BestDbaNameLegal_dba_name_permit2_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLegal_data_permits&2<>0);
    UNSIGNED BestDbaNameLegal_dba_name_permit3_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLegal_data_permits&4<>0);
    UNSIGNED BestDbaNameLegal_dba_name_permit4_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLegal_data_permits&8<>0);
    UNSIGNED BestDbaNameLegal_dba_name_permit5_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLegal_data_permits&16<>0);
    UNSIGNED BestDbaNameLegal_dba_name_permit6_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLegal_data_permits&32<>0);
    UNSIGNED BestDbaNameLegal_dba_name_permit7_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLegal_data_permits&64<>0);
    UNSIGNED BestDbaNameLegal_dba_name_permit8_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLegal_data_permits&128<>0);
    UNSIGNED BestDbaNameLegal_dba_name_permit9_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLegal_data_permits&256<>0);
    UNSIGNED BestDbaNameLegal_dba_name_permit10_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLegal_data_permits&512<>0);
    REAL8 BestDbaNameVotedSeleLevel_dba_name_pcnt := AVE(GROUP,IF(BestBy_Seleid.BestDbaNameVotedSeleLevel_dba_name=(typeof(BestBy_Seleid.BestDbaNameVotedSeleLevel_dba_name))'',0,100));
    UNSIGNED BestDbaNameVotedSeleLevel_dba_name_permit1_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedSeleLevel_data_permits&1<>0);
    UNSIGNED BestDbaNameVotedSeleLevel_dba_name_permit2_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedSeleLevel_data_permits&2<>0);
    UNSIGNED BestDbaNameVotedSeleLevel_dba_name_permit3_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedSeleLevel_data_permits&4<>0);
    UNSIGNED BestDbaNameVotedSeleLevel_dba_name_permit4_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedSeleLevel_data_permits&8<>0);
    UNSIGNED BestDbaNameVotedSeleLevel_dba_name_permit5_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedSeleLevel_data_permits&16<>0);
    UNSIGNED BestDbaNameVotedSeleLevel_dba_name_permit6_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedSeleLevel_data_permits&32<>0);
    UNSIGNED BestDbaNameVotedSeleLevel_dba_name_permit7_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedSeleLevel_data_permits&64<>0);
    UNSIGNED BestDbaNameVotedSeleLevel_dba_name_permit8_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedSeleLevel_data_permits&128<>0);
    UNSIGNED BestDbaNameVotedSeleLevel_dba_name_permit9_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedSeleLevel_data_permits&256<>0);
    UNSIGNED BestDbaNameVotedSeleLevel_dba_name_permit10_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedSeleLevel_data_permits&512<>0);
    REAL8 BestDbaNameCommon_dba_name_pcnt := AVE(GROUP,IF(BestBy_Seleid.BestDbaNameCommon_dba_name=(typeof(BestBy_Seleid.BestDbaNameCommon_dba_name))'',0,100));
    UNSIGNED BestDbaNameCommon_dba_name_permit1_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCommon_data_permits&1<>0);
    UNSIGNED BestDbaNameCommon_dba_name_permit2_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCommon_data_permits&2<>0);
    UNSIGNED BestDbaNameCommon_dba_name_permit3_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCommon_data_permits&4<>0);
    UNSIGNED BestDbaNameCommon_dba_name_permit4_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCommon_data_permits&8<>0);
    UNSIGNED BestDbaNameCommon_dba_name_permit5_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCommon_data_permits&16<>0);
    UNSIGNED BestDbaNameCommon_dba_name_permit6_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCommon_data_permits&32<>0);
    UNSIGNED BestDbaNameCommon_dba_name_permit7_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCommon_data_permits&64<>0);
    UNSIGNED BestDbaNameCommon_dba_name_permit8_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCommon_data_permits&128<>0);
    UNSIGNED BestDbaNameCommon_dba_name_permit9_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCommon_data_permits&256<>0);
    UNSIGNED BestDbaNameCommon_dba_name_permit10_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCommon_data_permits&512<>0);
    REAL8 BestDbaNameCurrent_dba_name_pcnt := AVE(GROUP,IF(BestBy_Seleid.BestDbaNameCurrent_dba_name=(typeof(BestBy_Seleid.BestDbaNameCurrent_dba_name))'',0,100));
    UNSIGNED BestDbaNameCurrent_dba_name_permit1_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent_data_permits&1<>0);
    UNSIGNED BestDbaNameCurrent_dba_name_permit2_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent_data_permits&2<>0);
    UNSIGNED BestDbaNameCurrent_dba_name_permit3_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent_data_permits&4<>0);
    UNSIGNED BestDbaNameCurrent_dba_name_permit4_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent_data_permits&8<>0);
    UNSIGNED BestDbaNameCurrent_dba_name_permit5_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent_data_permits&16<>0);
    UNSIGNED BestDbaNameCurrent_dba_name_permit6_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent_data_permits&32<>0);
    UNSIGNED BestDbaNameCurrent_dba_name_permit7_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent_data_permits&64<>0);
    UNSIGNED BestDbaNameCurrent_dba_name_permit8_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent_data_permits&128<>0);
    UNSIGNED BestDbaNameCurrent_dba_name_permit9_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent_data_permits&256<>0);
    UNSIGNED BestDbaNameCurrent_dba_name_permit10_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent_data_permits&512<>0);
    REAL8 BestDbaNameVoted_dba_name_pcnt := AVE(GROUP,IF(BestBy_Seleid.BestDbaNameVoted_dba_name=(typeof(BestBy_Seleid.BestDbaNameVoted_dba_name))'',0,100));
    UNSIGNED BestDbaNameVoted_dba_name_permit1_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVoted_data_permits&1<>0);
    UNSIGNED BestDbaNameVoted_dba_name_permit2_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVoted_data_permits&2<>0);
    UNSIGNED BestDbaNameVoted_dba_name_permit3_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVoted_data_permits&4<>0);
    UNSIGNED BestDbaNameVoted_dba_name_permit4_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVoted_data_permits&8<>0);
    UNSIGNED BestDbaNameVoted_dba_name_permit5_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVoted_data_permits&16<>0);
    UNSIGNED BestDbaNameVoted_dba_name_permit6_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVoted_data_permits&32<>0);
    UNSIGNED BestDbaNameVoted_dba_name_permit7_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVoted_data_permits&64<>0);
    UNSIGNED BestDbaNameVoted_dba_name_permit8_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVoted_data_permits&128<>0);
    UNSIGNED BestDbaNameVoted_dba_name_permit9_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVoted_data_permits&256<>0);
    UNSIGNED BestDbaNameVoted_dba_name_permit10_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVoted_data_permits&512<>0);
    REAL8 BestDbaNameLength_dba_name_pcnt := AVE(GROUP,IF(BestBy_Seleid.BestDbaNameLength_dba_name=(typeof(BestBy_Seleid.BestDbaNameLength_dba_name))'',0,100));
    UNSIGNED BestDbaNameLength_dba_name_permit1_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLength_data_permits&1<>0);
    UNSIGNED BestDbaNameLength_dba_name_permit2_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLength_data_permits&2<>0);
    UNSIGNED BestDbaNameLength_dba_name_permit3_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLength_data_permits&4<>0);
    UNSIGNED BestDbaNameLength_dba_name_permit4_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLength_data_permits&8<>0);
    UNSIGNED BestDbaNameLength_dba_name_permit5_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLength_data_permits&16<>0);
    UNSIGNED BestDbaNameLength_dba_name_permit6_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLength_data_permits&32<>0);
    UNSIGNED BestDbaNameLength_dba_name_permit7_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLength_data_permits&64<>0);
    UNSIGNED BestDbaNameLength_dba_name_permit8_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLength_data_permits&128<>0);
    UNSIGNED BestDbaNameLength_dba_name_permit9_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLength_data_permits&256<>0);
    UNSIGNED BestDbaNameLength_dba_name_permit10_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameLength_data_permits&512<>0);
    REAL8 BestDbaNameStrong_dba_name_pcnt := AVE(GROUP,IF(BestBy_Seleid.BestDbaNameStrong_dba_name=(typeof(BestBy_Seleid.BestDbaNameStrong_dba_name))'',0,100));
    UNSIGNED BestDbaNameStrong_dba_name_permit1_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameStrong_data_permits&1<>0);
    UNSIGNED BestDbaNameStrong_dba_name_permit2_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameStrong_data_permits&2<>0);
    UNSIGNED BestDbaNameStrong_dba_name_permit3_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameStrong_data_permits&4<>0);
    UNSIGNED BestDbaNameStrong_dba_name_permit4_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameStrong_data_permits&8<>0);
    UNSIGNED BestDbaNameStrong_dba_name_permit5_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameStrong_data_permits&16<>0);
    UNSIGNED BestDbaNameStrong_dba_name_permit6_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameStrong_data_permits&32<>0);
    UNSIGNED BestDbaNameStrong_dba_name_permit7_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameStrong_data_permits&64<>0);
    UNSIGNED BestDbaNameStrong_dba_name_permit8_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameStrong_data_permits&128<>0);
    UNSIGNED BestDbaNameStrong_dba_name_permit9_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameStrong_data_permits&256<>0);
    UNSIGNED BestDbaNameStrong_dba_name_permit10_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameStrong_data_permits&512<>0);
    REAL8 BestDbaNameCurrent2_dba_name_pcnt := AVE(GROUP,IF(BestBy_Seleid.BestDbaNameCurrent2_dba_name=(typeof(BestBy_Seleid.BestDbaNameCurrent2_dba_name))'',0,100));
    UNSIGNED BestDbaNameCurrent2_dba_name_permit1_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent2_data_permits&1<>0);
    UNSIGNED BestDbaNameCurrent2_dba_name_permit2_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent2_data_permits&2<>0);
    UNSIGNED BestDbaNameCurrent2_dba_name_permit3_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent2_data_permits&4<>0);
    UNSIGNED BestDbaNameCurrent2_dba_name_permit4_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent2_data_permits&8<>0);
    UNSIGNED BestDbaNameCurrent2_dba_name_permit5_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent2_data_permits&16<>0);
    UNSIGNED BestDbaNameCurrent2_dba_name_permit6_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent2_data_permits&32<>0);
    UNSIGNED BestDbaNameCurrent2_dba_name_permit7_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent2_data_permits&64<>0);
    UNSIGNED BestDbaNameCurrent2_dba_name_permit8_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent2_data_permits&128<>0);
    UNSIGNED BestDbaNameCurrent2_dba_name_permit9_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent2_data_permits&256<>0);
    UNSIGNED BestDbaNameCurrent2_dba_name_permit10_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameCurrent2_data_permits&512<>0);
    REAL8 BestDbaNameVotedUnrestricted_dba_name_pcnt := AVE(GROUP,IF(BestBy_Seleid.BestDbaNameVotedUnrestricted_dba_name=(typeof(BestBy_Seleid.BestDbaNameVotedUnrestricted_dba_name))'',0,100));
    UNSIGNED BestDbaNameVotedUnrestricted_dba_name_permit1_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedUnrestricted_data_permits&1<>0);
    UNSIGNED BestDbaNameVotedUnrestricted_dba_name_permit2_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedUnrestricted_data_permits&2<>0);
    UNSIGNED BestDbaNameVotedUnrestricted_dba_name_permit3_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedUnrestricted_data_permits&4<>0);
    UNSIGNED BestDbaNameVotedUnrestricted_dba_name_permit4_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedUnrestricted_data_permits&8<>0);
    UNSIGNED BestDbaNameVotedUnrestricted_dba_name_permit5_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedUnrestricted_data_permits&16<>0);
    UNSIGNED BestDbaNameVotedUnrestricted_dba_name_permit6_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedUnrestricted_data_permits&32<>0);
    UNSIGNED BestDbaNameVotedUnrestricted_dba_name_permit7_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedUnrestricted_data_permits&64<>0);
    UNSIGNED BestDbaNameVotedUnrestricted_dba_name_permit8_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedUnrestricted_data_permits&128<>0);
    UNSIGNED BestDbaNameVotedUnrestricted_dba_name_permit9_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedUnrestricted_data_permits&256<>0);
    UNSIGNED BestDbaNameVotedUnrestricted_dba_name_permit10_cnt := COUNT(GROUP,BestBy_Seleid.dba_name_BestDbaNameVotedUnrestricted_data_permits&512<>0);
  END;
...
SHARED F_BestBy_Seleid(DATASET({BestBy_Seleid}) d) := FUNCTION
  ...
  R T(BestBy_Seleid le) := TRANSFORM
    ...
    SELF.dba_name_cases := DATASET([
        ...
        {le.BestDbaNameVotedSeleLevel_dba_name,le.dba_name_BestDbaNameVotedSeleLevel_data_permits,2},
        {le.BestDbaNameCommon_dba_name,le.dba_name_BestDbaNameCommon_data_permits,3},
        {le.BestDbaNameCurrent_dba_name,le.dba_name_BestDbaNameCurrent_data_permits,4},
        {le.BestDbaNameVoted_dba_name,le.dba_name_BestDbaNameVoted_data_permits,5},
        {le.BestDbaNameLength_dba_name,le.dba_name_BestDbaNameLength_data_permits,6},
        {le.BestDbaNameStrong_dba_name,le.dba_name_BestDbaNameStrong_data_permits,7},
        {le.BestDbaNameCurrent2_dba_name,le.dba_name_BestDbaNameCurrent2_data_permits,8},
        {le.BestDbaNameVotedUnrestricted_dba_name,le.dba_name_BestDbaNameVotedUnrestricted_data_permits,9}

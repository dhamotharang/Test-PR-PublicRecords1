...
// There are 53 best types that we can propogate from this basis which means we will not need to use this basis again
  Layout_WithPropVars T_BestCompanyNameLegal_53_0(Layout_WithPropVars le,Best(ih).BestBy_Seleid_best ri) := TRANSFORM
    ...
    SELF.dba_name_prop_method := MAP ( ri.dba_name_method = 0 OR le.dba_name_prop_method <> 0 OR le.dba_name = ri.dba_name => le.dba_name_prop_method, // No propogation
      ...
      ri.dba_name_method = 9 AND (SALT30.StrType)ri.dba_name[1..length(trim((SALT30.StrType)le.dba_name))] = (SALT30.StrType)le.dba_name  => 9, // This field is extensible
    ...
    SELF.dba_name := MAP ( le.dba_name_prop > 0 OR le.dba_name = ri.dba_name OR ri.dba_name  IN SET(s.nulls_dba_name,dba_name) => le.dba_name, // No propogation
      ...
      ri.dba_name_method = 9 AND (SALT30.StrType)ri.dba_name[1..length(trim((SALT30.StrType)le.dba_name))] = (SALT30.StrType)le.dba_name  => ri.dba_name, // This field is extensible
      le.dba_name);
    SELF.dba_name_prop := IF ( le.dba_name = SELF.dba_name, le.dba_name_prop,1); // Flag the propagation for sliceouts
    SELF := le;
  END;
SHARED P_BestCompanyNameLegal_53_0 := JOIN(with_props(Seleid <> 0),Best(ih).BestBy_Seleid_best, LEFT.Seleid = RIGHT.Seleid ,T_BestCompanyNameLegal_53_0(LEFT,RIGHT),LEFT OUTER,HASH)
                 + with_props(Seleid = 0);
...
PostPropCounts := RECORD
  ...
  REAL8 dba_name_method_9 := AVE(GROUP,IF(propogated.dba_name_prop_method = 9, 100, 0));
...
layout_candidates add_predir(layout_candidates le,Specificities(ih).predir_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
j21 := JOIN(j22,PULL(Specificities(ih).predir_values_persisted),LEFT.predir=RIGHT.predir,add_predir(LEFT,RIGHT,TRUE),LOOKUP,FEW,LEFT OUTER);
layout_candidates add_fips_state(layout_candidates le,Specificities(ih).fips_state_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
j20 := JOIN(j21,PULL(Specificities(ih).fips_state_values_persisted),LEFT.fips_state=RIGHT.fips_state,add_fips_state(LEFT,RIGHT,TRUE),LOOKUP,FEW,LEFT OUTER);
layout_candidates add_st(layout_candidates le,Specificities(ih).st_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
j19 := JOIN(j20,PULL(Specificities(ih).st_values_persisted),LEFT.st=RIGHT.st,add_st(LEFT,RIGHT,TRUE),LOOKUP,FEW,LEFT OUTER);
layout_candidates add_unit_desig(layout_candidates le,Specificities(ih).unit_desig_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
j18 := JOIN(j19,PULL(Specificities(ih).unit_desig_values_persisted),LEFT.unit_desig=RIGHT.unit_desig,add_unit_desig(LEFT,RIGHT,TRUE),LOOKUP,FEW,LEFT OUTER);

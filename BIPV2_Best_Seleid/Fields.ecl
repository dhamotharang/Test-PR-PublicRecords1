...
EXPORT UIDConsistency(infile) := FUNCTIONMACRO
  f := TABLE(infile,{Seleid,rcid}); // Consistent type independent of input
  m := MODULE
    r := RECORD
 ...
 // Children with two parents
  ...
  EXPORT rcid_Twoparents := DEDUP(JOIN(f,f,LEFT.rcid=RIGHT.rcid AND LEFT.Seleid>RIGHT.Seleid,TRANSFORM({SALT30.UIDType Seleid1,SALT30.UIDType rcid,SALT30.UIDType Seleid2},SELF.Seleid1:=LEFT.Seleid,SELF.Seleid2:=RIGHT.Seleid,SELF.rcid:=LEFT.rcid),HASH),WHOLE RECORD,ALL);

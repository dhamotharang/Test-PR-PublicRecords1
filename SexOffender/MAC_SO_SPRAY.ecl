...
export Mac_SO_Spray(sourceIP,M_file,O_file,MP_file,filedate,group_name ='\'thor400_92\'',email_target='\' \'') := 
macro
...
%recordsizeMain% :=18420;
%recordsizeOffense% :=1348;
...
%build_SO% := doxie_build.Proc_Build_SO_Search(filedate);
...
%build_img_all% := images.proc_build_all(filedate);
	...
	//Old Offense Layout
	ds_offenses	:= SexOffender.File_Offenses;

	oldOffenseLayout := RECORD
	,maxLength(200000)
	...
	oldOffenseLayout oldOffTran(ds_offenses l):= transform
	 self := l;
	end; 

	offenses_oldLayout := project(ds_offenses, oldOffTran(left));

// Do STRATA stats work
SexOffender.Out_File_Main_Stats_Population(SexOffender.file_Main
                                          ,offenses_oldLayout
										  ,Images.File_Images
										  ,filedate
										  ,strata_output)
...
oldMainLayout oldTran(SexOffender.file_Main l):= transform
 self := l;
end; 

ds_main_project := project(SexOffender.file_Main, oldTran(left));
...
%build_offenses_delimited% :=output(offenses_oldLayout
								    ,

...
EXPORT Functions := MODULE
  ...
  SHARED  getRelkeyFlag(doxie.IDataAccess modAccess) :=
                MAP(modAccess.isDirectMarketing() => Relationship.constants.RelKeyFlag.DM,
                    modAccess.isConsumer() => Relationship.constants.RelKeyFlag.D2C,
                    '');
  ...
  EXPORT GetRelationshipBySubjectLexId(DATASET(EmailService.Assorted_Layouts.layout_relationship) in_relat_pairs,
    ...
    Relkey_flag := getRelKeyFlag(mod_access);
    ...
    subject_relat_pairs := IF(has_possible_rels,
             Relationship.proc_GetRelationshipNeutral(posbl_subject_rels,
                                           ...
                                           HighConfidenceAssociates := TRUE,
                                           RelKeyFlag := Relkey_flag).result,
  ...
  EXPORT GetRelationshipInReverse(DATASET(EmailService.Assorted_Layouts.layout_relationship) in_rel_pairs,
    ...
    reverse_rels := Relationship.functions_getRelationship.convertFlatToNeutral(
                    PROJECT(Relationship.proc_GetRelationshipPairs(unknown_rels,
                                    ...
                                    HighConfidenceRelatives := TRUE,
                                    HighConfidenceAssociates := TRUE,
                                    RelKeyFlag := getRelKeyFlag(mod_access)).result,
  ...
  EXPORT AddEmailRelationshipInfo (DATASET(EmailService.Assorted_Layouts.layout_search_out) email_recs,
    ...
    modAccess := doxie.compliance.GetGlobalDataAccessModuleTranslated (AutoStandardI.GlobalModule());
    ...
    rels_by_subject_lexid := GetRelationshipBySubjectLexId(possible_rel_pairs, modAccess, include_2ndDegree_relatives:=TRUE);
    ...
    rels_by_lexid := IF(EXISTS(rels_by_subject_lexid(relationship = EmailService.Constants.NoRelationship, subject_lexId>0, identity_lexid>0)),
              GetRelationshipInReverse(rels_by_subject_lexid, modAccess), // we attempt to hit relationship index using Lexids in reverse - in some cases relationship is available in 1 direction only in index

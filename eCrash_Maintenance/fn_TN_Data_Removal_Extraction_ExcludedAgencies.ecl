...
EXPORT fn_TN_Data_Removal_Extraction_ExcludedAgencies(STRING Pdate = (STRING) STD.Date.CurrentDate(TRUE)) := FUNCTION
...
//Despray file to target LZ
  BigLineFormatDespray (DATASET(SequencingRecSummary) inputRecs, STRING attachmentName) := FUNCTION
	...
	FilePath := '/data/super_credit/ecrash/despray/tnremoval' + '/' + attachmentName; 
...
//Incident input file 
 incident := dataset(Data_Services.foreign_prod+'thor_data400::in::ecrash::incidnt_raw_new'
  ...
  tn_incident := incident(std.str.touppercase(TRIM(loss_state_abbr, ALL)) = 'TN' AND creation_date < '2019-03-12' AND work_type_id = '0'
                         AND std.str.touppercase(TRIM(source_id, ALL)) IN ['TM','TF'] AND agency_id IN ['1627940','1626500','1629790','1521612']) :PERSIST('~THOR::BASE::PERSIST::ECRASH::TN_DATA_REMOVAL::INCIDENT', EXPIRE(30), SINGLE);
												 
	//Incident deleted at the end
	// OUTPUT(tn_incident);
	// OUTPUT(COUNT(tn_incident));
	
	d_tn_incident := DISTRIBUTE(tn_incident(incident_id <> ''), HASH32(incident_id));
	unq_tn_incident := DEDUP(SORT(d_tn_incident, incident_id, -(sent_to_hpcc_datetime), LOCAL), incident_id, LOCAL)  :PERSIST('~THOR::BASE::PERSIST::ECRASH::TN_DATA_REMOVAL::UNIQUE_INCIDENTS', EXPIRE(30), SINGLE);

  SequencingRecSummary xformCtr_inc(tn_incident L, INTEGER CTR) := TRANSFORM
	   SELF.recid            	:= CTR;
	...
	END;
	ExtractData_inc          := PROJECT(tn_incident, xformCtr_inc(LEFT, COUNTER));
	ExtractHeaderRec_inc  		:= DATASET([{0, IncidentHeader}], SequencingRecSummary);
	FORMATTEDFINALA_inc 		  := BigLineFormatDespray(ExtractData_inc & ExtractHeaderRec_inc, 'TN_Ecrash_Incident.csv');
...
tn_vehicl := JOIN(DISTRIBUTE(vehicl, HASH32(incident_id)), unq_tn_incident,
								     TRIM(LEFT.incident_id, ALL) = TRIM(RIGHT.incident_id,ALL),
								     TRANSFORM(LEFT), LOCAL) :PERSIST('~THOR::BASE::PERSIST::ECRASH::TN_DATA_REMOVAL::VEHICLE', EXPIRE(30));	

// OUTPUT(tn_vehicl);
// OUTPUT(COUNT(tn_vehicl));

SequencingRecSummary xformCtr_veh(tn_vehicl L, INTEGER CTR) := TRANSFORM
	   SELF.recid            	:= CTR;
		 SELF.LINE             	:= TRIM(TRIM(L.Vehicle_ID, LEFT, RIGHT) + ',' +  
		                                TRIM(L.Creation_Date, LEFT, RIGHT) + ',' + 
																...
																, LEFT, RIGHT); 
	END;
	ExtractData_veh           := PROJECT(tn_vehicl, xformCtr_veh(LEFT, COUNTER));
	ExtractHeaderRec_veh  		:= DATASET([{0, VehicleHeader}], SequencingRecSummary);
	FORMATTEDFINALA_veh 		  := BigLineFormatDespray(ExtractData_veh & ExtractHeaderRec_veh, 'TN_Ecrash_Vehicle.csv');
  ...
  tn_persn := JOIN(DISTRIBUTE(persn, HASH32(incident_id)), unq_tn_incident,
								   TRIM(LEFT.incident_id, ALL) = TRIM(RIGHT.incident_id,ALL),
								   TRANSFORM(LEFT), LOCAL) :PERSIST('~THOR::BASE::PERSIST::ECRASH::TN_DATA_REMOVAL::PERSON', EXPIRE(30));	

	// OUTPUT(tn_persn);
	// OUTPUT(COUNT(tn_persn));

  SequencingRecSummary xformCtr_per(tn_persn L, INTEGER CTR) := TRANSFORM
	   SELF.recid            	:= CTR;
	...
	END;
	ExtractData_per           := PROJECT(tn_persn, xformCtr_per(LEFT, COUNTER));
	ExtractHeaderRec_per  		:= DATASET([{0, PersonHeader}], SequencingRecSummary);
	FORMATTEDFINALA_per 		  := BigLineFormatDespray(ExtractData_per & ExtractHeaderRec_per, 'TN_Ecrash_Person.csv');
	
	Write_Output_Files := SEQUENTIAL(FORMATTEDFINALA_inc, FORMATTEDFINALA_per, FORMATTEDFINALA_veh);

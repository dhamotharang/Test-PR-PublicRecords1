...
EXPORT getEmailInfo(dataset (MemberPoint.Layouts.BestExtended) dsBestE, 
		...
		EmailBatchIn := project(dsBestE,transform(BatchServices.Layouts.email.rec_batch_email_input,
						//unsigned8   seq := 0;
							...
							EmailBatchIn1 := project(dsBestE,transform(BatchServices.Layouts.email.rec_batch_email_input,
              self.acctno 			:= (string) left.seq,							
							self.did 					:= left.did, 
							self:=[];));
	...
	//Added flag to identify call from Memberpoint service
	  EmailBatchInput:=if(InputParams.UseDMEmailSourcesOnly,EmailBatchIn1,EmailBatchIn);
		modEmail := BatchServices.Email_BatchService_Records(EmailBatchParams,EmailBatchInput,false,emailforMemberpoint := True);

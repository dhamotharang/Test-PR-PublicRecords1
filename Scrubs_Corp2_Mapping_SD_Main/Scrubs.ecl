...
// The module to handle the case where no scrubs exist
  ...
  EXPORT Expanded_Layout := RECORD(Layout_In_File)
    ...
    UNSIGNED1 corp_farm_status_cd_Invalid;
    UNSIGNED1 corp_farm_status_desc_Invalid;
...
EXPORT FromNone(DATASET(Layout_In_File) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_farm_status_cd_Invalid := Fields.InValid_corp_farm_status_cd((SALT38.StrType)le.corp_farm_status_cd);
    SELF.corp_farm_status_desc_Invalid := Fields.InValid_corp_farm_status_desc((SALT38.StrType)le.corp_farm_status_desc);
    SELF.recordorigin_Invalid := Fields.InValid_recordorigin((SALT38.StrType)le.recordorigin);
    SELF.InternalField1_Invalid := Fields.InValid_InternalField1((SALT38.StrType)le.InternalField1);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Layout_In_File);
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_ln_name_type_desc_Invalid << 22 ) + ( le.corp_status_cd_Invalid << 23 ) + ( le.corp_status_desc_Invalid << 24 ) + ( le.corp_status_date_Invalid << 25 ) + ( le.corp_inc_state_Invalid << 27 ) + ( le.corp_inc_date_Invalid << 28 ) + ( le.corp_foreign_domestic_ind_Invalid << 30 ) + ( le.corp_forgn_state_cd_Invalid << 31 ) + ( le.corp_forgn_date_Invalid << 32 ) + ( le.corp_orig_org_structure_cd_Invalid << 34 ) + ( le.corp_for_profit_ind_Invalid << 35 ) + ( le.corp_agent_assign_date_Invalid << 36 ) + ( le.corp_farm_status_cd_Invalid << 38 ) + ( le.corp_farm_status_desc_Invalid << 39 ) + ( le.recordorigin_Invalid << 40 ) + ( le.InternalField1_Invalid << 41 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_status_desc_Invalid := (le.ScrubsBits1 >> 24) & 1;
    ...
    SELF.corp_foreign_domestic_ind_Invalid := (le.ScrubsBits1 >> 30) & 1;
    SELF.corp_forgn_state_cd_Invalid := (le.ScrubsBits1 >> 31) & 1;
    SELF.corp_forgn_date_Invalid := (le.ScrubsBits1 >> 32) & 3;
    SELF.corp_orig_org_structure_cd_Invalid := (le.ScrubsBits1 >> 34) & 1;
    SELF.corp_for_profit_ind_Invalid := (le.ScrubsBits1 >> 35) & 1;
    SELF.corp_agent_assign_date_Invalid := (le.ScrubsBits1 >> 36) & 3;
    SELF.corp_farm_status_cd_Invalid := (le.ScrubsBits1 >> 38) & 1;
    SELF.corp_farm_status_desc_Invalid := (le.ScrubsBits1 >> 39) & 1;
    SELF.recordorigin_Invalid := (le.ScrubsBits1 >> 40) & 1;
    SELF.InternalField1_Invalid := (le.ScrubsBits1 >> 41) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_farm_status_cd_ENUM_ErrorCount := COUNT(GROUP,h.corp_farm_status_cd_Invalid=1);
    corp_farm_status_desc_ENUM_ErrorCount := COUNT(GROUP,h.corp_farm_status_desc_Invalid=1);
    ...
    AnyRule_WithErrorsCount := COUNT(GROUP, h.dt_vendor_first_reported_Invalid > 0 OR h.dt_vendor_last_reported_Invalid > 0 OR h.dt_first_seen_Invalid > 0 OR h.dt_last_seen_Invalid > 0 OR h.corp_ra_dt_first_seen_Invalid > 0 OR h.corp_ra_dt_last_seen_Invalid > 0 OR h.corp_key_Invalid > 0 OR h.corp_vendor_Invalid > 0 OR h.corp_state_origin_Invalid > 0 OR h.corp_process_date_Invalid > 0 OR h.corp_orig_sos_charter_nbr_Invalid > 0 OR h.corp_legal_name_Invalid > 0 OR h.corp_ln_name_type_cd_Invalid > 0 OR h.corp_ln_name_type_desc_Invalid > 0 OR h.corp_status_cd_Invalid > 0 OR h.corp_status_desc_Invalid > 0 OR h.corp_status_date_Invalid > 0 OR h.corp_inc_state_Invalid > 0 OR h.corp_inc_date_Invalid > 0 OR h.corp_foreign_domestic_ind_Invalid > 0 OR h.corp_forgn_state_cd_Invalid > 0 OR h.corp_forgn_date_Invalid > 0 OR h.corp_orig_org_structure_cd_Invalid > 0 OR h.corp_for_profit_ind_Invalid > 0 OR h.corp_agent_assign_date_Invalid > 0 OR h.corp_farm_status_cd_Invalid > 0 OR h.corp_farm_status_desc_Invalid > 0 OR h.recordorigin_Invalid > 0 OR h.InternalField1_Invalid > 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_farm_status_cd_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.corp_farm_status_desc_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.recordorigin_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.InternalField1_Invalid,'ALLOW','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'dt_vendor_first_reported','dt_vendor_last_reported','dt_first_seen','dt_last_seen','corp_ra_dt_first_seen','corp_ra_dt_last_seen','corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_orig_sos_charter_nbr','corp_legal_name','corp_ln_name_type_cd','corp_ln_name_type_desc','corp_status_cd','corp_status_desc','corp_status_date','corp_inc_state','corp_inc_date','corp_foreign_domestic_ind','corp_forgn_state_cd','corp_forgn_date','corp_orig_org_structure_cd','corp_for_profit_ind','corp_agent_assign_date','corp_farm_status_cd','corp_farm_status_desc','recordorigin','InternalField1','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter','invalid_mandatory','invalid_name_type_code','invalid_name_type_desc','invalid_status_code','invalid_status_desc','invalid_date','invalid_state_origin','invalid_date','invalid_forgn_dom_code','invalid_alphablank','invalid_date','invalid_org_struc_cd','invalid_flag_code','invalid_date','invalid_FARM_status_code','invalid_FARM_status_desc','invalid_recordorigin','invalid_alphablank','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT38.StrType)le.dt_vendor_first_reported,(SALT38.StrType)le.dt_vendor_last_reported,(SALT38.StrType)le.dt_first_seen,(SALT38.StrType)le.dt_last_seen,(SALT38.StrType)le.corp_ra_dt_first_seen,(SALT38.StrType)le.corp_ra_dt_last_seen,(SALT38.StrType)le.corp_key,(SALT38.StrType)le.corp_vendor,(SALT38.StrType)le.corp_state_origin,(SALT38.StrType)le.corp_process_date,(SALT38.StrType)le.corp_orig_sos_charter_nbr,(SALT38.StrType)le.corp_legal_name,(SALT38.StrType)le.corp_ln_name_type_cd,(SALT38.StrType)le.corp_ln_name_type_desc,(SALT38.StrType)le.corp_status_cd,(SALT38.StrType)le.corp_status_desc,(SALT38.StrType)le.corp_status_date,(SALT38.StrType)le.corp_inc_state,(SALT38.StrType)le.corp_inc_date,(SALT38.StrType)le.corp_foreign_domestic_ind,(SALT38.StrType)le.corp_forgn_state_cd,(SALT38.StrType)le.corp_forgn_date,(SALT38.StrType)le.corp_orig_org_structure_cd,(SALT38.StrType)le.corp_for_profit_ind,(SALT38.StrType)le.corp_agent_assign_date,(SALT38.StrType)le.corp_farm_status_cd,(SALT38.StrType)le.corp_farm_status_desc,(SALT38.StrType)le.recordorigin,(SALT38.StrType)le.InternalField1,'***SALTBUG***');
  END;
  ...
  // field error stats
    SALT38.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_status_cd:invalid_status_code:ENUM'
          ...
          ,'corp_orig_org_structure_cd:invalid_org_struc_cd:ENUM'
          ...
          ,'corp_farm_status_cd:invalid_FARM_status_code:ENUM'
          ,'corp_farm_status_desc:invalid_FARM_status_desc:ENUM'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_corp_farm_status_cd(1)
          ,Fields.InvalidMessage_corp_farm_status_desc(1)
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_farm_status_cd_ENUM_ErrorCount
          ,le.corp_farm_status_desc_ENUM_ErrorCount
      ...
      SELF.rulepcnt := IF(c <= NumRules, 100 * CHOOSE(c
          ...
          ,le.corp_farm_status_desc_ENUM_ErrorCount
          ,le.recordorigin_ENUM_ErrorCount
          ,le.InternalField1_ALLOW_ErrorCount,0) / le.TotalCnt + 0.5, CHOOSE(c - NumRules
...
EXPORT StandardStats(DATASET(Layout_In_File) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  ...
  SALT38.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Corp2_Mapping_SD_Main, Fields, 'RECORDOF(scrubsSummaryOverall)', '');

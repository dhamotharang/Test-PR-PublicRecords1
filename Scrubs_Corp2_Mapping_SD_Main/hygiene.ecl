...
EXPORT Summary(SALT38.Str30Type  txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_InternalField1 := MAX(GROUP,LENGTH(TRIM((SALT38.StrType)h.InternalField1)));
    avelength_InternalField1 := AVE(GROUP,LENGTH(TRIM((SALT38.StrType)h.InternalField1)),h.InternalField1<>(typeof(h.InternalField1))'');
  END;
...
SHARED FldIds := DATASET([{1,'dt_vendor_first_reported'}
      ...
      ,{289,'recordorigin'}
      ,{290,'InternalField1'}],SALT38.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED2 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_InternalField1((SALT38.StrType)le.InternalField1),
    0);
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT38.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_Corp2_Mapping_SD_Main, Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

...
EXPORT Summary(SALT30.Str30Type txt,BOOLEAN Glob=TRUE) := FUNCTION
  ...
  R1 := RECORD
    UNSIGNED LinkingPotential :=  + T.Populated_UPIN_pcnt *   0.00 / 100 + T.Populated_NPI_NUMBER_pcnt *  22.00 / 100 + T.Populated_DEA_NUMBER_pcnt *  21.00 / 100 + T.Populated_CLIA_NUMBER_pcnt *  22.00 / 100 + T.Populated_MEDICARE_FACILITY_NUMBER_pcnt *  21.00 / 100 + T.Populated_NCPDP_NUMBER_pcnt *  22.00 / 100 + T.Populated_TAX_ID_pcnt *  21.00 / 100 + T.Populated_FEIN_pcnt *  17.00 / 100 + T.Populated_C_LIC_NBR_pcnt *  19.00 / 100 + T.Populated_SRC_pcnt *   0.00 / 100 + T.Populated_CNAME_pcnt *   0.00 / 100 + T.Populated_CNP_NAME_pcnt *  13.00 / 100 + T.Populated_CNP_NUMBER_pcnt *  13.00 / 100 + T.Populated_CNP_STORE_NUMBER_pcnt *  20.00 / 100 + T.Populated_CNP_BTYPE_pcnt *   4.00 / 100 + T.Populated_CNP_LOWV_pcnt *   0.00 / 100 + T.Populated_CNP_TRANSLATED_pcnt *   0.00 / 100 + T.Populated_CNP_CLASSID_pcnt *   0.00 / 100 + T.Populated_ADDRESS_ID_pcnt *   0.00 / 100 + T.Populated_ADDRESS_CLASSIFICATION_pcnt *   0.00 / 100 + T.Populated_PRIM_RANGE_pcnt *  12.00 / 100 + T.Populated_PRIM_NAME_pcnt *  13.00 / 100 + T.Populated_SEC_RANGE_pcnt *   7.00 / 100 + T.Populated_ST_pcnt *   5.00 / 100 + T.Populated_V_CITY_NAME_pcnt *  11.00 / 100 + T.Populated_ZIP_pcnt *  13.00 / 100 + T.Populated_PHONE_pcnt *  20.00 / 100 + T.Populated_FAX_pcnt *  20.00 / 100 + T.Populated_TAXONOMY_pcnt *   7.00 / 100 + T.Populated_TAXONOMY_CODE_pcnt *   4.00 / 100 + T.Populated_MEDICAID_NUMBER_pcnt *  22.00 / 100 + T.Populated_VENDOR_ID_pcnt *  21.00 / 100 + T.Populated_LIC_STATE_pcnt *   0.00 / 100;
    T;
  END;
  RETURN TABLE(T,R1);
END;
...
invRec invert(summary0 le, INTEGER C) := TRANSFORM
  ...
  SELF.FieldName := CHOOSE(C,'UPIN','NPI_NUMBER','DEA_NUMBER','CLIA_NUMBER','MEDICARE_FACILITY_NUMBER','NCPDP_NUMBER','TAX_ID','FEIN','C_LIC_NBR','SRC','CNAME','CNP_NAME','CNP_NUMBER','CNP_STORE_NUMBER','CNP_BTYPE','CNP_LOWV','CNP_TRANSLATED','CNP_CLASSID','ADDRESS_ID','ADDRESS_CLASSIFICATION','PRIM_RANGE','PRIM_NAME','SEC_RANGE','ST','V_CITY_NAME','ZIP','PHONE','FAX','TAXONOMY','TAXONOMY_CODE','MEDICAID_NUMBER','VENDOR_ID','LIC_STATE');
  SELF.populated_pcnt := CHOOSE(C,le.populated_UPIN_pcnt,le.populated_NPI_NUMBER_pcnt,le.populated_DEA_NUMBER_pcnt,le.populated_CLIA_NUMBER_pcnt,le.populated_MEDICARE_FACILITY_NUMBER_pcnt,le.populated_NCPDP_NUMBER_pcnt,le.populated_TAX_ID_pcnt,le.populated_FEIN_pcnt,le.populated_C_LIC_NBR_pcnt,le.populated_SRC_pcnt,le.populated_CNAME_pcnt,le.populated_CNP_NAME_pcnt,le.populated_CNP_NUMBER_pcnt,le.populated_CNP_STORE_NUMBER_pcnt,le.populated_CNP_BTYPE_pcnt,le.populated_CNP_LOWV_pcnt,le.populated_CNP_TRANSLATED_pcnt,le.populated_CNP_CLASSID_pcnt,le.populated_ADDRESS_ID_pcnt,le.populated_ADDRESS_CLASSIFICATION_pcnt,le.populated_PRIM_RANGE_pcnt,le.populated_PRIM_NAME_pcnt,le.populated_SEC_RANGE_pcnt,le.populated_ST_pcnt,le.populated_V_CITY_NAME_pcnt,le.populated_ZIP_pcnt,le.populated_PHONE_pcnt,le.populated_FAX_pcnt,le.populated_TAXONOMY_pcnt,le.populated_TAXONOMY_CODE_pcnt,le.populated_MEDICAID_NUMBER_pcnt,le.populated_VENDOR_ID_pcnt,le.populated_LIC_STATE_pcnt);
  SELF.maxlength := CHOOSE(C,le.maxlength_UPIN,le.maxlength_NPI_NUMBER,le.maxlength_DEA_NUMBER,le.maxlength_CLIA_NUMBER,le.maxlength_MEDICARE_FACILITY_NUMBER,le.maxlength_NCPDP_NUMBER,le.maxlength_TAX_ID,le.maxlength_FEIN,le.maxlength_C_LIC_NBR,le.maxlength_SRC,le.maxlength_CNAME,le.maxlength_CNP_NAME,le.maxlength_CNP_NUMBER,le.maxlength_CNP_STORE_NUMBER,le.maxlength_CNP_BTYPE,le.maxlength_CNP_LOWV,le.maxlength_CNP_TRANSLATED,le.maxlength_CNP_CLASSID,le.maxlength_ADDRESS_ID,le.maxlength_ADDRESS_CLASSIFICATION,le.maxlength_PRIM_RANGE,le.maxlength_PRIM_NAME,le.maxlength_SEC_RANGE,le.maxlength_ST,le.maxlength_V_CITY_NAME,le.maxlength_ZIP,le.maxlength_PHONE,le.maxlength_FAX,le.maxlength_TAXONOMY,le.maxlength_TAXONOMY_CODE,le.maxlength_MEDICAID_NUMBER,le.maxlength_VENDOR_ID,le.maxlength_LIC_STATE);
  SELF.avelength := CHOOSE(C,le.avelength_UPIN,le.avelength_NPI_NUMBER,le.avelength_DEA_NUMBER,le.avelength_CLIA_NUMBER,le.avelength_MEDICARE_FACILITY_NUMBER,le.avelength_NCPDP_NUMBER,le.avelength_TAX_ID,le.avelength_FEIN,le.avelength_C_LIC_NBR,le.avelength_SRC,le.avelength_CNAME,le.avelength_CNP_NAME,le.avelength_CNP_NUMBER,le.avelength_CNP_STORE_NUMBER,le.avelength_CNP_BTYPE,le.avelength_CNP_LOWV,le.avelength_CNP_TRANSLATED,le.avelength_CNP_CLASSID,le.avelength_ADDRESS_ID,le.avelength_ADDRESS_CLASSIFICATION,le.avelength_PRIM_RANGE,le.avelength_PRIM_NAME,le.avelength_SEC_RANGE,le.avelength_ST,le.avelength_V_CITY_NAME,le.avelength_ZIP,le.avelength_PHONE,le.avelength_FAX,le.avelength_TAXONOMY,le.avelength_TAXONOMY_CODE,le.avelength_MEDICAID_NUMBER,le.avelength_VENDOR_ID,le.avelength_LIC_STATE);
END;
...
SALT30.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.id := le.LNPID;
  SELF.Src := le.SRC;
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT30.StrType)le.UPIN),TRIM((SALT30.StrType)le.NPI_NUMBER),TRIM((SALT30.StrType)le.DEA_NUMBER),TRIM((SALT30.StrType)le.CLIA_NUMBER),TRIM((SALT30.StrType)le.MEDICARE_FACILITY_NUMBER),TRIM((SALT30.StrType)le.NCPDP_NUMBER),TRIM((SALT30.StrType)le.TAX_ID),TRIM((SALT30.StrType)le.FEIN),TRIM((SALT30.StrType)le.C_LIC_NBR),TRIM((SALT30.StrType)le.SRC),TRIM((SALT30.StrType)le.CNAME),TRIM((SALT30.StrType)le.CNP_NAME),TRIM((SALT30.StrType)le.CNP_NUMBER),TRIM((SALT30.StrType)le.CNP_STORE_NUMBER),TRIM((SALT30.StrType)le.CNP_BTYPE),TRIM((SALT30.StrType)le.CNP_LOWV),TRIM((SALT30.StrType)le.CNP_TRANSLATED),TRIM((SALT30.StrType)le.CNP_CLASSID),TRIM((SALT30.StrType)le.ADDRESS_ID),TRIM((SALT30.StrType)le.ADDRESS_CLASSIFICATION),TRIM((SALT30.StrType)le.PRIM_RANGE),TRIM((SALT30.StrType)le.PRIM_NAME),TRIM((SALT30.StrType)le.SEC_RANGE),TRIM((SALT30.StrType)le.ST),TRIM((SALT30.StrType)le.V_CITY_NAME),TRIM((SALT30.StrType)le.ZIP),TRIM((SALT30.StrType)le.PHONE),TRIM((SALT30.StrType)le.FAX),TRIM((SALT30.StrType)le.TAXONOMY),TRIM((SALT30.StrType)le.TAXONOMY_CODE),TRIM((SALT30.StrType)le.MEDICAID_NUMBER),TRIM((SALT30.StrType)le.VENDOR_ID),TRIM((SALT30.StrType)le.LIC_STATE)));
  SELF.FldNo := C;
END;
...
SALT30.MAC_Correlate.Data_Layout IntoP(h le,UNSIGNED C) := TRANSFORM
  SELF.FldNo1 := 1 + (C / 33);
  SELF.FldNo2 := 1 + (C % 33);
  SELF.Fld1 := TRIM(CHOOSE(SELF.FldNo1,TRIM((SALT30.StrType)le.UPIN),TRIM((SALT30.StrType)le.NPI_NUMBER),TRIM((SALT30.StrType)le.DEA_NUMBER),TRIM((SALT30.StrType)le.CLIA_NUMBER),TRIM((SALT30.StrType)le.MEDICARE_FACILITY_NUMBER),TRIM((SALT30.StrType)le.NCPDP_NUMBER),TRIM((SALT30.StrType)le.TAX_ID),TRIM((SALT30.StrType)le.FEIN),TRIM((SALT30.StrType)le.C_LIC_NBR),TRIM((SALT30.StrType)le.SRC),TRIM((SALT30.StrType)le.CNAME),TRIM((SALT30.StrType)le.CNP_NAME),TRIM((SALT30.StrType)le.CNP_NUMBER),TRIM((SALT30.StrType)le.CNP_STORE_NUMBER),TRIM((SALT30.StrType)le.CNP_BTYPE),TRIM((SALT30.StrType)le.CNP_LOWV),TRIM((SALT30.StrType)le.CNP_TRANSLATED),TRIM((SALT30.StrType)le.CNP_CLASSID),TRIM((SALT30.StrType)le.ADDRESS_ID),TRIM((SALT30.StrType)le.ADDRESS_CLASSIFICATION),TRIM((SALT30.StrType)le.PRIM_RANGE),TRIM((SALT30.StrType)le.PRIM_NAME),TRIM((SALT30.StrType)le.SEC_RANGE),TRIM((SALT30.StrType)le.ST),TRIM((SALT30.StrType)le.V_CITY_NAME),TRIM((SALT30.StrType)le.ZIP),TRIM((SALT30.StrType)le.PHONE),TRIM((SALT30.StrType)le.FAX),TRIM((SALT30.StrType)le.TAXONOMY),TRIM((SALT30.StrType)le.TAXONOMY_CODE),TRIM((SALT30.StrType)le.MEDICAID_NUMBER),TRIM((SALT30.StrType)le.VENDOR_ID),TRIM((SALT30.StrType)le.LIC_STATE)));
  SELF.Fld2 := TRIM(CHOOSE(SELF.FldNo2,TRIM((SALT30.StrType)le.UPIN),TRIM((SALT30.StrType)le.NPI_NUMBER),TRIM((SALT30.StrType)le.DEA_NUMBER),TRIM((SALT30.StrType)le.CLIA_NUMBER),TRIM((SALT30.StrType)le.MEDICARE_FACILITY_NUMBER),TRIM((SALT30.StrType)le.NCPDP_NUMBER),TRIM((SALT30.StrType)le.TAX_ID),TRIM((SALT30.StrType)le.FEIN),TRIM((SALT30.StrType)le.C_LIC_NBR),TRIM((SALT30.StrType)le.SRC),TRIM((SALT30.StrType)le.CNAME),TRIM((SALT30.StrType)le.CNP_NAME),TRIM((SALT30.StrType)le.CNP_NUMBER),TRIM((SALT30.StrType)le.CNP_STORE_NUMBER),TRIM((SALT30.StrType)le.CNP_BTYPE),TRIM((SALT30.StrType)le.CNP_LOWV),TRIM((SALT30.StrType)le.CNP_TRANSLATED),TRIM((SALT30.StrType)le.CNP_CLASSID),TRIM((SALT30.StrType)le.ADDRESS_ID),TRIM((SALT30.StrType)le.ADDRESS_CLASSIFICATION),TRIM((SALT30.StrType)le.PRIM_RANGE),TRIM((SALT30.StrType)le.PRIM_NAME),TRIM((SALT30.StrType)le.SEC_RANGE),TRIM((SALT30.StrType)le.ST),TRIM((SALT30.StrType)le.V_CITY_NAME),TRIM((SALT30.StrType)le.ZIP),TRIM((SALT30.StrType)le.PHONE),TRIM((SALT30.StrType)le.FAX),TRIM((SALT30.StrType)le.TAXONOMY),TRIM((SALT30.StrType)le.TAXONOMY_CODE),TRIM((SALT30.StrType)le.MEDICAID_NUMBER),TRIM((SALT30.StrType)le.VENDOR_ID),TRIM((SALT30.StrType)le.LIC_STATE)));
  END;
SHARED Pairs0 := NORMALIZE(ENTH(h,Config.CorrelateSampleSize),33*33,IntoP(LEFT,COUNTER))(FldNo1<FldNo2);
SHARED FldIds := DATASET([{1,'UPIN'}
      ,{2,'NPI_NUMBER'}
      ,{3,'DEA_NUMBER'}
      ,{4,'CLIA_NUMBER'}
      ,{5,'MEDICARE_FACILITY_NUMBER'}
      ,{6,'NCPDP_NUMBER'}
      ,{7,'TAX_ID'}
      ,{8,'FEIN'}
      ,{9,'C_LIC_NBR'}
      ...
      ,{31,'MEDICAID_NUMBER'}
      ,{32,'VENDOR_ID'}
      ,{33,'LIC_STATE'}],SALT30.MAC_Character_Counts.Field_Identification);

...
EXPORT SALT30.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'UPIN','NPI_NUMBER','DEA_NUMBER','CLIA_NUMBER','MEDICARE_FACILITY_NUMBER','NCPDP_NUMBER','TAX_ID','FEIN','C_LIC_NBR','SRC','CNAME','CNP_NAME','CNP_NUMBER','CNP_STORE_NUMBER','CNP_BTYPE','CNP_LOWV','CNP_TRANSLATED','CNP_CLASSID','ADDRESS_ID','ADDRESS_CLASSIFICATION','PRIM_RANGE','PRIM_NAME','SEC_RANGE','ST','V_CITY_NAME','ZIP','PHONE','FAX','TAXONOMY','TAXONOMY_CODE','MEDICAID_NUMBER','VENDOR_ID','FAC_NAME','ADDR1','LOCALE','ADDRESS','LIC_STATE');
EXPORT FieldNum(SALT30.StrType fn) := CASE(fn,'UPIN' => 1,'NPI_NUMBER' => 2,'DEA_NUMBER' => 3,'CLIA_NUMBER' => 4,'MEDICARE_FACILITY_NUMBER' => 5,'NCPDP_NUMBER' => 6,'TAX_ID' => 7,'FEIN' => 8,'C_LIC_NBR' => 9,'SRC' => 10,'CNAME' => 11,'CNP_NAME' => 12,'CNP_NUMBER' => 13,'CNP_STORE_NUMBER' => 14,'CNP_BTYPE' => 15,'CNP_LOWV' => 16,'CNP_TRANSLATED' => 17,'CNP_CLASSID' => 18,'ADDRESS_ID' => 19,'ADDRESS_CLASSIFICATION' => 20,'PRIM_RANGE' => 21,'PRIM_NAME' => 22,'SEC_RANGE' => 23,'ST' => 24,'V_CITY_NAME' => 25,'ZIP' => 26,'PHONE' => 27,'FAX' => 28,'TAXONOMY' => 29,'TAXONOMY_CODE' => 30,'MEDICAID_NUMBER' => 31,'VENDOR_ID' => 32,'FAC_NAME' => 33,'ADDR1' => 34,'LOCALE' => 35,'ADDRESS' => 36,'LIC_STATE' => 37,0);
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_UPIN,1,0)+ IF( SELF.Diff_NPI_NUMBER,1,0)+ IF( SELF.Diff_DEA_NUMBER,1,0)+ IF( SELF.Diff_CLIA_NUMBER,1,0)+ IF( SELF.Diff_MEDICARE_FACILITY_NUMBER,1,0)+ IF( SELF.Diff_NCPDP_NUMBER,1,0)+ IF( SELF.Diff_TAX_ID,1,0)+ IF( SELF.Diff_FEIN,1,0)+ IF( SELF.Diff_C_LIC_NBR,1,0)+ IF( SELF.Diff_SRC,1,0)+ IF( SELF.Diff_CNAME,1,0)+ IF( SELF.Diff_CNP_NAME,1,0)+ IF( SELF.Diff_CNP_NUMBER,1,0)+ IF( SELF.Diff_CNP_STORE_NUMBER,1,0)+ IF( SELF.Diff_CNP_BTYPE,1,0)+ IF( SELF.Diff_CNP_LOWV,1,0)+ IF( SELF.Diff_CNP_TRANSLATED,1,0)+ IF( SELF.Diff_CNP_CLASSID,1,0)+ IF( SELF.Diff_ADDRESS_ID,1,0)+ IF( SELF.Diff_ADDRESS_CLASSIFICATION,1,0)+ IF( SELF.Diff_PRIM_RANGE,1,0)+ IF( SELF.Diff_PRIM_NAME,1,0)+ IF( SELF.Diff_SEC_RANGE,1,0)+ IF( SELF.Diff_ST,1,0)+ IF( SELF.Diff_V_CITY_NAME,1,0)+ IF( SELF.Diff_ZIP,1,0)+ IF( SELF.Diff_PHONE,1,0)+ IF( SELF.Diff_FAX,1,0)+ IF( SELF.Diff_TAXONOMY,1,0)+ IF( SELF.Diff_TAXONOMY_CODE,1,0)+ IF( SELF.Diff_MEDICAID_NUMBER,1,0)+ IF( SELF.Diff_VENDOR_ID,1,0)+ IF( SELF.Diff_LIC_STATE,1,0);
  END;

...
EXPORT FromNone(DATASET(Layout_HealthFacility) h) := MODULE
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.UPIN_Invalid << 0 ) + ( le.NPI_NUMBER_Invalid << 2 ) + ( le.DEA_NUMBER_Invalid << 3 ) + ( le.CLIA_NUMBER_Invalid << 5 ) + ( le.MEDICARE_FACILITY_NUMBER_Invalid << 7 ) + ( le.NCPDP_NUMBER_Invalid << 9 ) + ( le.TAX_ID_Invalid << 11 ) + ( le.FEIN_Invalid << 13 ) + ( le.C_LIC_NBR_Invalid << 15 ) + ( le.SRC_Invalid << 17 ) + ( le.CNAME_Invalid << 19 ) + ( le.CNP_NAME_Invalid << 21 ) + ( le.CNP_NUMBER_Invalid << 23 ) + ( le.CNP_STORE_NUMBER_Invalid << 25 ) + ( le.CNP_BTYPE_Invalid << 27 ) + ( le.CNP_LOWV_Invalid << 29 ) + ( le.CNP_TRANSLATED_Invalid << 31 ) + ( le.CNP_CLASSID_Invalid << 33 ) + ( le.ADDRESS_ID_Invalid << 35 ) + ( le.ADDRESS_CLASSIFICATION_Invalid << 36 ) + ( le.PRIM_RANGE_Invalid << 38 ) + ( le.PRIM_NAME_Invalid << 40 ) + ( le.SEC_RANGE_Invalid << 42 ) + ( le.ST_Invalid << 44 ) + ( le.V_CITY_NAME_Invalid << 46 ) + ( le.ZIP_Invalid << 48 ) + ( le.PHONE_Invalid << 50 ) + ( le.FAX_Invalid << 52 ) + ( le.TAXONOMY_Invalid << 54 ) + ( le.TAXONOMY_CODE_Invalid << 56 ) + ( le.MEDICAID_NUMBER_Invalid << 58 ) + ( le.VENDOR_ID_Invalid << 60 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    SELF.UPIN_Invalid := (le.ScrubsBits1 >> 0) & 3;
    SELF.NPI_NUMBER_Invalid := (le.ScrubsBits1 >> 2) & 1;
    SELF.DEA_NUMBER_Invalid := (le.ScrubsBits1 >> 3) & 3;
    SELF.CLIA_NUMBER_Invalid := (le.ScrubsBits1 >> 5) & 3;
    SELF.MEDICARE_FACILITY_NUMBER_Invalid := (le.ScrubsBits1 >> 7) & 3;
    SELF.NCPDP_NUMBER_Invalid := (le.ScrubsBits1 >> 9) & 3;
    SELF.TAX_ID_Invalid := (le.ScrubsBits1 >> 11) & 3;
    SELF.FEIN_Invalid := (le.ScrubsBits1 >> 13) & 3;
    SELF.C_LIC_NBR_Invalid := (le.ScrubsBits1 >> 15) & 3;
    SELF.SRC_Invalid := (le.ScrubsBits1 >> 17) & 3;
    SELF.CNAME_Invalid := (le.ScrubsBits1 >> 19) & 3;
    SELF.CNP_NAME_Invalid := (le.ScrubsBits1 >> 21) & 3;
    SELF.CNP_NUMBER_Invalid := (le.ScrubsBits1 >> 23) & 3;
    SELF.CNP_STORE_NUMBER_Invalid := (le.ScrubsBits1 >> 25) & 3;
    SELF.CNP_BTYPE_Invalid := (le.ScrubsBits1 >> 27) & 3;
    SELF.CNP_LOWV_Invalid := (le.ScrubsBits1 >> 29) & 3;
    SELF.CNP_TRANSLATED_Invalid := (le.ScrubsBits1 >> 31) & 3;
    SELF.CNP_CLASSID_Invalid := (le.ScrubsBits1 >> 33) & 3;
    SELF.ADDRESS_ID_Invalid := (le.ScrubsBits1 >> 35) & 1;
    SELF.ADDRESS_CLASSIFICATION_Invalid := (le.ScrubsBits1 >> 36) & 3;
    SELF.PRIM_RANGE_Invalid := (le.ScrubsBits1 >> 38) & 3;
    SELF.PRIM_NAME_Invalid := (le.ScrubsBits1 >> 40) & 3;
    SELF.SEC_RANGE_Invalid := (le.ScrubsBits1 >> 42) & 3;
    SELF.ST_Invalid := (le.ScrubsBits1 >> 44) & 3;
    SELF.V_CITY_NAME_Invalid := (le.ScrubsBits1 >> 46) & 3;
    SELF.ZIP_Invalid := (le.ScrubsBits1 >> 48) & 3;
    SELF.PHONE_Invalid := (le.ScrubsBits1 >> 50) & 3;
    SELF.FAX_Invalid := (le.ScrubsBits1 >> 52) & 3;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.VENDOR_ID_Invalid,'LEFTTRIM','QUOTES','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'UPIN','NPI_NUMBER','DEA_NUMBER','CLIA_NUMBER','MEDICARE_FACILITY_NUMBER','NCPDP_NUMBER','TAX_ID','FEIN','C_LIC_NBR','SRC','CNAME','CNP_NAME','CNP_NUMBER','CNP_STORE_NUMBER','CNP_BTYPE','CNP_LOWV','CNP_TRANSLATED','CNP_CLASSID','ADDRESS_ID','ADDRESS_CLASSIFICATION','PRIM_RANGE','PRIM_NAME','SEC_RANGE','ST','V_CITY_NAME','ZIP','PHONE','FAX','TAXONOMY','TAXONOMY_CODE','MEDICAID_NUMBER','VENDOR_ID','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'DEFAULT','NUMBER','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','WORDBAG','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','NUMBER','DEFAULT','DEFAULT','DEFAULT','DEFAULT','ALPHA','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','DEFAULT','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT30.StrType)le.UPIN,(SALT30.StrType)le.NPI_NUMBER,(SALT30.StrType)le.DEA_NUMBER,(SALT30.StrType)le.CLIA_NUMBER,(SALT30.StrType)le.MEDICARE_FACILITY_NUMBER,(SALT30.StrType)le.NCPDP_NUMBER,(SALT30.StrType)le.TAX_ID,(SALT30.StrType)le.FEIN,(SALT30.StrType)le.C_LIC_NBR,(SALT30.StrType)le.SRC,(SALT30.StrType)le.CNAME,(SALT30.StrType)le.CNP_NAME,(SALT30.StrType)le.CNP_NUMBER,(SALT30.StrType)le.CNP_STORE_NUMBER,(SALT30.StrType)le.CNP_BTYPE,(SALT30.StrType)le.CNP_LOWV,(SALT30.StrType)le.CNP_TRANSLATED,(SALT30.StrType)le.CNP_CLASSID,(SALT30.StrType)le.ADDRESS_ID,(SALT30.StrType)le.ADDRESS_CLASSIFICATION,(SALT30.StrType)le.PRIM_RANGE,(SALT30.StrType)le.PRIM_NAME,(SALT30.StrType)le.SEC_RANGE,(SALT30.StrType)le.ST,(SALT30.StrType)le.V_CITY_NAME,(SALT30.StrType)le.ZIP,(SALT30.StrType)le.PHONE,(SALT30.StrType)le.FAX,(SALT30.StrType)le.TAXONOMY,(SALT30.StrType)le.TAXONOMY_CODE,(SALT30.StrType)le.MEDICAID_NUMBER,(SALT30.StrType)le.VENDOR_ID,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD()) := FUNCTION
    SALT30.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'VENDOR_ID:DEFAULT:LEFTTRIM','VENDOR_ID:DEFAULT:QUOTES','UNKNOWN');
      SELF.ErrorMessage := '';
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.VENDOR_ID_LEFTTRIM_ErrorCount,le.VENDOR_ID_QUOTES_ErrorCount,0);
      SELF.rulepcnt := 100 * CHOOSE(c
          ...
          ,le.TAXONOMY_CODE_LEFTTRIM_ErrorCount,le.TAXONOMY_CODE_QUOTES_ErrorCount
          ,le.MEDICAID_NUMBER_LEFTTRIM_ErrorCount,le.MEDICAID_NUMBER_QUOTES_ErrorCount
          ,le.VENDOR_ID_LEFTTRIM_ErrorCount,le.VENDOR_ID_QUOTES_ErrorCount,0) / le.TotalCnt + 0.5;
    END;

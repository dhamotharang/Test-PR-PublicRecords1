...
SHARED match_candidates(ih).layout_matches match_join(match_candidates(ih).layout_candidates le,match_candidates(ih).layout_candidates ri,UNSIGNED c=0,UNSIGNED outside=0) := TRANSFORM
  ...
  INTEGER2 V_CITY_NAME_score := MAP(
	...
	INTEGER2 TAXONOMY_SCORE_TEMP := fn_Taxonomy (le.TAXONOMY, ri.TAXONOMY, le.TAXONOMY_weight100); 
  ...
  INTEGER2 TAXONOMY_CODE_score_temp := MAP(
                        ...
                        le.TAXONOMY_CODE = ri.TAXONOMY_CODE  => le.TAXONOMY_CODE_weight100,
												TAXONOMY_SCORE_TEMP > 0 => le.TAXONOMY_CODE_weight100,
  ...
  INTEGER2 CNP_NAME_score := IF ( CNP_NAME_score_temp > Config.CNP_NAME_Force * 100 OR ( FEIN_score > Config.CNP_NAME_OR1_FEIN_Force*100) OR ( NPI_NUMBER_score > Config.CNP_NAME_OR2_NPI_NUMBER_Force*100) OR ( C_LIC_NBR_score > Config.CNP_NAME_OR3_C_LIC_NBR_Force*100) OR ( CLIA_NUMBER_score > Config.CNP_NAME_OR4_CLIA_NUMBER_Force*100) OR ( MEDICARE_FACILITY_NUMBER_score > Config.CNP_NAME_OR5_MEDICARE_FACILITY_NUMBER_Force*100) OR ( NCPDP_NUMBER_score > Config.CNP_NAME_OR6_NCPDP_NUMBER_Force*100) OR ( DEA_NUMBER_score > Config.CNP_NAME_OR7_DEA_NUMBER_Force*100) OR FAC_NAME_score_pre > 0, CNP_NAME_score_temp, SKIP ); // Enforce FORCE parameter
  INTEGER2 CNP_NUMBER_score := IF ( CNP_NUMBER_score_temp >= Config.CNP_NUMBER_Force * 100 OR ( FEIN_score > Config.CNP_NUMBER_OR1_FEIN_Force*100) OR ( NPI_NUMBER_score > Config.CNP_NUMBER_OR2_NPI_NUMBER_Force*100) OR ( CLIA_NUMBER_score > Config.CNP_NUMBER_OR3_CLIA_NUMBER_Force*100) OR ( C_LIC_NBR_score > Config.CNP_NUMBER_OR4_C_LIC_NBR_Force*100) OR ( MEDICARE_FACILITY_NUMBER_score > Config.CNP_NUMBER_OR5_MEDICARE_FACILITY_NUMBER_Force*100) OR ( NCPDP_NUMBER_score > Config.CNP_NUMBER_OR6_NCPDP_NUMBER_Force*100) OR ( DEA_NUMBER_score > Config.CNP_NUMBER_OR7_DEA_NUMBER_Force*100), CNP_NUMBER_score_temp, SKIP ); // Enforce FORCE parameter
  ...
  INTEGER2 SEC_RANGE_score_temp := MAP(
                        ...
                        0 /* switch0 */)*IF(ADDR1_score_scale=0,1,ADDR1_score_scale)*IF(ADDRESS_score_scale=0,1,ADDRESS_score_scale);
  INTEGER2 CNP_BTYPE_score := IF ( CNP_BTYPE_score_temp >= Config.CNP_BTYPE_Force * 100 OR ( FEIN_score > Config.CNP_BTYPE_OR1_FEIN_Force*100) OR ( NPI_NUMBER_score > Config.CNP_BTYPE_OR2_NPI_NUMBER_Force*100) OR ( C_LIC_NBR_score > Config.CNP_BTYPE_OR3_C_LIC_NBR_Force*100) OR ( CLIA_NUMBER_score > Config.CNP_BTYPE_OR4_CLIA_NUMBER_Force*100) OR ( MEDICARE_FACILITY_NUMBER_score > Config.CNP_BTYPE_OR5_MEDICARE_FACILITY_NUMBER_Force*100) OR ( NCPDP_NUMBER_score > Config.CNP_BTYPE_OR6_NCPDP_NUMBER_Force*100) OR ( DEA_NUMBER_score > Config.CNP_BTYPE_OR7_DEA_NUMBER_Force*100), CNP_BTYPE_score_temp, SKIP ); // Enforce FORCE parameter
  INTEGER2 TAXONOMY_CODE_score_unweighted := IF ( TAXONOMY_CODE_score_temp >= Config.TAXONOMY_CODE_Force * 100, TAXONOMY_CODE_score_temp, SKIP ); // Enforce FORCE parameter
  INTEGER2 TAXONOMY_CODE_score := TAXONOMY_CODE_score_unweighted*0.25; 
  INTEGER2 ST_score := IF ( ST_score_temp > Config.ST_Force * 100 OR ( NPI_NUMBER_score > Config.ST_OR1_NPI_NUMBER_Force*100) OR LOCALE_score_pre > 0 OR ADDRESS_score_pre > 0, ST_score_temp, SKIP ); // Enforce FORCE parameter
  INTEGER2 CNP_STORE_NUMBER_score := IF ( CNP_STORE_NUMBER_score_temp >= Config.CNP_STORE_NUMBER_Force * 100 OR ( FEIN_score > Config.CNP_STORE_NUMBER_OR1_FEIN_Force*100) OR ( NPI_NUMBER_score > Config.CNP_STORE_NUMBER_OR2_NPI_NUMBER_Force*100) OR ( C_LIC_NBR_score > Config.CNP_STORE_NUMBER_OR3_C_LIC_NBR_Force*100) OR ( CLIA_NUMBER_score > Config.CNP_STORE_NUMBER_OR4_CLIA_NUMBER_Force*100) OR ( MEDICARE_FACILITY_NUMBER_score > Config.CNP_STORE_NUMBER_OR5_MEDICARE_FACILITY_NUMBER_Force*100) OR ( NCPDP_NUMBER_score > Config.CNP_STORE_NUMBER_OR6_NCPDP_NUMBER_Force*100) OR ( DEA_NUMBER_score > Config.CNP_STORE_NUMBER_OR7_DEA_NUMBER_Force*100), CNP_STORE_NUMBER_score_temp, SKIP ); // Enforce FORCE parameter
// Compute the score for the concept FAC_NAME
  ...
  INTEGER2 PRIM_NAME_score := IF ( PRIM_NAME_score_temp > Config.PRIM_NAME_Force * 100 OR ( NPI_NUMBER_score > Config.PRIM_NAME_OR1_NPI_NUMBER_Force*100) OR ADDR1_score_pre > 0 OR ADDRESS_score_pre > 0, PRIM_NAME_score_temp, SKIP ); // Enforce FORCE parameter
// Compute the score for the concept LOCALE
  ...
  INTEGER2 PRIM_RANGE_score := IF ( PRIM_RANGE_score_temp > Config.PRIM_RANGE_Force * 100 OR ( NPI_NUMBER_score > Config.PRIM_RANGE_OR1_NPI_NUMBER_Force*100) OR ADDR1_score_pre > 0 OR ADDRESS_score_pre > 0, PRIM_RANGE_score_temp, SKIP ); // Enforce FORCE parameter
  INTEGER2 SEC_RANGE_score := IF ( SEC_RANGE_score_temp >= Config.SEC_RANGE_Force * 100 OR ( NPI_NUMBER_score > Config.SEC_RANGE_OR1_NPI_NUMBER_Force*100), SEC_RANGE_score_temp, SKIP ); // Enforce FORCE parameter
...
// Compute the score for the concept ADDRESS
  ...
  SELF.Conf_Prop := (0
    ...
    +MAX(le.FEIN_prop,ri.FEIN_prop)*FEIN_score // Score if either field propogated
...
mjs0 := o0 : PERSIST('~temp::LNPID::HealthCareFacilityHeader::mj::0',EXPIRE(Config.PersistExpire));
...
dn8 := hfile(~ST_isnull AND (~CNP_STORE_NUMBER_isnull OR ~C_LIC_NBR_isnull OR ~MEDICARE_FACILITY_NUMBER_isnull));
dn8_deduped := dn8(ST_weight100 + CNP_STORE_NUMBER_weight100>=1700); // Use specificity to flag high-dup counts
mj8 := JOIN( dn8_deduped, dn8_deduped, LEFT.LNPID > RIGHT.LNPID AND LEFT.ST = RIGHT.ST AND LEFT.CNP_STORE_NUMBER = RIGHT.CNP_STORE_NUMBER AND ( ~left.ST_isnull AND ~right.ST_isnull ) AND ( left.NPI_NUMBER = right.NPI_NUMBER OR left.NPI_NUMBER_isnull OR right.NPI_NUMBER_isnull ) AND ( left.CLIA_NUMBER = right.CLIA_NUMBER OR left.CLIA_NUMBER_isnull OR right.CLIA_NUMBER_isnull ) AND ( left.NCPDP_NUMBER = right.NCPDP_NUMBER OR left.NCPDP_NUMBER_isnull OR right.NCPDP_NUMBER_isnull ) AND ( left.DEA_NUMBER = right.DEA_NUMBER OR left.DEA_NUMBER_isnull OR right.DEA_NUMBER_isnull ) AND ( left.CNP_STORE_NUMBER = right.CNP_STORE_NUMBER OR left.CNP_STORE_NUMBER_isnull OR right.CNP_STORE_NUMBER_isnull ) AND ( ~left.ADDRESS_isnull AND ~right.ADDRESS_isnull ) AND ( left.FEIN = right.FEIN OR left.FEIN_isnull OR right.FEIN_isnull ) AND (( ~left.CNP_NAME_isnull AND ~right.CNP_NAME_isnull ) OR ( ~left.C_LIC_NBR_isnull AND ~right.C_LIC_NBR_isnull ) OR ( ~left.MEDICARE_FACILITY_NUMBER_isnull AND ~right.MEDICARE_FACILITY_NUMBER_isnull )) AND ( left.CNP_NUMBER = right.CNP_NUMBER OR left.CNP_NUMBER_isnull OR right.CNP_NUMBER_isnull ) AND ( ~left.PRIM_NAME_isnull AND ~right.PRIM_NAME_isnull ) AND ( ~left.PRIM_RANGE_isnull AND ~right.PRIM_RANGE_isnull ) AND ( left.CNP_BTYPE = right.CNP_BTYPE OR left.CNP_BTYPE_isnull OR right.CNP_BTYPE_isnull ) AND ( left.TAXONOMY_CODE = right.TAXONOMY_CODE OR left.TAXONOMY_CODE_isnull OR right.TAXONOMY_CODE_isnull ),trans(LEFT,RIGHT,8),HINT(unsorted_output),ATMOST(LEFT.ST = RIGHT.ST AND LEFT.CNP_STORE_NUMBER = RIGHT.CNP_STORE_NUMBER,10000),HASH);
//Fixed fields ->:ST(5):PHONE(20)
dn9 := hfile(~ST_isnull AND ~PHONE_isnull);
dn9_deduped := dn9(ST_weight100 + PHONE_weight100>=1700); // Use specificity to flag high-dup counts
mj9 := JOIN( dn9_deduped, dn9_deduped, LEFT.LNPID > RIGHT.LNPID AND LEFT.ST = RIGHT.ST AND LEFT.PHONE = RIGHT.PHONE AND LEFT.CNP_NAME = RIGHT.CNP_NAME AND ( ~left.ST_isnull AND ~right.ST_isnull ) AND ( left.NPI_NUMBER = right.NPI_NUMBER OR left.NPI_NUMBER_isnull OR right.NPI_NUMBER_isnull ) AND ( left.CLIA_NUMBER = right.CLIA_NUMBER OR left.CLIA_NUMBER_isnull OR right.CLIA_NUMBER_isnull ) AND ( left.NCPDP_NUMBER = right.NCPDP_NUMBER OR left.NCPDP_NUMBER_isnull OR right.NCPDP_NUMBER_isnull ) AND ( left.DEA_NUMBER = right.DEA_NUMBER OR left.DEA_NUMBER_isnull OR right.DEA_NUMBER_isnull ) AND ( left.CNP_STORE_NUMBER = right.CNP_STORE_NUMBER OR left.CNP_STORE_NUMBER_isnull OR right.CNP_STORE_NUMBER_isnull ) AND ( ~left.ADDRESS_isnull AND ~right.ADDRESS_isnull ) AND ( left.FEIN = right.FEIN OR left.FEIN_isnull OR right.FEIN_isnull ) AND (( ~left.CNP_NAME_isnull AND ~right.CNP_NAME_isnull ) OR ( ~left.C_LIC_NBR_isnull AND ~right.C_LIC_NBR_isnull ) OR ( ~left.MEDICARE_FACILITY_NUMBER_isnull AND ~right.MEDICARE_FACILITY_NUMBER_isnull )) AND ( left.CNP_NUMBER = right.CNP_NUMBER OR left.CNP_NUMBER_isnull OR right.CNP_NUMBER_isnull ) AND ( ~left.PRIM_NAME_isnull AND ~right.PRIM_NAME_isnull ) AND ( ~left.PRIM_RANGE_isnull AND ~right.PRIM_RANGE_isnull ) AND ( left.CNP_BTYPE = right.CNP_BTYPE OR left.CNP_BTYPE_isnull OR right.CNP_BTYPE_isnull ) AND ( left.TAXONOMY_CODE = right.TAXONOMY_CODE OR left.TAXONOMY_CODE_isnull OR right.TAXONOMY_CODE_isnull ),trans(LEFT,RIGHT,9),HINT(unsorted_output),ATMOST(LEFT.ST = RIGHT.ST AND LEFT.PHONE = RIGHT.PHONE AND LEFT.CNP_NAME = RIGHT.CNP_NAME,10000),HASH);
mjs1_t := mj5+mj6+mj7+mj8+mj9;
SALT30.mac_select_best_matches(mjs1_t,RID1,RID2,o1);
mjs1 := o1 : PERSIST('~temp::LNPID::HealthCareFacilityHeader::mj::1',EXPIRE(Config.PersistExpire));
//Fixed fields ->:ST(5):FAX(20)
dn10 := hfile(~ST_isnull AND ~FAX_isnull);
dn10_deduped := dn10(ST_weight100 + FAX_weight100>=1700); // Use specificity to flag high-dup counts
mj10 := JOIN( dn10_deduped, dn10_deduped, LEFT.LNPID > RIGHT.LNPID AND LEFT.ST = RIGHT.ST AND LEFT.FAX = RIGHT.FAX AND LEFT.CNP_NAME = RIGHT.CNP_NAME AND ( ~left.ST_isnull AND ~right.ST_isnull ) AND ( left.NPI_NUMBER = right.NPI_NUMBER OR left.NPI_NUMBER_isnull OR right.NPI_NUMBER_isnull ) AND ( left.CLIA_NUMBER = right.CLIA_NUMBER OR left.CLIA_NUMBER_isnull OR right.CLIA_NUMBER_isnull ) AND ( left.NCPDP_NUMBER = right.NCPDP_NUMBER OR left.NCPDP_NUMBER_isnull OR right.NCPDP_NUMBER_isnull ) AND ( left.DEA_NUMBER = right.DEA_NUMBER OR left.DEA_NUMBER_isnull OR right.DEA_NUMBER_isnull ) AND ( left.CNP_STORE_NUMBER = right.CNP_STORE_NUMBER OR left.CNP_STORE_NUMBER_isnull OR right.CNP_STORE_NUMBER_isnull ) AND ( ~left.ADDRESS_isnull AND ~right.ADDRESS_isnull ) AND ( left.FEIN = right.FEIN OR left.FEIN_isnull OR right.FEIN_isnull ) AND (( ~left.CNP_NAME_isnull AND ~right.CNP_NAME_isnull ) OR ( ~left.C_LIC_NBR_isnull AND ~right.C_LIC_NBR_isnull ) OR ( ~left.MEDICARE_FACILITY_NUMBER_isnull AND ~right.MEDICARE_FACILITY_NUMBER_isnull )) AND ( left.CNP_NUMBER = right.CNP_NUMBER OR left.CNP_NUMBER_isnull OR right.CNP_NUMBER_isnull ) AND ( ~left.PRIM_NAME_isnull AND ~right.PRIM_NAME_isnull ) AND ( ~left.PRIM_RANGE_isnull AND ~right.PRIM_RANGE_isnull ) AND ( left.CNP_BTYPE = right.CNP_BTYPE OR left.CNP_BTYPE_isnull OR right.CNP_BTYPE_isnull ) AND ( left.TAXONOMY_CODE = right.TAXONOMY_CODE OR left.TAXONOMY_CODE_isnull OR right.TAXONOMY_CODE_isnull ),trans(LEFT,RIGHT,10),HINT(unsorted_output),ATMOST(LEFT.ST = RIGHT.ST AND LEFT.FAX = RIGHT.FAX AND LEFT.CNP_NAME = RIGHT.CNP_NAME,10000),HASH);
...
mjs2 := o2 : PERSIST('~temp::LNPID::HealthCareFacilityHeader::mj::2',EXPIRE(Config.PersistExpire));
...
EXPORT All_Matches := all_mjs : PERSIST('~temp::LNPID::HealthCareFacilityHeader::all_m',EXPIRE(Config.PersistExpire)); // To by used by RID and LNPID
SALT30.mac_avoid_transitives(All_Matches,LNPID1,LNPID2,Conf,DateOverlap,Rule,o);
EXPORT PossibleMatches := o : PERSIST('~temp::LNPID::HealthCareFacilityHeader::mt',EXPIRE(Config.PersistExpire));
SALT30.mac_get_BestPerRecord( All_Matches,RID1,LNPID1,RID2,LNPID2,o );
EXPORT BestPerRecord := o : PERSIST('~temp::LNPID::HealthCareFacilityHeader::mr',EXPIRE(Config.PersistExpire));
...
EXPORT ClusterLinkages := o1 : PERSIST('~temp::LNPID::HealthCareFacilityHeader::clu',EXPIRE(Config.PersistExpire));
...
EXPORT ToSlice := IF ( Config.DoSliceouts, DEDUP(SORT(DISTRIBUTE(BetterElsewhere(Pref_Margin>10),HASH(LNPID)),LNPID,-Pref_Margin,LOCAL),LNPID,LOCAL)) : PERSIST('~temp::LNPID::HealthCareFacilityHeader::Matches::ToSlice',EXPIRE(Config.PersistExpire));
...
SALT30.MAC_SliceOut_ByRID(ihbp,RID,LNPID,ToSlice,RID,sliced0); // Execute Sliceouts
...
EXPORT PreIDs := HealthCareFacilityHeader.Fields.UIDConsistency(ih); // Export whole consistency module
EXPORT PostIDs := HealthCareFacilityHeader.Fields.UIDConsistency(Patched_Infile); // Export whole consistency module

...
function
	...
	transform
		...
		trim(l.emailine) + 	'      ' + (string)r.record_id + ') ' + r.name[2..] + '\n\t\t\t' + r.superfilecontents + '\n');
		self := l;
	end;

	layout_twonames tprojectline(layout_twonames l) :=
	transform
		self.emailine := if(trim(l.emailine) = '', 'keys: ' + (string)l.record_id + ') ' + l.name[2..] + '' + l.superfilecontents + '\n',
							l.emailine);
		self := l;
	end;
	...
	projectcontents_good	:= project(rollupcontents_good	, tprojectline(left));
	projectcontents_bad		:= project(rollupcontents_bad		, tprojectline(left));
	...
	versions_rollup := rollup(versions_table_email, true, trollupversions(left,right));
	...
	decision_time := map(count(getcontents_badversion) > 0 =>
												...
												+ '\t' + versions_rollup[1].versions + '\n\n'
												+ 'List of keys with Wrong version follows.\n'
												+ stringlib.StringToLowerCase(trim(projectcontents_bad[1].emailine))
	...
	return 		decision_time
					+ if(count(getcontents_goodversion) > 0
							,		if(count(getcontents_badversion) > 0
								...
								+ stringlib.StringToLowerCase(trim(projectcontents_good[1].emailine))

...
//////////////////////////////////////////////////////////////////////////////////////
export Standardize_Unaffiliated_Individuals_AID (dataset(Layouts.Base.Unaffiliated_Individuals) pBaseFile)	:=	function

		Layouts.Temporary.Unaffiliated_Individuals tPreProcessUnaffiliated_Individuals(Layouts.Base.Unaffiliated_Individuals l, unsigned8 cnt) :=	transform
			self.unique_id									:=	cnt;
			self														:=	l;
		end;
		
		dPreProcess := project(pBaseFile, tPreProcessUnaffiliated_Individuals(left,counter));	
		...
		addresslayout tProjectAddress(Layouts.Temporary.Unaffiliated_Individuals l,unsigned8 cnt) :=
		transform
		...
		Layouts.Temporary.Unaffiliated_Individuals tGetStandardizedAddress(Layouts.Temporary.Unaffiliated_Individuals l	,dAddressStandardized_dist r) :=
		transform
		...
		dReformattedAddress	:=	PROJECT(dCleancontactAddressAppended,TRANSFORM(Layouts.Base.Unaffiliated_Individuals,SELF := LEFT;));															

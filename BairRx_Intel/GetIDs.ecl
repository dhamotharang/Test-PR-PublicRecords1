...
EXPORT GetIDs(BairRx_Common.IParam.SearchParam inMod) := FUNCTION
		...
		MC := inMod.ModeContext(mID=BairRx_Common.Constants.Mode.INT)[1];
		DR := MC.data_relation;
		...
		dBoolSrchRecs := IF(booleanSearch,BairRx_Common.BooleanFilterSearch(inMod,BairRx_Common.Constants.SearchType.Intel,MC.filters,GH.SearchGrid,BoolEIDs,booleanSearch));		

Import iesp, ABMS, STD, Healthcare_ABMS,Healthcare_Shared;
EXPORT Transforms := MODULE
	export Healthcare_ABMS.Layouts.autokeyInput convertIESPtoAKInput(iesp.abms.t_ABMSSearchBy inRec, boolean includeCareer=false,boolean includeEducation=false,boolean includeProfessionalAssociations=false, unsigned1 glb=8, unsigned1 dppa=0):= TRANSFORM
			//Handle unparsed input address
			clnAddr := Healthcare_ABMS.Functions.processAddressInput(inRec.Address);
			//Handle unparsed Name
			clnName := Healthcare_ABMS.Functions.processNameInput(inRec.Name);
			self.acctno := if(inRec.AccountNumber <>'',inRec.AccountNumber,'1');
			self.comp_name := STD.Str.ToUpperCase(inRec.CompanyName);
			self.name_first := STD.Str.ToUpperCase(clnName.FirstName);
			self.name_middle := STD.Str.ToUpperCase(clnName.MiddleName);
			self.name_last := STD.Str.ToUpperCase(clnName.LastName);
			self.name_suffix := STD.Str.ToUpperCase(clnName.Suffix);
			self.prim_range := STD.Str.ToUpperCase(clnAddr.prim_range);
			self.predir := STD.Str.ToUpperCase(clnAddr.predir);
			self.prim_name := STD.Str.ToUpperCase(clnAddr.prim_name);
			self.addr_suffix := STD.Str.ToUpperCase(clnAddr.addr_suffix);
			self.postdir := STD.Str.ToUpperCase(clnAddr.postdir);
			self.unit_desig := STD.Str.ToUpperCase(clnAddr.unit_desig);
			self.sec_range := STD.Str.ToUpperCase(clnAddr.sec_range);
			self.p_city_name := STD.Str.ToUpperCase(clnAddr.p_city_name);
			self.st := STD.Str.ToUpperCase(clnAddr.st);
			self.z5 := STD.Str.ToUpperCase(clnAddr.z5);
			self.dob := iesp.ECL2ESP.DateToString(inRec.DOB);
			self.DID	:= (integer)inRec.UniqueId;
			self.BDID	:= (integer)inRec.BusinessId;
			self.NPI := STD.Str.ToUpperCase(inRec.NPINumber);	
			self.Specialty := STD.Str.ToUpperCase(inRec.Specialty);
			self.ProviderID := (integer)inRec.ABMSBiogID;	
			self.ProviderSrc := Healthcare_Shared.Constants.SRC_ABMS;	
	...
	export Healthcare_ABMS.Layouts.LayoutOutput buildBase(Healthcare_ABMS.Layouts.searchKeyResults inCriteria, recordof(ABMS.Keys().Main.BIOGNumber.qa) inRec, integer PenaltThreshold):= TRANSFORM
			...
			penaltyVal:= Healthcare_ABMS.Functions.doPenalty(inRec,inCriteria);
	...
	export Healthcare_ABMS.Layouts.LayoutOutput doABMSBiogIDRollup(Healthcare_ABMS.Layouts.LayoutOutput inRec, dataset(Healthcare_ABMS.Layouts.LayoutOutput) allrows):= TRANSFORM
	...
	export Healthcare_ABMS.Layouts.LayoutOutput_batch fmtBatchResults(Healthcare_ABMS.Layouts.LayoutOutputBatch inRec):= TRANSFORM
	...
	export Healthcare_ABMS.Layouts.LayoutOutput_batch setBatchResultsFlags(Healthcare_ABMS.Layouts.autokeyInput inRecs, Healthcare_ABMS.Layouts.LayoutOutput_batch outRec):= TRANSFORM

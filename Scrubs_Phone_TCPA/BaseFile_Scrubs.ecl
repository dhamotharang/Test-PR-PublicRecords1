...
// The module to handle the case where no scrubs exist
  ...
  EXPORT Expanded_Layout := RECORD(BaseFile_Layout_Phone_TCPA)
  ...
  END;
  EXPORT  Bitmap_Layout := RECORD(BaseFile_Layout_Phone_TCPA)
    UNSIGNED8 ScrubsBits1;
  END;
  EXPORT Rule_Layout := RECORD(BaseFile_Layout_Phone_TCPA)
  ...
  SHARED toErrorMessage(UNSIGNED c) := CHOOSE(c
          ,BaseFile_Fields.InvalidMessage_num(1)
          ,BaseFile_Fields.InvalidMessage_phone(1)
          ,BaseFile_Fields.InvalidMessage_end_range(1)
          ,BaseFile_Fields.InvalidMessage_expand_end_range(1)
          ,BaseFile_Fields.InvalidMessage_expand_range_max_count(1)
          ,BaseFile_Fields.InvalidMessage_expand_phone(1)
          ,BaseFile_Fields.InvalidMessage_dt_first_seen(1)
          ,BaseFile_Fields.InvalidMessage_dt_last_seen(1)
          ,BaseFile_Fields.InvalidMessage_phone_type(1)
          ,'Fields with errors'
...
EXPORT FromNone(DATASET(BaseFile_Layout_Phone_TCPA) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.num_Invalid := BaseFile_Fields.InValid_num((SALT311.StrType)le.num);
    SELF.phone_Invalid := BaseFile_Fields.InValid_phone((SALT311.StrType)le.phone);
    SELF.end_range_Invalid := BaseFile_Fields.InValid_end_range((SALT311.StrType)le.end_range);
    SELF.expand_end_range_Invalid := BaseFile_Fields.InValid_expand_end_range((SALT311.StrType)le.expand_end_range);
    SELF.expand_range_max_count_Invalid := BaseFile_Fields.InValid_expand_range_max_count((SALT311.StrType)le.expand_range_max_count);
    SELF.expand_phone_Invalid := BaseFile_Fields.InValid_expand_phone((SALT311.StrType)le.expand_phone);
    SELF.dt_first_seen_Invalid := BaseFile_Fields.InValid_dt_first_seen((SALT311.StrType)le.dt_first_seen);
    SELF.dt_last_seen_Invalid := BaseFile_Fields.InValid_dt_last_seen((SALT311.StrType)le.dt_last_seen);
    SELF.phone_type_Invalid := BaseFile_Fields.InValid_phone_type((SALT311.StrType)le.phone_type);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),BaseFile_Layout_Phone_TCPA);
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,BaseFile_Layout_Phone_TCPA);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,BaseFile_Fields.InvalidMessage_num(le.num_Invalid),BaseFile_Fields.InvalidMessage_phone(le.phone_Invalid),BaseFile_Fields.InvalidMessage_end_range(le.end_range_Invalid),BaseFile_Fields.InvalidMessage_expand_end_range(le.expand_end_range_Invalid),BaseFile_Fields.InvalidMessage_expand_range_max_count(le.expand_range_max_count_Invalid),BaseFile_Fields.InvalidMessage_expand_phone(le.expand_phone_Invalid),BaseFile_Fields.InvalidMessage_dt_first_seen(le.dt_first_seen_Invalid),BaseFile_Fields.InvalidMessage_dt_last_seen(le.dt_last_seen_Invalid),BaseFile_Fields.InvalidMessage_phone_type(le.phone_type_Invalid),'UNKNOWN'));
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(BaseFile_Layout_Phone_TCPA) prevDS = DATASET([], BaseFile_Layout_Phone_TCPA), STRING10 Src='UNK'):= FUNCTION
  // field error stats
    ...
    mod_hygiene := BaseFile_hygiene(PROJECT(h, BaseFile_Layout_Phone_TCPA));
  ...
  // record count stats
    ...
    mod_Delta := BaseFile_Delta(prevDS, PROJECT(h, BaseFile_Layout_Phone_TCPA));
...
EXPORT StandardStats(DATASET(BaseFile_Layout_Phone_TCPA) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  myTimeStamp := (UNSIGNED6)SALT311.Fn_Now('YYYYMMDDHHMMSS') : INDEPENDENT;
  expandedFile := FromNone(inFile).ExpandedInfile;
  mod_fromexpandedOverall := FromExpanded(expandedFile);
  scrubsSummaryOverall := mod_fromexpandedOverall.SummaryStats;
 
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Phone_TCPA, BaseFile_Fields, 'RECORDOF(scrubsSummaryOverall)', '');

IMPORT SALT311;
EXPORT Wireline_to_Wireless2_NoRange_Fields := MODULE
...
EXPORT SALT311.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'cellphone','lf','filename');
EXPORT SALT311.StrType FlatName(UNSIGNED2 i) := CHOOSE(i,'cellphone','lf','filename');
EXPORT FieldNum(SALT311.StrType fn) := CASE(fn,'cellphone' => 0,'lf' => 1,'filename' => 2,0);
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_cellphone,1,0)+ IF( SELF.Diff_lf,1,0)+ IF( SELF.Diff_filename,1,0);
  END;

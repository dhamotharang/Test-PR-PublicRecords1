IMPORT SALT311,STD;
EXPORT BaseFile_hygiene(dataset(BaseFile_layout_Phone_TCPA) h) := MODULE
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    BaseFile_Fields.InValid_num((SALT311.StrType)le.num),
    BaseFile_Fields.InValid_phone((SALT311.StrType)le.phone),
    BaseFile_Fields.InValid_end_range((SALT311.StrType)le.end_range),
    BaseFile_Fields.InValid_expand_end_range((SALT311.StrType)le.expand_end_range),
    BaseFile_Fields.InValid_expand_range_max_count((SALT311.StrType)le.expand_range_max_count),
    BaseFile_Fields.InValid_expand_phone((SALT311.StrType)le.expand_phone),
    BaseFile_Fields.InValid_is_current((SALT311.StrType)le.is_current),
    BaseFile_Fields.InValid_dt_first_seen((SALT311.StrType)le.dt_first_seen),
    BaseFile_Fields.InValid_dt_last_seen((SALT311.StrType)le.dt_last_seen),
    BaseFile_Fields.InValid_phone_type((SALT311.StrType)le.phone_type),
    0);
...
PrettyErrorTotals := RECORD
  ...
  ErrorMessage := CHOOSE(TotalErrors.FieldNum,BaseFile_Fields.InValidMessage_num(TotalErrors.ErrorNum),BaseFile_Fields.InValidMessage_phone(TotalErrors.ErrorNum),BaseFile_Fields.InValidMessage_end_range(TotalErrors.ErrorNum),BaseFile_Fields.InValidMessage_expand_end_range(TotalErrors.ErrorNum),BaseFile_Fields.InValidMessage_expand_range_max_count(TotalErrors.ErrorNum),BaseFile_Fields.InValidMessage_expand_phone(TotalErrors.ErrorNum),BaseFile_Fields.InValidMessage_is_current(TotalErrors.ErrorNum),BaseFile_Fields.InValidMessage_dt_first_seen(TotalErrors.ErrorNum),BaseFile_Fields.InValidMessage_dt_last_seen(TotalErrors.ErrorNum),BaseFile_Fields.InValidMessage_phone_type(TotalErrors.ErrorNum));
  TotalErrors.Cnt;
END;
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_Phone_TCPA, BaseFile_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

IMPORT SALT311,STD;
EXPORT Wireline_to_Wireless2_hygiene(dataset(Wireline_to_Wireless2_layout_Phone_TCPA) h) := MODULE
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    Wireline_to_Wireless2_Fields.InValid_cellphone((SALT311.StrType)le.cellphone),
    Wireline_to_Wireless2_Fields.InValid_end_range((SALT311.StrType)le.end_range),
    Wireline_to_Wireless2_Fields.InValid_lf((SALT311.StrType)le.lf),
    Wireline_to_Wireless2_Fields.InValid_filename((SALT311.StrType)le.filename),
    0);
...
PrettyErrorTotals := RECORD
  FieldNme := Wireline_to_Wireless2_Fields.FieldName(TotalErrors.FieldNum);
  FieldType := CHOOSE(TotalErrors.FieldNum,'Invalid_Num','Invalid_Num_Space','Unknown','Unknown');
  ErrorMessage := CHOOSE(TotalErrors.FieldNum,Wireline_to_Wireless2_Fields.InValidMessage_cellphone(TotalErrors.ErrorNum),Wireline_to_Wireless2_Fields.InValidMessage_end_range(TotalErrors.ErrorNum),Wireline_to_Wireless2_Fields.InValidMessage_lf(TotalErrors.ErrorNum),Wireline_to_Wireless2_Fields.InValidMessage_filename(TotalErrors.ErrorNum));
  TotalErrors.Cnt;
END;
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_Phone_TCPA, Wireline_to_Wireless2_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

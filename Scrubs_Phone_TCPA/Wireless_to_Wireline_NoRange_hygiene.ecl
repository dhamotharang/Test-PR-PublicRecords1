IMPORT SALT311,STD;
EXPORT Wireless_to_Wireline_NoRange_hygiene(dataset(Wireless_to_Wireline_NoRange_layout_Phone_TCPA) h) := MODULE
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    Wireless_to_Wireline_NoRange_Fields.InValid_cellphone((SALT311.StrType)le.cellphone),
    Wireless_to_Wireline_NoRange_Fields.InValid_lf((SALT311.StrType)le.lf),
    Wireless_to_Wireline_NoRange_Fields.InValid_filename((SALT311.StrType)le.filename),
    0);
...
PrettyErrorTotals := RECORD
  FieldNme := Wireless_to_Wireline_NoRange_Fields.FieldName(TotalErrors.FieldNum);
  FieldType := CHOOSE(TotalErrors.FieldNum,'Invalid_Num','Unknown','Unknown');
  ErrorMessage := CHOOSE(TotalErrors.FieldNum,Wireless_to_Wireline_NoRange_Fields.InValidMessage_cellphone(TotalErrors.ErrorNum),Wireless_to_Wireline_NoRange_Fields.InValidMessage_lf(TotalErrors.ErrorNum),Wireless_to_Wireline_NoRange_Fields.InValidMessage_filename(TotalErrors.ErrorNum));
  TotalErrors.Cnt;
END;
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_Phone_TCPA, Wireless_to_Wireline_NoRange_Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

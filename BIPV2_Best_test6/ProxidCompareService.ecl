...
EXPORT ProxidCompareService := MACRO
  IMPORT SALT27,BIPV2_Best_test6,ut;
...
odl := PROJECT(CHOOSEN(BIPV2_Best_test6.Keys(BFile).Candidates(Proxid=Proxidone),100000),BIPV2_Best_test6.match_candidates(BFile).layout_candidates);
odr := PROJECT(CHOOSEN(BIPV2_Best_test6.Keys(BFile).Candidates(Proxid=ProxidTwo),100000),BIPV2_Best_test6.match_candidates(BFile).layout_candidates);
k := BIPV2_Best_test6.Keys(BFile).Specificities_Key;
s := GLOBAL(PROJECT(k,BIPV2_Best_test6.Layout_Specificities.R)[1]);
odlv := BIPV2_Best_test6.Debug(BFile,s).RolledEntities(odl);
odrv := BIPV2_Best_test6.Debug(BFile,s).RolledEntities(odr);
mtch := BIPV2_Best_test6.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,Proxidone,Proxidtwo,0,0}],BIPV2_Best_test6.match_candidates(BFile).layout_matches));
// Put out easy to read versions of the 5067616 data

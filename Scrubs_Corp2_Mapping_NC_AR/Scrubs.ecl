...
EXPORT FromNone(DATASET(Layout_in_file) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_sos_charter_nbr_Invalid := Fields.InValid_corp_sos_charter_nbr((SALT35.StrType)le.corp_sos_charter_nbr);
    SELF.ar_filed_dt_Invalid := Fields.InValid_ar_filed_dt((SALT35.StrType)le.ar_filed_dt);
    SELF.ar_type_Invalid := Fields.InValid_ar_type((SALT35.StrType)le.ar_type);
    SELF := le;
  END;
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.corp_key_Invalid << 0 ) + ( le.corp_vendor_Invalid << 2 ) + ( le.corp_state_origin_Invalid << 3 ) + ( le.corp_process_date_Invalid << 4 ) + ( le.corp_sos_charter_nbr_Invalid << 6 ) + ( le.ar_filed_dt_Invalid << 8 ) + ( le.ar_type_Invalid << 10 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.ar_type_Invalid := (le.ScrubsBits1 >> 10) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.corp_key_Invalid,le.corp_vendor_Invalid,le.corp_state_origin_Invalid,le.corp_process_date_Invalid,le.corp_sos_charter_nbr_Invalid,le.ar_filed_dt_Invalid,le.ar_type_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_corp_key(le.corp_key_Invalid),Fields.InvalidMessage_corp_vendor(le.corp_vendor_Invalid),Fields.InvalidMessage_corp_state_origin(le.corp_state_origin_Invalid),Fields.InvalidMessage_corp_process_date(le.corp_process_date_Invalid),Fields.InvalidMessage_corp_sos_charter_nbr(le.corp_sos_charter_nbr_Invalid),Fields.InvalidMessage_ar_filed_dt(le.ar_filed_dt_Invalid),Fields.InvalidMessage_ar_type(le.ar_type_Invalid),'UNKNOWN'));
    ...
    SELF.FieldName := CHOOSE(c,'corp_key','corp_vendor','corp_state_origin','corp_process_date','corp_sos_charter_nbr','ar_filed_dt','ar_type','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter_nbr','invalid_date','invalid_ar_type','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT35.StrType)le.corp_key,(SALT35.StrType)le.corp_vendor,(SALT35.StrType)le.corp_state_origin,(SALT35.StrType)le.corp_process_date,(SALT35.StrType)le.corp_sos_charter_nbr,(SALT35.StrType)le.ar_filed_dt,(SALT35.StrType)le.ar_type,'***SALTBUG***');
  END;

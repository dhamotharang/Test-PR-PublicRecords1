...
EXPORT FieldTypeNum(SALT32.StrType fn) := CASE(fn,'invalid_slashO_date' => 1,'invalid_alpha' => 2,'invalid_alphapound' => 3,'invalid_alphaspace' => 4,'invalid_alphaspacequote' => 5,'invalid_date' => 6,'invalid_numeric' => 7,'invalid_numericpound' => 8,'invalid_alphanumeric' => 9,'invalid_gender' => 10,'invalid_addr_type' => 11,'invalid_license_number' => 12,'invalid_marital_status' => 13,0);
 
EXPORT MakeFT_invalid_slashO_date(SALT32.StrType s0) := FUNCTION
  s1 := SALT32.stringfilter(s0,'/O0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_slashO_date(SALT32.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT32.StringFilter(s,'/O0123456789'))));
EXPORT InValidMessageFT_invalid_slashO_date(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInChars('/O0123456789'),SALT32.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_license_number(SALT32.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_invalid_license_number(SALT32.StrType s,SALT32.StrType reg_state) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT32.StringFilter(s,' .-#*\\ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'))),~ALC.Custom_Scrubs.fn_valid_tx_nurse_license_no(s,reg_state)<1);
EXPORT InValidMessageFT_invalid_license_number(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInChars(' .-#*\\ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'),SALT32.HygieneErrors.CustomFail('ALC.Custom_Scrubs.fn_valid_tx_nurse_license_no'),SALT32.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_marital_status(SALT32.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_marital_status(SALT32.StrType s) := WHICH(((SALT32.StrType) s) NOT IN ['M','S',' ']);
EXPORT InValidMessageFT_invalid_marital_status(UNSIGNED1 wh) := CHOOSE(wh,SALT32.HygieneErrors.NotInEnum('M|S| '),SALT32.HygieneErrors.Good);
...
EXPORT FieldNum(SALT32.StrType fn) := CASE(fn,'fname' => 0,'lname' => 1,'title' => 2,'company' => 3,'address1' => 4,'address2' => 5,'city' => 6,'state' => 7,'zip' => 8,'zip4' => 9,'cart' => 10,'bar' => 11,'gender' => 12,'country' => 13,'postal_cd' => 14,'dpv' => 15,'addr_type' => 16,'age' => 17,'county_cd' => 18,'household_income' => 19,'marital_status' => 20,'msa' => 21,'nielsen_county_cd' => 22,'nurse_type' => 23,'reg_state' => 24,'specialty' => 25,'email' => 26,'phone' => 27,'list_id' => 28,'scno' => 29,'keycode' => 30,'custno' => 31,'license_no' => 32,'dob' => 33,'orig_date' => 34,'exp_date' => 35,'degree' => 36,0);
...
EXPORT Make_household_income(SALT32.StrType s0) := MakeFT_invalid_numeric(s0);
EXPORT InValid_household_income(SALT32.StrType s) := InValidFT_invalid_numeric(s);
EXPORT InValidMessage_household_income(UNSIGNED1 wh) := InValidMessageFT_invalid_numeric(wh);
 
EXPORT Make_marital_status(SALT32.StrType s0) := MakeFT_invalid_marital_status(s0);
EXPORT InValid_marital_status(SALT32.StrType s) := InValidFT_invalid_marital_status(s);
...
EXPORT Make_nurse_type(SALT32.StrType s0) := MakeFT_invalid_numericpound(s0);
EXPORT InValid_nurse_type(SALT32.StrType s) := InValidFT_invalid_numericpound(s);
EXPORT InValidMessage_nurse_type(UNSIGNED1 wh) := InValidMessageFT_invalid_numericpound(wh);
 
EXPORT Make_reg_state(SALT32.StrType s0) := MakeFT_invalid_alphapound(s0);
EXPORT InValid_reg_state(SALT32.StrType s) := InValidFT_invalid_alphapound(s);
EXPORT InValidMessage_reg_state(UNSIGNED1 wh) := InValidMessageFT_invalid_alphapound(wh);
 
EXPORT Make_specialty(SALT32.StrType s0) := MakeFT_invalid_numericpound(s0);
EXPORT InValid_specialty(SALT32.StrType s) := InValidFT_invalid_numericpound(s);
...
EXPORT InValid_license_no(SALT32.StrType s,SALT32.StrType reg_state) := InValidFT_invalid_license_number(s,reg_state);
...
EXPORT Make_orig_date(SALT32.StrType s0) := MakeFT_invalid_slashO_date(s0);
EXPORT InValid_orig_date(SALT32.StrType s) := InValidFT_invalid_slashO_date(s);
EXPORT InValidMessage_orig_date(UNSIGNED1 wh) := InValidMessageFT_invalid_slashO_date(wh);
 
EXPORT Make_exp_date(SALT32.StrType s0) := MakeFT_invalid_slashO_date(s0);
EXPORT InValid_exp_date(SALT32.StrType s) := InValidFT_invalid_slashO_date(s);
...
export MAC_CountDifferencesByPivot(in_left,in_right,pivot_exp,bad_pivots,out_counts) := MACRO
  IMPORT SALT32,Scrubs_ALC_NURSES;

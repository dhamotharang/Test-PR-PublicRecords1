IMPORT SALT37;
EXPORT hygiene(dataset(layout_ALC_NURSES) h) := MODULE
...
EXPORT Summary(SALT37.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_age := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.age)));
    avelength_age := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.age)),h.age<>(typeof(h.age))'');
    ...
    maxlength_household_income := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.household_income)));
    avelength_household_income := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.household_income)),h.household_income<>(typeof(h.household_income))'');
    populated_marital_status_pcnt := AVE(GROUP,IF(h.marital_status = (TYPEOF(h.marital_status))'',0,100));
    maxlength_marital_status := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.marital_status)));
    avelength_marital_status := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.marital_status)),h.marital_status<>(typeof(h.marital_status))'');
    ...
    populated_nurse_type_pcnt := AVE(GROUP,IF(h.nurse_type = (TYPEOF(h.nurse_type))'',0,100));
    maxlength_nurse_type := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.nurse_type)));
    avelength_nurse_type := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.nurse_type)),h.nurse_type<>(typeof(h.nurse_type))'');
    populated_reg_state_pcnt := AVE(GROUP,IF(h.reg_state = (TYPEOF(h.reg_state))'',0,100));
    maxlength_reg_state := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.reg_state)));
    avelength_reg_state := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.reg_state)),h.reg_state<>(typeof(h.reg_state))'');
...
invRec invert(summary0 le, INTEGER C) := TRANSFORM
  ...
  SELF.FieldName := CHOOSE(C,'fname','lname','title','company','address1','address2','city','state','zip','zip4','cart','bar','gender','country','postal_cd','dpv','addr_type','age','county_cd','household_income','marital_status','msa','nielsen_county_cd','nurse_type','reg_state','specialty','email','phone','list_id','scno','keycode','custno','license_no','dob','orig_date','exp_date','degree');
  SELF.populated_pcnt := CHOOSE(C,le.populated_fname_pcnt,le.populated_lname_pcnt,le.populated_title_pcnt,le.populated_company_pcnt,le.populated_address1_pcnt,le.populated_address2_pcnt,le.populated_city_pcnt,le.populated_state_pcnt,le.populated_zip_pcnt,le.populated_zip4_pcnt,le.populated_cart_pcnt,le.populated_bar_pcnt,le.populated_gender_pcnt,le.populated_country_pcnt,le.populated_postal_cd_pcnt,le.populated_dpv_pcnt,le.populated_addr_type_pcnt,le.populated_age_pcnt,le.populated_county_cd_pcnt,le.populated_household_income_pcnt,le.populated_marital_status_pcnt,le.populated_msa_pcnt,le.populated_nielsen_county_cd_pcnt,le.populated_nurse_type_pcnt,le.populated_reg_state_pcnt,le.populated_specialty_pcnt,le.populated_email_pcnt,le.populated_phone_pcnt,le.populated_list_id_pcnt,le.populated_scno_pcnt,le.populated_keycode_pcnt,le.populated_custno_pcnt,le.populated_license_no_pcnt,le.populated_dob_pcnt,le.populated_orig_date_pcnt,le.populated_exp_date_pcnt,le.populated_degree_pcnt);
  SELF.maxlength := CHOOSE(C,le.maxlength_fname,le.maxlength_lname,le.maxlength_title,le.maxlength_company,le.maxlength_address1,le.maxlength_address2,le.maxlength_city,le.maxlength_state,le.maxlength_zip,le.maxlength_zip4,le.maxlength_cart,le.maxlength_bar,le.maxlength_gender,le.maxlength_country,le.maxlength_postal_cd,le.maxlength_dpv,le.maxlength_addr_type,le.maxlength_age,le.maxlength_county_cd,le.maxlength_household_income,le.maxlength_marital_status,le.maxlength_msa,le.maxlength_nielsen_county_cd,le.maxlength_nurse_type,le.maxlength_reg_state,le.maxlength_specialty,le.maxlength_email,le.maxlength_phone,le.maxlength_list_id,le.maxlength_scno,le.maxlength_keycode,le.maxlength_custno,le.maxlength_license_no,le.maxlength_dob,le.maxlength_orig_date,le.maxlength_exp_date,le.maxlength_degree);
  SELF.avelength := CHOOSE(C,le.avelength_fname,le.avelength_lname,le.avelength_title,le.avelength_company,le.avelength_address1,le.avelength_address2,le.avelength_city,le.avelength_state,le.avelength_zip,le.avelength_zip4,le.avelength_cart,le.avelength_bar,le.avelength_gender,le.avelength_country,le.avelength_postal_cd,le.avelength_dpv,le.avelength_addr_type,le.avelength_age,le.avelength_county_cd,le.avelength_household_income,le.avelength_marital_status,le.avelength_msa,le.avelength_nielsen_county_cd,le.avelength_nurse_type,le.avelength_reg_state,le.avelength_specialty,le.avelength_email,le.avelength_phone,le.avelength_list_id,le.avelength_scno,le.avelength_keycode,le.avelength_custno,le.avelength_license_no,le.avelength_dob,le.avelength_orig_date,le.avelength_exp_date,le.avelength_degree);
END;
...
SALT37.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT37.StrType)le.fname),TRIM((SALT37.StrType)le.lname),TRIM((SALT37.StrType)le.title),TRIM((SALT37.StrType)le.company),TRIM((SALT37.StrType)le.address1),TRIM((SALT37.StrType)le.address2),TRIM((SALT37.StrType)le.city),TRIM((SALT37.StrType)le.state),TRIM((SALT37.StrType)le.zip),TRIM((SALT37.StrType)le.zip4),TRIM((SALT37.StrType)le.cart),TRIM((SALT37.StrType)le.bar),TRIM((SALT37.StrType)le.gender),TRIM((SALT37.StrType)le.country),TRIM((SALT37.StrType)le.postal_cd),TRIM((SALT37.StrType)le.dpv),TRIM((SALT37.StrType)le.addr_type),TRIM((SALT37.StrType)le.age),TRIM((SALT37.StrType)le.county_cd),TRIM((SALT37.StrType)le.household_income),TRIM((SALT37.StrType)le.marital_status),TRIM((SALT37.StrType)le.msa),TRIM((SALT37.StrType)le.nielsen_county_cd),TRIM((SALT37.StrType)le.nurse_type),TRIM((SALT37.StrType)le.reg_state),TRIM((SALT37.StrType)le.specialty),TRIM((SALT37.StrType)le.email),TRIM((SALT37.StrType)le.phone),TRIM((SALT37.StrType)le.list_id),TRIM((SALT37.StrType)le.scno),TRIM((SALT37.StrType)le.keycode),TRIM((SALT37.StrType)le.custno),TRIM((SALT37.StrType)le.license_no),TRIM((SALT37.StrType)le.dob),TRIM((SALT37.StrType)le.orig_date),TRIM((SALT37.StrType)le.exp_date),TRIM((SALT37.StrType)le.degree)));
  SELF.FldNo := C;
END;
...
SHARED FldIds := DATASET([{1,'fname'}
      ...
      ,{35,'orig_date'}
      ,{36,'exp_date'}
      ,{37,'degree'}],SALT37.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_age((SALT37.StrType)le.age),
    Fields.InValid_county_cd((SALT37.StrType)le.county_cd),
    Fields.InValid_household_income((SALT37.StrType)le.household_income),
    Fields.InValid_marital_status((SALT37.StrType)le.marital_status),
    Fields.InValid_msa((SALT37.StrType)le.msa),
    Fields.InValid_nielsen_county_cd((SALT37.StrType)le.nielsen_county_cd),
    Fields.InValid_nurse_type((SALT37.StrType)le.nurse_type),
    Fields.InValid_reg_state((SALT37.StrType)le.reg_state),
    ...
    Fields.InValid_license_no((SALT37.StrType)le.license_no,(SALT37.StrType)le.reg_state),

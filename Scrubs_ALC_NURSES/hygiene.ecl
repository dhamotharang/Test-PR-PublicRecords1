...
EXPORT Summary(SALT32.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_age := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.age)));
    avelength_age := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.age)),h.age<>(typeof(h.age))'');
    ...
    maxlength_household_income := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.household_income)));
    avelength_household_income := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.household_income)),h.household_income<>(typeof(h.household_income))'');
    populated_marital_status_pcnt := AVE(GROUP,IF(h.marital_status = (TYPEOF(h.marital_status))'',0,100));
    maxlength_marital_status := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.marital_status)));
    avelength_marital_status := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.marital_status)),h.marital_status<>(typeof(h.marital_status))'');
    ...
    maxlength_nurse_type := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.nurse_type)));
    avelength_nurse_type := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.nurse_type)),h.nurse_type<>(typeof(h.nurse_type))'');
    populated_reg_state_pcnt := AVE(GROUP,IF(h.reg_state = (TYPEOF(h.reg_state))'',0,100));
    maxlength_reg_state := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.reg_state)));
    avelength_reg_state := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.reg_state)),h.reg_state<>(typeof(h.reg_state))'');
...
SALT32.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT32.StrType)le.fname),TRIM((SALT32.StrType)le.lname),TRIM((SALT32.StrType)le.title),TRIM((SALT32.StrType)le.company),TRIM((SALT32.StrType)le.address1),TRIM((SALT32.StrType)le.address2),TRIM((SALT32.StrType)le.city),TRIM((SALT32.StrType)le.state),TRIM((SALT32.StrType)le.zip),TRIM((SALT32.StrType)le.zip4),TRIM((SALT32.StrType)le.cart),TRIM((SALT32.StrType)le.bar),TRIM((SALT32.StrType)le.gender),TRIM((SALT32.StrType)le.country),TRIM((SALT32.StrType)le.postal_cd),TRIM((SALT32.StrType)le.dpv),TRIM((SALT32.StrType)le.addr_type),TRIM((SALT32.StrType)le.age),TRIM((SALT32.StrType)le.county_cd),TRIM((SALT32.StrType)le.household_income),TRIM((SALT32.StrType)le.marital_status),TRIM((SALT32.StrType)le.msa),TRIM((SALT32.StrType)le.nielsen_county_cd),TRIM((SALT32.StrType)le.nurse_type),TRIM((SALT32.StrType)le.reg_state),TRIM((SALT32.StrType)le.specialty),TRIM((SALT32.StrType)le.email),TRIM((SALT32.StrType)le.phone),TRIM((SALT32.StrType)le.list_id),TRIM((SALT32.StrType)le.scno),TRIM((SALT32.StrType)le.keycode),TRIM((SALT32.StrType)le.custno),TRIM((SALT32.StrType)le.license_no),TRIM((SALT32.StrType)le.dob),TRIM((SALT32.StrType)le.orig_date),TRIM((SALT32.StrType)le.exp_date),TRIM((SALT32.StrType)le.degree)));
  SELF.FldNo := C;
END;
...
SHARED FldIds := DATASET([{1,'fname'}
      ...
      ,{35,'orig_date'}
      ,{36,'exp_date'}
      ,{37,'degree'}],SALT32.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_age((SALT32.StrType)le.age),
    Fields.InValid_county_cd((SALT32.StrType)le.county_cd),
    Fields.InValid_household_income((SALT32.StrType)le.household_income),
    Fields.InValid_marital_status((SALT32.StrType)le.marital_status),
    Fields.InValid_msa((SALT32.StrType)le.msa),
    Fields.InValid_nielsen_county_cd((SALT32.StrType)le.nielsen_county_cd),
    Fields.InValid_nurse_type((SALT32.StrType)le.nurse_type),
    Fields.InValid_reg_state((SALT32.StrType)le.reg_state),
    ...
    Fields.InValid_license_no((SALT32.StrType)le.license_no,(SALT32.StrType)le.reg_state),

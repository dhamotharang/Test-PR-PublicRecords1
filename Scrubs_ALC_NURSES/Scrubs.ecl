...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Layout_ALC_NURSES)
    ...
    UNSIGNED1 household_income_Invalid;
    UNSIGNED1 marital_status_Invalid;
    UNSIGNED1 msa_Invalid;
    UNSIGNED1 nielsen_county_cd_Invalid;
    UNSIGNED1 nurse_type_Invalid;
    UNSIGNED1 reg_state_Invalid;
    UNSIGNED1 specialty_Invalid;
  ...
  END;
  EXPORT  Bitmap_Layout := RECORD(Layout_ALC_NURSES)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(Layout_ALC_NURSES) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.age_Invalid := Fields.InValid_age((SALT32.StrType)le.age);
    SELF.county_cd_Invalid := Fields.InValid_county_cd((SALT32.StrType)le.county_cd);
    SELF.household_income_Invalid := Fields.InValid_household_income((SALT32.StrType)le.household_income);
    SELF.marital_status_Invalid := Fields.InValid_marital_status((SALT32.StrType)le.marital_status);
    ...
    SELF.nurse_type_Invalid := Fields.InValid_nurse_type((SALT32.StrType)le.nurse_type);
    SELF.reg_state_Invalid := Fields.InValid_reg_state((SALT32.StrType)le.reg_state);
    SELF.specialty_Invalid := Fields.InValid_specialty((SALT32.StrType)le.specialty);
    ...
    SELF.license_no_Invalid := Fields.InValid_license_no((SALT32.StrType)le.license_no,(SALT32.StrType)le.reg_state);
  ...
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Layout_ALC_NURSES);
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.fname_Invalid << 0 ) + ( le.lname_Invalid << 1 ) + ( le.title_Invalid << 2 ) + ( le.state_Invalid << 3 ) + ( le.zip_Invalid << 4 ) + ( le.zip4_Invalid << 5 ) + ( le.cart_Invalid << 6 ) + ( le.bar_Invalid << 7 ) + ( le.gender_Invalid << 8 ) + ( le.addr_type_Invalid << 9 ) + ( le.age_Invalid << 10 ) + ( le.county_cd_Invalid << 11 ) + ( le.household_income_Invalid << 12 ) + ( le.marital_status_Invalid << 13 ) + ( le.msa_Invalid << 14 ) + ( le.nielsen_county_cd_Invalid << 15 ) + ( le.nurse_type_Invalid << 16 ) + ( le.reg_state_Invalid << 17 ) + ( le.specialty_Invalid << 18 ) + ( le.phone_Invalid << 19 ) + ( le.list_id_Invalid << 20 ) + ( le.scno_Invalid << 21 ) + ( le.custno_Invalid << 22 ) + ( le.license_no_Invalid << 23 ) + ( le.dob_Invalid << 25 ) + ( le.orig_date_Invalid << 27 ) + ( le.exp_date_Invalid << 28 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,Layout_ALC_NURSES);
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.household_income_Invalid := (le.ScrubsBits1 >> 12) & 1;
    SELF.marital_status_Invalid := (le.ScrubsBits1 >> 13) & 1;
    SELF.msa_Invalid := (le.ScrubsBits1 >> 14) & 1;
    SELF.nielsen_county_cd_Invalid := (le.ScrubsBits1 >> 15) & 1;
    SELF.nurse_type_Invalid := (le.ScrubsBits1 >> 16) & 1;
    SELF.reg_state_Invalid := (le.ScrubsBits1 >> 17) & 1;
    SELF.specialty_Invalid := (le.ScrubsBits1 >> 18) & 1;
    SELF.phone_Invalid := (le.ScrubsBits1 >> 19) & 1;
    SELF.list_id_Invalid := (le.ScrubsBits1 >> 20) & 1;
    SELF.scno_Invalid := (le.ScrubsBits1 >> 21) & 1;
    SELF.custno_Invalid := (le.ScrubsBits1 >> 22) & 1;
    SELF.license_no_Invalid := (le.ScrubsBits1 >> 23) & 3;
    SELF.dob_Invalid := (le.ScrubsBits1 >> 25) & 3;
    SELF.orig_date_Invalid := (le.ScrubsBits1 >> 27) & 1;
    SELF.exp_date_Invalid := (le.ScrubsBits1 >> 28) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    household_income_ALLOW_ErrorCount := COUNT(GROUP,h.household_income_Invalid=1);
    marital_status_ENUM_ErrorCount := COUNT(GROUP,h.marital_status_Invalid=1);
    ...
    nurse_type_ALLOW_ErrorCount := COUNT(GROUP,h.nurse_type_Invalid=1);
    reg_state_ALLOW_ErrorCount := COUNT(GROUP,h.reg_state_Invalid=1);
    specialty_ALLOW_ErrorCount := COUNT(GROUP,h.specialty_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.fname_Invalid,le.lname_Invalid,le.title_Invalid,le.state_Invalid,le.zip_Invalid,le.zip4_Invalid,le.cart_Invalid,le.bar_Invalid,le.gender_Invalid,le.addr_type_Invalid,le.age_Invalid,le.county_cd_Invalid,le.household_income_Invalid,le.marital_status_Invalid,le.msa_Invalid,le.nielsen_county_cd_Invalid,le.nurse_type_Invalid,le.reg_state_Invalid,le.specialty_Invalid,le.phone_Invalid,le.list_id_Invalid,le.scno_Invalid,le.custno_Invalid,le.license_no_Invalid,le.dob_Invalid,le.orig_date_Invalid,le.exp_date_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_fname(le.fname_Invalid),Fields.InvalidMessage_lname(le.lname_Invalid),Fields.InvalidMessage_title(le.title_Invalid),Fields.InvalidMessage_state(le.state_Invalid),Fields.InvalidMessage_zip(le.zip_Invalid),Fields.InvalidMessage_zip4(le.zip4_Invalid),Fields.InvalidMessage_cart(le.cart_Invalid),Fields.InvalidMessage_bar(le.bar_Invalid),Fields.InvalidMessage_gender(le.gender_Invalid),Fields.InvalidMessage_addr_type(le.addr_type_Invalid),Fields.InvalidMessage_age(le.age_Invalid),Fields.InvalidMessage_county_cd(le.county_cd_Invalid),Fields.InvalidMessage_household_income(le.household_income_Invalid),Fields.InvalidMessage_marital_status(le.marital_status_Invalid),Fields.InvalidMessage_msa(le.msa_Invalid),Fields.InvalidMessage_nielsen_county_cd(le.nielsen_county_cd_Invalid),Fields.InvalidMessage_nurse_type(le.nurse_type_Invalid),Fields.InvalidMessage_reg_state(le.reg_state_Invalid),Fields.InvalidMessage_specialty(le.specialty_Invalid),Fields.InvalidMessage_phone(le.phone_Invalid),Fields.InvalidMessage_list_id(le.list_id_Invalid),Fields.InvalidMessage_scno(le.scno_Invalid),Fields.InvalidMessage_custno(le.custno_Invalid),Fields.InvalidMessage_license_no(le.license_no_Invalid),Fields.InvalidMessage_dob(le.dob_Invalid),Fields.InvalidMessage_orig_date(le.orig_date_Invalid),Fields.InvalidMessage_exp_date(le.exp_date_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.household_income_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.marital_status_Invalid,'ENUM','UNKNOWN')
          ,CHOOSE(le.msa_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.nielsen_county_cd_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.nurse_type_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.reg_state_Invalid,'ALLOW','UNKNOWN')
          ,CHOOSE(le.specialty_Invalid,'ALLOW','UNKNOWN')
          ...
          ,CHOOSE(le.exp_date_Invalid,'ALLOW','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'fname','lname','title','state','zip','zip4','cart','bar','gender','addr_type','age','county_cd','household_income','marital_status','msa','nielsen_county_cd','nurse_type','reg_state','specialty','phone','list_id','scno','custno','license_no','dob','orig_date','exp_date','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_alphaspacequote','invalid_alphaspacequote','invalid_alphaspace','invalid_alpha','invalid_numeric','invalid_numeric','invalid_alphanumeric','invalid_numeric','invalid_gender','invalid_addr_type','invalid_numeric','invalid_alphanumeric','invalid_numeric','invalid_marital_status','invalid_numeric','invalid_alphanumeric','invalid_numericpound','invalid_alphapound','invalid_numericpound','invalid_numeric','invalid_numeric','invalid_numeric','invalid_alphanumeric','invalid_license_number','invalid_date','invalid_slashO_date','invalid_slashO_date','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT32.StrType)le.fname,(SALT32.StrType)le.lname,(SALT32.StrType)le.title,(SALT32.StrType)le.state,(SALT32.StrType)le.zip,(SALT32.StrType)le.zip4,(SALT32.StrType)le.cart,(SALT32.StrType)le.bar,(SALT32.StrType)le.gender,(SALT32.StrType)le.addr_type,(SALT32.StrType)le.age,(SALT32.StrType)le.county_cd,(SALT32.StrType)le.household_income,(SALT32.StrType)le.marital_status,(SALT32.StrType)le.msa,(SALT32.StrType)le.nielsen_county_cd,(SALT32.StrType)le.nurse_type,(SALT32.StrType)le.reg_state,(SALT32.StrType)le.specialty,(SALT32.StrType)le.phone,(SALT32.StrType)le.list_id,(SALT32.StrType)le.scno,(SALT32.StrType)le.custno,(SALT32.StrType)le.license_no,(SALT32.StrType)le.dob,(SALT32.StrType)le.orig_date,(SALT32.StrType)le.exp_date,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT32.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'household_income:invalid_numeric:ALLOW'
          ,'marital_status:invalid_marital_status:ENUM'
          ,'msa:invalid_numeric:ALLOW'
          ,'nielsen_county_cd:invalid_alphanumeric:ALLOW'
          ,'nurse_type:invalid_numericpound:ALLOW'
          ,'reg_state:invalid_alphapound:ALLOW'
          ,'specialty:invalid_numericpound:ALLOW'
          ...
          ,'exp_date:invalid_slashO_date:ALLOW','UNKNOWN');
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_household_income(1)
          ,Fields.InvalidMessage_marital_status(1)
          ,Fields.InvalidMessage_msa(1)
          ,Fields.InvalidMessage_nielsen_county_cd(1)
          ,Fields.InvalidMessage_nurse_type(1)
          ,Fields.InvalidMessage_reg_state(1)
          ,Fields.InvalidMessage_specialty(1)
          ...
          ,Fields.InvalidMessage_exp_date(1),'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.household_income_ALLOW_ErrorCount
          ,le.marital_status_ENUM_ErrorCount
          ,le.msa_ALLOW_ErrorCount
          ,le.nielsen_county_cd_ALLOW_ErrorCount
          ,le.nurse_type_ALLOW_ErrorCount
          ,le.reg_state_ALLOW_ErrorCount
          ,le.specialty_ALLOW_ErrorCount
          ...
          ,le.exp_date_ALLOW_ErrorCount,0);
      SELF.rulepcnt := 100 * CHOOSE(c
          ...
          ,le.dob_ALLOW_ErrorCount,le.dob_CUSTOM_ErrorCount
          ,le.orig_date_ALLOW_ErrorCount
          ,le.exp_date_ALLOW_ErrorCount,0) / le.TotalCnt + 0.5;
    END;

...
EXPORT Proc_Build_Experian_CRDB_Keys(STRING pIndexVersion) := FUNCTION 

rKeyExperian_CRDB___Linkids :=RECORD
  unsigned6 ultid;
  unsigned6 orgid;
  unsigned6 seleid;
  unsigned6 proxid;
 ...
 ds_LinkIDs 	:= DATASET([], rKeyExperian_CRDB___LinkIDs);
 linkids_Ind	:= INDEX(ds_linkids, {ultid,orgid,seleid,proxid,powid,empid,dotid,ultscore,orgscore,selescore,proxscore,powscore,empscore,dotscore,ultweight,orgweight,seleweight,proxweight,powweight,empweight,dotweight}, {ds_linkids}, '~prte::key::experian_crdb::' + pIndexVersion + '::linkids');	
 RETURN SEQUENTIAL(BUILD(linkids_Ind, update),
									 PRTE.UpdateVersion('ExperianCRDBKeys',								        // Package name

import	_control, PRTE_CSV, PRTE2_Common, phonesplus_v2, doxie, cellphone,RoxieKeyBuild, Address, ut, autokey, NID;

// Mar 18, 2015 - adding the Alpharetta Base file into the build process.
// Default to doing DOPS updates because personnel are accustomed to this happening that way
// May 22, 2015 - had to add emailTo because we cannot sandbox myInfo when running in Production

export	Proc_Build_Phonesplusv2_Keys (string filedate, boolean skipDOPS=FALSE, string emailTo='')	:= FUNCTION
		...
		boca_phonesplus		:= 	project(ge_data + PRTE_CSV.Phonesplus.dthor_data400__key__phonesplus__did, 
																transform(Phonesplus_v2.Layout_Phonesplus_Base, self.cellphoneidkey := [], self.did := (unsigned6) left.did,  self.src_all := phonesplus_v2.translation_codes.source_bitmap_code(left.src), self := left, self := []))(origname = 'orig_not scrambled');
		//---------------------- new Alpharetta data being appended ----------------------------------------
		alpha_did_data := PRTE_CSV.Phonesplus.Alpharetta_phonesplus_did;
		alpha_phonesplus	:= 	project(alpha_did_data,
																transform(Phonesplus_v2.Layout_Phonesplus_Base, self.cellphoneidkey := [], self.src_all := phonesplus_v2.translation_codes.source_bitmap_code(left.src),self := left, self := []));
		//---------------------- new Alpharetta data being appended ----------------------------------------

		f_phonesplus 			:= boca_phonesplus + alpha_phonesplus;
		//---------------------- from this point on, only change is to make DOPS optional ------------------ Mar 18, 2015
...
// ut.mac_suppress_by_phonetype(phones1,homephone,state,phones2,true,did);
		...
		return sequential(
					OUTPUT(f_phonesplus,,'~prct::audit::ct::phonesplus_base_all', overwrite);
					parallel(

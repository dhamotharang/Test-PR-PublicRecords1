...
EXPORT proc_CleanAddresses(dataset(layout_base2) basein) := FUNCTION

		fnBadAddress(string addr1, string addr2) :=
		 ...
		 recordof(basein) xPrep(basein l, integer c) := TRANSFORM


				SELF.Prepped_addr1            := STD.Str.CleanSpaces(choose(c
																					,trim(L.Physical_Street1)
																					+' '+TRIM(L.Physical_Street2)
																					
																					,trim(L.Mailing_Street1)
																					+' '+TRIM(L.Mailing_Street2)
																					));
				SELF.Prepped_addr2            := STD.Str.CleanSpaces(choose(c
																					,trim(L.Physical_City) + if(L.Physical_City <> '',',','')
																					+ ' '+ L.Physical_State
																					+ ' '+ L.Physical_Zip
																					
																					,trim(L.Mailing_City) + if(L.Mailing_City <> '',',','')
																					+ ' '+ L.Mailing_State
																					+ ' '+ L.Mailing_Zip
																					));
		...
		prepped := NORMALIZE(basein, 2, xPrep(left, counter));
		
		UniqueAddresses := dedup(
									prepped(
										Prepped_addr1<>''
										,Prepped_addr2<>''
										,trim(regexreplace(Mod_Sets.RegexBadAddress,Std.Str.CleanSpaces(Prepped_addr1),'$2',nocase))
															not in Mod_Sets.SetBadAddress
										,trim(regexreplace(Mod_Sets.RegexBadAddress,Std.Str.CleanSpaces(Prepped_addr2),'$2',nocase))
															not in Mod_Sets.SetBadAddress
													),Prepped_addr1,Prepped_addr2,all);

		NAC_V2.layout_Base2 tr4(UniqueAddresses l) := transform
		...
		restored := join(distribute(prepped,  hash(Prepped_addr1,Prepped_addr2))

...
NAC_v2.Layouts.Collisions tr(inf_ddp l, inf_ddp r) := transform
			...
			:= if((l.fname_field=r.fname_field
						...
						or l.lname_field=r.LastName),'N',
						if((l.lname_field=r.lname_field
							or l.lname_field=r.LastName
							or l.LastName=r.LastName)
							and
			...
			self.ClientEligibleStatusIndicator := l.Eligibility;
			self.ActivityType := '4';
			...
			self.SearchEligibilityStatus := fn_left(l.Eligibility,r.Eligibility);
			...
			self.ClientEligibilityStatus := fn_right(l.Eligibility,r.Eligibility);
...
match:=join(inf_ddp,inf_ddp,
		...
		//left.Eligibility=right.Eligibility and
		//left.ProgramCode=right.ProgramCode and
...
//		left.ProgramState<>right.ProgramState) and
		...
		/***
			collision options:
				...
				3) informational: state1=state2 and beneft1<>benefit2
		***/
		(left.ProgramState<>right.ProgramState
			OR
			(left.ProgramState=right.ProgramState and 
				left.ProgramCode=right.ProgramCode
				and left.Case_Identifier<>right.Case_Identifier)
			OR
			(left.ProgramState=right.ProgramState and 
				left.ProgramCode<>right.ProgramCode)
		) and

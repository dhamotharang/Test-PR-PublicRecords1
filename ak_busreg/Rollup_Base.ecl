...
function
   pDataset_Dist := distribute(pDataset, hash(rawfields.LicenseNumber));
   pDataset_sort := sort(pDataset_Dist ,except 
      ...
      ,record_type
      ,rawfields.CurrentIssueDate
      ,clean_dates.CurrentIssueDate
      ,local
   );
   ...
   dnotfull_sort        := sort  (pDataset_rollup, rawfields.LicenseNumber, local );
   dnotfull_group       := group(dnotfull_sort  , rawfields.LicenseNumber, local  );

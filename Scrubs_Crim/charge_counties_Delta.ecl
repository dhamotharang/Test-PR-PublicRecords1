IMPORT SALT311,STD;
EXPORT charge_counties_Delta(DATASET(charge_counties_Layout_crim)old_s, DATASET(charge_counties_Layout_crim) new_s) := MODULE//Routines to compute the differences between two instances of a file
  ...
  EXPORT DifferenceSummary(BOOLEAN Glob = TRUE) := charge_counties_hygiene(old_s).Summary('Old',Glob) + charge_counties_hygiene(new_s).Summary('New',Glob) + charge_counties_hygiene(PROJECT(Differences(deleted), TRANSFORM(charge_counties_Layout_crim, SELF := LEFT.old_rec))).Summary('Deletions',Glob) + charge_counties_hygiene(PROJECT(Differences(added), TRANSFORM(charge_counties_Layout_crim, SELF := LEFT.new_rec))).Summary('Additions',Glob);
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE, BOOLEAN doHygieneSummaryPerSrc = TRUE) := FUNCTION
    ...
    SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_Crim, charge_counties_Fields, 'RECORDOF(hygieneDiffOverall)', TRUE);

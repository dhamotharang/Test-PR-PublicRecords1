...
EXPORT charge_doc_Scrubs := MODULE
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(charge_doc_Layout_crim)
  ...
  END;
  EXPORT  Bitmap_Layout := RECORD(charge_doc_Layout_crim)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(charge_doc_Layout_crim) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.recordid_Invalid := charge_doc_Fields.InValid_recordid((SALT33.StrType)le.recordid);
    SELF.statecode_Invalid := charge_doc_Fields.InValid_statecode((SALT33.StrType)le.statecode);
    SELF.caseid_Invalid := charge_doc_Fields.InValid_caseid((SALT33.StrType)le.caseid);
    SELF.arrestdate_Invalid := charge_doc_Fields.InValid_arrestdate((SALT33.StrType)le.arrestdate);
    SELF.bookingdate_Invalid := charge_doc_Fields.InValid_bookingdate((SALT33.StrType)le.bookingdate);
    SELF.custodydate_Invalid := charge_doc_Fields.InValid_custodydate((SALT33.StrType)le.custodydate);
    SELF.initialchargedate_Invalid := charge_doc_Fields.InValid_initialchargedate((SALT33.StrType)le.initialchargedate);
    SELF.chargedisposeddate_Invalid := charge_doc_Fields.InValid_chargedisposeddate((SALT33.StrType)le.chargedisposeddate);
    SELF.amendedchargedate_Invalid := charge_doc_Fields.InValid_amendedchargedate((SALT33.StrType)le.amendedchargedate);
    SELF.sourceid_Invalid := charge_doc_Fields.InValid_sourceid((SALT33.StrType)le.sourceid);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),charge_doc_Layout_crim);
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,charge_doc_Layout_crim);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,charge_doc_Fields.InvalidMessage_recordid(le.recordid_Invalid),charge_doc_Fields.InvalidMessage_statecode(le.statecode_Invalid),charge_doc_Fields.InvalidMessage_caseid(le.caseid_Invalid),charge_doc_Fields.InvalidMessage_arrestdate(le.arrestdate_Invalid),charge_doc_Fields.InvalidMessage_bookingdate(le.bookingdate_Invalid),charge_doc_Fields.InvalidMessage_custodydate(le.custodydate_Invalid),charge_doc_Fields.InvalidMessage_initialchargedate(le.initialchargedate_Invalid),charge_doc_Fields.InvalidMessage_chargedisposeddate(le.chargedisposeddate_Invalid),charge_doc_Fields.InvalidMessage_amendedchargedate(le.amendedchargedate_Invalid),charge_doc_Fields.InvalidMessage_sourceid(le.sourceid_Invalid),'UNKNOWN'));
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD()) := FUNCTION
    SALT33.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ,charge_doc_Fields.InvalidMessage_recordid(1)
          ,charge_doc_Fields.InvalidMessage_statecode(1)
          ,charge_doc_Fields.InvalidMessage_caseid(1)
          ,charge_doc_Fields.InvalidMessage_arrestdate(1)
          ,charge_doc_Fields.InvalidMessage_bookingdate(1)
          ,charge_doc_Fields.InvalidMessage_custodydate(1)
          ,charge_doc_Fields.InvalidMessage_initialchargedate(1)
          ,charge_doc_Fields.InvalidMessage_chargedisposeddate(1)
          ,charge_doc_Fields.InvalidMessage_amendedchargedate(1)
          ,charge_doc_Fields.InvalidMessage_sourceid(1),'UNKNOWN');

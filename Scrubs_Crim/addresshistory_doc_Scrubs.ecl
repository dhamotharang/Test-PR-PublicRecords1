...
// The module to handle the case where no scrubs exist
  ...
  END;
  EXPORT Rule_Layout := RECORD(addresshistory_doc_Layout_crim)
  ...
  SHARED toErrorMessage(UNSIGNED c) := CHOOSE(c
          ...
          ,addresshistory_doc_Fields.InvalidMessage_sourceid(1)
          ,'Fields with errors'
...
EXPORT FromNone(DATASET(addresshistory_doc_Layout_crim) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.recordid_Invalid := addresshistory_doc_Fields.InValid_recordid((SALT311.StrType)le.recordid);
    SELF.statecode_Invalid := addresshistory_doc_Fields.InValid_statecode((SALT311.StrType)le.statecode);
    SELF.city_Invalid := addresshistory_doc_Fields.InValid_city((SALT311.StrType)le.city);
    SELF.orig_zip_Invalid := addresshistory_doc_Fields.InValid_orig_zip((SALT311.StrType)le.orig_zip);
    SELF.sourceid_Invalid := addresshistory_doc_Fields.InValid_sourceid((SALT311.StrType)le.sourceid);
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h, BOOLEAN Glob = FALSE) := MODULE
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(addresshistory_doc_Layout_crim) prevDS = DATASET([], addresshistory_doc_Layout_crim)):= FUNCTION
  // field error stats
    ...
    mod_hygiene := addresshistory_doc_hygiene(PROJECT(h, addresshistory_doc_Layout_crim));
  ...
  // record count stats
    ...
    mod_Delta := addresshistory_doc_Delta(prevDS, PROJECT(h, addresshistory_doc_Layout_crim));
...
EXPORT StandardStats(DATASET(addresshistory_doc_Layout_crim) inFile, BOOLEAN doErrorOverall = TRUE, BOOLEAN doErrorPerSrc = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Crim, addresshistory_doc_Fields, 'RECORDOF(scrubsSummaryOverall)', 'vendor');

IMPORT ut,SALT33;
EXPORT address_history_arrests_Scrubs := MODULE
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(address_history_arrests_Layout_crim)
  ...
  END;
  EXPORT  Bitmap_Layout := RECORD(address_history_arrests_Layout_crim)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(address_history_arrests_Layout_crim) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.recordid_Invalid := address_history_arrests_Fields.InValid_recordid((SALT33.StrType)le.recordid);
    SELF.statecode_Invalid := address_history_arrests_Fields.InValid_statecode((SALT33.StrType)le.statecode);
    SELF.city_Invalid := address_history_arrests_Fields.InValid_city((SALT33.StrType)le.city);
    SELF.orig_zip_Invalid := address_history_arrests_Fields.InValid_orig_zip((SALT33.StrType)le.orig_zip);
    SELF.sourceid_Invalid := address_history_arrests_Fields.InValid_sourceid((SALT33.StrType)le.sourceid);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),address_history_arrests_Layout_crim);
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,address_history_arrests_Layout_crim);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,address_history_arrests_Fields.InvalidMessage_recordid(le.recordid_Invalid),address_history_arrests_Fields.InvalidMessage_statecode(le.statecode_Invalid),address_history_arrests_Fields.InvalidMessage_city(le.city_Invalid),address_history_arrests_Fields.InvalidMessage_orig_zip(le.orig_zip_Invalid),address_history_arrests_Fields.InvalidMessage_sourceid(le.sourceid_Invalid),'UNKNOWN'));
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD()) := FUNCTION
    SALT33.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ,address_history_arrests_Fields.InvalidMessage_recordid(1)
          ,address_history_arrests_Fields.InvalidMessage_statecode(1)
          ,address_history_arrests_Fields.InvalidMessage_city(1)
          ,address_history_arrests_Fields.InvalidMessage_orig_zip(1)
          ,address_history_arrests_Fields.InvalidMessage_sourceid(1),'UNKNOWN');

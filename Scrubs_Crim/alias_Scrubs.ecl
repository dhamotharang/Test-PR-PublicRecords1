...
EXPORT alias_Scrubs := MODULE
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(alias_Layout_crim)
  ...
  END;
  EXPORT  Bitmap_Layout := RECORD(alias_Layout_crim)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(alias_Layout_crim) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.recordid_Invalid := alias_Fields.InValid_recordid((SALT33.StrType)le.recordid);
    SELF.statecode_Invalid := alias_Fields.InValid_statecode((SALT33.StrType)le.statecode);
    SELF.akaname_Invalid := alias_Fields.InValid_akaname((SALT33.StrType)le.akaname);
    SELF.akalastname_Invalid := alias_Fields.InValid_akalastname((SALT33.StrType)le.akalastname);
    SELF.akafirstname_Invalid := alias_Fields.InValid_akafirstname((SALT33.StrType)le.akafirstname);
    SELF.akamiddlename_Invalid := alias_Fields.InValid_akamiddlename((SALT33.StrType)le.akamiddlename);
    SELF.akadob_Invalid := alias_Fields.InValid_akadob((SALT33.StrType)le.akadob);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),alias_Layout_crim);
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,alias_Layout_crim);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,alias_Fields.InvalidMessage_recordid(le.recordid_Invalid),alias_Fields.InvalidMessage_statecode(le.statecode_Invalid),alias_Fields.InvalidMessage_akaname(le.akaname_Invalid),alias_Fields.InvalidMessage_akalastname(le.akalastname_Invalid),alias_Fields.InvalidMessage_akafirstname(le.akafirstname_Invalid),alias_Fields.InvalidMessage_akamiddlename(le.akamiddlename_Invalid),alias_Fields.InvalidMessage_akadob(le.akadob_Invalid),'UNKNOWN'));
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD()) := FUNCTION
    SALT33.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ,alias_Fields.InvalidMessage_recordid(1)
          ,alias_Fields.InvalidMessage_statecode(1)
          ,alias_Fields.InvalidMessage_akaname(1)
          ,alias_Fields.InvalidMessage_akalastname(1)
          ,alias_Fields.InvalidMessage_akafirstname(1)
          ,alias_Fields.InvalidMessage_akamiddlename(1)
          ,alias_Fields.InvalidMessage_akadob(1),'UNKNOWN');

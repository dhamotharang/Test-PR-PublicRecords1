...
// The module to handle the case where no scrubs exist
  ...
  END;
  EXPORT Rule_Layout := RECORD(defendant_counties_Layout_crim)
  ...
  SHARED toErrorMessage(UNSIGNED c) := CHOOSE(c
          ...
          ,defendant_counties_Fields.InvalidMessage_sexoffenderregexpirationdate(1)
          ,'Fields with errors'
...
EXPORT FromNone(DATASET(defendant_counties_Layout_crim) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.recordid_Invalid := defendant_counties_Fields.InValid_recordid((SALT311.StrType)le.recordid);
    SELF.statecode_Invalid := defendant_counties_Fields.InValid_statecode((SALT311.StrType)le.statecode);
    SELF.inmatenumber_Invalid := defendant_counties_Fields.InValid_inmatenumber((SALT311.StrType)le.inmatenumber);
    SELF.dob_Invalid := defendant_counties_Fields.InValid_dob((SALT311.StrType)le.dob);
    SELF.birthcity_Invalid := defendant_counties_Fields.InValid_birthcity((SALT311.StrType)le.birthcity);
    SELF.gender_Invalid := defendant_counties_Fields.InValid_gender((SALT311.StrType)le.gender);
    SELF.race_Invalid := defendant_counties_Fields.InValid_race((SALT311.StrType)le.race);
    SELF.dlstate_Invalid := defendant_counties_Fields.InValid_dlstate((SALT311.StrType)le.dlstate);
    SELF.city_Invalid := defendant_counties_Fields.InValid_city((SALT311.StrType)le.city);
    SELF.orig_zip_Invalid := defendant_counties_Fields.InValid_orig_zip((SALT311.StrType)le.orig_zip);
    SELF.institutionreceiptdate_Invalid := defendant_counties_Fields.InValid_institutionreceiptdate((SALT311.StrType)le.institutionreceiptdate);
    SELF.deceaseddate_Invalid := defendant_counties_Fields.InValid_deceaseddate((SALT311.StrType)le.deceaseddate);
    SELF.sexoffenderregistrydate_Invalid := defendant_counties_Fields.InValid_sexoffenderregistrydate((SALT311.StrType)le.sexoffenderregistrydate);
    SELF.sexoffenderregexpirationdate_Invalid := defendant_counties_Fields.InValid_sexoffenderregexpirationdate((SALT311.StrType)le.sexoffenderregexpirationdate);
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h, BOOLEAN Glob = FALSE) := MODULE
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(defendant_counties_Layout_crim) prevDS = DATASET([], defendant_counties_Layout_crim)):= FUNCTION
  // field error stats
    ...
    mod_hygiene := defendant_counties_hygiene(PROJECT(h, defendant_counties_Layout_crim));
  ...
  // record count stats
    ...
    mod_Delta := defendant_counties_Delta(prevDS, PROJECT(h, defendant_counties_Layout_crim));
...
EXPORT StandardStats(DATASET(defendant_counties_Layout_crim) inFile, BOOLEAN doErrorOverall = TRUE, BOOLEAN doErrorPerSrc = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Crim, defendant_counties_Fields, 'RECORDOF(scrubsSummaryOverall)', 'vendor');

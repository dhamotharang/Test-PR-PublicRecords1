...
EXPORT FromNone(DATASET(defendant_Layout_crim) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.recordid_Invalid := defendant_Fields.InValid_recordid((SALT33.StrType)le.recordid);
    SELF.statecode_Invalid := defendant_Fields.InValid_statecode((SALT33.StrType)le.statecode);
    SELF.inmatenumber_Invalid := defendant_Fields.InValid_inmatenumber((SALT33.StrType)le.inmatenumber);
    SELF.dob_Invalid := defendant_Fields.InValid_dob((SALT33.StrType)le.dob);
    SELF.birthcity_Invalid := defendant_Fields.InValid_birthcity((SALT33.StrType)le.birthcity);
    SELF.gender_Invalid := defendant_Fields.InValid_gender((SALT33.StrType)le.gender);
    SELF.race_Invalid := defendant_Fields.InValid_race((SALT33.StrType)le.race);
    SELF.dlstate_Invalid := defendant_Fields.InValid_dlstate((SALT33.StrType)le.dlstate);
    SELF.city_Invalid := defendant_Fields.InValid_city((SALT33.StrType)le.city);
    SELF.orig_zip_Invalid := defendant_Fields.InValid_orig_zip((SALT33.StrType)le.orig_zip);
    SELF.institutionreceiptdate_Invalid := defendant_Fields.InValid_institutionreceiptdate((SALT33.StrType)le.institutionreceiptdate);
    SELF.deceaseddate_Invalid := defendant_Fields.InValid_deceaseddate((SALT33.StrType)le.deceaseddate);
    SELF.sexoffenderregistrydate_Invalid := defendant_Fields.InValid_sexoffenderregistrydate((SALT33.StrType)le.sexoffenderregistrydate);
    SELF.sexoffenderregexpirationdate_Invalid := defendant_Fields.InValid_sexoffenderregexpirationdate((SALT33.StrType)le.sexoffenderregexpirationdate);
    SELF := le;
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD()) := FUNCTION
    SALT33.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,defendant_Fields.InvalidMessage_sexoffenderregexpirationdate(1),'UNKNOWN');

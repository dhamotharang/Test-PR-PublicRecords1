...
EXPORT FromNone(DATASET(alias_counties_Layout_crim) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.recordid_Invalid := alias_counties_Fields.InValid_recordid((SALT33.StrType)le.recordid);
    SELF.statecode_Invalid := alias_counties_Fields.InValid_statecode((SALT33.StrType)le.statecode);
    SELF.akaname_Invalid := alias_counties_Fields.InValid_akaname((SALT33.StrType)le.akaname);
    SELF.akalastname_Invalid := alias_counties_Fields.InValid_akalastname((SALT33.StrType)le.akalastname);
    SELF.akafirstname_Invalid := alias_counties_Fields.InValid_akafirstname((SALT33.StrType)le.akafirstname);
    SELF.akamiddlename_Invalid := alias_counties_Fields.InValid_akamiddlename((SALT33.StrType)le.akamiddlename);
    SELF.akadob_Invalid := alias_counties_Fields.InValid_akadob((SALT33.StrType)le.akadob);
    SELF := le;
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD()) := FUNCTION
    SALT33.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,alias_counties_Fields.InvalidMessage_akadob(1),'UNKNOWN');

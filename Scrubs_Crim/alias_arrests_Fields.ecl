...
EXPORT alias_arrests_Fields := MODULE
// Processing for each FieldType
EXPORT SALT33.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'Invalid_Record_ID','Invalid_State','Invalid_Date','AKA_Search');
EXPORT FieldTypeNum(SALT33.StrType fn) := CASE(fn,'Invalid_Record_ID' => 1,'Invalid_State' => 2,'Invalid_Date' => 3,'AKA_Search' => 4,0);
...
EXPORT MakeFT_AKA_Search(SALT33.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_AKA_Search(SALT33.StrType s) := WHICH(~Scrubs_Crim.fn_FindAKA(s)>0);
EXPORT InValidMessageFT_AKA_Search(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.CustomFail('Scrubs_Crim.fn_FindAKA'),SALT33.HygieneErrors.Good);
EXPORT SALT33.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'recordid','statecode','akaname','akalastname','akafirstname','akamiddlename','akasuffix','akadob','sourcename','vendor');
EXPORT FieldNum(SALT33.StrType fn) := CASE(fn,'recordid' => 0,'statecode' => 1,'akaname' => 2,'akalastname' => 3,'akafirstname' => 4,'akamiddlename' => 5,'akasuffix' => 6,'akadob' => 7,'sourcename' => 8,'vendor' => 9,0);
...
EXPORT Make_akaname(SALT33.StrType s0) := MakeFT_AKA_Search(s0);
EXPORT InValid_akaname(SALT33.StrType s) := InValidFT_AKA_Search(s);
EXPORT InValidMessage_akaname(UNSIGNED1 wh) := InValidMessageFT_AKA_Search(wh);
EXPORT Make_akalastname(SALT33.StrType s0) := MakeFT_AKA_Search(s0);
EXPORT InValid_akalastname(SALT33.StrType s) := InValidFT_AKA_Search(s);
EXPORT InValidMessage_akalastname(UNSIGNED1 wh) := InValidMessageFT_AKA_Search(wh);
EXPORT Make_akafirstname(SALT33.StrType s0) := MakeFT_AKA_Search(s0);
EXPORT InValid_akafirstname(SALT33.StrType s) := InValidFT_AKA_Search(s);
EXPORT InValidMessage_akafirstname(UNSIGNED1 wh) := InValidMessageFT_AKA_Search(wh);
EXPORT Make_akamiddlename(SALT33.StrType s0) := MakeFT_AKA_Search(s0);
EXPORT InValid_akamiddlename(SALT33.StrType s) := InValidFT_AKA_Search(s);
EXPORT InValidMessage_akamiddlename(UNSIGNED1 wh) := InValidMessageFT_AKA_Search(wh);
EXPORT Make_akasuffix(SALT33.StrType s0) := s0;
EXPORT InValid_akasuffix(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_akasuffix(UNSIGNED1 wh) := '';
EXPORT Make_akadob(SALT33.StrType s0) := MakeFT_Invalid_Date(s0);
EXPORT InValid_akadob(SALT33.StrType s) := InValidFT_Invalid_Date(s);

IMPORT SALT311,STD;
EXPORT addresshistory_Scrubs := MODULE
...
// The module to handle the case where no scrubs exist
  ...
  EXPORT Expanded_Layout := RECORD(addresshistory_Layout_crim)
  ...
  END;
  EXPORT  Bitmap_Layout := RECORD(addresshistory_Layout_crim)
    UNSIGNED8 ScrubsBits1;
  END;
  EXPORT Rule_Layout := RECORD(addresshistory_Layout_crim)
  ...
  SHARED toErrorMessage(UNSIGNED c) := CHOOSE(c
          ,addresshistory_Fields.InvalidMessage_recordid(1)
          ,addresshistory_Fields.InvalidMessage_statecode(1)
          ,addresshistory_Fields.InvalidMessage_city(1)
          ,addresshistory_Fields.InvalidMessage_orig_zip(1)
          ,addresshistory_Fields.InvalidMessage_sourceid(1)
          ,'Fields with errors'
...
EXPORT FromNone(DATASET(addresshistory_Layout_crim) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.recordid_Invalid := addresshistory_Fields.InValid_recordid((SALT311.StrType)le.recordid);
    SELF.statecode_Invalid := addresshistory_Fields.InValid_statecode((SALT311.StrType)le.statecode);
    SELF.city_Invalid := addresshistory_Fields.InValid_city((SALT311.StrType)le.city);
    SELF.orig_zip_Invalid := addresshistory_Fields.InValid_orig_zip((SALT311.StrType)le.orig_zip);
    SELF.sourceid_Invalid := addresshistory_Fields.InValid_sourceid((SALT311.StrType)le.sourceid);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),addresshistory_Layout_crim);
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,addresshistory_Layout_crim);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h, BOOLEAN Glob = FALSE) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,addresshistory_Fields.InvalidMessage_recordid(le.recordid_Invalid),addresshistory_Fields.InvalidMessage_statecode(le.statecode_Invalid),addresshistory_Fields.InvalidMessage_city(le.city_Invalid),addresshistory_Fields.InvalidMessage_orig_zip(le.orig_zip_Invalid),addresshistory_Fields.InvalidMessage_sourceid(le.sourceid_Invalid),'UNKNOWN'));
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(addresshistory_Layout_crim) prevDS = DATASET([], addresshistory_Layout_crim)):= FUNCTION
  // field error stats
    ...
    mod_hygiene := addresshistory_hygiene(PROJECT(h, addresshistory_Layout_crim));
  ...
  // record count stats
    ...
    mod_Delta := addresshistory_Delta(prevDS, PROJECT(h, addresshistory_Layout_crim));
...
EXPORT StandardStats(DATASET(addresshistory_Layout_crim) inFile, BOOLEAN doErrorOverall = TRUE, BOOLEAN doErrorPerSrc = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Crim, addresshistory_Fields, 'RECORDOF(scrubsSummaryOverall)', 'vendor');

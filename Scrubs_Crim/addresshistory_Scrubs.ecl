...
EXPORT FromNone(DATASET(addresshistory_Layout_crim) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.recordid_Invalid := addresshistory_Fields.InValid_recordid((SALT33.StrType)le.recordid);
    SELF.statecode_Invalid := addresshistory_Fields.InValid_statecode((SALT33.StrType)le.statecode);
    SELF.city_Invalid := addresshistory_Fields.InValid_city((SALT33.StrType)le.city);
    SELF.orig_zip_Invalid := addresshistory_Fields.InValid_orig_zip((SALT33.StrType)le.orig_zip);
    SELF.sourceid_Invalid := addresshistory_Fields.InValid_sourceid((SALT33.StrType)le.sourceid);
    SELF := le;
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD()) := FUNCTION
    SALT33.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,addresshistory_Fields.InvalidMessage_sourceid(1),'UNKNOWN');

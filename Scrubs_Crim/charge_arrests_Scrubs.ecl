...
EXPORT FromNone(DATASET(charge_arrests_Layout_crim) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.recordid_Invalid := charge_arrests_Fields.InValid_recordid((SALT33.StrType)le.recordid);
    SELF.statecode_Invalid := charge_arrests_Fields.InValid_statecode((SALT33.StrType)le.statecode);
    SELF.caseid_Invalid := charge_arrests_Fields.InValid_caseid((SALT33.StrType)le.caseid);
    SELF.arrestdate_Invalid := charge_arrests_Fields.InValid_arrestdate((SALT33.StrType)le.arrestdate);
    SELF.bookingdate_Invalid := charge_arrests_Fields.InValid_bookingdate((SALT33.StrType)le.bookingdate);
    SELF.custodydate_Invalid := charge_arrests_Fields.InValid_custodydate((SALT33.StrType)le.custodydate);
    SELF.initialchargedate_Invalid := charge_arrests_Fields.InValid_initialchargedate((SALT33.StrType)le.initialchargedate);
    SELF.chargedisposeddate_Invalid := charge_arrests_Fields.InValid_chargedisposeddate((SALT33.StrType)le.chargedisposeddate);
    SELF.amendedchargedate_Invalid := charge_arrests_Fields.InValid_amendedchargedate((SALT33.StrType)le.amendedchargedate);
    SELF.sourceid_Invalid := charge_arrests_Fields.InValid_sourceid((SALT33.StrType)le.sourceid);
    SELF := le;
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD()) := FUNCTION
    SALT33.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,charge_arrests_Fields.InvalidMessage_sourceid(1),'UNKNOWN');

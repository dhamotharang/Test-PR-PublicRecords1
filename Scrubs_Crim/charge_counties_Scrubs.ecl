...
EXPORT charge_counties_Scrubs := MODULE
...
// The module to handle the case where no scrubs exist
  ...
  EXPORT Expanded_Layout := RECORD(charge_counties_Layout_crim)
  ...
  END;
  EXPORT  Bitmap_Layout := RECORD(charge_counties_Layout_crim)
    UNSIGNED8 ScrubsBits1;
  END;
  EXPORT Rule_Layout := RECORD(charge_counties_Layout_crim)
  ...
  SHARED toErrorMessage(UNSIGNED c) := CHOOSE(c
          ,charge_counties_Fields.InvalidMessage_recordid(1)
          ,charge_counties_Fields.InvalidMessage_statecode(1)
          ,charge_counties_Fields.InvalidMessage_caseid(1)
          ,charge_counties_Fields.InvalidMessage_arrestdate(1)
          ,charge_counties_Fields.InvalidMessage_bookingdate(1)
          ,charge_counties_Fields.InvalidMessage_custodydate(1)
          ,charge_counties_Fields.InvalidMessage_initialchargedate(1)
          ,charge_counties_Fields.InvalidMessage_chargedisposeddate(1)
          ,charge_counties_Fields.InvalidMessage_amendedchargedate(1)
          ,charge_counties_Fields.InvalidMessage_sourceid(1)
          ,'Fields with errors'
...
EXPORT FromNone(DATASET(charge_counties_Layout_crim) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    SELF.recordid_Invalid := charge_counties_Fields.InValid_recordid((SALT311.StrType)le.recordid);
    SELF.statecode_Invalid := charge_counties_Fields.InValid_statecode((SALT311.StrType)le.statecode);
    SELF.caseid_Invalid := charge_counties_Fields.InValid_caseid((SALT311.StrType)le.caseid);
    SELF.arrestdate_Invalid := charge_counties_Fields.InValid_arrestdate((SALT311.StrType)le.arrestdate);
    SELF.bookingdate_Invalid := charge_counties_Fields.InValid_bookingdate((SALT311.StrType)le.bookingdate);
    SELF.custodydate_Invalid := charge_counties_Fields.InValid_custodydate((SALT311.StrType)le.custodydate);
    SELF.initialchargedate_Invalid := charge_counties_Fields.InValid_initialchargedate((SALT311.StrType)le.initialchargedate);
    SELF.chargedisposeddate_Invalid := charge_counties_Fields.InValid_chargedisposeddate((SALT311.StrType)le.chargedisposeddate);
    SELF.amendedchargedate_Invalid := charge_counties_Fields.InValid_amendedchargedate((SALT311.StrType)le.amendedchargedate);
    SELF.sourceid_Invalid := charge_counties_Fields.InValid_sourceid((SALT311.StrType)le.sourceid);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),charge_counties_Layout_crim);
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,charge_counties_Layout_crim);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h, BOOLEAN Glob = FALSE) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,charge_counties_Fields.InvalidMessage_recordid(le.recordid_Invalid),charge_counties_Fields.InvalidMessage_statecode(le.statecode_Invalid),charge_counties_Fields.InvalidMessage_caseid(le.caseid_Invalid),charge_counties_Fields.InvalidMessage_arrestdate(le.arrestdate_Invalid),charge_counties_Fields.InvalidMessage_bookingdate(le.bookingdate_Invalid),charge_counties_Fields.InvalidMessage_custodydate(le.custodydate_Invalid),charge_counties_Fields.InvalidMessage_initialchargedate(le.initialchargedate_Invalid),charge_counties_Fields.InvalidMessage_chargedisposeddate(le.chargedisposeddate_Invalid),charge_counties_Fields.InvalidMessage_amendedchargedate(le.amendedchargedate_Invalid),charge_counties_Fields.InvalidMessage_sourceid(le.sourceid_Invalid),'UNKNOWN'));
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10, UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(), DATASET(charge_counties_Layout_crim) prevDS = DATASET([], charge_counties_Layout_crim)):= FUNCTION
  // field error stats
    ...
    mod_hygiene := charge_counties_hygiene(PROJECT(h, charge_counties_Layout_crim));
  ...
  // record count stats
    ...
    mod_Delta := charge_counties_Delta(prevDS, PROJECT(h, charge_counties_Layout_crim));
...
EXPORT StandardStats(DATASET(charge_counties_Layout_crim) inFile, BOOLEAN doErrorOverall = TRUE, BOOLEAN doErrorPerSrc = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Crim, charge_counties_Fields, 'RECORDOF(scrubsSummaryOverall)', 'vendor');

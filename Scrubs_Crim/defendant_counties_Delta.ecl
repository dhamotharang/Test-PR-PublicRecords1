IMPORT SALT311,STD;
EXPORT defendant_counties_Delta(DATASET(defendant_counties_Layout_crim)old_s, DATASET(defendant_counties_Layout_crim) new_s) := MODULE//Routines to compute the differences between two instances of a file
  ...
  EXPORT DifferenceSummary(BOOLEAN Glob = TRUE) := defendant_counties_hygiene(old_s).Summary('Old',Glob) + defendant_counties_hygiene(new_s).Summary('New',Glob) + defendant_counties_hygiene(PROJECT(Differences(deleted), TRANSFORM(defendant_counties_Layout_crim, SELF := LEFT.old_rec))).Summary('Deletions',Glob) + defendant_counties_hygiene(PROJECT(Differences(added), TRANSFORM(defendant_counties_Layout_crim, SELF := LEFT.new_rec))).Summary('Additions',Glob);
  EXPORT StandardStats(BOOLEAN doHygieneSummaryGlobal = TRUE, BOOLEAN doHygieneSummaryPerSrc = TRUE) := FUNCTION
    ...
    SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_Crim, defendant_counties_Fields, 'RECORDOF(hygieneDiffOverall)', TRUE);

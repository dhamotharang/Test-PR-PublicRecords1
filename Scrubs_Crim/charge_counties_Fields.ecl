...
EXPORT charge_counties_Fields := MODULE
// Processing for each FieldType
EXPORT SALT33.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'Invalid_Record_ID','Invalid_State','Invalid_Case_ID','Invalid_Current_Date','Invalid_Future_Date','Invalid_Source_ID');
EXPORT FieldTypeNum(SALT33.StrType fn) := CASE(fn,'Invalid_Record_ID' => 1,'Invalid_State' => 2,'Invalid_Case_ID' => 3,'Invalid_Current_Date' => 4,'Invalid_Future_Date' => 5,'Invalid_Source_ID' => 6,0);
...
EXPORT MakeFT_Invalid_Case_ID(SALT33.StrType s0) := FUNCTION
  s1 := SALT33.stringfilter(s0,'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_Case_ID(SALT33.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-'))));
EXPORT InValidMessageFT_Invalid_Case_ID(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-'),SALT33.HygieneErrors.Good);
...
EXPORT MakeFT_Invalid_Source_ID(SALT33.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_Invalid_Source_ID(SALT33.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT33.StringFilter(s,'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'))));
EXPORT InValidMessageFT_Invalid_Source_ID(UNSIGNED1 wh) := CHOOSE(wh,SALT33.HygieneErrors.NotInChars('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'),SALT33.HygieneErrors.Good);
...
EXPORT Make_caseid(SALT33.StrType s0) := MakeFT_Invalid_Case_ID(s0);
EXPORT InValid_caseid(SALT33.StrType s) := InValidFT_Invalid_Case_ID(s);
EXPORT InValidMessage_caseid(UNSIGNED1 wh) := InValidMessageFT_Invalid_Case_ID(wh);
EXPORT Make_warrantnumber(SALT33.StrType s0) := s0;
EXPORT InValid_warrantnumber(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_warrantnumber(UNSIGNED1 wh) := '';
EXPORT Make_warrantdate(SALT33.StrType s0) := s0;
EXPORT InValid_warrantdate(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_warrantdate(UNSIGNED1 wh) := '';
EXPORT Make_warrantdesc(SALT33.StrType s0) := s0;
EXPORT InValid_warrantdesc(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_warrantdesc(UNSIGNED1 wh) := '';
EXPORT Make_warrantissuedate(SALT33.StrType s0) := s0;
EXPORT InValid_warrantissuedate(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_warrantissuedate(UNSIGNED1 wh) := '';
EXPORT Make_warrantissuingagency(SALT33.StrType s0) := s0;
EXPORT InValid_warrantissuingagency(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_warrantissuingagency(UNSIGNED1 wh) := '';
EXPORT Make_warrantstatus(SALT33.StrType s0) := s0;
EXPORT InValid_warrantstatus(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_warrantstatus(UNSIGNED1 wh) := '';
EXPORT Make_citationnumber(SALT33.StrType s0) := s0;
EXPORT InValid_citationnumber(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_citationnumber(UNSIGNED1 wh) := '';
EXPORT Make_bookingnumber(SALT33.StrType s0) := s0;
EXPORT InValid_bookingnumber(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_bookingnumber(UNSIGNED1 wh) := '';
EXPORT Make_arrestdate(SALT33.StrType s0) := MakeFT_Invalid_Current_Date(s0);
EXPORT InValid_arrestdate(SALT33.StrType s) := InValidFT_Invalid_Current_Date(s);
EXPORT InValidMessage_arrestdate(UNSIGNED1 wh) := InValidMessageFT_Invalid_Current_Date(wh);
EXPORT Make_arrestingagency(SALT33.StrType s0) := s0;
EXPORT InValid_arrestingagency(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_arrestingagency(UNSIGNED1 wh) := '';
EXPORT Make_bookingdate(SALT33.StrType s0) := MakeFT_Invalid_Future_Date(s0);
EXPORT InValid_bookingdate(SALT33.StrType s) := InValidFT_Invalid_Future_Date(s);
EXPORT InValidMessage_bookingdate(UNSIGNED1 wh) := InValidMessageFT_Invalid_Future_Date(wh);
EXPORT Make_custodydate(SALT33.StrType s0) := MakeFT_Invalid_Current_Date(s0);
EXPORT InValid_custodydate(SALT33.StrType s) := InValidFT_Invalid_Current_Date(s);
EXPORT InValidMessage_custodydate(UNSIGNED1 wh) := InValidMessageFT_Invalid_Current_Date(wh);
EXPORT Make_custodylocation(SALT33.StrType s0) := s0;
EXPORT InValid_custodylocation(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_custodylocation(UNSIGNED1 wh) := '';
EXPORT Make_initialcharge(SALT33.StrType s0) := s0;
EXPORT InValid_initialcharge(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_initialcharge(UNSIGNED1 wh) := '';
EXPORT Make_initialchargedate(SALT33.StrType s0) := MakeFT_Invalid_Current_Date(s0);
EXPORT InValid_initialchargedate(SALT33.StrType s) := InValidFT_Invalid_Current_Date(s);
EXPORT InValidMessage_initialchargedate(UNSIGNED1 wh) := InValidMessageFT_Invalid_Current_Date(wh);
EXPORT Make_initialchargecancelleddate(SALT33.StrType s0) := s0;
EXPORT InValid_initialchargecancelleddate(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_initialchargecancelleddate(UNSIGNED1 wh) := '';
EXPORT Make_chargedisposed(SALT33.StrType s0) := s0;
EXPORT InValid_chargedisposed(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_chargedisposed(UNSIGNED1 wh) := '';
EXPORT Make_chargedisposeddate(SALT33.StrType s0) := MakeFT_Invalid_Future_Date(s0);
EXPORT InValid_chargedisposeddate(SALT33.StrType s) := InValidFT_Invalid_Future_Date(s);
EXPORT InValidMessage_chargedisposeddate(UNSIGNED1 wh) := InValidMessageFT_Invalid_Future_Date(wh);
EXPORT Make_chargeseverity(SALT33.StrType s0) := s0;
EXPORT InValid_chargeseverity(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_chargeseverity(UNSIGNED1 wh) := '';
EXPORT Make_chargedisposition(SALT33.StrType s0) := s0;
EXPORT InValid_chargedisposition(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_chargedisposition(UNSIGNED1 wh) := '';
EXPORT Make_amendedcharge(SALT33.StrType s0) := s0;
EXPORT InValid_amendedcharge(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_amendedcharge(UNSIGNED1 wh) := '';
EXPORT Make_amendedchargedate(SALT33.StrType s0) := MakeFT_Invalid_Current_Date(s0);
EXPORT InValid_amendedchargedate(SALT33.StrType s) := InValidFT_Invalid_Current_Date(s);
EXPORT InValidMessage_amendedchargedate(UNSIGNED1 wh) := InValidMessageFT_Invalid_Current_Date(wh);
EXPORT Make_bondsman(SALT33.StrType s0) := s0;
EXPORT InValid_bondsman(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_bondsman(UNSIGNED1 wh) := '';
EXPORT Make_bondamount(SALT33.StrType s0) := s0;
EXPORT InValid_bondamount(SALT33.StrType s) := FALSE;
EXPORT InValidMessage_bondamount(UNSIGNED1 wh) := '';
EXPORT Make_bondtype(SALT33.StrType s0) := s0;
EXPORT InValid_bondtype(SALT33.StrType s) := FALSE;
...
EXPORT Make_sourceid(SALT33.StrType s0) := MakeFT_Invalid_Source_ID(s0);
EXPORT InValid_sourceid(SALT33.StrType s) := InValidFT_Invalid_Source_ID(s);

import Inquiry_AccLogs, Data_Services, ut, doxie, std;
...
filt := Inquiry_AccLogs.File_Inquiry_Base.fileFull(search_info.product_code not in inquiry_acclogs.fnCleanFunctions.ProductCode_FilterCds, person_q.Appended_ADL > 0, StringLib.StringToUpperCase(trim(search_info.function_description)) != 'MODELS.ITA_BATCH_SERVICE' );
...
account_opens := [
	...
	'INSTANT IDÂ® CONSUMER W/ RED FLAGS'
];
...
layout_final calc( dist le ) := TRANSFORM
	...
	boolean isCollections := trim(le.vertical) in Inquiry_AccLogs.shell_constants.collections_vertical_set;
	...
	ageBucket := map(
		...
		days <= 90  => 3,
		days <= 180 => 6,
		days <= iid_constants.oneyear    => 12,
		days <= iid_constants.twoyears   => 24,
		days <= iid_constants.threeyears => 36,
		days <= iid_constants.fiveyears  => 60,
...
export Key_Inquiry_Table_DID := index( rolled, {did}, {rolled}, Data_Services.Data_Location.Prefix('Inquiry')+'thor_data400::key::inquiry_table_did_' + doxie.Version_SuperKey);
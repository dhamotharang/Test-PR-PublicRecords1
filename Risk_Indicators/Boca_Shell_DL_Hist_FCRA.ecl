...
EXPORT Boca_Shell_DL_Hist_FCRA (GROUPED DATASET(Layout_Output) iid,
                                boolean isLN, unsigned1 dppa, boolean dppa_ok,
																unsigned3 history_date) := FUNCTION

kdl  := driversv2.key_dl_did_FCRA;
kdln := driversv2.key_dl_number_FCRA;
...
dlrec get_dl_num(iid le, STRING14 dlnum, BOOLEAN dlmatch) := TRANSFORM
	...
	SELF := [];
END;
dl_id := GROUP (SORT (GROUP (JOIN (iid, kdl,
...
dlrec get_dl_recs_FCRA (dlrec le, kdln ri) := TRANSFORM

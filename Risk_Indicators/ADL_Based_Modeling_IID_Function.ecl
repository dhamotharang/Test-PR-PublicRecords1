import _Control, dx_Gong, suppress, riskwise, ut, doxie, FCRA, gateway, risk_indicators, data_services, STD;
...
key_history_did := dx_Gong.key_history_did(data_environment);
...
with_gong_did1_roxie_unsuppressed := join(best_of_header, key_history_did,
...
with_gong_did1_thor_unsuppressed := join(distribute(best_of_header, hash64(did)),
									distribute(pull(key_history_did), hash64(did)),

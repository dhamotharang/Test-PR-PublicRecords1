import _Control, email_data, riskwise, ut, fcra, mdr, risk_indicators, dx_Email;
...
// key for did lookup
ekey_dids_roxie := join(clam_pre_email_in, dx_email.Key_Did(isFCRA),
...
ekey_dids_thor := join(distribute(clam_pre_email_in, hash(shell_input.did)), 
                distribute(pull(dx_Email.Key_Did(isFCRA)), hash(did)), 
                left.shell_input.did=right.did,
...
// key for clean_email lookup
email_keysv1 := Email_Data.Key_Email_Address();

email_keysv2_roxie := join(clam_pre_email_in, dx_Email.Key_Email_Address(), 
...
email_keysv2_thor := join(distribute(clam_pre_email_in, hash(shell_input.email_address)), 
                distribute(pull(dx_Email.Key_Email_Address()), hash(clean_email)), 
                left.shell_input.email_address=right.clean_email,
...
#IF(onThor)        
                email_join := email_join_thor;
#ELSE
                email_join := email_join_roxie;
#END 

correct_keys := JOIN(email_join, email_keysv1, left.clean_email= right.clean_email, INNER);
elim_duplicate_keys := JOIN(email_join, correct_keys, left.did = right.did, LEFT ONLY);
corrected_emails := elim_duplicate_keys + correct_keys;

// put EmailDataV2Keys into EmailDataKeys format
RECORDOF(Email_data.Layout_Email.Base) emailTransform(corrected_emails L) := TRANSFORM
  SELF.rec_src_all := Email_data.Translation_Codes.source_bitmap_code(L.email_src);
  SELF.email_src_all := SELF.rec_src_all;
  SELF.email_src_num := 1;
  SELF.best_ssn := L.clean_ssn;
  SELF.best_dob := L.clean_dob;
  SELF := L;
END;

// Rollups to make EmailDataV2Keys look like EmailDataKeys
projected_email_data := PROJECT(corrected_emails, emailTransform(left));
//append_best_data := Email_data.Fn_Append_Best (projected_email_data);
rollup_email_orig := Email_data.Fn_Rollup_Email_Data_orig(projected_email_data);
rollup_email_new := Email_data.FN_Rollup_Email_Data(rollup_email_orig);
emailaddr_key := dedup(sort(distribute(rollup_email_new, hash(did)) ,clean_email,did, if(clean_name.lname <> '' and clean_address.prim_range <> '' and clean_name.fname <> '' and clean_name.mname <> '', 1, if(clean_name.lname <>  '',  2, 3)), -date_last_seen, local),clean_email,did,local);
...
email_recs_nonfcra_minus_behavior_roxie := join(clam_pre_email, ekey,
                                                                                                left.did<>0 and 
...
email_recs_nonfcra_roxie := join(clam_pre_email, ekey,
                                                                                                left.did<>0 and 
                                                                                                //keyed(left.did=right.did) 
                                                                                                left.did=right.did
                                                                                                and right.email_src IN setsources                                                                                                                                                                                                                                                                                             
...
with_reverse_email_lookup_roxie := join(clam_pre_email, emailaddr_key,

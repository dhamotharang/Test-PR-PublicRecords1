...
layout_with_FDN_attributes :=	RECORD
	risk_indicators.Layout_Boca_Shell_Edina_v55;
...
export ToEdina_55( dataset(layout_internal) bs, boolean isFCRA=false, string dataRestrictionMask, string Intended_Purpose ) := FUNCTION

		temp convertToEdina_v55(bs le) := transform
			self.file_ind := le.file_ind;
	
#else

export ToEdina_55( dataset(riskprocessing.layouts.layout_internal_shell) bs, boolean isFCRA=false, string dataRestrictionMask, string Intended_Purpose ) := FUNCTION
	risk_indicators.Layout_Boca_Shell_Edina_v55 convertToEdina_v55(bs le) := transform
...
// blank these out for 5.0 FCRA shell	
	...
	edina_v55 := project( bs, convertToEdina_v55(left) );
	...
	with_riskview_attributes := join(edina_v55, rv5_attributes, left.seq=right.seq,
		transform(risk_indicators.Layout_Boca_Shell_Edina_v55, 
	...
	edina55_final := if(isFCRA, with_riskview_attributes, edina_v55);

...
END;
	...
	temp := record
			 string20 file_ind;
			 layout_with_FDN_attributes;
	end;

export ToEdina_v51( dataset(layout_internal_shell_noDatasets) bs, boolean isFCRA=false, string dataRestrictionMask ) := FUNCTION

		temp convertToEdina_v51(bs le) := transform
			self.file_ind := le.file_ind;
	
#else

export ToEdina_v51( dataset(riskprocessing.layouts.layout_internal_shell_noDatasets) bs, boolean isFCRA=false, string dataRestrictionMask ) := FUNCTION
	layout_with_FDN_attributes convertToEdina_v51(bs le) := transform
	...
	self.address_verification.address_history_2.avm_land_use_code := le.avm.address_history_2.avm_land_use_code;
	self.address_verification.address_history_2.avm_recording_date := le.avm.address_history_2.avm_recording_date;
	self.address_verification.address_history_2.avm_assessed_value_year := le.avm.address_history_2.avm_assessed_value_year;
	self.address_verification.address_history_2.avm_sales_price := le.avm.address_history_2.avm_sales_price;  
	self.address_verification.address_history_2.avm_assessed_total_value := le.avm.address_history_2.avm_assessed_total_value;
	self.address_verification.address_history_2.avm_market_total_value := le.avm.address_history_2.avm_market_total_value;
	self.address_verification.address_history_2.avm_tax_assessment_valuation := le.avm.address_history_2.avm_tax_assessment_valuation;
	self.address_verification.address_history_2.avm_price_index_valuation := le.avm.address_history_2.avm_price_index_valuation;
	self.address_verification.address_history_2.avm_hedonic_valuation := le.avm.address_history_2.avm_hedonic_valuation;
...
// blank these out for 5.0 FCRA shell	
		...
		//populate the FP3 FDN model output fields (this is also temporary placement until the real BS 5.1 layout is created)
		self.fraudpoint_V3_fdn 								:= le.FD_Scores.fraudpoint_V3_fdn;  
		self.StolenIdentityIndex_V3_fdn				:= le.FD_Scores.StolenIdentityIndex_V3_fdn;  
		self.SyntheticIdentityIndex_V3_fdn		:= le.FD_Scores.SyntheticIdentityIndex_V3_fdn;  
		self.ManipulatedIdentityIndex_V3_fdn	:= le.FD_Scores.ManipulatedIdentityIndex_V3_fdn;  
		self.VulnerableVictimIndex_V3_fdn			:= le.FD_Scores.VulnerableVictimIndex_V3_fdn;  
		self.FriendlyFraudIndex_V3_fdn				:= le.FD_Scores.FriendlyFraudIndex_V3_fdn;  
		self.SuspiciousActivityIndex_V3_fdn		:= le.FD_Scores.SuspiciousActivityIndex_V3_fdn;  
		self.reason1FP_V3_fdn									:= le.FD_Scores.reason1FP_V3_fdn;  
		self.reason2FP_V3_fdn									:= le.FD_Scores.reason2FP_V3_fdn;  
		self.reason3FP_V3_fdn									:= le.FD_Scores.reason3FP_V3_fdn;  
		self.reason4FP_V3_fdn									:= le.FD_Scores.reason4FP_V3_fdn;  
		self.reason5FP_V3_fdn									:= le.FD_Scores.reason5FP_V3_fdn;  
		self.reason6FP_V3_fdn									:= le.FD_Scores.reason6FP_V3_fdn;  
			
	self := le;
	end;

	edina_v51 := project( bs, convertToEdina_v51(left) );
	
	
	clam := project(bs, transform(risk_indicators.Layout_Boca_Shell, self := left, self := []));
	
	rv5_attributes := riskview.get_attributes_v5(group(clam,seq), false);
		
	with_riskview_attributes := join(edina_v51, rv5_attributes, left.seq=right.seq,
	...
	edina51_final := if(isFCRA, with_riskview_attributes, edina_v51);

...
EXPORT getAllBocaShellData_AML (
  ...
  boolean isPreScreen=false, boolean doScore=false, boolean filter_out_fares=false
) := FUNCTION
...
RelatRecProp := join(ids_wide,   single_property_relat,
  ...
  // =============== Vehicles ===============
  vehicles := Risk_Indicators.Boca_Shell_Vehicles(ids_only, mod_access.dppa, mod_access.isValidDppa(), includeRelativeInfo, BSversion);
  vehicles_hist := Risk_Indicators.Boca_Shell_Vehicles_Hist(ids_only, mod_access.dppa, mod_access.isValidDppa(), includeRelativeInfo, BSversion, mod_access);
 ...
 IncludeLnJ := false;
  ...
  derogs_hist := IF (IsFCRA,
                     ...
                     Risk_Indicators.Boca_Shell_Derogs_Hist      (if(BSversion>2,ids_only_mult_dids, ids_only_derogs), bsversion, mod_access));
  ...
  proflic := IF (IsFCRA,
                      ...
                      Risk_Indicators.Boca_Shell_Proflic      (ids_only(~isrelat), bsversion,
                                                               mod_access.lexid_source_optout,
                                                               mod_access.transaction_id,
                                                               , //not needed
                                                               mod_access.global_company_id));
...
relatEmpl :=  AML.AMLEmployment(pre_ids_only, mod_aml);

//AML  degree
relatStudent := AML.AMLStudent(ids_only, mod_aml);
//  AML professional license
relatProfLic :=  AML.AMLProfLicense(ids_only, mod_aml);
...
relatAddr  := AML.AMLAddrAttrib(ssnFlagsPrepforAddr, mod_access);


//  AML  header info
relatHeader :=  AML.AMLGetHeader(group(RelatInput,seq), mod_aml, isFCRA);
...
relatParentPubRec := AML.AMLRelativesAssocs(group(RelatInfo, seq), mod_aml);
...
advo_rolled := boca_shell_advo(ids_wide(~isrelat), isFCRA, mod_access.datarestrictionmask, isPreScreen, bsversion);  // change to neutral layout
...
inquiries_rolled := boca_shell_inquiries(ids_wide(~isrelat), BSOptions, bsversion, gateways, mod_access.datapermissionMask, mod_access);
...
email_summary_rolled := boca_shell_email(ids_wide(~isrelat), isFCRA, bsversion, LastSeenThreshold, BSOptions, mod_access);
...
with_bus_header_summary := if(isFCRA, with_email_summary, boca_shell_Bus_header(with_email_summary, mod_access, bsversion)); // won't use business header on FCRA queries

...
TurnOffRelativeProperty := (BSOptions & risk_indicators.iid_constants.BSOptions.TurnOffRelativeProperty) > 0;
includeRelativeInfoProperty := includeRelativeInfo and ~TurnOffRelativeProperty;

	prop_common_roxie := Risk_Indicators.Boca_Shell_Property_Common( p_address, ids_only, includeRelativeInfoProperty, filter_out_fares, IsFCRA, in_mod_property, FALSE);
	prop_common_thor_pre := distribute(Risk_Indicators.Boca_Shell_Property_Common( p_address, ids_only, includeRelativeInfoProperty, filter_out_fares, IsFCRA, in_mod_property, FALSE), hash64(seq))	:	PERSIST('~BOCASHELLFCRA::bocashell_property_results', expire(3));
...
//  AML  header info
relatHeader :=  AML.AMLGetHeader(group(RelatInput,seq), mod_access, isFCRA);
...
withBIP_Header := Risk_Indicators.Boca_Shell_BIP_Header(finalOffset, Options, linkingOptions, AllowedSourcesSet, isFCRA, mod_access, BSVersion);

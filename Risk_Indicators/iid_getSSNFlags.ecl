import _Control, risk_indicators, suppress, doxie, address, riskwise, NID, FCRA, Death_Master, MDR, Relationship, dx_header, STD, data_services;
onThor := _Control.Environment.OnThor;

export iid_getSSNFlags(grouped DATASET(risk_indicators.Layout_output) inrec, doxie.IDataAccess mod_access,
...
{layout_output_DE_src, UNSIGNED4 global_sid} getDeathSSN (layout_output_DE_src le, deathSSNKey ri) := transform
...
got_SSNMap_roxie := join(ssn_table_results, SSNMapKey,
												(left.ssn!='' AND LENGTH(STD.Str.Filter(LEFT.ssn, '0123456789')) = 9 AND LENGTH(TRIM(LEFT.ssn)) = 9) and
...
got_SSNMap_thor_pre := join(distribute(ssn_table_results(ssn<>'' or ssn_correct_ffid <> []), hash64(ssn[1..5])),
												...
												LENGTH(STD.Str.Filter(LEFT.ssn, '0123456789')) = 9 AND LENGTH(TRIM(LEFT.ssn)) = 9 and

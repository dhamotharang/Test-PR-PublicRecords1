...
outfile := risk_indicators.Collection_Shell_MOD.getBestCleaned(best_prep_deduped, DataRestrictionMask, mod_access.glb, false);
...
best_inquiries := if(isFCRA, risk_indicators.Boca_Shell_Inquiries_FCRA(inquiries_prep, best_bsoptions, bsversion, best_gateways),
														risk_indicators.Boca_Shell_Inquiries(inquiries_prep, best_bsoptions, bsversion, best_gateways, mod_access.DataPermissionMask, mod_access) );
...
IID_best_flags := risk_indicators.get_IID_Best_Flags(iid_prep, mod_access, isFCRA, bsversion, bsoptions);

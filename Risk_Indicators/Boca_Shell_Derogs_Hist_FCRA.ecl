...
export Boca_Shell_Derogs_Hist_FCRA (GROUPED DATASET(layouts.layout_derogs_input) ids, 
	...
	Bankruptcy := Risk_Indicators.Boca_Shell_Bankrucpty_FCRAHist(bsVersion, BSOptions, ids, onThor);
	BankLiens := if(bsVersion >= 3,
					Risk_Indicators.Boca_Shell_Liens_FCRAHist_tmsid(bsVersion, BSOptions, Bankruptcy, onThor),
					Risk_Indicators.Boca_Shell_Liens_FCRAHist(bsVersion, BSOptions, Bankruptcy, onThor));
	BankLiensCrim := Risk_Indicators.Boca_Shell_Crim_FCRAHist(bsVersion, BSOptions, BankLiens, onThor);
	BankLiensCrimSO := Risk_Indicators.Boca_Shell_SO_FCRAHist(bsVersion, BSOptions, BankLiensCrim, onThor);
	...
	BankLiensCrimSO_LNJ :=	Risk_Indicators.Boca_Shell_Liens_LnJ_FCRA_Hist(bsVersion, BSOptions, ids_formatted, IncludeLnJ, ReportingPeriod, onThor);									

import header, ut, avm_v2, fcra, header_quick, doxie_build;
...
valid_header := if(isFCRA, headerprod_building(~fcra.Restricted_Header_Src(src, vendor_id[1])), headerprod_building);

combo := Address_Table_Function(valid_header);				// full header
eq := Address_Table_Function(valid_header(src<>'EN'));	// header for use in equifax products
en := Address_Table_Function(valid_header(src<>'EQ'));	// header for use in experian products
...
persist_name := IF (IsFCRA, 'persist::address_risk_v3_filtered', 'persist::address_risk_v3'); 

addr_rsk_tbl := join(addr_table_eqen, avm_distr, left.zip=right.zip and left.prim_range=right.prim_range and left.predir=right.predir and
											 left.prim_name=right.prim_name and left.suffix=right.suffix and left.postdir=right.postdir and left.sec_range=right.sec_range,
											transform(layout_addr_risk, 

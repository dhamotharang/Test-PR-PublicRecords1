...
export RVRetail_TestSeed_Function(dataset(Layout_Input) inData, string30 account_value, string20 TestDataTableName) := 
				
FUNCTION
	
	Test_Data_Table_Name := stringlib.stringtouppercase(TestDataTableName);
	
	layout_rvRetailSeq := RECORD
		unsigned4 seq;
		Models.Layout_Model;
	END;
	
	layout_rvRetailSeq create_output(inData le, Seed_Files.Key_RVRetail ri) := TRANSFORM
		...
		scores := dataset([{ri.score,ri.model_name,'4',reason_codes}], Models.Layout_Score);
		...
		self.seq := le.seq;
	END;
	RiskView_rec := join(inData, Seed_Files.Key_RVRetail,keyed(right.dataset_name=Test_Data_Table_Name) and 

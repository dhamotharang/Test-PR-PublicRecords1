...
export RVAuto_TestSeed_Function(dataset(Layout_Input) inData, string30 account_value, string20 TestDataTableName) := 
				
FUNCTION
	
	Test_Data_Table_Name := stringlib.stringtouppercase(TestDataTableName);
	
	layout_rvAutoSeq := RECORD
		unsigned4 seq;
		Models.Layout_Model;
	END;
	
	layout_rvAutoSeq create_output(inData le, Seed_Files.Key_RVAuto ri) := TRANSFORM
		...
		scores := dataset([{ri.score,ri.model_name,'2',reason_codes}], Models.Layout_Score);
		...
		self.seq := le.seq;
	END;
	RiskView_rec := join(inData, Seed_Files.Key_RVAuto,keyed(right.dataset_name=Test_Data_Table_Name) and 

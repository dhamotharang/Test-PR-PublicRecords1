/*2015-05-22T23:17:14Z (David Schlangen)
populated the new riskview5 attribute index and primary factor fields
...
includeADLFields := FALSE; // If you set this to TRUE, make sure you set it to TRUE in Risk_Indicators.Layout_Boca_Shell_Edina_v50!
...
export ToEdina_v50( dataset(layout_internal) bs, boolean isFCRA=false, string dataRestrictionMask ) := FUNCTION

		temp convertToEdina_v50(bs le) := transform
			self.file_ind := le.file_ind;
	
#else

export ToEdina_v50( dataset(riskprocessing.layouts.layout_internal_shell_noDatasets) bs, boolean isFCRA=false, string dataRestrictionMask ) := FUNCTION
	risk_indicators.Layout_Boca_Shell_Edina_v50 convertToEdina_v50(bs le) := transform
	
#end
		
// start of shell 5.0 new content
	self.bsversion := 50;
...
// blank these out for 5.0 FCRA shell	
	...
	edina_v50 := project( bs, convertToEdina_v50(left) );
	...
	with_riskview_attributes := join(edina_v50, rv5_attributes, left.seq=right.seq,
		transform(risk_indicators.Layout_Boca_Shell_Edina_v50, 
	...
	edina50_final := if(isFCRA, with_riskview_attributes, edina_v50);
	
	// output(bs, named('bs'));
	// output(edina_v50, named('edina_v50'));
	// output(rv5_attributes, named('rv5_attributes'));
	// output(with_riskview_attributes, named('with_riskview_attributes'));
		
	return edina50_final;
END;

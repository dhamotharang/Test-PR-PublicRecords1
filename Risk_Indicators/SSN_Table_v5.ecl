...
combined_bureau_table := risk_indicators.ssn_table5(h, 'ALL', isFCRA);

// create unique credit bureau tables for each bureau
equifax_table := risk_indicators.ssn_table5(h(src not in [mdr.sourceTools.src_Experian_Credit_Header, mdr.sourceTools.src_TU_CreditHeader]), 
																						mdr.sourceTools.src_Equifax, 
																						isFCRA);
																						
experian_table := risk_indicators.ssn_table5(h(src not in [mdr.sourceTools.src_Equifax, mdr.sourceTools.src_TU_CreditHeader]), 
																						mdr.sourceTools.src_Experian_Credit_Header, 
																						isFCRA);
																						
transunion_table := risk_indicators.ssn_table5(h(src not in [mdr.sourceTools.src_Equifax,mdr.sourceTools.src_Experian_Credit_Header]), 
...
rec_temp pull_data(rec_temp le, pull_file rt) := transform
	...
	*/
	...
	ssnHighIssue := IF(randomizedSocial, ut.getDate, TRIM((STRING)le.official_last_seen));
...
// put it back into original layout without the bans and death dids
  persist_name := IF (IsFCRA, 'persist::ssn_table_v5_filtered', 'persist::ssn_table_v5'); 

...
import risk_indicators, riskprocessing, riskview;
	...
	edina_plus_bob_v52 := record
			 string20 file_ind;
			 risk_indicators.iid_constants.adl_based_modeling_flags ADL_Shell_Flags;
			 risk_indicators.Layout_Boca_Shell_Edina_v52;
	end;

export ToEdina_52_ADL( dataset(layout_internal) bs, boolean isFCRA=false, string dataRestrictionMask, string Intended_Purpose ) := FUNCTION

		edina_plus_bob_v52 convertToEdina_v52(bs le) := transform
			self.file_ind := le.file_ind;
	
#else

export ToEdina_52_ADL( dataset(riskprocessing.layouts.layout_internal_shell) bs, boolean isFCRA=false, string dataRestrictionMask, string Intended_Purpose, boolean includeADLFields=false ) := FUNCTION

...
+    #IF( #TEXT(Input_corp_dissolved_date)='' )
      '' 
    #ELSE
        IF( (unsigned)le.Input_corp_dissolved_date = 0,'', ':corp_dissolved_date(' + SALT311.fn_date_valid_as_text((unsigned)le.Input_corp_dissolved_date) + ')' )
    #END
 
+    #IF( #TEXT(Input_corp_merger_date)='' )
      '' 
    #ELSE
        IF( (unsigned)le.Input_corp_merger_date = 0,'', ':corp_merger_date(' + SALT311.fn_date_valid_as_text((unsigned)le.Input_corp_merger_date) + ')' )
    #END

...
EXPORT SALT34.StrType FieldTypeName(UNSIGNED2 i) := CHOOSE(i,'invalid_date','invalid_optional_date','invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_charter','invalid_mandatory','invalid_name_type_cd','invalid_name_type_desc','invalid_corp_status_desc','invalid_forgn_dom_code','invalid_orig_org_structure_cd','invalid_for_profit_ind','invalid_recordOrigin','invalid_alphablank');
EXPORT FieldTypeNum(SALT34.StrType fn) := CASE(fn,'invalid_date' => 1,'invalid_optional_date' => 2,'invalid_corp_key' => 3,'invalid_corp_vendor' => 4,'invalid_state_origin' => 5,'invalid_charter' => 6,'invalid_mandatory' => 7,'invalid_name_type_cd' => 8,'invalid_name_type_desc' => 9,'invalid_corp_status_desc' => 10,'invalid_forgn_dom_code' => 11,'invalid_orig_org_structure_cd' => 12,'invalid_for_profit_ind' => 13,'invalid_recordOrigin' => 14,'invalid_alphablank' => 15,0);
...
EXPORT MakeFT_invalid_corp_key(SALT34.StrType s0) := FUNCTION
  ...
  RETURN  s1;
END;
EXPORT InValidFT_invalid_corp_key(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,'0123456789-'))),~(LENGTH(TRIM(s)) = 10 OR LENGTH(TRIM(s)) = 11 OR LENGTH(TRIM(s)) = 12));
EXPORT InValidMessageFT_invalid_corp_key(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars('0123456789-'),SALT34.HygieneErrors.NotLength('10,11,12'),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_corp_vendor(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_vendor(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['53']);
EXPORT InValidMessageFT_invalid_corp_vendor(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('53'),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_state_origin(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_state_origin(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['WA']);
EXPORT InValidMessageFT_invalid_state_origin(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('WA'),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_charter(SALT34.StrType s0) := FUNCTION
  s1 := SALT34.stringfilter(s0,'0123456789'); // Only allow valid symbols
  RETURN  s1;
END;
EXPORT InValidFT_invalid_charter(SALT34.StrType s) := WHICH(LENGTH(TRIM(s))<>LENGTH(TRIM(SALT34.StringFilter(s,'0123456789'))),~(LENGTH(TRIM(s)) = 7 OR LENGTH(TRIM(s)) = 8 OR LENGTH(TRIM(s)) = 9));
EXPORT InValidMessageFT_invalid_charter(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInChars('0123456789'),SALT34.HygieneErrors.NotLength('7,8,9'),SALT34.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_name_type_cd(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_name_type_cd(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['01','07',' ']);
EXPORT InValidMessageFT_invalid_name_type_cd(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('01|07| '),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_name_type_desc(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_name_type_desc(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['LEGAL','RESERVED',' ']);
EXPORT InValidMessageFT_invalid_name_type_desc(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('LEGAL|RESERVED| '),SALT34.HygieneErrors.Good);
 
EXPORT MakeFT_invalid_corp_status_desc(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_corp_status_desc(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['ACTIVE','INACTIVE','FEDERAL BANK',' ']);
EXPORT InValidMessageFT_invalid_corp_status_desc(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('ACTIVE|INACTIVE|FEDERAL BANK| '),SALT34.HygieneErrors.Good);
...
EXPORT MakeFT_invalid_orig_org_structure_cd(SALT34.StrType s0) := FUNCTION
  RETURN  s0;
END;
EXPORT InValidFT_invalid_orig_org_structure_cd(SALT34.StrType s) := WHICH(((SALT34.StrType) s) NOT IN ['3LP','AUT','BNK','CAS','COP','CRU','FBS','FMA','FRA','GRG','INS','LLC','LLP','LTD','MAS','MMC','PBC','PLC','PRO','PUB','REG','SAL','SOL','FMA','MAS','MIL','MIN','MMC','PBC','PLC','PRO','PUB','REG','RFN','SAL','SOL','SPC',' ']);
EXPORT InValidMessageFT_invalid_orig_org_structure_cd(UNSIGNED1 wh) := CHOOSE(wh,SALT34.HygieneErrors.NotInEnum('3LP|AUT|BNK|CAS|COP|CRU|FBS|FMA|FRA|GRG|INS|LLC|LLP|LTD|MAS|MMC|PBC|PLC|PRO|PUB|REG|SAL|SOL|FMA|MAS|MIL|MIN|MMC|PBC|PLC|PRO|PUB|REG|RFN|SAL|SOL|SPC| '),SALT34.HygieneErrors.Good);
...
EXPORT FieldNum(SALT34.StrType fn) := CASE(fn,'dt_vendor_first_reported' => 0,'dt_vendor_last_reported' => 1,'dt_first_seen' => 2,'dt_last_seen' => 3,'corp_ra_dt_first_seen' => 4,'corp_ra_dt_last_seen' => 5,'corp_process_date' => 6,'corp_inc_date' => 7,'corp_forgn_date' => 8,'corp_dissolved_date' => 9,'corp_key' => 10,'corp_orig_sos_charter_nbr' => 11,'corp_vendor' => 12,'corp_state_origin' => 13,'corp_legal_name' => 14,'corp_inc_state' => 15,'corp_forgn_state_desc' => 16,'corp_foreign_domestic_ind' => 17,'corp_for_profit_ind' => 18,'corp_ln_name_type_cd' => 19,'corp_ln_name_type_desc' => 20,'corp_orig_org_structure_cd' => 21,'corp_status_desc' => 22,'recordorigin' => 23,0);

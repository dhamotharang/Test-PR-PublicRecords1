...
ds_input_cleaned_pre := BusinessInstantID20_Services.fn_CleanInput(ds_input);

// Add two months to the HistoryDate to account for Cortera's lag in archive mode.
fn_AddTwoMonths(INTEGER histdate) := IF( histdate % 100 IN [11,12], histdate + 90, histdate + 2 );

ds_input_cleaned := 
  PROJECT(
    ds_input_cleaned_pre,
    TRANSFORM( BusinessInstantID20_Services.layouts.InputCompanyAndAuthRepInfo,
      SELF.HistoryDate := fn_AddTwoMonths( LEFT.HistoryDate ),
      SELF := LEFT
    )
  );
...
ds_LINKID_Input_File :=
  DEDUP(
    SORT(
      DISTRIBUTE( ds_LINKID_Input_File_pre, HASH32(ID1,ID2,LINKID,RetroDate) ),
      ID1,ID2,LINKID,RetroDate, LOCAL
    ),
    ID1,ID2,LINKID,RetroDate, LOCAL
  );

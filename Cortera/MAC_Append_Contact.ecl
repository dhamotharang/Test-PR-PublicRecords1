...
EXPORT MAC_Append_Contact (ds_in, ds_out, mod_access, append_contact) := MACRO
IMPORT Cortera, Suppress;

  #uniquename(ds_contacts_all)
  %ds_contacts_all% := JOIN(ds_in, Cortera.Key_Executive_Link_Id,
  ...
  %ds_contacts% := IF(append_contact, 
                      PROJECT(SORT(%ds_contacts_supp%, link_id, Persistent_record_id),Cortera.Layout_ExecLinkID), // to remove the _internal_fpos_ field from the layout
                      DATASET([], Cortera.Layout_ExecLinkID));

  #uniquename(get_exec_name)
  %get_exec_name%(Cortera.Layout_ExecLinkID r) := 
    TRIM(r.title) + ' ' + TRIM(r.fname) + ' ' + TRIM(r.mname) + ' ' + TRIM(r.lname) + ' ' + TRIM(r.name_suffix);

  ds_in xfm_add_contacts(ds_in l, DATASET(Cortera.Layout_ExecLinkID) r) := TRANSFORM

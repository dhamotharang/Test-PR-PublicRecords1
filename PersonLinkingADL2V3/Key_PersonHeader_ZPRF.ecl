/*2013-07-29T19:45:15Z (alima)
C:\Users\limaam\AppData\Roaming\HPCC Systems\eclide\alima\default\PersonLinkingADL2V3\Key_PersonHeader_ZPRF\2013-07-29T19_45_15Z.ecl
*/
/*2011-08-04T12:35:27Z (Jose Bello)
Bug: 84022
*/
...
export Key := index(DataForKey2,,ut.Data_Location.Person_header+'thor_data400::key::PersonLinkingADL2V3PersonHeaderZPRFRefs_' + doxie.version_superkey);
  ...
  Process_xADL2_Layouts.LayoutScoredFetch Score_Batch(Key le,recs ri) := transform
    ...
    self.DOBWeight_day := (50+MAP ( le.DOB_day = 0 => 0,
      ...
      0))/100;
		...
		numdobmatches := 
		if(self.dobweight_year > 0, 1, 0)
		+if(self.dobweight_month > 0, 1, 0)
		+if(self.dobweight_day > 0, 1, 0);			
			
    self.Weight :=
		if(self.LNAMEWeight = 0 and numdobmatches < 2, //this fixes these records from W20110721-112019: 14760,14839,14879
			skip,
			// 0,			
			self.ZIPWeight +self.PRIM_RANGEWeight +self.FNAMEWeight +self.LNAMEWeight +self.PRIM_NAMEWeight +self.SEC_RANGEWeight + self.DOBWeight_year + self.DOBWeight_month + self.DOBWeight_day
		);
    self := le;
  end;
J0 := JOIN(Recs(ZIP <> (typeof(ZIP))'',PRIM_RANGE <> (typeof(PRIM_RANGE))''),Key,LEFT.ZIP = RIGHT.ZIP
     AND LEFT.PRIM_RANGE = RIGHT.PRIM_RANGE
		 AND RIGHT.FNAME_PreferredName = PersonLinkingADL2.FNPreferName(LEFT.FNAME)
...
J1 := JOIN(Recs(ZIP <> (typeof(ZIP))'',PRIM_RANGE <> (typeof(PRIM_RANGE))''),PULL(Key),LEFT.ZIP = RIGHT.ZIP
    ...
    ATMOST(LEFT.ZIP = RIGHT.ZIP
     AND LEFT.PRIM_RANGE = RIGHT.PRIM_RANGE		 
		 AND RIGHT.FNAME_PreferredName = PersonLinkingADL2.FNPreferName(LEFT.FNAME)
		 AND LEFT.FNAME = RIGHT.FNAME,5000),HASH); // PULL used to cause non-indexed join
...
#IF(#TEXT(Input_ZIP)<>'' AND #TEXT(Input_PRIM_RANGE)<>'')
  #uniquename(trans)
  PersonLinkingADL2V3.Key_PersonHeader_ZPRF.InputLayout_Batch %trans%(InFile le) := transform
  ...
  output_file := PersonLinkingADL2V3.Key_PersonHeader_ZPRF.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

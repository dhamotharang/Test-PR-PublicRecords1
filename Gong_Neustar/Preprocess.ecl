...
proc_CleanAddresses(dataset(layout_gongMaster) infile) := FUNCTION
	...
	master3 := PROJECT(master2, TRANSFORM(raid,
						...
						self.line2 := TRIM(STD.str.cleanspaces(Std.Str.ToUpperCase(
...
EXPORT Preprocess(dataset(Layout_Neustar) src, string rundate = ut.Now()[1..8]) := function

	adds := src(Action_code in ['A','I']);
	addsWithCaps := proc_AddIndents(adds);
	dels := PROJECT(src(Action_code = 'D'), layout_gongMaster);
	
	master1 := PROJECT(addsWithCaps(record_type<>'R' OR Original_Last_Name<>''), TRANSFORM(Layout_gongMaster,
				...
				self.company_name := if(L.Business_name='',MakeListedName(Left.first_name,Left.middle_name,Left.last_name,Left.suffix_name),

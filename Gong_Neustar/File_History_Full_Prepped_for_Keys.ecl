...
gong_in := PROJECT(
						PROJECT(gong_append_in, transform(gong.Layout_historyaid, self := left)),
						recordof(histGong_out));

trSwapCityNamesFiltWithGong0 := trSwapCityNamesFilt;
...
//output(count(trSwapCityNamesFiltWithGong0),named('before_count'));
//output(count(first_seen_patched),named('after_count'));

export File_History_Full_Prepped_for_Keys := 

...
layout_click_id.infile reformat1(file_in L) := TRANSFORM
...
layout_click_id.infile reformat2(file_in L) := TRANSFORM
...
layout_click_id.infile reformat3(file_in L) := TRANSFORM
	i_address_check    :=  if(l.i_address[1..2] in ['W ','S ','E ','N '] and REGEXFIND('^[[:digit:]]+$',l.i_address[2..4]),1,0); 
	self.i_housenum    :=  if(i_address_check = 1 and l.i_addr_chng = 0,functions_click_id.fn_do_if_t(l.i_address).i_housenum, l.i_housenum);
	self.i_streetname  :=  if(i_address_check = 1 and l.i_addr_chng = 0,functions_click_id.fn_do_if_t(l.i_address).i_streetname ,l.i_streetname);
	self.i_addr_chng   :=  if(i_address_check = 1  and l.i_addr_chng = 0 , 1, l.i_addr_chng);
    SELF := L;
	
END;  
projected3 := PROJECT(projected2, reformat2(LEFT)); 
layout_click_id.infile reformat4(file_in L) := TRANSFORM
...
layout_click_id.infile reformat5(projected4 L) := TRANSFORM
...
layout_click_id.infile reformat6(projected5 L) := TRANSFORM
...
layout_click_id.infile  reformat7(projected6 L) := TRANSFORM
...
layout_click_id.infile  reformat8(projected4 L) := TRANSFORM
...
layout_click_id.infile  reformat9(projected4 L) := TRANSFORM
   self.i_apt        := functions_click_id.fn_get_stringtype(l.i_streetname,l.i_apt).at20;
   self.i_streetname := functions_click_id.fn_get_stringtype(l.i_streetname,l.i_apt).st20;
   self := l;
   
END ; 
projected9:= PROJECT(projected8, reformat9(LEFT));
layout_click_id.out_rec   reformat10(projected9 l) := TRANSFORM
...
layout_click_id.out_rec   reformat11(projected10 l) := TRANSFORM

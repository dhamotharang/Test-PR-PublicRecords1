export mac_hist_full_slim_dep(in_file, usepid = false) := functionmacro
...
#if(usepid=true)
	in_slim_srt := sort(in_slim_dst, record, except persistent_record_id, local);
	return dedup(in_slim_srt, record, except persistent_record_id, local);
#else
	in_slim_srt := sort(in_slim_dst, record, local);
	return dedup(in_slim_srt, record, local);
#end

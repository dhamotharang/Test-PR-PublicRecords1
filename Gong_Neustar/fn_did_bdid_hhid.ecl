...
export fn_did_bdid_hhid(dataset(Layout_History) history_in) := function
...
Layout_History get_history_out(history_hhid l) := transform
	self := l;
end;
...
Layout_History get_hhid_by_did(out_no_hhid l, header.File_HHID_Current r) := transform
	self.hhid := r.hhid;
	self := l;
end;

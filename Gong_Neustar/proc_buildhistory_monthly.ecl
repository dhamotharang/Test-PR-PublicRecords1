import did_add, didville, ut, lib_thorlib, RoxieKeyBuild,idl_header;
...
File_Gong_Dirty := PROJECT(DISTRIBUTE(File_Gong_Base), TRANSFORM(Layout_bscurrent_raw,
 ...
 //                        stringlib.stringtouppercase(left.see_also_text) = stringlib.stringtouppercase(right.see_also_text),
	...
	history_did_hhid := gong.fn_did_bdid_hhid(gong_new);
...
/*** end gong lift ***/
	...
	history_new := history_disc + history_curr_marked + history_with_pdid;
	history_with_pid := gong.Mac_Assign_UniqueId(history_new, persistent_record_id);
...
export proc_buildhistory_monthly(dataset(layout_gongMaster) newrecords, string update_date) := function

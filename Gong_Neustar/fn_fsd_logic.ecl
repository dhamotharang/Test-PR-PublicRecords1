...
First Seen Date logic
Try to match up Neustar records with an LSSi record to determine earlier first seen date

*/

EXPORT fn_fsd_logic(Dataset(gong_neustar.Layout_history) h1) := FUNCTION
...
neustar_recs       := h(bell_id='NEU');
lssi_recs          := h(bell_id='LSS');
other_sourced_recs := h(bell_id not in ['NEU','LSS']);
...
gong_neustar.Layout_history xform4(concat_all_sources le) := transform
...
return first_seen_patched(deletion_date<>'' OR current_record_flag='Y');		// filter out "replaced" records)
END;
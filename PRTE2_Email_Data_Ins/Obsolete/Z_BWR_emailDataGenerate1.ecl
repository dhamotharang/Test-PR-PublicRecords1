IMPORT PRTE2_X_Ins_DataCleanse, PRTE2_Email_Data_Ins, Address;

//run two times data to generate email_data_newmhdr and email_data_newmhdr_all
mergedHeader := PRTE2_X_Ins_DataCleanse.Files_Alpha.MHDR_Primary_Recs_DS; //47324 filter (required_bc='Y' AND addr_ind= '1')
// mergedHeader := PRTE2_X_Ins_DataCleanse.Files_Alpha.Merged_Headers_SF_DS; //49153

PRTE2_Email_Data_Ins.layouts.base xEmailData(mergedHeader L):= TRANSFORM 
  //generate email   
  ...
  SELF.best_ssn                  := L.fb_ssn; //SSN in Boca and Alpha are different  
  ...
  SELF.current_rec   := 1; 																														 
  SELF.email_rec_key := hash64((data)SELF.clean_email + 
                               (data)SELF.clean_address.prim_range + 
                               (data)SELF.clean_address.prim_name +  
                               (data)SELF.clean_address.sec_range +  
                               (data)SELF.clean_address.zip + 
                               (data)SELF.Clean_Name.lname +  
                               (data)SELF.Clean_Name.fname);	
  SELF :=  L;
	SELF :=  [];
...
//append the 12 records before key build.

OUTPUT(emailResult,,'~prte::ct::base::email_alpha_generated::email_data_newMHDR', OVERWRITE); //47324

// DuplicateCheck := Table(emailResult, {did, unsigned cnt:=COUNT(group)}, did, all);
// DuplicateCheck(cnt <> 1);

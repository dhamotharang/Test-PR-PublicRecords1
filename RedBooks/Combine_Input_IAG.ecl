export Combine_Input_IAG:= 
module
as_of_title := PROJECT(RedBooks.Files().Vendor.IAG.Title,
...
as_of_corp_addr := 
   join(as_of_title,
          RedBooks.Files().Vendor.IAG.Corporate_Address,
...
as_of_mail_addr := 
   join(as_of_corp_addr,
          RedBooks.Files().Vendor.IAG.Mailing_Address,
		  LEFT.Key_Fields.DocId = RIGHT.DocId AND
		  LEFT.Key_Fields.Level = RIGHT.Level,
		  TRANSFORM(RedBooks.Layouts.Pre_Standardize.Layout_Combined,
...
as_of_phone_numbers := 
   join(as_of_mail_addr,
        RedBooks.Files().Vendor.IAG.Phone_Numbers,
		  LEFT.Key_Fields.DocId = RIGHT.DocId AND
		  LEFT.Key_Fields.Level = RIGHT.Level,
		  TRANSFORM(RedBooks.Layouts.Pre_Standardize.Layout_Combined,
...
as_of_executives := 
   join(as_of_phone_numbers,
        RedBooks.Files().Vendor.IAG.Executives,
		  LEFT.Key_Fields.DocId = RIGHT.DocId AND
		  LEFT.Key_Fields.Level = RIGHT.Level,
		  TRANSFORM(RedBooks.Layouts.Pre_Standardize.Layout_Combined,

export Combine_Input_ISDA_Agency := 
module
 as_of_title := PROJECT(RedBooks.Files().Vendor.ISDA.Agency_Title,
...
as_of_corp_addr := 
   join(as_of_title,
          RedBooks.Files().Vendor.ISDA.Agency_Corporate_Address,
...
as_of_mail_addr := 
   join(as_of_corp_addr,
          RedBooks.Files().Vendor.ISDA.Agency_Mailing_Address,
		  LEFT.Key_Fields.DocId = RIGHT.DocId AND
		  LEFT.Key_Fields.Level = RIGHT.Level,
		  TRANSFORM(RedBooks.Layouts.Pre_Standardize.Layout_Combined,

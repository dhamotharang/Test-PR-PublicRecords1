export Combine_Input_ISDA_Advertiser :=
module
as_of_title := PROJECT(RedBooks.Files().Vendor.ISDA.Title_Company_Number,
                          TRANSFORM(RedBooks.Layouts.Pre_Standardize.Layout_Combined,
					                 ...
					                 SELF.Key_Fields.Level := LEFT.Level;
								          SELF.Name := LEFT.Title;
								          SELF := [];)
								  );
							   

as_of_tradenames := PROJECT(RedBooks.Files().Vendor.ISDA.Tradenames,
                             TRANSFORM(RedBooks.Layouts.Pre_Standardize.Layout_Combined,
								          SELF.Key_Fields.DocId := LEFT.DocId; 
					                      SELF.Key_Fields.Level := LEFT.Level;
...
as_of_corp_addr := 
   join(as_of_company_names_combined,
          RedBooks.Files().Vendor.ISDA.Corporate_Address,
...
as_of_mail_addr := 
   join(as_of_corp_addr,
          RedBooks.Files().Vendor.ISDA.Mailing_Address,
		  LEFT.Key_Fields.DocId = RIGHT.DocId AND
		  LEFT.Key_Fields.Level = RIGHT.Level,
		  TRANSFORM(RedBooks.Layouts.Pre_Standardize.Layout_Combined,
...
as_of_phone_numbers := 
   join(as_of_mail_addr,
        RedBooks.Files().Vendor.ISDA.Phone_Numbers,
		  LEFT.Key_Fields.DocId = RIGHT.DocId AND
		  LEFT.Key_Fields.Level = RIGHT.Level,
		  TRANSFORM(RedBooks.Layouts.Pre_Standardize.Layout_Combined,
...
as_of_executives := 
   join(as_of_phone_numbers,
        RedBooks.Files().Vendor.ISDA.Executives,
		  LEFT.Key_Fields.DocId = RIGHT.DocId AND
		  LEFT.Key_Fields.Level = RIGHT.Level,
		  TRANSFORM(RedBooks.Layouts.Pre_Standardize.Layout_Combined,

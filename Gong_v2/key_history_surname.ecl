import doxie,gong,gong_v2, gong_services,ut;
...
f := gong_v2.proc_roxie_keybuild_prep;

// don't want any names that contain numbers, parens, etc.
...
surnames_all := f(listing_type_res = 'R' AND current_record_flag='Y' AND TRIM(name_last)<>'' AND TRIM(name_first)<>''
									AND ~isBadName(name_last) AND ~isBadName(name_first));

//get all of the permutations (lname is required; fname and state optional)
surnames_l := TABLE(surnames_all, {name_last, cnt := COUNT(GROUP)}, name_last);
surnames_lf := TABLE(surnames_all, {name_last,name_first, cnt := COUNT(GROUP)}, name_last, name_first);
surnames_ls := TABLE(surnames_all, {name_last,st, cnt := COUNT(GROUP)}, name_last, st);
surnames_lfs := TABLE(surnames_all, {name_last,name_first,st, cnt := COUNT(GROUP)}, name_last, name_first, st);
...
f_layout := Gong.Layout_HistorySurname;
...
surnames_l_j := JOIN(surnames_all, surnames_l_f, LEFT.name_last = RIGHT.name_last, 
										 TRANSFORM(f_layout_num, SELF.k_name_last := RIGHT.name_last, SELF.cnt := RIGHT.cnt, 
																						 SELF := LEFT, SELF := RIGHT, SELF :=[]), LIMIT(0));
...
surnames_ls_j := JOIN(surnames_l_j, surnames_ls_f, LEFT.name_last = RIGHT.name_last and LEFT.st = RIGHT.st, 
											TRANSFORM(f_layout_num, SELF.k_name_last := RIGHT.name_last, SELF.k_st := RIGHT.st,
																							SELF.cnt := RIGHT.cnt, SELF := LEFT, SELF := RIGHT, SELF :=[]), LIMIT(0));
...
surnames_lfs_j := JOIN(surnames_lf_j, surnames_lfs_f, LEFT.name_last = RIGHT.name_last and LEFT.name_first = RIGHT.name_first and LEFT.st = RIGHT.st, 
											 TRANSFORM(f_layout_num, SELF.k_name_last := RIGHT.name_last, SELF.k_name_first := RIGHT.name_first,
																							 SELF.k_st := RIGHT.st, SELF.cnt := RIGHT.cnt, SELF := LEFT, SELF := RIGHT, SELF :=[]),LIMIT(0));
...
d:=INDEX(surnames_final,{k_name_last,k_name_first,k_st},{surnames_final},
         Gong_v2.thor_cluster+'key::gongv2_history_surname_' + doxie.Version_SuperKey);

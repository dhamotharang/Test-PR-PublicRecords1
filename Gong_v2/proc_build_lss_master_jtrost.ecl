/*2014-09-24T20:05:28Z (ananth)
C:\Users\VenkatAN\AppData\Roaming\HPCC Systems\eclide\ananth\build1000\Gong_v2\proc_build_lss_master_jtrost\2014-09-24T20_05_28Z.ecl
*/
import Address,did_add,ut,header_slimsort,business_header_ss,risk_indicators,targus,_control, aid, gong;

export proc_build_lss_master_jtrost(string rundate, string weekday = ut.Weekday((integer)ut.GetDate)) := function

//filter then distribute, not the other way around
adds0 := Gong_v2.file_DailyAdditions(~(lststy in ['8','9'] or (lststy='2' and telno='0000000')));
adds  := distribute(adds0(vendor <> '0x1c'),hash(trim(seisintid,left,right)));
...
boolean_value := if ((ut.IsNewProdHeaderVersion('gong_lss_master','header_build_version',roxieip) or 
											ut.IsNewProdHeaderVersion('gong_lss_master','bheader_build_version',roxieip)) and 
											(weekday = 'SATURDAY' or weekday = 'SUNDAY'),
											true,false) : stored('boolean_value');



history_did_hhid := Gong_v2.fn_did_bdid_hhid_trg(concat, rundate, boolean_value);
...
sequential(
	output(nmmaster + masterNoAID,,Gong_v2.thor_cluster+'base::'+rundate+'::lss_master',overwrite,__compressed__),
			...
			fileServices.AddSuperFile(gong_v2.thor_cluster+'base::gongv2_master_delete',
									  gong_v2.thor_cluster+'base::gongv2_master_grandfather',,true,),
			fileservices.clearsuperfile(gong_v2.thor_cluster+'base::gongv2_master_grandfather'),
			
			fileServices.AddSuperFile(gong_v2.thor_cluster+'base::gongv2_master_grandfather',
									  gong_v2.thor_cluster+'base::gongv2_master_father',,true,),
			fileservices.clearsuperfile(gong_v2.thor_cluster+'base::gongv2_master_father'),
			
			fileServices.AddSuperFile(gong_v2.thor_cluster+'base::gongv2_master_father',
									  gong_v2.thor_cluster+'base::gongv2_master',,true,),
			...
			fileservices.clearsuperfile(gong_v2.thor_cluster+'base::gongv2_master_delete'),
			
			fileServices.FinishSuperFileTransaction(),
			if (boolean_value,
				sequential(
						ut.PostDID_HeaderVer_Update('gong_lss_master','header_build_version',roxieip),
						ut.PostDID_HeaderVer_Update('gong_lss_master','bheader_build_version',roxieip),
						output('Full re-DID')),
						output('Daily DID')
						)
				);

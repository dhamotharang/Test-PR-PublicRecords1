...
#workunit('name','Scrubs_DunnData_Email.BWR_Hygiene - Hygiene & Stats - SALT V3.11.6');
IMPORT Scrubs_DunnData_Email,SALT311;
// First create an instantiated hygiene module
  infile := Scrubs_DunnData_Email.In_DunnData_Email;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_DunnData_Email.hygiene(ip);
  ...
  OUTPUT(SALT311.MAC_Character_Counts.EclRecord(p,'Layout_DunnData_Email'),NAMED('OptimizedLayout'));// File layout suggested by data

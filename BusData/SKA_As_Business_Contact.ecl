...
SKA_Verified_Init := project(File_SKA_Verified_base, AddRecordID(left));
...
self.city := choose(CTR, l.mail_p_city_name, l.alt_p_city_name);
...
self.company_city := choose(CTR, l.mail_p_city_name, l.alt_p_city_name);
...
Business_Header.Layout_Business_Contact_Full Format_to_BCF(Layout_BCF_Local l) := transform
self := l;
end;

contacts_ska_verified := project(from_ska_verified_norm_rollup, Format_to_BCF(left));

Business_Header.Layout_Business_Contact_Full Translate_SKA_Nixie_to_BCF(Layout_SKA_Nixie_bdid l) := transform
self.dt_first_seen := (unsigned4)Stringlib.GetDateYYYYMMDD();
self.dt_last_seen := (unsigned4)Stringlib.GetDateYYYYMMDD();
...
self.company_city := l.mail_p_city_name;
...
contacts_ska_nixie := project(File_SKA_Nixie_base, Translate_SKA_Nixie_to_BCF(left));

export SKA_As_Business_Contact := contacts_ska_verified + contacts_ska_nixie : persist('TEMP::SKA_As_Business_Contact');

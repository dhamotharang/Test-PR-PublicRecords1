...
EXPORT Proc_SKA_Spray_In(string filedate, string OM_number = '379725', string filedate_m_d_yyyy = '') :=  module
...
year := filedate[1..4];
filedate_name := IF(filedate_m_d_yyyy != '', filedate_m_d_yyyy, month + '-' + day + '-' + year);

groupname := std.system.Thorlib.group();

Mainfile_a := '/data/prod_data_build_10/production_data/business_headers/ska/data/' + filedate + '/om' + OM_number + 'a_' + filedate_name + '_final_Final.txt' ;
...
Mainfile_b := '/data/prod_data_build_10/production_data/business_headers/ska/data/' + filedate + '/om' + OM_number + 'b_' + filedate_name + '_final_Final.txt' ;

spray_file_b := FileServices.SprayVariable(sourceIP, Mainfile_b, 1024, , , , groupname, '~thor_data400::in::' + filedate + '::ska_b', -1, , , true, , true);

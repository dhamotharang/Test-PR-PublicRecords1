IMPORT SALT37;
EXPORT hygiene(dataset(layout_ALC_DENTAL_PROFESSIONALS) h) := MODULE
...
EXPORT Summary(SALT37.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    populated_specialty_code_pcnt := AVE(GROUP,IF(h.specialty_code = (TYPEOF(h.specialty_code))'',0,100));
    maxlength_specialty_code := MAX(GROUP,LENGTH(TRIM((SALT37.StrType)h.specialty_code)));
    avelength_specialty_code := AVE(GROUP,LENGTH(TRIM((SALT37.StrType)h.specialty_code)),h.specialty_code<>(typeof(h.specialty_code))'');
...
invRec invert(summary0 le, INTEGER C) := TRANSFORM
  ...
  SELF.FieldName := CHOOSE(C,'fname','lname','title','company','address1','address2','city','state','zip','zip4','cart','bar','gender','country','postal_cd','dpv','addr_type','age','county_cd','orig_date','exp_date','license_state','license_type','msa','nielsen_county_cd','specialty_code','phone','email','list_id','scno','keycode','custno','license_no','dob','specialty','degree');
  SELF.populated_pcnt := CHOOSE(C,le.populated_fname_pcnt,le.populated_lname_pcnt,le.populated_title_pcnt,le.populated_company_pcnt,le.populated_address1_pcnt,le.populated_address2_pcnt,le.populated_city_pcnt,le.populated_state_pcnt,le.populated_zip_pcnt,le.populated_zip4_pcnt,le.populated_cart_pcnt,le.populated_bar_pcnt,le.populated_gender_pcnt,le.populated_country_pcnt,le.populated_postal_cd_pcnt,le.populated_dpv_pcnt,le.populated_addr_type_pcnt,le.populated_age_pcnt,le.populated_county_cd_pcnt,le.populated_orig_date_pcnt,le.populated_exp_date_pcnt,le.populated_license_state_pcnt,le.populated_license_type_pcnt,le.populated_msa_pcnt,le.populated_nielsen_county_cd_pcnt,le.populated_specialty_code_pcnt,le.populated_phone_pcnt,le.populated_email_pcnt,le.populated_list_id_pcnt,le.populated_scno_pcnt,le.populated_keycode_pcnt,le.populated_custno_pcnt,le.populated_license_no_pcnt,le.populated_dob_pcnt,le.populated_specialty_pcnt,le.populated_degree_pcnt);
  SELF.maxlength := CHOOSE(C,le.maxlength_fname,le.maxlength_lname,le.maxlength_title,le.maxlength_company,le.maxlength_address1,le.maxlength_address2,le.maxlength_city,le.maxlength_state,le.maxlength_zip,le.maxlength_zip4,le.maxlength_cart,le.maxlength_bar,le.maxlength_gender,le.maxlength_country,le.maxlength_postal_cd,le.maxlength_dpv,le.maxlength_addr_type,le.maxlength_age,le.maxlength_county_cd,le.maxlength_orig_date,le.maxlength_exp_date,le.maxlength_license_state,le.maxlength_license_type,le.maxlength_msa,le.maxlength_nielsen_county_cd,le.maxlength_specialty_code,le.maxlength_phone,le.maxlength_email,le.maxlength_list_id,le.maxlength_scno,le.maxlength_keycode,le.maxlength_custno,le.maxlength_license_no,le.maxlength_dob,le.maxlength_specialty,le.maxlength_degree);
  SELF.avelength := CHOOSE(C,le.avelength_fname,le.avelength_lname,le.avelength_title,le.avelength_company,le.avelength_address1,le.avelength_address2,le.avelength_city,le.avelength_state,le.avelength_zip,le.avelength_zip4,le.avelength_cart,le.avelength_bar,le.avelength_gender,le.avelength_country,le.avelength_postal_cd,le.avelength_dpv,le.avelength_addr_type,le.avelength_age,le.avelength_county_cd,le.avelength_orig_date,le.avelength_exp_date,le.avelength_license_state,le.avelength_license_type,le.avelength_msa,le.avelength_nielsen_county_cd,le.avelength_specialty_code,le.avelength_phone,le.avelength_email,le.avelength_list_id,le.avelength_scno,le.avelength_keycode,le.avelength_custno,le.avelength_license_no,le.avelength_dob,le.avelength_specialty,le.avelength_degree);
END;
...
SALT37.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT37.StrType)le.fname),TRIM((SALT37.StrType)le.lname),TRIM((SALT37.StrType)le.title),TRIM((SALT37.StrType)le.company),TRIM((SALT37.StrType)le.address1),TRIM((SALT37.StrType)le.address2),TRIM((SALT37.StrType)le.city),TRIM((SALT37.StrType)le.state),TRIM((SALT37.StrType)le.zip),TRIM((SALT37.StrType)le.zip4),TRIM((SALT37.StrType)le.cart),TRIM((SALT37.StrType)le.bar),TRIM((SALT37.StrType)le.gender),TRIM((SALT37.StrType)le.country),TRIM((SALT37.StrType)le.postal_cd),TRIM((SALT37.StrType)le.dpv),TRIM((SALT37.StrType)le.addr_type),TRIM((SALT37.StrType)le.age),TRIM((SALT37.StrType)le.county_cd),TRIM((SALT37.StrType)le.orig_date),TRIM((SALT37.StrType)le.exp_date),TRIM((SALT37.StrType)le.license_state),TRIM((SALT37.StrType)le.license_type),TRIM((SALT37.StrType)le.msa),TRIM((SALT37.StrType)le.nielsen_county_cd),TRIM((SALT37.StrType)le.specialty_code),TRIM((SALT37.StrType)le.phone),TRIM((SALT37.StrType)le.email),TRIM((SALT37.StrType)le.list_id),TRIM((SALT37.StrType)le.scno),TRIM((SALT37.StrType)le.keycode),TRIM((SALT37.StrType)le.custno),TRIM((SALT37.StrType)le.license_no),TRIM((SALT37.StrType)le.dob),TRIM((SALT37.StrType)le.specialty),TRIM((SALT37.StrType)le.degree)));
  SELF.FldNo := C;
END;
...
SHARED FldIds := DATASET([{1,'fname'}
      ...
      ,{34,'dob'}
      ,{35,'specialty'}
      ,{36,'degree'}],SALT37.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_license_state((SALT37.StrType)le.license_state),
    ...
    Fields.InValid_specialty_code((SALT37.StrType)le.specialty_code),
    ...
    Fields.InValid_license_no((SALT37.StrType)le.license_no),
...
PrettyErrorTotals := RECORD
  ...
  FieldType := CHOOSE(TotalErrors.FieldNum,'invalid_alphaspacequote','invalid_alphaspacequote','invalid_alphaspace','Unknown','Unknown','Unknown','Unknown','invalid_alpha','invalid_numeric','invalid_numeric','invalid_alphanumeric','invalid_numeric','invalid_gender','Unknown','Unknown','invalid_alpha','invalid_addr_type','invalid_numeric','invalid_alphanumeric','invalid_slashO_date','invalid_slashO_date','invalid_alpha','invalid_alphapound','invalid_numeric','invalid_alphanumeric','invalid_numericpound','invalid_numeric','Unknown','invalid_numeric','invalid_numeric','Unknown','invalid_alphanumeric','invalid_license_number','invalid_date','Unknown','Unknown');
  ErrorMessage := CHOOSE(TotalErrors.FieldNum,Fields.InValidMessage_fname(TotalErrors.ErrorNum),Fields.InValidMessage_lname(TotalErrors.ErrorNum),Fields.InValidMessage_title(TotalErrors.ErrorNum),Fields.InValidMessage_company(TotalErrors.ErrorNum),Fields.InValidMessage_address1(TotalErrors.ErrorNum),Fields.InValidMessage_address2(TotalErrors.ErrorNum),Fields.InValidMessage_city(TotalErrors.ErrorNum),Fields.InValidMessage_state(TotalErrors.ErrorNum),Fields.InValidMessage_zip(TotalErrors.ErrorNum),Fields.InValidMessage_zip4(TotalErrors.ErrorNum),Fields.InValidMessage_cart(TotalErrors.ErrorNum),Fields.InValidMessage_bar(TotalErrors.ErrorNum),Fields.InValidMessage_gender(TotalErrors.ErrorNum),Fields.InValidMessage_country(TotalErrors.ErrorNum),Fields.InValidMessage_postal_cd(TotalErrors.ErrorNum),Fields.InValidMessage_dpv(TotalErrors.ErrorNum),Fields.InValidMessage_addr_type(TotalErrors.ErrorNum),Fields.InValidMessage_age(TotalErrors.ErrorNum),Fields.InValidMessage_county_cd(TotalErrors.ErrorNum),Fields.InValidMessage_orig_date(TotalErrors.ErrorNum),Fields.InValidMessage_exp_date(TotalErrors.ErrorNum),Fields.InValidMessage_license_state(TotalErrors.ErrorNum),Fields.InValidMessage_license_type(TotalErrors.ErrorNum),Fields.InValidMessage_msa(TotalErrors.ErrorNum),Fields.InValidMessage_nielsen_county_cd(TotalErrors.ErrorNum),Fields.InValidMessage_specialty_code(TotalErrors.ErrorNum),Fields.InValidMessage_phone(TotalErrors.ErrorNum),Fields.InValidMessage_email(TotalErrors.ErrorNum),Fields.InValidMessage_list_id(TotalErrors.ErrorNum),Fields.InValidMessage_scno(TotalErrors.ErrorNum),Fields.InValidMessage_keycode(TotalErrors.ErrorNum),Fields.InValidMessage_custno(TotalErrors.ErrorNum),Fields.InValidMessage_license_no(TotalErrors.ErrorNum),Fields.InValidMessage_dob(TotalErrors.ErrorNum),Fields.InValidMessage_specialty(TotalErrors.ErrorNum),Fields.InValidMessage_degree(TotalErrors.ErrorNum));
  TotalErrors.Cnt;
END;

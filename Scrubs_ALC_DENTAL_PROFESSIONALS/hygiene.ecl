...
EXPORT Summary(SALT32.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    maxlength_specialty_code := MAX(GROUP,LENGTH(TRIM((SALT32.StrType)h.specialty_code)));
    avelength_specialty_code := AVE(GROUP,LENGTH(TRIM((SALT32.StrType)h.specialty_code)),h.specialty_code<>(typeof(h.specialty_code))'');
...
SALT32.MAC_Character_Counts.X_Data_Layout Into(h le,unsigned C) := TRANSFORM
  SELF.Fld := TRIM(CHOOSE(C,TRIM((SALT32.StrType)le.fname),TRIM((SALT32.StrType)le.lname),TRIM((SALT32.StrType)le.title),TRIM((SALT32.StrType)le.company),TRIM((SALT32.StrType)le.address1),TRIM((SALT32.StrType)le.address2),TRIM((SALT32.StrType)le.city),TRIM((SALT32.StrType)le.state),TRIM((SALT32.StrType)le.zip),TRIM((SALT32.StrType)le.zip4),TRIM((SALT32.StrType)le.cart),TRIM((SALT32.StrType)le.bar),TRIM((SALT32.StrType)le.gender),TRIM((SALT32.StrType)le.country),TRIM((SALT32.StrType)le.postal_cd),TRIM((SALT32.StrType)le.dpv),TRIM((SALT32.StrType)le.addr_type),TRIM((SALT32.StrType)le.age),TRIM((SALT32.StrType)le.county_cd),TRIM((SALT32.StrType)le.orig_date),TRIM((SALT32.StrType)le.exp_date),TRIM((SALT32.StrType)le.license_state),TRIM((SALT32.StrType)le.license_type),TRIM((SALT32.StrType)le.msa),TRIM((SALT32.StrType)le.nielsen_county_cd),TRIM((SALT32.StrType)le.specialty_code),TRIM((SALT32.StrType)le.phone),TRIM((SALT32.StrType)le.email),TRIM((SALT32.StrType)le.list_id),TRIM((SALT32.StrType)le.scno),TRIM((SALT32.StrType)le.keycode),TRIM((SALT32.StrType)le.custno),TRIM((SALT32.StrType)le.license_no),TRIM((SALT32.StrType)le.dob),TRIM((SALT32.StrType)le.specialty),TRIM((SALT32.StrType)le.degree)));
  SELF.FldNo := C;
END;
...
SHARED FldIds := DATASET([{1,'fname'}
      ...
      ,{34,'dob'}
      ,{35,'specialty'}
      ,{36,'degree'}],SALT32.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_specialty_code((SALT32.StrType)le.specialty_code),
    ...
    Fields.InValid_license_no((SALT32.StrType)le.license_no),

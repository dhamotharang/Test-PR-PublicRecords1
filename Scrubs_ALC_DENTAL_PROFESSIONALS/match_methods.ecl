IMPORT SALT32,std;
EXPORT match_methods(DATASET(layout_ALC_DENTAL_PROFESSIONALS) ih) := MODULE
...
EXPORT match_specialty_code(TYPEOF(h.specialty_code) L, TYPEOF(h.specialty_code) R, BOOLEAN RequiredField = FALSE) := IF(~RequiredField,
   MAP(L = R => SALT32.MatchCode.ExactMatch,
    SALT32.MatchCode.NoMatch),
  MAP(L = R => SALT32.MatchCode.ExactMatch, SALT32.MatchCode.NoMatch)
);

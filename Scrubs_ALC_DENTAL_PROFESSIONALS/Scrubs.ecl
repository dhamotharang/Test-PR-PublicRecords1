...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Layout_ALC_DENTAL_PROFESSIONALS)
    ...
    UNSIGNED1 specialty_code_Invalid;
  ...
  END;
  EXPORT  Bitmap_Layout := RECORD(Layout_ALC_DENTAL_PROFESSIONALS)
    UNSIGNED8 ScrubsBits1;
  END;
EXPORT FromNone(DATASET(Layout_ALC_DENTAL_PROFESSIONALS) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.license_state_Invalid := Fields.InValid_license_state((SALT32.StrType)le.license_state);
    ...
    SELF.specialty_code_Invalid := Fields.InValid_specialty_code((SALT32.StrType)le.specialty_code);
    ...
    SELF.license_no_Invalid := Fields.InValid_license_no((SALT32.StrType)le.license_no);
    SELF.dob_Invalid := Fields.InValid_dob((SALT32.StrType)le.dob);
    SELF := le;
  END;
  EXPORT ExpandedInfile := PROJECT(h,toExpanded(LEFT,FALSE));
  EXPORT ProcessedInfile := PROJECT(PROJECT(h,toExpanded(LEFT,TRUE)),Layout_ALC_DENTAL_PROFESSIONALS);
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.fname_Invalid << 0 ) + ( le.lname_Invalid << 1 ) + ( le.title_Invalid << 2 ) + ( le.state_Invalid << 3 ) + ( le.zip_Invalid << 4 ) + ( le.zip4_Invalid << 5 ) + ( le.cart_Invalid << 6 ) + ( le.bar_Invalid << 7 ) + ( le.gender_Invalid << 8 ) + ( le.dpv_Invalid << 9 ) + ( le.addr_type_Invalid << 10 ) + ( le.age_Invalid << 11 ) + ( le.county_cd_Invalid << 12 ) + ( le.orig_date_Invalid << 13 ) + ( le.exp_date_Invalid << 14 ) + ( le.license_state_Invalid << 15 ) + ( le.license_type_Invalid << 16 ) + ( le.msa_Invalid << 17 ) + ( le.nielsen_county_cd_Invalid << 18 ) + ( le.specialty_code_Invalid << 19 ) + ( le.phone_Invalid << 20 ) + ( le.list_id_Invalid << 21 ) + ( le.scno_Invalid << 22 ) + ( le.custno_Invalid << 23 ) + ( le.license_no_Invalid << 24 ) + ( le.dob_Invalid << 25 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  EXPORT Infile := PROJECT(h,Layout_ALC_DENTAL_PROFESSIONALS);
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.age_Invalid := (le.ScrubsBits1 >> 11) & 1;
    SELF.county_cd_Invalid := (le.ScrubsBits1 >> 12) & 1;
    SELF.orig_date_Invalid := (le.ScrubsBits1 >> 13) & 1;
    SELF.exp_date_Invalid := (le.ScrubsBits1 >> 14) & 1;
    ...
    SELF.specialty_code_Invalid := (le.ScrubsBits1 >> 19) & 1;
    SELF.phone_Invalid := (le.ScrubsBits1 >> 20) & 1;
    SELF.list_id_Invalid := (le.ScrubsBits1 >> 21) & 1;
    SELF.scno_Invalid := (le.ScrubsBits1 >> 22) & 1;
    SELF.custno_Invalid := (le.ScrubsBits1 >> 23) & 1;
    SELF.license_no_Invalid := (le.ScrubsBits1 >> 24) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    specialty_code_ALLOW_ErrorCount := COUNT(GROUP,h.specialty_code_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    UNSIGNED1 ErrNum := CHOOSE(c,le.fname_Invalid,le.lname_Invalid,le.title_Invalid,le.state_Invalid,le.zip_Invalid,le.zip4_Invalid,le.cart_Invalid,le.bar_Invalid,le.gender_Invalid,le.dpv_Invalid,le.addr_type_Invalid,le.age_Invalid,le.county_cd_Invalid,le.orig_date_Invalid,le.exp_date_Invalid,le.license_state_Invalid,le.license_type_Invalid,le.msa_Invalid,le.nielsen_county_cd_Invalid,le.specialty_code_Invalid,le.phone_Invalid,le.list_id_Invalid,le.scno_Invalid,le.custno_Invalid,le.license_no_Invalid,le.dob_Invalid,100);
    SELF.ErrorMessage := IF ( ErrNum = 0, SKIP, CHOOSE(c,Fields.InvalidMessage_fname(le.fname_Invalid),Fields.InvalidMessage_lname(le.lname_Invalid),Fields.InvalidMessage_title(le.title_Invalid),Fields.InvalidMessage_state(le.state_Invalid),Fields.InvalidMessage_zip(le.zip_Invalid),Fields.InvalidMessage_zip4(le.zip4_Invalid),Fields.InvalidMessage_cart(le.cart_Invalid),Fields.InvalidMessage_bar(le.bar_Invalid),Fields.InvalidMessage_gender(le.gender_Invalid),Fields.InvalidMessage_dpv(le.dpv_Invalid),Fields.InvalidMessage_addr_type(le.addr_type_Invalid),Fields.InvalidMessage_age(le.age_Invalid),Fields.InvalidMessage_county_cd(le.county_cd_Invalid),Fields.InvalidMessage_orig_date(le.orig_date_Invalid),Fields.InvalidMessage_exp_date(le.exp_date_Invalid),Fields.InvalidMessage_license_state(le.license_state_Invalid),Fields.InvalidMessage_license_type(le.license_type_Invalid),Fields.InvalidMessage_msa(le.msa_Invalid),Fields.InvalidMessage_nielsen_county_cd(le.nielsen_county_cd_Invalid),Fields.InvalidMessage_specialty_code(le.specialty_code_Invalid),Fields.InvalidMessage_phone(le.phone_Invalid),Fields.InvalidMessage_list_id(le.list_id_Invalid),Fields.InvalidMessage_scno(le.scno_Invalid),Fields.InvalidMessage_custno(le.custno_Invalid),Fields.InvalidMessage_license_no(le.license_no_Invalid),Fields.InvalidMessage_dob(le.dob_Invalid),'UNKNOWN'));
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.specialty_code_Invalid,'ALLOW','UNKNOWN')
          ...
          ,CHOOSE(le.dob_Invalid,'ALLOW','CUSTOM','UNKNOWN'),'UNKNOWN'));
    SELF.FieldName := CHOOSE(c,'fname','lname','title','state','zip','zip4','cart','bar','gender','dpv','addr_type','age','county_cd','orig_date','exp_date','license_state','license_type','msa','nielsen_county_cd','specialty_code','phone','list_id','scno','custno','license_no','dob','UNKNOWN');
    SELF.FieldType := CHOOSE(c,'invalid_alphaspacequote','invalid_alphaspacequote','invalid_alphaspace','invalid_alpha','invalid_numeric','invalid_numeric','invalid_alphanumeric','invalid_numeric','invalid_gender','invalid_alpha','invalid_addr_type','invalid_numeric','invalid_alphanumeric','invalid_slash_date','invalid_slash_date','invalid_alpha','invalid_alphapound','invalid_numeric','invalid_alphanumeric','invalid_numericpound','invalid_numeric','invalid_numeric','invalid_numeric','invalid_alphanumeric','invalid_license_number','invalid_date','UNKNOWN');
    SELF.FieldContents := CHOOSE(c,(SALT32.StrType)le.fname,(SALT32.StrType)le.lname,(SALT32.StrType)le.title,(SALT32.StrType)le.state,(SALT32.StrType)le.zip,(SALT32.StrType)le.zip4,(SALT32.StrType)le.cart,(SALT32.StrType)le.bar,(SALT32.StrType)le.gender,(SALT32.StrType)le.dpv,(SALT32.StrType)le.addr_type,(SALT32.StrType)le.age,(SALT32.StrType)le.county_cd,(SALT32.StrType)le.orig_date,(SALT32.StrType)le.exp_date,(SALT32.StrType)le.license_state,(SALT32.StrType)le.license_type,(SALT32.StrType)le.msa,(SALT32.StrType)le.nielsen_county_cd,(SALT32.StrType)le.specialty_code,(SALT32.StrType)le.phone,(SALT32.StrType)le.list_id,(SALT32.StrType)le.scno,(SALT32.StrType)le.custno,(SALT32.StrType)le.license_no,(SALT32.StrType)le.dob,'***SALTBUG***');
  END;
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT32.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'specialty_code:invalid_numericpound:ALLOW'
          ...
          ,'dob:invalid_date:ALLOW','dob:invalid_date:CUSTOM','UNKNOWN');
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Fields.InvalidMessage_specialty_code(1)
          ...
          ,Fields.InvalidMessage_dob(1),Fields.InvalidMessage_dob(2),'UNKNOWN');
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.specialty_code_ALLOW_ErrorCount
          ...
          ,le.dob_ALLOW_ErrorCount,le.dob_CUSTOM_ErrorCount,0);
      SELF.rulepcnt := 100 * CHOOSE(c
          ...
          ,le.custno_ALLOW_ErrorCount
          ,le.license_no_ALLOW_ErrorCount
          ,le.dob_ALLOW_ErrorCount,le.dob_CUSTOM_ErrorCount,0) / le.TotalCnt + 0.5;
    END;

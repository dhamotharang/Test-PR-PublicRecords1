...
EXPORT FromNone(DATASET(Corp2_Mapping.LayoutsCommon.Main) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_address1_effective_date_Invalid := Fields.InValid_corp_address1_effective_date((SALT32.StrType)le.corp_address1_effective_date);
    ...
    SELF.corp_orig_bus_type_cd_Invalid := Fields.InValid_corp_orig_bus_type_cd((SALT32.StrType)le.corp_orig_bus_type_cd);
    SELF.corp_ra_effective_date_Invalid := Fields.InValid_corp_ra_effective_date((SALT32.StrType)le.corp_ra_effective_date);
    SELF.corp_agent_assign_date_Invalid := Fields.InValid_corp_agent_assign_date((SALT32.StrType)le.corp_agent_assign_date);
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_status_cd_Invalid := (le.ScrubsBits1 >> 27) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    ...
    END;
    SummaryInfo := NORMALIZE(SummaryStats,51,Into(LEFT,COUNTER));
    orb_r := RECORD
      AllErrors.Src;

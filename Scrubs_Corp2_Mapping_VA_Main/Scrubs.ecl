IMPORT SALT311,STD;
IMPORT Scrubs,Scrubs_Corp2_Mapping_VA_Main,Corp2_Mapping; // Import modules for FieldTypes attribute definitions
...
EXPORT FromNone(DATASET(Corp2_Mapping.LayoutsCommon.Main) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.corp_address1_type_cd_Invalid := Fields.InValid_corp_address1_type_cd((SALT311.StrType)le.corp_address1_type_cd);
    SELF.corp_address1_type_desc_Invalid := Fields.InValid_corp_address1_type_desc((SALT311.StrType)le.corp_address1_type_desc);
    ...
    SELF.corp_orig_bus_type_cd_Invalid := Fields.InValid_corp_orig_bus_type_cd((SALT311.StrType)le.corp_orig_bus_type_cd);
    SELF.corp_ra_effective_date_Invalid := Fields.InValid_corp_ra_effective_date((SALT311.StrType)le.corp_ra_effective_date);
    SELF.corp_agent_status_desc_Invalid := Fields.InValid_corp_agent_status_desc((SALT311.StrType)le.corp_agent_status_desc,(SALT311.StrType)le.recordorigin);
    SELF.corp_agent_assign_date_Invalid := Fields.InValid_corp_agent_assign_date((SALT311.StrType)le.corp_agent_assign_date);
    SELF.corp_name_effective_date_Invalid := Fields.InValid_corp_name_effective_date((SALT311.StrType)le.corp_name_effective_date);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_ln_name_type_desc_Invalid << 22 ) + ( le.corp_address1_type_cd_Invalid << 23 ) + ( le.corp_address1_type_desc_Invalid << 24 ) + ( le.corp_address1_effective_date_Invalid << 25 ) + ( le.corp_status_desc_Invalid << 27 ) + ( le.corp_status_date_Invalid << 28 ) + ( le.corp_inc_state_Invalid << 30 ) + ( le.corp_inc_date_Invalid << 31 ) + ( le.corp_term_exist_cd_Invalid << 33 ) + ( le.corp_term_exist_exp_Invalid << 34 ) + ( le.corp_term_exist_desc_Invalid << 35 ) + ( le.corp_foreign_domestic_ind_Invalid << 36 ) + ( le.corp_forgn_state_cd_Invalid << 37 ) + ( le.corp_forgn_state_desc_Invalid << 38 ) + ( le.corp_forgn_date_Invalid << 39 ) + ( le.corp_orig_bus_type_cd_Invalid << 41 ) + ( le.corp_ra_effective_date_Invalid << 42 ) + ( le.corp_agent_status_desc_Invalid << 44 ) + ( le.corp_agent_assign_date_Invalid << 45 ) + ( le.corp_name_effective_date_Invalid << 47 ) + ( le.corp_name_reservation_expiration_date_Invalid << 49 ) + ( le.recordorigin_Invalid << 51 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_address1_effective_date_Invalid := (le.ScrubsBits1 >> 25) & 3;
    SELF.corp_status_desc_Invalid := (le.ScrubsBits1 >> 27) & 1;
    SELF.corp_status_date_Invalid := (le.ScrubsBits1 >> 28) & 3;
    SELF.corp_inc_state_Invalid := (le.ScrubsBits1 >> 30) & 1;
    SELF.corp_inc_date_Invalid := (le.ScrubsBits1 >> 31) & 3;
    SELF.corp_term_exist_cd_Invalid := (le.ScrubsBits1 >> 33) & 1;
    SELF.corp_term_exist_exp_Invalid := (le.ScrubsBits1 >> 34) & 1;
    ...
    SELF.corp_ra_effective_date_Invalid := (le.ScrubsBits1 >> 42) & 3;
    SELF.corp_agent_status_desc_Invalid := (le.ScrubsBits1 >> 44) & 1;
    SELF.corp_agent_assign_date_Invalid := (le.ScrubsBits1 >> 45) & 3;
    SELF.corp_name_effective_date_Invalid := (le.ScrubsBits1 >> 47) & 3;
    SELF.corp_name_reservation_expiration_date_Invalid := (le.ScrubsBits1 >> 49) & 3;
    SELF.recordorigin_Invalid := (le.ScrubsBits1 >> 51) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_orig_bus_type_cd_CUSTOM_ErrorCount := COUNT(GROUP,h.corp_orig_bus_type_cd_Invalid=1);
    ...
    corp_agent_status_desc_CUSTOM_ErrorCount := COUNT(GROUP,h.corp_agent_status_desc_Invalid=1);
    ...
    AnyRule_WithErrorsCount := COUNT(GROUP, h.dt_vendor_first_reported_Invalid > 0 OR h.dt_vendor_last_reported_Invalid > 0 OR h.dt_first_seen_Invalid > 0 OR h.dt_last_seen_Invalid > 0 OR h.corp_ra_dt_first_seen_Invalid > 0 OR h.corp_ra_dt_last_seen_Invalid > 0 OR h.corp_key_Invalid > 0 OR h.corp_vendor_Invalid > 0 OR h.corp_state_origin_Invalid > 0 OR h.corp_process_date_Invalid > 0 OR h.corp_orig_sos_charter_nbr_Invalid > 0 OR h.corp_legal_name_Invalid > 0 OR h.corp_ln_name_type_cd_Invalid > 0 OR h.corp_ln_name_type_desc_Invalid > 0 OR h.corp_address1_type_cd_Invalid > 0 OR h.corp_address1_type_desc_Invalid > 0 OR h.corp_address1_effective_date_Invalid > 0 OR h.corp_status_desc_Invalid > 0 OR h.corp_status_date_Invalid > 0 OR h.corp_inc_state_Invalid > 0 OR h.corp_inc_date_Invalid > 0 OR h.corp_term_exist_cd_Invalid > 0 OR h.corp_term_exist_exp_Invalid > 0 OR h.corp_term_exist_desc_Invalid > 0 OR h.corp_foreign_domestic_ind_Invalid > 0 OR h.corp_forgn_state_cd_Invalid > 0 OR h.corp_forgn_state_desc_Invalid > 0 OR h.corp_forgn_date_Invalid > 0 OR h.corp_orig_bus_type_cd_Invalid > 0 OR h.corp_ra_effective_date_Invalid > 0 OR h.corp_agent_status_desc_Invalid > 0 OR h.corp_agent_assign_date_Invalid > 0 OR h.corp_name_effective_date_Invalid > 0 OR h.corp_name_reservation_expiration_date_Invalid > 0 OR h.recordorigin_Invalid > 0);
  ...
  SummaryStats0 xAddErrSummary(SummaryStats0 le) := TRANSFORM
    SELF.FieldsChecked_WithErrors := IF(le.dt_vendor_first_reported_Total_ErrorCount > 0, 1, 0) + IF(le.dt_vendor_last_reported_Total_ErrorCount > 0, 1, 0) + IF(le.dt_first_seen_Total_ErrorCount > 0, 1, 0) + IF(le.dt_last_seen_Total_ErrorCount > 0, 1, 0) + IF(le.corp_ra_dt_first_seen_Total_ErrorCount > 0, 1, 0) + IF(le.corp_ra_dt_last_seen_Total_ErrorCount > 0, 1, 0) + IF(le.corp_key_Total_ErrorCount > 0, 1, 0) + IF(le.corp_vendor_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_state_origin_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_orig_sos_charter_nbr_Total_ErrorCount > 0, 1, 0) + IF(le.corp_legal_name_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_ln_name_type_cd_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_ln_name_type_desc_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_address1_type_cd_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_address1_type_desc_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_address1_effective_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_status_desc_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_status_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_inc_state_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_inc_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_term_exist_cd_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_term_exist_exp_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_term_exist_desc_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_foreign_domestic_ind_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_forgn_state_cd_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_forgn_state_desc_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_forgn_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_orig_bus_type_cd_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_ra_effective_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_agent_status_desc_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_agent_assign_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_name_effective_date_Total_ErrorCount > 0, 1, 0) + IF(le.corp_name_reservation_expiration_date_Total_ErrorCount > 0, 1, 0) + IF(le.recordorigin_ENUM_ErrorCount > 0, 1, 0);
    SELF.FieldsChecked_NoErrors := NumFieldsWithRules - SELF.FieldsChecked_WithErrors;
    SELF.Rules_WithErrors := IF(le.dt_vendor_first_reported_ALLOW_ErrorCount > 0, 1, 0) + IF(le.dt_vendor_first_reported_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.dt_vendor_last_reported_ALLOW_ErrorCount > 0, 1, 0) + IF(le.dt_vendor_last_reported_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.dt_first_seen_ALLOW_ErrorCount > 0, 1, 0) + IF(le.dt_first_seen_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.dt_last_seen_ALLOW_ErrorCount > 0, 1, 0) + IF(le.dt_last_seen_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_ra_dt_first_seen_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_ra_dt_first_seen_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_ra_dt_last_seen_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_ra_dt_last_seen_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_key_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_key_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_vendor_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_state_origin_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_process_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_orig_sos_charter_nbr_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_orig_sos_charter_nbr_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_legal_name_LENGTHS_ErrorCount > 0, 1, 0) + IF(le.corp_ln_name_type_cd_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_ln_name_type_desc_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_address1_type_cd_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_address1_type_desc_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_address1_effective_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_address1_effective_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_status_desc_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_status_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_status_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_inc_state_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_inc_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_inc_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_term_exist_cd_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_term_exist_exp_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_term_exist_desc_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_foreign_domestic_ind_ENUM_ErrorCount > 0, 1, 0) + IF(le.corp_forgn_state_cd_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_forgn_state_desc_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_forgn_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_forgn_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_orig_bus_type_cd_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_ra_effective_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_ra_effective_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_agent_status_desc_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_agent_assign_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_agent_assign_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_name_effective_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_name_effective_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.corp_name_reservation_expiration_date_ALLOW_ErrorCount > 0, 1, 0) + IF(le.corp_name_reservation_expiration_date_CUSTOM_ErrorCount > 0, 1, 0) + IF(le.recordorigin_ENUM_ErrorCount > 0, 1, 0);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_orig_bus_type_cd_Invalid,'CUSTOM','UNKNOWN')
          ,CHOOSE(le.corp_ra_effective_date_Invalid,'ALLOW','CUSTOM','UNKNOWN')
          ,CHOOSE(le.corp_agent_status_desc_Invalid,'CUSTOM','UNKNOWN')
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_address1_type_cd:invalid_addr_cd:ENUM'
          ,'corp_address1_type_desc:invalid_addr_desc:ENUM'
          ,'corp_address1_effective_date:invalid_date:ALLOW','corp_address1_effective_date:invalid_date:CUSTOM'
          ,'corp_status_desc:invalid_status_desc:CUSTOM'
...
EXPORT StandardStats(DATASET(Corp2_Mapping.LayoutsCommon.Main) inFile, BOOLEAN doErrorOverall = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_scrubsSummaryStatsFieldErrTransform(Scrubs_Corp2_Mapping_VA_Main, Fields, 'RECORDOF(scrubsSummaryOverall)', '');

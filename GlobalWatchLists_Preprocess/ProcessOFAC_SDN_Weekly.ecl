...
ProcessPrimaryRecords := FUNCTION	
	...
	PrimaryRecs := PROJECT(sort(Files_OFAC_Weekly.dsOFACPrimary, SDN_ID, AKA_ID, Last_Name, First_Name), ReformatPrimary(LEFT));
																						...
																						,if(STD.Str.Find(L.orig_last_name, '(COMMUNIST', 1) > 0
																							...
																							,if(STD.Str.Find(L.orig_last_name, '(a.k.a.', 1) > 0 or STD.Str.Find(L.orig_last_name, '(n.k.a.', 1) > 0 or STD.Str.Find(L.orig_last_name, '(f.k.a.', 1) > 0
																								...
																								,if(regexfind('\\([A-Z .\'-]+\\)', L.orig_last_name) = true
																										,TRIM(STD.Str.FilterOut(L.orig_last_name[STD.Str.Find(L.orig_last_name, '(', 1)+1..4 + STD.Str.Find(L.orig_last_name, '(', 1)+1-1], ')'), LEFT, RIGHT)
																										,'')))))))));
...
ProcessProgramRecords := FUNCTION
	...
	InRec := PROJECT(sort(Files_OFAC_Weekly.dsOFACProgram, SDN_ID, Program), makeRecord(LEFT));
...
ProcessAddressRecords := FUNCTION
	...
	return PROJECT(sort(Files_OFAC_Weekly.dsOFACAddress, SDN_ID, Address_ID, Address_Line_1, Address_Line_2, Address_Line_3, Address_City, Address_State, Address_Postal_Code, Address_Country), ReformatAddress(LEFT));
END;
...
ProcessIDRecords := FUNCTION
	...
	InRec := PROJECT(sort(Files_OFAC_Weekly.dsOFACID, SDN_ID, ID_ID, ID_Type, ID_Number), makeRecord(LEFT));
...
ProcessNationalityRecords := FUNCTION
	...
	InRec := PROJECT(sort(Files_OFAC_Weekly.dsOFACNationality, SDN_ID, Nationality_ID, Nationality), makeRecord(LEFT));
...
ProcessCitizenshipRecords := FUNCTION
	...
	InRec := PROJECT(sort(Files_OFAC_Weekly.dsOFACCitizenship, SDN_ID, Citizenship_ID, Citizenship), makeRecord(LEFT));
...
ProcessDOBRecords := FUNCTION
	...
	InRec := PROJECT(sort(Files_OFAC_Weekly.dsOFACDOB, SDN_ID, Date_of_Birth_ID, Date_of_Birth), makeRecord(LEFT));
...
ProcessPOBRecords := FUNCTION
	...
	InRec := PROJECT(sort(Files_OFAC_Weekly.dsOFACPOB, SDN_ID, Place_of_Birth_ID, Place_of_Birth), makeRecord(LEFT));
...
EXPORT ProcessOFAC_SDN_Weekly := FUNCTION	
	...
	rOutLayout ReformatToCommonlayout(dsOFAC L) := TRANSFORM
		...
		self.orig_pty_name 		:= if(L.orig_giv_designator = 'G' or L.orig_giv_designator = 'I'
																,STD.Str.ToUpperCase(TRIM(L.orig_last_name, LEFT, RIGHT)) 
		...
		self.orig_vessel_name := if(L.orig_giv_designator = 'V' or L.orig_giv_designator = 'A'
																,STD.Str.ToUpperCase(L.orig_last_name)
																,'');
		...
		self.date_first_seen 	:= GlobalWatchLists_Preprocess.Version_OFAC_Wkly.OFAC_SDN;
		self.date_last_seen 	:= GlobalWatchLists_Preprocess.Version_OFAC_Wkly.OFAC_SDN;
		self.date_vendor_first_reported := GlobalWatchLists_Preprocess.Version_OFAC_Wkly.OFAC_SDN;
		self.date_vendor_last_reported 	:= GlobalWatchLists_Preprocess.Version_OFAC_Wkly.OFAC_SDN;

...
EXPORT ProcessOFAC_Random(string filedate) := FUNCTION	//filedate = date processed
	...
	dsOFAC 		:= GlobalWatchLists_Preprocess.ProcessOFAC;
	dsOFACPLC	:= GlobalWatchLists_Preprocess.ProcessOFAC_PLC;
	dsGWLBase	:= dataset('~thor_data400::base::globalwatchlists',GlobalWatchLists.Layout_GlobalWatchLists,thor)(pty_key[1..4] != 'OFAC');
	
	CombineOFAC	:= dsOFAC + dsOFACPLC;
	...
	//Output to Base layout filter invalid address
	GlobalWatchLists.Layout_GlobalWatchLists xformFinal(CombineOFAC L) := TRANSFORM
		//Parse Address fields to fit output
		TempConcatAddr	:= REGEXREPLACE('^TELEPHONE:(.*)|^\\+[0-9](.*)',STD.Str.ToUpperCase(STD.Str.CleanSpaces(L.addr_1+'|'+L.addr_2+'|'+L.addr_3+'|')),'');
		...
		TempConcatRemark	:= STD.Str.ToUpperCase(REGEXREPLACE('[| |]{2,}',
																								REGEXREPLACE('^[| |]+',STD.Str.CleanSpaces(L.remarks_1+'|'+L.remarks_2+'|'+L.remarks_3+'|'+
    ...
    self.err_stat 		:= 	L.addr_clean[179..182];
		...
		self.id_number_1	:=			L.orig_id_number_1;
		self.id_number_2	:=			L.orig_id_number_2;
		self.id_number_3	:=			L.orig_id_number_3;
		self.id_number_4	:=			L.orig_id_number_4;
		self.id_number_5	:=			L.orig_id_number_5;
		self.id_number_6	:=			L.orig_id_number_6;
		self.id_number_7	:=			L.orig_id_number_7;
		self.id_number_8	:=			L.orig_id_number_8;
		self.id_number_9	:=			L.orig_id_number_9;
		self.id_number_10	:=			L.orig_id_number_10;

...
EXPORT Suspicious_Fraud_LN.layouts.Layout_Name_Inquiries Search_Inquiries_Name (DATASET(Suspicious_Fraud_LN.layouts.Layout_Batch_Plus) Input,
	...
	Suspicious_Fraud_LN.InquiryTransformMac (getInquiriesKey, Suspicious_Fraud_LN.layouts.Layout_Name_Inquiries, InquiriesKey);
	Suspicious_Fraud_LN.InquiryTransformMac (getInquiriesUpdateKey, Suspicious_Fraud_LN.layouts.Layout_Name_Inquiries, InquiriesUpdateKey);
	...
	rawInquiriesKey := JOIN(Input, InquiriesKey, LEFT.Clean_Input.FirstName <> '' AND KEYED(LEFT.Clean_Input.FirstName = RIGHT.fname) AND // First Name Matches
	...
	rawInquiriesUpdateKey := JOIN(Input, InquiriesUpdateKey, LEFT.Clean_Input.FirstName <> '' AND KEYED(LEFT.Clean_Input.FirstName = RIGHT.fname) AND // First Name Matches
																								((LEFT.Clean_Input.LastName <> '' AND LEFT.Clean_Input.LastName = RIGHT.lname) OR // And last name, or middle name/middle initial matches
																									((LEFT.Clean_Input.MiddleName <> '' AND LEFT.Clean_Input.MiddleName = RIGHT.mname) OR
																									 (LEFT.Clean_Input.MiddleInitial <> '' AND LEFT.Clean_Input.MiddleInitial = RIGHT.mname[1])))
																								 AND

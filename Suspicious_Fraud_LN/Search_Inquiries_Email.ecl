...
EXPORT Suspicious_Fraud_LN.layouts.Layout_Email_Inquiries Search_Inquiries_Email (DATASET(Suspicious_Fraud_LN.layouts.Layout_Batch_Plus) Input,
	...
	Suspicious_Fraud_LN.InquiryTransformMac (getInquiriesKey, Suspicious_Fraud_LN.layouts.Layout_Email_Inquiries, InquiriesKey);
	Suspicious_Fraud_LN.InquiryTransformMac (getInquiriesUpdateKey, Suspicious_Fraud_LN.layouts.Layout_Email_Inquiries, InquiriesUpdateKey);
	...
	rawInquiriesKey := JOIN(Input, InquiriesKey, TRIM(LEFT.Clean_Input.Email) <> '' AND KEYED(LEFT.Clean_Input.Email = RIGHT.Email_Address) AND
	...
	rawInquiriesUpdateKey := JOIN(Input, InquiriesUpdateKey, TRIM(LEFT.Clean_Input.Email) <> '' AND KEYED(LEFT.Clean_Input.Email = RIGHT.Email_Address) AND
																							 Inquiry_AccLogs.Shell_Constants.Valid_Suspicious_Fraud_Inquiry(LEFT.Clean_Input.ArchiveDate, RIGHT.Search_Info.DateTime[1..8], StringLib.StringToUpperCase(TRIM(RIGHT.Bus_Intel.Industry, ALL)), StringLib.StringToUpperCase(TRIM(RIGHT.Search_Info.Function_Description)), RIGHT.Bus_Intel.Use) AND
																							 (UNSIGNED)RIGHT.search_info.datetime[1..6] <= LEFT.Clean_Input.ArchiveDate,

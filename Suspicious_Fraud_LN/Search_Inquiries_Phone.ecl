...
EXPORT Suspicious_Fraud_LN.layouts.Layout_Phone_Inquiries Search_Inquiries_Phone (DATASET(Suspicious_Fraud_LN.layouts.Layout_Batch_Plus) Input,
	...
	Suspicious_Fraud_LN.InquiryTransformMac (getInquiriesKey, Suspicious_Fraud_LN.layouts.Layout_Phone_Inquiries, InquiriesKey);
	Suspicious_Fraud_LN.InquiryTransformMac (getInquiriesUpdateKey, Suspicious_Fraud_LN.layouts.Layout_Phone_Inquiries, InquiriesUpdateKey);
	...
	rawInquiriesKey := JOIN(Input, InquiriesKey, (UNSIGNED)LEFT.Clean_Input.Phone10 <> 0 AND KEYED(LEFT.Clean_Input.Phone10 = RIGHT.Phone10) AND
	...
	rawInquiriesUpdateKey := JOIN(Input, InquiriesUpdateKey, (UNSIGNED)LEFT.Clean_Input.Phone10 <> 0 AND KEYED(LEFT.Clean_Input.Phone10 = RIGHT.Phone10) AND
																							 Inquiry_AccLogs.Shell_Constants.Valid_Suspicious_Fraud_Inquiry(LEFT.Clean_Input.ArchiveDate, RIGHT.Search_Info.DateTime[1..8], StringLib.StringToUpperCase(TRIM(RIGHT.Bus_Intel.Industry, ALL)), StringLib.StringToUpperCase(TRIM(RIGHT.Search_Info.Function_Description)), RIGHT.Bus_Intel.Use) AND
																							 (UNSIGNED)RIGHT.search_info.datetime[1..6] <= LEFT.Clean_Input.ArchiveDate,

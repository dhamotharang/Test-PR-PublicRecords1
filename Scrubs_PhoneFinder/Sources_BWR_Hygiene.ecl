...
#workunit('name','Scrubs_PhoneFinder.Sources_BWR_Hygiene - Hygiene & Stats - SALT V3.11.11');
IMPORT Scrubs_PhoneFinder,SALT311;
// First create an instantiated hygiene module
  infile := Scrubs_PhoneFinder.Sources_In_PhoneFinder;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_PhoneFinder.Sources_hygiene(ip);
  ...
  OUTPUT(SALT311.MAC_Character_Counts.EclRecord(p,'Sources_Layout_PhoneFinder'),NAMED('OptimizedLayout'));// File layout suggested by data

...
EXPORT OtherPhones_Delta(DATASET(OtherPhones_Layout_PhoneFinder)old_s, DATASET(OtherPhones_Layout_PhoneFinder) new_s) := MODULE//Routines to compute the differences between two instances of a file
  SHARED inFieldList := ['transaction_id','sequence_number','phone_id','phonenumber','risk_indicator','phone_type','phone_status','listing_name','porting_code','phone_forwarded','verified_carrier','date_added','identity_count','carrier','phone_star_rating','filename'];

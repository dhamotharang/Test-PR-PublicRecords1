...
#workunit('name','Scrubs_Debt_Settlement.CC_BWR_Hygiene - Hygiene & Stats - SALT V3.11.8');
IMPORT Scrubs_Debt_Settlement,SALT311;
// First create an instantiated hygiene module
  infile := Scrubs_Debt_Settlement.CC_In_Debt_Settlement;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_Debt_Settlement.CC_hygiene(ip);
  ...
  OUTPUT(SALT311.MAC_Character_Counts.EclRecord(p,'CC_Layout_Debt_Settlement'),NAMED('OptimizedLayout'));// File layout suggested by data

...
EXPORT FromNone(DATASET(Layout_in_file) h) := MODULE
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dt_vendor_first_reported_Invalid << 0 ) + ( le.dt_vendor_last_reported_Invalid << 2 ) + ( le.dt_first_seen_Invalid << 4 ) + ( le.dt_last_seen_Invalid << 6 ) + ( le.corp_ra_dt_first_seen_Invalid << 8 ) + ( le.corp_ra_dt_last_seen_Invalid << 10 ) + ( le.corp_key_Invalid << 12 ) + ( le.corp_vendor_Invalid << 14 ) + ( le.corp_state_origin_Invalid << 15 ) + ( le.corp_process_date_Invalid << 16 ) + ( le.corp_orig_sos_charter_nbr_Invalid << 18 ) + ( le.corp_legal_name_Invalid << 20 ) + ( le.corp_ln_name_type_cd_Invalid << 21 ) + ( le.corp_ln_name_type_desc_Invalid << 22 ) + ( le.corp_status_desc_Invalid << 23 ) + ( le.corp_status_date_Invalid << 24 ) + ( le.corp_inc_state_Invalid << 26 ) + ( le.corp_inc_date_Invalid << 27 ) + ( le.corp_foreign_domestic_ind_Invalid << 29 ) + ( le.corp_forgn_state_desc_Invalid << 30 ) + ( le.corp_forgn_date_Invalid << 31 ) + ( le.corp_term_exist_cd_Invalid << 33 ) + ( le.corp_term_exist_desc_Invalid << 34 ) + ( le.corp_orig_org_structure_desc_Invalid << 35 ) + ( le.corp_for_profit_ind_Invalid << 36 ) + ( le.corp_name_reservation_expiration_date_Invalid << 37 ) + ( le.corp_orig_bus_type_desc_Invalid << 39 ) + ( le.corp_management_desc_Invalid << 40 ) + ( le.corp_fiscal_year_month_Invalid << 41 ) + ( le.corp_agent_county_Invalid << 42 ) + ( le.InternalField1_Invalid << 43 ) + ( le.InternalField2_Invalid << 44 ) + ( le.InternalField3_Invalid << 45 ) + ( le.recordorigin_Invalid << 46 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.corp_name_reservation_expiration_date_Invalid := (le.ScrubsBits1 >> 37) & 3;
    SELF.corp_orig_bus_type_desc_Invalid := (le.ScrubsBits1 >> 39) & 1;
    SELF.corp_management_desc_Invalid := (le.ScrubsBits1 >> 40) & 1;
    SELF.corp_fiscal_year_month_Invalid := (le.ScrubsBits1 >> 41) & 1;
    SELF.corp_agent_county_Invalid := (le.ScrubsBits1 >> 42) & 1;
    SELF.InternalField1_Invalid := (le.ScrubsBits1 >> 43) & 1;
    SELF.InternalField2_Invalid := (le.ScrubsBits1 >> 44) & 1;
    SELF.InternalField3_Invalid := (le.ScrubsBits1 >> 45) & 1;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    corp_name_reservation_expiration_date_LENGTHS_ErrorCount := COUNT(GROUP,h.corp_name_reservation_expiration_date_Invalid=3);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.corp_name_reservation_expiration_date_Invalid,'ALLOW','CUSTOM','LENGTHS','UNKNOWN')
  ...
  // field error stats
    SALT311.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'corp_name_reservation_expiration_date:invalid_exp_date:ALLOW','corp_name_reservation_expiration_date:invalid_exp_date:CUSTOM','corp_name_reservation_expiration_date:invalid_exp_date:LENGTHS'
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.corp_name_reservation_expiration_date_ALLOW_ErrorCount,le.corp_name_reservation_expiration_date_CUSTOM_ErrorCount,le.corp_name_reservation_expiration_date_LENGTHS_ErrorCount

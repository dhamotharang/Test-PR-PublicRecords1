...
EXPORT Summary(SALT311.Str30Type  txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    populated_InternalField2_cnt := COUNT(GROUP,h.InternalField2 <> (TYPEOF(h.InternalField2))'');
    populated_InternalField2_pcnt := AVE(GROUP,IF(h.InternalField2 = (TYPEOF(h.InternalField2))'',0,100));
    maxlength_InternalField2 := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.InternalField2)));
    avelength_InternalField2 := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.InternalField2)),h.InternalField2<>(typeof(h.InternalField2))'');
    populated_InternalField3_cnt := COUNT(GROUP,h.InternalField3 <> (TYPEOF(h.InternalField3))'');
    populated_InternalField3_pcnt := AVE(GROUP,IF(h.InternalField3 = (TYPEOF(h.InternalField3))'',0,100));
    maxlength_InternalField3 := MAX(GROUP,LENGTH(TRIM((SALT311.StrType)h.InternalField3)));
    avelength_InternalField3 := AVE(GROUP,LENGTH(TRIM((SALT311.StrType)h.InternalField3)),h.InternalField3<>(typeof(h.InternalField3))'');
  ...
  R1 := RECORD
    UNSIGNED LinkingPotential :=  + T.Populated_dt_vendor_first_reported_pcnt *   0.00 / 100 + T.Populated_dt_vendor_last_reported_pcnt *   0.00 / 100 + T.Populated_dt_first_seen_pcnt *   0.00 / 100 + T.Populated_dt_last_seen_pcnt *   0.00 / 100 + T.Populated_corp_ra_dt_first_seen_pcnt *   0.00 / 100 + T.Populated_corp_ra_dt_last_seen_pcnt *   0.00 / 100 + T.Populated_corp_key_pcnt *   0.00 / 100 + T.Populated_corp_vendor_pcnt *   0.00 / 100 + T.Populated_corp_state_origin_pcnt *   0.00 / 100 + T.Populated_corp_process_date_pcnt *   0.00 / 100 + T.Populated_corp_orig_sos_charter_nbr_pcnt *   0.00 / 100 + T.Populated_corp_legal_name_pcnt *   0.00 / 100 + T.Populated_corp_ln_name_type_cd_pcnt *   0.00 / 100 + T.Populated_corp_ln_name_type_desc_pcnt *   0.00 / 100 + T.Populated_corp_status_desc_pcnt *   0.00 / 100 + T.Populated_corp_status_date_pcnt *   0.00 / 100 + T.Populated_corp_inc_state_pcnt *   0.00 / 100 + T.Populated_corp_inc_date_pcnt *   0.00 / 100 + T.Populated_corp_foreign_domestic_ind_pcnt *   0.00 / 100 + T.Populated_corp_forgn_state_desc_pcnt *   0.00 / 100 + T.Populated_corp_forgn_date_pcnt *   0.00 / 100 + T.Populated_corp_term_exist_cd_pcnt *   0.00 / 100 + T.Populated_corp_term_exist_desc_pcnt *   0.00 / 100 + T.Populated_corp_orig_org_structure_desc_pcnt *   0.00 / 100 + T.Populated_corp_for_profit_ind_pcnt *   0.00 / 100 + T.Populated_corp_name_reservation_expiration_date_pcnt *   0.00 / 100 + T.Populated_corp_orig_bus_type_desc_pcnt *   0.00 / 100 + T.Populated_corp_management_desc_pcnt *   0.00 / 100 + T.Populated_corp_fiscal_year_month_pcnt *   0.00 / 100 + T.Populated_corp_agent_county_pcnt *   0.00 / 100 + T.Populated_InternalField1_pcnt *   0.00 / 100 + T.Populated_InternalField2_pcnt *   0.00 / 100 + T.Populated_InternalField3_pcnt *   0.00 / 100 + T.Populated_recordorigin_pcnt *   0.00 / 100;
    T;
  END;
  RETURN TABLE(T,R1);
END;
...
SHARED FldIds := DATASET([{1,'dt_vendor_first_reported'}
      ...
      ,{21,'corp_forgn_date'}
      ,{22,'corp_term_exist_cd'}
      ,{23,'corp_term_exist_desc'}
      ,{24,'corp_orig_org_structure_desc'}
      ,{25,'corp_for_profit_ind'}
      ,{26,'corp_name_reservation_expiration_date'}
      ,{27,'corp_orig_bus_type_desc'}
      ,{28,'corp_management_desc'}
      ,{29,'corp_fiscal_year_month'}
      ,{30,'corp_agent_county'}
      ,{31,'InternalField1'}
      ,{32,'InternalField2'}
      ,{33,'InternalField3'}
      ,{34,'recordorigin'}],SALT311.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,UNSIGNED1 c) := TRANSFORM
  SELF.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_InternalField2((SALT311.StrType)le.InternalField2),
    Fields.InValid_InternalField3((SALT311.StrType)le.InternalField3),
...
PrettyErrorTotals := RECORD
  ...
  FieldType := CHOOSE(TotalErrors.FieldNum,'invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_date','invalid_corp_key','invalid_corp_vendor','invalid_state_origin','invalid_date','invalid_charter','invalid_mandatory','invalid_name_type_code','invalid_name_type_desc','invalid_status','invalid_date','invalid_state_origin','invalid_date','invalid_forgn_dom_code','invalid_alphablank','invalid_date','invalid_term_exist_cd','invalid_term_exist_desc','invalid_org_structure_desc','invalid_flag_code','invalid_exp_date','invalid_alphablankcomma','invalid_management','invalid_month','invalid_alphablank','invalid_alphablank','invalid_alphablank','invalid_alphablank','invalid_recordorigin');
  ErrorMessage := CHOOSE(TotalErrors.FieldNum,Fields.InValidMessage_dt_vendor_first_reported(TotalErrors.ErrorNum),Fields.InValidMessage_dt_vendor_last_reported(TotalErrors.ErrorNum),Fields.InValidMessage_dt_first_seen(TotalErrors.ErrorNum),Fields.InValidMessage_dt_last_seen(TotalErrors.ErrorNum),Fields.InValidMessage_corp_ra_dt_first_seen(TotalErrors.ErrorNum),Fields.InValidMessage_corp_ra_dt_last_seen(TotalErrors.ErrorNum),Fields.InValidMessage_corp_key(TotalErrors.ErrorNum),Fields.InValidMessage_corp_vendor(TotalErrors.ErrorNum),Fields.InValidMessage_corp_state_origin(TotalErrors.ErrorNum),Fields.InValidMessage_corp_process_date(TotalErrors.ErrorNum),Fields.InValidMessage_corp_orig_sos_charter_nbr(TotalErrors.ErrorNum),Fields.InValidMessage_corp_legal_name(TotalErrors.ErrorNum),Fields.InValidMessage_corp_ln_name_type_cd(TotalErrors.ErrorNum),Fields.InValidMessage_corp_ln_name_type_desc(TotalErrors.ErrorNum),Fields.InValidMessage_corp_status_desc(TotalErrors.ErrorNum),Fields.InValidMessage_corp_status_date(TotalErrors.ErrorNum),Fields.InValidMessage_corp_inc_state(TotalErrors.ErrorNum),Fields.InValidMessage_corp_inc_date(TotalErrors.ErrorNum),Fields.InValidMessage_corp_foreign_domestic_ind(TotalErrors.ErrorNum),Fields.InValidMessage_corp_forgn_state_desc(TotalErrors.ErrorNum),Fields.InValidMessage_corp_forgn_date(TotalErrors.ErrorNum),Fields.InValidMessage_corp_term_exist_cd(TotalErrors.ErrorNum),Fields.InValidMessage_corp_term_exist_desc(TotalErrors.ErrorNum),Fields.InValidMessage_corp_orig_org_structure_desc(TotalErrors.ErrorNum),Fields.InValidMessage_corp_for_profit_ind(TotalErrors.ErrorNum),Fields.InValidMessage_corp_name_reservation_expiration_date(TotalErrors.ErrorNum),Fields.InValidMessage_corp_orig_bus_type_desc(TotalErrors.ErrorNum),Fields.InValidMessage_corp_management_desc(TotalErrors.ErrorNum),Fields.InValidMessage_corp_fiscal_year_month(TotalErrors.ErrorNum),Fields.InValidMessage_corp_agent_county(TotalErrors.ErrorNum),Fields.InValidMessage_InternalField1(TotalErrors.ErrorNum),Fields.InValidMessage_InternalField2(TotalErrors.ErrorNum),Fields.InValidMessage_InternalField3(TotalErrors.ErrorNum),Fields.InValidMessage_recordorigin(TotalErrors.ErrorNum));
  TotalErrors.Cnt;
END;
...
EXPORT StandardStats(BOOLEAN doSummaryGlobal = TRUE, BOOLEAN doAllProfiles = TRUE) := FUNCTION
  ...
  SALT311.mod_StandardStatsTransforms.mac_hygieneSummaryTransform(Scrubs_Corp2_Mapping_ID_Main, Fields, 'RECORDOF(fieldPopulationOverall)', FALSE);

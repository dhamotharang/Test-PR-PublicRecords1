IMPORT  STD, UPI_DataBuild__dev, HealthcareNoMatchHeader_Ingest,HealthcareNoMatchHeader_InternalLinking,Workman;
EXPORT  Build_All_Workman(
  ...
  pMaxNumIter     :=  UPI_DataBuild__dev.proc_Constants.maxNumIters;
  pPrimaryQueue   :=  HealthcareNoMatchHeader_InternalLinking.proc_Constants.primaryQueue; // thor44*
	pSecondaryQueue	:=  UPI_DataBuild__dev.proc_Constants.secondaryQueue; // hthor*
  pWuPrefix       :=  UPI_DataBuild__dev.Filenames_V2(pVersion,pUseProd,gcid,pHistMode).WUPrefix;
  pWuSuperfile    :=  UPI_DataBuild__dev.Filenames_V2(pVersion,pUseProd,gcid,pHistMode).MasterWUOutput_SF;
  pEmailTo        :=  UPI_DataBuild__dev.proc_Constants.emailNotify;
  pPollingFreq    :=  UPI_DataBuild__dev.proc_Constants.pollingFreq;

  //  Common Workman ECL Code
  workmanPreamble(STRING runText)  :=  FUNCTION
    workmanPreambleECL  :=  'IMPORT versioncontrol, _control, ut, tools, UPI_DataBuild__dev;' +
                            '\npVersion  				:= \'@version@\';' +
  ...
  step1_ECL   :=  workmanPreamble(step1_Text)+
                      '\nUPI_DataBuild__dev.Build_All_V2(pVersion,pUseProd,gcid,pLexidThreshold,pHistMode,gcid_name'+
											',pBatchJobID,pAppendOption,pReceivingID).Step1;';
  ...
  Step2_ECL  :=  workmanPreamble(Step2_Text) +
                        '\nUPI_DataBuild__dev.Build_all_V2(pVersion,pUseProd,gcid,pLexidThreshold,pHistMode,gcid_name'+
												',pBatchJobID,pAppendOption,pReceivingID).Step2;';
  ...
  Step3_ECL   := workmanPreamble(Step3_Text)+
                        '\nUPI_DataBuild__dev.Build_all_V2(pVersion,pUseProd,gcid,pLexidThreshold,pHistMode,gcid_name'+
												',pBatchJobID,pAppendOption,pReceivingID).Step3;';

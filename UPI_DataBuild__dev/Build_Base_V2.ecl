import _control, versioncontrol, UPI_DataBuild__dev;
...
// 4 - Append ONLY Customer Record Key for all records - NO LEXID appends for any records
					...
					shared build_base_member := UPI_DataBuild__dev.Update_Base_V2(pVersion,pUseProd,gcid,pLexidThreshold,pHistMode,gcid_name,pBatch_jobID,pAppendOption).all_data_base;
					...
					shared full_build_member	:=  
						sequential(				
						 		 Build_member_Base
								 ,if(pHistMode = 'A', UPI_DataBuild__dev.Promote_V2.promote_base(pVersion,pUseProd,gcid,pHistMode).buildfiles.New2Built,
								 sequential(
										fileservices.startsuperfiletransaction()
										,fileservices.addsuperfile('~ushc::crk::base::' + gcid + '::built_nosave',UPI_DataBuild__dev.Filenames_V2(pVersion, pUseProd, gcid, pHistMode).member_base.new)
										// ,fileservices.addsuperfile('~ushc::crk::base::' + gcid + '::qa_nosave',UPI_DataBuild__dev.Filenames_V2(pVersion, pUseProd, gcid, pHistMode).member_base.new)
...
// 4 - Append ONLY Customer Record Key for all records - NO LEXID appends for any records
					...
					shared build_processed_input := UPI_DataBuild__dev.Update_Base_V2(pVersion,pUseProd,gcid,pLexidThreshold,pHistMode,gcid_name,pBatch_jobID,pAppendOption).processed_input;
					VersionControl.macBuildNewLogicalFile(
...
// 4 - Append ONLY Customer Record Key for all records - NO LEXID appends for any records
					...
					shared build_batch_output := UPI_DataBuild__dev.Update_Base_V2(pVersion,pUseProd,gcid,pLexidThreshold,pHistMode,gcid_name,pBatch_jobID,pAppendOption).return_tobatch;
					VersionControl.macBuildNewLogicalFile(
					...
					shared full_batch_output	:=  
						sequential(				
						 		 Build_output_tobatch
								,UPI_DataBuild__dev.Functions.PipeFile(pVersion, pUseProd, gcid, pHistMode, pBatch_jobID)
...
// 4 - Append ONLY Customer Record Key for all records - NO LEXID appends for any records
					...
					shared build_base_metrics := UPI_DataBuild__dev.Update_Base_V2(pVersion,pUseProd,gcid,pLexidThreshold,pHistMode,gcid_name,pBatch_jobID,pAppendOption).Return_Metrics;
					VersionControl.macBuildNewLogicalFile(
					...
					export full_build_metrics	:=  
						sequential(				
						 		 Build_metrics_Base
								,UPI_DataBuild__dev.Functions.PipeMetricsFile(pVersion, pUseProd, gcid, pHistMode, pBatch_jobID)
								,UPI_DataBuild__dev.Functions.PipeHistoryFile(pVersion, pUseProd, gcid, pHistMode, pBatch_jobID)
		...
		EXPORT build_slim_report(
					...
					shared build_slim_report := UPI_DataBuild__dev.Update_Base_V2(pVersion,pUseProd,gcid,pLexidThreshold,pHistMode,gcid_name,pBatch_jobID,pAppendOption).slim_report;
					VersionControl.macBuildNewLogicalFile(
		...
		EXPORT Build_temp_header(
			...
			shared dBuildtemp_header := UPI_DataBuild__dev.Convert_Base_toHeader(pVersion,pUseProd,gcid,pHistMode).Build_it;
			
			VersionControl.macBuildNewLogicalFile(
																			 UPI_DataBuild__dev.Filenames_V2(pVersion, pUseProd, gcid, pHistMode).temp_header.new
																		 	,dBuildtemp_header

...
EXPORT Create_asHeader_V2 (string pVersion, boolean pUseProd, string gcid, string pHistMode, string100 gcid_name, string10 batch_jobid) := MODULE
		...
		EXPORT Build_it := FUNCTION
			...
			pBaseFile		:= IF(NOTHOR(FileServices.GetSuperFileSubCount(UPI_DataBuild__dev.Filenames_V2(pVersion, pUseProd, gcid, pHistMode).processed_input_lBaseTemplate_built)) = 0
												 ,dataset([],UPI_DataBuild__dev.Layouts_V2.input_processing)
												 ,UPI_DataBuild__dev.Files_V2(pVersion,pUseProd,gcid,pHistMode).processed_input.built);

			UPI_DataBuild__dev.Layouts_V2.as_header xformAH(UPI_DataBuild__dev.Layouts_V2.input_processing L) := TRANSFORM

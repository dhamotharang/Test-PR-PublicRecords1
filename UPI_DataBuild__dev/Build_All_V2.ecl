...
export Build_all_V2(pVersion, pUseProd, gcid, pLexidThreshold, pHistMode, gcid_name, pBatch_jobID, pAppendOption, pReceivingID, crk_suffix, pOrbEnv) := functionmacro
	...
	export Orbit_token					:= if(pOrbEnv = 'QA', UPI_DataBuild__dev.Orbit_Login(), UPI_DataBuild__dev.Orbit_LoginPROD());
	
	export createNewBuildQA			:= 	UPI_DataBuild__dev.Orbit_CreateBuild(
																						pMasterBuild
																					 ,UPI_DataBuild__dev.Orbit_Tracking.OrbitBuildInProgress
																					 ,pVersion_unique
																					 ,''
																					 ,pMasterBuild
																					 ,orbit_token
	...
	export createNewBuildPROD		:= 	UPI_DataBuild__dev.Orbit_CreateBuild(
																						pMasterBuild
																					 ,UPI_DataBuild__dev.Orbit_TrackingPROD.OrbitBuildInProgress
																					 ,pVersion_unique
																					 ,''
																					 ,pMasterBuild
																					 ,orbit_token
	...
	export addComponent					:= UPI_DataBuild__dev.Orbit_AddComponentsToABuild ( 		
																						pMasterBuild
																					 ,pVersion_unique
																					 ,pReceivingID
																					 ,orbit_token
	...
	export changeBuildStatusQA	:= UPI_DataBuild__dev.Orbit_UpdateBuildStatus (		
																						pMasterBuild
																					 ,UPI_DataBuild__dev.Orbit_Tracking.OrbitBuilt
																					 ,pVersion_unique
																					 ,orbit_token
																					 ,//comment
																					 ,pOrbEnv
	...
	export changeBuildStatusPROD:= UPI_DataBuild__dev.Orbit_UpdateBuildStatus (		
																						pMasterBuild
																					 ,UPI_DataBuild__dev.Orbit_TrackingPROD.OrbitBuilt
																					 ,pVersion_unique
																					 ,orbit_token
																					 ,//comment
																					 ,pOrbEnv
	...
	export changeRecdItemStatusSprayedQA	:= UPI_DataBuild__dev.Orbit_UpdateReceiveItem (
																					 pReceivingID
																					,UPI_DataBuild__dev.Orbit_Tracking.OrbitSprayed
																					,orbit_token
																					,pOrbEnv):independent;

	export changeRecdItemStatusSprayedPROD	:= UPI_DataBuild__dev.Orbit_UpdateReceiveItem (
																					 pReceivingID
																					,UPI_DataBuild__dev.Orbit_TrackingPROD.OrbitSprayed
																					,orbit_token
																					,pOrbEnv):independent;
																					
	export changeRecdItemStatusBuiltQA	:= UPI_DataBuild__dev.Orbit_UpdateReceiveItem (
																					 pReceivingID
																					,UPI_DataBuild__dev.Orbit_Tracking.OrbitBuilt
																					,orbit_token
																					,pOrbEnv):independent;
																					
	export changeRecdItemStatusBuiltPROD	:= UPI_DataBuild__dev.Orbit_UpdateReceiveItem (
																					 pReceivingID
																					,UPI_DataBuild__dev.Orbit_TrackingPROD.OrbitBuilt
																					,orbit_token
																					,pOrbEnv):independent;
	...
	export step1 := sequential(
			...
			,UPI_DataBuild__dev.Build_Base_V2.process_input_file(pVersion_unique,pUseProd,gcid,pLexidThreshold,pHistMode,gcid_name,pBatch_jobID,pAppendOption).process_input_all
			,UPI_DataBuild__dev.Promote_V2.promote_processed_input(pVersion_unique,pUseProd,gcid,pHistMode).buildfiles.Built2QA
			
			,UPI_DataBuild__dev.Build_asHeader_V2(pVersion_unique,pUseProd,gcid,pHistMode,gcid_name,pBatch_jobID).asHeader_all // for this product, the asheader is built from the previously processed base file
			,UPI_DataBuild__dev.Promote_V2.promote_asHeader(pVersion_unique,pUseProd,gcid,pHistMode).buildfiles.Built2QA
		
			,UPI_DataBuild__dev.Build_Base_V2.Build_temp_header(pVersion_unique,pUseProd,gcid,pHistMode,gcid_name,pBatch_jobID).temp_header_all
			,UPI_DataBuild__dev.Promote_V2.promote_temp_header(pVersion_unique,pUseProd,gcid,pHistMode).buildfiles.Built2QA
	);

	export step2	:= HealthcareNoMatchHeader_InternalLinking.MAC_AppendCRK(
				gcid
			 ,pVersion_unique
			 ,UPI_DataBuild__dev.Filenames_V2(pVersion_unique, pUseProd, gcid, pHistMode).temp_header.built
			 ,UPI_DataBuild__dev.Filenames_V2(pVersion_unique, pUseProd, gcid, pHistMode).asheader.built);
			 			 
	export step3 := sequential(
			UPI_DataBuild__dev.Build_Base_V2.Build_base_members(pVersion_unique,pUseProd,gcid,pLexidThreshold,pHistMode,gcid_name,pBatch_jobID,pAppendOption).member_all
			,if(pHistMode = 'A', UPI_DataBuild__dev.Promote_V2.promote_base(pVersion_unique,pUseProd,gcid,pHistMode).buildfiles.Built2QA,
					sequential(
						 fileservices.startsuperfiletransaction()
						,fileservices.addsuperfile('~ushc::crk::base::' + gcid + '::qa_nosave',UPI_DataBuild__dev.Filenames_V2(pVersion_unique, pUseProd, gcid, pHistMode).member_base.new)
						// ,fileservices.addsuperfile('~ushc::crk::base::' + gcid + '::built_nosave',UPI_DataBuild__dev.Filenames_V2(pVersion_unique, pUseProd, gcid, pHistMode).member_base.new)
			...
			,UPI_DataBuild__dev.Build_Base_V2.Build_batch_output(pVersion_unique,pUseProd,gcid,pLexidThreshold,pHistMode,gcid_name,pBatch_jobID,pAppendOption).full_batch_all
			,UPI_DataBuild__dev.Promote_V2.promote_return_tobatch(pVersion_unique,pUseProd,gcid,pHistMode).buildfiles.Built2QA
			
			,UPI_DataBuild__dev.Build_Base_V2.Build_base_metrics(pVersion_unique,pUseProd,gcid,pLexidThreshold,pHistMode,gcid_name,pBatch_jobID,pAppendOption).full_build_metrics
			,UPI_DataBuild__dev.Promote_V2.promote_tobatch_metrics(pVersion_unique,pUseProd,gcid,pHistMode).buildfiles.Built2QA
			
			,UPI_DataBuild__dev.Build_Base_V2.Build_slim_report(pVersion_unique,pUseProd,gcid,pLexidThreshold,pHistMode,gcid_name,pBatch_jobID,pAppendOption).full_build_slim_report
			,sequential(
						 fileservices.startsuperfiletransaction()
						,fileservices.addsuperfile('~ushc::crk::slim_history::' + gcid + '::qa',UPI_DataBuild__dev.Filenames_V2(pVersion_unique, pUseProd, gcid, pHistMode).slim_history_file.new)
						,fileservices.finishsuperfiletransaction())
						// push slim history files to qa superfile - no deletes and no automatic promote through additional generations
													
			,FileServices.StartSuperFileTransaction()
			,FileServices.RemoveOwnedSubFiles(UPI_DataBuild__dev.Filenames_V2(pVersion_unique,pUseProd,gcid,pHistMode).batch_lInputHistTemplate,true)
			,FileServices.ClearSuperFile(UPI_DataBuild__dev.Filenames_V2(pVersion_unique,pUseProd,gcid,pHistMode).batch_lInputHistTemplate)
			,FileServices.AddSuperFile(UPI_DataBuild__dev.Filenames_V2(pVersion_unique,pUseProd,gcid,pHistMode).batch_lInputHistTemplate,  UPI_DataBuild__dev.Filenames_V2(pVersion_unique,pUseProd,gcid,pHistMode).batch_lInputTemplate,,true)
			
			,FileServices.RemoveOwnedSubFiles(UPI_DataBuild__dev.Filenames_V2(pVersion_unique,pUseProd,gcid,pHistMode).batch_lInputTemplate,true)
			,FileServices.ClearSuperFile(UPI_DataBuild__dev.Filenames_V2(pVersion_unique,pUseProd,gcid,pHistMode).batch_lInputTemplate)

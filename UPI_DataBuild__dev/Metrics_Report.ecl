IMPORT  ut, mdr, tools,_validate, Address, Ut, lib_stringlib, _Control, business_header, UPI_DataBuild__dev,
Header, Header_Slimsort, didville, ut, DID_Add,Business_Header_SS, NID, AID;

EXPORT Metrics_Report (string pVersion, boolean pUseProd, string gcid, dataset (UPI_DataBuild__dev.Layouts_V2.input_processing) pBaseFile, string pHistMode):= MODULE
   ...
   d := dataset([{min_lexid_score, max_lexid_score, ave_lexid_score, cnt_lexid_change, pct_lexid_change, cnt_crk_change, pct_crk_change, guardian_min_lexid_score, guardian_max_lexid_score, guardian_ave_lexid_score, cnt_new_crk}],
                  UPI_DataBuild__dev.Layouts_V2.metrics_fields);          
	...
	export cnt_new_crk	:= function
		crk_only					:= record
			UPI_DataBuild__dev.Layouts_V2.base.crk;
		end;
		
		prev_base		:= map(												 
									pHistMode = 'N'	=> dataset([],UPI_DataBuild__dev.Layouts_V2.base),
									IF(NOTHOR(FileServices.GetSuperFileSubCount(UPI_DataBuild__dev.Filenames_V2(pVersion, pUseProd, gcid, pHistMode).member_base.father)) = 0
												 ,dataset([],UPI_DataBuild__dev.Layouts_V2.base)
												 ,UPI_DataBuild__dev.Files_V2(pVersion,pUseProd,gcid,pHistMode).member_base.father)); 

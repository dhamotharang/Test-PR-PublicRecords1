...
EXPORT FN_AppendOutlierGroupedV2
    (
        inFile,
        valueField,
        groupingFieldsStr,
        ...
        (
            sequencedData,
            {
                ...
                UNSIGNED2   recCount := MAX(GROUP, %recid%),
								ValueField_t mValue  := MAX (GROUP, tData)
            },
            %hashValue%,
            MERGE
        );
    ...
    LOCAL ResultRec := RECORD
        ...
        REAL        #EXPAND(appendPrefix + 'OutlierDiff');
				REAL				#EXPAND(appendPrefix + 'Percentile');
    END;
                    ...
                    SELF.#EXPAND(appendPrefix + 'OutlierDiff') := IF
                        (
                            ...
                            0
										...
										SELF.#EXPAND(appendPrefix + 'Percentile') := (LEFT.valueField / RIGHT.mValue) * 100;
                    SELF := LEFT
                ),
            LOCAL, LEFT OUTER
        );

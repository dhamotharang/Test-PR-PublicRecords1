...
src_lay:=RECORD
  ...
  unsigned4 date_vendor_last_reported_;
  unsigned1 __date_vendor_last_reported__flags;
  unsigned4 date_vendor_first_reported_;
  unsigned1 __date_vendor_first_reported__flags;
  DATASET(RECORD
   string source_;
   unsigned1 __source__flags;
   unsigned2 date_first_seen_;
   unsigned2 date_last_seen_;
   integer8 __recordcount;
  END) data_sources_;
  unsigned1 __data_sources__flags;
  unsigned2 date_first_seen_;
  unsigned2 date_last_seen_;
  integer8 __recordcount;
 END;
...
source_logical_file:= '~foreign::10.194.90.202::' + 'qview::pr::t_i_n_nonfcra_ent';
destination_logical_file:= '~foreign::10.194.90.202::' + 'qview::pr::tin_nonfcra_vault_src1';

source_file:=dataset(source_logical_file,src_lay,thor);
desti_file:=dataset(destination_logical_file,desti_lay,thor);

Flatten_src_Lay:=RECORD
  integer seq;
  string src_field;
  string desti_field;
  unsigned8 uid;
  unsigned1 uid_flags;
  unsigned4 date_vendor_last_reported_;
  unsigned1 date_vendor_last_reported__flags;
  unsigned4 date_vendor_first_reported_;
  unsigned1 date_vendor_first_reported__flags;
  unsigned1 data_sources__flags;
  unsigned2 date_first_seen_;
  unsigned2 date_last_seen_;
...
source_file_pjt:= project(source_file(uid=13355),transform(Flatten_src_Lay,
																									self.uid_flags:=left.__uid_flags;
																									self.date_vendor_last_reported__flags:=left.__date_vendor_last_reported__flags;
																									self.date_vendor_first_reported__flags:=left.__date_vendor_first_reported__flags;
																									self.data_sources__flags:=left.__data_sources__flags;
																									self.recordcount:=left.__recordcount;
																									
																									self:=left;
																									self:=[];));
																																	
																																	
//creating flatten files
output(source_file_pjt, ,'~kreddy::test::source_file',thor,expire(30),overwrite);
output(desti_file(ultid=13355), ,'~kreddy::test::desti_file',thor,expire(30),overwrite);

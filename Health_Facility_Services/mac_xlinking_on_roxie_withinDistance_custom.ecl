EXPORT mac_xlinking_on_roxie_withinDistance_custom (infile,Input_UniqueID,Input_CNAME = '',
															...
															OutFile, Input_IDL = '', Match_Records = false, Full_Match = false, score = 38, distance = 6, Max_ID = 10) := MACRO
...
%layout_seq% %AssignSeq%(infile l) := transform
	SELF.UniqueId := (typeof(SELF.uniqueId))l.Input_UniqueID;
  #IF ( #TEXT(Input_CNAME) <> '' )
...
%infile_seq% := project(infile, %AssignSeq%(left));
...
%pr% := PROJECT(%apply_clean_cname%,%into%(LEFT)); // Into roxie input format
	// output (%pr%);
#uniquename(res_out)
%res_out% := project(Health_Facility_Services.MEOW_xLNPID (%pr%).Data_,%c0%);
// output (%res_out% );
...
// #uniquename(outfile)
outfile := dedup(sort(join(%res_out%, %res1%, left.uniqueid = right.uniqueid and left.lnpid = right.lnpid and left.weight = right.weight, left outer),uniqueid,-weight,lnpid,src,vendor_id),uniqueid,lnpid,src,vendor_id,hash);
endmacro;
...
EXPORT KeyName := Health_Facility_Services.Files.FILE_Address_SF; //'~'+'key::Health_Facility_Services::LNPID::Refs::ADDRESS';
...
#IF(#TEXT(Input_PRIM_NAME)<>'' AND #TEXT(Input_PRIM_RANGE)<>'' AND #TEXT(Input_ZIP)<>'')
  #uniquename(trans)
  Health_Facility_Services.Key_HealthFacility_ADDRESS.InputLayout_Batch %trans%(InFile le) := TRANSFORM
  ...
  output_file := Health_Facility_Services.Key_HealthFacility_ADDRESS.ScoredFetch_Batch(%p%,AsIndex);
#ELSE

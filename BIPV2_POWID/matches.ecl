...
mj0 := JOIN( dn0_deduped, dn0_deduped, LEFT.POWID > RIGHT.POWID AND ( LEFT.SALT_Partition = RIGHT.SALT_Partition OR LEFT.SALT_Partition='' OR RIGHT.SALT_Partition = '' )
    ...
    AND ( left.RID_If_Big_Biz = right.RID_If_Big_Biz OR left.RID_If_Big_Biz_isnull OR right.RID_If_Big_Biz_isnull )
    AND ( ~left.cnp_name_isnull AND ~right.cnp_name_isnull )
    AND ( ~left.company_name_isnull AND ~right.company_name_isnull )
    AND ( left.cnp_number = right.cnp_number OR left.cnp_number_isnull OR right.cnp_number_isnull )
...
mjs0 := o : PERSIST('~temp::POWID::BIPV2_POWID::mj::0',EXPIRE(Config.PersistExpire));
...
EXPORT All_Matches := all_matches0 : PERSIST('~temp::POWID::BIPV2_POWID::all_m',EXPIRE(Config.PersistExpire)); // To by used by rcid and POWID
SALT33.mac_avoid_transitives(All_Matches,POWID1,POWID2,Conf,DateOverlap,Rule,o);
EXPORT PossibleMatches := o : PERSIST('~temp::POWID::BIPV2_POWID::mt',EXPIRE(Config.PersistExpire));
EXPORT Matches := PossibleMatches(Conf>=MatchThreshold) : PERSIST('~temp::POWID::BIPV2_POWID::Matches::Matches',EXPIRE(Config.PersistExpire));
...
SHARED ih_thin := TABLE(ih_init,{rcid,POWID,OrgID,UltID});
  ...
  PatchOrgID := SALT33.MAC_MultiParent_Collapse(o,OrgID,POWID);  // Collapse any OrgID now joined by POWID
  PatchUltID := SALT33.MAC_MultiParent_Collapse(PatchOrgID,UltID,OrgID);  // Collapse any UltID now joined by OrgID
...
EXPORT id_shift_r := RECORD
    SALT33.UIDType rcid;
    SALT33.UIDType POWID_before;
    SALT33.UIDType POWID_after;

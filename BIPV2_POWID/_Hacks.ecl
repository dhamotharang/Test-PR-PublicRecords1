...
=-=-=-= BIPV2_POWID.MOD_Attr_charter =-=-=-=
Replace with Vern's code.  See history.

=-=-= BIPV2_POWID.Proc_Iterate =-=-=
- added keysuffix to changes filename.  so it will be unique per build and prevent superfile error too.
  import bipv2;
  EXPORT OutputChanges := OUTPUT(MM.IdChanges,,'~temp::POWID::BIPV2_POWID::changes_it'+iter+'_'+ bipv2.KeySuffix,OVERWRITE,COMPRESSED);// Changes made

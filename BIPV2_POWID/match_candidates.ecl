...
SALT33.mac_prop_field(with_props(RID_If_Big_Biz NOT IN SET(s.nulls_RID_If_Big_Biz,RID_If_Big_Biz)),RID_If_Big_Biz,POWID,RID_If_Big_Biz_props); // For every DID find the best FULL RID_If_Big_Biz
...
SALT33.mac_prop_field(with_props(company_name NOT IN SET(s.nulls_company_name,company_name)),company_name,POWID,company_name_props); // For every DID find the best FULL company_name
...
SALT33.mac_prop_field(with_props(cnp_name NOT IN SET(s.nulls_cnp_name,cnp_name)),cnp_name,POWID,cnp_name_props); // For every DID find the best FULL cnp_name
SHARED charter_prop3 := JOIN(charter_prop2,cnp_name_props,left.POWID=right.POWID,LOCAL);
 
SALT33.mac_prop_field(with_props(cnp_number NOT IN SET(s.nulls_cnp_number,cnp_number)),cnp_number,POWID,cnp_number_props); // For every DID find the best FULL cnp_number
...
SALT33.mac_prop_field(with_props(prim_range NOT IN SET(s.nulls_prim_range,prim_range)),prim_range,POWID,prim_range_props); // For every DID find the best FULL prim_range
SHARED charter_prop5 := JOIN(charter_prop4,prim_range_props,left.POWID=right.POWID,LEFT OUTER,LOCAL);
 
SALT33.mac_prop_field(with_props(prim_name NOT IN SET(s.nulls_prim_name,prim_name)),prim_name,POWID,prim_name_props); // For every DID find the best FULL prim_name
...
pj8 do_computes(pj8 le) := TRANSFORM
  SELF := le;
END;
SHARED propogated := PROJECT(pj8,do_computes(left)) : PERSIST('~temp::POWID::BIPV2_POWID::mc_props::POWID',EXPIRE(Config.PersistExpire)); // to allow to 'jump' over an exported value
...
layout_candidates add_prim_range(layout_candidates le,Specificities(ih).prim_range_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT33.MAC_Choose_JoinType(h1,s.nulls_prim_range,Specificities(ih).prim_range_values_persisted,prim_range,prim_range_weight100,add_prim_range,j6);
layout_candidates add_zip(layout_candidates le,Specificities(ih).zip_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT33.MAC_Choose_JoinType(j6,s.nulls_zip,Specificities(ih).zip_values_persisted,zip,zip_weight100,add_zip,j5);
layout_candidates add_cnp_number(layout_candidates le,Specificities(ih).cnp_number_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT33.MAC_Choose_JoinType(j5,s.nulls_cnp_number,Specificities(ih).cnp_number_values_persisted,cnp_number,cnp_number_weight100,add_cnp_number,j4);
layout_candidates add_prim_name(layout_candidates le,Specificities(ih).prim_name_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT33.MAC_Choose_JoinType(j4,s.nulls_prim_name,Specificities(ih).prim_name_values_persisted,prim_name,prim_name_weight100,add_prim_name,j3);
layout_candidates add_company_name(layout_candidates le,Specificities(ih).company_name_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF.company_name := IF( ri.field_specificity<>0 or ri.word<>'',SELF.company_name_weight100+' '+ri.word,SALT33.Fn_WordBag_AppendSpecs_Fake(le.company_name, s.company_name_specificity) );// Copy in annotated wordstring
  SELF := le;
END;
SALT33.MAC_Choose_JoinType(j3,s.nulls_company_name,Specificities(ih).company_name_values_persisted,company_name,company_name_weight100,add_company_name,j2);
layout_candidates add_cnp_name(layout_candidates le,Specificities(ih).cnp_name_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT33.MAC_Choose_JoinType(j2,s.nulls_cnp_name,Specificities(ih).cnp_name_values_persisted,cnp_name,cnp_name_weight100,add_cnp_name,j1);
layout_candidates add_RID_If_Big_Biz(layout_candidates le,Specificities(ih).RID_If_Big_Biz_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT33.MAC_Choose_JoinType(j1,s.nulls_RID_If_Big_Biz,Specificities(ih).RID_If_Big_Biz_values_persisted,RID_If_Big_Biz,RID_If_Big_Biz_weight100,add_RID_If_Big_Biz,j0);
//Using HASH(did) to get smoother distribution
SHARED Annotated := DISTRIBUTE(j0,hash(POWID)) : PERSIST('~temp::POWID::BIPV2_POWID::mc',EXPIRE(Config.PersistExpire)); // Distributed for keybuild case
...
layout_charter_candidates add_charter_RID_If_Big_Biz(layout_charter_candidates le,Specificities(ih).RID_If_Big_Biz_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT33.MAC_Choose_JoinType(charter_pp,s.nulls_RID_If_Big_Biz,Specificities(ih).RID_If_Big_Biz_values_persisted,RID_If_Big_Biz,RID_If_Big_Biz_weight100,add_charter_RID_If_Big_Biz,jcharter_0);
layout_charter_candidates add_charter_cnp_name(layout_charter_candidates le,Specificities(ih).cnp_name_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT33.MAC_Choose_JoinType(jcharter_0,s.nulls_cnp_name,Specificities(ih).cnp_name_values_persisted,cnp_name,cnp_name_weight100,add_charter_cnp_name,jcharter_1);
layout_charter_candidates add_charter_company_name(layout_charter_candidates le,Specificities(ih).company_name_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT33.MAC_Choose_JoinType(jcharter_1,s.nulls_company_name,Specificities(ih).company_name_values_persisted,company_name,company_name_weight100,add_charter_company_name,jcharter_2);
layout_charter_candidates add_charter_prim_name(layout_charter_candidates le,Specificities(ih).prim_name_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT33.MAC_Choose_JoinType(jcharter_2,s.nulls_prim_name,Specificities(ih).prim_name_values_persisted,prim_name,prim_name_weight100,add_charter_prim_name,jcharter_3);
layout_charter_candidates add_charter_cnp_number(layout_charter_candidates le,Specificities(ih).cnp_number_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT33.MAC_Choose_JoinType(jcharter_3,s.nulls_cnp_number,Specificities(ih).cnp_number_values_persisted,cnp_number,cnp_number_weight100,add_charter_cnp_number,jcharter_4);
layout_charter_candidates add_charter_prim_range(layout_charter_candidates le,Specificities(ih).prim_range_values_persisted ri,BOOLEAN patch_default) := TRANSFORM
  ...
  SELF := le;
END;
SALT33.MAC_Choose_JoinType(jcharter_4,s.nulls_prim_range,Specificities(ih).prim_range_values_persisted,prim_range,prim_range_weight100,add_charter_prim_range,jcharter_6);
EXPORT charter_candidates := jcharter_6 : PERSIST('~temp::POWID::BIPV2_POWID::mc::charter',EXPIRE(Config.PersistExpire));

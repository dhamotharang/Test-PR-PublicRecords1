...
EXPORT FromNone(DATASET(Layout_POWID) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.num_legal_names_Invalid := Fields.InValid_num_legal_names((SALT33.StrType)le.num_legal_names);
      SELF.num_legal_names := IF(SELF.num_legal_names_Invalid=0 OR NOT withOnfail, le.num_legal_names, SKIP); // ONFAIL(REJECT)
    SELF.num_incs_Invalid := Fields.InValid_num_incs((SALT33.StrType)le.num_incs);
      SELF.num_incs := IF(SELF.num_incs_Invalid=0 OR NOT withOnfail, le.num_incs, SKIP); // ONFAIL(REJECT)
    SELF.nodes_total_Invalid := Fields.InValid_nodes_total((SALT33.StrType)le.nodes_total);
      SELF.nodes_total := IF(SELF.nodes_total_Invalid=0 OR NOT withOnfail, le.nodes_total, SKIP); // ONFAIL(REJECT)
    SELF.zip4_Invalid := Fields.InValid_zip4((SALT33.StrType)le.zip4);
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.FieldContents := CHOOSE(c,(SALT33.StrType)le.cnp_name,(SALT33.StrType)le.company_name,(SALT33.StrType)le.zip,(SALT33.StrType)le.num_legal_names,(SALT33.StrType)le.num_incs,(SALT33.StrType)le.nodes_total,(SALT33.StrType)le.zip4,'***SALTBUG***');
  END;

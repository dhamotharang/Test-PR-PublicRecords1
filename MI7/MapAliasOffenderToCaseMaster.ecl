...
SELF.SUBJECT_LAST_NAME_C :=    IF(USE_FIRST_LOGIC_NAMES='Y',L.LNAME,TRIM(L.PTY_NM)); 
...
SELF.SUBJECT_FULL_NAME_C := IF(USE_FIRST_LOGIC_NAMES='Y',
                                                                            TRIM(IF(LENGTH(TRIM(L.LNAME))> 0,TRIM(L.LNAME) + ' ','')
...
SELF.SOURCE_UID_C :=  L.CM_SOURCE_UID;
...
export MapAliasOffenderToCaseMaster := SORT(ds_CaseMasterAlias,ecl_cade_id)
                                                                    : persist ('~thor_data400::persist::out::crim::cross_aoc_and_county::MapAliasOffenderToCaseMaster');   

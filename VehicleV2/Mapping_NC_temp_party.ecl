...
dNCBaseDist	:=	distribute(	dNCBase,
														hash(	vehicle_key,
																	state_origin,
																	source_code,
																	ORIG_VIN,
...
dNCTempMainDist	:=	distribute(	dNCTempMain,
																hash( vehicle_key,
																			state_origin,
																			source_code,
																			ORIG_VIN,
...
dNCTempMainSort	:=	sort(	dNCTempMainDist,
													vehicle_key,
													state_origin,
													source_code,
													ORIG_VIN,
...
VehicleV2.Layout_NC.NC_as_VehicleV2_Layout	tIterationKey(dNCBaseSort	L,dNCTempMainSort	R)	:=
transform
	self.iteration_key	:=	R.iteration_key;
	self								:=	L;
end;

dNCPartyIterationKey	:=	join(	dNCBaseSort,
																dNCTempMainSort,
...
dNC2Party :=	normalize(dNCPartyIterationKey,2,tParty(left,counter));

dNCAddrLastPopulated		:=	dNC2Party(Append_Ace1_PrepAddr2	!=	'');
dNCAddrLastNotpopulated	:=	dNC2Party(Append_Ace1_PrepAddr2	=	'');
...
AID.MacAppendFromRaw_2Line(	dNCAddrLastPopulated,
...
dNCCleanAddress	:=	project(dNCPartyAppendAID,tAceAddress(left));

export	Mapping_NC_Temp_Party	:=	dNCCleanAddress	+	dNCAddrLastNotpopulated	:	persist('~thor_data400::persist::vehicleV2::nc_temp_party');
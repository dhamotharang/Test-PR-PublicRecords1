export MAC_SF_BuildProcess(thedataset, basename, seq_name, numgenerations = '3', csvout = 'false', pCompress = 'false') := macro
/*
	...
	if(%ng% not in [2,3], fail('ut.MAC_SF_BuildProcess failure, numgenerations not in [2,3]'),
  	 if(%workalreadydone%(FileServices.GetSuperFileSubName(basename,1)),
		...
		sequential(
			#if(csvout != true)
				#if(pCompress != true)
					output(thedataset,,basename + thorlib.WUID(),overwrite),
				#else
					output(thedataset,,basename + thorlib.WUID(),overwrite,__compressed__),
				#end
			#else
				output(thedataset,,basename+thorlib.wuid(),overwrite,csv(HEADING(single),SEPARATOR('|'),TERMINATOR('\r\n'))),
			#end

...
export Proc_Copy_Keys_To_Alpha(string filedate,string envment = 'nonfcra') := function
	dsname := 'VehicleV2Keys';
	...
	string_rec replacedate(ds l) := transform
		string modfilename := if(envment = 'ct',
									regexreplace(dsname+'_DATE','~'+l.logicalkeys,'custtest::'+filedate),
									regexreplace(dsname+'_DATE','~'+l.logicalkeys,filedate));

...
module
	...
	module
		...
		export	Main			          :=	dataset(ut.foreign_prod+'~thor_data400::base::vehicleV2::main',VehicleV2.Layout_Base.Main,thor);
		export	Party_bip	          :=	dataset(ut.foreign_prod+'~thor_data400::base::vehicleV2::party',VehicleV2.Layout_Base.Party_bip,thor);		
		export	Party			          :=	project(Party_bip,transform(VehicleV2.Layout_Base.Party,self:=left));
		export  Picker              :=  dataset(ut.foreign_prod+'~thor_data400::out::vehicle_valid_make_model', {string MakeID,string MakeCode,string MakeDescription,	string ModelDescription}, csv(separator('|')));
		export  MakeConst           :=  dataset(ut.foreign_prod+'~thor_data400::out::vehicle_valid_make_constant',{string MakeCode}, flat); 

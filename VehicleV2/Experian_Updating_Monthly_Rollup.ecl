...
dExperianAll			:=	VehicleV2.file_experian_in(first_nm<>'' or middle_nm<>'' or last_nm<>'');
...
dExperianFullDist		:=	distribute(dExperianAllReady,hash(MSTR_SRC_STATE, VIN));
...
unsigned3 fBestSeenDate(string8 pRegDate, string8 pRegExpDate,string8 pTitleDate, string8 pProcessDate, string8 pEarliestProcessDate)
 :=
  function
	unsigned3	lRegDate			:=	(unsigned8)(pRegDate[1..6]);
	unsigned3   lRegExpDate         :=  (unsigned8)(pRegExpDate[1..6]);
	...
	unsigned3	lReturnValue		:=	if(lProcessDate > lEarliestProcessDate + 2,
										   ...
										   if(lRegDate <= lProcessDate and lRegDate <> 0,
											  lRegDate,
										   if(lRegExpDate <= lProcessDate and lRegExpDate <> 0,
										      lRegExpDate,
...
VehicleV2.Layout_Experian_Updating_temp_module.Layout_Experian_Updating_as_VehicleV2	tExperianAsVehicleWithKey(dExperianFullDist pInput, dExperianMinProcessDate pMinDateTable)
 :=
  transform
  
	self.state_origin				:=	pInput.append_STATE_ORIGIN;
	self.source_code				:=	'AE';
	self.dt_first_seen				:=	fBestSeenDate(fFixDate(pInput.REG_RENEW_DT),fFixDate(pInput.REG_exp_DT),fFixDate(if(pInput.TITLE_TRANS_DT<>'',pInput.TITLE_TRANS_DT,pInput.ORG_TITLE_DT)),pInput.append_PROCESS_DATE,pMinDateTable.EarliestProcessDate);
    self.dt_last_seen				:=	fBestSeenDate(fFixDate(pInput.REG_RENEW_DT),fFixDate(pInput.REG_exp_DT),fFixDate(if(pInput.TITLE_TRANS_DT<>'',pInput.TITLE_TRANS_DT,pInput.ORG_TITLE_DT)),pInput.append_PROCESS_DATE,pMinDateTable.EarliestProcessDate);
	...
	self.REGISTRATION_EXPIRATION_DATE	:= if((unsigned8)fFixDate(pInput.REG_EXP_DT)[1..6] > (unsigned8)fFixDate(pInput.REG_RENEW_DT)[1..6], fFixDate(pInput.REG_EXP_DT), '');
	...
	self.REGISTRATION_EFFECTIVE_DATE	:= if((unsigned8)fFixDate(pInput.REG_EXP_DT)[1..6] > (unsigned8)fFixDate(pInput.REG_RENEW_DT)[1..6], fFixDate(pInput.REG_RENEW_DT), '');
	...
	self.orig_name                  :=  fFixUnknownForSort(fFixOrigName(pInput.OWNER_TYP_CD,pInput.FIRST_NM,pInput.MIDDLE_NM,pInput.LAST_NM,pInput.NAME_SUFFIX,pInput.PROF_SUFFIX));
	...
	self.orig_lien_date                  :=      '';
	self.history := '';						 
	self.title 		 :=      fFixUnknownForSort(pInput.P_NAME[01..05]);                                                                                                                      
    self.fname 		 :=      fFixUnknownForSort(pInput.P_NAME[06..25]);                                                                                                                      
    self.mname 		 :=      fFixUnknownForSort(pInput.P_NAME[26..45]);                                                                                                                      
    self.lname 		 :=      fFixUnknownForSort(pInput.P_NAME[46..65]);                                                                                                                      
    self.name_suffix :=      fFixUnknownForSort(pInput.P_NAME[66..70]);                                                                                                                      
    self.name_score	 :=      fFixUnknownForSort(pInput.P_NAME[71..73]); 
    self.Append_Clean_cname			     :=      fFixUnknownForSort(if(pInput.OWNER_TYP_CD = '2',self.orig_name,''));                                                                       
...
dMonthlyOnly			:=	join(dExperianFullDist,dExperianMinProcessDate,
							 left.append_STATE_ORIGIN = right.append_STATE_ORIGIN,
							 tExperianAsVehicleWithKey(left,right),
							 lookup);  // no version in Dataland, so changed directly in Prod

dMonthlySort   := sort(dMonthlyOnly, 
...
typeof(dMonthlySort) tRollup(dMonthlySort L, dMonthlySort R) := transform
...
dMonthlyRollup   := rollup(dMonthlySort, tRollup(left, right),
...
export experian_updating_monthly_rollup := dMonthlyRollup; 

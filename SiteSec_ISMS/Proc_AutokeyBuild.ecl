...
EXPORT proc_autokeybuild(STRING filedate) := FUNCTION
		...
		c := SiteSec_ISMS.Constants(filedate);
	...
	mod_AKB := module(AutokeyB2.Fn_Build.params) 
		...
		EXPORT STRING L_inkeyname := 	SiteSec_ISMS.constants().ak_keyname; 
		EXPORT STRING L_inlogical := 	SiteSec_ISMS.constants(filedate).ak_logical; 

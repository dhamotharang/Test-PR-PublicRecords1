IMPORT Header, MDR, PRTE2_Certegy;

export As_header(dataset(layouts.certegy_prod) pCertegy = Files.Certegy_Header) := function

  Header.Layout_New_Records Translate_certegy_to_Header(pCertegy L) := transform
    ...
    SELF.SRC                      := mdr.sourcetools.SRC_Certegy;
    self                          := L;
  end;

  certegy_project := project(pCertegy, Translate_certegy_to_Header(left));

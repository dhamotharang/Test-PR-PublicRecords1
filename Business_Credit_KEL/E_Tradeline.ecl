...
EXPORT E_Tradeline(CFG_graph.FDCDataset __in = CFG_graph.FDCDefault, CFG_graph __cfg = CFG_graph) := MODULE
  ...
  Layout Tradeline__Rollup(InLayout __r, DATASET(InLayout) __recs) := TRANSFORM
    ...
    SELF._ln__delinquency__date_ := KEL.Intake.SingleValue(__recs,_ln__delinquency__date_);
  ...
  EXPORT D_B_T___V5__SingleValue_Invalid := KEL.Intake.DetectMultipleValues(__PreResult,D_B_T___V5_);
  EXPORT _ln__delinquency__date__SingleValue_Invalid := KEL.Intake.DetectMultipleValues(__PreResult,_ln__delinquency__date_);
  ...
  EXPORT NullCounts := DATASET([
    ...
    {'Tradeline','Business_Credit_KEL.File_SBFE_temp','ln_delinquency_date',COUNT(__d0(__NL(_ln__delinquency__date_))),COUNT(__d0(__NN(_ln__delinquency__date_)))},

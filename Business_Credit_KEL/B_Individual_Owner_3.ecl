...
EXPORT B_Individual_Owner_3(CFG_graph.FDCDataset __in = CFG_graph.FDCDefault, CFG_graph __cfg = CFG_graph) := MODULE
  ...
  SHARED __EE506812 := __E_Individual_Owner;
  EXPORT __ST242475_Layout := RECORD
    KEL.typ.nuid UID;
  ...
  END;
  SHARED __ST242475_Layout __ND9857018__Project(E_Individual_Owner(__in,__cfg).Layout __PP506683) := TRANSFORM
    SELF.Name_ := __OP2(__OP2(__FN1(TRIM,__PP506683._first__name_),+,__CN(' ')),+,__FN1(TRIM,__PP506683._last__name_));
    SELF := __PP506683;
  END;
  EXPORT __ENH_Individual_Owner_3 := PROJECT(__EE506812,__ND9857018__Project(LEFT));
END;

//Executable code
EXPORT Proc_Iterate(STRING iter,DATASET(Layout_LGID3) InFile0 = BIPV2_LGID3_dev.In_LGID3,STRING OutFileNameP = '~temp::LGID3::BIPV2_LGID3_dev::it',UNSIGNED MatchThreshold = Config.MatchThreshold,BOOLEAN Debugging = true) := MODULE
SHARED InFile := InFile0;
SHARED MM := BIPV2_LGID3_dev.matches(InFile,MatchThreshold); // Get the matching module
...
SHARED BM := BIPV2_LGID3_dev.BasicMatch(InFile);
...
OS := OUTPUT(BIPV2_LGID3_dev.specificities(InFile).specificities,NAMED('Specificities'));
OPRP := OUTPUT(BIPV2_LGID3_dev.match_candidates(InFile).prepropogationstats,NAMED('PrePopStats'));
OPP := OUTPUT(BIPV2_LGID3_dev.match_candidates(InFile).postpropogationstats,NAMED('PostPopStats'));
...
MP := PARALLEL( OUTPUT(MM.MatchesPerformed,NAMED('MatchesPerformed')),OUTPUT(BIPV2_LGID3_dev.BasicMatch(InFile).basic_match_count,NAMED('BasicMatchesPerformed')));
...
SpcS := OUTPUT(BIPV2_LGID3_dev.specificities(InFile).SpcShift,NAMED('SPCShift'));
EXPORT ExecutionStats := PARALLEL(OS,SpcS,PRPP,PPP,PRPPS,PPPS,MP,SP,RE,CB,MPPA,MPPR,OPRP,OPP);
d := DATASET([{MM.PatchingError0,MM.DuplicateRids0,COUNT(BIPV2_LGID3_dev.match_candidates(InFile).Unlinkables),COUNT(BIPV2_LGID3_dev.specificities(InFile).Rejected_file)}],{INTEGER PatchingError0, INTEGER DuplicateRids0, UNSIGNED UnlinkableRecords0,UNSIGNED RecordsRejected0});
...
import bipv2;
ChangeName := '~temp::LGID3::BIPV2_LGID3_dev::changes_it'+iter+'_'+ bipv2.KeySuffix;
...
//EXPORT LoopN(UNSIGNED N,UNSIGNED mt=Config.MatchThreshold) := LOOP(InFile,N,BIPV2_LGID3_dev.matches(ROWS(LEFT),mt).patched_infile); // Loop N times
//EXPORT LoopThisN(DATASET(BIPV2_LGID3_dev.Layout_LGID3)d,UNSIGNED N,UNSIGNED mt=Config.MatchThreshold) := LOOP(d,N,BIPV2_LGID3_dev.matches(ROWS(LEFT),mt).patched_infile);
//EXPORT LoopThisK(DATASET(BIPV2_LGID3_dev.Layout_LGID3)d,UNSIGNED K,UNSIGNED NMax=100) := LOOP(d,COUNTER<=NMax AND COUNT(DEDUP(ROWS(LEFT),LGID3,ALL))>K,BIPV2_LGID3_dev.matches(ROWS(LEFT),0).patched_infile); // Loop until K clusters left

ih                      :=BIPV2_LGID3_dev.In_LGID3;
psetReviewers					  := ['TL','CM','LB','DW'];
pNumSamplesPerReviewer	:= 150;
ConfThreshold					  := '41';
kmtch									  := BIPV2_LGID3_dev.Keys(ih).MatchSample;
kcand									  := BIPV2_LGID3_dev.Keys(ih).Candidates;
outputecl               := true;
xtraMatchFilter         := 'company_inc_state_score <= 0';
lFractionAtThreshold    := 1.0                         ;

// out										  := tools.mac_GetSALTReviewSamples(kmtch,kcand,BIPV2_LGID3_dev.In_LGID3,lgid3,ConfThreshold,pNumSamplesPerReviewer,psetReviewers,outputecl,lFractionAtThreshold);
...
allproxidcands    := project(kcand(lgid3 in setsampleproxids)  ,BIPV2_LGID3_dev.match_candidates(BIPV2_LGID3_dev.In_LGID3).layout_candidates);
kspec						  := BIPV2_LGID3_dev.Keys(ih).Specificities_Key;
s := project(kspec,BIPV2_LGID3_dev.Layout_Specificities.R)[1];
// s := dataset([],BIPV2_LGID3_dev.Layout_Specificities.R);  //hack
//s := project(dataset([{''}],{string stuff}),transform(BIPV2_LGID3_dev.Layout_Specificities.R,self := []));  //hack
allproxidsrolled := BIPV2_LGID3_dev.Debug(BIPV2_LGID3_dev.In_LGID3,s).RolledEntities(allproxidcands);
layrolled := recordof(allproxidsrolled);


//////// -- hack
dmtches := project(allsamplerecs,transform(BIPV2_LGID3_dev.match_candidates(BIPV2_LGID3_dev.In_LGID3).layout_matches,self.LGID31 := left.lgid31,self.LGID32 := left.lgid32,self := []));
// DATASET([{0,0,0,0,LGID3one,LGID3two,0,0}],BIPV2_LGID3_dev.match_candidates(BIPV2_LGID3_dev.In_LGID3).layout_matches);

mtch := BIPV2_LGID3_dev.Debug(BIPV2_LGID3_dev.In_LGID3,s).AnnotateMatchesFromData(allproxidcands,dmtches );

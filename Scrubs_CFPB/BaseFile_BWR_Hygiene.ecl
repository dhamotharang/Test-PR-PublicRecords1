...
#workunit('name','Scrubs_CFPB.BaseFile_BWR_Hygiene - Hygiene & Stats - SALT V3.11.9');
IMPORT Scrubs_CFPB,SALT311;
// First create an instantiated hygiene module
  infile := Scrubs_CFPB.BaseFile_In_CFPB;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_CFPB.BaseFile_hygiene(ip);
  ...
  OUTPUT(SALT311.MAC_Character_Counts.EclRecord(p,'BaseFile_Layout_CFPB'),NAMED('OptimizedLayout'));// File layout suggested by data

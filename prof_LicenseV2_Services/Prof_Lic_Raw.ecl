import Autokey_batch, BatchServices, doxie_cbrs, doxie, Prof_LicenseV2, doxie_files,
       Ingenix_NatlProf, AutokeyB2, prof_licensev2_services,
			 AutoStandardI;

export Prof_Lic_raw := Module

	// Gets RMSIDs from BDIDs
	export get_Prolic_seq_id_from_bdids(dataset(doxie_cbrs.layout_references) in_bdids,
	  ...
	  key := Prof_LicenseV2.Key_Proflic_Bdid;
		res := join(dedup(sort(in_bdids,bdid),bdid),key,
		            keyed(left.bdid = right.bd),
								transform(prof_LicenseV2_Services.Layout_Search_Ids_Prolic,self := right),
    ...
    EXPORT doxie.ingenix_provider_module.layout_ingenix_provider_report_for_batch process_language ( doxie.ingenix_provider_module.layout_ingenix_provider_report_for_batch l ) :=
      ...
      END;
		...
		export prof_LicenseV2_Services.assorted_layouts.ProvId_dea
		                    get_prolic_seq_id_by_npi( dataset( prof_licensev2_services.assorted_layouts.ProvId_NPI) npi) := FUNCTION		                          					
           ...
           provID_dea_recs := dedup(sort(join(prov_dids(did <> 0), Prof_LicenseV2.Key_Proflic_Did (),
					                      left.did = right.did,
					                      transform( prof_LicenseV2_Services.assorted_layouts.ProvId_dea,
													  			self.acctno := left.acctno,
														  		self.did := left.did,																  

...
Search_layout_plus_compliance := RECORD(Search_layout_plus)
  unsigned4 global_sid := 0;
  unsigned8 record_sid := 0;
END;

report_layout_plus_compliance := RECORD(report_layout)
  unsigned4 global_sid := 0;
  unsigned8 record_sid := 0;
END;
...
EXPORT get_ProfLic(dataset(prof_LicenseV2_Services.Layout_Search_Ids_Prolic) Prolic_Ids,
  ...
  Search_layout_plus_compliance get_base_fields_prolic_search(prolic_Ids l, Prof_key r):=transform
    self.orig_license_number := if(in_licnum='' or
  ...
  prolics_search_xtra_fields_suppressed := Suppress.MAC_SuppressSource(_prolics_search_xtra_fields, mod_access);
  Doxie.compliance.logSoldToSources(prolics_search_xtra_fields_suppressed, mod_access);
  prolics_search_xtra_fields := PROJECT(prolics_search_xtra_fields_suppressed, Search_layout_plus);
  ...
  report_layout_plus_compliance get_base_fields_prolic_report(Prof_key r):= transform
    self.source_st_decoded := codes.GENERAL.state_long(r.source_st);
    self.uniqueid := (string17) ('PL' + trim((string15)r.prolic_seq_id));
    ...
    self := [];
  END;
  _prolics_report := dedup(sort(join(prolic_Ids, prof_key
              ,keyed(left.prolic_seq_id = right.prolic_seq_id)
  ...
  prolics_report_suppressed := Suppress.MAC_SuppressSource(_prolics_report, mod_access);
  Doxie.compliance.logSoldToSources(prolics_report_suppressed, mod_access);
  prolics_report := PROJECT(prolics_report_suppressed, report_layout);

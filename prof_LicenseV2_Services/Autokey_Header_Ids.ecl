...
export val_prolic(params in_params) := function
    ...
    tempmod_prolic := module(project(global_mod, AutoKeyI.AutoKeyStandardFetchArgumentInterface,opt))
    ...
    //***** DIDs
     ...
     newbydid_prolic := prof_LicenseV2_Services.Prof_Lic_Raw.get_Prolic_seq_id_from_dids(dedup(newdids_prolic
        ...
        +dids,all), mod_access := mod_access);

    //***** BDIDs
    tempbhmod := module(project(global_mod ,AutoHeaderI.LIBIN.FetchI_Hdr_Biz.full,opt))

...
#workunit('name','Scrubs_FraudGov.NAC_BWR_Hygiene - Hygiene & Stats - SALT V3.9.0');
IMPORT Scrubs_FraudGov,SALT39;
// First create an instantiated hygiene module
  infile := Scrubs_FraudGov.NAC_In_NAC;
  ip := DISTRIBUTE(infile, SKEW(0.1));
  h := Scrubs_FraudGov.NAC_hygiene(ip);
  ...
  OUTPUT(SALT39.MAC_Character_Counts.EclRecord(p,'NAC_Layout_NAC'),NAMED('OptimizedLayout'));// File layout suggested by data

...
EXPORT File_reader_targus := module 
  ...
  EXPORT formatRequest(dataset($.layouts.decoded_xml_in) infile) := FUNCTION
    ...
    END;
    recs := project(infile, TRANSFORM(targus.layout_targus_in, 
  ...
  EXPORT formatResponse(dataset($.layouts.decoded_xml_in) infile) := FUNCTION
    ...
    END;
    recs := project(infile, TRANSFORM(targus.Layout_Targus_out, 
      
		  // SELF.response := FROMXML(layout_response, '<Row>' + left.response_data + '</Row>').response;

...
EXPORT Delta(DATASET(Layout_AddressFeedback)old_s, DATASET(Layout_AddressFeedback) new_s) := MODULE//Routines to compute the differences between two instances of a file
  SHARED inFieldList := ['address_feedback_id','login_history_id','phone_number','unique_id','fname','lname','mname','orig_street_pre_direction','orig_street_post_direction','orig_street_number','orig_street_name','orig_street_suffix','orig_unit_number','orig_unit_designation','orig_zip5','orig_zip4','orig_city','orig_state','alt_phone','other_info','address_contact_type','feedback_source','transaction_id','date_time_added','loginid','companyid'];

...
EXPORT Summary(SALT311.Str30Type  txt) := FUNCTION
  ...
  R1 := RECORD
    UNSIGNED LinkingPotential :=  + T.Populated_address_feedback_id_pcnt *   0.00 / 100 + T.Populated_login_history_id_pcnt *   0.00 / 100 + T.Populated_phone_number_pcnt *   0.00 / 100 + T.Populated_unique_id_pcnt *   0.00 / 100 + T.Populated_fname_pcnt *   0.00 / 100 + T.Populated_lname_pcnt *   0.00 / 100 + T.Populated_mname_pcnt *   0.00 / 100 + T.Populated_orig_street_pre_direction_pcnt *   0.00 / 100 + T.Populated_orig_street_post_direction_pcnt *   0.00 / 100 + T.Populated_orig_street_number_pcnt *   0.00 / 100 + T.Populated_orig_street_name_pcnt *   0.00 / 100 + T.Populated_orig_street_suffix_pcnt *   0.00 / 100 + T.Populated_orig_unit_number_pcnt *   0.00 / 100 + T.Populated_orig_unit_designation_pcnt *   0.00 / 100 + T.Populated_orig_zip5_pcnt *   0.00 / 100 + T.Populated_orig_zip4_pcnt *   0.00 / 100 + T.Populated_orig_city_pcnt *   0.00 / 100 + T.Populated_orig_state_pcnt *   0.00 / 100 + T.Populated_alt_phone_pcnt *   0.00 / 100 + T.Populated_other_info_pcnt *   0.00 / 100 + T.Populated_address_contact_type_pcnt *   0.00 / 100 + T.Populated_feedback_source_pcnt *   0.00 / 100 + T.Populated_transaction_id_pcnt *   0.00 / 100 + T.Populated_date_time_added_pcnt *   0.00 / 100 + T.Populated_loginid_pcnt *   0.00 / 100 + T.Populated_companyid_pcnt *   0.00 / 100;
    T;
  END;
  RETURN TABLE(T,R1);
END;
...
invRec invert(summary0 le, INTEGER C) := TRANSFORM
  ...
  SELF.populated_pcnt := CHOOSE(C,le.populated_address_feedback_id_pcnt,le.populated_login_history_id_pcnt,le.populated_phone_number_pcnt,le.populated_unique_id_pcnt,le.populated_fname_pcnt,le.populated_lname_pcnt,le.populated_mname_pcnt,le.populated_orig_street_pre_direction_pcnt,le.populated_orig_street_post_direction_pcnt,le.populated_orig_street_number_pcnt,le.populated_orig_street_name_pcnt,le.populated_orig_street_suffix_pcnt,le.populated_orig_unit_number_pcnt,le.populated_orig_unit_designation_pcnt,le.populated_orig_zip5_pcnt,le.populated_orig_zip4_pcnt,le.populated_orig_city_pcnt,le.populated_orig_state_pcnt,le.populated_alt_phone_pcnt,le.populated_other_info_pcnt,le.populated_address_contact_type_pcnt,le.populated_feedback_source_pcnt,le.populated_transaction_id_pcnt,le.populated_date_time_added_pcnt,le.populated_loginid_pcnt,le.populated_companyid_pcnt);
  SELF.maxlength := CHOOSE(C,le.maxlength_address_feedback_id,le.maxlength_login_history_id,le.maxlength_phone_number,le.maxlength_unique_id,le.maxlength_fname,le.maxlength_lname,le.maxlength_mname,le.maxlength_orig_street_pre_direction,le.maxlength_orig_street_post_direction,le.maxlength_orig_street_number,le.maxlength_orig_street_name,le.maxlength_orig_street_suffix,le.maxlength_orig_unit_number,le.maxlength_orig_unit_designation,le.maxlength_orig_zip5,le.maxlength_orig_zip4,le.maxlength_orig_city,le.maxlength_orig_state,le.maxlength_alt_phone,le.maxlength_other_info,le.maxlength_address_contact_type,le.maxlength_feedback_source,le.maxlength_transaction_id,le.maxlength_date_time_added,le.maxlength_loginid,le.maxlength_companyid);
  SELF.avelength := CHOOSE(C,le.avelength_address_feedback_id,le.avelength_login_history_id,le.avelength_phone_number,le.avelength_unique_id,le.avelength_fname,le.avelength_lname,le.avelength_mname,le.avelength_orig_street_pre_direction,le.avelength_orig_street_post_direction,le.avelength_orig_street_number,le.avelength_orig_street_name,le.avelength_orig_street_suffix,le.avelength_orig_unit_number,le.avelength_orig_unit_designation,le.avelength_orig_zip5,le.avelength_orig_zip4,le.avelength_orig_city,le.avelength_orig_state,le.avelength_alt_phone,le.avelength_other_info,le.avelength_address_contact_type,le.avelength_feedback_source,le.avelength_transaction_id,le.avelength_date_time_added,le.avelength_loginid,le.avelength_companyid);
END;
...
SHARED FldIds := DATASET([{1,'address_feedback_id'}
      ,{2,'login_history_id'}
      ,{3,'phone_number'}
      ,{4,'unique_id'}
      ,{5,'fname'}
      ,{6,'lname'}
      ,{7,'mname'}
      ,{8,'orig_street_pre_direction'}
      ,{9,'orig_street_post_direction'}
      ,{10,'orig_street_number'}
      ,{11,'orig_street_name'}
      ,{12,'orig_street_suffix'}
      ,{13,'orig_unit_number'}
      ,{14,'orig_unit_designation'}
      ,{15,'orig_zip5'}
      ,{16,'orig_zip4'}
      ,{17,'orig_city'}
      ,{18,'orig_state'}
      ,{19,'alt_phone'}
      ,{20,'other_info'}
      ,{21,'address_contact_type'}
      ,{22,'feedback_source'}
      ,{23,'transaction_id'}
      ,{24,'date_time_added'}
      ,{25,'loginid'}
      ,{26,'companyid'}],SALT311.MAC_Character_Counts.Field_Identification);
...
PrettyErrorTotals := RECORD
  ...
  FieldType := CHOOSE(TotalErrors.FieldNum,'Invalid_No','Invalid_No','Invalid_Phone','Invalid_No','Invalid_Alpha','Invalid_Alpha','Invalid_Alpha','Invalid_Dir','Invalid_Dir','Invalid_Float','Invalid_AlphaNum','Invalid_Alpha','Invalid_AlphaNum','Invalid_Alpha','Invalid_Zip','Invalid_No','Invalid_Alpha','Invalid_State','Invalid_Phone','Unknown','Invalid_No','Invalid_No','Invalid_AlphaNum','Invalid_AlphaNumChar','Invalid_No','Invalid_No');
  ErrorMessage := CHOOSE(TotalErrors.FieldNum,Fields.InValidMessage_address_feedback_id(TotalErrors.ErrorNum),Fields.InValidMessage_login_history_id(TotalErrors.ErrorNum),Fields.InValidMessage_phone_number(TotalErrors.ErrorNum),Fields.InValidMessage_unique_id(TotalErrors.ErrorNum),Fields.InValidMessage_fname(TotalErrors.ErrorNum),Fields.InValidMessage_lname(TotalErrors.ErrorNum),Fields.InValidMessage_mname(TotalErrors.ErrorNum),Fields.InValidMessage_orig_street_pre_direction(TotalErrors.ErrorNum),Fields.InValidMessage_orig_street_post_direction(TotalErrors.ErrorNum),Fields.InValidMessage_orig_street_number(TotalErrors.ErrorNum),Fields.InValidMessage_orig_street_name(TotalErrors.ErrorNum),Fields.InValidMessage_orig_street_suffix(TotalErrors.ErrorNum),Fields.InValidMessage_orig_unit_number(TotalErrors.ErrorNum),Fields.InValidMessage_orig_unit_designation(TotalErrors.ErrorNum),Fields.InValidMessage_orig_zip5(TotalErrors.ErrorNum),Fields.InValidMessage_orig_zip4(TotalErrors.ErrorNum),Fields.InValidMessage_orig_city(TotalErrors.ErrorNum),Fields.InValidMessage_orig_state(TotalErrors.ErrorNum),Fields.InValidMessage_alt_phone(TotalErrors.ErrorNum),Fields.InValidMessage_other_info(TotalErrors.ErrorNum),Fields.InValidMessage_address_contact_type(TotalErrors.ErrorNum),Fields.InValidMessage_feedback_source(TotalErrors.ErrorNum),Fields.InValidMessage_transaction_id(TotalErrors.ErrorNum),Fields.InValidMessage_date_time_added(TotalErrors.ErrorNum),Fields.InValidMessage_loginid(TotalErrors.ErrorNum),Fields.InValidMessage_companyid(TotalErrors.ErrorNum));
  TotalErrors.Cnt;
END;

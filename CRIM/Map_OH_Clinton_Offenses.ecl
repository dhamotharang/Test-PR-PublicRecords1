...
ds_def := sort(CRIM.File_OH_Clinton.Defendant, def_id);
ds_chrg := sort(CRIM.File_OH_Clinton.Charge, def_id);
ds_sent := sort(CRIM.File_OH_Clinton.Sentence, def_id);
...
Crim_Common.Layout_In_Court_Offenses tOHCrim(d input) := Transform
	...
	self.offender_key			:= '3G'+hash(clean_def_name)+input.case_num+fSlashedMDYtoCYMD(input.file_dt);
	self.vendor 				:= '3G';
	self.state_origin 			:= 'OH';
	self.source_file 			:= '(CP)OH CLINTON Cnty';
	self.off_comp 				:= '';
	...
	self.court_disp_desc_1	 	:= if(trim(input.dispo_detail, left, right)='', 
										regexreplace(';',trim(input.comments, left, right),''), 
										regexreplace(';',trim(input.dispo_detail, left, right),''));
...
fOHOffend:= dedup(sort(distribute(pOHCrim,hash(offender_key)),
				offender_key,off_comp,court_statute,court_statute_desc,court_off_code,court_off_desc_1, court_disp_desc_1,local),
					offender_key,off_comp,court_statute,court_statute_desc,court_off_code,court_off_desc_1,court_disp_desc_1,all,local,left)
									:PERSIST('~thor_data400::persist::Crim_OH_Clinton_Offenses');

export Map_OH_Clinton_Offenses := fOHOffend;
...
self.offender_key		:= '1Z'+trim(l.casenumber,left,right)+l.id;
self.vendor				:= '1Z';
self.state_origin		:= 'OK';
self.data_type			:= '2';
self.source_file		:= 'OK_LOGAN_CRIM_COURT';
...
dedOffender := dedup(sort(distribute(cleanOffender,hash(offender_key)),
										...
										:PERSIST('~thor_dell400::persist::Crim_OK_LOGAN_Offender_Clean')
										;

export Map_OK_LOGAN_Offender := dedOffender;
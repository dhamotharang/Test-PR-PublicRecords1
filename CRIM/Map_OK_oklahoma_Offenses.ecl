...
input := crim.File_OK_OKLAHOMA(regexfind('[0-9]', casenumber));
...
self.court_disp_desc_1	 		:= regexreplace('DISMISSED Dismissed- Request of the State', regexreplace('DISMISSED WITH COSTS Dismissed with Cost', if(fSlashedMDYtoCYMD(l.disposed) = '00000000' or ~regexfind('(ACQUITTED)|(DEFERRED)|(DISCHARG)|(DISMISSED)|(Guilty)', l.disposed),'' , trim(regexreplace(' +', regexreplace('CONVICTION', StringLib.StringFilterOut(l.disposed, '0123456789/\\.,'), ''), ' '), left, right)), 'DISMISSED WITH COSTS'), 'DISMISSED');
...
dedFile:= dedup(sort(distribute(refFile,hash(offender_key)),
									...
									PERSIST('~thor_dell400::persist::Crim_OK_OKLAHOMA_Offenses');

export Map_OK_OKLAHOMA_Offenses := dedFile;
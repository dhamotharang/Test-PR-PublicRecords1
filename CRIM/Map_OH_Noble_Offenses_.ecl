...
self.court_off_code 			:= '';
self.court_off_desc_1 			:= trim(input.Descr,left,right);
self.court_off_desc_2			:= '';
self.court_off_type_cd 			:= '';
self.court_off_type_desc 		:= '';
self.court_off_lev 				:= if(trim(input.degree,left,right)[1] in ['M','F'] and regexfind('[0-9]',input.degree[2],0)<>'' and length(trim(input.degree,left,right))<3,
									trim(input.degree,left,right),
									'');
self.court_statute 				:= trim(input.section_num,left,right);
self.court_additional_statutes 	:= '';
self.court_statute_desc 		:= '';
self.court_disp_date 			:= '';
self.court_disp_code 			:= '';
self.court_disp_desc_1	 		:= if(regexfind('MATTER IS DISMISSED|DISMISS',input.sent_info,0)<>'' and regexfind('COURT|COUNT',input.sent_info,0)='',
									'DISMISSED',
									'');
self.court_disp_desc_2 			:= '';
self.sent_date 					:= if(fSlashedMDYtoCYMD(trim(input.sent_dt,left,right))[1..2] between '19' and '20',
									fSlashedMDYtoCYMD(trim(input.sent_dt,left,right)),
									'');
self.sent_jail 					:= '';
self.sent_susp_time 			:= '';
...
fOHOffend:= dedup(sort(distribute(pOHCrim,hash(offender_key)),
									...
									PERSIST('~thor_dell400::persist::Crim_OH_Noble_Offenses_Clean');

export map_OH_Noble_Offenses_	:= fOHOffend;

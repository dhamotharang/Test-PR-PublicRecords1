...
input := crim.File_OK_ROGERS(regexfind('[0-9]', casenumber));
...
:= regexreplace(' +', stringlib.stringfindreplace(regexreplace('(\\(FELONY\\))|(\\(MISDEMEANOR\\))|(FELONY)|(MISDEMEANOR)|(\\(MISD\\))|(MISD)|\\(F\\)|\\(M\\)' , if(length(CountAsFiled[1..DataLib.StringFind(CountAsFiled, ',',1)-1]) < 7, '',  CountAsFiled[1..DataLib.StringFind(CountAsFiled, ',',1)-1] + ' ') + trim(regexreplace('(\\(FELONY\\))(\\*MISDEMEANOR\\*)|(\\*FELONY\\*)|\\(MISDEMEANOR\\)|(\\(MISD\\))|\\(FELONY\\)|\\(F\\)|\\(M\\)',stringlib.stringtouppercase(if(regexfind('in violation of', CountAsFiled), CountAsFiled[DataLib.StringFind(CountAsFiled, ',',1)+1..DataLib.StringFind(CountAsFiled, 'in violation of',1)-3], CountAsFiled)),  ''), left, right), ''), '*', ' '), ' ');

string parse_offense(string o) :=
trim(if(length(off_desc(o)) > 6, off_desc(o), ''), left);

self.process_date 				:= Crim_Common.Version_In_Arrest_Offenses;
self.offender_key		:= '2D'+trim(l.casenumber,left,right)+l.id;
self.vendor 					:= '2D';
self.state_origin 				:= 'OK';
self.source_file 				:= 'OK_ROGERS_CRIM_COURT';
self.off_comp 					:= '';
...
self.court_off_desc_1 			:= if(parse_offense(l.CountAsFiled)[1..1] = '(', parse_offense(l.CountAsFiled)[stringlib.stringfind(parse_offense(l.CountAsFiled), ')', 1)+1..75], parse_offense(l.CountAsFiled));
self.court_off_desc_2			:= '';
self.court_off_type_cd 			:= '';
self.court_off_type_desc 		:= '';
self.court_off_lev 				:= if(regexfind('(MISD)|(\\(M\\))', l.CountAsFiled), 'M', if(regexfind('(FELO)|(\\(F\\))', l.CountAsFiled), 'F', ''));
...
dedFile:= dedup(sort(distribute(refFile,hash(offender_key)),
									...
									PERSIST('~thor_dell400::persist::Crim_OK_ROGERS_Offenses');

export Map_OK_ROGERS_Offenses := dedFile;
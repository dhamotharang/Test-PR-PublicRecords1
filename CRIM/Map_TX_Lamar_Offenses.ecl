...
ds_def := sort(CRIM.File_TX_Lamar.Defendant, def_id);
ds_chrg := sort(CRIM.File_TX_Lamar.Charge, def_id);
ds_sent := CRIM.File_TX_Lamar.Sentence;
...
integer hypen  		:=  StringLib.StringFind(input.license_num,'-',1);																											
varstring lic_state_cd	:=  IF (hypen >0 ,input.license_num[1..hypen-1], '');
varstring license_num	:=  IF (hypen >0 , input.license_num[hypen+1..], '');
	...
	self.le_agency_cd 			:= '';
	self.le_agency_desc 		:= '';
	self.le_agency_case_number 	:= input.case_num;
	self.traffic_ticket_number 	:= input.ticket_num;
	self.traffic_dl_no 			:= if(license_num ='NONE' or license_num ='NA' or license_num ='ID', '', license_num);
	self.traffic_dl_st 			:= if(regexfind('[A-Za-z]', lic_state_cd), lic_state_cd,'');
	self.arr_off_code 			:= '';
	self.arr_off_desc_1 		:= '';
	self.arr_off_desc_2 		:= '';
	...
	self.court_off_lev 			:= MAP(trim(input.chg_level, left, right) = '1st Degree Felony' => 'F1',
								  trim(input.chg_level, left, right) = '2nd Degree Felony' => 'F2',
								  trim(input.chg_level, left, right) = '3rd Degree Felony' => 'F3',
								  trim(input.chg_level, left, right) = 'Capital Felony' => 'CF',
								  trim(input.chg_level, left, right) = 'State Felony' => 'SF',
								  trim(input.chg_level, left, right) = 'Felony Unassigned' => 'F',
								  trim(input.chg_level, left, right) = 'F+2' => 'F2',
								  trim(input.chg_level, left, right) = 'F' => 'F',
								  trim(input.chg_level, left, right) = 'Misdemeanor A' => 'M1',
								  trim(input.chg_level, left, right) = 'Misdemeanor B' => 'M2',
								  trim(input.chg_level, left, right) = 'Misdemeanor C' => 'M3',
								  trim(input.chg_level) = 'Misdemeanor Class Appeal' => 'M',
								  trim(input.chg_level, left, right) = 'Misdemeanor Traffic' => 'T',
								  trim(input.chg_level, left, right) = 'Misdemeanor Traffic Appeal' => 'T',
								  trim(input.chg_level, left, right) = 'Misdemeanor Unassigned' => 'M',
								  trim(input.chg_level, left, right) = 'M' => 'M',
								  trim(input.chg_degree+' '+input.case_type, left, right) = '1st Degree F' => 'F1',
								  trim(input.chg_degree+' '+input.case_type, left, right) = '2nd Degree F' => 'F2',
								  trim(input.chg_degree+' '+input.case_type, left, right) = '3rd Degree F' => 'F3',
								  trim(input.chg_degree+' '+input.case_type, left, right) = '1st Degree M' => 'M1',
								  trim(input.chg_degree+' '+input.case_type, left, right) = '2nd Degree M' => 'M2',
								  trim(input.chg_degree+' '+input.case_type, left, right) = '3rd Degree M' => 'M3',
								  trim(input.chg_degree+' '+input.case_type, left, right) = 'F' => 'F',
								  trim(input.chg_degree+' '+input.case_type, left, right) = 'M' => 'M','');	
	...
	self.court_disp_desc_1	 	:= regexreplace('[\\\'|"|\\.|\\*]', trim(input.dispo_detail, left, right), '');
	...
	self.sent_jail 			:= if(input.sentence_type = 'Confinement', 
								...
								trim(regexreplace('TDC|SHOCK|TEXAS DEPARTMENT OF CRIMINAL JUSTICE|BOOT CAMP|PENALTY|COUNTY JAIL|STATE JAIL|TEXAS DEPT CORRECTIONS|-',StringLib.StringToUpperCase(trim(input.sentence_amount,left,right)),
									''), left, right)),''); 	
	self.sent_susp_time 		:= '';
	...
	self.sent_probation 		:= if(input.sentence_type = 'Probation', 
								if(trim(input.sentence_amount, left, right)=''
								or trim(input.sentence_amount, left, right)='0', '',
								regexreplace('COUNTY JAIL|STATE JAIL FACILITY|TEXAS DEPT CORRECTIONS|TEXAS DEPT CRIMINAL JUSTICE|-|\\.', StringLib.StringToUpperCase(trim(input.sentence_amount,left,right)),'')),'');
...
fOHOffend:= dedup(sort(distribute(rlppOHCrimOut,hash(offender_key)),
				offender_key,off_comp,court_statute,court_statute_desc,court_off_code,court_off_desc_1, sent_addl_prov_desc_1,sent_addl_prov_desc_2, local),
					...
					sent_consec, sent_addl_prov_desc_1, sent_addl_prov_desc_2,all,local,left)
									 :PERSIST('~thor_data400::persist::Crim_TX_Lamar_Offenses');

export Map_TX_Lamar_Offenses := fOHOffend;

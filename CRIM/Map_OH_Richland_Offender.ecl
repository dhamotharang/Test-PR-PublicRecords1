...
input := crim.File_OH_Richland(partiestype = 'DEFENDANT' and Charge_Decision <> 'Charge_Decision');
...
self.offender_key 		:= '2K'+l.casenumber;
self.vendor				:= '2K';
self.state_origin		:= 'OH';
self.data_type			:= '2';
self.source_file		:= '(CV)OH RICHLAND CRIM';
self.case_number		:= trim(l.casenumber,left,right);
self.case_court			:= '';
self.case_name			:= regexreplace( '(\\([A-Z]+\\))',trim(if(l.caption <>'' and l.casenumber <> '', regexreplace(l.casenumber, l.caption, ''), ''), left), '');
self.case_type			:= '';
self.case_type_desc		:= '';
...
self.pty_nm_fmt			:= 'L';
self.orig_lname			:= trim(if(l.officer = '2', '', regexreplace('^(AKA )|( AKA )|( AKA)$|^(AKA)$', trim(l.lastname, all), ' ')), left);
self.orig_fname			:= trim(if(l.officer = '2', '', regexreplace('^(AKA )|( AKA )|( AKA)$|^(AKA)$', trim(l.firstname, all), ' ')), left);     
self.orig_mname			:= trim(if(l.officer = '2', '', regexreplace('^(AKA)|( AKA )|( AKA)$|^(AKA)$', trim(stringlib.stringfindreplace(l.middlename, '"', ' '), all), ' ')), left);
self.orig_name_suffix	:= trim(if(l.officer = '2', '', regexreplace('^(AKA )|( AKA )|( AKA)$|^(AKA)$', trim(l.suffix, all), ' ')), left);
...
self.dob_alias			:= '';
self.place_of_birth		:= '';
self.street_address_1	:= if(regexfind(' ', trim(trim(regexreplace('(\\(.*\\))|(UNKNOWN +ADDRESS)', l.address, ''), left), left, right)), trim(regexreplace('(\\(.*\\))|(UNKNOWN +ADDRESS)', l.address, ''), left), '');
...
fOHOffend := dedup(sort(distribute(cleanOHOffend,hash(offender_key)),
										...
										PERSIST('~thor_dell400::persist::Crim_OH_Richland_Offender_Clean');

export Map_OH_Richland_Offender := fOHOffend;
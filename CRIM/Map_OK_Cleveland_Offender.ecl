...
self.offender_key		:= '1U'+trim(l.casenumber,left,right)+l.id;
self.vendor				:= '1U';
self.state_origin		:= 'OK';
self.data_type			:= '2';
self.source_file		:= 'OK_CLEVELAND_CRIM_COURT';
self.case_number		:= trim(l.casenumber,left,right);
self.case_court			:= '';
self.case_name			:= l.caption;
self.case_type			:= '';
self.case_type_desc		:= '';
self.case_filing_dt		:= if(~regexfind('[^0-9]', fSlashedMDYtoCYMD(l.casefiled)) and fSlashedMDYtoCYMD(l.casefiled) between '19000101' and Crim_Common.Version_Development,
							fSlashedMDYtoCYMD(l.casefiled),
							'');
self.pty_nm				:= if(l.pty_typ = '0' and stringlib.stringtouppercase(l.partytype) = 'DEFENDANT' and ~regexfind('[0-9]', l.firstname), l.lastname + ', ' + l.firstname + ' ' + l.middlename + ' ' + l.suffix, 
								if(l.pty_typ = '2' and stringlib.stringtouppercase(l.partytype) = 'DEFENDANT' and ~regexfind('[0-9]', l.pty_nm), l.pty_nm, ''));
self.pty_nm_fmt			:= 'L';
self.orig_lname			:= if(l.pty_typ = '2' and ~regexfind('[0-9]', l.lastname), '', l.lastname);
self.orig_fname			:= if(l.pty_typ = '2' and ~regexfind('[0-9]', l.firstname), '', l.firstname);     
self.orig_mname			:= if(l.pty_typ = '2' and ~regexfind('[0-9]', l.middlename), '', l.middlename);
self.orig_name_suffix	:= if(l.pty_typ = '2' and ~regexfind('[0-9]', l.suffix), '', l.suffix);
...
self.dl_state			:= '';
self.citizenship		:= '';
self.dob				:= if(~regexfind('[^0-9]', fSlashedMDYtoCYMD(l.birthdate)) and fSlashedMDYtoCYMD(l.birthdate) between '19000101' and (string)((integer)Crim_Common.Version_Development - 16),
							fSlashedMDYtoCYMD(l.birthdate),
							'');
self.dob_alias			:= '';
self.place_of_birth		:= '';
/*
self.street_address_1	:= trim(regexreplace(',', addrp1(l.address), ''), left, right);
self.street_address_2	:= trim(stringlib.stringfilterout(if(regexfind('(ALABAMA)|(ALASKA)|(AMERICAN SAMOA)|(ARIZONA)|(ARKANSAS)|(CALIFORNIA)|(COLORADO)|(CONNECTICUT)|(DELAWARE)|(DISTRICT OF COLUMBIA)|(FEDERATED STATES OF MICRONESIA)|(FLORIDA)|(GEORGIA)|(GUAM)|(HAWAII)|(IDAHO)|(ILLINOIS)|(INDIANA)|(IOWA)|(KANSAS)|(KENTUCKY)|(LOUISIANA)|(MAINE)|(MARSHALL ISLANDS)|(MARYLAND)|(MASSACHUSETTS)|(MICHIGAN)|(MINNESOTA)|(MISSISSIPPI)|(MISSOURI)|(MONTANA)|(NEBRASKA)|(NEVADA)|(NEW HAMPSHIRE)|(NEW JERSEY)|(NEW MEXICO)|(NEW YORK)|(NORTH CAROLINA)|(NORTH DAKOTA)|(NORTHERN MARIANA ISLANDS)|(OHIO)|(OKLAHOMA)|(OREGON)|(PALAU)|(PENNSYLVANIA)|(PUERTO RICO)|(RHODE ISLAND)|(SOUTH CAROLINA)|(SOUTH DAKOTA)|(TENNESSEE)|(TEXAS)|(UTAH)|(VERMONT)|(VIRGIN ISLANDS)|(VIRGINIA)|(WASHINGTON)|(WEST VIRGINIA)|(WISCONSIN)|(WYOMING)', state(l.address)), state(l.address), ''), ','), left, right);
...
self.race				:= '';
self.race_desc			:= if(~regexfind('(known)|(Toe)|(Leg)|(Cheek)', l.race) and l.race <> 'unknown' and l.race <> 'Unknown', l.race, '');
self.sex				:= stringlib.stringfilter(l.sex[1..1], 'FM');
self.hair_color			:= '';
self.hair_color_desc	:= l.hair;
self.eye_color			:= '';
self.eye_color_desc		:= if(~regexfind('(arbit)|(known)|(Toe)|(Leg)|(Cheek)', l.eyes), l.eyes, '');
self.skin_color			:= '';
self.skin_color_desc	:= '';
self.height				:= if(heightToInches(l.height) between '036' and '096', heightToInches(l.height), '');
...
dedOffender := dedup(sort(distribute(cleanOffender,hash(offender_key)),
										...
										:PERSIST('~thor_dell400::persist::Crim_OK_CLEVELAND_Offender_Clean')
										;

export Map_OK_Cleveland_Offender := dedOffender;
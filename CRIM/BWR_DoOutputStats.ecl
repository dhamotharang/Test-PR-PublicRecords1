...
#workunit('name', 'Crim Output File Stats for Release')

import crim_common, arrestlogs, _Control;

offender := CRIM.Map_oh_adam_Offender;
offenses := CRIM.Map_oh_adam_Offenses;
...
srtoutfile := 	sort((line1 + dataset([{' ', 1.9}], oneline) + line2 + dataset([{' ', 2.9}], oneline) + line3 + dataset([{' ', 3.9}], oneline) + 
			...
			dataset([{' ', 19.6}], oneline) + dataset([{'COURTT DISPOSITION DESCRIPTIONS:', 19.8}], oneline) + LINE20 + 
...
noatt :=	FileServices.SendEmail( 'jaya.pondugula@lexisnexis.com, jtao@seisint.com, cpettola@seisint.com, tgibson@seisint.com',
	'New Criminal Court Alert Email '+Thorlib.WUID(), 
	XtabOut[1].line);
///////////////////// SEND EMAIL WITH ATTACTMENT
wiatt :=	FileServices.SendEmailAttachText( 'jaya.pondugula@lexisnexis.com, jtao@seisint.com, cpettola@seisint.com, tgibson@seisint.com',
	'New Criminal Court Alert Email '+Thorlib.WUID(), 
	XtabOut[1].line,
	...
	'attachment.txt') ;
///////////////////// DESPRAY RESULTS
despray:=fileservices.Despray('~thor_dell400::out::stats::crim_new_ds_'+Thorlib.WUID(), _Control.IPAddress.edata12, '/stub_cleaning/court/work/crim_new_ds_'+Thorlib.WUID(),,,,true);
...
sequential(
	output(outfile,,'~thor_dell400::out::stats::crim_new_ds_'+Thorlib.WUID(), overwrite, expire (7)),
	noatt
);

...
self.offender_key		:= '2B'+trim(l.casenumber,left,right)+l.id;
self.vendor				:= '2B';
self.state_origin		:= 'OK';
self.data_type			:= '2';
self.source_file		:= 'OK_PAYNE_CRIM_COURT';
...
dedOffender := dedup(sort(distribute(cleanOffender,hash(offender_key)),
										...
										:PERSIST('~thor_dell400::persist::Crim_OK_PAYNE_Offender_Clean')
										;

export Map_OK_PAYNE_Offender := dedOffender;
...
//Flag court type
c := crim.File_CA_SantaClara_Superior;
...
d := crim.File_CA_SantaClara_Municipal;
...
Crim_Common.Layout_In_Court_Offenses tCACrim(preConcat input) := Transform
...
pCACrim := project(preConcat,tCACrim(left));

fCAOffend:= dedup(sort(distribute(pCACrim,hash(offender_key)),
									...
									PERSIST('~thor_dell400::persist::Crim_CA_SantaClara_Offenses_Clean');

export map_CA_SantaClara_Offenses	:= fCAOffend;

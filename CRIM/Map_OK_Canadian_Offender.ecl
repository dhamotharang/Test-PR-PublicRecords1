...
self.case_filing_dt		:= if(~regexfind('[^0-9]', fSlashedMDYtoCYMD(l.casefiled)) and fSlashedMDYtoCYMD(l.casefiled) between '19000101' and Crim_Common.Version_Development,
							...
							'');
self.pty_nm				:= if(l.pty_typ = '0' and stringlib.stringtouppercase(l.partytype) = 'DEFENDANT', l.pty_nm, 
...
self.street_address_2	:= trim(stringlib.stringfindreplace(if(addrp1(l.address) <> '' and state(l.address) <> '', StringLib.StringFindReplace(state(l.address), addrp1(l.address), ''), state(l.address)), ',', ''), left, right);
...
self.hair_color			:= '';
self.hair_color_desc	:= if(~regexfind('(Toe)|(Leg)|(Hip)', l.hair), l.hair, '');
self.eye_color			:= '';
self.eye_color_desc		:= if(~regexfind('(known)|(Toe)|(Leg)|(Hip)', l.eyes), l.eyes, '');
self.skin_color			:= '';
...
refOffender := project(input, rOK(left))(length(pty_nm) > 4 and ~regexfind('ALIAS', pty_nm) and regexfind('[0-9]', case_number) and ~regexfind('NONEFO', trim(stringlib.stringtouppercase(pty_nm), all)));

Crim.Crim_clean(refOffender,cleanOffender);

dedOffender := dedup(sort(distribute(cleanOffender(pty_nm<>''),hash(offender_key)),
										offender_key,case_number,case_court,pty_nm,pty_nm_fmt,pty_typ,dob,-street_address_1,local),
										offender_key,pty_nm,dob,local) 
										:PERSIST('~thor_dell400::persist::Crim_OK_CANADIAN_Offender_Clean')
										;

export Map_OK_Canadian_Offender := dedOffender;
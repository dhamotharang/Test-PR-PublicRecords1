...
/*
state(string address1) := regexreplace('UNKNOWN', stringlib.stringtouppercase(trim(address1[length(trim(address1, left, right)) - stringlib.stringfind( StringLib.StringReverse(trim(address1, left, right)), ',', 1)+1..75])), '');
addrp1(string address1) := regexreplace('UNKNOWN', stringlib.stringtouppercase(address1[1..25][1.. 25-stringlib.stringfind(StringLib.StringReverse(address1[1..25]), ' ', 1)]), '');
...
self.offender_key		:= '2A'+trim(l.casenumber,left,right)+l.id;
self.vendor				:= '2A';
self.state_origin		:= 'OK';
self.data_type			:= '2';
self.source_file		:= 'OK_OKLAHOMA_CRIM_COURT';
...
self.case_filing_dt		:= if(fSlashedMDYtoCYMD(l.casefiled) between '19000101' and Crim_Common.Version_Development,
							...
							'');
self.pty_nm				:= trim(regexreplace('^,' , trim(regexreplace(searchpattern2, regexreplace(searchpattern1, if(l.pty_typ = '0' and stringlib.stringtouppercase(l.partytype) = 'DEFENDANT', l.lastname + ', ' + l.firstname + ' ' + l.middlename + ' ' + l.suffix, 
...
self.place_of_birth		:= '';
/*
self.street_address_1	:= if(~regexfind('[A-W]', addrp1(l.address)) , '',trim(regexreplace(',', addrp1(l.address), ''), left, right));
...
self.height				:= if(heightToInches(l.height) > '000', heightToInches(l.height), '');
...
dedOffender := dedup(sort(distribute(cleanOffender,hash(offender_key)),
										...
										:PERSIST('~thor_dell400::persist::Crim_OK_OKLAHOMA_Offender_Clean')
										;

export Map_OK_OKLAHOMA_Offender := dedOffender;
...
self.case_type			:= '';
self.case_type_desc		:= '';
self.case_filing_dt		:= if(fSlashedMDYtoCYMD(regexreplace('-',input.file_dt,'/'))[1..2] between '19' and '20',
							fSlashedMDYtoCYMD(regexreplace('-',input.file_dt,'/')),
							'');
...
self.dl_state			:= '';
self.citizenship		:= '';
self.dob				:= if(fSlashedMDYtoCYMD(regexreplace('-',input.dob,'/'))[1..4] between '1900' and '1989',
...
fOHOffend := dedup(sort(distribute(cleanOHOffend,hash(offender_key)),
										...
										PERSIST('~thor_dell400::persist::Crim_OH_Sandusky_Offender_Clean');

export Map_OH_Sandusky_Offender := fOHOffend;
...
self.case_filing_dt		:= if(fSlashedMDYtoCYMD(l.casefiled) between '19000101' and Crim_Common.Version_Development,
							...
							'');
self.pty_nm				:= if(l.pty_typ = '0' and stringlib.stringtouppercase(l.partytype) = 'DEFENDANT', l.lastname + ', ' + l.firstname + ' ' + l.middlename + ' ' + l.suffix, 
								if(l.pty_typ = '2' and stringlib.stringtouppercase(l.partytype) = 'DEFENDANT', l.pty_nm, ''));
self.pty_nm_fmt			:= 'L';
...
self.street_address_2	:= trim(regexreplace(',', addrp2(l.address), ''), left, right);
self.street_address_3	:= if(trim(regexreplace(',', state(l.address), ''), left, right) = trim(regexreplace(',', addrp1(l.address), ''), left, right), '' , trim(regexreplace(',', state(l.address), ''), left, right));
*/
...
self.weight				:= if((string)intformat((integer)l.weight, 3, 1) between '050' and '400', l.weight, '');
self.party_status		:= '';
...
dedOffender := dedup(sort(distribute(cleanOffender,hash(offender_key)),
										...
										:PERSIST('~thor_dell400::persist::Crim_OK_ADAIR_Offender_Clean')
										;

export Map_OK_Adair_Offender := dedOffender;
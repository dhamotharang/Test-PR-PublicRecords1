...
Crim_Common.Layout_In_Court_Offender tAK(input l) := Transform
...
self.case_number		:= trim(l.CaseNum,left,right);
self.case_court 		:= '';
self.case_name			:= '';
self.case_type			:= l.casetype;
self.case_type_desc		:= '';
self.case_filing_dt 	:= if(l.DateFiled <> ''
							, getReasonableRange(fSlashedMDYtoCYMD(l.DateFiled))
							, getReasonableRange(fSlashedMDYtoCYMD(l.DateFiled)));
self.pty_nm 			:= regexreplace(' +',l.LastName + ' ' + l.FirstName + ' ' + l.MiddleInitial, ' ');
self.pty_nm_fmt			:= 'L';
...
self.citizenship		:= '';
self.dob				:= if(l.DOB <> ''
							, getReasonableRange(fSlashedMDYtoCYMD(l.DOB))
							, getReasonableRange(fSlashedMDYtoCYMD(l.DOB)));
self.dob_alias			:= '';
self.place_of_birth		:= '';
...
refOffender := project(input, tAK(left));
...
dedOffender := dedup(sort(distribute(cleanOffender,hash(offender_key)),
										...
										// :PERSIST('~thor_dell400::persist::Crim_AK_Offender')
										;

export Map_AK_Offender := dedOffender;

...
ds_def := crim.File_OH_Clinton.Defendant;
ds_alias := crim.File_OH_Clinton.Alias;
...
SELF.offender_key 	:='3G'+hash(clean_def_name)+l.case_num+fSlashedMDYtoCYMD(l.file_dt);
END;
...
Crim_Common.Layout_In_Court_Offender tOHOffend(d input) := Transform
	...
	self.pty_nm_fmt				:= if (regexfind(',', input.pty_nm), 'L', 'F');
	self.orig_lname				:= '';
	self.orig_fname				:= '';    
	self.orig_mname				:= '';
...
fOHOffend := dedup(sort(distribute(cleanOHOffend,hash(offender_key)),
			offender_key,case_number,pty_nm,dob,local),
				offender_key,case_number,pty_nm,dob,local,left)
					 :PERSIST('~thor_data400::persist::Crim_OH_Clinton_Offender');


export Map_OH_Clinton_Offender := fOHOffend;
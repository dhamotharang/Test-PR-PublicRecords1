...
input := crim.File_OK_TULSA(regexfind('[0-9]', casenumber));
...
self.offender_key		:= '2E'+trim(l.casenumber,left,right)+l.id;
self.vendor 					:= '2E';
self.state_origin 				:= 'OK';
self.source_file 				:= 'OK_TULSA_CRIM_COURT';
self.off_comp 					:= '';
...
dedFile:= dedup(sort(distribute(refFile(regexfind('[A-Za-z]', court_off_desc_1 )),hash(offender_key)),
									...
									PERSIST('~thor_dell400::persist::Crim_OK_TULSA_Offenses');

export Map_OK_TULSA_Offenses := dedFile;
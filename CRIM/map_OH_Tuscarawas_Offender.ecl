...
self.orig_fname			:= regexreplace('[0-9]+',trim(input.firstname,left,right),'');     
...
self.dob_alias			:= '';
self.place_of_birth		:= '';
self.street_address_1	:= if(regexfind('C/O|'+'c/o|'+'nka|'+'aka|'+'LKA'+'lka|'+'l.k.a.|'+'%|'+'/O|'+'Inmate',input.address,0)<>'',
							input.address[stringlib.stringfind(input.address,regexfind(' [0-9]+',input.address,0),1)+1..25],
							input.address);
...
fOHOffend := dedup(sort(distribute(cleanOHOffend,hash(offender_key)),
										...
										PERSIST('~thor_dell400::persist::Crim_OH_Tuscarawas_Offender_Clean');

export Map_OH_Tuscarawas_Offender := fOHOffend;
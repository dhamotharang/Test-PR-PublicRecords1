...
input := crim.File_OK_ROGERMILLS(regexfind('[0-9]', casenumber));
...
self.offender_key		:= '2F'+trim(l.casenumber,left,right)+l.id;
self.vendor 					:= '2F';
self.state_origin 				:= 'OK';
self.source_file 				:= 'OK_ROGERMILLS_CRIM_COURT';
self.off_comp 					:= '';
...
self.court_off_desc_1 			:= trim(regexreplace('^[^A-Za-z0-9]|(2ND.*DEGREE)|(3RD.*DEGREE)|(4TH.*DEGREE)|(1ST.*DEGREE)|(MODIFIED)|(DATE.*OF.*OFFENSE:)|(")', regexreplace('^[^A-Za-z0-9]|(2ND.*DEGREE)|(3RD.*DEGREE)|(4TH.*DEGREE)|(1ST.*DEGREE)|(MODIFIED)|(DATE.*OF.*OFFENSE:)|(")', if(length(off_desc(l.CountAsFiled)) > 6, off_desc(l.CountAsFiled), ''), ''), ''), left);
...
self.court_disp_desc_1	 		:= trim(regexreplace('(2ND DEGREE )|(3RD DEGREE )|(4TH DEGREE )|(1ST DEGREE )|(MODIFIED)|(DATE OF OFFENSE:)|(")', cleanDisp(regexreplace('DISMISSED Dismissed- Request of the State', regexreplace('DISMISSED WITH COSTS Dismissed with Cost', if(fSlashedMDYtoCYMD(l.disposed) = '00000000' or ~regexfind('(ACQUITTED)|(DEFERRED)|(DISCHARG)|(DISMISSED)|(Guilty)', l.disposed),'' , trim(regexreplace(' +', regexreplace('CONVICTION', StringLib.StringFilterOut(l.disposed, '0123456789/\\.,'), ''), ' '), left, right)), 'DISMISSED WITH COSTS'), 'DISMISSED')), ''), left);
...
dedFile:= dedup(sort(distribute(refFile(regexfind('[A-Za-z]', court_off_desc_1 )),hash(offender_key)),
									...
									PERSIST('~thor_dell400::persist::Crim_OK_ROGERMILLS_Offenses');

export Map_OK_ROGERMILLS_Offenses := dedFile;
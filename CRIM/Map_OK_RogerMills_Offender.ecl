...
self.case_filing_dt		:= if(fSlashedMDYtoCYMD(l.casefiled) between '19000101' and Crim_Common.Version_Development,
							...
							'');
self.pty_nm				:= trim(regexreplace('[\\*\\(\\)]|( AKA )|(^AKA )|( AKA$)', stringlib.stringtouppercase(trim(regexreplace('^,' , trim(regexreplace(searchpattern2, regexreplace(searchpattern1, if(l.pty_typ = '0' and stringlib.stringtouppercase(l.partytype) = 'DEFENDANT', l.lastname + ', ' + l.firstname + ' ' + l.middlename + ' ' + l.suffix, 
								if(l.pty_typ = '2' and stringlib.stringtouppercase(l.partytype) = 'DEFENDANT', l.pty_nm, '')), ''), ''), left, right), ''), left, right)), ' '), left);
self.pty_nm_fmt			:= 'L';
...
refOffender := project(input, rOK(left))(regexfind('[A-Wa-w]', pty_nm) and ~regexfind('MERGE', pty_nm) and regexfind('[0-9]', case_number) and ~regexfind('NONEFO', trim(stringlib.stringtouppercase(pty_nm), all)));
...
dedOffender := dedup(sort(distribute(cleanOffender,hash(offender_key)),
										...
										:PERSIST('~thor_dell400::persist::Crim_OK_ROGERMILLS_Offender_Clean')
										;

export Map_OK_ROGERMILLS_Offender := dedOffender;
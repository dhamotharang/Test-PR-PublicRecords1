...
self.dl_state			:= '';
self.citizenship		:= '';
self.dob				:= if(fSlashedMDYtoCYMD(l.birthdate) <> '19111111' and fSlashedMDYtoCYMD(l.birthdate)between '19000102' and (string)((integer)Crim_Common.Version_Development - 16),
...
self.race				:= '';
self.race_desc			:= if(l.race <> 'unknown' and l.race <> 'Unknown' and regexfind('(Asian)|(American)|(Black)|(White)', l.race), l.race, '');
self.sex				:= stringlib.stringfilter(l.sex[1..1], 'FM');
self.hair_color			:= '';
self.hair_color_desc	:= if(regexfind('(Breast)|(Back)', l.hair), '', l.hair);
self.eye_color			:= '';
self.eye_color_desc		:= if(l.eyes <> 'unknown' and l.eyes <> 'Unknown' and ~regexfind('(Extra)|(Cleft)', l.eyes), l.eyes, '');
self.skin_color			:= '';
self.skin_color_desc	:= '';
self.height				:= if(heightToInches(l.height) > '036' and heightToInches(l.height) < '084', heightToInches(l.height), '');
...
refOffender := project(input, rOK(left))(regexfind('[A-Wa-w]', pty_nm) and ~regexfind('[0-9]|(not same as )|(MERGE)', pty_nm) and regexfind('[0-9]', case_number) and ~regexfind('NONEFO', trim(stringlib.stringtouppercase(pty_nm), all)));
...
dedOffender := dedup(sort(distribute(cleanOffender,hash(offender_key)),
										...
										:PERSIST('~thor_dell400::persist::Crim_OK_TULSA_Offender_Clean')
										;

export Map_OK_TULSA_Offender := dedOffender;
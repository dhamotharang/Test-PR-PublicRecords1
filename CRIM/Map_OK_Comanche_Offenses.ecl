...
input := crim.File_OK_COMANCHE(regexfind('[0-9]', casenumber));
...
self.court_off_desc_1 			:= if(length(off_desc(l.CountAsFiled)) > 6, off_desc(l.CountAsFiled), '');
...
dedFile:= dedup(sort(distribute(refFile,hash(offender_key)),
									...
									PERSIST('~thor_dell400::persist::Crim_OK_COMANCHE_Offenses');

export Map_OK_COMANCHE_Offenses := dedFile;
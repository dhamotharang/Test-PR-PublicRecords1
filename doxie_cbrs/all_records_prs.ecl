import doxie_crs,doxie,LN_PropertyV2_Services,Gateway;

export all_records_prs(dataset(doxie_cbrs.layout_references) bdids, dataset(Gateway.Layouts.Config) gateway, unsigned1 ofac_version = 1,
                                                             boolean include_ofac = false, real global_watchlist_threshold = 0.84
) := FUNCTION

//***** MY RECORDSETS
cntr := doxie_cbrs.count_records_prs(bdids, gateway, ofac_version, include_ofac, global_watchlist_threshold);
...
patr := doxie_cbrs.Patriot_records_max(gateway, ofac_version, include_ofac, global_watchlist_threshold);

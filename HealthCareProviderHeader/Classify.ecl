...
SHARED TotalClusters := specificities(h).TotalClusters;
  ...
  UPIN_tokens := PROJECT(specificities(h).UPIN_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.UPIN; SELF.TokenType := 2; SELF.Spc := LEFT.field_Specificity ));
  NPI_NUMBER_tokens := PROJECT(specificities(h).NPI_NUMBER_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.NPI_NUMBER; SELF.TokenType := 3; SELF.Spc := LEFT.field_Specificity ));
  DEA_NUMBER_tokens := PROJECT(specificities(h).DEA_NUMBER_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.DEA_NUMBER; SELF.TokenType := 4; SELF.Spc := LEFT.field_Specificity ));
  ...
  VENDOR_ID_tokens := PROJECT(specificities(h).VENDOR_ID_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.VENDOR_ID; SELF.TokenType := 6; SELF.Spc := LEFT.field_Specificity ));
  CNSMR_SSN_tokens := PROJECT(specificities(h).CNSMR_SSN_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.CNSMR_SSN; SELF.TokenType := 7; SELF.Spc := LEFT.field_Specificity ));
  FAX_tokens := PROJECT(specificities(h).FAX_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.FAX; SELF.TokenType := 10; SELF.Spc := LEFT.field_Specificity ));
  C_LIC_NBR_tokens := PROJECT(specificities(h).C_LIC_NBR_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.C_LIC_NBR; SELF.TokenType := 11; SELF.Spc := LEFT.field_Specificity ));
  PHONE_tokens := PROJECT(specificities(h).PHONE_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.PHONE; SELF.TokenType := 14; SELF.Spc := LEFT.field_Specificity ));
  BILLING_TAX_ID_tokens := PROJECT(specificities(h).BILLING_TAX_ID_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.BILLING_TAX_ID; SELF.TokenType := 17; SELF.Spc := LEFT.field_Specificity ));
  BILLING_NPI_NUMBER_tokens := PROJECT(specificities(h).BILLING_NPI_NUMBER_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.BILLING_NPI_NUMBER; SELF.TokenType := 18; SELF.Spc := LEFT.field_Specificity ));
  ...
  LNAME_tokens := PROJECT(specificities(h).LNAME_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.LNAME; SELF.TokenType := 22; SELF.Spc := LEFT.field_Specificity ));
  PRIM_RANGE_tokens := PROJECT(specificities(h).PRIM_RANGE_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.PRIM_RANGE; SELF.TokenType := 23; SELF.Spc := LEFT.field_Specificity ));
  V_CITY_NAME_tokens := PROJECT(specificities(h).V_CITY_NAME_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.V_CITY_NAME; SELF.TokenType := 24; SELF.Spc := LEFT.field_Specificity ));
  LAT_LONG_tokens := PROJECT(specificities(h).LAT_LONG_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.LAT_LONG; SELF.TokenType := 25; SELF.Spc := LEFT.field_Specificity ));
  SNAME_tokens := PROJECT(specificities(h).SNAME_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.SNAME; SELF.TokenType := 26; SELF.Spc := LEFT.field_Specificity ));
  FNAME_tokens := PROJECT(specificities(h).FNAME_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.FNAME; SELF.TokenType := 27; SELF.Spc := LEFT.field_Specificity ));
  MNAME_tokens := PROJECT(specificities(h).MNAME_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.MNAME; SELF.TokenType := 28; SELF.Spc := LEFT.field_Specificity ));
  TAXONOMY_tokens := PROJECT(specificities(h).TAXONOMY_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.TAXONOMY; SELF.TokenType := 29; SELF.Spc := LEFT.field_Specificity ));
  ...
  TAX_ID_tokens := PROJECT(specificities(h).TAX_ID_values_persisted,TRANSFORM(SALT29.Layout_Classify_Token,SELF.TokenValue := (SALT29.StrType)LEFT.TAX_ID; SELF.TokenType := 39; SELF.Spc := LEFT.field_Specificity ));
SHARED all_tokens0 := SSN_tokens + UPIN_tokens + NPI_NUMBER_tokens + DEA_NUMBER_tokens + DID_tokens + VENDOR_ID_tokens + CNSMR_SSN_tokens + FAX_tokens + C_LIC_NBR_tokens + PHONE_tokens + BILLING_TAX_ID_tokens + BILLING_NPI_NUMBER_tokens + PRIM_NAME_tokens + ZIP_tokens + LNAME_tokens + PRIM_RANGE_tokens + V_CITY_NAME_tokens + LAT_LONG_tokens + SNAME_tokens + FNAME_tokens + MNAME_tokens + TAXONOMY_tokens + SEC_RANGE_tokens + LIC_TYPE_tokens + ST_tokens + GENDER_tokens + DERIVED_GENDER_tokens + TAX_ID_tokens;
  all_tokens := SALT29.fn_process_multitokens(all_tokens0);
EXPORT TokenKeyName := '~'+'key::HealthCareProviderHeader::LNPID::Token::TokenKey';
EXPORT TokenKey := INDEX(all_tokens,{UNSIGNED4 TokenHash := HASH32(TokenValue),TokenType},{all_tokens},TokenKeyName);
EXPORT MultiTokenKeyName := '~'+'key::HealthCareProviderHeader::LNPID::Token::MultiTokenKey';
EXPORT MultiTokenKey := INDEX(all_tokens0(SALT29.WordCount(TokenValue)>1),{UNSIGNED4 TokenHash := HASH32(TokenValue),TokenType},{all_tokens0},MultiTokenKeyName);
  MAINNAME_tokens := PROJECT(specificities(h).MAINNAME_values_persisted,TRANSFORM(SALT29.Layout_Classify_Concept,SELF.ConceptHash := LEFT.MAINNAME; SELF.TokenType := 8; SELF.Spc := LEFT.field_Specificity ));
  FULLNAME_tokens := PROJECT(specificities(h).FULLNAME_values_persisted,TRANSFORM(SALT29.Layout_Classify_Concept,SELF.ConceptHash := LEFT.FULLNAME; SELF.TokenType := 9; SELF.Spc := LEFT.field_Specificity ));
  ADDRESS_tokens := PROJECT(specificities(h).ADDRESS_values_persisted,TRANSFORM(SALT29.Layout_Classify_Concept,SELF.ConceptHash := LEFT.ADDRESS; SELF.TokenType := 12; SELF.Spc := LEFT.field_Specificity ));
  ADDR1_tokens := PROJECT(specificities(h).ADDR1_values_persisted,TRANSFORM(SALT29.Layout_Classify_Concept,SELF.ConceptHash := LEFT.ADDR1; SELF.TokenType := 13; SELF.Spc := LEFT.field_Specificity ));
...
EXPORT ConceptKeyName := '~'+'key::HealthCareProviderHeader::LNPID::Token::ConceptKey';
...
shared MAINNAME_combinations := o_tot;
  Layout_ConceptTemplate Into(MAINNAME_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.FNAME_filled => 27, le.MNAME_filled => 28, le.LNAME_filled => 22,0);
    SELF.FieldNumber2 := MAP ( le.MNAME_filled AND SELF.FieldNumber1 != 28 => 28, le.LNAME_filled AND SELF.FieldNumber1 != 22 => 22,0);
    SELF.FieldNumber3 := MAP ( le.LNAME_filled AND SELF.FieldNumber1 != 22 AND SELF.FieldNumber2 != 22 => 22,0);
    SELF := le;
  END;
...
shared FULLNAME_combinations := o_tot;
  Layout_ConceptTemplate Into(FULLNAME_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.MAINNAME_filled => 8, le.SNAME_filled => 26,0);
    SELF.FieldNumber2 := MAP ( le.SNAME_filled AND SELF.FieldNumber1 != 26 => 26,0);
    SELF := le;
  END;
...
shared ADDRESS_combinations := o_tot;
  Layout_ConceptTemplate Into(ADDRESS_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.ADDR1_filled => 13, le.LOCALE_filled => 21,0);
...
shared ADDR1_combinations := o_tot;
  Layout_ConceptTemplate Into(ADDR1_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.PRIM_RANGE_filled => 23, le.SEC_RANGE_filled => 30, le.PRIM_NAME_filled => 19,0);
...
shared LOCALE_combinations := o_tot;
  Layout_ConceptTemplate Into(LOCALE_combinations le) := TRANSFORM
    SELF.FieldNumber1 := MAP ( le.V_CITY_NAME_filled => 24, le.ST_filled => 32, le.ZIP_filled => 20,0);
...
EXPORT ConceptTemplatesKey := '~'+'key::HealthCareProviderHeader::LNPID::Token::ConceptKey';

...
EXPORT SALT29.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'SSN','UPIN','NPI_NUMBER','DEA_NUMBER','DID','VENDOR_ID','CNSMR_SSN','MAINNAME','FULLNAME','FAX','C_LIC_NBR','ADDRESS','ADDR1','PHONE','DOB','CNSMR_DOB','BILLING_TAX_ID','BILLING_NPI_NUMBER','PRIM_NAME','ZIP','LOCALE','LNAME','PRIM_RANGE','V_CITY_NAME','LAT_LONG','SNAME','FNAME','MNAME','TAXONOMY','SEC_RANGE','LIC_TYPE','ST','GENDER','DERIVED_GENDER','SRC','LIC_STATE','ADDRESS_ID','CNAME','TAX_ID','GEO_LAT','GEO_LONG','DT_FIRST_SEEN','DT_LAST_SEEN','DT_LIC_EXPIRATION','DT_DEA_EXPIRATION');
EXPORT FieldNum(SALT29.StrType fn) := CASE(fn,'SSN' => 1,'UPIN' => 2,'NPI_NUMBER' => 3,'DEA_NUMBER' => 4,'DID' => 5,'VENDOR_ID' => 6,'CNSMR_SSN' => 7,'MAINNAME' => 8,'FULLNAME' => 9,'FAX' => 10,'C_LIC_NBR' => 11,'ADDRESS' => 12,'ADDR1' => 13,'PHONE' => 14,'DOB' => 15,'CNSMR_DOB' => 16,'BILLING_TAX_ID' => 17,'BILLING_NPI_NUMBER' => 18,'PRIM_NAME' => 19,'ZIP' => 20,'LOCALE' => 21,'LNAME' => 22,'PRIM_RANGE' => 23,'V_CITY_NAME' => 24,'LAT_LONG' => 25,'SNAME' => 26,'FNAME' => 27,'MNAME' => 28,'TAXONOMY' => 29,'SEC_RANGE' => 30,'LIC_TYPE' => 31,'ST' => 32,'GENDER' => 33,'DERIVED_GENDER' => 34,'SRC' => 35,'LIC_STATE' => 36,'ADDRESS_ID' => 37,'CNAME' => 38,'TAX_ID' => 39,'GEO_LAT' => 40,'GEO_LONG' => 41,'DT_FIRST_SEEN' => 42,'DT_LAST_SEEN' => 43,'DT_LIC_EXPIRATION' => 44,'DT_DEA_EXPIRATION' => 45,0);
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_SSN,1,0)+ IF( SELF.Diff_UPIN,1,0)+ IF( SELF.Diff_NPI_NUMBER,1,0)+ IF( SELF.Diff_DEA_NUMBER,1,0)+ IF( SELF.Diff_DID,1,0)+ IF( SELF.Diff_VENDOR_ID,1,0)+ IF( SELF.Diff_CNSMR_SSN,1,0)+ IF( SELF.Diff_FAX,1,0)+ IF( SELF.Diff_C_LIC_NBR,1,0)+ IF( SELF.Diff_PHONE,1,0)+ IF( SELF.Diff_DOB,1,0)+ IF( SELF.Diff_CNSMR_DOB,1,0)+ IF( SELF.Diff_BILLING_TAX_ID,1,0)+ IF( SELF.Diff_BILLING_NPI_NUMBER,1,0)+ IF( SELF.Diff_PRIM_NAME,1,0)+ IF( SELF.Diff_ZIP,1,0)+ IF( SELF.Diff_LNAME,1,0)+ IF( SELF.Diff_PRIM_RANGE,1,0)+ IF( SELF.Diff_V_CITY_NAME,1,0)+ IF( SELF.Diff_LAT_LONG,1,0)+ IF( SELF.Diff_SNAME,1,0)+ IF( SELF.Diff_FNAME,1,0)+ IF( SELF.Diff_MNAME,1,0)+ IF( SELF.Diff_TAXONOMY,1,0)+ IF( SELF.Diff_SEC_RANGE,1,0)+ IF( SELF.Diff_LIC_TYPE,1,0)+ IF( SELF.Diff_ST,1,0)+ IF( SELF.Diff_GENDER,1,0)+ IF( SELF.Diff_DERIVED_GENDER,1,0)+ IF( SELF.Diff_SRC,1,0)+ IF( SELF.Diff_LIC_STATE,1,0)+ IF( SELF.Diff_ADDRESS_ID,1,0)+ IF( SELF.Diff_CNAME,1,0)+ IF( SELF.Diff_TAX_ID,1,0)+ IF( SELF.Diff_GEO_LAT,1,0)+ IF( SELF.Diff_GEO_LONG,1,0)+ IF( SELF.Diff_DT_LIC_EXPIRATION,1,0)+ IF( SELF.Diff_DT_DEA_EXPIRATION,1,0);
  END;

/*2013-08-16T16:32:12Z (Senthilkumar Periasamy)
...
// First step is to get all of the data slimmed and row-reduced
EXPORT MostRecent_tab_ := DISTRIBUTE(TABLE(h(LNPID <> 0),{LNPID,data_permits,UPIN,NPI_NUMBER,UNSIGNED Early_Date := MIN(GROUP,IF(DT_LAST_SEEN=0,99999999,DT_LAST_SEEN)),UNSIGNED Late_Date := MAX(GROUP,DT_LAST_SEEN),UNSIGNED Row_Cnt := COUNT(GROUP)},LNPID,data_permits,UPIN,NPI_NUMBER,MERGE),HASH(LNPID)); // Slim and reduce row-count
...
EXPORT MostRecentAddr_method_ADDR := DEDUP( SORT( MostRecentAddr_tab_ADDR(Late_Date>0,Early_Date<99999999),LNPID,ST,-Late_Date,-Early_Date,LOCAL),LNPID,ST,LOCAL);
...
Fuzzy_layout NoteSupport(MostRecentLIC_tab_LIC_NBR le,MostRecentLIC_tab_LIC_NBR ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(MostRecentLIC_tab_LIC_NBR,MostRecentLIC_tab_LIC_NBR, LEFT.LNPID = RIGHT.LNPID  AND ( LEFT.LIC_STATE = (TYPEOF(LEFT.LIC_STATE))'' OR RIGHT.LIC_STATE = (TYPEOF(RIGHT.LIC_STATE))'' OR LEFT.LIC_STATE = RIGHT.LIC_STATE  ) AND ( LEFT.LIC_NBR = (TYPEOF(LEFT.LIC_NBR))'' OR RIGHT.LIC_NBR = (TYPEOF(RIGHT.LIC_NBR))'' OR SALT27.WithinEditN(LEFT.LIC_NBR,RIGHT.LIC_NBR,2)  OR fn_cleanlicense(RIGHT.LIC_NBR) = fn_cleanlicense(LEFT.LIC_NBR)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.Early_Date := MIN(le.Early_Date,ri.Early_Date);
    SELF.Late_Date := MAX(le.Late_Date,ri.Late_Date);
    SELF.data_permits := le.data_permits|ri.data_permits;
...
//Now actually find the best value
EXPORT MostRecentLIC_method_LIC_NBR := DEDUP( SORT( MostRecentLIC_fuzz_LIC_NBR(Late_Date>0,Early_Date<99999999),LNPID,LIC_STATE,-Late_Date,-Early_Date,LOCAL),LNPID,LIC_STATE,LOCAL);
...
Fuzzy_layout NoteSupport(BestLICCommenest_tab_LIC_NBR le,BestLICCommenest_tab_LIC_NBR ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestLICCommenest_tab_LIC_NBR,BestLICCommenest_tab_LIC_NBR, LEFT.LNPID = RIGHT.LNPID  AND ( LEFT.LIC_NBR = (TYPEOF(LEFT.LIC_NBR))'' OR RIGHT.LIC_NBR = (TYPEOF(RIGHT.LIC_NBR))'' OR SALT27.WithinEditN(LEFT.LIC_NBR,RIGHT.LIC_NBR,2)  OR fn_cleanlicense(RIGHT.LIC_NBR) = fn_cleanlicense(LEFT.LIC_NBR)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
Fuzzy_layout NoteSupport(BestLICLongest_tab_LIC_NBR le,BestLICLongest_tab_LIC_NBR ri) := TRANSFORM
  ...
  SELF := le;
END;
Supports := JOIN(BestLICLongest_tab_LIC_NBR,BestLICLongest_tab_LIC_NBR, LEFT.LNPID = RIGHT.LNPID  AND ( LEFT.LIC_STATE = (TYPEOF(LEFT.LIC_STATE))'' OR RIGHT.LIC_STATE = (TYPEOF(RIGHT.LIC_STATE))'' OR LEFT.LIC_STATE = RIGHT.LIC_STATE  ) AND ( LEFT.LIC_NBR = (TYPEOF(LEFT.LIC_NBR))'' OR RIGHT.LIC_NBR = (TYPEOF(RIGHT.LIC_NBR))'' OR SALT27.WithinEditN(LEFT.LIC_NBR,RIGHT.LIC_NBR,2)  OR fn_cleanlicense(RIGHT.LIC_NBR) = fn_cleanlicense(LEFT.LIC_NBR)  ),NoteSupport(LEFT,RIGHT),LOCAL);
  Supports TR(Supports le, Supports ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
...
//Now actually find the best value
EXPORT BestLICLongest_method_LIC_NBR := DEDUP( SORT( BestLICLongest_fuzz_LIC_NBR,LNPID,LIC_STATE,-LENGTH(TRIM((SALT27.StrType)LIC_NBR)),-Row_Cnt,LOCAL),LNPID,LIC_STATE,LOCAL);
...
// First step is to get all of the data slimmed and row-reduced
EXPORT UniqueSingleValue_tab_ := DISTRIBUTE(TABLE(h(LNPID <> 0),{LNPID,data_permits,DERIVED_GENDER,UNSIGNED Row_Cnt := COUNT(GROUP)},LNPID,data_permits,DERIVED_GENDER,MERGE),HASH(LNPID)); // Slim and reduce row-count
  Slim := TABLE(UniqueSingleValue_tab_(DERIVED_GENDER NOT IN SET(s.nulls_DERIVED_GENDER,DERIVED_GENDER)),{LNPID,data_permits,DERIVED_GENDER,Row_Cnt});
  Slim TR(Slim le, Slim ri) := TRANSFORM
    SELF.data_permits := le.data_permits|ri.data_permits;
    SELF.Row_Cnt := le.Row_Cnt+ri.Row_Cnt;
    SELF := le;
  END;
EXPORT UniqueSingleValue_tab_DERIVED_GENDER := ROLLUP( SORT(Slim,EXCEPT Row_Cnt,data_permits,LOCAL),TR(LEFT,RIGHT), EXCEPT Row_Cnt,data_permits,LOCAL);
//Now actually find the best value
  grp := GROUP( UniqueSingleValue_tab_DERIVED_GENDER,LNPID,ALL,LOCAL);
  srt := SORT( grp,-Row_Cnt);
  cmn := GROUP( DEDUP( srt, true ) ); // Find the commonest value for a given basis
...
export UniqueSingleValue_method_DERIVED_GENDER := JOIN( cmn,Totals,left.LNPID = right.LNPID AND LEFT.Row_Cnt = RIGHT.Tot,TRANSFORM(LEFT),LOCAL);
//Create those fields with BestType: ExtendedCommonValue
// First step is to get all of the data slimmed and row-reduced
EXPORT ExtendedCommonValue_tab_ := DISTRIBUTE(TABLE(h(LNPID <> 0),{LNPID,data_permits,SNAME,MNAME,UPIN,DEA_NUMBER,NPI_NUMBER,UNSIGNED Row_Cnt := COUNT(GROUP)},LNPID,data_permits,SNAME,MNAME,UPIN,DEA_NUMBER,NPI_NUMBER,MERGE),HASH(LNPID)); // Slim and reduce row-count
...
EXPORT BestBy_LNPID__DOB := BestBy_LNPID__DOB_np  : PERSIST('~temp::LNPID::HealthCareProviderHeader::BestBy_LNPID__DOB::best');
...
EXPORT BestBy_LNPID__ADDRESS_ID := BestBy_LNPID__ADDRESS_ID_np  : PERSIST('~temp::LNPID::HealthCareProviderHeader::BestBy_LNPID__ADDRESS_ID::best');
// Now gather some statistics to see how we did
  R := RECORD
    NumberOfBasis := COUNT(GROUP);
...
// 5 - Gathering type:MostRecent Entries:2
  ...
  END;
  R5 T5(J4 le,MostRecent_method_UPIN ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J5 := JOIN(J4,MostRecent_method_UPIN,LEFT.LNPID = RIGHT.LNPID,T5(LEFT,RIGHT),FULL OUTER,LOCAL);
  R6 := RECORD
    J5; // The data so far
...
// 13 - Gathering type:UniqueSingleValue Entries:1
  R13 := RECORD
    J12; // The data so far
    TYPEOF(UniqueSingleValue_method_DERIVED_GENDER.DERIVED_GENDER) UniqueSingleValue_DERIVED_GENDER;
    UNSIGNED DERIVED_GENDER_UniqueSingleValue_Row_Cnt;
    UNSIGNED1 DERIVED_GENDER_UniqueSingleValue_data_permits;
  END;
  R13 T13(J12 le,UniqueSingleValue_method_DERIVED_GENDER ri) := TRANSFORM
    SELF.UniqueSingleValue_DERIVED_GENDER := ri.DERIVED_GENDER;
    SELF.DERIVED_GENDER_UniqueSingleValue_Row_Cnt := ri.Row_Cnt;
    SELF.DERIVED_GENDER_UniqueSingleValue_data_permits := ri.data_permits;
    BOOLEAN has_left := le.LNPID <> (TYPEOF(le.LNPID))''; // See if LHS is null
    SELF.LNPID := IF( has_left, le.LNPID, ri.LNPID );
    SELF := le;
  END;
  J13 := JOIN(J12,UniqueSingleValue_method_DERIVED_GENDER,LEFT.LNPID = RIGHT.LNPID,T13(LEFT,RIGHT),FULL OUTER,LOCAL);
// 14 - Gathering type:ExtendedCommonValue Entries:5
  ...
  END;
  R16 T16(J15 le,ExtendedCommonValue_method_UPIN ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J16 := JOIN(J15,ExtendedCommonValue_method_UPIN,LEFT.LNPID = RIGHT.LNPID,T16(LEFT,RIGHT),FULL OUTER,LOCAL);
  R17 := RECORD
    J16; // The data so far
  ...
  END;
  R18 T18(J17 le,ExtendedCommonValue_method_NPI_NUMBER ri) := TRANSFORM
    ...
    SELF := le;
  END;
  J18 := JOIN(J17,ExtendedCommonValue_method_NPI_NUMBER,LEFT.LNPID = RIGHT.LNPID,T18(LEFT,RIGHT),FULL OUTER,LOCAL);
...
EXPORT BestBy_LNPID := BestBy_LNPID_np  : PERSIST('~temp::LNPID::HealthCareProviderHeader::BestBy_LNPID::best');
// Now gather some statistics to see how we did
  R := RECORD
    NumberOfBasis := COUNT(GROUP);
    ...
    REAL8 UniqueSingleValue_DERIVED_GENDER_pcnt := AVE(GROUP,IF(BestBy_LNPID.UniqueSingleValue_DERIVED_GENDER=(typeof(BestBy_LNPID.UniqueSingleValue_DERIVED_GENDER))'',0,100));
    UNSIGNED UniqueSingleValue_DERIVED_GENDER_permit1_cnt := COUNT(GROUP,BestBy_LNPID.DERIVED_GENDER_UniqueSingleValue_data_permits&1<>0);
    UNSIGNED UniqueSingleValue_DERIVED_GENDER_permit2_cnt := COUNT(GROUP,BestBy_LNPID.DERIVED_GENDER_UniqueSingleValue_data_permits&2<>0);
    UNSIGNED UniqueSingleValue_DERIVED_GENDER_permit3_cnt := COUNT(GROUP,BestBy_LNPID.DERIVED_GENDER_UniqueSingleValue_data_permits&4<>0);
    UNSIGNED UniqueSingleValue_DERIVED_GENDER_permit4_cnt := COUNT(GROUP,BestBy_LNPID.DERIVED_GENDER_UniqueSingleValue_data_permits&8<>0);
    UNSIGNED UniqueSingleValue_DERIVED_GENDER_permit5_cnt := COUNT(GROUP,BestBy_LNPID.DERIVED_GENDER_UniqueSingleValue_data_permits&16<>0);
...
SHARED F_BestBy_LNPID(DATASET({BestBy_LNPID}) d) := FUNCTION
  ...
  R := RECORD
    ...
    TYPEOF(h.DERIVED_GENDER) DERIVED_GENDER;
    UNSIGNED1 DERIVED_GENDER_data_permits;
  ...
  R T(BestBy_LNPID le) := TRANSFORM
    ...
    SELF.DERIVED_GENDER := le.UniqueSingleValue_DERIVED_GENDER;
    SELF.DERIVED_GENDER_data_permits := le.DERIVED_GENDER_UniqueSingleValue_data_permits;
...
EXPORT BestBy_LNPID_ST := BestBy_LNPID_ST_np  : PERSIST('~temp::LNPID::HealthCareProviderHeader::BestBy_LNPID_ST::best');
// Now gather some statistics to see how we did
  R := RECORD
    NumberOfBasis := COUNT(GROUP);
...
EXPORT BestBy_LNPID__LIC_STATE := BestBy_LNPID__LIC_STATE_np  : PERSIST('~temp::LNPID::HealthCareProviderHeader::BestBy_LNPID__LIC_STATE::best');
// Now gather some statistics to see how we did
  R := RECORD
    NumberOfBasis := COUNT(GROUP);
...
EXPORT BestBy_LNPID_PRIM_RANGE_PRIM_NAME_ZIP := BestBy_LNPID_PRIM_RANGE_PRIM_NAME_ZIP_np  : PERSIST('~temp::LNPID::HealthCareProviderHeader::BestBy_LNPID_PRIM_RANGE_PRIM_NAME_ZIP::best');
// Now gather some statistics to see how we did
  R := RECORD
    NumberOfBasis := COUNT(GROUP);
...
EXPORT BestBy_LNPID_ZIP := BestBy_LNPID_ZIP_np  : PERSIST('~temp::LNPID::HealthCareProviderHeader::BestBy_LNPID_ZIP::best');
// Now gather some statistics to see how we did
  R := RECORD
    NumberOfBasis := COUNT(GROUP);
...
// Append flags to the regular file
  TYPEOF(ih) NoteFlags(ih le,BestBy_LNPID_Best ri) := TRANSFORM
    ...
    SELF.LIC_NBR_flag := MAP ( ri.LIC_NBR  IN SET(s.nulls_LIC_NBR,LIC_NBR) => SALT27.Flags.Null,
      ...
      le.LIC_NBR = (TYPEOF(le.LIC_NBR))'' OR ri.LIC_NBR = (TYPEOF(ri.LIC_NBR))'' OR SALT27.WithinEditN(le.LIC_NBR,ri.LIC_NBR,2)  OR fn_cleanlicense(ri.LIC_NBR) = fn_cleanlicense(le.LIC_NBR)  => SALT27.Flags.Fuzzy,
      SALT27.Flags.Bad);
    ...
    SELF.UPIN_flag := MAP ( ri.UPIN  IN SET(s.nulls_UPIN,UPIN) => SALT27.Flags.Null,
      ...
      le.UPIN = (TYPEOF(le.UPIN))'' OR ri.UPIN = (TYPEOF(ri.UPIN))'' OR SALT27.WithinEditN(le.UPIN,ri.UPIN,1)  => SALT27.Flags.Fuzzy,
      SALT27.Flags.Bad);
    SELF.DEA_NUMBER_flag := MAP ( ri.DEA_NUMBER  IN SET(s.nulls_DEA_NUMBER,DEA_NUMBER) => SALT27.Flags.Null,
      ...
      le.DEA_NUMBER = (TYPEOF(le.DEA_NUMBER))'' OR ri.DEA_NUMBER = (TYPEOF(ri.DEA_NUMBER))'' OR SALT27.WithinEditN(le.DEA_NUMBER,ri.DEA_NUMBER,1)  => SALT27.Flags.Fuzzy,
      SALT27.Flags.Bad);
    SELF.NPI_NUMBER_flag := MAP ( ri.NPI_NUMBER  IN SET(s.nulls_NPI_NUMBER,NPI_NUMBER) => SALT27.Flags.Null,
      ...
      le.NPI_NUMBER = (TYPEOF(le.NPI_NUMBER))'' OR ri.NPI_NUMBER = (TYPEOF(ri.NPI_NUMBER))'' OR SALT27.WithinEditN(le.NPI_NUMBER,ri.NPI_NUMBER,1)  => SALT27.Flags.Fuzzy,
      SALT27.Flags.Bad);

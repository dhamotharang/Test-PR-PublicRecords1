...
Process_LNPID_Layouts.OutputLayout GetResults(Process_LNPID_Layouts.InputLayout le) := TRANSFORM
  ...
  SELF.Results := TOPN(ROLLUP(
    SORTED(Key_HealthProvider_.ScoredLNPIDFetch(le.DID,(UNSIGNED4)le.DOB,le.PHONE,le.SNAME,le.FNAME,le.MNAME,le.LNAME,le.GENDER,le.DERIVED_GENDER,le.LIC_NBR,le.ADDRESS_ID,le.PRIM_NAME,le.PRIM_RANGE,le.SEC_RANGE,le.V_CITY_NAME,le.ST,le.ZIP,le.CNAME,le.TAX_ID,le.UPIN,le.DEA_NUMBER,le.VENDOR_ID,le.NPI_NUMBER,le.DT_LIC_EXPIRATION,le.DT_DEA_EXPIRATION,le.MAINNAME,le.FULLNAME,le.FULLNAME_DOB,le.ADDR,le.LOCALE,le.ADDRESS,le.LIC_STATE,le.SRC),LNPID)
...
END;
  ...
  SALT27.MAC_External_AddPcnt(RR0,Process_LNPID_Layouts.LayoutScoredFetch,Process_LNPID_Layouts.OutputLayout,22,RR1);
...
// Pass-thru any records which already had the LNPID on them
  process_LNPID_layouts.id_stream_layout ptt(in le) := TRANSFORM
    ...
    SELF.Weight := 36; // Assume at least 'threshold' met
...
// Transform to process 'real' results
  ...
  Layout_Matched_Data score_fields(Raw_Data le,In ri) := TRANSFORM
    ...
    SELF.Match_DERIVED_GENDER := MAP ( ri.DERIVED_GENDER = (typeof(ri.DERIVED_GENDER))'' => 0, le.DERIVED_GENDER = (typeof(ri.DERIVED_GENDER))'' => -1, ri.DERIVED_GENDER = le.DERIVED_GENDER => 2,-2);
    SELF.Match_LIC_NBR := MAP ( ri.LIC_NBR = (typeof(ri.LIC_NBR))'' => 0, le.LIC_NBR = (typeof(ri.LIC_NBR))'' => -1, ri.LIC_NBR = le.LIC_NBR => 2, SALT27.WithinEditN(le.LIC_NBR,ri.LIC_NBR,2) => 1,-2);
    ...
    SELF.Match_UPIN := MAP ( ri.UPIN = (typeof(ri.UPIN))'' => 0, le.UPIN = (typeof(ri.UPIN))'' => -1, ri.UPIN = le.UPIN => 2, SALT27.WithinEditN(le.UPIN,ri.UPIN,1) => 1,-2);
    SELF.Match_DEA_NUMBER := MAP ( ri.DEA_NUMBER = (typeof(ri.DEA_NUMBER))'' => 0, le.DEA_NUMBER = (typeof(ri.DEA_NUMBER))'' => -1, ri.DEA_NUMBER = le.DEA_NUMBER => 2, SALT27.WithinEditN(le.DEA_NUMBER,ri.DEA_NUMBER,1) => 1,-2);
    ...
    SELF.Match_NPI_NUMBER := MAP ( ri.NPI_NUMBER = (typeof(ri.NPI_NUMBER))'' => 0, le.NPI_NUMBER = (typeof(ri.NPI_NUMBER))'' => -1, ri.NPI_NUMBER = le.NPI_NUMBER => 2, SALT27.WithinEditN(le.NPI_NUMBER,ri.NPI_NUMBER,1) => 1,-2);
    ...
    SELF.Match_FULLNAME := MAP ( ri.FULLNAME = (typeof(ri.FULLNAME))'' => 0, le_FULLNAME = (typeof(ri.FULLNAME))'' => -1, ri_FULLNAME = le_FULLNAME => 2,SALT27.MatchBagOfWords(le_FULLNAME,ri_FULLNAME,0,1,0,0,0) > -300 => 1, -2);
    ri_FULLNAME_DOB := SALT27.Fn_WordBag_AppendSpecs_Fake((SALT27.StrType)ri.FULLNAME_DOB);//For later scoring
    le_FULLNAME_DOB := SALT27.Fn_WordBag_AppendSpecs_Fake(TRIM((SALT27.StrType)le.FNAME) + ' ' + TRIM((SALT27.StrType)le.MNAME) + ' ' + TRIM((SALT27.StrType)le.LNAME) + ' ' + TRIM((SALT27.StrType)le.SNAME) + ' ' + (STRING)(((UNSIGNED)le.DOB DIV 100) % 100)+'/'+(STRING)((UNSIGNED)le.DOB % 100)+'/'+(STRING)((UNSIGNED)le.DOB DIV 10000));//For later scoring
    SELF.Match_FULLNAME_DOB := MAP ( ri.FULLNAME_DOB = (typeof(ri.FULLNAME_DOB))'' => 0, le_FULLNAME_DOB = (typeof(ri.FULLNAME_DOB))'' => -1, ri_FULLNAME_DOB = le_FULLNAME_DOB => 2,SALT27.MatchBagOfWords(le_FULLNAME_DOB,ri_FULLNAME_DOB,0,1,0,0,0) > 1700 => 1, -2);
    ...
    SELF.Record_Score := SELF.Match_DID + SELF.Match_DOB + SELF.Match_PHONE + SELF.Match_SNAME + SELF.Match_FNAME + SELF.Match_MNAME + SELF.Match_LNAME + SELF.Match_GENDER + SELF.Match_DERIVED_GENDER + SELF.Match_LIC_NBR + SELF.Match_ADDRESS_ID + SELF.Match_PRIM_NAME + SELF.Match_PRIM_RANGE + SELF.Match_SEC_RANGE + SELF.Match_V_CITY_NAME + SELF.Match_ST + SELF.Match_ZIP + SELF.Match_CNAME + SELF.Match_TAX_ID + SELF.Match_UPIN + SELF.Match_DEA_NUMBER + SELF.Match_VENDOR_ID + SELF.Match_NPI_NUMBER + SELF.Match_DT_FIRST_SEEN + SELF.Match_DT_LAST_SEEN + SELF.Match_DT_LIC_EXPIRATION + SELF.Match_DT_DEA_EXPIRATION + SELF.Match_MAINNAME + SELF.Match_FULLNAME + SELF.Match_FULLNAME_DOB + SELF.Match_ADDR + SELF.Match_LOCALE + SELF.Match_ADDRESS + SELF.Match_LIC_STATE + SELF.Match_SRC;
    SELF.Is_FullMatch := SELF.Match_DID>=0 AND SELF.Match_DOB>=0 AND SELF.Match_PHONE>=0 AND SELF.Match_SNAME>=0 AND SELF.Match_FNAME>=0 AND SELF.Match_MNAME>=0 AND SELF.Match_LNAME>=0 AND SELF.Match_GENDER>=0 AND SELF.Match_DERIVED_GENDER>=0 AND SELF.Match_LIC_NBR>=0 AND SELF.Match_ADDRESS_ID>=0 AND SELF.Match_PRIM_NAME>=0 AND SELF.Match_PRIM_RANGE>=0 AND SELF.Match_SEC_RANGE>=0 AND SELF.Match_V_CITY_NAME>=0 AND SELF.Match_ST>=0 AND SELF.Match_ZIP>=0 AND SELF.Match_CNAME>=0 AND SELF.Match_TAX_ID>=0 AND SELF.Match_UPIN>=0 AND SELF.Match_DEA_NUMBER>=0 AND SELF.Match_VENDOR_ID>=0 AND SELF.Match_NPI_NUMBER>=0 AND SELF.Match_DT_FIRST_SEEN>=0 AND SELF.Match_DT_LAST_SEEN>=0 AND SELF.Match_DT_LIC_EXPIRATION>=0 AND SELF.Match_DT_DEA_EXPIRATION>=0 AND SELF.Match_MAINNAME>=0 AND SELF.Match_FULLNAME>=0 AND SELF.Match_FULLNAME_DOB>=0 AND SELF.Match_ADDR>=0 AND SELF.Match_LOCALE>=0 AND SELF.Match_ADDRESS>=0 AND SELF.Match_LIC_STATE>=0 AND SELF.Match_SRC>=0;

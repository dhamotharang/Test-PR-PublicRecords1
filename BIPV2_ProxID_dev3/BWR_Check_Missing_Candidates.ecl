EXPORT BWR_Check_Missing_Candidates := 'todo';output(BIPV2_ProxID_dev3.Files().outfile.qa(dotid in [1786858,4063037]));
...
BIPV2_ProxID_dev3.match_candidates(BFile).layout_attribute_matches ainto(BIPV2_ProxID_dev3.Keys(BFile).Attribute_Matches le) := TRANSFORM
SELF := le;
END;
am := PROJECT(BIPV2_ProxID_dev3.Keys(BFile).Attribute_Matches(Proxid1=Proxidone,Proxid2=Proxidtwo)+BIPV2_ProxID_dev3.Keys(BFile).Attribute_Matches(Proxid1=Proxidtwo,Proxid2=Proxidone),ainto(LEFT));
mtch := BIPV2_ProxID_dev3.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,Proxidone,Proxidtwo,0,0}],BIPV2_ProxID_dev3.match_candidates(BFile).layout_matches),am);
...
dit10 				:= BIPV2_ProxID_dev3.Files().outfile.qa;
keycandidates := BIPV2_ProxID_dev3.Keys(BFile).Candidates;
...
BIPV2_ProxID_dev3.output_test_cases(dProxidsInBoth);

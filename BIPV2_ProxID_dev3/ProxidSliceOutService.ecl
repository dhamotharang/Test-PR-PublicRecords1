...
BIPV2_ProxID_dev3.match_candidates(BFile).layout_candidates into(BIPV2_ProxID_dev3.Keys(BFile).Candidates le) := transform
  self := le;
end;
odl := project(choosen(BIPV2_ProxID_dev3.Keys(BFile).Candidates(Proxid=(unsigned)Proxidstr),10000),into(left));
odr := project(choosen(BIPV2_ProxID_dev3.Keys(BFile).Candidates(Proxid=(unsigned)Pref_Proxidstr),10000),into(left));
k := BIPV2_ProxID_dev3.Keys(BFile).Specificities_Key;
BIPV2_ProxID_dev3.Layout_Specificities.R s_into(k le) := transform
  self := le;
end;
s := global(project(k,s_into(left))[1]);
odl_noprop := BIPV2_ProxID_dev3.Debug(BFile,s).RemoveProps(odl); // Remove propogated values
odr_noprop := BIPV2_ProxID_dev3.Debug(BFile,s).RemoveProps(odr); // Remove propogated values
mtchnp := BIPV2_ProxID_dev3.Debug(BFile,s).AnnotateClusterMatches(odl_noprop,(unsigned)rcidstr);
mtchrnp := BIPV2_ProxID_dev3.Debug(BFile,s).AnnotateClusterMatches(odr_noprop+odl_noprop(rcid=(unsigned)rcidstr),(unsigned)rcidstr);
output( choosen(sort(mtchnp,-Conf),3),named('RecordMatches'));
output( choosen(sort(mtchrnp,-Conf),3),named('PreferredRecordMatches'));
mtch := BIPV2_ProxID_dev3.Debug(BFile,s).AnnotateClusterMatches(odl,(unsigned)rcidstr);
mtchr := BIPV2_ProxID_dev3.Debug(BFile,s).AnnotateClusterMatches(odr+odl(rcid=(unsigned)rcidstr),(unsigned)rcidstr);

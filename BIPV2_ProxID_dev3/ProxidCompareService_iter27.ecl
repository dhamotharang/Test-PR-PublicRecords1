...
IMPORT SALT24,BIPV2_ProxID_dev3,ut,tools,BIPV2;
...
odl := PROJECT(CHOOSEN(BIPV2_ProxID_dev3.Keys(BFile,both).Candidates(Proxid=Proxidone),100000),BIPV2_ProxID_dev3.match_candidates(BFile).layout_candidates);
odr := PROJECT(CHOOSEN(BIPV2_ProxID_dev3.Keys(BFile,both).Candidates(Proxid=ProxidTwo),100000),BIPV2_ProxID_dev3.match_candidates(BFile).layout_candidates);
k := BIPV2_ProxID_dev3.Keys(BFile,both).Specificities_Key;
...
mtch := BIPV2_ProxID_dev3.Debug(BFile,s).AnnotateMatchesFromData(odl+odr,DATASET([{0,0,0,0,Proxidone,Proxidtwo,0,0}],BIPV2_ProxID_dev3.match_candidates(BFile).layout_matches));

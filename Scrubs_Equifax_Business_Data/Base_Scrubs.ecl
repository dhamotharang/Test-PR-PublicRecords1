...
// The module to handle the case where no scrubs exist
  EXPORT  Expanded_Layout := RECORD(Base_Layout_Equifax_Business_Data)
    ...
    UNSIGNED1 EFX_SECSTAT_Invalid;
    UNSIGNED1 EFX_STATEC2_Invalid;
...
EXPORT FromNone(DATASET(Base_Layout_Equifax_Business_Data) h) := MODULE
  SHARED Expanded_Layout toExpanded(h le, BOOLEAN withOnfail) := TRANSFORM
    ...
    SELF.did_Invalid := Base_Fields.InValid_did((SALT37.StrType)le.did);
    SELF.did_score_Invalid := Base_Fields.InValid_did_score((SALT37.StrType)le.did_score);
    ...
    SELF.prim_name_Invalid := Base_Fields.InValid_prim_name((SALT37.StrType)le.prim_name);
    SELF.postdir_Invalid := Base_Fields.InValid_postdir((SALT37.StrType)le.postdir);
    SELF.p_city_name_Invalid := Base_Fields.InValid_p_city_name((SALT37.StrType)le.p_city_name);
    SELF.v_city_name_Invalid := Base_Fields.InValid_v_city_name((SALT37.StrType)le.v_city_name);
    SELF.st_Invalid := Base_Fields.InValid_st((SALT37.StrType)le.st,(SALT37.StrType)le.EFX_CTRYNAME);
    ...
    SELF.cart_Invalid := Base_Fields.InValid_cart((SALT37.StrType)le.cart);
    SELF.cr_sort_sz_Invalid := Base_Fields.InValid_cr_sort_sz((SALT37.StrType)le.cr_sort_sz);
    SELF.lot_Invalid := Base_Fields.InValid_lot((SALT37.StrType)le.lot);
    SELF.lot_order_Invalid := Base_Fields.InValid_lot_order((SALT37.StrType)le.lot_order);
    SELF.dbpc_Invalid := Base_Fields.InValid_dbpc((SALT37.StrType)le.dbpc);
    SELF.chk_digit_Invalid := Base_Fields.InValid_chk_digit((SALT37.StrType)le.chk_digit);
    ...
    SELF.fips_county_Invalid := Base_Fields.InValid_fips_county((SALT37.StrType)le.fips_county);
    ...
    SELF.msa_Invalid := Base_Fields.InValid_msa((SALT37.StrType)le.msa);
    SELF.geo_blk_Invalid := Base_Fields.InValid_geo_blk((SALT37.StrType)le.geo_blk);
    ...
    SELF.prep_addr_line1_Invalid := Base_Fields.InValid_prep_addr_line1((SALT37.StrType)le.prep_addr_line1);
    SELF.prep_addr_line_last_Invalid := Base_Fields.InValid_prep_addr_line_last((SALT37.StrType)le.prep_addr_line_last);
    ...
    SELF.EFX_STATEC_Invalid := Base_Fields.InValid_EFX_STATEC((SALT37.StrType)le.EFX_STATEC);
    ...
    SELF.EFX_STATE_Invalid := Base_Fields.InValid_EFX_STATE((SALT37.StrType)le.EFX_STATE,(SALT37.StrType)le.EFX_CTRYNAME);
    ...
    SELF.EFX_SECSTAT_Invalid := Base_Fields.InValid_EFX_SECSTAT((SALT37.StrType)le.EFX_SECSTAT,(SALT37.StrType)le.EFX_CTRYNAME);
    SELF.EFX_STATEC2_Invalid := Base_Fields.InValid_EFX_STATEC2((SALT37.StrType)le.EFX_STATEC2);
  ...
  Bitmap_Layout Into(ExpandedInfile le) := TRANSFORM
    SELF.ScrubsBits1 := ( le.dotid_Invalid << 0 ) + ( le.dotscore_Invalid << 1 ) + ( le.dotweight_Invalid << 2 ) + ( le.empid_Invalid << 3 ) + ( le.empscore_Invalid << 4 ) + ( le.empweight_Invalid << 5 ) + ( le.powid_Invalid << 6 ) + ( le.powscore_Invalid << 7 ) + ( le.powweight_Invalid << 8 ) + ( le.proxid_Invalid << 9 ) + ( le.proxscore_Invalid << 10 ) + ( le.proxweight_Invalid << 11 ) + ( le.seleid_Invalid << 12 ) + ( le.selescore_Invalid << 13 ) + ( le.seleweight_Invalid << 14 ) + ( le.orgid_Invalid << 15 ) + ( le.orgscore_Invalid << 16 ) + ( le.orgweight_Invalid << 17 ) + ( le.ultid_Invalid << 18 ) + ( le.ultscore_Invalid << 19 ) + ( le.ultweight_Invalid << 20 ) + ( le.did_Invalid << 21 ) + ( le.did_score_Invalid << 22 ) + ( le.dt_first_seen_Invalid << 23 ) + ( le.dt_last_seen_Invalid << 24 ) + ( le.dt_vendor_first_reported_Invalid << 25 ) + ( le.dt_vendor_last_reported_Invalid << 26 ) + ( le.process_date_Invalid << 27 ) + ( le.record_type_Invalid << 28 ) + ( le.normcompany_type_Invalid << 29 ) + ( le.normaddress_type_Invalid << 30 ) + ( le.clean_company_name_Invalid << 31 ) + ( le.clean_phone_Invalid << 32 ) + ( le.predir_Invalid << 33 ) + ( le.prim_name_Invalid << 34 ) + ( le.postdir_Invalid << 35 ) + ( le.p_city_name_Invalid << 36 ) + ( le.v_city_name_Invalid << 37 ) + ( le.st_Invalid << 38 ) + ( le.zip_Invalid << 39 ) + ( le.zip4_Invalid << 40 ) + ( le.cart_Invalid << 41 ) + ( le.cr_sort_sz_Invalid << 42 ) + ( le.lot_Invalid << 43 ) + ( le.lot_order_Invalid << 44 ) + ( le.dbpc_Invalid << 45 ) + ( le.chk_digit_Invalid << 46 ) + ( le.rec_type_Invalid << 47 ) + ( le.fips_state_Invalid << 48 ) + ( le.fips_county_Invalid << 49 ) + ( le.geo_lat_Invalid << 50 ) + ( le.geo_long_Invalid << 51 ) + ( le.msa_Invalid << 52 ) + ( le.geo_blk_Invalid << 53 ) + ( le.geo_match_Invalid << 54 ) + ( le.err_stat_Invalid << 55 ) + ( le.raw_aid_Invalid << 56 ) + ( le.ace_aid_Invalid << 57 ) + ( le.prep_addr_line1_Invalid << 58 ) + ( le.prep_addr_line_last_Invalid << 59 ) + ( le.EFX_BUSSTATCD_Invalid << 60 ) + ( le.EFX_CMSA_Invalid << 61 ) + ( le.EFX_CORPAMOUNTCD_Invalid << 62 ) + ( le.EFX_CORPAMOUNTPREC_Invalid << 63 );
    SELF.ScrubsBits2 := ( le.EFX_CORPAMOUNTTP_Invalid << 0 ) + ( le.EFX_CORPEMPCD_Invalid << 1 ) + ( le.EFX_CTRYISOCD_Invalid << 2 ) + ( le.EFX_CTRYNUM_Invalid << 3 ) + ( le.EFX_CTRYTELCD_Invalid << 4 ) + ( le.EFX_GEOPREC_Invalid << 5 ) + ( le.EFX_MERCTYPE_Invalid << 6 ) + ( le.EFX_MRKT_TELESCORE_Invalid << 7 ) + ( le.EFX_MRKT_TOTALIND_Invalid << 8 ) + ( le.EFX_MRKT_TOTALSCORE_Invalid << 9 ) + ( le.EFX_PUBLIC_Invalid << 10 ) + ( le.EFX_STATEC_Invalid << 11 ) + ( le.EFX_STKEXC_Invalid << 12 ) + ( le.EFX_PRIMSIC_Invalid << 13 ) + ( le.EFX_SECSIC1_Invalid << 14 ) + ( le.EFX_SECSIC2_Invalid << 15 ) + ( le.EFX_SECSIC3_Invalid << 16 ) + ( le.EFX_SECSIC4_Invalid << 17 ) + ( le.EFX_STATE_Invalid << 18 ) + ( le.EFX_ID_Invalid << 19 ) + ( le.EFX_NAME_Invalid << 20 ) + ( le.EFX_LEGAL_NAME_Invalid << 21 ) + ( le.EFX_ADDRESS_Invalid << 22 ) + ( le.EFX_CITY_Invalid << 23 ) + ( le.EFX_CTRYNAME_Invalid << 24 ) + ( le.EFX_SOHO_Invalid << 25 ) + ( le.EFX_BIZ_Invalid << 26 ) + ( le.EFX_RES_Invalid << 27 ) + ( le.EFX_CMRA_Invalid << 28 ) + ( le.EFX_SECSTAT_Invalid << 29 ) + ( le.EFX_STATEC2_Invalid << 30 ) + ( le.EFX_SECGEOPREC_Invalid << 31 ) + ( le.EFX_SECCTRYISOCD_Invalid << 32 ) + ( le.EFX_SECCTRYNUM_Invalid << 33 ) + ( le.EFX_PHONE_Invalid << 34 ) + ( le.EFX_FAXPHONE_Invalid << 35 ) + ( le.EFX_YREST_Invalid << 36 ) + ( le.EFX_CORPEMPCNT_Invalid << 37 ) + ( le.EFX_LOCEMPCNT_Invalid << 38 ) + ( le.EFX_LOCEMPCD_Invalid << 39 ) + ( le.EFX_CORPAMOUNT_Invalid << 40 ) + ( le.EFX_LOCAMOUNT_Invalid << 41 ) + ( le.EFX_LOCAMOUNTCD_Invalid << 42 ) + ( le.EFX_LOCAMOUNTTP_Invalid << 43 ) + ( le.EFX_LOCAMOUNTPREC_Invalid << 44 ) + ( le.EFX_PRIMNAICSCODE_Invalid << 45 ) + ( le.EFX_SECNAICS1_Invalid << 46 ) + ( le.EFX_SECNAICS2_Invalid << 47 ) + ( le.EFX_SECNAICS3_Invalid << 48 ) + ( le.EFX_SECNAICS4_Invalid << 49 ) + ( le.EFX_DEAD_Invalid << 50 ) + ( le.EFX_DEADDT_Invalid << 51 ) + ( le.EFX_MRKT_TELEVER_Invalid << 52 ) + ( le.EFX_MRKT_VACANT_Invalid << 53 ) + ( le.EFX_MRKT_SEASONAL_Invalid << 54 ) + ( le.EFX_MBE_Invalid << 55 ) + ( le.EFX_WBE_Invalid << 56 ) + ( le.EFX_MWBE_Invalid << 57 ) + ( le.EFX_SDB_Invalid << 58 ) + ( le.EFX_HUBZONE_Invalid << 59 ) + ( le.EFX_DBE_Invalid << 60 ) + ( le.EFX_VET_Invalid << 61 ) + ( le.EFX_DVET_Invalid << 62 ) + ( le.EFX_8a_Invalid << 63 );
    SELF.ScrubsBits3 := ( le.EFX_8aEXPDT_Invalid << 0 ) + ( le.EFX_DIS_Invalid << 1 ) + ( le.EFX_SBE_Invalid << 2 ) + ( le.EFX_BUSSIZE_Invalid << 3 ) + ( le.EFX_LBE_Invalid << 4 ) + ( le.EFX_GOV_Invalid << 5 ) + ( le.EFX_FGOV_Invalid << 6 ) + ( le.EFX_NONPROFIT_Invalid << 7 ) + ( le.EFX_HBCU_Invalid << 8 ) + ( le.EFX_GAYLESBIAN_Invalid << 9 ) + ( le.EFX_WSBE_Invalid << 10 ) + ( le.EFX_VSBE_Invalid << 11 ) + ( le.EFX_DVSBE_Invalid << 12 ) + ( le.EFX_MWBESTATUS_Invalid << 13 ) + ( le.EFX_NMSDC_Invalid << 14 ) + ( le.EFX_WBENC_Invalid << 15 ) + ( le.EFX_CA_PUC_Invalid << 16 ) + ( le.EFX_TX_HUB_Invalid << 17 ) + ( le.EFX_GSAX_Invalid << 18 ) + ( le.EFX_CALTRANS_Invalid << 19 ) + ( le.EFX_EDU_Invalid << 20 ) + ( le.EFX_MI_Invalid << 21 ) + ( le.EFX_ANC_Invalid << 22 ) + ( le.AT_CERTLEV1_Invalid << 23 ) + ( le.AT_CERTLEV2_Invalid << 24 ) + ( le.AT_CERTLEV3_Invalid << 25 ) + ( le.AT_CERTLEV4_Invalid << 26 ) + ( le.AT_CERTLEV5_Invalid << 27 ) + ( le.AT_CERTLEV6_Invalid << 28 ) + ( le.AT_CERTLEV7_Invalid << 29 ) + ( le.AT_CERTLEV8_Invalid << 30 ) + ( le.AT_CERTLEV9_Invalid << 31 ) + ( le.AT_CERTLEV10_Invalid << 32 ) + ( le.AT_CERTEXP1_Invalid << 33 ) + ( le.AT_CERTEXP2_Invalid << 34 ) + ( le.AT_CERTEXP3_Invalid << 35 ) + ( le.AT_CERTEXP4_Invalid << 36 ) + ( le.AT_CERTEXP5_Invalid << 37 ) + ( le.AT_CERTEXP6_Invalid << 38 ) + ( le.AT_CERTEXP7_Invalid << 39 ) + ( le.AT_CERTEXP8_Invalid << 40 ) + ( le.AT_CERTEXP9_Invalid << 41 ) + ( le.AT_CERTEXP10_Invalid << 42 ) + ( le.EFX_EXTRACT_DATE_Invalid << 43 ) + ( le.EFX_MERCHANT_ID_Invalid << 44 ) + ( le.EFX_PROJECT_ID_Invalid << 45 ) + ( le.EFX_FOREIGN_Invalid << 46 ) + ( le.Record_Update_Refresh_Date_Invalid << 47 ) + ( le.EFX_DATE_CREATED_Invalid << 48 );
    SELF := le;
  END;
...
EXPORT FromBits(DATASET(Bitmap_Layout) h) := MODULE
  ...
  Expanded_Layout into(h le) := TRANSFORM
    ...
    SELF.did_Invalid := (le.ScrubsBits1 >> 21) & 1;
    SELF.did_score_Invalid := (le.ScrubsBits1 >> 22) & 1;
    SELF.dt_first_seen_Invalid := (le.ScrubsBits1 >> 23) & 1;
    SELF.dt_last_seen_Invalid := (le.ScrubsBits1 >> 24) & 1;
    SELF.dt_vendor_first_reported_Invalid := (le.ScrubsBits1 >> 25) & 1;
    SELF.dt_vendor_last_reported_Invalid := (le.ScrubsBits1 >> 26) & 1;
    SELF.process_date_Invalid := (le.ScrubsBits1 >> 27) & 1;
    SELF.record_type_Invalid := (le.ScrubsBits1 >> 28) & 1;
    SELF.normcompany_type_Invalid := (le.ScrubsBits1 >> 29) & 1;
    SELF.normaddress_type_Invalid := (le.ScrubsBits1 >> 30) & 1;
    SELF.clean_company_name_Invalid := (le.ScrubsBits1 >> 31) & 1;
    SELF.clean_phone_Invalid := (le.ScrubsBits1 >> 32) & 1;
    SELF.predir_Invalid := (le.ScrubsBits1 >> 33) & 1;
    SELF.prim_name_Invalid := (le.ScrubsBits1 >> 34) & 1;
    SELF.postdir_Invalid := (le.ScrubsBits1 >> 35) & 1;
    ...
    SELF.zip_Invalid := (le.ScrubsBits1 >> 39) & 1;
    SELF.zip4_Invalid := (le.ScrubsBits1 >> 40) & 1;
    SELF.cart_Invalid := (le.ScrubsBits1 >> 41) & 1;
    SELF.cr_sort_sz_Invalid := (le.ScrubsBits1 >> 42) & 1;
    SELF.lot_Invalid := (le.ScrubsBits1 >> 43) & 1;
    SELF.lot_order_Invalid := (le.ScrubsBits1 >> 44) & 1;
    SELF.dbpc_Invalid := (le.ScrubsBits1 >> 45) & 1;
    SELF.chk_digit_Invalid := (le.ScrubsBits1 >> 46) & 1;
    SELF.rec_type_Invalid := (le.ScrubsBits1 >> 47) & 1;
    SELF.fips_state_Invalid := (le.ScrubsBits1 >> 48) & 1;
    SELF.fips_county_Invalid := (le.ScrubsBits1 >> 49) & 1;
    SELF.geo_lat_Invalid := (le.ScrubsBits1 >> 50) & 1;
    SELF.geo_long_Invalid := (le.ScrubsBits1 >> 51) & 1;
    SELF.msa_Invalid := (le.ScrubsBits1 >> 52) & 1;
    SELF.geo_blk_Invalid := (le.ScrubsBits1 >> 53) & 1;
    SELF.geo_match_Invalid := (le.ScrubsBits1 >> 54) & 1;
    SELF.err_stat_Invalid := (le.ScrubsBits1 >> 55) & 1;
    SELF.raw_aid_Invalid := (le.ScrubsBits1 >> 56) & 1;
    SELF.ace_aid_Invalid := (le.ScrubsBits1 >> 57) & 1;
    SELF.prep_addr_line1_Invalid := (le.ScrubsBits1 >> 58) & 1;
    SELF.prep_addr_line_last_Invalid := (le.ScrubsBits1 >> 59) & 1;
    SELF.EFX_BUSSTATCD_Invalid := (le.ScrubsBits1 >> 60) & 1;
    SELF.EFX_CMSA_Invalid := (le.ScrubsBits1 >> 61) & 1;
    SELF.EFX_CORPAMOUNTCD_Invalid := (le.ScrubsBits1 >> 62) & 1;
    SELF.EFX_CORPAMOUNTPREC_Invalid := (le.ScrubsBits1 >> 63) & 1;
    SELF.EFX_CORPAMOUNTTP_Invalid := (le.ScrubsBits2 >> 0) & 1;
    SELF.EFX_CORPEMPCD_Invalid := (le.ScrubsBits2 >> 1) & 1;
    SELF.EFX_CTRYISOCD_Invalid := (le.ScrubsBits2 >> 2) & 1;
    SELF.EFX_CTRYNUM_Invalid := (le.ScrubsBits2 >> 3) & 1;
    SELF.EFX_CTRYTELCD_Invalid := (le.ScrubsBits2 >> 4) & 1;
    SELF.EFX_GEOPREC_Invalid := (le.ScrubsBits2 >> 5) & 1;
    SELF.EFX_MERCTYPE_Invalid := (le.ScrubsBits2 >> 6) & 1;
    SELF.EFX_MRKT_TELESCORE_Invalid := (le.ScrubsBits2 >> 7) & 1;
    SELF.EFX_MRKT_TOTALIND_Invalid := (le.ScrubsBits2 >> 8) & 1;
    SELF.EFX_MRKT_TOTALSCORE_Invalid := (le.ScrubsBits2 >> 9) & 1;
    SELF.EFX_PUBLIC_Invalid := (le.ScrubsBits2 >> 10) & 1;
    SELF.EFX_STATEC_Invalid := (le.ScrubsBits2 >> 11) & 1;
    SELF.EFX_STKEXC_Invalid := (le.ScrubsBits2 >> 12) & 1;
    SELF.EFX_PRIMSIC_Invalid := (le.ScrubsBits2 >> 13) & 1;
    SELF.EFX_SECSIC1_Invalid := (le.ScrubsBits2 >> 14) & 1;
    SELF.EFX_SECSIC2_Invalid := (le.ScrubsBits2 >> 15) & 1;
    SELF.EFX_SECSIC3_Invalid := (le.ScrubsBits2 >> 16) & 1;
    SELF.EFX_SECSIC4_Invalid := (le.ScrubsBits2 >> 17) & 1;
    SELF.EFX_STATE_Invalid := (le.ScrubsBits2 >> 18) & 1;
    SELF.EFX_ID_Invalid := (le.ScrubsBits2 >> 19) & 1;
    SELF.EFX_NAME_Invalid := (le.ScrubsBits2 >> 20) & 1;
    SELF.EFX_LEGAL_NAME_Invalid := (le.ScrubsBits2 >> 21) & 1;
    SELF.EFX_ADDRESS_Invalid := (le.ScrubsBits2 >> 22) & 1;
    SELF.EFX_CITY_Invalid := (le.ScrubsBits2 >> 23) & 1;
    SELF.EFX_CTRYNAME_Invalid := (le.ScrubsBits2 >> 24) & 1;
    SELF.EFX_SOHO_Invalid := (le.ScrubsBits2 >> 25) & 1;
    SELF.EFX_BIZ_Invalid := (le.ScrubsBits2 >> 26) & 1;
    SELF.EFX_RES_Invalid := (le.ScrubsBits2 >> 27) & 1;
    SELF.EFX_CMRA_Invalid := (le.ScrubsBits2 >> 28) & 1;
    SELF.EFX_SECSTAT_Invalid := (le.ScrubsBits2 >> 29) & 1;
    SELF.EFX_STATEC2_Invalid := (le.ScrubsBits2 >> 30) & 1;
    SELF.EFX_SECGEOPREC_Invalid := (le.ScrubsBits2 >> 31) & 1;
    SELF.EFX_SECCTRYISOCD_Invalid := (le.ScrubsBits2 >> 32) & 1;
    SELF.EFX_SECCTRYNUM_Invalid := (le.ScrubsBits2 >> 33) & 1;
    SELF.EFX_PHONE_Invalid := (le.ScrubsBits2 >> 34) & 1;
    SELF.EFX_FAXPHONE_Invalid := (le.ScrubsBits2 >> 35) & 1;
    SELF.EFX_YREST_Invalid := (le.ScrubsBits2 >> 36) & 1;
    SELF.EFX_CORPEMPCNT_Invalid := (le.ScrubsBits2 >> 37) & 1;
    SELF.EFX_LOCEMPCNT_Invalid := (le.ScrubsBits2 >> 38) & 1;
    SELF.EFX_LOCEMPCD_Invalid := (le.ScrubsBits2 >> 39) & 1;
    SELF.EFX_CORPAMOUNT_Invalid := (le.ScrubsBits2 >> 40) & 1;
    SELF.EFX_LOCAMOUNT_Invalid := (le.ScrubsBits2 >> 41) & 1;
    SELF.EFX_LOCAMOUNTCD_Invalid := (le.ScrubsBits2 >> 42) & 1;
    SELF.EFX_LOCAMOUNTTP_Invalid := (le.ScrubsBits2 >> 43) & 1;
    SELF.EFX_LOCAMOUNTPREC_Invalid := (le.ScrubsBits2 >> 44) & 1;
    SELF.EFX_PRIMNAICSCODE_Invalid := (le.ScrubsBits2 >> 45) & 1;
    SELF.EFX_SECNAICS1_Invalid := (le.ScrubsBits2 >> 46) & 1;
    SELF.EFX_SECNAICS2_Invalid := (le.ScrubsBits2 >> 47) & 1;
    SELF.EFX_SECNAICS3_Invalid := (le.ScrubsBits2 >> 48) & 1;
    SELF.EFX_SECNAICS4_Invalid := (le.ScrubsBits2 >> 49) & 1;
    SELF.EFX_DEAD_Invalid := (le.ScrubsBits2 >> 50) & 1;
    SELF.EFX_DEADDT_Invalid := (le.ScrubsBits2 >> 51) & 1;
    SELF.EFX_MRKT_TELEVER_Invalid := (le.ScrubsBits2 >> 52) & 1;
    SELF.EFX_MRKT_VACANT_Invalid := (le.ScrubsBits2 >> 53) & 1;
    SELF.EFX_MRKT_SEASONAL_Invalid := (le.ScrubsBits2 >> 54) & 1;
    SELF.EFX_MBE_Invalid := (le.ScrubsBits2 >> 55) & 1;
    SELF.EFX_WBE_Invalid := (le.ScrubsBits2 >> 56) & 1;
    SELF.EFX_MWBE_Invalid := (le.ScrubsBits2 >> 57) & 1;
    SELF.EFX_SDB_Invalid := (le.ScrubsBits2 >> 58) & 1;
    SELF.EFX_HUBZONE_Invalid := (le.ScrubsBits2 >> 59) & 1;
    SELF.EFX_DBE_Invalid := (le.ScrubsBits2 >> 60) & 1;
    SELF.EFX_VET_Invalid := (le.ScrubsBits2 >> 61) & 1;
    SELF.EFX_DVET_Invalid := (le.ScrubsBits2 >> 62) & 1;
    SELF.EFX_8a_Invalid := (le.ScrubsBits2 >> 63) & 1;
    SELF.EFX_8aEXPDT_Invalid := (le.ScrubsBits3 >> 0) & 1;
    SELF.EFX_DIS_Invalid := (le.ScrubsBits3 >> 1) & 1;
    SELF.EFX_SBE_Invalid := (le.ScrubsBits3 >> 2) & 1;
    SELF.EFX_BUSSIZE_Invalid := (le.ScrubsBits3 >> 3) & 1;
    SELF.EFX_LBE_Invalid := (le.ScrubsBits3 >> 4) & 1;
    SELF.EFX_GOV_Invalid := (le.ScrubsBits3 >> 5) & 1;
    SELF.EFX_FGOV_Invalid := (le.ScrubsBits3 >> 6) & 1;
    SELF.EFX_NONPROFIT_Invalid := (le.ScrubsBits3 >> 7) & 1;
    SELF.EFX_HBCU_Invalid := (le.ScrubsBits3 >> 8) & 1;
    SELF.EFX_GAYLESBIAN_Invalid := (le.ScrubsBits3 >> 9) & 1;
    SELF.EFX_WSBE_Invalid := (le.ScrubsBits3 >> 10) & 1;
    SELF.EFX_VSBE_Invalid := (le.ScrubsBits3 >> 11) & 1;
    SELF.EFX_DVSBE_Invalid := (le.ScrubsBits3 >> 12) & 1;
    SELF.EFX_MWBESTATUS_Invalid := (le.ScrubsBits3 >> 13) & 1;
    SELF.EFX_NMSDC_Invalid := (le.ScrubsBits3 >> 14) & 1;
    SELF.EFX_WBENC_Invalid := (le.ScrubsBits3 >> 15) & 1;
    SELF.EFX_CA_PUC_Invalid := (le.ScrubsBits3 >> 16) & 1;
    SELF.EFX_TX_HUB_Invalid := (le.ScrubsBits3 >> 17) & 1;
    SELF.EFX_GSAX_Invalid := (le.ScrubsBits3 >> 18) & 1;
    SELF.EFX_CALTRANS_Invalid := (le.ScrubsBits3 >> 19) & 1;
    SELF.EFX_EDU_Invalid := (le.ScrubsBits3 >> 20) & 1;
    SELF.EFX_MI_Invalid := (le.ScrubsBits3 >> 21) & 1;
    SELF.EFX_ANC_Invalid := (le.ScrubsBits3 >> 22) & 1;
    SELF.AT_CERTLEV1_Invalid := (le.ScrubsBits3 >> 23) & 1;
    SELF.AT_CERTLEV2_Invalid := (le.ScrubsBits3 >> 24) & 1;
    SELF.AT_CERTLEV3_Invalid := (le.ScrubsBits3 >> 25) & 1;
    SELF.AT_CERTLEV4_Invalid := (le.ScrubsBits3 >> 26) & 1;
    SELF.AT_CERTLEV5_Invalid := (le.ScrubsBits3 >> 27) & 1;
    SELF.AT_CERTLEV6_Invalid := (le.ScrubsBits3 >> 28) & 1;
    SELF.AT_CERTLEV7_Invalid := (le.ScrubsBits3 >> 29) & 1;
    SELF.AT_CERTLEV8_Invalid := (le.ScrubsBits3 >> 30) & 1;
    SELF.AT_CERTLEV9_Invalid := (le.ScrubsBits3 >> 31) & 1;
    SELF.AT_CERTLEV10_Invalid := (le.ScrubsBits3 >> 32) & 1;
    SELF.AT_CERTEXP1_Invalid := (le.ScrubsBits3 >> 33) & 1;
    SELF.AT_CERTEXP2_Invalid := (le.ScrubsBits3 >> 34) & 1;
    SELF.AT_CERTEXP3_Invalid := (le.ScrubsBits3 >> 35) & 1;
    SELF.AT_CERTEXP4_Invalid := (le.ScrubsBits3 >> 36) & 1;
    SELF.AT_CERTEXP5_Invalid := (le.ScrubsBits3 >> 37) & 1;
    SELF.AT_CERTEXP6_Invalid := (le.ScrubsBits3 >> 38) & 1;
    SELF.AT_CERTEXP7_Invalid := (le.ScrubsBits3 >> 39) & 1;
    SELF.AT_CERTEXP8_Invalid := (le.ScrubsBits3 >> 40) & 1;
    SELF.AT_CERTEXP9_Invalid := (le.ScrubsBits3 >> 41) & 1;
    SELF.AT_CERTEXP10_Invalid := (le.ScrubsBits3 >> 42) & 1;
    SELF.EFX_EXTRACT_DATE_Invalid := (le.ScrubsBits3 >> 43) & 1;
    SELF.EFX_MERCHANT_ID_Invalid := (le.ScrubsBits3 >> 44) & 1;
    SELF.EFX_PROJECT_ID_Invalid := (le.ScrubsBits3 >> 45) & 1;
    SELF.EFX_FOREIGN_Invalid := (le.ScrubsBits3 >> 46) & 1;
    SELF.Record_Update_Refresh_Date_Invalid := (le.ScrubsBits3 >> 47) & 1;
    SELF.EFX_DATE_CREATED_Invalid := (le.ScrubsBits3 >> 48) & 1;
    SELF := le;
  END;
...
EXPORT FromExpanded(DATASET(Expanded_Layout) h) := MODULE
  r := RECORD
    ...
    EFX_STATE_CUSTOM_ErrorCount := COUNT(GROUP,h.EFX_STATE_Invalid=1);
    ...
    EFX_SECSTAT_CUSTOM_ErrorCount := COUNT(GROUP,h.EFX_SECSTAT_Invalid=1);
    EFX_STATEC2_CUSTOM_ErrorCount := COUNT(GROUP,h.EFX_STATEC2_Invalid=1);
  ...
  r into(h le,UNSIGNED c) := TRANSFORM
    ...
    SELF.ErrorType := IF ( ErrNum = 0, SKIP, CHOOSE(c
          ...
          ,CHOOSE(le.EFX_STATE_Invalid,'CUSTOM','UNKNOWN')
          ...
          ,CHOOSE(le.EFX_SECSTAT_Invalid,'CUSTOM','UNKNOWN')
          ,CHOOSE(le.EFX_STATEC2_Invalid,'CUSTOM','UNKNOWN')
  ...
  EXPORT OrbitStats(UNSIGNED examples = 10,UNSIGNED Pdate=(UNSIGNED)StringLib.getdateYYYYMMDD(),STRING10 Src='UNK') := FUNCTION
    SALT37.ScrubsOrbitLayout Into(SummaryStats le, UNSIGNED c) := TRANSFORM
      ...
      SELF.ruledesc := CHOOSE(c
          ...
          ,'dt_first_seen:invalid_date_seen:CUSTOM'
          ,'dt_last_seen:invalid_date_seen:CUSTOM'
          ,'dt_vendor_first_reported:invalid_vendor_reported_date:CUSTOM'
          ,'dt_vendor_last_reported:invalid_vendor_reported_date:CUSTOM'
          ,'process_date:invalid_process_date:CUSTOM'
          ...
          ,'EFX_SECSTAT:invalid_st:CUSTOM'
          ,'EFX_STATEC2:invalid_statec:CUSTOM'
      ...
      SELF.ErrorMessage := CHOOSE(c
          ...
          ,Base_Fields.InvalidMessage_EFX_STATEC(1)
          ...
          ,Base_Fields.InvalidMessage_EFX_STATE(1)
          ...
          ,Base_Fields.InvalidMessage_EFX_SECSTAT(1)
          ,Base_Fields.InvalidMessage_EFX_STATEC2(1)
      ...
      SELF.rulecnt := CHOOSE(c
          ...
          ,le.EFX_STATE_CUSTOM_ErrorCount
          ...
          ,le.EFX_SECSTAT_CUSTOM_ErrorCount
          ,le.EFX_STATEC2_CUSTOM_ErrorCount
    ...
    END;
    SummaryInfo := NORMALIZE(SummaryStats,177,Into(LEFT,COUNTER));
    orb_r := RECORD
      AllErrors.Src;
